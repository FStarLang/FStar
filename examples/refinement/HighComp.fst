...
// for combinator
let rec for (#wp : int -> hwp_mon unit)
            (lo : int)
            (hi : int{lo <= hi})
            (f : (i:int) -> HIGH unit (wp i))
    : HIGH unit (wp_for wp lo hi) (decreases (hi - lo))
    = if lo = hi then ()
...
let bind_elab #a #b #f_w ($f:_) #g_w ($g:_) : comp_wp b (bind_wp f_w g_w) =
  HIGH?.bind_elab a b f_w f g_w g
