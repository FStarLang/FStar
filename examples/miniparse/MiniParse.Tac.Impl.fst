...
let rec gen_serializer_impl' (p: T.term) : T.Tac T.term =
  let (hd, tl) = app_head_tail p in
  if hd `T.term_eq` (`(serialize_empty)) then T.mk_app (`(serialize_empty_impl)) tl else
  if hd `T.term_eq` (`(serialize_u8)) then (`(serialize_u8_impl)) else
  if hd `T.term_eq` (`(serialize_nondep_then)) then match tl with
  | [t1; (p1, _); (s1, _); t2; (p2, _); (s2, _)] ->
    let s1' = gen_serializer_impl' s1 in
    let s2' = gen_serializer_impl' s2 in
    T.mk_app (`(serialize_nondep_then_impl)) [
      t1;
      (p1, T.Q_Implicit);
      (s1, T.Q_Implicit);
      (s1', T.Q_Explicit);
  ...
  if hd `T.term_eq` (`(serialize_synth))
  then match tl with
  ...
  if hd `T.term_eq` (`serialize_bounded_u16)
  then match tl with
  | [(b, _)] ->
    T.mk_app (`serialize_bounded_u16_impl) [(b, T.Q_Explicit)]
  | _ -> tfail "not enough arguments to serialize_bounded_u16"
  else
  if hd `T.term_eq` (`serialize_filter)
  then match tl with
  | [(t, _); (p, _); (s, _); (f, _)] ->
    let s' = gen_serializer_impl' s in
    T.mk_app (`serialize_filter_impl) [
      (t, T.Q_Implicit);
      (p, T.Q_Implicit);
      (s, T.Q_Implicit);
      (s', T.Q_Explicit);
  ...
  | _ -> tfail "Not enough arguments to serialize_filter"
  else
  if hd `T.term_eq` (`(TS.package_serializer))
  then match tl with
  | [qt; (pk, _)] ->
    let (hd', tl') = app_head_tail pk in
    if hd' `T.term_eq` (`(TS.mk_package))
    then match tl' with
    | [_; _; (s, _)] ->
      gen_serializer_impl' s
    | _ -> tfail "Not enough arguments to mk_package"
    else begin match T.inspect hd with
    | T.Tv_FVar v ->
      let env = T.cur_env () in
      let t' = T.norm_term_env env [iota] (T.mk_app (unfold_term hd') tl') in
      gen_serializer_impl' (T.mk_app hd [qt; (t', T.Q_Explicit)])
  ...
  | T.Tv_FVar v ->
    let env = T.cur_env () in
    let t' = T.norm_term_env env [iota] (T.mk_app (unfold_term hd) tl) in
    gen_serializer_impl' t'
  | _ ->
    tfail "Unknown serializer combinator"

let gen_serializer_impl (pol: T.guard_policy) : T.Tac unit =
  let (hd, tl) = app_head_tail (T.cur_goal ()) in
  let hd1 = T.inspect hd in
  let hd2 = T.inspect (`serializer_impl) in
  match hd1, hd2, tl with
  | T.Tv_FVar lid1, T.Tv_FVar lid2, [_; _; (s, _)] ->
    if (let i1 = T.inspect_fv lid1 in let i2 = T.inspect_fv lid2 in i1 = i2)
    then begin
      let t = gen_serializer_impl' s in
      T.exact_guard t;
      according_to pol (fun () -> tconclude_with [
        synth_inverse_forall_bounded_u16_solve;
        synth_inverse_forall_tenum_solve;
      ]);
      T.print "gen_serializer_impl spits:";
      T.print (T.term_to_string t)
    end else
      tfail "Not a serializer_impl goal"
  | _ -> tfail "Not a serializer_impl goal"
