...
let swap (#a:Type) (ptr1 ptr2:B.pointer a)
  ...
  let x = frame_left #_ #(r_ptr ptr1) #(r_ptr ptr2) 
                     #(fun _ -> True) 
                     #(fun h0 x h1 -> sel (r_ptr ptr1) h0 == x /\ x == sel (r_ptr ptr1) h1) 
                     (fun _ -> r_ptr_read #_ #ptr1 ()) in
  let y = frame_right #_ #(r_ptr ptr1) #(r_ptr ptr2) 
                      #(fun _ -> True) 
                      #(fun h0 y h1 -> sel (r_ptr ptr2) h0 == y /\ y == sel (r_ptr ptr2) h1) 
                      (fun _ -> r_ptr_read #_ #ptr2 ()) in
  frame_left #_ #(r_ptr ptr1) #(r_ptr ptr2) 
             #(fun _ -> True) 
             #(fun _ _ h1 -> sel (r_ptr ptr1) h1 == y)
             (fun _ -> r_ptr_write #_ #ptr1 y);
  frame_right #_ #(r_ptr ptr1) #(r_ptr ptr2) 
              #(fun _ -> True) 
              #(fun _ _ h1 -> sel (r_ptr ptr2) h1 == x)
              (fun _ -> r_ptr_write #_ #ptr2 x)
...
let swap_incl (#a:Type) (ptr1 ptr2:B.pointer a)
  ...
  let x = r_include (star_includes_left (r_ptr ptr1) (r_ptr ptr2))
                    #(fun _ -> True)
                    #(fun h0 x h1 -> sel (r_ptr ptr1) h0 == x /\
                                     x == sel (r_ptr ptr1) h1)
                    (fun _ -> r_ptr_read #_ #ptr1 ()) in
  let y = r_include (star_includes_right (r_ptr ptr1) (r_ptr ptr2))
                    #(fun _ -> True)
                    #(fun h0 y h1 -> sel (r_ptr ptr2) h0 == y /\
                                     y == sel (r_ptr ptr2) h1)
                    (fun _ -> r_ptr_read #_ #ptr2 ()) in
