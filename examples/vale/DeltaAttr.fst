...
let test_4 (x:int) : int = synth_by_tactic
  (fun () -> normalize [delta_attr myattr; delta_only ["DeltaAttr.add"]] (add (sub_1 (add_1 x))))

// more than one delta_only specified
let test_5 (x:int) : int = synth_by_tactic
  (fun () -> normalize [delta_only ["DeltaAttr.add"];delta_only["DeltaAttr.add_1"];delta_only ["DeltaAttr.sub_1"]] (add (sub_1 (add_1 x))))
