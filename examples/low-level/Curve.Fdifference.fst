...
val fdifference_aux_1: a:bigint -> b:bigint{disjoint a b} -> ctr:u32{U32.v ctr < norm_length} -> STL unit
    (requires (fun h -> live h a /\ live h b /\ norm_length <= length a /\ norm_length <= length b
              /\ (forall (i:nat{ i >= U32.v ctr /\ i < norm_length}). v (getValue h b i) >= v (getValue h a i))))
    (ensures (fun h0 _ h1 -> 
      ...
      /\ (forall (i:nat).
	((i >= U32.v ctr+1 /\ i < norm_length) ==>  (v (getValue h1 b i) >= v (getValue h1 a i)
						/\ getValue h1 a i == getValue h0 a i))
	/\ (((i<U32.v ctr \/ i>=norm_length) /\ i<length a) ==> getValue h1 a i == getValue h0 a i))
      /\ v (getValue h1 a (U32.v ctr)) = v (getValue h0 b (U32.v ctr)) - v (getValue h0 a (U32.v ctr))))
...
let fdifference_aux_1 a b ctr =
  ...
  cut(b2t(v (getValue h0 b (U32.v i)) >= v (getValue h0 a (U32.v i)))); 
  let z = bi -^ ai in 
  upd a i z;
  let h1 = HST.get() in
...
val fdifference_aux_2_0: h0:heap -> h1:heap -> h2:heap -> a:bigint -> b:bigint{disjoint a b} -> 
  ctr:nat{ctr < norm_length} -> Lemma (requires (
	  ...
	  /\ (((i<ctr+1 \/ i >= norm_length) /\ i<length a) ==> (getValue h2 a i == getValue h1 a i)))
	  ...
	  /\ (((i<ctr \/ i >= norm_length) /\ i<length a) ==> getValue h1 a i == getValue h0 a i))
...
val fdifference_aux_2_1: h0:heap -> h1:heap -> h2:heap -> a:bigint -> b:bigint{disjoint a b} -> 
  ...
  Lemma 
    ...
    (ensures (
      ...
      /\ (modifies_1 a h0 h2)
      /\ (length a = length a)
      /\ (forall (i:nat). ((i<ctr \/ i >= norm_length) /\ i<length a) ==> (getValue h2 a i == getValue h0 a i))
    ))
...
val fdifference_aux_2: h0:heap -> h1:heap -> h2:heap -> a:bigint -> b:bigint{disjoint a b} -> 
  ctr:nat{ctr < norm_length} -> Lemma
    ...
    (ensures (
	  ...
	  ((i>= ctr /\ i<norm_length) ==> 
	    (v (getValue h2 a i) = v (getValue h0 b i) - v (getValue h0 a i)))
	  /\ (((i<ctr \/ i >= norm_length) /\ i<length a) ==> (getValue h2 a i == getValue h0 a i)))
      ))      
...
let subtraction_lemma_aux h0 h1 a b len =
  //@@
  eval_def h1 a len;
  assert(eval h1 a len = pow2 (bitweight (templ) (len-1)) * v (getValue h1 a (len-1)) + eval h1 a (len-1));
  assert(v (getValue h1 a (len-1)) = v (getValue h0 b (len-1)) - v (getValue h0 a (len-1))); 
  Math.Axioms.distributivity_sub_right (pow2 (bitweight (templ) (len-1))) (v (getValue h0 b (len-1)))  (v (getValue h0 a (len-1))); 
  assert(eval h1 a len = (pow2 (bitweight (templ) (len-1)) * v (getValue h0 b (len-1)) - pow2 (bitweight (templ) (len-1)) * v (getValue h0 a (len-1))) + eval h1 a (len-1));
  (* FdifferenceLemmas.helper_lemma_2  *)
  (* 		(pow2 (bitweight (templ) (len-1)) * v (getValue h0 b (len-1))) *)
  (* 		(pow2 (bitweight (templ) (len-1)) * v (getValue h0 a (len-1))) *)
  (* 		(eval h0 b (len-1)) *)
  (* 		(eval h0 a (len-1)); *)
  eval_def h0 a len;
  eval_def h0 b len
...
val subtraction_lemma:
  ...
  /\ (forall (i:nat). i < len ==> v (getValue h1 a i) = v (getValue h0 b i) - v (getValue h0 a i)) } -> Lemma
    (requires (True))
    (ensures ( eval h0 b len - eval h0 a len = eval h1 a len ))
...
val subtraction_max_value_lemma: h0:heap -> h1:heap -> a:bigint{live h0 a} -> 
  ...
  c:bigint{live h1 c /\ length c = length a
   /\ (forall (i:nat). (i<length c) ==> v (getValue h1 c i) = v (getValue h0 b i) - v (getValue h0 a i)) } ->
  Lemma
    (requires (True))
...
val max_value_lemma: h:heap -> a:bigint{live h a} -> m:nat ->Lemma 
    (requires (forall (n:nat). n < length a ==> v (getValue h a n) <= m))
...
let subtraction_max_value_lemma_extended h0 h1 a b c = 
  ...
  cut ( forall (i:nat). (i < length c /\ i >= norm_length)
	==> (v (getValue h1 c i) = v (getValue h0 a i) /\ v (getValue h1 c i) <= maxValue h0 a (length a)));
  cut ( forall (i:nat). (i < length c /\ i >= norm_length)
  	==> v (getValue h1 c i) <= max (maxValue h0 a (length a)) (maxValue h0 b (length b)));	
...
val auxiliary_lemma_2: ha:heap -> a:bigint{norm ha a} -> hb:heap -> b:bigint{filled hb b} -> 
  i:nat{i < norm_length} -> Lemma (v (getValue hb b i) - v (getValue ha a i) < pow2 (templ i + 1))
...
val auxiliary_lemma_0: ha:heap -> a:bigint{norm ha a} -> hb:heap -> b:bigint{filled hb b} ->
  Lemma (forall (i:nat). i < norm_length ==> v (getValue hb b i) - v (getValue ha a i) < pow2 (templ i + 1))
