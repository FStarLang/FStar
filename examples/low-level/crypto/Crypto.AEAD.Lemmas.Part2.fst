...
let find_entry_first (#i:id) (n:Cipher.iv (alg i)) (entries:Seq.seq (aead_entry i){Seq.length entries > 0})
  : Lemma (let head, tl = SeqProperties.head entries, SeqProperties.tail entries in
           if head.nonce = n then 
	     find_aead_entry n entries == Some head
	   else find_aead_entry n entries == find_aead_entry n tl)
  = ()
  
val false_elim : #a:Type -> u:unit{false} -> Tot a
let rec false_elim #a u = false_elim ()

let from_x_blocks_included_in (#i:id) (#rgn:rid) (x:PRF.domain i) (blocks:prf_table rgn i) (blocks':prf_table rgn i) = 
  forall (y:PRF.domain i).{:pattern (find blocks y)}
       y `above` x /\
       v y.ctr <= v (otp_offset i) + Seq.length blocks
       ==> find blocks y == find blocks' y

(** contains_all_blocks:
	blocks contains all entries relating to given aead_entry **)
let contains_all_entries (#i:id) (#r:rid) (e:aead_entry i) (blocks:prf_table r i) =
  let n = e.nonce in
  let x0 = {iv=n; ctr=ctr_0 i} in
  forall (y:domain i{y `above` x0 /\ v y.ctr <= v (ctr_0 i) + num_blocks_for_entry e}).{:pattern (find blocks y) \/ (y `above` x0)} 
      Some? (find blocks y)
...
val find_refined_aead_entry: 
    (i:id) -> (rgn:rid) ->
    (n:Cipher.iv (alg i){safeId i}) ->
    (aead_entries:aead_entries i) ->
    (prf_entries:prf_table rgn i) ->
    (h:mem{refines prf_entries aead_entries h /\
    ...
    Pure (aead_entry i)
	 ...
	 (ensures (fun entry ->
	  ...
	  (decreases (Seq.length aead_entries))
	  
#set-options "--initial_fuel 0 --max_fuel 0 --initial_ifuel 0 --max_ifuel 0 --z3rlimit 100"
let find_refined_aead_entry i rgn n aead_entries prf_entries h =
  (* let x0 = {iv=n; ctr=ctr_0 i} in *)
  (* let x1 = PRF.incr i x0 in *)
  (* invert_refines h i rgn entries prf_entries; *)
  match find_aead_entry n aead_entries with 
  | None -> assume (fresh_nonce n aead_entries); false_elim()
  | Some e -> assume (aead_entries `SeqProperties.contains` e); e
