...
void test_kremlin_aead(void *plain, void*cipher, int alg){
  ...
  switch (alg) {
  case AES_GCM:
    i =  Crypto_Indexing_testId(Crypto_Indexing_aeadAlg_AES_256_GCM);
    break;
  case AES128_GCM:
    i =  Crypto_Indexing_testId(Crypto_Indexing_aeadAlg_AES_128_GCM);
    break;
  case CHACHA_POLY:
    i =  Crypto_Indexing_testId(Crypto_Indexing_aeadAlg_CHACHA20_POLY1305);
    break;    
  }
...
void test_kremlin_prf(void *plain, void*cipher, int alg){
  ...
  d1 = b - a;
  t1 = ((double)c2 - c1)/CLOCKS_PER_SEC;
  print_results(alg == AES_GCM ? "Kremlin-C-aes256" : (alg == AES128_GCM ? "Kremlin-C-aes128" : "Kremlin-C-chacha20"), t1, d1, rounds, PLAINLEN);
}
...
void test_kremlin_chacha_alone(unsigned char *plain, unsigned char* cipher){
  clock_t c1, c2;
  double t1, t2;
  unsigned long long a,b,d1,d2;
  c1 = clock();
  ...
  for (int j = 0; j < ROUNDS; j++) {
    unsigned int ctx[16];
    Crypto_Symmetric_Chacha20_chacha_keysetup(ctx, key);
    Crypto_Symmetric_Chacha20_chacha_ietf_ivsetup(ctx, ivBuffer, 1);
    Crypto_Symmetric_Chacha20_chacha_encrypt_bytes(ctx, plain, cipher, PLAINLEN);
  }
  b = rdtsc();
  c2 = clock();
  d1 = b - a;
  t1 = ((double)c2 - c1)/CLOCKS_PER_SEC;
  print_results("Kremlin-CHACHA20-alone", t1, d1, ROUNDS, PLAINLEN);
...
void test_crypto_aead(){
  ...
  test_kremlin_aead(plain, cipher, AES128_GCM);
