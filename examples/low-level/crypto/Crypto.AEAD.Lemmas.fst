 ...
 * h2 is when all the otp entries have been added, but mac entry has not been updated.
 *)
(* unfold let pre_refines_one_entry (#i:id) (aead_st:aead_state i Writer) (nonce:Cipher.iv (alg i)) *)
(* 				 (len:nat{len <> 0}) (plainb:plainBuffer i len) (cipherb:lbuffer (len + v MAC.taglen)) *)
(* 				 (h0:mem) (h1:mem) (h2:mem) = *)
(*   (safeId i /\ prf i ==> ( *)
(*    safelen i len (ctr_0 i +^ 1ul) /\ *)
   
(*    (let prf_table_ref = PRF.itable i aead_st.prf in *)

(*     //all the mems contain prf_table_ref (AR: earlier this was for h1 only ?) *)
(*     h0 `HS.contains` prf_table_ref /\ h1 `HS.contains` prf_table_ref /\ h2 `HS.contains` prf_table_ref /\ *)

(*     (let prf_table_0 = HS.sel h0 prf_table_ref in  //PRF tables in h0, h1, and h2 *)
(*      let prf_table_1 = HS.sel h1 prf_table_ref in *)
(*      let prf_table_2 = HS.sel h2 prf_table_ref in *)
(*      let dom_0 = {iv = nonce; ctr = PRF.ctr_0 i} in *)
(*      let dom_1 = PRF.incr i dom_0 in *)

(*      //the mac block is either not present in h0 or if it is, then it's None, further no otp entries *)
...
(*      //the invariant for otp entries in h2 *)
(*      Buffer.live h2 cipherb  /\ *)
(*      Plain.live h2 plainb    /\ *)

(*      (let plain = Plain.sel_plain h2 (u len) plainb in *)
(*       let c_tagged = Buffer.as_seq h2 cipherb in *)
(*       let cipher, _ = SeqProperties.split c_tagged len in *)
(*       let otp_blocks = counterblocks i aead_st.prf.mac_rgn dom_1 len 0 len plain cipher in *)
(*       (forall (prf_entry:PRF.entry aead_st.prf.mac_rgn i). *)
(*          otp_blocks `SeqProperties.contains` prf_entry ==> *)
(*          PRF.find prf_table_2 prf_entry.x == Some (prf_entry.range))))))) *)

(* val frame_pre_refines: (i:id) -> (st:aead_state i Writer) -> (nonce:Cipher.iv (alg i)) ->  *)
(* 		       (len:nat{len<>0}) ->  (plainb:plainBuffer i len) -> (cipherb:lbuffer (len + v MAC.taglen)) ->  *)
(* 		       (h0:mem) -> (h1:mem) -> (h2:mem) (h3:mem) *)
(*    ->  Lemma (requires (let tagB = Buffer.sub cipherb (u len) MAC.taglen in *)
(* 		       pre_refines_one_entry st nonce len plainb cipherb h0 h1 h2 /\ *)
...
(* 	                Buffer.live h3 cipherb /\ *)
(* 		        Plain.live h3 plainb /\  *)
(* 		        (if mac_log *)
(* 		         then HS.modifies (Set.as_set [st.prf.mac_rgn; Buffer.frameOf cipherb]) h2 h3  /\ *)
(* 			      Buffer.modifies_buf_1 (Buffer.frameOf cipherb) tagB h2 h3 *)
(* 		         else Buffer.modifies_1 tagB h2 h3))) *)
(*              (ensures pre_refines_one_entry st nonce len plainb cipherb h0 h1 h3) *)
...
let rec counterblocks_len #i rgn x len from_pos plain cipher = admit ()
  (* if from_pos = len *)
  (* then () *)
...
let mac_refines (#i:id) 
		(st:aead_state i Writer) (nonce: Cipher.iv (alg i))
   ...
   = let mac_rgn = st.prf.mac_rgn in
     ...
     (safeId i /\ prf i ==> 
      (let tab = HS.sel h (PRF.itable i st.prf) in
       mac_is_set tab nonce ad len c tag h))

(*
 * AR: the requires and ensures clauses look the same ?
 * except the tag is read using Buffer.sub and not from split ?
 *)
let intro_mac_refines (#i:id) (st:aead_state i Writer) (nonce: Cipher.iv (alg i))
		      (#aadlen: UInt32.t {aadlen <=^ aadmax}) (aad: lbuffer (v aadlen))
                      (#len:nat{len<>0}) (plain:plainBuffer i len) (cipher:lbuffer (len + v MAC.taglen))
   		      (h:mem{Buffer.live h aad /\ Plain.live h plain /\ Buffer.live h cipher})
   : Lemma (requires (let x0 : PRF.domain i = {iv=nonce; ctr=ctr_0 i} in
           ...
           (ensures mac_refines st nonce aad plain cipher h)
  = ()

#reset-options "--z3timeout 100 --initial_fuel 0 --max_fuel 0 --initial_ifuel 1 --max_ifuel 1"
let rec all_above_counterblocks (i:id{safeId i})
                       (rgn:region)
                       (x:PRF.domain i{ctr_0 i <^ ctr x})
                       (l:nat)
                       (from_pos:nat)
                       ...
                       (plain:Plain.plain i l)
                       (cipher:lbytes l)
   : Lemma (counterblocks i rgn x l from_pos to_pos plain cipher `all_above` x)
   = admit ()   
     (* let remaining = to_pos - from_pos in *)
     (* if remaining = 0 then *)
     (*   let _ = counterblocks_lemma i rgn x l from_pos to_pos plain cipher in *)
     (*   SeqProperties.lemma_contains_empty #(PRF.entry rgn i) *)
     (* else *)
     (*   let bl = v (Cipher (blocklen (cipherAlg_of_id i))) in *)
     (*   let l0 = minNat remaining bl in *)
     (*   all_above_counterblocks i rgn (PRF.incr i x) l (from_pos + l0) to_pos plain cipher *)
...
let find_mac_counterblocks_none (#rgn:region) (#i:id) (s:prf_table rgn i)
    ...
    = SeqProperties.lemma_find_l_contains (is_entry_domain ({iv=nonce; ctr=ctr_0 i})) s
...
private let refines_empty (h:mem) (i:id{safeId i}) (rgn:region) 
  : Lemma (refines #rgn #i Seq.createEmpty Seq.createEmpty h)
  = SeqProperties.lemma_contains_empty #(aead_entry i);
    SeqProperties.lemma_contains_empty #(PRF.entry rgn i);
    let open FStar.Classical in
    forall_intro (move_requires (find_mac_counterblocks_none #rgn #i Seq.createEmpty))
