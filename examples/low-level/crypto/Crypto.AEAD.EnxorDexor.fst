...
(*** Some lemmas to enable calling dexor ***)
...
val intro_contains_all_blocks (#i:id)
    ...
    : Lemma (requires (let aead_entries = HS.sel h st.log in
		      ...
		      find_aead_entry n aead_entries == Some aead_entry /\
           ...
           (ensures (let x_1 = {iv=n; ctr=otp_offset i} in
		     ...
		     prf_contains_all_otp_blocks_st x_1 #(v plainlen) (v plainlen) (as_plain q) cipher st.prf h))
#reset-options "--initial_ifuel 0 --max_ifuel 0 --initial_fuel 0 --max_fuel 0"
let intro_contains_all_blocks #i n st #aadlen aad #plainlen cipher_tagged q aead_entry h = ()
...
val get_verified_plain : #i:id -> #n:Cipher.iv (alg i) -> st:aead_state i Reader ->
      ...
      ST (maybe_plain i (v plainlen))
         ...
         (ensures (fun h0 p h1 -> 
		    ...
		    h0 == h1 /\
		    (if verified
		     then prf_contains_all_otp_blocks_st x1 (v plainlen) p cipher st.prf h1 /\
...
let get_verified_plain #i #n st #aadlen aad #plainlen plain cipher_tagged ak acc verified = 
  if safeId i && verified then
    ...
    let _ : unit = 
      ...
      intro_contains_all_blocks n st aad cipher_tagged p e h in
    p
