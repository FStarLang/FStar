...
in (let _13_25 = (SST.memwrite li (liv + 1))
...
in (let _13_48 = (factorialLoop n li res)
in (let v = (SST.memread res)
in v)))))

let loopyFactorial2 = (fun ( n ) -> (SSTCombinators.withNewScope Set.empty (fun ( u ) -> (let li = (SST.salloc 0)
in (let res = (SST.salloc 1)
in (let _13_68 = (SSTCombinators.scopedWhile1 li (fun ( liv ) -> (not ((liv = n)))) () (Set.union (Set.singleton (Heap.Ref ((), (Obj.magic li)))) (Set.singleton (Heap.Ref ((), (Obj.magic res))))) (Obj.magic (Obj.magic (fun ( u ) -> (let liv = (SST.memread li)
in (let resv = (SST.memread res)
in (let _13_65 = (SST.memwrite li (liv + 1))
in (SST.memwrite res ((liv + 1) * resv)))))))))
in (let v = (SST.memread res)
in v)))))))
