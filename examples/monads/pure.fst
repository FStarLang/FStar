...
(* Is type equality primitive? If so, how can we write it?
   We can (re)define it constructively, but it seems like a waste *)

type cteq : a:Type -> b:Type -> Type =
  | TEq : #a:Type -> cteq a a

val cteq_trans : #a:Type -> #b:Type -> #c:Type ->
  h1:(cteq a b) -> h2:(cteq b c) -> cteq a c
let cteq_trans (a:Type) (b:Type) (c:Type) (h1:(cteq a b)) (h2:(cteq b c)) = TEq
...
(*   (forall x. cteq (f x) (g x)) -- a should be a kind not a type here *)

val left_identity : a:Type -> b:Type -> x:a -> f:(a -> PureWP b) ->
  p:(b->Type) -> cteq (pure_bind_wlp a b (pure_return a x) f p) (f x p)
let left_identity (a:Type) (b:Type) (x:a) (f:(a -> PureWP b)) (p:(b->Type)) = TEq

val right_identity : a:Type -> m:(PureWP a) ->
  p:(a->Type) -> cteq (pure_bind_wlp a a m (pure_return a) p) (m p)
let right_identity (a:Type) (m:(PureWP a)) (p:(a->Type)) = TEq

assume val associativity : a:Type -> b:Type -> c:Type ->
                    m:(PureWP a) -> f:(a -> PureWP b) -> g:(b -> PureWP c) ->
  p:(c->Type) -> cteq (pure_bind_wlp b c (pure_bind_wlp a b m f) g p)
...
(cteq (pure_bind_wlp b c (pure_bind_wlp a b m f) g p)
...
val test : a:Type -> f:((a -> Type) -> Type) ->
  cteq (f (fun (x:a) -> (fun (y:Type) -> y) int))
       (f (fun (x:a) -> int))
...
val test1 : a:Type -> b:Type -> c:Type ->
                    m:(PureWP a) -> f:(a -> PureWP b) -> g:(b -> PureWP c) ->
  p:(c->Type) -> cteq (m (fun y -> (fun x -> (pure_bind_wlp b c (f x) g)) y p))
...
val test2 : a:Type -> b:Type -> c:Type ->
                    m:(PureWP a) -> f:(a -> PureWP b) -> g:(b -> PureWP c) ->
  p:(c->Type) -> cteq (m (fun y -> (pure_bind_wlp b c (f y) g) p))
                      (m (fun y -> f y (fun z -> g z p)))
...
val test012 : a:Type -> b:Type -> c:Type ->
                    m:(PureWP a) -> f:(a -> PureWP b) -> g:(b -> PureWP c) ->
  p:(c->Type) -> cteq (m (fun y -> f y (fun z -> g z p)))
                      ((pure_bind_wlp a c m
...
val alpha : a:Type -> b:Type -> c:Type ->
                    m:(PureWP a) -> f:(a -> PureWP b) -> g:(b -> PureWP c) ->
  p:(c->Type) -> cteq (m (fun x -> f x (fun y -> g y p)))
                      (m (fun y -> f y (fun z -> g z p)))
...
let associativity' (a:Type) (b:Type) (c:Type) (m:(PureWP a)) (f:(a -> PureWP b))
  ...
  cteq_trans (test012' a b c m f g p) (test012 a b c m f g p)
