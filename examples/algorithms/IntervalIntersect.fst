...
/// [range] using a lambda expression. This is what is lost, but what is gained
/// by hiding the representation. We can change the way sets are represented in
/// [FStar.Set] under the hood, without affecting users of the library. Indeed
/// F* libraries often have two implementations, one acting as a model to prove
/// that the pre- and post-conditions of the API do not allow to prove [False],
/// and an actual implementation for running the actual program. In fact different
/// extraction targets, such as OCaml and F# would typically provide different
...
/// reasoning about programs and program verification. There are also
/// differences, the expressivity and multiple levels of abstraction offered by
/// a programming language like Haskell make the code more readable,
/// but supporting these in a verification tool may get in the way of automation
/// and may result in a large trusted computing base. The established wisdom
/// here is to have a core calculus that is as small as possible, and to
/// translate the more complex human readable code to that calculus.
