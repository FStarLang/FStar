...
val authenticateUser : uList -> req:request -> Tot bool
...
val ipAuthResp : #rsl:secLevel -> ruri:c_uri rsl -> r:request{Request?.rsl r = rsl} -> ML (resp:actResponse{requestResponseValid r resp})
...
let getIPAuthResp r =
    ...
    (
       let cid = declassify (getQSVal ruri.c_querystring "client_id") in
       if (getRPOrigin (Secret PublicVal cid) = s_getRequestOrigin r) then 
...
val ipReqResp : r:request -> ML (ret:retReqResp{isValidRetResp r ret /\ (RetResponse? ret)})
let ipReqResp r =
    let urisl = s_getReqURISecLevel r in
