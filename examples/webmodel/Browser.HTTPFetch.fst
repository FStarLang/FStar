...
val httpNetworkFetch : browser -> browserRequest -> bool -> (cowindow * cowindow * navType) -> Tot (result * browser)
let httpNetworkFetch b r cflag einfo = sendRequest b r cflag einfo

private val getFetchHeader : browser -> req:browserRequest -> bool -> Tot (bool * h:header{checkHeaderSecLevel h})
...
val httpNetworkOrCacheFetch : browser -> req:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf req).reqhead} -> bool -> bool -> (cowindow * cowindow * navType) -> 
			      Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
let httpNetworkOrCacheFetch b req cors af einfo =
    let r = BRequest?.rf req in 
    let (cflag, nh) = getFetchHeader b req cors in 
    let nr = BRequest (BRequest?.rsl req) ({r with reqhead=nh;corsflag=cors;corspfflag=false;authflag=af}) in
...
*)
val httpNetworkOrCachePreflightFetch : browser -> browserRequest -> bool -> bool -> (cowindow * cowindow * navType) -> Tot (result * browser)
...
val preflight_request : browser -> browserRequest -> h:header{checkHeaderSecLevel h /\ List.for_all (fun (hf,hv) -> Headervalue?.hvs hv = PublicVal) h} -> Tot browserRequest
let preflight_request b req h =
  let r = (BRequest?.rf req) in
  let rs = (BRequest?.rsl req) in
  BRequest rs ({reqb=b.bid;reqm="OPTIONS";requrl=r.requrl;reqhead=h;reqo=r.reqo;reqw=None;reqinit=r.reqinit;reqtype=r.reqtype;reqdest=r.reqdest;reqtarget=None;reqredirect=0;reqredmode="follow";reqref=r.reqref;reqrefPolicy=r.reqrefPolicy;reqnonce="";reqparser="";requnsafe=false;reqpreflight=false;reqsync=false;reqmode=NoCORS;reqtaint="basic";reqcredm="omit";reqcredflag=false;reqbody=(emptyString rs);corsflag=r.corsflag;corspfflag=true;authflag=r.authflag;recflag=r.recflag})

(* Section 4.7 - CORS-Preflight Fetch *)
val corsPreflightFetch : browser -> req:browserRequest -> (cowindow * cowindow * navType) -> Tot (result * browser)
let corsPreflightFetch b req einfo =
  ...
  let pr = (let r = (BRequest?.rf req) in
	   let rs = (BRequest?.rsl req) in
	   BRequest rs ({reqb=b.bid;reqm="OPTIONS";requrl=r.requrl;reqhead=nh;reqo=r.reqo;reqw=None;reqinit=r.reqinit;reqtype=r.reqtype;reqdest=r.reqdest;reqtarget=None;reqredirect=0;reqredmode="follow";reqref=r.reqref;reqrefPolicy=r.reqrefPolicy;reqnonce="";reqparser="";requnsafe=false;reqpreflight=false;reqsync=false;reqmode=NoCORS;reqtaint="basic";reqcredm="omit";reqcredflag=false;reqbody=(emptyString rs);corsflag=r.corsflag;corspfflag=true;authflag=r.authflag;recflag=r.recflag})) in
  httpNetworkOrCachePreflightFetch b pr false false einfo

(* Section 4.3 - HTTP Fetch *)
val httpFetch : browser -> r:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf r).reqhead} -> bool -> bool -> (cowindow * cowindow * navType) -> 
		Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
let httpFetch b r cors pf einfo = (* check for skip-service-worker. define service workers. for now assuming that the page doesnt use service workers *)
  if (pf) then
      let nreq = BRequest (BRequest?.rsl r) ({(BRequest?.rf r) with corsflag=cors;corspfflag=true}) in
...
val schemeFetch : browser -> r:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf r).reqhead} -> (cowindow * cowindow * navType) -> 
		  Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
let schemeFetch b r einfo =
  let rurl = (firstElement (BRequest?.rf r).requrl) in
...
val response_for_fetch : browser -> nreq:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nreq).reqhead} -> bool -> 
			 (cowindow * cowindow * navType) -> Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
let response_for_fetch b nreq cors einfo =
    let r = (BRequest?.rf nreq) in
    ...
    if (isSameOriginRR no r.reqo && cors = false) || (r.reqmode = Navigate) || (r.reqmode = WebSocket) then
    	let req = BRequest (BRequest?.rsl nreq) ({r with reqtaint="basic"}) in
    	...
    	let req = BRequest (BRequest?.rsl nreq) ({r with reqtaint="opaque"}) in
    	schemeFetch b req einfo (* perform basic fetch *)
	...
	let req = BRequest (BRequest?.rsl nreq) ({r with reqtaint="cors";corsflag=true;corspfflag=true}) in
	httpFetch b req true true einfo
    else
	let req = BRequest (BRequest?.rsl nreq) ({r with reqtaint="cors";corsflag=true;}) in
	httpFetch b req true false einfo

private val upReferRequest : r:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf r).reqhead} -> referer -> refPolicy -> bool -> bool -> 
			     Tot (nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead})
let upReferRequest r refer rP cors rf = BRequest (BRequest?.rsl r) ({(BRequest?.rf r) with reqref=refer;reqrefPolicy=rP;corsflag=cors;recflag=rf})
...
val fetch_resource : browser -> r:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf r).reqhead} -> bool -> bool -> (cowindow * cowindow * navType) -> 
		     Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
...
val httpRedirectFetch : browser -> req:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf req).reqhead /\ checkHeaderSecLevel (BRequest?.rf req).reqhead} -> 
			resp:actResponse{requestResponseValid req resp /\ isRedirectResponse (ActResponse?.ar resp)} ->
			bool -> (cowindow * cowindow * navType) -> Tot (result * nr:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf nr).reqhead} * browser)
  ...
  if (ActResponse?.ar resp).resploc = None then (Success "no url", req, b) (* NO URL to redirect to *)
    ...
    if r.reqmode = CORS && (not (isSameOriginRR r.reqo (getAOrigin u))) && (includesCredentials u) then (Error "CORS error", req, b) (* Network Error *)
      ...
      if ((((ActResponse?.ar resp).respcode = 301 || (ActResponse?.ar resp).respcode = 302) && r.reqm = "POST") || ((ActResponse?.ar resp).respcode = 303)) then
      	  let newr = BRequest (URI?.usl u) ({nr with requrl=u::nr.requrl;reqm="GET";reqo=ro;reqhead=nh;reqredirect=(nr.reqredirect + 1);reqbody=(emptyString (URI?.usl u));corsflag=cors;recflag=rf}) in
          fetch_resource nb newr cors true einfo
      	  ...
      	  let newr = BRequest (URI?.usl u) ({nr with requrl=u::nr.requrl;reqm=r.reqm;reqo=ro;reqhead=nh;reqredirect=(nr.reqredirect + 1);reqbody=(reclassify nr.reqbody (URI?.usl u));corsflag=cors;recflag=rf}) in
          fetch_resource nb newr cors true einfo
      else  (* other cases - mostly GET for 301/2/7/8 *)
	  let newr = BRequest (URI?.usl u) ({nr with requrl=u::nr.requrl;reqm=r.reqm;reqo=ro;reqhead=nh;reqredirect=(nr.reqredirect + 1);reqbody=(emptyString (URI?.usl u));corsflag=cors;recflag=rf}) in
          fetch_resource nb newr cors true einfo
...
val getResponse : browser -> connection -> actResponse ->
		  Tot (option ((browserRequest * cowindow * cowindow * navType) * actResponse * bool) * browser)
...
val processFetchResponse : browser -> req:browserRequest -> resp:response{validReqResp req resp} -> bool -> Tot (ores:response{validReqResp req ores})
...
val processCORSPreFlightResponse : browser -> r:browserRequest -> resp:actResponse{requestResponseValid r resp} -> bool -> Tot (option actResponse)
...
val processHttpRedirectResponse : browser -> browserRequest -> resp:response{notNetError resp} -> bool -> Tot (bool * result)
let processHttpRedirectResponse b req resp cf =
  ...
  else if (BRequest?.rf req).reqredirect = 20 then (false, (Error "exceeded redirect count"))
  else (true, Success "allow redirect")
  
(* Process http response *)
(* The fetch functions return the request but do not pass over to the callee *)
val processHttpResponse : browser -> req:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf req).reqhead} -> 
			  resp:actResponse{requestResponseValid req resp} -> bool ->
			  (cowindow * cowindow * navType) -> Tot (ores:response{validReqResp req ores} * browser)
let processHttpResponse b req resp cf einfo =
  let reqf = (BRequest?.rf req) in
...
val processNOCResponse : browser -> req:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf req).reqhead} -> 
			 resp:actResponse{requestResponseValid req resp} -> bool ->
			 (cowindow * cowindow * navType) -> Tot (ores:response{validReqResp req ores} * browser)
let processNOCResponse b req resp cf einfo =
  if ((ActResponse?.ar resp).respcode = 401 && cf = true && (BRequest?.rf req).corsflag = false && Some? (BRequest?.rf req).reqw) then (
     if ((BRequest?.rf req).reqcredflag = false || (BRequest?.rf req).authflag = true) then
	let (u,p) = getUserPassword b in
	let ourl = (firstElement (BRequest?.rf req).requrl) in
	let nurl = setUserPwd #(URI?.usl ourl) ourl (classify #PublicVal u (URI?.usl ourl)) (classify #PublicVal p (URI?.usl ourl)) in
	let nreq = BRequest (BRequest?.rsl req) ({(BRequest?.rf req) with requrl=(replace_elem_list (BRequest?.rf req).requrl 1 nurl)}) in
  ...
  else if ((ActResponse?.ar resp).respcode = 407 && Some? (BRequest?.rf req).reqw) then
      (* Prompt the end user as appropriate in request's window and store the result as a proxy-authentication entry *)
      let (r, nr, nb) = httpNetworkOrCacheFetch b req (BRequest?.rf req).corsflag (BRequest?.rf req).authflag einfo in
      match r with | Error e -> (NetworkError e, nb) | Success s -> (RespSuccess s, nb)
  else
      processHttpResponse b req resp cf einfo

(* Processing response for network fetch *)
val processNetworkResponse : browser -> connection -> r:browserRequest{notForbiddenHeaderfieldInReqHeader (BRequest?.rf r).reqhead} -> 
			     resp:actResponse{requestResponseValid r resp} -> 
			     Tot (ores:response{validReqResp r ores} * browser * option (browserRequest * cowindow * cowindow * navType))
