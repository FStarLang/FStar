...
private val getDirectiveForRequest: browserRequest -> Tot (option csp_directive_name)
let getDirectiveForRequest req = 
  ...
  if r.reqtype = "" then (
     ...
     else if (r.reqdest="document" && (match r.reqtarget with | Some f -> if f.cwin.cwparent <> None then true else false | None -> false))
...
private val get_nonce_list : list csp_directive_value -> Tot (list string)
...
(* Does nonce match source list - uses the auxil.ary function get_nonce_list above *)
private val nonce_match_source : string -> list csp_directive_value -> Tot bool
...
(* let isNonSubresource d = (d="document" || d="report" || d="serviceworker" || d="s.aredworker" || d="worker") *)
...
let connect_src_prerequest req d =
  ...
  if r.reqinit="fetch" || (r.reqtype="" && r.reqdest="subresource") then 
    uri_match_source r.requrl d.directive_value r.reqo r.reqredirect
  else true  
  (* if t="default" then ( *)
...
let frame_src_prerequest req d =
  let r = req.browser_request in
  if (r.reqtype = "document" && (match r.reqtarget with | None -> false | Some win -> win.cwin.cwparent <> None)) then (
     uri_match_source r.requrl d.directive_value r.reqo r.reqredirect)
  else true
...
let script_src_prerequest req d =
  ...
  if (r.reqtype = "script" && r.reqdest = "subresource") then ( 
    if (nonce_match_source r.reqnonce d.directive_value) then true (*check for cryptographic nonces*)
    else if (List.find (fun v -> v = DV_St_Dynamic) d.directive_value <> None) then 
      (if r.reqparser = "parser-inserted" then false else true)
    else uri_match_source r.requrl d.directive_value r.reqo r.reqredirect)
  else true
...
let child_src_prerequest r d p =
  let n=getDirectiveForRequest r in
    ...
    | Some dir -> if dir<>CSP_frame_src && dir<>CSP_worker_src then true
		 else if (List.find (fun f -> f.directive_name = dir) p <> None) then true
		 else (
		   match dir with
...
let default_src_prerequest r d p =
  let n=getDirectiveForRequest r in
    match n with
    | Some dir -> if (List.find (fun f -> f.directive_name = dir) p <> None) then true
		 else if (dir=CSP_frame_src || dir=CSP_worker_src) && (List.find (fun f -> f.directive_name = CSP_child_src) p <> None) then true
		 else (
		   match dir with
...
private val connect_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
let connect_src_postrequest req resp d =
  let r = req.browser_request in
  if r.reqinit="fetch" || (r.reqtype="" && r.reqdest="subresource") then 
    uri_match_resp_source resp.b_response.respurl d.directive_value r.reqo r.reqredirect
  else true 
  
(* Section 6.1.4.2. -- CSP's postrequest check for font-src *)
private val font_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val frame_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val img_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val manifest_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val media_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val object_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val script_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
let script_src_postrequest req resp d =
  let r = req.browser_request in
  if (r.reqtype = "script" && r.reqdest = "subresource") then ( 
    if (nonce_match_source r.reqnonce d.directive_value) then true
    else uri_match_resp_source resp.b_response.respurl d.directive_value r.reqo r.reqredirect)
  else true

(* Section 6.1.11.2. -- CSP's postrequest check for style-src *)
private val style_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val worker_src_postrequest : browserRequest -> browserResponse -> csp_directive -> Tot bool
...
private val child_src_postrequest : browserRequest -> browserResponse -> csp_directive -> csp_policy -> Tot bool
...
private val default_src_postrequest : browserRequest -> browserResponse -> csp_directive -> csp_policy -> Tot bool
...
let reqBlockedCSP r =
  match (BrowserRequest?.browser_request r).reqw with 
  ...
  | Some w -> match reqViolatePolicy r (getCSPList w) with
	     | None -> false
	     | Some v -> true (*reorigin_port all violations*)
 
private val respViolatePolicy: browserResponse -> browserRequest -> csp_policy -> Tot (option csp_directive)
...
(* true -> blocked; false -> allowed *)
val respReqBlockedCSP : browserResponse -> browserRequest -> Tot bool
let respReqBlockedCSP resp req =
  match req.browser_request.reqw with 
  | None -> false 
...
let form_action_prenavigate req t d =
  ...
  if (t = "form-submission") then 
    uri_match_source r.requrl d.directive_value r.reqo r.reqredirect
    (* match (List.find (fun s -> s.directive_name = CSP_form_action) p) with  *)
    (* | None -> true *)
    (* | Some d -> uri_match_source r.requrl d.directive_value r.reqo r.reqredirect *)
  else true 
...
(* CSP Level 3 - 4.2.4. Should navigation request of type from source to .arget be blocked by Content Security Policy? *)
...
private val checkAncestor : #bid:origin_opaque -> cowindow' bid -> list csp_directive_value -> uri -> nat -> Tot bool 
let rec checkAncestor #bid w v u n = 
  match (w.cwparent) with 
  | None -> true
  | Some p -> (origin_match_expr_source_list p.cwloc v u n) && (checkAncestor p v u n)
  
(* 6.3.2.1. frame-ancestors Navigation Response Check *)
(* request is not used by the algorithm currently *)
private val frame_ancestors_navresponse : browserRequest -> resp:browserResponse{not (list_is_empty resp.b_response.respurl)} ->
					  cowindow -> cowindow -> csp_directive -> Tot bool
let frame_ancestors_navresponse req resp sw tw d = 
  match resp.b_response.respurl with 
  | rurl::_ -> checkAncestor tw.cwin d.directive_value rurl 0

(* request is not used by the algorithm currently *)
private val navRespViolatePolicy: browserRequest -> browserResponse -> cowindow -> cowindow -> csp_policy -> Tot (option csp_directive)
let rec navRespViolatePolicy req resp sw tw p =
  match p with
  | [] -> None
  | ph::pt -> match ph.directive_name with
	      | CSP_frame_ancestors -> if (not (list_is_empty resp.b_response.respurl) && frame_ancestors_navresponse req resp sw tw ph) then 
					 navRespViolatePolicy req resp sw tw pt 
				      else Some ph
	      | _ -> navRespViolatePolicy req resp sw tw pt

(* 4.2.5. Should navigation response to navigation request of type from source in .arget be blocked by Content Security Policy? *)
(* Use of type? Not used by underlying algorithm *)
(* request is not used by the algorithm currently *)
val navRespBlockedCSP : browserRequest -> string -> browserResponse -> cowindow -> cowindow -> Tot bool
let navRespBlockedCSP r t resp sw tw =
  match navRespViolatePolicy r resp sw tw resp.b_response.respCSP with
    | None -> false
    | Some v -> true
