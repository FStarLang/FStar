...
let invariant (h:mem) (k:key) =
  ...
  Map.contains (HS.get_hmap h) k.region /\
  Map.contains (HS.get_hmap h) (MAC.Key?.region k.km) /\
  Map.contains (HS.get_hmap h) (CPA.Key?.region k.ke) /\
...
let keygen (parent:rid)
  ...
  (ensures  (fun h0 k h1 ->
    ...
    Map.contains (HS.get_hmap h1) k.region /\
    contains h1 k.log /\
