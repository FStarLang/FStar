...
noeq abstract type hpke_pkey (pk_id:id) =
  | PKey: #region:rid -> (dh_pk:dh_pkey)  -> hpke_pkey pk_id
...
*)
val encrypt: #(i:id) -> #(pk_id:id) -> #(m_id:id) -> hpke_pkey pk_id -> protected_hpke_plain m_id -> St c 
let encrypt #i #pk_id #m_id pk p =
  let region = new_region pk.region in
  let eph_dh_pk,eph_dh_sk = DH.keygen #i region in
  let k = prf_odh_snd eph_dh_sk pk.dh_pk in
  let p' = 
    match hpke_ind_cca && honest i with
    | true -> p
    | false -> PlainHPKE.repr p
  in
  let ae_m = ae_message_wrap p' in
  //let dh_m = dh_message_wrap #pk.i eph_dh_pk.rawpk in
  eph_dh_pk.rawpk,(AE.encrypt k ae_m)


val decrypt: #(i:id) -> hpke_skey i -> c -> option (protected_hpke_plain i)
let decrypt #i sk c =
  let (dh_sh,ae_c) = c in 
  let k = AE.keygen sk.hpke_pk.region sk.dh_sk dh_sh in
  let hpke_p =
    (match AE.decrypt k ae_c with
    | Some p -> Some (PlainAE.ae_message_unwrap p)
    | None -> None)
  in
  match hpke_p with
  | Some p' -> 
    (if hpke_ind_cca && honest i then
      Some p'
    else 
