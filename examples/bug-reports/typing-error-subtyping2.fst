...
Error typing-error-subtyping2.fst(6,0-17,0): The following problems were found:
	Subtyping check failed; expected type l:(list 'a){(forall x:'a. (b2p (mem  x l)) ==> False)}; got type (list 'a) (typing-error-subtyping2.fst(6,12-6,13))
*)

(* This one requires reduction below a quantifier
Subtyping check failed; expected type
[Before:l:(list 'a){(Forall #'a (fun x:'a => (l_imp (b2p (mem #'a x l)) False)))}]
[After:l:(list 'a){(forall x:'a. (b2p (mem #'a x l)) ==> False)}];
got type
