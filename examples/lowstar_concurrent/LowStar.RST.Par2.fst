...
assume
val lock (r:resource) (pred:r.t -> Type) : Type
...
assume
val new_lock (r:resource) (p:r.t -> Type)
  : RST (lock r p)
...
(* When we acquire a lock, the locked resource is now in context.
   ...
   *)
assume
val acquire (#r:resource) (#p:r.t -> Type) (l:lock r p)
  : RST unit
        (empty_resource)
        (fun _ -> r)
        (fun _ -> True)
        (fun _ _ h1 -> p (sel r.view h1))

(* Release is similar to new_lock, without the new value creation *)
assume
val release (#r:resource) (#p:r.t -> Type) (l:lock r p)
  : RST unit
        (r)
        (fun _ -> empty_resource)
        (fun h -> p (sel r.view h))
        (fun _ _ _ -> True)
