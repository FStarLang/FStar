(*--build-config
  options:--admit_fsi FStar.Seq --admit_fsi FStar.Set --verify_module Subtraction --z3timeout 150;
...
(* Auxiliary subtraction function putting the result in the first operand *)
...
let rec subtraction a a_idx b b_idx len =
  match len with
  | 0 -> ()
  |  _ -> 
      let h0 = (ST.get()) in
      subtraction a a_idx b b_idx (len-1);
      let h1 =
	(ST.get ()) in
...
(* Code *)
(* Tail recursive version of the subtraction *)
val subtraction_tr:
...
let rec subtraction_tr ghost_a a a_idx b b_idx len ctr =
  match len - ctr with
  | 0 -> ()
  | _ -> 
      let i = ctr in
	...
	(getSize h1 b (b_idx+i)) in
      ...
      subtraction_tr ghost_a a a_idx b b_idx len (ctr+1);
      ()
...
val subtraction_max_size_lemma:
  h0:heap -> 
  h1:heap -> 
  a:bigint{ (inHeap h0 a) } -> 
  b:bigint{ (inHeap h0 b)
...
let subtraction_max_size_lemma h0 h1 a b c = ()
...
val subtraction_max_value_lemma_extended:
  ...
  Lemma
    ...
    (ensures (maxValue h1 c <= maxValue h0 a + maxValue h0 b))
let subtraction_max_value_lemma_extended h0 h1 a b c idx len = 
  (
...
val subtraction_max_size_lemma_extended:
  h0:heap -> 
  h1:heap -> 
  a:bigint{ (inHeap h0 a) } -> 
  b:bigint{ (inHeap h0 b) } ->
  ...
  Lemma
    ...
    (ensures (maxSize h1 c <= max (maxSize h0 a) (maxSize h0 b) + 1))
let subtraction_max_size_lemma_extended h0 h1 a b c idx len = 
  cut ( forall (i:nat). (i < getLength h1 c /\ (i < idx \/ i >= idx + len))
...
let subtraction_with_lemma a b len =
  let h0 =
    (ST.get()) in
  subtraction_tr ((Array.to_seq (Bigint63.data a))) a 0 b 0 len 0;
  (
      let h1 = ST.get() in      
      subtraction_lemma h0 h1 a b len;
      subtraction_max_value_lemma_extended h0 h1 a b a 0 len;
      subtraction_max_size_lemma_extended h0 h1 a b a 0 len
    )
