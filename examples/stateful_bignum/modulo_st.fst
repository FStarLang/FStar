...
let rec modulo_bit a modulus shift =
  ...
  let s = shift_bits_over modulus shift in
  (* If smaller than a iterate *)
  if compare2 a s > 0 then 
    modulo_bit a modulus (shift+1)
  else (
    (* Else if shift > 0 subtract modulus shifted by shift - 1 *)    
    if shift > 0 then (
      Substraction.substraction_with_lemma a (shift_bits_over modulus (shift-1));
...
let modulo_cell a modulus offset =
  match offset with
  | 0 ->
...
let signed_modulo a modulus =
  ...
  let a = normalized_carry a in
  let modulus = normalized_carry modulus in
  ...
  if la < lmod then 
    ...
    let sign = compare2 a zero_bigint in
    let a = if sign = -1 then neg a else a in
    let b = Bigint.mk_zero_bigint (la + 1) in
    Array.blit (Bigint63.data a) 0 (Bigint63.data b) 0 la;
    modulo_cell b modulus (la - lmod);
    let c = Array.make lmod zero_tint in
    Array.blit (Bigint63.data b) 0 c 0 lmod;
    Bigint63.data b := c;
    if sign = -1 then neg b else b

(* An unsigned version of the modulo *)
let unsigned_modulo a modulus =
  let b = signed_modulo a modulus in
  if compare b zero_bigint = -1 then 
    addition_with_lemma b modulus
  else b
