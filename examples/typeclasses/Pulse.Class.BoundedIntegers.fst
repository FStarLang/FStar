...
class bounded_int (t:eqtype) = {
    ...
    op_Minus : (x:t -> y:t -> Pure t (requires fits (v x - v y)) (ensures fun z -> v z == v x - v y));
...
instance bounded_int_int : bounded_int int = {
    ...
    op_Minus = (fun x y -> Prims.op_Minus x y);
...
let sub_u32 (x:FStar.UInt32.t) (y:FStar.UInt32.t { ok op_Minus x y}) = x - y
...
instance bounded_int_nat : bounded_int nat = {
    ...
    u = (fun x -> x);
    ( + ) = (fun x y -> Prims.op_Plus x y);
    op_Minus = (fun x y -> Prims.op_Minus x y); //can't write ( - ), it doesn't parse
    ( < ) = (fun x y -> Prims.op_Less x y);
    ( <= ) = (fun x y -> Prims.op_Less_Equals x y);
    ( % ) = (fun x y -> Prims.op_Percent x y);
    properties = ()
