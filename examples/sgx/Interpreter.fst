...
let rec step (debugflag:bool) (env:cpuregstate) (defensivememop:memaccess) (myprogram:program) = function 
 ...
 | Jump (iaddr, e) -> 
	...
	(* Get the stmt list from updated "rip" *)
        let  stmtlist = defensivememop.decode true myprogram targetaddress in
...
and helperfunc  (debugflag:bool) (defensivememop:memaccess) (myprogram:program) (elem:stmt) (env':cpuregstate): STL cpuregstate 
					(requires (fun h -> true))
					(ensures (fun h0 r h1 -> false)) =
					step debugflag env' defensivememop myprogram elem 

and steps (debugflag:bool) (env:cpuregstate) (defensivememop:memaccess) myprogram instrlist = fold_right   (helperfunc debugflag defensivememop myprogram) instrlist env  
