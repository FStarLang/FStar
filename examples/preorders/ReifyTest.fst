...
(* A small example program demonstrating the unsoundness of the combination of reify and recall. *)
...
let test_reify_recall _ = 
  let h0 = ist_get () in
  let m = alloc #nat (fun n m -> n <= m) 0 in 
  ...
  let _ = w h0 in  
  assert (contains m h0 /\ sel h0 m > 0);      //accepted, demonstrates the unsoundness of the combination of reify and recall for IST
