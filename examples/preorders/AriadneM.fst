...
let sealable (Counter n c) s = 
  ...
  | Writing v _ -> u=v
  | Crash v w -> u=v \/ u=w))
let pre (x0 x1:counter) =  forall s. sealable x0 s ==> sealable x1 s
let _ = assert (monotonic counter pre)
...
type entry (e:enclave) = s:record {witnessed (fun h0 -> sealable (sel h0 e) s)}
...
let create v = 
  ...
  let r = (0,v) in 
  witness e (fun h0 -> sealable (sel h0 e) r);
  let b = alloc [r] in 
  let h1 = ST.get() in assume(sel h1 e = Counter 0 (Ok v)); //alloc framing?
  (|e,  b |) 
...
let seal e b w = 
  ...
  let log0 = read b in 
  write b (r::log0);
  let h1 = ST.get() in assume(sel h1 e = Counter n c1) //alloc framing?
...
let recover_state e b r = 
  ...
  then ( 
    // how to recall this? recall e (fun h0 -> sealable (sel h0 e) r);
    assume (sealable (Counter n c0) r);
    seal e b w;
    incr e w;
    seal e b w;
    incr e w;
