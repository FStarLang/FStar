...
abstract let equal (#a:pos) (h0:heap) (b:buffer a) (h1:heap) (c:buffer a) : GTot Type0 =
  live h0 b /\ live h1 c /\ length b = length c /\ 
  (forall (i:nat). {:pattern (get h0 b i) \/ (get h1 c i)} i < length b ==> v (get h0 b i) = v (get h1 c i))

let eq_trans (h0:heap) (h1:heap) (h2:heap) (#size:pos) (a:buffer size) 
  : Lemma (requires (equal h0 a h1 a /\ equal h1 a h2 a))
  	  (ensures (equal h0 a h2 a))
  	  [SMTPatT (equal h0 a h1 a); SMTPatT (equal h1 a h2 a)]
  = ()	  

let eq_refl (h0:heap)  (#size:pos) (a:buffer size) 
  : Lemma (requires (live h0 a))
  	  (ensures (equal h0 a h0 a))
  	  [SMTPatT (equal h0 a h0 a)]
  = ()	  

let unfold_equal h0 #size (a0:buffer size) h1 (a1:buffer size)
  : Lemma (requires (equal h0 a0 h1 a1))
          (ensures (live h0 a0 /\ live h1 a1 /\ length a0 = length a1 /\ 
	            (forall (i:nat). {:pattern (get h0 a0 i) \/ (get h1 a1 i)} i < length a0 ==> v (get h0 a0 i) = v (get h1 a1 i))))
  = ()
...
abstract let disjointSet (#t:pos) (b:buffer t) (buffs:FStar.Set.set abuffer) : GTot Type0 =
...
abstract let modifies_buf (buffs:FStar.Set.set abuffer) (h:heap) (h':heap) : GTot Type0 =
//  modifies buffs h h'
...
let disjoint_empty (#t:pos) (b:buffer t) 
  : Lemma (requires True)
          (ensures (disjointSet b empty))
	  [SMTPat (disjointSet b empty)]
  = ()

let disjoint_only_lemma (#t:pos) (b:buffer t) (#t':pos) (b':buffer t')
  : Lemma
     (requires (disjoint b b'))
     (ensures (disjointSet b (only b')))
     [SMTPatOr [[SMTPat (disjoint b b')];
	        [SMTPat (disjoint b' b)]]]
  = ()
  
let disjoint_union (#t:pos) (b:buffer t) (s1:Set.set abuffer) (s2:Set.set abuffer) 
  : Lemma (requires (disjointSet b s1 /\ disjointSet b s2))
          (ensures (disjointSet b (Set.union s1 s2)))
	  [SMTPat (disjointSet b (Set.union s1 s2))]
  = ()
...
let modifies_weakening_lemma (mods:FStar.Set.set abuffer) (h:heap)
  : Lemma (requires True)
	  (ensures (modifies_buf mods h h))
	  [SMTPatT (modifies_buf mods h h)]
  = ()	  
...
val modifies_fresh: h0:heap -> h1:heap -> mods:FStar.Set.set abuffer -> #size:pos -> b:buffer size ->
  Lemma (requires (not(contains h0 b) /\ modifies_buf (mods ++ b) h0 h1))
	(ensures (modifies_buf (mods) h0 h1))
	[SMTPatOr [[SMTPat (not(contains h0 b)); SMTPatT (modifies_buf (mods ++ b) h0 h1)];
		   [SMTPat (not(contains h0 b)); SMTPatT (modifies_buf (only b +++ mods) h0 h1)]]]
...
val modifies_subbuffer_lemma: h0:heap -> h1:heap -> mods:FStar.Set.set abuffer -> #size:pos ->
  b:buffer size -> b':buffer size -> Lemma 
    ...
    [SMTPatOr [[SMTPatT (modifies_buf (mods ++ b) h0 h1); SMTPatT (subBuffer #size b b')];
	       [SMTPatT (modifies_buf (only b +++ mods) h0 h1); SMTPatT (subBuffer #size b b')]]]
