...
let wrap_key key keylen =
  if keylen > 64 then
    let nkey = create #8 FStar.UInt8.zero 28 in
    sha224 nkey key keylen;
    nkey,28
  else
    key,keylen

(* Define the main function *)
val hmac_sha224: (mac     :sbytes { length mac = 28 }) ->
...
let hmac_sha224 mac data datalen key keylen =
  ...
  let s4 = create #8 FStar.UInt8.zero 28 in
  sha224 s4 s3 (64 + datalen);
  ...
  let s6 = create #8 FStar.UInt8.zero (28 + 64) in
  FStar.SBytes.blit s5 0 s6 0 64;
  FStar.SBytes.blit s4 0 s6 64 28;
