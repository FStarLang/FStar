...
let wrap_key key keylen =
  if keylen > 128 then
    let nkey = create #8 FStar.UInt8.zero 48 in
    sha384 nkey key keylen;
    nkey,48
  else
    key,keylen

(* Define the main function *)
val hmac_sha384: (mac     :sbytes { length mac = 48 }) ->
...
let hmac_sha384 mac data datalen key keylen =
  ...
  let s4 = create #8 FStar.UInt8.zero 48 in
  sha384 s4 s3 (128 + datalen);
  ...
  let s6 = create #8 FStar.UInt8.zero (48 + 128) in
  FStar.SBytes.blit s5 0 s6 0 128;
  FStar.SBytes.blit s4 0 s6 128 48;

  (* Step 7: apply H to "result of step 6" *)
  sha384 mac s6 (48 + 128)
