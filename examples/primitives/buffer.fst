...
let contains #a h (b:buffer a) : Tot (erased bool) = hide (Heap.contains #(seq a) h b.content)
let sel #a h (b:buffer a) : Tot (erased (seq a)) = hide (Heap.sel #(seq a) h b.content)

val max_length: #a:Type -> h:heap -> b:buffer a -> Tot (erased nat) //Tot (l:erased nat{reveal l = Seq.length (reveal (sel h b))})
let max_length #a h b =
  elift1 Seq.length (sel h b)
// JK: the following function fails
(*
let max_length1 (#a:Type0) (h:heap) (b:buffer a) : Tot (l:erased nat{reveal l = Seq.length (reveal (sel h b))})
  = elift1 Seq.length (sel h b) 
*)

let length #a (b:buffer a) : Tot (erased nat) = hide b.length
let idx #a (b:buffer a) : Tot (erased nat) = hide b.idx
let content #a (b:buffer a) : Tot (erased (ref (seq a))) = hide b.content
...
let live #a (h:heap) (b:buffer a) : GTot Type0 = 
  (b2t(reveal (contains h b)) /\ reveal (max_length h b) >= reveal (length b) + reveal (idx b))

let disjoint #a #a' (b:buffer a) (b':buffer a') : GTot Type0 =
  (b.content =!= b'.content
  \/ (b.content == b'.content  /\ a=a'
     /\ (reveal (idx b) + reveal (length b) <= reveal (idx b') \/ reveal (idx b') + reveal (length b') <= reveal (idx b))))

(* Defines that sb is a subbuffer of b *)
let subBuffer #a (sb:buffer a) (b:buffer a) : GTot Type0 =
  Heap.Ref b.content == Heap.Ref sb.content /\ reveal (idx sb) >= reveal (idx b) 
  /\ reveal (idx b) + reveal (length b) >= reveal (idx sb) + reveal (length sb)

val getValue: #a:Type -> h:heap -> b:buffer a -> i:nat{i<reveal (max_length h b)} -> Tot (erased a)
...
// JK: fails, and I do not understand why
let getValue #a h b i = 
  elift1
    (fun x -> 
      gcut(fun _ -> reveal (max_length h b) > i);
      gcut(fun _ -> Seq.length (Heap.sel h b.content) > i);
      Seq.index x i)
    (sel h b)
...
let getValue1 #a h (b:buffer a) (i:nat{i < reveal (max_length h b)}) : Tot (erased a)
  = elift1 (fun x -> Seq.index x i) (sel h b)
