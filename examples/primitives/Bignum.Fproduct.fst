...
abstract val gauxiliary_lemma_5: h0:heap -> h1:heap -> a:bigint_wide{live h0 a} -> 
...
abstract val auxiliary_lemma_5: h0:heap -> h1:heap -> a:bigint_wide{live h0 a} -> 
  b:bigint_wide{live h1 b} -> c:int -> len:nat -> len2:nat{len2 >= len /\ len2 <= length b /\ len2 <= length a} ->
...
let multiplication_step_0 a b ctr c tmp = 
  ...
  let h1 = ST.get() in 
  gcut(fun _ -> v s = v (get h0 b ctr)); 
  gcut(fun _ -> scalarProduct h0 h1 norm_length a (get h0 b ctr) tmp);
  ()
...
let multiplication_step_lemma_02 h0 h1 h2 a b ctr c tmp =
  ...
  gcut (fun _ -> eval h2 c (2*norm_length-1) - eval h2 c (norm_length+ctr) = eval h0 c (2*norm_length-1) - eval h0 c (norm_length+ctr)); 
  gcut (fun _ -> eval h1 tmp norm_length = eval h0 a norm_length * v (get h0 b ctr)); 
  gcut (fun _ -> storesSum h2 c h1 c h1 tmp ctr norm_length); 
  gcut (fun _ -> partialEquality h2 c h1 c ctr); 
  multiplication_step_lemma h1 h2 c tmp c ctr norm_length; 
  gcut (fun _ -> eval h2 c (norm_length+ctr) = eval h1 c (norm_length+ctr) + pow2 (bitweight templ ctr) * eval h0 a norm_length * v (get h0 b ctr)); 
  gcut (fun _ -> eval h2 c (norm_length+ctr) = eval h0 c (norm_length+ctr) + pow2 (bitweight templ ctr) * eval h0 a norm_length * v (get h0 b ctr)); 
  gcut (fun _ -> eval h2 c (2*norm_length-1) = eval h0 c (2*norm_length-1) + pow2 (bitweight templ ctr) * eval h0 a norm_length * v (get h0 b ctr)); 
  max_value_lemma h0 h1 h2 a b ctr c tmp; 
  gcut (fun _ -> maxValue h2 c (length c) <= (ctr+1) * pow2 53); 
...
abstract val ghelper_lemma_15: h0:heap -> h1:heap -> c:bigint_wide{length c >= 2*norm_length-1} -> GLemma unit
...
abstract val gmultiplication_lemma_1: h0:heap -> h1:heap -> c:bigint_wide{length c >= 2*norm_length-1} -> 
...
let helper_lemma_14 h0 h1 h2 c tmp =
  ...
  gcut (fun _ -> modifies_buf (only c ++ tmp) h0 h2); 
