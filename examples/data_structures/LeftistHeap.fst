...
let gt (#t: eqtype) {| _ : ordered t |} (a: t) b =
  leq b a && a <> b

let transitivity #a {| p : ordered a |} (x: a) y z:
...
let antisymmetry (#a: eqtype) {| p : ordered a |} (x: a) y:
  Lemma (requires leq x y /\ leq y x) (ensures x = y)
= p.properties

let total_order (#a: eqtype) {| p : ordered a |} (x: a) y:
...
let delta (#a: eqtype) (x:a) (y:a): GTot int = (if x = y then 1 else 0)
...
let rec lemma_lower_bounded_trans #a {| _ : ordered a |} (l: list a) m1 m2:
  Lemma (requires lower_bounded l m1 /\ leq m2 m1) (ensures lower_bounded l m2)
= match l with
  | [] -> ()
  | t::q -> (transitivity m2 m1 t; lemma_lower_bounded_trans q m1 m2)

let rec lemma_sorted_then_lower_bounded #a {| _ : ordered a |} (t: a) (q: list a):
  Lemma (requires sorted (t::q)) (ensures lower_bounded (t::q) t)
= match q with
  | [] -> reflexivity t
  | a::b ->
    (if Cons? b then transitivity t a (Cons?.hd b) else ();
    lemma_sorted_then_lower_bounded t b; assert (lower_bounded q t))

// Any element in the list is greater than or equal to a lower bound of the list
let rec lower_bounded_and_count (#a: eqtype) {| _ : ordered a |} (l: list a) (m: a) (x: a):
...
let rec lemma_injectivity_count_sorted (#t: eqtype) {| _ : ordered t |} (a: list t) b:
...
= match a, b with
  | ta::qa, tb::qb -> (
    ...
    assert (count a tb > 0);
    lemma_sorted_then_lower_bounded ta qa;
    lower_bounded_and_count a ta tb;
    // 2. ta <= tb
    eliminate forall y. count a y = count b y with ta;
    assert (count b ta > 0);
    lemma_sorted_then_lower_bounded tb qb;
    lower_bounded_and_count b tb ta;
    // 3. induction hypothesis
    antisymmetry ta tb;
    lemma_injectivity_count_sorted #t qa qb
    )
  | _ -> ()
#pop-options
#pop-options

let rec lemma_merge_count (#t: eqtype) {| _: ordered t |} (a: list t) b y:
  Lemma (ensures count (merge a b) y = count a y + count b y)
= match a, b with
  | ta::qa, tb::qb -> if (leq ta tb) then lemma_merge_count qa b y else lemma_merge_count a qb y
  | _ -> ()

let rec lemma_sorted_merge #t {| _ : ordered t |} (a: list t) (b: list t):
  Lemma (requires sorted #t a /\ sorted #t b) (ensures sorted (merge #t a b))
= match a, b with
  | ta::qa, tb::qb -> if (leq ta tb)
  then lemma_sorted_merge #t qa b
  else (lemma_sorted_merge #t a qb; total_order ta tb)
  | _ -> ()

let rec merge_lower_bounded (#t: eqtype) {| _ : ordered t |} (a: list t) b m:
...
= match t with
  ...
  | Node k l r _ -> (
    ...
    lemma_lower_bounded_trans (to_list_aux l) k m;
    to_list_aux_lower_bounded r k;
    lemma_lower_bounded_trans (to_list_aux r) k m;
    merge_lower_bounded (to_list_aux l) (to_list_aux r) m
  )

// The list representation of a sorted heap is sorted
let rec lemma_heap_property_then_sorted (#a: eqtype) {| _ : ordered a |} (t: heap a):
  Lemma (requires heap_property t) (ensures sorted (to_list_aux t))
= match t with
  | Node k l r _ ->
    (
      lemma_heap_property_then_sorted l;
      lemma_heap_property_then_sorted r;
      lemma_sorted_merge (to_list_aux l) (to_list_aux r)
    )
  | _ -> ()

let rec lemma_count_list_heap (#a: eqtype) {| _ : ordered a |} (t: heap a) y:
  Lemma (ensures count_heap t y = count (to_list_aux t) y)
= match t with
  | Leaf -> ()
  | Node k l r _ -> (lemma_count_list_heap l y; lemma_count_list_heap r y;
    lemma_merge_count (to_list_aux l) (to_list_aux r) y)
...
// Termination measure
let measure (#a: eqtype) {| _ : ordered a |} (t1: heap a) t2 =
  match t1, t2 with
  | Node _ _ _ _, Leaf -> 1
...
let rec merge_heaps_aux (#a: eqtype) {| _ : ordered a |}  (t1: heap a) (t2: heap a):
  Tot (heap a)
...
let rec lemma_heap_property_merge #a {| _ : ordered a |} (t1: heap a) t2:
  Lemma (requires heap_property t1 /\ heap_property t2) (ensures heap_property (merge_heaps_aux t1 t2))
  (decreases %[size t1 + size t2; measure t1 t2])
= match t1, t2 with
  | Node k1 l r _, Node k2 _ _ _ ->
  if gt k1 k2 then lemma_heap_property_merge t2 t1
  else (lemma_heap_property_merge r t2; total_order k1 k2)
  | _ -> ()

let rec lemma_count_merge_aux (#a: eqtype) {| _ : ordered a |} (t1: heap a) t2 y:
  Lemma (ensures count_heap (merge_heaps_aux t1 t2) y = count_heap t1 y + count_heap t2 y)
  (decreases %[size t1 + size t2; measure t1 t2])
= match t1, t2 with
  | Node k1 l r _, Node k2 _ _ _ ->
  if gt k1 k2 then lemma_count_merge_aux t2 t1 y
  else lemma_count_merge_aux r t2 y
  | _ -> ()

let lemma_merge_heap_charact (#t: eqtype) {| _ : ordered t |} (a: heap t) b:
...
= 
  ...
  (
    ...
    introduce forall (y: t). (count l1 y = count l2 y) with (
      calc (=) {
        count l1 y;
        = {lemma_count_list_heap (merge_heaps_aux a b) y}
        count_heap (merge_heaps_aux a b) y;
        = {lemma_count_merge_aux a b y}
        count_heap a y + count_heap b y;
        = {lemma_count_list_heap a y; lemma_count_list_heap b y}
        count (to_list_aux a) y + count (to_list_aux b) y;
        = {lemma_merge_count (to_list_aux a) (to_list_aux b) y}
        count l2 y;
      }
    );
    // Step 2: l1 is sorted
    lemma_heap_property_merge a b;
    lemma_heap_property_then_sorted (merge_heaps_aux a b);
    // Step 3: l2 is sorted
    lemma_heap_property_then_sorted a;
    lemma_heap_property_then_sorted b;
    lemma_sorted_merge (to_list_aux a) (to_list_aux b);
    // Step 4: Conclusion
    lemma_injectivity_count_sorted l1 l2
)
...
let rec lemma_correct_ranks_merge (#a: eqtype) {| _ : ordered a |} (t1: heap a) t2:
  Lemma (requires correct_ranks t1 && correct_ranks t2) (ensures correct_ranks (merge_heaps_aux t1 t2))
  (decreases %[size t1 + size t2; measure t1 t2])
= match t1, t2 with
  | Node k1 l r _, Node k2 _ _ _ ->
  if gt k1 k2 then lemma_correct_ranks_merge t2 t1
  else lemma_correct_ranks_merge r t2
  | _ -> ()

let rec lemma_leftist_property_merge (#a: eqtype) {| _ : ordered a |} (t1: heap a) t2:
  Lemma (requires leftist_property t1 && leftist_property t2) (ensures leftist_property (merge_heaps_aux t1 t2))
  (decreases %[size t1 + size t2; measure t1 t2])
= match t1, t2 with
  | Node k1 l r _, Node k2 _ _ _ ->
  if gt k1 k2 then lemma_leftist_property_merge t2 t1
  else lemma_leftist_property_merge r t2
  | _ -> ()
...
let leftist (a: eqtype) {| _ : ordered a |} = t:heap a{heap_property t /\ leftist_property t /\ correct_ranks t}

let to_list (#a: eqtype) {| _ : ordered a |} (t:leftist a): GTot (l:list a{sorted l}) =
  (lemma_heap_property_then_sorted t; to_list_aux t)
...
let merge_heaps (#a: eqtype) {| _ : ordered a |} (t1:leftist a) (t2:leftist a):
  (t:leftist a{ to_list t = merge (to_list t1) (to_list t2) })
= (lemma_merge_heap_charact t1 t2; lemma_heap_property_merge t1 t2; lemma_leftist_property_merge t1 t2; lemma_correct_ranks_merge t1 t2; merge_heaps_aux t1 t2)
