...
(* Analogous to ExnTotal, but we allow exceptions and hence
 * do partial correctness. *)
...
let interp (#a:Type) (l : repr a) : pure_wp a =
    fun p -> match l with | Inl x -> p x | _ -> True
...
let raise (#a:Type) (e:exn) : EXN a (fun p -> True)  =
  EXN?.reflect (Inr e)
...
(* This now succeeds, since raising the exception is just fine *)
