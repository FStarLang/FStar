...
let lemma_try_catch (#a:Type) 
  ...
  : Lemma (requires ((related c1 wp1) /\
          ...
          (ensures  (related (handle_rep c1 h_c c2) (wp_try_catch wp1 h_wp wp2))) =
  match c1 with
  | Inl x -> ()
  | Inr e -> ()
...
let try_catch (#a:Type) 
  ...
  EXN?.reflect (handle_rep (reify (c1 ())) (fun e -> reify (h_c e)) (fun x -> reify (c2 x)))
...
let try_div (i j:int)
  ...
  try_catch #_ #_ #(div_wp i j) (fun _ -> div i j) #(fun _ -> wp_return 0) (fun _ -> 0) #(wp_return) (fun x -> x)
