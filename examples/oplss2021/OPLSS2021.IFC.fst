...
let does_not_read_loc_v #a (f:comp a) (l:loc) (s0:store) v =
    ...
    (forall l'. {:nopattern} l' <> l ==>                 //nopattern: prevents matching loop with sel
...
let has_flow (from to:loc) (fs:flows) = exists rs. {:nopattern} rs `List.Tot.memP` fs /\ has_flow_1 from to rs //nopattern: prevents memP matching loop
...
let no_leakage #a (f:comp a) (from to:loc) = forall k. {:nopattern} no_leakage_k f from to k //nopattern: prevents matching loop with no_leakage_k
