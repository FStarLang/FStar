(*--build-config
    options:--admit_fsi Set --admit_fsi Seq --z3timeout 15 --max_fuel 1 --max_ifuel 1 --initial_ifuel 1 --initial_fuel 1;
...
val enc : e:encryptor -> p:plain -> ST cipher
    (requires (fun h -> enc_in_basic_fp e h /\ basic_fp_inv h))
    (ensures (fun h0 c h1 ->
...
val dec: d:decryptor -> c:cipher -> ST (option plain)
  (requires (fun h -> dec_in_basic_fp d h /\ basic_fp_inv h))
  (ensures (fun h0 p h1 ->
              modifies !{} h0 h1
...
let test_basic_frame b1 b2 =
  ...
  //  let c1'= enc (BI.e b1) q in // this (correctly) breaks typing, unless we strengthen our invariant
...
type enc_in_st_fp (e:st_encryptor) (h:heap) =
  exists d. st_paired e d /\ in_st_fp (STI e d) h /\ triggerSTI (STI e d)

type dec_in_st_fp (d:st_decryptor) (h:heap) =
  exists e. st_paired e d /\ List.mem (STI e d) (Heap.sel h st_fp) /\ triggerSTI (STI e d)
