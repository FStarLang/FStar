(*--build-config
    options:--admit_fsi Set --admit_fsi Seq --z3timeout 10;
...
type basic_fp_inv (h:heap) =
  ...
  /\ (forall bi.//{:pattern (List.mem bi (Heap.sel h basic_fp))}
        List.mem bi (Heap.sel h basic_fp)
         ==> (forall (a:Type) (r:ref a). //{:pattern (Heap.contains h r)}
...
type pairwise_distinct_sti (s:list sti) =
  forall s1 s2. List.mem s1 s /\ List.mem s2 s ==> (s1 = s2 \/ distinct_sti s1 s2)

let st_fp : ref (s:list sti{pairwise_distinct_sti s}) = ref []
...
type st_fp_inv (h:heap) =
   ...
   /\ (forall sti.
          ...
          ==> in_basic_fp (BI (StEnc.key (STI.e sti)) (StDec.key (STI.d sti))) h
              /\ st_inv (STI.e sti) (STI.d sti) h
              /\ (forall (a:Type) (r:ref a).
                    Set.mem (Ref r) (sti_refs sti)
                    ==> Heap.contains h r
                        /\ r =!= basic_fp
                        /\ r =!= st_fp))
