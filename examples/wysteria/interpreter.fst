...
let step c =
  ...
  else if is_value_ps c && is_sframe c F_aspar_ps? then Some (step_aspar_e2 c)
  else if is_value c && is_sframe c F_aspar_e? then Some (step_aspar_red c)
  ...
  else if is_value_ps c && is_sframe c F_box_ps? then Some (step_box_e2 c)
  else if is_value c && is_sframe c F_box_e? then Some (step_box_red c)
  else if pre_box c = Do then Some (step_box c)

  else if pre_eunbox c then Some (step_unbox_e c)
  else if is_value c && is_sframe c F_unbox? then Some (step_unbox_red c)
  else if pre_unbox c = Do then Some (step_unbox c)

  else if pre_emkwire c then Some (step_mkwire_e1 c)
  else if is_value_ps c && is_sframe c F_mkwire_ps? then Some (step_mkwire_e2 c)
  else if is_value c && is_sframe c F_mkwire_e? then Some (step_mkwire_red c)
  else if pre_mkwire c = Do then Some (step_mkwire c)
  
  else if pre_eprojwire c then Some (step_projwire_e1 c)
  else if is_value_p c && is_sframe c F_projwire_p? then Some (step_projwire_e2 c)
  else if is_value c && is_sframe c F_projwire_e? then Some (step_projwire_red c)
  else if pre_projwire c = Do then Some (step_projwire c)

  else if pre_econcatwire c then Some (step_concatwire_e1 c)
  else if is_value c && is_sframe c F_concatwire_e1? then Some (step_concatwire_e2 c)
  else if is_value c && is_sframe c F_concatwire_e2? then Some (step_concatwire_red c)
  ...
  else if is_value c && is_sframe c F_let? then Some (step_let_red c)
  ...
  else if is_value c && is_sframe c F_app_e1? then Some (step_app_e2 c)
  else if is_value c && is_sframe c F_app_e2? then Some (step_app_red c)
  else if pre_app c then Some (step_app c)

  else if pre_effi c then Some (step_ffi_e c)
  else if is_value c && is_sframe c F_ffi? then Some (step_ffi_l c)
  else if pre_ffi c = Do then Some (step_ffi c)

  else if pre_econd c then Some (step_cond_e c)
  else if is_value c && is_sframe c F_cond? then Some (step_cond_red c)
  else if pre_cond c then Some (step_cond c)

  else if pre_eassec c then Some (step_assec_e1 c)
  else if is_value_ps c && is_sframe c F_assec_ps? then Some (step_assec_e2 c)
  else if is_value c && is_sframe c F_assec_e? then Some (step_assec_red c)
  else if not (pre_assec c = NA) then Some (step_assec c)
  else if is_value c && is_sframe c F_assec_ret? then Some (step_assec_ret c)

  else if pre_emksh c then Some (step_mksh_e c)
  else if is_value c && is_sframe c F_mksh? then Some (step_mksh_red c)
  else if not (pre_mksh c = NA) then Some (step_mksh c)

  else if pre_ecombsh c then Some (step_combsh_e c)
  else if is_value c && is_sframe c F_combsh? then Some (step_combsh_red c)
  else if not (pre_combsh c = NA) then Some (step_combsh c)

  else None

val step_correctness: c:config{Some? (step c)} -> Tot (sstep c (Some.v (step c)))
let step_correctness c =
  let c' = v_of_some (step c) in

  if pre_easpar c then C_aspar_ps c c'
  else if is_value_ps c && is_sframe c F_aspar_ps? then C_aspar_e c c'
  else if is_value c && is_sframe c F_aspar_e? then C_aspar_red c c'
  ...
  else if is_value_ps c && is_sframe c F_box_ps? then C_box_e c c'
  else if is_value c && is_sframe c F_box_e? then C_box_red c c'
  else if pre_box c = Do then C_box_beta c c'

  else if pre_eunbox c then C_unbox c c'
  else if is_value c && is_sframe c F_unbox? then C_unbox_red c c'
  else if pre_unbox c = Do then C_unbox_beta c c'

  else if pre_emkwire c then C_mkwire_e1 c c'
  else if is_value_ps c && is_sframe c F_mkwire_ps? then C_mkwire_e2 c c'
  else if is_value c && is_sframe c F_mkwire_e? then C_mkwire_red c c'
  else if pre_mkwire c = Do then C_mkwire_beta c c'

  else if pre_eprojwire c then C_projwire_p c c'
  else if is_value_p c && is_sframe c F_projwire_p? then C_projwire_e c c'
  else if is_value c && is_sframe c F_projwire_e? then C_projwire_red c c'
  else if pre_projwire c = Do then C_projwire_beta c c'

  else if pre_econcatwire c then C_concatwire_e1 c c'
  else if is_value c && is_sframe c F_concatwire_e1? then C_concatwire_e2 c c'
  else if is_value c && is_sframe c F_concatwire_e2? then C_concatwire_red c c'
  ...
  else if is_value c && is_sframe c F_let? then C_let_red c c'
  ...
  else if is_value c && is_sframe c F_app_e1? then C_app_e2 c c'
  else if is_value c && is_sframe c F_app_e2? then C_app_red c c'
  else if pre_app c then C_app_beta c c'

  else if pre_effi c then C_ffi_e c c'
  else if is_value c && is_sframe c F_ffi? then C_ffi_l c c'
  else if pre_ffi c = Do then C_ffi_beta c c'

  else if pre_econd c then C_cond_e c c'
  else if is_value c && is_sframe c F_cond? then C_cond_red c c'
  else if pre_cond c then C_cond_beta c c'

  else if pre_eassec c then C_assec_ps c c'
  else if is_value_ps c && is_sframe c F_assec_ps? then C_assec_e c c'
  else if is_value c && is_sframe c F_assec_e? then C_assec_red c c'
  else if not (pre_assec c = NA) then C_assec_beta c c'
  else if is_value c && is_sframe c F_assec_ret? then C_assec_ret c c'

  else if pre_emksh c then C_mksh c c'
  else if is_value c && is_sframe c F_mksh? then C_mksh_red c c'
  else if not (pre_mksh c = NA) then C_mksh_beta c c'

  else if pre_ecombsh c then C_combsh c c'
  else if is_value c && is_sframe c F_combsh? then C_combsh_red c c'
  else C_combsh_beta c c'

val step_completeness: c:config -> c':config -> h:sstep c c' -> Lemma (requires (True)) (ensures (Some? (step c)))
...
val do_sec_comp:
  p:prin -> c:tstep_config p{T_red? (Conf.t c) /\ R_assec? (T_red.r (Conf.t c))
...
let tstep p c =
  ...
  if T_red? t && R_assec? (T_red.r t) && is_clos (R_assec.v (T_red.r (Conf.t c))) then
