...
val e_of_t_exp: t:term{T_exp? t} -> Tot exp
...
let pre_easpar (c:config) =
  T_exp? (t_of_conf c) && E_aspar? (e_of_t_exp (t_of_conf c)) && is_par c
...
val step_aspar_e2: c:config{is_value_ps c /\ is_sframe c F_aspar_ps?}
                  -> Tot config
...
val step_aspar_red: c:config{is_value c /\ is_sframe c F_aspar_e?}
                   -> Tot config
...
let pre_ebox (c:config) =
  T_exp? (t_of_conf c) && E_box? (e_of_t_exp (t_of_conf c))
...
val step_box_e2: c:config{is_value_ps c /\ is_sframe c F_box_ps?}
                  -> Tot config
...
val step_box_red: c:config{is_value c /\ is_sframe c F_box_e?}
                   -> Tot config
...
let pre_eapp (c:config) =
  T_exp? (t_of_conf c) && E_app? (e_of_t_exp (t_of_conf c))
...
val step_app_e2: c:config{is_value c /\ is_sframe c F_app_e1?}
                -> Tot config
...
val step_app_red: c:config{is_value c /\ is_sframe c F_app_e2?}
                 -> Tot config
...
let pre_eabs (c:config) =
  T_exp? (t_of_conf c) && E_abs? (e_of_t_exp (t_of_conf c))
...
let pre_efix (c:config) =
  T_exp? (t_of_conf c) && E_fix? (e_of_t_exp (t_of_conf c))
...
let pre_eempabs (c:config) =
  T_exp? (t_of_conf c) && E_empabs? (e_of_t_exp (t_of_conf c))
...
let pre_elet (c:config) =
  T_exp? (t_of_conf c) && E_let? (e_of_t_exp (t_of_conf c))
...
val step_let_red: c:config{is_value c /\ is_sframe c F_let?}
                 -> Tot config
...
(* TODO: FIXME: workaround for projectors *)
val x_of_e_var: e:exp{E_var? e} -> Tot varname
let x_of_e_var (E_var x) = x

(* TODO: FIXME: workaround for projectors *)
val Some?: option 'a -> Tot bool
...
let pre_evar (c:config) =
  T_exp? (t_of_conf c) && E_var? (e_of_t_exp (t_of_conf c)) &&
  Some? ((en_of_conf c) (x_of_e_var (e_of_t_exp (t_of_conf c))))
...
let pre_econst (c:config) =
  T_exp? (t_of_conf c) && E_const? (e_of_t_exp (t_of_conf c))
...
let pre_eunbox (c:config) =
  T_exp? (t_of_conf c) && E_unbox? (e_of_t_exp (t_of_conf c))
...
val step_unbox_red: c:config{is_value c /\ is_sframe c F_unbox?}
                   -> Tot config
...
let pre_emkwire (c:config) =
  T_exp? (t_of_conf c) && E_mkwire? (e_of_t_exp (t_of_conf c))
...
val step_mkwire_e2: c:config{is_value_ps c /\ is_sframe c F_mkwire_ps?}
                   -> Tot config
...
val step_mkwire_red: c:config{is_value c /\ is_sframe c F_mkwire_e?}
                    -> Tot config
...
let pre_eprojwire (c:config) =
  T_exp? (t_of_conf c) && E_projwire? (e_of_t_exp (t_of_conf c))
...
val step_projwire_e2: c:config{is_value_p c /\ is_sframe c F_projwire_p?}
                     -> Tot config
...
val step_projwire_red: c:config{is_value c /\ is_sframe c F_projwire_e?}
                      -> Tot config
...
(* TODO: FIXME: workaround for projectors *)
val v_of_some: x:option 'a{Some? x} -> Tot 'a
...
let pre_econcatwire (c:config) =
  T_exp? (t_of_conf c) && E_concatwire? (e_of_t_exp (t_of_conf c))
...
val step_concatwire_e2: c:config{is_value c /\ is_sframe c F_concatwire_e1?}
                       -> Tot config
...
val step_concatwire_red: c:config{is_value c /\ is_sframe c F_concatwire_e2?}
                        -> Tot config
...
let pre_effi (c:config) =
  T_exp? (t_of_conf c) && E_ffi? (e_of_t_exp (t_of_conf c))
...
val step_ffi_l: c:config{is_value c /\ is_sframe c F_ffi?} -> Tot config
...
let pre_econd (c:config) =
  T_exp? (t_of_conf c) && E_cond? (e_of_t_exp (t_of_conf c))
...
val step_cond_red: c:config{is_value c /\ is_sframe c F_cond?} -> Tot config
...
let pre_eassec (c:config) =
  T_exp? (t_of_conf c) && E_assec? (e_of_t_exp (t_of_conf c))
...
val step_assec_e2: c:config{is_value_ps c /\ is_sframe c F_assec_ps?}
                  -> Tot config
...
val step_assec_red: c:config{is_value c /\ is_sframe c F_assec_e?}
                   -> Tot config
...
val step_assec_ret: c:config{is_value c /\ is_sframe c F_assec_ret?}
                   -> Tot config
...
let pre_emksh (c:config) =
  T_exp? (t_of_conf c) && E_mksh? (e_of_t_exp (t_of_conf c)) && is_sec c
...
val step_mksh_red: c:config{is_value c /\ is_sframe c F_mksh?} -> Tot config
...
let pre_ecombsh (c:config) =
  T_exp? (t_of_conf c) && E_combsh? (e_of_t_exp (t_of_conf c)) && is_sec c
...
val step_combsh_red: c:config{is_value c /\ is_sframe c F_combsh?} -> Tot config
...
type sstep: config -> config -> Type =
  ...
  | C_aspar_e:
    c:config{is_value_ps c /\ is_sframe c F_aspar_ps?}
    -> c':config{c' = step_aspar_e2 c}
    -> sstep c c'
    
  | C_aspar_red:
    c:config{is_value c /\ is_sframe c F_aspar_e?}
  ...
  | C_aspar_ret:
    c:config{is_sframe c F_aspar_ret? /\ pre_aspar_ret c = Do}
  ...
  | C_box_e:
    c:config{is_value_ps c /\ is_sframe c F_box_ps?}
    -> c':config{c' = step_box_e2 c}
    -> sstep c c'

  | C_box_red:
    c:config{is_value c /\ is_sframe c F_box_e?}
  ...
  | C_unbox_red:
    c:config{is_value c /\ is_sframe c F_unbox?}
  ...
  | C_mkwire_e2:
    c:config{is_value_ps c /\ is_sframe c F_mkwire_ps?}
    -> c':config{c' = step_mkwire_e2 c}
    -> sstep c c'

  | C_mkwire_red:
    c:config{is_value c /\ is_sframe c F_mkwire_e?}
  ...
  | C_projwire_e:
    c:config{is_value_p c /\ is_sframe c F_projwire_p?}
    -> c':config{c' = step_projwire_e2 c}
    -> sstep c c'

  | C_projwire_red:
    c:config{is_value c /\ is_sframe c F_projwire_e?}
  ...
  | C_concatwire_e2:
    c:config{is_value c /\ is_sframe c F_concatwire_e1?}
    -> c':config{c' = step_concatwire_e2 c}
    -> sstep c c'

  | C_concatwire_red:
    c:config{is_value c /\ is_sframe c F_concatwire_e2?}
  ...
  | C_let_red:
    c:config{is_value c /\ is_sframe c F_let?}
  ...
  | C_app_e2:
    c:config{is_value c /\ is_sframe c F_app_e1?}
    -> c':config{c' = step_app_e2 c}
    -> sstep c c'

  | C_app_red:
    c:config{is_value c /\ is_sframe c F_app_e2?}
  ...
  | C_ffi_l:
    c:config{is_value c /\ is_sframe c F_ffi?}
  ...
  | C_cond_red:
    c:config{is_value c /\ is_sframe c F_cond?}
  ...
  | C_assec_e:
    c:config{is_value_ps c /\ is_sframe c F_assec_ps?}
    -> c':config{c' = step_assec_e2 c}
    -> sstep c c'

  | C_assec_red:
    c:config{is_value c /\ is_sframe c F_assec_e?}
  ...
  | C_assec_ret:
    c:config{is_value c /\ is_sframe c F_assec_ret?}
  ...
  | C_mksh_red:
    c:config{is_value c /\ is_sframe c F_mksh?}
  ...
  | C_combsh_red:
    c:config{is_value c /\ is_sframe c F_combsh?}
