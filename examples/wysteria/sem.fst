...
let step_match_e (Conf l m s en (T_exp (Exp (E_match e pats) _))) =
  Conf l m ((Frame m en (F_match pats))::s) en (T_exp e)
...
let step_match_red (Conf l _ ((Frame m en (F_match pats))::s) _ (T_val v)) =
  Conf l m s en (T_red (R_match v pats))
...
let pre_match c = match c with
  | Conf _ _ _ _ (T_red (R_match _ _)) -> Do
