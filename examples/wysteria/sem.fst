...
val sec_sstep_star_to_pstep_star:
...
let rec sec_sstep_star_to_pstep_star c c' h ps tp ts = admit ()
(* TODO: FIXME: this fails when verifying other files in emacs *)
(* match h with *)
(*   | SS_refl _                 -> PS_refl (tp, update (Mode.ps (Conf.m c)) c ts) *)
(*   | SS_tran #c #c' #c'' h1 h2 -> *)
(*     let pi = (tp, update (Mode.ps (Conf.m c)) c ts) in *)
(*     let pi'' = (tp, update (Mode.ps (Conf.m c)) c'' ts) in *)

(*     let pi' = (tp, update (Mode.ps (Conf.m c)) c' ts) in *)
(*     let ph = P_sec #ps #c' pi (Mode.ps (Conf.m c)) h1 pi' in *)
(*     PS_tran #ps #pi #pi' #pi'' ph (sec_sstep_star_to_pstep_star c' c'' h2 ps tp ts) *)

val par_sstep_to_pstep_star:
