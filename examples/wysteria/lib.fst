 ...
 *)
type wfold_pre (#b:Type) (m:mode) (ps:eprins) (w:Wire b) =
...
val waps: #a:Type -> #b:Type -> #req_f:(mode -> Type) -> #ens_f:(mode -> b -> Type)
          -> ps:prins
          -> w:Wire a{forall p. mem p ps ==> w_contains p w}
          -> =f:(p:prin{w_contains p w} -> x:a{w_select p w = x} -> Wys b req_f ens_f)
          -> Wys (Wire b) (fun m0 -> waps_pre #a #b m0 ps w /\ req_f m0)
...
type wapp_pre (#a:Type) (#b:Type) (m:mode) (ps:eprins) (w:Wire a) (req_f: mode -> Type) =
...
val wapp: #a:Type -> #b:Type -> #req_f:(mode -> Type) -> #ens_f:(mode -> b -> Type)
          ...
          -> w:Wire a{forall p. mem p ps ==> w_contains p w}
          -> =f:(p:prin{w_contains p w} -> x:a{w_select p w = x} -> Wys b req_f ens_f)
          -> Wys (Wire b) (fun m0 -> wapp_pre #a #b m0 ps w req_f)
                          (fun m0 r -> b2t (w_dom r = ps))
             (decreases (size ps))
