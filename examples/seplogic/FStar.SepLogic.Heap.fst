...
abstract let restrict (h:heap) (r:addr{h `contains` r}) :Tot heap
  = restrict h r

abstract let minus (h:heap) (r:addr{h `contains` r}) :Tot heap
  = minus h r 

abstract let join (h1:heap) (h2:heap{disjoint h1 h2}) :Tot heap
  = join h1 h2

abstract let points_to (r:addr) (x:t) :Tot heap
  = points_to r x

abstract let sel (h:heap) (r:addr{h `contains` r}) :Tot t
  = sel_tot h r

abstract let upd (h:heap) (r:addr{h `contains` r}) (x:t) :Tot heap
  = upd_tot h r x
...
//   :Lemma (requires h `contains` r /\ disjoint (restrict h r) (minus h r))
//          (ensures  (join (restrict h r) (minus h r) == h))
// 	 [SMTPat (join (restrict h r) (minus h r))]
//   = ()

// let lemma_join_is_comm' (h1:heap) (h2:heap{disjoint h1 h2})
//   :Lemma (requires True)
