...
val heap : Type u#1
val raw_memory : Type u#1
val defined : raw_memory -> Type0
let memory = m:raw_memory{defined m}
...
val disjoint_memories : raw_memory -> raw_memory -> Type0

val join : h0:heap -> h1:heap{disjoint_heaps h0 h1} -> Tot heap

val ( |> ) : #a:Type0 -> r:ref a -> x:a -> GTot memory
val ( <*> ) : m0:raw_memory -> m1:raw_memory -> GTot raw_memory

val split_heap : (m0:raw_memory) 
              -> (m1:raw_memory)
              -> (h:heap{defined (m0 <*> m1) /\ heap_memory h == (m0 <*> m1)}) 
              -> heap * heap

val hcontains : #a:Type0 -> heap -> ref a -> Type0
val mcontains : #a:Type0 -> raw_memory -> ref a -> Type0
...
val addrs_in : raw_memory -> Set.set nat
...
val lemma_disjoint_memories_emp (m:raw_memory)
  : Lemma (requires (defined m))
          (ensures  (disjoint_memories m emp))
          [SMTPat (disjoint_memories m emp)]

val lemma_disjoint_memories_comm (m0 m1:raw_memory)
...
val lemma_sep_disjoint_memories (m0 m1:raw_memory) 
...
val lemma_sep_unit (m:raw_memory)
  : Lemma ((m <*> emp) == m)
          [SMTPat (m <*> emp)]

val lemma_sep_comm (m0 m1:raw_memory)
  : Lemma ((m0 <*> m1) == (m1 <*> m0))
          
val lemma_sep_assoc (m0 m1 m2:raw_memory)
...
val lemma_sep_defined (m0 m1:raw_memory)
  : Lemma (requires (defined (m0 <*> m1)))
          (ensures  (defined m0 /\ defined m1))

(* split_heap *)

val lemma_split_heap_disjoint (m0 m1:raw_memory) (h:heap)
...
val lemma_split_heap_join (m0 m1:raw_memory) (h:heap)
  : Lemma (requires (defined (m0 <*> m1) /\ heap_memory h == (m0 <*> m1)))
          (ensures  (let (h0,h1) = split_heap m0 m1 h in
                     h == join h0 h1))
          [SMTPat (split_heap m0 m1 h)]

val lemma_split_heap_memories (m0 m1:raw_memory) (h:heap)
  : Lemma (requires (defined (m0 <*> m1) /\ heap_memory h == (m0 <*> m1)))
          (ensures  (let (h0,h1) = split_heap m0 m1 h in
...
val lemma_addrs_in_join (m0 m1:raw_memory)
  : Lemma (requires (defined (m0 <*> m1)))
