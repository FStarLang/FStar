...
let frame_wp0 (#a:Type) (wp:st_wp a) (post:memory -> (a * memory) -> Type0) (m:memory) (h0 h1:heap) =
  let m0 = heap_memory h0 in
  let m1 = heap_memory h1 in 
  disjoint_memories m0 m1 /\ m == (m0 <*> m1) /\ wp (post m1) m0

let frame_wp1 (#a:Type) (wp:st_wp a) (post:memory -> (a * memory) -> Type0) (m:memory) (h0:heap) =
  exists (h1:heap). frame_wp0 wp post m h0 h1

let frame_wp (#a:Type) (wp:st_wp a) (post:memory -> (a * memory) -> Type0) (m:memory) =
  exists (h0:heap). frame_wp1 wp post m h0
...
//Expanded out:
//         st a (fun post m -> exists m0 m1. disjoint m0 m1 /\ m == m0 <*> m1 /\ wp (fun m0' -> disjoint m0' m1 /\ post (x, (join_tot m0' m1))) m0
let frame #a #wp f = 
  fun post h ->
    ...
    bind_exists #(result a post) #heap #(frame_wp1 wp (frame_post post) (heap_memory h)) s (fun h0 s' -> 
      assert (frame_wp1 wp (frame_post post) (heap_memory h) h0);
      bind_exists #(result a post) #heap #(frame_wp0 wp (frame_post post) (heap_memory h) h0) s' (fun h1 _ -> 
        assert (frame_wp0 wp (frame_post post) (heap_memory h) h0 h1);
...
let alloc (#a:Type0) (x:a)
  : st (ref a) (fun post m0 -> m0 == emp /\ (exists (r:ref a) . post (r, (r |> x))))
  = fun post h0 ->
      assert (heap_memory h0 == emp);
      let (r,h1) = alloc h0 x in
      assert (heap_memory h1 == (r |> x));
      admit ()
...
let write (#a:Type0) (r:ref a) (v:a)
  : st unit (fun post m0 -> (exists (x:a). m0 == (r |> x) /\ (forall m1. m1 == (r |> v) ==> post ((), m1))))
    = fun post h0 -> 
        assert (mcontains (heap_memory h0) r);
