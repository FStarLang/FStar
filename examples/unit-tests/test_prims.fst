...
(* another singleton type, with its only inhabitant written '()'
   we assume it as primitive, for convenient interop with other languages *)
assume fresh type unit 

(* infix binary '=='; heterogeneous equality; primitive (TODO: make it an inductive?) *)
assume type Eq2 : #a:Type -> #b:Type -> a -> b -> Type  

type bool = 
 | B_true    //just 'true' in concrete syntax
 | B_false   //just 'false in concrete syntax

(* A proof irrelevant coercion to Type0: Elaborated in FStar.Squash.fst *)
...
// // type int32 = int
// // assume fresh type int64
// // assume fresh type uint8
// // assume fresh type uint16
// // assume fresh type uint32
// // assume fresh type uint64
// // assume fresh type char
// // assume fresh type float
// // assume fresh type string
// // ??type array : Type -> Type 
// // assume val strcat : string -> string -> Tot string
// // assume fresh type exn
...
// // type STPre_h  (heap:Type)          = heap -> Type
// // type STPost_h (heap:Type) (a:Type) = a -> heap -> Type
...
// // // (* Effect EXCEPTION *)
// // // type ExPre  = Type
// // // type ExPost (a:Type) = result a -> Type
...
// // //    (forall (rb:result b). p rb \/ wlp1 (fun ra1 -> ITE (b2t (is_V ra1))
...
// // //        ITE (b2t (is_V ra))
// // //            (wlp2 (V.v ra) (fun rb2 h2 -> rb=!=rb2 \/ h=!=h2) h1)
// // //            (rb=!=ra \/ h=!=h1)) h0 \/ p rb h)
