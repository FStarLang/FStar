(*
   Copyright 2008-2021 Jay Lorch and Microsoft Research
...
open Util.Seq.Ambient
open Util.Seq.Defs

private let test1 () : Lemma (True) =
  assert (seq_length (seq_empty int) == 0);
  assert (forall (s: seq bool). seq_length s == 0 ==> s == seq_empty bool);
  ...
  assert (~(seq_contains (seq_empty int) 42));
  assert (forall (s: seq int). seq_take s 0 == seq_empty int);
  assert (forall (s: seq int). seq_drop s 0 == s);
  ()

private let test2 () : Lemma (True) =
  let s1 = seq_build (seq_empty int) 1 in
  let s2 = seq_build s1 2 in
  let s3 = seq_build s2 3 in
  let s4 = seq_build s3 4 in
  let s5 = seq_build s4 5 in
  assert (seq_length s2 = 1 + seq_length s1);
  assert (seq_length s2 = 2);
  assert (seq_length s5 = 5);
  assert (seq_index s5 2 == 3);
  assert (forall (s: seq int) (n: nat). n < 2 ==> seq_index (seq_append s2 s) n = seq_index s2 n);
  assert (seq_drop (seq_drop s5 1) 2 == seq_drop s5 3);
  assert (forall (v: int). seq_length (seq_build s5 v) = 6);
  assert (seq_is_prefix s3 s5);
  assert (seq_length (seq_update s5 3 7) == 5);
  assert (seq_index (seq_update s5 3 7) 2 == 3);
  assert (seq_index (seq_update s5 3 7) 3 == 7)

private let test3 (s: seq int) (n: nat{n <= seq_length s})
  : Lemma (seq_equal s (seq_append (seq_take s n) (seq_drop s n))) =
  ()
...
private let test6 () : Lemma (True) =
  ...
  assert (forall (ty: Type) (s: seq ty).{:pattern seq_length s} seq_length s = 0 ==> s == seq_empty ty);
  ...
  assert (forall (ty: Type) (x: ty).{:pattern seq_contains (seq_empty ty) x} ~(seq_contains (seq_empty ty) x));
  ...
  assert (forall (ty: Type) (s: seq ty) (n: nat).{:pattern seq_take s n} n = 0 ==> seq_take s n == seq_empty ty);
