...
val h: x:erased int -> Pure (erased int) (requires (b2p (reveal x >= 0))) (ensures (fun y -> x == y))
let h x = x

//fine; having erased effects in specifications is ok
val i: x:erased int -> Pure int (requires (b2p (reveal x = 0))) (ensures (fun y -> x == hide y))
let i x = 0 //fine

//fine; having erased effects in specifications is ok
assume val j: x:erased int -> Pure int (requires (b2p (reveal x = 0)))
