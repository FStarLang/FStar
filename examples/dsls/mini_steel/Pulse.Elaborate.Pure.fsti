...
let binder_of_t_q t q = RT.mk_binder 0 t q
// let binder_of_t_q_s t q s = RT.mk_binder 0 t q
...
let mk_abs_with_name s ty qual t : R.term =  R.pack_ln (R.Tv_Abs (binder_of_t_q ty qual) t)
...
let rec elab_term (top:term)
  ...
  = let open R in
    match top with
    | Tm_BVar bv ->
      let bv = pack_bv (RT.make_bv bv.bv_index tun) in
      Some (pack_ln (Tv_BVar bv))
      
    | Tm_Var nm ->
      // tun because type does not matter at a use site
      let bv = pack_bv (RT.make_bv nm.nm_index tun) in
    ...
    | Tm_Arrow b q c ->
      ...
      Some (mk_tot_arrow1 (ty, elab_qual q) c)
...
val opening_pure_term_with_pure_term (x:pure_term) (v:pure_term) (i:index)
  : Lemma (ensures is_pure_term (open_term' x v i))
          [SMTPat (is_pure_term (open_term' x v i))]
val opening_pure_comp_with_pure_term (x:pure_comp) (v:pure_term) (i:index)
  : Lemma (ensures is_pure_comp (open_comp' x v i))
          [SMTPat (is_pure_comp (open_comp' x v i))]

val closing_pure_term (x:pure_term) (v:var) (i:index)
