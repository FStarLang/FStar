...
let rec size (e:src_exp)
  ...
  = match e with
    ...
    | ELam t e -> size_ty t + size e
    | EApp e1 e2 -> 1 + size e1 + size e2
and size_ty (t:src_ty)
  : nat
  = match t with
    | TBool -> 1
    | TRefineBool e -> 1 + size e
    | TArrow t1 t2 -> 1 + size_ty t1 + size_ty t2
...
let rec open_ty_ln (e:src_ty) (v:src_exp) (n:index) (m:int)
  : Lemma 
    (requires ln_ty' e n /\
              m == n - 1 /\
              ln v)
...
let rec close_exp_ln (e:src_exp) (v:var) (n:nat)
  ...
  = admit()
let close_ty_ln (e:src_ty) (v:var) (n:nat)
  : Lemma 
...
let rec open_ty_freevars (t:src_ty) (v:src_exp) (n:nat)
  : Lemma ((freevars_ty t `Set.subset` freevars_ty (open_ty' t v n))  /\
