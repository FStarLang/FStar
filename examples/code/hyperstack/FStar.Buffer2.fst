...
let sel #a h (b:buffer a) : GTot (seq a) = HS.sel #(bounded_seq a) h b.content
...
let get_ref #a (b:buffer a) : GTot (Heap.ref (bounded_seq a)) = as_ref (b.content)
...
let modifies_none h : Lemma (requires (True)) (ensures (modifies Set.empty h h)) = ()
...
let modifies_fresh #a h0 h1 bufs refs (b:buffer a) :
  Lemma
    (requires (~(contains h0 b) /\ contains h1 b
	       /\ modifies_buf (frameOf (content b)) (bufs ++ b) refs h0 h1 ))
    (ensures (modifies_buf (frameOf (content b)) bufs refs h0 h1))
    [SMTPatT (~(contains h0 b)); SMTPatT (modifies_buf (frameOf (content b)) (bufs ++ b) refs h0 h1)]
 = admit() // Check
...
let create #a (init:a) (len:u32) : 
  ST (buffer a)
     (requires (fun h -> True))
     (ensures (fun (h0:mem) b h1 ->
       ~(contains h0 b) /\ live h1 b /\ frameOf (content b) = h0.tip /\ idx b = 0 /\ length b = v len 
...
let index #a (b:buffer a) (n:u32{v n<length b}) : 
  STL a
...
(* let lemma_aux (#a:Type) (#a':Type) (b:buffer a) (h0:mem) (h1:mem) (b':buffer a') : Lemma *)
...
let upd #a (b:buffer a) (n:u32{v n < length b}) z :
  STL unit
     (requires (fun h -> live h b))
...
let sub #a (b:buffer a) (i:u32) (len:u32{v len <= length b /\ v i + v len <= length b}) :
  STL (buffer a)
     (requires (fun h -> live h b))
...
private val blit_aux: #a:Type -> b:buffer a -> idx_b:u32{v idx_b<=length b} -> 
				 b':buffer a{disjoint b b'} -> idx_b':u32{v idx_b'<=length b'} -> 
				 len:u32{v idx_b+v len <= length b /\ v idx_b'+v len <= length b'} -> 
  ...
  (ensures (fun h0 _ h1 -> 
    live h1 b /\ live h1 b' /\ live h0 b /\ live h0 b' 
    /\ length b >= v idx_b+v len /\ length b' >= v idx_b'+v len
...
val of_seq: #a:Type -> s:seq a -> l:u32{v l = Seq.length s /\ v l > 0} -> ST (buffer a)
  (requires (fun h -> True))
  (ensures (fun h0 b h1 -> 
    idx b = 0 /\ length b = v l /\ ~(contains h0 b) /\ live h1 b
