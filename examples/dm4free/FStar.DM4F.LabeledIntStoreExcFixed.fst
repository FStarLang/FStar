module FStar.DM4F.LabeledIntStoreExcFixed
...
let read (i:id)
  : INT_STORE_EXC (int * label) (fun s0 p -> p (Some (index s0 i), s0))
=
  let store = ISE?.get () in
  index store i


let write (i:id) (x:int * label)
  : INT_STORE_EXC unit (fun s0 p -> p (Some (), upd s0 i x))
=
  let store = ISE?.get () in
  ISE?.put (upd store i x)

open FStar.DM4F.LabeledIntStoreFixed
