...
let right_unit (a:Type) (f:st a) : Lemma (bind_st f return_st = f)
  = assert (feq (bind_st f return_st) f)
  
let left_unit (a:Type) (b:Type) (x:a) (f:a -> Tot (st b)) : Lemma (bind_st (return_st x) f = f x)
...
let return_ex (#a:Type) (x:a) : ex a = fun _ -> Some x
let bind_ex (#a:Type) (#b:Type) (f:ex a) (g: a -> Tot (ex b)) : ex b =
  fun _ -> match f () with 
        | None -> None
	| Some x -> g x ()
...
let return_stexn (#a:Type) (x:a) : stexn a = fun s -> Some x, s
let bind_stexn (#a:Type) (#b:Type) (f:stexn a) (g: a -> Tot (stexn b)) : stexn b =
  fun s0 -> match f s0 with 
        | None, s1 -> None, s1
	| Some x, s1 -> g x s1

let right_unit_stexn (a:Type) (f:stexn a) : Lemma (bind_stexn f return_stexn = f)
  = assert (feq (bind_stexn f return_stexn) f)
  
let left_unit_stexn (a:Type) (b:Type) (x:a) (f:a -> Tot (stexn b)) : Lemma (bind_stexn (return_stexn x) f = f x)
...
let lift_st_stexn (#a:Type) (f:st a) : stexn a = fun s0 -> let x, s1 = f s0 in Some x, s1
let lift_ex_stexn (#a:Type) (f:ex a) : stexn a = fun s0 -> f (), s0
...
let return_exnst (#a:Type) (x:a) : exnst a = fun s -> Some (x, s)
let bind_exnst (#a:Type) (#b:Type) (f:exnst a) (g: a -> Tot (exnst b)) : exnst b =
  fun s0 -> match f s0 with 
        | None -> None
	| Some (x, s1) -> g x s1
let right_unit_exnst (a:Type) (f:exnst a) : Lemma (bind_exnst f return_exnst = f)
  = assert (feq (bind_exnst f return_exnst) f)
  
let left_unit_exnst (a:Type) (b:Type) (x:a) (f:a -> Tot (exnst b)) : Lemma (bind_exnst (return_exnst x) f = f x)
...
let lift_st_exnst (#a:Type) (f:st a) : exnst a = fun s0 -> Some (f s0)
let lift_ex_exnst (#a:Type) (f:ex a) : exnst a = fun s0 -> 
  match f () with 
  | None -> None
  | Some x -> Some (x, s0)
