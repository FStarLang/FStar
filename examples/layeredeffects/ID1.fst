...
let ireturn (a : Type) (x : a) : irepr a (w_return x) =
 ...
 // Keep that in mind all ye who enter here.
  fun p _ -> interp_ret x;
          m_return x

let bind_str1 #a #b (wp : w a) (f : a -> w b) :
                    Lemma (requires (w_bind wp f (fun _ -> True)))
                          (ensures (wp (fun _ -> True))) = ()

// not true... only true if `wp` is somehow unprecise, as in
// ranging over every possible result in the type a.
//let bind_str2 a b (wp : w a) (f : a -> w b) :
//  Lemma (requires (w_bind a b wp f (fun _ -> True)))
//        (ensures (forall x. f x (fun _ -> True))) = admit ()

let ibind (a b : Type) (wp_v : w a) (wp_f: a -> w b) (v : irepr a wp_v) (f : (x:a -> irepr b (wp_f x))) : irepr b (w_bind wp_v wp_f) =
  fun p _ ->
  f (v (fun x -> wp_f x p) ()) p ()

let isubcomp (a:Type) (wp1 wp2: w a) (f : irepr a wp1) : Pure (irepr a wp2) (requires w_ord wp2 wp1) (ensures fun _ -> True) =
  fun p _ -> //w_ord_trans a wp2 wp1 (interp (f p ()));
          f p () 
