...
(* This example used to define GST as a layered effect.  The effect declaration
   has been removed; the state examples use the underlying indexed monad
   directly. *)
...
let (let!) #a #b #t0 #t1 (m : gst a t0) (f : a -> gst b t1)
  : gst b (t0 `op` t1)
  = bind a b t0 t1 m f
...
let read () : gst state R = gst_read
let write (s:state) : gst unit RW = gst_write s
