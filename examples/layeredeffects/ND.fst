...
val test_f : unit -> ND int (coerce_to_pure_wp (fun p -> p 5 /\ p 3))
let test_f () =
...
effect Nd (a:Type) (pre:pure_pre) (post:pure_post' a pre) =
        ND a (coerce_to_pure_wp (fun (p:pure_post a) -> pre /\ (forall (pure_result:a). post pure_result ==> p pure_result)))

val choose : #a:Type0 -> x:a -> y:a -> ND a (coerce_to_pure_wp (fun p -> p x /\ p y))
let choose #a x y =
    ND?.reflect (fun _ _ -> [x;y])

val fail : #a:Type0 -> unit -> ND a (coerce_to_pure_wp (fun p -> True))
let fail #a () =
    ND?.reflect (fun _ _ -> [])

let flip () : ND bool (coerce_to_pure_wp (fun p -> p true /\ p false)) =
    choose true false

let test () : ND int (coerce_to_pure_wp (fun p -> forall (x:int). 0 <= x /\ x < 10 ==> p x))  by (compute ()) =
    let x = choose 0 1 in
    let y = choose 2 3 in
    let z = choose 4 5 in
    x + y + z
...
[@expect_failure]
let test_bad () : ND int (coerce_to_pure_wp (fun p -> forall (x:int). 0 <= x /\ x < 5 ==> p x)) by (compute ()) =
    let x = choose 0 1 in
    let y = choose 2 3 in
    let z = choose 4 5 in
    x + y + z
...
let rec pick_from #a (l : list a) : ND a (coerce_to_pure_wp (fun p -> forall x. memP x l ==> p x)) by (compute ()) =
    match l with
    | [] -> fail ()
    | x::xs ->
      if flip ()
...
let pyths () : ND (int & int & int) (coerce_to_pure_wp (fun p -> forall x y z. x*x + y*y == z*z ==> p (x,y,z))) by (compute ()) =
