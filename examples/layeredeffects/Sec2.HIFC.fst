...
let no_leakage #a #p #q (f:hst a p q) (from to:loc) = forall k. {:nopattern (* override *)} no_leakage_k f from to k //nopattern: prevents matching loop with no_leakage_k
...
let bind_hst_no_leakage (#a #b:Type)
    ...
    = let f = bind_ifc' x y in
      ...
      then begin
        ...
        assert (v0 == v0');
        assert (forall l. {:nopattern (* override *)} l <> from ==> sel s1 l == sel s1' l); //nopattern: prevents matching loop with sel
