...
let read_bind
  ...
  (post_err_f: pure_post_err pre_f)
  ([@@@ defer_to refl_implicit] l_f:memory_invariant)
  (pre_g: (x:a) -> pure_pre) (post_g: (x:a) -> pure_post' b (pre_g x))
  (post_err_g: (x: a) -> pure_post_err (pre_g x))
  ([@@@ defer_to refl_implicit] l_g: memory_invariant)
  ([@@@ defer_to refl_implicit] pr:squash (l_f == l_g))
...
let bind (a:Type) (b:Type)
  (r_in_f:parser) ([@@@ defer_to refl_implicit] r_out_f: parser)
  (pre_f: pre_t r_in_f) (post_f: post_t a r_in_f r_out_f pre_f)
  (post_err_f: post_err_t r_in_f pre_f)
  ([@@@ defer_to refl_implicit] l_f: memory_invariant)
  ([@@@ defer_to refl_implicit] r_in_g:parser)
  (r_out_g: parser)
  (pre_g: (x:a) -> pre_t r_in_g) (post_g: (x:a) -> post_t b r_in_g r_out_g (pre_g x))
  (post_err_g: (x:a) -> post_err_t r_in_g (pre_g x))
  ([@@@ defer_to refl_implicit] l_g: memory_invariant)
  ([@@@ defer_to refl_implicit] pr:squash (l_f == l_g))
  ([@@@ defer_to refl_implicit] pr':squash (r_out_f == r_in_g))
...
inline_for_extraction
let subcomp (a:Type)
  ([@@@defer_to refl_implicit] r_in:parser)
  ([@@@ defer_to refl_implicit] r_out: parser)
  (pre: pre_t r_in) (post: post_t a r_in r_out pre) (post_err: post_err_t r_in pre)
  ([@@@ defer_to refl_implicit] l:memory_invariant)
  ([@@@defer_to refl_implicit] r_in':parser)
  ([@@@ defer_to refl_implicit] r_out': parser)
  (pre': pre_t r_in') (post': post_t a r_in' r_out' pre') (post_err': post_err_t r_in' pre')
  ([@@@ defer_to refl_implicit] l' : memory_invariant)
  ([@@@ defer_to refl_implicit] pr1:squash (r_in == r_in'))
  ([@@@ defer_to refl_implicit] pr2:squash (r_out == r_out'))
...
let if_then_else (a:Type)
  ...
  ([@@@ defer_to refl_implicit] pr1:squash (r_in == r_in'))
  ([@@@ defer_to refl_implicit] pr2:squash (r_out == r_out'))
  ([@@@ defer_to refl_implicit] pr3:squash (l == l'))
