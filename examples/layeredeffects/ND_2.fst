...
let dm (a : Type) (wp : w a) : Type = 
  ( pre:Type0 & (squash pre -> c:(m a){wp `w_ord` interp c}) )
...
= let wp = w_bind wp_v wp_f in
  let pre : Type0 = dfst v /\
                         // conjmapl (fun x -> dfst (f x)) (dsnd v ()) // precise! ... but a pain
                         (forall x. dfst (f x))
  in
  (| pre, (fun _ -> 
             let bm = dsnd v () in
...
let lift_pure_nd (a:Type) (wp:pure_wp a) (f:(eqtype_as_type unit -> PURE a wp)) :
  Pure (irepr a wp) (requires True)
                    (ensures (fun _ -> True))
  = (| wp (fun _ -> True), (fun _ -> let x = Common.elim_pure f (fun _ -> True) in
                            assume (pure_monotonic wp); // same old same old
                            [x] ) |)
...
let choose #a x y =
    ND?.reflect (| True, (fun _ -> [x;y]) |)

val fail : #a:Type0 -> unit -> ND a (fun p -> True)
let fail #a () =
    // Note the True!!! This is not a partial element.
    ND?.reflect (| True, (fun _ -> []) |)
