    ...
    for f in os.listdir(dir):
        ...
        if m is not None:
            ...
            if best_offer == None or mt > best_offer[1]:
                version, githash, bitness, platform, pversion = m.groups()
                best_offer = (fp, mt, version, githash, bitness, platform, pversion)
    return best_offer
...
def get(log, binary_name, platform, bitness, Tested=True):
    wd = os.getcwd()
    try:
        if Tested:
        ...
        for f in os.listdir(bsdir):
            ...
            if m is not None:
                ...
                version, githash, fbitness, fplatform, pversion = m.groups()
...
def print_help():
    print("Usage: %s (get-tested|get-latest|push-latest)" % sys.argv[0])
...
if  __name__ =='__main__':    
    if len(sys.argv) < 2:
        ...
        if op == "get-tested":
            ...
            if len(sys.argv) >= 4:
                bits = sys.argv[3]
            r = get(log, bn, pfm, bits, Tested=True)
        elif op == "get-latest":
            bn, pfm, bits = get_platform()
            if len(sys.argv) >= 3:
                pfm = sys.argv[2]
            if len(sys.argv) >= 4:
                bits = sys.argv[3]
            r = get(log, bn, pfm, bits, Tested=False)
        elif op == "push-latest":
            r = push_latest()
        else:
