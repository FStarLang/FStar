...
type clear_t ('a::*) ('Inv::heap => E) (fp:refset) (ghostdata:array (option 'a)) (size:pos) = 
    Writer unit (Requires (On fp 'Inv))
                (Ensures _ (fun h u h' => 
                          (On fp 'Inv h'
...
type head_t ('a::*) ('Inv::heap => E) (fp:refset) (ghostdata:array (option 'a)) (size:pos) = 
    Reader 'a (Requires (fun h => (On fp 'Inv h && (GT (Length (SelHeap h ghostdata)) 0))))
              (Provides _ (fun h a => (a=(Some_proj_1 (Index (SelHeap h ghostdata) 0)))))

type enqueue_t ('a::*) ('Inv::heap => E) (fp:refset) (ghostdata:array (option 'a)) (size:pos) (v:'a) = 
    Writer unit (Requires (fun h => (On fp 'Inv h && LT (Length (SelHeap h ghostdata)) size)))
                (Ensures _ (fun h u h' =>
                     (On fp 'Inv h'
...
type dequeue_t ('a::*) ('Inv::heap => E) (fp:refset) (ghostdata:array (option 'a)) (size:pos) = 
    Writer 'a (Requires (fun h => (On fp 'Inv h && GT (Length (SelHeap h ghostdata)) 0)))
              (Ensures _ (fun h v h' =>
                   (On fp 'Inv h'
...
type record ('a::*) ('Inv::heap => E) (fp:refset) (ghostdata:array (option 'a)) (size:pos) = 
    { 
      clear:   unit -> clear_t 'a 'Inv fp ghostdata size;
      head:    unit -> head_t 'a 'Inv fp ghostdata size;
      enqueue: v:'a -> enqueue_t 'a 'Inv fp ghostdata size v;
      dequeue: unit -> dequeue_t 'a 'Inv fp ghostdata size
    }
...
assume InvariantDef: forall ('a::*) (data:array (option 'a)) (start:ref nat) (len:ref nat) 
                            (ghostdata:array (option 'a)) (size:pos) (h:heap).
...
let create ('a::*) (size:pos) =
  ...
  let clear (u:unit) : clear_t 'a (Invariant 'a data start len ghostdata size) ghostfp ghostdata size =
    (len := 0);
    (ghostdata := Array.Emp) in 

  let head (u:unit) : head_t 'a (Invariant 'a data start len ghostdata size) ghostfp ghostdata size = 
    match Index (!data) (!start) with
      | None -> unreachable ()
      | Some v -> v in

  let enqueue (v:'a) : enqueue_t 'a (Invariant 'a data start len ghostdata size) ghostfp ghostdata size v = 
    let s = !start in 
    let l = !len in 
    let nextEmpty = 
      if (s + l) < size
  ...
  let dequeue (u:unit) : dequeue_t 'a (Invariant 'a data start len ghostdata size) ghostfp ghostdata size = 
      match Index (!data) (!start) with
        | None -> unreachable()
        | Some v -> 
            let g = !ghostdata in 
