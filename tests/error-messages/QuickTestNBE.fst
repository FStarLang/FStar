...
let quickProc_wp (a:Type0) : Type u#0 = (s0:vale_state) -> (wp_continue:vale_state -> a -> prop) -> prop

let t_ensure (#a:Type0) (wp:quickProc_wp a) (s0:vale_state) (k:(vale_state -> a -> prop)) =
  fun (sM, g) -> k sM g

let t_proof (#a:Type0) (wp:quickProc_wp a) : Type =
  s0:vale_state -> k:(vale_state -> a -> prop) -> Ghost (vale_state & a)
    (requires wp s0 k)
...
let labeled_wrap (r:range) (msg:string) (p:prop) : GTot prop = labeled r msg p

[@va_qattr "opaque_to_smt"]
let label (r:range) (msg:string) (p:prop) : Ghost prop (requires True) (ensures fun q -> q <==> p) =
...
[@va_qattr]
let rec wp (#a:Type0) (qcs:quickCodes a) (k:vale_state -> a -> prop) (s0:vale_state) :
  Tot prop (decreases %[0; qcs])
  =
  match qcs with
  | QEmpty g -> k s0 g
  | QPURE r msg pre l qcs ->
...
assume val qblock_proof (#a:Type) (qcs:vale_state -> GTot (quickCodes a)) (s0:vale_state) (k:vale_state -> a -> prop)
  : Ghost (vale_state & a)
...
[@va_qattr]
unfold let wp_sound_code_pre (#a:Type0) (qc:quickCode a) (s0:vale_state) (k:(s0':vale_state{s0 == s0'}) -> vale_state -> a -> prop) : prop =
  forall (ok:bool). QProc?.wp qc s0 (k s0)

unfold let wp_sound_code_post (#a:Type0) (qc:quickCode a) (s0:vale_state) (k:(s0':vale_state{s0 == s0'}) -> vale_state -> a -> prop) ((sN:vale_state), (gN:a)) : prop =
  k s0 sN gN

unfold let normal_steps : list string = [`%QProc?.wp;]
unfold let normal (x:prop) : prop = norm [nbe; iota; zeta; simplify; primops; delta_attr [`%va_qattr]; delta_only normal_steps] x

assume val wp_sound_code_norm (#a:Type0) (qc:quickCode a) (s0:vale_state) (k:(s0':vale_state{s0 == s0'}) -> vale_state -> a -> prop) : Lemma 
