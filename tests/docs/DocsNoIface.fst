...
[@@doc "Doubles 'x'. Documentation may also be attached to a definition in a module with no interface."]
...
(* Well-typed -- 'doc' takes a string and this is a string -- but
   attributes are recorded in the checked module unnormalized, so what
   is stored is the unevaluated application, not a literal. The export
   warns (warning 278) and skips this declaration rather than inventing
   text for it. *)
[@@doc (greeting ^ ", world")]
