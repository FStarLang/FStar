...
(* another try, with a raw PURE computation type *)
val plouf4 : s:sub -> f:exp -> PURE unit (requires True)
  (ensures fun _ -> esubst (sub_elam (sub_elam (sub_elam s))) (test f) = test (esubst s f))
