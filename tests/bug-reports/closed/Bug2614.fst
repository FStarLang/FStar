...
let test_exists_intro (x:int) (y:t x) (_: (p x y)) =
  introduce exists (a:int) (b:t a). p a b
  with x y
  and ()

//Ugly workaround
let test_exists_intro_workaround (x:int) (y:t x) (_: (p x y)) =
...
let test_exists_elim (_: (exists (x:int) (y:t x). p x y)) =
  eliminate exists (x:int) (y:t x). p x y
  returns True
  with _. ()

let test_forall_intro (f:(x:int -> y:t x -> (p x y))) =
  introduce forall (x:int) (y: t x). p x y
  with f x y

// Previously led to
//(Error 230) Variable "x#1492" not found
let test_forall_elim (a:int) (b:t a) (_: (forall (x:int) (y:t x). p x y)) =
  eliminate forall (x:int) (y:t x). p x y
  with a b

// //Ugly workaround
let test_forall_elim_workaround (a:int) (b:t a) (_: (forall (x:int) (y:t x). p x y)) =
