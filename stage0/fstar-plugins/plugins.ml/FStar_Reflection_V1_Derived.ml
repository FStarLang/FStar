open Fstarcompiler
open Prims
let bv_of_binder (b : FStarC_Reflection_Types.binder) :
...
let rec inspect_ln_unascribe (t : FStarC_Reflection_Types.term) :
  ...
  | FStarC_Reflection_V1_Data.Tv_AscribedC (t', uu___, uu___1, uu___2) ->
      inspect_ln_unascribe t'
  | tv -> tv
let mk_binder (bv : FStarC_Reflection_Types.bv)
...
let mk_implicit_binder (bv : FStarC_Reflection_Types.bv)
  (sort : FStarC_Reflection_Types.typ) : FStarC_Reflection_Types.binder=
  FStarC_Reflection_V1_Builtins.pack_binder
    {
      FStarC_Reflection_V1_Data.binder_bv = bv;
...
let rec collect_app_ln' (args : FStarC_Reflection_V1_Data.argv Prims.list)
  ...
  | FStarC_Reflection_V1_Data.Tv_App (l, r) -> collect_app_ln' (r :: args) l
  | uu___ -> (t, args)
let collect_app_ln :
...
let rec mk_app (t : FStarC_Reflection_Types.term)
  (args : FStarC_Reflection_V1_Data.argv Prims.list) :
  FStarC_Reflection_Types.term=
  match args with
  | [] -> t
  | x::xs ->
        ...
        (FStarC_Reflection_V1_Data.Tv_Arrow
           (b,
             (FStarC_Reflection_V1_Builtins.pack_comp
                (FStarC_Reflection_V1_Data.C_Total (mk_tot_arr_ln bs1 cod)))))
...
let is_uvar (t : FStarC_Reflection_Types.term) : Prims.bool=
  ...
  | uu___ -> false
let binder_set_qual (q : FStarC_Reflection_V1_Data.aqualv)
...
let rec is_name_imp (nm : FStarC_Reflection_Types.name)
  ...
  | FStarC_Reflection_V1_Data.Tv_App
      (l, (uu___, FStarC_Reflection_V1_Data.Q_Implicit)) -> is_name_imp nm l
  | uu___ -> false
