...
let (l_exact :
  ...
  fun t ->
    FStar_Tactics_V2_Derived.try_with
      ...
      (fun uu___ ->
         fun ps -> squash_intro () ps; FStar_Tactics_V2_Derived.exact t ps)
let _ =
  Fstarcompiler.FStarC_Tactics_Native.register_tactic
  ...
  fun tm ->
    fun ps ->
      ...
      | FStar_Reflection_V2_Formula.App (hd, arg) ->
          if term_eq hd tm
          then FStar_Tactics_V2_Derived.trivial () ps
          else ()
      | uu___ ->
          let x2 =
            FStar_Tactics_V2_Derived.destruct_equality_implication x ps in
          (match x2 with
           | FStar_Pervasives_Native.None ->
               FStar_Tactics_V2_Derived.fail "Not an equality implication" ps
           | FStar_Pervasives_Native.Some (uu___1, rhs) ->
                ...
                | () ->
                    (fun ps ->
                       ...
                       (let x2 =
                          ...
                          x3 <> Prims.int_one in
                        if x2
                        then FStar_Tactics_V2_Derived.fail "no" ps
                        else ());
