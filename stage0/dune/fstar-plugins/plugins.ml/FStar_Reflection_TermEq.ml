...
type _cmpres =
  ...
  | Neq 
  | Unknown 
let uu___is_Eq (projectee : _cmpres) : Prims.bool=
  match projectee with | Eq -> true | uu___ -> false
let uu___is_Neq (projectee : _cmpres) : Prims.bool=
...
let bv_cmp : FStarC_Reflection_Types.bv comparator_for=
  fun x1 x2 ->
    ...
    if
      v1.FStarC_Reflection_V2_Data.index = v2.FStarC_Reflection_V2_Data.index
    then Eq
    else Neq
let namedv_cmp : FStarC_Reflection_Types.namedv comparator_for=
  fun x1 x2 ->
    ...
    if v1.FStarC_Reflection_V2_Data.uniq = v2.FStarC_Reflection_V2_Data.uniq
    then Eq
    else Neq
...
let either_dec_cmp (uu___5 : 'c) (uu___4 : 'c)
  ...
  (uu___1 : ('a, 'b) Fstarcompiler.FStar_Pervasives.either)
  (uu___ : ('a, 'b) Fstarcompiler.FStar_Pervasives.either) :
  (('a, 'b) Fstarcompiler.FStar_Pervasives.either, Obj.t, Obj.t) cmpres=
...
let rec univ_cmp : FStarC_Reflection_Types.universe comparator_for=
  fun u1 u2 ->
    ...
    | (FStarC_Reflection_V2_Data.Uv_Unif u11,
       FStarC_Reflection_V2_Data.Uv_Unif u21) -> Obj.magic (Obj.repr Unknown)
...
let const_cmp : FStarC_Reflection_V2_Data.vconst comparator_for=
  fun c1 c2 ->
    ...
    | (FStarC_Reflection_V2_Data.C_Char c11, FStarC_Reflection_V2_Data.C_Char
       c21) -> Obj.magic (Obj.repr (eq_cmp c11 c21))
    | uu___ -> Obj.magic (Obj.repr Neq)
