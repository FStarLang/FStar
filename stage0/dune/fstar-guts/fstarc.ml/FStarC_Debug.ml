...
let any (uu___ : unit) : Prims.bool=
  let uu___1 = FStarC_Effect.op_Bang anyref in
...
let low (uu___ : unit) : Prims.bool=
  let uu___1 =
    let uu___2 = FStarC_Effect.op_Bang dbg_level in uu___2 >= Prims.int_one in
  if uu___1 then true else FStarC_Effect.op_Bang _debug_all
...
let enable_toggles (keys : Prims.string Prims.list) : unit=
  ...
  FStarC_List.iter
    (fun k ->
       ...
       | uu___1 ->
           let uu___2 =
             if (FStarC_String.length k) > Prims.int_zero
             then
               let uu___3 = FStarC_String.get k Prims.int_zero in uu___3 = 45
             else false in
           if uu___2
           then
             let k1 =
