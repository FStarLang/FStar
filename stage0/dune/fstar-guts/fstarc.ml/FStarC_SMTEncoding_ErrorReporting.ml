...
let uu___is_Not_a_wp_implication (projectee : Prims.exn) : Prims.bool=
  match projectee with | Not_a_wp_implication uu___ -> true | uu___ -> false
let __proj__Not_a_wp_implication__item__uu___ (projectee : Prims.exn) :
...
type ranges =
  (Prims.string FStar_Pervasives_Native.option * FStarC_Range_Type.t)
    Prims.list
let __ctr : Prims.int FStarC_Effect.ref= FStarC_Effect.mk_ref Prims.int_zero
...
let fresh_label (message : FStarC_Errors_Msg.error_message)
  (range : FStarC_Range_Type.t) (t : FStarC_SMTEncoding_Term.term) :
  (label * FStarC_SMTEncoding_Term.term)=
  let l =
    incr __ctr;
    (let uu___1 =
       let uu___2 = FStarC_Effect.op_Bang __ctr in
       FStarC_Class_Show.show FStarC_Class_Show.showable_int uu___2 in
     FStarC_Format.fmt1 "label_%s" uu___1) in
  let lvar =
...
let label_goals
  ...
  | (flag, msg_prefix) ->
      ...
      let rec aux default_msg ropt post_name_opt labels1 q1 =
        ...
        | FStarC_SMTEncoding_Term.Labeled (arg, d::[], label_range) when
            let uu___1 = FStarC_Errors_Msg.renderdoc d in
            uu___1 = "Could not prove post-condition" ->
            let fallback debug_msg =
              aux default_msg (FStar_Pervasives_Native.Some label_range)
                           ...
                           let post_name =
                             let uu___3 =
                               ...
                               FStarC_Class_Show.show
                                 FStarC_Class_Show.showable_int uu___4 in
                          ...
                          let uu___4 =
                            let uu___5 =
                              let uu___6 =
                                ...
                                FStarC_Class_Show.show
                                  FStarC_Class_Show.showable_int uu___7 in
                              Prims.strcat "^^" uu___6 in
                            (uu___5, s) in
                       ...
                       | (labels2, lhs_conjs) ->
                           let uu___5 =
                             aux default_msg FStar_Pervasives_Native.None
                               (FStar_Pervasives_Native.Some new_post_name)
                               labels2 rhs1 in
                           (match uu___5 with
                            | (labels3, rhs2) ->
                                let body =
...
let detail_errors (hint_replay : Prims.bool)
  ...
  (askZ3 :
    FStarC_SMTEncoding_Term.decl Prims.list -> FStarC_SMTEncoding_Z3.z3result)
  : unit=
  ...
  let rec linear_check eliminated errors active =
    FStarC_SMTEncoding_Z3.refresh
      (FStar_Pervasives_Native.Some (env.FStarC_TypeChecker_Env.proof_ns));
    (match active with
     | [] ->
         let results =
           let uu___1 = FStarC_List.map (fun x -> (x, true)) eliminated in
           let uu___2 = FStarC_List.map (fun x -> (x, false)) errors in
     ...
     | hd::tl ->
         ((let uu___2 =
           ...
           FStarC_Format.print1 "%s, " uu___2);
          (let decls =
             elim
               (FStarC_List.op_At eliminated (FStarC_List.op_At errors tl)) in
