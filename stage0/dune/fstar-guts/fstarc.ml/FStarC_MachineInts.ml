...
let (module_name_for : machint_kind -> Prims.string) =
  fun k ->
    ...
    | UInt128 -> "UInt128"
    | SizeT -> "SizeT"
let (mask : machint_kind -> Prims.int) =
  fun k ->
...
let (uu___is_Mk : machint_kind -> Obj.t machint -> Prims.bool) =
  fun k -> fun projectee -> true
let (__proj__Mk__item___0 : machint_kind -> Obj.t machint -> Prims.int) =
...
let (v : machint_kind -> Obj.t machint -> Prims.int) =
...
let (make_as : machint_kind -> Obj.t machint -> Prims.int -> Obj.t machint) =
  fun k -> fun x -> fun z -> let uu___ = meta k x in Mk (z, uu___)
let (showable_bounded_k :
  machint_kind -> Obj.t machint FStarC_Class_Show.showable) =
  fun k ->
    {
...
let (e_machint :
  machint_kind -> Obj.t machint FStarC_Syntax_Embeddings_Base.embedding) =
  fun k ->
    let with_meta_ds r =
      fun t ->
        fun m ->
...
let (nbe_machint :
  machint_kind -> Obj.t machint FStarC_TypeChecker_NBETerm.embedding) =
  fun k ->
    let with_meta_ds t =
      fun m ->
        match m with
