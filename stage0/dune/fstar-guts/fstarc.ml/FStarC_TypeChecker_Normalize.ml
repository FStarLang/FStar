...
let read_memo (cfg : FStarC_TypeChecker_Cfg.cfg)
  (r : (FStarC_TypeChecker_Cfg.cfg * 'a) FStarC_Syntax_Syntax.memo) :
...
let set_memo (cfg : FStarC_TypeChecker_Cfg.cfg)
  (r : (FStarC_TypeChecker_Cfg.cfg * 'a) FStarC_Syntax_Syntax.memo) (t : 'a)
  : unit=
  ...
  then
    ((let uu___1 =
     ...
     FStarC_Effect.op_Colon_Equals r (FStar_Pervasives_Native.Some (cfg, t)))
  else ()
...
let lookup_bvar (env1 : env) (x : FStarC_Syntax_Syntax.bv) : closure=
  try
    (fun uu___ ->
       match () with
       | () ->
           let uu___1 = FStarC_List.nth env1 x.FStarC_Syntax_Syntax.index in
...
let downgrade_ghost_effect_name (l : FStarC_Ident.lident) :
  ...
  let uu___ = FStarC_Ident.lid_equals l FStarC_Parser_Const.effect_Ghost_lid in
  if uu___
    ...
    | FStarC_Syntax_Syntax.U_bvar x ->
        (try
           (fun uu___ ->
              ...
              | () ->
                  let uu___1 =
                    let uu___2 = FStarC_List.nth env1 x in
                    FStar_Pervasives_Native.__proj__Mktuple3__item___2 uu___2 in
                  (match uu___1 with
                   | Univ u3 ->
                       ((let uu___3 = FStarC_Effect.op_Bang dbg_univ_norm in
                         if uu___3
                         then
                           let uu___4 =
                             FStarC_Class_Show.show
                   ...
                   | uu___2 ->
                       let uu___3 =
                         let uu___4 =
                           FStarC_Class_Show.show
                             FStarC_Class_Show.showable_int x in
                         FStarC_Format.fmt1
                ...
                then subst
                  ...
                  (let term1 = closure_as_term cfg env2 term in
                   ...
                   | FStar_Pervasives_Native.Some x ->
                       ...
                       let b_for_x =
                         let uu___7 =
                           let uu___8 = FStarC_Syntax_Syntax.bv_to_name b1 in
          ...
          | (head_term, universes) ->
              (match head_term.FStarC_Syntax_Syntax.n with
               ...
               | FStarC_Syntax_Syntax.Tm_constant
                   ...
                   Prims.op_Negation cfg.FStarC_TypeChecker_Cfg.strong ->
                   (FStarC_TypeChecker_Cfg.log_primops cfg
                      (fun uu___4 ->
                         let uu___5 =
                           FStarC_Class_Show.show
...
let nbe_eval (cfg : FStarC_TypeChecker_Cfg.cfg)
  ...
  (let tm_norm =
     ...
     uu___1.FStarC_TypeChecker_Env.nbe s cfg.FStarC_TypeChecker_Cfg.tcenv tm in
   FStarC_TypeChecker_Cfg.log_nbe cfg
     (fun uu___2 ->
        let uu___3 =
...
let rec maybe_weakly_reduced
  ...
  | FStarC_Syntax_Syntax.Tm_app
      ...
      (maybe_weakly_reduced t1) ||
        (FStarC_Util.for_some
           (fun uu___ ->
              match uu___ with | (a, uu___1) -> maybe_weakly_reduced a) args)
...
let decide_unfolding (cfg : FStarC_TypeChecker_Cfg.cfg)
  (stack1 : stack_elt Prims.list) (fv : FStarC_Syntax_Syntax.fv)
  (qninfo : FStarC_TypeChecker_Env.qninfo) :
  (FStarC_TypeChecker_Cfg.cfg FStar_Pervasives_Native.option * stack_elt
  ...
  | FStarC_TypeChecker_Normalize_Unfolding.Should_unfold_reify ->
      ...
      let ref =
        ...
        FStarC_Syntax_Syntax.mk uu___ FStarC_Range_Type.dummyRange in
      let stack2 =
        push
          (App
  ...
  | FStarC_Syntax_Syntax.Tm_app
      ...
      (match uu___1 with
       | FStarC_Syntax_Syntax.Tm_fvar fv when
           (is_on_dom fv) && ((FStarC_List.length args) = (Prims.of_int (3)))
           ->
           let f =
             let uu___2 =
               let uu___3 =
                 let uu___4 = FStarC_List.tl args in FStarC_List.tl uu___4 in
...
let __get_n_binders :
  ...
  FStarC_Effect.mk_ref
    (fun e s n t -> failwith "Impossible: __get_n_binders unset")
...
let can_reify_for_extraction (env1 : FStarC_TypeChecker_Env.env)
  (m : FStarC_Ident.lident) : Prims.bool=
    ...
    | ((t, uu___)::args1, b::bs1) ->
        ...
        (match uu___1 with
         | FStarC_Syntax_Syntax.Tm_name bv' ->
             (FStarC_Syntax_Syntax.bv_eq b.FStarC_Syntax_Syntax.binder_bv bv')
...
let is_applied_maybe_squashed (cfg : FStarC_TypeChecker_Cfg.cfg)
  (bs : FStarC_Syntax_Syntax.binders) (t : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.bv FStar_Pervasives_Native.option=
  if (cfg.FStarC_TypeChecker_Cfg.debug).FStarC_TypeChecker_Cfg.wpe
  then
  ...
  (let uu___1 = FStarC_Syntax_Util.is_squash t in
   ...
   | uu___2 ->
       ...
       (match uu___3 with
        | FStar_Pervasives_Native.Some (uu___4, t') -> is_applied cfg bs t'
     ...
     let types_match bs =
       ...
       | (bs_q, uu___1) ->
           let rec unrefine_true t =
             ...
             | uu___3 -> t in
           ((FStarC_List.length bs) = (FStarC_List.length bs_q)) &&
             (FStarC_List.forall2
                (fun b1 b2 ->
                   let s1 =
                     unrefine_true
       ...
       let t' =
         FStarC_Syntax_Visit.visit_term false
           (fun t1 ->
              ...
              | (hd, args) ->
                  let uu___1 =
                    ((FStarC_List.length args) = arity) && (is_bv bv1 hd) in
                  if uu___1
                             ...
                             (fun uu___5 ->
                                (fun q' ->
                                   ...
                                   let phi' =
                                     ...
                                     let uu___6 =
                                       let uu___7 =
                                         FStarC_Syntax_Syntax.as_arg p in
                                       let uu___8 =
                                         let uu___9 =
           ...
           (let uu___4 = FStarC_Syntax_Subst.open_term' bs body in
            ...
            | (bs1, body1, opening) ->
                let env' =
                  FStarC_List.fold_left
                    (fun env2 uu___5 ->
                       let uu___6 = dummy () in uu___6 :: env2) env1 bs1 in
                let rc_opt1 =
                  Obj.magic
                    (FStarC_Class_Monad.op_let_Bang
                       FStarC_Class_Monad.monad_option () ()
              ...
              FStarC_List.fold_right
                (fun uu___2 stack4 ->
                   ...
                   | (a, aq) ->
                       let a1 =
                         let uu___3 =
                           (((let uu___4 = FStarC_TypeChecker_Cfg.cfg_env cfg in
                             ...
                             && (FStarC_Syntax_Util.aqual_is_erasable aq) in
                         if uu___3 then FStarC_Syntax_Util.exp_unit else a in
                       let env2 =
                         let uu___3 =
                           let uu___4 = FStarC_Syntax_Subst.compress a1 in
                           uu___4.FStarC_Syntax_Syntax.n in
                         match uu___3 with
                         | FStarC_Syntax_Syntax.Tm_name uu___4 -> empty_env
                         | FStarC_Syntax_Syntax.Tm_constant uu___4 ->
                             empty_env
                         | FStarC_Syntax_Syntax.Tm_lazy uu___4 -> empty_env
                         | FStarC_Syntax_Syntax.Tm_fvar uu___4 -> empty_env
                         | uu___4 -> env1 in
                       let uu___3 =
                         let uu___4 =
                           let uu___5 =
                             ...
                             Clos uu___6 in
                           (uu___5, aq, (t1.FStarC_Syntax_Syntax.pos)) in
                         Arg uu___4 in
        ...
        | FStar_Pervasives_Native.Some strict_args1 ->
            let norm_args =
              FStarC_List.map
                (fun uu___2 ->
                   match uu___2 with
                   | (a, i) -> let uu___3 = norm cfg env1 [] a in (uu___3, i))
                args in
            let norm_args_len = FStarC_List.length norm_args in
            let uu___2 =
              FStarC_List.for_all
                (fun i ->
                   if i >= norm_args_len
                   then false
                     ...
                     (let uu___4 = FStarC_List.nth norm_args i in
                      match uu___4 with
                      | (arg_i, uu___5) ->
                          let uu___6 =
                            let uu___7 = FStarC_Syntax_Util.unmeta_safe arg_i in
                          ...
                          (match uu___6 with
                           | (head1, uu___7) ->
                               ...
                               (match uu___8 with
                                | FStarC_Syntax_Syntax.Tm_constant uu___9 ->
                                    true
                                | FStarC_Syntax_Syntax.Tm_fvar fv ->
                                    let uu___9 =
                                      FStarC_Syntax_Syntax.lid_of_fv fv in
                                    FStarC_TypeChecker_Env.is_datacon
                                      cfg.FStarC_TypeChecker_Cfg.tcenv uu___9
                         ...
                         let uu___4 =
                           let uu___5 =
                             let uu___6 =
                               ...
                               Clos uu___7 in
                             (uu___6, aq, (t1.FStarC_Syntax_Syntax.pos)) in
                           Arg uu___5 in
   ...
   | FStarC_Syntax_Syntax.Tm_refine
       ...
       then
         ...
         (let t_x = norm cfg env1 [] x.FStarC_Syntax_Syntax.sort in
          let uu___3 =
            let uu___4 =
              let uu___5 = FStarC_Syntax_Syntax.mk_binder x in [uu___5] in
          ...
          (let t12 = norm cfg env1 [] t11 in
           ...
           (let asc1 = norm_ascription cfg env1 asc in
            let uu___6 =
              ...
              FStarC_Syntax_Syntax.mk uu___7 t1.FStarC_Syntax_Syntax.pos in
       ...
       then
         ...
         let env2 =
           let uu___3 =
             let uu___4 =
               let uu___5 =
                 let uu___6 = fresh_memo () in (env1, def, uu___6, false) in
               Clos uu___5 in
               ...
               | (bs, body1) ->
                   (FStarC_TypeChecker_Cfg.log cfg
                    ...
                    (let ty = norm cfg env1 [] lb.FStarC_Syntax_Syntax.lbtyp in
                     let lbname =
                       let x =
                         let uu___9 = FStarC_List.hd bs in
                     ...
                     (let lb1 =
                      ...
                      let env' =
                        FStarC_List.fold_left
                          (fun env2 uu___10 ->
                             let uu___11 = dummy () in uu___11 :: env2) env1
                          bs in
                      FStarC_TypeChecker_Cfg.log cfg
                        (fun uu___11 ->
                           FStarC_Format.print_string
   ...
   | FStarC_Syntax_Syntax.Tm_uvar uu___2 ->
       (if
        ...
        (let t2 =
         ...
         rebuild cfg env1 stack2 t2)))
and do_unfold_fv (cfg : FStarC_TypeChecker_Cfg.cfg)
  (stack1 : stack_elt Prims.list) (t0 : FStarC_Syntax_Syntax.term)
  (qninfo : FStarC_TypeChecker_Env.qninfo) (f : FStarC_Syntax_Syntax.fv) :
  FStarC_Syntax_Syntax.term=
  let defn uu___ =
  ...
  let maybe_warn_if_unfolding_plugin uu___ =
    let uu___1 =
      (((FStar_Pervasives_Native.uu___is_Some
          ...
          &&
          (let uu___2 = FStarC_Options.no_plugins () in
           Prims.op_Negation uu___2))
         &&
         (let uu___2 = FStarC_Effect.op_Bang plugin_unfold_warn_ctr in
...
and handle_norm_request (cfg : FStarC_TypeChecker_Cfg.cfg) (env1 : env)
  ...
  then
    (let uu___1 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term hd in
     let uu___2 =
       let uu___3 =
         let uu___4 = firstn (Prims.of_int (5)) stack1 in
         FStar_Pervasives_Native.fst uu___4 in
       FStarC_Class_Show.show
         (FStarC_Class_Show.show_list showable_stack_elt) uu___3 in
       ...
       (if debug
        then
          (let uu___2 =
             ...
             let uu___4 =
               let uu___5 =
                 let uu___6 = FStarC_Errors_Msg.text "Steps =" in
                 let uu___7 =
                   let uu___8 =
                     FStarC_Class_Show.show
                       (FStarC_Class_Show.show_list
   ...
   (let top2 = FStarC_Syntax_Util.unmeta_safe top1 in
                   ...
                   | FStar_Pervasives.Inl x ->
                       ...
                       (match uu___7 with
                        | FStar_Pervasives_Native.Some e ->
                            ...
                            let uu___8 = FStarC_List.tl stack1 in
                            let uu___9 =
                              let uu___10 =
                                let uu___11 =
                                  let uu___12 =
                                      ...
                                      (FStar_Pervasives_Native.Some m) in
                                  {
                                    ...
                                    FStarC_Syntax_Syntax.body1 = uu___12
                                  } in
                              ...
                              | FStar_Pervasives_Native.Some
                                  {
                                    ...
                                    FStarC_Syntax_Syntax.vars = uu___11;
                                    FStarC_Syntax_Syntax.hash_code = uu___12;_}
                                     ...
                                     let uu___13 =
                                       let uu___14 =
                                         let uu___15 =
                                           ...
                                           uu___16 :: uu___17 in
                                         (bind, uu___15) in
                                       FStarC_Syntax_Syntax.Tm_uinst uu___14 in
                                     FStarC_Syntax_Syntax.mk uu___13 rng
                                 | uu___11 ->
                                     failwith
                                       ...
                                       let uu___15 =
                                         let uu___16 =
                                           ...
                                           let uu___18 =
                                             ...
                                             [uu___19] in
                                           uu___17 :: uu___18 in
                                         FStarC_List.op_At range_args uu___16 in
                                       FStarC_List.op_At unit_args uu___15 in
                                     uu___13 :: uu___14 in
                                   uu___11 :: uu___12
                                 else
                                   (let maybe_range_arg =
                                    ...
                                    let uu___13 =
                                      let uu___14 =
                                        ...
                                        let uu___16 =
                                          let uu___17 =
                                            FStarC_Syntax_Syntax.as_arg f_arg in
                                          let uu___18 =
                                            let uu___19 =
                                              FStarC_Syntax_Syntax.as_arg
                                                FStarC_Syntax_Syntax.tun in
                                          ...
                                          uu___17 :: uu___18 in
                                        uu___15 :: uu___16 in
                                      FStarC_List.op_At maybe_range_arg
                                        uu___14 in
                                    FStarC_List.op_At uu___12 uu___13) in
                               let reified =
                                 let is_total_effect =
                                 ...
                                 then
                                   let uu___11 =
                                     let uu___12 =
                                       let uu___13 = bind_inst_args head in
                                       {
                                         FStarC_Syntax_Syntax.hd = bind_inst;
                                         FStarC_Syntax_Syntax.args = uu___13
                                       } in
                                     FStarC_Syntax_Syntax.Tm_app uu___12 in
                                   FStarC_Syntax_Syntax.mk uu___11 rng
                                 else
                                   (let uu___12 =
                                      let bv =
                                    ...
                                    | (lb_head, head_bv, head1) ->
                                        let uu___13 =
                                          ...
                                          FStarC_Syntax_Syntax.Tm_let uu___14 in
                                        FStarC_Syntax_Syntax.mk uu___13 rng) in
                               FStarC_TypeChecker_Cfg.log cfg
                                 (fun uu___11 ->
                                    let uu___12 =
                                      FStarC_Class_Show.show
                                    ...
                                    FStarC_Format.print2
                                      "Reified (1) <%s> to %s\n" uu___12
                                      uu___13);
                               (let uu___11 = FStarC_List.tl stack1 in
                   ...
                   | FStarC_Syntax_Syntax.Tm_meta
                       { FStarC_Syntax_Syntax.tm2 = e0;
                         FStarC_Syntax_Syntax.meta =
                           ...
                           (m1, m2, t');_}
                       ->
                       let uu___7 = FStarC_Syntax_Util.is_pure_effect m1 in
                       Prims.op_Negation uu___7
                   | uu___7 -> false) in
            let uu___5 =
              let uu___6 =
                let uu___7 = FStarC_Syntax_Syntax.as_arg head in uu___7 ::
                  args in
              FStarC_Util.for_some is_arg_impure uu___6 in
            (if uu___5
             then
               let uu___6 =
                 let uu___7 =
               ...
               FStarC_Errors.log_issue
                 (FStarC_Syntax_Syntax.has_range_syntax ()) top2
            ...
            FStarC_TypeChecker_Cfg.log cfg
              (fun uu___6 ->
                 let uu___7 =
                   FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                     top0 in
                 FStarC_Format.print2 "Reified (2) <%s> to %s\n" uu___7 "");
            (let uu___6 = FStarC_List.tl stack1 in
             let uu___7 =
            ...
            FStarC_TypeChecker_Cfg.log cfg
              (fun uu___6 ->
                 let uu___7 =
                   FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                     top0 in
                 FStarC_Format.print2 "Reified (3) <%s> to %s\n" uu___7 "");
            (let uu___6 = FStarC_List.tl stack1 in
             let uu___7 =
               FStarC_Syntax_Syntax.mk
                 (FStarC_Syntax_Syntax.Tm_meta
                   ...
                   (let t1 =
                      let uu___9 =
                        FStarC_Syntax_Util.mk_reify head
                          (FStar_Pervasives_Native.Some m) in
                      FStarC_Syntax_Syntax.mk_Tm_app uu___9 args
                        t.FStarC_Syntax_Syntax.pos in
                    let uu___9 = FStarC_List.tl stack1 in
    ...
    | FStarC_Syntax_Syntax.Tm_meta
        ...
        (FStarC_TypeChecker_Cfg.log cfg
         ...
         (let uu___4 = FStarC_List.tl stack1 in norm cfg env1 uu___4 lifted))
    | FStarC_Syntax_Syntax.Tm_match
        ...
        let uu___3 = FStarC_List.tl stack1 in norm cfg env1 uu___3 tm
    | uu___3 -> fallback ()))
and reify_lift (cfg : FStarC_TypeChecker_Cfg.cfg)
  ...
  (let uu___1 =
     ((FStarC_Syntax_Util.is_pure_effect msrc) ||
       ...
       &&
       (let uu___2 = FStarC_TypeChecker_Env.is_layered_effect env1 mtgt in
   ...
   then
     ...
     | (uu___3, repr) ->
         ...
         (match uu___4 with
          | (uu___5, return_repr) ->
              let return_inst =
                let uu___6 =
                  let uu___7 = FStarC_Syntax_Subst.compress return_repr in
                  uu___7.FStarC_Syntax_Syntax.n in
                match uu___6 with
                | FStarC_Syntax_Syntax.Tm_uinst (return_tm, uu___7::[]) ->
                    let uu___8 =
                      let uu___9 =
                        let uu___10 =
                          let uu___11 =
                    ...
                    FStarC_Syntax_Syntax.mk uu___8 e.FStarC_Syntax_Syntax.pos
                | uu___7 -> failwith "NIY : Reification of indexed effects" in
              let uu___6 =
                let bv =
                  FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None t in
                let lb =
                  let uu___7 =
                    let uu___8 =
                      let uu___9 = FStarC_Syntax_Syntax.as_arg t in [uu___9] in
                    FStarC_Syntax_Util.mk_app repr uu___8 in
                  {
                    FStarC_Syntax_Syntax.lbname = (FStar_Pervasives.Inl bv);
                    FStarC_Syntax_Syntax.lbunivs = [];
                    FStarC_Syntax_Syntax.lbtyp = uu___7;
                    FStarC_Syntax_Syntax.lbeff = msrc;
                    FStarC_Syntax_Syntax.lbdef = e;
                    FStarC_Syntax_Syntax.lbattrs = [];
                    FStarC_Syntax_Syntax.lbpos = (e.FStarC_Syntax_Syntax.pos)
                  } in
               ...
               | (lb_e, e_bv, e1) ->
                   let uu___7 =
                     let uu___8 =
                       let uu___9 =
                         let uu___10 =
                           let uu___11 = FStarC_Syntax_Syntax.mk_binder e_bv in
                           [uu___11] in
                         let uu___11 =
                           let uu___12 =
                             let uu___13 =
                               let uu___14 =
                                 let uu___15 = FStarC_Syntax_Syntax.as_arg t in
                                 let uu___16 =
                                   let uu___17 =
                                 ...
                                 uu___15 :: uu___16 in
                               {
                                 FStarC_Syntax_Syntax.hd = return_inst;
                   ...
                   FStarC_Syntax_Syntax.mk uu___7 e1.FStarC_Syntax_Syntax.pos))
   else
     (let uu___3 = FStarC_TypeChecker_Env.monad_leq env1 msrc mtgt in
      ...
      | FStar_Pervasives_Native.Some
          ...
          ->
          let e1 =
            let uu___8 = FStarC_TypeChecker_Env.is_reifiable_effect env1 msrc in
            if uu___8
            then
              ...
              (let uu___10 =
                 ...
                 FStarC_Syntax_Syntax.Tm_abs uu___11 in
               FStarC_Syntax_Syntax.mk uu___10 e.FStarC_Syntax_Syntax.pos) in
          let uu___8 = env1.FStarC_TypeChecker_Env.universe_of env1 t in
          lift uu___8 t e1))
       ...
       let effect_args =
         let uu___1 =
           ...
           then
             FStarC_List.map
               (fun uu___3 ->
                  FStarC_Syntax_Syntax.as_arg FStarC_Syntax_Syntax.unit_const)
           else
             FStarC_List.mapi
               (fun idx uu___4 ->
                  match uu___4 with
                  | (a, i) -> let uu___5 = norm cfg env1 [] a in (uu___5, i)) in
...
and maybe_simplify_aux (cfg : FStarC_TypeChecker_Cfg.cfg) (env1 : env)
  (stack1 : stack) (tm : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.term * Prims.bool)=
  let uu___ = let uu___1 = norm_cb cfg in reduce_primops uu___1 cfg env1 tm in
  match uu___ with
  | (tm1, renorm) ->
      if
        Prims.op_Negation
        ...
        (let w t =
         ...
         let rec clearly_inhabited ty =
           let uu___2 =
             ...
             uu___3.FStarC_Syntax_Syntax.n in
           match uu___2 with
           | FStarC_Syntax_Syntax.Tm_uinst (t, uu___3) -> clearly_inhabited t
           | FStarC_Syntax_Syntax.Tm_arrow
               { FStarC_Syntax_Syntax.bs1 = uu___3;
                 FStarC_Syntax_Syntax.comp = c;_}
               ->
               let uu___4 = FStarC_Syntax_Util.comp_result c in
               clearly_inhabited uu___4
           | FStarC_Syntax_Syntax.Tm_fvar fv ->
                  ...
                  then squashed_head_un_auto_squash_args tm1
                    ...
                    (let uu___13 =
                     ...
                     then
                       ...
                       | uu___15::(FStar_Pervasives_Native.Some false,
                                   uu___16)::[]
                           -> ((w FStarC_Syntax_Util.t_false), false)
                       ...
                       (let uu___15 =
                        ...
                        then
                          ...
                          match uu___16 with
                          | (FStar_Pervasives_Native.Some true, uu___17)::uu___18::[]
                              -> ((w FStarC_Syntax_Util.t_true), false)
                          | uu___17::(FStar_Pervasives_Native.Some true,
                                      uu___18)::[]
                              -> ((w FStarC_Syntax_Util.t_true), false)
                          ...
                          | (uu___17, (arg, uu___18))::(FStar_Pervasives_Native.Some
                              ...
                              let uu___20 = maybe_auto_squash arg in
                              (uu___20, false)
                          | uu___17 -> squashed_head_un_auto_squash_args tm1
                        else
                          (let uu___17 =
                             FStarC_Syntax_Syntax.fv_eq_lid fv
                           ...
                           if uu___17
                           then
                             let uu___18 = FStarC_List.map simplify args in
                             match uu___18 with
                             | uu___19::(FStar_Pervasives_Native.Some true,
                                         uu___20)::[]
                                 -> ((w FStarC_Syntax_Util.t_true), false)
                             | (FStar_Pervasives_Native.Some false, uu___19)::uu___20::[]
                                 -> ((w FStarC_Syntax_Util.t_true), false)
                             ...
                             | (uu___19, (p, uu___20))::(uu___21,
                                                         (q, uu___22))::[]
                                 ->
                                 let uu___23 = FStarC_Syntax_Util.term_eq p q in
                                 (if uu___23
                                  then ((w FStarC_Syntax_Util.t_true), false)
                             ...
                             (let uu___19 =
                              ...
                              if uu___19
                              then
                                let uu___20 = FStarC_List.map simplify args in
                                match uu___20 with
                                | (FStar_Pervasives_Native.Some true,
                                   uu___21)::(FStar_Pervasives_Native.Some
                                              true, uu___22)::[]
                                    -> ((w FStarC_Syntax_Util.t_true), false)
                                | (FStar_Pervasives_Native.Some false,
                                   uu___21)::(FStar_Pervasives_Native.Some
                                ...
                                | (FStar_Pervasives_Native.Some false,
                                   uu___21)::(FStar_Pervasives_Native.Some
                                    ...
                                    ->
                                    ((w FStarC_Syntax_Util.t_false), false)
                                | (uu___21, (arg, uu___22))::(FStar_Pervasives_Native.Some
                                                              true, uu___23)::[]
                                    ->
                                    let uu___24 = maybe_auto_squash arg in
                                    (uu___24, false)
                                | (FStar_Pervasives_Native.Some true,
                                   uu___21)::(uu___22, (arg, uu___23))::[] ->
                                    let uu___24 = maybe_auto_squash arg in
                                    (uu___24, false)
                                | (uu___21, (arg, uu___22))::(FStar_Pervasives_Native.Some
                                                              false, uu___23)::[]
                          ...
                          (let uu___13 =
                           ...
                           then
                             ...
                             | (uu___15, (p, uu___16))::(uu___17,
                                                         (q, uu___18))::[]
                                 ->
                                 let uu___19 = FStarC_Syntax_Util.term_eq p q in
                                 (if uu___19
                                  then ((w FStarC_Syntax_Util.t_true), false)
                                  else squashed_head_un_auto_squash_args tm1)
                             | uu___15 ->
...
and rebuild (cfg : FStarC_TypeChecker_Cfg.cfg) (env1 : env) (stack1 : stack)
  ...
  (let f_opt = is_fext_on_domain t in
   if
     (FStar_Pervasives_Native.uu___is_Some f_opt) &&
...
and do_rebuild (cfg : FStarC_TypeChecker_Cfg.cfg) (env1 : env)
  ...
  | (Arg (Univ uu___, uu___1, uu___2))::uu___3 -> failwith "Impossible"
      ...
      let is_non_tac_layered_effect m =
        let norm_m =
          FStarC_TypeChecker_Env.norm_eff_name
            cfg.FStarC_TypeChecker_Cfg.tcenv m in
        (let uu___ =
           FStarC_Ident.lid_equals norm_m FStarC_Parser_Const.effect_TAC_lid in
         Prims.op_Negation uu___) &&
      ...
      (match uu___ with
       ...
       | FStarC_Syntax_Syntax.Tm_meta
           ...
           let uu___3 =
             ...
             let uu___5 = FStarC_Ident.string_of_lid mtgt in
             FStarC_Format.fmt2
               "Normalizer cannot reify %s ~> %s for extraction" uu___4
               uu___5 in
           FStarC_Errors.raise_error
             (FStarC_Syntax_Syntax.has_range_syntax ()) t
             FStarC_Errors_Codes.Fatal_UnexpectedEffect ()
       ...
       | FStarC_Syntax_Syntax.Tm_meta
           ...
           (FStarC_TypeChecker_Cfg.log cfg
            ...
            (let uu___2 = FStarC_List.tl stack1 in
      ...
      let uu___ =
        let uu___1 =
          let uu___2 =
            let uu___3 =
              let uu___4 =
                let uu___5 = fresh_memo () in (env1, t, uu___5, false) in
              Clos uu___4 in
    ...
    (fun uu___ ->
       ...
       FStarC_Errors.with_ctx
         ...
         (fun uu___1 ->
            let uu___2 =
              let uu___3 =
                let uu___4 = FStarC_TypeChecker_Env.current_module e in
         ...
         (fun uu___2 ->
            ...
            (let uu___8 =
               FStarC_Errors.with_ctx "While normalizing a computation type"
                 (fun uu___9 ->
                    FStarC_Timing.record_ms
                      (fun uu___10 -> norm_comp cfg [] c)) in
             match uu___8 with
             | (c1, ms) ->
                 (FStarC_TypeChecker_Cfg.log_top cfg
                    (fun uu___10 ->
                       ...
                       FStarC_Format.print2
                         ...
                         uu___12);
                  c1))) uu___1 "FStarC.TypeChecker.Normalize.normalize_comp")
...
let ghost_to_pure_aux (env1 : FStarC_TypeChecker_Env.env)
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      ...
      then
        let ct1 =
          let uu___1 =
            downgrade_ghost_effect_name ct.FStarC_Syntax_Syntax.effect_name in
          match uu___1 with
...
let ghost_to_pure_lcomp_aux (env1 : FStarC_TypeChecker_Env.env)
  ...
  let uu___ =
    (FStarC_Syntax_Util.is_ghost_effect lc.FStarC_TypeChecker_Common.eff_name)
      &&
      (maybe_promote_t env1 non_informative_only
...
let ghost_to_pure2 (env1 : FStarC_TypeChecker_Env.env)
  ...
  | (c1, c2) ->
      ...
      (match uu___1 with
       | (c11, c21) ->
           let c1_eff =
             let uu___2 = FStarC_Syntax_Util.comp_effect_name c11 in
             FStarC_TypeChecker_Env.norm_eff_name env1 uu___2 in
           let c2_eff =
             let uu___2 = FStarC_Syntax_Util.comp_effect_name c21 in
             FStarC_TypeChecker_Env.norm_eff_name env1 uu___2 in
                ...
                (let uu___6 =
                   c2_erasable &&
                     (FStarC_Ident.lid_equals c1_eff
                        FStarC_Parser_Const.effect_GHOST_lid) in
                 if uu___6
                 then let uu___7 = ghost_to_pure env1 c11 in (uu___7, c21)
                 else (c11, c21))))
       ...
       | (lc11, lc21) ->
           ...
           then (lc11, lc21)
             ...
             (let lc1_erasable =
              ...
              if uu___4
              then
                let uu___5 = ghost_to_pure_lcomp env1 lc21 in (lc11, uu___5)
              else
                (let uu___6 =
                   lc2_erasable &&
           ...
           (match uu___2 with
            | (actuals, uu___3, uu___4) ->
                ...
                then e
                  ...
                  (let uu___6 = FStarC_Syntax_Util.args_of_binders formals in
                   match uu___6 with
                   | (binders, args) ->
                       let uu___7 =
            ...
            (match uu___3 with
             ...
             | (FStarC_Syntax_Syntax.Tm_arrow
                ...
                FStar_Pervasives.Inl uu___4) ->
                 let uu___5 =
                   let uu___6 = FStarC_Syntax_Util.comp_result c in
...
let elim_uvars_aux_t (env1 : FStarC_TypeChecker_Env.env)
  ...
  | (univ_names1, binders1, tc) ->
      (univ_names1, binders1, (FStar_Pervasives.__proj__Inl__item__v tc))
let elim_uvars_aux_c (env1 : FStarC_TypeChecker_Env.env)
  ...
  match uu___ with
  | (univ_names1, binders1, tc) ->
      (univ_names1, binders1, (FStar_Pervasives.__proj__Inr__item__v tc))
let rec elim_uvars (env1 : FStarC_TypeChecker_Env.env)
       ...
       | (univs, binders, uu___1) ->
           ...
           (match uu___2 with
            | (univs_opening, univs_closing) ->
                ...
                (match uu___3 with
                 | (b_opening, b_closing) ->
                     let n = FStarC_List.length univs in
                     let n_binders = FStarC_List.length binders in
                     let elim_tscheme uu___4 =
                       match uu___4 with
                       | (us, t) ->
                           let n_us = FStarC_List.length us in
                           let uu___5 =
                                     ...
                                     let uu___7 =
                                       let uu___8 =
                                         FStarC_Syntax_Subst.shift_subst
                                           (n_us + n_binders) univs_opening in
                                       let uu___9 =
                                         FStarC_Syntax_Subst.shift_subst
                                           (n_us + n_binders) univs_closing in
                                       (uu___8, uu___9) in
                                     (match uu___7 with
                     ...
                     let ed1 =
                       ...
                       let uu___5 =
                         FStarC_Syntax_Util.apply_eff_combinators
                           elim_tscheme ed.FStarC_Syntax_Syntax.combinators in
                       let uu___6 =
                         FStarC_List.map elim_action
  ...
  | FStarC_Syntax_Syntax.Sig_sub_effect sub_eff ->
      let elim_tscheme_opt uu___ =
        ...
        | FStar_Pervasives_Native.Some (us, t) ->
            ...
            (match uu___1 with
             | (us1, uu___2, t1) -> FStar_Pervasives_Native.Some (us1, t1)) in
      let sub_eff1 =
        ...
        {
          FStarC_Syntax_Syntax.source = (sub_eff.FStarC_Syntax_Syntax.source);
          FStarC_Syntax_Syntax.target = (sub_eff.FStarC_Syntax_Syntax.target);
  ...
  | FStarC_Syntax_Syntax.Sig_polymonadic_bind
      { FStarC_Syntax_Syntax.m_lid = m; FStarC_Syntax_Syntax.n_lid = n;
        FStarC_Syntax_Syntax.p_lid = p; FStarC_Syntax_Syntax.tm3 = (us_t, t);
    ...
    | (bs, c) ->
        ...
        (match (bs, c) with
         ...
         | (bs1, c1) when
             ...
             ->
             let uu___1 =
               let uu___2 = FStarC_Syntax_Util.comp_result c1 in
               aux true (n1 - len) uu___2 in
             (match uu___1 with
