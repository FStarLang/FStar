...
let op_At_At :
  ...
  fun uu___ ->
    fun xs ->
      fun ys ->
        FStarC_List.fold_left (fun xs1 -> fun y -> snoc uu___ xs1 y) xs ys
...
let (names :
  FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.bv FStarC_FlatSet.t) =
  fun t ->
    let uu___ =
...
let (names_comp :
  ...
  fun c ->
    ...
    uu___.FStarC_Syntax_Syntax.free_names
let (uvars :
  FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.ctx_uvar FStarC_FlatSet.t)
  =
  fun t ->
    let uu___ =
      let uu___1 = free_names_and_uvars t Def in
...
let (univnames_comp :
  ...
  fun c ->
    let uu___ =
      ...
      FStar_Pervasives_Native.fst uu___1 in
    uu___.FStarC_Syntax_Syntax.free_univ_names
let (fvars : FStarC_Syntax_Syntax.term -> FStarC_Ident.lident FStarC_RBSet.t)
  =
  fun t ->
