open Prims
let compare_uv (uv1 : FStarC_Syntax_Syntax.ctx_uvar)
  (uv2 : FStarC_Syntax_Syntax.ctx_uvar) : Prims.int=
  let uu___ =
    FStarC_Syntax_Unionfind.uvar_id uv1.FStarC_Syntax_Syntax.ctx_uvar_head in
  let uu___1 =
    FStarC_Syntax_Unionfind.uvar_id uv2.FStarC_Syntax_Syntax.ctx_uvar_head in
  uu___ - uu___1
let compare_universe_uvar (x : FStarC_Syntax_Syntax.universe_uvar)
  (y : FStarC_Syntax_Syntax.universe_uvar) : Prims.int=
...
type use_cache_t =
  ...
  | NoCache 
  | Full 
let uu___is_Def (projectee : use_cache_t) : Prims.bool=
  match projectee with | Def -> true | uu___ -> false
let uu___is_NoCache (projectee : use_cache_t) : Prims.bool=
  match projectee with | NoCache -> true | uu___ -> false
let uu___is_Full (projectee : use_cache_t) : Prims.bool=
...
let singleton_uv (x : FStarC_Syntax_Syntax.ctx_uvar) :
  (FStarC_Syntax_Syntax.free_vars * FStarC_Ident.lident FStarC_RBSet.t)=
  let uu___ =
    let uu___1 = FStar_Pervasives_Native.fst no_free_vars in
    let uu___2 =
  ...
  (uu___, (FStar_Pervasives_Native.snd no_free_vars))
let singleton_univ (x : FStarC_Syntax_Syntax.universe_uvar) :
  (FStarC_Syntax_Syntax.free_vars * FStarC_Ident.lident FStarC_RBSet.t)=
  let uu___ =
    let uu___1 = FStar_Pervasives_Native.fst no_free_vars in
    let uu___2 =
  ...
  (uu___, (FStar_Pervasives_Native.snd no_free_vars))
let singleton_univ_name (x : FStarC_Syntax_Syntax.univ_name) :
  (FStarC_Syntax_Syntax.free_vars * FStarC_Ident.lident FStarC_RBSet.t)=
  let uu___ =
    let uu___1 = FStar_Pervasives_Native.fst no_free_vars in
    let uu___2 =
  ...
  (uu___, (FStar_Pervasives_Native.snd no_free_vars))
let op_Plus_Plus (f1 : free_vars_and_fvars) (f2 : free_vars_and_fvars) :
  (FStarC_Syntax_Syntax.free_vars * FStarC_Ident.lident FStarC_RBSet.t)=
  let uu___ =
    let uu___1 =
...
let rec free_univs (u : FStarC_Syntax_Syntax.universe) : free_vars_and_fvars=
  ...
  | FStarC_Syntax_Syntax.U_max us ->
      FStarC_List.fold_left
        (fun out x -> let uu___1 = free_univs x in op_Plus_Plus out uu___1)
        no_free_vars us
  | FStarC_Syntax_Syntax.U_unif u1 -> singleton_univ u1
let rec free_names_and_uvs' (tm : FStarC_Syntax_Syntax.term)
        ...
        let uu___2 =
          ...
          let uu___4 =
            ...
            | FStar_Pervasives_Native.Some (b, asc) ->
                ...
                let uu___6 = free_names_and_uvars_ascription asc use_cache in
             ...
             | (p, wopt, t2) ->
                 ...
                 let n3 =
                   ...
                   FStarC_List.fold_left
                     (fun n4 x ->
                        let uu___5 =
                          free_names_and_uvars x.FStarC_Syntax_Syntax.sort
                            use_cache in
                        op_Plus_Plus n4 uu___5) n uu___4 in
                 let uu___4 = op_Plus_Plus n3 n1 in op_Plus_Plus uu___4 n2)
          uu___2 pats in
      op_Plus_Plus uu___ uu___1
  ...
  | FStarC_Syntax_Syntax.Tm_let
      { FStarC_Syntax_Syntax.lbs = lbs; FStarC_Syntax_Syntax.body1 = t1;_} ->
      let uu___ = free_names_and_uvars t1 use_cache in
      FStarC_List.fold_left
        (fun n lb ->
           let uu___1 =
             let uu___2 =
               free_names_and_uvars lb.FStarC_Syntax_Syntax.lbtyp use_cache in
             op_Plus_Plus n uu___2 in
           let uu___2 =
             free_names_and_uvars lb.FStarC_Syntax_Syntax.lbdef use_cache in
      ...
      (match m with
       | FStarC_Syntax_Syntax.Meta_pattern (uu___, args) ->
           FStarC_List.fold_right
             (fun a acc -> free_names_and_uvars_args a acc use_cache) args u1
...
and free_names_and_uvars_binders (bs : FStarC_Syntax_Syntax.binders)
  (use_cache : use_cache_t) : free_vars_and_fvars=
  FStarC_List.fold_left
    (fun n b ->
       let uu___ =
...
and free_names_and_uvars
  ...
  (use_cache : use_cache_t) : free_vars_and_fvars=
  let t1 = FStarC_Syntax_Subst.compress t in free_names_and_uvs' t1 use_cache
and free_names_and_uvars_args
  (args :
    (FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax *
      FStarC_Syntax_Syntax.arg_qualifier FStar_Pervasives_Native.option)
      Prims.list)
  (acc : free_vars_and_fvars) (use_cache : use_cache_t) :
  free_vars_and_fvars=
  FStarC_List.fold_left
    (fun n uu___ ->
       match uu___ with
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      let decreases_vars =
        ...
        | FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.DECREASES
            dec_order) -> free_names_and_uvars_dec_order dec_order use_cache in
      let pat_vars =
        let uu___ =
          FStarC_List.tryFind
            (fun uu___1 ->
               match uu___1 with
               | FStarC_Syntax_Syntax.SMTPAT uu___2 -> true
               | uu___2 -> false) ct.FStarC_Syntax_Syntax.flags in
        match uu___ with
...
let names_comp (c : FStarC_Syntax_Syntax.comp) :
  FStarC_Syntax_Syntax.bv FStarC_FlatSet.t=
  let uu___ =
