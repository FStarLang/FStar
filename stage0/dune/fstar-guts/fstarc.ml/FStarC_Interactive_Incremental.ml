...
type 'a qst = (qid * FStarC_Interactive_Ide_Types.repl_state) -> ('a * qid)
let return (x : 'a) : 'a qst=
  fun uu___ -> match uu___ with | (q, uu___1) -> (x, q)
let op_let_Bang (f : 'a qst) (g : 'a -> 'b qst) : 'b qst=
  fun uu___ ->
    match uu___ with
    | (q, s) ->
        let uu___1 = f (q, s) in
        (match uu___1 with | (x, q') -> let uu___2 = g x in uu___2 (q', s))
let run_qst (f : 'a qst) (q : Prims.string)
  (st : FStarC_Interactive_Ide_Types.repl_state) : 'a=
  let uu___ = f ((q, Prims.int_zero), st) in
...
let get_filename : Prims.string qst=
  fun uu___ ->
    match uu___ with
...
let next_qid : qid qst=
  fun uu___ ->
    match uu___ with
    | (q, uu___1) -> let q1 = shift_qid q Prims.int_one in (q1, q1)
let get_qid : qid qst= fun uu___ -> match uu___ with | (q, uu___1) -> (q, q)
...
let repl_task_of_entry (uu___ : ('uuuuu * ('uuuuu1 * 'uuuuu2))) : 'uuuuu1=
...
let inspect_repl_stack (s : FStarC_Interactive_Ide_Types.repl_stack_t)
  ...
  | FStar_Pervasives_Native.Some (_prefix, first_push, rest) ->
        ...
        | (e::entries3, d::ds2) ->
            (match repl_task_of_entry e with
             ...
             | FStarC_Interactive_Ide_Types.PushFragment
                 (FStar_Pervasives.Inl frag, uu___1, uu___2, uu___3) ->
      ...
      let uu___1 = matching_prefix [] [] entries1 ds in
      op_let_Bang uu___1
        (fun uu___2 ->
           match uu___2 with | (queries, json) -> return (queries, json))
let reload_deps
  (repl_stack :
   ...
   then
     ((let uu___3 =
         FStarC_Class_Show.show
           FStarC_Interactive_Ide_Types.repl_state_showable st in
      ...
      (let uu___3 =
         ...
         FStarC_Class_Show.show
           (FStarC_Class_Show.show_list
              FStarC_Interactive_Ide_Types.repl_stack_entry_t_showable)
           uu___4 in
       FStarC_Format.print1 "run_full_buffer: repl_stack=%s\n" uu___3))
   else ());
  FStarC_Parser_ParseIt.add_vfs_entry
    st.FStarC_Interactive_Ide_Types.repl_fname code;
  (let parse_result = parse_code st FStar_Pervasives_Native.None code in
   ...
   let filter_decls decls =
     ...
     | FStarC_Interactive_Ide_Types.VerifyToPosition (uu___2, line, _col) ->
         FStarC_List.filter
           (fun uu___3 ->
              match uu___3 with
              | (d, uu___4) ->
     ...
     | FStarC_Interactive_Ide_Types.LaxToPosition (uu___2, line, _col) ->
         FStarC_List.filter
           (fun uu___3 ->
              match uu___3 with
              | (d, uu___4) ->
   ...
   let qs =
     ...
     | FStarC_Parser_ParseIt.IncrementalFragment (decls, uu___2, err_opt) ->
         ((let uu___4 =
          ...
          (match (request_type, decls) with
           | (FStarC_Interactive_Ide_Types.ReloadDeps, d::uu___4) ->
               let uu___5 =
                 let uu___6 =
                   let uu___7 =
                     FStarC_Effect.op_Bang
                       FStarC_Interactive_PushHelper.repl_stack in
                   reload_deps uu___7 in
                 op_let_Bang uu___6
                   (fun queries ->
                      let uu___7 =
               ...
               run_qst uu___5 qid1 st
           | uu___4 ->
               let decls1 = filter_decls decls in
               let push_kind =
                 match request_type with
                 | FStarC_Interactive_Ide_Types.LaxToPosition uu___5 ->
                     FStarC_Interactive_Ide_Types.LaxCheck
                 | FStarC_Interactive_Ide_Types.Lax ->
                     FStarC_Interactive_Ide_Types.LaxCheck
                 | uu___5 -> FStarC_Interactive_Ide_Types.FullCheck in
               let uu___5 =
                 let uu___6 =
                   let uu___7 =
                     FStarC_Effect.op_Bang
                       FStarC_Interactive_PushHelper.repl_stack in
                   inspect_repl_stack uu___7 decls1 push_kind with_symbols
                     write_full_buffer_fragment_progress in
                 run_qst uu___6 qid1 st in
               (match uu___5 with
                | (queries, issues) ->
