  ...
  FStarC_Ident.lid_of_ids
    (FStarC_List.op_At (FStarC_Ident.ns_of_lid lid)
       [FStarC_Ident.mk_ident
          ((Prims.strcat FStarC_Ident.reserved_prefix
              (Prims.strcat "is_"
    ...
    (fun i b ->
       ...
       then
         let bname =
           ...
           FStarC_Ident.id_of_text uu___ in
         let bv =
           {
...
let rec name_function_binders_from (i : Prims.int)
  ...
  | FStarC_Syntax_Syntax.Tm_arrow
        ...
        then
          let uu___ =
            let uu___1 =
              ...
              FStarC_Ident.id_of_text uu___2 in
            {
      ...
      let comp1 =
        ...
        | FStarC_Syntax_Syntax.Comp ct when
            ...
            let uu___ =
              let uu___1 =
                let uu___2 =
                  name_function_binders_from (i + Prims.int_one)
      ...
      FStarC_Syntax_Syntax.mk
        (FStarC_Syntax_Syntax.Tm_arrow
           { FStarC_Syntax_Syntax.b1 = b1; FStarC_Syntax_Syntax.comp = comp1
           }) t.FStarC_Syntax_Syntax.pos
  | uu___ -> t
let name_function_binders (t : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term= name_function_binders_from Prims.int_zero t
  ...
  then
    FStarC_List.fold_right2
      (fun f a out ->
         ...
         :: out) formals actuals []
  else FStarC_Effect.failwith "Ill-formed substitution"
...
let rec compare_univs (u1 : FStarC_Syntax_Syntax.universe)
  ...
  let rec compare_kernel uk1 uk2 =
    ...
    | (FStarC_Syntax_Syntax.U_bvar uu___1, uu___2) ->
        FStarC_Effect.failwith "Impossible: compare_kernel bvar"
...
let eq_univs_list (us : FStarC_Syntax_Syntax.universes)
  ...
  if (FStarC_List.length us) = (FStarC_List.length vs)
  then FStarC_List.forall2 eq_univs us vs
  else false
let ml_comp (t : FStarC_Syntax_Syntax.term) (r : FStarC_Range_Type.range) :
  FStarC_Syntax_Syntax.comp=
  let uu___ =
    let uu___1 =
      let uu___2 = FStarC_Parser_Const.effect_ALL_lid () in
      FStarC_Ident.set_lid_range uu___2 r in
    let uu___2 =
      let uu___3 = FStarC_Parser_Const.effect_ML_lid () in
      FStarC_Ident.set_lid_range uu___3 r in
    {
      FStarC_Syntax_Syntax.effect_name = uu___1;
      FStarC_Syntax_Syntax.result_typ = t;
      FStarC_Syntax_Syntax.flags = [];
      FStarC_Syntax_Syntax.source_effect_name = uu___2
    } in
...
let comp_source_effect_name (c : FStarC_Syntax_Syntax.comp) :
  FStarC_Ident.lident=
  match c.FStarC_Syntax_Syntax.n with
  | FStarC_Syntax_Syntax.Comp c1 ->
      c1.FStarC_Syntax_Syntax.source_effect_name
let combine_source_effect_name (eff : FStarC_Ident.lident)
  (s1 : FStarC_Ident.lident) (s2 : FStarC_Ident.lident) :
  FStarC_Ident.lident= if FStarC_Ident.lid_equals s1 s2 then s1 else eff
...
let comp_eff_name_and_res (c : FStarC_Syntax_Syntax.comp) :
  (FStarC_Ident.lident * FStarC_Syntax_Syntax.typ)=
  match c.FStarC_Syntax_Syntax.n with
  | FStarC_Syntax_Syntax.Comp c1 ->
      ((c1.FStarC_Syntax_Syntax.effect_name),
        (c1.FStarC_Syntax_Syntax.result_typ))
let comp_to_comp_typ (c : FStarC_Syntax_Syntax.comp) :
  FStarC_Syntax_Syntax.comp_typ=
  match c.FStarC_Syntax_Syntax.n with | FStarC_Syntax_Syntax.Comp ct -> ct
let un_uinst (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  ...
  | uu___ -> t1
let rec is_t_true (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  ...
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = hd;
        FStarC_Syntax_Syntax.arg = (a, uu___1);_}
      when
      let uu___2 = let uu___3 = un_uinst hd in uu___3.FStarC_Syntax_Syntax.n in
      match uu___2 with
      | FStarC_Syntax_Syntax.Tm_fvar fv ->
          FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.squash_lid
      | uu___3 -> false -> is_t_true a
  | uu___1 -> false
let rec is_t_false (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  let uu___ = let uu___1 = unmeta t in uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_fvar fv ->
  ...
  | FStarC_Syntax_Syntax.Tm_app
      ...
      | FStarC_Syntax_Syntax.Tm_fvar fv ->
          FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.squash_lid
      | uu___3 -> false -> is_t_false a
  | uu___1 -> false
let is_trivial_post (p : FStarC_Syntax_Syntax.term) : Prims.bool=
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress p in
    uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = uu___1; FStarC_Syntax_Syntax.body = body;
        FStarC_Syntax_Syntax.rc_opt = uu___2;_}
      -> let uu___3 = FStarC_Syntax_Subst.compress body in is_t_true uu___3
  | uu___1 -> false
let is_named_tot (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  FStarC_Parser_Const.is_tot_lid (comp_effect_name c)
let is_named_gtot (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  FStarC_Parser_Const.is_gtot_lid (comp_effect_name c)
let is_named_tot_or_gtot (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  FStarC_Parser_Const.is_tot_or_gtot_lid (comp_effect_name c)
...
let is_tot_or_gtot_comp (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  ...
  if uu___
  then true
  else FStarC_Parser_Const.is_ghost_effect_lid (comp_effect_name c)
let is_bare_tot_or_gtot_comp (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  match c.FStarC_Syntax_Syntax.n with
  | FStarC_Syntax_Syntax.Comp ct ->
      (FStarC_Parser_Const.is_tot_or_gtot_lid
         ct.FStarC_Syntax_Syntax.effect_name)
        &&
        ((match ct.FStarC_Syntax_Syntax.flags with
          | [] -> true
          | uu___ -> false))
let is_bare_total_comp (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  let uu___ = is_bare_tot_or_gtot_comp c in
  if uu___ then is_named_tot c else false
...
let is_pure_comp (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      ...
      if uu___
      then true
      else is_pure_effect ct.FStarC_Syntax_Syntax.effect_name
...
let is_ndet_effect (l : FStarC_Ident.lident) : Prims.bool=
...
let rec is_pure_or_ghost_function (t : FStarC_Syntax_Syntax.typ) :
  Prims.bool=
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress t in
    uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_arrow
      { FStarC_Syntax_Syntax.b1 = uu___1; FStarC_Syntax_Syntax.comp = c;_} ->
      (match c.FStarC_Syntax_Syntax.n with
       | FStarC_Syntax_Syntax.Comp ct when
           if
             FStarC_Parser_Const.is_tot_lid
           ...
           is_pure_or_ghost_function ct.FStarC_Syntax_Syntax.result_typ
...
let rec head_of (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  ...
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = t1; FStarC_Syntax_Syntax.arg = uu___1;_} ->
      head_of t1
  | FStarC_Syntax_Syntax.Tm_match
      ...
      -> head_of t1
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = uu___1; FStarC_Syntax_Syntax.body = t1;
        FStarC_Syntax_Syntax.rc_opt = uu___2;_}
      -> head_of t1
  | FStarC_Syntax_Syntax.Tm_ascribed
...
let rec __head_and_args_full
  ...
  match t1.FStarC_Syntax_Syntax.n with
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = head; FStarC_Syntax_Syntax.arg = arg;_} ->
      __head_and_args_full (arg :: acc) unmeta1 head
  | FStarC_Syntax_Syntax.Tm_meta
...
let rec leftmost_head (t : FStarC_Syntax_Syntax.term) :
  ...
  match t1.FStarC_Syntax_Syntax.n with
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = t0; FStarC_Syntax_Syntax.arg = uu___;_} ->
      leftmost_head t0
  | FStarC_Syntax_Syntax.Tm_meta
...
let comp_smt_pats (c : FStarC_Syntax_Syntax.comp) :
...
let rec is_uvar (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  ...
  | FStarC_Syntax_Syntax.Tm_app uu___1 ->
      let uu___2 =
        let uu___3 = head_and_args_full t in
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      ...
      -> is_uvar t1
  | uu___1 -> false
let is_exactly_unit (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress t in
    uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
...
let rec is_unit (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  ...
  | FStarC_Syntax_Syntax.Tm_fvar fv ->
      (FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.unit_lid) ||
        (FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.squash_lid)
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = head; FStarC_Syntax_Syntax.arg = uu___1;_}
      -> is_unit head
...
let destruct (typ : FStarC_Syntax_Syntax.typ) (lid : FStarC_Ident.lident) :
  ...
  let uu___ = let uu___1 = un_uinst typ1 in head_and_args_full uu___1 in
  match uu___ with
  | (head, args) ->
      let uu___1 =
        let uu___2 = un_uinst head in uu___2.FStarC_Syntax_Syntax.n in
      (match uu___1 with
...
let lids_of_sigelt (se : FStarC_Syntax_Syntax.sigelt) :
  ...
  | FStarC_Syntax_Syntax.Sig_effect_abbrev
      { FStarC_Syntax_Syntax.lid4 = lid; FStarC_Syntax_Syntax.root = uu___;_}
      -> [lid]
...
let range_of_args (args : FStarC_Syntax_Syntax.args)
  ...
  FStarC_List.fold_left
    (fun r1 a -> FStarC_Range_Ops.union_ranges r1 (range_of_arg a)) r args
  ...
  let uu___ =
    FStarC_List.map
      (fun b ->
         let uu___1 =
           FStarC_Syntax_Syntax.bv_to_name b.FStarC_Syntax_Syntax.binder_bv in
         (uu___1, (aqual_of_binder b))) bs in
  mk_app f uu___
    ...
    then i
      ...
      FStarC_Ident.mk_ident
        ((mk_field_projector_name_from_string
            (FStarC_Ident.string_of_id (FStarC_Ident.ident_of_lid lid)) itext),
          (FStarC_Ident.range_of_id i)) in
  FStarC_Ident.lid_of_ids
    (FStarC_List.op_At (FStarC_Ident.ns_of_lid lid) [newi])
  ...
  let nm =
    ...
    then
      let uu___ =
        ...
        (uu___1, (FStarC_Syntax_Syntax.range_of_bv x)) in
...
let ses_of_sigbundle (se : FStarC_Syntax_Syntax.sigelt) :
  ...
  | FStarC_Syntax_Syntax.Sig_bundle
      ...
      -> ses
  | uu___ -> FStarC_Effect.failwith "ses_of_sigbundle: not a Sig_bundle"
...
let qualifier_equal (q1 : FStarC_Syntax_Syntax.qualifier)
  ...
  | uu___ -> q1 = q2
let abs_ln (bs : FStarC_Syntax_Syntax.binders)
...
let abs (bs : FStarC_Syntax_Syntax.binders) (t : FStarC_Syntax_Syntax.term)
  ...
  | uu___ ->
      ...
      FStarC_Syntax_Syntax.mk_Tm_abs uu___1 body lopt1
        t.FStarC_Syntax_Syntax.pos
let arrow_ln (bs : FStarC_Syntax_Syntax.binders)
  ...
  | uu___ ->
      ...
      FStarC_Syntax_Syntax.mk_Tm_arrow bs c uu___1
...
let refine (b : FStarC_Syntax_Syntax.bv) (t : FStarC_Syntax_Syntax.term) :
  ...
  let uu___ =
    let uu___1 =
      let uu___2 =
        FStarC_Syntax_Subst.close [FStarC_Syntax_Syntax.mk_binder b] t in
      { FStarC_Syntax_Syntax.b2 = b; FStarC_Syntax_Syntax.phi = uu___2 } in
    FStarC_Syntax_Syntax.Tm_refine uu___1 in
  let uu___1 =
    FStarC_Range_Ops.union_ranges (FStarC_Syntax_Syntax.range_of_bv b)
...
let rec arrow_formals_comp_ln (k : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_arrow
      ...
      then
        ...
        (match uu___1 with
         | (bs', k') ->
             (match bs' with | [] -> ([b], c) | uu___2 -> ((b :: bs'), k')))
      else ([b], c)
  | FStarC_Syntax_Syntax.Tm_refine
      {
...
let rec arrow_node_formals_comp_ln (k : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.comp)=
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress k in
    uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_arrow
      { FStarC_Syntax_Syntax.b1 = b; FStarC_Syntax_Syntax.comp = comp;_} ->
      ...
      if uu___1
      then
        let uu___2 = arrow_node_formals_comp_ln (comp_result comp) in
        (match uu___2 with | (bs, c) -> ((b :: bs), c))
      else ([b], comp)
  | uu___1 -> let uu___2 = FStarC_Syntax_Syntax.mk_Total k in ([], uu___2)
let arrow_node_formals_comp (k : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.comp)=
  let uu___ = arrow_node_formals_comp_ln k in
  match uu___ with | (bs, c) -> FStarC_Syntax_Subst.open_comp bs c
let arrow_formals_comp_ln_strict (k : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.comp)=
  arrow_node_formals_comp_ln k
let arrow_formals_comp_strict (k : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.comp)=
         ...
         | (bs, c1) ->
             ...
             (match uu___1 with
              | FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.DECREASES
                  d) -> (bs, (FStar_Pervasives_Native.Some d))
              | uu___2 ->
                  ...
                  else (bs, FStar_Pervasives_Native.None)))
    | FStarC_Syntax_Syntax.Tm_refine
        {
          FStarC_Syntax_Syntax.b2 =
            { FStarC_Syntax_Syntax.ppname = uu___;
          ...
          (fun d ->
             let d_bvs =
                   ...
                   FStarC_List.fold_left
                     (fun s t ->
                        ...
                        FStarC_Class_Setlike.union
                          (FStarC_FlatSet.setlike_flat_set
                             FStarC_Syntax_Syntax.ord_bv) s uu___3) uu___2 l
               | FStarC_Syntax_Syntax.Decreases_wf (rel, e) ->
                   ...
                   FStarC_Class_Setlike.union
                     (FStarC_FlatSet.setlike_flat_set
                        FStarC_Syntax_Syntax.ord_bv) uu___2 uu___3 in
             let uu___2 =
               ...
               FStarC_List.map
                 (fun b ->
                    FStarC_Class_Setlike.mem
                      ...
                      b.FStarC_Syntax_Syntax.binder_bv d_bvs) bs in
             FStarC_List.op_At uu___2 uu___3) dopt in
      ((n_univs + (FStarC_List.length bs)), uu___1)
let rec __abs_formals_ln (t : FStarC_Syntax_Syntax.term)
  (abs_body_rc :
  ...
  (FStarC_Syntax_Syntax.binder Prims.list * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.residual_comp FStar_Pervasives_Native.option)=
  let uu___ = let uu___1 = unmeta_safe t in uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = t1;
        FStarC_Syntax_Syntax.rc_opt = what;_}
      ->
      let uu___1 = __abs_formals_ln t1 what in
      (match uu___1 with | (bs', t2, what1) -> ((b :: bs'), t2, what1))
  | uu___1 -> ([], t, abs_body_rc)
let abs_formals_ln (t : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.residual_comp FStar_Pervasives_Native.option)=
  __abs_formals_ln t FStar_Pervasives_Native.None
let rec abs_one_group_ln (t : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.residual_comp FStar_Pervasives_Native.option)=
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress t in
  ...
  | FStarC_Syntax_Syntax.Tm_abs
      ...
      -> ([b], body, (FStar_Pervasives_Native.Some rc))
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = body;
        FStarC_Syntax_Syntax.rc_opt = uu___1;_}
      ->
      let uu___2 = abs_one_group_ln body in
      (match uu___2 with | (bs, body1, rc) -> ((b :: bs), body1, rc))
  | uu___1 -> ([], t, FStar_Pervasives_Native.None)
let abs_formals_maybe_unascribe_body (maybe_unascribe : Prims.bool)
  (t : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.binders * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.residual_comp FStar_Pervasives_Native.option)=
  let subst_rc_opt s l =
    match l with
    | FStar_Pervasives_Native.Some rc ->
        let uu___ =
          let uu___1 =
  ...
  let rec spine t1 abs_body_rc =
    ...
    | FStarC_Syntax_Syntax.Tm_abs
        { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = t2;
          FStarC_Syntax_Syntax.rc_opt = what;_}
        ->
        let uu___1 = spine t2 what in
        (match uu___1 with | (bs', t3, what1) -> ((b :: bs'), t3, what1))
    | uu___1 -> ([], t1, abs_body_rc) in
  let rec aux t1 abs_body_rc =
  ...
  | (bs, t1, abs_body_rc) ->
      ...
      (match uu___1 with
       | (bs1, t2, opening) ->
           let abs_body_rc1 = subst_rc_opt opening abs_body_rc in
           (bs1, t2, abs_body_rc1))
...
let remove_inacc (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  ...
  | (bs, c) ->
      (match bs with
       | [] -> t
       | uu___1 ->
           let uu___2 = FStarC_List.map no_acc bs in
           FStarC_Syntax_Syntax.mk_Tm_arrow uu___2 c
             t.FStarC_Syntax_Syntax.pos)
let mk_letbinding
  (lbname :
...
let rec get_tycon (t : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = t2; FStarC_Syntax_Syntax.arg = uu___;_} ->
      get_tycon t2
  | uu___ -> FStar_Pervasives_Native.None
  ...
  FStarC_Syntax_Syntax.mk
    (FStarC_Syntax_Syntax.Tm_fvar
       (FStarC_Syntax_Syntax.lid_as_fv
          FStarC_Parser_Const.attr_substitute_lid
...
let mk_conj_opt
  ...
  | FStar_Pervasives_Native.Some phi11 ->
      let uu___ =
        ...
        FStarC_Syntax_Syntax.mk_Tm_app tand
          [FStarC_Syntax_Syntax.as_arg phi11;
          FStarC_Syntax_Syntax.as_arg phi2] uu___1 in
      FStar_Pervasives_Native.Some uu___
let mk_binop (op_t : FStarC_Syntax_Syntax.term)
  ...
  let uu___ =
    FStarC_Range_Ops.union_ranges phi1.FStarC_Syntax_Syntax.pos
      phi2.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app op_t
    [FStarC_Syntax_Syntax.as_arg phi1; FStarC_Syntax_Syntax.as_arg phi2]
    uu___
let mk_neg (phi : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Syntax.mk_Tm_app t_not [FStarC_Syntax_Syntax.as_arg phi]
...
let b2t (e : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Syntax.mk_Tm_app b2t_v [FStarC_Syntax_Syntax.as_arg e]
    e.FStarC_Syntax_Syntax.pos
let unb2t (e : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term FStar_Pervasives_Native.option=
  let uu___ = head_and_args_full e in
  match uu___ with
  | (hd, args) ->
      let uu___1 =
        let uu___2 =
...
let mk_imp_simp (t1 : FStarC_Syntax_Syntax.term)
  (t2 : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let uu___ = is_t_true t1 in
  if uu___
  then t2
  else (let uu___1 = is_t_true t2 in if uu___1 then t_true else mk_imp t1 t2)
...
let mk_has_type (us : FStarC_Syntax_Syntax.universes)
  (t : FStarC_Syntax_Syntax.term) (x : FStarC_Syntax_Syntax.term)
  (t' : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let t_has_type = fvar_const FStarC_Parser_Const.has_type_lid in
  let t_has_type1 =
    FStarC_Syntax_Syntax.mk (FStarC_Syntax_Syntax.Tm_uinst (t_has_type, us))
      FStarC_Range_Type.dummyRange in
  FStarC_Syntax_Syntax.mk_Tm_app t_has_type1
    [FStarC_Syntax_Syntax.iarg t;
    FStarC_Syntax_Syntax.as_arg x;
    FStarC_Syntax_Syntax.as_arg t'] FStarC_Range_Type.dummyRange
let refinement_hypothesis (t : FStarC_Syntax_Syntax.typ)
  ...
  | FStarC_Syntax_Syntax.Tm_refine
      ...
      let uu___1 =
        FStarC_Syntax_Subst.open_term [FStarC_Syntax_Syntax.mk_binder b] phi in
      (match uu___1 with
       | (bs, phi1) ->
           FStarC_Syntax_Subst.subst
             [FStarC_Syntax_Syntax.NT
                (((FStarC_List.hd bs).FStarC_Syntax_Syntax.binder_bv), v)]
             phi1)
  | uu___1 ->
      let uu___2 = head_and_args_full t in
      (match uu___2 with
       | (hd, args) ->
           let uu___3 =
             let uu___4 =
               let uu___5 = un_uinst hd in uu___5.FStarC_Syntax_Syntax.n in
             (uu___4, args) in
           (match uu___3 with
            | (FStarC_Syntax_Syntax.Tm_fvar fv, (p, uu___4)::[]) when
                FStarC_Syntax_Syntax.fv_eq_lid fv
                  FStarC_Parser_Const.squash_lid
                -> p
            | uu___4 -> t_true))
let apply_post (p : FStarC_Syntax_Syntax.term)
  (e : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let uu___ = is_trivial_post p in
  if uu___
  then t_true
  else
    (let uu___1 =
       let uu___2 = FStarC_Syntax_Subst.compress p in
     ...
     | FStarC_Syntax_Syntax.Tm_abs
         ...
         ->
         let uu___3 =
           FStarC_Syntax_Subst.open_term_bv b.FStarC_Syntax_Syntax.binder_bv
             body in
         (match uu___3 with
          | (bv, body1) ->
              FStarC_Syntax_Subst.subst [FStarC_Syntax_Syntax.NT (bv, e)]
                body1)
     | uu___2 ->
         FStarC_Syntax_Syntax.mk_Tm_app p [FStarC_Syntax_Syntax.as_arg e]
           p.FStarC_Syntax_Syntax.pos)
...
let mk_untyped_eq2 (e1 : FStarC_Syntax_Syntax.term)
  ...
  FStarC_Syntax_Syntax.mk_Tm_app teq
    [FStarC_Syntax_Syntax.as_arg e1; FStarC_Syntax_Syntax.as_arg e2] uu___
let mk_eq2 (u : FStarC_Syntax_Syntax.universe) (t : FStarC_Syntax_Syntax.typ)
  ...
  let uu___ =
    FStarC_Range_Ops.union_ranges e1.FStarC_Syntax_Syntax.pos
      e2.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app eq_inst
    [FStarC_Syntax_Syntax.iarg t;
    FStarC_Syntax_Syntax.as_arg e1;
    FStarC_Syntax_Syntax.as_arg e2] uu___
let mk_eq3_no_univ :
  FStarC_Syntax_Syntax.term ->
    FStarC_Syntax_Syntax.term ->
      FStarC_Syntax_Syntax.term ->
      ...
      fun e1 ->
        fun e2 ->
          let uu___ =
            FStarC_Range_Ops.union_ranges e1.FStarC_Syntax_Syntax.pos
              e2.FStarC_Syntax_Syntax.pos in
          FStarC_Syntax_Syntax.mk_Tm_app teq3
            [FStarC_Syntax_Syntax.iarg t1;
            FStarC_Syntax_Syntax.iarg t2;
            FStarC_Syntax_Syntax.as_arg e1;
            FStarC_Syntax_Syntax.as_arg e2] uu___
...
let mk_decidable_eq (t : FStarC_Syntax_Syntax.term)
  ...
  let uu___ =
    FStarC_Range_Ops.union_ranges e1.FStarC_Syntax_Syntax.pos
      e2.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app decidable_eq
    [FStarC_Syntax_Syntax.iarg t;
    FStarC_Syntax_Syntax.as_arg e1;
    FStarC_Syntax_Syntax.as_arg e2] uu___
...
let mk_or (e1 : FStarC_Syntax_Syntax.term) (e2 : FStarC_Syntax_Syntax.term) :
  ...
  let uu___ =
    FStarC_Range_Ops.union_ranges e1.FStarC_Syntax_Syntax.pos
      e2.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app b_or
    [FStarC_Syntax_Syntax.as_arg e1; FStarC_Syntax_Syntax.as_arg e2] uu___
...
let mk_and (e1 : FStarC_Syntax_Syntax.term) (e2 : FStarC_Syntax_Syntax.term)
  ...
  let uu___ =
    FStarC_Range_Ops.union_ranges e1.FStarC_Syntax_Syntax.pos
      e2.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app b_and
    [FStarC_Syntax_Syntax.as_arg e1; FStarC_Syntax_Syntax.as_arg e2] uu___
...
let mk_forall_aux (fa : FStarC_Syntax_Syntax.term)
  ...
  let uu___ =
    let uu___1 =
      let uu___2 =
        let uu___3 =
          abs [FStarC_Syntax_Syntax.mk_binder x] body
            (FStar_Pervasives_Native.Some (residual_tot ktype0)) in
        FStarC_Syntax_Syntax.as_arg uu___3 in
      [uu___2] in
    (FStarC_Syntax_Syntax.iarg x.FStarC_Syntax_Syntax.sort) :: uu___1 in
  FStarC_Syntax_Syntax.mk_Tm_app fa uu___ FStarC_Range_Type.dummyRange
...
let mk_exists_aux (fa : FStarC_Syntax_Syntax.term)
  (x : FStarC_Syntax_Syntax.bv) (body : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term=
  let uu___ =
    let uu___1 =
...
let if_then_else (b : FStarC_Syntax_Syntax.term)
  ...
  let then_branch =
    ((FStarC_Syntax_Syntax.withinfo
        ...
        t1.FStarC_Syntax_Syntax.pos), FStar_Pervasives_Native.None, t1) in
  let else_branch =
    ((FStarC_Syntax_Syntax.withinfo
        (FStarC_Syntax_Syntax.Pat_constant (FStarC_Const.Const_bool false))
        t2.FStarC_Syntax_Syntax.pos), FStar_Pervasives_Native.None, t2) in
  let uu___ =
    let uu___1 =
  ...
  | (head, args) ->
      ...
      (match uu___1 with
       | (FStarC_Syntax_Syntax.Tm_fvar fv, (p, uu___2)::[]) when
           ...
           -> FStar_Pervasives_Native.Some p
       | (FStarC_Syntax_Syntax.Tm_refine
          { FStarC_Syntax_Syntax.b2 = b; FStarC_Syntax_Syntax.phi = p;_}, [])
           ->
            ...
            | FStarC_Syntax_Syntax.Tm_fvar fv when
                ...
                let uu___2 =
                  FStarC_Syntax_Subst.open_term
                    [FStarC_Syntax_Syntax.mk_binder b] p in
                (match uu___2 with
                 | (bs, p1) ->
                     let b1 =
                       match bs with
                     ...
                     let uu___3 =
                       ...
                       FStarC_Class_Setlike.mem
                         (FStarC_FlatSet.setlike_flat_set
                            FStarC_Syntax_Syntax.ord_bv)
                         b1.FStarC_Syntax_Syntax.binder_bv uu___4 in
                     if uu___3
                     then FStar_Pervasives_Native.None
...
let mk_reify (t : FStarC_Syntax_Syntax.term)
  ...
  let reify_ =
    FStarC_Syntax_Syntax.mk
      ...
      t.FStarC_Syntax_Syntax.pos in
  FStarC_Syntax_Syntax.mk_Tm_app reify_ [FStarC_Syntax_Syntax.as_arg t]
    t.FStarC_Syntax_Syntax.pos
let mk_reflect (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  ...
  FStarC_Syntax_Syntax.mk_Tm_app reflect_ [FStarC_Syntax_Syntax.as_arg t]
    t.FStarC_Syntax_Syntax.pos
...
let mk_list (typ : FStarC_Syntax_Syntax.term) (rng : FStarC_Range_Type.range)
  ...
  let nil args pos =
    let uu___ =
      ...
      FStarC_Syntax_Syntax.mk_Tm_uinst uu___1 [FStarC_Syntax_Syntax.U_zero] in
    FStarC_Syntax_Syntax.mk_Tm_app uu___ args pos in
  let uu___ = nil [FStarC_Syntax_Syntax.iarg typ] rng in
  FStarC_List.fold_right
    (fun t a ->
       cons
         [FStarC_Syntax_Syntax.iarg typ;
         FStarC_Syntax_Syntax.as_arg t;
         FStarC_Syntax_Syntax.as_arg a] t.FStarC_Syntax_Syntax.pos) l uu___
let rec eqlist :
  'a .
  ...
  fun eq xs ys ->
    ...
    | ([], []) -> true
    | (x::xs1, y::ys1) ->
        let uu___ = eq x y in if uu___ then eqlist eq xs1 ys1 else false
    | uu___ -> false
...
let eqprod (e1 : 'a -> 'a -> Prims.bool) (e2 : 'b -> 'b -> Prims.bool)
  ...
  match (x, y) with
  | ((x1, x2), (y1, y2)) ->
      let uu___ = e1 x1 y1 in if uu___ then e2 x2 y2 else false
let eqopt (e : 'a -> 'a -> Prims.bool)
  ...
  | uu___ -> false
let check_term_eq (dbg : Prims.bool) (msg : Prims.string) (cond : Prims.bool)
  : Prims.bool=
  if cond
  then true
  else
    (if dbg then FStarC_Format.print1 ">>> term_eq failing: %s\n" msg else ();
     false)
let fail_term_eq (dbg : Prims.bool) (msg : Prims.string) : Prims.bool=
...
let rec term_eq_dbg (dbg : Prims.bool) (t1 : FStarC_Syntax_Syntax.term)
  ...
  | (FStarC_Syntax_Syntax.Tm_delayed uu___, uu___1) ->
      FStarC_Effect.failwith "term_eq: impossible, should have been removed"
  ...
  | (FStarC_Syntax_Syntax.Tm_uinst (t13, us1), FStarC_Syntax_Syntax.Tm_uinst
     (t23, us2)) ->
      let uu___ = eqlist eq_univs us1 us2 in
      if uu___ then term_eq_dbg dbg t13 t23 else false
  ...
  | (FStarC_Syntax_Syntax.Tm_fvar x, FStarC_Syntax_Syntax.Tm_fvar y) ->
      check "fvar" (FStarC_Syntax_Syntax.fv_eq x y)
  | (FStarC_Syntax_Syntax.Tm_constant c1, FStarC_Syntax_Syntax.Tm_constant
     c2) -> check "const" (FStarC_Const.eq_const c1 c2)
  | (FStarC_Syntax_Syntax.Tm_type uu___, FStarC_Syntax_Syntax.Tm_type uu___1)
      -> true
  | (FStarC_Syntax_Syntax.Tm_abs
     { FStarC_Syntax_Syntax.b = b1; FStarC_Syntax_Syntax.body = t13;
       FStarC_Syntax_Syntax.rc_opt = k1;_},
     FStarC_Syntax_Syntax.Tm_abs
     { FStarC_Syntax_Syntax.b = b2; FStarC_Syntax_Syntax.body = t23;
       FStarC_Syntax_Syntax.rc_opt = k2;_})
      ->
      let uu___ =
        let uu___1 = binder_eq_dbg dbg b1 b2 in check "abs binders" uu___1 in
      if uu___
      then let uu___1 = term_eq_dbg dbg t13 t23 in check "abs bodies" uu___1
      else false
  | (FStarC_Syntax_Syntax.Tm_arrow
     ...
     { FStarC_Syntax_Syntax.b1 = b2; FStarC_Syntax_Syntax.comp = c2;_}) ->
      let uu___ =
        let uu___1 = binder_eq_dbg dbg b1 b2 in check "arrow binders" uu___1 in
      if uu___
      then let uu___1 = comp_eq_dbg dbg c1 c2 in check "arrow comp" uu___1
      else false
  | (FStarC_Syntax_Syntax.Tm_refine
     { FStarC_Syntax_Syntax.b2 = b1; FStarC_Syntax_Syntax.phi = t13;_},
     FStarC_Syntax_Syntax.Tm_refine
     { FStarC_Syntax_Syntax.b2 = b2; FStarC_Syntax_Syntax.phi = t23;_}) ->
      let uu___ =
        let uu___1 =
          term_eq_dbg dbg b1.FStarC_Syntax_Syntax.sort
            b2.FStarC_Syntax_Syntax.sort in
        check "refine bv sort" uu___1 in
      if uu___
      then
        let uu___1 = term_eq_dbg dbg t13 t23 in check "refine formula" uu___1
      else false
  | (FStarC_Syntax_Syntax.Tm_app
     { FStarC_Syntax_Syntax.hd = f1; FStarC_Syntax_Syntax.arg = a1;_},
     FStarC_Syntax_Syntax.Tm_app
     { FStarC_Syntax_Syntax.hd = f2; FStarC_Syntax_Syntax.arg = a2;_}) ->
      let uu___ =
        let uu___1 = term_eq_dbg dbg f1 f2 in check "app head" uu___1 in
      if uu___
      then let uu___1 = arg_eq_dbg dbg a1 a2 in check "app args" uu___1
      else false
  | (FStarC_Syntax_Syntax.Tm_match
     ...
     { FStarC_Syntax_Syntax.scrutinee = t23;
      ...
      ->
      let uu___4 =
        let uu___5 = term_eq_dbg dbg t13 t23 in check "match head" uu___5 in
      if uu___4
      then
        let uu___5 = eqlist (branch_eq_dbg dbg) bs1 bs2 in
        check "match branches" uu___5
      else false
      ...
      (match (m1, m2) with
       | (FStarC_Syntax_Syntax.Meta_monadic (n1, ty1),
          FStarC_Syntax_Syntax.Meta_monadic (n2, ty2)) ->
           let uu___ =
             check "meta_monadic lid" (FStarC_Ident.lid_equals n1 n2) in
           if uu___
           then
  ...
  | (uu___, FStarC_Syntax_Syntax.Tm_unknown) -> fail "unk"
  ...
  | (uu___, FStarC_Syntax_Syntax.Tm_meta uu___1) ->
      let uu___2 =
        ...
        Prims.strcat "bottom: " uu___3 in
      fail uu___2
and arg_eq_dbg (dbg : Prims.bool) (a1 : FStarC_Syntax_Syntax.arg)
  (a2 : FStarC_Syntax_Syntax.arg) : Prims.bool=
  eqprod
    (fun t1 t2 ->
       let uu___ = term_eq_dbg dbg t1 t2 in check_term_eq dbg "arg tm" uu___)
    (fun q1 q2 ->
       let uu___ = aqual_eq_dbg dbg q1 q2 in
       check_term_eq dbg "arg qual" uu___) a1 a2
...
and comp_eq_dbg (dbg : Prims.bool) (c1 : FStarC_Syntax_Syntax.comp)
  ...
  let uu___ = comp_eff_name_and_res c1 in
  match uu___ with
  | (eff1, res1) ->
      let uu___1 = comp_eff_name_and_res c2 in
      (match uu___1 with
       | (eff2, res2) ->
           let uu___2 =
             let uu___3 =
...
and antiquotations_eq_dbg (dbg : Prims.bool)
  ...
  (a2 : FStarC_Syntax_Syntax.term Prims.list) : Prims.bool=
  match (a1, a2) with
  | ([], []) -> true
  | ([], uu___) -> false
  | (uu___, []) -> false
...
and bqual_eq_dbg (dbg : Prims.bool) (a1 : FStarC_Syntax_Syntax.bqual)
  ...
  | uu___ -> false
and aqual_eq_dbg (dbg : Prims.bool) (a1 : FStarC_Syntax_Syntax.aqual)
  (a2 : FStarC_Syntax_Syntax.aqual) : Prims.bool=
  match (a1, a2) with
...
let post_domain_needs_check (t : FStarC_Syntax_Syntax.typ)
  (p : FStarC_Syntax_Syntax.term) : Prims.bool=
  let comparable t1 = term_eq t1 t1 in
  let uu___ =
    let uu___1 = FStarC_Syntax_Subst.compress p in
    uu___1.FStarC_Syntax_Syntax.n in
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = uu___1;
        FStarC_Syntax_Syntax.rc_opt = uu___2;_}
      ->
      let uu___3 =
        let uu___4 =
      ...
      (match uu___3 with
       ...
       | uu___4 ->
           ...
           if uu___5 then comparable t else false)
  | uu___1 -> false
let refine_with_post (t : FStarC_Syntax_Syntax.typ)
  (p : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.typ=
  let keep_app = post_domain_needs_check t p in
  let uu___ =
    let uu___1 = is_trivial_post p in
    if uu___1 then Prims.not keep_app else false in
  if uu___
  then t
  else
    (let x =
       ...
       then
         let uu___1 =
           let uu___2 =
             ...
             FStarC_Syntax_Syntax.as_arg uu___3 in
           [uu___2] in
         FStarC_Syntax_Syntax.mk_Tm_app p uu___1 p.FStarC_Syntax_Syntax.pos
       else
         (let uu___1 = FStarC_Syntax_Syntax.bv_to_name x in
          apply_post p uu___1) in
     let t_is_unit =
       let uu___1 =
     ...
     let uu___1 =
       ...
       then
         let uu___2 =
           let uu___3 = FStarC_Syntax_Free.names body in
           FStarC_Class_Setlike.mem
             (FStarC_FlatSet.setlike_flat_set FStarC_Syntax_Syntax.ord_bv) x
             uu___3 in
         Prims.not uu___2
       else false in
     if uu___1 then mk_squash body else refine x body)
let post_of_result_typ (t : FStarC_Syntax_Syntax.typ) :
  FStarC_Syntax_Syntax.term=
  let uu___ = is_squash t in
  match uu___ with
  | FStar_Pervasives_Native.Some phi ->
      let uu___1 =
        ...
        [uu___2] in
      abs uu___1 phi FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.None ->
      let uu___1 =
        let uu___2 = FStarC_Syntax_Subst.compress t in
        uu___2.FStarC_Syntax_Syntax.n in
       ...
       | FStarC_Syntax_Syntax.Tm_refine
           ...
           let uu___2 =
             FStarC_Syntax_Subst.open_term [FStarC_Syntax_Syntax.mk_binder x]
               phi in
           (match uu___2 with
            | (bs, phi1) -> abs bs phi1 FStar_Pervasives_Native.None)
       | uu___2 -> FStarC_Syntax_Syntax.trivial_post t)
let bqual_compat (b1 : FStarC_Syntax_Syntax.bqual)
  (b2 : FStarC_Syntax_Syntax.bqual) : Prims.bool=
  match (b1, b2) with
  | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None) -> true
  | (FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Implicit uu___),
     FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Implicit uu___1)) ->
      true
  | (FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Implicit uu___),
     FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Meta uu___1)) -> true
...
let rec sizeof (t : FStarC_Syntax_Syntax.term) : Prims.int=
  ...
  | FStarC_Syntax_Syntax.Tm_abs
      ...
      let uu___2 =
        sizeof (b.FStarC_Syntax_Syntax.binder_bv).FStarC_Syntax_Syntax.sort in
      uu___1 + uu___2
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = hd;
        FStarC_Syntax_Syntax.arg = (a, uu___);_}
      -> let uu___1 = sizeof hd in let uu___2 = sizeof a in uu___1 + uu___2
  | uu___ -> Prims.int_one
                   ...
                   FStarC_Errors.raise_error
                     ...
                     (Obj.magic
                        [FStarC_Errors_Msg.text
                           (Prims.strcat "Failed to process pragma: " s1)])))
         ()
     with
...
let rec unbound_variables (tm : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = t1;
        FStarC_Syntax_Syntax.rc_opt = uu___;_}
      ->
      let uu___1 = FStarC_Syntax_Subst.open_term [b] t1 in
      (match uu___1 with
       | (bs, t2) ->
           let uu___2 =
             FStarC_List.collect
  ...
  | FStarC_Syntax_Syntax.Tm_refine
      ...
      let uu___ =
        FStarC_Syntax_Subst.open_term [FStarC_Syntax_Syntax.mk_binder b] t1 in
      (match uu___ with
       | (bs, t2) ->
           let uu___1 =
             FStarC_List.collect
  ...
  | FStarC_Syntax_Syntax.Tm_app
      { FStarC_Syntax_Syntax.hd = t1;
        FStarC_Syntax_Syntax.arg = (x, uu___);_}
      ->
      let uu___1 = unbound_variables x in
      let uu___2 = unbound_variables t1 in FStarC_List.op_At uu___1 uu___2
  | FStarC_Syntax_Syntax.Tm_match
      { FStarC_Syntax_Syntax.scrutinee = t1;
      ...
      let uu___1 =
        ...
        let uu___3 =
          ...
          | FStar_Pervasives.Inl bv ->
              let uu___4 =
                FStarC_Syntax_Subst.open_term
                  [FStarC_Syntax_Syntax.mk_binder bv] t1 in
...
let extract_attr' (attr_lid : FStarC_Ident.lid)
  ...
  let rec aux acc attrs1 =
    ...
    | [] -> FStar_Pervasives_Native.None
    | h::t ->
        let uu___ = head_and_args_full h in
        (match uu___ with
         | (head, args) ->
             let uu___1 =
               let uu___2 = FStarC_Syntax_Subst.compress head in
...
let comp_has_smt_pats (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  let uu___ = comp_smt_pats c in
  match uu___ with
  | FStar_Pervasives_Native.Some pats ->
      let uu___1 = let uu___2 = unmeta pats in head_and_args_full uu___2 in
      (match uu___1 with
       | (head, uu___2) ->
           let uu___3 =
             let uu___4 = un_uinst head in uu___4.FStarC_Syntax_Syntax.n in
           (match uu___3 with
            | FStarC_Syntax_Syntax.Tm_fvar fv ->
                FStarC_Syntax_Syntax.fv_eq_lid fv
...
let is_lemma_comp (c : FStarC_Syntax_Syntax.comp) : Prims.bool=
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      if
        FStarC_Ident.lid_equals ct.FStarC_Syntax_Syntax.source_effect_name
          FStarC_Parser_Const.effect_Lemma_lid
      then true
      else
        if FStarC_Parser_Const.is_tot_lid ct.FStarC_Syntax_Syntax.effect_name
        then comp_has_smt_pats c
        else false
  | uu___ -> false
...
let rec list_elements (e : FStarC_Syntax_Syntax.term) :
  ...
  let uu___ = let uu___1 = unmeta e in head_and_args_full uu___1 in
  match uu___ with
  | (head, args) ->
      let uu___1 =
        let uu___2 =
      ...
      (match uu___1 with
       ...
       | uu___2 -> FStar_Pervasives_Native.None)
let split_squash_binders (used : FStarC_Syntax_Syntax.term Prims.list)
  ...
  | b::rev_rest when
      ...
      let uu___ =
        un_squash
          (b.FStarC_Syntax_Syntax.binder_bv).FStarC_Syntax_Syntax.sort in
      (match uu___ with
       | FStar_Pervasives_Native.Some p when
           let uu___1 =
             FStarC_List.existsb
               (fun t ->
                  let uu___2 = FStarC_Syntax_Free.names t in
                  FStarC_Class_Setlike.mem
                    (FStarC_FlatSet.setlike_flat_set
                       FStarC_Syntax_Syntax.ord_bv)
                    b.FStarC_Syntax_Syntax.binder_bv uu___2) used in
           Prims.not uu___1 -> ((FStarC_List.rev rev_rest), p)
       | uu___1 -> (bs, t_true))
  | uu___ -> (bs, t_true)
let destruct_lemma_with_smt_patterns (t : FStarC_Syntax_Syntax.term) :
  ...
  let lemma_pats p =
    let smt_pat_or t1 =
      let uu___ = let uu___1 = unmeta t1 in head_and_args_full uu___1 in
      match uu___ with
      | (head, args) ->
          let uu___1 =
            let uu___2 =
    ...
    let one_pat p1 =
      let uu___ = let uu___1 = unmeta p1 in head_and_args_full uu___1 in
      match uu___ with
      | (head, args) ->
          let uu___1 =
            let uu___2 =
  ...
  | (bs, c) ->
      (match c.FStarC_Syntax_Syntax.n with
       | FStarC_Syntax_Syntax.Comp ct ->
           let uu___1 = comp_smt_pats c in
           (match uu___1 with
            | FStar_Pervasives_Native.Some pats ->
                let uu___2 =
                  split_squash_binders
                    [ct.FStarC_Syntax_Syntax.result_typ; pats] bs in
                (match uu___2 with
                 | (bs1, pre) ->
                     let post =
                       let uu___3 =
                         un_squash ct.FStarC_Syntax_Syntax.result_typ in
                       match uu___3 with
                       | FStar_Pervasives_Native.Some q -> q
                       | FStar_Pervasives_Native.None -> t_true in
                     let uu___3 =
                       let uu___4 = lemma_pats pats in
                       (bs1, pre, post, uu___4) in
...
let unthunk (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  ...
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.b = b; FStarC_Syntax_Syntax.body = e;
        FStarC_Syntax_Syntax.rc_opt = uu___1;_}
      ->
      ...
      (match uu___2 with
       | (bs, e1) ->
           ...
           if uu___3
           then mk_app t [FStarC_Syntax_Syntax.as_arg exp_unit]
           else e1)
  | uu___1 -> mk_app t [FStarC_Syntax_Syntax.as_arg exp_unit]
let smt_lemma_as_forall (t : FStarC_Syntax_Syntax.term)
  ...
  let uu___ =
    ...
    match uu___1 with
    | FStar_Pervasives_Native.None -> FStarC_Effect.failwith "impos"
...
let eff_decl_of_new_effect (se : FStarC_Syntax_Syntax.sigelt) :
  ...
  | uu___ ->
      FStarC_Effect.failwith "eff_decl_of_new_effect: not a Sig_new_effect"
let get_eff_repr (ed : FStarC_Syntax_Syntax.eff_decl) :
  ...
  | FStar_Pervasives_Native.Some c ->
      FStar_Pervasives_Native.Some (c.FStarC_Syntax_Syntax.repr)
let get_return_repr (ed : FStarC_Syntax_Syntax.eff_decl) :
  FStarC_Syntax_Syntax.tscheme FStar_Pervasives_Native.option=
  match ed.FStarC_Syntax_Syntax.combinators with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some c ->
      FStar_Pervasives_Native.Some (c.FStarC_Syntax_Syntax.return_repr)
let get_bind_repr (ed : FStarC_Syntax_Syntax.eff_decl) :
  FStarC_Syntax_Syntax.tscheme FStar_Pervasives_Native.option=
  match ed.FStarC_Syntax_Syntax.combinators with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some c ->
      FStar_Pervasives_Native.Some (c.FStarC_Syntax_Syntax.bind_repr)
let get_repr_universe (ed : FStarC_Syntax_Syntax.eff_decl) :
  FStarC_Syntax_Syntax.tscheme FStar_Pervasives_Native.option=
  match ed.FStarC_Syntax_Syntax.combinators with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some c ->
      FStar_Pervasives_Native.Some (c.FStarC_Syntax_Syntax.repr_universe)
let apply_eff_combinators
  ...
  let uu___ = f combs.FStarC_Syntax_Syntax.repr in
  let uu___1 = f combs.FStarC_Syntax_Syntax.return_repr in
  let uu___2 = f combs.FStarC_Syntax_Syntax.bind_repr in
  let uu___3 = f combs.FStarC_Syntax_Syntax.repr_universe in
  {
    FStarC_Syntax_Syntax.repr = uu___;
    FStarC_Syntax_Syntax.return_repr = uu___1;
    FStarC_Syntax_Syntax.bind_repr = uu___2;
  ...
  let head =
    FStarC_Syntax_Syntax.fvar
      (FStarC_Ident.set_lid_range FStarC_Parser_Const.reveal
         v.FStarC_Syntax_Syntax.pos) FStar_Pervasives_Native.None in
  let uu___ = FStarC_Syntax_Syntax.mk_Tm_uinst head [u] in
  FStarC_Syntax_Syntax.mk_Tm_app uu___
    [FStarC_Syntax_Syntax.iarg ty; FStarC_Syntax_Syntax.as_arg v]
...
let eq_binding (b1 : FStarC_Syntax_Syntax.binding)
  ...
  | (FStarC_Syntax_Syntax.Binding_var bv1, FStarC_Syntax_Syntax.Binding_var
     bv2) ->
      if FStarC_Syntax_Syntax.bv_eq bv1 bv2
      then
