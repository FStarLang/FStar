        ...
        (FStarC_List.iter
         ...
         then
           (if errs = Prims.int_one
            then
              FStarC_Format.print_error "1 error was reported (see above)\n"
            else
              (let uu___3 =
...
let (set_error_trap : unit -> unit) =
  fun uu___ ->
    ...
    let h' s =
      ...
      (let uu___5 =
         let uu___6 = FStarC_Errors_Msg.text "GOT SIGINT! Exiting" in
         [uu___6] in
                  ...
                  (check_no_filenames "--locate_file";
                   (let f =
                    ...
                    | FStar_Pervasives_Native.Some fn ->
                        ((let uu___10 =
                            FStarC_Filepath.normalize_file_path fn in
                          FStarC_Format.print1 "%s\n" uu___10);
                         FStarC_Effect.exit Prims.int_zero)))
              | FStarC_Getopt.Success when
