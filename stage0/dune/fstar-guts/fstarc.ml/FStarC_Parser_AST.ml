...
let (__proj__Mkterm__item__range : term -> FStarC_Range_Type.range) =
  fun projectee ->
...
let (__proj__Mkbinder__item__brange : binder -> FStarC_Range_Type.range) =
  fun projectee ->
...
let (__proj__Mkpattern__item__prange : pattern -> FStarC_Range_Type.range) =
...
let (__proj__Mkdecl__item__drange : decl -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
  ...
  fun s ->
    fun r ->
      fun l ->
        ...
        | uu___ ->
            let uu___1 =
              FStarC_Util.format1 "At most one %s is allowed on declarations"
                s in
...
let (mk_binder_with_attrs :
  binder' ->
    FStarC_Range_Type.range -> level -> aqual -> term Prims.list -> binder)
  =
  fun b ->
    fun r ->
      fun l ->
...
let (mk_binder :
  binder' -> FStarC_Range_Type.range -> level -> aqual -> binder) =
  fun b -> fun r -> fun l -> fun i -> mk_binder_with_attrs b r l i []
let (mk_term : term' -> FStarC_Range_Type.range -> level -> term) =
  fun t -> fun r -> fun l -> { tm = t; range = r; level = l }
let (mk_uminus :
  term -> FStarC_Range_Type.range -> FStarC_Range_Type.range -> level -> term)
  =
  fun t ->
    fun rminus ->
      fun r ->
        fun l ->
          let t1 =
            ...
            | uu___ ->
                ...
                Op uu___1 in
          mk_term t1 r l
let (mk_pattern : pattern' -> FStarC_Range_Type.range -> pattern) =
...
let (mk_function :
  branch Prims.list ->
    FStarC_Range_Type.range -> FStarC_Range_Type.range -> term)
  =
  fun branches ->
...
let (mkApp :
  term -> (term * imp) Prims.list -> FStarC_Range_Type.range -> term) =
  fun t ->
    fun args ->
      fun r ->
        match args with
            ...
            (match t.tm with
             ...
             | uu___1 ->
                 FStarC_List.fold_left
                   ...
                   args)
let (consPat : FStarC_Range_Type.range -> pattern -> pattern -> pattern') =
  fun r ->
    fun hd ->
      fun tl ->
        let uu___ =
          ...
          (uu___1, [hd; tl]) in
        PatApp uu___
let (consTerm : FStarC_Range_Type.range -> term -> term -> term) =
  fun r ->
    fun hd ->
      fun tl ->
        mk_term
          ...
          r Expr
let (mkListLit : FStarC_Range_Type.range -> term Prims.list -> term) =
  fun r -> fun elts -> mk_term (ListLiteral elts) r Expr
let (mkSeqLit : FStarC_Range_Type.range -> term Prims.list -> term) =
  fun r -> fun elts -> mk_term (SeqLiteral elts) r Expr
let (unit_const : FStarC_Range_Type.range -> term) =
...
let (tot_comp : term -> term) =
  fun t ->
    ...
    let t1 = mk_term (App (ml, t, Nothing)) t.range Expr in t1
let (mkRefSet : FStarC_Range_Type.range -> term Prims.list -> term) =
  fun r ->
    fun elts ->
      let uu___ =
      ...
      | (empty_lid, singleton_lid, union_lid, addr_of_lid) ->
          ...
          FStarC_List.fold_right
            ...
            empty
let (mkExplicitApp :
  term -> term Prims.list -> FStarC_Range_Type.range -> term) =
  fun t ->
    fun args ->
      fun r ->
        match args with
            ...
            (match t.tm with
             | Name s ->
                 let uu___1 =
                   let uu___2 =
                     let uu___3 =
                       FStarC_List.map (fun a -> (a, Nothing)) args in
                     (s, uu___3) in
                   Construct uu___2 in
                 mk_term uu___1 r Un
             | uu___1 ->
                 FStarC_List.fold_left
                   ...
                   args)
let (mkAdmitMagic : FStarC_Range_Type.range -> term) =
  fun r ->
    let admit =
      let admit_name =
        let uu___ =
    ...
    fun r ->
      ...
      then
        (FStarC_Errors.log_issue FStarC_Class_HasRange.hasRange_range r
         ...
         (let focussed =
            let uu___1 =
              FStarC_List.filter FStar_Pervasives_Native.fst branches in
...
let (focusLetBindings :
  (Prims.bool * (pattern * term)) Prims.list ->
    FStarC_Range_Type.range -> (pattern * term) Prims.list)
  =
  fun lbs ->
    fun r ->
      let should_filter =
        FStarC_Util.for_some FStar_Pervasives_Native.fst lbs in
      if should_filter
      then
        (FStarC_Errors.log_issue FStarC_Class_HasRange.hasRange_range r
           FStarC_Errors_Codes.Warning_Filtered ()
...
let (mkFsTypApp : term -> term Prims.list -> FStarC_Range_Type.range -> term)
  =
  fun t ->
    fun args ->
      fun r ->
        let uu___ = FStarC_List.map (fun a -> (a, FsTypApp)) args in
        mkApp t uu___ r
let (mkTuple : term Prims.list -> FStarC_Range_Type.range -> term) =
  fun args ->
    fun r ->
      let cons =
...
let (mkDTuple : term Prims.list -> FStarC_Range_Type.range -> term) =
  fun args ->
    fun r ->
      let cons =
    ...
    term ->
      Prims.bool ->
        term FStar_Pervasives_Native.option ->
          FStarC_Range_Type.range -> aqual -> term Prims.list -> binder)
  =
  fun id ->
    fun t ->
      fun should_bind_var ->
...
let (mkRefinedPattern :
  pattern ->
    term ->
      Prims.bool ->
        term FStar_Pervasives_Native.option ->
          FStarC_Range_Type.range -> FStarC_Range_Type.range -> pattern)
  =
  fun pat ->
    fun t ->
      fun should_bind_pat ->
...
let (strip_prefix :
  ...
  fun prefix ->
    fun s ->
      ...
      then
        let uu___ =
          FStarC_Util.substring_from s (FStarC_String.length prefix) in
        FStar_Pervasives_Native.Some uu___
      else FStar_Pervasives_Native.None
let (compile_op :
  Prims.int -> Prims.string -> FStarC_Range_Type.range -> Prims.string) =
  fun arity ->
    fun s ->
      fun r ->
        let name_of_char uu___ =
          ...
          | c ->
              let uu___1 =
                FStarC_Util.string_of_int (FStarC_Util.int_of_char c) in
            ...
            let uu___1 =
              ...
              then
                ...
                then
                  (let uu___3 =
                     let uu___4 =
                       FStarC_Util.substring s Prims.int_zero
                         (Prims.of_int (6)) in
                     Prims.strcat uu___4 "_" in
                   let uu___4 =
                     FStarC_Util.substring_from s (Prims.of_int (6)) in
                   (uu___3, uu___4))
                else ("", s) in
            (match uu___1 with
             | (prefix, s1) ->
...
let (compile_op' : Prims.string -> FStarC_Range_Type.range -> Prims.string) =
...
let rec (term_to_string : term -> Prims.string) =
  fun x ->
    ...
    | LexList l ->
        ...
        FStarC_Util.format1 "%[%s]" uu___
    | Decreases (t, uu___) ->
        let uu___1 = term_to_string t in
        FStarC_Util.format1 "(decreases %s)" uu___1
    | Requires (t, uu___) ->
        let uu___1 = term_to_string t in
        FStarC_Util.format1 "(requires %s)" uu___1
    | Ensures (t, uu___) ->
        let uu___1 = term_to_string t in
        FStarC_Util.format1 "(ensures %s)" uu___1
    | Labeled (t, l, uu___) ->
        let uu___1 = term_to_string t in
        FStarC_Util.format2 "(labeled %s %s)" l uu___1
          ...
          to_string_l " "
            (fun uu___2 ->
               ...
               | (a, imp1) ->
                   ...
                   FStarC_Util.format2 "%s%s" (imp_to_string imp1) uu___3)
            args in
        FStarC_Util.format2 "(%s %s)" uu___ uu___1
    | Function (branches, r) ->
        let uu___ =
          to_string_l " | "
            (fun uu___1 ->
               ...
               | (p, w, e) ->
                   ...
                   FStarC_Util.format2 "%s -> %s" uu___2 uu___3) branches in
        FStarC_Util.format1 "(function %s)" uu___
    | Abs (pats, t) ->
        let uu___ = to_string_l " " pat_to_string pats in
        let uu___1 = term_to_string t in
        FStarC_Util.format2 "(fun %s -> %s)" uu___ uu___1
    | App (t1, t2, imp1) ->
        let uu___ = term_to_string t1 in
        let uu___1 = term_to_string t2 in
        FStarC_Util.format3 "%s %s%s" uu___ (imp_to_string imp1) uu___1
    | Let (Rec, (a, (p, b))::lbs, body) ->
        let uu___ = attrs_opt_to_string a in
        let uu___1 =
          let uu___2 = pat_to_string p in
          let uu___3 = term_to_string b in
          FStarC_Util.format2 "%s=%s" uu___2 uu___3 in
        let uu___2 =
          to_string_l " "
            (fun uu___3 ->
               match uu___3 with
               | (a1, (p1, b1)) ->
                   ...
                   FStarC_Util.format3 "%sand %s=%s" uu___4 uu___5 uu___6)
            lbs in
        let uu___3 = term_to_string body in
        FStarC_Util.format4 "%slet rec %s%s in %s" uu___ uu___1 uu___2 uu___3
    | Let (q, (attrs, (pat, tm))::[], body) ->
        ...
        FStarC_Util.format5 "%slet %s %s = %s in %s" uu___ uu___1 uu___2
          uu___3 uu___4
    ...
    | LetOpen (lid, t) ->
        ...
        let uu___1 = term_to_string t in
        FStarC_Util.format2 "let open %s in %s" uu___ uu___1
    | Seq (t1, t2) ->
        let uu___ = term_to_string t1 in
        let uu___1 = term_to_string t2 in
        FStarC_Util.format2 "%s; %s" uu___ uu___1
          ...
          | FStar_Pervasives_Native.Some (as_opt, ret, use_eq) ->
              ...
              let uu___3 =
                ...
                | FStar_Pervasives_Native.Some as_ident ->
                    ...
                    FStarC_Util.format1 " as %s " uu___4 in
              let uu___4 = term_to_string ret in
              FStarC_Util.format3 "%s%s %s " uu___3 s uu___4 in
        let uu___3 = term_to_string t2 in
        let uu___4 = term_to_string t3 in
        FStarC_Util.format5 "if%s %s %sthen %s else %s" uu___ uu___1 uu___2
          uu___3 uu___4
    ...
    | Ascribed (t1, t2, FStar_Pervasives_Native.None, flag) ->
        ...
        let uu___ = term_to_string t1 in
        let uu___1 = term_to_string t2 in
        FStarC_Util.format3 "(%s %s %s)" uu___ s uu___1
    | Ascribed (t1, t2, FStar_Pervasives_Native.Some tac, flag) ->
        let s = if flag then "$:" else "<:" in
        let uu___ = term_to_string t1 in
        let uu___1 = term_to_string t2 in
        let uu___2 = term_to_string tac in
        FStarC_Util.format4 "(%s %s %s by %s)" uu___ s uu___1 uu___2
          ...
          to_string_l " "
            (fun uu___2 ->
               ...
               | (l, e1) ->
                   ...
                   FStarC_Util.format2 "%s=%s" uu___3 uu___4) fields in
        FStarC_Util.format2 "{%s with %s}" uu___ uu___1
          ...
          to_string_l " "
            (fun uu___1 ->
               ...
               | (l, e) ->
                   ...
                   let uu___3 = term_to_string e in
                   FStarC_Util.format2 "%s=%s" uu___2 uu___3) fields in
        FStarC_Util.format1 "{%s}" uu___
    | Project (e, l) ->
        let uu___ = term_to_string e in
    ...
    | QForall (bs, (uu___, pats), t) ->
        ...
        let uu___3 = term_to_string t in
        FStarC_Util.format3 "forall %s.{:pattern %s} %s" uu___1 uu___2 uu___3
    | QExists (bs, (uu___, pats), t) ->
        let uu___1 = to_string_l " " binder_to_string bs in
        let uu___2 =
          to_string_l " \\/ " (to_string_l "; " term_to_string) pats in
        let uu___3 = term_to_string t in
        FStarC_Util.format3 "exists %s.{:pattern %s} %s" uu___1 uu___2 uu___3
    ...
    | QuantOp (i, bs, (uu___, pats), t) ->
        ...
        FStarC_Util.format4 "%s %s.{:pattern %s} %s" uu___1 uu___2 uu___3
          uu___4
    | Refine (b, t) ->
        let uu___ = binder_to_string b in
        let uu___1 = term_to_string t in
    ...
    | NamedTyp (x1, t) ->
        ...
        FStarC_Util.format2 "%s:%s" uu___ uu___1
    | Paren t ->
        let uu___ = term_to_string t in FStarC_Util.format1 "(%s)" uu___
    | Product (bs, t) ->
        let uu___ =
          let uu___1 = FStarC_List.map binder_to_string bs in
          FStarC_String.concat "," uu___1 in
        let uu___1 = term_to_string t in
        FStarC_Util.format2 "Unidentified product: [%s] %s" uu___ uu___1
    | Discrim lid ->
        let uu___ = FStarC_Ident.string_of_lid lid in
        FStarC_Util.format1 "%s?" uu___
    | Attributes ts ->
        let uu___ =
          ...
          FStarC_String.concat " " uu___1 in
        FStarC_Util.format1 "(attributes %s)" uu___
    | Antiquote t ->
        let uu___ = term_to_string t in FStarC_Util.format1 "(`#%s)" uu___
    | Quote (t, Static) ->
        let uu___ = term_to_string t in FStarC_Util.format1 "(`(%s))" uu___
    | Quote (t, Dynamic) ->
        ...
        FStarC_Util.format1 "quote (%s)" uu___
    | VQuote t ->
        let uu___ = term_to_string t in FStarC_Util.format1 "`%%%s" uu___
    | CalcProof (rel, init, steps) ->
        ...
        FStarC_Util.format3 "calc (%s) { %s %s }" uu___ uu___1 uu___2
    | ElimForall (bs, t, vs) ->
        ...
        let uu___2 =
          ...
          FStarC_String.concat " " uu___3 in
        FStarC_Util.format3 "_elim_ forall %s. %s using %s" uu___ uu___1
          uu___2
    | ElimExists (bs, p, q, b, e) ->
        ...
        FStarC_Util.format5 "_elim_ exists %s. %s _to_ %s\n\\with %s. %s"
    ...
    | ElimImplies (p, q, e) ->
        ...
        FStarC_Util.format3 "_elim_ %s ==> %s with %s" uu___ uu___1 uu___2
    ...
    | ElimAnd (p, q, r, x1, y, e) ->
        let uu___ =
          ...
          let uu___2 =
            ...
            uu___3 :: uu___4 in
          uu___1 :: uu___2 in
        FStarC_Util.format "_elim_ %s /\\ %s _to_ %s\n\\with %s %s. %s" uu___
    | IntroForall (xs, p, e) ->
        let uu___ = binders_to_string " " xs in
        let uu___1 = term_to_string p in
        let uu___2 = term_to_string e in
        FStarC_Util.format3 "_intro_ forall %s. %s with %s" uu___ uu___1
          uu___2
    | IntroExists (xs, t, vs, e) ->
        ...
        let uu___2 =
          ...
          FStarC_String.concat " " uu___3 in
        let uu___3 = term_to_string e in
        FStarC_Util.format4 "_intro_ exists %s. %s using %s with %s" uu___
    ...
    | IntroImplies (p, q, x1, e) ->
        ...
        FStarC_Util.format4 "_intro_ %s ==> %s with %s. %s" uu___ uu___1
          uu___2 uu___3
    | IntroOr (b, p, q, r) ->
        let uu___ = term_to_string p in
        let uu___1 = term_to_string q in
        let uu___2 = term_to_string r in
        FStarC_Util.format4 "_intro_ %s \\/ %s using %s with %s" uu___ uu___1
    ...
    | IntroAnd (p, q, e1, e2) ->
        ...
        FStarC_Util.format4 "_intro_ %s /\\ %s with %s and %s" uu___ uu___1
          uu___2 uu___3
    | ListLiteral ts ->
    ...
    | SeqLiteral ts ->
        let uu___ = to_string_l "; " term_to_string ts in
        FStarC_Util.format1 "seq![%s]" uu___
              ...
              | FStar_Pervasives_Native.Some (as_opt, ret, use_eq) ->
                  ...
                  let uu___3 =
                    ...
                    | FStar_Pervasives_Native.Some as_ident ->
                        ...
                        FStarC_Util.format1 "as %s " uu___4 in
                  let uu___4 = term_to_string ret in
                  FStarC_Util.format3 "%s%s %s " s1 uu___3 uu___4 in
            let uu___3 =
              to_string_l " | "
                (fun uu___4 ->
                   match uu___4 with
                   | (p, w, e) ->
                       ...
                       let uu___6 =
                         ...
                         | FStar_Pervasives_Native.Some e1 ->
                             ...
                             FStarC_Util.format1 "when %s" uu___7 in
                       let uu___7 = term_to_string e in
                       FStarC_Util.format3 "%s %s -> %s" uu___5 uu___6 uu___7)
                branches in
            FStarC_Util.format5 "%s%s %s %swith %s" s uu___ uu___1 uu___2
              uu___3
...
and (binder_to_string : binder -> Prims.string) =
  fun x ->
    let pr x1 =
      ...
      FStarC_Util.format3 "%s%s%s" uu___ uu___1 s in
    match x.aqual with
...
and (pat_to_string : pattern -> Prims.string) =
  fun x ->
          ...
          to_string_l "; "
            (fun uu___1 ->
               ...
               | (f, e) ->
                   ...
                   FStarC_Util.format2 "%s=%s" uu___2 uu___3) l in
        FStarC_Util.format1 "{%s}" uu___
    | PatOr l -> to_string_l "|\n " pat_to_string l
    | PatOp op ->
    ...
    | PatAscribed (p, (t, FStar_Pervasives_Native.Some tac)) ->
        ...
        let uu___1 = term_to_string t in
        let uu___2 = term_to_string tac in
        FStarC_Util.format3 "(%s:%s by %s)" uu___ uu___1 uu___2
...
let (string_of_pragma : pragma -> Prims.string) =
  fun uu___ ->
    ...
    | ResetOptions s ->
        FStarC_Util.format1 "reset-options \"%s\"" (FStarC_Util.dflt "" s)
    | PushOptions s ->
        FStarC_Util.format1 "push-options \"%s\"" (FStarC_Util.dflt "" s)
...
let rec (decl_to_string : decl -> Prims.string) =
  fun d ->
    ...
    | ModuleAbbrev (i, l) ->
        let uu___ = FStarC_Ident.string_of_id i in
        let uu___1 = FStarC_Ident.string_of_lid l in
        FStarC_Util.format2 "module %s = %s" uu___ uu___1
    ...
    | Polymonadic_bind (l1, l2, l3, uu___) ->
        ...
        FStarC_Util.format3 "polymonadic_bind (%s, %s) |> %s" uu___1 uu___2
          uu___3
    | Polymonadic_subcomp (l1, l2, uu___) ->
        let uu___1 = FStarC_Ident.string_of_lid l1 in
        let uu___2 = FStarC_Ident.string_of_lid l2 in
        FStarC_Util.format2 "polymonadic_subcomp %s <: %s" uu___1 uu___2
    ...
    | UseLangDecls str -> FStarC_Util.format1 "#lang-%s" str
...
let (ident_of_binder :
  FStarC_Range_Type.range -> binder -> FStarC_Ident.ident) =
  fun r ->
    fun b ->
      match b.b with
      | Variable i -> i
...
let (idents_of_binders :
  binder Prims.list ->
    FStarC_Range_Type.range -> FStarC_Ident.ident Prims.list)
  = fun bs -> fun r -> FStarC_List.map (ident_of_binder r) bs
...
let (add_decorations : decl -> decoration Prims.list -> decl) =
  fun d ->
    fun decorations ->
            ...
            let attrs1 =
              ...
              | uu___1 ->
                  ...
                  FStarC_Errors.raise_error hasRange_decl d
                    ...
                    (Obj.magic uu___2) in
            let uu___1 =
              FStarC_List.map (fun uu___2 -> Qualifier uu___2) d.quals in
      ...
      let attributes_2 = FStarC_Util.dflt [] attributes_1 in
      let qualifiers1 =
        FStarC_List.choose
          (fun uu___ ->
             match uu___ with
...
let (mk_decl :
  decl' -> FStarC_Range_Type.range -> decoration Prims.list -> decl) =
  fun d ->
    fun r ->
      fun decorations ->
        let d1 =
