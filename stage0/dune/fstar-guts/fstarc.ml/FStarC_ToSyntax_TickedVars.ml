open Prims
let (ident_is_ticked : FStarC_Ident.ident -> Prims.bool) =
  fun id ->
...
let (lident_is_ticked : FStarC_Ident.lident -> Prims.bool) =
  fun id ->
              ...
              (FStarC_Class_Setlike.union ()
                 (Obj.magic
                    (FStarC_RBSet.setlike_rbset
                       FStarC_Syntax_Syntax.ord_ident)))) uu___2 uu___1)
  }
type 'a m =
  (FStarC_Ident.ident FStarC_RBSet.t, Obj.t, 'a) FStarC_Writer.writer
let (emit1 : FStarC_Ident.ident -> unit m) =
  fun x ->
    let uu___ =
      Obj.magic
...
let rec (go_term :
  FStarC_Syntax_DsEnv.env -> FStarC_Parser_AST.term -> unit m) =
  fun env ->
    fun t ->
                       ...
                       (fun uu___4 ->
                          (fun uu___4 ->
                             ...
                             | FStar_Pervasives_Native.Some tac ->
                                 Obj.magic (go_term env tac)) uu___4)))
                 uu___2)
          ...
          FStarC_Class_Monad.iterM (FStarC_Writer.monad_writer uu___0) ()
            (fun uu___1 ->
               (fun uu___1 ->
                  ...
                  match uu___1 with
                  | (a, uu___2) -> Obj.magic (go_term env a)) uu___1)
            (Obj.magic ts)
                                             ...
                                             Obj.magic
                                               (FStarC_Class_Monad.return
                                                  (FStarC_Writer.monad_writer
                                                     uu___0) ()
                                                  (Obj.magic env1))) uu___2))))
                        uu___2 uu___1) (Obj.magic env) (Obj.magic binders)) in
          FStarC_Class_Monad.op_let_Bang (FStarC_Writer.monad_writer uu___0)
            () () (Obj.magic uu___)
            (fun uu___1 ->
               (fun env' ->
          ...
          let uu___ =
            Obj.magic
              (FStarC_Class_Monad.foldM_left
                 (FStarC_Writer.monad_writer uu___0) () ()
                 (fun uu___2 ->
                    fun uu___1 -> (Obj.magic go_binder) uu___2 uu___1)
...
and (go_binder :
  FStarC_Syntax_DsEnv.env ->
    FStarC_Parser_AST.binder -> FStarC_Syntax_DsEnv.env m)
  =
  fun uu___1 ->
    fun uu___ ->
      (fun env ->
           ...
           | FStarC_Parser_AST.Variable x ->
               let uu___ =
                 ...
                 then emit1 x
                   ...
                   FStarC_Class_Monad.return
                     (FStarC_Writer.monad_writer uu___0) () (Obj.repr ()) in
               Obj.magic
                 (FStarC_Class_Monad.op_let_Bang
...
and (go_binders :
  FStarC_Syntax_DsEnv.env ->
    FStarC_Parser_AST.binder Prims.list -> FStarC_Syntax_DsEnv.env m)
  =
  fun uu___1 ->
    fun uu___ ->
      (fun env ->
         fun bs ->
           Obj.magic
             (FStarC_Class_Monad.foldM_left
                (FStarC_Writer.monad_writer uu___0) () ()
                (fun uu___1 ->
                   fun uu___ -> (Obj.magic go_binder) uu___1 uu___)
                (Obj.magic env) (Obj.magic bs))) uu___1 uu___
let (free_ticked_vars :
  FStarC_Syntax_DsEnv.env ->
