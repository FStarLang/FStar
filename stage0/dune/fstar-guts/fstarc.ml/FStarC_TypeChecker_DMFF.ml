...
let __proj__Mkenv__item__subst (projectee : env) :
  FStarC_Syntax_Syntax.subst_elt Prims.list=
  match projectee with | { tcenv; subst; tc_const;_} -> subst
let __proj__Mkenv__item__tc_const (projectee : env) :
...
let d (s : Prims.string) : unit=
  FStarC_Format.print1 "\027[01;36m%s\027[00m\n" s
let mk_toplevel_definition (env1 : FStarC_TypeChecker_Env.env_t)
  (lident : FStarC_Ident.lident) (def : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.sigelt * FStarC_Syntax_Syntax.term)=
  (let uu___1 = FStarC_Effect.op_Bang dbg in
   if uu___1
   then
     (d (FStarC_Ident.string_of_lid lident);
      (let uu___3 =
       ...
       FStarC_Format.print2 "Registering top-level definition: %s\n%s\n"
         uu___3 uu___4))
   else ());
  (let fv =
   ...
   let lb =
     (false,
       [FStarC_Syntax_Util.mk_letbinding lbname [] FStarC_Syntax_Syntax.tun
   ...
   ({
      ...
      FStarC_Syntax_Syntax.sigrng = (sig_ctx.FStarC_Syntax_Syntax.sigrng);
      FStarC_Syntax_Syntax.sigquals =
        [FStarC_Syntax_Syntax.Unfold_for_unification_and_vcgen];
      FStarC_Syntax_Syntax.sigmeta = (sig_ctx.FStarC_Syntax_Syntax.sigmeta);
      FStarC_Syntax_Syntax.sigattrs = (sig_ctx.FStarC_Syntax_Syntax.sigattrs);
      FStarC_Syntax_Syntax.sigopens_and_abbrevs =
        (sig_ctx.FStarC_Syntax_Syntax.sigopens_and_abbrevs);
      FStarC_Syntax_Syntax.sigopts = (sig_ctx.FStarC_Syntax_Syntax.sigopts)
    }, uu___1))
let empty (env1 : FStarC_TypeChecker_Env.env)
  (tc_const : FStarC_Const.sconst -> FStarC_Syntax_Syntax.typ) : env=
  { tcenv = env1; subst = []; tc_const }
let gen_wps_for_free (env1 : FStarC_TypeChecker_Env.env)
  (binders : FStarC_Syntax_Syntax.binders) (a : FStarC_Syntax_Syntax.bv)
  (wp_a : FStarC_Syntax_Syntax.term) (ed : FStarC_Syntax_Syntax.eff_decl) :
  (FStarC_Syntax_Syntax.sigelts * FStarC_Syntax_Syntax.eff_decl)=
  let wp_a1 =
    FStarC_TypeChecker_Normalize.normalize
      [FStarC_TypeChecker_Env.Beta; FStarC_TypeChecker_Env.EraseUniverses]
      env1 wp_a in
  let a1 =
    let uu___ =
      FStarC_TypeChecker_Normalize.normalize
  ...
  let d1 s = FStarC_Format.print1 "\027[01;36m%s\027[00m\n" s in
  (let uu___1 = FStarC_Effect.op_Bang dbg in
   if uu___1
   then
     (d1 "Elaborating extra WP combinators";
      (let uu___3 =
         FStarC_Class_Show.show FStarC_Syntax_Print.showable_term wp_a1 in
       FStarC_Format.print1 "wp_a is: %s\n" uu___3))
   else ());
      ...
      let uu___3 =
        let uu___4 =
          FStarC_Class_Show.show
            ...
            gamma in
        FStarC_Format.fmt1 "Gamma is %s\n" uu___4 in
      d1 uu___3
    else ());
   (let unknown = FStarC_Syntax_Syntax.tun in
    let mk x = FStarC_Syntax_Syntax.mk x FStarC_Range_Type.dummyRange in
    ...
    let register env2 lident def =
      ...
      | (sigelt, fv) ->
          ...
          ((let uu___4 =
              let uu___5 = FStarC_Effect.op_Bang sigelts in sigelt1 :: uu___5 in
    ...
    let binders_of_list =
      FStarC_List.map
        (fun uu___2 ->
           match uu___2 with
           | (t, b) ->
               FStarC_Syntax_Syntax.mk_binder_with_attrs t
                 (FStarC_Syntax_Syntax.as_bqual_implicit b)
                 FStar_Pervasives_Native.None []) in
    let mk_all_implicit =
      FStarC_List.map
        (fun t ->
           {
             ...
             FStarC_Syntax_Syntax.binder_qual =
               (FStarC_Syntax_Syntax.as_bqual_implicit true);
    ...
    let _ctx_gctx =
      let _ctx_gctx1 =
        let mk1 f =
          let t =
            FStarC_Syntax_Syntax.gen_bv "t" FStar_Pervasives_Native.None
              FStarC_Syntax_Util.ktype in
          let body =
            let uu___2 =
              let uu___3 = FStarC_Syntax_Syntax.bv_to_name t in f uu___3 in
            FStarC_Syntax_Util.arrow gamma uu___2 in
          FStarC_Syntax_Util.abs
            (FStarC_List.op_At binders
               [FStarC_Syntax_Syntax.mk_binder a1;
               FStarC_Syntax_Syntax.mk_binder t]) body
            FStar_Pervasives_Native.None in
        let uu___2 = mk1 FStarC_Syntax_Syntax.mk_Total in
        let uu___3 = mk1 FStarC_Syntax_Syntax.mk_GTotal in (uu___2, uu___3) in
      let ctx_def = FStar_Pervasives_Native.fst _ctx_gctx1 in
      let gctx_def = FStar_Pervasives_Native.snd _ctx_gctx1 in
      let ctx_lid = mk_lid "ctx" in
      let ctx_fv = register env1 ctx_lid ctx_def in
      let gctx_lid = mk_lid "gctx" in
      let gctx_fv = register env1 gctx_lid gctx_def in (ctx_fv, gctx_fv) in
    let ctx_fv = FStar_Pervasives_Native.fst _ctx_gctx in
    let gctx_fv = FStar_Pervasives_Native.snd _ctx_gctx in
              ...
              FStarC_List.map
                (fun uu___6 ->
                   ...
                   | { FStarC_Syntax_Syntax.binder_bv = bv;
                       ...
                       (uu___10,
                         (FStarC_Syntax_Syntax.as_aqual_implicit false)))
                binders in
            let uu___6 =
              let uu___7 =
                let uu___8 = FStarC_Syntax_Syntax.bv_to_name a1 in
                (uu___8, (FStarC_Syntax_Syntax.as_aqual_implicit false)) in
              [uu___7; (t, (FStarC_Syntax_Syntax.as_aqual_implicit false))] in
            FStarC_List.op_At uu___5 uu___6 in
          { FStarC_Syntax_Syntax.hd = fv; FStarC_Syntax_Syntax.args = uu___4
          } in
    ...
    let c_pure =
      let t =
        FStarC_Syntax_Syntax.gen_bv "t" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let x =
        let uu___2 = FStarC_Syntax_Syntax.bv_to_name t in
        FStarC_Syntax_Syntax.gen_bv "x" FStar_Pervasives_Native.None uu___2 in
      let ret =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name t in mk_ctx uu___4 in
          FStarC_Syntax_Util.residual_tot uu___3 in
        FStar_Pervasives_Native.Some uu___2 in
      let body =
        let uu___2 = FStarC_Syntax_Syntax.bv_to_name x in
        FStarC_Syntax_Util.abs gamma uu___2 ret in
      let uu___2 =
        let uu___3 = mk_all_implicit binders in
        let uu___4 = binders_of_list [(a1, true); (t, true); (x, false)] in
        FStarC_List.op_At uu___3 uu___4 in
      FStarC_Syntax_Util.abs uu___2 body ret in
    let c_pure1 = let uu___2 = mk_lid "pure" in register env1 uu___2 c_pure in
    let c_app =
      let t1 =
        FStarC_Syntax_Syntax.gen_bv "t1" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let t2 =
        FStarC_Syntax_Syntax.gen_bv "t2" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let l =
        let uu___2 =
          let uu___3 =
            let uu___4 =
              let uu___5 =
                let uu___6 =
                  let uu___7 = FStarC_Syntax_Syntax.bv_to_name t1 in
            ...
            let uu___5 =
              ...
              FStarC_Syntax_Syntax.mk_GTotal uu___6 in
            FStarC_Syntax_Util.arrow uu___4 uu___5 in
          mk_gctx uu___3 in
        FStarC_Syntax_Syntax.gen_bv "l" FStar_Pervasives_Native.None uu___2 in
      let r =
        let uu___2 =
          let uu___3 = FStarC_Syntax_Syntax.bv_to_name t1 in mk_gctx uu___3 in
        FStarC_Syntax_Syntax.gen_bv "r" FStar_Pervasives_Native.None uu___2 in
      let ret =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name t2 in mk_gctx uu___4 in
          FStarC_Syntax_Util.residual_tot uu___3 in
        FStar_Pervasives_Native.Some uu___2 in
      let outer_body =
        let gamma_as_args = args_of_binders gamma in
        let inner_body =
          let uu___2 = FStarC_Syntax_Syntax.bv_to_name l in
          let uu___3 =
            let uu___4 =
              let uu___5 =
                let uu___6 =
                  let uu___7 = FStarC_Syntax_Syntax.bv_to_name r in
                  FStarC_Syntax_Util.mk_app uu___7 gamma_as_args in
                FStarC_Syntax_Syntax.as_arg uu___6 in
              [uu___5] in
            FStarC_List.op_At gamma_as_args uu___4 in
          FStarC_Syntax_Util.mk_app uu___2 uu___3 in
        FStarC_Syntax_Util.abs gamma inner_body ret in
      let uu___2 =
        let uu___3 = mk_all_implicit binders in
        let uu___4 =
          binders_of_list
            [(a1, true); (t1, true); (t2, true); (l, false); (r, false)] in
        FStarC_List.op_At uu___3 uu___4 in
      FStarC_Syntax_Util.abs uu___2 outer_body ret in
    let c_app1 = let uu___2 = mk_lid "app" in register env1 uu___2 c_app in
    let c_lift1 =
      let t1 =
        FStarC_Syntax_Syntax.gen_bv "t1" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      ...
      let t_f =
        let uu___2 =
          let uu___3 =
            ...
            FStarC_Syntax_Syntax.null_binder uu___4 in
          [uu___3] in
        let uu___3 =
          let uu___4 = FStarC_Syntax_Syntax.bv_to_name t2 in
          FStarC_Syntax_Syntax.mk_GTotal uu___4 in
        FStarC_Syntax_Util.arrow uu___2 uu___3 in
      let f =
        FStarC_Syntax_Syntax.gen_bv "f" FStar_Pervasives_Native.None t_f in
      let a11 =
        let uu___2 =
          let uu___3 = FStarC_Syntax_Syntax.bv_to_name t1 in mk_gctx uu___3 in
        FStarC_Syntax_Syntax.gen_bv "a1" FStar_Pervasives_Native.None uu___2 in
      let ret =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name t2 in mk_gctx uu___4 in
      ...
      let uu___2 =
        ...
        let uu___4 =
          binders_of_list
            [(a1, true); (t1, true); (t2, true); (f, false); (a11, false)] in
        FStarC_List.op_At uu___3 uu___4 in
      let uu___3 =
        let uu___4 =
          let uu___5 =
            let uu___6 =
              let uu___7 =
                let uu___8 =
                  let uu___9 = FStarC_Syntax_Syntax.bv_to_name f in [uu___9] in
                FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___8 in
              FStarC_Syntax_Util.mk_app c_pure1 uu___7 in
            let uu___7 =
              let uu___8 = FStarC_Syntax_Syntax.bv_to_name a11 in [uu___8] in
            uu___6 :: uu___7 in
          FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___5 in
        FStarC_Syntax_Util.mk_app c_app1 uu___4 in
      FStarC_Syntax_Util.abs uu___2 uu___3 ret in
    let c_lift11 =
      let uu___2 = mk_lid "lift1" in register env1 uu___2 c_lift1 in
    let c_lift2 =
      let t1 =
        FStarC_Syntax_Syntax.gen_bv "t1" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let t2 =
        FStarC_Syntax_Syntax.gen_bv "t2" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let t3 =
        FStarC_Syntax_Syntax.gen_bv "t3" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let t_f =
        let uu___2 =
          let uu___3 =
          ...
          uu___3 :: uu___4 in
        let uu___3 =
          let uu___4 = FStarC_Syntax_Syntax.bv_to_name t3 in
          FStarC_Syntax_Syntax.mk_GTotal uu___4 in
        FStarC_Syntax_Util.arrow uu___2 uu___3 in
      let f =
        FStarC_Syntax_Syntax.gen_bv "f" FStar_Pervasives_Native.None t_f in
      ...
      let a2 =
        let uu___2 =
          let uu___3 = FStarC_Syntax_Syntax.bv_to_name t2 in mk_gctx uu___3 in
        FStarC_Syntax_Syntax.gen_bv "a2" FStar_Pervasives_Native.None uu___2 in
      let ret =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name t3 in mk_gctx uu___4 in
          FStarC_Syntax_Util.residual_tot uu___3 in
        FStar_Pervasives_Native.Some uu___2 in
      let uu___2 =
        let uu___3 = mk_all_implicit binders in
              ...
              let uu___7 =
                let uu___8 =
                  let uu___9 =
                    let uu___10 =
                      let uu___11 =
                        let uu___12 = FStarC_Syntax_Syntax.bv_to_name f in
                        [uu___12] in
                      FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___11 in
                    FStarC_Syntax_Util.mk_app c_pure1 uu___10 in
                  let uu___10 =
                    let uu___11 = FStarC_Syntax_Syntax.bv_to_name a11 in
                    [uu___11] in
                  uu___9 :: uu___10 in
                FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___8 in
              FStarC_Syntax_Util.mk_app c_app1 uu___7 in
            let uu___7 =
              let uu___8 = FStarC_Syntax_Syntax.bv_to_name a2 in [uu___8] in
            uu___6 :: uu___7 in
          FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___5 in
        FStarC_Syntax_Util.mk_app c_app1 uu___4 in
      FStarC_Syntax_Util.abs uu___2 uu___3 ret in
    let c_lift21 =
      let uu___2 = mk_lid "lift2" in register env1 uu___2 c_lift2 in
    let c_push =
      let t1 =
        FStarC_Syntax_Syntax.gen_bv "t1" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      ...
      let t_f =
        ...
        let uu___3 =
          let uu___4 =
            let uu___5 = FStarC_Syntax_Syntax.bv_to_name t2 in mk_gctx uu___5 in
      ...
      let ret =
        ...
        FStar_Pervasives_Native.Some uu___2 in
      let e1 =
        let uu___2 = FStarC_Syntax_Syntax.bv_to_name t1 in
        FStarC_Syntax_Syntax.gen_bv "e1" FStar_Pervasives_Native.None uu___2 in
      let body =
        let uu___2 =
          let uu___3 = FStarC_Syntax_Syntax.bv_to_name f in
          let uu___4 =
            let uu___5 =
              let uu___6 = FStarC_Syntax_Syntax.bv_to_name e1 in
              FStarC_Syntax_Syntax.as_arg uu___6 in
            let uu___6 = args_of_binders gamma in uu___5 :: uu___6 in
          FStarC_Syntax_Util.mk_app uu___3 uu___4 in
        FStarC_Syntax_Util.abs
          (FStarC_List.op_At gamma [FStarC_Syntax_Syntax.mk_binder e1])
          uu___2 ret in
      let uu___2 =
        let uu___3 = mk_all_implicit binders in
        let uu___4 =
          binders_of_list [(a1, true); (t1, true); (t2, true); (f, false)] in
        FStarC_List.op_At uu___3 uu___4 in
      FStarC_Syntax_Util.abs uu___2 body ret in
    let c_push1 = let uu___2 = mk_lid "push" in register env1 uu___2 c_push in
    let ret_tot_wp_a =
      FStar_Pervasives_Native.Some (FStarC_Syntax_Util.residual_tot wp_a1) in
    let mk_generic_app c =
      if (FStarC_List.length binders) > Prims.int_zero
      then
        let uu___2 =
          let uu___3 =
            let uu___4 = args_of_binders binders in
            { FStarC_Syntax_Syntax.hd = c; FStarC_Syntax_Syntax.args = uu___4
            } in
          FStarC_Syntax_Syntax.Tm_app uu___3 in
        mk uu___2
      else c in
    let wp_if_then_else =
      let result_comp =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.null_binder wp_a1 in
            let uu___5 =
              let uu___6 = FStarC_Syntax_Syntax.null_binder wp_a1 in [uu___6] in
            uu___4 :: uu___5 in
      ...
      let c =
        FStarC_Syntax_Syntax.gen_bv "c" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let uu___2 =
        let uu___3 = FStarC_Syntax_Syntax.binders_of_list [a1; c] in
        FStarC_List.op_At binders uu___3 in
      let uu___3 =
        let l_ite =
          FStarC_Syntax_Syntax.fvar_with_dd FStarC_Parser_Const.ite_lid
            FStar_Pervasives_Native.None in
        let uu___4 =
          let uu___5 =
            let uu___6 =
              let uu___7 =
                let uu___8 =
                  let uu___9 =
                    let uu___10 = FStarC_Syntax_Syntax.bv_to_name c in
                    FStarC_Syntax_Syntax.as_arg uu___10 in
                  [uu___9] in
                FStarC_Syntax_Util.mk_app l_ite uu___8 in
              [uu___7] in
            FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___6 in
          FStarC_Syntax_Util.mk_app c_lift21 uu___5 in
        FStarC_Syntax_Util.ascribe uu___4
          ((FStar_Pervasives.Inr result_comp), FStar_Pervasives_Native.None,
            false) in
      let uu___4 =
        let uu___5 = FStarC_Syntax_Util.residual_comp_of_comp result_comp in
        FStar_Pervasives_Native.Some uu___5 in
      FStarC_Syntax_Util.abs uu___2 uu___3 uu___4 in
    let wp_if_then_else1 =
      let uu___2 = mk_lid "wp_if_then_else" in
      register env1 uu___2 wp_if_then_else in
    let wp_if_then_else2 = mk_generic_app wp_if_then_else1 in
    let wp_close =
      let b =
        FStarC_Syntax_Syntax.gen_bv "b" FStar_Pervasives_Native.None
          FStarC_Syntax_Util.ktype in
      let t_f =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name b in
            FStarC_Syntax_Syntax.null_binder uu___4 in
          [uu___3] in
        let uu___3 = FStarC_Syntax_Syntax.mk_Total wp_a1 in
        FStarC_Syntax_Util.arrow uu___2 uu___3 in
      let f =
        FStarC_Syntax_Syntax.gen_bv "f" FStar_Pervasives_Native.None t_f in
      let body =
        let uu___2 =
          let uu___3 =
            let uu___4 =
              ...
              FStarC_Syntax_Util.mk_app c_pure1 uu___5 in
            let uu___5 =
              let uu___6 =
                let uu___7 =
                  let uu___8 =
                    let uu___9 = FStarC_Syntax_Syntax.bv_to_name f in
                    [uu___9] in
                  FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___8 in
                FStarC_Syntax_Util.mk_app c_push1 uu___7 in
              [uu___6] in
            uu___4 :: uu___5 in
          FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___3 in
        FStarC_Syntax_Util.mk_app c_app1 uu___2 in
      let uu___2 =
        let uu___3 = FStarC_Syntax_Syntax.binders_of_list [a1; b; f] in
        FStarC_List.op_At binders uu___3 in
      FStarC_Syntax_Util.abs uu___2 body ret_tot_wp_a in
    let wp_close1 =
      let uu___2 = mk_lid "wp_close" in register env1 uu___2 wp_close in
    let wp_close2 = mk_generic_app wp_close1 in
    let ret_tot_type =
      FStar_Pervasives_Native.Some
        (FStarC_Syntax_Util.residual_tot FStarC_Syntax_Util.ktype) in
    let ret_gtot_type =
      let uu___2 =
        let uu___3 =
          let uu___4 =
            FStarC_Syntax_Syntax.mk_GTotal FStarC_Syntax_Util.ktype in
          FStarC_TypeChecker_Common.lcomp_of_comp uu___4 in
        FStarC_TypeChecker_Common.residual_comp_of_lcomp uu___3 in
      FStar_Pervasives_Native.Some uu___2 in
            ...
            let uu___5 =
              let uu___6 =
                FStarC_Syntax_Util.abs [FStarC_Syntax_Syntax.mk_binder x]
                  body ret_tot_type in
              FStarC_Syntax_Syntax.as_arg uu___6 in
            [uu___5] in
          {
            FStarC_Syntax_Syntax.hd = FStarC_Syntax_Util.tforall;
            FStarC_Syntax_Syntax.args = uu___4
          } in
        FStarC_Syntax_Syntax.Tm_app uu___3 in
      FStarC_Syntax_Syntax.mk uu___2 FStarC_Range_Type.dummyRange in
            ...
            FStarC_List.for_all
              (fun uu___4 ->
                 ...
                 | { FStarC_Syntax_Syntax.binder_bv = b;
                     ...
                     is_discrete b.FStarC_Syntax_Syntax.sort) bs in
          if uu___3
          then is_discrete (FStarC_Syntax_Util.comp_result c)
          else false
      | uu___3 -> true in
            ...
            FStarC_List.for_all
              (fun uu___4 ->
                 ...
                 | { FStarC_Syntax_Syntax.binder_bv = b;
                     ...
                     is_discrete b.FStarC_Syntax_Syntax.sort) bs in
          if uu___3
          then is_monotonic (FStarC_Syntax_Util.comp_result c)
          else false
      | uu___3 -> is_discrete t in
    let rec mk_rel rel t x y =
      ...
      let t1 =
        FStarC_TypeChecker_Normalize.normalize
          ...
          FStarC_TypeChecker_Env.UnfoldUntil
            FStarC_Syntax_Syntax.delta_constant] env1 t in
      let uu___2 =
        let uu___3 = FStarC_Syntax_Subst.compress t1 in
        uu___3.FStarC_Syntax_Syntax.n in
      match uu___2 with
      | FStarC_Syntax_Syntax.Tm_type uu___3 -> rel x y
      | FStarC_Syntax_Syntax.Tm_arrow
          { FStarC_Syntax_Syntax.bs1 = binder::[];
            FStarC_Syntax_Syntax.comp =
              { FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.GTotal b;
          ...
          let uu___6 =
            let uu___7 = is_monotonic a2 in
            if uu___7 then true else is_monotonic b in
          if uu___6
          then
            let a11 =
              FStarC_Syntax_Syntax.gen_bv "a1" FStar_Pervasives_Native.None
                a2 in
            let body =
              let uu___7 =
                let uu___8 =
                  let uu___9 =
                    let uu___10 = FStarC_Syntax_Syntax.bv_to_name a11 in
                    FStarC_Syntax_Syntax.as_arg uu___10 in
                  [uu___9] in
                FStarC_Syntax_Util.mk_app x uu___8 in
              let uu___8 =
                let uu___9 =
                  let uu___10 =
                    let uu___11 = FStarC_Syntax_Syntax.bv_to_name a11 in
                  ...
                  [uu___10] in
                FStarC_Syntax_Util.mk_app y uu___9 in
            ...
            (let a11 =
               FStarC_Syntax_Syntax.gen_bv "a1" FStar_Pervasives_Native.None
                 a2 in
             let a21 =
               FStarC_Syntax_Syntax.gen_bv "a2" FStar_Pervasives_Native.None
                 a2 in
             let body =
               let uu___8 =
                 let uu___9 = FStarC_Syntax_Syntax.bv_to_name a11 in
                 let uu___10 = FStarC_Syntax_Syntax.bv_to_name a21 in
                 mk_rel1 a2 uu___9 uu___10 in
               let uu___9 =
                 let uu___10 =
                   let uu___11 =
                     let uu___12 =
                       let uu___13 = FStarC_Syntax_Syntax.bv_to_name a11 in
                       FStarC_Syntax_Syntax.as_arg uu___13 in
                     [uu___12] in
                   FStarC_Syntax_Util.mk_app x uu___11 in
                 let uu___11 =
                   let uu___12 =
                     let uu___13 =
                       let uu___14 = FStarC_Syntax_Syntax.bv_to_name a21 in
                       FStarC_Syntax_Syntax.as_arg uu___14 in
                     [uu___13] in
                   FStarC_Syntax_Util.mk_app y uu___12 in
                 mk_rel1 b uu___10 uu___11 in
               FStarC_Syntax_Util.mk_imp uu___8 uu___9 in
             let uu___8 = mk_forall a21 body in mk_forall a11 uu___8)
      | FStarC_Syntax_Syntax.Tm_arrow
          { FStarC_Syntax_Syntax.bs1 = binder::[];
            FStarC_Syntax_Syntax.comp =
              { FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Total b;
                FStarC_Syntax_Syntax.pos = uu___3;
                FStarC_Syntax_Syntax.vars = uu___4;
                FStarC_Syntax_Syntax.hash_code = uu___5;_};_}
          ->
      ...
      | FStarC_Syntax_Syntax.Tm_arrow
          ...
          let t2 =
            let uu___3 =
              let uu___4 =
                let uu___5 =
                  let uu___6 = FStarC_Syntax_Util.arrow binders1 comp in
      ...
      | FStarC_Syntax_Syntax.Tm_arrow
          ...
          -> FStarC_Effect.failwith "impossible: arrow with empty binders"
      | uu___3 -> FStarC_Syntax_Util.mk_untyped_eq2 x y in
    let stronger =
      let wp1 =
        FStarC_Syntax_Syntax.gen_bv "wp1" FStar_Pervasives_Native.None wp_a1 in
      let wp2 =
        FStarC_Syntax_Syntax.gen_bv "wp2" FStar_Pervasives_Native.None wp_a1 in
      let rec mk_stronger t x y =
        ...
        | FStarC_Syntax_Syntax.Tm_app
            ...
            FStarC_Syntax_Util.is_tuple_constructor uu___3 ->
            let project i tuple =
              let projector =
                let uu___3 =
                  let uu___4 =
                    FStarC_Parser_Const_Tuples.mk_tuple_data_lid
                      (FStarC_List.length args) FStarC_Range_Type.dummyRange in
                  FStarC_TypeChecker_Env.lookup_projector env1 uu___4 i in
              ...
              let uu___4 =
                FStarC_List.mapi
                  (fun i uu___5 ->
                     match uu___5 with
                     | (t2, q) ->
                         let uu___6 = project i x in
                         let uu___7 = project i y in
                         mk_stronger t2 uu___6 uu___7) args in
              match uu___4 with
              | [] ->
            ...
            (match uu___3 with
             | (rel0, rels) ->
                 FStarC_List.fold_left FStarC_Syntax_Util.mk_conj rel0 rels)
        | FStarC_Syntax_Syntax.Tm_arrow
                ...
                (fun i uu___6 ->
                   ...
                   | { FStarC_Syntax_Syntax.binder_bv = bv;
                       ...
                       let uu___9 =
                         ...
                         Prims.strcat "a" uu___10 in
                       FStarC_Syntax_Syntax.gen_bv uu___9
                         FStar_Pervasives_Native.None
                         bv.FStarC_Syntax_Syntax.sort) binders1 in
            let args =
              FStarC_List.map
                (fun ai ->
                   let uu___6 = FStarC_Syntax_Syntax.bv_to_name ai in
                   FStarC_Syntax_Syntax.as_arg uu___6) bvs in
            let body =
              let uu___6 = FStarC_Syntax_Util.mk_app x args in
              let uu___7 = FStarC_Syntax_Util.mk_app y args in
              mk_stronger b uu___6 uu___7 in
            FStarC_List.fold_right (fun bv body1 -> mk_forall bv body1) bvs
              body
        | FStarC_Syntax_Syntax.Tm_arrow
            { FStarC_Syntax_Syntax.bs1 = binders1;
              FStarC_Syntax_Syntax.comp =
            ...
            FStarC_List.fold_right (fun bv body1 -> mk_forall bv body1) bvs
              body
        | uu___3 -> FStarC_Effect.failwith "Not a DM elaborated type" in
      let body =
        let uu___2 = FStarC_Syntax_Util.unascribe wp_a1 in
        let uu___3 = FStarC_Syntax_Syntax.bv_to_name wp1 in
        let uu___4 = FStarC_Syntax_Syntax.bv_to_name wp2 in
        mk_stronger uu___2 uu___3 uu___4 in
      let uu___2 =
        let uu___3 =
          binders_of_list [(a1, false); (wp1, false); (wp2, false)] in
        FStarC_List.op_At binders uu___3 in
      FStarC_Syntax_Util.abs uu___2 body ret_tot_type in
    let stronger1 =
      let uu___2 = mk_lid "stronger" in register env1 uu___2 stronger in
    let stronger2 = mk_generic_app stronger1 in
    let ite_wp =
      let wp =
        FStarC_Syntax_Syntax.gen_bv "wp" FStar_Pervasives_Native.None wp_a1 in
      let uu___2 = FStarC_Util.prefix gamma in
      match uu___2 with
      | (wp_args, post) ->
          let k =
            FStarC_Syntax_Syntax.gen_bv "k" FStar_Pervasives_Native.None
          ...
          let equiv =
            ...
            let eq =
              ...
              mk_rel FStarC_Syntax_Util.mk_iff k.FStarC_Syntax_Syntax.sort
                k_tm uu___3 in
            let uu___3 = FStarC_Syntax_Formula.destruct_typ_as_formula eq in
            match uu___3 with
            | FStar_Pervasives_Native.Some (FStarC_Syntax_Formula.QAll
                (binders1, [], body)) ->
                let k_app =
                  let uu___4 = args_of_binders binders1 in
                  FStarC_Syntax_Util.mk_app k_tm uu___4 in
                let guard_free =
                  FStarC_Syntax_Syntax.fv_to_tm
                    (FStarC_Syntax_Syntax.lid_and_dd_as_fv
                       ...
                       FStar_Pervasives_Native.None) in
                let pat =
                  FStarC_Syntax_Util.mk_app guard_free
                ...
                let pattern_guarded_body =
                  let uu___4 =
                    let uu___5 =
                      let uu___6 =
                        let uu___7 =
                          let uu___8 =
                            FStarC_Syntax_Syntax.binders_to_names binders1 in
                          (uu___8, [[FStarC_Syntax_Syntax.as_arg pat]]) in
                        FStarC_Syntax_Syntax.Meta_pattern uu___7 in
                      {
                  ...
                  mk uu___4 in
                FStarC_Syntax_Util.close_forall_no_univs binders1
          ...
          let body =
            let uu___3 =
              let uu___4 =
                let uu___5 =
                  let uu___6 = FStarC_Syntax_Syntax.bv_to_name wp in
                  let uu___7 =
                    let uu___8 = args_of_binders wp_args in
                    let uu___9 =
                      let uu___10 =
                        let uu___11 = FStarC_Syntax_Syntax.bv_to_name k in
                        FStarC_Syntax_Syntax.as_arg uu___11 in
                      [uu___10] in
                    FStarC_List.op_At uu___8 uu___9 in
                  FStarC_Syntax_Util.mk_app uu___6 uu___7 in
                FStarC_Syntax_Util.mk_imp equiv uu___5 in
              FStarC_Syntax_Util.mk_forall_no_univ k uu___4 in
            FStarC_Syntax_Util.abs gamma uu___3 ret_gtot_type in
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.binders_of_list [a1; wp] in
            FStarC_List.op_At binders uu___4 in
          FStarC_Syntax_Util.abs uu___3 body ret_gtot_type in
    let ite_wp1 = let uu___2 = mk_lid "ite_wp" in register env1 uu___2 ite_wp in
    ...
    let null_wp =
      ...
      | (wp_args, post) ->
          ...
          let body =
            ...
            FStarC_Syntax_Util.mk_forall_no_univ x uu___3 in
          let uu___3 =
            let uu___4 =
              let uu___5 = FStarC_Syntax_Syntax.binders_of_list [a1] in
              FStarC_List.op_At uu___5 gamma in
            FStarC_List.op_At binders uu___4 in
          FStarC_Syntax_Util.abs uu___3 body ret_gtot_type in
    let null_wp1 =
      let uu___2 = mk_lid "null_wp" in register env1 uu___2 null_wp in
    ...
    let wp_trivial =
      ...
      let body =
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Syntax_Syntax.bv_to_name a1 in
            let uu___5 =
              let uu___6 =
                let uu___7 =
                  let uu___8 =
                    let uu___9 = FStarC_Syntax_Syntax.bv_to_name a1 in
                    FStarC_Syntax_Syntax.as_arg uu___9 in
                  [uu___8] in
                FStarC_Syntax_Util.mk_app null_wp2 uu___7 in
              let uu___7 =
                let uu___8 = FStarC_Syntax_Syntax.bv_to_name wp in [uu___8] in
              uu___6 :: uu___7 in
            uu___4 :: uu___5 in
          FStarC_List.map FStarC_Syntax_Syntax.as_arg uu___3 in
        FStarC_Syntax_Util.mk_app stronger2 uu___2 in
      let uu___2 =
        let uu___3 = FStarC_Syntax_Syntax.binders_of_list [a1; wp] in
        FStarC_List.op_At binders uu___3 in
      FStarC_Syntax_Util.abs uu___2 body ret_tot_type in
    let wp_trivial1 =
      let uu___2 = mk_lid "wp_trivial" in register env1 uu___2 wp_trivial in
    let wp_trivial2 = mk_generic_app wp_trivial1 in
    (let uu___3 = FStarC_Effect.op_Bang dbg in
     if uu___3 then d1 "End Dijkstra monads for free" else ());
    (let c = FStarC_Syntax_Subst.close binders in
     let ed_combs =
       ...
       | FStarC_Syntax_Syntax.DM4F_eff combs ->
           let uu___3 =
             let uu___4 = let uu___5 = c stronger2 in ([], uu___5) in
             let uu___5 = let uu___6 = c wp_if_then_else2 in ([], uu___6) in
             let uu___6 = let uu___7 = c ite_wp2 in ([], uu___7) in
             let uu___7 = let uu___8 = c wp_close2 in ([], uu___8) in
             let uu___8 = let uu___9 = c wp_trivial2 in ([], uu___9) in
             {
               ...
               FStarC_Syntax_Syntax.stronger = uu___4;
               FStarC_Syntax_Syntax.if_then_else = uu___5;
               FStarC_Syntax_Syntax.ite_wp = uu___6;
               FStarC_Syntax_Syntax.close_wp = uu___7;
               FStarC_Syntax_Syntax.trivial = uu___8;
               FStarC_Syntax_Syntax.repr = (combs.FStarC_Syntax_Syntax.repr);
           ...
           FStarC_Syntax_Syntax.DM4F_eff uu___3
       | uu___3 ->
           FStarC_Effect.failwith
             "Impossible! For a DM4F effect combinators must be in DM4f_eff" in
     let uu___3 =
       let uu___4 = FStarC_Effect.op_Bang sigelts in FStarC_List.rev uu___4 in
     (uu___3,
       {
         ...
         FStarC_Syntax_Syntax.signature = (ed.FStarC_Syntax_Syntax.signature);
...
type env_ = env
let get_env (env1 : env) : FStarC_TypeChecker_Env.env= env1.tcenv
let set_env (dmff_env : env) (env' : FStarC_TypeChecker_Env.env) : env=
  { tcenv = env'; subst = (dmff_env.subst); tc_const = (dmff_env.tc_const) }
type nm =
  | N of FStarC_Syntax_Syntax.typ 
  | M of FStarC_Syntax_Syntax.typ 
let uu___is_N (projectee : nm) : Prims.bool=
  match projectee with | N _0 -> true | uu___ -> false
let __proj__N__item___0 (projectee : nm) : FStarC_Syntax_Syntax.typ=
  match projectee with | N _0 -> _0
let uu___is_M (projectee : nm) : Prims.bool=
  match projectee with | M _0 -> true | uu___ -> false
let __proj__M__item___0 (projectee : nm) : FStarC_Syntax_Syntax.typ=
  match projectee with | M _0 -> _0
type nm_ = nm
let nm_of_comp (c : FStarC_Syntax_Syntax.comp' FStarC_Syntax_Syntax.syntax) :
  nm=
  ...
  | uu___ ->
      let uu___1 =
        ...
        FStarC_Format.fmt1 "[nm_of_comp]: unexpected computation type %s"
          uu___2 in
      FStarC_Errors.raise_error (FStarC_Syntax_Syntax.has_range_syntax ()) c
        FStarC_Errors_Codes.Error_UnexpectedDM4FType ()
        (Obj.magic FStarC_Errors_Msg.is_error_message_string)
        (Obj.magic uu___1)
let string_of_nm (uu___ : nm) : Prims.string=
  match uu___ with
  | N t ->
      let uu___1 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t in
      FStarC_Format.fmt1 "N[%s]" uu___1
  | M t ->
      let uu___1 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t in
      FStarC_Format.fmt1 "M[%s]" uu___1
let is_monadic_arrow (n : FStarC_Syntax_Syntax.term') : nm=
  match n with
  | FStarC_Syntax_Syntax.Tm_arrow
      { FStarC_Syntax_Syntax.bs1 = uu___; FStarC_Syntax_Syntax.comp = c;_} ->
      nm_of_comp c
  | uu___ -> FStarC_Effect.failwith "unexpected_argument: [is_monadic_arrow]"
let is_monadic_comp
  (c : FStarC_Syntax_Syntax.comp' FStarC_Syntax_Syntax.syntax) : Prims.bool=
  let uu___ = nm_of_comp c in
  match uu___ with | M uu___1 -> true | N uu___1 -> false
exception Not_found 
let uu___is_Not_found (projectee : Prims.exn) : Prims.bool=
  match projectee with | Not_found -> true | uu___ -> false
let double_star (typ : FStarC_Syntax_Syntax.typ) : FStarC_Syntax_Syntax.typ=
  let star_once typ1 =
    let uu___ =
      let uu___1 =
        let uu___2 =
...
let rec mk_star_to_type
  (mk : FStarC_Syntax_Syntax.term' -> FStarC_Syntax_Syntax.term) (env1 : env)
  (a : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax) :
    ...
    let uu___1 =
      let uu___2 =
        let uu___3 =
          ...
          FStarC_Syntax_Syntax.mk_binder_with_attrs uu___4
            (FStarC_Syntax_Syntax.as_bqual_implicit false)
            FStar_Pervasives_Native.None [] in
        [uu___3] in
      let uu___3 = FStarC_Syntax_Syntax.mk_Total FStarC_Syntax_Util.ktype0 in
      { FStarC_Syntax_Syntax.bs1 = uu___2; FStarC_Syntax_Syntax.comp = uu___3
      } in
    FStarC_Syntax_Syntax.Tm_arrow uu___1 in
  mk uu___
and star_type' (env1 : env)
                    ...
                    let uu___5 =
                      let uu___6 =
                        let uu___7 =
                          ...
                          FStarC_Syntax_Syntax.mk_binder_with_attrs uu___8
                            (FStarC_Syntax_Syntax.as_bqual_implicit false)
                            FStar_Pervasives_Native.None [] in
                        [uu___7] in
  ...
  | FStarC_Syntax_Syntax.Tm_app
      ...
      let debug t2 s =
        let uu___ =
          ...
          FStarC_Format.fmt2 "Dependency found in term %s : %s" uu___1 uu___2 in
        FStarC_Errors.log_issue (FStarC_Syntax_Syntax.has_range_syntax ()) t2
      ...
      let rec is_non_dependent_arrow ty n =
                 ...
                 (fun uu___3 ->
                    ...
                    | () ->
                        let non_dependent_or_raise s ty1 =
                          ...
                          if uu___4
                          then
                            (debug ty1 sinter; FStarC_Effect.raise Not_found)
                          else () in
                        let uu___4 = FStarC_Syntax_Subst.open_comp binders c in
                        (match uu___4 with
                         | (binders1, c1) ->
                             let s =
                               let uu___5 =
                                            ...
                                            Obj.magic
                                              (FStarC_Class_Setlike.add ()
                                                 (Obj.magic
                                                    (FStarC_FlatSet.setlike_flat_set
                                                       FStarC_Syntax_Syntax.ord_bv))
                                                 bv (Obj.magic s1)))) uu___7
                                      uu___6) uu___5 binders1 in
                             let ct = FStarC_Syntax_Util.comp_result c1 in
                             (non_dependent_or_raise s ct;
                              (let k = n - (FStarC_List.length binders1) in
                               if k > Prims.int_zero
        ...
        | uu___1 ->
            ((let uu___3 =
                ...
                FStarC_Format.fmt1 "Not a dependent arrow : %s" uu___4 in
                   ...
                   let res =
                     FStarC_TypeChecker_Normalize.normalize
                       ...
                       FStarC_TypeChecker_Env.DontUnfoldAttr
                         [FStarC_Parser_Const.tac_opaque_attr];
                       FStarC_TypeChecker_Env.UnfoldUntil
                         FStarC_Syntax_Syntax.delta_constant] env1.tcenv t1 in
                   let uu___5 =
                    ...
                    | uu___6 ->
                        ((let uu___8 =
                            ...
                            FStarC_Format.fmt1
                              ...
                              uu___9 in
                          FStarC_Errors.log_issue
                            (FStarC_Syntax_Syntax.has_range_syntax ()) head1
  ...
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.bs = binders; FStarC_Syntax_Syntax.body = repr;
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      ...
      let uu___ =
        let uu___1 =
          ...
          let uu___3 =
            let uu___4 =
              let uu___5 = star_type' env1 t2 in FStar_Pervasives.Inl uu___5 in
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      ...
      let uu___ =
        let uu___1 =
          ...
          let uu___3 =
            let uu___4 =
              let uu___5 = star_type' env1 (FStarC_Syntax_Util.comp_result c) in
  ...
  | FStarC_Syntax_Syntax.Tm_refine uu___ ->
      let uu___1 =
        ...
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t1 in
        FStarC_Format.fmt2 "%s is outside of the definition language: %s"
          uu___2 uu___3 in
      FStarC_Errors.raise_error0
        FStarC_Errors_Codes.Fatal_TermOutsideOfDefLanguage ()
        (Obj.magic FStarC_Errors_Msg.is_error_message_string)
  ...
  | FStarC_Syntax_Syntax.Tm_lazy i ->
      let uu___ = FStarC_Syntax_Util.unfold_lazy i in star_type' env1 uu___
  | FStarC_Syntax_Syntax.Tm_delayed uu___ ->
      FStarC_Effect.failwith "impossible"
let is_monadic
  (uu___ : FStarC_Syntax_Syntax.residual_comp FStar_Pervasives_Native.option)
  : Prims.bool=
  match uu___ with
  | FStar_Pervasives_Native.None ->
      FStarC_Effect.failwith "un-annotated lambda?!"
...
let rec is_C (t : FStarC_Syntax_Syntax.typ) : Prims.bool=
  let uu___ =
  ...
  | FStarC_Syntax_Syntax.Tm_app
      ...
      let r = is_C (FStar_Pervasives_Native.fst (FStarC_List.hd args)) in
      if r
      then
        ((let uu___2 =
            let uu___3 =
          ...
          then
            let uu___3 =
              ...
              FStarC_Format.fmt1 "Not a C-type (A * C): %s" uu___4 in
            FStarC_Errors.raise_error
        ...
        ((let uu___3 =
            let uu___4 =
              FStarC_List.for_all
                (fun uu___5 ->
                   match uu___5 with
                   | (h, uu___6) ->
                       let uu___7 = is_C h in Prims.op_Negation uu___7) args in
          ...
          then
            let uu___4 =
              let uu___5 =
                FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t in
              FStarC_Format.fmt1 "Not a C-type (C * A): %s" uu___5 in
            FStarC_Errors.raise_error
              (FStarC_Syntax_Syntax.has_range_syntax ()) t
              FStarC_Errors_Codes.Error_UnexpectedDM4FType ()
              (Obj.magic FStarC_Errors_Msg.is_error_message_string)
               ...
               let uu___4 =
                 let uu___5 =
                   FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                     t1 in
                 FStarC_Format.fmt1 "Not a C-type (C -> C): %s" uu___5 in
               FStarC_Errors.raise_error
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      ...
      -> is_C t1
  | uu___1 -> false
let mk_return (env1 : env) (t : FStarC_Syntax_Syntax.typ)
  ...
  let p =
    FStarC_Syntax_Syntax.gen_bv "p'" FStar_Pervasives_Native.None p_type in
  let body =
    let uu___ =
      let uu___1 =
        let uu___2 = FStarC_Syntax_Syntax.bv_to_name p in
        {
          ...
          FStarC_Syntax_Syntax.args =
            [(e, (FStarC_Syntax_Syntax.as_aqual_implicit false))]
        } in
      FStarC_Syntax_Syntax.Tm_app uu___1 in
    mk uu___ in
  FStarC_Syntax_Util.abs [FStarC_Syntax_Syntax.mk_binder p] body
    (FStar_Pervasives_Native.Some
       (FStarC_Syntax_Util.residual_tot FStarC_Syntax_Util.ktype0))
let is_unknown (uu___ : FStarC_Syntax_Syntax.term') : Prims.bool=
  match uu___ with
  | FStarC_Syntax_Syntax.Tm_unknown -> true
  | uu___1 -> false
let rec check (env1 : env) (e : FStarC_Syntax_Syntax.term) (context_nm : nm)
  ...
  let return_if uu___ =
    ...
    | (rec_nm, s_e, u_e) ->
        ...
        (match (rec_nm, context_nm) with
         ...
         | (N t1, M t2) ->
             (check1 t1 t2;
              (let uu___2 = mk_return env1 t1 s_e in ((M t1), uu___2, u_e)))
         | (M t1, N t2) ->
             let uu___1 =
               let uu___2 =
                 FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
    ...
    | N t ->
        let uu___ =
          ...
          Prims.strcat
            ...
            uu___1 in
        FStarC_Errors.raise_error (FStarC_Syntax_Syntax.has_range_syntax ())
          e2 FStarC_Errors_Codes.Fatal_LetBoundMonadicMismatch ()
          (Obj.magic FStarC_Errors_Msg.is_error_message_string)
          (Obj.magic uu___)
    | M uu___ -> let uu___1 = check env2 e2 context_nm in strip_m uu___1 in
  let uu___ =
  ...
  | FStarC_Syntax_Syntax.Tm_let
      ...
      ->
      mk_let env1 binding e2 (fun env2 e21 -> check env2 e21 context_nm)
        ensure_m
  | FStarC_Syntax_Syntax.Tm_match
      ...
      ->
      mk_match env1 e0 branches (fun env2 body -> check env2 body context_nm)
  | FStarC_Syntax_Syntax.Tm_meta
      { FStarC_Syntax_Syntax.tm2 = e1; FStarC_Syntax_Syntax.meta = uu___1;_}
      -> check env1 e1 context_nm
  | FStarC_Syntax_Syntax.Tm_uinst (e1, uu___1) -> check env1 e1 context_nm
  | FStarC_Syntax_Syntax.Tm_ascribed
  ...
  | FStarC_Syntax_Syntax.Tm_let uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[check]: Tm_let %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_type uu___1 ->
      FStarC_Effect.failwith "impossible (DM stratification)"
  ...
  | FStarC_Syntax_Syntax.Tm_refine uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[check]: Tm_refine %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_uvar uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[check]: Tm_uvar %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_delayed uu___1 ->
      FStarC_Effect.failwith "impossible (compressed)"
  | FStarC_Syntax_Syntax.Tm_unknown ->
      let uu___1 =
        let uu___2 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[check]: Tm_unknown %s" uu___2 in
      FStarC_Effect.failwith uu___1
and infer (env1 : env) (e : FStarC_Syntax_Syntax.term) :
  (nm * FStarC_Syntax_Syntax.term * FStarC_Syntax_Syntax.term)=
  let mk x = FStarC_Syntax_Syntax.mk x e.FStarC_Syntax_Syntax.pos in
  let normalize =
    FStarC_TypeChecker_Normalize.normalize
  ...
  | FStarC_Syntax_Syntax.Tm_bvar bv ->
      FStarC_Effect.failwith "I failed to open a binder... boo"
  ...
  | FStarC_Syntax_Syntax.Tm_abs
      { FStarC_Syntax_Syntax.bs = binders; FStarC_Syntax_Syntax.body = body;
      ...
      let env2 =
        let uu___1 = FStarC_TypeChecker_Env.push_binders env1.tcenv binders1 in
        { tcenv = uu___1; subst = (env1.subst); tc_const = (env1.tc_const) } in
      let s_binders =
        FStarC_List.map
          (fun b ->
             let sort =
                 ...
                 then
                     ...
                     FStarC_Syntax_Syntax.gen_bv
                       (Prims.strcat
                          (FStarC_Ident.string_of_id
                             bv.FStarC_Syntax_Syntax.ppname) "__w")
                   ...
                   let env4 =
                     let uu___8 =
                       let uu___9 =
                         let uu___10 =
                           let uu___11 = FStarC_Syntax_Syntax.bv_to_name xw in
                           (bv, uu___11) in
                   ...
                   (env4, ((FStarC_Syntax_Syntax.mk_binder x) ::
                     (FStarC_Syntax_Syntax.mk_binder xw) :: acc))
                 else
                   (let x =
                      let uu___9 =
                    ...
                    (env3, ((FStarC_Syntax_Syntax.mk_binder x) :: acc))))
          (env2, []) binders1 in
      (match uu___1 with
       | (env3, u_binders) ->
           ...
           let _csbu =
             let _r =
               let uu___2 = is_monadic rc_opt1 in
               if uu___2 then check_m env3 body1 else check_n env3 body1 in
             let t =
               let uu___2 = _r in
               match uu___2 with | (x, uu___3, uu___4) -> x in
             let s_body =
               let uu___2 = _r in
               match uu___2 with | (uu___3, x, uu___4) -> x in
             let u_body =
               let uu___2 = _r in
               match uu___2 with | (uu___3, uu___4, x) -> x in
             let uu___2 =
               let uu___3 =
                 let uu___4 = is_monadic rc_opt1 in
                 if uu___4 then M t else N t in
               comp_of_nm uu___3 in
             (uu___2, s_body, u_body) in
           let comp =
             let uu___2 = _csbu in
             match uu___2 with | (c, uu___3, uu___4) -> c in
           let s_body =
             let uu___2 = _csbu in
             match uu___2 with | (uu___3, s, uu___4) -> s in
           let u_body =
             let uu___2 = _csbu in
             match uu___2 with | (uu___3, uu___4, u) -> u in
                  ...
                  | FStar_Pervasives_Native.Some rt ->
                      let rt1 =
                        ...
                        FStarC_TypeChecker_Normalize.normalize
                          ...
                          FStarC_TypeChecker_Env.EraseUniverses] uu___2 rt in
                      let uu___2 =
                        FStarC_Util.for_some
                          (fun uu___3 ->
                             match uu___3 with
                      ...
                      then
                        ...
                        let uu___3 =
                          let uu___4 =
                            let uu___5 = double_star rt1 in
                            FStar_Pervasives_Native.Some uu___5 in
                          FStarC_Syntax_Util.mk_residual_comp
                            FStarC_Parser_Const.effect_Tot_lid uu___4 flags in
                        FStar_Pervasives_Native.Some uu___3
                      else
                        (let uu___4 =
                           let uu___5 =
                             let uu___6 = star_type' env3 rt1 in
           ...
           let uu___2 =
             let comp1 =
               ...
               let uu___4 = FStarC_Syntax_Subst.subst env3.subst s_body in
               trans_G env3 (FStarC_Syntax_Util.comp_result comp) uu___3
                 uu___4 in
             let uu___3 =
               FStarC_Syntax_Util.ascribe u_body
                 ((FStar_Pervasives.Inr comp1), FStar_Pervasives_Native.None,
                   false) in
             let uu___4 =
               let uu___5 = FStarC_Syntax_Util.residual_comp_of_comp comp1 in
               FStar_Pervasives_Native.Some uu___5 in
             (uu___3, uu___4) in
           (match uu___2 with
            | (u_body1, u_rc_opt) ->
                let s_body1 = FStarC_Syntax_Subst.close s_binders s_body in
                let s_binders1 = FStarC_Syntax_Subst.close_binders s_binders in
                let s_term =
                  let uu___3 =
                    let uu___4 =
                      let uu___5 =
                        let uu___6 =
                          FStarC_Syntax_Subst.closing_of_binders s_binders1 in
                        subst_rc_opt uu___6 s_rc_opt in
                      {
                  ...
                  mk uu___3 in
                let u_body2 = FStarC_Syntax_Subst.close u_binders1 u_body1 in
                let u_binders2 = FStarC_Syntax_Subst.close_binders u_binders1 in
                let u_term =
                  let uu___3 =
                    let uu___4 =
                      let uu___5 =
                        let uu___6 =
                          FStarC_Syntax_Subst.closing_of_binders u_binders2 in
                        subst_rc_opt uu___6 u_rc_opt in
                      {
                    ...
                    (FStarC_Syntax_Syntax.Tm_app
                       {
                         FStarC_Syntax_Syntax.hd = head;
                         FStarC_Syntax_Syntax.args =
                           [FStarC_Syntax_Syntax.as_arg s; a2]
                       }) in
                let uu___9 =
                  mk
                    (FStarC_Syntax_Syntax.Tm_app
                       {
                         FStarC_Syntax_Syntax.hd = head;
                         FStarC_Syntax_Syntax.args =
                           [FStarC_Syntax_Syntax.as_arg u; a2]
                       }) in
                (t, uu___8, uu___9)))
  | FStarC_Syntax_Syntax.Tm_app
      {
      ...
      let uu___5 =
        let uu___6 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "DMFF: Ill-applied constant %s" uu___6 in
       ...
       | (t_head, s_head, u_head) ->
           ...
           let rec flatten t =
             ...
             | FStarC_Syntax_Syntax.Tm_arrow
                 { FStarC_Syntax_Syntax.bs1 = binders;
                   FStarC_Syntax_Syntax.comp =
                     {
                       FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Total t1;
                       FStarC_Syntax_Syntax.pos = uu___3;
                       FStarC_Syntax_Syntax.vars = uu___4;
                       FStarC_Syntax_Syntax.hash_code = uu___5;_};_}
                 when is_arrow t1 ->
             ...
             | uu___3 ->
                 let uu___4 =
                   let uu___5 =
                     FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                       t_head in
                   FStarC_Format.fmt1 "%s: not a function type" uu___5 in
                 FStarC_Errors.raise_error0
           ...
           (match uu___2 with
            | (binders, comp) ->
                ...
                (if (FStarC_List.length binders) < (FStarC_List.length args)
                 then
                   (let uu___4 =
                      let uu___5 =
                 ...
                 (let uu___4 = FStarC_Syntax_Subst.open_comp binders comp in
                  match uu___4 with
                  | (binders1, comp1) ->
                      let rec final_type subst binders_comp args1 =
                        let binders2 =
                          FStar_Pervasives_Native.fst binders_comp in
                        let comp2 = FStar_Pervasives_Native.snd binders_comp in
                        match (binders2, args1) with
                        | ([], []) ->
                            let uu___5 =
                              FStarC_Syntax_Subst.subst_comp subst comp2 in
                            nm_of_comp uu___5
                        | (binders3, []) ->
                            let uu___5 =
                              let uu___6 =
                                      ...
                                      (FStarC_Syntax_Syntax.Tm_arrow
                                         {
                                           FStarC_Syntax_Syntax.bs1 =
                                             binders3;
                                           FStarC_Syntax_Syntax.comp = comp2
                                         }) in
                                  FStarC_Syntax_Subst.subst subst uu___8 in
                                FStarC_Syntax_Subst.compress uu___7 in
                              uu___6.FStarC_Syntax_Syntax.n in
                            (match uu___5 with
                             | FStarC_Syntax_Syntax.Tm_arrow
                                 { FStarC_Syntax_Syntax.bs1 = binders4;
                                 ...
                                 let uu___6 =
                                   ...
                                   mk uu___7 in
                                 N uu___6
                             | uu___6 -> FStarC_Effect.failwith "wat?")
                        | ([], uu___5::uu___6) ->
                            FStarC_Effect.failwith "just checked that?!"
                        | ({ FStarC_Syntax_Syntax.binder_bv = bv;
                             FStarC_Syntax_Syntax.binder_qual = uu___5;
                             FStarC_Syntax_Syntax.binder_positivity = uu___6;
                             FStarC_Syntax_Syntax.binder_attrs = uu___7;_}::binders3,
                           (arg, uu___8)::args2) ->
                            final_type ((FStarC_Syntax_Syntax.NT (bv, arg))
                              :: subst) (binders3, comp2) args2 in
                      let final_type1 = final_type [] (binders1, comp1) args in
                                       ...
                                       (arg, q)) ->
                                        ...
                                        (match uu___14 with
                                         | FStarC_Syntax_Syntax.Tm_type
                                             ...
                                             (uu___16, [(arg, q)])
                                         | uu___15 ->
                                             let uu___16 = check_n env1 arg in
                                             (match uu___16 with
                                              | (uu___17, s_arg, u_arg) ->
                                                  let uu___18 =
                                                    let uu___19 =
                                    ...
                                    (FStarC_Syntax_Syntax.Tm_app
                                       {
                                         ...
                                         FStarC_Syntax_Syntax.args = s_args
                                       }) in
                                let uu___9 =
                                  mk
                                    (FStarC_Syntax_Syntax.Tm_app
                                       {
                                         ...
                                         FStarC_Syntax_Syntax.args = u_args1
                                       }) in
  ...
  | FStarC_Syntax_Syntax.Tm_constant c ->
      let uu___1 = let uu___2 = env1.tc_const c in N uu___2 in (uu___1, e, e)
  ...
  | FStarC_Syntax_Syntax.Tm_let uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[infer]: Tm_let %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_type uu___1 ->
      FStarC_Effect.failwith "impossible (DM stratification)"
  | FStarC_Syntax_Syntax.Tm_arrow uu___1 ->
  ...
  | FStarC_Syntax_Syntax.Tm_refine uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[infer]: Tm_refine %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_uvar uu___1 ->
      let uu___2 =
        let uu___3 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[infer]: Tm_uvar %s" uu___3 in
      FStarC_Effect.failwith uu___2
  | FStarC_Syntax_Syntax.Tm_delayed uu___1 ->
      FStarC_Effect.failwith "impossible (compressed)"
  | FStarC_Syntax_Syntax.Tm_unknown ->
      let uu___1 =
        let uu___2 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
        FStarC_Format.fmt1 "[infer]: Tm_unknown %s" uu___2 in
      FStarC_Effect.failwith uu___1
and mk_match (env1 : env)
  (e0 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax)
  (branches :
            ...
            (fun b ->
               ...
               | (pat, FStar_Pervasives_Native.None, body) ->
                   let env2 =
                     let uu___5 =
                       ...
                       FStarC_List.fold_left FStarC_TypeChecker_Env.push_bv
                         env1.tcenv uu___6 in
                     {
                       tcenv = uu___5;
                       subst = (env1.subst);
                       tc_const = (env1.tc_const)
               ...
               | uu___5 ->
                   FStarC_Errors.raise_error0
                     ...
                     (Obj.magic FStarC_Errors_Msg.is_error_message_string)
                     (Obj.magic "No when clauses in the definition language"))
            branches in
        FStarC_List.split uu___3 in
      (match uu___2 with
       | (nms, branches1) ->
           let t1 =
             match FStarC_List.hd nms with | M t11 -> t11 | N t11 -> t11 in
           let has_m =
             FStarC_List.existsb
               (fun uu___3 ->
                  match uu___3 with | M uu___4 -> true | uu___4 -> false) nms in
           let uu___3 =
             let uu___4 =
               FStarC_List.map2
                 (fun nm1 uu___5 ->
                        ...
                        (match (nm1, has_m) with
                         ...
                         | (M t2, true) ->
                             (nm1, (pat, guard, s_body),
                               (pat, guard, u_body))
                         | (N t2, true) ->
                             let uu___6 = check env1 original_body (M t2) in
                             (match uu___6 with
                              | (uu___7, s_body1, u_body1) ->
                                  ((M t2), (pat, guard, s_body1),
                                    (pat, guard, u_body1)))
                         | (M uu___6, false) ->
                             FStarC_Effect.failwith "impossible")) nms
                 branches1 in
            ...
            | (nms1, s_branches, u_branches) ->
                ...
                then
                  ...
                  let s_e =
                    ...
                    FStarC_Syntax_Util.abs [FStarC_Syntax_Syntax.mk_binder p]
                      uu___4
                      (FStar_Pervasives_Native.Some
                  ...
                  (let s_branches1 =
                             ...
                             (FStarC_Syntax_Syntax.Tm_match
                                {
                                  ...
                                  FStarC_Syntax_Syntax.ret_opt =
                                    FStar_Pervasives_Native.None;
                                  FStarC_Syntax_Syntax.brs = s_branches1;
                                  FStarC_Syntax_Syntax.rc_opt1 =
                                    FStar_Pervasives_Native.None
                                }) in
                         {
                   ...
                   ((N t1), uu___5, uu___6))))
and mk_let (env1 : env_) (binding : FStarC_Syntax_Syntax.letbinding)
  (e2 : FStarC_Syntax_Syntax.term)
  (proceed :
    env_ ->
      FStarC_Syntax_Syntax.term ->
        (nm * FStarC_Syntax_Syntax.term * FStarC_Syntax_Syntax.term))
  ...
  let x =
    FStar_Pervasives.__proj__Inl__item__v binding.FStarC_Syntax_Syntax.lbname in
  let x_binders = [FStarC_Syntax_Syntax.mk_binder x] in
      ...
      (match uu___1 with
       | (N t1, s_e1, u_e1) ->
           let u_binding =
             ...
             then
               let uu___3 =
                 let uu___4 = FStarC_Syntax_Subst.subst env1.subst s_e1 in
           ...
           (match uu___2 with
            | (nm_rec, s_e2, u_e2) ->
                let s_binding =
                  let uu___3 =
                    star_type' env2 binding.FStarC_Syntax_Syntax.lbtyp in
                  {
                    FStarC_Syntax_Syntax.lbname =
                      (binding.FStarC_Syntax_Syntax.lbname);
                    FStarC_Syntax_Syntax.lbunivs =
                ...
                let uu___3 =
                  let uu___4 =
                    let uu___5 =
                      let uu___6 = FStarC_Syntax_Subst.close x_binders1 s_e2 in
                      {
                ...
                let uu___4 =
                  let uu___5 =
                    let uu___6 =
                      let uu___7 = FStarC_Syntax_Subst.close x_binders1 u_e2 in
                      {
                        FStarC_Syntax_Syntax.lbs =
                          (false,
                            [{
           ...
           (match uu___2 with
            | (t2, s_e2, u_e2) ->
                ...
                let s_e21 =
                  let uu___3 =
                    let uu___4 =
                      let uu___5 =
                        let uu___6 =
                          let uu___7 = FStarC_Syntax_Syntax.bv_to_name p in
                          (uu___7,
                ...
                let uu___4 =
                  ...
                  mk uu___5 in
                ((M t2), uu___3, uu___4)))
and check_n (env1 : env_) (e : FStarC_Syntax_Syntax.term) :
  ...
  | uu___1 -> FStarC_Effect.failwith "[check_n]: impossible"
and check_m (env1 : env_) (e : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.typ * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.term)=
  let mn =
    let uu___ =
  ...
  match uu___ with
  | (M t, s_e, u_e) -> (t, s_e, u_e)
  | uu___1 -> FStarC_Effect.failwith "[check_m]: impossible"
and comp_of_nm (nm1 : nm_) : FStarC_Syntax_Syntax.comp=
  match nm1 with | N t -> FStarC_Syntax_Syntax.mk_Total t | M t -> mk_M t
and mk_M (t : FStarC_Syntax_Syntax.typ) : FStarC_Syntax_Syntax.comp=
...
and type_of_comp (t : FStarC_Syntax_Syntax.comp) : FStarC_Syntax_Syntax.typ=
  FStarC_Syntax_Util.comp_result t
and trans_F_ (env1 : env_) (c : FStarC_Syntax_Syntax.typ)
  (wp : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  (let uu___1 = let uu___2 = is_C c in Prims.op_Negation uu___2 in
   ...
   then
     let uu___2 =
       ...
       FStarC_Format.fmt1 "Not a DM4F C-type: %s" uu___3 in
     FStarC_Errors.raise_error (FStarC_Syntax_Syntax.has_range_syntax ()) c
       FStarC_Errors_Codes.Error_UnexpectedDM4FType ()
       (Obj.magic FStarC_Errors_Msg.is_error_message_string)
       (Obj.magic uu___2)
  ...
  (let mk x = FStarC_Syntax_Syntax.mk x c.FStarC_Syntax_Syntax.pos in
                      ...
                      (fun uu___7 uu___8 ->
                         ...
                         | ((arg, q), (wp_arg, q')) ->
                             let print_implicit q1 =
                               if FStarC_Syntax_Syntax.is_aqual_implicit q1
                               then "implicit"
                               else "explicit" in
                             ((let uu___10 =
                                 let uu___11 =
        ...
        | (binders_orig, comp1) ->
                ...
                FStarC_List.map
                  (fun b ->
                     ...
                     | (bv, q) ->
                         ...
                         then
                           let w' =
                             let uu___7 = star_type' env1 h in
                             FStarC_Syntax_Syntax.gen_bv
                               (Prims.strcat
                                  (FStarC_Ident.string_of_id
                                     bv.FStarC_Syntax_Syntax.ppname) "__w'")
                             ...
                             let uu___8 =
                               let uu___9 =
                                 let uu___10 =
                                   let uu___11 =
                                     let uu___12 =
                                       FStarC_Syntax_Syntax.bv_to_name w' in
                                     trans_F_ env1 h uu___12 in
                                   FStarC_Syntax_Syntax.null_bv uu___11 in
                                 {
                                   FStarC_Syntax_Syntax.binder_bv = uu___10;
                                   FStarC_Syntax_Syntax.binder_qual =
                                     (b.FStarC_Syntax_Syntax.binder_qual);
                                   FStarC_Syntax_Syntax.binder_positivity =
                                     (b.FStarC_Syntax_Syntax.binder_positivity);
                           ...
                           (let x =
                              ...
                              FStarC_Syntax_Syntax.gen_bv
                                (Prims.strcat
                                   (FStarC_Ident.string_of_id
                                      bv.FStarC_Syntax_Syntax.ppname) "__x")
                                FStar_Pervasives_Native.None uu___8 in
                            (x,
                              [{
            ...
            (match uu___3 with
             | (bvs, binders2) ->
                 ...
                 let comp2 =
                   let uu___4 =
                     let uu___5 = FStarC_Syntax_Syntax.binders_of_list bvs in
                     FStarC_Syntax_Util.rename_binders binders_orig uu___5 in
   ...
   | FStarC_Syntax_Syntax.Tm_ascribed
       { FStarC_Syntax_Syntax.tm = e; FStarC_Syntax_Syntax.asc = uu___2;
         FStarC_Syntax_Syntax.eff_opt = uu___3;_}
       -> trans_F_ env1 e wp
   | uu___2 -> FStarC_Effect.failwith "impossible trans_F_")
and trans_G (env1 : env_) (h : FStarC_Syntax_Syntax.typ)
  (is_monadic1 : Prims.bool) (wp : FStarC_Syntax_Syntax.typ) :
  FStarC_Syntax_Syntax.comp=
  if is_monadic1
  then
    let uu___ =
      let uu___1 = star_type' env1 h in
      {
        ...
        FStarC_Syntax_Syntax.effect_args =
          [(wp, (FStarC_Syntax_Syntax.as_aqual_implicit false))];
        FStarC_Syntax_Syntax.flags = []
      } in
    FStarC_Syntax_Syntax.mk_Comp uu___
  else
    (let uu___1 = trans_F_ env1 h wp in FStarC_Syntax_Syntax.mk_Total uu___1)
let n :
  FStarC_TypeChecker_Env.env ->
    FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term=
  FStarC_TypeChecker_Normalize.normalize
    ...
    FStarC_TypeChecker_Env.Eager_unfolding;
    FStarC_TypeChecker_Env.EraseUniverses]
let star_type (env1 : env) (t : FStarC_Syntax_Syntax.typ) :
  FStarC_Syntax_Syntax.typ=
  let uu___ = n env1.tcenv t in star_type' env1 uu___
let star_expr (env1 : env) (t : FStarC_Syntax_Syntax.term) :
  (FStarC_Syntax_Syntax.typ * FStarC_Syntax_Syntax.term *
    FStarC_Syntax_Syntax.term)=
  let uu___ = n env1.tcenv t in check_n env1 uu___
let trans_F (env1 : env_) (c : FStarC_Syntax_Syntax.typ)
  (wp : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let uu___ = n env1.tcenv c in
  let uu___1 = n env1.tcenv wp in trans_F_ env1 uu___ uu___1
let recheck_debug (s : Prims.string) (env1 : FStarC_TypeChecker_Env.env)
   ...
   | (t', uu___2, uu___3) ->
       ((let uu___5 = FStarC_Effect.op_Bang dbg in
         ...
         then
           let uu___6 =
             FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t' in
           FStarC_Format.print1 "Re-checked; got:\n%s\n----------\n" uu___6
         else ());
        t'))
let cps_and_elaborate (env1 : FStarC_TypeChecker_Env.env)
       ...
       | (effect_binders, env2, uu___2) ->
           ...
           (match uu___3 with
            | (signature, uu___4) ->
                ...
                let uu___5 =
                  let uu___6 =
                    let uu___7 = FStarC_Syntax_Subst.compress signature_un in
                  ...
                  | FStarC_Syntax_Syntax.Tm_arrow
                      ...
                      -> (a, effect_marker)
                  | uu___7 ->
                      raise_error FStarC_Errors_Codes.Fatal_BadSignatureShape
                        "bad shape for effect-for-free signature" in
                (match uu___5 with
                 | (a, effect_marker) ->
                     let a1 =
                       if FStarC_Syntax_Syntax.is_null_bv a
                       then
                         FStarC_Syntax_Syntax.gen_bv "a"
                           (FStar_Pervasives_Native.Some
                              (FStarC_Syntax_Syntax.range_of_bv a))
                           a.FStarC_Syntax_Syntax.sort
                       else a in
                     let open_and_check env3 other_binders t =
                       let subst =
                         FStarC_Syntax_Subst.opening_of_binders
                           (FStarC_List.op_At effect_binders1 other_binders) in
                       let t1 = FStarC_Syntax_Subst.subst subst t in
                       let uu___6 = FStarC_TypeChecker_TcTerm.tc_term env3 t1 in
                       match uu___6 with | (t2, comp, uu___7) -> (t2, comp) in
                     let mk x =
                      ...
                      | (repr, _comp) ->
                          ((let uu___8 = FStarC_Effect.op_Bang dbg in
                            ...
                            then
                              ...
                              FStarC_Format.print1 "Representation is: %s\n"
                                uu___9
                            else ());
                           (let ed_range =
                            ...
                            let dmff_env =
                              empty env2
                                (FStarC_TypeChecker_TcTerm.tc_constant env2
                                   FStarC_Range_Type.dummyRange) in
                            ...
                            let effect_signature =
                              let binders =
                                let uu___9 =
                                  ...
                                  [uu___10] in
                                (FStarC_Syntax_Syntax.mk_binder_with_attrs a1
                            ...
                            let elaborate_and_star dmff_env1 other_binders
                              ...
                              | (u_item, item1) ->
                                  let uu___11 =
                                    open_and_check env3 other_binders item1 in
                                  (match uu___11 with
                                   | (item2, item_comp) ->
                                       ((let uu___13 =
                                           let uu___14 =
                                    ...
                                    return_elab) ->
                                     ...
                                     let return_wp1 =
                                       ...
                                       | FStarC_Syntax_Syntax.Tm_abs
                                           ...
                                           ->
                                           let uu___15 =
                                             FStarC_Syntax_Util.abs bs body
                                               what in
                                           FStarC_Syntax_Util.abs [b1; b2]
                                             uu___15
                                             (FStar_Pervasives_Native.Some
                                                rc_gtot)
                                       | uu___15 ->
                                     ...
                                     let apply_close t =
                                       if
                                         (FStarC_List.length effect_binders1)
                                           = Prims.int_zero
                                       then t
                                       else
                                         (let uu___15 =
                                     ...
                                     let rec apply_last f l =
                                       ...
                                       | a2::[] -> [f a2]
                                       | x::xs ->
                                           let uu___14 = apply_last f xs in x
                                             :: uu___14 in
                                     let register maybe_admit name item =
                                       ...
                                       let p' =
                                         apply_last
                                           ...
                                           p in
                                       let l' =
                                         FStarC_Ident.lid_of_path p' ed_range in
                                       let uu___14 =
                                     ...
                                     let mk_sigelt se =
                                       let uu___14 =
                                         FStarC_Syntax_Syntax.mk_sigelt se in
                                       {
                                         FStarC_Syntax_Syntax.sigel =
                                     ...
                                     let bind_elab1 =
                                       register_admit "bind_elab" bind_elab in
                                     let uu___14 =
                                       FStarC_List.fold_left
                                         (fun uu___15 action ->
                                            match uu___15 with
                                     ...
                                     (match uu___14 with
                                      | (dmff_env3, actions) ->
                                          ...
                                          let repr2 = register1 "repr" repr1 in
                                          let uu___16 =
                                            let uu___17 =
                                              let uu___18 =
                                                let uu___19 =
