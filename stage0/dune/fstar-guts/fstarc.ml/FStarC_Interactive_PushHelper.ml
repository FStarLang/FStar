       ...
       (FStarC_Interactive_Ide_Types.PushFragment (frag, push_kind, i), st))::rest
        ...
        let pf =
          FStarC_Interactive_Ide_Types.PushFragment
            (frag, push_kind, (FStarC_List.op_At issues i)) in
...
let (query_of_ids :
  ...
  = fun ids -> FStarC_List.map FStarC_Ident.string_of_id ids
let (query_of_lid :
      ...
      fun table ->
        let capitalize str =
          ...
          then str
            ...
            (let first =
               FStarC_String.substring str Prims.int_zero Prims.int_one in
             let uu___1 =
