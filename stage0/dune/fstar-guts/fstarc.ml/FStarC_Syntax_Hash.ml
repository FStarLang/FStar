...
let (mix_list :
  ...
  fun l -> FStarC_List.fold_right mix l nil_hc
let (mix_list_lit :
...
let hash_list :
  ...
  = fun h -> fun ts -> let uu___ = FStarC_List.map h ts in mix_list uu___
let hash_option :
  'a .
    ('a -> FStarC_Hash.hash_code mm) ->
...
let (hash_doc : FStar_Pprint.document -> FStarC_Hash.hash_code mm) =
  fun d ->
    let uu___ =
    ...
    of_string uu___
let (hash_doc_list :
  FStar_Pprint.document Prims.list -> FStarC_Hash.hash_code mm) =
...
and (hash_universe_uvar :
  (FStarC_Syntax_Syntax.universe FStar_Pervasives_Native.option
    ...
    FStarC_Range_Type.range) -> FStarC_Hash.hash_code mm)
  =
