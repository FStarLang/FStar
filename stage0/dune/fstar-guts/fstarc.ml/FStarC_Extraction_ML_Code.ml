...
let (reduce : doc Prims.list -> doc) =
  fun docs -> FStarC_List.fold_left cat empty docs
...
let (is_prims_ns :
  ...
  fun ns -> (ns = ["Prims"]) || (ns = ["Fstarcompiler.Prims"])
let (as_bin_op :
  FStarC_Extraction_ML_Syntax.mlpath ->
...
let (escape_char_hex : FStarC_BaseTypes.char -> Prims.string) =
  fun x -> escape_byte_hex (FStarC_Util.byte_of_char x)
let (escape_or :
...
let (string_of_mlconstant :
  ...
  fun sctt ->
    ...
    | FStarC_Extraction_ML_Syntax.MLC_String chars ->
        let uu___ =
          let uu___1 =
            FStarC_String.collect (escape_or FStarC_Util.string_of_char)
              chars in
          Prims.strcat uu___1 "\"" in
        Prims.strcat "\"" uu___
...
and (doc_of_sig :
  ...
  fun currentModule ->
    fun s ->
      ...
      let docs1 =
        FStarC_List.map (fun x -> reduce [x; hardline; hardline]) docs in
      reduce docs1
let (doc_of_mod1 :
  FStarC_Extraction_ML_Syntax.mlsymbol ->
