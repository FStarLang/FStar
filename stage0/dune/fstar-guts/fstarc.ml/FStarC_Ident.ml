...
let (__proj__Mkident__item__idRange : ident -> FStarC_Range_Type.range) =
...
let (mk_ident : (Prims.string * FStarC_Range_Type.range) -> ident) =
  fun uu___ ->
    match uu___ with | (text, range) -> { idText = text; idRange = range }
let (set_id_range : FStarC_Range_Type.range -> ident -> ident) =
  fun r -> fun i -> { idText = (i.idText); idRange = r }
let (reserved_prefix : Prims.string) = "uu___"
let (gen' : Prims.string -> FStarC_Range_Type.range -> ident) =
  fun s ->
    fun r ->
...
let (range_of_id : ident -> FStarC_Range_Type.range) = fun id -> id.idRange
let (id_of_text : Prims.string -> ident) =
  fun str -> mk_ident (str, FStarC_Range_Type.dummyRange)
let (string_of_id : ident -> Prims.string) = fun id -> id.idText
let (text_of_path : path -> Prims.string) =
  fun path1 -> FStarC_Util.concat_l "." path1
let (path_of_text : Prims.string -> path) =
  fun text -> FStarC_String.split [46] text
let (path_of_ns : ipath -> path) = fun ns -> FStarC_List.map string_of_id ns
...
let (ids_of_lid : lident -> ipath) =
  fun lid -> FStarC_List.op_At lid.ns [lid.ident]
...
let (lid_of_str : Prims.string -> lident) =
  fun str ->
    ...
    lid_of_ids uu___
let (lid_of_path : path -> FStarC_Range_Type.range -> lident) =
  fun path1 ->
    fun pos ->
...
let (range_of_lid : lident -> FStarC_Range_Type.range) =
  fun lid1 -> range_of_id lid1.ident
let (set_lid_range : lident -> FStarC_Range_Type.range -> lident) =
  fun l ->
    fun r ->
      {
        ns = (l.ns);
