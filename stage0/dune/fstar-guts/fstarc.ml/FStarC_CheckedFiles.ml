  ...
  FStarC_Stats.record "hash_dependences"
    (fun uu___ ->
       ...
       let interface_source_file_name =
         ...
         if uu___1
         then FStarC_Parser_Dep.interface_of deps module_name
       ...
       let maybe_add_iface_hash out =
         ...
         | FStar_Pervasives_Native.Some iface ->
             let iface1 =
               let uu___1 = FStarC_Find.find_file iface in
               match uu___1 with
               | FStar_Pervasives_Native.Some f -> f
               | FStar_Pervasives_Native.None -> iface in
             let uu___1 =
               let uu___2 =
                 let uu___3 =
                   let uu___4 = FStarC_Util.digest_of_file iface1 in
                   ("interface", uu___4) in
                 uu___3 :: out in
               ("source", source_hash) :: uu___2 in
             FStar_Pervasives.Inr uu___1 in
       let rec hash_deps out l =
         match l with
         | [] -> maybe_add_iface_hash out
         | fn2::deps1 ->
             let cache_fn = FStarC_Parser_Dep.cache_file_name fn2 in
             let digest =
               let uu___1 = try_find_in_cache cache_fn in
               match uu___1 with
               | FStar_Pervasives_Native.None ->
                   let msg =
                     FStarC_Format.fmt2
