...
let uu___is_Mk_map (f : 'k cmp) (projectee : ('k, 'v, Obj.t) ordmap) :
  Prims.bool= true
let __proj__Mk_map__item__d (f : 'k cmp) (projectee : ('k, 'v, Obj.t) ordmap)
  : ('k, Obj.t) FStar_OrdSet.ordset=
  match projectee with | Mk_map (d, m) -> d
let __proj__Mk_map__item__m (f : 'k cmp) (projectee : ('k, 'v, Obj.t) ordmap)
  : ('k, 'v, Obj.t, Obj.t) map_t= match projectee with | Mk_map (d, m) -> m
let empty (f : 'k cmp) : ('k, 'v, Obj.t) ordmap=
  let d = FStar_OrdSet.empty f in
  let g x = FStar_Pervasives_Native.None in Mk_map (d, g)
let const_on (f : 'k cmp) (d : ('k, Obj.t) FStar_OrdSet.ordset) (x : 'v) :
  ('k, 'v, Obj.t) ordmap=
  let g x1 =
