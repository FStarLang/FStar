open Prims
let get_env (uu___ : unit) : FStarC_TypeChecker_Env.env=
  let uu___1 =
    FStarC_Effect.op_Bang FStarC_TypeChecker_Normalize.reflection_env_hook in
  match uu___1 with
  | FStar_Pervasives_Native.None ->
      FStarC_Effect.failwith "impossible: env_hook unset in reflection"
  | FStar_Pervasives_Native.Some e -> e
let inspect_bqual (bq : FStarC_Syntax_Syntax.bqual) :
...
let inspect_aqual (aq : FStarC_Syntax_Syntax.aqual) :
  FStarC_Reflection_V1_Data.aqualv=
  match aq with
...
let pack_bqual (aqv : FStarC_Reflection_V1_Data.aqualv) :
  ...
  | FStarC_Reflection_V1_Data.Q_Meta t ->
      FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Meta t)
let pack_aqual (aqv : FStarC_Reflection_V1_Data.aqualv) :
  FStarC_Syntax_Syntax.aqual=
  match aqv with
  | FStarC_Reflection_V1_Data.Q_Implicit ->
      FStarC_Syntax_Syntax.as_aqual_implicit true
  | uu___ -> FStar_Pervasives_Native.None
let inspect_fv (fv : FStarC_Syntax_Syntax.fv) : Prims.string Prims.list=
  FStarC_Ident.path_of_lid (FStarC_Syntax_Syntax.lid_of_fv fv)
let pack_fv (ns : Prims.string Prims.list) : FStarC_Syntax_Syntax.fv=
  let lid = FStarC_Parser_Const.p2l ns in
  let fallback uu___ =
    let quals =
      if FStarC_Ident.lid_equals lid FStarC_Parser_Const.cons_lid
      then FStar_Pervasives_Native.Some FStarC_Syntax_Syntax.Data_ctor
      else
        if FStarC_Ident.lid_equals lid FStarC_Parser_Const.nil_lid
        then FStar_Pervasives_Native.Some FStarC_Syntax_Syntax.Data_ctor
        else
          if FStarC_Ident.lid_equals lid FStarC_Parser_Const.some_lid
          then FStar_Pervasives_Native.Some FStarC_Syntax_Syntax.Data_ctor
          else
            if FStarC_Ident.lid_equals lid FStarC_Parser_Const.none_lid
            then FStar_Pervasives_Native.Some FStarC_Syntax_Syntax.Data_ctor
  ...
  | FStar_Pervasives_Native.Some env ->
      ...
      (match qninfo with
       | FStar_Pervasives_Native.Some (FStar_Pervasives.Inr (se, _us), _rng)
           ->
...
let inspect_universe (u : FStarC_Syntax_Syntax.universe) :
  ...
  | FStarC_Syntax_Syntax.U_bvar n -> FStarC_Reflection_V1_Data.Uv_BVar n
  | FStarC_Syntax_Syntax.U_name i ->
      FStarC_Reflection_V1_Data.Uv_Name
...
let pack_universe (uv : FStarC_Reflection_V1_Data.universe_view) :
  ...
  | FStarC_Reflection_V1_Data.Uv_BVar n -> FStarC_Syntax_Syntax.U_bvar n
  | FStarC_Reflection_V1_Data.Uv_Name i ->
      FStarC_Syntax_Syntax.U_name (FStarC_Ident.mk_ident i)
...
let rec inspect_ln (t : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_constant c ->
      let uu___ = inspect_const c in FStarC_Reflection_V1_Data.Tv_Const uu___
  ...
  | FStarC_Syntax_Syntax.Tm_let
      ...
      then FStarC_Reflection_V1_Data.Tv_Unsupp
        ...
        (match lb.FStarC_Syntax_Syntax.lbname with
         | FStar_Pervasives.Inr uu___1 -> FStarC_Reflection_V1_Data.Tv_Unsupp
  ...
  | uu___ ->
      ((let uu___2 =
        ...
        FStarC_Errors.log_issue (FStarC_Syntax_Syntax.has_range_syntax ()) t1
          FStarC_Errors_Codes.Warning_CantInspect ()
          (Obj.magic FStarC_Errors_Msg.is_error_message_string)
          (Obj.magic uu___2));
       FStarC_Reflection_V1_Data.Tv_Unsupp)
let inspect_comp (c : FStarC_Syntax_Syntax.comp) :
    ...
    | FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.DECREASES
        ...
        ((let uu___3 =
          ...
          FStarC_Errors.log_issue (FStarC_Syntax_Syntax.has_range_syntax ())
            c FStarC_Errors_Codes.Warning_CantInspect ()
            (Obj.magic FStarC_Errors_Msg.is_error_message_string)
            (Obj.magic uu___3));
         [])
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      ...
      then
        ...
        (let inspect_arg uu___1 =
         ...
         FStarC_Reflection_V1_Data.C_Eff uu___1)
let pack_comp (cv : FStarC_Reflection_V1_Data.comp_view) :
  ...
  | FStarC_Reflection_V1_Data.C_GTotal t -> FStarC_Syntax_Syntax.mk_GTotal t
  | FStarC_Reflection_V1_Data.C_Lemma (pre, post, pats) ->
      let ct =
        {
  ...
  | FStarC_Reflection_V1_Data.C_Eff (us, ef, res, args, decrs) ->
      ...
      FStarC_Syntax_Syntax.mk_Comp ct
let pack_const (c : FStarC_Reflection_V1_Data.vconst) :
...
let pack_ln (tv : FStarC_Reflection_V1_Data.term_view) :
  FStarC_Syntax_Syntax.term=
  match tv with
  | FStarC_Reflection_V1_Data.Tv_Var bv -> FStarC_Syntax_Syntax.bv_to_name bv
  | FStarC_Reflection_V1_Data.Tv_BVar bv -> FStarC_Syntax_Syntax.bv_to_tm bv
  | FStarC_Reflection_V1_Data.Tv_FVar fv -> FStarC_Syntax_Syntax.fv_to_tm fv
  | FStarC_Reflection_V1_Data.Tv_UInst (fv, us) ->
  ...
  | FStarC_Reflection_V1_Data.Tv_Const c ->
      let uu___ =
        let uu___1 = pack_const c in FStarC_Syntax_Syntax.Tm_constant uu___1 in
  ...
  | FStarC_Reflection_V1_Data.Tv_Unsupp ->
      (FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_CantInspect ()
       ...
       FStarC_Syntax_Syntax.mk FStarC_Syntax_Syntax.Tm_unknown
         FStarC_Range_Type.dummyRange)
let compare_bv (x : FStarC_Syntax_Syntax.bv) (y : FStarC_Syntax_Syntax.bv) :
  FStarC_Order.order=
...
let lookup_attr (attr : FStarC_Syntax_Syntax.term)
  (env : FStarC_TypeChecker_Env.env) : FStarC_Syntax_Syntax.fv Prims.list=
  let uu___ =
  ...
  | FStarC_Syntax_Syntax.Tm_fvar fv ->
      ...
      FStarC_List.concatMap
        ...
        ses
  | uu___1 -> []
let all_defs_in_env (env : FStarC_TypeChecker_Env.env) :
  ...
  FStarC_List.map
    ...
    uu___
let defs_in_module (env : FStarC_TypeChecker_Env.env)
  (modul : FStarC_Reflection_V1_Data.name) :
  FStarC_Syntax_Syntax.fv Prims.list=
  let uu___ = FStarC_TypeChecker_Env.lidents env in
  FStarC_List.concatMap
    (fun l ->
       let ns =
         FStarC_List.map FStarC_Ident.string_of_id
           (FStarC_List.init (FStarC_Ident.ids_of_lid l)) in
       if ns = modul
       then [FStarC_Syntax_Syntax.lid_as_fv l FStar_Pervasives_Native.None]
       else []) uu___
let lookup_typ (env : FStarC_TypeChecker_Env.env)
...
let inspect_ident (i : FStarC_Ident.ident) : FStarC_Reflection_V1_Data.ident=
  FStarC_Reflection_V2_Builtins.inspect_ident i
let pack_ident (i : FStarC_Reflection_V1_Data.ident) : FStarC_Ident.ident=
  FStarC_Reflection_V2_Builtins.pack_ident i
let rd_to_syntax_qual (uu___ : FStarC_Reflection_V1_Data.qualifier) :
  ...
  | FStarC_Reflection_V1_Data.Irreducible -> FStarC_Syntax_Syntax.Irreducible
  ...
  | FStarC_Reflection_V1_Data.TotalEffect -> FStarC_Syntax_Syntax.TotalEffect
  ...
  | FStarC_Reflection_V1_Data.Projector (l, i) ->
      let uu___1 =
        let uu___2 = FStarC_Ident.lid_of_path l FStarC_Range_Type.dummyRange in
        (uu___2, (pack_ident i)) in
      FStarC_Syntax_Syntax.Projector uu___1
  | FStarC_Reflection_V1_Data.RecordType (l1, l2) ->
      let uu___1 =
        let uu___2 = FStarC_List.map pack_ident l1 in
        let uu___3 = FStarC_List.map pack_ident l2 in (uu___2, uu___3) in
...
let syntax_to_rd_qual (uu___ : FStarC_Syntax_Syntax.qualifier) :
  FStarC_Reflection_V1_Data.qualifier=
  match uu___ with
  ...
  | FStarC_Syntax_Syntax.Visible_default ->
      FStarC_Reflection_V1_Data.Visible_default
  | FStarC_Syntax_Syntax.Irreducible -> FStarC_Reflection_V1_Data.Irreducible
  | FStarC_Syntax_Syntax.Inline_for_extraction ->
      FStarC_Reflection_V1_Data.Inline_for_extraction
  ...
  | FStarC_Syntax_Syntax.TotalEffect -> FStarC_Reflection_V1_Data.TotalEffect
  ...
  | FStarC_Syntax_Syntax.Projector (l, i) ->
      let uu___1 =
        ...
        (uu___2, (inspect_ident i)) in
      FStarC_Reflection_V1_Data.Projector uu___1
  | FStarC_Syntax_Syntax.RecordType (l1, l2) ->
      let uu___1 =
        let uu___2 = FStarC_List.map inspect_ident l1 in
        let uu___3 = FStarC_List.map inspect_ident l2 in (uu___2, uu___3) in
...
let set_sigelt_quals (quals : FStarC_Reflection_V1_Data.qualifier Prims.list)
  (se : FStarC_Syntax_Syntax.sigelt) : FStarC_Syntax_Syntax.sigelt=
...
let inspect_sigelt (se : FStarC_Syntax_Syntax.sigelt) :
        ...
        | { FStarC_Syntax_Syntax.lbname = nm;
            ...
            (match uu___2 with
             | (s, us1) ->
                 ...
                 FStarC_Syntax_Util.mk_letbinding nm us1 typ1 eff def1 attrs
                   pos) in
      let uu___1 =
        let uu___2 = FStarC_List.map inspect_letbinding lbs in (r, uu___2) in
       ...
       | (s, us1) ->
           ...
           (match uu___4 with
            | (param_bs2, ty2) ->
                       ...
                       | (param_ctor_bs, c) ->
                           (if (FStarC_List.length param_ctor_bs) <> nparam
                            ...
                            (let cty2 = FStarC_Syntax_Util.comp_result c in
                             ...
                             let cty4 = FStarC_Syntax_Util.remove_inacc cty3 in
                ...
                let uu___5 =
                  let uu___6 = FStarC_List.map inspect_ident us1 in
                  let uu___7 = FStarC_List.map inspect_ctor c_lids in
  ...
  | FStarC_Syntax_Syntax.Sig_declare_typ
      ...
      (match uu___ with
       | (us1, ty1) ->
           let uu___1 =
             let uu___2 = FStarC_List.map inspect_ident us1 in
             (nm, uu___2, ty1) in
           FStarC_Reflection_V1_Data.Sg_Val uu___1)
  | uu___ -> FStarC_Reflection_V1_Data.Unk
let pack_sigelt (sv : FStarC_Reflection_V1_Data.sigelt_view) :
  ...
  let check_lid lid =
    ...
    then
      FStarC_Effect.failwith
        (Prims.strcat "pack_sigelt: invalid long identifier \""
  ...
  | FStarC_Reflection_V1_Data.Sg_Inductive
      ...
      let us_names1 = FStarC_List.map pack_ident us_names in
      let ind_lid = FStarC_Ident.lid_of_path nm FStarC_Range_Type.dummyRange in
      (check_lid ind_lid;
  ...
  | FStarC_Reflection_V1_Data.Sg_Val (nm, us_names, ty) ->
      let us_names1 = FStarC_List.map pack_ident us_names in
      let val_lid = FStarC_Ident.lid_of_path nm FStarC_Range_Type.dummyRange in
      (check_lid val_lid;
  ...
  | FStarC_Reflection_V1_Data.Unk ->
      FStarC_Effect.failwith "packing Unk, sorry"
let inspect_lb (lb : FStarC_Syntax_Syntax.letbinding) :
  FStarC_Reflection_V1_Data.lb_view=
  let uu___ = lb in
  match uu___ with
  | { FStarC_Syntax_Syntax.lbname = nm; FStarC_Syntax_Syntax.lbunivs = us;
      ...
      FStarC_Syntax_Syntax.lbdef = def; FStarC_Syntax_Syntax.lbattrs = attrs;
      FStarC_Syntax_Syntax.lbpos = pos;_} ->
      ...
      (match uu___1 with
       | (s, us1) ->
           let typ1 = FStarC_Syntax_Subst.subst s typ in
           let def1 = FStarC_Syntax_Subst.subst s def in
           let us2 = FStarC_List.map inspect_ident us1 in
           (match nm with
...
let pack_lb (lbv : FStarC_Reflection_V1_Data.lb_view) :
  ...
  | { FStarC_Reflection_V1_Data.lb_fv = fv;
      ...
      let us1 = FStarC_List.map pack_ident us in
      ...
      FStarC_Syntax_Util.mk_letbinding (FStar_Pervasives.Inr fv) us1 typ1
        ...
        FStarC_Range_Type.dummyRange
let inspect_bv (bv : FStarC_Syntax_Syntax.bv) :
  FStarC_Reflection_V1_Data.bv_view=
  ...
  then
    (let uu___1 =
       ...
       FStarC_Format.fmt3
         "inspect_bv: index is negative (%s : %s), index = %s"
         (FStarC_Ident.string_of_id bv.FStarC_Syntax_Syntax.ppname) uu___2
         uu___3 in
     FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_CantInspect ()
       (Obj.magic FStarC_Errors_Msg.is_error_message_string)
       (Obj.magic uu___1))
  else ();
  {
    ...
    FStarC_Reflection_V1_Data.bv_index = (bv.FStarC_Syntax_Syntax.index)
  }
let pack_bv (bvv : FStarC_Reflection_V1_Data.bv_view) :
  FStarC_Syntax_Syntax.bv=
  if bvv.FStarC_Reflection_V1_Data.bv_index < Prims.int_zero
  then
    (let uu___1 =
       let uu___2 =
         FStarC_Class_Show.show FStarC_Class_Show.showable_int
       ...
       FStarC_Format.fmt2 "pack_bv: index is negative (%s), index = %s"
         (FStarC_Sealed.unseal bvv.FStarC_Reflection_V1_Data.bv_ppname)
         uu___2 in
     FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_CantInspect ()
       (Obj.magic FStarC_Errors_Msg.is_error_message_string)
       (Obj.magic uu___1))
  else ();
  {
    FStarC_Syntax_Syntax.ppname =
      (FStarC_Ident.mk_ident
         ((FStarC_Sealed.unseal bvv.FStarC_Reflection_V1_Data.bv_ppname),
           FStarC_Range_Type.dummyRange));
    FStarC_Syntax_Syntax.index = (bvv.FStarC_Reflection_V1_Data.bv_index);
    FStarC_Syntax_Syntax.sort = FStarC_Syntax_Syntax.tun
  }
let inspect_binder (b : FStarC_Syntax_Syntax.binder) :
  ...
  {
    FStarC_Reflection_V1_Data.binder_bv = (b.FStarC_Syntax_Syntax.binder_bv);
    FStarC_Reflection_V1_Data.binder_qual =
      (inspect_bqual b.FStarC_Syntax_Syntax.binder_qual);
...
let pack_binder (bview : FStarC_Reflection_V1_Data.binder_view) :
  ...
  | (pqual, attrs) ->
      {
        FStarC_Syntax_Syntax.binder_bv =
          (let uu___1 = bview.FStarC_Reflection_V1_Data.binder_bv in
           {
             FStarC_Syntax_Syntax.ppname =
        ...
        FStarC_Syntax_Syntax.binder_qual =
          (pack_bqual bview.FStarC_Reflection_V1_Data.binder_qual);
        FStarC_Syntax_Syntax.binder_positivity = pqual;
        FStarC_Syntax_Syntax.binder_attrs = attrs
      }
let moduleof (e : FStarC_TypeChecker_Env.env) : Prims.string Prims.list=
  FStarC_Ident.path_of_lid e.FStarC_TypeChecker_Env.curmodule
let env_open_modules (e : FStarC_TypeChecker_Env.env) :
  FStarC_Reflection_V1_Data.name Prims.list=
  FStarC_List.map
    (fun uu___ ->
       match uu___ with
       | (l, m) ->
           FStarC_List.map FStarC_Ident.string_of_id
             (FStarC_Ident.ids_of_lid l))
    (FStarC_Syntax_DsEnv.open_modules e.FStarC_TypeChecker_Env.dsenv)
let binders_of_env (e : FStarC_TypeChecker_Env.env) :
  FStarC_Syntax_Syntax.binders= FStarC_TypeChecker_Env.all_binders e
let eqopt (uu___ : unit) :
  ('uuuuu -> 'uuuuu -> Prims.bool) ->
    'uuuuu FStar_Pervasives_Native.option ->
      'uuuuu FStar_Pervasives_Native.option -> Prims.bool=
  FStarC_Syntax_Util.eqopt
let eqlist (uu___ : unit) :
  ('uuuuu -> 'uuuuu -> Prims.bool) ->
    'uuuuu Prims.list -> 'uuuuu Prims.list -> Prims.bool=
  FStarC_Syntax_Util.eqlist
let eqprod (uu___ : unit) :
  ('uuuuu -> 'uuuuu -> Prims.bool) ->
...
let rec term_eq (t1 : FStarC_Syntax_Syntax.term)
  ...
  match uu___ with
  | (FStarC_Reflection_V1_Data.Tv_Var bv1, FStarC_Reflection_V1_Data.Tv_Var
     bv2) -> bv_eq bv1 bv2
  | (FStarC_Reflection_V1_Data.Tv_BVar bv1, FStarC_Reflection_V1_Data.Tv_BVar
     bv2) -> bv_eq bv1 bv2
  | (FStarC_Reflection_V1_Data.Tv_FVar fv1, FStarC_Reflection_V1_Data.Tv_FVar
  ...
  | (FStarC_Reflection_V1_Data.Tv_Type u1, FStarC_Reflection_V1_Data.Tv_Type
     u2) -> univ_eq u1 u2
  | (FStarC_Reflection_V1_Data.Tv_Refine (b1, sort1, t11),
     FStarC_Reflection_V1_Data.Tv_Refine (b2, sort2, t21)) ->
      let uu___1 = term_eq sort1 sort2 in
      let uu___2 = term_eq t11 t21 in op_Amp_Amp_Dot uu___1 uu___2
     ...
     FStarC_Reflection_V1_Data.Tv_Let (r2, ats2, bv2, ty2, m2, n2)) ->
      let uu___1 =
        let uu___2 =
          ...
          let uu___4 = term_eq ty1 ty2 in op_Amp_Amp_Dot uu___3 uu___4 in
  ...
  | uu___1 -> false
and arg_eq (arg1 : FStarC_Reflection_V1_Data.argv)
  (arg2 : FStarC_Reflection_V1_Data.argv) : Prims.bool=
  let uu___ = arg1 in
  match uu___ with
  | (a1, aq1) ->
      let uu___1 = arg2 in
...
and aqual_eq (aq1 : FStarC_Reflection_V1_Data.aqualv)
  (aq2 : FStarC_Reflection_V1_Data.aqualv) : Prims.bool=
  match (aq1, aq2) with
  | (FStarC_Reflection_V1_Data.Q_Implicit,
     FStarC_Reflection_V1_Data.Q_Implicit) -> true
  | (FStarC_Reflection_V1_Data.Q_Explicit,
     FStarC_Reflection_V1_Data.Q_Explicit) -> true
  | (FStarC_Reflection_V1_Data.Q_Meta t1, FStarC_Reflection_V1_Data.Q_Meta
     t2) -> term_eq t1 t2
  | uu___ -> false
and binder_eq (b1 : FStarC_Syntax_Syntax.binder)
...
and comp_eq (c1 : FStarC_Syntax_Syntax.comp) (c2 : FStarC_Syntax_Syntax.comp)
  ...
  match uu___ with
  | (FStarC_Reflection_V1_Data.C_Total t1, FStarC_Reflection_V1_Data.C_Total
     t2) -> term_eq t1 t2
...
and univ_eq (u1 : FStarC_Syntax_Syntax.universe)
...
let compare_string (s1 : Prims.string) (s2 : Prims.string) : Prims.int=
  FStarC_String.compare s1 s2
let push_binder (e : FStarC_TypeChecker_Env.env)
  (b : FStarC_Syntax_Syntax.binder) : FStarC_TypeChecker_Env.env=
  FStarC_TypeChecker_Env.push_binders e [b]
let subst (x : FStarC_Syntax_Syntax.bv) (n : FStarC_Syntax_Syntax.term)
  (m : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Subst.subst [FStarC_Syntax_Syntax.NT (x, n)] m
let close_term (b : FStarC_Syntax_Syntax.binder)
  (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Subst.close [b] t
