...
let run_tactic_on_typ (rng_tac : FStarC_Range_Type.t)
  (rng_goal : FStarC_Range_Type.t) (tactic : FStarC_Syntax_Syntax.term)
  (env : FStarC_TypeChecker_Env.env) (typ : FStarC_Syntax_Syntax.term) :
  (FStarC_Tactics_Types.goal Prims.list * FStarC_Syntax_Syntax.term)=
  let rng =
    FStarC_Range_Type.range_of_rng (FStarC_Range_Type.use_range rng_tac)
      (FStarC_Range_Type.use_range rng_goal) in
  let uu___ = FStarC_Tactics_V2_Basic.proofstate_of_goal_ty rng env typ in
  match uu___ with
  | (ps, w) ->
      let tactic_already_typed = false in
      let uu___1 =
        FStarC_Tactics_Interpreter.run_tactic_on_ps rng_tac rng_goal false
          FStarC_Syntax_Embeddings.e_unit () FStarC_Syntax_Embeddings.e_unit
          tactic tactic_already_typed ps in
      (match uu___1 with | (gs, _res) -> (gs, w))
let run_tactic_on_all_implicits (rng_tac : FStarC_Range_Type.t)
  (rng_goal : FStarC_Range_Type.t) (tactic : FStarC_Syntax_Syntax.term)
  (env : FStarC_TypeChecker_Env.env)
  (imps : FStarC_TypeChecker_Env.implicits) :
  FStarC_Tactics_Types.goal Prims.list=
  let uu___ =
    FStarC_Tactics_V2_Basic.proofstate_of_all_implicits rng_goal env imps in
  match uu___ with
  | (ps, uu___1) ->
      let tactic_already_typed = false in
      let uu___2 =
        FStarC_Tactics_Interpreter.run_tactic_on_ps
          (FStarC_TypeChecker_Env.get_range env) rng_goal true
          FStarC_Syntax_Embeddings.e_unit () FStarC_Syntax_Embeddings.e_unit
          tactic tactic_already_typed ps in
...
type pol =
  ...
  | Neg 
  | Both 
let uu___is_StrictlyPositive (projectee : pol) : Prims.bool=
  match projectee with | StrictlyPositive -> true | uu___ -> false
let uu___is_Pos (projectee : pol) : Prims.bool=
  match projectee with | Pos -> true | uu___ -> false
let uu___is_Neg (projectee : pol) : Prims.bool=
  match projectee with | Neg -> true | uu___ -> false
let uu___is_Both (projectee : pol) : Prims.bool=
...
let uu___is_Unchanged (projectee : 'a tres_m) : Prims.bool=
  match projectee with | Unchanged _0 -> true | uu___ -> false
let __proj__Unchanged__item___0 (projectee : 'a tres_m) : 'a=
  match projectee with | Unchanged _0 -> _0
let uu___is_Simplified (projectee : 'a tres_m) : Prims.bool=
  match projectee with | Simplified _0 -> true | uu___ -> false
let __proj__Simplified__item___0 (projectee : 'a tres_m) :
  ('a * FStarC_Tactics_Types.goal Prims.list)=
  match projectee with | Simplified _0 -> _0
let uu___is_Dual (projectee : 'a tres_m) : Prims.bool=
  match projectee with | Dual _0 -> true | uu___ -> false
let __proj__Dual__item___0 (projectee : 'a tres_m) :
  ('a * 'a * FStarC_Tactics_Types.goal Prims.list)=
  match projectee with | Dual _0 -> _0
type tres = FStarC_Syntax_Syntax.term tres_m
let tpure (x : 'uuuuu) : 'uuuuu tres_m= Unchanged x
let flip (p : pol) : pol=
  match p with
...
let by_tactic_interp (pol1 : pol) (e : FStarC_TypeChecker_Env.env)
  (t : FStarC_Syntax_Syntax.term) : tres=
      ...
      (match uu___1 with
       ...
       | (FStarC_Syntax_Syntax.Tm_fvar fv,
          (assertion, FStar_Pervasives_Native.None)::[]) when
           FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.spinoff_lid
           ->
           (match pol1 with
            | StrictlyPositive ->
                let g =
          ...
          (tm, FStar_Pervasives_Native.None)::[]) when
           ...
           let uu___3 =
             FStarC_TypeChecker_Env.new_implicit_var_aux
               "rewrite_with_tactic RHS" tm.FStarC_Syntax_Syntax.pos e typ
           ...
           (match uu___3 with
            | (uvtm, uu___4, g_imp) ->
                ...
                (match uu___5 with
                 | (gs, uu___6) ->
                     let tagged_imps =
                       FStarC_TypeChecker_Rel.resolve_implicits_tac e g_imp in
                     (FStarC_Tactics_Interpreter.report_implicits
                        tm.FStarC_Syntax_Syntax.pos tagged_imps;
                      Simplified (uvtm, gs))))
       | uu___2 -> Unchanged t)
let explode (t : 'a tres_m) :
  ('a * 'a * FStarC_Tactics_Types.goal Prims.list)=
  match t with
  | Unchanged t1 -> (t1, t1, [])
  | Simplified (t1, gs) -> (t1, t1, gs)
  | Dual (tn, tp, gs) -> (tn, tp, gs)
let comb1 (f : 'a -> 'b) (uu___ : 'a tres_m) : 'b tres_m=
  match uu___ with
  ...
  | Dual (tn, tp, gs) ->
      ...
      Dual uu___1
let comb2 (f : 'a -> 'b -> 'c) (x : 'a tres_m) (y : 'b tres_m) : 'c tres_m=
  match (x, y) with
  | (Unchanged t1, Unchanged t2) -> let uu___ = f t1 t2 in Unchanged uu___
  | (Unchanged t1, Simplified (t2, gs)) ->
      let uu___ = let uu___1 = f t1 t2 in (uu___1, gs) in Simplified uu___
       ...
       | (n1, p1, gs1) ->
           ...
           (match uu___2 with
            | (n2, p2, gs2) ->
                ...
                Dual uu___3))
let comb_list (rs : 'a tres_m Prims.list) : 'a Prims.list tres_m=
  let rec aux rs1 acc =
    match rs1 with
    | [] -> acc
    | hd::tl -> let uu___ = comb2 (fun l r -> l :: r) hd acc in aux tl uu___ in
  aux (FStarC_List.rev rs) (tpure [])
let emit (gs : FStarC_Tactics_Types.goal Prims.list) (m : 'a tres_m) :
  'a tres_m= comb2 (fun uu___ x -> x) (Simplified ((), gs)) m
let rec traverse
  (f :
    pol -> FStarC_TypeChecker_Env.env -> FStarC_Syntax_Syntax.term -> tres)
  (pol1 : pol) (e : FStarC_TypeChecker_Env.env)
  (t : FStarC_Syntax_Syntax.term) : tres=
  let r =
    let uu___ =
    ...
    | FStarC_Syntax_Syntax.Tm_meta
        ...
        comb1
          (fun t' ->
             FStarC_Syntax_Syntax.Tm_meta
               { FStarC_Syntax_Syntax.tm2 = t'; FStarC_Syntax_Syntax.meta = m
               }) tr
            ...
            (p, uu___6)::(q, uu___7)::[]) when
             ...
             let xp =
               ...
               FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None
                 uu___8 in
             let xq =
               let uu___8 = FStarC_Syntax_Util.mk_squash q in
               FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None
                 uu___8 in
             ...
             (match (r1, r2) with
              | (Unchanged uu___8, Unchanged uu___9) ->
                  comb2
                    (fun l r3 ->
                       let uu___10 = FStarC_Syntax_Util.mk_iff l r3 in
                       uu___10.FStarC_Syntax_Syntax.n) r1 r2
              | uu___8 ->
                  let uu___9 = explode r1 in
                  (match uu___9 with
                   | (pn, pp, gs1) ->
                       let uu___10 = explode r2 in
                       (match uu___10 with
                        | (qn, qp, gs2) ->
                            let t1 =
                              let uu___11 = FStarC_Syntax_Util.mk_imp pn qp in
                              let uu___12 = FStarC_Syntax_Util.mk_imp qn pp in
                              FStarC_Syntax_Util.mk_conj uu___11 uu___12 in
                            Simplified
               ...
               FStarC_List.fold_right
                 (fun uu___3 r2 ->
                    ...
                    | (a, q) ->
                        ...
                        comb2 (fun a1 args1 -> (a1, q) :: args1) r' r2) args
                 (tpure []) in
             comb2
               (fun hd1 args1 ->
                  let uu___3 =
                    FStarC_Syntax_Syntax.mk_Tm_app hd1 args1
                      t.FStarC_Syntax_Syntax.pos in
                  uu___3.FStarC_Syntax_Syntax.n) r0 r1)
    | FStarC_Syntax_Syntax.Tm_abs uu___1 ->
        let uu___2 = FStarC_Syntax_Util.abs_formals t in
        (match uu___2 with
         | (bs, topen, k) ->
             let e' = FStarC_TypeChecker_Env.push_binders e bs in
             let r0 =
               FStarC_List.map
                 (fun b ->
                    let r1 =
             ...
             comb2
               (fun bs1 t1 ->
                  let uu___3 = FStarC_Syntax_Util.abs bs1 t1 k in
                  uu___3.FStarC_Syntax_Syntax.n) rbs rt)
            ...
            FStarC_List.map
              (fun br ->
                 ...
                 | (pat, w, exp) ->
                     ...
                     comb1
                       (fun exp1 ->
                          FStarC_Syntax_Subst.close_branch (pat, w, exp1)) r1)
              brs in
          comb_list uu___3 in
        comb2
          (fun sc1 brs1 ->
...
let preprocess (env : FStarC_TypeChecker_Env.env)
  (goal : FStarC_Syntax_Syntax.term) :
                     ...
                     | (n, gs1) ->
                         ...
                         ((let uu___6 = FStarC_Effect.op_Bang dbg_Tac in
                           ...
                           then
                             ...
                             let uu___8 =
                               let uu___9 = FStarC_Tactics_Types.goal_type g in
                               FStarC_Class_Show.show
                             ...
                             let uu___6 =
                               let uu___7 =
                                 ...
                                 FStar_Pprint.op_Hat_Slash_Hat uu___8
                                   (if
                                      (FStarC_Tactics_Types.get_label g) = ""
                               ...
                               FStar_Pprint.op_Hat_Hat
                                 (FStar_Pprint.doc_of_string
                                    "Could not prove goal #") uu___7 in
                             [uu___6] in
                           let gt' =
                             FStarC_TypeChecker_Util.label label
                               (FStarC_Tactics_Types.goal_range g) phi in
                           ((n + Prims.int_one),
                             (((FStarC_Tactics_Types.goal_env g), gt',
                                (FStarC_Tactics_Types.goal_opts g)) :: gs1)))))
                  s gs in
              let uu___4 = s1 in
              match uu___4 with
              | (uu___5, gs1) ->
                  ...
                  let uu___6 =
                    ...
                    uu___7 :: gs2 in
                  (did_anything, uu___6)))))
let rec traverse_for_spinoff (pol1 : pol)
  (label_ctx :
    (FStar_Pprint.document Prims.list * FStarC_Range_Type.t)
      FStar_Pervasives_Native.option)
  (e : FStarC_TypeChecker_Env.env) (t : FStarC_Syntax_Syntax.term) : 
  tres=
  let debug_any = FStarC_Debug.any () in
  let traverse1 pol2 e1 t1 = traverse_for_spinoff pol2 label_ctx e1 t1 in
  let traverse_ctx pol2 ctx e1 t1 =
    ...
    (let uu___1 = FStarC_Effect.op_Bang dbg_SpinoffAll in
     ...
     then
       ...
       FStarC_Format.print2 "Changing label context from %s to %s" uu___2
         uu___3
     else ());
    traverse_for_spinoff pol2 (FStar_Pervasives_Native.Some ctx) e1 t1 in
      ...
      | StrictlyPositive ->
          ((let uu___1 = FStarC_Effect.op_Bang dbg_SpinoffAll in
            ...
            then
              ...
              FStarC_Format.print1 "Spinning off %s\n" uu___2
            else ());
           (let uu___1 =
              let uu___2 = let uu___3 = label_goal (e1, t2) in [uu___3] in
              (FStarC_Syntax_Util.t_true, uu___2) in
            Simplified uu___1))
      | uu___ -> Unchanged t2 in
    let t2 = FStarC_Syntax_Subst.compress t1 in
    let uu___ = let uu___1 = should_descend t2 in Prims.op_Negation uu___1 in
  ...
  let try_rewrite_match env t1 =
    let rec pat_as_exp env1 p =
      let uu___ = FStarC_TypeChecker_PatternUtils.raw_pat_as_exp env1 p in
      match uu___ with
    ...
    let mk_forall_l bv_univs term =
      FStarC_List.fold_right
        (fun uu___ out ->
           match uu___ with | (x, u) -> FStarC_Syntax_Util.mk_forall u x out)
        bv_univs term in
    let mk_exists_l bv_univs term =
      FStarC_List.fold_right
        (fun uu___ out ->
           match uu___ with | (x, u) -> FStarC_Syntax_Util.mk_exists u x out)
        bv_univs term in
                    ...
                    FStarC_Syntax_Syntax.Tm_meta
                      {
                        ...
                        FStarC_Syntax_Syntax.meta =
                          (FStarC_Syntax_Syntax.Meta_labeled (msg, r1, false))
                      }) tr
           | FStarC_Syntax_Syntax.Tm_meta
               { FStarC_Syntax_Syntax.tm2 = t2;
           ...
           | FStarC_Syntax_Syntax.Tm_app uu___3 ->
               ...
               (match uu___4 with
                | ({
                   ...
                   (p, uu___8)::(q, uu___9)::[]) when
                    FStarC_Syntax_Syntax.fv_eq_lid fv
                      FStarC_Parser_Const.imp_lid
                    ->
                    let x =
                      let uu___10 = FStarC_Syntax_Util.mk_squash p in
                      FStarC_Syntax_Syntax.new_bv
                        FStar_Pervasives_Native.None uu___10 in
                    let r1 = traverse1 (flip pol1) e p in
                    let r2 =
                      traverse1 pol1 (FStarC_TypeChecker_Env.push_bv e x) q in
                    comb2
                      (fun l r3 ->
                         let uu___10 = FStarC_Syntax_Util.mk_imp l r3 in
                         uu___10.FStarC_Syntax_Syntax.n) r1 r2
                | (hd, args) ->
                    let app_pos = t1.FStarC_Syntax_Syntax.pos in
                    let uu___5 =
                      let uu___6 =
                    ...
                    (match uu___5 with
                     | (FStarC_Syntax_Syntax.Tm_fvar fv,
                        (ty, FStar_Pervasives_Native.Some aq0)::(body, aq)::[])
                         when
                         ((FStarC_Syntax_Syntax.fv_eq_lid fv
                             FStarC_Parser_Const.forall_lid)
                            ||
                         ...
                         let rt = traverse1 (flip pol1) e ty in
                         let rbody = traverse1 pol1 e body in
                         let rargs =
                           comb2
                             (fun t2 body1 ->
                                [(t2, (FStar_Pervasives_Native.Some aq0));
                         ...
                         comb2
                           (fun hd1 args1 ->
                              let uu___6 =
                                FStarC_Syntax_Syntax.mk_Tm_app hd1 args1
                                  app_pos in
                              uu___6.FStarC_Syntax_Syntax.n) r0 rargs
                     | uu___6 ->
                         let r0 = traverse1 pol1 e hd in
                         let r1 =
                           FStarC_List.fold_right
                           ...
                           (fun hd1 args1 ->
                              ...
                              | uu___8 ->
                                  let uu___9 =
                                    FStarC_Syntax_Syntax.mk_Tm_app hd1 args1
                                      app_pos in
                                  uu___9.FStarC_Syntax_Syntax.n) r0 r1))
           | FStarC_Syntax_Syntax.Tm_abs uu___3 ->
               let uu___4 = FStarC_Syntax_Util.abs_formals t1 in
               (match uu___4 with
                | (bs, topen, k) ->
                    let e' = FStarC_TypeChecker_Env.push_binders e bs in
                    let r0 =
                    ...
                    comb2
                      (fun bs1 t2 ->
                         let uu___5 = FStarC_Syntax_Util.abs bs1 t2 k in
...
let pol_to_string (uu___ : pol) : Prims.string=
  match uu___ with
  | StrictlyPositive -> "StrictlyPositive"
  | Pos -> "Positive"
  | Neg -> "Negative"
  | Both -> "Both"
let spinoff_strictly_positive_goals (env : FStarC_TypeChecker_Env.env)
  (goal : FStarC_Syntax_Syntax.term) :
  ...
  (let uu___1 = FStarC_Effect.op_Bang dbg_SpinoffAll in
   ...
   then
     let uu___2 =
       FStarC_Class_Show.show FStarC_Syntax_Print.showable_term goal in
     FStarC_Format.print1 "spinoff_all called with %s\n" uu___2
   else ());
  FStarC_Errors.with_ctx "While spinning off all goals"
    (fun uu___1 ->
       ...
       let uu___2 =
         let uu___3 =
           traverse_for_spinoff StrictlyPositive FStar_Pervasives_Native.None
             env goal in
         match uu___3 with
         | Unchanged t' -> (t', [])
             ...
             FStarC_List.fold_left
               (fun uu___3 g ->
                  ...
                  | (n, gs1) ->
                      ...
                      ((n + Prims.int_one),
                        (((FStarC_Tactics_Types.goal_env g), phi) :: gs1))) s
               gs in
           let uu___3 = s1 in
           (match uu___3 with
            | (uu___4, gs1) ->
                            ...
                            | FStarC_TypeChecker_Common.NonTrivial t2 ->
                                ((let uu___8 =
                                  ...
                                  then
                                    ...
                                    FStarC_Format.print1 "Got goal: %s\n"
                                      uu___9
                                  else ());
...
let synthesize (env : FStarC_TypeChecker_Env.env)
  (typ : FStarC_Syntax_Syntax.typ) (tau : FStarC_Syntax_Syntax.term)
  (rng : FStarC_Range_Type.range) : FStarC_Syntax_Syntax.term=
  FStarC_Errors.with_ctx "While synthesizing term with a tactic"
    (fun uu___ ->
       if env.FStarC_TypeChecker_Env.flychecking
       then
         let uu___1 =
           FStarC_TypeChecker_Util.fvar_env env FStarC_Parser_Const.magic_lid in
         FStarC_Syntax_Syntax.mk_Tm_app uu___1
          ...
          | (gs, w) ->
              (FStarC_List.iter
                 (fun g ->
                    ...
                    | FStar_Pervasives_Native.None ->
                        FStarC_Errors.raise_error
                          (FStarC_Syntax_Syntax.has_range_syntax ()) typ
                          FStarC_Errors_Codes.Fatal_OpenGoalsInSynthesis ()
                          (Obj.magic
                             FStarC_Errors_Msg.is_error_message_string)
                          (Obj.magic "synthesis left open goals")) gs;
               w)))
let solve_implicits (env : FStarC_TypeChecker_Env.env)
  (tau : FStarC_Syntax_Syntax.term) (imps : FStarC_TypeChecker_Env.implicits)
  : unit=
                     ...
                     | FStar_Pervasives_Native.None ->
                         FStarC_Errors.raise_error
                           ...
                           (Obj.magic
                              FStarC_Errors_Msg.is_error_message_string)
                           (Obj.magic "synthesis left open goals"))) gs)))
let find_user_tac_for_attr (env : FStarC_TypeChecker_Env.env)
  (a : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.sigelt FStar_Pervasives_Native.option=
  let hooks =
    FStarC_TypeChecker_Env.lookup_attr env
      (FStarC_Ident.string_of_lid FStarC_Parser_Const.handle_smt_goals_attr) in
  FStarC_Util.try_find (fun uu___ -> true) hooks
let handle_smt_goal (env : FStarC_TypeChecker_Env.env)
  (goal : FStarC_TypeChecker_Env.goal) :
  (FStarC_TypeChecker_Env.env * FStarC_Syntax_Syntax.term) Prims.list=
           ...
           let tau =
             ...
             | FStarC_Syntax_Syntax.Sig_let
                 ...
                 FStarC_Syntax_Syntax.fv_to_tm
                   (FStarC_Syntax_Syntax.lid_as_fv lid
                      FStar_Pervasives_Native.None)
             | uu___2 -> FStarC_Effect.failwith "Resolve_tac not found" in
           let gs =
...
type dsl_tac_result_t =
  (dsl_typed_sigelt_t Prims.list * dsl_typed_sigelt_t * dsl_typed_sigelt_t
    Prims.list)
let rec us_equals (us : FStarC_Syntax_Syntax.univ_name Prims.list)
  (vs : FStarC_Syntax_Syntax.univ_name Prims.list) : Prims.bool=
  match (us, vs) with
  | (u::us1, v::vs1) ->
      if FStarC_Ident.ident_equals u v then us_equals us1 vs1 else false
  | uu___ -> true
let splice : FStarC_TypeChecker_Env.splice_t=
  fun env quals attrs is_typed lids tau rng ->
           ...
           (let uu___1 =
              ...
              then
                FStarC_TypeChecker_TcTerm.tc_check_tot_or_gtot_term env tau
                  FStarC_Syntax_Util.t_dsl_tac_typ
                  FStar_Pervasives_Native.None
              else
                                                    ...
                                                    (let uu___11 =
                                                       se.FStarC_Syntax_Syntax.sigmeta in
                                                     {
                                                       ...
                                                       FStarC_Syntax_Syntax.sigmeta_extension_decl
                                                         =
                                                         (uu___11.FStarC_Syntax_Syntax.sigmeta_extension_decl);
                                 ...
                                 (let uu___8 =
                                  ...
                                  | FStar_Pervasives_Native.None ->
                                      FStarC_Errors.raise_error
                                        ...
                                        (Obj.magic
                                           FStarC_Errors_Msg.is_error_message_string)
                                        (Obj.magic "splice left open goals")))
                              gs);
                       (let lids' =
                          FStarC_List.collect
                        ...
                        (let sigelts2 =
                         ...
                         (match () with | () -> sigelts2))))))))
let mpreprocess (env : FStarC_TypeChecker_Env.env)
  (tau : FStarC_Syntax_Syntax.term) (tm : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term=
  FStarC_Errors.with_ctx "While preprocessing a definition with a tactic"
    (fun uu___ ->
       if env.FStarC_TypeChecker_Env.flychecking
       then tm
       else
...
let postprocess (env : FStarC_TypeChecker_Env.env)
  ...
  FStarC_Errors.with_ctx "While postprocessing a definition with a tactic"
    (fun uu___ ->
       if env.FStarC_TypeChecker_Env.flychecking
       then tm
       else
                      ...
                      (fun g ->
                         ...
                         | FStar_Pervasives_Native.None ->
                             FStarC_Errors.raise_error
                               ...
                               (Obj.magic
                                  FStarC_Errors_Msg.is_error_message_string)
                               (Obj.magic "postprocessing left open goals"))
                      gs;
                    (let tagged_imps =
                       FStarC_TypeChecker_Rel.resolve_implicits_tac env g_imp in
                     FStarC_Tactics_Interpreter.report_implicits
                       tm.FStarC_Syntax_Syntax.pos tagged_imps;
                     uvtm)))))
