              ...
              (match uu___2 with
               | (annots', binder1, aqs2) ->
                   ((match p3.FStarC_Syntax_Syntax.v with
                     ...
                     | uu___4 when
                         if top then top_level_ascr_allowed else false -> ()
                     | uu___4 ->
                         FStarC_Errors.raise_error
            ...
            (FStarC_Syntax_Syntax.Tm_app
               {
                 ...
                 FStarC_Syntax_Syntax.args =
                   [(repr', (FStarC_Syntax_Syntax.as_aqual_implicit false))]
               }) range in
        FStarC_Syntax_Syntax.mk
   ...
   | FStarC_Parser_AST.Let (qual, lbs, body) ->
       ...
       (if
          (if Prims.op_Negation is_rec
           then (FStarC_List.length lbs) > Prims.int_one
           else false)
        then
                   ...
                   | b::[] ->
                       ...
                       let args =
                         let uu___3 =
                           let uu___4 =
                             let uu___5 =
                               FStarC_Syntax_Util.abs [FStarC_List.hd bs1] p2
                                 FStar_Pervasives_Native.None in
                             (uu___5, FStar_Pervasives_Native.None) in
                           [uu___4] in
                         ((x.FStarC_Syntax_Syntax.sort),
                 ...
                 let rec aux binders1 squash_token =
                   ...
                   | b::[] ->
                       ...
                       let uu___4 =
                         ...
                         FStarC_Syntax_Util.abs [b; b_pf_p] uu___5
                           FStar_Pervasives_Native.None in
                       mk_exists_elim x.FStarC_Syntax_Syntax.sort uu___3
                         squash_token uu___4
                         squash_token.FStarC_Syntax_Syntax.pos
                   | b::bs1 ->
                       let pf_i =
                         FStarC_Syntax_Syntax.gen_bv "pf"
                           (FStar_Pervasives_Native.Some
                              (FStarC_Syntax_Syntax.range_of_bv
                                 b.FStarC_Syntax_Syntax.binder_bv))
                           FStarC_Syntax_Syntax.tun in
                       let k =
                         let uu___3 = FStarC_Syntax_Syntax.bv_to_name pf_i in
                         aux bs1 uu___3 in
                       let x = unqual_bv_of_binder b in
                       let uu___3 =
                       ...
                       let uu___4 =
                         FStarC_Syntax_Util.abs
                           [b; FStarC_Syntax_Syntax.mk_binder pf_i] k
                           FStar_Pervasives_Native.None in
                       mk_exists_elim x.FStarC_Syntax_Syntax.sort uu___3
                         squash_token uu___4
                 ...
                 let range =
                   FStarC_List.fold_right
                     (fun bs1 r ->
                        FStarC_Range_Ops.union_ranges
                          (FStarC_Syntax_Syntax.range_of_bv
                             bs1.FStarC_Syntax_Syntax.binder_bv) r) bs
                     p1.FStarC_Syntax_Syntax.pos in
                 let uu___3 =
                   aux bs
                     {
...
and desugar_comp (r : FStarC_Range_Type.range)
  ...
  let is_smt_pat1 t1 =
    ...
    | FStarC_Parser_AST.Construct (smtpat, uu___) ->
        ...
        if (if s = "SMTPat" then true else s = "SMTPatT")
        then true
        else s = "SMTPatOr"
    | FStarC_Parser_AST.Var smtpat ->
        let s = FStarC_Ident.string_of_lid smtpat in
        if s = "smt_pat" then true else s = "smt_pat_or"
    | uu___ -> false in
                 ...
                 (match uu___4 with
                  | (dec, rest1) ->
                      ...
                      let no_additional_args =
                        ...
                        if
                          (if
                             (if is_empty decreases_clause
                              then is_empty rest2
                              else false)
                           then is_empty cattributes
                           else false)
                        then is_empty universes1
                        else false in
                      if
               ...
               (FStarC_Syntax_Syntax.Tm_app
                  {
                    ...
                    FStarC_Syntax_Syntax.args =
                      [FStarC_Syntax_Syntax.as_arg body3]
                  }))
    ...
    | (FStar_Pervasives_Native.Some (e1, rng1, l1),
       FStar_Pervasives_Native.Some (e2, rng2, l2)) ->
        let uu___ =
          ...
          ((FStarC_List.op_At e1 e2), uu___1, (if l1 then true else l2)) in
        FStar_Pervasives_Native.Some uu___
...
let rec desugar_effect (env : FStarC_Syntax_DsEnv.env)
  ...
  | (env1, binders) ->
      ...
      (if
         (if num_indices = Prims.int_one
          then Prims.op_Negation is_layered
          else false)
       then
...
and desugar_decl_core (env : FStarC_Syntax_DsEnv.env)
               ...
               FStarC_Class_Setlike.elems ()
                 (Obj.magic
                    (FStarC_FlatSet.setlike_flat_set
                       FStarC_Syntax_Syntax.ord_ident)) (Obj.magic uu___2) in
             if
               (if Prims.uu___is_Nil bvs
                then Prims.op_Negation (is_var_pattern pat)
                else false)
  ...
  | FStarC_Parser_AST.Val (id, t) ->
      ...
      let quals1 =
        if
          (if FStarC_Syntax_DsEnv.iface env
           then FStarC_Syntax_DsEnv.admitted_iface env
           else false)
  ...
  | FStarC_Parser_AST.SubEffect l ->
      ...
      if
        Prims.op_Negation
          (if FStarC_Syntax_Util.is_layered src_ed
           then true
           else FStarC_Syntax_Util.is_layered dst_ed)
      then
        let uu___ =
