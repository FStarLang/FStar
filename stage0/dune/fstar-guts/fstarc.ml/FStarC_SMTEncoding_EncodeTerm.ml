        ...
        (let uu___2 =
         ...
         | (fsym, fterm) ->
               ...
               FStarC_List.map
                 (fun p ->
                    ...
                    | FStarC_SMTEncoding_Term.App
                        (FStarC_SMTEncoding_Term.Var "HasType", args, uu___3)
                        ->
             ...
             let body1 =
               match body with
               | FStarC_SMTEncoding_Term.App
                   (FStarC_SMTEncoding_Term.Imp, guard::body'::[], uu___3) ->
                   let guard1 =
                     match guard with
                     | FStarC_SMTEncoding_Term.App
                         (FStarC_SMTEncoding_Term.And, guards, uu___4) ->
                         let uu___5 = add_fuel guards in
                         FStarC_SMTEncoding_Util.mk_and_l uu___5
                     | uu___4 ->
...
let norm (env : FStarC_SMTEncoding_Env.env_t) (t : FStarC_Syntax_Syntax.term)
  ...
  norm_with_steps
    [FStarC_TypeChecker_Env.Beta;
    FStarC_TypeChecker_Env.Exclude FStarC_TypeChecker_Env.Zeta;
    FStarC_TypeChecker_Env.Eager_unfolding] env.FStarC_SMTEncoding_Env.tcenv
    t
...
let isTotFun_axioms (pos : FStarC_Range_Type.t)
  (head : FStarC_SMTEncoding_Term.term)
  (extra_vars : FStarC_SMTEncoding_Term.fvs)
  (vars : FStarC_SMTEncoding_Term.fvs)
  (guards : FStarC_SMTEncoding_Term.term Prims.list) (is_pure : Prims.bool) :
  FStarC_SMTEncoding_Term.term=
  let maybe_mkForall pat vars1 body =
    match FStarC_List.op_At extra_vars vars1 with
    | [] -> body
    | vs -> FStarC_SMTEncoding_Term.mkForall pos (pat, vs, body) in
...
let maybe_curry_app (rng : FStarC_Range_Type.t)
  ...
  | FStar_Pervasives.Inl head1 ->
      ...
      then FStarC_SMTEncoding_Util.mkApp' (head1, args)
        ...
        if n_args > arity
        then
          (let uu___ = FStarC_Util.first_N arity args in
           match uu___ with
           | (args1, rest) ->
               let head2 = FStarC_SMTEncoding_Util.mkApp' (head1, args1) in
               mk_Apply_args head2 rest)
        else
          (let uu___ = FStarC_SMTEncoding_Term.op_to_string head1 in
           raise_arity_mismatch uu___ arity n_args rng)
  ...
  let pats1 =
    FStarC_List.map
      (fun uu___ ->
         match uu___ with
         | (x, uu___1) ->
             norm_with_steps [FStarC_TypeChecker_Env.Beta]
    ...
    | uu___ ->
        ...
        then let uu___1 = whnf env t1 in aux false uu___1
          ...
          (let uu___1 =
             ...
             FStarC_Format.fmt2 "(%s) Expected a function typ; got %s" uu___2
               uu___3 in
...
let rec curried_arrow_formals_comp (k : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_arrow uu___ ->
      FStarC_Syntax_Util.arrow_node_formals_comp k1
  | FStarC_Syntax_Syntax.Tm_refine
      { FStarC_Syntax_Syntax.b2 = bv; FStarC_Syntax_Syntax.phi = uu___;_} ->
...
let rec encode_universe (u : FStarC_Syntax_Syntax.universe) :
  ...
  | FStarC_Syntax_Syntax.U_succ u1 ->
      ...
      FStarC_SMTEncoding_Term.mk_U_succ uu___
  | FStarC_Syntax_Syntax.U_max (u1::us) ->
      let uu___ = encode_universe u1 in
  ...
  | FStarC_Syntax_Syntax.U_unif uv ->
      let uu___ =
        ...
        FStarC_SMTEncoding_Util.mkInteger' uu___1 in
      FStarC_SMTEncoding_Term.mk_U_unif uu___
  ...
  | (arg_tms, decls) ->
      ...
      let mk_nl box unbox nm op ts =
        ...
        then
          ...
          (let uu___2 = FStarC_Options.smtencoding_nl_arith_native () in
           if uu___2
           then
             let uu___3 = let uu___4 = binary unbox ts in op uu___4 in
             box uu___3
           else mk_default ()) in
      let add box unbox =
...
and encode_BitVector_term (env : FStarC_SMTEncoding_Env.env_t)
  ...
  | (tm_sz, uu___1) ->
      ...
      let bv_size_decls n =
        ...
        let uu___3 = FStarC_SMTEncoding_Term.mkBvConstructor n in
        match uu___3 with
              ...
              let uu___5 =
                ...
                let t =
                  ...
                  FStarC_Syntax_Util.mk_app uu___6
                    [(n_tm, FStar_Pervasives_Native.None)] in
      ...
      (match uu___3 with
       | (arg_tms, ext_sz) ->
           let sz_decls =
             let uu___4 = bv_size_decls sz in
             let uu___5 =
               match ext_sz with
               | FStar_Pervasives_Native.Some ext -> bv_size_decls (sz + ext)
               | FStar_Pervasives_Native.None -> [] in
...
and encode_deeply_embedded_quantifier (t : FStarC_Syntax_Syntax.term)
  ...
  | (tm, decls) ->
      ...
      (match tm with
       | FStarC_SMTEncoding_Term.App
           (uu___1, (FStarC_SMTEncoding_Term.FreeV
            uu___2)::(FStarC_SMTEncoding_Term.FreeV uu___3)::[], uu___4)
           ->
  ...
  (let t1 = FStarC_Syntax_Subst.compress t in
   ...
   (match t1.FStarC_Syntax_Syntax.n with
    ...
    | FStarC_Syntax_Syntax.Tm_type u ->
        let uu___2 =
          let uu___3 = encode_universe u in
          FStarC_SMTEncoding_Term.mk_Term_type uu___3 in
        (uu___2, [])
    | FStarC_Syntax_Syntax.Tm_uinst
        ({ FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_fvar fv;
         ...
         uu___4)
        when FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.lex_t_lid
        -> (FStarC_SMTEncoding_Util.mk_lex_t, [])
    | FStarC_Syntax_Syntax.Tm_uinst
        ({ FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_fvar fv;
           FStarC_Syntax_Syntax.pos = uu___2;
           FStarC_Syntax_Syntax.hash_code = uu___3;_},
         uu___4)
        when FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.lextop_lid
        -> (FStarC_SMTEncoding_Util.mk_LexTop, [])
    | FStarC_Syntax_Syntax.Tm_fvar v ->
        let us =
          match t1.FStarC_Syntax_Syntax.n with
          | FStarC_Syntax_Syntax.Tm_fvar uu___2 -> []
          | FStarC_Syntax_Syntax.Tm_uinst (uu___2, us1) -> us1 in
        let encode_freev uu___2 =
          let us1 = FStarC_List.map encode_universe us in
          let fvb =
          ...
          let uu___3 =
            ...
            | (FStarC_SMTEncoding_Term.App
               (FStarC_SMTEncoding_Term.Var "ApplyTF",
                ...
                (tok1, uu___4, uu___5)))::fuel::[], uu___6),
               us2) ->
                let uu___7 =
                  let uu___8 =
                    let uu___9 =
                      let uu___10 = FStarC_SMTEncoding_Util.mkApp (tok1, us2) in
                      [uu___10; fuel] in
                    ("ApplyTF", uu___9) in
            ...
            | (FStarC_SMTEncoding_Term.App
               (FStarC_SMTEncoding_Term.Var "ApplyTF",
                (FStarC_SMTEncoding_Term.App
                (FStarC_SMTEncoding_Term.Var tok1, [], uu___4))::fuel::[],
                uu___5),
               us2) ->
                let uu___6 =
                  let uu___7 =
                    let uu___8 =
                      let uu___9 = FStarC_SMTEncoding_Util.mkApp (tok1, us2) in
                      [uu___9; fuel] in
                    ("ApplyTF", uu___8) in
                  FStarC_SMTEncoding_Util.mkApp uu___7 in
                (true, uu___6)
            | (FStarC_SMTEncoding_Term.App (op, uu___4::uu___5, uu___6),
               uu___7::uu___8) ->
                let uu___9 =
                  let uu___10 =
                    FStarC_Class_Show.show
                      FStarC_SMTEncoding_Term.showable_smt_term tok in
                  FStarC_Format.fmt1
            ...
            | (FStarC_SMTEncoding_Term.App (op, [], uu___4), us2) ->
                (true,
                  (FStarC_SMTEncoding_Term.App
                     (op, us2, (FStarC_SMTEncoding_Term.range_of_term tok))))
            | (FStarC_SMTEncoding_Term.App (op, ts, uu___4), []) ->
                (false, tok) in
          match uu___3 with
          | (is_nullary, tok1) ->
              ...
              let uu___4 =
                ...
                then
                  ...
                  match uu___5 with
                  | FStar_Pervasives_Native.None -> ([], "")
                  | FStar_Pervasives_Native.Some kpa ->
                      let sym_name =
                        let uu___6 = FStarC_Util.digest_of_string tkey_hash in
                        Prims.strcat "@kick_partial_app_" uu___6 in
                      let uu___6 =
                        let uu___7 =
                          FStarC_SMTEncoding_Util.mkAssume
                            (kpa,
    ...
    | FStarC_Syntax_Syntax.Tm_uinst
        ({ FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_fvar v;
           FStarC_Syntax_Syntax.pos = uu___2;
           FStarC_Syntax_Syntax.hash_code = uu___3;_},
         uu___4)
        ->
        let us =
          ...
          | FStarC_Syntax_Syntax.Tm_uinst (uu___5, us1) -> us1 in
        let encode_freev uu___5 =
          let us1 = FStarC_List.map encode_universe us in
          let fvb =
            FStarC_SMTEncoding_Env.lookup_free_var_name env
              v.FStarC_Syntax_Syntax.fv_name in
          ...
          let uu___6 =
            match (tok, us1) with
            | (FStarC_SMTEncoding_Term.FreeV uu___7, uu___8::uu___9) ->
                FStarC_Effect.failwith
                  "Impossible: Universe applications on nullary symbol"
            | (FStarC_SMTEncoding_Term.App
               (FStarC_SMTEncoding_Term.Var "ApplyTF",
                (FStarC_SMTEncoding_Term.FreeV (FStarC_SMTEncoding_Term.FV
                (tok1, uu___7, uu___8)))::fuel::[], uu___9),
               us2) ->
                let uu___10 =
                  let uu___11 =
                    let uu___12 =
                      let uu___13 = FStarC_SMTEncoding_Util.mkApp (tok1, us2) in
                      [uu___13; fuel] in
                    ("ApplyTF", uu___12) in
            ...
            | (FStarC_SMTEncoding_Term.App
               (FStarC_SMTEncoding_Term.Var "ApplyTF",
                (FStarC_SMTEncoding_Term.App
                (FStarC_SMTEncoding_Term.Var tok1, [], uu___7))::fuel::[],
                uu___8),
               us2) ->
                let uu___9 =
                  let uu___10 =
                    let uu___11 =
                      let uu___12 = FStarC_SMTEncoding_Util.mkApp (tok1, us2) in
                      [uu___12; fuel] in
                    ("ApplyTF", uu___11) in
                  FStarC_SMTEncoding_Util.mkApp uu___10 in
                (true, uu___9)
            | (FStarC_SMTEncoding_Term.App (op, uu___7::uu___8, uu___9),
               uu___10::uu___11) ->
                let uu___12 =
                  let uu___13 =
                    FStarC_Class_Show.show
            ...
            | (FStarC_SMTEncoding_Term.FreeV uu___7, []) -> (true, tok)
            | (FStarC_SMTEncoding_Term.App (op, [], uu___7), us2) ->
                (true,
                  (FStarC_SMTEncoding_Term.App
                     (op, us2, (FStarC_SMTEncoding_Term.range_of_term tok))))
            | (FStarC_SMTEncoding_Term.App (op, ts, uu___7), []) ->
                (false, tok) in
          match uu___6 with
          | (is_nullary, tok1) ->
              let tkey_hash = FStarC_SMTEncoding_Term.hash_of_term tok1 in
              let uu___7 =
                if
                  (fvb.FStarC_SMTEncoding_Env.smt_arity > Prims.int_zero) &&
                    is_nullary
                then
                  let uu___8 = FStarC_SMTEncoding_Env.kick_partial_app fvb in
                  match uu___8 with
                  | FStar_Pervasives_Native.None -> ([], "")
                  | FStar_Pervasives_Native.Some kpa ->
                      let sym_name =
                        let uu___9 = FStarC_Util.digest_of_string tkey_hash in
                        Prims.strcat "@kick_partial_app_" uu___9 in
                      let uu___9 =
                        let uu___10 =
                          FStarC_SMTEncoding_Util.mkAssume
                            (kpa,
                   ...
                   let uu___8 =
                     ...
                     then FStarC_SMTEncoding_Term.mk_decls_trivial []
                       ...
                       FStarC_SMTEncoding_Term.mk_decls sym_name tkey_hash
                         aux_decls [] in
                   (tok1, uu___8)) in
        let uu___5 = head_redex env t1 in
        if uu___5
        then
          let uu___6 = maybe_whnf env t1 in
          (match uu___6 with
           | FStar_Pervasives_Native.None -> encode_freev ()
           | FStar_Pervasives_Native.Some t2 -> encode_term t2 env)
        else encode_freev ()
    ...
    | FStarC_Syntax_Syntax.Tm_arrow uu___2 ->
        ...
        let uu___3 = FStarC_Syntax_Util.arrow_node_formals_comp t1 in
        (match uu___3 with
         | (binders, res) ->
             let uu___4 =
               let uu___5 =
                    ...
                    let tcenv_bs =
                      let uu___6 = env'.FStarC_SMTEncoding_Env.tcenv in
                      {
                        ...
                        FStarC_TypeChecker_Env.iface_todo =
                          (uu___6.FStarC_TypeChecker_Env.iface_todo);
                        FStarC_TypeChecker_Env.iface_hidden =
                          (uu___6.FStarC_TypeChecker_Env.iface_hidden);
                        FStarC_TypeChecker_Env.iface_lids =
                          (uu___6.FStarC_TypeChecker_Env.iface_lids);
                        FStarC_TypeChecker_Env.iface_val_lids =
                          (uu___6.FStarC_TypeChecker_Env.iface_val_lids)
                      } in
                    let uu___6 =
                              ...
                              (match uu___8 with
                               | (guards, guard_decls) ->
                                             ...
                                             let uu___10 =
                                               let uu___11 =
                                                 let uu___12 =
                                                   ...
                                                   encode_universe uu___13 in
                                                 FStarC_SMTEncoding_Term.mk_Term_type
                                                   uu___12 in
                                               (acc, uu___11) in
                                             FStarC_SMTEncoding_Util.mkAnd
                                               uu___10 in
                                           add_bv_sorts bvs2 uu___9
                                       | uu___9 -> acc in
                                     let tkey_body1 = t_interp1 in
                                     let tkey_body2 =
                                       add_bv_sorts bvs t_interp1 in
                                     let tkey_body3 =
                                       let uu___9 =
                                         let uu___10 =
                                           let uu___11 =
                                             encode_universe t0_univ in
                                           FStarC_SMTEncoding_Term.mk_Term_type
                                             uu___11 in
                                         (tkey_body2, uu___10) in
                                       FStarC_SMTEncoding_Util.mkAnd uu___9 in
                                     tkey_body3 in
                                   let cvars =
                                     let cvars1 =
                                       let uu___9 =
                                   ...
                                   let t_has_kind =
                                     ...
                                     let uu___9 =
                                       FStarC_SMTEncoding_Term.mk_Term_type u in
                ...
                let uu___5 =
                  let fvs =
                    ...
                    FStarC_Class_Setlike.elems
                      (FStarC_FlatSet.setlike_flat_set
                         FStarC_Syntax_Syntax.ord_bv) uu___6 in
                  let getfreeV t2 =
                    match t2 with
                ...
                | (fstar_fvs, (env1, fv_decls, fv_vars, fv_tms, fv_guards))
                    ...
                    let t_kinding =
                      ...
                      let concl =
                        let uu___6 =
                          FStarC_SMTEncoding_Term.mk_Term_type
                            FStarC_SMTEncoding_Term.mk_U_unknown in
                        FStarC_SMTEncoding_Term.mk_HasType tapp uu___6 in
                      let body =
                        let uu___6 =
                          let uu___7 =
                      ...
                      let svars =
                        let uu___6 =
                          FStarC_SMTEncoding_Term.mkForall
                            t0.FStarC_Syntax_Syntax.pos ([], fv_vars1, body) in
                        FStarC_SMTEncoding_Term.free_variables uu___6 in
                      let pat =
                        if
                          match svars with | hd::tl -> true | uu___6 -> false
                        then concl :: fv_guards1
                        else [concl] in
                      let axiom =
                        FStarC_SMTEncoding_Term.mkForall
                          t0.FStarC_Syntax_Syntax.pos
                          ([pat], (FStarC_List.op_At fv_vars1 svars), body) in
                      FStarC_SMTEncoding_Util.mkAssume
                        (axiom,
                          (FStar_Pervasives_Native.Some
    ...
    | FStarC_Syntax_Syntax.Tm_refine uu___2 ->
        let uu___3 =
          let steps =
            [FStarC_TypeChecker_Env.Weak; FStarC_TypeChecker_Env.HNF] in
          let uu___4 =
                             ...
                             | (fsym, fterm) ->
                                 ...
                                 let cvars2 =
                                   let univ_vars =
                                     let uu___8 =
                                       let uu___9 =
                                         let uu___10 =
                                           FStarC_Syntax_Free.univnames t1 in
                                         let uu___11 =
                                           let uu___12 =
                                             FStarC_Syntax_Util.type_with_u
                                               t0_univ in
                                           FStarC_Syntax_Free.univnames
                                             uu___12 in
                                         FStarC_Class_Setlike.union
                                           (FStarC_FlatSet.setlike_flat_set
                                       ...
                                       FStarC_Class_Setlike.elems
                                         (FStarC_FlatSet.setlike_flat_set
                                            FStarC_Syntax_Syntax.ord_ident)
                                         uu___9 in
                                     FStarC_List.map encode_univ_name uu___8 in
                                   let cvars3 =
                                     let uu___8 =
                                       FStarC_List.map
                                         FStar_Pervasives_Native.fst
                                         univ_vars in
                                     FStarC_List.op_At uu___8 cvars1 in
                                   FStarC_Util.remove_dups
                                     FStarC_SMTEncoding_Term.fv_eq cvars3 in
                                 let xfv =
                                   FStarC_SMTEncoding_Term.mk_fv
                                 ...
                                 ((let uu___9 =
                                  ...
                                  (let tsym =
                                   ...
                                   let cvar_sorts =
                                     FStarC_List.map
                                       FStarC_SMTEncoding_Term.fv_sort cvars2 in
                                   let tdecl =
                                     FStarC_SMTEncoding_Term.DeclFun
                                       (tsym, cvar_sorts,
                                         FStarC_SMTEncoding_Term.Term_sort,
    ...
    | FStarC_Syntax_Syntax.Tm_uvar (uv, uu___2) ->
        let ttm =
          ...
          FStarC_SMTEncoding_Term.mk_Term_uvar uu___3 in
        let uu___3 =
    ...
    | FStarC_Syntax_Syntax.Tm_app uu___2 ->
        let uu___3 = FStarC_Syntax_Util.head_and_args_full t0 in
        (match uu___3 with
         | (head, args_e) ->
             let uu___4 =
               ...
               then
                 ...
                 | FStar_Pervasives_Native.Some t2 ->
                     FStarC_Syntax_Util.head_and_args_full t2
               else (head, args_e) in
             (match uu___4 with
              | (head1, args_e1) ->
                  let uu___5 =
                    ...
                    (uu___6, args_e1) in
                  (match uu___5 with
                   | uu___6 when is_arithmetic_primitive head1 args_e1 ->
                       encode_arith_term env head1 args_e1
                   | uu___6 when is_BitVector_primitive head1 args_e1 ->
                   ...
                   | (FStarC_Syntax_Syntax.Tm_fvar fv, (arg, uu___6)::[])
                       ...
                       then
                         let uu___7 =
                           FStarC_Syntax_Formula.destruct_typ_as_formula arg in
                         match uu___7 with
                         | FStar_Pervasives_Native.Some v -> true
                         | uu___8 -> false
                       else false ->
                       ...
                       encode_term t2 env
                   | (FStarC_Syntax_Syntax.Tm_fvar fv,
                      (_r, uu___6)::(_msg, uu___7)::(phi, uu___8)::[]) when
                       FStarC_Syntax_Syntax.fv_eq_lid fv
                   ...
                   | (FStarC_Syntax_Syntax.Tm_uinst
                      ({
                       ...
                       uu___8),
                      (_r, uu___9)::(_msg, uu___10)::(phi, uu___11)::[]) when
                       FStarC_Syntax_Syntax.fv_eq_lid fv
                         FStarC_Parser_Const.labeled_lid
                       -> encode_term phi env
                   ...
                   | (FStarC_Syntax_Syntax.Tm_uinst
                      ...
                      (v0, uu___9)::(v1, uu___10)::(v2, uu___11)::[]) when
                       FStarC_Syntax_Syntax.fv_eq_lid fv
                         FStarC_Parser_Const.lexcons_lid
                       ->
                            ...
                            (match uu___13 with
                             | (v11, decls1) ->
                                 ...
                                 (match uu___14 with
                                  | (v21, decls2) ->
                                      let uu___15 =
                                        FStarC_SMTEncoding_Util.mk_LexCons
                                          v01 v11 v21 in
                                      (uu___15,
                                        (FStarC_List.op_At decls0
                                           (FStarC_List.op_At decls1 decls2))))))
                   | (FStarC_Syntax_Syntax.Tm_fvar fv,
                      (v0, uu___6)::(v1, uu___7)::(v2, uu___8)::[]) when
                       FStarC_Syntax_Syntax.fv_eq_lid fv
                         FStarC_Parser_Const.lexcons_lid
                       ->
                       let uu___9 = encode_term v0 env in
                       (match uu___9 with
                        | (v01, decls0) ->
                            let uu___10 = encode_term v1 env in
                            (match uu___10 with
                             | (v11, decls1) ->
                                 let uu___11 = encode_term v2 env in
                                 (match uu___11 with
                                  | (v21, decls2) ->
                                      let uu___12 =
                                        FStarC_SMTEncoding_Util.mk_LexCons
                   ...
                   | (FStarC_Syntax_Syntax.Tm_constant
                      (FStarC_Const.Const_reify lopt), uu___6) ->
                       ...
                       fallback ()
                   | (FStarC_Syntax_Syntax.Tm_constant
                      (FStarC_Const.Const_reflect uu___6), (arg, uu___7)::[])
                       -> encode_term arg env
                   | (FStarC_Syntax_Syntax.Tm_fvar fv,
                        ...
                        | (args, decls) ->
                            let encode_partial_app ht_opt =
                              ...
                              | (smt_head, decls') ->
                                  ...
                                  (app_tm, (FStarC_List.op_At decls decls')) in
                            let encode_full_app fv univs =
                              ...
                              | (fname, fuel_args, arity) ->
                                  let arity1 =
                                    arity + (FStarC_List.length univs) in
                                  let all_args =
                                    FStarC_List.op_At fuel_args
                                      (FStarC_List.op_At univs args) in
                                  if (FStarC_List.length all_args) < arity1
                                  then
                                    encode_partial_app
                                      FStar_Pervasives_Native.None
                                  else
                                  ...
                                  { FStarC_Syntax_Syntax.tm = uu___8;
                                    FStarC_Syntax_Syntax.asc =
                                      (FStar_Pervasives.Inl t2, uu___9,
                                       uu___10);
                                    FStarC_Syntax_Syntax.eff_opt = uu___11;_}
                                  -> FStar_Pervasives_Native.Some t2
                              | FStarC_Syntax_Syntax.Tm_ascribed
                                  { FStarC_Syntax_Syntax.tm = uu___8;
                                    FStarC_Syntax_Syntax.asc =
                                 ...
                                 (match uu___8 with
                                  | (head_type2, formals, c) ->
                                      ((let uu___10 =
                                       ...
                                       (let declared_arity_matches l =
                                          match FStarC_SMTEncoding_Env.lookup_fvar_binding
                                                  env l
                                          with
                                        ...
                                        | FStarC_Syntax_Syntax.Tm_uinst
                                            ...
                                            ((FStarC_List.length formals) =
                                               (FStarC_List.length args))
                                              ||
                                              (declared_arity_matches
                                                 fv.FStarC_Syntax_Syntax.fv_name)
                                            ->
                                            let uu___12 =
                                              FStarC_List.map encode_universe
                                                us in
                                            encode_full_app
                                              fv.FStarC_Syntax_Syntax.fv_name
                                              uu___12
    ...
    | FStarC_Syntax_Syntax.Tm_abs uu___2 ->
        let uu___3 =
          FStarC_Syntax_Util.abs_formals_maybe_unascribe_body false t1 in
        (match uu___3 with
         | (bs, body, lopt) ->
             let fallback uu___4 =
               let uu___5 =
                 ...
                 let uu___6 =
                   FStarC_List.map
                     (fun uu___7 -> FStarC_SMTEncoding_Term.Term_sort) fvs in
                 (uu___6, tms) in
               match uu___5 with
               | (arg_sorts, arg_terms) ->
                   let f =
             ...
             (match lopt with
              ...
              | FStar_Pervasives_Native.Some rc ->
                  ...
                  then fallback ()
                    ...
                    (let uu___5 =
                       encode_binders FStar_Pervasives_Native.None bs env in
                     match uu___5 with
                         ...
                         let body1 =
                           ...
                           then
                             ...
                             FStarC_TypeChecker_Util.norm_reify
                               envbody.FStarC_SMTEncoding_Env.tcenv [] uu___8
                           else body in
                         let uu___7 = encode_term body1 envbody in
                         (match uu___7 with
                          | (body2, decls') ->
                              let is_pure =
                              ...
                              let uu___8 =
                                ...
                                | FStar_Pervasives_Native.Some c ->
                                    let tfun = FStarC_Syntax_Util.arrow bs c in
...
and encode_branch_pattern (env : FStarC_SMTEncoding_Env.env_t)
  (scr : FStarC_SMTEncoding_Term.term) (b : FStarC_Syntax_Syntax.branch) :
  (FStarC_SMTEncoding_Term.term * FStarC_Syntax_Syntax.pat *
    FStarC_Syntax_Syntax.term * FStarC_SMTEncoding_Env.env_t *
    FStarC_SMTEncoding_Term.decls_t)=
  let uu___ = FStarC_Syntax_Subst.open_branch b in
  match uu___ with
  | (p, w, br) ->
      let uu___1 = encode_pat env p in
      (match uu___1 with
       | (uu___2, pattern1) ->
           ...
           let env1 =
             FStarC_List.fold_left
               (fun env2 uu___3 ->
                  match uu___3 with
                  | (x, t) -> FStarC_SMTEncoding_Env.push_term_var env2 x t)
               env projections in
           let uu___3 =
             match w with
             | FStar_Pervasives_Native.None -> (guard, [])
             | FStar_Pervasives_Native.Some w1 ->
                 let uu___4 = encode_term w1 env1 in
                 (match uu___4 with
                  | (w2, decls) ->
                      let uu___5 =
                        let uu___6 =
       ...
       | (scr, decls) ->
           let uu___2 =
             let encode_branch b uu___3 =
               match uu___3 with
               | (else_case, decls1) ->
                   let uu___4 = encode_branch_pattern env1 scr' b in
                   (match uu___4 with
                    | (guard, uu___5, br, env2, decls2) ->
                        let uu___6 = encode_br br env2 in
                        (match uu___6 with
                         | (br1, decls3) ->
                             let uu___7 =
                               FStarC_SMTEncoding_Util.mkITE
                                 (guard, br1, else_case) in
                             (uu___7,
...
and encode_formula (phi : FStarC_Syntax_Syntax.typ)
  ...
  let enc f r l =
    ...
    | (decls, args) ->
        let uu___1 =
          let uu___2 = f args in FStarC_SMTEncoding_Term.set_range uu___2 r in
        (uu___1, decls) in
  let const_op f r uu___ = ((FStarC_SMTEncoding_Term.set_range f r), []) in
  ...
  let enc_prop_c f r l =
    ...
    | (decls, phis) ->
        let uu___1 =
          let uu___2 = f phis in FStarC_SMTEncoding_Term.set_range uu___2 r in
        (uu___1, decls) in
  let eq_op r args =
    let rf =
      FStarC_List.filter
    ...
    | (lhs, uu___1)::(rhs, uu___2)::[] ->
        ...
        (match uu___3 with
         | (l1, decls1) ->
             (match l1 with
              | FStarC_SMTEncoding_Term.App
                  (FStarC_SMTEncoding_Term.TrueOp, uu___4, uu___5) ->
                  (l1, decls1)
              | uu___4 ->
                  ...
                  (match uu___5 with
                   | (l2, decls2) ->
                       let uu___6 =
                         let uu___7 = FStarC_SMTEncoding_Term.mkImp (l2, l1) in
                         FStarC_SMTEncoding_Term.set_range uu___7 r in
             ...
             (match uu___5 with
              | (t, decls2) ->
                  ...
                  (match uu___6 with
                   | (e, decls3) ->
                       let res =
                         let uu___7 = FStarC_SMTEncoding_Term.mkITE (g, t, e) in
                         FStarC_SMTEncoding_Term.set_range uu___7 r in
                       (res,
                         (FStarC_List.op_At decls1
  ...
  let rec fallback phi1 =
    ...
    | FStarC_Syntax_Syntax.Tm_meta
        ...
        (match uu___ with
         | (phi2, decls) ->
             ((FStarC_SMTEncoding_Term.Labeled (phi2, msg, r)), decls))
    ...
    | FStarC_Syntax_Syntax.Tm_app uu___ ->
        let uu___1 = FStarC_Syntax_Util.head_and_args_full phi1 in
        (match uu___1 with
         | (head, args) ->
             let uu___2 =
               let uu___3 =
             ...
             (match uu___2 with
              | (FStarC_Syntax_Syntax.Tm_fvar fv,
                 uu___3::(x, uu___4)::(t, uu___5)::[]) when
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.has_type_lid
                  ->
                  let uu___6 = encode_term x env in
                  (match uu___6 with
                   | (x1, decls) ->
                       let uu___7 = encode_term t env in
                       (match uu___7 with
                        | (t1, decls') ->
                            let uu___8 =
                              FStarC_SMTEncoding_Term.mk_HasType x1 t1 in
                            (uu___8, (FStarC_List.op_At decls decls'))))
              | (FStarC_Syntax_Syntax.Tm_fvar fv, uu___3::(phi2, uu___4)::[])
                  when
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.by_tactic_lid
              ...
              | (FStarC_Syntax_Syntax.Tm_uinst
                 ({ FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_fvar fv;
                    ...
                    FStarC_Syntax_Syntax.hash_code = uu___4;_},
                  uu___5),
                 uu___6::uu___7::(phi2, uu___8)::[]) when
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.rewrite_by_tactic_lid
                  -> encode_formula phi2 env
              | (FStarC_Syntax_Syntax.Tm_fvar fv,
                 (r, uu___3)::(msg, uu___4)::(phi2, uu___5)::[]) when
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.labeled_lid
                  ->
                  let uu___6 =
                                ...
                                FStarC_Syntax_Syntax.meta =
                                  (FStarC_Syntax_Syntax.Meta_labeled
                                     ((FStarC_Errors_Msg.mkmsg s),
                                       (phi2.FStarC_Syntax_Syntax.pos),
                                       false))
                              }) phi2.FStarC_Syntax_Syntax.pos in
                       fallback phi3
                   | uu___7 -> fallback phi2)
              | (FStarC_Syntax_Syntax.Tm_fvar fv, (t, uu___3)::[]) when
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.squash_lid
                  -> encode_formula t env
              | uu___3 ->
                  let encode_valid uu___4 =
                    debug phi1;
                    (let uu___6 = encode_term phi1 env in
                     match uu___6 with
                     | (tt, decls) ->
                         let uu___7 =
                           FStarC_SMTEncoding_Term.mk_valid_at
                             phi1.FStarC_Syntax_Syntax.pos tt in
                         (uu___7, decls)) in
                  let uu___4 = head_redex env head in
                  if uu___4
                  then
                    let uu___5 = maybe_whnf env head in
                    (match uu___5 with
             ...
             (match uu___3 with
              | (body1, decls'') ->
                  let guards1 =
                    match pats with
                    | ((FStarC_SMTEncoding_Term.App
                        (FStarC_SMTEncoding_Term.Var gf, p::[], uu___4))::[])::[]
                        when
                        (FStarC_Ident.string_of_lid
