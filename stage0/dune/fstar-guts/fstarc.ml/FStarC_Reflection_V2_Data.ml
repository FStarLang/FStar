...
let (uu___is_C_Char : vconst -> Prims.bool) =
  fun projectee -> match projectee with | C_Char _0 -> true | uu___ -> false
let (__proj__C_Char__item___0 : vconst -> FStar_Char.char) =
  fun projectee -> match projectee with | C_Char _0 -> _0
...
let (__proj__Pat_Var__item__sort :
  pattern -> FStarC_Syntax_Syntax.term FStarC_Sealed.sealed) =
...
type namedv_view =
  ...
  ppname: ppname_t }
let (__proj__Mknamedv_view__item__uniq : namedv_view -> Prims.int) =
  fun projectee -> match projectee with | { uniq; sort; ppname;_} -> uniq
let (__proj__Mknamedv_view__item__sort :
  namedv_view -> typ FStarC_Sealed.sealed) =
...
type bv_view =
  ...
  ppname1: ppname_t }
let (__proj__Mkbv_view__item__index : bv_view -> Prims.int) =
  fun projectee ->
    match projectee with
    | { index; sort1 = sort; ppname1 = ppname;_} -> index
let (__proj__Mkbv_view__item__sort : bv_view -> typ FStarC_Sealed.sealed) =
  fun projectee ->
...
type binding = {
  ...
  sort3: typ ;
  ppname3: ppname_t }
let (__proj__Mkbinding__item__uniq : binding -> Prims.int) =
  fun projectee ->
    match projectee with
