...
let should_unfold (allow_strict : Prims.bool)
  (cfg : FStarC_TypeChecker_Cfg.cfg)
  ...
  let res =
    ...
    | uu___ when FStarC_TypeChecker_Env.qninfo_is_action qninfo ->
    ...
    | uu___ when
        ...
        (FStarC_TypeChecker_Cfg.log_unfolding cfg
         ...
         no)
    | uu___ when
        let uu___1 = FStarC_TypeChecker_Cfg.find_prim_step cfg fv in
        match uu___1 with
        | FStar_Pervasives_Native.Some v -> true
        | uu___2 -> false ->
        (FStarC_TypeChecker_Cfg.log_unfolding cfg
                        ...
                        let uu___8 =
                          FStarC_Util.for_some
                            (fun at ->
                               FStarC_Util.for_some
                                 (fun lid ->
                                    FStarC_Syntax_Util.is_fvar lid at) lids)
                            attrs in
                        yesno uu___8 in
                  let uu___8 =
                    let uu___9 =
