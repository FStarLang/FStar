...
let (__proj__U_unif__item___0 :
  universe ->
    (universe FStar_Pervasives_Native.option FStarC_Unionfind.p_uvar *
      version * FStarC_Range_Type.range))
...
let (__proj__SomeUseRange__item___0 :
  maybe_set_use_range -> FStarC_Range_Type.range) =
...
let (__proj__Mkctx_uvar__item__ctx_uvar_range :
  ctx_uvar -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
    | { ctx_uvar_head; ctx_uvar_gamma; ctx_uvar_binders; ctx_uvar_reason;
...
let (__proj__Mkletbinding__item__lbpos :
  letbinding -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
...
let __proj__Mksyntax__item__pos : 'a . 'a syntax -> FStarC_Range_Type.range =
...
let (__proj__Mklazyinfo__item__rng : lazyinfo -> FStarC_Range_Type.range) =
...
let (deq_qualifier : qualifier FStarC_Class_Deq.deq) =
  {
    FStarC_Class_Deq.op_Equals_Question =
      (fun q1 ->
         fun q2 -> let uu___ = cmp_qualifier q1 q2 in uu___ = FStarC_Order.Eq)
  }
...
let (__proj__Mksigelt'__Sig_fail__payload__item__rng :
  sigelt'__Sig_fail__payload -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
...
let (__proj__Mksigelt__item__sigel : sigelt -> sigelt') =
  fun projectee ->
    ...
    | { sigel; sigrng; sigquals; sigmeta; sigattrs; sigopens_and_abbrevs;
        sigopts;_} -> sigel
let (__proj__Mksigelt__item__sigrng : sigelt -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
    | { sigel; sigrng; sigquals; sigmeta; sigattrs; sigopens_and_abbrevs;
        sigopts;_} -> sigrng
...
let (order_fv : FStarC_Ident.lident -> FStarC_Ident.lident -> Prims.int) =
  fun x ->
    fun y ->
      ...
      FStarC_String.compare uu___ uu___1
let (range_of_lbname : lbname -> FStarC_Range_Type.range) =
  fun l ->
    match l with
...
let deq_instance_from_cmp :
  'uuuuu .
    ('uuuuu -> 'uuuuu -> FStarC_Order.order) -> 'uuuuu FStarC_Class_Deq.deq
  =
  fun f ->
    {
      FStarC_Class_Deq.op_Equals_Question =
        (fun x -> fun y -> let uu___ = f x y in FStarC_Order.eq uu___)
    }
let ord_instance_from_cmp :
  'uuuuu .
    ('uuuuu -> 'uuuuu -> FStarC_Order.order) -> 'uuuuu FStarC_Class_Ord.ord
  =
  fun f ->
    {
...
let (deq_bv : bv FStarC_Class_Deq.deq) =
  deq_instance_from_cmp
    (fun x ->
       fun y -> let uu___ = order_bv x y in FStarC_Order.order_from_int uu___)
...
let (binders_to_names : binders -> term Prims.list) =
  fun bs -> FStarC_List.map (fun b -> bv_to_name b.binder_bv) bs
let (mk_Tm_app : term -> args -> FStarC_Range_Type.range -> term) =
  fun t1 ->
    fun args1 ->
      fun p ->
        match args1 with
...
let (extend_app_n : term -> args -> FStarC_Range_Type.range -> term) =
  fun t ->
    fun args' ->
      fun r ->
        match t.n with
...
let (extend_app : term -> arg -> FStarC_Range_Type.range -> term) =
  fun t -> fun arg1 -> fun r -> extend_app_n t [arg1] r
let (mk_Tm_delayed : (term * subst_ts) -> FStarC_Range_Type.range -> term) =
  fun lr ->
    fun pos ->
      mk
        (Tm_delayed
...
let (mk_lb :
  (lbname * univ_name Prims.list * FStarC_Ident.lident * typ * term *
    attribute Prims.list * FStarC_Range_Type.range) -> letbinding)
  =
  fun uu___ ->
    match uu___ with
...
let (extend_subst : subst_elt -> subst_elt Prims.list -> subst_t) =
  fun x -> fun s -> x :: s
let (argpos : arg -> FStarC_Range_Type.range) =
...
let (gen_bv' :
  FStarC_Ident.ident ->
    FStarC_Range_Type.range FStar_Pervasives_Native.option -> typ -> bv)
  =
  fun id ->
    fun r ->
      fun t ->
...
let (binders_of_list : bv Prims.list -> binders) =
  fun fvs -> FStarC_List.map (fun t -> mk_binder t) fvs
...
let (new_univ_name :
  FStarC_Range_Type.range FStar_Pervasives_Native.option -> univ_name) =
  fun ropt ->
...
let (t_erased_of : term -> term) =
  fun t ->
    ...
    let uu___1 = let uu___2 = as_arg t in [uu___2] in
    mk_Tm_app uu___ uu___1 FStarC_Range_Type.dummyRange
let (unit_const_with_range : FStarC_Range_Type.range -> term) =
