...
type interactive_state =
  {
  chunk: FStarC_StringBuffer.t ;
  stdin:
    FStarC_Util.stream_reader FStar_Pervasives_Native.option
      FStarC_Effect.ref
    ;
  buffer: input_chunks Prims.list FStarC_Effect.ref ;
  log:
    FStarC_Util.out_channel FStar_Pervasives_Native.option FStarC_Effect.ref }
let (__proj__Mkinteractive_state__item__chunk :
  interactive_state -> FStarC_StringBuffer.t) =
  fun projectee ->
...
let (__proj__Mkinteractive_state__item__buffer :
  interactive_state -> input_chunks Prims.list FStarC_Effect.ref) =
  fun projectee ->
...
let (the_interactive_state : interactive_state) =
  let uu___ = FStarC_StringBuffer.create (Prims.of_int (100)) in
...
let rec (read_chunk : unit -> input_chunks) =
  fun uu___ ->
    ...
    let log =
      ...
      then
        let transcript =
          let uu___2 = FStarC_Effect.op_Bang s.log in
          match uu___2 with
          | FStar_Pervasives_Native.Some transcript1 -> transcript1
          | FStar_Pervasives_Native.None ->
              let transcript1 =
                FStarC_Util.open_file_for_writing "transcript" in
              (FStarC_Effect.op_Colon_Equals s.log
                 (FStar_Pervasives_Native.Some transcript1);
               transcript1) in
        fun line ->
          (FStarC_Util.append_to_file transcript line;
           FStarC_Util.flush transcript)
      else (fun uu___3 -> ()) in
    let stdin =
      let uu___1 = FStarC_Effect.op_Bang s.stdin in
      match uu___1 with
      | FStar_Pervasives_Native.Some i -> i
      | FStar_Pervasives_Native.None ->
          let i = FStarC_Util.open_stdin () in
          (FStarC_Effect.op_Colon_Equals s.stdin
             (FStar_Pervasives_Native.Some i);
           i) in
    let line =
      let uu___1 = FStarC_Util.read_line stdin in
      match uu___1 with
      | FStar_Pervasives_Native.None -> FStarC_Effect.exit Prims.int_zero
      | FStar_Pervasives_Native.Some l -> l in
    log line;
    (let l = FStarC_Util.trim_string line in
     if FStarC_Util.starts_with l "#end"
     then
       let responses =
         match FStarC_Util.split l " " with
         | uu___2::ok::fail::[] -> (ok, fail)
         | uu___2 -> ("ok", "fail") in
       let str = FStarC_StringBuffer.contents s.chunk in
       ((let uu___3 = FStarC_StringBuffer.clear s.chunk in ());
        Code (str, responses))
     else
       if FStarC_Util.starts_with l "#pop"
       then ((let uu___4 = FStarC_StringBuffer.clear s.chunk in ()); Pop l)
       else
         if FStarC_Util.starts_with l "#push"
         then
           ((let uu___5 = FStarC_StringBuffer.clear s.chunk in ());
            (let lc_lax =
               let uu___5 =
                 FStarC_Util.substring_from l (FStarC_String.length "#push") in
               FStarC_Util.trim_string uu___5 in
             let lc =
               match FStarC_Util.split lc_lax " " with
               | l1::c::"#lax"::[] ->
                   let uu___5 = FStarC_Util.int_of_string l1 in
                   let uu___6 = FStarC_Util.int_of_string c in
                   (true, uu___5, uu___6)
               | l1::c::[] ->
                   let uu___5 = FStarC_Util.int_of_string l1 in
                   let uu___6 = FStarC_Util.int_of_string c in
           ...
           if FStarC_Util.starts_with l "#info "
           then
             (match FStarC_Util.split l " " with
              | uu___5::symbol::[] ->
                  ((let uu___7 = FStarC_StringBuffer.clear s.chunk in ());
              ...
              | uu___5::symbol::file::row::col::[] ->
                  ((let uu___7 = FStarC_StringBuffer.clear s.chunk in ());
                   (let uu___7 =
                      let uu___8 =
                        let uu___9 =
                          let uu___10 = FStarC_Util.int_of_string row in
                          let uu___11 = FStarC_Util.int_of_string col in
             ...
             then
               (match FStarC_Util.split l " " with
                | uu___6::prefix::"#"::[] ->
                    ((let uu___8 = FStarC_StringBuffer.clear s.chunk in ());
                     Completions prefix)
                | uu___6 ->
                    (FStarC_Errors.log_issue0
                       FStarC_Errors_Codes.Error_IDEUnrecognized ()
               ...
               if l = "#finish"
               then FStarC_Effect.exit Prims.int_zero
               else
                 ((let uu___9 =
                     let uu___10 = FStarC_StringBuffer.add line s.chunk in
                     FStarC_StringBuffer.add "\n" uu___10 in
                   ());
                  read_chunk ()))
let (shift_chunk : unit -> input_chunks) =
  fun uu___ ->
    let s = the_interactive_state in
    let uu___1 = FStarC_Effect.op_Bang s.buffer in
    match uu___1 with
    | [] -> read_chunk ()
    | chunk::chunks -> (FStarC_Effect.op_Colon_Equals s.buffer chunks; chunk)
let (fill_buffer : unit -> unit) =
  fun uu___ ->
    let s = the_interactive_state in
    let uu___1 =
      let uu___2 = FStarC_Effect.op_Bang s.buffer in
      let uu___3 = let uu___4 = read_chunk () in [uu___4] in
      FStarC_List.op_At uu___2 uu___3 in
    FStarC_Effect.op_Colon_Equals s.buffer uu___1
        ...
        (match uu___1 with
         | (deps1, same_name) ->
             let maybe_intf =
               ...
               | uu___2 ->
                   ((let uu___4 =
                       FStarC_Format.fmt1 "Unexpected: ended up with %s"
                         (FStarC_String.concat " " same_name) in
                     FStarC_Errors.log_issue0
...
type m_timestamps =
  (Prims.string FStar_Pervasives_Native.option * Prims.string *
    FStarC_Time.time_of_day FStar_Pervasives_Native.option *
    FStarC_Time.time_of_day) Prims.list
let rec (tc_deps :
  modul_t ->
    stack_t ->
      FStarC_TypeChecker_Env.env ->
                ...
                fun intf_t ->
                  fun impl_t ->
                    let impl_mt =
                      FStarC_Parser_ParseIt.get_file_last_modification_time
                        impl in
                    (FStarC_Time.is_before impl_t impl_mt) ||
                       ...
                       | (FStar_Pervasives_Native.Some intf1,
                          FStar_Pervasives_Native.Some intf_t1) ->
                           let intf_mt =
                             FStarC_Parser_ParseIt.get_file_last_modification_time
                               intf1 in
                           FStarC_Time.is_before intf_t1 intf_mt
                       | (FStar_Pervasives_Native.None,
                          FStar_Pervasives_Native.None) -> false
                       | (uu___, uu___1) ->
                           failwith
                             "Impossible, if the interface is None, the timestamp entry should also be None") in
            let rec iterate depnames =
              fun st ->
                fun env' ->
                  fun ts1 ->
                    fun good_stack ->
                      fun good_ts ->
                        let match_dep depnames1 =
                          fun intf ->
                            fun impl ->
                              match intf with
                              | FStar_Pervasives_Native.None ->
                          ...
                          fun stack ->
                            fun ts2 ->
                              ...
                              | uu___::ts3 ->
                                  (pop env1 "";
                                   (let uu___2 =
                                      let uu___3 = FStarC_List.hd stack in
                                      let uu___4 = FStarC_List.tl stack in
                                      (uu___3, uu___4) in
                                    match uu___2 with
                                    | ((env2, uu___3), stack1) ->
                                      ...
                                      then
                                        let env1 =
                                          pop_tc_and_stack env'
                                            (FStarC_List.rev_append st [])
                                            ts1 in
                                        tc_deps m good_stack env1 depnames
                                          good_ts
                                      else
                                        (let uu___4 =
                                           let uu___5 = FStarC_List.hd st in
                                           let uu___6 = FStarC_List.tl st in
                                           (uu___5, uu___6) in
                                         match uu___4 with
                                         | (stack_elt, st') ->
                                             iterate depnames' st' env' ts'
                                               (stack_elt :: good_stack)
                                               (ts_elt :: good_ts))))
                        | [] -> tc_deps m good_stack env' depnames good_ts in
            let uu___ = deps_of_our_file filename in
            match uu___ with
            | (filenames, uu___1, dep_graph) ->
                iterate filenames (FStarC_List.rev_append stk []) env
                  (FStarC_List.rev_append ts []) [] []
let (format_info :
  FStarC_TypeChecker_Env.env ->
    Prims.string ->
  ...
  fun env ->
    fun name ->
      fun typ ->
        fun range ->
          fun doc ->
            let uu___ = FStarC_Range_Ops.string_of_range range in
            let uu___1 = FStarC_TypeChecker_Normalize.term_to_string env typ in
            let uu___2 =
              match doc with
              | FStar_Pervasives_Native.Some docstring ->
                  FStarC_Format.fmt1 "#doc %s" docstring
              | FStar_Pervasives_Native.None -> "" in
            FStarC_Format.fmt4 "(defined at %s) %s: %s%s" uu___ name uu___1
              uu___2
let rec (go :
  (Prims.int * Prims.int) ->
          ...
          fun env ->
            fun ts ->
              ...
              | Completions search_term ->
                  let rec measure_anchored_match search_term1 =
                    fun candidate ->
                      ...
                      | (hs::ts1, hc::tc) ->
                          ...
                          if FStarC_Util.starts_with hc_text hs
                          then
                            (match ts1 with
                             | [] ->
                                 FStar_Pervasives_Native.Some
                                   (candidate, (FStarC_String.length hs))
                             | uu___1 ->
                                 ...
                                 FStarC_Option.map
                                   (fun uu___3 ->
                                      ...
                                      | (matched, len) ->
                                          ((hc :: matched),
                                            (((FStarC_String.length hc_text)
                                                + Prims.int_one)
                                               + len))) uu___2)
                          else FStar_Pervasives_Native.None in
                  let rec locate_match needle =
                    fun candidate ->
                  ...
                  let str_of_ids ids =
                    let uu___1 =
                      FStarC_List.map FStarC_Ident.string_of_id ids in
                    FStarC_Util.concat_l "." uu___1 in
                  let match_lident_against needle =
                    fun lident ->
                      let uu___1 =
                  ...
                  let matches =
                        ...
                        fun id ->
                          ...
                          let matched_length =
                            FStarC_List.fold_left
                              (fun out ->
                                 fun s ->
                                   ((FStarC_String.length s) + out) +
                                     Prims.int_one) (FStarC_String.length id)
                              orig_ns in
                          FStarC_List.filter_map
                            (fun n ->
                               if FStarC_Util.starts_with n id
                               then
                                 let lid =
                                   let uu___1 = FStarC_Ident.ids_of_lid m in
                                   let uu___2 = FStarC_Ident.id_of_text n in
                                   FStarC_Ident.lid_of_ns_and_id uu___1
                                     uu___2 in
                                 let uu___1 =
                                   FStarC_Syntax_DsEnv.resolve_to_fully_qualified_name
                                     env.FStarC_TypeChecker_Env.dsenv lid in
                    ...
                    let case_b_find_matches_in_env uu___1 =
                      let matches1 =
                        FStarC_List.filter_map (match_lident_against needle)
                                ...
                                | FStar_Pervasives_Native.Some l ->
                                    let uu___5 =
                                      FStarC_Ident.lid_of_ids
                                        (FStarC_List.op_At ns id) in
                                    FStarC_Ident.lid_equals l uu___5))
                        matches1 in
                    let uu___1 = FStarC_Util.prefix needle in
                    match uu___1 with
                    | (ns, id) ->
                        let matched_ids =
                          match ns with
                        ...
                        (fun uu___3 ->
                           fun uu___4 ->
                             ...
                             | ((cd1, ns1, uu___5), (cd2, ns2, uu___6)) ->
                                 (match FStarC_String.compare cd1 cd2 with
                                  | uu___7 when uu___7 = Prims.int_zero ->
                                      FStarC_String.compare ns1 ns2
                                  | n -> n)) matches in
                    FStarC_List.iter
                      (fun uu___3 ->
                         match uu___3 with
                         | (candidate, ns, match_len) ->
                             let uu___4 =
                               FStarC_Class_Show.show
                                 FStarC_Class_Show.showable_int match_len in
                             FStarC_Format.print3 "%s %s %s \n" uu___4 ns
                               candidate) uu___2);
                   FStarC_Format.print_string "#done-ok\n";
                   go line_col filename stack curmod env ts)
              | Pop msg ->
                  (pop env msg;
                   (let uu___2 =
              ...
              | Push (lax, l, c) ->
                  let uu___1 =
                    if (FStarC_List.length stack) = (FStarC_List.length ts)
                    then
     ...
     | (filenames, maybe_intf, dep_graph) ->
         ...
         (match uu___2 with
          | (stack, env1, ts) ->
              let initial_range =
                let uu___3 =
                  FStarC_Range_Type.mk_pos Prims.int_one Prims.int_zero in
                let uu___4 =
                  FStarC_Range_Type.mk_pos Prims.int_one Prims.int_zero in
                FStarC_Range_Type.mk_range filename uu___3 uu___4 in
