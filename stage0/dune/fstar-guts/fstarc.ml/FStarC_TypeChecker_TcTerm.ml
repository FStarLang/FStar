...
let is_squash_typ (env : FStarC_TypeChecker_Env.env)
  (t : FStarC_Syntax_Syntax.typ) : Prims.bool=
  let uu___ =
    let uu___1 = FStarC_Syntax_Util.un_squash t in
    match uu___1 with
  ...
  then true
    ...
    (let uu___1 =
       let uu___2 = FStarC_TypeChecker_Normalize.unfold_whnf env t in
       FStarC_Syntax_Util.un_squash uu___2 in
     match uu___1 with
     | FStar_Pervasives_Native.Some v -> true
     | uu___2 -> false)
let is_eq
...
let flex_head_uvar (t : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.ctx_uvar FStar_Pervasives_Native.option=
  let uu___ =
    let uu___1 =
      let uu___2 =
        let uu___3 = FStarC_Syntax_Util.unmeta t in
        FStarC_Syntax_Util.leftmost_head uu___3 in
...
let bound_of_flex (require_ground : Prims.bool)
  (ds : FStarC_TypeChecker_Common.deferred) (t : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term FStar_Pervasives_Native.option=
  let uu___ = flex_head_uvar t in
  match uu___ with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some u ->
      let headed_by t1 =
        let uu___1 = flex_head_uvar t1 in
        match uu___1 with
        | FStar_Pervasives_Native.Some u' ->
        ...
        | (uu___2, uu___3, p) ->
            (match p with
             | FStarC_TypeChecker_Common.TProb p1 ->
                 if
                   p1.FStarC_TypeChecker_Common.relation =
                     FStarC_TypeChecker_Common.EQ
                 then FStar_Pervasives_Native.None
                 else
                   (let uu___4 =
                      let uu___5 = headed_by p1.FStarC_TypeChecker_Common.rhs in
                      if uu___5
                      then ground p1.FStarC_TypeChecker_Common.lhs
                      else false in
                    if uu___4
                    then
                      FStar_Pervasives_Native.Some
                        (p1.FStarC_TypeChecker_Common.lhs)
                    else
                      (let uu___5 =
                         let uu___6 =
                           headed_by p1.FStarC_TypeChecker_Common.lhs in
                         if uu___6
                         then ground p1.FStarC_TypeChecker_Common.rhs
                         else false in
                       if uu___5
                       then
                         FStar_Pervasives_Native.Some
                           (p1.FStarC_TypeChecker_Common.rhs)
                       else FStar_Pervasives_Native.None))
             | FStarC_TypeChecker_Common.CProb uu___4 ->
                 FStar_Pervasives_Native.None) in
      let uu___1 =
        FStarC_Class_Listlike.to_list (FStarC_CList.listlike_clist ()) ds in
      FStarC_List.tryPick bound uu___1
let check_expected_aqual_for_binder (aq : FStarC_Syntax_Syntax.aqual)
  ...
  let is_imp a =
    ...
    | uu___ -> false in
  if (is_imp aq) <> (is_imp expected_aq)
  then
    (let msg =
       [FStarC_Errors_Msg.text "Inconsistent argument qualifiers.";
       FStarC_Errors_Msg.text
         (FStarC_Format.fmt2
            "Expected an %splicit argument, got an %splicit one."
            (if is_imp aq then "im" else "ex")
            (if is_imp expected_aq then "im" else "ex"))] in
...
let check_erasable_binder_attributes (env : FStarC_TypeChecker_Env.env)
  ...
  FStarC_List.iter
    (fun attr ->
       let uu___ =
         let uu___1 =
           FStarC_Syntax_Util.is_fvar FStarC_Parser_Const.erasable_attr attr in
         if uu___1
         then
           let uu___2 =
       ...
       then
         FStarC_Errors.raise_error (FStarC_Syntax_Syntax.has_range_syntax ())
           attr FStarC_Errors_Codes.Fatal_QualifierListNotPermitted ()
...
let fv_is_machine_generated (fv : FStarC_Syntax_Syntax.fv) : Prims.bool=
  (FStarC_Range_Ops.file_of_range
     (FStarC_Ident.range_of_lid fv.FStarC_Syntax_Syntax.fv_name))
    = "dummy"
let maybe_warn_on_use (env : FStarC_TypeChecker_Env.env)
  ...
  | FStar_Pervasives_Native.Some attrs ->
      FStarC_List.iter
        (fun a ->
           let uu___1 = FStarC_Syntax_Util.head_and_args_full a in
           match uu___1 with
           | (head, args) ->
               let msg_arg m =
                 match args with
                 | ({
                    ...
                    uu___5)::[] ->
                     FStarC_List.op_At m [FStarC_Errors_Msg.text s]
                 | uu___2 -> m in
               (match head.FStarC_Syntax_Syntax.n with
                ...
                | FStarC_Syntax_Syntax.Tm_fvar attr_fv when
                    ...
                    let m =
                      FStarC_Errors_Msg.text
                        (FStarC_Format.fmt1 "%s is deprecated"
                           (FStarC_Ident.string_of_lid
                              fv.FStarC_Syntax_Syntax.fv_name)) in
                    FStarC_Errors.log_issue FStarC_Ident.hasrange_lident
                      fv.FStarC_Syntax_Syntax.fv_name
...
let value_check_expected_typ (env : FStarC_TypeChecker_Env.env)
  ...
  (let lc =
   ...
   let uu___1 =
     ...
     | FStar_Pervasives_Native.None -> (e, lc, guard)
         ...
         (match uu___2 with
          | (e1, lc1, g) ->
              ((let uu___4 = FStarC_Debug.medium () in
                ...
                then
                  ...
                  let uu___6 =
                    FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                      t' in
                  let uu___7 = FStarC_TypeChecker_Rel.guard_to_string env g in
                  let uu___8 =
               ...
               (let t1 = FStarC_Syntax_Util.comp_result lc1 in
                let g1 =
                  FStarC_Class_Monoid.op_Plus_Plus
                    FStarC_TypeChecker_Common.monoid_guard_t g guard in
                let msg =
                  if FStarC_TypeChecker_Env.is_trivial_guard_formula g1
                  then FStar_Pervasives_Native.None
                  else
                    (let uu___4 =
                       let uu___5 = FStarC_Syntax_Util.is_unit t1 in
                       if uu___5
                       then
                         let uu___6 = FStarC_Syntax_Util.un_squash t' in
                         match uu___6 with
                         | FStar_Pervasives_Native.Some v -> true
                         | uu___7 -> false
                       else false in
                ...
                let g2 =
                  FStarC_TypeChecker_Util.simplify_and_label_guard msg env g1 in
                let lc2 =
                  let uu___4 =
                    FStarC_TypeChecker_Util.keep_res_typ env t'
                      (FStarC_Syntax_Util.comp_result lc1) in
                  if uu___4
                  then lc1
                  else FStarC_Syntax_Util.set_result_typ lc1 t' in
                (e1, lc2, g2)))) in
...
let comp_check_expected_typ (env : FStarC_TypeChecker_Env.env)
  ...
  | FStar_Pervasives_Native.Some (t, use_eq) ->
      let uu___ = FStarC_TypeChecker_Util.maybe_coerce_lc env e lc t in
      (match uu___ with
       | (e1, lc1, g_c) ->
           let uu___1 =
             FStarC_TypeChecker_Util.weaken_result_typ env e1
               (lc1,
                 (FStarC_Class_Monoid.mzero
                    FStarC_TypeChecker_Common.monoid_guard_t)) t use_eq in
           (match uu___1 with
            | (e2, lc2, g) ->
                let uu___2 =
                  FStarC_Class_Monoid.op_Plus_Plus
...
let check_expected_effect (env : FStarC_TypeChecker_Env.env)
  ...
  | (e, c) ->
      let tot_or_gtot c1 =
        ...
        if uu___1
        then
          FStarC_Syntax_Syntax.mk_Total (FStarC_Syntax_Util.comp_result c1)
        else
          (let uu___2 = FStarC_Syntax_Util.is_pure_or_ghost_comp c1 in
           if uu___2
           then
             FStarC_Syntax_Syntax.mk_GTotal
               (FStarC_Syntax_Util.comp_result c1)
           else
             FStarC_Effect.failwith "Impossible: Expected pure_or_ghost comp") in
      let uu___1 =
            ...
            then
              ...
              (let uu___3 = FStarC_Syntax_Util.is_pure_or_ghost_comp c in
               ...
               then
                 ...
                 (uu___4, c, FStar_Pervasives_Native.None)
               else
                 (let uu___4 =
                  ...
                  then
                    ...
                    match uu___5 with
                    | (ct1, uu___6, g) ->
                        let uu___7 = FStarC_Syntax_Syntax.mk_Comp ct1 in
                   ...
                   (let c3 =
                      ...
                      then c2
                        ...
                        FStarC_TypeChecker_Util.maybe_assume_result_eq_pure_term
                          env e c2 in
                    let uu___5 = (c3, FStarC_TypeChecker_Env.trivial_guard) in
                    match uu___5 with
                    | (c4, g_c) ->
                        (FStarC_Defensive.def_check_scoped
                           FStarC_TypeChecker_Env.hasBinders_env
                              ...
                              ((let uu___11 = FStarC_Debug.medium () in
                               ...
                               (let e2 =
                                  FStarC_TypeChecker_Util.maybe_lift env e1
                                    (FStarC_Syntax_Util.comp_effect_name c4)
...
let no_logical_guard (env : FStarC_TypeChecker_Env.env)
  (x : ('uuuuu * 'uuuuu1 * FStarC_TypeChecker_Env.guard_t)) :
  ('uuuuu * 'uuuuu1 * FStarC_TypeChecker_Env.guard_t)=
  let uu___ = x in
  match uu___ with
  | (te, kt, f) ->
      (match FStarC_TypeChecker_Env.guard_form f with
...
let print_expected_ty_str (env : FStarC_TypeChecker_Env.env) : Prims.string=
  ...
  | FStar_Pervasives_Native.Some (t, use_eq) ->
      ...
      FStarC_Format.fmt2 "Expected type is (%s, use_eq = %s)" uu___ uu___1
let print_expected_ty (env : FStarC_TypeChecker_Env.env) : unit=
  let uu___ = print_expected_ty_str env in FStarC_Format.print1 "%s\n" uu___
let rec get_pat_vars' (all : FStarC_Syntax_Syntax.bv Prims.list)
  (andlist : Prims.bool) (pats : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.bv FStarC_FlatSet.t=
  let pats1 = FStarC_Syntax_Util.unmeta pats in
  let uu___ = FStarC_Syntax_Util.head_and_args_full pats1 in
  match uu___ with
  | (head, args) ->
      let uu___1 =
        let uu___2 =
      ...
      (match uu___1 with
       ...
       | (FStarC_Syntax_Syntax.Tm_fvar fv,
          (uu___2, FStar_Pervasives_Native.Some
           ...
           -> FStarC_Syntax_Free.names pat
       | (FStarC_Syntax_Syntax.Tm_fvar fv,
          (subpats, FStar_Pervasives_Native.None)::[]) when
           FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.smtpatOr_lid
...
let check_no_smt_theory_symbols (en : FStarC_TypeChecker_Env.env)
  ...
  let rec pat_terms t1 =
    ...
    let uu___ = FStarC_Syntax_Util.head_and_args_full t2 in
    match uu___ with
    | (head, args) ->
        let uu___1 =
          let uu___2 =
  ...
  let rec aux t1 =
    ...
    | FStarC_Syntax_Syntax.Tm_app uu___1 ->
        ...
        (match uu___2 with
         | (hd, args) ->
             let ts =
               let uu___3 = aux hd in
               let uu___4 =
                 FStarC_List.map
                   (fun uu___5 -> match uu___5 with | (t2, uu___6) -> aux t2)
                   args in
               uu___3 :: uu___4 in
  ...
  if match tlist with | [] -> true | uu___ -> false
  then ()
  else
    (let uu___ =
       let uu___1 =
         ...
         FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
           (FStarC_Errors_Msg.text
              "Pattern uses these theory symbols or terms that should not be in an SMT pattern:")
           uu___2 in
       [uu___1] in
...
let check_smt_pat (env : FStarC_TypeChecker_Env.env)
  (t : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax) : unit=
  ...
  then
    ...
    match uu___1 with
    | (bs, c) ->
        let pats_opt = FStarC_Syntax_Util.comp_smt_pats c in
        let used = (FStarC_Syntax_Util.comp_result c) ::
          (match pats_opt with
           | FStar_Pervasives_Native.Some p -> [p]
           | FStar_Pervasives_Native.None -> []) in
        let uu___2 = FStarC_Syntax_Util.split_squash_binders used bs in
        (match uu___2 with
         | (bs1, _pre) ->
             (match pats_opt with
              | FStar_Pervasives_Native.Some pats ->
                  (check_pat_fvs t.FStarC_Syntax_Syntax.pos env pats bs1;
...
let guard_letrecs (env : FStarC_TypeChecker_Env.env)
  ...
  | letrecs ->
      ...
      let is_precondition_binder env2 b =
        if
          (match b.FStarC_Syntax_Syntax.binder_qual with
           ...
           | uu___ -> false) &&
            (match match b.FStarC_Syntax_Syntax.binder_qual with
             ...
             with
             | FStarC_Syntax_Syntax.Implicit _0 -> true
             | uu___ -> false)
        then
          is_squash_typ env2
      ...
      let decreases_clause bs c =
        ...
        (let filter_types_and_functions bs1 =
         ...
         let rec spec_flags env2 c1 =
           let fl = FStarC_Syntax_Util.comp_flags c1 in
           let uu___1 =
             let uu___2 =
           ...
           then fl
             ...
             (let uu___2 =
              ...
              | FStarC_Syntax_Syntax.Tm_arrow
                  ...
                  when is_precondition_binder env2 b' ->
                  let uu___3 = FStarC_Syntax_Subst.open_comp [b'] c' in
                  (match uu___3 with
                   | (bs', c'1) ->
                       let uu___4 =
                         FStarC_TypeChecker_Env.push_binders env2 bs' in
                       spec_flags uu___4 c'1)
              | uu___3 -> fl) in
         let cflags =
           let uu___1 = FStarC_TypeChecker_Env.push_binders env1 bs in
           spec_flags uu___1 c in
         let uu___1 =
           FStarC_List.tryFind
             (fun uu___2 ->
                match uu___2 with
        ...
        let rec aux l1 l_prev1 =
          let type_of should_warn e1 e2 =
            ...
            let rec warn t11 t21 =
              ...
              then false
                ...
                (let uu___1 =
                   let uu___2 =
                     let uu___3 = FStarC_Syntax_Subst.compress t11 in
                     uu___3.FStarC_Syntax_Syntax.n in
                   let uu___3 =
                     let uu___4 = FStarC_Syntax_Subst.compress t21 in
                     uu___4.FStarC_Syntax_Syntax.n in
                   (uu___2, uu___3) in
                 match uu___1 with
                 | (FStarC_Syntax_Syntax.Tm_uinst (t12, uu___2),
                    FStarC_Syntax_Syntax.Tm_uinst (t22, uu___3)) ->
                     warn t12 t22
                 ...
                 | (FStarC_Syntax_Syntax.Tm_app uu___2,
                    FStarC_Syntax_Syntax.Tm_app uu___3) ->
                     let uu___4 = FStarC_Syntax_Util.head_and_args_full t11 in
                     (match uu___4 with
                      | (h1, args1) ->
                          let uu___5 =
                            FStarC_Syntax_Util.head_and_args_full t21 in
                          (match uu___5 with
                           | (h2, args2) ->
                               let uu___6 =
                                 let uu___7 = warn h1 h2 in
                                 if uu___7
                                 then true
                                 else
                 ...
                 | (FStarC_Syntax_Syntax.Tm_refine
                    { FStarC_Syntax_Syntax.b2 = t12;
                      FStarC_Syntax_Syntax.phi = phi1;_},
                    FStarC_Syntax_Syntax.Tm_refine
                    { FStarC_Syntax_Syntax.b2 = t22;
                      FStarC_Syntax_Syntax.phi = phi2;_})
                     ->
                     let uu___2 =
                       warn t12.FStarC_Syntax_Syntax.sort
                         t22.FStarC_Syntax_Syntax.sort in
                     if uu___2 then true else warn phi1 phi2
                 | (FStarC_Syntax_Syntax.Tm_uvar uu___2, uu___3) -> false
                 | (uu___2, FStarC_Syntax_Syntax.Tm_uvar uu___3) -> false
                 | (uu___2, uu___3) -> true) in
            (let uu___1 =
               if
                 (Prims.not env2.FStarC_TypeChecker_Env.phase1) &&
                   should_warn
               then warn t1 t2
               else false in
             if uu___1
                   ...
                   let uu___5 =
                     let uu___6 =
                       let uu___7 =
                             ...
                             let uu___11 =
                               let uu___12 =
                                 ...
                                 FStar_Pprint.op_Hat_Slash_Hat
                                   (FStarC_Errors_Msg.text "bound in")
                                   uu___13 in
                         ...
                         FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
                           (FStarC_Errors_Msg.text
                              "In the decreases clause for this function, the SMT solver may not be able to prove that the types of")
                           uu___8 in
                       let uu___8 =
                         let uu___9 =
                           let uu___10 =
                     ...
                     let uu___7 =
                       let uu___8 =
                         ...
                         FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
                           (FStarC_Errors_Msg.text
                              "The type of the first term is:") uu___9 in
                       let uu___9 =
                         let uu___10 =
                           let uu___11 =
                             FStarC_Class_PP.pp
                           ...
                           FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
                             (FStarC_Errors_Msg.text
                                "The type of the second term is:") uu___11 in
                         [uu___10;
                         FStarC_Errors_Msg.text
          ...
          | (x::[], x_prev::[]) ->
              ...
              (match uu___ with
               | (t_x, t_x_prev) ->
                   FStarC_Syntax_Syntax.mk_Tm_app precedes_t
                     [FStarC_Syntax_Syntax.iarg t_x;
                     FStarC_Syntax_Syntax.iarg t_x_prev;
                     FStarC_Syntax_Syntax.as_arg x;
                     FStarC_Syntax_Syntax.as_arg x_prev] r)
          | (x::tl, x_prev::tl_prev) ->
               ...
               | (t_x, t_x_prev) ->
                   let tm_precedes =
                     FStarC_Syntax_Syntax.mk_Tm_app precedes_t
                       ...
                       FStarC_Syntax_Syntax.iarg t_x_prev;
                       FStarC_Syntax_Syntax.as_arg x;
                       FStarC_Syntax_Syntax.as_arg x_prev] r in
                   let eq3_x_x_prev =
                ...
                then
                  (l,
                    (FStar_Pervasives_Native.fst
                       (FStarC_List.splitAt n l_prev)))
                else
                  ((FStar_Pervasives_Native.fst
                      (FStarC_List.splitAt n_prev l)), l_prev) in
      ...
      let mk_precedes env2 d d_prev =
        ...
        | (FStarC_Syntax_Syntax.Decreases_wf (rel, e),
           FStarC_Syntax_Syntax.Decreases_wf (rel_prev, e_prev)) ->
            let rel_guard =
              FStarC_Syntax_Syntax.mk_Tm_app rel
                [FStarC_Syntax_Syntax.as_arg e;
                FStarC_Syntax_Syntax.as_arg e_prev] r in
            let uu___ =
              let uu___1 =
      ...
      let guard_one_letrec uu___ =
        ...
        | (l, arity, t, u_names) ->
            let uu___1 =
              FStarC_TypeChecker_Normalize.get_n_binders_no_unrefine env1
                arity t in
            (match uu___1 with
             | (formals, c) ->
                 (if arity > (FStarC_List.length formals)
                  ...
                  (let formals1 =
                   ...
                   let env_formals =
                     FStarC_TypeChecker_Env.push_binders env1 formals1 in
                   let rec split_trailing_pre bs =
                     match bs with
                     | [] -> ([], [])
                     | b::tl ->
                         let uu___3 = split_trailing_pre tl in
                         (match uu___3 with
                          | ([], pre) when
                              is_precondition_binder env_formals b ->
                              ([], (b :: pre))
                          | (real, pre) -> ((b :: real), pre)) in
                   let uu___3 = split_trailing_pre formals1 in
                   match uu___3 with
                   | (real_formals, pre_formals) ->
                       let refined_formals =
                         match real_formals with
                         | [] ->
                             let uu___4 = FStarC_Util.prefix formals1 in
                             (match uu___4 with
                              | (bs,
                                 { FStarC_Syntax_Syntax.binder_bv = last;
                                  ...
                                  let last1 =
                                    let uu___5 =
                                      FStarC_Syntax_Util.refine last
                                        precedes1 in
                                    {
                                  ...
                                  FStarC_List.op_At bs
                                    [FStarC_Syntax_Syntax.mk_binder_with_attrs
                                       last1 imp pqual attrs])
                         | uu___4 ->
                             let uu___5 = FStarC_Util.prefix real_formals in
                             (match uu___5 with
                              | (bs,
                                 { FStarC_Syntax_Syntax.binder_bv = last;
                                   FStarC_Syntax_Syntax.binder_qual = imp;
                                  ...
                                  FStarC_List.op_At bs
                                    (FStarC_List.op_At
                                       [FStarC_Syntax_Syntax.mk_binder_with_attrs
                                          last1 imp pqual attrs] pre_formals)) in
...
let wrap_guard_with_tactic_opt
  ...
  | FStar_Pervasives_Native.Some tactic ->
      FStarC_TypeChecker_Env.always_map_guard g
        (fun g1 ->
           let uu___ = FStarC_Syntax_Util.mk_squash g1 in
...
let is_comp_ascribed_reflect (e : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      ...
      (match uu___5 with
       | FStarC_Syntax_Syntax.Tm_app uu___6 ->
           let uu___7 = FStarC_Syntax_Util.head_and_args_full e1 in
           (match uu___7 with
            | (head, args) ->
                (match args with
                 | (e2, aqual)::[] ->
                     let uu___8 =
                       let uu___9 = FStarC_Syntax_Subst.compress head in
                       uu___9.FStarC_Syntax_Syntax.n in
                     (match uu___8 with
                      | FStarC_Syntax_Syntax.Tm_constant
                          (FStarC_Const.Const_reflect l) ->
                          FStar_Pervasives_Native.Some (l, e2, aqual)
...
let rec tc_term (env : FStarC_TypeChecker_Env.env)
  ...
  (let do1 uu___1 =
   ...
   let uu___1 = let uu___2 = FStarC_Debug.medium () in Prims.not uu___2 in
   if uu___1
   then do1 ()
   else
     ((let uu___3 =
      ...
      (let uu___3 = FStarC_Timing.record_ms do1 in
       match uu___3 with
       | (r, ms) ->
           ((let uu___5 =
               FStarC_Class_Show.show FStarC_Range_Ops.showable_range
  ...
  (let top = FStarC_Syntax_Subst.compress e in
   ...
   (match top.FStarC_Syntax_Syntax.n with
    ...
    | FStarC_Syntax_Syntax.Tm_bvar uu___2 ->
        FStarC_Effect.failwith "Impossible: tc_maybe_toplevel_term: not LN"
    ...
    | FStarC_Syntax_Syntax.Tm_quoted (qt, qi) ->
        let projl uu___2 =
          ...
          | FStar_Pervasives.Inr uu___3 ->
              FStarC_Effect.failwith "projl fail" in
                   ...
                   | (qt1, uu___5, g) ->
                       ...
                       let t =
                         FStarC_Syntax_Syntax.mk
                           ...
                           top.FStarC_Syntax_Syntax.pos in
                       let uu___6 =
                         value_check_expected_typ env1 t
    ...
    | FStarC_Syntax_Syntax.Tm_meta
        ...
        ->
        let uu___2 =
          tc_check_tot_or_gtot_term env1 e1 FStarC_Syntax_Syntax.t_prop
            FStar_Pervasives_Native.None in
        (match uu___2 with
         | (e2, c, g) ->
             let uu___3 =
               let uu___4 = FStarC_TypeChecker_Env.clear_expected_typ env1 in
               match uu___4 with | (env2, uu___5) -> tc_smt_pats env2 pats in
             (match uu___3 with
              | (pats1, g') ->
                  let g'1 =
                    {
              ...
              | (env0, uu___6) ->
                  ...
                  (match uu___7 with
                   | (expected_c1, uu___8, g_c) ->
                       let expected_ct =
                         FStarC_Syntax_Util.comp_to_comp_typ expected_c1 in
                       (if
                          Prims.not
                            (FStarC_Ident.lid_equals effect_lid
                               expected_ct.FStarC_Syntax_Syntax.effect_name)
                        then
                          (let uu___10 =
                             let uu___11 =
                               FStarC_Class_Show.show
                        ...
                        (let u_c =
                           env0.FStarC_TypeChecker_Env.universe_of env0
                             expected_ct.FStarC_Syntax_Syntax.result_typ in
                         let repr =
                           let uu___11 =
                             let uu___12 =
                                    ...
                                    FStarC_Syntax_Syntax.mk_Comp
                                      {
                                        ...
                                        FStarC_Syntax_Syntax.result_typ =
                                          (expected_ct.FStarC_Syntax_Syntax.result_typ);
                                        FStarC_Syntax_Syntax.flags = [];
                                        FStarC_Syntax_Syntax.source_effect_name
                                          =
                                          (expected_ct.FStarC_Syntax_Syntax.source_effect_name)
                                      } in
                                  let uu___15 =
                                    FStarC_TypeChecker_Rel.sub_comp env0
                                      c_reflect expected_c1 in
                                  match uu___15 with
                                  | FStar_Pervasives_Native.Some g -> g
                                  | FStar_Pervasives_Native.None ->
                                      let uu___16 =
                                        ...
                                        let uu___18 =
                                          FStarC_Class_Show.show
                                            ...
                                            expected_c1 in
                                        FStarC_Format.fmt2
                                          "A reflected term only has the trivial specification %s, which does not subsume the ascribed %s\n"
                                          uu___17 uu___18 in
                                      FStarC_Errors.raise_error
                  ...
                  let uu___7 =
                    tc_term
                      (FStarC_TypeChecker_Env.set_expected_typ_maybe_eq env0
                         (FStarC_Syntax_Util.comp_result expected_c1) use_eq)
                      e1 in
                  (match uu___7 with
                   | (e2, c', g') ->
                       let uu___8 =
                         let uu___9 =
                           (c', FStarC_TypeChecker_Env.trivial_guard) in
                         match uu___9 with
                         | (c'1, g_c') ->
                             let uu___10 =
             ...
             (match uu___4 with
              | (t1, uu___5, f) ->
                  ...
                  (match uu___6 with
                   | (e2, c, g) ->
                       let f1 =
                         FStarC_TypeChecker_Util.simplify_and_label_guard
                           (FStar_Pervasives_Native.Some
                              (fun uu___7 ->
                                 FStarC_TypeChecker_Err.ill_kinded_type))
                           (FStarC_TypeChecker_Env.set_range env1
                              t1.FStarC_Syntax_Syntax.pos) f in
                       let c1 =
                         let uu___7 =
                           let uu___8 = FStarC_Syntax_Util.is_exactly_unit t1 in
                           if uu___8
                           then
                             FStarC_TypeChecker_Util.keep_res_typ env1 t1
                               (FStarC_Syntax_Util.comp_result c)
                           else false in
                         if uu___7
                         then c
                         else FStarC_Syntax_Util.set_result_typ c t1 in
                       let uu___7 =
                         let uu___8 =
                           FStarC_Syntax_Syntax.mk
                             (FStarC_Syntax_Syntax.Tm_ascribed
                            ...
                            let uu___8 =
                              let uu___9 =
                                FStarC_Class_Monoid.op_Plus_Plus
                                  ...
                                  f2 in
                              FStarC_Class_Monoid.op_Plus_Plus
                                FStarC_TypeChecker_Common.monoid_guard_t f1
                                uu___9 in
                            (e3, c2, uu___8)))))
    | FStarC_Syntax_Syntax.Tm_app uu___2 ->
        let uu___3 = FStarC_Syntax_Util.head_and_args_full top in
        (match uu___3 with
         | (lhead, largs) ->
             let dispatch =
               let uu___4 =
                 let uu___5 =
                   let uu___6 = FStarC_Syntax_Subst.compress lhead in
                   uu___6.FStarC_Syntax_Syntax.n in
                 (uu___5, largs) in
               match uu___4 with
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_range_of), a::rest) when
                   match rest with | hd::tl -> true | uu___5 -> false ->
                   let head =
                     let uu___5 =
                       FStarC_Range_Ops.union_ranges
                         lhead.FStarC_Syntax_Syntax.pos
                         (FStar_Pervasives_Native.fst a).FStarC_Syntax_Syntax.pos in
                     FStarC_Syntax_Syntax.mk_Tm_app lhead [a] uu___5 in
                   FStar_Pervasives.Inr (head, rest)
               | (FStarC_Syntax_Syntax.Tm_constant (FStarC_Const.Const_reify
                  uu___5), a::rest) when
                   match rest with | hd::tl -> true | uu___6 -> false ->
                   let head =
                     let uu___6 =
                       FStarC_Range_Ops.union_ranges
                         lhead.FStarC_Syntax_Syntax.pos
                         (FStar_Pervasives_Native.fst a).FStarC_Syntax_Syntax.pos in
                     FStarC_Syntax_Syntax.mk_Tm_app lhead [a] uu___6 in
                   FStar_Pervasives.Inr (head, rest)
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_reflect uu___5), a::rest) when
                   match rest with | hd::tl -> true | uu___6 -> false ->
                   let head =
                     let uu___6 =
                       FStarC_Range_Ops.union_ranges
                   ...
                   FStar_Pervasives.Inr (head, rest)
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_set_range_of), a1::a2::rest) when
                   match rest with | hd::tl -> true | uu___5 -> false ->
                   let head =
                     let uu___5 =
                       FStarC_Range_Ops.union_ranges
                     ...
                     FStarC_Syntax_Syntax.mk_Tm_app lhead [a1; a2] uu___5 in
                   FStar_Pervasives.Inr (head, rest)
               | (FStarC_Syntax_Syntax.Tm_constant
                  ...
                  (e1, FStar_Pervasives_Native.None)::[]) ->
                   let uu___5 =
                     tc_term
                       (FStar_Pervasives_Native.fst
                          (FStarC_TypeChecker_Env.clear_expected_typ env1))
                       e1 in
                   (match uu___5 with
                    | (e2, c, g) ->
                        let uu___6 =
                          let uu___7 =
                            FStarC_Syntax_Syntax.mk_Tm_app lhead
                          ...
                          let uu___8 =
                            ...
                            FStarC_Syntax_Syntax.mk_Total uu___9 in
                          (uu___7, uu___8, g) in
                  ...
                  (t, FStar_Pervasives_Native.None)::(r,
                   ...
                   let env' =
                     let uu___5 =
                       FStarC_Syntax_Syntax.tabbrev
                         FStarC_Parser_Const.range_lid in
                     FStarC_TypeChecker_Env.set_expected_typ env1 uu___5 in
                    ...
                    | (er, uu___6, gr) ->
                        ...
                        (match uu___7 with
                         | (t1, tt, gt) ->
                             let g =
                               FStarC_Class_Monoid.op_Plus_Plus
                                 FStarC_TypeChecker_Common.monoid_guard_t gr
                                 gt in
                             let uu___8 =
                               let uu___9 =
                                 FStarC_Syntax_Syntax.mk_Tm_app lhead
               ...
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_range_of), uu___5) ->
                   let uu___6 =
                     let uu___7 =
                       FStarC_Class_Show.show
                         FStarC_Syntax_Print.showable_term top in
                   ...
                   FStarC_Errors.raise_error
                     ...
                     (Obj.magic uu___6)
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_set_range_of), uu___5) ->
                   let uu___6 =
                     let uu___7 =
                       FStarC_Class_Show.show
                         FStarC_Syntax_Print.showable_term top in
                         ...
                         (match uu___9 with
                          | (e2, c, g) ->
                              let uu___10 =
                                ...
                                match uu___11 with
                                | (c1, g_c) ->
                                    ((FStarC_Syntax_Util.comp_to_comp_typ c1),
                                      g_c) in
                              (match uu___10 with
                               | (c1, g_c) ->
                                   ((let uu___12 =
                                    ...
                                    (let u_c =
                                     ...
                                     let repr =
                                       let uu___12 =
                                         FStarC_Syntax_Syntax.mk_Comp c1 in
                                     ...
                                     let c2 =
                                       ...
                                       if uu___12
                                       then
                                         FStarC_Syntax_Syntax.mk_GTotal repr
                                       else
                                         (let uu___13 =
               ...
               | (FStarC_Syntax_Syntax.Tm_constant
                  (FStarC_Const.Const_reflect l), (e1, aqual)::[]) ->
                   (if
                      (match aqual with
                       | FStar_Pervasives_Native.Some v -> true
                       | uu___6 -> false)
                    ...
                    (let uu___7 = FStarC_Syntax_Util.head_and_args_full top in
                     match uu___7 with
                     | (reflect_op, uu___8) ->
                         let uu___9 =
                           FStarC_TypeChecker_Env.effect_decl_opt env1 l in
                         (match uu___9 with
                          | FStar_Pervasives_Native.None ->
                              FStarC_Errors.raise_error
                                (FStarC_Syntax_Syntax.has_range_syntax ()) e1
                                ...
                                (Obj.magic
                                   (FStarC_Format.fmt1
                                      "Effect %s not found (for reflect)"
                                      (FStarC_Ident.string_of_lid l)))
                                                    ...
                                                    let uu___15 =
                                                      let uu___16 =
                                                        let uu___17 =
                                                          let uu___18 =
                                                            let uu___19 =
                                                              FStarC_TypeChecker_Util.monadic_annot_typ
                                                                (FStarC_Syntax_Util.comp_result
                                                                   c1) in
                                                            ((FStarC_Syntax_Util.comp_effect_name
                                                                c1), uu___19) in
                           ...
                           (let uu___8 =
                              let uu___9 =
                                FStarC_List.map FStar_Pervasives_Native.fst
                                  fields in
                              FStarC_List.zip
                                uc.FStarC_Syntax_Syntax.uc_fields uu___9 in
                            (base_term, uu___8)) in
                   (match uu___6 with
                    | (base_term, uc_fields) ->
                        let uu___7 =
                            ...
                            | FStar_Pervasives_Native.Some e1 ->
                                ...
                                (match uu___8 with
                                 | (uu___9, lc, uu___10) ->
                                     let uu___11 =
                                       let uu___12 =
                                         FStarC_TypeChecker_Util.head_fv_of_typ
                                           env2
                          ...
                          match FStarC_TypeChecker_Env.expected_typ env1 with
                        ...
                        (match uu___7 with
                         | ((rdc, constrname, constructor), topt) ->
                             let rdc1 = rdc in
                             (if
                                (match FStarC_TypeChecker_Env.expected_typ
                                         env1
                                 with
                                 | FStar_Pervasives_Native.Some v -> true
                                 | uu___9 -> false) &&
                                  (Prims.not
                                     rdc1.FStarC_Syntax_DsEnv.is_record)
                              then
                                (let rdc_field_names =
                                   FStarC_List.map
                                     (fun uu___9 ->
                                 ...
                                 let candidate =
                                   FStarC_Syntax_Syntax.fvar
                                     (FStarC_Ident.set_lid_range projname
                                        x.FStarC_Syntax_Syntax.pos) qual in
                                   ...
                                   FStarC_List.map
                                     (fun uu___10 ->
                                        FStarC_Syntax_Syntax.iarg
                                          FStarC_Syntax_Syntax.tun)
                                     rdc1.FStarC_Syntax_DsEnv.parms in
                                 FStarC_Syntax_Syntax.mk_Tm_app constructor1
                                   uu___9 top.FStarC_Syntax_Syntax.pos in
                               let term1 =
                                 let uu___9 =
                                   FStarC_List.map
                                     (fun uu___10 ->
                                        ...
                                        | (a, is_imp) ->
                                            (a,
                                              (FStarC_Syntax_Syntax.as_aqual_implicit
                                                 is_imp))) args1 in
                                 FStarC_Syntax_Syntax.mk_Tm_app term uu___9
                                   top.FStarC_Syntax_Syntax.pos in
                               let uu___9 = tc_term env1 term1 in
               ...
               | (FStarC_Syntax_Syntax.Tm_fvar
                  ...
                  (e1, FStar_Pervasives_Native.None)::rest) ->
                   let proceed_with choice =
                     ...
                     | FStar_Pervasives_Native.None ->
                         FStarC_Errors.raise_error
                           FStarC_Ident.hasrange_lident field_name
                           FStarC_Errors_Codes.Fatal_IdentifierNotFound ()
                           (Obj.magic
                              FStarC_Errors_Msg.is_error_message_list_doc)
                           (Obj.magic
                              [FStarC_Errors_Msg.text
                                 (FStarC_Format.fmt1
                                    "Field name %s could not be resolved."
                                    (FStarC_Ident.string_of_lid field_name))])
               ...
               | uu___5 ->
                   let uu___6 =
                     ...
                     if uu___7
                     then Prims.not env1.FStarC_TypeChecker_Env.phase1
                     else false in
                   if uu___6
                   then
                     (match largs with
                            ...
                            | uu___9 ->
                                FStarC_Errors.raise_error
                                  ...
                                  (Obj.magic
                                     FStarC_Errors_Msg.is_error_message_string)
                                  (Obj.magic
                                     "synth_by_tactic: bad application") in
                          (match uu___8 with
                           | (args1, args2) ->
                               let uu___9 =
                                 let uu___10 =
                                   FStarC_Syntax_Util.mk_app lhead args1 in
                                 (uu___10, args2) in
                               FStar_Pervasives.Inr uu___9))
                   else FStar_Pervasives.Inr (lhead, largs) in
             (match dispatch with
              | FStar_Pervasives.Inl res -> res
              | FStar_Pervasives.Inr (head, args) ->
                  let env0 = env1 in
                  let env2 =
                    instantiate_both
                      (FStar_Pervasives_Native.fst
                  ...
                  ((let uu___5 = FStarC_Debug.high () in
                    ...
                    then
                      ...
                      let uu___7 =
                        FStarC_Class_Show.show
                          FStarC_Syntax_Print.showable_term top in
                      let uu___8 = print_expected_ty_str env0 in
                      FStarC_Format.print3 "(%s) Checking app %s, %s\n"
                        uu___6 uu___7 uu___8
                    else ());
                   (let uu___5 = tc_term (no_inst env2) head in
                    match uu___5 with
                    | (head1, chead, g_head) ->
                        let uu___6 =
                          let uu___7 =
                            FStarC_TypeChecker_Util.short_circuit_head head1 in
                          if uu___7
                          then
                            (if env2.FStarC_TypeChecker_Env.phase1
                             then
                               let uu___8 =
                                 maybe_elaborate_short_circuit_args env0
                                   head1 args in
                               match uu___8 with
                               | FStar_Pervasives_Native.Some rewritten ->
                                   rewritten
                               | FStar_Pervasives_Native.None ->
                                   check_application_args env2 head1 chead
                                     g_head args
                                     (FStarC_TypeChecker_Env.expected_typ
                                        env0)
                             else
                               check_short_circuit_args env2 head1 chead
                                 g_head args
                                 (FStarC_TypeChecker_Env.expected_typ env0))
                          else
                            check_application_args env2 head1 chead g_head
                              args (FStarC_TypeChecker_Env.expected_typ env0) in
                        (match uu___6 with
                         | (e1, c, g) ->
                             let uu___7 =
                               let uu___8 =
                                 let uu___9 =
                                   FStarC_Syntax_Util.is_tot_or_gtot_comp c in
                                 if uu___9
                                 then true
                                 else
                                   if env2.FStarC_TypeChecker_Env.phase1
                               ...
                               then
                                 ...
                                 | (e2, res_typ, implicits) ->
                                     let uu___10 =
                                       FStarC_Syntax_Util.set_result_typ c
                                         res_typ in
                                     (e2, uu___10, implicits)
                               else
                                 (e1, c,
                                   (FStarC_Class_Monoid.mzero
                                      FStarC_TypeChecker_Common.monoid_guard_t)) in
  ...
  | FStarC_Syntax_Syntax.Tm_match
      ...
      let uu___2 =
        tc_term
          (instantiate_both
             (FStar_Pervasives_Native.fst
                (FStarC_TypeChecker_Env.clear_expected_typ env))) e1 in
      (match uu___2 with
       | (e11, c1, g1) ->
           let uu___3 =
             match eqns with
                           ...
                           (match uu___9 with
                            | (bs, c) ->
                                ...
                                FStarC_TypeChecker_Util.maybe_coerce_lc env'
                                  e11 c1 (FStarC_Syntax_Util.comp_result c))
                       | FStar_Pervasives_Native.None ->
                           (e11, c1,
                             (FStarC_Class_Monoid.mzero
                                FStarC_TypeChecker_Common.monoid_guard_t)))
                      ...
                      (if
                         Prims.not
                           (FStarC_Syntax_Util.is_pure_or_ghost_effect
                              (FStarC_Syntax_Util.comp_effect_name c11))
                       then
                         (let uu___6 =
                            let uu___7 =
                              FStarC_Class_Show.show
                 ...
                 | (env_branches, ret_opt1, g11) ->
                     let guard_x =
                       FStarC_Syntax_Syntax.new_bv
                         ...
                         (FStarC_Syntax_Util.comp_result c11) in
                     let t_eqns =
                       ...
                       | FStarC_TypeChecker_Common.NonTrivial f ->
                           let eq =
                             ...
                             FStarC_Syntax_Util.mk_eq2 uu___5
                               (FStarC_Syntax_Util.comp_result c11) uu___6
                               e12 in
                           let uu___5 =
                             let uu___6 =
                               FStarC_TypeChecker_Common.post_obligation
                                 guard_x eq f in
                            ...
                            | (cases, gs, erasable) ->
                                  ...
                                  | (neg_conds, uu___9) ->
                                      let hyps =
                                        FStarC_List.map2
                                          (fun cond neg ->
                                             let uu___10 =
                                               FStarC_Syntax_Util.b2t cond in
                                             FStarC_Syntax_Util.mk_conj neg
                                               uu___10) conds neg_conds in
                                      let uu___10 =
                                        FStarC_List.map2
                                          FStarC_TypeChecker_Common.weaken_guard_formula
                                          gs hyps in
                                ...
                                (match ret_opt1 with
                                         ...
                                         let eff =
                                           FStarC_List.fold_left
                                             (fun eff1 uu___8 ->
                                                ...
                                                | (uu___9, eff_label,
                                                   uu___10, uu___11) ->
                                                    FStarC_TypeChecker_Util.join_effects
                                                      env eff1 eff_label)
                                             FStarC_Parser_Const.primitive_pure_lid
                                             cases in
                                         Prims.not
                                           (FStarC_Syntax_Util.is_pure_or_ghost_effect
                                              eff) in
                                       let branch_res_typ x =
                                         let uu___8 = x in
                                         match uu___8 with
                                         | (uu___9, uu___10, uu___11, c) ->
                                             let uu___12 =
                                               let uu___13 = c should_return in
                                               FStar_Pervasives_Native.fst
                                                 uu___13 in
                                             FStarC_Syntax_Util.comp_result
                                               uu___12 in
                                           ...
                                           then t
                                             ...
                                             (let branch x =
                                                ...
                                                | (f, uu___10, uu___11, c) ->
                                                    ...
                                                    (f, uu___12) in
                                              let uu___9 =
                                                FStarC_List.map branch cases in
                                              FStarC_TypeChecker_Util.combine_branch_res_typs
                                                env guard_x res_t uu___9)
                                       | [] -> res_t in
                                     let uu___8 =
                                       FStarC_TypeChecker_Util.bind_cases env
                                         res_t1 cases guard_x in
                                     (match uu___8 with
                                 ...
                                 | FStar_Pervasives_Native.Some
                                     ...
                                     let uu___10 =
                                       FStarC_TypeChecker_Util.bind_cases env
                                         t1 cases2 guard_x in
                                     (match uu___10 with
                                      | (c, g_c1) -> (c, g_c1, g2, erasable)))) in
                     (match uu___5 with
                      | (c_branches, g_c_branches, g_branches, erasable) ->
                          let uu___6 =
                                          ...
                                          (let uu___10 =
                                             let uu___11 =
                                               let uu___12 =
                                                 ...
                                                 FStarC_Class_Monoid.op_Plus_Plus
                                                   FStarC_TypeChecker_Common.monoid_guard_t
                                                   uu___13 g_branches in
                                               FStarC_Class_Monoid.op_Plus_Plus
                                                 FStarC_TypeChecker_Common.monoid_guard_t
                                                 uu___12 g_cres1 in
                                             FStarC_Class_Monoid.op_Plus_Plus
                                               FStarC_TypeChecker_Common.monoid_guard_t
                                               uu___11 g_expected_type in
                                           (e2, cres2, uu___10))))))))))
  | uu___1 ->
      let uu___2 =
        let uu___3 =
              ...
              | (tau1, uu___6, g2) ->
                  (FStarC_TypeChecker_Rel.force_trivial_guard env g2;
                   (let t =
                    ...
                    (let uu___10 = FStarC_Syntax_Syntax.mk_Total typ1 in
                     (t, uu___10,
                       (FStarC_Class_Monoid.mzero
                          FStarC_TypeChecker_Common.monoid_guard_t)))))))))
  ...
  (let env1 = FStarC_TypeChecker_Env.set_range env e.FStarC_Syntax_Syntax.pos in
       ...
       let uu___1 =
         ...
         | FStar_Pervasives_Native.Some (t, use_eq) when use_eq ->
             let uu___2 =
               ...
               [uu___3; FStarC_Errors_Msg.text "Please use subtyping."] in
             FStarC_Errors.raise_error
               (FStarC_Syntax_Syntax.has_range_syntax ()) e
               FStarC_Errors_Codes.Fatal_NotSupported ()
               (Obj.magic FStarC_Errors_Msg.is_error_message_list_doc)
   ...
   | FStarC_Syntax_Syntax.Tm_uinst
       ...
       (FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.synth_lid) &&
         (Prims.not env1.FStarC_TypeChecker_Env.phase1)
       ->
       ...
       (match uu___3 with
        | ((us', t), range) ->
            ...
            let fv1 =
              if generated
              then fv
              else FStarC_Syntax_Syntax.set_range_of_fv fv range in
            (if Prims.not generated then maybe_warn_on_use env1 fv1 else ();
   ...
   | FStarC_Syntax_Syntax.Tm_fvar fv ->
       let maybe_set_fv_qual env2 fv1 =
         let uu___1 =
           if
             match fv1.FStarC_Syntax_Syntax.fv_qual with
       ...
       (match uu___1 with
        | ((us, t), range) ->
            ...
            (if Prims.not generated then maybe_warn_on_use env1 fv2 else ();
             (let uu___4 = FStarC_Effect.op_Bang dbg_Range in
              if uu___4
              then
                let uu___5 =
                  FStarC_Class_Show.show FStarC_Ident.showable_lident
                    (FStarC_Syntax_Syntax.lid_of_fv fv2) in
                let uu___6 =
   ...
   | FStarC_Syntax_Syntax.Tm_arrow uu___1 ->
       let uu___2 = FStarC_Syntax_Util.arrow_node_formals_comp_ln top in
       (match uu___2 with
        | (bs, c) ->
            ...
            (match uu___3 with
             | (bs1, c1) ->
                 ...
                 (match uu___4 with
                  | (env2, uu___5) ->
                      let uu___6 = tc_binders env2 bs1 in
                      (match uu___6 with
                       | (bs2, env3, g, us) ->
                           let uu___7 = tc_comp env3 c1 in
                           (match uu___7 with
                            | (c2, uc, f) ->
                                let e1 =
                                  let uu___8 =
                                    FStarC_Syntax_Util.arrow bs2 c2 in
                                  {
                                    FStarC_Syntax_Syntax.n =
                                      (uu___8.FStarC_Syntax_Syntax.n);
                                    FStarC_Syntax_Syntax.pos =
                                      (top.FStarC_Syntax_Syntax.pos);
                                    FStarC_Syntax_Syntax.hash_code =
                                      (uu___8.FStarC_Syntax_Syntax.hash_code)
                                  } in
                                (if
                                   Prims.not
                                     env3.FStarC_TypeChecker_Env.phase1
                                 then check_smt_pat env3 e1
                                 else ();
                                 (let u =
   ...
   | FStarC_Syntax_Syntax.Tm_refine
       ...
       (match uu___1 with
        | (x1, phi1) ->
            ...
            (match uu___2 with
             | (env2, uu___3) ->
                 let uu___4 = tc_binder env2 (FStarC_List.hd x1) in
                 (match uu___4 with
                  | (x2, env3, f1, u) ->
                      ((let uu___6 = FStarC_Debug.high () in
                       ...
                       (let uu___6 =
                          tc_check_tot_or_gtot_term env3 phi1
                            FStarC_Syntax_Syntax.t_prop
                            (FStar_Pervasives_Native.Some
   ...
   | FStarC_Syntax_Syntax.Tm_abs uu___1 ->
       let uu___2 = FStarC_Syntax_Util.abs_one_group_ln top in
       (match uu___2 with
        | (bs, body, uu___3) ->
            let bs1 =
            ...
            ((let uu___5 = FStarC_Debug.medium () in
              ...
              then
                let uu___6 =
                  let uu___7 =
                    FStarC_Syntax_Syntax.mk_Tm_abs bs1 body
                      FStar_Pervasives_Native.None
                      top.FStarC_Syntax_Syntax.pos in
                  FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                    uu___7 in
             ...
             (let uu___5 = FStarC_Syntax_Subst.open_term bs1 body in
              match uu___5 with | (bs2, body1) -> tc_abs env1 top bs2 body1)))
   | uu___1 ->
       let uu___2 =
         let uu___3 =
           FStarC_Class_Show.show FStarC_Syntax_Print.showable_term top in
...
and tc_constant (env : FStarC_TypeChecker_Env.env_t)
  ...
  let res =
    ...
    | FStarC_Const.Const_char uu___ ->
        ...
        (match uu___1 with | FStar_Pervasives_Native.Some v -> v)
...
and tc_comp (env : FStarC_TypeChecker_Env.env)
  ...
  | FStarC_Syntax_Syntax.Comp ct when
      ...
      (match uu___ with
       | (k, u) ->
           let uu___1 =
             tc_check_tot_or_gtot_term env ct.FStarC_Syntax_Syntax.result_typ
               k FStar_Pervasives_Native.None in
           (match uu___1 with
            | (t, uu___2, g) ->
                let uu___3 =
                  FStarC_Syntax_Syntax.mk_Comp
  ...
  | FStarC_Syntax_Syntax.Comp c1 ->
      ...
      let tc =
        FStarC_Syntax_Syntax.mk_Tm_app head
          [FStarC_Syntax_Syntax.as_arg c1.FStarC_Syntax_Syntax.result_typ]
      ...
      (match uu___ with
       | (tc1, uu___1, f) ->
           let uu___2 = FStarC_Syntax_Util.head_and_args_full tc1 in
           (match uu___2 with
            | (_head, args) ->
                let res = FStar_Pervasives_Native.fst (FStarC_List.hd args) in
                let uu___3 = FStarC_TypeChecker_Env.clear_expected_typ env in
                (match uu___3 with
                 | (env0, uu___4) ->
                           ...
                           (fun uu___7 ->
                              ...
                              | FStarC_Syntax_Syntax.SMTPAT p ->
                                  ...
                                  (match uu___8 with
                                   | (env1, uu___9) ->
                                       let uu___10 =
                                         tc_tot_or_gtot_term env1 p in
                                       (match uu___10 with
                                        | (p1, uu___11, g_p) ->
                                            ((FStarC_Syntax_Syntax.SMTPAT p1),
                                              g_p)))
                              | f1 ->
                                  (f1,
                                    (FStarC_Class_Monoid.mzero
                     ...
                     (match uu___5 with
                      | (flags, guards) ->
                          ...
                          let u_res =
                            env.FStarC_TypeChecker_Env.universe_of env res in
                          let u_c =
...
and tc_universe (env : FStarC_TypeChecker_Env.env)
  ...
  let rec aux u1 =
    ...
    | FStarC_Syntax_Syntax.U_bvar uu___ ->
        FStarC_Effect.failwith "Impossible: locally nameless"
    | FStarC_Syntax_Syntax.U_unknown ->
        FStarC_Effect.failwith "Unknown universe"
    ...
    | FStarC_Syntax_Syntax.U_name x ->
        ...
        then u2
          ...
          (let uu___1 =
             ...
             Prims.strcat "Universe variable " uu___2 in
  ...
  | FStar_Pervasives_Native.Some (t, use_eq) ->
      ...
      let rec as_function_typ norm1 t2 =
        let uu___ =
          ...
          uu___1.FStarC_Syntax_Syntax.n in
        match uu___ with
        | FStarC_Syntax_Syntax.Tm_uvar uu___1 when
            let uu___2 =
              let uu___3 =
                let uu___4 = FStarC_Syntax_Util.leftmost_head t2 in
                FStarC_Syntax_Subst.compress uu___4 in
              uu___3.FStarC_Syntax_Syntax.n in
            match uu___2 with
            | FStarC_Syntax_Syntax.Tm_uvar _0 -> true
            | uu___3 -> false ->
            ((match env.FStarC_TypeChecker_Env.letrecs with
              | [] -> ()
              | uu___3 ->
        ...
        | FStarC_Syntax_Syntax.Tm_arrow uu___1 ->
            let uu___2 = FStarC_Syntax_Util.arrow_formals_comp_strict t2 in
            (match uu___2 with
             | (bs_expected, c_expected) ->
                 let check_actuals_against_formals env1 bs1 bs_expected1
                   body1 =
                   let rec handle_more args c_expected1 body2 =
                     let uu___3 = args in
                     match uu___3 with
                         ...
                         (match more with
                          | FStar_Pervasives_Native.None ->
                              let uu___4 =
                                FStarC_Syntax_Subst.subst_comp subst
                                  c_expected1 in
                              (env_bs, bs2, guard_env, uu___4, body2)
                          | FStar_Pervasives_Native.Some
                          ...
                          | FStar_Pervasives_Native.Some
                              ...
                              let c =
                                FStarC_Syntax_Subst.subst_comp subst
                                  c_expected1 in
                              let uu___4 = FStarC_Syntax_Util.is_named_tot c in
                              if uu___4
                              then
                                let t3 =
                                  FStarC_TypeChecker_Normalize.unfold_whnf
                                    env_bs (FStarC_Syntax_Util.comp_result c) in
                                 ...
                                 | FStarC_Syntax_Syntax.Tm_arrow uu___5 ->
                                     let uu___6 =
                                       FStarC_Syntax_Util.arrow_formals_comp_strict
                                         t3 in
                                     (match uu___6 with
                                      | (bs_expected2, c_expected2) ->
                                          let uu___7 =
                                            tc_abs_check_binders env_bs
                                              more_bs bs_expected2 use_eq in
                                          (match uu___7 with
                                           | (env_bs_bs', bs', more1,
                                              guard'_env_bs, subst1) ->
                                               ...
                                               let uu___8 =
                                                 ...
                                                 (env_bs_bs',
                                                   ...
                                                   more1, uu___9, subst1) in
                                               handle_more uu___8 c_expected2
                                                 body2))
                                 | uu___5 ->
                                     let body3 =
                   ...
                   let uu___3 =
                     tc_abs_check_binders env1 bs1 bs_expected1 use_eq in
                   handle_more uu___3 c_expected body1 in
                 let mk_letrec_env envbody bs1 c =
                   ...
                   let envbody1 =
                     {
                       ...
                       FStarC_TypeChecker_Env.letrecs = [];
                       FStarC_TypeChecker_Env.rec_names =
                         (envbody.FStarC_TypeChecker_Env.rec_names);
                       FStarC_TypeChecker_Env.top_level =
                       ...
                       FStarC_TypeChecker_Env.disc_proj_tab =
                         (envbody.FStarC_TypeChecker_Env.disc_proj_tab);
                       FStarC_TypeChecker_Env.erasable_types_tab =
                       ...
                       FStarC_TypeChecker_Env.iface_todo =
                         (envbody.FStarC_TypeChecker_Env.iface_todo);
                       FStarC_TypeChecker_Env.iface_hidden =
                         (envbody.FStarC_TypeChecker_Env.iface_hidden);
                       FStarC_TypeChecker_Env.iface_lids =
                         (envbody.FStarC_TypeChecker_Env.iface_lids);
                       FStarC_TypeChecker_Env.iface_val_lids =
                         (envbody.FStarC_TypeChecker_Env.iface_val_lids)
                     } in
                   let uu___3 =
                     FStarC_List.fold_left
                       (fun uu___4 uu___5 ->
                   ...
                   | (envbody2, letrec_binders, g) ->
                       ...
                       (envbody2, letrec_binders, uu___4) in
                 let envbody =
                   {
                     FStarC_TypeChecker_Env.solver =
                       (env.FStarC_TypeChecker_Env.solver);
                 ...
                 let uu___3 =
                   check_actuals_against_formals envbody bs bs_expected body in
                 (match uu___3 with
                      ...
                      let envbody2 =
                        {
                          ...
                          FStarC_TypeChecker_Env.letrecs =
                            (env.FStarC_TypeChecker_Env.letrecs);
                          FStarC_TypeChecker_Env.rec_names =
                            (envbody1.FStarC_TypeChecker_Env.rec_names);
                          FStarC_TypeChecker_Env.top_level =
                          ...
                          FStarC_TypeChecker_Env.disc_proj_tab =
                            (envbody1.FStarC_TypeChecker_Env.disc_proj_tab);
                          FStarC_TypeChecker_Env.erasable_types_tab =
                          ...
                          FStarC_TypeChecker_Env.iface_todo =
                            (envbody1.FStarC_TypeChecker_Env.iface_todo);
                          FStarC_TypeChecker_Env.iface_hidden =
                            (envbody1.FStarC_TypeChecker_Env.iface_hidden);
                          FStarC_TypeChecker_Env.iface_lids =
                            (envbody1.FStarC_TypeChecker_Env.iface_lids);
                          FStarC_TypeChecker_Env.iface_val_lids =
                            (envbody1.FStarC_TypeChecker_Env.iface_val_lids)
                        } in
                      let uu___4 = mk_letrec_env envbody2 bs1 c in
                      (match uu___4 with
                       | (envbody3, letrecs, g_annots) ->
                           let envbody4 =
                             FStarC_TypeChecker_Env.set_expected_typ_maybe_eq
                               envbody3 (FStarC_Syntax_Util.comp_result c)
                               use_eq in
                           let uu___5 =
                             FStarC_Class_Monoid.op_Plus_Plus
        ...
        | uu___1 ->
            ...
            then
              ...
              (let uu___2 =
               ...
               match uu___2 with
               | (uu___3, bs1, uu___4, c_opt, envbody, body1, g_env) ->
...
and tc_abs_check_binders (env : FStarC_TypeChecker_Env.env)
  ...
  let rec aux env_subst bs1 bs_expected1 =
    let uu___ = env_subst in
    match uu___ with
    | (env1, subst) ->
        (match (bs1, bs_expected1) with
         ...
         | ([],
            { FStarC_Syntax_Syntax.binder_bv = hd_e;
              ...
              FStarC_Syntax_Syntax.binder_attrs = attrs;_}::uu___1)
             when
             if
               (match q with
                | FStar_Pervasives_Native.Some v -> true
                | uu___2 -> false) &&
                 (match match q with | FStar_Pervasives_Native.Some v -> v
                  with
                  | FStarC_Syntax_Syntax.Implicit _0 -> true
                  | uu___2 -> false)
             then
               ...
               FStarC_Syntax_Syntax.new_bv
                 (FStar_Pervasives_Native.Some
                    (FStarC_Ident.range_of_id
                       hd_e.FStarC_Syntax_Syntax.ppname)) uu___2 in
             aux (env1, subst)
               [FStarC_Syntax_Syntax.mk_binder_with_attrs bv q pqual attrs]
               bs_expected1
            ...
            { FStarC_Syntax_Syntax.binder_bv = hd_e;
             ...
             let bv =
               ...
               FStarC_Syntax_Syntax.new_bv
                 (FStar_Pervasives_Native.Some
                    (FStarC_Ident.range_of_id
                       hd_e.FStarC_Syntax_Syntax.ppname)) uu___6 in
             aux (env1, subst)
               ((FStarC_Syntax_Syntax.mk_binder_with_attrs bv q pqual attrs)
               :: bs1) bs_expected1
         | ({ FStarC_Syntax_Syntax.binder_bv = hd;
            ...
            { FStarC_Syntax_Syntax.binder_bv = hd_expected;
             ...
             ->
             (if Prims.not (FStarC_Syntax_Util.bqual_compat imp imp')
              then
                (let uu___2 =
                   let uu___3 =
                     let uu___4 =
                   ...
                   let uu___4 =
                     let uu___5 =
                       ...
                       FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
                         (FStarC_Errors_Msg.text "Got:") uu___6 in
                     let uu___6 =
                       let uu___7 =
                         let uu___8 =
                           let uu___9 =
                         ...
                         FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
                           (FStarC_Errors_Msg.text "Expected:") uu___8 in
                       [uu___7] in
                     uu___5 :: uu___6 in
                   uu___3 :: uu___4 in
               ...
               then
                 (let uu___3 =
                    ...
                    FStarC_Format.fmt3
                      ...
                      uu___4 (positivity_qual_to_string pqual_expected)
                  ...
                  | uu___5 ->
                      ((let uu___7 = FStarC_Debug.high () in
                       ...
                       (let uu___7 = FStarC_Syntax_Util.type_u () in
                        ...
                        | (tu, u) ->
                            let uu___8 =
                              tc_check_tot_or_gtot_term env1
                                hd.FStarC_Syntax_Syntax.sort tu
                                FStar_Pervasives_Native.None in
                            (match uu___8 with
                             | (t, uu___9, g1_env) ->
                                 let g2_env =
              ...
              let uu___5 =
                let uu___6 =
                  ...
                  let uu___7 =
                    ...
                    then
                      let uu___8 =
                        ...
                        uu___9.FStarC_Syntax_Syntax.n in
                      match uu___8 with
                      | FStarC_Syntax_Syntax.Tm_app uu___9 ->
                          let uu___10 =
                          ...
                          (match uu___10 with
                           | (head, uu___11::[]) ->
                               ...
                               (match uu___12 with
                                | FStarC_Syntax_Syntax.Tm_constant
                                    (FStarC_Const.Const_reflect uu___13) ->
                                    true
                                | uu___13 -> false)
                           | uu___11 -> false)
                      | uu___9 -> false
                  ...
                  then
                    let uu___8 =
                      let uu___9 =
                        let uu___10 =
                          let uu___11 =
                            let uu___12 =
                              let uu___13 = FStarC_Option.must c_opt in
                              FStar_Pervasives.Inr uu___13 in
                            let uu___13 = FStarC_Option.must use_eq_opt in
                            (uu___12, FStar_Pervasives_Native.None, uu___13) in
                          {
                            FStarC_Syntax_Syntax.tm = body1;
                            FStarC_Syntax_Syntax.asc = uu___11;
                            FStarC_Syntax_Syntax.eff_opt =
                              FStar_Pervasives_Native.None
                          } in
                    ...
                    ((FStar_Pervasives_Native.fst
                        (FStarC_TypeChecker_Env.clear_expected_typ envbody1)),
                      uu___8, (FStar_Pervasives.Inr ()))
                  else
                    (let uu___8 =
                       let uu___9 =
                         let uu___10 =
                           let uu___11 = FStarC_Syntax_Subst.compress body1 in
                           uu___11.FStarC_Syntax_Syntax.n in
                         (c_opt, uu___10) in
                       match uu___9 with
                       | (FStar_Pervasives_Native.None,
                          FStarC_Syntax_Syntax.Tm_ascribed
                          { FStarC_Syntax_Syntax.tm = uu___10;
                            FStarC_Syntax_Syntax.asc =
                              (FStar_Pervasives.Inr expected_c, uu___11,
                               uu___12);
                            FStarC_Syntax_Syntax.eff_opt = uu___13;_})
                           -> FStar_Pervasives.Inr ()
                       | uu___10 ->
                           FStar_Pervasives.Inl
                             (FStarC_Option.dflt false use_eq_opt) in
                     (envbody1, body1, uu___8)) in
                match uu___6 with
                      ...
                      tc_term
                        {
                          ...
                          FStarC_TypeChecker_Env.rec_names =
                            (envbody2.FStarC_TypeChecker_Env.rec_names);
                          FStarC_TypeChecker_Env.top_level = false;
                          FStarC_TypeChecker_Env.check_uvars =
                          ...
                          FStarC_TypeChecker_Env.disc_proj_tab =
                            (envbody2.FStarC_TypeChecker_Env.disc_proj_tab);
                          FStarC_TypeChecker_Env.erasable_types_tab =
                          ...
                          FStarC_TypeChecker_Env.iface_todo =
                            (envbody2.FStarC_TypeChecker_Env.iface_todo);
                          FStarC_TypeChecker_Env.iface_hidden =
                            (envbody2.FStarC_TypeChecker_Env.iface_hidden);
                          FStarC_TypeChecker_Env.iface_lids =
                            (envbody2.FStarC_TypeChecker_Env.iface_lids);
                          FStarC_TypeChecker_Env.iface_val_lids =
                            (envbody2.FStarC_TypeChecker_Env.iface_val_lids)
                        } body2 in
                    (match uu___7 with
                     | (body3, cbody, guard_body) ->
                         ...
                         (match should_check_expected_effect with
                          | FStar_Pervasives.Inl use_eq ->
                              let uu___8 =
                                (cbody, FStarC_TypeChecker_Env.trivial_guard) in
                              (match uu___8 with
                               | (cbody1, g_lc) ->
                                   let uu___9 =
                                     FStarC_Errors.with_ctx
...
and check_application_args (env : FStarC_TypeChecker_Env.env)
  ...
  (let monadic_application head_info subst arg_comps_rev arg_rets_rev guard
     fvs bs =
     let uu___1 = head_info in
     match uu___1 with
     | (head1, chead1, ghead1, cres) ->
         ...
         (match uu___2 with
          | (cres1, guard1) ->
              let uu___3 =
                FStarC_TypeChecker_Util.check_no_escape
                  (FStarC_TypeChecker_Util.Escapes_application head1) env fvs
                                      ...
                                      (fun uu___9 ->
                                         ...
                                         | (uu___10, uu___11, lc) ->
                                             let uu___12 =
                                               FStarC_Syntax_Util.is_pure_or_ghost_comp
                                                 lc in
                                             Prims.not uu___12) arg_comps_rev
                                  else false in
                                let term =
                                            ...
                                            let uu___9 =
                                              FStarC_Option.map
                                                (FStarC_TypeChecker_Env.push_bv
                                                   env1) name_opt in
                                            FStarC_Option.dflt env1 uu___9) in
                                     let uu___9 =
                                       FStarC_List.fold_left
                                         (fun uu___10 uu___11 ->
                                                ...
                                                ((let uu___13 =
                                                 ...
                                                 (let env1 =
                                                    ...
                                                    | (hd, uu___15) ->
                                                        ...
                                                        (match uu___16 with
                                                         | FStarC_Syntax_Syntax.Tm_fvar
                                                             ...
                                                             let uu___17 =
                                                               let uu___18 =
                                                                 FStarC_TypeChecker_Cfg.is_built_in_primop
                                                                   fv in
                                                               if uu___18
                                                               then
                                                                 let uu___19
                                                  ...
                                                  let no_capture =
                                                    let uu___13 =
                                                      let uu___14 =
                                                        if
                                                          Prims.not
                                                            head_is_data_constructor
                                                        then
                                                          let uu___15 =
                                                            arg_head_is_reducible_primop
                                                              () in
                                                          Prims.not uu___15
                                                        else false in
                                                      if uu___14
                                                  ...
                                                  let uu___13 =
                                                    if no_capture
                                                    then
                                                      FStarC_TypeChecker_Util.bind_no_capture
                                   ...
                                   (match uu___8 with
                                    | (comp, g_bind) ->
                                        ...
                                        ((let uu___10 =
                                         ...
                                         (app, comp, g))))))))) in
   let rec tc_args head_info tc_args_state bs args1 =
     let uu___1 = tc_args_state in
     match uu___1 with
     | (subst, outargs, arg_rets, g, fvs) ->
         let instantiate_one_and_go rng b rest_bs args2 =
           ...
           | (tm, ty, aq, g') ->
               let uu___3 =
                 FStarC_TypeChecker_Util.check_no_escape
                   (FStarC_TypeChecker_Util.Escapes_application head) env fvs
                   ty in
               (match uu___3 with
                | (ty1, g_ex) ->
                    let guard =
                    ...
                    let uu___4 =
                      let uu___5 =
                        let uu___6 =
                          let uu___7 = FStarC_Syntax_Syntax.mk_Total ty1 in
         ...
         (match (bs, args1) with
          | ({ FStarC_Syntax_Syntax.binder_bv = x;
             ...
             []) when
              ...
              is_squash_typ env uu___5 ->
              instantiate_one_and_go head.FStarC_Syntax_Syntax.pos
                (FStarC_List.hd bs) rest []
          | ({ FStarC_Syntax_Syntax.binder_bv = x;
               FStarC_Syntax_Syntax.binder_qual =
                 FStar_Pervasives_Native.Some (FStarC_Syntax_Syntax.Implicit
                 uu___2);
             ...
             (uu___5, FStar_Pervasives_Native.None)::uu___6) ->
              ...
              instantiate_one_and_go r1 (FStarC_List.hd bs) rest args1
          | ({ FStarC_Syntax_Syntax.binder_bv = x;
               FStarC_Syntax_Syntax.binder_qual =
          ...
          | ({ FStarC_Syntax_Syntax.binder_bv = x;
             ...
             ({ FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_unknown;
              ...
              FStar_Pervasives_Native.Some
              { FStarC_Syntax_Syntax.aqual_implicit = true;
                FStarC_Syntax_Syntax.aqual_attributes = uu___5;_})::rest')
              ->
              let uu___6 =
                FStarC_Class_HasRange.pos
                  (FStarC_Syntax_Syntax.has_range_syntax ())
                  (FStar_Pervasives_Native.fst (FStarC_List.hd args1)) in
              instantiate_one_and_go uu___6 (FStarC_List.hd bs) rest rest'
          | ({ FStarC_Syntax_Syntax.binder_bv = x;
             ...
             (e, aq)::rest') ->
              let aq1 =
                check_expected_aqual_for_binder aq (FStarC_List.hd bs)
                  e.FStarC_Syntax_Syntax.pos in
              let targ =
                FStarC_Syntax_Subst.subst subst x.FStarC_Syntax_Syntax.sort in
              ...
              ((let uu___4 = FStarC_Debug.extreme () in
               ...
               (let uu___4 =
                  FStarC_TypeChecker_Util.check_no_escape
                    (FStarC_TypeChecker_Util.Escapes_application head) env
                    fvs targ in
                match uu___4 with
                | (targ1, g_ex) ->
                    let bound =
                      bound_of_flex true g.FStarC_TypeChecker_Common.deferred
                        targ1 in
                    let e0 = e in
                    let env1 =
                    ...
                    ((let uu___6 = FStarC_Debug.high () in
                     ...
                     (let uu___6 = tc_term env1 e in
                      ...
                      | (e1, c, g_e) ->
                          let uu___7 =
                            match bound with
                            | FStar_Pervasives_Native.None -> (e1, c, g_e)
                            | FStar_Pervasives_Native.Some b ->
                                let uu___8 =
                                  bound_of_flex false
                                    g_e.FStarC_TypeChecker_Common.deferred
                                    targ1 in
                                (match uu___8 with
                                 | FStar_Pervasives_Native.None ->
                                     (e1, c, g_e)
                                 | FStar_Pervasives_Native.Some t_e ->
                                     let uu___9 =
                                       let uu___10 =
                                         FStarC_Syntax_Syntax.mk_Total t_e in
                                       FStarC_TypeChecker_Util.maybe_coerce_lc
                                         env1 e1 uu___10 b in
                                     (match uu___9 with
                                      | (uu___10, c', uu___11) ->
                                          let uu___12 =
                                            let uu___13 =
                               ...
                               then
                                 let subst1 =
                                   maybe_extend_subst subst
                                     (FStarC_List.hd bs) e2 in
                                 tc_args head_info
                                   (subst1,
                                     ((arg,
                                        (FStar_Pervasives_Native.Some x1),
                                        c1) :: outargs), (xterm :: arg_rets),
                                     g1, fvs) rest rest'
                               else
                                 tc_args head_info
                                   (subst,
              ...
              (match uu___3 with
               | (head1, chead1, ghead1) ->
                   let rec aux norm1 solve ghead2 tres =
                     let tres1 =
                       let uu___4 =
                         let uu___5 = FStarC_Syntax_Subst.compress tres in
                         FStarC_Syntax_Util.unrefine uu___5 in
                       FStarC_Syntax_Util.unmeta_safe uu___4 in
                     match tres1.FStarC_Syntax_Syntax.n with
                     | FStarC_Syntax_Syntax.Tm_arrow uu___4 ->
                         let uu___5 =
                           FStarC_Syntax_Util.arrow_formals_comp_strict tres1 in
                         (match uu___5 with
                          | (bs1, cres') ->
                              let head_info1 = (head1, chead1, ghead2, cres') in
                     ...
                     | uu___4 when Prims.not norm1 ->
                         let rec norm_tres tres2 =
                           ...
                           let uu___5 =
                             let uu___6 = FStarC_Syntax_Subst.compress tres3 in
                             uu___6.FStarC_Syntax_Syntax.n in
                           match uu___5 with
                           | FStarC_Syntax_Syntax.Tm_refine
                               {
                                 FStarC_Syntax_Syntax.b2 =
                                   { FStarC_Syntax_Syntax.ppname = uu___6;
                                     FStarC_Syntax_Syntax.index = uu___7;
                                     FStarC_Syntax_Syntax.sort = tres4;_};
                                 FStarC_Syntax_Syntax.phi = uu___8;_}
                               -> norm_tres tres4
                           | uu___6 -> tres3 in
                         let uu___5 = norm_tres tres1 in
                         aux true solve ghead2 uu___5
                     | uu___4 when Prims.not solve ->
                         let ghead3 =
                         ...
                         let uu___5 =
                           let uu___6 =
                             ...
                             FStar_Pprint.prefix (Prims.of_int 4)
                               Prims.int_one
                               (FStarC_Errors_Msg.text
                                  "Too many arguments to function of type")
                               uu___7 in
                           let uu___7 =
                             let uu___8 =
                               let uu___9 =
                               ...
                               FStar_Pprint.op_Hat_Slash_Hat
                                 (FStarC_Errors_Msg.text "Got") uu___9 in
                             let uu___9 =
                               let uu___10 =
                                 let uu___11 =
                                   FStarC_Class_PP.pp
                                     FStarC_Syntax_Print.pretty_term tres1 in
                                 FStar_Pprint.prefix (Prims.of_int 4)
                                   Prims.int_one
                                   (FStarC_Errors_Msg.text
                                      "Remaining type is") uu___11 in
                               [uu___10] in
                             uu___8 :: uu___9 in
                           uu___6 :: uu___7 in
                         FStarC_Errors.raise_error
                           FStarC_Class_HasRange.hasRange_range
                           (FStarC_Syntax_Syntax.argpos arg)
                           FStarC_Errors_Codes.Fatal_TooManyArgumentsToFunction
                           ()
                           (Obj.magic
                              FStarC_Errors_Msg.is_error_message_list_doc)
                           (Obj.magic uu___5) in
                   aux false false ghead1
                     (FStarC_Syntax_Util.comp_result chead1))) in
   let rec check_function_app tf guard =
     ...
     let uu___1 =
       ...
       uu___2.FStarC_Syntax_Syntax.n in
     match uu___1 with
     | FStarC_Syntax_Syntax.Tm_uvar uu___2 when
         let uu___3 =
           let uu___4 =
             let uu___5 =
               let uu___6 = FStarC_Syntax_Util.unmeta tf1 in
               FStarC_Syntax_Util.leftmost_head uu___6 in
     ...
     | FStarC_Syntax_Syntax.Tm_app uu___2 when
         let uu___3 =
           let uu___4 =
             let uu___5 =
               let uu___6 = FStarC_Syntax_Util.unmeta tf1 in
     ...
     | FStarC_Syntax_Syntax.Tm_arrow uu___2 ->
         let uu___3 =
           let uu___4 = FStarC_Syntax_Util.unmeta tf1 in
           FStarC_Syntax_Util.arrow_formals_comp_strict uu___4 in
         (match uu___3 with
          | (bs, c) ->
     ...
     | FStarC_Syntax_Syntax.Tm_refine
         { FStarC_Syntax_Syntax.b2 = bv; FStarC_Syntax_Syntax.phi = uu___2;_}
   ...
   check_function_app thead
     (FStarC_Class_Monoid.mzero FStarC_TypeChecker_Common.monoid_guard_t))
and maybe_elaborate_short_circuit_args (env0 : FStarC_TypeChecker_Env.env)
  (head : FStarC_Syntax_Syntax.term) (args : FStarC_Syntax_Syntax.args) :
  (FStarC_Syntax_Syntax.term * FStarC_Syntax_Syntax.comp *
    FStarC_TypeChecker_Env.guard_t) FStar_Pervasives_Native.option=
  let uu___ = FStarC_Syntax_Util.head_and_args_full head in
  match uu___ with
  | (head1, args') ->
      let args1 = FStarC_List.op_At args' args in
      let uu___1 =
        let uu___2 = FStarC_Syntax_Util.un_uinst head1 in
        uu___2.FStarC_Syntax_Syntax.n in
      (match uu___1 with
       | FStarC_Syntax_Syntax.Tm_fvar fv when
           (FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.op_And) ||
             (FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.op_Or)
           ->
           (match args1 with
            | (e1, _aq1)::(e2, _aq2)::[] ->
                let is_and =
                  FStarC_Syntax_Syntax.fv_eq_lid fv
                    FStarC_Parser_Const.op_And in
                let r = e1.FStarC_Syntax_Syntax.pos in
                let env1 =
                  FStarC_TypeChecker_Env.set_expected_typ env0
                    FStarC_Syntax_Util.t_bool in
                let uu___2 = tc_term env1 e1 in
                (match uu___2 with
                 | (e11, c1, g1) ->
                     let uu___3 = tc_term env1 e2 in
                     (match uu___3 with
                      | (e21, c2, g2) ->
                          let uu___4 =
                            FStarC_TypeChecker_Util.bind r false env0
                           ...
                           | (c, g_c) ->
                               ...
                               then
                                 ...
                                 let e =
                                   ...
                                   if is_and
                                   then
                                     FStarC_Syntax_Util.if_then_else x11 e21
                                       FStarC_Syntax_Util.exp_false_bool
                                   else
                                     FStarC_Syntax_Util.if_then_else x11
                                       FStarC_Syntax_Util.exp_true_bool e21 in
                                 let lb =
                                   FStarC_Syntax_Util.mk_letbinding
                                     (FStar_Pervasives.Inl x1) []
                                     FStarC_Syntax_Util.t_bool
                                     (FStarC_Syntax_Util.comp_effect_name c1)
                                     e11 [] e11.FStarC_Syntax_Syntax.pos in
                                 let e3 =
                                   let uu___6 =
                                     let uu___7 =
                                       let uu___8 =
                                         FStarC_Syntax_Subst.close
                                           [FStarC_Syntax_Syntax.mk_binder x1]
                                           e in
                                       {
                                         FStarC_Syntax_Syntax.lbs =
                                           (false, [lb]);
                                         FStarC_Syntax_Syntax.body1 = uu___8
                                       } in
                                     FStarC_Syntax_Syntax.Tm_let uu___7 in
                                 ...
                                 (let uu___6 =
                                  ...
                                  then
                                    let e =
                                      if is_and
                                      then
                                        FStarC_Syntax_Util.if_then_else e11
                                          e21
                                          FStarC_Syntax_Util.exp_false_bool
                                      else
                                        FStarC_Syntax_Util.if_then_else e11
                                    ...
                                    (let e =
                                       if is_and
                                       then FStarC_Syntax_Util.mk_and e11 e21
                                       else FStarC_Syntax_Util.mk_or e11 e21 in
                                     let uu___7 =
                                       let uu___8 =
                                         FStarC_Class_Monoid.op_Plus_Plus
                                           FStarC_TypeChecker_Common.monoid_guard_t
...
and check_short_circuit_args (env : FStarC_TypeChecker_Env.env)
  (head : FStarC_Syntax_Syntax.term) (chead : FStarC_Syntax_Syntax.comp)
  (g_head : FStarC_TypeChecker_Common.guard_t)
  (args : FStarC_Syntax_Syntax.args)
  (expected_topt :
    (FStarC_Syntax_Syntax.typ * Prims.bool) FStar_Pervasives_Native.option)
  :
  ...
  let r = FStarC_TypeChecker_Env.get_range env in
  let tf =
    FStarC_Syntax_Subst.compress (FStarC_Syntax_Util.comp_result chead) in
  let formals_opt =
    match tf.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Tm_arrow uu___ ->
        let uu___1 = FStarC_Syntax_Util.arrow_formals_comp_strict tf in
        FStar_Pervasives_Native.Some uu___1
    | uu___ -> FStar_Pervasives_Native.None in
  match formals_opt with
  | FStar_Pervasives_Native.Some (bs, c) when
      let uu___ = FStarC_Syntax_Util.is_total_comp c in
      if uu___
      then (FStarC_List.length bs) = (FStarC_List.length args)
      else false ->
                  ...
                  | (e1, c1, g) ->
                      ...
                      let ghost1 =
                        ...
                        then true
                          ...
                          (let uu___4 =
                             let uu___5 = FStarC_Syntax_Util.is_total_comp c1 in
                             Prims.not uu___5 in
                           if uu___4
                           then
                             Prims.not
      ...
      (match uu___ with
       | (args1, guard, ghost) ->
           ...
           let g =
             FStarC_TypeChecker_Util.simplify_and_label_guard
               FStar_Pervasives_Native.None env guard in
           (e, c1, g))
...
and tc_pat (env : FStarC_TypeChecker_Env.env)
  ...
  (FStarC_Syntax_Syntax.pat * FStarC_Syntax_Syntax.bv Prims.list *
    ...
    FStarC_TypeChecker_Common.guard_t * Prims.bool)=
  let fail msg =
    FStarC_Errors.raise_error FStarC_Class_HasRange.hasRange_range
      ...
      then t
        ...
        (let uu___1 =
         ...
         | FStar_Pervasives_Native.None ->
             let uu___2 =
               let uu___3 =
                 FStarC_TypeChecker_Env.disc_proj_qual env1
                   f.FStarC_Syntax_Syntax.fv_name in
               match uu___3 with
               | FStar_Pervasives_Native.None -> true
               | uu___4 -> false in
             ...
             then t
               ...
               (let t' =
                ...
                let uu___3 = FStarC_Syntax_Util.term_eq t' t in
                if uu___3 then t else aux false t')
  ...
  let pat_typ_ok env1 pat_t1 scrutinee_t =
    ...
    (let fail1 msg_str =
       let msg =
         if msg_str = "" then [] else [FStarC_Errors_Msg.text msg_str] in
       let msg1 =
         let uu___2 =
           let uu___3 =
             let uu___4 =
             ...
             FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
               (FStarC_Errors_Msg.text "Type of pattern") uu___4 in
           let uu___4 =
             let uu___5 =
               FStarC_Class_PP.pp FStarC_Syntax_Print.pretty_term scrutinee_t in
             FStar_Pprint.prefix (Prims.of_int 2) Prims.int_one
               (FStarC_Errors_Msg.text "does not match type of scrutinee")
               uu___5 in
       ...
       FStarC_Errors.raise_error FStarC_Class_HasRange.hasRange_range
         ...
         (Obj.magic FStarC_Errors_Msg.is_error_message_list_doc)
         (Obj.magic msg1) in
     let uu___2 = FStarC_Syntax_Util.head_and_args_full scrutinee_t in
     match uu___2 with
     | (head_s, args_s) ->
         let pat_t2 =
           FStarC_TypeChecker_Normalize.normalize
         ...
         (match uu___3 with
          | { FStarC_Syntax_Syntax.n = FStarC_Syntax_Syntax.Tm_fvar uu___4;
              ...
              let uu___7 = FStarC_Syntax_Util.head_and_args_full pat_t2 in
              (match uu___7 with
               | (head_p, args_p) ->
                   let uu___8 =
                   ...
                   then
                     let uu___9 =
                       let uu___10 = FStarC_Syntax_Util.un_uinst head_p in
                       uu___10.FStarC_Syntax_Syntax.n in
                     (match uu___9 with
                          ...
                          ((let uu___11 =
                              let uu___12 =
                                FStarC_TypeChecker_Env.is_type_constructor
                                  env1 (FStarC_Syntax_Syntax.lid_of_fv f) in
                              Prims.not uu___12 in
                            if uu___11
                            then
                           ...
                           (let uu___12 =
                              ...
                              | FStar_Pervasives_Native.Some n ->
                                  let uu___14 = FStarC_Util.first_N n args_p in
                                  (match uu___14 with
                                   | (params_p, uu___15) ->
                                       let uu___16 =
                     ...
                     (let uu___9 =
                        ...
                        FStarC_Format.fmt2 "Head mismatch %s vs %s" uu___10
                          uu___11 in
                      fail1 uu___9))
          | uu___4 ->
              let uu___5 =
         ...
         | FStarC_Syntax_Syntax.Tm_uinst
             ...
             let uu___5 =
               ...
               | FStarC_Syntax_Syntax.Tm_uinst (head1, us) ->
                   let uu___6 = head1.FStarC_Syntax_Syntax.n in
                   (match uu___6 with
                    | FStarC_Syntax_Syntax.Tm_fvar f ->
                        let res =
                        ...
                        (match res with
                         | FStar_Pervasives_Native.Some (t, uu___7) when
             ...
             (match uu___5 with
              | (head1, (us, t_f)) ->
                  let uu___6 = FStarC_Syntax_Util.arrow_formals t_f in
                  (match uu___6 with
                   | (formals, t) ->
                       let erasable =
                       ...
                       (if
                        ...
                        (let rec aux aux_state formals1 args1 =
                           let uu___8 = aux_state in
                           match uu___8 with
                           | (subst, args_out, bvs, guard) ->
                               (match (formals1, args1) with
                                | ([], []) ->
                                    ...
                                    (pat_e, uu___9, bvs, guard, erasable)
                                    ...
                                    let uu___12 =
                                      ...
                                      | FStarC_Syntax_Syntax.Tm_name x ->
                                          ...
                                          ((a1, imp_a), subst1,
                                            ...
                                            (FStarC_Class_Monoid.mzero
                                               FStarC_TypeChecker_Common.monoid_guard_t))
                                      | FStarC_Syntax_Syntax.Tm_uvar uu___14
                                          ->
                                          let use_eq = true in
                                          let env2 =
                                            FStarC_TypeChecker_Env.set_expected_typ_maybe_eq
  ...
  let rec check_nested_pattern env1 p t =
    ...
    (let id t1 =
       let y =
         FStarC_Syntax_Syntax.gen_bv "y" FStar_Pervasives_Native.None t1 in
       let uu___1 = FStarC_Syntax_Syntax.bv_to_name y in
       FStarC_Syntax_Util.abs [FStarC_Syntax_Syntax.mk_binder y] uu___1
         FStar_Pervasives_Native.None in
     let mk_disc_t disc inner_t =
       ...
       let tm =
         ...
         FStarC_Syntax_Syntax.mk_Tm_app disc uu___1
           FStarC_Range_Type.dummyRange in
       let tm1 =
         FStarC_Syntax_Syntax.mk_Tm_app inner_t
           [FStarC_Syntax_Syntax.as_arg tm] FStarC_Range_Type.dummyRange in
                      ...
                      (let uu___9 =
                         let uu___10 =
                           FStarC_TypeChecker_Rel.teq_nosmt_force env1
                             (FStarC_Syntax_Util.comp_result lc) expected_t in
     ...
     | FStarC_Syntax_Syntax.Pat_cons
         ...
         (match uu___2 with
          | (rdc, uu___3, constructor_fv) ->
              ...
              (if Prims.not rdc.FStarC_Syntax_DsEnv.is_record
               then
                 (let rdc_field_names =
                    FStarC_List.map
                      (fun uu___5 ->
                         match uu___5 with
                         | (i, uu___6, uu___7) -> FStarC_Ident.string_of_id i)
                      rdc.FStarC_Syntax_DsEnv.fields in
                  let bad_field =
                    FStarC_List.tryFind
                      (fun i ->
                         let uu___5 =
                           FStarC_List.existsb
                             (fun f ->
                                f =
                                  (FStarC_Ident.string_of_id
                                     (FStarC_Ident.ident_of_lid i)))
                             rdc_field_names in
                         Prims.not uu___5) uc.FStarC_Syntax_Syntax.uc_fields in
                  match bad_field with
                  | FStar_Pervasives_Native.Some f ->
                      let uu___5 =
                        let uu___6 =
                          ...
                          FStar_Pprint.op_Hat_Slash_Hat
                            (FStarC_Errors_Msg.text
                               "Expected a pattern of type") uu___7 in
                        let uu___7 =
                          let uu___8 =
                            let uu___9 =
                              let uu___10 =
                                  ...
                                  let uu___13 =
                                    ...
                                    FStar_Pprint.op_Hat_Slash_Hat uu___14
                                      (FStarC_Errors_Msg.text
                                         "is not valid for this type.") in
                                  FStar_Pprint.op_Hat_Slash_Hat
                                    (FStarC_Errors_Msg.text "field") uu___13 in
                                FStar_Pprint.op_Hat_Slash_Hat
                                  (FStarC_Errors_Msg.text
                                     "is not declared as a record type;")
                                  uu___12 in
                    ...
                    (fun uu___5 is_imp ->
                       ...
                       FStar_Pervasives_Native.Some
                         ((FStarC_Syntax_Syntax.withinfo
                             (FStarC_Syntax_Syntax.Pat_var x)
                             p.FStarC_Syntax_Syntax.p), false))
                    p.FStarC_Syntax_Syntax.p in
                let sub_pats2 =
                  FStarC_List.map
                    (fun uu___5 ->
                       match uu___5 with | ((p1, uu___6), b) -> (p1, b))
                    sub_pats1 in
                let p1 =
                  {
                    FStarC_Syntax_Syntax.v =
                      (FStarC_Syntax_Syntax.Pat_cons
                         (constructor_fv, us_opt, sub_pats2));
                    FStarC_Syntax_Syntax.p = (p.FStarC_Syntax_Syntax.p)
                  } in
                let p2 =
  ...
  (let uu___1 =
     let uu___2 = FStarC_TypeChecker_PatternUtils.elaborate_pat env p0 in
     let uu___3 = expected_pat_typ env p0.FStarC_Syntax_Syntax.p pat_t in
     check_nested_pattern
       (FStar_Pervasives_Native.fst
          (FStarC_TypeChecker_Env.clear_expected_typ env)) uu___2 uu___3 in
   match uu___1 with
       ...
       | (cpat, uu___2, cbr) ->
           ...
           (match uu___3 with
            | (scrutinee_env, uu___4) ->
                let uu___5 =
                  tc_pat (FStarC_TypeChecker_Env.push_bv env scrutinee) pat_t
                    pattern in
                (match uu___5 with
                         ...
                         | FStar_Pervasives_Native.Some e ->
                             ...
                             then
                               ...
                               (let uu___9 =
                                  tc_term
                                    (FStarC_TypeChecker_Env.set_expected_typ
                                       pat_env FStarC_Syntax_Util.t_bool) e in
                                match uu___9 with
                                | (e1, c, g) ->
                                    ...
                                    then FStarC_Syntax_Util.exp_true_bool
                                      ...
                                      (let rec build_branch_guard
                                         ...
                                         let fail uu___11 =
                                           ...
                                           FStarC_Effect.failwith uu___12 in
                                         ...
                                         let pat_exp2 =
                                           let uu___11 =
                                             FStarC_Syntax_Subst.compress
                                               pat_exp1 in
                                           FStarC_Syntax_Util.unmeta uu___11 in
                                         ...
                                         (match uu___13 with
                                          | (c_weak, g_when_weak, g_branch2)
                                              ...
                                              let g_branch3 =
                                                let uu___14 =
                                                  FStarC_TypeChecker_Env.close_guard
                                                    env binders g_branch2 in
                                                FStarC_TypeChecker_Util.close_guard_implicits
                                                  env false binders uu___14 in
                                              let subst_pat_bvs_in_res_typ
                                                c_weak1 =
                                                let env_s =
                                                  FStarC_TypeChecker_Env.push_bv
                                                    env scrutinee in
                                                  ...
                                                  (let env_s1 =
                                                     {
                                                       FStarC_TypeChecker_Env.solver
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.solver);
                                                       FStarC_TypeChecker_Env.range
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.range);
                                                       FStarC_TypeChecker_Env.curmodule
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.curmodule);
                                                       FStarC_TypeChecker_Env.gamma
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.gamma);
                                                       FStarC_TypeChecker_Env.gamma_sig
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.gamma_sig);
                                                       FStarC_TypeChecker_Env.gamma_cache
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.gamma_cache);
                                                       FStarC_TypeChecker_Env.modules
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.modules);
                                                       FStarC_TypeChecker_Env.expected_typ
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.expected_typ);
                                                       FStarC_TypeChecker_Env.sigtab
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.sigtab);
                                                       FStarC_TypeChecker_Env.attrtab
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.attrtab);
                                                       FStarC_TypeChecker_Env.instantiate_imp
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.instantiate_imp);
                                                       FStarC_TypeChecker_Env.effects
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.effects);
                                                       FStarC_TypeChecker_Env.generalize
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.generalize);
                                                       FStarC_TypeChecker_Env.letrecs
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.letrecs);
                                                       FStarC_TypeChecker_Env.rec_names
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.rec_names);
                                                       FStarC_TypeChecker_Env.top_level
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.top_level);
                                                       FStarC_TypeChecker_Env.check_uvars
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.check_uvars);
                                                       FStarC_TypeChecker_Env.use_eq_strict
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.use_eq_strict);
                                                       FStarC_TypeChecker_Env.is_iface
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.is_iface);
                                                       FStarC_TypeChecker_Env.admit
                                                         = true;
                                                       FStarC_TypeChecker_Env.phase1
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.phase1);
                                                       FStarC_TypeChecker_Env.failhard
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.failhard);
                                                       FStarC_TypeChecker_Env.flychecking
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.flychecking);
                                                       FStarC_TypeChecker_Env.uvar_subtyping
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.uvar_subtyping);
                                                       FStarC_TypeChecker_Env.intactics
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.intactics);
                                                       FStarC_TypeChecker_Env.nocoerce
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.nocoerce);
                                                       FStarC_TypeChecker_Env.tc_term
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.tc_term);
                                                       FStarC_TypeChecker_Env.typeof_tot_or_gtot_term
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.typeof_tot_or_gtot_term);
                                                       FStarC_TypeChecker_Env.universe_of
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.universe_of);
                                                       FStarC_TypeChecker_Env.typeof_well_typed_tot_or_gtot_term
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.typeof_well_typed_tot_or_gtot_term);
                                                       FStarC_TypeChecker_Env.teq_nosmt_force
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.teq_nosmt_force);
                                                       FStarC_TypeChecker_Env.subtype_nosmt_force
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.subtype_nosmt_force);
                                                       FStarC_TypeChecker_Env.qtbl_name_and_index
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.qtbl_name_and_index);
                                                       FStarC_TypeChecker_Env.fv_delta_depths
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.fv_delta_depths);
                                                       FStarC_TypeChecker_Env.proof_ns
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.proof_ns);
                                                       FStarC_TypeChecker_Env.synth_hook
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.synth_hook);
                                                       FStarC_TypeChecker_Env.try_solve_implicits_hook
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.try_solve_implicits_hook);
                                                       FStarC_TypeChecker_Env.splice
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.splice);
                                                       FStarC_TypeChecker_Env.mpreprocess
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.mpreprocess);
                                                       FStarC_TypeChecker_Env.postprocess
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.postprocess);
                                                       FStarC_TypeChecker_Env.identifier_info
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.identifier_info);
                                                       FStarC_TypeChecker_Env.tc_hooks
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.tc_hooks);
                                                       FStarC_TypeChecker_Env.dsenv
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.dsenv);
                                                       FStarC_TypeChecker_Env.nbe
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.nbe);
                                                       FStarC_TypeChecker_Env.strict_args_tab
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.strict_args_tab);
                                                       FStarC_TypeChecker_Env.disc_proj_tab
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.disc_proj_tab);
                                                       FStarC_TypeChecker_Env.erasable_types_tab
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.erasable_types_tab);
                                                       FStarC_TypeChecker_Env.enable_defer_to_tac
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.enable_defer_to_tac);
                                                       FStarC_TypeChecker_Env.unif_allow_ref_guards
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.unif_allow_ref_guards);
                                                       FStarC_TypeChecker_Env.erase_erasable_args
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.erase_erasable_args);
                                                       FStarC_TypeChecker_Env.core_check
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.core_check);
                                                       FStarC_TypeChecker_Env.missing_decl
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.missing_decl);
                                                       FStarC_TypeChecker_Env.iface_todo
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.iface_todo);
                                                       FStarC_TypeChecker_Env.iface_hidden
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.iface_hidden);
                                                       FStarC_TypeChecker_Env.iface_lids
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.iface_lids);
                                                       FStarC_TypeChecker_Env.iface_val_lids
                                                         =
                                                         (env_s.FStarC_TypeChecker_Env.iface_val_lids)
                                                     } in
                                                   let substs =
                                                     FStarC_List.fold_left2
                                                       (fun substs1 pat_bv_tm
                                                          bv ->
                                                          let expected_t =
                                                            FStarC_Syntax_Subst.subst
                                                              substs1
                                                    ...
                                                    let pat_bv_tms2 =
                                                      let env1 =
                                                            ...
                                                            env scrutinee in
                                                        {
                                                          FStarC_TypeChecker_Env.solver
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.solver);
                                                          FStarC_TypeChecker_Env.range
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.range);
                                                          FStarC_TypeChecker_Env.curmodule
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.curmodule);
                                                          FStarC_TypeChecker_Env.gamma
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.gamma);
                                                          FStarC_TypeChecker_Env.gamma_sig
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.gamma_sig);
                                                          FStarC_TypeChecker_Env.gamma_cache
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.gamma_cache);
                                                          FStarC_TypeChecker_Env.modules
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.modules);
                                                          FStarC_TypeChecker_Env.expected_typ
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.expected_typ);
                                                          FStarC_TypeChecker_Env.sigtab
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.sigtab);
                                                          FStarC_TypeChecker_Env.attrtab
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.attrtab);
                                                          FStarC_TypeChecker_Env.instantiate_imp
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.instantiate_imp);
                                                          FStarC_TypeChecker_Env.effects
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.effects);
                                                          FStarC_TypeChecker_Env.generalize
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.generalize);
                                                          FStarC_TypeChecker_Env.letrecs
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.letrecs);
                                                          FStarC_TypeChecker_Env.rec_names
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.rec_names);
                                                          FStarC_TypeChecker_Env.top_level
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.top_level);
                                                          FStarC_TypeChecker_Env.check_uvars
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.check_uvars);
                                                          FStarC_TypeChecker_Env.use_eq_strict
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.use_eq_strict);
                                                          FStarC_TypeChecker_Env.is_iface
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.is_iface);
                                                          FStarC_TypeChecker_Env.admit
                                                            = true;
                                                          FStarC_TypeChecker_Env.phase1
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.phase1);
                                                          FStarC_TypeChecker_Env.failhard
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.failhard);
                                                          FStarC_TypeChecker_Env.flychecking
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.flychecking);
                                                          FStarC_TypeChecker_Env.uvar_subtyping
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.uvar_subtyping);
                                                          FStarC_TypeChecker_Env.intactics
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.intactics);
                                                          FStarC_TypeChecker_Env.nocoerce
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.nocoerce);
                                                          FStarC_TypeChecker_Env.tc_term
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.tc_term);
                                                          FStarC_TypeChecker_Env.typeof_tot_or_gtot_term
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.typeof_tot_or_gtot_term);
                                                          FStarC_TypeChecker_Env.universe_of
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.universe_of);
                                                          FStarC_TypeChecker_Env.typeof_well_typed_tot_or_gtot_term
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.typeof_well_typed_tot_or_gtot_term);
                                                          FStarC_TypeChecker_Env.teq_nosmt_force
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.teq_nosmt_force);
                                                          FStarC_TypeChecker_Env.subtype_nosmt_force
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.subtype_nosmt_force);
                                                          FStarC_TypeChecker_Env.qtbl_name_and_index
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.qtbl_name_and_index);
                                                          FStarC_TypeChecker_Env.fv_delta_depths
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.fv_delta_depths);
                                                          FStarC_TypeChecker_Env.proof_ns
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.proof_ns);
                                                          FStarC_TypeChecker_Env.synth_hook
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.synth_hook);
                                                          FStarC_TypeChecker_Env.try_solve_implicits_hook
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.try_solve_implicits_hook);
                                                          FStarC_TypeChecker_Env.splice
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.splice);
                                                          FStarC_TypeChecker_Env.mpreprocess
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.mpreprocess);
                                                          FStarC_TypeChecker_Env.postprocess
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.postprocess);
                                                          FStarC_TypeChecker_Env.identifier_info
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.identifier_info);
                                                          FStarC_TypeChecker_Env.tc_hooks
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.tc_hooks);
                                                          FStarC_TypeChecker_Env.dsenv
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.dsenv);
                                                          FStarC_TypeChecker_Env.nbe
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.nbe);
                                                          FStarC_TypeChecker_Env.strict_args_tab
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.strict_args_tab);
                                                          FStarC_TypeChecker_Env.disc_proj_tab
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.disc_proj_tab);
                                                          FStarC_TypeChecker_Env.erasable_types_tab
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.erasable_types_tab);
                                                          FStarC_TypeChecker_Env.enable_defer_to_tac
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.enable_defer_to_tac);
                                                          FStarC_TypeChecker_Env.unif_allow_ref_guards
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.unif_allow_ref_guards);
                                                          FStarC_TypeChecker_Env.erase_erasable_args
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.erase_erasable_args);
                                                          FStarC_TypeChecker_Env.core_check
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.core_check);
                                                          FStarC_TypeChecker_Env.missing_decl
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.missing_decl);
                                                          FStarC_TypeChecker_Env.iface_todo
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.iface_todo);
                                                          FStarC_TypeChecker_Env.iface_hidden
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.iface_hidden);
                                                          FStarC_TypeChecker_Env.iface_lids
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.iface_lids);
                                                          FStarC_TypeChecker_Env.iface_val_lids
                                                            =
                                                            (uu___15.FStarC_TypeChecker_Env.iface_val_lids)
                                                        } in
                                                      let uu___15 =
                                                        let uu___16 =
                                                          FStarC_List.fold_left2
                                                    ...
                                                    (let g =
                                                       ...
                                                       | FStar_Pervasives_Native.Some
                                                           ...
                                                           FStarC_TypeChecker_Common.weaken_guard_formula
                                                             FStarC_TypeChecker_Env.trivial_guard
                                                             eqs1 in
                                                     FStarC_TypeChecker_Util.close_layered_comp_with_substitutions
                                                       (FStarC_TypeChecker_Env.push_bv
                                                          env scrutinee)
                                                       pat_bvs pat_bv_tms2
                                                       c_weak2 g)))
                                                else
                                                  FStarC_TypeChecker_Util.close_comp_and_guard
                                                    (FStarC_TypeChecker_Env.push_bv
                                                       env scrutinee) pat_bvs
                                                    c_weak2
                                                    FStarC_TypeChecker_Env.trivial_guard in
                                              let uu___14 =
                                                FStarC_TypeChecker_Env.close_guard
...
and check_nonempty_result (env : FStarC_TypeChecker_Env.env)
    ...
    let uu___ =
      let uu___1 =
        let uu___2 =
          FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t in
        FStarC_Format.fmt1
          "This definition has a non-total effect, so its type must be provably inhabited; could not prove `nonempty (%s)`"
          uu___2 in
      FStarC_Errors_Msg.mkmsg uu___1 in
    let uu___1 =
      let uu___2 =
        let uu___3 = FStarC_Syntax_Util.mk_nonempty u t in
        FStarC_TypeChecker_Common.NonTrivial uu___3 in
      FStarC_TypeChecker_Env.guard_of_guard_formula uu___2 in
    FStarC_TypeChecker_Util.label_guard
      (FStarC_TypeChecker_Env.get_range env) uu___ uu___1 in
      ...
      (match uu___ with
       | (e1, univ_vars, c1, g1, topt) ->
           ...
           let uu___1 =
             if
               annotated &&
                 (Prims.not env1.FStarC_TypeChecker_Env.generalize)
             then
               let uu___2 =
                 FStarC_TypeChecker_Normalize.reduce_uvar_solutions env1 e1 in
               (g1, uu___2, univ_vars, c1, true)
             else
               (let g11 =
                  let uu___2 =
                    FStarC_TypeChecker_Rel.solve_deferred_constraints env1 g1 in
                  FStarC_TypeChecker_Rel.resolve_implicits env1 uu___2 in
                let uu___2 = (c1, FStarC_TypeChecker_Env.trivial_guard) in
                match uu___2 with
                | (comp1, g_comp1) ->
                    let g12 =
                      FStarC_Class_Monoid.op_Plus_Plus
                    ...
                    (match uu___3 with
                     | (uu___4, univs, e11, c11, gvs) ->
                         ...
                         (g15, e11, univs, c11,
                           ((match univs with | [] -> true | uu___5 -> false)
                              &&
                              ((match gvs with | [] -> true | uu___5 -> false)))))) in
           (match uu___1 with
            | (g11, e11, univ_vars1, c11, annot_usable) ->
                let uu___2 =
                  let uu___3 =
                    let uu___4 =
                  ...
                  | (action, c12) ->
                      (match action with
                       ...
                       | FStarC_TypeChecker_Util.Mask_effect_silently ->
                           let c13 =
                             match topt with
                             | FStar_Pervasives_Native.Some t when
                                 Prims.not
                                   env1.FStarC_TypeChecker_Env.generalize
                                 -> FStarC_Syntax_Util.set_result_typ c12 t
                             | FStar_Pervasives_Native.Some uu___4 -> c12
                             | FStar_Pervasives_Native.None ->
                                 if
                                   (match action with
                                    | FStarC_TypeChecker_Util.Mask_effect_silently
                                 ...
                                 then c12
                                   ...
                                   (let uu___4 =
                                    ...
                                    FStarC_Syntax_Util.set_result_typ c12
                                      uu___4) in
                           (if
                              (match action with
                               | FStarC_TypeChecker_Util.Mask_effect_and_warn
                                   -> true
                               | uu___5 -> false) &&
                                (Prims.not env1.FStarC_TypeChecker_Env.phase1)
                            then
                              (FStarC_TypeChecker_Err.warn_top_level_effect
                                 (FStarC_TypeChecker_Env.get_range env1);
                               (let uu___6 =
                                  FStarC_TypeChecker_Env.push_univ_vars env1
                                    univ_vars1 in
                                check_nonempty_result uu___6
                                  (FStarC_Syntax_Util.comp_result c13)))
                            else ();
                            (let uu___5 =
                               FStarC_Syntax_Syntax.mk
                                 (FStarC_Syntax_Syntax.Tm_meta
                     ...
                     ((let uu___4 = FStarC_Debug.medium () in
                      ...
                      (let e12 =
                       ...
                       (let cres =
                        ...
                        let lbtyp =
                          match topt with
                          | FStar_Pervasives_Native.Some t when annot_usable
                              -> t
                          | uu___5 -> FStarC_Syntax_Util.comp_result c12 in
                        let lb1 =
                          FStarC_Syntax_Util.close_univs_and_mk_letbinding
                            FStar_Pervasives_Native.None
                            lb.FStarC_Syntax_Syntax.lbname univ_vars1 lbtyp
                        ...
                        (uu___5, cres,
                          (FStarC_Class_Monoid.mzero
                             FStarC_TypeChecker_Common.monoid_guard_t))))))))
  | uu___ ->
      FStarC_Effect.failwith "Impossible: check_top_level_let: not a let"
and maybe_intro_smt_lemma (env : FStarC_TypeChecker_Env.env)
  (lem_typ : FStarC_Syntax_Syntax.term) (c2 : FStarC_Syntax_Syntax.comp)
  (g_c2 : FStarC_TypeChecker_Env.guard_t) :
  (FStarC_Syntax_Syntax.comp * FStarC_TypeChecker_Env.guard_t)=
  ...
  then
    ...
    let uu___1 = FStarC_TypeChecker_Common.weaken_guard_formula g_c2 quant in
    (c2, uu___1)
  else (c2, g_c2)
and check_inner_let (env : FStarC_TypeChecker_Env.env)
  ...
  | FStarC_Syntax_Syntax.Tm_let
      ...
      let uu___ =
        check_let_bound_def false
          (FStar_Pervasives_Native.fst
             (FStarC_TypeChecker_Env.clear_expected_typ env2)) lb in
      (match uu___ with
       | (e1, uu___1, c1, g1, topt) ->
           let annotated =
             match topt with
             | FStar_Pervasives_Native.Some v -> true
             | uu___2 -> false in
           let pure_or_ghost = FStarC_Syntax_Util.is_pure_or_ghost_comp c1 in
           let is_inline_let =
             FStarC_Util.for_some
           ...
           let is_inline_let_vc =
             FStarC_Util.for_some
               ...
               lb.FStarC_Syntax_Syntax.lbattrs in
           ((let uu___3 =
               if is_inline_let || is_inline_let_vc
               then
                 let uu___4 =
                   if pure_or_ghost
                   then true
             ...
             | (xb, e21) ->
                 ...
                 let uu___4 =
                   ...
                   | (e22, c2, g2) ->
                       let g21 =
                         FStarC_TypeChecker_Util.simplify_and_label_guard
                           FStar_Pervasives_Native.None env_x g2 in
                       let g2_logical =
                         {
                           FStarC_TypeChecker_Common.guard_f =
                             (g21.FStarC_TypeChecker_Common.guard_f);
                           FStarC_TypeChecker_Common.deferred_to_tac =
                       ...
                       (e22, c2, g2_logical,
                         {
                           FStarC_TypeChecker_Common.guard_f =
                             FStarC_TypeChecker_Common.Trivial;
                           FStarC_TypeChecker_Common.deferred_to_tac =
                             (g21.FStarC_TypeChecker_Common.deferred_to_tac);
                           FStarC_TypeChecker_Common.deferred =
                             (g21.FStarC_TypeChecker_Common.deferred);
                           FStarC_TypeChecker_Common.univ_ineqs =
                             (g21.FStarC_TypeChecker_Common.univ_ineqs);
                           FStarC_TypeChecker_Common.implicits =
                             (g21.FStarC_TypeChecker_Common.implicits)
                         }) in
                 (match uu___4 with
                  | (e22, c2, g_c2, g2) ->
                      let uu___5 =
                        maybe_intro_smt_lemma env_x
                          (FStarC_Syntax_Util.comp_result c1) c2 g_c2 in
                      (match uu___5 with
                       | (c21, g_c21) ->
                           let uu___6 =
                             FStarC_TypeChecker_Util.maybe_return_e2_and_bind
                               e1.FStarC_Syntax_Syntax.pos
                               (Prims.not is_inline_let_vc) env2
                               (FStar_Pervasives_Native.Some e1)
                               (c1, FStarC_TypeChecker_Env.trivial_guard) e22
                            ...
                            | (cres, g_cres) ->
                                let e11 =
                                  FStarC_TypeChecker_Util.maybe_lift env2 e1
                                    (FStarC_Syntax_Util.comp_effect_name c1)
                                    (FStarC_Syntax_Util.comp_effect_name cres)
                                    (FStarC_Syntax_Util.comp_result c1) in
                                let e23 =
                                  FStarC_TypeChecker_Util.maybe_lift env2 e22
                                    (FStarC_Syntax_Util.comp_effect_name c21)
                                    (FStarC_Syntax_Util.comp_effect_name cres)
                                    (FStarC_Syntax_Util.comp_result c21) in
                                let lb1 =
                                  let attrs =
                                    let add_inline_let =
                                      if Prims.not is_inline_let
                                      then
                                        let uu___7 =
                                          if pure_or_ghost
                                          then
                                  ...
                                  let lbtyp =
                                    ...
                                    if uu___7
                                    then lb.FStarC_Syntax_Syntax.lbtyp
                                    else FStarC_Syntax_Util.comp_result c1 in
                                  FStarC_Syntax_Util.mk_letbinding
                                    (FStar_Pervasives.Inl x1) [] lbtyp
                                    (FStarC_Syntax_Util.comp_effect_name cres)
                                    e11 attrs lb.FStarC_Syntax_Syntax.lbpos in
                                let e3 =
                                  let uu___7 =
                                    let uu___8 =
                                      let uu___9 =
                                        FStarC_Syntax_Subst.close xb e23 in
                                      {
                                        FStarC_Syntax_Syntax.lbs =
                                          (false, [lb1]);
                                  ...
                                  FStarC_Syntax_Syntax.mk uu___7
                                    e.FStarC_Syntax_Syntax.pos in
                                let e4 =
                                  FStarC_TypeChecker_Util.maybe_monadic env2
                                    e3
                                    (FStarC_Syntax_Util.comp_effect_name cres)
                                    (FStarC_Syntax_Util.comp_result cres) in
                                let g21 =
                                  FStarC_TypeChecker_Util.close_guard_implicits
                                    env2 false xb g2 in
                                let guard =
                                ...
                                then
                                      ...
                                      let uu___10 =
                                        FStarC_Class_Show.show
                                          ...
                                          (FStarC_Syntax_Util.comp_result
                                             cres) in
                                      FStarC_Format.print2
                                        "Got expected type from env %s\\(U.comp_result ncres)=%s\n"
                                        uu___9 uu___10
                                    else ());
                                   (let cres1 =
                                      let uu___8 =
                                        let uu___9 =
                                          let uu___10 =
                                            let uu___11 =
                                              FStarC_Syntax_Util.is_exactly_unit
                                                tt in
                                            if uu___11
                                            then true
                                            else
                                              FStarC_TypeChecker_Util.is_bare_flex
                                                tt in
                                          if uu___10
                                          then
                                            FStarC_TypeChecker_Util.keep_res_typ
                                              env2 tt
                                              (FStarC_Syntax_Util.comp_result
                                                 cres)
                                          else false in
                                        if uu___9
                                        then
                                          FStarC_TypeChecker_Env.closed env2
                                            (FStarC_Syntax_Util.comp_result
                                               cres)
                                        else false in
                                      if uu___8
                                  ...
                                  (let uu___7 =
                                     FStarC_TypeChecker_Util.check_no_escape
                                       FStarC_TypeChecker_Util.Escapes_let
                                       env2 [x1]
                                       (FStarC_Syntax_Util.comp_result cres) in
                                   match uu___7 with
                                   | (t, g_ex) ->
                                       ((let uu___9 =
                                        ...
                                        (let uu___9 =
                                         ...
                                         (e4, uu___9, uu___10))))))))))
  | uu___ ->
      FStarC_Effect.failwith "Impossible (inner let with more than one lb)"
                      ...
                      | (lbs3, g_lbs) ->
                          ...
                          let all_lb_names =
                            let uu___4 =
                              FStarC_List.map
                                (fun lb ->
                                   match lb.FStarC_Syntax_Syntax.lbname with
                                   | FStar_Pervasives.Inr v -> v) lbs3 in
                          ...
                          let uu___4 =
                            ...
                            then
                              ...
                              (((FStarC_List.hd lbs4).FStarC_Syntax_Syntax.lbunivs),
                                lbs4, g_lbs1)
                            else
                              (let ecs =
                                 let uu___5 =
                      ...
                      | (lbs3, g_lbs) ->
                          ...
                          (match uu___4 with
                           | (env2, lbs4) ->
                               let bvs =
                                 FStarC_List.map
                                   (fun lb ->
                                      match lb.FStarC_Syntax_Syntax.lbname
                                      with
                                      | FStar_Pervasives.Inl v -> v) lbs4 in
                               let env3 =
                                 {
                                   FStarC_TypeChecker_Env.solver =
                               ...
                               let uu___5 = tc_term env3 e21 in
                               (match uu___5 with
                                | (e22, cres, g2) ->
                                    let uu___6 =
                                      FStarC_List.fold_right
                                        (fun lb uu___7 ->
                                           match uu___7 with
                                           | (cres1, g) ->
                                               maybe_intro_smt_lemma env3
                                                 lb.FStarC_Syntax_Syntax.lbtyp
                                                 cres1 g) lbs4 (cres, g2) in
                                    (match uu___6 with
                                     | (cres1, g21) ->
                                         let cres2 =
                                           FStarC_TypeChecker_Util.maybe_assume_result_eq_pure_term
                                               ...
                                               | (lbs5, e23) ->
                                                   ...
                                                   (match uu___10 with
                                                    | (tres1, g_ex) ->
                                                        ...
                                                        (e, cres5, uu___11)))))))))))
  | uu___ ->
      FStarC_Effect.failwith "Impossible: check_inner_let_rec: not a let rec"
and build_let_rec_env (_top_level : Prims.bool)
  ...
  let termination_check_enabled attrs lbname lbdef lbtyp =
    let lbtyp0 = lbtyp in
    let uu___ = FStarC_Syntax_Util.abs_formals lbdef in
    match uu___ with
    | (actuals, body, body_lc) ->
        let actuals1 =
          FStarC_TypeChecker_Util.maybe_add_implicit_binders
            (FStarC_TypeChecker_Env.set_expected_typ env lbtyp) actuals in
        let nactuals = FStarC_List.length actuals1 in
        let uu___1 =
          FStarC_TypeChecker_Normalize.get_n_binders_no_unrefine env nactuals
            lbtyp in
        (match uu___1 with
         | (formals, c) ->
             (if
                (match formals with | [] -> true | uu___3 -> false) ||
                  ((match actuals1 with | [] -> true | uu___3 -> false))
              then
                (let uu___3 =
                   let uu___4 =
                     let uu___5 =
                       let uu___6 =
                         ...
                         FStarC_Format.fmt3 "Got (%s) %s : %s" uu___7 uu___8
                           uu___9 in
                       FStarC_Errors_Msg.text uu___6 in
                     [uu___5] in
                   (FStarC_Errors_Msg.text
                      "Only function literals with arrow types can be defined recursively.")
                     :: uu___4 in
                 FStarC_Errors.raise_error
              ...
              (let nformals = FStarC_List.length formals in
               ...
               then
                 ((let uu___5 =
                     ...
                     Prims.strcat "Admitting termination of " uu___6 in
                   FStarC_Errors.log_issue
                     FStarC_TypeChecker_Env.hasRange_env env
                     FStarC_Errors_Codes.Warning_WarnOnUse ()
                     (Obj.magic FStarC_Errors_Msg.is_error_message_string)
      ...
      tc_check_tot_or_gtot_term
        {
          ...
          FStarC_TypeChecker_Env.rec_names =
            (env01.FStarC_TypeChecker_Env.rec_names);
          FStarC_TypeChecker_Env.top_level =
          ...
          FStarC_TypeChecker_Env.disc_proj_tab =
            (env01.FStarC_TypeChecker_Env.disc_proj_tab);
          FStarC_TypeChecker_Env.erasable_types_tab =
          ...
          FStarC_TypeChecker_Env.iface_todo =
            (env01.FStarC_TypeChecker_Env.iface_todo);
          FStarC_TypeChecker_Env.iface_hidden =
            (env01.FStarC_TypeChecker_Env.iface_hidden);
          FStarC_TypeChecker_Env.iface_lids =
            (env01.FStarC_TypeChecker_Env.iface_lids);
          FStarC_TypeChecker_Env.iface_val_lids =
            (env01.FStarC_TypeChecker_Env.iface_val_lids)
                    ...
                    | FStar_Pervasives_Native.Some uvs ->
                        ...
                        then
                          FStarC_Errors.raise_error
                            ...
                            (Obj.magic
                               [FStarC_Errors_Msg.text
                                  "Mutually recursive functions must all have the same universe variables in their type annotation"])
                        else (univ_vars_acc, env1) in
                  (match uu___3 with
                   | (univ_vars_acc1, env2) ->
                       let uu___4 =
                         if Prims.not check_t
                         then (g_acc, lbtyp)
                         else
               ...
               (match bs with
                ...
                | uu___3 ->
                    ...
                    (match uu___4 with
                     | (bs0, bs1) ->
                         let def =
                           if match bs1 with | [] -> true | uu___5 -> false
                           then FStarC_Syntax_Util.abs bs0 t comp
                           else
                             (let inner = FStarC_Syntax_Util.abs bs1 t comp in
                              let inner1 =
                              ...
                              FStarC_Syntax_Syntax.mk_Tm_abs bs01 inner1
                                (FStar_Pervasives_Native.Some node_rc)
  ...
  | (env1, uu___1) ->
      ...
      (match uu___2 with
       | (topt, wf_annot, univ_vars, univ_opening, env11) ->
           (if (Prims.not top_level) && (univ_vars <> [])
            then
              FStarC_Errors.raise_error
                (FStarC_Syntax_Syntax.has_range_syntax ()) e1
               ...
               tc_maybe_toplevel_term
                 {
                   ...
                   FStarC_TypeChecker_Env.rec_names =
                     (env11.FStarC_TypeChecker_Env.rec_names);
                   ...
                   FStarC_TypeChecker_Env.disc_proj_tab =
                     (env11.FStarC_TypeChecker_Env.disc_proj_tab);
                   FStarC_TypeChecker_Env.erasable_types_tab =
                   ...
                   FStarC_TypeChecker_Env.iface_todo =
                     (env11.FStarC_TypeChecker_Env.iface_todo);
                   FStarC_TypeChecker_Env.iface_hidden =
                     (env11.FStarC_TypeChecker_Env.iface_hidden);
                   FStarC_TypeChecker_Env.iface_lids =
                     (env11.FStarC_TypeChecker_Env.iface_lids);
                   FStarC_TypeChecker_Env.iface_val_lids =
                     (env11.FStarC_TypeChecker_Env.iface_val_lids)
                 } e11 in
             match uu___4 with
             | (e12, c1, g1) ->
                 let guard_f =
                   FStarC_TypeChecker_Util.simplify_and_label_guard
                     (FStar_Pervasives_Native.Some
                        (fun uu___5 -> FStarC_TypeChecker_Err.ill_kinded_type))
                     (FStarC_TypeChecker_Env.set_range env11
                        e12.FStarC_Syntax_Syntax.pos) wf_annot in
                 let g11 =
                   FStarC_Class_Monoid.op_Plus_Plus
       ...
       | uu___1 ->
           ...
           (match uu___2 with
            | (univ_opening, univ_vars) ->
                ...
                if
                  top_level &&
                    (Prims.not env.FStarC_TypeChecker_Env.generalize)
                then
                  ((FStar_Pervasives_Native.Some t1),
                    (FStarC_Class_Monoid.mzero
                       FStarC_TypeChecker_Common.monoid_guard_t), univ_vars,
                    univ_opening,
                    (FStarC_TypeChecker_Env.set_expected_typ env1 t1))
                else
                  (let uu___3 = FStarC_Syntax_Util.type_u () in
                   match uu___3 with
                   | (k, uu___4) ->
...
and tc_binders (env : FStarC_TypeChecker_Env.env)
  ...
  (FStarC_Syntax_Syntax.binders * FStarC_TypeChecker_Env.env *
    FStarC_TypeChecker_Common.guard_t * FStarC_Syntax_Syntax.universes)=
  (let uu___1 = FStarC_Debug.extreme () in
   if uu___1
   then
     let uu___2 =
  ...
  | (e1, c, g) ->
      ...
      then
        ...
        (let g1 =
         ...
         let uu___2 = (c, FStarC_TypeChecker_Env.trivial_guard) in
         match uu___2 with
         | (c1, g_c) ->
                  ...
                  (match uu___4 with
                   | FStar_Pervasives_Native.Some g' ->
                       let uu___5 =
                         ...
                         FStarC_Class_Monoid.op_Plus_Plus
                           FStarC_TypeChecker_Common.monoid_guard_t g1 uu___6 in
                       (e1, target_comp, uu___5)
                   | uu___5 ->
                       if allow_ghost
                       then
...
and tc_attributes (env : FStarC_TypeChecker_Env.env_t)
  (attrs : FStarC_Syntax_Syntax.term Prims.list) :
  (FStarC_TypeChecker_Common.guard_t * FStarC_Syntax_Syntax.term Prims.list)=
  FStarC_List.fold_left
    (fun uu___ attr ->
       match uu___ with
       | (g, attrs1) ->
  ...
  (let env1 =
   ...
   let uu___1 =
     ...
     | FStarC_Errors.Error (e1, msg, r, ctx) when
         ...
         FStarC_Effect.raise
           (FStarC_Errors.Error
              (e1, msg, (FStarC_TypeChecker_Env.get_range env1), ctx)) in
   match uu___1 with
   | (t, c, g) ->
       if must_tot
       then
         let c1 = FStarC_TypeChecker_Normalize.maybe_ghost_to_pure env1 c in
         let uu___2 = FStarC_Syntax_Util.is_total_comp c1 in
         (if uu___2
          then (t, (FStarC_Syntax_Util.comp_result c1), g)
          else
            (let uu___3 =
               let uu___4 =
                 FStarC_Class_Show.show FStarC_Syntax_Print.showable_term e in
             ...
             FStarC_Errors.raise_error FStarC_TypeChecker_Env.hasRange_env
               ...
               (Obj.magic FStarC_Errors_Msg.is_error_message_string)
               (Obj.magic uu___3)))
       else (t, (FStarC_Syntax_Util.comp_result c), g))
           ...
           | (t_u, u) ->
               ...
               ((match g.FStarC_TypeChecker_Common.guard_f with
                 | FStarC_TypeChecker_Common.NonTrivial f ->
                     ...
                     level_of_type_fail env1 e uu___4
                 | uu___4 ->
                     FStarC_TypeChecker_Rel.force_trivial_guard env1 g);
                u)) in
  aux true t
let rec apply_well_typed (env : FStarC_TypeChecker_Env.env)
  ...
  then FStar_Pervasives_Native.Some t_hd
    ...
    (let thd = FStarC_TypeChecker_Normalize.unfold_whnf env t_hd in
     match thd.FStarC_Syntax_Syntax.n with
     | FStarC_Syntax_Syntax.Tm_arrow uu___ ->
         let uu___1 = FStarC_Syntax_Util.arrow_formals_comp_ln_strict thd in
         (match uu___1 with
          | (bs, c) ->
              let n_args = FStarC_List.length args in
              ...
              let uu___2 =
                ...
                then
                  ...
                  | (bs1, rest) ->
                      let t =
                        FStarC_Syntax_Syntax.mk_Tm_arrow rest c
                  ...
                  (let uu___3 = FStarC_Syntax_Subst.open_comp bs c in
                   match uu___3 with
                   | (bs1, c1) ->
                       let uu___4 = FStarC_List.splitAt n_bs args in
                       (match uu___4 with
                        | (args1, remaining_args) ->
                            (bs1, args1, (FStarC_Syntax_Util.comp_result c1),
                              remaining_args))) in
              (match uu___2 with
              ...
              (fun ul ur ->
                 ...
                 | uu___7 ->
                     ...
                     FStarC_Errors.raise_error
                       ...
                       FStarC_Errors_Codes.Fatal_IncompatibleUniverse ()
                       (Obj.magic FStarC_Errors_Msg.is_error_message_string)
                       (Obj.magic uu___8)) us' us;
            t))
  | FStarC_Syntax_Syntax.Tm_uinst uu___1 ->
      FStarC_Effect.failwith "Impossible: Tm_uinst's head must be an fvar"
  | FStarC_Syntax_Syntax.Tm_refine
      { FStarC_Syntax_Syntax.b2 = x; FStarC_Syntax_Syntax.phi = uu___1;_} ->
      universe_of_aux env x.FStarC_Syntax_Syntax.sort
  | FStarC_Syntax_Syntax.Tm_arrow uu___1 ->
      let uu___2 = FStarC_Syntax_Util.arrow_formals_comp_strict e in
      (match uu___2 with
       | (bs, c) ->
             ...
             FStarC_List.map
               (fun uu___3 ->
                  ...
                  | { FStarC_Syntax_Syntax.binder_bv = b;
                      ...
                      level_of_type env1 b.FStarC_Syntax_Syntax.sort uu___7)
               bs in
           let u_res =
             let res = FStarC_Syntax_Util.comp_result c in
             let uu___3 = universe_of_aux env1 res in
             level_of_type env1 res uu___3 in
       ...
       | (hd, args) ->
           let rec type_of_head retry env1 hd1 args1 =
             ...
             | FStarC_Syntax_Syntax.Tm_delayed uu___3 ->
                 FStarC_Effect.failwith
                   "Impossible: universe_of_aux: Tm_app: unexpected head type"
             | FStarC_Syntax_Syntax.Tm_fvar uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             | FStarC_Syntax_Syntax.Tm_name uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             | FStarC_Syntax_Syntax.Tm_uvar uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             | FStarC_Syntax_Syntax.Tm_uinst uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             ...
             | FStarC_Syntax_Syntax.Tm_arrow uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             | FStarC_Syntax_Syntax.Tm_meta uu___3 ->
                 let uu___4 = universe_of_aux env1 hd2 in (uu___4, args1)
             ...
             | FStarC_Syntax_Syntax.Tm_match
                 { FStarC_Syntax_Syntax.scrutinee = uu___3;
                   ...
                   FStarC_Syntax_Syntax.brs = b::uu___5;
                   FStarC_Syntax_Syntax.rc_opt1 = uu___6;_}
                 ->
                 let uu___7 = FStarC_Syntax_Subst.open_branch b in
                 (match uu___7 with
                  | (pat, uu___8, tm) ->
                      let bvs = FStarC_Syntax_Syntax.pat_bvs pat in
                      let uu___9 = FStarC_Syntax_Util.head_and_args_full tm in
                      (match uu___9 with
                       | (hd3, args') ->
                           let uu___10 =
                             FStarC_TypeChecker_Env.push_bvs env1 bvs in
                           type_of_head retry uu___10 hd3
             ...
             | uu___3 when retry ->
                 ...
                 let uu___4 = FStarC_Syntax_Util.head_and_args_full e1 in
                 (match uu___4 with
             ...
             | uu___3 ->
                 ...
                 (match uu___4 with
                  | (env2, uu___5) ->
                      ...
                      ((let uu___7 = FStarC_Effect.op_Bang dbg_UniverseOf in
                        if uu___7
                        then
                          let uu___8 =
                            FStarC_Range_Ops.string_of_range
                       ...
                       (let uu___7 = tc_term env3 hd2 in
                        match uu___7 with
                        | (uu___8, c, g) ->
                            ...
                            ((let uu___10 =
                              ...
                              ());
                             (t, args1))))) in
           let uu___3 = type_of_head true env hd args in
           (match uu___3 with
            | (t, args1) ->
                let uu___4 = apply_well_typed env t args1 in
                (match uu___4 with
                 ...
                 | FStar_Pervasives_Native.None ->
                     let uu___5 =
                       FStarC_Class_Show.show
                         FStarC_Syntax_Print.showable_term t in
                     level_of_type_fail env e uu___5)))
  | FStarC_Syntax_Syntax.Tm_match
...
let rec __typeof_tot_or_gtot_term_fastpath (env : FStarC_TypeChecker_Env.env)
  ...
  let mk_tm_type u =
    FStarC_Syntax_Syntax.mk (FStarC_Syntax_Syntax.Tm_type u)
      t.FStarC_Syntax_Syntax.pos in
  let effect_ok k =
    if Prims.not must_tot
    then true
    else FStarC_TypeChecker_Normalize.non_info_norm env k in
  let t1 = FStarC_Syntax_Subst.compress t in
  match t1.FStarC_Syntax_Syntax.n with
      ...
      (match uu___1 with
       | (bs, body, FStar_Pervasives_Native.Some
          { FStarC_Syntax_Syntax.residual_effect = eff;
           ...
           ->
           let is_tot = FStarC_Parser_Const.is_tot_lid eff in
           let is_gtot = FStarC_Parser_Const.is_gtot_lid eff in
           if Prims.not (is_tot || is_gtot)
           then FStar_Pervasives_Native.None
           else
             (let tbody1 =
                match tbody with
                | FStar_Pervasives_Native.Some uu___3 -> tbody
                | FStar_Pervasives_Native.None ->
                    ...
                    (match uu___3 with
                     | (bs1, body1) ->
                         let uu___4 =
                           ...
                           __typeof_tot_or_gtot_term_fastpath uu___5 body1
                             false in
                         FStarC_Option.map (FStarC_Syntax_Subst.close bs1)
                           uu___4) in
              FStarC_Option.bind tbody1
                (fun tbody2 ->
                   let uu___3 = FStarC_Syntax_Subst.open_term bs tbody2 in
                   match uu___3 with
                   | (bs1, tbody3) ->
                       let u =
                         let uu___4 =
                           FStarC_TypeChecker_Env.push_binders env bs1 in
                         universe_of uu___4 tbody3 in
                       let uu___4 =
                         let uu___5 =
                           if is_tot
                           then FStarC_Syntax_Syntax.mk_Total tbody3
                           else FStarC_Syntax_Syntax.mk_GTotal tbody3 in
                         FStarC_Syntax_Util.arrow bs1 uu___5 in
  ...
  | FStarC_Syntax_Syntax.Tm_app uu___ ->
      ...
      (match uu___1 with
       | (hd, args) ->
           let apply_head hd1 args1 =
             let t_hd = __typeof_tot_or_gtot_term_fastpath env hd1 must_tot in
             FStarC_Option.bind t_hd
               (fun t_hd1 ->
                  let uu___2 = apply_well_typed env t_hd1 args1 in
                  FStarC_Option.bind uu___2
                    (fun t2 ->
                       let uu___3 =
                         let uu___4 = effect_ok t2 in
                         if uu___4
                         then true
                         else
                           FStarC_List.for_all
           ...
           (match uu___2 with
            | (FStarC_Syntax_Syntax.Tm_constant
               (FStarC_Const.Const_range_of), a::rest) when
                match rest with | hd1::tl -> true | uu___3 -> false ->
                let uu___3 =
                  let uu___4 =
                    FStarC_Range_Ops.union_ranges hd.FStarC_Syntax_Syntax.pos
                      (FStar_Pervasives_Native.fst a).FStarC_Syntax_Syntax.pos in
                  FStarC_Syntax_Syntax.mk_Tm_app hd [a] uu___4 in
                apply_head uu___3 rest
            | (FStarC_Syntax_Syntax.Tm_constant
               (FStarC_Const.Const_set_range_of), a1::a2::rest) when
                match rest with | hd1::tl -> true | uu___3 -> false ->
                let uu___3 =
                  let uu___4 =
                    FStarC_Range_Ops.union_ranges hd.FStarC_Syntax_Syntax.pos
                      (FStar_Pervasives_Native.fst a1).FStarC_Syntax_Syntax.pos in
                  FStarC_Syntax_Syntax.mk_Tm_app hd [a1; a2] uu___4 in
                apply_head uu___3 rest
            | (FStarC_Syntax_Syntax.Tm_constant
               (FStarC_Const.Const_range_of), uu___3::[]) ->
                FStar_Pervasives_Native.Some FStarC_Syntax_Syntax.t_range
            | (FStarC_Syntax_Syntax.Tm_constant
               (FStarC_Const.Const_set_range_of), (t2, uu___3)::uu___4::[])
                -> __typeof_tot_or_gtot_term_fastpath env t2 must_tot
            | uu___3 -> apply_head hd args))
  | FStarC_Syntax_Syntax.Tm_ascribed
  ...
  | FStarC_Syntax_Syntax.Tm_let
      ...
      let x =
        ...
        | FStar_Pervasives.Inl v -> v in
      let uu___ =
        FStarC_Syntax_Subst.open_term [FStarC_Syntax_Syntax.mk_binder x] body in
      (match uu___ with
       | (xb, body1) ->
           let xbinder = FStarC_List.hd xb in
           let x1 = xbinder.FStarC_Syntax_Syntax.binder_bv in
...
let rec effectof_tot_or_gtot_term_fastpath (env : FStarC_TypeChecker_Env.env)
  ...
  | FStarC_Syntax_Syntax.Tm_bvar uu___1 ->
      FStarC_Effect.failwith "Impossible!"
  | FStarC_Syntax_Syntax.Tm_name uu___1 ->
      FStar_Pervasives_Native.Some FStarC_Parser_Const.primitive_pure_lid
  | FStarC_Syntax_Syntax.Tm_lazy uu___1 ->
      FStar_Pervasives_Native.Some FStarC_Parser_Const.primitive_pure_lid
  | FStarC_Syntax_Syntax.Tm_fvar uu___1 ->
      FStar_Pervasives_Native.Some FStarC_Parser_Const.primitive_pure_lid
      ...
      (match uu___2 with
       | (hd, args) ->
           let join_effects eff1 eff2 =
             let uu___3 = (eff1, eff2) in
             match uu___3 with
             | (eff11, eff21) ->
                 let uu___4 =
                   (FStarC_Parser_Const.primitive_pure_lid,
                     FStarC_Parser_Const.primitive_ghost_lid) in
                 (match uu___4 with
                  | (pure, ghost) ->
                      if
                        (FStarC_Syntax_Util.is_pure_effect eff11) &&
                          (FStarC_Syntax_Util.is_pure_effect eff21)
                      then FStar_Pervasives_Native.Some pure
                      else
                        if
                          (FStarC_Syntax_Util.is_pure_or_ghost_effect eff11)
                            &&
                            (FStarC_Syntax_Util.is_pure_or_ghost_effect eff21)
                        then FStar_Pervasives_Native.Some ghost
                        else FStar_Pervasives_Native.None) in
           let uu___3 = effectof_tot_or_gtot_term_fastpath env hd in
                    ...
                    (fun eff_opt arg ->
                       FStarC_Option.bind eff_opt
                         (fun eff ->
                            ...
                            FStarC_Option.bind uu___5 (join_effects eff)))
                    (FStar_Pervasives_Native.Some eff_hd) args in
                FStarC_Option.bind uu___4
                  (fun eff_hd_and_args ->
                     let uu___5 =
                       typeof_tot_or_gtot_term_fastpath env hd true in
                     FStarC_Option.bind uu___5
                       (fun t_hd ->
                          let rec maybe_arrow t1 =
                            ...
                            | FStarC_Syntax_Syntax.Tm_arrow uu___6 -> t2
                            | FStarC_Syntax_Syntax.Tm_refine
                          ...
                          match ta.FStarC_Syntax_Syntax.n with
  ...
  | FStarC_Syntax_Syntax.Tm_ascribed
      { FStarC_Syntax_Syntax.tm = uu___1;
        ...
        FStarC_Syntax_Syntax.eff_opt = uu___4;_}
      ->
      let c_eff = FStarC_Syntax_Util.comp_effect_name c in
      if FStarC_Syntax_Util.is_pure_effect c_eff
      then
        FStar_Pervasives_Native.Some FStarC_Parser_Const.primitive_pure_lid
      else
        if FStarC_Syntax_Util.is_ghost_effect c_eff
        then
          FStar_Pervasives_Native.Some
