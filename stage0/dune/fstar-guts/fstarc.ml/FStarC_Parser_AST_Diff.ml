  ...
  | (t11, a1) ->
      ...
      (match uu___1 with
       | (t21, a2) ->
           ...
           if uu___2 then eq_imp a1 a2 else false)
...
and eq_pattern' (p1 : FStarC_Parser_AST.pattern')
  ...
  | (FStarC_Parser_AST.PatWild (q1, a1), FStarC_Parser_AST.PatWild (q2, a2))
      ->
      let uu___ = eq_aqual q1 q2 in if uu___ then eq_terms a1 a2 else false
  ...
  | (FStarC_Parser_AST.PatApp (p11, ps1), FStarC_Parser_AST.PatApp
     (p21, ps2)) ->
      ...
      if uu___ then eq_list eq_pattern ps1 ps2 else false
  | (FStarC_Parser_AST.PatVar (i1, aq1, as1), FStarC_Parser_AST.PatVar
     (i2, aq2, as2)) ->
      let uu___ =
        if FStarC_Ident.ident_equals i1 i2 then eq_aqual aq1 aq2 else false in
      if uu___ then eq_terms as1 as2 else false
      ...
      eq_list
        (fun uu___ uu___1 ->
           ...
           | ((l1, p11), (l2, p21)) ->
               ...
               then eq_pattern p11 p21
               else false) ps1 ps2
  | (FStarC_Parser_AST.PatAscribed (p11, (t1, topt1)),
     FStarC_Parser_AST.PatAscribed (p21, (t2, topt2))) ->
      let uu___ =
        let uu___1 = eq_pattern p11 p21 in
        if uu___1 then eq_term t1 t2 else false in
      if uu___ then eq_option eq_term topt1 topt2 else false
...
and eq_term' (t1 : FStarC_Parser_AST.term') (t2 : FStarC_Parser_AST.term') :
  ...
  | (FStarC_Parser_AST.Op (i1, ts1), FStarC_Parser_AST.Op (i2, ts2)) ->
      let uu___ = eq_ident i1 i2 in if uu___ then eq_terms ts1 ts2 else false
  ...
  | (FStarC_Parser_AST.App (h1, t11, i1), FStarC_Parser_AST.App
     (h2, t21, i2)) ->
      ...
      if uu___ then eq_imp i1 i2 else false
        ...
        then
          eq_list
            (fun uu___1 uu___2 ->
               ...
               | ((o1, (p1, t12)), (o2, (p2, t22))) ->
                   let uu___3 =
                     let uu___4 = eq_option eq_terms o1 o2 in
                     if uu___4 then eq_pattern p1 p2 else false in
                   if uu___3 then eq_term t12 t22 else false) defs1 defs2
        else false in
      if uu___ then eq_term t11 t21 else false
          ...
          (fun uu___1 uu___2 ->
             ...
             | ((i1, ps1, t12), (i2, ps2, t22)) ->
                 let uu___3 =
                   ...
                   if uu___4 then eq_pattern ps1 ps2 else false in
                 if uu___3 then eq_term t12 t22 else false) defs1 defs2 in
      if uu___ then eq_term t11 t21 else false
  | (FStarC_Parser_AST.LetOpen (l1, t11), FStarC_Parser_AST.LetOpen
     (l2, t21)) ->
      if FStarC_Ident.lid_equals l1 l2 then eq_term t11 t21 else false
  ...
  | (FStarC_Parser_AST.Seq (t11, t21), FStarC_Parser_AST.Seq (t3, t4)) ->
      let uu___ = eq_term t11 t3 in if uu___ then eq_term t21 t4 else false
  | (FStarC_Parser_AST.Bind (i1, t11, t21), FStarC_Parser_AST.Bind
     (i2, t3, t4)) ->
      let uu___ =
        if FStarC_Ident.ident_equals i1 i2 then eq_term t11 t3 else false in
      ...
      let uu___ =
        let uu___1 =
          let uu___2 =
            ...
            if uu___3 then eq_option eq_ident i1 i2 else false in
          if uu___2
          then eq_option eq_match_returns_annotation mra1 mra2
          else false in
        if uu___1 then eq_term t21 t5 else false in
      if uu___ then eq_term t3 t6 else false
  | (FStarC_Parser_AST.Match (t11, i1, mra1, bs1), FStarC_Parser_AST.Match
     (t21, i2, mra2, bs2)) ->
      let uu___ =
        let uu___1 =
          let uu___2 = eq_term t11 t21 in
          if uu___2 then eq_option eq_ident i1 i2 else false in
        if uu___1
        then eq_option eq_match_returns_annotation mra1 mra2
        else false in
      if uu___ then eq_list eq_branch bs1 bs2 else false
     ...
     FStarC_Parser_AST.Ascribed (t3, t4, topt2, b2)) ->
      let uu___ =
        let uu___1 =
          ...
          if uu___2 then eq_term t21 t4 else false in
        if uu___1 then eq_option eq_term topt1 topt2 else false in
      if uu___ then b1 = b2 else false
      ...
      then
        eq_list
          (fun uu___1 uu___2 ->
             ...
             | ((l1, t11), (l2, t21)) ->
                 if FStarC_Ident.lid_equals l1 l2
                 then eq_term t11 t21
                 else false) fs1 fs2
      else false
  | (FStarC_Parser_AST.Project (t11, l1), FStarC_Parser_AST.Project
     (t21, l2)) ->
      let uu___ = eq_term t11 t21 in
      if uu___ then FStarC_Ident.lid_equals l1 l2 else false
     ...
     (bs2, ps2, t21)) ->
      let eq_ps uu___ uu___1 =
        ...
        | ((is1, ts1), (is2, ts2)) ->
            ...
            if uu___2 then eq_list (eq_list eq_term) ts1 ts2 else false in
      let uu___ =
        let uu___1 = eq_list eq_binder bs1 bs2 in
        if uu___1 then eq_ps ps1 ps2 else false in
      if uu___ then eq_term t11 t21 else false
  ...
  | (FStarC_Parser_AST.QuantOp (i1, bs1, ps1, t11), FStarC_Parser_AST.QuantOp
     (i2, bs2, ps2, t21)) ->
      ...
      let uu___ =
        let uu___1 =
          if FStarC_Ident.ident_equals i1 i2
          then eq_list eq_binder bs1 bs2
          else false in
        if uu___1 then eq_ps ps1 ps2 else false in
      if uu___ then eq_term t11 t21 else false
  | (FStarC_Parser_AST.Refine (t11, t21), FStarC_Parser_AST.Refine (t3, t4))
      ->
      let uu___ = eq_binder t11 t3 in if uu___ then eq_term t21 t4 else false
  | (FStarC_Parser_AST.NamedTyp (i1, t11), FStarC_Parser_AST.NamedTyp
     (i2, t21)) ->
      let uu___ = eq_ident i1 i2 in if uu___ then eq_term t11 t21 else false
  ...
  | (FStarC_Parser_AST.Labeled (t11, s1, b1), FStarC_Parser_AST.Labeled
     (t21, s2, b2)) ->
      let uu___ =
        let uu___1 = eq_term t11 t21 in if uu___1 then s1 = s2 else false in
      if uu___ then b1 = b2 else false
  ...
  | (FStarC_Parser_AST.Quote (t11, k1), FStarC_Parser_AST.Quote (t21, k2)) ->
      let uu___ = eq_term t11 t21 in if uu___ then k1 = k2 else false
  ...
  | (FStarC_Parser_AST.CalcProof (t11, t21, cs1), FStarC_Parser_AST.CalcProof
     (t3, t4, cs2)) ->
      let uu___ =
        ...
        if uu___1 then eq_term t21 t4 else false in
      if uu___ then eq_list eq_calc_step cs1 cs2 else false
  ...
  | (FStarC_Parser_AST.IntroOr (b1, t11, t21, t3), FStarC_Parser_AST.IntroOr
     (b2, t4, t5, t6)) ->
      let uu___ =
        let uu___1 = if b1 = b2 then eq_term t11 t4 else false in
        if uu___1 then eq_term t21 t5 else false in
      if uu___ then eq_term t3 t6 else false
      ...
      (match uu___1 with
       | FStarC_Parser_AST.CalcStep (t4, t5, t6) ->
           let uu___2 =
             ...
             if uu___3 then eq_term t2 t5 else false in
           if uu___2 then eq_term t3 t6 else false)
and eq_binder (b1 : FStarC_Parser_AST.binder) (b2 : FStarC_Parser_AST.binder)
  ...
  let uu___ =
    ...
    if uu___1
    then eq_aqual b1.FStarC_Parser_AST.aqual b2.FStarC_Parser_AST.aqual
    else false in
  if uu___
  then
      ...
      (match uu___1 with
       | (i2, t2, b2) ->
           let uu___2 =
             ...
             if uu___3 then eq_term t1 t2 else false in
      ...
      (match uu___1 with
       | (p2, o2, t2) ->
           let uu___2 =
             ...
             if uu___3 then eq_option eq_term o1 o2 else false in
           if uu___2 then eq_term t1 t2 else false)
    ...
    (fun uu___ uu___1 ->
       ...
       | ((i1, a1, a2, t11), (i2, a3, a4, t21)) ->
           let uu___2 =
             let uu___3 =
               let uu___4 = eq_ident i1 i2 in
               if uu___4 then eq_aqual a1 a3 else false in
             if uu___3 then eq_list eq_term a2 a4 else false in
           if uu___2 then eq_term t11 t21 else false) t1 t2
let eq_constructor_payload (t1 : FStarC_Parser_AST.constructor_payload)
  ...
  | (FStarC_Parser_AST.VpRecord (r1, k1), FStarC_Parser_AST.VpRecord
     (r2, k2)) ->
      ...
      if uu___ then eq_option eq_term k1 k2 else false
  | uu___ -> false
  ...
  | (FStarC_Parser_AST.TyconAbbrev (i1, bs1, k1, t11),
     FStarC_Parser_AST.TyconAbbrev (i2, bs2, k2, t21)) ->
      let uu___ =
        ...
        if uu___1 then eq_option eq_term k1 k2 else false in
      if uu___ then eq_term t11 t21 else false
      ...
      let uu___ =
        let uu___1 =
          let uu___2 =
            ...
            if uu___3 then eq_list eq_binder bs1 bs2 else false in
          if uu___2 then eq_option eq_term k1 k2 else false in
        if uu___1 then eq_list eq_term a1 a2 else false in
      if uu___ then eq_tycon_record r1 r2 else false
      ...
      then
        eq_list
          (fun uu___1 uu___2 ->
             ...
             | ((i11, o1, a1), (i21, o2, a2)) ->
                 let uu___3 =
                   let uu___4 = eq_ident i11 i21 in
                   if uu___4
                   then eq_option eq_constructor_payload o1 o2
                   else false in
                 if uu___3 then eq_list eq_term a1 a2 else false) cs1 cs2
      else false
  | uu___ -> false
           ...
           | (a, b) ->
               ...
               (match uu___1 with
                | (c, d) ->
                    ...
                    if uu___2 then eq_option eq_ident b d else false)) l1 l2
  | uu___ -> false
let rec eq_decl' (d1 : FStarC_Parser_AST.decl')
  ...
  | (FStarC_Parser_AST.Open (lid1, restriction1), FStarC_Parser_AST.Open
     (lid2, restriction2)) ->
      if eq_lid lid1 lid2
      then eq_restriction restriction1 restriction2
      else false
  ...
  | (FStarC_Parser_AST.ModuleAbbrev (i1, lid1),
     FStarC_Parser_AST.ModuleAbbrev (i2, lid2)) ->
      let uu___ = eq_ident i1 i2 in if uu___ then eq_lid lid1 lid2 else false
      ...
      then
        eq_list
          (fun uu___ uu___1 ->
             match (uu___, uu___1) with
             | ((p1, t1), (p2, t2)) ->
                 let uu___2 = eq_pattern p1 p2 in
                 if uu___2 then eq_term t1 t2 else false) pats1 pats2
      else false
  ...
  | (FStarC_Parser_AST.Splice (is_typed1, is1, t1), FStarC_Parser_AST.Splice
     (is_typed2, is2, t2)) ->
      let uu___ =
        if is_typed1 = is_typed2 then eq_list eq_ident is1 is2 else false in
  ...
  | (FStarC_Parser_AST.DeclToBeDesugared tbs1,
     FStarC_Parser_AST.DeclToBeDesugared tbs2) ->
      if tbs1.FStarC_Parser_AST.lang_name = tbs2.FStarC_Parser_AST.lang_name
      then
...
and eq_decl (d1 : FStarC_Parser_AST.decl) (d2 : FStarC_Parser_AST.decl) :
  ...
  if uu___
  then eq_list eq_term d1.FStarC_Parser_AST.attrs d2.FStarC_Parser_AST.attrs
  else false
