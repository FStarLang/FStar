      ...
      fun f ->
        fun l ->
          let uu___ =
            ...
            FStar_Pprint.precede
              (FStar_Pprint.op_Hat_Hat prec FStar_Pprint.space) uu___1 in
          let uu___1 =
  ...
  fun sep ->
    fun f ->
      fun es ->
        ...
        let es1 =
          FStarC_List.mapi (fun i -> fun e -> f (i <> (l - Prims.int_one)) e)
            es in
...
let rec (extract_from_ref_set :
  ...
  fun e ->
    ...
    | FStarC_Parser_AST.App
        ...
        FStarC_List.op_At uu___5 uu___6
    | uu___ ->
        let uu___1 =
...
let (is_non_latin_char : FStar_Char.char -> Prims.bool) =
  fun s -> (FStarC_Util.int_of_char s) > (Prims.of_int (0x024f))
...
let (level_table :
  ...
  let levels_from_associativity l =
    fun uu___ ->
      match uu___ with
...
let (handleable_args_length : FStarC_Ident.ident -> Prims.int) =
  fun op ->
    ...
    let uu___ =
      (is_general_prefix_op op) || (FStarC_List.mem op_s ["-"; "~"]) in
    if uu___
    then Prims.int_one
    else
      (let uu___2 =
...
let (__proj__Mkdecl_meta__item__r : decl_meta -> FStarC_Range_Type.range) =
...
let with_comment :
  'uuuuu .
    ('uuuuu -> FStar_Pprint.document) ->
      'uuuuu -> FStarC_Range_Type.range -> FStar_Pprint.document
  =
  fun printer ->
    fun tm ->
      fun tmrange ->
        let rec comments_before_pos acc =
          fun print_pos ->
            fun lookahead_pos ->
              ...
              | (c, crange)::cs ->
                  let comment =
                    FStar_Pprint.op_Hat_Hat (str c) FStar_Pprint.hardline in
                  let uu___1 =
                    FStarC_Range_Ops.range_before_pos crange print_pos in
                  if uu___1
                  then
                    (FStarC_Effect.op_Colon_Equals comment_stack cs;
                     comments_before_pos
                       (FStar_Pprint.op_Hat_Hat acc comment) print_pos
                       lookahead_pos)
                  else
                    (let uu___3 =
        ...
        | (comments, has_lookahead) ->
            ...
            then printed_e
              ...
              FStar_Pprint.group
                (FStar_Pprint.op_Hat_Hat comments1 printed_e)
let with_comment_sep :
  'uuuuu 'uuuuu1 .
    ('uuuuu -> 'uuuuu1) ->
      'uuuuu -> FStarC_Range_Type.range -> (FStar_Pprint.document * 'uuuuu1)
  =
  fun printer ->
    fun tm ->
      fun tmrange ->
                        ...
                        then comment
                          ...
                          FStar_Pprint.op_Hat_Hat acc
                            (FStar_Pprint.op_Hat_Hat FStar_Pprint.hardline
                               comment)) print_pos lookahead_pos)
                  else
                    (let uu___3 =
                       FStarC_Range_Ops.range_before_pos crange lookahead_pos in
                     (acc, uu___3)) in
            ...
            fun r ->
              fun init ->
                ...
                | (comment, crange)::cs when
                    FStarC_Range_Ops.range_before_pos crange pos ->
                    (FStarC_Effect.op_Colon_Equals comment_stack cs;
                     (let lnum =
                        let uu___2 =
                          let uu___3 =
                            let uu___4 =
                              FStarC_Range_Ops.start_of_range crange in
                ...
                | uu___1 ->
                    ...
                    then FStar_Pprint.empty
                      ...
                      (let lnum =
                         let uu___3 = FStarC_Range_Ops.line_of_pos pos in
                         uu___3 - lbegin in
...
let (p_char_literal' :
  FStar_Char.char -> FStarC_BaseTypes.char -> FStar_Pprint.document) =
  fun quote_char ->
    fun c ->
      str
        (match c with
...
let (p_char_literal : FStarC_BaseTypes.char -> FStar_Pprint.document) =
...
let (string_of_id_or_underscore :
  FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid ->
    let uu___ =
...
let (text_of_lid_or_underscore :
  FStarC_Ident.lident -> FStar_Pprint.document) =
  fun lid ->
    let uu___ =
      (let uu___1 =
...
let (p_qlident : FStarC_Ident.lident -> FStar_Pprint.document) =
  fun lid -> text_of_lid_or_underscore lid
let (p_quident : FStarC_Ident.lident -> FStar_Pprint.document) =
  fun lid -> text_of_lid_or_underscore lid
let (p_ident : FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid -> string_of_id_or_underscore lid
let (p_lident : FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid -> string_of_id_or_underscore lid
let (p_uident : FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid -> string_of_id_or_underscore lid
let (p_tvar : FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid -> string_of_id_or_underscore lid
let (p_qlidentOrOperator : FStarC_Ident.lident -> FStar_Pprint.document) =
  fun lid -> p_lidentOrOperator' lid FStarC_Ident.string_of_lid p_qlident
let (p_lidentOrOperator : FStarC_Ident.ident -> FStar_Pprint.document) =
  fun lid -> p_lidentOrOperator' lid FStarC_Ident.string_of_id p_lident
let rec (p_decl : FStarC_Parser_AST.decl -> FStar_Pprint.document) =
  fun d ->
    let qualifiers =
    ...
    FStar_Pprint.op_Hat_Hat uu___ uu___1
and (p_attributes :
  Prims.bool -> FStarC_Parser_AST.attributes_ -> FStar_Pprint.document) =
  fun isTopLevel ->
    fun attrs ->
      match attrs with
      | [] -> FStar_Pprint.empty
            ...
            let uu___2 =
              let uu___3 =
                let uu___4 =
                  let uu___5 =
                    let uu___6 =
                      FStarC_List.map (p_noSeqTermAndComment false false)
                        attrs in
...
and (p_justSig : FStarC_Parser_AST.decl -> FStar_Pprint.document) =
  fun d ->
...
and (p_restriction :
  FStarC_Syntax_Syntax.restriction -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
              ...
              p_list
                (fun uu___4 ->
                   ...
                   | (id, renamed) ->
                       ...
                       FStar_Pprint.op_Hat_Slash_Hat uu___5 uu___6)
                (str ", ") ids in
...
and (p_rawDecl : FStarC_Parser_AST.decl -> FStar_Pprint.document) =
  fun d ->
    match d.FStarC_Parser_AST.d with
...
and (p_pragma : FStarC_Parser_AST.pragma -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_typeDeclWithKw :
  FStar_Pprint.document -> FStarC_Parser_AST.tycon -> FStar_Pprint.document)
  =
  fun kw ->
    fun typedecl ->
...
and (p_typeDeclRecord :
  FStarC_Parser_AST.tycon_record -> FStar_Pprint.document) =
  fun fields ->
    let p_recordField ps =
      fun uu___ ->
        match uu___ with
        | (lid, aq, attrs, t) ->
            let uu___1 =
              let uu___2 =
                FStarC_Range_Ops.extend_to_end_of_line
                  t.FStarC_Parser_AST.range in
          ...
          fun typ_opt ->
            ...
            then with_kw (fun n -> prefix2 n typ)
              ...
              (let binders = p_binders_list true bs in
               with_kw
                 (fun n ->
                    prefix2 (prefix2 n (FStar_Pprint.flow break1 binders))
                      typ))
...
and (p_term_list :
  Prims.bool ->
    Prims.bool -> FStarC_Parser_AST.term Prims.list -> FStar_Pprint.document)
  =
  fun ps ->
    fun pb ->
      fun l ->
        let rec aux uu___ =
          ...
          | x::xs ->
              ...
              let uu___2 =
                let uu___3 = aux xs in
                FStar_Pprint.op_Hat_Hat (str ";") uu___3 in
              FStar_Pprint.op_Hat_Hat uu___1 uu___2 in
        let uu___ =
          let uu___1 = aux l in FStar_Pprint.op_Hat_Hat uu___1 (str "]") in
        FStar_Pprint.op_Hat_Hat (str "[") uu___
and (p_newEffect : FStarC_Parser_AST.effect_decl -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_effectRedefinition :
  FStarC_Ident.ident ->
    FStarC_Parser_AST.binder Prims.list ->
      FStarC_Parser_AST.term -> FStar_Pprint.document)
  =
  fun uid ->
    fun bs ->
      fun t ->
...
and (p_effectDefinition :
  FStarC_Ident.ident ->
    FStarC_Parser_AST.binder Prims.list ->
      FStarC_Parser_AST.term ->
        FStarC_Parser_AST.decl Prims.list -> FStar_Pprint.document)
  =
  fun uid ->
    fun bs ->
      fun t ->
        fun eff_decls ->
          ...
          braces_with_nesting uu___
and (p_effectDecl :
  Prims.bool -> FStarC_Parser_AST.decl -> FStar_Pprint.document) =
  fun ps ->
    fun d ->
      match d.FStarC_Parser_AST.d with
      | FStarC_Parser_AST.Tycon
      ...
      | uu___ ->
          let uu___1 =
            ...
            FStarC_Format.fmt1
              ...
              uu___2 in
          failwith uu___1
and (p_subEffect : FStarC_Parser_AST.lift -> FStar_Pprint.document) =
  fun lift ->
    let lift_op_doc =
      let lifts =
...
and (p_qualifier : FStarC_Parser_AST.qualifier -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_qualifiers : FStarC_Parser_AST.qualifiers -> FStar_Pprint.document) =
  fun qs ->
    match qs with
    | [] -> FStar_Pprint.empty
    | q::[] ->
...
and (p_letqualifier :
  FStarC_Parser_AST.let_qualifier -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_aqual : FStarC_Parser_AST.arg_qualifier -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_disjunctivePattern :
  FStarC_Parser_AST.pattern -> FStar_Pprint.document) =
  fun p ->
...
and (p_tuplePattern : FStarC_Parser_AST.pattern -> FStar_Pprint.document) =
  fun p ->
    match p.FStarC_Parser_AST.pat with
...
and (p_constructorPattern :
  ...
  fun p ->
    ...
    | FStarC_Parser_AST.PatApp
        ...
        infix0
          (FStar_Pprint.op_Hat_Hat FStar_Pprint.colon FStar_Pprint.colon)
          uu___1 uu___2
    | FStarC_Parser_AST.PatApp
...
and (p_atomicPattern : FStarC_Parser_AST.pattern -> FStar_Pprint.document) =
  fun p ->
    match p.FStarC_Parser_AST.pat with
...
and (is_typ_tuple : FStarC_Parser_AST.term -> Prims.bool) =
  fun e ->
    ...
    | uu___ -> false
and (p_binder :
  Prims.bool -> FStarC_Parser_AST.binder -> FStar_Pprint.document) =
  fun is_atomic ->
    fun b ->
             ...
             | (b', t') ->
                 let catf1 =
                   ...
                   then
                     ...
                     (fun x ->
                        fun y -> FStar_Pprint.group (cat_with_colon x y)) in
        ...
        | FStarC_Parser_AST.NoName t ->
            (match t.FStarC_Parser_AST.tm with
             | FStarC_Parser_AST.Refine
                 ...
                 let uu___4 =
                   p_refinement' b.FStarC_Parser_AST.aqual
                     b.FStarC_Parser_AST.battributes FStar_Pprint.underscore
                     t1 phi in
                 (match uu___4 with
                  | (b', t') ->
                      (b',
                ...
                let uu___2 =
                  ...
                  FStar_Pprint.op_Hat_Hat uu___3
                    (FStar_Pprint.jump (Prims.of_int (2)) jump_break
                       (FStar_Pprint.group
                          (FStar_Pprint.ifflat
                             (soft_braces_with_nesting_tight phi2)
                             (soft_braces_with_nesting phi2)))) in
                (uu___1, uu___2)
and (p_binders_list :
  Prims.bool ->
    FStarC_Parser_AST.binder Prims.list -> FStar_Pprint.document Prims.list)
  = fun is_atomic -> fun bs -> FStarC_List.map (p_binder is_atomic) bs
and (p_binders :
  Prims.bool -> FStarC_Parser_AST.binder Prims.list -> FStar_Pprint.document)
  =
  fun is_atomic ->
    fun bs ->
...
and (p_binders_sep :
  FStarC_Parser_AST.binder Prims.list -> FStar_Pprint.document) =
  fun bs ->
...
and (p_term :
  Prims.bool -> Prims.bool -> FStarC_Parser_AST.term -> FStar_Pprint.document)
  =
  fun ps ->
    fun pb ->
      fun e ->
            ...
            (match uu___ with
             | (comm, t1) ->
                 let uu___1 =
                   ...
                   FStar_Pprint.op_Hat_Hat
                     (FStar_Pprint.group
                        (FStar_Pprint.op_Hat_Hat t1 FStar_Pprint.semi))
                     uu___2 in
                 (comm, uu___1))
        | FStarC_Parser_AST.Bind (x, e1, e2) ->
            let uu___ =
    ...
    fun pb ->
      fun e ->
                  ...
                  let uu___2 =
                    let uu___3 =
                      let uu___4 =
                        let uu___5 =
                          let uu___6 =
                            FStarC_Option.map FStarC_Ident.string_of_id
                              op_opt in
                       ...
                       let uu___4 =
                         let uu___5 =
                           ...
                           | FStar_Pervasives_Native.Some as_ident ->
                               ...
                               FStar_Pprint.op_Hat_Slash_Hat (str "as")
                                 uu___6 in
                         let uu___6 =
                ...
                fun is_last ->
                  ...
                  | (a, (pat, e2)) ->
                      ...
                      let doc_let_or_and =
                        match q1 with
                        | FStar_Pervasives_Native.Some
                            (FStarC_Parser_AST.LocalRec) ->
                            FStar_Pprint.group
                              (FStar_Pprint.op_Hat_Slash_Hat (str "let")
                                 (str "rec"))
                        | FStar_Pervasives_Native.Some
                            (FStarC_Parser_AST.LocalNoLetQualifier) ->
                            str "let"
                        | FStar_Pervasives_Native.Some
                            (FStarC_Parser_AST.LocalUnfold) ->
                            FStar_Pprint.group
                              (FStar_Pprint.op_Hat_Slash_Hat (str "let")
                                 (str "unfold"))
                        | uu___1 -> str "and" in
        ...
        | FStarC_Parser_AST.ElimForall (xs, p, vs) ->
            ...
            let p1 = p_noSeqTermAndComment false false p in
            let vs1 =
              FStarC_Pprint.separate_map FStar_Pprint.space p_atomicTerm vs in
    ...
    fun pb ->
      fun rel ->
        fun e ->
          ...
          FStar_Pprint.group uu___
and (p_calcStep :
  Prims.bool -> FStarC_Parser_AST.calc_step -> FStar_Pprint.document) =
  fun uu___ ->
    fun uu___1 ->
      match uu___1 with
...
and (p_typ_top :
  annotation_style ->
    Prims.bool ->
      Prims.bool -> FStarC_Parser_AST.term -> FStar_Pprint.document)
  =
  fun style ->
    fun ps ->
      fun pb ->
...
and (sig_as_binders_if_possible :
  FStarC_Parser_AST.term -> Prims.bool -> FStar_Pprint.document) =
  fun t ->
    fun extra_space ->
            ...
            (match bs with
             ...
             | hd::tl ->
                 ...
                 (match uu___1 with
                  | (b2s, t2, tc2, j2) ->
                      if ((t1 = t2) && j1) && j2
                      then ((FStarC_List.op_At b2s [b1]), t1, false, true) ::
                        tl
                ...
                let uu___2 =
                  FStarC_List.fold_left
                    (fun x ->
                       fun y ->
                         FStar_Pprint.op_Hat_Hat x
                           (FStar_Pprint.op_Hat_Hat FStar_Pprint.space y)) hd
                    tl in
...
and (p_quantifier : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
...
and (p_trigger :
  FStarC_Parser_AST.term Prims.list Prims.list -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
    | [] -> FStar_Pprint.empty
    | pats ->
...
and (p_disjunctivePats :
  FStarC_Parser_AST.term Prims.list Prims.list -> FStar_Pprint.document) =
  fun pats -> FStarC_Pprint.separate_map (str "\\/") p_conjunctivePats pats
and (p_conjunctivePats :
  FStarC_Parser_AST.term Prims.list -> FStar_Pprint.document) =
  fun pats ->
    let uu___ =
      FStarC_Pprint.separate_map
                 ...
                 uu___1
                   (if comm <> FStar_Pprint.empty
                    then
                      ...
                      FStar_Pprint.group (op_Hat_Slash_Plus_Hat prefix doc)))
        | uu___ -> p_tmIff e
...
and (p_tmIff : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    ...
    | uu___ -> p_tmImplies e
and (p_tmImplies : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    | FStarC_Parser_AST.Op (id, e1::e2::[]) when
        let uu___ = FStarC_Ident.string_of_id id in uu___ = "==>" ->
                 ...
                 | uu___1 ->
                     let uu___2 =
                                   ...
                                   let uu___9 =
                                     FStarC_List.map
                                       (fun x ->
                                          FStar_Pprint.align
                                            (FStar_Pprint.hang
                                               (Prims.of_int (2)) x)) terms in
                                   FStar_Pprint.separate
                                     (FStar_Pprint.op_Hat_Hat sep
                                        single_line_arg_indent) uu___9 in
                                 FStar_Pprint.op_Hat_Hat
                                   single_line_arg_indent uu___8 in
                               jump2 uu___7 in
                             FStar_Pprint.ifflat
                       ...
                       FStar_Pprint.ifflat
                         (FStar_Pprint.op_Hat_Hat fs
                            (FStar_Pprint.op_Hat_Hat
                               (FStar_Pprint.separate sep terms)
                               (FStar_Pprint.op_Hat_Hat one_line_space
                                  (FStar_Pprint.op_Hat_Hat last_op1 ret_d))))
                         uu___3 in
                     FStar_Pprint.group uu___2)
and (p_tmArrow :
  annotation_style ->
  ...
  fun style ->
    fun p_Tm ->
      fun e ->
        ...
        let rec accumulate_binders p_Tm1 =
          fun e1 ->
            match e1.FStarC_Parser_AST.tm with
            | FStarC_Parser_AST.Product (bs, tgt) ->
                let bs_ds =
                      ...
                      | (b2s, t2, tc2, j2) ->
                          (match (t1, t2) with
                           | (FStar_Pervasives_Native.Some (typ1, catf1),
                              FStar_Pervasives_Native.Some (typ2, uu___2))
                               when ((typ1 = typ2) && j1) && j2 ->
                               ((FStarC_List.op_At b2s [b1]), t1, false,
                                 true)
                               :: tl
        ...
        | (bs_ds, ret_d) ->
            ...
            (uu___1, ret_d)
and (p_tmFormula : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    let conj =
      FStar_Pprint.op_Hat_Hat FStar_Pprint.space
        (FStar_Pprint.op_Hat_Hat (str "/\\") break1) in
    ...
    FStarC_Pprint.flow_map disj
      ...
      formula
and (p_tmDisjunction :
  FStarC_Parser_AST.term -> FStar_Pprint.document Prims.list Prims.list) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    | FStarC_Parser_AST.Op (id, e1::e2::[]) when
        let uu___ = FStarC_Ident.string_of_id id in uu___ = "\\/" ->
...
and (p_tmConjunction :
  FStarC_Parser_AST.term -> FStar_Pprint.document Prims.list) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    | FStarC_Parser_AST.Op (id, e1::e2::[]) when
        let uu___ = FStarC_Ident.string_of_id id in uu___ = "/\\" ->
...
and (p_tmTuple : FStarC_Parser_AST.term -> FStar_Pprint.document) =
...
and (p_tmEqWith' :
  (FStarC_Parser_AST.term -> FStar_Pprint.document) ->
    Prims.int -> FStarC_Parser_AST.term -> FStar_Pprint.document)
  =
  fun p_X ->
    fun curr ->
      fun e ->
        ...
        fun e ->
          ...
          | FStarC_Parser_AST.Paren p when inside_tuple ->
              (match p.FStarC_Parser_AST.tm with
               ...
               | uu___ -> p_X e)
          | uu___ -> p_X e
and (p_tmEqNoRefinement : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e -> p_tmEqWith p_appTerm e
and (p_tmEq : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e -> p_tmEqWith p_tmRefinement e
and (p_tmNoEq : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e -> p_tmNoEqWith p_tmRefinement e
and (p_tmRefinement : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    ...
    | uu___ -> p_appTerm e
and (p_with_clause : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
...
and (p_refinedBinder :
  FStarC_Parser_AST.binder -> FStarC_Parser_AST.term -> FStar_Pprint.document)
  =
  fun b ->
    fun phi ->
      match b.FStarC_Parser_AST.b with
...
and (p_simpleDef :
  Prims.bool ->
    (FStarC_Ident.lid * FStarC_Parser_AST.term) -> FStar_Pprint.document)
  =
  fun ps ->
    fun uu___ ->
      match uu___ with
      | (lid, e) ->
          let uu___1 =
            ...
            FStar_Pprint.op_Hat_Slash_Hat uu___2 uu___3 in
          FStar_Pprint.group uu___1
and (p_appTerm : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
                       ...
                       let uu___4 =
                         soft_surround_map_or_flow (Prims.of_int (2))
                           ...
                           (FStar_Pprint.op_Hat_Hat head_doc
                              FStar_Pprint.space) break1 FStar_Pprint.empty
                           p_argTerm args1 in
                       FStar_Pprint.group uu___4)))
    | FStarC_Parser_AST.Construct (lid, args) when
...
and (p_argTerm :
  (FStarC_Parser_AST.term * FStarC_Parser_AST.imp) -> FStar_Pprint.document)
  =
  fun arg_imp ->
    match arg_imp with
  ...
  fun exit ->
    fun e ->
      ...
      | FStarC_Parser_AST.Op (id, e1::e2::[]) when
          ...
          FStar_Pprint.group uu___
      | uu___ -> exit e
and (p_indexingTerm : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e -> p_indexingTerm_aux p_atomicTerm e
and (p_atomicTerm : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
  ...
  fun e ->
    ...
    | FStarC_Parser_AST.Construct (lid, args) when
        ...
        let uu___ =
          FStarC_Pprint.separate_map
            (FStar_Pprint.op_Hat_Hat FStar_Pprint.comma break1)
            (fun uu___1 -> match uu___1 with | (e1, uu___2) -> p_tmEq e1)
            args in
        FStar_Pprint.surround (Prims.of_int (2)) Prims.int_one
          (FStar_Pprint.op_Hat_Hat FStar_Pprint.lparen FStar_Pprint.bar)
          uu___
...
and (p_projectionLHS : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e ->
    match e.FStarC_Parser_AST.tm with
    ...
    | FStarC_Parser_AST.Labeled (e1, s, b) ->
        let uu___ = p_term false false e1 in
        FStar_Pprint.op_Hat_Slash_Hat
          (str (Prims.strcat "(*" (Prims.strcat s "*)"))) uu___
...
and (p_constant : FStarC_Const.sconst -> FStar_Pprint.document) =
  fun uu___ ->
    match uu___ with
...
and (p_universe : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun u ->
    let uu___ = p_atomicUniverse u in
    FStar_Pprint.op_Hat_Hat (str "u#") uu___
and (p_universeFrom : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun u ->
...
and (p_atomicUniverse : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun u ->
    match u.FStarC_Parser_AST.tm with
    | FStarC_Parser_AST.Wild -> FStar_Pprint.underscore
    ...
    | uu___ ->
        ...
        failwith uu___1
let (term_to_document : FStarC_Parser_AST.term -> FStar_Pprint.document) =
  fun e -> p_term false false e
let (signature_to_document : FStarC_Parser_AST.decl -> FStar_Pprint.document)
  = fun e -> p_justSig e
let (decl_to_document : FStarC_Parser_AST.decl -> FStar_Pprint.document) =
  fun e -> p_decl e
let (pat_to_document : FStarC_Parser_AST.pattern -> FStar_Pprint.document) =
  fun p -> p_disjunctivePattern p
let (binder_to_document : FStarC_Parser_AST.binder -> FStar_Pprint.document)
  = fun b -> p_binder true b
let (modul_to_document : FStarC_Parser_AST.modul -> FStar_Pprint.document) =
  fun m ->
    match m with
...
let (comments_to_document :
  (Prims.string * FStarC_Range_Type.t) Prims.list -> FStar_Pprint.document) =
  fun comments ->
    FStarC_Pprint.separate_map FStar_Pprint.hardline
...
let (modul_with_comments_to_document :
  FStarC_Parser_AST.modul ->
    (Prims.string * FStarC_Range_Type.t) Prims.list ->
      (FStar_Pprint.document * (Prims.string * FStarC_Range_Type.t)
        Prims.list))
  =
  fun m ->
    fun comments ->
