...
let dbg_NBETop : Prims.bool FStarC_Effect.ref=
...
let debug_term (t : FStarC_Syntax_Syntax.term) : unit=
  let uu___ = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t in
  FStarC_Format.print1 "%s\n" uu___
let debug_sigmap (m : FStarC_Syntax_Syntax.sigelt FStarC_SMap.t) : unit=
  FStarC_SMap.fold m
    (fun k v u ->
...
let __proj__Mkconfig__item__core_cfg (projectee : config) :
  FStarC_TypeChecker_Cfg.cfg=
  match projectee with | { core_cfg; fv_cache;_} -> core_cfg
let __proj__Mkconfig__item__fv_cache (projectee : config) :
  ...
  then
    new_config
      (let uu___ = cfg.core_cfg in
       {
         FStarC_TypeChecker_Cfg.steps = (uu___.FStarC_TypeChecker_Cfg.steps);
         FStarC_TypeChecker_Cfg.tcenv = (uu___.FStarC_TypeChecker_Cfg.tcenv);
         FStarC_TypeChecker_Cfg.debug = (uu___.FStarC_TypeChecker_Cfg.debug);
...
let rec unlazy_unmeta (t : FStarC_TypeChecker_NBETerm.t) :
  ...
  | FStarC_TypeChecker_NBETerm.Meta (t0, m) ->
      ...
      (match uu___ with
       ...
       | FStarC_Syntax_Syntax.Meta_monadic_lift (uu___1, uu___2, uu___3) -> t
       | uu___1 -> unlazy_unmeta t0)
  | uu___ -> t
let pickBranch (cfg : config) (scrut : FStarC_TypeChecker_NBETerm.t)
  (branches : FStarC_Syntax_Syntax.branch Prims.list) :
  ...
  let rec pickBranch_aux scrut1 branches1 branches0 =
    let rec matches_pat scrutinee0 p =
      debug cfg
        (fun uu___1 ->
           let uu___2 = FStarC_TypeChecker_NBETerm.t_to_string scrutinee0 in
           let uu___3 =
      ...
      (let scrutinee = unlazy_unmeta scrutinee0 in
       let r =
         ...
         | FStarC_Syntax_Syntax.Pat_constant s ->
             let matches_const c s1 =
               debug cfg
                 (fun uu___2 ->
                    let uu___3 = FStarC_TypeChecker_NBETerm.t_to_string c in
                    let uu___4 =
               ...
               (match c.FStarC_TypeChecker_NBETerm.nbe_t with
                ...
                | FStarC_TypeChecker_NBETerm.Constant
                    ...
                    (match s1 with
                     | FStarC_Const.Const_string (p1, uu___3) -> st = p1
                     | uu___3 -> false)
                | FStarC_TypeChecker_NBETerm.Constant
       ...
       debug cfg
         (fun uu___2 ->
            let uu___3 = FStarC_TypeChecker_NBETerm.t_to_string scrutinee in
            let uu___4 =
              FStarC_Class_Show.show FStarC_Syntax_Print.showable_pat p in
...
let should_reduce_recursive_definition
  ...
  (formals_in_decreases : Prims.bool Prims.list) :
...
let find_sigelt_in_gamma (cfg : config) (env : FStarC_TypeChecker_Env.env)
  ...
  FStarC_Option.bind uu___ mapper
let is_univ (tm : FStarC_TypeChecker_NBETerm.t) : Prims.bool=
  match tm.FStarC_TypeChecker_NBETerm.nbe_t with
  | FStarC_TypeChecker_NBETerm.Univ uu___ -> true
  | uu___ -> false
let un_univ (tm : FStarC_TypeChecker_NBETerm.t) :
...
let is_constr (q : FStarC_TypeChecker_Env.qninfo) : Prims.bool=
  match q with
  | FStar_Pervasives_Native.Some
      (FStar_Pervasives.Inr
       ({
          FStarC_Syntax_Syntax.sigel = FStarC_Syntax_Syntax.Sig_datacon uu___;
...
let translate_univ (cfg : config)
  ...
  let rec aux u1 =
    ...
    | FStarC_Syntax_Syntax.U_bvar i ->
        ...
        then let u' = FStarC_List.nth bs i in un_univ u'
        else
          if
  ...
  aux u
let find_let (lbs : FStarC_Syntax_Syntax.letbinding Prims.list)
  (fvar : FStarC_Syntax_Syntax.fv) :
...
let mk_t (t : FStarC_TypeChecker_NBETerm.t') : FStarC_TypeChecker_NBETerm.t=
  {
    FStarC_TypeChecker_NBETerm.nbe_t = t;
    FStarC_TypeChecker_NBETerm.nbe_r = FStarC_Range_Type.dummyRange
  }
let rec translate (cfg : config)
  (bs : FStarC_TypeChecker_NBETerm.t Prims.list)
  (e : FStarC_Syntax_Syntax.term) : FStarC_TypeChecker_NBETerm.t=
  ...
  debug1
    (fun uu___1 ->
       let uu___2 =
         ...
         FStarC_Class_Tagged.tag_of FStarC_Syntax_Syntax.tagged_term uu___3 in
       let uu___3 =
         let uu___4 = FStarC_Syntax_Subst.compress e in
         FStarC_Class_Show.show FStarC_Syntax_Print.showable_term uu___4 in
  ...
  (let uu___1 =
   ...
   | FStarC_Syntax_Syntax.Tm_bvar db ->
       if db.FStarC_Syntax_Syntax.index < (FStarC_List.length bs)
       then
         let t = FStarC_List.nth bs db.FStarC_Syntax_Syntax.index in
         (debug1
            (fun uu___3 ->
               let uu___4 = FStarC_TypeChecker_NBETerm.t_to_string t in
               let uu___5 =
                 let uu___6 =
                   FStarC_List.map FStarC_TypeChecker_NBETerm.t_to_string bs in
   ...
   | FStarC_Syntax_Syntax.Tm_arrow uu___2 ->
       let uu___3 = FStarC_Syntax_Util.arrow_formals_comp_ln_strict e in
       (match uu___3 with
        | (xs, c) ->
            let norm uu___4 =
              let uu___5 =
                  ...
                  (fun uu___6 b ->
                     ...
                     | (ctx, binders_rev) ->
                         ...
                         let t =
                           ...
                           readback cfg uu___7 in
                         let x1 =
                           let uu___7 = FStarC_Syntax_Syntax.freshen_bv x in
                           {
                             FStarC_Syntax_Syntax.ppname =
                               (uu___7.FStarC_Syntax_Syntax.ppname);
              ...
              | (ctx, binders_rev) ->
                  let c1 =
                    let uu___6 = translate_comp cfg ctx c in
                    readback_comp cfg uu___6 in
                  FStarC_Syntax_Util.arrow (FStarC_List.rev binders_rev) c1 in
            let uu___4 =
              let uu___5 =
                let uu___6 = FStarC_Thunk.mk norm in
                FStar_Pervasives.Inl uu___6 in
              FStarC_TypeChecker_NBETerm.Arrow uu___5 in
            mk_t1 uu___4)
   | FStarC_Syntax_Syntax.Tm_refine
       { FStarC_Syntax_Syntax.b2 = bv; FStarC_Syntax_Syntax.phi = tm;_} ->
       if
         ...
         mk_t1
           (FStarC_TypeChecker_NBETerm.Refinement
              ((fun y -> translate cfg (y :: bs) tm),
                (fun uu___2 ->
                   let uu___3 = translate cfg bs bv.FStarC_Syntax_Syntax.sort in
                   FStarC_TypeChecker_NBETerm.as_arg uu___3)))
   | FStarC_Syntax_Syntax.Tm_ascribed
       { FStarC_Syntax_Syntax.tm = t; FStarC_Syntax_Syntax.asc = uu___2;
       ...
       let norm_uvar uu___2 =
         let norm_subst_elt uu___3 =
           ...
           | FStarC_Syntax_Syntax.NT (x, t) ->
               let uu___4 =
                 let uu___5 =
                   let uu___6 = translate cfg bs t in readback cfg uu___6 in
                 (x, uu___5) in
           ...
           | FStarC_Syntax_Syntax.NM (x, i) ->
               ...
               let t =
                 let uu___4 = translate cfg bs x_i in readback cfg uu___4 in
   ...
   | FStarC_Syntax_Syntax.Tm_abs uu___2 ->
       let uu___3 = FStarC_Syntax_Util.abs_formals_ln e in
       (match uu___3 with
        | (xs, body, resc) ->
            mk_t1
              (FStarC_TypeChecker_NBETerm.Lam
                 {
                   FStarC_TypeChecker_NBETerm.interp =
                     (fun ys ->
                        let uu___4 =
                          let uu___5 =
                            FStarC_List.map FStar_Pervasives_Native.fst ys in
                          FStarC_List.append uu___5 bs in
                        translate cfg uu___4 body);
                   FStarC_TypeChecker_NBETerm.shape =
                     (FStarC_TypeChecker_NBETerm.Lam_bs (bs, xs, resc));
                   FStarC_TypeChecker_NBETerm.arity = (FStarC_List.length xs)
                 }))
   ...
   | FStarC_Syntax_Syntax.Tm_app uu___2 ->
       let uu___3 = FStarC_Syntax_Util.head_and_args_full e in
       (match uu___3 with
        | (head, args) ->
            let uu___4 =
              let uu___5 =
            ...
            (match uu___4 with
             | (FStarC_Syntax_Syntax.Tm_constant (FStarC_Const.Const_reify
                uu___5), arg::more::args1) ->
                 let h =
                   let uu___6 =
                     FStarC_Syntax_Syntax.mk_Tm_app head [arg]
                       e.FStarC_Syntax_Syntax.pos in
                   translate cfg bs uu___6 in
                 let uu___6 =
                   FStarC_List.map
                     (fun x ->
                        let uu___7 =
                          translate cfg bs (FStar_Pervasives_Native.fst x) in
                        (uu___7, (FStar_Pervasives_Native.snd x))) (more ::
                     args1) in
                 iapp cfg h uu___6
             ...
             | (FStarC_Syntax_Syntax.Tm_constant (FStarC_Const.Const_reflect
                uu___5), arg::[]) when
                 (cfg.core_cfg).FStarC_TypeChecker_Cfg.reifying ->
             ...
             | (FStarC_Syntax_Syntax.Tm_constant (FStarC_Const.Const_reflect
                uu___5), arg::[]) ->
                 let uu___6 =
                   ...
                   FStarC_TypeChecker_NBETerm.Reflect uu___7 in
                 mk_t1 uu___6
             | (FStarC_Syntax_Syntax.Tm_fvar fv, uu___5::[]) when
                 ...
                 (debug1
                    (fun uu___7 ->
                       FStarC_Format.print_string "Eliminated assertion\n");
                  mk_t1
                    (FStarC_TypeChecker_NBETerm.Constant
                       FStarC_TypeChecker_NBETerm.Unit))
             | uu___5 ->
                 ...
                 then
                   let uu___6 = translate cfg bs head in
                   let uu___7 =
                     FStarC_List.map
                       (fun x ->
                          let uu___8 =
                          ...
                          then
                            ...
                            (let uu___9 =
                               translate cfg bs
                                 (FStar_Pervasives_Native.fst x) in
                             (uu___9, (FStar_Pervasives_Native.snd x)))) args in
                   iapp cfg uu___6 uu___7
                 else
                   (debug1
                      (fun uu___7 ->
                         let uu___8 =
                         ...
                         FStarC_Format.print2 "Application: %s @ %s\n" uu___8
                           uu___9);
                    (let uu___7 = translate cfg bs head in
                     let uu___8 =
                       FStarC_List.map
                         (fun x ->
                            let uu___9 =
                  ...
                  | (bs', args') ->
                      let us_opt1 =
                        ...
                        | FStar_Pervasives_Native.Some us ->
                            let uu___5 =
                              FStarC_List.map (translate_univ cfg1 bs1) us in
                            FStar_Pervasives_Native.Some uu___5 in
                      (bs',
                        (FStarC_Syntax_Syntax.Pat_cons
                           (fvar, us_opt1, (FStarC_List.rev args')))))
             | FStarC_Syntax_Syntax.Pat_var bvar ->
                 let x =
                   let uu___4 =
                     let uu___5 =
                       translate cfg1 bs1 bvar.FStarC_Syntax_Syntax.sort in
       ...
       (debug1
        ...
        (let scrut2 = unlazy_unmeta scrut1 in
             ...
             (debug1
              ...
              (let uu___4 = pickBranch cfg scrut2 branches in
               ...
               | FStar_Pervasives_Native.Some (branch, args1) ->
                   let uu___5 =
                     FStarC_List.fold_left (fun bs1 x -> x :: bs1) bs args1 in
         ...
         | FStarC_TypeChecker_NBETerm.Constant c ->
             (debug1
                (fun uu___4 ->
                   let uu___5 = FStarC_TypeChecker_NBETerm.t_to_string scrut2 in
   ...
   | FStarC_Syntax_Syntax.Tm_meta
       { FStarC_Syntax_Syntax.tm2 = e1;
         FStarC_Syntax_Syntax.meta = FStarC_Syntax_Syntax.Meta_monadic (m, t);_}
   ...
   | FStarC_Syntax_Syntax.Tm_meta
       { FStarC_Syntax_Syntax.tm2 = e1; FStarC_Syntax_Syntax.meta = meta;_}
       ->
       let norm_meta uu___2 =
         let norm t = let uu___3 = translate cfg bs t in readback cfg uu___3 in
         match meta with
         ...
         | FStarC_Syntax_Syntax.Meta_pattern (ts, args) ->
             let uu___3 =
               let uu___4 = FStarC_List.map norm ts in
               let uu___5 =
                 FStarC_List.map
                   (FStarC_List.map
                      (fun uu___6 ->
       ...
       then
         ...
         (let def uu___3 =
          ...
          let bs1 =
            ...
            :: bs in
          let body1 uu___3 = translate cfg bs1 body in
          let uu___3 =
            let uu___4 =
              let uu___5 =
                let uu___6 =
                  let uu___7 = FStarC_Thunk.mk typ in
                  let uu___8 = FStarC_Thunk.mk def in
                  let uu___9 = FStarC_Thunk.mk body1 in
                  (name, uu___7, uu___8, uu___9, lb) in
                FStarC_TypeChecker_NBETerm.UnreducedLet uu___6 in
              (uu___5, []) in
            FStarC_TypeChecker_NBETerm.Accu uu___4 in
          mk_t1 uu___3)
   | FStarC_Syntax_Syntax.Tm_let
       ...
       else (let uu___2 = make_rec_env lbs bs in translate cfg uu___2 body)
           ...
           FStarC_List.map
             (fun uu___2 ->
                FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None
                  FStarC_Syntax_Syntax.tun) bs in
         let s1 =
...
and iapp (cfg : config) (f : FStarC_TypeChecker_NBETerm.t)
  ...
  let uu___ =
    let uu___1 = unlazy_unmeta f in uu___1.FStarC_TypeChecker_NBETerm.nbe_t in
  match uu___ with
  | FStarC_TypeChecker_NBETerm.Lam
      ...
      ->
      let m = FStarC_List.length args in
      if m < n
      then
        let arg_values_rev = FStarC_List.rev args in
        let shape1 =
          match shape with
          | FStarC_TypeChecker_NBETerm.Lam_args raw_args ->
              let uu___1 = FStarC_List.splitAt m raw_args in
              (match uu___1 with
               | (uu___2, raw_args1) ->
                   FStarC_TypeChecker_NBETerm.Lam_args raw_args1)
          | FStarC_TypeChecker_NBETerm.Lam_bs (ctx, xs, rc) ->
              let uu___1 = FStarC_List.splitAt m xs in
              (match uu___1 with
               | (uu___2, xs1) ->
                   let ctx1 =
                     let uu___3 =
                       FStarC_List.map FStar_Pervasives_Native.fst
                         arg_values_rev in
                     FStarC_List.append uu___3 ctx in
          ...
          | FStarC_TypeChecker_NBETerm.Lam_primop (f2, args_acc) ->
              FStarC_TypeChecker_NBETerm.Lam_primop
                (f2, (FStarC_List.op_At args_acc args)) in
        mk
          (FStarC_TypeChecker_NBETerm.Lam
             {
               FStarC_TypeChecker_NBETerm.interp =
                 (fun l -> f1 (FStarC_List.append l arg_values_rev));
               FStarC_TypeChecker_NBETerm.shape = shape1;
               FStarC_TypeChecker_NBETerm.arity = (n - m)
             })
      else
        if m = n
        then (let arg_values_rev = FStarC_List.rev args in f1 arg_values_rev)
        else
          (let uu___1 = FStarC_List.splitAt n args in
           match uu___1 with
           | (args1, args') ->
               let uu___2 = f1 (FStarC_List.rev args1) in
               iapp cfg uu___2 args')
  | FStarC_TypeChecker_NBETerm.Accu (a, ts) ->
      mk
        (FStarC_TypeChecker_NBETerm.Accu
           (a, (FStarC_List.rev_append args ts)))
      ...
      (debug cfg
       ...
       then
         (let uu___2 =
            let uu___3 =
              let uu___4 =
                FStarC_Syntax_Util.unascribe lb.FStarC_Syntax_Syntax.lbdef in
            ...
            | FStarC_Syntax_Syntax.Tm_abs uu___4 ->
                let uu___5 =
                  ...
                  FStarC_Syntax_Util.abs_formals_ln uu___6 in
                (match uu___5 with | (bs, body, uu___6) -> (bs, body))
            | uu___4 -> ([], (lb.FStarC_Syntax_Syntax.lbdef)) in
          match uu___2 with
          | (bs, body) ->
              if (n_univs + (FStarC_List.length bs)) = arity
              then
                let uu___3 =
                  FStarC_Util.first_N (n_args_rev - arity) args_rev1 in
                (match uu___3 with
                 | (extra, args_rev2) ->
                     (debug cfg
                        (fun uu___5 ->
                      ...
                      (let t =
                       ...
                       match extra with
                       | [] -> t
                       | uu___5 -> iapp cfg t (FStarC_List.rev extra))))
              else
                (let uu___3 =
                   FStarC_Util.first_N (n_args_rev - n_univs) args_rev1 in
                 match uu___3 with
                 | (extra, univs) ->
                     let uu___4 =
                       let uu___5 =
                         FStarC_List.map FStar_Pervasives_Native.fst univs in
                       translate cfg uu___5 lb.FStarC_Syntax_Syntax.lbdef in
                     iapp cfg uu___4 (FStarC_List.rev extra)))
       else
         mk (FStarC_TypeChecker_NBETerm.TopLevelLet (lb, arity, args_rev1)))
  | FStarC_TypeChecker_NBETerm.TopLevelRec (lb, arity, decreases_list, args')
      ->
      let args1 = FStarC_List.append args' args in
      if (FStarC_List.length args1) >= arity
      then
                 ...
                 | (univs, rest) ->
                     let uu___6 =
                       let uu___7 =
                         let uu___8 =
                           FStarC_List.map FStar_Pervasives_Native.fst univs in
                         FStarC_List.rev uu___8 in
                       translate cfg uu___7 lb.FStarC_Syntax_Syntax.lbdef in
                     iapp cfg uu___6 rest)))
      else
        mk
        ...
        mk
          (FStarC_TypeChecker_NBETerm.LocalLetRec
             (i, lb, mutual_lbs, local_env,
               (FStarC_List.op_At acc_args args), remaining_arity,
               decreases_list))
      else
        (let n_args = FStarC_List.length args in
         ...
         then
           mk
             (FStarC_TypeChecker_NBETerm.LocalLetRec
                (i, lb, mutual_lbs, local_env,
                  (FStarC_List.op_At acc_args args),
                  (remaining_arity - n_args), decreases_list))
         else
           (let args1 = FStarC_List.op_At acc_args args in
            let uu___1 =
            ...
            | (should_reduce, uu___2, uu___3) ->
                ...
                then
                  mk
                    (FStarC_TypeChecker_NBETerm.LocalLetRec
                       (i, lb, mutual_lbs, local_env, args1, Prims.int_zero,
                         decreases_list))
                else
                   ...
                   debug cfg
                     (fun uu___5 ->
                        (let uu___7 =
                           let uu___8 =
                             FStarC_List.map
                               FStarC_TypeChecker_NBETerm.t_to_string env in
  ...
  then FStarC_TypeChecker_NBETerm.mkConstruct fvar [] []
    ...
    (let uu___ =
       FStarC_TypeChecker_Normalize_Unfolding.should_unfold false
         cfg.core_cfg
         (fun uu___1 -> (cfg.core_cfg).FStarC_TypeChecker_Cfg.reifying) fvar
         qninfo in
     match uu___ with
                  ...
                  (FStarC_TypeChecker_NBETerm.Lam
                     {
                       FStarC_TypeChecker_NBETerm.interp =
                         (fun args_rev ->
                            let args' = FStarC_List.rev args_rev in
                            let callbacks =
                              {
                                FStarC_TypeChecker_NBETerm.iapp = (iapp cfg);
                   ...
                   | FStar_Pervasives_Native.Some lb ->
                       if
                         is_rec &&
                           ((cfg.core_cfg).FStarC_TypeChecker_Cfg.steps).FStarC_TypeChecker_Cfg.zeta
                       then
                         let uu___8 = let_rec_arity lb in
                         (match uu___8 with
                          | (ar, lst) ->
...
and translate_letbinding (cfg : config)
  (bs : FStarC_TypeChecker_NBETerm.t Prims.list)
  (lb : FStarC_Syntax_Syntax.letbinding) : FStarC_TypeChecker_NBETerm.t=
  let debug1 = debug cfg in
  let us = lb.FStarC_Syntax_Syntax.lbunivs in
  let uu___ = FStarC_Syntax_Util.arrow_formals lb.FStarC_Syntax_Syntax.lbtyp in
  match uu___ with
  | (formals, uu___1) ->
      let arity = (FStarC_List.length us) + (FStarC_List.length formals) in
...
and mkRec (i : Prims.int) (b : FStarC_Syntax_Syntax.letbinding)
  ...
  (env : FStarC_TypeChecker_NBETerm.t Prims.list) :
  FStarC_TypeChecker_NBETerm.t=
...
and make_rec_env (all_lbs : FStarC_Syntax_Syntax.letbinding Prims.list)
  (all_outer_bs : FStarC_TypeChecker_NBETerm.t Prims.list) :
  FStarC_TypeChecker_NBETerm.t Prims.list=
  let rec_bindings =
    FStarC_List.mapi (fun i lb -> mkRec i lb all_lbs all_outer_bs) all_lbs in
  FStarC_List.rev_append rec_bindings all_outer_bs
and translate_constant (c : FStarC_Syntax_Syntax.sconst) :
  FStarC_TypeChecker_NBETerm.constant=
  match c with
...
and readback_comp (cfg : config) (c : FStarC_TypeChecker_NBETerm.comp) :
...
and translate_comp_typ (cfg : config)
  (bs : FStarC_TypeChecker_NBETerm.t Prims.list)
  ...
  | { FStarC_Syntax_Syntax.comp_univs = comp_univs;
      ...
      let uu___3 =
        FStarC_List.map
          (fun x ->
             let uu___4 = translate cfg bs (FStar_Pervasives_Native.fst x) in
             (uu___4, (FStar_Pervasives_Native.snd x))) effect_args in
      let uu___4 = FStarC_List.map (translate_flag cfg bs) flags in
      {
...
and translate_residual_comp (cfg : config)
  (bs : FStarC_TypeChecker_NBETerm.t Prims.list)
  (c : FStarC_Syntax_Syntax.residual_comp) :
  ...
  | { FStarC_Syntax_Syntax.residual_effect = residual_effect;
      ...
      let uu___1 =
        ...
        else FStarC_Option.map (translate cfg bs) residual_typ in
      let uu___2 = FStarC_List.map (translate_flag cfg bs) residual_flags in
      {
...
and readback_residual_comp (cfg : config)
  (c : FStarC_TypeChecker_NBETerm.residual_comp) :
  ...
  let uu___1 =
    FStarC_List.map (readback_flag cfg)
                ...
                let body_lam =
                  ...
                  FStarC_Syntax_Syntax.mk_Tm_abs
                    [FStarC_Syntax_Syntax.mk_binder
                       (FStar_Pervasives.__proj__Inl__item__v
                          lb.FStarC_Syntax_Syntax.lbname)] body
       ...
       | FStarC_Syntax_Syntax.Tm_app
           {
             ...
             FStarC_Syntax_Syntax.arg = (e2, uu___5);_}
           -> let uu___6 = reifying_false cfg in translate uu___6 bs e2
       | FStarC_Syntax_Syntax.Tm_app uu___1 ->
           let uu___2 = FStarC_Syntax_Util.head_and_args_full e1 in
           (match uu___2 with
            | (head, args) ->
                (debug cfg
                   (fun uu___4 ->
                 ...
                 (let fallback1 uu___4 = translate cfg bs e1 in
                  let fallback2 uu___4 =
                    let uu___5 = reifying_false cfg in
                    let uu___6 =
                  ...
                  | FStarC_Syntax_Syntax.Tm_fvar fv ->
                      ...
                      then fallback1 ()
                        ...
                        (let uu___6 =
                           ...
                           FStar_Pervasives_Native.uu___is_None uu___7 in
                         if uu___6
                         then fallback2 ()
                         else
                           (let e2 =
  ...
  | (msrc, mtgt, ty) ->
      ...
      then
        ...
        (let uu___1 =
         ...
         | FStar_Pervasives_Native.Some
             { FStarC_TypeChecker_Env.msource = uu___2;
               FStarC_TypeChecker_Env.mtarget = uu___3;
               FStarC_TypeChecker_Env.mlift =
                 { FStarC_TypeChecker_Env.mlift_wp = uu___4;
                   FStarC_TypeChecker_Env.mlift_term =
                     FStar_Pervasives_Native.None;_};
               FStarC_TypeChecker_Env.mpath = uu___5;_}
             ->
             FStarC_Effect.failwith
               (FStarC_Format.fmt2
         ...
         | FStar_Pervasives_Native.Some
             ...
             let lift_lam =
               ...
               let uu___6 =
                 ...
                 lift FStarC_Syntax_Syntax.U_unknown ty uu___7 in
               FStarC_Syntax_Util.abs [FStarC_Syntax_Syntax.mk_binder x]
                 uu___6 FStar_Pervasives_Native.None in
             let cfg' = reifying_false cfg in
             let t =
               let uu___6 = translate cfg' [] lift_lam in
               let uu___7 =
                 let uu___8 =
                   let uu___9 = translate cfg bs e1 in
                   (uu___9, FStar_Pervasives_Native.None) in
                 [uu___8] in
               iapp cfg uu___6 uu___7 in
             (debug cfg
                (fun uu___7 ->
                   let uu___8 = FStarC_TypeChecker_NBETerm.t_to_string t in
...
and readback (cfg : config) (x : FStarC_TypeChecker_NBETerm.t) :
  ...
  (match x.FStarC_TypeChecker_NBETerm.nbe_t with
   ...
   | FStarC_TypeChecker_NBETerm.Constant (FStarC_TypeChecker_NBETerm.String
       (s, r)) ->
       mk
         (FStarC_Syntax_Syntax.Tm_constant (FStarC_Const.Const_string (s, r)))
   | FStarC_TypeChecker_NBETerm.Constant (FStarC_TypeChecker_NBETerm.Char c)
              ...
              FStarC_List.fold_left
                (fun uu___2 b ->
                   ...
                   | (ctx1, binders_rev, accus_rev) ->
                       ...
                       let tnorm =
                         let uu___3 =
                           translate cfg ctx1 x1.FStarC_Syntax_Syntax.sort in
                         readback cfg uu___3 in
                       let x2 =
                         let uu___3 = FStarC_Syntax_Syntax.freshen_bv x1 in
                         {
                           FStarC_Syntax_Syntax.ppname =
             ...
             | (ctx1, binders_rev, accus_rev) ->
                 let rc1 =
                   ...
                   | FStar_Pervasives_Native.Some rc2 ->
                       let uu___2 =
                         let uu___3 = translate_residual_comp cfg ctx1 rc2 in
   ...
   | FStarC_TypeChecker_NBETerm.Refinement (f, targ) ->
       ...
       then
         ...
         (let x1 =
            let uu___1 =
              let uu___2 =
                let uu___3 = targ () in FStar_Pervasives_Native.fst uu___3 in
              readback cfg uu___2 in
            FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None uu___1 in
          let body =
            let uu___1 = f (FStarC_TypeChecker_NBETerm.mkAccuVar x1) in
            readback cfg uu___1 in
           ...
           (fun uu___1 ->
              ...
              | (t, q) ->
                  ...
                  let x1 =
                    FStarC_Syntax_Syntax.new_bv FStar_Pervasives_Native.None
                      t1 in
                  let uu___2 = FStarC_Syntax_Util.bqual_and_attrs_of_aqual q in
                  (match uu___2 with
                   | (q1, attrs) ->
                       let uu___3 =
                       ...
                       (match uu___3 with
                        | (pqual, attrs1) ->
                            FStarC_Syntax_Syntax.mk_binder_with_attrs x1 q1
                              pqual attrs1))) args in
   ...
   | FStarC_TypeChecker_NBETerm.FV (fv, us, args) ->
       ...
       with_range uu___1
   | FStarC_TypeChecker_NBETerm.Accu (FStarC_TypeChecker_NBETerm.Var bv, [])
       ->
       let uu___1 = FStarC_Syntax_Syntax.bv_to_name bv in with_range uu___1
   | FStarC_TypeChecker_NBETerm.Accu
   ...
   | FStarC_TypeChecker_NBETerm.Accu
       ...
       with_range uu___1
   | FStarC_TypeChecker_NBETerm.Accu
       (FStarC_TypeChecker_NBETerm.UnreducedLet (var, typ, defn, body, lb),
        args)
       ->
       let typ1 = let uu___1 = FStarC_Thunk.force typ in readback cfg uu___1 in
       let defn1 =
       ...
       let args1 = readback_args cfg args in
       let uu___1 = FStarC_Syntax_Util.mk_app hd args1 in with_range uu___1
   | FStarC_TypeChecker_NBETerm.Accu
       (FStarC_TypeChecker_NBETerm.UnreducedLetRec
   ...
   | FStarC_TypeChecker_NBETerm.TopLevelLet (lb, arity, args_rev) ->
       ...
       let uu___1 = FStarC_Util.first_N (n_args - n_univs) args_rev in
       (match uu___1 with
        | (args_rev1, univs) ->
            let uu___2 =
              let uu___3 =
              ...
              iapp cfg uu___3 (FStarC_List.rev args_rev1) in
            readback cfg uu___2)
   | FStarC_TypeChecker_NBETerm.TopLevelRec (lb, uu___1, uu___2, args) ->
       ...
       let args1 =
         FStarC_List.map
           (fun uu___3 ->
              match uu___3 with
              | (t, q) -> let uu___4 = readback cfg t in (uu___4, q)) args in
       let uu___3 = FStarC_Syntax_Util.mk_app head args1 in with_range uu___3
   | FStarC_TypeChecker_NBETerm.LocalLetRec
       ...
       let lbs1 =
         FStarC_List.map2
           (fun lb lbname ->
              let lbdef =
                let uu___2 =
                  translate cfg let_rec_env lb.FStarC_Syntax_Syntax.lbdef in
                readback cfg uu___2 in
              let lbtyp =
                let uu___2 = translate cfg bs lb.FStarC_Syntax_Syntax.lbtyp in
                readback cfg uu___2 in
              {
        ...
        | (lbs2, body1) ->
            ...
            let args1 =
              FStarC_List.map
                (fun uu___3 ->
                   match uu___3 with
                   | (x1, q) -> let uu___4 = readback cfg x1 in (uu___4, q))
                args in
...
let reduce_application (cfg : FStarC_TypeChecker_Cfg.cfg)
  (t : FStarC_TypeChecker_NBETerm.t) (args : FStarC_TypeChecker_NBETerm.args)
  : FStarC_TypeChecker_NBETerm.t=
  let uu___ = new_config cfg in iapp uu___ t args
let normalize
  (psteps : FStarC_TypeChecker_Primops_Base.primitive_step Prims.list)
  (steps : FStarC_TypeChecker_Env.step Prims.list)
  (env : FStarC_TypeChecker_Env.env) (e : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term=
  ...
  let cfg1 =
    {
      FStarC_TypeChecker_Cfg.steps =
        (let uu___ = cfg.FStarC_TypeChecker_Cfg.steps in
         {
           FStarC_TypeChecker_Cfg.beta = (uu___.FStarC_TypeChecker_Cfg.beta);
           FStarC_TypeChecker_Cfg.iota = (uu___.FStarC_TypeChecker_Cfg.iota);
           FStarC_TypeChecker_Cfg.zeta = (uu___.FStarC_TypeChecker_Cfg.zeta);
           FStarC_TypeChecker_Cfg.zeta_full =
             (uu___.FStarC_TypeChecker_Cfg.zeta_full);
           FStarC_TypeChecker_Cfg.weak = (uu___.FStarC_TypeChecker_Cfg.weak);
           FStarC_TypeChecker_Cfg.hnf = (uu___.FStarC_TypeChecker_Cfg.hnf);
           FStarC_TypeChecker_Cfg.primops =
             (uu___.FStarC_TypeChecker_Cfg.primops);
           FStarC_TypeChecker_Cfg.do_not_unfold_pure_lets =
  ...
  (let cfg2 = new_config cfg1 in
   ...
   r)
let normalize_for_unit_test (steps : FStarC_TypeChecker_Env.step Prims.list)
  (env : FStarC_TypeChecker_Env.env) (e : FStarC_Syntax_Syntax.term) :
  FStarC_Syntax_Syntax.term=
