...
let (fstar_refl_builtins_lid : Prims.string -> FStarC_Ident.lident) =
  fun s -> fstar_refl_lid ["V2"; "Builtins"; s]
let (fstar_refl_data_lid : Prims.string -> FStarC_Ident.lident) =
  fun s -> fstar_refl_lid ["V2"; "Data"; s]
...
let (mk_ss_lid_as_fv : Prims.string -> FStarC_Syntax_Syntax.fv) =
  fun s ->
    let uu___ = fstar_syntax_syntax_lid [s] in
    FStarC_Syntax_Syntax.fvconst uu___
let (mk_ss_lid_as_term : Prims.string -> FStarC_Syntax_Syntax.term) =
  fun s ->
    let uu___ = fstar_syntax_syntax_lid [s] in
...
let (fstar_refl_subst : FStarC_Syntax_Syntax.term) =
