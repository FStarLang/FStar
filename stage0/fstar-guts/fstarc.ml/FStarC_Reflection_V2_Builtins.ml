...
let pack_bqual (aqv : FStarC_Reflection_V2_Data.aqualv) :
...
let pack_aqual (aqv : FStarC_Reflection_V2_Data.aqualv) :
...
let pack_universe (uv : FStarC_Reflection_V2_Data.universe_view) :
...
let rec inspect_pat (p : FStarC_Syntax_Syntax.pat) :
...
let rec inspect_ln (t : FStarC_Syntax_Syntax.term) :
  ...
  | FStarC_Syntax_Syntax.Tm_constant c ->
      let uu___ = inspect_const c in FStarC_Reflection_V2_Data.Tv_Const uu___
...
let inspect_comp (c : FStarC_Syntax_Syntax.comp) :
  FStarC_Reflection_V2_Data.comp_view=
  let get_dec flags =
    let uu___ =
      FStarC_List.tryFind
        (fun uu___1 ->
...
let pack_comp (cv : FStarC_Reflection_V2_Data.comp_view) :
  ...
  | FStarC_Reflection_V2_Data.C_GTotal t -> FStarC_Syntax_Syntax.mk_GTotal t
  | FStarC_Reflection_V2_Data.C_Lemma (pre, post, pats) ->
      let ct =
        let uu___ =
...
let rec pack_pat (p : FStarC_Reflection_V2_Data.pattern) :
  ...
  | FStarC_Reflection_V2_Data.Pat_Constant c ->
      let uu___ =
        let uu___1 = pack_const c in FStarC_Syntax_Syntax.Pat_constant uu___1 in
      wrap uu___
...
let pack_ln (tv : FStarC_Reflection_V2_Data.term_view) :
  ...
  | FStarC_Reflection_V2_Data.Tv_Let (isrec, attrs, b, t1, t2) ->
      ...
      let lb =
        FStarC_Syntax_Util.mk_letbinding (FStar_Pervasives.Inl bv) []
          bv.FStarC_Syntax_Syntax.sort FStarC_Parser_Const.effect_Tot_lid t1
...
let compare_namedv (x : FStarC_Syntax_Syntax.bv)
  (y : FStarC_Syntax_Syntax.bv) : FStarC_Order.order=
  let n = FStarC_Syntax_Syntax.order_bv x y in
  if n < Prims.int_zero
  then FStarC_Order.Lt
  else if n = Prims.int_zero then FStarC_Order.Eq else FStarC_Order.Gt
let lookup_attr_ses (attr : FStarC_Syntax_Syntax.term)
...
let defs_in_module (env : FStarC_TypeChecker_Env.env)
  (modul : FStarC_Reflection_V2_Data.name) :
  FStarC_Syntax_Syntax.fv Prims.list=
  let uu___ = FStarC_TypeChecker_Env.lidents env in
  FStarC_List.concatMap
    (fun l ->
...
let rd_to_syntax_qual (uu___ : FStarC_Reflection_V2_Data.qualifier) :
...
let syntax_to_rd_qual (uu___ : FStarC_Syntax_Syntax.qualifier) :
  FStarC_Reflection_V2_Data.qualifier=
  match uu___ with
...
let inspect_ident (i : FStarC_Ident.ident) :
...
let inspect_sigelt (se : FStarC_Syntax_Syntax.sigelt) :
  ...
  | FStarC_Syntax_Syntax.Sig_inductive_typ
      ...
      let inspect_ctor c_lid =
        ...
        | uu___4 -> failwith "impossible: inspect_sigelt: did not find ctor" in
      let uu___3 =
...
let pack_sigelt (sv : FStarC_Reflection_V2_Data.sigelt_view) :
...
let inspect_lb (lb : FStarC_Syntax_Syntax.letbinding) :
  FStarC_Reflection_V2_Data.lb_view=
  let uu___ = lb in
  match uu___ with
  | { FStarC_Syntax_Syntax.lbname = nm; FStarC_Syntax_Syntax.lbunivs = us;
      FStarC_Syntax_Syntax.lbtyp = typ; FStarC_Syntax_Syntax.lbeff = uu___1;
...
let pack_lb (lbv : FStarC_Reflection_V2_Data.lb_view) :
  ...
  | { FStarC_Reflection_V2_Data.lb_fv = fv;
      ...
      FStarC_Syntax_Util.mk_letbinding (FStar_Pervasives.Inr fv) us typ
        ...
        FStarC_Range_Type.dummyRange
let inspect_namedv (v : FStarC_Syntax_Syntax.bv) :
...
let pack_namedv (vv : FStarC_Reflection_V2_Data.namedv_view) :
...
let pack_bv (bvv : FStarC_Reflection_V2_Data.bv_view) :
  ...
  then
    (let uu___1 =
       ...
       FStarC_Format.fmt2 "pack_bv: index is negative (%s), index = %s"
         (FStarC_Sealed.unseal bvv.FStarC_Reflection_V2_Data.ppname1) uu___2 in
     FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_CantInspect ()
       (Obj.magic FStarC_Errors_Msg.is_error_message_string)
       (Obj.magic uu___1))
  else ();
...
let pack_binder (bview : FStarC_Reflection_V2_Data.binder_view) :
  ...
  | (pqual, attrs) ->
      let uu___1 =
              ...
              FStarC_Range_Type.dummyRange) in
        {
          ...
          FStarC_Syntax_Syntax.sort = (bview.FStarC_Reflection_V2_Data.sort2)
        } in
...
let env_open_modules (e : FStarC_TypeChecker_Env.env) :
  FStarC_Reflection_V2_Data.name Prims.list=
...
let vars_of_env (e : FStarC_TypeChecker_Env.env) :
...
let rec term_eq (t1 : FStarC_Syntax_Syntax.term)
  ...
  match uu___ with
  | (FStarC_Reflection_V2_Data.Tv_Var bv1, FStarC_Reflection_V2_Data.Tv_Var
     bv2) -> bv_eq bv1 bv2
  | (FStarC_Reflection_V2_Data.Tv_BVar bv1, FStarC_Reflection_V2_Data.Tv_BVar
     bv2) -> bv_eq bv1 bv2
  | (FStarC_Reflection_V2_Data.Tv_FVar fv1, FStarC_Reflection_V2_Data.Tv_FVar
  ...
  | (FStarC_Reflection_V2_Data.Tv_Let (r1, ats1, b1, m1, n1),
     FStarC_Reflection_V2_Data.Tv_Let (r2, ats2, b2, m2, n2)) ->
      ((((r1 = r2) && ((eqlist ()) term_eq ats1 ats2)) && (binder_eq b1 b2))
         && (term_eq m1 m2))
        && (term_eq n1 n2)
  ...
  | uu___1 -> false
and arg_eq (arg1 : FStarC_Reflection_V2_Data.argv)
  (arg2 : FStarC_Reflection_V2_Data.argv) : Prims.bool=
  let uu___ = arg1 in
  match uu___ with
  | (a1, aq1) ->
      let uu___1 = arg2 in
...
and aqual_eq (aq1 : FStarC_Reflection_V2_Data.aqualv)
  ...
  | (FStarC_Reflection_V2_Data.Q_Meta t1, FStarC_Reflection_V2_Data.Q_Meta
     t2) -> term_eq t1 t2
  | uu___ -> false
...
and pattern_eq (p1 : FStarC_Reflection_V2_Data.pattern)
  (p2 : FStarC_Reflection_V2_Data.pattern) : Prims.bool=
  match (p1, p2) with
  ...
  | (FStarC_Reflection_V2_Data.Pat_Dot_Term topt1,
     FStarC_Reflection_V2_Data.Pat_Dot_Term topt2) ->
      (eqopt ()) term_eq topt1 topt2
  | uu___ -> false
and const_eq (c1 : FStarC_Reflection_V2_Data.vconst)
  (c2 : FStarC_Reflection_V2_Data.vconst) : Prims.bool= c1 = c2
...
let push_namedv (e : FStarC_TypeChecker_Env.env)
  (b : FStarC_Syntax_Syntax.bv) : FStarC_TypeChecker_Env.env=
  let uu___ = let uu___1 = FStarC_Syntax_Syntax.mk_binder b in [uu___1] in
  FStarC_TypeChecker_Env.push_binders e uu___
let subst_term (s : FStarC_Syntax_Syntax.subst_elt Prims.list)
  (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Subst.subst s t
