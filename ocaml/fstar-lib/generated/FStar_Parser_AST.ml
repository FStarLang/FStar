...
let (mk_binder :
  binder' -> FStar_Compiler_Range_Type.range -> level -> aqual -> binder) =
...
let (mkTuple : term Prims.list -> FStar_Compiler_Range_Type.range -> term) =
  fun args ->
    fun r ->
      ...
      let uu___1 = FStar_Compiler_List.map (fun x -> (x, Nothing)) args in
      ...
      then
        let uu___ =
          FStar_Compiler_Util.substring_from s
            (FStar_Compiler_String.length prefix) in
        FStar_Pervasives_Native.Some uu___
      else FStar_Pervasives_Native.None
let (compile_op :
  Prims.int ->
    Prims.string -> FStar_Compiler_Range_Type.range -> Prims.string)
  =
  fun arity ->
    fun s ->
      fun r ->
            ...
            (match uu___1 with
             | (prefix, s1) ->
                 let uu___2 =
                   let uu___3 =
                     let uu___4 =
                       let uu___5 = FStar_Compiler_String.list_of_string s1 in
                       FStar_Compiler_List.map name_of_char uu___5 in
                     FStar_Compiler_String.concat "_" uu___4 in
