...
let writer_return :
  'm .
    'm FStar_Class_Monoid.monoid -> unit -> Obj.t -> ('m, unit, Obj.t) writer
  = fun uu___ -> fun a -> fun x -> Wr ((FStar_Class_Monoid.mzero uu___), x)
let run_writer :
  'm .
    'm FStar_Class_Monoid.monoid ->
      unit -> ('m, unit, Obj.t) writer -> ('m * Obj.t)
          ...
          fun f ->
            ...
            | Wr (a1, x1) ->
                ...
                (match uu___2 with
                 | Wr (b1, y) ->
                     let uu___3 =
                       let uu___4 = FStar_Class_Monoid.mplus uu___ a1 b1 in
                       (uu___4, y) in
                     Wr uu___3)
let monad_writer :
  'm .
    'm FStar_Class_Monoid.monoid ->
      ('m, unit, unit) writer FStar_Class_Monad.monad
  =
  fun d ->
    {
...
let emit : 'm . 'm FStar_Class_Monoid.monoid -> 'm -> ('m, unit, unit) writer
