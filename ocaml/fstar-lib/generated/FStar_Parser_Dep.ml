  ...
  fun outc ->
    fun deps1 ->
      ...
      let no_fstar_stubs_file s =
        ...
        let uu___ =
          ((FStar_String.length s) >= l1) &&
            (let uu___1 = FStar_String.substring s Prims.int_zero l1 in
             uu___1 = s1) in
        if uu___
        then
          let uu___1 =
            FStar_String.substring s l1 ((FStar_String.length s) - l1) in
          FStar_String.op_Hat s2 uu___1
        else s in
