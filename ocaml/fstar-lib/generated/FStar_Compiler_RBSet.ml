...
let add : 'a . 'a FStar_Class_Ord.ord -> 'a -> 'a rbset -> 'a rbset =
  fun uu___ ->
    fun x ->
      fun s ->
        let rec add' s1 =
          ...
          | L -> N (R, L, x, L)
          | N (c, a1, y, b) ->
              let uu___1 = FStar_Class_Ord.op_Less_Question uu___ x y in
              if uu___1
              then let uu___2 = add' a1 in balance c uu___2 y b
              else
                (let uu___3 = FStar_Class_Ord.op_Greater_Question uu___ x y in
                 if uu___3
                 then let uu___4 = add' b in balance c a1 y uu___4
                 else s1) in
        let uu___1 = add' s in blackroot uu___1
let rec extract_min :
  'a . 'a FStar_Class_Ord.ord -> 'a rbset -> ('a rbset * 'a) =
  fun uu___ ->
    fun t1 ->
      match t1 with
      | N (uu___1, L, x, r) -> (r, x)
...
let rec remove : 'a . 'a FStar_Class_Ord.ord -> 'a -> 'a rbset -> 'a rbset =
  fun uu___ ->
    fun x ->
      fun t1 ->
        match t1 with
...
let rec mem : 'a . 'a FStar_Class_Ord.ord -> 'a -> 'a rbset -> Prims.bool =
  fun uu___ ->
    fun x ->
      fun s ->
        match s with
        | L -> false
        | N (uu___1, a1, y, b) ->
            let uu___2 = FStar_Class_Ord.op_Less_Question uu___ x y in
            if uu___2
            then mem uu___ x a1
            else
              (let uu___4 = FStar_Class_Ord.op_Greater_Question uu___ x y in
...
let equal : 'a . 'a FStar_Class_Ord.ord -> 'a rbset -> 'a rbset -> Prims.bool
  =
  fun uu___ ->
    fun s1 ->
      fun s2 ->
        ...
        FStar_Class_Deq.op_Equals_Question
          ...
          uu___2
let rec union :
  'a . 'a FStar_Class_Ord.ord -> 'a rbset -> 'a rbset -> 'a rbset =
  fun uu___ ->
    fun s1 ->
      fun s2 ->
        match s1 with
...
let inter : 'a . 'a FStar_Class_Ord.ord -> 'a rbset -> 'a rbset -> 'a rbset =
  fun uu___ ->
    fun s1 ->
      fun s2 ->
        let rec aux s11 acc =
    ...
    fun s1 ->
      fun s2 ->
        ...
        | N (uu___1, a1, x, b) ->
            ...
            diff uu___ uu___2 b
let rec subset :
  'a . 'a FStar_Class_Ord.ord -> 'a rbset -> 'a rbset -> Prims.bool =
  fun uu___ ->
    fun s1 ->
      fun s2 ->
        match s1 with
...
let from_list : 'a . 'a FStar_Class_Ord.ord -> 'a Prims.list -> 'a rbset =
  fun uu___ ->
    fun xs ->
      FStar_Compiler_List.fold_left (fun s -> fun e -> add uu___ e s) L xs
let addn :
  'a . 'a FStar_Class_Ord.ord -> 'a Prims.list -> 'a rbset -> 'a rbset =
  fun uu___ ->
    fun xs ->
      fun s ->
        FStar_Compiler_List.fold_left (fun s1 -> fun e -> add uu___ e s1) s
          xs
let collect :
  'a .
    'a FStar_Class_Ord.ord -> ('a -> 'a rbset) -> 'a Prims.list -> 'a rbset
  =
  fun uu___ ->
    fun f ->
      fun l ->
...
let setlike_rbset :
  'a . 'a FStar_Class_Ord.ord -> ('a, 'a t) FStar_Class_Setlike.setlike =
  fun uu___ ->
    {
      FStar_Class_Setlike.empty = empty;
      FStar_Class_Setlike.singleton = singleton;
