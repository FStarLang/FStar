...
module PO = FStar_TypeChecker_Primops
module POB = FStar_TypeChecker_Primops_Base
...
type itac =
    POB.psc -> FStar_Syntax_Embeddings_Base.norm_cb -> universes -> args -> term option
type nbe_itac =
...
let perr  s   = if FStar_Compiler_Debug.any () then BU.print_error s
let perr1 s x = if FStar_Compiler_Debug.any () then BU.print1_error s x
...
let register_plugin (s: string) (arity: Prims.int) (t: itac) (n:nbe_itac) =
    let step =
           { POB.name=FStar_Ident.lid_of_str s;
             POB.arity=arity;
