  ...
  match sopt {
    ...
    Some s -> {
      ...
      if U16.lt sid ctr {
        ...
        unfold session_state_perm;
        with t1. assert sid_pts_to trace_ref sid t ** sid_pts_to trace_ref sid t1;
        gather_sid_pts_to sid #t1 #t;
        assert (ghost_pcm_pts_to trace_ref (singleton sid 1.0R t1)) ** pure (t1 == t);
        let tbl, idx = HT.lookup tbl sid;
        with sst'. unfold pht_contains pht0 sid sst';
        match idx {
          Some idx -> {
