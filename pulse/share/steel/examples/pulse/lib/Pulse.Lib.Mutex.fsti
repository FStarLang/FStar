...
val belongs_to_mutex (#a:Type0) (#p:a -> vprop) (r:B.box a) (m:mutex p) : vprop

val lock (#a:Type0) (#p:a -> vprop) (m:mutex p)
  : stt (B.box a)
        (requires emp)
        (ensures fun r -> r `belongs_to_mutex` m ** (exists* v. B.pts_to r v ** p v))

val unlock (#a:Type0) (#p:a -> vprop) (m:mutex p) (r:B.box a)
  : stt unit
        (requires r `belongs_to_mutex` m ** (exists* v. B.pts_to r v ** p v))
        (ensures fun _ -> emp)
