...
cbor_map_iterator_next_serialized(CBOR_cbor_map_iterator_t i, CBOR_cbor_map_iterator_t *pi)
{
  ...
  CBOR_cbor_map_entry
  res1 = { .cbor_map_entry_key = res_key, .cbor_map_entry_value = res_value };
  ...
  else if (c.tag == CBOR_CBOR_Case_Map)
  {
    ...
    else
    {
      ...
      while (cond)
      {
        ...
        size_t sz11 = r1;
        size_t sz12 = CBOR_cbor_size_comp(x.cbor_map_entry_key, sz11, perr);
        bool err0 = *perr;
        size_t res;
        if (err0)
          res = sz12;
        ...
        {
          size_t sz2 = CBOR_cbor_size_comp(x.cbor_map_entry_value, sz12, perr);
          res = sz2;
        }
        size_t sz2 = res;
        bool err1 = *perr;
  ...
  else if (c.tag == CBOR_CBOR_Case_Map)
  {
    ...
    *out.len = xlen_;
    ...
    while (cond)
    {
      size_t n = r;
      CBOR_cbor_map_entry x = c_.cbor_map_payload[n];
      uint8_t *res1 = CBOR_cbor_l2r_write(x.cbor_map_entry_key, out);
      CBOR_cbor_l2r_write(x.cbor_map_entry_value, out);
      uint8_t *res10 = res1;
      KRML_MAYBE_UNUSED_VAR(res10);
      r = n + (size_t)1U;
      size_t n0 = r;
...
static CBOR_read_cbor_t read_cbor_(uint8_t *a, size_t sz)
{
  ...
  if (err == 0U)
  {
    ...
    v = ((CBOR_read_cbor_t){ .tag = CBOR_ParseError });
...
CBOR_read_cbor_t CBOR_read_deterministically_encoded_cbor(uint8_t *a, size_t sz)
{
  ...
  if (res.tag == CBOR_ParseError)
    return res;
  else if (res.tag == CBOR_ParseSuccess)
  {
    CBOR_read_cbor_success_t r = res._0;
    CBOR_cbor_serialized s = CBOR_destr_cbor_serialized(r.read_cbor_payload);
    bool r1 = true;
    uint8_t *r2 = s.cbor_serialized_payload;
    size_t r3 = (size_t)1U;
    bool r4 = (size_t)1U != (size_t)0U;
    bool cont = r4;
    bool cond;
    if (cont)
    {
    ...
    if (test)
      ...
      return ((CBOR_read_cbor_t){ .tag = CBOR_ParseError });
  }
  else
  {
    KRML_HOST_EPRINTF("KaRaMeL abort at %s:%d\n%s\n",
