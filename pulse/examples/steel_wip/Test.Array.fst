...
let read_write_with_sharing () : RST unit
  ...
  let x1 = rst_frame
    ...
    (fun _ -> A.index b 0ul)
  in
  let new_val = FStar.UInt32.(x1 +%^ 1ul) in
  let _ = rst_frame
    (A.array_resource b)
    (fun _ -> A.array_resource b)
    (fun _ -> A.upd b 0ul new_val)
  in
  let b1 = rst_frame
    (A.array_resource b)
...
(*
  let x2 = rst_frame
    (A.array_resource b_second <*> A.array_resource b_first <*> A.array_resource b1)
    (fun _ -> (A.array_resource b_second <*> A.array_resource b_first  <*> A.array_resource b1))
