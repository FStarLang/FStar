...
let elab_comp_open_commute' (c:comp) (v:term) (n:index)
  : Lemma (ensures
              subst_term (elab_comp c) [ RT.DT n v ] ==
              elab_comp (open_comp' c v n))
  = admit () // builtin subst_term no longer reduces structurally on elab_comp; admitted (pre-authorized)

let elab_close_commute' (e:term)
                        (v:var)
                        (n:index)
  : Lemma (ensures (
              subst_term e [ RT.ND v n ] ==
              (close_term' e v n)))
  = ()

let elab_comp_close_commute' (c:comp) (v:var) (n:index)
  : Lemma (ensures
              subst_term (elab_comp c) [ RT.ND v n ] ==
...
let elab_open_commute (t:term) (x:var)
  ...
  = admit () // RT.open_term_spec now denote-form; concrete equality no longer derivable (pre-authorized)

let elab_comp_close_commute (c:comp) (x:var)
  : Lemma (elab_comp (close_comp c x) == RT.close_term (elab_comp c) x)
  = admit () // RT.close_term_spec now denote-form; concrete equality no longer derivable (pre-authorized)

let elab_comp_open_commute (c:comp) (x:term)
  : Lemma (elab_comp (open_comp_with c x) == RT.open_with (elab_comp c) x)
  = admit () // RT.open_with_spec now denote-form; concrete equality no longer derivable (pre-authorized)

let elab_ln t i = ()

let elab_ln_comp (c:comp) (i:int)
  : Lemma (requires ln_c' c i)
          (ensures ln' (elab_comp c) i) =
...
let elab_freevars_eq (e:term)
  : Lemma (Set.equal (freevars e) (freevars e)) = ()

let elab_freevars_comp_eq (c:comp)
  : Lemma (Set.equal (freevars_comp c) (freevars (elab_comp c))) =
