...
let slprop_list_equiv_append (g:env) (vs1 vs2 vs3 vs4 : list slprop)
  (d1 : slprop_list_equiv g vs1 vs2)
  (d2 : slprop_list_equiv g vs3 vs4)
  : slprop_list_equiv g (vs1 @ vs3) (vs2 @ vs4)
  = admit()
let slprop_list_equiv_trans (g:env) (vs1 vs2 vs3 : list slprop)
  (d1 : slprop_list_equiv g vs1 vs2)
  (d2 : slprop_list_equiv g vs2 vs3)
  : slprop_list_equiv g vs1 vs3
  = admit()
let slprop_list_equiv_assoc_l (g:env) (vs1 vs2 vs3 : list slprop)
  : slprop_list_equiv g (vs1 @ (vs2 @ vs3)) ((vs1 @ vs2) @ vs3)
  = admit()
let slprop_list_equiv_assoc_r (g:env) (vs1 vs2 vs3 : list slprop)
  : slprop_list_equiv g ((vs1 @ vs2) @ vs3) (vs1 @ (vs2 @ vs3))
  = admit()
let slprop_list_equiv_elim_append (g:env) (vs1 vs2 : list slprop)
...
(* This is not trivial since list_as_slprop distinguishes the singleton case...
maybe it should not? *)
let list_as_slprop_cons_equiv 
  (g : env)
  (p : slprop)
  (ps : list slprop)
  : slprop_equiv g (list_as_slprop (p::ps)) (p * list_as_slprop ps)
= match ps with
  | [] ->
    let pf : slprop_equiv g p (p * tm_emp) =
      VE_Sym _ _ _ (VE_Unit _ _) >>> VE_Comm _ _ _
    in
    pf
  | _  -> VE_Refl _ _

let slprop_list_equiv_cons g p q ps qs d1 d2 =
  list_as_slprop_cons_equiv g p ps >>>
  VE_Ctxt _ _ _ _ _ d1 d2 >>>
  VE_Sym _ _ _ (list_as_slprop_cons_equiv g q qs)
...
let slprop_list_equiv_flip g p q ps =
  VE_Ctxt _ _ _ _ _ (VE_Refl _ _) (list_as_slprop_cons_equiv g q ps) >>>
  star_flip _ _ _ _ >>>
  VE_Ctxt _ _ _ _ _ (VE_Refl _ _) (VE_Sym _ _ _ <| list_as_slprop_cons_equiv g p ps)

let slprop_list_equiv_push_append g p ps qs = admit()

let slprop_list_equiv_append_r g ps qs rs pf = slprop_list_equiv_append _ _ _ _ _ (VE_Refl _ _) pf
...
let subst_list_as_slprop = admit()

(* Is this somewhere else? *)
let subst_star ss p q = admit()
(* Compose two match pass results. *)
let compose_mpr (g:env) (ss : PS.ss_t) (ctxt0 unsolved0 ctxt1 unsolved1 : list slprop)
...
= { ss' = mpr2.ss';
    ...
    ctxt_ok      = 
      ...
      mpr1.ctxt_ok >>*
      (slprop_list_equiv_append _ _ _ _ _ (slprop_list_equiv_refl _ _) mpr2.ctxt_ok) >>*
      (slprop_list_equiv_assoc_l _ _ _ _)
    ;
    ...
    unsolved_ok = (
      ...
      (* proof: *)
      mpr1.unsolved_ok >>*
      (slprop_list_equiv_append _ _ _ _ _ (slprop_list_equiv_refl _ _) mpr2.unsolved_ok) >>*
      (slprop_list_equiv_assoc_l _ _ _ _)
    );
    ...
    match_ok = (
      ...
      slprop_list_equiv_append _ _ _ _ _ mpr1.match_ok mpr2.match_ok >>*
      // VE_Sym _ _ _ (subst_append_equiv _ _ _ _) >>*
      slprop_list_equiv_append _ _ _ _ _
        (ve_refl_pf #g
           (list_as_slprop <| (mpr1.ss' $$ mpr1.unsolved_matched))
           (list_as_slprop <| (mpr2.ss' $$ mpr1.unsolved_matched))
...
let apply_mpr_vequiv_proof
  ...
  : slprop_equiv (push_env pst.pg pst.uvs)
                ((list_as_slprop pst.remaining_ctxt * preamble.frame) * (mpr.ss'.(pst.solved)))
                ((list_as_slprop mpr.ctxt1 * preamble.frame) * (mpr.ss'.(list_as_slprop mpr.unsolved_matched * pst.solved)))
=
  let pf0 : slprop_equiv (push_env pst.pg pst.uvs)
             (list_as_slprop pst.remaining_ctxt)
             (list_as_slprop mpr.ctxt_matched * list_as_slprop mpr.ctxt1)
  = mpr.ctxt_ok >>>
    slprop_list_equiv_elim_append _ _ _
  in
  let pf1 : slprop_equiv (push_env pst.pg pst.uvs)
             (list_as_slprop mpr.ctxt_matched * list_as_slprop mpr.ctxt1)
             (list_as_slprop mpr.ctxt1 * (mpr.ss'.(list_as_slprop mpr.unsolved_matched)))
  = subst_list_as_slprop mpr.ss' mpr.unsolved_matched;
    VE_Comm _ _ _ >>>
    VE_Ctxt _ _ _ _ _ (VE_Refl _ _) mpr.match_ok
  in
  let pf2 : slprop_equiv (push_env pst.pg pst.uvs)
             ((list_as_slprop pst.remaining_ctxt * preamble.frame) * (mpr.ss'.(pst.solved)))
             ((list_as_slprop mpr.ctxt1 * (mpr.ss'.(list_as_slprop mpr.unsolved_matched) * preamble.frame)) * (mpr.ss'.(pst.solved)))
  = cong_l (cong_l (pf0 >>> pf1)) >>>
    cong_l (VE_Sym _ _ _ (VE_Assoc _ _ _ _))
  in
  let eq : squash ((mpr.ss'.(list_as_slprop mpr.unsolved_matched) * mpr.ss'.(pst.solved))
                    ==
                   mpr.ss'.(list_as_slprop mpr.unsolved_matched * pst.solved))
  = subst_star mpr.ss' (list_as_slprop mpr.unsolved_matched) pst.solved;
    ()
  in
  let pf3 : slprop_equiv (push_env pst.pg pst.uvs)
             ((list_as_slprop mpr.ctxt1 * (mpr.ss'.(list_as_slprop mpr.unsolved_matched) * preamble.frame)) * (mpr.ss'.(pst.solved)))
             ((list_as_slprop mpr.ctxt1 * preamble.frame) * (mpr.ss'.(list_as_slprop mpr.unsolved_matched * pst.solved)))
  = 
    ...
    cong_r (
      ...
      VE_Sym _ _ _ (VE_Assoc _ _ _ _) >>>
      cong_r (ve_refl_pf ((mpr.ss'.(list_as_slprop mpr.unsolved_matched)) * mpr.ss'.(pst.solved))
                         (mpr.ss'.(list_as_slprop mpr.unsolved_matched * pst.solved))
                         eq)
    ) >>>
    VE_Assoc _ _ _ _
...
= { pst with
      ...
      solved   = list_as_slprop mpr.unsolved_matched * pst.solved;
      ...
      goals_inv =
          pst.goals_inv >>>
          (VE_Ctxt _ _ _ _ _
            (VE_Trans _ _ _ _ mpr.unsolved_ok (slprop_list_equiv_elim_append' _ _ _))
            (VE_Refl _ _)) >>>
          (VE_Sym _ _ _ <| VE_Assoc _ _ _ _)
      ;
      ...
      k = (
        ...
        let k1 : continuation_elaborator
                   pst.pg ((list_as_slprop pst.remaining_ctxt * preamble.frame) * pst.ss.(pst.solved))
                   pst.pg ((list_as_slprop pst.remaining_ctxt * preamble.frame) * mpr.ss'.(pst.solved))
        = (admit(); k_elab_unit _ _)
        in
        let k2 : continuation_elaborator
                   pst.pg ((list_as_slprop pst.remaining_ctxt * preamble.frame) * mpr.ss'.(pst.solved))
                   pst.pg ((list_as_slprop mpr.ctxt1          * preamble.frame) * mpr.ss'.(list_as_slprop mpr.unsolved_matched * pst.solved))
        = k_elab_equiv (k_elab_unit pst.pg ((list_as_slprop pst.remaining_ctxt * preamble.frame) * mpr.ss'.(pst.solved)))
            (VE_Refl _ _)
