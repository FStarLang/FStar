...
let effect_hint = option ctag
let default_effect_hint : effect_hint = None
let as_effect_hint (c:ctag) : effect_hint = Some c
...
and st_term = {
    ...
    source : bool; (* was this term in the user-written source code? false for anything generated by the checker. *)
    seq_lhs : bool; (* was this s1; ..? if so check that it types to unit *)
} 

and branch = {
  pat : pattern;
  e : st_term;
  norw: bool; (* do not automatically rewrite strutinee to the pattern. *)
}
