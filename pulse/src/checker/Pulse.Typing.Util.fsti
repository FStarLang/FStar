...
val check_equiv_now (g:env) (t1 t2 : term)
  : Tac (option (squash (equiv_token g t1 t2)) & issues)
...
(* Like check_equiv_now, but disallows unfolding.
   Produces structural arg-level guards without looking
   through type abbreviations. Useful for slprop comparisons
   where the abstraction level should be preserved. *)
val check_equiv_now_nounfold (g:env) (t1 t2 : term)
  : Tac (option (squash (equiv_token g t1 t2)) & issues)
