...
let rec (freevars :
  ...
  fun t ->
    ...
    | Pulse_Syntax_Base.Tm_PureApp (t1, uu___, t2) ->
...
let (open_term_nv :
  ...
  fun t ->
    fun nv ->
      open_term' t (Pulse_Elaborate_Pure.term_of_nvar nv) Prims.int_zero
let (open_st_term_nv :
  Pulse_Syntax_Base.st_term ->
  ...
  fun t ->
    fun nv ->
      open_st_term' t (Pulse_Elaborate_Pure.term_of_nvar nv) Prims.int_zero
let (open_comp_with :
