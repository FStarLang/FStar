...
let (elab_observability :
  Pulse_Syntax_Base.observability -> FStarC_Reflection_Types.term) =
  fun uu___ ->
    match uu___ with
...
let rec (elab_pat :
  Pulse_Syntax_Base.pattern -> FStarC_Reflection_V2_Data.pattern) =
  fun p ->
    let elab_fv f =
      FStarC_Reflection_V2_Builtins.pack_fv f.Pulse_Syntax_Base.fv_name in
    match p with
...
let (elab_comp : Pulse_Syntax_Base.comp -> FStarC_Reflection_Types.term) =
  fun c ->
    match c with
         ...
         | (u, res, pre, post) ->
             Pulse_Reflection_Util.mk_stt_comp u res pre
               (Pulse_Reflection_Util.mk_abs res
                  FStarC_Reflection_V2_Data.Q_Explicit post))
