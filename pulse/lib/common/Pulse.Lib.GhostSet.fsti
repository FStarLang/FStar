...
val mem_as_set' #t (x: t) (l: list t)
  : Lemma (mem x (as_set' l) <==> List.memP x l)
          [SMTPat (mem x (as_set' l))]

let as_set #t = as_set' #t
...
val is_finite_singleton #t (x:t) : Lemma (is_finite (singleton x)) [SMTPat (is_finite (singleton x))]
