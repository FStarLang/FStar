...
let unpack_pred #f (#ff: functor u#a f) n (x: pred' ff n (k' ff)) : pred (knot_t ff) ff.other ff.tt =
  F.on_dom (knot_t ff & ff.other) fun ((|m, h|), o) -> if m < n then x m (h, o) else ff.t_bot

let pack_pred #f (#ff: functor u#a f) n (x: pred (knot_t ff) ff.other ff.tt) : pred' ff n (k' ff) =
  fun m (h, o) -> x ((| m, h |), o)
...
let pack #f (#ff: functor u#a f) (x: nat & f (pred (knot_t ff) ff.other ff.tt)) : knot_t ff =
  (| fst x, k'_fold (ff.fmap (pack_pred (fst x)) (snd x)) |)

let unpack #f (#ff: functor u#a f) (x: knot_t ff) : nat & f (pred (knot_t ff) ff.other ff.tt) =
  (dfst x, ff.fmap (unpack_pred (dfst x)) (k'_unfold (dsnd x)))

let unpack_pack_pred #f (#ff: functor u#a f) (n:nat) (x: pred (knot_t ff) ff.other ff.tt) :
...
let unpack_pack #f (#ff: functor u#a f) (n: nat) (h: f (pred (knot_t ff) ff.other ff.tt)) :
