...
{
  let arr = to_array_mask r;
  from_mask arr;
  with v'. assert Pulse.Lib.Array.PtsTo.pts_to arr #p v';
  assert pure (v' `Seq.equal` seq![reveal v]);
  arr
}
...
{
  pts_to_len (to_array_ghost r);
  to_mask (to_array_ghost r);
  return_to_array_mask r;
}
