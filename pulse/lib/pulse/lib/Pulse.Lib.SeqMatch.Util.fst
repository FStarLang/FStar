...
requires
  ...
  returns res: (Cons? v /\ Seq.length c > 0)
ensures
...
requires
  ...
  returns res: (i < Seq.length s1 /\ List.Tot.length s2 == Seq.length s1)
ensures
    (
    ...
    )
{
  ...
  intro (Trade.trade (p (Seq.index s1 i) (List.Tot.index s2 i))
        (seq_list_match s1 s2 p))
      #(p (Seq.index s1 i) (List.Tot.index s2 i) @==>
        (seq_list_match s1 s2 p))
      fn _
...
requires
  ...
  returns res: (i <= j /\ j < k /\ k <= Seq.length s1 /\ k <= Seq.length s2)
ensures
