...
let k_intro_pure (g:env) (t:term) (frame:vprop)
  : T.Tac (continuation_elaborator g frame g (frame * tm_pure t)) =
...
let intro_pure (#preamble:_) (pst:prover_state preamble)
  ...
  let k : continuation_elaborator
    ...
    fun post_hint d ->
    let d
      : st_typing_in_ctxt pst.pg
          ((list_as_vprop pst.remaining_ctxt * preamble.frame) * pst.ss.(tm_pure t * pst.solved))
          post_hint = d in
    let (| t, c, d |) = d in
    magic () in
