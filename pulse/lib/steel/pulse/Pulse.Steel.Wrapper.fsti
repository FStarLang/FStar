...
: stt unit
    (requires A.pts_to a full_perm s)
    (ensures fun res -> A.pts_to a full_perm (Seq.upd s i v))
...
: stt t
    (requires A.pts_to a p s)
    (ensures fun res -> A.pts_to a p s `star` pure (res == Seq.index s i))
