...
val bindings_as_map (g:env) : Lemma (bindings g `is_related_to` as_map g)

let dom (g:env) : Set.set var = Map.domain (as_map g)

val mk_env (f:RT.fstar_top_env) : g:env { fstar_env g == f }

val mk_env_bs (f:RT.fstar_top_env) : Lemma (bindings (mk_env f) == [])
...
val env_extends_refl (g:env) : Lemma (g `env_extends` g)
