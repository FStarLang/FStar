...
(** The type of nullable pointers to values of type b inside "base objects" of type a. *)
val ptr (a: Type u#0) (b: Type u#b) : Type u#b

(** [pts_to p pb v] if p is non-null, has PCM pb, and points to v *)
val pts_to (p: ptr 'a 'b) (pb: pcm 'b) ([@@@smt_fallback] v: 'b): vprop

(** [pts_to_or_null p pb v] if p is null or points to v *)
val pts_to_or_null (p: ptr 'a 'b) (pb: pcm 'b) ([@@@smt_fallback] v: option 'b): vprop

(** The null pointer. *)
val nullptr (#a:Type) (#b:Type) : ptr a b

(** A non-null pointer that corresponds to underlying reference r. *)
...
(** If [pts_to p pb v] then p is non-null. *)
...
(** Introduction and elimination rules for pts_to *)
...
(** Introduction and elimination rules for pts_to_or_null *)
...
(** Write for pointers using the all-or-none PCM *)
val ptr_opt_write
