...
need to perform the following steps before your first use:
...
2. Verify the F\* standard library, producing `.checked` files that cache
   ...
   This step is needed mostly for size reasons (if those `.checked`
   files were part of the binary package, then its size would be more
   than 200 MB.)

3. If you want to compile OCaml code generated by F*, you need to
   ...
   To compile OCaml code generated by F*, using a F* binary package is
   not the preferred method. Indeed, you will need to use the same
   version of OCaml as the one that was used to build F* itself:
   currently F* binary packages for 0.9.8.x are built with OCaml 4.12,
   and binary packages for previous versions were built with OCaml
   4.09, or 4.05 for even older binary packages.

4. If you want to compile OCaml code generated by F* with a binary
