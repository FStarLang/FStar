...
private val sorted: #a:Type -> f:cmp a -> list a -> Tot bool
...
let size (#a:Type) #f s = FStar.List.Tot.length s
...
abstract val eq_lemma: #a:Type -> #f:cmp a -> s1:ordset a f -> s2:ordset a f
              -> Lemma (requires (Equal #a #f s1 s2))
                       (ensures (s1 = s2))
                 [SMTPatT (Equal #a #f s1 s2)]
...
abstract val mem_union: #a:Type -> #f:cmp a -> x:a -> s1:ordset a f -> s2:ordset a f
               -> Lemma (requires True)
...
abstract val mem_intersect: #a:Type -> #f:cmp a -> x:a -> s1:ordset a f -> s2:ordset a f
                   -> Lemma (requires True)
                            (ensures (mem #a #f x (intersect #a #f s1 s2) =
                                      (mem #a #f x s1 && mem #a #f x s2)))
...
abstract val mem_subset: #a:Type -> #f:cmp a -> s1:ordset a f -> s2:ordset a f
                -> Lemma (requires True)
...
abstract val mem_remove: #a:Type -> #f:cmp a -> x:a -> y:a -> s:ordset a f
                -> Lemma (requires True)
...
abstract val eq_remove: #a:Type -> #f:cmp a -> x:a -> s:ordset a f
...
abstract val size_empty: #a:Type -> #f:cmp a -> s:ordset a f
                -> Lemma (requires True) (ensures ((size #a #f s = 0) = (s = empty #a #f)))
                  [SMTPat (size #a #f s)]
let size_empty (#a:Type) #f s = ()

abstract val size_remove: #a:Type -> #f:cmp a -> y:a -> s:ordset a f
