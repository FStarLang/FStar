(*--build-config
    ...
    other-files:classical.fst ext.fst FStar.Set.fsi seq.fsi
  --*)
...
let lemma_swap_permutes #a s i j = Classical.forall_intro (lemma_swap_permutes_aux s i j)
...
let swap_frame_lo #a s lo i j = cut (Eq (slice s lo i) (slice (swap s i j) lo i))

val swap_frame_lo' : #a:Type -> s:seq a -> lo:nat -> i':nat {lo <= i'} -> i:nat{i' <= i} -> j:nat{i <= j && j < length s}
     -> Lemma (ensures (slice s lo i' == slice (swap s i j) lo i'))
let swap_frame_lo' #a s lo i' i j = cut (Eq (slice s lo i') (slice (swap s i j) lo i'))
...
let swap_frame_hi #a s i j k hi = cut (Eq (slice s k hi) (slice (swap s i j) k hi))
...
let splice_refl #a s i j = cut (Eq s (splice s i s j))
...
let lemma_swap_splice #a s start i j len = cut (Eq (swap s i j) (splice s start (swap s i j) len))
...
let lemma_weaken_frame_right #a s1 s2 i j k = cut (Eq s1 (splice s2 i s1 k))
...
val lemma_weaken_frame_left : #a:Type -> s1:seq a -> s2:seq a{length s1 = length s2} -> i:nat -> j:nat -> k:nat{i <= j && j <= k && k <= length s1}
  ...
  (ensures (s1 == splice s2 i s1 k))
let lemma_weaken_frame_left #a s1 s2 i j k = cut (Eq s1 (splice s2 i s1 k))
...
let lemma_trans_frame #a s1 s2 s3 i j = cut (Eq s1 (splice s3 i s1 j))
