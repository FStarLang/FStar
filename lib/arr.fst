...
(* opaque type Equal : #a:Type -> seq a -> seq a -> Type = fun (a:Type) (s1:seq a) (s2:seq a) ->  *)
...
assume Equals:      forall (a:Type) (s1:seq a) (s2:seq a).             {:pattern (Equal s1 s2)}             Equal s1 s2 <==> 
                            (forall (i:nat{i < s1.length && i < s2.length}).{:pattern (index s1 i); (index s2 i)} 
                                                                              (s1.length == s2.length
                                                                               /\ index s1 i == index s2 i))

assume Extensional: forall (a:Type) (s1:seq a) (s2:seq a).             {:pattern (Equal s1 s2)}             Equal s1 s2 <==> s1==s2
