...
| `make bump-stage0` | Replace `stage0/` with `stage0_new/` (prefer `./.scripts/bump-stage0-from-stage1.sh`) |
...
2. After the change is stable, update the snapshot with
   `./.scripts/bump-stage0-from-stage1.sh` (see "Stage0 bump policy"
   below). This also resets `mk/fstar-01.mk` = `mk/fstar-12.mk` and
   `mk/generic-0.mk` = `mk/generic-1.mk`.

### Stage0 bump policy

`stage0/` is a generated snapshot; these rules are mandatory (they are
also recorded in `CONTRIBUTING.md`):

- **`stage0/` may only be changed by `./.scripts/bump-stage0-from-stage1.sh`.**
  Never edit `stage0/` by hand and never stage a hand-made bump.
- **A stage0 bump commit must contain nothing else.** The script commits
  only `stage0/` and `mk/generic-0.mk`; put every other change in a
  separate commit.
- **A branch containing a bump commit must be rebased onto `master`, not
  merged.**
- **When rebasing, regenerate the bump**: drop the old bump commit and
  re-run `./.scripts/bump-stage0-from-stage1.sh` on the rebased branch.
...
8. If the change affects bootstrap: update the snapshot with
   `./.scripts/bump-stage0-from-stage1.sh`, in a commit of its own.
...
- **Do not edit files in `stage0/` directly** — it is a generated
  snapshot. Update it only via `./.scripts/bump-stage0-from-stage1.sh`,
  in a commit that contains nothing else (see "Stage0 bump policy").
