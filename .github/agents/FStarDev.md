...
│   ├── prettyprint/        # Term pretty-printing
...
│   ├── class/              # Typeclass resolution
│   ├── data/               # Persistent data structures
│   └── tools/              # Misc tooling
├── ulib/                   # F* standard library (verified .fst/.fsti files)
├── stage0/                 # OCaml snapshot: last-known-good bootstrap compiler
...
### Unit Tests
```bash
make 1.tests              # stage 1 OCaml unit tests
make 2.tests              # stage 2 OCaml unit tests
...
you need to work with `mk/fstar-01.mk` / `mk/generic-0.mk` to
provide stage-0-compatible extraction rules, then do a `bump-stage0`
once the change is through.
...
| `ADMIT` | Set to `1` to admit all SMT queries (fast but unsound) |
...
8. If the change affects bootstrap: update snapshot with `make save`
   then `make bump-stage0`.
...
- **Do not edit files in `stage0/` directly** — it is a generated
  snapshot. Use `make save && make bump-stage0` to update it.
