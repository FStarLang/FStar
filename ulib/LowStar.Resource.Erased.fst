...
*)
module LowStar.Resource.Erased
...
let resource = erased resource_t

let as_resource (#a:Type) (view:view a) : resource = hide ({
    t = a;
    view = view
  })

let type_of (res:resource) : Type0 = 
  (reveal res).t
...
let inv (res:resource) (h:HS.mem) : Type0 =
  (reveal res).view.inv h
...
let ( <*> ) (res1 res2:resource) : res:resource = 
  ...
  let sel h = (sel (view_of res1) h,sel (view_of res2) h) in
  let t = (type_of res1) & (type_of res2) in 
  let view = {
      fp = fp;
      inv = inv;
      sel = sel
...
let reveal_star () 
  : Lemma ((forall res1 res2 .{:pattern as_loc (fp (res1 <*> res2))} 
           ...
           (forall res1 res2 .{:pattern type_of (res1 <*> res2)} 
              type_of (res1 <*> res2) == type_of res1 & type_of res2) /\
           (forall res1 res2 h .{:pattern sel (view_of (res1 <*> res2)) h} 
              sel (view_of (res1 <*> res2)) h == (sel (view_of res1) h,sel (view_of res2) h))) = 
  ()
...
let reveal_empty_resource ()
  : Lemma ((fp empty_resource == Ghost.hide B.loc_none) /\
           ...
           (type_of empty_resource == unit) /\
           (forall h .{:pattern sel (view_of empty_resource) h} sel (view_of empty_resource) h == ())) =
  ()
