...
abstract type seq (a:Type) = list a

(* Destructors *)
abstract val length: #a:Type -> seq a -> Tot nat
let length #a s = List.length s

abstract val index:  #a:Type -> s:seq a -> i:nat{i < length s} -> Tot a
let index #a s i = List.index s i

abstract val create: #a:Type -> nat -> a -> Tot (seq a)
let rec create #a len v = if len = 0 then [] else v::create (len - 1) v

abstract val init: #a:Type -> len:nat -> contents: (i:nat { i < len } -> Tot a) -> Tot (seq a)
let rec init #a len contents = if len = 0 then [] else (contents 0)::(init (len - 1) contents)
...
let rec upd #a s n v = if n = 0 then v::(Cons.tl s) else (Cons.hd s)::(upd (Cons.tl s) (n - 1) v)

abstract val append: #a:Type -> seq a -> seq a -> Tot (seq a)
let append #a s1 s2 = List.append s1 s2
...
let rec slice #a s i j =
  if i > 0 then slice #a (Cons.tl s) (i - 1) (j - 1)
  else
    if j = 0 then []
    else (Cons.hd s)::(slice #a (Cons.tl s) i (j - 1))
...
let rec lemma_len_upd #a n v s = if n = 0 then () else lemma_len_upd #a (n - 1) v (Cons.tl s)
...
let lemma_len_append #a s1 s2 = append_length s1 s2
...
let rec lemma_len_slice #a s i j  =
  if i > 0 then lemma_len_slice #a (Cons.tl s) (i - 1) (j - 1)
  else if j = 0 then ()
  else lemma_len_slice #a (Cons.tl s) i (j - 1)
...
let rec lemma_index_upd1 #a s n v = if n = 0 then () else lemma_index_upd1 #a (Cons.tl s) (n - 1) v
...
let rec lemma_index_upd2 #a s n v i = match s with
  | []     -> ()
  | hd::tl  ->
    if i = 0 then ()
    else
      if n = 0 then ()
      else lemma_index_upd2 #a tl (n - 1) v (i - 1)
...
let rec lemma_index_app1 #a s1 s2 i  = match s1 with
  | []    -> ()
  | hd::tl ->
    if i = 0 then () else lemma_index_app1 #a tl s2 (i - 1)
...
abstract val lemma_index_app2: #a:Type -> s1:seq a -> s2:seq a -> i:nat{i < length s1 + length s2 /\ length s1 <= i} -> Lemma
  ...
  [SMTPat (index (append s1 s2) i)]
let rec lemma_index_app2 #a s1 s2 i  = match s1 with
  | []    -> ()
  | hd::tl -> lemma_index_app2 #a tl s2 (i - 1)
...
let rec lemma_index_slice #a s i j k =
  if i > 0 then lemma_index_slice #a (Cons.tl s) (i - 1) (j - 1) k
  else
    if j = 0 then ()
    else if k = 0 then () else lemma_index_slice #a (Cons.tl s) i (j - 1) (k - 1)
