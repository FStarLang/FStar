...
val mem_empty: #a: eqtype -> x: a -> Lemma (requires (mem x [])) (ensures False)
let mem_empty #a x = ()
...
val rev_acc_length: l: list 'a -> acc: list 'a ->
  Lemma (requires True) (ensures (length (rev_acc l acc) = length l + length acc))
...
val rev_acc_mem: #a: eqtype -> l: list a -> acc: list a -> x: a ->
  Lemma (requires True) (ensures (mem x (rev_acc l acc) <==> (mem x l \/ mem x acc)))
let rec rev_acc_mem #a l acc x =
  match l with
  | [] -> ()
...
val rev_mem: #a: eqtype -> l: list a -> x: a ->
  Lemma (requires True) (ensures (mem x (rev l) <==> mem x l))
let rev_mem #a l x = rev_acc_mem l [] x


(** Properties about append **)

val append_nil_l: l: list 'a -> Lemma (requires True) (ensures ([] @ l == l))
let append_nil_l l = ()
...
val append_cons_l: hd: 'a -> tl: list 'a -> l: list 'a ->
  Lemma (requires True) (ensures (((hd :: tl) @ l) == (hd :: (tl @ l))))
let append_cons_l hd tl l = ()

val append_l_cons: hd: 'a -> tl: list 'a -> l: list 'a ->
  Lemma (requires True) (ensures ((l @ (hd :: tl)) == ((l @ [hd]) @ tl)))
...
val append_assoc: l1: list 'a -> l2: list 'a -> l3: list 'a ->
  Lemma (requires True) (ensures ((l1 @ (l2 @ l3)) == ((l1 @ l2) @ l3)))
...
val append_mem: #t: eqtype -> l1: list t -> l2: list t -> a: t ->
  Lemma (requires True) (ensures (mem a (l1 @ l2) = (mem a l1 || mem a l2)))
(* [SMTPat (mem a (l1@l2))] *)
...
val append_mem_forall: #a: eqtype -> l1: list a -> l2: list a ->
  Lemma (requires True) (ensures (forall a. mem a (l1 @ l2) = (mem a l1 || mem a l2)))
let rec append_mem_forall #a l1 l2 =
  match l1 with
  | [] -> ()
  | hd :: tl -> append_mem_forall tl l2

val append_count: #t: eqtype -> l1: list t -> l2: list t -> a: t ->
  Lemma (requires True) (ensures (count a (l1 @ l2) = (count a l1 + count a l2)))
let rec append_count #t l1 l2 a =
  match l1 with
  | [] -> ()
  | hd :: tl -> append_count tl l2 a

val append_count_forall: #a: eqtype -> l1: list a -> l2: list a ->
  Lemma (requires True) (ensures (forall a. count a (l1 @ l2) = (count a l1 + count a l2)))
(* [SMTPat (l1@l2)] *)
...
val append_inv_tail: l: list 'a -> l1: list 'a -> l2: list 'a ->
  Lemma (requires ((l1 @ l) == (l2 @ l))) (ensures (l1 == l2))
...
let rec append_length_inv_head
  (#a: Type) (left1: list a) (right1: list a) (left2: list a) (right2: list a)
...
let append_length_inv_tail
  (#a: Type) (left1: list a) (right1: list a) (left2: list a) (right2: list a)
  : Lemma (requires (append left1 right1 == append left2 right2 /\ length right1 == length right2))
    (ensures (left1 == left2 /\ right1 == right2)) =
...
val rev_acc_rev': l: list 'a -> acc: list 'a ->
  Lemma (requires (True)) (ensures ((rev_acc l acc) == ((rev' l) @ acc)))
...
val rev'_append: l1: list 'a -> l2: list 'a ->
  Lemma (requires True) (ensures ((rev' (l1 @ l2)) == ((rev' l2) @ (rev' l1))))
...
val rev_ind: p: (list 'a -> Tot bool) -> l: list 'a ->
  Lemma (requires ((p []) /\ (forall hd tl. p hd ==> p (hd @ [tl])))) (ensures (p l))
let rev_ind p l = rev'_involutive l; rev'_list_ind p (rev' l)

(** Properties about iterators **)

val map_lemma: f: ('a -> Tot 'b) -> l: (list 'a) ->
  Lemma (requires True) (ensures (length (map f l)) = length l) [SMTPat (map f l)]
...
val partition_mem: #a: eqtype -> f: (a -> Tot bool) -> l: list a -> x: a ->
  Lemma (requires True)
    (ensures
...
val partition_mem_forall: #a: eqtype -> f: (a -> Tot bool) -> l: list a ->
  Lemma (requires True)
    (ensures
      (let l1, l2 = partition f l in
...
val partition_mem_p_forall: #a: eqtype -> p: (a -> Tot bool) -> l: list a ->
  Lemma (requires True)
    (ensures
...
[l1] and [l2]. *)
val partition_count: #a: eqtype -> f: (a -> Tot bool) -> l: list a -> x: a ->
  Lemma (requires True)
...
val partition_count_forall: #a: eqtype -> f: (a -> Tot bool) -> l: list a ->
  Lemma (requires True)
...
val append_sorted: 
  ...
  Lemma
    (requires
      (total_order #a f /\ (forall y. mem y l1 ==> not (f pivot y)) /\
...
val sortWith_sorted: #a: eqtype -> f: (a -> a -> Tot int) -> l: list a ->
  Lemma (requires (total_order #a (bool_of_compare f)))
    (ensures
      ((sorted (bool_of_compare f) (sortWith f l)) /\ (forall x. mem x l = mem x (sortWith f l))))
    (decreases (length l))
...
decidable equality *)
let rec mem_memP (#a: eqtype) (x: a) (l: list a) : Lemma (ensures (mem x l <==> memP x l)) =
  match l with
  | [] -> ()
  | a :: q -> mem_memP x q

(** The empty list has no elements. *)
val memP_empty: #a: Type -> x: a -> Lemma (requires (memP x [])) (ensures False)
let memP_empty #a x = ()
...
let rec memP_map_intro (#a: Type) (#b: Type) (f: (a -> Tot b)) (x: a) (l: list a)
  : Lemma (requires True) (ensures (memP x l ==> memP (f x) (map f l))) (decreases l) =
  match l with
  | [] -> ()
  | _ :: q ->
    (* NOTE: would fail if [requires memP x l] instead of [ ==> ] *)
    memP_map_intro f x q

let rec memP_map_elim (#a: Type) (#b: Type) (f: (a -> Tot b)) (y: b) (l: list a)
  : Lemma (requires True)
...
let noRepeats_nil (#a: eqtype) : Lemma (ensures (noRepeats #a [])) = ()

let noRepeats_cons (#a: eqtype) (h: a) (tl: list a)
  : Lemma (requires ((~(mem h tl)) /\ noRepeats tl)) (ensures (noRepeats #a (h :: tl))) = ()

let rec noRepeats_append_elim (#a: eqtype) (l1: list a) (l2: list a)
  : Lemma (requires (noRepeats (l1 @ l2)))
    (ensures (noRepeats l1 /\ noRepeats l2 /\ (forall x. mem x l1 ==> ~(mem x l2))))
    (decreases l1) =
  match l1 with
  | [] -> ()
  | x :: q1 -> append_mem q1 l2 x; noRepeats_append_elim q1 l2

let rec noRepeats_append_intro (#a: eqtype) (l1: list a) (l2: list a)
  : Lemma (requires (noRepeats l1 /\ noRepeats l2 /\ (forall x. mem x l1 ==> ~(mem x l2))))
    (ensures (noRepeats (l1 @ l2)))
    (decreases l1) =
  match l1 with
  | [] -> ()
  | x :: q1 -> append_mem q1 l2 x; noRepeats_append_intro q1 l2

(** Properties of [assoc] *)

let assoc_nil (#a: eqtype) (#b: Type) (x: a) : Lemma (ensures (assoc #a #b x [] == None)) = ()

let assoc_cons_eq (#a: eqtype) (#b: Type) (x: a) (y: b) (q: list (a * b))
  : Lemma (ensures (assoc x ((x, y) :: q) == Some y)) = ()

let assoc_cons_not_eq (#a: eqtype) (#b: Type) (x: a) (x': a) (y: b) (q: list (a * b))
  : Lemma (requires (x <> x')) (ensures (assoc x' ((x, y) :: q) == assoc x' q)) = ()

let rec assoc_append_elim_r (#a: eqtype) (#b: Type) (x: a) (l1: list (a * b)) (l2: list (a * b))
...
let rec assoc_append_elim_l (#a: eqtype) (#b: Type) (x: a) (l1: list (a * b)) (l2: list (a * b))
  : Lemma (requires (assoc x l1 == None)) (ensures (assoc x (l1 @ l2) == assoc x l2)) (decreases l1) =
  match l1 with
  | [] -> ()
...
let assoc_mem (#a: eqtype) (#b: Type) (x: a) (l: list (a * b))
  ...
  | None -> assoc_memP_none x l; mem_memP x (map fst l); memP_map_elim fst x l
  | Some y -> assoc_memP_some x y l; memP_map_intro fst (x, y) l; mem_memP x (map fst l)

(** Properties of [fold_left] *)

let rec fold_left_invar
  (#a: Type) (#b: Type) (f: (a -> b -> Tot a)) (l: list b) (p: (a -> Tot Type0))
...
let rec fold_left_map
  (#a: Type) (#b: Type) (#c: Type) (f_aba: (a -> b -> Tot a)) (f_bc: (b -> Tot c))
  (f_aca: (a -> c -> Tot a)) (l: list b)
...
let rec map_append (#a: Type) (#b: Type) (f: (a -> Tot b)) (l1: list a) (l2: list a)
  : Lemma (ensures map f (l1 @ l2) == map f l1 @ map f l2) =
  match l1 with
  | [] -> ()
  | x :: q -> map_append f q l2

let rec fold_left_append (#a: Type) (#b: Type) (f: (a -> b -> Tot a)) (l1: list b) (l2: list b)
...
let rec fold_left_monoid (#a: Type) (opA: (a -> a -> Tot a)) (zeroA: a) (l: list a)
  : Lemma
    (requires
      (forall u v w. (opA u (opA v w)) == (opA (opA u v) w)) /\ (forall x. (opA x zeroA) == x) /\
      (forall x. (opA zeroA x) == x))
    (ensures forall x. (fold_left opA x l) == (opA x (fold_left opA zeroA l))) =
  match l with
  | [] -> ()
  | x :: q -> fold_left_monoid opA zeroA q

let fold_left_append_monoid (#a: Type) (f: (a -> a -> Tot a)) (z: a) (l1: list a) (l2: list a)
  : Lemma
    (requires
      (forall u v w. f u (f v w) == f (f u v) w) /\ (forall x. f x z == x) /\ (forall x. f z x == x)
...
let rec index_extensionality_aux
  (#a: Type) (l1: list a) (l2: list a) (l_len: (l_len: unit{length l1 == length l2}))
...
let index_extensionality (#a: Type) (l1: list a) (l2: list a)
  : Lemma
    (requires
      (length l1 == length l2 /\ (forall (i: nat). i < length l1 ==> index l1 i == index l2 i)))
    (ensures (l1 == l2)) = index_extensionality_aux l1 l2 () (fun i -> ())

(** Properties of [strict_prefix_of] *)

let rec strict_prefix_of_nil (#a: Type) (x: a) (l: list a)
  : Lemma (requires True) (ensures (strict_prefix_of [] (x :: l))) (decreases l) =
  match l with
  | [] -> ()
  | a' :: q -> strict_prefix_of_nil a' q

let strict_prefix_of_or_eq_nil (#a: Type) (l: list a)
  : Lemma (ensures (strict_prefix_of [] l \/ l == [])) =
  match l with
  | [] -> ()
  | a :: q -> strict_prefix_of_nil a q

let strict_prefix_of_cons (#a: Type) (x: a) (l: list a)
  : Lemma (ensures (strict_prefix_of l (x :: l))) = ()

let rec strict_prefix_of_trans (#a: Type) (l1: list a) (l2: list a) (l3: list a)
  : Lemma (requires True)
...
let rec strict_prefix_of_correct #a (l1: list a) (l2: list a)
  : Lemma (requires True) (ensures (strict_prefix_of l1 l2 ==> l1 << l2)) (decreases l2) =
  match l2 with
  | [] -> ()
  | _ :: q -> strict_prefix_of_correct l1 q

let rec map_strict_prefix_of (#a: Type) (#b: Type) (f: (a -> Tot b)) (l1: list a) (l2: list a)
  : Lemma (requires True)
    (ensures (strict_prefix_of l1 l2 ==> strict_prefix_of (map f l1) (map f l2)))
    (decreases l2) =
  match l2 with
  | [] -> ()
  | a :: q -> map_strict_prefix_of f l1 q

let rec mem_strict_prefix_of (#a: eqtype) (l1: list a) (m: a) (l2: list a)
  : Lemma (requires True) (ensures ((mem m l1 /\ strict_prefix_of l1 l2) ==> mem m l2)) =
  match l2 with
  | [] -> ()
  | a :: q -> mem_strict_prefix_of l1 m q

let rec strict_prefix_of_exists_append (#a: Type) (l1: list a) (l2: list a)
  : Lemma (ensures (strict_prefix_of l1 l2 ==> (exists l3. l2 == append l3 l1))) =
  match l2 with
  | [] -> ()
  | a :: q ->
...
let strict_prefix_of_or_eq_exists_append (#a: Type) (l1: list a) (l2: list a)
  : Lemma (ensures ((strict_prefix_of l1 l2 \/ l1 == l2) ==> (exists l3. l2 == append l3 l1))) =
  FStar.Classical.or_elim #(strict_prefix_of l1 l2)
    #(l1 == l2)
    #(fun _ -> exists l3. l2 == append l3 l1)
    (fun _ -> strict_prefix_of_exists_append l1 l2)
    (fun _ -> FStar.Classical.exists_intro (fun l3 -> l2 == append l3 l1) [])

(** Properties of << with lists *)

let precedes_tl (#a: Type) (l: list a {Cons? l}) : Lemma (ensures (tl l << l)) = ()
...
let precedes_append_cons_prod_r
  (#a: Type) (#b: Type) (l: list (a * b)) (l1: list (a * b)) (x: a) (y: b) (l2: list (a * b))
  : Lemma (requires (l == append l1 ((x, y) :: l2)))
    (ensures (x << l /\ y << l))
    [
      SMTPatOr
      [
        [SMTPat (x << l); SMTPat (l == append l1 ((x, y) :: l2))];
        [SMTPat (y << l); SMTPat (l == append l1 ((x, y) :: l2))]
      ]
