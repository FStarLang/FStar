...
val includes_gcell_gen: 
  ...
  i: UInt32.t ->
  Lemma (requires (includes p q /\ UInt32.v i < UInt32.v length))
    (ensures (UInt32.v i < UInt32.v length /\ includes p (gcell q i)))
    [SMTPat (includes p (gcell q i))]

val loc_includes_union_assoc_r2l: s1: loc -> s2: loc -> s3: loc -> s: loc ->
  Lemma (requires (loc_includes (loc_union s1 (loc_union s2 s3)) s))
    (ensures (loc_includes (loc_union (loc_union s1 s2) s3) s))
    [SMTPat (loc_includes (loc_union (loc_union s1 s2) s3) s)]

val loc_includes_union_assoc_l2r: s1: loc -> s2: loc -> s3: loc -> s: loc ->
  Lemma (requires (loc_includes (loc_union (loc_union s1 s2) s3) s))
    (ensures (loc_includes (loc_union s1 (loc_union s2 s3)) s))
    [SMTPat (loc_includes (loc_union s1 (loc_union s2 s3)) s)]

val loc_includes_union_assoc_focalize_1: l1: loc -> l2: loc -> x: loc -> r: loc -> s: loc ->
  Lemma (requires (loc_includes (loc_union (loc_union l1 l2) (loc_union x r)) s))
...
val loc_includes_union_assoc_focalize_2: l: loc -> x: loc -> r1: loc -> r2: loc -> s: loc ->
  Lemma (requires (loc_includes (loc_union l (loc_union x (loc_union r1 r2))) s))
...
val loc_includes_region_union_r: l: loc -> s1: Set.set HH.rid -> s2: Set.set HH.rid ->
...
val loc_includes_region_union_assoc: l: loc -> r: loc -> s1: Set.set HH.rid -> s2: Set.set HH.rid ->
  Lemma
...
val loc_disjoint_none_l: s: loc ->
  Lemma (ensures (loc_disjoint loc_none s)) [SMTPat (loc_disjoint loc_none s)]

val loc_disjoint_union_l: s: loc -> s1: loc -> s2: loc ->
  Lemma (requires (loc_disjoint s1 s /\ loc_disjoint s2 s))
...
val loc_disjoint_gfield_r: 
  ...
  q: pointer (TStruct l) ->
  fd: struct_field l ->
  Lemma (requires (loc_disjoint p (loc_pointer q)))
...
val loc_disjoint_gfield_l: 
  ...
  q: pointer (TStruct l) ->
  fd: struct_field l ->
  Lemma (requires (loc_disjoint (loc_pointer q) p))
...
val loc_disjoint_gcell_r: 
  ...
  i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v len /\ loc_disjoint p (loc_pointer q)))
...
val loc_disjoint_gcell_l: 
  ...
  q: pointer (TArray len value) ->
  i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v len /\ loc_disjoint (loc_pointer q) p))
    (ensures (UInt32.v i < UInt32.v len /\ loc_disjoint (loc_pointer (gcell q i)) p))
    [SMTPat (loc_disjoint (loc_pointer (gcell q i)) p)]

val loc_disjoint_gsingleton_buffer_of_pointer_r: l: loc -> #t: typ -> p: pointer t ->
  Lemma (requires (loc_disjoint l (loc_pointer p)))
...
val loc_disjoint_gsingleton_buffer_of_pointer_l: l: loc -> #t: typ -> p: pointer t ->
  Lemma (requires (loc_disjoint (loc_pointer p) l))
...
val loc_disjoint_gpointer_of_buffer_cell_r: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t ->
...
val loc_disjoint_gpointer_of_buffer_cell_l: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t ->
...
val loc_disjoint_gsub_buffer_r: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ loc_disjoint l (loc_buffer b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        loc_disjoint l (loc_buffer (gsub_buffer b i len))))
    [SMTPat (loc_disjoint l (loc_buffer (gsub_buffer b i len)))]

val loc_disjoint_gsub_buffer_l: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ loc_disjoint (loc_buffer b) l))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        loc_disjoint (loc_buffer (gsub_buffer b i len)) l))
    [SMTPat (loc_disjoint (loc_buffer (gsub_buffer b i len)) l)]

val loc_disjoint_addresses_pointer: #t: typ -> p: pointer t -> r: HH.rid -> n: Set.set nat ->
...
val loc_disjoint_union_r_elim: l: loc -> l1: loc -> l2: loc ->
  Lemma (requires (loc_disjoint l (loc_union l1 l2)))
    (ensures (loc_disjoint l l1 /\ loc_disjoint l l2))
    [SMTPat (loc_disjoint l (loc_union l1 l2))]

val loc_disjoint_union_l_elim: l: loc -> l1: loc -> l2: loc ->
  Lemma (requires (loc_disjoint (loc_union l1 l2) l))
    (ensures (loc_disjoint l1 l /\ loc_disjoint l2 l))
    [SMTPat (loc_disjoint (loc_union l1 l2) l)]

val modifies_trans_incl_l: s12: loc -> h1: HS.mem -> h2: HS.mem -> s23: loc -> h3: HS.mem ->
  Lemma (requires (modifies s12 h1 h2 /\ modifies s23 h2 h3 /\ loc_includes s12 s23))
    (ensures (modifies s12 h1 h3))
    [SMTPat (modifies s12 h1 h2); SMTPat (modifies s23 h2 h3)]

val modifies_trans_incl_r: s12: loc -> h1: HS.mem -> h2: HS.mem -> s23: loc -> h3: HS.mem ->
  Lemma (requires (modifies s12 h1 h2 /\ modifies s23 h2 h3 /\ loc_includes s23 s12))
...
val modifies_fresh_frame_popped': h0: HS.mem -> h1: HS.mem -> s: loc -> h2: HS.mem -> h3: HS.mem ->
  Lemma
    (requires
      (HS.fresh_frame h0 h1 /\ modifies (loc_union (loc_regions (Set.singleton h1.HS.tip)) s) h1 h2 /\
...
val buffer_includes_gsub_r_gen: 
  ...
  i: UInt32.t ->
  len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ buffer_includes b0 b))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
...
val readable_gpointer_of_buffer_cell_gsub: 
  ...
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ UInt32.v i <= UInt32.v j /\
        UInt32.v j < UInt32.v i + UInt32.v len /\ buffer_readable h (gsub_buffer b i len)))
    (ensures (UInt32.v j < UInt32.v (buffer_length b) /\ readable h (gpointer_of_buffer_cell b j)))
    [
      SMTPat (readable h (gpointer_of_buffer_cell b j));
      SMTPat (buffer_readable h (gsub_buffer b i len))
    ]

val buffer_contents_equal: #a: typ -> b1: buffer a -> b2: buffer a -> len: UInt32.t ->
  HST.Stack bool
    (requires
      (fun h ->
          hasEq (type_of_typ a) /\ UInt32.v len <= UInt32.v (buffer_length b1) /\
          UInt32.v len <= UInt32.v (buffer_length b2) /\ buffer_readable h (gsub_buffer b1 0ul len) /\
    ...
    (ensures
      (fun h0 z h1 ->
          ...
          (z == true <==>
            Seq.equal (buffer_as_seq h0 (gsub_buffer b1 0ul len))
              (buffer_as_seq h0 (gsub_buffer b2 0ul len)))))

val buffer_readable_intro_empty: #t: typ -> h: HS.mem -> b: buffer t ->
  Lemma (requires (buffer_live h b /\ UInt32.v (buffer_length b) == 0))
...
val buffer_readable_gsub_intro: 
  ...
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ buffer_live h b /\
        (forall (j: UInt32.t).
            (UInt32.v i <= UInt32.v j /\ UInt32.v j < UInt32.v i + UInt32.v len) ==>
...
val buffer_as_seq_gsub_buffer_append: 
  ...
  Lemma
    ...
    (ensures
      (UInt32.v i + UInt32.v len1 + UInt32.v len2 <= UInt32.v (buffer_length b) /\
        buffer_as_seq h (gsub_buffer b i (UInt32.add len1 len2)) ==
...
val buffer_as_seq_gsub_buffer_snoc: 
  ...
  Lemma (requires (UInt32.v i + UInt32.v len < UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len < UInt32.v (buffer_length b) /\
        buffer_as_seq h (gsub_buffer b i (UInt32.add len 1ul)) ==
...
val buffer_snoc: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t -> v: type_of_typ t ->
  HST.Stack unit
    (requires
      (fun h ->
          UInt32.v i + UInt32.v len < UInt32.v (buffer_length b) /\
    ...
    (ensures
      (fun h _ h' ->
          ...
          buffer_as_seq h' (gsub_buffer b i (UInt32.add len 1ul)) ==
          Seq.snoc (buffer_as_seq h (gsub_buffer b i len)) v))

val buffer_cons: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t -> v: type_of_typ t ->
  HST.Stack unit
    (requires
      (fun h ->
          UInt32.v i + UInt32.v len < UInt32.v (buffer_length b) /\
    ...
    (ensures
      (fun h _ h' ->
          ...
          buffer_readable h' (gsub_buffer b i (UInt32.add len 1ul)) /\
          buffer_as_seq h' (gsub_buffer b i (UInt32.add len 1ul)) ==
          Seq.cons v (buffer_as_seq h (gsub_buffer b (UInt32.add i 1ul) len))))
...
val buffer_readable_gsub_merge: 
  ...
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        buffer_readable h (gsub_buffer b 0ul i) /\ buffer_readable h (gsub_buffer b i len) /\
...
val buffer_readable_modifies_gsub: 
  ...
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ modifies l h0 h1 /\
        loc_disjoint l (loc_buffer (gsub_buffer b 0ul i)) /\
        loc_disjoint l
          (loc_buffer (gsub_buffer b
                  (UInt32.add i len)
                  (UInt32.sub (buffer_length b) (UInt32.add i len)))) /\ buffer_readable h0 b /\
        buffer_readable h1 (gsub_buffer b i len)))
    (ensures (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ buffer_readable h1 b))
