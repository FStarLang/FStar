...
let liftM f x = xx <-- x ; return (f xx)

val liftM2: ('a -> 'b -> 'c) -> (tm 'a -> tm 'b -> tm 'c)
let liftM2 f x y = xx <-- x ; yy <-- y ; return (f xx yy)

val liftM3: ('a -> 'b -> 'c -> 'd) -> (tm 'a -> tm 'b -> tm 'c -> tm 'd)
let liftM3 f x y z = xx <-- x ; yy <-- y ; zz <-- z ; return (f xx yy zz)

private
let rec find_idx (f: ('a -> bool)) (l: list 'a) : option ((n: nat{n < List.length l}) * 'a) =
  match l with
  | [] -> None
  | x :: xs ->
    if f x
...
let rec compare_expr (e1: expr) (e2: expr) : O.order =
  match e1, e2 with
