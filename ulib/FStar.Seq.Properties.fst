...
let lseq (a: Type) (l: nat) : Type = s: Seq.seq a {Seq.length s == l}

let indexable (#a: Type) (s: Seq.seq a) (j: int) = 0 <= j /\ j < Seq.length s

val lemma_append_inj_l: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  t1: seq a ->
  t2: seq a {length s1 = length t1 /\ equal (append s1 s2) (append t1 t2)} ->
  i: nat{i < length s1} ->
...
val lemma_append_inj_r: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  t1: seq a ->
  t2: seq a {length s1 = length t1 /\ length s2 = length t2 /\ equal (append s1 s2) (append t1 t2)} ->
  i: nat{i < length s2} ->
...
val lemma_append_len_disj: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  t1: seq a ->
  t2:
  seq a {(length s1 = length t1 \/ length s2 = length t2) /\ (equal (append s1 s2) (append t1 t2))} ->
...
val lemma_append_inj: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  t1: seq a ->
  t2: seq a {length s1 = length t1 \/ length s2 = length t2} ->
  Lemma (requires (equal (append s1 s2) (append t1 t2))) (ensures (equal s1 t1 /\ equal s2 t2))
...
val lemma_head_append: #a: Type -> s1: seq a {length s1 > 0} -> s2: seq a ->
  Lemma (head (append s1 s2) == head s1)
let lemma_head_append #a s1 s2 = ()

val lemma_tail_append: #a: Type -> s1: seq a {length s1 > 0} -> s2: seq a ->
  Lemma (tail (append s1 s2) == append (tail s1) s2)
...
val lemma_cons_inj: #a: Type -> v1: a -> v2: a -> s1: seq a -> s2: seq a ->
  Lemma (requires (equal (cons v1 s1) (cons v2 s2))) (ensures (v1 == v2 /\ equal s1 s2))
...
val lemma_split: #a: Type -> s: seq a -> i: nat{(0 <= i /\ i <= length s)} ->
  Lemma (ensures (append (fst (split s i)) (snd (split s i)) == s))
let lemma_split #a s i = cut (equal (append (fst (split s i)) (snd (split s i))) s)
...
let rec lemma_slice_first_in_append #a s1 s2 i =
  if i = 0 then () else lemma_slice_first_in_append #a (tail s1) s2 (i - 1)
...
val slice_upd: 
  ...
  k: nat{k < length s} ->
  v: a ->
  Lemma (requires k < i \/ j <= k)
    (ensures slice (upd s k v) i j == slice s i j)
    [SMTPat (slice (upd s k v) i j)]
let slice_upd #a s i j k v = lemma_eq_intro (slice (upd s k v) i j) (slice s i j)
...
val upd_slice: 
  ...
  k: nat{k < j - i} ->
  v: a ->
  Lemma (requires i + k < j)
    (ensures upd (slice s i j) k v == slice (upd s (i + k) v) i j)
    [SMTPat (upd (slice s i j) k v)]
let upd_slice #a s i j k v = lemma_eq_intro (upd (slice s i j) k v) (slice (upd s (i + k) v) i j)

// TODO: should be renamed cons_head_append, or something like that (because it is NOT related to (append (cons _ _) _))
val lemma_append_cons: #a: Type -> s1: seq a {length s1 > 0} -> s2: seq a ->
  Lemma (requires True) (ensures (equal (append s1 s2) (cons (head s1) (append (tail s1) s2))))
let rec lemma_append_cons #a s1 s2 = ()

val lemma_tl: #a: Type -> hd: a -> tl: seq a -> Lemma (ensures (equal (tail (cons hd tl)) tl))
let lemma_tl #a hd tl = ()

val sorted: #a: Type -> (a -> a -> Tot bool) -> s: seq a -> Tot bool (decreases (length s))
...
#set-options "--max_fuel 1 --initial_fuel 1"
val lemma_append_count: #a: eqtype -> lo: seq a -> hi: seq a ->
  Lemma (requires True)
...
val lemma_append_count_aux: #a: eqtype -> x: a -> lo: seq a -> hi: seq a ->
  Lemma (requires True) (ensures (count x (append lo hi) = (count x lo + count x hi)))
let lemma_append_count_aux #a x lo hi = lemma_append_count lo hi

val lemma_mem_inversion: #a: eqtype -> s: seq a {length s > 0} ->
  Lemma (ensures (forall x. mem x s = (x = head s || mem x (tail s))))
...
val lemma_count_slice: #a: eqtype -> s: seq a -> i: nat{i <= length s} ->
  Lemma (requires True)
...
let rec sorted_concat_lemma #a f lo pivot hi =
  if length lo = 0
  then
    (cut (equal (append lo (cons pivot hi)) (cons pivot hi)); cut (equal (tail (cons pivot hi)) hi))
  else
    (sorted_concat_lemma f (tail lo) pivot hi;
      lemma_append_cons lo (cons pivot hi);
      lemma_tl (head lo) (append (tail lo) (cons pivot hi)))
abstract
val split_5: #a: Type -> s: seq a -> i: nat -> j: nat{i < j && j < length s} ->
  Pure (seq (seq a))
    (requires True)
    (ensures
      (fun x ->
          (length x = 5 /\
            equal s
              (append (index x 0)
                  (append (index x 1) (append (index x 2) (append (index x 3) (index x 4))))) /\
            equal (index x 0) (slice s 0 i) /\ equal (index x 1) (slice s i (i + 1)) /\
            equal (index x 2) (slice s (i + 1) j) /\ equal (index x 3) (slice s j (j + 1)) /\
...
val lemma_swap_permutes_aux_frag_eq: 
  #a: Type ->
  s: seq a ->
  i: nat{i < length s} ->
  j: nat{i <= j && j < length s} ->
  ...
  Lemma
  (ensures
    (slice s i' j' == slice (swap s i j) i' j' /\ slice s i (i + 1) == slice (swap s i j) j (j + 1) /\
...
val lemma_swap_permutes_aux: 
  #a: eqtype ->
  s: seq a ->
  i: nat{i < length s} ->
  j: nat{i <= j && j < length s} ->
  x: a ->
  Lemma (requires True) (ensures (count x s = count x (swap s i j)))
...
type permutation (a: eqtype) (s1: seq a) (s2: seq a) = (forall i. count i s1 = count i s2)
val lemma_swap_permutes: 
  #a: eqtype ->
  s: seq a ->
  i: nat{i < length s} ->
  j: nat{i <= j && j < length s} ->
...
val cons_perm: #a: eqtype -> tl: seq a -> s: seq a {length s > 0} ->
  Lemma (requires (permutation a tl (tail s))) (ensures (permutation a (cons (head s) tl) s))
...
val lemma_slice_snoc: #a: eqtype -> s: seq a -> i: nat -> j: nat{i < j && j <= length s} ->
  Lemma
  (ensures (forall x. mem x (slice s i j) <==> (x = index s (j - 1) || mem x (slice s i (j - 1)))))
...
val lemma_ordering_hi_cons: 
  #a: eqtype ->
  f: tot_ord a ->
  s: seq a ->
  back: nat ->
  len: nat{back < len && len <= length s} ->
  pv: a ->
...
val lemma_swap_slice_commute: 
  #a: Type ->
  s: seq a ->
  start: nat ->
  i: nat{start <= i} ->
  j: nat{i <= j} ->
  len: nat{j < len && len <= length s} ->
  Lemma
  (ensures (slice (swap s i j) start len == (swap (slice s start len) (i - start) (j - start))))
let lemma_swap_slice_commute #a s start i j len =
  cut (equal (slice (swap s i j) start len) (swap (slice s start len) (i - start) (j - start)))

val lemma_swap_permutes_slice: 
  #a: eqtype ->
  s: seq a ->
  start: nat ->
  i: nat{start <= i} ->
...
val lemma_swap_splice: 
  ...
  j: nat{i <= j} ->
  len: nat{j < len && len <= length s} ->
  Lemma (ensures (swap s i j == splice s start (swap s i j) len))
let lemma_swap_splice #a s start i j len =
  cut (equal (swap s i j) (splice s start (swap s i j) len))

val lemma_seq_frame_hi: 
  #a: Type ->
  s1: seq a ->
  s2: seq a {length s1 = length s2} ->
  i: nat ->
  j: nat{i <= j} ->
  m: nat{j <= m} ->
  n: nat{m < n && n <= length s1} ->
  Lemma (requires (s1 == (splice s2 i s1 j)))
    (ensures ((slice s1 m n == slice s2 m n) /\ (index s1 m == index s2 m)))
let lemma_seq_frame_hi #a s1 s2 i j m n = cut (equal (slice s1 m n) (slice s2 m n))

val lemma_seq_frame_lo: 
  #a: Type ->
  s1: seq a ->
  s2: seq a {length s1 = length s2} ->
  i: nat ->
  ...
  m: nat{j < m} ->
  n: nat{m <= n && n <= length s1} ->
  Lemma (requires (s1 == (splice s2 m s1 n)))
    (ensures ((slice s1 i j == slice s2 i j) /\ (index s1 j == index s2 j)))
let lemma_seq_frame_lo #a s1 s2 i j m n = cut (equal (slice s1 i j) (slice s2 i j))
...
let lemma_tail_slice #a s i j = cut (equal (tail (slice s i j)) (slice s (i + 1) j))

val lemma_weaken_frame_right: 
  #a: Type ->
  s1: seq a ->
  s2: seq a {length s1 = length s2} ->
  i: nat ->
  j: nat ->
  k: nat{i <= j && j <= k && k <= length s1} ->
  Lemma (requires (s1 == splice s2 i s1 j)) (ensures (s1 == splice s2 i s1 k))
let lemma_weaken_frame_right #a s1 s2 i j k = cut (equal s1 (splice s2 i s1 k))

val lemma_weaken_frame_left: 
  #a: Type ->
  s1: seq a ->
  s2: seq a {length s1 = length s2} ->
  i: nat ->
  j: nat ->
  k: nat{i <= j && j <= k && k <= length s1} ->
  Lemma (requires (s1 == splice s2 j s1 k)) (ensures (s1 == splice s2 i s1 k))
let lemma_weaken_frame_left #a s1 s2 i j k = cut (equal s1 (splice s2 i s1 k))

val lemma_trans_frame: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  s3: seq a {length s1 = length s2 /\ length s2 = length s3} ->
  i: nat ->
  j: nat{i <= j && j <= length s1} ->
  Lemma (requires ((s1 == splice s2 i s1 j) /\ s2 == splice s3 i s2 j))
    (ensures (s1 == splice s3 i s1 j))
let lemma_trans_frame #a s1 s2 s3 i j = cut (equal s1 (splice s3 i s1 j))

val lemma_weaken_perm_left: 
  #a: eqtype ->
  s1: seq a ->
  s2: seq a {length s1 = length s2} ->
  i: nat ->
  j: nat ->
  k: nat{i <= j /\ j <= k /\ k <= length s1} ->
  Lemma (requires (s1 == splice s2 j s1 k /\ permutation a (slice s2 j k) (slice s1 j k)))
...
val lemma_weaken_perm_right: 
  ...
  j: nat ->
  k: nat{i <= j /\ j <= k /\ k <= length s1} ->
  Lemma (requires (s1 == splice s2 i s1 j /\ permutation a (slice s2 i j) (slice s1 i j)))
    (ensures (permutation a (slice s2 i k) (slice s1 i k)))
...
val lemma_trans_perm: 
  ...
  Lemma
    (requires
      (permutation a (slice s1 i j) (slice s2 i j) /\ permutation a (slice s2 i j) (slice s3 i j)))
...
let lemma_cons_snoc (#a: Type) (hd: a) (s: Seq.seq a) (tl: a)
  : Lemma (requires True) (ensures (Seq.equal (cons hd (snoc s tl)) (snoc (cons hd s) tl))) = ()
...
val lemma_snoc_inj: #a: Type -> s1: seq a -> s2: seq a -> v1: a -> v2: a ->
  Lemma (requires (equal (snoc s1 v1) (snoc s2 v2))) (ensures (v1 == v2 /\ equal s1 s2))
...
val find_append_some: #a: Type -> s1: seq a -> s2: seq a -> f: (a -> Tot bool) ->
...
val find_append_none: #a: Type -> s1: seq a -> s2: seq a -> f: (a -> Tot bool) ->
...
val find_append_none_s2: #a: Type -> s1: seq a -> s2: seq a -> f: (a -> Tot bool) ->
...
let rec find_append_none_s2 #a s1 s2 f =
  ...
  then cut (equal (append s1 s2) s2)
    ...
    then ()
      ...
      (find_append_none_s2 (tail s1) s2 f; cut (equal (tail (append s1 s2)) (append (tail s1) s2)))

val find_snoc: #a: Type -> s: Seq.seq a -> x: a -> f: (a -> Tot bool) ->
  Lemma
    (ensures
      (let res = find_l f (snoc s x) in
        match res with
        | None -> find_l f s == None /\ not (f x)
        | Some y -> res == find_l f s \/ (f x /\ x == y))) (decreases (Seq.length s))
...
let rec seq_find_aux #a f l ctr =
  ...
  | _ ->
    let i = ctr - 1 in
    if f (Seq.index l i) then (cut (found i); Some (Seq.index l i)) else seq_find_aux f l i
...
let seq_find #a f l = seq_find_aux f l (Seq.length l)

let find_mem (#a: eqtype) (s: seq a) (f: (a -> Tot bool)) (x: a{f x})
  : Lemma (requires (mem x s)) (ensures (Some? (seq_find f s) /\ f (Some?.v (seq_find f s)))) =
...
val seq_to_list: #a: Type -> s: seq a ->
...
let rec lemma_seq_list_bij #a s =
  if length s = 0
  then (Seq.lemma_eq_intro s (seq_of_list (seq_to_list s)))
  else (lemma_seq_list_bij (slice s 1 (length s)); lemma_eq_intro s (seq_of_list (seq_to_list s)))
...
let rec lemma_index_is_nth #a s i =
  if i = 0 then () else (lemma_index_is_nth (slice s 1 (length s)) (i - 1))
...
////////////////////////////////////////////////////////////////////////////////
abstract
let contains (#a: Type) (s: seq a) (x: a) : Tot Type0 =
  exists (k: nat). k < Seq.length s /\ Seq.index s k == x

let contains_intro (#a: Type) (s: seq a) (k: nat) (x: a)
  : Lemma (k < Seq.length s /\ Seq.index s k == x ==> contains s x) = ()

let contains_elim (#a: Type) (s: seq a) (x: a)
  : Lemma (contains s x ==> (exists (k: nat). k < Seq.length s /\ Seq.index s k == x)) = ()

let lemma_contains_empty (#a: Type) : Lemma (forall (x: a). ~(contains Seq.empty x)) = ()

let lemma_contains_singleton (#a: Type) (x: a)
  : Lemma (forall (y: a). contains (create 1 x) y ==> y == x) = ()
private
let intro_append_contains_from_disjunction (#a: Type) (s1: seq a) (s2: seq a) (x: a)
  : Lemma (requires contains s1 x \/ contains s2 x) (ensures contains (append s1 s2) x) =
  let open FStar.Classical in
  let open FStar.StrongExcludedMiddle in
  let open FStar.Squash in
  if strong_excluded_middle (contains s1 x)
  then ()
  else
    let s = append s1 s2 in
    exists_elim (contains s x)
      (get_proof (contains s2 x))
      (fun k -> assert (Seq.index s (Seq.length s1 + k) == x))

let append_contains_equiv (#a: Type) (s1: seq a) (s2: seq a) (x: a)
  : Lemma (contains (append s1 s2) x <==> (contains s1 x \/ contains s2 x)) =
  FStar.Classical.move_requires (intro_append_contains_from_disjunction s1 s2) x

val contains_snoc: #a: Type -> s: Seq.seq a -> x: a ->
  Lemma (ensures (forall y. contains (snoc s x) y <==> contains s y \/ x == y))
let contains_snoc #a s x = FStar.Classical.forall_intro (append_contains_equiv s (Seq.create 1 x))

let rec lemma_find_l_contains (#a: Type) (f: (a -> Tot bool)) (l: seq a)
  : Lemma (requires True)
    (ensures Some? (find_l f l) ==> contains l (Some?.v (find_l f l)))
...
let contains_cons (#a: Type) (hd: a) (tl: Seq.seq a) (x: a)
  : Lemma (contains (cons hd tl) x <==> (x == hd \/ contains tl x)) =
  append_contains_equiv (Seq.create 1 hd) tl x

let append_cons_snoc (#a: Type) (u: Seq.seq a) (x: a) (v: Seq.seq a)
  : Lemma (Seq.equal (Seq.append u (cons x v)) (Seq.append (snoc u x) v)) = ()

let append_slices (#a: Type) (s1: Seq.seq a) (s2: Seq.seq a)
  : Lemma
  (Seq.equal s1 (Seq.slice (Seq.append s1 s2) 0 (Seq.length s1)) /\
...
let suffix_of (#a: Type) (s_suff: seq a) (s: seq a) = exists s_pref. (s == append s_pref s_suff)

let cons_head_tail (#a: Type) (s: seq a {length s > 0})
  : Lemma (requires True) (ensures (s == cons (head s) (tail s))) [SMTPat (cons (head s) (tail s))] =
...
let head_cons (#a: Type) (x: a) (s: seq a) : Lemma (ensures (head (cons x s) == x)) = ()

let suffix_of_tail (#a: Type) (s: seq a {length s > 0})
  : Lemma (requires True) (ensures (suffix_of (tail s) s)) [SMTPat (suffix_of (tail s) s)] =
  cons_head_tail s

let index_cons_l (#a: Type) (c: a) (s: seq a) : Lemma (ensures (index (cons c s) 0 == c)) = ()

let index_cons_r (#a: Type) (c: a) (s: seq a) (i: nat{1 <= i /\ i <= length s})
  : Lemma (ensures (index (cons c s) i == index s (i - 1))) = ()

let append_cons (#a: Type) (c: a) (s1: seq a) (s2: seq a)
  : Lemma (ensures (append (cons c s1) s2 == cons c (append s1 s2))) =
  lemma_eq_elim (append (cons c s1) s2) (cons c (append s1 s2))

let index_tail (#a: Type) (s: seq a {length s > 0}) (i: nat{i < length s - 1})
  : Lemma (ensures (index (tail s) i == index s (i + 1))) = ()

let mem_cons (#a: eqtype) (x: a) (s: seq a)
  : Lemma (ensures (forall y. mem y (cons x s) <==> mem y s \/ x = y)) =
  lemma_append_count (create 1 x) s

let snoc_slice_index (#a: Type) (s: seq a) (i: nat) (j: nat{i <= j /\ j < length s})
  : Lemma (requires True)
...
let cons_index_slice (#a: Type) (s: seq a) (i: nat) (j: nat{i < j /\ j <= length s})
  : Lemma (requires True)
...
let slice_is_empty (#a: Type) (s: seq a) (i: nat{i <= length s})
  : Lemma (requires True) (ensures (slice s i i == Seq.empty)) [SMTPat (slice s i i)] =
  lemma_eq_elim (slice s i i) Seq.empty

let slice_length (#a: Type) (s: seq a)
  : Lemma (requires True) (ensures (slice s 0 (length s) == s)) [SMTPat (slice s 0 (length s))] =
  lemma_eq_elim (slice s 0 (length s)) s

let slice_slice
  (#a: Type) (s: seq a) (i1: nat) (j1: nat{i1 <= j1 /\ j1 <= length s}) (i2: nat)
...
let seq_of_list_tl (#a: Type) (l: list a {List.Tot.length l > 0})
  : Lemma (requires True) (ensures (seq_of_list (List.Tot.tl l) == tail (seq_of_list l))) =
  lemma_seq_of_list_induction l;
  ()

let rec mem_seq_of_list (#a: eqtype) (x: a) (l: list a)
  : Lemma (requires True)
...
let rec explode_and
  (#a: Type) (i: nat) (s: seq a {i <= length s}) (l: list a {List.Tot.length l + i = length s})
  : Tot Type (decreases (List.Tot.length l)) =
  match l with
  | [] -> True
...
let intro_of_list (#a: Type) (s: seq a) (l: list a)
  : Lemma (requires (List.Tot.length l = length s /\ pointwise_and s l))
    (ensures (s == seq_of_list l)) = intro_of_list' 0 s l

val elim_of_list': #a: Type -> i: nat -> s: seq a -> l: list a ->
  Lemma
    (requires
      (List.Tot.length l + i = length s /\ i <= length s /\ slice s i (length s) == seq_of_list l))
    (ensures (explode_and i s l))
    (decreases (List.Tot.length l))

let rec elim_of_list' #a i s l =
  match l with
  | [] -> ()
  | hd :: tl -> lemma_seq_of_list_induction l; elim_of_list' (i + 1) s tl

let elim_of_list (#a: Type) (l: list a)
  : Lemma
  (ensures
    (let s = seq_of_list l in
      pointwise_and s l)) = elim_of_list' 0 (seq_of_list l) l
...
let lemma_seq_sortwith_correctness (#a: eqtype) (f: (a -> a -> Tot int)) (s: seq a)
  ...
  (* sortedness *)
  //the list returned by List.sortWith is sorted
  List.Tot.Properties.sortWith_sorted f l;
  //seq_of_list preserves sortedness
  lemma_seq_of_list_sorted cmp l';
  (* permutation *)
  //seq_to_list is a permutation
  lemma_seq_to_list_permutation s;
  //List.sortWith is a permutation
  List.Tot.Properties.sortWith_permutation f l;
  //seq_of_list is a permutation
...
let compatible_sub_preorder
  (#a: Type0) (len: nat) (rel: seq_pre a) (i: nat) (j: nat{i <= j /\ j <= len}) (sub_rel: seq_pre a) =
...
let lemma_seq_sub_compatilibity_is_reflexive (#a: Type0) (len: nat) (rel: seq_pre a)
  ...
  assert (forall (s1: seq a) (s2: seq a).
        length s1 == length s2 ==> equal (replace_subseq s1 0 (length s1) s2) s2)
...
let lemma_seq_sub_compatibility_is_transitive
  (#a: Type0) (len: nat) (rel: seq_pre a) (i1: nat) (j1: nat) (rel1: seq_pre a) (i2: nat) (j2: nat)
  (rel2: seq_pre a)
  : Lemma
    (requires
  ...
  let aux (a: Type0) (len: nat) (i1: nat) (j1: nat) (i2: nat) (j2: nat) (s: seq a) (s2: seq a)
    : Lemma
    ((i1 <= j1 /\ j1 <= len /\ i2 <= j2 /\ j2 <= j1 - i1 /\ length s == len /\ length s2 == j2 - i2) ==>
