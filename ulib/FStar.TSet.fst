...
abstract
type set (a: Type) = a -> Tot prop
abstract
type equal (#a: Type) (s1: set a) (s2: set a) = forall x. s1 x <==> s2 x
...
abstract
val singleton: 'a -> Tot (set 'a)
abstract
val union: set 'a -> set 'a -> Tot (set 'a)
abstract
val intersect: set 'a -> set 'a -> Tot (set 'a)
abstract
val complement: set 'a -> Tot (set 'a)
...
val mem_empty: #a: Type -> x: a ->
  Lemma (requires True) (ensures (~(mem x empty))) [SMTPat (mem x empty)]
abstract
...
abstract
let tset_of_set (#a: eqtype) (s: Set.set a) : set a = fun (x: a) -> squash (b2t (Set.mem x s))
private
let lemma_mem_tset_of_set_l (#a: eqtype) (s: Set.set a) (x: a)
  : Lemma (requires True) (ensures (mem x (tset_of_set s) ==> Set.mem x s)) =
  if FStar.StrongExcludedMiddle.strong_excluded_middle (mem x (tset_of_set s))
  then
...
private
let lemma_mem_tset_of_set_r (#a: eqtype) (s: Set.set a) (x: a)
  : Lemma (requires True) (ensures (Set.mem x s ==> mem x (tset_of_set s))) =
  if Set.mem x s
  then
...
let lemma_mem_tset_of_set (#a: eqtype) (s: Set.set a) (x: a)
  ...
  lemma_mem_tset_of_set_l #a s x;
  lemma_mem_tset_of_set_r #a s x

abstract
let filter (#a: Type) (f: (a -> Type0)) (s: set a) : set a = fun (x: a) -> f x /\ s x
