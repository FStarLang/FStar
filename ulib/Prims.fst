...
let pure_return0 (a: Type) (x: a) : pure_wp a =
  fun (p: pure_post a) ->
  forall (return_val: a). {:nopattern (* WP definition: any pattern breaks effect elaboration *)} return_val == x ==> p return_val
...
let pure_ite_wp0 (a: Type) (wp: pure_wp a) : pure_wp a =
  fun (post: pure_post a) ->
  forall (k: pure_post a). {:nopattern (* WP definition: any pattern breaks effect elaboration *)} (forall (x: a). {:pattern (guard_free (k x))} post x ==> k x) ==> wp k

(** Subsumption for the PURE effect *)
unfold
let pure_stronger (a: Type) (wp1 wp2: pure_wp a) = forall (p: pure_post a). {:nopattern (* WP definition: any pattern breaks effect elaboration *)} wp1 p ==> wp2 p
...
unfold
let pure_close_wp0 (a b: Type) (wp: (b -> GTot (pure_wp a))) : pure_wp a = fun (p: pure_post a) -> forall (b: b). {:nopattern (* WP definition: any pattern breaks effect elaboration *)} wp b p
