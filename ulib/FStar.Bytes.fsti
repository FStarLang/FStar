...
val reveal: bytes -> GTot (S.seq byte)

val length_reveal: x: bytes ->
  Lemma (ensures (S.length (reveal x) = length x))
    [SMTPatOr [[SMTPat (S.length (reveal x))]; [SMTPat (len x)]]]

val hide: s: S.seq byte {S.length s < pow2 32} -> GTot bytes

val hide_reveal: x: bytes -> Lemma (ensures (hide (reveal x) = x)) [SMTPat (reveal x)]

val reveal_hide: x: S.seq byte {S.length x < pow2 32} ->
...
val empty_unique: b: bytes -> Lemma (length b = 0 ==> b = empty_bytes) [SMTPat (len b)]

(** If you statically know the length, it is OK to read at arbitrary indexes *)
val get: b: bytes -> pos: u32{U32.v pos < length b} ->
  Pure byte (requires True) (ensures (fun y -> y == S.index (reveal b) (U32.v pos)))
unfold
...
val extensionality: b1: bytes -> b2: bytes -> Lemma (requires (equal b1 b2)) (ensures (b1 = b2))

(** creating byte values **)
val create: len: u32 -> v: byte ->
  b: lbytes (U32.v len) {forall (i: u32{let open U32 in i <^ len}). {:pattern b.[ i ]} b.[ i ] == v}

unfold
let create_ (n: nat{FStar.UInt.size n U32.n}) v = create (U32.uint_to_t n) v

val init: len: u32 -> f: (i: u32{let open U32 in i <^ len} -> byte) ->
  b:
  lbytes (U32.v len) {forall (i: u32{let open U32 in i <^ len}). {:pattern b.[ i ]} b.[ i ] == f i}
...
val slice: b: bytes -> s: u32 -> e: u32{U32.(s <=^ e) /\ U32.v e <= length b} ->
  r: bytes{reveal r == Seq.slice (reveal b) (U32.v s) (U32.v e)}
let slice_ b (s: nat) (e: nat{s <= e /\ e <= length b}) =
  slice b (U32.uint_to_t e) (U32.uint_to_t e)

val sub: b: bytes -> s: u32 -> l: u32{U32.v s + U32.v l <= length b} ->
  r: bytes{reveal r == Seq.slice (reveal b) (U32.v s) (U32.v s + U32.v l)}

val split: b: bytes -> k: u32{U32.v k <= length b} ->
  p:
  (bytes * bytes)
    { let x, y = p in
...
val repr_bytes: n: nat -> k: pos{fits_in_k_bytes n k}
              ...
              then k == 4
                ...
                then k == 5
                  ...
                  then k == 6
                    ...
                    then k == 7
                    else if n < 18446744073709551616 then k == 8 else True)) [SMTPat (repr_bytes n)]

val repr_bytes_size: k: nat -> n: uint_k k ->
  Lemma (ensures (repr_bytes n <= k)) [SMTPat (fits_in_k_bytes n k)]

val int_of_bytes: b: bytes -> Tot (uint_k (length b))

val bytes_of_int: k: nat -> n: nat{repr_bytes n <= k /\ k < pow2 32} -> lbytes k

val int_of_bytes_of_int: #k: nat{k <= 32} -> n: uint_k k ->
  Lemma (ensures (int_of_bytes (bytes_of_int k n) == n)) [SMTPat (bytes_of_int k n)]

val bytes_of_int_of_bytes: b: bytes{length b <= 32} ->
  Lemma (ensures (bytes_of_int (length b) (int_of_bytes b) == b)) [SMTPat (int_of_bytes b)]

//18-02-25 use [uint32] instead of [int32] etc? 
val int32_of_bytes: b: bytes{length b <= 4} -> n: u32{U32.v n == int_of_bytes b}

val int16_of_bytes: b: bytes{length b <= 2} -> n: u16{U16.v n == int_of_bytes b}

val int8_of_bytes: b: bytes{length b = 1} -> n: u8{U8.v n = int_of_bytes b}

val bytes_of_int32: n: U32.t -> b: lbytes 4 {b == bytes_of_int 4 (U32.v n)}

val bytes_of_int16: n: U16.t -> b: lbytes 2 {b == bytes_of_int 2 (U16.v n)}

val bytes_of_int8: n: U8.t -> b: lbytes 1 {b == bytes_of_int 1 (U8.v n)}

////////////////////////////////////////////////////////////////////////////////
type minbytes (n: nat) = b: bytes{length b >= n}

val xor: n: u32 -> b1: minbytes (U32.v n) -> b2: minbytes (U32.v n) -> b: bytes{len b = n}
unfold
let xor_ (#n: nat{FStar.UInt.size n U32.n}) (b1: minbytes n) (b2: minbytes n) =
  xor (U32.uint_to_t n) b1 b2

val xor_commutative: n: u32 -> b1: minbytes (U32.v n) -> b2: minbytes (U32.v n) ->
  Lemma (ensures (xor n b1 b2 == xor n b2 b1)) [SMTPat (xor n b1 b2)]
...
val xor_append: 
  ...
  Lemma
  (ensures
    (xor U32.(len b1 +^ len b2) (b1 @| b2) (x1 @| x2) == xor (len b1) b1 x1 @| xor (len b2) b2 x2))

val xor_idempotent: n: u32 -> b1: lbytes (U32.v n) -> b2: lbytes (U32.v n) ->
  Lemma (ensures (xor n (xor n b1 b2) b2 == b1))

val utf8_encode: s: string{Str.maxlen s (pow2 30)} ->
  b: bytes{length b <= op_Multiply 4 (Str.length s)}

val iutf8_opt: m: bytes -> (option (s: string{Str.maxlen s (pow2 30) /\ utf8_encode s == m}))
...
val of_buffer: l: UInt32.t -> buf: lbuffer l ->
  Stack (b: bytes{length b = U32.v l})
    (requires (fun h0 -> B.live h0 buf))
    (ensures (fun h0 b h1 -> M.(modifies loc_none h0 h1) /\ b = hide (B.as_seq h0 buf)))

val store_bytes: src: bytes{length src <> 0} -> dst: lbuffer (len src) ->
  Stack unit
    (requires (fun h0 -> B.live h0 dst))
    (ensures
      (fun h0 r h1 ->
          M.(modifies (loc_buffer dst) h0 h1) /\ Seq.equal (reveal src) (B.as_seq h1 dst)))
