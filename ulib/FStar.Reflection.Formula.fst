...
noeq
type comparison =
  (* Propositional equality (eq2), maybe annotated *)
  | Eq of option typ
  (* Decidable, boolean equality (eq), maybe annotated *)
  | BoolEq of option typ
  (* Orderings, at type `int` (and subtypes) *)
  | Lt
  | Le
  | Gt
  | Ge
...
type formula =
  ...
  | FV : fv -> formula
  | IntLit : int -> formula
  // Also a baked-in "None"
  | F_Unknown : formula

let mk_Forall (typ: term) (pred: term) : Tac formula =
  let b = pack_bv ({ bv_ppname = "x"; bv_sort = typ; bv_index = 0 }) in
  ...
  | Tv_App h0 t ->
    ...
    (match inspect_ln h, ts @ [t] with
      ...
      | Tv_FVar fv, [a, Q_Explicit] ->
        let qn = inspect_fv fv in
        if qn = not_qn then Not a else App h0 (fst t)
      | _ -> App h0 (fst t))
  | Tv_Arrow b c ->
    ...
    (match inspect_comp c with
      | C_Total t _ -> if is_free bv t then Forall bv t else Implies (type_of_bv bv) t
      | _ -> F_Unknown)
...
let rec is_name_imp (nm: name) (t: term) : bool =
  match inspect_ln t with
  | Tv_FVar fv -> if inspect_fv fv = nm then true else false
...
let unsquash_total (t: term) : term =
  match inspect_ln t with
  | Tv_App l (r, Q_Explicit) -> if is_name_imp squash_qn l then r else t
  | _ -> t
...
let rec term_as_formula_total (t: term) : Tac formula = term_as_formula' (unsquash_total t)
...
let formula_as_term_view (f: formula) : Tot term_view =
  ...
  | Forall b t ->
    // TODO: decide on meaning of this
    Tv_Unknown
  | Exists b t ->
    // TODO: ^
    Tv_Unknown
