...
let gsub_inj
  (#a: Type0) (b1: buffer a) (b2: buffer a) (i1: U32.t) (i2: U32.t) (len1: U32.t) (len2: U32.t)
  : Lemma
    (requires
      (U32.v i1 + U32.v len1 <= length b1 /\ U32.v i2 + U32.v len2 <= length b2 /\
        gsub b1 i1 len1 === gsub b2 i2 len2))
    (ensures
      (len1 == len2 /\ (b1 == b2 ==> i1 == i2) /\
        ((i1 == i2 /\ length b1 == length b2) ==> b1 == b2))) =
...
let sub (#a: Type0) (b: buffer a) (i: U32.t) (len: U32.t)
  : HST.Stack (buffer a)
    ...
    (ensures (fun h y h' -> h == h' /\ y == gsub b i len)) = msub b i len (trivial_preorder a)
...
let gcmalloc (#a: Type0) (r: HS.rid) (init: a) (len: U32.t)
  : HST.ST (b: buffer a {recallable b /\ alloc_post_static r (U32.v len) b})
    ...
    (ensures
      (fun h b h' ->
          alloc_post_common r (U32.v len) b h h' /\ as_seq h' b == Seq.create (U32.v len) init)) =
  ...
  : HST.StackInline (buffer a)
    ...
    (ensures
      (fun h b h' ->
          ...
          as_seq h' b == Seq.create (U32.v len) init)) = malloca init len
  ...
  : HST.StackInline (buffer a)
    ...
    (ensures
      (fun h b h' ->
          ...
          alloc_post_common (HS.get_tip h) len b h h' /\ as_seq h' b == Seq.seq_of_list init /\
          alloc_of_list_post #a len b)) = malloca_of_list init
...
let gcmalloc_of_list (#a: Type0) (r: HS.rid) (init: list a)
  : HST.ST
    (b:
      buffer a
        { let len = FStar.List.Tot.length init in
          recallable b /\ alloc_post_static r len b /\ alloc_of_list_post len b })
    (requires (fun h -> HST.is_eternal_region r /\ gcmalloc_of_list_pre #a init))
    (ensures
      (fun h b h' ->
          let len = FStar.List.Tot.length init in
          alloc_post_common r len b h h' /\ as_seq h' b == Seq.seq_of_list init)) =
...
let assign_list_t #a (l: list a) =
  b: buffer a ->
    HST.Stack unit
      ...
      (ensures
        (fun h0 _ h1 ->
            live h1 b /\ (modifies (loc_buffer b) h0 h1) /\ as_seq h1 b == Seq.seq_of_list l))
...
let rec assign_list #a (l: list a) : assign_list_t l =
  fun b ->
    ...
    | [] ->
      ...
      assert (Seq.equal (Seq.seq_of_list []) (Seq.empty #a))
    | hd :: tl ->
