...
noeq
type loc_aux : Type =
  | LocBuffer : #t: Type -> b: B.buffer t -> loc_aux
...
let rec loc_aux_in_addr (l: loc_aux) (r: HS.rid) (n: nat) : GTot Type0 (decreases l) =
  ...
  | LocBuffer b -> B.frameOf b == r /\ B.as_addr b == n
...
type loc' : Type =
  | Loc : 
    ...
    addrs:
    (r: HS.rid ->
        Ghost (Set.set nat) (requires (Set.mem r (Ghost.reveal regions))) (ensures (fun _ -> True))) ->
    aux_addrs:
    (r: HS.rid ->
        Ghost (Set.set nat)
    ...
    (r: HS.rid -> n: nat ->
        Ghost loc_aux
          (requires (Set.mem r (Ghost.reveal regions) /\ Set.mem n (aux_addrs r)))
...
let loc_none =
  Loc (Ghost.hide (Set.empty)) (fun _ -> Set.empty) (fun _ -> Set.empty) (fun _ _ -> false_elim ())

let regions_of_loc (s: loc) : GTot (Set.set HS.rid) = Ghost.reveal (Loc?.regions s)

let addrs_of_loc_weak (l: loc) (r: HS.rid) : GTot (Set.set nat) =
  if Set.mem r (regions_of_loc l) then Loc?.addrs l r else Set.empty

let addrs_of_loc_aux (l: loc) (r: HS.rid) : GTot (Set.set nat) =
  if Set.mem r (regions_of_loc l) then Loc?.aux_addrs l r else Set.empty

let addrs_of_loc (l: loc) (r: HS.rid) : GTot (Set.set nat) =
  Set.union (addrs_of_loc_weak l r) (addrs_of_loc_aux l r)

let addrs_of_loc_aux_prop (l: loc) (r: HS.rid)
  : Lemma (Set.subset (Set.intersect (addrs_of_loc_aux l r) (addrs_of_loc_weak l r)) Set.empty)
    [
      SMTPatOr
      [
...
let loc_union s1 s2 =
  ...
  then s1
    ...
    let addrs (r: HS.rid)
      : Ghost (Set.set nat) (requires (Set.mem r regions)) (ensures (fun _ -> True)) =
      Set.union (if Set.mem r regions1 then Loc?.addrs s1 r else Set.empty)
    ...
    let aux_addrs (r: HS.rid)
      : Ghost (Set.set nat)
        ...
        (ensures (fun y -> Set.subset (Set.intersect y (addrs r)) Set.empty)) =
      Set.intersect (Set.union (addrs_of_loc_aux s1 r) (addrs_of_loc_aux s2 r))
    ...
    let aux (r: HS.rid) (n: nat)
      ...
      | _, Some l2 -> l2
    in
    Loc (Ghost.hide regions) addrs aux_addrs aux

let loc_union_idem s = ()

let loc_buffer #t b =
  Loc (Ghost.hide (Set.singleton (B.frameOf b)))
    (fun _ -> Set.empty)
    (fun _ -> Set.singleton (B.as_addr b))
    (fun _ _ -> LocBuffer b)

let loc_addresses r n =
  Loc (Ghost.hide (Set.singleton r)) (fun _ -> n) (fun _ -> Set.empty) (fun _ _ -> false_elim ())
...
let rec loc_aux_includes_buffer (#a: Type) (s: loc_aux) (b: B.buffer a) : GTot Type0 (decreases s) =
  match s with
  | LocUnion s1 s2 -> loc_aux_includes_buffer s1 b \/ loc_aux_includes_buffer s2 b
  | LocBuffer #a0 b0 -> a == a0 /\ B.includes b0 b

let rec loc_aux_includes (s1: loc_aux) (s2: loc_aux) : GTot Type0 (decreases s2) =
  match s2 with
  | LocUnion s2l s2r -> loc_aux_includes s1 s2l /\ loc_aux_includes s1 s2r
  | LocBuffer b -> loc_aux_includes_buffer s1 b

let rec loc_aux_includes_union_l (s1: loc_aux) (s2: loc_aux) (s: loc_aux)
...
let loc_aux_includes_union_l_r (s: loc_aux) (s': loc_aux)
  : Lemma (loc_aux_includes (LocUnion s s') s) =
  loc_aux_includes_refl s;
  loc_aux_includes_union_l s s' s

let loc_aux_includes_union_l_l (s: loc_aux) (s': loc_aux)
  : Lemma (loc_aux_includes (LocUnion s' s) s) =
  loc_aux_includes_refl s;
  loc_aux_includes_union_l s' s s

let rec loc_aux_includes_buffer_includes (#a: Type) (s: loc_aux) (b1: B.buffer a) (b2: B.buffer a)
  : Lemma (requires (loc_aux_includes_buffer s b1 /\ B.includes b1 b2))
    (ensures (loc_aux_includes_buffer s b2))
    (decreases s) =
  match s with
  | LocUnion sl sr ->
    Classical.or_elim #(loc_aux_includes_buffer sl b1)
...
let loc_includes s1 s2 =
  ...
  (Set.subset regions2 regions1 /\
    ...
    (forall (r: HS.rid) (n: nat).
        (Set.mem r regions2 /\ Set.mem n (addrs_of_loc_aux s2 r) /\
          Set.mem n (addrs_of_loc_aux s1 r)) ==>
        loc_aux_includes (Loc?.aux s1 r n) (Loc?.aux s2 r n)))

let loc_includes_refl s =
  let pre (r: HS.rid) (n: nat) : GTot Type0 =
    Set.mem r (regions_of_loc s) /\ Set.mem n (Loc?.aux_addrs s r)
  in
...
let rec loc_aux_disjoint_buffer (l: loc_aux) (#t: Type) (p: B.buffer t) : GTot Type0 (decreases l) =
  match l with
  | LocUnion ll lr -> loc_aux_disjoint_buffer ll p /\ loc_aux_disjoint_buffer lr p
...
let rec loc_aux_disjoint_buffer_includes (l: loc_aux) (#t: Type) (p1: B.buffer t) (p2: B.buffer t)
  : Lemma (requires (loc_aux_disjoint_buffer l p1 /\ B.includes p1 p2))
...
let rec loc_aux_disjoint_loc_aux_includes (l1: loc_aux) (l2: loc_aux) (l3: loc_aux)
  ...
  | LocBuffer b3 -> loc_aux_disjoint_loc_aux_includes_buffer l1 l2 b3
...
let modifies_preserves_mreferences (s: loc) (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
...
let modifies_preserves_buffers (s: loc) (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  (forall (t: Type) (b: B.buffer t).
      let r = B.frameOf b in
      let a = B.as_addr b in
      (B.live h1 b /\ B.length b <> 0 /\
        (Set.mem r (regions_of_loc s) ==> ~(Set.mem a (addrs_of_loc_weak s r))) /\
        ((Set.mem r (regions_of_loc s) /\ Set.mem a (addrs_of_loc_aux s r)) ==>
          loc_aux_disjoint_buffer (Loc?.aux s r a) b)) ==>
      (B.live h2 b /\ B.as_seq h2 b == B.as_seq h1 b))

let modifies' (s: loc) (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  modifies_preserves_mreferences s h1 h2 /\ modifies_preserves_buffers s h1 h2
...
let loc_aux_disjoint_buffer_loc_aux_includes (l1: loc_aux)
  : Lemma
  (forall (l2: loc_aux) (t3: Type) (b3: B.buffer t3).
      (loc_aux_disjoint_buffer l1 b3 /\ loc_aux_includes l1 l2) ==> loc_aux_disjoint_buffer l2 b3) =
  let f (l2: loc_aux) (t3: Type) (b3: B.buffer t3)
    : Lemma (requires (loc_aux_disjoint_buffer l1 b3 /\ loc_aux_includes l1 l2))
  ...
  in
  Classical.forall_intro_3 (fun (l2: loc_aux) (t3: Type) (b3: B.buffer t3) ->
        Classical.move_requires (f l2 t3) b3)
...
let loc_includes_restrict_to_regions (l: loc) (rs: Set.set HS.rid)
  : Lemma (loc_includes l (restrict_to_regions l rs)) = Classical.forall_intro loc_aux_includes_refl
  ...
  : Lemma
    (requires
      (HS.live_region h2 r /\ modifies (loc_union (loc_region_only r) l) h1 h2 /\
        HS.modifies_ref r s h1 h2 /\ loc_disjoint l (loc_region_only r)))
    (ensures (modifies (loc_union (loc_addresses r s) l) h1 h2)) =
  assert (forall (t: Type) (pre: Preorder.preorder t) (p: HS.mreference t pre).
        (HS.frameOf p == r /\ HS.contains h1 p /\ ~(Set.mem (HS.as_addr p) s)) ==>
        (HS.contains h2 p /\ HS.sel h2 p == HS.sel h1 p));
...
let modifies_0_modifies h1 h2 = B.lemma_reveal_modifies_0 h1 h2

let modifies_1_modifies #a b h1 h2 = B.lemma_reveal_modifies_1 b h1 h2

let modifies_2_modifies #a1 #a2 b1 b2 h1 h2 = B.lemma_reveal_modifies_2 b1 b2 h1 h2

#set-options "--z3rlimit 32"

let modifies_3_modifies #a1 #a2 #a3 b1 b2 b3 h1 h2 = B.lemma_reveal_modifies_3 b1 b2 b3 h1 h2
...
let does_not_contain_addr' (h: HS.mem) (ra: HS.rid * nat) : GTot Type0 =
  forall (a: Type) (rel: Preorder.preorder a) (r: HS.mreference a rel). {:pattern (HS.contains h r)}
    (HS.frameOf r == fst ra /\ HS.as_addr r == snd ra /\ HS.contains h r) ==> False
...
let modifies_only_live_addresses_weak (r: HS.rid) (a: Set.set nat) (l: loc) (h: HS.mem) (h': HS.mem)
  : Lemma
    (requires
      (modifies (loc_union (loc_addresses r a) l) h h' /\ loc_disjoint (loc_addresses r a) l /\
...
let restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat)
  : Ghost loc (requires (loc_includes (loc_region_only r) l)) (ensures (fun _ -> True)) =
  let Loc regions addrs aux_addrs aux = l in
  let aux_addrs' =
    if Set.mem r (Ghost.reveal regions) then Set.intersect (aux_addrs r) as else Set.empty
  in
  Loc (Ghost.hide (Set.intersect (Ghost.reveal regions) (Set.singleton r)))
    (fun _ -> if Set.mem r (Ghost.reveal regions) then Set.intersect (addrs r) as else Set.empty)
    (fun _ -> aux_addrs')
    (fun r n -> aux r n)

let regions_of_loc_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat)
  : Lemma (requires (loc_includes (loc_region_only r) l))
    (ensures
      (regions_of_loc (restrict_to_addresses l r as) ==
        Set.intersect (regions_of_loc l) (Set.singleton r)))
    [SMTPat (regions_of_loc (restrict_to_addresses l r as))] =
  assert (Set.equal (regions_of_loc (restrict_to_addresses l r as))
        (Set.intersect (regions_of_loc l) (Set.singleton r)))

let addrs_of_loc_weak_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat) (r': HS.rid)
  : Lemma (requires (loc_includes (loc_region_only r) l))
    (ensures
      (addrs_of_loc_weak (restrict_to_addresses l r as) r' ==
        (if r = r' then Set.intersect (addrs_of_loc_weak l r) as else Set.empty)))
    [SMTPat (addrs_of_loc_weak (restrict_to_addresses l r as) r')] =
  assert (Set.equal (addrs_of_loc_weak (restrict_to_addresses l r as) r)
        (Set.intersect (addrs_of_loc_weak l r) as))

let addrs_of_loc_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat) (r': HS.rid)
  : Lemma (requires (loc_includes (loc_region_only r) l))
    (ensures
      (addrs_of_loc (restrict_to_addresses l r as) r' ==
        (if r = r' then Set.intersect (addrs_of_loc l r) as else Set.empty)))
    [SMTPat (addrs_of_loc (restrict_to_addresses l r as) r')] =
  assert (Set.equal (addrs_of_loc (restrict_to_addresses l r as) r)
        (Set.intersect (addrs_of_loc l r) as));
  assert (r <> r' ==> Set.equal (addrs_of_loc (restrict_to_addresses l r as) r') Set.empty)

let loc_includes_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat)
  : Lemma (requires (loc_includes (loc_region_only r) l))
    (ensures (loc_includes l (restrict_to_addresses l r as))) =
  Classical.forall_intro loc_aux_includes_refl

let loc_includes_loc_union_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat)
  : Lemma (requires (loc_includes (loc_region_only r) l))
    (ensures
      (loc_includes (loc_union (restrict_to_addresses l r as)
              (restrict_to_addresses l r (Set.complement as)))
          l)) = Classical.forall_intro loc_aux_includes_refl

let loc_includes_loc_addresses_restrict_to_addresses (l: loc) (r: HS.rid) (as: Set.set nat)
  : Lemma (requires (loc_includes (loc_region_only r) l))
...
let modifies_only_live_addresses r a l h h' =
  ...
  modifies_loc_includes (loc_union (loc_addresses r a) l_not_a)
    h
    h'
    (loc_union (loc_addresses r a) l);
