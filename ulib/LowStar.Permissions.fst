...
let rec sum_until (#a: Type0) (f:perm_id -> permission & a) (n:nat) : GTot real =
  if n = 0 then 0.0R
    ...
    let (x, _) = f n in Permission?.r x +. sum_until f (n - 1)

let perms_rec (a: Type0) = p:perms_rec' a{
  // Permissions are null above currently split objects
  (forall (n:perm_id). n > p.current_max ==> get_permission_from_pid' p n == zero_permission) /\
...
let new_value_perms (#a: Type0) (init: a) (fully_owned: bool) =
  let f:F.restricted_t perm_id (fun (x:perm_id) -> permission & a) =
     F.on_dom perm_id (fun (x:perm_id) -> if x = 1 then
         ((full_permission <: permission), init)
       else
         ((zero_permission <: permission), init)
     )
  in
...
let rec sum_until_change
  ...
  ) (ensures (let (v', _) = p1 i in
    sum_until p2 n = sum_until p1 n +. Permission?.r v -. Permission?.r v'
  ))
  =
...
let rec sum_until_extend_zeros
  ...
  : Lemma
  (requires
    (forall (i:nat{i > max}). fst (p i) == zero_permission))
...
let share_perms (#a: Type0) (#v: a) (v_perms: value_perms a v) (pid: live_pid v_perms)
  ...
  let perm_map1' = F.on_dom perm_id (fun (x:perm_id) ->
    ...
    if x = pid then ((half_permission p  <: permission), old_snap) else (old_p, old_snap))
  in
  sum_until_change v_perms.perm_map perm_map1' v_perms.current_max pid (half_permission p);
  let perm_map2' = F.on_dom perm_id (fun (x:perm_id) ->
    let (old_p, old_snap) = perm_map1' x in
    if x = current_max' then ((half_permission p <: permission), v) else (old_p, old_snap))
  in
  sum_until_change perm_map1' perm_map2' current_max' current_max' (half_permission p);
...
let share_perms_with_pid #a #v v_perms pid new_pid =
  ...
  let perm_map2' = F.on_dom perm_id (fun (x:perm_id) ->
    let (old_p, old_snap) = perm_map1' x in
    if x = new_pid then ((half_permission p <: permission), v) else (old_p, old_snap))
  in
  sum_until_extend_zeros perm_map1' v_perms.current_max new_pid;
  sum_until_change perm_map1' perm_map2' new_pid new_pid (half_permission p);
  let v_perms' : perms_rec' a =
    { v_perms with
      current_max = new_pid;
      perm_map = perm_map2'
...
noextract
let move_perms_with_pid #a #v v_perms pid new_pid =
  let (p, _) = v_perms.perm_map pid in
  let perm_map1' = F.on_dom perm_id (fun (x:perm_id) ->
    let (old_p, old_snap) = v_perms.perm_map x in
    if x = pid then ((zero_permission <: permission), old_snap) else (old_p, old_snap))
  in
  sum_until_change v_perms.perm_map perm_map1' v_perms.current_max pid zero_permission;
  let perm_map2' = F.on_dom perm_id (fun (x:perm_id) ->
    let (old_p, old_snap) = perm_map1' x in
    if x = new_pid then ((p <: permission), v) else (old_p, old_snap))
  in
  sum_until_extend_zeros perm_map1' v_perms.current_max new_pid;
  sum_until_change perm_map1' perm_map2' new_pid new_pid p;
  let v_perms' : perms_rec' a =
    { v_perms with
      current_max = new_pid;
      perm_map = perm_map2'
...
private let rec sum_greater_than_subterm (#a: Type0) (f:perm_id -> permission & a) (n:nat) (pid1:perm_id)
  : Lemma (ensures (
    ...
    sum_until f n >=. (let (x, _) = f pid1 in Permission?.r x)
  )) =
...
private let rec sum_greater_than_subterms (#a: Type0) (f:perm_id -> permission & a) (n:nat) (pid1: perm_id)
  ...
  : Lemma (ensures (
    ...
    if n < pid1 then sum_until f n >=. 0.0R else
    if n < pid2 then sum_until f n >=. (let (x, _) = f pid1 in Permission?.r x) else
    sum_until f n >=.
      (let (x, _) = f pid1 in Permission?.r x) +. (let (x, _) = f pid2 in Permission?.r x)
  )) =
...
let merge_perms
  ...
  let p_sum : permission = sum_permissions p1 p2 in
  let perm_map1' = F.on_dom perm_id (fun (x:perm_id) ->
    assert(sum_until v_perms.perm_map v_perms.current_max = 1.0R);
    if x = pid1 then ((sum_permissions p1 p2 <: permission), snap1)
    else v_perms.perm_map x
  ) in
  sum_until_change v_perms.perm_map perm_map1' v_perms.current_max pid1 p_sum;
  let perm_map2' = F.on_dom perm_id (fun (x:perm_id) ->
    if x = pid2 then ((zero_permission <: permission), snap2) else perm_map1' x
  ) in
  sum_until_change perm_map1' perm_map2' v_perms.current_max pid2 zero_permission;
  let v_perms': perms_rec' a =
