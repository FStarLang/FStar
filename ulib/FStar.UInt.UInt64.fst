...
val add: a:uint64 -> b:uint64{v a + v b < max_int n} -> Tot uint64
let add a b = MkUInt64 (add (v a) (v b))
val add_underspec: a:uint64 -> b:uint64 -> Tot uint64
...
val sub: a:uint64 -> b:uint64{v a - v b >= 0} -> Tot uint64
let sub a b = MkUInt64 (sub (v a) (v b))
val sub_underspec: a:uint64 -> b:uint64 -> Tot uint64
let sub_underspec a b = MkUInt64 (sub_underspec (v a) (v b))
val sub_mod: a:uint64 -> b:uint64 -> Tot uint64
let sub_mod a b = MkUInt64 (sub_mod (v a) (v b))
val mul: a:uint64 -> b:uint64{v a * v b < pow2 n} -> Tot uint64
let mul a b = MkUInt64(mul (v a) (v b))
val mul_underspec: a:uint64 -> b:uint64 -> Tot uint64
...
(* Division primitives *)
val div: a:uint64 -> b:uint64{v b <> 0} -> Tot uint64
let div a b = MkUInt64(div (v a) (v b))

(* Modulo primitives *)
val mod: a:uint64 -> b:uint64{v b <> 0} -> Tot uint64
...
val int_to_uint64: x:int -> Tot uint64
let int_to_uint64 x = MkUInt64 (to_uint 64 x)

(* Shift operators *)
val shift_right: a:uint64 -> s:nat -> Tot uint64
let shift_right a s = MkUInt64 (shift_right (v a) s)

val shift_left: a:uint64 -> s:nat -> Tot uint64
