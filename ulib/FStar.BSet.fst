...
assume val bset_to_set: #key:eqtype -> set key -> Tot (Set.set key)
assume val lemma_bset_to_set:
  #key:eqtype -> s:set key -> x:key
  -> Lemma (requires (True))
          (ensures (mem x s <==> Set.mem x (bset_to_set s)))
    [SMTPat (Set.mem x (bset_to_set s))]
