...
let tags' (l: P.struct_typ') : Tot Type0 =
  tl: list UInt32.t {List.Tot.length tl == List.Tot.length l /\ List.Tot.noRepeats tl}

let tags (l: P.union_typ) : Tot Type0 = tags' l.P.fields

(* Get a field from its physical tag. *)
let rec field_of_tag' (#l: P.struct_typ') (tgs: tags' l) (t: UInt32.t)
...
let field_of_tag (#l: P.union_typ) (tgs: tags l) (t: UInt32.t)
  : Pure (P.struct_field l) (requires (List.Tot.mem t tgs)) (ensures (fun _ -> True)) =
  field_of_tag' tgs t

(* Get the physical tag corresponding to a field. *)
let rec tag_of_field' (#l: P.struct_typ') (tgs: tags' l) (f: P.struct_field' l)
...
let tag_of_field (#l: P.union_typ) (tgs: tags l) (f: P.struct_field l)
  : Pure UInt32.t (requires True) (ensures (fun t -> List.Tot.mem t tgs)) = tag_of_field' tgs f
...
*)
val field_matches_tag: #l: P.union_typ -> tgs: tags l -> f: P.struct_field l -> t: UInt32.t ->
  Tot Type0

val field_of_tag_of_field: #l: P.union_typ -> tgs: tags l -> f: P.struct_field l ->
...
val field_matches_tag_intro: #l: P.union_typ -> tgs: tags l -> f: P.struct_field l -> t: UInt32.t ->
  Lemma (requires (tag_of_field tgs f == t)) (ensures (field_matches_tag tgs f t))

val field_matches_tag_elim: #l: P.union_typ -> tgs: tags l -> f: P.struct_field l -> t: UInt32.t ->
  Lemma (requires (field_matches_tag tgs f t)) (ensures (tag_of_field tgs f == t))
...
val assert_field_matches_tag: 
  ...
  f: P.struct_field l ->
  t: UInt32.t ->
  Lemma (requires (normalize_term (tag_of_field tgs f) == t)) (ensures (field_matches_tag tgs f t))
...
val valid: #l: P.union_typ -> h: HS.mem -> tgs: tags l -> p: P.pointer (typ l) -> GTot Type0

val valid_live: #l: P.union_typ -> h: HS.mem -> tgs: tags l -> p: P.pointer (typ l) ->
  Lemma (requires (valid h tgs p)) (ensures (P.live h p)) [SMTPat (valid h tgs p)]

(* Operations *)

val gread_tag: #l: P.union_typ -> h: HS.mem -> tgs: tags l -> p: P.pointer (typ l) -> GTot UInt32.t

val read_tag: #l: P.union_typ -> tgs: tags l -> p: P.pointer (typ l) ->
  HST.Stack UInt32.t
    (requires (fun h -> valid h tgs p))
    (ensures (fun h0 t h1 -> h0 == h1 /\ List.Tot.mem t tgs /\ t == gread_tag h0 tgs p))


val gfield: #l: P.union_typ -> tgs: tags l -> p: P.pointer (typ l) -> f: P.struct_field l ->
  GTot (p': P.pointer (P.typ_of_struct_field l f) {P.includes p p'})


val field: #l: P.union_typ -> tgs: tags l -> p: P.pointer (typ l) -> f: P.struct_field l ->
  HST.Stack (P.pointer (P.typ_of_struct_field l f))
    (requires (fun h -> valid h tgs p /\ gread_tag h tgs p == normalize_term (tag_of_field tgs f)))
    (ensures
      (fun h0 p' h1 ->
          h0 == h1 /\ field_matches_tag tgs f (gread_tag h0 tgs p) /\ p' == gfield tgs p f))
...
val write: 
  ...
  HST.Stack unit
    ...
    (ensures
      (fun h0 _ h1 ->
          P.live h0 p /\ P.live h1 p /\ P.modifies_1 p h0 h1 /\ P.readable h1 p /\ valid h1 tgs p /\
          gread_tag #l h1 tgs p == normalize_term (tag_of_field tgs f) /\
          field_matches_tag tgs f (gread_tag h1 tgs p) /\ P.gread h1 (gfield tgs p f) == v))

val write_tag: #l: P.union_typ -> tgs: tags l -> p: P.pointer (typ l) -> f: P.struct_field l ->
  HST.Stack unit
    (requires (fun h -> valid h tgs p))
    (ensures
      (fun h0 _ h1 ->
          valid h0 tgs p /\ valid h1 tgs p /\ P.modifies_1 p h0 h1 /\
          gread_tag #l h1 tgs p == normalize_term (tag_of_field tgs f) /\
...
val includes_gfield: 
  ...
  p: P.pointer (typ l) ->
  f: P.struct_field l ->
  Lemma (requires True) (ensures (P.includes p (gfield tgs p f)))

let includes_gfield_gen
  (#t: P.typ) (q: P.pointer t) (#l: P.union_typ) (tgs: tags l) (p: P.pointer (typ l))
  (f: P.struct_field l)
...
val live_gfield: 
  ...
  f: P.struct_field l ->
  h: HS.mem ->
  Lemma (P.live h (gfield tgs p f) <==> P.live h p) [SMTPat (P.live h (gfield tgs p f))]
...
val modifies_1_valid: 
  ...
  Lemma
    (requires
      (valid h0 tgs p /\ field_matches_tag tgs f (gread_tag h0 tgs p) /\
        P.modifies_1 (gfield tgs p f) h0 h1 /\ P.includes (gfield tgs p f) p' /\ P.readable h1 p'))
    (ensures (valid h1 tgs p))
    [SMTPat (valid #l h0 tgs p); SMTPat (P.readable h1 p'); SMTPat (gfield #l tgs p f)]
...
val modifies_1_field_tag: 
  ...
  h0: HS.mem ->
  h1: HS.mem ->
  Lemma (requires (valid h0 tgs p /\ P.modifies_1 (gfield tgs p f) h0 h1))
...
val readable_intro: 
  ...
  f: P.struct_field l ->
  h: HS.mem ->
  Lemma (requires (valid h tgs p /\ P.readable h (gfield tgs p f)))
...
val readable_field: 
  ...
  f: P.struct_field l ->
  h: HS.mem ->
  Lemma (requires (valid h tgs p /\ P.readable h p /\ field_matches_tag tgs f (gread_tag h tgs p)))
...
let matching_tags (#l: P.union_typ) (raw_tu: raw l) (tgs: tags l) : Tot Type =
  let t = raw_get_tag raw_tu in
  List.Tot.mem t tgs /\ field_of_tag tgs t == raw_get_field raw_tu


let t (l: P.union_typ) (tgs: tags l) : Tot Type0 = tu: raw l {matching_tags tu tgs}

let get_field (#l: P.union_typ) (#tgs: tags l) (tu: t l tgs) : GTot (P.struct_field l) =
  raw_get_field tu
...
let get_value (#l: P.union_typ) (#tgs: tags l) (tu: t l tgs) (f: P.struct_field l)
  : Pure (P.type_of_typ (P.typ_of_struct_field l f))
    (requires (get_field tu == f))
    (ensures (fun _ -> True)) = raw_get_value #l tu f

(* Lemma: "valid p ==> matching_tags (gread p)" *)

val valid_matching_tags: #l: P.union_typ -> h: HS.mem -> tgs: tags l -> p: P.pointer (typ l) ->
