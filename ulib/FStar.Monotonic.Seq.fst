...
let alloc_mref_iseq (#a: Type) (p: (seq a -> Type)) (r: rid) (init: seq a {p init})
  : ST (i_seq r a p)
    (requires (fun _ -> HST.witnessed (region_contains_pred r)))
    (ensures (fun h0 m h1 -> HST.ralloc_post r init h0 m h1)) = ralloc r init
...
let i_sel (#r: rid) (#a: Type) (#p: (seq a -> Type)) (h: mem) (m: i_seq r a p)
  : GTot (s: seq a {p s}) = HS.sel h m

let i_read (#a: Type) (#p: (Seq.seq a -> Type)) (#r: rid) (m: i_seq r a p)
  : ST (s: seq a {p s})
    (requires (fun h -> True))
    (ensures (fun h0 x h1 -> h0 == h1 /\ x == i_sel h0 m)) = !m
...
private
val test0: r: rid -> a: m_rref r (seq nat) grows -> k: nat ->
  ST unit (requires (fun h -> k < Seq.length (HS.sel h a))) (ensures (fun h0 result h1 -> True))
let test0 r a k =
  let h0 = HST.get () in
  let _ =
...
private
val itest: r: rid -> a: i_seq r nat invariant -> k: nat ->
  ST unit (requires (fun h -> k < Seq.length (i_sel h a))) (ensures (fun h0 result h1 -> True))
let itest r a k =
  let h0 = HST.get () in
...
val map_snoc: f: ('a -> Tot 'b) -> s: seq 'a -> a: 'a ->
  Lemma (map f (Seq.snoc s a) == Seq.snoc (map f s) (f a))
let map_snoc f s a =
...
val map_append: f: ('a -> Tot 'b) -> s1: seq 'a -> s2: seq 'a ->
  Lemma (requires True)
...
val map_index: f: ('a -> Tot 'b) -> s: seq 'a -> i: nat{i < Seq.length s} ->
  Lemma (requires True)
...
let map_grows (f: ('a -> Tot 'b)) (s1: seq 'a) (s3: seq 'a)
  : Lemma (grows s1 s3 ==> grows (map f s1) (map f s3)) = ()

let map_prefix
  (#a: Type) (#b: Type) (#i: rid) (r: m_rref i (seq a) grows) (f: (a -> Tot b)) (bs: seq b) (h: mem) =
  grows bs (map f (HS.sel h r))

//17-01-05  this applies to log_t's defined below. 
let map_prefix_stable
  (#a: Type) (#b: Type) (#i: rid) (r: m_rref i (seq a) grows) (f: (a -> Tot b)) (bs: seq b)
  : Lemma (stable_on_t r (map_prefix r f bs)) = ()

let map_has_at_index
  (#a: Type) (#b: Type) (#i: rid) (r: m_rref i (seq a) grows) (f: (a -> Tot b)) (n: nat) (v: b)
  (h: mem) =
  let s = HS.sel h r in
...
val collect_snoc: f: ('a -> Tot (seq 'b)) -> s: seq 'a -> a: 'a ->
  Lemma (collect f (Seq.snoc s a) == Seq.append (collect f s) (f a))
...
let collect_grows (f: ('a -> Tot (seq 'b))) (s1: seq 'a) (s2: seq 'a)
...
let collect_prefix
  (#a: Type) (#b: Type) (#i: rid) (r: m_rref i (seq a) grows) (f: (a -> Tot (seq b))) (bs: seq b)
  (h: mem) = grows bs (collect f (HS.sel h r))
...
let collect_has_at_index
  (#a: Type) (#b: Type) (#i: rid) (r: m_rref i (seq a) grows) (f: (a -> Tot (seq b))) (n: nat)
  (v: b) (h: mem) =
  let s = HS.sel h r in
...
type seqn_val (#l: rid) (#a: Type) (i: rid) (log: log_t l a) (max: nat) =
  //never more than the length of the log
  (x: nat{x <= max /\ witnessed (at_most_log_len x log)})
...
let new_seqn (#a: Type) (#l: rid) (#max: nat) (i: rid) (init: nat) (log: log_t l a)
  : ST (seqn i log max)
    (requires
      (fun h ->
          HST.witnessed (region_contains_pred i) /\ init <= max /\ init <= Seq.length (HS.sel h log)
      ))
    (//17-01-05 unify with ralloc_post? 
      ensures
      (fun h0 c h1 ->
          modifies_one i h0 h1 /\ modifies_ref i Set.empty h0 h1 /\ fresh_ref c h0 h1 /\
          HS.sel h1 c = init /\ FStar.Map.contains h1.h i)) =
  recall log;
  recall_region i;
  mr_witness log (at_most_log_len init log);
  ralloc i init

let increment_seqn (#a: Type) (#l: rid) (#max: nat) (#i: rid) (#log: log_t l a) ($c: seqn i log max)
  : ST unit
    (requires
      (fun h ->
...
//(p:(nat -> Type))
private
let test (i: rid) (l: rid) (a: Type0) (log: log_t l a) (r: seqn i log 8) (h: mem) =
