...
let pure_ite_wp (a: Type) (wp: pure_wp a) : pure_wp a =
  fun (post:pure_post a) -> forall (k: pure_post a). (forall (x: a). {:pattern (guard_free (k x))} post x ==> k x) ==> wp k
...
unfold
let pure_close_wp (a b: Type) (wp: (b -> GTot (pure_wp a))) : pure_wp a =
...
let pure_assert_wp (p: Type) : pure_wp unit =
  fun (post:pure_post unit) -> p /\ post ()
