...
assume private
type get : #world: Type -> (world -> Type0) -> Type0 
assume private
type set : #world: Type -> Type0 -> world -> Type0 

(* Weakening for the get operator *)
assume
val get_weakening: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
...
assume
val get_set_axiom: #world: Type -> p: Type0 ->
  Lemma (get (set #world p) <==> p) [SMTPat (get (set #world p))]
assume
val set_get_axiom: #world: Type -> w: world -> p: (world -> Type0) ->
  Lemma (set (get p) w <==> set (p w) w) [SMTPat (set (get p) w)]
assume
val set_set_axiom: #world: Type -> w: world -> w': world -> p: Type0 ->
  Lemma (set (set p w') w <==> set p w') [SMTPat (set (set p w') w)]

(* Useful derivable get lemma *)
private
val get_constant_lemma: world: Type -> p: Type0 ->
  Lemma (get #world (fun _ -> p) <==> p) [SMTPat (get #world (fun _ -> p))]
...
val get_true: world: Type ->
  Lemma (get #world (fun _ -> True)) [SMTPat (get #world (fun _ -> True))]
let get_true world = ()
assume private
val set_true: #world: Type -> w: world -> Lemma (set True w) [SMTPat (set True w)]
private
...
let get_false world = ()
assume
val set_false: #world: Type -> w: world ->
  Lemma (requires (set False w)) (ensures (False)) [SMTPat (set False w)]
private
val get_and_1: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
  Lemma (requires (get (fun w -> p w /\ q w)))
    (ensures (get p /\ get q))
    [SMTPat (get (fun w -> p w /\ q w))]
let get_and_1 #world p q = ()
assume private
val set_and_1: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set (p /\ q) w)) (ensures (set p w /\ set q w)) [SMTPat (set (p /\ q) w)]
assume private
val get_and_2: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
  Lemma (requires (get p /\ get q))
    (ensures (get (fun w -> p w /\ q w)))
    [SMTPat (get (fun w -> p w /\ q w))]
assume private
val set_and_2: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set p w /\ set q w)) (ensures (set (p /\ q) w)) [SMTPat (set (p /\ q) w)]
private
val get_or_1: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
  Lemma (requires (get p \/ get q))
    (ensures (get (fun w -> p w \/ q w)))
    [SMTPat (get (fun w -> p w \/ q w))]
let get_or_1 #world p q = ()
assume private
val set_or_1: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set p w \/ set q w)) (ensures (set (p \/ q) w)) [SMTPat (set (p \/ q) w)]
assume private
val get_or_2: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
  Lemma (requires (get (fun w -> p w \/ q w)))
    (ensures (get p \/ get q))
    [SMTPat (get (fun w -> p w \/ q w))]
assume private
val set_or_2: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set (p \/ q) w)) (ensures (set p w \/ set q w)) [SMTPat (set (p \/ q) w)]
private
val get_impl_1: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
...
assume private
val set_impl_1: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set (p ==> q) w /\ set p w)) (ensures (set q w)) [SMTPat (set (p ==> q) w)]
assume private
val get_impl_2: #world: Type -> p: (world -> Type0) -> q: (world -> Type0) ->
  Lemma (requires (get p ==> get q))
    (ensures (get (fun w -> p w ==> q w)))
    [SMTPat (get (fun w -> p w ==> q w))]
assume private
val set_impl_2: #world: Type -> w: world -> p: Type0 -> q: Type0 ->
  Lemma (requires (set p w ==> set q w)) (ensures (set (p ==> q) w)) [SMTPat (set (p ==> q) w)]
private
val get_forall_1_aux: #world: Type -> #t: Type -> p: (t -> world -> Type0) -> x: t ->
  Lemma (requires (get (fun w -> forall x. p x w)))
    (ensures (get (fun w -> p x w)))
    [SMTPat (get (fun w -> p x w)); SMTPat (get (fun w -> forall (x: t). p x w))]
let get_forall_1_aux #world #t p x = get_weakening (fun w -> forall x. p x w) (fun w -> p x w)
private
val get_forall_1: #world: Type -> #t: Type -> p: (t -> world -> Type0) ->
  Lemma (requires (get (fun w -> forall x. p x w)))
    (ensures (forall x. get (fun w -> p x w)))
    [SMTPat (get (fun w -> forall (x: t). p x w))]
let get_forall_1 #world #t p = ()
assume private
val set_forall_1: #world: Type -> #t: Type -> w: world -> p: (t -> Type0) ->
  Lemma (requires (set (forall x. p x) w))
    (ensures (forall x. set (p x) w))
    [SMTPat (set (forall x. p x) w)]
assume private
val get_forall_2: #world: Type -> #t: Type -> p: (t -> world -> Type0) ->
  Lemma (requires (forall x. get (fun w -> p x w)))
    (ensures (get (fun w -> forall x. p x w)))
    [SMTPat (get (fun w -> forall x. p x w))]
assume private
val set_forall_2: #world: Type -> #t: Type -> w: world -> p: (t -> Type0) ->
  Lemma (requires (forall x. set (p x) w))
    (ensures (set (forall x. p x) w))
    [SMTPat (set (forall x. p x) w)]
private
val get_exists_1_aux: #world: Type -> #t: Type -> p: (t -> world -> Type0) -> x: t ->
  Lemma (requires (get (fun w -> p x w)))
    (ensures (get (fun w -> exists x. p x w)))
    [SMTPat (get (fun w -> p x w)); SMTPat (get (fun w -> exists x. p x w))]
let get_exists_1_aux #world #t p x = get_weakening (fun w -> p x w) (fun w -> exists x. p x w)
private
val get_exists_1: #world: Type -> #t: Type -> p: (t -> world -> Type0) ->
  Lemma (requires (exists x. get (fun w -> p x w)))
    (ensures (get (fun w -> exists x. p x w)))
    [SMTPat (get (fun w -> exists x. p x w))]
let get_exists_1 #world #t p = ()
assume private
val set_exists_1: #world: Type -> #t: Type -> w: world -> p: (t -> Type0) ->
  Lemma (requires (exists x. set (p x) w))
    (ensures (set (exists x. p x) w))
    [SMTPat (set (exists x. p x) w)]
assume private
val get_exists_2: #world: Type -> #t: Type -> p: (t -> world -> Type0) ->
  Lemma (requires (get (fun w -> exists x. p x w)))
    (ensures (exists x. get (fun w -> p x w)))
    [SMTPat (get (fun w -> exists x. p x w))]
assume private
val set_exists_2: #world: Type -> #t: Type -> w: world -> p: (t -> Type0) ->
  Lemma (requires (set (exists x. p x) w))
    (ensures (exists x. set (p x) w))
    [SMTPat (set (exists x. p x) w)]
private
val get_eq: #world: Type -> #t: Type -> v: t -> v': t ->
  Lemma (get #world (fun _ -> v == v') <==> v == v') [SMTPat (get #world (fun _ -> v == v'))]
let get_eq #world #t v v' = get_constant_lemma world (v == v')
assume private
val set_eq: #world: Type -> #t: Type -> w: world -> v: t -> v': t ->
  Lemma (set (v == v') w <==> v == v') [SMTPat (set (v == v') w)]
...
let lemma_witnessed_and #state rel p q =
  let aux ()
    : Lemma (requires (witnessed rel p /\ witnessed rel q))
...
let lemma_witnessed_impl #state rel p q =
  let aux ()
    : Lemma (requires ((witnessed rel (fun s -> p s ==> q s) /\ witnessed rel p)))
      (ensures (witnessed rel q)) =
    get_and_2 (fun s -> forall s'. rel s s' ==> p s' ==> q s')
...
let lemma_witnessed_forall #state #t rel p =
  let aux ()
    : Lemma (requires (forall x. witnessed rel (fun s -> p x s)))
...
val witnessed_defs_equiv_1: #state: Type -> rel: preorder state -> p: (state -> Type0) ->
  Lemma (requires (witnessed #state rel p)) (ensures (witnessed #state rel p))
let witnessed_defs_equiv_1 #state rel p = ()

val witnessed_defs_equiv_2: #state: Type -> rel: preorder state -> p: (state -> Type0) ->
  Lemma (requires (witnessed #state rel p)) (ensures (witnessed #state rel p))
let witnessed_defs_equiv_2 #state rel p =
  get_weakening #state
    (fun s -> exists s'. rel s' s /\ (forall s''. rel s' s'' ==> p s''))
    (fun s -> forall s'. rel s s' ==> p s')
