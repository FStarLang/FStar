...
val bind_squash: #a: Type -> #b: Type -> squash a -> (a -> GTot (squash b)) -> Tot (squash b)

(* With a special ``push'' operation *)
val push_squash: #a: Type -> #b: (a -> Type) -> (x: a -> Tot (squash (b x))) ->
  Tot (squash (x: a -> Tot (b x)))

val get_proof: p: Type -> Pure (squash p) (requires p) (ensures (fun _ -> True))

val give_proof: #p: Type -> squash p -> Pure unit (requires True) (ensures (fun _ -> p))

val proof_irrelevance: p: Type -> x: squash p -> y: squash p -> Tot (squash (x == y))

val squash_double_arrow: 
  #a: Type ->
  #p: (a -> Type) ->
...
val map_squash: #a: Type -> #b: Type -> squash a -> (a -> GTot b) -> Tot (squash b)
