...
let and_elim' (h: binder) : Tac unit = and_elim (pack (Tv_Var (bv_of_binder h))); clear h
...
let print_binder (b: binder) : Tac unit = print (term_to_string (type_of_binder b))
...
let bind (#a: Type) (#b: Type) (f: match_res a) (g: (a -> Tac (match_res b))) : Tac (match_res b) =
  match f with
  | Success aa -> g aa
...
let rec interp_pattern_aux (pat: pattern) (cur_bindings: bindings) (tm: term)
  ...
  admit ();
  let interp_any () cur_bindings tm = return [] in
...
noeq
type matching_solution = { ms_vars:list (varname * term); ms_hyps:list (varname * hypothesis) }
...
let string_of_matching_solution ms =
  ...
  let hyps =
    String.concat "
        "
      (map (fun (nm, binder) -> nm ^ ": " ^ (binder_to_string binder)) ms.ms_hyps)
  in
  "
{ vars: " ^ vars ^ "
" ^ "  hyps: " ^ hyps ^ " }"
...
let ms_locate_hyp (a: Type) (solution: matching_solution) (name: varname) : Tac binder =
  assoc_varname_fail name solution.ms_hyps

let ms_locate_var (a: Type) (solution: matching_solution) (name: varname) : Tac a =
...
let rec solve_mp_for_single_hyp
  #a (name: varname) (pat: pattern) (hypotheses: list hypothesis)
  (body: (matching_solution -> Tac a)) (part_sol: matching_solution)
  : Tac a =
  match hypotheses with
  | [] -> fail #a "No matching hypothesis"
  | h :: hs ->
    // Must be in ``Tac`` here to run `body`
    or_else (fun () ->
...
let rec solve_mp_for_hyps
  #a (mp_hyps: list (varname * pattern)) (hypotheses: list hypothesis)
  (body: (matching_solution -> Tac a)) (partial_solution: matching_solution)
  : Tac a =
  match mp_hyps with
...
let solve_mp
  #a (problem: matching_problem) (hypotheses: binders) (goal: term)
...
let rec pattern_of_term_ex tm : Tac (match_res pattern) =
  match inspect tm with
  | Tv_Var bv -> return (PVar (name_of_bv bv))
  | Tv_FVar fv ->
    let qn = fv_to_string fv in
    return (if qn = any_qn then PAny else PQn qn)
  | Tv_Type () -> return PType
  | Tv_App f (x, _) ->
    let is_any =
      match inspect f with
      | Tv_FVar fv -> fv_to_string fv = any_qn
      | _ -> false
    in
    if is_any
    then return PAny
    else (fpat <-- pattern_of_term_ex f ; xpat <-- pattern_of_term_ex x ; return (PApp fpat xpat))
...
noeq
type abspat_argspec = { asa_name:absvar; asa_kind:abspat_binder_kind }

// We must store this continuation, because recomputing it yields different
// names when the binders are re-opened.
type abspat_continuation = list abspat_argspec * term
...
let matching_problem_of_abs (tm: term) : Tac (matching_problem * abspat_continuation) =
  ...
  debug ("Got binders: " ^
      (String.concat ", " (map (fun b -> name_of_binder b <: Tac string) binders)));
  ...
  let continuation =
    let abspat_argspec_of_binder xx : Tac abspat_argspec =
      match xx with | binder, xx, binder_kind, yy -> { asa_name = binder; asa_kind = binder_kind }
    in
...
let abspat_arg_of_abspat_argspec solution_term (argspec: abspat_argspec) : Tac term =
  ...
  let locate_args =
    [
      (arg_type_of_binder_kind argspec.asa_kind, Q_Explicit);
...
let specialize_abspat_continuation' (continuation: abspat_continuation) (solution_term: term)
  : Tac term =
  let mk_arg argspec = (abspat_arg_of_abspat_argspec solution_term argspec, Q_Explicit) in
...
bindings. **)
let specialize_abspat_continuation (continuation: abspat_continuation) : Tac term =
...
let match_abspat #b #a (abspat: a) (k: (abspat_continuation -> Tac (matching_solution -> Tac b)))
  ...
  //NS: imprecision in the encoding of the impure result function type
  admit ();
...
let fetch_eq_side' #a : Tac (term * term) =
  gpm (fun (left: a) (right: a) (g: goal (squash (left == right))) -> ((quote left), (quote right)))
    ()
...
let _ =
  assert_by_tactic (1 + 1 == 2)
    (gpm (fun (left: int) (right: int) (g: goal (squash (left == right))) ->
            let l, r = (quote left), (quote right) in
            print (term_to_string l ^ " / " ^ term_to_string r) <: Tac unit))
...
let example #a #b #c : unit =
  assert_by_tactic (a /\ b ==> c == b ==> c)
    (fun () ->
        repeat' (fun () ->
              or_else (gpm #unit (fun (a: Type) (h: hyp (squash a)) -> clear h <: Tac unit))
                (fun () ->
                    or_else (gpm #unit
                          (fun (a: Type0) (b: Type0) (g: goal (squash (a ==> b))) ->
                              implies_intro' () <: Tac unit))
                      (fun () ->
                          or_else (gpm #unit
                                (fun (a: Type0) (b: Type0) (h: hyp (a /\ b)) ->
                                    and_elim' h <: Tac unit))
                            (fun () ->
                                or_else (gpm #unit
                                      (fun
                                  ...
                                  (fun () ->
                                      gpm #unit
                                        (fun (a: Type0) (h: hyp a) (g: goal (squash a)) ->
...
let example2 #a #b #c : unit =
  assert_by_tactic (a /\ b ==> c == b ==> c)
    (fun () ->
        first #unit
          [
            gpm (fun (a: Type) (h: hyp (squash a)) -> clear h <: Tac unit);
            gpm (fun (a: Type0) (b: Type0) (_: goal (squash (a ==> b))) ->
                  implies_intro' () <: Tac unit);
            gpm (fun (a: Type0) (b: Type0) (h: hyp (a /\ b)) -> and_elim' h <: Tac unit);
            gpm (fun (a: Type0) (b: Type0) (h: hyp (a == b)) (_: goal (squash a)) ->
                  rewrite h <: Tac unit);
            gpm (fun (a: Type0) (h: hyp a) (_: goal (squash a)) -> exact_hyp a h <: Tac unit);
            idtac
          ];
        smt ();
        qed ())
