...
/// signedness/width `s`.
val secret_int: #sl: sl -> l: lattice_element sl -> s: sw -> Type0

/// A `secret_int l s` can be seen as an int in spec
val reveal: #sl: sl -> #l: lattice_element sl -> #s: sw -> x: secret_int l s ->
...
val hide: #sl: sl -> #l: lattice_element sl -> #s: sw -> x: int{within_bounds s x} ->
  GTot (secret_int l s)

val reveal_hide: #sl: sl -> #l: lattice_element sl -> #s: sw -> x: int{within_bounds s x} ->
  Lemma (reveal (hide #sl #l #s x) == x)

val hide_reveal: #sl: sl -> #l: lattice_element sl -> #s: sw -> x: secret_int l s ->
...
val promote: 
  ...
  Tot (y: secret_int (lub l1 l0) s {reveal y == reveal x})
...
noeq
type qual =
  | Secret : #sl: sl -> l: lattice_element sl -> sw: sw -> qual
  | Public : sw: signed_width -> qual
...
[@ mark_for_norm]
unfold
let op_Plus (#q: qual) (x: t q) (y: t q {ok ( + ) (i x) (i y)}) : Tot (t q) =
  match q with
  | Public s -> as_public x + as_public y
  | Secret l s -> addition (as_secret x) (as_secret y)

[@ mark_for_norm]
unfold
let op_Plus_Percent
  (#q: qual{norm (Unsigned? (q2s q) /\ width_of_sw (q2s q) <> W128)}) (x: t q) (y: t q)
  : Tot (t q) =
  match q with
  | Public s -> as_public x +% as_public y
  | Secret l s -> addition_mod (as_secret x) (as_secret y)
