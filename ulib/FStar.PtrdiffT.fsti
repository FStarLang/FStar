...
/// a ptrdiff_t value. Any value that fits in a uint_16 can be
/// cast directly to ptrdiff_t
/// Any value that might not fit in a uint_16 needs to be checked,
/// we will add a static_assert during extraction
val mk (x: I16.t) : Pure ptrdiff_t
  (requires True)
  (ensures (fun y -> v y == I16.v x))

val mk_checked (x: I64.t) : Pure ptrdiff_t
  (requires True)
  (ensures (fun y -> v y == I64.v x))

let zero : (zero_ptrdiff: ptrdiff_t { v zero_ptrdiff == 0 }) =
  mk 0s
