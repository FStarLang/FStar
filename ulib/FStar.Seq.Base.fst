...
abstract
type seq (a: Type u#a) : Type u#a = | MkSeq : l: list a -> seq a
...
inline_for_extraction abstract
val init: #a: Type -> len: nat -> contents: (i: nat{i < len} -> Tot a) -> Tot (seq a)
inline_for_extraction abstract
let init #a len contents = if len = 0 then MkSeq [] else init_aux len 0 contents

abstract
val of_list: #a: Type -> list a -> Tot (seq a)
let of_list #a l = MkSeq l

abstract
val lemma_of_list_length: #a: Type -> s: seq a -> l: list a ->
  Lemma (requires (s == of_list #a l))
    (ensures (length s = List.length l))
    [SMTPat (length s = List.length l)]
let lemma_of_list_length #a s l = ()

abstract
val lemma_of_list: #a: Type -> s: seq a -> l: list a -> i: nat{i < length s} ->
  Lemma (requires (s == of_list l))
...
abstract
val upd: #a: Type -> s: seq a -> n: nat{n < length s} -> a -> Tot (seq a) (decreases (length s))
...
val slice: #a: Type -> s: seq a -> i: nat -> j: nat{i <= j && j <= length s} ->
  Tot (r : seq a) (decreases (length s))
let rec slice #a s i j =
  if i > 0
  then slice #a (tl s) (i - 1) (j - 1)
  else if j = 0 then MkSeq [] else cons (hd s) (slice #a (tl s) i (j - 1))

(* Lemmas about length *)
abstract
val lemma_create_len: #a: Type -> n: nat -> i: a ->
  Lemma (requires True) (ensures (length (create n i) = n)) [SMTPat (length (create n i))]
let rec lemma_create_len #a n i = if n = 0 then () else lemma_create_len #a (n - 1) i

abstract
val lemma_init_len: #a: Type -> n: nat -> contents: (i: nat{i < n} -> Tot a) ->
  Lemma (requires True) (ensures (length (init n contents) = n)) [SMTPat (length (init n contents))]
...
abstract
val lemma_len_upd: #a: Type -> n: nat -> v: a -> s: seq a {n < length s} ->
  Lemma (requires True) (ensures (length (upd s n v) = length s)) [SMTPat (length (upd s n v))]
let rec lemma_len_upd #a n v s = if n = 0 then () else lemma_len_upd #a (n - 1) v (tl s)

abstract
val lemma_len_append: #a: Type -> s1: seq a -> s2: seq a ->
  Lemma (requires True)
...
let rec lemma_len_slice #a s i j =
  ...
  else if j = 0 then () else lemma_len_slice #a (tl s) i (j - 1)

(* Lemmas about index *)
abstract
val lemma_index_create: #a: Type -> n: nat -> v: a -> i: nat{i < n} ->
  Lemma (requires True) (ensures (index (create n v) i == v)) [SMTPat (index (create n v) i)]
let rec lemma_index_create #a n v i =
  if n = 0 then () else if i = 0 then () else lemma_index_create #a (n - 1) v (i - 1)

abstract
val lemma_index_upd1: #a: Type -> s: seq a -> n: nat{n < length s} -> v: a ->
  Lemma (requires True)
...
let rec lemma_index_upd2 #a s n v i =
  match (MkSeq?.l s) with
  | [] -> ()
  | hd :: tl ->
    if i = 0 then () else if n = 0 then () else lemma_index_upd2 #a (MkSeq tl) (n - 1) v (i - 1)

abstract
val lemma_index_app1: #a: Type -> s1: seq a -> s2: seq a -> i: nat{i < length s1} ->
  Lemma (requires True)
...
let rec lemma_index_app1 #a s1 s2 i =
  ...
  | [] -> ()
  | hd :: tl -> if i = 0 then () else lemma_index_app1 #a (MkSeq tl) s2 (i - 1)

abstract
val lemma_index_app2: 
  #a: Type ->
  s1: seq a ->
  s2: seq a ->
  ...
  Lemma (requires True)
    ...
    (decreases (length s1))
    [SMTPat (index (append s1 s2) i)]
let rec lemma_index_app2 #a s1 s2 i =
  match s1.l with
  | [] -> ()
...
let rec lemma_index_slice #a s i j k =
  if i > 0
  then lemma_index_slice #a (tl s) (i - 1) (j - 1) k
  else if j = 0 then () else if k = 0 then () else lemma_index_slice #a (tl s) i (j - 1) (k - 1)

val hasEq_lemma: a: Type ->
  Lemma (requires (hasEq a)) (ensures (hasEq (seq a))) [SMTPat (hasEq (seq a))]
let hasEq_lemma a = ()
...
(* decidable equality *)
private
val eq_i: #a: eqtype -> s1: seq a -> s2: seq a {length s1 = length s2} -> i: nat{i <= length s1} ->
...
abstract
val lemma_eq_intro: #a: Type -> s1: seq a -> s2: seq a ->
  Lemma
    (requires
      (length s1 = length s2 /\
        (forall (i: nat{i < length s1}). {:pattern (index s1 i); (index s2 i)}
            (index s1 i == index s2 i)))) (ensures (equal s1 s2)) [SMTPat (equal s1 s2)]
let lemma_eq_intro #a s1 s2 = ()

abstract
val lemma_eq_refl: #a: Type -> s1: seq a -> s2: seq a ->
  Lemma (requires (s1 == s2)) (ensures (equal s1 s2)) [SMTPat (equal s1 s2)]
let lemma_eq_refl #a s1 s2 = ()

abstract
val lemma_eq_elim: #a: Type -> s1: seq a -> s2: seq a ->
...
abstract
let append_assoc (#a: Type) (s1: seq a) (s2: seq a) (s3: seq a)
  : Lemma (ensures (append (append s1 s2) s3 == append s1 (append s2 s3))) =
  List.append_assoc (MkSeq?.l s1) (MkSeq?.l s2) (MkSeq?.l s3)

abstract
let append_empty_l (#a: Type) (s: seq a) : Lemma (ensures (append createEmpty s == s)) =
  List.append_nil_l (MkSeq?.l s)

abstract
let append_empty_r (#a: Type) (s: seq a) : Lemma (ensures (append s createEmpty == s)) =
...
let rec init_index_aux
  (#a: Type) (len: nat) (k: nat{k < len}) (contents: (i: nat{i < len} -> Tot a))
  : Lemma (requires True)
...
let init_index #a len contents = if len = 0 then () else init_index_aux #a len 0 contents
