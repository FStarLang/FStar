...
(*
 * Shorthand for erased sequence
 *)
type eseq (a:Type) = G.erased (Seq.seq a)

let cpred (#a:Type0) (s:eseq a) :spred a = fun s1 -> Seq.equal (G.reveal s) s1

let value_is #a (b:ibuffer a) (s:eseq a) =
  witnessed b (cpred s)

unfold let libuffer (a:Type0) (len:nat) (s:eseq a) =
  b:lmbuffer a (immutable_preorder a) (immutable_preorder a) len{b `value_is` s}

unfold let libuffer_or_null (a:Type0) (len:nat) (r:HS.rid) (s:eseq a) =
...
let igcmalloc (#a:Type0) (r:HS.rid) (init:a) (len:U32.t)
  :HST.ST (b:libuffer a (U32.v len) (G.hide (Seq.create (U32.v len) init)){frameOf b == r /\ recallable b})
  ...
  = let b = mgcmalloc r init len in
    witness_p b (cpred (G.hide (Seq.create (U32.v len) init)));
    b
...
let igcmalloc_and_blit (#a:Type0) (r:HS.rid)
  ...
  = let b = mgcmalloc_and_blit r src id_src len in
    let h0 = HST.get () in
    witness_p b (cpred (G.hide (Seq.slice (as_seq h0 src) (U32.v id_src) (U32.v id_src + U32.v len))));
    b

inline_for_extraction
let igcmalloc_partial (#a:Type0) (r:HS.rid) (init:a) (len:U32.t)
  :HST.ST (b:libuffer_or_null a (U32.v len) r (G.hide (Seq.create (U32.v len) init)){recallable b})
          (requires (fun _       -> malloc_pre r len))
...
let imalloc (#a:Type0) (r:HS.rid) (init:a) (len:U32.t)
  :HST.ST (b:libuffer a (U32.v len) (G.hide (Seq.create (U32.v len) init)){frameOf b == r /\ freeable b})
          (requires (fun _       -> malloc_pre r len))
          (ensures  (fun h0 b h1 -> alloc_post_mem_common b h0 h1 (Seq.create (U32.v len) init)))
...
let imalloc_partial (#a:Type0) (r:HS.rid) (init:a) (len:U32.t)
  :HST.ST (b:libuffer_or_null a (U32.v len) r (G.hide (Seq.create (U32.v len) init)){(not (g_is_null b)) ==> freeable b})
          (requires (fun _       -> malloc_pre r len))
          (ensures  (fun h0 b h1 -> alloc_partial_post_mem_common b h0 h1 (Seq.create (U32.v len) init)))
  = imalloc r init len

let ialloca (#a:Type0) (init:a) (len:U32.t)
  :HST.StackInline (libuffer a (U32.v len) (G.hide (Seq.create (U32.v len) init)))
...
let ialloca_of_list (#a:Type0) (init: list a)
  :HST.StackInline (b:libuffer a (normalize_term (List.Tot.length init)) (G.hide (Seq.seq_of_list init)))
  ...
  = let b = malloca_of_list init in
    witness_p b (cpred (G.hide (Seq.seq_of_list init)));
    b

let igcmalloc_of_list (#a:Type0) (r:HS.rid) (init:list a)
  :HST.ST (b:libuffer a (normalize_term (List.Tot.length init)) (G.hide (Seq.seq_of_list init)){frameOf b == r /\ recallable b})
...
let igcmalloc_of_list_partial (#a:Type0) (r:HS.rid) (init:list a)
  :HST.ST (b:libuffer_or_null a (normalize_term (List.Tot.length init)) r (G.hide (Seq.seq_of_list init)){recallable b})
          (requires (fun _       -> gcmalloc_of_list_pre r init))
...
let witness_contents (#a:Type0) (b:ibuffer a) (s:Seq.seq a)
  :HST.ST unit (requires (fun h0        -> Seq.equal (as_seq h0 b) s))
                 (ensures  (fun h0 _ h1 -> h0 == h1 /\ b `value_is` G.hide s))
  = witness_p b (cpred (G.hide s))

let recall_contents (#a:Type0) (b:ibuffer a) (s:Seq.seq a)
  :HST.ST unit (requires (fun h0      -> (recallable b \/ live h0 b) /\ b `value_is` G.hide s))
               (ensures  (fun h0 _ h1 -> h0 == h1 /\ live h0 b /\ as_seq h0 b == s))
  = recall_p b (cpred (G.hide s))
...
let recall_value (#a:Type0) (b:ibuffer a) (s:eseq a)
