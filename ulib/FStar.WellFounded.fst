...
let apply (#a: Type) (#b: (a -> Type)) (f: (x: a -> Tot (b x))) (x: a)
  : Pure (b x) (requires True) (ensures (fun r -> r == f x /\ f x << f)) = axiom1_dep #a #b f x; f x
...
let rec fix_F #aa #r #p f x a =
  f x (fun y h -> axiom1_dep (acc_inv x a) y; axiom1 (acc_inv x a y) h; fix_F f y (acc_inv x a y h))
