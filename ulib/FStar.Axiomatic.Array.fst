...
type seq : Type -> Type 
assume
val index: #a: Type -> seq a -> int -> Tot a
assume
val update: #a: Type -> seq a -> int -> a -> Tot (seq a)
assume
val emp: a: Type -> Tot (seq a)
assume
val create: #a: Type -> int -> a -> Tot (seq a)
assume
val length: #a: Type -> seq a -> Tot nat
assume
val slice: #a: Type -> seq a -> int -> int -> Tot (seq a)
assume
val append: #a: Type -> seq a -> seq a -> Tot (seq a)
assume
val proj_some: #a: Type -> seq (option a) -> Tot (seq a)
assume
...
assume IndexConst: forall (a: Type) (n: int) (v: a) (i: int). {:pattern (index (create n v) i)}
  (0 <= i /\ i < n) ==> index (create n v) i == v

assume LengthUpdate: forall (a: Type) (s: seq a) (i: int) (v: a). {:pattern (length (update s i v))}
  (0 <= i /\ i < length s) ==> length (update s i v) == length s

assume IndexUpdate: forall (a: Type) (s: seq a) (i: int) (v: a) (n: int).
...
assume LengthSlice: forall (a: Type) (s: seq a) (i: int) (j: int). {:pattern (length (slice s i j))}
  (0 <= i /\ i <= j /\ j <= length s) ==> j - i == length (slice s i j)

assume IndexSlice: forall (a: Type) (s: seq a) (i: int) (j: int) (k: int).
  {:pattern (index (slice s i j) k)}
  (0 <= k /\ k <= length (slice s i j)) ==> index (slice s i j) k == index s (i + k)

assume LengthAppend: forall (a: Type) (s1: seq a) (s2: seq a). {:pattern (length (append s1 s2))}
  length (append s1 s2) == length s1 + length s2

assume IndexAppend: forall (a: Type) (s1: seq a) (s2: seq a) (i: int).
  {:pattern (index (append s1 s2) i)}
  if (0 <= i && i < length s1)
...
assume SeqEquals: forall (a: Type) (s1: seq a) (s2: seq a). {:pattern (equal s1 s2)}
  ...
  (length s1 == length s2 /\
    (forall (i: int). {:pattern (index s1 i); (index s2 i)}
        (0 <= i /\ i < length s1) ==> index s1 i == index s2 i))
...
assume EmpConst: forall (a: Type) (s: seq a). {:pattern (length s)} length s == 0 ==> s == emp a

type is_Some_All (a: Type) (s: seq (option a)) =
  (forall (i: int). (0 <= i /\ i < length s) ==> Some? (index s i))
