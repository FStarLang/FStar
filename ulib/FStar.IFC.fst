...
abstract
let protected #sl (l: lattice_element sl) (b: Type) = b

/// At the specification level a `protected l b` is just a `b`
abstract
let reveal #sl (#l: lattice_element sl) #b (x: protected l b) : GTot b = x

/// And any `b` can be promoted to a `protected l b`
/// i.e., `protected l b` is only meant to enforce confidentiality
abstract
let hide #sl (#l: lattice_element sl) #b (x: b) : Tot (protected l b) = x

/// The next pair of lemmas show that reveal/hide are inverses
let reveal_hide #l #t #b (x: b) : Lemma (reveal (hide #l #t x) == x) [SMTPat (hide #l #t x)] = ()

let hide_reveal #sl (#l: lattice_element sl) #b (x: protected l b)
...
abstract
let map #a #b #sl (#l: lattice_element sl) (x: protected l a) (f: (y: a{y == reveal x} -> b))
  : Tot (y: protected l b {reveal y == f (reveal x)}) = f x
...
abstract
let join
  #sl (#l1: lattice_element sl) (#l2: lattice_element sl) #a (x: protected l1 (protected l2 a))
  : Tot (y: protected (lub l1 l2) a {reveal y == reveal (reveal x)}) =
  let y: a = x in
  y
...
unfold
let bind
  #sl (#l1: lattice_element sl) #a (x: protected l1 a) (#l2: lattice_element sl) #b
  (f: (y: a{y == reveal x} -> protected l2 b))
  : Tot (protected (lub l1 l2) b) = join (map x f)
