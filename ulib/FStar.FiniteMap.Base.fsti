...
type setfun_t (a: eqtype) (b: Type0) (s: FSet.set a) = f: (a ^-> option b){forall (key: a). FSet.mem key s <==> Some? (f key)}

val map (a: eqtype) (b: Type0)
  : Type0
...
val domain (#a: eqtype) (#b: Type0) (m: map a b)
  : FSet.set a
...
val elements (#a: eqtype) (#b: Type0) (m: map a b)
  : setfun_t a b (domain m)

/// We represent the Dafny operator `in` on maps with `mem`:

let mem (#a: eqtype) (#b: Type0) (key: a) (m: map a b) =
  FSet.mem key (domain m)

/// We can convert a map to a list of pairs with `map_as_list`:

let rec key_in_item_list (#a: eqtype) (#b: Type0) (key: a) (items: list (a * b)) : bool =
  match items with
  | [] -> false
  | (k, v) :: tl -> key = k || key_in_item_list key tl

let rec item_list_doesnt_repeat_keys (#a: eqtype) (#b: Type0) (items: list (a * b)) : bool =
  match items with
  | [] -> true
  | (k, v) :: tl -> not (key_in_item_list k tl) && item_list_doesnt_repeat_keys tl

val map_as_list (#a: eqtype) (#b: Type0) (m: map a b)
...
let lookup (#a: eqtype) (#b: Type0) (key: a) (m: map a b{mem key m})
  : b =
...
val cardinality (#a: eqtype) (#b: Type0) (m: map a b)
  : GTot nat
...
val values (#a: eqtype) (#b: Type0) (m: map a b)
  : GTot (b -> Type0)
...
val items (#a: eqtype) (#b: Type0) (m: map a b)
  : GTot ((a * b) -> Type0)
...
val glue (#a: eqtype) (#b: Type0) (keys: FSet.set a) (f: setfun_t a b keys)
  : map a b
...
val insert (#a: eqtype) (#b: Type0) (k: a) (v: b) (m: map a b)
  : map a b
...
val merge (#a: eqtype) (#b: Type0) (m1: map a b) (m2: map a b)
  : map a b
...
val subtract (#a: eqtype) (#b: Type0) (m: map a b) (s: FSet.set a)
  : map a b
...
val equal (#a: eqtype) (#b: Type0) (m1: map a b) (m2: map a b)
  : Type0
...
val disjoint (#a: eqtype) (#b: Type0) (m1: map a b) (m2: map a b)
  : Type0
...
val choose (#a: eqtype) (#b: Type0) (m: map a b{exists key. mem key m})
  : GTot (key: a{mem key m})

/// We add the utility functions `remove` and `notin`:

let remove (#a: eqtype) (#b: Type0) (key: a) (m: map a b)
  : map a b =
  subtract m (FSet.singleton key)

let notin (#a: eqtype) (#b: Type0) (key: a) (m: map a b)
  : bool =
  not (mem key m)
...
let cardinality_zero_iff_empty_fact =
  forall (a: eqtype) (b: Type0) (m: map a b).{:pattern cardinality m}
...
let empty_or_domain_occupied_fact =
  forall (a: eqtype) (b: Type0) (m: map a b).{:pattern domain m}
    m == emptymap \/ (exists k. mem k m)
...
let empty_or_values_occupied_fact =
  forall (a: eqtype) (b: Type0) (m: map a b).{:pattern values m}
    m == emptymap \/ (exists v. (values m) v)
...
let empty_or_items_occupied_fact =
  forall (a: eqtype) (b: Type0) (m: map a b).{:pattern items m}
    m == emptymap \/ (exists item. (items m) item)
...
let map_cardinality_matches_domain_fact =
  forall (a: eqtype) (b: Type0) (m: map a b).{:pattern FSet.cardinality (domain m)}
...
let values_contains_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (v: b).{:pattern (values m) v}
    (values m) v <==>
...
let items_contains_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (item: a * b).{:pattern (items m) item}
    (items m) item <==>
...
let empty_domain_empty_fact =
  forall (a: eqtype) (b: Type0) (u: a).{:pattern FSet.mem u (domain (emptymap #a #b))}
...
let glue_domain_fact =
  forall (a: eqtype) (b: Type0) (keys: FSet.set a) (f: setfun_t a b keys).{:pattern domain (glue keys f)}
...
let glue_elements_fact =
  forall (a: eqtype) (b: Type0) (keys: FSet.set a) (f: setfun_t a b keys).{:pattern elements (glue keys f)}
      domain (glue keys f) == keys
...
let insert_elements_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (key: a) (key': a) (value: b).
...
let insert_member_cardinality_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (key: a) (value: b).{:pattern cardinality (insert key value m)}
...
let merge_domain_is_union_fact =
  forall (a: eqtype) (b: Type0) (m1: map a b) (m2: map a b).{:pattern domain (merge m1 m2)}
...
let merge_element_fact =
  forall (a: eqtype) (b: Type0) (m1: map a b) (m2: map a b) (key: a).{:pattern (elements (merge m1 m2)) key}
...
let subtract_domain_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (s: FSet.set a).{:pattern domain (subtract m s)}
...
let subtract_element_fact =
  forall (a: eqtype) (b: Type0) (m: map a b) (s: FSet.set a) (key: a).{:pattern (elements (subtract m s)) key}
...
let map_equal_fact =
  forall (a: eqtype) (b: Type0) (m1: map a b) (m2: map a b).{:pattern equal m1 m2}
...
let disjoint_fact =
  forall (a: eqtype) (b: Type0) (m1: map a b) (m2: map a b).{:pattern disjoint m1 m2}
...
let all_finite_map_facts =
    cardinality_zero_iff_empty_fact
  /\ empty_or_domain_occupied_fact
  /\ empty_or_values_occupied_fact
  /\ empty_or_items_occupied_fact
  /\ map_cardinality_matches_domain_fact
  /\ values_contains_fact
  /\ items_contains_fact
  /\ empty_domain_empty_fact
  /\ glue_domain_fact
  /\ glue_elements_fact
  /\ insert_elements_fact
  /\ insert_member_cardinality_fact
  /\ insert_nonmember_cardinality_fact
  /\ merge_domain_is_union_fact
  /\ merge_element_fact
  /\ subtract_domain_fact
  /\ subtract_element_fact
  /\ map_equal_fact
  /\ map_extensionality_fact
  /\ disjoint_fact

val all_finite_map_facts_lemma : unit -> Lemma (all_finite_map_facts)
