...
let contains #a h s = FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains h s)
abstract
...
abstract
let addr_of (#a: Type) (arr: array a) : GTot nat = addr_of arr
...
abstract
val of_seq: #a: Type -> s: seq a ->
  ST (array a)
    (requires (fun h -> True))
    (ensures
      (fun h0 x h1 -> (unused_in x h0 /\ contains h1 x /\ modifies Set.empty h0 h1 /\ sel h1 x == s)
      ))
let of_seq #a s = ST.alloc s
abstract
val to_seq: #a: Type -> s: array a ->
  ST (seq a)
    (requires (fun h -> contains h s))
    (ensures (fun h0 x h1 -> (sel h0 s == x /\ h0 == h1)))
let to_seq #a s = !s
abstract
val create: #a: Type -> n: nat -> init: a ->
  ST (array a)
    (requires (fun h -> True))
    (ensures
      (fun h0 x h1 ->
          (unused_in x h0 /\ contains h1 x /\ modifies Set.empty h0 h1 /\
...
abstract
val index: #a: Type -> x: array a -> n: nat ->
  ST a
    (requires (fun h -> contains h x /\ n < Seq.length (sel h x)))
    (ensures (fun h0 v h1 -> (n < Seq.length (sel h0 x) /\ h0 == h1 /\ v == Seq.index (sel h0 x) n))
    )
let index #a x n =
  let s = to_seq x in
  Seq.index s n
abstract
val upd: #a: Type -> x: array a -> n: nat -> v: a ->
  ST unit
    (requires (fun h -> contains h x /\ n < Seq.length (sel h x)))
    (ensures
      (fun h0 u h1 ->
          (n < Seq.length (sel h0 x) /\ contains h1 x /\ modifies (Set.singleton (addr_of x)) h0 h1 /\
...
let length #a x =
  ...
  Seq.length s
abstract
val op: #a: Type -> f: (seq a -> Tot (seq a)) -> x: array a ->
  ST unit
    (requires (fun h -> contains h x))
...
val swap: #a: Type -> x: array a -> i: nat -> j: nat{i <= j} ->
  ST unit
    ...
    (ensures
      (fun h0 _u h1 ->
          (j < Seq.length (sel h0 x)) /\ contains h1 x /\ modifies (Set.singleton (addr_of x)) h0 h1 /\
...
val copy_aux: #a: Type -> s: array a -> cpy: array a -> ctr: nat ->
  ST unit
    (requires
      (fun h ->
          (contains h s /\ contains h cpy /\ addr_of s <> addr_of cpy) /\
          (Seq.length (sel h cpy) = Seq.length (sel h s)) /\ (ctr <= Seq.length (sel h cpy)) /\
          (forall (i: nat). i < ctr ==> Seq.index (sel h s) i == Seq.index (sel h cpy) i)))
    (ensures
      (fun h0 u h1 ->
          (contains h1 s /\ contains h1 cpy /\ addr_of s <> addr_of cpy) /\
          (modifies (only cpy) h0 h1) /\ (Seq.equal (sel h1 cpy) (sel h1 s))))
let rec copy_aux #a s cpy ctr =
  match length cpy - ctr with
  | 0 -> ()
  | _ -> upd cpy ctr (index s ctr); copy_aux s cpy (ctr + 1)

val copy: #a: Type -> s: array a ->
  ST (array a)
    (requires (fun h -> contains h s /\ Seq.length (sel h s) > 0))
    (ensures
      (fun h0 r h1 ->
          (modifies Set.empty h0 h1) /\ unused_in r h0 /\ (contains h1 r) /\
  ...
  ST unit
    (requires
      (fun h ->
          ...
          (Seq.length (sel h s) >= s_idx + len) /\ (Seq.length (sel h t) >= t_idx + len) /\
          (ctr <= len) /\
          (forall (i: nat).
              i < ctr ==> Seq.index (sel h s) (s_idx + i) == Seq.index (sel h t) (t_idx + i))))
    (ensures
      (fun h0 u h1 ->
          (contains h1 s /\ contains h1 t /\ addr_of s <> addr_of t) /\ (modifies (only t) h0 h1) /\
          (Seq.length (sel h1 s) >= s_idx + len) /\ (Seq.length (sel h1 t) >= t_idx + len) /\
...
let rec blit_aux #a s s_idx t t_idx len ctr =
  match len - ctr with
  | 0 -> ()
  | _ -> upd t (t_idx + ctr) (index s (s_idx + ctr)); blit_aux s s_idx t t_idx len (ctr + 1)
#set-options "--z3rlimit 5"
private
val blit: #a: Type -> s: array a -> s_idx: nat -> t: array a -> t_idx: nat -> len: nat ->
  ST unit
    (requires
      (fun h ->
          (contains h s) /\ (contains h t) /\ (addr_of s <> addr_of t) /\
    ...
    (ensures
      (fun h0 u h1 ->
          ...
          (Seq.length (sel h1 s) >= s_idx + len) /\ (Seq.length (sel h1 t) >= t_idx + len) /\
          (Seq.length (sel h0 s) = Seq.length (sel h1 s)) /\
          (Seq.length (sel h0 t) = Seq.length (sel h1 t)) /\ (modifies (only t) h0 h1) /\
          (forall (i: nat).
              i < len ==> Seq.index (sel h1 s) (s_idx + i) == Seq.index (sel h1 t) (t_idx + i)) /\
          (forall (i: nat). {:pattern (Seq.index (sel h1 t) i)}
              (i < Seq.length (sel h1 t) /\ (i < t_idx \/ i >= t_idx + len)) ==>
              (Seq.index (sel h1 t) i == Seq.index (sel h0 t) i))))
let rec blit #a s s_idx t t_idx len = blit_aux s s_idx t t_idx len 0

val sub: #a: Type -> s: array a -> idx: nat -> len: nat ->
  ST (array a)
    (requires
      (fun h -> (contains h s) /\ (Seq.length (sel h s) > 0) /\ (idx + len <= Seq.length (sel h s)))
    )
    (ensures
      (fun h0 t h1 ->
          (contains h1 t) /\ (contains h0 s) /\ unused_in t h0 /\ (modifies Set.empty h0 h1) /\
          (Seq.length (sel h0 s) > 0) /\ (idx + len <= Seq.length (sel h0 s)) /\
