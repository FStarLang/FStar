...
//
// This is the fully general reference type which supports both a monotonic relation
// and an data invariant.
//
// i(nvariant)m(onotonic)reference
noeq type imreference (a:Type) (inv:data_inv a) (rel:preorder a) =
| MkRef : id:rid -> ref:HH.mrref id a inv rel -> imreference a inv rel

// A compatibility layer, a mreference is just an imreference where the data invariant is trivial.
let mreference (a:Type) (rel:preorder a) = imreference a (FStar.Heap.trivial_invariant a) rel

let is_mm (#a:Type) (#inv:data_inv a) (#rel:preorder a) (r:imreference a inv rel) :GTot bool = HH.is_mm r.ref

//adding (not s.mm) to stackref and ref so as to keep their semantics as is
let imstackref (a:Type) (inv:data_inv a) (rel:preorder a) = s:imreference a inv rel{ is_stack_region s.id && not (is_mm s) }
let mstackref (a:Type) (rel:preorder a) = imstackref a (FStar.Heap.trivial_invariant a) rel

let imref (a:Type) (inv:data_inv a) (rel:preorder a) = s:imreference a inv rel{is_eternal_region s.id && not (is_mm s)}
let mref (a:Type) (rel:preorder a) = imref a (FStar.Heap.trivial_invariant a) rel

let immmstackref (a:Type) (inv:data_inv a) (rel:preorder a) = s:imreference a inv rel{is_stack_region s.id && is_mm s }
let mmmstackref (a:Type) (rel:preorder a) = immmstackref a (FStar.Heap.trivial_invariant a) rel

let immmref (a:Type) (inv:data_inv a) (rel:preorder a) = s:imreference a inv rel{is_eternal_region s.id && is_mm s}
let mmmref (a:Type) (rel:preorder a) = immmref a (FStar.Heap.trivial_invariant a) rel
...
let contains (#a:Type) (#inv:data_inv a) (#rel:preorder a) (m:mem) (s:imreference a inv rel) =
  live_region m s.id
  /\ HH.contains_ref s.ref m.h

let unused_in (#a:Type) (#inv:data_inv a) (#rel:preorder a) (r:imreference a inv rel) (h:mem) =
  ~ (live_region h r.id) \/
 ...
 *)
let weak_contains (#a:Type) (#inv:data_inv a) (#rel:preorder a) (m:mem) (s:imreference a inv rel) =
 ...
 *)
let sel (#a:Type) (#inv:data_inv a) (#rel:preorder a) (m:mem) (s:imreference a inv rel)
  : GTot a
  = m.h.[s.ref]
...
let as_ref #a #inv #rel (x:imreference a inv rel)  : GTot (Heap.mref a inv rel) = HH.as_ref #a #x.id x.ref
let as_addr #a #inv #rel (x:imreference a inv rel) : GTot nat = Heap.addr_of (HH.as_ref #a #x.id x.ref)
...
let lemma_upd_1 #a #inv #rel (h:mem) (x:imreference a inv rel) (v:a{rel (sel h x) v}) : Lemma
...
let lemma_upd_2 (#a:Type) (#inv:data_inv a) (#rel:preorder a) (h:mem) (x:imreference a inv rel) (v:a{rel (sel h x) v}) : Lemma
...
val lemma_live_1: #a:Type ->  #a':Type -> #inv:data_inv a -> #inv':data_inv a' -> #rel:preorder a -> #rel':preorder a'
                  -> h:mem -> x:imreference a inv rel -> x':imreference a' inv' rel' -> Lemma
...
noeq type some_ref =
| Ref : #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> imreference a inv rel -> some_ref
...
let f (a:Type0) (b:Type0) (inv_a inv_a' :data_inv a) (inv_b : data_inv b) (inv_n : data_inv nat) (rel_a:preorder a) (rel_b:preorder b) (rel_n:preorder nat)
                          (x:imreference a inv_a rel_a) (x':imreference a inv_a' rel_a)
        (y:imreference b inv_b rel_b) (z:imreference nat inv_n rel_n)
        (h0:mem) (h1:mem) =
