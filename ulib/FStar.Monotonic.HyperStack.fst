...
unfold
let is_in (r: rid) (h: hmap) = Map.contains h r
...
type sid =
  //stack region ids
  r: rid{is_stack_region r}

(*
 * AR: marking these unfolds, else I think there are pattern firing issues depending on which one we use
 *)
unfold
let is_above r1 r2 = includes r1 r2
unfold
let is_just_below r1 r2 = extends r1 r2
unfold
let is_below r1 r2 = is_above r2 r1
let is_strictly_below r1 r2 = is_below r1 r2 && r1 <> r2
let is_strictly_above r1 r2 = is_above r1 r2 && r1 <> r2
...
abstract
let map_invariant (m: hmap) : Type0 = map_invariant_predicate m
...
let downward_closed_predicate (h: hmap) : Type0 =
  ...
  forall (r: rid).
    ...
    //either is the root
    (r = root \/
      //or, any region beneath it
      (forall (s: rid).
          is_above r s /\
          //that is also in the memory
          is_in s h ==>
          //must be of the same flavor as itself
          (is_stack_region r = is_stack_region s)))
abstract
let downward_closed (h: hmap) : Type0 = downward_closed_predicate h

[@ "opaque_to_smt"]
unfold private
let tip_top_predicate (tip: rid) (h: hmap) : Type0 = forall (r: sid). is_in r h <==> is_above r tip
abstract
let tip_top (tip: rid) (h: hmap) : Type0 = tip_top_predicate tip h

let is_tip (tip: rid) (h: hmap) =
  //the tip is a stack region, or the root
  (is_stack_region tip \/ tip = root) /\
  //the tip is live
  is_in tip h /\
  //any other sid activation is a above (or equal to) the tip
  tip_top tip h

let rid_last_component (r: rid) : GTot int =
  let open FStar.List.Tot in
  let r = reveal r in
  if length r = 0 then 0 else snd (hd r)

[@ "opaque_to_smt"]
unfold private
let rid_ctr_pred_predicate (h: hmap) (n: int) : Type0 =
  forall (r: rid). Map.contains h r ==> rid_last_component r < n
 ...
 *)
abstract
let rid_ctr_pred (h: hmap) (n: int) : Type0 = rid_ctr_pred_predicate h n

let is_wf_with_ctr_and_tip (h: hmap) (ctr: int) (tip: rid) : Type0 =
  is_in root h /\ is_tip tip h /\ map_invariant h /\ downward_closed h /\ rid_ctr_pred h ctr
noeq abstract private
type mem' = | HS : rid_ctr: int -> h: hmap -> tip: rid -> mem'
abstract private
let mk_mem (rid_ctr: int) (h: hmap) (tip: rid) : mem' = HS rid_ctr h tip
abstract
let get_hmap (m: mem') : hmap = m.h
abstract
let get_rid_ctr (m: mem') : int = m.rid_ctr
abstract
let get_tip (m: mem') : rid = m.tip
private
let lemma_mk_mem'_projectors (rid_ctr: int) (h: hmap) (tip: rid)
  : Lemma (requires True)
    (ensures
      (let m = mk_mem rid_ctr h tip in
        (get_hmap m == h /\ get_rid_ctr m == rid_ctr /\ get_tip m == tip)))
    [
      SMTPatOr
      [
        [SMTPat (get_hmap (mk_mem rid_ctr h tip))];
...
private
let lemma_mem_projectors_are_in_wf_relation (m: mem)
  : Lemma (is_wf_with_ctr_and_tip (get_hmap m) (get_rid_ctr m) (get_tip m)) = ()
private
let lemma_is_wf_ctr_and_tip_intro (h: hmap) (ctr: int) (tip: rid)
  : Lemma
    (requires
      (is_in root h /\ (is_stack_region tip \/ tip = root) /\ is_in tip h /\ tip_top_predicate tip h /\
        map_invariant_predicate h /\ downward_closed_predicate h /\ rid_ctr_pred_predicate h ctr))
    (ensures (is_wf_with_ctr_and_tip h ctr tip)) = ()
private
let lemma_is_wf_ctr_and_tip_elim (m: mem)
  : Lemma
  (let h, rid_ctr, tip = get_hmap m, get_rid_ctr m, get_tip m in
    (is_in root h /\ (is_stack_region tip \/ tip = root) /\ is_in tip h /\ tip_top_predicate tip h /\
      map_invariant_predicate h /\ downward_closed_predicate h /\ rid_ctr_pred_predicate h rid_ctr)) =
  ()

(******* map_invariant related lemmas ******)

let lemma_map_invariant (m: mem) (r: rid) (s: rid)
  : Lemma (requires (is_in r (get_hmap m) /\ is_above s r))
    (ensures (is_in s (get_hmap m)))
    [SMTPat (is_in r (get_hmap m)); SMTPat (is_above s r); SMTPat (is_in s (get_hmap m))] = ()
...
let lemma_downward_closed (m: mem) (r: rid) (s: rid{s =!= root})
  : Lemma (requires (is_in r (get_hmap m) /\ is_above s r))
    (ensures
      (is_eternal_color (color r) == is_eternal_color (color s) /\
        is_stack_region r == is_stack_region s))
    [
      SMTPatOr
      [
        [
          SMTPat (Map.contains (get_hmap m) r);
          SMTPat (is_above s r);
          SMTPat (is_eternal_color (color s))
        ];
        [SMTPat (Map.contains (get_hmap m) r); SMTPat (is_above s r); SMTPat (is_stack_region s)]
      ]
    ] = ()
...
let lemma_tip_top (m: mem) (r: sid) : Lemma (is_in r (get_hmap m) <==> is_above r (get_tip m)) = ()
...
let lemma_tip_top_smt (m: mem) (r: rid)
  : Lemma (requires (is_stack_region r))
    (ensures (is_in r (get_hmap m) <==> is_above r (get_tip m)))
    [
      SMTPatOr
      [
        [SMTPat (is_stack_region r); SMTPat (is_above r (get_tip m))];
        [SMTPat (is_stack_region r); SMTPat (is_in r (get_hmap m))]
      ]
    ] = ()
...
let lemma_rid_ctr_pred ()
  : Lemma
  (forall (m: mem) (r: rid). {:pattern (Map.contains (get_hmap m) r)}
      Map.contains (get_hmap m) r ==> rid_last_component r < get_rid_ctr m) = ()
...
let eternal_region_does_not_overlap_with_tip
  ...
  (r:
    rid
      { is_eternal_color (color r) /\ not (disjoint r (get_tip m)) /\ r <> root /\
        is_stack_region (get_tip m) })
  : Lemma (requires True) (ensures (~(is_in r (get_hmap m)))) = root_has_color_zero ()

let poppable m = get_tip m <> root
...
let frameOf (#a: Type) (#rel: preorder a) (r: mreference a rel) : Tot rid = r.frame

let mk_mreference (#a: Type) (#rel: preorder a) (id: rid) (r: Heap.mref a rel)
  : Tot (mreference a rel) = MkRef id r

//Hopefully we can get rid of this one
abstract
let as_ref #a #rel (x: mreference a rel) : Tot (Heap.mref a rel) = MkRef?.ref x

//And make this one abstract
let as_addr #a #rel (x: mreference a rel) : GTot (x: nat{x > 0}) = Heap.addr_of (as_ref x)

let lemma_as_ref_inj (#a: Type) (#rel: preorder a) (r: mreference a rel)
  : Lemma (requires True) (ensures (mk_mreference (frameOf r) (as_ref r) == r)) [SMTPat (as_ref r)] =
  ()
 ...
 *)
let live_region (m: mem) (i: rid) : Tot bool = Map.contains (get_hmap m) i

let contains (#a: Type) (#rel: preorder a) (m: mem) (s: mreference a rel) : GTot bool =
  let i = frameOf s in
  live_region m i &&
  (FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains (Map.sel (get_hmap m) i)
          (as_ref s)))

let unused_in (#a: Type) (#rel: preorder a) (r: mreference a rel) (m: mem) : GTot bool =
  let h = get_hmap m in
  let i = frameOf r in
  not (Map.contains h i) ||
  FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.unused_in (as_ref r) (Map.sel h i))
...
let fresh_ref (#a: Type) (#rel: preorder a) (r: mreference a rel) (m0: mem) (m1: mem) : Type0 =
  let i = frameOf r in
  Heap.fresh (as_ref r) (Map.sel (get_hmap m0) i) (Map.sel (get_hmap m1) i)

let fresh_region (i: rid) (m0: mem) (m1: mem) =
  not (Map.contains (get_hmap m0) i) /\ Map.contains (get_hmap m1) i
private
val lemma_extends_fresh_disjoint: 
  ...
  Lemma
    (requires
      (let h0, h1 = get_hmap m0, get_hmap m1 in
        (map_invariant h0 /\ map_invariant h1 /\ fresh_region i m0 m1 /\ fresh_region j m0 m1 /\
          Map.contains h0 ipar /\ Map.contains h0 jpar /\ extends i ipar /\ extends j jpar /\ i <> j
...
let sel (#a: Type) (#rel: preorder a) (m: mem) (s: mreference a rel) : GTot a =
  Heap.sel (Map.sel (get_hmap m) (frameOf s)) (as_ref s)

let upd
  (#a: Type) (#rel: preorder a) (m: mem) (s: mreference a rel {live_region m (frameOf s)}) (v: a)
  : GTot mem =
  let h, rid_ctr, tip = get_hmap m, get_rid_ctr m, get_tip m in
...
let alloc
  (#a: Type0) (rel: preorder a) (id: rid) (init: a) (mm: bool)
  (m: mem{Map.contains (get_hmap m) id})
  : Tot
  (p:
    (mreference a rel * mem)
      { let r, h = Heap.alloc rel (Map.sel (get_hmap m) id) init mm in
...
let free (#a: Type0) (#rel: preorder a) (r: mreference a rel {is_mm r}) (m: mem{contains m r})
  : Tot mem =
  let h, rid_ctr, tip = get_hmap m, get_rid_ctr m, get_tip m in
  lemma_is_wf_ctr_and_tip_elim m;
  let i = frameOf r in
  let i_h = Map.sel h i in
  let i_h = Heap.free_mm i_h (as_ref r) in
  let h = Map.upd h i i_h in
  lemma_is_wf_ctr_and_tip_intro h rid_ctr tip;
  mk_mem rid_ctr h tip

let upd_tot (#a: Type) (#rel: preorder a) (m: mem) (r: mreference a rel {contains m r}) (v: a)
  : Tot mem =
  let h, rid_ctr, tip = get_hmap m, get_rid_ctr m, get_tip m in
  lemma_is_wf_ctr_and_tip_elim m;
  let i = frameOf r in
...
let sel_tot (#a: Type) (#rel: preorder a) (m: mem) (r: mreference a rel {contains m r}) : Tot a =
  Heap.sel_tot (Map.sel (get_hmap m) (frameOf r)) (as_ref r)

let fresh_frame (m0: mem) (m1: mem) =
  not (Map.contains (get_hmap m0) (get_tip m1)) /\ parent (get_tip m1) == get_tip m0 /\
...
let hs_push_frame (m: mem) : Tot (m': mem{fresh_frame m m'}) =
  ...
  assert (forall (s: rid). (is_above new_tip_rid s /\ is_in s h) ==> s = new_tip_rid);
  lemma_is_wf_ctr_and_tip_intro h (rid_ctr + 1) new_tip_rid;
  mk_mem (rid_ctr + 1) h new_tip_rid

let new_eternal_region
  (m: mem) (parent: rid{is_eternal_color (color parent) /\ Map.contains (get_hmap m) parent})
...
let lemma_sel_same_addr
  (#a: Type0) (#rel: preorder a) (h: mem) (r1: mreference a rel) (r2: mreference a rel)
  : Lemma
    (requires
      (frameOf r1 == frameOf r2 /\ contains h r1 /\ as_addr r1 = as_addr r2 /\ is_mm r1 == is_mm r2)
    ) (ensures (contains h r2 /\ sel h r1 == sel h r2)) =
  Heap.lemma_sel_same_addr (Map.sel h.h (frameOf r1)) (as_ref r1) (as_ref r2)

let lemma_upd_same_addr
  (#a: Type0) (#rel: preorder a) (h: mem) (r1: mreference a rel) (r2: mreference a rel) (x: a)
  : Lemma
    (requires
      (frameOf r1 == frameOf r2 /\ (contains h r1 \/ contains h r2) /\ as_addr r1 == as_addr r2 /\
        is_mm r1 == is_mm r2))
    (ensures (contains h r1 /\ contains h r2 /\ upd h r1 x == upd h r2 x)) =
  FStar.Monotonic.Heap.lemma_heap_equality_upd_same_addr (Map.sel h.h (frameOf r1))
    (as_ref r1)
    (as_ref r2)
    x;
  if StrongExcludedMiddle.strong_excluded_middle (contains h r1)
  then lemma_sel_same_addr h r1 r2
  else lemma_sel_same_addr h r2 r1

(* Two references with different reads are disjoint. *)

let mreference_distinct_sel_disjoint
  (#a: Type0) (#rel1: preorder a) (#rel2: preorder a) (h: mem) (r1: mreference a rel1)
  (r2: mreference a rel2)
  : Lemma
    (requires
...
let modifies_ref (id: rid) (s: Set.set nat) (h0: mem) (h1: mem) =
  Heap.modifies s (Map.sel (get_hmap h0) id) (Map.sel (get_hmap h1) id)
private
let lemma_upd_1 #a #rel (h: mem) (x: mreference a rel) (v: a{rel (sel h x) v})
  : Lemma (requires (contains h x))
    (ensures
      (contains h x /\ modifies_one (frameOf x) h (upd h x v) /\
        modifies_ref (frameOf x) (Set.singleton (as_addr x)) h (upd h x v) /\ sel (upd h x v) x == v
      )) = ()
private
let lemma_upd_2 (#a: Type) (#rel: preorder a) (h: mem) (x: mreference a rel) (v: a{rel (sel h x) v})
  : Lemma (requires (frameOf x = get_tip h /\ unused_in x h))
    (ensures
      (frameOf x = get_tip h /\ modifies_one (get_tip h) h (upd h x v) /\
        modifies_ref (get_tip h) Set.empty h (upd h x v) /\ sel (upd h x v) x == v)) = ()
private
val lemma_live_1: 
  #a: Type ->
  #a': Type ->
  ...
  x: mreference a rel ->
  x': mreference a' rel' ->
  Lemma (requires (contains h x /\ unused_in x' h))
    (ensures (frameOf x <> frameOf x' \/ ~(as_ref x === as_ref x')))
let lemma_live_1 #a #a' #rel #rel' h x x' = ()

let above_tip_is_live (#a: Type) (#rel: preorder a) (m: mem) (x: mreference a rel)
  : Lemma (requires (is_above (frameOf x) (get_tip m))) (ensures (is_in (frameOf x) (get_hmap m))) =
  ()
noeq
type some_ref = | Ref : #a: Type0 -> #rel: preorder a -> mreference a rel -> some_ref
...
[@ "opaque_to_smt"]
unfold
let mods (rs: some_refs) (h0: mem) (h1: mem) : GTot Type0 =
  (norm norm_steps (modifies (regions_of_some_refs rs) h0 h1)) /\
  (norm norm_steps (modifies_some_refs rs rs h0 h1))

////////////////////////////////////////////////////////////////////////////////
let eternal_disjoint_from_tip
  (h: mem{is_stack_region (get_tip h)})
  (r: rid{is_eternal_color (color r) /\ r <> root /\ is_in r (get_hmap h)})
  : Lemma (disjoint (get_tip h) r) = ()

////////////////////////////////////////////////////////////////////////////////

let lemma_heap_equality_cancel_same_mref_upd
  (#a: Type) (#rel: preorder a) (h: mem) (r: mreference a rel) (x: a) (y: a)
  : Lemma (requires (live_region h (frameOf r))) (ensures (upd (upd h r x) r y == upd h r y)) =
...
let lemma_heap_equality_upd_with_sel (#a: Type) (#rel: preorder a) (h: mem) (r: mreference a rel)
...
let lemma_heap_equality_commute_distinct_upds
  (#a: Type) (#b: Type) (#rel_a: preorder a) (#rel_b: preorder b) (h: mem) (r1: mreference a rel_a)
  (r2: mreference b rel_b) (x: a) (y: b)
  : Lemma
    (requires
      (as_addr r1 =!= as_addr r2 /\ live_region h (frameOf r1) /\ live_region h (frameOf r2)))
    (ensures (upd (upd h r1 x) r2 y == upd (upd h r2 y) r1 x)) =
  let h0 = upd (upd h r1 x) r2 y in
  let h1 = upd (upd h r2 y) r1 x in
  if frameOf r1 = frameOf r2
  then
    Heap.lemma_heap_equality_commute_distinct_upds (Map.sel h.h (frameOf r1))
      (as_ref r1)
      (as_ref r2)
      x
      y;
...
noeq abstract
type aref : Type0 = | ARef : aref_region: rid -> aref_aref: Heap.aref -> aref
abstract
let dummy_aref: aref = ARef root Heap.dummy_aref
abstract
let aref_equal (a1: aref) (a2: aref)
...
(* Introduction rule *)
abstract
let aref_of (#t: Type) (#rel: preorder t) (r: mreference t rel) : Tot aref =
  ARef (frameOf r) (Heap.aref_of (as_ref r))

(* Operators lifted from reference *)
abstract
let frameOf_aref (a: aref) : GTot rid = a.aref_region
abstract
let frameOf_aref_of (#t: Type) (#rel: preorder t) (r: mreference t rel)
  : Lemma (frameOf_aref (aref_of r) == frameOf r) [SMTPat (frameOf_aref (aref_of r))] = ()
abstract
let aref_as_addr (a: aref) : GTot (x: nat{x > 0}) = Heap.addr_of_aref a.aref_aref
abstract
let aref_as_addr_aref_of (#t: Type) (#rel: preorder t) (r: mreference t rel)
  : Lemma (aref_as_addr (aref_of r) == as_addr r) [SMTPat (aref_as_addr (aref_of r))] =
  Heap.addr_of_aref_of (as_ref r)
abstract
let aref_is_mm (r: aref) : GTot bool = Heap.aref_is_mm r.aref_aref
abstract
let is_mm_aref_of (#t: Type) (#rel: preorder t) (r: mreference t rel)
  : Lemma (aref_is_mm (aref_of r) == is_mm r) [SMTPat (aref_is_mm (aref_of r))] =
  Heap.is_mm_aref_of (as_ref r)
abstract
let aref_unused_in (a: aref) (h: mem) : GTot Type0 =
  ~(live_region h a.aref_region) \/ Heap.aref_unused_in a.aref_aref (Map.sel h.h a.aref_region)
abstract
let unused_in_aref_of (#t: Type) (#rel: preorder t) (r: mreference t rel) (h: mem)
  : Lemma (aref_unused_in (aref_of r) h <==> unused_in r h) [SMTPat (aref_unused_in (aref_of r) h)] =
  Heap.unused_in_aref_of (as_ref r) (Map.sel h.h (frameOf r))

abstract
val contains_aref_unused_in: 
  #a: Type ->
  #rel: preorder a ->
  h: mem ->
  x: mreference a rel ->
...
let contains_aref_unused_in #a #rel h x y =
  ...
  then Heap.contains_aref_unused_in (Map.sel h.h (frameOf x)) (as_ref x) y.aref_aref

(* Elimination rule *)

abstract
let aref_live_at (h: mem) (a: aref) (v: Type) (rel: preorder v) : GTot Type0 =
  live_region h a.aref_region /\ Heap.aref_live_at (Map.sel h.h a.aref_region) a.aref_aref v rel

abstract
let greference_of (a: aref) (v: Type) (rel: preorder v)
  : Ghost (mreference v rel) (requires (exists h. aref_live_at h a v rel)) (ensures (fun _ -> True)) =
  MkRef a.aref_region (Heap.gref_of a.aref_aref v rel)

abstract
let reference_of (h: mem) (a: aref) (v: Type) (rel: preorder v)
  : Pure (mreference v rel)
    (requires (aref_live_at h a v rel))
    (ensures
      (fun x ->
          aref_live_at h a v rel /\ frameOf x == frameOf_aref a /\ as_addr x == aref_as_addr a /\
          is_mm x == aref_is_mm a)) =
  MkRef a.aref_region (Heap.ref_of (Map.sel h.h a.aref_region) a.aref_aref v rel)

abstract
let aref_live_at_aref_of (h: mem) (#t: Type0) (#rel: preorder t) (r: mreference t rel)
  : Lemma (aref_live_at h (aref_of r) t rel <==> contains h r)
    [SMTPat (aref_live_at h (aref_of r) t rel)] = ()

abstract
let contains_greference_of (h: mem) (a: aref) (t: Type0) (rel: preorder t)
  : Lemma (requires (exists h'. aref_live_at h' a t rel))
    (ensures
      ((exists h'. aref_live_at h' a t rel) /\
        (contains h (greference_of a t rel) <==> aref_live_at h a t rel)))
    [SMTPatOr [[SMTPat (contains h (greference_of a t rel))]; [SMTPat (aref_live_at h a t rel)]]] =
  ()

abstract
let aref_of_greference_of (a: aref) (v: Type0) (rel: preorder v)
...
abstract
let frameOf_greference_of (a: aref) (t: Type) (rel: preorder t)
  : Lemma (requires (exists h. aref_live_at h a t rel))
    (ensures
      ((exists h. aref_live_at h a t rel) /\ frameOf (greference_of a t rel) == frameOf_aref a))
    [SMTPat (frameOf (greference_of a t rel))] = ()

abstract
let as_addr_greference_of (a: aref) (t: Type0) (rel: preorder t)
  : Lemma (requires (exists h. aref_live_at h a t rel))
    (ensures
      ((exists h. aref_live_at h a t rel) /\ as_addr (greference_of a t rel) == aref_as_addr a))
    [SMTPat (as_addr (greference_of a t rel))] = ()

abstract
let is_mm_greference_of (a: aref) (t: Type0) (rel: preorder t)
  : Lemma (requires (exists h. aref_live_at h a t rel))
    (ensures ((exists h. aref_live_at h a t rel) /\ is_mm (greference_of a t rel) == aref_is_mm a))
    [SMTPat (is_mm (greference_of a t rel))] = ()

abstract
let unused_in_greference_of (a: aref) (t: Type0) (rel: preorder t) (h: mem)
  : Lemma (requires (exists h. aref_live_at h a t rel))
    (ensures
      ((exists h. aref_live_at h a t rel) /\
        (unused_in (greference_of a t rel) h <==> aref_unused_in a h)))
    [SMTPat (unused_in (greference_of a t rel) h)] = ()

abstract
let sel_reference_of (a: aref) (v: Type0) (rel: preorder v) (h1: mem) (h2: mem)
  : Lemma (requires (aref_live_at h1 a v rel /\ aref_live_at h2 a v rel))
    (ensures
      (aref_live_at h2 a v rel /\ sel h1 (reference_of h2 a v rel) == sel h1 (greference_of a v rel)
      ))
    [SMTPat (sel h1 (reference_of h2 a v rel))] = ()

abstract
let upd_reference_of (a: aref) (v: Type0) (rel: preorder v) (h1: mem) (h2: mem) (x: v)
  : Lemma (requires (aref_live_at h1 a v rel /\ aref_live_at h2 a v rel))
    (ensures
      (aref_live_at h1 a v rel /\ aref_live_at h2 a v rel /\
