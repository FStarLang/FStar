...
abstract let mk_mem (rid_ctr:int) (h:hmap) (tip:rid{is_wf_with_ctr_and_tip h rid_ctr tip}) :mem =
  HS rid_ctr h tip
...
private let lemma_mem_projectors_are_in_relation (m:mem)
...
private let lemma_is_wf_ctr_and_tip_elim (m:mem)
  :Lemma (requires True)
         (ensures  (root `is_in` get_hmap m /\ (is_stack_region (get_tip m) \/ get_tip m = root) /\  get_tip m `is_in` get_hmap m /\
	            tip_top_predicate (get_tip m) (get_hmap m) /\ map_invariant_predicate (get_hmap m) /\
                    downward_closed_predicate (get_hmap m) /\ rid_ctr_pred_predicate (get_hmap m) (get_rid_ctr m)))
  = lemma_mem_projectors_are_in_relation m
...
let eternal_region_does_not_overlap_with_tip (m:mem) (r:rid{is_eternal_color (color r) /\ not (disjoint r (get_tip m)) /\ r<>root /\ is_stack_region (get_tip m)})
  : Lemma (requires True)
...
let popped m0 m1 =
  poppable m0
  /\ parent (get_tip m0) = get_tip m1
  /\ Set.equal (Map.domain (get_hmap m1)) (remove_elt (Map.domain (get_hmap m0)) (get_tip m0))
  /\ Map.equal (get_hmap m1) (Map.restrict (Map.domain (get_hmap m1)) (get_hmap m0))

let pop (m0:mem{poppable m0}) : Tot mem =
  root_has_color_zero();
  lemma_is_wf_ctr_and_tip_elim m0;
  let dom = remove_elt (Map.domain (get_hmap m0)) (get_tip m0) in
  let h0 = get_hmap m0 in
  let h1 = Map.restrict dom h0 in
  let tip0 = get_tip m0 in
  let tip1 = parent tip0 in
  lemma_is_wf_ctr_and_tip_intro h1 (get_rid_ctr m0) tip1;
  mk_mem (get_rid_ctr m0) h1 tip1
...
let contains (#a:Type) (#rel:preorder a) (m:mem) (s:mreference a rel) :GTot bool =
  let i = frameOf s in
  live_region m i && (FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains (Map.sel (get_hmap m) i) (as_ref s)))

let unused_in (#a:Type) (#rel:preorder a) (r:mreference a rel) (h:mem) :GTot bool =
  let i = frameOf r in
  not (Map.contains (get_hmap h) i) ||
  FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.unused_in (as_ref r) (Map.sel (get_hmap h) i))

let contains_ref_in_its_region (#a:Type) (#rel:preorder a) (h:mem) (r:mreference a rel) :GTot bool =
  let i = frameOf r in
  FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains (Map.sel (get_hmap h) i) (as_ref r))
...
private val lemma_extends_fresh_disjoint: i:rid -> j:rid -> ipar:rid -> jpar:rid
                               -> m0:mem{map_invariant (get_hmap m0)} -> m1:mem{map_invariant (get_hmap m1)} ->
  Lemma (requires (fresh_region i m0 m1
                  /\ fresh_region j m0 m1
                  /\ Map.contains (get_hmap m0) ipar
                  /\ Map.contains (get_hmap m0) jpar
                  /\ extends i ipar
                  /\ extends j jpar
                  /\ i<>j))
...
let upd (#a:Type) (#rel:preorder a) (m:mem) (s:mreference a rel{live_region m (frameOf s)}) (v:a)
  ...
  = lemma_is_wf_ctr_and_tip_elim m;
    let i = frameOf s in
    let h = Map.upd (get_hmap m) i (Heap.upd (Map.sel (get_hmap m) i) (as_ref s) v) in
    lemma_is_wf_ctr_and_tip_intro h (get_rid_ctr m) (get_tip m);
    mk_mem (get_rid_ctr m) h (get_tip m)

let alloc (#a:Type0) (rel:preorder a) (id:rid) (init:a) (mm:bool) (m:mem{(get_hmap m) `Map.contains` id})
  :Tot (p:(mreference a rel * mem){let (r, h) = Heap.alloc rel (Map.sel (get_hmap m) id) init mm in
                                   as_ref (fst p) == r /\
                                   get_hmap (snd p) == Map.upd (get_hmap m) id h})
  = lemma_is_wf_ctr_and_tip_elim m;
    let r, h = Heap.alloc rel (Map.sel (get_hmap m) id) init mm in
    let h = Map.upd (get_hmap m) id h in
    lemma_is_wf_ctr_and_tip_intro h (get_rid_ctr m) (get_tip m);
    (mk_mreference id r), (mk_mem (get_rid_ctr m) h (get_tip m))
