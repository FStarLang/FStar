...
noeq
type mem = | HS : rid_ctr: int -> h: hmap -> tip: rid{is_wf_with_ctr_and_tip h rid_ctr tip} -> mem
...
let lemma_is_wf_ctr_and_tip_elim (m: mem)
  : Lemma (requires (is_wf_with_ctr_and_tip m.h m.rid_ctr m.tip))
    (ensures
      (is_in root m.h /\ (is_stack_region m.tip \/ m.tip = root) /\ is_in m.tip m.h /\
        tip_top_predicate m.tip m.h /\ map_invariant_predicate m.h /\ downward_closed_predicate m.h /\
        rid_ctr_pred_predicate m.h m.rid_ctr)) = ()
...
let lemma_map_invariant (m: mem) (r: rid) (s: rid)
  : Lemma (requires (is_in r m.h /\ is_above s r))
    (ensures (is_in s m.h))
    [SMTPat (is_in r m.h); SMTPat (is_above s r); SMTPat (is_in s m.h)] = ()
...
let lemma_downward_closed (m: mem) (r: rid) (s: rid{s =!= root})
  : Lemma (requires (is_in r m.h /\ is_above s r))
    ...
    [
      SMTPatOr
      [
        [SMTPat (Map.contains m.h r); SMTPat (is_above s r); SMTPat (is_eternal_color (color s))];
        [SMTPat (Map.contains m.h r); SMTPat (is_above s r); SMTPat (is_stack_region s)]
      ]
    ] = ()
...
let lemma_tip_top (m: mem) (r: sid) : Lemma (is_in r m.h <==> is_above r m.tip) = ()
...
let lemma_tip_top_smt (m: mem) (r: rid)
  : Lemma (requires (is_stack_region r))
    (ensures (is_in r m.h <==> is_above r m.tip))
    [
      SMTPatOr
      [
        [SMTPat (is_stack_region r); SMTPat (is_above r m.tip)];
        [SMTPat (is_stack_region r); SMTPat (is_in r m.h)]
      ]
    ] = ()
...
let lemma_rid_ctr_pred ()
  : Lemma
  (forall (m: mem) (r: rid). {:pattern (Map.contains m.h r)}
      Map.contains m.h r ==> rid_last_component r < m.rid_ctr) = ()

let empty_mem (m: hmap) =
  let empty_map = Map.restrict (Set.empty) m in
  let h = Map.upd empty_map root Heap.emp in
  let tip = root in
  Prims._assume (rid_last_component root == 0);
...
let eternal_region_does_not_overlap_with_tip
  (m: mem)
  (r:
    rid{is_eternal_color (color r) /\ not (disjoint r m.tip) /\ r <> root /\ is_stack_region m.tip})
  : Lemma (requires True) (ensures (~(is_in r m.h))) = root_has_color_zero ()
...
let popped m0 m1 =
  poppable m0 /\ parent m0.tip = m1.tip /\
...
let as_addr #a #rel (x: mreference a rel) : GTot nat = Heap.addr_of (as_ref x)
...
let contains (#a: Type) (#rel: preorder a) (m: mem) (s: mreference a rel) : GTot bool =
  let i = frameOf s in
  live_region m i &&
  (FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains (Map.sel m.h i) (as_ref s)))
...
let contains_ref_in_its_region (#a: Type) (#rel: preorder a) (h: mem) (r: mreference a rel)
  : GTot bool =
  let i = frameOf r in
...
let fresh_region (i: rid) (m0: mem) (m1: mem) = not (Map.contains m0.h i) /\ Map.contains m1.h i
...
val lemma_extends_fresh_disjoint: 
  ...
  ipar: rid ->
  jpar: rid ->
  m0: mem{map_invariant m0.h} ->
  m1: mem{map_invariant m1.h} ->
  Lemma
    (requires
      (fresh_region i m0 m1 /\ fresh_region j m0 m1 /\ Map.contains m0.h ipar /\
        Map.contains m0.h jpar /\ extends i ipar /\ extends j jpar /\ i <> j))
...
let alloc (#a: Type0) (rel: preorder a) (id: rid) (init: a) (mm: bool) (m: mem{Map.contains m.h id})
  : Tot
  (p:
    (mreference a rel * mem)
      { let r, h = Heap.alloc rel (Map.sel m.h id) init mm in
        as_ref (fst p) == r /\ (snd p).h == Map.upd m.h id h }) =
  lemma_is_wf_ctr_and_tip_elim m;
  let r, h = Heap.alloc rel (Map.sel m.h id) init mm in
...
let fresh_frame (m0: mem) (m1: mem) =
  not (Map.contains m0.h m1.tip) /\ parent m1.tip = m0.tip /\ m1.h == Map.upd m0.h m1.tip Heap.emp
...
let new_eternal_region
  (m: mem) (parent: rid{is_eternal_color (color parent) /\ Map.contains m.h parent})
  (c: option int {None? c \/ is_eternal_color (Some?.v c)})
  : Tot (t: (rid * mem){fresh_region (fst t) m (snd t)}) =
  lemma_is_wf_ctr_and_tip_elim m;
  let new_rid =
    if None? c then extend_monochrome parent m.rid_ctr else extend parent m.rid_ctr (Some?.v c)
  in
...
// //  *)
let modifies (s: Set.set rid) (m0: mem) (m1: mem) = modifies_just s m0.h m1.h
...
let modifies_drop_tip (m0: mem) (m1: mem) (m2: mem) (s: Set.set rid)
  : Lemma
  (fresh_frame m0 m1 /\ m1.tip == m2.tip /\
...
let lemma_upd_2 (#a: Type) (#rel: preorder a) (h: mem) (x: mreference a rel) (v: a{rel (sel h x) v})
  : Lemma (requires (frameOf x = h.tip /\ unused_in x h))
    (ensures
      (frameOf x = h.tip /\ modifies_one h.tip h (upd h x v) /\
        modifies_ref h.tip Set.empty h (upd h x v) /\ sel (upd h x v) x == v)) = ()
...
let above_tip_is_live (#a: Type) (#rel: preorder a) (m: mem) (x: mreference a rel)
  : Lemma (requires (is_above (frameOf x) m.tip)) (ensures (is_in (frameOf x) m.h)) = ()
...
let eternal_disjoint_from_tip
  (h: mem{is_stack_region h.tip}) (r: rid{is_eternal_color (color r) /\ r <> root /\ is_in r h.h})
...
abstract
let aref_as_addr (a: aref) : GTot nat = Heap.addr_of_aref a.aref_aref
