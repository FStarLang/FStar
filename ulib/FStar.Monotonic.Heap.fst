...
type heap_rec = {
  ...
  memory:nat -> Tot (option (a: Type0 & rel: (option (preorder a)) & b: bool & a))
}
...
let emp = { next_addr = 0; memory = (fun (r: nat) -> None) }

private noeq
type mref' (a: Type0) (rel: preorder a) : Type0 = {
  addr:nat;
  init:a;
  //manually managed flag
  mm:bool
}
    ...
    then
      {
        ...
        memory
        =
        (fun (r': nat) -> if r' = r.addr then Some (| a, Some rel, r.mm, x |) else h.memory r')
      }
    else
      {
        h with
...
let alloc #a rel h x mm =
  ...
  {
    next_addr = r.addr + 1;
    memory = (fun (r': nat) -> if r' = r.addr then Some (| a, Some rel, r.mm, x |) else h.memory r')
  }
...
let aref = aref'
let dummy_aref = { a_addr = 0; a_mm = false }
