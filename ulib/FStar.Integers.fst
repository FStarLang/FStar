...
abstract
let nat_size (x: int) : Type = x >= 0
let reveal_nat_size x : Lemma (nat_size x <==> x >= 0) [SMTPat (nat_size x)] = ()

abstract
let uint_size (x: int) (n: nat) : Type = FStar.UInt.size x n
let reveal_uint_size x n : Lemma (uint_size x n <==> FStar.UInt.size x n) [SMTPat (uint_size x n)] =
  ()

abstract
let int_size (x: int) (n: pos) : Type = FStar.Int.size x n
let reveal_int_size x n : Lemma (int_size x n <==> FStar.Int.size x n) [SMTPat (int_size x n)] = ()
...
let u (#s: signedness) (#w: width) (x: int_t Signed Winfinite {normalize (within_bounds s w x)})
...
abstract
let cast
  (#s: signedness) (#s': signedness) (#w: width) (#w': width)
  (from: int_t s w {normalize (within_bounds s' w' (v from))})
  : Tot (to: int_t s' w' {normalize (v from == v to)}) = u (v from)

unfold
let op_Plus
  (#s: signedness) (#w: width) (x: int_t s w)
...
unfold
let op_Plus_Question (#w: width) (x: int_t Unsigned w) (y: int_t Unsigned w)
...
unfold
let op_Plus_Percent (#w: fixed_width) (x: int_t Unsigned w) (y: int_t Unsigned w)
...
unfold
let op_Subtraction
  (#s: signedness) (#w: width) (x: int_t s w) (y: int_t s w {within_bounds s w (v x - v y)})
...
unfold
let op_Subtraction_Question (#w: width) (x: int_t Unsigned w) (y: int_t Unsigned w)
...
unfold
let op_Subtraction_Percent (#w: fixed_width) (x: int_t Unsigned w) (y: int_t Unsigned w)
  : Tot (z: int_t Unsigned w {v z = modulo Unsigned (v x - v y) (pow2 (nat_of_fixed_width w))}) =
  match w with
  | W8 -> let open FStar.UInt8 in x -%^ y
  | W16 -> let open FStar.UInt16 in x -%^ y
...
unfold
let op_Star
  (#s: signedness) (#w: width{w <> W128}) (x: int_t s w)
...
unfold
let op_Star_Question (#w: width{w <> W128}) (x: int_t Unsigned w) (y: int_t Unsigned w)
  : Tot (z: int_t Unsigned w {within_bounds Unsigned w (v x * v y) ==> v z = v x * v y}) =
  match w with
  | Winfinite -> x * y
  | W8 -> let open FStar.UInt8 in x *?^ y
...
unfold
let op_Star_Percent (#w: fixed_width{w <> W128}) (x: int_t Unsigned w) (y: int_t Unsigned w)
  : Tot (z: int_t Unsigned w {v z = modulo Unsigned (v x * v y) (pow2 (nat_of_fixed_width w))}) =
  match w with
  | W8 -> let open FStar.UInt8 in x *%^ y
  | W16 -> let open FStar.UInt16 in x *%^ y
...
unfold
let ok
  (#s: signedness) (#w: width)
  (op: (int_t Signed Winfinite -> int_t Signed Winfinite -> int_t Signed Winfinite)) (x: int_t s w)
  (y: int_t s w) = normalize (within_bounds s w (op (v x) (v y)))
////////////////////////////////////////////////////////////////////////////////
//Test
////////////////////////////////////////////////////////////////////////////////
