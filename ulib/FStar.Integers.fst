...
type signed_width =
  | Signed of width
  //We don't support (Unsigned WInfinite); use nat instead
...
let cast #sw #sw' (from: int_t sw {within_bounds sw' (v from)})
  : Tot (to: int_t sw' {norm (v from == v to)}) = u (v from)
...
[@ mark_for_norm]
unfold
let op_Plus #sw (x: int_t sw) (y: int_t sw {within_bounds sw (v x + v y)}) : Tot (z : int_t sw) =
  match sw with
  | Signed Winfinite -> x + y
  | Unsigned W8 -> let open FStar.UInt8 in x +^ y
  | Unsigned W16 -> let open FStar.UInt16 in x +^ y
  | Unsigned W31 -> let open FStar.UInt31 in x +^ y
  | Unsigned W32 -> let open FStar.UInt32 in x +^ y
  | Unsigned W63 -> let open FStar.UInt63 in x +^ y
...
[@ mark_for_norm]
unfold
let op_Plus_Percent (#sw: _{Unsigned? sw}) (x: int_t sw) (y: int_t sw) : Tot (z : int_t sw) =
...
[@ mark_for_norm]
unfold
let op_Subtraction #sw (x: int_t sw) (y: int_t sw {within_bounds sw (v x - v y)})
  : Tot (z : int_t sw) =
  match sw with
  | Signed Winfinite -> x - y
  | Unsigned W8 -> let open FStar.UInt8 in x -^ y
...
[@ mark_for_norm]
unfold
let op_Subtraction_Question (#sw: _{Unsigned? sw}) (x: int_t sw) (y: int_t sw) : Tot (z : int_t sw) =
  let Unsigned w = sw in
  match w with
  | W8 -> let open FStar.UInt8 in x -?^ y
  | W16 -> let open FStar.UInt16 in x -?^ y
...
[@ mark_for_norm]
unfold
let op_Subtraction_Percent (#sw: _{Unsigned? sw}) (x: int_t sw) (y: int_t sw) : Tot (z : int_t sw) =
  let Unsigned w = sw in
  match w with
  | W8 -> let open FStar.UInt8 in x -%^ y
  | W16 -> let open FStar.UInt16 in x -%^ y
...
[@ mark_for_norm]
unfold
let op_Minus (#sw: _{Signed? sw}) (x: int_t sw {within_bounds sw (0 - v x)}) : Tot (z : int_t sw) =
  let Signed w = sw in
  match w with
  | Winfinite -> 0 - x
  | W8 -> let open FStar.Int8 in 0y -^ x
  | W16 -> let open FStar.Int16 in 0s -^ x
  | W31 -> let open FStar.Int31 in int_to_t 0 -^ x
  | W32 -> let open FStar.Int32 in 0l -^ x
  | W63 -> let open FStar.Int63 in int_to_t 0 -^ x
...
unfold
let op_Star
  (#sw: signed_width{width_of_sw sw <> W128}) (x: int_t sw)
...
[@ mark_for_norm]
unfold
let op_Star_Question (#sw: _{Unsigned? sw /\ width_of_sw sw <> W128}) (x: int_t sw) (y: int_t sw)
  : Tot (z : int_t sw) =
  let Unsigned w = sw in
  match w with
  | W8 -> let open FStar.UInt8 in x *?^ y
...
[@ mark_for_norm]
unfold
let op_Star_Percent (#sw: _{Unsigned? sw /\ width_of_sw sw <> W128}) (x: int_t sw) (y: int_t sw)
  : Tot (z : int_t sw) =
  let Unsigned w = sw in
  match w with
  | W8 -> let open FStar.UInt8 in x *%^ y
...
[@ mark_for_norm]
unfold
let op_Greater #sw (x: int_t sw) (y: int_t sw) : bool =
  match sw with
  | Signed Winfinite -> x > y
  | Unsigned W8 -> let open FStar.UInt8 in x >^ y
  | Unsigned W16 -> let open FStar.UInt16 in x >^ y
...
[@ mark_for_norm]
unfold
let op_Greater_Equals #sw (x: int_t sw) (y: int_t sw) : bool =
  match sw with
  | Signed Winfinite -> x >= y
  | Unsigned W8 -> let open FStar.UInt8 in x >=^ y
  | Unsigned W16 -> let open FStar.UInt16 in x >=^ y
...
[@ mark_for_norm]
unfold
let op_Less #sw (x: int_t sw) (y: int_t sw) : bool =
  match sw with
  | Signed Winfinite -> x < y
  | Unsigned W8 -> let open FStar.UInt8 in x <^ y
  | Unsigned W16 -> let open FStar.UInt16 in x <^ y
...
[@ mark_for_norm]
unfold
let op_Less_Equals #sw (x: int_t sw) (y: int_t sw) : bool =
  match sw with
  | Signed Winfinite -> x <= y
  | Unsigned W8 -> let open FStar.UInt8 in x <=^ y
  | Unsigned W16 -> let open FStar.UInt16 in x <=^ y
...
unfold
let op_Slash
  (#sw: signed_width{sw <> Unsigned W128}) (x: int_t sw)
  (y:
    int_t sw
      { 0 <> (v y <: Prims.int) /\
        (match sw with
          | Unsigned _ -> within_bounds sw (v x / v y)
          | Signed _ -> within_bounds sw (FStar.Int.op_Slash (v x) (v y))) })
  : Tot (int_t sw) =
  match sw with
  | Signed Winfinite -> x / y
  | Unsigned W8 -> let open FStar.UInt8 in x /^ y
  ...
  (y:
    int_t sw
      { 0 <> (v y <: Prims.int) /\
        (match sw with
          | Unsigned _ ->
            within_bounds sw (FStar.UInt.mod #(nat_of_fixed_width (width_of_sw sw)) (v x) (v y))
          | Signed Winfinite -> True
          | Signed _ ->
            within_bounds sw (FStar.Int.mod #(nat_of_fixed_width (width_of_sw sw)) (v x) (v y))) })
  : Tot (int_t sw) =
  match sw with
  | Signed Winfinite -> x % y
  | Unsigned W8 -> let open FStar.UInt8 in x %^ y
...
[@ mark_for_norm]
unfold
let op_Hat_Hat #sw (x: int_t sw) (y: int_t sw {width_of_sw sw <> Winfinite}) : Tot (int_t sw) =
  match sw with
  | Unsigned W8 -> let open FStar.UInt8 in x ^^ y
  | Unsigned W16 -> let open FStar.UInt16 in x ^^ y
  | Unsigned W31 -> let open FStar.UInt31 in x ^^ y
...
[@ mark_for_norm]
unfold
let op_Amp_Hat #sw (x: int_t sw) (y: int_t sw {width_of_sw sw <> Winfinite}) : Tot (int_t sw) =
  match sw with
  | Unsigned W8 -> let open FStar.UInt8 in x &^ y
  | Unsigned W16 -> let open FStar.UInt16 in x &^ y
  | Unsigned W31 -> let open FStar.UInt31 in x &^ y
...
[@ mark_for_norm]
unfold
let op_Bar_Hat #sw (x: int_t sw) (y: int_t sw {width_of_sw sw <> Winfinite}) : Tot (int_t sw) =
  match sw with
  | Unsigned W8 -> let open FStar.UInt8 in x |^ y
  | Unsigned W16 -> let open FStar.UInt16 in x |^ y
  | Unsigned W31 -> let open FStar.UInt31 in x |^ y
...
let op_Less_Less_Hat
  #sw (x: int_t sw)
  (y:
    int_t (Unsigned W32) {width_of_sw sw <> Winfinite /\ v y < nat_of_fixed_width (width_of_sw sw)})
  : Tot (int_t sw) =
  match sw with
  | Unsigned W8 -> let open FStar.UInt8 in x <<^ y
  | Unsigned W16 -> let open FStar.UInt16 in x <<^ y
...
unfold
let ok
  #sw (op: (int_t (Signed Winfinite) -> int_t (Signed Winfinite) -> int_t (Signed Winfinite)))
  (x: int_t sw) (y: int_t sw) = within_bounds sw (op (v x) (v y))
