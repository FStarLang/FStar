...
type rel (a: Type) (b: Type) : Type = | R : l: a -> r: b -> rel a b
...
val rel_map3T: ('a -> 'b -> 'c -> Tot 'd) -> (double 'a) -> (double 'b) -> (double 'c) ->
  Tot (double 'd)
let rel_map3T f (R x1 x2) (R y1 y2) (R z1 z2) = R (f x1 y1 z1) (f x2 y2 z2)

val rel_map3G: ('a -> 'b -> 'c -> GTot 'd) -> (double 'a) -> (double 'b) -> (double 'c) ->
  GTot (double 'd)
...
val tl_rel: #a: Type -> l: double (list a) {Cons? (R?.l l) /\ Cons? (R?.r l)} ->
  Tot (double (list a))
...
let eq_rel = rel_map2Teq (fun (x: bool) (y: bool) -> x = y)
