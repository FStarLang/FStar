...
abstract val const_on: #key:eqtype -> #value:eqtype -> #f:cmp key -> d:ordset key f -> x:value -> Tot (ordmap key value f)
abstract val select  : #key:eqtype -> #value:eqtype -> #f:cmp key -> k:key
              -> m:ordmap key value f -> Tot (option value)
abstract val update  : #key:eqtype -> #value:eqtype -> #f:cmp key -> key -> value
              -> m:ordmap key value f -> Tot (ordmap key value f)
abstract val contains: #key:eqtype -> #value:eqtype -> #f:cmp key -> key -> ordmap key value f
              -> Tot bool
abstract val dom     : #key:eqtype -> #value:eqtype -> #f:cmp key -> m:ordmap key value f ->
              Tot (ordset key f)

abstract val remove  : #key:eqtype -> #value:eqtype -> #f:cmp key -> key
              -> ordmap key value f -> Tot (ordmap key value f)
abstract val choose  : #key:eqtype -> #value:eqtype -> #f:cmp key -> ordmap key value f
              -> Tot (option (key * value))

abstract val size    : #key:eqtype -> #value:eqtype -> #f:cmp key -> ordmap key value f
              -> Tot nat

let empty (#k:eqtype) (#v:eqtype) #f =
  let d = OrdSet.empty in
  let g = (fun x -> None) in
  Mk_map d g

let const_on (#k:eqtype) (#v:eqtype) #f d x =
...
let eq_lemma (#k:eqtype) (#v:eqtype) #f m1 m2 =
...
let upd_order (#k:eqtype) (#v:eqtype) #f x y x' y' m =
  let (Mk_map s1 g1) = update #k #v #f x' y' (update #k #v #f x y m) in
  let (Mk_map s2 g2) = update #k #v #f x y (update #k #v #f x' y' m) in
  cut (feq g1 g2)

let upd_same_k (#k:eqtype) (#v:eqtype) #f x y y' m =
  let (Mk_map s1 g1) = update #k #v #f x y m in
  let (Mk_map s2 g2) = update #k #v #f x y (update #k #v #f x y' m) in
  cut (feq g1 g2)

let sel_upd1 (#k:eqtype) (#v:eqtype) #f x y m = ()

let sel_upd2 (#k:eqtype) (#v:eqtype) #f x y x' m = ()

let sel_empty (#k:eqtype) (#v:eqtype) #f x = ()

let sel_contains (#k:eqtype) (#v:eqtype) #f x m = ()

let contains_upd1 (#k:eqtype) (#v:eqtype) #f x y x' m = ()

let contains_upd2 (#k:eqtype) (#v:eqtype) #f x y x' m = ()

let contains_empty (#k:eqtype) (#v:eqtype) #f x = ()

let contains_remove (#k:eqtype) (#v:eqtype) #f x y m = ()

let eq_remove (#k:eqtype) (#v:eqtype) #f x m =
  let (Mk_map s g) = m in
...
let choose_empty (#k:eqtype) (#v:eqtype) #f = ()

private val dom_empty_helper: #k:eqtype -> #v:eqtype -> #f:cmp k -> m:ordmap k v f
                      -> Lemma (requires (True))
                               (ensures  ((dom #k #v #f m = OrdSet.empty) ==>
                                          (m == empty #k #v #f)))
let dom_empty_helper (#k:eqtype) (#v:eqtype) #f m =
  let (Mk_map s g) = m in
...
let choose_m (#k:eqtype) (#v:eqtype) #f m =
...
let size_remove (#k:eqtype) (#v:eqtype) #f y m = ()

let dom_lemma (#k:eqtype) (#v:eqtype) #f x m = ()

let contains_const_on (#k:eqtype) (#v:eqtype) #f d x y = ()

let select_const_on (#k:eqtype) (#v:eqtype) #f d x y = ()

let sel_rem1 (#k:eqtype) (#v:eqtype) #f x m = ()

let sel_rem2 (#k:eqtype) (#v:eqtype) #f x x' m = ()

let rem_upd (#k:eqtype) (#v:eqtype) #f x y x' m = ()
