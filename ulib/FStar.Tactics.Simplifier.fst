...
val fa_cong: #a: Type -> #p: (a -> Type) -> #q: (a -> Type) -> (x: a -> squash (p x <==> q x)) ->
  Lemma ((forall (x: a). p x) <==> (forall (x: a). q x))
//fix, this should certainly be provable
let fa_cong #a #p #q f = admit ()

val ex_cong: #a: Type -> #p: (a -> Type) -> #q: (a -> Type) -> (x: a -> squash (p x <==> q x)) ->
  Lemma ((exists (x: a). p x) <==> (exists (x: a). q x))
//fix, this should certainly be provable
let ex_cong #a #p #q f = admit ()

val neg_cong: #p: Type -> #q: Type -> squash (p <==> q) -> Lemma (~p <==> ~q)
let neg_cong #p #q _ = ()
    ...
    (match term_as_formula' l with
      ...
      | Forall b p ->
        ...
        then apply_lemma (`lem_fa_true)
          ...
          if is_false p
          then or_else (fun () -> apply_lemma (`lem_fa_false); inhabit ()) tiff
          else tiff ()
      | Exists b p ->
        if is_false p
        then apply_lemma (`lem_ex_false)
          ...
          if is_true p
          then or_else (fun () -> apply_lemma (`lem_ex_true); inhabit ()) tiff
          else tiff ()
      | Not p ->
        if is_true p
  ...
  | Iff l r ->
    (match term_as_formula' l with
      ...
      | Exists _ _ ->
        ...
        let _ = intro () in
        simplify_point ()
      | Not _ -> apply_lemma (`neg_cong); simplify_point ()
