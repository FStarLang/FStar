...
type map (a: Type) (b: (a -> Type)) (inv: (map' a b -> Type0)) = m: map' a b {inv m}
...
abstract
let grows #a #b #inv : Preorder.preorder (map a b inv) =
...
type t r a b inv =
  //maybe grows can include the inv?
  m_rref r (map a b inv) grows
...
let value #r #a #b #inv (m: t r a b inv) (x: a) (h: HS.mem{defined m x h})
  : GTot (r: b x {contains m x r h}) = Some?.v (sel (HS.sel h m) x)
...
let extend
  (#r: rid) (#a: eqtype) (#b: (a -> Type)) (#inv: (map' a b -> Type0)) (m: t r a b inv) (x: a)
  (y: b x)
  : ST unit
    (requires
      (fun h ->
  ...
  : ST (option (b x))
    ...
    (ensures
      (fun h0 y h1 ->
          ...
          (Some? y ==>
            defined m x h1 /\ contains m x (Some?.v y) h1 /\ HST.witnessed (defined m x) /\
            HST.witnessed (contains m x (Some?.v y))))) =
  let y = sel !m x in
  match y with
  | None -> y
  | Some b -> contains_stable m x b; mr_witness m (defined m x); mr_witness m (contains m x b); y
