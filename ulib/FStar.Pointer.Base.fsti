...
val buffer_as_addr: #t: typ -> b: buffer t -> GTot (x: nat{x > 0})
...
let modifies_0 (h0: HS.mem) (h1: HS.mem) : GTot Type0 = modifies loc_none h0 h1
...
val screate: value: typ -> s: option (type_of_typ value) ->
  HST.StackInline (pointer value)
    ...
    (ensures
      (fun (h0: HS.mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ frameOf b = HS.get_tip h0 /\ modifies_0 h0 h1 /\
          (match s with
...
val ecreate: t: typ -> r: HS.rid -> s: option (type_of_typ t) ->
  HST.ST (pointer t)
    ...
    (ensures
      (fun (h0: HS.mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ frameOf b == r /\ modifies_0 h0 h1 /\
          (match s with
            | Some s' -> readable h1 b /\ gread h1 b == s'
            | _ -> True) /\ ~(is_mm b)))
  ...
  Lemma
    (requires
      (HS.fresh_frame h0 h1 /\
        ...
        (HS.get_tip h2) == (HS.get_tip h1) /\ HS.popped h2 h3))
    (ensures (modifies s h0 h3 /\ (HS.get_tip h3) == HS.get_tip h0))
...
val modifies_loc_addresses_intro: 
  ...
  Lemma
    (requires
      (HS.live_region h2 r /\ modifies (loc_union (loc_regions (Set.singleton r)) l) h1 h2 /\
        HS.modifies_ref r a h1 h2)) (ensures (modifies (loc_union (loc_addresses r a) l) h1 h2))
...
val loc_of_cloc_of_loc: l: loc ->
  Lemma (loc_of_cloc (cloc_of_loc l) == l) [SMTPat (loc_of_cloc (cloc_of_loc l))]

val cloc_of_loc_of_cloc: l: MG.loc cloc_cls ->
  Lemma (cloc_of_loc (loc_of_cloc l) == l) [SMTPat (cloc_of_loc (loc_of_cloc l))]

val loc_includes_to_cloc: l1: loc -> l2: loc ->
  Lemma (loc_includes l1 l2 <==> MG.loc_includes (cloc_of_loc l1) (cloc_of_loc l2))

val loc_disjoint_to_cloc: l1: loc -> l2: loc ->
  Lemma (loc_disjoint l1 l2 <==> MG.loc_disjoint (cloc_of_loc l1) (cloc_of_loc l2))

val modifies_to_cloc: l: loc -> h1: HS.mem -> h2: HS.mem ->
  Lemma (modifies l h1 h2 <==> MG.modifies (cloc_of_loc l) h1 h2)
