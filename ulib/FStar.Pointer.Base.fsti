...
type typ =
  ...
  | TStruct : l: struct_typ -> typ
  | TUnion : l: union_typ -> typ
  | TArray : length: array_length_t -> t: typ -> typ
  | TPointer : t: typ -> typ
  | TNPointer : t: typ -> typ
  | TBuffer : t: typ -> typ
and struct_typ' =
  (l:
    list (string * typ)
      { // C11, 6.2.5 al. 20: structs and unions must have at least one field
        Cons? l /\ List.Tot.noRepeats (List.Tot.map fst l) })
and struct_typ = { name:string; fields:struct_typ' }
...
let struct_field (l: struct_typ) : Tot eqtype = struct_field' l.fields
...
let typ_of_struct_field' (l: struct_typ') (f: struct_field' l) : Tot (t: typ{t << l}) =
...
*)
let typ_of_union_field (l: union_typ) (f: union_field l) : Tot (t: typ{t << l}) =
  typ_of_struct_field l f

let rec typ_depth (t: typ) : GTot nat =
  match t with
  | TArray _ t -> 1 + typ_depth t
  | TUnion l | TStruct l -> 1 + struct_typ_depth l.fields
  | _ -> 0

and struct_typ_depth (l: list (string * typ)) : GTot nat =
  match l with
  | [] -> 0
  | h :: l ->
    // matching like this prevents needing two units of ifuel
    let _, t = h in
...
val g_is_null_intro: t: typ ->
  Lemma (g_is_null (nullptr #t) == true) [SMTPat (g_is_null (nullptr #t))]
...
let type_of_struct_field''
  (l: struct_typ') (type_of_typ: (t: typ{t << l} -> Tot Type0)) (f: struct_field' l)
  : Tot Type0 =
  List.Tot.assoc_mem f l;
...
let type_of_struct_field'
  (l: struct_typ) (type_of_typ: (t: typ{t << l} -> Tot Type0)) (f: struct_field l)
  : Tot Type0 = type_of_struct_field'' l.fields type_of_typ f
...
let type_of_typ_array (len: array_length_t) (t: typ)
  : Lemma (type_of_typ (TArray len t) == array len (type_of_typ t))
    [SMTPat (type_of_typ (TArray len t))] = ()

let type_of_struct_field (l: struct_typ) : Tot (struct_field l -> Tot Type0) =
...
let type_of_typ_type_of_struct_field (l: struct_typ) (f: struct_field l)
  : Lemma (type_of_typ (typ_of_struct_field l f) == type_of_struct_field l f)
...
let struct_create (s: struct_typ) (l: struct_literal s)
  : Pure (struct s)
    (requires (normalize_term (struct_literal_wf s l) == true))
    (ensures (fun _ -> True)) = struct_create_fun s (fun_of_list s l)

val struct_sel_struct_create_fun: 
  l: struct_typ ->
  f: (fd: struct_field l -> Tot (type_of_struct_field l fd)) ->
  fd: struct_field l ->
  Lemma (struct_sel (struct_create_fun l f) fd == f fd)
...
val union_get_value: #l: union_typ -> v: union l -> fd: struct_field l ->
  Pure (type_of_struct_field l fd) (requires (union_get_key v == fd)) (ensures (fun _ -> True))

val union_create: l: union_typ -> fd: struct_field l -> v: type_of_struct_field l fd ->
  Tot (union l)
...
val equal: #t1: typ -> #t2: typ -> p1: pointer t1 -> p2: pointer t2 ->
  Ghost bool (requires True) (ensures (fun b -> b == true <==> t1 == t2 /\ p1 == p2))

val as_addr: #t: typ -> p: pointer t -> GTot nat

val unused_in: #value: typ -> p: pointer value -> h: HS.mem -> GTot Type0

val live: #value: typ -> h: HS.mem -> p: pointer value -> GTot Type0

val nlive: #value: typ -> h: HS.mem -> p: npointer value -> GTot Type0

val live_nlive: #value: typ -> h: HS.mem -> p: pointer value ->
  Lemma (nlive h p <==> live h p) [SMTPat (nlive h p)]

val g_is_null_nlive: #t: typ -> h: HS.mem -> p: npointer t ->
  Lemma (requires (g_is_null p)) (ensures (nlive h p)) [SMTPat (g_is_null p); SMTPat (nlive h p)]

val live_not_unused_in: #value: typ -> h: HS.mem -> p: pointer value ->
  Lemma (ensures (live h p /\ unused_in p h ==> False)) [SMTPat (live h p); SMTPat (unused_in p h)]

val gread: #value: typ -> h: HS.mem -> p: pointer value -> GTot (type_of_typ value)

val frameOf: #value: typ -> p: pointer value -> GTot HS.rid

val live_region_frameOf: #value: typ -> h: HS.mem -> p: pointer value ->
  Lemma (requires (live h p))
    (ensures (HS.live_region h (frameOf p)))
    [SMTPatOr [[SMTPat (HS.live_region h (frameOf p))]; [SMTPat (live h p)]]]
...
val is_mm: #value: typ -> p: pointer value -> GTot bool
...
val gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  GTot (p' : pointer (typ_of_struct_field l fd))

val as_addr_gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (as_addr (gfield p fd) == as_addr p))
    [SMTPat (as_addr (gfield p fd))]

val unused_in_gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l -> h: HS.mem ->
  Lemma (requires True)
    (ensures (unused_in (gfield p fd) h <==> unused_in p h))
    [SMTPat (unused_in (gfield p fd) h)]

val live_gfield: h: HS.mem -> #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (live h (gfield p fd) <==> live h p))
    [SMTPat (live h (gfield p fd))]

val gread_gfield: h: HS.mem -> #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (gread h (gfield p fd) == struct_sel (gread h p) fd))
    [SMTPatOr [[SMTPat (gread h (gfield p fd))]; [SMTPat (struct_sel (gread h p) fd)]]]

val frameOf_gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (frameOf (gfield p fd) == frameOf p))
    [SMTPat (frameOf (gfield p fd))]

val is_mm_gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True) (ensures (is_mm (gfield p fd) <==> is_mm p)) [SMTPat (is_mm (gfield p fd))]

val gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  GTot (p' : pointer (typ_of_struct_field l fd))

val as_addr_gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (as_addr (gufield p fd) == as_addr p))
    [SMTPat (as_addr (gufield p fd))]

val unused_in_gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l -> h: HS.mem ->
  Lemma (requires True)
    (ensures (unused_in (gufield p fd) h <==> unused_in p h))
    [SMTPat (unused_in (gufield p fd) h)]

val live_gufield: h: HS.mem -> #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (live h (gufield p fd) <==> live h p))
    [SMTPat (live h (gufield p fd))]

val gread_gufield: h: HS.mem -> #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires (union_get_key (gread h p) == fd))
    (ensures
      (union_get_key (gread h p) == fd /\ gread h (gufield p fd) == union_get_value (gread h p) fd))
    [SMTPatOr [[SMTPat (gread h (gufield p fd))]; [SMTPat (union_get_value (gread h p) fd)]]]

val frameOf_gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True)
    (ensures (frameOf (gufield p fd) == frameOf p))
    [SMTPat (frameOf (gufield p fd))]

val is_mm_gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True)
...
val gcell: 
  ...
  i: UInt32.t ->
  Ghost (pointer value) (requires (UInt32.v i < UInt32.v length)) (ensures (fun _ -> True))
...
val as_addr_gcell: 
  ...
  p: pointer (TArray length value) ->
  i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v length))
...
val gread_gcell: 
  ...
  Lemma (requires (UInt32.v i < UInt32.v length))
    (ensures
      (UInt32.v i < UInt32.v length /\ gread h (gcell p i) == Seq.index (gread h p) (UInt32.v i)))
...
val includes: #value1: typ -> #value2: typ -> p1: pointer value1 -> p2: pointer value2 -> GTot bool

val includes_refl: #t: typ -> p: pointer t -> Lemma (ensures (includes p p)) [SMTPat (includes p p)]
...
val includes_trans: 
  ...
  p2: pointer t2 ->
  p3: pointer t3 ->
  Lemma (requires (includes p1 p2 /\ includes p2 p3)) (ensures (includes p1 p3))

val includes_gfield: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires True) (ensures (includes p (gfield p fd)))

val includes_gufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True) (ensures (includes p (gufield p fd)))
...
val readable: #a: typ -> h: HS.mem -> b: pointer a -> GTot Type0

val readable_live: #a: typ -> h: HS.mem -> b: pointer a ->
  Lemma (requires (readable h b))
    (ensures (live h b))
    [SMTPatOr [[SMTPat (readable h b)]; [SMTPat (live h b)]]]

val readable_gfield: #l: struct_typ -> h: HS.mem -> p: pointer (TStruct l) -> fd: struct_field l ->
  Lemma (requires (readable h p))
    (ensures (readable h (gfield p fd)))
    [SMTPat (readable h (gfield p fd))]

val readable_struct: #l: struct_typ -> h: HS.mem -> p: pointer (TStruct l) ->
...
val readable_struct_forall_mem: #l: struct_typ -> p: pointer (TStruct l) ->
  Lemma (forall (h: HS.mem). (forall (f: struct_field l). readable h (gfield p f)) ==> readable h p)
...
val readable_struct_fields_nil: #l: struct_typ -> h: HS.mem -> p: pointer (TStruct l) ->
  Lemma (readable_struct_fields h p []) [SMTPat (readable_struct_fields h p [])]
...
val readable_struct_fields_cons: 
  ...
  Lemma
    (requires
      (readable_struct_fields h p q /\
        (List.Tot.mem f (List.Tot.map fst l.fields) ==>
...
val readable_struct_fields_readable_struct: #l: struct_typ -> h: HS.mem -> p: pointer (TStruct l) ->
  Lemma (requires (readable_struct_fields h p (normalize_term (List.Tot.map fst l.fields))))
...
val readable_gcell: 
  ...
  p: pointer (TArray length value) ->
  i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v length /\ readable h p))
...
val readable_array: 
  ...
  h: HS.mem ->
  p: pointer (TArray length value) ->
  Lemma (requires (forall (i: UInt32.t). UInt32.v i < UInt32.v length ==> readable h (gcell p i)))
    (ensures (readable h p))
...
val readable_gufield: #l: union_typ -> h: HS.mem -> p: pointer (TUnion l) -> fd: struct_field l ->
  Lemma (requires True)
...
val is_active_union_live: 
  ...
  p: pointer (TUnion l) ->
  fd: struct_field l ->
  Lemma (requires (is_active_union_field h p fd))
    (ensures (live h p))
...
val is_active_union_field_eq: 
  ...
  fd1: struct_field l ->
  fd2: struct_field l ->
  Lemma (requires (is_active_union_field h p fd1 /\ is_active_union_field h p fd2))
    (ensures (fd1 == fd2))
...
val is_active_union_field_readable: 
  ...
  p: pointer (TUnion l) ->
  fd: struct_field l ->
  Lemma (requires (is_active_union_field h p fd /\ readable h (gufield p fd)))
    (ensures (readable h p))
...
val is_active_union_field_includes_readable: 
  ...
  #t': typ ->
  p': pointer t' ->
  Lemma (requires (includes (gufield p fd) p' /\ readable h p'))
...
let equal_values #a h (b: pointer a) h' (b': pointer a) : GTot Type0 =
  (live h b ==> live h' b') /\ (readable h b ==> (readable h' b' /\ gread h b == gread h' b'))
...
val gsingleton_buffer_of_pointer: #t: typ -> p: pointer t -> GTot (buffer t)

val singleton_buffer_of_pointer: #t: typ -> p: pointer t ->
  HST.Stack (buffer t)
    (requires (fun h -> live h p))
    (ensures (fun h b h' -> h' == h /\ b == gsingleton_buffer_of_pointer p))

val gbuffer_of_array_pointer: #t: typ -> #length: array_length_t -> p: pointer (TArray length t) ->
  GTot (buffer t)

val buffer_of_array_pointer: #t: typ -> #length: array_length_t -> p: pointer (TArray length t) ->
  HST.Stack (buffer t)
    (requires (fun h -> live h p))
...
val buffer_length_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t ->
  Lemma (requires True)
...
val buffer_live: #t: typ -> h: HS.mem -> b: buffer t -> GTot Type0

val buffer_live_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t -> h: HS.mem ->
...
val buffer_unused_in: #t: typ -> b: buffer t -> h: HS.mem -> GTot Type0

val buffer_live_not_unused_in: #t: typ -> b: buffer t -> h: HS.mem ->
  Lemma ((buffer_live h b /\ buffer_unused_in b h) ==> False)

val buffer_unused_in_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t -> h: HS.mem ->
  Lemma (ensures (buffer_unused_in (gsingleton_buffer_of_pointer p) h <==> unused_in p h))
...
val frameOf_buffer: #t: typ -> b: buffer t -> GTot HS.rid

val frameOf_buffer_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t ->
  Lemma (ensures (frameOf_buffer (gsingleton_buffer_of_pointer p) == frameOf p))
...
val frameOf_buffer_gbuffer_of_array_pointer: 
  ...
  #length: array_length_t ->
  p: pointer (TArray length t) ->
  Lemma (ensures (frameOf_buffer (gbuffer_of_array_pointer p) == frameOf p))
    [SMTPat (frameOf_buffer (gbuffer_of_array_pointer p))]

val live_region_frameOf_buffer: #value: typ -> h: HS.mem -> p: buffer value ->
  Lemma (requires (buffer_live h p))
    (ensures (HS.live_region h (frameOf_buffer p)))
    [SMTPatOr [[SMTPat (HS.live_region h (frameOf_buffer p))]; [SMTPat (buffer_live h p)]]]

val buffer_as_addr: #t: typ -> b: buffer t -> GTot nat

val buffer_as_addr_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t ->
  Lemma (ensures (buffer_as_addr (gsingleton_buffer_of_pointer p) == as_addr p))
...
val buffer_as_addr_gbuffer_of_array_pointer: 
  ...
  #length: array_length_t ->
  p: pointer (TArray length t) ->
  Lemma (ensures (buffer_as_addr (gbuffer_of_array_pointer p) == as_addr p))
    [SMTPat (buffer_as_addr (gbuffer_of_array_pointer p))]

val gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Ghost (buffer t)
    (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures (fun _ -> True))

val frameOf_buffer_gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
...
val buffer_as_addr_gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
...
val sub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  HST.Stack (buffer t)
    (requires (fun h -> UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ buffer_live h b))
    (ensures
      (fun h b' h' ->
          UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ h' == h /\
          b' == gsub_buffer b i len))

val offset_buffer: #t: typ -> b: buffer t -> i: UInt32.t ->
  HST.Stack (buffer t)
    (requires (fun h -> UInt32.v i <= UInt32.v (buffer_length b) /\ buffer_live h b))
    (ensures
      (fun h b' h' ->
          UInt32.v i <= UInt32.v (buffer_length b) /\ h' == h /\
          b' == gsub_buffer b i (UInt32.sub (buffer_length b) i)))

val buffer_length_gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        buffer_length (gsub_buffer b i len) == len))
...
val buffer_live_gsub_buffer_equiv: 
  ...
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        (buffer_live h (gsub_buffer b i len) <==> buffer_live h b)))
...
val buffer_live_gsub_buffer_intro: 
  ...
  len: UInt32.t ->
  h: HS.mem ->
  Lemma (requires (buffer_live h b /\ UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        buffer_live h (gsub_buffer b i len)))
    [SMTPat (buffer_live h (gsub_buffer b i len))]
...
val buffer_unused_in_gsub_buffer: 
  ...
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        (buffer_unused_in (gsub_buffer b i len) h <==> buffer_unused_in b h)))
...
val gsub_buffer_zero_buffer_length: #a: typ -> b: buffer a ->
  Lemma (ensures (gsub_buffer b 0ul (buffer_length b) == b))
    [SMTPat (gsub_buffer b 0ul (buffer_length b))]

val buffer_as_seq: #t: typ -> h: HS.mem -> b: buffer t -> GTot (Seq.seq (type_of_typ t))

val buffer_length_buffer_as_seq: #t: typ -> h: HS.mem -> b: buffer t ->
  Lemma (requires True)
    (ensures (Seq.length (buffer_as_seq h b) == UInt32.v (buffer_length b)))
    [SMTPat (Seq.length (buffer_as_seq h b))]

val buffer_as_seq_gsingleton_buffer_of_pointer: #t: typ -> h: HS.mem -> p: pointer t ->
  Lemma (requires True)
...
val buffer_as_seq_gsub_buffer: 
  ...
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        buffer_as_seq h (gsub_buffer b i len) ==
        Seq.slice (buffer_as_seq h b) (UInt32.v i) (UInt32.v i + UInt32.v len)))
    [SMTPat (buffer_as_seq h (gsub_buffer b i len))]

val gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t ->
  Ghost (pointer t) (requires (UInt32.v i < UInt32.v (buffer_length b))) (ensures (fun _ -> True))

val pointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t ->
  HST.Stack (pointer t)
    (requires (fun h -> UInt32.v i < UInt32.v (buffer_length b) /\ buffer_live h b))
    (ensures
      (fun h p h' ->
          UInt32.v i < UInt32.v (buffer_length b) /\ h' == h /\ p == gpointer_of_buffer_cell b i))
...
let gpointer_of_buffer_cell_gsub_buffer'
  (#t: typ) (b: buffer t) (i1: UInt32.t) (len: UInt32.t) (i2: UInt32.t)
  : Lemma
    (requires
      (UInt32.v i1 + UInt32.v len <= UInt32.v (buffer_length b) /\ UInt32.v i2 < UInt32.v len))
    (ensures
...
val live_gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t -> h: HS.mem ->
...
val gpointer_of_buffer_cell_gsingleton_buffer_of_pointer: #t: typ -> p: pointer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < 1))
  ...
  Lemma (requires (UInt32.v i < UInt32.v length))
    (ensures
      (UInt32.v i < UInt32.v length /\
        gpointer_of_buffer_cell (gbuffer_of_array_pointer p) i == gcell p i))
    [SMTPat (gpointer_of_buffer_cell (gbuffer_of_array_pointer p) i)]

val frameOf_gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
        frameOf (gpointer_of_buffer_cell b i) == frameOf_buffer b))
    [SMTPat (frameOf (gpointer_of_buffer_cell b i))]

val as_addr_gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
        as_addr (gpointer_of_buffer_cell b i) == buffer_as_addr b))
    [SMTPat (as_addr (gpointer_of_buffer_cell b i))]

val gread_gpointer_of_buffer_cell: #t: typ -> h: HS.mem -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
        gread h (gpointer_of_buffer_cell b i) == Seq.index (buffer_as_seq h b) (UInt32.v i)))
...
val index_buffer_as_seq: #t: typ -> h: HS.mem -> b: buffer t -> i: nat ->
  Lemma (requires (i < UInt32.v (buffer_length b)))
    (ensures
      (i < UInt32.v (buffer_length b) /\
        Seq.index (buffer_as_seq h b) i == gread h (gpointer_of_buffer_cell b (UInt32.uint_to_t i)))
    )
...
val gsingleton_buffer_of_pointer_gcell: 
  ...
  p: pointer (TArray len t) ->
  i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v len))
    (ensures
...
val gsingleton_buffer_of_pointer_gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
...
val buffer_readable: #t: typ -> h: HS.mem -> b: buffer t -> GTot Type0

val buffer_readable_buffer_live: #t: typ -> h: HS.mem -> b: buffer t ->
  Lemma (requires (buffer_readable h b))
    (ensures (buffer_live h b))
    [SMTPatOr [[SMTPat (buffer_readable h b)]; [SMTPat (buffer_live h b)]]]

val buffer_readable_gsingleton_buffer_of_pointer: #t: typ -> h: HS.mem -> p: pointer t ->
  Lemma (ensures (buffer_readable h (gsingleton_buffer_of_pointer p) <==> readable h p))
...
val buffer_readable_gsub_buffer: 
  ...
  i: UInt32.t ->
  len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ buffer_readable h b))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        buffer_readable h (gsub_buffer b i len)))
    [SMTPat (buffer_readable h (gsub_buffer b i len))]

val readable_gpointer_of_buffer_cell: #t: typ -> h: HS.mem -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b) /\ buffer_readable h b))
...
val buffer_readable_intro: #t: typ -> h: HS.mem -> b: buffer t ->
  Lemma
    (requires
      (buffer_live h b /\
        (forall (i: UInt32.t).
...
val buffer_readable_elim: #t: typ -> h: HS.mem -> b: buffer t ->
  Lemma (requires (buffer_readable h b))
    (ensures
      (buffer_live h b /\
        (forall (i: UInt32.t).
...
val loc_union: s1: loc -> s2: loc -> GTot loc
...
val loc_pointer: #t: typ -> p: pointer t -> GTot loc

val loc_buffer: #t: typ -> b: buffer t -> GTot loc

val loc_addresses: r: HS.rid -> n: Set.set nat -> GTot loc

val loc_regions: r: Set.set HS.rid -> GTot loc

(* Inclusion of memory locations *)

val loc_includes: s1: loc -> s2: loc -> GTot Type0

val loc_includes_refl: s: loc -> Lemma (loc_includes s s) [SMTPat (loc_includes s s)]

val loc_includes_trans: s1: loc -> s2: loc -> s3: loc ->
  Lemma (requires (loc_includes s1 s2 /\ loc_includes s2 s3)) (ensures (loc_includes s1 s3))

val loc_includes_union_r: s: loc -> s1: loc -> s2: loc ->
...
val loc_includes_union_l: s1: loc -> s2: loc -> s: loc ->
...
val loc_includes_pointer_pointer: #t1: typ -> #t2: typ -> p1: pointer t1 -> p2: pointer t2 ->
  Lemma (requires (includes p1 p2))
    (ensures (loc_includes (loc_pointer p1) (loc_pointer p2)))
    [SMTPat (loc_includes (loc_pointer p1) (loc_pointer p2))]

val loc_includes_gsingleton_buffer_of_pointer: l: loc -> #t: typ -> p: pointer t ->
  Lemma (requires (loc_includes l (loc_pointer p)))
...
val loc_includes_gpointer_of_array_cell: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t ->
  Lemma (requires (UInt32.v i < UInt32.v (buffer_length b) /\ loc_includes l (loc_buffer b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
        loc_includes l (loc_pointer (gpointer_of_buffer_cell b i))))
    [SMTPat (loc_includes l (loc_pointer (gpointer_of_buffer_cell b i)))]

val loc_includes_gsub_buffer_r: l: loc -> #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\ loc_includes l (loc_buffer b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
        loc_includes l (loc_buffer (gsub_buffer b i len))))
...
val loc_includes_gsub_buffer_l: 
  ...
  Lemma
    (requires
      (UInt32.v i1 + UInt32.v len1 <= UInt32.v (buffer_length b) /\ UInt32.v i1 <= UInt32.v i2 /\
        UInt32.v i2 + UInt32.v len2 <= UInt32.v i1 + UInt32.v len1))
    (ensures
      (UInt32.v i1 + UInt32.v len1 <= UInt32.v (buffer_length b) /\ UInt32.v i1 <= UInt32.v i2 /\
        UInt32.v i2 + UInt32.v len2 <= UInt32.v i1 + UInt32.v len1 /\
        loc_includes (loc_buffer (gsub_buffer b i1 len1)) (loc_buffer (gsub_buffer b i2 len2))))
    [
      SMTPat
      (loc_includes (loc_buffer (gsub_buffer b i1 len1)) (loc_buffer (gsub_buffer b i2 len2)))
    ]

val loc_includes_addresses_pointer: #t: typ -> r: HS.rid -> s: Set.set nat -> p: pointer t ->
...
val loc_includes_addresses_buffer: #t: typ -> r: HS.rid -> s: Set.set nat -> p: buffer t ->
  Lemma (requires (frameOf_buffer p == r /\ Set.mem (buffer_as_addr p) s))
    (ensures (loc_includes (loc_addresses r s) (loc_buffer p)))
    [SMTPat (loc_includes (loc_addresses r s) (loc_buffer p))]

val loc_includes_region_pointer: #t: typ -> s: Set.set HS.rid -> p: pointer t ->
  Lemma (requires (Set.mem (frameOf p) s))
    (ensures (loc_includes (loc_regions s) (loc_pointer p)))
    [SMTPat (loc_includes (loc_regions s) (loc_pointer p))]

val loc_includes_region_buffer: #t: typ -> s: Set.set HS.rid -> b: buffer t ->
  Lemma (requires (Set.mem (frameOf_buffer b) s))
    (ensures (loc_includes (loc_regions s) (loc_buffer b)))
    [SMTPat (loc_includes (loc_regions s) (loc_buffer b))]

val loc_includes_region_addresses: s: Set.set HS.rid -> r: HS.rid -> a: Set.set nat ->
...
val loc_includes_region_region: s1: Set.set HS.rid -> s2: Set.set HS.rid ->
  Lemma (requires (Set.subset s2 s1))
    (ensures (loc_includes (loc_regions s1) (loc_regions s2)))
    [SMTPat (loc_includes (loc_regions s1) (loc_regions s2))]

val loc_includes_region_union_l: l: loc -> s1: Set.set HS.rid -> s2: Set.set HS.rid ->
  Lemma (requires (loc_includes l (loc_regions (Set.intersect s2 (Set.complement s1)))))
...
val loc_disjoint: s1: loc -> s2: loc -> GTot Type0

val loc_disjoint_sym: s1: loc -> s2: loc ->
  Lemma (requires (loc_disjoint s1 s2)) (ensures (loc_disjoint s2 s1)) [SMTPat (loc_disjoint s1 s2)]

val loc_disjoint_none_r: s: loc ->
  Lemma (ensures (loc_disjoint s loc_none)) [SMTPat (loc_disjoint s loc_none)]

val loc_disjoint_union_r: s: loc -> s1: loc -> s2: loc ->
  Lemma (requires (loc_disjoint s s1 /\ loc_disjoint s s2))
    (ensures (loc_disjoint s (loc_union s1 s2)))
    [SMTPat (loc_disjoint s (loc_union s1 s2))]

val loc_disjoint_root: #value1: typ -> #value2: typ -> p1: pointer value1 -> p2: pointer value2 ->
...
val loc_disjoint_includes: p1: loc -> p2: loc -> p1': loc -> p2': loc ->
  Lemma (requires (loc_includes p1 p1' /\ loc_includes p2 p2' /\ loc_disjoint p1 p2))
...
val pointer_live_reference_unused_in_disjoint: 
  ...
  Lemma (requires (live h p1 /\ HS.unused_in p2 h))
    (ensures
      (loc_disjoint (loc_pointer p1) (loc_addresses (HS.frameOf p2) (Set.singleton (HS.as_addr p2)))
      ))
...
val reference_live_pointer_unused_in_disjoint: 
  ...
  Lemma (requires (HS.contains h p1 /\ unused_in p2 h))
    (ensures
      (loc_disjoint (loc_addresses (HS.frameOf p1) (Set.singleton (HS.as_addr p1))) (loc_pointer p2)
      ))
  ...
  Lemma
    (requires
      (UInt32.v i1 + UInt32.v len1 <= UInt32.v (buffer_length b) /\
        ...
        (UInt32.v i1 + UInt32.v len1 <= UInt32.v i2 \/ UInt32.v i2 + UInt32.v len2 <= UInt32.v i1)))
    (ensures
      (UInt32.v i1 + UInt32.v len1 <= UInt32.v (buffer_length b) /\
        UInt32.v i2 + UInt32.v len2 <= UInt32.v (buffer_length b) /\
...
val loc_disjoint_gpointer_of_buffer_cell: #t: typ -> b: buffer t -> i1: UInt32.t -> i2: UInt32.t ->
  Lemma
    (requires
      (UInt32.v i1 < UInt32.v (buffer_length b) /\ UInt32.v i2 < UInt32.v (buffer_length b) /\
        (UInt32.v i1 <> UInt32.v i2)))
    (ensures
      (UInt32.v i1 < UInt32.v (buffer_length b) /\ UInt32.v i2 < UInt32.v (buffer_length b) /\
        loc_disjoint (loc_pointer (gpointer_of_buffer_cell b i1))
          (loc_pointer (gpointer_of_buffer_cell b i2))))
    [
      SMTPat
      (loc_disjoint (loc_pointer (gpointer_of_buffer_cell b i1))
          (loc_pointer (gpointer_of_buffer_cell b i2)))
...
val loc_disjoint_addresses: r1: HS.rid -> r2: HS.rid -> n1: Set.set nat -> n2: Set.set nat ->
  Lemma (requires (r1 <> r2 \/ Set.subset (Set.intersect n1 n2) Set.empty))
    (ensures (loc_disjoint (loc_addresses r1 n1) (loc_addresses r2 n2)))
    [SMTPat (loc_disjoint (loc_addresses r1 n1) (loc_addresses r2 n2))]

val loc_disjoint_pointer_addresses: #t: typ -> p: pointer t -> r: HS.rid -> n: Set.set nat ->
  Lemma (requires (r <> frameOf p \/ (~(Set.mem (as_addr p) n))))
    (ensures (loc_disjoint (loc_pointer p) (loc_addresses r n)))
    [SMTPat (loc_disjoint (loc_pointer p) (loc_addresses r n))]

val loc_disjoint_buffer_addresses: #t: typ -> p: buffer t -> r: HH.rid -> n: Set.set nat ->
  Lemma (requires (r <> frameOf_buffer p \/ (~(Set.mem (buffer_as_addr p) n))))
    (ensures (loc_disjoint (loc_buffer p) (loc_addresses r n)))
    [SMTPat (loc_disjoint (loc_buffer p) (loc_addresses r n))]

val loc_disjoint_regions: rs1: Set.set HS.rid -> rs2: Set.set HS.rid ->
  Lemma (requires (Set.subset (Set.intersect rs1 rs2) Set.empty))
...
val modifies: s: loc -> h1: HS.mem -> h2: HS.mem -> GTot Type0

val modifies_loc_regions_intro: rs: Set.set HS.rid -> h1: HS.mem -> h2: HS.mem ->
  Lemma (requires (HS.modifies rs h1 h2)) (ensures (modifies (loc_regions rs) h1 h2))

val modifies_pointer_elim: s: loc -> h1: HS.mem -> h2: HS.mem -> #a': typ -> p': pointer a' ->
  Lemma (requires (modifies s h1 h2 /\ live h1 p' /\ loc_disjoint (loc_pointer p') s))
...
val modifies_buffer_elim: #t1: typ -> b: buffer t1 -> p: loc -> h: HS.mem -> h': HS.mem ->
  Lemma
    (requires
      (loc_disjoint (loc_buffer b) p /\ buffer_live h b /\
        // necessary for liveness, because all buffers of size 0 are disjoint for any memory location, so we cannot talk about their liveness individually without referring to a larger nonempty buffer
        (UInt32.v (buffer_length b) == 0 ==> buffer_live h' b) /\ modifies p h h'))
    (ensures
      (buffer_live h' b /\
...
val modifies_reference_elim: #t: Type0 -> b: HS.reference t -> p: loc -> h: HS.mem -> h': HS.mem ->
  Lemma
    (requires
      (loc_disjoint (loc_addresses (HS.frameOf b) (Set.singleton (HS.as_addr b))) p /\
        HS.contains h b /\ modifies p h h'))
...
val modifies_loc_includes: s1: loc -> h: HS.mem -> h': HS.mem -> s2: loc ->
  Lemma (requires (modifies s2 h h' /\ loc_includes s1 s2))
    (ensures (modifies s1 h h'))
    [SMTPat (modifies s1 h h'); SMTPat (modifies s2 h h')]

val modifies_regions_elim: rs: Set.set HS.rid -> h: HS.mem -> h': HS.mem ->
  Lemma (requires (modifies (loc_regions rs) h h')) (ensures (HS.modifies rs h h'))

val modifies_addresses_elim: r: HS.rid -> a: Set.set nat -> l: loc -> h: HS.mem -> h': HS.mem ->
  Lemma
    (requires
      (modifies (loc_union (loc_addresses r a) l) h h' /\
        loc_disjoint (loc_regions (Set.singleton r)) l /\ HS.live_region h r))
    (ensures (HS.modifies_ref r a h h'))

val modifies_trans: s12: loc -> h1: HS.mem -> h2: HS.mem -> s23: loc -> h3: HS.mem ->
  Lemma (requires (modifies s12 h1 h2 /\ modifies s23 h2 h3))
    (ensures (modifies (loc_union s12 s23) h1 h3))
    [SMTPat (modifies s12 h1 h2); SMTPat (modifies s23 h2 h3)]

let modifies_0 (h0: HS.mem) (h1: HS.mem) : GTot Type0 =
  modifies (loc_addresses h0.HS.tip Set.empty) h0 h1

let modifies_1 (#t: typ) (p: pointer t) (h0: HS.mem) (h1: HS.mem) : GTot Type0 =
...
val screate: value: typ -> s: option (type_of_typ value) ->
  HST.StackInline (pointer value)
    (requires (fun h -> True))
    (ensures
      (fun (h0: HS.mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ frameOf b = h0.HS.tip /\ modifies_0 h0 h1 /\
          (match s with
            | Some s' -> readable h1 b /\ gread h1 b == s'
            | _ -> True)))

val ecreate: t: typ -> r: HS.rid -> s: option (type_of_typ t) ->
  HST.ST (pointer t)
    (requires (fun h -> is_eternal_region r /\ HST.witnessed (region_contains_pred r)))
    (ensures
      (fun (h0: HS.mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ frameOf b == r /\
          modifies (loc_addresses r Set.empty) h0 h1 /\
          (match s with
            | Some s' -> readable h1 b /\ gread h1 b == s'
            | _ -> True) /\ ~(is_mm b)))

val field: #l: struct_typ -> p: pointer (TStruct l) -> fd: struct_field l ->
  HST.Stack (pointer (typ_of_struct_field l fd))
    (requires (fun h -> live h p))
    (ensures (fun h0 p' h1 -> h0 == h1 /\ p' == gfield p fd))

val ufield: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  HST.Stack (pointer (typ_of_struct_field l fd))
    (requires (fun h -> live h p))
...
val read: #value: typ -> p: pointer value ->
  HST.Stack (type_of_typ value)
    (requires (fun h -> readable h p))
    (ensures (fun h0 v h1 -> readable h0 p /\ h0 == h1 /\ v == gread h0 p))

val is_null: #t: typ -> p: npointer t ->
  HST.Stack bool
    (requires (fun h -> nlive h p))
    (ensures (fun h b h' -> h' == h /\ b == g_is_null p))

val write: #a: typ -> b: pointer a -> z: type_of_typ a ->
  HST.Stack unit
    (requires (fun h -> live h b))
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h1 b /\ modifies_1 b h0 h1 /\ readable h1 b /\ gread h1 b == z))
...
*)
val write_union_field: #l: union_typ -> p: pointer (TUnion l) -> fd: struct_field l ->
  HST.Stack unit
    (requires (fun h -> live h p))
    (ensures
      (fun h0 _ h1 -> live h0 p /\ live h1 p /\ modifies_1 p h0 h1 /\ is_active_union_field h1 p fd)
    )

val no_upd_fresh: h0: HS.mem -> h1: HS.mem ->
  Lemma (requires (HS.fresh_frame h0 h1))
    (ensures (modifies loc_none h0 h1))
    [SMTPat (HS.fresh_frame h0 h1)]

val no_upd_popped: #t: typ -> h0: HS.mem -> h1: HS.mem -> b: pointer t ->
  Lemma (requires (live h0 b /\ frameOf b <> h0.HS.tip /\ HS.popped h0 h1))
...
val no_upd_popped_buffer: #t: typ -> h0: HS.mem -> h1: HS.mem -> b: buffer t ->
  Lemma (requires (buffer_live h0 b /\ frameOf_buffer b <> h0.HS.tip /\ HS.popped h0 h1))
    (ensures
      (buffer_live h1 b /\
        (buffer_readable h0 b ==> (buffer_readable h1 b /\ buffer_as_seq h1 b == buffer_as_seq h0 b)
        )))
    [
      SMTPatOr
      [
...
val modifies_fresh_frame_popped: h0: HS.mem -> h1: HS.mem -> s: loc -> h2: HS.mem -> h3: HS.mem ->
  Lemma
    (requires
...
val modifies_only_live_regions: rs: Set.set HS.rid -> l: loc -> h: HS.mem -> h': HS.mem ->
  Lemma
    (requires
...
val modifies_loc_addresses_intro: 
  ...
  Lemma
    (requires
      (modifies (loc_union (loc_regions (Set.singleton r)) l) h1 h2 /\ HS.modifies_ref r a h1 h2))
...
val modifies_1_readable_struct: 
  ...
  h: HS.mem ->
  h': HS.mem ->
  Lemma (requires (readable h p /\ modifies_1 (gfield p f) h h' /\ readable h' (gfield p f)))
    (ensures (readable h' p))
    [
      SMTPatOr
      [
...
val modifies_1_readable_array: 
  ...
  Lemma
    (requires
      (UInt32.v i < UInt32.v len /\ readable h p /\ modifies_1 (gcell p i) h h' /\
...
val read_buffer: #t: typ -> b: buffer t -> i: UInt32.t ->
  HST.Stack (type_of_typ t)
    (requires
      (fun h -> UInt32.v i < UInt32.v (buffer_length b) /\ readable h (gpointer_of_buffer_cell b i))
    )
    (ensures
      (fun h v h' ->
          UInt32.v i < UInt32.v (buffer_length b) /\ h' == h /\
...
val write_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> v: type_of_typ t ->
  HST.Stack unit
    (requires (fun h -> UInt32.v i < UInt32.v (buffer_length b) /\ buffer_live h b))
    (ensures
      (fun h _ h' ->
          UInt32.v i < UInt32.v (buffer_length b) /\ modifies_1 (gpointer_of_buffer_cell b i) h h' /\
          buffer_live h' b /\ readable h' (gpointer_of_buffer_cell b i) /\
...
val buffer_live_unused_in_disjoint: 
  ...
  b1: buffer t1 ->
  b2: buffer t2 ->
  Lemma (requires (buffer_live h b1 /\ buffer_unused_in b2 h))
...
val pointer_live_buffer_unused_in_disjoint: 
  ...
  b1: pointer t1 ->
  b2: buffer t2 ->
  Lemma (requires (live h b1 /\ buffer_unused_in b2 h))
...
val buffer_live_pointer_unused_in_disjoint: 
  ...
  b1: buffer t1 ->
  b2: pointer t2 ->
  Lemma (requires (buffer_live h b1 /\ unused_in b2 h))
...
val reference_live_buffer_unused_in_disjoint: 
  ...
  b1: HS.reference t1 ->
  b2: buffer t2 ->
  Lemma (requires (HS.contains h b1 /\ buffer_unused_in b2 h))
    (ensures
      (loc_disjoint (loc_addresses (HS.frameOf b1) (Set.singleton (HS.as_addr b1))) (loc_buffer b2))
    )
...
val buffer_live_reference_unused_in_disjoint: 
  ...
  b1: buffer t1 ->
  b2: HS.reference t2 ->
  Lemma (requires (buffer_live h b1 /\ HS.unused_in b2 h))
    (ensures
      (loc_disjoint (loc_buffer b1) (loc_addresses (HS.frameOf b2) (Set.singleton (HS.as_addr b2))))
    )
...
val root_buffer: #t: typ -> b: buffer t -> GTot (buffer t)

(* Return the "offset" of b within its root buffer *)

val buffer_idx: #t: typ -> b: buffer t ->
  Ghost UInt32.t
    (requires True)
    (ensures
      (fun y -> UInt32.v y + UInt32.v (buffer_length b) <= UInt32.v (buffer_length (root_buffer b)))
    )

val buffer_eq_gsub_root: #t: typ -> b: buffer t ->
  Lemma (b == gsub_buffer (root_buffer b) (buffer_idx b) (buffer_length b))

val root_buffer_gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
...
val buffer_idx_gsub_buffer: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
...
val buffer_includes: #t: typ -> blarge: buffer t -> bsmall: buffer t -> GTot Type0

val buffer_includes_refl: #t: typ -> b: buffer t ->
  Lemma (buffer_includes b b) [SMTPat (buffer_includes b b)]

val buffer_includes_trans: #t: typ -> b1: buffer t -> b2: buffer t -> b3: buffer t ->
  Lemma (requires (buffer_includes b1 b2 /\ buffer_includes b2 b3))
    (ensures (buffer_includes b1 b3))

val buffer_includes_gsub_r: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t ->
  Lemma (requires (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b)))
    (ensures
      (UInt32.v i + UInt32.v len <= UInt32.v (buffer_length b) /\
...
val buffer_includes_elim: #t: typ -> b1: buffer t -> b2: buffer t ->
  Lemma (requires (buffer_includes b1 b2))
    (ensures
      (UInt32.v (buffer_idx b1) <= UInt32.v (buffer_idx b2) /\
        UInt32.v (buffer_idx b2) + UInt32.v (buffer_length b2) <=
        UInt32.v (buffer_idx b1) + UInt32.v (buffer_length b1) /\
        b2 == gsub_buffer b1 (UInt32.sub (buffer_idx b2) (buffer_idx b1)) (buffer_length b2)))

val buffer_includes_loc_includes: #t: typ -> b1: buffer t -> b2: buffer t ->
  Lemma (requires (buffer_includes b1 b2))
    (ensures (loc_includes (loc_buffer b1) (loc_buffer b2)))
    [
      SMTPatOr
      [[SMTPat (buffer_includes b1 b2)]; [SMTPat (loc_includes (loc_buffer b1) (loc_buffer b2))]]
    ]
