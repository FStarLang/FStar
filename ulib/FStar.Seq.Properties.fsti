...
val lemma_mem_inversion: #a:eqtype -> s:seq a{length s > 0} -> Lemma
  (ensures (forall x. {:pattern (mem x s)} mem x s = (x=head s || mem x (tail s))))
...
val lemma_count_slice: #a:eqtype -> s:seq a -> i:nat{i<=length s} -> Lemma
  (requires True)
  (ensures (forall x. {:pattern (count x s)} count x s = count x (slice s 0 i) + count x (slice s i (length s))))
...
val lemma_mem_append : #a:eqtype -> s1:seq a -> s2:seq a
      -> Lemma (ensures (forall x. {:pattern (mem x (append s1 s2))} mem x (append s1 s2) <==> (mem x s1 || mem x s2)))

val lemma_slice_cons: #a:eqtype -> s:seq a -> i:nat -> j:nat{i < j && j <= length s}
  -> Lemma (ensures (forall x. {:pattern (mem x (slice s i j))} mem x (slice s i j) <==> (x = index s i || mem x (slice s (i + 1) j))))

val lemma_slice_snoc: #a:eqtype -> s:seq a -> i:nat -> j:nat{i < j && j <= length s}
  -> Lemma (ensures (forall x. {:pattern (mem x (slice s i j))} mem x (slice s i j) <==> (x = index s (j - 1) || mem x (slice s i (j - 1)))))

val lemma_ordering_lo_snoc: #a:eqtype -> f:tot_ord a -> s:seq a -> i:nat -> j:nat{i <= j && j < length s} -> pv:a
   -> Lemma (requires ((forall y. {:pattern (mem y (slice s i j))} mem y (slice s i j) ==> f y pv) /\ f (index s j) pv))
            (ensures ((forall y. {:pattern (mem y (slice s i (j + 1)))} mem y (slice s i (j + 1)) ==> f y pv)))

val lemma_ordering_hi_cons: #a:eqtype -> f:tot_ord a -> s:seq a -> back:nat -> len:nat{back < len && len <= length s} -> pv:a
   -> Lemma (requires ((forall y. {:pattern (mem y (slice s (back + 1) len))} mem y (slice s (back + 1) len) ==> f pv y) /\ f pv (index s back)))
            (ensures ((forall y. {:pattern (mem y (slice s back len))} mem y (slice s back len) ==> f pv y)))
...
val lemma_mem_snoc : #a:eqtype -> s:Seq.seq a -> x:a ->
   Lemma (ensures (forall y. {:pattern (mem y (snoc s x))} mem y (snoc s x) <==> mem y s \/ x=y))
...
: Pure (option a)
  (requires (forall (i:nat{ i < Seq.length l /\ i >= ctr}). {:pattern (Seq.index l i)}
               not (f (Seq.index l i) )))
  (ensures (function
            | None -> forall (i:nat{i < Seq.length l}). {:pattern (Seq.index l i)}  not (f (Seq.index l i))
...
: Pure bool
  (requires True)
  (ensures (fun b -> (b == true <==> (forall (i: nat {i < Seq.length l} ) . {:pattern (index l i)} f (index l i) == true))))
...
val lemma_contains_empty (#a:Type) : Lemma (forall (x:a). {:pattern (contains Seq.empty x)} ~ (contains Seq.empty x))

val lemma_contains_singleton (#a:Type) (x:a) : Lemma (forall (y:a). {:pattern (contains (create 1 x) y)} contains (create 1 x) y ==> y == x)
...
val contains_snoc : #a:Type -> s:Seq.seq a -> x:a ->
   Lemma (ensures (forall y. {:pattern (contains (snoc s x) y)} (snoc s x) `contains` y  <==> s `contains` y \/ x==y))
...
val append_slices (#a:Type) (s1:Seq.seq a) (s2:Seq.seq a)
   : Lemma ( Seq.equal s1 (Seq.slice (Seq.append s1 s2) 0 (Seq.length s1)) /\
             ...
             (forall (i:nat) (j:nat). {:pattern (Seq.slice s2 i j)}
                i <= j /\ j <= Seq.length s2 ==>
...
let suffix_of
  (#a: Type)
  (s_suff s: seq a)
= exists s_pref . {:pattern (append s_pref s_suff)} (s == append s_pref s_suff)
...
val mem_cons
  ...
  (s:seq a)
: Lemma
  (ensures (forall y. {:pattern (mem y (cons x s))} mem y (cons x s) <==> mem y s \/ x=y))
...
val lemma_seq_of_list_permutation (#a:eqtype) (l:list a)
  :Lemma (forall x. {:pattern (count x (seq_of_list l))} List.Tot.Base.count x l == count x (seq_of_list l))
