...
let rec first (f: ('a -> Tac 'b)) (l: list 'a) : Tac 'b =
  match l with
  | [] -> fail "no cands"
  | x :: xs -> or_else (fun () -> f x) (fun () -> first f xs)

(* TODO: memoization?. And better errors. *)
private
let rec tcresolve' (seen: list term) (fuel: int) : Tac unit =
  if fuel < 0 then fail "out of fuel";
  debug ("fuel = " ^ string_of_int fuel);
  let g = cur_goal () in
  if FStar.List.Tot.Base.existsb (term_eq g) seen then fail "loop";
  let seen = g :: seen in
  or_else (local seen fuel)
    (fun () -> or_else (global seen fuel) (fun () -> fail "could not solve constraint"))
...
and trywith (seen: list term) (fuel: int) (t: term) : Tac unit =
  ...
  seq (fun () -> apply t) (fun () -> tcresolve' seen (fuel - 1))
...
let mk_class (nm: string) : Tac unit =
  ...
  guard (List.length ctors = 1); // Must have a single constructor
  let [ctor] = ctors in
