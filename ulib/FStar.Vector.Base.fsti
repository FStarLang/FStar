...
val raw: a: Type u#a -> l: len_t -> Type u#a
...
val reveal: #a: Type -> #l: len_t -> v: raw a l -> GTot (s: S.seq a {S.length s = raw_length v})

val hide: #a: Type -> s: S.seq a {S.length s < pow2 32} -> GTot (raw a (U32.uint_to_t (S.length s)))

val hide_reveal: #a: Type -> #l: len_t -> v: raw a l ->
  Lemma (ensures (hide (reveal v) == v)) [SMTPat (reveal v)]

val reveal_hide: #a: Type -> s: S.seq a {S.length s < pow2 32} ->
  Lemma (ensures (reveal (hide s) == s)) [SMTPat (hide s)]
...
val extensionality: #a: Type -> #l: len_t -> v1: raw a l -> v2: raw a l ->
  Lemma (requires (equal v1 v2)) (ensures (v1 == v2))
...
val init: #a: Type -> l: len_t -> contents: (i: nat{i < U32.v l} -> Tot a) -> Tot (raw a l)

/// `index v i`: get the `i`th element of `v`
val index: #a: Type -> #l: len_t -> v: raw a l -> i: index_t v -> Tot a
...
val update: #a: Type -> #l: len_t -> v: raw a l -> i: index_t v -> x: a -> Tot (raw a l)
...
unfold
let op_At_Bar #a #l1 #l2 = append #a #l1 #l2
...
val sub: 
  ...
  v: raw a l ->
  i: len_t ->
  j: len_t{let open U32 in v i <= v j /\ v j <= v l} ->
  Tot (raw a U32.(j -^ i))
...
val reveal_init: #a: Type -> l: len_t -> contents: (i: nat{i < U32.v l} -> Tot a) ->
  Lemma (ensures (reveal (init l contents) == Seq.init (U32.v l) contents))
    [SMTPat (init l contents)]

val reveal_index: #a: Type -> #l: len_t -> v: raw a l -> i: index_t v ->
  Lemma (ensures (v.[ i ] == Seq.index (reveal v) (U32.v i))) [SMTPat (v.[ i ])]

val reveal_update: #a: Type -> #l: len_t -> v: raw a l -> i: index_t v -> x: a ->
  Lemma (ensures (reveal (v.[ i ] <- x) == Seq.upd (reveal v) (U32.v i) x)) [SMTPat (v.[ i ] <- x)]
...
val reveal_append: 
  ...
  v1: raw a l1 ->
  v2: raw a l2 {UInt.size U32.(v l1 + v l2) U32.n} ->
  Lemma (ensures (reveal (v1 @| v2) == Seq.append (reveal v1) (reveal v2))) [SMTPat (v1 @| v2)]
...
val reveal_sub: 
  ...
  i: len_t ->
  j: len_t{let open U32 in v i <= v j /\ v j <= v l} ->
  Lemma (ensures (reveal (sub v i j) == S.slice (reveal v) (U32.v i) (U32.v j)))
    [SMTPat (sub v i j)]
...
val t_has_eq: a: Type u#a ->
  Lemma (requires (hasEq a)) (ensures (hasEq (t a))) [SMTPat (hasEq (t a))]

/// The length of a t-vector is a dynamically computable u32
val len: #a: Type -> t a -> len_t
...
val as_raw: #a: Type -> x: t a -> raw a (len x)

/// Promote a raw vector
val from_raw: #a: Type -> #l: len_t -> v: raw a l -> x: t a {len x = l}

/// as_raw and from_raw are mutual inverses
val as_raw_from_raw: #a: Type -> #l: len_t -> v: raw a l ->
  Lemma (ensures (as_raw (from_raw v) == v)) [SMTPat (from_raw v)]

val from_raw_as_raw: #a: Type -> x: t a ->
  Lemma (ensures (from_raw (as_raw x) == x)) [SMTPat (as_raw x)]
...
unfold
let op_Array_Assignment (#a: Type) (x: t a) (i: index_t (as_raw x)) (v: a) : Tot (t a) =
  from_raw ((as_raw x).[ i ] <- v)

/// `v1 @@ v2`: appending t-vectors
unfold
let op_At_At (#a: Type) (x1: t a) (x2: t a {UInt.size (length x1 + length x2) U32.n}) : Tot (t a) =
