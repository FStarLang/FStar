...
abstract type bv_t (n:nat) = vec:seq bool{length vec = n}

assume val nat_to_bv: #n:pos -> a:nat{a < pow2 n} -> Tot (bv_t n)

(* Constants *)
abstract val zero_vec: #n:pos -> Tot (bv_t n)
let zero_vec #n = create n false
abstract val elem_vec: #n:pos -> i:nat{i < n} -> Tot (bv_t n)
let elem_vec #n i = upd (create n false) i true
abstract val ones_vec: #n:pos -> Tot (bv_t n)
let ones_vec #n = create n true

(* Bitwise operators *)
abstract val logand_vec: #n:pos -> a:bv_t n -> b:bv_t n -> Tot (bv_t n)
...
// private val logand_vec_definition: #n:pos -> a:bv_t n -> b:bv_t n -> i:nat{i < n} ->
//   Lemma (requires True)
...
// private val logxor_vec_definition: #n:pos -> a:bv_t n -> b:bv_t n -> i:nat{i < n} ->
//   Lemma (requires True)
...
// private val logor_vec_definition: #n:pos -> a:bv_t n -> b:bv_t n -> i:nat{i < n} ->
//   Lemma (requires True)
...
// private val lognot_vec_definition: #n:pos -> a:bv_t n -> i:nat{i < n} ->
//   Lemma (requires True)
...
// private val lemma_xor_bounded: m:pos -> n:nat -> x:bv_t m -> y:bv_t m ->
...
(* Shift operators *)
abstract val shift_left_vec: #n:pos -> a:bv_t n -> s:nat -> Tot (bv_t n)
let shift_left_vec #n a s =
  if s >= n then zero_vec #n
  else if s = 0 then a
  else append (slice a s n) (zero_vec #s)
  
abstract val shift_right_vec: #n:pos -> a:bv_t n -> s:nat -> Tot (bv_t n)
