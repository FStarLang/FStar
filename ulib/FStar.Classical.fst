...
val get_squashed: #b: Type -> a: Type ->
  Pure a (requires (a /\ a == squash b)) (ensures (fun _ -> True))
let get_squashed #b a =
  let p = get_proof a in
  join_squash #b p

val get_equality: #t: Type -> a: t -> b: t ->
  Pure (a == b) (requires (a == b)) (ensures (fun _ -> True))
...
val forall_intro_gtot: #a: Type -> #p: (a -> GTot Type) -> $f: (x: a -> GTot (p x)) ->
...
val lemma_forall_intro_gtot: #a: Type -> #p: (a -> GTot Type) -> $f: (x: a -> GTot (p x)) ->
  Lemma (forall (x: a). p x)
let lemma_forall_intro_gtot #a #p $f =
  let aux () : u: unit{forall x. p x} = forall_intro_gtot #a #p f in
  aux ()

val gtot_to_lemma: #a: Type -> #p: (a -> GTot Type) -> $f: (x: a -> GTot (p x)) -> x: a ->
  Lemma (p x)
let gtot_to_lemma #a #p $f x = give_proof #(p x) (return_squash (f x))

val lemma_to_squash_gtot: #a: Type -> #p: (a -> GTot Type) -> $f: (x: a -> Lemma (p x)) -> x: a ->
  GTot (squash (p x))
...
let forall_intro_2
  (#a: Type) (#b: (a -> Type)) (#p: (x: a -> b x -> GTot Type0))
...
let forall_intro_3
  ...
  (#p: (x: a -> y: b x -> z: c x y -> Type0)) ($f: (x: a -> y: b x -> z: c x y -> Lemma (p x y z)))
...
let forall_to_exists (#a: Type) (#p: (a -> Type)) (#r: Type) ($f: (x: a -> Lemma (p x ==> r)))
  : Lemma ((exists (x: a). p x) ==> r) = forall_intro f

let forall_to_exists_2
  (#a: Type) (#p: (a -> Type)) (#b: Type) (#q: (b -> Type)) (#r: Type)
  ($f: (x: a -> y: b -> Lemma ((p x /\ q y) ==> r)))
  : Lemma (((exists (x: a). p x) /\ (exists (y: b). q y)) ==> r) = forall_intro_2 f
...
val exists_elim: 
  ...
  #a: Type ->
  #p: (a -> Type) ->
  $have: squash (exists (x: a). p x) ->
  f: (x: a{p x} -> GTot (squash goal)) ->
  Lemma goal
...
let or_elim
  (#l: Type0) (#r: Type0) (#goal: (squash (l \/ r) -> Tot Type0))
  (hl: (squash l -> Lemma (goal ()))) (hr: (squash r -> Lemma (goal ())))
  : Lemma ((l \/ r) ==> goal ()) =
