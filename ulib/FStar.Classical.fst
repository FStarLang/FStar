...
val arrow_to_impl : #a:prop -> #b:prop -> f:(squash a -> GTot (squash b)) -> GTot (a ==> b)
...
val lemma_to_squash_gtot  : #a:Type -> #p:(a -> GTot prop) -> $f:(x:a -> Lemma (p x)) -> x:a -> GTot (squash (p x))
let lemma_to_squash_gtot #a #p $f x = f x; get_proof (p x)

val forall_intro_squash_gtot  : #a:Type -> #p:(a -> GTot prop) -> $f:(x:a -> GTot (squash (p x))) -> Tot (squash (forall (x:a). p x))
...
let forall_intro_2 (#a:Type) (#b:(a -> Type)) (#p:(x:a -> b x -> GTot prop))
...
let impl_intro_gtot (#p #q:prop) ($f:p -> GTot q) : GTot (p ==> q) = return_squash f
...
val excluded_middle : p:prop -> Lemma (requires (True))
                                       (ensures (p \/ ~p))
