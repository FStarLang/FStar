...
(* monadic bind *)
let __bind (a: Type) (b: Type) (r1: range) (r2: range) (t1: __tac a) (t2: (a -> __tac b)) : __tac b =
  fun ps ->
    ...
    | Success a ps' ->
      ...
      (match tracepoint ps' with | () -> t2 a (decr_depth ps'))
 ...
 *)
unfold
let g_bind (a: Type) (b: Type) (wp: __tac_wp a) (f: (a -> __tac_wp b)) =
  fun ps post ->
    wp ps
      (function
...
unfold
let g_compact (a: Type) (wp: __tac_wp a) : __tac_wp a =
  fun ps post ->
    forall k. (forall (r: __result a). {:pattern (guard_free (k r))} post r ==> k r) ==> wp ps k

unfold
let __TAC_eff_override_bind_wp
  (r: range) (a: Type) (b: Type) (wp: __tac_wp a) (f: (a -> __tac_wp b)) =
  g_compact b (g_bind a b wp f)

(* total  *)
//disable the termination check, although it remains reifiable
...
effect TacF (a: Type) = TAC a (fun _ _ -> False)

unfold
let lift_div_tac (a: Type) (wp: pure_wp a) : __tac_wp a = fun ps p -> wp (fun x -> p (Success x ps))

sub_effect DIV ~> TAC { lift_wp = lift_div_tac }

abstract
let __by_tactic (t: __tac 'a) (p: Type) : Type = p

unfold
let by_tactic (t: (unit -> Tac 'a)) (p: Type) : Type = __by_tactic (reify (t ())) p
...
let assert_by_tactic (p: Type) (t: (unit -> Tac unit))
  : Pure unit (requires (by_tactic t (squash p))) (ensures (fun _ -> p)) = ()

(* We don't peel off all `by_tactic`s in negative positions, so give the SMT a way to reason about them *)
val by_tactic_seman: a: Type -> tau: (unit -> Tac a) -> phi: Type ->
  Lemma (by_tactic tau phi ==> phi) [SMTPat (by_tactic tau phi)]
let by_tactic_seman a tau phi = ()

// TcTerm needs these two names typecheck tactics against
// we don't care if the tactic is satisfiable before running it
private
let tactic a = unit -> TacF a
private
