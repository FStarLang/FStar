...
effect TacH
  (a: Type)
  (pre: (proofstate -> Tot Type0))
  (post: (proofstate -> __result a -> Tot Type0))
=
...
val by_tactic_seman: a: Type -> tau: (unit -> Tac a) -> phi: Type ->
  Lemma (with_tactic tau phi ==> phi) [SMTPat (with_tactic tau phi)]
...
let assume_safe #a tau =
  admit ();
  tau ()
private
let tactic a = unit -> Tac a
