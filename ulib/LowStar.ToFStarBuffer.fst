...
assume
val new_to_old_ghost: #t: Type0 -> b: New.buffer t -> GTot (Old.buffer t)
assume
val old_to_new_ghost: #t: Type0 -> b: Old.buffer t -> GTot (New.buffer t)
assume
val new_to_old_st: #t: Type0 -> b: New.buffer t ->
  HST.Stack (b': Old.buffer t {b' == new_to_old_ghost b})
    (requires (fun h -> New.live h b))
    (ensures (fun h b' h' -> h' == h))
assume
val old_to_new_st: #t: Type0 -> b: Old.buffer t ->
  HST.Stack (b': New.buffer t {b' == old_to_new_ghost b})
    (requires (fun h -> Old.live h b))
    (ensures (fun h b' h' -> h' == h))
assume
val new_to_old_to_new: #t: Type0 -> b: New.buffer t ->
  Lemma (old_to_new_ghost (new_to_old_ghost b) == b)
    [SMTPat (old_to_new_ghost (new_to_old_ghost b))]
assume
val old_to_new_to_old: #t: Type0 -> b: Old.buffer t ->
  Lemma (new_to_old_ghost (old_to_new_ghost b) == b)
    [SMTPat (new_to_old_ghost (old_to_new_ghost b))]
assume
val live_new_to_old: #t: Type0 -> h: HS.mem -> b: New.buffer t ->
  Lemma (Old.live h (new_to_old_ghost b) <==> New.live h b)
    [SMTPat (Old.live h (new_to_old_ghost b))]
assume
val live_old_to_new: #t: Type0 -> h: HS.mem -> b: Old.buffer t ->
  Lemma (New.live h (old_to_new_ghost b) <==> Old.live h b)
    [SMTPat (New.live h (old_to_new_ghost b))]
assume
val frameOf_new_to_old: #t: Type0 -> b: New.buffer t ->
  Lemma (Old.frameOf (new_to_old_ghost b) == New.frameOf b)
    [SMTPat (Old.frameOf (new_to_old_ghost b))]
assume
val frameOf_old_to_new: #t: Type0 -> b: Old.buffer t ->
  Lemma (New.frameOf (old_to_new_ghost b) == Old.frameOf b)
    [SMTPat (New.frameOf (old_to_new_ghost b))]
assume
val as_addr_new_to_old: #t: Type0 -> b: New.buffer t ->
  Lemma (Old.as_addr (new_to_old_ghost b) == New.as_addr b)
    [SMTPat (Old.as_addr (new_to_old_ghost b))]
assume
val as_addr_old_to_new: #t: Type0 -> b: Old.buffer t ->
  Lemma (New.as_addr (old_to_new_ghost b) == Old.as_addr b)
    [SMTPat (New.as_addr (old_to_new_ghost b))]
assume
val length_new_to_old: #t: Type0 -> b: New.buffer t ->
  Lemma (Old.length (new_to_old_ghost b) == New.length b) [SMTPat (Old.length (new_to_old_ghost b))]
assume
val length_old_to_new: #t: Type0 -> b: Old.buffer t ->
  Lemma (New.length (old_to_new_ghost b) == Old.length b) [SMTPat (New.length (old_to_new_ghost b))]
assume
val as_seq_new_to_old: #t: Type0 -> h: HS.mem -> b: New.buffer t ->
  Lemma (Old.as_seq h (new_to_old_ghost b) == New.as_seq h b)
    [SMTPat (Old.as_seq h (new_to_old_ghost b))]
assume
val as_seq_old_to_new: #t: Type0 -> h: HS.mem -> b: Old.buffer t ->
  Lemma (New.as_seq h (old_to_new_ghost b) == Old.as_seq h b)
    [SMTPat (New.as_seq h (old_to_new_ghost b))]
assume
val gsub_new_to_old: #t: Type0 -> b: New.buffer t -> i: U32.t -> len: U32.t ->
  Lemma (requires (U32.v i + U32.v len <= New.length b))
...
assume
val gsub_old_to_new: #t: Type0 -> b: Old.buffer t -> i: U32.t -> len: U32.t ->
...
assume
val new_to_old_includes_left: #t: Type0 -> b1: New.buffer t -> b2: Old.buffer t ->
  Lemma ((Old.includes (new_to_old_ghost b1) b2) <==> (New.includes b1 (old_to_new_ghost b2)))
    [
      SMTPatOr
      [
        [SMTPat (Old.includes (new_to_old_ghost b1) b2)];
        [SMTPat (New.includes b1 (old_to_new_ghost b2))]
      ]
    ]
assume
val new_to_old_includes_right: #t: Type0 -> b1: Old.buffer t -> b2: New.buffer t ->
  Lemma ((Old.includes b1 (new_to_old_ghost b2)) <==> (New.includes (old_to_new_ghost b1) b2))
    [
      SMTPatOr
      [
        [SMTPat (Old.includes b1 (new_to_old_ghost b2))];
        [SMTPat (New.includes (old_to_new_ghost b1) b2)]
      ]
    ]
assume
val new_to_old_disjoint: #t1: Type0 -> #t2: Type0 -> b1: New.buffer t1 -> b2: Old.buffer t2 ->
  Lemma ((Old.disjoint (new_to_old_ghost b1) b2) <==> (New.disjoint b1 (old_to_new_ghost b2)))
    [
      SMTPatOr
      [
        [SMTPat (Old.disjoint (new_to_old_ghost b1) b2)];
        [SMTPat (New.disjoint b1 (old_to_new_ghost b2))]
      ]
    ]
...
let old_to_new_modifies (old_l: OldM.loc) (new_l: NewM.loc) (h: HS.mem) (h': HS.mem)
  : Lemma (requires (OldM.modifies old_l h h' /\ old_to_union_loc old_l == new_to_union_loc new_l))
...
let old_to_union_loc_union (old1: OldM.loc) (old2: OldM.loc)
  : Lemma
    (old_to_union_loc (OldM.loc_union old1 old2) ==
      M.loc_union (old_to_union_loc old1) (old_to_union_loc old2))
    [SMTPat (old_to_union_loc (OldM.loc_union old1 old2))] =
...
let new_to_union_loc_union (new1: NewM.loc) (new2: NewM.loc)
  : Lemma
    (new_to_union_loc (NewM.loc_union new1 new2) ==
      M.loc_union (new_to_union_loc new1) (new_to_union_loc new2))
    [SMTPat (new_to_union_loc (NewM.loc_union new1 new2))] =
  NewM.cloc_of_loc_union new1 new2;
  M.raise_loc_union (NewM.cloc_of_loc new1) (NewM.cloc_of_loc new2);
  M.union_loc_of_loc_union old_and_new_cl
    true
    (M.raise_loc (NewM.cloc_of_loc new1))
    (M.raise_loc (NewM.cloc_of_loc new2))

let old_to_union_loc_addresses (preserve_liveness: bool) (r: HS.rid) (n: Set.set nat)
  : Lemma
...
let new_to_union_loc_addresses (preserve_liveness: bool) (r: HS.rid) (n: Set.set nat)
  : Lemma
...
let old_to_union_loc_regions (preserve_liveness: bool) (r: Set.set HS.rid)
  : Lemma
...
let new_to_union_loc_regions (preserve_liveness: bool) (r: Set.set HS.rid)
  : Lemma
...
let union_loc_to_new_new_to_union_loc (l: NewM.loc)
...
let coerce (t2: Type) (#t1: Type) (x: t1)
  : Pure t2 (requires (t1 == t2)) (ensures (fun y -> x == y)) = x

let union_loc_to_new_union (l1: M.loc old_and_new_cl_union) (l2: M.loc old_and_new_cl_union)
  : Lemma
    (union_loc_to_new (M.loc_union l1 l2) ==
      NewM.loc_union (union_loc_to_new l1) (union_loc_to_new l2))
    [SMTPat (union_loc_to_new (M.loc_union l1 l2))] =
  ...
  let j2: M.loc (M.raise_cls u#0 u#0 NewM.cloc_cls) =
    coerce (M.loc (M.raise_cls NewM.cloc_cls)) i2
  in
  M.lower_loc_union u#0 u#0 j1 j2;
  NewM.cloc_of_loc_union (NewM.loc_of_cloc (M.lower_loc j1)) (NewM.loc_of_cloc (M.lower_loc j2));
  NewM.loc_of_cloc_of_loc (NewM.loc_union (NewM.loc_of_cloc (M.lower_loc j1))
        (NewM.loc_of_cloc (M.lower_loc j2)))

let union_loc_to_new_addresses (preserve_liveness: bool) (r: HS.rid) (n: Set.set nat)
  : Lemma
...
let union_loc_to_new_regions (preserve_liveness: bool) (r: Set.set HS.rid)
  : Lemma
...
let old_to_new_modifies' (old_l: OldM.loc) (h: HS.mem) (h': HS.mem)
  : Lemma
    (requires
...
assume
val loc_buffer_new_to_old: #t: Type0 -> b: New.buffer t ->
  Lemma
...
let modifies_0_modifies (h: HS.mem) (h': HS.mem)
  : Lemma (requires (Old.modifies_0 h h')) (ensures (NewM.modifies NewM.loc_none h h')) = ()

let modifies_1_modifies (#t: Type) (b: New.buffer t) (h: HS.mem) (h': HS.mem)
  : Lemma (requires (Old.modifies_1 (new_to_old_ghost b) h h'))
    (ensures (NewM.modifies (NewM.loc_buffer b) h h')) = ()

let modifies_2_modifies
  (#t1: Type) (#t2: Type) (b1: New.buffer t1) (b2: New.buffer t2) (h: HS.mem) (h': HS.mem)
  : Lemma (requires (Old.modifies_2 (new_to_old_ghost b1) (new_to_old_ghost b2) h h'))
...
let new_eqb (#a: eqtype) (b1: New.buffer a) (b2: New.buffer a) (len: U32.t)
  : HST.Stack bool
    (requires
      (fun h ->
          New.live h b1 /\ New.live h b2 /\ U32.v len <= New.length b1 /\ U32.v len <= New.length b2
      ))
    (ensures
      (fun h res h' ->
          h' == h /\
          (res <==>
            Seq.equal (New.as_seq h (New.gsub b1 0ul len)) (New.as_seq h (New.gsub b2 0ul len))))) =
  let b1' = new_to_old_st b1 in
  let b2' = new_to_old_st b2 in
  Old.eqb b1' b2' len

let new_blit
  (#t: Type) (src: New.buffer t) (idx_src: U32.t) (dst: New.buffer t) (idx_dst: U32.t) (len: U32.t)
  : HST.Stack unit
    (requires
      (fun h ->
          New.live h src /\ New.live h dst /\ New.disjoint src dst /\
          U32.v idx_src + U32.v len <= New.length src /\ U32.v idx_dst + U32.v len <= New.length dst
      ))
    (ensures
      (fun h _ h' ->
...
let new_fill (#t: Type) (b: New.buffer t) (z: t) (len: U32.t)
  : HST.Stack unit
...
val loc_disjoint_new_disjoint_old: 
  ...
  b1: New.buffer t1 ->
  b2: New.buffer t2 ->
  Lemma (requires (NewM.loc_disjoint (NewM.loc_buffer b1) (NewM.loc_buffer b2)))
    (ensures (Old.disjoint (new_to_old_ghost b1) (new_to_old_ghost b2)))
    [SMTPat (Old.disjoint (new_to_old_ghost b1) (new_to_old_ghost b2))]

assume
val modifies_modifies_0: h: HS.mem -> h': HS.mem ->
  Lemma (requires (NewM.modifies NewM.loc_none h h'))
    (ensures (Old.modifies_0 h h'))
    [SMTPat (NewM.modifies NewM.loc_none h h')]

assume
val modifies_modifies_1: #t: Type -> b: Old.buffer t -> h: HS.mem -> h': HS.mem ->
  Lemma (requires (NewM.modifies (NewM.loc_buffer (old_to_new_ghost b)) h h'))
...
val modifies_modifies_2: 
  ...
  Lemma
    (requires
      (NewM.modifies (NewM.loc_union (NewM.loc_buffer (old_to_new_ghost b1))
              (NewM.loc_buffer (old_to_new_ghost b2)))
          h
          h'))
...
let ex1' (#a: Type) (b: Old.buffer nat {Old.length b > 0}) (b1: Old.buffer a)
  : HST.ST unit
    ...
    (ensures
      (fun h0 _ h1 ->
          Old.get h1 b 0 == 0 /\ New.get h1 (old_to_new_ghost b) 0 == 0 /\
          Old.as_seq h0 b1 == Old.as_seq h1 b1 /\
          NewM.modifies (NewM.loc_buffer (old_to_new_ghost b)) h0 h1 /\ Old.modifies_1 b h0 h1)) =
