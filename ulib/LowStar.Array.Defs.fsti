...
/// A array is viewed as an `FStar.Seq.seq`. However, since these arrays have permissions attached
/// to them, `as_seq` is doubled by an `as_seq_perm` which returns the list of permission held by
/// the array on each of its cells.
...
/// Liveness is an abstract predicate, required or ensured by functions in `LowStar.Array`.
/// As a helper to understand its behavior, one can view it as "the array is correctly allocated
/// and all of its cells have a strictly positive permission".
...
/// Abstract predicate used to describe what happens after sharing an array to get two immutable
/// references from it. It implies that the two arrays have the same length, and is commutative.
...
/// `gsub` enjoys various properties related to the view and the length of the sub-array,
...
val gsub_inj
  ...
  : Lemma
    (requires (
      U32.v i1 + U32.v len1 <= vlength b1 /\
...
/// There exists a more sophisticated form of sub-array extraction that involves splitting an array
/// into two disjoint parts, and this is precisely what this abstract predicate captures.
