...
noeq abstract
type t (key: eqtype) (value: Type) = { mappings:key -> Tot value; domain:set key }
abstract
...
let upd #key #value m k v =
  { mappings = (fun x -> if x = k then v else m.mappings x); domain = union m.domain (singleton k) }
let const #key #value v = { mappings = (fun _ -> v); domain = complement empty }
...
val lemma_SelConcat1: 
  ...
  k: key ->
  Lemma (requires True)
    (ensures (contains m2 k ==> sel (concat m1 m2) k == sel m2 k))
    [SMTPat (sel (concat m1 m2) k)]
abstract
val lemma_SelConcat2: 
  #key: eqtype ->
  #value: Type ->
  m1: t key value ->
  m2: t key value ->
  k: key ->
  Lemma (requires True)
    (ensures (not (contains m2 k) ==> sel (concat m1 m2) k == sel m1 k))
    [SMTPat (sel (concat m1 m2) k)]
abstract
val lemma_InDomUpd1: 
  #key: eqtype ->
...
val lemma_InDomConstMap: #key: eqtype -> #value: Type -> v: value -> k: key ->
  Lemma (requires True) (ensures (contains (const v) k)) [SMTPat (contains (const v) k)]
abstract
val lemma_InDomConcat: 
  #key: eqtype ->
  #value: Type ->
  m1: t key value ->
  m2: t key value ->
...
abstract
type equal (#key: eqtype) (#value: Type) (m1: t key value) (m2: t key value) : Type0 =
...
val lemma_equal_refl: #key: eqtype -> #value: Type -> m1: t key value -> m2: t key value ->
  Lemma (requires (m1 == m2)) (ensures (equal m1 m2)) [SMTPat (equal m1 m2)]
