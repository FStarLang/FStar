...
noeq
type aloc (#al: aloc_t) (c: cls al) =
  | ALoc : region: HS.rid -> addr: nat -> loc: option (al region addr) -> aloc c

let aloc_domain
  (#al: aloc_t) (c: cls al) (regions: Ghost.erased (Set.set HS.rid))
  (addrs: (r: HS.rid{Set.mem r (Ghost.reveal regions)} -> GTot (GSet.set nat)))
  : GTot (GSet.set (aloc c)) =
  GSet.comprehend (fun a ->
        Set.mem a.region (Ghost.reveal regions) && GSet.mem a.addr (addrs a.region))
...
type loc' (#al: aloc_t u#x) (c: cls al) : Type u#x =
  | Loc : 
    ...
    Ghost.erased (Set.set HS.rid)
      {Set.subset (Ghost.reveal region_liveness_tags) (Ghost.reveal regions)} ->
    non_live_addrs:
    (r: HS.rid{Set.mem r (Ghost.reveal regions)} ->
        GTot
        (y:
          GSet.set nat
            { Set.mem r (Ghost.reveal region_liveness_tags) ==>
              GSet.subset (GSet.complement GSet.empty) y })) ->
    live_addrs:
    (r: HS.rid{Set.mem r (Ghost.reveal regions)} ->
        GTot (y: GSet.set nat {GSet.subset (non_live_addrs r) y})) ->
    aux:
    Ghost.erased (GSet.set (aloc c))
      { GSet.subset (aloc_domain c regions live_addrs) (Ghost.reveal aux) /\
        GSet.subset (Ghost.reveal aux) (aloc_domain c regions (fun _ -> GSet.complement GSet.empty))
      } ->
    loc' c
...
let regions_of_loc (#al: aloc_t) (#c: cls al) (s: loc c) : GTot (Set.set HS.rid) =
  Ghost.reveal (Loc?.regions s)

let addrs_of_loc_liveness_not_preserved (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid)
  : GTot (GSet.set nat) =
  if Set.mem r (regions_of_loc l) then Loc?.non_live_addrs l r else GSet.empty

let addrs_of_loc_weak (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid) : GTot (GSet.set nat) =
  if Set.mem r (regions_of_loc l) then Loc?.live_addrs l r else GSet.empty

let addrs_of_loc_aux_pred (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid) (addr: nat) : GTot bool =
  StrongExcludedMiddle.strong_excluded_middle (exists a.
        GSet.mem a (Ghost.reveal (Loc?.aux l)) /\ a.region == r /\ a.addr == addr)

let addrs_of_loc_aux (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid)
  : GTot (y: GSet.set nat {GSet.subset (GSet.intersect y (addrs_of_loc_weak l r)) GSet.empty}) =
  GSet.intersect (GSet.comprehend (addrs_of_loc_aux_pred l r))
    (GSet.complement (addrs_of_loc_weak l r))

let addrs_of_loc (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid) : GTot (GSet.set nat) =
  GSet.union (addrs_of_loc_weak l r) (addrs_of_loc_aux l r)

let addrs_of_loc_aux_prop (#al: aloc_t) (#c: cls al) (l: loc c) (r: HS.rid)
  : Lemma (GSet.subset (GSet.intersect (addrs_of_loc_aux l r) (addrs_of_loc_weak l r)) GSet.empty)
    [
      SMTPatOr
      [
...
let loc_union #al #c s1 s2 =
  ...
  in
  let non_live_addrs (r: HS.rid{Set.mem r regions})
    : GTot
    (y:
      GSet.set nat
        { Set.mem r (Ghost.reveal region_liveness_tags) ==>
  ...
  in
  let live_addrs (r: HS.rid{Set.mem r regions})
    : GTot (y: GSet.set nat {GSet.subset (non_live_addrs r) y}) =
  ...
  in
  let aux = Ghost.hide (GSet.union (Ghost.reveal (Loc?.aux s1)) (Ghost.reveal (Loc?.aux s2))) in
  Loc (Ghost.hide regions) region_liveness_tags non_live_addrs live_addrs aux

let fun_set_equal
  (#t: Type) (#t': Type) (f1: (t -> GTot (GSet.set t'))) (f2: (t -> GTot (GSet.set t')))
  : GTot Type0 = forall (x: t). {:pattern (f1 x)\/(f2 x)} GSet.equal (f1 x) (f2 x)

let fun_set_equal_elim
  (#t: Type) (#t': Type) (f1: (t -> GTot (GSet.set t'))) (f2: (t -> GTot (GSet.set t')))
  : Lemma (requires (fun_set_equal f1 f2)) (ensures (f1 == f2)) [SMTPat (fun_set_equal f1 f2)] =
  assert (FunctionalExtensionality.gfeq f1 f2)

let loc_equal (#al: aloc_t) (#c: cls al) (s1: loc c) (s2: loc c) : GTot Type0 =
  let Loc regions1 region_liveness_tags1 _ _ aux1 = s1 in
  let Loc regions2 region_liveness_tags2 _ _ aux2 = s2 in
  Set.equal (Ghost.reveal regions1) (Ghost.reveal regions2) /\
  Set.equal (Ghost.reveal region_liveness_tags1) (Ghost.reveal region_liveness_tags2) /\
  fun_set_equal (Loc?.non_live_addrs s1) (Loc?.non_live_addrs s2) /\
  fun_set_equal (Loc?.live_addrs s1) (Loc?.live_addrs s2) /\
  GSet.equal (Ghost.reveal (Loc?.aux s1)) (Ghost.reveal (Loc?.aux s2))

let loc_equal_elim (#al: aloc_t) (#c: cls al) (s1: loc c) (s2: loc c)
  : Lemma (requires (loc_equal s1 s2)) (ensures (s1 == s2)) [SMTPat (loc_equal s1 s2)] = ()

let loc_union_idem #al #c s = assert (loc_equal (loc_union s s) s)

let loc_union_comm #al #c s1 s2 = assert (loc_equal (loc_union s1 s2) (loc_union s2 s1))

let loc_union_assoc #al #c s1 s2 s3 =
  assert (loc_equal (loc_union s1 (loc_union s2 s3)) (loc_union (loc_union s1 s2) s3))

let loc_union_loc_none_l #al #c s = assert (loc_equal (loc_union loc_none s) s)

let loc_union_loc_none_r #al #c s = assert (loc_equal (loc_union s loc_none) s)

let loc_of_aloc #al #c #r #n b =
  Loc (Ghost.hide (Set.singleton r))
    (Ghost.hide Set.empty)
    (fun _ -> GSet.empty)
    (fun _ -> GSet.empty)
...
let loc_regions #al #c preserve_liveness r =
  ...
  in
  let addrs (r': HS.rid{Set.mem r' r})
    : GTot
    (y:
      GSet.set nat
        { Set.mem r' (Ghost.reveal region_liveness_tags) ==>
  ...
  in
  let live_addrs (r': HS.rid{Set.mem r' r}) : GTot (y: GSet.set nat {GSet.subset (addrs r') y}) =
    addrs r'
  in
  Loc (Ghost.hide r)
    region_liveness_tags
...
let aloc_includes (#al: aloc_t) (#c: cls al) (b0: aloc c) (b: aloc c) : GTot Type0 =
  b0.region == b.region /\ b0.addr == b.addr /\ Some? b0.loc == Some? b.loc /\
  (if Some? b0.loc && Some? b.loc then c.aloc_includes (Some?.v b0.loc) (Some?.v b.loc) else True)

let loc_aux_includes_buffer (#al: aloc_t) (#c: cls al) (s: GSet.set (aloc c)) (b: aloc c)
  : GTot Type0 (decreases s) = exists (b0: aloc c). GSet.mem b0 s /\ aloc_includes b0 b

let loc_aux_includes (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c))
  : GTot Type0 (decreases s2) =
  forall (b2: aloc c). GSet.mem b2 s2 ==> loc_aux_includes_buffer s1 b2

let loc_aux_includes_union_l
  (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c)) (s: GSet.set (aloc c))
  : Lemma (requires (loc_aux_includes s1 s \/ loc_aux_includes s2 s))
    (ensures (loc_aux_includes (GSet.union s1 s2) s))
    (decreases s) = ()

let loc_aux_includes_refl (#al: aloc_t) (#c: cls al) (s: GSet.set (aloc c))
  : Lemma (loc_aux_includes s s) =
  Classical.forall_intro_3 (fun r a b -> c.aloc_includes_refl #r #a b)

let loc_aux_includes_subset
  (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c))
  : Lemma (requires (GSet.subset s1 s2)) (ensures (loc_aux_includes s2 s1)) =
  Classical.forall_intro_3 (fun r a b -> c.aloc_includes_refl #r #a b)

let loc_aux_includes_subset'
  (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c))
  : Lemma (requires (GSet.subset s1 s2))
    (ensures (loc_aux_includes s2 s1))
    [SMTPatOr [[SMTPat (GSet.subset s1 s2)]; [SMTPat (loc_aux_includes s2 s1)]]] =
  loc_aux_includes_subset s1 s2

let loc_aux_includes_union_l_r
  (#al: aloc_t) (#c: cls al) (s: GSet.set (aloc c)) (s': GSet.set (aloc c))
  : Lemma (loc_aux_includes (GSet.union s s') s) =
...
let loc_aux_includes_buffer_includes
  (#al: aloc_t) (#c: cls al) (s: GSet.set (aloc c)) (b1: aloc c) (b2: aloc c)
  : Lemma (requires (loc_aux_includes_buffer s b1 /\ aloc_includes b1 b2))
    (ensures (loc_aux_includes_buffer s b2)) =
  Classical.forall_intro_3 (fun r a b1 ->
...
let loc_aux_includes_loc_aux_includes_buffer
  (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c)) (b: aloc c)
  : Lemma (requires (loc_aux_includes s1 s2 /\ loc_aux_includes_buffer s2 b))
    (ensures (loc_aux_includes_buffer s1 b)) =
  Classical.forall_intro_3 (fun s b1 b2 ->
        Classical.move_requires (loc_aux_includes_buffer_includes #al #c s b1) b2)

let loc_aux_includes_trans
  (#al: aloc_t) (#c: cls al) (s1: GSet.set (aloc c)) (s2: GSet.set (aloc c)) (s3: GSet.set (aloc c))
...
let addrs_of_loc_weak_loc_union (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c) (r: HS.rid)
  : Lemma
    (addrs_of_loc_weak (loc_union l1 l2) r ==
      GSet.union (addrs_of_loc_weak l1 r) (addrs_of_loc_weak l2 r))
    [SMTPat (addrs_of_loc_weak (loc_union l1 l2) r)] =
  assert (GSet.equal (addrs_of_loc_weak (loc_union l1 l2) r)
        (GSet.union (addrs_of_loc_weak l1 r) (addrs_of_loc_weak l2 r)))

let addrs_of_loc_union (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c) (r: HS.rid)
  : Lemma (addrs_of_loc (loc_union l1 l2) r == GSet.union (addrs_of_loc l1 r) (addrs_of_loc l2 r))
    [SMTPat (addrs_of_loc (loc_union l1 l2) r)] =
  assert (GSet.equal (addrs_of_loc (loc_union l1 l2) r)
        (GSet.union (addrs_of_loc l1 r) (addrs_of_loc l2 r)))

let loc_includes #al #c s1 s2 =
  let regions1 = Ghost.reveal (Loc?.regions s1) in
  let regions2 = Ghost.reveal (Loc?.regions s2) in
  (Set.subset regions2 regions1 /\
    Set.subset (Ghost.reveal (Loc?.region_liveness_tags s2))
      (Ghost.reveal (Loc?.region_liveness_tags s1)) /\
    ...
    (loc_aux_includes (Ghost.reveal (Loc?.aux s1)) (Ghost.reveal (Loc?.aux s2))))

let loc_includes_refl #al #c s = loc_aux_includes_refl (Ghost.reveal (Loc?.aux s))

let loc_includes_refl' (#al: aloc_t) (#c: cls al) (s: loc c)
  : Lemma (loc_includes s s) [SMTPat (loc_includes s s)] = loc_includes_refl s

let loc_includes_trans #al #c s1 s2 s3 =
  loc_aux_includes_trans (Ghost.reveal (Loc?.aux s1))
    (Ghost.reveal (Loc?.aux s2))
    (Ghost.reveal (Loc?.aux s3))
...
let loc_includes_none_elim #al #c s = assert (loc_equal s loc_none)
...
let addrs_of_loc_loc_of_aloc
  (#al: aloc_t) (#c: cls al) (#r: HS.rid) (#a: nat) (p: al r a) (r': HS.rid)
  : Lemma
    (GSet.equal (addrs_of_loc (loc_of_aloc #_ #c p) r')
        (if r = r' then GSet.singleton a else GSet.empty))
...
let loc_includes_region_union_l #al #c preserve_liveness l s1 s2 =
  assert (loc_equal (loc_union (loc_regions #_
                #c
                preserve_liveness
                (Set.intersect s2 (Set.complement s1)))
            (loc_regions #_ #c preserve_liveness (Set.intersect s2 s1)))
  ...
  loc_includes_union_l (loc_regions preserve_liveness s1)
    l
    (loc_regions preserve_liveness (Set.intersect s2 (Set.complement s1)));
  loc_includes_union_l (loc_regions preserve_liveness s1)
    l
    (loc_regions preserve_liveness (Set.intersect s2 s1));
  loc_includes_union_r (loc_union (loc_regions preserve_liveness s1) l)
    (loc_regions preserve_liveness (Set.intersect s2 (Set.complement s1)))
    (loc_regions preserve_liveness (Set.intersect s2 s1))
...
let aloc_disjoint (#al: aloc_t) (#c: cls al) (b1: aloc c) (b2: aloc c) : GTot Type0 =
  if b1.region = b2.region && b1.addr = b2.addr
  then Some? b1.loc /\ Some? b2.loc /\ c.aloc_disjoint (Some?.v b1.loc) (Some?.v b2.loc)
  else True

let aloc_disjoint_sym (#al: aloc_t) (#c: cls al) (b1: aloc c) (b2: aloc c)
  : Lemma (aloc_disjoint b1 b2 <==> aloc_disjoint b2 b1) =
  Classical.forall_intro_2 (fun r a ->
        Classical.forall_intro_2 (fun b1 b2 -> c.aloc_disjoint_sym #r #a b1 b2))

let loc_aux_disjoint (#al: aloc_t) (#c: cls al) (l1: GSet.set (aloc c)) (l2: GSet.set (aloc c))
  : GTot Type0 =
  forall (b1: aloc c) (b2: aloc c). (GSet.mem b1 l1 /\ GSet.mem b2 l2) ==> aloc_disjoint b1 b2

let loc_aux_disjoint_union_l
  (#al: aloc_t) (#c: cls al) (ll1: GSet.set (aloc c)) (lr1: GSet.set (aloc c))
  (l2: GSet.set (aloc c))
  : Lemma
  (ensures
    (loc_aux_disjoint (GSet.union ll1 lr1) l2 <==>
      (loc_aux_disjoint ll1 l2 /\ loc_aux_disjoint lr1 l2))) = ()

let loc_aux_disjoint_union_r
  (#al: aloc_t) (#c: cls al) (l1: GSet.set (aloc c)) (ll2: GSet.set (aloc c))
  (lr2: GSet.set (aloc c))
  : Lemma
  (loc_aux_disjoint l1 (GSet.union ll2 lr2) <==>
    (loc_aux_disjoint l1 ll2 /\ loc_aux_disjoint l1 lr2)) = ()

let loc_aux_disjoint_sym (#al: aloc_t) (#c: cls al) (l1: GSet.set (aloc c)) (l2: GSet.set (aloc c))
  : Lemma (ensures (loc_aux_disjoint l1 l2 <==> loc_aux_disjoint l2 l1)) =
  Classical.forall_intro_2 (aloc_disjoint_sym #al #c)

let regions_of_loc_loc_union (#al: aloc_t) (#c: cls al) (s1: loc c) (s2: loc c)
...
let regions_of_loc_monotonic (#al: aloc_t) (#c: cls al) (s1: loc c) (s2: loc c)
  : Lemma (requires (loc_includes s1 s2))
    (ensures (Set.subset (regions_of_loc s2) (regions_of_loc s1))) = ()

let loc_disjoint_region_liveness_tags (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c)
  : GTot Type0 =
  Set.subset (Set.intersect (Ghost.reveal (Loc?.region_liveness_tags l1))
        (Ghost.reveal (Loc?.region_liveness_tags l2)))
    Set.empty

let loc_disjoint_addrs (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c) : GTot Type0 =
  (forall (r: HS.rid).
...
let loc_disjoint_aux (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c) : GTot Type0 =
  loc_aux_disjoint (Ghost.reveal (Loc?.aux l1)) (Ghost.reveal (Loc?.aux l2))

let loc_disjoint' (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c) : GTot Type0 =
  loc_disjoint_region_liveness_tags l1 l2 /\ loc_disjoint_addrs l1 l2 /\ loc_disjoint_aux l1 l2

let loc_disjoint = loc_disjoint'

let loc_disjoint_sym #al #c l1 l2 = Classical.forall_intro_2 (loc_aux_disjoint_sym #al #c)

let loc_disjoint_sym' (#al: aloc_t) (#c: cls al) (s1: loc c) (s2: loc c)
  : Lemma (requires (loc_disjoint s1 s2))
    (ensures (loc_disjoint s2 s1))
    [SMTPat (loc_disjoint s1 s2)] = loc_disjoint_sym s1 s2

let loc_disjoint_none_r #al #c s = ()

let loc_disjoint_union_r #al #c s s1 s2 = ()

let aloc_disjoint_includes (#al: aloc_t) (#c: cls al) (b1: aloc c) (b2: aloc c) (b3: aloc c)
  : Lemma (requires (aloc_disjoint b1 b2 /\ aloc_includes b2 b3)) (ensures (aloc_disjoint b1 b3)) =
  if b1.region = b2.region && b1.addr = b2.addr
  then
...
let loc_aux_disjoint_loc_aux_includes
  (#al: aloc_t) (#c: cls al) (l1: GSet.set (aloc c)) (l2: GSet.set (aloc c)) (l3: GSet.set (aloc c))
  ...
  assert (forall (b1: aloc c) (b3: aloc c).
        (GSet.mem b1 l1 /\ GSet.mem b3 l3) ==>
        (exists (b2: aloc c). GSet.mem b2 l2 /\ aloc_disjoint b1 b2 /\ aloc_includes b2 b3));
  Classical.forall_intro_3 (fun b1 b2 b3 ->
        Classical.move_requires (aloc_disjoint_includes #al #c b1 b2) b3)
...
let loc_disjoint_addresses_intro #al #c preserve_liveness1 preserve_liveness2 r1 r2 n1 n2 =
  // FIXME: WHY WHY WHY this assert?
  assert (loc_aux_disjoint (Ghost.reveal (Loc?.aux (loc_addresses #_ #c preserve_liveness1 r1 n1)))
...
let address_liveness_insensitive_locs #al c =
  Loc (Ghost.hide (Set.complement Set.empty))
    (Ghost.hide Set.empty)
    (fun _ -> GSet.empty)
    (fun _ -> GSet.complement GSet.empty)
    (Ghost.hide (aloc_domain c
...
let modifies_preserves_livenesses (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
...
let modifies_preserves_livenesses_intro
  (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  (f:
  ...
  let f' (t: Type) (pre: Preorder.preorder t) (p: HS.mreference t pre)
    : Lemma
    ((HS.contains h1 p /\
        (Set.mem (HS.frameOf p) (regions_of_loc s) ==>
          ~(GSet.mem (HS.as_addr p) (Loc?.non_live_addrs s (HS.frameOf p))))) ==>
...
let modifies_preserves_mreferences (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
  (forall (t: Type) (pre: Preorder.preorder t) (p: HS.mreference t pre).
      let r = HS.frameOf p in
      (HS.contains h1 p /\
        (Set.mem r (regions_of_loc s) ==> ~(GSet.mem (HS.as_addr p) (addrs_of_loc s r)))) ==>
      (HS.contains h2 p /\ HS.sel h2 p == HS.sel h1 p))
...
let modifies_preserves_alocs (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
...
let modifies_preserves_alocs_intro
  ...
  (f:
    (r: HS.rid -> a: nat -> b: al r a ->
        Lemma
          (requires
            (Set.mem r (regions_of_loc s) /\ (~(GSet.mem a (addrs_of_loc_weak s r))) /\
              (GSet.mem a (addrs_of_loc_aux s r) /\
                loc_aux_disjoint (Ghost.reveal (Loc?.aux s)) (GSet.singleton (ALoc r a (Some b))))))
          (ensures (c.aloc_preserved b h1 h2))))
  : Lemma (modifies_preserves_alocs s h1 h2) =
  let f' (r: HS.rid) (a: nat) (b: al r a)
    : Lemma
    ...
    then
      ...
      then Classical.move_requires (f r a) b
      else c.same_mreference_aloc_preserved b h1 h2 (fun a' pre' r' -> ())
    else
      if Set.mem r (regions_of_loc s)
      then
...
let modifies_preserves_regions (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
  forall (r: HS.rid).
    (HS.live_region h1 r /\ ~(Set.mem r (Ghost.reveal (Loc?.region_liveness_tags s)))) ==>
    HS.live_region h2 r


let modifies_preserves_not_unused_in (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
...
let modifies_preserves_not_unused_in_intro
  ...
  : Lemma (modifies_preserves_not_unused_in s h1 h2) =
  let f' (r: HS.rid) (n: nat)
    : Lemma
    ((HS.live_region h1 r /\ HS.live_region h2 r /\
        Heap.addr_unused_in n (Map.sel (HS.get_hmap h2) r) /\
        (Set.mem r (regions_of_loc s) ==> ~(GSet.mem n (Loc?.non_live_addrs s r)))) ==>
...
let modifies' (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  modifies_preserves_regions s h1 h2 /\ modifies_preserves_not_unused_in s h1 h2 /\
  modifies_preserves_mreferences s h1 h2 /\ modifies_preserves_livenesses s h1 h2 /\
...
let modifies_intro_strong #al #c l h h' regions mrefs lives unused_ins alocs =
  ...
  modifies_preserves_mreferences_intro l
    h
    h'
    (fun t pre p ->
        ...
        assert (loc_aux_disjoint (Ghost.reveal (Loc?.aux (loc_mreference p)))
...
let modifies_intro #al #c l h h' regions mrefs lives unused_ins alocs =
  modifies_intro_strong l h h' regions mrefs lives (fun r n -> unused_ins r n) alocs

let modifies_none_intro #al #c h h' regions mrefs unused_ins =
  modifies_intro_strong #_ #c loc_none h h' (fun r -> regions r) (fun t pre b -> mrefs t pre b)
    (fun t pre b -> mrefs t pre b) (fun r n -> unused_ins r n)
    (fun r a x -> c.same_mreference_aloc_preserved x h h' (fun t pre b -> mrefs t pre b))

let modifies_address_intro #al #c r n h h' regions mrefs unused_ins =
  Classical.forall_intro (Classical.move_requires regions);
  let l: loc c = loc_addresses #_ #c false r (Set.singleton n) in
  modifies_preserves_mreferences_intro l h h' (fun t pre p -> mrefs t pre p);
  modifies_preserves_livenesses_intro l h h' (fun t pre p -> mrefs t pre p);
  modifies_preserves_not_unused_in_intro l h h' (fun r n -> unused_ins r n);
  modifies_preserves_alocs_intro l
    h
    h'
    ()
    (fun r a b -> c.same_mreference_aloc_preserved b h h' (fun t pre p -> mrefs t pre p))

let modifies_aloc_intro #al #c #r #n x h h' regions mrefs livenesses unused_ins alocs =
  modifies_intro_strong #_ #c (loc_of_aloc x) h h' (fun r -> regions r)
    (fun t pre b -> mrefs t pre b) (fun t pre b -> livenesses t pre b) (fun r n -> unused_ins r n)
    (fun r' n' z ->
        if r' = r && n' = n
        then (loc_disjoint_aloc_elim #_ #c z x; alocs z)
        else c.same_mreference_aloc_preserved z h h' (fun t pre p -> mrefs t pre p))
...
let modifies_loc_includes #al #c s1 h h' s2 =
  ...
  Classical.forall_intro_3 (fun l1 l2 l3 ->
        Classical.move_requires (loc_aux_disjoint_loc_aux_includes #al #c l1 l2) l3);
...
let modifies_preserves_liveness_strong #al #c s1 s2 h h' #t #pre r x =
  assert (Set.mem (HS.frameOf r) (regions_of_loc s1) ==>
      (~(GSet.mem (HS.as_addr r) (Loc?.non_live_addrs (loc_union s1 s2) (HS.frameOf r)))))
...
let modifies_trans' (#al: aloc_t) (#c: cls al) (s: loc c) (h1: HS.mem) (h2: HS.mem) (h3: HS.mem)
  : Lemma (requires (modifies s h1 h2 /\ modifies s h2 h3)) (ensures (modifies s h1 h3)) =
  Classical.forall_intro_3 (fun r a b ->
        Classical.move_requires (c.aloc_preserved_trans #r #a b h1 h2) h3)
...
let addr_unused_in_aloc_preserved
  (#al: aloc_t) (#c: cls al) (#r: HS.rid) (#a: nat) (b: al r a) (h1: HS.mem) (h2: HS.mem)
...
let modifies_only_live_regions_weak
  (#al: aloc_t) (#c: cls al) (rs: Set.set HS.rid) (l: loc c) (h: HS.mem) (h': HS.mem)
  : Lemma
    (requires
  ...
  Classical.forall_intro_3 (fun r a b ->
        Classical.move_requires (addr_unused_in_aloc_preserved #al #c #r #a b h) h')

(* Restrict a set of locations along a set of regions *)

let restrict_to_regions (#al: aloc_t) (#c: cls al) (l: loc c) (rs: Set.set HS.rid) : GTot (loc c) =
  let Loc regions region_liveness_tags non_live_addrs live_addrs aux = l in
  Loc (Ghost.hide (Set.intersect (Ghost.reveal regions) rs))
    (Ghost.hide (Set.intersect (Ghost.reveal region_liveness_tags) rs))
    (fun r -> non_live_addrs r)
    (fun r -> live_addrs r)
    (Ghost.hide (GSet.intersect (Ghost.reveal aux)
            (aloc_domain c (Ghost.hide rs) (fun r -> GSet.complement GSet.empty))))

let regions_of_loc_restrict_to_regions (#al: aloc_t) (#c: cls al) (l: loc c) (rs: Set.set HS.rid)
...
let addrs_of_loc_weak_restrict_to_regions
  (#al: aloc_t) (#c: cls al) (l: loc c) (rs: Set.set HS.rid) (r: HS.rid)
  : Lemma
    (addrs_of_loc_weak (restrict_to_regions l rs) r ==
      (if Set.mem r rs then addrs_of_loc_weak l r else GSet.empty))
    [SMTPat (addrs_of_loc_weak (restrict_to_regions l rs) r)] =
  assert (GSet.equal (addrs_of_loc_weak (restrict_to_regions l rs) r)
        (if Set.mem r rs then addrs_of_loc_weak l r else GSet.empty))
...
let addrs_of_loc_restrict_to_regions
  ...
  assert (GSet.equal (addrs_of_loc (restrict_to_regions l rs) r)
        (if Set.mem r rs then addrs_of_loc l r else GSet.empty))

let loc_includes_restrict_to_regions (#al: aloc_t) (#c: cls al) (l: loc c) (rs: Set.set HS.rid)
...
let loc_includes_loc_union_restrict_to_regions
  (#al: aloc_t) (#c: cls al) (l: loc c) (rs: Set.set HS.rid)
  : Lemma
...
let fresh_frame_modifies #al c h0 h1 =
  modifies_intro_strong #_ #c loc_none h0 h1 (fun _ -> ()) (fun _ _ _ -> ()) (fun _ _ _ -> ())
    (fun _ _ -> ()) (fun r a x -> c.same_mreference_aloc_preserved #r #a x h0 h1 (fun _ _ _ -> ()))

let new_region_modifies #al c m0 r0 col =
  let _, m1 = HS.new_eternal_region m0 r0 col in
  modifies_intro_strong #_ #c loc_none m0 m1 (fun _ -> ()) (fun _ _ _ -> ()) (fun _ _ _ -> ())
    (fun _ _ -> ()) (fun r a x -> c.same_mreference_aloc_preserved #r #a x m0 m1 (fun _ _ _ -> ()))
...
let modifies_fresh_frame_popped #al #c h0 h1 s h2 h3 =
  ...
  let f23 (r: HS.rid) (a: nat) (b: al r a)
    : Lemma (requires (r <> HS.get_tip h2)) (ensures (c.aloc_preserved b h2 h3)) =
    c.same_mreference_aloc_preserved #r #a b h2 h3 (fun a' pre r' -> ())
  in
  modifies_preserves_alocs_intro r h2 h3 () (fun r a b -> f23 r a b);
  modifies_trans' s' h0 h2 h3;
  modifies_only_live_regions rs s h0 h3

let modifies_loc_regions_intro #al #c rs h1 h2 =
  let f (r: HS.rid) (a: nat) (b: al r a)
    : Lemma (requires (not (Set.mem r rs))) (ensures (c.aloc_preserved b h1 h2)) =
  ...
  modifies_preserves_alocs_intro (loc_regions #_ #c true rs) h1 h2 () (fun r a b -> f r a b)

let modifies_loc_addresses_intro_weak
  (#al: aloc_t) (#c: cls al) (r: HS.rid) (s: Set.set nat) (l: loc c) (h1: HS.mem) (h2: HS.mem)
  : Lemma
    (requires
  ...
  modifies_preserves_mreferences_intro (loc_union (loc_addresses true r s) l)
    h1
    h2
    (fun r' a' b' -> ());
  modifies_preserves_livenesses_intro (loc_union (loc_addresses true r s) l)
    h1
    h2
    (fun r' a' b' -> ());
  modifies_preserves_not_unused_in_intro (loc_union (loc_addresses true r s) l)
    h1
    h2
    (fun r' n' -> ());
  let f (a: nat) (b: al r a)
    : Lemma (requires (not (Set.mem a s))) (ensures (c.aloc_preserved b h1 h2)) =
    c.same_mreference_aloc_preserved #r #a b h1 h2 (fun a' pre r_ -> ())
  in
  modifies_preserves_alocs_intro (loc_union (loc_addresses true r s) l)
    h1
    h2
    ()
    (fun r' a b -> if r = r' then f a b)
...
let modifies_loc_addresses_intro #al #c r s l h1 h2 =
  ...
  assert (modifies (loc_union (loc_region_only false r)
            (loc_union (restrict_to_regions l (Set.singleton r))
                (restrict_to_regions l (Set.complement (Set.singleton r)))))
        h1
        h2);
  ...
  loc_disjoint_includes (loc_regions #_ #c false (Set.complement (Set.singleton r)))
    (loc_region_only false r)
    l'
    (loc_region_only false r);
...
let modifies_ralloc_post #al #c #a #rel i init h x h' =
  let g (r: HS.rid) (a: nat) (b: al r a) : Lemma (c.aloc_preserved b h h') =
...
let modifies_free #al #c #a #rel r m =
  let g (r': HS.rid) (a: nat) (b: al r' a)
    : Lemma (requires (r' <> HS.frameOf r \/ a <> HS.as_addr r))
      (ensures (c.aloc_preserved b m (HS.free r m))) =
    c.same_mreference_aloc_preserved #r' #a b m (HS.free r m) (fun a' pre r' -> ())
  in
  modifies_preserves_alocs_intro (loc_freed_mreference #_ #c r)
    m
    (HS.free r m)
    ()
    (fun r a b -> g r a b)
...
let modifies_none_modifies #al #c h1 h2 =
  let g (r: HS.rid) (a: nat) (b: al r a) : Lemma (c.aloc_preserved b h1 h2) =
    c.same_mreference_aloc_preserved #r #a b h1 h2 (fun a' pre r' -> ())
  in
  Classical.forall_intro_3 g
...
let modifies_upd #al #c #t #pre r v h =
  ...
  modifies_intro #_ #c (loc_mreference r) h h' (fun r -> ()) (fun t pre b -> ()) (fun t pre b -> ())
    (fun r n -> ())
...
let disjoint_addrs_of_loc_loc_disjoint (#al: aloc_t) (#c: cls al) (l1: loc c) (l2: loc c)
  : Lemma
    (requires
      (Set.subset (Set.intersect (Ghost.reveal (Loc?.region_liveness_tags l1))
              (Ghost.reveal (Loc?.region_liveness_tags l2)))
  ...
  assert (forall (b1: aloc c) (b2: aloc c).
        (GSet.mem b1 l1' /\ GSet.mem b2 l2') ==> aloc_disjoint b1 b2)


let loc_not_unused_in #al c h =
  let f (r: HS.rid) : GTot (GSet.set nat) =
    GSet.comprehend (fun a ->
          StrongExcludedMiddle.strong_excluded_middle (HS.live_region h r /\
              ~(does_not_contain_addr h (r, a))))
  in
  Loc (Ghost.hide (Set.complement Set.empty))
    (Ghost.hide Set.empty)
    f
...
let loc_unused_in #al c h =
  let f (r: HS.rid) : GTot (GSet.set nat) =
    GSet.comprehend (fun a ->
          StrongExcludedMiddle.strong_excluded_middle (does_not_contain_addr h (r, a)))
  in
  Loc (Ghost.hide (Set.complement Set.empty))
    (Ghost.hide Set.empty)
    f
...
let loc_unused_in_not_unused_in_disjoint #al c h =
  assert (loc_aux_disjoint (Ghost.reveal (Loc?.aux (loc_unused_in c h)))
        (Ghost.reveal (Loc?.aux (loc_not_unused_in c h))))
...
let modifies_only_not_unused_in #al #c l h h' =
  ...
  modifies_preserves_alocs_intro l
    ...
    ()
    (fun r a b ->
        if StrongExcludedMiddle.strong_excluded_middle (does_not_contain_addr h (r, a))
...
let mreference_live_loc_not_unused_in #al c #t #pre h b =
  ...
  assert (~(does_not_contain_addr h (HS.frameOf b, HS.as_addr b)));
...
let mreference_unused_in_loc_unused_in #al c #t #pre h b =
  ...
  loc_includes_addresses_addresses c
    false
    true
    (HS.frameOf b)
...
noeq
type cls_union_aloc (al: (bool -> HS.rid -> nat -> Tot (Type u#x))) (r: HS.rid) (n: nat)
  : Type u#x
  =
...
let bool_of_cls_union_aloc
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (#r: HS.rid) (#n: nat) (l: cls_union_aloc al r n)
  : Tot bool =
  match l with
  | ALOC_FALSE _ -> false
  | ALOC_TRUE _ -> true
...
let make_cls_union_aloc
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (b: bool) (#r: HS.rid) (#n: nat) (l: (al b) r n)
  : Tot (cls_union_aloc al r n) = if b then ALOC_TRUE l else ALOC_FALSE l

let cls_union_aloc_includes
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (#r: HS.rid)
  (#a: nat) (larger: cls_union_aloc al r a) (smaller: cls_union_aloc al r a)
  : GTot Type0 =
  bool_of_cls_union_aloc larger == bool_of_cls_union_aloc smaller /\
  (c (bool_of_cls_union_aloc larger)).aloc_includes (aloc_of_cls_union_aloc larger)
...
let cls_union_aloc_disjoint
  ...
  : GTot Type0 =
  bool_of_cls_union_aloc larger == bool_of_cls_union_aloc smaller /\
  (c (bool_of_cls_union_aloc larger)).aloc_disjoint (aloc_of_cls_union_aloc larger)
    (aloc_of_cls_union_aloc smaller)

let cls_union_aloc_preserved
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (#r: HS.rid)
  (#a: nat) (x: cls_union_aloc al r a) (h: HS.mem) (h': HS.mem)
  : GTot Type0 = (c (bool_of_cls_union_aloc x)).aloc_preserved (aloc_of_cls_union_aloc x) h h'

let aloc_union = cls_union_aloc

let cls_union #al c =
  Cls #(cls_union_aloc al)
    (cls_union_aloc_includes c)
    ((* aloc_includes_refl *)
      fun #r #a x -> (c (bool_of_cls_union_aloc x)).aloc_includes_refl (aloc_of_cls_union_aloc x))
    ((* aloc_includes_trans *)
      fun #r #a x1 x2 x3 ->
        (c (bool_of_cls_union_aloc x1)).aloc_includes_trans (aloc_of_cls_union_aloc x1)
    ...
    ((* aloc_disjoint_sym *)
      fun #r #a x1 x2 ->
        ...
        then
          (c (bool_of_cls_union_aloc x1)).aloc_disjoint_sym (aloc_of_cls_union_aloc x1)
            (aloc_of_cls_union_aloc x2))
    ((* aloc_disjoint_includes *)
      fun #r #a larger1 larger2 smaller1 smaller2 ->
        (c (bool_of_cls_union_aloc larger1)).aloc_disjoint_includes (aloc_of_cls_union_aloc larger1)
    ...
    ((* aloc_preserved_refl *)
      fun #r #a x h ->
        (c (bool_of_cls_union_aloc x)).aloc_preserved_refl (aloc_of_cls_union_aloc x) h)
    ((* aloc_preserved_trans *)
      fun #r #a x h1 h2 h3 ->
        (c (bool_of_cls_union_aloc x)).aloc_preserved_trans (aloc_of_cls_union_aloc x) h1 h2 h3)
    ((* same_mreference_aloc_preserved *)
      fun #r #a b h1 h2 f ->
        (c (bool_of_cls_union_aloc b)).same_mreference_aloc_preserved (aloc_of_cls_union_aloc b)
          h1
          h2
          f)

let union_aux_of_aux_left_pred
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (s: GSet.set (aloc (c b))) (x: aloc (cls_union c))
  : GTot bool =
...
let union_aux_of_aux_left
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (s: GSet.set (aloc (c b)))
  : Tot (GSet.set (aloc (cls_union c))) = GSet.comprehend (union_aux_of_aux_left_pred c b s)

let union_loc_of_loc #al c b l =
  let Loc regions region_liveness_tags non_live_addrs live_addrs aux = l in
  let aux': GSet.set (aloc #(cls_union_aloc al) (cls_union c)) =
    GSet.union (union_aux_of_aux_left c b (Ghost.reveal aux))
...
let union_aux_of_aux_left_inv_pred
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (#c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (s: GSet.set (aloc (cls_union c))) (x: aloc (c b))
  : GTot bool =
  let ALoc region addr loc = x in
  match loc with
  | None -> GSet.mem (ALoc region addr None) s
  | Some loc -> GSet.mem (ALoc region addr (Some (make_cls_union_aloc b loc))) s

let union_aux_of_aux_left_inv
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (#c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (s: GSet.set (aloc (cls_union c)))
  : Tot (GSet.set (aloc (c b))) = GSet.comprehend (union_aux_of_aux_left_inv_pred b s)

let mem_union_aux_of_aux_left_intro
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (x: aloc (c b)) (aux: GSet.set (aloc (c b)))
  : Lemma
    (GSet.mem x aux <==>
      GSet.mem (ALoc x.region
            x.addr
            (if None? x.loc then None else Some (make_cls_union_aloc b (Some?.v x.loc))))
        (union_aux_of_aux_left c b aux)) [SMTPat (GSet.mem x aux)] = ()

let mem_union_aux_of_aux_left_elim
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (x: aloc (cls_union c)) (aux: GSet.set (aloc (c b)))
  : Lemma
    (GSet.mem x (union_aux_of_aux_left c b aux) <==>
      (if None? x.loc
        then GSet.mem (ALoc x.region x.addr None) aux
        else
          (bool_of_cls_union_aloc (Some?.v x.loc) == b /\
            GSet.mem (ALoc x.region x.addr (Some (aloc_of_cls_union_aloc (Some?.v x.loc)))) aux)))
    [SMTPat (GSet.mem x (union_aux_of_aux_left c b aux))] = ()

let addrs_of_loc_union_loc_of_loc
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (l: loc (c b)) (r: HS.rid)
  : Lemma (GSet.equal (addrs_of_loc (union_loc_of_loc c b l) r) (addrs_of_loc l r))
    [SMTPat (addrs_of_loc (union_loc_of_loc c b l) r)] = ()

let union_loc_of_loc_none #al c b =
  assert (loc_equal #_
        #(cls_union c)
        (union_loc_of_loc c b (loc_none #_ #(c b)))
        (loc_none #_ #(cls_union c)))
...
let union_loc_of_loc_includes_intro
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (larger: loc (c b)) (smaller: loc (c b))
  : Lemma (requires (loc_includes larger smaller))
    (ensures (loc_includes (union_loc_of_loc c b larger) (union_loc_of_loc c b smaller))) =
  ...
  assert (loc_aux_includes auxl auxs);
  ...
  assert (loc_aux_includes doml doms)
...
let union_loc_of_loc_includes_elim
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (larger: loc (c b)) (smaller: loc (c b))
  : Lemma (requires (loc_includes (union_loc_of_loc c b larger) (union_loc_of_loc c b smaller)))
    (ensures (loc_includes larger smaller)) =
  ...
  let g (r: HS.rid) (a: nat) (x: aloc (c b)) (y: aloc (c b)) : GTot Type0 =
    GSet.mem y (GSet.union auxl doml) /\ aloc_includes y x
  in
  let g' (r: HS.rid) (a: nat) (x: aloc (c b)) : GTot Type0 = exists (y: aloc (c b)). g r a x y in
  let f (r: HS.rid) (a: nat) (x: aloc (c b))
    : Lemma
      (requires (GSet.mem x auxs /\ (~(GSet.mem x.addr (addrs_of_loc_weak smaller x.region)))))
      (ensures (g' r a x)) =
    let x': aloc (cls_union c) =
      ALoc x.region
        x.addr
        (if None? x.loc then None else Some (make_cls_union_aloc b (Some?.v x.loc)))
    in
    Classical.exists_elim (g' r a x)
      #(aloc (cls_union c))
      #(fun y' -> GSet.mem y' (GSet.union auxl' doml') /\ aloc_includes y' x')
      ()
      (fun (y': aloc (cls_union c) {GSet.mem y' (GSet.union auxl' doml') /\ aloc_includes y' x'}) ->
          let y: aloc (c b) =
            ALoc y'.region
              y'.addr
              (if None? y'.loc then None else Some (aloc_of_cls_union_aloc (Some?.v y'.loc)))
          in
          assert (g r a x y))
  in
  let f' (r: HS.rid) (a: nat) (x: aloc (c b))
  ...
  assert (loc_includes larger smaller)
...
let union_loc_of_loc_disjoint_intro
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (larger: loc (c b)) (smaller: loc (c b))
  : Lemma (requires (loc_disjoint larger smaller))
    (ensures (loc_disjoint (union_loc_of_loc c b larger) (union_loc_of_loc c b smaller))) =
  let auxl = union_aux_of_aux_left c b (Ghost.reveal (Loc?.aux larger)) in
  let auxs = union_aux_of_aux_left c b (Ghost.reveal (Loc?.aux smaller)) in
  assert (forall (xl: aloc (cls_union c)) (xs: aloc (cls_union c)).
        (GSet.mem xl auxl /\ GSet.mem xs auxs) ==>
        (let xl': aloc (c b) =
            ALoc xl.region
              xl.addr
              (if None? xl.loc then None else Some (aloc_of_cls_union_aloc (Some?.v xl.loc)))
          in
          let xs': aloc (c b) =
            ALoc xs.region
              xs.addr
              (if None? xs.loc then None else Some (aloc_of_cls_union_aloc (Some?.v xs.loc)))
          in
          GSet.mem xl' (Ghost.reveal (Loc?.aux larger)) /\
          GSet.mem xs' (Ghost.reveal (Loc?.aux smaller)) /\ aloc_disjoint xl' xs' /\
          aloc_disjoint xl xs));
  assert (forall xl xs. (GSet.mem xl auxl /\ GSet.mem xs auxs) ==> aloc_disjoint xl xs);
  assert (loc_aux_disjoint auxl auxs);
  ...
  assert (forall (xl: aloc (cls_union c)) (xs: aloc (cls_union c)).
        ...
        (GSet.mem xl.addr (addrs_of_loc_weak larger xl.region) /\
          GSet.mem xs.addr (addrs_of_loc smaller xs.region) /\ aloc_disjoint xl xs));
  assert (loc_aux_disjoint doml auxs);
  assert (forall (xl: aloc (cls_union c)) (xs: aloc (cls_union c)).
        (GSet.mem xl auxl /\ GSet.mem xs doms) ==>
        (GSet.mem xl.addr (addrs_of_loc larger xl.region) /\
          GSet.mem xs.addr (addrs_of_loc_weak smaller xs.region) /\ aloc_disjoint xl xs));
  assert (loc_aux_disjoint auxl doms);
...
let union_loc_of_loc_disjoint_elim
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (larger: loc (c b)) (smaller: loc (c b))
  : Lemma (requires (loc_disjoint (union_loc_of_loc c b larger) (union_loc_of_loc c b smaller)))
    (ensures (loc_disjoint larger smaller)) =
  let auxl = Ghost.reveal (Loc?.aux larger) in
  let auxl' = union_aux_of_aux_left c b auxl in
  let auxs = Ghost.reveal (Loc?.aux smaller) in
  let auxs' = union_aux_of_aux_left c b auxs in
  assert (forall (x: aloc (c b)) (y: aloc (c b)).
        (GSet.mem x auxl /\ GSet.mem y auxs) ==>
        (let x' =
            ALoc x.region
              x.addr
              (if None? x.loc then None else Some (make_cls_union_aloc b (Some?.v x.loc)))
          ...
          let y' =
            ALoc y.region
              y.addr
              (if None? y.loc then None else Some (make_cls_union_aloc b (Some?.v y.loc)))
          in
...
let modifies_union_loc_of_loc_elim
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (l: loc (c b)) (h1: HS.mem) (h2: HS.mem)
  : Lemma (requires (modifies #_ #(cls_union c) (union_loc_of_loc c b l) h1 h2))
    (ensures (modifies #_ #(c b) l h1 h2)) =
  assert (modifies_preserves_regions l h1 h2);
  assert (modifies_preserves_mreferences l h1 h2);
  modifies_preserves_alocs_intro #_
    #(c b)
    l
    h1
    h2
          ...
          : Lemma
            ...
            (ensures
              (aloc_disjoint #_
                  #(cls_union c)
                  x
                  (ALoc #_ #(cls_union c) r' a' (Some (make_cls_union_aloc b b'))))) =
          ...
          then
            let x': aloc (c b) =
              ALoc #_
                ...
                r'
                a'
                (if None? x.loc then None else Some (aloc_of_cls_union_aloc (Some?.v x.loc)))
            in
...
let modifies_union_loc_of_loc_intro
  (#al: (bool -> HS.rid -> nat -> Tot Type)) (c: (b: bool -> Tot (cls (al b)))) (b: bool)
  (l: loc (c b)) (h1: HS.mem) (h2: HS.mem)
  : Lemma (requires (modifies #_ #(c b) l h1 h2))
  ...
  modifies_preserves_alocs_intro #_
    ...
    (fun r' a' b' ->
        ...
        assert (exists (x: aloc (c b)).
              GSet.mem x (Ghost.reveal (Loc?.aux l)) /\
              (let xr = x.region in
                let xa = x.addr in
                let xl: option (al b xr xa) = x.loc in
                xr == r' /\ xa == a' /\
                (let xl': option (aloc_union al r' a') =
                    if None? xl then None else Some (make_cls_union_aloc #al b (Some?.v xl))
                  in
                  let x': aloc (cls_union c) = ALoc r' a' xl' in
                  GSet.mem x' (Ghost.reveal (Loc?.aux l')) /\ aloc_disjoint #_ #(cls_union c) x' ll'
                )));
        assert (b_ == b);
        let f (x: aloc (c b))
          : Lemma (requires (GSet.mem x (Ghost.reveal (Loc?.aux l))))
            (ensures (aloc_disjoint #_ #(c b) x ll)) =
          let xr = x.region in
          let xa = x.addr in
          let xl: option (al b xr xa) = x.loc in
          let xl': option (aloc_union al xr xa) =
            if None? xl then None else Some (make_cls_union_aloc #al b (Some?.v xl))
          in
...
let loc_of_union_loc #al #c b l =
  ...
  Loc regions region_liveness_tags non_live_addrs live_addrs (Ghost.hide aux')

let loc_of_union_loc_union_loc_of_loc #al c b s =
  assert (loc_equal (loc_of_union_loc b (union_loc_of_loc c b s)) s)

let loc_of_union_loc_none #al c b = assert (loc_equal (loc_of_union_loc #_ #c b loc_none) loc_none)

let loc_of_union_loc_union #al c b l1 l2 =
  assert (loc_equal (loc_of_union_loc b (loc_union l1 l2))
        (loc_union (loc_of_union_loc b l1) (loc_of_union_loc b l2)))

let loc_of_union_loc_addresses #al c b preserve_liveness r n =
  assert (loc_equal (loc_of_union_loc #_ #c b (loc_addresses preserve_liveness r n))
        (loc_addresses preserve_liveness r n))

let loc_of_union_loc_regions #al c b preserve_liveness r =
  assert (loc_equal (loc_of_union_loc #_ #c b (loc_regions preserve_liveness r))
        (loc_regions preserve_liveness r))
...
let raise_cls #al c =
  Cls #(raise_aloc u#x u#y al)
    ...
    (fun #r #a larger1 larger2 smaller1 smaller2 ->
        c.aloc_disjoint_includes (U.downgrade_val larger1)
          (U.downgrade_val larger2)
          (U.downgrade_val smaller1)
          (U.downgrade_val smaller2))
...
let downgrade_aloc_upgrade_aloc (#al: aloc_t u#a) (#c: cls al) (a: aloc c)
  : Lemma (downgrade_aloc (upgrade_aloc u#a u#b a) == a)
    [SMTPat (downgrade_aloc (upgrade_aloc u#a u#b a))] = ()

let upgrade_aloc_downgrade_aloc (#al: aloc_t u#a) (#c: cls al) (a: aloc (raise_cls u#a u#b c))
  : Lemma (upgrade_aloc (downgrade_aloc a) == a) [SMTPat (upgrade_aloc u#a u#b (downgrade_aloc a))] =
  ()

let raise_loc_aux_pred
  (#al: aloc_t u#a) (c: cls al) (aux: Ghost.erased (GSet.set (aloc c)))
  (a: aloc (raise_cls u#a u#b c))
  : GTot bool = GSet.mem (downgrade_aloc a) (Ghost.reveal aux)
...
let raise_loc_none #al #c = assert (loc_equal (raise_loc u#x u#y (loc_none #_ #c)) loc_none)

let raise_loc_union #al #c l1 l2 =
  assert (loc_equal (raise_loc u#x u#y (loc_union l1 l2)) (loc_union (raise_loc l1) (raise_loc l2)))

let raise_loc_addresses #al #c preserve_liveness r a =
  assert (loc_equal (raise_loc u#x u#y (loc_addresses #_ #c preserve_liveness r a))
        (loc_addresses preserve_liveness r a))

let raise_loc_regions #al #c preserve_liveness r =
  assert (loc_equal (raise_loc u#x u#y (loc_regions #_ #c preserve_liveness r))
        (loc_regions preserve_liveness r))
...
let raise_loc_includes #al #c l1 l2 =
  ...
  assert (forall (x: aloc (raise_cls c)).
        GSet.mem x (Ghost.reveal (Loc?.aux l1')) <==>
        GSet.mem (downgrade_aloc x) (Ghost.reveal (Loc?.aux l1)));
  assert (forall (x: aloc (raise_cls c)).
        GSet.mem x (Ghost.reveal (Loc?.aux l2')) <==>
        GSet.mem (downgrade_aloc x) (Ghost.reveal (Loc?.aux l2)));
  assert (forall (x: aloc c).
        GSet.mem x (Ghost.reveal (Loc?.aux l1)) <==>
        GSet.mem (upgrade_aloc x) (Ghost.reveal (Loc?.aux l1')));
  assert (forall (x: aloc c).
        GSet.mem x (Ghost.reveal (Loc?.aux l2)) <==>
        GSet.mem (upgrade_aloc x) (Ghost.reveal (Loc?.aux l2')));
  assert (loc_aux_includes (Ghost.reveal (Loc?.aux l1')) (Ghost.reveal (Loc?.aux l2')) <==>
      loc_aux_includes (Ghost.reveal (Loc?.aux l1)) (Ghost.reveal (Loc?.aux l2)))
...
let raise_loc_disjoint #al #c l1 l2 =
  ...
  assert (forall (x: aloc c).
        GSet.mem x (Ghost.reveal (Loc?.aux l2)) <==>
        GSet.mem (upgrade_aloc x) (Ghost.reveal (Loc?.aux l2')));
  assert (forall r. GSet.equal (addrs_of_loc l1' r) (addrs_of_loc l1 r));
  assert (forall r. GSet.equal (addrs_of_loc l2' r) (addrs_of_loc l2 r));
  assert (forall (x1: aloc (raise_cls u#x u#y c)) (x2: aloc (raise_cls u#x u#y c)).
        aloc_disjoint x1 x2 <==> aloc_disjoint (downgrade_aloc x1) (downgrade_aloc x2));
  assert (forall (x1: aloc (c)) (x2: aloc (c)).
        aloc_disjoint x1 x2 <==> aloc_disjoint (upgrade_aloc u#x u#y x1) (upgrade_aloc x2))

let modifies_raise_loc #al #c l h1 h2 =
  let l' = raise_loc l in
  assert (forall (x: aloc (raise_cls c)).
        GSet.mem x (Ghost.reveal (Loc?.aux l')) <==>
        GSet.mem (downgrade_aloc x) (Ghost.reveal (Loc?.aux l)));
  assert (forall (x: aloc c).
        GSet.mem x (Ghost.reveal (Loc?.aux l)) <==>
        GSet.mem (upgrade_aloc x) (Ghost.reveal (Loc?.aux l')));
  assert (forall r. GSet.equal (addrs_of_loc l' r) (addrs_of_loc l r));
  assert (forall (x1: aloc (raise_cls u#x u#y c)) (x2: aloc (raise_cls u#x u#y c)).
        aloc_disjoint x1 x2 <==> aloc_disjoint (downgrade_aloc x1) (downgrade_aloc x2));
  assert (forall (r: HS.rid) (a: nat) (b: raise_aloc al r a).
        loc_aux_disjoint (Ghost.reveal (Loc?.aux l')) (GSet.singleton (ALoc r a (Some b))) ==>
        loc_aux_disjoint (Ghost.reveal (Loc?.aux l))
          (GSet.singleton (ALoc r a (Some (U.downgrade_val b)))));
  assert (modifies_preserves_alocs l h1 h2 ==> modifies_preserves_alocs l' h1 h2);
  assert (forall (r: HS.rid) (a: nat) (b: al r a).
        loc_aux_disjoint (Ghost.reveal (Loc?.aux l)) (GSet.singleton (ALoc r a (Some b))) ==>
        loc_aux_disjoint (Ghost.reveal (Loc?.aux l'))
          (GSet.singleton (ALoc r a (Some (U.raise_val b)))));
  assert (modifies_preserves_alocs l' h1 h2 ==> modifies_preserves_alocs l h1 h2)

let lower_loc_aux_pred
  (#al: aloc_t u#a) (c: cls al) (aux: Ghost.erased (GSet.set (aloc (raise_cls u#a u#b c))))
  (a: aloc c)
  : GTot bool = GSet.mem (upgrade_aloc a) (Ghost.reveal aux)
...
let lower_loc_raise_loc #al #c l = assert (loc_equal (lower_loc (raise_loc u#x u#y l)) l)

let raise_loc_lower_loc #al #c l = assert (loc_equal (raise_loc (lower_loc l)) l)

let lower_loc_none #al #c = assert (loc_equal (lower_loc u#x u#y #_ #c loc_none) loc_none)

let lower_loc_union #al #c l1 l2 =
  assert (loc_equal (lower_loc u#x u#y (loc_union l1 l2)) (loc_union (lower_loc l1) (lower_loc l2)))

let lower_loc_addresses #al #c preserve_liveness r a =
  assert (loc_equal (lower_loc u#x u#y #_ #c (loc_addresses preserve_liveness r a))
        (loc_addresses preserve_liveness r a))

let lower_loc_regions #al #c preserve_liveness r =
  assert (loc_equal (lower_loc u#x u#y #_ #c (loc_regions preserve_liveness r))
        (loc_regions preserve_liveness r))
