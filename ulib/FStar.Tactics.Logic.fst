...
private
val revert_squash: #a: Type -> #b: (a -> Type) -> (squash (forall (x: a). b x)) -> x: a ->
  squash (b x)
...
let rec l_revert_all (bs: binders) : Tac unit =
  match bs with
  | [] -> ()
  | _ :: tl -> l_revert (); l_revert_all tl
private
val fa_intro_lem: #a: Type -> #p: (a -> Type) -> (x: a -> squash (p x)) ->
...
let forall_intro () : Tac binder =
  ...
  | Forall _ _ -> apply_lemma (`fa_intro_lem); intro ()
  | _ -> fail "not a forall"

let forall_intro_as (s: string) : Tac binder =
  let g = cur_goal () in
  match term_as_formula g with
  | Forall _ _ -> apply_lemma (`fa_intro_lem); intro_as s
  | _ -> fail "not a forall"

let forall_intros () : Tac binders = repeat1 forall_intro
private
val split_lem: #a: Type -> #b: Type -> squash a -> squash b -> Lemma (a /\ b)
...
let implies_intro () : Tac binder =
  let g = cur_goal () in
  match term_as_formula g with
  | Implies _ _ -> apply_lemma (`imp_intro_lem); intro ()
  | _ -> fail "not an implication"

let implies_intros () : Tac binders = repeat1 implies_intro

let l_intro () = or_else forall_intro implies_intro
let l_intros () = repeat l_intro

let explode () : Tac unit =
  ignore (repeatseq (fun () ->
            first [(fun () -> ignore (l_intro ())); (fun () -> ignore (split ()))]))
...
private
val __and_elim: #p: Type -> #q: Type -> #phi: Type -> (p /\ q) -> squash (p ==> q ==> phi) ->
  Lemma phi
...
let sklem0 (#a: Type) (#p: (a -> Type0)) ($v: (exists (x: a). p x)) (phi: Type0)
  : Lemma (requires (forall x. p x ==> phi)) (ensures phi) = ()
...
let rec sk_binder' (acc: binders) (b: binder) : Tac (binders * binder) =
  focus (fun () ->
        or_else (fun () ->
              ...
              let b' = implies_intro () in
              (* We might have introduced a new existential, so possibly recurse *)
              sk_binder' (bx :: acc) b')
          ((* If the above failed, just return *)
            fun () -> (acc, b)))
