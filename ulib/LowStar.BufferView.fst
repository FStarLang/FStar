...
type buffer_view (a: Type0) (b: Type u#b) : Type0 =
  | BufferView : buf: buffer a -> v: view a b {length buf % View?.n v == 0} -> buffer_view a b
...
let as_buffer_mk_buffer_view
  (#src: Type) (#dest: Type) (b: B.buffer src) (v: view src dest {length b % View?.n v == 0}) = ()

let get_view (#b: Type) (v: buffer b) = BufferView?.v (dsnd v)

let get_view_mk_buffer_view
  (#src: Type) (#dest: Type) (b: B.buffer src) (v: view src dest {length b % View?.n v == 0}) = ()

let length (#b: _) (vb: buffer b) : GTot nat = B.length (as_buffer vb) / View?.n (get_view vb)
...
let lt_leq_mul (min: nat) (max: nat{min < max}) (n: nat)
  ...
  let open FStar.Mul in assert ((min * n) + n = (min + 1) * n); assert ((min * n) + n <= max * n)

let split_at_i (#b: _) (vb: buffer b) (i: nat{i < length vb}) (h: HS.mem)
  : GTot
  (frags:
    (Seq.seq (dfst vb) * Seq.lseq (dfst vb) (View?.n (get_view vb)) * Seq.seq (dfst vb))
      { let prefix, as, suffix = frags in
        B.as_seq h (as_buffer vb) == (Seq.append prefix (Seq.append as suffix)) }) =
...
let sel (#b: _) (h: HS.mem) (vb: buffer b) (i: nat{i < length vb}) : GTot b =
  let v = get_view vb in
  let _, as, _ = split_at_i vb i h in
  View?.get v as

let upd (#b: _) (h: HS.mem) (vb: buffer b {live h vb}) (i: nat{i < length vb}) (x: b) : GTot HS.mem =
  let open FStar.Mul in
  let v = get_view vb in
  let prefix, _, suffix = split_at_i vb i h in
  let s1 = Seq.append prefix (Seq.append (View?.put v x) suffix) in
...
let sel_upd1 (#b: _) (vb: buffer b) (i: nat{i < length vb}) (x: b) (h: HS.mem{live h vb})
  ...
  B.g_upd_seq_as_seq (as_buffer vb) (Seq.append prefix (Seq.append as' suffix)) h;
  assert (s' == Seq.append prefix (Seq.append as' suffix));
  let prefix', as'', suffix' = split_at_i vb i h' in
  assert (s' == Seq.append prefix' (Seq.append as'' suffix'));
  Seq.lemma_append_inj prefix (Seq.append as' suffix) prefix' (Seq.append as'' suffix');
  Seq.lemma_append_inj as' suffix as'' suffix';
  assert (as' == as'')

#set-options "--z3rlimit 20"
let sel_upd2
  (#b: _) (vb: buffer b) (i: nat{i < length vb}) (j: nat{j < length vb /\ i <> j}) (x: b)
  (h: HS.mem{live h vb})
  ...
  B.g_upd_seq_as_seq (as_buffer vb) (Seq.append prefix (Seq.append (View?.put v x) suffix)) h;
  if i < j
  then (assert (Seq.equal max0 s_j); assert (Seq.equal max1 s_j'); assert (Seq.equal s_j s_j'))
  else (assert (Seq.equal min0 s_j); assert (Seq.equal min1 s_j'); assert (Seq.equal s_j s_j'))

let sel_upd
  (#b: _) (vb: buffer b) (i: nat{i < length vb}) (j: nat{j < length vb}) (x: b)
  (h: HS.mem{live h vb}) = if i = j then sel_upd1 vb i x h else sel_upd2 vb i j x h
...
let rec as_seq' (#b: _) (h: HS.mem) (vb: buffer b) (i: nat{i <= length vb})
  ...
  then Seq.empty
    ...
    Seq.cons (View?.get v s_i) (as_seq' h vb (i + 1))
