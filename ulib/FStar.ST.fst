...
sub_effect DIV ~> GST { lift_wp = lift_div_gst }

let heap_rel (h1: heap) (h2: heap) =
  forall (a: Type0) (rel: preorder a) (r: mref a rel).
    contains h1 r ==> (contains h2 r /\ rel (sel h1 r) (sel h2 r))
assume
...
assume
val gst_witness: p: heap_predicate ->
  GST unit (fun post h0 -> stable p /\ p h0 /\ (witnessed p ==> post () h0))
assume
val gst_recall: p: heap_predicate ->
  GST unit (fun post h0 -> stable p /\ witnessed p /\ (p h0 ==> post () h0))

val lemma_functoriality: 
  p: heap_predicate{stable p /\ witnessed p} ->
...
sub_effect GST ~> STATE { lift_wp = lift_gst_state }
...
let contains_pred (#a: Type0) (#rel: preorder a) (r: mref a rel) = fun h -> contains h r
...
let recall (#a: Type) (#rel: preorder a) (r: mref a rel)
  : STATE unit (fun p h -> Heap.contains h r ==> p () h) = gst_recall (contains_pred r)
abstract
...
let write (#a: Type) (#rel: preorder a) (r: mref a rel) (v: a)
  : ST unit
    (fun h -> rel (sel h r) v)
    (fun h0 x h1 ->
        rel (sel h0 r) v /\ contains h0 r /\ modifies (Set.singleton (addr_of r)) h0 h1 /\
...
abstract
let op_Bang (#a: Type) (#rel: preorder a) (r: mref a rel) : STATE a (fun p h -> p (sel h r) h) =
  read #a #rel r
abstract
let op_Colon_Equals (#a: Type) (#rel: preorder a) (r: mref a rel) (v: a)
  : ST unit
    ...
    (fun h0 x h1 ->
        rel (sel h0 r) v /\ contains h0 r /\ modifies (Set.singleton (addr_of r)) h0 h1 /\
        equal_dom h0 h1 /\ sel h1 r == v) = write #a #rel r v

type ref (a: Type0) = mref a (trivial_preorder a)

let modifies_none (h0: heap) (h1: heap) = modifies FStar.Set.empty h0 h1
