...
let exact (t: term) : Tac unit = with_policy SMT (fun () -> t_exact false t)


(** Like [exact], but allows for the term [e] to have a type [t] only
under some guard [g], adding the guard as a goal. *)
let exact_guard (t: term) : Tac unit = with_policy Goal (fun () -> t_exact false t)
...
let fresh_binder_named nm t = mk_binder (fresh_bv_named nm t)
...
let guard (b: bool) : Tac unit = if b then () else fail "guard failed"
...
let __witness #a x #p _ =
  let id (a: Type) = a in
  //an indirection to tickle the SMT encoding
  let x: squash (exists x. id (p x)) = () in
  x
    ...
    | C_Lemma pre post ->
      ...
      (match trytac (fun () -> apply_lemma t) with
        ...
        | None ->
          ...
          | Implies p q -> apply (`push1); apply_squash_or_lem (d - 1) t
...
let dump_admit () : Tac unit =
  // gets rid of the unit binder
  clear_top ();
  admit1 ()

assume
val admit_goal: #a: Type -> unit ->
  Pure a (requires (by_tactic dump_admit a)) (ensures (fun _ -> False))

let change_with t1 t2 = focus (fun () -> grewrite t1 t2; iseq [idtac; trivial])

private
val conv: #x: Type -> #y: Type -> squash (y == x) -> x -> y
private
let conv #x #y eq w = w

let change_sq t1 = change (mk_e_app (`squash) [t1])
