...
let smt () : Tac unit =
  ...
  | g :: gs, gs' -> set_goals gs; set_smt_goals (g :: gs')
...
let apply (t: term) : Tac unit = t_apply true t
...
let apply_raw (t: term) : Tac unit = t_apply false t
...
let cur_module () : Tac (list string) = moduleof (cur_env ())
...
let fresh_binder_named nm t : Tac binder = mk_binder (fresh_bv_named nm t)
...
let guard (b: bool)
  : TacH unit
    (requires (fun _ -> True))
    (ensures
      (fun ps r -> if b then Success? r /\ Success?.ps r == ps else Failed? r /\ Failed?.ps r == ps)
...
val op_Less_Bar_Greater: (unit -> Tac 'a) -> (unit -> Tac 'a) -> (unit -> Tac 'a)
let op_Less_Bar_Greater t1 t2 = fun () -> or_else t1 t2
...
let rec repeatseq (#a: Type) (t: (unit -> Tac a)) : Tac unit =
  let _ = trytac (fun () -> seq (discard t) (discard (fun () -> repeatseq t))) in
  ()
...
let intros' () : Tac unit =
  let _ = intros () in
  ()
let destruct tm : Tac unit =
  let _ = t_destruct tm in
  ()
let destruct_intros tm : Tac unit =
  seq (fun () ->
        let _ = t_destruct tm in
        ())
    intros'
...
let pose (t: term) : Tac binder =
  ...
  flip ();
  exact t;
  // maybe we have less than 2 goals now
  let _ = trytac flip in
  intro ()
...
let for_each_binder (f: (binder -> Tac 'a)) : Tac (list 'a) = map f (binders_of_env (cur_env ()))

let rec revert_all (bs: binders) : Tac unit =
  match bs with
  | [] -> ()
  | _ :: tl -> revert (); revert_all tl
...
let assumption () : Tac unit = __assumption_aux (binders_of_env (cur_env ()))
...
let rec rewrite_all_context_equalities (bs: binders) : Tac unit =
  ...
  | [] -> ()
  | x_t :: bs ->
    (match term_as_formula_total (type_of_binder x_t) with
      | Comp (Eq _) lhs _ ->
...
let rewrite_equality (t: term) : Tac unit = try_rewrite_equality t (binders_of_env (cur_env ()))
...
let grewrite' (t1: term) (t2: term) (eq: term) : Tac unit =
  let g = cur_goal () in
  match term_as_formula g with
  | Comp (Eq _) l _ -> if term_eq l t1 then exact eq else trefl ()
  | _ -> fail "impossible"
...
let l_to_r (lems: list term) : Tac unit =
  let first_or_trefl () : Tac unit =
    fold_left (fun k l () -> or_else (fun () -> apply_lemma l) k) trefl lems ()
  in
...
let rec iseq (ts: list (unit -> Tac unit)) : Tac unit =
  ...
  | [] -> ()
private
val __witness: #a: Type -> x: a -> #p: (a -> Type) -> squash (p x) -> squash (l_Exists p)
private
...
private
val push1: #p: Type -> #q: Type -> squash (p ==> q) -> squash p -> squash q
private
let push1 #p #q f u = ()
    ...
    | C_Lemma pre post ->
      ...
      (match trytac (fun () -> apply_lemma t) with
        ...
        | None ->
          ...
          | Implies p q -> apply_lemma (`push1); apply_squash_or_lem (d - 1) t
          | _ -> fail "mapply: can't apply (1)")
    | C_Total rt _ ->
      (match unsquash rt with
        | Some _ ->
...
val admit_dump: 
  #a: Type ->
  (#[(dump "Admitting"; exact (quote (fun () -> admit #a () <: Admit a)))] x: (unit -> Admit a)) ->
  unit ->
  Admit a
let admit_dump #a #x () = x ()

val magic_dump: #a: Type -> (#[(dump "Admitting"; exact (quote (magic #a ())))] x: a) -> unit ->
  Tot a
let magic_dump #a #x () = x

let change_with t1 t2 : Tac unit = focus (fun () -> grewrite t1 t2; iseq [idtac; trivial])
