...
effect ST (a: Type) (pre: st_pre) (post: (h0: t -> Tot (st_post' a (pre h0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: t) -> pre h /\ (forall a h1. pre h /\ post h a h1 ==> p a h1))
effect St (a: Type) = ST a (fun h -> True) (fun h0 r h1 -> True)
sub_effect DIV ~> STATE {
  lift_wp = fun (a: Type) (wp: pure_wp a) (p: st_post a) (h: t) -> wp (fun a -> p a h)
}
...
val lemma_as_ref_inj: #a: Type -> #i: rid -> r: rref i a ->
  Lemma (requires (True)) (ensures ((ref_as_rref i (as_ref r) == r))) [SMTPat (as_ref r)]
...
val new_region: unit ->
  ST rid
    (requires (fun m -> True))
    (ensures
      (fun (m0: t) (id: rid) (m1: t) ->
          exists (h: heap). not (Map.contains m0 id) /\ m1 == Map.upd m0 id h))
...
assume
val ralloc: #a: Type -> i: rid -> init: a ->
  ST (rref i a)
...
assume
val op_Colon_Equals: #a: Type -> #i: rid -> r: rref i a -> v: a ->
  ST unit
    (requires (fun m -> True))
    (ensures (fun m0 _u m1 -> m1 == Map.upd m0 i (Heap.upd (Map.sel m0 i) (as_ref r) v)))
assume
val op_Bang: #a: Type -> #i: rid -> r: rref i a ->
  ST a
    (requires (fun m -> True))
    (ensures (fun m0 x m1 -> m1 == m0 /\ x == Heap.sel (Map.sel m0 i) (as_ref r)))
assume
val get: unit -> ST t (requires (fun m -> True)) (ensures (fun m0 x m1 -> m0 == x /\ m1 == m0))

let modifies (s: Set.set rid) (m0: t) (m1: t) =
  Map.equal m1 (Map.concat m1 (Map.restrict (Set.complement s) m0))

let fresh_region (i: rid) (m0: t) (m1: t) = not (Map.contains m0 i) /\ Map.contains m1 i
...
let fresh_rref (#a: Type) (#i: rid) (r: rref i a) (m0: t) (m1: t) =
  ~(Heap.contains (Map.sel m0 i) (as_ref r)) /\ (Heap.contains (Map.sel m1 i) (as_ref r))
