...
abstract
let shift_right (a s: t)
    : Pure t (requires (v s < n)) (ensures (fun c -> FStar.UInt.shift_right (v a) (v s) = v c)) =
  Mk (shift_right (v a) (v s))

(** Shift left with zero fill, shifting at most the integer width *)
abstract
let shift_left (a s: t)
    : Pure t (requires (v s < n)) (ensures (fun c -> FStar.UInt.shift_left (v a) (v s) = v c)) =
