...
let rec default_decreaser (#a: Type u#a) (x: a) : Tot (acc_classical (default_relation #a) x) (decreases x) =
  let smaller (y: a{default_relation y x}) : acc_classical (default_relation #a) y =
    default_decreaser y in
  AccClassicalIntro smaller

let default_wfr (a: Type u#a) : (wfr: wfr_t a{wfr.relation == default_relation}) =
  let proof (x1: a) (x2: a) : Lemma (requires default_relation x1 x2) (ensures default_decreaser x1 << default_decreaser x2) =
    assert ((default_decreaser x2).access_smaller x1 == default_decreaser x1) in
...
let rec empty_decreaser (#a: Type u#a) (x: a) : Tot (acc_classical (empty_relation #a) x) (decreases x) =
  let smaller (y: a{empty_relation y x}) : acc_classical (empty_relation #a) y =
    empty_decreaser y in
  AccClassicalIntro smaller

let empty_wfr (a: Type u#a) : (wfr: wfr_t a{wfr.relation == empty_relation}) =
  let proof (x1: a) (x2: a) : Lemma (requires empty_relation x1 x2) (ensures empty_decreaser x1 << empty_decreaser x2) =
    assert ((empty_decreaser x2).access_smaller x1 == empty_decreaser x1) in
  { relation = empty_relation; decreaser = empty_decreaser; proof = proof; }

let rec acc_decreaser (#a: Type u#a) (r: a -> a -> Type0)
                      (f: FStar.WellFounded.well_founded r{forall x1 x2 (p: r x1 x2). (f x2).access_smaller x1 p == f x1})
                      (x: a) : Tot (acc_classical (acc_relation r) x) (decreases (f x)) =
...
let rec subrelation_decreaser (#a: Type u#a) (r: a -> a -> Type0) (wfr: wfr_t a{forall x1 x2. r x1 x2 ==> wfr.relation x1 x2}) (x: a)
  ...
  let smaller (y: a{r y x}) : (acc_classical r y) =
    subrelation_decreaser r wfr y in
  AccClassicalIntro smaller

let subrelation_to_wfr (#a: Type u#a) (r: a -> a -> Type0) (wfr: wfr_t a{forall x1 x2. r x1 x2 ==> wfr.relation x1 x2})
  : (wfr': wfr_t a{wfr'.relation == r}) =
  let proof (x1: a) (x2: a) : Lemma (requires r x1 x2) (ensures subrelation_decreaser r wfr x1 << subrelation_decreaser r wfr x2) =
    assert ((subrelation_decreaser r wfr x2).access_smaller x1 == subrelation_decreaser r wfr x1) in
...
let rec inverse_image_decreaser  (#a: Type u#a) (#b: Type u#b) (r: a -> a -> Type0) (f: a -> b)
                                 (wfr: wfr_t b{forall x1 x2. r x1 x2 ==> wfr.relation (f x1) (f x2)}) (x: a)
  : Tot (acc_classical r x) (decreases wfr.decreaser (f x)) =
  let smaller (y: a{r y x}) : (acc_classical r y) =
    inverse_image_decreaser r f wfr y in
  AccClassicalIntro smaller

let inverse_image_to_wfr (#a: Type u#a) (#b: Type u#b) (r: a -> a -> Type0) (f: a -> b)
                         (wfr: wfr_t b{forall x1 x2. r x1 x2 ==> wfr.relation (f x1) (f x2)})
  : (wfr': wfr_t a{wfr'.relation == r}) =
  let proof (x1: a) (x2: a) : Lemma (requires r x1 x2) (ensures inverse_image_decreaser r f wfr x1 << inverse_image_decreaser r f wfr x2) =
    assert ((inverse_image_decreaser r f wfr x2).access_smaller x1 == inverse_image_decreaser r f wfr x1) in
  { relation = r; decreaser = inverse_image_decreaser r f wfr; proof = proof; }

let rec lex_nondep_decreaser (#a: Type u#a) (#b: Type u#b) (wfr_a: wfr_t a) (wfr_b: wfr_t b) (x: a * b)
  : Tot (acc_classical (lex_nondep_relation wfr_a wfr_b) x) (decreases %[wfr_a.decreaser (fst x); wfr_b.decreaser (snd x)]) =
  let smaller (y: a * b{lex_nondep_relation wfr_a wfr_b y x}) : (acc_classical (lex_nondep_relation wfr_a wfr_b) y) =
    lex_nondep_decreaser wfr_a wfr_b y in
  AccClassicalIntro smaller
...
let lex_nondep_wfr (#a: Type u#a) (#b: Type u#b) (wfr_a: wfr_t a) (wfr_b: wfr_t b)
  ...
  let proof (x1: a * b) (x2: a * b)
    ...
    assert ((lex_nondep_decreaser wfr_a wfr_b x2).access_smaller x1 == lex_nondep_decreaser wfr_a wfr_b x1) in
  { relation = lex_nondep_relation wfr_a wfr_b; decreaser = lex_nondep_decreaser wfr_a wfr_b; proof = proof; }

let rec lex_dep_decreaser (#a: Type u#a) (#b: a -> Type u#b) (wfr_a: wfr_t a) (a_to_wfr_b: (x: a -> wfr_t (b x))) (x: (v: a & b v))
  : Tot (acc_classical (lex_dep_relation wfr_a a_to_wfr_b) x)
        (decreases %[wfr_a.decreaser (dfst x); (a_to_wfr_b (dfst x)).decreaser (dsnd x)]) =
  let smaller (y: (v: a & b v){lex_dep_relation wfr_a a_to_wfr_b y x}) : (acc_classical (lex_dep_relation wfr_a a_to_wfr_b) y) =
    lex_dep_decreaser wfr_a a_to_wfr_b y in
  AccClassicalIntro smaller

let lex_dep_wfr (#a: Type u#a) (#b: a -> Type u#b) (wfr_a: wfr_t a) (a_to_wfr_b: (x: a -> wfr_t (b x)))
  ...
  let proof (x1: (v: a & b v)) (x2: (v: a) & b v)
    : Lemma (requires lex_dep_relation wfr_a a_to_wfr_b x1 x2)
            (ensures  lex_dep_decreaser wfr_a a_to_wfr_b x1 << lex_dep_decreaser wfr_a a_to_wfr_b x2) =
    assert ((lex_dep_decreaser wfr_a a_to_wfr_b x2).access_smaller x1 == lex_dep_decreaser wfr_a a_to_wfr_b x1) in
  { relation = lex_dep_relation wfr_a a_to_wfr_b; decreaser = lex_dep_decreaser wfr_a a_to_wfr_b; proof = proof; }
...
let option_wfr (#a: Type u#a) (wfr: wfr_t a) : wfr': wfr_t (option a){wfr'.relation == option_relation wfr} =
