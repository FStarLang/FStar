...
and struct_typ_supported (l: list (string * P.typ)) : Tot bool =
  match l with
  | [] -> true
...
let as_seq (#a: typ) (h: HS.mem) (b: buffer a)
  : GTot (s: Seq.seq (P.type_of_typ a) {Seq.length s == length b}) = P.buffer_as_seq h b
...
unfold
let includes (#a: typ) (x: buffer a) (y: buffer a) : GTot Type0 = P.buffer_includes x y

let includes_live (#a: typ) (h: HS.mem) (x: buffer a) (y: buffer a)
  : Lemma (requires (includes x y /\ live h x)) (ensures (live h y)) = P.buffer_includes_elim x y

let includes_as_seq #a h1 h2 (x: buffer a) (y: buffer a)
  : Lemma (requires (includes x y /\ as_seq h1 x == as_seq h2 x))
    (ensures (as_seq h1 y == as_seq h2 y)) = P.buffer_includes_elim x y

let includes_trans #a (x: buffer a) (y: buffer a) (z: buffer a)
  : Lemma (requires (includes x y /\ includes y z)) (ensures (includes x z)) =
  P.buffer_includes_trans x y z

unfold
let disjoint (#a: typ) (#a': typ) (x: buffer a) (y: buffer a') : GTot Type0 =
  P.loc_disjoint (P.loc_buffer x) (P.loc_buffer y)

(* Disjointness is symmetric *)
let lemma_disjoint_symm #a #a' (x: buffer a) (y: buffer a')
  : Lemma (requires True) (ensures (disjoint x y <==> disjoint y x)) [SMTPat (disjoint x y)] = ()
...
let lemma_live_disjoint #a #a' h (b: buffer a) (b': buffer a')
  : Lemma (requires (live h b /\ unused_in b' h))
    (ensures (disjoint b b'))
    [SMTPat (disjoint b b'); SMTPat (live h b)] = ()

(** Concrete getters and setters *)
val create: #a: typ -> init: P.type_of_typ a -> len: UInt32.t ->
  HST.StackInline (buffer a)
    (requires (fun h -> UInt32.v len > 0))
    (ensures
      (fun (h0: HS.mem) b h1 ->
          UInt32.v len > 0 /\ unused_in b h0 /\ live h1 b /\ length b == UInt32.v len /\
          frameOf b == (HS.get_tip h0) /\ P.modifies_0 h0 h1 /\
...
val createL: #a: typ -> init: list (P.type_of_typ a) ->
  HST.StackInline (buffer a)
    (requires (fun h -> p #a init))
    (ensures
      (fun (h0: HS.mem) b h1 ->
          let len = FStar.List.Tot.length init in
          len > 0 /\ unused_in b h0 /\ live h1 b /\ length b == len /\ frameOf b == (HS.get_tip h0) /\
          P.modifies_0 h0 h1 /\ as_seq h1 b == Seq.seq_of_list init /\ q #a len b))

#set-options "--initial_fuel 1 --max_fuel 1"
//the normalize_term (length init) in the pre-condition will be unfolded
...
val rcreate: #a: typ -> r: HH.rid -> init: P.type_of_typ a -> len: UInt32.t ->
  HST.ST (buffer a)
    (requires
      (fun h ->
          HST.is_eternal_region r /\ HST.witnessed (HST.region_contains_pred r) /\ UInt32.v len > 0)
    )
    (ensures
      (fun (h0: HS.mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ length b == UInt32.v len /\
          (HS.get_tip h1) == (HS.get_tip h0) /\ P.modifies (P.loc_addresses r Set.empty) h0 h1 /\
          as_seq h1 b == Seq.create (UInt32.v len) init))
...
val index: #a: typ -> b: buffer a -> n: UInt32.t ->
  HST.Stack (P.type_of_typ a)
    (requires (fun h -> UInt32.v n < length b /\ live h b))
    (ensures
      (fun h0 z h1 -> UInt32.v n < length b /\ h1 == h0 /\ z == Seq.index (as_seq h0 b) (UInt32.v n)
      ))

let index #a b n = P.read_buffer b n

val upd: #a: typ -> b: buffer a -> n: UInt32.t -> z: P.type_of_typ a ->
  HST.Stack unit
    (requires (fun h -> live h b /\ UInt32.v n < length b))
    (ensures
      (fun h0 _ h1 ->
          live h1 b /\ UInt32.v n < length b /\
...
unfold
let gsub (#a: typ) (b: buffer a) (i: UInt32.t) (len: UInt32.t)
  : Ghost (buffer a) (requires (UInt32.v i + UInt32.v len <= length b)) (ensures (fun _ -> True)) =
  P.gsub_buffer b i len

let sub (#a: typ) (b: buffer a) (i: UInt32.t) (len: UInt32.t)
  : HST.Stack (buffer a)
    (requires (fun h -> live h b /\ UInt32.v i + UInt32.v len <= length b))
    (ensures
      (fun h0 b' h1 ->
          live h0 b /\ UInt32.v i + UInt32.v len <= length b /\ h1 == h0 /\ b' == gsub b i len /\
          includes b b')) = P.sub_buffer b i len

let sub_sub (#a: typ) (b: buffer a) (i1: UInt32.t) (len1: UInt32.t) (i2: UInt32.t) (len2: UInt32.t)
  : Lemma
    (requires
      (UInt32.v i1 + UInt32.v len1 <= length b /\ UInt32.v i2 + UInt32.v len2 <= UInt32.v len1))
    (ensures
      (UInt32.v i1 + UInt32.v len1 <= length b /\ UInt32.v i2 + UInt32.v len2 <= UInt32.v len1 /\
        gsub (gsub b i1 len1) i2 len2 == gsub b (UInt32.add i1 i2) len2)) = ()

let sub_zero_length (#a: typ) (b: buffer a)
  : Lemma (ensures (gsub b (UInt32.uint_to_t 0) (UInt32.uint_to_t (length b)) == b)) = ()

let lemma_sub_spec (#a: typ) (b: buffer a) (i: UInt32.t) (len: UInt32.t) (h: HS.mem)
  ...
  Seq.lemma_eq_intro (as_seq h (gsub b i len))
...
let goffset (#a: typ) (b: buffer a) (i: UInt32.t)
  : Ghost (buffer a)
    ...
    (ensures (fun b' -> UInt32.v i <= length b /\ b' == gsub b i (UInt32.sub (P.buffer_length b) i))
...
let lemma_offset_spec (#a: typ) (b: buffer a) (i: UInt32.t) (h: HS.mem)
  : Lemma (requires (UInt32.v i <= length b /\ live h b))
    (ensures
      (UInt32.v i <= length b /\
        as_seq h (goffset b i) == Seq.slice (as_seq h b) (UInt32.v i) (length b))) = ()

abstract
val eqb: #a: typ -> b1: buffer a -> b2: buffer a -> len: UInt32.t ->
  HST.ST bool
    (requires
      (fun h ->
          hasEq (P.type_of_typ a) /\ UInt32.v len <= length b1 /\ UInt32.v len <= length b2 /\
          live h b1 /\ live h b2))
    (ensures
      (fun h0 z h1 ->
          h1 == h0 /\ UInt32.v len <= length b1 /\ UInt32.v len <= length b2 /\
...
val op_Array_Access: #a: typ -> b: buffer a -> n: UInt32.t ->
  HST.Stack (P.type_of_typ a)
    (requires (fun h -> UInt32.v n < length b /\ live h b))
    (ensures
      (fun h0 z h1 -> h1 == h0 /\ UInt32.v n < length b /\ z == Seq.index (as_seq h0 b) (UInt32.v n)
      ))
let op_Array_Access #a b n = index #a b n

val op_Array_Assignment: #a: typ -> b: buffer a -> n: UInt32.t -> z: P.type_of_typ a ->
  HST.Stack unit
    (requires (fun h -> live h b /\ UInt32.v n < length b))
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h1 b /\ UInt32.v n < length b /\
          P.modifies (P.loc_pointer (P.gpointer_of_buffer_cell b n)) h0 h1 /\
          as_seq h1 b == Seq.upd (as_seq h0 b) (UInt32.v n) z))
let op_Array_Assignment #a b n z = upd #a b n z

val live_slice_middle: #t: typ -> b: buffer t -> i: UInt32.t -> len: UInt32.t -> h: HS.mem ->
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= length b /\ live h (gsub b 0ul i) /\ live h (gsub b i len) /\
...
val blit: 
  ...
  idx_b: UInt32.t ->
  len:
  UInt32.t{UInt32.v idx_a + UInt32.v len <= length a /\ UInt32.v idx_b + UInt32.v len <= length b} ->
  HST.Stack unit
    (requires (fun h -> live h a /\ live h b))
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h0 a /\ live h1 b /\ live h1 a /\
          P.modifies (P.loc_buffer (gsub b idx_b len)) h0 h1 /\
          ...
          Seq.slice (as_seq h1 b) 0 (UInt32.v idx_b) == Seq.slice (as_seq h0 b) 0 (UInt32.v idx_b) /\
...
abstract
val fill: #t: typ -> b: buffer t -> z: P.type_of_typ t -> len: UInt32.t{UInt32.v len <= length b} ->
  HST.Stack unit
    (requires (fun h -> live h b))
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h1 b /\ P.modifies (P.loc_buffer (gsub b 0ul len)) h0 h1 /\
