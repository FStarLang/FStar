...
let div_eucl (a: int) (b: nonzero) : Tot int =
  if a < 0 then if a % b = 0 then - (- a / b) else - (- a / b) - 1 else a / b
...
(* Addition primitives *)
val add: #n: pos -> a: int_t n -> b: int_t n ->
...
val sub: #n: pos -> a: int_t n -> b: int_t n ->
  Pure (int_t n) (requires (size (a - b) n)) (ensures (fun _ -> True))
let sub #n a b = a - b
...
val mul: #n: pos -> a: int_t n -> b: int_t n ->
  Pure (int_t n) (requires (size (a * b) n)) (ensures (fun _ -> True))
let mul #n a b = a * b
...
(* Division primitives *)
val div: #n: pos -> a: int_t n -> b: int_t n {b <> 0} ->
...
(* Bitwise operators *)
assume
val logand: #n: pos -> int_t n -> int_t n -> Tot (int_t n)
assume
val logxor: #n: pos -> int_t n -> int_t n -> Tot (int_t n)
assume
val logor: #n: pos -> int_t n -> int_t n -> Tot (int_t n)
assume
val lognot: #n: pos -> int_t n -> Tot (int_t n)
...
(* Shift operators *)
assume
val shift_right: #n: pos -> a: int_t n -> s: nat ->
  Pure (int_t n) (requires True) (ensures (fun b -> b = (a /% (pow2 s))))
