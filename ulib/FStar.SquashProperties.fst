...
val squash_arrow: #a: Type -> #p: (a -> Type) -> $f: (x: a -> GTot (squash (p x))) ->
...
val bool_of_or: #p: Type -> #q: Type -> c_or p q -> Tot (b: bool{(b ==> p) /\ (not (b) ==> q)})
...
let excluded_middle_squash p =
  bind_squash (excluded_middle p)
    (fun x ->
        if x
        then map_squash (get_proof p) Left
        else
          return_squash (Right
              (return_squash (fun (h: p) -> give_proof (return_squash h); false_elim #False ()))))
...
type retract 'a 'b : Type =
  | MkR : i: ('a -> GTot 'b) -> j: ('b -> GTot 'a) -> inv: (x: 'a -> GTot (ceq (j (i x)) x)) ->
    retract 'a 'b
noeq
...
(* unused below *)
val ac: r: retract_cond 'a 'b -> retract 'a 'b -> x: 'a -> GTot (ceq ((MkC?.j2 r) (MkC?.i2 r x)) x)
let ac (MkC _ _ inv2) = inv2

let false_elim (#a: Type) (f: False) : Tot a = match f with 
...
let l1 (a: Type) (b: Type) =
  bind_squash (excluded_middle_squash (retract (pow a) (pow b)))
    (fun (x: c_or (retract (pow a) (pow b)) (~(retract (pow a) (pow b)))) ->
        match x with
        | Left (MkR f0 g0 e) -> return_squash (MkC f0 g0 (fun _ -> e))
        | Right nr ->
...
(* let f x = x u  *)
//fails here without a means of denoting universes
