...
let lemma_size (x: int)
  : Lemma (requires (UInt.size x n)) (ensures (x >= 0)) [SMTPat (UInt.size x n)] = ()

let lseq (a: Type) (l: nat) : Type = (s: seq a {Seq.length s == l})
...
val recall: #a: Type -> b: buffer a {is_eternal_region (frameOf b) /\ not (is_mm b.content)} ->
  Stack unit (requires (fun m -> True)) (ensures (fun m0 _ m1 -> m0 == m1 /\ live m1 b))
...
let get #a h (b: buffer a) (i: nat{i < length b}) : GTot a = Seq.index (as_seq h b) i

(* Equality predicate on buffer contents, without quantifiers *)
//17-01-04 revise comment? rename?
let equal #a h (b: buffer a) h' (b': buffer a) : GTot Type0 = as_seq h b == as_seq h' b'
...
let includes_live #a h (x: buffer a) (y: buffer a)
  : Lemma (requires (includes x y)) (ensures (live h x <==> live h y)) = ()
...
val lemma_live_disjoint: #a: Type -> #a': Type -> h: mem -> b: buffer a -> b': buffer a' ->
...
assume
val arefs: TSet.set abuffer -> Tot (Set.set nat)
...
val lemma_arefs_1: s: TSet.set abuffer ->
  Lemma (requires (s == TSet.empty #abuffer))
    (ensures (arefs s == Set.empty #nat))
    [SMTPat (arefs s)]
let lemma_arefs_1 s = Set.lemma_equal_intro (arefs s) (Set.empty)

val lemma_arefs_2: s1: TSet.set abuffer -> s2: TSet.set abuffer ->
  Lemma (requires True)
    (ensures (arefs (s1 ++ s2) == Set.union (arefs s1) (arefs s2)))
    [SMTPatOr [[SMTPat (arefs (s2 ++ s1))]; [SMTPat (arefs (s1 ++ s2))]]]
let lemma_arefs_2 s1 s2 = Set.lemma_equal_intro (arefs (s1 ++ s2)) (Set.union (arefs s1) (arefs s2))

val lemma_arefs_3: s1: TSet.set abuffer -> s2: TSet.set abuffer ->
  Lemma (requires (TSet.subset s1 s2)) (ensures (Set.subset (arefs s1) (arefs s2)))
...
let disjoint_ref_1 (#t: Type) a (r: reference t) =
...
val disjoint_only_lemma: #a: Type -> #a': Type -> b: buffer a -> b': buffer a' ->
  Lemma (requires (disjoint b b')) (ensures (disjoint_from_bufs b (only b')))
...
let modifies_bufs_and_refs (bufs: TSet.set abuffer) (refs: Set.set nat) h h' : GTot Type0 =
  (forall rid.
      Set.mem rid (Map.domain h.h) ==>
      (HS.modifies_ref rid (Set.union (arefs bufs) refs) h h' /\
        (forall (#a: Type) (b: buffer a).
            (frameOf b == rid /\ live h b /\ disjoint_from_bufs b bufs /\ disjoint_from_refs b refs) ==>
            equal h b h' b /\ live h' b)))
...
let modifies_buffers (bufs: TSet.set abuffer) h h' : GTot Type0 =
  (forall rid.
      ...
      (HS.modifies_ref rid (arefs bufs) h h' /\
        (forall (#a: Type) (b: buffer a).
            {:pattern (frameOf b == rid /\ live h b /\ disjoint_from_bufs b bufs)}
            (frameOf b == rid /\ live h b /\ disjoint_from_bufs b bufs ==>
              equal h b h' b /\ live h' b))))
...
let modifies_none h h' = h'.tip == h.tip /\ HS.modifies_transitively Set.empty h h'
...
//would be good to drop the rid argument on these, since they can be computed from the buffers
...
let to_set_3 (n1: nat) (n2: nat) (n3: nat) : Set.set nat =
  Set.union (Set.union (Set.singleton n1) (Set.singleton n2)) (Set.singleton n3)

let modifies_buf_3
  (#t: Type) (#t': Type) (#t'': Type) rid (b: buffer t) (b': buffer t') (b'': buffer t'') h h' =
...
let modifies_buf_4
  (#t: Type) (#t': Type) (#t'': Type) (#t''': Type) rid (b: buffer t) (b': buffer t')
  (b'': buffer t'') (b''': buffer t''') h h' =
...
let lemma_modifies_bufs_sub rid bufs subbufs h0 h1
  : Lemma (requires (TSet.subset subbufs bufs /\ modifies_bufs rid subbufs h0 h1))
    (ensures (modifies_bufs rid bufs h0 h1))
    [SMTPat (modifies_bufs rid subbufs h0 h1); SMTPat (TSet.subset subbufs bufs)] = ()

val lemma_modifies_bufs_subset: 
  #a: Type ->
  #a': Type ->
  h0: mem ->
  h1: mem ->
  bufs: TSet.set abuffer ->
  b: buffer a ->
  b': buffer a' ->
  Lemma (requires (disjoint_from_bufs b (bufs ++ (only b'))))
...
val lemma_modifies_bufs_superset: 
  #a: Type ->
  #a': Type ->
  h0: mem ->
  h1: mem ->
  ...
  b: buffer a ->
  b': buffer a' ->
  Lemma (requires (unused_in b' h0 /\ live h0 b /\ disjoint_from_bufs b bufs))
    (ensures (disjoint_from_bufs b (bufs ++ (only b'))))
    [SMTPat (modifies_bufs h0.tip bufs h0 h1); SMTPat (unmapped_in b' h0); SMTPat (live h0 b)]
let lemma_modifies_bufs_superset #a #a' h0 h1 bufs b b' = ()

(* Specialized lemmas *)
let modifies_trans_0_0 (rid: rid) (h0: mem) (h1: mem) (h2: mem)
...
let modifies_trans_1_0 (#t: Type) (rid: rid) (b: buffer t) (h0: mem) (h1: mem) (h2: mem)
...
let modifies_trans_0_1 (#t: Type) (rid: rid) (b: buffer t) (h0: mem) (h1: mem) (h2: mem)
...
let modifies_trans_1_1 (#t: Type) (rid: rid) (b: buffer t) (h0: mem) (h1: mem) (h2: mem)
...
let modifies_trans_1_1'
  (#t: Type) (#t': Type) (rid: rid) (b: buffer t) (b': buffer t') (h0: mem) (h1: mem) (h2: mem)
  : Lemma (requires (modifies_buf_1 rid b h0 h1 /\ modifies_buf_1 rid b' h1 h2))
    (ensures (modifies_buf_2 rid b b' h0 h2))
    [SMTPat (modifies_buf_1 rid b h0 h1); SMTPat (modifies_buf_1 rid b' h1 h2)] = ()

let modifies_trans_2_0
  (#t: Type) (#t': Type) (rid: rid) (b: buffer t) (b': buffer t') (h0: mem) (h1: mem) (h2: mem)
...
let modifies_trans_3_3
  (#t: Type) (#t': Type) (#t'': Type) (rid: rid) (b: buffer t) (b': buffer t') (b'': buffer t'')
...
let modifies_trans_4_4
  (#t: Type) (#t': Type) (#t'': Type) (#t''': Type) (rid: rid) (b: buffer t) (b': buffer t')
  (b'': buffer t'') (b''': buffer t''') (h0: mem) (h1: mem) (h2: mem)
  : Lemma
    (requires (modifies_buf_4 rid b b' b'' b''' h0 h1 /\ modifies_buf_4 rid b b' b'' b''' h1 h2))
    (ensures (modifies_buf_4 rid b b' b'' b''' h0 h2))
    [
      SMTPat (modifies_buf_4 rid b b' b'' b''' h0 h1);
      SMTPat (modifies_buf_4 rid b b' b'' b''' h1 h2)
    ] = ()
...
abstract
let modifies_0 (h0: mem) (h1: mem) : Type0 =
  modifies_one h0.tip h0 h1 /\ modifies_buf_0 h0.tip h0 h1 /\ h0.tip == h1.tip
...
abstract
let modifies_1 (#a: Type) (b: buffer a) (h0: mem) (h1: mem) : Type0 =
  let rid = frameOf b in
  modifies_one rid h0 h1 /\ modifies_buf_1 rid b h0 h1 /\ h0.tip == h1.tip

abstract
let modifies_2_1 (#a: Type) (b: buffer a) (h0: mem) (h1: mem) : Type0 =
  h0.tip == h1.tip /\
  (let rid = frameOf b in
...
abstract
let modifies_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') (h0: mem) (h1: mem) : Type0 =
  h0.tip == h1.tip /\
  (let rid = frameOf b in
    let rid' = frameOf b' in
...
abstract
let modifies_3
  (#a: Type) (#a': Type) (#a'': Type) (b: buffer a) (b': buffer a') (b'': buffer a'') (h0: mem)
  (h1: mem)
  : Type0 =
  h0.tip == h1.tip /\
  (let rid = frameOf b in
...
abstract
let modifies_3_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') (h0: mem) (h1: mem) : Type0 =
  h0.tip == h1.tip /\
  (let rid = frameOf b in
    let rid' = frameOf b' in
    ((rid == rid' /\ rid' == h0.tip /\ modifies_buf_2 rid b b' h0 h1 /\ modifies_one rid h0 h1) \/
      (rid == rid' /\ rid' =!= h0.tip /\ modifies_buf_2 rid b b' h0 h1 /\
      ...
      (rid =!= rid' /\ rid' =!= h0.tip /\ rid =!= h0.tip /\
        HS.modifies (Set.union (Set.union (Set.singleton rid) (Set.singleton rid'))
              (Set.singleton h0.tip))
          h0
          h1 /\ modifies_buf_1 rid b h0 h1 /\ modifies_buf_1 rid' b' h0 h1 /\
        modifies_buf_0 h0.tip h0 h1)))

abstract
let modifies_region (rid: rid) (bufs: TSet.set abuffer) (h0: mem) (h1: mem) : Type0 =
  modifies_one rid h0 h1 /\ modifies_bufs rid bufs h0 h1 /\ h0.tip == h1.tip

(* Lemmas introducing the 'modifies' predicates *)
let lemma_intro_modifies_0 h0 h1
  : Lemma (requires (modifies_one h0.tip h0 h1 /\ modifies_buf_0 h0.tip h0 h1 /\ h0.tip == h1.tip))
    (ensures (modifies_0 h0 h1)) = ()

let lemma_intro_modifies_1 (#a: Type) (b: buffer a) h0 h1
  : Lemma
    (requires
      (let rid = frameOf b in
        modifies_one rid h0 h1 /\ modifies_buf_1 rid b h0 h1 /\ h0.tip == h1.tip))
...
let lemma_intro_modifies_2_1 (#a: Type) (b: buffer a) h0 h1
  : Lemma
    (requires
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
...
let lemma_intro_modifies_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma
    (requires
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
...
let lemma_intro_modifies_3
  (#a: Type) (#a': Type) (#a'': Type) (b: buffer a) (b': buffer a') (b'': buffer a'') h0 h1
  : Lemma
    (requires
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          let rid' = frameOf b' in
          let rid'' = frameOf b'' in
          ((rid == rid' /\ rid' == rid'' /\ modifies_buf_3 rid b b' b'' h0 h1 /\
              modifies_one rid h0 h1) \/
            (rid == rid' /\ rid' =!= rid'' /\ modifies_buf_2 rid b b' h0 h1 /\
              modifies_buf_1 rid'' b'' h0 h1 /\
              HS.modifies (Set.union (Set.singleton rid) (Set.singleton rid'')) h0 h1) \/
            (rid =!= rid' /\ rid' == rid'' /\ modifies_buf_2 rid' b' b'' h0 h1 /\
            ...
            (rid =!= rid' /\ rid' =!= rid'' /\ rid =!= rid'' /\
              HS.modifies (Set.union (Set.union (Set.singleton rid) (Set.singleton rid'))
                ...
                h0
                h1 /\ modifies_buf_1 rid b h0 h1 /\ modifies_buf_1 rid' b' h0 h1 /\
              modifies_buf_1 rid'' b'' h0 h1))))) (ensures (modifies_3 b b' b'' h0 h1)) = ()

let lemma_intro_modifies_3_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma
    (requires
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          ...
          ((rid == rid' /\ rid' == h0.tip /\ modifies_buf_2 rid b b' h0 h1 /\ modifies_one rid h0 h1
            ...
            (rid == rid' /\ rid' =!= h0.tip /\ modifies_buf_2 rid b b' h0 h1 /\
              modifies_buf_0 h0.tip h0 h1 /\
              HS.modifies (Set.union (Set.singleton rid) (Set.singleton h0.tip)) h0 h1) \/
            (rid =!= rid' /\ rid == h0.tip /\ modifies_buf_1 rid b h0 h1 /\
              modifies_buf_1 rid' b' h0 h1 /\
              HS.modifies (Set.union (Set.singleton rid') (Set.singleton h0.tip)) h0 h1) \/
            (rid =!= rid' /\ rid' == h0.tip /\ modifies_buf_1 rid' b' h0 h1 /\
              modifies_buf_1 rid b h0 h1 /\
            ...
            (rid =!= rid' /\ rid' =!= h0.tip /\ rid =!= h0.tip /\
              HS.modifies (Set.union (Set.union (Set.singleton rid) (Set.singleton rid'))
                ...
                h0
                h1 /\ modifies_buf_1 rid b h0 h1 /\ modifies_buf_1 rid' b' h0 h1 /\
              modifies_buf_0 h0.tip h0 h1))))) (ensures (modifies_3_2 b b' h0 h1)) = ()

let lemma_intro_modifies_region (rid: rid) bufs h0 h1
  : Lemma (requires (modifies_one rid h0 h1 /\ modifies_bufs rid bufs h0 h1 /\ h0.tip == h1.tip))
...
let lemma_reveal_modifies_0 h0 h1
  : Lemma (requires (modifies_0 h0 h1))
    (ensures (modifies_one h0.tip h0 h1 /\ modifies_buf_0 h0.tip h0 h1 /\ h0.tip == h1.tip)) = ()

let lemma_reveal_modifies_1 (#a: Type) (b: buffer a) h0 h1
  : Lemma (requires (modifies_1 b h0 h1))
    (ensures
      (let rid = frameOf b in
        modifies_one rid h0 h1 /\ modifies_buf_1 rid b h0 h1 /\ h0.tip == h1.tip)) = ()

let lemma_reveal_modifies_2_1 (#a: Type) (b: buffer a) h0 h1
  : Lemma (requires (modifies_2_1 b h0 h1))
    (ensures
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          ((rid == h0.tip /\ modifies_buf_1 rid b h0 h1 /\ modifies_one rid h0 h1) \/
...
let lemma_reveal_modifies_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma (requires (modifies_2 b b' h0 h1))
    (ensures
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          let rid' = frameOf b' in
...
let lemma_reveal_modifies_3
  (#a: Type) (#a': Type) (#a'': Type) (b: buffer a) (b': buffer a') (b'': buffer a'') h0 h1
  : Lemma (requires (modifies_3 b b' b'' h0 h1))
    (ensures
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          let rid' = frameOf b' in
...
let lemma_reveal_modifies_3_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma (requires (modifies_3_2 b b' h0 h1))
    (ensures
      (h0.tip == h1.tip /\
        (let rid = frameOf b in
          let rid' = frameOf b' in
...
let lemma_reveal_modifies_region (rid: rid) bufs h0 h1
  : Lemma (requires (modifies_region rid bufs h0 h1))
...
let lemma_stack_1 (#a: Type) (b: buffer a) h0 h1 h2 h3
  : Lemma (requires (live h0 b /\ fresh_frame h0 h1 /\ modifies_1 b h1 h2 /\ popped h2 h3))
    (ensures (modifies_buf_1 (frameOf b) b h0 h3))
    [SMTPat (modifies_1 b h1 h2); SMTPat (fresh_frame h0 h1); SMTPat (popped h2 h3)] = ()

let lemma_stack_2 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2 h3
  : Lemma
    (requires
      (live h0 b /\ live h0 b' /\ fresh_frame h0 h1 /\ modifies_2 b b' h1 h2 /\ popped h2 h3))
...
let lemma_modifies_2_comm (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma (requires True)
    (ensures (modifies_2 b b' h0 h1 <==> modifies_2 b' b h0 h1))
    [SMTPat (modifies_2 b b' h0 h1)] = ()

let lemma_modifies_3_2_comm (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1
  : Lemma (requires True)
...
(** Transitivity lemmas *)
let lemma_modifies_0_trans h0 h1 h2
  : Lemma (requires (modifies_0 h0 h1 /\ modifies_0 h1 h2))
    (ensures (modifies_0 h0 h2))
    [SMTPat (modifies_0 h0 h1); SMTPat (modifies_0 h1 h2)] = ()

let lemma_modifies_1_trans (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (modifies_1 b h0 h1 /\ modifies_1 b h1 h2))
    (ensures (modifies_1 b h0 h2))
    [SMTPat (modifies_1 b h0 h1); SMTPat (modifies_1 b h1 h2)] = ()

let lemma_modifies_2_1_trans (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (modifies_2_1 b h0 h1 /\ modifies_2_1 b h1 h2))
    (ensures (modifies_2_1 b h0 h2))
    [SMTPat (modifies_2_1 b h0 h1); SMTPat (modifies_2_1 b h1 h2)] = ()

let lemma_modifies_2_trans (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2
  : Lemma (requires (modifies_2 b b' h0 h1 /\ modifies_2 b b' h1 h2))
...
let lemma_modifies_3_trans
  (#a: Type) (#a': Type) (#a'': Type) (b: buffer a) (b': buffer a') (b'': buffer a'') h0 h1 h2
  : Lemma (requires (modifies_3 b b' b'' h0 h1 /\ modifies_3 b b' b'' h1 h2))
    (ensures (modifies_3 b b' b'' h0 h2))
    [
      (* TODO: add the appropriate SMTPatOr patterns so as not to rewrite X times the same lemma *)
      SMTPat (modifies_3 b b' b'' h0 h1);
      SMTPat (modifies_3 b b' b'' h1 h2)
    ] = ()

#reset-options "--z3rlimit 200"

let lemma_modifies_3_2_trans (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2
  : Lemma (requires (modifies_3_2 b b' h0 h1 /\ modifies_3_2 b b' h1 h2))
...
val lemma_modifies_0_0: h0: mem -> h1: mem -> h2: mem ->
  Lemma (requires (modifies_0 h0 h1 /\ modifies_0 h1 h2))
    (ensures (modifies_0 h0 h2))
    [SMTPat (modifies_0 h0 h1); SMTPat (modifies_0 h1 h2)]
...
let lemma_modifies_1_0 (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (live h0 b /\ modifies_1 b h0 h1 /\ modifies_0 h1 h2))
    (ensures (live h2 b /\ modifies_2_1 b h0 h2))
    [SMTPat (modifies_1 b h0 h1); SMTPat (modifies_0 h1 h2)] = ()

let lemma_modifies_0_1 (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (live h0 b /\ modifies_0 h0 h1 /\ modifies_1 b h1 h2))
    (ensures (modifies_2_1 b h0 h2))
    [SMTPat (modifies_0 h0 h1); SMTPat (modifies_1 b h1 h2)] = ()

let lemma_modifies_0_1' (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (unused_in b h0 /\ modifies_0 h0 h1 /\ live h1 b /\ modifies_1 b h1 h2))
    (ensures (modifies_0 h0 h2))
    [SMTPat (modifies_0 h0 h1); SMTPat (modifies_1 b h1 h2)] = ()

#reset-options "--z3rlimit 100 --initial_fuel 0 --max_fuel 0"

let lemma_modifies_1_1 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2
...
let lemma_modifies_0_2 (#t: Type) (#t': Type) (b: buffer t) (b': buffer t') h0 h1 h2
  : Lemma
    (requires
      (live h0 b /\ unused_in b' h0 /\ modifies_0 h0 h1 /\ live h1 b' /\ modifies_2 b b' h1 h2))
    (ensures (modifies_2_1 b h0 h2))
...
let lemma_modifies_1_2 (#t: Type) (#t': Type) (b: buffer t) (b': buffer t') h0 h1 h2
  : Lemma
    (requires
      (live h0 b /\ modifies_1 b h0 h1 /\ unused_in b' h0 /\ live h1 b' /\ modifies_2 b b' h1 h2))
    (ensures (modifies_2_1 b h0 h2))
...
let lemma_modifies_1_2'' (#t: Type) (#t': Type) (b: buffer t) (b': buffer t') h0 h1 h2
  : Lemma (requires (live h0 b /\ live h0 b' /\ modifies_1 b h0 h1 /\ modifies_2 b b' h1 h2))
    (ensures (modifies_2 b b' h0 h2))
    [SMTPat (modifies_1 b h0 h1); SMTPat (modifies_2 b b' h1 h2)] = ()
...
let lemma_modifies_1_1_prime (#t: Type) (#t': Type) (b: buffer t) (b': buffer t') h0 h1 h2
  : Lemma
    (requires
      (live h0 b /\ modifies_1 b h0 h1 /\ unused_in b' h0 /\ live h1 b' /\ modifies_1 b' h1 h2))
    (ensures (modifies_2_1 b h0 h2))
    [SMTPat (modifies_1 b h0 h1); SMTPat (modifies_1 b' h1 h2)] = ()

let lemma_modifies_2_1 (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2
  : Lemma (requires (live h0 b /\ live h0 b' /\ modifies_2 b b' h0 h1 /\ modifies_1 b h1 h2))
    (ensures (modifies_2 b b' h0 h2))
...
let lemma_modifies_2_1'' (#a: Type) (#a': Type) (b: buffer a) (b': buffer a') h0 h1 h2
  : Lemma (requires (live h0 b /\ live h0 b' /\ modifies_2_1 b h0 h1 /\ modifies_1 b' h1 h2))
    (ensures (modifies_3_2 b b' h0 h2))
    [SMTPat (modifies_2_1 b h0 h1); SMTPat (modifies_1 b' h1 h2)] = ()

(* TODO: lemmas for modifies_3 *)

let lemma_modifies_0_unalloc (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma
    (requires (unused_in b h0 /\ frameOf b == h0.tip /\ modifies_0 h0 h1 /\ modifies_1 b h1 h2))
    (ensures (modifies_0 h0 h2)) = ()

let lemma_modifies_none_1_trans (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma (requires (modifies_none h0 h1 /\ live h0 b /\ modifies_1 b h1 h2))
    (ensures (modifies_1 b h0 h2)) = ()

let lemma_modifies_0_none_trans h0 h1 h2
  : Lemma (requires (modifies_0 h0 h1 /\ modifies_none h1 h2)) (ensures (modifies_0 h0 h2)) = ()

#reset-options "--initial_fuel 0 --max_fuel 0"


(** Concrete getters and setters *)
val create: #a: Type -> init: a -> len: UInt32.t ->
  StackInline (buffer a)
    (requires (fun h -> True))
    (ensures
      (fun (h0: mem) b h1 ->
          unused_in b h0 /\ live h1 b /\ idx b == 0 /\ length b == v len /\ frameOf b == h0.tip /\
          Map.domain h1.h == Map.domain h0.h /\ modifies_0 h0 h1 /\
...
(** Concrete getters and setters *)
val createL: #a: Type0 -> init: list a ->
  StackInline (buffer a)
    (requires (fun h -> p #a init))
    (ensures
      (fun (h0: mem) b h1 ->
          let len = FStar.List.Tot.length init in
          len > 0 /\ unused_in b h0 /\ live h1 b /\ idx b == 0 /\ length b == len /\
          frameOf b == h0.tip /\ Map.domain h1.h == Map.domain h0.h /\ modifies_0 h0 h1 /\
          as_seq h1 b == Seq.of_list init /\ q #a len b))
...
let createL #a init =
  ...
  let content: reference (lseq a (v len)) = salloc (Seq.of_list init) in
  let b = MkBuffer len content 0ul len in
  let h = HST.get () in
  assert (Seq.equal (as_seq h b) (sel h b));
  b
...
let lemma_upd (#a: Type) (h: mem) (x: reference a {live_region h (HS.frameOf x)}) (v: a)
  : Lemma (requires True) (ensures (Map.domain h.h == Map.domain (upd h x v).h)) =
  let m = h.h in
...
let rcreate_post_common
  (#a: Type) (r: rid) (init: a) (len: UInt32.t) (b: buffer a) (h0: mem) (h1: mem)
  : Type0 =
  unused_in b h0 /\ live h1 b /\ idx b == 0 /\ length b == v len /\
  Map.domain h1.h == Map.domain h0.h /\ h1.tip == h0.tip /\ modifies (Set.singleton r) h0 h1 /\
  modifies_ref r Set.empty h0 h1 /\ as_seq h1 b == Seq.create (v len) init
...
val rcreate: #a: Type -> r: rid -> init: a -> len: UInt32.t ->
  ST (buffer a)
...
val to_seq: #a: Type -> b: buffer a -> l: UInt32.t{v l <= length b} ->
  STL (seq a)
    (requires (fun h -> live h b))
    (ensures
      (*/\ r == as_seq #a h1 b *)
      (fun h0 r h1 -> h0 == h1 /\ live h1 b /\ Seq.length r == v l))
let to_seq #a b l =
  let s = !b.content in
  let i = v b.idx in
  Seq.slice s i (i + v l)
...
// ocaml-only, used for conversions to Platform.bytes
val to_seq_full: #a: Type -> b: buffer a ->
  ST (seq a)
    (requires (fun h -> live h b))
...
val index: #a: Type -> b: buffer a -> n: UInt32.t{v n < length b} ->
  Stack a
    (requires (fun h -> live h b))
    (ensures (fun h0 z h1 -> live h0 b /\ h1 == h0 /\ z == Seq.index (as_seq h0 b) (v n)))
let index #a b n =
  let s = !b.content in
...
private
let lemma_aux_1 (#a: Type) (b: buffer a) (n: UInt32.t{v n < length b}) (z: a) (h0: mem) (tt: Type)
...
private
let lemma_aux_2 (#a: Type) (b: buffer a) (n: UInt32.t{v n < length b}) (z: a) (h0: mem)
  : Lemma (requires (live h0 b))
    (ensures
      (live h0 b /\
        (forall (tt: Type) (bb: buffer tt).
...
private
val lemma_aux: #a: Type -> b: buffer a -> n: UInt32.t{v n < length b} -> z: a -> h0: mem ->
  Lemma (requires (live h0 b))
    (ensures
      (live h0 b /\ modifies_1 b h0 (HS.upd h0 b.content (Seq.upd (sel h0 b) (idx b + v n) z))))
    [SMTPat (HS.upd h0 b.content (Seq.upd (sel h0 b) (idx b + v n) z))]
let lemma_aux #a b n z h0 = lemma_aux_2 b n z h0

abstract
val upd: #a: Type -> b: buffer a -> n: UInt32.t -> z: a ->
  Stack unit
    (requires (fun h -> live h b /\ v n < length b))
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h1 b /\ v n < length b /\ modifies_1 b h0 h1 /\
...
val sub: #a: Type -> b: buffer a -> i: UInt32.t -> len: UInt32.t{v i + v len <= length b} ->
  Tot (b': buffer a {includes b b' /\ length b' == v len})
let sub #a b i len =
  // was formerly a precondition
  assert (v i + v b.idx < pow2 n);
  MkBuffer b.max_length b.content (i +^ b.idx) len

let sub_sub
  (#a: Type) (b: buffer a) (i1: UInt32.t) (len1: UInt32.t{v i1 + v len1 <= length b}) (i2: UInt32.t)
  (len2: UInt32.t{v i2 + v len2 <= v len1})
  : Lemma (ensures (sub (sub b i1 len1) i2 len2 == sub b (i1 +^ i2) len2)) = ()

let sub_zero_length (#a: Type) (b: buffer a)
  : Lemma (ensures (sub b (UInt32.uint_to_t 0) (UInt32.uint_to_t (length b)) == b)) = ()

let lemma_sub_spec
  (#a: Type) (b: buffer a) (i: UInt32.t)
  (len: UInt32.t{v len <= length b /\ v i + v len <= length b}) h
  : Lemma (requires (live h b))
    (ensures
      (live h (sub b i len) /\ as_seq h (sub b i len) == Seq.slice (as_seq h b) (v i) (v i + v len))
    )
...
let lemma_sub_spec'
  ...
  : Lemma (requires (live h b))
    ...
    [SMTPat (live h (sub b i len))] = lemma_sub_spec b i len h

val offset: #a: Type -> b: buffer a -> i: UInt32.t{v i + v b.idx < pow2 n /\ v i <= v b.length} ->
  Tot (b': buffer a {includes b b'})
let offset #a b i = MkBuffer b.max_length b.content (i +^ b.idx) (b.length -^ i)

let lemma_offset_spec
  (#a: Type) (b: buffer a) (i: UInt32.t{v i + v b.idx < pow2 n /\ v i <= v b.length}) h
  : Lemma (requires True)
...
val eq_lemma1: 
  ...
  h: mem ->
  Lemma (requires (forall (j: nat). j < v len ==> get h b1 j == get h b2 j))
...
val eq_lemma2: 
  ...
  len: UInt32.t{v len <= length b1 /\ v len <= length b2} ->
  h: mem ->
  Lemma (requires equal h (sub b1 0ul len) h (sub b2 0ul len))
...
val eqb: 
  ...
  ST bool
    ...
    (ensures (fun h0 z h1 -> h1 == h0 /\ (z <==> equal h0 (sub b1 0ul len) h0 (sub b2 0ul len))))
...
//    *)
val op_Array_Access: #a: Type -> b: buffer a -> n: UInt32.t{v n < length b} ->
  Stack a
    (requires (fun h -> live h b))
    (ensures (fun h0 z h1 -> h1 == h0 /\ z == Seq.index (as_seq h0 b) (v n)))
let op_Array_Access #a b n = index #a b n

val op_Array_Assignment: #a: Type -> b: buffer a -> n: UInt32.t -> z: a ->
  Stack unit
    (requires (fun h -> live h b /\ v n < length b))
    (ensures
      (fun h0 _ h1 ->
...
let lemma_modifies_one_trans_1 (#a: Type) (b: buffer a) (h0: mem) (h1: mem) (h2: mem)
  : Lemma (requires (modifies_one (frameOf b) h0 h1 /\ modifies_one (frameOf b) h1 h2))
...
val blit: 
  ...
  Stack unit
    ...
    (ensures
      (fun h0 _ h1 ->
          live h0 b /\ live h0 a /\ live h1 b /\ live h1 a /\ modifies_1 b h0 h1 /\
...
val fill: #t: Type -> b: buffer t -> z: t -> len: UInt32.t{v len <= length b} ->
  Stack unit
    (requires (fun h -> live h b))
    (ensures
      (fun h0 _ h1 ->
...
let join
  ...
  (b':
    buffer t
      {b.max_length == b'.max_length /\ b.content === b'.content /\ idx b + length b == idx b'})
  : Tot (buffer t) =
  MkBuffer (b.max_length) (b.content) (b.idx) (let open FStar.UInt32 in b.length +^ b'.length)

val no_upd_lemma_0: #t: Type -> h0: mem -> h1: mem -> b: buffer t ->
  Lemma (requires (live h0 b /\ modifies_0 h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
    [SMTPat (modifies_0 h0 h1); SMTPat (live h0 b)]
let no_upd_lemma_0 #t h0 h1 b = ()

val no_upd_lemma_1: #t: Type -> #t': Type -> h0: mem -> h1: mem -> a: buffer t -> b: buffer t' ->
  Lemma (requires (live h0 b /\ disjoint a b /\ modifies_1 a h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
...
val no_upd_lemma_2: 
  ...
  a': buffer t' ->
  b: buffer t'' ->
  Lemma (requires (live h0 b /\ disjoint a b /\ disjoint a' b /\ modifies_2 a a' h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
    [SMTPat (live h0 b); SMTPat (modifies_2 a a' h0 h1)]
let no_upd_lemma_2 #t #t' #t'' h0 h1 a a' b = ()

val no_upd_lemma_2_1: #t: Type -> #t': Type -> h0: mem -> h1: mem -> a: buffer t -> b: buffer t' ->
  Lemma (requires (live h0 b /\ disjoint a b /\ modifies_2_1 a h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
    [SMTPat (live h0 b); SMTPat (modifies_2_1 a h0 h1)]
let no_upd_lemma_2_1 #t #t' h0 h1 a b = ()

val no_upd_fresh: #t: Type -> h0: mem -> h1: mem -> a: buffer t ->
  Lemma (requires (live h0 a /\ fresh_frame h0 h1))
    (ensures (live h0 a /\ live h1 a /\ equal h0 a h1 a))
    [SMTPat (live h0 a); SMTPat (fresh_frame h0 h1)]
let no_upd_fresh #t h0 h1 a = ()

val no_upd_popped: #t: Type -> h0: mem -> h1: mem -> b: buffer t ->
  Lemma (requires (live h0 b /\ frameOf b =!= h0.tip /\ popped h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
    [SMTPat (live h0 b); SMTPat (popped h0 h1)]
let no_upd_popped #t h0 h1 b = ()

(* Modifies of subset lemmas *)
let lemma_modifies_sub_0 h0 h1
  : Lemma (requires (h1 == h0)) (ensures (modifies_0 h0 h1)) [SMTPat (modifies_0 h0 h1)] = ()

let lemma_modifies_sub_1 #t h0 h1 (b: buffer t)
  : Lemma (requires (h1 == h0))
    (ensures (modifies_1 b h0 h1))
    [SMTPat (live h0 b); SMTPat (modifies_1 b h0 h1)] = ()

let lemma_modifies_sub_2 #t #t' h0 h1 (b: buffer t) (b': buffer t')
  : Lemma (requires (h1 == h0))
    (ensures (modifies_2 b b' h0 h1))
    [SMTPat (live h0 b); SMTPat (live h0 b'); SMTPat (modifies_2 b b' h0 h1)] = ()

let lemma_modifies_sub_2_1 #t h0 h1 (b: buffer t)
  : Lemma (requires (modifies_0 h0 h1 /\ live h0 b))
    (ensures (modifies_2_1 b h0 h1))
    [SMTPat (live h0 b); SMTPat (modifies_2_1 b h0 h1)] = ()

#reset-options "--z3rlimit 100 --initial_fuel 0 --max_fuel 0"

let modifies_subbuffer_1 (#t: Type) h0 h1 (sub: buffer t) (a: buffer t)
  : Lemma (requires (live h0 a /\ modifies_1 sub h0 h1 /\ includes a sub))
    (ensures (modifies_1 a h0 h1 /\ live h1 a))
    [SMTPat (modifies_1 sub h0 h1); SMTPat (includes a sub)] = ()

let modifies_subbuffer_2 (#t: Type) (#t': Type) h0 h1 (sub: buffer t) (a': buffer t') (a: buffer t)
  : Lemma (requires (live h0 a /\ live h0 a' /\ includes a sub /\ modifies_2 sub a' h0 h1))
    (ensures (modifies_2 a a' h0 h1 /\ modifies_2 a' a h0 h1 /\ live h1 a))
    [SMTPat (modifies_2 sub a' h0 h1); SMTPat (includes a sub)] = ()

let modifies_subbuffer_2' (#t: Type) (#t': Type) h0 h1 (sub: buffer t) (a': buffer t') (a: buffer t)
  : Lemma (requires (live h0 a /\ live h0 a' /\ includes a sub /\ modifies_2 a' sub h0 h1))
    (ensures (modifies_2 a a' h0 h1 /\ live h1 a))
    [SMTPat (modifies_2 a' sub h0 h1); SMTPat (includes a sub)] = ()

let modifies_subbuffer_2_1 (#t: Type) h0 h1 (sub: buffer t) (a: buffer t)
  : Lemma (requires (live h0 a /\ includes a sub /\ modifies_2_1 sub h0 h1))
    (ensures (modifies_2_1 a h0 h1 /\ live h1 a))
    [SMTPat (modifies_2_1 sub h0 h1); SMTPat (includes a sub)] = ()

let modifies_subbuffer_2_prime (#t: Type) h0 h1 (sub1: buffer t) (sub2: buffer t) (a: buffer t)
  : Lemma (requires (live h0 a /\ includes a sub1 /\ includes a sub2 /\ modifies_2 sub1 sub2 h0 h1))
    (ensures (modifies_1 a h0 h1 /\ live h1 a))
    [SMTPat (modifies_2 sub1 sub2 h0 h1); SMTPat (includes a sub1); SMTPat (includes a sub2)] = ()

let modifies_popped_3_2 (#t: Type) #t' (a: buffer t) (b: buffer t') h0 h1 h2 h3
  : Lemma
    (requires
      (live h0 a /\ live h0 b /\ fresh_frame h0 h1 /\ popped h2 h3 /\ modifies_3_2 a b h1 h2))
    (ensures (modifies_2 a b h0 h3))
    [SMTPat (fresh_frame h0 h1); SMTPat (popped h2 h3); SMTPat (modifies_3_2 a b h1 h2)] = ()

let modifies_popped_2 (#t: Type) #t' (a: buffer t) (b: buffer t') h0 h1 h2 h3
  : Lemma
...
let modifies_popped_1 (#t: Type) (a: buffer t) h0 h1 h2 h3
  : Lemma (requires (live h0 a /\ fresh_frame h0 h1 /\ popped h2 h3 /\ modifies_2_1 a h1 h2))
    (ensures (modifies_1 a h0 h3))
    [SMTPat (fresh_frame h0 h1); SMTPat (popped h2 h3); SMTPat (modifies_2_1 a h1 h2)] = ()

let modifies_popped_1' (#t: Type) (a: buffer t) h0 h1 h2 h3
  : Lemma (requires (live h0 a /\ fresh_frame h0 h1 /\ popped h2 h3 /\ modifies_1 a h1 h2))
    (ensures (modifies_1 a h0 h3))
    [SMTPat (fresh_frame h0 h1); SMTPat (popped h2 h3); SMTPat (modifies_1 a h1 h2)] = ()

let modifies_popped_0 h0 h1 h2 h3
  : Lemma (requires (fresh_frame h0 h1 /\ popped h2 h3 /\ modifies_0 h1 h2))
    (ensures (modifies_0 h0 h3))
    [SMTPat (fresh_frame h0 h1); SMTPat (popped h2 h3); SMTPat (modifies_0 h1 h2)] = ()

let live_popped (#t: Type) (b: buffer t) h0 h1
  : Lemma (requires (popped h0 h1 /\ live h0 b /\ frameOf b =!= h0.tip))
    (ensures (live h1 b))
    [SMTPat (popped h0 h1); SMTPat (live h0 b)] = ()

let live_fresh (#t: Type) (b: buffer t) h0 h1
  : Lemma (requires (fresh_frame h0 h1 /\ live h0 b))
    (ensures (live h1 b))
    [SMTPat (fresh_frame h0 h1); SMTPat (live h0 b)] = ()

let modifies_0_to_2_1_lemma (#t: Type) h0 h1 (b: buffer t)
  : Lemma (requires (modifies_0 h0 h1 /\ live h0 b))
    (ensures (modifies_2_1 b h0 h1))
    [SMTPat (modifies_2_1 b h0 h1); SMTPat (live h0 b)] = ()

let lemma_modifies_none_push_pop h0 h1 h2
  : Lemma (requires (fresh_frame h0 h1 /\ popped h1 h2)) (ensures (modifies_none h0 h2)) = ()

let lemma_modifies_0_push_pop h0 h1 h2 h3
  : Lemma (requires (fresh_frame h0 h1 /\ modifies_0 h1 h2 /\ popped h2 h3))
    (ensures (modifies_none h0 h3)) = ()

let modifies_1_to_2_1_lemma (#t: Type) h0 h1 (b: buffer t)
  : Lemma (requires (modifies_1 b h0 h1 /\ live h0 b))
    (ensures (modifies_2_1 b h0 h1))
    [SMTPat (modifies_2_1 b h0 h1); SMTPat (live h0 b)] = ()
...
let modifies_poppable_0 h0 h1
  : Lemma (requires (modifies_0 h0 h1 /\ HS.poppable h0))
    (ensures (HS.poppable h1))
    [SMTPat (modifies_0 h0 h1)] = ()

let modifies_poppable_1 #t h0 h1 (b: buffer t)
  : Lemma (requires (modifies_1 b h0 h1 /\ HS.poppable h0))
    (ensures (HS.poppable h1))
    [SMTPat (modifies_1 b h0 h1)] = ()

let modifies_poppable_2_1 #t h0 h1 (b: buffer t)
  : Lemma (requires (modifies_2_1 b h0 h1 /\ HS.poppable h0))
    (ensures (HS.poppable h1))
    [SMTPat (modifies_2_1 b h0 h1)] = ()

let modifies_poppable_2 #t #t' h0 h1 (b: buffer t) (b': buffer t')
  : Lemma (requires (modifies_2 b b' h0 h1 /\ HS.poppable h0))
    (ensures (HS.poppable h1))
    [SMTPat (modifies_2 b' b h0 h1)] = ()

let modifies_poppable_3_2 #t #t' h0 h1 (b: buffer t) (b': buffer t')
  : Lemma (requires (modifies_3_2 b b' h0 h1 /\ HS.poppable h0))
    (ensures (HS.poppable h1))
    [SMTPat (modifies_3_2 b' b h0 h1)] = ()

let lemma_fresh_poppable h0 h1
  : Lemma (requires (fresh_frame h0 h1)) (ensures (poppable h1)) [SMTPat (fresh_frame h0 h1)] = ()

let lemma_equal_domains_popped h0 h1 h2 h3
  : Lemma (requires (equal_domains h0 h1 /\ popped h0 h2 /\ popped h1 h3))
    (ensures (equal_domains h2 h3)) = ()

let lemma_equal_domains h0 h1 h2 h3
  : Lemma (requires (fresh_frame h0 h1 /\ equal_domains h1 h2 /\ popped h2 h3))
    (ensures (equal_domains h0 h3))
    [SMTPat (fresh_frame h0 h1); SMTPat (equal_domains h1 h2); SMTPat (popped h2 h3)] = ()

let lemma_equal_domains_2 h0 h1 h2 h3 h4
  : Lemma
    (requires
      (fresh_frame h0 h1 /\ modifies_0 h1 h2 /\ Map.domain h1.h == Map.domain h2.h /\
