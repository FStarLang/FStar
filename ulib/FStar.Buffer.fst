...
let modifies_bufs_and_refs (bufs: TSet.set abuffer) (refs: Set.set nat) h h' : GTot Type0 =
  (forall rid.
      Set.mem rid (Map.domain (HS.get_hmap h)) ==>
...
val lemma_modifies_bufs_superset: 
  ...
  Lemma (requires (unused_in b' h0 /\ live h0 b /\ disjoint_from_bufs b bufs))
    ...
    [
      SMTPat (modifies_bufs (HS.get_tip h0) bufs h0 h1);
      SMTPat (unmapped_in b' h0);
      SMTPat (live h0 b)
    ]
  ...
  (let rid = frameOf b in
    ...
    ((rid == rid' /\ rid' == HS.get_tip h0 /\ modifies_buf_2 rid b b' h0 h1 /\
      ...
      (rid == rid' /\ rid' =!= HS.get_tip h0 /\ modifies_buf_2 rid b b' h0 h1 /\
        ...
        HS.modifies (Set.union (Set.singleton rid) (Set.singleton (HS.get_tip h0))) h0 h1) \/
      (rid =!= rid' /\ rid == HS.get_tip h0 /\ modifies_buf_1 rid b h0 h1 /\
        modifies_buf_1 rid' b' h0 h1 /\
        HS.modifies (Set.union (Set.singleton rid') (Set.singleton (HS.get_tip h0))) h0 h1) \/
      (rid =!= rid' /\ rid' == HS.get_tip h0 /\ modifies_buf_1 rid' b' h0 h1 /\
        modifies_buf_1 rid b h0 h1 /\
...
let lemma_intro_modifies_0 h0 h1
  : Lemma
    (requires
      (modifies_one (HS.get_tip h0) h0 h1 /\ modifies_buf_0 (HS.get_tip h0) h0 h1 /\
        HS.get_tip h0 == HS.get_tip h1)) (ensures (modifies_0 h0 h1)) = ()
          ...
          ((rid == rid' /\ rid' == HS.get_tip h0 /\ modifies_buf_2 rid b b' h0 h1 /\
            ...
            (rid =!= rid' /\ rid' =!= HS.get_tip h0 /\ rid =!= HS.get_tip h0 /\
              HS.modifies (Set.union (Set.union (Set.singleton rid) (Set.singleton rid'))
                ...
                h0
                h1 /\ modifies_buf_1 rid b h0 h1 /\ modifies_buf_1 rid' b' h0 h1 /\
              modifies_buf_0 (HS.get_tip h0) h0 h1))))) (ensures (modifies_3_2 b b' h0 h1)) = ()
...
let lemma_modifies_0_unalloc (#a: Type) (b: buffer a) h0 h1 h2
  : Lemma
    (requires
      (unused_in b h0 /\ frameOf b == HS.get_tip h0 /\ modifies_0 h0 h1 /\ modifies_1 b h1 h2))
    (ensures (modifies_0 h0 h2)) = ()
  ...
  StackInline (buffer a)
    ...
    (ensures
      (fun (h0: mem) b h1 ->
          ...
          frameOf b == HS.get_tip h0 /\ Map.domain (HS.get_hmap h1) == Map.domain (HS.get_hmap h0) /\
          modifies_0 h0 h1 /\ as_seq h1 b == Seq.create (v len) init))
  ...
  StackInline (buffer a)
    ...
    (ensures
      (fun (h0: mem) b h1 ->
          ...
          frameOf b == (HS.get_tip h0) /\ Map.domain (HS.get_hmap h1) == Map.domain (HS.get_hmap h0) /\
          modifies_0 h0 h1 /\ as_seq h1 b == Seq.seq_of_list init /\ q #a len b))
...
val no_upd_popped: #t: Type -> h0: mem -> h1: mem -> b: buffer t ->
  Lemma (requires (live h0 b /\ frameOf b =!= HS.get_tip h0 /\ popped h0 h1))
    (ensures (live h0 b /\ live h1 b /\ equal h0 b h1 b))
...
let live_popped (#t: Type) (b: buffer t) h0 h1
  : Lemma (requires (popped h0 h1 /\ live h0 b /\ frameOf b =!= HS.get_tip h0))
    (ensures (live h1 b))
...
let lemma_equal_domains_2 (h0: mem) (h1: mem) (h2: mem) (h3: mem) (h4: mem)
  : Lemma
    (requires
      (fresh_frame h0 h1 /\ modifies_0 h1 h2 /\
        Map.domain (HS.get_hmap h1) == Map.domain (HS.get_hmap h2) /\ equal_domains h2 h3 /\
        popped h3 h4))
