...
assume
val fail: #a: Type -> m: string ->
  TacH a (requires (fun _ -> True)) (ensures (fun ps r -> Failed? r /\ Failed?.ps r == ps))
...
assume
val catch: #a: Type -> (unit -> Tac a) -> TacS (either string a)
...
assume
val t_pointwise: direction -> (unit -> Tac unit) -> Tac unit
