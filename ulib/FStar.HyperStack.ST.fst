...
[@ "opaque_to_smt"]
private unfold
let contains_region (m: mem) (r: rid) = Map.contains (get_hmap m) r

(* Eternal regions remain contained *)
private abstract
let eternal_region_pred (m1: mem) (m2: mem) : Type0 =
  forall (r: HS.rid). {:pattern (HS.is_eternal_color (color r)); (contains_region m1 r)}
    (HS.is_eternal_color (color r) /\ contains_region m1 r) ==> contains_region m2 r

(* rid counter increases monotonically *)
private abstract
let rid_ctr_pred (m1: mem) (m2: mem) : Type0 = get_rid_ctr m1 <= get_rid_ctr m2
 ...
 *)
private abstract
let rid_last_component_pred (m1: mem) (m2: mem) : Type0 =
  forall (r: HS.rid). {:pattern (contains_region m1 r)}
    ((~(contains_region m1 r)) /\ rid_last_component r < get_rid_ctr m1) ==>
    (~(contains_region m2 r))

(* Predicate for refs *)
private abstract
let eternal_refs_pred (m1: mem) (m2: mem) : Type0 =
  forall (a: Type) (rel: preorder a) (r: HS.mreference a rel). {:pattern (HS.contains m1 r)}
    if is_mm r
    then True
    else
      if HS.contains m1 r
      then
        if HS.is_eternal_color (color (HS.frameOf r))
        then HS.contains m2 r /\ rel (HS.sel m1 r) (HS.sel m2 r)
        else
          if contains_region m2 (HS.frameOf r)
          then HS.contains m2 r /\ rel (HS.sel m1 r) (HS.sel m2 r)
          else True
      else True
...
let mem_rel: relation mem =
  fun (m1: mem) (m2: mem) ->
    eternal_region_pred m1 m2 /\ rid_ctr_pred m1 m2 /\ rid_last_component_pred m1 m2 /\
    eternal_refs_pred m1 m2
private abstract
let mem_pre: preorder mem = HS.lemma_rid_ctr_pred (); mem_rel

type mem_predicate = mem -> Type0

(* Predicates that we will witness with regions and refs *)
abstract
let region_contains_pred (r: HS.rid) : mem_predicate =
  fun m -> (not (HS.is_eternal_color (color r))) \/ contains_region m r
abstract
let ref_contains_pred (#a: Type) (#rel: preorder a) (r: HS.mreference a rel) : mem_predicate =
  fun m ->
    rid_last_component (HS.frameOf r) < get_rid_ctr m /\
    (HS.is_mm r \/ (not (contains_region m (HS.frameOf r)) \/ HS.contains m r))
 ...
 *)
abstract
let stable (p: mem_predicate) : Type0 =
  forall (h1: mem) (h2: mem). {:pattern (mem_rel h1 h2)} (p h1 /\ mem_rel h1 h2) ==> p h2
abstract
let witnessed (p: mem_predicate) : Type0 = W.witnessed mem_rel p

(* TODO: we should derive these using DM4F *)
assume private
val gst_get: unit -> GST mem (fun p h0 -> p h0 h0)
assume private
val gst_put: h1: mem -> GST unit (fun p h0 -> mem_rel h0 h1 /\ p () h1)
assume private
val gst_witness: p: mem_predicate ->
  GST unit (fun post h0 -> p h0 /\ stable p /\ (witnessed p ==> post () h0))
assume private
val gst_recall: p: mem_predicate -> GST unit (fun post h0 -> witnessed p /\ (p h0 ==> post () h0))

val lemma_functoriality: 
  p: mem_predicate{witnessed p} ->
...
effect Unsafe (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) -> pre h /\ (forall a h1. pre h /\ post h a h1 ==> p a h1))
...
[@ "opaque_to_smt"]
unfold private
let equal_heap_dom (r: rid) (m0: mem) (m1: mem) : Type0 =
  Heap.equal_dom (Map.sel (get_hmap m0) r) (Map.sel (get_hmap m1) r)

[@ "opaque_to_smt"]
unfold private
let contained_region: mem -> mem -> rid -> Type0 =
  fun m0 m1 r -> contains_region m0 r /\ contains_region m1 r

[@ "opaque_to_smt"]
unfold private
let contained_stack_region: mem -> mem -> rid -> Type0 =
...
[@ "opaque_to_smt"]
unfold private
let contained_non_tip_stack_region: mem -> mem -> rid -> Type0 =
  fun m0 m1 r -> is_stack_region r /\ contained_non_tip_region m0 m1 r

[@ "opaque_to_smt"]
unfold private
let same_refs_common (p: (mem -> mem -> rid -> Type0)) (m0: mem) (m1: mem) =
  forall (r: rid). p m0 m1 r ==> equal_heap_dom r m0 m1

(* predicates *)
abstract
let same_refs_in_all_regions (m0: mem) (m1: mem) : Type0 = same_refs_common contained_region m0 m1
abstract
let same_refs_in_stack_regions (m0: mem) (m1: mem) : Type0 =
  same_refs_common contained_stack_region m0 m1
abstract
let same_refs_in_non_tip_regions (m0: mem) (m1: mem) : Type0 =
  same_refs_common contained_non_tip_region m0 m1
abstract
let same_refs_in_non_tip_stack_regions (m0: mem) (m1: mem) : Type0 =
  same_refs_common contained_non_tip_stack_region m0 m1

(* intro and elim forms *)
let lemma_same_refs_in_all_regions_intro (m0: mem) (m1: mem)
  : Lemma (requires (same_refs_common contained_region m0 m1))
    (ensures (same_refs_in_all_regions m0 m1))
    [SMTPat (same_refs_in_all_regions m0 m1)] = ()
let lemma_same_refs_in_all_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma (requires (same_refs_in_all_regions m0 m1 /\ contained_region m0 m1 r))
    (ensures (equal_heap_dom r m0 m1))
    [
      SMTPatOr
      [
        [SMTPat (same_refs_in_all_regions m0 m1); SMTPat (contains_region m0 r)];
        [SMTPat (same_refs_in_all_regions m0 m1); SMTPat (contains_region m1 r)]
      ]
    ] = ()
let lemma_same_refs_in_stack_regions_intro (m0: mem) (m1: mem)
  : Lemma (requires (same_refs_common contained_stack_region m0 m1))
    (ensures (same_refs_in_stack_regions m0 m1))
    [SMTPat (same_refs_in_stack_regions m0 m1)] = ()
let lemma_same_refs_in_stack_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma (requires (same_refs_in_stack_regions m0 m1 /\ contained_stack_region m0 m1 r))
    (ensures (equal_heap_dom r m0 m1))
    [
      SMTPatOr
      [
        [
          SMTPat (same_refs_in_stack_regions m0 m1);
          SMTPat (is_stack_region r);
          SMTPat (contains_region m0 r)
        ];
        [
          SMTPat (same_refs_in_stack_regions m0 m1);
          SMTPat (is_stack_region r);
          SMTPat (contains_region m1 r)
        ]
      ]
    ] = ()

let lemma_same_refs_in_non_tip_regions_intro (m0: mem) (m1: mem)
  : Lemma (requires (same_refs_common contained_non_tip_region m0 m1))
    (ensures (same_refs_in_non_tip_regions m0 m1))
    [SMTPat (same_refs_in_non_tip_regions m0 m1)] = ()
let lemma_same_refs_in_non_tip_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma (requires (same_refs_in_non_tip_regions m0 m1 /\ contained_non_tip_region m0 m1 r))
    (ensures (equal_heap_dom r m0 m1))
    [
      SMTPatOr
      [
        [SMTPat (same_refs_in_non_tip_regions m0 m1); SMTPat (contains_region m0 r)];
        [SMTPat (same_refs_in_non_tip_regions m0 m1); SMTPat (contains_region m1 r)]
      ]
    ] = ()

let lemma_same_refs_in_non_tip_stack_regions_intro (m0: mem) (m1: mem)
  : Lemma (requires (same_refs_common contained_non_tip_stack_region m0 m1))
    (ensures (same_refs_in_non_tip_stack_regions m0 m1))
    [SMTPat (same_refs_in_non_tip_stack_regions m0 m1)] = ()
let lemma_same_refs_in_non_tip_stack_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma
    (requires (same_refs_in_non_tip_stack_regions m0 m1 /\ contained_non_tip_stack_region m0 m1 r))
    (ensures (equal_heap_dom r m0 m1))
    [
      SMTPatOr
      [
...
let lemma_equal_domains_trans (m0: mem) (m1: mem) (m2: mem)
...
effect Stack (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\ (forall a h1. (pre h /\ post h a h1 /\ equal_domains h h1) ==> p a h1))
  ...
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\
        (forall a h1.
            (pre h /\ post h a h1 /\ get_tip h = HS.root /\ get_tip h1 = HS.root) ==> p a h1))
...
effect ST (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\ (forall a h1. (pre h /\ post h a h1 /\ equal_stack_domains h h1) ==> p a h1))
...
effect StackInline (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\ is_stack_region (get_tip h) /\
...
effect Inline (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\ (forall a h1. (pre h /\ post h a h1 /\ inline_inv h h1) ==> p a h1))
...
sub_effect DIV ~> STATE {
  lift_wp = fun (a: Type) (wp: pure_wp a) (p: st_post a) (h: mem) -> wp (fun a -> p a h)
}
...
type mreference (a: Type) (rel: preorder a) =
  r:
  HS.mreference a rel
    {witnessed (ref_contains_pred r) /\ witnessed (region_contains_pred (HS.frameOf r))}
...
type mref (a: Type) (rel: preorder a) =
  r:
  HS.mref a rel {witnessed (ref_contains_pred r) /\ witnessed (region_contains_pred (HS.frameOf r))}
type mmmstackref (a: Type) (rel: preorder a) =
  r:
  HS.mmmstackref a rel
    {witnessed (ref_contains_pred r) /\ witnessed (region_contains_pred (HS.frameOf r))}
...
let salloc_post
  (#a: Type) (#rel: preorder a) (init: a) (m0: mem)
  ...
  get_tip m0 == get_tip m1 /\ frameOf s = get_tip m1 /\
  //it's a fresh reference in the top frame
  HS.fresh_ref s m0 m1 /\
  //and it's been initialized
  m1 == HyperStack.upd m0 s init
private
let salloc_common (#a: Type) (#rel: preorder a) (init: a) (mm: bool)
  : StackInline (mreference a rel)
    (requires (fun m -> is_stack_region (get_tip m)))
    (ensures
      (fun m0 s m1 -> is_stack_region (HS.frameOf s) /\ salloc_post init m0 s m1 /\ is_mm s == mm)) =
  let m0 = gst_get () in
  let r, m1 = HS.alloc rel (get_tip m0) init mm m0 in
  gst_put m1;
  assert (Set.equal (Map.domain (get_hmap m0)) (Map.domain (get_hmap m1)));
  //AR: to prove that rid_last_component of r.id is < rid_ctr
...
let salloc (#a: Type) (#rel: preorder a) (init: a)
  : StackInline (mstackref a rel)
    (requires (fun m -> is_stack_region (get_tip m)))
    (ensures salloc_post init) = salloc_common init false
...
let salloc_mm (#a: Type) (#rel: preorder a) (init: a)
  : StackInline (mmmstackref a rel)
    (requires (fun m -> is_stack_region (get_tip m)))
    (ensures salloc_post init) = salloc_common init true

[@ (deprecated "use salloc instead")]
let sfree (#a: Type) (#rel: preorder a) (r: mmmstackref a rel)
  : StackInline unit
    (requires (fun m0 -> frameOf r = get_tip m0 /\ contains m0 r))
    (ensures (fun m0 _ m1 -> contains m0 r /\ m1 == HS.free r m0)) =
  let m0 = gst_get () in
...
let new_region (r0: rid)
  : ST rid
    ...
    (ensures
      (fun m0 r1 m1 ->
          HS.extends r1 r0 /\ HS.fresh_region r1 m0 m1 /\ HS.color r1 = HS.color r0 /\
          is_eternal_region r1 /\ get_hmap m1 == Map.upd (get_hmap m0) r1 Heap.emp /\
          get_tip m1 == get_tip m0 /\ HS.live_region m0 r0 /\
          (r1, m1) == HS.new_eternal_region m0 r0 None)) =
  //recall containment of r0
...
let new_colored_region (r0: rid) (c: int)
  : ST rid
    ...
    (ensures
      (fun m0 r1 m1 ->
          HS.extends r1 r0 /\ HS.fresh_region r1 m0 m1 /\ HS.color r1 = c /\ is_eternal_region r1 /\
          HS.live_region m0 r0 /\ (r1, m1) == HS.new_eternal_region m0 r0 (Some c) /\
          get_hmap m1 == Map.upd (get_hmap m0) r1 Heap.emp /\ get_tip m1 == get_tip m0)) =
  //recall containment of r0
  if r0 <> HS.root then gst_recall (region_contains_pred r0);
  HS.lemma_rid_ctr_pred ();
  let m0 = gst_get () in
...
let ralloc_post
  (#a: Type) (#rel: preorder a) (i: rid) (init: a) (m0: mem)
  (x: mreference a rel {is_eternal_region (frameOf x)}) (m1: mem) =
  let region_i = Map.sel (get_hmap m0) i in
  Heap.unused_in (as_ref x) region_i /\ is_in i (get_hmap m0) /\ i = frameOf x /\
  m1 == upd m0 x init
private
let ralloc_common (#a: Type) (#rel: preorder a) (i: rid) (init: a) (mm: bool)
  : ST (mreference a rel)
    (requires (fun m -> is_eternal_region i))
    (ensures
      (fun m0 r m1 -> is_eternal_region (frameOf r) /\ ralloc_post i init m0 r m1 /\ is_mm r == mm)) =
...
let ralloc (#a: Type) (#rel: preorder a) (i: rid) (init: a)
  : ST (mref a rel) (requires (fun m -> is_eternal_region i)) (ensures (ralloc_post i init)) =
  ralloc_common i init false

let ralloc_mm (#a: Type) (#rel: preorder a) (i: rid) (init: a)
  : ST (mmmref a rel) (requires (fun m -> is_eternal_region i)) (ensures (ralloc_post i init)) =
...
let is_live_for_rw_in (#a: Type) (#rel: preorder a) (r: mreference a rel) (m: mem) : Type0 =
  ...
  (let i = HS.frameOf r in
    (is_eternal_region i \/ HS.is_above i (get_tip m)) /\
    (not (is_mm r) \/ HS.contains_ref_in_its_region m r))

let rfree (#a: Type) (#rel: preorder a) (r: mmmref a rel)
  : ST unit
    (requires (fun m0 -> is_live_for_rw_in r m0))
    (ensures (fun m0 _ m1 -> contains m0 r /\ m1 == HS.free r m0)) =
  let m0 = gst_get () in
  ...
  gst_put m1
unfold
let assign_post (#a: Type) (#rel: preorder a) (r: mreference a rel) (v: a) m0 (_u: unit) m1 =
  contains m0 r /\ m1 == HyperStack.upd m0 r v
...
let op_Colon_Equals (#a: Type) (#rel: preorder a) (r: mreference a rel) (v: a)
  : STL unit
    (requires (fun m -> is_live_for_rw_in r m /\ rel (HS.sel m r) v))
  ...
  Heap.lemma_distinct_addrs_distinct_preorders ();
  Heap.lemma_distinct_addrs_distinct_mm ();
  Heap.lemma_upd_equals_upd_tot_for_contained_refs (Map.sel (get_hmap m0) (HS.frameOf r))
    (HS.as_ref r)
    v;
  gst_put m1
unfold
let deref_post (#a: Type) (#rel: preorder a) (r: mreference a rel) m0 x m1 =
  m1 == m0 /\ contains m0 r /\ x == HyperStack.sel m0 r
...
let op_Bang (#a: Type) (#rel: preorder a) (r: mreference a rel)
  : Stack a (requires (fun m -> is_live_for_rw_in r m)) (ensures (deref_post r)) =
  let m0 = gst_get () in
  gst_recall (region_contains_pred (HS.frameOf r));
  gst_recall (ref_contains_pred r);
  Heap.lemma_sel_equals_sel_tot_for_contained_refs (Map.sel (get_hmap m0) (HS.frameOf r))
    (HS.as_ref r);
  HS.sel_tot m0 r
 ...
 *)
let get (_: unit)
  : Stack mem (requires (fun m -> True)) (ensures (fun m0 x m1 -> m0 == x /\ m1 == m0)) = gst_get ()

(**
 * We can only recall refs with mm bit unset, not stack refs
 *)
let recall (#a: Type) (#rel: preorder a) (r: mref a rel)
  : Stack unit (requires (fun m -> True)) (ensures (fun m0 _ m1 -> m0 == m1 /\ contains m1 r)) =
  gst_recall (ref_contains_pred r);
  gst_recall (region_contains_pred (HS.frameOf r))
...
unfold
type stable_on_t (#i: erid) (#a: Type) (#b: preorder a) (r: m_rref i a b) (p: mem_predicate) =
  forall h0 h1. {:pattern (p h0); b (HS.sel h0 r) (HS.sel h1 r)}
    (p h0 /\ b (HS.sel h0 r) (HS.sel h1 r)) ==> p h1

let mr_witness (#r: erid) (#a: Type) (#b: preorder a) (m: m_rref r a b) (p: mem_predicate)
  : ST unit
    (requires (fun h0 -> p h0 /\ stable_on_t m p))
    (ensures (fun h0 _ h1 -> h0 == h1 /\ witnessed p)) =
  recall m;
  let p_pred (#i: erid) (#a: Type) (#b: preorder a) (r: m_rref i a b) (p: mem_predicate)
    : mem_predicate =
    fun m -> contains m r /\ p m
  in
  gst_witness (p_pred m p);
  lemma_functoriality (p_pred m p) p

let weaken_witness (p: mem_predicate) (q: mem_predicate)
...
let testify_forall
  (#c: Type) (#p: (c -> mem -> Type0)) ($s: squash (forall (x: c). witnessed (p x)))
  : ST unit
    (requires (fun h -> True))
...
let testify_forall_region_contains_pred
  (#c: Type) (#p: (c -> rid)) ($s: squash (forall (x: c). witnessed (region_contains_pred (p x))))
  : ST unit
    (requires (fun _ -> True))
    (ensures
      (fun h0 _ h1 ->
          h0 == h1 /\
          (forall (x: c). (not (HS.is_eternal_color (color (p x)))) \/ contains_region h1 (p x)))) =
...
let lemma_witnessed_constant (p: Type0) : Lemma (witnessed (fun (m: mem) -> p) <==> p) =
...
let lemma_witnessed_nested (p: mem_predicate)
  ...
  assert (FStar.FunctionalExtensionality.feq (fun (m: mem) -> witnessed p)
        (fun (m: mem) -> W.witnessed mem_rel p))

let lemma_witnessed_and (p: mem_predicate) (q: mem_predicate)
  : Lemma (witnessed (fun s -> p s /\ q s) <==> (witnessed p /\ witnessed q)) =
  W.lemma_witnessed_and mem_rel p q

let lemma_witnessed_or (p: mem_predicate) (q: mem_predicate)
  : Lemma ((witnessed p \/ witnessed q) ==> witnessed (fun s -> p s \/ q s)) =
  W.lemma_witnessed_or mem_rel p q

let lemma_witnessed_impl (p: mem_predicate) (q: mem_predicate)
  : Lemma ((witnessed (fun s -> p s ==> q s) /\ witnessed p) ==> witnessed q) =
