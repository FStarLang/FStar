...
let eternal_region_pred (m1: mem) (m2: mem) : Type0 =
  forall (r: HS.rid). {:pattern (HS.is_eternal_color (color r)); (Map.contains m1.h r)}
    (HS.is_eternal_color (color r) /\ Map.contains m1.h r) ==> Map.contains m2.h r

(* rid counter increases monotonically *)
private abstract
let rid_ctr_pred (m1: mem) (m2: mem) : Type0 = m1.rid_ctr <= m2.rid_ctr
...
let rid_last_component_pred (m1: mem) (m2: mem) : Type0 =
  forall (r: HS.rid). {:pattern (Map.contains m1.h r)}
    ((~(Map.contains m1.h r)) /\ rid_last_component r < m1.rid_ctr) ==> (~(Map.contains m2.h r))
    ...
    then True
      ...
      then
        ...
        then HS.contains m2 r /\ rel (HS.sel m1 r) (HS.sel m2 r)
          ...
          if Map.contains m2.h (HS.frameOf r)
          then HS.contains m2 r /\ rel (HS.sel m1 r) (HS.sel m2 r)
          else True
      else True
...
let region_contains_pred (r: HS.rid) : mem_predicate =
  fun m -> (not (HS.is_eternal_color (color r))) \/ Map.contains m.h r

abstract
let ref_contains_pred (#a: Type) (#rel: preorder a) (r: HS.mreference a rel) : mem_predicate =
  fun m ->
    rid_last_component (HS.frameOf r) < m.rid_ctr /\
    (HS.is_mm r \/ (not (Map.contains m.h (HS.frameOf r)) \/ HS.contains m r))
...
let contained_region: mem -> mem -> rid -> Type0 =
  fun m0 m1 r -> Map.contains m0.h r /\ Map.contains m1.h r
...
let lemma_same_refs_in_all_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma (requires (same_refs_in_all_regions m0 m1 /\ contained_region m0 m1 r))
    ...
    [
      SMTPatOr
      [
        [SMTPat (same_refs_in_all_regions m0 m1); SMTPat (Map.contains m0.h r)];
        [SMTPat (same_refs_in_all_regions m0 m1); SMTPat (Map.contains m1.h r)]
      ]
    ] = ()
    ...
    [
      ...
      [
        [
          ...
          SMTPat (Map.contains m0.h r)
        ];
        [
          SMTPat (same_refs_in_stack_regions m0 m1);
          SMTPat (is_stack_region r);
          SMTPat (Map.contains m1.h r)
        ]
      ]
    ] = ()
...
let lemma_same_refs_in_non_tip_regions_elim (m0: mem) (m1: mem) (r: rid)
  : Lemma (requires (same_refs_in_non_tip_regions m0 m1 /\ contained_non_tip_region m0 m1 r))
    ...
    [
      SMTPatOr
      [
        [SMTPat (same_refs_in_non_tip_regions m0 m1); SMTPat (Map.contains m0.h r)];
        [SMTPat (same_refs_in_non_tip_regions m0 m1); SMTPat (Map.contains m1.h r)]
      ]
    ] = ()
...
let equal_domains (m0: mem) (m1: mem) =
  m0.tip == m1.tip /\ Set.equal (Map.domain m0.h) (Map.domain m1.h) /\
...
effect Heap (a: Type) (pre: st_pre) (post: (m0: mem -> Tot (st_post' a (pre m0)))) =
  STATE a
    ((* WP *)
      fun (p: st_post a) (h: mem) ->
        pre h /\
        (forall a h1. (pre h /\ post h a h1 /\ h.tip = HS.root /\ h1.tip = HS.root) ==> p a h1))

let equal_stack_domains (m0: mem) (m1: mem) = m0.tip == m1.tip /\ same_refs_in_stack_regions m0 m1
...
let salloc_post
  ...
  is_stack_region m0.tip /\ Map.domain m0.h == Map.domain m1.h /\ m0.tip = m1.tip /\
  frameOf s = m1.tip /\
...
let sfree (#a: Type) (#rel: preorder a) (r: mmmstackref a rel)
  : StackInline unit
    (requires (fun m0 -> frameOf r = m0.tip /\ contains m0 r))
  ...
  : ST rid
    ...
    (ensures
      (fun m0 r1 m1 ->
          ...
          is_eternal_region r1 /\ m1.h == Map.upd m0.h r1 Heap.emp /\ m1.tip = m0.tip)) =
  ...
  : ST rid
    ...
    (ensures
      (fun m0 r1 m1 ->
          ...
          m1.h == Map.upd m0.h r1 Heap.emp /\ m1.tip = m0.tip)) =
  //recall containment of r0
  if r0 <> HS.root then gst_recall (region_contains_pred r0);
  HS.lemma_rid_ctr_pred ();
  let m0 = gst_get () in
...
let ralloc_post
  ...
  Heap.unused_in (as_ref x) region_i /\ is_in i m0.h /\ i = frameOf x /\ m1 == upd m0 x init
...
let is_live_for_rw_in (#a: Type) (#rel: preorder a) (r: mreference a rel) (m: mem) : Type0 =
  ...
  (let i = HS.frameOf r in
    (is_eternal_region i \/ HS.is_above i m.tip) /\
...
let recall_region (i: rid{is_eternal_region i})
  : Stack unit (requires (fun m -> True)) (ensures (fun m0 _ m1 -> m0 == m1 /\ is_in i m1.h)) =
  if i <> HS.root then gst_recall (region_contains_pred i)

let witness_region (i: rid)
  : Stack unit
    (requires (fun m0 -> is_eternal_color (color i) ==> is_in i m0.h))
...
let testify_forall_region_contains_pred
  ...
  : ST unit
    ...
    (ensures
      (fun h0 _ h1 ->
          h0 == h1 /\
          (forall (x: c). (not (HS.is_eternal_color (color (p x)))) \/ Map.contains h1.h (p x)))) =
