...
layered_effect {
  MSTATE : a:Type -> state:Type u#2 -> rel:P.preorder state -> req:pre_t state -> ens:post_t state a -> Effect
  with
  repr = repr;
  return = return;
  bind = bind;
...
let lift_pure_mst (a:Type) (state:Type u#2) (rel:P.preorder state) (wp:pure_wp a) (f:unit -> PURE a wp)
: repr a state rel
