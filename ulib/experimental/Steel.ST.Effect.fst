...
let bind_div_st_ (a:Type) (b:Type)
  ...
  = let c
      : Steel.Effect.repr b
                   ...
                   pre
                   post
                   (bind_div_steel_req wp (fun x _ -> req x))
                   (bind_div_steel_ens wp (fun x _ y _ -> ens x y))
      =(Steel.Effect.bind_div_steel a b
                  wp
                  framed
                  pre
                  post
...
polymonadic_bind (DIV, STBase) |> STBase = bind_div_st_
...
let reflect (a:Type)
            ...
            (#req:Type0)
            (#ens:a -> Type0)
            (f:repr a false pre post req ens)
