...
let array_struct_unitless_pcm' : unitless_pcm' u#(a+1) (array_struct u#a) = {
  unitless_composable = composable_array_struct u#a;
  unitless_op = compose_array_struct u#a;
}
...
let pointwise_array_pcm
  ...
  let pcm' : unitless_pcm' (array_type cell_type len) = {
    unitless_composable = composable;
    unitless_op = compose;
  } in
  {
    unitless_p = pcm';
    (* The proofs here should simply follow by generalization for all indexes *)
    unitless_comm = (fun x y -> admit());
    unitless_assoc = (fun _ _ _ -> admit());
    unitless_assoc_r = (fun _ _ _ -> admit());
  }
...
let pointwise_struct_pcm
  ...
  let compose
    ...
    (y: struct_type field_types{composable x y})
  ...
  {
    unitless_p = pcm';
    (* The proofs here should simply follow by generalization for all fields *)
    unitless_comm = (fun x y -> admit());
    unitless_assoc = (fun _ _ _ -> admit());
    unitless_assoc_r = (fun _ _ _ -> admit());
  }
...
let two_fields_restricted_func
  ...
  : (field_id ^-> a) =
  on _ (fun field ->
    if field = field1 then value1
    else if field = field2 then value2
    else by_default
  )
...
let two_fields_dep_map
  ...
  : (DepMap.t field_id a) =
  DepMap.create #field_id #a (fun field ->
    if field = field1 then begin admit(); value1 end (* Why can't F* find this ? *)
    else if field = field2 then begin admit(); value2 end
    else by_default field
  )


let point_2d : array_struct =
  let field_descriptors =
    two_fields_restricted_func "x" "y" (DBase int) (DBase int) (DBase unit)
  in
  ...
  let field_pcms : (field:field_id -> unitless_pcm (field_types field)) =
    fun field ->
    if field = "x" then
      exclusive_unitless_pcm
    else if field = "y" then
      immutable_unitless_pcm
    else
      exclusive_unitless_pcm
  in
