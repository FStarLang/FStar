...
let ens_depends_only_on (#a:Type) (pre:slprop) (post:a -> slprop)
  ...
  = //can join any disjoint heap to the pre-heap and q is still valid
    (forall x (h_pre:hmem pre) h_post (h:mem{disjoint h_pre h}).
      (//join_preserves_interp pre h_pre h; 
...
unfold
let return_req (p:slprop) : req_t p = fun _ -> True

unfold
let return_ens (#a:Type) (x:a) (p:a -> slprop) : ens_t (p x) a p = fun _ r _ -> r == x
...
let polymonadic_bind_steel_pure_pre (#a:Type) (#b:Type)
  (#pre_f:pre_t) (#post_f:Mem.slprop) (req_f:req_t pre_f) (ens_f:ens_t pre_f a (fun _ -> post_f))
  (wp_g:a -> pure_wp b)
: req_t pre_f
...
let polymonadic_bind_steel_pure_post
  (#a:Type) (#b:Type)
  (#pre_f:pre_t) (#post_f:Mem.slprop) (ens_f:ens_t pre_f a (fun _ -> post_f))
  (wp_g:a -> pure_wp b)
...
let intro_emp_left (p1 p2:slprop) (m:mem)
: Lemma
...
//   (#pre:slprop)
//   (#post:a -> slprop)
//   (act:Mem.action_except a FStar.Set.empty pre post)
...
let read (#a:Type) (#pcm:_) (r:ref a pcm) (v0:G.erased a)
  : SteelT (v:a { Steel.PCM.compatible pcm v0 v })
          (pts_to r v0)
          (fun _ -> pts_to r v0)
  = Steel?.reflect (sel_action FStar.Set.empty r v0)          
...
let write (#a:Type) (#pcm:_) (r:ref a pcm) 
  ...
  = Steel?.reflect (upd_action FStar.Set.empty r v0 v1)

let alloc (#a:Type) (#pcm:_) (x:a{Steel.PCM.compatible pcm x x })
  : SteelT (ref a pcm) emp (fun r -> pts_to r x)
  = Steel?.reflect (alloc_action FStar.Set.empty x)
