...
type cand p1 p2 = | Conj : h1: p1 -> h2: p2 -> cand p1 p2
...
noeq
type cexists (#a: Type) (p: (a -> Type)) = | ExIntro : x: a -> h: p x -> cexists p

// val ex_intro_x : #a:Type -> #p:(a -> Type) -> projectee:cexists p -> Tot a
// let ex_intro_x #a #p = function
//   | ExIntro x _ -> x

type ceq (#a: Type) x : a -> Type = | Refl : ceq #a x x

type ceq_type (a: Type) : Type -> Type = | ReflType : ceq_type a a
...
val ceq_congruence: #a: Type -> #b: Type -> #x: a -> #y: a -> ceq x y -> f: (a -> GTot b) ->
  GTot (ceq (f x) (f y))
let ceq_congruence #a #b #x #y h f =
  //refuse to infer terms with non-Tot effect
  Refl #_ #(f x)
