...
type view (a: Type) (b: Type) =
  | View : 
    ...
    put: (b -> GTot (Seq.lseq a n)){inverses get put} ->
    view a b
...
val mk_buffer_view: 
  ...
  #dest: Type ->
  b: B.buffer src ->
  v: view src dest {length b % View?.n v == 0} ->
  GTot (buffer dest)
...
val as_buffer_mk_buffer_view: 
  #src: Type ->
  #dest: Type ->
  b: B.buffer src ->
  v: view src dest {length b % View?.n v == 0} ->
  Lemma
    (let bv = mk_buffer_view b v in
      dfst bv == src /\ as_buffer bv == b) [SMTPat (as_buffer (mk_buffer_view b v) == b)]

/// `get_view`: Projecting the view functions itself
val get_view: #b: Type -> v: buffer b -> view (dfst v) b

/// A lemma-relating projector to constructor
val get_view_mk_buffer_view: 
  #src: Type ->
  #dest: Type ->
  b: B.buffer src ->
  v: view src dest {length b % View?.n v == 0} ->
  Lemma
    (let bv = mk_buffer_view b v in
      dfst bv == src /\ get_view bv == v) [SMTPat (get_view (mk_buffer_view b v) == v)]
...
val length: #b: _ -> vb: buffer b -> GTot nat
...
val sel: #b: _ -> h: HS.mem -> vb: buffer b -> i: nat{i < length vb} -> GTot b

/// `upd h vb i x`: stores `x` at index `i` in the buffer `vb` in heap `h`
val upd: #b: _ -> h: HS.mem -> vb: buffer b {live h vb} -> i: nat{i < length vb} -> x: b ->
  GTot HS.mem
...
val sel_upd: 
  ...
  x: b ->
  h: HS.mem{live h vb} ->
  Lemma (if i = j then sel (upd h vb i x) vb j == x else sel (upd h vb i x) vb j == sel h vb j)
    [SMTPat (sel (upd h vb i x) vb j)]

/// `modifies` on views is just defined in terms of the underlying buffer
unfold
let modifies (#b: _) (vb: buffer b) (h: HS.mem) (h': HS.mem) =
  B.modifies (B.loc_buffer (as_buffer vb)) h h'

/// `upd_modifies`: Footprint of `upd`
val upd_modifies: #b: _ -> h: HS.mem -> vb: buffer b {live h vb} -> i: nat{i < length vb} -> x: b ->
  Lemma (ensures (modifies vb h (upd h vb i x) /\ live (upd h vb i x) vb)) [SMTPat (upd h vb i x)]

/// `as_seq h vb`: Viewing the entire buffer as a sequence of `b`
val as_seq: #b: _ -> h: HS.mem -> vb: buffer b -> GTot (Seq.lseq b (length vb))

/// `as_seq_sel`:
///
/// Relates selecting an element in the heap to selecting an element
/// from the sequence
val as_seq_sel: #b: _ -> h: HS.mem -> vb: buffer b -> i: nat{i < length vb} ->
...
val get_sel: #b: _ -> h: HS.mem -> vb: buffer b -> i: nat{i < length vb} ->
  Lemma
  (let open FStar.Mul in
    ...
    length_eq vb;
    view_indexing vb i;
    sel h vb i == View?.get v (Seq.slice (B.as_seq h (as_buffer vb)) (i * n) (i * n + n)))

/// `put_sel`:
///
/// Relates selecting a subsequence of the underlying buffer
/// to selecting and translating an element from the view.
val put_sel: #b: _ -> h: HS.mem -> vb: buffer b -> i: nat{i < length vb} ->
  Lemma
  (let open FStar.Mul in
    let v = get_view vb in
    let n = View?.n v in
    length_eq vb;
    view_indexing vb i;
    View?.put v (sel h vb i) == Seq.slice (B.as_seq h (as_buffer vb)) (i * n) (i * n + n))
