...
private let p_pred (#a:Type) (#b:preorder a) (r:mref a b) (p:(a -> prop))
  = fun h -> h `contains` r /\ p (sel h r)

abstract let token (#a:Type) (#b:preorder a) (r:mref a b) (p:(a -> prop){stable p b})
  = witnessed (p_pred r p)

abstract val witness_token: #a:Type -> #b:preorder a -> m:mref a b -> p:(a -> prop){stable p b}
...
abstract val recall_token: #a:Type -> #b:preorder a -> m:mref a b -> p:(a -> prop){stable p b}
                           -> ST unit (requires (fun _ ->  token m p))
                                     (ensures (fun h0 _ h1 -> h0==h1 /\ p (sel h1 m)))
let recall_token #a #b m p = gst_recall (p_pred m p)

let spred (#a:Type) (rel:preorder a) = p:(a -> prop){Preorder.stable p rel}
...
abstract val recall: p:(heap -> prop){ST.stable p} ->
  ST unit
    (requires (fun _ ->  witnessed p))
    (ensures (fun h0 _ h1 -> h0 == h1 /\ p h1))
let recall p = gst_recall p

abstract val witness: p:(heap -> prop){ST.stable p} ->
  ST unit
