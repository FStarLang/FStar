...
unfold val as_ref: #a:Type -> #b:reln a -> mref a b -> GTot (ref a)
unfold let as_ref #a #b m = m

unfold val contains: #a:Type -> #b:reln a -> h:heap -> mref a b -> GTot bool
unfold let contains #a #b h m = Heap.contains h (as_ref m)

unfold val sel: #a:Type -> #b:reln a -> h:heap -> mref a b -> GTot a
unfold let sel #a #b h m = Heap.sel h (as_ref m)

unfold val upd: #a:Type -> #b:reln a -> h:heap -> mref a b -> a -> GTot heap
unfold let upd #a #b h m v = Heap.upd h (as_ref m) v
