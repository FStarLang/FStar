...
abstract
type set (a: Type u#a {hasEq a}) : Type u#a = a -> Tot bool
abstract
type equal (#a: eqtype) (s1: set a) (s2: set a) = feq s1 s2
...
abstract
val mem_singleton: #a: eqtype -> x: a -> y: a ->
  Lemma (requires True) (ensures (mem y (singleton x) = (x = y))) [SMTPat (mem y (singleton x))]
abstract
val mem_union: #a: eqtype -> x: a -> s1: set a -> s2: set a ->
  Lemma (requires True)
...
abstract
val mem_subset: #a: eqtype -> s1: set a -> s2: set a ->
  Lemma (requires (forall x. mem x s1 ==> mem x s2))
    (ensures (subset s1 s2))
    [SMTPat (subset s1 s2)]
abstract
val subset_mem: #a: eqtype -> s1: set a -> s2: set a ->
...
(* extensionality *)
abstract
val lemma_equal_intro: #a: eqtype -> s1: set a -> s2: set a ->
  Lemma (requires (forall x. mem x s1 = mem x s2)) (ensures (equal s1 s2)) [SMTPat (equal s1 s2)]
abstract
val lemma_equal_elim: #a: eqtype -> s1: set a -> s2: set a ->
  Lemma (requires (equal s1 s2)) (ensures (s1 == s2)) [SMTPat (equal s1 s2)]
abstract
val lemma_equal_refl: #a: eqtype -> s1: set a -> s2: set a ->
...
unfold
val as_set: #a: eqtype -> l: list a -> Tot (set a)
let as_set (#a: eqtype) (l: list a) = normalize_term (as_set' l)
assume
val lemma_disjoint_subset: #a: eqtype -> s1: set a -> s2: set a -> s3: set a ->
