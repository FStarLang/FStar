...
abstract let mref (a:Type) (rel:preorder a) = ref a
val as_ref: #a:Type -> #rel:preorder a -> mref a rel -> GTot (ref a)
let as_ref #a #rel m = m

let contains (#a:Type) (#rel:preorder a) (h:heap) (m:mref a rel) = Heap.contains h (as_ref m)

val sel: #a:Type -> #rel:preorder a -> h:heap -> mref a rel -> GTot a
let sel #a #rel h m = Heap.sel h (as_ref m)

val upd: #a:Type -> #rel:preorder a -> h:heap -> mref a rel -> a -> GTot heap
let upd #a #rel h m v = Heap.upd h (as_ref m) v


(* Generated preorder on heaps *)

let h_rel (h0:heap) (h1:heap) 
...
(*val test_lemma: (a:Type) 
             -> (rel:preorder a) 
	     -> (m:mref a rel) 
	     -> (h:heap)
	     -> (v:a)
	     -> Lemma (requires (rel (sel h m) v))
		      (ensures  (h_rel h (upd h m v)))
let test_lemma a rel m h v = ()*)
