...
let rec find #a f l =
  match l with
  | [] ->
    //These type annotations are only present because it makes bootstrapping go much faster
    None #(x: a{f x})
...
let mem_filter_spec (#a: Type) (f: (a -> Tot bool)) (m: list a) (u: option (x: unit{hasEq a}))
  : Tot Type0 =
  match u with
  | None -> True
...
val filter: #a: Type -> f: (a -> Tot bool) -> l: list a ->
  Tot (m: list a {forall u. mem_filter_spec f m u})
let rec filter #a f =
  function
  | [] -> []
...
type-checking time, [f] to be a pure total function.*)
val mem_filter: #a: eqtype -> f: (a -> Tot bool) -> l: list a -> x: a ->
  Lemma (requires (mem #a x (filter f l))) (ensures (f x))
...
function. *)
val mem_filter_forall: #a: eqtype -> f: (a -> Tot bool) -> l: list a ->
  Lemma (requires True) (ensures (forall x. mem #a x (filter f l) ==> f x)) [SMTPat (filter f l)]
...
let rec for_all_mem (#a: eqtype) (f: (a -> Tot bool)) (l: list a)
  : Lemma (for_all f l <==> (forall x. mem x l ==> f x)) =
  match l with
  | [] -> ()
  | _ :: q -> for_all_mem f q
...
val partition_length: f: ('a -> Tot bool) -> l: list 'a ->
  Lemma (requires True)
...
let compare_of_bool #a rel x y = if rel x y then 1 else if x = y then 0 else 0 - 1
...
private abstract
let test_sort: unit = assert (sortWith (compare_of_bool ( < )) [3; 2; 1] = [1; 2; 3])

(** A l1 is a strict prefix of l2. *)

let rec strict_prefix_of (#a: Type) (l1: list a) (l2: list a)
  : Pure Type0 (requires True) (ensures (fun _ -> True)) (decreases l2) =
  match l2 with
  | [] -> False
  | _ :: q -> l1 == q \/ strict_prefix_of l1 q
