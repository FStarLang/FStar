...
let seqn (n:nat) (a:Type) = s:seq a{S.length s = n}

let in_ (#a:Type) (s:seq a) = n:nat{n < S.length s}

let rec pigeonhole (#n:nat) (s:seq (fin n))
  : Pure (in_ s * in_ s)
    (requires (b2t (S.length s = n + 1)))
    (ensures (fun (i1, i2) -> i1 < i2 /\ S.index s i1 = S.index s i2))
    (decreases n)
=
  if n = 0 then (match index s 0 with)
  else if n = 1 then (assert (index s 0 = index s 1) ; 0,1)
  else
    let k0 = index s 0 in
    match find s (fun k -> k = k0) 1 with
    | Some i -> 0, i
    | None ->
      let reduced_s : seq (id (n-1)) =
        init #(id (n-1)) n (fun i ->
          let k : nat = index s (i+1) in
          assert (k <> k0) ;
          if k < k0 then (k <: id (n -1))
          else (k - 1 <: id (n-1)))
          in
...
(*       S.for_alli (fun i l -> L.for_all (fun x -> index v x = i) l) seen /\ *)
(*       max_arg counts = max_count /\ *)
(*       L.length (satisfies (fun v -> v = 0) counts) = support /\ *)
(*       (i-1)/support <= index counts max_count *)
(*     )) *)
(*     (ensures (fun r -> L.length r >= k / n /\ *)
(*         k > n ==> (let i = index v (index r 0) in S.for_all (fun x -> index v x = i) r) *)
(*     )) *)
(* = *)
(*   let k0 = index v i in *)
(*   let c = index counts k0 in *)
(*   if c + 1 >= k / n *)
(*   then i :: index seen k0 *)
(*   else *)
(*     let counts = upd count k0 (c+1) in *)
(*     if i +1 < k *)
(*     then *)
(*       let seen = upd seen k0 (i::index seen k0) in *)
(*       let max_count= *)
(*         if index counts max_count < c+1 *)
(*         then k0 *)
(*         else max_count *)
(*       in *)
...
(*       (\* [index counts x < k/support] indices in v with value x *\) *)
...
(*     (requires True) *)
(*     (ensures (fun v' -> exists x. L.forallp (fun i -> index v i == x) v')) *)
