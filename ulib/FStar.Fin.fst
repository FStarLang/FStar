...
#push-options "--fuel 0 --ifuel 0 --z3refresh --query_stats --log_queries"
#restart-solver
// for better readability, I kept the signature explicit
let pigeonhole_eq (#a:Type) (eq: equivalence_relation a) 
                      (holes: S.seq a{S.length holes > 0}) 
                      (pigeons: S.seq (items_of eq holes)) =   
  assume (S.length holes <> 1);                    
  let first_pigeon = S.index pigeons 0 in 
  let pp = find pigeons (fun k -> eq k first_pigeon) 1 in
  match pp with
  | Some i -> 
    // admit();
    symm_lemma eq (S.index pigeons 0) (S.index pigeons i);
    (0,i)
  | None ->
    admit()
