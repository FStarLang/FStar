...
private
let mul_lemma (a: nat) (b: nat) (c: nat) : Lemma (requires (a <= b)) (ensures (c * a <= c * b)) = ()
private
let mul_lemma' (a: nat) (b: nat) (c: pos) : Lemma (requires (c * a <= c * b)) (ensures (a <= b)) =
  ()
private
let mul_div_lemma (a: nat) (b: pos) : Lemma (b * (a / b) <= a) = ()

val slash_decr_axiom: a: nat -> b: pos -> Lemma (a / b <= a)
let slash_decr_axiom a b = mul_lemma 1 b a; mul_div_lemma a b; mul_lemma' (a / b) a b
private
let lemma_mul_minus_distr_l (a: int) (b: int) (c: int) : Lemma (a * (b - c) = a * b - a * c) = ()

(* Axiom: definition of the "b divides c" relation *)
#reset-options "--z3rlimit 20"
val slash_star_axiom: a: nat -> b: pos -> c: nat ->
...
let div a b =
  if a < 0
  then (slash_decr_axiom (- a) b; if a % b = 0 then - (- a / b) else - (- a / b) - 1)
  else a / b
...
let rec powx_lemma2 x n m =
  let ass (x: int) (y: int) (z: int) : Lemma ((x * y) * z == x * (y * z)) = () in
  match n with
  | 0 -> ()
...
val div_non_eucl_bigger_denom_lemma: a: int -> b: pos ->
  Lemma (requires (b > abs a)) (ensures (div_non_eucl a b = 0))
