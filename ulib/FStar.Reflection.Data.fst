...
type vconst =
  | C_Unit : vconst
  // Not exposing the full details of our integer repr.
  | C_Int : int -> vconst
  | C_True : vconst
  | C_False : vconst
...
type pattern =
  // A built-in constant
  | Pat_Constant : vconst -> pattern
  // A fully applied constructor
  | Pat_Cons : fv -> list pattern -> pattern
  // Pattern bound variable
  | Pat_Var : bv -> pattern
  // Wildcard (GM: why is this not Pat_var too?)
  | Pat_Wild : bv -> pattern
  // Dot pattern: resolved by other elements in the pattern and type
  | Pat_Dot_Term : bv -> term -> pattern
...
noeq
type bv_view = { bv_ppname:string; bv_index:int; bv_sort:typ }
...
type comp_view =
  ...
  // pre & post
  | C_Lemma : term -> term -> comp_view
  | C_Unknown : comp_view

noeq
type sigelt_view =
  | Sg_Let : r: bool -> fv: fv -> typ: typ -> def: term -> sigelt_view
  ...
  | Sg_Inductive : 
    ...
    binders ->
    typ:
    // the type annotation for the inductive, i.e., indices -> Type #u
    typ ->
    cts:
    // constructor names
    list name ->
    sigelt_view
  | Sg_Constructor : name: name -> typ: typ -> sigelt_view
  | Unk
...
let smaller tv t =
  match tv with
  | Tv_App l r -> l << t /\ r << t /\ fst r << t
  | Tv_Abs b t' | Tv_Arrow b t' -> b << t /\ t' << t
  | Tv_Refine b t' -> bv << t /\ t' << t
  | Tv_Let r bv t1 t2 -> bv << t /\ t1 << t /\ t2 << t
  | Tv_Match t1 brs -> t1 << t /\ (forall_list (fun (b, t') -> t' << t) brs)
  | Tv_AscribedT e ty tac -> e << t /\ ty << t /\ tac << t
  | Tv_AscribedC e c tac -> e << t /\ c << t /\ tac << t
  | Tv_Type _ | Tv_Const _ | Tv_Unknown | Tv_Var _ | Tv_BVar _ | Tv_Uvar _ _ | Tv_FVar _ -> True
...
let smaller_comp cv c =
  ...
  | C_Total t md ->
    ...
    (match md with
      | Some d -> d << c
      | None -> True)
  | C_Lemma pre post -> pre << c /\ post << c
  | C_Unknown -> True

val smaller_bv: bv_view -> bv -> Type0
let smaller_bv bvv bv = bvv.bv_sort << bv

val smaller_binder: binder -> (bv * aqualv) -> Type0
let smaller_binder b (bv, _) = bv << b
