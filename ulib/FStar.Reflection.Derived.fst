...
let name_of_bv (bv: bv) : string = (inspect_bv bv).bv_ppname

let type_of_bv (bv: bv) : typ = (inspect_bv bv).bv_sort
...
let mk_binder (bv: bv) : binder = pack_binder bv Q_Explicit

let name_of_binder (b: binder) : string = name_of_bv (bv_of_binder b)

let type_of_binder (b: binder) : typ = type_of_bv (bv_of_binder b)

let binder_to_string (b: binder) : string = bv_to_string (bv_of_binder b) //TODO: print aqual
...
let compare_binder (b1: binder) (b2: binder) : order =
  let bv1, _ = inspect_binder b1 in
  let bv2, _ = inspect_binder b2 in
  compare_bv bv1 bv2

let rec compare_term (s: term) (t: term) : order =
  match inspect_ln s, inspect_ln t with
  | Tv_Var sv, Tv_Var tv -> compare_bv sv tv
  | Tv_BVar sv, Tv_BVar tv -> compare_bv sv tv
  | Tv_FVar sv, Tv_FVar tv -> compare_fv sv tv
  | Tv_App h1 a1, Tv_App h2 a2 -> lex (compare_term h1 h2) (fun () -> compare_argv a1 a2)
  | Tv_Abs b1 e1, Tv_Abs b2 e2 -> lex (compare_binder b1 b2) (fun () -> compare_term e1 e2)
  | Tv_Refine bv1 e1, Tv_Refine bv2 e2 -> lex (compare_bv bv1 bv2) (fun () -> compare_term e1 e2)
  | Tv_Arrow b1 e1, Tv_Arrow b2 e2 -> lex (compare_binder b1 b2) (fun () -> compare_comp e1 e2)
  | Tv_Type (), Tv_Type () -> Eq
  | Tv_Const c1, Tv_Const c2 -> compare_const c1 c2
  | Tv_Uvar u1 _, Tv_Uvar u2 _ -> compare_int u1 u2
  | Tv_Let r1 bv1 t1 t1', Tv_Let r2 bv2 t2 t2' ->
    lex (compare_bv bv1 bv2) (fun () -> lex (compare_term t1 t2) (fun () -> compare_term t1' t2'))
  | Tv_Match _ _, Tv_Match _ _ -> Eq // TODO
  | Tv_AscribedT e1 t1 tac1, Tv_AscribedT e2 t2 tac2 ->
    lex (compare_term e1 e2)
      (fun () ->
  ...
  | Tv_Match _ _, _ -> Lt
  | _, Tv_Match _ _ -> Gt
  | Tv_AscribedT _ _ _, _ -> Lt
  | _, Tv_AscribedT _ _ _ -> Gt
  | Tv_AscribedC _ _ _, _ -> Lt
  | _, Tv_AscribedC _ _ _ -> Gt
  | Tv_Unknown, _ -> Lt
  | _, Tv_Unknown -> Gt
and compare_argv (a1: argv) (a2: argv) : order =
  let a1, q1 = a1 in
  let a2, q2 = a2 in
  match q1, q2 with
  | Q_Implicit, Q_Explicit -> Lt (* We should never see Q_Meta here *)
  | Q_Explicit, Q_Implicit -> Gt
  | _, _ -> compare_term a1 a2
and compare_comp (c1: comp) (c2: comp) : order =
  let cv1 = inspect_comp c1 in
  let cv2 = inspect_comp c2 in
  match cv1, cv2 with
  | C_Total t1 md1, C_Total t2 md2 ->
    lex (compare_term t1 t2)
      (fun () ->
          match md1, md2 with
          | None, None -> Eq
...
let mk_strcat (t1: term) (t2: term) : term =
  mk_e_app (pack_ln (Tv_FVar (pack_fv ["Prims"; "strcat"]))) [t1; t2]

let mk_cons (h: term) (t: term) : term = mk_e_app (pack_ln (Tv_FVar (pack_fv cons_qn))) [h; t]

let mk_cons_t (ty: term) (h: term) (t: term) : term =
...
let mkpair (t1: term) (t2: term) : term = mktuple_n [t1; t2]
