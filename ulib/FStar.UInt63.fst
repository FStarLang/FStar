...
abstract
type t = | Mk : v: uint_t n -> t

abstract
let v (x: t) : Tot (uint_t n) = x.v

abstract
let uint_to_t (x: uint_t n) : Pure t (requires True) (ensures (fun y -> v y = x)) = Mk x

let uv_inv (x: t) : Lemma (ensures (uint_to_t (v x) == x)) [SMTPat (v x)] = ()

let vu_inv (x: uint_t n) : Lemma (ensures (v (uint_to_t x) == x)) [SMTPat (uint_to_t x)] = ()

let v_inj (x1: t) (x2: t) : Lemma (requires (v x1 == v x2)) (ensures (x1 == x2)) = ()

abstract
let add (a: t) (b: t) : Pure t (requires (size (v a + v b) n)) (ensures (fun c -> v a + v b = v c)) =
  Mk (add (v a) (v b))
...
abstract
let add_mod (a: t) (b: t)
  : Pure t (requires True) (ensures (fun c -> FStar.UInt.add_mod (v a) (v b) = v c)) =
  Mk (add_mod (v a) (v b))

(* Subtraction primitives *)

abstract
let sub (a: t) (b: t) : Pure t (requires (size (v a - v b) n)) (ensures (fun c -> v a - v b = v c)) =
  Mk (sub (v a) (v b))
...
abstract
let sub_mod (a: t) (b: t)
  : Pure t (requires True) (ensures (fun c -> FStar.UInt.sub_mod (v a) (v b) = v c)) =
  Mk (sub_mod (v a) (v b))

(* Multiplication primitives *)

abstract
let mul (a: t) (b: t) : Pure t (requires (size (v a * v b) n)) (ensures (fun c -> v a * v b = v c)) =
  Mk (mul (v a) (v b))

abstract
let mul_underspec (a: t) (b: t)
  : Pure t (requires True) (ensures (fun c -> size (v a * v b) n ==> v a * v b = v c)) =
  Mk (mul_underspec (v a) (v b))

abstract
let mul_mod (a: t) (b: t)
  : Pure t (requires True) (ensures (fun c -> FStar.UInt.mul_mod (v a) (v b) = v c)) =
  Mk (mul_mod (v a) (v b))

abstract
let mul_div (a: t) (b: t)
  : Pure t (requires True) (ensures (fun c -> FStar.UInt.mul_div (v a) (v b) = v c)) =
  Mk (mul_div (v a) (v b))

(* Division primitives *)

abstract
let div (a: t) (b: t{v b <> 0}) : Pure t (requires (True)) (ensures (fun c -> v a / v b = v c)) =
  Mk (div (v a) (v b))

(* Modulo primitives *)

abstract
let rem (a: t) (b: t{v b <> 0})
  : Pure t (requires True) (ensures (fun c -> FStar.UInt.mod (v a) (v b) = v c)) =
  Mk (mod (v a) (v b))

(* Bitwise operators *)

abstract
let logand (x: t) (y: t) : Pure t (requires True) (ensures (fun z -> logand (v x) (v y) = v z)) =
  Mk (logand (v x) (v y))

abstract
let logxor (x: t) (y: t) : Pure t (requires True) (ensures (fun z -> logxor (v x) (v y) == v z)) =
  Mk (logxor (v x) (v y))

abstract
let logor (x: t) (y: t) : Pure t (requires True) (ensures (fun z -> logor (v x) (v y) == v z)) =
  Mk (logor (v x) (v y))

abstract
let lognot (x: t) : Pure t (requires True) (ensures (fun z -> lognot (v x) == v z)) =
  Mk (lognot (v x))
...
let eq_mask (a: t) (b: t)
  ...
  if v a = v b then Mk (pow2 n - 1) else Mk 0
...
private unfold
let __uint_to_t (x: int) : Tot t = uint_to_t x
#reset-options
