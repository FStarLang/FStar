...
abstract
type rid : (a: Type0{hasEq a}) = erased (list (int * int))

abstract
let reveal (r: rid) : GTot (list (int * int)) = reveal r

abstract
let color (x: rid) : GTot int =
  match reveal x with
  | [] -> 0
  | (c, _) :: _ -> c
...
let has_eq_rid (u: unit) : Lemma (requires True) (ensures hasEq rid) = ()

//TODO: we just proved this above, but we need to expose it as an argument-free SMT lemma, which isn't supported yet
...
val lemma_root_has_color_zero: r: rid{r == root} ->
  Lemma (requires True) (ensures (color r = 0)) [SMTPat (color r)]
...
private abstract
let rid_length (r: rid) : GTot nat = List.Tot.length (reveal r)

private abstract
let rid_tail (r: rid{rid_length r > 0}) : rid = elift1_p Cons?.tl r
...
let rec includes r1 r2 =
  if r1 = r2
  then true
  else if rid_length r2 > rid_length r1 then includes r1 (rid_tail r2) else false
...
let rec lemma_disjoint_includes i j k =
  ...
  then ()
    ...
    (lemma_disjoint_includes i j (rid_tail k);
      if rid_length i <= rid_length (rid_tail k) then () else (if includes k i then lemma_aux k i))

abstract
val extends: rid -> rid -> GTot bool
...
val lemma_includes_refl: i: rid ->
  Lemma (requires (True)) (ensures (includes i i)) [SMTPat (includes i i)]
...
let lemma_includes_anti_symmetric (i: rid) (j: rid)
  : Lemma (requires (includes i j /\ i <> j)) (ensures (not (includes j i))) [SMTPat (includes i j)] =
  ()

abstract
val lemma_extends_disjoint: i: rid -> j: rid -> k: rid ->
  Lemma (requires (extends j i /\ extends k i /\ j <> k)) (ensures (disjoint j k))
let lemma_extends_disjoint i j k = ()

abstract
val lemma_extends_parent: i: rid{i <> root} ->
  Lemma (requires True) (ensures (extends i (parent i))) [SMTPat (parent i)]
let lemma_extends_parent i = ()

abstract
val lemma_extends_not_root: i: rid -> j: rid{extends j i} ->
  Lemma (requires True) (ensures (j <> root)) [SMTPat (extends j i)]
let lemma_extends_not_root i j = ()

abstract
val lemma_extends_only_parent: i: rid -> j: rid{extends j i} ->
  Lemma (requires True) (ensures (i = parent j)) [SMTPat (extends j i)]
let lemma_extends_only_parent i j = ()

private abstract
let test0: unit = assert (includes (hide [(0, 1); (1, 0)]) (hide [(2, 2); (0, 1); (1, 0)]))
private abstract
let test1 (r1: rid) (r2: rid{includes r1 r2}) : unit =
  assert (includes r1 (hide ((0, 0) :: (reveal r2))))
...
abstract
val lemma_modifies_trans: m1: hmap -> m2: hmap -> m3: hmap -> s1: Set.set rid -> s2: Set.set rid ->
...
let rec lemma_includes_trans i j k =
  if j = k then () else match reveal k with | hd :: tl -> lemma_includes_trans i j (hide tl)
...
let root_is_root (s: rid)
  : Lemma (requires (includes s root)) (ensures (s = root)) [SMTPat (includes s root)] = ()

abstract
let extend (r: rid) (n: int) (c: int)
  : Pure rid
    (requires True)
    (ensures
      (fun s -> extends s r /\ Cons? (reveal s) /\ Cons?.hd (reveal s) == (c, n) /\ color s == c)) =
  elift1 (fun r -> (c, n) :: r) r

abstract
let extend_monochrome (r: rid) (n: int)
  : Pure rid
    (requires True)
    (ensures
      (fun s ->
          extends s r /\ Cons? (reveal s) /\ Cons?.hd (reveal s) == ((color r), n) /\
          color s == color r)) =
  elift1 (fun r ->
        ((match r with
            | [] -> 0
