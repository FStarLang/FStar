...
abstract type hh_rid = erased (list (int * int))

let reveal (r:hh_rid) : GTot (list (int * int)) = reveal r

abstract let hh_color (x:hh_rid): GTot int =
  match reveal x with
  | [] -> 0
  | (c, _)::_ -> c

type t = Map.t hh_rid heap

let has_eq_rid (u:unit) :
  Lemma (requires True)
        (ensures hasEq hh_rid)
  = ()

assume HasEq_rid: hasEq hh_rid //TODO: we just proved this above, but we need to expose it as an argument-free SMT lemma, which isn't supported yet

(* AR: see issue#1262 *)
abstract let hh_root :hh_rid = let x:hh_rid = hide [] in x

//is this SMTPat bad ?
val lemma_root_has_color_zero: r:hh_rid{r == hh_root}
                               -> Lemma (requires True) (ensures (hh_color r = 0))
                                 [SMTPat (hh_color r)]
let lemma_root_has_color_zero r = ()

//expose this so that no-one should assume otheriwse
let root_has_color_zero (u:unit) :Lemma (hh_color hh_root = 0) = ()

abstract type mrref (id:hh_rid) (a:Type) (rel:preorder a) = mref a rel

(* let has_eq_rref (id:hh_rid) (a:Type) : *)
(*   Lemma (requires True) *)
(*         (ensures hasEq (rref id a)) *)
(*      [SMTPat (hasEq (rref id a))] *)
(*   = ()        *)

abstract val as_ref : #a:Type -> #id:hh_rid -> #rel:preorder a -> r:mrref id a rel -> Tot (mref a rel)
let as_ref #a #id #rel r = r

val addr_of: #a:Type -> #id:hh_rid -> #rel:preorder a -> r:mrref id a rel -> GTot nat
let addr_of #a #id #rel r = addr_of (as_ref r)

let is_mm (#a:Type) (#id:hh_rid) (#rel:preorder a) (r:mrref id a rel) :GTot bool
  = is_mm (as_ref r)

abstract val ref_as_rref : #a:Type -> #rel:preorder a -> i:hh_rid -> r:mref a rel -> GTot (mrref i a rel)
let ref_as_rref #a #rel i r = r

val lemma_as_ref_inj: #a:Type -> #i:hh_rid -> #rel:preorder a -> r:mrref i a rel
    -> Lemma (requires (True))
             (ensures ((ref_as_rref i (as_ref r) == r)))
       [SMTPat (as_ref r)]
let lemma_as_ref_inj #a #i #rel r = ()

private let rid_tail (r:hh_rid{Cons? (reveal r)}) :hh_rid =
  elift1_p Cons?.tl r

private let rid_length (r:hh_rid) :GTot nat = List.Tot.length (reveal r)

abstract val includes : r1:hh_rid -> r2:hh_rid -> GTot bool (decreases (reveal r2))
...
let disjoint (i:hh_rid) (j:hh_rid) : GTot bool =
  not (includes i j) && not (includes j i)

private val lemma_aux: k:hh_rid -> i:hh_rid
      -> Lemma  (requires
                    rid_length k > 0
...
abstract val lemma_disjoint_includes: i:hh_rid -> j:hh_rid -> k:hh_rid ->
...
abstract val extends: hh_rid -> hh_rid -> GTot bool
let extends r0 r1 = Cons? (reveal r0) && rid_tail r0 = r1

abstract val parent: r:hh_rid{r<>hh_root} -> Tot hh_rid
let parent r = rid_tail r

abstract val lemma_includes_refl: i:hh_rid
                      -> Lemma (requires (True))
                               (ensures (includes i i))
                               [SMTPat (includes i i)]
let lemma_includes_refl i = ()

abstract val lemma_extends_includes: i:hh_rid -> j:hh_rid ->
  Lemma (requires (extends j i))
        (ensures (includes i j /\ not(includes j i)))
        [SMTPat (extends j i)]
let lemma_extends_includes i j = ()

let lemma_includes_anti_symmetric (i:hh_rid) (j:hh_rid) :
  Lemma (requires (includes i j /\ i <> j))
        (ensures (not (includes j i)))
        [SMTPat (includes i j)]
  = ()

abstract val lemma_extends_disjoint: i:hh_rid -> j:hh_rid -> k:hh_rid ->
    Lemma (requires (extends j i /\ extends k i /\ j<>k))
          (ensures (disjoint j k))
let lemma_extends_disjoint i j k = ()

abstract val lemma_extends_parent: i:hh_rid{i<>hh_root} ->
  Lemma (requires True)
        (ensures (extends i (parent i)))
        [SMTPat (parent i)]
let lemma_extends_parent i = ()

abstract val lemma_extends_not_root: i:hh_rid -> j:hh_rid{extends j i} ->
  Lemma (requires True)
        (ensures (j<>hh_root))
        [SMTPat (extends j i)]
let lemma_extends_not_root i j = ()

abstract val lemma_extends_only_parent: i:hh_rid -> j:hh_rid{extends j i} ->
  Lemma (requires True)
        (ensures (i = parent j))
        [SMTPat (extends j i)]
...
private let test1 (r1:hh_rid) (r2:hh_rid{includes r1 r2}) = assert (includes r1 (hide ((0,0)::(reveal r2))))


let sel (#a:Type) (#rel:preorder a) (#i:hh_rid) (m:t) (r:mrref i a rel) : GTot a
  = sel (Map.sel m i) (as_ref r)
unfold let op_String_Access (#a:Type) (#rel:preorder a) (#i:hh_rid) (m:t) (r:mrref i a rel) = sel m r

let upd (#a:Type) (#rel:preorder a) (#i:hh_rid) (m:t) (r:mrref i a rel) (v:a) :GTot t
  = Map.upd m i (upd (Map.sel m i) (as_ref r) v)
unfold let op_String_Assignment (#a:Type) (#rel:preorder a) (#i:hh_rid) (m:t) (r:mrref i a rel) v = upd m r v


assume val mod_set : Set.set hh_rid -> Tot (Set.set hh_rid)
assume Mod_set_def: forall (x:hh_rid) (s:Set.set hh_rid). {:pattern Set.mem x (mod_set s)}
                    Set.mem x (mod_set s) <==> (exists (y:hh_rid). Set.mem y s /\ includes y x)

let modifies (s:Set.set hh_rid) (m0:t) (m1:t) =
  Map.equal m1 (Map.concat m1 (Map.restrict (Set.complement (mod_set s)) m0))
  /\ Set.subset (Map.domain m0) (Map.domain m1)

let modifies_just (s:Set.set hh_rid) (m0:t) (m1:t) =
  Map.equal m1 (Map.concat m1 (Map.restrict (Set.complement s) m0))
  /\ Set.subset (Map.domain m0) (Map.domain m1)

let modifies_one (r:hh_rid) (m0:t) (m1:t) =
  modifies_just (Set.singleton r) m0 m1

let equal_on (s:Set.set hh_rid) (m0:t) (m1:t) =
 (forall (r:hh_rid). {:pattern (Map.contains m0 r)} (Set.mem r (mod_set s) /\ Map.contains m0 r) ==> Map.contains m1 r)
 /\ Map.equal m1 (Map.concat m1 (Map.restrict (mod_set s) m0))

abstract val lemma_modifies_just_trans: m1:t -> m2:t -> m3:t
                       -> s1:Set.set hh_rid -> s2:Set.set hh_rid
...
abstract val lemma_includes_trans: i:hh_rid -> j:hh_rid -> k:hh_rid
...
abstract val lemma_modset: i:hh_rid -> j:hh_rid
...
abstract val lemma_disjoint_parents: pr:hh_rid -> r:hh_rid -> ps:hh_rid -> s:hh_rid -> Lemma
...
let unused_in (#a:Type) (#rel:preorder a) (#i:hh_rid) (r:mrref i a rel) (m:t) :GTot bool =
  not (Map.contains m i) ||
  FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.unused_in (as_ref r) (Map.sel m i))

let contains_ref_in_its_region (#a:Type) (#rel:preorder a) (#i:hh_rid) (r:mrref i a rel) (m:t) : GTot bool =
  FStar.StrongExcludedMiddle.strong_excluded_middle (Heap.contains (Map.sel m i) (as_ref r))

let modifies_rref (r:hh_rid) (s:Set.set nat) h0 h1 =
  Heap.modifies s (Map.sel h0 r) (Map.sel h1 r)

abstract val lemma_include_cons: i:hh_rid -> j:hh_rid -> Lemma
...
abstract let aref (i: hh_rid) : Type0 = Heap.aref

abstract let as_heap_aref (#i:hh_rid) (r:aref i) :Heap.aref = r

abstract let heap_aref_as_aref (i:hh_rid) (r:Heap.aref) :aref i = r

let lemma_as_aref_injective (#i:hh_rid) (r:aref i)
  :Lemma (requires True)
         (ensures  (heap_aref_as_aref i (as_heap_aref r) == r))
	 [SMTPat (as_heap_aref r)]
  = ()

abstract let dummy_aref (i: hh_rid) : aref i = Heap.dummy_aref
...
// abstract
// val contains_ref_aref_unused_in: #i: hh_rid -> #a:Type -> #rel: preorder a -> h:t -> x:mrref i a rel -> y:aref i -> Lemma
...
let aref_live_at (m: t) (#i: hh_rid) (a: aref i) (v: Type) (rel: preorder v) : GTot Type0 =
  Map.contains m i /\
...
abstract let extend (r:hh_rid) (n:int) (c:int)
  :Pure hh_rid (requires True) (ensures (fun s -> s `extends` r /\ Cons? (reveal s) /\ Cons?.hd (reveal s) == (c, n) /\ hh_color s == c))
  = elift1 (fun r -> (c, n)::r) r

abstract let extend_monochrome (r:hh_rid) (n:int)
  : Pure hh_rid (requires True) (ensures (fun s -> s `extends` r /\ Cons? (reveal s) /\ Cons?.hd (reveal s) == ((hh_color r), n) /\ hh_color s == hh_color r))
= elift1 (fun r -> ((match r with | [] -> 0 | (c, _) :: _ -> c), n)::r) r


abstract let alloc (#a:Type0) (rel:preorder a) (id:hh_rid) (init:a) (mm:bool) (m:t{m `Map.contains` id})
