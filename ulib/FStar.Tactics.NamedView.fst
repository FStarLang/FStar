...
let open_universe_view (v:RD.universe_view) : named_universe_view =
  ...
  | R.Uv_Name i -> Uv_Name (R.inspect_ident i)
...
let close_universe_view (v:named_universe_view) : R.universe_view =
  ...
  | Uv_Name i -> R.Uv_Name (R.pack_ident i)
...
let __binding_to_binder (bnd : binding) (b : R.binder) : binder =
  {
      ...
      uniq   = bnd.uniq;
      sort   = bnd.sort;
      qual   = (R.inspect_binder b).qual;
      attrs  = (R.inspect_binder b).attrs;
  }
...
let open_term_with (b : R.binder) (nb : binder) (t : term) : Tac term =
  ...
  in
  let t' = R.subst_term [DB 0 nv] t in
  t'
...
let r_subst_binder_sort (s : subst_t) (b : R.binder) : R.binder =
  let v = R.inspect_binder b in
  let v = { v with sort = R.subst_term s v.sort } in
  R.pack_binder v

let subst_binder_sort (s : subst_t) (b : binder) : binder =
  { b with sort = R.subst_term s b.sort }
...
let close_term_n (bs : list binder) (t : term) : list R.binder & term =
  let rec aux (bs : list binder) (cbs : list R.binder) (s : subst_t) : list R.binder & subst_t =
    ...
    | b::bs ->
      ...
      aux bs (b::cbs) (NM nv 0 :: R.shift_subst 1 s)
  in
  let cbs, s = aux bs [] [] in
  List.Tot.rev cbs, R.subst_term s t
...
let rec open_pat (p : R.pattern) (s : subst_t) : Tac (pattern & subst_t) =
  ...
  | R.Pat_Var ssort n ->
    ...
    let sort = R.subst_term s sort in
    let nvv : namedv = {
      uniq = fresh();
      sort = seal sort;
      ppname = n;
    ...
    in
    let nv = R.pack_namedv nvv in
    Pat_Var {v=nvv; sort=seal sort}, (DB 0 nv) :: R.shift_subst 1 s
...
let rec close_pat (p : pattern) (s : subst_t) : Tot (R.pattern & subst_t) =
  ...
  | Pat_Var {v; sort} ->
    ...
    let s = (NM nv 0) :: R.shift_subst 1 s in
...
let open_view (tv:term_view) : Tac (tv':named_term_view{ctor_matches tv' tv}) =
  ...
  | RD.Tv_BVar v -> Tv_BVar (R.inspect_bv v)
  ...
  | RD.Tv_AscribedC e c tac use_eq -> Tv_AscribedC e (R.inspect_comp c) tac use_eq
  ...
  | RD.Tv_Let recf attrs b def body ->
    ...
    let def =
      if recf
      then R.subst_term [DB 0 (r_binder_to_namedv nb)] def
      else def
    in
...
let close_view (tv : named_term_view) : Tot (tv':term_view{ctor_matches tv tv'}) =
  ...
  | Tv_BVar v -> RD.Tv_BVar (R.pack_bv v)
  ...
  | Tv_AscribedC e c tac use_eq -> RD.Tv_AscribedC e (R.pack_comp c) tac use_eq
  ...
  | Tv_Let recf attrs nb def body ->
    let def =
      if recf
      then R.subst_term [NM (r_binder_to_namedv nb) 0] def
      else def
    in
...
let open_univ_s (us : list R.univ_name) : Tac (list univ_name & subst_t) =
  ...
  Util.map (fun i -> R.inspect_ident i) us, s

private
let close_univ_s (us : list univ_name) : list R.univ_name & subst_t =
  let n = List.Tot.length us in
  let us = List.Tot.map (fun i -> R.pack_ident i) us in
  let s = List.Tot.mapi (fun i u -> UD u (n-i-1)) us in
  us, s

private
let open_lb (lb : R.letbinding) : Tac letbinding =
  let {lb_fv; lb_us; lb_typ; lb_def} = R.inspect_lb lb in
  let lb_us, s = open_univ_s lb_us in
  let lb_typ = R.subst_term s lb_typ in
  let lb_def = R.subst_term s lb_def in
...
let close_lb (lb : letbinding) : R.letbinding =
  ...
  let lb_typ = R.subst_term s lb_typ in
  let lb_def = R.subst_term s lb_def in
  R.pack_lb { lb_fv; lb_us; lb_typ; lb_def }

private
let subst_r_binders (s:subst_t) (bs : list R.binder) : list R.binder =
  List.Tot.mapi (fun i b -> r_subst_binder_sort (R.shift_subst i s) b) bs
...
let open_sigelt_view (sv : sigelt_view) : Tac named_sigelt_view =
  ...
  | RD.Sg_Inductive nm univs params typ ctors ->
    ...
    let typ = R.subst_term (R.shift_subst nparams s) typ in
    let ctors = map (fun (nm, ty) -> nm, R.subst_term s ty) ctors in
