...
let path_disjoint_decomp
  ...
  : Lemma (requires (path_disjoint p1 p2))
    (ensures (exists (d: path_disjoint_decomp_t p1 p2). True)) =
  path_disjoint_ind (fun #v1 #v2 p1 p2 ->
        exists (d: path_disjoint_decomp_t #from #v1 #v2 p1 p2). True)
...
let path_sel_upd_other
  ...
  path_disjoint_ind (fun #v1 #v2 p1_ p2_ ->
    ...
    (fun #through #to1_ #to2_ p s1 s2 ->
        FStar.Classical.forall_intro_sub #_
          #(fun m ->
...
let as_addr (#t: typ) (p: pointer t) = HS.aref_as_addr (Pointer?.contents p)
...
let loc_aux_includes_pointer (s: loc_aux) (#t: typ) (p: pointer t) : GTot Type0 =
  match s with
...
let loc_aux_includes (s: loc_aux) (s2: loc_aux) : GTot Type0 (decreases s2) =
  match s2 with
  | LocPointer p -> loc_aux_includes_pointer s p
  | LocBuffer b -> loc_aux_includes_buffer s b

let loc_aux_includes_refl' (s: loc_aux) : Lemma (ensures (loc_aux_includes s s)) = ()
...
let loc_aux_includes_refl'' (s: loc_aux)
  : Lemma (loc_aux_includes s s) [SMTPat (loc_aux_includes s s)] = loc_aux_includes_refl' s

let loc_aux_includes_loc_aux_includes_pointer (s1: loc_aux) (s2: loc_aux) (#t: typ) (p: pointer t)
...
let loc_aux_includes_trans (s1: loc_aux) (s2: loc_aux) (s3: loc_aux)
...
let loc_aux_disjoint_pointer (l: loc_aux) (#t: typ) (p: pointer t) : GTot Type0 =
  match l with
...
let loc_aux_disjoint (l1: loc_aux) (l2: loc_aux) : GTot Type0 (decreases l2) =
  match l2 with
...
let loc_aux_disjoint_loc_aux_includes (l1: loc_aux) (l2: loc_aux) (l3: loc_aux)
...
let pointer_preserved (#t: typ) (p: pointer t) (h: HS.mem) (h': HS.mem) : GTot Type0 =
  equal_values h p h' p

let buffer_preserved (#t: typ) (b: buffer t) (h: HS.mem) (h': HS.mem) : GTot Type0 =
  forall (i: FStar.UInt32.t).
    FStar.UInt32.v i < FStar.UInt32.v (buffer_length b) ==>
    pointer_preserved (gpointer_of_buffer_cell b i) h h'

let loc_aux_preserved (l: loc_aux) (h: HS.mem) (h': HS.mem) : GTot Type0 =
  match l with
  | LocBuffer b -> buffer_preserved b h h'
  | LocPointer p -> pointer_preserved p h h'

let pointer_preserved_intro
  (#t: typ) (p: pointer t) (h1: HS.mem) (h2: HS.mem)
  (f:
    (a': Type0 -> pre: Preorder.preorder a' -> r': HS.mreference a' pre ->
        Lemma
          (requires (HS.contains h1 r' /\ frameOf p == HS.frameOf r' /\ as_addr p == HS.as_addr r'))
          (ensures (HS.contains h2 r' /\ HS.sel h1 r' == HS.sel h2 r'))))
  : Lemma (pointer_preserved p h1 h2) =
  let g () : Lemma (requires (live h1 p)) (ensures (pointer_preserved p h1 h2)) =
    f _ _ (greference_of p)
  in
  Classical.move_requires g ()

let buffer_preserved_intro
  (#t: typ) (p: buffer t) (h1: HS.mem) (h2: HS.mem)
  (f:
    (a': Type0 -> pre: Preorder.preorder a' -> r': HS.mreference a' pre ->
        Lemma
          (requires
            (HS.contains h1 r' /\ frameOf_buffer p == HS.frameOf r' /\
              buffer_as_addr p == HS.as_addr r'))
          (ensures (HS.contains h2 r' /\ HS.sel h1 r' == HS.sel h2 r'))))
  : Lemma (buffer_preserved p h1 h2) =
  let g (i: FStar.UInt32.t{FStar.UInt32.v i < FStar.UInt32.v (buffer_length p)})
    : Lemma (ensures (pointer_preserved (gpointer_of_buffer_cell p i) h1 h2)) =
    pointer_preserved_intro (gpointer_of_buffer_cell p i) h1 h2 f
  in
  Classical.forall_intro g

let disjoint_not_self (#t: typ) (p: pointer t) : Lemma (disjoint p p ==> False) =
...
let loc_aux_in_addr (l: loc_aux) (r: HS.rid) (n: nat) : GTot Type0 =
  match l with
  | LocBuffer b -> frameOf_buffer b == r /\ buffer_as_addr b == n
  | LocPointer p -> frameOf p == r /\ as_addr p == n

let aloc (r: HS.rid) (n: nat) : Tot Type0 = (l: loc_aux{loc_aux_in_addr l r n})
...
let loc_pointer #t p = MG.loc_of_aloc #_ #cls #(frameOf p) #(as_addr p) (LocPointer p)

let loc_buffer #t p = MG.loc_of_aloc #_ #cls #(frameOf_buffer p) #(buffer_as_addr p) (LocBuffer p)
...
let loc_includes_gsingleton_buffer_of_pointer l #t p =
  MG.loc_includes_aloc #_
    ...
    #(as_addr p)
    (LocPointer p)
    (LocBuffer (gsingleton_buffer_of_pointer p));
...
let loc_includes_gbuffer_of_array_pointer l #len #t p =
  MG.loc_includes_aloc #_
    ...
    #(as_addr p)
    (LocPointer p)
    (LocBuffer (gbuffer_of_array_pointer p));
...
let loc_includes_gpointer_of_array_cell l #t b i =
  MG.loc_includes_aloc #_
    ...
    #(buffer_as_addr b)
    (LocBuffer b)
    (LocPointer (gpointer_of_buffer_cell b i));
...
let loc_disjoint_root #value1 #value2 p1 p2 =
  MG.loc_disjoint_addresses #_
    #cls
    false
    false
    (frameOf p1)
    (frameOf p2)
    (Set.singleton (as_addr p1))
    (Set.singleton (as_addr p2));
  ...
  MG.loc_disjoint_includes #_
    #cls
    (loc_addresses (frameOf p1) (Set.singleton (as_addr p1)))
    (loc_addresses (frameOf p2) (Set.singleton (as_addr p2)))
    (loc_pointer p1)
    (loc_pointer p2)
...
let loc_disjoint_gfield #l p fd1 fd2 =
  MG.loc_disjoint_aloc_intro #_
    ...
    #(frameOf p)
    #(as_addr p)
    (LocPointer (gfield p fd1))
    (LocPointer (gfield p fd2))
...
let loc_disjoint_gcell #length #value p i1 i2 =
  MG.loc_disjoint_aloc_intro #_
    ...
    #(frameOf p)
    #(as_addr p)
    (LocPointer (gcell p i1))
    (LocPointer (gcell p i2))

let loc_disjoint_includes = MG.loc_disjoint_includes

let live_unused_in_disjoint_strong #value1 #value2 h p1 p2 = ()

let live_unused_in_disjoint #value1 #value2 h p1 p2 = loc_disjoint_root p1 p2
...
let pointer_live_reference_unused_in_disjoint #value1 #value2 h p1 p2 =
  ...
  MG.loc_disjoint_addresses #_
    ...
    (HS.frameOf p2)
    (Set.singleton (as_addr p1))
    (Set.singleton (HS.as_addr p2));
  MG.loc_disjoint_includes #_
    #cls
    (loc_addresses (frameOf p1) (Set.singleton (as_addr p1)))
    (MG.loc_freed_mreference p2)
    (loc_pointer p1)
    (MG.loc_freed_mreference p2)
...
let reference_live_pointer_unused_in_disjoint #value1 #value2 h p1 p2 =
  ...
  MG.loc_disjoint_addresses #_
    ...
    (HS.frameOf p1)
    (frameOf p2)
    (Set.singleton (HS.as_addr p1))
    (Set.singleton (as_addr p2));
  MG.loc_disjoint_includes #_
    #cls
    (MG.loc_freed_mreference p1)
    (loc_addresses (frameOf p2) (Set.singleton (as_addr p2)))
    (MG.loc_freed_mreference p1)
    (loc_pointer p2)
...
let loc_disjoint_gsub_buffer #t b i1 len1 i2 len2 =
  MG.loc_disjoint_aloc_intro #_
    ...
    #(frameOf_buffer b)
    #(buffer_as_addr b)
    (LocBuffer (gsub_buffer b i1 len1))
    (LocBuffer (gsub_buffer b i2 len2))
...
let loc_disjoint_gpointer_of_buffer_cell #t b i1 i2 =
  MG.loc_disjoint_aloc_intro #_
    ...
    #(frameOf_buffer b)
    #(buffer_as_addr b)
    (LocPointer (gpointer_of_buffer_cell b i1))
    (LocPointer (gpointer_of_buffer_cell b i2))

let loc_disjoint_addresses = MG.loc_disjoint_addresses #_ #cls false false

let loc_disjoint_pointer_addresses #t p r n =
  loc_disjoint_includes (loc_addresses (frameOf p) (Set.singleton (as_addr p)))
    (loc_addresses r n)
    (loc_pointer p)
    (loc_addresses r n)

let loc_disjoint_buffer_addresses #t p r n =
  loc_disjoint_includes (loc_addresses (frameOf_buffer p) (Set.singleton (buffer_as_addr p)))
    (loc_addresses r n)
    (loc_buffer p)
    (loc_addresses r n)
...
let modifies_reference_elim #t b p h h' =
  MG.loc_includes_addresses_addresses #_
    cls
    false
    true
    (HS.frameOf b)
...
let screate (value: typ) (s: option (type_of_typ value)) =
  ...
  MG.modifies_intro loc_none
    ...
    (fun _ _ _ -> ())
    (fun _ _ -> ())
    (fun r a b -> cls.MG.same_mreference_aloc_preserved b h0 h1 (fun _ _ _ -> ()));
  p
...
let ecreate (t: typ) (r: HS.rid) (s: option (type_of_typ t)) =
  ...
  let s =
    ...
    | _ -> none_ovalue t
  in
  let content: HS.ref pointer_ref_contents = HST.ralloc r (| t, s |) in
...
let owrite (#a: typ) (b: pointer a) (z: otype_of_typ a)
  ...
  let f' (t: typ) (p: pointer t)
    : Lemma
    ((frameOf p == frameOf b /\ as_addr p == as_addr b /\ live h0 p /\ disjoint b p) ==>
      (equal_values h0 p h1 p)) =
    Classical.move_requires (f t) p
  in
  Classical.forall_intro_2 f';
  MG.modifies_intro (loc_pointer b)
    h0
    h1
    (fun _ -> ())
    (fun t' pre' p' ->
...
let modifies_loc_addresses_intro r a l h1 h2 =
  ...
  MG.modifies_loc_includes (loc_union (loc_addresses r a) l)
    h1
    h2
    (loc_union (MG.loc_addresses true r a) l)
...
let reference_live_buffer_unused_in_disjoint #t1 #t2 h b1 b2 =
  ...
  MG.loc_disjoint_addresses #_
    ...
    (HS.frameOf b1)
    (frameOf_buffer b2)
    (Set.singleton (HS.as_addr b1))
    (Set.singleton (buffer_as_addr b2));
  MG.loc_disjoint_includes #_
    #cls
    (MG.loc_freed_mreference b1)
    (loc_addresses (frameOf_buffer b2) (Set.singleton (buffer_as_addr b2)))
    (MG.loc_freed_mreference b1)
    (loc_buffer b2)
...
let buffer_live_reference_unused_in_disjoint #t1 #t2 h b1 b2 =
  ...
  MG.loc_disjoint_addresses #_
    ...
    (frameOf_buffer b1)
    (HS.frameOf b2)
    (Set.singleton (buffer_as_addr b1))
    (Set.singleton (HS.as_addr b2));
  MG.loc_disjoint_includes #_
    #cls
    (loc_addresses (frameOf_buffer b1) (Set.singleton (buffer_as_addr b1)))
    (MG.loc_freed_mreference b2)
    (loc_buffer b1)
    (MG.loc_freed_mreference b2)
