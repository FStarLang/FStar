...
type step : from: typ -> to: typ -> Tot Type0 =
  | StepField : l: struct_typ -> fd: struct_field l -> step (TStruct l) (typ_of_struct_field l fd)
  | StepUField : l: union_typ -> fd: struct_field l -> step (TUnion l) (typ_of_struct_field l fd)
  | StepCell : 
    length: UInt32.t ->
    value: typ ->
...
type path (from: typ) : to: typ -> Tot Type0 =
  | PathBase : path from from
  | PathStep : through: typ -> to: typ -> p: path from through -> s: step through to -> path from to

let step_typ_depth (#from: typ) (#to: typ) (s: step from to) : Lemma (typ_depth from > typ_depth to) =
  match s with
  | StepUField l fd | StepField l fd -> typ_depth_typ_of_struct_field l.fields fd
  | _ -> ()

let rec path_typ_depth (#from: typ) (#to: typ) (p: path from to)
  : Lemma
    (ensures
      (typ_depth from >= typ_depth to /\ ((~(PathBase? p)) ==> typ_depth from <> typ_depth to)))
    (decreases p) =
  match p with
  | PathBase -> ()
  | PathStep _ _ p' s -> path_typ_depth p'; step_typ_depth s
...
noeq
type _npointer (to: typ) : Type0 =
  | Pointer : from: typ -> contents: HS.aref -> p: path from to -> _npointer to
  | NullPtr

let npointer (t: typ) : Tot Type0 = _npointer t
...
let g_is_null_intro (t: typ) : Lemma (g_is_null (nullptr #t) == true) = ()
...
noeq
type buffer_root (t: typ) =
  | BufferRootSingleton : p: pointer t {not_an_array_cell p} -> buffer_root t
  | BufferRootArray : #max_length: array_length_t -> p: pointer (TArray max_length t) ->
    buffer_root t

let buffer_root_length (#t: typ) (b: buffer_root t) : Tot UInt32.t =
  match b with
...
(* ghostly-tagged data *)
let gtdata (key: eqtype u#0) (value: (key -> Tot Type0)) : Tot Type0 = (k: key & value k)

let _gtdata_get_key (#key: eqtype) (#value: (key -> Tot Type0)) (u: gtdata key value) : Tot key =
  dfst u

let gtdata_get_key (#key: eqtype) (#value: (key -> Tot Type0)) (u: gtdata key value) : GTot key =
  // important: must be Ghost, the tag is not actually stored in memory
  _gtdata_get_key u

let gtdata_get_value (#key: eqtype) (#value: (key -> Tot Type0)) (u: gtdata key value) (k: key)
  : Pure (value k) (requires (gtdata_get_key u == k)) (ensures (fun _ -> True)) =
  let (| _ , v |) = u in
  v

let gtdata_create (#key: eqtype) (#value: (key -> Tot Type0)) (k: key) (v: value k)
  : Pure (gtdata key value)
    (requires True)
    (ensures (fun x -> gtdata_get_key x == k /\ gtdata_get_value x k == v)) = (| k, v |)

let gtdata_extensionality
  (#key: eqtype) (#value: (key -> Tot Type0)) (u1: gtdata key value) (u2: gtdata key value)
  : Lemma
    (requires
      (let k = gtdata_get_key u1 in
        (k == gtdata_get_key u2 /\ gtdata_get_value u1 k == gtdata_get_value u2 k)))
    (ensures (u1 == u2)) = ()

(* Interperets a type code (`typ`) as a FStar type (`Type0`). *)
let rec type_of_typ' (t: typ) : Tot Type0 =
  match t with
...
let rec type_of_typ'_eq (t: typ) : Lemma (type_of_typ' t == type_of_typ t) [SMTPat (type_of_typ t)] =
  match t with
...
let struct_upd (#l: struct_typ) (s: struct l) (f: struct_field l) (v: type_of_struct_field l f)
  : Tot (struct l) = DM.upd s f v

let struct_create_fun (l: struct_typ) (f: (fd: struct_field l -> Tot (type_of_struct_field l fd)))
  : Tot (struct l) =
...
*)
let rec dummy_val (t: typ) : Tot (type_of_typ t) =
  match t with
  | TBase b ->
    (match b with
      | TUInt -> 0
      ...
      | TBool -> false
      | TUnit -> ())
  | TStruct l -> struct_create_fun l (fun f -> (dummy_val (typ_of_struct_field l f)))
  | TUnion l ->
...
let rec otype_of_typ (t: typ) : Tot Type0 =
  match t with
  | TBase b -> option (type_of_base_typ b)
  | TStruct l -> option (DM.t (struct_field l) (type_of_struct_field' l otype_of_typ))
  | TUnion l -> option (gtdata (struct_field l) (type_of_struct_field' l otype_of_typ))
  | TArray length t -> option (array length (otype_of_typ t))
  | TPointer t -> option (pointer t)
  | TNPointer t -> option (npointer t)
  | TBuffer t -> option (buffer t)

let otype_of_struct_field (l: struct_typ) : Tot (struct_field l -> Tot Type0) =
  type_of_struct_field' l otype_of_typ

let otype_of_typ_otype_of_struct_field (l: struct_typ) (f: struct_field l)
  : Lemma (otype_of_typ (typ_of_struct_field l f) == otype_of_struct_field l f)
    [SMTPat (type_of_typ (typ_of_struct_field l f))] = ()

let otype_of_typ_base (b: base_typ)
  : Lemma (otype_of_typ (TBase b) == option (type_of_base_typ b)) [SMTPat (otype_of_typ (TBase b))] =
  ()

let otype_of_typ_array (len: array_length_t) (t: typ)
  : Lemma (otype_of_typ (TArray len t) == option (array len (otype_of_typ t)))
    [SMTPat (otype_of_typ (TArray len t))] = ()

let ostruct (l: struct_typ) = option (DM.t (struct_field l) (otype_of_struct_field l))

let ostruct_sel (#l: struct_typ) (s: ostruct l {Some? s}) (f: struct_field l)
  : Tot (otype_of_struct_field l f) = DM.sel (Some?.v s) f

let ostruct_upd
  (#l: struct_typ) (s: ostruct l {Some? s}) (f: struct_field l) (v: otype_of_struct_field l f)
  : Tot (s': ostruct l {Some? s'}) = Some (DM.upd (Some?.v s) f v)

let ostruct_create (l: struct_typ) (f: (fd: struct_field l -> Tot (otype_of_struct_field l fd)))
  : Tot (s': ostruct l {Some? s'}) = Some (DM.create #(struct_field l) #(otype_of_struct_field l) f)

let otype_of_typ_struct (l: struct_typ)
...
let ounion_get_key (#l: union_typ) (v: ounion l {Some? v}) : Tot (struct_field l) =
  _gtdata_get_key (Some?.v v)

let ounion_get_value (#l: union_typ) (v: ounion l {Some? v}) (fd: struct_field l)
  : Pure (otype_of_struct_field l fd) (requires (ounion_get_key v == fd)) (ensures (fun _ -> True)) =
  gtdata_get_value (Some?.v v) fd

let ounion_create (l: union_typ) (fd: struct_field l) (v: otype_of_struct_field l fd)
  : Tot (ounion l) = Some (gtdata_create fd v)

let otype_of_typ_union (l: union_typ)
...
let struct_field_is_readable
  (l: struct_typ)
  (ovalue_is_readable:
    (t: typ -> v: otype_of_typ t -> Pure bool (requires (t << l)) (ensures (fun _ -> True))))
  (v: ostruct l {Some? v}) (s: string)
  : Tot bool =
  if List.Tot.mem s (List.Tot.map fst l.fields)
  then ovalue_is_readable (typ_of_struct_field l s) (ostruct_sel v s)
  else true

let rec ovalue_is_readable (t: typ) (v: otype_of_typ t) : Tot bool (decreases t) =
  match t with
  | TStruct l ->
    let v: ostruct l = v in
    Some? v &&
    (let keys = List.Tot.map fst l.fields in
      let pred (t': typ) (v: otype_of_typ t')
        : Pure bool (requires (t' << l)) (ensures (fun _ -> True)) =
  ...
  | TArray len t ->
    ...
    Some? v && Seq.for_all (ovalue_is_readable t) (Some?.v v)
  | TBase t ->
...
let ovalue_is_readable_struct_intro' (l: struct_typ) (v: otype_of_typ (TStruct l))
  : Lemma
    (requires
      (let v: ostruct l = v in
        (Some? v /\
          List.Tot.for_all (struct_field_is_readable l ovalue_is_readable v)
            (List.Tot.map fst l.fields)))) (ensures (ovalue_is_readable (TStruct l) v)) =
...
let rec ovalue_is_readable_struct_elim
  (l: struct_typ) (v: otype_of_typ (TStruct l)) (fd: struct_field l)
  ...
  assert (List.Tot.for_all (struct_field_is_readable l ovalue_is_readable v)
...
let ovalue_is_readable_array_elim
  (#len: array_length_t) (#t: typ) (v: otype_of_typ (TArray len t))
  (i: UInt32.t{UInt32.v i < UInt32.v len})
  : Lemma (requires (ovalue_is_readable (TArray len t) v))
    (ensures
      (let v: option (array len (otype_of_typ t)) = v in
        (Some? v /\ ovalue_is_readable t (Seq.index (Some?.v v) (UInt32.v i))))) = ()

let ovalue_is_readable_array_intro (#len: array_length_t) (#t: typ) (v: otype_of_typ (TArray len t))
  : Lemma
    (requires
      (let v: option (array len (otype_of_typ t)) = v in
        (Some? v /\
  ...
  let f (i: nat{i < UInt32.v len}) : Lemma (ovalue_is_readable t (Seq.index v i)) =
    let j: j: UInt32.t{UInt32.v j < UInt32.v len} = UInt32.uint_to_t i in
...
let ostruct_field_of_struct_field
  ...
  (ovalue_of_value:
    (t: typ -> v: type_of_typ t ->
        Pure (otype_of_typ t) (requires (t << l)) (ensures (fun _ -> True)))) (v: struct l)
  (f: struct_field l)
  : Tot (otype_of_struct_field l f) = ovalue_of_value (typ_of_struct_field l f) (struct_sel #l v f)

(* TODO: move to Seq.Base *)

let seq_init_index (#a: Type) (len: nat) (contents: (i: nat{i < len} -> Tot a)) (i: nat)
  : Lemma (requires (i < len))
    (ensures (i < len /\ Seq.index (Seq.init len contents) i == contents i))
    [SMTPat (Seq.index (Seq.init len contents) i)] = Seq.init_index len contents

let rec ovalue_of_value (t: typ) (v: type_of_typ t) : Tot (otype_of_typ t) (decreases t) =
  match t with
  | TStruct l ->
    let oval (t': typ) (v': type_of_typ t')
  ...
  | TArray len t ->
    ...
    let f (i: nat{i < UInt32.v len}) : Tot (otype_of_typ t) = ovalue_of_value t (Seq.index v i) in
...
let ovalue_is_readable_ostruct_field_of_struct_field
  ...
  (ih:
    (t: typ -> v: type_of_typ t ->
        Lemma (requires (t << l)) (ensures (ovalue_is_readable t (ovalue_of_value t v)))))
  (v: struct l) (f: struct_field l)
  : Lemma
  (ovalue_is_readable (typ_of_struct_field l f)
      (ostruct_field_of_struct_field l ovalue_of_value v f)) =
  ih (typ_of_struct_field l f) (struct_sel #l v f)

let rec ovalue_is_readable_ovalue_of_value (t: typ) (v: type_of_typ t)
  : Lemma (requires True)
  ...
  | TArray len t ->
    ...
    let v': array len (otype_of_typ t) = Some?.v v' in
    let phi (i: nat{i < Seq.length v'}) : Lemma (ovalue_is_readable t (Seq.index v' i)) =
...
let rec value_of_ovalue (t: typ) (v: otype_of_typ t) : Tot (type_of_typ t) (decreases t) =
  match t with
  | TStruct l ->
    let v: ostruct l = v in
    if Some? v
    then
      let phi (f: struct_field l) : Tot (type_of_struct_field l f) =
  ...
  | TArray len t' ->
    ...
    (match v with
      | None -> dummy_val t
      | Some v ->
        let phi (i: nat{i < UInt32.v len}) : Tot (type_of_typ t') =
...
let ovalue_of_value_array_index
  (#len: array_length_t) (t': typ) (v: array len (type_of_typ t')) (sv: array len (otype_of_typ t'))
  : Lemma (requires (ovalue_of_value (TArray len t') v == Some sv))
    (ensures
      (forall (i: nat). i < UInt32.v len ==> Seq.index sv i == ovalue_of_value t' (Seq.index v i))) =
  ()

let value_of_ovalue_array_index (#len: array_length_t) (t': typ) (sv: array len (otype_of_typ t'))
  : Lemma
  (ensures
    (forall (i: nat).
        i < UInt32.v len ==>
        Seq.index (value_of_ovalue (TArray len t') (Some sv)) i ==
        value_of_ovalue t' (Seq.index sv i))) = ()

#set-options "--z3rlimit 16"

let rec value_of_ovalue_of_value (t: typ) (v: type_of_typ t)
  : Lemma (value_of_ovalue t (ovalue_of_value t v) == v)
    [SMTPat (value_of_ovalue t (ovalue_of_value t v))] =
  match t with
  | TStruct l ->
    let v: struct l = v in
    let v': struct l = value_of_ovalue t (ovalue_of_value t v) in
    let phi (f: struct_field l) : Lemma (struct_sel #l v' f == struct_sel #l v f) =
  ...
  | TArray len t' ->
    ...
    let phi (i: nat{i < UInt32.v len})
      : Lemma (value_of_ovalue t' (ovalue_of_value t' (Seq.index v i)) == Seq.index v i) =
...
let none_ovalue (t: typ) : Tot (otype_of_typ t) =
  match t with
...
let not_ovalue_is_readable_none_ovalue (t: typ)
  : Lemma (ovalue_is_readable t (none_ovalue t) == false) = ()
...
let step_sel (#from: typ) (#to: typ) (m': otype_of_typ from) (s: step from to) =
  match s with
  | StepField l fd ->
  ...
  | StepUField l fd ->
    ...
    (match m' with
      | None -> none_ovalue to
      | _ -> if fd = ounion_get_key m' then ounion_get_value m' fd else none_ovalue to)
...
let ovalue_is_readable_step_sel_cell
  (#length: array_length_t) (#value: typ) (m': otype_of_typ (TArray length value))
  (index: UInt32.t{UInt32.v index < UInt32.v length})
  : Lemma (requires (ovalue_is_readable (TArray length value) m'))
...
let ovalue_is_readable_step_sel_field (#l: struct_typ) (m: ostruct l) (fd: struct_field l)
  : Lemma (requires (ovalue_is_readable (TStruct l) m))
...
let ovalue_is_readable_step_sel_union_same (#l: union_typ) (m: ounion l) (fd: struct_field l)
  : Lemma (requires (ovalue_is_readable (TUnion l) m /\ ounion_get_key m == fd))
    (ensures (ovalue_is_readable (typ_of_struct_field l fd) (step_sel m (StepUField l fd)))) = ()

let step_sel_none_ovalue (#from: typ) (#to: typ) (s: step from to)
  : Lemma (step_sel (none_ovalue from) s == none_ovalue to) = ()

let rec path_sel (#from: typ) (#to: typ) (m: otype_of_typ from) (p: path from to)
  : Tot (otype_of_typ to) (decreases p) =
  match p with
  | PathBase -> m
...
let rec path_sel_none_ovalue (#from: typ) (#to: typ) (p: path from to)
  : Lemma (requires True) (ensures (path_sel (none_ovalue from) p == none_ovalue to)) (decreases p) =
  match p with
  | PathBase -> ()
  | PathStep through' to' p' s -> path_sel_none_ovalue p'

let step_upd (#from: typ) (#to: typ) (m: otype_of_typ from) (s: step from to) (v: otype_of_typ to)
  : Tot (otype_of_typ from) (decreases s) =
  match s with
  | StepField l fd ->
    let m: ostruct l = m in
    (match m with
      | None ->
        ...
        let phi (fd': struct_field l) : Tot (otype_of_struct_field l fd') =
          if fd' = fd then v else none_ovalue (typ_of_struct_field l fd')
        in
    ...
    (match m with
      | None ->
        (* whole array does not exist yet,
         ...
         and all others uninitialized *)
        let phi (j: nat{j < UInt32.v len}) : Tot (otype_of_typ to) =
          if j = UInt32.v i then v else none_ovalue to
        in
        let m': option (array len (otype_of_typ to)) = Some (Seq.init (UInt32.v len) phi) in
        m'
      | Some m ->
        let m': option (array len (otype_of_typ to)) = Some (Seq.upd m (UInt32.v i) v) in
        m')
  | StepUField l fd ->
...
let step_sel_upd_same
  (#from: typ) (#to: typ) (m: otype_of_typ from) (s: step from to) (v: otype_of_typ to)
  : Lemma (step_sel (step_upd m s v) s == v) = ()

let rec path_upd
  (#from: typ) (#to: typ) (m: otype_of_typ from) (p: path from to) (v: otype_of_typ to)
  : Tot (otype_of_typ from) (decreases p) =
  match p with
  | PathBase -> v
...
let rec path_concat
  (#from: typ) (#through: typ) (#to: typ) (p: path from through) (q: path through to)
  : Pure (path from to) (requires True) (ensures (fun _ -> True)) (decreases q) =
  match q with
  | PathBase -> p
  | PathStep through' to' q' st -> PathStep through' to' (path_concat p q') st

let path_concat_base_r (#from: typ) (#to: typ) (p: path from to)
  : Lemma (ensures (path_concat p PathBase == p)) = ()

let rec path_concat_base_l (#from: typ) (#to: typ) (p: path from to)
  : Lemma (requires True)
...
let rec path_concat_assoc
  (#t0: typ) (#t1: typ) (#t2: typ) (#t3: typ) (p01: path t0 t1) (p12: path t1 t2) (p23: path t2 t3)
  : Lemma (requires True)
...
let rec path_sel_concat
  (#from: typ) (#through: typ) (#to: typ) (m: otype_of_typ from) (p: path from through)
  (q: path through to)
  : Lemma (requires True)
...
let rec path_upd_concat
  (#from: typ) (#through: typ) (#to: typ) (m: otype_of_typ from) (p: path from through)
  (q: path through to) (v: otype_of_typ to)
  : Lemma (requires True)
...
let rec path_includes (#from: typ) (#to1: typ) (#to2: typ) (p1: path from to1) (p2: path from to2)
  : Ghost bool (requires True) (ensures (fun _ -> True)) (decreases p2) =
  (to1 = to2 && p1 = p2) ||
  (match p2 with
    | PathBase -> false
    | PathStep _ _ p2' _ -> path_includes p1 p2')

let rec path_includes_base (#from: typ) (#to: typ) (p: path from to)
  : Lemma (requires True)
...
let path_includes_refl (#from: typ) (#to: typ) (p: path from to)
  : Lemma (requires True) (ensures (path_includes p p)) [SMTPat (path_includes p p)] = ()

let path_includes_step_r
  (#from: typ) (#through: typ) (#to: typ) (p: path from through) (s: step through to)
  : Lemma (requires True)
    (ensures (path_includes p (PathStep through to p s)))
    [SMTPat (path_includes p (PathStep through to p s))] = ()

let rec path_includes_trans
  (#from: typ) (#to1: typ) (#to2: typ) (#to3: typ) (p1: path from to1) (p2: path from to2)
  (p3: path from to3 {path_includes p1 p2 /\ path_includes p2 p3})
  : Lemma (requires True) (ensures (path_includes p1 p3)) (decreases p3) =
  FStar.Classical.or_elim #(to2 == to3 /\ p2 == p3)
    #(match p3 with
      | PathBase -> False
      | PathStep _ _ p3' _ -> path_includes p2 p3')
    #(fun _ -> path_includes p1 p3)
    (fun _ -> ())
    (fun _ ->
        match p3 with
        | PathBase -> assert False
        | PathStep _ _ p3' _ -> path_includes_trans p1 p2 p3')

let rec path_includes_ind
  (#from: typ)
  (x:
    (#to1: typ -> #to2: typ -> p1: path from to1 -> p2: path from to2 {path_includes p1 p2} ->
        GTot Type0))
  (h_step:
    (
        #through: typ ->
  ...
  (h_refl: (#to: typ -> p: path from to {path_includes p p} -> Lemma (x p p)))
  (h_trans:
    (
        #to1: typ ->
        #to2: typ ->
        ...
        p3:
        path from to3
          {path_includes p1 p2 /\ path_includes p2 p3 /\ path_includes p1 p3 /\ x p1 p2 /\ x p2 p3} ->
        Lemma (x p1 p3))) (#to1: typ) (#to2: typ) (p1: path from to1)
  (p2: path from to2 {path_includes p1 p2})
  : Lemma (requires True) (ensures (x p1 p2)) (decreases p2) =
  FStar.Classical.or_elim #(to1 == to2 /\ p1 == p2)
    #(match p2 with
      | PathBase -> False
...
let rec path_length (#from: typ) (#to: typ) (p: path from to) : Tot nat (decreases p) =
  match p with
  | PathBase -> 0
  | PathStep _ _ p' _ -> 1 + path_length p'

let path_includes_length
  (#from: typ) (#to1: typ) (#to2: typ) (p1: path from to1) (p2: path from to2 {path_includes p1 p2})
  : Lemma (ensures (path_length p1 <= path_length p2)) =
  path_includes_ind (fun #to1_ #to2_ p1_ p2_ -> path_length p1_ <= path_length p2_)
    (fun #through #to p st -> ())
    (fun #to p -> ())
...
let path_includes_exists_concat
  (#from: typ) (#through: typ) (p: path from through) (#to: typ)
  (q: path from to {path_includes p q})
  : Lemma (ensures (exists (r: path through to). q == path_concat p r)) =
  path_includes_ind (fun #to1_ #to2_ p1_ p2_ -> exists r. p2_ == path_concat p1_ r)
    (fun #through #to_ p s ->
    ...
    (fun #to1_ #to2_ #to3_ p1_ p2_ p3_ ->
        FStar.Classical.exists_elim (exists r. p3_ == path_concat p1_ r)
          #_
          #(fun r12 -> p2_ == path_concat p1_ r12)
          ()
          (fun r12 ->
              FStar.Classical.exists_elim (exists r. p3_ == path_concat p1_ r)
                #_
                #(fun r23 -> p3_ == path_concat p2_ r23)
                ()
                (fun r23 ->
                    path_concat_assoc p1_ r12 r23;
                    FStar.Classical.exists_intro (fun r -> p3_ == path_concat p1_ r)
                      (path_concat r12 r23))))
    p
    q

let path_concat_includes
  (#from: typ) (#through: typ) (p: path from through)
  (phi:
    (#to: typ -> p': path from to ->
        Ghost Type0 (requires (path_includes p p')) (ensures (fun _ -> True))))
  (f: (to: typ -> p': path through to -> Lemma (ensures (phi (path_concat p p'))))) (#to: typ)
  (q: path from to)
  : Lemma (requires (path_includes p q)) (ensures (path_includes p q /\ phi q)) =
  Classical.forall_intro_2 f;
  path_includes_exists_concat p q

let step_disjoint (#from: typ) (#to1: typ) (#to2: typ) (s1: step from to1) (s2: step from to2)
  : GTot bool =
  match s1 with
  | StepField _ fd1 ->
...
let step_eq (#from: typ) (#to1: typ) (#to2: typ) (s1: step from to1) (s2: step from to2)
...
let step_disjoint_not_eq
  (#from: typ) (#to1: typ) (#to2: typ) (s1: step from to1) (s2: step from to2)
  : Lemma (requires (step_disjoint s1 s2 == true)) (ensures (step_eq s1 s2 == false)) =
  (* Note: the converse is now wrong, due to unions *)
  ()

let step_disjoint_sym (#from: typ) (#to1: typ) (#to2: typ) (s1: step from to1) (s2: step from to2)
  : Lemma (requires (step_disjoint s1 s2)) (ensures (step_disjoint s2 s1)) = ()

noeq
type path_disjoint_t (#from: typ)
  : #to1: typ -> #to2: typ -> p1: path from to1 -> p2: path from to2 -> Type0
  =
  | PathDisjointStep : 
    #through: typ ->
    #to1: typ ->
...
let rec path_disjoint_t_rect
  ...
  (h_includes:
    (
        ...
        h': path_disjoint_t p1' p2' ->
        ihx: x p1 p2 h ->
        GTot (x p1' p2' h'))) (#value1: typ) (#value2: typ) (p1: path from value1)
  (p2: path from value2) (h: path_disjoint_t p1 p2)
  : Ghost (x p1 p2 h) (requires True) (ensures (fun _ -> True)) (decreases h) =
  match h with
  | PathDisjointStep p s1 s2 -> h_step p s1 s2 h
  | PathDisjointIncludes p1_ p2_ p1' p2' h_ ->
    h_includes p1_ p2_ p1' p2' h_ h (path_disjoint_t_rect x h_step h_includes p1_ p2_ h_)

let path_disjoint
  (#from: typ) (#value1: typ) (#value2: typ) (p1: path from value1) (p2: path from value2)
  : GTot Type0 = squash (path_disjoint_t p1 p2)
...
let path_disjoint_ind
  ...
  (h_includes:
    (
        ...
        p2':
        path from value2'
          {path_includes p2 p2' /\ path_disjoint p1 p2 /\ path_disjoint p1' p2' /\ x p1 p2} ->
        Lemma (x p1' p2'))) (#value1: typ) (#value2: typ) (p1: path from value1)
  (p2: path from value2 {path_disjoint p1 p2})
  : Lemma (x p1 p2) =
  let h: squash (path_disjoint_t p1 p2) = FStar.Squash.join_squash () in
  FStar.Squash.bind_squash h
    (fun (h: path_disjoint_t p1 p2) ->
        path_disjoint_t_rect (fun #v1 #v2 p1 p2 h ->
              let _ = FStar.Squash.return_squash h in
              squash (x p1 p2))
          (fun #through #to1 #to2 p s1 s2 h ->
              let _ = FStar.Squash.return_squash h in
              h_step p s1 s2)
...
let path_disjoint_step
  (#from: typ) (#through: typ) (#to1: typ) (#to2: typ) (p: path from through) (s1: step through to1)
  (s2: step through to2 {step_disjoint s1 s2})
  : Lemma (requires True)
...
let path_disjoint_includes
  (#from: typ) (#to1: typ) (#to2: typ) (p1: path from to1) (p2: path from to2) (#to1': typ)
  (#to2': typ) (p1': path from to1') (p2': path from to2')
  : Lemma (requires (path_disjoint p1 p2 /\ path_includes p1 p1' /\ path_includes p2 p2'))
    (ensures (path_disjoint p1' p2')) =
  let h: squash (path_disjoint_t p1 p2) = FStar.Squash.join_squash () in
  FStar.Squash.bind_squash h
    (fun h -> FStar.Squash.return_squash (PathDisjointIncludes p1 p2 p1' p2' h))
...
let rec path_disjoint_sym
  (#from: typ) (#value1: typ) (#value2: typ) (p1: path from value1) (p2: path from value2)
  : Lemma (requires (path_disjoint p1 p2))
...
let rec path_equal
  (#from: typ) (#value1: typ) (#value2: typ) (p1: path from value1) (p2: path from value2)
  : Tot (b: bool{b == true <==> (value1 == value2 /\ p1 == p2)}) (decreases p1) =
  match p1 with
  | PathBase -> PathBase? p2
  | PathStep _ _ p1' s1 ->
    PathStep? p2 &&
    (let PathStep _ _ p2' s2 = p2 in
      (path_equal p1' p2' && step_eq s1 s2))

let rec path_length_concat (#t0: typ) (#t1: typ) (#t2: typ) (p01: path t0 t1) (p12: path t1 t2)
  : Lemma (requires True)
...
let rec path_concat_inj_l
  (#from: typ) (#through1: typ) (p1_: path from through1) (#v1: typ) (p1: path through1 v1)
  (#through2: typ) (#v2: typ) (p2_: path from through2) (p2: path through2 v2)
  : Lemma
    (requires
      (path_equal (path_concat p1_ p1) (path_concat p2_ p2) == true /\
        path_length p1_ == path_length p2_))
...
type path_disjoint_decomp_t
  ...
  | PathDisjointDecomp : 
    ...
    squash (step_disjoint d_s1 d_s2 == true /\ p1 == path_concat (PathStep _ _ d_p d_s1) d_p1' /\
        p2 == path_concat (PathStep _ _ d_p d_s2) d_p2') ->
    path_disjoint_decomp_t p1 p2

let path_disjoint_decomp_includes
  (#from: typ) (#value1: typ) (#value2: typ) (p1: path from value1) (p2: path from value2)
  (#value1': typ) (#value2': typ) (p1': path from value1') (p2': path from value2')
  : Lemma
    (requires
      (path_includes p1 p1' /\ path_includes p2 p2' /\
        (exists (d: path_disjoint_decomp_t p1 p2). True)))
    (ensures (exists (d: path_disjoint_decomp_t p1' p2'). True)) =
  let f (q1: path value1 value1') (q2: path value2 value2') (d: path_disjoint_decomp_t p1 p2)
    : Lemma (requires (p1' == path_concat p1 q1 /\ p2' == path_concat p2 q2))
      (ensures (exists (d: path_disjoint_decomp_t p1' p2'). True)) =
  ...
  in
  let g (q1: path value1 value1') (q2: path value2 value2') (d: path_disjoint_decomp_t p1 p2)
    : Lemma
    ((p1' == path_concat p1 q1 /\ p2' == path_concat p2 q2) ==>
      (exists (d: path_disjoint_decomp_t p1' p2'). True)) =
    // FIXME: annoying to repeat those type annotations above. WHY WHY WHY can't I just use (fun q1 q2 d -> Classical.move_requires (f q1 q2) d) as an argument of Classical.forall_intro_3 below instead of this g???
    Classical.move_requires (f q1 q2) d
  in
  path_includes_exists_concat p1 p1';
  path_includes_exists_concat p2 p2';
  let _: squash (exists (d: path_disjoint_decomp_t p1' p2'). True) = Classical.forall_intro_3 g in
  ()
...
let path_disjoint_decomp
  ...
  : Lemma (requires (path_disjoint p1 p2))
    (ensures (exists (d: path_disjoint_decomp_t p1 p2). True)) =
  path_disjoint_ind (fun #v1 #v2 p1 p2 -> exists (d: path_disjoint_decomp_t p1 p2). True)
...
let path_disjoint_not_path_equal
  (#from: typ) (#value1: typ) (#value2: typ) (p1: path from value1) (p2: path from value2)
  : Lemma (requires (path_disjoint p1 p2)) (ensures (path_equal p1 p2 == false)) =
  let f (d: path_disjoint_decomp_t p1 p2) : Lemma (path_equal p1 p2 == false) =
    if path_equal p1 p2
    then
      let PathDisjointDecomp _ p _ s1 p1_ _ s2 p2_ _ = d in
...
let rec path_destruct_l (#t0: typ) (#t2: typ) (p: path t0 t2)
  : Tot
    (x:
      option (t1: typ &
          (s: step t0 t1 &
...
let rec path_equal' (#from: typ) (#to1: typ) (#to2: typ) (p1: path from to1) (p2: path from to2)
  : Tot (b: bool{b == true <==> to1 == to2 /\ p1 == p2}) (decreases (path_length p1)) =
  match path_destruct_l p1 with
  | None -> PathBase? p2
  | Some (| t1 , (| s1 , p1' |) |) ->
    match path_destruct_l p2 with
    | None -> false
    | Some (| t2 , (| s2 , p2' |) |) -> step_eq s1 s2 && path_equal' p1' p2'

let path_includes_concat_l
  (#from: typ) (#through: typ) (#to1: typ) (#to2: typ) (p0: path from through)
  (p1: path through to1) (p2: path through to2)
  : Lemma (requires (path_includes p1 p2))
    (ensures (path_includes (path_concat p0 p1) (path_concat p0 p2))) =
  path_includes_ind (fun #to1_ #to2_ p1_ p2_ ->
        path_includes (path_concat p0 p1_) (path_concat p0 p2_))
    (fun #through #to p st -> ())
    (fun #to p -> path_includes_refl (path_concat p0 p))
    (fun #to1_ #to2_ #to3_ p1_ p2_ p3_ ->
        path_includes_trans (path_concat p0 p1_) (path_concat p0 p2_) (path_concat p0 p3_))
    p1
    p2
...
let path_disjoint_concat
  ...
  path_disjoint_ind (fun #v1 #v2 p1 p2 -> path_disjoint (path_concat p0 p1) (path_concat p0 p2))
    ...
    (fun #v1 #v2 p1 p2 #v1' #v2' p1' p2' ->
        ...
        path_disjoint_includes (path_concat p0 p1)
          (path_concat p0 p2)
          (path_concat p0 p1')
          (path_concat p0 p2'))
    p1
...
let step_sel_upd_other
  (#from: typ) (#to1: typ) (#to2: typ) (s1: step from to1) (s2: step from to2 {step_disjoint s1 s2})
  (m: otype_of_typ from) (v: otype_of_typ to1)
  : Lemma (step_sel (step_upd m s1 v) s2 == step_sel m s2) =
  match s1 with
  | StepField l1 fd1 ->
  ...
  | StepCell length1 _ i1 ->
    ...
    match m with
    | None -> ()
    | Some m -> Seq.lemma_index_upd2 m (UInt32.v i1) v (UInt32.v i2)

let path_sel_upd_other
  (#from: typ) (#to1: typ) (#to2: typ) (p1: path from to1) (p2: path from to2 {path_disjoint p1 p2})
  : Lemma
  (ensures
    (forall (m: otype_of_typ from) (v: otype_of_typ to1).
        path_sel (path_upd m p1 v) p2 == path_sel m p2)) =
  path_disjoint_ind (fun #v1 #v2 p1_ p2_ ->
        forall (m: otype_of_typ from) (v: otype_of_typ v1).
          path_sel (path_upd m p1_ v) p2_ == path_sel m p2_)
    (fun #through #to1_ #to2_ p s1 s2 ->
        FStar.Classical.forall_intro' #_
          #(fun m ->
              forall (v: otype_of_typ to1_).
                path_sel (path_upd m (PathStep through to1_ p s1) v) (PathStep through to2_ p s2) ==
                path_sel m (PathStep through to2_ p s2))
          (fun m ->
              FStar.Classical.forall_intro' #_
                #(fun v ->
                    path_sel (path_upd m (PathStep through to1_ p s1) v)
                      (PathStep through to2_ p s2) ==
                    path_sel m (PathStep through to2_ p s2))
                (fun v ->
    ...
    (fun #v1 #v2 p1 p2 #v1' #v2' p1' p2' ->
        ...
        FStar.Classical.forall_intro' #_
          #(fun (m: otype_of_typ from) ->
              forall v. path_sel (path_upd m p1' v) p2' == path_sel m p2')
          (fun (m: otype_of_typ from) ->
              FStar.Classical.forall_intro' #_
                #(fun (v: otype_of_typ v1') -> path_sel (path_upd m p1' v) p2' == path_sel m p2')
                (fun (v: otype_of_typ v1') ->
                    FStar.Classical.exists_elim (path_sel (path_upd m p1' v) p2' == path_sel m p2')
                      h1
                      (fun r1 ->
                          FStar.Classical.exists_elim (path_sel (path_upd m p1' v) p2' ==
                              path_sel m p2')
                            h2
                            (fun r2 -> path_upd_concat m p1 r1 v; path_sel_concat m p2 r2)))))
    p1
    p2

let path_sel_upd_other'
  (#from: typ) (#to1: typ) (p1: path from to1) (m: otype_of_typ from) (v: otype_of_typ to1)
  (#to2: typ) (p2: path from to2)
  : Lemma (requires (path_disjoint p1 p2))
    (ensures (path_sel (path_upd m p1 v) p2 == path_sel m p2)) = path_sel_upd_other p1 p2


(** Operations on pointers *)


let equal (#t1: typ) (#t2: typ) (p1: pointer t1) (p2: pointer t2)
  : Ghost bool (requires True) (ensures (fun b -> b == true <==> t1 == t2 /\ p1 == p2)) =
  Pointer?.from p1 = Pointer?.from p2 && HS.aref_equal (Pointer?.contents p1) (Pointer?.contents p2) &&
  path_equal (Pointer?.p p1) (Pointer?.p p2)

let as_addr (#t: typ) (p: pointer t) : GTot nat = HS.aref_as_addr (Pointer?.contents p)

let _field (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l)
...
let _cell
  (#length: array_length_t) (#value: typ) (p: pointer (TArray length value))
...
let _ufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l)
  : Tot (pointer (typ_of_struct_field l fd)) =
  let Pointer from contents p' = p in
...
let unused_in (#value: typ) (p: pointer value) (h: HS.mem) : GTot Type0 =
  let Pointer from contents p' = p in
  HS.aref_unused_in contents h

let pointer_ref_contents: Type0 = (t: typ & otype_of_typ t)

let live (#value: typ) (h: HS.mem) (p: pointer value) : GTot Type0 =
...
let nlive (#value: typ) (h: HS.mem) (p: npointer value) : GTot Type0 =
  if g_is_null p then True else live h p

let live_nlive (#value: typ) (h: HS.mem) (p: pointer value) = ()

let g_is_null_nlive (#t: typ) (h: HS.mem) (p: npointer t) = ()

let greference_of (#value: typ) (p: pointer value)
  : Ghost (HS.reference pointer_ref_contents)
    ...
    (ensures
      (fun x ->
          (exists h. live h p) /\
          x ==
          HS.greference_of (Pointer?.contents p)
            pointer_ref_contents
            (Heap.trivial_preorder pointer_ref_contents) /\ HS.aref_of x == Pointer?.contents p)) =
  HS.greference_of (Pointer?.contents p)
    pointer_ref_contents
    (Heap.trivial_preorder pointer_ref_contents)

let unused_in_greference_of (#value: typ) (p: pointer value) (h: HS.mem)
  : Lemma (requires (exists h. live h p))
    (ensures ((exists h. live h p) /\ (HS.unused_in (greference_of p) h <==> unused_in p h)))
    [SMTPatOr [[SMTPat (HS.unused_in (greference_of p) h)]; [SMTPat (unused_in p h)]]] = ()

let live_not_unused_in (#value: typ) (h: HS.mem) (p: pointer value) =
  let f () : Lemma (requires (live h p /\ unused_in p h)) (ensures False) =
    let r = greference_of p in
    HS.contains_aref_unused_in h r (Pointer?.contents p)
  in
  Classical.move_requires f ()

let gread (#value: typ) (h: HS.mem) (p: pointer value) : GTot (type_of_typ value) =
  ...
  else dummy_val value

let frameOf (#value: typ) (p: pointer value) : GTot HS.rid = HS.frameOf_aref (Pointer?.contents p)
...
let disjoint_roots_intro_pointer_vs_pointer
  (#value1: typ) (value2: typ) (h: HS.mem) (p1: pointer value1) (p2: pointer value2)
  : Lemma (requires (live h p1 /\ unused_in p2 h))
    (ensures (frameOf p1 <> frameOf p2 \/ as_addr p1 =!= as_addr p2)) = ()

let disjoint_roots_intro_pointer_vs_reference
  (#value1: typ) (#value2: Type) (h: HS.mem) (p1: pointer value1) (p2: HS.reference value2)
  : Lemma (requires (live h p1 /\ HS.unused_in p2 h))
...
let disjoint_roots_intro_reference_vs_pointer
  (#value1: Type) (#value2: typ) (h: HS.mem) (p1: HS.reference value1) (p2: pointer value2)
  : Lemma (requires (HS.contains h p1 /\ unused_in p2 h))
    (ensures (HS.frameOf p1 <> frameOf p2 \/ HS.as_addr p1 =!= as_addr p2)) = ()

let is_mm (#value: typ) (p: pointer value) : GTot bool = HS.aref_is_mm (Pointer?.contents p)
...
let gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = _field p fd

let as_addr_gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let unused_in_gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) (h: HS.mem) = ()

let live_gfield (h: HS.mem) (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let gread_gfield (h: HS.mem) (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let frameOf_gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let is_mm_gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = _ufield p fd

let as_addr_gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let unused_in_gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) (h: HS.mem) = ()

let live_gufield (h: HS.mem) (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let gread_gufield (h: HS.mem) (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let frameOf_gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let is_mm_gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let gcell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = _cell p i

let as_addr_gcell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = ()

let unused_in_gcell
  (#length: array_length_t) (#value: typ) (h: HS.mem) (p: pointer (TArray length value)) i = ()
...
let frameOf_gcell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = ()

let is_mm_gcell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = ()

let includes (#value1: typ) (#value2: typ) (p1: pointer value1) (p2: pointer value2) : GTot bool =
  Pointer?.from p1 = Pointer?.from p2 && HS.aref_equal (Pointer?.contents p1) (Pointer?.contents p2) &&
  path_includes (Pointer?.p p1) (Pointer?.p p2)

let includes_refl (#value: typ) (p: pointer value) = ()

let includes_trans
  (#value1: typ) (#value2: typ) (#value3: typ) (p1: pointer value1) (p2: pointer value2)
  (p3: pointer value3) = path_includes_trans (Pointer?.p p1) (Pointer?.p p2) (Pointer?.p p3)

let includes_gfield (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let includes_gufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let includes_gcell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = ()
...
let includes_ind
  ...
  (h_trans:
    (
        ...
        p2: pointer value2 ->
        p3:
        pointer value3 {includes p1 p2 /\ includes p2 p3 /\ includes p1 p3 /\ x p1 p2 /\ x p2 p3} ->
        Lemma (x p1 p3))) (#value1: typ) (#value2: typ) (p1: pointer value1)
  (p2: pointer value2 {includes p1 p2})
  : Lemma (x p1 p2) =
  let Pointer from contents _ = p1 in
  path_includes_ind (fun #to1 #to2 p1_ p2_ ->
        x (Pointer from contents p1_) (Pointer from contents p2_))
    (fun #through #to p s ->
        match s with
        | StepField l fd ->
          let pt: pointer (TStruct l) = (Pointer from contents p) in
          h_field l pt fd
        | StepUField l fd ->
          let pt: pointer (TUnion l) = (Pointer from contents p) in
          h_ufield l pt fd
        | StepCell length value i ->
          let pt: pointer (TArray length value) = (Pointer from contents p) in
          h_cell pt i)
    (fun #to p -> h_refl (Pointer from contents p))
    (fun #to1 #to2 #to3 p1_ p2_ p3_ ->
        h_trans (Pointer from contents p1_) (Pointer from contents p2_) (Pointer from contents p3_))
    (Pointer?.p p1)
    (Pointer?.p p2)
...
let readable (#a: typ) (h: HS.mem) (b: pointer a) : GTot Type0 =
  // necessary to somehow remove the `logic` qualifier
  let () = () in
  live h b /\
...
let readable_live (#a: typ) (h: HS.mem) (b: pointer a) = ()

let readable_gfield (#l: struct_typ) (h: HS.mem) (p: pointer (TStruct l)) (fd: struct_field l) = ()

let readable_struct (#l: struct_typ) (h: HS.mem) (p: pointer (TStruct l))
  : Lemma (requires (forall (f: struct_field l). readable h (gfield p f))) (ensures (readable h p)) =
  //  [SMTPat (readable #(TStruct l) h p)] // TODO: dubious pattern, will probably trigger unreplayable hints
  // struct is nonempty
  let dummy_field: struct_field l = fst (List.Tot.hd l.fields) in
  ...
  let v: v: ostruct l {Some? v} = v in
  ovalue_is_readable_struct_intro l v

let readable_struct_forall_mem (#l: struct_typ) (p: pointer (TStruct l))
  : Lemma
  (forall (h: HS.mem). (forall (f: struct_field l). readable h (gfield p f)) ==> readable h p) =
  let f (h: HS.mem)
    : Lemma
      (// FIXME: WHY WHY WHY do we need this explicit annotation?
        requires (forall (f: struct_field l). readable h (gfield p f))) (ensures (readable h p)) =
    readable_struct h p
  in
  Classical.forall_intro (Classical.move_requires f)

let rec readable_struct_fields'
  (#l: struct_typ) (h: HS.mem) (p: pointer (TStruct l)) (s: list string)
  : GTot Type0 (decreases s) =
  match s with
  | [] -> True
  | f :: s' ->
...
let rec readable_struct_fields_elim
  (#l: struct_typ) (h: HS.mem) (p: pointer (TStruct l)) (s: list string)
  : Lemma (requires (readable_struct_fields h p s))
    (ensures
      (forall f.
          (List.Tot.mem f s /\ List.Tot.mem f (List.Tot.map fst l.fields)) ==>
          (let f: struct_field l = f in
            readable h (gfield p f))))
    (decreases s) =
  match s with
...
let readable_array
  (#length: array_length_t) (#value: typ) (h: HS.mem) (p: pointer (TArray length value)) =
  // for Some?
  assert (readable h (gcell p 0ul));
  let content = greference_of p in
  let (| _ , c |) = HS.sel h content in
...
let readable_gufield (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd: struct_field l) = ()


(** The active field of a union *)


let is_active_union_field (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd: struct_field l)
  : GTot Type0 =
  // necessary to somehow remove the `logic` qualifier
  let () = () in
  live h p /\
...
let is_active_union_live (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd: struct_field l) =
  ()

let is_active_union_field_live
  (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd: struct_field l) = ()

let is_active_union_field_eq
  (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd1: struct_field l) (fd2: struct_field l) =
  ()
...
let is_active_union_field_includes_readable
  (#l: union_typ) (h: HS.mem) (p: pointer (TUnion l)) (fd: struct_field l) (#t': typ)
  (p': pointer t') =
  let content = greference_of p in
  let (| _ , c |) = HS.sel h content in
  ...
  let phi (#t': typ) (pt': path from t')
    : Ghost Type0 (requires (path_includes pf pt')) (ensures (fun _ -> True)) =
    (~(path_sel c pt' == none_ovalue t')) ==> is_active_union_field h p fd
  in
  let f (t': typ) (pt': path t t') : Lemma (ensures (phi (path_concat pf pt'))) =
...
let _singleton_buffer_of_pointer (#t: typ) (p: pointer t) : Tot (buffer t) =
  ...
  | _ -> Buffer (BufferRootSingleton p) 0ul 1ul
...
let gbuffer_of_array_pointer (#t: typ) (#length: array_length_t) (p: pointer (TArray length t))
  : GTot (buffer t) = Buffer (BufferRootArray p) 0ul length

let buffer_of_array_pointer (#t: typ) (#length: array_length_t) (p: pointer (TArray length t))
  : HST.Stack (buffer t)
    (requires (fun h -> live h p))
    (ensures (fun h b h' -> h' == h /\ b == gbuffer_of_array_pointer p)) =
  Buffer (BufferRootArray p) 0ul length

let buffer_length (#t: typ) (b: buffer t) : GTot UInt32.t = Buffer?.blength b

let buffer_length_gsingleton_buffer_of_pointer (#t: typ) (p: pointer t)
  : Lemma (requires True)
...
let buffer_length_gbuffer_of_array_pointer
  (#t: typ) (#len: array_length_t) (p: pointer (TArray len t))
  : Lemma (requires True)
    (ensures (buffer_length (gbuffer_of_array_pointer p) == len))
    [SMTPat (buffer_length (gbuffer_of_array_pointer p))] = ()

let buffer_live (#t: typ) (h: HS.mem) (b: buffer t) : GTot Type0 =
  // necessary to somehow remove the `logic` qualifier
  let () = () in
  (match b.broot with
...
let buffer_live_gsingleton_buffer_of_pointer (#t: typ) (p: pointer t) (h: HS.mem)
  : Lemma (ensures (buffer_live h (gsingleton_buffer_of_pointer p) <==> live h p))
    [SMTPat (buffer_live h (gsingleton_buffer_of_pointer p))] = ()

let buffer_live_gbuffer_of_array_pointer
  (#t: typ) (#length: array_length_t) (p: pointer (TArray length t)) (h: HS.mem)
  : Lemma (requires True)
...
let frameOf_buffer (#t: typ) (b: buffer t) : GTot HS.rid =
  match b.broot with
  | BufferRootSingleton p -> frameOf p
  | BufferRootArray #mlen p -> frameOf p

let frameOf_buffer_gsingleton_buffer_of_pointer (#t: typ) (p: pointer t) = ()

let frameOf_buffer_gbuffer_of_array_pointer
  (#t: typ) (#length: array_length_t) (p: pointer (TArray length t)) = ()
...
let gsub_buffer (#t: typ) (b: buffer t) (i: UInt32.t) len =
...
let sub_buffer (#t: typ) (b: buffer t) (i: UInt32.t) len =
  Buffer (Buffer?.broot b) FStar.UInt32.(Buffer?.bidx b +^ i) len

let offset_buffer #t b i = sub_buffer b i (UInt32.sub (Buffer?.blength b) i)

let buffer_length_gsub_buffer (#t: typ) (b: buffer t) (i: UInt32.t) len = ()

let buffer_live_gsub_buffer_equiv (#t: typ) (b: buffer t) (i: UInt32.t) len h = ()

let buffer_live_gsub_buffer_intro (#t: typ) (b: buffer t) (i: UInt32.t) len h = ()

let buffer_unused_in_gsub_buffer #t b i len h = ()

let gsub_buffer_gsub_buffer (#a: typ) (b: buffer a) (i1: UInt32.t) len1 i2 len2 = ()

let gsub_buffer_zero_buffer_length (#a: typ) (b: buffer a) = ()

let buffer_root_as_seq (#t: typ) (h: HS.mem) (b: buffer_root t) : GTot (Seq.seq (type_of_typ t)) =
  match b with
  | BufferRootSingleton p -> Seq.create 1 (gread h p)
  | BufferRootArray p -> gread h p

let length_buffer_root_as_seq (#t: typ) (h: HS.mem) (b: buffer_root t)
  : Lemma (requires True)
    (ensures (Seq.length (buffer_root_as_seq h b) == UInt32.v (buffer_root_length b)))
    [SMTPat (Seq.length (buffer_root_as_seq h b))] = ()

let buffer_as_seq (#t: typ) (h: HS.mem) (b: buffer t) : GTot (Seq.seq (type_of_typ t)) =
  let i = UInt32.v (Buffer?.bidx b) in
  Seq.slice (buffer_root_as_seq h (Buffer?.broot b)) i (i + UInt32.v (Buffer?.blength b))

let buffer_length_buffer_as_seq (#t: typ) (h: HS.mem) (b: buffer t) = ()
...
let buffer_as_seq_gsingleton_buffer_of_pointer #t h p =
  ...
  | _ -> Seq.slice_length (Seq.create 1 (gread h p))

let buffer_as_seq_gbuffer_of_array_pointer
  (#length: array_length_t) (#t: typ) (h: HS.mem) (p: pointer (TArray length t)) =
  let s: array length (type_of_typ t) = gread h p in
  Seq.slice_length s

let buffer_as_seq_gsub_buffer (#t: typ) (h: HS.mem) (b: buffer t) (i: UInt32.t) len =
  Seq.slice_slice (buffer_root_as_seq h (Buffer?.broot b))
    (UInt32.v (Buffer?.bidx b))
    (UInt32.v (Buffer?.bidx b) + UInt32.v (Buffer?.blength b))
    (UInt32.v i)
    (UInt32.v i + UInt32.v len)

let gpointer_of_buffer_cell (#t: typ) (b: buffer t) i =
  match Buffer?.broot b with
  | BufferRootSingleton p -> p
  | BufferRootArray p -> gcell p FStar.UInt32.(Buffer?.bidx b +^ i)

let pointer_of_buffer_cell (#t: typ) (b: buffer t) i =
  match Buffer?.broot b with
  | BufferRootSingleton p -> p
  | BufferRootArray p -> _cell p FStar.UInt32.(Buffer?.bidx b +^ i)

let gpointer_of_buffer_cell_gsub_buffer (#t: typ) (b: buffer t) i1 len i2 = ()

let live_gpointer_of_buffer_cell (#t: typ) (b: buffer t) i h = ()

#set-options "--initial_ifuel 2 --max_ifuel 2"
let gpointer_of_buffer_cell_gsingleton_buffer_of_pointer (#t: typ) (p: pointer t) i = ()

#set-options "--initial_ifuel 1 --max_ifuel 1"
let gpointer_of_buffer_cell_gbuffer_of_array_pointer
  (#length: array_length_t) (#t: typ) (p: pointer (TArray length t)) i = ()

let frameOf_gpointer_of_buffer_cell #t b i = ()

let as_addr_gpointer_of_buffer_cell #t b i = ()

let gread_gpointer_of_buffer_cell (#t: typ) (h: HS.mem) (b: buffer t) i = ()

let gread_gpointer_of_buffer_cell' (#t: typ) (h: HS.mem) (b: buffer t) i = ()

let index_buffer_as_seq (#t: typ) (h: HS.mem) (b: buffer t) i = ()
...
let buffer_readable' (#t: typ) (h: HS.mem) (b: buffer t) : GTot Type0 =
  buffer_live h b /\
  (forall (i: UInt32.t).
      UInt32.v i < UInt32.v (buffer_length b) ==> readable h (gpointer_of_buffer_cell b i))

let buffer_readable (#t: typ) (h: HS.mem) (b: buffer t) : GTot Type0 = buffer_readable' h b

let buffer_readable_buffer_live (#t: typ) (h: HS.mem) (b: buffer t) = ()

let buffer_readable_gsingleton_buffer_of_pointer (#t: typ) (h: HS.mem) (p: pointer t) =
  let phi ()
    : Lemma (requires (buffer_readable h (gsingleton_buffer_of_pointer p))) (ensures (readable h p))
  =
...
let buffer_readable_gbuffer_of_array_pointer
  (#len: array_length_t) (#t: typ) (h: HS.mem) (p: pointer (TArray len t)) =
  let phi ()
    : Lemma (requires (buffer_readable h (gbuffer_of_array_pointer p))) (ensures (readable h p)) =
    let psi (i: UInt32.t{UInt32.v i < UInt32.v len}) : Lemma (readable h (gcell p i)) =
...
let buffer_readable_gsub_buffer (#t: typ) (h: HS.mem) (b: buffer t) (i: UInt32.t) len =
  Classical.forall_intro (Classical.move_requires (gpointer_of_buffer_cell_gsub_buffer b i len))

let readable_gpointer_of_buffer_cell (#t: typ) (h: HS.mem) (b: buffer t) i = ()

let buffer_readable_intro (#t: typ) (h: HS.mem) (b: buffer t) = ()
...
let disjoint (#value1: typ) (#value2: typ) (p1: pointer value1) (p2: pointer value2) : GTot Type0 =
  if frameOf p1 = frameOf p2 && as_addr p1 = as_addr p2
  then
    Pointer?.from p1 == Pointer?.from p2 /\ Pointer?.contents p1 == Pointer?.contents p2 /\
    path_disjoint (Pointer?.p p1) (Pointer?.p p2)
  else True

let disjoint_root (#value1: typ) (#value2: typ) (p1: pointer value1) (p2: pointer value2)
  : Lemma (requires (frameOf p1 <> frameOf p2 \/ as_addr p1 <> as_addr p2))
    (ensures (disjoint p1 p2)) = ()

let disjoint_gfield
  (#l: struct_typ) (p: pointer (TStruct l)) (fd1: struct_field l) (fd2: struct_field l)
  : Lemma (requires (fd1 <> fd2))
    (ensures (disjoint (gfield p fd1) (gfield p fd2)))
    [SMTPat (disjoint (gfield p fd1) (gfield p fd2))] = ()

let disjoint_gcell
  (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) (i1: UInt32.t)
  (i2: UInt32.t)
  : Lemma
    (requires
      (UInt32.v i1 < UInt32.v length /\ UInt32.v i2 < UInt32.v length /\ UInt32.v i1 <> UInt32.v i2)
    )
    (ensures
      (UInt32.v i1 < UInt32.v length /\ UInt32.v i2 < UInt32.v length /\
        disjoint (gcell p i1) (gcell p i2)))
    [SMTPat (disjoint (gcell p i1) (gcell p i2))] = ()

let disjoint_includes
  (#value1: typ) (#value2: typ) (p1: pointer value1) (p2: pointer value2) (#value1': typ)
  (#value2': typ) (p1': pointer value1') (p2': pointer value2')
  : Lemma (requires (includes p1 p1' /\ includes p2 p2' /\ disjoint p1 p2))
    (ensures (disjoint p1' p2')) =
  if frameOf p1 = frameOf p2 && as_addr p1 = as_addr p2
  then path_disjoint_includes (Pointer?.p p1) (Pointer?.p p2) (Pointer?.p p1') (Pointer?.p p2')
...
let disjoint_ind
  ...
  (h_includes:
    (
        ...
        Lemma (x p1' p2'))) (#value1: typ) (#value2: typ) (p1: pointer value1)
  ...
  then
    ...
    path_disjoint_ind (fun #v1 #v2 p1_ p2_ ->
      ...
      (fun #v1 #v2 p1_ p2_ #v1' #v2' p1' p2' ->
          h_includes (Pointer from contents p1_)
            (Pointer from contents p2_)
            (Pointer from contents p1')
            (Pointer from contents p2'))
      (Pointer?.p p1)
      (Pointer?.p p2);
    assert (x p1 p2)
  else h_root p1 p2

let disjoint_sym (#value1: typ) (#value2: typ) (p1: pointer value1) (p2: pointer value2)
  : Lemma (requires (disjoint p1 p2)) (ensures (disjoint p2 p1)) =
...
let disjoint_sym'' (value1: typ) (value2: typ) (p1: pointer value1) (p2: pointer value2)
  : Lemma (ensures (disjoint p1 p2 <==> disjoint p2 p1)) = disjoint_sym' p1 p2

let disjoint_includes_l #a #as #a' (x: pointer a) (subx: pointer as) (y: pointer a')
  : Lemma (requires (includes x subx /\ disjoint x y))
    (ensures (disjoint subx y))
    [SMTPat (disjoint subx y); SMTPat (includes x subx)] = disjoint_includes x y subx y

let disjoint_includes_l_swap #a #as #a' (x: pointer a) (subx: pointer as) (y: pointer a')
  : Lemma (requires (includes x subx /\ disjoint x y))
...
let disjoint_includes_r #t1 #t2 #t3 (p1: pointer t1) (p2: pointer t2) (p3: pointer t3)
...
type loc_aux : Type0 =
  ...
  | LocUnion : l1: loc_aux -> l2: loc_aux -> loc_aux

let rec loc_aux_in_addr (l: loc_aux) (r: HS.rid) (n: nat) : GTot Type0 (decreases l) =
  match l with
  | LocUnion l1 l2 -> loc_aux_in_addr l1 r n /\ loc_aux_in_addr l2 r n
  | LocBuffer b -> frameOf_buffer b == r /\ buffer_as_addr b == n /\ UInt32.v (buffer_length b) > 0
  | LocPointer p -> frameOf p == r /\ as_addr p == n

(* TODO: move to FStar.Set?
   Necessary to handle quantifiers *)
let set_nonempty (#t: eqtype) (s: Set.set t) : GTot Type0 = exists (i: t). Set.mem i s
...
type loc' : Type0 =
  | Loc : 
    ...
    (r: HS.rid -> n: nat ->
        Ghost loc_aux
          (requires (Set.mem r (Ghost.reveal aux_regions) /\ Set.mem n (aux_addrs r)))
          (ensures (fun y -> loc_aux_in_addr y r n))) ->
    loc'

let loc = loc'

let aux_addrs_nonempty (l: loc)
  : Lemma
  (forall r.
      Set.mem r (Ghost.reveal (Loc?.aux_regions l)) ==> (exists i. Set.mem i (Loc?.aux_addrs l r))) =
  let f (r: HS.rid)
    : Lemma (requires (Set.mem r (Ghost.reveal (Loc?.aux_regions l))))
...
let loc_union s1 s2 =
  ...
  then s1
    ...
    let addrs (r: HS.rid)
      : Ghost (Set.set nat) (requires (Set.mem r addr_regions)) (ensures (fun _ -> True)) =
      Set.union (if Set.mem r addr_regions1 then Loc?.addrs s1 r else Set.empty)
    ...
    in
    Loc
      (Ghost.hide (Set.union (Ghost.reveal (Loc?.whole_regions s1))
              (Ghost.reveal (Loc?.whole_regions s2))))
...
let rec loc_aux_syntactically_includes (s1: loc_aux) (s2: loc_aux) : GTot Type0 (decreases s1) =
  s1 == s2 \/
  (match s1 with
    | LocUnion sl sr -> loc_aux_syntactically_includes sl s2 \/ loc_aux_syntactically_includes sr s2
    | _ -> False)

let loc_aux_syntactically_includes_refl (s: loc_aux) : Lemma (loc_aux_syntactically_includes s s) =
  ()

let rec loc_aux_syntactically_includes_trans (s1: loc_aux) (s2: loc_aux) (s3: loc_aux)
  : Lemma (requires (loc_aux_syntactically_includes s1 s2 /\ loc_aux_syntactically_includes s2 s3))
    (ensures (loc_aux_syntactically_includes s1 s3))
    (decreases s1) =
  match s1 with
  | LocUnion sl sr ->
    Classical.or_elim #(loc_aux_syntactically_includes sl s2)
...
let loc_aux_syntactically_includes_union_r (s1: loc_aux) (s2: loc_aux) (s3: loc_aux)
  : Lemma (requires (loc_aux_syntactically_includes s1 (LocUnion s2 s3)))
...
let buffer_includes_pointer (#t1: typ) (#t2: typ) (b: buffer t1) (p: pointer t2) : GTot Type0 =
  exists (i: UInt32.t).
    UInt32.v i < UInt32.v (buffer_length b) /\ includes (gpointer_of_buffer_cell b i) p

let rec loc_aux_includes_pointer (s: loc_aux) (#t: typ) (p: pointer t) : GTot Type0 =
  match s with
  | LocPointer p' -> includes p' p
  | LocBuffer b -> buffer_includes_pointer b p
  | LocUnion s1 s2 -> loc_aux_includes_pointer s1 p \/ loc_aux_includes_pointer s2 p

let rec loc_aux_includes_pointer_refl_pointer (s: loc_aux) (#t: typ) (p: pointer t)
  : Lemma (requires (loc_aux_syntactically_includes s (LocPointer p)))
    (ensures (loc_aux_includes_pointer s p))
    (decreases s) =
  match s with
  | LocUnion s1 s2 ->
    Classical.or_elim #(loc_aux_syntactically_includes s1 (LocPointer p))
...
let rec loc_aux_includes_pointer_refl_buffer (s: loc_aux) (#t: typ) (b: buffer t) (i: UInt32.t)
  : Lemma
    (requires
      (UInt32.v i < UInt32.v (buffer_length b) /\ loc_aux_syntactically_includes s (LocBuffer b)))
    (ensures
      (UInt32.v i < UInt32.v (buffer_length b) /\
        loc_aux_includes_pointer s (gpointer_of_buffer_cell b i)))
    (decreases s) =
  match s with
  | LocUnion s1 s2 ->
    Classical.or_elim #(loc_aux_syntactically_includes s1 (LocBuffer b))
...
let rec loc_aux_includes_pointer_trans
  (s: loc_aux) (#t1: typ) (#t2: typ) (p1: pointer t1) (p2: pointer t2)
  : Lemma (requires (loc_aux_includes_pointer s p1 /\ includes p1 p2))
    (ensures (loc_aux_includes_pointer s p2)) =
  match s with
  | LocUnion s1 s2 ->
    Classical.or_elim #(loc_aux_includes_pointer s1 p1)
  ...
  | LocBuffer b ->
    let f (i: UInt32.t)
      : Lemma
        (requires
          (UInt32.v i < UInt32.v (buffer_length b) /\ includes (gpointer_of_buffer_cell b i) p1))
        (ensures
          (UInt32.v i < UInt32.v (buffer_length b) /\ includes (gpointer_of_buffer_cell b i) p2)) =
...
let loc_aux_includes_buffer (s: loc_aux) (#t: typ) (b: buffer t) : GTot Type0 =
  forall (i: UInt32.t).
    UInt32.v i < UInt32.v (buffer_length b) ==>
    loc_aux_includes_pointer s (gpointer_of_buffer_cell b i)

let rec loc_aux_includes (s: loc_aux) (s2: loc_aux) : GTot Type0 (decreases s2) =
  match s2 with
  | LocUnion s2l s2r -> loc_aux_includes s s2l /\ loc_aux_includes s s2r
  | LocPointer p -> loc_aux_includes_pointer s p
  | LocBuffer b -> loc_aux_includes_buffer s b

let rec loc_aux_includes_refl (s: loc_aux) (s2: loc_aux)
  : Lemma (requires (loc_aux_syntactically_includes s s2))
  ...
  | LocPointer p -> loc_aux_includes_pointer_refl_pointer s p
  | LocBuffer b ->
    Classical.forall_intro (Classical.move_requires (loc_aux_includes_pointer_refl_buffer s b))

let rec loc_aux_includes_refl' (s: loc_aux) : Lemma (loc_aux_includes s s) =
...
let rec loc_aux_includes_refl'' (s: loc_aux)
  : Lemma (loc_aux_includes s s) [SMTPat (loc_aux_includes s s)] = loc_aux_includes_refl' s

let rec loc_aux_includes_union_l_r (s: loc_aux) (s': loc_aux)
  : Lemma (loc_aux_includes (LocUnion s s') s) = loc_aux_includes_refl (LocUnion s s') s

let rec loc_aux_includes_union_l_l (s: loc_aux) (s': loc_aux)
  : Lemma (loc_aux_includes (LocUnion s' s) s) = loc_aux_includes_refl (LocUnion s' s) s

let rec loc_aux_includes_loc_aux_includes_pointer
  (s1: loc_aux) (s2: loc_aux) (#t: typ) (p: pointer t)
  : Lemma (requires (loc_aux_includes s1 s2 /\ loc_aux_includes_pointer s2 p))
    (ensures (loc_aux_includes_pointer s1 p))
    (decreases s2) =
  match s2 with
  | LocUnion s2l s2r ->
    Classical.or_elim #(loc_aux_includes_pointer s2l p)
  ...
  | LocPointer p' -> loc_aux_includes_pointer_trans s1 p' p
  | LocBuffer b ->
    let f (i: UInt32.t)
      : Lemma
        (requires
...
let rec loc_aux_includes_trans (s1: loc_aux) (s2: loc_aux) (s3: loc_aux)
  : Lemma (requires (loc_aux_includes s1 s2 /\ loc_aux_includes s2 s3))
    (ensures (loc_aux_includes s1 s3))
    (decreases s3) =
  match s3 with
  | LocUnion s3l s3r -> loc_aux_includes_trans s1 s2 s3l; loc_aux_includes_trans s1 s2 s3r
  | LocPointer p -> loc_aux_includes_loc_aux_includes_pointer s1 s2 p
  | LocBuffer b ->
    let f (i: UInt32.t)
      : Lemma (requires (UInt32.v i < UInt32.v (buffer_length b)))
        (ensures
          (UInt32.v i < UInt32.v (buffer_length b) /\
            loc_aux_includes_pointer s1 (gpointer_of_buffer_cell b i))) =
...
let addrs_of_loc_weak (l: loc) (r: HS.rid) : GTot (Set.set nat) =
  if Set.mem r (Ghost.reveal (Loc?.whole_regions l))
  then Set.complement Set.empty
  else if Set.mem r (Ghost.reveal (Loc?.addr_regions l)) then Loc?.addrs l r else Set.empty

let addrs_of_loc_weak_loc_union (l1: loc) (l2: loc) (r: HS.rid)
  : Lemma
    (addrs_of_loc_weak (loc_union l1 l2) r ==
      Set.union (addrs_of_loc_weak l1 r) (addrs_of_loc_weak l2 r))
    [SMTPat (addrs_of_loc_weak (loc_union l1 l2) r)] =
  assert (Set.equal (addrs_of_loc_weak (loc_union l1 l2) r)
        (Set.union (addrs_of_loc_weak l1 r) (addrs_of_loc_weak l2 r)))

let addrs_of_loc (l: loc) (r: HS.rid) : GTot (Set.set nat) =
  Set.union (addrs_of_loc_weak l r)
    (if Set.mem r (Ghost.reveal (Loc?.aux_regions l)) then Loc?.aux_addrs l r else Set.empty)

let addrs_of_loc_union (l1: loc) (l2: loc) (r: HS.rid)
  : Lemma (addrs_of_loc (loc_union l1 l2) r == Set.union (addrs_of_loc l1 r) (addrs_of_loc l2 r))
    [SMTPat (addrs_of_loc (loc_union l1 l2) r)] =
  assert (Set.equal (addrs_of_loc (loc_union l1 l2) r)
        (Set.union (addrs_of_loc l1 r) (addrs_of_loc l2 r)))
...
let loc_includes s1 s2 =
  ...
  (Set.subset whole_regions2 whole_regions1 /\
    ...
    (forall (r: HS.rid). Set.subset (addrs_of_loc_weak s2 r) (addrs_of_loc_weak s1 r)) /\
    (forall (r: HS.rid). Set.subset (addrs_of_loc s2 r) (addrs_of_loc s1 r)) /\
    (forall (r: HS.rid) (n: nat).
        ...
        (Set.mem n (addrs_of_loc_weak s1 r) \/
          (Set.mem r (Ghost.reveal (Loc?.aux_regions s1)) /\ Set.mem n (Loc?.aux_addrs s1 r) /\
            loc_aux_includes (Loc?.aux s1 r n) (Loc?.aux s2 r n)))))

let loc_includes_refl s =
  let pre (r: HS.rid) (n: nat) : GTot Type0 =
    Set.mem r (Ghost.reveal (Loc?.aux_regions s)) /\ Set.mem n (Loc?.aux_addrs s r)
  in
  let post (r: HS.rid) (n: nat) : GTot Type0 =
    pre r n /\ loc_aux_includes (Loc?.aux s r n) (Loc?.aux s r n)
  in
  let f (r: HS.rid) (n: nat) : Lemma (requires (pre r n)) (ensures (post r n)) =
    loc_aux_includes_refl (Loc?.aux s r n) (Loc?.aux s r n)
  in
  Classical.forall_intro_2 (fun r -> Classical.move_requires (f r))

let loc_includes_trans s1 s2 s3 = Classical.forall_intro_3 loc_aux_includes_trans'
...
let loc_includes_union_l s1 s2 s =
  ...
  then ()
    ...
    (Classical.forall_intro loc_aux_includes_refl';
      ...
      Classical.or_elim #(loc_includes s1 s)
        #(loc_includes s2 s)
...
let loc_includes_gsub_buffer_r l #t b i len =
  if len = 0ul
  then loc_includes_none l
  else
    let g = gsub_buffer b i len in
    let f () : Lemma (loc_aux_includes (LocBuffer b) (LocBuffer g)) =
      let pre (j: UInt32.t) : GTot Type0 = UInt32.v j < UInt32.v len in
      let post (j: UInt32.t) : GTot Type0 =
        pre j /\ loc_aux_includes_pointer (LocBuffer b) (gpointer_of_buffer_cell g j)
      in
      let k (j: UInt32.t) : Lemma (requires (pre j)) (ensures (post j)) =
...
let loc_includes_gsub_buffer_l #t b i1 len1 i2 len2 =
  ...
  if len2 = 0ul
  then loc_includes_none (loc_buffer g1)
  else
    let pre (j: UInt32.t) : GTot Type0 = UInt32.v j < UInt32.v (buffer_length g2) in
    let post (j: UInt32.t) : GTot Type0 =
      pre j /\ buffer_includes_pointer g1 (gpointer_of_buffer_cell g2 j)
    in
    let f (j: UInt32.t) : Lemma (requires (pre j)) (ensures (post j)) =
      assert (includes (gpointer_of_buffer_cell g1 (FStar.UInt32.add (FStar.UInt32.sub i2 i1) j))
            (gpointer_of_buffer_cell g2 j))
    in
    Classical.forall_intro (Classical.move_requires f);
...
let disjoint_buffer_vs_pointer (#t1: typ) (#t2: typ) (b: buffer t1) (p: pointer t2) : GTot Type0 =
  forall (i: UInt32.t).
    UInt32.v i < UInt32.v (buffer_length b) ==> disjoint (gpointer_of_buffer_cell b i) p

let rec loc_aux_disjoint_pointer (l: loc_aux) (#t: typ) (p: pointer t) : GTot Type0 (decreases l) =
  match l with
  | LocUnion ll lr -> loc_aux_disjoint_pointer ll p /\ loc_aux_disjoint_pointer lr p
  | LocPointer p' -> disjoint p' p
  | LocBuffer b -> disjoint_buffer_vs_pointer b p

let loc_aux_disjoint_buffer (l: loc_aux) (#t: typ) (b: buffer t) : GTot Type0 =
  forall (i: UInt32.t).
    UInt32.v i < UInt32.v (buffer_length b) ==>
    loc_aux_disjoint_pointer l (gpointer_of_buffer_cell b i)

let loc_aux_disjoint_buffer_sym (#t1: typ) (#t2: typ) (b1: buffer t1) (b2: buffer t2)
  : Lemma (loc_aux_disjoint_buffer (LocBuffer b1) b2 <==> loc_aux_disjoint_buffer (LocBuffer b2) b1) =
  Classical.forall_intro_2 (disjoint_sym'' t1 t2)

let loc_aux_disjoint_pointer_buffer_sym (#t1: typ) (#t2: typ) (b1: buffer t1) (p2: pointer t2)
  : Lemma
  (loc_aux_disjoint_pointer (LocBuffer b1) p2 <==> loc_aux_disjoint_buffer (LocPointer p2) b1) =
  Classical.forall_intro_2 (disjoint_sym'' t1 t2)

let rec loc_aux_disjoint (l1: loc_aux) (l2: loc_aux) : GTot Type0 (decreases l2) =
  match l2 with
  | LocUnion ll lr -> loc_aux_disjoint l1 ll /\ loc_aux_disjoint l1 lr
  | LocPointer p -> loc_aux_disjoint_pointer l1 p
  | LocBuffer b -> loc_aux_disjoint_buffer l1 b

let rec loc_aux_disjoint_union_l (ll1: loc_aux) (lr1: loc_aux) (l2: loc_aux)
  : Lemma
    (ensures
      (loc_aux_disjoint (LocUnion ll1 lr1) l2 <==>
        (loc_aux_disjoint ll1 l2 /\ loc_aux_disjoint lr1 l2))) (decreases l2) =
  match l2 with
  | LocUnion ll2 lr2 -> loc_aux_disjoint_union_l ll1 lr1 ll2; loc_aux_disjoint_union_l ll1 lr1 lr2
  | _ -> ()

let loc_aux_disjoint_union_r (l1: loc_aux) (ll2: loc_aux) (lr2: loc_aux)
  : Lemma
  (loc_aux_disjoint l1 (LocUnion ll2 lr2) <==> (loc_aux_disjoint l1 ll2 /\ loc_aux_disjoint l1 lr2)) =
  ()

let rec loc_aux_size (l: loc_aux) : GTot nat =
  match l with
  | LocUnion l1 l2 ->
...
let rec loc_aux_disjoint_sym (l1: loc_aux) (l2: loc_aux)
...
let loc_aux_disjoint_sym' (l1: loc_aux) (l2: loc_aux)
  : Lemma (loc_aux_disjoint l1 l2 <==> loc_aux_disjoint l2 l1) = loc_aux_disjoint_sym l1 l2

let regions_of_loc (s: loc) : GTot (Set.set HS.rid) =
  Set.union (Ghost.reveal (Loc?.whole_regions s))
    (Set.union (Ghost.reveal (Loc?.addr_regions s)) (Ghost.reveal (Loc?.aux_regions s)))

let regions_of_loc_loc_union (s1: loc) (s2: loc)
  : Lemma (regions_of_loc (loc_union s1 s2) == Set.union (regions_of_loc s1) (regions_of_loc s2))
    [SMTPat (regions_of_loc (loc_union s1 s2))] =
  assert (Set.equal (regions_of_loc (loc_union s1 s2))
        (Set.union (regions_of_loc s1) (regions_of_loc s2)))

let regions_of_loc_monotonic (s1: loc) (s2: loc)
  : Lemma (requires (loc_includes s1 s2))
    (ensures (Set.subset (regions_of_loc s2) (regions_of_loc s1))) = ()

let loc_disjoint' (l1: loc) (l2: loc) : GTot Type0 =
...
let loc_disjoint_sym l1 l2 = Classical.forall_intro_2 loc_aux_disjoint_sym'

let loc_disjoint_none_r s = ()

let loc_disjoint_union_r s s1 s2 = ()

let loc_disjoint_root #value1 #value2 p1 p2 = disjoint_root p1 p2
...
let rec loc_aux_disjoint_pointer_includes
  (l: loc_aux) (#t1: typ) (p1: pointer t1) (#t2: typ) (p2: pointer t2)
  : Lemma (requires (loc_aux_disjoint_pointer l p1 /\ includes p1 p2))
...
let rec loc_aux_disjoint_loc_aux_includes_pointer
  ...
  | LocPointer p2 -> loc_aux_disjoint_pointer_includes l1 p2 p3
  | LocBuffer b2 ->
    let f (i: UInt32.t)
      : Lemma
        (requires
          (UInt32.v i < UInt32.v (buffer_length b2) /\ includes (gpointer_of_buffer_cell b2 i) p3))
        (ensures (loc_aux_disjoint_pointer l1 p3)) =
      loc_aux_disjoint_pointer_includes l1 (gpointer_of_buffer_cell b2 i) p3
    in
    Classical.forall_intro (Classical.move_requires f)

let rec loc_aux_disjoint_loc_aux_includes (l1: loc_aux) (l2: loc_aux) (l3: loc_aux)
  ...
  | LocPointer p3 -> loc_aux_disjoint_loc_aux_includes_pointer l1 l2 p3
  | LocBuffer b3 ->
    let f (i: UInt32.t)
      : Lemma (requires (UInt32.v i < UInt32.v (buffer_length b3)))
        (ensures
...
let loc_disjoint_includes p1 p2 p1' p2' =
  ...
  //A rather brutal way to force inlining of pre and post in VC of the continuation
  let pre =
    (fun r n ->
  ...
  in
  let post =
    (fun r n -> pre r n /\ loc_aux_disjoint (Loc?.aux p1' r n) (Loc?.aux p2' r n))
    <:
    Tot (r: HS.rid -> n: nat -> GTot Type0)
  in
  let f (r: HS.rid) (n: nat) : Lemma (requires (pre r n)) (ensures (post r n)) =
...
let modifies' (s: loc) (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  HS.modifies (regions_of_loc s) h1 h2 /\
  (forall r.
      (HS.live_region h1 r /\ Set.mem r (regions_of_loc s) /\
        (~(Set.mem r (Ghost.reveal (Loc?.whole_regions s))))) ==>
      (HS.live_region h2 r /\ HS.modifies_ref r (addrs_of_loc s r) h1 h2)) /\
...
let modifies_loc_addresses_intro_weak
  (r: HS.rid) (s: Set.set nat) (l: loc) (h1: HS.mem) (h2: HS.mem)
  : Lemma
    (requires
      (modifies (loc_union (loc_regions (Set.singleton r)) l) h1 h2 /\ HS.modifies_ref r s h1 h2 /\
        HS.live_region h1 r /\ loc_disjoint l (loc_regions (Set.singleton r))))
...
let modifies_pointer_elim s h1 h2 #a' p' =
  ...
  let r = frameOf p' in
  let a = as_addr p' in
  if Set.mem r (Ghost.reveal (Loc?.aux_regions s)) && Set.mem a (Loc?.aux_addrs s r)
  then ()
  else
    let r = greference_of p' in
...
val modifies_buffer_elim': #t1: typ -> b: buffer t1 -> p: loc -> h: HS.mem -> h': HS.mem ->
  Lemma
    (requires
      (loc_disjoint (loc_buffer b) p /\ buffer_live h b /\ UInt32.v (buffer_length b) > 0 /\
        modifies p h h'))
    (ensures
      (buffer_live h' b /\
        (buffer_readable h b ==> (buffer_readable h' b /\ buffer_as_seq h b == buffer_as_seq h' b)))
    )
...
let modifies_buffer_elim' #t1 b p h h' =
  ...
  assert (n > 0);
  let pre (i: UInt32.t) : GTot Type0 = UInt32.v i < n in
  let post (i: UInt32.t) : GTot Type0 =
    pre i /\
    (let q = gpointer_of_buffer_cell b i in
      equal_values h q h' q)
  in
  let f (i: UInt32.t) : Lemma (requires (pre i)) (ensures (post i)) =
    modifies_pointer_elim p h h' (gpointer_of_buffer_cell b i)
  in
  // for the liveness of the whole buffer
  f 0ul;
  Classical.forall_intro (Classical.move_requires f);
  assert (buffer_readable h b ==> buffer_readable h' b);
  let g ()
    : Lemma (requires (buffer_readable h b)) (ensures (buffer_as_seq h b == buffer_as_seq h' b)) =
...
let modifies_buffer_elim #t1 b p h h' =
  if buffer_length b = 0ul then () else modifies_buffer_elim' b p h h'
...
let modifies_loc_includes s1 h h' s2 =
  admit ();
  assert_spinoff (forall rs r.
        (HS.modifies rs h h' /\ HS.live_region h r /\ (~(Set.mem r rs))) ==>
  ...
  assert_spinoff (forall r.
        (HS.live_region h1 r /\ Set.mem r (regions_of_loc s1) /\
          (~(Set.mem r (Ghost.reveal (Loc?.whole_regions s1))))) ==>
        (HS.live_region h2 r /\ HS.modifies_ref r (addrs_of_loc s1 r) h1 h2));
  let f (t: typ) (p: pointer t)
    : Lemma
      (requires
        (Set.mem (frameOf p) (Ghost.reveal (Loc?.aux_regions s1)) /\
          ...
          loc_aux_disjoint_pointer (Loc?.aux s1 (frameOf p) (as_addr p)) p /\ live h1 p))
      (ensures (equal_values h1 p h2 p)) =
  ...
  in
  //AR: this was the same pattern as above (forall_intro_2 and move_requires, there was no g)
  Classical.forall_intro_2 g

#set-options "--z3rlimit 100 --max_fuel 1 --max_ifuel 1"
let modifies_only_live_regions_weak (rs: Set.set HS.rid) (l: loc) (h: HS.mem) (h': HS.mem)
  : Lemma
    (requires
      (modifies (loc_union (loc_regions rs) l) h h' /\ loc_disjoint (loc_regions rs) l /\
...
let modifies_trans' (s: loc) (h1: HS.mem) (h2: HS.mem) (h3: HS.mem)
  : Lemma (requires (modifies s h1 h2 /\ modifies s h2 h3)) (ensures (modifies s h1 h3)) = ()
...
let screate (value: typ) (s: option (type_of_typ value)) =
  let h0 = HST.get () in
  let s =
    match s with
    ...
    | _ -> none_ovalue value
  in
  let content: HS.reference pointer_ref_contents = HST.salloc (| value, s |) in
  ...
  let f ()
    : Lemma
    (let gref =
        HS.greference_of aref pointer_ref_contents (Heap.trivial_preorder pointer_ref_contents)
      in
...
let domain_upd (#a: Type) (h: HS.mem) (x: HS.reference a {HS.live_region h (HS.frameOf x)}) (v: a)
  : Lemma (requires True) (ensures (Map.domain h.HS.h == Map.domain (HS.upd h x v).HS.h)) =
  let m = h.HS.h in
...
let ecreate (t: typ) (r: HS.rid) (s: option (type_of_typ t)) =
  let h0 = HST.get () in
  let s =
    match s with
    ...
    | _ -> none_ovalue t
  in
  let content: HS.reference pointer_ref_contents = HST.ralloc r (| t, s |) in
...
let field (#l: struct_typ) (p: pointer (TStruct l)) (fd: struct_field l) = _field p fd

let ufield (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) = _ufield p fd

let cell (#length: array_length_t) (#value: typ) (p: pointer (TArray length value)) i = _cell p i

let reference_of (#value: typ) (h: HS.mem) (p: pointer value)
  : Pure (HS.reference pointer_ref_contents)
    ...
    (ensures
      (fun x ->
          ...
          (forall h'. HS.contains h' x <==> HS.contains h' (greference_of p)) /\
          (forall h'.
              (HS.contains h' x \/ HS.contains h' (greference_of p)) ==>
              (HS.contains h' x /\ HS.contains h' (greference_of p) /\
                HS.sel h' x == HS.sel h' (greference_of p))) /\
          (forall h' z.
              (HS.contains h' x \/ HS.contains h' (greference_of p)) ==>
              (HS.contains h' x /\ HS.contains h' (greference_of p) /\
                HS.upd h' x z == HS.upd h' (greference_of p) z)))) =
  let x =
    HS.reference_of h
      (Pointer?.contents p)
      pointer_ref_contents
  ...
  let f (h': HS.mem)
    : Lemma
    (// necessary to typecheck Classical.forall_intro
      (exists h'. live h' p) /\ (HS.contains h' x <==> HS.contains h' (greference_of p)) /\
      ((HS.contains h' x \/ HS.contains h' (greference_of p)) ==>
        HS.sel h' x == HS.sel h' (greference_of p))) =
  ...
  in
  let g (z: pointer_ref_contents) (h': HS.mem)
    : Lemma
    ((exists h'. live h' p) /\
      ((HS.contains h' x \/ HS.contains h' (greference_of p)) ==>
        (HS.contains h' x /\ HS.contains h' (greference_of p) /\
...
let read (#value: typ) (p: pointer value) =
  let h = HST.get () in
...
let is_null (#t: typ) (p: npointer t) =
  match p with
  | NullPtr -> true
  | _ -> false

#set-options "--z3rlimit 100"
let owrite (#a: typ) (b: pointer a) (z: otype_of_typ a)
  : HST.Stack unit
  ...
  let e ()
    ...
    (let gref = greference_of b in
      (HS.frameOf r == HS.frameOf gref /\ HS.as_addr r == HS.as_addr gref /\ HS.sel h0 gref == v0 /\
        HS.sel h1 gref == v1)) =
    let gref = greference_of b in
  ...
  let f (t: typ) (p: pointer t)
    : Lemma
      (requires (frameOf p == frameOf b /\ as_addr p == as_addr b /\ live h0 p /\ disjoint b p))
...
let write #a b z = owrite b (ovalue_of_value a z)

let write_union_field (#l: union_typ) (p: pointer (TUnion l)) (fd: struct_field l) =
  ...
  let vu: option (gtdata (struct_field l) (type_of_struct_field' l otype_of_typ)) =
    Some (gtdata_create fd (none_ovalue field_t))
  in
...
let hs_modifies_just_fresh_frame_popped
  (h0: HS.mem) (h1: HS.mem) (s: Set.set HS.rid) (h2: HS.mem) (h3: HS.mem)
  : Lemma
    (requires
      (HS.fresh_frame h0 h1 /\ HS.modifies s h1 h2 /\ h2.HS.tip == h1.HS.tip /\ HS.popped h2 h3))
    (ensures
...
let hs_modifies_rref_fresh_frame_popped
  (h0: HS.mem) (h1: HS.mem) (h2: HS.mem) (h3: HS.mem) (r: HS.rid) (a: Set.set nat)
  : Lemma
    (requires
      (HS.fresh_frame h0 h1 /\ (~(Set.mem r (HS.mod_set (Set.singleton h1.HS.tip)))) /\
        HS.live_region h0 r /\ Set.subset (Map.domain h1.HS.h) (Map.domain h2.HS.h) /\
        HS.modifies_ref r a h1 h2 /\ h2.HS.tip == h1.HS.tip /\ HS.popped h2 h3))
    (ensures (HS.live_region h3 r /\ HS.modifies_ref r a h0 h3)) = ()

let modifies_fresh_frame_popped_weak (h0: HS.mem) (h1: HS.mem) (s: loc) (h2: HS.mem) (h3: HS.mem)
  : Lemma
    (requires
      (HS.fresh_frame h0 h1 /\
        modifies (loc_union (loc_regions (HS.mod_set (Set.singleton h1.HS.tip))) s) h1 h2 /\
        h2.HS.tip == h1.HS.tip /\ HS.popped h2 h3 /\
        loc_disjoint (loc_regions (HS.mod_set (Set.singleton h1.HS.tip))) s))
    (ensures (modifies s h0 h3 /\ h3.HS.tip == h0.HS.tip)) =
  ...
  let f (r: HS.rid)
    : Lemma
      (requires
        (HS.live_region h0 r /\ Set.mem r (regions_of_loc s) /\
          (~(Set.mem r (Ghost.reveal (Loc?.whole_regions s))))))
      (ensures (HS.live_region h3 r /\ HS.modifies_ref r (addrs_of_loc s r) h0 h3)) =
    ...
    : Lemma
      (requires
        (Set.mem (frameOf p) (Ghost.reveal (Loc?.aux_regions s)) /\
          Set.mem (as_addr p) (Loc?.aux_addrs s (frameOf p)) /\
          (~(Set.mem (as_addr p) (addrs_of_loc_weak s (frameOf p)))) /\
          loc_aux_disjoint_pointer (Loc?.aux s (frameOf p) (as_addr p)) p /\ live h0 p))
      (ensures (equal_values h0 p h3 p)) =
    let r = frameOf p in
    let a = as_addr p in
...
let restrict_to_regions (l: loc) (rs: Set.set HS.rid) : GTot loc =
  let Loc whole_regions addr_regions addrs aux_regions aux_addrs aux = l in
  Loc (Ghost.hide (Set.intersect (Ghost.reveal whole_regions) rs))
...
let regions_of_loc_restrict_to_regions (l: loc) (rs: Set.set HS.rid)
  ...
  assert (Set.equal (regions_of_loc (restrict_to_regions l rs))
        (Set.intersect (regions_of_loc l) rs))

let addrs_of_loc_weak_restrict_to_regions (l: loc) (rs: Set.set HS.rid) (r: HS.rid)
  : Lemma
    (addrs_of_loc_weak (restrict_to_regions l rs) r ==
      (if Set.mem r rs then addrs_of_loc_weak l r else Set.empty))
    [SMTPat (addrs_of_loc_weak (restrict_to_regions l rs) r)] =
  assert (Set.equal (addrs_of_loc_weak (restrict_to_regions l rs) r)
        (if Set.mem r rs then addrs_of_loc_weak l r else Set.empty))

let addrs_of_loc_restrict_to_regions (l: loc) (rs: Set.set HS.rid) (r: HS.rid)
  : Lemma
    (addrs_of_loc (restrict_to_regions l rs) r ==
      (if Set.mem r rs then addrs_of_loc l r else Set.empty))
    [SMTPat (addrs_of_loc (restrict_to_regions l rs) r)] =
  assert (Set.equal (addrs_of_loc (restrict_to_regions l rs) r)
        (if Set.mem r rs then addrs_of_loc l r else Set.empty))

let loc_includes_restrict_to_regions (l: loc) (rs: Set.set HS.rid)
  : Lemma (loc_includes l (restrict_to_regions l rs)) =
  Classical.forall_intro loc_aux_includes_refl'

let loc_includes_loc_union_restrict_to_regions (l: loc) (rs: Set.set HS.rid)
  : Lemma
  (loc_includes (loc_union (restrict_to_regions l rs) (restrict_to_regions l (Set.complement rs))) l
  ) = Classical.forall_intro loc_aux_includes_refl'

let loc_includes_loc_regions_restrict_to_regions (l: loc) (rs: Set.set HS.rid)
  : Lemma (loc_includes (loc_regions rs) (restrict_to_regions l rs)) =
  Classical.forall_intro loc_aux_includes_refl'
...
let modifies_loc_addresses_intro r a l h1 h2 =
  ...
  then
    (loc_includes_loc_regions_restrict_to_regions s rs;
      ...
      modifies_loc_includes (loc_union (loc_addresses r a) l)
        h1
        h2
        (loc_union (loc_addresses r a) s_c_rs))
  else
...
let modifies_1_readable_struct #l f p h h' = readable_struct h' p

let modifies_1_readable_array #t #len i p h h' = readable_array h' p

(* buffer read: can be defined as a derived operation: pointer_of_buffer_cell ; read *)

let read_buffer (#t: typ) (b: buffer t) i = read (pointer_of_buffer_cell b i)

let write_buffer (#t: typ) (b: buffer t) i v = write (pointer_of_buffer_cell b i) v
...
let buffer_idx #t b = Buffer?.bidx b

let buffer_eq_gsub_root #t b = assert (UInt32.add 0ul (buffer_idx b) == buffer_idx b)
...
let buffer_includes #t blarge bsmall =
  ...
  (root_buffer blarge == root_buffer bsmall /\
    ...
    UInt32.v (buffer_idx bsmall) + UInt32.v (buffer_length bsmall) <=
    UInt32.v (buffer_idx blarge) + UInt32.v (buffer_length blarge))
...
let buffer_includes_loc_includes #t b1 b2 =
  ...
  loc_includes_gsub_buffer_r (loc_buffer b1)
    b1
    (UInt32.sub (buffer_idx b2) (buffer_idx b1))
    (buffer_length b2)
