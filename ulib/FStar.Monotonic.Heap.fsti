...
val mref (a:Type0) (inv:data_inv a) (rel:preorder a):Type0

val addr_of: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> mref a inv rel -> GTot nat

val is_mm: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> mref a inv rel -> GTot bool

val compare_addrs:
  #a:Type0 -> #b:Type0 -> (#inv1:data_inv a) -> (#inv2:data_inv b) -> #rel1:preorder a -> #rel2:preorder b ->
  r1:mref a inv1 rel1 -> r2:mref b inv2 rel2 -> Tot (b:bool{b = (addr_of r1 = addr_of r2)})

val contains: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> heap -> mref a inv rel -> Type0

val addr_unused_in: nat -> heap -> Type0

val unused_in: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> mref a inv rel -> heap -> Type0

let fresh (#a:Type) (#inv:data_inv a) (#rel:preorder a) (r:mref a inv rel) (h0:heap) (h1:heap) =
  r `unused_in` h0 /\ h1 `contains` r

let only_t (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (x:mref a inv rel) :GTot (tset nat) = TS.singleton (addr_of x)

let only (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (x:mref a inv rel) :GTot (set nat) = S.singleton (addr_of x)

let op_Hat_Plus_Plus (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (r:mref a inv rel) (s:set nat) :GTot (set nat) = S.union (only r) s

let op_Plus_Plus_Hat (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (s:set nat) (r:mref a inv rel) :GTot (set nat) = S.union s (only r)

let op_Hat_Plus_Hat (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (#rel1:preorder a) (#rel2:preorder b) (r1:mref a inv1 rel1) (r2:mref b inv2 rel2)
  :GTot (set nat) = S.union (only r1) (only r2)

val sel_tot: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> h:heap -> r:mref a inv rel{h `contains` r} -> Tot a
...
val upd: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> h:heap -> r:mref a inv rel -> x:a{valid_upd h r x} -> GTot heap

val alloc: #a:Type0 -> inv:data_inv a -> rel:preorder a -> heap -> a -> mm:bool -> Tot (mref a inv rel * heap)

val free_mm: #a:Type0 -> #inv:data_inv a -> #rel:preorder a -> h:heap -> r:mref a inv rel{h `contains` r /\ is_mm r} -> GTot heap

let modifies_t (s:tset nat) (h0:heap) (h1:heap) =
  (forall (a:Type) (inv:data_inv a) (rel:preorder a) (r:mref a inv rel).{:pattern (sel h1 r)}
                               ((~ (TS.mem (addr_of r) s)) /\ h0 `contains` r) ==> sel h1 r == sel h0 r) /\
  (forall (a:Type) (inv:data_inv a) (rel:preorder a) (r:mref a inv rel).{:pattern (contains h1 r)}
                               h0 `contains` r ==> h1 `contains` r) /\
  (forall (a:Type) (inv:data_inv a) (rel:preorder a) (r:mref a inv rel).{:pattern (r `unused_in` h0)}
...
let equal_dom (h1:heap) (h2:heap) :GTot Type0 =
  (forall (a:Type0) (inv:data_inv a)(rel:preorder a) (r:mref a inv rel). h1 `contains` r <==> h2 `contains` r) /\
  (forall (a:Type0) (inv:data_inv a)(rel:preorder a) (r:mref a inv rel). r `unused_in` h1 <==> r `unused_in` h2)

val lemma_ref_unused_iff_addr_unused (#a:Type0) (#inv:data_inv a)(#rel:preorder a) (h:heap) (r:mref a inv rel)
  :Lemma (requires True)
         (ensures  (r `unused_in` h <==> addr_of r `addr_unused_in` h))
	 [SMTPatOr [[SMTPat (r `unused_in` h)]; [SMTPat (addr_of r `addr_unused_in` h)]]]

val lemma_contains_implies_used (#a:Type0) (#inv:data_inv a)(#rel:preorder a) (h:heap) (r:mref a inv rel)
...
val lemma_distinct_addrs_distinct_types
  (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (#rel1:preorder a) (#rel2:preorder b) (h:heap) (r1:mref a inv1 rel1) (r2:mref b inv2 rel2)
...
val lemma_distinct_addrs_distinct_preorders
  (#a:Type0) (#inv1 #inv2:data_inv a) (#rel1:preorder a) (#rel2:preorder a) (h:heap) (r1:mref a inv1 rel1) (r2:mref a inv2 rel2)
...
val lemma_free_mm_sel
  (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (#rel1:preorder a) (#rel2:preorder b) (h0:heap)
  (r1:mref a inv1 rel1{h0 `contains` r1 /\ is_mm r1}) (r2:mref b inv2 rel2)
  :Lemma (requires True)
 ...
 *)
val lemma_sel_same_addr (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r1:mref a inv rel) (r2:mref a inv rel)
 ...
 *)
val lemma_sel_upd1 (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r1:mref a inv rel) (x:a{valid_upd h r1 x}) (r2:mref a inv rel)
  :Lemma (requires (addr_of r1 = addr_of r2))
         (ensures  (sel (upd h r1 x) r2 == x))
         [SMTPat (sel (upd h r1 x) r2)]

val lemma_sel_upd2 (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b)
  (#rel1:preorder a) (#rel2:preorder b) (h:heap) (r1:mref a inv1 rel1) (r2:mref b inv2 rel2) (x:b{valid_upd h r2 x})
  :Lemma (requires (addr_of r1 <> addr_of r2))
         (ensures  (sel (upd h r2 x) r1 == sel h r1))
	 [SMTPat (sel (upd h r2 x) r1)]

val lemma_mref_injectivity
  :(u:unit{forall (a:Type0) (b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (rel1:preorder a) (rel2:preorder b) (r1:mref a inv1 rel1) (r2:mref b inv2 rel2). a =!= b ==> ~ (eq3 r1 r2)})

val lemma_in_dom_emp (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (r:mref a inv rel)
  :Lemma (requires True)
         (ensures  (r `unused_in` emp))
	 [SMTPat (r `unused_in` emp)]

val lemma_upd_contains (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r:mref a inv rel) (x:a{valid_upd h r x})
  :Lemma (requires True)
         (ensures  ((upd h r x) `contains` r))
	 [SMTPat ((upd h r x) `contains` r)]

val lemma_well_typed_upd_contains
  (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (#rel1:preorder a) (#rel2:preorder b) (h:heap) (r1:mref a inv1 rel1) (x:a{valid_upd h r1 x}) (r2:mref b inv2 rel2)
...
val lemma_unused_upd_contains
  (#a:Type0) (#b:Type0) (#inv1:data_inv a) (#inv2:data_inv b) (#rel1:preorder a) (#rel2:preorder b) (h:heap)
  (r1:mref a inv1 rel1) (x:a{valid_upd h r1 x}) (r2:mref b inv2 rel2)
...
val lemma_contains_upd_modifies (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r:mref a inv rel) (x:a{valid_upd h r x})
  :Lemma (requires (h `contains` r))
         (ensures  (modifies (S.singleton (addr_of r)) h (upd h r x)))
         [SMTPat (upd h r x); SMTPatT (h `contains` r)]

val lemma_unused_upd_modifies (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r:mref a inv rel) (x:a{valid_upd h r x})
...
val lemma_sel_equals_sel_tot_for_contained_refs
  (#a:Type0) (#inv:data_inv a) (#rel:preorder a) (h:heap) (r:mref a inv rel{h `contains` r})
  :Lemma (requires True)
