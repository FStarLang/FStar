...
private
let replace_subseq
  (#a: Type0) (s: Seq.seq a) (offset: nat) (len: nat{offset + len <= Seq.length s})
  (sub: Seq.lseq a len)
  : Tot (Seq.seq a) = Seq.replace_subseq s offset (offset + len) sub
private
let lemma_replace_subseq_elim
  (#a: Type0) (s: Seq.seq a) (offset: nat) (len: nat{offset + len <= Seq.length s})
  (sub: Seq.lseq a len)
  : Lemma
    (let s1 = replace_subseq s offset len sub in
      Seq.length s1 == Seq.length s /\ Seq.equal (Seq.slice s1 0 offset) (Seq.slice s 0 offset) /\
      Seq.equal (Seq.slice s1 offset (offset + len)) sub /\
      Seq.equal (Seq.slice s1 (offset + len) (Seq.length s1))
        (Seq.slice s (offset + len) (Seq.length s))) [SMTPat (replace_subseq s offset len sub)] = ()
private
...
let unused_in #_ #_ #_ b h =
  match b with
  | Null -> False
  | Buffer _ content _ _ _ -> HS.unused_in content h

let live #_ #_ #_ h b =
  match b with
  | Null -> True
  | Buffer _ content _ _ _ -> HS.contains h content
...
let mgsub #a #rrel #rel b i len sub_rel =
  ...
  | Buffer max_len content idx length () ->
    Seq.lemma_seq_sub_compatibility_is_transitive (U32.v max_len)
      rrel
      (U32.v idx)
      (U32.v idx + U32.v length)
      rel
...
let gsub_gsub #_ #_ #rel b i1 len1 sub_rel1 i2 len2 sub_rel2 =
  Seq.lemma_seq_sub_compatibility_is_transitive (length b)
    rel
    (U32.v i1)
    (U32.v i1 + U32.v len1)
    sub_rel1
...
noeq
type ubuffer_ : Type0 = { b_max_length:nat; b_offset:nat; b_length:nat; b_is_mm:bool }
...
let ubuffer_preserved' (#r: HS.rid) (#a: nat) (b: ubuffer r a) (h: HS.mem) (h': HS.mem) : GTot Type0 =
...
val ubuffer_preserved: #r: HS.rid -> #a: nat -> b: ubuffer r a -> h: HS.mem -> h': HS.mem ->
  GTot Type0

let ubuffer_preserved = ubuffer_preserved'

let ubuffer_preserved_intro
  (#r: HS.rid) (#a: nat) (b: ubuffer r a) (h: HS.mem) (h': HS.mem)
  (f:
  ...
  let g' (t': Type0) (rrel: srel t') (rel: srel t') (b': mbuffer t' rrel rel)
    : Lemma
    ((frameOf b' == r /\ as_addr b' == a /\ ubuffer_of_buffer' b' == b /\ live h b') ==>
...
val ubuffer_preserved_refl: #r: HS.rid -> #a: nat -> b: ubuffer r a -> h: HS.mem ->
  Lemma (ubuffer_preserved b h h)

let ubuffer_preserved_refl #r #a b h = ()

val ubuffer_preserved_trans: 
  #r: HS.rid ->
  #a: nat ->
  b: ubuffer r a ->
  h1: HS.mem ->
  h2: HS.mem ->
  h3: HS.mem ->
  Lemma (requires (ubuffer_preserved b h1 h2 /\ ubuffer_preserved b h2 h3))
...
val same_mreference_ubuffer_preserved: 
  ...
  Lemma (ubuffer_preserved b h1 h2)
...
val addr_unused_in_ubuffer_preserved: 
  ...
  h1: HS.mem ->
  h2: HS.mem ->
  Lemma (requires (HS.live_region h1 r ==> Heap.addr_unused_in a (Map.sel (HS.get_hmap h1) r)))
    (ensures (ubuffer_preserved b h1 h2))

let addr_unused_in_ubuffer_preserved #r #a b h1 h2 = ()

val ubuffer_of_buffer: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  Tot (ubuffer (frameOf b) (as_addr b))

let ubuffer_of_buffer #_ #_ #_ b = ubuffer_of_buffer' b

val ubuffer_preserved_elim: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
...
let unused_in_ubuffer_preserved
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (h: HS.mem) (h': HS.mem)
...
let ubuffer_includes' (larger: ubuffer_) (smaller: ubuffer_) : GTot Type0 =
...
let ubuffer_includes0
  (#r1: HS.rid) (#r2: HS.rid) (#a1: nat) (#a2: nat) (larger: ubuffer r1 a1) (smaller: ubuffer r2 a2) =
  r1 == r2 /\ a1 == a2 /\ ubuffer_includes' (G.reveal larger) (G.reveal smaller)

val ubuffer_includes: #r: HS.rid -> #a: nat -> larger: ubuffer r a -> smaller: ubuffer r a ->
  GTot Type0

let ubuffer_includes #r #a larger smaller = ubuffer_includes0 larger smaller

val ubuffer_includes_refl: #r: HS.rid -> #a: nat -> b: ubuffer r a -> Lemma (ubuffer_includes b b)

let ubuffer_includes_refl #r #a b = ()

val ubuffer_includes_trans: 
  #r: HS.rid ->
  #a: nat ->
  b1: ubuffer r a ->
  b2: ubuffer r a ->
  b3: ubuffer r a ->
  Lemma (requires (ubuffer_includes b1 b2 /\ ubuffer_includes b2 b3))
    (ensures (ubuffer_includes b1 b3))
...
let ubuffer_disjoint' (x1: ubuffer_) (x2: ubuffer_) : GTot Type0 =
  (x1.b_max_length == x2.b_max_length /\
    (x1.b_offset + x1.b_length <= x2.b_offset \/ x2.b_offset + x2.b_length <= x1.b_offset))

(* TODO: added this because of #606, now that it is fixed, we may not need it anymore *)
let ubuffer_disjoint0
  (#r1: HS.rid) (#r2: HS.rid) (#a1: nat) (#a2: nat) (b1: ubuffer r1 a1) (b2: ubuffer r2 a2) =
  r1 == r2 /\ a1 == a2 /\ ubuffer_disjoint' (G.reveal b1) (G.reveal b2)

val ubuffer_disjoint: #r: HS.rid -> #a: nat -> b1: ubuffer r a -> b2: ubuffer r a -> GTot Type0
let ubuffer_disjoint #r #a b1 b2 = ubuffer_disjoint0 b1 b2

val ubuffer_disjoint_sym: #r: HS.rid -> #a: nat -> b1: ubuffer r a -> b2: ubuffer r a ->
  Lemma (ubuffer_disjoint b1 b2 <==> ubuffer_disjoint b2 b1)
let ubuffer_disjoint_sym #_ #_ b1 b2 = ()

val ubuffer_disjoint_includes: 
  #r: HS.rid ->
  #a: nat ->
  larger1: ubuffer r a ->
  larger2: ubuffer r a ->
  smaller1: ubuffer r a ->
  smaller2: ubuffer r a ->
  Lemma
    (requires
      (ubuffer_disjoint larger1 larger2 /\ ubuffer_includes larger1 smaller1 /\
        ubuffer_includes larger2 smaller2)) (ensures (ubuffer_disjoint smaller1 smaller2))
...
val liveness_preservation_intro: 
  ...
  b: mbuffer a rrel rel ->
  f:
  (t': Type0 -> pre: Preorder.preorder t' -> r: HS.mreference t' pre ->
      Lemma (requires (HS.frameOf r == frameOf b /\ HS.as_addr r == as_addr b /\ HS.contains h r))
        (ensures (HS.contains h' r))) ->
  Lemma (requires (live h b)) (ensures (live h' b))

let liveness_preservation_intro #_ #_ #_ _ _ b f = if Null? b then () else f _ _ (Buffer?.content b)

(* Basic, non-compositional modifies clauses, used only to implement the generic modifies clause. DO NOT USE in client code *)

let modifies_0_preserves_mreferences (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  forall (a: Type) (pre: Preorder.preorder a) (r: HS.mreference a pre).
    HS.contains h1 r ==> (HS.contains h2 r /\ HS.sel h1 r == HS.sel h2 r)

let modifies_0_preserves_regions (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  forall (r: HS.rid). HS.live_region h1 r ==> HS.live_region h2 r

let modifies_0_preserves_not_unused_in (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  forall (r: HS.rid) (n: nat).
    (HS.live_region h1 r /\ HS.live_region h2 r /\
      Heap.addr_unused_in n (Map.sel (HS.get_hmap h2) r)) ==>
    (Heap.addr_unused_in n (Map.sel (HS.get_hmap h1) r))

let modifies_0' (h1: HS.mem) (h2: HS.mem) : GTot Type0 =
  modifies_0_preserves_mreferences h1 h2 /\ modifies_0_preserves_regions h1 h2 /\
  modifies_0_preserves_not_unused_in h1 h2

val modifies_0: h1: HS.mem -> h2: HS.mem -> GTot Type0

let modifies_0 = modifies_0'

val modifies_0_live_region: h1: HS.mem -> h2: HS.mem -> r: HS.rid ->
  Lemma (requires (modifies_0 h1 h2 /\ HS.live_region h1 r)) (ensures (HS.live_region h2 r))

let modifies_0_live_region h1 h2 r = ()

val modifies_0_mreference: 
  #a: Type ->
  #pre: Preorder.preorder a ->
  h1: HS.mem ->
  h2: HS.mem ->
  r: HS.mreference a pre ->
  Lemma (requires (modifies_0 h1 h2 /\ HS.contains h1 r))
    (ensures (HS.contains h2 r /\ HS.sel h1 r == HS.sel h2 r))

let modifies_0_mreference #a #pre h1 h2 r = ()

let modifies_0_ubuffer (#r: HS.rid) (#a: nat) (b: ubuffer r a) (h1: HS.mem) (h2: HS.mem)
...
val modifies_0_unused_in: h1: HS.mem -> h2: HS.mem -> r: HS.rid -> n: nat ->
  Lemma
    (requires
      (modifies_0 h1 h2 /\ HS.live_region h1 r /\ HS.live_region h2 r /\
        Heap.addr_unused_in n (Map.sel (HS.get_hmap h2) r)))
    (ensures (Heap.addr_unused_in n (Map.sel (HS.get_hmap h1) r)))

let modifies_0_unused_in h1 h2 r n = ()

let modifies_1_preserves_mreferences
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (h1: HS.mem) (h2: HS.mem)
  : GTot Type0 =
  forall (a': Type) (pre: Preorder.preorder a') (r': HS.mreference a' pre).
    ((frameOf b <> HS.frameOf r' \/ as_addr b <> HS.as_addr r') /\ HS.contains h1 r') ==>
    (HS.contains h2 r' /\ HS.sel h1 r' == HS.sel h2 r')
...
let modifies_1_preserves_livenesses
  ...
  : GTot Type0 =
  forall (a': Type) (pre: Preorder.preorder a') (r': HS.mreference a' pre).
    HS.contains h1 r' ==> HS.contains h2 r'
...
val modifies_1_liveness: 
  ...
  #pre: Preorder.preorder a' ->
  r': HS.mreference a' pre ->
  Lemma (requires (modifies_1 b h1 h2 /\ HS.contains h1 r')) (ensures (HS.contains h2 r'))
...
val modifies_1_unused_in: 
  ...
  Lemma
    (requires
      (modifies_1 b h1 h2 /\ HS.live_region h1 r /\ HS.live_region h2 r /\
        Heap.addr_unused_in n (Map.sel (HS.get_hmap h2) r)))
    (ensures (Heap.addr_unused_in n (Map.sel (HS.get_hmap h1) r)))
...
val modifies_1_mreference: 
  ...
  Lemma
    (requires
      (modifies_1 b h1 h2 /\ (frameOf b <> HS.frameOf r' \/ as_addr b <> HS.as_addr r') /\
        HS.contains h1 r')) (ensures (HS.contains h2 r' /\ HS.sel h1 r' == HS.sel h2 r'))
...
val modifies_1_ubuffer: 
  ...
  Lemma
    (requires
      (modifies_1 b h1 h2 /\ ubuffer_disjoint #(frameOf b) #(as_addr b) (ubuffer_of_buffer b) b'))
...
val modifies_1_null: 
  ...
  h1: HS.mem ->
  h2: HS.mem ->
  Lemma (requires (modifies_1 b h1 h2 /\ g_is_null b)) (ensures (modifies_0 h1 h2))
...
val modifies_addr_of_live_region: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
  ...
  h2: HS.mem ->
  r: HS.rid ->
  Lemma (requires (modifies_addr_of b h1 h2 /\ HS.live_region h1 r)) (ensures (HS.live_region h2 r))
...
val modifies_addr_of_mreference: 
  ...
  Lemma
    (requires
      (modifies_addr_of b h1 h2 /\ (frameOf b <> HS.frameOf r' \/ as_addr b <> HS.as_addr r') /\
        HS.contains h1 r')) (ensures (HS.contains h2 r' /\ HS.sel h1 r' == HS.sel h2 r'))
...
val modifies_addr_of_unused_in: 
  ...
  Lemma
    (requires
      (modifies_addr_of b h1 h2 /\ (r <> frameOf b \/ n <> as_addr b) /\ HS.live_region h1 r /\
        HS.live_region h2 r /\ Heap.addr_unused_in n (Map.sel (HS.get_hmap h2) r)))
    (ensures (Heap.addr_unused_in n (Map.sel (HS.get_hmap h1) r)))
let modifies_addr_of_unused_in #_ #_ #_ _ _ _ _ _ = ()

module MG = FStar.ModifiesGen

let cls: MG.cls ubuffer =
  MG.Cls #ubuffer
    ubuffer_includes
...
val loc_includes_buffer: 
  ...
  Lemma
    (requires
      (frameOf b1 == frameOf b2 /\ as_addr b1 == as_addr b2 /\
        ubuffer_includes0 #(frameOf b1)
          #(frameOf b2)
          #(as_addr b1)
          #(as_addr b2)
          (ubuffer_of_buffer b1)
          (ubuffer_of_buffer b2))) (ensures (loc_includes (loc_buffer b1) (loc_buffer b2)))
  ...
  then ()
    ...
    then
      ...
      (MG.loc_includes_aloc_elim #_
          ...
          (ubuffer_of_buffer smaller);
        //TODO: we should be able to prove this somehow in HS?
        assume (rrel1 == rrel2);
...
val loc_disjoint_buffer: 
  ...
  Lemma
    (requires
      ((frameOf b1 == frameOf b2 /\ as_addr b1 == as_addr b2) ==>
        ubuffer_disjoint0 #(frameOf b1)
          #(frameOf b2)
          #(as_addr b1)
          #(as_addr b2)
          (ubuffer_of_buffer b1)
          (ubuffer_of_buffer b2))) (ensures (loc_disjoint (loc_buffer b1) (loc_buffer b2)))
...
let modifies_buffer_elim #_ #_ #_ b p h h' =
  if g_is_null b
  then assert (Seq.equal (as_seq h b) (as_seq h' b))
  else
...
let modifies_liveness_insensitive_buffer l1 l2 h h' #_ #_ #_ x =
  ...
  then ()
    ...
    liveness_preservation_intro h
      h'
      x
      (fun t' pre r -> MG.modifies_preserves_liveness_strong l1 l2 h h' r (ubuffer_of_buffer x))
...
val modifies_0_modifies: h1: HS.mem -> h2: HS.mem ->
  Lemma (requires (modifies_0 h1 h2)) (ensures (modifies loc_none h1 h2))
...
val modifies_1_modifies: 
  ...
  h1: HS.mem ->
  h2: HS.mem ->
  Lemma (requires (modifies_1 b h1 h2)) (ensures (modifies (loc_buffer b) h1 h2))
let modifies_1_modifies #t #_ #_ b h1 h2 =
  if g_is_null b
  then (modifies_1_null b h1 h2; modifies_0_modifies h1 h2)
  else
...
val modifies_addr_of_modifies: 
  ...
  h1: HS.mem ->
  h2: HS.mem ->
  Lemma (requires (modifies_addr_of b h1 h2)) (ensures (modifies (loc_addr_of_buffer b) h1 h2))
...
let loc_unused_in_not_unused_in_disjoint = MG.loc_unused_in_not_unused_in_disjoint cls
...
let modifies_inert_liveness_insensitive_mreference_weak =
  modifies_liveness_insensitive_mreference_weak
...
let modifies_inert_liveness_insensitive_region_mreference_weak =
  modifies_liveness_insensitive_region_mreference_weak

let modifies_inert_liveness_insensitive_region_buffer_weak =
  modifies_liveness_insensitive_region_buffer_weak
...
let pointer_distinct_sel_disjoint #a #_ #_ #_ #_ b1 b2 h =
  ...
  else loc_disjoint_buffer b1 b2
...
let recallable (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) : GTot Type0 =
  (not (g_is_null b)) ==> (HST.is_eternal_region (frameOf b) /\ not (HS.is_mm (Buffer?.content b)))
...
let freeable (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) =
  (not (g_is_null b)) /\ HS.is_mm (Buffer?.content b) /\ HST.is_eternal_region (frameOf b) /\
...
let alloc_common (#a: Type0) (#rrel: srel a) (r: HS.rid) (init: a) (len: U32.t) (mm: bool)
  : HST.ST (mbuffer a rrel rrel)
    (requires (fun h0 -> HST.is_eternal_region r /\ U32.v len > 0))
    (ensures
      (fun h0 b h1 ->
          alloc_post_common r (U32.v len) b h0 h1 /\ as_seq h1 b == Seq.create (U32.v len) init /\
          HS.is_mm (Buffer?.content b) == mm /\ Buffer?.idx b == 0ul /\
          Buffer?.length b == Buffer?.max_length b)) =
...
let coerce (t2: Type) (#t1: Type) (x1: t1)
  : Pure t2 (requires (t1 == t2)) (ensures (fun y -> y == x1)) = x1
