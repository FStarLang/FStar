...
let rec flatten_correct_aux (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (xs1 xs2:list atom) 
  : Lemma (xsdenote m am (xs1 @ xs2) `EQ?.eq eq` CM?.mult m (xsdenote m am xs1)
                                                            (xsdenote m am xs2)) = 
  match xs1 with
  | [] -> 
      CM?.identity m (xsdenote m am xs2);
      EQ?.symmetry eq (CM?.mult m (CM?.unit m) (xsdenote m am xs2)) (xsdenote m am xs2)
  | [x] -> (
      if (Nil? xs2) 
      then (right_identity eq m (select x am);
            EQ?.symmetry eq (CM?.mult m (select x am) (CM?.unit m)) (select x am))
      else EQ?.reflexivity eq (CM?.mult m (xsdenote m am [x]) (xsdenote m am xs2)))
  | x::xs1' -> 
      flatten_correct_aux m am xs1' xs2;
      EQ?.reflexivity eq (select x am);
      CM?.congruence m (select x am) (xsdenote m am (xs1' @ xs2)) 
                       (select x am) (CM?.mult m (xsdenote m am xs1') (xsdenote m am xs2));
      CM?.associativity m (select x am) (xsdenote m am xs1') (xsdenote m am xs2);
      EQ?.symmetry eq (CM?.mult m (CM?.mult m (select x am) (xsdenote m am xs1')) (xsdenote m am xs2)) 
                      (CM?.mult m (select x am) (CM?.mult m (xsdenote m am xs1') (xsdenote m am xs2)));
      EQ?.transitivity eq (CM?.mult m (select x am) (xsdenote m am (xs1' @ xs2)))
                          (CM?.mult m (select x am) (CM?.mult m (xsdenote m am xs1') (xsdenote m am xs2)))
                          (CM?.mult m (CM?.mult m (select x am) (xsdenote m am xs1')) (xsdenote m am xs2))

let rec flatten_correct (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (e:exp) 
  : Lemma (mdenote m am e `EQ?.eq eq` xsdenote m am (flatten e)) = 
  match e with
  | Unit -> EQ?.reflexivity eq (CM?.unit m)
  | Atom x -> EQ?.reflexivity eq (select x am)
  | Mult e1 e2 -> 
      flatten_correct_aux m am (flatten e1) (flatten e2);
      EQ?.symmetry eq (xsdenote m am (flatten e1 @ flatten e2)) 
                      (CM?.mult m (xsdenote m am (flatten e1)) (xsdenote m am (flatten e2)));
      flatten_correct m am e1; 
      flatten_correct m am e2;
      CM?.congruence m (mdenote m am e1) (mdenote m am e2) 
                       (xsdenote m am (flatten e1)) (xsdenote m am (flatten e2));
      EQ?.transitivity eq (CM?.mult m (mdenote m am e1) (mdenote m am e2)) 
                          (CM?.mult m (xsdenote m am (flatten e1)) (xsdenote m am (flatten e2)))
                          (xsdenote m am (flatten e1 @ flatten e2))  
...
let permute_correct (p:permute) =
  #a:Type -> #eq:equiv a -> m:cm a eq -> am:amap a -> xs:list atom ->
    Lemma (xsdenote m am xs `EQ?.eq eq` xsdenote m am (p xs))
...
let rec apply_swap_aux_correct (#a:Type) (n:nat) (#eq:equiv a) (m:cm a eq) (am:amap a)
  ...
  : Lemma (requires True)
      (ensures (xsdenote m am xs `EQ?.eq eq` xsdenote m am (apply_swap_aux n xs s)))
      (decreases xs) =
  match xs with
  ...
  | [x1;x2] -> 
      ...
      else EQ?.reflexivity eq (xsdenote m am [x1;x2])
  | x1 :: x2 :: xs' -> 
      if n = (s <: nat)
      then (
        CM?.associativity m (select x1 am) (select x2 am) (xsdenote m am xs');
        EQ?.symmetry eq (CM?.mult m (CM?.mult m (select x1 am) (select x2 am)) (xsdenote m am xs')) 
                        (CM?.mult m (select x1 am) (CM?.mult m (select x2 am) (xsdenote m am xs')));
        CM?.commutativity m (select x1 am) (select x2 am);
        EQ?.reflexivity eq (xsdenote m am xs');
        CM?.congruence m (CM?.mult m (select x1 am) (select x2 am)) (xsdenote m am xs') 
                         (CM?.mult m (select x2 am) (select x1 am)) (xsdenote m am xs');
        CM?.associativity m (select x2 am) (select x1 am) (xsdenote m am xs');
        EQ?.transitivity eq (CM?.mult m (select x1 am) (CM?.mult m (select x2 am) (xsdenote m am xs')))
                            (CM?.mult m (CM?.mult m (select x1 am) (select x2 am)) (xsdenote m am xs'))
                            (CM?.mult m (CM?.mult m (select x2 am) (select x1 am)) (xsdenote m am xs'));
        EQ?.transitivity eq (CM?.mult m (select x1 am) (CM?.mult m (select x2 am) (xsdenote m am xs')))
                            (CM?.mult m (CM?.mult m (select x2 am) (select x1 am)) (xsdenote m am xs'))
                            (CM?.mult m (select x2 am) (CM?.mult m (select x1 am) (xsdenote m am xs'))))
      else (
        apply_swap_aux_correct (n+1) m am (x2 :: xs') s;
        EQ?.reflexivity eq (select x1 am);
        CM?.congruence m (select x1 am) (xsdenote m am (x2 :: xs'))
                         (select x1 am) (xsdenote m am (apply_swap_aux (n+1) (x2 :: xs') s)))

let apply_swap_correct (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a)
                       (xs:list atom) (s:swap (length xs))
  : Lemma (ensures (xsdenote m am xs `EQ?.eq eq` xsdenote m am (apply_swap xs s)))
          (decreases xs) = 
...
let rec apply_swaps_correct (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a)
  ...
  : Lemma (requires True)
      (ensures (xsdenote m am xs `EQ?.eq eq` xsdenote m am (apply_swaps xs ss)))
      (decreases ss) =
  match ss with
  | [] -> EQ?.reflexivity eq (xsdenote m am xs)
  | s::ss' -> 
      ...
      EQ?.transitivity eq (xsdenote m am xs)
                          (xsdenote m am (apply_swap xs s))
                          (xsdenote m am (apply_swaps (apply_swap xs s) ss'))
...
let rec permute_via_swaps_correct_aux (p:permute) (pvs:permute_via_swaps p)
  ...
  exists_elim (xsdenote m am xs `EQ?.eq eq` xsdenote m am (p xs))
...
let rec sort_correct_aux (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (xs:list atom) 
  : Lemma (xsdenote m am xs `EQ?.eq eq` xsdenote m am (sort xs)) =
...
let canon_correct (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (e:exp) 
  : Lemma (mdenote m am e `EQ?.eq eq` xsdenote m am (canon e)) =
  flatten_correct m am e; 
  sort_correct m am (flatten e);
  EQ?.transitivity eq (mdenote m am e)
                      (xsdenote m am (flatten e))
                      (xsdenote m am (sort (flatten e)))

let monoid_reflect_orig (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (e1 e2:exp)
  : Lemma (requires (xsdenote m am (canon e1) `EQ?.eq eq` xsdenote m am (canon e2)))
          (ensures (mdenote m am e1 `EQ?.eq eq` mdenote m am e2)) =
  canon_correct m am e1; 
  canon_correct m am e2;
  EQ?.symmetry eq (mdenote m am e2) (xsdenote m am (canon e2));
  EQ?.transitivity eq (mdenote m am e1)
                      (xsdenote m am (canon e1))
                      (xsdenote m am (canon e2));
  EQ?.transitivity eq (mdenote m am e1)
                      (xsdenote m am (canon e2))
                      (mdenote m am e2)

let monoid_reflect (#a:Type) (#eq:equiv a) (m:cm a eq) (am:amap a) (e1 e2:exp)
    (_ : squash (xsdenote m am (canon e1) `EQ?.eq eq` xsdenote m am (canon e2)))
       : squash (mdenote m am e1 `EQ?.eq eq` mdenote m am e2) =
  monoid_reflect_orig #a m am e1 e2
