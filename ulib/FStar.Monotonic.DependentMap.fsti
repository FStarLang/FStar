...
let partial_dependent_map (a: eqtype) (b: (a -> Type)) = DM.t a (opt b)

/// An empty partial, dependent map maps all keys to None
let empty_partial_dependent_map (#a: _) (#b: _) : partial_dependent_map a b =
...
val map: a: eqtype u#a -> b: (a -> Type u#b) -> Type u#(max a b)

/// `repr m`: A ghost function that reveals the internal `map` as a `DM.t`
val repr: #a: _ -> #b: _ -> r: map a b -> GTot (partial_dependent_map a b)

/// An `empty : map a b` is equivalent to the `empty_partial_dependent_map`
val empty: #a: _ -> #b: _ -> r: map a b {repr r == empty_partial_dependent_map}

/// Selecting a key from a map `sel r x` is equivalent to selecting it from its `repr`
val sel: #a: _ -> #b: _ -> r: map a b -> x: a ->
  Pure (option (b x)) (requires True) (ensures (fun o -> DM.sel (repr r) x == o))

/// Updating a map using `upd r x v` is equivalent to updating its repr
val upd: #a: _ -> #b: _ -> r: map a b -> x: a -> v: b x ->
  Pure (map a b) (requires True) (ensures (fun r' -> repr r' == DM.upd (repr r) x (Some v)))

/// `imap a b inv` further augments a map with an invariant on its repr
let imap (a: eqtype) (b: (a -> Type)) (inv: (DM.t a (opt b) -> Type)) = r: map a b {inv (repr r)}
...
let defined
  (#a: eqtype) (#b: (a -> Type)) (#inv: (DM.t a (opt b) -> Type)) (#r: HST.erid) (t: t r a b inv)
  (x: a) (h: HS.mem)
  : GTot Type = Some? (sel (HS.sel h t) x)

/// `fresh t x h`: The map is not defined at point `x`
let fresh
  (#a: eqtype) (#b: (a -> Type)) (#inv: (DM.t a (opt b) -> Type)) (#r: HST.erid) (t: t r a b inv)
  (x: a) (h: HS.mem)
  : GTot Type0 = ~(defined t x h)

/// `value_of t x h`: Get the value of `x` in the map `t` in state `h`
let value_of
  (#a: eqtype) (#b: (a -> Type)) (#inv: (DM.t a (opt b) -> Type)) (#r: HST.erid) (t: t r a b inv)
  (x: a) (h: HS.mem{defined t x h})
  : GTot (b x) = Some?.v (sel (HS.sel h t) x)

/// `contains t x y h`: In state `h`, `t` maps `x` to `y`
let contains
  (#a: eqtype) (#b: (a -> Type)) (#inv: (DM.t a (opt b) -> Type)) (#r: HST.erid) (t: t r a b inv)
  (x: a) (y: b x) (h: HS.mem)
  : GTot Type0 = defined t x h /\ value_of t x h == y
...
val extend: 
  ...
  y: b x ->
  ST unit
    (requires (fun h -> ~(defined t x h) /\ inv (repr (upd (HS.sel h t) x y))))
    (ensures
      (fun h0 u h1 ->
          let cur = HS.sel h0 t in
          HS.contains h1 t /\ HS.modifies (Set.singleton r) h0 h1 /\
          HS.modifies_ref r (Set.singleton (HS.as_addr t)) h0 h1 /\ HS.sel h1 t == upd cur x y /\
...
val lookup: 
  ...
  ST (option (b x))
    ...
    (ensures
      (fun h0 y h1 ->
          h0 == h1 /\ y == sel (HS.sel h1 t) x /\
          (match y with
            | None -> ~(defined t x h1)
            | Some v -> contains t x v h1 /\ witnessed (contains t x v))))

let forall_t
  (#a: eqtype) (#b: (a -> Type)) (#inv: (DM.t a (opt b) -> Type)) (#r: HST.erid) (t: t r a b inv)
  (h: HS.mem) (pred: (x: a -> b x -> Type0)) =
  forall (x: a). {:pattern (sel (HS.sel h t) x)\/(DM.sel (repr (HS.sel h t)) x)}
    defined t x h ==> pred x (Some?.v (sel (HS.sel h t) x))

let f_opt (#a: eqtype) (#b: (a -> Type)) (#c: (a -> Type)) (f: (x: a -> b x -> c x))
  : x: a -> option (b x) -> option (c x) =
  fun x y ->
    match y with
    | None -> None
    | Some y -> Some (f x y)
...
val map_f: 
  ...
  ST (t r' a c inv')
    (requires
      (fun h0 -> inv' (DM.map (f_opt f) (repr (HS.sel h0 m))) /\ witnessed (region_contains_pred r')
      ))
    (ensures
      (fun h0 m' h1 ->
          //AR: surprised that even after the fix for #57, we need this repetetion from the requires clause
          inv' (DM.map (f_opt f) (repr (HS.sel h0 m))) /\
