...
unfold
let srel (a: Type0) = Seq.seq_pre a
...
val mbuffer: a: Type0 -> rrel: srel a -> rel: srel a -> Tot Type0
...
val g_is_null: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot bool

val mnull: #a: Type0 -> #rrel: srel a -> #rel: srel a -> Tot (b: mbuffer a rrel rel {g_is_null b})

val null_unique: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  Lemma (g_is_null b <==> b == mnull)

/// ``unused_in b h`` holds only if buffer ``b`` has not been allocated
/// yet.

val unused_in: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> h: HS.mem ->
  GTot Type0
...
val live: #a: Type0 -> #rrel: srel a -> #rel: srel a -> h: HS.mem -> b: mbuffer a rrel rel ->
  GTot Type0

/// The null pointer is always live.

val live_null: a: Type0 -> rrel: srel a -> rel: srel a -> h: HS.mem ->
  Lemma (live h (mnull #a #rrel #rel))
...
val live_not_unused_in: 
  ...
  Lemma (requires (live h b /\ unused_in b h)) (ensures False)
...
let live_not_unused_in'
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (h: HS.mem) (b: mbuffer a rrel rel)
  : Lemma (requires (live h b /\ unused_in b h))
    (ensures False)
    [SMTPat (live h b); SMTPat (unused_in b h)] = live_not_unused_in h b
...
val frameOf: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> Tot HS.rid
...
val as_addr: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot nat
...
val unused_in_equiv: 
  ...
  Lemma
  (unused_in b h <==>
    (HS.live_region h (frameOf b) ==>
      Heap.addr_unused_in (as_addr b) (Map.sel (HS.get_hmap h) (frameOf b))))
...
val live_region_frameOf: 
  ...
  Lemma (requires (live h b))
    ...
    [SMTPatOr [[SMTPat (live h b)]; [SMTPat (HS.live_region h (frameOf b))]]]
...
val len: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot U32.t

let length (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) : GTot nat =
  U32.v (len b)

/// The null pointer has length 0.

val len_null: a: Type0 -> rrel: srel a -> rel: srel a -> Lemma (len (mnull #a #rrel #rel) == 0ul)
...
val as_seq: #a: Type0 -> #rrel: srel a -> #rel: srel a -> h: HS.mem -> b: mbuffer a rrel rel ->
  GTot (Seq.seq a)
...
val length_as_seq: 
  ...
  h: HS.mem ->
  b: mbuffer a rrel rel ->
  Lemma (Seq.length (as_seq h b) == length b) [SMTPat (Seq.length (as_seq h b))]
...
let get (#a: Type0) (#rrel: srel a) (#rel: srel a) (h: HS.mem) (p: mbuffer a rrel rel) (i: nat)
  : Ghost a (requires (i < length p)) (ensures (fun _ -> True)) = Seq.index (as_seq h p) i
...
let compatible_sub
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (i: U32.t)
  (len: U32.t{U32.v i + U32.v len <= length b}) (sub_rel: srel a) =
...
val len_gsub: 
  ...
  Lemma (requires (U32.v i + U32.v len' <= length b /\ compatible_sub b i len' sub_rel))
    ...
    [SMTPatOr [[SMTPat (len (mgsub b i len' sub_rel))]; [SMTPat (length (mgsub b i len' sub_rel))]]]
...
val mgsub_inj: 
  ...
  #rrel: srel a ->
  #rel: srel a ->
  b1: mbuffer a rrel rel ->
  b2: mbuffer a rrel rel ->
  i1: U32.t ->
  i2: U32.t ->
  len1: U32.t ->
  len2: U32.t ->
  ...
  Lemma
    (requires
      (U32.v i1 + U32.v len1 <= length b1 /\ compatible_sub b1 i1 len1 sub_rel1 /\
...
val gsub_gsub: 
  ...
  Lemma
    (requires
      (U32.v i1 + U32.v len1 <= length b /\ compatible_sub b i1 len1 sub_rel1 /\
        U32.v i2 + U32.v len2 <= U32.v len1 /\
        compatible_sub (mgsub b i1 len1 sub_rel1) i2 len2 sub_rel2))
    (ensures
      (compatible_sub b (U32.add i1 i2) len2 sub_rel2 /\
...
val gsub_zero_length: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
...
val as_seq_gsub: 
  ...
  Lemma (requires (U32.v i + U32.v len <= length b /\ compatible_sub b i len sub_rel))
    (ensures
      (as_seq h (mgsub b i len sub_rel) == Seq.slice (as_seq h b) (U32.v i) (U32.v i + U32.v len)))
...
let loc_union_idem_1 (s1: loc) (s2: loc)
  : Lemma (loc_union s1 (loc_union s1 s2) == loc_union s1 s2)
    [SMTPat (loc_union s1 (loc_union s1 s2) == loc_union s1 s2)] = loc_union_assoc s1 s1 s2

let loc_union_idem_2 (s1: loc) (s2: loc)
  : Lemma (loc_union (loc_union s1 s2) s2 == loc_union s1 s2)
    [SMTPat (loc_union (loc_union s1 s2) s2)] = loc_union_assoc s1 s2 s2
...
val loc_buffer: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot loc

val loc_buffer_null: a: Type0 -> rrel: srel a -> rel: srel a ->
  Lemma (loc_buffer (mnull #a #rrel #rel) == loc_none) [SMTPat (loc_buffer (mnull #a #rrel #rel))]
...
unfold
let loc_addr_of_buffer (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) : GTot loc =
...
let loc_includes_trans_backwards (s1: loc) (s2: loc) (s3: loc)
  : Lemma (requires (loc_includes s1 s2 /\ loc_includes s2 s3))
    (ensures (loc_includes s1 s3))
    [SMTPat (loc_includes s1 s3); SMTPat (loc_includes s2 s3)] = loc_includes_trans s1 s2 s3
...
let loc_includes_union_r' (s: loc) (s1: loc) (s2: loc)
...
val loc_includes_gsub_buffer_r: 
  ...
  Lemma
    (requires
      (UInt32.v i + UInt32.v len <= (length b) /\ compatible_sub b i len sub_rel /\
...
let loc_includes_gsub_buffer_r'
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (i: UInt32.t) (len: UInt32.t)
  (sub_rel: srel a)
...
val loc_includes_gsub_buffer_l: 
  ...
  Lemma
    (requires
      (UInt32.v i1 + UInt32.v len1 <= (length b) /\ UInt32.v i1 <= UInt32.v i2 /\
        UInt32.v i2 + UInt32.v len2 <= UInt32.v i1 + UInt32.v len1 /\
        compatible_sub b i1 len1 sub_rel1 /\ compatible_sub b i2 len2 sub_rel2))
    (ensures
      (loc_includes (loc_buffer (mgsub b i1 len1 sub_rel1)) (loc_buffer (mgsub b i2 len2 sub_rel2)))
    )
...
val loc_includes_as_seq: 
  ...
  h1: HS.mem ->
  h2: HS.mem ->
  larger: mbuffer a rrel1 rel1 ->
  smaller: mbuffer a rrel2 rel2 ->
  Lemma
    (requires
      (loc_includes (loc_buffer larger) (loc_buffer smaller) /\ as_seq h1 larger == as_seq h2 larger /\
...
let loc_includes_addresses_buffer'
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel)
...
let loc_includes_region_buffer' (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel)
...
let loc_includes_region_addresses' (preserve_liveness: bool) (r: HS.rid) (a: Set.set nat)
  : Lemma (loc_includes (loc_regions true (Set.singleton r)) (loc_addresses preserve_liveness r a))
...
let loc_includes_region_region' (preserve_liveness: bool) (s: Set.set HS.rid)
  : Lemma (loc_includes (loc_regions false s) (loc_regions preserve_liveness s))
...
let loc_includes_addresses_addresses_1
  (preserve_liveness1: bool) (preserve_liveness2: bool) (r1: HS.rid) (r2: HS.rid) (s1: Set.set nat)
  (s2: Set.set nat)
...
let loc_includes_addresses_addresses_2 (preserve_liveness: bool) (r: HS.rid) (s: Set.set nat)
  : Lemma (loc_includes (loc_addresses false r s) (loc_addresses preserve_liveness r s))
    [SMTPat (loc_addresses preserve_liveness r s)] = ()

/// Patterns with loc_includes, union on the left

let loc_includes_union_l_buffer
  (s1: loc) (s2: loc) (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel)
...
let loc_includes_union_l_addresses (s1: loc) (s2: loc) (prf: bool) (r: HS.rid) (a: Set.set nat)
  : Lemma
...
let loc_includes_union_l_regions (s1: loc) (s2: loc) (prf: bool) (r: Set.set HS.rid)
...
[@ "opaque_to_smt"]
private
let rec loc_disjoint_from_list (l: loc) (ls: list loc) : Type0 =
  match ls with
  | [] -> True
...
[@ "opaque_to_smt"]
private
let rec loc_pairwise_disjoint_aux (l: list loc) : Type0 =
  match l with
  | [] -> True
...
[@ "opaque_to_smt"]
unfold
let loc_pairwise_disjoint (l: list loc) : Type0 =
  norm [iota; zeta; delta; delta_only [`%loc_disjoint_from_list; `%loc_pairwise_disjoint_aux]]
    (loc_pairwise_disjoint_aux l)
...
let loc_disjoint_union_r' (s: loc) (s1: loc) (s2: loc)
  : Lemma (ensures (loc_disjoint s (loc_union s1 s2) <==> (loc_disjoint s s1 /\ loc_disjoint s s2)))
...
let loc_disjoint_includes_r (b1: loc) (b2: loc) (b2': loc)
...
val loc_disjoint_gsub_buffer: 
  ...
  Lemma
    (requires
      (UInt32.v i1 + UInt32.v len1 <= (length b) /\ UInt32.v i2 + UInt32.v len2 <= (length b) /\
        compatible_sub b i1 len1 sub_rel1 /\ compatible_sub b i2 len2 sub_rel2 /\
        (UInt32.v i1 + UInt32.v len1 <= UInt32.v i2 \/ UInt32.v i2 + UInt32.v len2 <= UInt32.v i1)))
    (ensures
      (loc_disjoint (loc_buffer (mgsub b i1 len1 sub_rel1)) (loc_buffer (mgsub b i2 len2 sub_rel2)))
    )
    [SMTPat (mgsub b i1 len1 sub_rel1); SMTPat (mgsub b i2 len2 sub_rel2)]
...
val modifies_live_region: s: loc -> h1: HS.mem -> h2: HS.mem -> r: HS.rid ->
  Lemma
...
val address_liveness_insensitive_buffer: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
...
val region_liveness_insensitive_buffer: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
  ...
  Lemma
    (requires
      (modifies (loc_union l1 l2) h h' /\ loc_disjoint l1 (loc_buffer x) /\
        loc_includes address_liveness_insensitive_locs l2 /\ live h x)) (ensures (live h' x))
...
let modifies_liveness_insensitive_mreference_weak
  ...
  : Lemma
    ...
    (ensures (HS.contains h' x)) = modifies_liveness_insensitive_mreference loc_none l h h' x

let modifies_liveness_insensitive_buffer_weak
  (l: loc) (h: HS.mem) (h': HS.mem) (#a: Type0) (#rrel: srel a) (#rel: srel a)
  (x: mbuffer a rrel rel)
  : Lemma
    (requires (modifies l h h' /\ loc_includes address_liveness_insensitive_locs l /\ live h x))
    (ensures (live h' x)) = modifies_liveness_insensitive_buffer loc_none l h h' x
...
val modifies_liveness_insensitive_region_buffer: 
  ...
  Lemma
    (requires
      (modifies (loc_union l1 l2) h h' /\ loc_disjoint l1 (loc_buffer x) /\
        loc_includes region_liveness_insensitive_locs l2 /\ HS.live_region h (frameOf x)))
...
let modifies_liveness_insensitive_region_weak (l2: loc) (h: HS.mem) (h': HS.mem) (x: HS.rid)
  : Lemma
    ...
    (ensures (HS.live_region h' x)) = modifies_liveness_insensitive_region loc_none l2 h h' x
  ...
  : Lemma
    (requires
      (modifies l2 h h' /\ loc_includes region_liveness_insensitive_locs l2 /\
        HS.live_region h (HS.frameOf x))) (ensures (HS.live_region h' (HS.frameOf x))) =
  modifies_liveness_insensitive_region_mreference loc_none l2 h h' x

let modifies_liveness_insensitive_region_buffer_weak
  (l2: loc) (h: HS.mem) (h': HS.mem) (#a: Type0) (#rrel: srel a) (#rel: srel a)
  (x: mbuffer a rrel rel)
  : Lemma
    (requires
      (modifies l2 h h' /\ loc_includes region_liveness_insensitive_locs l2 /\
        HS.live_region h (frameOf x))) (ensures (HS.live_region h' (frameOf x))) =
...
val fresh_frame_modifies: h0: HS.mem -> h1: HS.mem ->
  Lemma (requires (HS.fresh_frame h0 h1)) (ensures (modifies loc_none h0 h1))

val new_region_modifies: m0: HS.mem -> r0: HS.rid -> col: option int ->
  Lemma
    (requires
      (HST.is_eternal_region r0 /\ HS.live_region m0 r0 /\
        (None? col \/ HS.is_eternal_color (Some?.v col))))
    (ensures
      (let _, m1 = HS.new_eternal_region m0 r0 col in
        modifies loc_none m0 m1))
    [SMTPat (HS.new_eternal_region m0 r0 col)]

val popped_modifies: h0: HS.mem -> h1: HS.mem ->
  Lemma (requires (HS.popped h0 h1))
...
val modifies_ralloc_post: 
  ...
  h': HS.mem ->
  Lemma (requires (HST.ralloc_post i init h x h'))
...
val modifies_salloc_post: 
  ...
  h': HS.mem ->
  Lemma (requires (HST.salloc_post init h x h'))
    (ensures (modifies loc_none h h'))
...
val modifies_free: 
  ...
  Lemma (modifies (loc_freed_mreference r) m (HS.free r m)) [SMTPat (HS.free r m)]

/// Another compatibility lemma

val modifies_none_modifies: h1: HS.mem -> h2: HS.mem ->
  Lemma (requires (HST.modifies_none h1 h2))
...
val modifies_upd: 
  ...
  v: t ->
  h: HS.mem ->
  Lemma (requires (HS.contains h r))
...
val loc_unused_in_not_unused_in_disjoint: h: HS.mem ->
  Lemma (loc_disjoint (loc_unused_in h) (loc_not_unused_in h))

val not_live_region_loc_not_unused_in_disjoint: h0: HS.mem -> r: HS.rid ->
  Lemma (requires (~(HS.live_region h0 r)))
    (ensures (loc_disjoint (loc_region_only false r) (loc_not_unused_in h0)))

let fresh_frame_loc_not_unused_in_disjoint (h0: HS.mem) (h1: HS.mem)
  : Lemma (requires (HS.fresh_frame h0 h1))
    (ensures (loc_disjoint (loc_region_only false (HS.get_tip h1)) (loc_not_unused_in h0)))
    [SMTPat (HS.fresh_frame h0 h1)] = not_live_region_loc_not_unused_in_disjoint h0 (HS.get_tip h1)

val live_loc_not_unused_in: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
  h: HS.mem ->
  Lemma (requires (live h b))
    (ensures (loc_includes (loc_not_unused_in h) (loc_addr_of_buffer b)))
    [SMTPat (live h b)]

val unused_in_loc_unused_in: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
  ...
  Lemma (requires (unused_in b h))
    (ensures (loc_includes (loc_unused_in h) (loc_addr_of_buffer b)))
    [SMTPat (unused_in b h)]

val modifies_address_liveness_insensitive_unused_in: h: HS.mem -> h': HS.mem ->
  Lemma (requires (modifies (address_liveness_insensitive_locs) h h'))
    (ensures
      (loc_includes (loc_not_unused_in h') (loc_not_unused_in h) /\
        loc_includes (loc_unused_in h) (loc_unused_in h')))

/// Addresses that have not been allocated yet can be removed from
/// modifies clauses.

val modifies_only_not_unused_in: l: loc -> h: HS.mem -> h': HS.mem ->
  Lemma (requires (modifies (loc_union (loc_unused_in h) l) h h')) (ensures (modifies l h h'))
...
val mreference_live_loc_not_unused_in: 
  ...
  Lemma (requires (HS.contains h r))
    (ensures
      (loc_includes (loc_not_unused_in h) (loc_freed_mreference r) /\
        loc_includes (loc_not_unused_in h) (loc_mreference r)))
...
val mreference_unused_in_loc_unused_in: 
  ...
  Lemma (requires (HS.unused_in r h))
    (ensures
      (loc_includes (loc_unused_in h) (loc_freed_mreference r) /\
        loc_includes (loc_unused_in h) (loc_mreference r)))
    [SMTPat (HS.unused_in r h)]


let unused_in_not_unused_in_disjoint_2 (l1: loc) (l2: loc) (l1': loc) (l2': loc) (h: HS.mem)
  : Lemma
    (requires
      (loc_includes (loc_unused_in h) l1 /\ loc_includes (loc_not_unused_in h) l2 /\
        loc_includes l1 l1' /\ loc_includes l2 l2'))
    (ensures (loc_disjoint l1' l2'))
    [
      SMTPat (loc_disjoint l1' l2');
      SMTPat (loc_includes (loc_unused_in h) l1);
      SMTPat (loc_includes (loc_not_unused_in h) l2)
    ] =
...
val modifies_inert: s: loc -> h1: HS.mem -> h2: HS.mem -> GTot Type0

val modifies_inert_intro: s: loc -> h1: HS.mem -> h2: HS.mem ->
  Lemma (requires (modifies s h1 h2)) (ensures (modifies_inert s h1 h2)) [SMTPat (modifies s h1 h2)]

val modifies_inert_live_region: s: loc -> h1: HS.mem -> h2: HS.mem -> r: HS.rid ->
  Lemma
    (requires
      (modifies_inert s h1 h2 /\ loc_disjoint s (loc_region_only false r) /\ HS.live_region h1 r))
    (ensures (HS.live_region h2 r))
    [
      SMTPatOr
      [
...
val modifies_inert_mreference_elim: 
  ...
  h: HS.mem ->
  h': HS.mem ->
  Lemma (requires (loc_disjoint (loc_mreference b) p /\ HS.contains h b /\ modifies_inert p h h'))
    (ensures (HS.contains h' b /\ HS.sel h b == HS.sel h' b))
    [
      SMTPatOr
      [
...
val modifies_inert_buffer_elim: 
  ...
  h: HS.mem ->
  h': HS.mem ->
  Lemma (requires (loc_disjoint (loc_buffer b) p /\ live h b /\ modifies_inert p h h'))
    (ensures (live h' b /\ (as_seq h b == as_seq h' b)))
    [
      SMTPatOr
      [
...
val modifies_inert_liveness_insensitive_mreference_weak: 
  ...
  Lemma
    (requires
      (modifies_inert l h h' /\ loc_includes address_liveness_insensitive_locs l /\ HS.contains h x)
    )
    (ensures (HS.contains h' x))
    [
      SMTPatOr
      [
        [SMTPat (HS.contains h x); SMTPat (modifies_inert l h h')];
        [SMTPat (HS.contains h' x); SMTPat (modifies_inert l h h')]
      ]
    ]
...
val modifies_inert_liveness_insensitive_buffer_weak: 
  ...
  Lemma
    (requires
      (modifies_inert l h h' /\ loc_includes address_liveness_insensitive_locs l /\ live h x))
    (ensures (live h' x))
    [
      SMTPatOr
      [
...
val modifies_inert_liveness_insensitive_region_weak: 
  ...
  Lemma
    (requires
      (modifies_inert l2 h h' /\ loc_includes region_liveness_insensitive_locs l2 /\
        HS.live_region h x))
    (ensures (HS.live_region h' x))
    [
      SMTPatOr
...
val fresh_frame_modifies_inert: h0: HS.mem -> h1: HS.mem ->
  Lemma (requires (HS.fresh_frame h0 h1))
    (ensures (modifies_inert loc_none h0 h1))
    [SMTPat (HS.fresh_frame h0 h1)]

val popped_modifies_inert: h0: HS.mem -> h1: HS.mem ->
  Lemma (requires (HS.popped h0 h1))
    (ensures (modifies_inert (loc_region_only false (HS.get_tip h0)) h0 h1))
    [SMTPat (HS.popped h0 h1)]

val modifies_inert_loc_unused_in: l: loc -> h1: HS.mem -> h2: HS.mem -> l': loc ->
  Lemma
    (requires
      (modifies_inert l h1 h2 /\ loc_includes address_liveness_insensitive_locs l /\
        loc_includes (loc_unused_in h2) l'))
    (ensures (loc_includes (loc_unused_in h1) l'))
    [SMTPat (modifies_inert l h1 h2); SMTPat (loc_includes (loc_unused_in h2) l')]

/// Legacy shorthands for disjointness and inclusion of buffers
///

let disjoint
  (#a1: Type0) (#a2: Type0) (#rrel1: srel a1) (#rel1: srel a1) (#rrel2: srel a2) (#rel2: srel a2)
  (b1: mbuffer a1 rrel1 rel1) (b2: mbuffer a2 rrel2 rel2)
  : GTot Type0 = loc_disjoint (loc_buffer b1) (loc_buffer b2)

let includes
  (#a1: Type0) (#a2: Type0) (#rrel1: srel a1) (#rel1: srel a1) (#rrel2: srel a2) (#rel2: srel a2)
  (b1: mbuffer a1 rrel1 rel1) (b2: mbuffer a2 rrel2 rel2)
  : GTot Type0 = loc_includes (loc_buffer b1) (loc_buffer b2) /\ (g_is_null b1 <==> g_is_null b2)
...
val disjoint_neq: 
  ...
  Lemma (requires (disjoint b1 b2 /\ U32.v (len b1) > 0)) (ensures (~(b1 === b2)))
...
val includes_live: 
  ...
  #rel2: srel a ->
  h: HS.mem ->
  larger: mbuffer a rrel rel1 ->
  smaller: mbuffer a rrel rel2 ->
  Lemma (requires (includes larger smaller))
...
val includes_frameOf_as_addr: 
  ...
  #rrel2: srel a2 ->
  #rel2: srel a2 ->
  larger: mbuffer a1 rrel1 rel1 ->
  smaller: mbuffer a2 rrel2 rel2 ->
  Lemma (requires (includes larger smaller))
    (ensures
      (g_is_null larger == g_is_null smaller /\ frameOf larger == frameOf smaller /\
        as_addr larger == as_addr smaller))
    [SMTPat (includes larger smaller)]

/// 
/// Useful shorthands for pointers, or maybe-null pointers

inline_for_extraction
type mpointer (a: Type0) (rrel: srel a) (rel: srel a) = b: mbuffer a rrel rel {length b == 1}
...
unfold
let deref (#a: Type0) (#rrel: srel a) (#rel: srel a) (h: HS.mem) (x: mpointer a rrel rel) =
  get h x 0
...
val pointer_distinct_sel_disjoint: 
  ...
  h: HS.mem ->
  Lemma (requires (live h b1 /\ live h b2 /\ get h b1 0 =!= get h b2 0)) (ensures (disjoint b1 b2))
...
val is_null: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  HST.Stack bool
...
val msub: 
  ...
  HST.Stack (mbuffer a rrel sub_rel)
    (requires
      (fun h -> U32.v i + U32.v len <= length b /\ compatible_sub b i len sub_rel /\ live h b))
...
val moffset: 
  ...
  HST.Stack (mbuffer a rrel sub_rel)
    (requires
      (fun h -> U32.v i <= length b /\ compatible_sub b i (U32.sub (len b) i) sub_rel /\ live h b))
...
val index: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> i: U32.t ->
  HST.Stack a
...
val g_upd_seq: 
  ...
  h:
  HS.mem
    { (* TODO: we should not need this rel precondition *)
      live h b /\ rel (as_seq h b) s } ->
  GTot HS.mem

/// A lemma specifying `g_upd_seq` in terms of its effect on the
/// buffer's underlying sequence

val g_upd_seq_as_seq: 
  #a: Type0 ->
  #rrel: srel a ->
  #rel: srel a ->
  b: mbuffer a rrel rel ->
  ...
  (let h' = g_upd_seq b s h in
    (Seq.length s > 0 ==> not (g_is_null b)) /\ modifies (loc_buffer b) h h' /\ live h' b /\
    as_seq h' b == s)
...
let g_upd
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (i: nat{i < length b}) (v: a)
  (h: HS.mem{live h b /\ rel (as_seq h b) (Seq.upd (as_seq h b) i v)})
  : GTot HS.mem = g_upd_seq b (Seq.upd (as_seq h b) i v) h

/// ``upd b i v`` writes ``v`` to the memory, at offset ``i`` of
/// buffer ``b``. KreMLin compiles it as ``b[i] = v``.

val upd': #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> i: U32.t -> v: a ->
  HST.Stack unit
    (requires
      (fun h ->
...
inline_for_extraction
let upd (#a: Type0) (#rrel: srel a) (#rel: srel a) (b: mbuffer a rrel rel) (i: U32.t) (v: a)
  : HST.Stack unit
    (requires
      (fun h ->
          live h b /\ U32.v i < length b /\ rel (as_seq h b) (Seq.upd (as_seq h b) (U32.v i) v)))
...
val recallable: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot Type0

val recallable_null: #a: Type0 -> #rrel: srel a -> #rel: srel a ->
  Lemma (recallable (mnull #a #rrel #rel)) [SMTPat (recallable (mnull #a #rrel #rel))]

val recallable_includes: 
  #a1: Type0 ->
  #a2: Type0 ->
  #rrel1: srel a1 ->
  #rel1: srel a1 ->
...
val recall: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  HST.Stack unit (requires (fun _ -> recallable b)) (ensures (fun m0 _ m1 -> m0 == m1 /\ live m1 b))

/// Deallocation. A buffer that was allocated by ``malloc`` (see below)
/// can be ``free`` d.

val freeable: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel -> GTot Type0

val free: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  HST.ST unit
    (requires (fun h0 -> live h0 b /\ freeable b))
    (ensures
      (fun h0 _ h1 ->
          (not (g_is_null b)) /\
          Set.equal (Map.domain (HS.get_hmap h1)) (Map.domain (HS.get_hmap h0)) /\
          (HS.get_tip h1) == (HS.get_tip h0) /\ modifies (loc_addr_of_buffer b) h0 h1 /\
          HS.live_region h1 (frameOf b)))

val freeable_length: #a: Type0 -> #rrel: srel a -> #rel: srel a -> b: mbuffer a rrel rel ->
  Lemma (requires (freeable b)) (ensures (length b > 0)) [SMTPat (freeable b)]

val freeable_disjoint: 
  #a1: Type0 ->
  #a2: Type0 ->
  #rrel1: srel a1 ->
  #rel1: srel a1 ->
...
let freeable_disjoint'
  ...
  : Lemma (requires (freeable b1 /\ length b2 > 0 /\ disjoint b1 b2))
    ...
    [SMTPat (freeable b1); SMTPat (disjoint b1 b2)] = freeable_disjoint b1 b2
...
let alloc_post_static
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (r: HS.rid) (len: nat) (b: mbuffer a rrel rel) =
  (not (g_is_null b)) /\ frameOf b == r /\ length b == len

unfold
let alloc_post_common
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (r: HS.rid) (len: nat) (b: mbuffer a rrel rel)
  (h0: HS.mem) (h1: HS.mem) =
  alloc_post_static r len b /\ unused_in b h0 /\ live h1 b /\
  Set.equal (Map.domain (HS.get_hmap h1)) (Map.domain (HS.get_hmap h0)) /\
  (HS.get_tip h1) == (HS.get_tip h0) /\ modifies loc_none h0 h1
...
val mgcmalloc: #a: Type0 -> #rrel: srel a -> r: HS.rid -> init: a -> len: U32.t ->
...
val mmalloc: #a: Type0 -> #rrel: srel a -> r: HS.rid -> init: a -> len: U32.t ->
...
val malloca: #a: Type0 -> #rrel: srel a -> init: a -> len: U32.t ->
...
unfold
let alloc_of_list_pre (#a: Type0) (init: list a) =
  normalize (0 < FStar.List.Tot.length init) /\
  normalize (FStar.List.Tot.length init <= UInt.max_int 32)
unfold
let alloc_of_list_post
  (#a: Type0) (#rrel: srel a) (#rel: srel a) (len: nat) (buf: mbuffer a rrel rel) =
...
unfold
let gcmalloc_of_list_pre (#a: Type0) (init: list a) =
  normalize (FStar.List.Tot.length init <= UInt.max_int 32)
...
val blit: 
  ...
  HST.Stack unit
    (requires
      (fun h ->
          live h src /\ live h dst /\ disjoint src dst /\ U32.v idx_src + U32.v len <= length src /\
    ...
    (ensures
      (fun h _ h' ->
          modifies (loc_buffer dst) h h' /\ live h' dst /\
          Seq.slice (as_seq h' dst) (U32.v idx_dst) (U32.v idx_dst + U32.v len) ==
          Seq.slice (as_seq h src) (U32.v idx_src) (U32.v idx_src + U32.v len) /\
          Seq.slice (as_seq h' dst) 0 (U32.v idx_dst) == Seq.slice (as_seq h dst) 0 (U32.v idx_dst) /\
