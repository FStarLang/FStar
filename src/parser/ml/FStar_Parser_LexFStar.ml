...
let regexp letter = upper | lower
let regexp digit  = ['0'-'9']
let regexp hex    = ['0'-'9'] | ['A'-'F'] | ['a'-'f']

(* -------------------------------------------------------------------- *)
let regexp truewhite = [' ']
let regexp offwhite  = ['\t']
let regexp anywhite  = truewhite | offwhite
let regexp newline   = ('\n' | '\r' '\n')
...
let regexp floate     = digit+ ('.' digit* )? ('e'| 'E') ['+' '-']? digit+
let regexp float      = floatp | floate
let regexp bigint     = integer 'I'
let regexp bignum     = integer 'N'
let regexp ieee64     = float
let regexp ieee32     = float ('f' | 'F')
let regexp decimal    = (float | integer) ('m' | 'M')
let regexp xieee32    = xinteger 'l' 'f'
...
(* -------------------------------------------------------------------- *)
let regexp escape_char = ('\\' ["\\\"'bfntrv0"])
let regexp char        = [^'\\''\n''\r''\t''\b'] | escape_char
...
let rec token = lexer
 ...
 | ( '\'' (char as c) '\'' ) | ( '\'' (char as c) '\'' 'B' ) ->
   CHAR (match c.[0] with | '\\' -> char_of_ec c.[1] | _ -> c.[0])
 | '`' -> BACKTICK
 | ident ->
 ...
 | (integer | xinteger | float) ident_char+ -> failwith "This is not a valid numeric literal."
 ...
 | "//" [^ '\n''\r']* -> push_one_line_comment lexbuf; token lexbuf

 | '"' -> string (Buffer.create 0) lexbuf

 | truewhite+ | offwhile+ -> token lexbuf
