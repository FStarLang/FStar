...
let tc_one_fragment curmod dsenv env frag =
  try
    ...
    | Parser.Driver.Decls (dsenv, decls) -> begin
        match curmod with
...
type stack_elt = (option<modul> * Parser.DesugarEnv.env * Tc.Env.env)
...
let interactive_mode dsenv env =
  if Option.isSome !Options.codegen then
    ...
    let rec go (stack:stack) (curmod:option<modul>) dsenv env = begin
      ...
      | Code (text, (ok, fail)) ->
          ...
          let res = tc_one_fragment curmod dsenv_mark env_mark text in begin
            match res with
            | Some (curmod, dsenv, env) ->
                if !Tc.Errors.num_errs = 0 then begin
                  Util.print1 "\n%s\n" ok;
                  let dsenv, env = commit_mark dsenv env in
                  go stack curmod dsenv env
                  end
                else fail curmod dsenv_mark env_mark
            | _ -> fail curmod dsenv_mark env_mark
            end
    end in
    go [] None dsenv env
...
let go _ =
  ...
  match res with
    ...
    | GoOn ->
        if !Options.dep <> None then Parser.Dep.print (Parser.Dep.collect filenames)
        else if !Options.universes then
          (test_universes filenames |> ignore;
...
let main () =
  ...
  with | e ->
    ...
    else if not (Util.handleable e) then
