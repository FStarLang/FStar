...
(* Section 108.  And the name does not survive on its own: §65.4's argument
   for rooting a type *abbreviation* declared in an entry module --- Custard
   unfolds it, so nothing refers to the name and it is dead by construction
   --- is this declaration's argument too, the collapse above being exactly
   an unfolding.  Section 110 is where that is decided; it cannot be decided
   here, because whether the name is worth keeping is a question about the
   *live* program and nothing in this file knows which that is. *)
