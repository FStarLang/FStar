...
(* Section 125.7.  The exceptions emitted under their plain F* identifier
   ...
   is what [FStar.Exception.string_of_exn] is realized as -- prints the
   constructor, so the mangling is *observable* here in a way it is nowhere
   else: [StringOfExn.A] came out as [StringOfExn.StringOfExn_A].

   Mangling exists to keep one flat file collision-free (section 12.7), so it
   is applied where there is a collision and not otherwise.  A variant
...
(* OCaml's own exception constructors.  An [exception Not_found] of ours would
   be legal and would shadow [Stdlib.Not_found] for the rest of the file,
   where a hand-written realization -- which is not generated and does not
   know -- may still mean the original.  A collision with the language is a
   collision. *)
let predefined_ctors : list string = [
  "Out_of_memory"; "Sys_error"; "Failure"; "Invalid_argument"; "End_of_file";
  "Division_by_zero"; "Not_found"; "Match_failure"; "Stack_overflow";
  "Sys_blocked_io"; "Assert_failure"; "Undefined_recursive_module"; "Exit";
  "None"; "Some"; "Nil"; "Cons"
]
...
let reject_fwidth (fw:fwidth) : ML unit =
  ...
  (* Section 66.  Name the width the program actually used, in the reader's
...
let build_tables (homes : SMap.t string) (p:program) : ML unit =
  ...
  (* Section 125.7.  Last, because it reads [at_home] through
     ...
     else in the file wants that spelling.  [taken] is every other constructor
