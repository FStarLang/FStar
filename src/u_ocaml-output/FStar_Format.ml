    ...
    fun docs  ->
      ...
      | Doc sep ->
          let select uu____79 =
            match uu____79 with
            | Doc d ->
                (match d = "" with | true  -> None | uu____83 -> Some d) in
...
let nest: Prims.int -> doc -> doc =
  fun i  -> fun uu____102  -> match uu____102 with | Doc d -> Doc d
let align: doc Prims.list -> doc =
  fun docs  ->
    let uu____109 = combine hardline docs in
    match uu____109 with | Doc doc -> Doc doc
let hbox: doc -> doc = fun d  -> d
let pretty: Prims.int -> doc -> Prims.string =
  fun sz  -> fun uu____119  -> match uu____119 with | Doc doc -> doc