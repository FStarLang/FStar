...
let qop_to_string: qop -> Prims.string =
  fun uu___86_804  ->
    match uu___86_804 with | Forall  -> "forall" | Exists  -> "exists"
let op_to_string: op -> Prims.string =
  fun uu___87_807  ->
    match uu___87_807 with
    | TrueOp  -> "true"
    | FalseOp  -> "false"
    | Not  -> "not"
    | And  -> "and"
      ...
      | Caption c ->
          let _0_294 =
            FStar_All.pipe_right (FStar_Util.splitlines c)
              (fun uu___90_2179  ->
                 match uu___90_2179 with | [] -> "" | h::t -> h) in
          FStar_Util.format1 "\n; %s" _0_294
...
let mk_Valid: term -> term =
  fun t  ->
    ...
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___91_2564 = unboxBool t1 in
        {
          tm = (uu___91_2564.tm);
          freevars = (uu___91_2564.freevars);
          rng = (t.rng)
        }
