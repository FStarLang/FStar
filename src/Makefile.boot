...
%.checked.lax: %
	$(FSTAR_C) $*
	touch $@      #we touch the file, because if F* determined
		      #that the .checked.lax file was already up to
		      #date, it doesn't touch it. Touching it here
		      #ensures that if this rule is successful then
		      #%.checked.lax is more recent than its
		      #dependences
