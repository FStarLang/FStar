...
(* Force FStarC_Version module to be linked and executed, setting version info *)
let () = ignore (Fstarcompiler_stage1.FStarC_Version.dummy)

(* Force FStarC_Hooks to be linked - it sets up critical callbacks at init time *)
let () = ignore (Fstarcompiler_stage1.FStarC_Hooks.uu___0)
...
let () =
  (* On Unix, if we write to a pipe that tries to send something
     ...
     to detect this. *)
  if Fstarcompiler_stage1.FStarC_Platform.unix then
  ...
  if Fstarcompiler_stage1.FStarC_Platform.unix then (
    let hdlr (_s:int) =
      Fstarcompiler_stage1.FStarC_Util.kill_all ();
      exit 130
    in
  ...
  Fstarcompiler_stage1.FStarC_Main.main ()
