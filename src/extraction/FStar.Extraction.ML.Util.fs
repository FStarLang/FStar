...
let not_implemented_warning r t msg =
    Errors.maybe_fatal_error r (Errors.Fatal_CallNotImplemented, BU.format2 ". Tactic %s will not run natively because %s.\n" t msg)
...
let mk_tactic_unembedding (args: list<mlexpr'>) =
    ...
    | n ->
        raise (Fatal_CallNotImplemented (BU.format1 "Unembedding not defined for tactics of %s arguments\n" (BU.string_of_int n))) in
       ...
       (match (FStar.Syntax.Subst.compress h).n with
        | Tm_uinst (h', _) ->
           (match (FStar.Syntax.Subst.compress h').n with
            ...
            | _ ->
                raise (Fatal_CallNotImplemented ("Type term not defined for higher-order type" ^ (Print.term_to_string (FStar.Syntax.Subst.compress h')))))
        | _ ->
            raise (Fatal_CallNotImplemented "Impossible\n"))
     | _ ->
         raise (Fatal_CallNotImplemented ("Type term not defined for " ^ (Print.term_to_string (FStar.Syntax.Subst.compress t))))
    ...
    | Tm_app (h, args) ->
        (match (FStar.Syntax.Subst.compress h).n with
         | Tm_uinst (h', _) ->
            let ht, hargs, type_arg, is_tactic =
                (match (FStar.Syntax.Subst.compress h').n with
                 ...
                 | _ ->
                     raise (Fatal_CallNotImplemented ("Embedding not defined for higher-order type " ^ (Print.term_to_string (FStar.Syntax.Subst.compress h'))))) in
            let hargs =
                match m with
            ...
            if is_tactic then
                match m with
                | Embed ->
                    raise (Fatal_CallNotImplemented "Embedding not defined for tactic type\n")
         ...
         | _ ->
             raise (Fatal_CallNotImplemented "Impossible\n"))
    | _ ->
        raise (Fatal_CallNotImplemented ("Embedding not defined for type " ^ (Print.term_to_string (FStar.Syntax.Subst.compress t))))
...
let mk_interpretation_fun tac_lid assm_lid t bs =
    ...
    with Fatal_CallNotImplemented msg ->
