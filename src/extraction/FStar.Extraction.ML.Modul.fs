...
let rec extract_meta x =
  ...
  | { n = Tm_meta (x, _) } ->
      extract_meta x
  | a ->
      Errors.maybe_fatal_error a.pos (Errors.Warning_UnrecognizedAttribute, (BU.format1 "Unrecognized attribute (%s), valid attributes are `c_inline`, `substitute`, and `gc`.\n")
