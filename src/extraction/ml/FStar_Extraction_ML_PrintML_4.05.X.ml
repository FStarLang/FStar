...
let build_constant (c: mlconstant): Parsetree.constant =
  ...
  | MLC_Unit -> failwith "Unit case not handled"
...
let rec build_core_type ?(annots:string Asttypes.loc list = []) (ty: mlty): core_type =
  let t =
  match ty with
...
let rec build_expr ?annot (e: mlexpr): expression =
  ...
  | Some ts ->
          ...
          let loc_vars = List.map (fun v -> {txt=v; loc=no_location}) vars in
          let ty = snd ts in
          Exp.constraint_ e' (build_core_type ~annots:loc_vars ty)
