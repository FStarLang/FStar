...
module FStar.Extraction.SMT.Code
open FStar.All
...
let smt2_reserved = [ "assert"; "check-sat"; "declare-fun";
...
let mlpath_of_mlpath (currentModule : mlsymbol) (x : mlpath) : mlpath =
    ...
    | "Prims.Some" //-> ([], "Some")
    | "Prims.None" -> failwith "A sort has to be created for option 'a"  //([], "None")
    | _ ->
     let ns, x = x in
...
let rec doc_of_mltype' (currentModule : mlsymbol) (outer : level) (ty : mlty) =
    ...
    | MLTY_Fun (t1, _, t2) ->
        let d1 = parens (doc_of_mltype currentModule (t_prio_fun, Left ) t1) in
        let d2 = parens (doc_of_mltype currentModule (t_prio_fun, Right) t2) in
        maybe_paren outer t_prio_fun (hbox (reduce1 [d1; text " "; d2]))

    | MLTY_Top ->
        failwith "Obj type should not appear"
...
let rec doc_of_expr (currentModule : mlsymbol) (outer : level) (e : mlexpr) : doc =
    ...
    | MLE_Let ((rec_, _, lets), body) ->
        ...
        parens (combine hardline [pre; doc; reduce1 [// text "in";
                                                     body]])
    ...
    | MLE_Fun (ids, body) ->
        ...
        let body = doc_of_expr currentModule (min_op_prec, NonAssoc) body in
        let doc  = reduce1 [// text "fun"; reduce1 ids; text "->";
                            body] in
        parens doc
...
and doc_of_lets (currentModule : mlsymbol) (rec_, top_level, lets) =
    let for1 {mllb_name=name; mllb_tysc=tys; mllb_def=e; print_typ=pt} =
        let e' =  e in
        let outer = (min_op_prec, NonAssoc) in
        let e   = doc_of_expr currentModule  outer e in
        let ids  =
            match e'.expr with
            | MLE_Fun (ids, _) ->
                let bvar_annot x xt =
                    reduce1 [text "("; text x ;
                          (match xt with
                           | Some xxt -> reduce1 [text " "; doc_of_mltype currentModule outer xxt]
                           | _ -> text "");
                     text ")"] in
                List.map (fun ((x, _),xt) -> bvar_annot x (Some xt)) ids
             | _ -> [ text "()" ] in
        //let f x = x
        //let f = fun x -> x
...
//                       parens (ty)
//             else text "" in
        reduce1 [text (idsym name); reduce1 ids; hardline; text " "; e] in

    let letdoc = if rec_ = Rec then failwith "Recursion is not supported" // reduce1 [text "let"; text "rec"]
                else if top_level then text "define-fun"
                else text "let" in

    let lets = List.map for1 lets in
    let lets = List.mapi (fun i doc ->
        parens (reduce1 [(if i = 0 then letdoc else text "and"); doc]))
        lets in
...
let doc_of_mltydecl (currentModule : mlsymbol) (decls : mltydecl) =
    let for1 (_, x, mangle_opt, tparams, body) =
        ...
        let forbody (body : mltybody) =
            ...
            | MLTD_Record fields ->
                begin
                failwith "Records are not yet supported"
            ...
            | MLTD_DType ctors ->
                failwith "DataConstructors are not yet supported"
        ...
        let doc = reduce1 [text (ptsym currentModule  ([], x)); tparams] in
    ...
    let doc = if (List.length doc >0) then
        (if (List.length doc > 1) then failwith "Cannot handle mutually inductive types yet"
         else  parens (reduce1 [text "define-sort"; combine (text " \n and ") doc]))
         else text "" in
    doc
...
let rec doc_of_mllib_r (MLLib mllib) =
    ...
    and for1_mod istop (mod_name, sigmod, MLLib sub) =
        ...
        let sub  = List.map (fun x -> reduce [x; hardline; hardline]) sub in
        let prefix = [text ";(set-logic QF_ABV)";
                             hardline;
                             text "(define-sort uint32 () (_ BitVec 32))"] in
        reduce <| (prefix @ [
            head;
            // hardline;
            // text "open Prims"
