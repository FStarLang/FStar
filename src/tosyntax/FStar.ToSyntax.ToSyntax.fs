...
let rec desugar_data_pat env p is_mut : (env_t * bnd * list<Syntax.pat>) = 
  let check_linear_pattern_variables pats r : unit =
    let nonLinearPatternError s =
        raise_error ( Errors.Fatal_NonLinearPatternNotPermitted, 
                      "Non-linear patterns are not permitted. " ^ s) 
                    r
    in
      ...
      | Pat_cons(_, pats) ->
        pats |> List.fold_left (fun out (p, _) ->
                                  ...
                                  then BU.set_union out (pat_vars p)
                                    ...
                                    BU.set_elements intersection
                                    |> List.hd
                                    |> (fun bv -> bv.ppname.idText)
                                    |> BU.format1 "%s appears more than once in this pattern."
    ...
    | p::ps -> 
      ...
      ps |> List.iter (fun p -> if BU.set_eq pvars (pat_vars p) then () 
                                else BU.set_symmetric_difference pvars (pat_vars p)
                                     |> BU.set_elements
                                     |> List.hd
                                     |> (fun bv -> bv.ppname.idText)
                                     |> BU.format1 "%s does not appear in all pattern branches"
                                     |> nonLinearPatternError )    
  in
