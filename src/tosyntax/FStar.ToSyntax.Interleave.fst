...
let rec prefix_with_iface_decls
   ...
   | iface_hd::iface_tl -> begin
     ...
     | Tycon(_, _, tys) when (tys |> Util.for_some (function (TyconAbstract _)  -> true | _ -> false)) ->
        raise_error impl.drange Errors.Fatal_AbstractTypeDeclarationInInterface [
     ...
     | Val(x, _) ->
       ...
       if not defines_x then (
         ...
         then
           raise_error impl.drange Errors.Fatal_WrongDefinitionOrder [
         ...
         let rec aux mutuals iface =
           ...
           | y::ys, iface_hd::iface_tl when Option.isSome <| List.tryFind (is_val (ident_of_lid y)) iface_tl ->
             raise_error iface_hd.drange Errors.Fatal_WrongDefinitionOrder [
...
let check_initial_interface (iface:list decl) =
    let rec aux iface =
        ...
        | hd::tl -> begin
            match hd.d with
            | Tycon(_, _, tys) when (tys |> Util.for_some (function (TyconAbstract _)  -> true | _ -> false)) ->
              raise_error hd.drange Errors.Fatal_AbstractTypeDeclarationInInterface
                "Interface contains an abstract 'type' declaration; use 'val' instead"

            | Val(x, t) ->  //we have a 'val x' in the interface
              if Util.for_some (is_definition_of x) tl
              then raise_error hd.drange Errors.Fatal_BothValAndLetInInterface 
              ...
              then raise_error hd.drange Errors.Fatal_AssumeValInInterface
...
let ml_mode_check_initial_interface mname (iface:list decl) =
  iface |> List.filter (fun d ->
    ...
    | Tycon(_, _, tys)
      ...
      raise_error d.drange Errors.Fatal_AbstractTypeDeclarationInInterface
        "Interface contains an abstract 'type' declaration; use 'val' instead"
    | Tycon _
    | Val _
    | Open _
...
let initialize_interface (mname:Ident.lid) (l:list decl) : E.withenv unit =
  fun (env:E.env) ->
    ...
    | Some _ ->
      raise_error (Ident.range_of_lid mname) Errors.Fatal_InterfaceAlreadyProcessed
    ...
    | Module(l, impls) -> begin
      ...
      | Some iface ->
        ...
        | _::_ when expect_complete_modul ->
          ...
          log_issue (Ident.range_of_lid l) Errors.Fatal_InterfaceNotImplementedByModule [
