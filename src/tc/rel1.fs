...
let close bs f = 
   List.fold_right (fun b f -> match b with 
      ...
      | Inl a, _ -> Util.mk_forallT a f) bs f
//
...
let close_guard (bs:binders) g = match g with 
  | Trivial -> g
  | NonTrivial f -> NonTrivial <| close bs f
//    List.fold_right (fun b f -> match b with 
//      | Inr x, _ -> Util.mk_forall x f
//      | Inl a, _ -> Util.mk_forallT a f) bs f
...
type solution = 
    | Success of list<uvar_inst> * guard_f
...
let close_predicate f = match (Util.compress_typ f).n with
    | Typ_lam(bs, f) -> close bs f
    | _ -> f
//    
//    
//    match (Util.compress_kind f.tk).n with 
//    | Kind_arrow(bs, {n=Kind_type}) ->
//        begin match (Util.compress_typ f).n with 
//            | Typ_lam(bs')
//         close bs f
//    | Kind_type -> f
...
let try_teq env t1 t2 : option<guard_f> = 
