...
let unembed_binder (t:term) : option<binder> =
    ...
    with | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "binder", (BU.format1 "Not an embedded binder: %s" (Print.term_to_string t)));
        None
...
let unembed_fvar (t:term) : option<fv> =
    try Some (U.un_alien t |> FStar.Dyn.undyn)
    with | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "fvar", (BU.format1 "Not an embedded fvar: %s" (Print.term_to_string t)));
        None
...
let unembed_comp (t:term) : option<comp> =
    try Some (U.un_alien t |> FStar.Dyn.undyn)
    with | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "comp", (BU.format1 "Not an embedded comp: %s" (Print.term_to_string t)));
        None
...
let unembed_env (t:term) : option<Env.env> =
    try Some (U.un_alien t |> FStar.Dyn.undyn)
    with | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "env", (BU.format1 "Not an embedded env: %s" (Print.term_to_string t)));
        None
...
let unembed_const (t:term) : option<vconst> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "vconst", (BU.format1 "Not an embedded vconst: %s" (Print.term_to_string t)));
        None
...
let rec unembed_pattern (t : term) : option<pattern> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "pattern", (BU.format1 "Not an embedded pattern: %s" (Print.term_to_string t)));
        None
...
let unembed_aqualv (t : term) : option<aqualv> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "aqualv", (BU.format1 "Not an embedded aqualv: %s" (Print.term_to_string t)));
        None
...
let unembed_term_view (t:term) : option<term_view> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "term_view", (BU.format1 "Not an embedded term_view: %s" (Print.term_to_string t)));
        None
...
let unembed_comp_view (t : term) : option<comp_view> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "comp_view", (BU.format1 "Not an embedded comp_view: %s" (Print.term_to_string t)));
        None
...
let unembed_order (t:term) : option<order> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "order", (BU.format1 "Not an embedded order: %s" (Print.term_to_string t)));
        None
...
let unembed_ctor (t:term) : option<ctor> =
    ...
    | Tm_fvar fv, [(nm, _); (t, _)] when S.fv_eq_lid fv ref_Ctor_lid ->
        ...
        Some <| Ctor (nm, t)))
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "ctor", (BU.format1 "Not an embedded ctor: %s" (Print.term_to_string t)));
        None
...
let unembed_sigelt_view (t:term) : option<sigelt_view> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.Warning_NotEmbedded "siglt_view", (BU.format1 "Not an embedded sigelt_view: %s" (Print.term_to_string t)));
        None
