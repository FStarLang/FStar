...
let is_smt_reifiable_effect : FStar_TypeChecker_Env.env  ->  FStar_Ident.lident  ->  Prims.bool = (fun en l -> ((FStar_TypeChecker_Env.is_reifiable_effect en l) && (

let uu____851 = (

let uu____853 = (

let uu____854 = (FStar_All.pipe_right l (FStar_TypeChecker_Env.norm_eff_name en))
in (FStar_All.pipe_right uu____854 (FStar_TypeChecker_Env.get_effect_decl en)))
in (FStar_All.pipe_right uu____853 FStar_Syntax_Util.is_layered))
in (not (uu____851)))))


let is_smt_reifiable_comp : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.comp  ->  Prims.bool = (fun en c -> (match (c.FStar_Syntax_Syntax.n) with
...
let is_smt_reifiable_rc : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.residual_comp  ->  Prims.bool = (fun en rc -> (is_smt_reifiable_effect en rc.FStar_Syntax_Syntax.residual_effect))


let is_smt_reifiable_function : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  Prims.bool = (fun en t -> (

let uu____898 = (

let uu____899 = (FStar_Syntax_Subst.compress t)
in uu____899.FStar_Syntax_Syntax.n)
in (match (uu____898) with
| FStar_Syntax_Syntax.Tm_arrow (uu____903, c) -> begin
