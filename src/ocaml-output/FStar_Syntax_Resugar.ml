  ...
  fun env  ->
    fun se  ->
      ...
      | FStar_Syntax_Syntax.Sig_polymonadic_subcomp
          (m,n,(uu____9568,t),uu____9570) ->
          let uu____9579 =
            let uu____9580 =
              let uu____9581 =
                let uu____9588 = resugar_term' env t  in (m, n, uu____9588)
                 in
              FStar_Parser_AST.Polymonadic_subcomp uu____9581  in
            decl'_to_decl se uu____9580  in
...
let (resugar_term : FStar_Syntax_Syntax.term -> FStar_Parser_AST.term) =
  fun t  -> let uu____9612 = noenv resugar_term'  in uu____9612 t 
let (resugar_sigelt :
  FStar_Syntax_Syntax.sigelt ->
    FStar_Parser_AST.decl FStar_Pervasives_Native.option)
  = fun se  -> let uu____9630 = noenv resugar_sigelt'  in uu____9630 se 
let (resugar_comp : FStar_Syntax_Syntax.comp -> FStar_Parser_AST.term) =
  fun c  -> let uu____9648 = noenv resugar_comp'  in uu____9648 c 
let (resugar_pat :
  FStar_Syntax_Syntax.pat ->
    FStar_Syntax_Syntax.bv FStar_Util.set -> FStar_Parser_AST.pattern)
  =
  fun p  ->
    fun branch_bv  ->
      let uu____9671 = noenv resugar_pat'  in uu____9671 p branch_bv
...
let (resugar_binder :
  ...
  =
  fun b  ->
    fun r  -> let uu____9705 = noenv resugar_binder'  in uu____9705 b r
  
let (resugar_tscheme : FStar_Syntax_Syntax.tscheme -> FStar_Parser_AST.decl)
  = fun ts  -> let uu____9730 = noenv resugar_tscheme'  in uu____9730 ts 
let (resugar_eff_decl :
  FStar_Range.range ->
    FStar_Syntax_Syntax.qualifier Prims.list ->
      FStar_Syntax_Syntax.eff_decl -> FStar_Parser_AST.decl)
  ...
  fun r  ->
    fun q  ->
      fun ed  ->
        let uu____9758 = noenv resugar_eff_decl'  in uu____9758 r q ed
