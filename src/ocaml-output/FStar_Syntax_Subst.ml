  ...
  fun s ->
    fun asc ->
      ...
      | (annot, topt) ->
          let annot1 =
            match annot with
            | Prims.Inl t -> let uu___1 = subst' s t in Prims.Inl uu___1
            | Prims.Inr c -> let uu___1 = subst_comp' s c in Prims.Inr uu___1 in
...
let rec (deep_compress :
  ...
  fun t ->
    ...
    let elim_bv x =
      let uu___ = deep_compress x.FStar_Syntax_Syntax.sort in
      {
        FStar_Syntax_Syntax.ppname = (x.FStar_Syntax_Syntax.ppname);
        FStar_Syntax_Syntax.index = (x.FStar_Syntax_Syntax.index);
        FStar_Syntax_Syntax.sort = uu___
    ...
    | FStar_Syntax_Syntax.Tm_let (lbs, t2) ->
        let elim_lb lb =
          let uu___ =
            match lb.FStar_Syntax_Syntax.lbname with
            | Prims.Inl bv -> let uu___1 = elim_bv bv in Prims.Inl uu___1
            | Prims.Inr fv -> Prims.Inr fv in
...
and (elim_ascription :
  ...
  fun uu___ ->
    ...
    | (tc, topt) ->
        let uu___1 =
          match tc with
          | Prims.Inl t -> let uu___2 = deep_compress t in Prims.Inl uu___2
          | Prims.Inr c ->
              let uu___2 = deep_compress_comp c in Prims.Inr uu___2 in
