...
let chk_v_t : (FStar_Syntax_Syntax.term FStar_Pervasives_Native.option FStar_Unionfind.p_uvar * FStar_Syntax_Syntax.version)  ->  FStar_Syntax_Syntax.term FStar_Pervasives_Native.option FStar_Unionfind.p_uvar = (fun uu____430 -> (match (uu____430) with
| (u, v1) -> begin
(

let uvar_to_string = (fun u1 -> (

let uu____468 = (

let uu____470 = (

let uu____472 = (get_term_graph ())
in (FStar_Unionfind.puf_id uu____472 u1))
in (FStar_All.pipe_right uu____470 FStar_Util.string_of_int))
in (Prims.op_Hat "?" uu____468)))
in (
...
let uu____493 = (uvar_to_string u)
in (

let uu____495 = (version_to_string expected)
in (

let uu____497 = (version_to_string v1)
in (FStar_Util.format3 "Incompatible version for term unification variable %s: current version is %s; got version %s" uu____493 uu____495 uu____497))))
in (failwith uu____491))
end)))
end))
...
let uu____511 = (get_term_graph ())
in (

let uu____516 = (chk_v_t u)
in (FStar_Unionfind.puf_id uu____511 uu____516))))
...
let uu____538 = (get_term_graph ())
in (FStar_Unionfind.puf_fromid uu____538 n1))
in (

let uu____545 = (get_version ())
in ((uu____531), (uu____545)))))


let fresh : unit  ->  FStar_Syntax_Syntax.uvar = (fun uu____557 -> (

let uu____558 = (

let uu____565 = (get_term_graph ())
in (FStar_Unionfind.puf_fresh uu____565 FStar_Pervasives_Native.None))
in (

let uu____572 = (get_version ())
in ((uu____558), (uu____572)))))


let find : FStar_Syntax_Syntax.uvar  ->  FStar_Syntax_Syntax.term FStar_Pervasives_Native.option = (fun u -> (

let uu____587 = (get_term_graph ())
in (

let uu____592 = (chk_v_t u)
in (FStar_Unionfind.puf_find uu____587 uu____592))))
...
let uu____611 = (get_term_graph ())
in (

let uu____616 = (chk_v_t u)
in (FStar_Unionfind.puf_change uu____611 uu____616 (FStar_Pervasives_Native.Some (t)))))
in (set_term_graph uu____610)))


let equiv : FStar_Syntax_Syntax.uvar  ->  FStar_Syntax_Syntax.uvar  ->  Prims.bool = (fun u v1 -> (

let uu____635 = (get_term_graph ())
in (

let uu____640 = (chk_v_t u)
in (

let uu____645 = (chk_v_t v1)
in (FStar_Unionfind.puf_equivalent uu____635 uu____640 uu____645)))))
...
let uu____664 = (get_term_graph ())
in (

let uu____669 = (chk_v_t u)
in (

let uu____674 = (chk_v_t v1)
in (FStar_Unionfind.puf_union uu____664 uu____669 uu____674))))
in (set_term_graph uu____663)))


let get_univ_graph : unit  ->  ugraph = (fun uu____686 -> (

let uu____687 = (get ())
in uu____687.univ_graph))


let chk_v_u : (FStar_Syntax_Syntax.universe FStar_Pervasives_Native.option FStar_Unionfind.p_uvar * FStar_Syntax_Syntax.version)  ->  FStar_Syntax_Syntax.universe FStar_Pervasives_Native.option FStar_Unionfind.p_uvar = (fun uu____705 -> (match (uu____705) with
| (u, v1) -> begin
(
...
let uu____747 = (get_univ_graph ())
in (FStar_Unionfind.puf_id uu____747 u1))
in (FStar_All.pipe_right uu____745 FStar_Util.string_of_int))
in (Prims.op_Hat "?" uu____743)))
in (
...
let uu____768 = (uvar_to_string u)
in (

let uu____770 = (version_to_string expected)
in (

let uu____772 = (version_to_string v1)
in (FStar_Util.format3 "Incompatible version for universe unification variable %s: current version is %s; got version %s" uu____768 uu____770 uu____772))))
in (failwith uu____766))
end)))
end))
...
let uu___75_786 = (get ())
in {term_graph = uu___75_786.term_graph; univ_graph = ug; version = uu___75_786.version})
in (set uu____785)))


let univ_uvar_id : FStar_Syntax_Syntax.universe_uvar  ->  Prims.int = (fun u -> (

let uu____794 = (get_univ_graph ())
in (

let uu____799 = (chk_v_u u)
in (FStar_Unionfind.puf_id uu____794 uu____799))))
...
let uu____819 = (get_univ_graph ())
in (FStar_Unionfind.puf_fromid uu____819 n1))
in (

let uu____826 = (get_version ())
in ((uu____814), (uu____826)))))


let univ_fresh : unit  ->  FStar_Syntax_Syntax.universe_uvar = (fun uu____836 -> (

let uu____837 = (

let uu____842 = (get_univ_graph ())
in (FStar_Unionfind.puf_fresh uu____842 FStar_Pervasives_Native.None))
in (

let uu____849 = (get_version ())
in ((uu____837), (uu____849)))))
...
let uu____867 = (chk_v_u u)
in (FStar_Unionfind.puf_find uu____862 uu____867))))
...
let uu____891 = (chk_v_u u)
in (FStar_Unionfind.puf_change uu____886 uu____891 (FStar_Pervasives_Native.Some (t)))))
in (set_univ_graph uu____885)))


let univ_equiv : FStar_Syntax_Syntax.universe_uvar  ->  FStar_Syntax_Syntax.universe_uvar  ->  Prims.bool = (fun u v1 -> (

let uu____910 = (get_univ_graph ())
in (

let uu____915 = (chk_v_u u)
in (

let uu____920 = (chk_v_u v1)
in (FStar_Unionfind.puf_equivalent uu____910 uu____915 uu____920)))))
...
let uu____939 = (get_univ_graph ())
in (

let uu____944 = (chk_v_u u)
in (

let uu____949 = (chk_v_u v1)
in (FStar_Unionfind.puf_union uu____939 uu____944 uu____949))))
in (set_univ_graph uu____938)))
