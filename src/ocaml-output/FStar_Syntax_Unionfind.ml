...
let (new_transaction : unit -> tx) =
  fun uu____509  ->
    ...
    (let uu____513 =
       let uu___90_514 = get ()  in
       let uu____515 = vops.next_minor ()  in
       {
         term_graph = (uu___90_514.term_graph);
         univ_graph = (uu___90_514.univ_graph);
         version = uu____515
       }  in
     set uu____513);
    tx
...
let (set_term_graph : tgraph -> unit) =
  fun tg  ->
    let uu____604 =
      let uu___91_605 = get ()  in
      {
        term_graph = tg;
        univ_graph = (uu___91_605.univ_graph);
        version = (uu___91_605.version)
      }  in
    set uu____604
...
let (set_univ_graph : ugraph -> unit) =
  fun ug  ->
    let uu____871 =
      let uu___92_872 = get ()  in
      {
        term_graph = (uu___92_872.term_graph);
        univ_graph = ug;
        version = (uu___92_872.version)
      }  in
    set uu____871
