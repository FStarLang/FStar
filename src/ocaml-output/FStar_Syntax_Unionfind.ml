...
let new_transaction: Prims.unit -> tx =
  fun uu____539  ->
    ...
    (let uu____543 =
       let uu___147_544 = get () in
       let uu____545 = vops.next_minor () in
       {
         term_graph = (uu___147_544.term_graph);
         univ_graph = (uu___147_544.univ_graph);
         version = uu____545
       } in
     set uu____543);
    tx
...
let set_term_graph: tgraph -> Prims.unit =
  fun tg  ->
    let uu____609 =
      let uu___148_610 = get () in
      {
        term_graph = tg;
        univ_graph = (uu___148_610.univ_graph);
        version = (uu___148_610.version)
      } in
    set uu____609
let chk_v:
  'Auu____615 .
...
let set_univ_graph: ugraph -> Prims.unit =
  fun ug  ->
    let uu____834 =
      let uu___149_835 = get () in
      {
        term_graph = (uu___149_835.term_graph);
        univ_graph = ug;
        version = (uu___149_835.version)
      } in
    set uu____834
