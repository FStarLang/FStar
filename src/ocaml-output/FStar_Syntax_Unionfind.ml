...
let (new_transaction : unit -> tx) =
  fun uu____509  ->
    ...
    (let uu____513 =
       let uu___88_514 = get ()  in
       let uu____515 = vops.next_minor ()  in
       {
         term_graph = (uu___88_514.term_graph);
         univ_graph = (uu___88_514.univ_graph);
         version = uu____515
       }  in
     set uu____513);
    tx
...
let (set_term_graph : tgraph -> unit) =
  fun tg  ->
    let uu____604 =
      let uu___89_605 = get ()  in
      {
        term_graph = tg;
        univ_graph = (uu___89_605.univ_graph);
        version = (uu___89_605.version)
      }  in
    set uu____604
  
let chk_v :
  'Auu____611 .
    ('Auu____611,FStar_Syntax_Syntax.version)
      FStar_Pervasives_Native_Tuple2.tuple2 -> 'Auu____611
  =
  fun uu____620  ->
...
let (set_univ_graph : ugraph -> unit) =
  fun ug  ->
    let uu____871 =
      let uu___90_872 = get ()  in
      {
        term_graph = (uu___90_872.term_graph);
        univ_graph = ug;
        version = (uu___90_872.version)
      }  in
    set uu____871
