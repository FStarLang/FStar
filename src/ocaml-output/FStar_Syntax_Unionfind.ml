...
let (vops : vops_t) =
  ...
  let next_major uu____52 =
    ...
    (let uu____149 = FStar_Util.incr major; FStar_ST.op_Bang major in
     {
       FStar_Syntax_Syntax.major = uu____149;
       FStar_Syntax_Syntax.minor = (Prims.parse_int "0")
     }) in
  let next_minor uu____354 =
    let uu____355 = FStar_ST.op_Bang major in
    let uu____451 = FStar_Util.incr minor; FStar_ST.op_Bang minor in
    {
      FStar_Syntax_Syntax.major = uu____355;
...
let (empty : FStar_Syntax_Syntax.version -> uf) =
  fun v1 ->
    let uu____694 = FStar_Unionfind.puf_empty () in
    let uu____697 = FStar_Unionfind.puf_empty () in
    { term_graph = uu____694; univ_graph = uu____697; version = v1 }
let (version_to_string : FStar_Syntax_Syntax.version -> Prims.string) =
  fun v1 ->
    let uu____703 = FStar_Util.string_of_int v1.FStar_Syntax_Syntax.major in
    let uu____704 = FStar_Util.string_of_int v1.FStar_Syntax_Syntax.minor in
    FStar_Util.format2 "%s.%s" uu____703 uu____704
let (state : uf FStar_ST.ref) =
  let uu____742 = let uu____743 = vops.next_major () in empty uu____743 in
  FStar_Util.mk_ref uu____742
type tx =
...
let (reset : Prims.unit -> Prims.unit) =
  fun uu____813 ->
    let v1 = vops.next_major () in let uu____815 = empty v1 in set uu____815
let (new_transaction : Prims.unit -> tx) =
  fun uu____818 ->
    let tx = let uu____820 = get () in TX uu____820 in
    (let uu____822 =
       let uu___26_823 = get () in
       let uu____824 = vops.next_minor () in
       {
         term_graph = (uu___26_823.term_graph);
         univ_graph = (uu___26_823.univ_graph);
         version = uu____824
       } in
     set uu____822);
    tx
...
let (rollback : tx -> Prims.unit) =
  fun uu____830 -> match uu____830 with | TX uf -> set uf
...
let (set_term_graph : tgraph -> Prims.unit) =
  fun tg ->
    let uu____964 =
      let uu___27_965 = get () in
      {
        term_graph = tg;
        univ_graph = (uu___27_965.univ_graph);
        version = (uu___27_965.version)
      } in
    set uu____964
let chk_v :
  'Auu____968 .
...
let (set_univ_graph : ugraph -> Prims.unit) =
  fun ug ->
    let uu____1175 =
      let uu___28_1176 = get () in
      {
        term_graph = (uu___28_1176.term_graph);
        univ_graph = ug;
        version = (uu___28_1176.version)
      } in
    set uu____1175
