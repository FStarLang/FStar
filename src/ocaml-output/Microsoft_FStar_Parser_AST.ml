...
let imp_to_string = (fun _348076 -> (match (_348076) with
| Hash -> begin
"#"
end
| _ -> begin
...
(Support.Microsoft.FStar.Util.format2 "(%s %s)" (Microsoft_FStar_Absyn_Print.sli l) (to_string_l " " (fun _348296 -> (match (_348296) with
| (a, imp) -> begin
...
(Support.Microsoft.FStar.Util.format2 "let rec %s in %s" (to_string_l " and " (fun _348326 -> (match (_348326) with
| (p, b) -> begin
...
(Support.Microsoft.FStar.Util.format2 "match %s with %s" (term_to_string t) (to_string_l " | " (fun _348343 -> (match (_348343) with
| (p, w, e) -> begin
...
(Support.Microsoft.FStar.Util.format2 "{%s with %s}" (term_to_string e) (to_string_l " " (fun _348358 -> (match (_348358) with
| (l, e) -> begin
(Support.Microsoft.FStar.Util.format2 "%s=%s" (Microsoft_FStar_Absyn_Print.sli l) (term_to_string e))
end)) fields))
end
| Record ((None, fields)) -> begin
(Support.Microsoft.FStar.Util.format1 "{%s}" (to_string_l " " (fun _348365 -> (match (_348365) with
| (l, e) -> begin
(Support.Microsoft.FStar.Util.format2 "%s=%s" (Microsoft_FStar_Absyn_Print.sli l) (term_to_string e))
end)) fields))
end
...
| PatRecord (l) -> begin
(Support.Microsoft.FStar.Util.format1 "{%s}" (to_string_l "; " (fun _348479 -> (match (_348479) with
| (f, e) -> begin
...
| _ -> begin
(Support.List.fold_left (fun t _348523 -> (match (_348523) with
| (a, imp) -> begin
...
let mkRefSet = (fun r elts -> (let empty = (mk_term (Var ((Microsoft_FStar_Absyn_Util.set_lid_range Microsoft_FStar_Absyn_Const.set_empty r))) r Expr)
in (let ref_constr = (mk_term (Var ((Microsoft_FStar_Absyn_Util.set_lid_range Microsoft_FStar_Absyn_Const.heap_ref r))) r Expr)
in (let singleton = (mk_term (Var ((Microsoft_FStar_Absyn_Util.set_lid_range Microsoft_FStar_Absyn_Const.set_singleton r))) r Expr)
in (let union = (mk_term (Var ((Microsoft_FStar_Absyn_Util.set_lid_range Microsoft_FStar_Absyn_Const.set_union r))) r Expr)
...
(let _348558 = (Microsoft_FStar_Tc_Errors.warn r "Focusing on only some cases")
...
in if should_filter then begin
(let _348564 = (Microsoft_FStar_Tc_Errors.warn r "Focusing on only some cases in this (mutually) recursive definition")
in (Support.List.map (fun _348568 -> (match (_348568) with
| (f, lb) -> begin
if f then begin
lb
end else begin
