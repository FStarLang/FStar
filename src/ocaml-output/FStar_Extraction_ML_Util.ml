...
with
| uu____56 -> begin
(failwith (let _0_353 = (FStar_Range.string_of_range p)
in (let _0_352 = (FStar_Syntax_Print.const_to_string c)
in (FStar_Util.format2 "(%s) Failed to translate constant %s " _0_353 _0_352))))
end)
...
FStar_Extraction_ML_Syntax.MLTY_Fun ((let _0_355 = (subst_aux subst t1)
in (let _0_354 = (subst_aux subst t2)
in ((_0_355), (f), (_0_354)))))
end
| FStar_Extraction_ML_Syntax.MLTY_Named (args, path) -> begin
FStar_Extraction_ML_Syntax.MLTY_Named ((let _0_356 = (FStar_List.map (subst_aux subst) args)
in ((_0_356), (path))))
end
...
| uu____112 -> begin
(let _0_357 = (FStar_List.zip formals args)
in (subst_aux _0_357 t))
end)
end))


let udelta_unfold : FStar_Extraction_ML_UEnv.env  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty Prims.option = (fun g uu___131_119 -> (match (uu___131_119) with
...
let eff_to_string : FStar_Extraction_ML_Syntax.e_tag  ->  Prims.string = (fun uu___132_144 -> (match (uu___132_144) with
...
| uu____154 -> begin
(failwith (let _0_358 = (FStar_Range.string_of_range r)
in (FStar_Util.format3 "Impossible (%s): Inconsistent effects %s and %s" _0_358 (eff_to_string f) (eff_to_string f'))))
end))
...
in (let _0_359 = ((mk_ty_fun ()) xs body.FStar_Extraction_ML_Syntax.mlty)
in (FStar_Extraction_ML_Syntax.with_ty _0_359 e)))
end))
in (match (e) with
...
in (let _0_361 = Some ((let _0_360 = (FStar_Extraction_ML_Syntax.with_ty ((mk_ty_fun ()) ((x)::[]) body.FStar_Extraction_ML_Syntax.mlty))
in (FStar_All.pipe_left _0_360 (FStar_Extraction_ML_Syntax.MLE_Fun ((((x)::[]), (body)))))))
in ((true), (_0_361))))
end
| uu____326 -> begin
((false), (None))
...
let uu____329 = (let _0_364 = (let _0_363 = (mk_fun xs body)
in (FStar_All.pipe_left (fun _0_362 -> Some (_0_362)) _0_363))
in (type_leq_c unfold_ty _0_364 t2 t2'))
...
and type_leq : unfold_t  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty  ->  Prims.bool = (fun g t1 t2 -> (let _0_365 = (type_leq_c g None t1 t2)
in (FStar_All.pipe_right _0_365 Prims.fst)))


let is_type_abstraction = (fun uu___133_496 -> (match (uu___133_496) with
...
let record_field_path : FStar_Ident.lident Prims.list  ->  Prims.string Prims.list = (fun uu___134_557 -> (match (uu___134_557) with
| (f)::uu____561 -> begin
(
...
let codegen_fsharp : Prims.unit  ->  Prims.bool = (fun uu____634 -> (let _0_366 = (FStar_Option.get (FStar_Options.codegen ()))
in (_0_366 = "FSharp")))


let flatten_ns : Prims.string Prims.list  ->  Prims.string = (fun ns -> (

let uu____640 = (codegen_fsharp ())
...
let flatten_mlpath : (Prims.string Prims.list * Prims.string)  ->  Prims.string = (fun uu____647 -> (match (uu____647) with
| (ns, n) -> begin
(

let uu____655 = (codegen_fsharp ())
...
let mlpath_of_lid : FStar_Ident.lident  ->  (Prims.string Prims.list * Prims.string) = (fun l -> (let _0_367 = (FStar_All.pipe_right l.FStar_Ident.ns (FStar_List.map (fun i -> i.FStar_Ident.idText)))
in ((_0_367), (l.FStar_Ident.ident.FStar_Ident.idText))))
...
| true -> begin
FStar_Extraction_ML_Syntax.MLTY_Fun ((let _0_369 = (eraseTypeDeep unfold_ty tyd)
in (let _0_368 = (eraseTypeDeep unfold_ty tycd)
in ((_0_369), (etag), (_0_368)))))
end
| uu____702 -> begin
t
end)
...
let uu____707 = (erasableType unfold_ty t)
in (match (uu____707) with
| true -> begin
FStar_Extraction_ML_UEnv.erasedContent
end
| uu____711 -> begin
FStar_Extraction_ML_Syntax.MLTY_Named ((let _0_370 = (FStar_List.map (eraseTypeDeep unfold_ty) lty)
in ((_0_370), (mlp))))
end))
end
...
let prims_op_amp_amp : FStar_Extraction_ML_Syntax.mlexpr = (let _0_371 = (FStar_Extraction_ML_Syntax.with_ty ((mk_ty_fun ()) (((((("x"), ((Prims.parse_int "0")))), (FStar_Extraction_ML_Syntax.ml_bool_ty)))::((((("y"), ((Prims.parse_int "0")))), (FStar_Extraction_ML_Syntax.ml_bool_ty)))::[]) FStar_Extraction_ML_Syntax.ml_bool_ty))
in (FStar_All.pipe_left _0_371 (FStar_Extraction_ML_Syntax.MLE_Name (((("Prims")::[]), ("op_AmpAmp"))))))
...
in (let _0_372 = (FStar_Range.file_of_range r)
in ((line), (_0_372))))))


let rec argTypes : FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty Prims.list = (fun t -> (match (t) with
| FStar_Extraction_ML_Syntax.MLTY_Fun (a, uu____788, b) -> begin
(let _0_373 = (argTypes b)
in (a)::_0_373)
end
| uu____790 -> begin
[]
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (a, uu____801, b) -> begin
(

let uu____803 = (uncurry_mlty_fun b)
