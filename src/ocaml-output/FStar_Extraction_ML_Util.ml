...
let mlconst_of_const' : FStar_Range.range  ->  FStar_Const.sconst  ->  FStar_Extraction_ML_Syntax.mlconstant = (fun p c -> (FStar_All.try_with (fun uu___152_82 -> (match (()) with
| () -> begin
(mlconst_of_const c)
end)) (fun uu___151_86 -> (match (uu___151_86) with
| uu____87 -> begin
(
...
let udelta_unfold : FStar_Extraction_ML_UEnv.env  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty FStar_Pervasives_Native.option = (fun g uu___147_236 -> (match (uu___147_236) with
...
let eff_to_string : FStar_Extraction_ML_Syntax.e_tag  ->  Prims.string = (fun uu___148_284 -> (match (uu___148_284) with
...
in (FStar_All.pipe_left (fun _0_41 -> FStar_Pervasives_Native.Some (_0_41)) uu____584))
...
let is_type_abstraction : 'Auu____824 'Auu____825 'Auu____826 . (('Auu____826, 'Auu____825) FStar_Util.either * 'Auu____824) Prims.list  ->  Prims.bool = (fun uu___149_840 -> (match (uu___149_840) with
...
let record_field_path : FStar_Ident.lident Prims.list  ->  Prims.string Prims.list = (fun uu___150_945 -> (match (uu___150_945) with
| (f)::uu____951 -> begin
(
...
| _0_42 when (_0_42 = (Prims.parse_int "1")) -> begin
(
...
let mk_interpretation_fun : 'Auu____1928 . FStar_Ident.lident  ->  FStar_Extraction_ML_Syntax.mlexpr'  ->  FStar_Syntax_Syntax.term  ->  (FStar_Syntax_Syntax.bv * 'Auu____1928) Prims.list  ->  FStar_Extraction_ML_Syntax.mlexpr' FStar_Pervasives_Native.option = (fun tac_lid assm_lid t bs -> (FStar_All.try_with (fun uu___154_1970 -> (match (()) with
| () -> begin
(
...
let uu____1997 = (FStar_Util.string_of_int arity)
in (Prims.strcat "FStar_Tactics_Interpreter.mk_tactic_interpretation_" uu____1997))
in (str_to_top_name uu____1996))
in (
...
let uu____2014 = (FStar_Util.string_of_int arity)
in (Prims.strcat "FStar_Tactics_Native.from_tactic_" uu____2014))
in (str_to_top_name uu____2013))
in (

let uu____2021 = (

let uu____2024 = (lid_to_top_name tac_lid)
in (uu____2024)::[])
in ((uu____2012), (uu____2021))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2005))
in (
...
let uu____2035 = (str_to_top_name "FStar_Ident.lid_of_str")
in ((uu____2035), (((FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top assm_lid))::[])))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2028))
in (
...
let uu____2045 = (FStar_List.map (mk_tac_embedding_path Unembed) arg_types)
in (

let uu____2048 = (

let uu____2051 = (mk_tac_embedding_path Embed t)
in (

let uu____2052 = (

let uu____2055 = (mk_tac_param_type t)
in (
...
let uu____2065 = (str_to_name "args")
in (uu____2065)::[])
in (psc)::uu____2062)
in (tac_lid_app)::uu____2059)
in (uu____2055)::uu____2056))
in (uu____2051)::uu____2052))
in (FStar_List.append uu____2045 uu____2048)))
in (FStar_List.append ((tac_fun)::[]) uu____2042))
in (
...
let uu____2075 = (FStar_List.map (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) args)
in ((h), (uu____2075)))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2068))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____2067))
in FStar_Pervasives_Native.Some (FStar_Extraction_ML_Syntax.MLE_Fun (((((("psc"), (FStar_Extraction_ML_Syntax.MLTY_Top)))::((("args"), (FStar_Extraction_ML_Syntax.MLTY_Top)))::[]), (app))))))))))))
end)) (fun uu___153_2100 -> (match (uu___153_2100) with
| CallNotImplemented -> begin
((

let uu____2104 = (FStar_Ident.string_of_lid tac_lid)
in (not_implemented_warning uu____2104));
FStar_Pervasives_Native.None;
)
end))))
