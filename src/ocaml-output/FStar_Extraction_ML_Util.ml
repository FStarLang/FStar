...
let mlconst_of_const : FStar_Range.range  ->  FStar_Const.sconst  ->  FStar_Extraction_ML_Syntax.mlconstant = (fun p c -> (FStar_All.try_with (fun uu___68_100 -> (match (()) with
| () -> begin
(mlconst_of_const' c)
end)) (fun uu___67_104 -> (match (uu___67_104) with
| uu____105 -> begin
(
...
let udelta_unfold : FStar_Extraction_ML_UEnv.env  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty FStar_Pervasives_Native.option = (fun g uu___63_363 -> (match (uu___63_363) with
...
let eff_to_string : FStar_Extraction_ML_Syntax.e_tag  ->  Prims.string = (fun uu___64_414 -> (match (uu___64_414) with
...
let is_type_abstraction : 'a 'b 'c . (('a, 'b) FStar_Util.either * 'c) Prims.list  ->  Prims.bool = (fun uu___65_1001 -> (match (uu___65_1001) with
...
let record_field_path : FStar_Ident.lident Prims.list  ->  Prims.string Prims.list = (fun uu___66_1109 -> (match (uu___66_1109) with
| (f)::uu____1115 -> begin
(
...
let uu____2490 = (str_to_name "args_tail")
in (uu____2490)::[])
in ((body), (uu____2487)))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2480))
in (FStar_All.pipe_left w uu____2479))
in ((pattern), (FStar_Pervasives_Native.None), (uu____2478)))
in (
...
let uu____2513 = (str_to_name "failwith")
in (
...
let uu____2518 = (mlexpr_of_const FStar_Range.dummyRange (FStar_Const.Const_string ((("arity mismatch"), (FStar_Range.dummyRange)))))
in (FStar_All.pipe_left w uu____2518))
in (uu____2517)::[])
in ((uu____2513), (uu____2514))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2506))
in (FStar_All.pipe_left w uu____2505))
in ((FStar_Extraction_ML_Syntax.MLP_Wild), (FStar_Pervasives_Native.None), (uu____2504)))
in (
...
let uu____2540 = (str_to_name "args")
in ((uu____2540), ((branch1)::(default_branch)::[])))
in FStar_Extraction_ML_Syntax.MLE_Match (uu____2525))
in (FStar_All.pipe_left w uu____2524))
in (mk_lam "args" body1))))))))
end))
in (

let uu____2575 = (FStar_Syntax_Util.arrow_formals_comp t1)
...
let uu____2633 = (FStar_Util.prefix_until (fun uu____2667 -> (match (uu____2667) with
| (b, uu____2673) -> begin
(

let uu____2674 = (

let uu____2675 = (FStar_Syntax_Subst.compress b.FStar_Syntax_Syntax.sort)
in uu____2675.FStar_Syntax_Syntax.n)
in (match (uu____2674) with
| FStar_Syntax_Syntax.Tm_type (uu____2678) -> begin
false
end
| uu____2679 -> begin
true
...
let uu____2862 = (FStar_Util.string_of_int i)
in (Prims.strcat "tv_" uu____2862))) type_vars)
in (
...
let uu____2979 = (lid_to_top_name fv)
in (uu____2979)::[])
in (res_embedding)::uu____2976)
in (FStar_List.append arg_unembeddings uu____2973))
in (
...
let uu____2991 = (mk_lam "_psc" tabs)
in ((uu____2991), (arity), (true)))
in FStar_Pervasives_Native.Some (uu____2984))))))
end
| uu____3002 -> begin
(
...
let uu____3004 = (FStar_TypeChecker_Env.norm_eff_name tcenv (FStar_Syntax_Util.comp_effect_name c))
in (FStar_Ident.lid_equals uu____3004 FStar_Parser_Const.effect_TAC_lid))
...
let uu____3015 = (FStar_Util.string_of_int non_tvar_arity)
in (Prims.strcat "FStar_Tactics_Interpreter.mk_tactic_interpretation_" uu____3015))
in (str_to_top_name uu____3014))
in (
...
let uu____3033 = (FStar_Util.string_of_int non_tvar_arity)
in (Prims.strcat "FStar_Tactics_Native.from_tactic_" uu____3033))
in (str_to_top_name uu____3032))
in (

let uu____3040 = (

let uu____3043 = (lid_to_top_name fv)
in (uu____3043)::[])
in ((uu____3031), (uu____3040))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____3024))
in (FStar_All.pipe_left w uu____3023))
in (
...
let uu____3055 = (str_to_top_name "FStar_Ident.lid_of_str")
in ((uu____3055), (((w ml_fv))::[])))
in FStar_Extraction_ML_Syntax.MLE_App (uu____3048))
in (FStar_All.pipe_left w uu____3047))
in (
...
let uu____3066 = (FStar_All.pipe_left w (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Bool (true))))
in (uu____3066)::(tac_fun)::[])
in (FStar_List.append uu____3063 (FStar_List.append arg_unembeddings ((res_embedding)::(tac_lid_app)::(psc)::[]))))
in (
...
let uu____3068 = (FStar_All.pipe_left w (FStar_Extraction_ML_Syntax.MLE_App (((h), ((FStar_List.append args ((all_args)::[])))))))
in (mk_lam "args" uu____3068))
end
| uu____3071 -> begin
(

let uu____3074 = (FStar_All.pipe_left w (FStar_Extraction_ML_Syntax.MLE_App (((h), (args)))))
in (abstract_tvars tvar_names uu____3074))
end)
in (

let uu____3077 = (

let uu____3084 = (mk_lam "psc" tabs)
in ((uu____3084), ((arity + (Prims.parse_int "1"))), (false)))
in FStar_Pervasives_Native.Some (uu____3077)))))))))
end
| uu____3097 -> begin
(
...
in (Prims.strcat "Plugins not defined for type " uu____3100))
in NoTacticEmbedding (uu____3099))
in (FStar_Exn.raise uu____3098))
end))
end))))
end
| ((b, uu____3110))::bs3 -> begin
(

let uu____3122 = (

let uu____3125 = (mk_embedding env b.FStar_Syntax_Syntax.sort)
in (uu____3125)::accum_embeddings)
in (aux uu____3122 env bs3))
end))
in (FStar_All.try_with (fun uu___70_3135 -> (match (()) with
| () -> begin
(aux [] tvar_context bs1)
end)) (fun uu___69_3147 -> (match (uu___69_3147) with
| NoTacticEmbedding (msg) -> begin
((

let uu____3158 = (FStar_Ident.string_of_lid fv)
in (not_implemented_warning t1.FStar_Syntax_Syntax.pos uu____3158 msg));
FStar_Pervasives_Native.None;
)
end)))))))
end))))
