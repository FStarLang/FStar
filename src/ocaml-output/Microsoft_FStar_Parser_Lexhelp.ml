...
| None -> begin
(let _414598 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _414611 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
in (let _414624 = (Support.Microsoft.FStar.Util.for_range 0 (((Support.Microsoft.FStar.Bytes.length bytes) / 2) - 1) (fun i -> if ((Support.Microsoft.FStar.Bytes.get bytes ((i * 2) + 1)) <> 0) then begin
(ok := false)
end))
in (! (ok))))))
...
let stringKeywords = (Support.List.map (fun _414654 -> (match (_414654) with
| (_, w, _) -> begin
w
end)) keywords)

let unreserve_words = (Support.List.choose (fun _414659 -> (match (_414659) with
...
in (let _414665 = (Support.List.iter (fun _414664 -> (match (_414664) with
...
let mkLexargs = (fun _414676 -> (match (_414676) with
...
(let _414682 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. (%s)" s (Support.Microsoft.FStar.Range.string_of_range r)))
