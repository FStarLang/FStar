...
| None -> begin
(let _177782 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _177795 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
in (let _177808 = (Support.Microsoft.FStar.Util.for_range 0 (((Support.Microsoft.FStar.Bytes.length bytes) / 2) - 1) (fun i -> if ((Support.Microsoft.FStar.Bytes.get bytes ((i * 2) + 1)) <> 0) then begin
(Support.ST.op_ColonEquals ok false)
end))
...
in (let _177849 = (Support.List.iter (fun _177848 -> (match (_177848) with
...
let mkLexargs = (fun _177860 -> (match (_177860) with
...
(let _177866 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. \x28%s\x29" s (Support.Microsoft.FStar.Range.string_of_range r)))
