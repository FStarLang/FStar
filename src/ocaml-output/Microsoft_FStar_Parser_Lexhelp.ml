...
| None -> begin
(let _38_6 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _38_19 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
let stringKeywords = (Support.List.map (fun _38_62 -> (match (_38_62) with
| (_, w, _) -> begin
w
end)) keywords)

let unreserve_words = (Support.List.choose (fun _38_67 -> (match (_38_67) with
...
in (let _38_73 = (Support.List.iter (fun _38_72 -> (match (_38_72) with
...
let mkLexargs = (fun _38_84 -> (match (_38_84) with
...
(let _38_90 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. \x28%s\x29" s (Support.Microsoft.FStar.Range.string_of_range r)))
