...
| None -> begin
(let _411207 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _411220 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
in (let _411233 = (Support.Microsoft.FStar.Util.for_range 0 (((Support.Microsoft.FStar.Bytes.length bytes) / 2) - 1) (fun i -> if ((Support.Microsoft.FStar.Bytes.get bytes ((i * 2) + 1)) <> 0) then begin
(ok := false)
end))
in (! (ok))))))
...
let stringKeywords = (Support.List.map (fun _411263 -> (match (_411263) with
| (_, w, _) -> begin
w
end)) keywords)

let unreserve_words = (Support.List.choose (fun _411268 -> (match (_411268) with
...
in (let _411274 = (Support.List.iter (fun _411273 -> (match (_411273) with
...
let mkLexargs = (fun _411285 -> (match (_411285) with
...
(let _411291 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. (%s)" s (Support.Microsoft.FStar.Range.string_of_range r)))
