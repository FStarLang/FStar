...
| None -> begin
(let _413512 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _413525 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
in (let _413538 = (Support.Microsoft.FStar.Util.for_range 0 (((Support.Microsoft.FStar.Bytes.length bytes) / 2) - 1) (fun i -> if ((Support.Microsoft.FStar.Bytes.get bytes ((i * 2) + 1)) <> 0) then begin
(ok := false)
end))
in (! (ok))))))
...
let stringKeywords = (Support.List.map (fun _413568 -> (match (_413568) with
| (_, w, _) -> begin
w
end)) keywords)

let unreserve_words = (Support.List.choose (fun _413573 -> (match (_413573) with
...
in (let _413579 = (Support.List.iter (fun _413578 -> (match (_413578) with
...
let mkLexargs = (fun _413590 -> (match (_413590) with
...
(let _413596 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. (%s)" s (Support.Microsoft.FStar.Range.string_of_range r)))
