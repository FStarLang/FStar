...
| None -> begin
(let _177928 = (Support.Microsoft.FStar.Util.smap_add strings s s)
in s)
end)))
...
let add_int_char = (fun buf c -> (let _177941 = (Support.Microsoft.FStar.Bytes.emit_int_as_byte buf (c mod 256))
...
in (let _177954 = (Support.Microsoft.FStar.Util.for_range 0 (((Support.Microsoft.FStar.Bytes.length bytes) / 2) - 1) (fun i -> if ((Support.Microsoft.FStar.Bytes.get bytes ((i * 2) + 1)) <> 0) then begin
(ok := false)
end))
in (! (ok))))))
...
let stringKeywords = (Support.List.map (fun _177984 -> (match (_177984) with
| (_, w, _) -> begin
w
end)) keywords)

let unreserve_words = (Support.List.choose (fun _177989 -> (match (_177989) with
...
in (let _177995 = (Support.List.iter (fun _177994 -> (match (_177994) with
...
let mkLexargs = (fun _178006 -> (match (_178006) with
...
(let _178012 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "The keyword \'%s\' is reserved for future use by F#. (%s)" s (Support.Microsoft.FStar.Range.string_of_range r)))
