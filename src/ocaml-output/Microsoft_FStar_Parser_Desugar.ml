...
let as_imp = (fun _361294 -> (match (_361294) with
...
let compile_op = (fun s -> (let name_of_char = (fun _361295 -> (match (_361295) with
| '&' -> begin
"Amp"
end
| '@' -> begin
...
(let _361674 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env id)
...
(let _361768 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env x)
in (match (_361768) with
| (env, _) -> begin
(env, (x)::[])
end))
end
...
(let _361782 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env id)
in (match (_361782) with
| (env, _) -> begin
(env, [])
end))
end
...
(Support.List.collect (fun _361834 -> (match (_361834) with
| (t, _) -> begin
(free_type_vars env t)
end)) ts)
end
...
(let _361852 = (free_type_vars_b env b)
in (match (_361852) with
| (env, f) -> begin
...
(let _361868 = (Support.List.fold_left (fun _361861 binder -> (match (_361861) with
| (env, free) -> begin
(let _361865 = (free_type_vars_b env binder)
in (match (_361865) with
| (env, f) -> begin
(env, (Support.List.append f free))
end))
end)) (env, []) binders)
in (match (_361868) with
| (env, free) -> begin
...
(let _361971 = (destruct_app_pattern env is_top_level p)
...
let binder_of_bnd = (fun _361296 -> (match (_361296) with
...
let as_binder = (fun env imp _361297 -> (match (_361297) with
...
(let _362033 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env a)
...
(let _362041 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_362041) with
| (env, x) -> begin
...
let rec desugar_data_pat = (fun env p -> (let resolvex = (fun l e x -> (match (((Support.Microsoft.FStar.Util.find_opt (fun _361298 -> (match (_361298) with
...
| _ -> begin
(let _362092 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding e x)
in (match (_362092) with
| (e, x) -> begin
((Support.Microsoft.FStar.Util.Inr (x))::l, e, x)
end))
end))
in (let resolvea = (fun l e a -> (match (((Support.Microsoft.FStar.Util.find_opt (fun _361299 -> (match (_361299) with
...
| _ -> begin
(let _362109 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding e a)
...
(let _362129 = (aux loc env p)
in (match (_362129) with
| (loc, env, var, p) -> begin
(let _362144 = (Support.List.fold_left (fun _362133 p -> (match (_362133) with
| (loc, env, ps) -> begin
(let _362140 = (aux loc env p)
in (match (_362140) with
| (loc, env, _, p) -> begin
(loc, env, (p)::ps)
end))
end)) (loc, env, []) ps)
in (match (_362144) with
| (loc, env, ps) -> begin
(let pat = (pos (Microsoft_FStar_Absyn_Syntax.Pat_disj ((p)::(Support.List.rev ps))))
in (let _362146 = (Support.Prims.ignore (Microsoft_FStar_Absyn_Syntax.pat_vars pat))
in (loc, env, var, pat)))
end))
end))
end
...
(let _362159 = p
in {Microsoft_FStar_Parser_AST.pat = Microsoft_FStar_Parser_AST.PatTvar ((x, imp)); Microsoft_FStar_Parser_AST.prange = _362159.Microsoft_FStar_Parser_AST.prange})
end
| _ -> begin
...
in (let _362168 = (aux loc env p)
in (match (_362168) with
...
(let _362195 = (resolvea loc env a)
in (match (_362195) with
| (loc, env, abvd) -> begin
...
in (let _362210 = (resolvex loc env x)
in (match (_362210) with
| (loc, env, xbvd) -> begin
...
(let _362237 = (Support.List.fold_right (fun arg _362227 -> (match (_362227) with
| (loc, env, args) -> begin
(let _362233 = (aux loc env arg)
in (match (_362233) with
| (loc, env, _, arg) -> begin
(loc, env, (arg)::args)
end))
end)) args (loc, env, []))
in (match (_362237) with
| (loc, env, args) -> begin
(let l = (Microsoft_FStar_Parser_DesugarEnv.fail_or env (Microsoft_FStar_Parser_DesugarEnv.try_lookup_datacon env) l)
in (let x = (Microsoft_FStar_Absyn_Util.new_bvd (Some (p.Microsoft_FStar_Parser_AST.prange)))
...
(let _362259 = (Support.List.fold_right (fun pat _362249 -> (match (_362249) with
| (loc, env, pats) -> begin
(let _362255 = (aux loc env pat)
in (match (_362255) with
| (loc, env, _, pat) -> begin
(loc, env, (pat)::pats)
end))
end)) pats (loc, env, []))
in (match (_362259) with
| (loc, env, pats) -> begin
...
(let _362283 = (Support.List.fold_left (fun _362272 p -> (match (_362272) with
| (loc, env, pats) -> begin
(let _362279 = (aux loc env p)
in (match (_362279) with
| (loc, env, _, pat) -> begin
(loc, env, (pat)::pats)
end))
end)) (loc, env, []) args)
in (match (_362283) with
| (loc, env, args) -> begin
...
(let _362303 = (Support.List.hd fields)
in (match (_362303) with
| (f, _) -> begin
(let _362307 = (Microsoft_FStar_Parser_DesugarEnv.fail_or env (Microsoft_FStar_Parser_DesugarEnv.try_lookup_record_by_field_name env) f)
in (match (_362307) with
| (record, _) -> begin
(let fields = ((Support.List.map (fun _362310 -> (match (_362310) with
| (f, p) -> begin
((Microsoft_FStar_Parser_DesugarEnv.fail_or env (Microsoft_FStar_Parser_DesugarEnv.qualify_field_to_record env record) f), p)
end))) fields)
in (let args = ((Support.List.map (fun _362315 -> (match (_362315) with
| (f, _) -> begin
(match (((Support.List.tryFind (fun _362319 -> (match (_362319) with
| (g, _) -> begin
...
in (let _362333 = (aux [] env p)
in (match (_362333) with
| (_, env, b, p) -> begin
(env, b, p)
end))))))
...
(let _362358 = (desugar_data_pat env p)
in (match (_362358) with
| (env, binder, p) -> begin
...
and desugar_match_pat_maybe_top = (fun _362373 env pat -> (let _362381 = (desugar_data_pat env pat)
in (match (_362381) with
| (env, _, pat) -> begin
(env, pat)
end)))
...
in (let setpos = (fun e -> (let _362395 = e
in {Microsoft_FStar_Absyn_Syntax.n = _362395.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = _362395.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = top.Microsoft_FStar_Parser_AST.range; Microsoft_FStar_Absyn_Syntax.fvs = _362395.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _362395.Microsoft_FStar_Absyn_Syntax.uvs}))
...
| _ -> begin
(let args = (Support.List.map (fun _362425 -> (match (_362425) with
| (t, imp) -> begin
...
(let _362462 = (Support.List.fold_left (fun _362434 pat -> (match (_362434) with
| (env, ftvs) -> begin
...
in (let rec aux = (fun env bs sc_pat_opt _361300 -> (match (_361300) with
| [] -> begin
...
| p::rest -> begin
(let _362485 = (desugar_binding_pat env p)
in (match (_362485) with
| (env, b, pat) -> begin
(let _362543 = (match (b) with
| LetBinder (_) -> begin
(failwith "Impossible")
end
| TBinder ((a, k, aq)) -> begin
...
(let ds_let_rec = (fun _362586 -> (match (_362586) with
| () -> begin
(let bindings = ((pat, _snd))::_tl
in (let funs = ((Support.List.map (fun _362590 -> (match (_362590) with
| (p, def) -> begin
...
in (let _362640 = (Support.List.fold_left (fun _362618 _362627 -> (match ((_362618, _362627)) with
| ((env, fnames), ((f, _, _), _)) -> begin
(let _362637 = (match (f) with
| Support.Microsoft.FStar.Util.Inl (x) -> begin
(let _362632 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_362632) with
| (env, xx) -> begin
...
in (match (_362637) with
| (env, lbname) -> begin
(env, (lbname)::fnames)
end))
end)) (env, []) funs)
in (match (_362640) with
| (env', fnames) -> begin
(let fnames = (Support.List.rev fnames)
in (let desugar_one_def = (fun env lbname _362651 -> (match (_362651) with
...
in (let _362671 = (desugar_binding_pat_maybe_top top_level env pat)
in (match (_362671) with
...
(let desugar_branch = (fun _362722 -> (match (_362722) with
| (pat, wopt, b) -> begin
(let _362725 = (desugar_match_pat env pat)
in (match (_362725) with
| (env, pat) -> begin
...
(let _362747 = (Support.List.hd fields)
in (match (_362747) with
| (f, _) -> begin
(let _362751 = (Microsoft_FStar_Parser_DesugarEnv.fail_or env (Microsoft_FStar_Parser_DesugarEnv.try_lookup_record_by_field_name env) f)
in (match (_362751) with
| (record, _) -> begin
(let get_field = (fun xopt f -> (let fn = f.Microsoft_FStar_Absyn_Syntax.ident
in (let found = ((Support.Microsoft.FStar.Util.find_opt (fun _362759 -> (match (_362759) with
| (g, _) -> begin
...
| None -> begin
Microsoft_FStar_Parser_AST.Construct ((record.Microsoft_FStar_Parser_DesugarEnv.constrname, ((Support.List.map (fun _362775 -> (match (_362775) with
| (f, _) -> begin
...
in (let record = Microsoft_FStar_Parser_AST.Construct ((record.Microsoft_FStar_Parser_DesugarEnv.constrname, ((Support.List.map (fun _362783 -> (match (_362783) with
| (f, _) -> begin
...
(let _362794 = (Microsoft_FStar_Parser_DesugarEnv.fail_or env (Microsoft_FStar_Parser_DesugarEnv.try_lookup_record_by_field_name env) f)
in (match (_362794) with
| (_, fieldname) -> begin
...
in (let setpos = (fun t -> (let _362806 = t
in {Microsoft_FStar_Absyn_Syntax.n = _362806.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = _362806.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = top.Microsoft_FStar_Parser_AST.range; Microsoft_FStar_Absyn_Syntax.fvs = _362806.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _362806.Microsoft_FStar_Absyn_Syntax.uvs}))
...
in (let args = (Support.List.map (fun _362889 -> (match (_362889) with
| (t, imp) -> begin
...
| Microsoft_FStar_Parser_AST.Abs ((binders, body)) -> begin
(let rec aux = (fun env bs _361301 -> (match (_361301) with
| [] -> begin
(let body = (desugar_typ env body)
in (wpos (Microsoft_FStar_Absyn_Syntax.mk_Typ_lam ((Support.List.rev bs), body))))
end
| hd::tl -> begin
(let _362907 = (desugar_binding_pat env hd)
in (match (_362907) with
| (env, bnd, pat) -> begin
(match (pat) with
| Some (q) -> begin
...
| Microsoft_FStar_Parser_AST.Product ((binders, t)) -> begin
(let _362937 = (uncurry binders t)
in (match (_362937) with
| (bs, t) -> begin
(let rec aux = (fun env bs _361302 -> (match (_361302) with
| [] -> begin
...
in (let _362951 = (as_binder env hd.Microsoft_FStar_Parser_AST.aqual bb)
in (match (_362951) with
| (b, env) -> begin
(aux env ((b)::bs) tl)
end))))
end))
...
| b -> begin
(let _362972 = (match ((as_binder env None (Support.Microsoft.FStar.Util.Inr (b)))) with
...
(let _363006 = (Support.List.fold_left (fun _362991 b -> (match (_362991) with
| (env, tparams, typs) -> begin
(let _362995 = (desugar_exp_binder env b)
in (match (_362995) with
| (xopt, t) -> begin
(let _363001 = (match (xopt) with
| None -> begin
...
in (let pre_process_comp_typ = (fun t -> (let _363031 = (head_and_args t)
in (match (_363031) with
| (head, args) -> begin
...
in (let _363057 = ((Support.List.partition (fun _363039 -> (match (_363039) with
| (arg, _) -> begin
...
in (let _363072 = (Microsoft_FStar_Absyn_Util.head_and_args t)
in (match (_363072) with
| (head, args) -> begin
...
(let _363119 = ((Support.List.partition (fun _361303 -> (match (_361303) with
...
in (match (_363119) with
| (dec, rest) -> begin
(let decreases_clause = ((Support.List.map (fun _361304 -> (match (_361304) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(match (t.Microsoft_FStar_Absyn_Syntax.n) with
...
in (let setpos = (fun kk -> (let _363150 = kk
in {Microsoft_FStar_Absyn_Syntax.n = _363150.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = _363150.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = k.Microsoft_FStar_Parser_AST.range; Microsoft_FStar_Absyn_Syntax.fvs = _363150.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _363150.Microsoft_FStar_Absyn_Syntax.uvs}))
in (let k = (unparen k)
...
(let _363184 = (uncurry bs k)
in (match (_363184) with
| (bs, k) -> begin
(let rec aux = (fun env bs _361305 -> (match (_361305) with
| [] -> begin
(pos (Microsoft_FStar_Absyn_Syntax.mk_Kind_arrow ((Support.List.rev bs), (desugar_kind env k))))
end
| hd::tl -> begin
(let _363195 = ((as_binder env hd.Microsoft_FStar_Parser_AST.aqual) (desugar_binder (Microsoft_FStar_Parser_DesugarEnv.default_ml env) hd))
in (match (_363195) with
| (b, env) -> begin
(aux env ((b)::bs) tl)
end))
end))
...
| Some (l) -> begin
(let args = (Support.List.map (fun _363205 -> (match (_363205) with
| (t, b) -> begin
...
in (let setpos = (fun t -> (let _363225 = t
in {Microsoft_FStar_Absyn_Syntax.n = _363225.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = _363225.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = f.Microsoft_FStar_Parser_AST.range; Microsoft_FStar_Absyn_Syntax.fvs = _363225.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _363225.Microsoft_FStar_Absyn_Syntax.uvs}))
in (let desugar_quant = (fun q qt b pats body -> (let tk = (desugar_binder env (let _363233 = b
in {Microsoft_FStar_Parser_AST.b = _363233.Microsoft_FStar_Parser_AST.b; Microsoft_FStar_Parser_AST.brange = _363233.Microsoft_FStar_Parser_AST.brange; Microsoft_FStar_Parser_AST.blevel = Microsoft_FStar_Parser_AST.Formula; Microsoft_FStar_Parser_AST.aqual = _363233.Microsoft_FStar_Parser_AST.aqual}))
in (match (tk) with
| Support.Microsoft.FStar.Util.Inl ((Some (a), k)) -> begin
(let _363243 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env a)
in (match (_363243) with
| (env, a) -> begin
...
| Support.Microsoft.FStar.Util.Inr ((Some (x), t)) -> begin
(let _363259 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_363259) with
| (env, x) -> begin
(let pats = (Support.List.map (fun e -> ((arg_withimp_t Microsoft_FStar_Parser_AST.Nothing) (desugar_typ_or_exp env e))) pats)
in (let body = (desugar_formula env body)
in (let body = (match (pats) with
...
and desugar_formula = (fun env t -> (desugar_formula' (let _363365 = env
in {Microsoft_FStar_Parser_DesugarEnv.curmodule = _363365.Microsoft_FStar_Parser_DesugarEnv.curmodule; Microsoft_FStar_Parser_DesugarEnv.modules = _363365.Microsoft_FStar_Parser_DesugarEnv.modules; Microsoft_FStar_Parser_DesugarEnv.open_namespaces = _363365.Microsoft_FStar_Parser_DesugarEnv.open_namespaces; Microsoft_FStar_Parser_DesugarEnv.sigaccum = _363365.Microsoft_FStar_Parser_DesugarEnv.sigaccum; Microsoft_FStar_Parser_DesugarEnv.localbindings = _363365.Microsoft_FStar_Parser_DesugarEnv.localbindings; Microsoft_FStar_Parser_DesugarEnv.recbindings = _363365.Microsoft_FStar_Parser_DesugarEnv.recbindings; Microsoft_FStar_Parser_DesugarEnv.phase = Microsoft_FStar_Parser_AST.Formula; Microsoft_FStar_Parser_DesugarEnv.sigmap = _363365.Microsoft_FStar_Parser_DesugarEnv.sigmap; Microsoft_FStar_Parser_DesugarEnv.default_result_effect = _363365.Microsoft_FStar_Parser_DesugarEnv.default_result_effect; Microsoft_FStar_Parser_DesugarEnv.iface = _363365.Microsoft_FStar_Parser_DesugarEnv.iface; Microsoft_FStar_Parser_DesugarEnv.admitted_iface = _363365.Microsoft_FStar_Parser_DesugarEnv.admitted_iface}) t))
...
and typars_of_binders = (fun env bs -> (let _363398 = (Support.List.fold_left (fun _363373 b -> (match (_363373) with
| (env, out) -> begin
(let tk = (desugar_binder env (let _363375 = b
in {Microsoft_FStar_Parser_AST.b = _363375.Microsoft_FStar_Parser_AST.b; Microsoft_FStar_Parser_AST.brange = _363375.Microsoft_FStar_Parser_AST.brange; Microsoft_FStar_Parser_AST.blevel = Microsoft_FStar_Parser_AST.Formula; Microsoft_FStar_Parser_AST.aqual = _363375.Microsoft_FStar_Parser_AST.aqual}))
in (match (tk) with
| Support.Microsoft.FStar.Util.Inl ((Some (a), k)) -> begin
(let _363385 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env a)
in (match (_363385) with
| (env, a) -> begin
(env, ((Support.Microsoft.FStar.Util.Inl ((Microsoft_FStar_Absyn_Util.bvd_to_bvar_s a k)), b.Microsoft_FStar_Parser_AST.aqual))::out)
end))
end
| Support.Microsoft.FStar.Util.Inr ((Some (x), t)) -> begin
(let _363393 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_363393) with
| (env, x) -> begin
...
and desugar_type_binder = (fun env b -> (let fail = (fun _363416 -> (match (_363416) with
| () -> begin
...
| Microsoft_FStar_Parser_AST.TVariable (x) -> begin
(Some (x), (let _363427 = Microsoft_FStar_Absyn_Syntax.mk_Kind_type
in {Microsoft_FStar_Absyn_Syntax.n = _363427.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = _363427.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = b.Microsoft_FStar_Parser_AST.brange; Microsoft_FStar_Absyn_Syntax.fvs = _363427.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _363427.Microsoft_FStar_Absyn_Syntax.uvs}))
end
| _ -> begin
(fail ())
end)))
...
in (let imp_binders = ((Support.List.map (fun _363459 -> (match (_363459) with
| (x, _) -> begin
...
let mk_indexed_projectors = (fun refine_domain env _363470 lid formals t -> (match (_363470) with
| (tc, tps, k) -> begin
...
in (let imp_binders = ((Support.List.map (fun _363484 -> (match (_363484) with
| (x, _) -> begin
(x, Some (Microsoft_FStar_Absyn_Syntax.Implicit))
end))) binders)
...
if ((Support.Microsoft.FStar.Util.for_some (fun _361306 -> (match (_361306) with
...
(let _363503 = (Microsoft_FStar_Absyn_Util.mk_field_projector_name lid a i)
in (match (_363503) with
| (field_name, _) -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _361307 -> (match (_361307) with
...
(let _363518 = (Microsoft_FStar_Absyn_Util.mk_field_projector_name lid x i)
in (match (_363518) with
| (field_name, _) -> begin
...
| Support.Microsoft.FStar.Util.Inl (a) -> begin
(let _363528 = (Microsoft_FStar_Absyn_Util.mk_field_projector_name lid a i)
in (match (_363528) with
| (field_name, _) -> begin
...
| Support.Microsoft.FStar.Util.Inr (x) -> begin
(let _363535 = (Microsoft_FStar_Absyn_Util.mk_field_projector_name lid x i)
in (match (_363535) with
| (field_name, _) -> begin
...
let mk_data_projectors = (fun env _361309 -> (match (_361309) with
...
(let refine_domain = if ((Support.Microsoft.FStar.Util.for_some (fun _361308 -> (match (_361308) with
...
let rec desugar_tycon = (fun env rng quals tcs -> (let tycon_id = (fun _361310 -> (match (_361310) with
...
in (let tycon_record_as_variant = (fun _361311 -> (match (_361311) with
...
in (let mfields = (Support.List.map (fun _363648 -> (match (_363648) with
| (x, t) -> begin
...
in (let desugar_abstract_tc = (fun quals _env mutuals _361312 -> (match (_361312) with
| Microsoft_FStar_Parser_AST.TyconAbstract ((id, binders, kopt)) -> begin
(let _363667 = (typars_of_binders _env binders)
in (match (_363667) with
| (_env', typars) -> begin
...
in (let push_tparam = (fun env _361313 -> (match (_361313) with
...
in (let _363705 = (desugar_abstract_tc quals env [] tc)
in (match (_363705) with
| (_, _, se, _) -> begin
...
(let _363716 = (typars_of_binders env binders)
in (match (_363716) with
| (env', typars) -> begin
(let k = (match (kopt) with
| None -> begin
if (Support.Microsoft.FStar.Util.for_some (fun _361314 -> (match (_361314) with
...
in (let t0 = t
in (let quals = if ((Support.Microsoft.FStar.Util.for_some (fun _361315 -> (match (_361315) with
...
in Microsoft_FStar_Absyn_Syntax.Sig_effect_abbrev (((Microsoft_FStar_Parser_DesugarEnv.qualify env id), typars, c, ((Support.List.filter (fun _361316 -> (match (_361316) with
| Microsoft_FStar_Absyn_Syntax.Effect -> begin
false
end
| _ -> begin
...
in (let _363746 = (tycon_record_as_variant trec)
in (match (_363746) with
| (t, fs) -> begin
...
in (let rec collect_tcs = (fun quals et tc -> (let _363761 = et
in (match (_363761) with
| (env, tcs) -> begin
(match (tc) with
| Microsoft_FStar_Parser_AST.TyconRecord (_) -> begin
(let trec = tc
in (let _363768 = (tycon_record_as_variant trec)
in (match (_363768) with
| (t, fs) -> begin
...
(let _363782 = (desugar_abstract_tc quals env mutuals (Microsoft_FStar_Parser_AST.TyconAbstract ((id, binders, kopt))))
in (match (_363782) with
| (env, (_, tps), se, tconstr) -> begin
(env, (Support.Microsoft.FStar.Util.Inl ((se, tps, constructors, tconstr, quals)))::tcs)
end))
end
| Microsoft_FStar_Parser_AST.TyconAbbrev ((id, binders, kopt, t)) -> begin
(let _363796 = (desugar_abstract_tc quals env mutuals (Microsoft_FStar_Parser_AST.TyconAbstract ((id, binders, kopt))))
in (match (_363796) with
| (env, (_, tps), se, tconstr) -> begin
...
in (let _363801 = (Support.List.fold_left (collect_tcs quals) (env, []) tcs)
in (match (_363801) with
| (env, tcs) -> begin
(let tcs = (Support.List.rev tcs)
in (let sigelts = ((Support.List.collect (fun _361318 -> (match (_361318) with
...
in (let _363868 = ((Support.List.split) ((Support.List.map (fun _363846 -> (match (_363846) with
...
in (let quals = ((Support.List.collect (fun _361317 -> (match (_361317) with
...
in (name, Microsoft_FStar_Absyn_Syntax.Sig_datacon ((name, (Microsoft_FStar_Absyn_Util.name_function_binders (Microsoft_FStar_Absyn_Util.close_typ (Support.List.map (fun _363865 -> (match (_363865) with
| (x, _) -> begin
(x, Some (Microsoft_FStar_Absyn_Syntax.Implicit))
end)) tps) t)), tycon, quals, mutuals, rng)))))))
end))) constrs))
in (match (_363868) with
...
in (let discs = ((Support.List.collect (fun _361319 -> (match (_361319) with
...
let desugar_binders = (fun env binders -> (let _363920 = (Support.List.fold_left (fun _363898 b -> (match (_363898) with
| (env, binders) -> begin
(match ((desugar_binder env b)) with
| Support.Microsoft.FStar.Util.Inl ((Some (a), k)) -> begin
(let _363907 = (Microsoft_FStar_Parser_DesugarEnv.push_local_tbinding env a)
in (match (_363907) with
| (env, a) -> begin
(env, ((Microsoft_FStar_Absyn_Syntax.t_binder (Microsoft_FStar_Absyn_Util.bvd_to_bvar_s a k)))::binders)
end))
end
| Support.Microsoft.FStar.Util.Inr ((Some (x), t)) -> begin
(let _363915 = (Microsoft_FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_363915) with
| (env, x) -> begin
...
(let lids = ((Support.List.map (fun _361320 -> (match (_361320) with
...
in (let quals = ((Support.List.collect (fun _361321 -> (match (_361321) with
...
(let _364026 = (desugar_binders env binders)
in (match (_364026) with
| (env_k, binders) -> begin
...
(let env0 = env
in (let _364042 = (desugar_binders env eff_binders)
in (match (_364042) with
| (env, binders) -> begin
(let defn = (desugar_typ env defn)
in (let _364046 = (Microsoft_FStar_Absyn_Util.head_and_args defn)
in (match (_364046) with
| (head, args) -> begin
...
in (let _364072 = (desugar_binders env eff_binders)
in (match (_364072) with
| (env, binders) -> begin
(let eff_k = (desugar_kind env eff_kind)
in (let _364083 = ((Support.List.fold_left (fun _364076 decl -> (match (_364076) with
| (env, out) -> begin
(let _364080 = (desugar_decl env decl)
in (match (_364080) with
| (env, ses) -> begin
(env, ((Support.List.hd ses))::out)
end))
end)) (env, [])) eff_decls)
in (match (_364083) with
| (env, decls) -> begin
...
let desugar_decls = (fun env decls -> (Support.List.fold_left (fun _364108 d -> (match (_364108) with
| (env, sigelts) -> begin
(let _364112 = (desugar_decl env d)
in (match (_364112) with
| (env, se) -> begin
...
in (match (_364139) with
| (env, mname, decls, intf) -> begin
(let _364142 = (desugar_decls env decls)
in (match (_364142) with
| (env, sigelts) -> begin
...
let desugar_modul = (fun env m -> (let _364148 = (desugar_partial_modul None env m)
in (match (_364148) with
| (env, modul) -> begin
(let env = (Microsoft_FStar_Parser_DesugarEnv.finish_module_or_interface env modul)
in (let _364150 = if (Microsoft_FStar_Options.should_dump modul.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
(Support.Microsoft.FStar.Util.fprint1 "%s\n" (Microsoft_FStar_Absyn_Print.modul_to_string modul))
end
in (env, modul)))
end)))

let desugar_file = (fun env f -> (let _364163 = (Support.List.fold_left (fun _364156 m -> (match (_364156) with
| (env, mods) -> begin
(let _364160 = (desugar_modul env m)
in (match (_364160) with
| (env, m) -> begin
(env, (m)::mods)
end))
end)) (env, []) f)
in (match (_364163) with
| (env, mods) -> begin
...
in (let en = (Support.List.fold_left Microsoft_FStar_Parser_DesugarEnv.push_sigelt (let _364167 = en
in {Microsoft_FStar_Parser_DesugarEnv.curmodule = Some (m.Microsoft_FStar_Absyn_Syntax.name); Microsoft_FStar_Parser_DesugarEnv.modules = _364167.Microsoft_FStar_Parser_DesugarEnv.modules; Microsoft_FStar_Parser_DesugarEnv.open_namespaces = _364167.Microsoft_FStar_Parser_DesugarEnv.open_namespaces; Microsoft_FStar_Parser_DesugarEnv.sigaccum = _364167.Microsoft_FStar_Parser_DesugarEnv.sigaccum; Microsoft_FStar_Parser_DesugarEnv.localbindings = _364167.Microsoft_FStar_Parser_DesugarEnv.localbindings; Microsoft_FStar_Parser_DesugarEnv.recbindings = _364167.Microsoft_FStar_Parser_DesugarEnv.recbindings; Microsoft_FStar_Parser_DesugarEnv.phase = _364167.Microsoft_FStar_Parser_DesugarEnv.phase; Microsoft_FStar_Parser_DesugarEnv.sigmap = _364167.Microsoft_FStar_Parser_DesugarEnv.sigmap; Microsoft_FStar_Parser_DesugarEnv.default_result_effect = _364167.Microsoft_FStar_Parser_DesugarEnv.default_result_effect; Microsoft_FStar_Parser_DesugarEnv.iface = _364167.Microsoft_FStar_Parser_DesugarEnv.iface; Microsoft_FStar_Parser_DesugarEnv.admitted_iface = _364167.Microsoft_FStar_Parser_DesugarEnv.admitted_iface}) m.Microsoft_FStar_Absyn_Syntax.exports)
