...
let ptsym_of_symbol = (fun s -> (match (((let _127_35 = (Microsoft_FStar_String.get s 0)
in (Microsoft_FStar_Char.lowercase _127_35)) <> (Microsoft_FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "l__" s)
end
| false -> begin
...
in (_127_40)::[])
in (Microsoft_FStar_List.append p _127_41))
in (Microsoft_FStar_String.concat "." _127_42))
end))
end))
...
| (p, s) -> begin
(let s = (match (((let _127_47 = (Microsoft_FStar_String.get s 0)
in (Microsoft_FStar_Char.uppercase _127_47)) <> (Microsoft_FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "U__" s)
end
| false -> begin
...
| true -> begin
(Microsoft_FStar_List.tryFind (fun _61_55 -> (match (_61_55) with
...
(match ((is_prims_ns ns)) with
| true -> begin
(Microsoft_FStar_List.tryFind (fun _61_63 -> (match (_61_63) with
| (y, _61_62) -> begin
(x = y)
end)) prim_uni_ops)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(Microsoft_FStar_List.tryFind (fun _61_71 -> (match (_61_71) with
| (y, _61_70) -> begin
(x = y)
end)) prim_types)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(Microsoft_FStar_List.tryFind (fun _61_79 -> (match (_61_79) with
| (y, _61_78) -> begin
(x = y)
...
in (Prims.strcat "\'" _127_87))
in (Prims.strcat _127_88 "\'"))
end
...
in (let _127_100 = (All.pipe_left escape_tyvar (Microsoft_FStar_Extraction_ML_Syntax.idsym x))
...
in (Option.get _127_107))
in (Prims.snd _127_108))
end
| false -> begin
...
in (let docs = (Microsoft_FStar_List.map (fun d -> (let _127_154 = (let _127_153 = (let _127_152 = (FSharp_Format.text ";")
...
in (let _127_164 = (Microsoft_FStar_List.map for1 fields)
...
in (Option.get _127_167))
in (Prims.snd _127_168))
end
| false -> begin
...
in (Option.get _127_169))
in (Prims.snd _127_170))
end
| false -> begin
(ptctor currentModule ctor)
end)
...
in (let _127_191 = (let _127_190 = (FSharp_Format.text (Prims.snd f))
in (_127_190)::[])
...
in (let ids = (Microsoft_FStar_List.map (fun _61_333 -> (match (_61_333) with
| ((x, _61_330), xt) -> begin
(bvar_annot x xt)
end)) ids)
in (let body = (doc_of_expr currentModule (min_op_prec, NonAssoc) body)
...
in (let _127_291 = (let _127_290 = (let _127_289 = (Microsoft_FStar_List.map (doc_of_branch currentModule) pats)
...
in (let _127_326 = (Microsoft_FStar_List.map for1 fields)
...
in (Option.get _127_329))
in (Prims.snd _127_330))
end
| false -> begin
(ptctor currentModule ctor)
end)
...
in (Option.get _127_331))
in (Prims.snd _127_332))
end
| false -> begin
(ptctor currentModule ctor)
end)
...
in (let _127_341 = (let _127_340 = (let _127_339 = (Microsoft_FStar_List.hd pats)
...
in (let _127_344 = (Microsoft_FStar_List.map (doc_of_pattern currentModule) pats)
...
(let e = (doc_of_expr currentModule (min_op_prec, NonAssoc) e)
in (let ids = []
in (let ids = (Microsoft_FStar_List.map (fun _61_468 -> (match (_61_468) with
| (x, _61_467) -> begin
(FSharp_Format.text x)
end)) ids)
...
in (let lets = (Microsoft_FStar_List.mapi (fun i doc -> (let _127_398 = (let _127_397 = (match ((i = 0)) with
| true -> begin
letdoc
end
| false -> begin
...
in (let _127_415 = (Microsoft_FStar_List.map forfield fields)
...
in (let ctors = (Microsoft_FStar_List.map (fun d -> (let _127_427 = (let _127_426 = (FSharp_Format.text "|")
in (_127_426)::(d)::[])
...
in (let doc = (Option.map (fun _61_609 -> (match (_61_609) with
| (s, _61_608) -> begin
(doc_of_sig x s)
end)) sigmod)
...
| (x, sigmod, Microsoft_FStar_Extraction_ML_Syntax.MLLib (sub)) -> begin
(let _61_623 = (Microsoft_FStar_Util.fprint1 "Gen Code: %s\n" x)
...
in (let doc = (Option.map (fun _61_630 -> (match (_61_630) with
| (_61_628, m) -> begin
(doc_of_mod x m)
end)) sigmod)
...
in (Microsoft_FStar_List.append prefix _127_564))
in (All.pipe_left FSharp_Format.reduce _127_565)))))))))
end))
in (let docs = (Microsoft_FStar_List.map (fun _61_642 -> (match (_61_642) with
| (x, s, m) -> begin
