        ...
        let env1 =
          ...
          | (Some
             { FStar_Syntax_Syntax.name = prev_lid;
               ...
               FStar_Syntax_Syntax.is_interface = uu____10309;_},FStar_Parser_AST.Module
             (current_lid,uu____10311)) when
              (FStar_Ident.lid_equals prev_lid current_lid) &&
                (FStar_Options.interactive ())
              -> env
          | (Some prev_mod,uu____10316) ->
          ...
          | FStar_Parser_AST.Interface (mname,decls,admitted) ->
              let uu____10338 =
                FStar_ToSyntax_Env.prepare_module_or_interface true admitted
                  env1 mname in
              (uu____10338, mname, decls, true)
          | FStar_Parser_AST.Module (mname,decls) ->
              let uu____10348 =
                FStar_ToSyntax_Env.prepare_module_or_interface false false
                  env1 mname in
              (uu____10348, mname, decls, false) in
        match uu____10318 with
        | ((env2,pop_when_done),mname,decls,intf) ->
            let uu____10366 = desugar_decls env2 decls in
        ...
        let m1 =
          let uu____10391 =
            (FStar_Options.interactive ()) &&
              (let uu____10392 =
                 let uu____10393 =
                   let uu____10394 = FStar_Options.file_list () in
                   FStar_List.hd uu____10394 in
                 FStar_Util.get_file_extension uu____10393 in
               uu____10392 = "fsti") in
          if uu____10391
          then
            match m with
            ...
            | FStar_Parser_AST.Interface (mname,uu____10402,uu____10403) ->
                failwith
                  (Prims.strcat "Impossible: "
                     (mname.FStar_Ident.ident).FStar_Ident.idText)
          else m in
        let uu____10407 = desugar_modul_common curmod env m1 in
        match uu____10407 with
        | (x,y,pop_when_done) ->
            (if pop_when_done
             then (let uu____10417 = FStar_ToSyntax_Env.pop () in ())
             else ();
             (x, y))
let desugar_modul:
  FStar_ToSyntax_Env.env ->
  ...
  fun env  ->
    fun m  ->
      let uu____10429 = desugar_modul_common None env m in
      ...
      | (env1,modul,pop_when_done) ->
          ...
          ((let uu____10440 =
            ...
            if uu____10440
            then
              let uu____10441 = FStar_Syntax_Print.modul_to_string modul in
              FStar_Util.print1 "%s\n" uu____10441
            else ());
           (let uu____10443 =
              if pop_when_done
        ...
        FStar_List.fold_left
          (fun uu____10461  ->
             fun m  ->
               ...
               | (env1,mods) ->
                   let uu____10473 = desugar_modul env1 m in
                   (match uu____10473 with
                    | (env2,m1) -> (env2, (m1 :: mods)))) (env, []) f in
      match uu____10454 with | (env1,mods) -> (env1, (FStar_List.rev mods))
  ...
  fun m  ->
    fun en  ->
      ...
      | (en1,pop_when_done) ->
          let en2 =
            FStar_List.fold_left FStar_ToSyntax_Env.push_sigelt
              (let uu___224_10503 = en1 in
               {
                 FStar_ToSyntax_Env.curmodule =
                   (Some (m.FStar_Syntax_Syntax.name));
                 FStar_ToSyntax_Env.curmonad =
                   (uu___224_10503.FStar_ToSyntax_Env.curmonad);
                 FStar_ToSyntax_Env.modules =
                   (uu___224_10503.FStar_ToSyntax_Env.modules);
                 FStar_ToSyntax_Env.scope_mods =
                   (uu___224_10503.FStar_ToSyntax_Env.scope_mods);
                 FStar_ToSyntax_Env.exported_ids =
                   (uu___224_10503.FStar_ToSyntax_Env.exported_ids);
                 FStar_ToSyntax_Env.trans_exported_ids =
                   (uu___224_10503.FStar_ToSyntax_Env.trans_exported_ids);
                 FStar_ToSyntax_Env.includes =
                   (uu___224_10503.FStar_ToSyntax_Env.includes);
                 FStar_ToSyntax_Env.sigaccum =
                   (uu___224_10503.FStar_ToSyntax_Env.sigaccum);
                 FStar_ToSyntax_Env.sigmap =
                   (uu___224_10503.FStar_ToSyntax_Env.sigmap);
                 FStar_ToSyntax_Env.iface =
                   (uu___224_10503.FStar_ToSyntax_Env.iface);
                 FStar_ToSyntax_Env.admitted_iface =
                   (uu___224_10503.FStar_ToSyntax_Env.admitted_iface);
                 FStar_ToSyntax_Env.expect_typ =
                   (uu___224_10503.FStar_ToSyntax_Env.expect_typ)
