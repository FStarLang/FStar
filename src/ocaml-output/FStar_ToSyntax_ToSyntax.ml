...
(match (se) with
| FStar_Syntax_Syntax.Sig_datacon (lid, uu____5863, t, uu____5865, n, quals, uu____5868, uu____5869) when (not ((FStar_Ident.lid_equals lid FStar_Syntax_Const.lexcons_lid))) -> begin
(

let uu____5874 = (FStar_Syntax_Util.arrow_formals t)
in (match (uu____5874) with
| (formals, uu____5884) -> begin
(match (formals) with
| [] -> begin
[]
end
...
let filter_records = (fun uu___200_5906 -> (match (uu___200_5906) with
| FStar_Syntax_Syntax.RecordConstructor (uu____5908, fns) -> begin
...
let uu____5917 = (FStar_Util.find_map quals filter_records)
...
let uu____5924 = (FStar_Util.first_N n formals)
in (match (uu____5924) with
| (uu____5936, rest) -> begin
...
let uu____5988 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Abstract))
...
let tycon_id = (fun uu___201_6022 -> (match (uu___201_6022) with
...
let tycon_record_as_variant = (fun uu___202_6091 -> (match (uu___202_6091) with
...
let mfields = (FStar_List.map (fun uu____6120 -> (match (uu____6120) with
| (x, t, uu____6127) -> begin
...
in (let _0_600 = (FStar_All.pipe_right fields (FStar_List.map (fun uu____6167 -> (match (uu____6167) with
| (x, uu____6173, uu____6174) -> begin
...
let desugar_abstract_tc = (fun quals _env mutuals uu___203_6199 -> (match (uu___203_6199) with
| FStar_Parser_AST.TyconAbstract (id, binders, kopt) -> begin
(

let uu____6213 = (typars_of_binders _env binders)
...
let uu____6277 = (FStar_List.fold_left (fun uu____6293 uu____6294 -> (match (((uu____6293), (uu____6294))) with
| ((env, tps), (x, imp)) -> begin
(

let uu____6342 = (FStar_ToSyntax_Env.push_bv env x.FStar_Syntax_Syntax.ppname)
...
let uu____6408 = (desugar_abstract_tc quals env [] tc)
in (match (uu____6408) with
| (uu____6415, uu____6416, se, uu____6418) -> begin
(

let se = (match (se) with
| FStar_Syntax_Syntax.Sig_inductive_typ (l, uu____6421, typars, k, [], [], quals, rng) -> begin
(

let quals = (

let uu____6432 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Assumption))
...
let uu____6462 = (typars_of_binders env binders)
...
let uu____6482 = (FStar_Util.for_some (fun uu___204_6483 -> (match (uu___204_6483) with
...
let uu____6490 = (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___205_6492 -> (match (uu___205_6492) with
...
let uu____6499 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Effect))
...
let uu____6505 = (unparen t).FStar_Parser_AST.tm
in (match (uu____6505) with
| FStar_Parser_AST.Construct (head, args) -> begin
(

let uu____6517 = (match ((FStar_List.rev args)) with
| ((last_arg, uu____6533))::args_rev -> begin
(

let uu____6540 = (unparen last_arg).FStar_Parser_AST.tm
...
in (let _0_607 = (FStar_All.pipe_right quals (FStar_List.filter (fun uu___206_6601 -> (match (uu___206_6601) with
| FStar_Syntax_Syntax.Effect -> begin
false
end
| uu____6602 -> begin
...
| (FStar_Parser_AST.TyconRecord (uu____6608))::[] -> begin
(

let trec = (FStar_List.hd tcs)
in (

let uu____6621 = (tycon_record_as_variant trec)
...
| (env, tcs) -> begin
(match (tc) with
| FStar_Parser_AST.TyconRecord (uu____6835) -> begin
(

let trec = tc
in (

let uu____6848 = (tycon_record_as_variant trec)
...
let uu____6924 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (uu____6924) with
| (env, uu____6955, se, tconstr) -> begin
...
let uu____7033 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (uu____7033) with
| (env, uu____7064, se, tconstr) -> begin
...
let uu____7152 = (FStar_List.fold_left (collect_tcs quals) ((env), ([])) tcs)
...
let tps_sigelts = (FStar_All.pipe_right tcs (FStar_List.collect (fun uu___208_7390 -> (match (uu___208_7390) with
| FStar_Util.Inr (FStar_Syntax_Syntax.Sig_inductive_typ (id, uvs, tpars, k, uu____7422, uu____7423, uu____7424, uu____7425), binders, t, quals) -> begin
(

let t = (

let uu____7458 = (typars_of_binders env binders)
in (match (uu____7458) with
| (env, tpars) -> begin
(

let uu____7475 = (push_tparams env tpars)
...
| FStar_Util.Inl (FStar_Syntax_Syntax.Sig_inductive_typ (tname, univs, tpars, k, mutuals, uu____7518, tags, uu____7520), constrs, tconstr, quals) -> begin
(

let tycon = ((tname), (tpars), (k))
in (

let uu____7568 = (push_tparams env tpars)
in (match (uu____7568) with
| (env_tps, tps) -> begin
(

let data_tpars = (FStar_List.map (fun uu____7603 -> (match (uu____7603) with
| (x, uu____7611) -> begin
((x), (Some (FStar_Syntax_Syntax.Implicit (true))))
end)) tps)
in (

let uu____7615 = (let _0_623 = (FStar_All.pipe_right constrs (FStar_List.map (fun uu____7681 -> (match (uu____7681) with
| (id, topt, uu____7698, of_notation) -> begin
(
...
let quals = (FStar_All.pipe_right tags (FStar_List.collect (fun uu___207_7715 -> (match (uu___207_7715) with
...
let uu____7835 = (let _0_624 = (FStar_List.collect FStar_Syntax_Util.lids_of_sigelt sigelts)
...
let discs = (FStar_All.pipe_right sigelts (FStar_List.collect (fun uu___209_7871 -> (match (uu___209_7871) with
| FStar_Syntax_Syntax.Sig_inductive_typ (tname, uu____7874, tps, k, uu____7877, constrs, quals, uu____7880) when ((FStar_List.length constrs) > (Prims.parse_int "1")) -> begin
(
...
let uu____7912 = (FStar_List.fold_left (fun uu____7919 b -> (match (uu____7919) with
| (env, binders) -> begin
(

let uu____7931 = (desugar_binder env b)
in (match (uu____7931) with
| (Some (a), k) -> begin
(

let uu____7941 = (FStar_ToSyntax_Env.push_bv env a)
in (match (uu____7941) with
| (env, a) -> begin
(let _0_626 = (let _0_625 = (FStar_Syntax_Syntax.mk_binder (

let uu___223_7950 = a
in {FStar_Syntax_Syntax.ppname = uu___223_7950.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = uu___223_7950.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = k}))
in (_0_625)::binders)
in ((env), (_0_626)))
end))
end
...
let uu____8045 = (desugar_binders monad_env eff_binders)
...
let uu____8057 = (FStar_All.pipe_right eff_decls (FStar_List.fold_left (fun uu____8068 decl -> (match (uu____8068) with
| (env, out) -> begin
(

let uu____8080 = (desugar_decl env decl)
...
| FStar_Parser_AST.Tycon (uu____8103, ((FStar_Parser_AST.TyconAbbrev (name, uu____8105, uu____8106, {FStar_Parser_AST.tm = FStar_Parser_AST.Construct (uu____8107, ((def, uu____8109))::((cps_type, uu____8111))::[]); FStar_Parser_AST.range = uu____8112; FStar_Parser_AST.level = uu____8113}), uu____8114))::[]) when (not (for_free)) -> begin
...
| FStar_Parser_AST.Tycon (uu____8140, ((FStar_Parser_AST.TyconAbbrev (name, uu____8142, uu____8143, defn), uu____8145))::[]) when for_free -> begin
...
let uu____8217 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Parser_AST.Reflectable))
...
let uu____8245 = (desugar_binders env eff_binders)
...
let uu____8265 = (head_and_args defn)
...
let uu____8290 = (match ((FStar_List.rev args)) with
| ((last_arg, uu____8306))::args_rev -> begin
(

let uu____8313 = (unparen last_arg).FStar_Parser_AST.tm
...
let sub = (fun uu____8390 -> (match (uu____8390) with
| (uu____8397, x) -> begin
(

let uu____8401 = (FStar_Syntax_Subst.open_term ed.FStar_Syntax_Syntax.binders x)
...
let uu____8441 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Parser_AST.Reflectable))
...
| FStar_Parser_AST.Fsdoc (uu____8464) -> begin
((env), ([]))
end
...
let tcs = (FStar_List.map (fun uu____8496 -> (match (uu____8496) with
| (x, uu____8501) -> begin
x
end)) tcs)
...
let uu____8557 = (FStar_All.pipe_left FStar_Syntax_Subst.compress ds_lets).FStar_Syntax_Syntax.n
in (match (uu____8557) with
| FStar_Syntax_Syntax.Tm_let (lbs, uu____8561) -> begin
(
...
| (uu____8581)::uu____8582 -> begin
(FStar_List.map (trans_qual None) quals)
end
| uu____8584 -> begin
(FStar_All.pipe_right (Prims.snd lbs) (FStar_List.collect (fun uu___210_8588 -> (match (uu___210_8588) with
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (uu____8590); FStar_Syntax_Syntax.lbunivs = uu____8591; FStar_Syntax_Syntax.lbtyp = uu____8592; FStar_Syntax_Syntax.lbeff = uu____8593; FStar_Syntax_Syntax.lbdef = uu____8594} -> begin
[]
end
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (fv); FStar_Syntax_Syntax.lbunivs = uu____8601; FStar_Syntax_Syntax.lbtyp = uu____8602; FStar_Syntax_Syntax.lbeff = uu____8603; FStar_Syntax_Syntax.lbdef = uu____8604} -> begin
...
let uu____8616 = (FStar_All.pipe_right lets (FStar_Util.for_some (fun uu____8622 -> (match (uu____8622) with
| (uu____8625, t) -> begin
...
let uu____8633 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Abstract))
...
let uu___224_8645 = lb
in {FStar_Syntax_Syntax.lbname = FStar_Util.Inr ((

let uu___225_8646 = fv
in {FStar_Syntax_Syntax.fv_name = uu___225_8646.FStar_Syntax_Syntax.fv_name; FStar_Syntax_Syntax.fv_delta = FStar_Syntax_Syntax.Delta_abstract (fv.FStar_Syntax_Syntax.fv_delta); FStar_Syntax_Syntax.fv_qual = uu___225_8646.FStar_Syntax_Syntax.fv_qual})); FStar_Syntax_Syntax.lbunivs = uu___224_8645.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = uu___224_8645.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = uu___224_8645.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = uu___224_8645.FStar_Syntax_Syntax.lbdef})))))
...
let uu____8671 = (match (lets) with
| ((pat, body))::[] -> begin
((pat), (body))
end
| uu____8682 -> begin
...
let uu___226_8698 = pat
in {FStar_Parser_AST.pat = FStar_Parser_AST.PatAscribed (((var_pat), (ty))); FStar_Parser_AST.prange = uu___226_8698.FStar_Parser_AST.prange})
end
| uu____8699 -> begin
var_pat
end))
...
let uu___227_8703 = d
in {FStar_Parser_AST.d = FStar_Parser_AST.TopLevelLet (((isrec), ((((fresh_pat), (body)))::[]))); FStar_Parser_AST.drange = uu___227_8703.FStar_Parser_AST.drange; FStar_Parser_AST.doc = uu___227_8703.FStar_Parser_AST.doc; FStar_Parser_AST.quals = (FStar_Parser_AST.Private)::d.FStar_Parser_AST.quals; FStar_Parser_AST.attrs = uu___227_8703.FStar_Parser_AST.attrs}))
in (

let build_projection = (fun uu____8722 id -> (match (uu____8722) with
| (env, ses) -> begin
(
...
let uu____8762 = (desugar_decl env id_decl)
...
let uu____8802 = (FStar_ToSyntax_Env.fail_or env (FStar_ToSyntax_Env.try_lookup_lid env) FStar_Syntax_Const.exn_lid)
...
let uu____8865 = (desugar_binders env binders)
...
let uu____8925 = (FStar_ToSyntax_Env.try_lookup_effect_name env l)
...
let uu____8930 = (match (l.FStar_Parser_AST.lift_op) with
...
let desugar_decls : FStar_ToSyntax_Env.env  ->  FStar_Parser_AST.decl Prims.list  ->  (FStar_ToSyntax_Env.env * FStar_Syntax_Syntax.sigelts) = (fun env decls -> (FStar_List.fold_left (fun uu____9036 d -> (match (uu____9036) with
| (env, sigelts) -> begin
(

let uu____9048 = (desugar_decl env d)
...
let uu____9132 = (desugar_decls env decls)
...
let uu____9157 = ((FStar_Options.interactive ()) && (let _0_718 = (FStar_Util.get_file_extension (FStar_List.hd (FStar_Options.file_list ())))
in (_0_718 = "fsti")))
...
| FStar_Parser_AST.Interface (mname, uu____9164, uu____9165) -> begin
...
let uu____9169 = (desugar_modul_common curmod env m)
in (match (uu____9169) with
| (x, y, uu____9177) -> begin
((x), (y))
end))))
...
let uu____9188 = (desugar_modul_common None env m)
...
let uu____9199 = (FStar_Options.dump_module modul.FStar_Syntax_Syntax.name.FStar_Ident.str)
...
let uu____9211 = (FStar_List.fold_left (fun uu____9218 m -> (match (uu____9218) with
| (env, mods) -> begin
(

let uu____9230 = (desugar_modul env m)
...
let uu____9254 = (FStar_ToSyntax_Env.prepare_module_or_interface false false en m.FStar_Syntax_Syntax.name)
...
let uu___228_9260 = en
in {FStar_ToSyntax_Env.curmodule = Some (m.FStar_Syntax_Syntax.name); FStar_ToSyntax_Env.curmonad = uu___228_9260.FStar_ToSyntax_Env.curmonad; FStar_ToSyntax_Env.modules = uu___228_9260.FStar_ToSyntax_Env.modules; FStar_ToSyntax_Env.scope_mods = uu___228_9260.FStar_ToSyntax_Env.scope_mods; FStar_ToSyntax_Env.exported_ids = uu___228_9260.FStar_ToSyntax_Env.exported_ids; FStar_ToSyntax_Env.trans_exported_ids = uu___228_9260.FStar_ToSyntax_Env.trans_exported_ids; FStar_ToSyntax_Env.includes = uu___228_9260.FStar_ToSyntax_Env.includes; FStar_ToSyntax_Env.sigaccum = uu___228_9260.FStar_ToSyntax_Env.sigaccum; FStar_ToSyntax_Env.sigmap = uu___228_9260.FStar_ToSyntax_Env.sigmap; FStar_ToSyntax_Env.default_result_effect = uu___228_9260.FStar_ToSyntax_Env.default_result_effect; FStar_ToSyntax_Env.iface = uu___228_9260.FStar_ToSyntax_Env.iface; FStar_ToSyntax_Env.admitted_iface = uu___228_9260.FStar_ToSyntax_Env.admitted_iface; FStar_ToSyntax_Env.expect_typ = uu___228_9260.FStar_ToSyntax_Env.expect_typ}) m.FStar_Syntax_Syntax.exports)
in (
