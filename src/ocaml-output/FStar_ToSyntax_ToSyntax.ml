...
in {FStar_Syntax_Syntax.sigel = uu____6536; FStar_Syntax_Syntax.sigrng = (FStar_Ident.range_of_lid disc_name)}))))))))
...
in {FStar_Syntax_Syntax.sigel = uu____6631; FStar_Syntax_Syntax.sigrng = p})
...
let uu____6815 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Abstract))
in (match (uu____6815) with
| true -> begin
(

let uu____6817 = (FStar_Syntax_Util.incr_delta_qualifier t)
in FStar_Syntax_Syntax.Delta_abstract (uu____6817))
end
| uu____6818 -> begin
...
let uu____6823 = (FStar_Syntax_Syntax.lid_as_fv lid dd None)
in FStar_Util.Inr (uu____6823))
in (

let uu____6824 = (

let uu____6827 = (FStar_Syntax_Syntax.mk_Total k)
in (FStar_Syntax_Util.arrow typars uu____6827))
in (

let uu____6830 = (no_annot_abs typars t)
in {FStar_Syntax_Syntax.lbname = uu____6820; FStar_Syntax_Syntax.lbunivs = uvs; FStar_Syntax_Syntax.lbtyp = uu____6824; FStar_Syntax_Syntax.lbeff = FStar_Syntax_Const.effect_Tot_lid; FStar_Syntax_Syntax.lbdef = uu____6830})))
...
let tycon_id = (fun uu___197_6864 -> (match (uu___197_6864) with
...
in FStar_Parser_AST.Var (uu____6904))
in (FStar_Parser_AST.mk_term uu____6903 x.FStar_Ident.idRange FStar_Parser_AST.Expr))
end
...
let uu____6934 = (binder_to_term b)
in ((out), (uu____6934), ((imp_of_aqual b))))
in FStar_Parser_AST.App (uu____6930))
in (FStar_Parser_AST.mk_term uu____6929 out.FStar_Parser_AST.range out.FStar_Parser_AST.level))) t binders)))
in (

let tycon_record_as_variant = (fun uu___198_6941 -> (match (uu___198_6941) with
...
let mfields = (FStar_List.map (fun uu____6970 -> (match (uu____6970) with
| (x, t, uu____6977) -> begin
...
let uu____6983 = (FStar_Ident.lid_of_ids ((id)::[]))
in FStar_Parser_AST.Var (uu____6983))
in (FStar_Parser_AST.mk_term uu____6982 id.FStar_Ident.idRange FStar_Parser_AST.Type_level))
in (apply_binders uu____6981 parms))
in (
...
let uu____6986 = (FStar_All.pipe_right fields (FStar_List.map (fun uu____6998 -> (match (uu____6998) with
| (x, uu____7004, uu____7005) -> begin
(FStar_Syntax_Util.unmangle_field_name x)
end))))
in ((FStar_Parser_AST.TyconVariant (((id), (parms), (kopt), ((((constrName), (Some (constrTyp)), (None), (false)))::[])))), (uu____6986)))))))
end
| uu____7032 -> begin
(failwith "impossible")
end))
...
let desugar_abstract_tc = (fun quals1 _env mutuals uu___199_7054 -> (match (uu___199_7054) with
| FStar_Parser_AST.TyconAbstract (id, binders, kopt) -> begin
(

let uu____7068 = (typars_of_binders _env binders)
...
let uu____7098 = (FStar_Ident.lid_of_ids ((id)::[]))
in FStar_Parser_AST.Var (uu____7098))
in (FStar_Parser_AST.mk_term uu____7097 id.FStar_Ident.idRange FStar_Parser_AST.Type_level))
in (apply_binders uu____7096 binders))
in (
...
let uu____7135 = (FStar_List.fold_left (fun uu____7151 uu____7152 -> (match (((uu____7151), (uu____7152))) with
| ((env2, tps), (x, imp)) -> begin
(

let uu____7200 = (FStar_ToSyntax_Env.push_bv env2 x.FStar_Syntax_Syntax.ppname)
...
let uu____7261 = (tm_type_z id.FStar_Ident.idRange)
in Some (uu____7261))
end
| uu____7262 -> begin
kopt
...
let uu____7267 = (desugar_abstract_tc quals env [] tc)
in (match (uu____7267) with
| (uu____7274, uu____7275, se, uu____7277) -> begin
(

let se1 = (match (se.FStar_Syntax_Syntax.sigel) with
| FStar_Syntax_Syntax.Sig_inductive_typ (l, uu____7280, typars, k, [], [], quals1) -> begin
(

let quals2 = (

let uu____7290 = (FStar_All.pipe_right quals1 (FStar_List.contains FStar_Syntax_Syntax.Assumption))
...
let uu____7295 = (FStar_Range.string_of_range se.FStar_Syntax_Syntax.sigrng)
in (

let uu____7296 = (FStar_Syntax_Print.lid_to_string l)
...
let uu____7313 = (FStar_Syntax_Syntax.mk_Total k)
in ((typars), (uu____7313)))
in FStar_Syntax_Syntax.Tm_arrow (uu____7305))
in (FStar_Syntax_Syntax.mk uu____7304))
in (uu____7301 None se.FStar_Syntax_Syntax.sigrng))
end)
in (

let uu___219_7322 = se
in {FStar_Syntax_Syntax.sigel = FStar_Syntax_Syntax.Sig_declare_typ (((l), ([]), (t), (quals2))); FStar_Syntax_Syntax.sigrng = uu___219_7322.FStar_Syntax_Syntax.sigrng})))
end
| uu____7325 -> begin
(failwith "Impossible")
end)
...
let uu____7328 = (FStar_ToSyntax_Env.qualify env1 id)
in (FStar_ToSyntax_Env.push_doc env1 uu____7328 d.FStar_Parser_AST.doc))
in ((env2), ((se1)::[])))))
end))))
end
...
let uu____7338 = (typars_of_binders env binders)
...
let uu____7358 = (FStar_Util.for_some (fun uu___200_7359 -> (match (uu___200_7359) with
...
let uu____7366 = (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___201_7368 -> (match (uu___201_7368) with
...
let uu____7376 = (FStar_All.pipe_right quals1 (FStar_List.contains FStar_Syntax_Syntax.Effect))
...
let uu____7383 = (unparen t)
in uu____7383.FStar_Parser_AST.tm)
in (match (uu____7382) with
| FStar_Parser_AST.Construct (head1, args) -> begin
(

let uu____7395 = (match ((FStar_List.rev args)) with
| ((last_arg, uu____7411))::args_rev -> begin
(

let uu____7418 = (

let uu____7419 = (unparen last_arg)
in uu____7419.FStar_Parser_AST.tm)
...
let uu____7460 = (desugar_attributes env cattributes)
in (((FStar_Parser_AST.mk_term (FStar_Parser_AST.Construct (((head1), (args1)))) t.FStar_Parser_AST.range t.FStar_Parser_AST.level)), (uu____7460)))
end))
end
| uu____7466 -> begin
((t), ([]))
...
let uu____7487 = (FStar_All.pipe_right quals1 (FStar_List.filter (fun uu___202_7491 -> (match (uu___202_7491) with
| FStar_Syntax_Syntax.Effect -> begin
false
end
| uu____7492 -> begin
...
in ((qlid), ([]), (typars1), (c1), (uu____7487), ((FStar_List.append cattributes (FStar_Syntax_Util.comp_flags c1)))))
in FStar_Syntax_Syntax.Sig_effect_abbrev (uu____7478))
in {FStar_Syntax_Syntax.sigel = uu____7477; FStar_Syntax_Syntax.sigrng = rng}))))
end))
end
| uu____7496 -> begin
(
...
| (FStar_Parser_AST.TyconRecord (uu____7501))::[] -> begin
(

let trec = (FStar_List.hd tcs)
in (

let uu____7514 = (tycon_record_as_variant trec)
...
let uu____7534 = (FStar_ToSyntax_Env.current_module env)
in (FStar_Ident.ids_of_lid uu____7534))
in ((uu____7532), (fs)))
in FStar_Syntax_Syntax.RecordType (uu____7527))
in (uu____7526)::quals)
in (desugar_tycon env d uu____7524 ((t)::[])))
end)))
end
| (uu____7537)::uu____7538 -> begin
(
...
| (env1, tcs1) -> begin
(match (tc) with
| FStar_Parser_AST.TyconRecord (uu____7739) -> begin
(

let trec = tc
in (

let uu____7752 = (tycon_record_as_variant trec)
...
let uu____7793 = (FStar_ToSyntax_Env.current_module env1)
in (FStar_Ident.ids_of_lid uu____7793))
in ((uu____7791), (fs)))
in FStar_Syntax_Syntax.RecordType (uu____7786))
in (uu____7785)::quals1)
in (collect_tcs uu____7783 ((env1), (tcs1)) t))
end)))
end
| FStar_Parser_AST.TyconVariant (id, binders, kopt, constructors) -> begin
(

let uu____7839 = (desugar_abstract_tc quals1 env1 mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (uu____7839) with
| (env2, uu____7870, se, tconstr) -> begin
...
let uu____7948 = (desugar_abstract_tc quals1 env1 mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (uu____7948) with
| (env2, uu____7979, se, tconstr) -> begin
...
let uu____8067 = (FStar_List.fold_left (collect_tcs quals) ((env), ([])) tcs)
...
let docs_tps_sigelts = (FStar_All.pipe_right tcs2 (FStar_List.collect (fun uu___204_8317 -> (match (uu___204_8317) with
| FStar_Util.Inr ({FStar_Syntax_Syntax.sigel = FStar_Syntax_Syntax.Sig_inductive_typ (id, uvs, tpars, k, uu____8353, uu____8354, uu____8355); FStar_Syntax_Syntax.sigrng = uu____8356}, binders, t, quals1) -> begin
(

let t1 = (

let uu____8389 = (typars_of_binders env1 binders)
in (match (uu____8389) with
| (env2, tpars1) -> begin
(

let uu____8406 = (push_tparams env2 tpars1)
...
let uu____8436 = (mk_typ_abbrev id uvs tpars k t1 ((id)::[]) quals1 rng)
in ((((id), (d.FStar_Parser_AST.doc))), ([]), (uu____8436)))
in (uu____8425)::[]))
end
| FStar_Util.Inl ({FStar_Syntax_Syntax.sigel = FStar_Syntax_Syntax.Sig_inductive_typ (tname, univs, tpars, k, mutuals1, uu____8473, tags); FStar_Syntax_Syntax.sigrng = uu____8475}, constrs, tconstr, quals1) -> begin
(
...
let uu____8528 = (push_tparams env1 tpars)
in (match (uu____8528) with
| (env_tps, tps) -> begin
(

let data_tpars = (FStar_List.map (fun uu____8567 -> (match (uu____8567) with
| (x, uu____8575) -> begin
...
let uu____8595 = (FStar_All.pipe_right constrs (FStar_List.map (fun uu____8647 -> (match (uu____8647) with
...
let uu____8680 = (close env_tps t)
in (desugar_term env_tps uu____8680))
in (

let name = (FStar_ToSyntax_Env.qualify env1 id)
in (

let quals2 = (FStar_All.pipe_right tags (FStar_List.collect (fun uu___203_8686 -> (match (uu___203_8686) with
...
let uu____8728 = (FStar_All.pipe_right t1 FStar_Syntax_Util.name_function_binders)
in (FStar_Syntax_Syntax.mk_Total uu____8728))
in (FStar_Syntax_Util.arrow data_tpars uu____8725))
in ((name), (univs), (uu____8722), (tname), (ntps), (quals2), (mutuals1)))
in FStar_Syntax_Syntax.Sig_datacon (uu____8712))
in {FStar_Syntax_Syntax.sigel = uu____8711; FStar_Syntax_Syntax.sigrng = rng})
in ((((name), (doc1))), (tps), (uu____8710)))
in ((name), (uu____8699))))))))
end))))
in (FStar_All.pipe_left FStar_List.split uu____8595))
...
let name_docs = (FStar_All.pipe_right docs_tps_sigelts (FStar_List.map (fun uu____8918 -> (match (uu____8918) with
| (name_doc, uu____8933, uu____8934) -> begin
name_doc
end))))
in (

let sigelts = (FStar_All.pipe_right docs_tps_sigelts (FStar_List.map (fun uu____8973 -> (match (uu____8973) with
| (uu____8984, uu____8985, se) -> begin
se
end))))
in (
...
let uu____9005 = (FStar_List.collect FStar_Syntax_Util.lids_of_sigelt sigelts)
in (FStar_Syntax_MutRecTy.disentangle_abbrevs_from_bundle sigelts quals uu____9005 rng))
...
let data_ops = (FStar_All.pipe_right docs_tps_sigelts (FStar_List.collect (fun uu____9039 -> (match (uu____9039) with
| (uu____9051, tps, se) -> begin
...
| FStar_Syntax_Syntax.Sig_inductive_typ (tname, uu____9083, tps, k, uu____9086, constrs, quals1) when ((FStar_List.length constrs) > (Prims.parse_int "1")) -> begin
(
...
let env5 = (FStar_List.fold_left (fun acc uu____9111 -> (match (uu____9111) with
| (lid, doc1) -> begin
...
let uu____9133 = (FStar_List.fold_left (fun uu____9140 b -> (match (uu____9140) with
| (env1, binders1) -> begin
(

let uu____9152 = (desugar_binder env1 b)
in (match (uu____9152) with
| (Some (a), k) -> begin
(

let uu____9162 = (FStar_ToSyntax_Env.push_bv env1 a)
...
let uu____9172 = (FStar_Syntax_Syntax.mk_binder (

let uu___220_9173 = a1
in {FStar_Syntax_Syntax.ppname = uu___220_9173.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = uu___220_9173.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = k}))
in (uu____9172)::binders1)
in ((env2), (uu____9170)))
end))
end
| uu____9175 -> begin
...
let uu____9253 = (desugar_binders monad_env eff_binders)
...
let uu____9271 = (FStar_Syntax_Util.arrow_formals eff_t)
in (Prims.fst uu____9271))
in (FStar_List.length uu____9267))
in (uu____9266 = (Prims.parse_int "1")))
in (
...
| FStar_Parser_AST.Tycon (uu____9299, ((FStar_Parser_AST.TyconAbbrev (name, uu____9301, uu____9302, uu____9303), uu____9304))::[]) -> begin
...
let uu____9322 = (FStar_List.partition (fun decl -> (

let uu____9328 = (name_of_eff_decl decl)
in (FStar_List.mem uu____9328 mandatory_members))) eff_decls)
in (match (uu____9322) with
| (mandatory_members_decls, actions) -> begin
(

let uu____9338 = (FStar_All.pipe_right mandatory_members_decls (FStar_List.fold_left (fun uu____9349 decl -> (match (uu____9349) with
| (env2, out) -> begin
(

let uu____9361 = (desugar_decl env2 decl)
...
let uu____9371 = (FStar_List.hd ses)
in (uu____9371)::out)
in ((env3), (uu____9369)))
end))
end)) ((env1), ([]))))
...
| FStar_Parser_AST.Tycon (uu____9399, ((FStar_Parser_AST.TyconAbbrev (name, uu____9401, uu____9402, {FStar_Parser_AST.tm = FStar_Parser_AST.Construct (uu____9403, ((def, uu____9405))::((cps_type, uu____9407))::[]); FStar_Parser_AST.range = uu____9408; FStar_Parser_AST.level = uu____9409}), doc1))::[]) when (not (for_free)) -> begin
(

let uu____9436 = (

let uu____9437 = (FStar_ToSyntax_Env.qualify env2 name)
in (

let uu____9438 = (

let uu____9439 = (desugar_term env2 def)
in (FStar_Syntax_Subst.close binders1 uu____9439))
in (

let uu____9440 = (

let uu____9441 = (desugar_typ env2 cps_type)
in (FStar_Syntax_Subst.close binders1 uu____9441))
in {FStar_Syntax_Syntax.action_name = uu____9437; FStar_Syntax_Syntax.action_unqualified_name = name; FStar_Syntax_Syntax.action_univs = []; FStar_Syntax_Syntax.action_defn = uu____9438; FStar_Syntax_Syntax.action_typ = uu____9440})))
in ((uu____9436), (doc1)))
end
| FStar_Parser_AST.Tycon (uu____9443, ((FStar_Parser_AST.TyconAbbrev (name, uu____9445, uu____9446, defn), doc1))::[]) when for_free -> begin
(

let uu____9465 = (

let uu____9466 = (FStar_ToSyntax_Env.qualify env2 name)
in (

let uu____9467 = (

let uu____9468 = (desugar_term env2 defn)
in (FStar_Syntax_Subst.close binders1 uu____9468))
in {FStar_Syntax_Syntax.action_name = uu____9466; FStar_Syntax_Syntax.action_unqualified_name = name; FStar_Syntax_Syntax.action_univs = []; FStar_Syntax_Syntax.action_defn = uu____9467; FStar_Syntax_Syntax.action_typ = FStar_Syntax_Syntax.tun}))
in ((uu____9465), (doc1)))
end
| uu____9470 -> begin
...
let uu____9490 = (FStar_ToSyntax_Env.fail_or env2 (FStar_ToSyntax_Env.try_lookup_definition env2) l)
in (FStar_All.pipe_left (FStar_Syntax_Subst.close binders1) uu____9490))
in (([]), (uu____9489)))))
in (
...
let uu____9502 = (FStar_Syntax_Syntax.mk FStar_Syntax_Syntax.Tm_unknown None FStar_Range.dummyRange)
in (([]), (uu____9502)))
in (
...
let uu____9534 = (lookup "return_wp")
in (

let uu____9535 = (lookup "bind_wp")
in (

let uu____9536 = (lookup "if_then_else")
in (

let uu____9537 = (lookup "ite_wp")
in (

let uu____9538 = (lookup "stronger")
in (

let uu____9539 = (lookup "close_wp")
in (

let uu____9540 = (lookup "assert_p")
in (

let uu____9541 = (lookup "assume_p")
in (

let uu____9542 = (lookup "null_wp")
in (

let uu____9543 = (lookup "trivial")
in (

let uu____9544 = (match (rr) with
| true -> begin
(

let uu____9545 = (lookup "repr")
in (FStar_All.pipe_left Prims.snd uu____9545))
end
| uu____9553 -> begin
FStar_Syntax_Syntax.tun
end)
...
in {FStar_Syntax_Syntax.qualifiers = qualifiers; FStar_Syntax_Syntax.cattributes = []; FStar_Syntax_Syntax.mname = mname; FStar_Syntax_Syntax.univs = []; FStar_Syntax_Syntax.binders = binders1; FStar_Syntax_Syntax.signature = eff_t1; FStar_Syntax_Syntax.ret_wp = uu____9534; FStar_Syntax_Syntax.bind_wp = uu____9535; FStar_Syntax_Syntax.if_then_else = uu____9536; FStar_Syntax_Syntax.ite_wp = uu____9537; FStar_Syntax_Syntax.stronger = uu____9538; FStar_Syntax_Syntax.close_wp = uu____9539; FStar_Syntax_Syntax.assert_p = uu____9540; FStar_Syntax_Syntax.assume_p = uu____9541; FStar_Syntax_Syntax.null_wp = uu____9542; FStar_Syntax_Syntax.trivial = uu____9543; FStar_Syntax_Syntax.repr = uu____9544; FStar_Syntax_Syntax.return_repr = uu____9554; FStar_Syntax_Syntax.bind_repr = uu____9556; FStar_Syntax_Syntax.actions = actions1})))))))))))))
in FStar_Syntax_Syntax.Sig_new_effect (uu____9533))
in {FStar_Syntax_Syntax.sigel = uu____9532; FStar_Syntax_Syntax.sigrng = d.FStar_Parser_AST.drange})))
end)
in (
...
let env5 = (FStar_All.pipe_right actions_docs (FStar_List.fold_left (fun env5 uu____9569 -> (match (uu____9569) with
| (a, doc1) -> begin
(

let env6 = (

let uu____9578 = (FStar_Syntax_Util.action_as_lb mname a)
in (FStar_ToSyntax_Env.push_sigelt env5 uu____9578))
...
let uu____9580 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Parser_AST.Reflectable))
...
let uu____9604 = (desugar_binders env1 eff_binders)
...
let uu____9625 = (head_and_args defn)
...
let uu____9657 = (FStar_Parser_AST.term_to_string head1)
in (Prims.strcat uu____9657 " not found"))
in (Prims.strcat "Effect " uu____9656))
in ((uu____9655), (d.FStar_Parser_AST.drange)))
in FStar_Errors.Error (uu____9652))
in (Prims.raise uu____9651))
end)
in (
...
let uu____9659 = (match ((FStar_List.rev args)) with
| ((last_arg, uu____9675))::args_rev -> begin
(

let uu____9682 = (

let uu____9683 = (unparen last_arg)
in uu____9683.FStar_Parser_AST.tm)
...
let uu____9730 = (desugar_args env2 args1)
in (

let uu____9735 = (desugar_attributes env2 cattributes)
in ((lid), (ed), (uu____9730), (uu____9735))))
end))))
end))
...
let sub1 = (fun uu____9769 -> (match (uu____9769) with
| (uu____9776, x) -> begin
(

let uu____9780 = (FStar_Syntax_Subst.open_term ed.FStar_Syntax_Syntax.binders x)
...
let uu____9801 = (FStar_Syntax_Subst.subst s x1)
in (FStar_Syntax_Subst.close binders1 uu____9801))
in (([]), (uu____9800))));
)
end))
end))
...
let uu____9807 = (trans_qual1 (Some (mname)))
in (FStar_List.map uu____9807 quals))
in (

let uu____9810 = (

let uu____9811 = (sub1 (([]), (ed.FStar_Syntax_Syntax.signature)))
in (Prims.snd uu____9811))
in (

let uu____9817 = (sub1 ed.FStar_Syntax_Syntax.ret_wp)
in (

let uu____9818 = (sub1 ed.FStar_Syntax_Syntax.bind_wp)
in (

let uu____9819 = (sub1 ed.FStar_Syntax_Syntax.if_then_else)
in (

let uu____9820 = (sub1 ed.FStar_Syntax_Syntax.ite_wp)
in (

let uu____9821 = (sub1 ed.FStar_Syntax_Syntax.stronger)
in (

let uu____9822 = (sub1 ed.FStar_Syntax_Syntax.close_wp)
in (

let uu____9823 = (sub1 ed.FStar_Syntax_Syntax.assert_p)
in (

let uu____9824 = (sub1 ed.FStar_Syntax_Syntax.assume_p)
in (

let uu____9825 = (sub1 ed.FStar_Syntax_Syntax.null_wp)
in (

let uu____9826 = (sub1 ed.FStar_Syntax_Syntax.trivial)
in (

let uu____9827 = (

let uu____9828 = (sub1 (([]), (ed.FStar_Syntax_Syntax.repr)))
in (Prims.snd uu____9828))
in (

let uu____9834 = (sub1 ed.FStar_Syntax_Syntax.return_repr)
in (

let uu____9835 = (sub1 ed.FStar_Syntax_Syntax.bind_repr)
in (

let uu____9836 = (FStar_List.map (fun action -> (

let uu____9839 = (FStar_ToSyntax_Env.qualify env2 action.FStar_Syntax_Syntax.action_unqualified_name)
in (

let uu____9840 = (

let uu____9841 = (sub1 (([]), (action.FStar_Syntax_Syntax.action_defn)))
in (Prims.snd uu____9841))
in (

let uu____9847 = (

let uu____9848 = (sub1 (([]), (action.FStar_Syntax_Syntax.action_typ)))
in (Prims.snd uu____9848))
in {FStar_Syntax_Syntax.action_name = uu____9839; FStar_Syntax_Syntax.action_unqualified_name = action.FStar_Syntax_Syntax.action_unqualified_name; FStar_Syntax_Syntax.action_univs = action.FStar_Syntax_Syntax.action_univs; FStar_Syntax_Syntax.action_defn = uu____9840; FStar_Syntax_Syntax.action_typ = uu____9847})))) ed.FStar_Syntax_Syntax.actions)
in {FStar_Syntax_Syntax.qualifiers = uu____9805; FStar_Syntax_Syntax.cattributes = cattributes; FStar_Syntax_Syntax.mname = mname; FStar_Syntax_Syntax.univs = []; FStar_Syntax_Syntax.binders = binders1; FStar_Syntax_Syntax.signature = uu____9810; FStar_Syntax_Syntax.ret_wp = uu____9817; FStar_Syntax_Syntax.bind_wp = uu____9818; FStar_Syntax_Syntax.if_then_else = uu____9819; FStar_Syntax_Syntax.ite_wp = uu____9820; FStar_Syntax_Syntax.stronger = uu____9821; FStar_Syntax_Syntax.close_wp = uu____9822; FStar_Syntax_Syntax.assert_p = uu____9823; FStar_Syntax_Syntax.assume_p = uu____9824; FStar_Syntax_Syntax.null_wp = uu____9825; FStar_Syntax_Syntax.trivial = uu____9826; FStar_Syntax_Syntax.repr = uu____9827; FStar_Syntax_Syntax.return_repr = uu____9834; FStar_Syntax_Syntax.bind_repr = uu____9835; FStar_Syntax_Syntax.actions = uu____9836}))))))))))))))))
in (
...
let uu____9861 = (FStar_Syntax_Util.arrow_formals ed1.FStar_Syntax_Syntax.signature)
in (Prims.fst uu____9861))
in (FStar_List.length uu____9857))
in (uu____9856 = (Prims.parse_int "1")))
...
let uu____9890 = (FStar_Syntax_Util.action_as_lb mname a)
in (FStar_ToSyntax_Env.push_sigelt env5 uu____9890))
in (FStar_ToSyntax_Env.push_doc env6 a.FStar_Syntax_Syntax.action_name doc1)))) env4))
in (

let env6 = (

let uu____9892 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Parser_AST.Reflectable))
...
| FStar_Parser_AST.Fsdoc (uu____9918) -> begin
((env), ([]))
end
...
let uu____9930 = (FStar_ToSyntax_Env.push_module_abbrev env x l)
in ((uu____9930), ([])))
end
...
let tcs1 = (FStar_List.map (fun uu____9951 -> (match (uu____9951) with
| (x, uu____9956) -> begin
x
end)) tcs)
in (

let uu____9959 = (FStar_List.map (trans_qual1 None) quals)
in (desugar_tycon env d uu____9959 tcs1))))
end
...
let uu____10003 = (is_app_pattern p)
in (not (uu____10003)))
end
| uu____10004 -> begin
false
...
let uu____10016 = (FStar_All.pipe_left FStar_Syntax_Subst.compress ds_lets)
in uu____10016.FStar_Syntax_Syntax.n)
in (match (uu____10015) with
| FStar_Syntax_Syntax.Tm_let (lbs, uu____10022) -> begin
(
...
| (uu____10042)::uu____10043 -> begin
(FStar_List.map (trans_qual1 None) quals)
end
| uu____10045 -> begin
(FStar_All.pipe_right (Prims.snd lbs) (FStar_List.collect (fun uu___206_10049 -> (match (uu___206_10049) with
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (uu____10051); FStar_Syntax_Syntax.lbunivs = uu____10052; FStar_Syntax_Syntax.lbtyp = uu____10053; FStar_Syntax_Syntax.lbeff = uu____10054; FStar_Syntax_Syntax.lbdef = uu____10055} -> begin
[]
end
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (fv); FStar_Syntax_Syntax.lbunivs = uu____10062; FStar_Syntax_Syntax.lbtyp = uu____10063; FStar_Syntax_Syntax.lbeff = uu____10064; FStar_Syntax_Syntax.lbdef = uu____10065} -> begin
...
let uu____10077 = (FStar_All.pipe_right lets (FStar_Util.for_some (fun uu____10083 -> (match (uu____10083) with
| (uu____10086, t) -> begin
...
let uu____10094 = (FStar_All.pipe_right quals2 (FStar_List.contains FStar_Syntax_Syntax.Abstract))
in (match (uu____10094) with
| true -> begin
(

let uu____10099 = (FStar_All.pipe_right (Prims.snd lbs) (FStar_List.map (fun lb -> (

let fv = (FStar_Util.right lb.FStar_Syntax_Syntax.lbname)
in (

let uu___221_10106 = lb
in {FStar_Syntax_Syntax.lbname = FStar_Util.Inr ((

let uu___222_10107 = fv
in {FStar_Syntax_Syntax.fv_name = uu___222_10107.FStar_Syntax_Syntax.fv_name; FStar_Syntax_Syntax.fv_delta = FStar_Syntax_Syntax.Delta_abstract (fv.FStar_Syntax_Syntax.fv_delta); FStar_Syntax_Syntax.fv_qual = uu___222_10107.FStar_Syntax_Syntax.fv_qual})); FStar_Syntax_Syntax.lbunivs = uu___221_10106.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = uu___221_10106.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = uu___221_10106.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = uu___221_10106.FStar_Syntax_Syntax.lbdef})))))
in (((Prims.fst lbs)), (uu____10099)))
end
| uu____10113 -> begin
lbs
end))
...
let uu____10139 = (match (lets) with
| ((pat, body))::[] -> begin
((pat), (body))
end
| uu____10150 -> begin
...
let uu___223_10166 = pat1
in {FStar_Parser_AST.pat = FStar_Parser_AST.PatAscribed (((var_pat), (ty))); FStar_Parser_AST.prange = uu___223_10166.FStar_Parser_AST.prange})
end
| uu____10167 -> begin
var_pat
end))
...
let uu___224_10171 = d
in {FStar_Parser_AST.d = FStar_Parser_AST.TopLevelLet (((isrec), ((((fresh_pat), (body)))::[]))); FStar_Parser_AST.drange = uu___224_10171.FStar_Parser_AST.drange; FStar_Parser_AST.doc = uu___224_10171.FStar_Parser_AST.doc; FStar_Parser_AST.quals = (FStar_Parser_AST.Private)::d.FStar_Parser_AST.quals; FStar_Parser_AST.attrs = uu___224_10171.FStar_Parser_AST.attrs}))
in (

let build_projection = (fun uu____10190 id -> (match (uu____10190) with
| (env1, ses) -> begin
(
...
let uu____10204 = (FStar_Ident.lid_of_ids ((fresh_toplevel_name)::[]))
in FStar_Parser_AST.Var (uu____10204))
in (FStar_Parser_AST.mk_term uu____10203 FStar_Range.dummyRange FStar_Parser_AST.Expr))
in (
...
let uu____10232 = (desugar_decl env1 id_decl)
...
let uu____10243 = (gather_pattern_bound_vars pat)
in (FStar_All.pipe_right uu____10243 FStar_Util.set_elements))
...
let uu____10265 = (close_fun env t)
in (desugar_term env uu____10265))
in (
...
let uu____10279 = (FStar_List.map (trans_qual1 None) quals1)
in ((lid), ([]), (t1), (uu____10279)))
in FStar_Syntax_Syntax.Sig_declare_typ (uu____10273))
in {FStar_Syntax_Syntax.sigel = uu____10272; FStar_Syntax_Syntax.sigrng = d.FStar_Parser_AST.drange})
in (
...
let uu____10288 = (FStar_ToSyntax_Env.fail_or env (FStar_ToSyntax_Env.try_lookup_lid env) FStar_Syntax_Const.exn_lid)
...
let uu____10328 = (FStar_Syntax_Syntax.null_binder t)
in (uu____10328)::[])
in (
...
let uu____10333 = (FStar_ToSyntax_Env.fail_or env (FStar_ToSyntax_Env.try_lookup_lid env) FStar_Syntax_Const.exn_lid)
in (Prims.fst uu____10333))
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_Total uu____10332))
in (FStar_Syntax_Util.arrow uu____10324 uu____10329)))
in (
...
let uu____10380 = (FStar_ToSyntax_Env.try_lookup_effect_name env l1)
...
let uu____10388 = (FStar_Syntax_Print.lid_to_string l1)
in (Prims.strcat uu____10388 " not found"))
in (Prims.strcat "Effect name " uu____10387))
in ((uu____10386), (d.FStar_Parser_AST.drange)))
in FStar_Errors.Error (uu____10383))
in (Prims.raise uu____10382))
end
| Some (l2) -> begin
l2
end)))
...
let uu____10392 = (match (l.FStar_Parser_AST.lift_op) with
...
let uu____10423 = (desugar_term env t)
in (([]), (uu____10423)))
in Some (uu____10419))
in ((uu____10414), (None)))
end
...
let uu____10450 = (desugar_term env wp)
in (([]), (uu____10450)))
in Some (uu____10446))
in (
...
let uu____10464 = (desugar_term env t)
in (([]), (uu____10464)))
in Some (uu____10460))
in ((uu____10441), (uu____10455))))
end
...
let uu____10487 = (desugar_term env t)
in (([]), (uu____10487)))
in Some (uu____10483))
in ((None), (uu____10478)))
end)
...
let desugar_decls : FStar_ToSyntax_Env.env  ->  FStar_Parser_AST.decl Prims.list  ->  (FStar_ToSyntax_Env.env * FStar_Syntax_Syntax.sigelts) = (fun env decls -> (FStar_List.fold_left (fun uu____10538 d -> (match (uu____10538) with
| (env1, sigelts) -> begin
(

let uu____10550 = (desugar_decl env1 d)
...
| (None, uu____10592) -> begin
env
end
| (Some ({FStar_Syntax_Syntax.name = prev_lid; FStar_Syntax_Syntax.declarations = uu____10595; FStar_Syntax_Syntax.exports = uu____10596; FStar_Syntax_Syntax.is_interface = uu____10597}), FStar_Parser_AST.Module (current_lid, uu____10599)) when ((FStar_Ident.lid_equals prev_lid current_lid) && (FStar_Options.interactive ())) -> begin
env
end
| (Some (prev_mod), uu____10604) -> begin
(FStar_ToSyntax_Env.finish_module_or_interface env prev_mod)
end)
in (

let uu____10606 = (match (m) with
| FStar_Parser_AST.Interface (mname, decls, admitted) -> begin
(

let uu____10626 = (FStar_ToSyntax_Env.prepare_module_or_interface true admitted env1 mname)
in ((uu____10626), (mname), (decls), (true)))
end
| FStar_Parser_AST.Module (mname, decls) -> begin
(

let uu____10636 = (FStar_ToSyntax_Env.prepare_module_or_interface false false env1 mname)
in ((uu____10636), (mname), (decls), (false)))
end)
in (match (uu____10606) with
| ((env2, pop_when_done), mname, decls, intf) -> begin
(

let uu____10654 = (desugar_decls env2 decls)
...
let uu____10679 = ((FStar_Options.interactive ()) && (

let uu____10680 = (

let uu____10681 = (

let uu____10682 = (FStar_Options.file_list ())
in (FStar_List.hd uu____10682))
in (FStar_Util.get_file_extension uu____10681))
in (uu____10680 = "fsti")))
...
| FStar_Parser_AST.Interface (mname, uu____10690, uu____10691) -> begin
...
let uu____10695 = (desugar_modul_common curmod env m1)
...
let uu____10705 = (FStar_ToSyntax_Env.pop ())
in ())
end
| uu____10706 -> begin
()
...
let uu____10717 = (desugar_modul_common None env m)
...
let uu____10728 = (FStar_Options.dump_module modul.FStar_Syntax_Syntax.name.FStar_Ident.str)
in (match (uu____10728) with
| true -> begin
(

let uu____10729 = (FStar_Syntax_Print.modul_to_string modul)
...
let uu____10731 = (match (pop_when_done) with
| true -> begin
...
let uu____10742 = (FStar_List.fold_left (fun uu____10749 m -> (match (uu____10749) with
| (env1, mods) -> begin
(

let uu____10761 = (desugar_modul env1 m)
...
let uu____10785 = (FStar_ToSyntax_Env.prepare_module_or_interface false false en m.FStar_Syntax_Syntax.name)
...
let uu___225_10791 = en1
in {FStar_ToSyntax_Env.curmodule = Some (m.FStar_Syntax_Syntax.name); FStar_ToSyntax_Env.curmonad = uu___225_10791.FStar_ToSyntax_Env.curmonad; FStar_ToSyntax_Env.modules = uu___225_10791.FStar_ToSyntax_Env.modules; FStar_ToSyntax_Env.scope_mods = uu___225_10791.FStar_ToSyntax_Env.scope_mods; FStar_ToSyntax_Env.exported_ids = uu___225_10791.FStar_ToSyntax_Env.exported_ids; FStar_ToSyntax_Env.trans_exported_ids = uu___225_10791.FStar_ToSyntax_Env.trans_exported_ids; FStar_ToSyntax_Env.includes = uu___225_10791.FStar_ToSyntax_Env.includes; FStar_ToSyntax_Env.sigaccum = uu___225_10791.FStar_ToSyntax_Env.sigaccum; FStar_ToSyntax_Env.sigmap = uu___225_10791.FStar_ToSyntax_Env.sigmap; FStar_ToSyntax_Env.iface = uu___225_10791.FStar_ToSyntax_Env.iface; FStar_ToSyntax_Env.admitted_iface = uu___225_10791.FStar_ToSyntax_Env.admitted_iface; FStar_ToSyntax_Env.expect_typ = uu___225_10791.FStar_ToSyntax_Env.expect_typ; FStar_ToSyntax_Env.docs = uu___225_10791.FStar_ToSyntax_Env.docs}) m.FStar_Syntax_Syntax.exports)
in (
