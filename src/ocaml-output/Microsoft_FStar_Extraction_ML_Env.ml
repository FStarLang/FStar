...
type env =
{tcenv : Microsoft_FStar_Tc_Env.env; gamma : binding Prims.list; tydefs : (Microsoft_FStar_Extraction_ML_Syntax.mlsymbol Prims.list * Microsoft_FStar_Extraction_ML_Syntax.mltydecl) Prims.list; currentModule : Microsoft_FStar_Extraction_ML_Syntax.mlpath}

let is_Mkenv = (fun _ -> (All.failwith "Not yet implemented:is_Mkenv"))

let debug = (fun g f -> (match ((((ST.read Microsoft_FStar_Options.debug) <> []) && ((let _122_65 = (ST.read Microsoft_FStar_Options.debug)
in (Microsoft_FStar_List.contains "Prims" _122_65)) || (g.currentModule <> ([], "Prims"))))) with
| true -> begin
(f ())
end
| false -> begin
...
let mkFvvar = (fun l t -> (let _122_70 = (Microsoft_FStar_Range.mk_range "" 0 0)
in {Microsoft_FStar_Absyn_Syntax.v = l; Microsoft_FStar_Absyn_Syntax.sort = t; Microsoft_FStar_Absyn_Syntax.p = _122_70}))
...
| (x, n) -> begin
(match ((Microsoft_FStar_Util.starts_with x "\'")) with
| true -> begin
(x, n)
end
| false -> begin
...
(match ((Microsoft_FStar_Util.starts_with x "\'")) with
| true -> begin
(let _122_75 = (Microsoft_FStar_Util.substring_from x 1)
in (_122_75, n))
end
| false -> begin
(x, n)
...
| [] -> begin
(All.failwith (Prims.strcat "extraction: unbound type var " b.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname.Microsoft_FStar_Absyn_Syntax.idText))
end))
...
(Microsoft_FStar_Util.find_map env.tydefs (fun _56_75 -> (match (_56_75) with
| (m, tds) -> begin
(match ((module_name = m)) with
| true -> begin
(Microsoft_FStar_Util.find_map tds (fun td -> (let _56_82 = td
...
let lookup_fv_by_lid = (fun g fv -> (let x = (Microsoft_FStar_Util.find_map g.gamma (fun _56_1 -> (match (_56_1) with
...
in (match (x) with
| None -> begin
(let _122_105 = (let _122_104 = (Microsoft_FStar_Absyn_Print.sli fv)
in (Microsoft_FStar_Util.format1 "free Variable %s not found\n" _122_104))
in (All.failwith _122_105))
end
| Some (y) -> begin
y
end)))

let lookup_fv = (fun g fv -> (let x = (Microsoft_FStar_Util.find_map g.gamma (fun _56_2 -> (match (_56_2) with
...
in (match (x) with
| None -> begin
(let _122_113 = (let _122_112 = (Microsoft_FStar_Range.string_of_range fv.Microsoft_FStar_Absyn_Syntax.p)
in (let _122_111 = (Microsoft_FStar_Absyn_Print.sli fv.Microsoft_FStar_Absyn_Syntax.v)
in (Microsoft_FStar_Util.format2 "(%s) free Variable %s not found\n" _122_112 _122_111)))
in (All.failwith _122_113))
end
| Some (y) -> begin
y
end)))

let lookup_bv = (fun g bv -> (let x = (Microsoft_FStar_Util.find_map g.gamma (fun _56_3 -> (match (_56_3) with
...
in (match (x) with
| None -> begin
(let _122_121 = (let _122_120 = (Microsoft_FStar_Range.string_of_range bv.Microsoft_FStar_Absyn_Syntax.p)
in (let _122_119 = (Microsoft_FStar_Absyn_Print.strBvd bv.Microsoft_FStar_Absyn_Syntax.v)
in (Microsoft_FStar_Util.format2 "(%s) bound Variable %s not found\n" _122_120 _122_119)))
in (All.failwith _122_121))
end
| Some (y) -> begin
y
end)))
...
(let _122_128 = (lookup g (Microsoft_FStar_Util.Inl (x)))
in (_122_128, None))
end
| Microsoft_FStar_Absyn_Syntax.Exp_fvar (x, b) -> begin
(let _122_129 = (lookup g (Microsoft_FStar_Util.Inr (x)))
in (_122_129, b))
end
| _56_142 -> begin
...
| h::tla -> begin
((Microsoft_FStar_List.contains h lb) && (subsetMlidents tla lb))
end
| [] -> begin
true
end))
...
| Microsoft_FStar_Util.Inl (x) -> begin
(let _122_187 = (extend_bv g (Microsoft_FStar_Absyn_Util.bvd_to_bvar_s x t) t_x add_unit false)
in (_122_187, (Microsoft_FStar_Extraction_ML_Syntax.as_mlident x)))
end
| Microsoft_FStar_Util.Inr (f) -> begin
(let _56_219 = (Microsoft_FStar_Extraction_ML_Syntax.mlpath_of_lident f)
in (match (_56_219) with
| (p, y) -> begin
(let _122_189 = (let _122_188 = (Microsoft_FStar_Absyn_Util.fvvar_of_lid f t)
in (extend_fv' g _122_188 (p, y) t_x add_unit))
in (_122_189, (y, 0)))
end))
end))

let extend_tydef = (fun g td -> (let m = (Microsoft_FStar_List.append (Prims.fst g.currentModule) (((Prims.snd g.currentModule))::[]))
in (let _56_223 = g
...
in (let _122_196 = (extend_lb env (Microsoft_FStar_Util.Inr (Microsoft_FStar_Absyn_Const.failwith_lid)) Microsoft_FStar_Absyn_Syntax.tun failwith_ty false)
in (All.pipe_right _122_196 Prims.fst))))))
