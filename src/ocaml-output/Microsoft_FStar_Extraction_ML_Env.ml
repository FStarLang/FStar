...
let debug = (fun ( g ) ( f ) -> (match ((((Support.ST.read Microsoft_FStar_Options.debug) <> []) && ((let _70_27125 = (Support.ST.read Microsoft_FStar_Options.debug)
in (Support.List.contains "Prims" _70_27125)) || (g.currentModule <> ([], "Prims"))))) with
| true -> begin
(f ())
end
| false -> begin
...
let mkFvvar = (fun ( l ) ( t ) -> (let _70_27130 = (Support.Microsoft.FStar.Range.mk_range "" 0 0)
in {Microsoft_FStar_Absyn_Syntax.v = l; Microsoft_FStar_Absyn_Syntax.sort = t; Microsoft_FStar_Absyn_Syntax.p = _70_27130}))
...
(match ((Support.Microsoft.FStar.Util.starts_with x "\'")) with
| true -> begin
(let _70_27135 = (Support.Microsoft.FStar.Util.substring_from x 1)
in (_70_27135, n))
end
| false -> begin
(x, n)
...
in (match (x) with
| None -> begin
(let _70_27165 = (let _70_27164 = (Microsoft_FStar_Absyn_Print.sli fv)
in (Support.Microsoft.FStar.Util.format1 "free Variable %s not found\n" _70_27164))
in (Support.All.failwith _70_27165))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _70_27173 = (let _70_27172 = (Support.Microsoft.FStar.Range.string_of_range fv.Microsoft_FStar_Absyn_Syntax.p)
in (let _70_27171 = (Microsoft_FStar_Absyn_Print.sli fv.Microsoft_FStar_Absyn_Syntax.v)
in (Support.Microsoft.FStar.Util.format2 "(%s) free Variable %s not found\n" _70_27172 _70_27171)))
in (Support.All.failwith _70_27173))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _70_27181 = (let _70_27180 = (Support.Microsoft.FStar.Range.string_of_range bv.Microsoft_FStar_Absyn_Syntax.p)
in (let _70_27179 = (Microsoft_FStar_Absyn_Print.strBvd bv.Microsoft_FStar_Absyn_Syntax.v)
in (Support.Microsoft.FStar.Util.format2 "(%s) bound Variable %s not found\n" _70_27180 _70_27179)))
in (Support.All.failwith _70_27181))
end
| Some (y) -> begin
y
end)))
...
(let _70_27188 = (lookup g (Support.Microsoft.FStar.Util.Inl (x)))
in (_70_27188, None))
end
| Microsoft_FStar_Absyn_Syntax.Exp_fvar ((x, b)) -> begin
(let _70_27189 = (lookup g (Support.Microsoft.FStar.Util.Inr (x)))
in (_70_27189, b))
end
| _56_142 -> begin
...
(let _70_27207 = (mltyFvars t1)
in (let _70_27206 = (mltyFvars t2)
in (Support.List.append _70_27207 _70_27206)))
end
...
(let _70_27209 = (mltyFvars t1)
in (let _70_27208 = (mltyFvars t2)
in (Support.List.append _70_27209 _70_27208)))
end
...
let tySchemeIsClosed = (fun ( tys ) -> (let _70_27216 = (mltyFvars (Support.Prims.snd tys))
in (subsetMlidents _70_27216 (Support.Prims.fst tys))))
...
| Support.Microsoft.FStar.Util.Inl (x) -> begin
(let _70_27245 = (extend_bv g (Microsoft_FStar_Absyn_Util.bvd_to_bvar_s x t) t_x add_unit)
in (_70_27245, (Microsoft_FStar_Extraction_ML_Syntax.as_mlident x)))
end
...
| (p, y) -> begin
(let _70_27247 = (let _70_27246 = (Microsoft_FStar_Absyn_Util.fvvar_of_lid f t)
in (extend_fv' g _70_27246 (p, y) t_x add_unit))
in (_70_27247, (y, 0)))
end))
end))
...
in (let _70_27254 = (extend_lb env (Support.Microsoft.FStar.Util.Inr (Microsoft_FStar_Absyn_Const.failwith_lid)) Microsoft_FStar_Absyn_Syntax.tun failwith_ty false)
in (Support.All.pipe_right _70_27254 Support.Prims.fst))))))
