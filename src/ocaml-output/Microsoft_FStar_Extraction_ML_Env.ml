...
let debug = (fun ( g ) ( f ) -> (match ((((Support.ST.read Microsoft_FStar_Options.debug) <> []) && ((let _122_65 = (Support.ST.read Microsoft_FStar_Options.debug)
in (Support.List.contains "Prims" _122_65)) || (g.currentModule <> ([], "Prims"))))) with
| true -> begin
(f ())
end
| false -> begin
...
let mkFvvar = (fun ( l ) ( t ) -> (let _122_70 = (Support.Microsoft.FStar.Range.mk_range "" 0 0)
...
(match ((Support.Microsoft.FStar.Util.starts_with x "\'")) with
| true -> begin
(let _122_75 = (Support.Microsoft.FStar.Util.substring_from x 1)
in (_122_75, n))
end
| false -> begin
(x, n)
...
| [] -> begin
(Support.All.failwith (Support.Prims.strcat "extraction: unbound type var " b.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname.Microsoft_FStar_Absyn_Syntax.idText))
end))
...
in (Support.All.failwith _122_105))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _122_113 = (let _122_112 = (Support.Microsoft.FStar.Range.string_of_range fv.Microsoft_FStar_Absyn_Syntax.p)
...
in (Support.All.failwith _122_113))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _122_121 = (let _122_120 = (Support.Microsoft.FStar.Range.string_of_range bv.Microsoft_FStar_Absyn_Syntax.p)
...
in (Support.All.failwith _122_121))
end
| Some (y) -> begin
y
end)))
...
(let _122_128 = (lookup g (Support.Microsoft.FStar.Util.Inl (x)))
in (_122_128, None))
end
| Microsoft_FStar_Absyn_Syntax.Exp_fvar ((x, b)) -> begin
(let _122_129 = (lookup g (Support.Microsoft.FStar.Util.Inr (x)))
in (_122_129, b))
end
| _56_142 -> begin
...
in (Support.List.append _122_149 _122_148)))
end
...
in (Support.List.append _122_151 _122_150)))
end
...
let tySchemeIsClosed = (fun ( tys ) -> (let _122_158 = (mltyFvars (Support.Prims.snd tys))
in (subsetMlidents _122_158 (Support.Prims.fst tys))))
...
in (let _122_196 = (extend_lb env (Support.Microsoft.FStar.Util.Inr (Microsoft_FStar_Absyn_Const.failwith_lid)) Microsoft_FStar_Absyn_Syntax.tun failwith_ty false)
in (Support.All.pipe_right _122_196 Support.Prims.fst))))))
