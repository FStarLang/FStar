...
let uu___173_425 = bv.FStar_Syntax_Syntax.ppname
in {FStar_Ident.idText = uu___173_425.FStar_Ident.idText; FStar_Ident.idRange = r})
in (

let uu___174_426 = bv
in {FStar_Syntax_Syntax.ppname = id; FStar_Syntax_Syntax.index = uu___174_426.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = uu___174_426.FStar_Syntax_Syntax.sort})))
...
let option_of_cont = (fun k_ignore uu___143_571 -> (match (uu___143_571) with
| Cont_ok (a) -> begin
Some (a)
end
| Cont_fail -> begin
...
let string_of_exported_id_kind : exported_id_kind  ->  Prims.string = (fun uu___144_657 -> (match (uu___144_657) with
| Exported_id_field -> begin
"field"
end
| Exported_id_term_type -> begin
...
let rec aux = (fun uu___145_706 -> (match (uu___145_706) with
| [] -> begin
...
let is_exported_id_field : exported_id_kind  ->  Prims.bool = (fun uu___146_764 -> (match (uu___146_764) with
| Exported_id_field -> begin
true
end
| uu____765 -> begin
...
let check_local_binding_id = (fun uu___147_854 -> (match (uu___147_854) with
| (id', uu____856, uu____857) -> begin
(id'.FStar_Ident.idText = id.FStar_Ident.idText)
end))
in (

let check_rec_binding_id = (fun uu___148_861 -> (match (uu___148_861) with
...
let proc = (fun uu___149_872 -> (match (uu___149_872) with
...
let rec aux = (fun uu___150_889 -> (match (uu___150_889) with
| (a)::q -> begin
(
...
let rec aux = (fun uu___151_1142 -> (match (uu___151_1142) with
| [] -> begin
(
...
let cont_of_option = (fun k_none uu___152_1281 -> (match (uu___152_1281) with
| Some (v) -> begin
Cont_ok (v)
end
| None -> begin
...
let fv_qual_of_se : FStar_Syntax_Syntax.sigelt  ->  FStar_Syntax_Syntax.fv_qual Prims.option = (fun uu___154_1390 -> (match (uu___154_1390) with
| FStar_Syntax_Syntax.Sig_datacon (uu____1392, uu____1393, uu____1394, l, uu____1396, quals, uu____1398, uu____1399) -> begin
(

let qopt = (FStar_Util.find_map quals (fun uu___153_1406 -> (match (uu___153_1406) with
...
let k_global_def = (fun source_lid uu___158_1478 -> (match (uu___158_1478) with
...
let uu____1551 = (any_val || (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___155_1553 -> (match (uu___155_1553) with
...
let uu____1558 = ((FStar_Syntax_Util.is_primop_lid lid) || ((ns_of_lid_equals lid FStar_Syntax_Const.prims_lid) && (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___156_1560 -> (match (uu___156_1560) with
...
let uu____1565 = (FStar_Util.find_map quals (fun uu___157_1567 -> (match (uu___157_1567) with
...
let k_global_def = (fun lid uu___159_1897 -> (match (uu___159_1897) with
...
let k_global_def = (fun lid uu___160_1976 -> (match (uu___160_1976) with
...
let k_global_def = (fun lid uu___161_2023 -> (match (uu___161_2023) with
...
let uu___175_2125 = env
in {curmodule = uu___175_2125.curmodule; curmonad = uu___175_2125.curmonad; modules = uu___175_2125.modules; scope_mods = []; exported_ids = empty_exported_id_smap; trans_exported_ids = uu___175_2125.trans_exported_ids; includes = empty_include_smap; sigaccum = uu___175_2125.sigaccum; sigmap = uu___175_2125.sigmap; iface = uu___175_2125.iface; admitted_iface = uu___175_2125.admitted_iface; expect_typ = uu___175_2125.expect_typ})
in (try_lookup_lid env' l)))


let try_lookup_datacon : env  ->  FStar_Ident.lident  ->  FStar_Syntax_Syntax.fv Prims.option = (fun env lid -> (

let k_global_def = (fun lid uu___163_2142 -> (match (uu___163_2142) with
| (FStar_Syntax_Syntax.Sig_declare_typ (uu____2146, uu____2147, uu____2148, quals, uu____2150), uu____2151) -> begin
(

let uu____2154 = (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___162_2156 -> (match (uu___162_2156) with
| FStar_Syntax_Syntax.Assumption -> begin
true
end
| uu____2157 -> begin
...
let k_global_def = (fun lid uu___164_2197 -> (match (uu___164_2197) with
...
let extract_record : env  ->  scope_mod Prims.list FStar_ST.ref  ->  FStar_Syntax_Syntax.sigelt  ->  Prims.unit = (fun e new_globs uu___168_2850 -> (match (uu___168_2850) with
| FStar_Syntax_Syntax.Sig_bundle (sigs, uu____2855, uu____2856, uu____2857) -> begin
(

let is_rec = (FStar_Util.for_some (fun uu___165_2868 -> (match (uu___165_2868) with
...
let find_dc = (fun dc -> (FStar_All.pipe_right sigs (FStar_Util.find_opt (fun uu___166_2879 -> (match (uu___166_2879) with
...
in (FStar_All.pipe_right sigs (FStar_List.iter (fun uu___167_2894 -> (match (uu___167_2894) with
...
in (fun uu___169_3223 -> (match (uu___169_3223) with
| Exported_id_term_type -> begin
term_type_set
end
| Exported_id_field -> begin
...
let filter_scope_mods = (fun uu___170_3243 -> (match (uu___170_3243) with
...
let uu___176_3247 = env
in (

let uu____3248 = (FStar_List.filter filter_scope_mods env.scope_mods)
in {curmodule = uu___176_3247.curmodule; curmonad = uu___176_3247.curmonad; modules = uu___176_3247.modules; scope_mods = uu____3248; exported_ids = empty_exported_id_smap; trans_exported_ids = uu___176_3247.trans_exported_ids; includes = empty_include_smap; sigaccum = uu___176_3247.sigaccum; sigmap = uu___176_3247.sigmap; iface = uu___176_3247.iface; admitted_iface = uu___176_3247.admitted_iface; expect_typ = uu___176_3247.expect_typ}))
in (
...
let uu___177_3267 = env
in {curmodule = uu___177_3267.curmodule; curmonad = uu___177_3267.curmonad; modules = uu___177_3267.modules; scope_mods = (scope_mod)::env.scope_mods; exported_ids = uu___177_3267.exported_ids; trans_exported_ids = uu___177_3267.trans_exported_ids; includes = uu___177_3267.includes; sigaccum = uu___177_3267.sigaccum; sigmap = uu___177_3267.sigmap; iface = uu___177_3267.iface; admitted_iface = uu___177_3267.admitted_iface; expect_typ = uu___177_3267.expect_typ}))
...
let uu___178_3386 = env
in {curmodule = uu___178_3386.curmodule; curmonad = uu___178_3386.curmonad; modules = uu___178_3386.modules; scope_mods = uu___178_3386.scope_mods; exported_ids = uu___178_3386.exported_ids; trans_exported_ids = uu___178_3386.trans_exported_ids; includes = uu___178_3386.includes; sigaccum = (s)::env.sigaccum; sigmap = uu___178_3386.sigmap; iface = uu___178_3386.iface; admitted_iface = uu___178_3386.admitted_iface; expect_typ = uu___178_3386.expect_typ});
)
end)))
end))
in (
...
let uu___179_3388 = env
in (

let uu____3389 = (FStar_ST.read globals)
in {curmodule = uu___179_3388.curmodule; curmonad = uu___179_3388.curmonad; modules = uu___179_3388.modules; scope_mods = uu____3389; exported_ids = uu___179_3388.exported_ids; trans_exported_ids = uu___179_3388.trans_exported_ids; includes = uu___179_3388.includes; sigaccum = uu___179_3388.sigaccum; sigmap = uu___179_3388.sigmap; iface = uu___179_3388.iface; admitted_iface = uu___179_3388.admitted_iface; expect_typ = uu___179_3388.expect_typ}))
in (
...
let uu___180_3513 = env
in (

let uu____3514 = (FStar_ST.read globals)
in {curmodule = uu___180_3513.curmodule; curmonad = uu___180_3513.curmonad; modules = uu___180_3513.modules; scope_mods = uu____3514; exported_ids = uu___180_3513.exported_ids; trans_exported_ids = uu___180_3513.trans_exported_ids; includes = uu___180_3513.includes; sigaccum = uu___180_3513.sigaccum; sigmap = uu___180_3513.sigmap; iface = uu___180_3513.iface; admitted_iface = uu___180_3513.admitted_iface; expect_typ = uu___180_3513.expect_typ}))
in env);
)
end)))))))
...
let finish : env  ->  FStar_Syntax_Syntax.modul  ->  env = (fun env modul -> ((FStar_All.pipe_right modul.FStar_Syntax_Syntax.declarations (FStar_List.iter (fun uu___172_3774 -> (match (uu___172_3774) with
...
| true -> begin
(FStar_All.pipe_right ses (FStar_List.iter (fun uu___171_3786 -> (match (uu___171_3786) with
...
let uu___181_3954 = env
in {curmodule = None; curmonad = uu___181_3954.curmonad; modules = (((modul.FStar_Syntax_Syntax.name), (modul)))::env.modules; scope_mods = []; exported_ids = uu___181_3954.exported_ids; trans_exported_ids = uu___181_3954.trans_exported_ids; includes = uu___181_3954.includes; sigaccum = []; sigmap = uu___181_3954.sigmap; iface = uu___181_3954.iface; admitted_iface = uu___181_3954.admitted_iface; expect_typ = uu___181_3954.expect_typ})
end);
)
end);
));
...
let uu___182_3977 = env
in (

let uu____3978 = (FStar_Util.smap_copy (sigmap env))
in {curmodule = uu___182_3977.curmodule; curmonad = uu___182_3977.curmonad; modules = uu___182_3977.modules; scope_mods = uu___182_3977.scope_mods; exported_ids = uu___182_3977.exported_ids; trans_exported_ids = uu___182_3977.trans_exported_ids; includes = uu___182_3977.includes; sigaccum = uu___182_3977.sigaccum; sigmap = uu____3978; iface = uu___182_3977.iface; admitted_iface = uu___182_3977.admitted_iface; expect_typ = uu___182_3977.expect_typ}));
))
...
let uu___183_4144 = env
in (

let uu____4145 = (FStar_List.map (fun lid -> Open_module_or_namespace (((lid), (Open_namespace)))) open_ns)
in {curmodule = Some (mname); curmonad = uu___183_4144.curmonad; modules = uu___183_4144.modules; scope_mods = uu____4145; exported_ids = uu___183_4144.exported_ids; trans_exported_ids = uu___183_4144.trans_exported_ids; includes = uu___183_4144.includes; sigaccum = uu___183_4144.sigaccum; sigmap = env.sigmap; iface = intf; admitted_iface = admitted; expect_typ = uu___183_4144.expect_typ}))
end);
)
end);
)
...
let uu___184_4192 = env
in {curmodule = uu___184_4192.curmodule; curmonad = Some (mname); modules = uu___184_4192.modules; scope_mods = uu___184_4192.scope_mods; exported_ids = uu___184_4192.exported_ids; trans_exported_ids = uu___184_4192.trans_exported_ids; includes = uu___184_4192.includes; sigaccum = uu___184_4192.sigaccum; sigmap = uu___184_4192.sigmap; iface = uu___184_4192.iface; admitted_iface = uu___184_4192.admitted_iface; expect_typ = uu___184_4192.expect_typ})
end))
