...
type open_kind = Open_module | Open_namespace [@@deriving show]
...
let __proj__Mkrecord_or_dc__item__typename : record_or_dc -> FStar_Ident.lident =
  fun projectee ->
    match projectee
    with
    | { typename= __fname__typename
      ; constrname= __fname__constrname
      ; parms= __fname__parms
      ; fields= __fname__fields
      ; is_private_or_abstract= __fname__is_private_or_abstract
      ; is_record= __fname__is_record; _ }
    -> __fname__typename
...
and dsenv_hooks =
  ...
  ; ds_push_module_abbrev_hook:
      env -> FStar_Ident.ident -> FStar_Ident.lident -> Prims.unit }
  [@@deriving show]
...
let __proj__Mkenv__item__curmodule
  ...
  fun projectee ->
    match projectee
    with
    | { curmodule= __fname__curmodule
      ; curmonad= __fname__curmonad
      ; modules= __fname__modules
      ; scope_mods= __fname__scope_mods
      ; exported_ids= __fname__exported_ids
      ; trans_exported_ids= __fname__trans_exported_ids
      ; includes= __fname__includes
      ; sigaccum= __fname__sigaccum
      ; sigmap= __fname__sigmap
      ; iface= __fname__iface
      ; admitted_iface= __fname__admitted_iface
      ; expect_typ= __fname__expect_typ
      ; docs= __fname__docs
      ; remaining_iface_decls= __fname__remaining_iface_decls
      ; syntax_only= __fname__syntax_only
      ; ds_hooks= __fname__ds_hooks; _ }
    -> __fname__curmodule
...
let __proj__Mkdsenv_hooks__item__ds_push_module_abbrev_hook
    : dsenv_hooks -> env -> FStar_Ident.ident -> FStar_Ident.lident
      -> Prims.unit =
  fun projectee ->
    match projectee
    with
...
type 'a withenv =
  env -> ('a, env) FStar_Pervasives_Native.tuple2
  [@@deriving show]

let default_ds_hooks : dsenv_hooks =
  { ds_push_open_hook= (fun uu____1535 uu____1536 -> ())
  ; ds_push_include_hook= (fun uu____1539 uu____1540 -> ())
  ; ds_push_module_abbrev_hook= (fun uu____1544 uu____1545 uu____1546 -> ()) }
...
let transitive_exported_ids
  ...
  fun env lid ->
    ...
    let uu____1680 =
      FStar_Util.smap_try_find env.trans_exported_ids module_name
    in
  ...
  fun env ->
    FStar_List.filter_map
      (fun uu___80_1817 ->
        ...
        | uu____1822 -> FStar_Pervasives_Native.None )
      env.scope_mods
...
let iface_decls
    : env -> FStar_Ident.lident
      -> FStar_Parser_AST.decl Prims.list FStar_Pervasives_Native.option =
  fun env l ->
    let uu____1844 =
      FStar_All.pipe_right env.remaining_iface_decls
        (FStar_List.tryFind (fun uu____1878 ->
             match uu____1878 with m, uu____1886 -> FStar_Ident.lid_equals l m
         ))
    in
  ...
  fun env l ds ->
    let uu____1930 =
      FStar_List.partition
        (fun uu____1960 ->
          match uu____1960 with m, uu____1968 -> FStar_Ident.lid_equals l m )
        env.remaining_iface_decls
    in
    match uu____1930 with uu____1973, rest ->
...
let qualify : env -> FStar_Ident.ident -> FStar_Ident.lident =
  fun env id1 ->
    ...
    | FStar_Pervasives_Native.None ->
        let uu____2026 = current_module env in
        qual uu____2026 id1
    | FStar_Pervasives_Native.Some monad ->
        let uu____2028 =
          let uu____2029 = current_module env in
          qual uu____2029 monad
        in
...
let has_all_in_scope : env -> Prims.bool =
  fun env ->
    FStar_List.existsb
      (fun uu____2122 ->
        match uu____2122 with m, uu____2128 ->
...
let bv_to_name
  ...
  fun bv r -> FStar_Syntax_Syntax.bv_to_name (set_bv_range bv r)
...
let unmangleOpName
  ...
  fun id1 ->
    let t =
      FStar_Util.find_map unmangleMap (fun uu____2224 ->
          match uu____2224 with x, y, dd, dq ->
            if id1.FStar_Ident.idText = x then
              let uu____2247 =
                let uu____2248 =
                  FStar_Ident.lid_of_path ["Prims"; y] id1.FStar_Ident.idRange
                in
                FStar_Syntax_Syntax.fvar uu____2248 dd dq
              in
...
type 'a cont_t = Cont_ok of 'a | Cont_fail | Cont_ignore [@@deriving show]
...
let option_of_cont:
      'a. (Prims.unit -> 'a FStar_Pervasives_Native.option) -> 'a cont_t
      -> 'a FStar_Pervasives_Native.option =
  fun k_ignore uu___81_2357 ->
...
let find_in_record:
      'Auu____2370. FStar_Ident.ident Prims.list -> FStar_Ident.ident
      -> record_or_dc -> (record_or_dc -> 'Auu____2370 cont_t)
      -> 'Auu____2370 cont_t =
  fun ns id1 record cont ->
    let typename' =
      FStar_Ident.lid_of_ids
        (FStar_List.append ns [record.typename.FStar_Ident.ident])
    in
    if FStar_Ident.lid_equals typename' record.typename then
      let fname =
        FStar_Ident.lid_of_ids
          (FStar_List.append record.typename.FStar_Ident.ns [id1])
      in
      let find1 =
        FStar_Util.find_map record.fields (fun uu____2416 ->
            match uu____2416 with f, uu____2424 ->
              if id1.FStar_Ident.idText = f.FStar_Ident.idText then
...
let get_exported_id_set
    : env -> Prims.string
      -> (exported_id_kind -> string_set FStar_ST.ref)
         FStar_Pervasives_Native.option =
  fun e mname -> FStar_Util.smap_try_find e.exported_ids mname
...
let get_trans_exported_id_set
    : env -> Prims.string
      -> (exported_id_kind -> string_set FStar_ST.ref)
         FStar_Pervasives_Native.option =
  fun e mname -> FStar_Util.smap_try_find e.trans_exported_ids mname
...
let find_in_module_with_includes:
      'a. exported_id_kind -> (FStar_Ident.lident -> 'a cont_t) -> 'a cont_t
      -> env -> FStar_Ident.lident -> FStar_Ident.ident -> 'a cont_t =
  fun eikind find_in_module find_in_module_default env ns id1 ->
          ...
          let not_shadowed =
            ...
            | FStar_Pervasives_Native.Some mex ->
                let mexports =
                  ...
                  FStar_ST.op_Bang uu____2558
                in
                FStar_Util.set_mem idstr mexports
          in
          let mincludes =
            let uu____2672 = FStar_Util.smap_try_find env.includes mname in
            match uu____2672 with
            | FStar_Pervasives_Native.None -> []
            | FStar_Pervasives_Native.Some minc -> FStar_ST.op_Bang minc
          in
          let look_into =
            if not_shadowed then
...
let is_exported_id_field : exported_id_kind -> Prims.bool =
  fun uu___84_2757 ->
    match uu___84_2757 with Exported_id_field -> true | uu____2758 -> false


let try_lookup_id'':
      'a. env -> FStar_Ident.ident -> exported_id_kind
      -> (local_binding -> 'a cont_t) -> (rec_binding -> 'a cont_t)
      -> (record_or_dc -> 'a cont_t) -> (FStar_Ident.lident -> 'a cont_t)
      -> ('a cont_t -> FStar_Ident.ident -> 'a cont_t)
      -> 'a FStar_Pervasives_Native.option =
  fun env id1 eikind k_local_binding k_rec_binding k_record find_in_module
      lookup_default_id ->
    let check_local_binding_id uu___85_2860 =
      match uu___85_2860 with id', uu____2862, uu____2863 ->
        id'.FStar_Ident.idText = id1.FStar_Ident.idText
    in
    let check_rec_binding_id uu___86_2867 =
      match uu___86_2867 with id', uu____2869, uu____2870 ->
        id'.FStar_Ident.idText = id1.FStar_Ident.idText
    in
    let curmod_ns =
      let uu____2874 = current_module env in
      FStar_Ident.ids_of_lid uu____2874
    in
    let proc uu___87_2880 =
      ...
      | Open_module_or_namespace (ns, Open_module) ->
          find_in_module_with_includes eikind find_in_module Cont_ignore env ns
            id1
      | Top_level_def id' when id'.FStar_Ident.idText = id1.FStar_Ident.idText ->
...
let found_local_binding:
      'Auu____2924 'Auu____2925. FStar_Range.range
      -> ( 'Auu____2925
         , FStar_Syntax_Syntax.bv
         , 'Auu____2924 )
         FStar_Pervasives_Native.tuple3
      -> ( FStar_Syntax_Syntax.term
         ...
         FStar_Pervasives_Native.tuple2 =
  fun r uu____2943 ->
    match uu____2943 with id', x, mut ->
      let uu____2953 = bv_to_name x r in
      (uu____2953, mut)


let find_in_module:
      'Auu____2959. env -> FStar_Ident.lident
      -> (FStar_Ident.lident
          -> ( FStar_Syntax_Syntax.sigelt
             , Prims.bool )
             FStar_Pervasives_Native.tuple2 -> 'Auu____2959) -> 'Auu____2959
      -> 'Auu____2959 =
  fun env lid k_global_def k_not_found ->
    let uu____2994 =
    ...
    | uu____3056 ->
        try_lookup_id'' env id1 Exported_id_term_type
          (fun r ->
            ...
            Cont_ok uu____3070 )
          (fun uu____3080 -> Cont_fail)
          (fun uu____3086 -> Cont_ignore)
          (fun i ->
            find_in_module env i
              (fun uu____3101 uu____3102 -> Cont_fail)
              Cont_ignore )
          (fun uu____3117 uu____3118 -> Cont_fail)


let lookup_default_id:
      'a. env -> FStar_Ident.ident
      -> (FStar_Ident.lident
          -> ( FStar_Syntax_Syntax.sigelt
             , Prims.bool )
             FStar_Pervasives_Native.tuple2 -> 'a cont_t) -> 'a cont_t
      -> 'a cont_t =
  fun env id1 k_global_def k_not_found ->
    let find_in_monad =
      match env.curmonad with
    ...
    | FStar_Pervasives_Native.None ->
        let lid =
          let uu____3237 = current_module env in
          qual uu____3237 id1
        in
...
let module_is_defined : env -> FStar_Ident.lident -> Prims.bool =
  fun env lid ->
    ...
    || FStar_List.existsb
         (fun x -> FStar_Ident.lid_equals lid (FStar_Pervasives_Native.fst x))
         env.modules


let resolve_module_name
    : env -> FStar_Ident.lident -> Prims.bool
      -> FStar_Ident.lident FStar_Pervasives_Native.option =
  fun env lid honor_ns ->
    let nslen = FStar_List.length lid.FStar_Ident.ns in
    let rec aux uu___89_3279 =
      match uu___89_3279 with
      | [] ->
          let uu____3284 = module_is_defined env lid in
          if uu____3284 then FStar_Pervasives_Native.Some lid
          else FStar_Pervasives_Native.None
      | (Open_module_or_namespace (ns, Open_namespace)) :: q when honor_ns ->
          let new_lid =
            let uu____3293 =
              ...
              FStar_List.append uu____3296 uu____3299
            in
            FStar_Ident.lid_of_path uu____3293 (FStar_Ident.range_of_lid lid)
          in
          let uu____3302 = module_is_defined env new_lid in
          if uu____3302 then FStar_Pervasives_Native.Some new_lid else aux q
      | (Module_abbrev (name, modul)) :: uu____3308
        when nslen = Prims.parse_int "0"
...
let fail_if_curmodule
  ...
  fun env ns_original ns_resolved ->
    let uu____3328 =
      let uu____3329 = current_module env in
      FStar_Ident.lid_equals ns_resolved uu____3329
    in
    if uu____3328 then
      if FStar_Ident.lid_equals ns_resolved FStar_Parser_Const.prims_lid then
        ()
      else
        let uu____3331 =
          let uu____3336 =
          ...
          in
          (FStar_Errors.Fatal_ForbiddenReferenceToCurrentModule, uu____3336)
        in
...
let shorten_module_path
    : env -> FStar_Ident.ident Prims.list -> Prims.bool
      -> ( FStar_Ident.ident Prims.list
         , FStar_Ident.ident Prims.list )
         FStar_Pervasives_Native.tuple2 =
  fun env ids is_full_path ->
    let rec aux revns id1 =
      let lid = FStar_Ident.lid_of_ns_and_id (FStar_List.rev revns) id1 in
      if namespace_is_open env lid then
        ...
        | ns_last_id :: rev_ns_prefix ->
            ...
            FStar_All.pipe_right uu____3454
              (FStar_Util.map_option (fun uu____3504 ->
                   match uu____3504 with stripped_ids, rev_kept_ids ->
...
let shorten_lid : env -> FStar_Ident.lid -> FStar_Ident.lid =
  fun env lid ->
    ...
    match uu____3626 with uu____3635, short ->
      FStar_Ident.lid_of_ns_and_id short lid.FStar_Ident.ident


let resolve_in_open_namespaces'':
      'a. env -> FStar_Ident.lident -> exported_id_kind
      -> (local_binding -> 'a cont_t) -> (rec_binding -> 'a cont_t)
      -> (record_or_dc -> 'a cont_t) -> (FStar_Ident.lident -> 'a cont_t)
      -> ('a cont_t -> FStar_Ident.ident -> 'a cont_t)
      -> 'a FStar_Pervasives_Native.option =
  fun env lid eikind k_local_binding k_rec_binding k_record f_module l_default ->
    match lid.FStar_Ident.ns with
    | uu____3743 :: uu____3744 -> (
        let uu____3747 =
          let uu____3750 =
            let uu____3751 = FStar_Ident.lid_of_ids lid.FStar_Ident.ns in
            FStar_Ident.set_lid_range uu____3751 (FStar_Ident.range_of_lid lid)
          in
          resolve_module_name env uu____3750 true
        in
...
let resolve_in_open_namespaces':
      ...
      -> (FStar_Ident.lident
          ...
          -> 'a FStar_Pervasives_Native.option)
      -> 'a FStar_Pervasives_Native.option =
  fun env lid k_local_binding k_rec_binding k_global_def ->
    let k_global_def' k lid1 def =
      let uu____3876 = k_global_def lid1 def in
      cont_of_option k uu____3876
    in
    let f_module lid' =
      let k = Cont_ignore in
      find_in_module env lid' (k_global_def' k) k
    in
...
let lb_fv
    : FStar_Syntax_Syntax.letbinding Prims.list -> FStar_Ident.lident
  ...
  fun lbs lid ->
    let uu____3975 =
      FStar_Util.find_map lbs (fun lb ->
          ...
          if uu____3983 then FStar_Pervasives_Native.Some fv
          else FStar_Pervasives_Native.None )
    in
...
let try_lookup_name
    : Prims.bool -> Prims.bool -> env -> FStar_Ident.lident
      -> foundname FStar_Pervasives_Native.option =
  fun any_val exclude_interf env lid ->
    let occurrence_range = FStar_Ident.range_of_lid lid in
    let k_global_def source_lid uu___97_4026 =
      match uu___97_4026 with
      | uu____4033, true when exclude_interf -> FStar_Pervasives_Native.None
      | se, uu____4035 ->
        match se.FStar_Syntax_Syntax.sigel with
              ...
              let uu____4056 =
                let uu____4065 =
                  FStar_Syntax_Syntax.fvar source_lid
                    ...
                    FStar_Pervasives_Native.None
                in
                (uu____4065, false, se.FStar_Syntax_Syntax.sigattrs)
              in
              Term_name uu____4056
            in
        ...
        | FStar_Syntax_Syntax.Sig_datacon uu____4068 ->
            let uu____4083 =
              let uu____4084 =
                ...
                in
                (uu____4093, false, se.FStar_Syntax_Syntax.sigattrs)
              in
              Term_name uu____4084
            in
        ...
        | FStar_Syntax_Syntax.Sig_let ((uu____4099, lbs), uu____4101) ->
            ...
            let uu____4117 =
              let uu____4118 =
                ...
                in
                (uu____4127, false, se.FStar_Syntax_Syntax.sigattrs)
              in
              Term_name uu____4118
            in
            FStar_Pervasives_Native.Some uu____4117
        | FStar_Syntax_Syntax.Sig_declare_typ (lid1, uu____4131, uu____4132) ->
            let quals = se.FStar_Syntax_Syntax.sigquals in
            let uu____4136 =
              any_val
              || FStar_All.pipe_right quals
                   (FStar_Util.for_some (fun uu___93_4140 ->
            ...
            if uu____4136 then
              ...
              let dd =
                let uu____4146 =
                  ...
                  || FStar_All.pipe_right quals
                       (FStar_Util.for_some (fun uu___94_4151 ->
                            match uu___94_4151 with
                            | FStar_Syntax_Syntax.Projector uu____4152 -> true
                            | FStar_Syntax_Syntax.Discriminator uu____4157 ->
                                true
                            | uu____4158 -> false ))
                in
                if uu____4146 then FStar_Syntax_Syntax.Delta_equational
                else FStar_Syntax_Syntax.Delta_constant
              in
              let dd1 =
                let uu____4161 =
                  FStar_All.pipe_right quals
                    (FStar_Util.for_some (fun uu___95_4165 ->
                         match uu___95_4165 with
                         | FStar_Syntax_Syntax.Abstract -> true
                         | uu____4166 -> false ))
                in
                if uu____4161 then FStar_Syntax_Syntax.Delta_abstract dd
                else dd
              in
              let uu____4168 =
                FStar_Util.find_map quals (fun uu___96_4173 ->
              ...
              | FStar_Pervasives_Native.Some refl_monad ->
                  ...
                  FStar_Pervasives_Native.Some
                    (Term_name
                       (refl_const, false, se.FStar_Syntax_Syntax.sigattrs))
              | uu____4188 ->
                  let uu____4191 =
                    let uu____4192 =
                      let uu____4201 =
                        let uu____4202 = fv_qual_of_se se in
                        FStar_Syntax_Syntax.fvar lid2 dd1 uu____4202
                      in
                      (uu____4201, false, se.FStar_Syntax_Syntax.sigattrs)
                    in
                    Term_name uu____4192
                  in
    ...
    let k_local_binding r =
      let uu____4242 = found_local_binding (FStar_Ident.range_of_lid lid) r in
      match uu____4242 with t, mut ->
        FStar_Pervasives_Native.Some (Term_name (t, mut, []))
    in
    let k_rec_binding uu____4264 =
      match uu____4264 with id1, l, dd ->
        let uu____4276 =
          let uu____4277 =
            let uu____4286 =
              FStar_Syntax_Syntax.fvar
                (FStar_Ident.set_lid_range l (FStar_Ident.range_of_lid lid))
                dd FStar_Pervasives_Native.None
            in
            (uu____4286, false, [])
          in
          Term_name uu____4277
        in
    ...
    let found_unmangled =
      ...
      | uu____4318 -> FStar_Pervasives_Native.None
    in
    match found_unmangled with
    | FStar_Pervasives_Native.None ->
        resolve_in_open_namespaces' env lid k_local_binding k_rec_binding
          k_global_def
    | x -> x


let try_lookup_effect_name'
    : Prims.bool -> env -> FStar_Ident.lident
      -> ( FStar_Syntax_Syntax.sigelt
         , FStar_Ident.lident )
         FStar_Pervasives_Native.tuple2
         FStar_Pervasives_Native.option =
  fun exclude_interf env lid ->
...
let try_lookup_effect_name
  ...
  fun env l ->
    let uu____4377 =
      try_lookup_effect_name' (Prims.op_Negation env.iface) env l
    in
  ...
  fun env l ->
    ...
    | FStar_Pervasives_Native.Some
        ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4454; _ }
        , l1 ) ->
        FStar_Pervasives_Native.Some (l1, ne.FStar_Syntax_Syntax.cattributes)
    | FStar_Pervasives_Native.Some
        ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4480; _ }
        , l1 ) ->
  ...
  fun env l ->
    ...
    | FStar_Pervasives_Native.Some
        ( { FStar_Syntax_Syntax.sigel= FStar_Syntax_Syntax.Sig_new_effect ne
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4536; _ }
        , uu____4537 ) ->
    ...
    | FStar_Pervasives_Native.Some
        ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4550; _ }
        , uu____4551 ) ->
        FStar_Pervasives_Native.Some ne
  ...
  fun env l ->
    ...
    | FStar_Pervasives_Native.Some
        ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4602; _ }
        , uu____4603 ) ->
  ...
  fun env lid ->
    let k_global_def lid1 uu___98_4695 =
      ...
      | ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____4710; _ }
        , uu____4711 ) ->
    ...
    let uu____4725 =
      resolve_in_open_namespaces' env lid
        ...
        (fun uu____4737 -> FStar_Pervasives_Native.None)
        k_global_def
    in
...
let try_lookup_module
    : env -> Prims.string Prims.list
      -> FStar_Syntax_Syntax.modul FStar_Pervasives_Native.option =
  fun env path ->
    let uu____4764 =
      FStar_List.tryFind
        (fun uu____4779 ->
          match uu____4779 with mlid, modul ->
...
let try_lookup_let
  ...
  fun env lid ->
    ...
    resolve_in_open_namespaces' env lid
      ...
      (fun uu____4866 -> FStar_Pervasives_Native.None)
      k_global_def
...
let try_lookup_definition
  ...
  fun env lid ->
    let k_global_def lid1 uu___100_4889 =
      ...
      | ( { FStar_Syntax_Syntax.sigel=
              FStar_Syntax_Syntax.Sig_let (lbs, uu____4899)
          ; FStar_Syntax_Syntax.sigrng= uu____4900
          ; FStar_Syntax_Syntax.sigquals= uu____4901
          ; FStar_Syntax_Syntax.sigmeta= uu____4902
          ; FStar_Syntax_Syntax.sigattrs= uu____4903; _ }
        , uu____4904 ) ->
          FStar_Util.find_map (FStar_Pervasives_Native.snd lbs) (fun lb ->
              match lb.FStar_Syntax_Syntax.lbname with
              | FStar_Util.Inr fv when FStar_Syntax_Syntax.fv_eq_lid fv lid1 ->
    ...
    resolve_in_open_namespaces' env lid
      ...
      (fun uu____4948 -> FStar_Pervasives_Native.None)
      k_global_def


let empty_include_smap
    : FStar_Ident.lident Prims.list FStar_ST.ref FStar_Util.smap =
  new_sigmap ()
...
let try_lookup_lid'
    : Prims.bool -> Prims.bool -> env -> FStar_Ident.lident
      -> ( FStar_Syntax_Syntax.term
         ...
         FStar_Pervasives_Native.tuple3
         FStar_Pervasives_Native.option =
  fun any_val exclude_interface env lid ->
...
let try_lookup_lid_with_attributes
    : env -> FStar_Ident.lident
      -> ( FStar_Syntax_Syntax.term
         ...
         FStar_Pervasives_Native.tuple3
         FStar_Pervasives_Native.option =
  fun env l -> try_lookup_lid' env.iface false env l
  ...
  fun env l ->
    ...
    | FStar_Pervasives_Native.Some (e, uu____5195) ->
        let uu____5200 =
          ...
          uu____5201.FStar_Syntax_Syntax.n
        in
...
let try_lookup_doc
  ...
  fun env l -> FStar_Util.smap_try_find env.docs l.FStar_Ident.str
  ...
  fun env lid ->
    let k_global_def lid1 uu___102_5309 =
      ...
      | ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____5322; _ }
        , uu____5323 ) ->
          let uu____5328 =
            FStar_All.pipe_right quals
              (FStar_Util.for_some (fun uu___101_5332 ->
    ...
    resolve_in_open_namespaces' env lid
      ...
      (fun uu____5371 -> FStar_Pervasives_Native.None)
      k_global_def
  ...
  fun env lid ->
    let k_global_def lid1 uu___103_5396 =
      ...
      | ( { FStar_Syntax_Syntax.sigel=
          ...
          ; FStar_Syntax_Syntax.sigattrs= uu____5414; _ }
        , uu____5415 ) ->
    ...
    resolve_in_open_namespaces' env lid
      ...
      (fun uu____5444 -> FStar_Pervasives_Native.None)
      k_global_def
...
let record_cache_aux_with_filter
    : ( ( Prims.unit -> Prims.unit
        , Prims.unit -> Prims.unit
        , Prims.unit -> record_or_dc Prims.list
        , record_or_dc -> Prims.unit )
        FStar_Pervasives_Native.tuple4
      , Prims.unit -> Prims.unit )
      FStar_Pervasives_Native.tuple2 =
  ...
  let push1 uu____5489 =
    let uu____5490 =
      let uu____5495 =
        ...
        FStar_List.hd uu____5498
      in
      let uu____5554 = FStar_ST.op_Bang record_cache in
      uu____5495 :: uu____5554
    in
    FStar_ST.op_Colon_Equals record_cache uu____5490
  in
  let pop1 uu____5662 =
    let uu____5663 =
      let uu____5668 = FStar_ST.op_Bang record_cache in
      FStar_List.tl uu____5668
    in
    FStar_ST.op_Colon_Equals record_cache uu____5663
  in
  let peek1 uu____5778 =
    let uu____5779 = FStar_ST.op_Bang record_cache in
    FStar_List.hd uu____5779
  in
  let insert r =
    let uu____5839 =
      let uu____5844 =
        let uu____5847 = peek1 () in
        r :: uu____5847
      in
      let uu____5850 =
        let uu____5855 = FStar_ST.op_Bang record_cache in
        FStar_List.tl uu____5855
      in
      uu____5844 :: uu____5850
    in
    FStar_ST.op_Colon_Equals record_cache uu____5839
  in
  let filter1 uu____5965 =
    let rc = peek1 () in
    let filtered =
      FStar_List.filter
        (fun r -> Prims.op_Negation r.is_private_or_abstract)
        rc
    in
    let uu____5974 =
      let uu____5979 =
        let uu____5984 = FStar_ST.op_Bang record_cache in
        FStar_List.tl uu____5984
      in
      filtered :: uu____5979
    in
    FStar_ST.op_Colon_Equals record_cache uu____5974
  in
  let aux = (push1, pop1, peek1, insert) in
  (aux, filter1)
...
let extract_record
    : env -> scope_mod Prims.list FStar_ST.ref -> FStar_Syntax_Syntax.sigelt
      -> Prims.unit =
  fun e new_globs se ->
    match se.FStar_Syntax_Syntax.sigel with
    | FStar_Syntax_Syntax.Sig_bundle (sigs, uu____6546) ->
        let is_record =
          FStar_Util.for_some (fun uu___104_6562 ->
        ...
        let find_dc dc =
          FStar_All.pipe_right sigs
            (FStar_Util.find_opt (fun uu___105_6603 ->
        ...
        in
        FStar_All.pipe_right sigs
          (FStar_List.iter (fun uu___106_6657 ->
               ...
               | { FStar_Syntax_Syntax.sigel=
                 ...
                 -> (
                   let uu____6678 =
                     ...
                     FStar_All.pipe_left FStar_Util.must uu____6679
                   in
                   ...
                   | { FStar_Syntax_Syntax.sigel=
                     ...
                     -> (
                       ...
                       match uu____6702 with formals, uu____6716 ->
                         let is_rec = is_record typename_quals in
                         let formals' =
                           FStar_All.pipe_right formals
                             (FStar_List.collect (fun uu____6765 ->
                                  match uu____6765 with x, q ->
                                    let uu____6778 =
                                      FStar_Syntax_Syntax.is_null_bv x
                                      || is_rec
                                         && FStar_Syntax_Syntax.is_implicit q
                                    in
                                    if uu____6778 then [] else [(x, q)] ))
                         in
                         let fields' =
                           FStar_All.pipe_right formals'
                             (FStar_List.map (fun uu____6835 ->
                                  match uu____6835 with x, q ->
                                    let uu____6848 =
                                      if is_rec then
                                        FStar_Syntax_Util.unmangle_field_name
                                          x.FStar_Syntax_Syntax.ppname
                                      else x.FStar_Syntax_Syntax.ppname
                                    in
                                    (uu____6848, x.FStar_Syntax_Syntax.sort) ))
                         in
                         let fields = fields' in
                         let record =
                           { typename
                           ; constrname= constrname.FStar_Ident.ident
                           ; parms
                           ; fields
                           ; is_private_or_abstract=
                               FStar_List.contains FStar_Syntax_Syntax.Private
                                 typename_quals
                               || FStar_List.contains
                                    FStar_Syntax_Syntax.Abstract typename_quals
                           ; is_record= is_rec }
                         in
                         (let uu____6863 =
                            let uu____6866 = FStar_ST.op_Bang new_globs in
                            Record_or_dc record :: uu____6866
                          in
                          FStar_ST.op_Colon_Equals new_globs uu____6863) ;
                         match () with () ->
                           (let add_field uu____6969 =
                              match uu____6969 with id1, uu____6977 ->
                                let modul =
                                  let uu____6983 =
                                    FStar_Ident.lid_of_ids
                                ...
                                in
                                let uu____6984 = get_exported_id_set e modul in
                                match uu____6984 with
                                | FStar_Pervasives_Native.Some my_ex -> (
                                    let my_exported_ids =
                                      my_ex Exported_id_field
                                    in
                                    (let uu____7015 =
                                       let uu____7016 =
                                         FStar_ST.op_Bang my_exported_ids
                                       in
                                       FStar_Util.set_add
                                         id1.FStar_Ident.idText uu____7016
                                     in
                                     FStar_ST.op_Colon_Equals my_exported_ids
                                       uu____7015) ;
                                    match () with () ->
                                      let projname =
                                        let uu____7102 =
                                          let uu____7103 =
                                            FStar_Syntax_Util.
                                            mk_field_projector_name_from_ident
                                              constrname id1
                                          in
                                      ...
                                      let uu____7105 =
                                        let uu____7106 =
                                          FStar_ST.op_Bang my_exported_ids
                                        in
                                        FStar_Util.set_add projname uu____7106
                                      in
                                      FStar_ST.op_Colon_Equals my_exported_ids
                                        uu____7105 )
                                | FStar_Pervasives_Native.None -> ()
                           ...
                           match () with () -> insert_record_cache record )
                   | uu____7201 -> () )
               | uu____7202 -> () ))
    | uu____7203 -> ()
...
let try_lookup_record_or_dc_by_field_name
  ...
  fun env fieldname ->
    let find_in_cache fieldname1 =
      let uu____7218 =
        (fieldname1.FStar_Ident.ns, fieldname1.FStar_Ident.ident)
      in
      match uu____7218 with ns, id1 ->
        let uu____7235 = peek_record_cache () in
        FStar_Util.find_map uu____7235 (fun record ->
            let uu____7241 =
              find_in_record ns id1 record (fun r -> Cont_ok r)
            in
            option_of_cont
    ...
    resolve_in_open_namespaces'' env fieldname Exported_id_field
      (fun uu____7249 -> Cont_ignore)
      (fun uu____7251 -> Cont_ignore)
      (fun r -> Cont_ok r)
      (fun fn ->
...
let belongs_to_record : env -> FStar_Ident.lident -> record_or_dc -> Prims.bool =
  fun env lid record ->
    ...
    | FStar_Pervasives_Native.Some record'
      when let uu____7296 =
             ...
             in
             FStar_Ident.text_of_path uu____7297
           in
           let uu____7300 =
             let uu____7301 =
               FStar_Ident.path_of_ns record'.typename.FStar_Ident.ns
             in
             FStar_Ident.text_of_path uu____7301
           in
           uu____7296 = uu____7300
      -> (
        let uu____7304 =
          find_in_record record.typename.FStar_Ident.ns lid.FStar_Ident.ident
            record (fun uu____7308 -> Cont_ok () )
        in
    ...
    | FStar_Pervasives_Native.Some r ->
        let uu____7338 =
          let uu____7343 =
            let uu____7344 =
              FStar_Ident.lid_of_ids
                (FStar_List.append r.typename.FStar_Ident.ns [r.constrname])
            in
            FStar_Ident.set_lid_range uu____7344
              (FStar_Ident.range_of_lid fieldname)
          in
          (uu____7343, r.is_record)
        in
        FStar_Pervasives_Native.Some uu____7338
    | uu____7349 -> FStar_Pervasives_Native.None


let string_set_ref_new : Prims.unit -> Prims.string FStar_Util.set FStar_ST.ref =
  fun uu____7373 ->
...
let exported_id_set_new
    : Prims.unit -> exported_id_kind
      -> Prims.string FStar_Util.set FStar_ST.ref =
  fun uu____7398 ->
...
let unique
  ...
  fun any_val exclude_interface env lid ->
    ...
    let this_env =
      ...
      let uu____7456 = FStar_List.filter filter_scope_mods env.scope_mods in
    ...
    in
    let uu____7459 = try_lookup_lid' any_val exclude_interface this_env lid in
...
let push_bv_mutable
    : env -> FStar_Ident.ident
      -> (env, FStar_Syntax_Syntax.bv) FStar_Pervasives_Native.tuple2 =
  fun env x -> push_bv' env x true


let push_bv
    : env -> FStar_Ident.ident
      -> (env, FStar_Syntax_Syntax.bv) FStar_Pervasives_Native.tuple2 =
  fun env x -> push_bv' env x false
...
let push_top_level_rec_binding
  ...
  fun env x dd ->
    ...
    let uu____7546 = unique false true env l || FStar_Options.interactive () in
    if uu____7546 then push_scope_mod env (Rec_binding (x, l, dd))
    else
  ...
  fun env s ->
      ...
      let uu____7584 =
        let uu____7589 =
          FStar_Util.format2
            ...
            r
        in
        (FStar_Errors.Fatal_DuplicateTopLevelNames, uu____7589)
      in
      FStar_Errors.raise_error uu____7584 (FStar_Ident.range_of_lid l)
    in
    ...
    let env1 =
      let uu____7598 =
        ...
        | uu____7623 -> (false, false)
      in
      match uu____7598 with any_val, exclude_interface ->
        let lids = FStar_Syntax_Util.lids_of_sigelt s in
        let uu____7629 =
          FStar_Util.find_map lids (fun l ->
              let uu____7635 =
                let uu____7636 = unique any_val exclude_interface env l in
                Prims.op_Negation uu____7636
              in
              if uu____7635 then FStar_Pervasives_Native.Some l
              else FStar_Pervasives_Native.None )
        in
        ...
        | uu____7641 ->
            ...
            ; remaining_iface_decls= uu___124_7667.remaining_iface_decls
    ...
    in
    match uu____7718 with env3, lss ->
      FStar_All.pipe_right lss
        (FStar_List.iter (fun uu____7839 ->
             match uu____7839 with lids, se ->
               FStar_All.pipe_right lids
                 (FStar_List.iter (fun lid ->
                      (let uu____7861 =
                         let uu____7864 = FStar_ST.op_Bang globals in
                         Top_level_def lid.FStar_Ident.ident :: uu____7864
                       in
                       FStar_ST.op_Colon_Equals globals uu____7861) ;
                      match () with () ->
                        let modul =
                          let uu____7958 =
                            FStar_Ident.lid_of_ids lid.FStar_Ident.ns
                          in
                          uu____7958.FStar_Ident.str
                        in
                        (let uu____7960 = get_exported_id_set env3 modul in
                        ...
                        match () with () ->
                          let is_iface =
                            env3.iface && Prims.op_Negation env3.admitted_iface
                          in
                          FStar_Util.smap_add (sigmap env3) lid.FStar_Ident.str
                            ( se
                            , env3.iface
                              && Prims.op_Negation env3.admitted_iface ) )) )) ;
      let env4 =
      ...
      | FStar_Pervasives_Native.None ->
          ...
          let uu____8160 =
            FStar_All.pipe_right modules
              (FStar_Util.for_some (fun uu____8174 ->
                   match uu____8174 with m, uu____8180 ->
                     FStar_Util.starts_with
                       (Prims.strcat (FStar_Ident.text_of_lid m) ".")
                       (Prims.strcat (FStar_Ident.text_of_lid ns) ".") ))
          in
          if uu____8160 then (ns, Open_namespace)
          else
            let uu____8186 =
              let uu____8191 =
              ...
              in
              (FStar_Errors.Fatal_NameSpaceNotFound, uu____8191)
            in
            FStar_Errors.raise_error uu____8186 (FStar_Ident.range_of_lid ns)
      | FStar_Pervasives_Native.Some ns' ->
          fail_if_curmodule env ns ns' ;
          (ns', Open_module)
    in
    match uu____8141 with ns', kd ->
    ...
    | FStar_Pervasives_Native.Some ns1 -> (
        ...
        (let uu____8218 = FStar_Util.smap_try_find env1.includes curmod in
         ...
         | FStar_Pervasives_Native.Some incl ->
             let uu____8242 =
               let uu____8245 = FStar_ST.op_Bang incl in
               ns1 :: uu____8245
             in
             FStar_ST.op_Colon_Equals incl uu____8242) ;
        match () with () ->
          let uu____8338 =
            get_trans_exported_id_set env1 ns1.FStar_Ident.str
          in
          match uu____8338 with
          | FStar_Pervasives_Native.Some ns_trans_exports -> (
              (let uu____8355 =
                 let uu____8372 = get_exported_id_set env1 curmod in
                 let uu____8379 = get_trans_exported_id_set env1 curmod in
                 ...
                 , FStar_Pervasives_Native.Some cur_trans_exports ) ->
                   let update_exports k =
                     ...
                     match () with () ->
                       ...
                       let uu____8660 =
                         let uu____8661 = FStar_ST.op_Bang trans_ex in
                         FStar_Util.set_union uu____8661 ns_ex
                       in
                       FStar_ST.op_Colon_Equals trans_ex uu____8660
                   in
          ...
          | FStar_Pervasives_Native.None ->
              let uu____8767 =
                let uu____8772 =
                  FStar_Util.format1 "include: Module %s was not prepared"
                    ns1.FStar_Ident.str
                in
                (FStar_Errors.Fatal_IncludeModuleNotPrepared, uu____8772)
              in
        ...
        let uu____8776 =
          let uu____8781 =
            FStar_Util.format1 "include: Module %s cannot be found"
              ns.FStar_Ident.str
          in
          (FStar_Errors.Fatal_ModuleNotFound, uu____8781)
        in
      ...
      let uu____8795 =
        let uu____8800 =
          FStar_Util.format1 "Module %s cannot be found"
            (FStar_Ident.text_of_lid l)
        in
        (FStar_Errors.Fatal_ModuleNotFound, uu____8800)
      in
...
let push_doc
  ...
  fun env l doc_opt ->
    ...
    | FStar_Pervasives_Native.Some doc1 ->
        (let uu____8816 =
           FStar_Util.smap_try_find env.docs l.FStar_Ident.str
         in
         ...
         | FStar_Pervasives_Native.Some old_doc ->
             let uu____8820 =
               let uu____8825 =
                 ...
                 let uu____8827 = FStar_Parser_AST.string_of_fsdoc old_doc in
                 let uu____8828 = FStar_Parser_AST.string_of_fsdoc doc1 in
                 FStar_Util.format3
                   "Overwriting doc of %s; old doc was [%s]; new doc are [%s]"
                   uu____8826 uu____8827 uu____8828
               in
               (FStar_Errors.Warning_DocOverwrite, uu____8825)
             in
             FStar_Errors.log_issue (FStar_Ident.range_of_lid l) uu____8820) ;
    ...
    FStar_All.pipe_right env.sigaccum
      (FStar_List.iter (fun se ->
           ...
           | FStar_Syntax_Syntax.Sig_declare_typ (l, u, t) -> (
               let uu____8844 = try_lookup_lid env l in
               match uu____8844 with
               | FStar_Pervasives_Native.None ->
                   (let uu____8856 =
                      let uu____8857 = FStar_Options.interactive () in
                      Prims.op_Negation uu____8857
                    in
                    if uu____8856 then
                      let uu____8858 =
                        let uu____8863 =
                          ...
                          in
                          FStar_Util.format1
                            "Admitting %s without a definition" uu____8864
                        in
                   ...
                   FStar_Util.smap_add (sigmap env) l.FStar_Ident.str
                     ( (let uu___127_8874 = se in
                        { FStar_Syntax_Syntax.sigel=
                            uu___127_8874.FStar_Syntax_Syntax.sigel
                        ; FStar_Syntax_Syntax.sigrng=
                            uu___127_8874.FStar_Syntax_Syntax.sigrng
                        ; FStar_Syntax_Syntax.sigquals= quals
                        ; FStar_Syntax_Syntax.sigmeta=
                            uu___127_8874.FStar_Syntax_Syntax.sigmeta
                        ; FStar_Syntax_Syntax.sigattrs=
                            uu___127_8874.FStar_Syntax_Syntax.sigattrs })
                     , false )
               | FStar_Pervasives_Native.Some uu____8875 -> () )
           | uu____8884 -> () ))
  ...
  fun env modul ->
    FStar_All.pipe_right modul.FStar_Syntax_Syntax.declarations
      (FStar_List.iter (fun se ->
           let quals = se.FStar_Syntax_Syntax.sigquals in
           match se.FStar_Syntax_Syntax.sigel with
           | FStar_Syntax_Syntax.Sig_bundle (ses, uu____8901) ->
               if FStar_List.contains FStar_Syntax_Syntax.Private quals
                  || FStar_List.contains FStar_Syntax_Syntax.Abstract quals
               then
                 FStar_All.pipe_right ses
                   (FStar_List.iter (fun se1 ->
                                    ...
                                    let uu____8964 =
                                      let uu____8967 =
                                        let uu____8968 =
                                          ...
                                          (binders, uu____8981)
                                        in
                                        FStar_Syntax_Syntax.Tm_arrow uu____8968
                                      in
                                      FStar_Syntax_Syntax.mk uu____8967
                                    in
                                    uu____8964 FStar_Pervasives_Native.None
                                      (FStar_Ident.range_of_lid lid)
                                  in
                                  (lid, univ_names, uu____8961)
                                in
                                FStar_Syntax_Syntax.Sig_declare_typ uu____8954
                              in
                              let se2 =
                                let uu___128_8988 = se1 in
                                { FStar_Syntax_Syntax.sigel
                                ; FStar_Syntax_Syntax.sigrng=
                                    uu___128_8988.FStar_Syntax_Syntax.sigrng
                                ; FStar_Syntax_Syntax.sigquals=
                                    FStar_Syntax_Syntax.Assumption :: quals
                                ; FStar_Syntax_Syntax.sigmeta=
                                    uu___128_8988.FStar_Syntax_Syntax.sigmeta
                                ; FStar_Syntax_Syntax.sigattrs=
                                    uu___128_8988.FStar_Syntax_Syntax.sigattrs
                                }
                              in
           ...
           | FStar_Syntax_Syntax.Sig_declare_typ (lid, uu____8997, uu____8998) ->
               if FStar_List.contains FStar_Syntax_Syntax.Private quals then
                 FStar_Util.smap_remove (sigmap env) lid.FStar_Ident.str
               else ()
           | FStar_Syntax_Syntax.Sig_let ((uu____9004, lbs), uu____9006) ->
               if FStar_List.contains FStar_Syntax_Syntax.Private quals
                  || FStar_List.contains FStar_Syntax_Syntax.Abstract quals
               then
                        ...
                        let uu____9027 =
                          let uu____9028 =
                            let uu____9029 =
                              ...
                              in
                              uu____9032.FStar_Syntax_Syntax.fv_name
                            in
                            uu____9029.FStar_Syntax_Syntax.v
                          in
                          uu____9028.FStar_Ident.str
                        in
                        FStar_Util.smap_remove (sigmap env) uu____9027 ))
               else () ;
               if FStar_List.contains FStar_Syntax_Syntax.Abstract quals
                  && Prims.op_Negation
                        ...
                        let lid =
                          let uu____9046 =
                            let uu____9049 =
                              FStar_Util.right lb.FStar_Syntax_Syntax.lbname
                            in
                            uu____9049.FStar_Syntax_Syntax.fv_name
                          in
                          uu____9046.FStar_Syntax_Syntax.v
                        in
                        let quals1 = FStar_Syntax_Syntax.Assumption :: quals in
                        let decl =
                          let uu___129_9054 = se in
                          { FStar_Syntax_Syntax.sigel=
                              FStar_Syntax_Syntax.Sig_declare_typ
                                ( lid
                          ...
                          ; FStar_Syntax_Syntax.sigrng=
                              uu___129_9054.FStar_Syntax_Syntax.sigrng
                          ; FStar_Syntax_Syntax.sigquals= quals1
                          ; FStar_Syntax_Syntax.sigmeta=
                              uu___129_9054.FStar_Syntax_Syntax.sigmeta
                          ; FStar_Syntax_Syntax.sigattrs=
                              uu___129_9054.FStar_Syntax_Syntax.sigattrs }
                        in
                        FStar_Util.smap_add (sigmap env) lid.FStar_Ident.str
                          (decl, false) ))
               else ()
           | uu____9064 -> () )) ;
    let curmod =
      let uu____9066 = current_module env in
      uu____9066.FStar_Ident.str
    in
    (let uu____9068 =
       let uu____9085 = get_exported_id_set env curmod in
       let uu____9092 = get_trans_exported_id_set env curmod in
       (uu____9085, uu____9092)
     in
     match uu____9068 with
     | ( FStar_Pervasives_Native.Some cur_ex
       , FStar_Pervasives_Native.Some cur_trans_ex ) ->
         let update_exports eikind =
           let cur_ex_set =
             let uu____9146 = cur_ex eikind in
             FStar_ST.op_Bang uu____9146
           in
           let cur_trans_ex_set_ref = cur_trans_ex eikind in
           let uu____9271 =
             let uu____9272 = FStar_ST.op_Bang cur_trans_ex_set_ref in
             FStar_Util.set_union cur_ex_set uu____9272
           in
           FStar_ST.op_Colon_Equals cur_trans_ex_set_ref uu____9271
         in
         FStar_List.iter update_exports all_exported_id_kinds
     | uu____9357 -> ()) ;
    match () with () ->
      filter_record_cache () ;
      match () with () ->
        let uu___130_9375 = env in
        { curmodule= FStar_Pervasives_Native.None
        ; curmonad= uu___130_9375.curmonad
        ; modules= (modul.FStar_Syntax_Syntax.name, modul) :: env.modules
        ; scope_mods= []
        ; exported_ids= uu___130_9375.exported_ids
        ; trans_exported_ids= uu___130_9375.trans_exported_ids
        ; includes= uu___130_9375.includes
        ; sigaccum= []
        ; sigmap= uu___130_9375.sigmap
        ; iface= uu___130_9375.iface
        ; admitted_iface= uu___130_9375.admitted_iface
        ; expect_typ= uu___130_9375.expect_typ
        ; docs= uu___130_9375.docs
        ; remaining_iface_decls= uu___130_9375.remaining_iface_decls
        ; syntax_only= uu___130_9375.syntax_only
        ; ds_hooks= uu___130_9375.ds_hooks }
...
let push : env -> env =
  fun env ->
    push_record_cache () ;
    (let uu____9402 =
       let uu____9405 = FStar_ST.op_Bang stack in
       env :: uu____9405
     in
     FStar_ST.op_Colon_Equals stack uu____9402) ;
    let uu___131_9454 = env in
    let uu____9455 = FStar_Util.smap_copy (sigmap env) in
    let uu____9466 = FStar_Util.smap_copy env.docs in
    { curmodule= uu___131_9454.curmodule
    ; curmonad= uu___131_9454.curmonad
    ; modules= uu___131_9454.modules
    ; scope_mods= uu___131_9454.scope_mods
    ; exported_ids= uu___131_9454.exported_ids
    ; trans_exported_ids= uu___131_9454.trans_exported_ids
    ; includes= uu___131_9454.includes
    ; sigaccum= uu___131_9454.sigaccum
    ; sigmap= uu____9455
    ; iface= uu___131_9454.iface
    ; admitted_iface= uu___131_9454.admitted_iface
    ; expect_typ= uu___131_9454.expect_typ
    ; docs= uu____9466
    ; remaining_iface_decls= uu___131_9454.remaining_iface_decls
    ; syntax_only= uu___131_9454.syntax_only
    ; ds_hooks= uu___131_9454.ds_hooks }


let pop : Prims.unit -> env =
  fun uu____9471 ->
    let uu____9472 = FStar_ST.op_Bang stack in
    match uu____9472 with
    | env :: tl1 ->
        pop_record_cache () ;
        FStar_ST.op_Colon_Equals stack tl1 ;
        env
    | uu____9527 -> failwith "Impossible: Too many pops"
...
let export_interface : FStar_Ident.lident -> env -> env =
  fun m env ->
    let sigelt_in_m se =
      ...
      | l :: uu____9541 -> l.FStar_Ident.nsstr = m.FStar_Ident.str
      | uu____9544 -> false
    in
    ...
    FStar_All.pipe_right keys
      (FStar_List.iter (fun k ->
           let uu____9578 = FStar_Util.smap_try_find sm' k in
           ...
           | FStar_Pervasives_Native.Some (se, true) when sigelt_in_m se ->
               ...
               let se1 =
                 match se.FStar_Syntax_Syntax.sigel with
                 | FStar_Syntax_Syntax.Sig_declare_typ (l, u, t) ->
                     let uu___132_9603 = se in
                     { FStar_Syntax_Syntax.sigel=
                         uu___132_9603.FStar_Syntax_Syntax.sigel
                     ; FStar_Syntax_Syntax.sigrng=
                         uu___132_9603.FStar_Syntax_Syntax.sigrng
                     ; FStar_Syntax_Syntax.sigquals=
                         FStar_Syntax_Syntax.Assumption
                         :: se.FStar_Syntax_Syntax.sigquals
                     ; FStar_Syntax_Syntax.sigmeta=
                         uu___132_9603.FStar_Syntax_Syntax.sigmeta
                     ; FStar_Syntax_Syntax.sigattrs=
                         uu___132_9603.FStar_Syntax_Syntax.sigattrs }
                 | uu____9604 -> se
               in
...
let finish_module_or_interface : env -> FStar_Syntax_Syntax.modul -> env =
  fun env modul ->
    if Prims.op_Negation modul.FStar_Syntax_Syntax.is_interface then
      check_admits env
    else () ;
    finish env modul
...
let as_exported_ids : exported_id_set -> exported_ids =
  fun e ->
    let terms =
      let uu____9701 =
        let uu____9704 = e Exported_id_term_type in
        FStar_ST.op_Bang uu____9704
      in
      FStar_Util.set_elements uu____9701
    in
    let fields =
      let uu____9818 =
        let uu____9821 = e Exported_id_field in
        FStar_ST.op_Bang uu____9821
      in
      FStar_Util.set_elements uu____9818
    in
    {exported_id_terms= terms; exported_id_fields= fields}


let as_exported_id_set
    : exported_ids FStar_Pervasives_Native.option -> exported_id_kind
      -> Prims.string FStar_Util.set FStar_ST.ref =
  fun e ->
    match e with
    ...
    | FStar_Pervasives_Native.Some e1 ->
        let terms =
          let uu____9968 =
            FStar_Util.as_set e1.exported_id_terms FStar_Util.compare
          in
          FStar_Util.mk_ref uu____9968
        in
        let fields =
          let uu____9978 =
            FStar_Util.as_set e1.exported_id_fields FStar_Util.compare
          in
          FStar_Util.mk_ref uu____9978
        in
        fun uu___109_9983 ->
          match uu___109_9983 with
...
let as_includes:
      'Auu____10103. 'Auu____10103 Prims.list FStar_Pervasives_Native.option
      -> 'Auu____10103 Prims.list FStar_ST.ref =
  fun uu___110_10115 ->
    match uu___110_10115 with
...
let inclusion_info : env -> FStar_Ident.lident -> module_inclusion_info =
  fun env l ->
    ...
    let as_ids_opt m =
      let uu____10148 = FStar_Util.smap_try_find m mname in
      FStar_Util.map_opt uu____10148 as_exported_ids
    in
    let uu____10151 = as_ids_opt env.exported_ids in
    let uu____10154 = as_ids_opt env.trans_exported_ids in
    let uu____10157 =
      let uu____10162 = FStar_Util.smap_try_find env.includes mname in
      FStar_Util.map_opt uu____10162 (fun r -> FStar_ST.op_Bang r)
    in
    { mii_exported_ids= uu____10151
    ; mii_trans_exported_ids= uu____10154
    ; mii_includes= uu____10157 }
...
let prepare_module_or_interface
  ...
  fun intf admitted env mname mii ->
    let prep env1 =
      let filename =
        FStar_Util.strcat (FStar_Ident.text_of_lid mname) ".fst"
      in
      let auto_open = FStar_Parser_Dep.hard_coded_dependencies filename in
      let auto_open1 =
        let convert_kind uu___111_10282 =
          match uu___111_10282 with
          | FStar_Parser_Dep.Open_namespace -> Open_namespace
          | FStar_Parser_Dep.Open_module -> Open_module
        in
        FStar_List.map
          (fun uu____10294 ->
            match uu____10294 with lid, kind -> (lid, convert_kind kind) )
          auto_open
      in
      let namespace_of_module =
        if FStar_List.length mname.FStar_Ident.ns > Prims.parse_int "0" then
          let uu____10318 =
            let uu____10323 = FStar_Ident.lid_of_ids mname.FStar_Ident.ns in
            (uu____10323, Open_namespace)
          in
          [uu____10318]
        else []
      in
      let auto_open2 =
        FStar_List.append namespace_of_module (FStar_List.rev auto_open1)
      in
      (let uu____10353 = as_exported_id_set mii.mii_exported_ids in
       FStar_Util.smap_add env1.exported_ids mname.FStar_Ident.str uu____10353) ;
      match () with () ->
        (let uu____10377 = as_exported_id_set mii.mii_trans_exported_ids in
         FStar_Util.smap_add env1.trans_exported_ids mname.FStar_Ident.str
           uu____10377) ;
        match () with () ->
          (let uu____10401 = as_includes mii.mii_includes in
           FStar_Util.smap_add env1.includes mname.FStar_Ident.str uu____10401) ;
          match () with () ->
            let env' =
              let uu___133_10433 = env1 in
              let uu____10434 =
                FStar_List.map (fun x -> Open_module_or_namespace x) auto_open2
              in
              { curmodule= FStar_Pervasives_Native.Some mname
              ; curmonad= uu___133_10433.curmonad
              ; modules= uu___133_10433.modules
              ; scope_mods= uu____10434
              ; exported_ids= uu___133_10433.exported_ids
              ; trans_exported_ids= uu___133_10433.trans_exported_ids
              ; includes= uu___133_10433.includes
              ; sigaccum= uu___133_10433.sigaccum
              ; sigmap= env1.sigmap
              ; iface= intf
              ; admitted_iface= admitted
              ; expect_typ= uu___133_10433.expect_typ
              ; docs= uu___133_10433.docs
              ; remaining_iface_decls= uu___133_10433.remaining_iface_decls
              ; syntax_only= uu___133_10433.syntax_only
              ; ds_hooks= uu___133_10433.ds_hooks }
            in
            FStar_List.iter
              (fun op -> env1.ds_hooks.ds_push_open_hook env' op)
              (FStar_List.rev auto_open2) ;
            env'
    in
    let uu____10446 =
      FStar_All.pipe_right env.modules
        (FStar_Util.find_opt (fun uu____10472 ->
             match uu____10472 with l, uu____10478 ->
               FStar_Ident.lid_equals l mname ))
    in
    match uu____10446 with
    | FStar_Pervasives_Native.None ->
        let uu____10487 = prep env in
        (uu____10487, false)
    | FStar_Pervasives_Native.Some (uu____10488, m) ->
        (let uu____10495 =
           (let uu____10498 = FStar_Options.interactive () in
            Prims.op_Negation uu____10498)
           && (Prims.op_Negation m.FStar_Syntax_Syntax.is_interface || intf)
         in
         if uu____10495 then
           let uu____10499 =
             let uu____10504 =
               FStar_Util.format1 "Duplicate module or interface name: %s"
                 mname.FStar_Ident.str
             in
             (FStar_Errors.Fatal_DuplicateModuleOrInterface, uu____10504)
           in
           FStar_Errors.raise_error uu____10499
             (FStar_Ident.range_of_lid mname)
         else ()) ;
        let uu____10506 =
          let uu____10507 = push env in
          prep uu____10507
        in
        (uu____10506, true)
          ...
          , Prims.strcat "Trying to define monad "
              (Prims.strcat mname.FStar_Ident.idText
                 (Prims.strcat ", but already in monad scope "
                    mname'.FStar_Ident.idText)) ) mname.FStar_Ident.idRange
    | FStar_Pervasives_Native.None ->
        let uu___134_10515 = env in
        { curmodule= uu___134_10515.curmodule
        ; curmonad= FStar_Pervasives_Native.Some mname
        ; modules= uu___134_10515.modules
        ; scope_mods= uu___134_10515.scope_mods
        ; exported_ids= uu___134_10515.exported_ids
        ; trans_exported_ids= uu___134_10515.trans_exported_ids
        ; includes= uu___134_10515.includes
        ; sigaccum= uu___134_10515.sigaccum
        ; sigmap= uu___134_10515.sigmap
        ; iface= uu___134_10515.iface
        ; admitted_iface= uu___134_10515.admitted_iface
        ; expect_typ= uu___134_10515.expect_typ
        ; docs= uu___134_10515.docs
        ; remaining_iface_decls= uu___134_10515.remaining_iface_decls
        ; syntax_only= uu___134_10515.syntax_only
        ; ds_hooks= uu___134_10515.ds_hooks }


let fail_or:
      'a. env -> (FStar_Ident.lident -> 'a FStar_Pervasives_Native.option)
      -> FStar_Ident.lident -> 'a =
  fun env lookup lid ->
    let uu____10542 = lookup lid in
    match uu____10542 with
    | FStar_Pervasives_Native.None ->
        let opened_modules =
          FStar_List.map
            (fun uu____10555 ->
              match uu____10555 with lid1, uu____10561 ->
        ...
        in
        let msg1 =
          if FStar_List.length lid.FStar_Ident.ns = Prims.parse_int "0" then
            msg
          else
            let modul =
              let uu____10566 = FStar_Ident.lid_of_ids lid.FStar_Ident.ns in
              FStar_Ident.set_lid_range uu____10566
                (FStar_Ident.range_of_lid lid)
            in
            let uu____10567 = resolve_module_name env modul true in
            ...
            | FStar_Pervasives_Native.Some modul' ->
                FStar_Util.format4
                  "%s\nModule %s resolved into %s, definition %s not found" msg
                  modul.FStar_Ident.str modul'.FStar_Ident.str
                  lid.FStar_Ident.ident.FStar_Ident.idText
        in
        FStar_Errors.raise_error (FStar_Errors.Fatal_IdentifierNotFound, msg1)
          (FStar_Ident.range_of_lid lid)
    | FStar_Pervasives_Native.Some r -> r


let fail_or2:
      'a. (FStar_Ident.ident -> 'a FStar_Pervasives_Native.option)
      -> FStar_Ident.ident -> 'a =
  fun lookup id1 ->
    let uu____10598 = lookup id1 in
