...
let btree_find_prefix : 'a . 'a btree  ->  Prims.string  ->  (prefix_match * 'a) Prims.list = (fun bt prefix1 -> (
...
let uu____2313 = (btree_fold rbt f acc)
in (f k v1 uu____2313))
in (btree_fold lbt f uu____2312))
end))
...
let trie_empty : 'Auu____2553 . unit  ->  'Auu____2553 trie = (fun uu____2556 -> {bindings = []; namespaces = []})
...
let uu____2631 = (btree_find_exact bt ns)
in ((uu____2631), (FStar_Pervasives_Native.Some (names1))))
end
| (ImportedNames (uu____2642, names1))::more_names -> begin
(

let uu____2661 = (names_find_exact names1 ns)
in ((uu____2661), (FStar_Pervasives_Native.Some (more_names))))
end)
...
let uu____2755 = (names_find_exact tr.namespaces ns)
in (FStar_Util.bind_opt uu____2755 (fun scope -> (trie_descend_exact scope query1))))
end))
...
let uu____2810 = (names_find_exact tr.namespaces ns)
in (FStar_Util.bind_opt uu____2810 (fun scope -> (trie_find_exact scope query1))))
end))


let names_insert : 'a . 'a names  ->  Prims.string  ->  'a  ->  'a names = (fun name_collections id1 v1 -> (

let uu____2856 = (match (name_collections) with
| (Names (bt))::tl1 -> begin
((bt), (tl1))
end
| uu____2893 -> begin
...
let uu____2921 = (btree_insert_replace bt id1 v1)
in Names (uu____2921))
in (uu____2918)::name_collections1)
end)))
...
let uu____3121 = (names_find_exact namespaces ns)
in (FStar_Util.dflt (trie_empty ()) uu____3121))
in (

let uu____3130 = (trie_mutate trie q rev_acc1 mut_node mut_leaf)
in (names_insert namespaces ns uu____3130))))
...
let trie_mutate_leaf : 'a . 'a trie  ->  query  ->  ('a trie  ->  query  ->  'a trie)  ->  'a trie = (fun tr query -> (trie_mutate tr query [] (fun tr1 uu____3230 uu____3231 uu____3232 namespaces -> (

let uu___379_3241 = tr1
in {bindings = uu___379_3241.bindings; namespaces = namespaces}))))


let trie_insert : 'a . 'a trie  ->  query  ->  Prims.string  ->  'a  ->  'a trie = (fun tr ns_query id1 v1 -> (trie_mutate_leaf tr ns_query (fun tr1 uu____3289 -> (

let uu___388_3292 = tr1
in (

let uu____3295 = (names_insert tr1.bindings id1 v1)
in {bindings = uu____3295; namespaces = uu___388_3292.namespaces})))))
...
let uu____3370 = (trie_descend_exact tr included_query)
in (FStar_Util.dflt (trie_empty ()) uu____3370))
in (trie_mutate_leaf tr host_query (fun tr1 uu____3380 -> (mutator tr1 included_trie label))))))
...
let uu___406_3426 = tr1
in {bindings = (ImportedNames (((label), (inc.bindings))))::tr1.bindings; namespaces = uu___406_3426.namespaces}))))
...
let uu___415_3479 = tr1
in {bindings = uu___415_3479.bindings; namespaces = (ImportedNames (((label), (inc.namespaces))))::tr1.namespaces}))))


let trie_add_alias : 'a . 'a trie  ->  Prims.string  ->  query  ->  query  ->  'a trie = (fun tr key host_query included_query -> (trie_import tr host_query included_query (fun tr1 inc label -> (trie_mutate_leaf tr1 ((key)::[]) (fun _ignored_overwritten_trie uu____3551 -> {bindings = (ImportedNames (((label), (inc.bindings))))::[]; namespaces = []})))))


let names_revmap : 'a 'b . ('a btree  ->  'b)  ->  'a names  ->  (Prims.string Prims.list * 'b) Prims.list = (fun fn name_collections -> (

let rec aux = (fun acc imports name_collections1 -> (FStar_List.fold_left (fun acc1 uu___5_3688 -> (match (uu___5_3688) with
| Names (bt) -> begin
(

let uu____3712 = (

let uu____3720 = (fn bt)
in ((imports), (uu____3720)))
in (uu____3712)::acc1)
end
...
let matching_values_per_collection = (FStar_List.map (fun uu____3995 -> (match (uu____3995) with
| (imports, matches) -> begin
(FStar_List.map (fun uu____4046 -> (match (uu____4046) with
| (segment, v1) -> begin
(((mk_path_el imports segment)), (v1))
end)) matches)
end)) matching_values_per_collection_with_imports)
in (merge_increasing_lists_rev (fun uu____4064 -> (match (uu____4064) with
| (path_el, uu____4071) -> begin
...
let uu____4126 = (match (query) with
| [] -> begin
((NSTAll), (NSTAll), ([]))
end
| (id1)::[] -> begin
...
let acc_with_recursive_bindings = (FStar_List.fold_left (fun acc1 uu____4197 -> (match (uu____4197) with
| (path_el, trie) -> begin
...
in (FStar_List.rev_map_onto (fun uu____4242 -> (match (uu____4242) with
| (path_el, v1) -> begin
...
let uu___533_4558 = tbl
in (

let uu____4559 = (trie_insert tbl.tbl_lids host_query id1 c)
in {tbl_lids = uu____4559; tbl_mods = uu___533_4558.tbl_mods})))


let register_alias : table  ->  Prims.string  ->  query  ->  query  ->  table = (fun tbl key host_query included_query -> (

let uu___543_4585 = tbl
in (

let uu____4586 = (trie_add_alias tbl.tbl_lids key host_query included_query)
in {tbl_lids = uu____4586; tbl_mods = uu___543_4585.tbl_mods})))


let register_include : table  ->  query  ->  query  ->  table = (fun tbl host_query included_query -> (

let uu___551_4605 = tbl
in (

let uu____4606 = (trie_include tbl.tbl_lids host_query included_query)
in {tbl_lids = uu____4606; tbl_mods = uu___551_4605.tbl_mods})))
...
let uu___562_4635 = tbl
in (

let uu____4636 = (trie_open_namespace tbl.tbl_lids host_query included_query)
in {tbl_lids = uu____4636; tbl_mods = uu___562_4635.tbl_mods}))
end))
...
let uu____4705 = (names_find_exact bindings id1)
in ((uu____4705), (loaded1)))
in (match (uu____4697) with
| (FStar_Pervasives_Native.None, uu____4713) -> begin
(names_insert bindings id1 (Namespace ({ns_name = full_name; ns_loaded = loaded1})))
end
| (FStar_Pervasives_Native.Some (Namespace ({ns_name = uu____4718; ns_loaded = false})), true) -> begin
(names_insert bindings id1 (Namespace ({ns_name = full_name; ns_loaded = loaded1})))
end
| (FStar_Pervasives_Native.Some (uu____4725), uu____4726) -> begin
bindings
end)))
in (
...
let uu___607_4837 = tbl
in (

let uu____4838 = (trie_mutate tbl.tbl_mods mod_query [] (fun tr id1 q revq namespaces -> (

let uu___616_4861 = tr
in (

let uu____4864 = (ins id1 q revq tr.bindings loaded)
in {bindings = uu____4864; namespaces = namespaces}))) (fun tr uu____4870 -> tr))
in {tbl_lids = uu___607_4837.tbl_lids; tbl_mods = uu____4838})))))))


let string_of_path : path  ->  Prims.string = (fun path -> (

let uu____4881 = (FStar_List.map (fun el -> el.segment.completion) path)
in (FStar_String.concat "." uu____4881)))


let match_length_of_path : path  ->  Prims.int = (fun path -> (

let uu____4897 = (FStar_List.fold_left (fun acc elem -> (

let uu____4937 = acc
in (match (uu____4937) with
| (acc_len, uu____4959) -> begin
...
| ({imports = imports; segment = uu____5033})::uu____5034 -> begin
...
let completion_result_of_lid : 'Auu____5183 . (path * 'Auu____5183)  ->  completion_result = (fun uu____5192 -> (match (uu____5192) with
| (path, _lid) -> begin
(

let uu____5199 = (match_length_of_path path)
in (

let uu____5201 = (string_of_path path)
in (

let uu____5203 = (

let uu____5205 = (first_import_of_path path)
in (FStar_Util.dflt "" uu____5205))
in {completion_match_length = uu____5199; completion_candidate = uu____5201; completion_annotation = uu____5203})))
end))


let completion_result_of_mod : Prims.string  ->  Prims.bool  ->  path  ->  completion_result = (fun annot loaded path -> (

let uu____5231 = (match_length_of_path path)
in (

let uu____5233 = (string_of_path path)
in (

let uu____5235 = (FStar_Util.format1 (match (loaded) with
| true -> begin
" %s "
end
| uu____5240 -> begin
...
in {completion_match_length = uu____5231; completion_candidate = uu____5233; completion_annotation = uu____5235}))))


let completion_result_of_ns_or_mod : (path * mod_symbol)  ->  completion_result = (fun uu____5252 -> (match (uu____5252) with
| (path, symb) -> begin
(match (symb) with
| Module ({mod_name = uu____5259; mod_path = uu____5260; mod_loaded = loaded}) -> begin
(completion_result_of_mod "mod" loaded path)
end
| Namespace ({ns_name = uu____5266; ns_loaded = loaded}) -> begin
...
let uu____5297 = (trie_find_prefix tbl.tbl_lids query)
in (FStar_List.map completion_result_of_lid uu____5297)))
...
let uu____5358 = (trie_find_prefix tbl.tbl_mods query)
in (FStar_All.pipe_right uu____5358 (FStar_List.filter_map filter1)))
in (FStar_All.pipe_right uu____5351 (FStar_List.map completion_result_of_ns_or_mod))))
