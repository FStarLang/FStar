...
let string_compare : Prims.string -> Prims.string -> Prims.int =
  fun s1 s2 -> FStar_String.compare s1 s2
...
let uu___is_EmptyHeap: 'a. 'a heap -> Prims.bool =
  fun projectee -> match projectee with EmptyHeap -> true | uu____37 -> false


let uu___is_Heap: 'a. 'a heap -> Prims.bool =
  fun projectee -> match projectee with Heap _0 -> true | uu____60 -> false
...
let heap_merge:
      'Auu____98. ('Auu____98 -> 'Auu____98 -> Prims.int) -> 'Auu____98 heap
      -> 'Auu____98 heap -> 'Auu____98 heap =
  fun cmp h1 h2 ->
    match (h1, h2) with
    | EmptyHeap, h -> h
    | h, EmptyHeap -> h
    | Heap (v1, hh1), Heap (v2, hh2) ->
        let uu____173 =
          let uu____174 = cmp v1 v2 in
          uu____174 < Prims.parse_int "0"
        in
        if uu____173 then Heap (v1, h2 :: hh1) else Heap (v2, h1 :: hh2)


let heap_insert:
      'Auu____194. ('Auu____194 -> 'Auu____194 -> Prims.int)
      -> 'Auu____194 heap -> 'Auu____194 -> 'Auu____194 heap =
  fun cmp h v1 -> heap_merge cmp (Heap (v1, [])) h


let rec heap_merge_pairs:
          'Auu____233. ('Auu____233 -> 'Auu____233 -> Prims.int)
...
let heap_peek:
      'Auu____294. 'Auu____294 heap
      -> 'Auu____294 FStar_Pervasives_Native.option =
  fun uu___30_302 ->
...
let heap_pop:
      'Auu____318. ('Auu____318 -> 'Auu____318 -> Prims.int)
      -> 'Auu____318 heap
  ...
  fun cmp uu___31_342 ->
    ...
    | Heap (v1, hh) ->
        let uu____365 =
          let uu____372 = heap_merge_pairs cmp hh in
          (v1, uu____372)
        in
...
let heap_from_list:
      'Auu____386. ('Auu____386 -> 'Auu____386 -> Prims.int)
      -> 'Auu____386 Prims.list -> 'Auu____386 heap =
  fun cmp values -> FStar_List.fold_left (heap_insert cmp) EmptyHeap values


let push_nodup:
      'Auu____417. ('Auu____417 -> Prims.string) -> 'Auu____417
      -> 'Auu____417 Prims.list -> 'Auu____417 Prims.list =
  fun key_fn x uu___32_436 ->
  ...
  fun key_fn lists ->
    let cmp v1 v2 =
      ...
      | (pr1, h1 :: uu____616), (pr2, h2 :: uu____619) ->
          let cmp_h =
            ...
            string_compare uu____641 uu____642
          in
    ...
    let rec aux lists1 acc =
      ...
      | FStar_Pervasives_Native.Some ((pr, [v1]), lists2) ->
          let uu____811 = push_nodup key_fn v1 acc in
          aux lists2 uu____811
    ...
    | uu____906 ->
        ...
        let uu____928 = heap_from_list cmp lists2 in
        aux uu____928 []
...
let uu___is_StrEmpty: 'a. 'a btree -> Prims.bool =
  fun projectee -> match projectee with StrEmpty -> true | uu____977 -> false
...
let rec btree_to_list_rev:
          'Auu____1050. 'Auu____1050 btree
          -> (Prims.string, 'Auu____1050) FStar_Pervasives_Native.tuple2
             Prims.list
          -> (Prims.string, 'Auu____1050) FStar_Pervasives_Native.tuple2
             Prims.list =
  fun btree acc ->
    ...
    | StrBranch (key, value, lbt, rbt) ->
        let uu____1093 =
          let uu____1100 = btree_to_list_rev lbt acc in
          (key, value) :: uu____1100
        in
...
let rec btree_from_list:
          'Auu____1115.
          (Prims.string, 'Auu____1115) FStar_Pervasives_Native.tuple2
  ...
  fun nodes size ->
    if size = Prims.parse_int "0" then (StrEmpty, nodes)
    else
      ...
      match uu____1159 with lbt, nodes_left ->
        ...
        | (k, v1) :: nodes_left1 ->
            ...
            match uu____1243 with rbt, nodes_left2 ->
...
let rec btree_find_exact:
  ...
  fun bt k ->
    ...
    | StrBranch (k', v1, lbt, rbt) ->
        let cmp = string_compare k k' in
        if cmp < Prims.parse_int "0" then btree_find_exact lbt k
        else if cmp > Prims.parse_int "0" then btree_find_exact rbt k
...
let __proj__Mkprefix_match__item__prefix
  ...
  fun projectee ->
    match projectee
    with {prefix= __fname__prefix; completion= __fname__completion; _} ->
      __fname__prefix
...
let __proj__Mkpath_elem__item__imports : path_elem -> Prims.string Prims.list =
  fun projectee ->
    match projectee
    with {imports= __fname__imports; segment= __fname__segment; _} ->
      __fname__imports
...
let mk_path_el : Prims.string Prims.list -> prefix_match -> path_elem =
  fun imports segment -> {imports; segment}


let rec btree_find_prefix:
          'a. 'a btree -> Prims.string
  ...
  fun bt prefix1 ->
    let rec aux bt1 prefix2 acc =
      ...
      | StrBranch (k, v1, lbt, rbt) ->
          ...
          let explore_right = cmp <= Prims.parse_int "0" || include_middle in
          ...
          let matches1 =
            if include_middle then
              ( {prefix= FStar_Pervasives_Native.Some prefix2; completion= k}
              , v1 )
              :: matches
            else matches
          in
          let matches2 =
            if explore_left then aux lbt prefix2 matches1 else matches1
          in
          matches2
    in
    aux bt prefix1 []
...
let rec btree_fold:
  ...
  fun bt f acc ->
    ...
    | StrBranch (k, v1, lbt, rbt) ->
        let uu____1861 =
          let uu____1862 = btree_fold rbt f acc in
          f k v1 uu____1862
        in
...
let rec names_find_exact:
  ...
  fun names ns ->
    ...
    in
    match uu____2100 with result, names1 ->
      match (result, names1) with
      | FStar_Pervasives_Native.None, FStar_Pervasives_Native.Some scopes ->
...
let rec trie_descend_exact:
  ...
  fun tr query ->
    ...
    | ns :: query1 ->
        ...
        FStar_Util.bind_opt uu____2284 (fun scope ->
            trie_descend_exact scope query1 )
...
let rec trie_find_exact:
  ...
  fun tr query ->
    ...
    | ns :: query1 ->
        ...
        FStar_Util.bind_opt uu____2325 (fun scope ->
            trie_find_exact scope query1 )


let names_insert: 'a. 'a names -> Prims.string -> 'a -> 'a names =
  fun name_collections id1 v1 ->
    let uu____2365 =
      match name_collections with
      | (Names bt) :: tl1 -> (bt, tl1)
      | uu____2403 -> (StrEmpty, name_collections)
    in
    match uu____2365 with bt, name_collections1 ->
      let uu____2428 =
...
let rec namespaces_mutate:
          'a. 'a trie names -> Prims.string -> query -> query
          -> ('a trie -> Prims.string -> query -> query -> 'a trie names
              -> 'a trie) -> ('a trie -> query -> 'a trie) -> 'a trie names =
  fun namespaces ns q rev_acc1 mut_node mut_leaf ->
    let trie =
      let uu____2609 = names_find_exact namespaces ns in
      FStar_Util.dflt (trie_empty ()) uu____2609
    in
...
and trie_mutate:
      'a. 'a trie -> query -> query
      -> ('a trie -> Prims.string -> query -> query -> 'a trie names -> 'a trie)
      -> ('a trie -> query -> 'a trie) -> 'a trie =
  fun tr q rev_acc1 mut_node mut_leaf ->
    match q with
    | [] -> mut_leaf tr rev_acc1
    | id1 :: q1 ->
        let ns' =
          namespaces_mutate tr.namespaces id1 q1 (id1 :: rev_acc1) mut_node
            mut_leaf
        in
...
let trie_mutate_leaf:
  ...
  fun tr query ->
    trie_mutate tr query []
      (fun tr1 uu____2709 uu____2710 uu____2711 namespaces ->
        let uu___35_2720 = tr1 in
        {bindings= uu___35_2720.bindings; namespaces} )


let trie_insert: 'a. 'a trie -> query -> Prims.string -> 'a -> 'a trie =
  fun tr ns_query id1 v1 ->
    trie_mutate_leaf tr ns_query (fun tr1 uu____2758 ->
        let uu___36_2761 = tr1 in
        let uu____2764 = names_insert tr1.bindings id1 v1 in
        {bindings= uu____2764; namespaces= uu___36_2761.namespaces} )


let trie_import:
      'a. 'a trie -> query -> query
      -> ('a trie -> 'a trie -> Prims.string -> 'a trie) -> 'a trie =
  fun tr host_query included_query mutator ->
    let label = query_to_string included_query in
    let included_trie =
      let uu____2828 = trie_descend_exact tr included_query in
      FStar_Util.dflt (trie_empty ()) uu____2828
    in
    trie_mutate_leaf tr host_query (fun tr1 uu____2838 ->
        mutator tr1 included_trie label )


let trie_include: 'a. 'a trie -> query -> query -> 'a trie =
  fun tr host_query included_query ->
    trie_import tr host_query included_query (fun tr1 inc label ->
        let uu___37_2875 = tr1 in
        { bindings= ImportedNames (label, inc.bindings) :: tr1.bindings
...
let trie_add_alias: 'a. 'a trie -> Prims.string -> query -> query -> 'a trie =
  fun tr key host_query included_query ->
    trie_import tr host_query included_query (fun tr1 inc label ->
        trie_mutate_leaf tr1 [key] (fun _ignored_overwritten_trie uu____2970 ->
            {bindings= [ImportedNames (label, inc.bindings)]; namespaces= []}
        ) )


let names_revmap:
      'a 'b. ('a btree -> 'b) -> 'a names
      -> (Prims.string Prims.list, 'b) FStar_Pervasives_Native.tuple2
         Prims.list =
  fun fn name_collections ->
    let rec aux acc imports name_collections1 =
      FStar_List.fold_left
        (fun acc1 uu___34_3087 ->
          match uu___34_3087 with
          | Names bt ->
              let uu____3109 =
                let uu____3116 = fn bt in
                (imports, uu____3116)
              in
              uu____3109 :: acc1
          | ImportedNames (nm, name_collections2) ->
              aux acc1 (nm :: imports) name_collections2 )
        acc name_collections1
    in
    aux [] [] name_collections


let btree_find_all:
      'a. Prims.string FStar_Pervasives_Native.option -> 'a btree
      -> (prefix_match, 'a) FStar_Pervasives_Native.tuple2 Prims.list =
  fun prefix1 bt ->
    btree_fold bt
      (fun k tr acc -> ({prefix= prefix1; completion= k}, tr) :: acc)
      []
...
let uu___is_NSTAll : name_search_term -> Prims.bool =
  fun projectee -> match projectee with NSTAll -> true | uu____3204 -> false


let uu___is_NSTNone : name_search_term -> Prims.bool =
  fun projectee -> match projectee with NSTNone -> true | uu____3208 -> false
...
let names_find_rev:
  ...
  fun names id1 ->
    ...
    let matching_values_per_collection =
      FStar_List.map
        (fun uu____3346 ->
          match uu____3346 with imports, matches ->
            FStar_List.map
              (fun uu____3394 ->
                match uu____3394 with segment, v1 ->
                  (mk_path_el imports segment, v1) )
              matches )
        matching_values_per_collection_with_imports
    in
    merge_increasing_lists_rev
      (fun uu____3412 ->
        match uu____3412 with path_el, uu____3418 -> path_el.segment.completion
        )
...
let rec trie_find_prefix':
  ...
  fun tr path_acc query acc ->
    ...
    in
    match uu____3464 with ns_search_term, bindings_search_term, query1 ->
      let matching_namespaces_rev =
        names_find_rev tr.namespaces ns_search_term
      in
      let acc_with_recursive_bindings =
        FStar_List.fold_left
          (fun acc1 uu____3540 ->
            match uu____3540 with path_el, trie ->
              trie_find_prefix' trie (path_el :: path_acc) query1 acc1 )
          acc matching_namespaces_rev
      in
      let matching_bindings_rev =
        names_find_rev tr.bindings bindings_search_term
      in
      FStar_List.rev_map_onto
        (fun uu____3585 ->
          match uu____3585 with path_el, v1 ->
...
let trie_find_prefix:
      'a. 'a trie -> query
      -> (path, 'a) FStar_Pervasives_Native.tuple2 Prims.list =
  fun tr query -> trie_find_prefix' tr [] query []


type ns_info = {ns_name: Prims.string; ns_loaded: Prims.bool} [@@deriving show]

let __proj__Mkns_info__item__ns_name : ns_info -> Prims.string =
  fun projectee ->
    match projectee
    with {ns_name= __fname__ns_name; ns_loaded= __fname__ns_loaded; _} ->
      __fname__ns_name
...
let __proj__Mkmod_info__item__mod_name : mod_info -> Prims.string =
  fun projectee ->
    match projectee
    with
    | { mod_name= __fname__mod_name
      ; mod_path= __fname__mod_path
      ; mod_loaded= __fname__mod_loaded; _ }
    -> __fname__mod_name
...
type mod_symbol = Module of mod_info | Namespace of ns_info [@@deriving show]
...
type symbol = ModOrNs of mod_symbol | Lid of lid_symbol [@@deriving show]
...
let uu___is_Lid : symbol -> Prims.bool =
  fun projectee -> match projectee with Lid _0 -> true | uu____3733 -> false
...
let __proj__Mktable__item__tbl_lids : table -> lid_symbol trie =
  fun projectee ->
    match projectee
    with {tbl_lids= __fname__tbl_lids; tbl_mods= __fname__tbl_mods; _} ->
      __fname__tbl_lids
...
let register_alias : table -> Prims.string -> query -> query -> table =
  fun tbl key host_query included_query ->
    let uu___40_3807 = tbl in
    let uu____3808 =
      trie_add_alias tbl.tbl_lids key host_query included_query
    in
    {tbl_lids= uu____3808; tbl_mods= uu___40_3807.tbl_mods}


let register_include : table -> query -> query -> table =
  fun tbl host_query included_query ->
...
let register_open : table -> Prims.bool -> query -> query -> table =
  fun tbl is_module host_query included_query ->
    if is_module then register_include tbl host_query included_query
    else
      let uu___42_3837 = tbl in
      let uu____3838 =
        trie_open_namespace tbl.tbl_lids host_query included_query
      in
  ...
  fun tbl loaded path mod_query ->
    let ins_ns id1 bindings full_name loaded1 =
      ...
      | ( FStar_Pervasives_Native.Some
            Namespace {ns_name= uu____3895; ns_loaded= false; _}
        , true ) ->
          names_insert bindings id1
            (Namespace {ns_name= full_name; ns_loaded= loaded1})
      | FStar_Pervasives_Native.Some uu____3900, uu____3901 -> bindings
    in
    let ins_mod id1 bindings full_name loaded1 =
      names_insert bindings id1
        (Module {mod_name= full_name; mod_path= path; mod_loaded= loaded1})
    in
    let name_of_revq query = FStar_String.concat "." (FStar_List.rev query) in
    let ins id1 q revq bindings loaded1 =
      let name = name_of_revq (id1 :: revq) in
      match q with
      | [] -> ins_mod id1 bindings name loaded1
      | uu____3974 -> ins_ns id1 bindings name loaded1
    in
    let uu___43_3980 = tbl in
    let uu____3981 =
      trie_mutate tbl.tbl_mods mod_query []
        (fun tr id1 q revq namespaces ->
          let uu___44_4003 = tr in
          let uu____4006 = ins id1 q revq tr.bindings loaded in
          {bindings= uu____4006; namespaces} )
        (fun tr uu____4017 -> tr)
    in
...
let match_length_of_path : path -> Prims.int =
  fun path ->
    let uu____4031 =
      FStar_List.fold_left
        (fun acc elem ->
          ...
          match uu____4065 with acc_len, uu____4083 ->
            match elem.segment.prefix with
            | FStar_Pervasives_Native.Some prefix1 ->
                let completion_len =
                  FStar_String.length elem.segment.completion
                in
    ...
    in
    match uu____4031 with length1, (last_prefix, last_completion_length) ->
...
let completion_result_of_lid:
      'Auu____4224. (path, 'Auu____4224) FStar_Pervasives_Native.tuple2
      -> completion_result =
  fun uu____4232 ->
    match uu____4232 with path, _lid ->
...
let completion_result_of_ns_or_mod
  ...
  fun uu____4264 ->
    match uu____4264 with path, symb ->
      match symb with
      | Module
          {mod_name= uu____4271; mod_path= uu____4272; mod_loaded= loaded; _} ->
...
let find_module_or_ns
  ...
  fun tbl query -> trie_find_exact tbl.tbl_mods query
...
let autocomplete_mod_or_ns
    : table -> query
      -> ((path, mod_symbol) FStar_Pervasives_Native.tuple2
          -> (path, mod_symbol) FStar_Pervasives_Native.tuple2
             FStar_Pervasives_Native.option) -> completion_result Prims.list =
  fun tbl query filter1 ->
    let uu____4339 =
      let uu____4346 = trie_find_prefix tbl.tbl_mods query in
      FStar_All.pipe_right uu____4346 (FStar_List.filter_map filter1)
    in
