        ...
        (fun tr1  ->
           fun uu____2922  ->
             fun uu____2923  ->
               fun uu____2924  ->
                 fun namespaces  ->
                   let uu___64_2933 = tr1  in
                   { bindings = (uu___64_2933.bindings); namespaces })
      ...
      fun id1  ->
        fun v1  ->
          trie_mutate_leaf tr ns_query
            (fun tr1  ->
               fun uu____2980  ->
                 let uu___65_2983 = tr1  in
                 let uu____2986 = names_insert tr1.bindings id1 v1  in
                 {
                   bindings = uu____2986;
                   namespaces = (uu___65_2983.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___66_3113 = tr1  in
                 {
                   bindings = ((ImportedNames (label, (inc.bindings))) ::
                     (tr1.bindings));
                   namespaces = (uu___66_3113.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___67_3163 = tr1  in
                 {
                   bindings = (uu___67_3163.bindings);
...
let (uu___is_Module : mod_symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Module _0 -> true | uu____4003 -> false
  
let (__proj__Module__item___0 : mod_symbol -> mod_info) =
  fun projectee  -> match projectee with | Module _0 -> _0 
let (uu___is_Namespace : mod_symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Namespace _0 -> true | uu____4017 -> false
...
let (uu___is_ModOrNs : symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | ModOrNs _0 -> true | uu____4041 -> false
...
let (uu___is_Lid : symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Lid _0 -> true | uu____4055 -> false
...
let (insert : table -> query -> Prims.string -> lid_symbol -> table) =
  fun tbl  ->
    fun host_query  ->
      fun id1  ->
        fun c  ->
          let uu___68_4127 = tbl  in
          let uu____4128 = trie_insert tbl.tbl_lids host_query id1 c  in
          { tbl_lids = uu____4128; tbl_mods = (uu___68_4127.tbl_mods) }
...
let (register_alias : table -> Prims.string -> query -> query -> table) =
  fun tbl  ->
    fun key  ->
      fun host_query  ->
        fun included_query  ->
          let uu___69_4151 = tbl  in
          let uu____4152 =
            trie_add_alias tbl.tbl_lids key host_query included_query  in
          { tbl_lids = uu____4152; tbl_mods = (uu___69_4151.tbl_mods) }
  
let (register_include : table -> query -> query -> table) =
  fun tbl  ->
    fun host_query  ->
      fun included_query  ->
        let uu___70_4170 = tbl  in
        let uu____4171 = trie_include tbl.tbl_lids host_query included_query
           in
        { tbl_lids = uu____4171; tbl_mods = (uu___70_4170.tbl_mods) }
      ...
      fun host_query  ->
        fun included_query  ->
          ...
          then register_include tbl host_query included_query
            ...
            (let uu___71_4195 = tbl  in
             let uu____4196 =
               trie_open_namespace tbl.tbl_lids host_query included_query  in
             { tbl_lids = uu____4196; tbl_mods = (uu___71_4195.tbl_mods) })
      ...
      fun path  ->
        fun mod_query  ->
          let ins_ns id1 bindings full_name loaded1 =
            let uu____4248 =
              let uu____4255 = names_find_exact bindings id1  in
              (uu____4255, loaded1)  in
            match uu____4248 with
            | (FStar_Pervasives_Native.None ,uu____4264) ->
                names_insert bindings id1
                  (Namespace { ns_name = full_name; ns_loaded = loaded1 })
            | (FStar_Pervasives_Native.Some (Namespace
               { ns_name = uu____4269; ns_loaded = false ;_}),true ) ->
                names_insert bindings id1
                  (Namespace { ns_name = full_name; ns_loaded = loaded1 })
            | (FStar_Pervasives_Native.Some uu____4274,uu____4275) ->
                bindings
             in
          ...
          let ins id1 q revq bindings loaded1 =
            ...
            | uu____4368 -> ins_ns id1 bindings name loaded1  in
          let uu___72_4374 = tbl  in
          let uu____4375 =
              ...
              (fun tr  ->
                 fun id1  ->
                   fun q  ->
                     fun revq  ->
                       fun namespaces  ->
                         let uu___73_4397 = tr  in
                         let uu____4400 = ins id1 q revq tr.bindings loaded
                            in
                         { bindings = uu____4400; namespaces })
              (fun tr  -> fun uu____4411  -> tr)
             in
          { tbl_lids = (uu___72_4374.tbl_lids); tbl_mods = uu____4375 }
  
let (string_of_path : path -> Prims.string) =
  fun path  ->
    let uu____4419 = FStar_List.map (fun el  -> (el.segment).completion) path
       in
    FStar_String.concat "." uu____4419
...
let (first_import_of_path :
  ...
  fun path  ->
    ...
    | { imports; segment = uu____4537;_}::uu____4538 ->
...
let completion_result_of_lid :
  'Auu____4641 .
    (path,'Auu____4641) FStar_Pervasives_Native.tuple2 -> completion_result
  =
  fun uu____4650  ->
    ...
    | (path,_lid) ->
        ...
        let uu____4659 =
          let uu____4660 = first_import_of_path path  in
          FStar_Util.dflt "" uu____4660  in
        {
...
let (completion_result_of_mod :
  ...
  fun annot  ->
    fun loaded  ->
      fun path  ->
        let uu____4678 = match_length_of_path path  in
        let uu____4679 = string_of_path path  in
        let uu____4680 =
          FStar_Util.format1 (if loaded then " %s " else "(%s)") annot  in
        {
          completion_match_length = uu____4678;
          completion_candidate = uu____4679;
          completion_annotation = uu____4680
        }
...
let (completion_result_of_ns_or_mod :
  ...
  fun uu____4690  ->
    ...
    | (path,symb) ->
        (match symb with
         | Module
             { mod_name = uu____4697; mod_path = uu____4698;
               mod_loaded = loaded;_}
             -> completion_result_of_mod "mod" loaded path
         | Namespace { ns_name = uu____4700; ns_loaded = loaded;_} ->
...
let (autocomplete_lid : table -> query -> completion_result Prims.list) =
  fun tbl  ->
    fun query  ->
      let uu____4726 = trie_find_prefix tbl.tbl_lids query  in
      FStar_List.map completion_result_of_lid uu____4726
...
let (autocomplete_mod_or_ns :
  ...
  fun tbl  ->
    fun query  ->
      fun filter1  ->
        let uu____4779 =
          let uu____4786 = trie_find_prefix tbl.tbl_mods query  in
          FStar_All.pipe_right uu____4786 (FStar_List.filter_map filter1)  in
        FStar_All.pipe_right uu____4779
