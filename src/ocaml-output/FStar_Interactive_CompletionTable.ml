...
let uu____2453 = (names_find_exact tr.namespaces ns)
in (FStar_Util.bind_opt uu____2453 (fun scope -> (trie_descend_exact scope query1))))
end))
...
let uu____2499 = (names_find_exact tr.namespaces ns)
in (FStar_Util.bind_opt uu____2499 (fun scope -> (trie_find_exact scope query1))))
end))


let names_insert : 'a . 'a names  ->  Prims.string  ->  'a  ->  'a names = (fun name_collections id1 v1 -> (

let uu____2546 = (match (name_collections) with
| (Names (bt))::tl1 -> begin
((bt), (tl1))
end
| uu____2584 -> begin
...
let uu____2612 = (btree_insert_replace bt id1 v1)
in Names (uu____2612))
in (uu____2609)::name_collections1)
end)))
...
let uu____2814 = (names_find_exact namespaces ns)
in (FStar_Util.dflt (trie_empty ()) uu____2814))
in (

let uu____2825 = (trie_mutate trie q rev_acc1 mut_node mut_leaf)
in (names_insert namespaces ns uu____2825))))
...
let trie_mutate_leaf : 'a . 'a trie  ->  query  ->  ('a trie  ->  query  ->  'a trie)  ->  'a trie = (fun tr query -> (trie_mutate tr query [] (fun tr1 uu____2922 uu____2923 uu____2924 namespaces -> (

let uu___37_2933 = tr1
in {bindings = uu___37_2933.bindings; namespaces = namespaces}))))


let trie_insert : 'a . 'a trie  ->  query  ->  Prims.string  ->  'a  ->  'a trie = (fun tr ns_query id1 v1 -> (trie_mutate_leaf tr ns_query (fun tr1 uu____2980 -> (

let uu___38_2983 = tr1
in (

let uu____2986 = (names_insert tr1.bindings id1 v1)
in {bindings = uu____2986; namespaces = uu___38_2983.namespaces})))))
...
let uu____3059 = (trie_descend_exact tr included_query)
in (FStar_Util.dflt (trie_empty ()) uu____3059))
in (trie_mutate_leaf tr host_query (fun tr1 uu____3069 -> (mutator tr1 included_trie label))))))
...
let uu___39_3113 = tr1
in {bindings = (ImportedNames (((label), (inc.bindings))))::tr1.bindings; namespaces = uu___39_3113.namespaces}))))
...
let uu___40_3163 = tr1
in {bindings = uu___40_3163.bindings; namespaces = (ImportedNames (((label), (inc.namespaces))))::tr1.namespaces}))))


let trie_add_alias : 'a . 'a trie  ->  Prims.string  ->  query  ->  query  ->  'a trie = (fun tr key host_query included_query -> (trie_import tr host_query included_query (fun tr1 inc label -> (trie_mutate_leaf tr1 ((key)::[]) (fun _ignored_overwritten_trie uu____3228 -> {bindings = (ImportedNames (((label), (inc.bindings))))::[]; namespaces = []})))))


let names_revmap : 'a 'b . ('a btree  ->  'b)  ->  'a names  ->  (Prims.string Prims.list * 'b) Prims.list = (fun fn name_collections -> (

let rec aux = (fun acc imports name_collections1 -> (FStar_List.fold_left (fun acc1 uu___36_3359 -> (match (uu___36_3359) with
| Names (bt) -> begin
(

let uu____3381 = (

let uu____3388 = (fn bt)
in ((imports), (uu____3388)))
in (uu____3381)::acc1)
end
...
let matching_values_per_collection = (FStar_List.map (fun uu____3635 -> (match (uu____3635) with
| (imports, matches) -> begin
(FStar_List.map (fun uu____3683 -> (match (uu____3683) with
| (segment, v1) -> begin
(((mk_path_el imports segment)), (v1))
end)) matches)
end)) matching_values_per_collection_with_imports)
in (merge_increasing_lists_rev (fun uu____3701 -> (match (uu____3701) with
| (path_el, uu____3707) -> begin
...
let uu____3761 = (match (query) with
| [] -> begin
((NSTAll), (NSTAll), ([]))
end
| (id1)::[] -> begin
...
let acc_with_recursive_bindings = (FStar_List.fold_left (fun acc1 uu____3827 -> (match (uu____3827) with
| (path_el, trie) -> begin
...
in (FStar_List.rev_map_onto (fun uu____3872 -> (match (uu____3872) with
| (path_el, v1) -> begin
...
let uu___41_4126 = tbl
in (

let uu____4127 = (trie_insert tbl.tbl_lids host_query id1 c)
in {tbl_lids = uu____4127; tbl_mods = uu___41_4126.tbl_mods})))


let register_alias : table  ->  Prims.string  ->  query  ->  query  ->  table = (fun tbl key host_query included_query -> (

let uu___42_4150 = tbl
in (

let uu____4151 = (trie_add_alias tbl.tbl_lids key host_query included_query)
in {tbl_lids = uu____4151; tbl_mods = uu___42_4150.tbl_mods})))


let register_include : table  ->  query  ->  query  ->  table = (fun tbl host_query included_query -> (

let uu___43_4169 = tbl
in (

let uu____4170 = (trie_include tbl.tbl_lids host_query included_query)
in {tbl_lids = uu____4170; tbl_mods = uu___43_4169.tbl_mods})))
...
let uu___44_4194 = tbl
in (

let uu____4195 = (trie_open_namespace tbl.tbl_lids host_query included_query)
in {tbl_lids = uu____4195; tbl_mods = uu___44_4194.tbl_mods}))
end))
...
let uu____4254 = (names_find_exact bindings id1)
in ((uu____4254), (loaded1)))
in (match (uu____4247) with
| (FStar_Pervasives_Native.None, uu____4263) -> begin
(names_insert bindings id1 (Namespace ({ns_name = full_name; ns_loaded = loaded1})))
end
| (FStar_Pervasives_Native.Some (Namespace ({ns_name = uu____4268; ns_loaded = false})), true) -> begin
(names_insert bindings id1 (Namespace ({ns_name = full_name; ns_loaded = loaded1})))
end
| (FStar_Pervasives_Native.Some (uu____4273), uu____4274) -> begin
bindings
end)))
in (
...
let uu___45_4373 = tbl
in (

let uu____4374 = (trie_mutate tbl.tbl_mods mod_query [] (fun tr id1 q revq namespaces -> (

let uu___46_4396 = tr
in (

let uu____4399 = (ins id1 q revq tr.bindings loaded)
in {bindings = uu____4399; namespaces = namespaces}))) (fun tr uu____4410 -> tr))
in {tbl_lids = uu___45_4373.tbl_lids; tbl_mods = uu____4374})))))))


let string_of_path : path  ->  Prims.string = (fun path -> (

let uu____4418 = (FStar_List.map (fun el -> el.segment.completion) path)
in (FStar_String.concat "." uu____4418)))


let match_length_of_path : path  ->  Prims.int = (fun path -> (

let uu____4428 = (FStar_List.fold_left (fun acc elem -> (

let uu____4462 = acc
in (match (uu____4462) with
| (acc_len, uu____4480) -> begin
...
| ({imports = imports; segment = uu____4536})::uu____4537 -> begin
...
let completion_result_of_lid : 'Auu____4640 . (path * 'Auu____4640)  ->  completion_result = (fun uu____4649 -> (match (uu____4649) with
| (path, _lid) -> begin
(

let uu____4656 = (match_length_of_path path)
in (

let uu____4657 = (string_of_path path)
in (

let uu____4658 = (

let uu____4659 = (first_import_of_path path)
in (FStar_Util.dflt "" uu____4659))
in {completion_match_length = uu____4656; completion_candidate = uu____4657; completion_annotation = uu____4658})))
end))


let completion_result_of_mod : Prims.string  ->  Prims.bool  ->  path  ->  completion_result = (fun annot loaded path -> (

let uu____4677 = (match_length_of_path path)
in (

let uu____4678 = (string_of_path path)
in (

let uu____4679 = (FStar_Util.format1 (match (loaded) with
| true -> begin
" %s "
end
| uu____4680 -> begin
...
in {completion_match_length = uu____4677; completion_candidate = uu____4678; completion_annotation = uu____4679}))))


let completion_result_of_ns_or_mod : (path * mod_symbol)  ->  completion_result = (fun uu____4689 -> (match (uu____4689) with
| (path, symb) -> begin
(match (symb) with
| Module ({mod_name = uu____4696; mod_path = uu____4697; mod_loaded = loaded}) -> begin
(completion_result_of_mod "mod" loaded path)
end
| Namespace ({ns_name = uu____4699; ns_loaded = loaded}) -> begin
...
let uu____4725 = (trie_find_prefix tbl.tbl_lids query)
in (FStar_List.map completion_result_of_lid uu____4725)))
...
let uu____4785 = (trie_find_prefix tbl.tbl_mods query)
in (FStar_All.pipe_right uu____4785 (FStar_List.filter_map filter1)))
in (FStar_All.pipe_right uu____4778 (FStar_List.map completion_result_of_ns_or_mod))))
