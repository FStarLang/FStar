        ...
        (fun tr1  ->
           fun uu____2897  ->
             fun uu____2898  ->
               fun uu____2899  ->
                 fun namespaces  ->
                   let uu___91_2908 = tr1  in
                   { bindings = (uu___91_2908.bindings); namespaces })
      ...
      fun id1  ->
        fun v1  ->
          trie_mutate_leaf tr ns_query
            (fun tr1  ->
               fun uu____2955  ->
                 let uu___92_2958 = tr1  in
                 let uu____2961 = names_insert tr1.bindings id1 v1  in
                 {
                   bindings = uu____2961;
                   namespaces = (uu___92_2958.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___93_3088 = tr1  in
                 {
                   bindings = ((ImportedNames (label, (inc.bindings))) ::
                     (tr1.bindings));
                   namespaces = (uu___93_3088.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___94_3138 = tr1  in
                 {
                   bindings = (uu___94_3138.bindings);
...
let (insert : table -> query -> Prims.string -> lid_symbol -> table) =
  fun tbl  ->
    fun host_query  ->
      fun id1  ->
        fun c  ->
          let uu___95_4097 = tbl  in
          let uu____4098 = trie_insert tbl.tbl_lids host_query id1 c  in
          { tbl_lids = uu____4098; tbl_mods = (uu___95_4097.tbl_mods) }
      ...
      fun host_query  ->
        fun included_query  ->
          ...
          then register_include tbl host_query included_query
            ...
            (let uu___98_4165 = tbl  in
             let uu____4166 =
               trie_open_namespace tbl.tbl_lids host_query included_query  in
             { tbl_lids = uu____4166; tbl_mods = (uu___98_4165.tbl_mods) })
