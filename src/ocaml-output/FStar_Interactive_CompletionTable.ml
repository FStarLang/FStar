...
let rec trie_descend_exact :
  ...
  fun tr  ->
    fun query  ->
      ...
      | ns::query1 ->
          let uu____2453 = names_find_exact tr.namespaces ns  in
...
let rec trie_find_exact :
  ...
  fun tr  ->
    fun query  ->
      ...
      | ns::query1 ->
          let uu____2499 = names_find_exact tr.namespaces ns  in
          FStar_Util.bind_opt uu____2499
    ...
    fun id1  ->
      fun v1  ->
        let uu____2546 =
          ...
          | (Names bt)::tl1 -> (bt, tl1)
          | uu____2584 -> (StrEmpty, name_collections)  in
        match uu____2546 with
        | (bt,name_collections1) ->
            let uu____2609 =
              let uu____2612 = btree_insert_replace bt id1 v1  in
              Names uu____2612  in
            uu____2609 :: name_collections1
      ...
      fun q  ->
        fun rev_acc1  ->
          fun mut_node  ->
            fun mut_leaf  ->
              let trie =
                let uu____2814 = names_find_exact namespaces ns  in
                FStar_Util.dflt (trie_empty ()) uu____2814  in
              let uu____2825 = trie_mutate trie q rev_acc1 mut_node mut_leaf
                 in
              names_insert namespaces ns uu____2825
        ...
        (fun tr1  ->
           fun uu____2922  ->
             fun uu____2923  ->
               fun uu____2924  ->
                 fun namespaces  ->
                   let uu___37_2933 = tr1  in
                   { bindings = (uu___37_2933.bindings); namespaces })
      ...
      fun id1  ->
        fun v1  ->
          trie_mutate_leaf tr ns_query
            (fun tr1  ->
               fun uu____2980  ->
                 let uu___38_2983 = tr1  in
                 let uu____2986 = names_insert tr1.bindings id1 v1  in
                 {
                   bindings = uu____2986;
                   namespaces = (uu___38_2983.namespaces)
                 })
    ...
    fun host_query  ->
      fun included_query  ->
        fun mutator  ->
          ...
          let included_trie =
            let uu____3059 = trie_descend_exact tr included_query  in
            FStar_Util.dflt (trie_empty ()) uu____3059  in
          trie_mutate_leaf tr host_query
            (fun tr1  -> fun uu____3069  -> mutator tr1 included_trie label)
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___39_3113 = tr1  in
                 {
                   bindings = ((ImportedNames (label, (inc.bindings))) ::
                     (tr1.bindings));
                   namespaces = (uu___39_3113.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___40_3163 = tr1  in
                 {
                   bindings = (uu___40_3163.bindings);
    ...
    fun name_collections  ->
      let rec aux acc imports name_collections1 =
        FStar_List.fold_left
          (fun acc1  ->
             fun uu___36_3359  ->
               match uu___36_3359 with
               | Names bt ->
                   let uu____3381 =
                     let uu____3388 = fn bt  in (imports, uu____3388)  in
                   uu____3381 :: acc1
...
let (uu___is_NSTAll : name_search_term -> Prims.bool) =
  fun projectee  ->
    match projectee with | NSTAll  -> true | uu____3484 -> false
  
let (uu___is_NSTNone : name_search_term -> Prims.bool) =
  fun projectee  ->
    match projectee with | NSTNone  -> true | uu____3490 -> false
  
let (uu___is_NSTPrefix : name_search_term -> Prims.bool) =
  fun projectee  ->
    match projectee with | NSTPrefix _0 -> true | uu____3497 -> false
    ...
    fun id1  ->
      ...
      merge_increasing_lists_rev
        (fun uu____3701  ->
           ...
           | (path_el,uu____3707) -> (path_el.segment).completion)
...
let (uu___is_Module : mod_symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Module _0 -> true | uu____4002 -> false
  
let (__proj__Module__item___0 : mod_symbol -> mod_info) =
  fun projectee  -> match projectee with | Module _0 -> _0 
let (uu___is_Namespace : mod_symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Namespace _0 -> true | uu____4016 -> false
...
let (uu___is_ModOrNs : symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | ModOrNs _0 -> true | uu____4040 -> false
...
let (uu___is_Lid : symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | Lid _0 -> true | uu____4054 -> false
...
let (insert : table -> query -> Prims.string -> lid_symbol -> table) =
  fun tbl  ->
    fun host_query  ->
      fun id1  ->
        fun c  ->
          let uu___41_4126 = tbl  in
          let uu____4127 = trie_insert tbl.tbl_lids host_query id1 c  in
          { tbl_lids = uu____4127; tbl_mods = (uu___41_4126.tbl_mods) }
      ...
      fun host_query  ->
        fun included_query  ->
          ...
          then register_include tbl host_query included_query
            ...
            (let uu___44_4194 = tbl  in
             let uu____4195 =
               trie_open_namespace tbl.tbl_lids host_query included_query  in
             { tbl_lids = uu____4195; tbl_mods = (uu___44_4194.tbl_mods) })
      ...
      fun path  ->
        fun mod_query  ->
          let ins_ns id1 bindings full_name loaded1 =
            let uu____4247 =
              let uu____4254 = names_find_exact bindings id1  in
              (uu____4254, loaded1)  in
            match uu____4247 with
            | (FStar_Pervasives_Native.None ,uu____4263) ->
                names_insert bindings id1
                  (Namespace { ns_name = full_name; ns_loaded = loaded1 })
            | (FStar_Pervasives_Native.Some (Namespace
               { ns_name = uu____4268; ns_loaded = false ;_}),true ) ->
                names_insert bindings id1
                  (Namespace { ns_name = full_name; ns_loaded = loaded1 })
            | (FStar_Pervasives_Native.Some uu____4273,uu____4274) ->
                bindings
             in
          ...
          let ins id1 q revq bindings loaded1 =
            ...
            | uu____4367 -> ins_ns id1 bindings name loaded1  in
          let uu___45_4373 = tbl  in
          let uu____4374 =
              ...
              (fun tr  ->
                 fun id1  ->
                   fun q  ->
                     fun revq  ->
                       fun namespaces  ->
                         let uu___46_4396 = tr  in
                         let uu____4399 = ins id1 q revq tr.bindings loaded
                            in
                         { bindings = uu____4399; namespaces })
              (fun tr  -> fun uu____4410  -> tr)
             in
          { tbl_lids = (uu___45_4373.tbl_lids); tbl_mods = uu____4374 }
  
let (string_of_path : path -> Prims.string) =
  fun path  ->
    let uu____4418 = FStar_List.map (fun el  -> (el.segment).completion) path
       in
    FStar_String.concat "." uu____4418
...
let (first_import_of_path :
  ...
  fun path  ->
    ...
    | { imports; segment = uu____4536;_}::uu____4537 ->
...
let completion_result_of_lid :
  'Auu____4640 .
    (path,'Auu____4640) FStar_Pervasives_Native.tuple2 -> completion_result
  =
  fun uu____4649  ->
    match uu____4649 with
    | (path,_lid) ->
        let uu____4656 = match_length_of_path path  in
        let uu____4657 = string_of_path path  in
        let uu____4658 =
          let uu____4659 = first_import_of_path path  in
          FStar_Util.dflt "" uu____4659  in
        {
          completion_match_length = uu____4656;
          completion_candidate = uu____4657;
          completion_annotation = uu____4658
        }
...
let (completion_result_of_mod :
  ...
  fun annot  ->
    fun loaded  ->
      fun path  ->
        let uu____4677 = match_length_of_path path  in
        let uu____4678 = string_of_path path  in
        let uu____4679 =
          FStar_Util.format1 (if loaded then " %s " else "(%s)") annot  in
        {
          completion_match_length = uu____4677;
          completion_candidate = uu____4678;
          completion_annotation = uu____4679
        }
...
let (completion_result_of_ns_or_mod :
  ...
  fun uu____4689  ->
    ...
    | (path,symb) ->
        (match symb with
         | Module
             { mod_name = uu____4696; mod_path = uu____4697;
               mod_loaded = loaded;_}
             -> completion_result_of_mod "mod" loaded path
         | Namespace { ns_name = uu____4699; ns_loaded = loaded;_} ->
...
let (autocomplete_lid : table -> query -> completion_result Prims.list) =
  fun tbl  ->
    fun query  ->
      let uu____4725 = trie_find_prefix tbl.tbl_lids query  in
      FStar_List.map completion_result_of_lid uu____4725
...
let (autocomplete_mod_or_ns :
  ...
  fun tbl  ->
    fun query  ->
      fun filter1  ->
        let uu____4778 =
          let uu____4785 = trie_find_prefix tbl.tbl_mods query  in
          FStar_All.pipe_right uu____4785 (FStar_List.filter_map filter1)  in
