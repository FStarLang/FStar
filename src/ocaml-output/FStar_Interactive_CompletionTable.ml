      ...
      let cmp v1 v2 =
        ...
        | ((pr1, h1::uu____683), (pr2, h2::uu____686)) ->
            let cmp_h =
              ...
              let uu____709 = key_fn h2 in string_compare uu____708 uu____709 in
  ...
  'Auu____1132 .
    'Auu____1132 btree ->
      (Prims.string, 'Auu____1132) FStar_Pervasives_Native.tuple2 Prims.list
        ->
        (Prims.string, 'Auu____1132) FStar_Pervasives_Native.tuple2
          Prims.list
  =
  fun btree ->
    fun acc ->
...
let rec btree_from_list :
  'Auu____1202 .
    (Prims.string, 'Auu____1202) FStar_Pervasives_Native.tuple2 Prims.list ->
      Prims.int ->
        ('Auu____1202 btree,
          (Prims.string, 'Auu____1202) FStar_Pervasives_Native.tuple2
            Prims.list)
    ...
    fun ns ->
      let uu____2272 =
        ...
        | (ImportedNames (uu____2337, names1))::more_names ->
            ...
            (uu____2354, (FStar_Pervasives_Native.Some more_names)) in
        ...
        fun rev_acc1 ->
          fun mut_node ->
            fun mut_leaf ->
              ...
              let uu____2833 = trie_mutate trie q rev_acc1 mut_node mut_leaf in
        ...
        (fun tr1 ->
           fun uu____2930 ->
             fun uu____2931 ->
               fun uu____2932 ->
                 fun namespaces ->
                   let uu___40_2941 = tr1 in
                   { bindings = (uu___40_2941.bindings); namespaces })
      ...
      fun id1 ->
        fun v1 ->
          trie_mutate_leaf tr ns_query
            (fun tr1 ->
               fun uu____2988 ->
                 let uu___41_2991 = tr1 in
                 let uu____2994 = names_insert tr1.bindings id1 v1 in
                 {
                   bindings = uu____2994;
                   namespaces = (uu___41_2991.namespaces)
                 })
let trie_import :
  'a .
    'a trie ->
      ...
      fun included_query ->
        trie_import tr host_query included_query
          (fun tr1 ->
             fun inc ->
               fun label ->
                 let uu___42_3121 = tr1 in
                 {
                   bindings = ((ImportedNames (label, (inc.bindings))) ::
                     (tr1.bindings));
                   namespaces = (uu___42_3121.namespaces)
                 })
      ...
      fun included_query ->
        trie_import tr host_query included_query
          (fun tr1 ->
             fun inc ->
               fun label ->
                 let uu___43_3169 = tr1 in
                 {
                   bindings = (uu___43_3169.bindings);
...
let (insert : table -> query -> Prims.string -> lid_symbol -> table) =
  fun tbl ->
    fun host_query ->
      fun id1 ->
        fun c ->
          let uu___44_4139 = tbl in
          let uu____4140 = trie_insert tbl.tbl_lids host_query id1 c in
          { tbl_lids = uu____4140; tbl_mods = (uu___44_4139.tbl_mods) }
let (register_alias : table -> Prims.string -> query -> query -> table) =
  fun tbl ->
    fun key ->
      fun host_query ->
        fun included_query ->
          let uu___45_4163 = tbl in
          let uu____4164 =
            trie_add_alias tbl.tbl_lids key host_query included_query in
          { tbl_lids = uu____4164; tbl_mods = (uu___45_4163.tbl_mods) }
let (register_include : table -> query -> query -> table) =
  fun tbl ->
    fun host_query ->
      fun included_query ->
        let uu___46_4182 = tbl in
        let uu____4183 = trie_include tbl.tbl_lids host_query included_query in
        { tbl_lids = uu____4183; tbl_mods = (uu___46_4182.tbl_mods) }
      ...
      fun host_query ->
        fun included_query ->
          ...
          then register_include tbl host_query included_query
            ...
            (let uu___47_4207 = tbl in
             let uu____4208 =
               trie_open_namespace tbl.tbl_lids host_query included_query in
             { tbl_lids = uu____4208; tbl_mods = (uu___47_4207.tbl_mods) })
              ...
              (fun tr ->
                 fun id1 ->
                   fun q ->
                     fun revq ->
                       fun namespaces ->
                         let uu___49_4409 = tr in
                         let uu____4412 = ins id1 q revq tr.bindings loaded in
                         { bindings = uu____4412; namespaces })
              (fun tr -> fun uu____4423 -> tr) in
          { tbl_lids = (uu___48_4386.tbl_lids); tbl_mods = uu____4387 }
let (string_of_path : path -> Prims.string) =
  fun path ->
    let uu____4431 = FStar_List.map (fun el -> (el.segment).completion) path in
