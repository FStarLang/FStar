        ...
        (fun tr1  ->
           fun uu____2921  ->
             fun uu____2922  ->
               fun uu____2923  ->
                 fun namespaces  ->
                   let uu___63_2932 = tr1  in
                   { bindings = (uu___63_2932.bindings); namespaces })
      ...
      fun id1  ->
        fun v1  ->
          trie_mutate_leaf tr ns_query
            (fun tr1  ->
               fun uu____2979  ->
                 let uu___64_2982 = tr1  in
                 let uu____2985 = names_insert tr1.bindings id1 v1  in
                 {
                   bindings = uu____2985;
                   namespaces = (uu___64_2982.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___65_3112 = tr1  in
                 {
                   bindings = ((ImportedNames (label, (inc.bindings))) ::
                     (tr1.bindings));
                   namespaces = (uu___65_3112.namespaces)
                 })
      ...
      fun included_query  ->
        trie_import tr host_query included_query
          (fun tr1  ->
             fun inc  ->
               fun label  ->
                 let uu___66_3162 = tr1  in
                 {
                   bindings = (uu___66_3162.bindings);
...
let (insert : table -> query -> Prims.string -> lid_symbol -> table) =
  fun tbl  ->
    fun host_query  ->
      fun id1  ->
        fun c  ->
          let uu___67_4125 = tbl  in
          let uu____4126 = trie_insert tbl.tbl_lids host_query id1 c  in
          { tbl_lids = uu____4126; tbl_mods = (uu___67_4125.tbl_mods) }
      ...
      fun host_query  ->
        fun included_query  ->
          ...
          then register_include tbl host_query included_query
            ...
            (let uu___70_4193 = tbl  in
             let uu____4194 =
               trie_open_namespace tbl.tbl_lids host_query included_query  in
             { tbl_lids = uu____4194; tbl_mods = (uu___70_4193.tbl_mods) })
