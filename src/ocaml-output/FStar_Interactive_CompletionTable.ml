...
let uu___is_EmptyHeap : 'a . 'a heap -> Prims.bool =
  fun projectee  ->
    match projectee with | EmptyHeap  -> true | uu___ -> false
    ...
    fun h1  ->
      fun h2  ->
        ...
        | (Heap (v1,hh1),Heap (v2,hh2)) ->
            let uu___ = let uu___1 = cmp v1 v2  in uu___1 < Prims.int_zero
               in
...
let heap_pop :
  ...
  fun cmp  ->
    fun uu___  ->
      ...
      | EmptyHeap  -> FStar_Pervasives_Native.None
      | Heap (v,hh) ->
          let uu___1 = let uu___2 = heap_merge_pairs cmp hh  in (v, uu___2)
             in
    ...
    fun lists  ->
      ...
      let rec aux lists1 acc =
        ...
        | FStar_Pervasives_Native.Some ((pr,v::tl),lists2) ->
            ...
            let uu___2 = push_nodup key_fn v acc  in aux uu___1 uu___2
         in
  ...
  fun btree1  ->
    fun acc  ->
      ...
      | StrBranch (key,value,lbt,rbt) ->
          let uu___ =
            let uu___1 = btree_to_list_rev lbt acc  in (key, value) :: uu___1
             in
    ...
    fun prefix  ->
      let rec aux bt1 prefix1 acc =
        ...
        | StrBranch (k,v,lbt,rbt) ->
            ...
            let matches = if explore_right then aux rbt prefix1 acc else acc
               in
            let matches1 =
              if include_middle
              then
  ...
  fun bt  ->
    fun f  ->
      fun acc  ->
        ...
        | StrEmpty  -> acc
        | StrBranch (k,v,lbt,rbt) ->
            let uu___ = let uu___1 = btree_fold rbt f acc  in f k v uu___1
               in
    ...
    fun ns  ->
      let uu___ =
        ...
        | (ImportedNames (uu___1,names2))::more_names ->
            ...
            (uu___2, (FStar_Pervasives_Native.Some more_names))
         in
      match uu___ with
      | (result,names2) ->
                     ...
                     fun uu___1  ->
                       ...
                       | (path_el,trie1) ->
                           trie_find_prefix' trie1 (path_el :: path_acc)
                             query2 acc1) acc matching_namespaces_rev
                 in
...
let (uu___is_ModOrNs : symbol -> Prims.bool) =
  fun projectee  ->
    match projectee with | ModOrNs _0 -> true | uu___ -> false
      ...
      fun path1  ->
        fun mod_query  ->
          let ins_ns id bindings full_name loaded1 =
            let uu___ =
              let uu___1 = names_find_exact bindings id  in (uu___1, loaded1)
               in
            match uu___ with
           ...
           fun elem  ->
             ...
             | (acc_len,uu___2) ->
                 (match (elem.segment).prefix with
                  ...
                  | FStar_Pervasives_Native.None  -> acc))
        (Prims.int_zero, ("", Prims.int_zero)) path1
       in
    match uu___ with
