...
let uu___104_297 = r1
in {FStar_Range.def_range = rng.FStar_Range.def_range; FStar_Range.use_range = uu___104_297.FStar_Range.use_range})
end)
...
in ((uu____1005), (FStar_SMTEncoding_Util.mkTrue)))
in (FStar_SMTEncoding_Util.mkEq uu____1002))
in ((uu____1001), (Some ("Disabling label")), ((Prims.strcat "disable_label_" (Prims.fst l)))))
...
let uu____1039 = (FStar_List.map (fun x -> ((x), (true))) eliminated)
in (

let uu____1046 = (FStar_List.map (fun x -> ((x), (false))) errors)
in (FStar_List.append uu____1039 uu____1046)))
in (sort_labels results))
end
| (hd1)::tl1 -> begin
((

let uu____1059 = (FStar_Util.string_of_int (FStar_List.length active))
in (FStar_Util.print1 "%s, " uu____1059));
(FStar_SMTEncoding_Z3.refresh ());
(

let uu____1064 = (

let uu____1071 = (FStar_All.pipe_left elim (FStar_List.append eliminated (FStar_List.append errors tl1)))
in (askZ3 uu____1071))
in (match (uu____1064) with
| (result, uu____1086) -> begin
(

let uu____1095 = (FStar_Util.is_left result)
