...
let uu____560 = (FStar_Range.def_range rng)
in (FStar_Range.set_def_range r1 uu____560))
end)
...
in (FStar_All.try_with (fun uu___142_659 -> (match (()) with
| () -> begin
...
end)) (fun uu___141_1013 -> (match (uu___141_1013) with
...
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvAdd, uu____1791) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvSub, uu____1802) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvShl, uu____1813) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvShr, uu____1824) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvUdiv, uu____1835) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvMod, uu____1846) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvMul, uu____1857) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvUext (uu____1868), uu____1869) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.BvToNat, uu____1880) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.NatToBv (uu____1891), uu____1892) -> begin
(failwith "Impossible: non-propositional term")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.ITE, uu____1903) -> begin
(failwith "Impossible: arity mismatch")
end
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.Imp, uu____1914) -> begin
(failwith "Impossible: arity mismatch")
end
| FStar_SMTEncoding_Term.Quant (FStar_SMTEncoding_Term.Forall, pats, iopt, sorts, body) -> begin
(

let uu____1945 = (aux default_msg ropt post_name_opt labels body)
in (match (uu____1945) with
| (labels1, body1) -> begin
(

let uu____1964 = (FStar_SMTEncoding_Term.mk (FStar_SMTEncoding_Term.Quant (((FStar_SMTEncoding_Term.Forall), (pats), (iopt), (sorts), (body1)))) q1.FStar_SMTEncoding_Term.rng)
in ((labels1), (uu____1964)))
end))
end
| FStar_SMTEncoding_Term.Let (es, body) -> begin
(

let uu____1981 = (aux default_msg ropt post_name_opt labels body)
in (match (uu____1981) with
| (labels1, body1) -> begin
(

let uu____2000 = (FStar_SMTEncoding_Term.mkLet ((es), (body1)) q1.FStar_SMTEncoding_Term.rng)
in ((labels1), (uu____2000)))
end))
end))
...
let print_banner = (fun uu____2029 -> (

let msg = (

let uu____2031 = (

let uu____2032 = (FStar_TypeChecker_Env.get_range env)
in (FStar_Range.string_of_range uu____2032))
in (

let uu____2033 = (FStar_Util.string_of_int (Prims.parse_int "5"))
in (

let uu____2034 = (FStar_Util.string_of_int (FStar_List.length all_labels))
...
| uu____2035 -> begin
"error"
end) uu____2031 uu____2033 uu____2034))))
in (FStar_Util.print_error msg)))
in (

let print_result = (fun uu____2049 -> (match (uu____2049) with
| ((uu____2060, msg, r), success) -> begin
(match (success) with
| true -> begin
(

let uu____2070 = (FStar_Range.string_of_range r)
...
let uu____2074 = (FStar_Range.string_of_range r)
...
let elim = (fun labs -> (FStar_All.pipe_right labs (FStar_List.map (fun uu____2134 -> (match (uu____2134) with
| (l, uu____2146, uu____2147) -> begin
(
...
let uu____2163 = (FStar_SMTEncoding_Util.mkFreeV l)
in ((uu____2163), (FStar_SMTEncoding_Util.mkTrue)))
in (FStar_SMTEncoding_Util.mkEq uu____2158))
in {FStar_SMTEncoding_Term.assumption_term = uu____2157; FStar_SMTEncoding_Term.assumption_caption = FStar_Pervasives_Native.Some ("Disabling label"); FStar_SMTEncoding_Term.assumption_name = (Prims.strcat "@disable_label_" (FStar_Pervasives_Native.fst l)); FStar_SMTEncoding_Term.assumption_fact_ids = []})
...
let uu____2218 = (FStar_List.map (fun x -> ((x), (true))) eliminated)
in (

let uu____2231 = (FStar_List.map (fun x -> ((x), (false))) errors)
in (FStar_List.append uu____2218 uu____2231)))
in (sort_labels results))
end
| (hd1)::tl1 -> begin
((

let uu____2253 = (FStar_Util.string_of_int (FStar_List.length active))
in (FStar_Util.print1 "%s, " uu____2253));
(
...
| FStar_SMTEncoding_Z3.UNSAT (uu____2284) -> begin
