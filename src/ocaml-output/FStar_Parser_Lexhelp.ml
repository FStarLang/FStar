...
let stringKeywords : Prims.string Prims.list = (FStar_List.map (fun uu____425 -> (match (uu____425) with
| (uu____429, w, uu____431) -> begin
w
end)) keywords)


let unreserve_words : Prims.string Prims.list = (FStar_List.choose (fun uu____436 -> (match (uu____436) with
| (mode, keyword, uu____443) -> begin
...
in ((FStar_List.iter (fun uu____453 -> (match (uu____453) with
...
let mkLexargs : ((Prims.unit  ->  Prims.string) * Prims.string * Prims.string)  ->  lexargs = (fun uu____500 -> (match (uu____500) with
...
let uu____526 = (args.getSourceDirectory ())
in (FStar_Bytes.string_as_unicode_bytes uu____526))
in FStar_Parser_Parse.STRING (uu____525))
end
...
let uu____528 = (FStar_Range.file_of_range r)
in (FStar_Bytes.string_as_unicode_bytes uu____528))
in FStar_Parser_Parse.STRING (uu____527))
end
| "__LINE__" -> begin
(
...
let uu____534 = (FStar_Range.start_of_range r)
in (FStar_Range.line_of_pos uu____534))
in (FStar_All.pipe_left FStar_Util.string_of_int uu____533))
in ((uu____532), (false)))
in FStar_Parser_Parse.INT (uu____529))
end
| uu____535 -> begin
...
let uu____537 = (intern_string s)
in FStar_Parser_Parse.IDENT (uu____537))
end)
end)
end)))
