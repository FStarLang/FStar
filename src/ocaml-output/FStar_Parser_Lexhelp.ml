...
let mkLexargs : ((Prims.unit  ->  Prims.string) * Prims.string * Prims.string)  ->  lexargs = (fun uu____507 -> (match (uu____507) with
...
in (FStar_Bytes.string_as_unicode_bytes uu____533))
in FStar_Parser_Parse.STRING (uu____532))
end
...
let uu____535 = (FStar_Range.file_of_range r)
...
let uu____541 = (FStar_Range.start_of_range r)
in (FStar_Range.line_of_pos uu____541))
in (FStar_All.pipe_left FStar_Util.string_of_int uu____540))
in ((uu____539), (false)))
in FStar_Parser_Parse.INT (uu____536))
end
| uu____542 -> begin
...
let uu____544 = (intern_string s)
in FStar_Parser_Parse.IDENT (uu____544))
end)
end)
end)))
