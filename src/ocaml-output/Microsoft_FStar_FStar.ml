...
let process_args = (fun _56_1 -> (match (_56_1) with
| () -> begin
...
let cleanup = (fun _56_10 -> (match (_56_10) with
| () -> begin
...
let tc_prims = (fun _56_12 -> (match (_56_12) with
| () -> begin
...
in (let _56_15 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.init env)
in (let p = (Microsoft_FStar_Options.prims ())
in (let _56_20 = (Microsoft_FStar_Parser_Driver.parse_file (Microsoft_FStar_Parser_DesugarEnv.empty_env ()) p)
in (match (_56_20) with
| (dsenv, prims_mod) -> begin
(let _56_23 = (Microsoft_FStar_Tc_Tc.check_module env (Support.List.hd prims_mod))
in (match (_56_23) with
| (prims_mod, env) -> begin
(prims_mod, dsenv, env)
end))
end))))))
...
(let _56_29 = (Support.Microsoft.FStar.Util.fprint1 "Error: %s errors were reported (see above)\n" (Support.Microsoft.FStar.Util.string_of_int errs))
in (exit (1)))
end))

let tc_one_file = (fun dsenv env fn -> (let _56_36 = (Microsoft_FStar_Parser_Driver.parse_file dsenv fn)
in (match (_56_36) with
| (dsenv, fmods) -> begin
(let _56_46 = ((Support.List.fold_left (fun _56_39 m -> (match (_56_39) with
| (env, all_mods) -> begin
(let _56_43 = (Microsoft_FStar_Tc_Tc.check_module env m)
in (match (_56_43) with
| (ms, env) -> begin
(env, ms)
end))
end)) (env, [])) fmods)
in (match (_56_46) with
| (env, all_mods) -> begin
(dsenv, env, (Support.List.rev all_mods))
end))
end)))

let tc_one_fragment = (fun curmod dsenv env frag -> (Support.Prims.try_with (fun _56_52 -> (match (_56_52) with
| () -> begin
...
in (let _56_79 = (Microsoft_FStar_Tc_Tc.tc_partial_modul env modul)
...
(let _56_92 = (Microsoft_FStar_Tc_Tc.tc_more_partial_modul env modul decls)
in (match (_56_92) with
| (modul, npds', env) -> begin
Some ((Some ((modul, (Support.List.append npds npds'))), dsenv, env))
end))
end)
end)
end)) (fun _56_51 -> (match (_56_51) with
| Microsoft_FStar_Absyn_Syntax.Error ((msg, r)) -> begin
(let _56_58 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, r))::[]))
in None)
end
| Microsoft_FStar_Absyn_Syntax.Err (msg) -> begin
(let _56_62 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, Microsoft_FStar_Absyn_Syntax.dummyRange))::[]))
in None)
end
| e -> begin
(raise (e))
...
in (fun line -> (let _56_104 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (Support.Microsoft.FStar.Util.flush_file transcript))))
end else begin
(fun line -> ())
end
in (let _56_110 = if (Support.Option.isSome (! (Microsoft_FStar_Options.codegen))) then begin
(let _56_108 = (Support.Microsoft.FStar.Util.print_string "Code-generation is not supported in interactive mode")
in (exit (1)))
end
in (let chunk = (Support.Microsoft.FStar.Util.new_string_builder ())
in (let stdin = (Support.Microsoft.FStar.Util.open_stdin ())
in (let rec fill_chunk = (fun _56_115 -> (match (_56_115) with
| () -> begin
...
in (let _56_132 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#pop") then begin
(let _56_134 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#push") then begin
(let _56_136 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end else begin
if (l = "#finish") then begin
(exit (0))
end else begin
(let _56_138 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _56_140 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end
end
end
...
| Pop (msg) -> begin
(let _56_149 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Env.pop env msg))
in (let _56_151 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.refresh ())
in (let _56_153 = ((Support.Prims.ignore) (Microsoft_FStar_Options.reset_options ()))
in (let _56_164 = (match (stack) with
| [] -> begin
...
in (dsenv, env))))
in (let fail = (fun curmod dsenv_mark env_mark -> (let _56_195 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
in (let _56_197 = (Support.ST.op_Colon_Equals Microsoft_FStar_Tc_Errors.num_errs 0)
in (let _56_199 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
in (let _56_203 = (reset_mark dsenv_mark env_mark)
in (match (_56_203) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end))))))
in (let _56_206 = (mark dsenv env)
...
(let _56_213 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
in (let _56_217 = (commit_mark dsenv env)
in (match (_56_217) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end)))
end else begin
...
let batch_mode_tc = (fun filenames -> (let _56_224 = (tc_prims ())
in (match (_56_224) with
| (prims_mod, dsenv, env) -> begin
(let _56_239 = ((Support.List.fold_left (fun _56_228 f -> (match (_56_228) with
| (all_mods, dsenv, env) -> begin
(let _56_230 = (Microsoft_FStar_Absyn_Util.reset_gensym ())
in (let _56_235 = (tc_one_file dsenv env f)
in (match (_56_235) with
| (dsenv, env, ms) -> begin
((Support.List.append all_mods ms), dsenv, env)
end)))
end)) (prims_mod, dsenv, env)) filenames)
in (match (_56_239) with
| (all_mods, dsenv, env) -> begin
(let _56_240 = if ((! (Microsoft_FStar_Options.interactive)) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0)) then begin
...
in (let _56_245 = ((Support.List.iter (fun m -> if (Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
