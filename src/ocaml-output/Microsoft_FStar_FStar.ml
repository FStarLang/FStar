...
let process_args = (fun _527821 -> (match (_527821) with
| () -> begin
...
let cleanup = (fun _527830 -> (match (_527830) with
| () -> begin
...
let tc_prims = (fun _527832 -> (match (_527832) with
| () -> begin
...
in (let _527835 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.init env)
in (let p = (Microsoft_FStar_Options.prims ())
in (let _527840 = (Microsoft_FStar_Parser_Driver.parse_file (Microsoft_FStar_Parser_DesugarEnv.empty_env ()) p)
in (match (_527840) with
| (dsenv, prims_mod) -> begin
(let _527843 = (Microsoft_FStar_Tc_Tc.check_module env (Support.List.hd prims_mod))
in (match (_527843) with
| (prims_mod, env) -> begin
(prims_mod, dsenv, env)
end))
end))))))
...
(let _527849 = (Support.Microsoft.FStar.Util.fprint1 "Error: %s errors were reported (see above)\n" (Support.Microsoft.FStar.Util.string_of_int errs))
in (exit (1)))
end))

let tc_one_file = (fun dsenv env fn -> (let _527856 = (Microsoft_FStar_Parser_Driver.parse_file dsenv fn)
in (match (_527856) with
| (dsenv, fmods) -> begin
(let _527866 = ((Support.List.fold_left (fun _527859 m -> (match (_527859) with
| (env, all_mods) -> begin
(let _527863 = (Microsoft_FStar_Tc_Tc.check_module env m)
in (match (_527863) with
| (ms, env) -> begin
(env, ms)
end))
end)) (env, [])) fmods)
in (match (_527866) with
| (env, all_mods) -> begin
(dsenv, env, (Support.List.rev all_mods))
end))
end)))

let tc_one_fragment = (fun curmod dsenv env frag -> (Support.Prims.try_with (fun _527872 -> (match (_527872) with
| () -> begin
...
in (let _527899 = (Microsoft_FStar_Tc_Tc.tc_partial_modul env modul)
in (match (_527899) with
...
(let _527912 = (Microsoft_FStar_Tc_Tc.tc_more_partial_modul env modul decls)
in (match (_527912) with
| (modul, npds', env) -> begin
Some ((Some ((modul, (Support.List.append npds npds'))), dsenv, env))
end))
end)
end)
end)) (fun _527871 -> (match (_527871) with
| Microsoft_FStar_Absyn_Syntax.Error ((msg, r)) -> begin
(let _527878 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, r))::[]))
in None)
end
| Microsoft_FStar_Absyn_Syntax.Err (msg) -> begin
(let _527882 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, Microsoft_FStar_Absyn_Syntax.dummyRange))::[]))
in None)
end
| e -> begin
(raise (e))
...
in (fun line -> (let _527924 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (Support.Microsoft.FStar.Util.flush_file transcript))))
end else begin
(fun line -> ())
end
in (let _527930 = if (Support.Option.isSome (! (Microsoft_FStar_Options.codegen))) then begin
(let _527928 = (Support.Microsoft.FStar.Util.print_string "Code-generation is not supported in interactive mode")
in (exit (1)))
end
in (let chunk = (Support.Microsoft.FStar.Util.new_string_builder ())
in (let stdin = (Support.Microsoft.FStar.Util.open_stdin ())
in (let rec fill_chunk = (fun _527935 -> (match (_527935) with
| () -> begin
...
in (let _527952 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#pop") then begin
(let _527954 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#push") then begin
(let _527956 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end else begin
if (l = "#finish") then begin
(exit (0))
end else begin
(let _527958 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _527960 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end
end
end
...
| Pop (msg) -> begin
(let _527969 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Env.pop env msg))
in (let _527971 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.refresh ())
in (let _527973 = ((Support.Prims.ignore) (Microsoft_FStar_Options.reset_options ()))
in (let _527984 = (match (stack) with
| [] -> begin
...
in (dsenv, env))))
in (let fail = (fun curmod dsenv_mark env_mark -> (let _528015 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
in (let _528017 = (Microsoft_FStar_Tc_Errors.num_errs := 0)
in (let _528019 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
in (let _528023 = (reset_mark dsenv_mark env_mark)
in (match (_528023) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end))))))
in (let _528026 = (mark dsenv env)
in (match (_528026) with
...
(let _528033 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
in (let _528037 = (commit_mark dsenv env)
in (match (_528037) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end)))
end else begin
...
let batch_mode_tc = (fun filenames -> (let _528044 = (tc_prims ())
in (match (_528044) with
| (prims_mod, dsenv, env) -> begin
(let _528057 = ((Support.List.fold_left (fun _528048 f -> (match (_528048) with
| (all_mods, dsenv, env) -> begin
(let _528053 = (tc_one_file dsenv env f)
in (match (_528053) with
| (dsenv, env, ms) -> begin
((Support.List.append all_mods ms), dsenv, env)
end))
end)) (prims_mod, dsenv, env)) filenames)
in (match (_528057) with
| (all_mods, dsenv, env) -> begin
(let _528058 = if ((! (Microsoft_FStar_Options.interactive)) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0)) then begin
...
in (let _528063 = ((Support.List.iter (fun m -> if (Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
...
(Support.Prims.try_with (fun _528067 -> (match (_528067) with
| () -> begin
...
in (Support.List.iter (fun _528081 -> (match (_528081) with
| (n, d) -> begin
...
end)) (fun _528066 -> (match (_528066) with
| Microsoft_FStar_Backends_OCaml_ASTTrans.OCamlFailure ((rg, error)) -> begin
(let _528073 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "OCaml Backend Error: %s %s\n" (Support.Microsoft.FStar.Range.string_of_range rg) (Microsoft_FStar_Backends_OCaml_ASTTrans.string_of_error error)))
in (exit (1)))
end)))
end)

let go = (fun _528082 -> (let _528086 = (process_args ())
in (match (_528086) with
| (res, filenames) -> begin
(match (res) with
...
| _ -> begin
(let _528095 = (Support.Microsoft.FStar.Util.print_string "--use_build_config expects just a single file on the command line and no other arguments")
in (exit (1)))
end)
end else begin
filenames
end
in (let _528101 = (batch_mode_tc filenames)
in (match (_528101) with
| (fmods, dsenv, env) -> begin
(let _528102 = (report_errors None)
in if (! (Microsoft_FStar_Options.interactive)) then begin
(interactive_mode dsenv env)
end else begin
(let _528104 = (codegen fmods)
...
let main = (fun _528106 -> (match (_528106) with
| () -> begin
(Support.Prims.try_with (fun _528108 -> (match (_528108) with
| () -> begin
(let _528119 = (go ())
in (let _528121 = (cleanup ())
in (exit (0))))
end)) (fun _528107 -> (match (_528107) with
| e -> begin
(let _528111 = if (Microsoft_FStar_Absyn_Util.handleable e) then begin
(Microsoft_FStar_Absyn_Util.handle_err false () e)
end
in (let _528113 = if (! (Microsoft_FStar_Options.trace_error)) then begin
