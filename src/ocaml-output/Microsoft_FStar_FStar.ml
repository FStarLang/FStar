...
let process_args = (fun _528299 -> (match (_528299) with
| () -> begin
...
let cleanup = (fun _528308 -> (match (_528308) with
| () -> begin
...
let tc_prims = (fun _528310 -> (match (_528310) with
| () -> begin
...
in (let _528313 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.init env)
in (let p = (Microsoft_FStar_Options.prims ())
in (let _528318 = (Microsoft_FStar_Parser_Driver.parse_file (Microsoft_FStar_Parser_DesugarEnv.empty_env ()) p)
in (match (_528318) with
| (dsenv, prims_mod) -> begin
(let _528321 = (Microsoft_FStar_Tc_Tc.check_module env (Support.List.hd prims_mod))
in (match (_528321) with
| (prims_mod, env) -> begin
(prims_mod, dsenv, env)
end))
end))))))
...
(let _528327 = (Support.Microsoft.FStar.Util.fprint1 "Error: %s errors were reported (see above)\n" (Support.Microsoft.FStar.Util.string_of_int errs))
in (exit (1)))
end))

let tc_one_file = (fun dsenv env fn -> (let _528334 = (Microsoft_FStar_Parser_Driver.parse_file dsenv fn)
in (match (_528334) with
| (dsenv, fmods) -> begin
(let _528344 = ((Support.List.fold_left (fun _528337 m -> (match (_528337) with
| (env, all_mods) -> begin
(let _528341 = (Microsoft_FStar_Tc_Tc.check_module env m)
in (match (_528341) with
| (ms, env) -> begin
(env, ms)
end))
end)) (env, [])) fmods)
in (match (_528344) with
| (env, all_mods) -> begin
(dsenv, env, (Support.List.rev all_mods))
end))
end)))

let tc_one_fragment = (fun curmod dsenv env frag -> (Support.Prims.try_with (fun _528350 -> (match (_528350) with
| () -> begin
...
in (let _528377 = (Microsoft_FStar_Tc_Tc.tc_partial_modul env modul)
in (match (_528377) with
...
(let _528390 = (Microsoft_FStar_Tc_Tc.tc_more_partial_modul env modul decls)
in (match (_528390) with
| (modul, npds', env) -> begin
Some ((Some ((modul, (Support.List.append npds npds'))), dsenv, env))
end))
end)
end)
end)) (fun _528349 -> (match (_528349) with
| Microsoft_FStar_Absyn_Syntax.Error ((msg, r)) -> begin
(let _528356 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, r))::[]))
in None)
end
| Microsoft_FStar_Absyn_Syntax.Err (msg) -> begin
(let _528360 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, Microsoft_FStar_Absyn_Syntax.dummyRange))::[]))
in None)
end
| e -> begin
(raise (e))
...
in (fun line -> (let _528402 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (Support.Microsoft.FStar.Util.flush_file transcript))))
end else begin
(fun line -> ())
end
in (let _528408 = if (Support.Option.isSome (! (Microsoft_FStar_Options.codegen))) then begin
(let _528406 = (Support.Microsoft.FStar.Util.print_string "Code-generation is not supported in interactive mode")
in (exit (1)))
end
in (let chunk = (Support.Microsoft.FStar.Util.new_string_builder ())
in (let stdin = (Support.Microsoft.FStar.Util.open_stdin ())
in (let rec fill_chunk = (fun _528413 -> (match (_528413) with
| () -> begin
...
in (let _528430 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#pop") then begin
(let _528432 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#push") then begin
(let _528434 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end else begin
if (l = "#finish") then begin
(exit (0))
end else begin
(let _528436 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _528438 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end
end
end
...
| Pop (msg) -> begin
(let _528447 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Env.pop env msg))
in (let _528449 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.refresh ())
in (let _528451 = ((Support.Prims.ignore) (Microsoft_FStar_Options.reset_options ()))
in (let _528462 = (match (stack) with
| [] -> begin
...
in (dsenv, env))))
in (let fail = (fun curmod dsenv_mark env_mark -> (let _528493 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
in (let _528495 = (Microsoft_FStar_Tc_Errors.num_errs := 0)
in (let _528497 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
in (let _528501 = (reset_mark dsenv_mark env_mark)
in (match (_528501) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end))))))
in (let _528504 = (mark dsenv env)
in (match (_528504) with
...
(let _528511 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
in (let _528515 = (commit_mark dsenv env)
in (match (_528515) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end)))
end else begin
...
let batch_mode_tc = (fun filenames -> (let _528522 = (tc_prims ())
in (match (_528522) with
| (prims_mod, dsenv, env) -> begin
(let _528535 = ((Support.List.fold_left (fun _528526 f -> (match (_528526) with
| (all_mods, dsenv, env) -> begin
(let _528531 = (tc_one_file dsenv env f)
in (match (_528531) with
| (dsenv, env, ms) -> begin
((Support.List.append all_mods ms), dsenv, env)
end))
end)) (prims_mod, dsenv, env)) filenames)
in (match (_528535) with
| (all_mods, dsenv, env) -> begin
(let _528536 = if ((! (Microsoft_FStar_Options.interactive)) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0)) then begin
...
in (let _528541 = ((Support.List.iter (fun m -> if (Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
...
(Support.Prims.try_with (fun _528545 -> (match (_528545) with
| () -> begin
...
in (Support.List.iter (fun _528559 -> (match (_528559) with
| (n, d) -> begin
...
end)) (fun _528544 -> (match (_528544) with
| Microsoft_FStar_Backends_OCaml_ASTTrans.OCamlFailure ((rg, error)) -> begin
(let _528551 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "OCaml Backend Error: %s %s\n" (Support.Microsoft.FStar.Range.string_of_range rg) (Microsoft_FStar_Backends_OCaml_ASTTrans.string_of_error error)))
in (exit (1)))
end)))
end)

let go = (fun _528560 -> (let _528564 = (process_args ())
in (match (_528564) with
| (res, filenames) -> begin
(match (res) with
...
| _ -> begin
(let _528573 = (Support.Microsoft.FStar.Util.print_string "--use_build_config expects just a single file on the command line and no other arguments")
in (exit (1)))
end)
end else begin
filenames
end
in (let _528579 = (batch_mode_tc filenames)
in (match (_528579) with
| (fmods, dsenv, env) -> begin
(let _528580 = (report_errors None)
in if (! (Microsoft_FStar_Options.interactive)) then begin
(interactive_mode dsenv env)
end else begin
(let _528582 = (codegen fmods)
...
let main = (fun _528584 -> (match (_528584) with
| () -> begin
(Support.Prims.try_with (fun _528586 -> (match (_528586) with
| () -> begin
(let _528597 = (go ())
in (let _528599 = (cleanup ())
in (exit (0))))
end)) (fun _528585 -> (match (_528585) with
| e -> begin
(let _528589 = if (Microsoft_FStar_Absyn_Util.handleable e) then begin
(Microsoft_FStar_Absyn_Util.handle_err false () e)
end
in (let _528591 = if (! (Microsoft_FStar_Options.trace_error)) then begin
