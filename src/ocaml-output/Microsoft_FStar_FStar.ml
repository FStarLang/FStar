...
in (Support.Microsoft.FStar.Getopt.parse_cmdline _131_6 (fun ( i ) -> (let _131_5 = (let _131_4 = (Support.ST.read file_list)
in (Support.List.append _131_4 ((i)::[])))
in (Support.ST.op_Colon_Equals file_list _131_5)))))
in (let _65_8 = (match (res) with
| Support.Microsoft.FStar.Getopt.GoOn -> begin
(let _131_7 = (Microsoft_FStar_Options.set_fstar_home ())
in (Support.Prims.ignore _131_7))
end
| _65_7 -> begin
()
end)
in (let _131_8 = (Support.ST.read file_list)
in (res, _131_8)))))
end))
...
(let _65_23 = (let _131_16 = (Support.List.hd prims_mod)
...
| true -> begin
(let _65_29 = (let _131_19 = (Support.Microsoft.FStar.Util.string_of_int errs)
...
| (dsenv, fmods) -> begin
(let _65_46 = (Support.All.pipe_right fmods (Support.List.fold_left (fun ( _65_39 ) ( m ) -> (match (_65_39) with
| (env, all_mods) -> begin
...
let tc_one_fragment = (fun ( curmod ) ( dsenv ) ( env ) ( frag ) -> (Support.All.try_with (fun ( _65_52 ) -> (match (()) with
| () -> begin
...
in (fun ( line ) -> (let _65_107 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (Support.Microsoft.FStar.Util.flush_file transcript))))
end
| false -> begin
(fun ( line ) -> ())
end)
in (let _65_111 = (match ((let _131_87 = (Support.ST.read Microsoft_FStar_Options.codegen)
in (Support.Option.isSome _131_87))) with
| true -> begin
...
in (let _65_133 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end
| false -> begin
(match ((Support.Microsoft.FStar.Util.starts_with l "#pop")) with
| true -> begin
(let _65_135 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end
| false -> begin
(match ((Support.Microsoft.FStar.Util.starts_with l "#push")) with
| true -> begin
(let _65_137 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end
| false -> begin
(match ((l = "#finish")) with
...
| false -> begin
(let _65_139 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _65_141 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end)
end)
end)
...
in (Support.All.pipe_right _131_98 Support.Prims.ignore))
...
in (Support.All.pipe_right _131_99 Support.Prims.ignore))
...
in (Support.All.pipe_right _131_118 Support.Prims.ignore))
in (let _65_198 = (Support.ST.op_Colon_Equals Microsoft_FStar_Tc_Errors.num_errs 0)
in (let _65_200 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
...
| true -> begin
(let _65_214 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
...
(let _65_240 = (Support.All.pipe_right filenames (Support.List.fold_left (fun ( _65_229 ) ( f ) -> (match (_65_229) with
...
| (all_mods, dsenv, env) -> begin
(let _65_241 = (match (((Support.ST.read Microsoft_FStar_Options.interactive) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0))) with
| true -> begin
...
in (let _65_246 = (Support.All.pipe_right fmods (Support.List.iter (fun ( m ) -> (match ((Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str)) with
| true -> begin
...
in (Support.Microsoft.FStar.Util.print_string _131_127))
end
| false -> begin
()
end))))
...
let codegen = (fun ( fmods ) ( env ) -> (match (((((Support.ST.read Microsoft_FStar_Options.codegen) = Some ("OCaml")) || ((Support.ST.read Microsoft_FStar_Options.codegen) = Some ("OCaml-experimental"))) || ((Support.ST.read Microsoft_FStar_Options.codegen) = Some ("FSharp")))) with
| true -> begin
(let _65_252 = (let _131_132 = (Microsoft_FStar_Extraction_ML_Env.mkContext env)
in (Support.Microsoft.FStar.Util.fold_map Microsoft_FStar_Extraction_ML_ExtractMod.extract _131_132 fmods))
in (match (_65_252) with
| (c, mllibs) -> begin
(let mllibs = (Support.List.flatten mllibs)
in (let _65_256 = (match (((Support.ST.read Microsoft_FStar_Options.codegen) = Some ("FSharp"))) with
| true -> begin
(let _131_133 = (Support.List.collect Microsoft_FStar_Extraction_ML_Code.doc_of_mllib mllibs)
in (_131_133, ".fs"))
end
| false -> begin
(let _131_134 = (Support.List.collect Microsoft_FStar_Extraction_OCaml_Code.doc_of_mllib mllibs)
in (_131_134, ".ml"))
end)
in (match (_65_256) with
| (newDocs, ext) -> begin
(Support.List.iter (fun ( _65_259 ) -> (match (_65_259) with
| (n, d) -> begin
(let _131_137 = (Microsoft_FStar_Options.prependOutputDir (Support.Prims.strcat n ext))
in (let _131_136 = (FSharp_Format.pretty 120 d)
in (Support.Microsoft.FStar.Util.write_file _131_137 _131_136)))
end)) newDocs)
end)))
end))
end
...
let go = (fun ( _65_260 ) -> (let _65_264 = (process_args ())
in (match (_65_264) with
| (res, filenames) -> begin
(match (res) with
| Support.Microsoft.FStar.Getopt.Help -> begin
(let _131_139 = (Microsoft_FStar_Options.specs ())
in (Microsoft_FStar_Options.display_usage _131_139))
end
...
| _65_272 -> begin
(let _65_273 = (Support.Microsoft.FStar.Util.print_string "--use_build_config expects just a single file on the command line and no other arguments")
in (Support.All.exit 1))
end)
end
| false -> begin
...
in (let _65_279 = (batch_mode_tc filenames)
in (match (_65_279) with
| (fmods, dsenv, env) -> begin
(let _65_280 = (report_errors None)
in (match ((Support.ST.read Microsoft_FStar_Options.interactive)) with
| true -> begin
(interactive_mode dsenv env)
end
| false -> begin
(let _65_282 = (codegen fmods env)
...
let main = (fun ( _65_284 ) -> (match (()) with
| () -> begin
(Support.All.try_with (fun ( _65_286 ) -> (match (()) with
| () -> begin
(let _65_297 = (go ())
in (let _65_299 = (cleanup ())
in (Support.All.exit 0)))
end)) (fun ( _65_285 ) -> (match (_65_285) with
| e -> begin
(let _65_289 = (match ((Microsoft_FStar_Absyn_Util.handleable e)) with
| true -> begin
...
in (let _65_291 = (match ((Support.ST.read Microsoft_FStar_Options.trace_error)) with
| true -> begin
(let _131_144 = (Support.Microsoft.FStar.Util.message_of_exn e)
in (let _131_143 = (Support.Microsoft.FStar.Util.trace_of_exn e)
in (Support.Microsoft.FStar.Util.fprint2 "\nUnexpected error\n%s\n%s\n" _131_144 _131_143)))
end
| false -> begin
(match ((not ((Microsoft_FStar_Absyn_Util.handleable e)))) with
| true -> begin
(let _131_145 = (Support.Microsoft.FStar.Util.message_of_exn e)
