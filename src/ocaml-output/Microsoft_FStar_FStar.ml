...
let process_args = (fun _524040 -> (match (_524040) with
| () -> begin
...
let cleanup = (fun _524049 -> (match (_524049) with
| () -> begin
...
let tc_prims = (fun _524051 -> (match (_524051) with
| () -> begin
...
in (let _524054 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.init env)
in (let p = (Microsoft_FStar_Options.prims ())
in (let _524059 = (Microsoft_FStar_Parser_Driver.parse_file (Microsoft_FStar_Parser_DesugarEnv.empty_env ()) p)
in (match (_524059) with
| (dsenv, prims_mod) -> begin
(let _524062 = (Microsoft_FStar_Tc_Tc.check_module env (Support.List.hd prims_mod))
in (match (_524062) with
| (prims_mod, env) -> begin
(prims_mod, dsenv, env)
end))
end))))))
...
(let _524068 = (Support.Microsoft.FStar.Util.fprint1 "Error: %s errors were reported (see above)\n" (Support.Microsoft.FStar.Util.string_of_int errs))
in (exit (1)))
end))

let tc_one_file = (fun dsenv env fn -> (let _524075 = (Microsoft_FStar_Parser_Driver.parse_file dsenv fn)
in (match (_524075) with
| (dsenv, fmods) -> begin
(let _524085 = ((Support.List.fold_left (fun _524078 m -> (match (_524078) with
| (env, all_mods) -> begin
(let _524082 = (Microsoft_FStar_Tc_Tc.check_module env m)
in (match (_524082) with
| (ms, env) -> begin
(env, ms)
end))
end)) (env, [])) fmods)
in (match (_524085) with
| (env, all_mods) -> begin
(dsenv, env, (Support.List.rev all_mods))
end))
end)))

let tc_one_fragment = (fun curmod dsenv env frag -> (Support.Prims.try_with (fun _524091 -> (match (_524091) with
| () -> begin
...
in (let _524118 = (Microsoft_FStar_Tc_Tc.tc_partial_modul env modul)
in (match (_524118) with
...
(let _524131 = (Microsoft_FStar_Tc_Tc.tc_more_partial_modul env modul decls)
in (match (_524131) with
| (modul, npds', env) -> begin
Some ((Some ((modul, (Support.List.append npds npds'))), dsenv, env))
end))
end)
end)
end)) (fun _524090 -> (match (_524090) with
| Microsoft_FStar_Absyn_Syntax.Error ((msg, r)) -> begin
(let _524097 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, r))::[]))
in None)
end
| Microsoft_FStar_Absyn_Syntax.Err (msg) -> begin
(let _524101 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, Microsoft_FStar_Absyn_Syntax.dummyRange))::[]))
in None)
end
| e -> begin
(raise (e))
...
let interactive_mode = (fun dsenv env -> (let _524142 = if (Support.Option.isSome (Support.ST.read Microsoft_FStar_Options.codegen)) then begin
(let _524140 = (Support.Microsoft.FStar.Util.print_string "Code-generation is not supported in interactive mode")
in (exit (1)))
end
in (let transcript = (Support.Microsoft.FStar.Util.open_file_for_writing "transcript")
in (let chunk = (Support.Microsoft.FStar.Util.new_string_builder ())
in (let stdin = (Support.Microsoft.FStar.Util.open_stdin ())
in (let rec fill_chunk = (fun _524148 -> (match (_524148) with
| () -> begin
...
in (let _524153 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (let _524155 = (Support.Microsoft.FStar.Util.flush_file transcript)
...
in (let _524167 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#pop") then begin
(let _524169 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#push") then begin
(let _524171 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end else begin
if (l = "#finish") then begin
(exit (0))
end else begin
(let _524173 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _524175 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end
end
end
...
| Pop (msg) -> begin
(let _524184 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Env.pop env msg))
in (let _524186 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.refresh ())
in (let _524188 = ((Support.Prims.ignore) (Microsoft_FStar_Options.reset_options ()))
in (let _524199 = (match (stack) with
| [] -> begin
...
in (dsenv, env))))
in (let fail = (fun curmod dsenv_mark env_mark -> (let _524230 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
in (let _524232 = (Support.ST.op_ColonEquals Microsoft_FStar_Tc_Errors.num_errs 0)
in (let _524234 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
in (let _524238 = (reset_mark dsenv_mark env_mark)
in (match (_524238) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end))))))
in (let _524241 = (mark dsenv env)
in (match (_524241) with
...
(let _524248 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
in (let _524252 = (commit_mark dsenv env)
in (match (_524252) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end)))
end else begin
...
let batch_mode_tc = (fun filenames -> (let _524259 = (tc_prims ())
in (match (_524259) with
| (prims_mod, dsenv, env) -> begin
(let _524272 = ((Support.List.fold_left (fun _524263 f -> (match (_524263) with
| (all_mods, dsenv, env) -> begin
(let _524268 = (tc_one_file dsenv env f)
in (match (_524268) with
| (dsenv, env, ms) -> begin
((Support.List.append all_mods ms), dsenv, env)
end))
end)) (prims_mod, dsenv, env)) filenames)
in (match (_524272) with
| (all_mods, dsenv, env) -> begin
(let _524273 = if ((Support.ST.read Microsoft_FStar_Options.interactive) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0)) then begin
...
in (let _524278 = ((Support.List.iter (fun m -> if (Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
...
(Support.Prims.try_with (fun _524282 -> (match (_524282) with
| () -> begin
...
in (Support.List.iter (fun _524296 -> (match (_524296) with
| (n, d) -> begin
...
end)) (fun _524281 -> (match (_524281) with
| Microsoft_FStar_Backends_OCaml_ASTTrans.OCamlFailure ((rg, error)) -> begin
(let _524288 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "OCaml Backend Error: %s %s\n" (Support.Microsoft.FStar.Range.string_of_range rg) (Microsoft_FStar_Backends_OCaml_ASTTrans.string_of_error error)))
in (exit (1)))
end)))
end)

let go = (fun _524297 -> (let _524301 = (process_args ())
in (match (_524301) with
| (res, filenames) -> begin
(match (res) with
...
| _ -> begin
(let _524310 = (Support.Microsoft.FStar.Util.print_string "--use_build_config expects just a single file on the command line and no other arguments")
in (exit (1)))
end)
end else begin
filenames
end
in (let _524316 = (batch_mode_tc filenames)
in (match (_524316) with
| (fmods, dsenv, env) -> begin
(let _524317 = (report_errors None)
in if (Support.ST.read Microsoft_FStar_Options.interactive) then begin
(interactive_mode dsenv env)
end else begin
(let _524319 = (codegen fmods)
...
let main = (fun _524321 -> (match (_524321) with
| () -> begin
(Support.Prims.try_with (fun _524323 -> (match (_524323) with
| () -> begin
(let _524334 = (go ())
in (let _524336 = (cleanup ())
in (exit (0))))
end)) (fun _524322 -> (match (_524322) with
| e -> begin
(let _524326 = if (Microsoft_FStar_Absyn_Util.handleable e) then begin
(Microsoft_FStar_Absyn_Util.handle_err false () e)
end
in (let _524328 = if (Support.ST.read Microsoft_FStar_Options.trace_error) then begin
