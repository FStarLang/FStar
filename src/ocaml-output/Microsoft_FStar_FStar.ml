...
let process_args = (fun _529390 -> (match (_529390) with
| () -> begin
...
let cleanup = (fun _529399 -> (match (_529399) with
| () -> begin
...
let tc_prims = (fun _529401 -> (match (_529401) with
| () -> begin
...
in (let _529404 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.init env)
in (let p = (Microsoft_FStar_Options.prims ())
in (let _529409 = (Microsoft_FStar_Parser_Driver.parse_file (Microsoft_FStar_Parser_DesugarEnv.empty_env ()) p)
in (match (_529409) with
| (dsenv, prims_mod) -> begin
(let _529412 = (Microsoft_FStar_Tc_Tc.check_module env (Support.List.hd prims_mod))
in (match (_529412) with
| (prims_mod, env) -> begin
(prims_mod, dsenv, env)
end))
end))))))
...
(let _529418 = (Support.Microsoft.FStar.Util.fprint1 "Error: %s errors were reported (see above)\n" (Support.Microsoft.FStar.Util.string_of_int errs))
in (exit (1)))
end))

let tc_one_file = (fun dsenv env fn -> (let _529425 = (Microsoft_FStar_Parser_Driver.parse_file dsenv fn)
in (match (_529425) with
| (dsenv, fmods) -> begin
(let _529435 = ((Support.List.fold_left (fun _529428 m -> (match (_529428) with
| (env, all_mods) -> begin
(let _529432 = (Microsoft_FStar_Tc_Tc.check_module env m)
in (match (_529432) with
| (ms, env) -> begin
(env, ms)
end))
end)) (env, [])) fmods)
in (match (_529435) with
| (env, all_mods) -> begin
(dsenv, env, (Support.List.rev all_mods))
end))
end)))

let tc_one_fragment = (fun curmod dsenv env frag -> (Support.Prims.try_with (fun _529441 -> (match (_529441) with
| () -> begin
...
in (let _529468 = (Microsoft_FStar_Tc_Tc.tc_partial_modul env modul)
in (match (_529468) with
...
(let _529481 = (Microsoft_FStar_Tc_Tc.tc_more_partial_modul env modul decls)
...
end)) (fun _529440 -> (match (_529440) with
| Microsoft_FStar_Absyn_Syntax.Error ((msg, r)) -> begin
(let _529447 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, r))::[]))
in None)
end
| Microsoft_FStar_Absyn_Syntax.Err (msg) -> begin
(let _529451 = (Microsoft_FStar_Tc_Errors.add_errors env (((msg, Microsoft_FStar_Absyn_Syntax.dummyRange))::[]))
in None)
end
| e -> begin
(raise (e))
...
in (fun line -> (let _529493 = (Support.Microsoft.FStar.Util.append_to_file transcript line)
in (Support.Microsoft.FStar.Util.flush_file transcript))))
end else begin
(fun line -> ())
end
in (let _529499 = if (Support.Option.isSome (! (Microsoft_FStar_Options.codegen))) then begin
(let _529497 = (Support.Microsoft.FStar.Util.print_string "Code-generation is not supported in interactive mode")
in (exit (1)))
end
in (let chunk = (Support.Microsoft.FStar.Util.new_string_builder ())
in (let stdin = (Support.Microsoft.FStar.Util.open_stdin ())
in (let rec fill_chunk = (fun _529504 -> (match (_529504) with
| () -> begin
...
in (let _529521 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Code ((str, responses)))))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#pop") then begin
(let _529523 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Pop (l))
end else begin
if (Support.Microsoft.FStar.Util.starts_with l "#push") then begin
(let _529525 = (Support.Microsoft.FStar.Util.clear_string_builder chunk)
in Push (l))
end else begin
if (l = "#finish") then begin
(exit (0))
end else begin
(let _529527 = (Support.Microsoft.FStar.Util.string_builder_append chunk line)
in (let _529529 = (Support.Microsoft.FStar.Util.string_builder_append chunk "\n")
in (fill_chunk ())))
end
end
end
...
| Pop (msg) -> begin
(let _529538 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Env.pop env msg))
in (let _529540 = (env.Microsoft_FStar_Tc_Env.solver.Microsoft_FStar_Tc_Env.refresh ())
in (let _529542 = ((Support.Prims.ignore) (Microsoft_FStar_Options.reset_options ()))
in (let _529553 = (match (stack) with
| [] -> begin
...
in (dsenv, env))))
in (let fail = (fun curmod dsenv_mark env_mark -> (let _529584 = ((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
in (let _529586 = (Microsoft_FStar_Tc_Errors.num_errs := 0)
in (let _529588 = (Support.Microsoft.FStar.Util.fprint1 "%s\n" fail)
in (let _529592 = (reset_mark dsenv_mark env_mark)
in (match (_529592) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end))))))
in (let _529595 = (mark dsenv env)
in (match (_529595) with
...
(let _529602 = (Support.Microsoft.FStar.Util.fprint1 "\n%s\n" ok)
in (let _529606 = (commit_mark dsenv env)
in (match (_529606) with
| (dsenv, env) -> begin
(go stack curmod dsenv env)
end)))
end else begin
...
let batch_mode_tc = (fun filenames -> (let _529613 = (tc_prims ())
in (match (_529613) with
| (prims_mod, dsenv, env) -> begin
(let _529626 = ((Support.List.fold_left (fun _529617 f -> (match (_529617) with
| (all_mods, dsenv, env) -> begin
(let _529622 = (tc_one_file dsenv env f)
in (match (_529622) with
| (dsenv, env, ms) -> begin
((Support.List.append all_mods ms), dsenv, env)
end))
end)) (prims_mod, dsenv, env)) filenames)
in (match (_529626) with
| (all_mods, dsenv, env) -> begin
(let _529627 = if ((! (Microsoft_FStar_Options.interactive)) && ((Microsoft_FStar_Tc_Errors.get_err_count ()) = 0)) then begin
...
in (let _529632 = ((Support.List.iter (fun m -> if (Microsoft_FStar_Options.should_verify m.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.str) then begin
...
(Support.Prims.try_with (fun _529636 -> (match (_529636) with
| () -> begin
...
in (Support.List.iter (fun _529650 -> (match (_529650) with
| (n, d) -> begin
...
end)) (fun _529635 -> (match (_529635) with
| Microsoft_FStar_Backends_OCaml_ASTTrans.OCamlFailure ((rg, error)) -> begin
(let _529642 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "OCaml Backend Error: %s %s\n" (Support.Microsoft.FStar.Range.string_of_range rg) (Microsoft_FStar_Backends_OCaml_ASTTrans.string_of_error error)))
in (exit (1)))
end)))
end)

let go = (fun _529651 -> (let _529655 = (process_args ())
in (match (_529655) with
| (res, filenames) -> begin
(match (res) with
...
| _ -> begin
(let _529664 = (Support.Microsoft.FStar.Util.print_string "--use_build_config expects just a single file on the command line and no other arguments")
in (exit (1)))
end)
end else begin
filenames
end
in (let _529670 = (batch_mode_tc filenames)
in (match (_529670) with
| (fmods, dsenv, env) -> begin
(let _529671 = (report_errors None)
in if (! (Microsoft_FStar_Options.interactive)) then begin
(interactive_mode dsenv env)
end else begin
(let _529673 = (codegen fmods)
...
let main = (fun _529675 -> (match (_529675) with
| () -> begin
(Support.Prims.try_with (fun _529677 -> (match (_529677) with
| () -> begin
(let _529688 = (go ())
in (let _529690 = (cleanup ())
in (exit (0))))
end)) (fun _529676 -> (match (_529676) with
| e -> begin
(let _529680 = if (Microsoft_FStar_Absyn_Util.handleable e) then begin
(Microsoft_FStar_Absyn_Util.handle_err false () e)
end
in (let _529682 = if (! (Microsoft_FStar_Options.trace_error)) then begin
