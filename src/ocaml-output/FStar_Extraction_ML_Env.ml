...
let debug : env  ->  (Prims.unit  ->  Prims.unit)  ->  Prims.unit = (fun g f -> if (((FStar_ST.read FStar_Options.debug) <> []) && ((let _138_65 = (FStar_ST.read FStar_Options.debug)
in (FStar_List.contains "Prims" _138_65)) || (g.currentModule <> ([], "Prims")))) then begin
(f ())
end else begin
()
end)
...
let mkFvvar : FStar_Ident.lident  ->  FStar_Absyn_Syntax.typ  ->  (FStar_Ident.lident, FStar_Absyn_Syntax.typ) FStar_Absyn_Syntax.withinfo_t = (fun l t -> (let _138_70 = (FStar_Range.mk_range "" 0 0)
in {FStar_Absyn_Syntax.v = l; FStar_Absyn_Syntax.sort = t; FStar_Absyn_Syntax.p = _138_70}))
...
(match (t) with
| FStar_Extraction_ML_Syntax.MLTY_Named (_59_24, ("FStar"::"Ghost"::[], "erased")) -> begin
true
end
| _59_33 -> begin
false
...
let prependTick = (fun _59_36 -> (match (_59_36) with
| (x, n) -> begin
...
let removeTick = (fun _59_39 -> (match (_59_39) with
| (x, n) -> begin
if (FStar_Util.starts_with x "\'") then begin
(let _138_75 = (FStar_Util.substring_from x 1)
in (_138_75, n))
end else begin
(x, n)
end
...
let tyscheme_of_td = (fun _59_62 -> (match (_59_62) with
| (_59_59, vars, body_opt) -> begin
(match (body_opt) with
...
let lookup_ty_const : env  ->  FStar_Extraction_ML_Syntax.mlpath  ->  FStar_Extraction_ML_Syntax.mltyscheme Prims.option = (fun env _59_71 -> (match (_59_71) with
| (module_name, ty_name) -> begin
(FStar_Util.find_map env.tydefs (fun _59_74 -> (match (_59_74) with
| (m, tds) -> begin
if (module_name = m) then begin
(FStar_Util.find_map tds (fun td -> (
# 105 "FStar.Extraction.ML.Env.fst"
let _59_81 = td
in (match (_59_81) with
| (n, _59_78, _59_80) -> begin
...
let x = (FStar_Util.find_map g.gamma (fun _59_1 -> (match (_59_1) with
...
in (match (x) with
| None -> begin
(let _138_105 = (let _138_104 = (FStar_Absyn_Print.sli fv)
in (FStar_Util.format1 "free Variable %s not found\n" _138_104))
in (FStar_All.failwith _138_105))
end
| Some (y) -> begin
y
end)))
...
let x = (FStar_Util.find_map g.gamma (fun _59_2 -> (match (_59_2) with
...
in (match (x) with
| None -> begin
(let _138_113 = (let _138_112 = (FStar_Range.string_of_range fv.FStar_Absyn_Syntax.p)
in (let _138_111 = (FStar_Absyn_Print.sli fv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _138_112 _138_111)))
in (FStar_All.failwith _138_113))
end
| Some (y) -> begin
y
end)))
...
let x = (FStar_Util.find_map g.gamma (fun _59_3 -> (match (_59_3) with
...
in (match (x) with
| None -> begin
(let _138_121 = (let _138_120 = (FStar_Range.string_of_range bv.FStar_Absyn_Syntax.p)
in (let _138_119 = (FStar_Absyn_Print.strBvd bv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _138_120 _138_119)))
in (FStar_All.failwith _138_121))
end
| Some (y) -> begin
y
end)))
...
(let _138_128 = (lookup g (FStar_Util.Inl (x)))
in (_138_128, None))
end
| FStar_Absyn_Syntax.Exp_fvar (x, b) -> begin
(let _138_129 = (lookup g (FStar_Util.Inr (x)))
in (_138_129, b))
end
| _59_144 -> begin
...
let _59_155 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _59_155.tydefs; currentModule = _59_155.currentModule}))))))
...
let _59_173 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _59_173.tydefs; currentModule = _59_173.currentModule})))))))
...
(let _138_151 = (mltyFvars t1)
in (let _138_150 = (mltyFvars t2)
in (FStar_List.append _138_151 _138_150)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _138_158 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _138_158 (Prims.fst tys))))
...
let _59_213 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _59_213.tydefs; currentModule = _59_213.currentModule}))))))
end else begin
...
| FStar_Util.Inl (x) -> begin
(let _138_193 = (extend_bv g (FStar_Absyn_Util.bvd_to_bvar_s x t) t_x add_unit is_rec false)
in (_138_193, (FStar_Extraction_ML_Syntax.as_mlident x)))
end
| FStar_Util.Inr (f) -> begin
(
# 224 "FStar.Extraction.ML.Env.fst"
let _59_233 = (FStar_Extraction_ML_Syntax.mlpath_of_lident f)
in (match (_59_233) with
| (p, y) -> begin
(let _138_194 = (extend_fv' g (FStar_Absyn_Util.fvvar_of_lid f t) (p, y) t_x add_unit is_rec)
in (_138_194, (y, 0)))
end))
end))
...
let _59_237 = g
in {tcenv = _59_237.tcenv; gamma = _59_237.gamma; tydefs = ((m, td))::g.tydefs; currentModule = _59_237.currentModule})))
...
in (let _138_201 = (extend_lb env (FStar_Util.Inr (FStar_Absyn_Const.failwith_lid)) FStar_Absyn_Syntax.tun failwith_ty false false)
in (FStar_All.pipe_right _138_201 Prims.fst))))))
