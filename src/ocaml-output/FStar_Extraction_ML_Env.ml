...
let debug = (fun g f -> if (((FStar_ST.read FStar_Options.debug) <> []) && ((let _123_65 = (FStar_ST.read FStar_Options.debug)
in (FStar_List.contains "Prims" _123_65)) || (g.currentModule <> ([], "Prims")))) then begin
(f ())
end else begin
()
end)
