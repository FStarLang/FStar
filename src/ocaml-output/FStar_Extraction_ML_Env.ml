...
let debug : env  ->  (Prims.unit  ->  Prims.unit)  ->  Prims.unit = (fun g f -> if (((FStar_ST.read FStar_Options.debug) <> []) && ((let _178_65 = (FStar_ST.read FStar_Options.debug)
in (FStar_List.contains "Prims" _178_65)) || (g.currentModule <> ([], "Prims")))) then begin
(f ())
end else begin
()
end)

let mkFvvar : FStar_Ident.lident  ->  FStar_Absyn_Syntax.typ  ->  (FStar_Ident.lident, FStar_Absyn_Syntax.typ) FStar_Absyn_Syntax.withinfo_t = (fun l t -> (let _178_70 = (FStar_Range.mk_range "" 0 0)
in {FStar_Absyn_Syntax.v = l; FStar_Absyn_Syntax.sort = t; FStar_Absyn_Syntax.p = _178_70}))
...
| (x, n) -> begin
if (FStar_Util.starts_with x "\'") then begin
(let _178_75 = (FStar_Util.substring_from x 1)
in (_178_75, n))
end else begin
(x, n)
end
...
let tyscheme_of_td = (fun _75_62 -> (match (_75_62) with
| (_75_59, vars, body_opt) -> begin
(match (body_opt) with
...
let lookup_ty_const : env  ->  FStar_Extraction_ML_Syntax.mlpath  ->  FStar_Extraction_ML_Syntax.mltyscheme Prims.option = (fun env _75_71 -> (match (_75_71) with
| (module_name, ty_name) -> begin
(FStar_Util.find_map env.tydefs (fun _75_74 -> (match (_75_74) with
| (m, tds) -> begin
if (module_name = m) then begin
(FStar_Util.find_map tds (fun td -> (let _75_81 = td
in (match (_75_81) with
| (n, _75_78, _75_80) -> begin
...
let lookup_fv_by_lid : env  ->  FStar_Ident.lident  ->  (FStar_Extraction_ML_Syntax.mlexpr * FStar_Extraction_ML_Syntax.mltyscheme * Prims.bool) = (fun g fv -> (let x = (FStar_Util.find_map g.gamma (fun _75_1 -> (match (_75_1) with
...
in (match (x) with
| None -> begin
(let _178_105 = (let _178_104 = (FStar_Absyn_Print.sli fv)
in (FStar_Util.format1 "free Variable %s not found\n" _178_104))
in (FStar_All.failwith _178_105))
end
| Some (y) -> begin
y
end)))

let lookup_fv : env  ->  FStar_Absyn_Syntax.fvvar  ->  (FStar_Extraction_ML_Syntax.mlexpr * FStar_Extraction_ML_Syntax.mltyscheme * Prims.bool) = (fun g fv -> (let x = (FStar_Util.find_map g.gamma (fun _75_2 -> (match (_75_2) with
...
in (match (x) with
| None -> begin
(let _178_113 = (let _178_112 = (FStar_Range.string_of_range fv.FStar_Absyn_Syntax.p)
in (let _178_111 = (FStar_Absyn_Print.sli fv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _178_112 _178_111)))
in (FStar_All.failwith _178_113))
end
| Some (y) -> begin
y
end)))

let lookup_bv : env  ->  FStar_Absyn_Syntax.bvvar  ->  (FStar_Extraction_ML_Syntax.mlexpr * FStar_Extraction_ML_Syntax.mltyscheme * Prims.bool) = (fun g bv -> (let x = (FStar_Util.find_map g.gamma (fun _75_3 -> (match (_75_3) with
...
in (match (x) with
| None -> begin
(let _178_121 = (let _178_120 = (FStar_Range.string_of_range bv.FStar_Absyn_Syntax.p)
in (let _178_119 = (FStar_Absyn_Print.strBvd bv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _178_120 _178_119)))
in (FStar_All.failwith _178_121))
end
| Some (y) -> begin
y
end)))
...
(let _178_128 = (lookup g (FStar_Util.Inl (x)))
in (_178_128, None))
end
| FStar_Absyn_Syntax.Exp_fvar (x, b) -> begin
(let _178_129 = (lookup g (FStar_Util.Inr (x)))
in (_178_129, b))
end
| _75_144 -> begin
...
in (let _75_155 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _75_155.tydefs; currentModule = _75_155.currentModule}))))))
...
in (let _75_173 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _75_173.tydefs; currentModule = _75_173.currentModule})))))))
...
(let _178_151 = (mltyFvars t1)
in (let _178_150 = (mltyFvars t2)
in (FStar_List.append _178_151 _178_150)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _178_158 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _178_158 (Prims.fst tys))))
...
in (let _75_213 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _75_213.tydefs; currentModule = _75_213.currentModule}))))))
end else begin
...
| FStar_Util.Inl (x) -> begin
(let _178_193 = (extend_bv g (FStar_Absyn_Util.bvd_to_bvar_s x t) t_x add_unit is_rec false)
in (_178_193, (FStar_Extraction_ML_Syntax.as_mlident x)))
end
| FStar_Util.Inr (f) -> begin
(let _75_233 = (FStar_Extraction_ML_Syntax.mlpath_of_lident f)
in (match (_75_233) with
| (p, y) -> begin
(let _178_194 = (extend_fv' g (FStar_Absyn_Util.fvvar_of_lid f t) (p, y) t_x add_unit is_rec)
in (_178_194, (y, 0)))
end))
end))
...
in (let _75_237 = g
in {tcenv = _75_237.tcenv; gamma = _75_237.gamma; tydefs = ((m, td))::g.tydefs; currentModule = _75_237.currentModule})))
...
in (let _178_201 = (extend_lb env (FStar_Util.Inr (FStar_Absyn_Const.failwith_lid)) FStar_Absyn_Syntax.tun failwith_ty false false)
in (FStar_All.pipe_right _178_201 Prims.fst))))))
