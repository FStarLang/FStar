...
let debug = (fun g f -> (match ((((FStar_ST.read FStar_Options.debug) <> []) && ((let _126_65 = (FStar_ST.read FStar_Options.debug)
in (FStar_List.contains "Prims" _126_65)) || (g.currentModule <> ([], "Prims"))))) with
| true -> begin
(f ())
end
| false -> begin
...
let mkFvvar = (fun l t -> (let _126_70 = (FStar_Range.mk_range "" 0 0)
in {FStar_Absyn_Syntax.v = l; FStar_Absyn_Syntax.sort = t; FStar_Absyn_Syntax.p = _126_70}))
...
| false -> begin
(match (t) with
| FStar_Extraction_ML_Syntax.MLTY_Named (_57_26, ("Ghost"::[], "erased")) -> begin
true
end
| _57_34 -> begin
false
...
let prependTick = (fun _57_37 -> (match (_57_37) with
| (x, n) -> begin
...
let removeTick = (fun _57_40 -> (match (_57_40) with
| (x, n) -> begin
(match ((FStar_Util.starts_with x "\'")) with
| true -> begin
(let _126_75 = (FStar_Util.substring_from x 1)
in (_126_75, n))
end
| false -> begin
(x, n)
...
let tyscheme_of_td = (fun _57_63 -> (match (_57_63) with
| (_57_60, vars, body_opt) -> begin
(match (body_opt) with
...
let lookup_ty_const = (fun env _57_72 -> (match (_57_72) with
| (module_name, ty_name) -> begin
(FStar_Util.find_map env.tydefs (fun _57_75 -> (match (_57_75) with
| (m, tds) -> begin
(match ((module_name = m)) with
| true -> begin
(FStar_Util.find_map tds (fun td -> (let _57_82 = td
in (match (_57_82) with
| (n, _57_79, _57_81) -> begin
...
in (match (x) with
| None -> begin
(let _126_105 = (let _126_104 = (FStar_Absyn_Print.sli fv)
in (FStar_Util.format1 "free Variable %s not found\n" _126_104))
in (FStar_All.failwith _126_105))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _126_113 = (let _126_112 = (FStar_Range.string_of_range fv.FStar_Absyn_Syntax.p)
in (let _126_111 = (FStar_Absyn_Print.sli fv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _126_112 _126_111)))
in (FStar_All.failwith _126_113))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _126_121 = (let _126_120 = (FStar_Range.string_of_range bv.FStar_Absyn_Syntax.p)
in (let _126_119 = (FStar_Absyn_Print.strBvd bv.FStar_Absyn_Syntax.v)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _126_120 _126_119)))
in (FStar_All.failwith _126_121))
end
| Some (y) -> begin
y
end)))
...
(let _126_128 = (lookup g (FStar_Util.Inl (x)))
in (_126_128, None))
end
| FStar_Absyn_Syntax.Exp_fvar (x, b) -> begin
(let _126_129 = (lookup g (FStar_Util.Inr (x)))
in (_126_129, b))
end
| _57_142 -> begin
...
in (let _57_153 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _57_153.tydefs; currentModule = _57_153.currentModule}))))))
...
in (let _57_164 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _57_164.tydefs; currentModule = _57_164.currentModule}))))))
...
(let _126_149 = (mltyFvars t1)
in (let _126_148 = (mltyFvars t2)
in (FStar_List.append _126_149 _126_148)))
end
...
(let _126_151 = (mltyFvars t1)
in (let _126_150 = (mltyFvars t2)
in (FStar_List.append _126_151 _126_150)))
end
...
let tySchemeIsClosed = (fun tys -> (let _126_158 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _126_158 (Prims.fst tys))))
...
in (let _57_201 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _57_201.tydefs; currentModule = _57_201.currentModule})))))
end
| false -> begin
...
| FStar_Util.Inl (x) -> begin
(let _126_187 = (extend_bv g (FStar_Absyn_Util.bvd_to_bvar_s x t) t_x add_unit false)
in (_126_187, (FStar_Extraction_ML_Syntax.as_mlident x)))
end
| FStar_Util.Inr (f) -> begin
(let _57_219 = (FStar_Extraction_ML_Syntax.mlpath_of_lident f)
in (match (_57_219) with
| (p, y) -> begin
(let _126_189 = (let _126_188 = (FStar_Absyn_Util.fvvar_of_lid f t)
in (extend_fv' g _126_188 (p, y) t_x add_unit))
in (_126_189, (y, 0)))
end))
end))
...
in (let _57_223 = g
in {tcenv = _57_223.tcenv; gamma = _57_223.gamma; tydefs = ((m, td))::g.tydefs; currentModule = _57_223.currentModule})))
...
in (let _126_196 = (extend_lb env (FStar_Util.Inr (FStar_Absyn_Const.failwith_lid)) FStar_Absyn_Syntax.tun failwith_ty false)
in (FStar_All.pipe_right _126_196 Prims.fst))))))
