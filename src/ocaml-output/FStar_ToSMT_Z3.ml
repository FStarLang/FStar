...
let x = (let _129_29 = (FStar_Util.substring_from x (FStar_String.length prefix))
...
let t = if (let _129_34 = (get_z3version ())
in (z3v_le _129_34 (4, 3, 1))) then begin
...
let timeout = (let _129_35 = (FStar_Util.string_of_int t)
in (FStar_Util.format1 "-t:%s" _129_35))
in (
# 74 "FStar.ToSMT.Z3.fst"
let relevancy = if (let _129_36 = (get_z3version ())
in (z3v_le _129_36 (4, 3, 1))) then begin
"RELEVANCY"
end else begin
"SMT.RELEVANCY"
end
...
| () -> begin
(let _129_45 = (FStar_Util.current_tid ())
in (FStar_All.pipe_right _129_45 FStar_Util.string_of_int))
end))
...
in x))
in (let _129_53 = (FStar_ST.read FStar_Options.z3_exe)
in (let _129_52 = (ini_params ())
in (FStar_Util.start_process id _129_53 _129_52 cond)))))
...
in (let _129_86 = (let _129_85 = (let _129_84 = (FStar_ST.read ctr)
in (FStar_Util.string_of_int _129_84))
in (FStar_Util.format1 "queries-%s.smt2" _129_85))
in (FStar_Util.open_file_for_writing _129_86)))
end else begin
...
| () -> begin
(let _129_95 = (let _129_94 = (
# 131 "FStar.ToSMT.Z3.fst"
let _44_102 = (FStar_Util.incr ctr)
in (let _129_93 = (FStar_ST.read ctr)
in (FStar_All.pipe_right _129_93 FStar_Util.string_of_int)))
in (FStar_Util.format1 "bg-%s" _129_94))
in (new_z3proc _129_95))
end))
in (
# 132 "FStar.ToSMT.Z3.fst"
let z3proc = (let _129_96 = (new_proc ())
in (FStar_Util.mk_ref _129_96))
in (
...
let _44_117 = (let _129_103 = (new_proc ())
in (FStar_ST.op_Colon_Equals z3proc _129_103))
in (
...
let fh = (let _129_106 = (let _129_105 = (let _129_104 = (FStar_ST.read ctr)
in (FStar_All.pipe_right _129_104 FStar_Util.string_of_int))
in (FStar_Util.format1 "queries-bg-%s.smt2" _129_105))
in (FStar_Util.open_file_for_writing _129_106))
...
| "unknown"::tl -> begin
(let _129_118 = (lblnegs tl)
in (UNKNOWN, _129_118))
end
| "sat"::tl -> begin
(let _129_119 = (lblnegs tl)
in (SAT, _129_119))
end
| "unsat"::tl -> begin
(UNSAT, [])
...
| _44_166 -> begin
(let _129_123 = (let _129_122 = (let _129_121 = (FStar_List.map (fun l -> (FStar_Util.format1 "<%s>" (FStar_Util.trim_string l))) lines)
in (FStar_String.concat "\n" _129_121))
in (FStar_Util.format1 "Got output lines: %s\n" _129_122))
in (FStar_All.pipe_left FStar_All.failwith _129_123))
end))
in (result lines)))))
in (
...
in (let _129_129 = (let _129_128 = (FStar_ST.read ctr)
in (FStar_Util.string_of_int _129_128))
in (new_z3proc _129_129)))
end else begin
(bg_z3_proc.grab ())
end
in (
...
let mbqi = if (let _129_132 = (get_z3version ())
in (z3v_le _129_132 (4, 3, 1))) then begin
"mbqi"
end else begin
"smt.mbqi"
end
...
let model_on_timeout = if (let _129_133 = (get_z3version ())
in (z3v_le _129_133 (4, 3, 1))) then begin
...
| () -> begin
(let _129_157 = (let _129_156 = (let _129_155 = (FStar_Range.mk_range "" 0 0)
in ("", _129_155))
in (_129_156)::[])
in (false, _129_157))
end)); callback = (fun a -> ())})::[]))
...
(let _129_168 = (FStar_Util.format1 "Z3 says: %s\n" (status_to_string status))
in (FStar_All.pipe_left FStar_Util.print_string _129_168))
end else begin
()
end
in (
...
and run_job : z3job  ->  Prims.unit = (fun j -> (let _129_180 = (j.job ())
in (FStar_All.pipe_left j.callback _129_180)))
...
let _44_265 = (let _129_190 = (let _129_189 = (FStar_ST.read job_queue)
in (FStar_List.append _129_189 ((j)::[])))
in (FStar_ST.op_Colon_Equals job_queue _129_190))
in (
...
| () -> begin
(let _129_198 = (FStar_ST.read pending_jobs)
in (let _129_197 = (let _129_196 = (FStar_ST.read job_queue)
in (FStar_List.length _129_196))
in (_129_198, _129_197)))
end)))
in (match (_44_280) with
| (n, m) -> begin
if ((n + m) = 0) then begin
(let _129_199 = (FStar_Tc_Errors.report_all ())
in (FStar_All.pipe_right _129_199 Prims.ignore))
end else begin
(
...
let _44_284 = (let _129_203 = (let _129_202 = (FStar_ST.read fresh_scope)
in ((FStar_ToSMT_Term.Caption (msg))::[])::_129_202)
in (FStar_ST.op_Colon_Equals fresh_scope _129_203))
in (let _129_205 = (let _129_204 = (FStar_ST.read bg_scope)
in (FStar_List.append ((FStar_ToSMT_Term.Caption (msg))::(FStar_ToSMT_Term.Push)::[]) _129_204))
in (FStar_ST.op_Colon_Equals bg_scope _129_205))))

# 282 "FStar.ToSMT.Z3.fst"
let pop : Prims.string  ->  Prims.unit = (fun msg -> (
# 284 "FStar.ToSMT.Z3.fst"
let _44_287 = (let _129_209 = (let _129_208 = (FStar_ST.read fresh_scope)
in (FStar_List.tl _129_208))
in (FStar_ST.op_Colon_Equals fresh_scope _129_209))
in (let _129_211 = (let _129_210 = (FStar_ST.read bg_scope)
in (FStar_List.append ((FStar_ToSMT_Term.Caption (msg))::(FStar_ToSMT_Term.Pop)::[]) _129_210))
in (FStar_ST.op_Colon_Equals bg_scope _129_211))))
...
in (let _129_215 = (let _129_214 = (FStar_ST.read bg_scope)
in (FStar_List.append (FStar_List.rev decls) _129_214))
in (FStar_ST.op_Colon_Equals bg_scope _129_215))))

# 290 "FStar.ToSMT.Z3.fst"
let bgtheory : Prims.bool  ->  FStar_ToSMT_Term.decl Prims.list = (fun fresh -> if fresh then begin
(let _129_219 = (let _129_218 = (FStar_ST.read fresh_scope)
in (FStar_List.rev _129_218))
in (FStar_All.pipe_right _129_219 FStar_List.flatten))
end else begin
(
...
let input = (let _129_236 = (let _129_235 = (let _129_234 = (z3_options ())
in (FStar_ToSMT_Term.declToSmt _129_234))
in (FStar_List.map _129_235 theory))
in (FStar_All.pipe_right _129_236 (FStar_String.concat "\n")))
in (
