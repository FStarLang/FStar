...
in (Microsoft_FStar_Util.format2 "Subtyping check failed; expected type %s; got type %s" _99_10 _99_9))))

let ill_kinded_type = "Ill-kinded type"

let totality_check = "This term may not terminate"

let diag = (fun r msg -> (let _99_16 = (let _99_15 = (Microsoft_FStar_Range.string_of_range r)
in (Microsoft_FStar_Util.format2 "%s (Diagnostic): %s\n" _99_15 msg))
in (Microsoft_FStar_Util.print_string _99_16)))

let warn = (fun r msg -> (let _99_22 = (let _99_21 = (Microsoft_FStar_Range.string_of_range r)
in (Microsoft_FStar_Util.format2 "%s (Warning): %s\n" _99_21 msg))
in (Microsoft_FStar_Util.print_string _99_22)))

let num_errs = (Microsoft_FStar_Util.mk_ref 0)

let verification_errs = (Microsoft_FStar_Util.mk_ref [])

let add_errors = (fun env errs -> (let errs = (All.pipe_right errs (Microsoft_FStar_List.map (fun _33_14 -> (match (_33_14) with
| (msg, r) -> begin
...
in (let n_errs = (Microsoft_FStar_List.length errs)
in (Microsoft_FStar_Util.atomically (fun _33_18 -> (match (()) with
| () -> begin
(let _33_19 = (let _99_30 = (let _99_29 = (ST.read verification_errs)
in (Microsoft_FStar_List.append errs _99_29))
in (ST.op_Colon_Equals verification_errs _99_30))
in (let _99_31 = ((ST.read num_errs) + n_errs)
in (ST.op_Colon_Equals num_errs _99_31)))
end))))))

let report_all = (fun _33_21 -> (match (()) with
| () -> begin
(let all_errs = (Microsoft_FStar_Util.atomically (fun _33_22 -> (match (()) with
| () -> begin
(let x = (ST.read verification_errs)
in (let _33_24 = (ST.op_Colon_Equals verification_errs [])
in x))
end)))
in (let all_errs = (Microsoft_FStar_List.sortWith (fun _33_30 _33_34 -> (match ((_33_30, _33_34)) with
| ((r1, _33_29), (r2, _33_33)) -> begin
(Microsoft_FStar_Range.compare r1 r2)
end)) all_errs)
in (let _33_39 = (All.pipe_right all_errs (Microsoft_FStar_List.iter (fun _33_38 -> (match (_33_38) with
| (r, msg) -> begin
(let _99_38 = (Microsoft_FStar_Range.string_of_range r)
in (Microsoft_FStar_Util.fprint2 "%s: %s\n" _99_38 msg))
end))))
in (Microsoft_FStar_List.length all_errs))))
end))

let report = (fun r msg -> (let _33_43 = (Microsoft_FStar_Util.incr num_errs)
in (let _99_44 = (let _99_43 = (Microsoft_FStar_Range.string_of_range r)
in (Microsoft_FStar_Util.format2 "%s: %s\n" _99_43 msg))
in (Microsoft_FStar_Util.print_string _99_44))))
...
in (Microsoft_FStar_Util.format2 "Unexpected signature for monad \"%s\". Expected a kind of the form (\'a:Type => WP \'a => WP \'a => Type);\ngot %s" m.Microsoft_FStar_Absyn_Syntax.str _99_53)))
...
in (Microsoft_FStar_Util.format3 "Expected a term of type \"%s\";\ngot a function \"%s\" (%s)" _99_63 _99_62 msg))))
...
in (Microsoft_FStar_Util.format3 "Expected expression of type \"%s\";\ngot expression \"%s\" of type \"%s\"" _99_74 _99_73 _99_72)))))
...
in (Microsoft_FStar_Util.format3 "Expected a function with a parameter of type \"%s\"; this function has a parameter of type \"%s\"" _99_86 _99_85))))
...
in (Microsoft_FStar_Util.format3 "Expected pattern of type \"%s\";\ngot pattern \"%s\" of type \"%s\"" _99_97 _99_96 _99_95)))))
...
in (Microsoft_FStar_Util.format2 "Expected type \"%s\";\ngot type \"%s\"" _99_107 _99_106)))
end
| Some (e) -> begin
...
in (Microsoft_FStar_Util.format3 "Expected type \"%s\"; but \"%s\" has type \"%s\"" _99_110 _99_109 _99_108))))
end))
...
in (Microsoft_FStar_Util.format2 "Kinds \"%s\" and \"%s\" are incompatible" _99_118 _99_117))))
...
in (Microsoft_FStar_Util.format3 "Constructor \"%s\" builds a value of type \"%s\"; expected \"%s\"" _99_129 _99_128 _99_127)))))
...
in (Microsoft_FStar_Util.format2 "Constructor \"%s\" fails the strict positivity check; the constructed type \"%s\" occurs to the left of a pure function type" _99_134 _99_133))))

let inline_type_annotation_and_val_decl = (fun l -> (let _99_137 = (Microsoft_FStar_Absyn_Print.sli l)
in (Microsoft_FStar_Util.format1 "\"%s\" has a val declaration as well as an inlined type annotation; remove one" _99_137)))
...
in (Microsoft_FStar_Util.format2 "Inferred type \"%s\" causes variable \"%s\" to escape its scope" _99_142 _99_141))))
...
in (Microsoft_FStar_Util.format3 "Expected type of kind \"%s\";\ngot \"%s\" of kind \"%s\"" _99_153 _99_152 _99_151)))))
...
in (Microsoft_FStar_Util.format2 "Expected a type-to-type constructor or function;\ngot a type \"%s\" of kind \"%s\"" _99_161 _99_160))))
...
in (Microsoft_FStar_Util.format1 "The pattern variable \"%s\" was used more than once" m)))

let disjunctive_pattern_vars = (fun v1 v2 -> (let vars = (fun v -> (let _99_192 = (All.pipe_right v (Microsoft_FStar_List.map (fun _33_1 -> (match (_33_1) with
...
in (All.pipe_right _99_192 (Microsoft_FStar_String.concat ", "))))
in (let _99_194 = (vars v1)
in (let _99_193 = (vars v2)
in (Microsoft_FStar_Util.format2 "Every alternative of an \'or\' pattern must bind the same variables; here one branch binds (\"%s\") and another (\"%s\")" _99_194 _99_193)))))
...
in (Microsoft_FStar_Util.format4 "Computed type \"%s\" and effect \"%s\" is not compatible with the annotated type \"%s\" effect \"%s\"" _99_202 f1 _99_201 f2)))
end))
end)))
...
in (Microsoft_FStar_Util.format1 "Term has an unexpected non-trivial pre-condition: %s" _99_207)))
...
in (All.pipe_left Prims.fst _99_210))
in (Microsoft_FStar_Util.format2 "Expected a pure expression;\ngot an expression \"%s\" with effect \"%s\"" _99_212 _99_211))))
...
in (All.pipe_left Prims.fst _99_215))
in (Microsoft_FStar_Util.format2 "Expected a ghost expression;\ngot an expression \"%s\" with effect \"%s\"" _99_217 _99_216))))
...
in (Microsoft_FStar_Util.format2 "Expected a computation with effect %s; but it has effect %s\n" _99_223 _99_222))))

let failed_to_prove_specification_of = (fun l lbls -> (let _99_229 = (Microsoft_FStar_Absyn_Print.lbname_to_string l)
in (let _99_228 = (All.pipe_right lbls (Microsoft_FStar_String.concat ", "))
in (Microsoft_FStar_Util.format2 "Failed to prove specification of %s; assertions at [%s] may fail" _99_229 _99_228))))
...
| _33_144 -> begin
(let _99_232 = (All.pipe_right lbls (Microsoft_FStar_String.concat "\n\t"))
in (Microsoft_FStar_Util.format1 "The following problems were found:\n\t%s" _99_232))
end))
