...
let add_errors = (fun env errs -> (let errs = ((Support.List.map (fun _26_14 -> (match (_26_14) with
| (msg, r) -> begin
...
in (let _26_39 = ((Support.List.iter (fun _26_38 -> (match (_26_38) with
| (r, msg) -> begin
...
let report = (fun r msg -> (let _26_43 = (Support.Microsoft.FStar.Util.incr num_errs)
in (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "%s: %s\n" (Support.Microsoft.FStar.Range.string_of_range r) msg))))

let get_err_count = (fun _26_45 -> (match (_26_45) with
| () -> begin
(! (num_errs))
end))
...
let disjunctive_pattern_vars = (fun v1 v2 -> (let vars = (fun v -> ((Support.String.concat ", ") ((Support.List.map (fun _26_1 -> (match (_26_1) with
...
let computed_computation_type_does_not_match_annotation = (fun env e c c' -> (let _26_127 = (name_and_result c)
in (match (_26_127) with
| (f1, r1) -> begin
(let _26_130 = (name_and_result c')
in (match (_26_130) with
| (f2, r2) -> begin
