...
let add_errors = (fun env errs -> (let errs = ((Support.List.map (fun _147334 -> (match (_147334) with
| (msg, r) -> begin
...
in (Support.Microsoft.FStar.Util.atomically (fun _147338 -> (match (_147338) with
| () -> begin
(let _147339 = (verification_errs := (Support.List.append errs (! (verification_errs))))
in (num_errs := ((! (num_errs)) + n_errs)))
end))))))

let report_all = (fun _147341 -> (match (_147341) with
| () -> begin
(let all_errs = (Support.Microsoft.FStar.Util.atomically (fun _147342 -> (match (_147342) with
| () -> begin
(let x = (! (verification_errs))
in (let _147344 = (verification_errs := [])
in x))
end)))
in (let all_errs = (Support.List.sortWith (fun _147350 _147354 -> (match ((_147350, _147354)) with
| ((r1, _), (r2, _)) -> begin
(Support.Microsoft.FStar.Range.compare r1 r2)
end)) all_errs)
in (let _147359 = ((Support.List.iter (fun _147358 -> (match (_147358) with
| (r, msg) -> begin
...
let report = (fun r msg -> (let _147363 = (Support.Microsoft.FStar.Util.incr num_errs)
in (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format2 "%s: %s\n" (Support.Microsoft.FStar.Range.string_of_range r) msg))))

let get_err_count = (fun _147365 -> (match (_147365) with
| () -> begin
(! (num_errs))
end))
...
let disjunctive_pattern_vars = (fun v1 v2 -> (let vars = (fun v -> ((Support.String.concat ", ") ((Support.List.map (fun _147321 -> (match (_147321) with
...
let computed_computation_type_does_not_match_annotation = (fun env e c c' -> (let _147447 = (name_and_result c)
in (match (_147447) with
| (f1, r1) -> begin
(let _147450 = (name_and_result c')
in (match (_147450) with
| (f2, r2) -> begin
