...
let z3v_compare = (fun known _422236 -> (match (_422236) with
| (w1, w2, w3) -> begin
(match (known) with
| Z3V_Unknown -> begin
None
...
let get_z3version = (fun _422248 -> (match (_422248) with
| () -> begin
...
| None -> begin
(let _422258 = (Support.Microsoft.FStar.Util.run_proc (! (Microsoft_FStar_Options.z3_exe)) "-version" "")
in (match (_422258) with
| (_, out, _) -> begin
...
in (let x = (Support.Prims.try_with (fun _422265 -> (match (_422265) with
| () -> begin
...
in (let _422281 = (_z3version := Some (out))
in out))
end))
end))
end))

let ini_params = (fun _422283 -> (match (_422283) with
| () -> begin
...
let status_to_string = (fun _422229 -> (match (_422229) with
| SAT -> begin
"sat"
end
| UNSAT -> begin
...
let tid = (fun _422292 -> (match (_422292) with
| () -> begin
...
(let _422304 = (Support.Microsoft.FStar.Util.incr ctr)
...
in (let _422308 = (queries_dot_smt2 := Some (fh))
in fh))
end
| Some (fh) -> begin
fh
...
in (let _422315 = (Support.Microsoft.FStar.Util.append_to_file fh i)
...
in (let new_proc = (fun _422319 -> (match (_422319) with
| () -> begin
(new_z3proc (Support.Microsoft.FStar.Util.format1 "bg-%s" (let _422320 = (Support.Microsoft.FStar.Util.incr ctr)
in (Support.Microsoft.FStar.Util.string_of_int (! (ctr))))))
end))
in (let z3proc = (Support.Microsoft.FStar.Util.mk_ref (new_proc ()))
in (let x = []
in (let grab = (fun _422325 -> (match (_422325) with
| () -> begin
(let _422326 = (Support.Microsoft.FStar.Util.monitor_enter x)
in (! (z3proc)))
end))
in (let release = (fun _422329 -> (match (_422329) with
| () -> begin
(Support.Microsoft.FStar.Util.monitor_exit x)
end))
in (let refresh = (fun _422331 -> (match (_422331) with
| () -> begin
(let proc = (grab ())
in (let _422333 = (Support.Microsoft.FStar.Util.kill_process proc)
in (let _422335 = (z3proc := (new_proc ()))
in (let _422343 = (match ((! (queries_dot_smt2))) with
| None -> begin
()
end
| Some (fh) -> begin
(let _422340 = (Support.Microsoft.FStar.Util.close_file fh)
...
let z3_options = (fun _422396 -> (match (_422396) with
| () -> begin
...
let job_queue = (let x = (Support.Microsoft.FStar.Util.mk_ref (({job = (fun _422403 -> (match (_422403) with
| () -> begin
...
let with_monitor = (fun m f -> (let _422410 = (Support.Microsoft.FStar.Util.monitor_enter m)
in (let res = (f ())
in (let _422413 = (Support.Microsoft.FStar.Util.monitor_exit m)
in res))))

let z3_job = (fun fresh label_messages input _422418 -> (match (_422418) with
| () -> begin
(let _422421 = (doZ3Exe fresh input)
in (match (_422421) with
...
| _ -> begin
(let _422425 = if ((! (Microsoft_FStar_Options.debug)) <> []) then begin
(Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format1 "Z3 says: %s\n" (status_to_string status)))
end
in (let failing_assertions = ((Support.List.collect (fun l -> (match (((Support.List.tryFind (fun _422433 -> (match (_422433) with
| (m, _, _) -> begin
...
let rec dequeue' = (fun _422443 -> (match (_422443) with
| () -> begin
...
| hd::tl -> begin
(let _422448 = (job_queue := tl)
in hd)
end)
in (let _422451 = (Support.Microsoft.FStar.Util.incr pending_jobs)
in (let _422453 = (Support.Microsoft.FStar.Util.monitor_exit job_queue)
in (let _422455 = (run_job j)
in (let _422458 = (with_monitor job_queue (fun _422457 -> (match (_422457) with
| () -> begin
(Support.Microsoft.FStar.Util.decr pending_jobs)
end)))
in (dequeue ()))))))
end))
and dequeue = (fun _422460 -> (match (_422460) with
| () -> begin
(let _422461 = (Support.Microsoft.FStar.Util.monitor_enter job_queue)
in (let rec aux = (fun _422464 -> (match (_422464) with
| () -> begin
(match ((! (job_queue))) with
| [] -> begin
(let _422466 = (Support.Microsoft.FStar.Util.monitor_wait job_queue)
in (aux ()))
end
| _ -> begin
(dequeue' ())
...
let init = (fun _422471 -> (match (_422471) with
| () -> begin
(let n_runners = ((! (Microsoft_FStar_Options.n_cores)) - 1)
in (let rec aux = (fun n -> if (n = 0) then begin
()
end else begin
(let _422475 = (Support.Microsoft.FStar.Util.spawn (dequeue))
in (aux (n - 1)))
end)
in (aux n_runners)))
end))
...
(let _422479 = (Support.Microsoft.FStar.Util.monitor_enter job_queue)
in (let _422481 = (job_queue := (Support.List.append (! (job_queue)) ((j)::[])))
in (let _422483 = (Support.Microsoft.FStar.Util.monitor_pulse job_queue)
in (Support.Microsoft.FStar.Util.monitor_exit job_queue))))
end)

let finish = (fun _422485 -> (match (_422485) with
| () -> begin
(let bg = (bg_z3_proc.grab ())
in (let _422487 = (Support.Microsoft.FStar.Util.kill_process bg)
in (let _422489 = (bg_z3_proc.release ())
in (let rec aux = (fun _422492 -> (match (_422492) with
| () -> begin
(let _422496 = (with_monitor job_queue (fun _422493 -> (match (_422493) with
| () -> begin
((! (pending_jobs)), (Support.List.length (! (job_queue))))
end)))
in (match (_422496) with
| (n, m) -> begin
if ((n + m) = 0) then begin
((Support.Prims.ignore) (Microsoft_FStar_Tc_Errors.report_all ()))
end else begin
(let _422497 = (Support.Microsoft.FStar.Util.sleep 500)
in (aux ()))
end
end))
end))
...
(let bg = (! (bg_scope))
in (let _422515 = (bg_scope := [])
in (Support.List.rev bg)))
end)

let refresh = (fun _422517 -> (match (_422517) with
| () -> begin
(let _422518 = (bg_z3_proc.refresh ())
...
let reset_mark = (fun msg -> (let _422523 = (pop msg)
in (refresh ())))
...
in (let _422541 = if (! (Microsoft_FStar_Options.logQueries)) then begin
