...
let extend_env' = (fun env b -> (let _126245 = env
in {context = (b)::env.context; label_suffix = _126245.label_suffix}))

let extend_env = (fun env bindings -> (let _126249 = env
in {context = (Support.List.append bindings env.context); label_suffix = _126249.label_suffix}))

let lookup_env = (fun env key -> ((Support.Microsoft.FStar.Util.find_opt (fun _126217 -> (match (_126217) with
| T ((a, _)) -> begin
...
let rec subst_of_env' = (fun env -> (fold_env env (fun _126280 v acc -> (match (v) with
| T ((a, (t, env'))) -> begin
...
| ([], more) -> begin
(let _126351 = (Microsoft_FStar_Absyn_Util.args_of_binders more)
in (match (_126351) with
| (more, args) -> begin
...
| _ -> begin
(let _126357 = (Microsoft_FStar_Absyn_Util.args_of_binders binders)
...
| _ -> begin
(let _126396 = (Microsoft_FStar_Absyn_Util.args_of_binders bs)
in (match (_126396) with
| (bs, args) -> begin
...
let no_eta = (Support.List.filter (fun _126218 -> (match (_126218) with
| Eta -> begin
false
end
| _ -> begin
...
let no_eta_cfg = (fun c -> (let _126404 = c
in {code = _126404.code; environment = _126404.environment; stack = _126404.stack; close = _126404.close; steps = (no_eta c.steps)}))
...
(let binders' = (Support.List.map (fun _126219 -> (match (_126219) with
...
in (let c = (Microsoft_FStar_Absyn_Syntax.mk_Comp (let _126435 = (Microsoft_FStar_Absyn_Util.comp_to_comp_typ c1)
in {Microsoft_FStar_Absyn_Syntax.effect_name = _126435.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = _126435.Microsoft_FStar_Absyn_Syntax.result_typ; Microsoft_FStar_Absyn_Syntax.effect_args = _126435.Microsoft_FStar_Absyn_Syntax.effect_args; Microsoft_FStar_Absyn_Syntax.flags = c.Microsoft_FStar_Absyn_Syntax.flags}))
...
let simplify_then_apply = (fun steps head args pos -> (let fallback = (fun _126479 -> (match (_126479) with
| () -> begin
...
let rec sn_delay = (fun tcenv cfg -> (let aux = (fun _126666 -> (match (_126666) with
| () -> begin
(sn tcenv cfg).code
end))
...
in (let _126668 = cfg
in {code = t; environment = _126668.environment; stack = empty_stack; close = _126668.close; steps = _126668.steps}))))
...
in (let args = ((Support.List.map (fun _126220 -> (match (_126220) with
...
in (let _126692 = config
in {code = t; environment = _126692.environment; stack = empty_stack; close = _126692.close; steps = _126692.steps}))))
end)
...
(let _126699 = config
in {code = (eta_expand tcenv t); environment = _126699.environment; stack = _126699.stack; close = _126699.close; steps = _126699.steps})
end else begin
(let _126701 = config
in {code = t; environment = _126701.environment; stack = _126701.stack; close = _126701.close; steps = _126701.steps})
end))))
...
in (let config = (let _126718 = cfg
in {code = (Microsoft_FStar_Absyn_Util.compress_typ cfg.code); environment = _126718.environment; stack = _126718.stack; close = _126718.close; steps = _126718.steps})
...
| Some (t) -> begin
(sn tcenv (let _126739 = config
in {code = t; environment = _126739.environment; stack = _126739.stack; close = _126739.close; steps = _126739.steps}))
end)
end else begin
(rebuild config)
end
...
| Some (T ((_, (t, e)))) -> begin
(sn tcenv (let _126752 = config
in {code = t; environment = e; stack = _126752.stack; close = _126752.close; steps = _126752.steps}))
end
| _ -> begin
...
in (let stack = (let _126763 = config.stack
in {args = args})
in (sn tcenv (let _126766 = config
in {code = head; environment = _126766.environment; stack = stack; close = _126766.close; steps = _126766.steps}))))
end
| Microsoft_FStar_Absyn_Syntax.Typ_lam ((binders, t2)) -> begin
(match (config.stack.args) with
| [] -> begin
(let _126775 = (sn_binders tcenv binders config.environment config.steps)
in (match (_126775) with
...
in (let _126783 = t2_cfg
in {code = (mk_lam t2_cfg.code); environment = _126783.environment; stack = _126783.stack; close = _126783.close; steps = _126783.steps})))
end))
end
| args -> begin
...
in (sn tcenv (let _126795 = config
in {code = t2; environment = env; stack = (let _126797 = config.stack
in {args = args}); close = _126795.close; steps = _126795.steps})))
end
| (_, []) -> begin
...
in (sn tcenv (let _126805 = config
in {code = t; environment = env; stack = empty_stack; close = _126805.close; steps = _126805.steps}))))
end
...
(sn tcenv (let _126848 = config
in {code = t; environment = _126848.environment; stack = _126848.stack; close = _126848.close; steps = _126848.steps}))
end
| _ -> begin
(match (config.code.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, comp)) -> begin
(let _126858 = (sn_binders tcenv bs config.environment config.steps)
in (match (_126858) with
| (binders, environment) -> begin
(let c2 = (sncomp tcenv (c_config comp environment config.steps))
in (let _126860 = config
in {code = (wk (Microsoft_FStar_Absyn_Syntax.mk_Typ_fun (binders, c2.code))); environment = _126860.environment; stack = _126860.stack; close = _126860.close; steps = _126860.steps}))
end))
end
...
(sn tcenv (let _126883 = config
in {code = t; environment = _126883.environment; stack = _126883.stack; close = _126883.close; steps = _126883.steps}))
end else begin
...
in (sn tcenv (let _126888 = config
in {code = t; environment = _126888.environment; stack = _126888.stack; close = (close_with_config config pat); steps = _126888.steps})))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_labeled ((t, l, r, b))) -> begin
if (unmeta config) then begin
(sn tcenv (let _126897 = config
in {code = t; environment = _126897.environment; stack = _126897.stack; close = _126897.close; steps = _126897.steps}))
end else begin
...
(let _126911 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
(let _126913 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
in (sn tcenv (let _126917 = config
in {code = t; environment = _126917.environment; stack = _126917.stack; close = (close_with_config config lab); steps = _126917.steps})))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_refresh_label ((t, b, r))) -> begin
if (unmeta config) then begin
(sn tcenv (let _126925 = config
in {code = t; environment = _126925.environment; stack = _126925.stack; close = _126925.close; steps = _126925.steps}))
end else begin
...
in (let config = (let _126932 = config
in {code = t; environment = (let _126934 = config.environment
in {context = _126934.context; label_suffix = ((b, sfx))::config.environment.label_suffix}); stack = _126932.stack; close = _126932.close; steps = _126932.steps})
in (sn tcenv config)))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_slack_formula ((t1, t2, flag))) -> begin
if (! (flag)) then begin
(sn tcenv (let _126943 = config
in {code = (Microsoft_FStar_Absyn_Util.mk_conj t1 t2); environment = _126943.environment; stack = _126943.stack; close = _126943.close; steps = _126943.steps}))
end else begin
...
in (rebuild (let _126947 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_slack_formula ((c1.code, c2.code, flag)))); environment = _126947.environment; stack = _126947.stack; close = _126947.close; steps = _126947.steps}))))
end
end
...
and sn_binders = (fun tcenv binders env steps -> (let rec aux = (fun out env _126221 -> (match (_126221) with
...
in (let _126991 = cfg
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Comp ctconf.code); environment = _126991.environment; stack = _126991.stack; close = _126991.close; steps = _126991.steps}))
end
...
in (let norm = (fun _127000 -> (match (_127000) with
| () -> begin
...
in (let _127007 = cfg
in {code = c; environment = _127007.environment; stack = _127007.stack; close = _127007.close; steps = _127007.steps})))
in (let res = (sn tcenv (with_new_code cfg m.Microsoft_FStar_Absyn_Syntax.result_typ)).code
in (let sn_flags = (fun flags -> ((Support.List.map (fun _126222 -> (match (_126222) with
...
in (let _127019 = ((sn_flags m.Microsoft_FStar_Absyn_Syntax.flags), (sn_args true tcenv cfg.environment cfg.steps m.Microsoft_FStar_Absyn_Syntax.effect_args))
in (match (_127019) with
| (flags, args) -> begin
...
in (sncomp_typ tcenv (let _127024 = cfg
in {code = c; environment = _127024.environment; stack = _127024.stack; close = _127024.close; steps = _127024.steps})))
end
| _ -> begin
(norm ())
end)
...
and sn_args = (fun delay tcenv env steps args -> ((Support.List.map (fun _126223 -> (match (_126223) with
...
in (let _127063 = cfg
in {code = (w (Microsoft_FStar_Absyn_Syntax.mk_Kind_uvar (uv, args))); environment = _127063.environment; stack = _127063.stack; close = _127063.close; steps = _127063.steps}))
end
...
(let _127084 = (Microsoft_FStar_Tc_Env.lookup_kind_abbrev tcenv l)
in (match (_127084) with
| (_, binders, body) -> begin
(let subst = (Microsoft_FStar_Absyn_Util.subst_of_list binders args)
in (snk tcenv (let _127086 = cfg
in {code = (Microsoft_FStar_Absyn_Util.subst_kind subst body); environment = _127086.environment; stack = _127086.stack; close = _127086.close; steps = _127086.steps})))
end))
end
| Microsoft_FStar_Absyn_Syntax.Kind_abbrev ((_, k)) -> begin
(snk tcenv (let _127093 = cfg
in {code = k; environment = _127093.environment; stack = _127093.stack; close = _127093.close; steps = _127093.steps}))
end
| Microsoft_FStar_Absyn_Syntax.Kind_arrow ((bs, k)) -> begin
(let _127101 = (sn_binders tcenv bs cfg.environment cfg.steps)
in (match (_127101) with
| (bs, env) -> begin
(let c2 = (snk tcenv (k_config k env cfg.steps))
in (let _127111 = (match (c2.code.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Kind_arrow ((bs', k)) -> begin
...
in (match (_127111) with
| (bs, rhs) -> begin
(let _127112 = cfg
in {code = (w (Microsoft_FStar_Absyn_Syntax.mk_Kind_arrow (bs, rhs))); environment = _127112.environment; stack = _127112.stack; close = _127112.close; steps = _127112.steps})
end)))
end))
end
| Microsoft_FStar_Absyn_Syntax.Kind_unknown -> begin
...
in (let config = (let _127118 = cfg
in {code = e; environment = _127118.environment; stack = _127118.stack; close = _127118.close; steps = _127118.steps})
in (let rebuild = (fun config -> if (is_stack_empty config) then begin
config
end else begin
(let s' = (no_eta config.steps)
in (let args = ((Support.List.map (fun _126224 -> (match (_126224) with
| ((Support.Microsoft.FStar.Util.Inl (t), imp), env) -> begin
((Support.Microsoft.FStar.Util.Inl (sn tcenv (t_config t env s')).code), imp)
end
| ((Support.Microsoft.FStar.Util.Inr (v), imp), env) -> begin
...
in (let _127138 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_app (config.code, args) None config.code.Microsoft_FStar_Absyn_Syntax.pos); environment = _127138.environment; stack = empty_stack; close = _127138.close; steps = _127138.steps})))
end)
...
(wne tcenv (let _127163 = config
in {code = vc; environment = env; stack = _127163.stack; close = _127163.close; steps = _127163.steps}))
end
| _ -> begin
...
(let args = (Support.List.fold_right (fun a out -> ((a, config.environment))::out) args config.stack.args)
in (let stack = (let _127174 = config.stack
in {args = args})
in (wne tcenv (let _127177 = config
in {code = head; environment = _127177.environment; stack = stack; close = _127177.close; steps = _127177.steps}))))
end
...
in (wne tcenv (let _127192 = config
in {code = body; environment = env; stack = (let _127194 = config.stack
in {args = args}); close = _127192.close; steps = _127192.steps})))
end
| (_, []) -> begin
(let env = (extend_env config.environment entries)
in (let _127203 = (sn_binders tcenv binders env config.steps)
in (match (_127203) with
| (binders, env) -> begin
(let mk_abs = (fun t -> (Microsoft_FStar_Absyn_Syntax.mk_Exp_abs (binders, t) None body.Microsoft_FStar_Absyn_Syntax.pos))
in (let c = (wne tcenv (let _127206 = config
in {code = body; environment = env; stack = (let _127208 = config.stack
in {args = []}); close = _127206.close; steps = (no_eta config.steps)}))
in (let _127211 = c
in {code = (mk_abs c.code); environment = _127211.environment; stack = _127211.stack; close = _127211.close; steps = _127211.steps})))
end)))
end
| (formal::rest, actual::rest') -> begin
(let m = (match ((formal, actual)) with
...
(let c_e1 = (wne tcenv (let _127253 = config
in {code = e1; environment = _127253.environment; stack = empty_stack; close = _127253.close; steps = _127253.steps}))
in (let wn_eqn = (fun _127260 -> (match (_127260) with
| (pat, w, body) -> begin
...
in (let _127301 = x
in {Microsoft_FStar_Absyn_Syntax.v = _127301.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = t.code; Microsoft_FStar_Absyn_Syntax.p = _127301.Microsoft_FStar_Absyn_Syntax.p})))
in (let norm_btvar = (fun a -> (let k = (snk tcenv (k_config a.Microsoft_FStar_Absyn_Syntax.sort config.environment config.steps))
in (let _127306 = a
in {Microsoft_FStar_Absyn_Syntax.v = _127306.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = k.code; Microsoft_FStar_Absyn_Syntax.p = _127306.Microsoft_FStar_Absyn_Syntax.p})))
...
| Some (w) -> begin
(let c_w = (wne tcenv (let _127352 = config
in {code = w; environment = env; stack = empty_stack; close = _127352.close; steps = _127352.steps}))
in Some (c_w.code))
end)
in (let c_body = (wne tcenv (let _127356 = config
in {code = body; environment = env; stack = empty_stack; close = _127356.close; steps = _127356.steps}))
...
in (rebuild (let _127361 = config
in {code = e; environment = _127361.environment; stack = _127361.stack; close = _127361.close; steps = _127361.steps}))))))
end
| Microsoft_FStar_Absyn_Syntax.Exp_let (((is_rec, lbs), body)) -> begin
(let _127392 = ((Support.List.fold_left (fun _127371 _127375 -> (match ((_127371, _127375)) with
| ((env, lbs), (x, t, e)) -> begin
(let c = (wne tcenv (let _127376 = config
in {code = e; environment = _127376.environment; stack = empty_stack; close = _127376.close; steps = _127376.steps}))
in (let t = (sn tcenv (t_config t config.environment config.steps))
in (let _127389 = (match (x) with
...
in (match (_127389) with
| (y, env) -> begin
(env, ((y, t.code, c.code))::lbs)
end))))
end)) (config.environment, [])) lbs)
in (match (_127392) with
| (env, lbs) -> begin
(let lbs = (Support.List.rev lbs)
in (let c_body = (wne tcenv (let _127394 = config
in {code = body; environment = env; stack = empty_stack; close = _127394.close; steps = _127394.steps}))
in (let e = (Microsoft_FStar_Absyn_Syntax.mk_Exp_let ((is_rec, lbs), c_body.code) None e.Microsoft_FStar_Absyn_Syntax.pos)
in (rebuild (let _127398 = config
in {code = e; environment = _127398.environment; stack = _127398.stack; close = _127398.close; steps = _127398.steps})))))
end))
end
| Microsoft_FStar_Absyn_Syntax.Exp_ascribed ((e, t)) -> begin
(let c = (wne tcenv (let _127404 = config
in {code = e; environment = _127404.environment; stack = _127404.stack; close = _127404.close; steps = _127404.steps}))
in if (is_stack_empty config) then begin
(let t = (sn tcenv (t_config t config.environment config.steps))
in (rebuild (let _127408 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_ascribed (c.code, t.code) e.Microsoft_FStar_Absyn_Syntax.pos); environment = _127408.environment; stack = _127408.stack; close = _127408.close; steps = _127408.steps})))
end else begin
c
end)
end
| Microsoft_FStar_Absyn_Syntax.Exp_meta (Microsoft_FStar_Absyn_Syntax.Meta_desugared ((e, info))) -> begin
(let c = (wne tcenv (let _127415 = config
in {code = e; environment = _127415.environment; stack = _127415.stack; close = _127415.close; steps = _127415.steps}))
in if (is_stack_empty config) then begin
(rebuild (let _127418 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_meta (Microsoft_FStar_Absyn_Syntax.Meta_desugared ((c.code, info)))); environment = _127418.environment; stack = _127418.stack; close = _127418.close; steps = _127418.steps}))
end else begin
c
end)
end)))))
...
let norm_sigelt = (fun tcenv _126225 -> (match (_126225) with
