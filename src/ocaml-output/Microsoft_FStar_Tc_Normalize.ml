...
in (let c = (Microsoft_FStar_Absyn_Syntax.mk_Comp (let _25_219 = (Microsoft_FStar_Absyn_Util.comp_to_comp_typ c1)
in {Microsoft_FStar_Absyn_Syntax.effect_name = _25_219.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = _25_219.Microsoft_FStar_Absyn_Syntax.result_typ; Microsoft_FStar_Absyn_Syntax.effect_args = _25_219.Microsoft_FStar_Absyn_Syntax.effect_args; Microsoft_FStar_Absyn_Syntax.flags = c.Microsoft_FStar_Absyn_Syntax.flags}))
...
let simplify_then_apply = (fun steps head args pos -> (let fallback = (fun _25_263 -> (match (_25_263) with
| () -> begin
...
let rec sn_delay = (fun tcenv cfg -> (let aux = (fun _25_450 -> (match (_25_450) with
| () -> begin
(sn tcenv cfg).code
end))
...
in (let _25_452 = cfg
in {code = t; environment = _25_452.environment; stack = empty_stack; close = _25_452.close; steps = _25_452.steps}))))
...
in (let _25_476 = config
in {code = t; environment = _25_476.environment; stack = empty_stack; close = _25_476.close; steps = _25_476.steps}))))
end)
...
(let _25_483 = config
in {code = (eta_expand tcenv t); environment = _25_483.environment; stack = _25_483.stack; close = _25_483.close; steps = _25_483.steps})
end else begin
(let _25_485 = config
in {code = t; environment = _25_485.environment; stack = _25_485.stack; close = _25_485.close; steps = _25_485.steps})
end))))
...
in (let config = (let _25_502 = cfg
in {code = (Microsoft_FStar_Absyn_Util.compress_typ cfg.code); environment = _25_502.environment; stack = _25_502.stack; close = _25_502.close; steps = _25_502.steps})
...
| Some (t) -> begin
(sn tcenv (let _25_523 = config
in {code = t; environment = _25_523.environment; stack = _25_523.stack; close = _25_523.close; steps = _25_523.steps}))
end)
end else begin
(rebuild config)
end
...
| Some (T ((_, (t, e)))) -> begin
(sn tcenv (let _25_536 = config
in {code = t; environment = e; stack = _25_536.stack; close = _25_536.close; steps = _25_536.steps}))
end
| _ -> begin
...
in (let stack = (let _25_547 = config.stack
in {args = args})
in (sn tcenv (let _25_550 = config
in {code = head; environment = _25_550.environment; stack = stack; close = _25_550.close; steps = _25_550.steps}))))
end
| Microsoft_FStar_Absyn_Syntax.Typ_lam ((binders, t2)) -> begin
(match (config.stack.args) with
| [] -> begin
(let _25_559 = (sn_binders tcenv binders config.environment config.steps)
...
in (let _25_567 = t2_cfg
in {code = (mk_lam t2_cfg.code); environment = _25_567.environment; stack = _25_567.stack; close = _25_567.close; steps = _25_567.steps})))
end))
end
| args -> begin
...
in (sn tcenv (let _25_579 = config
in {code = t2; environment = env; stack = (let _25_581 = config.stack
in {args = args}); close = _25_579.close; steps = _25_579.steps})))
end
| (_, []) -> begin
...
in (sn tcenv (let _25_589 = config
in {code = t; environment = env; stack = empty_stack; close = _25_589.close; steps = _25_589.steps}))))
end
...
(sn tcenv (let _25_632 = config
in {code = t; environment = _25_632.environment; stack = _25_632.stack; close = _25_632.close; steps = _25_632.steps}))
end
| _ -> begin
(match (config.code.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, comp)) -> begin
(let _25_642 = (sn_binders tcenv bs config.environment config.steps)
in (match (_25_642) with
| (binders, environment) -> begin
(let c2 = (sncomp tcenv (c_config comp environment config.steps))
in (let _25_644 = config
in {code = (wk (Microsoft_FStar_Absyn_Syntax.mk_Typ_fun (binders, c2.code))); environment = _25_644.environment; stack = _25_644.stack; close = _25_644.close; steps = _25_644.steps}))
end))
end
...
(sn tcenv (let _25_667 = config
in {code = t; environment = _25_667.environment; stack = _25_667.stack; close = _25_667.close; steps = _25_667.steps}))
end else begin
...
in (sn tcenv (let _25_672 = config
in {code = t; environment = _25_672.environment; stack = _25_672.stack; close = (close_with_config config pat); steps = _25_672.steps})))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_labeled ((t, l, r, b))) -> begin
if (unmeta config) then begin
(sn tcenv (let _25_681 = config
in {code = t; environment = _25_681.environment; stack = _25_681.stack; close = _25_681.close; steps = _25_681.steps}))
end else begin
...
(let _25_695 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
(let _25_697 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
in (sn tcenv (let _25_701 = config
in {code = t; environment = _25_701.environment; stack = _25_701.stack; close = (close_with_config config lab); steps = _25_701.steps})))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_refresh_label ((t, b, r))) -> begin
if (unmeta config) then begin
(sn tcenv (let _25_709 = config
in {code = t; environment = _25_709.environment; stack = _25_709.stack; close = _25_709.close; steps = _25_709.steps}))
end else begin
...
in (let config = (let _25_716 = config
in {code = t; environment = (let _25_718 = config.environment
in {context = _25_718.context; label_suffix = ((b, sfx))::config.environment.label_suffix}); stack = _25_716.stack; close = _25_716.close; steps = _25_716.steps})
in (sn tcenv config)))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_slack_formula ((t1, t2, flag))) -> begin
if (! (flag)) then begin
(sn tcenv (let _25_727 = config
in {code = (Microsoft_FStar_Absyn_Util.mk_conj t1 t2); environment = _25_727.environment; stack = _25_727.stack; close = _25_727.close; steps = _25_727.steps}))
end else begin
...
in (rebuild (let _25_731 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta (Microsoft_FStar_Absyn_Syntax.Meta_slack_formula ((c1.code, c2.code, flag)))); environment = _25_731.environment; stack = _25_731.stack; close = _25_731.close; steps = _25_731.steps}))))
end
end
...
in (let _25_775 = cfg
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Comp ctconf.code); environment = _25_775.environment; stack = _25_775.stack; close = _25_775.close; steps = _25_775.steps}))
end
...
in (let norm = (fun _25_784 -> (match (_25_784) with
| () -> begin
...
in (let _25_791 = cfg
in {code = c; environment = _25_791.environment; stack = _25_791.stack; close = _25_791.close; steps = _25_791.steps})))
...
in (let _25_803 = ((sn_flags m.Microsoft_FStar_Absyn_Syntax.flags), (sn_args true tcenv cfg.environment cfg.steps m.Microsoft_FStar_Absyn_Syntax.effect_args))
in (match (_25_803) with
| (flags, args) -> begin
...
in (sncomp_typ tcenv (let _25_808 = cfg
in {code = c; environment = _25_808.environment; stack = _25_808.stack; close = _25_808.close; steps = _25_808.steps})))
end
| _ -> begin
(norm ())
end)
...
in (let _25_847 = cfg
in {code = (w (Microsoft_FStar_Absyn_Syntax.mk_Kind_uvar (uv, args))); environment = _25_847.environment; stack = _25_847.stack; close = _25_847.close; steps = _25_847.steps}))
end
...
(let _25_868 = (Microsoft_FStar_Tc_Env.lookup_kind_abbrev tcenv l)
in (match (_25_868) with
| (_, binders, body) -> begin
(let subst = (Microsoft_FStar_Absyn_Util.subst_of_list binders args)
in (snk tcenv (let _25_870 = cfg
in {code = (Microsoft_FStar_Absyn_Util.subst_kind subst body); environment = _25_870.environment; stack = _25_870.stack; close = _25_870.close; steps = _25_870.steps})))
end))
end
| Microsoft_FStar_Absyn_Syntax.Kind_abbrev ((_, k)) -> begin
(snk tcenv (let _25_877 = cfg
in {code = k; environment = _25_877.environment; stack = _25_877.stack; close = _25_877.close; steps = _25_877.steps}))
end
| Microsoft_FStar_Absyn_Syntax.Kind_arrow ((bs, k)) -> begin
(let _25_885 = (sn_binders tcenv bs cfg.environment cfg.steps)
in (match (_25_885) with
| (bs, env) -> begin
(let c2 = (snk tcenv (k_config k env cfg.steps))
in (let _25_895 = (match (c2.code.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Kind_arrow ((bs', k)) -> begin
...
in (match (_25_895) with
| (bs, rhs) -> begin
(let _25_896 = cfg
in {code = (w (Microsoft_FStar_Absyn_Syntax.mk_Kind_arrow (bs, rhs))); environment = _25_896.environment; stack = _25_896.stack; close = _25_896.close; steps = _25_896.steps})
end)))
end))
end
| Microsoft_FStar_Absyn_Syntax.Kind_unknown -> begin
...
in (let config = (let _25_902 = cfg
in {code = e; environment = _25_902.environment; stack = _25_902.stack; close = _25_902.close; steps = _25_902.steps})
...
in (let _25_922 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_app (config.code, args) None config.code.Microsoft_FStar_Absyn_Syntax.pos); environment = _25_922.environment; stack = empty_stack; close = _25_922.close; steps = _25_922.steps})))
end)
...
(wne tcenv (let _25_947 = config
in {code = vc; environment = env; stack = _25_947.stack; close = _25_947.close; steps = _25_947.steps}))
end
| _ -> begin
...
(let args = (Support.List.fold_right (fun a out -> ((a, config.environment))::out) args config.stack.args)
in (let stack = (let _25_958 = config.stack
in {args = args})
in (wne tcenv (let _25_961 = config
in {code = head; environment = _25_961.environment; stack = stack; close = _25_961.close; steps = _25_961.steps}))))
end
...
in (wne tcenv (let _25_976 = config
in {code = body; environment = env; stack = (let _25_978 = config.stack
in {args = args}); close = _25_976.close; steps = _25_976.steps})))
end
| (_, []) -> begin
(let env = (extend_env config.environment entries)
in (let _25_987 = (sn_binders tcenv binders env config.steps)
in (match (_25_987) with
| (binders, env) -> begin
(let mk_abs = (fun t -> (Microsoft_FStar_Absyn_Syntax.mk_Exp_abs (binders, t) None body.Microsoft_FStar_Absyn_Syntax.pos))
in (let c = (wne tcenv (let _25_990 = config
in {code = body; environment = env; stack = (let _25_992 = config.stack
in {args = []}); close = _25_990.close; steps = (no_eta config.steps)}))
in (let _25_995 = c
in {code = (mk_abs c.code); environment = _25_995.environment; stack = _25_995.stack; close = _25_995.close; steps = _25_995.steps})))
end)))
end
| (formal::rest, actual::rest') -> begin
(let m = (match ((formal, actual)) with
...
(let c_e1 = (wne tcenv (let _25_1037 = config
in {code = e1; environment = _25_1037.environment; stack = empty_stack; close = _25_1037.close; steps = _25_1037.steps}))
in (let wn_eqn = (fun _25_1044 -> (match (_25_1044) with
| (pat, w, body) -> begin
...
in (let _25_1085 = x
in {Microsoft_FStar_Absyn_Syntax.v = _25_1085.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = t.code; Microsoft_FStar_Absyn_Syntax.p = _25_1085.Microsoft_FStar_Absyn_Syntax.p})))
in (let norm_btvar = (fun a -> (let k = (snk tcenv (k_config a.Microsoft_FStar_Absyn_Syntax.sort config.environment config.steps))
in (let _25_1090 = a
in {Microsoft_FStar_Absyn_Syntax.v = _25_1090.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = k.code; Microsoft_FStar_Absyn_Syntax.p = _25_1090.Microsoft_FStar_Absyn_Syntax.p})))
...
| Some (w) -> begin
(let c_w = (wne tcenv (let _25_1136 = config
in {code = w; environment = env; stack = empty_stack; close = _25_1136.close; steps = _25_1136.steps}))
in Some (c_w.code))
end)
in (let c_body = (wne tcenv (let _25_1140 = config
in {code = body; environment = env; stack = empty_stack; close = _25_1140.close; steps = _25_1140.steps}))
...
in (rebuild (let _25_1145 = config
in {code = e; environment = _25_1145.environment; stack = _25_1145.stack; close = _25_1145.close; steps = _25_1145.steps}))))))
end
| Microsoft_FStar_Absyn_Syntax.Exp_let (((is_rec, lbs), body)) -> begin
(let _25_1176 = ((Support.List.fold_left (fun _25_1155 _25_1159 -> (match ((_25_1155, _25_1159)) with
| ((env, lbs), (x, t, e)) -> begin
(let c = (wne tcenv (let _25_1160 = config
in {code = e; environment = _25_1160.environment; stack = empty_stack; close = _25_1160.close; steps = _25_1160.steps}))
in (let t = (sn tcenv (t_config t config.environment config.steps))
in (let _25_1173 = (match (x) with
...
in (match (_25_1173) with
| (y, env) -> begin
(env, ((y, t.code, c.code))::lbs)
end))))
end)) (config.environment, [])) lbs)
in (match (_25_1176) with
| (env, lbs) -> begin
(let lbs = (Support.List.rev lbs)
in (let c_body = (wne tcenv (let _25_1178 = config
in {code = body; environment = env; stack = empty_stack; close = _25_1178.close; steps = _25_1178.steps}))
in (let e = (Microsoft_FStar_Absyn_Syntax.mk_Exp_let ((is_rec, lbs), c_body.code) None e.Microsoft_FStar_Absyn_Syntax.pos)
in (rebuild (let _25_1182 = config
in {code = e; environment = _25_1182.environment; stack = _25_1182.stack; close = _25_1182.close; steps = _25_1182.steps})))))
end))
end
| Microsoft_FStar_Absyn_Syntax.Exp_ascribed ((e, t)) -> begin
(let c = (wne tcenv (let _25_1188 = config
in {code = e; environment = _25_1188.environment; stack = _25_1188.stack; close = _25_1188.close; steps = _25_1188.steps}))
in if (is_stack_empty config) then begin
(let t = (sn tcenv (t_config t config.environment config.steps))
in (rebuild (let _25_1192 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_ascribed (c.code, t.code) e.Microsoft_FStar_Absyn_Syntax.pos); environment = _25_1192.environment; stack = _25_1192.stack; close = _25_1192.close; steps = _25_1192.steps})))
end else begin
c
end)
end
| Microsoft_FStar_Absyn_Syntax.Exp_meta (Microsoft_FStar_Absyn_Syntax.Meta_desugared ((e, info))) -> begin
(let c = (wne tcenv (let _25_1199 = config
in {code = e; environment = _25_1199.environment; stack = _25_1199.stack; close = _25_1199.close; steps = _25_1199.steps}))
in if (is_stack_empty config) then begin
(rebuild (let _25_1202 = config
in {code = (Microsoft_FStar_Absyn_Syntax.mk_Exp_meta (Microsoft_FStar_Absyn_Syntax.Meta_desugared ((c.code, info)))); environment = _25_1202.environment; stack = _25_1202.stack; close = _25_1202.close; steps = _25_1202.steps}))
end else begin
c
end)
end)))))
