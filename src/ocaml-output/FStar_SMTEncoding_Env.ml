  ...
  fun lid  ->
    fun a  ->
      let uu___ =
        ...
        let uu___2 = FStar_Ident.string_of_id a.FStar_Syntax_Syntax.ppname
           in
...
let mk_data_tester :
  ...
  fun env  ->
    fun l  ->
      fun x  ->
        let uu___ =
          let uu___1 = FStar_Ident.string_of_lid l  in escape uu___1  in
...
let (varops : varops_t) =
  ...
  let pop uu___ =
    let uu___1 =
      let uu___2 = FStar_ST.op_Bang scopes  in FStar_List.tl uu___2  in
    FStar_ST.op_Colon_Equals scopes uu___1  in
                 ...
                 (fun _i  ->
                    fun uu___  ->
                      fun acc1  ->
                        match uu___ with
                        | (x,_term) ->
                            let uu___1 = FStar_Syntax_Print.bv_to_string x
                               in
                            uu___1 :: acc1) acc) []
       in
    let allvars =
...
let (new_term_constant_and_tok_from_lid :
  ...
  fun env  ->
    fun x  ->
      fun arity  ->
        let uu___ = new_term_constant_and_tok_from_lid_aux env x arity false
           in
        match uu___ with
                                ...
                                let uu___7 =
                                  let uu___8 =
                                    FStar_SMTEncoding_Term.mk_fv
                                     ...
                                     in
                                  FStar_All.pipe_left
                                    FStar_SMTEncoding_Util.mkFreeV uu___8
                                   in
                                FStar_SMTEncoding_Term.mk_ApplyTF uu___7 fuel
                                 in
                              FStar_All.pipe_left
                                (fun uu___7  ->
