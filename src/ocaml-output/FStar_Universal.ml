...
let _88_188 = (FStar_All.pipe_right fmods (FStar_List.fold_left (fun _88_181 m -> (match (_88_181) with
| (env, all_mods) -> begin
(

let _88_185 = (FStar_TypeChecker_Tc.check_module env m)
in (match (_88_185) with
| (m, env) -> begin
(env, (m)::all_mods)
end))
end)) (env, [])))
in (match (_88_188) with
| (env, all_mods) -> begin
...
let _88_199 = (FStar_Syntax_Syntax.reset_gensym ())
in (

let _88_204 = acc
in (match (_88_204) with
| (all_mods, dsenv, env) -> begin
(

let _88_229 = (match (intf) with
| None -> begin
(tc_one_file dsenv env intf impl)
end
| Some (_88_207) when ((FStar_Options.codegen ()) <> None) -> begin
(

let _88_209 = if (not ((FStar_Options.lax ()))) then begin
...
let _88_213 = (FStar_Util.print1 "Interleaving iface+module: %s\n" iname)
in (

let caption = (Prims.strcat "interface: " iname)
in (

let _88_218 = (push_context (dsenv, env) caption)
in (match (_88_218) with
| (dsenv', env') -> begin
(

let _88_223 = (tc_one_file dsenv' env' intf impl)
in (match (_88_223) with
| (_88_220, dsenv', env') -> begin
(

let _88_224 = (pop_context (dsenv', env') caption)
...
let _88_246 = (tc_fold_interleave ([], dsenv, env) filenames)
in (match (_88_246) with
| (all_mods, dsenv, env) -> begin
(

let _88_247 = if ((FStar_Options.interactive ()) && ((FStar_TypeChecker_Errors.get_err_count ()) = 0)) then begin
...
let _88_253 = (tc_prims ())
in (match (_88_253) with
...
let _88_259 = if ((not ((FStar_Options.explicit_deps ()))) && (FStar_Options.debug_any ())) then begin
(

let _88_255 = (FStar_Util.print_endline "Auto-deps kicked in; here\'s some info.")
in (

let _88_257 = (FStar_Util.print1 "Here\'s the list of filenames we will process: %s\n" (FStar_String.concat " " filenames))
in (let _178_99 = (let _178_98 = (FStar_Options.verify_module ())
in (FStar_String.concat " " _178_98))
...
let _88_264 = (batch_mode_tc_no_prims dsenv env filenames)
in (match (_88_264) with
| (all_mods, dsenv, env) -> begin
