...
let uu____433 = (free_names_and_uvars t1 use_cache)
in (free_names_and_uvars_args args uu____433 use_cache))
end
...
let uu____497 = (free_names_and_uvars t1 use_cache)
in (FStar_List.fold_left (fun n1 uu____520 -> (match (uu____520) with
| (p, wopt, t2) -> begin
(
...
let uu____558 = (FStar_Syntax_Syntax.pat_bvs p)
in (FStar_All.pipe_right uu____558 (FStar_List.fold_left (fun n3 x -> (

let uu____568 = (free_names_and_uvars x.FStar_Syntax_Syntax.sort use_cache)
in (union n3 uu____568))) n1)))
in (

let uu____569 = (union n11 n2)
in (union n3 uu____569)))))
end)) uu____497))
in (FStar_All.pipe_right pats uu____478))
end
| FStar_Syntax_Syntax.Tm_ascribed (t1, asc, uu____586) -> begin
(
...
let uu____674 = (union u1 u2)
in (

let uu____675 = (free_names_and_uvars tac use_cache)
in (union uu____674 uu____675)))
end)))
end
...
let uu____701 = (free_names_and_uvars t1 use_cache)
in (FStar_List.fold_left (fun n1 lb -> (

let uu____707 = (

let uu____708 = (free_names_and_uvars lb.FStar_Syntax_Syntax.lbtyp use_cache)
in (

let uu____709 = (free_names_and_uvars lb.FStar_Syntax_Syntax.lbdef use_cache)
in (union uu____708 uu____709)))
in (union n1 uu____707))) uu____701))
in (FStar_All.pipe_right (FStar_Pervasives_Native.snd lbs) uu____694))
end
...
| FStar_Syntax_Syntax.Meta_monadic (uu____770, t') -> begin
(

let uu____776 = (free_names_and_uvars t' use_cache)
in (union u1 uu____776))
end
| FStar_Syntax_Syntax.Meta_monadic_lift (uu____777, uu____778, t') -> begin
(

let uu____784 = (free_names_and_uvars t' use_cache)
in (union u1 uu____784))
end
| FStar_Syntax_Syntax.Meta_labeled (uu____785) -> begin
u1
end
| FStar_Syntax_Syntax.Meta_desugared (uu____792) -> begin
u1
end
| FStar_Syntax_Syntax.Meta_named (uu____793) -> begin
u1
end))
end))))
...
let uu____799 = (FStar_ST.op_Bang t1.FStar_Syntax_Syntax.vars)
in (match (uu____799) with
| FStar_Pervasives_Native.Some (n1) when (

let uu____830 = (should_invalidate_cache n1 use_cache)
in (not (uu____830))) -> begin
(

let uu____831 = (FStar_Syntax_Syntax.new_fv_set ())
in ((n1), (uu____831)))
end
| uu____836 -> begin
...
and free_names_and_uvars_args : (FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax * FStar_Syntax_Syntax.aqual) Prims.list  ->  (FStar_Syntax_Syntax.free_vars * FStar_Ident.lident FStar_Util.set)  ->  Prims.bool  ->  (FStar_Syntax_Syntax.free_vars * FStar_Ident.lident FStar_Util.set) = (fun args acc use_cache -> (FStar_All.pipe_right args (FStar_List.fold_left (fun n1 uu____941 -> (match (uu____941) with
| (x, uu____949) -> begin
(

let uu____954 = (free_names_and_uvars x use_cache)
in (union n1 uu____954))
end)) acc)))
and free_names_and_uvars_binders : FStar_Syntax_Syntax.binders  ->  free_vars_and_fvars  ->  Prims.bool  ->  free_vars_and_fvars = (fun bs acc use_cache -> (FStar_All.pipe_right bs (FStar_List.fold_left (fun n1 uu____974 -> (match (uu____974) with
| (x, uu____980) -> begin
(

let uu____981 = (free_names_and_uvars x.FStar_Syntax_Syntax.sort use_cache)
in (union n1 uu____981))
end)) acc)))
...
let uu____986 = (FStar_ST.op_Bang c.FStar_Syntax_Syntax.vars)
in (match (uu____986) with
| FStar_Pervasives_Native.Some (n1) -> begin
(

let uu____1017 = (should_invalidate_cache n1 use_cache)
...
let uu____1047 = (FStar_Syntax_Syntax.new_fv_set ())
in ((n1), (uu____1047)))
end))
end
| uu____1052 -> begin
...
let uu____1090 = (free_univs u)
in (

let uu____1091 = (free_names_and_uvars t use_cache)
in (union uu____1090 uu____1091)))
end
| FStar_Syntax_Syntax.Total (t, FStar_Pervasives_Native.Some (u)) -> begin
(

let uu____1100 = (free_univs u)
in (

let uu____1101 = (free_names_and_uvars t use_cache)
in (union uu____1100 uu____1101)))
end
...
let uu____1110 = (free_names_and_uvars ct.FStar_Syntax_Syntax.result_typ use_cache)
in (free_names_and_uvars_args ct.FStar_Syntax_Syntax.effect_args uu____1110 use_cache))
in (FStar_List.fold_left (fun us1 u -> (

let uu____1122 = (free_univs u)
in (union us1 uu____1122))) us ct.FStar_Syntax_Syntax.comp_univs))
end)
...
let uu____1161 = (FStar_Syntax_Unionfind.find u.FStar_Syntax_Syntax.ctx_uvar_head)
in (match (uu____1161) with
| FStar_Pervasives_Native.Some (uu____1164) -> begin
true
end
| uu____1165 -> begin
false
...
let uu____1174 = (FStar_Syntax_Unionfind.univ_find u)
in (match (uu____1174) with
| FStar_Pervasives_Native.Some (uu____1177) -> begin
true
end
| FStar_Pervasives_Native.None -> begin
false
...
let uu____1188 = (FStar_Syntax_Unionfind.uvar_id uv1.FStar_Syntax_Syntax.ctx_uvar_head)
in (

let uu____1189 = (FStar_Syntax_Unionfind.uvar_id uv2.FStar_Syntax_Syntax.ctx_uvar_head)
in (uu____1188 - uu____1189))))


let new_uv_set : unit  ->  FStar_Syntax_Syntax.uvars = (fun uu____1194 -> (FStar_Util.new_set compare_uv))


let compare_universe_uvar : FStar_Syntax_Syntax.universe_uvar  ->  FStar_Syntax_Syntax.universe_uvar  ->  Prims.int = (fun x y -> (

let uu____1205 = (FStar_Syntax_Unionfind.univ_uvar_id x)
in (

let uu____1206 = (FStar_Syntax_Unionfind.univ_uvar_id y)
in (uu____1205 - uu____1206))))


let new_universe_uvar_set : unit  ->  FStar_Syntax_Syntax.universe_uvar FStar_Util.set = (fun uu____1213 -> (FStar_Util.new_set compare_universe_uvar))
...
let uu____1229 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1229))
in uu____1228.FStar_Syntax_Syntax.free_names)
in (FStar_Util.as_set uu____1225 FStar_Syntax_Syntax.order_bv)))
...
let uu____1249 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1249))
in uu____1248.FStar_Syntax_Syntax.free_uvars)
in (FStar_Util.as_set uu____1245 compare_uv)))
...
let uu____1269 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1269))
in uu____1268.FStar_Syntax_Syntax.free_univs)
in (FStar_Util.as_set uu____1265 compare_universe_uvar)))
...
let uu____1289 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1289))
in uu____1288.FStar_Syntax_Syntax.free_univ_names)
in (FStar_Util.as_set uu____1285 FStar_Syntax_Syntax.order_univ_name)))
...
let uu____1309 = (free_names_and_uvars_comp c true)
in (FStar_Pervasives_Native.fst uu____1309))
in uu____1308.FStar_Syntax_Syntax.free_univ_names)
in (FStar_Util.as_set uu____1305 FStar_Syntax_Syntax.order_univ_name)))


let fvars : FStar_Syntax_Syntax.term  ->  FStar_Ident.lident FStar_Util.set = (fun t -> (

let uu____1325 = (free_names_and_uvars t false)
in (FStar_Pervasives_Native.snd uu____1325)))
...
let uu____1345 = (free_names_and_uvars_binders bs no_free_vars true)
in (FStar_Pervasives_Native.fst uu____1345))
in uu____1344.FStar_Syntax_Syntax.free_names)
in (FStar_Util.as_set uu____1341 FStar_Syntax_Syntax.order_bv)))
