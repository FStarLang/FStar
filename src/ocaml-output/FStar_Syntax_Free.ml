...
let from_binders = (FStar_All.pipe_right bs (FStar_List.fold_left (fun n1 uu____321 -> (match (uu____321) with
| (x, uu____329) -> begin
(

let uu____334 = (free_names_and_uvars x.FStar_Syntax_Syntax.sort use_cache)
in (union n1 uu____334))
end)) no_free_vars))
...
| FStar_Syntax_Syntax.Tm_delayed (uu____336) -> begin
(failwith "Impossible")
end
| FStar_Syntax_Syntax.Tm_name (x) -> begin
(singleton_bv x)
end
| FStar_Syntax_Syntax.Tm_uvar (uv, uu____362) -> begin
(singleton_uv uv)
end
| FStar_Syntax_Syntax.Tm_type (u) -> begin
(free_univs u)
end
| FStar_Syntax_Syntax.Tm_bvar (uu____380) -> begin
no_free_vars
end
| FStar_Syntax_Syntax.Tm_fvar (fv) -> begin
(singleton_fvar fv)
end
| FStar_Syntax_Syntax.Tm_constant (uu____382) -> begin
no_free_vars
end
| FStar_Syntax_Syntax.Tm_lazy (uu____383) -> begin
no_free_vars
end
...
let uu____396 = (free_univs u)
in (union out uu____396))) f us))
end
| FStar_Syntax_Syntax.Tm_abs (bs, t1, uu____399) -> begin
(

let uu____424 = (free_names_and_uvars t1 use_cache)
in (aux_binders bs uu____424))
end
| FStar_Syntax_Syntax.Tm_arrow (bs, c) -> begin
(

let uu____447 = (free_names_and_uvars_comp c use_cache)
in (aux_binders bs uu____447))
end
| FStar_Syntax_Syntax.Tm_refine (bv, t1) -> begin
(

let uu____454 = (free_names_and_uvars t1 use_cache)
in (aux_binders ((((bv), (FStar_Pervasives_Native.None)))::[]) uu____454))
end
| FStar_Syntax_Syntax.Tm_app (t1, args) -> begin
(

let uu____495 = (free_names_and_uvars t1 use_cache)
in (free_names_and_uvars_args args uu____495 use_cache))
end
...
let uu____559 = (free_names_and_uvars t1 use_cache)
in (FStar_List.fold_left (fun n1 uu____582 -> (match (uu____582) with
| (p, wopt, t2) -> begin
(
...
let uu____620 = (FStar_Syntax_Syntax.pat_bvs p)
in (FStar_All.pipe_right uu____620 (FStar_List.fold_left (fun n3 x -> (

let uu____630 = (free_names_and_uvars x.FStar_Syntax_Syntax.sort use_cache)
in (union n3 uu____630))) n1)))
in (

let uu____631 = (union n11 n2)
in (union n3 uu____631)))))
end)) uu____559))
in (FStar_All.pipe_right pats uu____540))
end
| FStar_Syntax_Syntax.Tm_ascribed (t1, asc, uu____648) -> begin
(
...
let uu____737 = (free_names_and_uvars tac use_cache)
in (union uu____736 uu____737)))
end)))
end
...
let uu____765 = (free_names_and_uvars t1 use_cache)
in (FStar_List.fold_left (fun n1 lb -> (

let uu____771 = (

let uu____772 = (free_names_and_uvars lb.FStar_Syntax_Syntax.lbtyp use_cache)
in (

let uu____773 = (free_names_and_uvars lb.FStar_Syntax_Syntax.lbdef use_cache)
in (union uu____772 uu____773)))
in (union n1 uu____771))) uu____765))
in (FStar_All.pipe_right (FStar_Pervasives_Native.snd lbs) uu____758))
end
...
let u1 = (free_names_and_uvars t1 use_cache)
in (match (m) with
| FStar_Syntax_Syntax.Meta_pattern (uu____792, args) -> begin
(FStar_List.fold_right (fun a acc -> (free_names_and_uvars_args a acc use_cache)) args u1)
end
| FStar_Syntax_Syntax.Meta_monadic (uu____862, t') -> begin
(

let uu____868 = (free_names_and_uvars t' use_cache)
in (union u1 uu____868))
end
| FStar_Syntax_Syntax.Meta_monadic_lift (uu____869, uu____870, t') -> begin
(

let uu____876 = (free_names_and_uvars t' use_cache)
in (union u1 uu____876))
end
| FStar_Syntax_Syntax.Meta_labeled (uu____877) -> begin
u1
end
| FStar_Syntax_Syntax.Meta_desugared (uu____886) -> begin
u1
end
| FStar_Syntax_Syntax.Meta_named (uu____887) -> begin
u1
end))
end))))
...
let uu____894 = (FStar_ST.op_Bang t1.FStar_Syntax_Syntax.vars)
in (match (uu____894) with
| FStar_Pervasives_Native.Some (n1) when (

let uu____921 = (should_invalidate_cache n1 use_cache)
in (not (uu____921))) -> begin
(

let uu____923 = (FStar_Syntax_Syntax.new_fv_set ())
in ((n1), (uu____923)))
end
| uu____928 -> begin
...
and free_names_and_uvars_args : (FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax * FStar_Syntax_Syntax.arg_qualifier FStar_Pervasives_Native.option) Prims.list  ->  (FStar_Syntax_Syntax.free_vars * FStar_Ident.lident FStar_Util.set)  ->  Prims.bool  ->  (FStar_Syntax_Syntax.free_vars * FStar_Ident.lident FStar_Util.set) = (fun args acc use_cache -> (FStar_All.pipe_right args (FStar_List.fold_left (fun n1 uu____1032 -> (match (uu____1032) with
| (x, uu____1042) -> begin
(

let uu____1051 = (free_names_and_uvars x use_cache)
in (union n1 uu____1051))
end)) acc)))
and free_names_and_uvars_binders : FStar_Syntax_Syntax.binders  ->  free_vars_and_fvars  ->  Prims.bool  ->  free_vars_and_fvars = (fun bs acc use_cache -> (FStar_All.pipe_right bs (FStar_List.fold_left (fun n1 uu____1076 -> (match (uu____1076) with
| (x, uu____1084) -> begin
(

let uu____1089 = (free_names_and_uvars x.FStar_Syntax_Syntax.sort use_cache)
in (union n1 uu____1089))
end)) acc)))
...
let uu____1095 = (FStar_ST.op_Bang c.FStar_Syntax_Syntax.vars)
in (match (uu____1095) with
| FStar_Pervasives_Native.Some (n1) -> begin
(

let uu____1122 = (should_invalidate_cache n1 use_cache)
...
let uu____1151 = (FStar_Syntax_Syntax.new_fv_set ())
in ((n1), (uu____1151)))
end))
end
| uu____1156 -> begin
...
let uu____1194 = (free_univs u)
in (

let uu____1195 = (free_names_and_uvars t use_cache)
in (union uu____1194 uu____1195)))
end
| FStar_Syntax_Syntax.Total (t, FStar_Pervasives_Native.Some (u)) -> begin
(

let uu____1204 = (free_univs u)
in (

let uu____1205 = (free_names_and_uvars t use_cache)
in (union uu____1204 uu____1205)))
end
...
let uu____1214 = (free_names_and_uvars ct.FStar_Syntax_Syntax.result_typ use_cache)
in (free_names_and_uvars_args ct.FStar_Syntax_Syntax.effect_args uu____1214 use_cache))
in (FStar_List.fold_left (fun us1 u -> (

let uu____1226 = (free_univs u)
in (union us1 uu____1226))) us ct.FStar_Syntax_Syntax.comp_univs))
end)
...
let uu____1263 = (FStar_Syntax_Unionfind.find u.FStar_Syntax_Syntax.ctx_uvar_head)
in (match (uu____1263) with
| FStar_Pervasives_Native.Some (uu____1267) -> begin
true
end
| uu____1269 -> begin
false
...
let uu____1280 = (FStar_Syntax_Unionfind.univ_find u)
in (match (uu____1280) with
| FStar_Pervasives_Native.Some (uu____1284) -> begin
true
end
| FStar_Pervasives_Native.None -> begin
false
...
let uu____1299 = (FStar_Syntax_Unionfind.uvar_id uv1.FStar_Syntax_Syntax.ctx_uvar_head)
in (

let uu____1301 = (FStar_Syntax_Unionfind.uvar_id uv2.FStar_Syntax_Syntax.ctx_uvar_head)
in (uu____1299 - uu____1301))))


let new_uv_set : unit  ->  FStar_Syntax_Syntax.uvars = (fun uu____1308 -> (FStar_Util.new_set compare_uv))


let compare_universe_uvar : FStar_Syntax_Syntax.universe_uvar  ->  FStar_Syntax_Syntax.universe_uvar  ->  Prims.int = (fun x y -> (

let uu____1321 = (FStar_Syntax_Unionfind.univ_uvar_id x)
in (

let uu____1323 = (FStar_Syntax_Unionfind.univ_uvar_id y)
in (uu____1321 - uu____1323))))


let new_universe_uvar_set : unit  ->  FStar_Syntax_Syntax.universe_uvar FStar_Util.set = (fun uu____1332 -> (FStar_Util.new_set compare_universe_uvar))
...
let uu____1350 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1350))
in uu____1349.FStar_Syntax_Syntax.free_names)
in (FStar_Util.as_set uu____1346 FStar_Syntax_Syntax.order_bv)))
...
let uu____1372 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1372))
in uu____1371.FStar_Syntax_Syntax.free_uvars)
in (FStar_Util.as_set uu____1368 compare_uv)))
...
let uu____1394 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1394))
in uu____1393.FStar_Syntax_Syntax.free_univs)
in (FStar_Util.as_set uu____1390 compare_universe_uvar)))
...
let uu____1416 = (free_names_and_uvars t true)
in (FStar_Pervasives_Native.fst uu____1416))
in uu____1415.FStar_Syntax_Syntax.free_univ_names)
in (FStar_Util.as_set uu____1412 FStar_Syntax_Syntax.order_univ_name)))
...
let uu____1438 = (free_names_and_uvars_comp c true)
in (FStar_Pervasives_Native.fst uu____1438))
in uu____1437.FStar_Syntax_Syntax.free_univ_names)
in (FStar_Util.as_set uu____1434 FStar_Syntax_Syntax.order_univ_name)))


let fvars : FStar_Syntax_Syntax.term  ->  FStar_Ident.lident FStar_Util.set = (fun t -> (

let uu____1456 = (free_names_and_uvars t false)
in (FStar_Pervasives_Native.snd uu____1456)))
...
in (FStar_Pervasives_Native.fst uu____1478))
in uu____1477.FStar_Syntax_Syntax.free_names)
in (FStar_Util.as_set uu____1474 FStar_Syntax_Syntax.order_bv)))
