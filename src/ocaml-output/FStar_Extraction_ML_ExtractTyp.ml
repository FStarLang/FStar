...
let rec argIsExp = (fun k typeName -> (match ((let _129_7 = (FStar_Absyn_Util.compress_kind k)
in _129_7.FStar_Absyn_Syntax.n)) with
| FStar_Absyn_Syntax.Kind_type -> begin
[]
end
| FStar_Absyn_Syntax.Kind_arrow (bs, r) -> begin
(let _129_9 = (FStar_List.map binderIsExp bs)
in (let _129_8 = (argIsExp r typeName)
in (FStar_List.append _129_9 _129_8)))
end
...
let numIndices = (fun k typeName -> (let _129_14 = (argIsExp k typeName)
in (FStar_List.length _129_14)))
...
| h1::h2::tl -> begin
(let _129_34 = (let _129_33 = (curry ((h2)::tl) f out)
in (h1, FStar_Extraction_ML_Syntax.E_PURE, _129_33))
in FStar_Extraction_ML_Syntax.MLTY_Fun (_129_34))
end))
...
(let _129_86 = (FStar_Extraction_ML_Util.mkTypApp tyin (FStar_List.append argsin arrgs) ty)
in (extractTyp c _129_86))
end
| _62_127 -> begin
FStar_Extraction_ML_Env.unknownType
end)
...
in (let _129_98 = (let _129_97 = (FStar_Extraction_ML_Syntax.mlpath_of_lident ftv.FStar_Absyn_Syntax.v)
in ((FStar_List.append mlargs argCompletion), _129_97))
in FStar_Extraction_ML_Syntax.MLTY_Named (_129_98)))
end)))))
end else begin
...
(let _129_102 = (extractKind c btv.FStar_Absyn_Syntax.sort)
in (let _129_101 = (extendContextAsTyvar false (FStar_Util.Inl (btv)) c)
in (_129_102, _129_101)))
end
| (FStar_Util.Inr (bvv), _62_210) -> begin
(let _129_104 = (extractTyp c bvv.FStar_Absyn_Syntax.sort)
in (let _129_103 = (extendContextAsTyvar false (FStar_Util.Inr (bvv)) c)
in (_129_104, _129_103)))
end))
and extractBindersTypes = (fun c bs -> (let _129_110 = (FStar_List.fold_left (fun _62_216 b -> (match (_62_216) with
| (lt, cp) -> begin
...
(let _129_117 = (extractTyp c ty)
in (_129_117, FStar_Extraction_ML_Syntax.E_PURE))
end
| FStar_Absyn_Syntax.Comp (cm) -> begin
(let _129_119 = (extractTyp c cm.FStar_Absyn_Syntax.result_typ)
in (let _129_118 = (translate_eff c cm.FStar_Absyn_Syntax.effect_name)
in (_129_119, _129_118)))
end))
...
if (l = l') then begin
(let t = (let _129_167 = (FStar_List.map (fun _62_291 -> (match (_62_291) with
| (x, _62_290) -> begin
(x, Some (FStar_Absyn_Syntax.Implicit))
end)) tps)
in (FStar_Absyn_Util.close_typ _129_167 t))
in Some (t))
end else begin
None
end
...
let parseInductiveConstructors = (fun c cnames sigb -> (FStar_List.map (fun h -> (let _129_175 = (lookupDataConType c sigb h)
in {cname = h; ctype = _129_175})) cnames))
...
(let _129_199 = (totalType_of_comp cp)
in (ll, _129_199))
end else begin
(let _129_200 = (FStar_Extraction_ML_Util.mkTypFun lr cp t)
in (ll, _129_200))
end
end))
end
...
in (let newC = (let _129_215 = (FStar_List.map (fun _62_420 -> (match (_62_420) with
| (x, y) -> begin
((Prims.fst x), (Prims.fst y))
end)) lp)
in (extendContextWithRepAsTyVars _129_215 c))
in (let mlt = (let _129_216 = (extractTyp newC tr)
in (FStar_Extraction_ML_Util.eraseTypeDeep c _129_216))
in (let tys = (let _129_217 = (FStar_List.map mlTyIdentOfBinder tyBinders)
in (_129_217, mlt))
in (let fvv = (FStar_Extraction_ML_Env.mkFvvar ctor.cname ctor.ctype)
in (let _129_220 = (FStar_Extraction_ML_Env.extend_fv c fvv tys false)
in (let _129_219 = (let _129_218 = (argTypes mlt)
in ((lident2mlsymbol ctor.cname), _129_218))
in (_129_220, _129_219)))))))))
end)))

let rec firstNNats = (fun n -> if (0 < n) then begin
(let _129_223 = (firstNNats (n - 1))
in (n)::_129_223)
end else begin
[]
end)

let dummyIdent = (fun n -> (let _129_227 = (let _129_226 = (FStar_Util.string_of_int n)
in (Prims.strcat "\'dummyV" _129_226))
in (_129_227, 0)))

let dummyIndexIdents = (fun n -> (let _129_230 = (firstNNats n)
in (FStar_List.map dummyIdent _129_230)))
...
| (nc, tyb) -> begin
(let mlbs = (let _129_236 = (FStar_List.map mlTyIdentOfBinder ind.tyBinders)
in (let _129_235 = (dummyIndexIdents nIndices)
in (FStar_List.append _129_236 _129_235)))
...
(let _62_470 = (let _129_246 = (let _129_245 = (mfst bs)
in (extendContext c _129_245))
in (headBinders _129_246 t))
...
in (let c = (let _129_251 = (mfst bs)
in (extendContext c _129_251))
...
in (let td = (let _129_252 = (FStar_List.map mlTyIdentOfBinder bs)
in ((mlsymbolOfLident l), _129_252, Some (tyDecBody)))
...
in (let ex_decl = (let _129_258 = (let _129_257 = (argTypes mlt)
in ((lident2mlsymbol exnConstr.cname), _129_257))
in FStar_Extraction_ML_Syntax.MLM_Exn (_129_258))
in (let _129_259 = (FStar_Extraction_ML_Env.extend_fv c fvv tys false)
in (_129_259, ex_decl))))))))
...
| (c, tds) -> begin
(let _129_264 = if (FStar_All.pipe_right quals (FStar_List.contains FStar_Absyn_Syntax.Logic)) then begin
[]
end else begin
...
(let _62_527 = ()
in (let _62_531 = (let _129_265 = (FStar_List.hd exConstrs)
in (extractExn c _129_265))
