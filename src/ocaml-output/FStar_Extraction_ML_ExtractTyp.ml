...
let rec argIsExp : FStar_Absyn_Syntax.knd  ->  Prims.string  ->  Prims.bool Prims.list = (fun k typeName -> (match ((let _182_7 = (FStar_Absyn_Util.compress_kind k)
in _182_7.FStar_Absyn_Syntax.n)) with
| FStar_Absyn_Syntax.Kind_type -> begin
[]
end
| FStar_Absyn_Syntax.Kind_arrow (bs, r) -> begin
(let _182_9 = (FStar_List.map binderIsExp bs)
in (let _182_8 = (argIsExp r typeName)
in (FStar_List.append _182_9 _182_8)))
end
| FStar_Absyn_Syntax.Kind_delayed (k, _79_13, _79_15) -> begin
(FStar_All.failwith "extraction.numIndices : expected a compressed argument")
end
| FStar_Absyn_Syntax.Kind_abbrev (_79_19, k) -> begin
(argIsExp k typeName)
end
| _79_24 -> begin
...
let numIndices : FStar_Absyn_Syntax.knd  ->  Prims.string  ->  Prims.nat = (fun k typeName -> (let _182_14 = (argIsExp k typeName)
in (FStar_List.length _182_14)))
...
in (let _79_42 = (FStar_Util.smap_add cache l.FStar_Ident.str res)
in res))
end))
in delta_norm_eff))
...
| h1::h2::tl -> begin
(let _182_34 = (let _182_33 = (curry ((h2)::tl) f out)
in (h1, FStar_Extraction_ML_Syntax.E_PURE, _182_33))
in FStar_Extraction_ML_Syntax.MLTY_Fun (_182_34))
end))
...
| (FStar_Util.Inr (bv), FStar_Util.Inr (_79_68)) -> begin
...
(let _79_104 = (extractBindersTypes c bs)
in (match (_79_104) with
| (bts, newC) -> begin
(let _79_107 = (extractComp newC codomain)
in (match (_79_107) with
| (codomainML, erase) -> begin
(curry bts erase codomainML)
end))
end))
end
| FStar_Absyn_Syntax.Typ_refine (bv, _79_110) -> begin
...
(let _182_86 = (FStar_Extraction_ML_Util.mkTypApp tyin (FStar_List.append argsin arrgs) ty)
in (extractTyp c _182_86))
end
| _79_127 -> begin
FStar_Extraction_ML_Env.unknownType
end)
...
(let _79_135 = (extractBindersTypes c bs)
in (match (_79_135) with
| (bts, c) -> begin
(extractTyp c ty)
end))
end
| FStar_Absyn_Syntax.Typ_ascribed (ty, _79_138) -> begin
(extractTyp c ty)
end
| FStar_Absyn_Syntax.Typ_meta (mt) -> begin
(extractMeta c mt)
end
| FStar_Absyn_Syntax.Typ_uvar (_79_144) -> begin
FStar_Extraction_ML_Env.unknownType
end
| FStar_Absyn_Syntax.Typ_delayed (_79_147) -> begin
...
in (let _79_198 = (FStar_Util.first_N (FStar_List.length mlargs) ar)
in (match (_79_198) with
| (_79_196, missingArgs) -> begin
(let argCompletion = (FStar_List.map mlty_of_isExp missingArgs)
in (let _182_98 = (let _182_97 = (FStar_Extraction_ML_Syntax.mlpath_of_lident ftv.FStar_Absyn_Syntax.v)
in ((FStar_List.append mlargs argCompletion), _182_97))
in FStar_Extraction_ML_Syntax.MLTY_Named (_182_98)))
end)))))
end else begin
...
| (FStar_Util.Inl (btv), _79_205) -> begin
(let _182_102 = (extractKind c btv.FStar_Absyn_Syntax.sort)
in (let _182_101 = (extendContextAsTyvar false (FStar_Util.Inl (btv)) c)
in (_182_102, _182_101)))
end
| (FStar_Util.Inr (bvv), _79_210) -> begin
(let _182_104 = (extractTyp c bvv.FStar_Absyn_Syntax.sort)
in (let _182_103 = (extendContextAsTyvar false (FStar_Util.Inr (bvv)) c)
in (_182_104, _182_103)))
end))
and extractBindersTypes : context  ->  FStar_Absyn_Syntax.binder Prims.list  ->  (FStar_Extraction_ML_Syntax.mlty Prims.list * context) = (fun c bs -> (let _182_110 = (FStar_List.fold_left (fun _79_216 b -> (match (_79_216) with
| (lt, cp) -> begin
(let _79_220 = (extractBinderType cp b)
in (match (_79_220) with
| (nt, nc) -> begin
((nt)::lt, nc)
end))
end)) ([], c) bs)
in ((fun _79_223 -> (match (_79_223) with
| (x, c) -> begin
((FStar_List.rev x), c)
end)) _182_110)))
...
(let _182_117 = (extractTyp c ty)
in (_182_117, FStar_Extraction_ML_Syntax.E_PURE))
end
| FStar_Absyn_Syntax.Comp (cm) -> begin
(let _182_119 = (extractTyp c cm.FStar_Absyn_Syntax.result_typ)
in (let _182_118 = (translate_eff c cm.FStar_Absyn_Syntax.effect_name)
in (_182_119, _182_118)))
end))

let binderPPnames : FStar_Absyn_Syntax.binder  ->  FStar_Ident.ident = (fun bn -> (match (bn) with
| (FStar_Util.Inl (btv), _79_238) -> begin
btv.FStar_Absyn_Syntax.v.FStar_Absyn_Syntax.ppname
end
| (FStar_Util.Inr (bvv), _79_243) -> begin
...
| (FStar_Util.Inl (btv), _79_249) -> begin
btv.FStar_Absyn_Syntax.v.FStar_Absyn_Syntax.realname
end
| (FStar_Util.Inr (bvv), _79_254) -> begin
...
| FStar_Absyn_Syntax.Sig_datacon (l', t, (_79_277, tps, _79_280), quals, lids, _79_285) -> begin
if (l = l') then begin
(let t = (let _182_167 = (FStar_List.map (fun _79_291 -> (match (_79_291) with
| (x, _79_290) -> begin
(x, Some (FStar_Absyn_Syntax.Implicit))
end)) tps)
in (FStar_Absyn_Util.close_typ _182_167 t))
in Some (t))
end else begin
None
end
...
let parseInductiveConstructors : context  ->  FStar_Ident.lident Prims.list  ->  FStar_Absyn_Syntax.sigelts  ->  inductiveConstructor Prims.list = (fun c cnames sigb -> (FStar_List.map (fun h -> (let _182_175 = (lookupDataConType c sigb h)
in {cname = h; ctype = _182_175})) cnames))
...
| FStar_Absyn_Syntax.Sig_tycon (l, bs, kk, _79_308, constrs, qs, _79_312)::tlsig -> begin
(let indConstrs = (parseInductiveConstructors c constrs tlsig)
in (let _79_320 = (parseInductiveTypesFromSigBundle c tlsig)
in (match (_79_320) with
| (inds, abbs, exns) -> begin
(({tyName = l; k = kk; tyBinders = bs; constructors = indConstrs; qualifiers = qs})::inds, abbs, exns)
end)))
end
| FStar_Absyn_Syntax.Sig_datacon (l, _79_324, tc, quals, lids, _79_329)::tlsig -> begin
...
| FStar_Absyn_Syntax.Sig_typ_abbrev (l, bs, _79_340, t, _79_343, _79_345)::tlsig -> begin
(let _79_352 = (parseInductiveTypesFromSigBundle c tlsig)
in (match (_79_352) with
| (inds, abbs, exns) -> begin
(inds, ({abTyName = l; abTyBinders = bs; abBody = t})::abbs, exns)
end))
end
| se::tlsig -> begin
(let _182_181 = (let _182_180 = (FStar_Absyn_Print.sigelt_to_string se)
in (FStar_Util.format1 "unexpected content in a  sig bundle : %s\n" _182_180))
in (FStar_All.failwith _182_181))
end))

let rec argTypes : FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty Prims.list = (fun t -> (match (t) with
| FStar_Extraction_ML_Syntax.MLTY_Fun (a, _79_359, b) -> begin
(let _182_184 = (argTypes b)
in (a)::_182_184)
end
| _79_364 -> begin
[]
...
in (match (t.FStar_Absyn_Syntax.n) with
| FStar_Absyn_Syntax.Typ_fun (lb, cp) -> begin
(let _79_390 = (FStar_Util.first_N n lb)
in (match (_79_390) with
| (ll, lr) -> begin
if (FStar_List.isEmpty lr) then begin
(let _182_199 = (totalType_of_comp cp)
in (ll, _182_199))
end else begin
(let _182_200 = (FStar_Extraction_ML_Util.mkTypFun lr cp t)
in (ll, _182_200))
end
end))
end
...
let extractCtor : FStar_Absyn_Syntax.binder Prims.list  ->  context  ->  inductiveConstructor  ->  (context * (FStar_Extraction_ML_Syntax.mlsymbol * FStar_Extraction_ML_Syntax.mlty Prims.list)) = (fun tyBinders c ctor -> (let _79_414 = (bindersOfFuntype c (FStar_List.length tyBinders) ctor.ctype)
in (match (_79_414) with
| (lb, tr) -> begin
(let _79_415 = ()
in (let lp = (FStar_List.zip tyBinders lb)
in (let newC = (let _182_215 = (FStar_List.map (fun _79_420 -> (match (_79_420) with
| (x, y) -> begin
((Prims.fst x), (Prims.fst y))
end)) lp)
in (extendContextWithRepAsTyVars _182_215 c))
in (let mlt = (let _182_216 = (extractTyp newC tr)
in (FStar_Extraction_ML_Util.eraseTypeDeep c _182_216))
in (let tys = (let _182_217 = (FStar_List.map mlTyIdentOfBinder tyBinders)
in (_182_217, mlt))
in (let fvv = (FStar_Extraction_ML_Env.mkFvvar ctor.cname ctor.ctype)
in (let _182_220 = (FStar_Extraction_ML_Env.extend_fv c fvv tys false false)
in (let _182_219 = (let _182_218 = (argTypes mlt)
in ((lident2mlsymbol ctor.cname), _182_218))
in (_182_220, _182_219)))))))))
end)))

let rec firstNNats : Prims.int  ->  Prims.int Prims.list = (fun n -> if (0 < n) then begin
(let _182_223 = (firstNNats (n - 1))
in (n)::_182_223)
end else begin
[]
end)

let dummyIdent : Prims.int  ->  (Prims.string * Prims.int) = (fun n -> (let _182_227 = (let _182_226 = (FStar_Util.string_of_int n)
in (Prims.strcat "\'dummyV" _182_226))
in (_182_227, 0)))

let dummyIndexIdents : Prims.int  ->  (Prims.string * Prims.int) Prims.list = (fun n -> (let _182_230 = (firstNNats n)
in (FStar_List.map dummyIdent _182_230)))
...
in (let _79_434 = (FStar_Util.fold_map (extractCtor ind.tyBinders) newContext ind.constructors)
in (match (_79_434) with
| (nc, tyb) -> begin
(let mlbs = (let _182_236 = (FStar_List.map mlTyIdentOfBinder ind.tyBinders)
in (let _182_235 = (dummyIndexIdents nIndices)
in (FStar_List.append _182_236 _182_235)))
in (let tbody = (match ((FStar_Util.find_opt (fun _79_1 -> (match (_79_1) with
| FStar_Absyn_Syntax.RecordType (_79_438) -> begin
true
end
| _79_441 -> begin
false
...
(let _79_445 = ()
in (let _79_450 = (FStar_List.hd tyb)
in (match (_79_450) with
| (_79_448, c_ty) -> begin
(let _79_451 = ()
...
(let _79_470 = (let _182_246 = (let _182_245 = (mfst bs)
in (extendContext c _182_245))
in (headBinders _182_246 t))
...
in (let c = (let _182_251 = (mfst bs)
in (extendContext c _182_251))
in (let _79_482 = (headBinders c t)
in (match (_79_482) with
...
in (let td = (let _182_252 = (FStar_List.map mlTyIdentOfBinder bs)
in ((mlsymbolOfLident l), _182_252, Some (tyDecBody)))
...
in (let ex_decl = (let _182_258 = (let _182_257 = (argTypes mlt)
in ((lident2mlsymbol exnConstr.cname), _182_257))
in FStar_Extraction_ML_Syntax.MLM_Exn (_182_258))
in (let _182_259 = (FStar_Extraction_ML_Env.extend_fv c fvv tys false false)
in (_182_259, ex_decl))))))))
...
| FStar_Absyn_Syntax.Sig_typ_abbrev (l, bs, _79_502, t, quals, _79_506) -> begin
(let _79_511 = (extractTypeAbbrev c {abTyName = l; abTyBinders = bs; abBody = t})
in (match (_79_511) with
| (c, tds) -> begin
(let _182_264 = if (FStar_All.pipe_right quals (FStar_List.contains FStar_Absyn_Syntax.Logic)) then begin
[]
end else begin
...
| FStar_Absyn_Syntax.Sig_bundle (sigs, FStar_Absyn_Syntax.ExceptionConstructor::[], _79_516, _79_518) -> begin
(let _79_526 = (parseInductiveTypesFromSigBundle c sigs)
in (match (_79_526) with
| (_79_522, _79_524, exConstrs) -> begin
(let _79_527 = ()
in (let _79_531 = (let _182_265 = (FStar_List.hd exConstrs)
in (extractExn c _182_265))
in (match (_79_531) with
| (c, exDecl) -> begin
(c, (exDecl)::[])
end)))
end))
end
| FStar_Absyn_Syntax.Sig_bundle (sigs, _79_534, _79_536, _79_538) -> begin
(let _79_545 = (parseInductiveTypesFromSigBundle c sigs)
in (match (_79_545) with
| (inds, abbs, _79_544) -> begin
(let _79_548 = (FStar_Util.fold_map extractInductive c inds)
in (match (_79_548) with
| (c, indDecls) -> begin
(let _79_551 = (FStar_Util.fold_map extractTypeAbbrev c abbs)
...
| FStar_Absyn_Syntax.Sig_tycon (l, bs, k, _79_556, _79_558, quals, r) -> begin
if ((FStar_All.pipe_right quals (FStar_List.contains FStar_Absyn_Syntax.Assumption)) && (not ((FStar_All.pipe_right quals (FStar_Util.for_some (fun _79_2 -> (match (_79_2) with
...
(let _79_575 = (FStar_Absyn_Util.kind_formals k)
in (match (_79_575) with
| (kbs, _79_574) -> begin
