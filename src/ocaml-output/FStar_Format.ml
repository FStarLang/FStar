...
let (enclose : doc -> doc -> doc -> doc) =
  fun uu____23870  ->
    fun uu____23871  ->
      fun uu____23872  ->
        match (uu____23870, uu____23871, uu____23872) with
        | (Doc l,Doc r,Doc x) -> Doc (Prims.op_Hat l (Prims.op_Hat x r))
  
let (brackets : doc -> doc) =
  fun uu____23884  ->
    match uu____23884 with | Doc d -> enclose (text "[") (text "]") (Doc d)
  
let (cbrackets : doc -> doc) =
  fun uu____23894  ->
    match uu____23894 with | Doc d -> enclose (text "{") (text "}") (Doc d)
  
let (parens : doc -> doc) =
  fun uu____23904  ->
    match uu____23904 with | Doc d -> enclose (text "(") (text ")") (Doc d)
  
let (cat : doc -> doc -> doc) =
  fun uu____23918  ->
    fun uu____23919  ->
      match (uu____23918, uu____23919) with
      | (Doc d1,Doc d2) -> Doc (Prims.op_Hat d1 d2)
  
let (reduce : doc Prims.list -> doc) =
  fun docs  -> FStar_List.fold_left cat empty docs 
let (group : doc -> doc) =
  fun uu____23939  -> match uu____23939 with | Doc d -> Doc d 
let (groups : doc Prims.list -> doc) =
  fun docs  -> let uu____23952 = reduce docs  in group uu____23952 
let (combine : doc -> doc Prims.list -> doc) =
  fun uu____23964  ->
    fun docs  ->
      match uu____23964 with
      | Doc sep ->
          let select uu____23978 =
...
let (nest : Prims.int -> doc -> doc) =
  fun i  -> fun uu____24031  -> match uu____24031 with | Doc d -> Doc d 
let (align : doc Prims.list -> doc) =
  fun docs  ->
    let uu____24045 = combine hardline docs  in
    match uu____24045 with | Doc doc -> Doc doc
  
let (hbox : doc -> doc) = fun d  -> d 
let (pretty : Prims.int -> doc -> Prims.string) =
  fun sz  -> fun uu____24066  -> match uu____24066 with | Doc doc -> doc 