...
let group : doc  ->  doc = (fun uu____62 -> (match (uu____62) with
| Doc (d) -> begin
Doc (d)
end))
...
let uu____71 = (reduce docs)
in (group uu____71)))


let combine : doc  ->  doc Prims.list  ->  doc = (fun uu____78 docs -> (match (uu____78) with
| Doc (sep) -> begin
(

let select = (fun uu____88 -> (match (uu____88) with
| Doc (d) -> begin
...
let nest : Prims.int  ->  doc  ->  doc = (fun i uu____116 -> (match (uu____116) with
| Doc (d) -> begin
Doc (d)
end))
...
let uu____125 = (combine hardline docs)
...
let pretty : Prims.int  ->  doc  ->  Prims.string = (fun sz uu____135 -> (match (uu____135) with
| Doc (doc) -> begin
doc
end))
