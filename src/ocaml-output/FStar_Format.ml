...
let group : doc  ->  doc = (fun uu____75 -> (match (uu____75) with
| Doc (d) -> begin
Doc (d)
end))
...
let uu____83 = (reduce docs1)
in (group uu____83)))


let combine : doc  ->  doc Prims.list  ->  doc = (fun uu____91 docs1 -> (match (uu____91) with
| Doc (sep) -> begin
(

let select = (fun uu____99 -> (match (uu____99) with
| Doc (d) -> begin
(match ((d = "")) with
| true -> begin
...
let nest : Prims.int  ->  doc  ->  doc = (fun i uu____127 -> (match (uu____127) with
| Doc (d) -> begin
Doc (d)
end))
...
let uu____135 = (combine hardline docs1)
...
let pretty : Prims.int  ->  doc  ->  Prims.string = (fun sz uu____148 -> (match (uu____148) with
| Doc (doc1) -> begin
doc1
end))
