...
let new_kvar = (fun ( r ) ( binders ) -> (let u = (Support.Microsoft.FStar.Unionfind.fresh Microsoft_FStar_Absyn_Syntax.Uvar)
...
let new_tvar = (fun ( r ) ( binders ) ( k ) -> (let binders = (Support.All.pipe_right binders (Support.List.filter (fun ( x ) -> (let _101_15 = (Microsoft_FStar_Absyn_Syntax.is_null_binder x)
in (Support.All.pipe_right _101_15 Support.Prims.op_Negation)))))
...
let new_evar = (fun ( r ) ( binders ) ( t ) -> (let binders = (Support.All.pipe_right binders (Support.List.filter (fun ( x ) -> (let _101_24 = (Microsoft_FStar_Absyn_Syntax.is_null_binder x)
in (Support.All.pipe_right _101_24 Support.Prims.op_Negation)))))
in (let uv = (Support.Microsoft.FStar.Unionfind.fresh Microsoft_FStar_Absyn_Syntax.Uvar)
in (match (binders) with
| [] -> begin
...
type worklist =
{attempting : probs; wl_deferred : (int * string * prob) list; subst : uvi list; ctr : int; slack_vars : (bool * Microsoft_FStar_Absyn_Syntax.typ) list; defer_ok : bool; smt_ok : bool; tcenv : Microsoft_FStar_Tc_Env.env}
...
in (let _101_238 = (let _101_237 = (let _101_236 = (Support.All.pipe_right p.reason Support.List.hd)
...
in (let _101_231 = (let _101_230 = (Microsoft_FStar_Tc_Normalize.formula_norm_to_string env (Support.Prims.fst p.logical_guard))
in (_101_230)::[])
...
| false -> begin
(let _101_252 = (Support.Microsoft.FStar.Unionfind.uvar_id u)
in (Support.All.pipe_right _101_252 Support.Microsoft.FStar.Util.string_of_int))
end))
in (match (uvi) with
| UK ((u, _35_141)) -> begin
(let _101_253 = (str u)
in (Support.All.pipe_right _101_253 (Support.Microsoft.FStar.Util.format1 "UK %s")))
end
| UT (((u, _35_146), t)) -> begin
(let _101_256 = (str u)
in (Support.All.pipe_right _101_256 (fun ( x ) -> (let _101_255 = (Microsoft_FStar_Tc_Normalize.typ_norm_to_string env t)
in (Support.Microsoft.FStar.Util.format2 "UT %s %s" x _101_255)))))
end
| UE (((u, _35_154), _35_157)) -> begin
(let _101_257 = (str u)
in (Support.All.pipe_right _101_257 (Support.Microsoft.FStar.Util.format1 "UE %s")))
end)))
...
| KProb (p) -> begin
(Support.All.pipe_right (maybe_invert p) (fun ( _101_264 ) -> KProb (_101_264)))
end
| TProb (p) -> begin
(Support.All.pipe_right (maybe_invert p) (fun ( _101_265 ) -> TProb (_101_265)))
end
| EProb (p) -> begin
(Support.All.pipe_right (maybe_invert p) (fun ( _101_266 ) -> EProb (_101_266)))
end
| CProb (p) -> begin
(Support.All.pipe_right (maybe_invert p) (fun ( _101_267 ) -> CProb (_101_267)))
end))
...
| KProb (p) -> begin
(Support.All.pipe_left (fun ( _101_286 ) -> KProb (_101_286)) (invert p))
end
| TProb (p) -> begin
(Support.All.pipe_left (fun ( _101_287 ) -> TProb (_101_287)) (invert p))
end
| EProb (p) -> begin
(Support.All.pipe_left (fun ( _101_288 ) -> EProb (_101_288)) (invert p))
end
| CProb (p) -> begin
(Support.All.pipe_left (fun ( _101_289 ) -> CProb (_101_289)) (invert p))
end))
...
| true -> begin
(let _101_333 = (let _101_332 = (Support.List.map (uvi_to_string wl.tcenv) uvis)
in (Support.All.pipe_right _101_332 (Support.String.concat ", ")))
...
in (let _35_299 = wl
in {attempting = _35_299.attempting; wl_deferred = _35_299.wl_deferred; subst = (Support.List.append uvis wl.subst); ctr = (wl.ctr + 1); slack_vars = _35_299.slack_vars; defer_ok = _35_299.defer_ok; smt_ok = _35_299.smt_ok; tcenv = _35_299.tcenv}))
end))
end))))
...
let explain = (fun ( env ) ( d ) ( s ) -> (let _101_354 = (Support.All.pipe_left Support.Microsoft.FStar.Range.string_of_range (p_loc d))
in (let _101_353 = (prob_to_string env d)
in (let _101_352 = (Support.All.pipe_right (p_reason d) (Support.String.concat "\n\t>"))
...
in (let _101_365 = (Support.List.map Support.Prims.snd g.carry)
...
in {attempting = (Support.List.append probs wl.attempting); wl_deferred = _35_328.wl_deferred; subst = _35_328.subst; ctr = _35_328.ctr; slack_vars = _35_328.slack_vars; defer_ok = _35_328.defer_ok; smt_ok = _35_328.smt_ok; tcenv = _35_328.tcenv}))
...
in (Support.All.pipe_left (fun ( _101_419 ) -> Support.Microsoft.FStar.Util.Inl (_101_419)) _101_420))
in (_101_421, (Support.Prims.snd a)))
end
...
in (Support.All.pipe_left (fun ( _101_422 ) -> Support.Microsoft.FStar.Util.Inr (_101_422)) _101_423))
in (_101_424, (Support.Prims.snd a)))
end))

let whnf = (fun ( env ) ( t ) -> (let _101_429 = (Microsoft_FStar_Tc_Normalize.whnf env t)
in (Support.All.pipe_right _101_429 Microsoft_FStar_Absyn_Util.compress_typ)))
...
in (Support.All.pipe_right _101_434 Microsoft_FStar_Absyn_Util.compress_typ)))
...
in Support.Microsoft.FStar.Util.Inl (_101_439))
in (_101_440, imp))
end
...
in Support.Microsoft.FStar.Util.Inr (_101_442))
in (_101_443, imp))
end)))))
...
in (Support.All.pipe_right _101_448 Microsoft_FStar_Absyn_Util.compress_kind)))
...
in (Support.All.pipe_right _101_453 Microsoft_FStar_Absyn_Util.compress_exp)))
...
in (Support.All.failwith _101_493))
end)
end)
end
...
in (Support.All.failwith _101_497))
end))
...
in (Support.All.pipe_right _101_503 Support.Prims.fst)))
...
in (let _101_525 = (Support.All.pipe_right uvs.Microsoft_FStar_Absyn_Syntax.uvars_t Support.Microsoft.FStar.Util.set_elements)
in (Support.All.pipe_right _101_525 (Support.Microsoft.FStar.Util.for_some (fun ( _35_615 ) -> (match (_35_615) with
| (uvt, _35_614) -> begin
...
in (let _101_535 = (let _101_534 = (Support.All.pipe_right wl.subst (Support.List.map (uvi_to_string env)))
in (Support.All.pipe_right _101_534 (Support.String.concat ", ")))
in (Support.Microsoft.FStar.Util.format3 "occurs-check failed (%s occurs in %s) (with substitution %s)" _101_537 _101_536 _101_535))))
in Some (_101_538))
end)
in (occurs_ok, msg))))
...
in (let _101_559 = (let _101_557 = (let _101_556 = (Support.Microsoft.FStar.Util.set_elements uvs.Microsoft_FStar_Absyn_Syntax.uvars_e)
in (Support.All.pipe_right _101_556 (Support.List.map Microsoft_FStar_Absyn_Print.uvar_e_to_string)))
in (Support.All.pipe_right _101_557 (Support.String.concat ", ")))
in (let _101_558 = (Microsoft_FStar_Tc_Normalize.exp_norm_to_string env e)
in (Support.Microsoft.FStar.Util.format3 "occurs-check failed (%s occurs in {%s} uvars of %s)" _101_560 _101_559 _101_558))))
in Some (_101_561))
end)
in (occurs_ok, msg)))))
...
in (let _101_568 = (let _101_567 = (Support.Microsoft.FStar.Util.set_intersect fvs1.Microsoft_FStar_Absyn_Syntax.ftvs fvs2.Microsoft_FStar_Absyn_Syntax.ftvs)
in (let _101_566 = (Support.Microsoft.FStar.Util.set_intersect fvs1.Microsoft_FStar_Absyn_Syntax.fxvs fvs2.Microsoft_FStar_Absyn_Syntax.fxvs)
...
in (Support.All.pipe_right _101_602 head_match))
end
...
in (Support.All.pipe_right _101_603 head_match))
end
...
in (Support.All.pipe_right _101_604 head_match))
end
...
in (Support.All.pipe_left (fun ( _101_739 ) -> KProb (_101_739)) _101_740))
...
in (Support.All.pipe_left (fun ( _101_742 ) -> TProb (_101_742)) _101_743))
...
in (Support.All.pipe_left (fun ( _101_745 ) -> EProb (_101_745)) _101_746))
...
in (Support.All.pipe_left (fun ( _101_754 ) -> Microsoft_FStar_Absyn_Syntax.C (_101_754)) _101_755))
in (_101_756, (prob)::[]))
end
| _35_1189 -> begin
(Support.All.failwith "impossible")
...
in (let _35_1222 = (let _101_758 = (Support.List.map (sub_prob scope args) components)
in (Support.All.pipe_right _101_758 Support.List.unzip))
in (match (_35_1222) with
| (ktecs, sub_probs) -> begin
(let gi_xs = (let _101_763 = (let _101_762 = (let _101_759 = (Support.List.hd ktecs)
in (Support.All.pipe_right _101_759 un_T))
in (let _101_761 = (let _101_760 = (Support.List.tl ktecs)
in (Support.All.pipe_right _101_760 (Support.List.map arg_of_ktec)))
...
in (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.mk_Comp _101_763))
...
| None -> begin
(let _101_768 = (let _101_767 = (Support.All.pipe_right probs (Support.List.map (fun ( prob ) -> (Support.All.pipe_right (p_guard prob) Support.Prims.fst))))
in (f)::_101_767)
in (Microsoft_FStar_Absyn_Util.mk_conj_l _101_768))
end
| Some (b) -> begin
(let _101_772 = (let _101_771 = (Microsoft_FStar_Absyn_Util.close_forall ((b)::[]) f)
in (let _101_770 = (Support.All.pipe_right probs (Support.List.map (fun ( prob ) -> (Support.All.pipe_right (p_guard prob) Support.Prims.fst))))
...
let new_slack_var = (fun ( env ) ( slack ) -> (let xs = (let _101_798 = (let _101_797 = (destruct_flex_pattern env (Support.Prims.snd slack.lower))
in (Support.All.pipe_right _101_797 Support.Prims.snd))
in (Support.All.pipe_right _101_798 Support.Microsoft.FStar.Util.must))
in (let _101_799 = (new_tvar (Support.Prims.fst slack.lower).Microsoft_FStar_Absyn_Syntax.pos xs Microsoft_FStar_Absyn_Syntax.ktype)
in (_101_799, xs))))
...
in (let _101_809 = (let _101_808 = (let _101_807 = (let _101_806 = (Support.Microsoft.FStar.Util.mk_ref false)
...
in Support.Microsoft.FStar.Util.Inl (_101_834))
end
| false -> begin
(let low = (let _101_835 = (compress env wl phi1)
in (Support.All.pipe_left (destruct Microsoft_FStar_Absyn_Const.or_lid Microsoft_FStar_Absyn_Util.mk_disj) _101_835))
in (let hi = (let _101_836 = (compress env wl phi2)
in (Support.All.pipe_left (destruct Microsoft_FStar_Absyn_Const.and_lid Microsoft_FStar_Absyn_Util.mk_disj) _101_836))
...
in Support.Microsoft.FStar.Util.Inl (_101_837)))
end
...
in (Support.All.pipe_right _101_867 (fun ( _101_866 ) -> KProb (_101_866))))
end
| TProb (p) -> begin
...
in (Support.All.pipe_right _101_871 (fun ( _101_870 ) -> TProb (_101_870))))
end
| EProb (p) -> begin
...
in (Support.All.pipe_right _101_875 (fun ( _101_874 ) -> EProb (_101_874))))
end
...
in (Support.All.pipe_right _101_880 maybe_invert_p))
in (match (prob) with
| KProb (kp) -> begin
...
in (let _101_882 = (Support.All.pipe_right (let _35_1558 = kp
...
| (rank, tp) -> begin
(let _101_888 = (Support.All.pipe_right (let _35_1626 = tp
...
in (let _101_890 = (Support.All.pipe_right (let _35_1663 = ep
...
| CProb (cp) -> begin
(let _101_892 = (Support.All.pipe_right (let _35_1667 = cp
...
in (Support.All.pipe_left (fun ( _101_951 ) -> TProb (_101_951)) _101_952))
in (_101_953)::[])
in Some (_101_954))
end)
end
...
| _35_1878 -> begin
(let _35_1887 = (Support.All.pipe_right probs.wl_deferred (Support.List.partition (fun ( _35_1884 ) -> (match (_35_1884) with
...
(match (attempt) with
| [] -> begin
(let _101_983 = (let _101_982 = (let _101_981 = (Support.List.map (fun ( _35_1893 ) -> (match (_35_1893) with
| (_35_1890, x, y) -> begin
(x, y)
...
in (let _101_985 = (Support.All.pipe_right attempt (Support.List.map (fun ( _35_1903 ) -> (match (_35_1903) with
...
in (Support.List.append _101_1012 subst))
...
(let phi = (let _101_1023 = (Support.All.pipe_right (p_guard prob) Support.Prims.fst)
...
| false -> begin
(let _101_1061 = (let _101_1060 = (Support.All.pipe_right xs Microsoft_FStar_Absyn_Util.args_of_non_null_binders)
...
in (Support.All.pipe_left (solve env) _101_1062))
end
...
in (Support.All.pipe_left (fun ( _101_1069 ) -> KProb (_101_1069)) _101_1071)))
...
in (let _101_1097 = (let _101_1095 = (Support.List.map (prob_to_string env) sub_probs)
in (Support.All.pipe_right _101_1095 (Support.String.concat ", ")))
...
in (match ((let _101_1123 = (let _101_1122 = (Support.List.nth xs i)
in (Support.All.pipe_left Support.Prims.fst _101_1122))
in ((Support.Prims.fst pi), _101_1123))) with
| (Support.Microsoft.FStar.Util.Inl (pi), Support.Microsoft.FStar.Util.Inl (xi)) -> begin
(match ((let _101_1124 = (matches pi)
in (Support.All.pipe_left Support.Prims.op_Negation _101_1124))) with
| true -> begin
None
end
| false -> begin
...
in (let _101_1130 = (let _101_1129 = (Support.List.map (fun ( _35_2259 ) -> (match (_35_2259) with
| (_35_2255, _35_2257, y) -> begin
y
end)) qs)
in (Support.All.pipe_left h _101_1129))
in (mk_problem (p_scope orig) orig _101_1131 (p_rel orig) _101_1130 None "projection")))
in (Support.All.pipe_left (fun ( _101_1127 ) -> TProb (_101_1127)) _101_1132))
...
in (let wl = (let _101_1136 = (let _101_1135 = (Support.All.pipe_left Support.Prims.fst (p_guard sub))
in Some (_101_1135))
in (solve_prob orig _101_1136 ((UT ((u, proj)))::[]) wl))
in (let _101_1138 = (solve env (attempt ((sub)::[]) wl))
in (Support.All.pipe_left (fun ( _101_1137 ) -> Some (_101_1137)) _101_1138)))))))
end))
end)
end
| _35_2265 -> begin
...
in (Support.All.pipe_right _101_1165 Support.Prims.fst))
...
in (Support.Microsoft.FStar.Util.fprint1 "Free variables are %s" _101_1181))
end
| false -> begin
()
end)
...
in (Support.All.pipe_right _101_1184 Support.Prims.fst))
in (Support.All.pipe_right _101_1185 Microsoft_FStar_Absyn_Util.freevars_typ))
...
| true -> begin
(let _101_1187 = (let _101_1186 = (Support.Option.get msg)
in (Support.Prims.strcat "occurs-check failed: " _101_1186))
...
in (imitate_or_project (Support.List.length args_lhs) _101_1197 (- (1)))))
end
| false -> begin
...
in (imitate_or_project (Support.List.length args_lhs) _101_1200 im_ok))))
end
| false -> begin
...
in (Support.All.pipe_right _101_1221 (Microsoft_FStar_Absyn_Util.gen_bvar_p a.Microsoft_FStar_Absyn_Syntax.pos)))
in (Support.All.pipe_right _101_1222 Microsoft_FStar_Absyn_Syntax.t_binder))
end
| Support.Microsoft.FStar.Util.Inr (x) -> begin
(let _101_1224 = (let _101_1223 = (Microsoft_FStar_Tc_Recheck.recompute_typ x)
in (Support.All.pipe_right _101_1223 (Microsoft_FStar_Absyn_Util.gen_bvar_p x.Microsoft_FStar_Absyn_Syntax.pos)))
in (Support.All.pipe_right _101_1224 Microsoft_FStar_Absyn_Syntax.v_binder))
end))
...
in (Support.All.pipe_right _101_1247 (fun ( _101_1246 ) -> TProb (_101_1246))))
end
...
in (Support.All.pipe_right _101_1249 (fun ( _101_1248 ) -> EProb (_101_1248))))
end
| _35_2449 -> begin
(Support.All.failwith "Impossible")
end))) xs args2)
in (let guard = (let _101_1251 = (Support.List.map (fun ( p ) -> (Support.All.pipe_right (p_guard p) Support.Prims.fst)) sub_probs)
...
in (let _101_1260 = (let _101_1259 = (Support.List.map (uvi_to_string wl.tcenv) wl.subst)
in (Support.All.pipe_right _101_1259 (Support.String.concat "; ")))
...
in (Support.All.pipe_left (fun ( _101_1303 ) -> Some (_101_1303)) _101_1304))
...
in (Support.All.pipe_right _101_1317 Support.Microsoft.FStar.Range.string_of_range))
...
in (Support.All.pipe_left (fun ( _101_1319 ) -> CProb (_101_1319)) _101_1320)))))))
end)))
end
...
in (Support.All.pipe_left (fun ( _101_1330 ) -> TProb (_101_1330)) _101_1331)))))
end)))
end
...
in (Support.All.pipe_left (fun ( _101_1332 ) -> TProb (_101_1332)) _101_1333))
...
in (Support.All.pipe_right _101_1352 (guard_on_element problem x1))))
...
in (let guard = (let _101_1355 = (Support.All.pipe_right (p_guard base_prob) Support.Prims.fst)
...
in (Support.All.pipe_left (fun ( _101_1356 ) -> TProb (_101_1356)) _101_1357))
...
(let guard = (let _101_1360 = (Support.All.pipe_right (p_guard base_prob) Support.Prims.fst)
in (let _101_1359 = (let _101_1358 = (Support.All.pipe_right (p_guard ref_prob) Support.Prims.fst)
in (Support.All.pipe_right _101_1358 (guard_on_element problem x1)))
...
in (Support.All.pipe_left Support.Prims.op_Negation _101_1364))) with
| true -> begin
(let _101_1367 = (Support.All.pipe_left (fun ( _101_1365 ) -> TProb (_101_1365)) (let _35_2796 = problem
...
(match (ref_opt) with
| None -> begin
(let _101_1370 = (Support.All.pipe_left (fun ( _101_1368 ) -> TProb (_101_1368)) (let _35_2802 = problem
...
in (Support.All.pipe_left (fun ( _101_1371 ) -> TProb (_101_1371)) _101_1372))
in (let guard = (let _101_1373 = (Support.All.pipe_right (p_guard base_prob) Support.Prims.fst)
...
in (Support.All.pipe_left force_refinement _101_1374))
...
in (Support.All.pipe_left force_refinement _101_1375))
...
in (Support.All.pipe_right _101_1376 Support.Prims.fst))
in (let head2 = (let _101_1377 = (Microsoft_FStar_Absyn_Util.head_and_args t2)
in (Support.All.pipe_right _101_1377 Support.Prims.fst))
...
in (Support.All.pipe_left (fun ( _101_1380 ) -> Some (_101_1380)) _101_1381))
...
| (_35_2928, None) -> begin
(let _35_2931 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.Microsoft.FStar.Util.format4 "unequal number of arguments: %s[%s] and %s[%s]" _101_1389 _101_1388 _101_1387 _101_1386)))))
...
in (Support.All.failwith _101_1394))
end
| false -> begin
()
end)
...
in (Support.All.pipe_left (fun ( _101_1397 ) -> TProb (_101_1397)) _101_1398))
end
...
in (Support.All.pipe_left (fun ( _101_1399 ) -> EProb (_101_1399)) _101_1400))
end
| _35_2964 -> begin
(Support.All.failwith "Impossible")
end)) args args')
in (let formula = (let _101_1402 = (Support.List.map (fun ( p ) -> (Support.Prims.fst (p_guard p))) subprobs)
...
in (let solve_eq = (fun ( c1_comp ) ( c2_comp ) -> (let _35_3029 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("EQ")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1416 ) -> TProb (_101_1416)) _101_1417))
end
| (Support.Microsoft.FStar.Util.Inr (e1), Support.Microsoft.FStar.Util.Inr (e2)) -> begin
(let _101_1419 = (sub_prob e1 EQ e2 "effect arg")
in (Support.All.pipe_left (fun ( _101_1418 ) -> EProb (_101_1418)) _101_1419))
end
| _35_3044 -> begin
(Support.All.failwith "impossible")
end)) c1_comp.Microsoft_FStar_Absyn_Syntax.effect_args c2_comp.Microsoft_FStar_Absyn_Syntax.effect_args)
in (let guard = (let _101_1421 = (Support.List.map (fun ( p ) -> (Support.All.pipe_right (p_guard p) Support.Prims.fst)) sub_probs)
...
| false -> begin
(let _35_3049 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (let _101_1425 = (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.mk_Comp (Microsoft_FStar_Absyn_Util.comp_to_comp_typ c1))
...
in (let _101_1427 = (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.mk_Comp (Microsoft_FStar_Absyn_Util.comp_to_comp_typ c2))
...
in (let _35_3089 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.Microsoft.FStar.Util.format2 "incompatible monad ordering: %s </: %s" _101_1430 _101_1429)))
...
| (Support.Microsoft.FStar.Util.Inl (wp1), _35_3102)::(Support.Microsoft.FStar.Util.Inl (wlp1), _35_3097)::[] -> begin
(wp1, wlp1)
end
| _35_3106 -> begin
...
in (Support.Microsoft.FStar.Range.string_of_range _101_1432))
in (Support.Microsoft.FStar.Util.format1 "Unexpected number of indices on a normalized effect (%s)" _101_1433))
in (Support.All.failwith _101_1434))
end)
...
| ((Support.Microsoft.FStar.Util.Inl (wp1), _35_3123)::_35_3119, (Support.Microsoft.FStar.Util.Inl (wp2), _35_3131)::_35_3127) -> begin
(wp1, wp2)
end
| _35_3136 -> begin
...
in (Support.Microsoft.FStar.Util.format2 "Got effects %s and %s, expected normalized effects" _101_1443 _101_1442)))
in (Support.All.failwith _101_1444))
end)
...
| true -> begin
(let _35_3141 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.targ _101_1445))
in (_101_1446)::[])
...
in (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.targ _101_1452))
...
in (Support.All.pipe_left Microsoft_FStar_Absyn_Syntax.targ _101_1453))
in (_101_1454)::[])
...
in (Support.All.pipe_left (fun ( _101_1468 ) -> TProb (_101_1468)) _101_1469))
in (let wl = (let _101_1473 = (let _101_1472 = (let _101_1471 = (Support.All.pipe_right (p_guard base_prob) Support.Prims.fst)
in (Microsoft_FStar_Absyn_Util.mk_conj _101_1471 g))
in (Support.All.pipe_left (fun ( _101_1470 ) -> Some (_101_1470)) _101_1472))
...
in (let _35_3169 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
(let maybe_vars1 = (pat_vars env [] args1)
in (let sub_problems = (fun ( xs ) ( args2 ) -> (let _35_3203 = (let _101_1499 = (Support.All.pipe_right args2 (Support.List.map (fun ( _35_34 ) -> (match (_35_34) with
...
in (Support.All.pipe_left (fun ( _101_1493 ) -> TProb (_101_1493)) _101_1494))
in ((Support.Microsoft.FStar.Util.Inl (gi_xi), imp), _101_1495)))
end)))
end
...
in (Support.All.pipe_left (fun ( _101_1496 ) -> EProb (_101_1496)) _101_1497))
in ((Support.Microsoft.FStar.Util.Inr (gi_xi), imp), _101_1498)))
end)))
end))))
in (Support.All.pipe_right _101_1499 Support.List.unzip))
in (match (_35_3203) with
| (gi_xi, gi_pi) -> begin
(let formula = (let _101_1501 = (Support.List.map (fun ( p ) -> (Support.All.pipe_right (p_guard p) Support.Prims.fst)) gi_pi)
in (Microsoft_FStar_Absyn_Util.mk_conj_l _101_1501))
in (gi_xi, gi_pi, formula))
end)))
in (let project_e = (fun ( head2 ) ( args2 ) -> (let giveup = (fun ( reason ) -> (let _101_1508 = (Support.Microsoft.FStar.Util.format1 "flex-rigid: refusing to project expressions (%s)" reason)
...
| ((Support.Microsoft.FStar.Util.Inl (_35_3237), _35_3240)::xs, (Support.Microsoft.FStar.Util.Inl (_35_3245), _35_3248)::args) -> begin
(aux xs args)
end
| ((Support.Microsoft.FStar.Util.Inr (xi), _35_3256)::xs, (Support.Microsoft.FStar.Util.Inr (arg), _35_3263)::args) -> begin
...
in (let _35_3274 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (let _101_1523 = (let _101_1522 = (Support.All.pipe_right gi_pi (Support.List.map (prob_to_string env)))
in (Support.All.pipe_right _101_1522 (Support.String.concat "\n")))
...
| false -> begin
(let _35_3289 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (let _35_3316 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (let _101_1538 = (let _101_1537 = (Support.All.pipe_right gi_pi (Support.List.map (prob_to_string env)))
in (Support.All.pipe_right _101_1537 (Support.String.concat "\n")))
...
| true -> begin
(let _35_3367 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1557 ) -> Some (_101_1557)) _101_1558))
...
in (Support.All.pipe_left (fun ( _101_1566 ) -> Some (_101_1566)) _101_1569))
...
| ([], []) -> begin
(let guard = (let _101_1591 = (let _101_1590 = (Support.List.map p_guard sub_probs)
in (Support.All.pipe_right _101_1590 (Support.List.map Support.Prims.fst)))
...
in (Support.All.pipe_left (fun ( _101_1597 ) -> TProb (_101_1597)) _101_1598))
end
| (Support.Microsoft.FStar.Util.Inr (e1), Support.Microsoft.FStar.Util.Inr (e2)) -> begin
(let _101_1600 = (mk_problem (p_scope orig) orig e1 EQ e2 None "expression arg")
in (Support.All.pipe_left (fun ( _101_1599 ) -> EProb (_101_1599)) _101_1600))
end
| _35_3669 -> begin
...
| (Microsoft_FStar_Absyn_Syntax.Exp_fvar ((f, _35_3695)), Microsoft_FStar_Absyn_Syntax.Exp_fvar ((g, _35_3700))) when (((Microsoft_FStar_Absyn_Util.fvar_eq f g) && (not ((Microsoft_FStar_Absyn_Util.is_interpreted f.Microsoft_FStar_Absyn_Syntax.v)))) && ((Support.List.length args1) = (Support.List.length args2))) -> begin
(solve_args [] wl args1 args2)
end
...
in (let _35_3749 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1616 ) -> Some (_101_1616)) _101_1617))
...
in (let carry = (let _101_1650 = (Support.List.map (fun ( _35_3767 ) -> (match (_35_3767) with
| (_35_3765, x) -> begin
(prob_to_string env x)
end)) g.deferred.carry)
in (Support.All.pipe_right _101_1650 (Support.String.concat ",\n")))
...
in (Support.All.pipe_left (fun ( _101_1661 ) -> NonTrivial (_101_1661)) _101_1665))
...
in (Support.All.pipe_left (Microsoft_FStar_Absyn_Syntax.syn f.Microsoft_FStar_Absyn_Syntax.pos (Some (Microsoft_FStar_Absyn_Syntax.ktype))) _101_1677))
...
in {guard_f = _101_1709; deferred = {carry = (Support.List.append g1.deferred.carry g2.deferred.carry); slack = (Support.List.append g1.deferred.slack g2.deferred.slack)}; implicits = (Support.List.append g1.implicits g2.implicits)}))
...
in (Support.All.pipe_right _101_1723 (fun ( _101_1722 ) -> NonTrivial (_101_1722))))
...
in (Support.All.pipe_left (fun ( _101_1746 ) -> Some (_101_1746)) _101_1747))
...
| Failed ((d, s)) -> begin
(let _35_3916 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("ExplainRel")))) with
| true -> begin
(let _101_1774 = (explain env d s)
in (Support.All.pipe_left Support.Microsoft.FStar.Util.print_string _101_1774))
end
| false -> begin
()
end)
...
| Some (d) -> begin
(let _101_1786 = (let _101_1785 = (let _101_1784 = (let _101_1783 = (Support.All.pipe_right (p_guard prob) Support.Prims.fst)
in (Support.All.pipe_right _101_1783 (fun ( _101_1782 ) -> NonTrivial (_101_1782))))
in {guard_f = _101_1784; deferred = d; implicits = []})
in (simplify_guard env _101_1785))
in (Support.All.pipe_left (fun ( _101_1781 ) -> Some (_101_1781)) _101_1786))
end))

let try_keq = (fun ( env ) ( k1 ) ( k2 ) -> (let _35_3927 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1795 ) -> KProb (_101_1795)) _101_1799))
in (let _101_1801 = (solve_and_commit env (singleton env prob) (fun ( _35_3930 ) -> None))
in (Support.All.pipe_left (with_guard env prob) _101_1801)))))
...
let subkind = (fun ( env ) ( k1 ) ( k2 ) -> (let _35_3949 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
(let _101_1825 = (let _101_1822 = (Microsoft_FStar_Tc_Env.get_range env)
in (Support.All.pipe_left Support.Microsoft.FStar.Range.string_of_range _101_1822))
...
in (Support.All.pipe_left (fun ( _101_1826 ) -> KProb (_101_1826)) _101_1830))
...
in (raise (_101_1835)))))
in (Support.All.pipe_left (with_guard env prob) _101_1836))
in (Support.Microsoft.FStar.Util.must _101_1837))
in res))))

let try_teq = (fun ( env ) ( t1 ) ( t2 ) -> (let _35_3958 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1846 ) -> TProb (_101_1846)) _101_1848))
in (let g = (let _101_1850 = (solve_and_commit env (singleton env prob) (fun ( _35_3961 ) -> None))
in (Support.All.pipe_left (with_guard env prob) _101_1850))
in g))))
...
| Some (g) -> begin
(let _35_3970 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
let try_subtype = (fun ( env ) ( t1 ) ( t2 ) -> (let _35_3975 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (with_guard env prob) _101_1873))
in (let _35_3983 = (match (((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel"))) && (Support.Microsoft.FStar.Util.is_some g))) with
| true -> begin
...
in (let _101_1875 = (let _101_1874 = (Support.Microsoft.FStar.Util.must g)
...
let sub_comp = (fun ( env ) ( c1 ) ( c2 ) -> (let _35_3997 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
...
in (Support.All.pipe_left (fun ( _101_1899 ) -> CProb (_101_1899)) _101_1901))
in (let _101_1903 = (solve_and_commit env (singleton env prob) (fun ( _35_4001 ) -> None))
in (Support.All.pipe_left (with_guard env prob) _101_1903))))))
...
in (let _35_4013 = (match (((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel"))) && ((Support.List.length g.deferred.carry) <> 0))) with
| true -> begin
(let _101_1916 = (let _101_1915 = (Support.All.pipe_right g.deferred.carry (Support.List.map (fun ( _35_4012 ) -> (match (_35_4012) with
| (msg, x) -> begin
(let _101_1914 = (Support.All.pipe_left Support.Microsoft.FStar.Range.string_of_range (p_loc x))
in (let _101_1913 = (prob_to_string env x)
in (let _101_1912 = (let _101_1911 = (Support.All.pipe_right (p_guard x) Support.Prims.fst)
...
in (Support.All.pipe_right _101_1915 (Support.String.concat "\n")))
...
| NonTrivial (vc) -> begin
(let _35_4037 = (match ((Support.All.pipe_left (Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel")))) with
| true -> begin
