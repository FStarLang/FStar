...
in (Support.All.pipe_left Support.Microsoft.FStar.Util.print_string _129_5))
in (Support.All.exit 1)))

let err_uninst = (fun ( e ) -> (let _129_8 = (let _129_7 = (Microsoft_FStar_Absyn_Print.exp_to_string e)
in (Support.Microsoft.FStar.Util.format1 "Variable %s has a polymorphic type; expected it to be fully instantiated" _129_7))
...
in (Support.Microsoft.FStar.Util.format3 "for expression %s, Expected effect %s; got effect %s" _129_19 (eff_to_string f0) (eff_to_string f1)))
...
| true -> begin
(Support.All.pipe_right args (Support.List.for_all (fun ( _63_57 ) -> (match (_63_57) with
| (te, _63_56) -> begin
(match (te) with
| Support.Microsoft.FStar.Util.Inl (_63_59) -> begin
true
end
...
(Support.All.pipe_right args (Support.List.for_all (fun ( _63_2 ) -> (match (_63_2) with
| (Support.Microsoft.FStar.Util.Inl (te), _63_73) -> begin
true
end
| _63_76 -> begin
false
...
(Support.Microsoft.FStar.Util.for_all (fun ( _63_120 ) -> (match (_63_120) with
| (_63_118, e) -> begin
(is_ml_value e)
end)) fields)
end
...
| (g, p) -> begin
(let _129_101 = (let _129_100 = (let _129_99 = (let _129_98 = (Support.List.collect (fun ( x ) -> (let _129_97 = (extract_pat true false g x)
in (Support.Prims.snd _129_97))) pats)
in (Support.List.append p _129_98))
...
in (let _63_205 = (Support.Microsoft.FStar.Util.first_N nTyVars pats)
in (match (_63_205) with
| (tysVarPats, restPats) -> begin
(let _63_212 = (Support.Microsoft.FStar.Util.fold_map (fun ( g ) ( _63_209 ) -> (match (_63_209) with
| (p, imp) -> begin
(extract_pat disj true g p)
end)) g tysVarPats)
in (match (_63_212) with
| (g, tyMLPats) -> begin
(let _63_219 = (Support.Microsoft.FStar.Util.fold_map (fun ( g ) ( _63_216 ) -> (match (_63_216) with
| (p, imp) -> begin
...
in (let _129_110 = (let _129_109 = (Support.All.pipe_left (Microsoft_FStar_Extraction_ML_Util.resugar_pat q) (Microsoft_FStar_Extraction_ML_Syntax.MLP_CTor ((d, (Support.List.flatten mlPats)))))
in (_129_109)::[])
in (g, _129_110)))
end))
end))
...
| true -> begin
(let _129_119 = (Support.Microsoft.FStar.Util.string_of_int n)
in (Support.Prims.strcat "mktuple" _129_119))
end
| false -> begin
(match ((n = 2)) with
| true -> begin
...
in (let tup = (let _129_124 = (let _129_123 = (let _129_122 = (ffi_mltuple_mlp (Support.List.length args))
...
| _63_321 -> begin
(let _63_324 = (Support.List.unzip eargs)
in (match (_63_324) with
| (binders, eargs) -> begin
(match (e) with
| Microsoft_FStar_Extraction_ML_Syntax.MLE_CTor ((head, args)) -> begin
(let body = (let _129_147 = (Support.All.pipe_left (as_record qual) (Microsoft_FStar_Extraction_ML_Syntax.MLE_CTor ((head, (Support.List.append args eargs)))))
in (Support.All.pipe_left Microsoft_FStar_Extraction_ML_Util.resugar_exp _129_147))
...
| (e, f_e, t_e) -> begin
(let mlbranches = (Support.All.pipe_right pats (Support.List.map (fun ( _63_418 ) -> (match (_63_418) with
...
in (let _129_173 = (Support.List.hd p)
...
in (Support.Microsoft.FStar.Util.print_string _129_180))))
...
in (Support.All.pipe_left (fun ( _129_182 ) -> Microsoft_FStar_Extraction_ML_Syntax.MLE_Const (_129_182)) _129_183))
...
| true -> begin
(let _129_195 = (Support.All.pipe_right (Support.List.rev mlargs_f) (Support.List.map Support.Prims.fst))
in ([], _129_195))
end
| false -> begin
(Support.List.fold_left (fun ( _63_494 ) ( _63_497 ) -> (match ((_63_494, _63_497)) with
...
in (let l_app = (Support.List.fold_right (fun ( _63_505 ) ( out ) -> (match (_63_505) with
| (x, arg) -> begin
...
| ((Support.Microsoft.FStar.Util.Inl (_63_510), _63_513)::rest, Microsoft_FStar_Extraction_ML_Syntax.MLTY_Fun ((tunit, f', t))) -> begin
...
| ((Support.Microsoft.FStar.Util.Inr (e0), _63_526)::rest, Microsoft_FStar_Extraction_ML_Syntax.MLTY_Fun ((tExpected, f', t))) -> begin
...
| true -> begin
(let _63_562 = (Support.Microsoft.FStar.Util.first_N n args)
...
| Microsoft_FStar_Absyn_Syntax.Exp_abs ((bs, body)) -> begin
(let _63_597 = (Support.List.fold_left (fun ( _63_581 ) ( _63_585 ) -> (match ((_63_581, _63_585)) with
...
in (let _129_207 = (Support.All.pipe_left (fun ( _129_206 ) -> Some (_129_206)) Microsoft_FStar_Extraction_ML_Syntax.ml_unit_ty)
...
| (ml_body, f, t) -> begin
(let _63_612 = (Support.List.fold_right (fun ( _63_606 ) ( _63_609 ) -> (match ((_63_606, _63_609)) with
| ((_63_604, targ), (f, t)) -> begin
(let _129_213 = (let _129_212 = (let _129_211 = (Support.Microsoft.FStar.Util.must targ)
in (_129_211, f, t))
...
(let _63_648 = (match ((Support.Microsoft.FStar.Util.prefix_until (fun ( _63_3 ) -> (match (_63_3) with
| (Support.Microsoft.FStar.Util.Inr (_63_633), _63_636) -> begin
true
end
| _63_639 -> begin
false
...
| true -> begin
(let _63_657 = (Support.Microsoft.FStar.Util.first_N n bs)
...
in (let targs = (Support.All.pipe_right targs (Support.List.map (fun ( _63_4 ) -> (match (_63_4) with
| (Support.Microsoft.FStar.Util.Inl (a), _63_666) -> begin
a
end
| _63_669 -> begin
(Support.All.failwith "Impossible")
...
in (let polytype = (let _129_221 = (Support.All.pipe_right targs (Support.List.map Microsoft_FStar_Extraction_ML_Env.btvar_as_mltyvar))
...
in (let _63_732 = (Support.List.fold_right (fun ( lb ) ( _63_713 ) -> (match (_63_713) with
| (env, lbs) -> begin
(let _63_726 = lb
...
| (e', f', t') -> begin
(let f = (let _129_231 = (let _129_230 = (Support.List.map Support.Prims.fst lbs)
in (f')::_129_230)
in (join_l _129_231))
in (let _129_235 = (let _129_234 = (let _129_233 = (let _129_232 = (Support.List.map Support.Prims.snd lbs)
in (is_rec, _129_232))
in (_129_233, e'))
...
in (fun ( x ) -> (let _63_758 = (Support.Microsoft.FStar.Util.incr c)
in (let _129_238 = (Support.ST.read c)
in (x, _129_238)))))
...
let dummyPatIdent = (fun ( n ) -> (let _129_257 = (let _129_256 = (Support.Microsoft.FStar.Util.string_of_int n)
in (Support.Prims.strcat "dummyPat" _129_256))
in (_129_257, 0)))

let dummyPatIdents = (fun ( n ) -> (let _129_260 = (Microsoft_FStar_Extraction_ML_ExtractTyp.firstNNats n)
in (Support.List.map dummyPatIdent _129_260)))
