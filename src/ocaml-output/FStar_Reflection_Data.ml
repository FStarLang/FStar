...
| {lid = __fname__lid; fv = __fname__fv; t = __fname__t} -> begin
__fname__lid
end))
...
let uu____1269 = (FStar_Syntax_Syntax.lid_as_fv lid FStar_Syntax_Syntax.delta_constant (FStar_Pervasives_Native.Some (FStar_Syntax_Syntax.Data_ctor)))
in (

let uu____1270 = (FStar_Syntax_Syntax.tdataconstr lid)
in {lid = lid; fv = uu____1269; t = uu____1270}))))


let mk_refl_types_lid_as_term : Prims.string  ->  FStar_Syntax_Syntax.term = (fun s -> (

let uu____1276 = (fstar_refl_types_lid s)
in (FStar_Syntax_Syntax.tconst uu____1276)))


let mk_refl_types_lid_as_fv : Prims.string  ->  FStar_Syntax_Syntax.fv = (fun s -> (

let uu____1282 = (fstar_refl_types_lid s)
in (FStar_Syntax_Syntax.fvconst uu____1282)))


let mk_refl_syntax_lid_as_term : Prims.string  ->  FStar_Syntax_Syntax.term = (fun s -> (

let uu____1288 = (fstar_refl_syntax_lid s)
in (FStar_Syntax_Syntax.tconst uu____1288)))


let mk_refl_syntax_lid_as_fv : Prims.string  ->  FStar_Syntax_Syntax.fv = (fun s -> (

let uu____1294 = (fstar_refl_syntax_lid s)
in (FStar_Syntax_Syntax.fvconst uu____1294)))


let mk_refl_data_lid_as_term : Prims.string  ->  FStar_Syntax_Syntax.term = (fun s -> (

let uu____1300 = (fstar_refl_data_lid s)
in (FStar_Syntax_Syntax.tconst uu____1300)))


let mk_refl_data_lid_as_fv : Prims.string  ->  FStar_Syntax_Syntax.fv = (fun s -> (

let uu____1306 = (fstar_refl_data_lid s)
in (FStar_Syntax_Syntax.fvconst uu____1306)))
...
let uu____1321 = (FStar_Syntax_Syntax.fv_to_tm inspect_fv)
in {lid = inspect_lid; fv = inspect_fv; t = uu____1321})
in (

let pack = (

let uu____1323 = (FStar_Syntax_Syntax.fv_to_tm pack_fv)
in {lid = pack_lid; fv = pack_fv; t = uu____1323})
in ((inspect), (pack)))))))))


let uu___82 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_ln")


let fstar_refl_inspect_ln : refl_constant = (match (uu___82) with
| (fstar_refl_inspect_ln, fstar_refl_pack_ln) -> begin
fstar_refl_inspect_ln
end)


let fstar_refl_pack_ln : refl_constant = (match (uu___82) with
| (fstar_refl_inspect_ln1, fstar_refl_pack_ln) -> begin
fstar_refl_pack_ln
end)


let uu___83 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_fv")


let fstar_refl_inspect_fv : refl_constant = (match (uu___83) with
| (fstar_refl_inspect_fv, fstar_refl_pack_fv) -> begin
fstar_refl_inspect_fv
end)


let fstar_refl_pack_fv : refl_constant = (match (uu___83) with
| (fstar_refl_inspect_fv1, fstar_refl_pack_fv) -> begin
fstar_refl_pack_fv
end)


let uu___84 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_bv")


let fstar_refl_inspect_bv : refl_constant = (match (uu___84) with
| (fstar_refl_inspect_bv, fstar_refl_pack_bv) -> begin
fstar_refl_inspect_bv
end)


let fstar_refl_pack_bv : refl_constant = (match (uu___84) with
| (fstar_refl_inspect_bv1, fstar_refl_pack_bv) -> begin
fstar_refl_pack_bv
end)


let uu___85 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_binder")


let fstar_refl_inspect_binder : refl_constant = (match (uu___85) with
| (fstar_refl_inspect_binder, fstar_refl_pack_binder) -> begin
fstar_refl_inspect_binder
end)


let fstar_refl_pack_binder : refl_constant = (match (uu___85) with
| (fstar_refl_inspect_binder1, fstar_refl_pack_binder) -> begin
fstar_refl_pack_binder
end)


let uu___86 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_comp")


let fstar_refl_inspect_comp : refl_constant = (match (uu___86) with
| (fstar_refl_inspect_comp, fstar_refl_pack_comp) -> begin
fstar_refl_inspect_comp
end)


let fstar_refl_pack_comp : refl_constant = (match (uu___86) with
| (fstar_refl_inspect_comp1, fstar_refl_pack_comp) -> begin
fstar_refl_pack_comp
end)


let uu___87 : (refl_constant * refl_constant) = (mk_inspect_pack_pair "_sigelt")


let fstar_refl_inspect_sigelt : refl_constant = (match (uu___87) with
| (fstar_refl_inspect_sigelt, fstar_refl_pack_sigelt) -> begin
fstar_refl_inspect_sigelt
end)


let fstar_refl_pack_sigelt : refl_constant = (match (uu___87) with
...
let uu____1381 = (fstar_refl_data_lid "bv_view")
in (

let uu____1382 = (

let uu____1385 = (FStar_Ident.mk_ident (("bv_ppname"), (FStar_Range.dummyRange)))
in (

let uu____1386 = (

let uu____1389 = (FStar_Ident.mk_ident (("bv_index"), (FStar_Range.dummyRange)))
in (

let uu____1390 = (

let uu____1393 = (FStar_Ident.mk_ident (("bv_sort"), (FStar_Range.dummyRange)))
in (uu____1393)::[])
in (uu____1389)::uu____1390))
in (uu____1385)::uu____1386))
in ((uu____1381), (uu____1382))))
in FStar_Syntax_Syntax.Record_ctor (uu____1374))
in (

let fv = (FStar_Syntax_Syntax.lid_as_fv lid FStar_Syntax_Syntax.delta_constant (FStar_Pervasives_Native.Some (attr)))
in (

let uu____1397 = (FStar_Syntax_Syntax.fv_to_tm fv)
in {lid = lid; fv = fv; t = uu____1397}))))
...
let uu____1398 = (FStar_Ident.lid_of_path (("FStar")::("Reflection")::("Data")::("exp")::[]) FStar_Range.dummyRange)
in (FStar_Syntax_Syntax.tconst uu____1398))
