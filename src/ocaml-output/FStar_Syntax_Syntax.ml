...
| Sig_inductive_typ (_29_238) -> begin
_29_238
end))
...
# 380 "FStar.Syntax.Syntax.fst"
let _29_300 = x
in {ppname = (FStar_Ident.mk_ident (x.ppname.FStar_Ident.idText, r)); index = _29_300.index; sort = _29_300.sort}))

# 387 "FStar.Syntax.Syntax.fst"
let syn = (fun p k f -> (f k p))

# 388 "FStar.Syntax.Syntax.fst"
let mk_fvs = (fun _29_305 -> (match (()) with
| () -> begin
(FStar_Util.mk_ref None)
end))

# 389 "FStar.Syntax.Syntax.fst"
let mk_uvs = (fun _29_306 -> (match (()) with
| () -> begin
(FStar_Util.mk_ref None)
end))

# 390 "FStar.Syntax.Syntax.fst"
let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun _29_307 -> (match (()) with
| () -> begin
(FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText))))
end))

# 391 "FStar.Syntax.Syntax.fst"
let new_uv_set : Prims.unit  ->  uvars = (fun _29_309 -> (match (()) with
| () -> begin
(FStar_Util.new_set (fun _29_317 _29_321 -> (match ((_29_317, _29_321)) with
| ((x, _29_316), (y, _29_320)) -> begin
((FStar_Unionfind.uvar_id x) - (FStar_Unionfind.uvar_id y))
end)) (fun _29_313 -> (match (_29_313) with
| (x, _29_312) -> begin
...
let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun _29_322 -> (match (()) with
| () -> begin
...
let mk = (fun t topt r -> (let _110_1192 = (FStar_Util.mk_ref topt)
in (let _110_1191 = (FStar_Util.mk_ref None)
in {n = t; tk = _110_1192; pos = r; vars = _110_1191})))

# 416 "FStar.Syntax.Syntax.fst"
let bv_to_tm : bv  ->  term = (fun bv -> (let _110_1195 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) (Some (bv.sort.n)) _110_1195)))

# 417 "FStar.Syntax.Syntax.fst"
let bv_to_name : bv  ->  term = (fun bv -> (let _110_1198 = (range_of_bv bv)
in (mk (Tm_name (bv)) (Some (bv.sort.n)) _110_1198)))
...
let mk_Tm_delayed : ((term * subst_ts), Prims.unit  ->  term) FStar_Util.either  ->  FStar_Range.range  ->  term = (fun lr pos -> (let _110_1233 = (let _110_1232 = (let _110_1231 = (FStar_Util.mk_ref None)
in (lr, _110_1231))
in Tm_delayed (_110_1232))
in (mk _110_1233 None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun _29_375 -> (match (_29_375) with
...
let null_binder : term  ->  binder = (fun t -> (let _110_1260 = (null_bv t)
in (_110_1260, None)))
...
| {lbname = FStar_Util.Inr (_29_410); lbunivs = _29_408; lbtyp = _29_406; lbeff = _29_404; lbdef = _29_402}::_29_400 -> begin
true
end
| _29_415 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun _29_420 out -> (match (_29_420) with
| (x, _29_419) -> begin
...
let binders_of_freenames : freenames  ->  binders = (fun fvs -> (let _110_1280 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right _110_1280 binders_of_list)))

# 469 "FStar.Syntax.Syntax.fst"
let is_implicit : aqual  ->  Prims.bool = (fun _29_3 -> (match (_29_3) with
| Some (Implicit (_29_427)) -> begin
true
end
| _29_431 -> begin
false
...
| Pat_cons (_29_450, pats) -> begin
(FStar_List.fold_left (fun b _29_458 -> (match (_29_458) with
| (p, _29_457) -> begin
(aux b p)
end)) b pats)
end
| Pat_disj (p::_29_460) -> begin
(aux b p)
end
| Pat_disj ([]) -> begin
(FStar_All.failwith "impossible")
end))
in (let _110_1293 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev _110_1293))))
...
let gen = (fun _29_468 -> (match (()) with
| () -> begin
(
# 486 "FStar.Syntax.Syntax.fst"
let _29_469 = (FStar_Util.incr x)
in (FStar_ST.read x))
end))
in (
# 487 "FStar.Syntax.Syntax.fst"
let reset = (fun _29_472 -> (match (()) with
| () -> begin
...
# 491 "FStar.Syntax.Syntax.fst"
let _29_474 = bv
in (let _110_1312 = (next_id ())
in {ppname = _29_474.ppname; index = _110_1312; sort = _29_474.sort})))
...
in (let _110_1321 = (next_id ())
in {ppname = id; index = _110_1321; sort = t})))
...
let id = (next_id ())
in (let _110_1329 = (let _110_1328 = (FStar_Util.string_of_int id)
in (_110_1328, (range_of_ropt ropt)))
in (FStar_Ident.mk_ident _110_1329))))
...
# 509 "FStar.Syntax.Syntax.fst"
let _29_511 = bv
in {ppname = (FStar_Ident.mk_ident (bv.ppname.FStar_Ident.idText, r)); index = _29_511.index; sort = _29_511.sort}))

# 510 "FStar.Syntax.Syntax.fst"
let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (let _110_1358 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = _110_1358; fv_delta = dd; fv_qual = dq}))
...
let fvar : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  term = (fun l dd dq -> (let _110_1367 = (lid_as_fv l dd dq)
in (fv_to_tm _110_1367)))
