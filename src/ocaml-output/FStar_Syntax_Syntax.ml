...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___88_5332 -> (match (uu___88_5332) with
| Reflectable (uu____5333) -> begin
true
end
| uu____5334 -> begin
false
...
let uu___95_5407 = x
in {ppname = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r))); index = uu___95_5407.index; sort = uu___95_5407.sort}))


let syn : 'Auu____5420 'Auu____5421 'Auu____5422 . 'Auu____5422  ->  'Auu____5421  ->  ('Auu____5421  ->  'Auu____5422  ->  'Auu____5420)  ->  'Auu____5420 = (fun p k f -> (f k p))


let mk_fvs : 'Auu____5456 . Prims.unit  ->  'Auu____5456 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____5464 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let mk_uvs : 'Auu____5478 . Prims.unit  ->  'Auu____5478 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____5486 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____5494 -> (FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText)))))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____5504 -> (FStar_Util.new_set order_fv (fun x -> (FStar_Util.hashcode x.FStar_Ident.str))))


let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____5514 -> (FStar_Util.new_fifo_set (fun x y -> (FStar_String.compare (FStar_Ident.text_of_id x) (FStar_Ident.text_of_id y))) (fun x -> (FStar_Util.hashcode (FStar_Ident.text_of_id x)))))
...
let mk : 'a . 'a  ->  'a mk_t_a = (fun t uu____5564 r -> (

let uu____5568 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in {n = t; pos = r; vars = uu____5568}))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____5591 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) FStar_Pervasives_Native.None uu____5591)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____5596 = (range_of_bv bv)
in (mk (Tm_name (bv)) FStar_Pervasives_Native.None uu____5596)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___89_5629 -> (match (uu___89_5629) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____5631) -> begin
...
in ((lr), (uu____5741)))
in Tm_delayed (uu____5716))
in (mk uu____5715))
in (uu____5712 FStar_Pervasives_Native.None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____5836 -> (match (uu____5836) with
...
let uu____5913 = (null_bv t)
in ((uu____5913), (FStar_Pervasives_Native.None))))
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___90_5941 -> (match (uu___90_5941) with
| ({lbname = FStar_Util.Inr (uu____5944); lbunivs = uu____5945; lbtyp = uu____5946; lbeff = uu____5947; lbdef = uu____5948})::uu____5949 -> begin
true
end
| uu____5958 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____5975 out -> (match (uu____5975) with
| (x, uu____5986) -> begin
...
let uu____6019 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____6019 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___91_6027 -> (match (uu___91_6027) with
| FStar_Pervasives_Native.Some (Implicit (uu____6028)) -> begin
true
end
| uu____6029 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___92_6033 -> (match (uu___92_6033) with
| true -> begin
...
| Pat_dot_term (uu____6062) -> begin
b
end
| Pat_constant (uu____6069) -> begin
b
end
| Pat_wild (x) -> begin
(x)::b
...
| Pat_cons (uu____6072, pats) -> begin
(FStar_List.fold_left (fun b1 uu____6103 -> (match (uu____6103) with
| (p2, uu____6115) -> begin
(aux b1 p2)
end)) b pats)
end))
...
let uu____6120 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____6120))))
...
let gen1 = (fun uu____6141 -> ((FStar_Util.incr x);
(FStar_ST.op_Bang x);
))
in (

let reset = (fun uu____6191 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range FStar_Pervasives_Native.option  ->  FStar_Range.range = (fun uu___93_6239 -> (match (uu___93_6239) with
...
let uu____6268 = (next_id ())
in {ppname = id; index = uu____6268; sort = t})))
...
let uu____6289 = (range_of_bv bv)
in FStar_Pervasives_Native.Some (uu____6289))
in (new_bv uu____6286 bv.sort))
end
| uu____6290 -> begin
(
...
let uu____6292 = (next_id ())
in {ppname = uu___96_6291.ppname; index = uu____6292; sort = uu___96_6291.sort}))
end)))
...
let uu____6308 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____6308))
in ((uu____6307), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____6302))))
...
let uu___97_6414 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___97_6414.index; sort = uu___97_6414.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual FStar_Pervasives_Native.option  ->  fv = (fun l dd dq -> (

let uu____6431 = (withinfo l (FStar_Ident.range_of_lid l))
in {fv_name = uu____6431; fv_delta = dd; fv_qual = dq}))
...
let uu____6452 = (lid_as_fv l dd dq)
in (fv_to_tm uu____6452)))
...
let uu____6461 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____6461)))
...
let uu___99_6472 = fv.fv_name
in (

let uu____6473 = (

let uu____6474 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____6474 r))
in {v = uu____6473; p = uu___99_6472.p}))
in {fv_name = uu____6471; fv_delta = uu___98_6470.fv_delta; fv_qual = uu___98_6470.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___94_6494 -> (match (uu___94_6494) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____6498)); pos = uu____6499; vars = uu____6500} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____6507 -> begin
false
...
let uu____6518 = (lid_as_fv l Delta_constant FStar_Pervasives_Native.None)
in Tm_fvar (uu____6518))
in (mk uu____6517))
in (uu____6514 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____6530 = (lid_as_fv l (Delta_defined_at_level ((Prims.parse_int "1"))) FStar_Pervasives_Native.None)
in Tm_fvar (uu____6530))
in (mk uu____6529))
in (uu____6526 FStar_Pervasives_Native.None FStar_Range.dummyRange)))


let tdataconstr : FStar_Ident.lident  ->  term = (fun l -> (

let uu____6538 = (lid_as_fv l Delta_constant (FStar_Pervasives_Native.Some (Data_ctor)))
in (fv_to_tm uu____6538)))
...
let uu____6543 = (tabbrev FStar_Parser_Const.tactic_lid)
in (mk_Tm_uinst uu____6543 ((U_zero)::[])))
in (

let uu____6544 = (

let uu____6545 = (as_arg t_unit)
in (uu____6545)::[])
in (mk_Tm_app uu____6542 uu____6544)))
in (uu____6541 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____6552 = (tabbrev FStar_Parser_Const.u_tac_lid)
in (mk_Tm_uinst uu____6552 ((U_zero)::[])))
in (

let uu____6553 = (

let uu____6554 = (as_arg t_unit)
in (uu____6554)::[])
in (mk_Tm_app uu____6551 uu____6553)))
in (uu____6550 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____6563 = (tabbrev FStar_Parser_Const.list_lid)
in (mk_Tm_uinst uu____6563 ((U_zero)::[])))
in (

let uu____6564 = (

let uu____6565 = (as_arg t)
in (uu____6565)::[])
in (mk_Tm_app uu____6562 uu____6564)))
in (uu____6561 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____6574 = (tabbrev FStar_Parser_Const.option_lid)
in (mk_Tm_uinst uu____6574 ((U_zero)::[])))
in (

let uu____6575 = (

let uu____6576 = (as_arg t)
in (uu____6576)::[])
in (mk_Tm_app uu____6573 uu____6575)))
in (uu____6572 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
