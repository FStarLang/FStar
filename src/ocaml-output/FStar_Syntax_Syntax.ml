...
let mk = (fun t topt r -> (let _111_1206 = (FStar_Util.mk_ref topt)
in (let _111_1205 = (FStar_Util.mk_ref None)
in {n = t; tk = _111_1206; pos = r; vars = _111_1205})))

# 417 "FStar.Syntax.Syntax.fst"
let bv_to_tm : bv  ->  term = (fun bv -> (let _111_1209 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) (Some (bv.sort.n)) _111_1209)))

# 418 "FStar.Syntax.Syntax.fst"
let bv_to_name : bv  ->  term = (fun bv -> (let _111_1212 = (range_of_bv bv)
in (mk (Tm_name (bv)) (Some (bv.sort.n)) _111_1212)))
...
let mk_Tm_delayed : ((term * subst_ts), Prims.unit  ->  term) FStar_Util.either  ->  FStar_Range.range  ->  term = (fun lr pos -> (let _111_1247 = (let _111_1246 = (let _111_1245 = (FStar_Util.mk_ref None)
in (lr, _111_1245))
in Tm_delayed (_111_1246))
in (mk _111_1247 None pos)))
...
let null_binder : term  ->  binder = (fun t -> (let _111_1274 = (null_bv t)
in (_111_1274, None)))
...
let binders_of_freenames : freenames  ->  binders = (fun fvs -> (let _111_1294 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right _111_1294 binders_of_list)))
...
in (let _111_1307 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev _111_1307))))
...
# 492 "FStar.Syntax.Syntax.fst"
let _29_477 = bv
in (let _111_1326 = (next_id ())
in {ppname = _29_477.ppname; index = _111_1326; sort = _29_477.sort})))
...
in (let _111_1335 = (next_id ())
in {ppname = id; index = _111_1335; sort = t})))
...
let id = (next_id ())
in (let _111_1343 = (let _111_1342 = (FStar_Util.string_of_int id)
in (_111_1342, (range_of_ropt ropt)))
in (FStar_Ident.mk_ident _111_1343))))
...
let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (let _111_1372 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = _111_1372; fv_delta = dd; fv_qual = dq}))
...
let fvar : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  term = (fun l dd dq -> (let _111_1381 = (lid_as_fv l dd dq)
in (fv_to_tm _111_1381)))
