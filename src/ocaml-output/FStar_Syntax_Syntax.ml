...
let syn p k f = f k p
let mk_fvs uu____3276 = FStar_Util.mk_ref None
let mk_uvs uu____3288 = FStar_Util.mk_ref None
...
let mk_Tm_uinst: term -> universes -> term =
  fun t  ->
    fun uu___88_3570  ->
      ...
      | us ->
          (match t.n with
           | Tm_fvar uu____3572 -> mk (Tm_uinst (t, us)) None t.pos
  ...
  fun lr  ->
    fun pos  ->
      let uu____3652 =
        let uu____3655 =
          let uu____3656 =
            let uu____3677 = FStar_Util.mk_ref None in (lr, uu____3677) in
...
let null_binder: term -> (bv* arg_qualifier option) =
  fun t  -> let uu____3819 = null_bv t in (uu____3819, None)
...
let is_implicit: aqual -> Prims.bool =
  fun uu___90_3891  ->
    ...
    | Some (Implicit uu____3892) -> true
    | uu____3893 -> false
let as_implicit: Prims.bool -> arg_qualifier option =
  fun uu___91_3896  -> if uu___91_3896 then Some imp_tag else None
...
let range_of_ropt: FStar_Range.range option -> FStar_Range.range =
  fun uu___92_4005  ->
    match uu___92_4005 with | None  -> FStar_Range.dummyRange | Some r -> r
...
let freshen_bv: bv -> bv =
  fun bv  ->
    ...
    if uu____4032
    then
      let uu____4033 = let uu____4035 = range_of_bv bv in Some uu____4035 in
