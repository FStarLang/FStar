...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___86_3224 -> (match (uu___86_3224) with
| Reflectable (uu____3225) -> begin
true
end
| uu____3226 -> begin
false
...
let uu___93_3302 = x
in {ppname = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r))); index = uu___93_3302.index; sort = uu___93_3302.sort}))


let syn = (fun p k f -> (f k p))


let mk_fvs = (fun uu____3343 -> (FStar_Util.mk_ref None))


let mk_uvs = (fun uu____3355 -> (FStar_Util.mk_ref None))


let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____3361 -> (FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText)))))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____3367 -> (FStar_Util.new_set order_fv (fun x -> (FStar_Util.hashcode x.FStar_Ident.str))))


let new_uv_set : Prims.unit  ->  uvars = (fun uu____3372 -> (FStar_Util.new_set (fun uu____3381 uu____3382 -> (match (((uu____3381), (uu____3382))) with
| ((x, uu____3416), (y, uu____3418)) -> begin
(

let uu____3459 = (FStar_Unionfind.uvar_id x)
in (

let uu____3463 = (FStar_Unionfind.uvar_id y)
in (uu____3459 - uu____3463)))
end)) (fun uu____3467 -> (match (uu____3467) with
| (x, uu____3477) -> begin
(FStar_Unionfind.uvar_id x)
end))))


let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun uu____3496 -> (FStar_Util.new_set (fun x y -> (

let uu____3506 = (FStar_Unionfind.uvar_id x)
in (

let uu____3508 = (FStar_Unionfind.uvar_id y)
in (uu____3506 - uu____3508)))) (fun x -> (FStar_Unionfind.uvar_id x))))


let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____3519 -> (FStar_Util.new_fifo_set (fun x y -> (FStar_String.compare (FStar_Ident.text_of_id x) (FStar_Ident.text_of_id y))) (fun x -> (FStar_Util.hashcode (FStar_Ident.text_of_id x)))))
...
let uu____3581 = (FStar_Util.mk_ref topt)
in (

let uu____3585 = (FStar_Util.mk_ref None)
in {n = t; tk = uu____3581; pos = r; vars = uu____3585})))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____3596 = (range_of_bv bv)
in ((mk (Tm_bvar (bv))) (Some (bv.sort.n)) uu____3596)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____3608 = (range_of_bv bv)
in ((mk (Tm_name (bv))) (Some (bv.sort.n)) uu____3608)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___87_3649 -> (match (uu___87_3649) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____3651) -> begin
...
let uu____3766 = (FStar_Util.mk_ref None)
in ((lr), (uu____3766)))
in Tm_delayed (uu____3745))
in (mk uu____3744))
in (uu____3741 None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____3856 -> (match (uu____3856) with
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___88_3947 -> (match (uu___88_3947) with
| ({lbname = FStar_Util.Inr (uu____3949); lbunivs = uu____3950; lbtyp = uu____3951; lbeff = uu____3952; lbdef = uu____3953})::uu____3954 -> begin
true
end
| uu____3961 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____3969 out -> (match (uu____3969) with
| (x, uu____3976) -> begin
...
let uu____3995 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____3995 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___89_4000 -> (match (uu___89_4000) with
| Some (Implicit (uu____4001)) -> begin
true
end
| uu____4002 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___90_4005 -> (match (uu___90_4005) with
| true -> begin
Some (imp_tag)
end
| uu____4006 -> begin
...
| Pat_cons (uu____4030, pats) -> begin
(FStar_List.fold_left (fun b1 uu____4048 -> (match (uu____4048) with
| (p2, uu____4056) -> begin
(aux b1 p2)
end)) b pats)
end
| Pat_disj ((p2)::uu____4062) -> begin
(aux b p2)
end
| Pat_disj ([]) -> begin
(failwith "impossible")
...
let uu____4073 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____4073))))
...
let gen1 = (fun uu____4089 -> ((FStar_Util.incr x);
(FStar_ST.read x);
))
in (

let reset = (fun uu____4099 -> (FStar_ST.write x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range Prims.option  ->  FStar_Range.range = (fun uu___91_4121 -> (match (uu___91_4121) with
| None -> begin
...
let uu____4136 = (next_id ())
in {ppname = id; index = uu____4136; sort = t})))
...
let uu____4151 = (range_of_bv bv)
in Some (uu____4151))
in (new_bv uu____4149 bv.sort))
end
| uu____4152 -> begin
(
...
let uu____4154 = (next_id ())
in {ppname = uu___94_4153.ppname; index = uu____4154; sort = uu___94_4153.sort}))
end)))
...
let uu____4165 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____4165))
in ((uu____4164), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____4161))))
...
let uu___95_4246 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___95_4246.index; sort = uu___95_4246.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (

let uu____4258 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = uu____4258; fv_delta = dd; fv_qual = dq}))
...
let uu____4293 = (lid_as_fv l dd dq)
in (fv_to_tm uu____4293)))
...
let uu____4304 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____4304)))
...
let uu___97_4316 = fv.fv_name
in (

let uu____4321 = (

let uu____4322 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____4322 r))
in {v = uu____4321; ty = uu___97_4316.ty; p = uu___97_4316.p}))
in {fv_name = uu____4312; fv_delta = uu___96_4311.fv_delta; fv_qual = uu___96_4311.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___92_4346 -> (match (uu___92_4346) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____4350)); tk = uu____4351; pos = uu____4352; vars = uu____4353} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____4358 -> begin
false
