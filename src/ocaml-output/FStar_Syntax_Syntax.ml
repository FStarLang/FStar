...
let (__proj__Mkeff_decl__item__is_layered : eff_decl -> Prims.bool) =
  fun projectee  ->
    match projectee with
...
let (uu___is_Sig_inductive_typ : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_inductive_typ _0 -> true | uu____7490 -> false
...
let (uu___is_Sig_bundle : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_bundle _0 -> true | uu____7565 -> false
...
let (uu___is_Sig_datacon : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_datacon _0 -> true | uu____7623 -> false
...
let (uu___is_Sig_declare_typ : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_declare_typ _0 -> true | uu____7693 -> false
...
let (uu___is_Sig_let : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_let _0 -> true | uu____7736 -> false
...
let (uu___is_Sig_main : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_main _0 -> true | uu____7773 -> false
...
let (uu___is_Sig_assume : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_assume _0 -> true | uu____7798 -> false
...
let (uu___is_Sig_new_effect : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_new_effect _0 -> true | uu____7835 -> false
...
let (uu___is_Sig_sub_effect : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_sub_effect _0 -> true | uu____7873 -> false
...
let (uu___is_Sig_pragma : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_pragma _0 -> true | uu____7959 -> false
...
let (uu___is_Sig_splice : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_splice _0 -> true | uu____7984 -> false
...
let (contains_reflectable : qualifier Prims.list -> Prims.bool) =
  fun l  ->
    FStar_Util.for_some
      (fun uu___0_8206  ->
         match uu___0_8206 with
         | Reflectable uu____8208 -> true
         | uu____8210 -> false) l
...
let (set_range_of_bv : bv -> FStar_Range.range -> bv) =
  fun x  ->
    fun r  ->
      let uu___425_8328 = x  in
      let uu____8329 =
        FStar_Ident.mk_ident (((x.ppname).FStar_Ident.idText), r)  in
      {
        ppname = uu____8329;
        index = (uu___425_8328.index);
        sort = (uu___425_8328.sort)
      }
      ...
      let aq =
        FStar_List.map
          (fun uu____8366  ->
             ...
             | (bv,t) -> let uu____8377 = f t  in (bv, uu____8377))
          qi.antiquotes
         in
      let uu___433_8378 = qi  in
      { qkind = (uu___433_8378.qkind); antiquotes = aq }
  ...
  fun bv  ->
    fun aq  ->
      let uu____8394 =
        FStar_List.tryFind
          (fun uu____8412  ->
             match uu____8412 with | (bv',uu____8421) -> bv_eq bv bv') aq
         in
      match uu____8394 with
      | FStar_Pervasives_Native.Some (uu____8428,e) ->
          FStar_Pervasives_Native.Some e
      | FStar_Pervasives_Native.None  -> FStar_Pervasives_Native.None
  
let syn :
  'Auu____8459 'Auu____8460 'Auu____8461 .
    'Auu____8459 ->
      'Auu____8460 ->
        ('Auu____8460 -> 'Auu____8459 -> 'Auu____8461) -> 'Auu____8461
  = fun p  -> fun k  -> fun f  -> f k p 
let mk_fvs :
  'Auu____8492 .
    unit -> 'Auu____8492 FStar_Pervasives_Native.option FStar_ST.ref
  = fun uu____8501  -> FStar_Util.mk_ref FStar_Pervasives_Native.None 
let mk_uvs :
  'Auu____8509 .
    unit -> 'Auu____8509 FStar_Pervasives_Native.option FStar_ST.ref
  = fun uu____8518  -> FStar_Util.mk_ref FStar_Pervasives_Native.None 
let (new_bv_set : unit -> bv FStar_Util.set) =
  fun uu____8528  -> FStar_Util.new_set order_bv 
let (new_id_set : unit -> FStar_Ident.ident FStar_Util.set) =
  fun uu____8538  -> FStar_Util.new_set order_ident 
let (new_fv_set : unit -> FStar_Ident.lident FStar_Util.set) =
  fun uu____8548  -> FStar_Util.new_set order_fv 
let (order_univ_name : univ_name -> univ_name -> Prims.int) =
  fun x  ->
    fun y  ->
      let uu____8563 = FStar_Ident.text_of_id x  in
      let uu____8565 = FStar_Ident.text_of_id y  in
      FStar_String.compare uu____8563 uu____8565
  
let (new_universe_names_set : unit -> univ_name FStar_Util.set) =
  fun uu____8574  -> FStar_Util.new_set order_univ_name 
let (eq_binding : binding -> binding -> Prims.bool) =
  fun b1  ->
    fun b2  ->
      match (b1, b2) with
      | (Binding_var bv1,Binding_var bv2) -> bv_eq bv1 bv2
      | (Binding_lid (lid1,uu____8593),Binding_lid (lid2,uu____8595)) ->
...
let mk : 'a . 'a -> 'a mk_t_a =
  fun t  ->
    fun uu____8684  ->
      fun r  ->
        let uu____8688 = FStar_Util.mk_ref FStar_Pervasives_Native.None  in
        { n = t; pos = r; vars = uu____8688 }
  
let (bv_to_tm : bv -> term) =
  fun bv  ->
    let uu____8699 = range_of_bv bv  in
    mk (Tm_bvar bv) FStar_Pervasives_Native.None uu____8699
  
let (bv_to_name : bv -> term) =
  fun bv  ->
    let uu____8706 = range_of_bv bv  in
    mk (Tm_name bv) FStar_Pervasives_Native.None uu____8706
...
let (binders_to_names : binders -> term Prims.list) =
  fun bs  ->
    FStar_All.pipe_right bs
      (FStar_List.map
         (fun uu____8736  ->
            match uu____8736 with | (x,uu____8744) -> bv_to_name x))
...
let (mk_Tm_uinst : term -> universes -> term) =
  fun t  ->
    fun uu___1_8799  ->
      match uu___1_8799 with
      | [] -> t
      | us ->
          (match t.n with
           | Tm_fvar uu____8801 ->
               mk (Tm_uinst (t, us)) FStar_Pervasives_Native.None t.pos
           | uu____8804 -> failwith "Unexpected universe instantiation")
    ...
    fun args'  ->
      fun kopt  ->
        fun r  ->
          ...
          | uu____8863 -> mk_Tm_app t args' kopt r
  ...
  fun lr  ->
    fun pos  ->
      let uu____8920 =
        let uu____8927 =
          let uu____8928 =
            let uu____8951 = FStar_Util.mk_ref FStar_Pervasives_Native.None
               in
            (lr, uu____8951)  in
          Tm_delayed uu____8928  in
        mk uu____8927  in
      uu____8920 FStar_Pervasives_Native.None pos
...
let (is_type : term -> Prims.bool) =
  fun t  -> match t.n with | Tm_type uu____9154 -> true | uu____9156 -> false 
...
let (null_binder : term -> binder) =
  fun t  ->
    let uu____9182 = null_bv t  in (uu____9182, FStar_Pervasives_Native.None)
...
let (is_top_level : letbinding Prims.list -> Prims.bool) =
  fun uu___2_9232  ->
    match uu___2_9232 with
    | { lbname = FStar_Util.Inr uu____9236; lbunivs = uu____9237;
        lbtyp = uu____9238; lbeff = uu____9239; lbdef = uu____9240;
        lbattrs = uu____9241; lbpos = uu____9242;_}::uu____9243 -> true
    | uu____9257 -> false
...
let (freenames_of_binders : binders -> freenames) =
  fun bs  ->
    FStar_List.fold_right
      (fun uu____9279  ->
         fun out  ->
           match uu____9279 with | (x,uu____9292) -> FStar_Util.set_add x out)
      bs no_names
...
let (binders_of_freenames : freenames -> binders) =
  fun fvs  ->
    let uu____9325 = FStar_Util.set_elements fvs  in
    FStar_All.pipe_right uu____9325 binders_of_list
  
let (is_implicit : aqual -> Prims.bool) =
  fun uu___3_9336  ->
    match uu___3_9336 with
    | FStar_Pervasives_Native.Some (Implicit uu____9338) -> true
    | uu____9341 -> false
...
let (pat_bvs : pat -> bv Prims.list) =
  fun p  ->
    let rec aux b p1 =
      match p1.v with
      | Pat_dot_term uu____9387 -> b
      | Pat_constant uu____9394 -> b
      | Pat_wild x -> x :: b
      | Pat_var x -> x :: b
      | Pat_cons (uu____9397,pats) ->
          FStar_List.fold_left
            (fun b1  ->
               fun uu____9431  ->
                 match uu____9431 with | (p2,uu____9444) -> aux b1 p2) b pats
       in
    let uu____9451 = aux [] p  in
    FStar_All.pipe_left FStar_List.rev uu____9451
  
let (range_of_ropt :
  FStar_Range.range FStar_Pervasives_Native.option -> FStar_Range.range) =
  fun uu___5_9465  ->
    match uu___5_9465 with
  ...
  fun s  ->
    fun r  ->
      fun t  ->
        ...
        let uu____9505 = FStar_Ident.next_id ()  in
        { ppname = id1; index = uu____9505; sort = t }
...
let (freshen_bv : bv -> bv) =
  fun bv  ->
    let uu____9528 = is_null_bv bv  in
    if uu____9528
    then
      let uu____9531 =
        let uu____9534 = range_of_bv bv  in
        FStar_Pervasives_Native.Some uu____9534  in
      new_bv uu____9531 bv.sort
    else
      (let uu___614_9537 = bv  in
       let uu____9538 = FStar_Ident.next_id ()  in
       {
         ppname = (uu___614_9537.ppname);
         index = uu____9538;
         sort = (uu___614_9537.sort)
       })
...
let (freshen_binder : binder -> binder) =
  fun b  ->
    ...
    | (bv,aq) -> let uu____9553 = freshen_bv bv  in (uu____9553, aq)
...
let (new_univ_name :
  ...
  fun ropt  ->
    ...
    let uu____9568 =
      let uu____9574 =
        let uu____9576 = FStar_Util.string_of_int id1  in
        Prims.op_Hat FStar_Ident.reserved_prefix uu____9576  in
      (uu____9574, (range_of_ropt ropt))  in
    FStar_Ident.mk_ident uu____9568
...
let (set_bv_range : bv -> FStar_Range.range -> bv) =
  fun bv  ->
    fun r  ->
      let uu___644_9707 = bv  in
      let uu____9708 =
        FStar_Ident.mk_ident (((bv.ppname).FStar_Ident.idText), r)  in
      {
        ppname = uu____9708;
        index = (uu___644_9707.index);
        sort = (uu___644_9707.sort)
      }
...
let (lid_as_fv :
  ...
  fun l  ->
    fun dd  ->
      fun dq  ->
        let uu____9730 =
          let uu____9731 = FStar_Ident.range_of_lid l  in
          withinfo l uu____9731  in
        { fv_name = uu____9730; fv_delta = dd; fv_qual = dq }
  
let (fv_to_tm : fv -> term) =
  fun fv  ->
    let uu____9738 = FStar_Ident.range_of_lid (fv.fv_name).v  in
    mk (Tm_fvar fv) FStar_Pervasives_Native.None uu____9738
...
let (fvar :
  ...
  fun l  ->
    fun dd  ->
      fun dq  -> let uu____9759 = lid_as_fv l dd dq  in fv_to_tm uu____9759
  
let (lid_of_fv : fv -> FStar_Ident.lid) = fun fv  -> (fv.fv_name).v 
let (range_of_fv : fv -> FStar_Range.range) =
  fun fv  ->
    let uu____9772 = lid_of_fv fv  in FStar_Ident.range_of_lid uu____9772
  
let (set_range_of_fv : fv -> FStar_Range.range -> fv) =
  fun fv  ->
    fun r  ->
      let uu___657_9784 = fv  in
      let uu____9785 =
        let uu___659_9786 = fv.fv_name  in
        let uu____9787 =
          let uu____9788 = lid_of_fv fv  in
          FStar_Ident.set_lid_range uu____9788 r  in
        { v = uu____9787; p = (uu___659_9786.p) }  in
      {
        fv_name = uu____9785;
        fv_delta = (uu___657_9784.fv_delta);
        fv_qual = (uu___657_9784.fv_qual)
      }
...
let (has_simple_attribute : term Prims.list -> Prims.string -> Prims.bool) =
  fun l  ->
    fun s  ->
      FStar_List.existsb
        (fun uu___6_9814  ->
           match uu___6_9814 with
           | { n = Tm_constant (FStar_Const.Const_string (data,uu____9819));
               pos = uu____9820; vars = uu____9821;_} when data = s -> true
           | uu____9828 -> false) l
  ...
  fun p1  ->
    fun p2  ->
      ...
      | (Pat_cons (fv1,as1),Pat_cons (fv2,as2)) ->
          let uu____9887 = fv_eq fv1 fv2  in
          if uu____9887
          then
            let uu____9892 = FStar_List.zip as1 as2  in
      ...
      | (Pat_var uu____9997,Pat_var uu____9998) -> true
      | (Pat_wild uu____10000,Pat_wild uu____10001) -> true
...
let (tconst : FStar_Ident.lident -> term) =
  fun l  ->
    let uu____10035 =
      let uu____10042 = let uu____10043 = fvconst l  in Tm_fvar uu____10043
         in
      mk uu____10042  in
...
let (t_tactic_unit : term) =
  let uu____10085 =
    let uu____10090 =
      let uu____10091 = tabbrev FStar_Parser_Const.tactic_lid  in
      mk_Tm_uinst uu____10091 [U_zero]  in
    let uu____10092 = let uu____10093 = as_arg t_unit  in [uu____10093]  in
    mk_Tm_app uu____10090 uu____10092  in
  uu____10085 FStar_Pervasives_Native.None FStar_Range.dummyRange 
