...
let contains_reflectable: qualifier Prims.list -> Prims.bool =
  fun l  ->
    FStar_Util.for_some
      (fun uu___86_3260  ->
         match uu___86_3260 with
         | Reflectable uu____3261 -> true
         | uu____3262 -> false) l
...
let set_range_of_bv: bv -> FStar_Range.range -> bv =
  fun x  ->
    fun r  ->
      let uu___93_3338 = x in
      {
        ppname = (FStar_Ident.mk_ident (((x.ppname).FStar_Ident.idText), r));
        index = (uu___93_3338.index);
        sort = (uu___93_3338.sort)
      }
let syn p k f = f k p
let mk_fvs uu____3379 = FStar_Util.mk_ref None
let mk_uvs uu____3391 = FStar_Util.mk_ref None
...
let new_uv_set:
  ...
  fun uu____3408  ->
    FStar_Util.new_set
      (fun uu____3417  ->
         fun uu____3418  ->
           match (uu____3417, uu____3418) with
           | ((x,uu____3452),(y,uu____3454)) ->
               let uu____3495 = FStar_Unionfind.uvar_id x in
               let uu____3499 = FStar_Unionfind.uvar_id y in
               uu____3495 - uu____3499)
      (fun uu____3503  ->
         match uu____3503 with | (x,uu____3513) -> FStar_Unionfind.uvar_id x)
let new_universe_uvar_set:
  ...
  fun uu____3532  ->
    FStar_Util.new_set
      (fun x  ->
         fun y  ->
           let uu____3542 = FStar_Unionfind.uvar_id x in
           let uu____3544 = FStar_Unionfind.uvar_id y in
           uu____3542 - uu____3544) (fun x  -> FStar_Unionfind.uvar_id x)
...
let mk t topt r =
  let uu____3617 = FStar_Util.mk_ref topt in
  let uu____3621 = FStar_Util.mk_ref None in
  { n = t; tk = uu____3617; pos = r; vars = uu____3621 }
let bv_to_tm: bv -> (term',term') syntax =
  fun bv  ->
    let uu____3632 = range_of_bv bv in
    (mk (Tm_bvar bv)) (Some ((bv.sort).n)) uu____3632
let bv_to_name: bv -> (term',term') syntax =
  fun bv  ->
    let uu____3644 = range_of_bv bv in
    (mk (Tm_name bv)) (Some ((bv.sort).n)) uu____3644
let mk_Tm_app:
  typ ->
    arg Prims.list ->
    ...
    fun args  ->
      fun k  ->
        fun p  ->
          match args with
          | [] -> t1
          | uu____3669 -> (mk (Tm_app (t1, args))) k p
let mk_Tm_uinst: term -> universes -> term =
  fun t  ->
    fun uu___87_3685  ->
      match uu___87_3685 with
      | [] -> t
      | us ->
          (match t.n with
           | Tm_fvar uu____3687 -> (mk (Tm_uinst (t, us))) None t.pos
           | uu____3696 -> failwith "Unexpected universe instantiation")
let extend_app_n:
  term ->
    ...
    fun args'  ->
      fun kopt  ->
        fun r  ->
          ...
          | uu____3736 -> (mk_Tm_app t args') kopt r
let extend_app:
  term ->
  ...
  fun lr  ->
    fun pos  ->
      let uu____3777 =
        let uu____3780 =
          let uu____3781 =
            let uu____3802 = FStar_Util.mk_ref None in (lr, uu____3802) in
          Tm_delayed uu____3781 in
        mk uu____3780 in
      uu____3777 None pos
...
let is_type: term -> Prims.bool =
  fun t  -> match t.n with | Tm_type uu____3952 -> true | uu____3953 -> false
...
let null_binder: term -> (bv* arg_qualifier Prims.option) =
  fun t  -> let uu____3964 = null_bv t in (uu____3964, None)
...
let is_top_level: letbinding Prims.list -> Prims.bool =
  fun uu___88_3983  ->
    match uu___88_3983 with
    | { lbname = FStar_Util.Inr uu____3985; lbunivs = uu____3986;
        lbtyp = uu____3987; lbeff = uu____3988; lbdef = uu____3989;_}::uu____3990
        -> true
    | uu____3997 -> false
let freenames_of_binders: binders -> bv FStar_Util.set =
  fun bs  ->
    FStar_List.fold_right
      (fun uu____4005  ->
         fun out  ->
           match uu____4005 with | (x,uu____4012) -> FStar_Util.set_add x out)
      bs no_names
let binders_of_list:
...
let binders_of_freenames: freenames -> binders =
  fun fvs  ->
    let uu____4031 = FStar_Util.set_elements fvs in
    FStar_All.pipe_right uu____4031 binders_of_list
let is_implicit: aqual -> Prims.bool =
  fun uu___89_4036  ->
    match uu___89_4036 with
    | Some (Implicit uu____4037) -> true
    | uu____4038 -> false
let as_implicit: Prims.bool -> arg_qualifier Prims.option =
  fun uu___90_4041  -> if uu___90_4041 then Some imp_tag else None
let pat_bvs: pat -> bv Prims.list =
  fun p  ->
    let rec aux b p1 =
      ...
      | Pat_cons (uu____4066,pats) ->
          FStar_List.fold_left
            (fun b1  ->
               fun uu____4084  ->
                 match uu____4084 with | (p2,uu____4092) -> aux b1 p2) b pats
      | Pat_disj (p2::uu____4098) -> aux b p2
      | Pat_disj [] -> failwith "impossible" in
    let uu____4109 = aux [] p in
    FStar_All.pipe_left FStar_List.rev uu____4109
let gen_reset: ((Prims.unit -> Prims.int)* (Prims.unit -> Prims.unit)) =
  let x = FStar_Util.mk_ref (Prims.parse_int "0") in
  let gen1 uu____4125 = FStar_Util.incr x; FStar_ST.read x in
  let reset uu____4135 = FStar_ST.write x (Prims.parse_int "0") in
  (gen1, reset)
...
let range_of_ropt: FStar_Range.range Prims.option -> FStar_Range.range =
  fun uu___91_4157  ->
    match uu___91_4157 with | None  -> FStar_Range.dummyRange | Some r -> r
let gen_bv: Prims.string -> FStar_Range.range Prims.option -> typ -> bv =
  fun s  ->
    fun r  ->
      fun t  ->
        let id = FStar_Ident.mk_ident (s, (range_of_ropt r)) in
        let uu____4172 = next_id () in
        { ppname = id; index = uu____4172; sort = t }
...
let freshen_bv: bv -> bv =
  fun bv  ->
    let uu____4184 = is_null_bv bv in
    if uu____4184
    then
      let uu____4185 = let uu____4187 = range_of_bv bv in Some uu____4187 in
      new_bv uu____4185 bv.sort
    else
      (let uu___94_4189 = bv in
       let uu____4190 = next_id () in
       {
         ppname = (uu___94_4189.ppname);
         index = uu____4190;
         sort = (uu___94_4189.sort)
       })
let new_univ_name: FStar_Range.range Prims.option -> FStar_Ident.ident =
  fun ropt  ->
    ...
    let uu____4197 =
      let uu____4200 =
        let uu____4201 = FStar_Util.string_of_int id in
        Prims.strcat FStar_Ident.reserved_prefix uu____4201 in
      (uu____4200, (range_of_ropt ropt)) in
    FStar_Ident.mk_ident uu____4197
...
let set_bv_range: bv -> FStar_Range.range -> bv =
  fun bv  ->
    fun r  ->
      let uu___95_4282 = bv in
      {
        ppname = (FStar_Ident.mk_ident (((bv.ppname).FStar_Ident.idText), r));
        index = (uu___95_4282.index);
        sort = (uu___95_4282.sort)
      }
let lid_as_fv:
  ...
  fun l  ->
    fun dd  ->
      fun dq  ->
        let uu____4294 = withinfo l tun (FStar_Ident.range_of_lid l) in
        { fv_name = uu____4294; fv_delta = dd; fv_qual = dq }
...
let fvar: FStar_Ident.lident -> delta_depth -> fv_qual Prims.option -> term =
  fun l  ->
    fun dd  ->
      fun dq  -> let uu____4329 = lid_as_fv l dd dq in fv_to_tm uu____4329
let lid_of_fv: fv -> FStar_Ident.lident = fun fv  -> (fv.fv_name).v
let range_of_fv: fv -> FStar_Range.range =
  fun fv  ->
    let uu____4340 = lid_of_fv fv in FStar_Ident.range_of_lid uu____4340
let set_range_of_fv: fv -> FStar_Range.range -> fv =
  fun fv  ->
    fun r  ->
      let uu___96_4347 = fv in
      let uu____4348 =
        let uu___97_4352 = fv.fv_name in
        let uu____4357 =
          let uu____4358 = lid_of_fv fv in
          FStar_Ident.set_lid_range uu____4358 r in
        { v = uu____4357; ty = (uu___97_4352.ty); p = (uu___97_4352.p) } in
      {
        fv_name = uu____4348;
        fv_delta = (uu___96_4347.fv_delta);
        fv_qual = (uu___96_4347.fv_qual)
      }
let has_simple_attribute: term Prims.list -> Prims.string -> Prims.bool =
  fun l  ->
    fun s  ->
      FStar_List.existsb
        (fun uu___92_4382  ->
           match uu___92_4382 with
           | { n = Tm_constant (FStar_Const.Const_string (data,uu____4386));
               tk = uu____4387; pos = uu____4388; vars = uu____4389;_} when
