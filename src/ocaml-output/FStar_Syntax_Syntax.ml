...
| Sig_pragma of pragma 
 and sigelt =
{sigel : sigelt'; sigrng : FStar_Range.range}
...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___91_3233 -> (match (uu___91_3233) with
| Reflectable (uu____3234) -> begin
true
end
| uu____3235 -> begin
false
...
let uu___98_3311 = x
in {ppname = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r))); index = uu___98_3311.index; sort = uu___98_3311.sort}))


let syn = (fun p k f -> (f k p))


let mk_fvs = (fun uu____3352 -> (FStar_Util.mk_ref None))


let mk_uvs = (fun uu____3364 -> (FStar_Util.mk_ref None))


let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____3370 -> (FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText)))))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____3376 -> (FStar_Util.new_set order_fv (fun x -> (FStar_Util.hashcode x.FStar_Ident.str))))


let new_uv_set : Prims.unit  ->  uvars = (fun uu____3381 -> (FStar_Util.new_set (fun uu____3390 uu____3391 -> (match (((uu____3390), (uu____3391))) with
| ((x, uu____3425), (y, uu____3427)) -> begin
(

let uu____3468 = (FStar_Unionfind.uvar_id x)
in (

let uu____3472 = (FStar_Unionfind.uvar_id y)
in (uu____3468 - uu____3472)))
end)) (fun uu____3476 -> (match (uu____3476) with
| (x, uu____3486) -> begin
(FStar_Unionfind.uvar_id x)
end))))


let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun uu____3505 -> (FStar_Util.new_set (fun x y -> (

let uu____3515 = (FStar_Unionfind.uvar_id x)
in (

let uu____3517 = (FStar_Unionfind.uvar_id y)
in (uu____3515 - uu____3517)))) (fun x -> (FStar_Unionfind.uvar_id x))))


let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____3528 -> (FStar_Util.new_fifo_set (fun x y -> (FStar_String.compare (FStar_Ident.text_of_id x) (FStar_Ident.text_of_id y))) (fun x -> (FStar_Util.hashcode (FStar_Ident.text_of_id x)))))
...
let uu____3590 = (FStar_Util.mk_ref topt)
in (

let uu____3594 = (FStar_Util.mk_ref None)
in {n = t; tk = uu____3590; pos = r; vars = uu____3594})))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____3605 = (range_of_bv bv)
in ((mk (Tm_bvar (bv))) (Some (bv.sort.n)) uu____3605)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____3617 = (range_of_bv bv)
in ((mk (Tm_name (bv))) (Some (bv.sort.n)) uu____3617)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___92_3658 -> (match (uu___92_3658) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____3660) -> begin
...
let uu____3775 = (FStar_Util.mk_ref None)
in ((lr), (uu____3775)))
in Tm_delayed (uu____3754))
in (mk uu____3753))
in (uu____3750 None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____3865 -> (match (uu____3865) with
| (x, univs, eff, t, e) -> begin
{lbname = x; lbunivs = univs; lbtyp = t; lbeff = eff; lbdef = e}
end))


let mk_sigelt : sigelt'  ->  sigelt = (fun e -> {sigel = e; sigrng = FStar_Range.dummyRange})
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___93_3956 -> (match (uu___93_3956) with
| ({lbname = FStar_Util.Inr (uu____3958); lbunivs = uu____3959; lbtyp = uu____3960; lbeff = uu____3961; lbdef = uu____3962})::uu____3963 -> begin
true
end
| uu____3970 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____3978 out -> (match (uu____3978) with
| (x, uu____3985) -> begin
...
let uu____4004 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____4004 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___94_4009 -> (match (uu___94_4009) with
| Some (Implicit (uu____4010)) -> begin
true
end
| uu____4011 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___95_4014 -> (match (uu___95_4014) with
| true -> begin
Some (imp_tag)
end
| uu____4015 -> begin
...
| Pat_cons (uu____4039, pats) -> begin
(FStar_List.fold_left (fun b1 uu____4057 -> (match (uu____4057) with
| (p2, uu____4065) -> begin
(aux b1 p2)
end)) b pats)
end
| Pat_disj ((p2)::uu____4071) -> begin
(aux b p2)
end
| Pat_disj ([]) -> begin
(failwith "impossible")
...
let uu____4082 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____4082))))
...
let gen1 = (fun uu____4098 -> ((FStar_Util.incr x);
(FStar_ST.read x);
))
in (

let reset = (fun uu____4108 -> (FStar_ST.write x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range Prims.option  ->  FStar_Range.range = (fun uu___96_4130 -> (match (uu___96_4130) with
| None -> begin
...
let uu____4145 = (next_id ())
in {ppname = id; index = uu____4145; sort = t})))
...
let uu____4160 = (range_of_bv bv)
in Some (uu____4160))
in (new_bv uu____4158 bv.sort))
end
| uu____4161 -> begin
(
...
let uu____4163 = (next_id ())
in {ppname = uu___99_4162.ppname; index = uu____4163; sort = uu___99_4162.sort}))
end)))
...
let uu____4174 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____4174))
in ((uu____4173), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____4170))))
...
let uu___100_4255 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___100_4255.index; sort = uu___100_4255.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (

let uu____4267 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = uu____4267; fv_delta = dd; fv_qual = dq}))
...
let uu____4302 = (lid_as_fv l dd dq)
in (fv_to_tm uu____4302)))
...
let uu____4313 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____4313)))


let set_range_of_fv : fv  ->  FStar_Range.range  ->  fv = (fun fv r -> (

let uu___101_4320 = fv
in (

let uu____4321 = (

let uu___102_4325 = fv.fv_name
in (

let uu____4330 = (

let uu____4331 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____4331 r))
in {v = uu____4330; ty = uu___102_4325.ty; p = uu___102_4325.p}))
in {fv_name = uu____4321; fv_delta = uu___101_4320.fv_delta; fv_qual = uu___101_4320.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___97_4355 -> (match (uu___97_4355) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____4359)); tk = uu____4360; pos = uu____4361; vars = uu____4362} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____4367 -> begin
false
