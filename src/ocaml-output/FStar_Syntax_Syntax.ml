...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___90_3288 -> (match (uu___90_3288) with
| Reflectable (uu____3289) -> begin
true
end
| uu____3290 -> begin
false
...
let uu___97_3366 = x
in {ppname = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r))); index = uu___97_3366.index; sort = uu___97_3366.sort}))


let syn = (fun p k f -> (f k p))


let mk_fvs = (fun uu____3407 -> (FStar_Util.mk_ref None))


let mk_uvs = (fun uu____3419 -> (FStar_Util.mk_ref None))


let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____3425 -> (FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText)))))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____3431 -> (FStar_Util.new_set order_fv (fun x -> (FStar_Util.hashcode x.FStar_Ident.str))))


let new_uv_set : Prims.unit  ->  uvars = (fun uu____3436 -> (FStar_Util.new_set (fun uu____3445 uu____3446 -> (match (((uu____3445), (uu____3446))) with
| ((x, uu____3480), (y, uu____3482)) -> begin
(

let uu____3523 = (FStar_Unionfind.uvar_id x)
in (

let uu____3527 = (FStar_Unionfind.uvar_id y)
in (uu____3523 - uu____3527)))
end)) (fun uu____3531 -> (match (uu____3531) with
| (x, uu____3541) -> begin
(FStar_Unionfind.uvar_id x)
end))))


let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun uu____3560 -> (FStar_Util.new_set (fun x y -> (

let uu____3570 = (FStar_Unionfind.uvar_id x)
in (

let uu____3572 = (FStar_Unionfind.uvar_id y)
in (uu____3570 - uu____3572)))) (fun x -> (FStar_Unionfind.uvar_id x))))


let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____3583 -> (FStar_Util.new_fifo_set (fun x y -> (FStar_String.compare (FStar_Ident.text_of_id x) (FStar_Ident.text_of_id y))) (fun x -> (FStar_Util.hashcode (FStar_Ident.text_of_id x)))))
...
let uu____3645 = (FStar_Util.mk_ref topt)
in (

let uu____3649 = (FStar_Util.mk_ref None)
in {n = t; tk = uu____3645; pos = r; vars = uu____3649})))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____3660 = (range_of_bv bv)
in ((mk (Tm_bvar (bv))) (Some (bv.sort.n)) uu____3660)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____3672 = (range_of_bv bv)
in ((mk (Tm_name (bv))) (Some (bv.sort.n)) uu____3672)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___91_3713 -> (match (uu___91_3713) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____3715) -> begin
...
let uu____3830 = (FStar_Util.mk_ref None)
in ((lr), (uu____3830)))
in Tm_delayed (uu____3809))
in (mk uu____3808))
in (uu____3805 None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____3920 -> (match (uu____3920) with
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___92_4008 -> (match (uu___92_4008) with
| ({lbname = FStar_Util.Inr (uu____4010); lbunivs = uu____4011; lbtyp = uu____4012; lbeff = uu____4013; lbdef = uu____4014})::uu____4015 -> begin
true
end
| uu____4022 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____4030 out -> (match (uu____4030) with
| (x, uu____4037) -> begin
...
let uu____4056 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____4056 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___93_4061 -> (match (uu___93_4061) with
| Some (Implicit (uu____4062)) -> begin
true
end
| uu____4063 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___94_4066 -> (match (uu___94_4066) with
| true -> begin
Some (imp_tag)
end
| uu____4067 -> begin
...
| Pat_cons (uu____4091, pats) -> begin
(FStar_List.fold_left (fun b1 uu____4109 -> (match (uu____4109) with
| (p2, uu____4117) -> begin
(aux b1 p2)
end)) b pats)
end
| Pat_disj ((p2)::uu____4123) -> begin
(aux b p2)
end
| Pat_disj ([]) -> begin
(failwith "impossible")
...
let uu____4134 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____4134))))
...
let gen1 = (fun uu____4150 -> ((FStar_Util.incr x);
(FStar_ST.read x);
))
in (

let reset = (fun uu____4160 -> (FStar_ST.write x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range Prims.option  ->  FStar_Range.range = (fun uu___95_4182 -> (match (uu___95_4182) with
| None -> begin
...
let uu____4197 = (next_id ())
in {ppname = id; index = uu____4197; sort = t})))
...
let uu____4212 = (range_of_bv bv)
in Some (uu____4212))
in (new_bv uu____4210 bv.sort))
end
| uu____4213 -> begin
(
...
let uu____4215 = (next_id ())
in {ppname = uu___98_4214.ppname; index = uu____4215; sort = uu___98_4214.sort}))
end)))
...
let uu____4226 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____4226))
in ((uu____4225), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____4222))))
...
let uu___99_4307 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___99_4307.index; sort = uu___99_4307.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (

let uu____4319 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = uu____4319; fv_delta = dd; fv_qual = dq}))
...
let uu____4354 = (lid_as_fv l dd dq)
in (fv_to_tm uu____4354)))
...
let uu____4365 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____4365)))


let set_range_of_fv : fv  ->  FStar_Range.range  ->  fv = (fun fv r -> (

let uu___100_4372 = fv
in (

let uu____4373 = (

let uu___101_4377 = fv.fv_name
in (

let uu____4382 = (

let uu____4383 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____4383 r))
in {v = uu____4382; ty = uu___101_4377.ty; p = uu___101_4377.p}))
in {fv_name = uu____4373; fv_delta = uu___100_4372.fv_delta; fv_qual = uu___100_4372.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___96_4407 -> (match (uu___96_4407) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____4411)); tk = uu____4412; pos = uu____4413; vars = uu____4414} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____4419 -> begin
false
