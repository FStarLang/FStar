...
let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____5494 -> (FStar_Util.new_set order_bv))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____5502 -> (FStar_Util.new_set order_fv))
...
let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____5518 -> (FStar_Util.new_fifo_set order_univ_name))
...
let mk : 'a . 'a  ->  'a mk_t_a = (fun t uu____5562 r -> (

let uu____5566 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in {n = t; pos = r; vars = uu____5566}))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____5589 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) FStar_Pervasives_Native.None uu____5589)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____5594 = (range_of_bv bv)
in (mk (Tm_name (bv)) FStar_Pervasives_Native.None uu____5594)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___89_5627 -> (match (uu___89_5627) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____5629) -> begin
...
in ((lr), (uu____5739)))
in Tm_delayed (uu____5714))
in (mk uu____5713))
in (uu____5710 FStar_Pervasives_Native.None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____5834 -> (match (uu____5834) with
...
let uu____5911 = (null_bv t)
in ((uu____5911), (FStar_Pervasives_Native.None))))
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___90_5939 -> (match (uu___90_5939) with
| ({lbname = FStar_Util.Inr (uu____5942); lbunivs = uu____5943; lbtyp = uu____5944; lbeff = uu____5945; lbdef = uu____5946})::uu____5947 -> begin
true
end
| uu____5956 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____5973 out -> (match (uu____5973) with
| (x, uu____5984) -> begin
...
let uu____6017 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____6017 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___91_6025 -> (match (uu___91_6025) with
| FStar_Pervasives_Native.Some (Implicit (uu____6026)) -> begin
true
end
| uu____6027 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___92_6031 -> (match (uu___92_6031) with
| true -> begin
...
| Pat_dot_term (uu____6060) -> begin
b
end
| Pat_constant (uu____6067) -> begin
b
end
| Pat_wild (x) -> begin
(x)::b
...
| Pat_cons (uu____6070, pats) -> begin
(FStar_List.fold_left (fun b1 uu____6101 -> (match (uu____6101) with
| (p2, uu____6113) -> begin
(aux b1 p2)
end)) b pats)
end))
...
let uu____6118 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____6118))))
...
let gen1 = (fun uu____6139 -> ((FStar_Util.incr x);
(FStar_ST.op_Bang x);
))
in (

let reset = (fun uu____6189 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range FStar_Pervasives_Native.option  ->  FStar_Range.range = (fun uu___93_6237 -> (match (uu___93_6237) with
...
let uu____6266 = (next_id ())
in {ppname = id; index = uu____6266; sort = t})))
...
let uu____6287 = (range_of_bv bv)
in FStar_Pervasives_Native.Some (uu____6287))
in (new_bv uu____6284 bv.sort))
end
| uu____6288 -> begin
(
...
let uu____6290 = (next_id ())
in {ppname = uu___96_6289.ppname; index = uu____6290; sort = uu___96_6289.sort}))
end)))
...
let uu____6306 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____6306))
in ((uu____6305), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____6300))))
...
let uu___97_6412 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___97_6412.index; sort = uu___97_6412.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual FStar_Pervasives_Native.option  ->  fv = (fun l dd dq -> (

let uu____6429 = (withinfo l (FStar_Ident.range_of_lid l))
in {fv_name = uu____6429; fv_delta = dd; fv_qual = dq}))
...
let uu____6450 = (lid_as_fv l dd dq)
in (fv_to_tm uu____6450)))
...
let uu____6459 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____6459)))
...
let uu___99_6470 = fv.fv_name
in (

let uu____6471 = (

let uu____6472 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____6472 r))
in {v = uu____6471; p = uu___99_6470.p}))
in {fv_name = uu____6469; fv_delta = uu___98_6468.fv_delta; fv_qual = uu___98_6468.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___94_6492 -> (match (uu___94_6492) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____6496)); pos = uu____6497; vars = uu____6498} when (Prims.op_Equality data s) -> begin
true
end
| uu____6501 -> begin
false
...
let uu____6512 = (lid_as_fv l Delta_constant FStar_Pervasives_Native.None)
in Tm_fvar (uu____6512))
in (mk uu____6511))
in (uu____6508 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____6524 = (lid_as_fv l (Delta_defined_at_level ((Prims.parse_int "1"))) FStar_Pervasives_Native.None)
in Tm_fvar (uu____6524))
in (mk uu____6523))
in (uu____6520 FStar_Pervasives_Native.None FStar_Range.dummyRange)))


let tdataconstr : FStar_Ident.lident  ->  term = (fun l -> (

let uu____6532 = (lid_as_fv l Delta_constant (FStar_Pervasives_Native.Some (Data_ctor)))
in (fv_to_tm uu____6532)))
...
let uu____6537 = (tabbrev FStar_Parser_Const.tactic_lid)
in (mk_Tm_uinst uu____6537 ((U_zero)::[])))
in (

let uu____6538 = (

let uu____6539 = (as_arg t_unit)
in (uu____6539)::[])
in (mk_Tm_app uu____6536 uu____6538)))
in (uu____6535 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____6546 = (tabbrev FStar_Parser_Const.u_tac_lid)
in (mk_Tm_uinst uu____6546 ((U_zero)::[])))
in (

let uu____6547 = (

let uu____6548 = (as_arg t_unit)
in (uu____6548)::[])
in (mk_Tm_app uu____6545 uu____6547)))
in (uu____6544 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____6557 = (tabbrev FStar_Parser_Const.list_lid)
in (mk_Tm_uinst uu____6557 ((U_zero)::[])))
in (

let uu____6558 = (

let uu____6559 = (as_arg t)
in (uu____6559)::[])
in (mk_Tm_app uu____6556 uu____6558)))
in (uu____6555 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____6568 = (tabbrev FStar_Parser_Const.option_lid)
in (mk_Tm_uinst uu____6568 ((U_zero)::[])))
in (

let uu____6569 = (

let uu____6570 = (as_arg t)
in (uu____6570)::[])
in (mk_Tm_app uu____6567 uu____6569)))
in (uu____6566 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
