...
let (uu___is_Sig_inductive_typ : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_inductive_typ _0 -> true | uu____7309 -> false
...
let (uu___is_Sig_bundle : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_bundle _0 -> true | uu____7384 -> false
...
let (uu___is_Sig_datacon : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_datacon _0 -> true | uu____7442 -> false
...
let (uu___is_Sig_declare_typ : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_declare_typ _0 -> true | uu____7512 -> false
...
let (uu___is_Sig_let : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_let _0 -> true | uu____7555 -> false
...
let (uu___is_Sig_assume : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_assume _0 -> true | uu____7598 -> false
...
let (uu___is_Sig_new_effect : sigelt' -> Prims.bool) =
  fun projectee  ->
    match projectee with | Sig_new_effect _0 -> true | uu____7635 -> false
...
let (order_bv : bv -> bv -> Prims.int) =
  fun x  ->
    fun y  ->
      let i =
        let uu____8267 = FStar_Ident.text_of_id x.ppname  in
        let uu____8269 = FStar_Ident.text_of_id y.ppname  in
        FStar_String.compare uu____8267 uu____8269  in
      if i = Prims.int_zero then x.index - y.index else i
  
let (order_ident : FStar_Ident.ident -> FStar_Ident.ident -> Prims.int) =
  fun x  ->
    fun y  ->
      let uu____8289 = FStar_Ident.text_of_id x  in
      let uu____8291 = FStar_Ident.text_of_id y  in
      FStar_String.compare uu____8289 uu____8291
  
let (order_fv : FStar_Ident.lident -> FStar_Ident.lident -> Prims.int) =
  fun x  ->
    fun y  ->
      let uu____8305 = FStar_Ident.string_of_lid x  in
      let uu____8307 = FStar_Ident.string_of_lid y  in
      FStar_String.compare uu____8305 uu____8307
...
let (set_range_of_bv : bv -> FStar_Range.range -> bv) =
  fun x  ->
    fun r  ->
      let uu___414_8334 = x  in
      let uu____8335 = FStar_Ident.set_id_range r x.ppname  in
      {
        ppname = uu____8335;
        index = (uu___414_8334.index);
        sort = (uu___414_8334.sort)
      }
      ...
      let aq =
        FStar_List.map
          (fun uu____8371  ->
             ...
             | (bv1,t) -> let uu____8382 = f t  in (bv1, uu____8382))
          qi.antiquotes
         in
      let uu___422_8383 = qi  in
      { qkind = (uu___422_8383.qkind); antiquotes = aq }
  ...
  fun bv1  ->
    fun aq  ->
      let uu____8399 =
        FStar_List.tryFind
          (fun uu____8417  ->
             match uu____8417 with | (bv',uu____8426) -> bv_eq bv1 bv') aq
         in
      match uu____8399 with
      | FStar_Pervasives_Native.Some (uu____8433,e) ->
          FStar_Pervasives_Native.Some e
      | FStar_Pervasives_Native.None  -> FStar_Pervasives_Native.None
  
let syn :
  'uuuuuu8464 'uuuuuu8465 'uuuuuu8466 .
    'uuuuuu8464 ->
      'uuuuuu8465 ->
        ('uuuuuu8465 -> 'uuuuuu8464 -> 'uuuuuu8466) -> 'uuuuuu8466
  = fun p  -> fun k  -> fun f  -> f k p 
let mk_fvs :
  'uuuuuu8497 .
    unit -> 'uuuuuu8497 FStar_Pervasives_Native.option FStar_ST.ref
  = fun uu____8506  -> FStar_Util.mk_ref FStar_Pervasives_Native.None 
let mk_uvs :
  'uuuuuu8514 .
    unit -> 'uuuuuu8514 FStar_Pervasives_Native.option FStar_ST.ref
  = fun uu____8523  -> FStar_Util.mk_ref FStar_Pervasives_Native.None 
let (new_bv_set : unit -> bv FStar_Util.set) =
  fun uu____8533  -> FStar_Util.new_set order_bv 
let (new_id_set : unit -> FStar_Ident.ident FStar_Util.set) =
  fun uu____8543  -> FStar_Util.new_set order_ident 
let (new_fv_set : unit -> FStar_Ident.lident FStar_Util.set) =
  fun uu____8553  -> FStar_Util.new_set order_fv 
let (order_univ_name : univ_name -> univ_name -> Prims.int) =
  fun x  ->
    fun y  ->
      let uu____8568 = FStar_Ident.text_of_id x  in
      let uu____8570 = FStar_Ident.text_of_id y  in
      FStar_String.compare uu____8568 uu____8570
  
let (new_universe_names_set : unit -> univ_name FStar_Util.set) =
  fun uu____8579  -> FStar_Util.new_set order_univ_name 
let (eq_binding : binding -> binding -> Prims.bool) =
  fun b1  ->
    fun b2  ->
      match (b1, b2) with
      | (Binding_var bv1,Binding_var bv2) -> bv_eq bv1 bv2
      | (Binding_lid (lid1,uu____8598),Binding_lid (lid2,uu____8600)) ->
...
let mk : 'a . 'a -> 'a mk_t_a =
  fun t  ->
    fun uu____8689  ->
      fun r  ->
        let uu____8693 = FStar_Util.mk_ref FStar_Pervasives_Native.None  in
        { n = t; pos = r; vars = uu____8693 }
  
let (bv_to_tm : bv -> term) =
  fun bv1  ->
    let uu____8704 = range_of_bv bv1  in
    mk (Tm_bvar bv1) FStar_Pervasives_Native.None uu____8704
  
let (bv_to_name : bv -> term) =
  fun bv1  ->
    let uu____8711 = range_of_bv bv1  in
    mk (Tm_name bv1) FStar_Pervasives_Native.None uu____8711
...
let (binders_to_names : binders -> term Prims.list) =
  fun bs  ->
    FStar_All.pipe_right bs
      (FStar_List.map
         (fun uu____8741  ->
            match uu____8741 with | (x,uu____8749) -> bv_to_name x))
...
let (mk_Tm_uinst : term -> universes -> term) =
  fun t  ->
    fun us  ->
      ...
      | uu____8809 -> failwith "Unexpected universe instantiation"
    ...
    fun args'  ->
      fun kopt  ->
        fun r  ->
          ...
          | uu____8868 -> mk_Tm_app t args' kopt r
...
let (is_type : term -> Prims.bool) =
  fun t  -> match t.n with | Tm_type uu____9113 -> true | uu____9115 -> false 
...
let (null_binder : term -> binder) =
  fun t  ->
    let uu____9141 = null_bv t  in (uu____9141, FStar_Pervasives_Native.None)
...
let (is_null_bv : bv -> Prims.bool) =
  fun b  ->
    let uu____9173 = FStar_Ident.text_of_id b.ppname  in
    let uu____9175 = FStar_Ident.text_of_id null_id  in
...
let (is_top_level : letbinding Prims.list -> Prims.bool) =
  fun uu___1_9195  ->
    match uu___1_9195 with
    | { lbname = FStar_Util.Inr uu____9199; lbunivs = uu____9200;
        lbtyp = uu____9201; lbeff = uu____9202; lbdef = uu____9203;
        lbattrs = uu____9204; lbpos = uu____9205;_}::uu____9206 -> true
    | uu____9220 -> false
...
let (freenames_of_binders : binders -> freenames) =
  fun bs  ->
    FStar_List.fold_right
      (fun uu____9242  ->
         fun out  ->
           match uu____9242 with | (x,uu____9255) -> FStar_Util.set_add x out)
      bs no_names
...
let (binders_of_freenames : freenames -> binders) =
  fun fvs  ->
    let uu____9288 = FStar_Util.set_elements fvs  in
    FStar_All.pipe_right uu____9288 binders_of_list
  
let (is_implicit : aqual -> Prims.bool) =
  fun uu___2_9299  ->
    match uu___2_9299 with
    | FStar_Pervasives_Native.Some (Implicit uu____9301) -> true
    | uu____9304 -> false
  
let (is_implicit_or_meta : aqual -> Prims.bool) =
  fun uu___3_9312  ->
    match uu___3_9312 with
    | FStar_Pervasives_Native.Some (Implicit uu____9314) -> true
    | FStar_Pervasives_Native.Some (Meta uu____9317) -> true
    | uu____9321 -> false
    ...
    let rec aux b p1 =
      ...
      | Pat_cons (uu____9377,pats) ->
          FStar_List.fold_left
            (fun b1  ->
               fun uu____9411  ->
                 match uu____9411 with | (p2,uu____9424) -> aux b1 p2) b pats
       in
...
let (freshen_bv : bv -> bv) =
  fun bv1  ->
    ...
    then
      ...
      (let uu___612_9517 = bv1  in
       let uu____9518 = FStar_Ident.next_id ()  in
       {
         ppname = (uu___612_9517.ppname);
         index = uu____9518;
         sort = (uu___612_9517.sort)
       })
...
let (set_bv_range : bv -> FStar_Range.range -> bv) =
  fun bv1  ->
    fun r  ->
      let uu___639_9665 = bv1  in
      let uu____9666 = FStar_Ident.set_id_range r bv1.ppname  in
      {
        ppname = uu____9666;
        index = (uu___639_9665.index);
        sort = (uu___639_9665.sort)
      }
...
let (fv_to_tm : fv -> term) =
  fun fv1  ->
    ...
    mk (Tm_fvar fv1) FStar_Pervasives_Native.None uu____9695
...
let (set_range_of_fv : fv -> FStar_Range.range -> fv) =
  fun fv1  ->
    fun r  ->
      let uu___652_9741 = fv1  in
      let uu____9742 =
        let uu___654_9743 = fv1.fv_name  in
        let uu____9744 =
          let uu____9745 = lid_of_fv fv1  in
          FStar_Ident.set_lid_range uu____9745 r  in
        { v = uu____9744; p = (uu___654_9743.p) }  in
      {
        fv_name = uu____9742;
        fv_delta = (uu___652_9741.fv_delta);
        fv_qual = (uu___652_9741.fv_qual)
      }
...
let (tconst : FStar_Ident.lident -> term) =
  fun l  ->
    let uu____9992 =
      let uu____9999 = let uu____10000 = fvconst l  in Tm_fvar uu____10000
         in
      mk uu____9999  in
    uu____9992 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let (t_tac_of : term -> term -> term) =
  fun a  ->
    fun b  ->
      let uu____10053 =
        let uu____10058 =
          let uu____10059 = tabbrev FStar_Parser_Const.tac_lid  in
          mk_Tm_uinst uu____10059 [U_zero; U_zero]  in
        let uu____10060 =
          let uu____10061 = as_arg a  in
          let uu____10070 = let uu____10081 = as_arg b  in [uu____10081]  in
          uu____10061 :: uu____10070  in
        mk_Tm_app uu____10058 uu____10060  in
...
let (t_tactic_of : term -> term) =
  fun t  ->
    let uu____10120 =
      let uu____10125 =
        let uu____10126 = tabbrev FStar_Parser_Const.tactic_lid  in
        mk_Tm_uinst uu____10126 [U_zero]  in
      let uu____10127 = let uu____10128 = as_arg t  in [uu____10128]  in
      mk_Tm_app uu____10125 uu____10127  in
    uu____10120 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let (t_list_of : term -> term) =
  fun t  ->
    let uu____10160 =
      let uu____10165 =
        let uu____10166 = tabbrev FStar_Parser_Const.list_lid  in
        mk_Tm_uinst uu____10166 [U_zero]  in
      let uu____10167 = let uu____10168 = as_arg t  in [uu____10168]  in
      mk_Tm_app uu____10165 uu____10167  in
    uu____10160 FStar_Pervasives_Native.None FStar_Range.dummyRange
  
let (t_option_of : term -> term) =
  fun t  ->
    let uu____10199 =
      let uu____10204 =
        let uu____10205 = tabbrev FStar_Parser_Const.option_lid  in
        mk_Tm_uinst uu____10205 [U_zero]  in
      let uu____10206 = let uu____10207 = as_arg t  in [uu____10207]  in
      mk_Tm_app uu____10204 uu____10206  in
    uu____10199 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let (t_tuple2_of : term -> term -> term) =
  fun t1  ->
    fun t2  ->
      let uu____10243 =
        let uu____10248 =
          let uu____10249 = tabbrev FStar_Parser_Const.lid_tuple2  in
          mk_Tm_uinst uu____10249 [U_zero; U_zero]  in
        let uu____10250 =
          let uu____10251 = as_arg t1  in
          let uu____10260 = let uu____10271 = as_arg t2  in [uu____10271]  in
          uu____10251 :: uu____10260  in
        mk_Tm_app uu____10248 uu____10250  in
      uu____10243 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let (t_either_of : term -> term -> term) =
  fun t1  ->
    fun t2  ->
      let uu____10315 =
        let uu____10320 =
          let uu____10321 = tabbrev FStar_Parser_Const.either_lid  in
          mk_Tm_uinst uu____10321 [U_zero; U_zero]  in
        let uu____10322 =
          let uu____10323 = as_arg t1  in
          let uu____10332 = let uu____10343 = as_arg t2  in [uu____10343]  in
          uu____10323 :: uu____10332  in
        mk_Tm_app uu____10320 uu____10322  in
      uu____10315 FStar_Pervasives_Native.None FStar_Range.dummyRange
