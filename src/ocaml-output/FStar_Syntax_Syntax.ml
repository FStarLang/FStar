...
let syn : 'Auu____7026 'Auu____7027 'Auu____7028 . 'Auu____7026  ->  'Auu____7027  ->  ('Auu____7027  ->  'Auu____7026  ->  'Auu____7028)  ->  'Auu____7028 = (fun p k f -> (f k p))


let mk_fvs : 'Auu____7069 . unit  ->  'Auu____7069 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7078 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let mk_uvs : 'Auu____7096 . unit  ->  'Auu____7096 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7105 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let new_bv_set : unit  ->  bv FStar_Util.set = (fun uu____7114 -> (FStar_Util.new_set order_bv))


let new_fv_set : unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____7123 -> (FStar_Util.new_set order_fv))


let order_univ_name : univ_name  ->  univ_name  ->  Prims.int = (fun x y -> (

let uu____7136 = (FStar_Ident.text_of_id x)
in (

let uu____7137 = (FStar_Ident.text_of_id y)
in (FStar_String.compare uu____7136 uu____7137))))


let new_universe_names_set : unit  ->  univ_name FStar_Util.set = (fun uu____7144 -> (FStar_Util.new_set order_univ_name))
...
let mk : 'a . 'a  ->  'a mk_t_a = (fun t uu____7197 r -> (

let uu____7201 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in {n = t; pos = r; vars = uu____7201}))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____7233 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) FStar_Pervasives_Native.None uu____7233)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____7239 = (range_of_bv bv)
in (mk (Tm_name (bv)) FStar_Pervasives_Native.None uu____7239)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___33_7290 -> (match (uu___33_7290) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____7292) -> begin
...
in ((lr), (uu____7438)))
in Tm_delayed (uu____7413))
in (mk uu____7412))
in (uu____7405 FStar_Pervasives_Native.None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term * FStar_Range.range)  ->  letbinding = (fun uu____7563 -> (match (uu____7563) with
...
let uu____7653 = (null_bv t)
in ((uu____7653), (FStar_Pervasives_Native.None))))
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___34_7684 -> (match (uu___34_7684) with
| ({lbname = FStar_Util.Inr (uu____7687); lbunivs = uu____7688; lbtyp = uu____7689; lbeff = uu____7690; lbdef = uu____7691; lbattrs = uu____7692; lbpos = uu____7693})::uu____7694 -> begin
true
end
| uu____7707 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____7725 out -> (match (uu____7725) with
| (x, uu____7736) -> begin
...
let uu____7763 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____7763 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___35_7772 -> (match (uu___35_7772) with
| FStar_Pervasives_Native.Some (Implicit (uu____7773)) -> begin
true
end
| uu____7774 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___36_7779 -> (match (uu___36_7779) with
| true -> begin
...
| Pat_dot_term (uu____7813) -> begin
b
end
| Pat_constant (uu____7820) -> begin
b
end
| Pat_wild (x) -> begin
(x)::b
...
| Pat_cons (uu____7823, pats) -> begin
(FStar_List.fold_left (fun b1 uu____7854 -> (match (uu____7854) with
| (p2, uu____7866) -> begin
(aux b1 p2)
end)) b pats)
end))
...
let uu____7871 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____7871))))
...
let gen1 = (fun uu____7896 -> ((FStar_Util.incr x);
(FStar_ST.op_Bang x);
))
in (

let reset = (fun uu____7981 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range FStar_Pervasives_Native.option  ->  FStar_Range.range = (fun uu___37_8059 -> (match (uu___37_8059) with
| FStar_Pervasives_Native.None -> begin
...
let uu____8094 = (next_id ())
in {ppname = id1; index = uu____8094; sort = t})))
...
let uu____8118 = (range_of_bv bv)
in FStar_Pervasives_Native.Some (uu____8118))
in (new_bv uu____8115 bv.sort))
end
| uu____8119 -> begin
(
...
let uu____8121 = (next_id ())
in {ppname = uu___41_8120.ppname; index = uu____8121; sort = uu___41_8120.sort}))
end)))
...
let uu____8138 = (FStar_Util.string_of_int id1)
in (Prims.strcat FStar_Ident.reserved_prefix uu____8138))
in ((uu____8137), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____8132))))
...
let uu____8256 = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r)))
in {ppname = uu____8256; index = uu___42_8255.index; sort = uu___42_8255.sort})))
...
let uu____8277 = (FStar_Ident.range_of_lid l)
in (withinfo l uu____8277))
in {fv_name = uu____8276; fv_delta = dd; fv_qual = dq}))


let fv_to_tm : fv  ->  term = (fun fv -> (

let uu____8283 = (FStar_Ident.range_of_lid fv.fv_name.v)
in (mk (Tm_fvar (fv)) FStar_Pervasives_Native.None uu____8283)))


let fvar : FStar_Ident.lident  ->  delta_depth  ->  fv_qual FStar_Pervasives_Native.option  ->  term = (fun l dd dq -> (

let uu____8303 = (lid_as_fv l dd dq)
in (fv_to_tm uu____8303)))
...
let uu____8314 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____8314)))
...
let uu___44_8327 = fv.fv_name
in (

let uu____8328 = (

let uu____8329 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____8329 r))
in {v = uu____8328; p = uu___44_8327.p}))
in {fv_name = uu____8326; fv_delta = uu___43_8325.fv_delta; fv_qual = uu___43_8325.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___38_8351 -> (match (uu___38_8351) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____8355)); pos = uu____8356; vars = uu____8357} when (Prims.op_Equality data s) -> begin
true
end
| uu____8360 -> begin
false
...
let uu____8411 = (fv_eq fv1 fv2)
in (match (uu____8411) with
| true -> begin
(

let uu____8413 = (FStar_List.zip as1 as2)
in (FStar_All.pipe_right uu____8413 (FStar_List.for_all (fun uu____8479 -> (match (uu____8479) with
| ((p11, b1), (p21, b2)) -> begin
...
| (Pat_var (uu____8505), Pat_var (uu____8506)) -> begin
true
end
| (Pat_wild (uu____8507), Pat_wild (uu____8508)) -> begin
true
end
| (Pat_dot_term (bv1, t1), Pat_dot_term (bv2, t2)) -> begin
true
end
| (uu____8521, uu____8522) -> begin
false
end))
...
let uu____8536 = (lid_as_fv l Delta_constant FStar_Pervasives_Native.None)
in Tm_fvar (uu____8536))
in (mk uu____8535))
in (uu____8528 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____8553 = (lid_as_fv l (Delta_defined_at_level ((Prims.parse_int "1"))) FStar_Pervasives_Native.None)
in Tm_fvar (uu____8553))
in (mk uu____8552))
in (uu____8545 FStar_Pervasives_Native.None FStar_Range.dummyRange)))


let tdataconstr : FStar_Ident.lident  ->  term = (fun l -> (

let uu____8562 = (lid_as_fv l Delta_constant (FStar_Pervasives_Native.Some (Data_ctor)))
in (fv_to_tm uu____8562)))
...
let uu____8571 = (tabbrev FStar_Parser_Const.tactic_lid)
in (mk_Tm_uinst uu____8571 ((U_zero)::[])))
in (

let uu____8572 = (

let uu____8573 = (as_arg t_unit)
in (uu____8573)::[])
in (mk_Tm_app uu____8570 uu____8572)))
in (uu____8565 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____8602 = (tabbrev FStar_Parser_Const.u_tac_lid)
in (mk_Tm_uinst uu____8602 ((U_zero)::[])))
in (

let uu____8603 = (

let uu____8604 = (as_arg t_unit)
in (uu____8604)::[])
in (mk_Tm_app uu____8601 uu____8603)))
in (uu____8596 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____8636 = (tabbrev FStar_Parser_Const.list_lid)
in (mk_Tm_uinst uu____8636 ((U_zero)::[])))
in (

let uu____8637 = (

let uu____8638 = (as_arg t)
in (uu____8638)::[])
in (mk_Tm_app uu____8635 uu____8637)))
in (uu____8630 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____8670 = (tabbrev FStar_Parser_Const.option_lid)
in (mk_Tm_uinst uu____8670 ((U_zero)::[])))
in (

let uu____8671 = (

let uu____8672 = (as_arg t)
in (uu____8672)::[])
in (mk_Tm_app uu____8669 uu____8671)))
in (uu____8664 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____8709 = (tabbrev FStar_Parser_Const.lid_tuple2)
in (mk_Tm_uinst uu____8709 ((U_zero)::[])))
in (

let uu____8710 = (

let uu____8711 = (as_arg t1)
in (

let uu____8718 = (

let uu____8727 = (as_arg t2)
in (uu____8727)::[])
in (uu____8711)::uu____8718))
in (mk_Tm_app uu____8708 uu____8710)))
in (uu____8703 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
