...
let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun uu____3532 -> (FStar_Util.new_set (fun x y -> (let _0_26 = (FStar_Unionfind.uvar_id x)
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___91_3939 -> (match (uu___91_3939) with
| ({lbname = FStar_Util.Inr (uu____3941); lbunivs = uu____3942; lbtyp = uu____3943; lbeff = uu____3944; lbdef = uu____3945})::uu____3946 -> begin
true
end
| uu____3953 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____3961 out -> (match (uu____3961) with
| (x, uu____3968) -> begin
...
let is_implicit : aqual  ->  Prims.bool = (fun uu___92_3990 -> (match (uu___92_3990) with
| Some (Implicit (uu____3991)) -> begin
true
end
| uu____3992 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___93_3995 -> (match (uu___93_3995) with
| true -> begin
Some (imp_tag)
end
| uu____3996 -> begin
...
| Pat_cons (uu____4020, pats) -> begin
(FStar_List.fold_left (fun b uu____4038 -> (match (uu____4038) with
| (p, uu____4046) -> begin
(aux b p)
end)) b pats)
end
| Pat_disj ((p)::uu____4052) -> begin
(aux b p)
end
| Pat_disj ([]) -> begin
(failwith "impossible")
...
let gen = (fun uu____4077 -> ((FStar_Util.incr x);
(FStar_ST.read x);
))
in (

let reset = (fun uu____4087 -> (FStar_ST.write x (Prims.parse_int "0")))
in ((gen), (reset)))))
...
let range_of_ropt : FStar_Range.range Prims.option  ->  FStar_Range.range = (fun uu___94_4109 -> (match (uu___94_4109) with
| None -> begin
...
in {ppname = uu___97_4137.ppname; index = _0_37; sort = uu___97_4137.sort}))
end)))
...
let uu___98_4224 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___98_4224.index; sort = uu___98_4224.sort}))
...
let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___95_4284 -> (match (uu___95_4284) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____4288)); tk = uu____4289; pos = uu____4290; vars = uu____4291} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____4296 -> begin
false
