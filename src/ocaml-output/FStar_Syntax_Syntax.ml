...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___86_3217 -> (match (uu___86_3217) with
| Reflectable (uu____3218) -> begin
true
end
| uu____3219 -> begin
false
...
let uu___93_3295 = x
in {ppname = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r))); index = uu___93_3295.index; sort = uu___93_3295.sort}))


let syn = (fun p k f -> (f k p))


let mk_fvs = (fun uu____3336 -> (FStar_Util.mk_ref None))


let mk_uvs = (fun uu____3348 -> (FStar_Util.mk_ref None))


let new_bv_set : Prims.unit  ->  bv FStar_Util.set = (fun uu____3354 -> (FStar_Util.new_set order_bv (fun x -> (x.index + (FStar_Util.hashcode x.ppname.FStar_Ident.idText)))))


let new_fv_set : Prims.unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____3360 -> (FStar_Util.new_set order_fv (fun x -> (FStar_Util.hashcode x.FStar_Ident.str))))


let new_uv_set : Prims.unit  ->  uvars = (fun uu____3365 -> (FStar_Util.new_set (fun uu____3374 uu____3375 -> (match (((uu____3374), (uu____3375))) with
| ((x, uu____3409), (y, uu____3411)) -> begin
(

let uu____3452 = (FStar_Unionfind.uvar_id x)
in (

let uu____3456 = (FStar_Unionfind.uvar_id y)
in (uu____3452 - uu____3456)))
end)) (fun uu____3460 -> (match (uu____3460) with
| (x, uu____3470) -> begin
(FStar_Unionfind.uvar_id x)
end))))


let new_universe_uvar_set : Prims.unit  ->  universe_uvar FStar_Util.set = (fun uu____3489 -> (FStar_Util.new_set (fun x y -> (

let uu____3499 = (FStar_Unionfind.uvar_id x)
in (

let uu____3501 = (FStar_Unionfind.uvar_id y)
in (uu____3499 - uu____3501)))) (fun x -> (FStar_Unionfind.uvar_id x))))


let new_universe_names_fifo_set : Prims.unit  ->  univ_name FStar_Util.fifo_set = (fun uu____3512 -> (FStar_Util.new_fifo_set (fun x y -> (FStar_String.compare (FStar_Ident.text_of_id x) (FStar_Ident.text_of_id y))) (fun x -> (FStar_Util.hashcode (FStar_Ident.text_of_id x)))))
...
let uu____3574 = (FStar_Util.mk_ref topt)
in (

let uu____3578 = (FStar_Util.mk_ref None)
in {n = t; tk = uu____3574; pos = r; vars = uu____3578})))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____3589 = (range_of_bv bv)
in ((mk (Tm_bvar (bv))) (Some (bv.sort.n)) uu____3589)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____3601 = (range_of_bv bv)
in ((mk (Tm_name (bv))) (Some (bv.sort.n)) uu____3601)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___87_3642 -> (match (uu___87_3642) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____3644) -> begin
...
let uu____3759 = (FStar_Util.mk_ref None)
in ((lr), (uu____3759)))
in Tm_delayed (uu____3738))
in (mk uu____3737))
in (uu____3734 None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term)  ->  letbinding = (fun uu____3849 -> (match (uu____3849) with
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___88_3940 -> (match (uu___88_3940) with
| ({lbname = FStar_Util.Inr (uu____3942); lbunivs = uu____3943; lbtyp = uu____3944; lbeff = uu____3945; lbdef = uu____3946})::uu____3947 -> begin
true
end
| uu____3954 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____3962 out -> (match (uu____3962) with
| (x, uu____3969) -> begin
...
let uu____3988 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____3988 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___89_3993 -> (match (uu___89_3993) with
| Some (Implicit (uu____3994)) -> begin
true
end
| uu____3995 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___90_3998 -> (match (uu___90_3998) with
| true -> begin
Some (imp_tag)
end
| uu____3999 -> begin
...
| Pat_cons (uu____4023, pats) -> begin
(FStar_List.fold_left (fun b1 uu____4041 -> (match (uu____4041) with
| (p2, uu____4049) -> begin
(aux b1 p2)
end)) b pats)
end
| Pat_disj ((p2)::uu____4055) -> begin
(aux b p2)
end
| Pat_disj ([]) -> begin
(failwith "impossible")
...
let uu____4066 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____4066))))
...
let gen1 = (fun uu____4082 -> ((FStar_Util.incr x);
(FStar_ST.read x);
))
in (

let reset = (fun uu____4092 -> (FStar_ST.write x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range Prims.option  ->  FStar_Range.range = (fun uu___91_4114 -> (match (uu___91_4114) with
| None -> begin
...
let uu____4129 = (next_id ())
in {ppname = id; index = uu____4129; sort = t})))
...
let uu____4144 = (range_of_bv bv)
in Some (uu____4144))
in (new_bv uu____4142 bv.sort))
end
| uu____4145 -> begin
(
...
let uu____4147 = (next_id ())
in {ppname = uu___94_4146.ppname; index = uu____4147; sort = uu___94_4146.sort}))
end)))
...
let uu____4158 = (FStar_Util.string_of_int id)
in (Prims.strcat FStar_Ident.reserved_prefix uu____4158))
in ((uu____4157), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____4154))))
...
let uu___95_4239 = bv
in {ppname = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r))); index = uu___95_4239.index; sort = uu___95_4239.sort}))


let lid_as_fv : FStar_Ident.lident  ->  delta_depth  ->  fv_qual Prims.option  ->  fv = (fun l dd dq -> (

let uu____4251 = (withinfo l tun (FStar_Ident.range_of_lid l))
in {fv_name = uu____4251; fv_delta = dd; fv_qual = dq}))
...
let uu____4286 = (lid_as_fv l dd dq)
in (fv_to_tm uu____4286)))
...
let uu____4297 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____4297)))
...
let uu___97_4309 = fv.fv_name
in (

let uu____4314 = (

let uu____4315 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____4315 r))
in {v = uu____4314; ty = uu___97_4309.ty; p = uu___97_4309.p}))
in {fv_name = uu____4305; fv_delta = uu___96_4304.fv_delta; fv_qual = uu___96_4304.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___92_4339 -> (match (uu___92_4339) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____4343)); tk = uu____4344; pos = uu____4345; vars = uu____4346} when ((FStar_Util.string_of_unicode data) = s) -> begin
true
end
| uu____4351 -> begin
false
