...
let uu____4501 = (FStar_Util.mk_ref (FStar_Util.Inl (comp_thunk)))
in {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = uu____4501}))


let lcomp_comp : lcomp  ->  comp = (fun lc -> (

let uu____4604 = (FStar_ST.op_Bang lc.comp_thunk)
...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___35_6895 -> (match (uu___35_6895) with
| Reflectable (uu____6896) -> begin
true
end
| uu____6897 -> begin
false
...
let uu____6984 = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r)))
in {ppname = uu____6984; index = uu___42_6983.index; sort = uu___42_6983.sort})))


let on_antiquoted : (term  ->  term)  ->  quoteinfo  ->  quoteinfo = (fun f qi -> (

let aq = (FStar_List.map (fun uu____7026 -> (match (uu____7026) with
| (bv, b, t) -> begin
(
...
let uu___43_7043 = qi
in {qkind = uu___43_7043.qkind; antiquotes = aq})))


let lookup_aq : bv  ->  antiquotations  ->  (Prims.bool * term) FStar_Pervasives_Native.option = (fun bv aq -> (

let uu____7066 = (FStar_List.tryFind (fun uu____7089 -> (match (uu____7089) with
| (bv', uu____7099, uu____7100) -> begin
(bv_eq bv bv')
end)) aq)
in (match (uu____7066) with
| FStar_Pervasives_Native.Some (uu____7111, b, e) -> begin
...
let syn : 'Auu____7158 'Auu____7159 'Auu____7160 . 'Auu____7158  ->  'Auu____7159  ->  ('Auu____7159  ->  'Auu____7158  ->  'Auu____7160)  ->  'Auu____7160 = (fun p k f -> (f k p))


let mk_fvs : 'Auu____7225 . unit  ->  'Auu____7225 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7234 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let mk_uvs : 'Auu____7276 . unit  ->  'Auu____7276 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7285 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let new_bv_set : unit  ->  bv FStar_Util.set = (fun uu____7294 -> (FStar_Util.new_set order_bv))


let new_fv_set : unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____7303 -> (FStar_Util.new_set order_fv))


let order_univ_name : univ_name  ->  univ_name  ->  Prims.int = (fun x y -> (

let uu____7316 = (FStar_Ident.text_of_id x)
in (

let uu____7317 = (FStar_Ident.text_of_id y)
in (FStar_String.compare uu____7316 uu____7317))))


let new_universe_names_set : unit  ->  univ_name FStar_Util.set = (fun uu____7324 -> (FStar_Util.new_set order_univ_name))
...
let mk : 'a . 'a  ->  'a mk_t_a = (fun t uu____7377 r -> (

let uu____7381 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in {n = t; pos = r; vars = uu____7381}))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____7461 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) FStar_Pervasives_Native.None uu____7461)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____7467 = (range_of_bv bv)
in (mk (Tm_name (bv)) FStar_Pervasives_Native.None uu____7467)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___36_7510 -> (match (uu___36_7510) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____7512) -> begin
...
let uu____7644 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in ((lr), (uu____7644)))
in Tm_delayed (uu____7619))
in (mk uu____7618))
in (uu____7611 FStar_Pervasives_Native.None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term * FStar_Range.range)  ->  letbinding = (fun uu____7805 -> (match (uu____7805) with
...
let uu____7897 = (null_bv t)
in ((uu____7897), (FStar_Pervasives_Native.None))))
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___37_7930 -> (match (uu___37_7930) with
| ({lbname = FStar_Util.Inr (uu____7933); lbunivs = uu____7934; lbtyp = uu____7935; lbeff = uu____7936; lbdef = uu____7937; lbattrs = uu____7938; lbpos = uu____7939})::uu____7940 -> begin
true
end
| uu____7953 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____7971 out -> (match (uu____7971) with
| (x, uu____7982) -> begin
...
let uu____8017 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____8017 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___38_8026 -> (match (uu___38_8026) with
| FStar_Pervasives_Native.Some (Implicit (uu____8027)) -> begin
true
end
| uu____8028 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___39_8033 -> (match (uu___39_8033) with
| true -> begin
...
| Pat_dot_term (uu____8067) -> begin
b
end
| Pat_constant (uu____8074) -> begin
b
end
| Pat_wild (x) -> begin
(x)::b
...
| Pat_cons (uu____8077, pats) -> begin
(FStar_List.fold_left (fun b1 uu____8108 -> (match (uu____8108) with
| (p2, uu____8120) -> begin
(aux b1 p2)
end)) b pats)
end))
...
let uu____8125 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____8125))))
...
let gen1 = (fun uu____8150 -> ((FStar_Util.incr x);
(FStar_ST.op_Bang x);
))
in (

let reset = (fun uu____8361 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range FStar_Pervasives_Native.option  ->  FStar_Range.range = (fun uu___40_8493 -> (match (uu___40_8493) with
| FStar_Pervasives_Native.None -> begin
...
let uu____8528 = (next_id ())
in {ppname = id1; index = uu____8528; sort = t})))
...
let uu____8552 = (range_of_bv bv)
in FStar_Pervasives_Native.Some (uu____8552))
in (new_bv uu____8549 bv.sort))
end
| uu____8553 -> begin
(
...
let uu____8555 = (next_id ())
in {ppname = uu___44_8554.ppname; index = uu____8555; sort = uu___44_8554.sort}))
end)))
...
let uu____8572 = (FStar_Util.string_of_int id1)
in (Prims.strcat FStar_Ident.reserved_prefix uu____8572))
in ((uu____8571), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____8566))))
...
let uu____8690 = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r)))
in {ppname = uu____8690; index = uu___45_8689.index; sort = uu___45_8689.sort})))
...
let uu____8711 = (FStar_Ident.range_of_lid l)
in (withinfo l uu____8711))
in {fv_name = uu____8710; fv_delta = dd; fv_qual = dq}))


let fv_to_tm : fv  ->  term = (fun fv -> (

let uu____8717 = (FStar_Ident.range_of_lid fv.fv_name.v)
in (mk (Tm_fvar (fv)) FStar_Pervasives_Native.None uu____8717)))


let fvar : FStar_Ident.lident  ->  delta_depth  ->  fv_qual FStar_Pervasives_Native.option  ->  term = (fun l dd dq -> (

let uu____8737 = (lid_as_fv l dd dq)
in (fv_to_tm uu____8737)))
...
let uu____8748 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____8748)))
...
let uu___47_8761 = fv.fv_name
in (

let uu____8762 = (

let uu____8763 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____8763 r))
in {v = uu____8762; p = uu___47_8761.p}))
in {fv_name = uu____8760; fv_delta = uu___46_8759.fv_delta; fv_qual = uu___46_8759.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___41_8785 -> (match (uu___41_8785) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____8789)); pos = uu____8790; vars = uu____8791} when (Prims.op_Equality data s) -> begin
true
end
| uu____8794 -> begin
false
...
let uu____8845 = (fv_eq fv1 fv2)
in (match (uu____8845) with
| true -> begin
(

let uu____8847 = (FStar_List.zip as1 as2)
in (FStar_All.pipe_right uu____8847 (FStar_List.for_all (fun uu____8913 -> (match (uu____8913) with
| ((p11, b1), (p21, b2)) -> begin
...
| (Pat_var (uu____8939), Pat_var (uu____8940)) -> begin
true
end
| (Pat_wild (uu____8941), Pat_wild (uu____8942)) -> begin
true
end
| (Pat_dot_term (bv1, t1), Pat_dot_term (bv2, t2)) -> begin
true
end
| (uu____8955, uu____8956) -> begin
false
end))
...
let uu____8970 = (lid_as_fv l Delta_constant FStar_Pervasives_Native.None)
in Tm_fvar (uu____8970))
in (mk uu____8969))
in (uu____8962 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____8987 = (lid_as_fv l (Delta_defined_at_level ((Prims.parse_int "1"))) FStar_Pervasives_Native.None)
in Tm_fvar (uu____8987))
in (mk uu____8986))
in (uu____8979 FStar_Pervasives_Native.None FStar_Range.dummyRange)))


let tdataconstr : FStar_Ident.lident  ->  term = (fun l -> (

let uu____8996 = (lid_as_fv l Delta_constant (FStar_Pervasives_Native.Some (Data_ctor)))
in (fv_to_tm uu____8996)))
...
let uu____9005 = (tabbrev FStar_Parser_Const.tactic_lid)
in (mk_Tm_uinst uu____9005 ((U_zero)::[])))
in (

let uu____9006 = (

let uu____9007 = (as_arg t_unit)
in (uu____9007)::[])
in (mk_Tm_app uu____9004 uu____9006)))
in (uu____8999 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____9018 = (tabbrev FStar_Parser_Const.u_tac_lid)
in (mk_Tm_uinst uu____9018 ((U_zero)::[])))
in (

let uu____9019 = (

let uu____9020 = (as_arg t_unit)
in (uu____9020)::[])
in (mk_Tm_app uu____9017 uu____9019)))
in (uu____9012 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____9034 = (tabbrev FStar_Parser_Const.list_lid)
in (mk_Tm_uinst uu____9034 ((U_zero)::[])))
in (

let uu____9035 = (

let uu____9036 = (as_arg t)
in (uu____9036)::[])
in (mk_Tm_app uu____9033 uu____9035)))
in (uu____9028 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____9050 = (tabbrev FStar_Parser_Const.option_lid)
in (mk_Tm_uinst uu____9050 ((U_zero)::[])))
in (

let uu____9051 = (

let uu____9052 = (as_arg t)
in (uu____9052)::[])
in (mk_Tm_app uu____9049 uu____9051)))
in (uu____9044 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____9071 = (tabbrev FStar_Parser_Const.lid_tuple2)
in (mk_Tm_uinst uu____9071 ((U_zero)::[])))
in (

let uu____9072 = (

let uu____9073 = (as_arg t1)
in (

let uu____9074 = (

let uu____9077 = (as_arg t2)
in (uu____9077)::[])
in (uu____9073)::uu____9074))
in (mk_Tm_app uu____9070 uu____9072)))
in (uu____9065 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
