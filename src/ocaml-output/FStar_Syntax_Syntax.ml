...
{qkind : quote_kind; antiquotes : (bv * Prims.bool * term' syntax) Prims.list} 
 and comp_typ =
{comp_univs : universes; effect_name : FStar_Ident.lident; result_typ : term' syntax; effect_args : (term' syntax * arg_qualifier FStar_Pervasives_Native.option) Prims.list; flags : cflags Prims.list} 
 and comp' =
...
| {blob = __fname__blob; lkind = __fname__lkind; ltyp = __fname__ltyp; rng = __fname__rng} -> begin
__fname__blob
end))
...
type lcomp =
{eff_name : FStar_Ident.lident; res_typ : typ; cflags : cflags Prims.list; comp_thunk : (unit  ->  comp, comp) FStar_Util.either FStar_ST.ref}
...
let uu____4821 = (FStar_Util.mk_ref (FStar_Util.Inl (comp_thunk)))
in {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = uu____4821}))


let lcomp_comp : lcomp  ->  comp = (fun lc -> (

let uu____4868 = (FStar_ST.op_Bang lc.comp_thunk)
...
let contains_reflectable : qualifier Prims.list  ->  Prims.bool = (fun l -> (FStar_Util.for_some (fun uu___82_7101 -> (match (uu___82_7101) with
| Reflectable (uu____7102) -> begin
true
end
| uu____7103 -> begin
false
...
let uu____7190 = (FStar_Ident.mk_ident ((x.ppname.FStar_Ident.idText), (r)))
in {ppname = uu____7190; index = uu___89_7189.index; sort = uu___89_7189.sort})))


let on_antiquoted : (term  ->  term)  ->  quoteinfo  ->  quoteinfo = (fun f qi -> (

let aq = (FStar_List.map (fun uu____7232 -> (match (uu____7232) with
| (bv, b, t) -> begin
(
...
let uu___90_7249 = qi
in {qkind = uu___90_7249.qkind; antiquotes = aq})))


let lookup_aq : bv  ->  antiquotations  ->  (Prims.bool * term) FStar_Pervasives_Native.option = (fun bv aq -> (

let uu____7272 = (FStar_List.tryFind (fun uu____7295 -> (match (uu____7295) with
| (bv', uu____7305, uu____7306) -> begin
(bv_eq bv bv')
end)) aq)
in (match (uu____7272) with
| FStar_Pervasives_Native.Some (uu____7317, b, e) -> begin
...
let syn : 'Auu____7360 'Auu____7361 'Auu____7362 . 'Auu____7360  ->  'Auu____7361  ->  ('Auu____7361  ->  'Auu____7360  ->  'Auu____7362)  ->  'Auu____7362 = (fun p k f -> (f k p))


let mk_fvs : 'Auu____7403 . unit  ->  'Auu____7403 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7412 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let mk_uvs : 'Auu____7430 . unit  ->  'Auu____7430 FStar_Pervasives_Native.option FStar_ST.ref = (fun uu____7439 -> (FStar_Util.mk_ref FStar_Pervasives_Native.None))


let new_bv_set : unit  ->  bv FStar_Util.set = (fun uu____7448 -> (FStar_Util.new_set order_bv))


let new_fv_set : unit  ->  FStar_Ident.lident FStar_Util.set = (fun uu____7457 -> (FStar_Util.new_set order_fv))


let order_univ_name : univ_name  ->  univ_name  ->  Prims.int = (fun x y -> (

let uu____7470 = (FStar_Ident.text_of_id x)
in (

let uu____7471 = (FStar_Ident.text_of_id y)
in (FStar_String.compare uu____7470 uu____7471))))


let new_universe_names_set : unit  ->  univ_name FStar_Util.set = (fun uu____7478 -> (FStar_Util.new_set order_univ_name))
...
let mk : 'a . 'a  ->  'a mk_t_a = (fun t uu____7529 r -> (

let uu____7533 = (FStar_Util.mk_ref FStar_Pervasives_Native.None)
in {n = t; pos = r; vars = uu____7533}))


let bv_to_tm : bv  ->  term = (fun bv -> (

let uu____7565 = (range_of_bv bv)
in (mk (Tm_bvar (bv)) FStar_Pervasives_Native.None uu____7565)))


let bv_to_name : bv  ->  term = (fun bv -> (

let uu____7571 = (range_of_bv bv)
in (mk (Tm_name (bv)) FStar_Pervasives_Native.None uu____7571)))
...
let mk_Tm_uinst : term  ->  universes  ->  term = (fun t uu___83_7624 -> (match (uu___83_7624) with
| [] -> begin
t
end
| us -> begin
(match (t.n) with
| Tm_fvar (uu____7626) -> begin
...
in ((lr), (uu____7780)))
in Tm_delayed (uu____7757))
in (mk uu____7756))
in (uu____7749 FStar_Pervasives_Native.None pos)))
...
let mk_lb : (lbname * univ_name Prims.list * FStar_Ident.lident * typ * term * FStar_Range.range)  ->  letbinding = (fun uu____7903 -> (match (uu____7903) with
...
let uu____7993 = (null_bv t)
in ((uu____7993), (FStar_Pervasives_Native.None))))
...
let is_top_level : letbinding Prims.list  ->  Prims.bool = (fun uu___84_8032 -> (match (uu___84_8032) with
| ({lbname = FStar_Util.Inr (uu____8035); lbunivs = uu____8036; lbtyp = uu____8037; lbeff = uu____8038; lbdef = uu____8039; lbattrs = uu____8040; lbpos = uu____8041})::uu____8042 -> begin
true
end
| uu____8055 -> begin
false
...
let freenames_of_binders : binders  ->  freenames = (fun bs -> (FStar_List.fold_right (fun uu____8075 out -> (match (uu____8075) with
| (x, uu____8088) -> begin
...
let uu____8119 = (FStar_Util.set_elements fvs)
in (FStar_All.pipe_right uu____8119 binders_of_list)))


let is_implicit : aqual  ->  Prims.bool = (fun uu___85_8128 -> (match (uu___85_8128) with
| FStar_Pervasives_Native.Some (Implicit (uu____8129)) -> begin
true
end
| uu____8130 -> begin
false
...
let as_implicit : Prims.bool  ->  aqual = (fun uu___86_8135 -> (match (uu___86_8135) with
| true -> begin
...
| Pat_dot_term (uu____8169) -> begin
b
end
| Pat_constant (uu____8176) -> begin
b
end
| Pat_wild (x) -> begin
(x)::b
...
| Pat_cons (uu____8179, pats) -> begin
(FStar_List.fold_left (fun b1 uu____8210 -> (match (uu____8210) with
| (p2, uu____8222) -> begin
(aux b1 p2)
end)) b pats)
end))
...
let uu____8227 = (aux [] p)
in (FStar_All.pipe_left FStar_List.rev uu____8227))))
...
let gen1 = (fun uu____8252 -> ((FStar_Util.incr x);
(FStar_ST.op_Bang x);
))
in (

let reset = (fun uu____8333 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let range_of_ropt : FStar_Range.range FStar_Pervasives_Native.option  ->  FStar_Range.range = (fun uu___87_8407 -> (match (uu___87_8407) with
| FStar_Pervasives_Native.None -> begin
...
let uu____8442 = (next_id ())
in {ppname = id1; index = uu____8442; sort = t})))
...
let uu____8466 = (range_of_bv bv)
in FStar_Pervasives_Native.Some (uu____8466))
in (new_bv uu____8463 bv.sort))
end
| uu____8467 -> begin
(
...
let uu____8469 = (next_id ())
in {ppname = uu___91_8468.ppname; index = uu____8469; sort = uu___91_8468.sort}))
end)))
...
let uu____8501 = (FStar_Util.string_of_int id1)
in (Prims.strcat FStar_Ident.reserved_prefix uu____8501))
in ((uu____8500), ((range_of_ropt ropt))))
in (FStar_Ident.mk_ident uu____8495))))
...
let uu____8619 = (FStar_Ident.mk_ident ((bv.ppname.FStar_Ident.idText), (r)))
in {ppname = uu____8619; index = uu___92_8618.index; sort = uu___92_8618.sort})))
...
let uu____8640 = (FStar_Ident.range_of_lid l)
in (withinfo l uu____8640))
in {fv_name = uu____8639; fv_delta = dd; fv_qual = dq}))


let fv_to_tm : fv  ->  term = (fun fv -> (

let uu____8646 = (FStar_Ident.range_of_lid fv.fv_name.v)
in (mk (Tm_fvar (fv)) FStar_Pervasives_Native.None uu____8646)))


let fvar : FStar_Ident.lident  ->  delta_depth  ->  fv_qual FStar_Pervasives_Native.option  ->  term = (fun l dd dq -> (

let uu____8666 = (lid_as_fv l dd dq)
in (fv_to_tm uu____8666)))
...
let uu____8677 = (lid_of_fv fv)
in (FStar_Ident.range_of_lid uu____8677)))
...
let uu___94_8690 = fv.fv_name
in (

let uu____8691 = (

let uu____8692 = (lid_of_fv fv)
in (FStar_Ident.set_lid_range uu____8692 r))
in {v = uu____8691; p = uu___94_8690.p}))
in {fv_name = uu____8689; fv_delta = uu___93_8688.fv_delta; fv_qual = uu___93_8688.fv_qual})))


let has_simple_attribute : term Prims.list  ->  Prims.string  ->  Prims.bool = (fun l s -> (FStar_List.existsb (fun uu___88_8714 -> (match (uu___88_8714) with
| {n = Tm_constant (FStar_Const.Const_string (data, uu____8718)); pos = uu____8719; vars = uu____8720} when (Prims.op_Equality data s) -> begin
true
end
| uu____8723 -> begin
false
...
let uu____8774 = (fv_eq fv1 fv2)
in (match (uu____8774) with
| true -> begin
(

let uu____8776 = (FStar_List.zip as1 as2)
in (FStar_All.pipe_right uu____8776 (FStar_List.for_all (fun uu____8834 -> (match (uu____8834) with
| ((p11, b1), (p21, b2)) -> begin
...
| (Pat_var (uu____8860), Pat_var (uu____8861)) -> begin
true
end
| (Pat_wild (uu____8862), Pat_wild (uu____8863)) -> begin
true
end
| (Pat_dot_term (bv1, t1), Pat_dot_term (bv2, t2)) -> begin
true
end
| (uu____8876, uu____8877) -> begin
false
end))
...
let uu____8896 = (fvconst l)
in Tm_fvar (uu____8896))
in (mk uu____8895))
in (uu____8888 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____8913 = (lid_as_fv l (Delta_constant_at_level ((Prims.parse_int "1"))) FStar_Pervasives_Native.None)
in Tm_fvar (uu____8913))
in (mk uu____8912))
in (uu____8905 FStar_Pervasives_Native.None FStar_Range.dummyRange)))


let tdataconstr : FStar_Ident.lident  ->  term = (fun l -> (

let uu____8922 = (lid_as_fv l delta_constant (FStar_Pervasives_Native.Some (Data_ctor)))
in (fv_to_tm uu____8922)))
...
let uu____8929 = (tabbrev FStar_Parser_Const.tactic_lid)
in (mk_Tm_uinst uu____8929 ((U_zero)::[])))
in (

let uu____8930 = (

let uu____8931 = (as_arg t_unit)
in (uu____8931)::[])
in (mk_Tm_app uu____8928 uu____8930)))
in (uu____8923 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____8964 = (tabbrev FStar_Parser_Const.u_tac_lid)
in (mk_Tm_uinst uu____8964 ((U_zero)::[])))
in (

let uu____8965 = (

let uu____8966 = (as_arg t_unit)
in (uu____8966)::[])
in (mk_Tm_app uu____8963 uu____8965)))
in (uu____8958 FStar_Pervasives_Native.None FStar_Range.dummyRange))
...
let uu____9004 = (tabbrev FStar_Parser_Const.list_lid)
in (mk_Tm_uinst uu____9004 ((U_zero)::[])))
in (

let uu____9005 = (

let uu____9006 = (as_arg t)
in (uu____9006)::[])
in (mk_Tm_app uu____9003 uu____9005)))
in (uu____8998 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____9044 = (tabbrev FStar_Parser_Const.option_lid)
in (mk_Tm_uinst uu____9044 ((U_zero)::[])))
in (

let uu____9045 = (

let uu____9046 = (as_arg t)
in (uu____9046)::[])
in (mk_Tm_app uu____9043 uu____9045)))
in (uu____9038 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
...
let uu____9089 = (tabbrev FStar_Parser_Const.lid_tuple2)
in (mk_Tm_uinst uu____9089 ((U_zero)::(U_zero)::[])))
in (

let uu____9090 = (

let uu____9091 = (as_arg t1)
in (

let uu____9100 = (

let uu____9111 = (as_arg t2)
in (uu____9111)::[])
in (uu____9091)::uu____9100))
in (mk_Tm_app uu____9088 uu____9090)))
in (uu____9083 FStar_Pervasives_Native.None FStar_Range.dummyRange)))
