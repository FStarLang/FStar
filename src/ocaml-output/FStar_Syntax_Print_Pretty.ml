...
let (univ_to_string : FStar_Syntax_Syntax.universe -> Prims.string) =
  fun u ->
    let e =
      FStar_Syntax_Resugar.resugar_universe u FStar_Compiler_Range.dummyRange in
    let d = FStar_Parser_ToDocument.term_to_document e in pp d
...
let (pat_to_string : FStar_Syntax_Syntax.pat -> Prims.string) =
  fun p ->
    let e = FStar_Syntax_Resugar.resugar_pat p FStar_Syntax_Syntax.no_names in
...
let (binder_to_string' :
  ...
  fun is_arrow ->
    fun b ->
      let uu___ =
        FStar_Syntax_Resugar.resugar_binder b FStar_Compiler_Range.dummyRange in
      match uu___ with
...
let (sigelt_to_string : FStar_Syntax_Syntax.sigelt -> Prims.string) =
  fun se ->
    let uu___ = FStar_Syntax_Resugar.resugar_sigelt se in
    match uu___ with
    | FStar_Pervasives_Native.None -> ""
