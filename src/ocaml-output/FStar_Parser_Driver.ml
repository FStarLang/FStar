...
| FStar_Util.Inl (FStar_Util.Inr (decls), uu____123) -> begin
Decls (decls)
end
...
let uu____196 = (FStar_Parser_ParseIt.parse (FStar_Util.Inl (fn)))
in (match (uu____196) with
| FStar_Util.Inl (FStar_Util.Inl (ast), comments) -> begin
((ast), (comments))
end
| FStar_Util.Inl (FStar_Util.Inr (uu____273), uu____274) -> begin
(
