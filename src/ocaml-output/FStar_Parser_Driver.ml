...
let (parse_fragment : FStar_Parser_ParseIt.input_frag -> fragment) =
  fun frag ->
    ...
    match uu___ with
    | FStar_Parser_ParseIt.ASTFragment (Prims.Inl modul, uu___1) ->
        Modul modul
    | FStar_Parser_ParseIt.ASTFragment (Prims.Inr [], uu___1) -> Empty
...
let (parse_file :
  ...
  fun fn ->
    ...
    match uu___ with
    | FStar_Parser_ParseIt.ASTFragment (Prims.Inl ast, comments) ->
        (ast, comments)
