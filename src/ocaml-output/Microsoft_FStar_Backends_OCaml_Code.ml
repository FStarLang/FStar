...
let as_bin_op = (fun _522835 -> (match (_522835) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522841 -> (match (_522841) with
| (y, _, _) -> begin
(x = y)
end)) infix_prim_ops)
end else begin
...
let as_uni_op = (fun _522845 -> (match (_522845) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522849 -> (match (_522849) with
| (y, _) -> begin
(x = y)
end)) prim_uni_ops)
end else begin
...
let as_standard_type = (fun _522853 -> (match (_522853) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522857 -> (match (_522857) with
| (y, _) -> begin
(x = y)
end)) prim_types)
end else begin
...
let as_standard_constructor = (fun _522861 -> (match (_522861) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522865 -> (match (_522865) with
| (y, _) -> begin
(x = y)
...
let maybe_paren = (fun _522869 inner doc -> (match (_522869) with
| (outer, side) -> begin
(let noparens = (fun _inner _outer side -> (let _522878 = _inner
in (match (_522878) with
| (pi, fi) -> begin
(let _522881 = _outer
in (match (_522881) with
| (po, fo) -> begin
...
(let for1 = (fun _522975 -> (match (_522975) with
| (name, e) -> begin
...
(let _523024 = (Support.Option.get (as_bin_op p))
in (match (_523024) with
| (_, prio, txt) -> begin
...
(let _523036 = (Support.Option.get (as_uni_op p))
in (match (_523036) with
| (_, txt) -> begin
...
(let ids = (Support.List.map (fun _523056 -> (match (_523056) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
in (let body = (doc_of_expr (min_op_prec, NonAssoc) body)
...
(let for1 = (fun _523109 -> (match (_523109) with
| (name, p) -> begin
...
and doc_of_branch = (fun _523142 -> (match (_523142) with
| (p, cond, e) -> begin
...
and doc_of_lets = (fun _523150 -> (match (_523150) with
| (rec_, lets) -> begin
(let for1 = (fun _523155 -> (match (_523155) with
| (name, ids, e) -> begin
(let e = (doc_of_expr (min_op_prec, NonAssoc) e)
in (let ids = (Support.List.map (fun _523160 -> (match (_523160) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
...
let doc_of_mltydecl = (fun decls -> (let for1 = (fun _523204 -> (match (_523204) with
...
(let forfield = (fun _523222 -> (match (_523222) with
| (name, ty) -> begin
...
(let forctor = (fun _523230 -> (match (_523230) with
| (name, tys) -> begin
(match (tys) with
| [] -> begin
...
(let lets = (Support.List.map (fun _523295 -> (match (_523295) with
| (x, y, z) -> begin
((x, (- (1))), y, z)
end)) lets)
...
let rec doc_of_mllib_r = (fun _523304 -> (match (_523304) with
| Microsoft_FStar_Backends_OCaml_Syntax.MLLib (mllib) -> begin
(let rec for1_sig = (fun _523311 -> (match (_523311) with
...
in (let doc = (Support.Option.map (fun _523317 -> (match (_523317) with
| (s, _) -> begin
(doc_of_sig s)
end)) sigmod)
...
and for1_mod = (fun istop _523330 -> (match (_523330) with
| (x, sigmod, Microsoft_FStar_Backends_OCaml_Syntax.MLLib (sub)) -> begin
(let _523331 = (Support.Microsoft.FStar.Util.fprint1 "Gen Code: %s\n" x)
...
in (let doc = (Support.Option.map (fun _523338 -> (match (_523338) with
| (_, m) -> begin
(doc_of_mod m)
end)) sigmod)
...
in (let docs = (Support.List.map (fun _523349 -> (match (_523349) with
| (x, s, m) -> begin
