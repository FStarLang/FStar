...
let as_bin_op = (fun _522904 -> (match (_522904) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522910 -> (match (_522910) with
| (y, _, _) -> begin
(x = y)
end)) infix_prim_ops)
end else begin
...
let as_uni_op = (fun _522914 -> (match (_522914) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522918 -> (match (_522918) with
| (y, _) -> begin
(x = y)
end)) prim_uni_ops)
end else begin
...
let as_standard_type = (fun _522922 -> (match (_522922) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522926 -> (match (_522926) with
| (y, _) -> begin
(x = y)
end)) prim_types)
end else begin
...
let as_standard_constructor = (fun _522930 -> (match (_522930) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _522934 -> (match (_522934) with
| (y, _) -> begin
(x = y)
...
let maybe_paren = (fun _522938 inner doc -> (match (_522938) with
| (outer, side) -> begin
(let noparens = (fun _inner _outer side -> (let _522947 = _inner
in (match (_522947) with
| (pi, fi) -> begin
(let _522950 = _outer
in (match (_522950) with
| (po, fo) -> begin
...
(let for1 = (fun _523044 -> (match (_523044) with
| (name, e) -> begin
...
(let _523093 = (Support.Option.get (as_bin_op p))
in (match (_523093) with
| (_, prio, txt) -> begin
...
(let _523105 = (Support.Option.get (as_uni_op p))
in (match (_523105) with
| (_, txt) -> begin
...
(let ids = (Support.List.map (fun _523125 -> (match (_523125) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
in (let body = (doc_of_expr (min_op_prec, NonAssoc) body)
...
(let for1 = (fun _523178 -> (match (_523178) with
| (name, p) -> begin
...
and doc_of_branch = (fun _523211 -> (match (_523211) with
| (p, cond, e) -> begin
...
and doc_of_lets = (fun _523219 -> (match (_523219) with
| (rec_, lets) -> begin
(let for1 = (fun _523224 -> (match (_523224) with
| (name, ids, e) -> begin
(let e = (doc_of_expr (min_op_prec, NonAssoc) e)
in (let ids = (Support.List.map (fun _523229 -> (match (_523229) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
...
let doc_of_mltydecl = (fun decls -> (let for1 = (fun _523273 -> (match (_523273) with
...
(let forfield = (fun _523291 -> (match (_523291) with
| (name, ty) -> begin
...
(let forctor = (fun _523299 -> (match (_523299) with
| (name, tys) -> begin
(match (tys) with
| [] -> begin
...
(let lets = (Support.List.map (fun _523364 -> (match (_523364) with
| (x, y, z) -> begin
((x, (- (1))), y, z)
end)) lets)
...
let rec doc_of_mllib_r = (fun _523373 -> (match (_523373) with
| Microsoft_FStar_Backends_OCaml_Syntax.MLLib (mllib) -> begin
(let rec for1_sig = (fun _523380 -> (match (_523380) with
...
in (let doc = (Support.Option.map (fun _523386 -> (match (_523386) with
| (s, _) -> begin
(doc_of_sig s)
end)) sigmod)
...
and for1_mod = (fun istop _523399 -> (match (_523399) with
| (x, sigmod, Microsoft_FStar_Backends_OCaml_Syntax.MLLib (sub)) -> begin
(let _523400 = (Support.Microsoft.FStar.Util.fprint1 "Gen Code: %s\n" x)
...
in (let doc = (Support.Option.map (fun _523407 -> (match (_523407) with
| (_, m) -> begin
(doc_of_mod m)
end)) sigmod)
...
in (let docs = (Support.List.map (fun _523418 -> (match (_523418) with
| (x, s, m) -> begin
