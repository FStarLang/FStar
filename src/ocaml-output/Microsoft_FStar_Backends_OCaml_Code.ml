...
let as_bin_op = (fun _521266 -> (match (_521266) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _521272 -> (match (_521272) with
| (y, _, _) -> begin
(x = y)
end)) infix_prim_ops)
end else begin
...
let as_uni_op = (fun _521276 -> (match (_521276) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _521280 -> (match (_521280) with
| (y, _) -> begin
(x = y)
end)) prim_uni_ops)
end else begin
...
let as_standard_type = (fun _521284 -> (match (_521284) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _521288 -> (match (_521288) with
| (y, _) -> begin
(x = y)
end)) prim_types)
end else begin
...
let as_standard_constructor = (fun _521292 -> (match (_521292) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(Support.List.tryFind (fun _521296 -> (match (_521296) with
| (y, _) -> begin
(x = y)
...
let maybe_paren = (fun _521300 inner doc -> (match (_521300) with
| (outer, side) -> begin
(let noparens = (fun _inner _outer side -> (let _521309 = _inner
in (match (_521309) with
| (pi, fi) -> begin
(let _521312 = _outer
in (match (_521312) with
| (po, fo) -> begin
...
(let for1 = (fun _521406 -> (match (_521406) with
| (name, e) -> begin
...
(let _521455 = (Support.Option.get (as_bin_op p))
in (match (_521455) with
| (_, prio, txt) -> begin
...
(let _521467 = (Support.Option.get (as_uni_op p))
in (match (_521467) with
| (_, txt) -> begin
...
(let ids = (Support.List.map (fun _521487 -> (match (_521487) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
in (let body = (doc_of_expr (min_op_prec, NonAssoc) body)
...
(let for1 = (fun _521540 -> (match (_521540) with
| (name, p) -> begin
...
and doc_of_branch = (fun _521573 -> (match (_521573) with
| (p, cond, e) -> begin
...
and doc_of_lets = (fun _521581 -> (match (_521581) with
| (rec_, lets) -> begin
(let for1 = (fun _521586 -> (match (_521586) with
| (name, ids, e) -> begin
(let e = (doc_of_expr (min_op_prec, NonAssoc) e)
in (let ids = (Support.List.map (fun _521591 -> (match (_521591) with
| (x, _) -> begin
(FSharp_Format.text x)
end)) ids)
...
let doc_of_mltydecl = (fun decls -> (let for1 = (fun _521635 -> (match (_521635) with
...
(let forfield = (fun _521653 -> (match (_521653) with
| (name, ty) -> begin
...
(let forctor = (fun _521661 -> (match (_521661) with
| (name, tys) -> begin
(match (tys) with
| [] -> begin
...
(let lets = (Support.List.map (fun _521726 -> (match (_521726) with
| (x, y, z) -> begin
((x, (- (1))), y, z)
end)) lets)
...
let rec doc_of_mllib_r = (fun _521735 -> (match (_521735) with
| Microsoft_FStar_Backends_OCaml_Syntax.MLLib (mllib) -> begin
(let rec for1_sig = (fun _521742 -> (match (_521742) with
...
in (let doc = (Support.Option.map (fun _521748 -> (match (_521748) with
| (s, _) -> begin
(doc_of_sig s)
end)) sigmod)
...
and for1_mod = (fun istop _521761 -> (match (_521761) with
| (x, sigmod, Microsoft_FStar_Backends_OCaml_Syntax.MLLib (sub)) -> begin
(let _521762 = (Support.Microsoft.FStar.Util.fprint1 "Gen Code: %s\n" x)
...
in (let doc = (Support.Option.map (fun _521769 -> (match (_521769) with
| (_, m) -> begin
(doc_of_mod m)
end)) sigmod)
...
in (let docs = (Support.List.map (fun _521780 -> (match (_521780) with
| (x, s, m) -> begin
