...
(let _0_883 = (FStar_Parser_AST.lids_of_let defs)
in (FStar_All.pipe_right _0_883 (FStar_Util.for_some (id_eq_lid x))))
end
| FStar_Parser_AST.Tycon (uu____79, tys) -> begin
(let _0_884 = (FStar_All.pipe_right tys (FStar_List.map (fun uu____98 -> (match (uu____98) with
| (x, uu____103) -> begin
x
end))))
in (FStar_All.pipe_right _0_884 (FStar_Util.for_some (fun uu___143_106 -> (match (uu___143_106) with
...
| [] -> begin
(let _0_885 = (FStar_All.pipe_right (FStar_List.rev out) FStar_List.flatten)
in (FStar_List.append _0_885 impl))
end
| (d)::ds -> begin
...
(Prims.raise (FStar_Errors.Error ((let _0_887 = (let _0_886 = (FStar_Parser_AST.decl_to_string d)
in (FStar_Util.format1 "%s is repeated in the implementation" _0_886))
in ((_0_887), (r))))))
end
| Some (i) -> begin
(Prims.raise (FStar_Errors.Error ((let _0_889 = (let _0_888 = (FStar_Parser_AST.decl_to_string d)
in (FStar_Util.format1 "%s in the interface is implemented with a \'type\'" _0_888))
in ((_0_889), (i.FStar_Parser_AST.drange))))))
end));
(
...
(Prims.raise (FStar_Errors.Error ((let _0_890 = (FStar_Util.format2 "\'val %s\' and \'let %s\' cannot both be provided in an interface" x.FStar_Ident.idText x.FStar_Ident.idText)
in ((_0_890), (d.FStar_Parser_AST.drange))))))
end
| None -> begin
(
...
| true -> begin
(Prims.raise (FStar_Errors.Error ((let _0_892 = (let _0_891 = (FStar_Range.string_of_range let_x.FStar_Parser_AST.drange)
in (FStar_Util.format2 "Assumed declaration %s is defined at %s" x.FStar_Ident.idText _0_891))
in ((_0_892), (d.FStar_Parser_AST.drange))))))
end
| uu____344 -> begin
(
...
| Some (d) -> begin
(Prims.raise (FStar_Errors.Error ((let _0_895 = (let _0_894 = (FStar_Parser_AST.decl_to_string d)
in (let _0_893 = (FStar_Parser_AST.decl_to_string let_x)
in (FStar_Util.format2 "%s is out of order with %s" _0_894 _0_893)))
in ((_0_895), (d.FStar_Parser_AST.drange))))))
end
| uu____364 -> begin
