...
(let _0_866 = (FStar_Parser_AST.lids_of_let defs)
in (FStar_All.pipe_right _0_866 (FStar_Util.for_some (id_eq_lid x))))
end
| FStar_Parser_AST.Tycon (uu____79, tys) -> begin
(let _0_867 = (FStar_All.pipe_right tys (FStar_List.map (fun uu____98 -> (match (uu____98) with
| (x, uu____103) -> begin
x
end))))
in (FStar_All.pipe_right _0_867 (FStar_Util.for_some (fun uu___143_106 -> (match (uu___143_106) with
...
| [] -> begin
(let _0_868 = (FStar_All.pipe_right (FStar_List.rev out) FStar_List.flatten)
in (FStar_List.append _0_868 impl))
end
| (d)::ds -> begin
...
(Prims.raise (FStar_Errors.Error ((let _0_870 = (let _0_869 = (FStar_Parser_AST.decl_to_string d)
in (FStar_Util.format1 "%s is repeated in the implementation" _0_869))
in ((_0_870), (r))))))
end
| Some (i) -> begin
(Prims.raise (FStar_Errors.Error ((let _0_872 = (let _0_871 = (FStar_Parser_AST.decl_to_string d)
in (FStar_Util.format1 "%s in the interface is implemented with a \'type\'" _0_871))
in ((_0_872), (i.FStar_Parser_AST.drange))))))
end));
(
...
(Prims.raise (FStar_Errors.Error ((let _0_873 = (FStar_Util.format2 "\'val %s\' and \'let %s\' cannot both be provided in an interface" x.FStar_Ident.idText x.FStar_Ident.idText)
in ((_0_873), (d.FStar_Parser_AST.drange))))))
end
| None -> begin
(
...
| true -> begin
(Prims.raise (FStar_Errors.Error ((let _0_875 = (let _0_874 = (FStar_Range.string_of_range let_x.FStar_Parser_AST.drange)
in (FStar_Util.format2 "Assumed declaration %s is defined at %s" x.FStar_Ident.idText _0_874))
...
| Some (d) -> begin
(Prims.raise (FStar_Errors.Error ((let _0_878 = (let _0_877 = (FStar_Parser_AST.decl_to_string d)
in (let _0_876 = (FStar_Parser_AST.decl_to_string let_x)
in (FStar_Util.format2 "%s is out of order with %s" _0_877 _0_876)))
in ((_0_878), (d.FStar_Parser_AST.drange))))))
end
| uu____364 -> begin
