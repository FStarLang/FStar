  ...
  fun subst1 ->
    fun goal ->
      ...
      let uu____70 = FStar_Syntax_Subst.subst subst1 goal.goal_ty in
      {
        ...
        witness = uu____69;
        goal_ty = uu____70;
        opts = (uu___54_67.opts);
        is_guard = (uu___54_67.is_guard)
      }
type guard_policy =
  | Goal 
  | SMT 
  ...
  fun subst1 ->
    fun ps ->
      ...
      then ps
        ...
        (let uu___55_409 = ps in
         let uu____410 = subst_goal subst1 ps.main_goal in
         let uu____411 = FStar_List.map (subst_goal subst1) ps.goals in
         {
           main_context = (uu___55_409.main_context);
           main_goal = uu____410;
           all_implicits = (uu___55_409.all_implicits);
           goals = uu____411;
           smt_goals = (uu___55_409.smt_goals);
           depth = (uu___55_409.depth);
           __dump = (uu___55_409.__dump);
           psc = (uu___55_409.psc);
           entry_range = (uu___55_409.entry_range);
           guard_policy = (uu___55_409.guard_policy);
           freshness = (uu___55_409.freshness)
         })
let (decr_depth : proofstate -> proofstate) =
  fun ps ->
    let uu___56_417 = ps in
    {
      main_context = (uu___56_417.main_context);
      main_goal = (uu___56_417.main_goal);
      all_implicits = (uu___56_417.all_implicits);
      goals = (uu___56_417.goals);
      smt_goals = (uu___56_417.smt_goals);
      depth = (ps.depth - (Prims.parse_int "1"));
      __dump = (uu___56_417.__dump);
      psc = (uu___56_417.psc);
      entry_range = (uu___56_417.entry_range);
      guard_policy = (uu___56_417.guard_policy);
      freshness = (uu___56_417.freshness)
    }
let (incr_depth : proofstate -> proofstate) =
  fun ps ->
    let uu___57_421 = ps in
    {
      main_context = (uu___57_421.main_context);
      main_goal = (uu___57_421.main_goal);
      all_implicits = (uu___57_421.all_implicits);
      goals = (uu___57_421.goals);
      smt_goals = (uu___57_421.smt_goals);
      depth = (ps.depth + (Prims.parse_int "1"));
      __dump = (uu___57_421.__dump);
      psc = (uu___57_421.psc);
      entry_range = (uu___57_421.entry_range);
      guard_policy = (uu___57_421.guard_policy);
      freshness = (uu___57_421.freshness)
    }
...
let (set_ps_psc :
  ...
  fun psc ->
    fun ps ->
      let uu___58_437 = ps in
      {
        main_context = (uu___58_437.main_context);
        main_goal = (uu___58_437.main_goal);
        all_implicits = (uu___58_437.all_implicits);
        goals = (uu___58_437.goals);
        smt_goals = (uu___58_437.smt_goals);
        depth = (uu___58_437.depth);
        __dump = (uu___58_437.__dump);
        psc;
        entry_range = (uu___58_437.entry_range);
        guard_policy = (uu___58_437.guard_policy);
        freshness = (uu___58_437.freshness)
      }
let (set_proofstate_range : proofstate -> FStar_Range.range -> proofstate) =
  fun ps ->
    fun r ->
      let uu___59_444 = ps in
      {
        main_context = (uu___59_444.main_context);
        main_goal = (uu___59_444.main_goal);
        all_implicits = (uu___59_444.all_implicits);
        goals = (uu___59_444.goals);
        smt_goals = (uu___59_444.smt_goals);
        depth = (uu___59_444.depth);
        __dump = (uu___59_444.__dump);
        psc = (uu___59_444.psc);
        entry_range = r;
        guard_policy = (uu___59_444.guard_policy);
        freshness = (uu___59_444.freshness)
      }
type direction =
  | TopDown 
