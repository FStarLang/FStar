...
let __proj__Mkgoal__item__context : goal -> FStar_TypeChecker_Env.env =
  fun projectee ->
    match projectee
    with
    | { context= __fname__context
      ; witness= __fname__witness
      ; goal_ty= __fname__goal_ty
      ; opts= __fname__opts
      ; is_guard= __fname__is_guard; _ }
    -> __fname__context
...
let __proj__Mkproofstate__item__main_context
  ...
  fun projectee ->
    match projectee
    with
    | { main_context= __fname__main_context
      ; main_goal= __fname__main_goal
      ; all_implicits= __fname__all_implicits
      ; goals= __fname__goals
      ; smt_goals= __fname__smt_goals
      ; depth= __fname__depth
      ; __dump= __fname____dump
      ; psc= __fname__psc
      ; entry_range= __fname__entry_range; _ }
...
let subst_proof_state : FStar_Syntax_Syntax.subst_t -> proofstate -> proofstate =
  fun subst1 ps ->
    let uu____321 = FStar_Options.tactic_raw_binders () in
    if uu____321 then ps
    else
      ...
      ; main_goal= uu____324
      ; all_implicits= uu___52_323.all_implicits
      ; goals= uu____325
      ; smt_goals= uu___52_323.smt_goals
      ; depth= uu___52_323.depth
      ; __dump= uu___52_323.__dump
      ; psc= uu___52_323.psc
...
let decr_depth : proofstate -> proofstate =
  fun ps ->
    ...
    ; all_implicits= uu___53_331.all_implicits
    ; goals= uu___53_331.goals
    ; smt_goals= uu___53_331.smt_goals
    ; depth= ps.depth - Prims.parse_int "1"
    ; __dump= uu___53_331.__dump
    ; psc= uu___53_331.psc
...
let incr_depth : proofstate -> proofstate =
  fun ps ->
    ...
    ; all_implicits= uu___54_335.all_implicits
    ; goals= uu___54_335.goals
    ; smt_goals= uu___54_335.smt_goals
    ; depth= ps.depth + Prims.parse_int "1"
    ; __dump= uu___54_335.__dump
    ; psc= uu___54_335.psc
  ...
  fun ps ->
    ...
    if uu____339 then
      let uu____342 =
        ...
        subst_proof_state uu____343 ps
      in
...
let set_ps_psc : FStar_TypeChecker_Normalize.psc -> proofstate -> proofstate =
  fun psc ps ->
    ...
    ; all_implicits= uu___55_351.all_implicits
    ; goals= uu___55_351.goals
    ; smt_goals= uu___55_351.smt_goals
    ; depth= uu___55_351.depth
    ; __dump= uu___55_351.__dump
    ; psc
...
let set_proofstate_range : proofstate -> FStar_Range.range -> proofstate =
  fun ps r ->
    ...
    ; all_implicits= uu___56_358.all_implicits
    ; goals= uu___56_358.goals
    ; smt_goals= uu___56_358.smt_goals
    ; depth= uu___56_358.depth
    ; __dump= uu___56_358.__dump
    ; psc= uu___56_358.psc
    ; entry_range= r }


type direction = TopDown | BottomUp [@@deriving show]

let uu___is_TopDown : direction -> Prims.bool =
  fun projectee -> match projectee with TopDown -> true | uu____362 -> false


let uu___is_BottomUp : direction -> Prims.bool =
  fun projectee -> match projectee with BottomUp -> true | uu____366 -> false
