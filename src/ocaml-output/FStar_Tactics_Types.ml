  ...
  fun subst  ->
    fun goal1  ->
      ...
      let ctx_uvar =
        ...
        let uu___3 =
          FStar_Syntax_Subst.subst subst g.FStar_Syntax_Syntax.ctx_uvar_typ
           in
        {
          FStar_Syntax_Syntax.ctx_uvar_head =
            (uu___.FStar_Syntax_Syntax.ctx_uvar_head);
    ...
    fun ps  ->
      let uu___ =
        (FStar_Options.tactic_trace ()) ||
          (let uu___1 = FStar_Options.tactic_trace_d ()  in
           ps.depth <= uu___1)
         in
      if uu___
      then
        ...
        let uu___1 = subst_proof_state subst ps1  in
        ps1.__dump uu___1 "TRACE"
      else ()
  ...
  fun goal1  ->
    let uu___ =
      FStar_Syntax_Unionfind.find
        (goal1.goal_ctx_uvar).FStar_Syntax_Syntax.ctx_uvar_head
       in
    match uu___ with
