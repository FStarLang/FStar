...
let subst_proof_state:
  ...
  fun subst1  ->
    fun ps  ->
      let uu___121_277 = ps in
      let uu____278 = subst_goal subst1 ps.main_goal in
      let uu____279 = FStar_List.map (subst_goal subst1) ps.goals in
      {
        main_context = (uu___121_277.main_context);
        main_goal = uu____278;
        all_implicits = (uu___121_277.all_implicits);
        goals = uu____279;
        smt_goals = (uu___121_277.smt_goals);
        depth = (uu___121_277.depth);
        __dump = (uu___121_277.__dump)
      }
let decr_depth: proofstate -> proofstate =
  fun ps  ->
    let uu___122_286 = ps in
    {
      main_context = (uu___122_286.main_context);
      main_goal = (uu___122_286.main_goal);
      all_implicits = (uu___122_286.all_implicits);
      goals = (uu___122_286.goals);
      smt_goals = (uu___122_286.smt_goals);
      depth = (ps.depth - (Prims.parse_int "1"));
      __dump = (uu___122_286.__dump)
    }
let incr_depth: proofstate -> proofstate =
  fun ps  ->
    let uu___123_291 = ps in
    {
      main_context = (uu___123_291.main_context);
      main_goal = (uu___123_291.main_goal);
      all_implicits = (uu___123_291.all_implicits);
      goals = (uu___123_291.goals);
      smt_goals = (uu___123_291.smt_goals);
      depth = (ps.depth + (Prims.parse_int "1"));
      __dump = (uu___123_291.__dump)
    }
