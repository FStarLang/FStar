...
let (uu___is_Retain : argument_tag -> Prims.bool) =
  fun projectee  ->
    match projectee with | Retain  -> true | uu____10 -> false
    ...
    fun mlty  ->
      ...
      | FStar_Extraction_ML_Syntax.MLTY_Fun (t0,e,t1) ->
          let uu____189 =
            ...
            let uu____197 = elim_mlty env t1  in (uu____196, e, uu____197)
             in
            ...
            let uu____413 =
              FStar_List.map
                (fun uu____435  ->
                   ...
                   | (x,t) ->
                       let uu____450 = elim_mlty env t  in (x, uu____450))
                bvs
               in
            ...
            let ts =
              FStar_Util.map_opt lb.FStar_Extraction_ML_Syntax.mllb_tysc
                (fun uu____829  ->
                   ...
                   | (vars,t) ->
                       let uu____836 = elim_mlty env t  in (vars, uu____836))
               in
            let expr = elim_mlexpr env lb.FStar_Extraction_ML_Syntax.mllb_def
               in
                  ...
                  FStar_List.fold_right
                    (fun p  ->
                       fun uu____977  ->
                         ...
                         | (params,entry1) ->
                             let uu____1009 = FStar_Util.set_mem p freevars
                                in
                             if uu____1009
                               ...
                               FStar_List.map
                                 (fun uu____1241  ->
                                    ...
                                    | (constr,ty) ->
                                        ...
                                        (constr, uu____1256)) constrs
                                in
                             (i, uu____1219)) inductive
                     in
                  FStar_Extraction_ML_Syntax.MLTD_DType uu____1133  in
                (env, parameters, uu____1132)
             in
          let uu____1269 =
            match body with
            ...
            | FStar_Pervasives_Native.Some td1 ->
                ...
                (match uu____1289 with
                 | (env1,parameters1,td2) ->
                     (env1, parameters1, (FStar_Pervasives_Native.Some td2)))
             in
    ...
    fun m  ->
      let elim_module1 env1 m1 =
        ...
        | FStar_Extraction_ML_Syntax.MLM_Ty td ->
            let uu____1377 = FStar_Util.fold_map elim_one_mltydecl env1 td
               in
          ...
          let elim_one_lib env1 lib =
            ...
            | (name,sig_mod,_libs) ->
                ...
                (match uu____1693 with
                 | (sig_mod1,env3) -> (env3, (name, sig_mod1, _libs)))
             in
          let uu____1873 = FStar_Util.fold_map elim_one_lib env libs  in
          (match uu____1873 with
           | (env1,libs1) -> (env1, (FStar_Extraction_ML_Syntax.MLLib libs1)))
