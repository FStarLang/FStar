...
let uu____959 = (FStar_Syntax_Subst.compress tm)
in uu____959.FStar_Syntax_Syntax.n)
in (match (uu____958) with
| FStar_Syntax_Syntax.Tm_uinst (tm1, uu____965) -> begin
(extract_fv tm1)
end
...
let uu____981 = (FStar_Extraction_ML_UEnv.lookup_fv g fv)
in (FStar_All.pipe_left FStar_Util.right uu____981))
in (match (uu____976) with
| (uu____1010, uu____1011, tysc, uu____1013) -> begin
(

let uu____1014 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) (FStar_Extraction_ML_Syntax.MLE_Name (mlp)))
in ((uu____1014), (tysc)))
end)))
end
| uu____1015 -> begin
...
let uu____1027 = (extract_fv a.FStar_Syntax_Syntax.action_defn)
in (match (uu____1027) with
| (a_tm, ty_sc) -> begin
(

let uu____1034 = (FStar_Extraction_ML_UEnv.action_name ed a)
...
let uu____1044 = (extract_fv (Prims.snd ed.FStar_Syntax_Syntax.return_repr))
in (match (uu____1044) with
| (return_tm, ty_sc) -> begin
(

let uu____1052 = (FStar_Extraction_ML_UEnv.monad_op_name ed "return")
...
let uu____1067 = (extract_fv (Prims.snd ed.FStar_Syntax_Syntax.bind_repr))
in (match (uu____1067) with
| (bind_tm, ty_sc) -> begin
(

let uu____1075 = (FStar_Extraction_ML_UEnv.monad_op_name ed "bind")
...
let uu____1087 = (FStar_Util.fold_map extract_action g2 ed.FStar_Syntax_Syntax.actions)
...
| FStar_Syntax_Syntax.Sig_new_effect (uu____1099) -> begin
((g), ([]))
end
| FStar_Syntax_Syntax.Sig_declare_typ (lid, uu____1102, t, quals) when (FStar_Extraction_ML_Term.is_arity g t) -> begin
(

let uu____1107 = (

let uu____1108 = (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___151_1110 -> (match (uu___151_1110) with
| FStar_Syntax_Syntax.Assumption -> begin
true
end
| uu____1111 -> begin
...
in (FStar_All.pipe_right uu____1108 Prims.op_Negation))
...
let uu____1117 = (FStar_Syntax_Util.arrow_formals t)
...
let uu____1141 = (FStar_Syntax_Util.abs bs FStar_TypeChecker_Common.t_unit None)
in (extract_typ_abbrev g fv quals uu____1141)))
end))
end))
end
| FStar_Syntax_Syntax.Sig_let ((false, (lb)::[]), uu____1148, quals, uu____1150) when (FStar_Extraction_ML_Term.is_arity g lb.FStar_Syntax_Syntax.lbtyp) -> begin
(

let uu____1161 = (FStar_Util.right lb.FStar_Syntax_Syntax.lbname)
in (extract_typ_abbrev g uu____1161 quals lb.FStar_Syntax_Syntax.lbdef))
end
| FStar_Syntax_Syntax.Sig_let (lbs, uu____1163, quals, attrs) -> begin
(
...
let uu____1183 = (FStar_Extraction_ML_Term.term_as_mlexpr g elet)
in (match (uu____1183) with
| (ml_let, uu____1191, uu____1192) -> begin
(match (ml_let.FStar_Extraction_ML_Syntax.expr) with
| FStar_Extraction_ML_Syntax.MLE_Let ((flavor, uu____1197, bindings), uu____1199) -> begin
(

let uu____1206 = (FStar_List.fold_left2 (fun uu____1213 ml_lb uu____1215 -> (match (((uu____1213), (uu____1215))) with
| ((env, ml_lbs), {FStar_Syntax_Syntax.lbname = lbname; FStar_Syntax_Syntax.lbunivs = uu____1228; FStar_Syntax_Syntax.lbtyp = t; FStar_Syntax_Syntax.lbeff = uu____1230; FStar_Syntax_Syntax.lbdef = uu____1231}) -> begin
(
...
let uu____1250 = (FStar_Util.right lbname)
in uu____1250.FStar_Syntax_Syntax.fv_name)
in uu____1245.FStar_Syntax_Syntax.v)
in (

let uu____1254 = (

let uu____1257 = (FStar_All.pipe_right quals (FStar_Util.for_some (fun uu___152_1259 -> (match (uu___152_1259) with
| FStar_Syntax_Syntax.Projector (uu____1260) -> begin
true
end
| uu____1263 -> begin
false
...
let uu____1267 = (mangle_projector_lid lb_lid)
in (FStar_All.pipe_right uu____1267 FStar_Extraction_ML_Syntax.mlpath_of_lident))
in (

let uu____1268 = (

let uu____1271 = (FStar_Util.right lbname)
in (

let uu____1272 = (FStar_Util.must ml_lb.FStar_Extraction_ML_Syntax.mllb_tysc)
in (FStar_Extraction_ML_UEnv.extend_fv' env uu____1271 mname uu____1272 ml_lb.FStar_Extraction_ML_Syntax.mllb_add_unit false)))
in (match (uu____1268) with
| (env1, uu____1276) -> begin
((env1), ((

let uu___157_1277 = ml_lb
in {FStar_Extraction_ML_Syntax.mllb_name = (((Prims.snd mname)), ((Prims.parse_int "0"))); FStar_Extraction_ML_Syntax.mllb_tysc = uu___157_1277.FStar_Extraction_ML_Syntax.mllb_tysc; FStar_Extraction_ML_Syntax.mllb_add_unit = uu___157_1277.FStar_Extraction_ML_Syntax.mllb_add_unit; FStar_Extraction_ML_Syntax.mllb_def = uu___157_1277.FStar_Extraction_ML_Syntax.mllb_def; FStar_Extraction_ML_Syntax.print_typ = uu___157_1277.FStar_Extraction_ML_Syntax.print_typ})))
end)))
end
| uu____1279 -> begin
(
...
let uu____1284 = (FStar_Util.must ml_lb.FStar_Extraction_ML_Syntax.mllb_tysc)
in (FStar_Extraction_ML_UEnv.extend_lb env lbname t uu____1284 ml_lb.FStar_Extraction_ML_Syntax.mllb_add_unit false))
in (FStar_All.pipe_left Prims.fst uu____1281))
in ((uu____1280), (ml_lb)))
end))
...
let flags = (FStar_List.choose (fun uu___153_1304 -> (match (uu___153_1304) with
| FStar_Syntax_Syntax.Assumption -> begin
...
let flags' = (FStar_List.choose (fun uu___154_1311 -> (match (uu___154_1311) with
| {FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_string (data, uu____1316)); FStar_Syntax_Syntax.tk = uu____1317; FStar_Syntax_Syntax.pos = uu____1318; FStar_Syntax_Syntax.vars = uu____1319} -> begin
...
let uu____1331 = (FStar_Extraction_ML_Util.mlloc_of_range se.FStar_Syntax_Syntax.sigrng)
in FStar_Extraction_ML_Syntax.MLM_Loc (uu____1331))
in (uu____1330)::(FStar_Extraction_ML_Syntax.MLM_Let (((flavor), ((FStar_List.append flags flags')), ((FStar_List.rev ml_lbs')))))::[])
in ((g1), (uu____1328)))))
end))
end
| uu____1335 -> begin
...
let uu____1337 = (FStar_Extraction_ML_Code.string_of_mlexpr g.FStar_Extraction_ML_UEnv.currentModule ml_let)
in (FStar_Util.format1 "Impossible: Translated a let to a non-let: %s" uu____1337))
in (failwith uu____1336))
end)
end)))
end
| FStar_Syntax_Syntax.Sig_declare_typ (lid, uu____1342, t, quals) -> begin
(

let uu____1347 = (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Assumption))
...
let uu____1373 = (FStar_Syntax_Syntax.gen_bv "_" None t1)
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_binder uu____1373))
in (

let uu____1374 = (fail_exp lid t1)
in (FStar_Syntax_Util.abs ((b)::[]) uu____1374 None)))
end
| (bs, t1) -> begin
(

let uu____1392 = (fail_exp lid t1)
in (FStar_Syntax_Util.abs bs uu____1392 None))
end))
in (

let uu___158_1398 = se
in (
...
let uu____1418 = (FStar_Syntax_Syntax.lid_as_fv lid FStar_Syntax_Syntax.Delta_constant None)
in FStar_Util.Inr (uu____1418))
in {FStar_Syntax_Syntax.lbname = uu____1415; FStar_Syntax_Syntax.lbunivs = []; FStar_Syntax_Syntax.lbtyp = t; FStar_Syntax_Syntax.lbeff = FStar_Syntax_Const.effect_ML_lid; FStar_Syntax_Syntax.lbdef = imp})
in (uu____1414)::[])
in ((false), (uu____1412)))
in ((uu____1408), ([]), (quals), ([])))
in FStar_Syntax_Syntax.Sig_let (uu____1400))
in {FStar_Syntax_Syntax.sigel = uu____1399; FStar_Syntax_Syntax.sigrng = uu___158_1398.FStar_Syntax_Syntax.sigrng})))
in (

let uu____1426 = (extract_sig g always_fail)
in (match (uu____1426) with
| (g1, mlm) -> begin
(

let uu____1437 = (FStar_Util.find_map quals (fun uu___155_1439 -> (match (uu___155_1439) with
...
let uu____1450 = (FStar_Extraction_ML_Util.mlloc_of_range se.FStar_Syntax_Syntax.sigrng)
in FStar_Extraction_ML_Syntax.MLM_Loc (uu____1450))
in (

let uu____1451 = (

let uu____1453 = (FStar_Extraction_ML_Term.ind_discriminator_body g1 lid l)
in (uu____1453)::[])
in (uu____1449)::uu____1451))
in ((g1), (uu____1447)))
end
| uu____1455 -> begin
(

let uu____1457 = (FStar_Util.find_map quals (fun uu___156_1459 -> (match (uu___156_1459) with
| FStar_Syntax_Syntax.Projector (l, uu____1462) -> begin
Some (l)
end
| uu____1463 -> begin
None
...
let uu____1475 = (FStar_Extraction_ML_Term.term_as_mlexpr g e)
in (match (uu____1475) with
| (ml_main, uu____1483, uu____1484) -> begin
(
...
let uu____1488 = (FStar_Extraction_ML_Util.mlloc_of_range se.FStar_Syntax_Syntax.sigrng)
in FStar_Extraction_ML_Syntax.MLM_Loc (uu____1488))
in (uu____1487)::(FStar_Extraction_ML_Syntax.MLM_Top (ml_main))::[])
in ((g), (uu____1485)))
end))
end
| FStar_Syntax_Syntax.Sig_new_effect_for_free (uu____1490) -> begin
...
let uu____1508 = (FStar_Util.fold_map extract_sig g m.FStar_Syntax_Syntax.declarations)
in (FStar_All.pipe_right uu____1508 Prims.fst)))
...
let uu____1533 = (FStar_Options.restore_cmd_line_options true)
in (
...
let uu___159_1536 = g
in {FStar_Extraction_ML_UEnv.tcenv = uu___159_1536.FStar_Extraction_ML_UEnv.tcenv; FStar_Extraction_ML_UEnv.gamma = uu___159_1536.FStar_Extraction_ML_UEnv.gamma; FStar_Extraction_ML_UEnv.tydefs = uu___159_1536.FStar_Extraction_ML_UEnv.tydefs; FStar_Extraction_ML_UEnv.currentModule = name})
in (

let uu____1537 = (FStar_Util.fold_map extract_sig g1 m.FStar_Syntax_Syntax.declarations)
...
let uu____1554 = (FStar_Options.codegen ())
...
let uu____1558 = (((m.FStar_Syntax_Syntax.name.FStar_Ident.str <> "Prims") && (is_kremlin || (not (m.FStar_Syntax_Syntax.is_interface)))) && (FStar_Options.should_extract m.FStar_Syntax_Syntax.name.FStar_Ident.str))
in (match (uu____1558) with
| true -> begin
((

let uu____1563 = (FStar_Syntax_Print.lid_to_string m.FStar_Syntax_Syntax.name)
