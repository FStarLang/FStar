                ...
                let uu___7 =
                  let uu___8 =
                    let uu___9 =
                      let uu___10 =
                        let uu___11 =
                          let uu___12 = FStar_Syntax_Print.lid_to_string lid
                             in
  ...
  fun lid  ->
    fun t  ->
      let imp =
        ...
        | (bs,t1) ->
            let uu___1 = fail_exp lid t1  in
            FStar_Syntax_Util.abs bs uu___1 FStar_Pervasives_Native.None
         in
      let lb =
        let uu___ =
          let uu___1 =
              ...
              | (FStar_Syntax_Syntax.Tm_fvar fv,uu___4::[]) when
                  ...
                  (match uu___5 with
                   ...
                   | FStar_Pervasives_Native.Some l ->
                       let uu___6 =
                         let uu___7 =
                           let uu___8 = FStar_Syntax_Syntax.range_of_fv fv
                              in
                           (l, uu___8)  in
           ...
           fun uu___  ->
             ...
             | (bv,uu___1) ->
                 let env2 = FStar_Extraction_ML_UEnv.extend_ty env1 bv false
                    in
                 let name =
                   let uu___2 = FStar_Extraction_ML_UEnv.lookup_bv env2 bv
                      in
                   match uu___2 with
                   | FStar_Util.Inl ty ->
          ...
          FStar_Util.fold_map
            (fun env1  ->
               fun se  ->
                 ...
                 | uu___1 -> (env1, [])) env ses
           in
        match uu___ with | (env1,ifams) -> (env1, (FStar_List.flatten ifams))
...
let (iface_of_tydefs : FStar_Extraction_ML_UEnv.tydef Prims.list -> iface) =
  fun tds  ->
    let uu___ = empty_iface  in
    let uu___1 = FStar_List.map (fun uu___2  -> FStar_Util.Inl uu___2) tds
       in
    let uu___2 =
      FStar_List.map
        (fun td  ->
...
let (iface_to_string : iface -> Prims.string) =
  fun iface1  ->
    ...
    let print_type_name uu___ =
      match uu___ with | (tn,uu___1) -> FStar_Syntax_Print.fv_to_string tn
       in
    let uu___ =
      let uu___1 = FStar_List.map (print_binding cm) iface1.iface_bindings
         in
        ...
        fun lb  ->
          let uu___ =
            ...
            | (tcenv,uu___2,def_typ) ->
                let uu___3 = as_pair def_typ  in (tcenv, uu___3)
             in
          match uu___ with
          | (tcenv,(lbdef,lbtyp)) ->
              let lbtyp1 =
                FStar_TypeChecker_Normalize.normalize
                  ...
                  FStar_TypeChecker_Env.UnfoldUntil
                    FStar_Syntax_Syntax.delta_constant;
                  FStar_TypeChecker_Env.ForExtraction] tcenv lbtyp
                 in
              let lbdef1 =
                    ...
                    | (env1,ml_bs) ->
                        ...
                        let metadata =
                          ...
                          let meta =
                            ...
                            let uu___4 = FStar_List.choose flag_of_qual quals
                               in
                            FStar_List.append uu___3 uu___4  in
                          if has_val_decl
                          then
                            let uu___3 =
                              ...
                              FStar_Extraction_ML_Syntax.HasValDecl uu___4
                               in
                            uu___3 :: meta
                          else meta  in
                        ...
                        (match uu___3 with
                         | (mlpath,iface1,env2) ->
                             ...
                             let def2 =
                               let uu___4 =
                                 let uu___5 =
                                   let uu___6 = FStar_Ident.range_of_lid lid
                                      in
                        ...
                        let def =
                          let uu___4 =
                            let uu___5 =
                              ...
                              FStar_Extraction_ML_Util.mlloc_of_range uu___6
                               in
                            FStar_Extraction_ML_Syntax.MLM_Loc uu___5  in
                          [uu___4; FStar_Extraction_ML_Syntax.MLM_Ty [td]]
                           in
    ...
    fun se  ->
      let extract_ctor env_iparams ml_tyvars env1 ctor =
        let mlt =
          ...
          FStar_Extraction_ML_Util.eraseTypeDeep
            (FStar_Extraction_ML_Util.udelta_unfold env_iparams) uu___
           in
      ...
      | (FStar_Syntax_Syntax.Sig_bundle
         ({
            ...
            FStar_Syntax_Syntax.sigopts = uu___8;_}::[],uu___9),(FStar_Syntax_Syntax.ExceptionConstructor
         )::[]) ->
    ...
    fun ed  ->
      ...
      let rec extract_fv tm =
        (let uu___1 =
           ...
           FStar_All.pipe_left uu___2 (FStar_Options.Other "ExtractionReify")
            in
         if uu___1
         then
      ...
      let extract_action g1 a =
        (let uu___2 =
           ...
           FStar_All.pipe_left uu___3 (FStar_Options.Other "ExtractionReify")
            in
         if uu___2
         then
           let uu___3 =
        ...
        (let lbname =
         ...
         let action_lb =
           FStar_Syntax_Syntax.mk
             ...
             (a.FStar_Syntax_Syntax.action_defn).FStar_Syntax_Syntax.pos
            in
         let uu___2 = FStar_Extraction_ML_Term.term_as_mlexpr g1 action_lb
            in
         match uu___2 with
         | (a_let,uu___3,ty) ->
             let uu___4 =
        ...
        | (return_tm,ty_sc) ->
            ...
            (match uu___2 with
             | (return_nm,return_lid,return_b,g1) ->
                 ...
                 (match uu___3 with | (iface1,impl) -> (g1, iface1, impl)))
         in
      match uu___ with
            ...
            | (bind_tm,ty_sc) ->
                ...
                (match uu___3 with
                 | (bind_nm,bind_lid,bind_b,g2) ->
                     ...
                     (match uu___4 with | (iface1,impl) -> (g2, iface1, impl)))
             in
          (match uu___1 with
          ...
          let uu___ =
            (FStar_All.pipe_right quals
              ...
              (let uu___1 =
                 ...
                 FStar_TypeChecker_Util.must_erase_for_extraction uu___2 t
                  in
                   ...
                   FStar_List.map
                     (fun uu___2  ->
                        ...
                        | FStar_Util.Inr (p,m,n) ->
                            (p, m, (FStar_Util.Inr n))) iface1.iface_tydefs
                    in
                 let uu___2 =
          ...
          let uu___ =
            let uu___1 =
              let uu___2 =
                ...
                FStar_TypeChecker_Normalize.normalize steps uu___3 ctor.dtyp
                 in
              ...
              FStar_List.map
                (fun uu___2  ->
                   ...
                   | ({ FStar_Syntax_Syntax.ppname = ppname;
                        ...
                        FStar_Syntax_Syntax.sort = uu___4;_},uu___5)
                        ...
                        let uu___4 =
                          FStar_All.pipe_right indices
                            (FStar_List.mapi
                               (fun i  ->
                                  fun uu___5  ->
                                    let uu___6 = FStar_Util.string_of_int i
                                       in
                                    Prims.op_Hat "'dummyV" uu___6))
                           in
                         ...
                         | (register,args) ->
                             ...
                             let arity1 =
                               let uu___5 =
                                 ...
                                 FStar_Extraction_ML_Syntax.MLC_Int uu___6
                                  in
                               FStar_Extraction_ML_Syntax.MLE_Const uu___5
                                in
                             let app =
                               FStar_All.pipe_left
                                 (FStar_Extraction_ML_Syntax.with_ty
                                         ...
                                         then (env, ml_lbs)
                                           ...
                                           (let lb_lid =
                                              let uu___17 =
                                                ...
                                                uu___18.FStar_Syntax_Syntax.fv_name
                                                 in
                                              uu___17.FStar_Syntax_Syntax.v
                                               in
                                            let flags'' =
                                              let uu___17 =
                                                let uu___18 =
                               ...
                               let uu___8 =
                                 let uu___9 =
                                   ...
                                   FStar_Extraction_ML_Syntax.MLM_Let
                                    ...
                                    in
                                 let uu___10 = maybe_register_plugin g1 se
                                    in
           ...
           let uu___2 =
             (FStar_All.pipe_right quals
               ...
               (let uu___3 =
                  ...
                  FStar_TypeChecker_Util.must_erase_for_extraction uu___4 t
                   in
           ...
           then
             let always_fail1 =
               ...
               let uu___4 =
                 let uu___5 =
                   let uu___6 =
                     let uu___7 = let uu___8 = always_fail lid t  in [uu___8]
                        in
                     (false, uu___7)  in
                   (uu___6, [])  in
                 ...
                 fun se  ->
                   ...
                   then
                     ...
                     (FStar_Util.print1 "+++About to extract {%s}\n" nm;
                      (let uu___5 = FStar_Util.format1 "---Extracted {%s}" nm
                          in
...
let (extract :
  ...
  fun g  ->
    fun m  ->
      ...
      (let uu___2 =
         let uu___3 =
           let uu___4 = FStar_Ident.string_of_lid m.FStar_Syntax_Syntax.name
              in
          ...
          | (g1,mllib) ->
              let uu___5 =
                ...
                | FStar_Pervasives_Native.Some mllib1 ->
                    ...
                    (match uu___6 with
                     | (g2,mllib2) ->
                         (g2, (FStar_Pervasives_Native.Some mllib2)))
                 in
              (match uu___5 with
               | (g2,mllib1) ->
                   ((let uu___7 = FStar_Options.restore_cmd_line_options true
                        in
