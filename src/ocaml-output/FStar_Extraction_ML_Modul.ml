...
let (extract_sigelt_iface :
  ...
  fun g  ->
    fun se  ->
      ...
      | FStar_Syntax_Syntax.Sig_fail uu____3960 ->
          failwith "impossible: trying to extract Sig_fail"
      | FStar_Syntax_Syntax.Sig_new_effect ed ->
          let uu____3979 =
            (let uu____3983 = FStar_Extraction_ML_UEnv.tcenv_of_uenv g  in
             FStar_TypeChecker_Env.is_reifiable_effect uu____3983
               ed.FStar_Syntax_Syntax.mname)
              && (FStar_List.isEmpty ed.FStar_Syntax_Syntax.binders)
             in
          if uu____3979
          then
            let uu____3995 = extract_reifiable_effect g ed  in
            (match uu____3995 with | (env,iface1,uu____4010) -> (env, iface1))
          else (g, empty_iface)
...
let (extract_iface' :
  ...
  fun g  ->
    fun modul  ->
      let uu____4032 = FStar_Options.interactive ()  in
      if uu____4032
      then (g, empty_iface)
      else
        (let uu____4041 = FStar_Options.restore_cmd_line_options true  in
         let decls = modul.FStar_Syntax_Syntax.declarations  in
         let iface1 =
           let uu___621_4045 = empty_iface  in
           let uu____4046 = FStar_Extraction_ML_UEnv.current_module_of_uenv g
              in
           {
             iface_module_name = uu____4046;
             iface_bindings = (uu___621_4045.iface_bindings);
             iface_tydefs = (uu___621_4045.iface_tydefs);
             iface_type_names = (uu___621_4045.iface_type_names)
           }  in
         let res =
           FStar_List.fold_left
             (fun uu____4064  ->
                fun se  ->
                  ...
                  | (g1,iface2) ->
                      let uu____4076 = extract_sigelt_iface g1 se  in
                      (match uu____4076 with
                       | (g2,iface') ->
                           let uu____4087 = iface_union iface2 iface'  in
                           (g2, uu____4087))) (g, iface1) decls
            in
         (let uu____4089 = FStar_Options.restore_cmd_line_options true  in
          FStar_All.pipe_left (fun uu____4091  -> ()) uu____4089);
         res)
      ...
      then
        let uu____4114 =
          FStar_Util.format1 "Extracted interface of %s"
           ...
           in
        FStar_Util.measure_execution_time uu____4114
          (fun uu____4122  -> extract_iface' g modul)
  ...
  fun env  ->
    fun se  ->
      let extract_ctor env_iparams ml_tyvars env1 ctor =
        let mlt =
          let uu____4213 =
            FStar_Extraction_ML_Term.term_as_mlty env_iparams ctor.dtyp  in
          FStar_Extraction_ML_Util.eraseTypeDeep
            (FStar_Extraction_ML_Util.udelta_unfold env_iparams) uu____4213
           in
        let steps =
          ...
          let uu____4221 =
            let uu____4222 =
              let uu____4225 =
                ...
                FStar_TypeChecker_Normalize.normalize steps uu____4226
                  ctor.dtyp
                 in
              FStar_Syntax_Subst.compress uu____4225  in
            uu____4222.FStar_Syntax_Syntax.n  in
          match uu____4221 with
          | FStar_Syntax_Syntax.Tm_arrow (bs,uu____4231) ->
              FStar_List.map
                (fun uu____4264  ->
                   match uu____4264 with
                   | ({ FStar_Syntax_Syntax.ppname = ppname;
                        FStar_Syntax_Syntax.index = uu____4273;
                        FStar_Syntax_Syntax.sort = uu____4274;_},uu____4275)
        ...
        | (env2,uu____4318,uu____4319) ->
            let uu____4322 =
              let uu____4335 = lident_as_mlsymbol ctor.dname  in
              let uu____4337 =
                let uu____4345 = FStar_Extraction_ML_Util.argTypes mlt  in
                FStar_List.zip names1 uu____4345  in
              (uu____4335, uu____4337)  in
            (env2, uu____4322)
         in
      let extract_one_family env1 ind =
        let uu____4406 = binders_as_mlty_binders env1 ind.iparams  in
        ...
        | (env_iparams,vars) ->
            ...
            (match uu____4450 with
             | (env2,ctors) ->
                 let uu____4557 = FStar_Syntax_Util.arrow_formals ind.ityp
                    in
                            ...
                            (FStar_List.mapi
                               (fun i  ->
                                  fun uu____4650  ->
                                    ...
                                    Prims.op_Hat "'dummyV" uu____4658))
                           in
                        FStar_List.append vars uu____4624  in
                      let tbody =
                        let uu____4663 =
                          FStar_Util.find_opt
                            (fun uu___7_4668  ->
                               match uu___7_4668 with
                               | FStar_Syntax_Syntax.RecordType uu____4670 ->
                                   true
                               | uu____4680 -> false) ind.iquals
                           in
                        match uu____4663 with
                        | FStar_Pervasives_Native.Some
                            (FStar_Syntax_Syntax.RecordType (ns,ids)) ->
                            let uu____4692 = FStar_List.hd ctors  in
                        ...
                        | uu____4780 ->
                         ...
                         in
                      let uu____4783 =
                        let uu____4806 = lident_as_mlsymbol ind.iname  in
                        (false, uu____4806, FStar_Pervasives_Native.None,
      ...
      | (FStar_Syntax_Syntax.Sig_bundle
         ({
            FStar_Syntax_Syntax.sigel = FStar_Syntax_Syntax.Sig_datacon
              (l,uu____4851,t,uu____4853,uu____4854,uu____4855);
            ...
            FStar_Syntax_Syntax.sigopts = uu____4860;_}::[],uu____4861),(FStar_Syntax_Syntax.ExceptionConstructor
         )::[]) ->
          let uu____4882 = extract_ctor env [] env { dname = l; dtyp = t }
             in
          (match uu____4882 with
           | (env1,ctor) -> (env1, [FStar_Extraction_ML_Syntax.MLM_Exn ctor]))
      | (FStar_Syntax_Syntax.Sig_bundle (ses,uu____4935),quals) ->
          let uu____4949 =
            FStar_Syntax_Util.has_attribute se.FStar_Syntax_Syntax.sigattrs
              FStar_Parser_Const.erasable_attr
             in
          ...
          then (env, [])
            ...
            (let uu____4962 = bundle_as_inductive_families env ses quals  in
      ...
      | uu____5090 -> failwith "Unexpected signature element"
    ...
    fun se  ->
      ...
      let plugin_with_arity attrs =
        FStar_Util.find_map attrs
          (fun t  ->
             let uu____5148 = FStar_Syntax_Util.head_and_args t  in
                 ...
                 let uu____5196 =
                   let uu____5198 =
                     FStar_Syntax_Util.is_fvar FStar_Parser_Const.plugin_attr
                       head1
                      in
                   Prims.op_Negation uu____5198  in
                 if uu____5196
                 then FStar_Pervasives_Native.None
                   ...
                   (match args with
                    | ({
                         FStar_Syntax_Syntax.n =
                           FStar_Syntax_Syntax.Tm_constant
                           (FStar_Const.Const_int (s,uu____5217));
                         FStar_Syntax_Syntax.pos = uu____5218;
                         FStar_Syntax_Syntax.vars = uu____5219;_},uu____5220)::[]
                        ->
                        let uu____5259 =
                          let uu____5263 = FStar_Util.int_of_string s  in
         ...
         in
      let uu____5284 =
        let uu____5286 = FStar_Options.codegen ()  in
        uu____5286 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)  in
      if uu____5284
      then []
      else
        (let uu____5296 = plugin_with_arity se.FStar_Syntax_Syntax.sigattrs
            in
             ...
             (match se.FStar_Syntax_Syntax.sigel with
              | FStar_Syntax_Syntax.Sig_let (lbs,lids) ->
                  let mk_registration lb =
                    ...
                    let ml_name_str =
                      let uu____5338 =
                        let uu____5339 = FStar_Ident.string_of_lid fv_lid1
                           in
                        FStar_Extraction_ML_Syntax.MLC_String uu____5339  in
                      FStar_Extraction_ML_Syntax.MLE_Const uu____5338  in
                    let uu____5341 =
                      let uu____5354 =
                        FStar_Extraction_ML_UEnv.tcenv_of_uenv g  in
                      FStar_Extraction_ML_Util.interpret_plugin_as_term_fun
                        uu____5354 fv fv_t arity_opt ml_name_str
                       in
  ...
  fun g  ->
    fun se  ->
      FStar_Extraction_ML_UEnv.debug g
        (fun u  ->
           let uu____5486 = FStar_Syntax_Print.sigelt_to_string se  in
           FStar_Util.print1 ">>>> extract_sig %s \n" uu____5486);
      (match se.FStar_Syntax_Syntax.sigel with
       | FStar_Syntax_Syntax.Sig_bundle uu____5495 -> extract_bundle g se
       | FStar_Syntax_Syntax.Sig_inductive_typ uu____5504 ->
           extract_bundle g se
       | FStar_Syntax_Syntax.Sig_datacon uu____5521 -> extract_bundle g se
       | FStar_Syntax_Syntax.Sig_new_effect ed when
           let uu____5538 = FStar_Extraction_ML_UEnv.tcenv_of_uenv g  in
           FStar_TypeChecker_Env.is_reifiable_effect uu____5538
             ed.FStar_Syntax_Syntax.mname
           ->
           let uu____5539 = extract_reifiable_effect g ed  in
           (match uu____5539 with | (env,_iface,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_splice uu____5563 ->
           failwith "impossible: trying to extract splice"
       | FStar_Syntax_Syntax.Sig_fail uu____5577 ->
           failwith "impossible: trying to extract Sig_fail"
       | FStar_Syntax_Syntax.Sig_new_effect uu____5597 -> (g, [])
       | FStar_Syntax_Syntax.Sig_declare_typ (lid,univs1,t) when
           FStar_Extraction_ML_Term.is_arity g t ->
           let uu____5603 =
             extract_type_declaration g lid se.FStar_Syntax_Syntax.sigquals
               se.FStar_Syntax_Syntax.sigattrs univs1 t
              in
           (match uu____5603 with | (env,uu____5619,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let ((false ,lb::[]),uu____5628) when
           FStar_Extraction_ML_Term.is_arity g lb.FStar_Syntax_Syntax.lbtyp
           ->
           let uu____5637 =
             extract_typ_abbrev g se.FStar_Syntax_Syntax.sigquals
               se.FStar_Syntax_Syntax.sigattrs lb
              in
           (match uu____5637 with | (env,uu____5653,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let ((true ,lbs),uu____5662) when
           FStar_Util.for_some
             (fun lb  ->
                FStar_Extraction_ML_Term.is_arity g
                  lb.FStar_Syntax_Syntax.lbtyp) lbs
           ->
           let uu____5675 = extract_let_rec_types se g lbs  in
           (match uu____5675 with | (env,uu____5691,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let (lbs,uu____5700) ->
           ...
           let uu____5711 =
             ...
             | FStar_Pervasives_Native.Some
                 (ats,(tau,FStar_Pervasives_Native.None )::uu____5749) ->
           ...
           (match uu____5711 with
            | (attrs1,post_tau) ->
                let postprocess_lb tau lb =
                  let lbdef =
                    let uu____5835 = FStar_Extraction_ML_UEnv.tcenv_of_uenv g
                       in
                    FStar_TypeChecker_Env.postprocess uu____5835 tau
                      lb.FStar_Syntax_Syntax.lbtyp
                      lb.FStar_Syntax_Syntax.lbdef
                     in
                  let uu___871_5836 = lb  in
                  {
                    FStar_Syntax_Syntax.lbname =
                      (uu___871_5836.FStar_Syntax_Syntax.lbname);
                    FStar_Syntax_Syntax.lbunivs =
                      (uu___871_5836.FStar_Syntax_Syntax.lbunivs);
                    FStar_Syntax_Syntax.lbtyp =
                      (uu___871_5836.FStar_Syntax_Syntax.lbtyp);
                    FStar_Syntax_Syntax.lbeff =
                      (uu___871_5836.FStar_Syntax_Syntax.lbeff);
                    FStar_Syntax_Syntax.lbdef = lbdef;
                    FStar_Syntax_Syntax.lbattrs =
                      (uu___871_5836.FStar_Syntax_Syntax.lbattrs);
                    FStar_Syntax_Syntax.lbpos =
                      (uu___871_5836.FStar_Syntax_Syntax.lbpos)
                  }  in
                let lbs1 =
                  let uu____5845 =
                    match post_tau with
                    ...
                    | FStar_Pervasives_Native.None  ->
                        FStar_Pervasives_Native.snd lbs
                     in
                  ((FStar_Pervasives_Native.fst lbs), uu____5845)  in
                let uu____5863 =
                  let uu____5870 =
                    FStar_Syntax_Syntax.mk
                      (FStar_Syntax_Syntax.Tm_let
                      ...
                      se.FStar_Syntax_Syntax.sigrng
                     in
                  FStar_Extraction_ML_Term.term_as_mlexpr g uu____5870  in
                (match uu____5863 with
                 | (ml_let,uu____5887,uu____5888) ->
                     (match ml_let.FStar_Extraction_ML_Syntax.expr with
                      | FStar_Extraction_ML_Syntax.MLE_Let
                          ((flavor,bindings),uu____5897) ->
                          ...
                          let uu____5914 =
                            FStar_List.fold_left2
                              (fun uu____5940  ->
                                 fun ml_lb  ->
                                   fun uu____5942  ->
                                     match (uu____5940, uu____5942) with
                                     | ((env,ml_lbs),{
                                                       FStar_Syntax_Syntax.lbname
                                                         = lbname;
                                                       FStar_Syntax_Syntax.lbunivs
                                         ...
                                         then (env, ml_lbs)
                                           ...
                                           (let lb_lid =
                                              let uu____6011 =
                                                ...
                                                uu____6014.FStar_Syntax_Syntax.fv_name
                                                 in
                                              uu____6011.FStar_Syntax_Syntax.v
                                               in
                                            let flags'' =
                                              let uu____6018 =
                                                let uu____6019 =
                                                  FStar_Syntax_Subst.compress
                                                    t
                                                   in
                                                uu____6019.FStar_Syntax_Syntax.n
                                                 in
                                            ...
                                            let meta =
                                              FStar_List.append flags
                                               ...
                                               in
                                            let ml_lb1 =
                                              let uu___919_6075 = ml_lb  in
                                              {
                                                FStar_Extraction_ML_Syntax.mllb_name
                                                  =
                                                  (uu___919_6075.FStar_Extraction_ML_Syntax.mllb_name);
                                                FStar_Extraction_ML_Syntax.mllb_tysc
                                                  =
                                                  (uu___919_6075.FStar_Extraction_ML_Syntax.mllb_tysc);
                                                FStar_Extraction_ML_Syntax.mllb_add_unit
                                                  =
                                                  (uu___919_6075.FStar_Extraction_ML_Syntax.mllb_add_unit);
                                                FStar_Extraction_ML_Syntax.mllb_def
                                                  =
                                                  (uu___919_6075.FStar_Extraction_ML_Syntax.mllb_def);
                                                FStar_Extraction_ML_Syntax.mllb_meta
                                                  = meta;
                                                FStar_Extraction_ML_Syntax.print_typ
                                                  =
                                                  (uu___919_6075.FStar_Extraction_ML_Syntax.print_typ)
                                              }  in
                                            let uu____6076 =
                                              let uu____6081 =
                                                FStar_All.pipe_right quals
                                                    ...
                                                    (env1,
                                                      (let uu___932_6142 =
                                                         ml_lb1  in
                                                       {
                                                         ...
                                                         FStar_Extraction_ML_Syntax.mllb_tysc
                                                           =
                                                           (uu___932_6142.FStar_Extraction_ML_Syntax.mllb_tysc);
                                                         FStar_Extraction_ML_Syntax.mllb_add_unit
                                                           =
                                                           (uu___932_6142.FStar_Extraction_ML_Syntax.mllb_add_unit);
                                                         FStar_Extraction_ML_Syntax.mllb_def
                                                           =
                                                           (uu___932_6142.FStar_Extraction_ML_Syntax.mllb_def);
                                                         FStar_Extraction_ML_Syntax.mllb_meta
                                                           =
                                                           (uu___932_6142.FStar_Extraction_ML_Syntax.mllb_meta);
                                                         FStar_Extraction_ML_Syntax.print_typ
                                                           =
                                                           (uu___932_6142.FStar_Extraction_ML_Syntax.print_typ)
                                                       }))
                                              else
                                                (let uu____6149 =
                                                   let uu____6157 =
                                                   ...
                                                   FStar_Extraction_ML_UEnv.extend_lb
                                                     env lbname t uu____6157
                                                     ml_lb1.FStar_Extraction_ML_Syntax.mllb_add_unit
                                                     false
                                                    in
                                                 match uu____6149 with
                                                 | (env1,uu____6164,uu____6165)
                                                     -> (env1, ml_lb1))
                                               in
                               ...
                               let uu____6195 =
                                 let uu____6198 =
                                   ...
                                   FStar_Extraction_ML_Syntax.MLM_Let
                                    ...
                                    in
                                 let uu____6205 = maybe_register_plugin g1 se
                                    in
                                 FStar_List.append uu____6198 uu____6205  in
                               (g1, uu____6195))
                      | uu____6210 ->
                          let uu____6211 =
                            let uu____6213 =
                            ...
                            FStar_Util.format1
                             ...
                             in
                          failwith uu____6211)))
       | FStar_Syntax_Syntax.Sig_declare_typ (lid,uu____6224,t) ->
           let quals = se.FStar_Syntax_Syntax.sigquals  in
           let uu____6229 =
             (FStar_All.pipe_right quals
                (FStar_List.contains FStar_Syntax_Syntax.Assumption))
               &&
               (let uu____6235 =
                  let uu____6237 = FStar_Extraction_ML_UEnv.tcenv_of_uenv g
                     in
                  FStar_TypeChecker_Util.must_erase_for_extraction uu____6237
                    t
                   in
           ...
           then
             let always_fail1 =
               let uu___952_6246 = se  in
               let uu____6247 =
                 let uu____6248 =
                   let uu____6255 =
                     let uu____6256 =
                       let uu____6259 = always_fail lid t  in [uu____6259]
                        in
                     (false, uu____6256)  in
                   (uu____6255, [])  in
                 FStar_Syntax_Syntax.Sig_let uu____6248  in
               {
                 FStar_Syntax_Syntax.sigel = uu____6247;
                 FStar_Syntax_Syntax.sigrng =
                   (uu___952_6246.FStar_Syntax_Syntax.sigrng);
                 FStar_Syntax_Syntax.sigquals =
                   (uu___952_6246.FStar_Syntax_Syntax.sigquals);
                 FStar_Syntax_Syntax.sigmeta =
                   (uu___952_6246.FStar_Syntax_Syntax.sigmeta);
                 FStar_Syntax_Syntax.sigattrs =
                   (uu___952_6246.FStar_Syntax_Syntax.sigattrs);
                 FStar_Syntax_Syntax.sigopts =
                   (uu___952_6246.FStar_Syntax_Syntax.sigopts)
               }  in
             let uu____6266 = extract_sig g always_fail1  in
             (match uu____6266 with
              | (g1,mlm) ->
                  let uu____6285 =
                    FStar_Util.find_map quals
                      (fun uu___9_6290  ->
                         match uu___9_6290 with
                         ...
                         let uu____6305 =
                           let uu____6306 =
                             FStar_Extraction_ML_Util.mlloc_of_range
                               se.FStar_Syntax_Syntax.sigrng
                              in
                           FStar_Extraction_ML_Syntax.MLM_Loc uu____6306  in
                         let uu____6307 =
                           let uu____6310 =
                             FStar_Extraction_ML_Term.ind_discriminator_body
                              ...
                              in
                           [uu____6310]  in
                         uu____6305 :: uu____6307  in
                       (g1, uu____6302)
                   | uu____6313 ->
                       let uu____6316 =
                         FStar_Util.find_map quals
                           (fun uu___10_6322  ->
                              match uu___10_6322 with
                              | FStar_Syntax_Syntax.Projector (l,uu____6326)
                                  -> FStar_Pervasives_Native.Some l
                              | uu____6327 -> FStar_Pervasives_Native.None)
                          in
                       (match uu____6316 with
                        | FStar_Pervasives_Native.Some uu____6334 -> (g1, [])
                        | uu____6337 -> (g1, mlm))))
           else (g, [])
       | FStar_Syntax_Syntax.Sig_main e ->
           let uu____6347 = FStar_Extraction_ML_Term.term_as_mlexpr g e  in
           (match uu____6347 with
            | (ml_main,uu____6361,uu____6362) ->
                let uu____6363 =
                  let uu____6366 =
                    let uu____6367 =
                      FStar_Extraction_ML_Util.mlloc_of_range
                        se.FStar_Syntax_Syntax.sigrng
                       in
                    FStar_Extraction_ML_Syntax.MLM_Loc uu____6367  in
                  [uu____6366; FStar_Extraction_ML_Syntax.MLM_Top ml_main]
                   in
                (g, uu____6363))
       | FStar_Syntax_Syntax.Sig_assume uu____6370 -> (g, [])
       | FStar_Syntax_Syntax.Sig_sub_effect uu____6379 -> (g, [])
       | FStar_Syntax_Syntax.Sig_effect_abbrev uu____6382 -> (g, [])
       | FStar_Syntax_Syntax.Sig_polymonadic_bind uu____6397 -> (g, [])
       | FStar_Syntax_Syntax.Sig_pragma p ->
           (FStar_Syntax_Util.process_pragma p se.FStar_Syntax_Syntax.sigrng;
            (g, [])))
...
let (extract' :
  ...
  fun g  ->
    fun m  ->
      let uu____6437 = FStar_Options.restore_cmd_line_options true  in
      let name =
        FStar_Extraction_ML_Syntax.mlpath_of_lident
      ...
      let g1 =
        let uu____6441 =
          let uu____6442 = FStar_Extraction_ML_UEnv.tcenv_of_uenv g  in
          FStar_TypeChecker_Env.set_current_module uu____6442
            m.FStar_Syntax_Syntax.name
           in
        FStar_Extraction_ML_UEnv.set_tcenv g uu____6441  in
          ...
          (fun g3  ->
             fun se  ->
               ...
               then
                 let nm =
                   let uu____6474 =
                     let uu____6478 = FStar_Syntax_Util.lids_of_sigelt se  in
                     FStar_All.pipe_right uu____6478
                       (FStar_List.map FStar_Ident.string_of_lid)
                      in
                   FStar_All.pipe_right uu____6474 (FStar_String.concat ", ")
                    in
                 (FStar_Util.print1 "+++About to extract {%s}\n" nm;
                  (let uu____6494 = FStar_Util.format1 "---Extracted {%s}" nm
                      in
                   FStar_Util.measure_execution_time uu____6494
                     (fun uu____6504  -> extract_sig g3 se)))
      ...
      | (g3,sigs) ->
          ...
          let is_kremlin =
            let uu____6526 = FStar_Options.codegen ()  in
            uu____6526 = (FStar_Pervasives_Native.Some FStar_Options.Kremlin)
             in
          if
          ...
          then
            ((let uu____6541 =
                let uu____6543 = FStar_Options.silent ()  in
                Prims.op_Negation uu____6543  in
              if uu____6541
              then
                let uu____6546 =
...
let (extract :
  ...
  fun g  ->
    fun m  ->
      (let uu____6621 = FStar_Options.restore_cmd_line_options true  in
       FStar_All.pipe_left (fun uu____6623  -> ()) uu____6621);
      (let uu____6625 =
         let uu____6627 =
           FStar_Options.should_extract
             (m.FStar_Syntax_Syntax.name).FStar_Ident.str
            in
         Prims.op_Negation uu____6627  in
       if uu____6625
       then
         let uu____6630 =
           let uu____6632 =
      ...
      (let uu____6637 = FStar_Options.interactive ()  in
       if uu____6637
       then (g, FStar_Pervasives_Native.None)
       else
         (let res =
            let uu____6657 = FStar_Options.debug_any ()  in
            if uu____6657
            then
              let msg =
                let uu____6668 =
              ...
              FStar_Util.measure_execution_time msg
                (fun uu____6678  -> extract' g m)
            else extract' g m  in
          (let uu____6682 = FStar_Options.restore_cmd_line_options true  in
           FStar_All.pipe_left (fun uu____6684  -> ()) uu____6682);
          res))
