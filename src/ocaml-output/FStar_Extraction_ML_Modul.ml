...
let (extract_sigelt_iface :
  ...
  fun g  ->
    fun se  ->
      ...
      | FStar_Syntax_Syntax.Sig_assume uu____3700 -> (g, empty_iface)
      | FStar_Syntax_Syntax.Sig_sub_effect uu____3707 -> (g, empty_iface)
      | FStar_Syntax_Syntax.Sig_effect_abbrev uu____3708 -> (g, empty_iface)
      | FStar_Syntax_Syntax.Sig_pragma p ->
          (FStar_Syntax_Util.process_pragma p se.FStar_Syntax_Syntax.sigrng;
           (g, empty_iface))
      | FStar_Syntax_Syntax.Sig_splice uu____3723 ->
      ...
      | FStar_Syntax_Syntax.Sig_new_effect ed ->
          ...
          if uu____3736
          then
            let uu____3749 = extract_reifiable_effect g ed  in
            (match uu____3749 with | (env,iface1,uu____3764) -> (env, iface1))
          else (g, empty_iface)
...
let (extract_iface' :
  ...
  fun g  ->
    fun modul  ->
      let uu____3786 = FStar_Options.interactive ()  in
      if uu____3786
      then (g, empty_iface)
      else
        (let uu____3795 = FStar_Options.restore_cmd_line_options true  in
         let decls = modul.FStar_Syntax_Syntax.declarations  in
         let iface1 =
           let uu___607_3799 = empty_iface  in
           {
             iface_module_name = (g.FStar_Extraction_ML_UEnv.currentModule);
             iface_bindings = (uu___607_3799.iface_bindings);
             iface_tydefs = (uu___607_3799.iface_tydefs);
             iface_type_names = (uu___607_3799.iface_type_names)
           }  in
         let res =
           FStar_List.fold_left
             (fun uu____3817  ->
                fun se  ->
                  ...
                  | (g1,iface2) ->
                      let uu____3829 = extract_sigelt_iface g1 se  in
                      (match uu____3829 with
                       | (g2,iface') ->
                           let uu____3840 = iface_union iface2 iface'  in
                           (g2, uu____3840))) (g, iface1) decls
            in
         (let uu____3842 = FStar_Options.restore_cmd_line_options true  in
          FStar_All.pipe_left (fun a1  -> ()) uu____3842);
         res)
...
let (extract_iface :
  ...
  fun g  ->
    fun modul  ->
      let uu____3859 = FStar_Options.debug_any ()  in
      if uu____3859
      then
        let uu____3866 =
          FStar_Util.format1 "Extracted interface of %s"
           ...
           in
        FStar_Util.measure_execution_time uu____3866
          (fun uu____3874  -> extract_iface' g modul)
...
let (extend_with_iface :
  ...
  fun g  ->
    fun iface1  ->
      let uu___625_3888 = g  in
      let uu____3889 =
        let uu____3892 =
          FStar_List.map (fun _3899  -> FStar_Extraction_ML_UEnv.Fv _3899)
            iface1.iface_bindings
           in
        FStar_List.append uu____3892 g.FStar_Extraction_ML_UEnv.env_bindings
         in
      {
        FStar_Extraction_ML_UEnv.env_tcenv =
          (uu___625_3888.FStar_Extraction_ML_UEnv.env_tcenv);
        FStar_Extraction_ML_UEnv.env_bindings = uu____3889;
        ...
        FStar_Extraction_ML_UEnv.currentModule =
          (uu___625_3888.FStar_Extraction_ML_UEnv.currentModule)
      }
  ...
  fun env  ->
    fun se  ->
      let extract_ctor env_iparams ml_tyvars env1 ctor =
        let mlt =
          let uu____3977 =
            FStar_Extraction_ML_Term.term_as_mlty env_iparams ctor.dtyp  in
          FStar_Extraction_ML_Util.eraseTypeDeep
            (FStar_Extraction_ML_Util.udelta_unfold env_iparams) uu____3977
           in
        let steps =
            ...
            let uu____3986 =
              let uu____3989 =
                FStar_TypeChecker_Normalize.normalize steps
                 ...
                 in
              FStar_Syntax_Subst.compress uu____3989  in
            uu____3986.FStar_Syntax_Syntax.n  in
          match uu____3985 with
          | FStar_Syntax_Syntax.Tm_arrow (bs,uu____3994) ->
              FStar_List.map
                (fun uu____4027  ->
                   match uu____4027 with
                   | ({ FStar_Syntax_Syntax.ppname = ppname;
                        FStar_Syntax_Syntax.index = uu____4036;
                        FStar_Syntax_Syntax.sort = uu____4037;_},uu____4038)
        ...
        | (env2,uu____4081,uu____4082) ->
            let uu____4085 =
              let uu____4098 = lident_as_mlsymbol ctor.dname  in
              let uu____4100 =
                let uu____4108 = FStar_Extraction_ML_Util.argTypes mlt  in
                FStar_List.zip names1 uu____4108  in
              (uu____4098, uu____4100)  in
            (env2, uu____4085)
         in
      let extract_one_family env1 ind =
        let uu____4169 = binders_as_mlty_binders env1 ind.iparams  in
        ...
        | (env_iparams,vars) ->
            ...
            (match uu____4213 with
             | (env2,ctors) ->
                 let uu____4320 = FStar_Syntax_Util.arrow_formals ind.ityp
                    in
                            ...
                            (FStar_List.mapi
                               (fun i  ->
                                  fun uu____4413  ->
                                    ...
                                    Prims.op_Hat "'dummyV" uu____4421))
                           in
                        FStar_List.append vars uu____4387  in
                      let tbody =
                        let uu____4426 =
                          FStar_Util.find_opt
                            (fun uu___7_4431  ->
                               match uu___7_4431 with
                               | FStar_Syntax_Syntax.RecordType uu____4433 ->
                                   true
                               | uu____4443 -> false) ind.iquals
                           in
                        match uu____4426 with
                        | FStar_Pervasives_Native.Some
                            (FStar_Syntax_Syntax.RecordType (ns,ids)) ->
                            let uu____4455 = FStar_List.hd ctors  in
                        ...
                        | uu____4543 ->
                         ...
                         in
                      let uu____4546 =
                        let uu____4569 = lident_as_mlsymbol ind.iname  in
                        (false, uu____4569, FStar_Pervasives_Native.None,
      ...
      | (FStar_Syntax_Syntax.Sig_bundle
         ({
            FStar_Syntax_Syntax.sigel = FStar_Syntax_Syntax.Sig_datacon
              (l,uu____4614,t,uu____4616,uu____4617,uu____4618);
            FStar_Syntax_Syntax.sigrng = uu____4619;
            FStar_Syntax_Syntax.sigquals = uu____4620;
            FStar_Syntax_Syntax.sigmeta = uu____4621;
            FStar_Syntax_Syntax.sigattrs = uu____4622;_}::[],uu____4623),(FStar_Syntax_Syntax.ExceptionConstructor
         )::[]) ->
          let uu____4642 = extract_ctor env [] env { dname = l; dtyp = t }
             in
          (match uu____4642 with
           | (env1,ctor) -> (env1, [FStar_Extraction_ML_Syntax.MLM_Exn ctor]))
      | (FStar_Syntax_Syntax.Sig_bundle (ses,uu____4695),quals) ->
          let uu____4709 =
            bundle_as_inductive_families env ses quals
              se.FStar_Syntax_Syntax.sigattrs
             in
      ...
      | uu____4837 -> failwith "Unexpected signature element"
    ...
    fun se  ->
      ...
      let plugin_with_arity attrs =
        FStar_Util.find_map attrs
          (fun t  ->
             let uu____4895 = FStar_Syntax_Util.head_and_args t  in
                 ...
                 then FStar_Pervasives_Native.None
                   ...
                   (match args with
                    | ({
                         FStar_Syntax_Syntax.n =
                           FStar_Syntax_Syntax.Tm_constant
                           (FStar_Const.Const_int (s,uu____4964));
                         FStar_Syntax_Syntax.pos = uu____4965;
                         FStar_Syntax_Syntax.vars = uu____4966;_},uu____4967)::[]
                        ->
                        let uu____5006 =
                          let uu____5010 = FStar_Util.int_of_string s  in
                          FStar_Pervasives_Native.Some uu____5010  in
         ...
         in
      let uu____5031 =
        let uu____5033 = FStar_Options.codegen ()  in
        uu____5033 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)  in
      if uu____5031
      then []
      else
        (let uu____5043 = plugin_with_arity se.FStar_Syntax_Syntax.sigattrs
            in
             ...
             (match se.FStar_Syntax_Syntax.sigel with
              | FStar_Syntax_Syntax.Sig_let (lbs,lids) ->
                  let mk_registration lb =
                    ...
                    let ml_name_str =
                      let uu____5085 =
                        let uu____5086 = FStar_Ident.string_of_lid fv_lid1
                           in
                        FStar_Extraction_ML_Syntax.MLC_String uu____5086  in
                      FStar_Extraction_ML_Syntax.MLE_Const uu____5085  in
                    let uu____5088 =
  ...
  fun g  ->
    fun se  ->
      FStar_Extraction_ML_UEnv.debug g
        (fun u  ->
           let uu____5232 = FStar_Syntax_Print.sigelt_to_string se  in
           FStar_Util.print1 ">>>> extract_sig %s \n" uu____5232);
      (match se.FStar_Syntax_Syntax.sigel with
       | FStar_Syntax_Syntax.Sig_bundle uu____5241 -> extract_bundle g se
       | FStar_Syntax_Syntax.Sig_inductive_typ uu____5250 ->
           extract_bundle g se
       | FStar_Syntax_Syntax.Sig_datacon uu____5267 -> extract_bundle g se
       | FStar_Syntax_Syntax.Sig_new_effect ed when
           FStar_TypeChecker_Env.is_reifiable_effect
             g.FStar_Extraction_ML_UEnv.env_tcenv
             ed.FStar_Syntax_Syntax.mname
           ->
           let uu____5284 = extract_reifiable_effect g ed  in
           (match uu____5284 with | (env,_iface,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_splice uu____5308 ->
           failwith "impossible: trying to extract splice"
       | FStar_Syntax_Syntax.Sig_new_effect uu____5322 -> (g, [])
       | FStar_Syntax_Syntax.Sig_declare_typ (lid,univs1,t) when
           FStar_Extraction_ML_Term.is_arity g t ->
           let uu____5328 =
             extract_type_declaration g lid se.FStar_Syntax_Syntax.sigquals
               se.FStar_Syntax_Syntax.sigattrs univs1 t
              in
           (match uu____5328 with | (env,uu____5344,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let ((false ,lb::[]),uu____5353) when
           FStar_Extraction_ML_Term.is_arity g lb.FStar_Syntax_Syntax.lbtyp
           ->
           let uu____5362 =
             extract_typ_abbrev g se.FStar_Syntax_Syntax.sigquals
               se.FStar_Syntax_Syntax.sigattrs lb
              in
           (match uu____5362 with | (env,uu____5378,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let ((true ,lbs),uu____5387) when
           FStar_Util.for_some
             (fun lb  ->
                FStar_Extraction_ML_Term.is_arity g
                  lb.FStar_Syntax_Syntax.lbtyp) lbs
           ->
           let uu____5400 = extract_let_rec_types se g lbs  in
           (match uu____5400 with | (env,uu____5416,impl) -> (env, impl))
       | FStar_Syntax_Syntax.Sig_let (lbs,uu____5425) ->
           ...
           let uu____5436 =
             ...
             | FStar_Pervasives_Native.Some
                 (ats,(tau,FStar_Pervasives_Native.None )::uu____5474) ->
                ...
                let postprocess_lb tau lb =
                  let lbdef =
                    FStar_TypeChecker_Env.postprocess
                     ...
                     in
                  let uu___855_5560 = lb  in
                  {
                    FStar_Syntax_Syntax.lbname =
                      (uu___855_5560.FStar_Syntax_Syntax.lbname);
                    FStar_Syntax_Syntax.lbunivs =
                      (uu___855_5560.FStar_Syntax_Syntax.lbunivs);
                    FStar_Syntax_Syntax.lbtyp =
                      (uu___855_5560.FStar_Syntax_Syntax.lbtyp);
                    FStar_Syntax_Syntax.lbeff =
                      (uu___855_5560.FStar_Syntax_Syntax.lbeff);
                    FStar_Syntax_Syntax.lbdef = lbdef;
                    FStar_Syntax_Syntax.lbattrs =
                      (uu___855_5560.FStar_Syntax_Syntax.lbattrs);
                    FStar_Syntax_Syntax.lbpos =
                      (uu___855_5560.FStar_Syntax_Syntax.lbpos)
                  }  in
                let lbs1 =
                  let uu____5569 =
                    match post_tau with
                    ...
                    | FStar_Pervasives_Native.None  ->
                        FStar_Pervasives_Native.snd lbs
                     in
                  ((FStar_Pervasives_Native.fst lbs), uu____5569)  in
                let uu____5587 =
                  let uu____5594 =
                    FStar_Syntax_Syntax.mk
                      (FStar_Syntax_Syntax.Tm_let
                      ...
                      se.FStar_Syntax_Syntax.sigrng
                     in
                  FStar_Extraction_ML_Term.term_as_mlexpr g uu____5594  in
                (match uu____5587 with
                 | (ml_let,uu____5611,uu____5612) ->
                     (match ml_let.FStar_Extraction_ML_Syntax.expr with
                      | FStar_Extraction_ML_Syntax.MLE_Let
                          ((flavor,bindings),uu____5621) ->
                          ...
                          let uu____5638 =
                            FStar_List.fold_left2
                              (fun uu____5664  ->
                                 fun ml_lb  ->
                                   fun uu____5666  ->
                                     match (uu____5664, uu____5666) with
                                     | ((env,ml_lbs),{
                                                       FStar_Syntax_Syntax.lbname
                                                         = lbname;
                                                       FStar_Syntax_Syntax.lbunivs
                                         ...
                                         then (env, ml_lbs)
                                           ...
                                           (let lb_lid =
                                              let uu____5735 =
                                                ...
                                                uu____5738.FStar_Syntax_Syntax.fv_name
                                                 in
                                              uu____5735.FStar_Syntax_Syntax.v
                                               in
                                            let flags'' =
                                              let uu____5742 =
                                                let uu____5743 =
                                                  FStar_Syntax_Subst.compress
                                                    t
                                                   in
                                                uu____5743.FStar_Syntax_Syntax.n
                                                 in
                                            ...
                                            let meta =
                                              FStar_List.append flags
                                               ...
                                               in
                                            let ml_lb1 =
                                              let uu___903_5799 = ml_lb  in
                                              {
                                                FStar_Extraction_ML_Syntax.mllb_name
                                                  =
                                                  (uu___903_5799.FStar_Extraction_ML_Syntax.mllb_name);
                                                FStar_Extraction_ML_Syntax.mllb_tysc
                                                  =
                                                  (uu___903_5799.FStar_Extraction_ML_Syntax.mllb_tysc);
                                                FStar_Extraction_ML_Syntax.mllb_add_unit
                                                  =
                                                  (uu___903_5799.FStar_Extraction_ML_Syntax.mllb_add_unit);
                                                FStar_Extraction_ML_Syntax.mllb_def
                                                  =
                                                  (uu___903_5799.FStar_Extraction_ML_Syntax.mllb_def);
                                                FStar_Extraction_ML_Syntax.mllb_meta
                                                  = meta;
                                                FStar_Extraction_ML_Syntax.print_typ
                                                  =
                                                  (uu___903_5799.FStar_Extraction_ML_Syntax.print_typ)
                                              }  in
                                            let uu____5800 =
                                              let uu____5805 =
                                                FStar_All.pipe_right quals
                                              ...
                                              then
                                                ...
                                                let uu____5845 =
                                                  ...
                                                  FStar_Extraction_ML_UEnv.extend_fv'
                                                    env uu____5853 mname
                                                    uu____5854
                                                    ml_lb1.FStar_Extraction_ML_Syntax.mllb_add_unit
                                                    false
                                                   in
                                                match uu____5845 with
                                                | (env1,uu____5861,uu____5862)
                                                    ->
                                                    (env1,
                                                      (let uu___916_5866 =
                                                         ml_lb1  in
                                                       {
                                                         ...
                                                         FStar_Extraction_ML_Syntax.mllb_tysc
                                                           =
                                                           (uu___916_5866.FStar_Extraction_ML_Syntax.mllb_tysc);
                                                         FStar_Extraction_ML_Syntax.mllb_add_unit
                                                           =
                                                           (uu___916_5866.FStar_Extraction_ML_Syntax.mllb_add_unit);
                                                         FStar_Extraction_ML_Syntax.mllb_def
                                                           =
                                                           (uu___916_5866.FStar_Extraction_ML_Syntax.mllb_def);
                                                         FStar_Extraction_ML_Syntax.mllb_meta
                                                           =
                                                           (uu___916_5866.FStar_Extraction_ML_Syntax.mllb_meta);
                                                         FStar_Extraction_ML_Syntax.print_typ
                                                           =
                                                           (uu___916_5866.FStar_Extraction_ML_Syntax.print_typ)
                                                       }))
                                              else
                                                (let uu____5873 =
                                                   let uu____5881 =
                                                   ...
                                                   FStar_Extraction_ML_UEnv.extend_lb
                                                     env lbname t uu____5881
                                                     ml_lb1.FStar_Extraction_ML_Syntax.mllb_add_unit
                                                     false
                                                    in
                                                 match uu____5873 with
                                                 | (env1,uu____5888,uu____5889)
                                                     -> (env1, ml_lb1))
                                               in
                               ...
                               let uu____5919 =
                                 let uu____5922 =
                                   ...
                                   FStar_Extraction_ML_Syntax.MLM_Let
                                    ...
                                    in
                                 let uu____5929 = maybe_register_plugin g1 se
                                    in
                      ...
                      | uu____5934 ->
                          let uu____5935 =
                            ...
                            FStar_Util.format1
                             ...
                             in
                          failwith uu____5935)))
       | FStar_Syntax_Syntax.Sig_declare_typ (lid,uu____5947,t) ->
           let quals = se.FStar_Syntax_Syntax.sigquals  in
           let uu____5952 =
             (FStar_All.pipe_right quals
                (FStar_List.contains FStar_Syntax_Syntax.Assumption))
               ...
               (let uu____5958 =
                  FStar_TypeChecker_Util.must_erase_for_extraction
                    g.FStar_Extraction_ML_UEnv.env_tcenv t
                   in
                Prims.op_Negation uu____5958)
              in
           if uu____5952
           then
             let always_fail1 =
               let uu___936_5968 = se  in
               let uu____5969 =
                 let uu____5970 =
                   let uu____5977 =
                     let uu____5978 =
                       let uu____5981 = always_fail lid t  in [uu____5981]
                        in
                     (false, uu____5978)  in
                   (uu____5977, [])  in
                 FStar_Syntax_Syntax.Sig_let uu____5970  in
               {
                 FStar_Syntax_Syntax.sigel = uu____5969;
                 FStar_Syntax_Syntax.sigrng =
                   (uu___936_5968.FStar_Syntax_Syntax.sigrng);
                 FStar_Syntax_Syntax.sigquals =
                   (uu___936_5968.FStar_Syntax_Syntax.sigquals);
                 FStar_Syntax_Syntax.sigmeta =
                   (uu___936_5968.FStar_Syntax_Syntax.sigmeta);
                 FStar_Syntax_Syntax.sigattrs =
                   (uu___936_5968.FStar_Syntax_Syntax.sigattrs)
               }  in
             let uu____5988 = extract_sig g always_fail1  in
             (match uu____5988 with
              | (g1,mlm) ->
                  let uu____6007 =
                    FStar_Util.find_map quals
                      (fun uu___9_6012  ->
                         match uu___9_6012 with
                  ...
                  (match uu____6007 with
                         ...
                         let uu____6027 =
                           let uu____6028 =
                             FStar_Extraction_ML_Util.mlloc_of_range
                               se.FStar_Syntax_Syntax.sigrng
                              in
                           FStar_Extraction_ML_Syntax.MLM_Loc uu____6028  in
                         let uu____6029 =
                           let uu____6032 =
                   ...
                   | uu____6035 ->
                       let uu____6038 =
                         FStar_Util.find_map quals
                           (fun uu___10_6044  ->
                              match uu___10_6044 with
                              | FStar_Syntax_Syntax.Projector (l,uu____6048)
                                  -> FStar_Pervasives_Native.Some l
                              | uu____6049 -> FStar_Pervasives_Native.None)
                          in
                       (match uu____6038 with
                        | FStar_Pervasives_Native.Some uu____6056 -> (g1, [])
                        | uu____6059 -> (g1, mlm))))
           else (g, [])
       | FStar_Syntax_Syntax.Sig_main e ->
           let uu____6069 = FStar_Extraction_ML_Term.term_as_mlexpr g e  in
           (match uu____6069 with
            | (ml_main,uu____6083,uu____6084) ->
                let uu____6085 =
                  let uu____6088 =
                    let uu____6089 =
                      FStar_Extraction_ML_Util.mlloc_of_range
                        se.FStar_Syntax_Syntax.sigrng
                       in
                    FStar_Extraction_ML_Syntax.MLM_Loc uu____6089  in
                  [uu____6088; FStar_Extraction_ML_Syntax.MLM_Top ml_main]
                   in
                (g, uu____6085))
       | FStar_Syntax_Syntax.Sig_assume uu____6092 -> (g, [])
       | FStar_Syntax_Syntax.Sig_sub_effect uu____6101 -> (g, [])
       | FStar_Syntax_Syntax.Sig_effect_abbrev uu____6104 -> (g, [])
       | FStar_Syntax_Syntax.Sig_pragma p ->
           (FStar_Syntax_Util.process_pragma p se.FStar_Syntax_Syntax.sigrng;
            (g, [])))
...
let (extract' :
  ...
  fun g  ->
    fun m  ->
      let uu____6146 = FStar_Options.restore_cmd_line_options true  in
      let name =
        FStar_Extraction_ML_Syntax.mlpath_of_lident
          m.FStar_Syntax_Syntax.name
         in
      let g1 =
        let uu___977_6150 = g  in
        let uu____6151 =
          FStar_TypeChecker_Env.set_current_module
            g.FStar_Extraction_ML_UEnv.env_tcenv m.FStar_Syntax_Syntax.name
           in
        {
          FStar_Extraction_ML_UEnv.env_tcenv = uu____6151;
          FStar_Extraction_ML_UEnv.env_bindings =
            (uu___977_6150.FStar_Extraction_ML_UEnv.env_bindings);
          FStar_Extraction_ML_UEnv.tydefs =
            (uu___977_6150.FStar_Extraction_ML_UEnv.tydefs);
          FStar_Extraction_ML_UEnv.type_names =
            (uu___977_6150.FStar_Extraction_ML_UEnv.type_names);
                 ...
                 let nm =
                   let uu____6182 =
                     FStar_All.pipe_right
                      ...
                      in
                   FStar_All.pipe_right uu____6182 (FStar_String.concat ", ")
                    in
                 (FStar_Util.print1 "+++About to extract {%s}\n" nm;
                  (let uu____6199 = FStar_Util.format1 "---Extracted {%s}" nm
                      in
                   FStar_Util.measure_execution_time uu____6199
                     (fun uu____6209  -> extract_sig g2 se)))
      ...
      | (g2,sigs) ->
          ...
          let is_kremlin =
            let uu____6231 = FStar_Options.codegen ()  in
            uu____6231 = (FStar_Pervasives_Native.Some FStar_Options.Kremlin)
             in
          if
          ...
          then
            ((let uu____6246 =
                let uu____6248 = FStar_Options.silent ()  in
...
let (extract :
  ...
  fun g  ->
    fun m  ->
      (let uu____6326 = FStar_Options.restore_cmd_line_options true  in
       FStar_All.pipe_left (fun a2  -> ()) uu____6326);
      (let uu____6329 =
         let uu____6331 =
      ...
      (let uu____6341 = FStar_Options.interactive ()  in
       if uu____6341
       then (g, FStar_Pervasives_Native.None)
       else
         (let res =
            let uu____6361 = FStar_Options.debug_any ()  in
            if uu____6361
            then
              let msg =
                let uu____6372 =
              ...
              FStar_Util.measure_execution_time msg
                (fun uu____6382  -> extract' g m)
            else extract' g m  in
          (let uu____6386 = FStar_Options.restore_cmd_line_options true  in
           FStar_All.pipe_left (fun a3  -> ()) uu____6386);
          res))
