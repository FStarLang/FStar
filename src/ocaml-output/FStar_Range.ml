...
let (set_use_range : range -> rng -> range) =
  fun r2  ->
    fun use_rng  ->
      ...
      then
        let uu___80_187 = r2  in
        { def_range = (uu___80_187.def_range); use_range = use_rng }
      else r2
...
let (set_def_range : range -> rng -> range) =
  fun r2  ->
    fun def_rng  ->
      ...
      then
        let uu___81_202 = r2  in
        { def_range = def_rng; use_range = (uu___81_202.use_range) }
      else r2
...
let (set_file_of_range : range -> Prims.string -> range) =
  fun r  ->
    fun f  ->
      let uu___84_379 = r  in
      {
        def_range =
          (let uu___85_382 = r.def_range  in
           {
             file_name = f;
             start_pos = (uu___85_382.start_pos);
             end_pos = (uu___85_382.end_pos)
           });
        use_range = (uu___84_379.use_range)
      }
...
let (end_of_line : pos -> pos) =
  fun p  ->
    let uu___86_542 = p  in
    { line = (uu___86_542.line); col = FStar_Util.max_int }
