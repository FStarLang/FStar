...
let (set_use_range : range -> rng -> range) =
  fun r2  ->
    fun use_rng  ->
      ...
      then
        let uu___81_187 = r2  in
        { def_range = (uu___81_187.def_range); use_range = use_rng }
      else r2
...
let (set_def_range : range -> rng -> range) =
  fun r2  ->
    fun def_rng  ->
      ...
      then
        let uu___82_202 = r2  in
        { def_range = def_rng; use_range = (uu___82_202.use_range) }
      else r2
...
let (set_file_of_range : range -> Prims.string -> range) =
  fun r  ->
    fun f  ->
      let uu___85_379 = r  in
      {
        def_range =
          (let uu___86_382 = r.def_range  in
           {
             file_name = f;
             start_pos = (uu___86_382.start_pos);
             end_pos = (uu___86_382.end_pos)
           });
        use_range = (uu___85_379.use_range)
      }
...
let (end_of_line : pos -> pos) =
  fun p  ->
    let uu___87_542 = p  in
    { line = (uu___87_542.line); col = FStar_Util.max_int }
...
let (prims_to_fstar_range :
  ((Prims.string,(Prims.int,Prims.int) FStar_Pervasives_Native_Tuple2.tuple2,
     (Prims.int,Prims.int) FStar_Pervasives_Native_Tuple2.tuple2)
     FStar_Pervasives_Native_Tuple3.tuple3,(Prims.string,(Prims.int,Prims.int)
                                                           FStar_Pervasives_Native_Tuple2.tuple2,
                                             (Prims.int,Prims.int)
                                               FStar_Pervasives_Native_Tuple2.tuple2)
                                             FStar_Pervasives_Native_Tuple3.tuple3)
    FStar_Pervasives_Native_Tuple2.tuple2 -> range)
  =
  fun r  ->
    let uu____644 = r  in
