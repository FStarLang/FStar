...
type pos = {line: Prims.int; col: Prims.int} [@@deriving show]

let __proj__Mkpos__item__line : pos -> Prims.int =
  fun projectee ->
    match projectee with {line= __fname__line; col= __fname__col; _} ->
      __fname__line
...
let max : Prims.int -> Prims.int -> Prims.int =
  fun i j -> if i < j then j else i


let pos_geq : pos -> pos -> Prims.bool =
  fun p1 p2 -> p1.line >= p2.line || p1.line = p2.line && p1.col >= p2.col
...
let __proj__Mkrng__item__file_name : rng -> file_name =
  fun projectee ->
    match projectee
    with
    | { file_name= __fname__file_name
      ; start_pos= __fname__start_pos
      ; end_pos= __fname__end_pos; _ }
    -> __fname__file_name
...
type range = {def_range: rng; use_range: rng} [@@deriving show]
...
let mk_pos : Prims.int -> Prims.int -> pos =
  fun l c ->
    {line= max (Prims.parse_int "0") l; col= max (Prims.parse_int "0") c}


let mk_rng : file_name -> pos -> pos -> rng =
  fun file_name start_pos end_pos -> {file_name; start_pos; end_pos}
...
let union_rng : rng -> rng -> rng =
  fun r1 r2 ->
    if r1.file_name <> r2.file_name then r2
      ...
      let start_pos =
        if pos_geq r1.start_pos r2.start_pos then r2.start_pos
        else r1.start_pos
      in
      let end_pos =
        if pos_geq r1.start_pos r2.start_pos then r1.start_pos
        else r2.start_pos
      in
...
let extend_to_end_of_line : range -> range =
  fun r ->
    ...
    let uu____236 =
      let uu____237 = end_of_range r in
      end_of_line uu____237
    in
...
let prims_to_fstar_range
  ...
  fun r ->
    let uu____305 = r in
    match uu____305 with r1, r2 ->
      let uu____396 = r1 in
      match uu____396 with f1, s1, e1 ->
        let uu____430 = r2 in
        match uu____430 with f2, s2, e2 ->
          let s11 =
            mk_pos
