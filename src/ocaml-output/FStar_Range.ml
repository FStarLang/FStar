...
let set_use_range: range -> rng -> range =
  fun r2  ->
    fun use_rng  ->
      ...
      then
        let uu___48_115 = r2 in
        { def_range = (uu___48_115.def_range); use_range = use_rng }
      else r2
let set_def_range: range -> rng -> range =
  fun r2  ->
    fun def_rng  ->
      ...
      then
        let uu___49_125 = r2 in
        { def_range = def_rng; use_range = (uu___49_125.use_range) }
      else r2
...
let end_of_line: pos -> pos =
  fun p  ->
    let uu___50_284 = p in
    { line = (uu___50_284.line); col = FStar_Util.max_int }
