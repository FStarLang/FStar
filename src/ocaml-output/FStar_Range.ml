...
let (set_use_range : range -> rng -> range) =
  fun r2 ->
    fun use_rng ->
      ...
      then
        let uu___28_139 = r2 in
        { def_range = (uu___28_139.def_range); use_range = use_rng }
      else r2
let (set_def_range : range -> rng -> range) =
  fun r2 ->
    fun def_rng ->
      ...
      then
        let uu___29_151 = r2 in
        { def_range = def_rng; use_range = (uu___29_151.use_range) }
      else r2
...
let (range_before_pos : range -> pos -> Prims.bool) =
  fun m1 -> fun p -> let uu____339 = end_of_range m1 in pos_geq p uu____339
let (end_of_line : pos -> pos) =
  fun p ->
    let uu___30_345 = p in
    { line = (uu___30_345.line); col = FStar_Util.max_int }
let (extend_to_end_of_line : range -> range) =
  fun r ->
    let uu____351 = file_of_range r in
    let uu____352 = start_of_range r in
    let uu____353 = let uu____354 = end_of_range r in end_of_line uu____354 in
