...
let set_use_range: range -> rng -> range =
  fun r2  ->
    fun use_rng  ->
      ...
      then
        let uu___40_98 = r2 in
        { def_range = (uu___40_98.def_range); use_range = use_rng }
      else r2
let set_def_range: range -> rng -> range =
  fun r2  ->
    fun def_rng  ->
      ...
      then
        let uu___41_106 = r2 in
        { def_range = def_rng; use_range = (uu___41_106.use_range) }
      else r2
...
let end_of_line: pos -> pos =
  fun p  ->
    let uu___42_230 = p in
    { line = (uu___42_230.line); col = FStar_Util.max_int }
