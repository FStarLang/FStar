...
let set_use_range: range -> rng -> range =
  fun r2  ->
    fun use_rng  ->
      ...
      then
        let uu___60_115 = r2 in
        { def_range = (uu___60_115.def_range); use_range = use_rng }
      else r2
let set_def_range: range -> rng -> range =
  fun r2  ->
    fun def_rng  ->
      ...
      then
        let uu___61_125 = r2 in
        { def_range = def_rng; use_range = (uu___61_125.use_range) }
      else r2
...
let end_of_line: pos -> pos =
  fun p  ->
    let uu___62_284 = p in
    { line = (uu___62_284.line); col = FStar_Util.max_int }
