              ...
              (let env1 =
               ...
               let env2 =
                 FStar_List.fold_left
                   ...
                   tvars
                  in
               let rec find_return_type eff i uu___287_5179 =
                 match uu___287_5179 with
  ...
  fun env  ->
    fun e  ->
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ({
             FStar_Extraction_ML_Syntax.expr =
               FStar_Extraction_ML_Syntax.MLE_Name
               ("FStar"::"Int"::"Cast"::[],c);
             FStar_Extraction_ML_Syntax.mlty = uu____7683;
             FStar_Extraction_ML_Syntax.loc = uu____7684;_},arg::[])
          ->
          let is_known_type =
          ...
          then
            let uu____7712 =
              let uu____7717 = translate_expr env arg  in
              (uu____7717, (TInt UInt64))  in
            ECast uu____7712
          else
            if (FStar_Util.ends_with c "uint32") && is_known_type
            then
              (let uu____7722 =
                 let uu____7727 = translate_expr env arg  in
                 (uu____7727, (TInt UInt32))  in
               ECast uu____7722)
            else
              if (FStar_Util.ends_with c "uint16") && is_known_type
              then
                (let uu____7732 =
                   let uu____7737 = translate_expr env arg  in
                   (uu____7737, (TInt UInt16))  in
                 ECast uu____7732)
              else
                if (FStar_Util.ends_with c "uint8") && is_known_type
                then
                  (let uu____7742 =
                     let uu____7747 = translate_expr env arg  in
                     (uu____7747, (TInt UInt8))  in
                   ECast uu____7742)
                else
                  if (FStar_Util.ends_with c "int64") && is_known_type
                  then
                    (let uu____7752 =
                       let uu____7757 = translate_expr env arg  in
                       (uu____7757, (TInt Int64))  in
                     ECast uu____7752)
                  else
                    if (FStar_Util.ends_with c "int32") && is_known_type
                    then
                      (let uu____7762 =
                         let uu____7767 = translate_expr env arg  in
                         (uu____7767, (TInt Int32))  in
                       ECast uu____7762)
                    else
                      if (FStar_Util.ends_with c "int16") && is_known_type
                      then
                        (let uu____7772 =
                           let uu____7777 = translate_expr env arg  in
                           (uu____7777, (TInt Int16))  in
                         ECast uu____7772)
                      else
                        if (FStar_Util.ends_with c "int8") && is_known_type
                        then
                          (let uu____7782 =
                             let uu____7787 = translate_expr env arg  in
                             (uu____7787, (TInt Int8))  in
                           ECast uu____7782)
                        else
                          (let uu____7790 =
                             let uu____7797 =
      ...
      | FStar_Extraction_ML_Syntax.MLE_App (head1,args) ->
          let uu____7820 =
            let uu____7827 = translate_expr env head1  in
            let uu____7828 = FStar_List.map (translate_expr env) args  in
            (uu____7827, uu____7828)  in
          EApp uu____7820
      | FStar_Extraction_ML_Syntax.MLE_TApp (head1,ty_args) ->
          let uu____7839 =
            let uu____7846 = translate_expr env head1  in
            let uu____7847 = FStar_List.map (translate_type env) ty_args  in
            (uu____7846, uu____7847)  in
          ETypApp uu____7839
      | FStar_Extraction_ML_Syntax.MLE_Coerce (e1,t_from,t_to) ->
          let uu____7855 =
            let uu____7860 = translate_expr env e1  in
            let uu____7861 = translate_type env t_to  in
            (uu____7860, uu____7861)  in
          ECast uu____7855
      | FStar_Extraction_ML_Syntax.MLE_Record (uu____7862,fields) ->
          let uu____7884 =
            let uu____7896 = assert_lid env e.FStar_Extraction_ML_Syntax.mlty
               in
            let uu____7897 =
              FStar_List.map
                (fun uu____7919  ->
                   ...
                   | (field,expr) ->
                       let uu____7934 = translate_expr env expr  in
                       (field, uu____7934)) fields
               in
      ...
      | FStar_Extraction_ML_Syntax.MLE_Proj (e1,path) ->
          let uu____7945 =
            ...
            let uu____7954 = translate_expr env e1  in
            (uu____7953, uu____7954, (FStar_Pervasives_Native.snd path))  in
          EField uu____7945
      | FStar_Extraction_ML_Syntax.MLE_Let uu____7960 ->
          failwith "todo: translate_expr [MLE_Let]"
      | FStar_Extraction_ML_Syntax.MLE_App (head1,uu____7973) ->
          let uu____7978 =
            let uu____7980 =
      ...
      | FStar_Extraction_ML_Syntax.MLE_Seq seqs ->
          let uu____7992 = FStar_List.map (translate_expr env) seqs  in
          ESequence uu____7992
      | FStar_Extraction_ML_Syntax.MLE_Tuple es ->
          let uu____7998 = FStar_List.map (translate_expr env) es  in
          ETuple uu____7998
      | FStar_Extraction_ML_Syntax.MLE_CTor ((uu____8001,cons1),es) ->
          let uu____8016 =
            let uu____8026 = assert_lid env e.FStar_Extraction_ML_Syntax.mlty
               in
            let uu____8027 = FStar_List.map (translate_expr env) es  in
            (uu____8026, cons1, uu____8027)  in
          ECons uu____8016
      | FStar_Extraction_ML_Syntax.MLE_Fun (args,body) ->
          let binders = translate_binders env args  in
          let env1 = add_binders env args  in
          let uu____8053 =
            let uu____8062 = translate_expr env1 body  in
            let uu____8063 =
              translate_type env1 body.FStar_Extraction_ML_Syntax.mlty  in
            (binders, uu____8062, uu____8063)  in
          EFun uu____8053
      | FStar_Extraction_ML_Syntax.MLE_If (e1,e2,e3) ->
          let uu____8073 =
            let uu____8080 = translate_expr env e1  in
            let uu____8081 = translate_expr env e2  in
            let uu____8082 =
              match e3 with
              | FStar_Pervasives_Native.None  -> EUnit
              | FStar_Pervasives_Native.Some e31 -> translate_expr env e31
               in
            (uu____8080, uu____8081, uu____8082)  in
          EIfThenElse uu____8073
      | FStar_Extraction_ML_Syntax.MLE_Raise uu____8084 ->
          failwith "todo: translate_expr [MLE_Raise]"
      | FStar_Extraction_ML_Syntax.MLE_Try uu____8092 ->
          failwith "todo: translate_expr [MLE_Try]"
      | FStar_Extraction_ML_Syntax.MLE_Coerce uu____8108 ->
  ...
  fun env  ->
    fun t  ->
      ...
      | FStar_Extraction_ML_Syntax.MLTY_Named (ts,lid) ->
          ...
          then
            let uu____8126 =
              let uu____8141 = FStar_List.map (translate_type env) ts  in
              (lid, uu____8141)  in
            TApp uu____8126
          else TQualified lid
      | uu____8156 -> failwith "invalid argument: assert_lid"
  ...
  fun env  ->
    fun uu____8183  ->
      ...
      | (pat,guard,expr) ->
          ...
          then
            let uu____8210 = translate_pat env pat  in
            (match uu____8210 with
             | (env1,pat1) ->
                 let uu____8221 = translate_expr env1 expr  in
                 (pat1, uu____8221))
  ...
  fun env  ->
    fun p  ->
      ...
      | FStar_Extraction_ML_Syntax.MLP_Const
          ...
          let uu____8296 =
            let uu____8297 =
              let uu____8303 = translate_width sw  in (uu____8303, s)  in
            PConstant uu____8297  in
          (env, uu____8296)
      ...
      | FStar_Extraction_ML_Syntax.MLP_CTor ((uu____8313,cons1),ps) ->
          let uu____8328 =
            FStar_List.fold_left
              (fun uu____8348  ->
                 fun p1  ->
                   ...
                   | (env1,acc) ->
                       let uu____8368 = translate_pat env1 p1  in
                       (match uu____8368 with
                        | (env2,p2) -> (env2, (p2 :: acc)))) (env, []) ps
             in
          (match uu____8328 with
           | (env1,ps1) -> (env1, (PCons (cons1, (FStar_List.rev ps1)))))
      | FStar_Extraction_ML_Syntax.MLP_Record (uu____8398,ps) ->
          let uu____8420 =
            FStar_List.fold_left
              (fun uu____8457  ->
                 fun uu____8458  ->
                   match (uu____8457, uu____8458) with
                   | ((env1,acc),(field,p1)) ->
                       let uu____8538 = translate_pat env1 p1  in
            ...
            FStar_List.fold_left
              (fun uu____8629  ->
                 fun p1  ->
                   ...
                   | (env1,acc) ->
                       let uu____8649 = translate_pat env1 p1  in
                       (match uu____8649 with
                        | (env2,p2) -> (env2, (p2 :: acc)))) (env, []) ps
             in
          (match uu____8609 with
           | (env1,ps1) -> (env1, (PTuple (FStar_List.rev ps1))))
      | FStar_Extraction_ML_Syntax.MLP_Const uu____8676 ->
          failwith "todo: translate_pat [MLP_Const]"
      | FStar_Extraction_ML_Syntax.MLP_Branch uu____8682 ->
...
and (translate_constant : FStar_Extraction_ML_Syntax.mlconstant -> expr) =
  fun c  ->
    ...
    | FStar_Extraction_ML_Syntax.MLC_String s ->
        ((let uu____8696 =
            let uu____8698 = FStar_String.list_of_string s  in
    ...
    | FStar_Extraction_ML_Syntax.MLC_Int
        (s,FStar_Pervasives_Native.Some uu____8740) ->
        failwith
          "impossible: machine integer not desugared to a function call"
    | FStar_Extraction_ML_Syntax.MLC_Float uu____8758 ->
        failwith "todo: translate_expr [MLC_Float]"
    | FStar_Extraction_ML_Syntax.MLC_Bytes uu____8760 ->
  ...
  fun env  ->
    fun w  ->
      fun op  ->
        fun args  ->
          let uu____8784 =
            let uu____8791 = FStar_List.map (translate_expr env) args  in
            ((EOp (op, w)), uu____8791)  in
          EApp uu____8784
