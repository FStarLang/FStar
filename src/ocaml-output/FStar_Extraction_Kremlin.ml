...
let extend : env -> Prims.string -> Prims.bool -> env =
  fun env  ->
    fun x  ->
      fun is_mut  ->
        let uu___200_2890 = env  in
        {
          names = ({ pretty = x; mut = is_mut } :: (env.names));
          names_t = (uu___200_2890.names_t);
          module_name = (uu___200_2890.module_name)
        }
  
let extend_t : env -> Prims.string -> env =
  fun env  ->
    fun x  ->
      let uu___201_2897 = env  in
      {
        names = (uu___201_2897.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___201_2897.module_name)
      }
        ...
        fun lb  ->
          ...
          | { FStar_Extraction_ML_Syntax.mllb_name = name;
              ...
              let assumed =
                FStar_Util.for_some
                  (fun uu___196_3356  ->
                     match uu___196_3356 with
                     | FStar_Extraction_ML_Syntax.Assumed  -> true
                     | uu____3357 -> false) flags
                 in
              let env1 =
              ...
              let env2 =
                FStar_List.fold_left
                 ...
                 in
              let rec find_return_type eff i uu___197_3378 =
                match uu___197_3378 with
          ...
          | { FStar_Extraction_ML_Syntax.mllb_name = name;
              ...
              let assumed =
                FStar_Util.for_some
                  (fun uu___196_3546  ->
                     match uu___196_3546 with
                     | FStar_Extraction_ML_Syntax.Assumed  -> true
                     | uu____3547 -> false) flags
                 in
              let env1 =
                if flavor = FStar_Extraction_ML_Syntax.Rec
                then extend env name false
              ...
              let env2 =
                FStar_List.fold_left
                  (fun env2  -> fun name1  -> extend_t env2 name1) env1 tvars
                 in
              let rec find_return_type eff i uu___197_3568 =
                match uu___197_3568 with
          ...
          then
            ...
            (let uu____3820 =
               let uu____3821 =
                 ...
                 (name1, uu____3838, (FStar_List.length args), uu____3841)
                  in
                         ...
                         let uu____4138 =
                           FStar_List.map
                             (fun uu____4165  ->
                                ...
                                | (name2,t) ->
                                    let uu____4180 =
                                      let uu____4185 = translate_type env1 t
                                         in
                                      (uu____4185, false)  in
                                    (name2, uu____4180)) ts
                            in
                         (cons1, uu____4138)) branches
                 in
  ...
  fun env  ->
    fun t  ->
      ...
      | FStar_Extraction_ML_Syntax.MLTY_Named
          ...
          (let uu____4298 = FStar_Extraction_ML_Syntax.string_of_mlpath p  in
           uu____4298 = "FStar.Monotonic.HyperStack.s_mref") ||
            (let uu____4300 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                in
      ...
      | FStar_Extraction_ML_Syntax.MLTY_Named (arg::uu____4303::[],p) when
          (((((let uu____4309 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                  in
              ...
              ||
              (let uu____4315 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                  in
               uu____4315 = "FStar.Monotonic.HyperStack.mmmstackref"))
             ||
             (let uu____4317 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                 in
              uu____4317 = "FStar.Monotonic.HyperStack.mmmref"))
            ||
            (let uu____4319 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                in
    ...
    fun e  ->
      ...
      | FStar_Extraction_ML_Syntax.MLE_Let
          ...
          let is_mut =
            FStar_Util.for_some
              (fun uu___198_4478  ->
                 match uu___198_4478 with
                 | FStar_Extraction_ML_Syntax.Mutable  -> true
                 | uu____4479 -> false) flags
             in
          let uu____4480 =
            if is_mut
            then
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ...
          (let uu____4627 = FStar_Extraction_ML_Syntax.string_of_mlpath p  in
           uu____4627 = "FStar.Buffer.index") ||
            (let uu____4629 = FStar_Extraction_ML_Syntax.string_of_mlpath p
                in
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ...
          let uu____5055 =
            let uu____5060 = FStar_Util.must (mk_width m)  in (uu____5060, c)
             in
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ...
          when is_machine_int m ->
          let uu____5081 =
            let uu____5086 = FStar_Util.must (mk_width m)  in (uu____5086, c)
             in
      ...
      | FStar_Extraction_ML_Syntax.MLE_Record (uu____5237,fields) ->
          let uu____5255 =
            let uu____5266 = assert_lid env e.FStar_Extraction_ML_Syntax.mlty
               in
            let uu____5267 =
              FStar_List.map
                (fun uu____5286  ->
      ...
      | FStar_Extraction_ML_Syntax.MLE_CTor ((uu____5352,cons1),es) ->
          let uu____5369 =
            let uu____5378 = assert_lid env e.FStar_Extraction_ML_Syntax.mlty
               in
