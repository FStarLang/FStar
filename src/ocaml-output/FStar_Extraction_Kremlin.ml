...
let mk_width : Prims.string  ->  width FStar_Pervasives_Native.option = (fun uu___148_2811 -> (match (uu___148_2811) with
| "UInt8" -> begin
...
let mk_bool_op : Prims.string  ->  op FStar_Pervasives_Native.option = (fun uu___149_2820 -> (match (uu___149_2820) with
| "op_Negation" -> begin
...
let mk_op : Prims.string  ->  op FStar_Pervasives_Native.option = (fun uu___150_2835 -> (match (uu___150_2835) with
| "add" -> begin
...
let uu___155_2960 = env
in {names = ({pretty = x; mut = is_mut})::env.names; names_t = uu___155_2960.names_t; module_name = uu___155_2960.module_name}))


let extend_t : env  ->  Prims.string  ->  env = (fun env x -> (

let uu___156_2969 = env
in {names = uu___156_2969.names; names_t = (x)::env.names_t; module_name = uu___156_2969.module_name}))
...
let find : env  ->  Prims.string  ->  Prims.int = (fun env x -> (FStar_All.try_with (fun uu___158_3002 -> (match (()) with
| () -> begin
(FStar_List.index (fun name -> (Prims.op_Equality name.pretty x)) env.names)
end)) (fun uu___157_3008 -> (match (uu___157_3008) with
| uu____3009 -> begin
(
...
let find_t : env  ->  Prims.string  ->  Prims.int = (fun env x -> (FStar_All.try_with (fun uu___160_3020 -> (match (()) with
| () -> begin
(FStar_List.index (fun name -> (Prims.op_Equality name x)) env.names_t)
end)) (fun uu___159_3026 -> (match (uu___159_3026) with
| uu____3027 -> begin
(
...
in (FStar_All.try_with (fun uu___162_3306 -> (match (()) with
| () -> begin
...
end)) (fun uu___161_3314 -> (match (uu___161_3314) with
| e -> begin
((
...
and translate_flags : FStar_Extraction_ML_Syntax.meta Prims.list  ->  flag Prims.list = (fun flags -> (FStar_List.choose (fun uu___151_3399 -> (match (uu___151_3399) with
...
let assumed = (FStar_Util.for_some (fun uu___152_3439 -> (match (uu___152_3439) with
...
let rec find_return_type = (fun i uu___153_3454 -> (match (uu___153_3454) with
...
| uu____3519 -> begin
(FStar_All.try_with (fun uu___164_3524 -> (match (()) with
| () -> begin
(
...
end)) (fun uu___163_3545 -> (match (uu___163_3545) with
| e -> begin
(
...
let assumed = (FStar_Util.for_some (fun uu___152_3601 -> (match (uu___152_3601) with
| FStar_Extraction_ML_Syntax.Assumed -> begin
true
end
| uu____3602 -> begin
...
let rec find_return_type = (fun i uu___153_3616 -> (match (uu___153_3616) with
...
| uu____3681 -> begin
(FStar_All.try_with (fun uu___164_3686 -> (match (()) with
| () -> begin
(

let body1 = (translate_expr env3 body)
in FStar_Pervasives_Native.Some (DFunction (((FStar_Pervasives_Native.None), (flags1), ((FStar_List.length tvars)), (t), (name1), (binders), (body1)))))
end)) (fun uu___163_3707 -> (match (uu___163_3707) with
| e -> begin
(
...
in (FStar_All.try_with (fun uu___166_3762 -> (match (()) with
| () -> begin
(

let expr1 = (translate_expr env expr)
in FStar_Pervasives_Native.Some (DGlobal (((flags1), (name1), (t1), (expr1)))))
end)) (fun uu___165_3777 -> (match (uu___165_3777) with
| e -> begin
((
...
let is_mut = (FStar_Util.for_some (fun uu___154_4608 -> (match (uu___154_4608) with
