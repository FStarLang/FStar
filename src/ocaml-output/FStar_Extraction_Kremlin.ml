...
let (extend : env -> Prims.string -> Prims.bool -> env) =
  fun env ->
    fun x ->
      fun is_mut ->
        let uu___45_2958 = env in
        {
          names = ({ pretty = x; mut = is_mut } :: (env.names));
          names_t = (uu___45_2958.names_t);
          module_name = (uu___45_2958.module_name)
        }
let (extend_t : env -> Prims.string -> env) =
  fun env ->
    fun x ->
      let uu___46_2965 = env in
      {
        names = (uu___46_2965.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___46_2965.module_name)
      }
    ...
    fun flavor ->
      fun lb ->
        ...
        | { FStar_Extraction_ML_Syntax.mllb_name = name;
            ...
            let rec find_return_type eff i uu___42_3446 =
              match uu___42_3446 with
        ...
        | { FStar_Extraction_ML_Syntax.mllb_name = name;
            ...
            let env2 =
              FStar_List.fold_left
                (fun env2 -> fun name1 -> extend_t env2 name1) env1 tvars in
            let rec find_return_type eff i uu___42_3643 =
              match uu___42_3643 with
