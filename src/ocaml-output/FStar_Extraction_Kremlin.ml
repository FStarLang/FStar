...
let (extend : env -> Prims.string -> env) =
  fun env  ->
    fun x  ->
      let uu___20_4356 = env  in
      {
        names = ({ pretty = x } :: (env.names));
        names_t = (uu___20_4356.names_t);
        module_name = (uu___20_4356.module_name)
      }
  
let (extend_t : env -> Prims.string -> env) =
  fun env  ->
    fun x  ->
      let uu___21_4370 = env  in
      {
        names = (uu___21_4370.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___21_4370.module_name)
      }
              ...
              (let env1 =
               ...
               let env2 =
                 FStar_List.fold_left
                   ...
                   tvars
                  in
               let rec find_return_type eff i uu___18_5179 =
                 match uu___18_5179 with
