...
let mk_width : Prims.string  ->  width Prims.option = (fun uu___122_1724 -> (match (uu___122_1724) with
| "UInt8" -> begin
Some (UInt8)
end
| "UInt16" -> begin
...
let mk_bool_op : Prims.string  ->  op Prims.option = (fun uu___123_1730 -> (match (uu___123_1730) with
| "op_Negation" -> begin
Some (Not)
end
| "op_AmpAmp" -> begin
...
let mk_op : Prims.string  ->  op Prims.option = (fun uu___124_1740 -> (match (uu___124_1740) with
...
let uu___129_1809 = env
in {names = ({pretty = x; mut = is_mut})::env.names; names_t = uu___129_1809.names_t; module_name = uu___129_1809.module_name}))


let extend_t : env  ->  Prims.string  ->  env = (fun env x -> (

let uu___130_1816 = env
in {names = uu___130_1816.names; names_t = (x)::env.names_t; module_name = uu___130_1816.module_name}))
...
and translate_flags : FStar_Extraction_ML_Syntax.c_flag Prims.list  ->  flag Prims.list = (fun flags -> (FStar_List.choose (fun uu___125_2093 -> (match (uu___125_2093) with
...
let assumed = (FStar_Util.for_some (fun uu___126_2142 -> (match (uu___126_2142) with
...
let rec find_return_type = (fun uu___127_2158 -> (match (uu___127_2158) with
...
let is_mut = (FStar_Util.for_some (fun uu___128_2733 -> (match (uu___128_2733) with
