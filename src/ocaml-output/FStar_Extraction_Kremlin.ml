...
let extend: env -> Prims.string -> Prims.bool -> env =
  fun env  ->
    fun x  ->
      fun is_mut  ->
        let uu___406_2780 = env in
        {
          names = ({ pretty = x; mut = is_mut } :: (env.names));
          names_t = (uu___406_2780.names_t);
          module_name = (uu___406_2780.module_name)
        }
let extend_t: env -> Prims.string -> env =
  fun env  ->
    fun x  ->
      let uu___407_2787 = env in
      {
        names = (uu___407_2787.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___407_2787.module_name)
      }
  ...
  fun env  ->
    fun d  ->
      ...
      | FStar_Extraction_ML_Syntax.MLM_Let
          ...
          let rec find_return_type i uu___404_3261 =
            match uu___404_3261 with
      ...
      | FStar_Extraction_ML_Syntax.MLM_Let
          ...
          let env2 =
            FStar_List.fold_left
              (fun env2  -> fun name1  -> extend_t env2 name1) env1 tvars in
          let rec find_return_type i uu___404_3423 =
            match uu___404_3423 with
