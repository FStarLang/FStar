              ...
              (let res =
               ...
               | FStar_Pervasives_Native.Some (uu___1, lcomp, g) ->
                   ...
                   then FStar_Tactics_Monad.ret tm
                     ...
                     (let uu___4 =
                        let uu___5 =
                          (Prims.op_Negation allow_subtyping) ||
                        ...
                        then
                          ...
                          (let uu___7 =
                             ...
                             FStar_Tactics_Monad.new_uvar "do_rewrite.eq_ty"
                               env uu___8 FStar_Pervasives_Native.None []
                               (rangeof g0) in
                        ...
                        (fun typ ->
                           let uu___5 =
                             FStar_Tactics_Monad.new_uvar "do_rewrite.rhs"
                               env typ FStar_Pervasives_Native.None []
                               (rangeof g0) in
    ...
    fun allow_subtyping ->
      fun rewriter ->
        fun env ->
          fun tm ->
            let uu___ =
              let uu___1 = __do_rewrite g0 allow_subtyping rewriter env tm in
        ...
        fun controller ->
          fun rewriter ->
            fun env ->
              fun tm ->
                let recurse tm1 =
                  ctrl_fold_env g0 d allow_subtyping controller rewriter env
                    tm1 in
                match d with
                | FStar_Tactics_Types.TopDown ->
                    let uu___ =
                      seq_ctac
                        (maybe_rewrite g0 allow_subtyping controller rewriter
                           env)
                        (on_subterms g0 d allow_subtyping controller rewriter
                           env) in
                    uu___ tm
            ...
            fun env ->
              fun tm ->
                let recurse env1 tm1 =
                  ctrl_fold_env g0 d allow_subtyping controller rewriter env1
                    tm1 in
                               ...
                               (match flag with
                                ...
                                | uu___2 ->
                                    ...
                                    descend_binders orig (b1 ::
                                      ...
                                      (par_combine (accum_flag, flag)) env2
                                      bs1 t k rebuild)) in
                let go uu___ =
        ...
        fun controller ->
          fun rewriter ->
            fun env ->
              fun tm ->
                let uu___ =
                  ctrl_fold_env g0 dir allow_subtyping controller rewriter
                    env tm in
                FStar_Tactics_Monad.bind uu___
                  (fun uu___1 ->
                     match uu___1 with
                     ...
                     FStar_Tactics_Monad.bind FStar_Tactics_Monad.dismiss_all
                       (fun uu___2 ->
                          ...
                          (let uu___4 =
                             ...
                             do_ctrl_rewrite g dir allow_subtyping controller
                               rewriter uu___5 gt in
                           FStar_Tactics_Monad.bind uu___4
                             (fun gt' ->
                                FStar_Tactics_Monad.log ps
                                  (fun uu___6 ->
