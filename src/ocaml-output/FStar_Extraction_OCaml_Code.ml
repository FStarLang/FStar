...
let path_of_ns = (fun currentModule ns -> (let outsupport = (fun ns -> (let ns' = (FStar_Extraction_ML_Util.flatten_ns ns)
...
in (match ((FStar_List.tryPick chkin outmod)) with
| None -> begin
(match ((let _125_32 = (FStar_ST.read FStar_Options.codegen_libs)
in (FStar_List.tryPick chkin _125_32))) with
| None -> begin
(outsupport ns)
end
| _59_32 -> begin
...
let ptsym_of_symbol = (fun s -> (match (((let _125_40 = (FStar_String.get s 0)
in (FStar_Char.lowercase _125_40)) <> (FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "l__" s)
end
| false -> begin
...
in (_125_45)::[])
in (FStar_List.append p _125_46))
in (FStar_String.concat "." _125_47))
end))
end))
...
| (p, s) -> begin
(let s = (match (((let _125_52 = (FStar_String.get s 0)
in (FStar_Char.uppercase _125_52)) <> (FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "U__" s)
end
| false -> begin
...
| true -> begin
(FStar_List.tryFind (fun _59_69 -> (match (_59_69) with
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _59_77 -> (match (_59_77) with
| (y, _59_76) -> begin
(x = y)
end)) prim_uni_ops)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _59_85 -> (match (_59_85) with
| (y, _59_84) -> begin
(x = y)
end)) prim_types)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _59_93 -> (match (_59_93) with
| (y, _59_92) -> begin
(x = y)
...
in (let _125_106 = (FStar_All.pipe_left escape_tyvar (FStar_Extraction_ML_Syntax.idsym x))
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (t1, _59_205, t2) -> begin
...
in (let docs = (FStar_List.map (fun d -> (let _125_157 = (let _125_156 = (let _125_155 = (FSharp_Format.text ";")
...
| FStar_Extraction_ML_Syntax.MLE_Var (x, _59_239) -> begin
...
in (let _125_167 = (FStar_List.map for1 fields)
...
(let ids = (FStar_List.map (fun _59_340 -> (match (_59_340) with
| ((x, _59_337), xt) -> begin
...
in (let _125_279 = (let _125_278 = (let _125_277 = (FStar_List.map (doc_of_branch currentModule) pats)
...
in (let _125_314 = (FStar_List.map for1 fields)
...
| FStar_Extraction_ML_Syntax.MLP_CTor (ctor, ps) -> begin
...
| {FStar_Extraction_ML_Syntax.mllb_name = name; FStar_Extraction_ML_Syntax.mllb_tysc = tys; FStar_Extraction_ML_Syntax.mllb_add_unit = _59_462; FStar_Extraction_ML_Syntax.mllb_def = e} -> begin
(let e = (doc_of_expr currentModule (min_op_prec, NonAssoc) e)
in (let ids = []
in (let ids = (FStar_List.map (fun _59_471 -> (match (_59_471) with
| (x, _59_470) -> begin
(FSharp_Format.text x)
end)) ids)
in (let _125_366 = (let _125_365 = (FSharp_Format.text (FStar_Extraction_ML_Syntax.idsym name))
...
in (let lets = (FStar_List.mapi (fun i doc -> (let _125_374 = (let _125_373 = (match ((i = 0)) with
| true -> begin
letdoc
end
| false -> begin
...
in (let _125_391 = (FStar_List.map forfield fields)
...
in (let ctors = (FStar_List.map (fun d -> (let _125_403 = (let _125_402 = (FSharp_Format.text "|")
in (_125_402)::(d)::[])
...
| FStar_Extraction_ML_Syntax.MLS_Val (x, (_59_544, ty)) -> begin
...
in (let doc = (FStar_Option.map (fun _59_596 -> (match (_59_596) with
| (s, _59_595) -> begin
(doc_of_sig x s)
end)) sigmod)
...
| (x, sigmod, FStar_Extraction_ML_Syntax.MLLib (sub)) -> begin
(let _59_610 = (FStar_Util.fprint1 "Gen Code: %s\n" x)
...
in (let doc = (FStar_Option.map (fun _59_617 -> (match (_59_617) with
| (_59_615, m) -> begin
(doc_of_mod x m)
end)) sigmod)
...
in (let docs = (FStar_List.map (fun _59_628 -> (match (_59_628) with
| (x, s, m) -> begin
...
let string_of_mlexpr = (fun env e -> (let doc = (let _125_544 = (FStar_Extraction_ML_Util.flatten_mlpath env.FStar_Extraction_ML_Env.currentModule)
in (doc_of_expr _125_544 (min_op_prec, NonAssoc) e))
in (FSharp_Format.pretty 0 doc)))

let string_of_mlty = (fun env e -> (let doc = (let _125_549 = (FStar_Extraction_ML_Util.flatten_mlpath env.FStar_Extraction_ML_Env.currentModule)
