...
let guard_eq
    : Prims.int -> FStar_TypeChecker_Common.guard_formula
      -> FStar_TypeChecker_Common.guard_formula -> Prims.unit =
  fun i g g' ->
    let uu____18 =
      match (g, g') with
      | FStar_TypeChecker_Common.Trivial, FStar_TypeChecker_Common.Trivial ->
          (true, g, g')
      | ( FStar_TypeChecker_Common.NonTrivial f
        , FStar_TypeChecker_Common.NonTrivial f' ) ->
          let f1 =
    ...
    match uu____18 with b, g1, g'1 ->
      if Prims.op_Negation b then
        ...
        in
        FStar_Errors.raise_error (FStar_Errors.Fatal_UnexpectedGuard, msg)
...
let unify
    : Prims.int -> FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
      -> FStar_TypeChecker_Common.guard_formula -> (Prims.unit -> Prims.unit)
      -> Prims.unit =
  fun i x1 y1 g' check ->
    ...
    try
      let g =
        let uu____116 =
          ...
          FStar_TypeChecker_Rel.teq uu____117 x1 y1
        in
        let uu____118 =
          let uu____121 = tcenv () in
          FStar_TypeChecker_Rel.solve_deferred_constraints uu____121
        in
        FStar_All.pipe_right uu____116 uu____118
      in
  ...
  fun x1 y1 ->
    ...
    let g =
      let uu____146 =
        ...
        FStar_TypeChecker_Rel.teq uu____147 x2 y2
      in
      let uu____148 =
        let uu____151 = tcenv () in
        FStar_TypeChecker_Rel.solve_deferred_constraints uu____151
      in
      FStar_All.pipe_right uu____146 uu____148
    in
  ...
  fun n1 tm1 ->
    let rec aux out n2 =
      if n2 = Prims.parse_int "0" then out
        ...
        in
        match uu____191 with t, uu____203 ->
          let uu____204 =
            FStar_TypeChecker_Rel.new_uvar FStar_Range.dummyRange [] t
          in
          match uu____204 with u, uu____216 ->
            aux (u :: out) (n2 - Prims.parse_int "1")
    in
    let us = aux [] n1 in
    let uu____220 =
      let uu____221 = FStar_Tests_Util.app tm1 us in
      norm uu____221
    in
    (uu____220, us)
...
let run_all : Prims.unit -> Prims.unit =
  fun uu____226 ->
    ...
    let x1 =
      ...
      in
      FStar_All.pipe_right uu____267 FStar_Syntax_Syntax.bv_to_name
    in
    let y1 =
      let uu____269 =
        FStar_Syntax_Syntax.gen_bv "y" FStar_Pervasives_Native.None int_t
      in
      FStar_All.pipe_right uu____269 FStar_Syntax_Syntax.bv_to_name
    in
    ...
    (let uu____272 =
       ...
       in
       FStar_TypeChecker_Common.NonTrivial uu____273
     in
    ...
    (let uu____276 = FStar_Tests_Util.app id1 [x1] in
     unify1 (Prims.parse_int "2") x1 uu____276 FStar_TypeChecker_Common.Trivial) ;
    ...
    (let uu____289 = FStar_Tests_Pars.tc "fun (x:int) (y:int) -> y" in
     ...
     let uu____291 =
       ...
       in
       FStar_TypeChecker_Common.NonTrivial uu____292
     in
     unify1 (Prims.parse_int "7") uu____289 uu____290 uu____291) ;
    (let uu____294 = FStar_Tests_Pars.tc "fun (x:int) (y:int) (z:int) -> y" in
     let uu____295 = FStar_Tests_Pars.tc "fun (x:int) (y:int) (z:int) -> z" in
     let uu____296 =
       let uu____297 =
         FStar_Tests_Pars.tc
           "(forall (x:int). (forall (y:int). (forall (z:int). y==z)))"
       in
       FStar_TypeChecker_Common.NonTrivial uu____297
     in
    ...
    match uu____318 with tm1, us ->
      ...
      match uu____343 with tm2, us1 ->
        ...
        unify_check (Prims.parse_int "10") tm2 sol1
          FStar_TypeChecker_Common.Trivial (fun uu____363 ->
            let uu____364 =
              ...
              FStar_Tests_Util.term_eq uu____365 uu____367
            in
            FStar_Tests_Util.always (Prims.parse_int "10") uu____364 ) ;
        let tm11 = FStar_Tests_Pars.tc "x:int -> y:int{eq2 y x} -> bool" in
        ...
        (let uu____371 =
           ...
           in
           FStar_TypeChecker_Common.NonTrivial uu____372
         in
        ...
        let uu____376 =
          ...
          let x2 =
            FStar_Syntax_Syntax.new_bv FStar_Pervasives_Native.None int_typ
          in
          let typ = FStar_Tests_Pars.tc "unit -> Type0" in
          let l = FStar_Tests_Pars.tc "fun (q:(unit -> Type0)) -> q ()" in
          let q =
          ...
          let tm13 =
            let uu____389 =
              let uu____390 =
                let uu____393 = FStar_Syntax_Syntax.bv_to_name q in
                [uu____393]
              in
              FStar_Tests_Util.app l uu____390
            in
            norm uu____389
          in
          ...
          let uu____396 =
            let uu____401 =
              ...
              let uu____403 =
                let uu____406 = FStar_Syntax_Syntax.mk_binder q in
                [uu____406]
              in
              uu____402 :: uu____403
            in
            FStar_TypeChecker_Rel.new_uvar FStar_Range.dummyRange uu____401 typ
          in
          match uu____396 with u_p, uu____414 ->
            let tm23 =
              let uu____418 =
                let uu____421 = FStar_Tests_Util.app l1 [u_p] in
                norm uu____421
              in
              FStar_Tests_Util.app uu____418 [unit]
            in
            (tm13, tm23)
        in
        match uu____376 with tm13, tm23 ->
          (let uu____431 =
             let uu____432 = FStar_Tests_Pars.tc "Prims.l_True" in
             FStar_TypeChecker_Common.NonTrivial uu____432
           in
           unify1 (Prims.parse_int "13") tm13 tm23 uu____431) ;
          let uu____433 =
            let int_typ = FStar_Tests_Pars.tc "int" in
            let x2 =
              FStar_Syntax_Syntax.new_bv FStar_Pervasives_Native.None int_typ
            ...
            let l = FStar_Tests_Pars.tc "fun (q:pure_post unit) -> q ()" in
            let q =
              FStar_Syntax_Syntax.new_bv FStar_Pervasives_Native.None typ
            in
            let tm14 =
              let uu____446 =
                let uu____447 =
                  let uu____450 = FStar_Syntax_Syntax.bv_to_name q in
                  [uu____450]
                in
                FStar_Tests_Util.app l uu____447
              in
              norm uu____446
            in
            let l1 = FStar_Tests_Pars.tc "fun (p:pure_post unit) -> p" in
            let unit = FStar_Tests_Pars.tc "()" in
            let uu____453 =
              let uu____458 =
                let uu____459 = FStar_Syntax_Syntax.mk_binder x2 in
                let uu____460 =
                  let uu____463 = FStar_Syntax_Syntax.mk_binder q in
                  [uu____463]
                in
                uu____459 :: uu____460
              in
              FStar_TypeChecker_Rel.new_uvar FStar_Range.dummyRange uu____458
                typ
            in
            match uu____453 with u_p, uu____471 ->
              let tm24 =
                let uu____475 =
                  let uu____478 = FStar_Tests_Util.app l1 [u_p] in
                  norm uu____478
                in
              ...
              (tm14, tm24)
          in
          match uu____433 with tm14, tm24 ->
            (let uu____488 =
               let uu____489 = FStar_Tests_Pars.tc "Prims.l_True" in
               FStar_TypeChecker_Common.NonTrivial uu____489
             in
             unify1 (Prims.parse_int "14") tm14 tm24 uu____488) ;
