      ...
      'Auu____3241 ->
        FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax ->
          (FStar_TypeChecker_Env.env ->
             'Auu____3241 -> FStar_Syntax_Syntax.term)
            -> unit
  =
  fun i  ->
    fun r  ->
      fun expected  ->
        fun normalizer  ->
          (let uu____3277 = FStar_Util.string_of_int i  in
           FStar_Util.print1 "%s: ... \n\n" uu____3277);
          (let tcenv = FStar_Tests_Pars.init ()  in
           (let uu____3280 = FStar_Main.process_args ()  in
            FStar_All.pipe_right uu____3280 (fun a242  -> ()));
           (let x1 = normalizer tcenv r  in
            ...
            FStar_Options.set_option "print_implicits"
              (FStar_Options.Bool true);
            (let uu____3297 =
               let uu____3298 = FStar_Syntax_Util.unascribe x1  in
               FStar_Tests_Util.term_eq uu____3298 expected  in
             FStar_Tests_Util.always i uu____3297)))
...
let (run_interpreter_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let interp uu____3367 = run_interpreter i r expected  in
        let uu____3368 =
          let uu____3369 = FStar_Util.return_execution_time interp  in
...
let (run_nbe_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let nbe1 uu____3402 = run_nbe i r expected  in
        let uu____3403 =
          let uu____3404 = FStar_Util.return_execution_time nbe1  in
          FStar_Pervasives_Native.snd uu____3404  in
        (i, uu____3403)
...
let run_tests :
  'Auu____3413 .
    (Prims.int ->
       FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax ->
         FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax -> 'Auu____3413)
      -> 'Auu____3413 Prims.list
  =
  fun run1  ->
    ...
    (let l =
       FStar_List.map
         (fun uu___456_3462  ->
            match uu___456_3462 with | (no,test,res) -> run1 no test res)
         tests
        in
...
let (run_both_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let nbe1 uu____3575 = run_nbe i r expected  in
        let norm1 uu____3581 = run_interpreter i r expected  in
...
let (compare : unit -> unit) =
  fun uu____3589  ->
    ...
    (let uu____3591 =
       let uu____3592 = encode (Prims.parse_int "1000")  in
       let uu____3593 =
         let uu____3596 = FStar_Tests_Util.nm FStar_Tests_Util.x  in
         let uu____3597 = FStar_Tests_Util.nm FStar_Tests_Util.x  in
         minus uu____3596 uu____3597  in
       let_ FStar_Tests_Util.x uu____3592 uu____3593  in
     run_both_with_time (Prims.parse_int "14") uu____3591 z)
  ...
  fun l_int  ->
    fun l_nbe  ->
      ...
      FStar_List.iter2
        (fun res1  ->
           fun res2  ->
             let uu____3662 = res1  in
             match uu____3662 with
             | (t1,time_int) ->
                 let uu____3669 = res2  in
                 (match uu____3669 with
                  | (t2,time_nbe) ->
                      if t1 = t2
                      then
                        let uu____3676 = FStar_Util.string_of_int t1  in
                        FStar_Util.print3 "Test %s\nNBE %s\nInterpreter %s\n"
                          uu____3676 (FStar_Util.string_of_float time_nbe)
...
let (run_all : unit -> unit) =
  fun uu____3682  ->
    (let uu____3684 = FStar_Syntax_Print.term_to_string znat  in
     FStar_Util.print1 "%s" uu____3684);
