...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____899 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____907 = FStar_Util.write x Prims.int_zero  in (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____938 = FStar_Options.print_real_names ()  in
    if uu____938
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____967) -> FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____971 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____980) -> x1
    | FStar_Const.Const_char c ->
        let uu____997 = FStar_String.op_Hat (FStar_Util.string_of_char c) "'"
           in
        FStar_String.op_Hat "'" uu____997
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____1006 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____1006
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1024 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____1024  in
      let uu____1027 = psnconst t  in FStar_Ident.set_lid_range uu____1027 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1048 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1048
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1065 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____1065  in
      let uu____1068 = psnconst t  in FStar_Ident.set_lid_range uu____1068 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1096 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1096
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1144 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____1144  in
      let uu____1147 = let uu____1148 = mod_prefix_dtuple n1  in uu____1148 t
         in
      FStar_Ident.set_lid_range uu____1147 r
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "Mkdtuple%s" uu____1189  in
      let uu____1192 = let uu____1193 = mod_prefix_dtuple n1  in uu____1193 t
         in
      FStar_Ident.set_lid_range uu____1192 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1226 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1226
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1234 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1234
