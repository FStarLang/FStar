...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____2585 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____2593 = FStar_Util.write x (Prims.parse_int "0")  in
  (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____2632 = FStar_Options.print_real_names ()  in
    if uu____2632
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____2661) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____2665 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____2674) -> x1
    | FStar_Const.Const_char c ->
        let uu____2691 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'"  in
        FStar_String.op_Hat "'" uu____2691
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____2704 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____2704
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____2726 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____2726  in
      let uu____2729 = psnconst t  in FStar_Ident.set_lid_range uu____2729 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____2770 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____2770
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____2791 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____2791  in
      let uu____2794 = psnconst t  in FStar_Ident.set_lid_range uu____2794 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____2846 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____2846
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "dtuple%s" uu____2926  in
      let uu____2929 = let uu____2938 = mod_prefix_dtuple n1  in uu____2938 t
         in
      FStar_Ident.set_lid_range uu____2929 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____2995 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____2995  in
      let uu____2998 = let uu____3007 = mod_prefix_dtuple n1  in uu____3007 t
         in
      FStar_Ident.set_lid_range uu____2998 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____3052 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____3052
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____3076 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____3076
