...
let (inl_lid : FStar_Ident.lident) = pconst "Inl"
let (inr_lid : FStar_Ident.lident) = pconst "Inr"
...
let (c_and_lid : FStar_Ident.lident) = pconst "tuple2"
let (c_or_lid : FStar_Ident.lident) = pconst "either"
...
let (is_tuple_constructor_string : Prims.string -> Prims.bool) =
  fun s ->
    (s = "Prims.tuple2") ||
      (FStar_Compiler_Util.starts_with s "FStar.Pervasives.Native.tuple")
