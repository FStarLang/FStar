...
let assume_strictly_positive_attr_lid : FStar_Ident.lident = (p2l (("FStar")::("Pervasives")::("assume_strictly_positive")::[]))
...
let gen1 = (fun uu____44 -> ((FStar_Util.incr x);
(FStar_Util.read x);
))
in (

let reset = (fun uu____106 -> (FStar_Util.write x (Prims.parse_int "0")))
in ((gen1), (reset)))))
...
let uu____150 = (FStar_Options.print_real_names ())
...
| FStar_Const.Const_string (s, uu____161) -> begin
(FStar_Util.format1 "\"%s\"" s)
end
| FStar_Const.Const_bytearray (uu____162) -> begin
"<bytearray>"
end
| FStar_Const.Const_int (x1, uu____171) -> begin
x1
end
...
let uu____188 = (sli l)
in (FStar_Util.format1 "[[%s.reflect]]" uu____188))
end))
...
let uu____200 = (FStar_Util.string_of_int n1)
in (FStar_Util.format1 "tuple%s" uu____200))
in (

let uu____201 = (psnconst t)
in (FStar_Ident.set_lid_range uu____201 r))))
...
let uu____212 = (FStar_Ident.text_of_id lid)
in (is_tuple_constructor_string uu____212)))
...
let uu____224 = (FStar_Util.string_of_int n1)
in (FStar_Util.format1 "Mktuple%s" uu____224))
in (

let uu____225 = (psnconst t)
in (FStar_Ident.set_lid_range uu____225 r))))
...
let uu____241 = (mk_tuple_data_lid n1 FStar_Range.dummyRange)
in (FStar_Ident.lid_equals f uu____241)))
...
let uu____273 = (FStar_Util.string_of_int n1)
in (FStar_Util.format1 "dtuple%s" uu____273))
in (

let uu____274 = (

let uu____275 = (mod_prefix_dtuple n1)
in (uu____275 t))
in (FStar_Ident.set_lid_range uu____274 r))))
...
let uu____301 = (FStar_Util.string_of_int n1)
in (FStar_Util.format1 "Mkdtuple%s" uu____301))
in (

let uu____302 = (

let uu____303 = (mod_prefix_dtuple n1)
in (uu____303 t))
in (FStar_Ident.set_lid_range uu____302 r))))
...
let uu____323 = (mk_dtuple_data_lid n1 FStar_Range.dummyRange)
in (FStar_Ident.lid_equals f uu____323)))


let is_dtuple_data_lid' : FStar_Ident.lident  ->  Prims.bool = (fun f -> (

let uu____329 = (FStar_Ident.text_of_lid f)
in (is_dtuple_datacon_string uu____329)))
...
let tcnorm_lid : FStar_Ident.lid = (fstar_tactics_lid' (("Typeclasses")::("tcnorm")::[]))


let tcinstance_lid : FStar_Ident.lid = (fstar_tactics_lid' (("Typeclasses")::("instance")::[]))
