...
let (mk_n_tuple_lid :
  ...
  fun n1  ->
    fun nm  ->
      fun r  ->
        let n_str = FStar_Util.string_of_int n1  in
        let module_name = FStar_Util.format1 "Tuple%s" n_str  in
        let t = FStar_Util.format2 "%s%s" nm n_str  in
        let l = p2l ["FStar"; "Pervasives"; "Native"; module_name; t]  in
        FStar_Ident.set_lid_range l r
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  -> fun r  -> mk_n_tuple_lid n1 "tuple" r 
let (lid_tuple2 : FStar_Ident.lident) =
  mk_tuple_lid (Prims.parse_int "2") FStar_Range.dummyRange 
let (is_tuple_name : Prims.string -> Prims.string -> Prims.bool) =
  fun s  ->
    fun nm  ->
      let prefix1 = "FStar.Pervasives.Native.Tuple"  in
      let n_prefix = FStar_String.length prefix1  in
      ((FStar_Util.starts_with s prefix1) &&
         ((FStar_String.length s) > n_prefix))
        &&
        (let sfx = FStar_Util.substring_from s n_prefix  in
         match FStar_Util.split sfx "." with
         | uu____1096::nm'::[] -> FStar_Util.starts_with nm' nm
         | uu____1103 -> false)
  
let (is_tuple_constructor_string : Prims.string -> Prims.bool) =
  fun s  -> is_tuple_name s "tuple" 
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1126 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1126
  
let (mk_tuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  -> fun r  -> mk_n_tuple_lid n1 "Mktuple" r 
...
let (is_tuple_datacon_string : Prims.string -> Prims.bool) =
  fun s  -> is_tuple_name s "Mktuple" 
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1169 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1169
  ...
  fun n1  ->
    fun nm  ->
      ...
      then pconst t
        ...
        (let module_name = FStar_Util.format1 "DTuple%s" n_str  in
         let l = p2l ["FStar"; "Pervasives"; module_name; t]  in l)
  
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let uu____1226 = mod_prefix_dtuple n1 "dtuple"  in
      FStar_Ident.set_lid_range uu____1226 r
  
let (is_dtuple_constructor_string : Prims.string -> Prims.bool) =
  fun s  ->
    ((s = "Prims.dtuple2") ||
       (FStar_Util.starts_with s "FStar.Pervasives.DTuple3.dtuple"))
      || (FStar_Util.starts_with s "FStar.Pervasives.DTuple4.dtuple")
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let uu____1262 = mod_prefix_dtuple n1 "Mkdtuple"  in
      FStar_Ident.set_lid_range uu____1262 r
  
let (is_dtuple_datacon_string : Prims.string -> Prims.bool) =
  fun s  ->
    ((s = "Prims.Mkdtuple2") ||
       (FStar_Util.starts_with s "FStar.Pervasives.DTuple3.Mkdtuple"))
      || (FStar_Util.starts_with s "FStar.Pervasives.DTuple4.Mkdtuple")
  
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1292 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1292
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1300 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1300
