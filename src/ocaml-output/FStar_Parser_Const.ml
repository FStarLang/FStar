...
let sli : FStar_Ident.lident -> Prims.string =
  fun l ->
    ...
    if uu____130 then l.FStar_Ident.str
...
let const_to_string : FStar_Const.sconst -> Prims.string =
  fun x ->
    ...
    | FStar_Const.Const_string (s, uu____139) -> FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____140 -> "<bytearray>"
    | FStar_Const.Const_int (x1, uu____148) -> x1
...
let mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident =
  fun n1 r ->
    let t =
      ...
      FStar_Util.format1 "tuple%s" uu____172
    in
    let uu____173 = psnconst t in
    FStar_Ident.set_lid_range uu____173 r
...
let mk_tuple_data_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident =
  fun n1 r ->
    let t =
      ...
      FStar_Util.format1 "Mktuple%s" uu____187
    in
    let uu____188 = psnconst t in
    FStar_Ident.set_lid_range uu____188 r
...
let mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident =
  fun n1 r ->
    let t =
      ...
      FStar_Util.format1 "dtuple%s" uu____219
    in
    let uu____220 =
      let uu____221 = mod_prefix_dtuple n1 in
      uu____221 t
    in
...
let is_dtuple_constructor_string : Prims.string -> Prims.bool =
  fun s ->
    s = "Prims.dtuple2" || FStar_Util.starts_with s "FStar.Pervasives.dtuple"
...
let mk_dtuple_data_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident =
  fun n1 r ->
    let t =
      ...
      FStar_Util.format1 "Mkdtuple%s" uu____237
    in
    let uu____238 =
      let uu____239 = mod_prefix_dtuple n1 in
      uu____239 t
    in
...
let fstar_tactics_lid' : Prims.string Prims.list -> FStar_Ident.lid =
  fun s ->
    FStar_Ident.lid_of_path
      (FStar_List.append ["FStar"; "Tactics"] s)
