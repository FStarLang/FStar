...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____37546 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____37609 = FStar_Util.write x (Prims.parse_int "0")  in
  (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____37662 = FStar_Options.print_real_names ()  in
    if uu____37662
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____37691) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____37695 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____37704) -> x1
    | FStar_Const.Const_char c ->
        let uu____37721 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'"  in
        FStar_String.op_Hat "'" uu____37721
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____37730 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____37730
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____37748 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____37748  in
      let uu____37751 = psnconst t  in
      FStar_Ident.set_lid_range uu____37751 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____37772 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____37772
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____37789 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____37789  in
      let uu____37792 = psnconst t  in
      FStar_Ident.set_lid_range uu____37792 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____37820 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____37820
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____37868 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____37868  in
      let uu____37871 =
        let uu____37872 = mod_prefix_dtuple n1  in uu____37872 t  in
      FStar_Ident.set_lid_range uu____37871 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____37913 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____37913  in
      let uu____37916 =
        let uu____37917 = mod_prefix_dtuple n1  in uu____37917 t  in
      FStar_Ident.set_lid_range uu____37916 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____37950 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____37950
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____37958 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____37958
