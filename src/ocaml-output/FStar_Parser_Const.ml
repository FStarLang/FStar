...
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____942 = FStar_Options.print_real_names ()  in
    if uu____942
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_reflect l ->
        let uu____1005 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____1005
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1023 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____1023  in
      let uu____1026 = psnconst t  in FStar_Ident.set_lid_range uu____1026 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1047 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1047
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "Mktuple%s" uu____1064  in
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1095 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1095
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1143 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____1143  in
      let uu____1146 = let uu____1147 = mod_prefix_dtuple n1  in uu____1147 t
         in
      FStar_Ident.set_lid_range uu____1146 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1188 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____1188  in
      let uu____1191 = let uu____1192 = mod_prefix_dtuple n1  in uu____1192 t
         in
      FStar_Ident.set_lid_range uu____1191 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1225 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1225
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1233 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1233
