...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____881 = FStar_Util.incr x; FStar_Util.read x  in
...
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____920 = FStar_Options.print_real_names ()  in
    if uu____920
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____949) -> FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____953 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____962) -> x1
    | FStar_Const.Const_char c ->
        let uu____979 = FStar_String.op_Hat (FStar_Util.string_of_char c) "'"
           in
        FStar_String.op_Hat "'" uu____979
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____988 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____988
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "tuple%s" uu____1006  in
      let uu____1009 = psnconst t  in FStar_Ident.set_lid_range uu____1009 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1030 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1030
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1047 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____1047  in
      let uu____1050 = psnconst t  in FStar_Ident.set_lid_range uu____1050 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1078 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1078
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "dtuple%s" uu____1126  in
      let uu____1129 = let uu____1130 = mod_prefix_dtuple n1  in uu____1130 t
         in
      FStar_Ident.set_lid_range uu____1129 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1171 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____1171  in
      let uu____1174 = let uu____1175 = mod_prefix_dtuple n1  in uu____1175 t
         in
      FStar_Ident.set_lid_range uu____1174 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1208 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1208
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1216 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1216
