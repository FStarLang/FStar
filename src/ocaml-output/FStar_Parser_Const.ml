...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen uu____1006 = FStar_Util.incr x; FStar_Util.read x in
  let reset uu____1014 = FStar_Util.write x Prims.int_zero in (gen, reset)
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l ->
    let uu____1045 = FStar_Options.print_real_names () in
    if uu____1045
    then FStar_Ident.string_of_lid l
    else
      (let uu____1051 = FStar_Ident.ident_of_lid l in
       FStar_Ident.string_of_id uu____1051)
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x ->
    ...
    | FStar_Const.Const_string (s, uu____1075) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____1079 -> "<bytearray>"
    | FStar_Const.Const_int (x1, uu____1089) -> x1
    | FStar_Const.Const_char c ->
        let uu____1106 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'" in
        FStar_String.op_Hat "'" uu____1106
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of -> "range_of"
    | FStar_Const.Const_set_range_of -> "set_range_of"
    | FStar_Const.Const_reify -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____1115 = sli l in
        FStar_Util.format1 "[[%s.reflect]]" uu____1115
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1133 = FStar_Util.string_of_int n in
        FStar_Util.format1 "tuple%s" uu____1133 in
      let uu____1136 = psnconst t in FStar_Ident.set_lid_range uu____1136 r
...
let (is_tuple_constructor_id : FStar_Ident.ident -> Prims.bool) =
  fun id ->
    let uu____1157 = FStar_Ident.string_of_id id in
    is_tuple_constructor_string uu____1157
let (is_tuple_constructor_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1166 = FStar_Ident.string_of_lid lid in
    is_tuple_constructor_string uu____1166
let (mk_tuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1183 = FStar_Util.string_of_int n in
        FStar_Util.format1 "Mktuple%s" uu____1183 in
      let uu____1186 = psnconst t in FStar_Ident.set_lid_range uu____1186 r
...
let (is_tuple_datacon_id : FStar_Ident.ident -> Prims.bool) =
  fun id ->
    let uu____1207 = FStar_Ident.string_of_id id in
    is_tuple_datacon_string uu____1207
let (is_tuple_datacon_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1216 = FStar_Ident.string_of_lid lid in
    is_tuple_datacon_string uu____1216
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n ->
      let uu____1232 = mk_tuple_data_lid n FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____1232
let (is_tuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f ->
    let uu____1240 = FStar_Ident.string_of_lid f in
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1282 = FStar_Util.string_of_int n in
        FStar_Util.format1 "dtuple%s" uu____1282 in
      let uu____1285 = let uu____1286 = mod_prefix_dtuple n in uu____1286 t in
      FStar_Ident.set_lid_range uu____1285 r
...
let (is_dtuple_constructor_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1312 = FStar_Ident.string_of_lid lid in
    is_dtuple_constructor_string uu____1312
let (mk_dtuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1329 = FStar_Util.string_of_int n in
        FStar_Util.format1 "Mkdtuple%s" uu____1329 in
      let uu____1332 = let uu____1333 = mod_prefix_dtuple n in uu____1333 t in
      FStar_Ident.set_lid_range uu____1332 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n ->
      let uu____1366 = mk_dtuple_data_lid n FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____1366
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f ->
    let uu____1374 = FStar_Ident.string_of_lid f in
    is_dtuple_datacon_string uu____1374
let (is_name : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let c =
      let uu____1385 =
        let uu____1387 = FStar_Ident.ident_of_lid lid in
        FStar_Ident.string_of_id uu____1387 in
      FStar_Util.char_at uu____1385 Prims.int_zero in
