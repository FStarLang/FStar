...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____33938 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____33946 = FStar_Util.write x (Prims.parse_int "0")  in
  (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____33977 = FStar_Options.print_real_names ()  in
    if uu____33977
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____34006) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____34010 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____34019) -> x1
    | FStar_Const.Const_char c ->
        let uu____34036 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'"  in
        FStar_String.op_Hat "'" uu____34036
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____34045 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____34045
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____34063 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____34063  in
      let uu____34066 = psnconst t  in
      FStar_Ident.set_lid_range uu____34066 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____34087 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____34087
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____34104 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____34104  in
      let uu____34107 = psnconst t  in
      FStar_Ident.set_lid_range uu____34107 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____34135 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____34135
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____34183 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____34183  in
      let uu____34186 =
        let uu____34187 = mod_prefix_dtuple n1  in uu____34187 t  in
      FStar_Ident.set_lid_range uu____34186 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____34228 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____34228  in
      let uu____34231 =
        let uu____34232 = mod_prefix_dtuple n1  in uu____34232 t  in
      FStar_Ident.set_lid_range uu____34231 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____34265 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____34265
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____34273 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____34273
