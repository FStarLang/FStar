...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen uu____1010 = FStar_Util.incr x; FStar_Util.read x in
  let reset uu____1018 = FStar_Util.write x Prims.int_zero in (gen, reset)
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l ->
    let uu____1049 = FStar_Options.print_real_names () in
    if uu____1049
    then FStar_Ident.string_of_lid l
    else
      (let uu____1055 = FStar_Ident.ident_of_lid l in
       FStar_Ident.string_of_id uu____1055)
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x ->
    ...
    | FStar_Const.Const_string (s, uu____1079) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____1083 -> "<bytearray>"
    | FStar_Const.Const_int (x1, uu____1093) -> x1
    | FStar_Const.Const_char c ->
        let uu____1110 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'" in
        FStar_String.op_Hat "'" uu____1110
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of -> "range_of"
    | FStar_Const.Const_set_range_of -> "set_range_of"
    | FStar_Const.Const_reify -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____1119 = sli l in
        FStar_Util.format1 "[[%s.reflect]]" uu____1119
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1137 = FStar_Util.string_of_int n in
        FStar_Util.format1 "tuple%s" uu____1137 in
      let uu____1140 = psnconst t in FStar_Ident.set_lid_range uu____1140 r
...
let (is_tuple_constructor_id : FStar_Ident.ident -> Prims.bool) =
  fun id ->
    let uu____1161 = FStar_Ident.string_of_id id in
    is_tuple_constructor_string uu____1161
let (is_tuple_constructor_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1170 = FStar_Ident.string_of_lid lid in
    is_tuple_constructor_string uu____1170
let (mk_tuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1187 = FStar_Util.string_of_int n in
        FStar_Util.format1 "Mktuple%s" uu____1187 in
      let uu____1190 = psnconst t in FStar_Ident.set_lid_range uu____1190 r
...
let (is_tuple_datacon_id : FStar_Ident.ident -> Prims.bool) =
  fun id ->
    let uu____1211 = FStar_Ident.string_of_id id in
    is_tuple_datacon_string uu____1211
let (is_tuple_datacon_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1220 = FStar_Ident.string_of_lid lid in
    is_tuple_datacon_string uu____1220
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n ->
      let uu____1236 = mk_tuple_data_lid n FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____1236
let (is_tuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f ->
    let uu____1244 = FStar_Ident.string_of_lid f in
    is_tuple_datacon_string uu____1244
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1286 = FStar_Util.string_of_int n in
        FStar_Util.format1 "dtuple%s" uu____1286 in
      let uu____1289 = let uu____1290 = mod_prefix_dtuple n in uu____1290 t in
      FStar_Ident.set_lid_range uu____1289 r
...
let (is_dtuple_constructor_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let uu____1316 = FStar_Ident.string_of_lid lid in
    is_dtuple_constructor_string uu____1316
let (mk_dtuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n ->
    fun r ->
      let t =
        let uu____1333 = FStar_Util.string_of_int n in
        FStar_Util.format1 "Mkdtuple%s" uu____1333 in
      let uu____1336 = let uu____1337 = mod_prefix_dtuple n in uu____1337 t in
      FStar_Ident.set_lid_range uu____1336 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n ->
      let uu____1370 = mk_dtuple_data_lid n FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____1370
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f ->
    let uu____1378 = FStar_Ident.string_of_lid f in
    is_dtuple_datacon_string uu____1378
let (is_name : FStar_Ident.lident -> Prims.bool) =
  fun lid ->
    let c =
      let uu____1389 =
        let uu____1391 = FStar_Ident.ident_of_lid lid in
        FStar_Ident.string_of_id uu____1391 in
      FStar_Util.char_at uu____1389 Prims.int_zero in
