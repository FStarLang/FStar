...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____990 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____998 = FStar_Util.write x Prims.int_zero  in (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____1029 = FStar_Options.print_real_names ()  in
    if uu____1029
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____1058) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____1062 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____1071) -> x1
    | FStar_Const.Const_char c ->
        let uu____1088 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'"  in
        FStar_String.op_Hat "'" uu____1088
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____1097 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____1097
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1115 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____1115  in
      let uu____1118 = psnconst t  in FStar_Ident.set_lid_range uu____1118 r
...
let (is_tuple_constructor_id : FStar_Ident.ident -> Prims.bool) =
  fun id1  ->
    let uu____1139 = FStar_Ident.text_of_id id1  in
    is_tuple_constructor_string uu____1139
  
let (is_tuple_constructor_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid  ->
    let uu____1148 = FStar_Ident.text_of_lid lid  in
    is_tuple_constructor_string uu____1148
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1165 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____1165  in
      let uu____1168 = psnconst t  in FStar_Ident.set_lid_range uu____1168 r
...
let (is_tuple_datacon_id : FStar_Ident.ident -> Prims.bool) =
  fun id1  ->
    let uu____1189 = FStar_Ident.text_of_id id1  in
    is_tuple_datacon_string uu____1189
  
let (is_tuple_datacon_lid : FStar_Ident.lident -> Prims.bool) =
  fun lid  ->
    let uu____1198 = FStar_Ident.text_of_lid lid  in
    is_tuple_datacon_string uu____1198
  
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1214 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1214
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1262 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____1262  in
      let uu____1265 = let uu____1266 = mod_prefix_dtuple n1  in uu____1266 t
         in
      FStar_Ident.set_lid_range uu____1265 r
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "Mkdtuple%s" uu____1307  in
      let uu____1310 = let uu____1311 = mod_prefix_dtuple n1  in uu____1311 t
         in
      FStar_Ident.set_lid_range uu____1310 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1344 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1344
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1352 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1352
