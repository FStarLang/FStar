...
let (string_strcat_lid : FStar_Ident.lident) = s2l "strcat" 
...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____834 = FStar_Util.incr x; FStar_Util.read x  in
  let reset uu____897 = FStar_Util.write x (Prims.parse_int "0")  in
  (gen1, reset) 
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset 
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____950 = FStar_Options.print_real_names ()  in
    if uu____950
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____976) -> FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____980 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____989) -> x1
    | FStar_Const.Const_char c ->
        let uu____1006 =
          FStar_String.op_Hat (FStar_Util.string_of_char c) "'"  in
        FStar_String.op_Hat "'" uu____1006
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____1015 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____1015
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1033 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____1033  in
      let uu____1036 = psnconst t  in FStar_Ident.set_lid_range uu____1036 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1057 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1057
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1074 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____1074  in
      let uu____1077 = psnconst t  in FStar_Ident.set_lid_range uu____1077 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1105 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1105
  ...
  fun n1  ->
    fun r  ->
      let t =
        ...
        FStar_Util.format1 "dtuple%s" uu____1153  in
      let uu____1156 = let uu____1157 = mod_prefix_dtuple n1  in uu____1157 t
         in
      FStar_Ident.set_lid_range uu____1156 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1198 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____1198  in
      let uu____1201 = let uu____1202 = mod_prefix_dtuple n1  in uu____1202 t
         in
      FStar_Ident.set_lid_range uu____1201 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1235 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1235
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1243 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1243
