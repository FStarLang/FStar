...
let (p_refine_lid : FStar_Ident.lident) = pconst "p_refine"
let (t_refine_lid : FStar_Ident.lident) = pconst "t_refine"
...
let (t_forall_lid : FStar_Ident.lident) = pconst "t_Forall"
let (haseq_lid : FStar_Ident.lident) = pconst "hasEq"
let (b2p_lid : FStar_Ident.lident) = pconst "b2p"
...
let (t_eq2_lid : FStar_Ident.lident) = pconst "t_eq2"
...
let (gen_reset :
  ...
  let reset uu____226 = FStar_Util.write x (Prims.parse_int "0") in
  (gen1, reset)
let (next_id : unit -> Prims.int) = FStar_Pervasives_Native.fst gen_reset
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l ->
    let uu____318 = FStar_Options.print_real_names () in
    if uu____318
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x ->
    ...
    | FStar_Const.Const_string (s, uu____329) ->
        FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____330 -> "<bytearray>"
    | FStar_Const.Const_int (x1, uu____338) -> x1
    ...
    | FStar_Const.Const_reflect l ->
        let uu____354 = sli l in
        FStar_Util.format1 "[[%s.reflect]]" uu____354
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1 ->
    fun r ->
      let t =
        let uu____366 = FStar_Util.string_of_int n1 in
        FStar_Util.format1 "tuple%s" uu____366 in
      let uu____367 = psnconst t in FStar_Ident.set_lid_range uu____367 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid ->
    let uu____378 = FStar_Ident.text_of_id lid in
    is_tuple_constructor_string uu____378
let (mk_tuple_data_lid :
  Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1 ->
    fun r ->
      let t =
        let uu____390 = FStar_Util.string_of_int n1 in
        FStar_Util.format1 "Mktuple%s" uu____390 in
      let uu____391 = psnconst t in FStar_Ident.set_lid_range uu____391 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n1 ->
      let uu____407 = mk_tuple_data_lid n1 FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____407
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1 ->
    fun r ->
      let t =
        let uu____439 = FStar_Util.string_of_int n1 in
        FStar_Util.format1 "dtuple%s" uu____439 in
      let uu____440 = let uu____441 = mod_prefix_dtuple n1 in uu____441 t in
      FStar_Ident.set_lid_range uu____440 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1 ->
    fun r ->
      let t =
        let uu____467 = FStar_Util.string_of_int n1 in
        FStar_Util.format1 "Mkdtuple%s" uu____467 in
      let uu____468 = let uu____469 = mod_prefix_dtuple n1 in uu____469 t in
      FStar_Ident.set_lid_range uu____468 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f ->
    fun n1 ->
      let uu____489 = mk_dtuple_data_lid n1 FStar_Range.dummyRange in
      FStar_Ident.lid_equals f uu____489
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f ->
    let uu____495 = FStar_Ident.text_of_lid f in
    is_dtuple_datacon_string uu____495
