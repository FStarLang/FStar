...
exception Error of ((Prims.string * Microsoft_FStar_Range.range))
...
exception Warning of ((Prims.string * Microsoft_FStar_Range.range))
...
type ident =
{idText : Prims.string; idRange : Microsoft_FStar_Range.range}

let is_Mkident = (fun _ -> (All.failwith "Not yet implemented:is_Mkident"))

type l__LongIdent =
{ns : ident Prims.list; ident : ident; nsstr : Prims.string; str : Prims.string}

let is_MkLongIdent = (fun _ -> (All.failwith "Not yet implemented:is_MkLongIdent"))

type lident =
l__LongIdent

type ('a, 't) withinfo_t =
{v : 'a; sort : 't; p : Microsoft_FStar_Range.range}

let is_Mkwithinfo_t = (fun _ -> (All.failwith "Not yet implemented:is_Mkwithinfo_t"))
...
let is_Mkbvdef = (fun _ -> (All.failwith "Not yet implemented:is_Mkbvdef"))
...
| Const_bytearray of (Prims.byte Prims.array * Microsoft_FStar_Range.range)
| Const_string of (Prims.byte Prims.array * Microsoft_FStar_Range.range)
...
| Typ_delayed of (((typ * subst_t), Prims.unit  ->  typ) Microsoft_FStar_Util.either * typ memo)
| Typ_unknown 
 and comp_typ =
...
| Meta_labeled of (typ * Prims.string * Microsoft_FStar_Range.range * Prims.bool)
| Meta_refresh_label of (typ * Prims.bool Prims.option * Microsoft_FStar_Range.range)
| Meta_slack_formula of (typ * typ * Prims.bool ST.ref) 
 and 'a uvar_basis =
| Uvar
| Fixed of 'a 
 and exp' =
...
{lbname : lbname; lbtyp : typ; lbeff : lident; lbdef : exp} 
 and freevars =
{ftvs : btvar Microsoft_FStar_Util.set; fxvs : bvvar Microsoft_FStar_Util.set} 
 and uvars =
{uvars_k : uvar_k Microsoft_FStar_Util.set; uvars_t : (uvar_t * knd) Microsoft_FStar_Util.set; uvars_e : (uvar_e * typ) Microsoft_FStar_Util.set} 
 and ('a, 'b) syntax =
{n : 'a; tk : 'b memo; pos : Microsoft_FStar_Range.range; fvs : freevars memo; uvs : uvars memo} 
 and arg =
((typ, exp) Microsoft_FStar_Util.either * aqual) 
 and args =
arg Prims.list 
 and binder =
((btvar, bvvar) Microsoft_FStar_Util.either * arg_qualifier Prims.option) 
 and binders =
binder Prims.list 
 and typ =
(typ', knd) syntax 
...
(comp', Prims.unit) syntax 
 and uvar_t =
typ uvar_basis Microsoft_FStar_Unionfind.uvar 
 and exp =
(exp', typ) syntax 
 and uvar_e =
exp uvar_basis Microsoft_FStar_Unionfind.uvar 
 and btvdef =
typ bvdef 
 and bvvdef =
exp bvdef 
 and pat =
(pat', (knd, typ) Microsoft_FStar_Util.either Prims.option) withinfo_t 
 and knd =
...
(lident * args) 
 and uvar_k =
knd uvar_basis Microsoft_FStar_Unionfind.uvar 
 and lbname =
(bvvdef, lident) Microsoft_FStar_Util.either 
 and letbindings =
(Prims.bool * letbinding Prims.list) 
 and subst_t =
subst_elt Prims.list Prims.list 
 and subst_map =
(typ, exp) Microsoft_FStar_Util.either Microsoft_FStar_Util.smap 
 and subst_elt =
((btvdef * typ), (bvvdef * exp)) Microsoft_FStar_Util.either 
 and fvar =
(btvdef, bvvdef) Microsoft_FStar_Util.either 
 and btvar =
(typ, knd) bvar 
 and bvvar =
(exp, typ) bvar 
...
let is_Mkcomp_typ = (fun _ -> (All.failwith "Not yet implemented:is_Mkcomp_typ"))
...
let is_Mkletbinding = (fun _ -> (All.failwith "Not yet implemented:is_Mkletbinding"))

let is_Mkfreevars = (fun _ -> (All.failwith "Not yet implemented:is_Mkfreevars"))

let is_Mkuvars = (fun _ -> (All.failwith "Not yet implemented:is_Mkuvars"))

let is_Mksyntax = (fun _ -> (All.failwith "Not yet implemented:is_Mksyntax"))
...
type either_var =
(btvar, bvvar) Microsoft_FStar_Util.either
...
| Projector of (lident * (btvdef, bvvdef) Microsoft_FStar_Util.either)
...
let is_Mkmonad_abbrev = (fun _ -> (All.failwith "Not yet implemented:is_Mkmonad_abbrev"))

type sub_eff =
{source : lident; target : lident; lift : typ}

let is_Mksub_eff = (fun _ -> (All.failwith "Not yet implemented:is_Mksub_eff"))
...
{mname : lident; binders : binders; qualifiers : qualifier Prims.list; signature : knd; ret : typ; bind_wp : typ; bind_wlp : typ; if_then_else : typ; ite_wp : typ; ite_wlp : typ; wp_binop : typ; wp_as_type : typ; close_wp : typ; close_wp_t : typ; assert_p : typ; assume_p : typ; null_wp : typ; trivial : typ} 
 and sigelt =
| Sig_tycon of (lident * binders * knd * lident Prims.list * lident Prims.list * qualifier Prims.list * Microsoft_FStar_Range.range)
| Sig_kind_abbrev of (lident * binders * knd * Microsoft_FStar_Range.range)
| Sig_typ_abbrev of (lident * binders * knd * typ * qualifier Prims.list * Microsoft_FStar_Range.range)
| Sig_datacon of (lident * typ * tycon * qualifier Prims.list * lident Prims.list * Microsoft_FStar_Range.range)
| Sig_val_decl of (lident * typ * qualifier Prims.list * Microsoft_FStar_Range.range)
| Sig_assume of (lident * formula * qualifier Prims.list * Microsoft_FStar_Range.range)
| Sig_let of (letbindings * Microsoft_FStar_Range.range * lident Prims.list * qualifier Prims.list)
| Sig_main of (exp * Microsoft_FStar_Range.range)
| Sig_bundle of (sigelt Prims.list * qualifier Prims.list * lident Prims.list * Microsoft_FStar_Range.range)
| Sig_new_effect of (eff_decl * Microsoft_FStar_Range.range)
| Sig_sub_effect of (sub_eff * Microsoft_FStar_Range.range)
| Sig_effect_abbrev of (lident * binders * comp * qualifier Prims.list * Microsoft_FStar_Range.range)
| Sig_pragma of (pragma * Microsoft_FStar_Range.range)

let is_Mkeff_decl = (fun _ -> (All.failwith "Not yet implemented:is_Mkeff_decl"))
...
let is_Mkmodul = (fun _ -> (All.failwith "Not yet implemented:is_Mkmodul"))
...
let is_Mklcomp = (fun _ -> (All.failwith "Not yet implemented:is_Mklcomp"))
...
let text_of_path = (fun path -> (Microsoft_FStar_Util.concat_l "." path))

let path_of_text = (fun text -> (Microsoft_FStar_String.split (('.')::[]) text))

let path_of_ns = (fun ns -> (Microsoft_FStar_List.map text_of_id ns))

let path_of_lid = (fun lid -> (Microsoft_FStar_List.map text_of_id (Microsoft_FStar_List.append lid.ns ((lid.ident)::[]))))

let ids_of_lid = (fun lid -> (Microsoft_FStar_List.append lid.ns ((lid.ident)::[])))

let lid_of_ids = (fun ids -> (let _22_354 = (Microsoft_FStar_Util.prefix ids)
in (match (_22_354) with
| (ns, id) -> begin
(let nsstr = (let _88_1403 = (Microsoft_FStar_List.map text_of_id ns)
in (All.pipe_right _88_1403 text_of_path))
...
let lid_of_path = (fun path pos -> (let ids = (Microsoft_FStar_List.map (fun s -> (mk_ident (s, pos))) path)
in (lid_of_ids ids)))
...
| (Microsoft_FStar_Util.Inl (_22_369), Microsoft_FStar_Util.Inr (_22_372)) -> begin
(- (1))
end
| (Microsoft_FStar_Util.Inr (_22_376), Microsoft_FStar_Util.Inl (_22_379)) -> begin
1
end
| (Microsoft_FStar_Util.Inl (x), Microsoft_FStar_Util.Inl (y)) -> begin
(Microsoft_FStar_String.compare x.realname.idText y.realname.idText)
end
...
| Microsoft_FStar_Util.Inr (l) -> begin
(range_of_lid l)
end))
...
| () -> begin
(Microsoft_FStar_Util.mk_ref None)
end))
...
| () -> begin
(Microsoft_FStar_Util.new_set (fun x y -> (Microsoft_FStar_Util.compare x.v.realname.idText y.v.realname.idText)) (fun x -> (Microsoft_FStar_Util.hashcode x.v.realname.idText)))
end))

let new_uv_set = (fun _22_414 -> (match (()) with
| () -> begin
(Microsoft_FStar_Util.new_set (fun x y -> ((Microsoft_FStar_Unionfind.uvar_id x) - (Microsoft_FStar_Unionfind.uvar_id y))) Microsoft_FStar_Unionfind.uvar_id)
end))

let new_uvt_set = (fun _22_417 -> (match (()) with
| () -> begin
(Microsoft_FStar_Util.new_set (fun _22_425 _22_429 -> (match ((_22_425, _22_429)) with
| ((x, _22_424), (y, _22_428)) -> begin
((Microsoft_FStar_Unionfind.uvar_id x) - (Microsoft_FStar_Unionfind.uvar_id y))
end)) (fun _22_421 -> (match (_22_421) with
| (x, _22_420) -> begin
(Microsoft_FStar_Unionfind.uvar_id x)
end)))
end))
...
let memo_no_uvs = (Microsoft_FStar_Util.mk_ref (Some (no_uvs)))

let memo_no_fvs = (Microsoft_FStar_Util.mk_ref (Some (no_fvs)))

let freevars_of_list = (fun l -> (All.pipe_right l (Microsoft_FStar_List.fold_left (fun out _22_1 -> (match (_22_1) with
| Microsoft_FStar_Util.Inl (btv) -> begin
(let _22_435 = out
in (let _88_1460 = (Microsoft_FStar_Util.set_add btv out.ftvs)
in {ftvs = _88_1460; fxvs = _22_435.fxvs}))
end
| Microsoft_FStar_Util.Inr (bxv) -> begin
(let _22_439 = out
in (let _88_1461 = (Microsoft_FStar_Util.set_add bxv out.fxvs)
in {ftvs = _22_439.ftvs; fxvs = _88_1461}))
end)) no_fvs)))

let list_of_freevars = (fun fvs -> (let _88_1469 = (let _88_1465 = (Microsoft_FStar_Util.set_elements fvs.ftvs)
in (All.pipe_right _88_1465 (Microsoft_FStar_List.map (fun x -> Microsoft_FStar_Util.Inl (x)))))
in (let _88_1468 = (let _88_1467 = (Microsoft_FStar_Util.set_elements fvs.fxvs)
in (All.pipe_right _88_1467 (Microsoft_FStar_List.map (fun x -> Microsoft_FStar_Util.Inr (x)))))
in (Microsoft_FStar_List.append _88_1469 _88_1468))))

let get_unit_ref = (fun _22_444 -> (match (()) with
| () -> begin
(let x = (Microsoft_FStar_Util.mk_ref (Some (())))
in (let _22_446 = (ST.op_Colon_Equals x None)
in x))
end))
...
(mk_Kind_arrow ((Microsoft_FStar_List.append bs bs'), k') p)
end
| _22_468 -> begin
...
| () -> begin
(let x = (Microsoft_FStar_Util.mk_ref (Some (mk_Kind_unknown)))
in (let _22_482 = (ST.op_Colon_Equals x None)
in x))
end))

let get_knd_ref = (fun k -> (let x = (Microsoft_FStar_Util.mk_ref (Some (mk_Kind_unknown)))
in (let _22_486 = (ST.op_Colon_Equals x k)
in x)))
...
in (let _88_1551 = (Microsoft_FStar_Util.mk_ref k)
...
| (x, phi) -> begin
(let _88_1561 = (Microsoft_FStar_Util.mk_ref k)
...
| [] -> begin
(All.failwith "Empty arg list!")
end
| _22_517 -> begin
Typ_app ((t1, args))
end)
in (let _88_1570 = (Microsoft_FStar_Util.mk_ref k)
...
(mk_Typ_app (h, (Microsoft_FStar_List.append args ((arg)::[]))) k p)
end
| _22_536 -> begin
...
in (let _88_1592 = (Microsoft_FStar_Util.mk_ref k)
...
| (t, k) -> begin
(let _88_1608 = (Microsoft_FStar_Util.mk_ref k')
...
let mk_Typ_meta' = (fun m k p -> (let _88_1621 = (Microsoft_FStar_Util.mk_ref k)
...
(All.failwith "NESTED DELAYED TYPES!")
end
| _22_614 -> begin
Typ_delayed ((Microsoft_FStar_Util.Inl ((t, s)), m))
end)
in (let _88_1652 = (Microsoft_FStar_Util.mk_ref k)
in (let _88_1651 = (mk_fvs ())
in (let _88_1650 = (mk_uvs ())
in {n = _88_1653; tk = _88_1652; pos = p; fvs = _88_1651; uvs = _88_1650}))))
end))

let mk_Typ_delayed' = (fun st k p -> (let _88_1675 = (let _88_1671 = (let _88_1670 = (Microsoft_FStar_Util.mk_ref None)
in (st, _88_1670))
in Typ_delayed (_88_1671))
in (let _88_1674 = (Microsoft_FStar_Util.mk_ref k)
...
| () -> begin
(let x = (Microsoft_FStar_Util.mk_ref (Some (mk_Typ_unknown)))
in (let _22_620 = (ST.op_Colon_Equals x None)
in x))
end))

let get_typ_ref = (fun t -> (let x = (Microsoft_FStar_Util.mk_ref (Some (mk_Typ_unknown)))
in (let _22_624 = (ST.op_Colon_Equals x t)
in x)))

let mk_Total = (fun t -> (let _88_1687 = (Microsoft_FStar_Util.mk_ref None)
in (let _88_1686 = (mk_fvs ())
in (let _88_1685 = (mk_uvs ())
in {n = Total (t); tk = _88_1687; pos = t.pos; fvs = _88_1686; uvs = _88_1685}))))

let mk_Comp = (fun ct -> (let _88_1692 = (Microsoft_FStar_Util.mk_ref None)
...
| [] -> begin
(All.failwith "abstraction with no binders!")
end
| _22_646 -> begin
Exp_abs ((b, e))
end)
...
Exp_abs (((Microsoft_FStar_List.append b ((b0)::bs)), body))
end
| ([], _22_663) -> begin
(All.failwith "abstraction with no binders!")
end
...
(mk_Exp_app (e1', (Microsoft_FStar_List.append args' args)) t p)
end
| _22_685 -> begin
...
(let vars = (Microsoft_FStar_List.collect (fun _22_705 -> (match (_22_705) with
| (x, _22_704) -> begin
(pat_vars x)
end)) ps)
in (match ((All.pipe_right vars (Microsoft_FStar_Util.nodups (fun x y -> (match ((x, y)) with
| (Microsoft_FStar_Util.Inl (x), Microsoft_FStar_Util.Inl (y)) -> begin
(bvd_eq x y)
end
| (Microsoft_FStar_Util.Inr (x), Microsoft_FStar_Util.Inr (y)) -> begin
...
| Pat_var (x) -> begin
(Microsoft_FStar_Util.Inr (x.v))::[]
end
| Pat_tvar (a) -> begin
(Microsoft_FStar_Util.Inl (a.v))::[]
end
| Pat_disj (ps) -> begin
(let vars = (Microsoft_FStar_List.map pat_vars ps)
in (match ((not ((let _88_1770 = (Microsoft_FStar_List.tl vars)
in (let _88_1769 = (let _88_1768 = (let _88_1767 = (Microsoft_FStar_List.hd vars)
in (Microsoft_FStar_Util.set_eq order_bvd _88_1767))
in (Microsoft_FStar_Util.for_all _88_1768))
in (All.pipe_right _88_1770 _88_1769)))))) with
| true -> begin
(let vars = (let _88_1774 = (All.pipe_right vars (Microsoft_FStar_List.map (fun v -> (let _88_1773 = (Microsoft_FStar_List.map (fun _22_2 -> (match (_22_2) with
...
in (Microsoft_FStar_Util.concat_l ", " _88_1773)))))
in (Microsoft_FStar_Util.concat_l ";\n" _88_1774))
in (let _88_1777 = (let _88_1776 = (let _88_1775 = (Microsoft_FStar_Util.format1 "Each branch of this pattern binds different variables: %s" vars)
in (_88_1775, p.p))
in Error (_88_1776))
in (Prims.raise _88_1777)))
end
| false -> begin
(Microsoft_FStar_List.hd vars)
end))
end
...
| (Microsoft_FStar_Util.Inl (t), _22_802) -> begin
t.pos
end
| (Microsoft_FStar_Util.Inr (e), _22_807) -> begin
e.pos
end))
...
let t_binder = (fun a -> (Microsoft_FStar_Util.Inl (a), None))

let v_binder = (fun a -> (Microsoft_FStar_Util.Inr (a), None))

let null_t_binder = (fun t -> (let _88_1857 = (let _88_1856 = (null_bvar t)
in Microsoft_FStar_Util.Inl (_88_1856))
in (_88_1857, None)))

let null_v_binder = (fun t -> (let _88_1861 = (let _88_1860 = (null_bvar t)
in Microsoft_FStar_Util.Inr (_88_1860))
in (_88_1861, None)))

let itarg = (fun t -> (Microsoft_FStar_Util.Inl (t), Some (Implicit)))

let ivarg = (fun v -> (Microsoft_FStar_Util.Inr (v), Some (Implicit)))

let targ = (fun t -> (Microsoft_FStar_Util.Inl (t), None))

let varg = (fun v -> (Microsoft_FStar_Util.Inr (v), None))
...
| (Microsoft_FStar_Util.Inl (a), _22_829) -> begin
(is_null_bvar a)
end
| (Microsoft_FStar_Util.Inr (x), _22_834) -> begin
(is_null_bvar x)
end))

let freevars_of_binders = (fun bs -> (All.pipe_right bs (Microsoft_FStar_List.fold_left (fun out _22_3 -> (match (_22_3) with
| (Microsoft_FStar_Util.Inl (btv), _22_842) -> begin
(let _22_844 = out
in (let _88_1882 = (Microsoft_FStar_Util.set_add btv out.ftvs)
in {ftvs = _88_1882; fxvs = _22_844.fxvs}))
end
| (Microsoft_FStar_Util.Inr (bxv), _22_849) -> begin
(let _22_851 = out
in (let _88_1883 = (Microsoft_FStar_Util.set_add bxv out.fxvs)
in {ftvs = _22_851.ftvs; fxvs = _88_1883}))
end)) no_fvs)))

let binders_of_list = (fun fvs -> (All.pipe_right fvs (Microsoft_FStar_List.map (fun t -> (t, None)))))

let binders_of_freevars = (fun fvs -> (let _88_1892 = (let _88_1889 = (Microsoft_FStar_Util.set_elements fvs.ftvs)
in (All.pipe_right _88_1889 (Microsoft_FStar_List.map t_binder)))
in (let _88_1891 = (let _88_1890 = (Microsoft_FStar_Util.set_elements fvs.fxvs)
in (All.pipe_right _88_1890 (Microsoft_FStar_List.map v_binder)))
in (Microsoft_FStar_List.append _88_1892 _88_1891))))
