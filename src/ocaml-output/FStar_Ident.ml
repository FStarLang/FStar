...
let mk_ident: (Prims.string* FStar_Range.range) -> ident =
  fun uu____74  ->
    match uu____74 with | (text,range) -> { idText = text; idRange = range }
let reserved_prefix: Prims.string = "uu___"
let gen: FStar_Range.range -> ident =
  let x = FStar_Util.mk_ref (Prims.parse_int "0") in
  fun r  ->
    (let uu____86 =
       let uu____87 = FStar_ST.read x in uu____87 + (Prims.parse_int "1") in
     FStar_ST.write x uu____86);
    (let uu____92 =
       let uu____95 =
         let uu____96 =
           let uu____97 = FStar_ST.read x in Prims.string_of_int uu____97 in
         Prims.strcat reserved_prefix uu____96 in
       (uu____95, r) in
     mk_ident uu____92)
...
let lid_of_ns_and_id: ident Prims.list -> ident -> lident =
  fun ns  ->
    fun id  ->
      let nsstr =
        let uu____138 = FStar_List.map text_of_id ns in
        FStar_All.pipe_right uu____138 text_of_path in
      {
        ns;
        ident = id;
        nsstr;
...
let lid_of_ids: ident Prims.list -> lident =
  fun ids  ->
    let uu____147 = FStar_Util.prefix ids in
    match uu____147 with | (ns,id) -> lid_of_ns_and_id ns id
let lid_of_str: Prims.string -> lident =
  fun str  ->
    let uu____158 = FStar_List.map id_of_text (FStar_Util.split str ".") in
    lid_of_ids uu____158
...
let set_lid_range: lident -> FStar_Range.range -> lident =
  fun l  ->
    fun r  ->
      let uu___47_195 = l in
      {
        ns = (uu___47_195.ns);
        ident =
          (let uu___48_196 = l.ident in
           { idText = (uu___48_196.idText); idRange = r });
        nsstr = (uu___47_195.nsstr);
        str = (uu___47_195.str)
      }
...
let string_of_lid: lident -> Prims.string =
  fun lid  -> let uu____208 = path_of_lid lid in text_of_path uu____208