...
let (__proj__Mklident__item__ident : lident -> ident) =
  fun projectee -> match projectee with | { ns; ident; nsstr; str;_} -> ident
let (__proj__Mklident__item__nsstr : lident -> Prims.string) =
  fun projectee -> match projectee with | { ns; ident; nsstr; str;_} -> nsstr
...
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid ->
    let uu____519 =
      ...
      let uu____527 = let uu____531 = text_of_id lid.ident in [uu____531] in
