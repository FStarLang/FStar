...
| {idText = idText; idRange = idRange} -> begin
idText
end))
...
| {ns = ns; ident = ident; nsstr = nsstr; str = str} -> begin
ns
end))
...
let mk_ident : (Prims.string * FStar_Range.range)  ->  ident = (fun uu____137 -> (match (uu____137) with
| (text, range) -> begin
...
let next_id = (fun uu____174 -> (

let v1 = (FStar_ST.op_Bang x)
in ((FStar_ST.op_Colon_Equals x (v1 + (Prims.parse_int "1")));
v1;
)))
in (

let reset = (fun uu____226 -> (FStar_ST.op_Colon_Equals x (Prims.parse_int "0")))
in ((next_id), (reset)))))


let next_id : unit  ->  Prims.int = (fun uu____262 -> (FStar_Pervasives_Native.fst _gen ()))


let reset_gensym : unit  ->  unit = (fun uu____275 -> (FStar_Pervasives_Native.snd _gen ()))


let gen : FStar_Range.range  ->  ident = (fun r -> (

let i = (next_id ())
in (mk_ident (((Prims.op_Hat reserved_prefix (Prims.string_of_int i))), (r)))))
...
let uu____376 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____376 text_of_path))
in {ns = ns; ident = id1; nsstr = nsstr; str = (match ((Prims.op_Equality nsstr "")) with
| true -> begin
id1.idText
end
| uu____383 -> begin
(Prims.op_Hat nsstr (Prims.op_Hat "." id1.idText))
end)}))


let lid_of_ids : ident Prims.list  ->  lident = (fun ids -> (

let uu____396 = (FStar_Util.prefix ids)
...
let uu____417 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____417)))
...
let range_of_lid : lident  ->  FStar_Range.range = (fun lid -> (range_of_id lid.ident))


let set_lid_range : lident  ->  FStar_Range.range  ->  lident = (fun l r -> (

let uu___61_491 = l
in {ns = uu___61_491.ns; ident = (

let uu___63_493 = l.ident
in {idText = uu___63_493.idText; idRange = r}); nsstr = uu___61_491.nsstr; str = uu___61_491.str}))
...
let uu____508 = (range_of_lid l)
in (lid_of_path (FStar_List.append path ((s)::[])) uu____508))))
...
let uu____523 = (path_of_ns lid.ns)
in (

let uu____527 = (

let uu____531 = (text_of_id lid.ident)
in (uu____531)::[])
in (FStar_List.append uu____523 uu____527)))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____519)))
...
let uu____555 = (path_of_lid lid)
in (text_of_path uu____555)))
