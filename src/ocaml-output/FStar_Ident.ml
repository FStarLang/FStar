...
let (_gen : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let next_id uu____169 =
    let v1 = FStar_ST.op_Bang x  in
    FStar_ST.op_Colon_Equals x (v1 + (Prims.parse_int "1")); v1  in
  let reset uu____265 = FStar_ST.op_Colon_Equals x (Prims.parse_int "0")  in
  (next_id, reset) 
let (next_id : unit -> Prims.int) =
  fun uu____323  -> FStar_Pervasives_Native.fst _gen () 
let (reset_gensym : unit -> unit) =
  fun uu____336  -> FStar_Pervasives_Native.snd _gen () 
...
let (lid_of_ns_and_id : ident Prims.list -> ident -> lident) =
  fun ns  ->
    fun id1  ->
      let nsstr =
        let uu____437 = FStar_List.map text_of_id ns  in
        FStar_All.pipe_right uu____437 text_of_path  in
      {
        ns;
        ident = id1;
        nsstr;
...
let (lid_of_ids : ident Prims.list -> lident) =
  fun ids  ->
    let uu____457 = FStar_Util.prefix ids  in
    match uu____457 with | (ns,id1) -> lid_of_ns_and_id ns id1
  
let (lid_of_str : Prims.string -> lident) =
  fun str  ->
    let uu____478 = FStar_List.map id_of_text (FStar_Util.split str ".")  in
    lid_of_ids uu____478
...
let (set_lid_range : lident -> FStar_Range.range -> lident) =
  fun l  ->
    fun r  ->
      let uu___11_552 = l  in
      {
        ns = (uu___11_552.ns);
        ident =
          (let uu___12_554 = l.ident  in
           { idText = (uu___12_554.idText); idRange = r });
        nsstr = (uu___11_552.nsstr);
        str = (uu___11_552.str)
      }
...
let (lid_add_suffix : lident -> Prims.string -> lident) =
  fun l  ->
    fun s  ->
      ...
      let uu____569 = range_of_lid l  in
      lid_of_path (FStar_List.append path [s]) uu____569
  
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid  ->
    let uu____580 =
      let uu____584 = path_of_ns lid.ns  in
      let uu____588 = let uu____592 = text_of_id lid.ident  in [uu____592]
         in
      FStar_List.append uu____584 uu____588  in
    FStar_All.pipe_left (FStar_String.concat "_") uu____580
  
let (string_of_ident : ident -> Prims.string) = fun id1  -> id1.idText 
let (string_of_lid : lident -> Prims.string) =
  fun lid  -> let uu____616 = path_of_lid lid  in text_of_path uu____616 