...
let (mk_ident :
  ...
  fun uu____99  ->
    match uu____99 with | (text,range) -> { idText = text; idRange = range }
...
let (lid_of_ns_and_id : ident Prims.list -> ident -> lident) =
  fun ns  ->
    fun id1  ->
      let nsstr =
        let uu____324 = FStar_List.map text_of_id ns  in
        FStar_All.pipe_right uu____324 text_of_path  in
      {
        ns;
        ident = id1;
        nsstr;
...
let (lid_of_ids : ident Prims.list -> lident) =
  fun ids  ->
    let uu____339 = FStar_Util.prefix ids  in
    match uu____339 with | (ns,id1) -> lid_of_ns_and_id ns id1
  
let (lid_of_str : Prims.string -> lident) =
  fun str  ->
    let uu____357 = FStar_List.map id_of_text (FStar_Util.split str ".")  in
    lid_of_ids uu____357
...
let (set_lid_range : lident -> FStar_Range.range -> lident) =
  fun l  ->
    fun r  ->
      let uu___25_415 = l  in
      {
        ns = (uu___25_415.ns);
        ident =
          (let uu___26_417 = l.ident  in
           { idText = (uu___26_417.idText); idRange = r });
        nsstr = (uu___25_415.nsstr);
        str = (uu___25_415.str)
      }
...
let (lid_add_suffix : lident -> Prims.string -> lident) =
  fun l  ->
    fun s  ->
      ...
      let uu____429 = range_of_lid l  in
      lid_of_path (FStar_List.append path [s]) uu____429
  
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid  ->
    let uu____435 =
      let uu____438 = path_of_ns lid.ns  in
      let uu____441 = let uu____444 = text_of_id lid.ident  in [uu____444]
         in
      FStar_List.append uu____438 uu____441  in
    FStar_All.pipe_left (FStar_String.concat "_") uu____435
  
let (string_of_ident : ident -> Prims.string) = fun id1  -> id1.idText 
let (string_of_lid : lident -> Prims.string) =
  fun lid  -> let uu____457 = path_of_lid lid  in text_of_path uu____457 