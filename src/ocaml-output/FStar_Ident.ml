...
let mk_ident : (Prims.string * FStar_Range.range)  ->  ident = (fun uu____132 -> (match (uu____132) with
| (text, range) -> begin
...
let uu____160 = (FStar_ST.op_Bang x)
in (uu____160 + (Prims.parse_int "1")))
in (FStar_ST.op_Colon_Equals x uu____158));
(
...
let uu____259 = (FStar_ST.op_Bang x)
in (Prims.string_of_int uu____259))
in (Prims.strcat reserved_prefix uu____257))
in ((uu____255), (r)))
in (mk_ident uu____249));
)))
...
let uu____395 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____395 text_of_path))
...
let uu____415 = (FStar_Util.prefix ids)
...
let uu____436 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____436)))
...
let uu___81_510 = l
in {ns = uu___81_510.ns; ident = (

let uu___82_512 = l.ident
in {idText = uu___82_512.idText; idRange = r}); nsstr = uu___81_510.nsstr; str = uu___81_510.str}))
...
let uu____527 = (range_of_lid l)
in (lid_of_path (FStar_List.append path ((s)::[])) uu____527))))
...
let uu____542 = (path_of_ns lid.ns)
in (

let uu____546 = (

let uu____550 = (text_of_id lid.ident)
in (uu____550)::[])
in (FStar_List.append uu____542 uu____546)))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____538)))
...
let uu____574 = (path_of_lid lid)
in (text_of_path uu____574)))
