...
let gen: FStar_Range.range -> ident =
  ...
  fun r  ->
    (let uu____76 =
       let uu____77 = FStar_ST.read x in uu____77 + (Prims.parse_int "1") in
     FStar_ST.write x uu____76);
    (let uu____82 =
       let uu____85 =
         let uu____86 =
           let uu____87 = FStar_ST.read x in Prims.string_of_int uu____87 in
         Prims.strcat reserved_prefix uu____86 in
       (uu____85, r) in
     mk_ident uu____82)
...
let lid_of_ns_and_id: ident Prims.list -> ident -> lident =
  fun ns  ->
    fun id  ->
      let nsstr =
        let uu____137 = FStar_List.map text_of_id ns in
        FStar_All.pipe_right uu____137 text_of_path in
      {
        ns;
        ident = id;
        nsstr;
...
let lid_of_str: Prims.string -> lident =
  fun str  ->
    let uu____159 = FStar_List.map id_of_text (FStar_Util.split str ".") in
    lid_of_ids uu____159
...
let set_lid_range: lident -> FStar_Range.range -> lident =
  fun l  ->
    fun r  ->
      let uu___47_206 = l in
      {
        ns = (uu___47_206.ns);
        ident =
          (let uu___48_207 = l.ident in
           { idText = (uu___48_207.idText); idRange = r });
        nsstr = (uu___47_206.nsstr);
        str = (uu___47_206.str)
      }
