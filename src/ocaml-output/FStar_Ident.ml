...
let (gen : FStar_Range.range -> ident) =
  ...
  fun r ->
    ...
    (let uu____291 =
       let uu____296 =
         let uu____297 =
           let uu____298 = FStar_ST.op_Bang x in
           Prims.string_of_int uu____298 in
         Prims.strcat reserved_prefix uu____297 in
       (uu____296, r) in
     mk_ident uu____291)
...
let (lid_of_ns_and_id : ident Prims.list -> ident -> lident) =
  fun ns ->
    fun id1 ->
      let nsstr =
        let uu____445 = FStar_List.map text_of_id ns in
        FStar_All.pipe_right uu____445 text_of_path in
      {
        ns;
        ident = id1;
        nsstr;
...
let (lid_of_ids : ident Prims.list -> lident) =
  fun ids ->
    let uu____458 = FStar_Util.prefix ids in
    match uu____458 with | (ns, id1) -> lid_of_ns_and_id ns id1
let (lid_of_str : Prims.string -> lident) =
  fun str ->
    let uu____474 = FStar_List.map id_of_text (FStar_Util.split str ".") in
    lid_of_ids uu____474
...
let (set_lid_range : lident -> FStar_Range.range -> lident) =
  fun l ->
    fun r ->
      let uu___26_516 = l in
      {
        ns = (uu___26_516.ns);
        ident =
          (let uu___27_518 = l.ident in
           { idText = (uu___27_518.idText); idRange = r });
        nsstr = (uu___26_516.nsstr);
        str = (uu___26_516.str)
      }
...
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid ->
    let uu____531 =
      let uu____534 = path_of_ns lid.ns in
      FStar_List.append uu____534 [text_of_id lid.ident] in
    FStar_All.pipe_left (FStar_String.concat "_") uu____531
let (string_of_ident : ident -> Prims.string) = fun id1 -> id1.idText
let (string_of_lid : lident -> Prims.string) =
  fun lid -> let uu____545 = path_of_lid lid in text_of_path uu____545