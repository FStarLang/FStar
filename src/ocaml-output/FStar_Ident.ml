...
let (lid_of_ns_and_id : ident Prims.list -> ident -> lident) =
  fun ns  ->
    fun id1  ->
      let nsstr =
        let uu____34766 = FStar_List.map text_of_id ns  in
        FStar_All.pipe_right uu____34766 text_of_path  in
      {
        ns;
        ident = id1;
        nsstr;
...
let (lid_of_ids : ident Prims.list -> lident) =
  fun ids  ->
    let uu____34786 = FStar_Util.prefix ids  in
    match uu____34786 with | (ns,id1) -> lid_of_ns_and_id ns id1
  
let (lid_of_str : Prims.string -> lident) =
  fun str  ->
    let uu____34807 = FStar_List.map id_of_text (FStar_Util.split str ".")
       in
    lid_of_ids uu____34807
...
let (set_lid_range : lident -> FStar_Range.range -> lident) =
  fun l  ->
    fun r  ->
      let uu___350_34881 = l  in
      {
        ns = (uu___350_34881.ns);
        ident =
          (let uu___352_34883 = l.ident  in
           { idText = (uu___352_34883.idText); idRange = r });
        nsstr = (uu___350_34881.nsstr);
        str = (uu___350_34881.str)
      }
...
let (lid_add_suffix : lident -> Prims.string -> lident) =
  fun l  ->
    fun s  ->
      ...
      let uu____34898 = range_of_lid l  in
      lid_of_path (FStar_List.append path [s]) uu____34898
  
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid  ->
    let uu____34909 =
      let uu____34913 = path_of_ns lid.ns  in
      let uu____34917 =
        let uu____34921 = text_of_id lid.ident  in [uu____34921]  in
      FStar_List.append uu____34913 uu____34917  in
    FStar_All.pipe_left (FStar_String.concat "_") uu____34909
  
let (string_of_ident : ident -> Prims.string) = fun id1  -> id1.idText 
let (string_of_lid : lident -> Prims.string) =
  fun lid  -> let uu____34945 = path_of_lid lid  in text_of_path uu____34945 