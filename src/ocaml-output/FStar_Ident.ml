...
let gen : FStar_Range.range -> ident =
  ...
  fun r  ->
    ...
    (let uu____225 =
       let uu____230 =
         let uu____231 =
           let uu____232 = FStar_ST.op_Bang x  in
           Prims.string_of_int uu____232  in
         Prims.strcat reserved_prefix uu____231  in
       (uu____230, r)  in
     mk_ident uu____225)
...
let lid_of_ns_and_id : ident Prims.list -> ident -> lident =
  fun ns  ->
    fun id1  ->
      let nsstr =
        let uu____346 = FStar_List.map text_of_id ns  in
        FStar_All.pipe_right uu____346 text_of_path  in
      {
        ns;
        ident = id1;
        nsstr;
...
let lid_of_ids : ident Prims.list -> lident =
  fun ids  ->
    let uu____359 = FStar_Util.prefix ids  in
    match uu____359 with | (ns,id1) -> lid_of_ns_and_id ns id1
  
let lid_of_str : Prims.string -> lident =
  fun str  ->
    let uu____375 = FStar_List.map id_of_text (FStar_Util.split str ".")  in
    lid_of_ids uu____375
...
let set_lid_range : lident -> FStar_Range.range -> lident =
  fun l  ->
    fun r  ->
      let uu___28_417 = l  in
      {
        ns = (uu___28_417.ns);
        ident =
          (let uu___29_419 = l.ident  in
           { idText = (uu___29_419.idText); idRange = r });
        nsstr = (uu___28_417.nsstr);
        str = (uu___28_417.str)
      }
...
let ml_path_of_lid : lident -> Prims.string =
  fun lid  ->
    let uu____432 =
      let uu____435 = path_of_ns lid.ns  in
      FStar_List.append uu____435 [text_of_id lid.ident]  in
    FStar_All.pipe_left (FStar_String.concat "_") uu____432
  
let string_of_lid : lident -> Prims.string =
  fun lid  -> let uu____443 = path_of_lid lid  in text_of_path uu____443 