...
let __proj__Mkident__item__idText : ident -> Prims.string =
  fun projectee ->
    match projectee
    with {idText= __fname__idText; idRange= __fname__idRange; _} ->
      __fname__idText
...
let __proj__Mklident__item__ns : lident -> ident Prims.list =
  fun projectee ->
    ...
    with
    | { ns= __fname__ns
      ; ident= __fname__ident
      ; nsstr= __fname__nsstr
      ; str= __fname__str; _ }
    -> __fname__ns
    ...
    let uu____183 =
      let uu____188 =
        let uu____189 =
          ...
          Prims.string_of_int uu____190
        in
        Prims.strcat reserved_prefix uu____189
      in
      (uu____188, r)
    in
    mk_ident uu____183
...
let lid_of_ns_and_id : ident Prims.list -> ident -> lident =
  fun ns id1 ->
    let nsstr =
      ...
      FStar_All.pipe_right uu____283 text_of_path
    in
    { ns
    ; ident= id1
    ; nsstr
    ; str=
        ( if nsstr = "" then id1.idText
...
let ident_equals : ident -> ident -> Prims.bool =
  fun id1 id2 -> id1.idText = id2.idText
...
let ml_path_of_lid : lident -> Prims.string =
  fun lid ->
    let uu____369 =
      ...
      FStar_List.append uu____372 [text_of_id lid.ident]
    in
...
let string_of_lid : lident -> Prims.string =
  fun lid ->
    let uu____380 = path_of_lid lid in
    text_of_path uu____380
