...
let mk_ident : (Prims.string * FStar_Range.range)  ->  ident = (fun uu____90 -> (match (uu____90) with
| (text, range) -> begin
...
let uu____106 = (FStar_ST.op_Bang x)
in (uu____106 + (Prims.parse_int "1")))
in (FStar_ST.op_Colon_Equals x uu____105));
(
...
let uu____162 = (FStar_ST.op_Bang x)
in (Prims.string_of_int uu____162))
in (Prims.strcat reserved_prefix uu____161))
in ((uu____160), (r)))
in (mk_ident uu____155));
)))
...
let uu____244 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____244 text_of_path))
...
let uu____258 = (FStar_Util.prefix ids)
...
let uu____275 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____275)))
...
let uu___47_327 = l
in {ns = uu___47_327.ns; ident = (

let uu___48_329 = l.ident
in {idText = uu___48_329.idText; idRange = r}); nsstr = uu___47_327.nsstr; str = uu___47_327.str}))
...
let uu____348 = (path_of_ns lid.ns)
in (FStar_List.append uu____348 (((text_of_id lid.ident))::[])))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____345)))


let string_of_lid : lident  ->  Prims.string = (fun lid -> (

let uu____357 = (path_of_lid lid)
in (text_of_path uu____357)))
