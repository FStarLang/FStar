...
let mk_ident : (Prims.string * FStar_Range.range)  ->  ident = (fun uu____56 -> (match (uu____56) with
| (text, range) -> begin
...
let uu____69 = (FStar_ST.read x)
in (uu____69 + (Prims.parse_int "1")))
in (FStar_ST.write x uu____68));
(
...
let uu____79 = (FStar_ST.read x)
in (Prims.string_of_int uu____79))
in (Prims.strcat reserved_prefix uu____78))
in ((uu____77), (r)))
in (mk_ident uu____74));
)))
...
let uu____120 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____120 text_of_path))
...
let uu____129 = (FStar_Util.prefix ids)
...
let uu____140 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____140)))
...
let uu___47_177 = l
in {ns = uu___47_177.ns; ident = (

let uu___48_178 = l.ident
in {idText = uu___48_178.idText; idRange = r}); nsstr = uu___47_177.nsstr; str = uu___47_177.str}))
...
let uu____190 = (path_of_lid lid)
in (text_of_path uu____190)))
