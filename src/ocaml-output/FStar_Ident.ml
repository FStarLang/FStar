...
let (_gen : FStar_Range.range -> ident) =
  ...
  fun r ->
    ...
    (let uu____321 =
       let uu____326 =
         let uu____327 =
           let uu____328 = FStar_ST.op_Bang x in
           Prims.string_of_int uu____328 in
         Prims.strcat reserved_prefix uu____327 in
       (uu____326, r) in
     mk_ident uu____321)
...
let (path_of_lid : lident -> path) =
  fun lid -> FStar_List.map text_of_id (FStar_List.append lid.ns [lid.ident])
...
let (lid_of_ns_and_id : ident Prims.list -> ident -> lident) =
  fun ns ->
    fun id1 ->
      let nsstr =
        let uu____492 = FStar_List.map text_of_id ns in
        FStar_All.pipe_right uu____492 text_of_path in
      {
        ns;
        ident = id1;
        nsstr;
...
let (lid_of_ids : ident Prims.list -> lident) =
  fun ids ->
    let uu____507 = FStar_Util.prefix ids in
    match uu____507 with | (ns, id1) -> lid_of_ns_and_id ns id1
let (lid_of_str : Prims.string -> lident) =
  fun str ->
    let uu____525 = FStar_List.map id_of_text (FStar_Util.split str ".") in
    lid_of_ids uu____525
...
let (set_lid_range : lident -> FStar_Range.range -> lident) =
  fun l ->
    fun r ->
      let uu___28_583 = l in
      {
        ns = (uu___28_583.ns);
        ident =
          (let uu___29_585 = l.ident in
           { idText = (uu___29_585.idText); idRange = r });
        nsstr = (uu___28_583.nsstr);
        str = (uu___28_583.str)
      }
let (lid_add_suffix : lident -> Prims.string -> lident) =
  fun l ->
    fun s ->
      ...
      let uu____597 = range_of_lid l in
      lid_of_path (FStar_List.append path [s]) uu____597
let (ml_path_of_lid : lident -> Prims.string) =
  fun lid ->
    let uu____603 =
      let uu____606 = path_of_ns lid.ns in
      let uu____609 = let uu____612 = text_of_id lid.ident in [uu____612] in
      FStar_List.append uu____606 uu____609 in
    FStar_All.pipe_left (FStar_String.concat "_") uu____603
let (string_of_ident : ident -> Prims.string) = fun id1 -> id1.idText
let (string_of_lid : lident -> Prims.string) =
  fun lid -> let uu____625 = path_of_lid lid in text_of_path uu____625