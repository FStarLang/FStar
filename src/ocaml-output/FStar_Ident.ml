...
let uu____234 = (FStar_ST.op_Bang x)
in (Prims.string_of_int uu____234))
in (Prims.strcat reserved_prefix uu____233))
in ((uu____232), (r)))
in (mk_ident uu____227));
)))
...
let uu____352 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____352 text_of_path))
...
let uu____366 = (FStar_Util.prefix ids)
...
let uu____383 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____383)))
...
let uu___46_435 = l
in {ns = uu___46_435.ns; ident = (

let uu___47_437 = l.ident
in {idText = uu___47_437.idText; idRange = r}); nsstr = uu___46_435.nsstr; str = uu___46_435.str}))
...
let uu____456 = (path_of_ns lid.ns)
in (FStar_List.append uu____456 (((text_of_id lid.ident))::[])))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____453)))


let string_of_lid : lident  ->  Prims.string = (fun lid -> (

let uu____465 = (path_of_lid lid)
in (text_of_path uu____465)))
