    ...
    if b then ()
      ...
      let uu____8 =
        let uu____13 =
          ...
          FStar_Util.format1 "Assertion failed: test %s" uu____14
        in
        (FStar_Errors.Fatal_AssertionFailure, uu____13)
      in
    ...
    let uu____43 =
      let uu____46 =
        let uu____47 =
          ...
          (x1, uu____62)
        in
        FStar_Syntax_Syntax.Tm_app uu____47
      in
      FStar_Syntax_Syntax.mk uu____46
    in
...
let rec term_eq'
  ...
  fun t1 t2 ->
    ...
    let binders_eq xs ys =
      ...
      && FStar_List.forall2
           (fun uu____101 uu____102 ->
             match (uu____101, uu____102)
             with (x1, uu____104), (y1, uu____106) ->
    ...
    let args_eq xs ys =
      FStar_List.length xs = FStar_List.length ys
      && FStar_List.forall2
           (fun uu____168 uu____169 ->
             match (uu____168, uu____169) with (a, imp), (b, imp') ->
               term_eq' a b && imp = imp' )
           xs ys
    in
    let comp_eq1 c d =
      ...
      | ( FStar_Syntax_Syntax.Total (t, uu____206)
        , FStar_Syntax_Syntax.Total (s, uu____208) ) ->
          term_eq' t s
    ...
    | ( FStar_Syntax_Syntax.Tm_uinst (t, uu____239)
      , FStar_Syntax_Syntax.Tm_uinst (s, uu____241) ) ->
        term_eq' t s
    | FStar_Syntax_Syntax.Tm_constant c1, FStar_Syntax_Syntax.Tm_constant c2 ->
        c1 = c2
    | FStar_Syntax_Syntax.Tm_type u, FStar_Syntax_Syntax.Tm_type v1 -> u = v1
    | ( FStar_Syntax_Syntax.Tm_abs (xs, t, uu____256)
      , FStar_Syntax_Syntax.Tm_abs (ys, u, uu____259) )
    ...
    | ( FStar_Syntax_Syntax.Tm_abs (xs, t, uu____310)
      ...
      -> (
        if FStar_List.length xs > FStar_List.length ys then
          let uu____362 = FStar_Util.first_N (FStar_List.length ys) xs in
          match uu____362 with xs1, xs' ->
            let t12 =
              let uu____416 =
                let uu____419 =
                  let uu____420 =
                    ...
                    in
                    (xs1, uu____437, FStar_Pervasives_Native.None)
                  in
                  FStar_Syntax_Syntax.Tm_abs uu____420
                in
                FStar_Syntax_Syntax.mk uu____419
              in
              ...
              let uu____518 =
                let uu____521 =
                  let uu____522 =
                    ...
                    in
                    (ys1, uu____539, FStar_Pervasives_Native.None)
                  in
                  FStar_Syntax_Syntax.Tm_abs uu____522
                in
                FStar_Syntax_Syntax.mk uu____521
              in
              uu____518 FStar_Pervasives_Native.None
                t21.FStar_Syntax_Syntax.pos
            in
            term_eq' t11 t22 )
    | FStar_Syntax_Syntax.Tm_arrow (xs, c), FStar_Syntax_Syntax.Tm_arrow (ys, d) ->
        binders_eq xs ys && comp_eq1 c d
    | ( FStar_Syntax_Syntax.Tm_refine (x1, t)
      , FStar_Syntax_Syntax.Tm_refine (y1, u) ) ->
        term_eq' x1.FStar_Syntax_Syntax.sort y1.FStar_Syntax_Syntax.sort
        && term_eq' t u
    | ( FStar_Syntax_Syntax.Tm_app
          ...
          , [ ( uu____616
              , FStar_Pervasives_Native.Some
                  FStar_Syntax_Syntax.Implicit uu____617 )
            ; t12
            ; t22 ] )
      , FStar_Syntax_Syntax.Tm_app
    ...
    | ( FStar_Syntax_Syntax.Tm_let ((is_rec, lbs), t)
      , FStar_Syntax_Syntax.Tm_let ((is_rec', lbs'), s) )
      when is_rec = is_rec' ->
    ...
    | uu____1328, FStar_Syntax_Syntax.Tm_delayed uu____1329 ->
        ...
        in
        failwith uu____1354
    | FStar_Syntax_Syntax.Tm_unknown, FStar_Syntax_Syntax.Tm_unknown -> true
    | uu____1357 -> false
