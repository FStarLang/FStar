...
let x2 = (FStar_All.try_with (fun uu___100_198 -> (match (()) with
| () -> begin
(FStar_List.map FStar_Util.int_of_string (FStar_Util.split x1 "."))
end)) (fun uu___99_203 -> (match (uu___99_203) with
| uu____206 -> begin
[]
end)))
in (match (x2) with
...
let status_tag : z3status  ->  Prims.string = (fun uu___97_438 -> (match (uu___97_438) with
| SAT (uu____439) -> begin
"sat"
end
...
let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  Prims.unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___98_3829 -> (match (uu___98_3829) with
