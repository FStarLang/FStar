...
let z3version_as_string : z3version  ->  Prims.string = (fun uu___96_55 -> (match (uu___96_55) with
| Z3V_Unknown (s) -> begin
...
let uu____60 = (FStar_Util.string_of_int i)
in (

let uu____61 = (FStar_Util.string_of_int j)
in (

let uu____62 = (FStar_Util.string_of_int k)
in (FStar_Util.format3 "%s.%s.%s" uu____60 uu____61 uu____62))))
end))


let z3v_compare : z3version  ->  (Prims.int * Prims.int * Prims.int)  ->  Prims.int FStar_Pervasives_Native.option = (fun known uu____74 -> (match (uu____74) with
| (w1, w2, w3) -> begin
(match (known) with
| Z3V_Unknown (uu____83) -> begin
...
let get_z3version : Prims.unit  ->  z3version = (fun uu____112 -> (

let prefix1 = "Z3 version "
in (

let uu____114 = (FStar_ST.read _z3version)
...
let uu____124 = (FStar_Options.z3_exe ())
in (FStar_Util.run_proc uu____124 "-version" ""))
in (match (uu____120) with
| (uu____125, out, uu____127) -> begin
(

let out1 = (match ((FStar_Util.splitlines out)) with
| (x)::uu____130 when (FStar_Util.starts_with x prefix1) -> begin
(

let x1 = (

let uu____133 = (FStar_Util.substring_from x (FStar_String.length prefix1))
in (FStar_Util.trim_string uu____133))
in (
...
let ini_params : Prims.unit  ->  Prims.string = (fun uu____163 -> (
...
let uu____167 = (get_z3version ())
in (z3v_le uu____167 (((Prims.parse_int "4")), ((Prims.parse_int "4")), ((Prims.parse_int "0")))))
...
let uu____170 = (z3version_as_string z3_v)
in (FStar_Util.format1 "Z3 4.5.0 recommended; at least Z3 v4.4.1 required; got %s\n" uu____170))
in FStar_Util.Failure (uu____169))
in (FStar_All.pipe_left FStar_Pervasives.raise uu____168))
end
| uu____171 -> begin
()
end));
...
let uu____180 = (FStar_Options.z3_seed ())
in (FStar_Util.string_of_int uu____180))
in (FStar_Util.format1 "smt.random_seed=%s" uu____179))
in (uu____178)::[])
in ("-smt2 -in auto_config=false model=true smt.relevancy=2")::uu____176)
in (

let uu____181 = (FStar_Options.z3_cliopt ())
in (FStar_List.append uu____174 uu____181)))
in (FStar_String.concat " " uu____172));
)))
...
let status_to_string : z3status  ->  Prims.string = (fun uu___97_281 -> (match (uu___97_281) with
| SAT (uu____282) -> begin
"sat"
end
| UNSAT (uu____284) -> begin
"unsat"
end
| UNKNOWN (uu____285) -> begin
"unknown"
end
| TIMEOUT (uu____287) -> begin
"timeout"
end
| KILLED -> begin
"killed"
...
let tid : Prims.unit  ->  Prims.string = (fun uu____292 -> (

let uu____293 = (FStar_Util.current_tid ())
in (FStar_All.pipe_right uu____293 FStar_Util.string_of_int)))
...
let uu____306 = (FStar_Options.z3_exe ())
in (

let uu____307 = (ini_params ())
in (FStar_Util.start_process id uu____306 uu____307 cond)))))
...
let get_module_name = (fun uu____623 -> (

let uu____624 = (FStar_ST.read current_module_name)
...
let new_log_file = (fun uu____633 -> (

let uu____634 = (FStar_ST.read current_module_name)
...
let uu____645 = (FStar_ST.read used_file_names)
in (FStar_List.tryFind (fun uu____659 -> (match (uu____659) with
| (m, uu____663) -> begin
(n1 = m)
end)) uu____645))
...
let uu____671 = (FStar_ST.read used_file_names)
in (((n1), ((Prims.parse_int "0"))))::uu____671)
in (FStar_ST.write used_file_names uu____667));
n1;
)
end
| FStar_Pervasives_Native.Some (uu____687, k) -> begin
((

let uu____692 = (

let uu____696 = (FStar_ST.read used_file_names)
in (((n1), ((k + (Prims.parse_int "1")))))::uu____696)
in (FStar_ST.write used_file_names uu____692));
(

let uu____712 = (FStar_Util.string_of_int (k + (Prims.parse_int "1")))
in (FStar_Util.format2 "%s-%s" n1 uu____712));
)
end))
in (
...
let get_log_file = (fun uu____726 -> (

let uu____727 = (FStar_ST.read log_file_opt)
...
let uu____737 = (get_log_file ())
in (FStar_Util.append_to_file uu____737 str)))
in (
...
let uu____743 = (FStar_ST.read current_file_name)
...
let uu____749 = (FStar_ST.read current_module_name)
...
let uu____766 = (FStar_ST.read query_number)
in (uu____766 + (Prims.parse_int "1")))
in (FStar_ST.write query_number uu____765));
(

let file_name = (

let uu____772 = (FStar_Util.string_of_int qnum)
in (FStar_Util.format1 "query-%s.smt2" uu____772))
in (
...
let uu____779 = (FStar_Options.n_cores ())
in (uu____779 > (Prims.parse_int "1")))
...
let close_log = (fun uu____784 -> (

let uu____785 = (FStar_ST.read log_file_opt)
...
let log_file_name = (fun uu____798 -> (

let uu____799 = (FStar_ST.read current_file_name)
...
let uu____818 = ((FStar_Util.incr ctr);
(

let uu____823 = (FStar_ST.read ctr)
in (FStar_All.pipe_right uu____823 FStar_Util.string_of_int));
)
in (FStar_Util.format1 "bg-%s" uu____818))
in (new_z3proc uu____817))))
in (

let z3proc = (fun uu____829 -> ((

let uu____831 = (

let uu____832 = (FStar_ST.read the_z3proc)
in (uu____832 = FStar_Pervasives_Native.None))
...
let uu____840 = (new_proc ())
in FStar_Pervasives_Native.Some (uu____840))
in (FStar_ST.write the_z3proc uu____838))
end
| uu____844 -> begin
()
end));
...
let uu____845 = (FStar_ST.read the_z3proc)
in (FStar_Util.must uu____845));
))
in (
...
let grab = (fun uu____855 -> ((FStar_Util.monitor_enter x);
(z3proc ());
))
in (

let release = (fun uu____861 -> (FStar_Util.monitor_exit x))
in (

let refresh = (fun uu____866 -> (
...
let uu____872 = (new_proc ())
in FStar_Pervasives_Native.Some (uu____872))
in (FStar_ST.write the_z3proc uu____870));
(query_logging.close_log ());
(release ());
)))
in (

let restart = (fun uu____880 -> ((FStar_Util.monitor_enter ());
(query_logging.close_log ());
...
let uu____890 = (new_proc ())
in FStar_Pervasives_Native.Some (uu____890))
in (FStar_ST.write the_z3proc uu____888));
(FStar_Util.monitor_exit ());
))
in {grab = grab; release = release; refresh = refresh; restart = restart}))))))))


let at_log_file : Prims.unit  ->  Prims.string = (fun uu____897 -> (

let uu____898 = (FStar_Options.log_queries ())
in (match (uu____898) with
| true -> begin
(

let uu____899 = (query_logging.log_file_name ())
in (Prims.strcat "@" uu____899))
end
| uu____900 -> begin
""
end)))
...
let uu____948 = (FStar_All.pipe_right (FStar_Util.split s2 " ") (FStar_Util.sort_with FStar_String.compare))
in (FStar_All.pipe_right uu____948 (fun _0_38 -> FStar_Pervasives_Native.Some (_0_38))))
end))))
in (
...
let uu____994 = (FStar_ST.read in_core)
in (match (uu____994) with
| true -> begin
(

let uu____997 = (parse_core line)
in (FStar_ST.write core uu____997))
end
| uu____1004 -> begin
(

let uu____1005 = (FStar_ST.read in_statistics)
...
let uu____1051 = (FStar_ST.read in_reason_unknown)
...
| (uu____1057)::(txt)::(uu____1059)::[] -> begin
txt
end
| uu____1060 -> begin
line
...
let uu____1067 = (FStar_ST.read core)
in (

let uu____1074 = (FStar_ST.read reason_unknown)
in ((uu____1067), (statistics), (uu____1074))));
))))))))))
in (
...
let uu____1090 = (lblnegs rest)
in (lname)::uu____1090)
end
| (lname)::(uu____1093)::rest when (FStar_Util.starts_with lname "label_") -> begin
(lblnegs rest)
end
| uu____1096 -> begin
[]
...
let uu____1111 = (lblnegs tl1)
in UNKNOWN (uu____1111))
end
| ("sat")::tl1 -> begin
(

let uu____1115 = (lblnegs tl1)
in SAT (uu____1115))
end
| ("unsat")::tl1 -> begin
UNSAT (core)
...
let uu____1127 = (query_logging.get_module_name ())
in (FStar_Util.format2 "%s: Unexpected output from Z3: %s\n" uu____1127 hd1))
in (FStar_Errors.warn FStar_Range.dummyRange uu____1126));
(result tl1 core);
)
end
| uu____1128 -> begin
...
let uu____1132 = (FStar_List.map (fun l -> (FStar_Util.format1 "<%s>" (FStar_Util.trim_string l))) lines1)
in (FStar_String.concat "\n" uu____1132))
in (FStar_Util.format1 "Unexpected output from Z3: got output lines: %s\n" uu____1131))
in (FStar_All.pipe_left failwith uu____1130))
end))
in (

let uu____1136 = (get_data lines)
in (match (uu____1136) with
| (core, statistics, reason_unknown) -> begin
(

let uu____1151 = (result lines core)
in ((uu____1151), (statistics)))
end)))))))
in (
...
let uu____1162 = (FStar_Options.z3_exe ())
in (

let uu____1163 = (ini_params ())
in (FStar_Util.launch_process uu____1161 uu____1162 uu____1163 input cond))))
end
| uu____1164 -> begin
(
...
let z3_options : Prims.unit  ->  Prims.string = (fun uu____1181 -> "(set-option :global-decls false)\n(set-option :smt.mbqi false)\n(set-option :auto_config false)\n(set-option :produce-unsat-cores true)\n")

type 'a job =
{job : Prims.unit  ->  'a; callback : 'a  ->  Prims.unit}


let __proj__Mkjob__item__job = (fun projectee -> (match (projectee) with
| {job = __fname__job; callback = __fname__callback} -> begin
__fname__job
end))


let __proj__Mkjob__item__callback = (fun projectee -> (match (projectee) with
| {job = __fname__job; callback = __fname__callback} -> begin
__fname__callback
end))
...
let z3_job : Prims.bool  ->  FStar_SMTEncoding_Term.error_labels  ->  Prims.string  ->  Prims.unit  ->  ((unsat_core, (FStar_SMTEncoding_Term.error_labels * error_kind)) FStar_Util.either * Prims.int * z3statistics) = (fun fresh1 label_messages input uu____1320 -> (

let ekind = (fun uu___98_1331 -> (match (uu___98_1331) with
| TIMEOUT (uu____1332) -> begin
Timeout
end
| SAT (uu____1334) -> begin
Default
end
| UNKNOWN (uu____1336) -> begin
Default
end
| KILLED -> begin
Kill
...
let uu____1340 = (doZ3Exe fresh1 input)
...
let uu____1355 = (FStar_Util.now ())
in (FStar_Util.time_diff start uu____1355))
in (match (uu____1352) with
| (uu____1363, elapsed_time) -> begin
(
...
let uu____1443 = (FStar_Options.debug_any ())
in (match (uu____1443) with
| true -> begin
(

let uu____1444 = (FStar_Util.format1 "Z3 says: %s\n" (status_to_string status))
in (FStar_All.pipe_left FStar_Util.print_string uu____1444))
end
| uu____1445 -> begin
()
end));
...
let uu____1468 = (FStar_All.pipe_right label_messages (FStar_List.tryFind (fun uu____1490 -> (match (uu____1490) with
| (m, uu____1497, uu____1498) -> begin
...
let uu____1563 = (ekind status)
in ((failing_assertions), (uu____1563)))
in FStar_Util.Inr (uu____1554))
in ((uu____1543), (elapsed_time), (statistics))));
)
end
| SAT (lblnegs) -> begin
((
...
let uu____1592 = (FStar_Util.format1 "Z3 says: %s\n" (status_to_string status))
in (FStar_All.pipe_left FStar_Util.print_string uu____1592))
end
| uu____1593 -> begin
()
end));
...
let uu____1616 = (FStar_All.pipe_right label_messages (FStar_List.tryFind (fun uu____1638 -> (match (uu____1638) with
| (m, uu____1645, uu____1646) -> begin
((FStar_Pervasives_Native.fst m) = l)
end))))
...
let uu____1711 = (ekind status)
in ((failing_assertions), (uu____1711)))
in FStar_Util.Inr (uu____1702))
in ((uu____1691), (elapsed_time), (statistics))));
)
end
| UNKNOWN (lblnegs) -> begin
((

let uu____1739 = (FStar_Options.debug_any ())
in (match (uu____1739) with
| true -> begin
(

let uu____1740 = (FStar_Util.format1 "Z3 says: %s\n" (status_to_string status))
in (FStar_All.pipe_left FStar_Util.print_string uu____1740))
end
| uu____1741 -> begin
()
end));
...
let uu____1764 = (FStar_All.pipe_right label_messages (FStar_List.tryFind (fun uu____1786 -> (match (uu____1786) with
| (m, uu____1793, uu____1794) -> begin
((FStar_Pervasives_Native.fst m) = l)
end))))
...
let uu____1859 = (ekind status)
in ((failing_assertions), (uu____1859)))
in FStar_Util.Inr (uu____1850))
in ((uu____1839), (elapsed_time), (statistics))));
)
end)
in result)
end))
...
let rec dequeue' : Prims.unit  ->  Prims.unit = (fun uu____1893 -> (

let j = (

let uu____1895 = (FStar_ST.read job_queue)
...
and dequeue : Prims.unit  ->  Prims.unit = (fun uu____1928 -> (

let uu____1929 = (FStar_ST.read running)
in if uu____1929 then begin
(

let rec aux = (fun uu____1935 -> ((FStar_Util.monitor_enter job_queue);
(

let uu____1941 = (FStar_ST.read job_queue)
...
let uu____1955 = (j.job ())
in (FStar_All.pipe_left j.callback uu____1955)))


let init : Prims.unit  ->  Prims.unit = (fun uu____1987 -> ((FStar_ST.write running true);
(
...
let uu____2011 = (FStar_ST.read job_queue)
in (FStar_List.append uu____2011 ((j)::[])))
in (FStar_ST.write job_queue uu____2009));
(FStar_Util.monitor_pulse job_queue);
(FStar_Util.monitor_exit job_queue);
))


let finish : Prims.unit  ->  Prims.unit = (fun uu____2031 -> (

let rec aux = (fun uu____2035 -> (

let uu____2036 = (with_monitor job_queue (fun uu____2048 -> (

let uu____2049 = (FStar_ST.read pending_jobs)
in (

let uu____2052 = (

let uu____2053 = (FStar_ST.read job_queue)
in (FStar_List.length uu____2053))
in ((uu____2049), (uu____2052))))))
...
let mk_fresh_scope : Prims.unit  ->  scope_t = (fun uu____2088 -> (FStar_ST.read fresh_scope))
...
let uu____2106 = (FStar_ST.read fresh_scope)
in ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Push)::[])::uu____2106)
in (FStar_ST.write fresh_scope uu____2103));
(

let uu____2118 = (

let uu____2120 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2120 ((FStar_SMTEncoding_Term.Push)::(FStar_SMTEncoding_Term.Caption (msg))::[])))
in (FStar_ST.write bg_scope uu____2118));
))
...
let uu____2136 = (FStar_ST.read fresh_scope)
in (FStar_List.tl uu____2136))
in (FStar_ST.write fresh_scope uu____2133));
(

let uu____2148 = (

let uu____2150 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2150 ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Pop)::[])))
in (FStar_ST.write bg_scope uu____2148));
))


let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  Prims.unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___99_2167 -> (match (uu___99_2167) with
...
let uu____2170 = (FStar_ST.read fresh_scope)
...
let uu____2193 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2193 decls))
in (FStar_ST.write bg_scope uu____2191));
))


let refresh : Prims.unit  ->  Prims.unit = (fun uu____2204 -> ((

let uu____2206 = (

let uu____2207 = (FStar_Options.n_cores ())
in (uu____2207 < (Prims.parse_int "2")))
...
let uu____2214 = (FStar_ST.read fresh_scope)
in (FStar_List.rev uu____2214))
in (FStar_List.flatten uu____2211))
in (FStar_ST.write bg_scope uu____2209));
))
...
let uu____2238 = (FStar_ST.read fresh_scope)
...
let mk_cb = (fun used_unsat_core cb uu____2316 -> (match (uu____2316) with
| (uc_errs, time, statistics) -> begin
(match (used_unsat_core) with
| true -> begin
(match (uc_errs) with
| FStar_Util.Inl (uu____2348) -> begin
(cb ((uc_errs), (time), (statistics)))
end
| FStar_Util.Inr (uu____2357, ek) -> begin
...
let uu____2386 = (FStar_List.map (FStar_SMTEncoding_Term.declToSmt (z3_options ())) theory)
in (FStar_All.pipe_right uu____2386 (FStar_String.concat "\n")))
in ((

let uu____2390 = (FStar_Options.log_queries ())
...
let uu____2430 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2430 (FStar_List.append ((FStar_SMTEncoding_Term.Push)::[]) (FStar_List.append qry ((FStar_SMTEncoding_Term.Pop)::[])))))
in (

let uu____2435 = (filter_theory theory)
...
let uu____2500 = (match (scope) with
...
let uu____2511 = (FStar_ST.read fresh_scope)
in (FStar_List.rev uu____2511));
)
end)
in (FStar_List.flatten uu____2500))
in (
...
let uu____2521 = (filter_theory theory1)
...
let uu____2582 = (FStar_Options.n_cores ())
in (uu____2582 = (Prims.parse_int "1")))
