...
let z3version_as_string : z3version  ->  Prims.string = (fun uu___97_48 -> (match (uu___97_48) with
| Z3V_Unknown (s) -> begin
...
let status_to_string : z3status  ->  Prims.string = (fun uu___98_248 -> (match (uu___98_248) with
| SAT (uu____249) -> begin
"sat"
end
...
let ekind = (fun uu___99_1121 -> (match (uu___99_1121) with
...
let uu____1561 = (FStar_ST.read fresh_scope)
in ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Push)::[])::uu____1561)
in (FStar_ST.write fresh_scope uu____1558));
(

let uu____1573 = (

let uu____1575 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____1575 ((FStar_SMTEncoding_Term.Push)::(FStar_SMTEncoding_Term.Caption (msg))::[])))
in (FStar_ST.write bg_scope uu____1573));
))
...
let uu____1590 = (FStar_ST.read fresh_scope)
in (FStar_List.tl uu____1590))
in (FStar_ST.write fresh_scope uu____1587));
(

let uu____1602 = (

let uu____1604 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____1604 ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Pop)::[])))
in (FStar_ST.write bg_scope uu____1602));
))


let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  Prims.unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___100_1619 -> (match (uu___100_1619) with
...
let uu____1622 = (FStar_ST.read fresh_scope)
...
let uu____1645 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____1645 decls))
in (FStar_ST.write bg_scope uu____1643));
))


let refresh : Prims.unit  ->  Prims.unit = (fun uu____1655 -> ((

let uu____1657 = (

let uu____1658 = (FStar_Options.n_cores ())
in (uu____1658 < (Prims.parse_int "2")))
...
let uu____1665 = (FStar_ST.read fresh_scope)
in (FStar_List.rev uu____1665))
in (FStar_List.flatten uu____1662))
in (FStar_ST.write bg_scope uu____1660));
))
...
let uu____1690 = (FStar_ST.read fresh_scope)
...
let uu____1737 = (FStar_List.fold_right (fun d uu____1747 -> (match (uu____1747) with
| (theory1, n_retained, n_pruned) -> begin
(match (d) with
| FStar_SMTEncoding_Term.Assume (uu____1765, uu____1766, name) -> begin
...
let uu____1804 = (FStar_All.pipe_right core2 (FStar_List.filter (fun nm -> (

let uu____1809 = (FStar_All.pipe_right th (FStar_Util.for_some (fun uu___101_1811 -> (match (uu___101_1811) with
| FStar_SMTEncoding_Term.Assume (uu____1812, uu____1813, nm') -> begin
(nm = nm')
end
| uu____1815 -> begin
false
end))))
in (FStar_All.pipe_right uu____1809 Prims.op_Negation)))))
in (FStar_All.pipe_right uu____1804 (FStar_String.concat ", ")))
in (

let included = (

let uu____1818 = (FStar_All.pipe_right th (FStar_List.collect (fun uu___102_1822 -> (match (uu___102_1822) with
| FStar_SMTEncoding_Term.Assume (uu____1824, uu____1825, nm) -> begin
(nm)::[]
end
| uu____1827 -> begin
[]
end))))
in (FStar_All.pipe_right uu____1818 (FStar_String.concat ", ")))
in (FStar_Util.format2 "missed={%s}; included={%s}" missed included))))
in ((

let uu____1830 = ((FStar_Options.hint_info ()) && (FStar_Options.debug_any ()))
...
let uu____1837 = (FStar_Util.string_of_int n_retained)
in (

let uu____1838 = (match ((n1 <> n_retained)) with
| true -> begin
(

let uu____1841 = (FStar_Util.string_of_int n1)
in (FStar_Util.format2 " (expected %s (%s); replay may be inaccurate)" uu____1841 missed))
end
| uu____1845 -> begin
""
end)
...
let uu____1854 = (FStar_All.pipe_right core1 (FStar_String.concat ", "))
in (Prims.strcat "UNSAT CORE: " uu____1854))
in FStar_SMTEncoding_Term.Caption (uu____1853))
in (uu____1852)::[])
in (FStar_List.append theory' uu____1850))
in ((uu____1848), (true)));
))
end))
end))
...
let mk_cb = (fun used_unsat_core cb uu____1898 -> (match (uu____1898) with
| (uc_errs, time) -> begin
(match (used_unsat_core) with
| true -> begin
(match (uc_errs) with
| FStar_Util.Inl (uu____1927) -> begin
(cb ((uc_errs), (time)))
end
| FStar_Util.Inr (uu____1936, ek) -> begin
...
let uu____1964 = (FStar_List.map (FStar_SMTEncoding_Term.declToSmt (z3_options ())) theory)
in (FStar_All.pipe_right uu____1964 (FStar_String.concat "\n")))
in ((

let uu____1968 = (FStar_Options.log_queries ())
...
let uu____2015 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2015 (FStar_List.append ((FStar_SMTEncoding_Term.Push)::[]) (FStar_List.append qry ((FStar_SMTEncoding_Term.Pop)::[])))))
in (

let uu____2020 = (filter_assertions core theory)
...
let uu____2099 = (match (scope) with
| Some (s) -> begin
(FStar_List.rev s)
end
| None -> begin
...
let uu____2110 = (FStar_ST.read fresh_scope)
in (FStar_List.rev uu____2110));
)
end)
in (FStar_List.flatten uu____2099))
in (
...
let uu____2120 = (filter_assertions core theory1)
...
let uu____2194 = (FStar_Options.n_cores ())
in (uu____2194 = (Prims.parse_int "1")))
