                  ...
                  (bg_z3_proc.refresh ();
                   (let uu____4165 =
                    ...
                    ((UNKNOWN
                        ([],
                          (FStar_Pervasives_Native.Some
                             "Z3 raised an exception"))), uu____4165))) in
            match uu____4139 with
            | (status,statistics) ->
                let uu____4176 =
                  let uu____4181 = FStar_Util.now () in
                  FStar_Util.time_diff start uu____4181 in
                (match uu____4176 with
                 | (uu____4182,elapsed_time) ->
                     {
...
let rec dequeue': Prims.unit -> Prims.unit =
  fun uu____4199  ->
    let j =
      let uu____4201 = FStar_ST.op_Bang job_queue in
    ...
    run_job j;
    with_monitor job_queue (fun uu____4322  -> FStar_Util.decr pending_jobs);
    dequeue ()
and dequeue: Prims.unit -> Prims.unit =
  fun uu____4324  ->
    let uu____4325 = FStar_ST.op_Bang running in
    if uu____4325
    then
      let rec aux uu____4375 =
        FStar_Util.monitor_enter job_queue;
        (let uu____4381 = FStar_ST.op_Bang job_queue in
         match uu____4381 with
         | [] ->
             (FStar_Util.monitor_exit job_queue;
              FStar_Util.sleep (Prims.parse_int "50");
              aux ())
         | uu____4440 -> dequeue' ()) in
      aux ()
    else ()
and run_job: z3job -> Prims.unit =
  fun j  ->
    let uu____4444 = j.job () in FStar_All.pipe_left j.callback uu____4444
...
let enqueue: z3job -> Prims.unit =
  fun j  ->
    FStar_Util.monitor_enter job_queue;
    (let uu____4514 =
       let uu____4517 = FStar_ST.op_Bang job_queue in
       FStar_List.append uu____4517 [j] in
     FStar_ST.op_Colon_Equals job_queue uu____4514);
    FStar_Util.monitor_pulse job_queue;
    FStar_Util.monitor_exit job_queue
let finish: Prims.unit -> Prims.unit =
  fun uu____4632  ->
    let rec aux uu____4636 =
      let uu____4637 =
        with_monitor job_queue
          (fun uu____4653  ->
             let uu____4654 = FStar_ST.op_Bang pending_jobs in
             let uu____4701 =
               let uu____4702 = FStar_ST.op_Bang job_queue in
               FStar_List.length uu____4702 in
             (uu____4654, uu____4701)) in
...
let mk_fresh_scope: Prims.unit -> scope_t =
  fun uu____4841  -> FStar_ST.op_Bang fresh_scope
let bg_scope: FStar_SMTEncoding_Term.decl Prims.list FStar_ST.ref =
  FStar_Util.mk_ref []
let push: Prims.string -> Prims.unit =
  fun msg  ->
    (let uu____4914 =
       let uu____4919 = FStar_ST.op_Bang fresh_scope in
       [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push] ::
         uu____4919 in
     FStar_ST.op_Colon_Equals fresh_scope uu____4914);
    (let uu____5034 =
       let uu____5037 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5037
         [FStar_SMTEncoding_Term.Push; FStar_SMTEncoding_Term.Caption msg] in
     FStar_ST.op_Colon_Equals bg_scope uu____5034)
let pop: Prims.string -> Prims.unit =
  fun msg  ->
    (let uu____5145 =
       let uu____5150 = FStar_ST.op_Bang fresh_scope in
       FStar_List.tl uu____5150 in
     FStar_ST.op_Colon_Equals fresh_scope uu____5145);
    (let uu____5265 =
       let uu____5268 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5268
         [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Pop] in
     FStar_ST.op_Colon_Equals bg_scope uu____5265)
let giveZ3: FStar_SMTEncoding_Term.decl Prims.list -> Prims.unit =
  fun decls  ->
    FStar_All.pipe_right decls
      (FStar_List.iter
         (fun uu___117_5383  ->
            match uu___117_5383 with
            | FStar_SMTEncoding_Term.Push  -> failwith "Unexpected push/pop"
            | FStar_SMTEncoding_Term.Pop  -> failwith "Unexpected push/pop"
            | uu____5384 -> ()));
    (let uu____5386 = FStar_ST.op_Bang fresh_scope in
     match uu____5386 with
     | hd1::tl1 ->
         FStar_ST.op_Colon_Equals fresh_scope ((FStar_List.append hd1 decls)
           :: tl1)
     | uu____5511 -> failwith "Impossible");
    (let uu____5516 =
       let uu____5519 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5519 decls in
     FStar_ST.op_Colon_Equals bg_scope uu____5516)
let refresh: Prims.unit -> Prims.unit =
  fun uu____5625  ->
    (let uu____5627 =
       let uu____5628 = FStar_Options.n_cores () in
       uu____5628 < (Prims.parse_int "2") in
     if uu____5627 then bg_z3_proc.refresh () else ());
    (let uu____5630 =
       let uu____5633 =
         let uu____5638 = FStar_ST.op_Bang fresh_scope in
         FStar_List.rev uu____5638 in
       FStar_List.flatten uu____5633 in
     FStar_ST.op_Colon_Equals bg_scope uu____5630)
let mk_input:
      ...
      then
          ...
          let uu____5789 =
            FStar_All.pipe_right theory
              (FStar_Util.prefix_until
                 (fun uu___118_5817  ->
                    match uu___118_5817 with
                    | FStar_SMTEncoding_Term.CheckSat  -> true
                    | uu____5818 -> false)) in
          FStar_All.pipe_right uu____5789 FStar_Option.get in
        ...
        | (prefix1,check_sat,suffix) ->
            ...
            let ss =
              ...
              FStar_String.concat "\n" uu____5876 in
            let uu____5879 =
              let uu____5882 = FStar_Util.digest_of_string ps in
        ...
        (let uu____5886 =
           ...
           FStar_All.pipe_right uu____5887 (FStar_String.concat "\n") in
         (uu____5886, FStar_Pervasives_Native.None)) in
    match uu____5764 with
    | (r,hash) ->
        ((let uu____5907 = FStar_Options.log_queries () in
          if uu____5907 then query_logging.write_to_log r else ());
         (r, hash))
    ...
    fun cache  ->
      fun label_messages  ->
        fun qry  ->
          fun cb  ->
            let theory =
              let uu____6010 = FStar_ST.op_Bang bg_scope in
              FStar_List.append uu____6010
                (FStar_List.append [FStar_SMTEncoding_Term.Push]
                   (FStar_List.append qry [FStar_SMTEncoding_Term.Pop])) in
            let uu____6063 = filter_theory theory in
            match uu____6063 with
            | (theory1,used_unsat_core) ->
                let uu____6070 = mk_input theory1 in
                (match uu____6070 with
                 | (input,qhash) ->
                     (FStar_ST.op_Colon_Equals bg_scope [];
                      (let uu____6134 =
                         let uu____6135 = cache_hit cache qhash cb in
                         Prims.op_Negation uu____6135 in
                       if uu____6134
                       then
                         run_job
                           {
            ...
            fun cb  ->
              let theory =
                let uu____6199 =
                  ...
                  | FStar_Pervasives_Native.None  ->
                      (FStar_ST.op_Colon_Equals bg_scope [];
                       (let uu____6262 = FStar_ST.op_Bang fresh_scope in
                        FStar_List.rev uu____6262)) in
                FStar_List.flatten uu____6199 in
              let theory1 =
              ...
              | (theory2,used_unsat_core) ->
                  let uu____6333 = mk_input theory2 in
                  (match uu____6333 with
                   | (input,qhash) ->
                       let uu____6346 =
                         let uu____6347 = cache_hit cache qhash cb in
      ...
      fun label_messages  ->
        fun qry  ->
          fun scope  ->
            fun cb  ->
              let uu____6410 =
                let uu____6411 = FStar_Options.n_cores () in
                uu____6411 = (Prims.parse_int "1") in
              if uu____6410
