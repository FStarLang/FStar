...
let (parse_z3_version_lines :
  ...
  fun out  ->
    ...
    | x::uu____42988 ->
        ...
        let rec aux uu___413_43012 =
          match uu___413_43012 with
          | hash::[] ->
              let n1 =
              ...
              then
                ((let uu____43034 = FStar_Options.debug_any ()  in
                  if uu____43034
                  then
                    let msg =
                      FStar_Util.format1
    ...
    let run_proc_result =
      try
        (fun uu___442_43090  ->
           ...
           | () ->
               let uu____43094 =
                 let uu____43096 = FStar_Options.z3_exe ()  in
                 FStar_Util.run_process "z3_version" uu____43096 ["-version"]
                   FStar_Pervasives_Native.None
                  in
               FStar_Pervasives_Native.Some uu____43094) ()
      with | uu___441_43105 -> FStar_Pervasives_Native.None  in
    match run_proc_result with
    | FStar_Pervasives_Native.None  ->
        FStar_Pervasives_Native.Some
          (FStar_Errors.Error_Z3InvocationError, "Could not run Z3")
    | FStar_Pervasives_Native.Some out ->
        let uu____43128 = parse_z3_version_lines out  in
        (match uu____43128 with
...
let (check_z3hash : unit -> unit) =
  fun uu____43159  ->
    let uu____43160 =
      let uu____43162 = FStar_ST.op_Bang _z3hash_checked  in
      Prims.op_Negation uu____43162  in
    if uu____43160
    then
      (FStar_ST.op_Colon_Equals _z3hash_checked true;
       (let uu____43209 = z3hash_warning_message ()  in
    ...
    (let uu____43249 =
       let uu____43253 =
         let uu____43257 =
           let uu____43261 =
             let uu____43263 =
               let uu____43265 = FStar_Options.z3_seed ()  in
               FStar_Util.string_of_int uu____43265  in
             FStar_Util.format1 "smt.random_seed=%s" uu____43263  in
           [uu____43261]  in
         "-in" :: uu____43257  in
       "-smt2" :: uu____43253  in
     let uu____43274 = FStar_Options.z3_cliopt ()  in
     FStar_List.append uu____43249 uu____43274)
...
let (uu___is_UNSAT : z3status -> Prims.bool) =
  fun projectee  ->
    match projectee with | UNSAT _0 -> true | uu____43337 -> false
  
let (__proj__UNSAT__item___0 : z3status -> unsat_core) =
  fun projectee  -> match projectee with | UNSAT _0 -> _0 
let (uu___is_SAT : z3status -> Prims.bool) =
  fun projectee  ->
    match projectee with | SAT _0 -> true | uu____43364 -> false
...
let (uu___is_UNKNOWN : z3status -> Prims.bool) =
  fun projectee  ->
    match projectee with | UNKNOWN _0 -> true | uu____43412 -> false
...
let (uu___is_TIMEOUT : z3status -> Prims.bool) =
  fun projectee  ->
    match projectee with | TIMEOUT _0 -> true | uu____43460 -> false
...
let (uu___is_KILLED : z3status -> Prims.bool) =
  fun projectee  ->
    match projectee with | KILLED  -> true | uu____43500 -> false
  
type z3statistics = Prims.string FStar_Util.smap
let (status_tag : z3status -> Prims.string) =
  fun uu___414_43511  ->
    match uu___414_43511 with
    | SAT uu____43513 -> "sat"
    | UNSAT uu____43522 -> "unsat"
    | UNKNOWN uu____43524 -> "unknown"
    | TIMEOUT uu____43533 -> "timeout"
    | KILLED  -> "killed"
...
let (status_string_and_errors :
  ...
  fun s  ->
    match s with
    | KILLED  -> ((status_tag s), [])
    | UNSAT uu____43560 -> ((status_tag s), [])
    | SAT (errs,msg) ->
        let uu____43570 =
...
let (tid : unit -> Prims.string) =
  fun uu____43625  ->
    let uu____43626 = FStar_Util.current_tid ()  in
    FStar_All.pipe_right uu____43626 FStar_Util.string_of_int
  
let (new_z3proc : Prims.string -> FStar_Util.proc) =
  fun id1  ->
    let uu____43638 = FStar_Options.z3_exe ()  in
    let uu____43640 = ini_params ()  in
    FStar_Util.start_process id1 uu____43638 uu____43640
      (fun s  -> s = "Done!")
...
let (new_z3proc_with_id : unit -> FStar_Util.proc) =
  ...
  fun uu____43660  ->
    let uu____43661 =
      let uu____43663 =
        FStar_Util.incr ctr;
        (let uu____43666 = FStar_ST.op_Bang ctr  in
         FStar_All.pipe_right uu____43666 FStar_Util.string_of_int)
         in
      FStar_Util.format1 "bg-%s" uu____43663  in
    new_z3proc uu____43661
...
let (query_logging : query_log) =
  ...
  let set_module_name n1 =
    FStar_ST.op_Colon_Equals current_module_name
     ...
     in
  let get_module_name uu____44138 =
    let uu____44139 = FStar_ST.op_Bang current_module_name  in
    match uu____44139 with
    | FStar_Pervasives_Native.None  -> failwith "Module name not set"
    | FStar_Pervasives_Native.Some n1 -> n1  in
  let next_file_name uu____44181 =
    let n1 = get_module_name ()  in
    let file_name =
      let uu____44186 =
        let uu____44195 = FStar_ST.op_Bang used_file_names  in
        FStar_List.tryFind
          (fun uu____44248  ->
             match uu____44248 with | (m,uu____44257) -> n1 = m) uu____44195
         in
      match uu____44186 with
      | FStar_Pervasives_Native.None  ->
          ((let uu____44271 =
              let uu____44280 = FStar_ST.op_Bang used_file_names  in
              (n1, (Prims.parse_int "0")) :: uu____44280  in
            FStar_ST.op_Colon_Equals used_file_names uu____44271);
           n1)
      | FStar_Pervasives_Native.Some (uu____44368,k) ->
          ((let uu____44381 =
              let uu____44390 = FStar_ST.op_Bang used_file_names  in
              (n1, (k + (Prims.parse_int "1"))) :: uu____44390  in
            FStar_ST.op_Colon_Equals used_file_names uu____44381);
           (let uu____44478 =
              FStar_Util.string_of_int (k + (Prims.parse_int "1"))  in
            FStar_Util.format2 "%s-%s" n1 uu____44478))
       in
    FStar_Util.format1 "queries-%s.smt2" file_name  in
  let new_log_file uu____44493 =
    ...
    (let fh = FStar_Util.open_file_for_writing file_name  in
     ...
     (fh, file_name))
     in
  let get_log_file uu____44575 =
    let uu____44576 = FStar_ST.op_Bang log_file_opt  in
    match uu____44576 with
    | FStar_Pervasives_Native.None  -> new_log_file ()
    | FStar_Pervasives_Native.Some fh -> fh  in
  let append_to_log str =
    let uu____44647 = get_log_file ()  in
    match uu____44647 with | (f,nm) -> (FStar_Util.append_to_file f str; nm)
     in
  ...
  let write_to_log fresh str =
    let uu____44687 =
      fresh ||
        (let uu____44690 = FStar_Options.n_cores ()  in
         uu____44690 > (Prims.parse_int "1"))
       in
    if uu____44687 then write_to_new_log str else append_to_log str  in
  let close_log uu____44702 =
    let uu____44703 = FStar_ST.op_Bang log_file_opt  in
    match uu____44703 with
    | FStar_Pervasives_Native.None  -> ()
    | FStar_Pervasives_Native.Some (fh,uu____44750) ->
        (FStar_Util.close_file fh;
         FStar_ST.op_Colon_Equals log_file_opt FStar_Pervasives_Native.None)
     in
  let log_file_name uu____44803 =
    let uu____44804 = FStar_ST.op_Bang current_file_name  in
...
let (bg_z3_proc : bgproc FStar_ST.ref) =
  ...
  let z3proc uu____44855 =
    (let uu____44857 =
       let uu____44859 = FStar_ST.op_Bang the_z3proc  in
       uu____44859 = FStar_Pervasives_Native.None  in
     if uu____44857
     then
       let uu____44888 =
         let uu____44891 = new_z3proc_with_id ()  in
         FStar_Pervasives_Native.Some uu____44891  in
       FStar_ST.op_Colon_Equals the_z3proc uu____44888
     else ());
    (let uu____44917 = FStar_ST.op_Bang the_z3proc  in
     FStar_Util.must uu____44917)
     in
  let x = []  in
  let ask input =
    let kill_handler uu____44961 = "\nkilled\n"  in
    let uu____44963 = z3proc ()  in
    FStar_Util.ask_process uu____44963 input kill_handler  in
  let refresh uu____44969 =
    (let uu____44971 = z3proc ()  in FStar_Util.kill_process uu____44971);
    (let uu____44973 =
       let uu____44976 = new_z3proc_with_id ()  in
       FStar_Pervasives_Native.Some uu____44976  in
     FStar_ST.op_Colon_Equals the_z3proc uu____44973);
    query_logging.close_log ()  in
  let restart uu____45005 =
    query_logging.close_log ();
    FStar_ST.op_Colon_Equals the_z3proc FStar_Pervasives_Native.None;
    (let uu____45031 =
       let uu____45034 = new_z3proc_with_id ()  in
       FStar_Pervasives_Native.Some uu____45034  in
     FStar_ST.op_Colon_Equals the_z3proc uu____45031)
     in
  FStar_Util.mk_ref
    {
    ...
    fun r  ->
      fun lines  ->
        let rec until tag lines1 =
          ...
          | l::lines2 ->
              ...
              then FStar_Pervasives_Native.Some ([], lines2)
                ...
                (let uu____45360 = until tag lines2  in
                 FStar_Util.map_opt uu____45360
                   (fun uu____45396  ->
                      match uu____45396 with
        ...
        let find_section tag lines1 =
          let uu____45503 = until (start_tag tag) lines1  in
          match uu____45503 with
          | FStar_Pervasives_Native.None  ->
              (FStar_Pervasives_Native.None, lines1)
          | FStar_Pervasives_Native.Some (prefix1,suffix) ->
              let uu____45573 = until (end_tag tag) suffix  in
              (match uu____45573 with
               | FStar_Pervasives_Native.None  ->
                   failwith
           ...
           in
        let uu____45658 = find_section "result" lines  in
        match uu____45658 with
        | (result_opt,lines1) ->
            let result = FStar_Util.must result_opt  in
            let uu____45697 = find_section "reason-unknown" lines1  in
            (match uu____45697 with
             | (reason_unknown,lines2) ->
                 let uu____45729 = find_section "unsat-core" lines2  in
                 (match uu____45729 with
                  | (unsat_core,lines3) ->
                      let uu____45761 = find_section "statistics" lines3  in
                      (match uu____45761 with
                       | (statistics,lines4) ->
                           let uu____45793 = find_section "labels" lines4  in
                           (match uu____45793 with
                            | (labels,lines5) ->
                                let remaining =
                                  let uu____45829 = until "Done!" lines5  in
                                  match uu____45829 with
            ...
            let parse z3out =
                    ...
                    then FStar_Pervasives_Native.None
                      ...
                      (let uu____45994 =
                         FStar_All.pipe_right (FStar_Util.split s2 " ")
                          ...
                          in
                       FStar_Pervasives_Native.Some uu____45994)
                 in
              let labels =
                ...
                | FStar_Pervasives_Native.Some lines1 ->
                    let rec lblnegs lines2 =
                      ...
                      | lname::"false"::rest when
                          ...
                          let uu____46039 = lblnegs rest  in lname ::
                            uu____46039
                      | lname::uu____46045::rest when
                          FStar_Util.starts_with lname "label_" ->
                          lblnegs rest
                      | uu____46055 -> []  in
                         ...
                         (fun l  ->
                            let uu____46079 =
                              FStar_All.pipe_right label_messages
                                (FStar_List.tryFind
                                   (fun uu____46119  ->
                                      match uu____46119 with
                                      | (m,uu____46129,uu____46130) ->
                                          let uu____46133 =
                                            FStar_SMTEncoding_Term.fv_name m
                                             in
                                          uu____46133 = l))
                               in
                            match uu____46079 with
                            | FStar_Pervasives_Native.None  -> []
              ...
              let reason_unknown =
                FStar_Util.map_opt smt_output.smt_reason_unknown
                 ...
                 in
              let status =
                (let uu____46295 = FStar_Options.debug_any ()  in
                 if uu____46295
                 then
                   let uu____46298 =
                     FStar_Util.format1 "Z3 says: %s\n"
                      ...
                      in
                   FStar_All.pipe_left FStar_Util.print_string uu____46298
                 else ());
                (match smt_output.smt_result with
                 ...
                 | "killed"::[] ->
                     ((let uu____46330 =
                         let uu____46335 = FStar_ST.op_Bang bg_z3_proc  in
                         uu____46335.restart  in
                       uu____46330 ());
                      KILLED)
                 | uu____46355 ->
                     let uu____46356 =
            ...
            fun qhash  ->
              fun uu____46712  ->
                ...
                let uu____46722 =
                  ...
                  | uu___810_46739 ->
                      if
                        let uu____46744 = FStar_Options.trace_error ()  in
                        Prims.op_Negation uu____46744
                      then
                        Obj.magic
                          (Obj.repr
                             ((let uu____46747 =
                                 ...
                                 uu____46752.refresh  in
                               uu____46747 ());
                              FStar_Exn.raise uu___810_46739))
                      else Obj.magic (Obj.repr (failwith "unreachable"))
                   in
                match uu____46722 with
                | (status,statistics) ->
                    let uu____46778 =
                      let uu____46784 = FStar_Util.now ()  in
                      FStar_Util.time_diff start uu____46784  in
                    (match uu____46778 with
                     | (uu____46785,elapsed_time) ->
                         {
...
let rec (dequeue' : unit -> unit) =
  fun uu____46808  ->
    let j =
      let uu____46810 = FStar_ST.op_Bang job_queue  in
      match uu____46810 with
      | [] -> failwith "Impossible"
    ...
    FStar_Util.with_monitor job_queue
      (fun uu____46878  -> FStar_Util.decr pending_jobs) ();
    dequeue ()

and (dequeue : unit -> unit) =
  fun uu____46880  ->
    let uu____46881 = FStar_ST.op_Bang running  in
    if uu____46881
    then
      let rec aux uu____46909 =
        FStar_Util.monitor_enter job_queue;
        (let uu____46915 = FStar_ST.op_Bang job_queue  in
         match uu____46915 with
         | [] ->
             (FStar_Util.monitor_exit job_queue;
...
and (run_job : z3job -> unit) =
  fun j  ->
    let uu____46952 = j.job ()  in FStar_All.pipe_left j.callback uu____46952
...
let (enqueue : z3job -> unit) =
  fun j  ->
    FStar_Util.with_monitor job_queue
      (fun uu____47015  ->
         (let uu____47017 =
            let uu____47020 = FStar_ST.op_Bang job_queue  in
            FStar_List.append uu____47020 [j]  in
          FStar_ST.op_Colon_Equals job_queue uu____47017);
         FStar_Util.monitor_pulse job_queue) ()
  
let (finish : unit -> unit) =
  fun uu____47078  ->
    let rec aux uu____47084 =
      let uu____47085 =
        FStar_Util.with_monitor job_queue
          (fun uu____47103  ->
             let uu____47104 = FStar_ST.op_Bang pending_jobs  in
             let uu____47127 =
               let uu____47128 = FStar_ST.op_Bang job_queue  in
               FStar_List.length uu____47128  in
             (uu____47104, uu____47127)) ()
         in
      match uu____47085 with
      | (n1,m) ->
...
let (mk_fresh_scope : unit -> scope_t) =
  fun uu____47204  -> FStar_ST.op_Bang fresh_scope 
let (flatten_fresh_scope : unit -> FStar_SMTEncoding_Term.decl Prims.list) =
  fun uu____47231  ->
    let uu____47232 =
      let uu____47237 = FStar_ST.op_Bang fresh_scope  in
      FStar_List.rev uu____47237  in
    FStar_List.flatten uu____47232
...
let (push : Prims.string -> unit) =
  fun msg  ->
    FStar_Util.atomically
      (fun uu____47281  ->
         (let uu____47283 =
            let uu____47284 = FStar_ST.op_Bang fresh_scope  in
            [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push]
              :: uu____47284
             in
          FStar_ST.op_Colon_Equals fresh_scope uu____47283);
         (let uu____47329 =
            let uu____47332 = FStar_ST.op_Bang bg_scope  in
            FStar_List.append uu____47332
              [FStar_SMTEncoding_Term.Push;
              FStar_SMTEncoding_Term.Caption msg]
             in
          FStar_ST.op_Colon_Equals bg_scope uu____47329))
...
let (pop : Prims.string -> unit) =
  fun msg  ->
    FStar_Util.atomically
      (fun uu____47392  ->
         (let uu____47394 =
            let uu____47395 = FStar_ST.op_Bang fresh_scope  in
            FStar_List.tl uu____47395  in
          FStar_ST.op_Colon_Equals fresh_scope uu____47394);
         (let uu____47440 =
            let uu____47443 = FStar_ST.op_Bang bg_scope  in
            FStar_List.append uu____47443
              [FStar_SMTEncoding_Term.Caption msg;
              FStar_SMTEncoding_Term.Pop]
             in
          FStar_ST.op_Colon_Equals bg_scope uu____47440))
...
let (rollback :
  ...
  fun msg  ->
    fun depth  ->
      FStar_Common.rollback (fun uu____47530  -> pop msg) fresh_scope depth
...
let (giveZ3 : FStar_SMTEncoding_Term.decl Prims.list -> unit) =
  fun decls  ->
    FStar_All.pipe_right decls
      (FStar_List.iter
         (fun uu___415_47545  ->
            match uu___415_47545 with
            | FStar_SMTEncoding_Term.Push  -> failwith "Unexpected push/pop"
            | FStar_SMTEncoding_Term.Pop  -> failwith "Unexpected push/pop"
            | uu____47548 -> ()));
    (let uu____47550 = FStar_ST.op_Bang fresh_scope  in
     match uu____47550 with
     | hd1::tl1 ->
         FStar_ST.op_Colon_Equals fresh_scope ((FStar_List.append hd1 decls)
           :: tl1)
     | uu____47601 -> failwith "Impossible");
    (let uu____47603 =
       let uu____47606 = FStar_ST.op_Bang bg_scope  in
       FStar_List.append uu____47606 decls  in
     FStar_ST.op_Colon_Equals bg_scope uu____47603)
  
let (refresh : unit -> unit) =
  fun uu____47660  ->
    (let uu____47662 =
       let uu____47664 = FStar_Options.n_cores ()  in
       uu____47664 < (Prims.parse_int "2")  in
     if uu____47662
     then
       let uu____47668 =
         let uu____47673 = FStar_ST.op_Bang bg_z3_proc  in
         uu____47673.refresh  in
       uu____47668 ()
     else ());
    (let uu____47695 = flatten_fresh_scope ()  in
     FStar_ST.op_Colon_Equals bg_scope uu____47695)
                 ...
                 (match d with
                  | FStar_SMTEncoding_Term.Module (name,decls) ->
                      let decls1 =
                        FStar_List.filter
                          (fun uu___416_47833  ->
                             match uu___416_47833 with
                             | FStar_SMTEncoding_Term.Assume uu____47835 ->
                                 true
                             | uu____47837 -> false) decls
                         in
                      let n1 = FStar_List.length decls1  in
                      (((name, n1) :: out), (n1 + _total))
                  | uu____47854 -> (out, _total)))
    ...
    | (modules,total_decls) ->
        let modules1 =
          FStar_List.sortWith
            (fun uu____47916  ->
               fun uu____47917  ->
                 match (uu____47916, uu____47917) with
                 | ((uu____47943,n1),(uu____47945,m)) -> m - n1) modules
           in
        (if modules1 <> []
         then
           (let uu____47983 = FStar_Util.string_of_int total_decls  in
            FStar_Util.print1
         ...
         FStar_List.iter
           (fun uu____47998  ->
              match uu____47998 with
              | (m,n1) ->
                  if n1 <> (Prims.parse_int "0")
                  then
                    let uu____48014 = FStar_Util.string_of_int n1  in
                    FStar_Util.print2
...
let (mk_input :
  ...
  fun fresh  ->
    fun theory  ->
      ...
      (let uu____48073 = FStar_Options.print_z3_statistics ()  in
       if uu____48073 then context_profile theory else ());
      (let uu____48078 =
         let uu____48087 =
         ...
         then
             ...
             let uu____48109 =
               FStar_All.pipe_right theory
                 (FStar_Util.prefix_until
                    (fun uu___417_48137  ->
                       match uu___417_48137 with
                       | FStar_SMTEncoding_Term.CheckSat  -> true
                       | uu____48140 -> false))
                in
             FStar_All.pipe_right uu____48109 FStar_Option.get  in
           ...
           | (prefix1,check_sat,suffix) ->
               ...
               let hs =
                 let uu____48223 = FStar_Options.keep_query_captions ()  in
                 if uu____48223
                 then
                   let uu____48227 =
                     FStar_All.pipe_right prefix1
                      ...
                      in
                   FStar_All.pipe_right uu____48227
                     (FStar_String.concat "\n")
                 else ps  in
               let uu____48244 =
                 let uu____48248 = FStar_Util.digest_of_string hs  in
                 FStar_Pervasives_Native.Some uu____48248  in
           ...
           (let uu____48258 =
            ...
            (uu____48258, FStar_Pervasives_Native.None))
          in
       match uu____48078 with
       | (r,hash) ->
           let log_file_name =
             let uu____48302 = FStar_Options.log_queries ()  in
             if uu____48302
             then
               let uu____48308 = query_logging.write_to_log fresh r  in
               FStar_Pervasives_Native.Some uu____48308
              ...
              fun fresh  ->
                ...
                let theory1 =
                  FStar_List.append theory
                   ...
                   in
                let uu____48533 = filter_theory theory1  in
                match uu____48533 with
                | (theory2,_used_unsat_core) ->
                    let uu____48549 = mk_input fresh theory2  in
                    (match uu____48549 with
                         ...
                         let uu____48580 =
                           let uu____48582 =
                             fresh &&
                              ...
                              in
                           Prims.op_Negation uu____48582  in
                         if uu____48580
                         then
                           run_job
                             {
                  ...
                  let uu____48665 =
                    ...
                    | FStar_Pervasives_Native.None  ->
                        (FStar_ST.op_Colon_Equals bg_scope [];
                         (let uu____48701 = FStar_ST.op_Bang fresh_scope  in
                          FStar_List.rev uu____48701))
                     in
                  FStar_List.flatten uu____48665  in
                let theory1 =
                  FStar_List.append theory
                    (FStar_List.append [FStar_SMTEncoding_Term.Push]
                       (FStar_List.append qry [FStar_SMTEncoding_Term.Pop]))
                   in
                let uu____48730 = filter_theory theory1  in
                match uu____48730 with
                | (theory2,used_unsat_core) ->
                    let uu____48746 = mk_input true theory2  in
                    (match uu____48746 with
                     | (input,qhash,log_file_name) ->
                         let uu____48778 =
                           let uu____48780 =
                             cache_hit log_file_name cache qhash cb  in
                           Prims.op_Negation uu____48780  in
                         if uu____48778
                         then
                           enqueue
                             {
          ...
          fun qry  ->
            fun scope  ->
              fun cb  ->
                fun fresh  ->
                  let uu____48868 =
                    let uu____48870 = FStar_Options.n_cores ()  in
                    uu____48870 = (Prims.parse_int "1")  in
                  if uu____48868
