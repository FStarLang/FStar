...
let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  Prims.unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___96_1624 -> (match (uu___96_1624) with
...
(match (d) with
| FStar_SMTEncoding_Term.Assume (uu____1770, uu____1771, name) -> begin
...
let uu____1809 = (FStar_All.pipe_right core2 (FStar_List.filter (fun nm -> (

let uu____1814 = (FStar_All.pipe_right th (FStar_Util.for_some (fun uu___97_1816 -> (match (uu___97_1816) with
| FStar_SMTEncoding_Term.Assume (uu____1817, uu____1818, nm') -> begin
(nm = nm')
end
| uu____1820 -> begin
false
end))))
in (FStar_All.pipe_right uu____1814 Prims.op_Negation)))))
in (FStar_All.pipe_right uu____1809 (FStar_String.concat ", ")))
in (

let included = (

let uu____1823 = (FStar_All.pipe_right th (FStar_List.collect (fun uu___98_1827 -> (match (uu___98_1827) with
| FStar_SMTEncoding_Term.Assume (uu____1829, uu____1830, nm) -> begin
(nm)::[]
end
| uu____1832 -> begin
[]
end))))
in (FStar_All.pipe_right uu____1823 (FStar_String.concat ", ")))
in (FStar_Util.format2 "missed={%s}; included={%s}" missed included))))
in ((

let uu____1835 = ((FStar_Options.hint_info ()) && (FStar_Options.debug_any ()))
...
let uu____1842 = (FStar_Util.string_of_int n_retained)
in (

let uu____1843 = (match ((n1 <> n_retained)) with
| true -> begin
(

let uu____1846 = (FStar_Util.string_of_int n1)
in (FStar_Util.format2 " (expected %s (%s); replay may be inaccurate)" uu____1846 missed))
end
| uu____1850 -> begin
""
end)
...
let uu____1851 = (FStar_Util.string_of_int n_pruned)
...
let uu____1859 = (FStar_All.pipe_right core1 (FStar_String.concat ", "))
in (Prims.strcat "UNSAT CORE: " uu____1859))
in FStar_SMTEncoding_Term.Caption (uu____1858))
in (uu____1857)::[])
in (FStar_List.append theory' uu____1855))
in ((uu____1853), (true)));
))
end))
end))
...
let mk_cb = (fun used_unsat_core cb uu____1903 -> (match (uu____1903) with
| (uc_errs, time) -> begin
(match (used_unsat_core) with
| true -> begin
(match (uc_errs) with
| FStar_Util.Inl (uu____1932) -> begin
(cb ((uc_errs), (time)))
end
| FStar_Util.Inr (uu____1941, ek) -> begin
...
let uu____1969 = (FStar_List.map (FStar_SMTEncoding_Term.declToSmt (z3_options ())) theory)
in (FStar_All.pipe_right uu____1969 (FStar_String.concat "\n")))
in ((

let uu____1973 = (FStar_Options.log_queries ())
...
let uu____2020 = (FStar_ST.read bg_scope)
in (FStar_List.append uu____2020 (FStar_List.append ((FStar_SMTEncoding_Term.Push)::[]) (FStar_List.append qry ((FStar_SMTEncoding_Term.Pop)::[])))))
in (

let uu____2025 = (filter_assertions core theory)
...
let uu____2104 = (match (scope) with
| Some (s) -> begin
(FStar_List.rev s)
end
| None -> begin
...
let uu____2115 = (FStar_ST.read fresh_scope)
in (FStar_List.rev uu____2115));
)
end)
in (FStar_List.flatten uu____2104))
in (
...
let uu____2125 = (filter_assertions core theory1)
...
let uu____2199 = (FStar_Options.n_cores ())
in (uu____2199 = (Prims.parse_int "1")))
