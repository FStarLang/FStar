...
let z3version_as_string : z3version  ->  Prims.string = (fun uu___114_70 -> (match (uu___114_70) with
| Z3V_Unknown (s) -> begin
...
let x2 = (FStar_All.try_with (fun uu___118_198 -> (match (()) with
| () -> begin
(FStar_List.map FStar_Util.int_of_string (FStar_Util.split x1 "."))
end)) (fun uu___117_203 -> (match (uu___117_203) with
| uu____206 -> begin
[]
end)))
in (match (x2) with
...
in FStar_Util.HardError (uu____244))
in (FStar_Exn.raise uu____243))
end
| uu____246 -> begin
()
end));
...
let status_tag : z3status  ->  Prims.string = (fun uu___115_438 -> (match (uu___115_438) with
| SAT (uu____439) -> begin
"sat"
end
...
in (FStar_Util.start_process false id uu____519 uu____520 cond)))))
...
in (Prims.op_Equality uu____1767 FStar_Pervasives_Native.None))
...
in (FStar_Util.launch_process false uu____3023 uu____3024 uu____3025 input cond))))
end
| uu____3026 -> begin
(
...
let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  Prims.unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___116_3829 -> (match (uu___116_3829) with
...
let uu____4024 = (FStar_List.map (FStar_SMTEncoding_Term.declToSmt (z3_options ())) theory)
in (FStar_All.pipe_right uu____4024 (FStar_String.concat "\n")))
in ((

let uu____4030 = (FStar_Options.log_queries ())
...
let uu____4074 = (FStar_ST.op_Bang bg_scope)
in (FStar_List.append uu____4074 (FStar_List.append ((FStar_SMTEncoding_Term.Push)::[]) (FStar_List.append qry ((FStar_SMTEncoding_Term.Pop)::[])))))
in (

let uu____4095 = (filter_theory theory)
...
let uu____4172 = (match (scope) with
| FStar_Pervasives_Native.Some (s) -> begin
(FStar_List.rev s)
end
| FStar_Pervasives_Native.None -> begin
...
let uu____4203 = (FStar_ST.op_Bang fresh_scope)
in (FStar_List.rev uu____4203));
)
end)
in (FStar_List.flatten uu____4172))
in (
...
let uu____4239 = (filter_theory theory1)
...
let uu____4297 = (FStar_Options.n_cores ())
in (Prims.op_Equality uu____4297 (Prims.parse_int "1")))
