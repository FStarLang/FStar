...
let z3version_as_string : z3version -> Prims.string =
  fun uu___90_48  ->
    ...
    | Z3V (i,j,k) ->
        let _0_383 = FStar_Util.string_of_int i  in
        let _0_382 = FStar_Util.string_of_int j  in
        let _0_381 = FStar_Util.string_of_int k  in
        FStar_Util.format3 "%s.%s.%s" _0_383 _0_382 _0_381
...
let get_z3version : Prims.unit -> z3version =
  fun uu____97  ->
    ...
    | None  ->
        let uu____105 =
          let _0_384 = FStar_Options.z3_exe ()  in
          FStar_Util.run_proc _0_384 "-version" ""  in
  ...
  fun uu____140  ->
    ...
    (let uu____143 =
     ...
     then
       let _0_387 =
         FStar_Util.Failure
           (let _0_386 = z3version_as_string z3_v  in
            FStar_Util.format1
              "Z3 4.5.0 recommended; at least Z3 v4.4.1 required; got %s\n"
              _0_386)
          in
       FStar_All.pipe_left Prims.raise _0_387
     else ());
    (let _0_393 =
       let _0_392 =
         let _0_390 =
           let _0_389 =
             let _0_388 = FStar_Util.string_of_int (FStar_Options.z3_seed ())
                in
             FStar_Util.format1 "smt.random_seed=%s" _0_388  in
           [_0_389]  in
         "-smt2 -in auto_config=false model=true smt.relevancy=2" :: _0_390
          in
       let _0_391 = FStar_Options.z3_cliopt ()  in
       FStar_List.append _0_392 _0_391  in
     FStar_String.concat " " _0_393)
...
let tid : Prims.unit -> Prims.string =
  fun uu____238  ->
    let _0_394 = FStar_Util.current_tid ()  in
    FStar_All.pipe_right _0_394 FStar_Util.string_of_int
  
let new_z3proc : Prims.string -> FStar_Util.proc =
  fun id  ->
    let cond pid s = let x = (FStar_Util.trim_string s) = "Done!"  in x  in
    let _0_396 = FStar_Options.z3_exe ()  in
    let _0_395 = ini_params ()  in
    FStar_Util.start_process id _0_396 _0_395 cond
...
let query_logging : query_log =
  ...
  let new_log_file uu____425 =
    ...
    | Some n ->
        let file_name =
          let uu____433 =
            let _0_397 = FStar_ST.read used_file_names  in
            FStar_List.tryFind
              (fun uu____439  ->
                 match uu____439 with | (m,uu____443) -> n = m) _0_397
             in
          match uu____433 with
          | None  ->
              ((let _0_399 =
                  let _0_398 = FStar_ST.read used_file_names  in
                  (n, (Prims.parse_int "0")) :: _0_398  in
                FStar_ST.write used_file_names _0_399);
               n)
          | Some (uu____464,k) ->
              ((let _0_401 =
                  let _0_400 = FStar_ST.read used_file_names  in
                  (n, (k + (Prims.parse_int "1"))) :: _0_400  in
                FStar_ST.write used_file_names _0_401);
               (let _0_402 =
                  FStar_Util.string_of_int (k + (Prims.parse_int "1"))  in
                FStar_Util.format2 "%s-%s" n _0_402))
           in
  ...
  let append_to_log str =
    let _0_403 = get_log_file ()  in FStar_Util.append_to_file _0_403 str  in
  let write_to_new_log str =
    ...
    (let _0_405 =
       let _0_404 = FStar_ST.read query_number  in
       _0_404 + (Prims.parse_int "1")  in
     FStar_ST.write query_number _0_405);
    (let file_name =
       let _0_406 = FStar_Util.string_of_int qnum  in
       FStar_Util.format1 "query-%s.smt2" _0_406  in
     let file_name = FStar_Util.concat_dir_filename dir_name file_name  in
     FStar_Util.write_file file_name str)
     in
  let write_to_log str =
    let uu____542 =
      let _0_407 = FStar_Options.n_cores ()  in
      _0_407 > (Prims.parse_int "1")  in
...
let bg_z3_proc : bgproc =
  ...
  let new_proc =
    ...
    fun uu____579  ->
      new_z3proc
        (let _0_409 =
           FStar_Util.incr ctr;
           (let _0_408 = FStar_ST.read ctr  in
            FStar_All.pipe_right _0_408 FStar_Util.string_of_int)
            in
         FStar_Util.format1 "bg-%s" _0_409)
     in
  let z3proc uu____589 =
    (let uu____591 = let _0_410 = FStar_ST.read the_z3proc  in _0_410 = None
        in
     if uu____591
     then
       let _0_411 = Some (new_proc ())  in FStar_ST.write the_z3proc _0_411
     else ());
  ...
  let refresh uu____619 =
    ...
    (let _0_412 = Some (new_proc ())  in FStar_ST.write the_z3proc _0_412);
  ...
  let restart uu____630 =
    ...
    (let _0_413 = Some (new_proc ())  in FStar_ST.write the_z3proc _0_413);
...
let at_log_file : Prims.unit -> Prims.string =
  fun uu____643  ->
    ...
    if uu____644
    then
      let _0_414 = query_logging.log_file_name ()  in Prims.strcat "@" _0_414
    else ""
    ...
    fun input  ->
      let parse z3out =
        ...
        let print_stats lines =
          let starts_with c s =
            ((FStar_String.length s) >= (Prims.parse_int "1")) &&
              (let _0_415 = FStar_String.get s (Prims.parse_int "0")  in
               _0_415 = c)
             in
          let ends_with c s =
            ((FStar_String.length s) >= (Prims.parse_int "1")) &&
          ...
          then
            let uu____699 =
              (((FStar_List.length lines) >= (Prims.parse_int "2")) &&
                 (let _0_417 = FStar_List.hd lines  in starts_with '(' _0_417))
                && (let _0_418 = last lines  in ends_with ')' _0_418)
               in
            (if uu____699
             then
               (FStar_Util.print_string
                  (let _0_421 =
                     let _0_419 = query_logging.get_module_name ()  in
                     FStar_Util.format1 "BEGIN-STATS %s\n" _0_419  in
                   let _0_420 = at_log_file ()  in Prims.strcat _0_421 _0_420);
                FStar_List.iter
                  (fun s  ->
        ...
        let rec result x =
          ...
          | hd::tl ->
              ((let _0_428 =
                  let _0_427 = query_logging.get_module_name ()  in
                  FStar_Util.format2 "%s: Unexpected output from Z3: %s\n"
                    _0_427 hd
                   in
                FStar_Errors.warn FStar_Range.dummyRange _0_428);
               result tl)
          | uu____845 ->
              let _0_431 =
                let _0_430 =
                ...
                FStar_Util.format1
                 ...
                 in
              FStar_All.pipe_left failwith _0_431
           in
        result lines  in
      let cond pid s = let x = (FStar_Util.trim_string s) = "Done!"  in x  in
      let stdout =
        ...
        then
          let _0_434 = tid ()  in
          let _0_433 = FStar_Options.z3_exe ()  in
          let _0_432 = ini_params ()  in
          FStar_Util.launch_process _0_434 _0_433 _0_432 input cond
        else
...
let z3_job :
  ...
  fun fresh  ->
    fun label_messages  ->
      fun input  ->
        fun uu____989  ->
          let ekind uu___92_1005 =
            match uu___92_1005 with
          ...
          let uu____1013 =
            let _0_435 = FStar_Util.now ()  in
            FStar_Util.time_diff start _0_435  in
                ...
                | TIMEOUT lblnegs|SAT lblnegs|UNKNOWN lblnegs ->
                    ((let uu____1102 = FStar_Options.debug_any ()  in
                      ...
                      then
                        let _0_436 =
                          FStar_Util.format1 "Z3 says: %s\n"
                           ...
                           in
                        FStar_All.pipe_left FStar_Util.print_string _0_436
                      else ());
                     (let failing_assertions =
                      ...
                      let _0_438 =
                        FStar_Util.Inr
                          (let _0_437 = ekind status  in
                           (failing_assertions, _0_437))
                         in
                      (_0_438, elapsed_time)))
                 in
              result
...
and run_job : z3job -> Prims.unit =
  fun j  -> let _0_439 = j.job ()  in FStar_All.pipe_left j.callback _0_439
  ...
  fun fresh  ->
    fun j  ->
      ...
      then run_job j
        ...
        (FStar_Util.monitor_enter job_queue;
         (let _0_441 =
            let _0_440 = FStar_ST.read job_queue  in
            FStar_List.append _0_440 [j]  in
          FStar_ST.write job_queue _0_441);
    ...
    let rec aux uu____1361 =
      let uu____1362 =
        with_monitor job_queue
          (fun uu____1371  ->
             let _0_443 = FStar_ST.read pending_jobs  in
             let _0_442 = FStar_List.length (FStar_ST.read job_queue)  in
             (_0_443, _0_442))
         in
      ...
      | (n,m) ->
          ...
          then
            (FStar_ST.write running false;
             (let _0_444 = FStar_Errors.report_all ()  in
              FStar_All.pipe_right _0_444 Prims.ignore))
...
let push : Prims.string -> Prims.unit =
  fun msg  ->
    (let _0_446 =
       let _0_445 = FStar_ST.read fresh_scope  in
       [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push] ::
         _0_445
        in
     FStar_ST.write fresh_scope _0_446);
    (let _0_448 =
       let _0_447 = FStar_ST.read bg_scope  in
       FStar_List.append
         [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push]
         _0_447
        in
     FStar_ST.write bg_scope _0_448)
  
let pop : Prims.string -> Prims.unit =
  fun msg  ->
    (let _0_449 = FStar_List.tl (FStar_ST.read fresh_scope)  in
     FStar_ST.write fresh_scope _0_449);
    (let _0_451 =
       let _0_450 = FStar_ST.read bg_scope  in
       FStar_List.append
         [FStar_SMTEncoding_Term.Pop; FStar_SMTEncoding_Term.Caption msg]
         _0_450
        in
     FStar_ST.write bg_scope _0_451)
...
let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list -> Prims.unit =
  fun decls  ->
    ...
    (let _0_453 =
       let _0_452 = FStar_ST.read bg_scope  in
       FStar_List.append (FStar_List.rev decls) _0_452  in
     FStar_ST.write bg_scope _0_453)
  ...
  fun fresh  ->
    ...
    then
      (FStar_ST.write bg_scope [];
       (let _0_454 = FStar_List.rev (FStar_ST.read fresh_scope)  in
        FStar_All.pipe_right _0_454 FStar_List.flatten))
    else
...
let refresh : Prims.unit -> Prims.unit =
  fun uu____1510  ->
    (let uu____1512 =
       let _0_455 = FStar_Options.n_cores ()  in
       _0_455 < (Prims.parse_int "2")  in
  ...
  fun core  ->
    fun label_messages  ->
      fun qry  ->
        fun cb  ->
          let fresh =
            let _0_456 = FStar_Options.n_cores ()  in
            _0_456 > (Prims.parse_int "1")  in
                 ...
                 | (theory',n_retained,n_pruned) ->
                                     ...
                                     FStar_All.pipe_right th
                                       (FStar_Util.for_some
                                          (fun uu___94_1691  ->
                                             ...
                                             | uu____1696 -> false))
                                      in
                                   FStar_All.pipe_right _0_457
                                     Prims.op_Negation))
                            in
                         FStar_All.pipe_right _0_458
                           (FStar_String.concat ", ")
                          in
                       let included =
                         let _0_459 =
                     ...
                     ((let uu____1709 =
                       ...
                       then
                         ...
                         let _0_463 = FStar_Util.string_of_int n_retained  in
                         let _0_462 =
                           if n <> n_retained
                           then
                             let _0_460 = FStar_Util.string_of_int n  in
                             FStar_Util.format2
                               " (expected %s (%s); replay may be inaccurate)"
                               _0_460 missed
                           else ""  in
                         let _0_461 = FStar_Util.string_of_int n_pruned  in
                         FStar_Util.print3
                             ...
                             FStar_SMTEncoding_Term.Caption
                               (let _0_464 =
                                  FStar_All.pipe_right core
                                    (FStar_String.concat ", ")
                                   in
                                Prims.strcat "UNSAT CORE: " _0_464)
                              in
                           [_0_465]  in
                         FStar_List.append theory' _0_466  in
                       (_0_467, true))))
             in
              ...
              let input =
                let _0_468 =
                  FStar_List.map
                   ...
                   in
                FStar_All.pipe_right _0_468 (FStar_String.concat "\n")  in
