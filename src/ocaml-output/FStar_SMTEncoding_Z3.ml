...
let (parse_z3_version_lines :
  ...
  fun out ->
    ...
    | x::uu____46 ->
        ...
        let rec aux uu___39_60 =
          match uu___39_60 with
          | hash::[] ->
              let n1 =
              ...
              then
                ((let uu____71 = FStar_Options.debug_any () in
                  if uu____71
                  then
                    let msg =
                      FStar_Util.format1
        ...
        aux parts
    | uu____83 -> FStar_Pervasives_Native.Some "No Z3 version string found"
let (z3hash_warning_message :
  ...
  fun uu____94 ->
    let run_proc_result =
      try
        let uu____123 =
          let uu____130 = FStar_Options.z3_exe () in
          FStar_Util.run_proc uu____130 "-version" "" in
        FStar_Pervasives_Native.Some uu____123
      with | uu____148 -> FStar_Pervasives_Native.None in
    match run_proc_result with
    | FStar_Pervasives_Native.None ->
        FStar_Pervasives_Native.Some
          (FStar_Errors.Error_Z3InvocationError, "Could not run Z3")
    | FStar_Pervasives_Native.Some (uu____171, out, uu____173) ->
        let uu____180 = parse_z3_version_lines out in
...
let (check_z3hash : Prims.unit -> Prims.unit) =
  fun uu____200 ->
    let uu____201 =
      let uu____202 = FStar_ST.op_Bang _z3hash_checked in
      Prims.op_Negation uu____202 in
    if uu____201
    then
      (FStar_ST.op_Colon_Equals _z3hash_checked true;
       (let uu____254 = z3hash_warning_message () in
    ...
    (let uu____277 =
       let uu____280 =
         let uu____283 =
           let uu____286 =
             let uu____287 =
               let uu____288 = FStar_Options.z3_seed () in
               FStar_Util.string_of_int uu____288 in
             FStar_Util.format1 "smt.random_seed=%s" uu____287 in
           [uu____286] in
         "-smt2 -in auto_config=false model=true smt.relevancy=2 smt.case_split=3"
           :: uu____283 in
       let uu____289 = FStar_Options.z3_cliopt () in
       FStar_List.append uu____280 uu____289 in
     FStar_String.concat " " uu____277)
...
let (uu___is_UNSAT : z3status -> Prims.bool) =
  fun projectee ->
    match projectee with | UNSAT _0 -> true | uu____334 -> false
let (__proj__UNSAT__item___0 : z3status -> unsat_core) =
  fun projectee -> match projectee with | UNSAT _0 -> _0
let (uu___is_SAT : z3status -> Prims.bool) =
  fun projectee -> match projectee with | SAT _0 -> true | uu____352 -> false
...
let (uu___is_UNKNOWN : z3status -> Prims.bool) =
  fun projectee ->
    match projectee with | UNKNOWN _0 -> true | uu____388 -> false
...
let (uu___is_KILLED : z3status -> Prims.bool) =
  fun projectee -> match projectee with | KILLED -> true | uu____453 -> false
type z3statistics = Prims.string FStar_Util.smap[@@deriving show]
let (status_tag : z3status -> Prims.string) =
  fun uu___40_458 ->
    match uu___40_458 with
    | SAT uu____459 -> "sat"
    | UNSAT uu____466 -> "unsat"
    | UNKNOWN uu____467 -> "unknown"
    | TIMEOUT uu____474 -> "timeout"
    | KILLED -> "killed"
...
let (tid : Prims.unit -> Prims.string) =
  fun uu____523 ->
    let uu____524 = FStar_Util.current_tid () in
    FStar_All.pipe_right uu____524 FStar_Util.string_of_int
let (new_z3proc : Prims.string -> FStar_Util.proc) =
  fun id1 ->
    let cond pid s = let x = (FStar_Util.trim_string s) = "Done!" in x in
    let uu____536 = FStar_Options.z3_exe () in
    let uu____537 = ini_params () in
    FStar_Util.start_process false id1 uu____536 uu____537 cond
type bgproc =
  {
...
let (query_logging : query_log) =
  ...
  let get_module_name uu____945 =
    let uu____946 = FStar_ST.op_Bang current_module_name in
    match uu____946 with
    | FStar_Pervasives_Native.None -> failwith "Module name not set"
    | FStar_Pervasives_Native.Some n1 -> n1 in
  let new_log_file uu____1052 =
    let uu____1053 = FStar_ST.op_Bang current_module_name in
    match uu____1053 with
    | FStar_Pervasives_Native.None -> failwith "current module not set"
    | FStar_Pervasives_Native.Some n1 ->
        let file_name =
          let uu____1157 =
            let uu____1164 = FStar_ST.op_Bang used_file_names in
            FStar_List.tryFind
              (fun uu____1285 ->
                 match uu____1285 with | (m, uu____1291) -> n1 = m)
              uu____1164 in
          match uu____1157 with
          | FStar_Pervasives_Native.None ->
              ((let uu____1297 =
                  let uu____1304 = FStar_ST.op_Bang used_file_names in
                  (n1, (Prims.parse_int "0")) :: uu____1304 in
                FStar_ST.op_Colon_Equals used_file_names uu____1297);
               n1)
          | FStar_Pervasives_Native.Some (uu____1529, k) ->
              ((let uu____1536 =
                  let uu____1543 = FStar_ST.op_Bang used_file_names in
                  (n1, (k + (Prims.parse_int "1"))) :: uu____1543 in
                FStar_ST.op_Colon_Equals used_file_names uu____1536);
               (let uu____1768 =
                  FStar_Util.string_of_int (k + (Prims.parse_int "1")) in
                FStar_Util.format2 "%s-%s" n1 uu____1768)) in
        ...
        (FStar_ST.op_Colon_Equals current_file_name
         ...
         (let fh = FStar_Util.open_file_for_writing file_name1 in
          ...
          fh)) in
  let get_log_file uu____1974 =
    let uu____1975 = FStar_ST.op_Bang log_file_opt in
    match uu____1975 with
    | FStar_Pervasives_Native.None -> new_log_file ()
    | FStar_Pervasives_Native.Some fh -> fh in
  let append_to_log str =
    let uu____2082 = get_log_file () in
    FStar_Util.append_to_file uu____2082 str in
  let write_to_new_log str =
    let dir_name =
      let uu____2088 = FStar_ST.op_Bang current_file_name in
      match uu____2088 with
      | FStar_Pervasives_Native.None ->
          let dir_name =
            let uu____2191 = FStar_ST.op_Bang current_module_name in
    ...
    (let uu____2493 =
       let uu____2494 = FStar_ST.op_Bang query_number in
       uu____2494 + (Prims.parse_int "1") in
     FStar_ST.op_Colon_Equals query_number uu____2493);
    (let file_name =
       let uu____2686 = FStar_Util.string_of_int qnum in
       FStar_Util.format1 "query-%s.smt2" uu____2686 in
  ...
  let write_to_log str =
    let uu____2692 =
      let uu____2693 = FStar_Options.n_cores () in
      uu____2693 > (Prims.parse_int "1") in
    if uu____2692 then write_to_new_log str else append_to_log str in
  let close_log uu____2698 =
    let uu____2699 = FStar_ST.op_Bang log_file_opt in
    match uu____2699 with
    | FStar_Pervasives_Native.None -> ()
    | FStar_Pervasives_Native.Some fh ->
        (FStar_Util.close_file fh;
         FStar_ST.op_Colon_Equals log_file_opt FStar_Pervasives_Native.None) in
  let log_file_name uu____2905 =
    let uu____2906 = FStar_ST.op_Bang current_file_name in
  ...
  let new_proc =
    ...
    fun uu____3022 ->
      let uu____3023 =
        let uu____3024 =
          FStar_Util.incr ctr;
          (let uu____3131 = FStar_ST.op_Bang ctr in
           FStar_All.pipe_right uu____3131 FStar_Util.string_of_int) in
        FStar_Util.format1 "bg-%s" uu____3024 in
      new_z3proc uu____3023 in
  let z3proc uu____3230 =
    (let uu____3232 =
       let uu____3233 = FStar_ST.op_Bang the_z3proc in
       uu____3233 = FStar_Pervasives_Native.None in
     if uu____3232
     then
       let uu____3337 =
         let uu____3340 = new_proc () in
         FStar_Pervasives_Native.Some uu____3340 in
       FStar_ST.op_Colon_Equals the_z3proc uu____3337
     else ());
    (let uu____3441 = FStar_ST.op_Bang the_z3proc in
     FStar_Util.must uu____3441) in
  let x = [] in
  let grab uu____3549 = FStar_Util.monitor_enter x; z3proc () in
  let release uu____3556 = FStar_Util.monitor_exit x in
  let refresh uu____3562 =
    let proc = grab () in
    FStar_Util.kill_process proc;
    (let uu____3566 =
       let uu____3569 = new_proc () in
       FStar_Pervasives_Native.Some uu____3569 in
     FStar_ST.op_Colon_Equals the_z3proc uu____3566);
    query_logging.close_log ();
    release () in
  let restart uu____3673 =
    FStar_Util.monitor_enter ();
    query_logging.close_log ();
    FStar_ST.op_Colon_Equals the_z3proc FStar_Pervasives_Native.None;
    (let uu____3777 =
       let uu____3780 = new_proc () in
       FStar_Pervasives_Native.Some uu____3780 in
     FStar_ST.op_Colon_Equals the_z3proc uu____3777);
    FStar_Util.monitor_exit () in
  { grab; release; refresh; restart }
let (at_log_file : Prims.unit -> Prims.string) =
  fun uu____3882 ->
    let uu____3883 = FStar_Options.log_queries () in
    if uu____3883
    then
      let uu____3884 = query_logging.log_file_name () in
      Prims.strcat "@" uu____3884
    else ""
...
let (smt_output_sections : Prims.string Prims.list -> smt_output) =
  fun lines ->
    let rec until tag lines1 =
      ...
      | l::lines2 ->
          ...
          then FStar_Pervasives_Native.Some ([], lines2)
            ...
            (let uu____4085 = until tag lines2 in
             FStar_Util.map_opt uu____4085
               (fun uu____4115 ->
    ...
    let find_section tag lines1 =
      let uu____4185 = until (start_tag tag) lines1 in
      match uu____4185 with
      | FStar_Pervasives_Native.None ->
          (FStar_Pervasives_Native.None, lines1)
      | FStar_Pervasives_Native.Some (prefix1, suffix) ->
          let uu____4240 = until (end_tag tag) suffix in
    ...
    let uu____4305 = find_section "result" lines in
    match uu____4305 with
    | (result_opt, lines1) ->
        let result = FStar_Util.must result_opt in
        let uu____4335 = find_section "reason-unknown" lines1 in
        (match uu____4335 with
         | (reason_unknown, lines2) ->
             let uu____4360 = find_section "unsat-core" lines2 in
             (match uu____4360 with
              | (unsat_core, lines3) ->
                  let uu____4385 = find_section "statistics" lines3 in
                  (match uu____4385 with
                   | (statistics, lines4) ->
                       let uu____4410 = find_section "labels" lines4 in
                       (match uu____4410 with
                        | (labels, lines5) ->
                            let remaining =
                              let uu____4438 = until "Done!" lines5 in
                            ...
                            ((match remaining with
                              ...
                              | uu____4478 ->
                                  ...
                                  FStar_Errors.log_issue
                                    FStar_Range.dummyRange uu____4481);
                             (let uu____4488 = FStar_Util.must result_opt in
                              {
                                smt_result = uu____4488;
    ...
    fun input ->
      fun label_messages ->
        let parse z3out =
            ...
            | FStar_Pervasives_Native.Some lines1 ->
                let rec lblnegs lines2 =
                  ...
                  | lname::"false"::rest when
                      ...
                      let uu____4609 = lblnegs rest in lname :: uu____4609
                  | lname::uu____4613::rest when
          ...
          let status =
            (let uu____4832 = FStar_Options.debug_any () in
             if uu____4832
             then
               let uu____4833 =
                 FStar_Util.format1 "Z3 says: %s\n"
                   (FStar_String.concat "\n" smt_output.smt_result) in
               FStar_All.pipe_left FStar_Util.print_string uu____4833
             else ());
             ...
             | uu____4878 ->
                 let uu____4879 =
                   FStar_Util.format1
                     ...
                     (FStar_String.concat "\n" smt_output.smt_result) in
                 failwith uu____4879) in
          (status, statistics) in
        let cond pid s = let x = (FStar_Util.trim_string s) = "Done!" in x in
        let stdout1 =
          if fresh
          then
            let uu____4889 = tid () in
            let uu____4890 = FStar_Options.z3_exe () in
            let uu____4891 = ini_params () in
            FStar_Util.launch_process false uu____4889 uu____4890 uu____4891
              input cond
          else
...
let with_monitor :
  'Auu____5099 'Auu____5100 .
    'Auu____5099 -> (Prims.unit -> 'Auu____5100) -> 'Auu____5100
  =
  fun m ->
    fun f ->
        ...
        fun qhash ->
          fun uu____5133 ->
            ...
            let uu____5137 =
              ...
              with
              | uu____5161 when
                  let uu____5162 = FStar_Options.trace_error () in
                  ...
                  (bg_z3_proc.refresh ();
                   (let uu____5164 =
                    ...
                    ((UNKNOWN
                        ([],
                          (FStar_Pervasives_Native.Some
                             "Z3 raised an exception"))), uu____5164))) in
            match uu____5137 with
            | (status, statistics) ->
                let uu____5175 =
                  let uu____5180 = FStar_Util.now () in
                  FStar_Util.time_diff start uu____5180 in
                (match uu____5175 with
                 | (uu____5181, elapsed_time) ->
                     {
...
let rec (dequeue' : Prims.unit -> Prims.unit) =
  fun uu____5226 ->
    let j =
      let uu____5228 = FStar_ST.op_Bang job_queue in
...
and (dequeue : Prims.unit -> Prims.unit) =
  fun uu____5309 ->
    let uu____5310 = FStar_ST.op_Bang running in
    if uu____5310
    then
      let rec aux uu____5339 =
        FStar_Util.monitor_enter job_queue;
        (let uu____5345 = FStar_ST.op_Bang job_queue in
         match uu____5345 with
         | [] ->
             (FStar_Util.monitor_exit job_queue;
              FStar_Util.sleep (Prims.parse_int "50");
              aux ())
         | uu____5383 -> dequeue' ()) in
      aux ()
    else ()
and (run_job : z3job -> Prims.unit) =
  fun j ->
    let uu____5387 = j.job () in FStar_All.pipe_left j.callback uu____5387
...
let (enqueue : z3job -> Prims.unit) =
  fun j ->
    FStar_Util.monitor_enter job_queue;
    (let uu____5434 =
       let uu____5437 = FStar_ST.op_Bang job_queue in
       FStar_List.append uu____5437 [j] in
     FStar_ST.op_Colon_Equals job_queue uu____5434);
    FStar_Util.monitor_pulse job_queue;
    FStar_Util.monitor_exit job_queue
let (finish : Prims.unit -> Prims.unit) =
  fun uu____5509 ->
    let rec aux uu____5513 =
      let uu____5514 =
        with_monitor job_queue
          (fun uu____5530 ->
             let uu____5531 = FStar_ST.op_Bang pending_jobs in
             let uu____5557 =
               let uu____5558 = FStar_ST.op_Bang job_queue in
               FStar_List.length uu____5558 in
             (uu____5531, uu____5557)) in
...
let (mk_fresh_scope : Prims.unit -> scope_t) =
  fun uu____5682 -> FStar_ST.op_Bang fresh_scope
let (bg_scope : FStar_SMTEncoding_Term.decl Prims.list FStar_ST.ref) =
  FStar_Util.mk_ref []
let (push : Prims.string -> Prims.unit) =
  fun msg ->
    (let uu____5761 =
       let uu____5766 = FStar_ST.op_Bang fresh_scope in
       [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push] ::
         uu____5766 in
     FStar_ST.op_Colon_Equals fresh_scope uu____5761);
    (let uu____5839 =
       let uu____5842 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5842
         [FStar_SMTEncoding_Term.Push; FStar_SMTEncoding_Term.Caption msg] in
     FStar_ST.op_Colon_Equals bg_scope uu____5839)
let (pop : Prims.string -> Prims.unit) =
  fun msg ->
    (let uu____5907 =
       let uu____5912 = FStar_ST.op_Bang fresh_scope in
       FStar_List.tl uu____5912 in
     FStar_ST.op_Colon_Equals fresh_scope uu____5907);
    (let uu____5985 =
       let uu____5988 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5988
         [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Pop] in
     FStar_ST.op_Colon_Equals bg_scope uu____5985)
let (giveZ3 : FStar_SMTEncoding_Term.decl Prims.list -> Prims.unit) =
  fun decls ->
    FStar_All.pipe_right decls
      (FStar_List.iter
         (fun uu___41_6060 ->
            match uu___41_6060 with
            | FStar_SMTEncoding_Term.Push -> failwith "Unexpected push/pop"
            | FStar_SMTEncoding_Term.Pop -> failwith "Unexpected push/pop"
            | uu____6061 -> ()));
    (let uu____6063 = FStar_ST.op_Bang fresh_scope in
    ...
    (let uu____6151 =
       let uu____6154 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____6154 decls in
     FStar_ST.op_Colon_Equals bg_scope uu____6151)
let (refresh : Prims.unit -> Prims.unit) =
  fun uu____6217 ->
    (let uu____6219 =
       let uu____6220 = FStar_Options.n_cores () in
       uu____6220 < (Prims.parse_int "2") in
     if uu____6219 then bg_z3_proc.refresh () else ());
    (let uu____6222 =
       let uu____6225 =
         let uu____6230 = FStar_ST.op_Bang fresh_scope in
         FStar_List.rev uu____6230 in
       FStar_List.flatten uu____6225 in
     FStar_ST.op_Colon_Equals bg_scope uu____6222)
let (mk_input :
    ...
    let uu____6313 =
      ...
      then
          ...
          let uu____6338 =
            FStar_All.pipe_right theory
              (FStar_Util.prefix_until
                 (fun uu___42_6366 ->
                    match uu___42_6366 with
                    | FStar_SMTEncoding_Term.CheckSat -> true
                    | uu____6367 -> false)) in
          FStar_All.pipe_right uu____6338 FStar_Option.get in
        ...
        | (prefix1, check_sat, suffix) ->
            ...
            let uncaption uu___43_6445 =
              match uu___43_6445 with
              | FStar_SMTEncoding_Term.Caption uu____6446 ->
                  FStar_SMTEncoding_Term.Caption ""
              | FStar_SMTEncoding_Term.Assume a ->
                  FStar_SMTEncoding_Term.Assume
                    (let uu___48_6450 = a in
                     {
                       FStar_SMTEncoding_Term.assumption_term =
                         (uu___48_6450.FStar_SMTEncoding_Term.assumption_term);
                       FStar_SMTEncoding_Term.assumption_caption =
                         FStar_Pervasives_Native.None;
                       FStar_SMTEncoding_Term.assumption_name =
                         (uu___48_6450.FStar_SMTEncoding_Term.assumption_name);
                       FStar_SMTEncoding_Term.assumption_fact_ids =
                         (uu___48_6450.FStar_SMTEncoding_Term.assumption_fact_ids)
                     })
              | FStar_SMTEncoding_Term.DeclFun (n1, a, s, uu____6454) ->
                  FStar_SMTEncoding_Term.DeclFun
                    (n1, a, s, FStar_Pervasives_Native.None)
              | FStar_SMTEncoding_Term.DefineFun (n1, a, s, b, uu____6467) ->
            ...
            let hs =
              let uu____6478 =
                let uu____6481 =
                  ...
                  FStar_All.pipe_right uu____6484 pp_no_cap in
                FStar_All.pipe_right uu____6481
                  (FStar_List.filter (fun s -> s <> "")) in
              FStar_All.pipe_right uu____6478 (FStar_String.concat "\n") in
            let uu____6503 =
              let uu____6506 = FStar_Util.digest_of_string hs in
              FStar_Pervasives_Native.Some uu____6506 in
            ((Prims.strcat ps (Prims.strcat "\n" ss)), uu____6503)
      else
        (let uu____6510 =
           let uu____6511 =
             FStar_List.map (FStar_SMTEncoding_Term.declToSmt options) theory in
           FStar_All.pipe_right uu____6511 (FStar_String.concat "\n") in
         (uu____6510, FStar_Pervasives_Native.None)) in
    match uu____6313 with
    | (r, hash) ->
        ((let uu____6531 = FStar_Options.log_queries () in
          if uu____6531 then query_logging.write_to_log r else ());
         (r, hash))
    ...
    fun cache ->
      fun label_messages ->
        fun qry ->
          fun cb ->
            let theory =
              let uu____6608 = FStar_ST.op_Bang bg_scope in
              FStar_List.append uu____6608
                (FStar_List.append [FStar_SMTEncoding_Term.Push]
                   (FStar_List.append qry [FStar_SMTEncoding_Term.Pop])) in
            let uu____6640 = filter_theory theory in
            match uu____6640 with
            | (theory1, used_unsat_core) ->
                let uu____6647 = mk_input theory1 in
                (match uu____6647 with
                 | (input, qhash) ->
                     (FStar_ST.op_Colon_Equals bg_scope [];
                      (let uu____6690 =
                         let uu____6691 = cache_hit cache qhash cb in
                         Prims.op_Negation uu____6691 in
                       if uu____6690
                       then
                         run_job
                           {
            ...
            fun cb ->
              let theory =
                let uu____6741 =
                  ...
                  | FStar_Pervasives_Native.None ->
                      (FStar_ST.op_Colon_Equals bg_scope [];
                       (let uu____6783 = FStar_ST.op_Bang fresh_scope in
                        FStar_List.rev uu____6783)) in
                FStar_List.flatten uu____6741 in
              let theory1 =
                FStar_List.append theory
                  (FStar_List.append [FStar_SMTEncoding_Term.Push]
                     (FStar_List.append qry [FStar_SMTEncoding_Term.Pop])) in
              let uu____6826 = filter_theory theory1 in
              match uu____6826 with
              | (theory2, used_unsat_core) ->
                  let uu____6833 = mk_input theory2 in
                  (match uu____6833 with
                   | (input, qhash) ->
                       let uu____6846 =
                         let uu____6847 = cache_hit cache qhash cb in
                         Prims.op_Negation uu____6847 in
                       if uu____6846
                       then
                         enqueue
                           {
      ...
      fun label_messages ->
        fun qry ->
          fun scope ->
            fun cb ->
              let uu____6896 =
                let uu____6897 = FStar_Options.n_cores () in
