  ...
  let next_file_name uu___ =
    ...
    let file_name =
      let uu___1 =
        ...
        FStar_List.tryFind
          (fun uu___3  -> match uu___3 with | (m,uu___4) -> n = m) uu___2
         in
      match uu___1 with
      | FStar_Pervasives_Native.None  ->
          ((let uu___3 =
      ...
      | FStar_Pervasives_Native.Some (uu___2,k) ->
          ((let uu___4 =
           ...
           (let uu___4 = FStar_Util.string_of_int (k + Prims.int_one)  in
            FStar_Util.format2 "%s-%s" n uu___4))
       in
      ...
      let uu___ =
        let uu___1 =
          ...
          (let uu___3 = FStar_ST.op_Bang ctr  in
           FStar_All.pipe_right uu___3 FStar_Util.string_of_int)
           in
...
let (bg_z3_proc : bgproc FStar_ST.ref) =
  ...
  let ask input =
    ...
    (let kill_handler uu___1 = "\nkilled\n"  in
     ...
     FStar_Util.ask_process uu___1 input kill_handler)
     in
  let maybe_kill_z3proc uu___ =
    ...
    then
      ((let uu___3 =
          let uu___4 = FStar_ST.op_Bang the_z3proc  in FStar_Util.must uu___4
           in
              ...
              (match uu___1 with
               ...
               | FStar_Pervasives_Native.Some (section,suffix1) ->
                   ((FStar_Pervasives_Native.Some section),
                     (FStar_List.append prefix suffix1)))
           in
                           ...
                           (match uu___4 with
                            | (labels,lines5) ->
                                let remaining =
                                  ...
                                  | FStar_Pervasives_Native.Some
                                      (prefix,suffix) ->
                                      FStar_List.append prefix suffix
                                   in
                ...
                | FStar_Pervasives_Native.Some lines1 ->
                    let parse_line line =
                      ...
                      | ""::entry::[] ->
                          ...
                          FStar_Util.smap_add statistics1 key value
                      | uu___ -> ()  in
                    (FStar_List.iter parse_line lines1; statistics1)
                 in
              let reason_unknown =
             ...
             | (out,_total) ->
                 (match d with
                  | FStar_SMTEncoding_Term.Module (name,decls) ->
                      ...
                      (((name, n) :: out), (n + _total))
                  | uu___2 -> (out, _total))) ([], Prims.int_zero) theory
       in
    match uu___ with
    | (modules,total_decls) ->
        let modules1 =
              ...
              fun uu___  ->
                let uu___1 =
                  ...
                  FStar_Profiling.profile
                    ...
                    uu___2 "FStar.SMTEncoding.Z3 (aggregate query time)"
                   in
                match uu___1 with
                ...
                let theory =
                  ...
                  then flatten_fresh_scope ()
                    ...
                    (let theory1 = FStar_ST.op_Bang bg_scope  in
                     FStar_ST.op_Colon_Equals bg_scope []; theory1)
                   in
                let theory1 =
                ...
                | (theory2,_used_unsat_core) ->
                    ...
                    (match uu___1 with
                     | (input,qhash,log_file_name) ->
                         ...
                         if fresh
                         then
                           let uu___2 = cache_hit log_file_name cache qhash
                              in
                           (match uu___2 with
