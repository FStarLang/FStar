...
let uu____3291 = (FStar_All.pipe_right (FStar_Util.split s2 " ") (FStar_Util.sort_with FStar_String.compare))
in FStar_Pervasives_Native.Some (uu____3291))
end)))
end)
in (
...
let uu____3354 = (lblnegs rest)
in (lname)::uu____3354)
end
| (lname)::(uu____3358)::rest when (FStar_Util.starts_with lname "label_") -> begin
(lblnegs rest)
end
| uu____3362 -> begin
[]
...
let uu____3395 = (FStar_All.pipe_right label_messages (FStar_List.tryFind (fun uu____3446 -> (match (uu____3446) with
| (m, uu____3458, uu____3459) -> begin
...
let uu____3591 = (FStar_Options.debug_any ())
in (match (uu____3591) with
| true -> begin
(

let uu____3592 = (FStar_Util.format1 "Z3 says: %s\n" (FStar_String.concat "\n" smt_output.smt_result))
in (FStar_All.pipe_left FStar_Util.print_string uu____3592))
end
| uu____3593 -> begin
()
end));
...
let uu____3602 = (FStar_Util.format1 "Unexpected output from Z3: got output result: %s\n" (FStar_String.concat "\n" smt_output.smt_result))
in (failwith uu____3602))
end);
)
...
let uu____3617 = (FStar_Options.z3_exe ())
in (

let uu____3618 = (ini_params ())
in (FStar_Util.launch_process false uu____3616 uu____3617 uu____3618 input cond))))
end
| uu____3619 -> begin
(
...
let z3_options : unit  ->  Prims.string = (fun uu____3627 -> "(set-option :global-decls false)\n(set-option :smt.mbqi false)\n(set-option :auto_config false)\n(set-option :produce-unsat-cores true)\n")
...
let with_monitor : 'Auu____3814 'Auu____3815 . 'Auu____3814  ->  (unit  ->  'Auu____3815)  ->  'Auu____3815 = (fun m f -> ((FStar_Util.monitor_enter m);
(
...
let z3_job : Prims.bool  ->  FStar_SMTEncoding_Term.error_labels  ->  Prims.string  ->  Prims.string FStar_Pervasives_Native.option  ->  unit  ->  z3result = (fun fresh label_messages input qhash uu____3860 -> (

let start = (FStar_Util.now ())
in (

let uu____3864 = (FStar_All.try_with (fun uu___46_3874 -> (match (()) with
| () -> begin
(doZ3Exe fresh input label_messages)
end)) (fun uu___45_3883 -> (match (uu___45_3883) with
| uu____3888 when (

let uu____3889 = (FStar_Options.trace_error ())
in (not (uu____3889))) -> begin
((bg_z3_proc.refresh ());
(

let uu____3891 = (FStar_Util.smap_create (Prims.parse_int "0"))
in ((UNKNOWN ((([]), (FStar_Pervasives_Native.Some ("Z3 raised an exception"))))), (uu____3891)));
)
end)))
...
let uu____3901 = (FStar_Util.now ())
in (FStar_Util.time_diff start uu____3901))
in (match (uu____3896) with
| (uu____3902, elapsed_time) -> begin
...
let rec dequeue' : unit  ->  unit = (fun uu____3929 -> (

let j = (

let uu____3931 = (FStar_ST.op_Bang job_queue)
...
and dequeue : unit  ->  unit = (fun uu____4008 -> (

let uu____4009 = (FStar_ST.op_Bang running)
in if uu____4009 then begin
(

let rec aux = (fun uu____4038 -> ((FStar_Util.monitor_enter job_queue);
(

let uu____4044 = (FStar_ST.op_Bang job_queue)
...
let uu____4084 = (j.job ())
in (FStar_All.pipe_left j.callback uu____4084)))


let init : unit  ->  unit = (fun uu____4089 -> ((FStar_ST.op_Colon_Equals running true);
(
...
let uu____4138 = (FStar_ST.op_Bang job_queue)
in (FStar_List.append uu____4138 ((j)::[])))
in (FStar_ST.op_Colon_Equals job_queue uu____4135));
(FStar_Util.monitor_pulse job_queue);
(FStar_Util.monitor_exit job_queue);
))


let finish : unit  ->  unit = (fun uu____4208 -> (

let rec aux = (fun uu____4214 -> (

let uu____4215 = (with_monitor job_queue (fun uu____4231 -> (

let uu____4232 = (FStar_ST.op_Bang pending_jobs)
in (

let uu____4256 = (

let uu____4257 = (FStar_ST.op_Bang job_queue)
in (FStar_List.length uu____4257))
in ((uu____4232), (uu____4256))))))
...
let mk_fresh_scope : unit  ->  scope_t = (fun uu____4347 -> (FStar_ST.op_Bang fresh_scope))
...
let uu____4395 = (FStar_ST.op_Bang fresh_scope)
in ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Push)::[])::uu____4395)
in (FStar_ST.op_Colon_Equals fresh_scope uu____4394));
(

let uu____4448 = (

let uu____4451 = (FStar_ST.op_Bang bg_scope)
in (FStar_List.append uu____4451 ((FStar_SMTEncoding_Term.Push)::(FStar_SMTEncoding_Term.Caption (msg))::[])))
in (FStar_ST.op_Colon_Equals bg_scope uu____4448));
))
...
let uu____4515 = (FStar_ST.op_Bang fresh_scope)
in (FStar_List.tl uu____4515))
in (FStar_ST.op_Colon_Equals fresh_scope uu____4514));
(

let uu____4568 = (

let uu____4571 = (FStar_ST.op_Bang bg_scope)
in (FStar_List.append uu____4571 ((FStar_SMTEncoding_Term.Caption (msg))::(FStar_SMTEncoding_Term.Pop)::[])))
in (FStar_ST.op_Colon_Equals bg_scope uu____4568));
))


let giveZ3 : FStar_SMTEncoding_Term.decl Prims.list  ->  unit = (fun decls -> ((FStar_All.pipe_right decls (FStar_List.iter (fun uu___40_4641 -> (match (uu___40_4641) with
...
let uu____4644 = (FStar_ST.op_Bang fresh_scope)
...
let uu____4707 = (FStar_ST.op_Bang bg_scope)
in (FStar_List.append uu____4707 decls))
in (FStar_ST.op_Colon_Equals bg_scope uu____4704));
))


let refresh : unit  ->  unit = (fun uu____4768 -> ((

let uu____4770 = (

let uu____4771 = (FStar_Options.n_cores ())
in (uu____4771 < (Prims.parse_int "2")))
...
let uu____4781 = (FStar_ST.op_Bang fresh_scope)
in (FStar_List.rev uu____4781))
in (FStar_List.flatten uu____4776))
in (FStar_ST.op_Colon_Equals bg_scope uu____4773));
))
...
let uu____4861 = ((FStar_Options.record_hints ()) || ((FStar_Options.use_hints ()) && (FStar_Options.use_hint_hashes ())))
...
let uu____4879 = (FStar_All.pipe_right theory (FStar_Util.prefix_until (fun uu___41_4907 -> (match (uu___41_4907) with
...
in (FStar_All.pipe_right uu____4879 FStar_Option.get))
...
let uncaption = (fun uu___42_4992 -> (match (uu___42_4992) with
| FStar_SMTEncoding_Term.Caption (uu____4993) -> begin
...
let uu___47_4997 = a
in {FStar_SMTEncoding_Term.assumption_term = uu___47_4997.FStar_SMTEncoding_Term.assumption_term; FStar_SMTEncoding_Term.assumption_caption = FStar_Pervasives_Native.None; FStar_SMTEncoding_Term.assumption_name = uu___47_4997.FStar_SMTEncoding_Term.assumption_name; FStar_SMTEncoding_Term.assumption_fact_ids = uu___47_4997.FStar_SMTEncoding_Term.assumption_fact_ids}))
end
| FStar_SMTEncoding_Term.DeclFun (n1, a, s, uu____5001) -> begin
FStar_SMTEncoding_Term.DeclFun (((n1), (a), (s), (FStar_Pervasives_Native.None)))
end
| FStar_SMTEncoding_Term.DefineFun (n1, a, s, b, uu____5012) -> begin
...
let uu____5027 = (FStar_All.pipe_right prefix1 (FStar_List.map uncaption))
in (FStar_All.pipe_right uu____5027 pp_no_cap))
in (FStar_All.pipe_right uu____5024 (FStar_List.filter (fun s -> (Prims.op_disEquality s "")))))
in (FStar_All.pipe_right uu____5021 (FStar_String.concat "\n")))
in (

let uu____5046 = (

let uu____5049 = (FStar_Util.digest_of_string hs)
in FStar_Pervasives_Native.Some (uu____5049))
...
let uu____5054 = (FStar_List.map (FStar_SMTEncoding_Term.declToSmt options) theory)
in (FStar_All.pipe_right uu____5054 (FStar_String.concat "\n")))
in ((uu____5053), (FStar_Pervasives_Native.None)))
end))
in (match (uu____4854) with
| (r, hash) -> begin
((

let uu____5074 = (FStar_Options.log_queries ())
...
let uu____5108 = ((FStar_Options.use_hints ()) && (FStar_Options.use_hint_hashes ()))
...
let uu____5172 = (FStar_ST.op_Bang bg_scope)
in (FStar_List.append uu____5172 (FStar_List.append ((FStar_SMTEncoding_Term.Push)::[]) (FStar_List.append qry ((FStar_SMTEncoding_Term.Pop)::[])))))
in (

let uu____5202 = (filter_theory theory)
in (match (uu____5202) with
| (theory1, used_unsat_core) -> begin
(

let uu____5209 = (mk_input theory1)
...
let uu____5251 = (cache_hit cache qhash cb)
in (not (uu____5251)))
...
let uu____5315 = (match (scope) with
| FStar_Pervasives_Native.Some (s) -> begin
(FStar_List.rev s)
end
| FStar_Pervasives_Native.None -> begin
...
let uu____5355 = (FStar_ST.op_Bang fresh_scope)
in (FStar_List.rev uu____5355));
)
end)
in (FStar_List.flatten uu____5315))
in (
...
let uu____5388 = (filter_theory theory1)
in (match (uu____5388) with
| (theory2, used_unsat_core) -> begin
(

let uu____5395 = (mk_input theory2)
...
let uu____5409 = (cache_hit cache qhash cb)
in (not (uu____5409)))
...
let uu____5473 = (FStar_Options.n_cores ())
in (Prims.op_Equality uu____5473 (Prims.parse_int "1")))
