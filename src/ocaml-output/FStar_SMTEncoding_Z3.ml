      ...
      fun input  ->
        fun qhash  ->
          fun uu____4135  ->
            ...
            let uu____4139 = doZ3Exe fresh1 input label_messages in
            match uu____4139 with
            | (status,statistics) ->
                let uu____4146 =
                  let uu____4151 = FStar_Util.now () in
                  FStar_Util.time_diff start uu____4151 in
                (match uu____4146 with
                 | (uu____4152,elapsed_time) ->
                     {
...
let rec dequeue': Prims.unit -> Prims.unit =
  fun uu____4169  ->
    let j =
      let uu____4171 = FStar_ST.op_Bang job_queue in
    ...
    run_job j;
    with_monitor job_queue (fun uu____4292  -> FStar_Util.decr pending_jobs);
    dequeue ()
and dequeue: Prims.unit -> Prims.unit =
  fun uu____4294  ->
    let uu____4295 = FStar_ST.op_Bang running in
    if uu____4295
    then
      let rec aux uu____4345 =
        FStar_Util.monitor_enter job_queue;
        (let uu____4351 = FStar_ST.op_Bang job_queue in
         match uu____4351 with
         | [] ->
             (FStar_Util.monitor_exit job_queue;
              FStar_Util.sleep (Prims.parse_int "50");
              aux ())
         | uu____4410 -> dequeue' ()) in
      aux ()
    else ()
and run_job: z3job -> Prims.unit =
  fun j  ->
    let uu____4414 = j.job () in FStar_All.pipe_left j.callback uu____4414
...
let enqueue: z3job -> Prims.unit =
  fun j  ->
    FStar_Util.monitor_enter job_queue;
    (let uu____4484 =
       let uu____4487 = FStar_ST.op_Bang job_queue in
       FStar_List.append uu____4487 [j] in
     FStar_ST.op_Colon_Equals job_queue uu____4484);
    FStar_Util.monitor_pulse job_queue;
    FStar_Util.monitor_exit job_queue
let finish: Prims.unit -> Prims.unit =
  fun uu____4602  ->
    let rec aux uu____4606 =
      let uu____4607 =
        with_monitor job_queue
          (fun uu____4623  ->
             let uu____4624 = FStar_ST.op_Bang pending_jobs in
             let uu____4671 =
               let uu____4672 = FStar_ST.op_Bang job_queue in
               FStar_List.length uu____4672 in
             (uu____4624, uu____4671)) in
...
let mk_fresh_scope: Prims.unit -> scope_t =
  fun uu____4811  -> FStar_ST.op_Bang fresh_scope
let bg_scope: FStar_SMTEncoding_Term.decl Prims.list FStar_ST.ref =
  FStar_Util.mk_ref []
let push: Prims.string -> Prims.unit =
  fun msg  ->
    (let uu____4884 =
       let uu____4889 = FStar_ST.op_Bang fresh_scope in
       [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Push] ::
         uu____4889 in
     FStar_ST.op_Colon_Equals fresh_scope uu____4884);
    (let uu____5004 =
       let uu____5007 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5007
         [FStar_SMTEncoding_Term.Push; FStar_SMTEncoding_Term.Caption msg] in
     FStar_ST.op_Colon_Equals bg_scope uu____5004)
let pop: Prims.string -> Prims.unit =
  fun msg  ->
    (let uu____5115 =
       let uu____5120 = FStar_ST.op_Bang fresh_scope in
       FStar_List.tl uu____5120 in
     FStar_ST.op_Colon_Equals fresh_scope uu____5115);
    (let uu____5235 =
       let uu____5238 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5238
         [FStar_SMTEncoding_Term.Caption msg; FStar_SMTEncoding_Term.Pop] in
     FStar_ST.op_Colon_Equals bg_scope uu____5235)
let giveZ3: FStar_SMTEncoding_Term.decl Prims.list -> Prims.unit =
  fun decls  ->
    FStar_All.pipe_right decls
      (FStar_List.iter
         (fun uu___98_5353  ->
            match uu___98_5353 with
            | FStar_SMTEncoding_Term.Push  -> failwith "Unexpected push/pop"
            | FStar_SMTEncoding_Term.Pop  -> failwith "Unexpected push/pop"
            | uu____5354 -> ()));
    (let uu____5356 = FStar_ST.op_Bang fresh_scope in
     match uu____5356 with
     | hd1::tl1 ->
         FStar_ST.op_Colon_Equals fresh_scope ((FStar_List.append hd1 decls)
           :: tl1)
     | uu____5481 -> failwith "Impossible");
    (let uu____5486 =
       let uu____5489 = FStar_ST.op_Bang bg_scope in
       FStar_List.append uu____5489 decls in
     FStar_ST.op_Colon_Equals bg_scope uu____5486)
let refresh: Prims.unit -> Prims.unit =
  fun uu____5595  ->
    (let uu____5597 =
       let uu____5598 = FStar_Options.n_cores () in
       uu____5598 < (Prims.parse_int "2") in
     if uu____5597 then bg_z3_proc.refresh () else ());
    (let uu____5600 =
       let uu____5603 =
         let uu____5608 = FStar_ST.op_Bang fresh_scope in
         FStar_List.rev uu____5608 in
       FStar_List.flatten uu____5603 in
     FStar_ST.op_Colon_Equals bg_scope uu____5600)
let mk_input:
  ...
  fun theory  ->
    let uu____5733 =
      ...
      then
        ...
        let split_decl2smt uu____5777 x =
          match uu____5777 with
          | (pre,suf) ->
              let xstr = FStar_SMTEncoding_Term.declToSmt options x in
              (match (pre, suf) with
               | (uu____5842,FStar_Pervasives_Native.None ) when
                   x = FStar_SMTEncoding_Term.CheckSat ->
                   (pre, (FStar_Pervasives_Native.Some [xstr]))
               | (uu____5867,FStar_Pervasives_Native.Some s) ->
                   (pre, (FStar_Pervasives_Native.Some (xstr :: s)))
               | (FStar_Pervasives_Native.None ,FStar_Pervasives_Native.None
                  ) ->
                   ((FStar_Pervasives_Native.Some [xstr]),
                     FStar_Pervasives_Native.None)
               | (FStar_Pervasives_Native.Some p,FStar_Pervasives_Native.None
                  ) ->
                   ((FStar_Pervasives_Native.Some (xstr :: p)),
                     FStar_Pervasives_Native.None)
               | uu____5946 -> failwith "unreachable") in
        let uu____5971 =
          FStar_List.fold_left split_decl2smt
            (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None)
            theory in
        ...
        | (pl,sl) ->
            let ps =
              let uu____6033 =
                let uu____6036 = FStar_Option.get pl in
                FStar_List.rev uu____6036 in
              FStar_String.concat "\n" uu____6033 in
            let ss =
              let uu____6042 =
                let uu____6045 = FStar_Option.get sl in
                FStar_List.rev uu____6045 in
              FStar_String.concat "\n" uu____6042 in
            let uu____6050 =
              let uu____6053 = FStar_Util.digest_of_string ps in
              FStar_Pervasives_Native.Some uu____6053 in
        ...
        (let uu____6057 =
           ...
           FStar_All.pipe_right uu____6058 (FStar_String.concat "\n") in
         (uu____6057, FStar_Pervasives_Native.None)) in
    match uu____5733 with
    | (r,hash) ->
        ((let uu____6078 = FStar_Options.log_queries () in
          if uu____6078 then query_logging.write_to_log r else ());
         (r, hash))
    ...
    fun cache  ->
      fun label_messages  ->
        fun qry  ->
          fun cb  ->
            let theory =
              let uu____6181 = FStar_ST.op_Bang bg_scope in
              FStar_List.append uu____6181
                (FStar_List.append [FStar_SMTEncoding_Term.Push]
                   (FStar_List.append qry [FStar_SMTEncoding_Term.Pop])) in
            let uu____6234 = filter_theory theory in
            match uu____6234 with
            | (theory1,used_unsat_core) ->
                let uu____6241 = mk_input theory1 in
                (match uu____6241 with
                 | (input,qhash) ->
                     (FStar_ST.op_Colon_Equals bg_scope [];
                      (let uu____6305 =
                         let uu____6306 = cache_hit cache qhash cb in
            ...
            fun cb  ->
              let theory =
                let uu____6370 =
                  ...
                  | FStar_Pervasives_Native.None  ->
                      (FStar_ST.op_Colon_Equals bg_scope [];
                       (let uu____6433 = FStar_ST.op_Bang fresh_scope in
                        FStar_List.rev uu____6433)) in
                FStar_List.flatten uu____6370 in
              let theory1 =
                FStar_List.append theory
                  (FStar_List.append [FStar_SMTEncoding_Term.Push]
                     (FStar_List.append qry [FStar_SMTEncoding_Term.Pop])) in
              let uu____6497 = filter_theory theory1 in
              match uu____6497 with
              | (theory2,used_unsat_core) ->
                  let uu____6504 = mk_input theory2 in
                  (match uu____6504 with
                   | (input,qhash) ->
                       let uu____6517 =
                         let uu____6518 = cache_hit cache qhash cb in
                         Prims.op_Negation uu____6518 in
                       if uu____6517
                       then
                         enqueue
                           {
      ...
      fun label_messages  ->
        fun qry  ->
          fun scope  ->
            fun cb  ->
              let uu____6581 =
                let uu____6582 = FStar_Options.n_cores () in
                uu____6582 = (Prims.parse_int "1") in
              if uu____6581
