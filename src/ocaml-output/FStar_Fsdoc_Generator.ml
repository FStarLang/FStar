...
| FStar_Parser_AST.TopLevelLet (uu____587, pats) -> begin
(

let termty = (FStar_List.map (fun uu____621 -> (match (uu____621) with
| (p, t) -> begin
(

let uu____632 = (FStar_Parser_AST.pat_to_string p)
in (

let uu____633 = (FStar_Parser_AST.term_to_string t)
in ((uu____632), (uu____633))))
end)) pats)
in (

let termty' = (FStar_List.map (fun uu____644 -> (match (uu____644) with
| (p, t) -> begin
(Prims.strcat p (Prims.strcat ":" t))
end)) termty)
in (Prims.strcat "let " (FStar_String.concat ", " termty'))))
end
| FStar_Parser_AST.Main (uu____651) -> begin
"main ..."
end
...
let uu____656 = (FStar_Parser_AST.term_to_string t)
in (Prims.strcat ":" uu____656))
in (Prims.strcat i.FStar_Ident.idText uu____655))
in (Prims.strcat "assume " uu____654))
end
| FStar_Parser_AST.Tycon (uu____657, tys) -> begin
(

let uu____675 = (

let uu____676 = (FStar_All.pipe_right tys (FStar_List.map (fun uu____720 -> (match (uu____720) with
| (t, d1) -> begin
(

let uu____763 = (string_of_tycon t)
in (

let uu____764 = (

let uu____765 = (string_of_fsdoco d1)
in (Prims.strcat " " uu____765))
in (Prims.strcat uu____763 uu____764)))
end))))
in (FStar_All.pipe_right uu____676 (FStar_String.concat " and ")))
in (Prims.strcat "type " uu____675))
end
...
let uu____772 = (FStar_Parser_AST.term_to_string t)
in (Prims.strcat ":" uu____772))
in (Prims.strcat i.FStar_Ident.idText uu____771))
in (Prims.strcat "val " uu____770))
end
| FStar_Parser_AST.Exception (i, uu____774) -> begin
(Prims.strcat "exception " i.FStar_Ident.idText)
end
| FStar_Parser_AST.NewEffect (FStar_Parser_AST.DefineEffect (i, uu____780, uu____781, uu____782)) -> begin
(Prims.strcat "new_effect " i.FStar_Ident.idText)
end
| FStar_Parser_AST.NewEffect (FStar_Parser_AST.RedefineEffect (i, uu____792, uu____793)) -> begin
(Prims.strcat "new_effect " i.FStar_Ident.idText)
end
| FStar_Parser_AST.SubEffect (uu____798) -> begin
"sub_effect"
end
| FStar_Parser_AST.Pragma (uu____799) -> begin
"pragma"
end
| FStar_Parser_AST.Splice (ids, t) -> begin
(

let uu____806 = (FStar_Parser_AST.term_to_string t)
in (Prims.strcat "splice " uu____806))
end
| FStar_Parser_AST.Fsdoc (comm, uu____808) -> begin
comm
end))
...
| FStar_Parser_AST.TyconAbstract (uu____854) -> begin
false
end
| FStar_Parser_AST.TyconAbbrev (uu____865) -> begin
false
end
| FStar_Parser_AST.TyconRecord (uu____878, uu____879, uu____880, fields) -> begin
(FStar_List.existsb (fun uu____922 -> (match (uu____922) with
| (_id, _t, doco) -> begin
(FStar_Util.is_some doco)
end)) fields)
end
| FStar_Parser_AST.TyconVariant (uu____938, uu____939, uu____940, vars) -> begin
(FStar_List.existsb (fun uu____995 -> (match (uu____995) with
| (_id, _t, doco, _u) -> begin
(FStar_Util.is_some doco)
end)) vars)
end))
in (FStar_List.existsb (fun uu____1029 -> (match (uu____1029) with
| (tycon, doco) -> begin
((tyconvars_documented tycon) || (FStar_Util.is_some doco))
end)) tt)))
in (match (d.FStar_Parser_AST.doc) with
| FStar_Pervasives_Native.Some (uu____1042) -> begin
true
end
| uu____1043 -> begin
(match (d.FStar_Parser_AST.d) with
| FStar_Parser_AST.Fsdoc (uu____1046) -> begin
true
end
| FStar_Parser_AST.Tycon (uu____1047, ty) -> begin
(tycon_documented ty)
end
| uu____1065 -> begin
false
...
| {FStar_Parser_AST.d = decl; FStar_Parser_AST.drange = uu____1083; FStar_Parser_AST.doc = fsdoc; FStar_Parser_AST.quals = uu____1085; FStar_Parser_AST.attrs = uu____1086} -> begin
((

let uu____1090 = (

let uu____1091 = (string_of_decl' d.FStar_Parser_AST.d)
in (code_wrap uu____1091))
in (w uu____1090));
(match (fsdoc) with
...
let document_toplevel : 'Auu____1117 . 'Auu____1117  ->  FStar_Parser_AST.decl  ->  (Prims.string FStar_Pervasives_Native.option * Prims.string FStar_Pervasives_Native.option) = (fun name topdecl -> (match (topdecl.FStar_Parser_AST.d) with
| FStar_Parser_AST.TopLevelModule (uu____1136) -> begin
(match (topdecl.FStar_Parser_AST.doc) with
| FStar_Pervasives_Native.Some (doc1, kw) -> begin
(

let uu____1159 = (FStar_List.tryFind (fun uu____1173 -> (match (uu____1173) with
| (k, v1) -> begin
...
| FStar_Pervasives_Native.Some (uu____1196, summary) -> begin
...
let uu____1224 = (match (m) with
| FStar_Parser_AST.Module (n1, d) -> begin
((n1), (d), ("module"))
end
| FStar_Parser_AST.Interface (n1, d, uu____1251) -> begin
((n1), (d), ("interface"))
end)
...
let uu____1265 = (one_toplevel decls)
...
let uu____1295 = (document_toplevel name top_decl)
...
let uu____1319 = (FStar_Util.format "# module %s" ((name.FStar_Ident.str)::[]))
in (w uu____1319));
(
...
let uu____1337 = (FStar_Util.format1 "No singleton toplevel in module %s" name.FStar_Ident.str)
in ((FStar_Errors.Fatal_NonSingletonTopLevel), (uu____1337)))
in (FStar_Errors.raise_err uu____1332))
end))
end)))
...
let uu____1353 = (FStar_Parser_Driver.parse_file fn)
in (FStar_Pervasives_Native.fst uu____1353))) files)
in (
...
in (FStar_Util.append_to_file fd uu____1379))) mods);
(FStar_Util.close_file fd);
))))))
