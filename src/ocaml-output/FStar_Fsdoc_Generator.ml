...
let document_toplevel : 'Auu____1097 . 'Auu____1097  ->  FStar_Parser_AST.decl  ->  (Prims.string FStar_Pervasives_Native.option * Prims.string FStar_Pervasives_Native.option) = (fun name topdecl -> (match (topdecl.FStar_Parser_AST.d) with
| FStar_Parser_AST.TopLevelModule (uu____1116) -> begin
(match (topdecl.FStar_Parser_AST.doc) with
| FStar_Pervasives_Native.Some (doc1, kw) -> begin
(

let uu____1149 = (FStar_List.tryFind (fun uu____1163 -> (match (uu____1163) with
| (k, v1) -> begin
...
| FStar_Pervasives_Native.Some (uu____1186, summary) -> begin
...
let uu____1214 = (match (m) with
| FStar_Parser_AST.Module (n1, d) -> begin
((n1), (d), ("module"))
end
| FStar_Parser_AST.Interface (n1, d, uu____1241) -> begin
((n1), (d), ("interface"))
end)
...
let uu____1255 = (one_toplevel decls)
...
let uu____1285 = (document_toplevel name top_decl)
...
let uu____1309 = (FStar_Util.format "# module %s" ((name.FStar_Ident.str)::[]))
in (w uu____1309));
(
...
let uu____1327 = (FStar_Util.format1 "No singleton toplevel in module %s" name.FStar_Ident.str)
in ((FStar_Errors.Fatal_NonSingletonTopLevel), (uu____1327)))
in (FStar_Errors.raise_err uu____1322))
end))
end)))
...
let uu____1343 = (FStar_Parser_Driver.parse_file fn)
in (FStar_Pervasives_Native.fst uu____1343))) files)
in (
...
in (FStar_Util.append_to_file fd uu____1369))) mods);
(FStar_Util.close_file fd);
))))))
