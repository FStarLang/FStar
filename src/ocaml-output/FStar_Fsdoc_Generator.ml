      ...
      (match topdecl.FStar_Parser_AST.doc with
       | Some (doc1,kw) ->
           let uu____651 =
             FStar_List.tryFind
               (fun uu____657  ->
                  match uu____657 with | (k,v1) -> k = "summary") kw
              in
           (match uu____651 with
            | None  -> (None, (Some doc1))
            | Some (uu____670,summary) -> ((Some summary), (Some doc1)))
       | None  -> (None, None))
...
let document_module : FStar_Parser_AST.modul -> FStar_Ident.lid =
  fun m  ->
    let uu____686 =
      ...
      | FStar_Parser_AST.Interface (n1,d,uu____702) -> (n1, d, "interface")
       in
