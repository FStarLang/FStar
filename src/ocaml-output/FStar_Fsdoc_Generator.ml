...
let string_of_decl' : FStar_Parser_AST.decl' -> Prims.string =
  fun d  ->
    ...
    | FStar_Parser_AST.SubEffect uu____401 -> "sub_effect"
    | FStar_Parser_AST.Pragma uu____402 -> "pragma"
    | FStar_Parser_AST.Fsdoc (comm,uu____404) -> comm
...
let decl_documented : FStar_Parser_AST.decl -> Prims.bool =
  fun d  ->
    let tycon_documented tt =
      let tyconvars_documented tycon =
        ...
        | FStar_Parser_AST.TyconAbstract _|FStar_Parser_AST.TyconAbbrev _ ->
            false
        | FStar_Parser_AST.TyconRecord (uu____432,uu____433,uu____434,fields)
            ->
            FStar_List.existsb
              (fun uu____454  ->
                 match uu____454 with
                 | (_id,_t,doco) -> FStar_Util.is_some doco) fields
        | FStar_Parser_AST.TyconVariant (uu____464,uu____465,uu____466,vars)
            ->
            FStar_List.existsb
              (fun uu____492  ->
    ...
    | uu____519 ->
        (match d.FStar_Parser_AST.d with
         | FStar_Parser_AST.Fsdoc uu____521 -> true
         | FStar_Parser_AST.Tycon (uu____522,ty) -> tycon_documented ty
         | uu____532 -> false)
  ...
  fun w  ->
    fun d  ->
      ...
      then
        ...
        | { FStar_Parser_AST.d = decl; FStar_Parser_AST.drange = uu____546;
            FStar_Parser_AST.doc = fsdoc; FStar_Parser_AST.quals = uu____548;
            FStar_Parser_AST.attrs = uu____549;_} ->
...
let document_toplevel name topdecl =
  ...
  | FStar_Parser_AST.TopLevelModule uu____585 ->
      (match topdecl.FStar_Parser_AST.doc with
       | Some (doc,kw) ->
           let uu____603 =
             FStar_List.tryFind
               (fun uu____609  ->
                  match uu____609 with | (k,v) -> k = "summary") kw
              in
           (match uu____603 with
            | None  -> (None, (Some doc))
            | Some (uu____622,summary) -> ((Some summary), (Some doc)))
       | None  -> (None, None))
  | uu____630 -> Prims.raise (FStar_Errors.Err "Not a TopLevelModule") 
let document_module : FStar_Parser_AST.modul -> FStar_Ident.lid =
  fun m  ->
    let uu____638 =
      match m with
      | FStar_Parser_AST.Module (n,d) -> (n, d, "module")
      | FStar_Parser_AST.Interface (n,d,uu____654) -> (n, d, "interface")  in
    match uu____638 with
    | (name,decls,_mt) ->
        let uu____663 = one_toplevel decls  in
        (match uu____663 with
         | Some (top_decl,other_decls) ->
             ...
             let uu____682 = document_toplevel name top_decl  in
