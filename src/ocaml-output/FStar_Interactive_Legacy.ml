...
let (uu___is_Pop : input_chunks -> Prims.bool) =
  fun projectee  ->
    match projectee with | Pop _0 -> true | uu____660 -> false
...
let (the_interactive_state : interactive_state) =
  let uu____1028 = FStar_Util.new_string_builder ()  in
  let uu____1029 = FStar_Util.mk_ref FStar_Pervasives_Native.None  in
  let uu____1036 = FStar_Util.mk_ref []  in
  let uu____1043 = FStar_Util.mk_ref FStar_Pervasives_Native.None  in
  {
    chunk = uu____1028;
    stdin = uu____1029;
    buffer = uu____1036;
...
let rec (read_chunk : unit -> input_chunks) =
  fun uu____1055  ->
    ...
    let log1 =
      let uu____1063 = FStar_Options.debug_any ()  in
      if uu____1063
      then
        let transcript =
          let uu____1071 = FStar_ST.op_Bang s.log  in
      ...
      else (fun uu____1129  -> ())  in
    let stdin =
      let uu____1132 = FStar_ST.op_Bang s.stdin  in
      ...
      | FStar_Pervasives_Native.None  ->
       ...
       in
    let line =
      let uu____1186 = FStar_Util.read_line stdin  in
    ...
    (let l = FStar_Util.trim_string line  in
     ...
     then
       let responses =
         ...
         | uu____1216::ok::fail1::[] -> (ok, fail1)
         | uu____1228 -> ("ok", "fail")  in
         ...
         then
            ...
            (let lc_lax =
               let uu____1258 =
                 FStar_Util.substring_from l (FStar_String.length "#push")
                  in
               FStar_Util.trim_string uu____1258  in
             let lc =
               match FStar_Util.split lc_lax " " with
               | l1::c::"#lax"::[] ->
                   let uu____1290 = FStar_Util.int_of_string l1  in
                   let uu____1292 = FStar_Util.int_of_string c  in
                   (true, uu____1290, uu____1292)
               | l1::c::[] ->
                   let uu____1305 = FStar_Util.int_of_string l1  in
                   let uu____1307 = FStar_Util.int_of_string c  in
                   (false, uu____1305, uu____1307)
               | uu____1313 ->
           ...
           then
             (match FStar_Util.split l " " with
              ...
              | uu____1362::symbol::file::row::col::[] ->
                  (FStar_Util.clear_string_builder s.chunk;
                   (let uu____1379 =
                      let uu____1399 =
                        let uu____1411 =
                          let uu____1421 = FStar_Util.int_of_string row  in
                          let uu____1423 = FStar_Util.int_of_string col  in
                          (file, uu____1421, uu____1423)  in
                        FStar_Pervasives_Native.Some uu____1411  in
                      (symbol, false, uu____1399)  in
                    Info uu____1379))
              | uu____1451 ->
                  (FStar_Errors.log_issue FStar_Range.dummyRange
...
let (shift_chunk : unit -> input_chunks) =
  fun uu____1498  ->
    let s = the_interactive_state  in
    let uu____1500 = FStar_ST.op_Bang s.buffer  in
...
let (fill_buffer : unit -> unit) =
  fun uu____1559  ->
    let s = the_interactive_state  in
    let uu____1561 =
      let uu____1564 = FStar_ST.op_Bang s.buffer  in
      let uu____1590 = let uu____1593 = read_chunk ()  in [uu____1593]  in
      FStar_List.append uu____1564 uu____1590  in
    FStar_ST.op_Colon_Equals s.buffer uu____1561
    ...
    | (deps,dep_graph1) ->
        let uu____1667 =
          FStar_List.partition
            (fun x  ->
               let uu____1684 = FStar_Parser_Dep.lowercase_module_name x  in
               let uu____1686 =
                 FStar_Parser_Dep.lowercase_module_name filename  in
               uu____1684 <> uu____1686) deps
           in
        (match uu____1667 with
         | (deps1,same_name) ->
             let maybe_intf =
               ...
               | intf::impl::[] ->
                   ((let uu____1730 =
                       (let uu____1734 = FStar_Parser_Dep.is_interface intf
                           in
                   ...
                   ((let uu____1763 =
                       let uu____1769 =
                         FStar_Util.format1 "Unexpected: ended up with %s"
                          ...
                          in
                       (FStar_Errors.Warning_UnexpectedFile, uu____1769)  in
                     FStar_Errors.log_issue FStar_Range.dummyRange uu____1763);
                    FStar_Pervasives_Native.None)
                in
        ...
        fun remaining  ->
          fun ts  ->
            ...
            | uu____1842 ->
                ...
                let env1 =
                  let uu____1858 = FStar_Options.lax ()  in
                  push_with_kind env uu____1858 true "typecheck_modul"  in
                let uu____1862 = tc_one_file remaining env1  in
            ...
            let rec iterate depnames st env' ts1 good_stack good_ts =
              ...
              let rec pop_tc_and_stack env1 stack ts2 =
                ...
                | uu____2351::ts3 ->
                    (pop env1 "";
                     (let uu____2399 =
                        let uu____2414 = FStar_List.hd stack  in
                        let uu____2423 = FStar_List.tl stack  in
                        (uu____2414, uu____2423)  in
                      match uu____2399 with
                      | ((env2,uu____2445),stack1) ->
                          pop_tc_and_stack env2 stack1 ts3))
                 in
              match ts1 with
              | ts_elt::ts' ->
                  let uu____2515 = ts_elt  in
                  (match uu____2515 with
                   | (intf,impl,intf_t,impl_t) ->
                       let uu____2552 = match_dep depnames intf impl  in
                       (match uu____2552 with
                        | (b,depnames') ->
                            ...
                            then
                              ...
                              (let uu____2597 =
                                 let uu____2606 = FStar_List.hd st  in
                                 let uu____2615 = FStar_List.tl st  in
                                 (uu____2606, uu____2615)  in
                               match uu____2597 with
                               | (stack_elt,st') ->
                                   iterate depnames' st' env' ts' (stack_elt
                                     :: good_stack) (ts_elt :: good_ts))))
              | [] -> tc_deps m good_stack env' depnames good_ts  in
            let uu____2672 = deps_of_our_file filename  in
            match uu____2672 with
            | (filenames,uu____2692,dep_graph1) ->
  ...
  fun env  ->
    fun name  ->
      fun typ  ->
        fun range  ->
          fun doc  ->
            let uu____2795 = FStar_Range.string_of_range range  in
            let uu____2797 =
            ...
            FStar_Util.format4 "(defined at %s) %s: %s%s" uu____2795 name
            ...
            fun ts  ->
                          ...
                          (let lid =
                             let uu____3040 =
                               FStar_List.map FStar_Ident.id_of_text
                                ...
                                in
                             FStar_Ident.lid_of_ids uu____3040  in
                           let lid1 =
                             if fqn_only
                             then lid
                             else
                  ...
                  ((match info_opt with
                    ...
                    | FStar_Pervasives_Native.Some (name_or_lid,typ,rng) ->
                        let uu____3180 =
                          ...
                          | FStar_Util.Inr lid ->
                              let uu____3207 = FStar_Ident.string_of_lid lid
                                 in
              ...
              | Completions search_term ->
                  let rec measure_anchored_match search_term1 candidate =
                    ...
                    | (uu____3293,[]) -> FStar_Pervasives_Native.None
                    | (hs::ts1,hc::tc1) ->
                  ...
                  let rec locate_match needle candidate =
                    let uu____3476 = measure_anchored_match needle candidate
                       in
                    ...
                    | FStar_Pervasives_Native.None  ->
                        (match candidate with
                         | [] -> FStar_Pervasives_Native.None
                         | hc::tc1 ->
                             let uu____3565 = locate_match needle tc1  in
                    ...
                    | (prefix1,matched,match_len) ->
                        ...
                        (match uu____3793 with
                         | (stripped_ns,shortened) ->
                             let uu____3824 = str_of_ids shortened  in
                             let uu____3826 = str_of_ids matched  in
                             let uu____3828 = str_of_ids stripped_ns  in
                             (uu____3824, uu____3826, uu____3828, match_len))
                     in
                  let prepare_candidate uu____3860 =
                  ...
                  let matches =
                      ...
                      FStar_All.pipe_right exported_names
                        (FStar_List.filter_map
                           (fun n1  ->
                              ...
                              then
                                let lid =
                                  let uu____4049 = FStar_Ident.ids_of_lid m
                                     in
                                  let uu____4052 = FStar_Ident.id_of_text n1
                                     in
                                  FStar_Ident.lid_of_ns_and_id uu____4049
                                    uu____4052
                                   in
                                let uu____4053 =
                                  FStar_Syntax_DsEnv.resolve_to_fully_qualified_name
                                     ...
                                     let uu____4070 =
                                       let uu____4073 =
                                         FStar_List.map
                                          ...
                                          in
                                       FStar_List.append uu____4073
                                         [fqn.FStar_Ident.ident]
                                        in
                                     ([], uu____4070, matched_length))
                                  uu____4053
                              else FStar_Pervasives_Native.None))
                       in
                    let case_b_find_matches_in_env uu____4113 =
                      let matches =
                                   ...
                                   | FStar_Pervasives_Native.Some l ->
                                       let uu____4227 =
                                         FStar_Ident.lid_of_ids
                                          ...
                                          in
                                       FStar_Ident.lid_equals l uu____4227)))
                       in
                    let uu____4228 = FStar_Util.prefix needle  in
                    ...
                    | (ns,id1) ->
                        ...
                        FStar_All.pipe_right matched_ids
                          (FStar_List.map
                             (fun x  ->
                                let uu____4368 = shorten_namespace x  in
                                prepare_candidate uu____4368))
                     in
                  ((let uu____4382 =
                      FStar_Util.sort_with
                        (fun uu____4411  ->
                           fun uu____4412  ->
                             match (uu____4411, uu____4412) with
                             | ((cd1,ns1,uu____4452),(cd2,ns2,uu____4455)) ->
                                 (match FStar_String.compare cd1 cd2 with
                                  | uu____4487 when
                                      uu____4487 = Prims.int_zero ->
              ...
              | Code (text,(ok,fail1)) ->
                  ...
                  (match res with
                   ...
                   | uu____4766 -> fail2 curmod env)
  
let (interactive_mode : Prims.string -> unit) =
  fun filename  ->
    (let uu____4787 =
       let uu____4789 = FStar_Options.codegen ()  in
       FStar_Option.isSome uu____4789  in
     if uu____4787
     then
       FStar_Errors.log_issue FStar_Range.dummyRange
    ...
    (let uu____4797 = deps_of_our_file filename  in
         ...
         (match uu____4826 with
          | (stack,env1,ts) ->
              let initial_range =
                ...
                let uu____4858 =
                  FStar_Range.mk_pos Prims.int_one Prims.int_zero  in
                FStar_Range.mk_range filename uu____4855 uu____4858  in
              ...
              then
                let uu____4870 =
                  let uu____4872 = FStar_Options.file_list ()  in
                  FStar_List.hd uu____4872  in
                FStar_SMTEncoding_Solver.with_hints_db uu____4870
                  (fun uu____4878  ->
