...
let (pack_fv : Prims.string Prims.list -> FStar_Syntax_Syntax.fv) =
  fun ns  ->
    ...
    let fallback uu___ =
      let quals =
        let uu___1 = FStar_Ident.lid_equals lid FStar_Parser_Const.cons_lid
           in
        if uu___1
        then FStar_Pervasives_Native.Some FStar_Syntax_Syntax.Data_ctor
        else
          (let uu___3 = FStar_Ident.lid_equals lid FStar_Parser_Const.nil_lid
              in
           if uu___3
           then FStar_Pervasives_Native.Some FStar_Syntax_Syntax.Data_ctor
           else
         ...
         | (a,q) ->
             ...
             let uu___1 =
               let uu___2 =
                 ...
                 FStar_Syntax_Util.mk_app hd uu___3  in
               (uu___2, (a, q'))  in
                ...
                let uu___1 =
                  FStar_List.map
                    (fun uu___2  ->
                       match uu___2 with
                       | (p1,b) ->
                           let uu___3 = inspect_pat p1  in (uu___3, b)) ps
                   in
                (fv, uu___1)  in
        ...
        let brs1 =
          FStar_List.map
            (fun uu___  ->
               match uu___ with
               | (pat,uu___1,t5) ->
                   let uu___2 = inspect_pat pat  in (uu___2, t5)) brs
           in
             ...
             (let uu___4 =
              ...
              then
                ...
                (let inspect_arg uu___6 =
                   match uu___6 with
                   | (a,q) -> let uu___7 = inspect_aqual q  in (a, uu___7)
                    in
                 let uu___6 =
                   let uu___7 =
                   ...
                   let uu___8 =
                     FStar_List.map inspect_arg
                      ...
                      in
                   ([], uu___7, (ct.FStar_Syntax_Syntax.result_typ), uu___8)
                    in
          ...
          FStar_List.map
            (fun uu___  ->
               ...
               | (pat,t1) ->
                   ...
                   (uu___1, FStar_Pervasives_Native.None, t1)) brs
           in
                                        ...
                                        let uu___11 =
                                          let uu___12 =
                                            FStar_Syntax_Syntax.bv_to_name
                                              (FStar_Pervasives_Native.fst b2)
                                             in
                                          ((FStar_Pervasives_Native.fst b1),
                                            uu___12)
                                           in
                                        FStar_Syntax_Syntax.NT uu___11)
                                   param_ctor_bs param_bs2
                                  in
                               let cty3 = FStar_Syntax_Subst.subst s' cty2
                                  in
                               let cty4 = FStar_Syntax_Util.remove_inacc cty3
                                  in
                               let uu___11 = FStar_Ident.path_of_lid lid1  in
                               (uu___11, cty4))))
                    | uu___3 ->
                        failwith
                          "impossible: inspect_sigelt: did not find ctor"
                     in
                  let uu___2 =
  ...
  fun sv  ->
    ...
    | FStar_Reflection_Data.Sg_Inductive (nm,us_names,param_bs,ty,ctors) ->
        ...
        let pack_ctor c =
          ...
          match uu___ with
          | (nm1,ty1) ->
              let lid = FStar_Ident.lid_of_path nm1 FStar_Range.dummyRange
                 in
              let ty2 =
              ...
              FStar_All.pipe_left FStar_Syntax_Syntax.mk_sigelt
                (FStar_Syntax_Syntax.Sig_datacon
                   (lid, us_names, ty3, ind_lid, nparam, []))
           in
        ...
        let c_lids =
          FStar_List.map
            (fun se  ->
               ...
               FStar_Util.must uu___) ctor_ses
           in
        let ind_se =
          ...
          FStar_All.pipe_left FStar_Syntax_Syntax.mk_sigelt
            (FStar_Syntax_Syntax.Sig_inductive_typ
               (ind_lid, us_names, param_bs2, ty2, [], c_lids))
           in
        let se =
          FStar_All.pipe_left FStar_Syntax_Syntax.mk_sigelt
            (FStar_Syntax_Syntax.Sig_bundle
               ((ind_se :: ctor_ses), (ind_lid :: c_lids)))
           in
        let uu___ = se  in
        {
          FStar_Syntax_Syntax.sigel = (uu___.FStar_Syntax_Syntax.sigel);
  ...
  fun bvv  ->
    let uu___ =
      FStar_Ident.mk_ident
       ...
       in
    let uu___1 = FStar_BigInt.to_int_fs bvv.FStar_Reflection_Data.bv_index
       in
    {
...
let (env_open_modules :
  ...
  fun e  ->
    let uu___ = FStar_Syntax_DsEnv.open_modules e.FStar_TypeChecker_Env.dsenv
       in
    FStar_List.map
      (fun uu___1  ->
         match uu___1 with
