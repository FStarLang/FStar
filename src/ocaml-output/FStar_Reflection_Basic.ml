...
open Prims
<<<<<<< 55174ed7b60b0541529586eac26b875de7954ba9
let (env_hook :
...
let uu____507 = (FStar_Syntax_Util.arrow_one t3)
...
| uu____613 -> begin
(match (lb.FStar_Syntax_Syntax.lbname) with
| FStar_Util.Inr (uu____615) -> begin
FStar_Reflection_Data.Tv_Unknown
end
| FStar_Util.Inl (bv) -> begin
...
let uu____668 = (inspect_const c)
in FStar_Reflection_Data.Pat_Constant (uu____668))
end
...
let uu____696 = (FStar_List.map (fun uu____709 -> (match (uu____709) with
| (p1, uu____718) -> begin
(inspect_pat p1)
end)) ps)
in ((fv), (uu____696)))
in FStar_Reflection_Data.Pat_Cons (uu____689))
end
...
let brs1 = (FStar_List.map (fun uu___0_769 -> (match (uu___0_769) with
| (pat, uu____791, t5) -> begin
(

let uu____809 = (inspect_pat pat)
in ((uu____809), (t5)))
end)) brs)
...
let uu____824 = (FStar_Syntax_Print.tag_of_term t3)
in (

let uu____826 = (FStar_Syntax_Print.term_to_string t3)
in (FStar_Util.format2 "inspect_ln: outside of expected syntax (%s, %s)\n" uu____824 uu____826)))
in ((FStar_Errors.Warning_CantInspect), (uu____822)))
in (FStar_Errors.log_issue t3.FStar_Syntax_Syntax.pos uu____816));
FStar_Reflection_Data.Tv_Unknown;
)
end)))))
...
| FStar_Syntax_Syntax.Total (t, uu____837) -> begin
FStar_Reflection_Data.C_Total (((t), (FStar_Pervasives_Native.None)))
end
| FStar_Syntax_Syntax.Comp (ct) -> begin
(

let uu____849 = (FStar_Ident.lid_equals ct.FStar_Syntax_Syntax.effect_name FStar_Parser_Const.effect_Lemma_lid)
in (match (uu____849) with
| true -> begin
(match (ct.FStar_Syntax_Syntax.effect_args) with
| ((pre, uu____853))::((post, uu____855))::uu____856 -> begin
FStar_Reflection_Data.C_Lemma (((pre), (post)))
end
| uu____899 -> begin
(failwith "inspect_comp: Lemma does not have enough arguments?")
...
let uu____913 = (FStar_Ident.lid_equals ct.FStar_Syntax_Syntax.effect_name FStar_Parser_Const.effect_Tot_lid)
in (match (uu____913) with
| true -> begin
(

let maybe_dec = (FStar_List.tryFind (fun uu___1_921 -> (match (uu___1_921) with
| FStar_Syntax_Syntax.DECREASES (uu____923) -> begin
true
end
| uu____927 -> begin
false
...
let uu____982 = (FStar_Syntax_Syntax.as_arg pre)
in (

let uu____991 = (

let uu____1002 = (FStar_Syntax_Syntax.as_arg post)
in (uu____1002)::[])
in (uu____982)::uu____991))
in {FStar_Syntax_Syntax.comp_univs = []; FStar_Syntax_Syntax.effect_name = FStar_Parser_Const.effect_Lemma_lid; FStar_Syntax_Syntax.result_typ = FStar_Syntax_Syntax.t_unit; FStar_Syntax_Syntax.effect_args = uu____971; FStar_Syntax_Syntax.flags = []})
...
let uu____1056 = (FStar_BigInt.string_of_big_int i)
in ((uu____1056), (FStar_Pervasives_Native.None)))
in FStar_Const.Const_int (uu____1044))
end
...
let uu____1076 = (FStar_Ident.lid_of_path ns FStar_Range.dummyRange)
in FStar_Const.Const_reflect (uu____1076))
end))
...
let uu____1149 = (pack_const c)
in FStar_Syntax_Syntax.Tm_constant (uu____1149))
in (FStar_Syntax_Syntax.mk uu____1148))
in (uu____1141 FStar_Pervasives_Native.None FStar_Range.dummyRange))
end
...
let uu____1216 = (pack_const c)
in FStar_Syntax_Syntax.Pat_constant (uu____1216))
in (FStar_All.pipe_left wrap uu____1215))
end
...
let uu____1238 = (FStar_List.map (fun p1 -> (

let uu____1256 = (pack_pat p1)
in ((uu____1256), (false)))) ps)
in ((fv), (uu____1238)))
in FStar_Syntax_Syntax.Pat_cons (uu____1224))
in (FStar_All.pipe_left wrap uu____1223))
end
...
let brs1 = (FStar_List.map (fun uu___2_1305 -> (match (uu___2_1305) with
| (pat, t1) -> begin
(

let uu____1322 = (pack_pat pat)
in ((uu____1322), (FStar_Pervasives_Native.None), (t1)))
end)) brs)
...
let uu____1484 = (FStar_Syntax_Subst.compress attr)
in uu____1484.FStar_Syntax_Syntax.n)
...
let uu____1495 = (FStar_Syntax_Syntax.lid_of_fv fv)
in (FStar_Ident.text_of_lid uu____1495))
in (FStar_TypeChecker_Env.lookup_attr env uu____1493))
in (FStar_List.concatMap (fun se -> (

let uu____1499 = (FStar_Syntax_Util.lid_of_sigelt se)
...
let uu____1505 = (FStar_Syntax_Syntax.lid_as_fv l (FStar_Syntax_Syntax.Delta_constant_at_level ((Prims.parse_int "999"))) FStar_Pervasives_Native.None)
in (uu____1505)::[])
end))) ses))
end
| uu____1507 -> begin
...
let uu___363_1555 = se
in {FStar_Syntax_Syntax.sigel = uu___363_1555.FStar_Syntax_Syntax.sigel; FStar_Syntax_Syntax.sigrng = uu___363_1555.FStar_Syntax_Syntax.sigrng; FStar_Syntax_Syntax.sigquals = uu___363_1555.FStar_Syntax_Syntax.sigquals; FStar_Syntax_Syntax.sigmeta = uu___363_1555.FStar_Syntax_Syntax.sigmeta; FStar_Syntax_Syntax.sigattrs = attrs}))
...
| FStar_Syntax_Syntax.Sig_let ((r, (lb)::[]), uu____1564) -> begin
(
...
| FStar_Util.Inl (uu____1575) -> begin
(failwith "impossible: global Sig_let has bv")
end)
in (

let uu____1577 = (FStar_Syntax_Subst.univ_var_opening lb.FStar_Syntax_Syntax.lbunivs)
...
| FStar_Syntax_Syntax.Sig_inductive_typ (lid, us, bs, ty, uu____1605, c_lids) -> begin
(

let nm = (FStar_Ident.path_of_lid lid)
in (

let uu____1616 = (FStar_Syntax_Subst.univ_var_opening us)
...
let uu____1654 = (FStar_List.map FStar_Ident.path_of_lid c_lids)
in ((nm), (us1), (bs1), (ty1), (uu____1654)))
in FStar_Reflection_Data.Sg_Inductive (uu____1637))))
end)))
end
| FStar_Syntax_Syntax.Sig_datacon (lid, us, ty, uu____1666, n1, uu____1668) -> begin
(

let uu____1675 = (FStar_Syntax_Subst.univ_var_opening us)
...
let uu____1700 = (FStar_Ident.path_of_lid lid)
in ((uu____1700), (ty1)))
in FStar_Reflection_Data.Sg_Constructor (uu____1695)))
end))
end
| uu____1701 -> begin
...
let uu____1738 = (FStar_Syntax_Syntax.lid_of_fv fv)
in (uu____1738)::[])
in ((((r), ((lb)::[]))), (uu____1735)))
in FStar_Syntax_Syntax.Sig_let (uu____1728))
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_sigelt uu____1727))))))
end
| FStar_Reflection_Data.Sg_Constructor (uu____1744) -> begin
(failwith "packing Sg_Constructor, sorry")
end
| FStar_Reflection_Data.Sg_Inductive (uu____1750) -> begin
(failwith "packing Sg_Inductive, sorry")
end
...
let uu____1775 = (FStar_Ident.string_of_ident bv.FStar_Syntax_Syntax.ppname)
in (

let uu____1777 = (FStar_BigInt.of_int_fs bv.FStar_Syntax_Syntax.index)
in {FStar_Reflection_Data.bv_ppname = uu____1775; FStar_Reflection_Data.bv_index = uu____1777; FStar_Reflection_Data.bv_sort = bv.FStar_Syntax_Syntax.sort})))


let pack_bv : FStar_Reflection_Data.bv_view  ->  FStar_Syntax_Syntax.bv = (fun bvv -> (

let uu____1784 = (FStar_Ident.mk_ident ((bvv.FStar_Reflection_Data.bv_ppname), (FStar_Range.dummyRange)))
in (

let uu____1786 = (FStar_BigInt.to_int_fs bvv.FStar_Reflection_Data.bv_index)
in {FStar_Syntax_Syntax.ppname = uu____1784; FStar_Syntax_Syntax.index = uu____1786; FStar_Syntax_Syntax.sort = bvv.FStar_Reflection_Data.bv_sort})))
...
let uu____1813 = (inspect_aqual aq)
in ((bv), (uu____1813)))
end)))
...
let uu____1852 = (FStar_Syntax_DsEnv.open_modules e.FStar_TypeChecker_Env.dsenv)
in (FStar_List.map (fun uu____1870 -> (match (uu____1870) with
| (l, m) -> begin
(

let uu____1880 = (FStar_Ident.ids_of_lid l)
in (FStar_List.map FStar_Ident.text_of_id uu____1880))
end)) uu____1852)))
...
let uu____1902 = (FStar_Syntax_Util.un_uinst t1)
in (

let uu____1903 = (FStar_Syntax_Util.un_uinst t2)
in (FStar_Syntax_Util.term_eq uu____1902 uu____1903))))
