                ...
                let uu____500 =
                  (FStar_Parser_Dep.is_interface fn2) &&
                    (let uu____503 =
                     ...
                     uu____503 = module_name) in
                Prims.op_Negation uu____500)) in
      let binary_deps1 =
        FStar_List.sortWith
          (fun fn11 ->
             fun fn2 ->
               let uu____519 = FStar_Parser_Dep.lowercase_module_name fn11 in
               let uu____521 = FStar_Parser_Dep.lowercase_module_name fn2 in
               FStar_String.compare uu____519 uu____521) binary_deps in
    ...
    fun fn ->
      fun checked_fn ->
        ...
        | (Valid uu____995, uu____996) ->
            let uu____1008 =
              let uu____1009 = FStar_All.pipe_right checked_fn load_tc_result in
    ...
    let cache_file =
      try
        (fun uu___172_1475 ->
           match () with
           | () ->
               let uu____1479 = FStar_Parser_Dep.cache_file_name file_name in
          ...
          let uu____1722 =
            (FStar_Options.cache_checked_modules ()) &&
              (let uu____1725 = FStar_Options.cache_off () in
               Prims.op_Negation uu____1725) in
          if uu____1722
          then
            let cache_file = FStar_Parser_Dep.cache_file_name fn in
            let digest =
