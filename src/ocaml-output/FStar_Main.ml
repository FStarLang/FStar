...
let (setup_hooks : Prims.unit -> Prims.unit) =
  fun uu____577  ->
...
let (handle_error : Prims.exn -> Prims.unit) =
  fun e  ->
    if FStar_Errors.handleable e then FStar_Errors.err_exn e else ();
    (let uu____679 = FStar_Options.trace_error ()  in
     if uu____679
     then
       let uu____680 = FStar_Util.message_of_exn e  in
       let uu____681 = FStar_Util.trace_of_exn e  in
       FStar_Util.print2_error "Unexpected error\n%s\n%s\n" uu____680
         uu____681
     else
       if Prims.op_Negation (FStar_Errors.handleable e)
       then
         (let uu____683 = FStar_Util.message_of_exn e  in
          FStar_Util.print1_error
...
let main : 'Auu____696 . Prims.unit -> 'Auu____696 =
  fun uu____700  ->
    try
      setup_hooks ();
      (let uu____710 = FStar_Util.record_time go  in
       match uu____710 with
       | (uu____715,time) ->
           ((let uu____718 = FStar_Options.query_stats ()  in
             if uu____718
             then
               let uu____719 = FStar_Util.string_of_int time  in
               let uu____720 =
                 let uu____721 = FStar_Getopt.cmdline ()  in
                 FStar_String.concat " " uu____721  in
