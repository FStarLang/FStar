open Prims
let uu___186 : Prims.unit -> Prims.unit = FStar_Version.dummy 
let process_args :
  Prims.unit -> (FStar_Getopt.parse_cmdline_res * Prims.string Prims.list) =
  fun uu____8  -> FStar_Options.parse_cmd_line () 
...
let finished_message :
  ...
  fun fmods  ->
    fun errs  ->
      ...
      let uu____37 = Prims.op_Negation (FStar_Options.silent ())  in
      if uu____37
      then
                          ...
                          print_to
                            (let _0_832 = FStar_Util.string_of_int time  in
                             FStar_Util.format3
                               ...
                               (FStar_Ident.text_of_lid name) _0_832)
                        else
                          print_to
    ...
    | (umods,env) ->
        ...
        then
          ...
          let ext =
            ...
            | Some "OCaml" -> ".ml"
            | Some "Kremlin" -> ".krml"
            | uu____113 -> failwith "Unrecognized option"  in
          (match opt with
      ...
      (match res with
       ...
       | FStar_Getopt.Success  ->
           ...
           then
             ...
             (let uu____146 = FStar_Options.interactive ()  in
              if uu____146
              then
                ((let uu____148 = FStar_Options.explicit_deps ()  in
                  if uu____148
                  then
                ...
                (let uu____163 = FStar_Options.doc ()  in
                 if uu____163
                 then FStar_Fsdoc_Generator.generate filenames
                 else
                   (let uu____165 = FStar_Options.indent ()  in
                    if uu____165
                    then
                      ...
                      then
                        (let verify_mode =
                           let uu____172 = FStar_Options.verify_all ()  in
                           if uu____172
                           then
                             ((let uu____174 =
                                 let _0_840 = FStar_Options.verify_module ()
                                    in
                                 _0_840 <> []  in
                               if uu____174
                               then
...
let main uu____247 =
  ...
  | e ->
      (if FStar_Errors.handleable e
       ...
       (let uu____256 = FStar_Options.trace_error ()  in
        if uu____256
        then
