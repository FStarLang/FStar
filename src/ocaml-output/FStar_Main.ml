                 ...
                 | ((iface,name),time) ->
                     ...
                     then
                       (if time >= (Prims.parse_int "0")
                        then
                          print_to
                            (let _0_834 = FStar_Util.string_of_int time  in
                             FStar_Util.format3
                               "Verified %s: %s (%s milliseconds)\n" tag
                               (FStar_Ident.text_of_lid name) _0_834)
                        else
                          print_to
         ...
         then
           (if errs = (Prims.parse_int "1")
            then FStar_Util.print_error "1 error was reported (see above)\n"
              ...
              (let _0_835 = FStar_Util.string_of_int errs  in
  ...
  fun uu____85  ->
    ...
    | (umods,env) ->
        ...
        then
          let mllibs =
            let _0_838 =
              let _0_837 = FStar_Extraction_ML_UEnv.mkContext env  in
              FStar_Util.fold_map FStar_Extraction_ML_Modul.extract _0_837
                umods
               in
            FStar_All.pipe_left Prims.snd _0_838  in
          ...
          (match opt with
           ...
           | Some "Kremlin" ->
               ...
               let bin = (FStar_Extraction_Kremlin.current_version, programs)
                  in
               let _0_839 = FStar_Options.prepend_output_dir "out.krml"  in
               FStar_Util.save_value_to_file _0_839 bin
  ...
  | (res,filenames) ->
      (match res with
       ...
       | FStar_Getopt.Success  ->
           let uu____140 =
             let _0_840 = FStar_Options.dep ()  in _0_840 <> None  in
           if uu____140
           then
                      ...
                      then
                        (let verify_mode =
                           ...
                           then
                             ...
                             (let uu____177 =
                                let _0_843 = FStar_Options.verify_module ()
                                   in
                                _0_843 <> []  in
                              if uu____177
      ...
      (if FStar_Errors.handleable e
       ...
       (let uu____254 = FStar_Options.trace_error ()  in
        ...
        then
          ...
          let _0_845 = FStar_Util.trace_of_exn e  in
          FStar_Util.print2_error "Unexpected error\n%s\n%s\n" _0_846 _0_845
        else
          if Prims.op_Negation (FStar_Errors.handleable e)
          then
            (let _0_847 = FStar_Util.message_of_exn e  in
             FStar_Util.print1_error
               "Unexpected error; please file a bug report, ideally with a minimized version of the source program that triggered the error.\n%s\n"
               _0_847)
          else ());
       cleanup ();
       (let _0_848 = FStar_Errors.report_all ()  in
        FStar_All.pipe_right _0_848 Prims.ignore);
       report_errors [];
       FStar_All.exit (Prims.parse_int "1"))
