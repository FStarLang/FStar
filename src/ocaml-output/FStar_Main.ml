...
let uu___278 : Prims.unit = (FStar_Version.dummy ())
...
let module_names_and_times = (FStar_All.pipe_right fmods (FStar_List.map (fun uu____566 -> (match (uu____566) with
| (x, t) -> begin
...
in ((uu____594), (env)))
in (codegen uu____587));
(

let uu____612 = (FStar_Options.gen_native_tactics ())
...
let uu____623 = (FStar_All.pipe_right fmods (FStar_List.map FStar_Pervasives_Native.fst))
in ((uu____623), (env)))
in (gen_native_tactics uu____616 dir))
end
| FStar_Pervasives_Native.None -> begin
()
end));
...
let main : 'Auu____645 . Prims.unit  ->  'Auu____645 = (fun uu____649 -> (FStar_All.try_with (fun uu___280_653 -> (match (()) with
| () -> begin
((go ());
(cleanup ());
(FStar_All.exit (Prims.parse_int "0"));
)
end)) (fun uu___279_662 -> (match (uu___279_662) with
| e -> begin
(
...
let uu____668 = (FStar_Options.trace_error ())
in (match (uu____668) with
| true -> begin
(

let uu____669 = (FStar_Util.message_of_exn e)
...
let uu____671 = (FStar_Util.message_of_exn e)
