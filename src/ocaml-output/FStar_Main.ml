                 ...
                 | ((iface,name),time) ->
                     ...
                     then
                       (if time >= (Prims.parse_int "0")
                        then
                          print_to
                            (let _0_854 = FStar_Util.string_of_int time in
                             FStar_Util.format3
                               "Verified %s: %s (%s milliseconds)\n" tag
                               (FStar_Ident.text_of_lid name) _0_854)
                        else
                          print_to
         ...
         then
           (if errs = (Prims.parse_int "1")
            then FStar_Util.print_error "1 error was reported (see above)\n"
              ...
              (let _0_855 = FStar_Util.string_of_int errs in
  ...
  fun uu____85  ->
    ...
    | (umods,env) ->
        ...
        then
          let mllibs =
            let _0_858 =
              let _0_857 = FStar_Extraction_ML_UEnv.mkContext env in
              FStar_Util.fold_map FStar_Extraction_ML_Modul.extract _0_857
                umods in
            FStar_All.pipe_left Prims.snd _0_858 in
          ...
          (match opt with
           ...
           | Some "Kremlin" ->
               ...
               let _0_859 = FStar_Options.prepend_output_dir "out.krml" in
               FStar_Util.save_value_to_file _0_859 bin
  ...
  | (res,filenames) ->
      (match res with
       ...
       | FStar_Getopt.Success  ->
           let uu____140 =
             let _0_860 = FStar_Options.dep () in _0_860 <> None in
           if uu____140
           then
                 ...
                 (let filename = FStar_List.hd filenames in
                  let filename =
                    FStar_Parser_Dep.try_convert_file_name_to_windows
                      filename in
                  (let uu____157 =
                     let _0_861 = FStar_Options.verify_module () in
                     _0_861 <> [] in
                   if uu____157
                   then
                    ...
                    then
                      ...
                      then
                        (let verify_mode =
                           ...
                           then
                             ((let uu____172 =
                                 let _0_862 = FStar_Options.verify_module () in
                                 _0_862 <> [] in
                               if uu____172
                               then
                                 (FStar_Util.print_error
                             ...
                             (let uu____177 =
                                let _0_863 = FStar_Options.verify_module () in
                                _0_863 <> [] in
                              if uu____177
  ...
  | e ->
      (if FStar_Errors.handleable e
       ...
       (let uu____254 = FStar_Options.trace_error () in
        if uu____254
        then
          let _0_866 = FStar_Util.message_of_exn e in
          let _0_865 = FStar_Util.trace_of_exn e in
          FStar_Util.print2_error "Unexpected error\n%s\n%s\n" _0_866 _0_865
        else
          if Prims.op_Negation (FStar_Errors.handleable e)
          then
            (let _0_867 = FStar_Util.message_of_exn e in
             FStar_Util.print1_error
               "Unexpected error; please file a bug report, ideally with a minimized version of the source program that triggered the error.\n%s\n"
               _0_867)
          else ());
       cleanup ();
       (let _0_868 = FStar_Errors.report_all () in
        FStar_All.pipe_right _0_868 Prims.ignore);
       report_errors [];
       FStar_All.exit (Prims.parse_int "1"))