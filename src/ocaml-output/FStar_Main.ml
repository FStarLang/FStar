...
let (finished_message :
  ((Prims.bool, FStar_Ident.lident) FStar_Pervasives_Native.tuple2,
    Prims.int) FStar_Pervasives_Native.tuple2 Prims.list -> Prims.int -> unit)
  =
  fun fmods ->
    fun errs ->
      let print_to =
      ...
      let uu____62 =
        let uu____63 = FStar_Options.silent () in Prims.op_Negation uu____63 in
      if uu____62
      then
...
let (report_errors :
  ((Prims.bool, FStar_Ident.lident) FStar_Pervasives_Native.tuple2,
    Prims.int) FStar_Pervasives_Native.tuple2 Prims.list -> unit)
  =
  fun fmods ->
...
let (codegen :
  (FStar_Syntax_Syntax.modul Prims.list, FStar_TypeChecker_Env.env,
    FStar_Universal.delta_env) FStar_Pervasives_Native.tuple3 -> unit)
  =
  fun uu____167 ->
...
let (load_native_tactics : unit -> unit) =
  fun uu____276 ->
    let modules_to_load =
      ...
      FStar_All.pipe_right uu____280 (FStar_List.map FStar_Ident.lid_of_str) in
    let ml_module_name m =
      let uu____293 = FStar_Extraction_ML_Util.mlpath_of_lid m in
      FStar_All.pipe_right uu____293 FStar_Extraction_ML_Util.flatten_mlpath in
    let ml_file m =
      let uu____312 = ml_module_name m in Prims.strcat uu____312 ".ml" in
    let cmxs_file m =
      let cmxs =
        let uu____320 = ml_module_name m in Prims.strcat uu____320 ".cmxs" in
      let uu____321 = FStar_Options.find_file cmxs in
      match uu____321 with
      | FStar_Pervasives_Native.Some f -> f
      | FStar_Pervasives_Native.None ->
          let uu____325 =
            let uu____328 = ml_file m in FStar_Options.find_file uu____328 in
               ...
               let uu____329 =
                 let uu____334 =
                   ...
                   FStar_Util.format1
                     ...
                     uu____335 in
                 (FStar_Errors.Fatal_FailToCompileNativeTactic, uu____334) in
               ...
               then
                 ...
                 (let uu____419 =
                    ((FStar_Options.use_extracted_interfaces ()) &&
                       (let uu____421 = FStar_Options.expose_interfaces () in
...
let (handle_error : Prims.exn -> unit) =
  fun e ->
    ...
    (let uu____756 = FStar_Options.trace_error () in
     if uu____756
     then
       let uu____757 = FStar_Util.message_of_exn e in
       let uu____758 = FStar_Util.trace_of_exn e in
       FStar_Util.print2_error "Unexpected error\n%s\n%s\n" uu____757
         uu____758
     else
       if Prims.op_Negation (FStar_Errors.handleable e)
       then
         (let uu____760 = FStar_Util.message_of_exn e in
          FStar_Util.print1_error
    ...
    cleanup ();
    report_errors []
let main : 'Auu____775 . unit -> 'Auu____775 =
  fun uu____780 ->
    try
      setup_hooks ();
      (let uu____790 = FStar_Util.record_time go in
       match uu____790 with
       | (uu____795, time) ->
           ((let uu____798 = FStar_Options.query_stats () in
             if uu____798
             then
               let uu____799 = FStar_Util.string_of_int time in
               let uu____800 =
                 let uu____801 = FStar_Getopt.cmdline () in
                 FStar_String.concat " " uu____801 in
