...
let (report_errors :
  ...
  fun fmods  ->
    (let uu____147 = FStar_Errors.report_all ()  in
     FStar_All.pipe_right uu____147 (fun a243  -> ()));
...
let main : 'Auu____628 . unit -> 'Auu____628 =
  fun uu____633  ->
    try
      ...
      (let uu____761 = FStar_Util.record_time go  in
       match uu____761 with
       | (uu____766,time) ->
           ((let uu____769 = FStar_Options.query_stats ()  in
             if uu____769
             then
               let uu____770 = FStar_Util.string_of_int time  in
               let uu____771 =
                 let uu____772 = FStar_Getopt.cmdline ()  in
                 FStar_String.concat " " uu____772  in
    ...
    | e ->
        ...
        (if FStar_Errors.handleable e then FStar_Errors.err_exn e else ();
         (let uu____789 = FStar_Options.trace_error ()  in
          if uu____789
          then
