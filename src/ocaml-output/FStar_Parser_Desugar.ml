...
| FStar_Parser_AST.Product (_62_649) -> begin
t
end
| _62_652 -> begin
...
| FStar_Parser_AST.PatAscribed (p, _62_666) -> begin
(is_app_pattern p)
end
| FStar_Parser_AST.PatApp ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (_62_672); FStar_Parser_AST.prange = _62_670}, _62_676) -> begin
true
end
| _62_680 -> begin
false
...
let _62_692 = (destruct_app_pattern env is_top_level p)
in (match (_62_692) with
| (name, args, _62_691) -> begin
((name), (args), (Some (t)))
end))
end
| FStar_Parser_AST.PatApp ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (id, _62_697); FStar_Parser_AST.prange = _62_694}, args) when is_top_level -> begin
(let _157_148 = (let _157_147 = (FStar_Parser_DesugarEnv.qualify env id)
in FStar_Util.Inr (_157_147))
in ((_157_148), (args), (None)))
end
| FStar_Parser_AST.PatApp ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (id, _62_708); FStar_Parser_AST.prange = _62_705}, args) -> begin
...
| TBinder (_62_719) -> begin
_62_719
end))


let ___VBinder____0 = (fun projectee -> (match (projectee) with
| VBinder (_62_722) -> begin
_62_722
end))


let ___LetBinder____0 = (fun projectee -> (match (projectee) with
| LetBinder (_62_725) -> begin
_62_725
end))
...
let _62_764 = (FStar_Parser_DesugarEnv.push_local_tbinding env a)
in (match (_62_764) with
| (env, a) -> begin
...
let _62_772 = (FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_62_772) with
| (env, x) -> begin
...
let mk_lb : (FStar_Absyn_Syntax.lbname * FStar_Absyn_Syntax.typ * FStar_Absyn_Syntax.exp)  ->  FStar_Absyn_Syntax.letbinding = (fun _62_808 -> (match (_62_808) with
| (n, t, e) -> begin
...
let _62_827 = (FStar_Parser_DesugarEnv.push_local_vbinding e x)
...
let _62_844 = (FStar_Parser_DesugarEnv.push_local_tbinding e a)
in (match (_62_844) with
| (e, a) -> begin
...
| FStar_Parser_AST.PatOp (_62_855) -> begin
...
let _62_869 = (aux loc env p)
in (match (_62_869) with
| (loc, env, var, p, _62_868) -> begin
(

let _62_886 = (FStar_List.fold_left (fun _62_873 p -> (match (_62_873) with
| (loc, env, ps) -> begin
(

let _62_882 = (aux loc env p)
in (match (_62_882) with
| (loc, env, _62_878, p, _62_881) -> begin
((loc), (env), ((p)::ps))
end))
end)) ((loc), (env), ([])) ps)
in (match (_62_886) with
| (loc, env, ps) -> begin
(
...
let _62_888 = (let _157_307 = (FStar_Absyn_Syntax.pat_vars pat)
...
(match (p.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatTvar (_62_895) -> begin
p
end
...
let _62_901 = p
in {FStar_Parser_AST.pat = FStar_Parser_AST.PatTvar (((x), (imp))); FStar_Parser_AST.prange = _62_901.FStar_Parser_AST.prange})
end
| _62_904 -> begin
...
let _62_911 = (aux loc env p)
in (match (_62_911) with
| (loc, env', binder, p, imp) -> begin
(

let binder = (match (binder) with
| LetBinder (_62_913) -> begin
(FStar_All.failwith "impossible")
end
| TBinder (x, _62_917, aq) -> begin
(let _157_309 = (let _157_308 = (desugar_kind env t)
in ((x), (_157_308), (aq)))
in TBinder (_157_309))
end
| VBinder (x, _62_923, aq) -> begin
(
...
let _62_939 = (resolvea loc env a)
in (match (_62_939) with
| (loc, env, abvd) -> begin
...
let _62_955 = (resolvex loc env x)
in (match (_62_955) with
| (loc, env, xbvd) -> begin
...
| FStar_Parser_AST.PatApp ({FStar_Parser_AST.pat = FStar_Parser_AST.PatName (l); FStar_Parser_AST.prange = _62_961}, args) -> begin
(

let _62_983 = (FStar_List.fold_right (fun arg _62_972 -> (match (_62_972) with
| (loc, env, args) -> begin
(

let _62_979 = (aux loc env arg)
in (match (_62_979) with
| (loc, env, _62_976, arg, imp) -> begin
((loc), (env), ((((arg), (imp)))::args))
end))
end)) args ((loc), (env), ([])))
in (match (_62_983) with
| (loc, env, args) -> begin
(
...
| FStar_Parser_AST.PatApp (_62_987) -> begin
(Prims.raise (FStar_Absyn_Syntax.Error ((("Unexpected pattern"), (p.FStar_Parser_AST.prange)))))
end
| FStar_Parser_AST.PatList (pats) -> begin
(

let _62_1007 = (FStar_List.fold_right (fun pat _62_995 -> (match (_62_995) with
| (loc, env, pats) -> begin
(

let _62_1003 = (aux loc env pat)
in (match (_62_1003) with
| (loc, env, _62_999, pat, _62_1002) -> begin
((loc), (env), ((pat)::pats))
end))
end)) pats ((loc), (env), ([])))
in (match (_62_1007) with
| (loc, env, pats) -> begin
(
...
let _62_1033 = (FStar_List.fold_left (fun _62_1020 p -> (match (_62_1020) with
| (loc, env, pats) -> begin
(

let _62_1029 = (aux loc env p)
in (match (_62_1029) with
| (loc, env, _62_1025, pat, _62_1028) -> begin
((loc), (env), ((((pat), (false)))::pats))
end))
end)) ((loc), (env), ([])) args)
in (match (_62_1033) with
| (loc, env, args) -> begin
(
...
| FStar_Absyn_Syntax.Exp_fvar (v, _62_1039) -> begin
v
end
| _62_1043 -> begin
(FStar_All.failwith "impossible")
...
let _62_1053 = (FStar_List.hd fields)
in (match (_62_1053) with
| (f, _62_1052) -> begin
(

let _62_1057 = (FStar_Parser_DesugarEnv.fail_or env (FStar_Parser_DesugarEnv.try_lookup_record_by_field_name env) f)
in (match (_62_1057) with
| (record, _62_1056) -> begin
(

let fields = (FStar_All.pipe_right fields (FStar_List.map (fun _62_1060 -> (match (_62_1060) with
| (f, p) -> begin
...
let args = (FStar_All.pipe_right record.FStar_Parser_DesugarEnv.fields (FStar_List.map (fun _62_1065 -> (match (_62_1065) with
| (f, _62_1064) -> begin
(match ((FStar_All.pipe_right fields (FStar_List.tryFind (fun _62_1069 -> (match (_62_1069) with
| (g, _62_1068) -> begin
...
let _62_1084 = (aux loc env app)
in (match (_62_1084) with
| (env, e, b, p, _62_1083) -> begin
(

let p = (match (p.FStar_Absyn_Syntax.v) with
| FStar_Absyn_Syntax.Pat_cons (fv, _62_1087, args) -> begin
...
let _62_1101 = (aux [] env p)
in (match (_62_1101) with
| (_62_1095, env, b, p, _62_1100) -> begin
((env), (b), (p))
end))))))
...
(match (p.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatVar (x, _62_1107) -> begin
...
| FStar_Parser_AST.PatAscribed ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (x, _62_1114); FStar_Parser_AST.prange = _62_1111}, t) -> begin
...
let _62_1126 = (desugar_data_pat env p)
in (match (_62_1126) with
| (env, binder, p) -> begin
(
...
and desugar_match_pat_maybe_top : Prims.bool  ->  FStar_Parser_DesugarEnv.env  ->  FStar_Parser_AST.pattern  ->  (env_t * FStar_Absyn_Syntax.pat) = (fun _62_1141 env pat -> (

let _62_1149 = (desugar_data_pat env pat)
in (match (_62_1149) with
| (env, _62_1147, pat) -> begin
((env), (pat))
end)))
...
let _62_1169 = e
in {FStar_Absyn_Syntax.n = _62_1169.FStar_Absyn_Syntax.n; FStar_Absyn_Syntax.tk = _62_1169.FStar_Absyn_Syntax.tk; FStar_Absyn_Syntax.pos = top.FStar_Parser_AST.range; FStar_Absyn_Syntax.fvs = _62_1169.FStar_Absyn_Syntax.fvs; FStar_Absyn_Syntax.uvs = _62_1169.FStar_Absyn_Syntax.uvs}))
...
let args = (FStar_List.map (fun _62_1196 -> (match (_62_1196) with
| (t, imp) -> begin
(
...
let _62_1233 = (FStar_List.fold_left (fun _62_1205 pat -> (match (_62_1205) with
| (env, ftvs) -> begin
(match (pat.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatAscribed ({FStar_Parser_AST.pat = FStar_Parser_AST.PatTvar (a, imp); FStar_Parser_AST.prange = _62_1208}, t) -> begin
(
...
let _62_1256 = (desugar_binding_pat env p)
in (match (_62_1256) with
| (env, b, pat) -> begin
(

let _62_1316 = (match (b) with
| LetBinder (_62_1258) -> begin
(FStar_All.failwith "Impossible")
end
| TBinder (a, k, aq) -> begin
...
| (FStar_Absyn_Syntax.Exp_bvar (_62_1287), _62_1290) -> begin
(
...
| (FStar_Absyn_Syntax.Exp_app (_62_1296, args), FStar_Absyn_Syntax.Pat_cons (_62_1301, _62_1303, pats)) -> begin
(
...
| FStar_Parser_AST.App ({FStar_Parser_AST.tm = FStar_Parser_AST.Var (a); FStar_Parser_AST.range = _62_1320; FStar_Parser_AST.level = _62_1318}, arg, _62_1326) when ((FStar_Ident.lid_equals a FStar_Absyn_Const.assert_lid) || (FStar_Ident.lid_equals a FStar_Absyn_Const.assume_lid)) -> begin
(
...
| FStar_Parser_AST.App (_62_1331) -> begin
(
...
| FStar_Parser_AST.LetOpen (_62_1350) -> begin
...
let ds_let_rec = (fun _62_1363 -> (match (()) with
| () -> begin
(
...
let funs = (FStar_All.pipe_right bindings (FStar_List.map (fun _62_1367 -> (match (_62_1367) with
| (p, def) -> begin
...
| _62_1373 -> begin
(match (p.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatAscribed ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (id, _62_1378); FStar_Parser_AST.prange = _62_1375}, t) -> begin
...
| FStar_Parser_AST.PatVar (id, _62_1387) -> begin
if top_level then begin
...
let _62_1417 = (FStar_List.fold_left (fun _62_1395 _62_1404 -> (match (((_62_1395), (_62_1404))) with
| ((env, fnames), ((f, _62_1398, _62_1400), _62_1403)) -> begin
(

let _62_1414 = (match (f) with
| FStar_Util.Inl (x) -> begin
(

let _62_1409 = (FStar_Parser_DesugarEnv.push_local_vbinding env x)
...
in (match (_62_1414) with
| (env, lbname) -> begin
((env), ((lbname)::fnames))
end))
end)) ((env), ([])) funs)
in (match (_62_1417) with
| (env', fnames) -> begin
(
...
let desugar_one_def = (fun env lbname _62_1428 -> (match (_62_1428) with
| ((_62_1423, args, result_t), def) -> begin
(
...
let _62_1448 = (desugar_binding_pat_maybe_top top_level env pat)
in (match (_62_1448) with
| (env, binder, pat) -> begin
(match (binder) with
| TBinder (_62_1450) -> begin
...
let desugar_branch = (fun _62_1499 -> (match (_62_1499) with
| (pat, wopt, b) -> begin
(

let _62_1502 = (desugar_match_pat env pat)
...
let _62_1530 = (FStar_Parser_DesugarEnv.fail_or env (FStar_Parser_DesugarEnv.try_lookup_record_by_field_name env) f)
in (match (_62_1530) with
| (record, _62_1529) -> begin
(
...
let found = (FStar_All.pipe_right fields (FStar_Util.find_opt (fun _62_1538 -> (match (_62_1538) with
| (g, _62_1537) -> begin
(
...
| None -> begin
(let _157_540 = (let _157_539 = (FStar_All.pipe_right record.FStar_Parser_DesugarEnv.fields (FStar_List.map (fun _62_1554 -> (match (_62_1554) with
| (f, _62_1553) -> begin
...
let record = (let _157_545 = (let _157_544 = (FStar_All.pipe_right record.FStar_Parser_DesugarEnv.fields (FStar_List.map (fun _62_1562 -> (match (_62_1562) with
| (f, _62_1561) -> begin
...
| FStar_Absyn_Syntax.Exp_meta (FStar_Absyn_Syntax.Meta_desugared ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Exp_app ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Exp_fvar (fv, _62_1585); FStar_Absyn_Syntax.tk = _62_1582; FStar_Absyn_Syntax.pos = _62_1580; FStar_Absyn_Syntax.fvs = _62_1578; FStar_Absyn_Syntax.uvs = _62_1576}, args); FStar_Absyn_Syntax.tk = _62_1574; FStar_Absyn_Syntax.pos = _62_1572; FStar_Absyn_Syntax.fvs = _62_1570; FStar_Absyn_Syntax.uvs = _62_1568}, FStar_Absyn_Syntax.Data_app)) -> begin
(
...
let _62_1606 = (FStar_Parser_DesugarEnv.fail_or env (FStar_Parser_DesugarEnv.try_lookup_projector_by_field_name env) f)
in (match (_62_1606) with
...
let _62_1611 = (FStar_Util.prefix fieldname.FStar_Ident.ns)
in (match (_62_1611) with
| (ns, _62_1610) -> begin
...
let _62_1632 = t
in {FStar_Absyn_Syntax.n = _62_1632.FStar_Absyn_Syntax.n; FStar_Absyn_Syntax.tk = _62_1632.FStar_Absyn_Syntax.tk; FStar_Absyn_Syntax.pos = top.FStar_Parser_AST.range; FStar_Absyn_Syntax.fvs = _62_1632.FStar_Absyn_Syntax.fvs; FStar_Absyn_Syntax.uvs = _62_1632.FStar_Absyn_Syntax.uvs}))
in (
...
| FStar_Parser_AST.Op ("*", (t1)::(_62_1664)::[]) -> begin
...
let args = (FStar_List.map (fun _62_1714 -> (match (_62_1714) with
| (t, imp) -> begin
...
let _62_1732 = (desugar_binding_pat env hd)
...
| FStar_Parser_AST.App (_62_1738) -> begin
(
...
let _62_1762 = (uncurry binders t)
in (match (_62_1762) with
| (bs, t) -> begin
(
...
let _62_1776 = (as_binder env hd.FStar_Parser_AST.aqual bb)
in (match (_62_1776) with
| (b, env) -> begin
(aux env ((b)::bs) tl)
end))))
end))
...
let _62_1797 = (match ((as_binder env None (FStar_Util.Inr (b)))) with
| ((FStar_Util.Inr (x), _62_1789), env) -> begin
((x), (env))
end
| _62_1794 -> begin
(FStar_All.failwith "impossible")
end)
in (match (_62_1797) with
| (b, env) -> begin
(
...
let _62_1831 = (FStar_List.fold_left (fun _62_1816 b -> (match (_62_1816) with
| (env, tparams, typs) -> begin
(

let _62_1820 = (desugar_exp_binder env b)
in (match (_62_1820) with
| (xopt, t) -> begin
(

let _62_1826 = (match (xopt) with
| None -> begin
...
in (match (_62_1831) with
| (env, _62_1829, targs) -> begin
(
...
| FStar_Parser_AST.Record (_62_1834) -> begin
...
| _62_1853 when (top.FStar_Parser_AST.level = FStar_Parser_AST.Formula) -> begin
(desugar_formula env top)
end
| _62_1855 -> begin
...
let _62_1866 = (head_and_args t)
in (match (_62_1866) with
| (head, args) -> begin
...
let _62_1892 = (FStar_All.pipe_right args (FStar_List.partition (fun _62_1874 -> (match (_62_1874) with
| (arg, _62_1873) -> begin
(match ((let _157_667 = (unparen arg)
in _157_667.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.App ({FStar_Parser_AST.tm = FStar_Parser_AST.Var (d); FStar_Parser_AST.range = _62_1878; FStar_Parser_AST.level = _62_1876}, _62_1883, _62_1885) -> begin
...
let args = (FStar_List.map (fun _62_1907 -> (match (_62_1907) with
| (t, imp) -> begin
...
let _62_1914 = (FStar_Absyn_Util.head_and_args t)
in (match (_62_1914) with
| (head, args) -> begin
(match ((let _157_673 = (let _157_672 = (FStar_Absyn_Util.compress_typ head)
in _157_672.FStar_Absyn_Syntax.n)
in ((_157_673), (args)))) with
| (FStar_Absyn_Syntax.Typ_const (eff), ((FStar_Util.Inl (result_typ), _62_1921))::rest) -> begin
(

let _62_1961 = (FStar_All.pipe_right rest (FStar_List.partition (fun _62_11 -> (match (_62_11) with
| (FStar_Util.Inr (_62_1927), _62_1930) -> begin
false
end
| (FStar_Util.Inl (t), _62_1935) -> begin
(match (t.FStar_Absyn_Syntax.n) with
| FStar_Absyn_Syntax.Typ_app ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Typ_const (fv); FStar_Absyn_Syntax.tk = _62_1944; FStar_Absyn_Syntax.pos = _62_1942; FStar_Absyn_Syntax.fvs = _62_1940; FStar_Absyn_Syntax.uvs = _62_1938}, ((FStar_Util.Inr (_62_1949), _62_1952))::[]) -> begin
...
in (match (_62_1961) with
| (dec, rest) -> begin
(

let decreases_clause = (FStar_All.pipe_right dec (FStar_List.map (fun _62_12 -> (match (_62_12) with
| (FStar_Util.Inl (t), _62_1966) -> begin
(match (t.FStar_Absyn_Syntax.n) with
| FStar_Absyn_Syntax.Typ_app (_62_1969, ((FStar_Util.Inr (arg), _62_1973))::[]) -> begin
...
let _62_2002 = kk
in {FStar_Absyn_Syntax.n = _62_2002.FStar_Absyn_Syntax.n; FStar_Absyn_Syntax.tk = _62_2002.FStar_Absyn_Syntax.tk; FStar_Absyn_Syntax.pos = k.FStar_Parser_AST.range; FStar_Absyn_Syntax.fvs = _62_2002.FStar_Absyn_Syntax.fvs; FStar_Absyn_Syntax.uvs = _62_2002.FStar_Absyn_Syntax.uvs}))
in (

let k = (unparen k)
in (match (k.FStar_Parser_AST.tm) with
| FStar_Parser_AST.Name ({FStar_Ident.ns = _62_2011; FStar_Ident.ident = _62_2009; FStar_Ident.nsstr = _62_2007; FStar_Ident.str = "Type"}) -> begin
(setpos FStar_Absyn_Syntax.mk_Kind_type)
end
| FStar_Parser_AST.Name ({FStar_Ident.ns = _62_2020; FStar_Ident.ident = _62_2018; FStar_Ident.nsstr = _62_2016; FStar_Ident.str = "Effect"}) -> begin
...
let _62_2036 = (uncurry bs k)
in (match (_62_2036) with
| (bs, k) -> begin
(
...
let _62_2047 = (let _157_709 = (let _157_708 = (FStar_Parser_DesugarEnv.default_ml env)
...
let args = (FStar_List.map (fun _62_2057 -> (match (_62_2057) with
| (t, b) -> begin
(
...
let _62_2077 = t
in {FStar_Absyn_Syntax.n = _62_2077.FStar_Absyn_Syntax.n; FStar_Absyn_Syntax.tk = _62_2077.FStar_Absyn_Syntax.tk; FStar_Absyn_Syntax.pos = f.FStar_Parser_AST.range; FStar_Absyn_Syntax.fvs = _62_2077.FStar_Absyn_Syntax.fvs; FStar_Absyn_Syntax.uvs = _62_2077.FStar_Absyn_Syntax.uvs}))
in (
...
let _62_2085 = b
in {FStar_Parser_AST.b = _62_2085.FStar_Parser_AST.b; FStar_Parser_AST.brange = _62_2085.FStar_Parser_AST.brange; FStar_Parser_AST.blevel = FStar_Parser_AST.Formula; FStar_Parser_AST.aqual = _62_2085.FStar_Parser_AST.aqual}))
in (
...
let _62_2100 = (FStar_Parser_DesugarEnv.push_local_tbinding env a)
...
let _62_2115 = (FStar_Parser_DesugarEnv.push_local_vbinding env x)
...
| (Some (conn), (_62_2164)::(_62_2162)::[]) -> begin
...
let _62_2234 = env
in {FStar_Parser_DesugarEnv.curmodule = _62_2234.FStar_Parser_DesugarEnv.curmodule; FStar_Parser_DesugarEnv.modules = _62_2234.FStar_Parser_DesugarEnv.modules; FStar_Parser_DesugarEnv.open_namespaces = _62_2234.FStar_Parser_DesugarEnv.open_namespaces; FStar_Parser_DesugarEnv.modul_abbrevs = _62_2234.FStar_Parser_DesugarEnv.modul_abbrevs; FStar_Parser_DesugarEnv.sigaccum = _62_2234.FStar_Parser_DesugarEnv.sigaccum; FStar_Parser_DesugarEnv.localbindings = _62_2234.FStar_Parser_DesugarEnv.localbindings; FStar_Parser_DesugarEnv.recbindings = _62_2234.FStar_Parser_DesugarEnv.recbindings; FStar_Parser_DesugarEnv.phase = FStar_Parser_AST.Formula; FStar_Parser_DesugarEnv.sigmap = _62_2234.FStar_Parser_DesugarEnv.sigmap; FStar_Parser_DesugarEnv.default_result_effect = _62_2234.FStar_Parser_DesugarEnv.default_result_effect; FStar_Parser_DesugarEnv.iface = _62_2234.FStar_Parser_DesugarEnv.iface; FStar_Parser_DesugarEnv.admitted_iface = _62_2234.FStar_Parser_DesugarEnv.admitted_iface}) t))
...
let _62_2267 = (FStar_List.fold_left (fun _62_2242 b -> (match (_62_2242) with
| (env, out) -> begin
(
...
let _62_2244 = b
in {FStar_Parser_AST.b = _62_2244.FStar_Parser_AST.b; FStar_Parser_AST.brange = _62_2244.FStar_Parser_AST.brange; FStar_Parser_AST.blevel = FStar_Parser_AST.Formula; FStar_Parser_AST.aqual = _62_2244.FStar_Parser_AST.aqual}))
in (match (tk) with
| FStar_Util.Inl (Some (a), k) -> begin
(

let _62_2254 = (FStar_Parser_DesugarEnv.push_local_tbinding env a)
...
let _62_2262 = (FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_62_2262) with
| (env, x) -> begin
...
let fail = (fun _62_2285 -> (match (()) with
| () -> begin
...
let _62_2296 = FStar_Absyn_Syntax.mk_Kind_type
in {FStar_Absyn_Syntax.n = _62_2296.FStar_Absyn_Syntax.n; FStar_Absyn_Syntax.tk = _62_2296.FStar_Absyn_Syntax.tk; FStar_Absyn_Syntax.pos = b.FStar_Parser_AST.brange; FStar_Absyn_Syntax.fvs = _62_2296.FStar_Absyn_Syntax.fvs; FStar_Absyn_Syntax.uvs = _62_2296.FStar_Absyn_Syntax.uvs})))
end
| _62_2299 -> begin
(fail ())
end)))
...
| FStar_Absyn_Syntax.Kind_abbrev (_62_2310, k) -> begin
(aux bs k)
end
| _62_2315 -> begin
bs
...
let imp_binders = (FStar_All.pipe_right binders (FStar_List.map (fun _62_2329 -> (match (_62_2329) with
| (x, _62_2328) -> begin
((x), (Some (imp_tag)))
end))))
in (
...
let mk_indexed_projectors = (fun fvq refine_domain env _62_2341 lid formals t -> (match (_62_2341) with
| (tc, tps, k) -> begin
(
...
let imp_binders = (FStar_All.pipe_right binders (FStar_List.map (fun _62_2358 -> (match (_62_2358) with
| (x, _62_2357) -> begin
((x), (Some (imp_tag)))
end))))
in (
...
let _62_2369 = (FStar_Absyn_Util.mk_field_projector_name lid a i)
in (match (_62_2369) with
| (field_name, _62_2368) -> begin
(
...
let _62_2376 = (FStar_Absyn_Util.mk_field_projector_name lid x i)
in (match (_62_2376) with
| (field_name, _62_2375) -> begin
(
...
let all_params = (let _157_892 = (FStar_All.pipe_right tps (FStar_List.map (fun _62_2383 -> (match (_62_2383) with
| (b, _62_2382) -> begin
((b), (Some (imp_tag)))
end))))
...
let _62_2392 = (FStar_Absyn_Util.mk_field_projector_name lid a i)
in (match (_62_2392) with
| (field_name, _62_2391) -> begin
(
...
let _62_2399 = (FStar_Absyn_Util.mk_field_projector_name lid x i)
in (match (_62_2399) with
| (field_name, _62_2398) -> begin
(
...
| Some (FStar_Absyn_Syntax.Implicit (_62_2407)) -> begin
true
end
| _62_2411 -> begin
false
...
| (FStar_Util.Inl (_62_2416), imp) -> begin
...
| (FStar_Util.Inr (_62_2421), imp) -> begin
...
| FStar_Absyn_Syntax.Sig_datacon (lid, t, tycon, quals, _62_2438, _62_2440) when (not ((FStar_Ident.lid_equals lid FStar_Absyn_Const.lexcons_lid))) -> begin
(

let refine_domain = if (FStar_All.pipe_right quals (FStar_Util.for_some (fun _62_15 -> (match (_62_15) with
| FStar_Absyn_Syntax.RecordConstructor (_62_2445) -> begin
true
end
| _62_2448 -> begin
false
...
let _62_2454 = tycon
in (match (_62_2454) with
| (l, _62_2451, _62_2453) -> begin
...
let mfields = (FStar_List.map (fun _62_2557 -> (match (_62_2557) with
| (x, t, _62_2556) -> begin
...
in (let _157_963 = (FStar_All.pipe_right fields (FStar_List.map (fun _62_2566 -> (match (_62_2566) with
| (x, _62_2563, _62_2565) -> begin
...
let _62_2582 = (typars_of_binders _env binders)
in (match (_62_2582) with
| (_env', typars) -> begin
(
...
| (FStar_Util.Inr (x), _62_2600) -> begin
(FStar_Parser_DesugarEnv.push_bvvdef env x.FStar_Absyn_Syntax.v)
end
| (FStar_Util.Inl (a), _62_2605) -> begin
...
in (match (tcs) with
| (FStar_Parser_AST.TyconAbstract (_62_2609))::[] -> begin
(

let tc = (FStar_List.hd tcs)
in (

let _62_2620 = (desugar_abstract_tc quals env [] tc)
in (match (_62_2620) with
| (_62_2614, _62_2616, se, _62_2619) -> begin
(
...
let _62_2621 = (let _157_984 = (FStar_Range.string_of_range rng)
...
let _62_2634 = (typars_of_binders env binders)
in (match (_62_2634) with
| (env', typars) -> begin
(
...
| (FStar_Parser_AST.TyconRecord (_62_2658))::[] -> begin
(

let trec = (FStar_List.hd tcs)
in (

let _62_2664 = (tycon_record_as_variant trec)
in (match (_62_2664) with
| (t, fs) -> begin
(desugar_tycon env rng ((FStar_Absyn_Syntax.RecordType (fs))::quals) ((t)::[]))
end)))
end
| (_62_2668)::_62_2666 -> begin
(
...
let _62_2679 = et
in (match (_62_2679) with
| (env, tcs) -> begin
(match (tc) with
| FStar_Parser_AST.TyconRecord (_62_2681) -> begin
(

let trec = tc
in (

let _62_2686 = (tycon_record_as_variant trec)
in (match (_62_2686) with
| (t, fs) -> begin
...
let _62_2698 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (_62_2698) with
| (env, _62_2695, se, tconstr) -> begin
...
let _62_2710 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (_62_2710) with
| (env, _62_2707, se, tconstr) -> begin
...
let _62_2715 = (FStar_List.fold_left (collect_tcs quals) ((env), ([])) tcs)
in (match (_62_2715) with
| (env, tcs) -> begin
(
...
| FStar_Util.Inr (FStar_Absyn_Syntax.Sig_tycon (id, tpars, k, _62_2722, _62_2724, _62_2726, _62_2728), t, quals) -> begin
(
...
| FStar_Util.Inl (FStar_Absyn_Syntax.Sig_tycon (tname, tpars, k, mutuals, _62_2742, tags, _62_2745), constrs, tconstr, quals) -> begin
(
...
let _62_2778 = (let _157_1008 = (FStar_All.pipe_right constrs (FStar_List.map (fun _62_2760 -> (match (_62_2760) with
| (id, topt, _62_2758, of_notation) -> begin
(
...
| FStar_Absyn_Syntax.Sig_tycon (tname, tps, k, _62_2790, constrs, quals, _62_2794) -> begin
...
let _62_2829 = (FStar_List.fold_left (fun _62_2807 b -> (match (_62_2807) with
| (env, binders) -> begin
(match ((desugar_binder env b)) with
| FStar_Util.Inl (Some (a), k) -> begin
(

let _62_2816 = (FStar_Parser_DesugarEnv.push_local_tbinding env a)
in (match (_62_2816) with
| (env, a) -> begin
...
let _62_2824 = (FStar_Parser_DesugarEnv.push_local_vbinding env x)
in (match (_62_2824) with
| (env, x) -> begin
