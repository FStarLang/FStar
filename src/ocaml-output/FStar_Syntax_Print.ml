  ...
  fun x  ->
    ...
    | FStar_Syntax_Syntax.Sig_let
        ((uu____2551,{ FStar_Syntax_Syntax.lbname = lb;
                       ...
                       FStar_Syntax_Syntax.lbdef = uu____2556;
                       FStar_Syntax_Syntax.lbattrs = uu____2557;_}::[]),uu____2558)
        ->
        let uu____2585 = lbname_to_string lb in
        let uu____2586 = term_to_string t in
        FStar_Util.format2 "let %s : %s" uu____2585 uu____2586
    | uu____2587 ->
        let uu____2588 =
...
let rec modul_to_string: FStar_Syntax_Syntax.modul -> Prims.string =
  fun m  ->
    let uu____2602 = sli m.FStar_Syntax_Syntax.name in
    let uu____2603 =
      let uu____2604 =
        FStar_List.map sigelt_to_string m.FStar_Syntax_Syntax.declarations in
      FStar_All.pipe_right uu____2604 (FStar_String.concat "\n") in
    FStar_Util.format2 "module %s\n%s" uu____2602 uu____2603
let subst_elt_to_string: FStar_Syntax_Syntax.subst_elt -> Prims.string =
  fun uu___76_2611  ->
    match uu___76_2611 with
    | FStar_Syntax_Syntax.DB (i,x) ->
        let uu____2614 = FStar_Util.string_of_int i in
        let uu____2615 = bv_to_string x in
        FStar_Util.format2 "DB (%s, %s)" uu____2614 uu____2615
    | FStar_Syntax_Syntax.NM (x,i) ->
        let uu____2618 = bv_to_string x in
        let uu____2619 = FStar_Util.string_of_int i in
        FStar_Util.format2 "NM (%s, %s)" uu____2618 uu____2619
    | FStar_Syntax_Syntax.NT (x,t) ->
        let uu____2626 = bv_to_string x in
    ...
    | FStar_Syntax_Syntax.UN (i,u) ->
        ...
        let uu____2631 = univ_to_string u in
        FStar_Util.format2 "UN (%s, %s)" uu____2630 uu____2631
    | FStar_Syntax_Syntax.UD (u,i) ->
        let uu____2634 = FStar_Util.string_of_int i in
        FStar_Util.format2 "UD (%s, %s)" u.FStar_Ident.idText uu____2634
let subst_to_string: FStar_Syntax_Syntax.subst_t -> Prims.string =
  fun s  ->
    let uu____2638 =
      FStar_All.pipe_right s (FStar_List.map subst_elt_to_string) in
    FStar_All.pipe_right uu____2638 (FStar_String.concat "; ")
  ...
  fun f  ->
    fun elts  ->
      ...
      | x::xs ->
          let strb = FStar_Util.new_string_builder () in
          (FStar_Util.string_builder_append strb "[";
           (let uu____2706 = f x in
            FStar_Util.string_builder_append strb uu____2706);
           FStar_List.iter
             (fun x1  ->
                FStar_Util.string_builder_append strb "; ";
                (let uu____2713 = f x1 in
                 FStar_Util.string_builder_append strb uu____2713)) xs;
           FStar_Util.string_builder_append strb "]";
           FStar_Util.string_of_string_builder strb)
let set_to_string:
  ...
  fun f  ->
    fun s  ->
      ...
      | x::xs ->
          let strb = FStar_Util.new_string_builder () in
          (FStar_Util.string_builder_append strb "{";
           (let uu____2746 = f x in
            FStar_Util.string_builder_append strb uu____2746);
           FStar_List.iter
             (fun x1  ->
                FStar_Util.string_builder_append strb ", ";
                (let uu____2753 = f x1 in
                 FStar_Util.string_builder_append strb uu____2753)) xs;
           FStar_Util.string_builder_append strb "}";
           FStar_Util.string_of_string_builder strb)