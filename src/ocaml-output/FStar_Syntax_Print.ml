...
let nm_to_string: FStar_Syntax_Syntax.bv -> Prims.string =
  fun bv  ->
    ...
    | uu____25 -> (bv.FStar_Syntax_Syntax.ppname).FStar_Ident.idText
...
let rec reconstruct_lex:
  ...
  fun e  ->
    ...
    | uu____325 ->
        ...
        (match uu____326 with | true  -> Some [] | uu____336 -> None)
let rec find f l =
  match l with
  | [] -> failwith "blah"
  | hd::tl ->
      let uu____362 = f hd in
      (match uu____362 with | true  -> hd | uu____363 -> find f tl)
let find_lid:
  FStar_Ident.lident ->
...
let const_to_string: FStar_Const.sconst -> Prims.string =
  fun x  ->
    ...
    | FStar_Const.Const_bool b ->
        (match b with | true  -> "true" | uu____424 -> "false")
...
let rec term_to_string: FStar_Syntax_Syntax.term -> Prims.string =
  fun x  ->
    ...
    let x =
      ...
      | true  -> x
      | uu____811 -> FStar_Syntax_Util.unmeta x in
  ...
  fun quals  ->
    fun lbs  ->
      ...
      FStar_Util.format3 "%slet %s %s" uu____1365
        (match Prims.fst lbs with | true  -> "rec" | uu____1384 -> "")
        uu____1366
  ...
  fun sep  ->
    fun bs  ->
      let bs =
        ...
        match uu____1425 with | true  -> bs | uu____1426 -> filter_imp bs in
      match sep = " -> " with
      | true  ->
          let uu____1427 =
...
and args_to_string: FStar_Syntax_Syntax.args -> Prims.string =
  fun args  ->
    let args =
      ...
      match uu____1451 with | true  -> args | uu____1452 -> filter_imp args in
                        ...
                        (match uu____1531 with
                         ...
                         | uu____1537 ->
                             ...
                             let uu____1539 =
                               term_to_string
                                 c.FStar_Syntax_Syntax.result_typ in
                             FStar_Util.format2 "%s (%s)" uu____1538
                               uu____1539))) in
        let dec =
          let uu____1541 =
            FStar_All.pipe_right c.FStar_Syntax_Syntax.flags
  ...
  fun for_free  ->
    fun ed  ->
      ...
      let uu____1599 =
        ...
        (match for_free with | true  -> "_for_free " | uu____1656 -> "") ::
          uu____1601 in
      FStar_Util.format
...
let rec sigelt_to_string: FStar_Syntax_Syntax.sigelt -> Prims.string =
  fun x  ->
    ...
    | FStar_Syntax_Syntax.Sig_inductive_typ
        (lid,univs,tps,k,uu____1670,uu____1671,quals,uu____1673) ->
        let uu____1680 = quals_to_string' quals in
        let uu____1681 = binders_to_string " " tps in
        let uu____1682 = term_to_string k in
        FStar_Util.format4 "%stype %s %s : %s" uu____1680 lid.FStar_Ident.str
          uu____1681 uu____1682
    | FStar_Syntax_Syntax.Sig_datacon
        (lid,univs,t,uu____1686,uu____1687,uu____1688,uu____1689,uu____1690)
        ->
        let uu____1695 = FStar_Options.print_universes () in
        (match uu____1695 with
         | true  ->
             let uu____1696 = FStar_Syntax_Subst.open_univ_vars univs t in
             (match uu____1696 with
              | (univs,t) ->
                  let uu____1701 = univ_names_to_string univs in
                  let uu____1702 = term_to_string t in
                  FStar_Util.format3 "datacon<%s> %s : %s" uu____1701
                    lid.FStar_Ident.str uu____1702)
         | uu____1703 ->
             let uu____1704 = term_to_string t in
             FStar_Util.format2 "datacon %s : %s" lid.FStar_Ident.str
               uu____1704)
    | FStar_Syntax_Syntax.Sig_declare_typ (lid,univs,t,quals,uu____1709) ->
        let uu____1712 = FStar_Syntax_Subst.open_univ_vars univs t in
        (match uu____1712 with
         | (univs,t) ->
             let uu____1717 = quals_to_string' quals in
             let uu____1718 =
               let uu____1719 = FStar_Options.print_universes () in
               match uu____1719 with
               | true  ->
                   let uu____1720 = univ_names_to_string univs in
                   FStar_Util.format1 "<%s>" uu____1720
               | uu____1721 -> "" in
             let uu____1722 = term_to_string t in
             FStar_Util.format4 "%sval %s %s : %s" uu____1717
               lid.FStar_Ident.str uu____1718 uu____1722)
    | FStar_Syntax_Syntax.Sig_assume (lid,f,uu____1725,uu____1726) ->
        let uu____1729 = term_to_string f in
        FStar_Util.format2 "val %s : %s" lid.FStar_Ident.str uu____1729
    | FStar_Syntax_Syntax.Sig_let (lbs,uu____1731,uu____1732,qs,uu____1734)
        -> lbs_to_string qs lbs
    | FStar_Syntax_Syntax.Sig_main (e,uu____1742) ->
        let uu____1743 = term_to_string e in
        FStar_Util.format1 "let _ = %s" uu____1743
    | FStar_Syntax_Syntax.Sig_bundle (ses,uu____1745,uu____1746,uu____1747)
        ->
        let uu____1754 = FStar_List.map sigelt_to_string ses in
        FStar_All.pipe_right uu____1754 (FStar_String.concat "\n")
    | FStar_Syntax_Syntax.Sig_new_effect (ed,uu____1758) ->
        eff_decl_to_string false ed
    | FStar_Syntax_Syntax.Sig_new_effect_for_free (ed,uu____1760) ->
    ...
    | FStar_Syntax_Syntax.Sig_sub_effect (se,r) ->
        let lift_wp =
          ...
          | (Some lift_wp,uu____1769) -> lift_wp
          | (uu____1773,Some lift) -> lift in
        let uu____1778 =
          FStar_Syntax_Subst.open_univ_vars (Prims.fst lift_wp)
            (Prims.snd lift_wp) in
        (match uu____1778 with
         | (us,t) ->
             let uu____1785 = lid_to_string se.FStar_Syntax_Syntax.source in
             let uu____1786 = lid_to_string se.FStar_Syntax_Syntax.target in
             let uu____1787 = univ_names_to_string us in
             let uu____1788 = term_to_string t in
             FStar_Util.format4 "sub_effect %s ~> %s : <%s> %s" uu____1785
               uu____1786 uu____1787 uu____1788)
    | FStar_Syntax_Syntax.Sig_effect_abbrev
        (l,univs,tps,c,uu____1793,flags,uu____1795) ->
        let uu____1800 = FStar_Options.print_universes () in
        (match uu____1800 with
         | true  ->
             let uu____1801 =
               ...
               FStar_Syntax_Subst.open_univ_vars univs uu____1804 in
             (match uu____1801 with
              | (univs,t) ->
                  let uu____1815 =
                    ...
                    | uu____1851 -> failwith "impossible" in
                  (match uu____1815 with
                   | (tps,c) ->
                       let uu____1871 = sli l in
                       let uu____1872 = univ_names_to_string univs in
                       let uu____1873 = binders_to_string " " tps in
                       let uu____1874 = comp_to_string c in
                       FStar_Util.format4 "effect %s<%s> %s = %s" uu____1871
                         uu____1872 uu____1873 uu____1874))
         | uu____1875 ->
             let uu____1876 = sli l in
             let uu____1877 = binders_to_string " " tps in
             let uu____1878 = comp_to_string c in
             FStar_Util.format3 "effect %s %s = %s" uu____1876 uu____1877
               uu____1878)
let format_error: FStar_Range.range -> Prims.string -> Prims.string =
  fun r  ->
    fun msg  ->
      let uu____1885 = FStar_Range.string_of_range r in
      FStar_Util.format2 "%s: %s\n" uu____1885 msg
let rec sigelt_to_string_short: FStar_Syntax_Syntax.sigelt -> Prims.string =
  fun x  ->
    match x with
    | FStar_Syntax_Syntax.Sig_let
        ((uu____1889,{ FStar_Syntax_Syntax.lbname = lb;
                       FStar_Syntax_Syntax.lbunivs = uu____1891;
                       FStar_Syntax_Syntax.lbtyp = t;
                       FStar_Syntax_Syntax.lbeff = uu____1893;
                       FStar_Syntax_Syntax.lbdef = uu____1894;_}::[]),uu____1895,uu____1896,uu____1897,uu____1898)
        ->
        let uu____1916 = lbname_to_string lb in
        let uu____1917 = term_to_string t in
        FStar_Util.format2 "let %s : %s" uu____1916 uu____1917
    | uu____1918 ->
        let uu____1919 =
...
let rec modul_to_string: FStar_Syntax_Syntax.modul -> Prims.string =
  fun m  ->
    let uu____1928 = sli m.FStar_Syntax_Syntax.name in
    let uu____1929 =
      let uu____1930 =
...
let subst_elt_to_string: FStar_Syntax_Syntax.subst_elt -> Prims.string =
  fun uu___189_1935  ->
    match uu___189_1935 with
    | FStar_Syntax_Syntax.DB (i,x) ->
        let uu____1938 = FStar_Util.string_of_int i in
        let uu____1939 = bv_to_string x in
        FStar_Util.format2 "DB (%s, %s)" uu____1938 uu____1939
    | FStar_Syntax_Syntax.NM (x,i) ->
        let uu____1942 = bv_to_string x in
        let uu____1943 = FStar_Util.string_of_int i in
        FStar_Util.format2 "NM (%s, %s)" uu____1942 uu____1943
    | FStar_Syntax_Syntax.NT (x,t) ->
        let uu____1950 = bv_to_string x in
        let uu____1951 = term_to_string t in
        FStar_Util.format2 "DB (%s, %s)" uu____1950 uu____1951
    | FStar_Syntax_Syntax.UN (i,u) ->
        let uu____1954 = FStar_Util.string_of_int i in
        let uu____1955 = univ_to_string u in
        FStar_Util.format2 "UN (%s, %s)" uu____1954 uu____1955
    | FStar_Syntax_Syntax.UD (u,i) ->
        let uu____1958 = FStar_Util.string_of_int i in
        FStar_Util.format2 "UD (%s, %s)" u.FStar_Ident.idText uu____1958
...
let list_to_string f elts =
  ...
  | x::xs ->
      let strb = FStar_Util.new_string_builder () in
      (FStar_Util.string_builder_append strb "[";
       (let uu____2012 = f x in
        FStar_Util.string_builder_append strb uu____2012);
       FStar_List.iter
         (fun x  ->
            FStar_Util.string_builder_append strb "; ";
            (let uu____2016 = f x in
             FStar_Util.string_builder_append strb uu____2016)) xs;
       FStar_Util.string_builder_append strb "]";
       FStar_Util.string_of_string_builder strb)
let set_to_string f s =
  ...
  | x::xs ->
      let strb = FStar_Util.new_string_builder () in
      (FStar_Util.string_builder_append strb "{";
       (let uu____2045 = f x in
        FStar_Util.string_builder_append strb uu____2045);
       FStar_List.iter
         (fun x  ->
            FStar_Util.string_builder_append strb ", ";
            (let uu____2049 = f x in
             FStar_Util.string_builder_append strb uu____2049)) xs;
       FStar_Util.string_builder_append strb "}";
       FStar_Util.string_of_string_builder strb)