...
let find_nearest_preceding_col_info col col_infos =
  let rec aux out uu___95_526 =
    match uu___95_526 with
    | [] -> out
  ...
  fun id  ->
    fun ty  ->
      fun range  ->
        ...
        let use_range =
          let uu___96_576 = range in
          {
            FStar_Range.def_range = (range.FStar_Range.use_range);
            FStar_Range.use_range = (uu___96_576.FStar_Range.use_range)
          } in
