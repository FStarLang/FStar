...
let (uu___is_TProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | TProb _0 -> true | uu____523 -> false
...
let (as_tprob :
  prob -> (FStar_Syntax_Syntax.typ,FStar_Syntax_Syntax.term) problem) =
  fun uu___28_581  ->
    match uu___28_581 with
    | TProb p -> p
    | uu____591 -> failwith "Expected a TProb"
...
let (uu___is_Trivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | Trivial  -> true | uu____607 -> false
  ...
  fun tac  ->
    fun f  ->
      let t_by_tactic =
        ...
        FStar_Syntax_Syntax.mk_Tm_uinst uu____645
      ...
      let tac1 =
        let uu____651 =
          let uu____654 =
            ...
            let uu____656 =
              let uu____659 = FStar_Syntax_Syntax.as_arg tac  in [uu____659]
               in
            uu____655 :: uu____656  in
          FStar_Syntax_Syntax.mk_Tm_app t_reify_tactic uu____654  in
        uu____651 FStar_Pervasives_Native.None FStar_Range.dummyRange  in
      let uu____662 =
        let uu____665 =
          let uu____666 = FStar_Syntax_Syntax.iarg FStar_Syntax_Syntax.t_unit
             in
          let uu____667 =
            let uu____670 = FStar_Syntax_Syntax.as_arg tac1  in
            let uu____671 =
              let uu____674 = FStar_Syntax_Syntax.as_arg f  in [uu____674]
               in
            uu____670 :: uu____671  in
          uu____666 :: uu____667  in
        FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____665  in
      uu____662 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let rec (delta_depth_greater_than :
  ...
  fun l  ->
    fun m  ->
      match (l, m) with
      | (FStar_Syntax_Syntax.Delta_constant ,uu____687) -> false
      | (FStar_Syntax_Syntax.Delta_equational ,uu____688) -> true
      | (uu____689,FStar_Syntax_Syntax.Delta_equational ) -> false
      | (FStar_Syntax_Syntax.Delta_defined_at_level
         i,FStar_Syntax_Syntax.Delta_defined_at_level j) -> i > j
      | (FStar_Syntax_Syntax.Delta_defined_at_level
         uu____692,FStar_Syntax_Syntax.Delta_constant ) -> true
      | (FStar_Syntax_Syntax.Delta_abstract d,uu____694) ->
          delta_depth_greater_than d m
      | (uu____695,FStar_Syntax_Syntax.Delta_abstract d) ->
...
let insert_col_info :
  'Auu____776 .
    Prims.int ->
      'Auu____776 ->
        (Prims.int,'Auu____776) FStar_Pervasives_Native.tuple2 Prims.list ->
          (Prims.int,'Auu____776) FStar_Pervasives_Native.tuple2 Prims.list
  =
  fun col  ->
    fun info  ->
      fun col_infos  ->
        let rec __insert aux rest =
          ...
          | (c,i)::rest' ->
           ...
           in
        let uu____949 = __insert [] col_infos  in
...
let find_nearest_preceding_col_info :
  'Auu____1016 .
    Prims.int ->
      (Prims.int,'Auu____1016) FStar_Pervasives_Native.tuple2 Prims.list ->
        'Auu____1016 FStar_Pervasives_Native.option
  =
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___30_1059 =
        match uu___30_1059 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
...
let (id_info_table_empty : id_info_table) =
  let uu____1148 = FStar_Util.psmap_empty ()  in
  { id_info_enabled = false; id_info_db = uu____1148; id_info_buffer = [] } 
let (id_info__insert :
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let use_range1 =
          let uu____1201 = FStar_Range.use_range range  in
          FStar_Range.set_def_range range uu____1201  in
        let info1 =
          let uu___31_1203 = info  in
          let uu____1204 = ty_map info.identifier_ty  in
          {
            identifier = (uu___31_1203.identifier);
            identifier_ty = uu____1204;
        ...
        let uu____1207 =
          let uu____1212 = FStar_Range.line_of_pos start  in
          let uu____1213 = FStar_Range.col_of_pos start  in
          (uu____1212, uu____1213)  in
        match uu____1207 with
        | (row,col) ->
            let rows =
              let uu____1235 = FStar_Util.pimap_empty ()  in
              FStar_Util.psmap_find_default db fn uu____1235  in
            let cols = FStar_Util.pimap_find_default rows row []  in
            let uu____1275 =
              let uu____1284 = insert_col_info col info1 cols  in
              FStar_All.pipe_right uu____1284 (FStar_Util.pimap_add rows row)
               in
            FStar_All.pipe_right uu____1275 (FStar_Util.psmap_add db fn)
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___32_1366 = table  in
          {
            id_info_enabled = (uu___32_1366.id_info_enabled);
            id_info_db = (uu___32_1366.id_info_db);
  ...
  fun table  ->
    fun bv  ->
      fun ty  ->
        ...
        then
          let uu____1382 = FStar_Syntax_Syntax.range_of_bv bv  in
          id_info_insert table (FStar_Util.Inl bv) ty uu____1382
        else table
  ...
  fun table  ->
    fun fv  ->
      fun ty  ->
        if table.id_info_enabled
        then
          let uu____1399 = FStar_Syntax_Syntax.range_of_fv fv  in
          id_info_insert table (FStar_Util.Inr fv) ty uu____1399
        else table
  
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___33_1411 = table  in
      let uu____1412 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____1412;
        id_info_db = (uu___33_1411.id_info_db);
        id_info_buffer = (uu___33_1411.id_info_buffer)
      }
...
let (id_info_promote :
  ...
  fun table  ->
    fun ty_map  ->
      let uu___34_1428 = table  in
      let uu____1429 =
        FStar_List.fold_left (id_info__insert ty_map) table.id_info_db
          table.id_info_buffer
         in
      {
        id_info_enabled = (uu___34_1428.id_info_enabled);
        id_info_db = uu____1429;
        id_info_buffer = []
      }
  ...
  fun table  ->
    fun fn  ->
      fun row  ->
        fun col  ->
          let rows =
            let uu____1465 = FStar_Util.pimap_empty ()  in
            FStar_Util.psmap_find_default table.id_info_db fn uu____1465  in
          let cols = FStar_Util.pimap_find_default rows row []  in
          let uu____1471 = find_nearest_preceding_col_info col cols  in
          ...
          | FStar_Pervasives_Native.Some info ->
              let last_col =
                ...
                FStar_Range.col_of_pos uu____1478  in
              if col <= last_col
