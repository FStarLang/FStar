...
type rel = EQ | SUB | SUBINV [@@deriving show]
...
let uu___is_SUBINV : rel -> Prims.bool =
  fun projectee -> match projectee with SUBINV -> true | uu____12 -> false
...
let __proj__Mkproblem__item__pid: 'a 'b. ('a, 'b) problem -> Prims.int =
  fun projectee ->
    ...
    | { pid= __fname__pid
      ; lhs= __fname__lhs
      ; relation= __fname__relation
      ; rhs= __fname__rhs
      ; element= __fname__element
      ; logical_guard= __fname__logical_guard
      ; scope= __fname__scope
      ; reason= __fname__reason
      ; loc= __fname__loc
      ; rank= __fname__rank; _ }
    -> __fname__pid
...
let __proj__Mkproblem__item__loc: 'a 'b. ('a, 'b) problem -> FStar_Range.range =
  fun projectee ->
    match projectee
    with
    | { pid= __fname__pid
...
let uu___is_TProb : prob -> Prims.bool =
  fun projectee -> match projectee with TProb _0 -> true | uu____475 -> false
...
let uu___is_CProb : prob -> Prims.bool =
  fun projectee -> match projectee with CProb _0 -> true | uu____503 -> false
...
let uu___is_Trivial : guard_formula -> Prims.bool =
  fun projectee -> match projectee with Trivial -> true | uu____553 -> false
...
let __proj__NonTrivial__item___0 : guard_formula -> FStar_Syntax_Syntax.formula =
...
let mk_by_tactic
  ...
  fun tac f ->
    let t_by_tactic =
      let uu____585 =
        FStar_Syntax_Syntax.tabbrev FStar_Parser_Const.by_tactic_lid
      in
      FStar_Syntax_Syntax.mk_Tm_uinst uu____585 [FStar_Syntax_Syntax.U_zero]
    in
    let t_reify_tactic =
      let uu____587 =
        FStar_Syntax_Syntax.tabbrev FStar_Parser_Const.reify_tactic_lid
      in
      FStar_Syntax_Syntax.mk_Tm_uinst uu____587 [FStar_Syntax_Syntax.U_zero]
    in
    let tac1 =
      let uu____591 =
        let uu____592 =
          let uu____593 =
            FStar_Syntax_Syntax.iarg FStar_Syntax_Syntax.t_unit
          in
          let uu____594 =
            let uu____597 = FStar_Syntax_Syntax.as_arg tac in
            [uu____597]
          in
          uu____593 :: uu____594
        in
        FStar_Syntax_Syntax.mk_Tm_app t_reify_tactic uu____592
      in
    ...
    let uu____600 =
      let uu____601 =
        ...
        let uu____603 =
          ...
          let uu____607 =
            let uu____610 = FStar_Syntax_Syntax.as_arg f in
            [uu____610]
          in
          uu____606 :: uu____607
        in
        uu____602 :: uu____603
      in
      FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____601
    in
...
let rec delta_depth_greater_than
  ...
  fun l m ->
    ...
    | ( FStar_Syntax_Syntax.Delta_defined_at_level i
      , FStar_Syntax_Syntax.Delta_defined_at_level j ) ->
        i > j
    | ( FStar_Syntax_Syntax.Delta_defined_at_level uu____624
...
let rec decr_delta_depth
  ...
  fun uu___27_633 ->
    ...
    | FStar_Syntax_Syntax.Delta_defined_at_level _0_27
      when _0_27 = Prims.parse_int "1" ->
...
let insert_col_info:
      'Auu____696. Prims.int -> 'Auu____696
      -> (Prims.int, 'Auu____696) FStar_Pervasives_Native.tuple2 Prims.list
      -> (Prims.int, 'Auu____696) FStar_Pervasives_Native.tuple2 Prims.list =
  fun col info col_infos ->
    let rec __insert aux rest =
      match rest with
      | [] -> (aux, [(col, info)])
      | (c, i) :: rest' ->
          if col < c then (aux, (col, info) :: rest)
          else __insert ((c, i) :: aux) rest'
    in
    let uu____864 = __insert [] col_infos in
    match uu____864 with l, r -> FStar_List.append (FStar_List.rev l) r


let find_nearest_preceding_col_info:
      'Auu____928. Prims.int
...
let id_info__insert
    : (FStar_Syntax_Syntax.typ -> FStar_Syntax_Syntax.typ)
      -> (Prims.int, identifier_info) FStar_Pervasives_Native.tuple2 Prims.list
         FStar_Util.pimap
         FStar_Util.psmap -> identifier_info
      -> (Prims.int, identifier_info) FStar_Pervasives_Native.tuple2 Prims.list
         FStar_Util.pimap
         FStar_Util.psmap =
  fun ty_map db info ->
    ...
    let use_range1 =
      ...
      FStar_Range.set_def_range range uu____1097
    in
    let info1 =
    ...
    in
    match uu____1103 with row, col ->
      let rows =
        let uu____1131 = FStar_Util.pimap_empty () in
        FStar_Util.psmap_find_default db fn uu____1131
      in
...
let id_info_insert
  ...
  fun table id1 ty range ->
    let info = {identifier= id1; identifier_ty= ty; identifier_range= range} in
...
let id_info_insert_bv
    : id_info_table -> FStar_Syntax_Syntax.bv -> FStar_Syntax_Syntax.typ
      -> id_info_table =
  fun table bv ty ->
    if table.id_info_enabled then
...
let id_info_insert_fv
    : id_info_table -> FStar_Syntax_Syntax.fv -> FStar_Syntax_Syntax.typ
      -> id_info_table =
  fun table fv ty ->
    if table.id_info_enabled then
...
let id_info_promote
    : id_info_table -> (FStar_Syntax_Syntax.typ -> FStar_Syntax_Syntax.typ)
      -> id_info_table =
  fun table ty_map ->
...
let id_info_at_pos
    : id_info_table -> Prims.string -> Prims.int -> Prims.int
      -> identifier_info FStar_Pervasives_Native.option =
  fun table fn row col ->
    let rows =
      let uu____1325 = FStar_Util.pimap_empty () in
      FStar_Util.psmap_find_default table.id_info_db fn uu____1325
    in
    let cols = FStar_Util.pimap_find_default rows row [] in
    ...
    | FStar_Pervasives_Native.Some info ->
        let last_col =
          let uu____1338 = FStar_Range.end_of_range info.identifier_range in
          FStar_Range.col_of_pos uu____1338
        in
        if col <= last_col then FStar_Pervasives_Native.Some info
