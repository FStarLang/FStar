...
let (uu___is_EQ : rel -> Prims.bool) =
  fun projectee  -> match projectee with | EQ  -> true | uu____55310 -> false 
let (uu___is_SUB : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUB  -> true | uu____55321 -> false
  
let (uu___is_SUBINV : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUBINV  -> true | uu____55332 -> false
...
let (uu___is_Rigid_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_rigid  -> true | uu____55343 -> false
  
let (uu___is_Flex_rigid_eq : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid_eq  -> true | uu____55354 -> false
...
let (uu___is_Flex_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid  -> true | uu____55376 -> false
  
let (uu___is_Rigid_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_flex  -> true | uu____55387 -> false
  
let (uu___is_Flex_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_flex  -> true | uu____55398 -> false
...
let (uu___is_TProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | TProb _0 -> true | uu____55826 -> false
  
let (__proj__TProb__item___0 : prob -> FStar_Syntax_Syntax.typ problem) =
  fun projectee  -> match projectee with | TProb _0 -> _0 
let (uu___is_CProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | CProb _0 -> true | uu____55854 -> false
...
let (as_tprob : prob -> FStar_Syntax_Syntax.typ problem) =
  fun uu___429_55877  ->
    match uu___429_55877 with
    | TProb p -> p
    | uu____55883 -> failwith "Expected a TProb"
...
let (uu___is_Trivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | Trivial  -> true | uu____55903 -> false
  
let (uu___is_NonTrivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | NonTrivial _0 -> true | uu____55915 -> false
  ...
  fun tac  ->
    fun f  ->
      let t_by_tactic =
        ...
        FStar_Syntax_Syntax.mk_Tm_uinst uu____55948
      ...
      let uu____55949 =
        let uu____55954 =
          ...
          let uu____55964 =
            let uu____55975 = FStar_Syntax_Syntax.as_arg tac  in
            let uu____55984 =
              let uu____55995 = FStar_Syntax_Syntax.as_arg f  in
              [uu____55995]  in
            uu____55975 :: uu____55984  in
          uu____55955 :: uu____55964  in
        FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____55954  in
      uu____55949 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let rec (delta_depth_greater_than :
  ...
  fun l  ->
    fun m  ->
      ...
      | (FStar_Syntax_Syntax.Delta_abstract d,uu____56060) ->
          delta_depth_greater_than d m
      | (uu____56061,FStar_Syntax_Syntax.Delta_abstract d) ->
          delta_depth_greater_than l d
      | (FStar_Syntax_Syntax.Delta_equational_at_level
         uu____56063,uu____56064) -> true
      | (uu____56067,FStar_Syntax_Syntax.Delta_equational_at_level
         uu____56068) -> false
...
let rec (decr_delta_depth :
  ...
  =
  fun uu___430_56078  ->
    match uu___430_56078 with
    | FStar_Syntax_Syntax.Delta_constant_at_level _56081 when
        _56081 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
    | FStar_Syntax_Syntax.Delta_equational_at_level _56082 when
        _56082 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
      ...
      fun col_infos  ->
        let rec __insert aux rest =
          ...
          | (c,i)::rest' ->
           ...
           in
        let uu____56364 = __insert [] col_infos  in
        match uu____56364 with
...
let (find_nearest_preceding_col_info :
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___431_56485 =
        match uu___431_56485 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
...
let (id_info_table_empty : id_info_table) =
  let uu____56596 = FStar_Util.psmap_empty ()  in
  { id_info_enabled = false; id_info_db = uu____56596; id_info_buffer = [] } 
let (id_info__insert :
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let use_range1 =
          let uu____56654 = FStar_Range.use_range range  in
          FStar_Range.set_def_range range uu____56654  in
        let info1 =
          let uu___568_56656 = info  in
          let uu____56657 = ty_map info.identifier_ty  in
          {
            identifier = (uu___568_56656.identifier);
            identifier_ty = uu____56657;
        ...
        let uu____56661 =
          let uu____56668 = FStar_Range.line_of_pos start  in
          let uu____56670 = FStar_Range.col_of_pos start  in
          (uu____56668, uu____56670)  in
        match uu____56661 with
        | (row,col) ->
            let rows =
              let uu____56701 = FStar_Util.pimap_empty ()  in
              FStar_Util.psmap_find_default db fn uu____56701  in
            let cols = FStar_Util.pimap_find_default rows row []  in
            let uu____56747 =
              let uu____56757 = insert_col_info col info1 cols  in
              FStar_All.pipe_right uu____56757
                (FStar_Util.pimap_add rows row)
               in
            FStar_All.pipe_right uu____56747 (FStar_Util.psmap_add db fn)
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___583_56847 = table  in
          {
            id_info_enabled = (uu___583_56847.id_info_enabled);
            id_info_db = (uu___583_56847.id_info_db);
  ...
  fun table  ->
    fun bv  ->
      fun ty  ->
        ...
        then
          let uu____56865 = FStar_Syntax_Syntax.range_of_bv bv  in
          id_info_insert table (FStar_Util.Inl bv) ty uu____56865
        else table
  ...
  fun table  ->
    fun fv  ->
      fun ty  ->
        if table.id_info_enabled
        then
          let uu____56885 = FStar_Syntax_Syntax.range_of_fv fv  in
          id_info_insert table (FStar_Util.Inr fv) ty uu____56885
        else table
  
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___595_56901 = table  in
      let uu____56902 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____56902;
        id_info_db = (uu___595_56901.id_info_db);
        id_info_buffer = (uu___595_56901.id_info_buffer)
      }
...
let (id_info_promote :
  ...
  fun table  ->
    fun ty_map  ->
      let uu___599_56920 = table  in
      let uu____56921 =
        FStar_List.fold_left (id_info__insert ty_map) table.id_info_db
          table.id_info_buffer
         in
      {
        id_info_enabled = (uu___599_56920.id_info_enabled);
        id_info_db = uu____56921;
        id_info_buffer = []
      }
  ...
  fun table  ->
    fun fn  ->
      fun row  ->
        fun col  ->
          let rows =
            let uu____56965 = FStar_Util.pimap_empty ()  in
            FStar_Util.psmap_find_default table.id_info_db fn uu____56965  in
          let cols = FStar_Util.pimap_find_default rows row []  in
          let uu____56972 = find_nearest_preceding_col_info col cols  in
          ...
          | FStar_Pervasives_Native.Some info ->
              let last_col =
                ...
                FStar_Range.col_of_pos uu____56980  in
              if col <= last_col
            ...
            let print_gamma gamma =
              let uu____57027 =
                FStar_All.pipe_right gamma
                  (FStar_List.map
                     (fun uu___432_57040  ->
                        match uu___432_57040 with
                        | FStar_Syntax_Syntax.Binding_var x ->
                            let uu____57043 =
                              FStar_Syntax_Print.bv_to_string x  in
                            Prims.op_Hat "Binding_var " uu____57043
                        | FStar_Syntax_Syntax.Binding_univ u ->
                            Prims.op_Hat "Binding_univ " u.FStar_Ident.idText
                        | FStar_Syntax_Syntax.Binding_lid (l,uu____57049) ->
                            let uu____57066 = FStar_Ident.string_of_lid l  in
                            Prims.op_Hat "Binding_lid " uu____57066))
                 in
              FStar_All.pipe_right uu____57027 (FStar_String.concat "::\n")
               in
            let fail1 uu____57079 =
              let uu____57080 =
                let uu____57082 = FStar_Range.string_of_range r  in
                let uu____57084 = print_gamma g  in
                let uu____57086 =
                  FStar_Syntax_Print.binders_to_string ", " bs  in
                FStar_Util.format5
                  "Invariant violation: gamma and binders are out of sync\n\treason=%s, range=%s, should_check=%s\n\t\n                               gamma=%s\n\tbinders=%s\n"
                  reason uu____57082
                  (if should_check then "true" else "false") uu____57084
                  uu____57086
                 in
              failwith uu____57080  in
              ...
              (let uu____57099 =
                 let uu____57124 =
                   FStar_Util.prefix_until
                     (fun uu___433_57139  ->
                        match uu___433_57139 with
                        | FStar_Syntax_Syntax.Binding_var uu____57141 -> true
                        | uu____57143 -> false) g
                    in
                 (uu____57124, bs)  in
               match uu____57099 with
               | (FStar_Pervasives_Native.None ,[]) -> ()
               | (FStar_Pervasives_Native.Some
                  (uu____57201,hd1,gamma_tail),uu____57204::uu____57205) ->
                   let uu____57264 = FStar_Util.prefix bs  in
                   (match uu____57264 with
                    | (uu____57289,(x,uu____57291)) ->
                        (match hd1 with
