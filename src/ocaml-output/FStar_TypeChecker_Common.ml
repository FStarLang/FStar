...
| (FStar_Syntax_Syntax.Delta_equational_at_level (uu____708), uu____709) -> begin
true
end
| (uu____710, FStar_Syntax_Syntax.Delta_equational_at_level (uu____711)) -> begin
false
end
...
let rec decr_delta_depth : FStar_Syntax_Syntax.delta_depth  ->  FStar_Syntax_Syntax.delta_depth FStar_Pervasives_Native.option = (fun uu___32_722 -> (match (uu___32_722) with
| FStar_Syntax_Syntax.Delta_constant_at_level (_0_4) when (_0_4 = (Prims.parse_int "0")) -> begin
FStar_Pervasives_Native.None
end
| FStar_Syntax_Syntax.Delta_equational_at_level (uu____725) -> begin
FStar_Pervasives_Native.None
end
...
let insert_col_info : 'Auu____799 . Prims.int  ->  'Auu____799  ->  (Prims.int * 'Auu____799) Prims.list  ->  (Prims.int * 'Auu____799) Prims.list = (fun col info col_infos -> (
...
let uu____974 = (__insert [] col_infos)
...
let find_nearest_preceding_col_info : 'Auu____1041 . Prims.int  ->  (Prims.int * 'Auu____1041) Prims.list  ->  'Auu____1041 FStar_Pervasives_Native.option = (fun col col_infos -> (

let rec aux = (fun out uu___33_1086 -> (match (uu___33_1086) with
| [] -> begin
out
end
| ((c, i))::rest -> begin
...
let uu____1178 = (FStar_Util.psmap_empty ())
in {id_info_enabled = false; id_info_db = uu____1178; id_info_buffer = []})
...
let uu____1231 = (FStar_Range.use_range range)
in (FStar_Range.set_def_range range uu____1231))
in (

let info1 = (

let uu___34_1233 = info
in (

let uu____1234 = (ty_map info.identifier_ty)
in {identifier = uu___34_1233.identifier; identifier_ty = uu____1234; identifier_range = use_range1}))
in (
...
let uu____1242 = (FStar_Range.line_of_pos start)
in (

let uu____1243 = (FStar_Range.col_of_pos start)
in ((uu____1242), (uu____1243))))
...
let uu____1265 = (FStar_Util.pimap_empty ())
in (FStar_Util.psmap_find_default db fn uu____1265))
in (
...
let uu____1314 = (insert_col_info col info1 cols)
in (FStar_All.pipe_right uu____1314 (FStar_Util.pimap_add rows row)))
in (FStar_All.pipe_right uu____1305 (FStar_Util.psmap_add db fn)))))
end))))))))
...
let uu___35_1396 = table
in {id_info_enabled = uu___35_1396.id_info_enabled; id_info_db = uu___35_1396.id_info_db; id_info_buffer = (info)::table.id_info_buffer})))
...
let uu____1412 = (FStar_Syntax_Syntax.range_of_bv bv)
in (id_info_insert table (FStar_Util.Inl (bv)) ty uu____1412))
end
| uu____1413 -> begin
table
end))
...
let uu____1429 = (FStar_Syntax_Syntax.range_of_fv fv)
in (id_info_insert table (FStar_Util.Inr (fv)) ty uu____1429))
end
| uu____1430 -> begin
table
end))
...
let uu___36_1441 = table
in (

let uu____1442 = (enabled && (FStar_Options.ide ()))
in {id_info_enabled = uu____1442; id_info_db = uu___36_1441.id_info_db; id_info_buffer = uu___36_1441.id_info_buffer})))


let id_info_promote : id_info_table  ->  (FStar_Syntax_Syntax.typ  ->  FStar_Syntax_Syntax.typ)  ->  id_info_table = (fun table ty_map -> (

let uu___37_1458 = table
in (

let uu____1459 = (FStar_List.fold_left (id_info__insert ty_map) table.id_info_db table.id_info_buffer)
in {id_info_enabled = uu___37_1458.id_info_enabled; id_info_db = uu____1459; id_info_buffer = []})))
...
let uu____1495 = (FStar_Util.pimap_empty ())
in (FStar_Util.psmap_find_default table.id_info_db fn uu____1495))
in (

let cols = (FStar_Util.pimap_find_default rows row [])
in (

let uu____1501 = (find_nearest_preceding_col_info col cols)
...
let uu____1508 = (FStar_Range.end_of_range info.identifier_range)
in (FStar_Range.col_of_pos uu____1508))
