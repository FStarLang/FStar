...
let find_nearest_preceding_col_info:
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___135_967 =
        match uu___135_967 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let info1 =
          let uu___136_1099 = info in
          let uu____1100 = ty_map info.identifier_ty in
          {
            identifier = (uu___136_1099.identifier);
    ...
    fun id  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___137_1254 = table in
          {
            id_info_enabled = (uu___137_1254.id_info_enabled);
            id_info_db = (uu___137_1254.id_info_db);
...
let id_info_toggle: id_info_table -> Prims.bool -> id_info_table =
  fun table  ->
    fun enabled  ->
      let uu___138_1283 = table in
      let uu____1284 = enabled && (FStar_Options.ide ()) in
      {
        id_info_enabled = uu____1284;
        id_info_db = (uu___138_1283.id_info_db);
        id_info_buffer = (uu___138_1283.id_info_buffer)
      }
let id_info_promote:
  id_info_table ->
