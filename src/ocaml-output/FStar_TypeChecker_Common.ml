...
let find_nearest_preceding_col_info :
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___135_1023 =
        match uu___135_1023 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let info1 =
          let uu___136_1161 = info  in
          let uu____1162 = ty_map info.identifier_ty  in
          {
            identifier = (uu___136_1161.identifier);
        ...
        fun range  ->
          let info =
            { identifier = id; identifier_ty = ty; identifier_range = range }
             in
          let uu___137_1320 = table  in
          {
            id_info_enabled = (uu___137_1320.id_info_enabled);
            id_info_db = (uu___137_1320.id_info_db);
...
let id_info_toggle : id_info_table -> Prims.bool -> id_info_table =
  fun table  ->
    fun enabled  ->
      let uu___138_1357 = table  in
      let uu____1358 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____1358;
        id_info_db = (uu___138_1357.id_info_db);
        id_info_buffer = (uu___138_1357.id_info_buffer)
      }
