  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let use_range =
          let uu___128_1159 = range in
          {
            FStar_Range.def_range = (range.FStar_Range.use_range);
            FStar_Range.use_range = (uu___128_1159.FStar_Range.use_range)
          } in
        let info1 =
          let uu___129_1161 = info in
          let uu____1162 = ty_map info.identifier_ty in
          {
            identifier = (uu___129_1161.identifier);
    ...
    fun id  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___130_1320 = table in
          {
            id_info_enabled = (uu___130_1320.id_info_enabled);
            id_info_db = (uu___130_1320.id_info_db);
...
let id_info_toggle: id_info_table -> Prims.bool -> id_info_table =
  fun table  ->
    fun enabled  ->
      let uu___131_1357 = table in
      let uu____1358 = enabled && (FStar_Options.ide ()) in
      {
        id_info_enabled = uu____1358;
        id_info_db = (uu___131_1357.id_info_db);
        id_info_buffer = (uu___131_1357.id_info_buffer)
      }
let id_info_promote:
  id_info_table ->
