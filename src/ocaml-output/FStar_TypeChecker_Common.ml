...
type deferred =
  (Prims.string,prob) FStar_Pervasives_Native_Tuple2.tuple2 Prims.list
type univ_ineq =
...
let (insert_col_info :
  Prims.int ->
    identifier_info ->
      (Prims.int,identifier_info) FStar_Pervasives_Native_Tuple2.tuple2
        Prims.list ->
        (Prims.int,identifier_info) FStar_Pervasives_Native_Tuple2.tuple2
          Prims.list)
  =
...
let (find_nearest_preceding_col_info :
  Prims.int ->
    (Prims.int,identifier_info) FStar_Pervasives_Native_Tuple2.tuple2
      Prims.list -> identifier_info FStar_Pervasives_Native.option)
  =
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___222_1182 =
        match uu___222_1182 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let info1 =
          let uu___225_1353 = info  in
          let uu____1354 = ty_map info.identifier_ty  in
          {
            identifier = (uu___225_1353.identifier);
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___226_1544 = table  in
          {
            id_info_enabled = (uu___226_1544.id_info_enabled);
            id_info_db = (uu___226_1544.id_info_db);
...
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___227_1598 = table  in
      let uu____1599 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____1599;
        id_info_db = (uu___227_1598.id_info_db);
        id_info_buffer = (uu___227_1598.id_info_buffer)
      }
