  ...
  fun table  ->
    fun fn  ->
      fun row  ->
        fun col  ->
          let rows =
            let uu____1516 = FStar_Util.pimap_empty ()  in
            FStar_Util.psmap_find_default table.id_info_db fn uu____1516  in
          let cols = FStar_Util.pimap_find_default rows row []  in
          let uu____1522 = find_nearest_preceding_col_info col cols  in
          ...
          | FStar_Pervasives_Native.Some info ->
              let last_col =
                ...
                FStar_Range.col_of_pos uu____1529  in
              if col <= last_col
            ...
            let print_gamma gamma =
              let uu____1568 =
                FStar_All.pipe_right gamma
                  (FStar_List.map
                     (fun uu___58_1578  ->
                        match uu___58_1578 with
                        | FStar_Syntax_Syntax.Binding_var x ->
                            let uu____1580 =
                              FStar_Syntax_Print.bv_to_string x  in
                            Prims.strcat "Binding_var " uu____1580
                        | FStar_Syntax_Syntax.Binding_univ u ->
                            Prims.strcat "Binding_univ " u.FStar_Ident.idText
                        | FStar_Syntax_Syntax.Binding_lid (l,uu____1583) ->
                            let uu____1600 = FStar_Ident.string_of_lid l  in
                            Prims.strcat "Binding_lid " uu____1600))
                 in
              FStar_All.pipe_right uu____1568 (FStar_String.concat "::\n")
               in
            let fail1 uu____1608 =
              let uu____1609 =
                let uu____1610 = FStar_Range.string_of_range r  in
                let uu____1611 = print_gamma g  in
                let uu____1612 = FStar_Syntax_Print.binders_to_string ", " bs
                   in
                FStar_Util.format5
                  "Invariant violation: gamma and binders are out of sync\n\treason=%s, range=%s, should_check=%s\n\t\n                               gamma=%s\n\tbinders=%s\n"
                  reason uu____1610
                  (if should_check then "true" else "false") uu____1611
                  uu____1612
                 in
              failwith uu____1609  in
              ...
              (let uu____1615 =
                 let uu____1638 =
                   FStar_Util.prefix_until
                     (fun uu___59_1653  ->
                        match uu___59_1653 with
                        | FStar_Syntax_Syntax.Binding_var uu____1654 -> true
                        | uu____1655 -> false) g
                    in
                 (uu____1638, bs)  in
               ...
               | (FStar_Pervasives_Native.Some
                  (uu____1706,hd1,gamma_tail),uu____1709::uu____1710) ->
                   let uu____1761 = FStar_Util.prefix bs  in
                   (match uu____1761 with
                    | (uu____1780,(x,uu____1782)) ->
                        (match hd1 with
