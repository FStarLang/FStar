...
let insert_identifier_info:
  ...
  fun id  ->
    fun ty  ->
      fun range  ->
        let use_range =
          let uu___103_1093 = range in
          {
            FStar_Range.def_range = (range.FStar_Range.use_range);
            FStar_Range.use_range = (uu___103_1093.FStar_Range.use_range)
          } in
        ...
        | (row,col) ->
            ((let uu____1113 = FStar_Util.smap_try_find file_info_table fn in
              ...
              | FStar_Pervasives_Native.Some file_rows ->
                  let uu____1173 = FStar_Util.imap_try_find file_rows row in
                  (match uu____1173 with
                   | FStar_Pervasives_Native.None  ->
                       let col_info =
                         let uu____1191 = insert_col_info col info [] in
                         FStar_Util.mk_ref uu____1191 in
                       FStar_Util.imap_add file_rows row col_info
                   | FStar_Pervasives_Native.Some col_infos ->
                       let uu____1217 =
                         let uu____1218 = FStar_ST.read col_infos in
                         insert_col_info col info uu____1218 in
                       FStar_ST.write col_infos uu____1217));
             (fn, row, col))
let info_at_pos:
  Prims.string ->
  ...
  fun fn  ->
    fun row  ->
      fun col  ->
        let uu____1245 = FStar_Util.smap_try_find file_info_table fn in
        match uu____1245 with
        | FStar_Pervasives_Native.None  -> FStar_Pervasives_Native.None
        | FStar_Pervasives_Native.Some rows ->
            let uu____1251 = FStar_Util.imap_try_find rows row in
            (match uu____1251 with
             | FStar_Pervasives_Native.None  -> FStar_Pervasives_Native.None
             | FStar_Pervasives_Native.Some cols ->
                 let uu____1267 =
                   let uu____1270 = FStar_ST.read cols in
                   find_nearest_preceding_col_info col uu____1270 in
                 (match uu____1267 with
                  ...
                  | FStar_Pervasives_Native.Some ci ->
                      let last_col =
                        ...
                        FStar_Range.col_of_pos uu____1283 in
                      if col <= last_col
...
let insert_id_info: insert_id_info_ops =
  ...
  let enable b =
    let uu____1504 = (FStar_Options.ide ()) && b in
    FStar_ST.write enabled uu____1504 in
  let bv x t =
    let uu____1514 = FStar_ST.read enabled in
    if uu____1514
    then
      let uu____1517 =
        let uu____1530 =
          let uu____1541 = FStar_Syntax_Syntax.range_of_bv x in
          ((FStar_Util.Inl x), t, uu____1541) in
        let uu____1546 = FStar_ST.read id_info_buffer in uu____1530 ::
          uu____1546 in
      FStar_ST.write id_info_buffer uu____1517
    else () in
  let fv x t =
    let uu____1605 = FStar_ST.read enabled in
    if uu____1605
    then
      let uu____1608 =
        let uu____1621 =
          let uu____1632 = FStar_Syntax_Syntax.range_of_fv x in
          ((FStar_Util.Inr x), t, uu____1632) in
        let uu____1637 = FStar_ST.read id_info_buffer in uu____1621 ::
          uu____1637 in
      FStar_ST.write id_info_buffer uu____1608
    else () in
  let promote cb =
    (let uu____1699 = FStar_ST.read id_info_buffer in
     FStar_All.pipe_right uu____1699
       (FStar_List.iter
          (fun uu____1753  ->
             ...
             | (i,t,r) ->
                 let uu____1775 =
                   let uu____1782 = cb t in
                   insert_identifier_info i uu____1782 r in
                 FStar_All.pipe_left FStar_Pervasives.ignore uu____1775)));
