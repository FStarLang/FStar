...
let rec decr_delta_depth:
  ...
  fun uu___96_433  ->
    ...
    | FStar_Syntax_Syntax.Delta_defined_at_level l when
        l = (Prims.parse_int "1") -> Some FStar_Syntax_Syntax.Delta_constant
...
let rec insert_col_info col info col_infos =
  ...
  | (c,i)::rest ->
      ...
      then (col, info) :: col_infos
        ...
        (let uu____506 = insert_col_info col info rest in (c, i) :: uu____506)
let find_nearest_preceding_col_info col col_infos =
  let rec aux out uu___97_542 =
    match uu___97_542 with
    | [] -> out
  ...
  fun id  ->
    fun ty  ->
      fun range  ->
        ...
        let use_range =
          let uu___98_592 = range in
          {
            FStar_Range.def_range = (range.FStar_Range.use_range);
            FStar_Range.use_range = (uu___98_592.FStar_Range.use_range)
          } in
        let fn = FStar_Range.file_of_range use_range in
        let start = FStar_Range.start_of_range use_range in
        let uu____595 =
          let uu____598 = FStar_Range.line_of_pos start in
          let uu____599 = FStar_Range.col_of_pos start in
          (uu____598, uu____599) in
        match uu____595 with
        | (row,col) ->
            let uu____602 = FStar_Util.smap_try_find file_info_table fn in
            (match uu____602 with
             | None  ->
                 let col_info =
                   let uu____609 = insert_col_info col info [] in
                   FStar_Util.mk_ref uu____609 in
             ...
             | Some file_rows ->
                 let uu____637 = FStar_Util.imap_try_find file_rows row in
                 (match uu____637 with
                  | None  ->
                      let col_info =
                        let uu____647 = insert_col_info col info [] in
                        FStar_Util.mk_ref uu____647 in
                      FStar_Util.imap_add file_rows row col_info
                  | Some col_infos ->
                      let uu____663 =
                        let uu____664 = FStar_ST.read col_infos in
                        insert_col_info col info uu____664 in
                      FStar_ST.write col_infos uu____663))
let info_at_pos:
  Prims.string -> Prims.int -> Prims.int -> identifier_info option =
  fun fn  ->
    fun row  ->
      fun col  ->
        let uu____683 = FStar_Util.smap_try_find file_info_table fn in
        match uu____683 with
        | None  -> None
        | Some rows ->
            let uu____687 = FStar_Util.imap_try_find rows row in
            (match uu____687 with
             | None  -> None
             | Some cols ->
                 let uu____696 =
                   let uu____698 = FStar_ST.read cols in
                   find_nearest_preceding_col_info col uu____698 in
                 (match uu____696 with | None  -> None | Some ci -> Some ci))
let insert_bv:
  FStar_Syntax_Syntax.bv -> FStar_Syntax_Syntax.typ -> Prims.unit =
  fun bv  ->
    fun ty  ->
      let uu____712 = FStar_Syntax_Syntax.range_of_bv bv in
      insert_identifier_info (FStar_Util.Inl bv) ty uu____712
let insert_fv:
  FStar_Syntax_Syntax.fv -> FStar_Syntax_Syntax.typ -> Prims.unit =
  fun fv  ->
    fun ty  ->
      let uu____719 = FStar_Syntax_Syntax.range_of_fv fv in
      insert_identifier_info (FStar_Util.Inr fv) ty uu____719