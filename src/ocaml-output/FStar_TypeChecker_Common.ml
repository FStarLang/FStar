...
let as_tprob : prob  ->  FStar_Syntax_Syntax.typ problem = (fun uu___0_602 -> (match (uu___0_602) with
| TProb (p) -> begin
p
end
| uu____608 -> begin
...
let uu____672 = (FStar_Syntax_Syntax.tabbrev FStar_Parser_Const.by_tactic_lid)
in (FStar_Syntax_Syntax.mk_Tm_uinst uu____672 ((FStar_Syntax_Syntax.U_zero)::[])))
in (
...
let uu____679 = (FStar_Syntax_Syntax.as_arg tac)
in (

let uu____688 = (

let uu____699 = (FStar_Syntax_Syntax.as_arg f)
in (uu____699)::[])
in (uu____679)::uu____688))
in (FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____678))
in (uu____673 FStar_Pervasives_Native.None FStar_Range.dummyRange))))
...
| (FStar_Syntax_Syntax.Delta_abstract (d), uu____754) -> begin
(delta_depth_greater_than d m)
end
| (uu____755, FStar_Syntax_Syntax.Delta_abstract (d)) -> begin
(delta_depth_greater_than l d)
end
| (FStar_Syntax_Syntax.Delta_equational_at_level (uu____757), uu____758) -> begin
true
end
| (uu____761, FStar_Syntax_Syntax.Delta_equational_at_level (uu____762)) -> begin
false
end))


let rec decr_delta_depth : FStar_Syntax_Syntax.delta_depth  ->  FStar_Syntax_Syntax.delta_depth FStar_Pervasives_Native.option = (fun uu___1_772 -> (match (uu___1_772) with
| FStar_Syntax_Syntax.Delta_constant_at_level (_775) when (_775 = (Prims.parse_int "0")) -> begin
FStar_Pervasives_Native.None
end
| FStar_Syntax_Syntax.Delta_equational_at_level (_776) when (_776 = (Prims.parse_int "0")) -> begin
FStar_Pervasives_Native.None
end
...
let uu____1058 = (__insert [] col_infos)
...
let rec aux = (fun out uu___2_1179 -> (match (uu___2_1179) with
| [] -> begin
out
end
| ((c, i))::rest -> begin
...
let uu____1290 = (FStar_Util.psmap_empty ())
in {id_info_enabled = false; id_info_db = uu____1290; id_info_buffer = []})
...
let uu____1348 = (FStar_Range.use_range range)
in (FStar_Range.set_def_range range uu____1348))
in (

let info1 = (

let uu___143_1350 = info
in (

let uu____1351 = (ty_map info.identifier_ty)
in {identifier = uu___143_1350.identifier; identifier_ty = uu____1351; identifier_range = use_range1}))
in (
...
let uu____1362 = (FStar_Range.line_of_pos start)
in (

let uu____1364 = (FStar_Range.col_of_pos start)
in ((uu____1362), (uu____1364))))
...
let uu____1395 = (FStar_Util.pimap_empty ())
in (FStar_Util.psmap_find_default db fn uu____1395))
in (
...
let uu____1451 = (insert_col_info col info1 cols)
in (FStar_All.pipe_right uu____1451 (FStar_Util.pimap_add rows row)))
in (FStar_All.pipe_right uu____1441 (FStar_Util.psmap_add db fn)))))
end))))))))
...
let uu___158_1541 = table
in {id_info_enabled = uu___158_1541.id_info_enabled; id_info_db = uu___158_1541.id_info_db; id_info_buffer = (info)::table.id_info_buffer})))
...
let uu____1559 = (FStar_Syntax_Syntax.range_of_bv bv)
in (id_info_insert table (FStar_Util.Inl (bv)) ty uu____1559))
end
| uu____1560 -> begin
table
end))
...
let uu____1579 = (FStar_Syntax_Syntax.range_of_fv fv)
in (id_info_insert table (FStar_Util.Inr (fv)) ty uu____1579))
end
| uu____1580 -> begin
table
end))
...
let uu___170_1595 = table
in {id_info_enabled = enabled; id_info_db = uu___170_1595.id_info_db; id_info_buffer = uu___170_1595.id_info_buffer}))


let id_info_promote : id_info_table  ->  (FStar_Syntax_Syntax.typ  ->  FStar_Syntax_Syntax.typ)  ->  id_info_table = (fun table ty_map -> (

let uu___174_1612 = table
in (

let uu____1613 = (FStar_List.fold_left (id_info__insert ty_map) table.id_info_db table.id_info_buffer)
in {id_info_enabled = uu___174_1612.id_info_enabled; id_info_db = uu____1613; id_info_buffer = []})))
...
let uu____1657 = (FStar_Util.pimap_empty ())
in (FStar_Util.psmap_find_default table.id_info_db fn uu____1657))
in (

let cols = (FStar_Util.pimap_find_default rows row [])
in (

let uu____1664 = (find_nearest_preceding_col_info col cols)
...
let uu____1672 = (FStar_Range.end_of_range info.identifier_range)
in (FStar_Range.col_of_pos uu____1672))
...
let uu____1719 = (FStar_All.pipe_right gamma (FStar_List.map (fun uu___3_1732 -> (match (uu___3_1732) with
| FStar_Syntax_Syntax.Binding_var (x) -> begin
(

let uu____1735 = (FStar_Syntax_Print.bv_to_string x)
in (Prims.op_Hat "Binding_var " uu____1735))
end
| FStar_Syntax_Syntax.Binding_univ (u) -> begin
(Prims.op_Hat "Binding_univ " u.FStar_Ident.idText)
end
| FStar_Syntax_Syntax.Binding_lid (l, uu____1741) -> begin
(

let uu____1758 = (FStar_Ident.string_of_lid l)
in (Prims.op_Hat "Binding_lid " uu____1758))
end))))
in (FStar_All.pipe_right uu____1719 (FStar_String.concat "::\n"))))
in (

let fail1 = (fun uu____1771 -> (

let uu____1772 = (

let uu____1774 = (FStar_Range.string_of_range r)
in (

let uu____1776 = (print_gamma g)
in (

let uu____1778 = (FStar_Syntax_Print.binders_to_string ", " bs)
in (FStar_Util.format5 "Invariant violation: gamma and binders are out of sync\n\treason=%s, range=%s, should_check=%s\n\t\n                               gamma=%s\n\tbinders=%s\n" reason uu____1774 (match (should_check) with
| true -> begin
"true"
end
| uu____1785 -> begin
"false"
end) uu____1776 uu____1778))))
...
let uu____1816 = (FStar_Util.prefix_until (fun uu___4_1831 -> (match (uu___4_1831) with
| FStar_Syntax_Syntax.Binding_var (uu____1833) -> begin
true
end
| uu____1835 -> begin
false
...
| (FStar_Pervasives_Native.Some (uu____1893, hd1, gamma_tail), (uu____1896)::uu____1897) -> begin
(

let uu____1956 = (FStar_Util.prefix bs)
in (match (uu____1956) with
| (uu____1981, (x, uu____1983)) -> begin
(match (hd1) with
...
type guard_t =
{guard_f : guard_formula; deferred : deferred; univ_ineqs : (FStar_Syntax_Syntax.universe Prims.list * univ_ineq Prims.list); implicits : implicits}


let __proj__Mkguard_t__item__guard_f : guard_t  ->  guard_formula = (fun projectee -> (match (projectee) with
| {guard_f = guard_f; deferred = deferred; univ_ineqs = univ_ineqs; implicits = implicits} -> begin
guard_f
end))


let __proj__Mkguard_t__item__deferred : guard_t  ->  deferred = (fun projectee -> (match (projectee) with
| {guard_f = guard_f; deferred = deferred; univ_ineqs = univ_ineqs; implicits = implicits} -> begin
deferred
end))


let __proj__Mkguard_t__item__univ_ineqs : guard_t  ->  (FStar_Syntax_Syntax.universe Prims.list * univ_ineq Prims.list) = (fun projectee -> (match (projectee) with
| {guard_f = guard_f; deferred = deferred; univ_ineqs = univ_ineqs; implicits = implicits} -> begin
univ_ineqs
end))


let __proj__Mkguard_t__item__implicits : guard_t  ->  implicits = (fun projectee -> (match (projectee) with
| {guard_f = guard_f; deferred = deferred; univ_ineqs = univ_ineqs; implicits = implicits} -> begin
implicits
end))


let trivial_guard : guard_t = {guard_f = Trivial; deferred = []; univ_ineqs = (([]), ([])); implicits = []}
...
let uu____2247 = (FStar_Syntax_Util.mk_conj f1 f2)
in NonTrivial (uu____2247))
end))


let check_trivial : FStar_Syntax_Syntax.term  ->  guard_formula = (fun t -> (

let uu____2254 = (

let uu____2255 = (FStar_Syntax_Util.unmeta t)
in uu____2255.FStar_Syntax_Syntax.n)
...
type lcomp =
{eff_name : FStar_Ident.lident; res_typ : FStar_Syntax_Syntax.typ; cflags : FStar_Syntax_Syntax.cflag Prims.list; comp_thunk : (unit  ->  (FStar_Syntax_Syntax.comp * guard_t), FStar_Syntax_Syntax.comp) FStar_Util.either FStar_ST.ref}
...
let __proj__Mklcomp__item__res_typ : lcomp  ->  FStar_Syntax_Syntax.typ = (fun projectee -> (match (projectee) with
| {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = comp_thunk} -> begin
res_typ
end))


let __proj__Mklcomp__item__cflags : lcomp  ->  FStar_Syntax_Syntax.cflag Prims.list = (fun projectee -> (match (projectee) with
| {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = comp_thunk} -> begin
cflags
end))


let __proj__Mklcomp__item__comp_thunk : lcomp  ->  (unit  ->  (FStar_Syntax_Syntax.comp * guard_t), FStar_Syntax_Syntax.comp) FStar_Util.either FStar_ST.ref = (fun projectee -> (match (projectee) with
| {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = comp_thunk} -> begin
comp_thunk
end))


let mk_lcomp : FStar_Ident.lident  ->  FStar_Syntax_Syntax.typ  ->  FStar_Syntax_Syntax.cflag Prims.list  ->  (unit  ->  (FStar_Syntax_Syntax.comp * guard_t))  ->  lcomp = (fun eff_name res_typ cflags comp_thunk -> (

let uu____2582 = (FStar_Util.mk_ref (FStar_Util.Inl (comp_thunk)))
in {eff_name = eff_name; res_typ = res_typ; cflags = cflags; comp_thunk = uu____2582}))


let lcomp_comp : lcomp  ->  (FStar_Syntax_Syntax.comp * guard_t) = (fun lc -> (

let uu____2624 = (FStar_ST.op_Bang lc.comp_thunk)
in (match (uu____2624) with
| FStar_Util.Inl (thunk1) -> begin
(

let uu____2696 = (thunk1 ())
...
let apply_lcomp : (FStar_Syntax_Syntax.comp  ->  FStar_Syntax_Syntax.comp)  ->  (guard_t  ->  guard_t)  ->  lcomp  ->  lcomp = (fun fc fg lc -> (mk_lcomp lc.eff_name lc.res_typ lc.cflags (fun uu____2796 -> (

let uu____2797 = (lcomp_comp lc)
...
let lcomp_to_string : lcomp  ->  Prims.string = (fun lc -> (

let uu____2817 = (FStar_Options.print_effect_args ())
...
let uu____2822 = (FStar_All.pipe_right lc lcomp_comp)
in (FStar_All.pipe_right uu____2822 FStar_Pervasives_Native.fst))
in (FStar_Syntax_Print.comp_to_string uu____2821))
end
| uu____2835 -> begin
(

let uu____2837 = (FStar_Syntax_Print.lid_to_string lc.eff_name)
in (

let uu____2839 = (FStar_Syntax_Print.term_to_string lc.res_typ)
in (FStar_Util.format2 "%s %s" uu____2837 uu____2839)))
end)))


let lcomp_set_flags : lcomp  ->  FStar_Syntax_Syntax.cflag Prims.list  ->  lcomp = (fun lc fs -> (

let comp_typ_set_flags = (fun c -> (match (c.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Total (uu____2867) -> begin
c
end
| FStar_Syntax_Syntax.GTotal (uu____2876) -> begin
c
end
...
let uu___342_2887 = ct
in {FStar_Syntax_Syntax.comp_univs = uu___342_2887.FStar_Syntax_Syntax.comp_univs; FStar_Syntax_Syntax.effect_name = uu___342_2887.FStar_Syntax_Syntax.effect_name; FStar_Syntax_Syntax.result_typ = uu___342_2887.FStar_Syntax_Syntax.result_typ; FStar_Syntax_Syntax.effect_args = uu___342_2887.FStar_Syntax_Syntax.effect_args; FStar_Syntax_Syntax.flags = fs})
in (

let uu___345_2888 = c
in {FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Comp (ct1); FStar_Syntax_Syntax.pos = uu___345_2888.FStar_Syntax_Syntax.pos; FStar_Syntax_Syntax.vars = uu___345_2888.FStar_Syntax_Syntax.vars}))
end))
in (mk_lcomp lc.eff_name lc.res_typ fs (fun uu____2891 -> (

let uu____2892 = (FStar_All.pipe_right lc lcomp_comp)
in (FStar_All.pipe_right uu____2892 (fun uu____2914 -> (match (uu____2914) with
| (c, g) -> begin
(((comp_typ_set_flags c)), (g))
end))))))))


let is_total_lcomp : lcomp  ->  Prims.bool = (fun c -> ((FStar_Ident.lid_equals c.eff_name FStar_Parser_Const.effect_Tot_lid) || (FStar_All.pipe_right c.cflags (FStar_Util.for_some (fun uu___5_2940 -> (match (uu___5_2940) with
...
let is_tot_or_gtot_lcomp : lcomp  ->  Prims.bool = (fun c -> (((FStar_Ident.lid_equals c.eff_name FStar_Parser_Const.effect_Tot_lid) || (FStar_Ident.lid_equals c.eff_name FStar_Parser_Const.effect_GTot_lid)) || (FStar_All.pipe_right c.cflags (FStar_Util.for_some (fun uu___6_2957 -> (match (uu___6_2957) with
| FStar_Syntax_Syntax.TOTAL -> begin
true
end
| FStar_Syntax_Syntax.RETURN -> begin
...
let is_lcomp_partial_return : lcomp  ->  Prims.bool = (fun c -> (FStar_All.pipe_right c.cflags (FStar_Util.for_some (fun uu___7_2974 -> (match (uu___7_2974) with
| FStar_Syntax_Syntax.RETURN -> begin
true
end
...
let is_pure_lcomp : lcomp  ->  Prims.bool = (fun lc -> (((is_total_lcomp lc) || (FStar_Syntax_Util.is_pure_effect lc.eff_name)) || (FStar_All.pipe_right lc.cflags (FStar_Util.for_some (fun uu___8_2991 -> (match (uu___8_2991) with
...
let is_pure_or_ghost_lcomp : lcomp  ->  Prims.bool = (fun lc -> ((is_pure_lcomp lc) || (FStar_Syntax_Util.is_ghost_effect lc.eff_name)))


let set_result_typ_lc : lcomp  ->  FStar_Syntax_Syntax.typ  ->  lcomp = (fun lc t -> (mk_lcomp lc.eff_name t lc.cflags (fun uu____3016 -> (

let uu____3017 = (FStar_All.pipe_right lc lcomp_comp)
in (FStar_All.pipe_right uu____3017 (fun uu____3044 -> (match (uu____3044) with
| (c, g) -> begin
(

let uu____3061 = (FStar_Syntax_Util.set_result_typ c t)
in ((uu____3061), (g)))
end)))))))


let residual_comp_of_lcomp : lcomp  ->  FStar_Syntax_Syntax.residual_comp = (fun lc -> {FStar_Syntax_Syntax.residual_effect = lc.eff_name; FStar_Syntax_Syntax.residual_typ = FStar_Pervasives_Native.Some (lc.res_typ); FStar_Syntax_Syntax.residual_flags = lc.cflags})


let lcomp_of_comp : FStar_Syntax_Syntax.comp  ->  lcomp = (fun c0 -> (

let uu____3076 = (match (c0.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Total (uu____3089) -> begin
((FStar_Parser_Const.effect_Tot_lid), ((FStar_Syntax_Syntax.TOTAL)::[]))
end
| FStar_Syntax_Syntax.GTotal (uu____3100) -> begin
...
(mk_lcomp eff_name (FStar_Syntax_Util.comp_result c0) flags (fun uu____3121 -> ((c0), (trivial_guard))))
end)))
