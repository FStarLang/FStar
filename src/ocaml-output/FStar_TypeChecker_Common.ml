...
let (uu___is_EQ : rel -> Prims.bool) =
  fun projectee  -> match projectee with | EQ  -> true | uu____50697 -> false 
let (uu___is_SUB : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUB  -> true | uu____50708 -> false
  
let (uu___is_SUBINV : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUBINV  -> true | uu____50719 -> false
...
let (uu___is_Rigid_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_rigid  -> true | uu____50730 -> false
  
let (uu___is_Flex_rigid_eq : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid_eq  -> true | uu____50741 -> false
...
let (uu___is_Flex_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid  -> true | uu____50763 -> false
  
let (uu___is_Rigid_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_flex  -> true | uu____50774 -> false
  
let (uu___is_Flex_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_flex  -> true | uu____50785 -> false
...
let (uu___is_TProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | TProb _0 -> true | uu____51213 -> false
  
let (__proj__TProb__item___0 : prob -> FStar_Syntax_Syntax.typ problem) =
  fun projectee  -> match projectee with | TProb _0 -> _0 
let (uu___is_CProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | CProb _0 -> true | uu____51240 -> false
...
let (as_tprob : prob -> FStar_Syntax_Syntax.typ problem) =
  fun uu___429_51262  ->
    match uu___429_51262 with
    | TProb p -> p
    | uu____51268 -> failwith "Expected a TProb"
...
let (uu___is_Trivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | Trivial  -> true | uu____51288 -> false
  
let (uu___is_NonTrivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | NonTrivial _0 -> true | uu____51300 -> false
  ...
  fun tac  ->
    fun f  ->
      let t_by_tactic =
        ...
        FStar_Syntax_Syntax.mk_Tm_uinst uu____51332
      ...
      let uu____51333 =
        let uu____51338 =
          ...
          let uu____51348 =
            let uu____51359 = FStar_Syntax_Syntax.as_arg tac  in
            let uu____51368 =
              let uu____51379 = FStar_Syntax_Syntax.as_arg f  in
              [uu____51379]  in
            uu____51359 :: uu____51368  in
          uu____51339 :: uu____51348  in
        FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____51338  in
      uu____51333 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let rec (delta_depth_greater_than :
  ...
  fun l  ->
    fun m  ->
      ...
      | (FStar_Syntax_Syntax.Delta_abstract d,uu____51442) ->
          delta_depth_greater_than d m
      | (uu____51443,FStar_Syntax_Syntax.Delta_abstract d) ->
          delta_depth_greater_than l d
      | (FStar_Syntax_Syntax.Delta_equational_at_level
         uu____51445,uu____51446) -> true
      | (uu____51449,FStar_Syntax_Syntax.Delta_equational_at_level
         uu____51450) -> false
...
let rec (decr_delta_depth :
  ...
  =
  fun uu___430_51460  ->
    match uu___430_51460 with
    | FStar_Syntax_Syntax.Delta_constant_at_level _51463 when
        _51463 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
    | FStar_Syntax_Syntax.Delta_equational_at_level _51464 when
        _51464 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
      ...
      fun col_infos  ->
        let rec __insert aux rest =
          ...
          | (c,i)::rest' ->
           ...
           in
        let uu____51746 = __insert [] col_infos  in
        match uu____51746 with
...
let (find_nearest_preceding_col_info :
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___431_51867 =
        match uu___431_51867 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
...
let (id_info_table_empty : id_info_table) =
  let uu____51978 = FStar_Util.psmap_empty ()  in
  { id_info_enabled = false; id_info_db = uu____51978; id_info_buffer = [] } 
let (id_info__insert :
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let use_range1 =
          let uu____52036 = FStar_Range.use_range range  in
          FStar_Range.set_def_range range uu____52036  in
        let info1 =
          let uu___568_52038 = info  in
          let uu____52039 = ty_map info.identifier_ty  in
          {
            identifier = (uu___568_52038.identifier);
            identifier_ty = uu____52039;
        ...
        let uu____52043 =
          let uu____52050 = FStar_Range.line_of_pos start  in
          let uu____52052 = FStar_Range.col_of_pos start  in
          (uu____52050, uu____52052)  in
        match uu____52043 with
        | (row,col) ->
            let rows =
              let uu____52083 = FStar_Util.pimap_empty ()  in
              FStar_Util.psmap_find_default db fn uu____52083  in
            let cols = FStar_Util.pimap_find_default rows row []  in
            let uu____52129 =
              let uu____52139 = insert_col_info col info1 cols  in
              FStar_All.pipe_right uu____52139
                (FStar_Util.pimap_add rows row)
               in
            FStar_All.pipe_right uu____52129 (FStar_Util.psmap_add db fn)
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___583_52229 = table  in
          {
            id_info_enabled = (uu___583_52229.id_info_enabled);
            id_info_db = (uu___583_52229.id_info_db);
  ...
  fun table  ->
    fun bv  ->
      fun ty  ->
        ...
        then
          let uu____52247 = FStar_Syntax_Syntax.range_of_bv bv  in
          id_info_insert table (FStar_Util.Inl bv) ty uu____52247
        else table
  ...
  fun table  ->
    fun fv  ->
      fun ty  ->
        if table.id_info_enabled
        then
          let uu____52267 = FStar_Syntax_Syntax.range_of_fv fv  in
          id_info_insert table (FStar_Util.Inr fv) ty uu____52267
        else table
  
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___595_52283 = table  in
      let uu____52284 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____52284;
        id_info_db = (uu___595_52283.id_info_db);
        id_info_buffer = (uu___595_52283.id_info_buffer)
      }
...
let (id_info_promote :
  ...
  fun table  ->
    fun ty_map  ->
      let uu___599_52302 = table  in
      let uu____52303 =
        FStar_List.fold_left (id_info__insert ty_map) table.id_info_db
          table.id_info_buffer
         in
      {
        id_info_enabled = (uu___599_52302.id_info_enabled);
        id_info_db = uu____52303;
        id_info_buffer = []
      }
  ...
  fun table  ->
    fun fn  ->
      fun row  ->
        fun col  ->
          let rows =
            let uu____52347 = FStar_Util.pimap_empty ()  in
            FStar_Util.psmap_find_default table.id_info_db fn uu____52347  in
          let cols = FStar_Util.pimap_find_default rows row []  in
          let uu____52354 = find_nearest_preceding_col_info col cols  in
          ...
          | FStar_Pervasives_Native.Some info ->
              let last_col =
                ...
                FStar_Range.col_of_pos uu____52362  in
              if col <= last_col
            ...
            let print_gamma gamma =
              let uu____52409 =
                FStar_All.pipe_right gamma
                  (FStar_List.map
                     (fun uu___432_52422  ->
                        match uu___432_52422 with
                        | FStar_Syntax_Syntax.Binding_var x ->
                            let uu____52425 =
                              FStar_Syntax_Print.bv_to_string x  in
                            Prims.op_Hat "Binding_var " uu____52425
                        | FStar_Syntax_Syntax.Binding_univ u ->
                            Prims.op_Hat "Binding_univ " u.FStar_Ident.idText
                        | FStar_Syntax_Syntax.Binding_lid (l,uu____52431) ->
                            let uu____52448 = FStar_Ident.string_of_lid l  in
                            Prims.op_Hat "Binding_lid " uu____52448))
                 in
              FStar_All.pipe_right uu____52409 (FStar_String.concat "::\n")
               in
            let fail1 uu____52461 =
              let uu____52462 =
                let uu____52464 = FStar_Range.string_of_range r  in
                let uu____52466 = print_gamma g  in
                let uu____52468 =
                  FStar_Syntax_Print.binders_to_string ", " bs  in
                FStar_Util.format5
                  "Invariant violation: gamma and binders are out of sync\n\treason=%s, range=%s, should_check=%s\n\t\n                               gamma=%s\n\tbinders=%s\n"
                  reason uu____52464
                  (if should_check then "true" else "false") uu____52466
                  uu____52468
                 in
              failwith uu____52462  in
              ...
              (let uu____52481 =
                 let uu____52506 =
                   FStar_Util.prefix_until
                     (fun uu___433_52521  ->
                        match uu___433_52521 with
                        | FStar_Syntax_Syntax.Binding_var uu____52523 -> true
                        | uu____52525 -> false) g
                    in
                 (uu____52506, bs)  in
               match uu____52481 with
               | (FStar_Pervasives_Native.None ,[]) -> ()
               | (FStar_Pervasives_Native.Some
                  (uu____52583,hd1,gamma_tail),uu____52586::uu____52587) ->
                   let uu____52646 = FStar_Util.prefix bs  in
                   (match uu____52646 with
                    | (uu____52671,(x,uu____52673)) ->
                        (match hd1 with
