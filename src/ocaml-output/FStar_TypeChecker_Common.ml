...
let (uu___is_EQ : rel -> Prims.bool) =
  fun projectee  -> match projectee with | EQ  -> true | uu____55324 -> false 
let (uu___is_SUB : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUB  -> true | uu____55335 -> false
  
let (uu___is_SUBINV : rel -> Prims.bool) =
  fun projectee  ->
    match projectee with | SUBINV  -> true | uu____55346 -> false
...
let (uu___is_Rigid_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_rigid  -> true | uu____55357 -> false
  
let (uu___is_Flex_rigid_eq : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid_eq  -> true | uu____55368 -> false
...
let (uu___is_Flex_rigid : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_rigid  -> true | uu____55390 -> false
  
let (uu___is_Rigid_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Rigid_flex  -> true | uu____55401 -> false
  
let (uu___is_Flex_flex : rank_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Flex_flex  -> true | uu____55412 -> false
...
let (uu___is_TProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | TProb _0 -> true | uu____55840 -> false
  
let (__proj__TProb__item___0 : prob -> FStar_Syntax_Syntax.typ problem) =
  fun projectee  -> match projectee with | TProb _0 -> _0 
let (uu___is_CProb : prob -> Prims.bool) =
  fun projectee  ->
    match projectee with | CProb _0 -> true | uu____55868 -> false
...
let (as_tprob : prob -> FStar_Syntax_Syntax.typ problem) =
  fun uu___429_55891  ->
    match uu___429_55891 with
    | TProb p -> p
    | uu____55897 -> failwith "Expected a TProb"
...
let (uu___is_Trivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | Trivial  -> true | uu____55917 -> false
  
let (uu___is_NonTrivial : guard_formula -> Prims.bool) =
  fun projectee  ->
    match projectee with | NonTrivial _0 -> true | uu____55929 -> false
  ...
  fun tac  ->
    fun f  ->
      let t_by_tactic =
        ...
        FStar_Syntax_Syntax.mk_Tm_uinst uu____55962
      ...
      let uu____55963 =
        let uu____55968 =
          ...
          let uu____55978 =
            let uu____55989 = FStar_Syntax_Syntax.as_arg tac  in
            let uu____55998 =
              let uu____56009 = FStar_Syntax_Syntax.as_arg f  in
              [uu____56009]  in
            uu____55989 :: uu____55998  in
          uu____55969 :: uu____55978  in
        FStar_Syntax_Syntax.mk_Tm_app t_by_tactic uu____55968  in
      uu____55963 FStar_Pervasives_Native.None FStar_Range.dummyRange
...
let rec (delta_depth_greater_than :
  ...
  fun l  ->
    fun m  ->
      ...
      | (FStar_Syntax_Syntax.Delta_abstract d,uu____56074) ->
          delta_depth_greater_than d m
      | (uu____56075,FStar_Syntax_Syntax.Delta_abstract d) ->
          delta_depth_greater_than l d
      | (FStar_Syntax_Syntax.Delta_equational_at_level
         uu____56077,uu____56078) -> true
      | (uu____56081,FStar_Syntax_Syntax.Delta_equational_at_level
         uu____56082) -> false
...
let rec (decr_delta_depth :
  ...
  =
  fun uu___430_56092  ->
    match uu___430_56092 with
    | FStar_Syntax_Syntax.Delta_constant_at_level _56095 when
        _56095 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
    | FStar_Syntax_Syntax.Delta_equational_at_level _56096 when
        _56096 = (Prims.parse_int "0") -> FStar_Pervasives_Native.None
      ...
      fun col_infos  ->
        let rec __insert aux rest =
          ...
          | (c,i)::rest' ->
           ...
           in
        let uu____56378 = __insert [] col_infos  in
        match uu____56378 with
...
let (find_nearest_preceding_col_info :
  ...
  fun col  ->
    fun col_infos  ->
      let rec aux out uu___431_56499 =
        match uu___431_56499 with
        | [] -> out
        | (c,i)::rest ->
            if c > col
            then out
...
let (id_info_table_empty : id_info_table) =
  let uu____56610 = FStar_Util.psmap_empty ()  in
  { id_info_enabled = false; id_info_db = uu____56610; id_info_buffer = [] } 
let (id_info__insert :
  ...
  fun ty_map  ->
    fun db  ->
      fun info  ->
        ...
        let use_range1 =
          let uu____56668 = FStar_Range.use_range range  in
          FStar_Range.set_def_range range uu____56668  in
        let info1 =
          let uu___568_56670 = info  in
          let uu____56671 = ty_map info.identifier_ty  in
          {
            identifier = (uu___568_56670.identifier);
            identifier_ty = uu____56671;
        ...
        let uu____56675 =
          let uu____56682 = FStar_Range.line_of_pos start  in
          let uu____56684 = FStar_Range.col_of_pos start  in
          (uu____56682, uu____56684)  in
        match uu____56675 with
        | (row,col) ->
            let rows =
              let uu____56715 = FStar_Util.pimap_empty ()  in
              FStar_Util.psmap_find_default db fn uu____56715  in
            let cols = FStar_Util.pimap_find_default rows row []  in
            let uu____56761 =
              let uu____56771 = insert_col_info col info1 cols  in
              FStar_All.pipe_right uu____56771
                (FStar_Util.pimap_add rows row)
               in
            FStar_All.pipe_right uu____56761 (FStar_Util.psmap_add db fn)
    ...
    fun id1  ->
      fun ty  ->
        fun range  ->
          ...
          let uu___583_56861 = table  in
          {
            id_info_enabled = (uu___583_56861.id_info_enabled);
            id_info_db = (uu___583_56861.id_info_db);
  ...
  fun table  ->
    fun bv  ->
      fun ty  ->
        ...
        then
          let uu____56879 = FStar_Syntax_Syntax.range_of_bv bv  in
          id_info_insert table (FStar_Util.Inl bv) ty uu____56879
        else table
  ...
  fun table  ->
    fun fv  ->
      fun ty  ->
        if table.id_info_enabled
        then
          let uu____56899 = FStar_Syntax_Syntax.range_of_fv fv  in
          id_info_insert table (FStar_Util.Inr fv) ty uu____56899
        else table
  
let (id_info_toggle : id_info_table -> Prims.bool -> id_info_table) =
  fun table  ->
    fun enabled  ->
      let uu___595_56915 = table  in
      let uu____56916 = enabled && (FStar_Options.ide ())  in
      {
        id_info_enabled = uu____56916;
        id_info_db = (uu___595_56915.id_info_db);
        id_info_buffer = (uu___595_56915.id_info_buffer)
      }
...
let (id_info_promote :
  ...
  fun table  ->
    fun ty_map  ->
      let uu___599_56934 = table  in
      let uu____56935 =
        FStar_List.fold_left (id_info__insert ty_map) table.id_info_db
          table.id_info_buffer
         in
      {
        id_info_enabled = (uu___599_56934.id_info_enabled);
        id_info_db = uu____56935;
        id_info_buffer = []
      }
  ...
  fun table  ->
    fun fn  ->
      fun row  ->
        fun col  ->
          let rows =
            let uu____56979 = FStar_Util.pimap_empty ()  in
            FStar_Util.psmap_find_default table.id_info_db fn uu____56979  in
          let cols = FStar_Util.pimap_find_default rows row []  in
          let uu____56986 = find_nearest_preceding_col_info col cols  in
          match uu____56986 with
          | FStar_Pervasives_Native.None  -> FStar_Pervasives_Native.None
          | FStar_Pervasives_Native.Some info ->
              let last_col =
                let uu____56994 =
                  FStar_Range.end_of_range info.identifier_range  in
                FStar_Range.col_of_pos uu____56994  in
              if col <= last_col
            ...
            let print_gamma gamma =
              let uu____57041 =
                FStar_All.pipe_right gamma
                  (FStar_List.map
                     (fun uu___432_57054  ->
                        match uu___432_57054 with
                        | FStar_Syntax_Syntax.Binding_var x ->
                            let uu____57057 =
                              FStar_Syntax_Print.bv_to_string x  in
                            Prims.op_Hat "Binding_var " uu____57057
                        | FStar_Syntax_Syntax.Binding_univ u ->
                            Prims.op_Hat "Binding_univ " u.FStar_Ident.idText
                        | FStar_Syntax_Syntax.Binding_lid (l,uu____57063) ->
                            let uu____57080 = FStar_Ident.string_of_lid l  in
                            Prims.op_Hat "Binding_lid " uu____57080))
                 in
              FStar_All.pipe_right uu____57041 (FStar_String.concat "::\n")
               in
            let fail1 uu____57093 =
              let uu____57094 =
                let uu____57096 = FStar_Range.string_of_range r  in
                let uu____57098 = print_gamma g  in
                let uu____57100 =
                  FStar_Syntax_Print.binders_to_string ", " bs  in
                FStar_Util.format5
                  "Invariant violation: gamma and binders are out of sync\n\treason=%s, range=%s, should_check=%s\n\t\n                               gamma=%s\n\tbinders=%s\n"
                  reason uu____57096
                  (if should_check then "true" else "false") uu____57098
                  uu____57100
                 in
              failwith uu____57094  in
              ...
              (let uu____57113 =
                 let uu____57138 =
                   FStar_Util.prefix_until
                     (fun uu___433_57153  ->
                        match uu___433_57153 with
                        | FStar_Syntax_Syntax.Binding_var uu____57155 -> true
                        | uu____57157 -> false) g
                    in
                 (uu____57138, bs)  in
               ...
               | (FStar_Pervasives_Native.Some
                  (uu____57215,hd1,gamma_tail),uu____57218::uu____57219) ->
                   let uu____57278 = FStar_Util.prefix bs  in
                   (match uu____57278 with
                    | (uu____57303,(x,uu____57305)) ->
                        (match hd1 with
