...
let _93_71 = (FStar_Util.string_builder_append s.chunk line)
in (

let _93_73 = (FStar_Util.string_builder_append s.chunk "\n")
in (read_chunk ())))
end
end
end
...
let shift_chunk : Prims.unit  ->  input_chunks = (fun _93_75 -> (match (()) with
| () -> begin
(
...
let _93_81 = (FStar_ST.op_Colon_Equals s.buffer chunks)
in chunk)
end))
end))


let fill_buffer : Prims.unit  ->  Prims.unit = (fun _93_83 -> (match (()) with
| () -> begin
(

let s = the_interactive_state
in (let _190_164 = (let _190_163 = (FStar_ST.read s.buffer)
in (let _190_162 = (let _190_161 = (read_chunk ())
in (_190_161)::[])
in (FStar_List.append _190_163 _190_162)))
in (FStar_ST.op_Colon_Equals s.buffer _190_164)))
end))
...
let find_initial_module_name : Prims.unit  ->  Prims.string Prims.option = (fun _93_87 -> (match (()) with
| () -> begin
(

let _93_88 = (fill_buffer ())
in (

let _93_90 = (fill_buffer ())
in try
(match (()) with
| () -> begin
(

let _93_114 = (match ((FStar_ST.read the_interactive_state.buffer)) with
| (Push (_93_105))::(Code (code, _93_101))::[] -> begin
(
...
let detect_dependencies_with_first_interactive_chunk : Prims.unit  ->  (Prims.string * Prims.string Prims.list) = (fun _93_116 -> (match (()) with
| () -> begin
(

let failr = (fun msg r -> (

let _93_120 = if (FStar_Options.universes ()) then begin
...
| None -> begin
(let _190_184 = (FStar_Util.format2 "I found a \"module %s\" directive, but there is no %s.fst\n" module_name module_name)
in (fail _190_184))
end
| (Some (None, Some (filename))) | (Some (Some (filename), None)) -> begin
(

let _93_154 = (FStar_Options.add_verify_module module_name)
in (

let _93_161 = (FStar_Parser_Dep.collect FStar_Parser_Dep.VerifyUserList ((filename)::[]))
in (match (_93_161) with
| (_93_157, all_filenames, _93_160) -> begin
(let _190_186 = (let _190_185 = (FStar_List.tl all_filenames)
in (FStar_List.rev _190_185))
in ((filename), (_190_186)))
end)))
end
| Some (Some (_93_163), Some (_93_166)) -> begin
(let _190_187 = (FStar_Util.format1 "The combination of split interfaces and interactive verification is not supported for: %s\n" module_name)
in (fail _190_187))
end
...
let _93_180 = if (let _190_193 = (FStar_Options.codegen ())
in (FStar_Option.isSome _190_193)) then begin
...
let _93_188 = (tc.pop env msg)
in (

let _93_200 = (match (stack) with
| [] -> begin
(

let _93_191 = (FStar_Util.print_error "too many pops")
in (FStar_All.exit (Prims.parse_int "1")))
end
| (hd)::tl -> begin
((hd), (tl))
...
let _93_214 = (tc.report_fail ())
in (

let _93_216 = (FStar_Util.print1 "%s\n" fail)
in (
...
(let _190_205 = (FStar_Option.get filename)
in (FStar_SMTEncoding_Solver.with_hints_db _190_205 (fun _93_231 -> (match (()) with
| () -> begin
(go [] initial_mod env)
end))))
end else begin
