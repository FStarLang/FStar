...
let uu____2386 = (match (maybe_intf) with
| Some (intf) -> begin
(

let frag = (

let uu____2399 = (FStar_Util.file_get_contents intf)
in {FStar_Parser_ParseIt.frag_text = uu____2399; FStar_Parser_ParseIt.frag_line = (Prims.parse_int "0"); FStar_Parser_ParseIt.frag_col = (Prims.parse_int "0")})
in (

let uu____2400 = (check_frag env1 None frag)
...
let uu____2430 = (FStar_Util.format1 "Found the interface %s but it has errors!" intf)
in (FStar_Util.print_warning uu____2430));
(FStar_All.exit (Prims.parse_int "1"));
)
end
| uu____2431 -> begin
...
let uu____2443 = (FStar_Util.format1 "Found the interface %s but could not parse it first!" intf)
in (FStar_Util.print_warning uu____2443));
(FStar_All.exit (Prims.parse_int "1"));
)
end)))
end
...
let uu____2458 = ((FStar_Options.record_hints ()) || (FStar_Options.use_hints ()))
...
let uu____2460 = (FStar_Options.file_list ())
in (FStar_List.hd uu____2460))
in (FStar_SMTEncoding_Solver.with_hints_db uu____2459 (fun uu____2462 -> (go (((Prims.parse_int "1")), ((Prims.parse_int "0"))) filename stack initial_mod env2 ts))))
end
| uu____2463 -> begin
