...
let js_int : FStar_Util.json -> Prims.int =
  fun uu___230_1048  ->
    match uu___230_1048 with
    | FStar_Util.JsonInt i -> i
    | other -> js_fail "int" other
  
let js_str : FStar_Util.json -> Prims.string =
  fun uu___231_1053  ->
    match uu___231_1053 with
    | FStar_Util.JsonStr s -> s
    | other -> js_fail "string" other
  
let js_list k uu___232_1071 =
  match uu___232_1071 with
  | FStar_Util.JsonList l -> FStar_List.map k l
  | other -> js_fail "list" other 
let js_assoc : FStar_Util.json -> (Prims.string * FStar_Util.json) Prims.list
  =
  fun uu___233_1083  ->
    match uu___233_1083 with
  ...
  | (env,curmod)::stack ->
      (pop st.repl_env "#pop";
       ...
       ((QueryOK, FStar_Util.JsonNull),
         (FStar_Util.Inl
            ((let uu___246_2179 = st  in
              {
                repl_line = (uu___246_2179.repl_line);
                repl_column = (uu___246_2179.repl_column);
                repl_fname = (uu___246_2179.repl_fname);
                repl_stack = stack;
                repl_curmod = curmod;
                repl_env = env;
                repl_ts = (uu___246_2179.repl_ts);
                repl_stdin = (uu___246_2179.repl_stdin)
              })))))
  ...
  | (stack,env,ts) ->
      ...
      (match uu____2231 with
       | (restore_cmd_line_options1,(stack1,env1,ts1)) ->
           ...
           (FStar_Errors.clear ();
            (let st' =
               let uu___247_2307 = st  in
               {
                 repl_line = line;
                 repl_column = column1;
                 repl_fname = (uu___247_2307.repl_fname);
                 repl_stack = stack2;
                 repl_curmod = (uu___247_2307.repl_curmod);
                 repl_env = (uu___247_2307.repl_env);
                 repl_ts = ts1;
                 repl_stdin = (uu___247_2307.repl_stdin)
               }  in
             match res with
             | Some (curmod,env3,nerrs) when
                 ...
                 ((QueryOK, (FStar_Util.JsonList errors)),
                   (FStar_Util.Inl
                      (let uu___248_2336 = st'  in
                       {
                         repl_line = (uu___248_2336.repl_line);
                         repl_column = (uu___248_2336.repl_column);
                         repl_fname = (uu___248_2336.repl_fname);
                         repl_stack = (uu___248_2336.repl_stack);
                         repl_curmod = curmod;
                         repl_env = env4;
                         repl_ts = (uu___248_2336.repl_ts);
                         repl_stdin = (uu___248_2336.repl_stdin)
                       })))
             | uu____2337 ->
                 let env3 = reset_mark env_mark  in
                 let uu____2348 =
                   run_pop
                     (let uu___249_2355 = st'  in
                      {
                        repl_line = (uu___249_2355.repl_line);
                        repl_column = (uu___249_2355.repl_column);
                        repl_fname = (uu___249_2355.repl_fname);
                        repl_stack = (uu___249_2355.repl_stack);
                        repl_curmod = (uu___249_2355.repl_curmod);
                        repl_env = env3;
                        repl_ts = (uu___249_2355.repl_ts);
                        repl_stdin = (uu___249_2355.repl_stdin)
                      })
                    in
...
let run_compute st term rules =
  let run_and_rewind st1 task =
    ...
    let st' =
      let uu___250_3348 = st1  in
      {
        repl_line = (uu___250_3348.repl_line);
        repl_column = (uu___250_3348.repl_column);
        repl_fname = (uu___250_3348.repl_fname);
        repl_stack = (uu___250_3348.repl_stack);
        repl_curmod = (uu___250_3348.repl_curmod);
        repl_env = env;
        repl_ts = (uu___250_3348.repl_ts);
        repl_stdin = (uu___250_3348.repl_stdin)
      }  in
