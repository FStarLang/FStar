  ...
  | (env,curmod)::stack ->
      (pop st.repl_env "#pop";
       ...
       ((QueryOK, FStar_Util.JsonNull),
         (FStar_Util.Inl
            ((let uu___216_2207 = st in
              {
                repl_line = (uu___216_2207.repl_line);
                repl_column = (uu___216_2207.repl_column);
                repl_fname = (uu___216_2207.repl_fname);
                repl_stack = stack;
                repl_curmod = curmod;
                repl_env = env;
                repl_ts = (uu___216_2207.repl_ts);
                repl_stdin = (uu___216_2207.repl_stdin)
              })))))
  ...
  | (stack,env,ts) ->
      ...
      (match uu____2259 with
       | (restore_cmd_line_options1,(stack1,env1,ts1)) ->
           ...
           (FStar_Errors.clear ();
            (let st' =
               let uu___217_2335 = st in
               {
                 repl_line = line;
                 repl_column = column1;
                 repl_fname = (uu___217_2335.repl_fname);
                 repl_stack = stack2;
                 repl_curmod = (uu___217_2335.repl_curmod);
                 repl_env = (uu___217_2335.repl_env);
                 repl_ts = ts1;
                 repl_stdin = (uu___217_2335.repl_stdin)
               } in
             match res with
             | Some (curmod,env3,nerrs) when
                 ...
                 ((QueryOK, (FStar_Util.JsonList errors)),
                   (FStar_Util.Inl
                      (let uu___218_2364 = st' in
                       {
                         repl_line = (uu___218_2364.repl_line);
                         repl_column = (uu___218_2364.repl_column);
                         repl_fname = (uu___218_2364.repl_fname);
                         repl_stack = (uu___218_2364.repl_stack);
                         repl_curmod = curmod;
                         repl_env = env4;
                         repl_ts = (uu___218_2364.repl_ts);
                         repl_stdin = (uu___218_2364.repl_stdin)
                       })))
             | uu____2365 ->
                 let env3 = reset_mark env_mark in
                 let uu____2376 =
                   run_pop
                     (let uu___219_2383 = st' in
                      {
                        repl_line = (uu___219_2383.repl_line);
                        repl_column = (uu___219_2383.repl_column);
                        repl_fname = (uu___219_2383.repl_fname);
                        repl_stack = (uu___219_2383.repl_stack);
                        repl_curmod = (uu___219_2383.repl_curmod);
                        repl_env = env3;
                        repl_ts = (uu___219_2383.repl_ts);
                        repl_stdin = (uu___219_2383.repl_stdin)
                      }) in
...
let run_compute st term rules =
  let run_and_rewind st1 task =
    ...
    let st' =
      let uu___220_3376 = st1 in
      {
        repl_line = (uu___220_3376.repl_line);
        repl_column = (uu___220_3376.repl_column);
        repl_fname = (uu___220_3376.repl_fname);
        repl_stack = (uu___220_3376.repl_stack);
        repl_curmod = (uu___220_3376.repl_curmod);
        repl_env = env;
        repl_ts = (uu___220_3376.repl_ts);
        repl_stdin = (uu___220_3376.repl_stdin)
      } in
    ...
    fun sc  ->
      ...
      | None  ->
          let typ =
            ...
            | Some ((uu____3730,typ),uu____3732) -> typ in
...
let sc_fvars:
  ...
  fun tcenv  ->
    fun sc  ->
      let uu____3752 = FStar_ST.read sc.sc_fvars in
      match uu____3752 with
      | Some fv -> fv
      | None  ->
          let fv =
            let uu____3766 = sc_typ tcenv sc in
            FStar_Syntax_Free.fvars uu____3766 in
    ...
    fun tcenv  ->
      fun sc  ->
        let typ_str =
          ...
          FStar_Syntax_Print.term_to_string uu____3783 in
        let uu____3784 =
          let uu____3788 =
            let uu____3791 =
              let uu____3792 =
                ...
                uu____3793.FStar_Ident.str in
              FStar_Util.JsonStr uu____3792 in
            ("lid", uu____3791) in
          [uu____3788; ("type", (FStar_Util.JsonStr typ_str))] in
        FStar_Util.JsonAssoc uu____3784
exception InvalidSearch of Prims.string
let uu___is_InvalidSearch: Prims.exn -> Prims.bool =
  fun projectee  ->
    match projectee with
    | InvalidSearch uu____3805 -> true
    | uu____3806 -> false
let __proj__InvalidSearch__item__uu___: Prims.exn -> Prims.string =
  fun projectee  ->
    match projectee with | InvalidSearch uu____3813 -> uu____3813
let run_search st search_str =
  let uu____3832 = st.repl_env in
  ...
  | (dsenv,tcenv) ->
      ...
      let st_matches candidate term =
        let found =
          ...
          | TypeContainsLid lid ->
              let uu____3853 = sc_fvars tcenv candidate in
              FStar_Util.set_mem lid uu____3853 in
        ...
        let parse_one term =
          ...
          let parsed =
            ...
            then
              ...
              if beg_quote
              then
                (let uu____3884 = strip_quotes term1 in
                 NameContainsStr uu____3884)
              else
      ...
      let pprint_one term =
        ...
        Prims.strcat (if term.st_negate then "-" else "") uu____3905 in
      let results =
        try
          ...
          | [] ->
              let kwds =
                let uu____3946 = FStar_List.map pprint_one terms in
                FStar_Util.concat_l " " uu____3946 in
              let uu____3948 =
                let uu____3949 =
                  FStar_Util.format1 "No results found for query [%s]" kwds in
                InvalidSearch uu____3949 in
              raise uu____3948
          | uu____3952 -> (QueryOK, (FStar_Util.JsonList js))
...
let rec go: repl_state -> Prims.unit =
  fun st  ->
    ...
    let uu____4019 = let uu____4026 = run_query st in uu____4026 query.qq in
...
let interactive_error_handler: FStar_Errors.error_handler =
  ...
  let add_one1 e =
    let uu____4056 = let uu____4058 = FStar_ST.read issues in e :: uu____4058 in
    FStar_ST.write issues uu____4056 in
  let count_errors uu____4069 =
    let uu____4070 =
      let uu____4072 = FStar_ST.read issues in
      FStar_List.filter
        (fun e  -> e.FStar_Errors.issue_level = FStar_Errors.EError)
        uu____4072 in
    FStar_List.length uu____4070 in
  let report1 uu____4083 =
    let uu____4084 = FStar_ST.read issues in
    FStar_List.sortWith FStar_Errors.compare_issues uu____4084 in
  let clear1 uu____4092 = FStar_ST.write issues [] in
  {
...
let interactive_mode': Prims.string -> Prims.unit =
  fun filename  ->
    write_hello ();
    (let uu____4102 = deps_of_our_file filename in
     match uu____4102 with
     | (filenames,maybe_intf) ->
         let env = tc_prims () in
         let uu____4116 = tc_deps None [] env filenames [] in
         (match uu____4116 with
          | (stack,env1,ts) ->
              ...
              then
                let uu____4147 =
                  let uu____4148 = FStar_Options.file_list () in
                  FStar_List.hd uu____4148 in
                FStar_SMTEncoding_Solver.with_hints_db uu____4147
                  (fun uu____4150  -> go init_st)
              else go init_st))
let interactive_mode: Prims.string -> Prims.unit =
  fun filename  ->
