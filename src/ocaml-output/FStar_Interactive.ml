        ...
        | intf::impl::remaining1 when
            ...
            (match uu____61 with
             | (uu____76,dsenv1,env1) ->
                 (((FStar_Pervasives_Native.Some intf), impl), dsenv1, env1,
                   remaining1))
        | intf_or_impl::remaining1 ->
            ...
            (match uu____93 with
             | (uu____108,dsenv1,env1) ->
                 ((FStar_Pervasives_Native.None, intf_or_impl), dsenv1, env1,
                   remaining1))
        | [] -> failwith "Impossible" in
      (match uu____40 with
       | ((intf,impl),dsenv1,env1,remaining1) ->
           ((intf, impl), (dsenv1, env1), FStar_Pervasives_Native.None,
             remaining1))
let tc_prims:
  Prims.unit ->
                         ...
                         | FStar_Pervasives_Native.Some intf1 ->
                             let uu____586 =
                               FStar_Util.get_file_last_modification_time
                                 intf1 in
                             FStar_Pervasives_Native.Some uu____586
                         | FStar_Pervasives_Native.None  ->
                             FStar_Pervasives_Native.None in
                       let impl_t =
...
let js_int: FStar_Util.json -> Prims.int =
  fun uu___198_1048  ->
    match uu___198_1048 with
    | FStar_Util.JsonInt i -> i
    | other -> js_fail "int" other
let js_str: FStar_Util.json -> Prims.string =
  fun uu___199_1053  ->
    match uu___199_1053 with
    | FStar_Util.JsonStr s -> s
    | other -> js_fail "string" other
let js_list k uu___200_1071 =
...
let try_assoc key a =
  let uu____1358 =
    FStar_Util.try_find
      ...
      a in
  FStar_Util.map_option FStar_Pervasives_Native.snd uu____1358
  ...
  | (env,curmod)::stack ->
      (pop st.repl_env "#pop";
       ...
       ((QueryOK, FStar_Util.JsonNull),
         (FStar_Util.Inl
            ((let uu___214_2214 = st in
              {
                repl_line = (uu___214_2214.repl_line);
                repl_column = (uu___214_2214.repl_column);
                repl_fname = (uu___214_2214.repl_fname);
                repl_stack = stack;
                repl_curmod = curmod;
                repl_env = env;
                repl_ts = (uu___214_2214.repl_ts);
                repl_stdin = (uu___214_2214.repl_stdin)
              })))))
  ...
  | (stack,env,ts) ->
      ...
      (match uu____2266 with
       | (restore_cmd_line_options1,(stack1,env1,ts1)) ->
           ...
           (FStar_Errors.clear ();
            (let st' =
               let uu___215_2342 = st in
               {
                 repl_line = line;
                 repl_column = column1;
                 repl_fname = (uu___215_2342.repl_fname);
                 repl_stack = stack2;
                 repl_curmod = (uu___215_2342.repl_curmod);
                 repl_env = (uu___215_2342.repl_env);
                 repl_ts = ts1;
                 repl_stdin = (uu___215_2342.repl_stdin)
               } in
             match res with
             | FStar_Pervasives_Native.Some (curmod,env3,nerrs) when
                 ...
                 ((QueryOK, (FStar_Util.JsonList errors)),
                   (FStar_Util.Inl
                      (let uu___216_2371 = st' in
                       {
                         repl_line = (uu___216_2371.repl_line);
                         repl_column = (uu___216_2371.repl_column);
                         repl_fname = (uu___216_2371.repl_fname);
                         repl_stack = (uu___216_2371.repl_stack);
                         repl_curmod = curmod;
                         repl_env = env4;
                         repl_ts = (uu___216_2371.repl_ts);
                         repl_stdin = (uu___216_2371.repl_stdin)
                       })))
             | uu____2372 ->
                 let env3 = reset_mark env_mark in
                 let uu____2383 =
                   run_pop
                     (let uu___217_2390 = st' in
                      {
                        repl_line = (uu___217_2390.repl_line);
                        repl_column = (uu___217_2390.repl_column);
                        repl_fname = (uu___217_2390.repl_fname);
                        repl_stack = (uu___217_2390.repl_stack);
                        repl_curmod = (uu___217_2390.repl_curmod);
                        repl_env = env3;
                        repl_ts = (uu___217_2390.repl_ts);
                        repl_stdin = (uu___217_2390.repl_stdin)
                      }) in
...
let run_lookup st symbol pos_opt requested_info =
  ...
  | (dsenv,tcenv) ->
      ...
      let def_of_lid lid =
        ...
        FStar_Util.bind_opt uu____2542
          (fun uu___204_2562  ->
             ...
             | uu____2591 -> FStar_Pervasives_Native.None) in
      let info_at_pos_opt =
      ...
      let info_opt =
        ...
        | FStar_Pervasives_Native.Some uu____2642 -> info_at_pos_opt
        | FStar_Pervasives_Native.None  ->
            if symbol = ""
        ...
        | FStar_Pervasives_Native.Some (name_or_lid,typ,rng) ->
            ...
            let doc_str =
              ...
              | FStar_Util.Inr lid when
                  ...
                  docs_of_lid lid
              | uu____2704 -> FStar_Pervasives_Native.None in
            let def_str =
              match name_or_lid with
              | FStar_Util.Inr lid when
                  FStar_List.mem "definition" requested_info ->
                  def_of_lid lid
              | uu____2711 -> FStar_Pervasives_Native.None in
            let def_range =
...
let run_completions st search_term =
  ...
  | (dsenv,tcenv) ->
      let rec measure_anchored_match search_term1 candidate =
        ...
        | (uu____2780,[]) -> FStar_Pervasives_Native.None
        | (hs::ts,hc::tc) ->
...
let run_compute st term rules =
  let run_and_rewind st1 task =
    ...
    let st' =
      let uu___218_3383 = st1 in
      {
        repl_line = (uu___218_3383.repl_line);
        repl_column = (uu___218_3383.repl_column);
        repl_fname = (uu___218_3383.repl_fname);
        repl_stack = (uu___218_3383.repl_stack);
        repl_curmod = (uu___218_3383.repl_curmod);
        repl_env = env;
        repl_ts = (uu___218_3383.repl_ts);
        repl_stdin = (uu___218_3383.repl_stdin)
      } in
...
let sc_of_lid: FStar_Ident.lid -> search_candidate =
  fun lid  ->
    let uu____3689 = FStar_Util.mk_ref FStar_Pervasives_Native.None in
    let uu____3694 = FStar_Util.mk_ref FStar_Pervasives_Native.None in
    ...
    fun sc  ->
      ...
      | FStar_Pervasives_Native.None  ->
          let typ =
            ...
            | FStar_Pervasives_Native.Some ((uu____3737,typ),uu____3739) ->
                typ in
         ...
         (match uu____4123 with
          | (stack,env1,ts) ->
              ...
              let env2 =
                ...
                ((FStar_Pervasives_Native.fst env1), uu____4144) in
              let env3 =
