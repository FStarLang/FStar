...
in (let _103_7 = (let _103_6 = (let _103_5 = (FStar_Absyn_Util.args_of_non_null_binders binders)
in (u, _103_5))
in (FStar_Absyn_Syntax.mk_Kind_uvar _103_6 r))
in (_103_7, u))))

let new_tvar = (fun r binders k -> (let binders = (FStar_All.pipe_right binders (FStar_List.filter (fun x -> (let _103_15 = (FStar_Absyn_Syntax.is_null_binder x)
in (FStar_All.pipe_right _103_15 Prims.op_Negation)))))
...
in (let _103_16 = (FStar_Absyn_Syntax.mk_Typ_app (uv, args) None r)
in (_103_16, uv)))))
end))))

let new_evar = (fun r binders t -> (let binders = (FStar_All.pipe_right binders (FStar_List.filter (fun x -> (let _103_24 = (FStar_Absyn_Syntax.is_null_binder x)
in (FStar_All.pipe_right _103_24 Prims.op_Negation)))))
in (let uv = (FStar_Unionfind.fresh FStar_Absyn_Syntax.Uvar)
in (match (binders) with
| [] -> begin
...
| _36_60 -> begin
(let args = (FStar_Absyn_Util.args_of_non_null_binders binders)
in (let t' = (let _103_26 = (let _103_25 = (FStar_Absyn_Syntax.mk_Total t)
in (binders, _103_25))
in (FStar_Absyn_Syntax.mk_Typ_fun _103_26 None r))
...
| _36_66 -> begin
(let _103_27 = (FStar_Absyn_Syntax.mk_Exp_app (uv, args) None r)
in (_103_27, uv))
end))))
end))))
...
| KProb (p) -> begin
(let _103_229 = (FStar_Absyn_Print.kind_to_string p.lhs)
in (let _103_228 = (FStar_Absyn_Print.kind_to_string p.rhs)
in (FStar_Util.format3 "\t%s\n\t\t%s\n\t%s" _103_229 (rel_to_string p.relation) _103_228)))
end
| TProb (p) -> begin
(let _103_242 = (let _103_241 = (FStar_Tc_Normalize.typ_norm_to_string env p.lhs)
in (let _103_240 = (let _103_239 = (FStar_Absyn_Print.tag_of_typ p.lhs)
in (let _103_238 = (let _103_237 = (let _103_236 = (FStar_All.pipe_right p.reason FStar_List.hd)
in (let _103_235 = (let _103_234 = (FStar_Tc_Normalize.typ_norm_to_string env p.rhs)
in (let _103_233 = (let _103_232 = (FStar_Absyn_Print.tag_of_typ p.rhs)
in (let _103_231 = (let _103_230 = (FStar_Tc_Normalize.formula_norm_to_string env (Prims.fst p.logical_guard))
in (_103_230)::[])
in (_103_232)::_103_231))
in (_103_234)::_103_233))
in (_103_236)::_103_235))
in ((rel_to_string p.relation))::_103_237)
in (_103_239)::_103_238))
in (_103_241)::_103_240))
in (FStar_Util.format "\t%s (%s) \n\t\t%s(%s)\n\t%s (%s) (guard %s)" _103_242))
end
| EProb (p) -> begin
(let _103_244 = (FStar_Tc_Normalize.exp_norm_to_string env p.lhs)
in (let _103_243 = (FStar_Tc_Normalize.exp_norm_to_string env p.rhs)
in (FStar_Util.format3 "\t%s \n\t\t%s\n\t%s" _103_244 (rel_to_string p.relation) _103_243)))
end
| CProb (p) -> begin
(let _103_246 = (FStar_Tc_Normalize.comp_typ_norm_to_string env p.lhs)
in (let _103_245 = (FStar_Tc_Normalize.comp_typ_norm_to_string env p.rhs)
in (FStar_Util.format3 "\t%s \n\t\t%s\n\t%s" _103_246 (rel_to_string p.relation) _103_245)))
end))

let uvi_to_string = (fun env uvi -> (let str = (fun u -> if (FStar_ST.read FStar_Options.hide_uvar_nums) then begin
"?"
end else begin
(let _103_252 = (FStar_Unionfind.uvar_id u)
in (FStar_All.pipe_right _103_252 FStar_Util.string_of_int))
end)
in (match (uvi) with
| UK (u, _36_141) -> begin
(let _103_253 = (str u)
in (FStar_All.pipe_right _103_253 (FStar_Util.format1 "UK %s")))
end
| UT ((u, _36_146), t) -> begin
(let _103_256 = (str u)
in (FStar_All.pipe_right _103_256 (fun x -> (let _103_255 = (FStar_Tc_Normalize.typ_norm_to_string env t)
in (FStar_Util.format2 "UT %s %s" x _103_255)))))
end
| UE ((u, _36_154), _36_157) -> begin
(let _103_257 = (str u)
in (FStar_All.pipe_right _103_257 (FStar_Util.format1 "UE %s")))
end)))
...
| KProb (p) -> begin
(FStar_All.pipe_right (maybe_invert p) (fun _103_264 -> KProb (_103_264)))
end
| TProb (p) -> begin
(FStar_All.pipe_right (maybe_invert p) (fun _103_265 -> TProb (_103_265)))
end
| EProb (p) -> begin
(FStar_All.pipe_right (maybe_invert p) (fun _103_266 -> EProb (_103_266)))
end
| CProb (p) -> begin
(FStar_All.pipe_right (maybe_invert p) (fun _103_267 -> CProb (_103_267)))
end))
...
| KProb (p) -> begin
(FStar_All.pipe_left (fun _103_286 -> KProb (_103_286)) (invert p))
end
| TProb (p) -> begin
(FStar_All.pipe_left (fun _103_287 -> TProb (_103_287)) (invert p))
end
| EProb (p) -> begin
(FStar_All.pipe_left (fun _103_288 -> EProb (_103_288)) (invert p))
end
| CProb (p) -> begin
(FStar_All.pipe_left (fun _103_289 -> CProb (_103_289)) (invert p))
end))

let is_top_level_prob = (fun p -> ((FStar_All.pipe_right (p_reason p) FStar_List.length) = 1))

let mk_problem = (fun scope orig lhs rel rhs elt reason -> (let _103_299 = (new_tvar (p_loc orig) scope FStar_Absyn_Syntax.ktype)
in {lhs = lhs; relation = rel; rhs = rhs; element = elt; logical_guard = _103_299; scope = []; reason = (reason)::(p_reason orig); loc = (p_loc orig); rank = None}))

let new_problem = (fun env lhs rel rhs elt loc reason -> (let _103_309 = (let _103_308 = (FStar_Tc_Env.get_range env)
in (let _103_307 = (FStar_Tc_Env.binders env)
in (new_tvar _103_308 _103_307 FStar_Absyn_Syntax.ktype)))
in {lhs = lhs; relation = rel; rhs = rhs; element = elt; logical_guard = _103_309; scope = []; reason = (reason)::[]; loc = loc; rank = None}))
...
| None -> begin
(let _103_320 = (let _103_319 = (FStar_Absyn_Syntax.v_binder x)
in (_103_319)::[])
in (FStar_Absyn_Util.close_forall _103_320 phi))
end
| Some (e) -> begin
...
| (_36_282, uv) -> begin
(let _36_292 = (match ((let _103_331 = (FStar_Absyn_Util.compress_typ uv)
in _103_331.FStar_Absyn_Syntax.n)) with
...
(let _103_333 = (let _103_332 = (FStar_List.map (uvi_to_string wl.tcenv) uvis)
in (FStar_All.pipe_right _103_332 (FStar_String.concat ", ")))
...
let explain = (fun env d s -> (let _103_354 = (FStar_All.pipe_left FStar_Range.string_of_range (p_loc d))
in (let _103_353 = (prob_to_string env d)
in (let _103_352 = (FStar_All.pipe_right (p_reason d) (FStar_String.concat "\n\t>"))
in (FStar_Util.format4 "(%s) Failed to solve the sub-problem\n%s\nWhich arose because:\n\t%s\nFailed because:%s\n" _103_354 _103_353 _103_352 s)))))
...
in (let _103_365 = (FStar_List.map Prims.snd g.carry)
in {attempting = _103_365; wl_deferred = _36_319.wl_deferred; subst = _36_319.subst; ctr = _36_319.ctr; slack_vars = g.slack; defer_ok = false; smt_ok = _36_319.smt_ok; tcenv = _36_319.tcenv})))
...
(let _103_390 = (prob_to_string env prob)
...
(let _103_421 = (let _103_420 = (norm_targ env t)
in (FStar_All.pipe_left (fun _103_419 -> FStar_Util.Inl (_103_419)) _103_420))
in (_103_421, (Prims.snd a)))
end
| FStar_Util.Inr (v) -> begin
(let _103_424 = (let _103_423 = (FStar_Tc_Normalize.norm_exp ((FStar_Tc_Normalize.Beta)::[]) env v)
in (FStar_All.pipe_left (fun _103_422 -> FStar_Util.Inr (_103_422)) _103_423))
in (_103_424, (Prims.snd a)))
end))

let whnf = (fun env t -> (let _103_429 = (FStar_Tc_Normalize.whnf env t)
in (FStar_All.pipe_right _103_429 FStar_Absyn_Util.compress_typ)))

let sn = (fun env t -> (let _103_434 = (FStar_Tc_Normalize.norm_typ ((FStar_Tc_Normalize.Beta)::(FStar_Tc_Normalize.Eta)::[]) env t)
in (FStar_All.pipe_right _103_434 FStar_Absyn_Util.compress_typ)))

let sn_binders = (fun env binders -> (FStar_All.pipe_right binders (FStar_List.map (fun _36_17 -> (match (_36_17) with
| (FStar_Util.Inl (a), imp) -> begin
(let _103_440 = (let _103_439 = (let _36_417 = a
in (let _103_438 = (FStar_Tc_Normalize.norm_kind ((FStar_Tc_Normalize.Beta)::[]) env a.FStar_Absyn_Syntax.sort)
in {FStar_Absyn_Syntax.v = _36_417.FStar_Absyn_Syntax.v; FStar_Absyn_Syntax.sort = _103_438; FStar_Absyn_Syntax.p = _36_417.FStar_Absyn_Syntax.p}))
in FStar_Util.Inl (_103_439))
in (_103_440, imp))
end
| (FStar_Util.Inr (x), imp) -> begin
(let _103_443 = (let _103_442 = (let _36_423 = x
in (let _103_441 = (norm_targ env x.FStar_Absyn_Syntax.sort)
in {FStar_Absyn_Syntax.v = _36_423.FStar_Absyn_Syntax.v; FStar_Absyn_Syntax.sort = _103_441; FStar_Absyn_Syntax.p = _36_423.FStar_Absyn_Syntax.p}))
in FStar_Util.Inr (_103_442))
in (_103_443, imp))
end)))))

let whnf_k = (fun env k -> (let _103_448 = (FStar_Tc_Normalize.norm_kind ((FStar_Tc_Normalize.Beta)::(FStar_Tc_Normalize.Eta)::(FStar_Tc_Normalize.WHNF)::[]) env k)
in (FStar_All.pipe_right _103_448 FStar_Absyn_Util.compress_kind)))

let whnf_e = (fun env e -> (let _103_453 = (FStar_Tc_Normalize.norm_exp ((FStar_Tc_Normalize.Beta)::(FStar_Tc_Normalize.Eta)::(FStar_Tc_Normalize.WHNF)::[]) env e)
in (FStar_All.pipe_right _103_453 FStar_Absyn_Util.compress_exp)))
...
(let k = (let _103_460 = (FStar_Absyn_Util.subst_of_list formals actuals)
in (FStar_Absyn_Util.subst_kind _103_460 body))
...
let rec compress = (fun env wl t -> (let t = (let _103_467 = (FStar_Absyn_Util.unmeta_typ t)
in (whnf env _103_467))
...
let normalize_refinement = (fun steps env wl t0 -> (let _103_482 = (compress env wl t0)
in (FStar_Tc_Normalize.normalize_refinement steps env _103_482)))
...
| tt -> begin
(let _103_495 = (let _103_494 = (FStar_Absyn_Print.typ_to_string tt)
in (let _103_493 = (FStar_Absyn_Print.tag_of_typ tt)
in (FStar_Util.format2 "impossible: Got %s ... %s\n" _103_494 _103_493)))
in (FStar_All.failwith _103_495))
end)
end
end
...
(let _36_555 = (let _103_496 = (normalize_refinement [] env wl t1)
in (aux true _103_496))
...
(let _103_499 = (let _103_498 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_497 = (FStar_Absyn_Print.tag_of_typ t1)
in (FStar_Util.format2 "impossible (outer): Got %s ... %s\n" _103_498 _103_497)))
in (FStar_All.failwith _103_499))
end))
in (let _103_500 = (compress env wl t1)
in (aux false _103_500))))

let unrefine = (fun env t -> (let _103_505 = (base_and_refinement env (empty_worklist env) t)
in (FStar_All.pipe_right _103_505 Prims.fst)))

let trivial_refinement = (fun t -> (let _103_507 = (FStar_Absyn_Util.gen_bvar_p t.FStar_Absyn_Syntax.pos t)
in (_103_507, FStar_Absyn_Util.t_true)))
...
in (let _103_527 = (FStar_All.pipe_right uvs.FStar_Absyn_Syntax.uvars_t FStar_Util.set_elements)
in (FStar_All.pipe_right _103_527 (FStar_Util.for_some (fun _36_616 -> (match (_36_616) with
| (uvt, _36_615) -> begin
...
(let _103_540 = (let _103_539 = (FStar_Absyn_Print.uvar_t_to_string uk)
in (let _103_538 = (FStar_Absyn_Print.typ_to_string t)
in (let _103_537 = (let _103_536 = (FStar_All.pipe_right wl.subst (FStar_List.map (uvi_to_string env)))
in (FStar_All.pipe_right _103_536 (FStar_String.concat ", ")))
in (FStar_Util.format3 "occurs-check failed (%s occurs in %s) (with substitution %s)" _103_539 _103_538 _103_537))))
in Some (_103_540))
end
in (occurs_ok, msg))))
...
| (occurs_ok, msg) -> begin
(let _103_551 = (FStar_Absyn_Util.fvs_included fvs_t fvs)
in (occurs_ok, _103_551, (msg, fvs, fvs_t)))
end))))
...
(let _103_563 = (let _103_562 = (FStar_Absyn_Print.uvar_e_to_string ut)
in (let _103_561 = (let _103_559 = (let _103_558 = (FStar_Util.set_elements uvs.FStar_Absyn_Syntax.uvars_e)
in (FStar_All.pipe_right _103_558 (FStar_List.map FStar_Absyn_Print.uvar_e_to_string)))
in (FStar_All.pipe_right _103_559 (FStar_String.concat ", ")))
in (let _103_560 = (FStar_Tc_Normalize.exp_norm_to_string env e)
in (FStar_Util.format3 "occurs-check failed (%s occurs in {%s} uvars of %s)" _103_562 _103_561 _103_560))))
in Some (_103_563))
end
in (occurs_ok, msg)))))
...
in (let _103_570 = (let _103_569 = (FStar_Util.set_intersect fvs1.FStar_Absyn_Syntax.ftvs fvs2.FStar_Absyn_Syntax.ftvs)
in (let _103_568 = (FStar_Util.set_intersect fvs1.FStar_Absyn_Syntax.fxvs fvs2.FStar_Absyn_Syntax.fxvs)
in {FStar_Absyn_Syntax.ftvs = _103_569; FStar_Absyn_Syntax.fxvs = _103_568}))
in (FStar_Absyn_Syntax.binders_of_freevars _103_570)))))
...
(let _103_586 = (FStar_Absyn_Print.arg_to_string hd)
...
let rec head_matches = (fun t1 t2 -> (match ((let _103_603 = (let _103_600 = (FStar_Absyn_Util.unmeta_typ t1)
in _103_600.FStar_Absyn_Syntax.n)
in (let _103_602 = (let _103_601 = (FStar_Absyn_Util.unmeta_typ t2)
in _103_601.FStar_Absyn_Syntax.n)
in (_103_603, _103_602)))) with
...
(let _103_604 = (head_matches x.FStar_Absyn_Syntax.sort y.FStar_Absyn_Syntax.sort)
in (FStar_All.pipe_right _103_604 head_match))
end
| (FStar_Absyn_Syntax.Typ_refine (x, _36_835), _36_839) -> begin
(let _103_605 = (head_matches x.FStar_Absyn_Syntax.sort t2)
in (FStar_All.pipe_right _103_605 head_match))
end
| (_36_842, FStar_Absyn_Syntax.Typ_refine (x, _36_845)) -> begin
(let _103_606 = (head_matches t1 x.FStar_Absyn_Syntax.sort)
in (FStar_All.pipe_right _103_606 head_match))
end
...
in (let _103_743 = (let _103_742 = (mk_problem scope orig gi_ps (vary_rel rel variance) ki None "kind subterm")
in (FStar_All.pipe_left (fun _103_741 -> KProb (_103_741)) _103_742))
in (FStar_Absyn_Syntax.K (gi_xs), _103_743)))
end))
end
...
in (let _103_746 = (let _103_745 = (mk_problem scope orig gi_ps (vary_rel rel variance) ti None "type subterm")
in (FStar_All.pipe_left (fun _103_744 -> TProb (_103_744)) _103_745))
in (FStar_Absyn_Syntax.T ((gi_xs, Some (k))), _103_746)))
end)))
end
...
in (let _103_749 = (let _103_748 = (mk_problem scope orig gi_ps (vary_rel rel variance) ei None "expression subterm")
in (FStar_All.pipe_left (fun _103_747 -> EProb (_103_747)) _103_748))
in (FStar_Absyn_Syntax.E (gi_xs), _103_749)))
end)))
end
...
(let _103_758 = (let _103_757 = (FStar_Absyn_Syntax.mk_Total gi_xs)
in (FStar_All.pipe_left (fun _103_756 -> FStar_Absyn_Syntax.C (_103_756)) _103_757))
in (_103_758, (prob)::[]))
end
| _36_1192 -> begin
(FStar_All.failwith "impossible")
...
in (let _36_1225 = (let _103_760 = (FStar_List.map (sub_prob scope args) components)
in (FStar_All.pipe_right _103_760 FStar_List.unzip))
in (match (_36_1225) with
| (ktecs, sub_probs) -> begin
(let gi_xs = (let _103_765 = (let _103_764 = (let _103_761 = (FStar_List.hd ktecs)
in (FStar_All.pipe_right _103_761 un_T))
in (let _103_763 = (let _103_762 = (FStar_List.tl ktecs)
in (FStar_All.pipe_right _103_762 (FStar_List.map arg_of_ktec)))
in {FStar_Absyn_Syntax.effect_name = c.FStar_Absyn_Syntax.effect_name; FStar_Absyn_Syntax.result_typ = _103_764; FStar_Absyn_Syntax.effect_args = _103_763; FStar_Absyn_Syntax.flags = c.FStar_Absyn_Syntax.flags}))
in (FStar_All.pipe_left FStar_Absyn_Syntax.mk_Comp _103_765))
...
(let _103_767 = (let _103_766 = (FStar_Absyn_Util.arg_of_non_null_binder b)
in (_103_766)::args)
in (Some (b), (b)::scope, _103_767))
end
| _36_1243 -> begin
(None, scope, args)
end)
...
| None -> begin
(let _103_770 = (let _103_769 = (FStar_All.pipe_right probs (FStar_List.map (fun prob -> (FStar_All.pipe_right (p_guard prob) Prims.fst))))
in (f)::_103_769)
in (FStar_Absyn_Util.mk_conj_l _103_770))
end
| Some (b) -> begin
(let _103_774 = (let _103_773 = (FStar_Absyn_Util.close_forall ((b)::[]) f)
in (let _103_772 = (FStar_All.pipe_right probs (FStar_List.map (fun prob -> (FStar_All.pipe_right (p_guard prob) Prims.fst))))
in (_103_773)::_103_772))
in (FStar_Absyn_Util.mk_conj_l _103_774))
end)
...
| (mul, s) -> begin
(match ((let _103_792 = (FStar_Absyn_Util.compress_typ s)
in _103_792.FStar_Absyn_Syntax.n)) with
| FStar_Absyn_Syntax.Typ_uvar (uv, k) -> begin
...
let new_slack_var = (fun env slack -> (let xs = (let _103_800 = (let _103_799 = (destruct_flex_pattern env (Prims.snd slack.lower))
in (FStar_All.pipe_right _103_799 Prims.snd))
in (FStar_All.pipe_right _103_800 FStar_Util.must))
in (let _103_801 = (new_tvar (Prims.fst slack.lower).FStar_Absyn_Syntax.pos xs FStar_Absyn_Syntax.ktype)
in (_103_801, xs))))
...
in (let _103_811 = (let _103_810 = (let _103_809 = (let _103_808 = (FStar_Util.mk_ref false)
in (low, high, _103_808))
in FStar_Absyn_Syntax.Meta_slack_formula (_103_809))
in (FStar_Absyn_Syntax.mk_Typ_meta _103_810))
in (_103_811, wl)))))
end)))
end)))
...
(let _103_835 = (let _103_834 = (mk_conn lhs rest)
in (_103_834, uvar))
in Some (_103_835))
end)
end))
...
(let _103_836 = (FStar_Absyn_Util.unmeta_typ phi)
in FStar_Util.Inl (_103_836))
end else begin
(let low = (let _103_837 = (compress env wl phi1)
in (FStar_All.pipe_left (destruct FStar_Absyn_Const.or_lid FStar_Absyn_Util.mk_disj) _103_837))
in (let hi = (let _103_838 = (compress env wl phi2)
in (FStar_All.pipe_left (destruct FStar_Absyn_Const.and_lid FStar_Absyn_Util.mk_disj) _103_838))
in (match ((low, hi)) with
| (None, None) -> begin
(let _36_1395 = (FStar_ST.op_Colon_Equals flag true)
in (let _103_839 = (FStar_Absyn_Util.unmeta_typ phi)
in FStar_Util.Inl (_103_839)))
end
...
| KProb (p) -> begin
(let _103_869 = (let _36_1518 = p
in (let _103_867 = (compress_k wl.tcenv wl p.lhs)
in (let _103_866 = (compress_k wl.tcenv wl p.rhs)
in {lhs = _103_867; relation = _36_1518.relation; rhs = _103_866; element = _36_1518.element; logical_guard = _36_1518.logical_guard; scope = _36_1518.scope; reason = _36_1518.reason; loc = _36_1518.loc; rank = _36_1518.rank})))
in (FStar_All.pipe_right _103_869 (fun _103_868 -> KProb (_103_868))))
end
| TProb (p) -> begin
(let _103_873 = (let _36_1522 = p
in (let _103_871 = (compress wl.tcenv wl p.lhs)
in (let _103_870 = (compress wl.tcenv wl p.rhs)
in {lhs = _103_871; relation = _36_1522.relation; rhs = _103_870; element = _36_1522.element; logical_guard = _36_1522.logical_guard; scope = _36_1522.scope; reason = _36_1522.reason; loc = _36_1522.loc; rank = _36_1522.rank})))
in (FStar_All.pipe_right _103_873 (fun _103_872 -> TProb (_103_872))))
end
| EProb (p) -> begin
(let _103_877 = (let _36_1526 = p
in (let _103_875 = (compress_e wl.tcenv wl p.lhs)
in (let _103_874 = (compress_e wl.tcenv wl p.rhs)
in {lhs = _103_875; relation = _36_1526.relation; rhs = _103_874; element = _36_1526.element; logical_guard = _36_1526.logical_guard; scope = _36_1526.scope; reason = _36_1526.reason; loc = _36_1526.loc; rank = _36_1526.rank})))
in (FStar_All.pipe_right _103_877 (fun _103_876 -> EProb (_103_876))))
end
| CProb (_36_1529) -> begin
p
end))

let rank = (fun wl prob -> (let prob = (let _103_882 = (compress_prob wl prob)
in (FStar_All.pipe_right _103_882 maybe_invert_p))
in (match (prob) with
| KProb (kp) -> begin
...
in (let _103_884 = (FStar_All.pipe_right (let _36_1561 = kp
in {lhs = _36_1561.lhs; relation = _36_1561.relation; rhs = _36_1561.rhs; element = _36_1561.element; logical_guard = _36_1561.logical_guard; scope = _36_1561.scope; reason = _36_1561.reason; loc = _36_1561.loc; rank = Some (rank)}) (fun _103_883 -> KProb (_103_883)))
in (rank, _103_884)))
end
| TProb (tp) -> begin
...
in (let _103_886 = (let _36_1605 = tp
in (let _103_885 = (force_refinement (b, ref_opt))
in {lhs = _36_1605.lhs; relation = _36_1605.relation; rhs = _103_885; element = _36_1605.element; logical_guard = _36_1605.logical_guard; scope = _36_1605.scope; reason = _36_1605.reason; loc = _36_1605.loc; rank = _36_1605.rank}))
in (rank, _103_886)))
end)
end))
end
...
| _36_1618 -> begin
(let _103_888 = (let _36_1619 = tp
in (let _103_887 = (force_refinement (b, ref_opt))
in {lhs = _103_887; relation = _36_1619.relation; rhs = _36_1619.rhs; element = _36_1619.element; logical_guard = _36_1619.logical_guard; scope = _36_1619.scope; reason = _36_1619.reason; loc = _36_1619.loc; rank = _36_1619.rank}))
in (refine_flex, _103_888))
end)
end))
end
...
| (rank, tp) -> begin
(let _103_890 = (FStar_All.pipe_right (let _36_1629 = tp
in {lhs = _36_1629.lhs; relation = _36_1629.relation; rhs = _36_1629.rhs; element = _36_1629.element; logical_guard = _36_1629.logical_guard; scope = _36_1629.scope; reason = _36_1629.reason; loc = _36_1629.loc; rank = Some (rank)}) (fun _103_889 -> TProb (_103_889)))
in (rank, _103_890))
end))
end))
end))
...
in (let _103_892 = (FStar_All.pipe_right (let _36_1666 = ep
in {lhs = _36_1666.lhs; relation = _36_1666.relation; rhs = _36_1666.rhs; element = _36_1666.element; logical_guard = _36_1666.logical_guard; scope = _36_1666.scope; reason = _36_1666.reason; loc = _36_1666.loc; rank = Some (rank)}) (fun _103_891 -> EProb (_103_891)))
in (rank, _103_892)))
end))
end))
end
| CProb (cp) -> begin
(let _103_894 = (FStar_All.pipe_right (let _36_1670 = cp
in {lhs = _36_1670.lhs; relation = _36_1670.relation; rhs = _36_1670.rhs; element = _36_1670.element; logical_guard = _36_1670.logical_guard; scope = _36_1670.scope; reason = _36_1670.reason; loc = _36_1670.loc; rank = Some (rigid_rigid)}) (fun _103_893 -> CProb (_103_893)))
in (rigid_rigid, _103_894))
end)))
...
(let _103_944 = (prob_to_string env (TProb (tp)))
...
(let _103_956 = (let _103_955 = (let _103_954 = (new_problem env t1 EQ t2 None t1.FStar_Absyn_Syntax.pos "joining refinements")
in (FStar_All.pipe_left (fun _103_953 -> TProb (_103_953)) _103_954))
in (_103_955)::[])
in Some (_103_956))
end
end else begin
...
| Some (m) -> begin
(let phi2 = (let _103_963 = (let _103_962 = (FStar_Absyn_Syntax.v_binder x)
in (let _103_961 = (FStar_Absyn_Syntax.v_binder y)
in (FStar_Absyn_Util.mk_subst_one_binder _103_962 _103_961)))
in (FStar_Absyn_Util.subst_typ _103_963 phi2))
in (let _103_967 = (let _103_966 = (let _103_965 = (let _103_964 = (FStar_Absyn_Util.mk_conj phi1 phi2)
in (x, _103_964))
in (FStar_Absyn_Syntax.mk_Typ_refine _103_965 (Some (FStar_Absyn_Syntax.ktype)) t1.FStar_Absyn_Syntax.pos))
in (_103_966, m))
in Some (_103_967)))
end))
end
...
| (u', _36_1792) -> begin
(match ((let _103_969 = (compress env wl u')
in _103_969.FStar_Absyn_Syntax.n)) with
...
| TProb (hd)::tl -> begin
(match ((let _103_974 = (compress env wl hd.rhs)
in (conjoin bound _103_974))) with
...
in (match ((let _103_976 = (let _103_975 = (compress env wl tp.rhs)
in (_103_975, []))
in (make_upper_bound _103_976 upper_bounds))) with
| None -> begin
...
(match (attempt) with
| [] -> begin
(let _103_985 = (let _103_984 = (let _103_983 = (FStar_List.map (fun _36_1896 -> (match (_36_1896) with
| (_36_1893, x, y) -> begin
(x, y)
end)) probs.wl_deferred)
in {carry = _103_983; slack = probs.slack_vars})
in (probs.subst, _103_984))
in Success (_103_985))
end
| _36_1898 -> begin
(let _103_988 = (let _36_1899 = probs
in (let _103_987 = (FStar_All.pipe_right attempt (FStar_List.map (fun _36_1906 -> (match (_36_1906) with
| (_36_1902, _36_1904, y) -> begin
y
end))))
in {attempting = _103_987; wl_deferred = rest; subst = _36_1899.subst; ctr = _36_1899.ctr; slack_vars = _36_1899.slack_vars; defer_ok = _36_1899.defer_ok; smt_ok = _36_1899.smt_ok; tcenv = _36_1899.tcenv}))
in (solve env _103_988))
end)
end))
end)
end))
...
| (hd1::xs, hd2::ys) -> begin
(let subst = (let _103_1014 = (FStar_Absyn_Util.mk_subst_one_binder hd2 hd1)
in (FStar_List.append _103_1014 subst))
in (let env = (let _103_1015 = (FStar_Tc_Env.binding_of_binder hd2)
in (FStar_Tc_Env.push_local_binding env _103_1015))
in (let prob = (match (((Prims.fst hd1), (Prims.fst hd2))) with
| (FStar_Util.Inl (a), FStar_Util.Inl (b)) -> begin
(let _103_1019 = (let _103_1018 = (FStar_Absyn_Util.subst_kind subst a.FStar_Absyn_Syntax.sort)
in (let _103_1017 = (FStar_All.pipe_left invert_rel (p_rel orig))
in (mk_problem ((hd2)::scope) orig _103_1018 _103_1017 b.FStar_Absyn_Syntax.sort None "Formal type parameter")))
in (FStar_All.pipe_left (fun _103_1016 -> KProb (_103_1016)) _103_1019))
end
| (FStar_Util.Inr (x), FStar_Util.Inr (y)) -> begin
(let _103_1023 = (let _103_1022 = (FStar_Absyn_Util.subst_typ subst x.FStar_Absyn_Syntax.sort)
in (let _103_1021 = (FStar_All.pipe_left invert_rel (p_rel orig))
in (mk_problem ((hd2)::scope) orig _103_1022 _103_1021 y.FStar_Absyn_Syntax.sort None "Formal value parameter")))
in (FStar_All.pipe_left (fun _103_1020 -> TProb (_103_1020)) _103_1023))
end
| _36_1982 -> begin
(FStar_All.failwith "impos")
end)
...
(let phi = (let _103_1025 = (FStar_All.pipe_right (p_guard prob) Prims.fst)
in (let _103_1024 = (FStar_Absyn_Util.close_forall ((hd2)::[]) phi)
in (FStar_Absyn_Util.mk_conj _103_1025 _103_1024)))
...
(let _103_1032 = (solve_prob orig None [] wl)
in (solve env _103_1032))
end else begin
(let k1 = problem.lhs
in (let k2 = problem.rhs
in if (FStar_Util.physical_equality k1 k2) then begin
(let _103_1033 = (solve_prob orig None [] wl)
in (solve env _103_1033))
end else begin
...
(let im = (let _103_1049 = (let _103_1048 = (h gs_xs)
in (xs, _103_1048))
in (FStar_Absyn_Syntax.mk_Kind_lam _103_1049 r))
...
in (let _103_1058 = (solve_prob orig None ((UK ((u, k1)))::[]) wl)
in (solve env _103_1058)))
end else begin
(let _103_1063 = (let _103_1062 = (FStar_All.pipe_right xs FStar_Absyn_Util.args_of_non_null_binders)
in (let _103_1061 = (decompose_kind env k)
in (rel, u, _103_1062, xs, _103_1061)))
in (imitate_k _103_1063))
end)))
end
| None -> begin
...
(let _103_1064 = (solve_prob orig None [] wl)
in (FStar_All.pipe_left (solve env) _103_1064))
end
...
(let sub_prob = (fun scope env subst -> (let _103_1073 = (let _103_1072 = (FStar_Absyn_Util.subst_kind subst k1')
in (mk_problem scope orig _103_1072 problem.relation k2' None "Arrow-kind result"))
in (FStar_All.pipe_left (fun _103_1071 -> KProb (_103_1071)) _103_1073)))
...
(let _103_1084 = (prob_to_string env orig)
...
(let im = (let _103_1096 = (let _103_1095 = (h gs_xs)
in (xs, _103_1095))
in (FStar_Absyn_Syntax.mk_Typ_lam' _103_1096 None r))
in (let _36_2190 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1101 = (FStar_Absyn_Print.typ_to_string im)
in (let _103_1100 = (FStar_Absyn_Print.tag_of_typ im)
in (let _103_1099 = (let _103_1097 = (FStar_List.map (prob_to_string env) sub_probs)
in (FStar_All.pipe_right _103_1097 (FStar_String.concat ", ")))
in (let _103_1098 = (FStar_Tc_Normalize.formula_norm_to_string env formula)
in (FStar_Util.print4 "Imitating %s (%s)\nsub_probs = %s\nformula=%s\n" _103_1101 _103_1100 _103_1099 _103_1098)))))
end else begin
()
end
...
in (let _103_1121 = (FStar_Absyn_Syntax.targ gi_xs)
in (let _103_1120 = (FStar_Absyn_Syntax.targ gi_ps)
in (_103_1121, _103_1120, subst))))))
end)))
end
| FStar_Util.Inr (x) -> begin
...
in (let _103_1123 = (FStar_Absyn_Syntax.varg gi_xs)
in (let _103_1122 = (FStar_Absyn_Syntax.varg gi_ps)
in (_103_1123, _103_1122, subst))))))
end)))
end)
...
in (match ((let _103_1125 = (let _103_1124 = (FStar_List.nth xs i)
in (FStar_All.pipe_left Prims.fst _103_1124))
in ((Prims.fst pi), _103_1125))) with
| (FStar_Util.Inl (pi), FStar_Util.Inl (xi)) -> begin
if (let _103_1126 = (matches pi)
in (FStar_All.pipe_left Prims.op_Negation _103_1126)) then begin
None
end else begin
...
in (let proj = (let _103_1128 = (let _103_1127 = (FStar_Absyn_Syntax.mk_Typ_app' (xi, g_xs) (Some (FStar_Absyn_Syntax.ktype)) r)
in (xs, _103_1127))
in (FStar_Absyn_Syntax.mk_Typ_lam _103_1128 None r))
in (let sub = (let _103_1134 = (let _103_1133 = (FStar_Absyn_Syntax.mk_Typ_app' (proj, ps) (Some (FStar_Absyn_Syntax.ktype)) r)
in (let _103_1132 = (let _103_1131 = (FStar_List.map (fun _36_2262 -> (match (_36_2262) with
| (_36_2258, _36_2260, y) -> begin
y
end)) qs)
in (FStar_All.pipe_left h _103_1131))
in (mk_problem (p_scope orig) orig _103_1133 (p_rel orig) _103_1132 None "projection")))
in (FStar_All.pipe_left (fun _103_1129 -> TProb (_103_1129)) _103_1134))
in (let _36_2264 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1136 = (FStar_Absyn_Print.typ_to_string proj)
in (let _103_1135 = (prob_to_string env sub)
in (FStar_Util.print2 "Projecting %s\n\tsubprob=%s\n" _103_1136 _103_1135)))
end else begin
()
end
in (let wl = (let _103_1138 = (let _103_1137 = (FStar_All.pipe_left Prims.fst (p_guard sub))
in Some (_103_1137))
in (solve_prob orig _103_1138 ((UT ((u, proj)))::[]) wl))
in (let _103_1140 = (solve env (attempt ((sub)::[]) wl))
in (FStar_All.pipe_left (fun _103_1139 -> Some (_103_1139)) _103_1140)))))))
end))
end
end
| _36_2268 -> begin
...
(let subterms = (fun ps -> (let xs = (let _103_1167 = (FStar_Absyn_Util.kind_formals k)
in (FStar_All.pipe_right _103_1167 Prims.fst))
in (let xs = (FStar_Absyn_Util.name_binders xs)
in (let _103_1172 = (decompose_typ env t2)
in ((uv, k), ps, xs, _103_1172)))))
...
(let _103_1183 = (FStar_Absyn_Print.freevars_to_string fvs_hd)
in (FStar_Util.print1 "Free variables are %s" _103_1183))
end else begin
()
end
in false)
...
in (let imitate_ok = (fun t2 -> (let fvs_hd = (let _103_1187 = (let _103_1186 = (FStar_Absyn_Util.head_and_args t2)
in (FStar_All.pipe_right _103_1186 Prims.fst))
in (FStar_All.pipe_right _103_1187 FStar_Absyn_Util.freevars_typ))
...
(let _103_1189 = (let _103_1188 = (FStar_Option.get msg)
in (Prims.strcat "occurs-check failed: " _103_1188))
in (giveup_or_defer orig _103_1189))
end else begin
if (FStar_Absyn_Util.fvs_included fvs2 fvs1) then begin
if ((FStar_Absyn_Util.is_function_typ t2) && ((p_rel orig) <> EQ)) then begin
(let _103_1190 = (subterms args_lhs)
in (imitate_t orig env wl _103_1190))
end else begin
(let _36_2333 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1193 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1192 = (FStar_Absyn_Print.freevars_to_string fvs1)
in (let _103_1191 = (FStar_Absyn_Print.freevars_to_string fvs2)
in (FStar_Util.print3 "Pattern %s with fvars=%s succeeded fvar check: %s\n" _103_1193 _103_1192 _103_1191))))
end else begin
()
end
...
| _36_2337 -> begin
(let _103_1195 = (let _103_1194 = (sn_binders env vars)
in (_103_1194, t2))
in (FStar_Absyn_Syntax.mk_Typ_lam _103_1195 None t1.FStar_Absyn_Syntax.pos))
end)
...
(let _103_1198 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1197 = (FStar_Absyn_Print.freevars_to_string fvs1)
in (let _103_1196 = (FStar_Absyn_Print.freevars_to_string fvs2)
in (FStar_Util.print3 "Pattern %s with fvars=%s failed fvar check: %s ... imitating\n" _103_1198 _103_1197 _103_1196))))
end else begin
()
end
in (let _103_1199 = (subterms args_lhs)
in (imitate_or_project (FStar_List.length args_lhs) _103_1199 (- (1)))))
end else begin
...
if (let _103_1200 = (FStar_Absyn_Util.freevars_typ t1)
in (check_head _103_1200 t2)) then begin
(let im_ok = (imitate_ok t2)
in (let _36_2344 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1201 = (FStar_Absyn_Print.typ_to_string t1)
in (FStar_Util.print2 "Not a pattern (%s) ... %s\n" _103_1201 (if (im_ok < 0) then begin
"imitating"
end else begin
"projecting"
end)))
...
in (let _103_1202 = (subterms args_lhs)
in (imitate_or_project (FStar_List.length args_lhs) _103_1202 im_ok))))
end else begin
...
(let sol = (let _103_1220 = (let _103_1219 = (FStar_Absyn_Syntax.mk_Typ_lam (binders, u1_ys) (Some (k)) t.FStar_Absyn_Syntax.pos)
in ((u, k), _103_1219))
in UT (_103_1220))
in (sol, (t', u, k1, ys)))
end))
...
| FStar_Util.Inl (a) -> begin
(let _103_1224 = (let _103_1223 = (FStar_Tc_Recheck.recompute_kind a)
in (FStar_All.pipe_right _103_1223 (FStar_Absyn_Util.gen_bvar_p a.FStar_Absyn_Syntax.pos)))
in (FStar_All.pipe_right _103_1224 FStar_Absyn_Syntax.t_binder))
end
| FStar_Util.Inr (x) -> begin
(let _103_1226 = (let _103_1225 = (FStar_Tc_Recheck.recompute_typ x)
in (FStar_All.pipe_right _103_1225 (FStar_Absyn_Util.gen_bvar_p x.FStar_Absyn_Syntax.pos)))
in (FStar_All.pipe_right _103_1226 FStar_Absyn_Syntax.v_binder))
end))
in (let _36_2393 = (match ((pat_var_opt env ys hd)) with
| None -> begin
(let _103_1227 = (new_binder hd)
in (_103_1227, ys))
end
| Some (y) -> begin
(match (xs_opt) with
...
if ((FStar_Unionfind.equivalent u1 u2) && (binders_eq xs ys)) then begin
(let _103_1239 = (solve_prob orig None [] wl)
in (solve env _103_1239))
end else begin
(let xs = (sn_binders env xs)
...
(let _103_1245 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1244 = (FStar_Absyn_Print.typ_to_string t2)
...
(let _103_1249 = (mk_problem (p_scope orig) orig t1 EQ t2 None "flex-flex index")
in (FStar_All.pipe_right _103_1249 (fun _103_1248 -> TProb (_103_1248))))
end
| (FStar_Util.Inr (t1), FStar_Util.Inr (t2)) -> begin
(let _103_1251 = (mk_problem (p_scope orig) orig t1 EQ t2 None "flex-flex index")
in (FStar_All.pipe_right _103_1251 (fun _103_1250 -> EProb (_103_1250))))
end
| _36_2452 -> begin
(FStar_All.failwith "Impossible")
end))) xs args2)
in (let guard = (let _103_1253 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
in (FStar_Absyn_Util.mk_conj_l _103_1253))
...
(let sol = (let _103_1255 = (let _103_1254 = (FStar_Absyn_Syntax.mk_Typ_lam' (xs, t2) (Some (k1)) t1.FStar_Absyn_Syntax.pos)
in ((u1, k1), _103_1254))
in UT (_103_1255))
...
(let _103_1256 = (uvi_to_string env sol)
...
| (Some (xs), Some (ys)) -> begin
(let _103_1257 = (FStar_Tc_Recheck.recompute_kind t2)
in (solve_both_pats wl (u1, k1, xs) (u2, k2, ys) _103_1257 t2.FStar_Absyn_Syntax.pos))
end
| (Some (xs), None) -> begin
...
(let _103_1258 = (uvi_to_string env sol)
...
in if (FStar_Util.physical_equality problem.lhs problem.rhs) then begin
(let _103_1259 = (solve_prob orig None [] wl)
in (solve env _103_1259))
end else begin
(let t1 = problem.lhs
in (let t2 = problem.rhs
in if (FStar_Util.physical_equality t1 t2) then begin
(let _103_1260 = (solve_prob orig None [] wl)
in (solve env _103_1260))
end else begin
(let _36_2523 = if (FStar_Tc_Env.debug env (FStar_Options.Other ("Rel"))) then begin
(let _103_1263 = (prob_to_string env orig)
in (let _103_1262 = (let _103_1261 = (FStar_List.map (uvi_to_string wl.tcenv) wl.subst)
in (FStar_All.pipe_right _103_1261 (FStar_String.concat "; ")))
...
| (bs, rest) -> begin
(let _103_1293 = (mk_cod rest)
in (bs, _103_1293))
end)))
in (let l1 = (FStar_List.length bs1)
in (let l2 = (FStar_List.length bs2)
in if (l1 = l2) then begin
(let _103_1297 = (let _103_1294 = (mk_cod1 [])
in (bs1, _103_1294))
in (let _103_1296 = (let _103_1295 = (mk_cod2 [])
in (bs2, _103_1295))
in (_103_1297, _103_1296)))
end else begin
if (l1 > l2) then begin
(let _103_1300 = (curry l2 bs1 mk_cod1)
in (let _103_1299 = (let _103_1298 = (mk_cod2 [])
in (bs2, _103_1298))
in (_103_1300, _103_1299)))
end else begin
(let _103_1303 = (let _103_1301 = (mk_cod1 [])
in (bs1, _103_1301))
in (let _103_1302 = (curry l1 bs2 mk_cod2)
in (_103_1303, _103_1302)))
end
end)))
end))
in (match ((t1.FStar_Absyn_Syntax.n, t2.FStar_Absyn_Syntax.n)) with
...
(let _103_1304 = (solve_prob orig None [] wl)
in (solve env _103_1304))
end else begin
(let _103_1308 = (let _103_1307 = (let _103_1306 = (FStar_Absyn_Util.mk_eq_typ t1 t2)
in (FStar_All.pipe_left (fun _103_1305 -> Some (_103_1305)) _103_1306))
in (solve_prob orig _103_1307 [] wl))
in (solve env _103_1308))
end
end
...
| bs -> begin
(let _103_1313 = (FStar_Absyn_Syntax.mk_Typ_fun (bs, c) None c.FStar_Absyn_Syntax.pos)
in (FStar_Absyn_Syntax.mk_Total _103_1313))
end))
...
(let _103_1320 = (let _103_1319 = (FStar_Tc_Env.get_range env)
in (FStar_All.pipe_right _103_1319 FStar_Range.string_of_range))
in (FStar_Util.print2 "(%s) Using relation %s at higher order\n" _103_1320 (rel_to_string rel)))
end else begin
()
end
in (let _103_1322 = (mk_problem scope orig c1 rel c2 None "function co-domain")
in (FStar_All.pipe_left (fun _103_1321 -> CProb (_103_1321)) _103_1322)))))))
end)))
end
...
in (let _103_1333 = (mk_problem scope orig t1' problem.relation t2' None "lambda co-domain")
in (FStar_All.pipe_left (fun _103_1332 -> TProb (_103_1332)) _103_1333)))))
end)))
end
...
| (x2, phi2) -> begin
(let base_prob = (let _103_1335 = (mk_problem (p_scope orig) orig x1.FStar_Absyn_Syntax.sort problem.relation x2.FStar_Absyn_Syntax.sort problem.element "refinement base type")
in (FStar_All.pipe_left (fun _103_1334 -> TProb (_103_1334)) _103_1335))
in (let x1_for_x2 = (let _103_1337 = (FStar_Absyn_Syntax.v_binder x1)
in (let _103_1336 = (FStar_Absyn_Syntax.v_binder x2)
in (FStar_Absyn_Util.mk_subst_one_binder _103_1337 _103_1336)))
in (let phi2 = (FStar_Absyn_Util.subst_typ x1_for_x2 phi2)
in (let mk_imp = (fun imp phi1 phi2 -> (let _103_1354 = (imp phi1 phi2)
in (FStar_All.pipe_right _103_1354 (guard_on_element problem x1))))
...
in (let guard = (let _103_1357 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Absyn_Util.mk_conj _103_1357 impl))
in (let wl = (solve_prob orig (Some (guard)) [] wl)
in (solve env (attempt ((base_prob)::[]) wl)))))
end))
in if (problem.relation = EQ) then begin
(let ref_prob = (let _103_1359 = (mk_problem (p_scope orig) orig phi1 EQ phi2 None "refinement formula")
in (FStar_All.pipe_left (fun _103_1358 -> TProb (_103_1358)) _103_1359))
...
(let guard = (let _103_1362 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (let _103_1361 = (let _103_1360 = (FStar_All.pipe_right (p_guard ref_prob) Prims.fst)
in (FStar_All.pipe_right _103_1360 (guard_on_element problem x1)))
in (FStar_Absyn_Util.mk_conj _103_1362 _103_1361)))
in (let wl = (solve_prob orig (Some (guard)) [] wl)
...
(let _103_1364 = (destruct_flex_t t1)
in (let _103_1363 = (destruct_flex_t t2)
in (flex_flex orig _103_1364 _103_1363)))
end
...
(let _103_1365 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid orig _103_1365 t2 wl))
end
...
in if (let _103_1366 = (is_top_level_prob orig)
in (FStar_All.pipe_left Prims.op_Negation _103_1366)) then begin
(let _103_1369 = (FStar_All.pipe_left (fun _103_1367 -> TProb (_103_1367)) (let _36_2799 = problem
...
in (let _103_1368 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid _103_1369 _103_1368 t2 wl)))
end else begin
...
(match (ref_opt) with
| None -> begin
(let _103_1372 = (FStar_All.pipe_left (fun _103_1370 -> TProb (_103_1370)) (let _36_2805 = problem
...
in (let _103_1371 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid _103_1372 _103_1371 t_base wl)))
end
| Some (y, phi) -> begin
...
in (let base_prob = (let _103_1374 = (mk_problem problem.scope orig t1 new_rel y.FStar_Absyn_Syntax.sort problem.element "flex-rigid: base type")
in (FStar_All.pipe_left (fun _103_1373 -> TProb (_103_1373)) _103_1374))
in (let guard = (let _103_1375 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Absyn_Util.mk_conj _103_1375 impl))
in (let wl = (solve_prob orig (Some (guard)) [] wl)
in (solve env (attempt ((base_prob)::[]) wl)))))))
end)
end))
...
(let t2 = (let _103_1376 = (base_and_refinement env wl t2)
in (FStar_All.pipe_left force_refinement _103_1376))
...
(let t1 = (let _103_1377 = (base_and_refinement env wl t1)
in (FStar_All.pipe_left force_refinement _103_1377))
...
(let head1 = (let _103_1378 = (FStar_Absyn_Util.head_and_args t1)
in (FStar_All.pipe_right _103_1378 Prims.fst))
in (let head2 = (let _103_1379 = (FStar_Absyn_Util.head_and_args t2)
in (FStar_All.pipe_right _103_1379 Prims.fst))
...
(let _103_1385 = (let _103_1384 = (let _103_1383 = (FStar_Absyn_Util.mk_eq_typ t1 t2)
in (FStar_All.pipe_left (fun _103_1382 -> Some (_103_1382)) _103_1383))
in (solve_prob orig _103_1384 [] wl))
in (solve env _103_1385))
end else begin
...
(let _103_1387 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1386 = (FStar_Absyn_Print.typ_to_string t2)
...
(let _103_1392 = (let _103_1391 = (FStar_Absyn_Print.typ_to_string head)
in (let _103_1390 = (FStar_Absyn_Print.args_to_string args)
in (let _103_1389 = (FStar_Absyn_Print.typ_to_string head')
in (let _103_1388 = (FStar_Absyn_Print.args_to_string args')
in (FStar_Util.format4 "unequal number of arguments: %s[%s] and %s[%s]" _103_1391 _103_1390 _103_1389 _103_1388)))))
in (giveup env _103_1392 orig))
end else begin
if ((nargs = 0) || (eq_args args args')) then begin
(let _103_1393 = (solve_prob orig None [] wl)
in (solve env _103_1393))
end else begin
...
(let _103_1396 = (let _103_1395 = (FStar_Absyn_Print.typ_to_string head)
in (let _103_1394 = (FStar_Absyn_Print.typ_to_string head')
in (FStar_Util.format2 "Assertion failed: expected full match of %s and %s\n" _103_1395 _103_1394)))
in (FStar_All.failwith _103_1396))
end else begin
()
end
in (let subprobs = (FStar_List.map2 (fun a a' -> (match (((Prims.fst a), (Prims.fst a'))) with
| (FStar_Util.Inl (t), FStar_Util.Inl (t')) -> begin
(let _103_1400 = (mk_problem (p_scope orig) orig t EQ t' None "type index")
in (FStar_All.pipe_left (fun _103_1399 -> TProb (_103_1399)) _103_1400))
end
| (FStar_Util.Inr (v), FStar_Util.Inr (v')) -> begin
(let _103_1402 = (mk_problem (p_scope orig) orig v EQ v' None "term index")
in (FStar_All.pipe_left (fun _103_1401 -> EProb (_103_1401)) _103_1402))
end
| _36_2967 -> begin
(FStar_All.failwith "Impossible")
end)) args args')
in (let formula = (let _103_1404 = (FStar_List.map (fun p -> (Prims.fst (p_guard p))) subprobs)
in (FStar_Absyn_Util.mk_conj_l _103_1404))
...
| (FStar_Util.Inl (t1), FStar_Util.Inl (t2)) -> begin
(let _103_1419 = (sub_prob t1 EQ t2 "effect arg")
in (FStar_All.pipe_left (fun _103_1418 -> TProb (_103_1418)) _103_1419))
end
| (FStar_Util.Inr (e1), FStar_Util.Inr (e2)) -> begin
(let _103_1421 = (sub_prob e1 EQ e2 "effect arg")
in (FStar_All.pipe_left (fun _103_1420 -> EProb (_103_1420)) _103_1421))
end
| _36_3047 -> begin
(FStar_All.failwith "impossible")
end)) c1_comp.FStar_Absyn_Syntax.effect_args c2_comp.FStar_Absyn_Syntax.effect_args)
in (let guard = (let _103_1423 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
in (FStar_Absyn_Util.mk_conj_l _103_1423))
in (let wl = (solve_prob orig (Some (guard)) [] wl)
in (solve env (attempt sub_probs wl)))))))
in if (FStar_Util.physical_equality c1 c2) then begin
(let _103_1424 = (solve_prob orig None [] wl)
in (solve env _103_1424))
end else begin
(let _36_3052 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1426 = (FStar_Absyn_Print.comp_typ_to_string c1)
in (let _103_1425 = (FStar_Absyn_Print.comp_typ_to_string c2)
in (FStar_Util.print3 "solve_c %s %s %s\n" _103_1426 (rel_to_string problem.relation) _103_1425)))
end else begin
()
end
in (let r = (FStar_Tc_Env.get_range env)
...
(let _103_1428 = (let _36_3070 = problem
in (let _103_1427 = (FStar_All.pipe_left FStar_Absyn_Syntax.mk_Comp (FStar_Absyn_Util.comp_to_comp_typ c1))
in {lhs = _103_1427; relation = _36_3070.relation; rhs = _36_3070.rhs; element = _36_3070.element; logical_guard = _36_3070.logical_guard; scope = _36_3070.scope; reason = _36_3070.reason; loc = _36_3070.loc; rank = _36_3070.rank}))
in (solve_c env _103_1428 wl))
end
| (FStar_Absyn_Syntax.Comp (_36_3073), FStar_Absyn_Syntax.Total (_36_3076)) -> begin
(let _103_1430 = (let _36_3079 = problem
in (let _103_1429 = (FStar_All.pipe_left FStar_Absyn_Syntax.mk_Comp (FStar_Absyn_Util.comp_to_comp_typ c2))
in {lhs = _36_3079.lhs; relation = _36_3079.relation; rhs = _103_1429; element = _36_3079.element; logical_guard = _36_3079.logical_guard; scope = _36_3079.scope; reason = _36_3079.reason; loc = _36_3079.loc; rank = _36_3079.rank}))
in (solve_c env _103_1430 wl))
end
...
| None -> begin
(let _103_1433 = (let _103_1432 = (FStar_Absyn_Print.sli c1.FStar_Absyn_Syntax.effect_name)
in (let _103_1431 = (FStar_Absyn_Print.sli c2.FStar_Absyn_Syntax.effect_name)
in (FStar_Util.format2 "incompatible monad ordering: %s </: %s" _103_1432 _103_1431)))
in (giveup env _103_1433 orig))
end
| Some (edge) -> begin
...
| _36_3109 -> begin
(let _103_1436 = (let _103_1435 = (let _103_1434 = (FStar_Absyn_Syntax.range_of_lid c1.FStar_Absyn_Syntax.effect_name)
in (FStar_Range.string_of_range _103_1434))
in (FStar_Util.format1 "Unexpected number of indices on a normalized effect (%s)" _103_1435))
in (FStar_All.failwith _103_1436))
end)
in (match (_36_3112) with
| (wp, wlp) -> begin
(let c1 = (let _103_1442 = (let _103_1441 = (let _103_1437 = (edge.FStar_Tc_Env.mlift c1.FStar_Absyn_Syntax.result_typ wp)
in (FStar_Absyn_Syntax.targ _103_1437))
in (let _103_1440 = (let _103_1439 = (let _103_1438 = (edge.FStar_Tc_Env.mlift c1.FStar_Absyn_Syntax.result_typ wlp)
in (FStar_Absyn_Syntax.targ _103_1438))
in (_103_1439)::[])
in (_103_1441)::_103_1440))
in {FStar_Absyn_Syntax.effect_name = c2.FStar_Absyn_Syntax.effect_name; FStar_Absyn_Syntax.result_typ = c1.FStar_Absyn_Syntax.result_typ; FStar_Absyn_Syntax.effect_args = _103_1442; FStar_Absyn_Syntax.flags = c1.FStar_Absyn_Syntax.flags})
in (solve_eq c1 c2))
end))
end else begin
...
| _36_3139 -> begin
(let _103_1446 = (let _103_1445 = (FStar_Absyn_Print.sli c1.FStar_Absyn_Syntax.effect_name)
in (let _103_1444 = (FStar_Absyn_Print.sli c2.FStar_Absyn_Syntax.effect_name)
in (FStar_Util.format2 "Got effects %s and %s, expected normalized effects" _103_1445 _103_1444)))
in (FStar_All.failwith _103_1446))
end)
...
in (let _103_1452 = (let _103_1451 = (let _103_1450 = (FStar_Absyn_Syntax.targ c1.FStar_Absyn_Syntax.result_typ)
in (let _103_1449 = (let _103_1448 = (let _103_1447 = (edge.FStar_Tc_Env.mlift c1.FStar_Absyn_Syntax.result_typ wpc1)
in (FStar_All.pipe_left FStar_Absyn_Syntax.targ _103_1447))
in (_103_1448)::[])
in (_103_1450)::_103_1449))
in (c2_decl.FStar_Absyn_Syntax.trivial, _103_1451))
in (FStar_Absyn_Syntax.mk_Typ_app _103_1452 (Some (FStar_Absyn_Syntax.ktype)) r)))
end else begin
(let wp2_imp_wp1 = (let _103_1464 = (let _103_1463 = (let _103_1462 = (FStar_Absyn_Syntax.targ c2.FStar_Absyn_Syntax.result_typ)
in (let _103_1461 = (let _103_1460 = (FStar_Absyn_Syntax.targ wpc2)
in (let _103_1459 = (let _103_1458 = (let _103_1454 = (let _103_1453 = (FStar_Absyn_Const.kbin FStar_Absyn_Syntax.ktype FStar_Absyn_Syntax.ktype FStar_Absyn_Syntax.ktype)
in (FStar_Absyn_Util.ftv FStar_Absyn_Const.imp_lid _103_1453))
in (FStar_All.pipe_left FStar_Absyn_Syntax.targ _103_1454))
in (let _103_1457 = (let _103_1456 = (let _103_1455 = (edge.FStar_Tc_Env.mlift c1.FStar_Absyn_Syntax.result_typ wpc1)
in (FStar_All.pipe_left FStar_Absyn_Syntax.targ _103_1455))
in (_103_1456)::[])
in (_103_1458)::_103_1457))
in (_103_1460)::_103_1459))
in (_103_1462)::_103_1461))
in (c2_decl.FStar_Absyn_Syntax.wp_binop, _103_1463))
in (FStar_Absyn_Syntax.mk_Typ_app _103_1464 None r))
in (let _103_1469 = (let _103_1468 = (let _103_1467 = (FStar_Absyn_Syntax.targ c2.FStar_Absyn_Syntax.result_typ)
in (let _103_1466 = (let _103_1465 = (FStar_Absyn_Syntax.targ wp2_imp_wp1)
in (_103_1465)::[])
in (_103_1467)::_103_1466))
in (c2_decl.FStar_Absyn_Syntax.wp_as_type, _103_1468))
in (FStar_Absyn_Syntax.mk_Typ_app _103_1469 (Some (FStar_Absyn_Syntax.ktype)) r)))
end
in (let base_prob = (let _103_1471 = (sub_prob c1.FStar_Absyn_Syntax.result_typ problem.relation c2.FStar_Absyn_Syntax.result_typ "result type")
in (FStar_All.pipe_left (fun _103_1470 -> TProb (_103_1470)) _103_1471))
in (let wl = (let _103_1475 = (let _103_1474 = (let _103_1473 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Absyn_Util.mk_conj _103_1473 g))
in (FStar_All.pipe_left (fun _103_1472 -> Some (_103_1472)) _103_1474))
in (solve_prob orig _103_1475 [] wl))
in (solve env (attempt ((base_prob)::[]) wl))))))
end
end)))
end
...
(let _103_1485 = (prob_to_string env orig)
...
(let maybe_vars1 = (pat_vars env [] args1)
in (let sub_problems = (fun xs args2 -> (let _36_3206 = (let _103_1501 = (FStar_All.pipe_right args2 (FStar_List.map (fun _36_34 -> (match (_36_34) with
...
in (let _103_1497 = (let _103_1496 = (sub_prob gi_pi t "type index")
in (FStar_All.pipe_left (fun _103_1495 -> TProb (_103_1495)) _103_1496))
in ((FStar_Util.Inl (gi_xi), imp), _103_1497)))
end)))
end
...
in (let _103_1500 = (let _103_1499 = (sub_prob gi_pi v "expression index")
in (FStar_All.pipe_left (fun _103_1498 -> EProb (_103_1498)) _103_1499))
in ((FStar_Util.Inr (gi_xi), imp), _103_1500)))
end)))
end))))
in (FStar_All.pipe_right _103_1501 FStar_List.unzip))
in (match (_36_3206) with
| (gi_xi, gi_pi) -> begin
(let formula = (let _103_1503 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) gi_pi)
in (FStar_Absyn_Util.mk_conj_l _103_1503))
in (gi_xi, gi_pi, formula))
end)))
in (let project_e = (fun head2 args2 -> (let giveup = (fun reason -> (let _103_1510 = (FStar_Util.format1 "flex-rigid: refusing to project expressions (%s)" reason)
in (giveup env _103_1510 orig)))
in (match ((let _103_1511 = (FStar_Absyn_Util.compress_exp head2)
in _103_1511.FStar_Absyn_Syntax.n)) with
...
(let _103_1514 = (let _103_1513 = (FStar_Absyn_Print.binders_to_string ", " all_xs)
in (let _103_1512 = (FStar_Absyn_Print.args_to_string args2)
in (FStar_Util.format2 "unequal arity:\n\texpetced binders %s\n\tgot args {%s}" _103_1513 _103_1512)))
in (giveup _103_1514))
end else begin
...
(match ((let _103_1519 = (FStar_Absyn_Util.compress_exp arg)
in _103_1519.FStar_Absyn_Syntax.n)) with
...
| (gi_xi, gi_pi, f) -> begin
(let sol = (let _103_1523 = (let _103_1522 = (let _103_1521 = (let _103_1520 = (FStar_Absyn_Util.bvar_to_exp xi)
in (_103_1520, gi_xi))
in (FStar_Absyn_Syntax.mk_Exp_app' _103_1521 None e1.FStar_Absyn_Syntax.pos))
in (all_xs, _103_1522))
in (FStar_Absyn_Syntax.mk_Exp_abs _103_1523 None e1.FStar_Absyn_Syntax.pos))
in (let _36_3277 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1527 = (FStar_Absyn_Print.uvar_e_to_string (u1, t1))
in (let _103_1526 = (FStar_Absyn_Print.exp_to_string sol)
in (let _103_1525 = (let _103_1524 = (FStar_All.pipe_right gi_pi (FStar_List.map (prob_to_string env)))
in (FStar_All.pipe_right _103_1524 (FStar_String.concat "\n")))
in (FStar_Util.print3 "Projected: %s -> %s\nSubprobs=\n%s\n" _103_1527 _103_1526 _103_1525))))
end else begin
()
end
in (let _103_1529 = (let _103_1528 = (solve_prob orig (Some (f)) ((UE (((u1, t1), sol)))::[]) wl)
in (attempt gi_pi _103_1528))
in (solve env _103_1529))))
end))
end else begin
(aux xs args)
end
...
| (x::xs, arg::args) -> begin
(let _103_1532 = (let _103_1531 = (FStar_Absyn_Print.binder_to_string x)
in (let _103_1530 = (FStar_Absyn_Print.arg_to_string arg)
...
(let _103_1536 = (FStar_Absyn_Print.exp_to_string e1)
in (let _103_1535 = (FStar_Absyn_Print.exp_to_string e2)
...
| _36_3317 -> begin
(let _103_1538 = (let _103_1537 = (FStar_Absyn_Syntax.mk_Exp_app' (head2, gi_xi) None e1.FStar_Absyn_Syntax.pos)
in (xs, _103_1537))
in (FStar_Absyn_Syntax.mk_Exp_abs _103_1538 None e1.FStar_Absyn_Syntax.pos))
end))
in (let _36_3319 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1542 = (FStar_Absyn_Print.uvar_e_to_string (u1, t1))
in (let _103_1541 = (FStar_Absyn_Print.exp_to_string sol)
in (let _103_1540 = (let _103_1539 = (FStar_All.pipe_right gi_pi (FStar_List.map (prob_to_string env)))
in (FStar_All.pipe_right _103_1539 (FStar_String.concat "\n")))
in (FStar_Util.print3 "Imitated: %s -> %s\nSubprobs=\n%s\n" _103_1542 _103_1541 _103_1540))))
end else begin
()
end
in (let _103_1544 = (let _103_1543 = (solve_prob orig (Some (f)) ((UE (((u1, t1), sol)))::[]) wl)
in (attempt gi_pi _103_1543))
in (solve env _103_1544))))
end))
end))
end else begin
...
in (let _103_1545 = (solve_prob orig None ((UE (((u1, t1), sol)))::[]) wl)
in (solve env _103_1545)))
end else begin
(imitate_or_project_e ())
end
end))))
...
in (let _36_3364 = (let _103_1550 = (FStar_Tc_Env.get_range env)
in (new_evar _103_1550 zs tt))
...
in (let _103_1551 = (solve_prob orig None ((UE (((u1, t1), sub1)))::(UE (((u2, t2), sub2)))::[]) wl)
in (solve env _103_1551))))
end))))
end
end)))
end))
...
(let _103_1556 = (prob_to_string env orig)
in (FStar_Util.print1 "Using SMT to solve:\n%s\n" _103_1556))
end else begin
()
end
in (let _36_3381 = (let _103_1558 = (FStar_Tc_Env.get_range env)
in (let _103_1557 = (FStar_Tc_Env.binders env)
in (new_tvar _103_1558 _103_1557 FStar_Absyn_Syntax.ktype)))
in (match (_36_3381) with
| (t, _36_3380) -> begin
(let _103_1562 = (let _103_1561 = (let _103_1560 = (FStar_Absyn_Util.mk_eq t t e1 e2)
in (FStar_All.pipe_left (fun _103_1559 -> Some (_103_1559)) _103_1560))
in (solve_prob orig _103_1561 [] wl))
in (solve env _103_1562))
end)))
end else begin
...
(let _103_1564 = (destruct_flex_e e1)
in (let _103_1563 = (destruct_flex_e e2)
in (flex_flex _103_1564 _103_1563)))
end
...
(let _103_1565 = (destruct_flex_e e1)
in (flex_rigid _103_1565 e2))
end
...
(let _103_1566 = (destruct_flex_e e2)
in (flex_rigid _103_1566 e1))
end
...
(let _103_1567 = (solve_prob orig None [] wl)
in (solve env _103_1567))
end else begin
(let _103_1573 = (let _103_1572 = (let _103_1571 = (let _103_1570 = (FStar_Tc_Recheck.recompute_typ e1)
in (let _103_1569 = (FStar_Tc_Recheck.recompute_typ e2)
in (FStar_Absyn_Util.mk_eq _103_1570 _103_1569 e1 e2)))
in (FStar_All.pipe_left (fun _103_1568 -> Some (_103_1568)) _103_1571))
in (solve_prob orig _103_1572 [] wl))
in (solve env _103_1573))
end
end
...
(let _103_1574 = (solve_prob orig None [] wl)
in (solve env _103_1574))
end else begin
...
(let _103_1579 = (solve_prob orig None [] wl)
in (solve env _103_1579))
end else begin
...
(let _103_1581 = (let _36_3603 = problem
in (let _103_1580 = (whnf_e env e1)
in {lhs = _103_1580; relation = _36_3603.relation; rhs = _36_3603.rhs; element = _36_3603.element; logical_guard = _36_3603.logical_guard; scope = _36_3603.scope; reason = _36_3603.reason; loc = _36_3603.loc; rank = _36_3603.rank}))
in (solve_e env _103_1581 wl))
end
...
(let _103_1583 = (let _36_3624 = problem
in (let _103_1582 = (whnf_e env e2)
in {lhs = _36_3624.lhs; relation = _36_3624.relation; rhs = _103_1582; element = _36_3624.element; logical_guard = _36_3624.logical_guard; scope = _36_3624.scope; reason = _36_3624.reason; loc = _36_3624.loc; rank = _36_3624.rank}))
in (solve_e env _103_1583 wl))
end
...
| ([], []) -> begin
(let guard = (let _103_1593 = (let _103_1592 = (FStar_List.map p_guard sub_probs)
in (FStar_All.pipe_right _103_1592 (FStar_List.map Prims.fst)))
in (FStar_Absyn_Util.mk_conj_l _103_1593))
...
(let _103_1594 = (solve_prob orig None wl.subst (let _36_3649 = orig_wl
...
in (solve env _103_1594))
end
| _36_3652 -> begin
(let _36_3656 = (let _103_1596 = (FStar_Tc_Env.get_range env)
in (let _103_1595 = (FStar_Tc_Env.binders env)
in (new_tvar _103_1596 _103_1595 FStar_Absyn_Syntax.ktype)))
in (match (_36_3656) with
| (t, _36_3655) -> begin
(let guard = (let _103_1597 = (FStar_Absyn_Util.mk_eq t t e1 e2)
in (FStar_Absyn_Util.mk_disj g _103_1597))
in (let _103_1598 = (solve_prob orig (Some (guard)) wl.subst (let _36_3658 = orig_wl
...
| (FStar_Util.Inl (t1), FStar_Util.Inl (t2)) -> begin
(let _103_1600 = (mk_problem (p_scope orig) orig t1 EQ t2 None "expression type arg")
in (FStar_All.pipe_left (fun _103_1599 -> TProb (_103_1599)) _103_1600))
end
| (FStar_Util.Inr (e1), FStar_Util.Inr (e2)) -> begin
(let _103_1602 = (mk_problem (p_scope orig) orig e1 EQ e2 None "expression arg")
in (FStar_All.pipe_left (fun _103_1601 -> EProb (_103_1601)) _103_1602))
end
| _36_3678 -> begin
...
in (let rec match_head_and_args = (fun head1 head2 -> (match ((let _103_1610 = (let _103_1607 = (FStar_Absyn_Util.compress_exp head1)
in _103_1607.FStar_Absyn_Syntax.n)
in (let _103_1609 = (let _103_1608 = (FStar_Absyn_Util.compress_exp head2)
in _103_1608.FStar_Absyn_Syntax.n)
in (_103_1610, _103_1609)))) with
...
(let _103_1612 = (let _36_3739 = problem
in (let _103_1611 = (whnf_e env e1)
in {lhs = _103_1611; relation = _36_3739.relation; rhs = _36_3739.rhs; element = _36_3739.element; logical_guard = _36_3739.logical_guard; scope = _36_3739.scope; reason = _36_3739.reason; loc = _36_3739.loc; rank = _36_3739.rank}))
in (solve_e env _103_1612 wl))
end
| (_36_3742, FStar_Absyn_Syntax.Exp_abs (_36_3744)) -> begin
(let _103_1614 = (let _36_3747 = problem
in (let _103_1613 = (whnf_e env e2)
in {lhs = _36_3747.lhs; relation = _36_3747.relation; rhs = _103_1613; element = _36_3747.element; logical_guard = _36_3747.logical_guard; scope = _36_3747.scope; reason = _36_3747.reason; loc = _36_3747.loc; rank = _36_3747.rank}))
in (solve_e env _103_1614 wl))
end
| _36_3750 -> begin
(smt_fallback e1 e2)
end))
...
| _36_3752 -> begin
(let _36_3756 = (let _103_1616 = (FStar_Tc_Env.get_range env)
in (let _103_1615 = (FStar_Tc_Env.binders env)
in (new_tvar _103_1616 _103_1615 FStar_Absyn_Syntax.ktype)))
...
(let _103_1617 = (FStar_Absyn_Print.typ_to_string guard)
in (FStar_Util.print1 "Emitting guard %s\n" _103_1617))
end else begin
()
end
in (let _103_1621 = (let _103_1620 = (let _103_1619 = (FStar_Absyn_Util.mk_eq t t e1 e2)
in (FStar_All.pipe_left (fun _103_1618 -> Some (_103_1618)) _103_1619))
in (solve_prob orig _103_1620 [] wl))
in (solve env _103_1621))))
end))
end)))))))))))
...
in (let carry = (let _103_1652 = (FStar_List.map (fun _36_3776 -> (match (_36_3776) with
| (_36_3774, x) -> begin
(prob_to_string env x)
end)) g.deferred.carry)
in (FStar_All.pipe_right _103_1652 (FStar_String.concat ",\n")))
...
in (let _103_1669 = (let _36_3807 = g
in (let _103_1668 = (let _103_1667 = (let _103_1666 = (let _103_1665 = (let _103_1664 = (FStar_Absyn_Syntax.v_binder x)
in (_103_1664)::[])
in (_103_1665, f))
in (FStar_Absyn_Syntax.mk_Typ_lam _103_1666 None f.FStar_Absyn_Syntax.pos))
in (FStar_All.pipe_left (fun _103_1663 -> NonTrivial (_103_1663)) _103_1667))
in {guard_f = _103_1668; deferred = _36_3807.deferred; implicits = _36_3807.implicits}))
in Some (_103_1669)))
end))
...
| NonTrivial (f) -> begin
(let _36_3814 = g
in (let _103_1681 = (let _103_1680 = (let _103_1679 = (let _103_1678 = (let _103_1677 = (let _103_1676 = (FStar_Absyn_Syntax.varg e)
in (_103_1676)::[])
in (f, _103_1677))
in (FStar_Absyn_Syntax.mk_Typ_app _103_1678))
in (FStar_All.pipe_left (FStar_Absyn_Syntax.syn f.FStar_Absyn_Syntax.pos (Some (FStar_Absyn_Syntax.ktype))) _103_1679))
in NonTrivial (_103_1680))
in {guard_f = _103_1681; deferred = _36_3814.deferred; implicits = _36_3814.implicits}))
end))
...
(let _103_1688 = (FStar_Absyn_Util.mk_conj f1 f2)
in NonTrivial (_103_1688))
end))
...
let binop_guard = (fun f g1 g2 -> (let _103_1711 = (f g1.guard_f g2.guard_f)
in {guard_f = _103_1711; deferred = {carry = (FStar_List.append g1.deferred.carry g2.deferred.carry); slack = (FStar_List.append g1.deferred.slack g2.deferred.slack)}; implicits = (FStar_List.append g1.implicits g2.implicits)}))
...
| NonTrivial (f) -> begin
(let _36_3864 = g
in (let _103_1726 = (let _103_1725 = (FStar_Absyn_Util.close_forall binders f)
in (FStar_All.pipe_right _103_1725 (fun _103_1724 -> NonTrivial (_103_1724))))
in {guard_f = _103_1726; deferred = _36_3864.deferred; implicits = _36_3864.implicits}))
end))
...
(let _103_1738 = (FStar_Tc_Normalize.typ_norm_to_string env lhs)
in (let _103_1737 = (FStar_Tc_Normalize.typ_norm_to_string env rhs)
in (FStar_Util.format3 "Top-level:\n%s\n\t%s\n%s" _103_1738 (rel_to_string rel) _103_1737)))
end else begin
"TOP"
end
...
let new_t_prob = (fun env t1 rel t2 -> (let x = (let _103_1747 = (FStar_Tc_Env.get_range env)
in (FStar_Absyn_Util.gen_bvar_p _103_1747 t1))
in (let env = (FStar_Tc_Env.push_local_binding env (FStar_Tc_Env.Binding_var ((x.FStar_Absyn_Syntax.v, x.FStar_Absyn_Syntax.sort))))
in (let p = (let _103_1751 = (let _103_1749 = (FStar_Absyn_Util.bvar_to_exp x)
in (FStar_All.pipe_left (fun _103_1748 -> Some (_103_1748)) _103_1749))
in (let _103_1750 = (FStar_Tc_Env.get_range env)
in (new_t_problem env t1 rel t2 _103_1751 _103_1750)))
in (TProb (p), x)))))
...
(let _103_1759 = (FStar_Tc_Normalize.kind_norm_to_string env lhs)
in (let _103_1758 = (FStar_Tc_Normalize.kind_norm_to_string env rhs)
in (FStar_Util.format3 "Top-level:\n%s\n\t%s\n%s" _103_1759 (rel_to_string rel) _103_1758)))
end else begin
"TOP"
end
in (let p = (new_problem env lhs rel rhs elt loc reason)
...
(let _103_1764 = (FStar_Absyn_Print.typ_to_string f)
...
(let _103_1776 = (explain env d s)
in (FStar_All.pipe_left FStar_Util.print_string _103_1776))
end else begin
()
end
in (err (d, s)))
...
| Some (d) -> begin
(let _103_1788 = (let _103_1787 = (let _103_1786 = (let _103_1785 = (FStar_All.pipe_right (p_guard prob) Prims.fst)
in (FStar_All.pipe_right _103_1785 (fun _103_1784 -> NonTrivial (_103_1784))))
in {guard_f = _103_1786; deferred = d; implicits = []})
in (simplify_guard env _103_1787))
in (FStar_All.pipe_left (fun _103_1783 -> Some (_103_1783)) _103_1788))
end))

let try_keq = (fun env k1 k2 -> (let _36_3936 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1796 = (FStar_Absyn_Print.kind_to_string k1)
in (let _103_1795 = (FStar_Absyn_Print.kind_to_string k2)
in (FStar_Util.print2 "try_keq of %s and %s\n" _103_1796 _103_1795)))
end else begin
()
end
in (let prob = (let _103_1801 = (let _103_1800 = (FStar_Tc_Normalize.norm_kind ((FStar_Tc_Normalize.Beta)::[]) env k1)
in (let _103_1799 = (FStar_Tc_Normalize.norm_kind ((FStar_Tc_Normalize.Beta)::[]) env k2)
in (let _103_1798 = (FStar_Tc_Env.get_range env)
in (new_k_problem env _103_1800 EQ _103_1799 None _103_1798))))
in (FStar_All.pipe_left (fun _103_1797 -> KProb (_103_1797)) _103_1801))
in (let _103_1803 = (solve_and_commit env (singleton env prob) (fun _36_3939 -> None))
in (FStar_All.pipe_left (with_guard env prob) _103_1803)))))
...
in (match (t) with
| None -> begin
(let _103_1814 = (let _103_1813 = (let _103_1812 = (FStar_Tc_Errors.incompatible_kinds env k2 k1)
in (_103_1812, r))
in FStar_Absyn_Syntax.Error (_103_1813))
in (Prims.raise _103_1814))
end
| Some (t) -> begin
(let _103_1817 = (let _103_1816 = (let _103_1815 = (FStar_Tc_Errors.expected_typ_of_kind env k2 t k1)
in (_103_1815, r))
in FStar_Absyn_Syntax.Error (_103_1816))
in (Prims.raise _103_1817))
end))
end
| Some (g) -> begin
g
...
(let _103_1827 = (let _103_1824 = (FStar_Tc_Env.get_range env)
in (FStar_All.pipe_left FStar_Range.string_of_range _103_1824))
in (let _103_1826 = (FStar_Absyn_Print.kind_to_string k1)
in (let _103_1825 = (FStar_Absyn_Print.kind_to_string k2)
in (FStar_Util.print3 "(%s) subkind of %s and %s\n" _103_1827 _103_1826 _103_1825))))
end else begin
()
end
in (let prob = (let _103_1832 = (let _103_1831 = (whnf_k env k1)
in (let _103_1830 = (whnf_k env k2)
in (let _103_1829 = (FStar_Tc_Env.get_range env)
in (new_k_problem env _103_1831 SUB _103_1830 None _103_1829))))
in (FStar_All.pipe_left (fun _103_1828 -> KProb (_103_1828)) _103_1832))
in (let res = (let _103_1839 = (let _103_1838 = (solve_and_commit env (singleton env prob) (fun _36_3961 -> (let _103_1837 = (let _103_1836 = (let _103_1835 = (FStar_Tc_Errors.incompatible_kinds env k1 k2)
in (let _103_1834 = (FStar_Tc_Env.get_range env)
in (_103_1835, _103_1834)))
in FStar_Absyn_Syntax.Error (_103_1836))
in (Prims.raise _103_1837))))
in (FStar_All.pipe_left (with_guard env prob) _103_1838))
in (FStar_Util.must _103_1839))
in res))))

let try_teq = (fun env t1 t2 -> (let _36_3967 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1847 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1846 = (FStar_Absyn_Print.typ_to_string t2)
in (FStar_Util.print2 "try_teq of %s and %s\n" _103_1847 _103_1846)))
end else begin
()
end
in (let prob = (let _103_1850 = (let _103_1849 = (FStar_Tc_Env.get_range env)
in (new_t_problem env t1 EQ t2 None _103_1849))
in (FStar_All.pipe_left (fun _103_1848 -> TProb (_103_1848)) _103_1850))
in (let g = (let _103_1852 = (solve_and_commit env (singleton env prob) (fun _36_3970 -> None))
in (FStar_All.pipe_left (with_guard env prob) _103_1852))
in g))))

let teq = (fun env t1 t2 -> (match ((try_teq env t1 t2)) with
| None -> begin
(let _103_1862 = (let _103_1861 = (let _103_1860 = (FStar_Tc_Errors.basic_type_error env None t2 t1)
in (let _103_1859 = (FStar_Tc_Env.get_range env)
in (_103_1860, _103_1859)))
in FStar_Absyn_Syntax.Error (_103_1861))
in (Prims.raise _103_1862))
end
| Some (g) -> begin
(let _36_3979 = if (FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _103_1865 = (FStar_Absyn_Print.typ_to_string t1)
in (let _103_1864 = (FStar_Absyn_Print.typ_to_string t2)
in (let _103_1863 = (guard_to_string env g)
in (FStar_Util.print3 "teq of %s and %s succeeded with guard %s\n" _103_1865 _103_1864 _103_1863))))
end else begin
()
end
in g)
...
(let _103_1873 = (FStar_Tc_Normalize.typ_norm_to_string env t1)
in (let _103_1872 = (FStar_Tc_Normalize.typ_norm_to_string env t2)
...
| (prob, x) -> begin
(let g = (let _103_1875 = (solve_and_commit env (singleton env prob) (fun _36_3989 -> None))
in (FStar_All.pipe_left (with_guard env prob) _103_1875))
in (let _36_3992 = if ((FStar_All.pipe_left (FStar_Tc_Env.debug env) (FStar_Options.Other ("Rel"))) && (FStar_Util.is_some g)) then begin
(let _103_1879 = (FStar_Tc_Normalize.typ_norm_to_string env t1)
in (let _103_1878 = (FStar_Tc_Normalize.typ_norm_to_string env t2)
in (let _103_1877 = (let _103_1876 = (FStar_Util.must g)
in (guard_to_string env _103_1876))
in (FStar_Util.print3 "try_subtype succeeded: %s <: %s\n\tguard is %s\n" _103_1879 _103_1878 _103_1877))))
end else begin
()
end
...
let subtype_fail = (fun env t1 t2 -> (let _103_1886 = (let _103_1885 = (let _103_1884 = (FStar_Tc_Errors.basic_type_error env None t2 t1)
in (let _103_1883 = (FStar_Tc_Env.get_range env)
in (_103_1884, _103_1883)))
in FStar_Absyn_Syntax.Error (_103_1885))
in (Prims.raise _103_1886)))
...
(let _103_1900 = (FStar_Absyn_Print.comp_typ_to_string c1)
in (let _103_1899 = (FStar_Absyn_Print.comp_typ_to_string c2)
...
in (let prob = (let _103_1903 = (let _103_1902 = (FStar_Tc_Env.get_range env)
in (new_problem env c1 rel c2 None _103_1902 "sub_comp"))
in (FStar_All.pipe_left (fun _103_1901 -> CProb (_103_1901)) _103_1903))
in (let _103_1905 = (solve_and_commit env (singleton env prob) (fun _36_4010 -> None))
in (FStar_All.pipe_left (with_guard env prob) _103_1905))))))
...
(let _103_1918 = (let _103_1917 = (FStar_All.pipe_right g.deferred.carry (FStar_List.map (fun _36_4021 -> (match (_36_4021) with
| (msg, x) -> begin
(let _103_1916 = (FStar_All.pipe_left FStar_Range.string_of_range (p_loc x))
in (let _103_1915 = (prob_to_string env x)
in (let _103_1914 = (let _103_1913 = (FStar_All.pipe_right (p_guard x) Prims.fst)
in (FStar_Tc_Normalize.formula_norm_to_string env _103_1913))
in (FStar_Util.format4 "(At %s) %s\n%s\nguard is %s\n" _103_1916 msg _103_1915 _103_1914))))
end))))
in (FStar_All.pipe_right _103_1917 (FStar_String.concat "\n")))
in (FStar_All.pipe_left (FStar_Util.print1 "Trying to solve carried problems: begin\n%s\nend\n") _103_1918))
end else begin
()
end
in (let gopt = (let _103_1919 = (wl_of_guard env g.deferred)
in (solve_and_commit env _103_1919 fail))
in (match (gopt) with
...
(let _103_1926 = (FStar_Tc_Env.get_range env)
in (let _103_1925 = (let _103_1924 = (FStar_Absyn_Print.formula_to_string vc)
in (FStar_Util.format1 "Checking VC=\n%s\n" _103_1924))
in (FStar_Tc_Errors.diag _103_1926 _103_1925)))
end else begin
()
end
