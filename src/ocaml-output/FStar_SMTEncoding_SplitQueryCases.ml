...
| true -> begin
(let _0_348 = (f FStar_SMTEncoding_Util.mkTrue)
in ((true), (_0_348), (negs), (t)))
end
| uu____30 -> begin
(match (t.FStar_SMTEncoding_Term.tm) with
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.ITE, (g)::(t)::(e)::uu____38) -> begin
(let _0_350 = (FStar_SMTEncoding_Util.mkAnd (let _0_349 = (FStar_SMTEncoding_Util.mkNot g)
in ((negs), (_0_349))))
in (get_next_n_ite (n - (Prims.parse_int "1")) e _0_350 (fun x -> (f (FStar_SMTEncoding_Util.mkITE ((g), (t), (x)))))))
end
| FStar_SMTEncoding_Term.FreeV (uu____42) -> begin
(let _0_351 = (f FStar_SMTEncoding_Util.mkTrue)
in ((true), (_0_351), (negs), (t)))
end
| uu____45 -> begin
...
(let _0_353 = (FStar_SMTEncoding_Util.mkAnd (let _0_352 = (FStar_SMTEncoding_Util.mkNot t)
in ((negs), (_0_352))))
in ((true), (l), (_0_353)))
end
| uu____85 -> begin
...
(match (b) with
| true -> begin
(let _0_355 = (let _0_354 = (FStar_SMTEncoding_Util.mkImp ((negs), (t)))
in (_0_354)::l)
in (is_ite_all_the_way n rest negs' _0_355))
end
| uu____104 -> begin
...
let rec check_split_cases : (FStar_SMTEncoding_Term.term  ->  FStar_SMTEncoding_Term.term)  ->  FStar_SMTEncoding_Term.term Prims.list  ->  (FStar_SMTEncoding_Term.decl  ->  Prims.unit)  ->  Prims.unit = (fun f l check -> (FStar_List.iter (fun t -> (check (FStar_SMTEncoding_Term.Assume ((let _0_356 = (FStar_SMTEncoding_Util.mkNot (f t))
in ((_0_356), (None), (None))))))) (FStar_List.rev l)))


let check_exhaustiveness : (FStar_SMTEncoding_Term.term  ->  FStar_SMTEncoding_Term.term)  ->  FStar_SMTEncoding_Term.term  ->  (FStar_SMTEncoding_Term.decl  ->  Prims.unit)  ->  Prims.unit = (fun f negs check -> (check (FStar_SMTEncoding_Term.Assume ((let _0_357 = (FStar_SMTEncoding_Util.mkNot (f (FStar_SMTEncoding_Util.mkNot negs)))
in ((_0_357), (None), (None)))))))
...
(let _0_358 = (strip_not q')
in (parse_query_for_split_cases n _0_358 (fun x -> x)))
end
| uu____368 -> begin
