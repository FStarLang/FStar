      ...
      | FStar_SMTEncoding_Term.App
          ...
          let uu____52 =
            let uu____53 =
              ...
              (negs, uu____58)
            in
            FStar_SMTEncoding_Util.mkAnd uu____53
          in
          get_next_n_ite
...
let rec is_ite_all_the_way
  ...
  fun n1 t negs l ->
    if n1 <= Prims.parse_int "0" then FStar_Exn.raise FStar_Util.Impos
    else
      match t.FStar_SMTEncoding_Term.tm with
      | FStar_SMTEncoding_Term.FreeV uu____119 ->
          let uu____124 =
            let uu____125 =
              ...
              (negs, uu____130)
            in
            FStar_SMTEncoding_Util.mkAnd uu____125
          in
          (true, l, uu____124)
      | uu____133 ->
          let uu____134 = get_next_n_ite n1 t negs (fun x -> x) in
          match uu____134 with b, t1, negs', rest ->
            if b then
              let uu____165 =
                let uu____168 = FStar_SMTEncoding_Util.mkImp (negs, t1) in
                uu____168 :: l
              in
...
let rec parse_query_for_split_cases
    : Prims.int -> FStar_SMTEncoding_Term.term
      -> (FStar_SMTEncoding_Term.term -> FStar_SMTEncoding_Term.term)
      -> ( Prims.bool
         , ( FStar_SMTEncoding_Term.term -> FStar_SMTEncoding_Term.term
           , FStar_SMTEncoding_Term.term Prims.list
  ...
  fun n1 t f ->
    ...
    | FStar_SMTEncoding_Term.Quant
        ...
        parse_query_for_split_cases n1 t1 (fun x ->
            let uu____237 =
              FStar_SMTEncoding_Util.mkForall'' (l, opt, l', x)
            in
            f uu____237 )
    | FStar_SMTEncoding_Term.App
        ...
        let r =
          ...
          | FStar_SMTEncoding_Term.Quant
              ...
              parse_query_for_split_cases n1 t2 (fun x ->
                  ...
                  f uu____305 )
          | FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.ITE, uu____306)
            -> (
              let uu____311 =
                is_ite_all_the_way n1 t2 FStar_SMTEncoding_Util.mkTrue []
              in
              match uu____311 with b, l, negs ->
                ( b
                , ( (fun x ->
                      let uu____358 = FStar_SMTEncoding_Util.mkImp (t1, x) in
                      f uu____358 )
                  , l
                  , negs ) ) )
          | uu____359 ->
    ...
    | FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.ITE, uu____376) -> (
        let uu____381 =
          is_ite_all_the_way n1 t FStar_SMTEncoding_Util.mkTrue []
        in
