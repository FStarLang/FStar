    ...
    fun t  ->
      fun negs  ->
        fun f  ->
          ...
          then
            let _0_514 = f FStar_SMTEncoding_Util.mkTrue  in
            (true, _0_514, negs, t)
          else
             ...
             | FStar_SMTEncoding_Term.App
                 ...
                 let _0_516 =
                   FStar_SMTEncoding_Util.mkAnd
                     (let _0_515 = FStar_SMTEncoding_Util.mkNot g  in
                      (negs, _0_515))
                    in
                 get_next_n_ite (n - (Prims.parse_int "1")) e _0_516
                   (fun x  -> f (FStar_SMTEncoding_Util.mkITE (g, t, x)))
             | FStar_SMTEncoding_Term.FreeV uu____42 ->
                 let _0_517 = f FStar_SMTEncoding_Util.mkTrue  in
                 (true, _0_517, negs, t)
             | uu____45 ->
          ...
          then Prims.raise FStar_Util.Impos
            ...
            (match t.FStar_SMTEncoding_Term.tm with
             | FStar_SMTEncoding_Term.FreeV uu____81 ->
                 let _0_519 =
                   FStar_SMTEncoding_Util.mkAnd
                     (let _0_518 = FStar_SMTEncoding_Util.mkNot t  in
                      (negs, _0_518))
                    in
                 (true, l, _0_519)
             | uu____85 ->
                 ...
                 (match uu____86 with
                  | (b,t,negs',rest) ->
                      ...
                      then
                        let _0_521 =
                          let _0_520 = FStar_SMTEncoding_Util.mkImp (negs, t)
                             in
                          _0_520 :: l  in
                        is_ite_all_the_way n rest negs' _0_521
          ...
          (fun t  ->
             check
               (FStar_SMTEncoding_Term.Assume
                  (let _0_522 = FStar_SMTEncoding_Util.mkNot (f t)  in
                   (_0_522, None, None)))) (FStar_List.rev l)
  ...
  fun n  ->
    fun q  ->
      ...
      | FStar_SMTEncoding_Term.Assume (q',uu____363,uu____364) ->
          let _0_524 = strip_not q'  in
          parse_query_for_split_cases n _0_524 (fun x  -> x)
      | uu____368 ->
