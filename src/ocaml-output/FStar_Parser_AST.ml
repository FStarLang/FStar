        ...
        | (d,ds1) ->
            (match d.d with
             | TopLevelModule m ->
                 ...
                 ((let uu____3394 = FStar_List.tl ms in
                   match uu____3394 with
                   | (Module (m',uu____3397))::uu____3398 ->
                       let msg =
             ...
             | uu____3408 ->
                 ...
                 (FStar_List.iter
                    (fun uu___110_3412  ->
                       ...
                       | { d = TopLevelModule uu____3413; drange = r;
                           doc = uu____3415; quals = uu____3416;
                           attrs = uu____3417;_} ->
                           Prims.raise
                             (FStar_Errors.Error
...
let compile_op: Prims.int -> Prims.string -> Prims.string =
  fun arity  ->
    fun s  ->
      let name_of_char uu___111_3431 =
        match uu___111_3431 with
        | '&' -> "Amp"
        | '@' -> "At"
        | '+' -> "Plus"
      ...
      | uu____3433 ->
          let uu____3434 =
            let uu____3435 =
              let uu____3437 = FStar_String.list_of_string s in
              FStar_List.map name_of_char uu____3437 in
            FStar_String.concat "_" uu____3435 in
          Prims.strcat "op_" uu____3434
  ...
  fun uu____3449  ->
    ...
    | (comment,keywords) ->
        let uu____3463 =
          ...
          FStar_String.concat "," uu____3464 in
        Prims.strcat comment uu____3463
let string_of_let_qualifier: let_qualifier -> Prims.string =
  fun uu___112_3475  ->
    match uu___112_3475 with
    | NoLetQualifier  -> ""
    | Rec  -> "rec"
    | Mutable  -> "mutable"
let to_string_l sep f l =
  let uu____3500 = FStar_List.map f l in FStar_String.concat sep uu____3500
let imp_to_string: imp -> Prims.string =
  fun uu___113_3504  ->
    match uu___113_3504 with | Hash  -> "#" | uu____3505 -> ""
let rec term_to_string: term -> Prims.string =
  fun x  ->
    match x.tm with
    | Wild  -> "_"
    | Requires (t,uu____3516) ->
        let uu____3519 = term_to_string t in
        FStar_Util.format1 "(requires %s)" uu____3519
    | Ensures (t,uu____3521) ->
        let uu____3524 = term_to_string t in
        FStar_Util.format1 "(ensures %s)" uu____3524
    | Labeled (t,l,uu____3527) ->
        let uu____3528 = term_to_string t in
        FStar_Util.format2 "(labeled %s %s)" l uu____3528
    ...
    | Op (s,xs) ->
        let uu____3534 =
          ...
          FStar_String.concat ", " uu____3535 in
        FStar_Util.format2 "%s(%s)" s uu____3534
        ...
        let uu____3548 =
          to_string_l " "
            (fun uu____3551  ->
               ...
               | (a,imp) ->
                   let uu____3556 = term_to_string a in
                   FStar_Util.format2 "%s%s" (imp_to_string imp) uu____3556)
            args in
        FStar_Util.format2 "(%s %s)" l.FStar_Ident.str uu____3548
    | Abs (pats,t) ->
        let uu____3561 = to_string_l " " pat_to_string pats in
        let uu____3562 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "(fun %s -> %s)" uu____3561 uu____3562
    | App (t1,t2,imp) ->
        let uu____3566 = FStar_All.pipe_right t1 term_to_string in
        let uu____3567 = FStar_All.pipe_right t2 term_to_string in
        FStar_Util.format3 "%s %s%s" uu____3566 (imp_to_string imp)
          uu____3567
    | Let (Rec ,lbs,body) ->
        let uu____3576 =
          to_string_l " and "
            (fun uu____3579  ->
               ...
               | (p,b) ->
                   let uu____3584 = FStar_All.pipe_right p pat_to_string in
                   let uu____3585 = FStar_All.pipe_right b term_to_string in
                   FStar_Util.format2 "%s=%s" uu____3584 uu____3585) lbs in
        let uu____3586 = FStar_All.pipe_right body term_to_string in
        FStar_Util.format2 "let rec %s in %s" uu____3576 uu____3586
    | Let (q,(pat,tm)::[],body) ->
        let uu____3598 = FStar_All.pipe_right pat pat_to_string in
        let uu____3599 = FStar_All.pipe_right tm term_to_string in
        let uu____3600 = FStar_All.pipe_right body term_to_string in
        FStar_Util.format4 "let %s %s = %s in %s" (string_of_let_qualifier q)
          uu____3598 uu____3599 uu____3600
    | Seq (t1,t2) ->
        let uu____3603 = FStar_All.pipe_right t1 term_to_string in
        let uu____3604 = FStar_All.pipe_right t2 term_to_string in
        FStar_Util.format2 "%s; %s" uu____3603 uu____3604
    | If (t1,t2,t3) ->
        let uu____3608 = FStar_All.pipe_right t1 term_to_string in
        let uu____3609 = FStar_All.pipe_right t2 term_to_string in
        let uu____3610 = FStar_All.pipe_right t3 term_to_string in
        FStar_Util.format3 "if %s then %s else %s" uu____3608 uu____3609
          uu____3610
    | Match (t,branches) ->
        let uu____3623 = FStar_All.pipe_right t term_to_string in
        let uu____3624 =
          to_string_l " | "
            (fun uu____3629  ->
               match uu____3629 with
               | (p,w,e) ->
                   let uu____3639 = FStar_All.pipe_right p pat_to_string in
                   let uu____3640 =
                     match w with
                     | None  -> ""
                     | Some e1 ->
                         let uu____3642 = term_to_string e1 in
                         FStar_Util.format1 "when %s" uu____3642 in
                   let uu____3643 = FStar_All.pipe_right e term_to_string in
                   FStar_Util.format3 "%s %s -> %s" uu____3639 uu____3640
                     uu____3643) branches in
        FStar_Util.format2 "match %s with %s" uu____3623 uu____3624
    | Ascribed (t1,t2,None ) ->
        let uu____3647 = FStar_All.pipe_right t1 term_to_string in
        let uu____3648 = FStar_All.pipe_right t2 term_to_string in
        FStar_Util.format2 "(%s : %s)" uu____3647 uu____3648
    | Ascribed (t1,t2,Some tac) ->
        let uu____3653 = FStar_All.pipe_right t1 term_to_string in
        let uu____3654 = FStar_All.pipe_right t2 term_to_string in
        let uu____3655 = FStar_All.pipe_right tac term_to_string in
        FStar_Util.format3 "(%s : %s by %s)" uu____3653 uu____3654 uu____3655
    | Record (Some e,fields) ->
        let uu____3665 = FStar_All.pipe_right e term_to_string in
        let uu____3666 =
          to_string_l " "
            (fun uu____3669  ->
               match uu____3669 with
               | (l,e1) ->
                   let uu____3674 = FStar_All.pipe_right e1 term_to_string in
                   FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3674)
            fields in
        FStar_Util.format2 "{%s with %s}" uu____3665 uu____3666
    | Record (None ,fields) ->
        let uu____3683 =
          to_string_l " "
            (fun uu____3686  ->
               ...
               | (l,e) ->
                   ...
                   FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3691)
            fields in
        FStar_Util.format1 "{%s}" uu____3683
    | Project (e,l) ->
        let uu____3694 = FStar_All.pipe_right e term_to_string in
        FStar_Util.format2 "%s.%s" uu____3694 l.FStar_Ident.str
    ...
    | Product (b::[],t) when x.level = Type_level ->
        let uu____3708 = FStar_All.pipe_right b binder_to_string in
        let uu____3709 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "%s -> %s" uu____3708 uu____3709
    ...
    | Sum (binders,t) ->
        let uu____3719 =
          ...
          FStar_All.pipe_right uu____3720 (FStar_String.concat " * ") in
        let uu____3725 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "%s * %s" uu____3719 uu____3725
    | QForall (bs,pats,t) ->
        let uu____3735 = to_string_l " " binder_to_string bs in
        let uu____3736 =
          to_string_l " \\/ " (to_string_l "; " term_to_string) pats in
        let uu____3738 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format3 "forall %s.{:pattern %s} %s" uu____3735 uu____3736
          uu____3738
    | QExists (bs,pats,t) ->
        let uu____3748 = to_string_l " " binder_to_string bs in
        let uu____3749 =
          to_string_l " \\/ " (to_string_l "; " term_to_string) pats in
        let uu____3751 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format3 "exists %s.{:pattern %s} %s" uu____3748 uu____3749
          uu____3751
    | Refine (b,t) ->
        let uu____3754 = FStar_All.pipe_right b binder_to_string in
        let uu____3755 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "%s:{%s}" uu____3754 uu____3755
    | NamedTyp (x1,t) ->
        let uu____3758 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "%s:%s" x1.FStar_Ident.idText uu____3758
    | Paren t ->
    ...
    | Product (bs,t) ->
        let uu____3765 =
          ...
          FStar_All.pipe_right uu____3766 (FStar_String.concat ",") in
        let uu____3771 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "Unidentified product: [%s] %s" uu____3765
          uu____3771
    | t -> "_"
  ...
  fun x  ->
    let s =
      ...
      | TAnnotated (i,t)|Annotated (i,t) ->
          let uu____3779 = FStar_All.pipe_right t term_to_string in
          FStar_Util.format2 "%s:%s" i.FStar_Ident.idText uu____3779
      | NoName t -> FStar_All.pipe_right t term_to_string in
    let uu____3781 = aqual_to_string x.aqual in
    FStar_Util.format2 "%s%s" uu____3781 s
and aqual_to_string: aqual -> Prims.string =
  fun uu___114_3782  ->
    match uu___114_3782 with
    | Some (Equality ) -> "$"
    | Some (Implicit ) -> "#"
    | uu____3783 -> ""
and pat_to_string: pattern -> Prims.string =
  fun x  ->
    ...
    | PatApp (p,ps) ->
        let uu____3790 = FStar_All.pipe_right p pat_to_string in
        let uu____3791 = to_string_l " " pat_to_string ps in
        FStar_Util.format2 "(%s %s)" uu____3790 uu____3791
    | PatTvar (i,aq)|PatVar (i,aq) ->
        let uu____3798 = aqual_to_string aq in
        FStar_Util.format2 "%s%s" uu____3798 i.FStar_Ident.idText
    | PatName l -> l.FStar_Ident.str
    | PatList l ->
        let uu____3802 = to_string_l "; " pat_to_string l in
        FStar_Util.format1 "[%s]" uu____3802
    | PatTuple (l,false ) ->
        let uu____3806 = to_string_l ", " pat_to_string l in
        FStar_Util.format1 "(%s)" uu____3806
    | PatTuple (l,true ) ->
        let uu____3810 = to_string_l ", " pat_to_string l in
        FStar_Util.format1 "(|%s|)" uu____3810
    | PatRecord l ->
        let uu____3815 =
          to_string_l "; "
            (fun uu____3818  ->
               ...
               | (f,e) ->
                   let uu____3823 = FStar_All.pipe_right e pat_to_string in
                   FStar_Util.format2 "%s=%s" f.FStar_Ident.str uu____3823) l in
        FStar_Util.format1 "{%s}" uu____3815
    | PatOr l -> to_string_l "|\n " pat_to_string l
    | PatOp op -> FStar_Util.format1 "(%s)" op
    | PatAscribed (p,t) ->
        let uu____3829 = FStar_All.pipe_right p pat_to_string in
        let uu____3830 = FStar_All.pipe_right t term_to_string in
        FStar_Util.format2 "(%s:%s)" uu____3829 uu____3830
let rec head_id_of_pat: pattern -> FStar_Ident.lid Prims.list =
  fun p  ->
    match p.pat with
    | PatName l -> [l]
    | PatVar (i,uu____3838) ->
        let uu____3841 = FStar_Ident.lid_of_ids [i] in [uu____3841]
    | PatApp (p1,uu____3843) -> head_id_of_pat p1
    | PatAscribed (p1,uu____3847) -> head_id_of_pat p1
    | uu____3848 -> []
let lids_of_let defs =
  FStar_All.pipe_right defs
    (FStar_List.collect
       (fun uu____3869  ->
          match uu____3869 with | (p,uu____3874) -> head_id_of_pat p))
let id_of_tycon: tycon -> Prims.string =
  fun uu___115_3877  ->
    match uu___115_3877 with
    | TyconAbstract (i,_,_)
  ...
  fun d  ->
    ...
    | ModuleAbbrev (i,l) ->
        FStar_Util.format2 "module %s = %s" i.FStar_Ident.idText
          l.FStar_Ident.str
    | TopLevelLet (uu____3918,pats) ->
        let uu____3926 =
          let uu____3927 =
            let uu____3929 = lids_of_let pats in
            FStar_All.pipe_right uu____3929
              (FStar_List.map (fun l  -> l.FStar_Ident.str)) in
          FStar_All.pipe_right uu____3927 (FStar_String.concat ", ") in
        Prims.strcat "let " uu____3926
    | Main uu____3935 -> "main ..."
    | Assume (i,uu____3937) -> Prims.strcat "assume " i.FStar_Ident.idText
    | Tycon (uu____3938,tys) ->
        let uu____3948 =
          let uu____3949 =
            FStar_All.pipe_right tys
              (FStar_List.map
                 (fun uu____3959  ->
                    match uu____3959 with | (x,uu____3964) -> id_of_tycon x)) in
          FStar_All.pipe_right uu____3949 (FStar_String.concat ", ") in
        Prims.strcat "type " uu____3948
    | Val (i,uu____3969) -> Prims.strcat "val " i.FStar_Ident.idText
    | Exception (i,uu____3971) ->
        Prims.strcat "exception " i.FStar_Ident.idText
    | NewEffect (DefineEffect (i,_,_,_))|NewEffect (RedefineEffect (i,_,_))
        -> Prims.strcat "new_effect " i.FStar_Ident.idText
    | SubEffect uu____3983 -> "sub_effect"
    | Pragma uu____3984 -> "pragma"
    | Fsdoc uu____3985 -> "fsdoc"
let modul_to_string: modul -> Prims.string =
  fun m  ->
    ...
    | Module (_,decls)|Interface (_,decls,_) ->
        ...
        FStar_All.pipe_right uu____3997 (FStar_String.concat "\n")
let error msg tm r =
