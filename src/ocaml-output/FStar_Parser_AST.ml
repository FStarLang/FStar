...
let uu____3386 = (mk_decl (Pragma (LightOff)) light_range [])
in (uu____3386)::ds1)
end
| uu____3387 -> begin
ds1
...
let uu____3394 = (FStar_List.tl ms)
in (match (uu____3394) with
| (Module (m', uu____3397))::uu____3398 -> begin
(

let msg = "Support for more than one module in a file is deprecated"
in (

let uu____3403 = (FStar_Range.string_of_range (FStar_Ident.range_of_lid m'))
...
let ds2 = (d)::ds1
in ((FStar_List.iter (fun uu___107_3412 -> (match (uu___107_3412) with
| {d = TopLevelModule (uu____3413); drange = r; doc = uu____3415; quals = uu____3416; attrs = uu____3417} -> begin
...
let name_of_char = (fun uu___108_3431 -> (match (uu___108_3431) with
| '&' -> begin
"Amp"
end
| '@' -> begin
...
let uu____3437 = (FStar_String.list_of_string s)
in (FStar_List.map name_of_char uu____3437))
in (FStar_String.concat "_" uu____3435))
in (Prims.strcat "op_" uu____3434))
end)))
...
let string_of_fsdoc : (Prims.string * (Prims.string * Prims.string) Prims.list)  ->  Prims.string = (fun uu____3449 -> (match (uu____3449) with
| (comment, keywords) -> begin
(

let uu____3463 = (

let uu____3464 = (FStar_List.map (fun uu____3468 -> (match (uu____3468) with
| (k, v1) -> begin
(Prims.strcat k (Prims.strcat "->" v1))
end)) keywords)
in (FStar_String.concat "," uu____3464))
in (Prims.strcat comment uu____3463))
end))


let string_of_let_qualifier : let_qualifier  ->  Prims.string = (fun uu___109_3475 -> (match (uu___109_3475) with
| NoLetQualifier -> begin
""
end
| Rec -> begin
...
let uu____3500 = (FStar_List.map f l)
in (FStar_String.concat sep uu____3500)))


let imp_to_string : imp  ->  Prims.string = (fun uu___110_3504 -> (match (uu___110_3504) with
| Hash -> begin
"#"
end
| uu____3505 -> begin
...
| Requires (t, uu____3516) -> begin
(

let uu____3519 = (term_to_string t)
in (FStar_Util.format1 "(requires %s)" uu____3519))
end
| Ensures (t, uu____3521) -> begin
(

let uu____3524 = (term_to_string t)
in (FStar_Util.format1 "(ensures %s)" uu____3524))
end
| Labeled (t, l, uu____3527) -> begin
(

let uu____3528 = (term_to_string t)
in (FStar_Util.format2 "(labeled %s %s)" l uu____3528))
end
| Const (c) -> begin
...
let uu____3535 = (FStar_List.map (fun x1 -> (FStar_All.pipe_right x1 term_to_string)) xs)
in (FStar_String.concat ", " uu____3535))
in (FStar_Util.format2 "%s(%s)" s uu____3534))
end
...
let uu____3548 = (to_string_l " " (fun uu____3551 -> (match (uu____3551) with
| (a, imp) -> begin
(

let uu____3556 = (term_to_string a)
in (FStar_Util.format2 "%s%s" (imp_to_string imp) uu____3556))
end)) args)
in (FStar_Util.format2 "(%s %s)" l.FStar_Ident.str uu____3548))
end
| Abs (pats, t) -> begin
(

let uu____3561 = (to_string_l " " pat_to_string pats)
in (

let uu____3562 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(fun %s -> %s)" uu____3561 uu____3562)))
end
| App (t1, t2, imp) -> begin
(

let uu____3566 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3567 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format3 "%s %s%s" uu____3566 (imp_to_string imp) uu____3567)))
end
| Let (Rec, lbs, body) -> begin
(

let uu____3576 = (to_string_l " and " (fun uu____3579 -> (match (uu____3579) with
| (p, b) -> begin
(

let uu____3584 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3585 = (FStar_All.pipe_right b term_to_string)
in (FStar_Util.format2 "%s=%s" uu____3584 uu____3585)))
end)) lbs)
in (

let uu____3586 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format2 "let rec %s in %s" uu____3576 uu____3586)))
end
| Let (q, ((pat, tm))::[], body) -> begin
(

let uu____3598 = (FStar_All.pipe_right pat pat_to_string)
in (

let uu____3599 = (FStar_All.pipe_right tm term_to_string)
in (

let uu____3600 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format4 "let %s %s = %s in %s" (string_of_let_qualifier q) uu____3598 uu____3599 uu____3600))))
end
| Seq (t1, t2) -> begin
(

let uu____3603 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3604 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "%s; %s" uu____3603 uu____3604)))
end
| If (t1, t2, t3) -> begin
(

let uu____3608 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3609 = (FStar_All.pipe_right t2 term_to_string)
in (

let uu____3610 = (FStar_All.pipe_right t3 term_to_string)
in (FStar_Util.format3 "if %s then %s else %s" uu____3608 uu____3609 uu____3610))))
end
| Match (t, branches) -> begin
(

let uu____3623 = (FStar_All.pipe_right t term_to_string)
in (

let uu____3624 = (to_string_l " | " (fun uu____3629 -> (match (uu____3629) with
| (p, w, e) -> begin
(

let uu____3639 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3640 = (match (w) with
| None -> begin
""
end
| Some (e1) -> begin
...
let uu____3642 = (term_to_string e1)
in (FStar_Util.format1 "when %s" uu____3642))
end)
in (

let uu____3643 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format3 "%s %s -> %s" uu____3639 uu____3640 uu____3643))))
end)) branches)
in (FStar_Util.format2 "match %s with %s" uu____3623 uu____3624)))
end
| Ascribed (t1, t2, None) -> begin
(

let uu____3647 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3648 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "(%s : %s)" uu____3647 uu____3648)))
end
| Ascribed (t1, t2, Some (tac)) -> begin
(

let uu____3653 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3654 = (FStar_All.pipe_right t2 term_to_string)
in (

let uu____3655 = (FStar_All.pipe_right tac term_to_string)
in (FStar_Util.format3 "(%s : %s by %s)" uu____3653 uu____3654 uu____3655))))
end
| Record (Some (e), fields) -> begin
(

let uu____3665 = (FStar_All.pipe_right e term_to_string)
in (

let uu____3666 = (to_string_l " " (fun uu____3669 -> (match (uu____3669) with
| (l, e1) -> begin
(

let uu____3674 = (FStar_All.pipe_right e1 term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3674))
end)) fields)
in (FStar_Util.format2 "{%s with %s}" uu____3665 uu____3666)))
end
| Record (None, fields) -> begin
(

let uu____3683 = (to_string_l " " (fun uu____3686 -> (match (uu____3686) with
| (l, e) -> begin
(

let uu____3691 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3691))
end)) fields)
in (FStar_Util.format1 "{%s}" uu____3683))
end
| Project (e, l) -> begin
(

let uu____3694 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s.%s" uu____3694 l.FStar_Ident.str))
end
| Product ([], t) -> begin
(term_to_string t)
end
...
let uu____3708 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3709 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s -> %s" uu____3708 uu____3709)))
end
| Product ((b)::[], t) when (x.level = Kind) -> begin
(

let uu____3713 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3714 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s => %s" uu____3713 uu____3714)))
end
| Sum (binders, t) -> begin
(
...
let uu____3720 = (FStar_All.pipe_right binders (FStar_List.map binder_to_string))
in (FStar_All.pipe_right uu____3720 (FStar_String.concat " * ")))
in (

let uu____3725 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s * %s" uu____3719 uu____3725)))
end
| QForall (bs, pats, t) -> begin
(

let uu____3735 = (to_string_l " " binder_to_string bs)
in (

let uu____3736 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (

let uu____3738 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "forall %s.{:pattern %s} %s" uu____3735 uu____3736 uu____3738))))
end
| QExists (bs, pats, t) -> begin
(

let uu____3748 = (to_string_l " " binder_to_string bs)
in (

let uu____3749 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (

let uu____3751 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "exists %s.{:pattern %s} %s" uu____3748 uu____3749 uu____3751))))
end
| Refine (b, t) -> begin
(

let uu____3754 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3755 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:{%s}" uu____3754 uu____3755)))
end
| NamedTyp (x1, t) -> begin
(

let uu____3758 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" x1.FStar_Ident.idText uu____3758))
end
| Paren (t) -> begin
(

let uu____3760 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format1 "(%s)" uu____3760))
end
| Product (bs, t) -> begin
(
...
let uu____3766 = (FStar_All.pipe_right bs (FStar_List.map binder_to_string))
in (FStar_All.pipe_right uu____3766 (FStar_String.concat ",")))
in (

let uu____3771 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "Unidentified product: [%s] %s" uu____3765 uu____3771)))
end
| t -> begin
"_"
end))
...
let uu____3779 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" i.FStar_Ident.idText uu____3779))
end
| NoName (t) -> begin
...
let uu____3781 = (aqual_to_string x.aqual)
in (FStar_Util.format2 "%s%s" uu____3781 s))))
and aqual_to_string : aqual  ->  Prims.string = (fun uu___111_3782 -> (match (uu___111_3782) with
| Some (Equality) -> begin
"$"
end
| Some (Implicit) -> begin
...
let uu____3790 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3791 = (to_string_l " " pat_to_string ps)
in (FStar_Util.format2 "(%s %s)" uu____3790 uu____3791)))
end
| (PatTvar (i, aq)) | (PatVar (i, aq)) -> begin
(

let uu____3798 = (aqual_to_string aq)
in (FStar_Util.format2 "%s%s" uu____3798 i.FStar_Ident.idText))
end
| PatName (l) -> begin
l.FStar_Ident.str
end
...
let uu____3802 = (to_string_l "; " pat_to_string l)
in (FStar_Util.format1 "[%s]" uu____3802))
end
| PatTuple (l, false) -> begin
(

let uu____3806 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(%s)" uu____3806))
end
| PatTuple (l, true) -> begin
(

let uu____3810 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(|%s|)" uu____3810))
end
| PatRecord (l) -> begin
(

let uu____3815 = (to_string_l "; " (fun uu____3818 -> (match (uu____3818) with
| (f, e) -> begin
(

let uu____3823 = (FStar_All.pipe_right e pat_to_string)
in (FStar_Util.format2 "%s=%s" f.FStar_Ident.str uu____3823))
end)) l)
in (FStar_Util.format1 "{%s}" uu____3815))
end
| PatOr (l) -> begin
(to_string_l "|\n " pat_to_string l)
end
...
let uu____3829 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3830 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(%s:%s)" uu____3829 uu____3830)))
end))
...
| PatVar (i, uu____3838) -> begin
(

let uu____3841 = (FStar_Ident.lid_of_ids ((i)::[]))
in (uu____3841)::[])
end
| PatApp (p1, uu____3843) -> begin
(head_id_of_pat p1)
end
| PatAscribed (p1, uu____3847) -> begin
(head_id_of_pat p1)
end
| uu____3848 -> begin
[]
...
let lids_of_let = (fun defs -> (FStar_All.pipe_right defs (FStar_List.collect (fun uu____3869 -> (match (uu____3869) with
| (p, uu____3874) -> begin
(head_id_of_pat p)
end)))))


let id_of_tycon : tycon  ->  Prims.string = (fun uu___112_3877 -> (match (uu___112_3877) with
...
| TopLevelLet (uu____3918, pats) -> begin
(
...
let uu____3929 = (lids_of_let pats)
in (FStar_All.pipe_right uu____3929 (FStar_List.map (fun l -> l.FStar_Ident.str))))
in (FStar_All.pipe_right uu____3927 (FStar_String.concat ", ")))
in (Prims.strcat "let " uu____3926))
end
| Main (uu____3935) -> begin
"main ..."
end
| Assume (i, uu____3937) -> begin
(Prims.strcat "assume " i.FStar_Ident.idText)
end
| Tycon (uu____3938, tys) -> begin
(

let uu____3948 = (

let uu____3949 = (FStar_All.pipe_right tys (FStar_List.map (fun uu____3959 -> (match (uu____3959) with
| (x, uu____3964) -> begin
(id_of_tycon x)
end))))
in (FStar_All.pipe_right uu____3949 (FStar_String.concat ", ")))
in (Prims.strcat "type " uu____3948))
end
| Val (i, uu____3969) -> begin
(Prims.strcat "val " i.FStar_Ident.idText)
end
| Exception (i, uu____3971) -> begin
(Prims.strcat "exception " i.FStar_Ident.idText)
end
| (NewEffect (DefineEffect (i, _, _, _))) | (NewEffect (RedefineEffect (i, _, _))) -> begin
(Prims.strcat "new_effect " i.FStar_Ident.idText)
end
| SubEffect (uu____3983) -> begin
"sub_effect"
end
| Pragma (uu____3984) -> begin
"pragma"
end
| Fsdoc (uu____3985) -> begin
"fsdoc"
end))
...
let uu____3997 = (FStar_All.pipe_right decls (FStar_List.map decl_to_string))
in (FStar_All.pipe_right uu____3997 (FStar_String.concat "\n")))
end))
...
let uu____4024 = (FStar_Util.substring tm1 (Prims.parse_int "0") (Prims.parse_int "77"))
in (Prims.strcat uu____4024 "..."))
end
| uu____4025 -> begin
tm1
end)
