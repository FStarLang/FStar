...
(let _148_1003 = (let _148_1002 = (let _148_1001 = (FStar_Util.format1 "Invalid identifer \'%s\'; expected a symbol that begins with a lower-case character" id.FStar_Ident.idText)
in (_148_1001, id.FStar_Ident.idRange))
in FStar_Syntax_Syntax.Error (_148_1002))
in (Prims.raise _148_1003))
end)
end else begin
()
end)
...
in (let _148_1043 = (let _148_1042 = (let _148_1041 = (let _148_1040 = (let _148_1039 = (let _148_1038 = (let _148_1037 = (let _148_1036 = (FStar_Ident.lid_of_ids ((x)::[]))
in Var (_148_1036))
in (mk_term _148_1037 r1 Expr))
in (_148_1038, branches))
in Match (_148_1039))
in (mk_term _148_1040 r2 Expr))
in (((mk_pattern (PatVar ((x, None))) r1))::[], _148_1041))
in Abs (_148_1042))
in (mk_term _148_1043 r2 Expr))))
...
let lid_with_range : FStar_Ident.lident  ->  FStar_Range.range  ->  FStar_Ident.lident = (fun lid r -> (let _148_1052 = (FStar_Ident.path_of_lid lid)
in (FStar_Ident.lid_of_path _148_1052 r)))
...
let empty = (let _148_1096 = (let _148_1095 = (FStar_Ident.set_lid_range FStar_Absyn_Const.set_empty r)
in Var (_148_1095))
in (mk_term _148_1096 r Expr))
in (

let ref_constr = (let _148_1098 = (let _148_1097 = (FStar_Ident.set_lid_range FStar_Absyn_Const.heap_ref r)
in Var (_148_1097))
in (mk_term _148_1098 r Expr))
in (

let singleton = (let _148_1100 = (let _148_1099 = (FStar_Ident.set_lid_range FStar_Absyn_Const.set_singleton r)
in Var (_148_1099))
in (mk_term _148_1100 r Expr))
in (

let union = (let _148_1102 = (let _148_1101 = (FStar_Ident.set_lid_range FStar_Absyn_Const.set_union r)
in Var (_148_1101))
in (mk_term _148_1102 r Expr))
...
(match (t.tm) with
| Name (s) -> begin
(let _148_1114 = (let _148_1113 = (let _148_1112 = (FStar_List.map (fun a -> (a, Nothing)) args)
in (s, _148_1112))
in Construct (_148_1113))
in (mk_term _148_1114 r Un))
end
| _58_315 -> begin
...
let admit_name = (let _148_1120 = (let _148_1119 = (FStar_Ident.set_lid_range FStar_Absyn_Const.admit_lid r)
in Var (_148_1119))
in (mk_term _148_1120 r Expr))
in (mkExplicitApp admit_name ((unit_const)::[]) r))
in (

let magic = (

let magic_name = (let _148_1122 = (let _148_1121 = (FStar_Ident.set_lid_range FStar_Absyn_Const.magic_lid r)
in Var (_148_1121))
in (mk_term _148_1122 r Expr))
...
let mkWildAdmitMagic = (fun r -> (let _148_1124 = (mkAdmitMagic r)
in ((mk_pattern PatWild r), None, _148_1124)))
...
let focussed = (let _148_1127 = (FStar_List.filter Prims.fst branches)
in (FStar_All.pipe_right _148_1127 (FStar_List.map Prims.snd)))
in (let _148_1129 = (let _148_1128 = (mkWildAdmitMagic r)
in (_148_1128)::[])
in (FStar_List.append focussed _148_1129))))
end else begin
...
(let _148_1133 = (mkAdmitMagic r)
in ((Prims.fst lb), _148_1133))
end
end)) lbs))
end else begin
...
let mkFsTypApp : term  ->  term Prims.list  ->  FStar_Range.range  ->  term = (fun t args r -> (let _148_1141 = (FStar_List.map (fun a -> (a, FsTypApp)) args)
in (mkApp t _148_1141 r)))
...
in (let _148_1147 = (FStar_List.map (fun x -> (x, Nothing)) args)
in (mkApp (mk_term (Name (cons)) r Expr) _148_1147 r))))
...
in (let _148_1153 = (FStar_List.map (fun x -> (x, Nothing)) args)
in (mkApp (mk_term (Name (cons)) r Expr) _148_1153 r))))
...
let to_string_l = (fun sep f l -> (let _148_1174 = (FStar_List.map f l)
in (FStar_String.concat sep _148_1174)))
...
(let _148_1182 = (term_to_string t)
in (FStar_Util.format1 "(requires %s)" _148_1182))
end
| Ensures (t, _58_404) -> begin
(let _148_1183 = (term_to_string t)
in (FStar_Util.format1 "(ensures %s)" _148_1183))
end
| Labeled (t, l, _58_410) -> begin
(let _148_1184 = (term_to_string t)
in (FStar_Util.format2 "(labeled %s %s)" l _148_1184))
end
| Const (c) -> begin
(FStar_Absyn_Print.const_to_string c)
end
| Op (s, xs) -> begin
(let _148_1187 = (let _148_1186 = (FStar_List.map (fun x -> (FStar_All.pipe_right x term_to_string)) xs)
in (FStar_String.concat ", " _148_1186))
in (FStar_Util.format2 "%s(%s)" s _148_1187))
end
| Tvar (id) -> begin
...
(let _148_1190 = (to_string_l " " (fun _58_431 -> (match (_58_431) with
| (a, imp) -> begin
(let _148_1189 = (term_to_string a)
in (FStar_Util.format2 "%s%s" (imp_to_string imp) _148_1189))
end)) args)
in (FStar_Util.format2 "(%s %s)" l.FStar_Ident.str _148_1190))
end
| Abs (pats, t) when (x.level = Expr) -> begin
(let _148_1192 = (to_string_l " " pat_to_string pats)
in (let _148_1191 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(fun %s -> %s)" _148_1192 _148_1191)))
end
| Abs (pats, t) when (x.level = Type) -> begin
(let _148_1194 = (to_string_l " " pat_to_string pats)
in (let _148_1193 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(fun %s => %s)" _148_1194 _148_1193)))
end
| App (t1, t2, imp) -> begin
(let _148_1196 = (FStar_All.pipe_right t1 term_to_string)
in (let _148_1195 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format3 "%s %s%s" _148_1196 (imp_to_string imp) _148_1195)))
end
| Let (Rec, lbs, body) -> begin
(let _148_1201 = (to_string_l " and " (fun _58_452 -> (match (_58_452) with
| (p, b) -> begin
(let _148_1199 = (FStar_All.pipe_right p pat_to_string)
in (let _148_1198 = (FStar_All.pipe_right b term_to_string)
in (FStar_Util.format2 "%s=%s" _148_1199 _148_1198)))
end)) lbs)
in (let _148_1200 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format2 "let rec %s in %s" _148_1201 _148_1200)))
end
| Let (q, (pat, tm)::[], body) -> begin
(let _148_1204 = (FStar_All.pipe_right pat pat_to_string)
in (let _148_1203 = (FStar_All.pipe_right tm term_to_string)
in (let _148_1202 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format4 "let %s %s = %s in %s" (string_of_let_qualifier q) _148_1204 _148_1203 _148_1202))))
end
| Seq (t1, t2) -> begin
(let _148_1206 = (FStar_All.pipe_right t1 term_to_string)
in (let _148_1205 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "%s; %s" _148_1206 _148_1205)))
end
| If (t1, t2, t3) -> begin
(let _148_1209 = (FStar_All.pipe_right t1 term_to_string)
in (let _148_1208 = (FStar_All.pipe_right t2 term_to_string)
in (let _148_1207 = (FStar_All.pipe_right t3 term_to_string)
in (FStar_Util.format3 "if %s then %s else %s" _148_1209 _148_1208 _148_1207))))
end
| Match (t, branches) -> begin
(let _148_1216 = (FStar_All.pipe_right t term_to_string)
in (let _148_1215 = (to_string_l " | " (fun _58_477 -> (match (_58_477) with
| (p, w, e) -> begin
(let _148_1214 = (FStar_All.pipe_right p pat_to_string)
in (let _148_1213 = (match (w) with
| None -> begin
""
end
| Some (e) -> begin
(let _148_1211 = (term_to_string e)
in (FStar_Util.format1 "when %s" _148_1211))
end)
in (let _148_1212 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format3 "%s %s -> %s" _148_1214 _148_1213 _148_1212))))
end)) branches)
in (FStar_Util.format2 "match %s with %s" _148_1216 _148_1215)))
end
| Ascribed (t1, t2) -> begin
(let _148_1218 = (FStar_All.pipe_right t1 term_to_string)
in (let _148_1217 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "(%s : %s)" _148_1218 _148_1217)))
end
| Record (Some (e), fields) -> begin
(let _148_1222 = (FStar_All.pipe_right e term_to_string)
in (let _148_1221 = (to_string_l " " (fun _58_492 -> (match (_58_492) with
| (l, e) -> begin
(let _148_1220 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str _148_1220))
end)) fields)
in (FStar_Util.format2 "{%s with %s}" _148_1222 _148_1221)))
end
| Record (None, fields) -> begin
(let _148_1225 = (to_string_l " " (fun _58_499 -> (match (_58_499) with
| (l, e) -> begin
(let _148_1224 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str _148_1224))
end)) fields)
in (FStar_Util.format1 "{%s}" _148_1225))
end
| Project (e, l) -> begin
(let _148_1226 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s.%s" _148_1226 l.FStar_Ident.str))
end
| Product ([], t) -> begin
(term_to_string t)
end
...
(let _148_1228 = (FStar_All.pipe_right b binder_to_string)
in (let _148_1227 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s -> %s" _148_1228 _148_1227)))
end
| Product (b::[], t) when (x.level = Kind) -> begin
(let _148_1230 = (FStar_All.pipe_right b binder_to_string)
in (let _148_1229 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s => %s" _148_1230 _148_1229)))
end
| Sum (binders, t) -> begin
(let _148_1233 = (let _148_1231 = (FStar_All.pipe_right binders (FStar_List.map binder_to_string))
in (FStar_All.pipe_right _148_1231 (FStar_String.concat " * ")))
in (let _148_1232 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s * %s" _148_1233 _148_1232)))
end
| QForall (bs, pats, t) -> begin
(let _148_1236 = (to_string_l " " binder_to_string bs)
in (let _148_1235 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (let _148_1234 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "forall %s.{:pattern %s} %s" _148_1236 _148_1235 _148_1234))))
end
| QExists (bs, pats, t) -> begin
(let _148_1239 = (to_string_l " " binder_to_string bs)
in (let _148_1238 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (let _148_1237 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "exists %s.{:pattern %s} %s" _148_1239 _148_1238 _148_1237))))
end
| Refine (b, t) -> begin
(let _148_1241 = (FStar_All.pipe_right b binder_to_string)
in (let _148_1240 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:{%s}" _148_1241 _148_1240)))
end
| NamedTyp (x, t) -> begin
(let _148_1242 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" x.FStar_Ident.idText _148_1242))
end
| Paren (t) -> begin
(let _148_1243 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format1 "(%s)" _148_1243))
end
| Product (bs, t) -> begin
(let _148_1246 = (let _148_1244 = (FStar_All.pipe_right bs (FStar_List.map binder_to_string))
in (FStar_All.pipe_right _148_1244 (FStar_String.concat ",")))
in (let _148_1245 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "Unidentified product: [%s] %s" _148_1246 _148_1245)))
end
| t -> begin
...
(let _148_1248 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" i.FStar_Ident.idText _148_1248))
end
| NoName (t) -> begin
(FStar_All.pipe_right t term_to_string)
end)
in (let _148_1249 = (aqual_to_string x.aqual)
in (FStar_Util.format2 "%s%s" _148_1249 s))))
...
| PatApp (p, ps) -> begin
(let _148_1253 = (FStar_All.pipe_right p pat_to_string)
in (let _148_1252 = (to_string_l " " pat_to_string ps)
in (FStar_Util.format2 "(%s %s)" _148_1253 _148_1252)))
end
| (PatTvar (i, aq)) | (PatVar (i, aq)) -> begin
(let _148_1254 = (aqual_to_string aq)
in (FStar_Util.format2 "%s%s" _148_1254 i.FStar_Ident.idText))
end
| PatName (l) -> begin
l.FStar_Ident.str
end
| PatList (l) -> begin
(let _148_1255 = (to_string_l "; " pat_to_string l)
in (FStar_Util.format1 "[%s]" _148_1255))
end
| PatTuple (l, false) -> begin
(let _148_1256 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(%s)" _148_1256))
end
| PatTuple (l, true) -> begin
(let _148_1257 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(|%s|)" _148_1257))
end
| PatRecord (l) -> begin
(let _148_1260 = (to_string_l "; " (fun _58_606 -> (match (_58_606) with
| (f, e) -> begin
(let _148_1259 = (FStar_All.pipe_right e pat_to_string)
in (FStar_Util.format2 "%s=%s" f.FStar_Ident.str _148_1259))
end)) l)
in (FStar_Util.format1 "{%s}" _148_1260))
end
| PatOr (l) -> begin
(to_string_l "|\n " pat_to_string l)
end
| PatAscribed (p, t) -> begin
(let _148_1262 = (FStar_All.pipe_right p pat_to_string)
in (let _148_1261 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(%s:%s)" _148_1262 _148_1261)))
end))
...
(let _148_1265 = (FStar_Ident.lid_of_ids ((i)::[]))
in (_148_1265)::[])
end
...
(let _148_1275 = (let _148_1274 = (let _148_1273 = (lids_of_let pats)
in (FStar_All.pipe_right _148_1273 (FStar_List.map (fun l -> l.FStar_Ident.str))))
in (FStar_All.pipe_right _148_1274 (FStar_String.concat ", ")))
in (Prims.strcat "let " _148_1275))
end
| Main (_58_695) -> begin
"main ..."
end
...
(let _148_1277 = (let _148_1276 = (FStar_All.pipe_right tys (FStar_List.map id_of_tycon))
in (FStar_All.pipe_right _148_1276 (FStar_String.concat ", ")))
in (Prims.strcat "type " _148_1277))
end
...
(let _148_1280 = (FStar_All.pipe_right decls (FStar_List.map decl_to_string))
in (FStar_All.pipe_right _148_1280 (FStar_String.concat "\n")))
end))
...
(let _148_1284 = (FStar_Util.substring tm 0 77)
in (Prims.strcat _148_1284 "..."))
end else begin
tm
end
