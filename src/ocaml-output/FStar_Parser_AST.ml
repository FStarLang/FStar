...
let check_id : FStar_Ident.ident -> unit =
  fun id1  ->
    let first_char =
      FStar_String.substring id1.FStar_Ident.idText
...
let compile_op :
  ...
  fun arity  ->
    fun s  ->
      fun r  ->
        let name_of_char uu___42_6090 =
          match uu___42_6090 with
          | 38 -> "Amp"
          | 64 -> "At"
          | 43 -> "Plus"
          | 45 when arity = (Prims.lift_native_int (1)) -> "Minus"
          | 45 -> "Subtraction"
          | 126 -> "Tilde"
          | 47 -> "Slash"
          | 92 -> "Backslash"
...
let compile_op' : Prims.string -> FStar_Range.range -> Prims.string =
  fun s  -> fun r  -> compile_op (~- (Prims.lift_native_int (1))) s r 
let string_of_fsdoc :
  (Prims.string,(Prims.string,Prims.string) FStar_Pervasives_Native.tuple2
                  Prims.list)
...
let imp_to_string : imp -> Prims.string =
  fun uu___44_6237  ->
    match uu___44_6237 with | Hash  -> "#" | uu____6238 -> ""
