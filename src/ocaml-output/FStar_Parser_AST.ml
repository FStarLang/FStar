...
let uu____2428 = (FStar_Util.format1 "Invalid identifer \'%s\'; expected a symbol that begins with a lower-case character" id.FStar_Ident.idText)
in ((uu____2428), (id.FStar_Ident.idRange)))
in FStar_Errors.Error (uu____2425))
in (Prims.raise uu____2424))
end)))
...
let uu____2456 = (FStar_Util.format1 "At most one %s is allowed on declarations" s)
in ((uu____2456), (r)))
in FStar_Errors.Error (uu____2453))
in (Prims.raise uu____2452))
end))
...
let uu____2471 = (FStar_List.choose (fun uu___104_2473 -> (match (uu___104_2473) with
| Doc (d) -> begin
Some (d)
end
| uu____2476 -> begin
...
in (at_most_one "fsdoc" r uu____2471))
in (

let attributes_ = (

let uu____2480 = (FStar_List.choose (fun uu___105_2484 -> (match (uu___105_2484) with
...
in (at_most_one "attribute set" r uu____2480))
in (

let attributes_ = (FStar_Util.dflt [] attributes_)
in (

let qualifiers = (FStar_List.choose (fun uu___106_2498 -> (match (uu___106_2498) with
| Qualifier (q) -> begin
Some (q)
end
| uu____2501 -> begin
...
let uu____2606 = (FStar_Ident.lid_of_ids ((x)::[]))
in Var (uu____2606))
in (mk_term uu____2605 r1 Expr))
in ((uu____2604), (branches)))
in Match (uu____2596))
in (mk_term uu____2595 r2 Expr))
in ((((mk_pattern (PatVar (((x), (None)))) r1))::[]), (uu____2594)))
in Abs (uu____2590))
in (mk_term uu____2589 r2 Expr))))


let un_function : pattern  ->  term  ->  (pattern * term) Prims.option = (fun p tm -> (match (((p.pat), (tm.tm))) with
| (PatVar (uu____2626), Abs (pats, body)) -> begin
...
let uu____2648 = (FStar_Ident.path_of_lid lid)
in (FStar_Ident.lid_of_path uu____2648 r)))
...
| uu____2763 -> begin
(FStar_List.fold_left (fun t uu____2767 -> (match (uu____2767) with
| (a, imp) -> begin
...
let uu____2780 = ((FStar_Syntax_Const.tset_empty), (FStar_Syntax_Const.tset_singleton), (FStar_Syntax_Const.tset_union))
...
let uu____2830 = (FStar_List.map (fun a -> ((a), (Nothing))) args)
in ((s), (uu____2830)))
in Construct (uu____2824))
in (mk_term uu____2823 r Un))
end
| uu____2840 -> begin
...
let uu____2863 = (mkAdmitMagic r)
in (((mk_pattern PatWild r)), (None), (uu____2863))))
...
let uu____2919 = (FStar_List.filter Prims.fst branches)
in (FStar_All.pipe_right uu____2919 (FStar_List.map Prims.snd)))
in (

let uu____2963 = (

let uu____2969 = (mkWildAdmitMagic r)
in (uu____2969)::[])
in (FStar_List.append focussed uu____2963)));
)
end
| uu____2986 -> begin
...
(FStar_List.map (fun uu____3055 -> (match (uu____3055) with
| (f, lb) -> begin
(match (f) with
| true -> begin
lb
...
let uu____3071 = (mkAdmitMagic r)
in (((Prims.fst lb)), (uu____3071)))
end)
end)) lbs);
)
end
...
let uu____3100 = (FStar_List.map (fun a -> ((a), (FsTypApp))) args)
in (mkApp t uu____3100 r)))
...
let uu____3119 = (FStar_List.map (fun x -> ((x), (Nothing))) args)
in (mkApp (mk_term (Name (cons)) r Expr) uu____3119 r))))
...
let uu____3138 = (FStar_List.map (fun x -> ((x), (Nothing))) args)
in (mkApp (mk_term (Name (cons)) r Expr) uu____3138 r))))
...
let uu____3201 = (FStar_Ident.lid_of_ids ((x)::[]))
in Var (uu____3201))
in (mk_term uu____3200 phi.range Formula))
in (
...
let uu____3215 = (FStar_Ident.lid_of_path (("False")::[]) phi.range)
in Name (uu____3215))
in (mk_term uu____3214 phi.range Formula))
in (((mk_pattern PatWild phi.range)), (None), (uu____3213)))
...
| Refine ({b = Annotated (x, t); brange = uu____3258; blevel = uu____3259; aqual = uu____3260}, t') -> begin
Some (((x), (t), (Some (t'))))
end
| Paren (t) -> begin
...
let uu____3357 = (mk_decl (Pragma (LightOff)) light_range [])
in (uu____3357)::ds)
end
| uu____3358 -> begin
ds
...
let uu____3365 = (FStar_List.tl ms)
in (match (uu____3365) with
| (Module (m', uu____3368))::uu____3369 -> begin
(

let msg = "Support for more than one module in a file is deprecated"
in (

let uu____3374 = (FStar_Range.string_of_range (FStar_Ident.range_of_lid m'))
...
let ds = (d)::ds
in ((FStar_List.iter (fun uu___107_3383 -> (match (uu___107_3383) with
| {d = TopLevelModule (uu____3384); drange = r; doc = uu____3386; quals = uu____3387; attrs = uu____3388} -> begin
...
let name_of_char = (fun uu___108_3402 -> (match (uu___108_3402) with
| '&' -> begin
"Amp"
end
| '@' -> begin
...
let uu____3408 = (FStar_String.list_of_string s)
in (FStar_List.map name_of_char uu____3408))
in (FStar_String.concat "_" uu____3406))
in (Prims.strcat "op_" uu____3405))
end)))
...
let string_of_fsdoc : (Prims.string * (Prims.string * Prims.string) Prims.list)  ->  Prims.string = (fun uu____3420 -> (match (uu____3420) with
| (comment, keywords) -> begin
(

let uu____3434 = (

let uu____3435 = (FStar_List.map (fun uu____3439 -> (match (uu____3439) with
| (k, v) -> begin
(Prims.strcat k (Prims.strcat "->" v))
end)) keywords)
in (FStar_String.concat "," uu____3435))
in (Prims.strcat comment uu____3434))
end))


let string_of_let_qualifier : let_qualifier  ->  Prims.string = (fun uu___109_3446 -> (match (uu___109_3446) with
| NoLetQualifier -> begin
""
end
| Rec -> begin
...
let uu____3471 = (FStar_List.map f l)
in (FStar_String.concat sep uu____3471)))


let imp_to_string : imp  ->  Prims.string = (fun uu___110_3475 -> (match (uu___110_3475) with
| Hash -> begin
"#"
end
| uu____3476 -> begin
...
| Requires (t, uu____3487) -> begin
(

let uu____3490 = (term_to_string t)
in (FStar_Util.format1 "(requires %s)" uu____3490))
end
| Ensures (t, uu____3492) -> begin
(

let uu____3495 = (term_to_string t)
in (FStar_Util.format1 "(ensures %s)" uu____3495))
end
| Labeled (t, l, uu____3498) -> begin
(

let uu____3499 = (term_to_string t)
in (FStar_Util.format2 "(labeled %s %s)" l uu____3499))
end
| Const (c) -> begin
...
let uu____3506 = (FStar_List.map (fun x -> (FStar_All.pipe_right x term_to_string)) xs)
in (FStar_String.concat ", " uu____3506))
in (FStar_Util.format2 "%s(%s)" s uu____3505))
end
...
let uu____3519 = (to_string_l " " (fun uu____3522 -> (match (uu____3522) with
| (a, imp) -> begin
(

let uu____3527 = (term_to_string a)
in (FStar_Util.format2 "%s%s" (imp_to_string imp) uu____3527))
end)) args)
in (FStar_Util.format2 "(%s %s)" l.FStar_Ident.str uu____3519))
end
| Abs (pats, t) -> begin
(

let uu____3532 = (to_string_l " " pat_to_string pats)
in (

let uu____3533 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(fun %s -> %s)" uu____3532 uu____3533)))
end
| App (t1, t2, imp) -> begin
(

let uu____3537 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3538 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format3 "%s %s%s" uu____3537 (imp_to_string imp) uu____3538)))
end
| Let (Rec, lbs, body) -> begin
(

let uu____3547 = (to_string_l " and " (fun uu____3550 -> (match (uu____3550) with
| (p, b) -> begin
(

let uu____3555 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3556 = (FStar_All.pipe_right b term_to_string)
in (FStar_Util.format2 "%s=%s" uu____3555 uu____3556)))
end)) lbs)
in (

let uu____3557 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format2 "let rec %s in %s" uu____3547 uu____3557)))
end
| Let (q, ((pat, tm))::[], body) -> begin
(

let uu____3569 = (FStar_All.pipe_right pat pat_to_string)
in (

let uu____3570 = (FStar_All.pipe_right tm term_to_string)
in (

let uu____3571 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format4 "let %s %s = %s in %s" (string_of_let_qualifier q) uu____3569 uu____3570 uu____3571))))
end
| Seq (t1, t2) -> begin
(

let uu____3574 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3575 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "%s; %s" uu____3574 uu____3575)))
end
| If (t1, t2, t3) -> begin
(

let uu____3579 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3580 = (FStar_All.pipe_right t2 term_to_string)
in (

let uu____3581 = (FStar_All.pipe_right t3 term_to_string)
in (FStar_Util.format3 "if %s then %s else %s" uu____3579 uu____3580 uu____3581))))
end
| Match (t, branches) -> begin
(

let uu____3594 = (FStar_All.pipe_right t term_to_string)
in (

let uu____3595 = (to_string_l " | " (fun uu____3600 -> (match (uu____3600) with
| (p, w, e) -> begin
(

let uu____3610 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3611 = (match (w) with
| None -> begin
""
end
| Some (e) -> begin
...
let uu____3613 = (term_to_string e)
in (FStar_Util.format1 "when %s" uu____3613))
end)
in (

let uu____3614 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format3 "%s %s -> %s" uu____3610 uu____3611 uu____3614))))
end)) branches)
in (FStar_Util.format2 "match %s with %s" uu____3594 uu____3595)))
end
| Ascribed (t1, t2) -> begin
(

let uu____3617 = (FStar_All.pipe_right t1 term_to_string)
in (

let uu____3618 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "(%s : %s)" uu____3617 uu____3618)))
end
| Record (Some (e), fields) -> begin
(

let uu____3628 = (FStar_All.pipe_right e term_to_string)
in (

let uu____3629 = (to_string_l " " (fun uu____3632 -> (match (uu____3632) with
| (l, e) -> begin
(

let uu____3637 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3637))
end)) fields)
in (FStar_Util.format2 "{%s with %s}" uu____3628 uu____3629)))
end
| Record (None, fields) -> begin
(

let uu____3646 = (to_string_l " " (fun uu____3649 -> (match (uu____3649) with
| (l, e) -> begin
(

let uu____3654 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str uu____3654))
end)) fields)
in (FStar_Util.format1 "{%s}" uu____3646))
end
| Project (e, l) -> begin
(

let uu____3657 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s.%s" uu____3657 l.FStar_Ident.str))
end
| Product ([], t) -> begin
(term_to_string t)
end
...
let uu____3671 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3672 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s -> %s" uu____3671 uu____3672)))
end
| Product ((b)::[], t) when (x.level = Kind) -> begin
(

let uu____3676 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3677 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s => %s" uu____3676 uu____3677)))
end
| Sum (binders, t) -> begin
(
...
let uu____3683 = (FStar_All.pipe_right binders (FStar_List.map binder_to_string))
in (FStar_All.pipe_right uu____3683 (FStar_String.concat " * ")))
in (

let uu____3688 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s * %s" uu____3682 uu____3688)))
end
| QForall (bs, pats, t) -> begin
(

let uu____3698 = (to_string_l " " binder_to_string bs)
in (

let uu____3699 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (

let uu____3701 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "forall %s.{:pattern %s} %s" uu____3698 uu____3699 uu____3701))))
end
| QExists (bs, pats, t) -> begin
(

let uu____3711 = (to_string_l " " binder_to_string bs)
in (

let uu____3712 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (

let uu____3714 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "exists %s.{:pattern %s} %s" uu____3711 uu____3712 uu____3714))))
end
| Refine (b, t) -> begin
(

let uu____3717 = (FStar_All.pipe_right b binder_to_string)
in (

let uu____3718 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:{%s}" uu____3717 uu____3718)))
end
| NamedTyp (x, t) -> begin
(

let uu____3721 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" x.FStar_Ident.idText uu____3721))
end
| Paren (t) -> begin
(

let uu____3723 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format1 "(%s)" uu____3723))
end
| Product (bs, t) -> begin
(
...
let uu____3729 = (FStar_All.pipe_right bs (FStar_List.map binder_to_string))
in (FStar_All.pipe_right uu____3729 (FStar_String.concat ",")))
in (

let uu____3734 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "Unidentified product: [%s] %s" uu____3728 uu____3734)))
end
| t -> begin
"_"
end))
...
in (FStar_Util.format2 "%s:%s" i.FStar_Ident.idText uu____3742))
end
| NoName (t) -> begin
...
let uu____3744 = (aqual_to_string x.aqual)
in (FStar_Util.format2 "%s%s" uu____3744 s))))
and aqual_to_string : aqual  ->  Prims.string = (fun uu___111_3745 -> (match (uu___111_3745) with
| Some (Equality) -> begin
"$"
end
| Some (Implicit) -> begin
...
let uu____3753 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3754 = (to_string_l " " pat_to_string ps)
in (FStar_Util.format2 "(%s %s)" uu____3753 uu____3754)))
end
| (PatTvar (i, aq)) | (PatVar (i, aq)) -> begin
(

let uu____3761 = (aqual_to_string aq)
in (FStar_Util.format2 "%s%s" uu____3761 i.FStar_Ident.idText))
end
| PatName (l) -> begin
l.FStar_Ident.str
end
...
let uu____3765 = (to_string_l "; " pat_to_string l)
in (FStar_Util.format1 "[%s]" uu____3765))
end
| PatTuple (l, false) -> begin
(

let uu____3769 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(%s)" uu____3769))
end
| PatTuple (l, true) -> begin
(

let uu____3773 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(|%s|)" uu____3773))
end
| PatRecord (l) -> begin
(
...
let uu____3786 = (FStar_All.pipe_right e pat_to_string)
in (FStar_Util.format2 "%s=%s" f.FStar_Ident.str uu____3786))
end)) l)
...
let uu____3792 = (FStar_All.pipe_right p pat_to_string)
in (

let uu____3793 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(%s:%s)" uu____3792 uu____3793)))
end))
...
| PatVar (i, uu____3801) -> begin
(

let uu____3804 = (FStar_Ident.lid_of_ids ((i)::[]))
in (uu____3804)::[])
end
| PatApp (p, uu____3806) -> begin
(head_id_of_pat p)
end
| PatAscribed (p, uu____3810) -> begin
(head_id_of_pat p)
end
| uu____3811 -> begin
[]
...
let lids_of_let = (fun defs -> (FStar_All.pipe_right defs (FStar_List.collect (fun uu____3832 -> (match (uu____3832) with
| (p, uu____3837) -> begin
(head_id_of_pat p)
end)))))


let id_of_tycon : tycon  ->  Prims.string = (fun uu___112_3840 -> (match (uu___112_3840) with
...
| TopLevelLet (uu____3881, pats) -> begin
(
...
let uu____3892 = (lids_of_let pats)
in (FStar_All.pipe_right uu____3892 (FStar_List.map (fun l -> l.FStar_Ident.str))))
in (FStar_All.pipe_right uu____3890 (FStar_String.concat ", ")))
in (Prims.strcat "let " uu____3889))
end
| Main (uu____3898) -> begin
"main ..."
end
| Assume (i, uu____3900) -> begin
(Prims.strcat "assume " i.FStar_Ident.idText)
end
| Tycon (uu____3901, tys) -> begin
(

let uu____3911 = (

let uu____3912 = (FStar_All.pipe_right tys (FStar_List.map (fun uu____3922 -> (match (uu____3922) with
| (x, uu____3927) -> begin
(id_of_tycon x)
end))))
in (FStar_All.pipe_right uu____3912 (FStar_String.concat ", ")))
in (Prims.strcat "type " uu____3911))
end
| Val (i, uu____3932) -> begin
(Prims.strcat "val " i.FStar_Ident.idText)
end
| Exception (i, uu____3934) -> begin
(Prims.strcat "exception " i.FStar_Ident.idText)
end
| (NewEffect (DefineEffect (i, _, _, _))) | (NewEffect (RedefineEffect (i, _, _))) -> begin
(Prims.strcat "new_effect " i.FStar_Ident.idText)
end
| SubEffect (uu____3946) -> begin
"sub_effect"
end
| Pragma (uu____3947) -> begin
"pragma"
end
| Fsdoc (uu____3948) -> begin
"fsdoc"
end))
...
let uu____3960 = (FStar_All.pipe_right decls (FStar_List.map decl_to_string))
...
let uu____3987 = (FStar_Util.substring tm (Prims.parse_int "0") (Prims.parse_int "77"))
in (Prims.strcat uu____3987 "..."))
end
| uu____3988 -> begin
tm
end)
