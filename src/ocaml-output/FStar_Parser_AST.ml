...
let empty = (let _157_1197 = (let _157_1196 = (FStar_Ident.set_lid_range empty_lid r)
in Var (_157_1196))
in (mk_term _157_1197 r Expr))
in (

let ref_constr = (let _157_1199 = (let _157_1198 = (FStar_Ident.set_lid_range FStar_Absyn_Const.heap_ref r)
in Var (_157_1198))
in (mk_term _157_1199 r Expr))
in (

let singleton = (let _157_1201 = (let _157_1200 = (FStar_Ident.set_lid_range singleton_lid r)
in Var (_157_1200))
in (mk_term _157_1201 r Expr))
in (

let union = (let _157_1203 = (let _157_1202 = (FStar_Ident.set_lid_range union_lid r)
in Var (_157_1202))
in (mk_term _157_1203 r Expr))
...
(match (t.tm) with
| Name (s) -> begin
(let _157_1215 = (let _157_1214 = (let _157_1213 = (FStar_List.map (fun a -> ((a), (Nothing))) args)
in ((s), (_157_1213)))
in Construct (_157_1214))
in (mk_term _157_1215 r Un))
end
| _60_354 -> begin
...
let admit_name = (let _157_1221 = (let _157_1220 = (FStar_Ident.set_lid_range FStar_Absyn_Const.admit_lid r)
in Var (_157_1220))
in (mk_term _157_1221 r Expr))
in (mkExplicitApp admit_name ((unit_const)::[]) r))
in (

let magic = (

let magic_name = (let _157_1223 = (let _157_1222 = (FStar_Ident.set_lid_range FStar_Absyn_Const.magic_lid r)
in Var (_157_1222))
in (mk_term _157_1223 r Expr))
...
let mkWildAdmitMagic = (fun r -> (let _157_1225 = (mkAdmitMagic r)
in (((mk_pattern PatWild r)), (None), (_157_1225))))
...
let focussed = (let _157_1228 = (FStar_List.filter Prims.fst branches)
in (FStar_All.pipe_right _157_1228 (FStar_List.map Prims.snd)))
in (let _157_1230 = (let _157_1229 = (mkWildAdmitMagic r)
in (_157_1229)::[])
in (FStar_List.append focussed _157_1230))))
end else begin
...
(let _157_1234 = (mkAdmitMagic r)
in (((Prims.fst lb)), (_157_1234)))
end
end)) lbs))
end else begin
...
let mkFsTypApp : term  ->  term Prims.list  ->  FStar_Range.range  ->  term = (fun t args r -> (let _157_1242 = (FStar_List.map (fun a -> ((a), (FsTypApp))) args)
in (mkApp t _157_1242 r)))
...
in (let _157_1248 = (FStar_List.map (fun x -> ((x), (Nothing))) args)
in (mkApp (mk_term (Name (cons)) r Expr) _157_1248 r))))
...
in (let _157_1254 = (FStar_List.map (fun x -> ((x), (Nothing))) args)
in (mkApp (mk_term (Name (cons)) r Expr) _157_1254 r))))
...
(let _157_1271 = (let _157_1270 = (FStar_List.map (fun _60_428 -> (match (_60_428) with
| (k, v) -> begin
(Prims.strcat k (Prims.strcat "->" v))
end)) keywords)
in (FStar_String.concat "," _157_1270))
in (Prims.strcat comment _157_1271))
end))
...
let to_string_l = (fun sep f l -> (let _157_1280 = (FStar_List.map f l)
in (FStar_String.concat sep _157_1280)))
...
(let _157_1288 = (term_to_string t)
in (FStar_Util.format1 "(requires %s)" _157_1288))
end
| Ensures (t, _60_449) -> begin
(let _157_1289 = (term_to_string t)
in (FStar_Util.format1 "(ensures %s)" _157_1289))
end
| Labeled (t, l, _60_455) -> begin
(let _157_1290 = (term_to_string t)
in (FStar_Util.format2 "(labeled %s %s)" l _157_1290))
end
| Const (c) -> begin
(FStar_Absyn_Print.const_to_string c)
end
| Op (s, xs) -> begin
(let _157_1293 = (let _157_1292 = (FStar_List.map (fun x -> (FStar_All.pipe_right x term_to_string)) xs)
in (FStar_String.concat ", " _157_1292))
in (FStar_Util.format2 "%s(%s)" s _157_1293))
end
| Tvar (id) -> begin
...
(let _157_1296 = (to_string_l " " (fun _60_476 -> (match (_60_476) with
| (a, imp) -> begin
(let _157_1295 = (term_to_string a)
in (FStar_Util.format2 "%s%s" (imp_to_string imp) _157_1295))
end)) args)
in (FStar_Util.format2 "(%s %s)" l.FStar_Ident.str _157_1296))
end
| Abs (pats, t) -> begin
(let _157_1298 = (to_string_l " " pat_to_string pats)
in (let _157_1297 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(fun %s -> %s)" _157_1298 _157_1297)))
end
| App (t1, t2, imp) -> begin
(let _157_1300 = (FStar_All.pipe_right t1 term_to_string)
in (let _157_1299 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format3 "%s %s%s" _157_1300 (imp_to_string imp) _157_1299)))
end
| Let (Rec, lbs, body) -> begin
(let _157_1305 = (to_string_l " and " (fun _60_493 -> (match (_60_493) with
| (p, b) -> begin
(let _157_1303 = (FStar_All.pipe_right p pat_to_string)
in (let _157_1302 = (FStar_All.pipe_right b term_to_string)
in (FStar_Util.format2 "%s=%s" _157_1303 _157_1302)))
end)) lbs)
in (let _157_1304 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format2 "let rec %s in %s" _157_1305 _157_1304)))
end
| Let (q, ((pat, tm))::[], body) -> begin
(let _157_1308 = (FStar_All.pipe_right pat pat_to_string)
in (let _157_1307 = (FStar_All.pipe_right tm term_to_string)
in (let _157_1306 = (FStar_All.pipe_right body term_to_string)
in (FStar_Util.format4 "let %s %s = %s in %s" (string_of_let_qualifier q) _157_1308 _157_1307 _157_1306))))
end
| Seq (t1, t2) -> begin
...
| If (t1, t2, t3) -> begin
(let _157_1313 = (FStar_All.pipe_right t1 term_to_string)
in (let _157_1312 = (FStar_All.pipe_right t2 term_to_string)
in (let _157_1311 = (FStar_All.pipe_right t3 term_to_string)
in (FStar_Util.format3 "if %s then %s else %s" _157_1313 _157_1312 _157_1311))))
end
| Match (t, branches) -> begin
(let _157_1320 = (FStar_All.pipe_right t term_to_string)
in (let _157_1319 = (to_string_l " | " (fun _60_518 -> (match (_60_518) with
| (p, w, e) -> begin
(let _157_1318 = (FStar_All.pipe_right p pat_to_string)
in (let _157_1317 = (match (w) with
| None -> begin
""
end
| Some (e) -> begin
(let _157_1315 = (term_to_string e)
in (FStar_Util.format1 "when %s" _157_1315))
end)
in (let _157_1316 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format3 "%s %s -> %s" _157_1318 _157_1317 _157_1316))))
end)) branches)
in (FStar_Util.format2 "match %s with %s" _157_1320 _157_1319)))
end
| Ascribed (t1, t2) -> begin
(let _157_1322 = (FStar_All.pipe_right t1 term_to_string)
in (let _157_1321 = (FStar_All.pipe_right t2 term_to_string)
in (FStar_Util.format2 "(%s : %s)" _157_1322 _157_1321)))
end
| Record (Some (e), fields) -> begin
(let _157_1326 = (FStar_All.pipe_right e term_to_string)
in (let _157_1325 = (to_string_l " " (fun _60_533 -> (match (_60_533) with
| (l, e) -> begin
(let _157_1324 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str _157_1324))
end)) fields)
in (FStar_Util.format2 "{%s with %s}" _157_1326 _157_1325)))
end
| Record (None, fields) -> begin
(let _157_1329 = (to_string_l " " (fun _60_540 -> (match (_60_540) with
| (l, e) -> begin
(let _157_1328 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s=%s" l.FStar_Ident.str _157_1328))
end)) fields)
in (FStar_Util.format1 "{%s}" _157_1329))
end
| Project (e, l) -> begin
(let _157_1330 = (FStar_All.pipe_right e term_to_string)
in (FStar_Util.format2 "%s.%s" _157_1330 l.FStar_Ident.str))
end
| Product ([], t) -> begin
(term_to_string t)
end
...
(let _157_1332 = (FStar_All.pipe_right b binder_to_string)
in (let _157_1331 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s -> %s" _157_1332 _157_1331)))
end
| Product ((b)::[], t) when (x.level = Kind) -> begin
(let _157_1334 = (FStar_All.pipe_right b binder_to_string)
in (let _157_1333 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s => %s" _157_1334 _157_1333)))
end
| Sum (binders, t) -> begin
(let _157_1337 = (let _157_1335 = (FStar_All.pipe_right binders (FStar_List.map binder_to_string))
in (FStar_All.pipe_right _157_1335 (FStar_String.concat " * ")))
in (let _157_1336 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s * %s" _157_1337 _157_1336)))
end
| QForall (bs, pats, t) -> begin
(let _157_1340 = (to_string_l " " binder_to_string bs)
in (let _157_1339 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (let _157_1338 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "forall %s.{:pattern %s} %s" _157_1340 _157_1339 _157_1338))))
end
| QExists (bs, pats, t) -> begin
(let _157_1343 = (to_string_l " " binder_to_string bs)
in (let _157_1342 = (to_string_l " \\/ " (to_string_l "; " term_to_string) pats)
in (let _157_1341 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format3 "exists %s.{:pattern %s} %s" _157_1343 _157_1342 _157_1341))))
end
| Refine (b, t) -> begin
(let _157_1345 = (FStar_All.pipe_right b binder_to_string)
in (let _157_1344 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:{%s}" _157_1345 _157_1344)))
end
| NamedTyp (x, t) -> begin
(let _157_1346 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" x.FStar_Ident.idText _157_1346))
end
| Paren (t) -> begin
(let _157_1347 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format1 "(%s)" _157_1347))
end
| Product (bs, t) -> begin
(let _157_1350 = (let _157_1348 = (FStar_All.pipe_right bs (FStar_List.map binder_to_string))
in (FStar_All.pipe_right _157_1348 (FStar_String.concat ",")))
in (let _157_1349 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "Unidentified product: [%s] %s" _157_1350 _157_1349)))
end
| t -> begin
"_"
end))
...
(let _157_1352 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "%s:%s" i.FStar_Ident.idText _157_1352))
end
| NoName (t) -> begin
(FStar_All.pipe_right t term_to_string)
end)
in (let _157_1353 = (aqual_to_string x.aqual)
in (FStar_Util.format2 "%s%s" _157_1353 s))))
...
| PatApp (p, ps) -> begin
(let _157_1357 = (FStar_All.pipe_right p pat_to_string)
in (let _157_1356 = (to_string_l " " pat_to_string ps)
in (FStar_Util.format2 "(%s %s)" _157_1357 _157_1356)))
end
| (PatTvar (i, aq)) | (PatVar (i, aq)) -> begin
(let _157_1358 = (aqual_to_string aq)
in (FStar_Util.format2 "%s%s" _157_1358 i.FStar_Ident.idText))
end
| PatName (l) -> begin
l.FStar_Ident.str
end
| PatList (l) -> begin
(let _157_1359 = (to_string_l "; " pat_to_string l)
in (FStar_Util.format1 "[%s]" _157_1359))
end
| PatTuple (l, false) -> begin
(let _157_1360 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(%s)" _157_1360))
end
| PatTuple (l, true) -> begin
(let _157_1361 = (to_string_l ", " pat_to_string l)
in (FStar_Util.format1 "(|%s|)" _157_1361))
end
| PatRecord (l) -> begin
(let _157_1364 = (to_string_l "; " (fun _60_647 -> (match (_60_647) with
| (f, e) -> begin
(let _157_1363 = (FStar_All.pipe_right e pat_to_string)
in (FStar_Util.format2 "%s=%s" f.FStar_Ident.str _157_1363))
end)) l)
in (FStar_Util.format1 "{%s}" _157_1364))
end
| PatOr (l) -> begin
(to_string_l "|\n " pat_to_string l)
end
...
(let _157_1366 = (FStar_All.pipe_right p pat_to_string)
in (let _157_1365 = (FStar_All.pipe_right t term_to_string)
in (FStar_Util.format2 "(%s:%s)" _157_1366 _157_1365)))
end))
...
(let _157_1369 = (FStar_Ident.lid_of_ids ((i)::[]))
in (_157_1369)::[])
end
...
(let _157_1379 = (let _157_1378 = (let _157_1377 = (lids_of_let pats)
in (FStar_All.pipe_right _157_1377 (FStar_List.map (fun l -> l.FStar_Ident.str))))
in (FStar_All.pipe_right _157_1378 (FStar_String.concat ", ")))
in (Prims.strcat "let " _157_1379))
end
| Main (_60_738) -> begin
"main ..."
end
...
(let _157_1382 = (let _157_1381 = (FStar_All.pipe_right tys (FStar_List.map (fun _60_755 -> (match (_60_755) with
| (x, _60_754) -> begin
(id_of_tycon x)
end))))
in (FStar_All.pipe_right _157_1381 (FStar_String.concat ", ")))
in (Prims.strcat "type " _157_1382))
end
...
(let _157_1385 = (FStar_All.pipe_right decls (FStar_List.map decl_to_string))
in (FStar_All.pipe_right _157_1385 (FStar_String.concat "\n")))
end))
...
(let _157_1389 = (FStar_Util.substring tm (Prims.parse_int "0") (Prims.parse_int "77"))
in (Prims.strcat _157_1389 "..."))
end else begin
tm
end
