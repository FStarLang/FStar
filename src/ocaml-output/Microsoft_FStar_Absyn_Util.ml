...
(let _68721 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format3 "%s : %s\n%s\n" (Support.Microsoft.FStar.Range.string_of_range r) (if warning then begin
"Warning"
end else begin
"Error"
end) msg))
...
(let _68725 = (Support.Microsoft.FStar.Util.print_string (Support.Microsoft.FStar.Util.format1 "Feature not yet implemented: %s" s))
in ret)
end
...
let handleable = (fun _68689 -> (match (_68689) with
...
let mkbvd = (fun _68760 -> (match (_68760) with
| (x, y) -> begin
...
let arg_of_non_null_binder = (fun _68856 -> (match (_68856) with
| (b, imp) -> begin
(match (b) with
...
let null_binders_of_tks = (fun tks -> ((Support.List.map (fun _68690 -> (match (_68690) with
...
let binders_of_tks = (fun tks -> ((Support.List.map (fun _68691 -> (match (_68691) with
| (Support.Microsoft.FStar.Util.Inl (k), imp) -> begin
...
let subst_to_string = (fun s -> ((Support.String.concat ", ") ((Support.List.map (fun _68692 -> (match (_68692) with
...
let subst_tvar = (fun s a -> (Support.Microsoft.FStar.Util.find_map s (fun _68693 -> (match (_68693) with
...
let subst_xvar = (fun s a -> (Support.Microsoft.FStar.Util.find_map s (fun _68694 -> (match (_68694) with
...
(let t = (mk_t ())
in (let _68964 = (Support.ST.op_ColonEquals m (Some (t)))
in (subst_typ' s t)))
end
...
and subst_flags' = (fun s flags -> ((Support.List.map (fun _68695 -> (match (_68695) with
...
| _ -> begin
(let _69053 = t
in {Microsoft_FStar_Absyn_Syntax.effect_name = _69053.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = (subst_typ' s t.Microsoft_FStar_Absyn_Syntax.result_typ); Microsoft_FStar_Absyn_Syntax.effect_args = (Support.List.map (fun _68696 -> (match (_68696) with
...
let subst_binder = (fun s _68697 -> (match (_68697) with
| (Support.Microsoft.FStar.Util.Inl (a), imp) -> begin
(Support.Microsoft.FStar.Util.Inl ((let _69094 = a
in {Microsoft_FStar_Absyn_Syntax.v = _69094.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = (subst_kind s a.Microsoft_FStar_Absyn_Syntax.sort); Microsoft_FStar_Absyn_Syntax.p = _69094.Microsoft_FStar_Absyn_Syntax.p})), imp)
end
| (Support.Microsoft.FStar.Util.Inr (x), imp) -> begin
(Support.Microsoft.FStar.Util.Inr ((let _69100 = x
in {Microsoft_FStar_Absyn_Syntax.v = _69100.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = (subst_typ s x.Microsoft_FStar_Absyn_Syntax.sort); Microsoft_FStar_Absyn_Syntax.p = _69100.Microsoft_FStar_Absyn_Syntax.p})), imp)
end))

let subst_arg = (fun s _68698 -> (match (_68698) with
| (Support.Microsoft.FStar.Util.Inl (t), imp) -> begin
(Support.Microsoft.FStar.Util.Inl ((subst_typ s t)), imp)
end
| (Support.Microsoft.FStar.Util.Inr (e), imp) -> begin
...
let map_flags = (fun s map_exp descend binders flags -> ((Support.List.map (fun _68699 -> (match (_68699) with
...
| Microsoft_FStar_Absyn_Syntax.Total (t) -> begin
(let _69228 = (map_typ descend binders t)
in (match (_69228) with
| (t, descend) -> begin
((Microsoft_FStar_Absyn_Syntax.mk_Total t), descend)
end))
end
| Microsoft_FStar_Absyn_Syntax.Comp (ct) -> begin
(let _69233 = (map_typ descend binders ct.Microsoft_FStar_Absyn_Syntax.result_typ)
in (match (_69233) with
| (t, descend) -> begin
(let _69236 = (Microsoft_FStar_Absyn_Visit.map_args map_typ map_exp descend binders ct.Microsoft_FStar_Absyn_Syntax.effect_args)
in (match (_69236) with
| (args, descend) -> begin
((Microsoft_FStar_Absyn_Syntax.mk_Comp (let _69237 = ct
in {Microsoft_FStar_Absyn_Syntax.effect_name = _69237.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = t; Microsoft_FStar_Absyn_Syntax.effect_args = args; Microsoft_FStar_Absyn_Syntax.flags = (map_flags s map_exp descend binders ct.Microsoft_FStar_Absyn_Syntax.flags)})), descend)
end))
end))
end))
...
(let _69250 = (vk null_ctrl binders k)
in (match (_69250) with
| (k, _) -> begin
(k, ctrl)
end))
end else begin
...
in (let _69260 = (Support.ST.op_ColonEquals m (Some (k')))
in k')))
end
...
let rec visit_typ = (fun s mk vt me ctrl boundvars t -> (let visit_prod = (fun bs tc -> (let _69338 = ((Support.List.fold_left (fun _69291 b -> (match (_69291) with
| (bs, boundvars, s) -> begin
(match (b) with
| (Support.Microsoft.FStar.Util.Inl (a), imp) -> begin
(let _69300 = (map_knd s mk vt me null_ctrl boundvars a.Microsoft_FStar_Absyn_Syntax.sort)
in (match (_69300) with
| (k, _) -> begin
(let a = (let _69301 = a
in {Microsoft_FStar_Absyn_Syntax.v = _69301.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = k; Microsoft_FStar_Absyn_Syntax.p = _69301.Microsoft_FStar_Absyn_Syntax.p})
...
| (Support.Microsoft.FStar.Util.Inr (x), imp) -> begin
(let _69321 = (map_typ s mk vt me null_ctrl boundvars x.Microsoft_FStar_Absyn_Syntax.sort)
in (match (_69321) with
| (t, _) -> begin
(let x = (let _69322 = x
in {Microsoft_FStar_Absyn_Syntax.v = _69322.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = t; Microsoft_FStar_Absyn_Syntax.p = _69322.Microsoft_FStar_Absyn_Syntax.p})
in if (Microsoft_FStar_Absyn_Syntax.is_null_binder b) then begin
...
| _ -> begin
(let _69400 = (vt null_ctrl boundvars t)
...
in (let _69412 = (Support.ST.op_ColonEquals m (Some (res)))
in res)))
end
| Microsoft_FStar_Absyn_Syntax.Typ_delayed ((Support.Microsoft.FStar.Util.Inr (mk_t), m)) -> begin
(let t = (compress_typ' (mk_t ()))
in (let _69420 = (Support.ST.op_ColonEquals m (Some (t)))
in t))
end
| _ -> begin
t
...
| _ -> begin
(let _69449 = (ve null_ctrl binders e)
in (match (_69449) with
| (e, _) -> begin
(e, ctrl)
end))
end)))
...
in (let _69459 = (Support.ST.op_ColonEquals m (Some (res)))
in res)))
end
| _ -> begin
e
...
let compress_typ_opt = (fun _68700 -> (match (_68700) with
| None -> begin
None
end
| Some (t) -> begin
...
| Microsoft_FStar_Absyn_Syntax.Comp (ct) -> begin
(let _69532 = c
in {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Comp ((let _69534 = ct
in {Microsoft_FStar_Absyn_Syntax.effect_name = _69534.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = _69534.Microsoft_FStar_Absyn_Syntax.result_typ; Microsoft_FStar_Absyn_Syntax.effect_args = _69534.Microsoft_FStar_Absyn_Syntax.effect_args; Microsoft_FStar_Absyn_Syntax.flags = f})); Microsoft_FStar_Absyn_Syntax.tk = _69532.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = _69532.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _69532.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _69532.Microsoft_FStar_Absyn_Syntax.uvs})
end))
...
let is_total_comp = (fun c -> ((Support.Microsoft.FStar.Util.for_some (fun _68701 -> (match (_68701) with
...
let is_total_lcomp = (fun c -> ((Microsoft_FStar_Absyn_Syntax.lid_equals c.Microsoft_FStar_Absyn_Syntax.eff_name Microsoft_FStar_Absyn_Const.tot_effect_lid) || ((Support.Microsoft.FStar.Util.for_some (fun _68702 -> (match (_68702) with
| (Microsoft_FStar_Absyn_Syntax.TOTAL) | (Microsoft_FStar_Absyn_Syntax.RETURN) -> begin
true
end
| _ -> begin
...
let is_partial_return = (fun c -> ((Support.Microsoft.FStar.Util.for_some (fun _68703 -> (match (_68703) with
...
let is_lcomp_partial_return = (fun c -> ((Support.Microsoft.FStar.Util.for_some (fun _68704 -> (match (_68704) with
| (Microsoft_FStar_Absyn_Syntax.RETURN) | (Microsoft_FStar_Absyn_Syntax.PARTIAL_RETURN) -> begin
true
end
| _ -> begin
...
| Microsoft_FStar_Absyn_Syntax.Comp (ct) -> begin
((((is_tot_or_gtot_comp c) || (Support.Microsoft.FStar.Util.starts_with ct.Microsoft_FStar_Absyn_Syntax.effect_name.Microsoft_FStar_Absyn_Syntax.str "Prims.PURE")) || (Support.Microsoft.FStar.Util.starts_with ct.Microsoft_FStar_Absyn_Syntax.effect_name.Microsoft_FStar_Absyn_Syntax.str "Prims.Pure")) || ((Support.Microsoft.FStar.Util.for_some (fun _68705 -> (match (_68705) with
...
let is_pure_lcomp = (fun lc -> ((((is_total_lcomp lc) || (Support.Microsoft.FStar.Util.starts_with lc.Microsoft_FStar_Absyn_Syntax.eff_name.Microsoft_FStar_Absyn_Syntax.str "Prims.Pure")) || (Support.Microsoft.FStar.Util.starts_with lc.Microsoft_FStar_Absyn_Syntax.eff_name.Microsoft_FStar_Absyn_Syntax.str "Prims.PURE")) || ((Support.Microsoft.FStar.Util.for_some (fun _68706 -> (match (_68706) with
| Microsoft_FStar_Absyn_Syntax.LEMMA -> begin
true
end
| _ -> begin
...
| Microsoft_FStar_Absyn_Syntax.Comp (c) -> begin
((Microsoft_FStar_Absyn_Syntax.lid_equals c.Microsoft_FStar_Absyn_Syntax.effect_name Microsoft_FStar_Absyn_Const.ml_effect_lid) || ((Support.Microsoft.FStar.Util.for_some (fun _68707 -> (match (_68707) with
...
| Microsoft_FStar_Absyn_Syntax.Comp (ct) -> begin
(Microsoft_FStar_Absyn_Syntax.mk_Comp (let _69683 = ct
in {Microsoft_FStar_Absyn_Syntax.effect_name = _69683.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = t; Microsoft_FStar_Absyn_Syntax.effect_args = _69683.Microsoft_FStar_Absyn_Syntax.effect_args; Microsoft_FStar_Absyn_Syntax.flags = _69683.Microsoft_FStar_Absyn_Syntax.flags}))
end))

let is_trivial_wp = (fun c -> ((Support.Microsoft.FStar.Util.for_some (fun _68708 -> (match (_68708) with
| (Microsoft_FStar_Absyn_Syntax.TOTAL) | (Microsoft_FStar_Absyn_Syntax.RETURN) -> begin
true
end
| _ -> begin
...
let range_of_lb = (fun _68709 -> (match (_68709) with
...
let range_of_arg = (fun _68710 -> (match (_68710) with
...
in (let y = (let _70077 = x.Microsoft_FStar_Absyn_Syntax.v
in {Microsoft_FStar_Absyn_Syntax.ppname = nm; Microsoft_FStar_Absyn_Syntax.realname = _70077.Microsoft_FStar_Absyn_Syntax.realname})
...
let uv_eq = (fun _70120 _70124 -> (match ((_70120, _70124)) with
| ((uv1, _), (uv2, _)) -> begin
...
let union_fvs_uvs = (fun _70131 _70134 -> (match ((_70131, _70134)) with
| ((fvs1, uvs1), (fvs2, uvs2)) -> begin
((union_fvs fvs1 fvs2), (union_uvs uvs1 uvs2))
end))

let sub_fv = (fun _70137 _70140 -> (match ((_70137, _70140)) with
...
let stash = (fun uvonly s _70149 -> (match (_70149) with
| (fvs, uvs) -> begin
(let _70150 = (Support.ST.op_ColonEquals s.Microsoft_FStar_Absyn_Syntax.uvs (Some (uvs)))
in if uvonly then begin
()
end else begin
(Support.ST.op_ColonEquals s.Microsoft_FStar_Absyn_Syntax.fvs (Some (fvs)))
end)
end))
...
(cont ((let _70165 = Microsoft_FStar_Absyn_Syntax.no_fvs
in {Microsoft_FStar_Absyn_Syntax.ftvs = (single_fv a); Microsoft_FStar_Absyn_Syntax.fxvs = _70165.Microsoft_FStar_Absyn_Syntax.fxvs}), Microsoft_FStar_Absyn_Syntax.no_uvs))
end
end
| Microsoft_FStar_Absyn_Syntax.Typ_uvar ((uv, k)) -> begin
(cont (Microsoft_FStar_Absyn_Syntax.no_fvs, (let _70171 = Microsoft_FStar_Absyn_Syntax.no_uvs
in {Microsoft_FStar_Absyn_Syntax.uvars_k = _70171.Microsoft_FStar_Absyn_Syntax.uvars_k; Microsoft_FStar_Absyn_Syntax.uvars_t = (single_uvt (uv, k)); Microsoft_FStar_Absyn_Syntax.uvars_e = _70171.Microsoft_FStar_Absyn_Syntax.uvars_e})))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_unknown) | (Microsoft_FStar_Absyn_Syntax.Typ_const (_)) -> begin
(cont (Microsoft_FStar_Absyn_Syntax.no_fvs, Microsoft_FStar_Absyn_Syntax.no_uvs))
end
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, c)) -> begin
(vs_binders bs uvonly (fun _70183 -> (match (_70183) with
| (bvs, vs1) -> begin
(vs_comp c uvonly (fun vs2 -> (cont (sub_fv (union_fvs_uvs vs1 vs2) bvs))))
end)))
end
| Microsoft_FStar_Absyn_Syntax.Typ_lam ((bs, t)) -> begin
(vs_binders bs uvonly (fun _70191 -> (match (_70191) with
| (bvs, vs1) -> begin
(vs_typ t uvonly (fun vs2 -> (cont (sub_fv (union_fvs_uvs vs1 vs2) bvs))))
end)))
end
| Microsoft_FStar_Absyn_Syntax.Typ_refine ((x, t)) -> begin
(vs_binders (((Support.Microsoft.FStar.Util.Inr (x), None))::[]) uvonly (fun _70199 -> (match (_70199) with
| (bvs, vs1) -> begin
(vs_typ t uvonly (fun vs2 -> (cont (sub_fv (union_fvs_uvs vs1 vs2) bvs))))
end)))
end
...
(vs_kind a.Microsoft_FStar_Absyn_Syntax.sort uvonly (fun vs -> (vs_binders rest uvonly (fun _70265 -> (match (_70265) with
...
(vs_typ x.Microsoft_FStar_Absyn_Syntax.sort uvonly (fun vs -> (vs_binders rest uvonly (fun _70278 -> (match (_70278) with
| ((tvars, vvars), vs2) -> begin
...
and vs_typ = (fun t uvonly cont -> (match (((Support.ST.read t.Microsoft_FStar_Absyn_Syntax.fvs), (Support.ST.read t.Microsoft_FStar_Absyn_Syntax.uvs))) with
| (Some (_), None) -> begin
(failwith ("Impossible"))
end
| (None, None) -> begin
(vs_typ' t uvonly (fun fvs -> (let _70315 = (stash uvonly t fvs)
in (cont fvs))))
end
| (None, Some (uvs)) -> begin
if uvonly then begin
...
(vs_typ' t uvonly (fun fvs -> (let _70322 = (stash uvonly t fvs)
in (cont fvs))))
end
end
...
(vs_args args uvonly (fun _70351 -> (match (_70351) with
| (fvs, uvs) -> begin
(cont (fvs, (let _70352 = uvs
in {Microsoft_FStar_Absyn_Syntax.uvars_k = (Support.Microsoft.FStar.Util.set_add uv uvs.Microsoft_FStar_Absyn_Syntax.uvars_k); Microsoft_FStar_Absyn_Syntax.uvars_t = _70352.Microsoft_FStar_Absyn_Syntax.uvars_t; Microsoft_FStar_Absyn_Syntax.uvars_e = _70352.Microsoft_FStar_Absyn_Syntax.uvars_e})))
end)))
end
...
(vs_binders bs uvonly (fun _70365 -> (match (_70365) with
| (bvs, vs1) -> begin
(vs_kind k uvonly (fun vs2 -> (cont (sub_fv (union_fvs_uvs vs1 vs2) bvs))))
end)))
end)))
and vs_kind = (fun k uvonly cont -> (match (((Support.ST.read k.Microsoft_FStar_Absyn_Syntax.fvs), (Support.ST.read k.Microsoft_FStar_Absyn_Syntax.uvs))) with
| (Some (_), None) -> begin
(failwith ("Impossible"))
end
| (None, None) -> begin
(vs_kind' k uvonly (fun fvs -> (let _70380 = (stash uvonly k fvs)
in (cont fvs))))
end
| (None, Some (uvs)) -> begin
if uvonly then begin
...
(vs_kind' k uvonly (fun fvs -> (let _70387 = (stash uvonly k fvs)
in (cont fvs))))
end
end
| (Some (fvs), Some (uvs)) -> begin
...
(cont (Microsoft_FStar_Absyn_Syntax.no_fvs, (let _70412 = Microsoft_FStar_Absyn_Syntax.no_uvs
in {Microsoft_FStar_Absyn_Syntax.uvars_k = _70412.Microsoft_FStar_Absyn_Syntax.uvars_k; Microsoft_FStar_Absyn_Syntax.uvars_t = _70412.Microsoft_FStar_Absyn_Syntax.uvars_t; Microsoft_FStar_Absyn_Syntax.uvars_e = (single_uvt (uv, t))})))
end
| Microsoft_FStar_Absyn_Syntax.Exp_bvar (x) -> begin
if uvonly then begin
(cont (Microsoft_FStar_Absyn_Syntax.no_fvs, Microsoft_FStar_Absyn_Syntax.no_uvs))
end else begin
(cont ((let _70416 = Microsoft_FStar_Absyn_Syntax.no_fvs
in {Microsoft_FStar_Absyn_Syntax.ftvs = _70416.Microsoft_FStar_Absyn_Syntax.ftvs; Microsoft_FStar_Absyn_Syntax.fxvs = (single_fv x)}), Microsoft_FStar_Absyn_Syntax.no_uvs))
end
end
| Microsoft_FStar_Absyn_Syntax.Exp_ascribed ((e, _)) -> begin
...
(vs_binders bs uvonly (fun _70429 -> (match (_70429) with
| (bvs, vs1) -> begin
...
and vs_exp = (fun e uvonly cont -> (match (((Support.ST.read e.Microsoft_FStar_Absyn_Syntax.fvs), (Support.ST.read e.Microsoft_FStar_Absyn_Syntax.uvs))) with
| (Some (_), None) -> begin
(failwith ("Impossible"))
end
| (None, None) -> begin
(vs_exp' e uvonly (fun fvs -> (let _70462 = (stash uvonly e fvs)
in (cont fvs))))
end
| (None, Some (uvs)) -> begin
if uvonly then begin
...
(vs_exp' e uvonly (fun fvs -> (let _70469 = (stash uvonly e fvs)
in (cont fvs))))
end
end
| (Some (fvs), Some (uvs)) -> begin
...
and vs_comp = (fun c uvonly cont -> (match (((Support.ST.read c.Microsoft_FStar_Absyn_Syntax.fvs), (Support.ST.read c.Microsoft_FStar_Absyn_Syntax.uvs))) with
| (Some (_), None) -> begin
(failwith ("Impossible"))
end
| (None, None) -> begin
(vs_comp' c uvonly (fun fvs -> (let _70499 = (stash uvonly c fvs)
in (cont fvs))))
end
| (None, Some (uvs)) -> begin
if uvonly then begin
...
(vs_comp' c uvonly (fun fvs -> (let _70506 = (stash uvonly c fvs)
in (cont fvs))))
end
end
| (Some (fvs), Some (uvs)) -> begin
...
let freevars_kind = (fun k -> (vs_kind k false (fun _70535 -> (match (_70535) with
| (x, _) -> begin
x
end))))

let freevars_typ = (fun t -> (vs_typ t false (fun _70540 -> (match (_70540) with
| (x, _) -> begin
x
end))))

let freevars_exp = (fun e -> (vs_exp e false (fun _70545 -> (match (_70545) with
| (x, _) -> begin
x
end))))

let freevars_comp = (fun c -> (vs_comp c false (fun _70550 -> (match (_70550) with
| (x, _) -> begin
x
end))))
...
let is_free = (fun axs fvs -> ((Support.Microsoft.FStar.Util.for_some (fun _68711 -> (match (_68711) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
...
| _ -> begin
(let _70581 = out
in {Microsoft_FStar_Absyn_Syntax.uvars_k = (Support.Microsoft.FStar.Util.set_add u out.Microsoft_FStar_Absyn_Syntax.uvars_k); Microsoft_FStar_Absyn_Syntax.uvars_t = _70581.Microsoft_FStar_Absyn_Syntax.uvars_t; Microsoft_FStar_Absyn_Syntax.uvars_e = _70581.Microsoft_FStar_Absyn_Syntax.uvars_e})
...
in (let out = ((Support.List.fold_left (fun out _70587 -> (match (_70587) with
| (u, t) -> begin
...
| _ -> begin
(let _70592 = out
in {Microsoft_FStar_Absyn_Syntax.uvars_k = _70592.Microsoft_FStar_Absyn_Syntax.uvars_k; Microsoft_FStar_Absyn_Syntax.uvars_t = (Support.Microsoft.FStar.Util.set_add (u, t) out.Microsoft_FStar_Absyn_Syntax.uvars_t); Microsoft_FStar_Absyn_Syntax.uvars_e = _70592.Microsoft_FStar_Absyn_Syntax.uvars_e})
end)
end)) out) (Support.Microsoft.FStar.Util.set_elements uvs.Microsoft_FStar_Absyn_Syntax.uvars_t))
in (let out = ((Support.List.fold_left (fun out _70598 -> (match (_70598) with
| (u, t) -> begin
(match ((Support.Microsoft.FStar.Unionfind.find u)) with
...
| _ -> begin
(let _70603 = out
in {Microsoft_FStar_Absyn_Syntax.uvars_k = _70603.Microsoft_FStar_Absyn_Syntax.uvars_k; Microsoft_FStar_Absyn_Syntax.uvars_t = _70603.Microsoft_FStar_Absyn_Syntax.uvars_t; Microsoft_FStar_Absyn_Syntax.uvars_e = (Support.Microsoft.FStar.Util.set_add (u, t) out.Microsoft_FStar_Absyn_Syntax.uvars_e)})
end)
end)) out) (Support.Microsoft.FStar.Util.set_elements uvs.Microsoft_FStar_Absyn_Syntax.uvars_e))
in (let _70614 = (match (s) with
| SynSumKind (k) -> begin
(Support.ST.op_ColonEquals k.Microsoft_FStar_Absyn_Syntax.uvs (Some (out)))
end
| SynSumType (t) -> begin
(Support.ST.op_ColonEquals t.Microsoft_FStar_Absyn_Syntax.uvs (Some (out)))
end
| SynSumExp (e) -> begin
(Support.ST.op_ColonEquals e.Microsoft_FStar_Absyn_Syntax.uvs (Some (out)))
end
| SynSumComp (c) -> begin
(Support.ST.op_ColonEquals c.Microsoft_FStar_Absyn_Syntax.uvs (Some (out)))
end)
in out)))))
and uvars_in_kind = (fun k -> ((update_uvars (SynSumKind (k))) (vs_kind k true (fun _70620 -> (match (_70620) with
| (_, x) -> begin
x
end)))))
and uvars_in_typ = (fun t -> ((update_uvars (SynSumType (t))) (vs_typ t true (fun _70625 -> (match (_70625) with
| (_, x) -> begin
x
end)))))
and uvars_in_exp = (fun e -> ((update_uvars (SynSumExp (e))) (vs_exp e true (fun _70630 -> (match (_70630) with
| (_, x) -> begin
x
end)))))
and uvars_in_comp = (fun c -> ((update_uvars (SynSumComp (c))) (vs_comp c true (fun _70635 -> (match (_70635) with
| (_, x) -> begin
x
end)))))
...
| Microsoft_FStar_Absyn_Syntax.Kind_arrow ((bs, k)) -> begin
(let _70655 = (kind_formals k)
in (match (_70655) with
| (bs', k) -> begin
...
let close_for_kind = (fun t k -> (let _70669 = (kind_formals k)
in (match (_70669) with
| (bs, _) -> begin
(match (bs) with
| [] -> begin
t
...
| _ -> begin
(let _70700 = (match (t.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs', c)) -> begin
...
let base_kind = (fun _68712 -> (match (_68712) with
...
let sortByFieldName = (fun fn_a_l -> ((Support.List.sortWith (fun _70779 _70783 -> (match ((_70779, _70783)) with
| ((fn1, _), (fn2, _)) -> begin
...
let destruct_typ_as_formula = (fun f -> (let destruct_base_conn = (fun f -> (let _70947 = (true, false)
in (match (_70947) with
...
in (let rec aux = (fun f _70957 -> (match (_70957) with
| (lid, arity) -> begin
(let _70960 = (head_and_args f)
in (match (_70960) with
| (t, args) -> begin
...
in (let flat = (fun t -> (let _70994 = (head_and_args t)
in (match (_70994) with
| (t, args) -> begin
(t, ((Support.List.map (fun _68713 -> (match (_68713) with
| (Support.Microsoft.FStar.Util.Inl (t), imp) -> begin
...
| (Some (true), _) -> begin
(let _71146 = (patterns t)
in (match (_71146) with
| (pats, body) -> begin
Some (QAll (((Support.List.rev out), pats, body)))
end))
end
| (Some (false), _) -> begin
(let _71154 = (patterns t)
in (match (_71154) with
| (pats, body) -> begin
