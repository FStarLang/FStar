...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____56682 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____56717 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____57051,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____57067,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____57084 = FStar_Options.codegen ()  in
           uu____57084 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____57089 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____57146 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____57146
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____57155 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____57155
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____57181))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____57189::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____57205 'Auu____57206 'Auu____57207 'Auu____57208 .
    ('Auu____57205 * 'Auu____57206 * 'Auu____57207 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____57208 *
  ...
  =
  fun uu____57229  ->
    match uu____57229 with
    | (uu____57244,uu____57245,uu____57246,vars,uu____57248,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____57259 -> FStar_Pervasives_Native.None)
    ...
    fun lid  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___526_57439  ->
             match uu___526_57439 with
             | Fv (fv',x) when FStar_Syntax_Syntax.fv_eq_lid fv' lid ->
                 FStar_Pervasives_Native.Some x
             | uu____57444 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____57445 =
...
let (try_lookup_fv :
  ...
  fun g  ->
    fun fv  ->
      FStar_Util.find_map g.env_bindings
        (fun uu___527_57467  ->
           match uu___527_57467 with
           | Fv (fv',t) when FStar_Syntax_Syntax.fv_eq fv fv' ->
               FStar_Pervasives_Native.Some t
           | uu____57472 -> FStar_Pervasives_Native.None)
  
let (lookup_fv : uenv -> FStar_Syntax_Syntax.fv -> exp_binding) =
  fun g  ->
    fun fv  ->
      let uu____57484 = try_lookup_fv g fv  in
      match uu____57484 with
      | FStar_Pervasives_Native.None  ->
          let uu____57487 =
            let uu____57489 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____57489 uu____57491
             in
          failwith uu____57487
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_57512  ->
             match uu___528_57512 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____57517 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____57518 =
            let uu____57520 =
              FStar_Range.string_of_range
               ...
               in
            let uu____57522 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____57520 uu____57522
             in
          failwith uu____57518
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____57558 = lookup_bv g x1  in
          (uu____57558, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____57562 =
            let uu____57563 = lookup_fv g x1  in FStar_Util.Inr uu____57563
             in
          (uu____57562, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____57591 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_57627 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_57627.tydefs);
          type_names = (uu___726_57627.type_names);
          currentModule = (uu___726_57627.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____57672 =
               let uu____57676 = valid c  in
               if uu____57676 then [c] else [95; 95]  in
             FStar_List.append uu____57672 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_57766  ->
               match uu___529_57766 with
               | Bv (uu____57768,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____57771,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____57774,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____57839 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____57842 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____57842  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____57863 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____57863
                 in
              ((let uu___787_57866 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_57866.tydefs);
                  type_names = (uu___787_57866.type_names);
                  currentModule = (uu___787_57866.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____57886 = mltyFvars t1  in
        let uu____57890 = mltyFvars t2  in
        FStar_List.append uu____57886 uu____57890
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____57951 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____57951 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____58004 = tySchemeIsClosed t_x  in
              if uu____58004
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____58017 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____58018 =
                  let uu____58024 = y  in
                  match uu____58024 with
                  | (ns,i) ->
                      let mlsymbol =
                        FStar_Extraction_ML_Syntax.avoid_keyword i  in
                      ((ns, mlsymbol), mlsymbol)
                   in
                match uu____58018 with
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_58075 = g  in
                      {
                        env_tcenv = (uu___838_58075.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_58075.tydefs);
                        type_names = (uu___838_58075.type_names);
                        currentModule = (uu___838_58075.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____58233 = td  in
        match uu____58233 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____58259 =
                let uu____58260 = tyscheme_of_td td  in
                FStar_Option.get uu____58260  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____58259
              }  in
            ((let uu___871_58269 = g  in
              {
                env_tcenv = (uu___871_58269.env_tcenv);
                env_bindings = (uu___871_58269.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_58269.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_58281 = g  in
      {
        env_tcenv = (uu___875_58281.env_tcenv);
        env_bindings = (uu___875_58281.env_bindings);
        tydefs = (uu___875_58281.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_58281.currentModule)
      }
      ...
      let uu____58356 =
        let uu____58357 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____58357  in
      extend_lb env uu____58356 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____58348 with | (g,uu____58361,uu____58362) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____58383 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____58383
         in
      let uu____58384 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____58384, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____58406 =
          let uu____58409 =
            let uu____58412 = FStar_Ident.id_of_text nm  in [uu____58412]  in
          FStar_List.append module_name uu____58409  in
        FStar_Ident.lid_of_ids uu____58406  in
      let uu____58413 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____58413, lid)
