...
in if ((let _152_52 = (FStar_ST.read FStar_Options.debug)
in (FStar_All.pipe_right _152_52 (FStar_Util.for_some (fun x -> (c = x))))) && (FStar_Options.debug_level_geq (FStar_Options.Other ("Extraction")))) then begin
(f ())
end else begin
()
end))
...
(match (t) with
| FStar_Extraction_ML_Syntax.MLTY_Named (_67_23, ("FStar"::"Ghost"::[], "erased")) -> begin
true
end
| _67_32 -> begin
false
...
let prependTick = (fun _67_35 -> (match (_67_35) with
| (x, n) -> begin
...
let removeTick = (fun _67_38 -> (match (_67_38) with
| (x, n) -> begin
if (FStar_Util.starts_with x "\'") then begin
(let _152_61 = (FStar_Util.substring_from x 1)
in (_152_61, n))
end else begin
(x, n)
end
...
let bv_as_ml_tyvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _152_68 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (prependTick _152_68)))

# 83 "FStar.Extraction.ML.UEnv.fst"
let bv_as_ml_termvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _152_71 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (removeTick _152_71)))
...
| Bv (b', FStar_Util.Inr (_67_57))::tl -> begin
if (FStar_Syntax_Syntax.bv_eq b b') then begin
...
let tyscheme_of_td = (fun _67_71 -> (match (_67_71) with
| (_67_68, vars, body_opt) -> begin
(match (body_opt) with
...
let lookup_ty_const : env  ->  FStar_Extraction_ML_Syntax.mlpath  ->  FStar_Extraction_ML_Syntax.mltyscheme Prims.option = (fun env _67_80 -> (match (_67_80) with
| (module_name, ty_name) -> begin
(FStar_Util.find_map env.tydefs (fun _67_83 -> (match (_67_83) with
| (m, tds) -> begin
if (module_name = m) then begin
(FStar_Util.find_map tds (fun td -> (
# 101 "FStar.Extraction.ML.UEnv.fst"
let _67_90 = td
in (match (_67_90) with
| (n, _67_87, _67_89) -> begin
...
let x = (FStar_Util.find_map g.gamma (fun _67_1 -> (match (_67_1) with
...
in (match (x) with
| None -> begin
(let _152_92 = (FStar_Util.format1 "free Variable %s not found\n" fv.FStar_Ident.nsstr)
in (FStar_All.failwith _152_92))
end
| Some (y) -> begin
y
end)))
...
let x = (FStar_Util.find_map g.gamma (fun _67_2 -> (match (_67_2) with
...
in (match (x) with
| None -> begin
(let _152_100 = (let _152_99 = (FStar_Range.string_of_range fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.p)
in (let _152_98 = (FStar_Syntax_Print.lid_to_string fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _152_99 _152_98)))
in (FStar_All.failwith _152_100))
end
| Some (y) -> begin
y
end)))
...
let x = (FStar_Util.find_map g.gamma (fun _67_3 -> (match (_67_3) with
...
in (match (x) with
| None -> begin
(let _152_108 = (let _152_107 = (FStar_Range.string_of_range bv.FStar_Syntax_Syntax.ppname.FStar_Ident.idRange)
in (let _152_106 = (FStar_Syntax_Print.bv_to_string bv)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _152_107 _152_106)))
in (FStar_All.failwith _152_108))
end
| Some (y) -> begin
y
end)))
...
(let _152_113 = (lookup_bv g x)
in (_152_113, None))
end
| FStar_Util.Inr (x) -> begin
(let _152_114 = (lookup_fv g x)
in (_152_114, x.FStar_Syntax_Syntax.fv_qual))
end))
...
let _67_156 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _67_156.tydefs; currentModule = _67_156.currentModule}))))))
...
let mlx = (let _152_137 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in FStar_Extraction_ML_Syntax.MLE_Var (_152_137))
in (
...
let tcenv = (let _152_138 = (FStar_Syntax_Syntax.binders_of_list ((x)::[]))
in (FStar_TypeChecker_Env.push_binders g.tcenv _152_138))
in (
# 175 "FStar.Extraction.ML.UEnv.fst"
let _67_174 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _67_174.tydefs; currentModule = _67_174.currentModule})))))))
...
(let _152_142 = (mltyFvars t1)
in (let _152_141 = (mltyFvars t2)
in (FStar_List.append _152_142 _152_141)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _152_149 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _152_149 (Prims.fst tys))))
...
let _67_214 = g
in {tcenv = tcenv; gamma = gamma; tydefs = _67_214.tydefs; currentModule = _67_214.currentModule}))))))
end else begin
...
| FStar_Util.Inl (x) -> begin
(let _152_185 = (extend_bv g x t_x add_unit is_rec false)
in (let _152_184 = (bv_as_ml_termvar x)
in (_152_185, _152_184)))
end
| FStar_Util.Inr (f) -> begin
(
# 220 "FStar.Extraction.ML.UEnv.fst"
let _67_234 = (FStar_Extraction_ML_Syntax.mlpath_of_lident f.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (match (_67_234) with
| (p, y) -> begin
(let _152_186 = (extend_fv' g f (p, y) t_x add_unit is_rec)
in (_152_186, (y, 0)))
end))
end))
...
let _67_238 = g
in {tcenv = _67_238.tcenv; gamma = _67_238.gamma; tydefs = ((m, td))::g.tydefs; currentModule = _67_238.currentModule})))
...
in (let _152_195 = (let _152_194 = (let _152_193 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
in FStar_Util.Inr (_152_193))
in (extend_lb env _152_194 FStar_Syntax_Syntax.tun failwith_ty false false))
in (FStar_All.pipe_right _152_195 Prims.fst))))))
