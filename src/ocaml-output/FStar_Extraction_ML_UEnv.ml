...
let idents = (FStar_String.split (('_')::[]) name.FStar_Ident.ident.FStar_Ident.idText)
in try
(match (()) with
| () -> begin
(
...
let uu____465 = (FStar_List.index (fun x1 -> (x1 = "proj")) idents)
in (

let uu____467 = (FStar_List.index (fun x1 -> (x1 = "item")) idents)
in ((uu____465), (uu____467))))
...
let uu____480 = (FStar_List.splitAt (j - (Prims.parse_int "1")) idents)
in (Prims.fst uu____480))
in (FStar_List.splitAt (i + (Prims.parse_int "2")) uu____478))
in (Prims.snd uu____473))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____471))
end))
end)
with
| uu____491 -> begin
...
let uu____495 = (FStar_List.map (fun x1 -> x1.FStar_Ident.idText) name.FStar_Ident.ns)
in (FStar_List.append uu____495 ((eff_name)::(name.FStar_Ident.ident.FStar_Ident.idText)::[])))
in (FStar_All.pipe_left (FStar_String.concat ".") uu____493))
in (

let x1 = (FStar_Util.find_map g.gamma (fun uu___104_501 -> (match (uu___104_501) with
| Fv (fv', t) when (fv'.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v.FStar_Ident.str = action_name) -> begin
Some (t)
end
| uu____509 -> begin
None
...
let x = (FStar_Util.find_map g.gamma (fun uu___107_547 -> (match (uu___107_547) with
...
let uu____553 = (FStar_Range.string_of_range bv.FStar_Syntax_Syntax.ppname.FStar_Ident.idRange)
in (

let uu____554 = (FStar_Syntax_Print.bv_to_string bv)
...
let uu____576 = (lookup_bv g x1)
in ((uu____576), (None)))
end
| FStar_Util.Inr (x1) -> begin
(

let uu____579 = (lookup_fv g x1)
in ((uu____579), (x1.FStar_Syntax_Syntax.fv_qual)))
end))
...
let uu___111_638 = g
in {tcenv = tcenv; gamma = gamma; tydefs = uu___111_638.tydefs; currentModule = uu___111_638.currentModule}))))))
...
let has_collision = (FStar_List.existsb (fun uu___108_658 -> (match (uu___108_658) with
...
let uu____756 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
...
let uu____787 = (FStar_Syntax_Syntax.binders_of_list ((x)::[]))
in (FStar_TypeChecker_Env.push_binders g.tcenv uu____787))
in (((

let uu___112_790 = g
in {tcenv = tcenv; gamma = gamma; tydefs = uu___112_790.tydefs; currentModule = uu___112_790.currentModule})), (mlident1))))))))
end))))
...
let uu____801 = (mltyFvars t1)
in (

let uu____803 = (mltyFvars t2)
in (FStar_List.append uu____801 uu____803)))
end
...
let uu____827 = (mltyFvars (Prims.snd tys))
in (subsetMlidents uu____827 (Prims.fst tys))))
...
let uu____851 = (tySchemeIsClosed t_x)
...
let uu___113_911 = g
in {tcenv = uu___113_911.tcenv; gamma = gamma; tydefs = uu___113_911.tydefs; currentModule = uu___113_911.currentModule})), (((mlsymbol), ((Prims.parse_int "0"))))))))
end)))
end
| uu____912 -> begin
...
let uu____965 = (FStar_Extraction_ML_Syntax.mlpath_of_lident f.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
...
let uu___114_988 = g
in {tcenv = uu___114_988.tcenv; gamma = uu___114_988.gamma; tydefs = (((m), (td)))::g.tydefs; currentModule = uu___114_988.currentModule})))
...
let uu____1029 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
in FStar_Util.Inr (uu____1029))
in (extend_lb env uu____1028 FStar_Syntax_Syntax.tun failwith_ty false false))
in (FStar_All.pipe_right uu____1025 Prims.fst))))))
...
let uu____1040 = ((ed.FStar_Syntax_Syntax.mname.FStar_Ident.ns), (ed.FStar_Syntax_Syntax.mname.FStar_Ident.ident))
