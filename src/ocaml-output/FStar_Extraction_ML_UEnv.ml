...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____61287 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____61323 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61658,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61674,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____61691 = FStar_Options.codegen ()  in
           uu____61691 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____61696 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61753 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____61753
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61762 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____61762
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____61788))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____61796::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____61812 'Auu____61813 'Auu____61814 'Auu____61815 .
    ('Auu____61812 * 'Auu____61813 * 'Auu____61814 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____61815 *
  ...
  fun uu____61836  ->
    ...
    | (uu____61851,uu____61852,uu____61853,vars,uu____61855,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____61866 -> FStar_Pervasives_Native.None)
...
let (lookup_fv : uenv -> FStar_Syntax_Syntax.fv -> exp_binding) =
  fun g  ->
    fun fv  ->
      let uu____62091 = try_lookup_fv g fv  in
      match uu____62091 with
      | FStar_Pervasives_Native.None  ->
          let uu____62094 =
            let uu____62096 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____62096 uu____62098
             in
          failwith uu____62094
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_62119  ->
             match uu___528_62119 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____62124 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62125 =
            let uu____62127 =
              FStar_Range.string_of_range
               ...
               in
            let uu____62129 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____62127 uu____62129
             in
          failwith uu____62125
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____62165 = lookup_bv g x1  in
          (uu____62165, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____62169 =
            let uu____62170 = lookup_fv g x1  in FStar_Util.Inr uu____62170
             in
          (uu____62169, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____62198 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_62234 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_62234.tydefs);
          type_names = (uu___726_62234.type_names);
          currentModule = (uu___726_62234.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____62279 =
               let uu____62283 = valid c  in
               if uu____62283 then [c] else [95; 95]  in
             FStar_List.append uu____62279 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_62373  ->
               match uu___529_62373 with
               | Bv (uu____62375,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____62378,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____62381,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____62446 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____62449 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____62449  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____62470 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____62470
                 in
              ((let uu___787_62473 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_62473.tydefs);
                  type_names = (uu___787_62473.type_names);
                  currentModule = (uu___787_62473.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____62493 = mltyFvars t1  in
        let uu____62497 = mltyFvars t2  in
        FStar_List.append uu____62493 uu____62497
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____62558 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____62558 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____62611 = tySchemeIsClosed t_x  in
              if uu____62611
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____62624 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____62625 =
                  let uu____62631 = y  in
                  match uu____62631 with
                  | (ns,i) ->
                      let mlsymbol =
                        FStar_Extraction_ML_Syntax.avoid_keyword i  in
                      ((ns, mlsymbol), mlsymbol)
                   in
                match uu____62625 with
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_62682 = g  in
                      {
                        env_tcenv = (uu___838_62682.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_62682.tydefs);
                        type_names = (uu___838_62682.type_names);
                        currentModule = (uu___838_62682.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____62840 = td  in
        match uu____62840 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____62866 =
                let uu____62867 = tyscheme_of_td td  in
                FStar_Option.get uu____62867  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____62866
              }  in
            ((let uu___871_62876 = g  in
              {
                env_tcenv = (uu___871_62876.env_tcenv);
                env_bindings = (uu___871_62876.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_62876.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_62888 = g  in
      {
        env_tcenv = (uu___875_62888.env_tcenv);
        env_bindings = (uu___875_62888.env_bindings);
        tydefs = (uu___875_62888.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_62888.currentModule)
      }
      ...
      let uu____62963 =
        let uu____62964 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____62964  in
      extend_lb env uu____62963 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____62955 with | (g,uu____62968,uu____62969) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____62990 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____62990
         in
      let uu____62991 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____62991, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____63013 =
          let uu____63016 =
            let uu____63019 = FStar_Ident.id_of_text nm  in [uu____63019]  in
          FStar_List.append module_name uu____63016  in
        FStar_Ident.lid_of_ids uu____63013  in
      let uu____63020 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63020, lid)
