    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____293,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____306,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____319 = FStar_Options.codegen ()  in
           uu____319 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____324 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____352 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____352
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____358 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____358
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____409))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____450::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____464 'Auu____465 'Auu____466 'Auu____467 .
    ('Auu____464,'Auu____465,'Auu____466,FStar_Extraction_ML_Syntax.mlidents,
      'Auu____467,FStar_Extraction_ML_Syntax.mltybody
  ...
  fun uu____488  ->
    ...
    | (uu____503,uu____504,uu____505,vars,uu____507,body_opt) ->
        (match body_opt with
                   ...
                   then
                     FStar_Util.find_map tds
                       (fun td  ->
                          ...
                          | (uu____587,n1,uu____589,uu____590,uu____591,uu____592)
                              ->
                              if n1 = ty_name
           ...
           | (m,tds) ->
               FStar_Util.find_map tds
                 (fun uu____687  ->
                    ...
                    | (uu____696,n1,mangle_opt,uu____699,uu____700,uu____701)
                        ->
                        if m = mname
                        then
                          (if n1 = ty_name
          ...
          let uu____866 =
            let uu____867 =
              FStar_Range.string_of_range
               ...
               in
            let uu____868 = FStar_Syntax_Print.bv_to_string bv  in
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____901 = lookup_bv g x1  in
          (uu____901, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____905 = lookup_fv g x1  in
          (uu____905, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____932 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___61_993 = g  in
        {
          tcenv;
          gamma;
          tydefs = (uu___61_993.tydefs);
          type_names = (uu___61_993.type_names);
          currentModule = (uu___61_993.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____1023 =
               let uu____1026 = valid c  in
               if uu____1026 then [c] else [95; 95]  in
             FStar_List.append uu____1023 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___60_1085  ->
               match uu___60_1085 with
               | Bv (uu____1086,FStar_Util.Inl (mlident',uu____1088)) ->
                   target_mlident = mlident'
               | Fv (uu____1117,FStar_Util.Inl (mlident',uu____1119)) ->
                   target_mlident = mlident'
               | Fv
                   (uu____1148,FStar_Util.Inr
                    (mlident',uu____1150,uu____1151,uu____1152))
                   -> target_mlident = mlident'
               | Bv
                   (uu____1181,FStar_Util.Inr
                    (mlident',uu____1183,uu____1184,uu____1185))
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____1258 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____1260 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.gamma uu____1260  in
              ...
              let gamma =
                ...
                (g.gamma)  in
              let tcenv =
                let uu____1301 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.tcenv uu____1301  in
              ((let uu___62_1303 = g  in
                {
                  tcenv;
                  gamma;
                  tydefs = (uu___62_1303.tydefs);
                  type_names = (uu___62_1303.type_names);
                  currentModule = (uu___62_1303.currentModule)
                }), mlident)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____1317 = mltyFvars t1  in
        let uu____1320 = mltyFvars t2  in
        FStar_List.append uu____1317 uu____1320
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____1359 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____1359 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____1400 = tySchemeIsClosed t_x  in
              if uu____1400
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____1409 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____1410 =
                  let uu____1421 = y  in
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma)  in
                    ((let uu___63_1504 = g  in
                      {
                        tcenv = (uu___63_1504.tcenv);
                        gamma;
                        tydefs = (uu___63_1504.tydefs);
                        type_names = (uu___63_1504.type_names);
                        currentModule = (uu___63_1504.currentModule)
                      }), mlsymbol)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___64_1615 = g  in
        {
          tcenv = (uu___64_1615.tcenv);
          gamma = (uu___64_1615.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          type_names = (fv :: (g.type_names));
          currentModule = (uu___64_1615.currentModule)
        }
  
let (extend_type_name : env -> FStar_Syntax_Syntax.fv -> env) =
  fun g  ->
    fun fv  ->
      let uu___65_1634 = g  in
      {
        tcenv = (uu___65_1634.tcenv);
        gamma = (uu___65_1634.gamma);
        tydefs = (uu___65_1634.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___65_1634.currentModule)
      }
      ...
      let uu____1692 =
        let uu____1693 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____1693  in
      extend_lb env uu____1692 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    FStar_All.pipe_right uu____1687 FStar_Pervasives_Native.fst
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____1713 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____1713
         in
      let uu____1714 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____1714, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____1734 =
          let uu____1737 =
            let uu____1740 = FStar_Ident.id_of_text nm  in [uu____1740]  in
          FStar_List.append module_name uu____1737  in
        FStar_Ident.lid_of_ids uu____1734  in
      let uu____1741 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____1741, lid)
