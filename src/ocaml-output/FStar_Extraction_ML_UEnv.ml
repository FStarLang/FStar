...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____61379 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____61415 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61750,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61766,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____61783 = FStar_Options.codegen ()  in
           uu____61783 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____61788 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61845 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____61845
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61854 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____61854
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____61880))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____61888::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____61904 'Auu____61905 'Auu____61906 'Auu____61907 .
    ('Auu____61904 * 'Auu____61905 * 'Auu____61906 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____61907 *
  ...
  fun uu____61928  ->
    ...
    | (uu____61943,uu____61944,uu____61945,vars,uu____61947,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____61958 -> FStar_Pervasives_Native.None)
    ...
    fun lid  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___526_62138  ->
             match uu___526_62138 with
             | Fv (fv',x) when FStar_Syntax_Syntax.fv_eq_lid fv' lid ->
                 FStar_Pervasives_Native.Some x
             | uu____62143 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62144 =
      ...
      | FStar_Pervasives_Native.None  ->
          let uu____62186 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____62188 uu____62190
             in
          failwith uu____62186
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_62211  ->
             match uu___528_62211 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____62216 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62217 =
            let uu____62219 =
              FStar_Range.string_of_range
               ...
               in
            let uu____62221 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____62219 uu____62221
             in
          failwith uu____62217
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____62257 = lookup_bv g x1  in
          (uu____62257, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____62261 =
            let uu____62262 = lookup_fv g x1  in FStar_Util.Inr uu____62262
             in
          (uu____62261, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____62290 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_62326 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_62326.tydefs);
          type_names = (uu___726_62326.type_names);
          currentModule = (uu___726_62326.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____62371 =
               let uu____62375 = valid c  in
               if uu____62375 then [c] else [95; 95]  in
             FStar_List.append uu____62371 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_62465  ->
               match uu___529_62465 with
               | Bv (uu____62467,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____62470,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____62473,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____62538 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____62541 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____62541  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____62562 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____62562
                 in
              ((let uu___787_62565 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_62565.tydefs);
                  type_names = (uu___787_62565.type_names);
                  currentModule = (uu___787_62565.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____62585 = mltyFvars t1  in
        let uu____62589 = mltyFvars t2  in
        FStar_List.append uu____62585 uu____62589
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____62650 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____62650 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____62703 = tySchemeIsClosed t_x  in
              if uu____62703
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____62716 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____62717 =
                  let uu____62723 = y  in
                  match uu____62723 with
                  | (ns,i) ->
                      let mlsymbol =
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_62774 = g  in
                      {
                        env_tcenv = (uu___838_62774.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_62774.tydefs);
                        type_names = (uu___838_62774.type_names);
                        currentModule = (uu___838_62774.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____62932 = td  in
        match uu____62932 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____62958 =
                let uu____62959 = tyscheme_of_td td  in
                FStar_Option.get uu____62959  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____62958
              }  in
            ((let uu___871_62968 = g  in
              {
                env_tcenv = (uu___871_62968.env_tcenv);
                env_bindings = (uu___871_62968.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_62968.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_62980 = g  in
      {
        env_tcenv = (uu___875_62980.env_tcenv);
        env_bindings = (uu___875_62980.env_bindings);
        tydefs = (uu___875_62980.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_62980.currentModule)
      }
      ...
      let uu____63055 =
        let uu____63056 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____63056  in
      extend_lb env uu____63055 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____63047 with | (g,uu____63060,uu____63061) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____63082 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____63082
         in
      let uu____63083 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63083, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____63105 =
          let uu____63108 =
            let uu____63111 = FStar_Ident.id_of_text nm  in [uu____63111]  in
          FStar_List.append module_name uu____63108  in
        FStar_Ident.lid_of_ids uu____63105  in
      let uu____63112 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63112, lid)
