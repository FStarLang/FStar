...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____56681 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____56716 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____57050,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____57066,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____57083 = FStar_Options.codegen ()  in
           uu____57083 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____57088 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____57145 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____57145
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____57154 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____57154
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____57180))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____57188::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____57204 'Auu____57205 'Auu____57206 'Auu____57207 .
    ('Auu____57204 * 'Auu____57205 * 'Auu____57206 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____57207 *
  ...
  =
  fun uu____57228  ->
    match uu____57228 with
    | (uu____57243,uu____57244,uu____57245,vars,uu____57247,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____57258 -> FStar_Pervasives_Native.None)
    ...
    fun lid  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___526_57438  ->
             match uu___526_57438 with
             | Fv (fv',x) when FStar_Syntax_Syntax.fv_eq_lid fv' lid ->
                 FStar_Pervasives_Native.Some x
             | uu____57443 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____57444 =
...
let (lookup_fv : uenv -> FStar_Syntax_Syntax.fv -> exp_binding) =
  fun g  ->
    fun fv  ->
      let uu____57483 = try_lookup_fv g fv  in
      ...
      | FStar_Pervasives_Native.None  ->
          let uu____57486 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____57488 uu____57490
             in
          failwith uu____57486
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_57511  ->
             match uu___528_57511 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____57516 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____57517 =
            let uu____57519 =
              FStar_Range.string_of_range
               ...
               in
            let uu____57521 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____57519 uu____57521
             in
          failwith uu____57517
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____57557 = lookup_bv g x1  in
          (uu____57557, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____57561 =
            let uu____57562 = lookup_fv g x1  in FStar_Util.Inr uu____57562
             in
          (uu____57561, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____57590 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_57626 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_57626.tydefs);
          type_names = (uu___726_57626.type_names);
          currentModule = (uu___726_57626.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____57671 =
               let uu____57675 = valid c  in
               if uu____57675 then [c] else [95; 95]  in
             FStar_List.append uu____57671 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_57765  ->
               match uu___529_57765 with
               | Bv (uu____57767,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____57770,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____57773,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____57838 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____57841 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____57841  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____57862 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____57862
                 in
              ((let uu___787_57865 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_57865.tydefs);
                  type_names = (uu___787_57865.type_names);
                  currentModule = (uu___787_57865.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____57885 = mltyFvars t1  in
        let uu____57889 = mltyFvars t2  in
        FStar_List.append uu____57885 uu____57889
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____57950 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____57950 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____58003 = tySchemeIsClosed t_x  in
              if uu____58003
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____58016 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____58017 =
                  let uu____58023 = y  in
                  match uu____58023 with
                  | (ns,i) ->
                      let mlsymbol =
                        FStar_Extraction_ML_Syntax.avoid_keyword i  in
                      ((ns, mlsymbol), mlsymbol)
                   in
                match uu____58017 with
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_58074 = g  in
                      {
                        env_tcenv = (uu___838_58074.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_58074.tydefs);
                        type_names = (uu___838_58074.type_names);
                        currentModule = (uu___838_58074.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____58232 = td  in
        match uu____58232 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____58258 =
                let uu____58259 = tyscheme_of_td td  in
                FStar_Option.get uu____58259  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____58258
              }  in
            ((let uu___871_58268 = g  in
              {
                env_tcenv = (uu___871_58268.env_tcenv);
                env_bindings = (uu___871_58268.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_58268.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_58280 = g  in
      {
        env_tcenv = (uu___875_58280.env_tcenv);
        env_bindings = (uu___875_58280.env_bindings);
        tydefs = (uu___875_58280.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_58280.currentModule)
      }
      ...
      let uu____58355 =
        let uu____58356 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____58356  in
      extend_lb env uu____58355 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____58347 with | (g,uu____58360,uu____58361) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____58382 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____58382
         in
      let uu____58383 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____58383, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____58405 =
          let uu____58408 =
            let uu____58411 = FStar_Ident.id_of_text nm  in [uu____58411]  in
          FStar_List.append module_name uu____58408  in
        FStar_Ident.lid_of_ids uu____58405  in
      let uu____58412 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____58412, lid)
