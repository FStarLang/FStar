...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____61384 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____61420 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61755,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61771,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____61788 = FStar_Options.codegen ()  in
           uu____61788 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____61793 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61850 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____61850
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61859 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____61859
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____61885))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____61893::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____61909 'Auu____61910 'Auu____61911 'Auu____61912 .
    ('Auu____61909 * 'Auu____61910 * 'Auu____61911 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____61912 *
  ...
  =
  fun uu____61933  ->
    match uu____61933 with
    | (uu____61948,uu____61949,uu____61950,vars,uu____61952,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____61963 -> FStar_Pervasives_Native.None)
      ...
      | FStar_Pervasives_Native.None  ->
          let uu____62191 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____62193 uu____62195
             in
          failwith uu____62191
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_62216  ->
             match uu___528_62216 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____62221 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62222 =
            let uu____62224 =
              FStar_Range.string_of_range
               ...
               in
            let uu____62226 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____62224 uu____62226
             in
          failwith uu____62222
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____62262 = lookup_bv g x1  in
          (uu____62262, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____62266 =
            let uu____62267 = lookup_fv g x1  in FStar_Util.Inr uu____62267
             in
          (uu____62266, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____62295 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_62331 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_62331.tydefs);
          type_names = (uu___726_62331.type_names);
          currentModule = (uu___726_62331.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____62376 =
               let uu____62380 = valid c  in
               if uu____62380 then [c] else [95; 95]  in
             FStar_List.append uu____62376 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_62470  ->
               match uu___529_62470 with
               | Bv (uu____62472,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____62475,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____62478,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____62543 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____62546 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____62546  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____62567 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____62567
                 in
              ((let uu___787_62570 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_62570.tydefs);
                  type_names = (uu___787_62570.type_names);
                  currentModule = (uu___787_62570.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____62590 = mltyFvars t1  in
        let uu____62594 = mltyFvars t2  in
        FStar_List.append uu____62590 uu____62594
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____62655 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____62655 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____62708 = tySchemeIsClosed t_x  in
              if uu____62708
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____62721 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____62722 =
                  let uu____62728 = y  in
                  match uu____62728 with
                  | (ns,i) ->
                      let mlsymbol =
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_62779 = g  in
                      {
                        env_tcenv = (uu___838_62779.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_62779.tydefs);
                        type_names = (uu___838_62779.type_names);
                        currentModule = (uu___838_62779.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____62937 = td  in
        match uu____62937 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____62963 =
                let uu____62964 = tyscheme_of_td td  in
                FStar_Option.get uu____62964  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____62963
              }  in
            ((let uu___871_62973 = g  in
              {
                env_tcenv = (uu___871_62973.env_tcenv);
                env_bindings = (uu___871_62973.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_62973.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_62985 = g  in
      {
        env_tcenv = (uu___875_62985.env_tcenv);
        env_bindings = (uu___875_62985.env_bindings);
        tydefs = (uu___875_62985.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_62985.currentModule)
      }
      ...
      let uu____63060 =
        let uu____63061 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____63061  in
      extend_lb env uu____63060 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____63052 with | (g,uu____63065,uu____63066) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____63087 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____63087
         in
      let uu____63088 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63088, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____63110 =
          let uu____63113 =
            let uu____63116 = FStar_Ident.id_of_text nm  in [uu____63116]  in
          FStar_List.append module_name uu____63113  in
        FStar_Ident.lid_of_ids uu____63110  in
      let uu____63117 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63117, lid)
