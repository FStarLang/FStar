...
let removeTick uu____161 =
  ...
  | (x,n) ->
      if FStar_Util.starts_with x "'"
      then
        let _0_192 = FStar_Util.substring_from x (Prims.parse_int "1") in
        (_0_192, n)
      else (x, n)
      ...
      | None  ->
          failwith
            (let _0_196 =
             ...
             let _0_195 = FStar_Syntax_Print.bv_to_string bv in
...
let lookup:
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x -> let _0_197 = lookup_bv g x in (_0_197, None)
      | FStar_Util.Inr x ->
          let _0_198 = lookup_fv g x in
          (_0_198, (x.FStar_Syntax_Syntax.fv_qual))
let lookup_term:
  env ->
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___105_542 = g in
        {
          tcenv;
          gamma;
          tydefs = (uu___105_542.tydefs);
          currentModule = (uu___105_542.currentModule)
        }
let extend_bv:
  env ->
    FStar_Syntax_Syntax.bv ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              ...
              let gamma = (Bv (x, (FStar_Util.Inr (mlx, t_x, is_rec)))) ::
                (g.gamma) in
              let tcenv =
                let _0_199 = FStar_Syntax_Syntax.binders_of_list [x] in
                FStar_TypeChecker_Env.push_binders g.tcenv _0_199 in
              let uu___106_585 = g in
              {
                tcenv;
                gamma;
                tydefs = (uu___106_585.tydefs);
                currentModule = (uu___106_585.currentModule)
              }
let rec mltyFvars:
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let _0_201 = mltyFvars t1 in
        let _0_200 = mltyFvars t2 in FStar_List.append _0_201 _0_200
...
let tySchemeIsClosed: FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool =
  fun tys  ->
    let _0_202 = mltyFvars (Prims.snd tys) in
    subsetMlidents _0_202 (Prims.fst tys)
let extend_fv':
  env ->
    FStar_Syntax_Syntax.fv ->
      FStar_Extraction_ML_Syntax.mlpath ->
            ...
            fun is_rec  ->
              ...
              then
                ...
                let gamma = (Fv (x, (FStar_Util.Inr (mly, t_x, is_rec)))) ::
                  (g.gamma) in
                let uu___107_665 = g in
                {
                  tcenv = (uu___107_665.tcenv);
                  gamma;
                  tydefs = (uu___107_665.tydefs);
                  currentModule = (uu___107_665.currentModule)
                }
        ...
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              match l with
              | FStar_Util.Inl x ->
                  let _0_204 = extend_bv g x t_x add_unit is_rec false in
                  let _0_203 = bv_as_ml_termvar x in (_0_204, _0_203)
              | FStar_Util.Inr f ->
                  let uu____715 =
                    FStar_Extraction_ML_Syntax.mlpath_of_lident
                  ...
                  (match uu____715 with
                   | (p,y) ->
                       let _0_205 = extend_fv' g f (p, y) t_x add_unit is_rec in
                       (_0_205,
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___108_740 = g in
        {
          tcenv = (uu___108_740.tcenv);
          gamma = (uu___108_740.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          currentModule = (uu___108_740.currentModule)
        }
let emptyMlPath:
...
let mkContext: FStar_TypeChecker_Env.env -> env =
  fun e  ->
    ...
    let _0_207 =
      ...
      extend_lb env _0_206 FStar_Syntax_Syntax.tun failwith_ty false false in
    FStar_All.pipe_right _0_207 Prims.fst
let monad_op_name:
