  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___229_964 = g  in
        {
          tcenv;
          gamma;
          tydefs = (uu___229_964.tydefs);
          type_names = (uu___229_964.type_names);
          currentModule = (uu___229_964.currentModule)
        }
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____1186 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____1188 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.gamma uu____1188  in
              ...
              let gamma =
                ...
                (g.gamma)  in
              let tcenv =
                let uu____1213 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.tcenv uu____1213  in
              ((let uu___230_1215 = g  in
                {
                  tcenv;
                  gamma;
                  tydefs = (uu___230_1215.tydefs);
                  type_names = (uu___230_1215.type_names);
                  currentModule = (uu___230_1215.currentModule)
                }), mlident)
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____1271 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____1271 (FStar_Pervasives_Native.fst tys)
              ...
              then
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma)  in
                    ((let uu___231_1370 = g  in
                      {
                        tcenv = (uu___231_1370.tcenv);
                        gamma;
                        tydefs = (uu___231_1370.tydefs);
                        type_names = (uu___231_1370.type_names);
                        currentModule = (uu___231_1370.currentModule)
                      }), mlsymbol)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___232_1481 = g  in
        {
          tcenv = (uu___232_1481.tcenv);
          gamma = (uu___232_1481.gamma);
          tydefs = ((fv, m, td) :: (g.tydefs));
          type_names = (fv :: (g.type_names));
          currentModule = (uu___232_1481.currentModule)
        }
  
let (extend_type_name : env -> FStar_Syntax_Syntax.fv -> env) =
  fun g  ->
    fun fv  ->
      let uu___233_1502 = g  in
      {
        tcenv = (uu___233_1502.tcenv);
        gamma = (uu___233_1502.gamma);
        tydefs = (uu___233_1502.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___233_1502.currentModule)
      }
      ...
      let uu____1594 =
        let uu____1595 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____1595  in
      extend_lb env uu____1594 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    FStar_All.pipe_right uu____1589 FStar_Pervasives_Native.fst
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____1615 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____1615
         in
      let uu____1616 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____1616, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____1636 =
          let uu____1639 =
            let uu____1642 = FStar_Ident.id_of_text nm  in [uu____1642]  in
          FStar_List.append module_name uu____1639  in
        FStar_Ident.lid_of_ids uu____1636  in
      let uu____1643 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____1643, lid)
