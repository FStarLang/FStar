...
in (match (x) with
| None -> begin
(let _168_90 = (FStar_Util.format1 "free Variable %s not found\n" lid.FStar_Ident.nsstr)
in (FStar_All.failwith _168_90))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _168_98 = (let _168_97 = (FStar_Range.string_of_range fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.p)
in (let _168_96 = (FStar_Syntax_Print.lid_to_string fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _168_97 _168_96)))
in (FStar_All.failwith _168_98))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _168_106 = (let _168_105 = (FStar_Range.string_of_range bv.FStar_Syntax_Syntax.ppname.FStar_Ident.idRange)
in (let _168_104 = (FStar_Syntax_Print.bv_to_string bv)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _168_105 _168_104)))
in (FStar_All.failwith _168_106))
end
| Some (y) -> begin
y
end)))
...
(let _168_111 = (lookup_bv g x)
in ((_168_111), (None)))
end
| FStar_Util.Inr (x) -> begin
(let _168_112 = (lookup_fv g x)
in ((_168_112), (x.FStar_Syntax_Syntax.fv_qual)))
end))
...
let mlx = (let _168_135 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in FStar_Extraction_ML_Syntax.MLE_Var (_168_135))
in (
...
let tcenv = (let _168_136 = (FStar_Syntax_Syntax.binders_of_list ((x)::[]))
in (FStar_TypeChecker_Env.push_binders g.tcenv _168_136))
in (
...
(let _168_140 = (mltyFvars t1)
in (let _168_139 = (mltyFvars t2)
in (FStar_List.append _168_140 _168_139)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _168_147 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _168_147 (Prims.fst tys))))
...
| FStar_Util.Inl (x) -> begin
(let _168_183 = (extend_bv g x t_x add_unit is_rec false)
in (let _168_182 = (bv_as_ml_termvar x)
in ((_168_183), (_168_182))))
end
...
| (p, y) -> begin
(let _168_184 = (extend_fv' g f ((p), (y)) t_x add_unit is_rec)
in ((_168_184), ((((FStar_Extraction_ML_Syntax.avoid_keyword y)), ((Prims.parse_int "0"))))))
end))
end))
...
in (let _168_193 = (let _168_192 = (let _168_191 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
in FStar_Util.Inr (_168_191))
in (extend_lb env _168_192 FStar_Syntax_Syntax.tun failwith_ty false false))
in (FStar_All.pipe_right _168_193 Prims.fst))))))
