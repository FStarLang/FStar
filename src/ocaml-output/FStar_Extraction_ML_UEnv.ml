...
| (x, n) -> begin
if (FStar_Util.starts_with x "\'") then begin
(let _174_59 = (FStar_Util.substring_from x (Prims.parse_int "1"))
in ((_174_59), (n)))
end else begin
((x), (n))
end
...
let bv_as_ml_tyvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _174_66 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (prependTick _174_66)))


let bv_as_ml_termvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _174_69 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (removeTick _174_69)))
...
in (match (x) with
| None -> begin
(let _174_99 = (FStar_Util.format1 "free Variable %s not found\n" lid.FStar_Ident.nsstr)
in (FStar_All.failwith _174_99))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _174_107 = (let _174_106 = (FStar_Range.string_of_range fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.p)
in (let _174_105 = (FStar_Syntax_Print.lid_to_string fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _174_106 _174_105)))
in (FStar_All.failwith _174_107))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _174_115 = (let _174_114 = (FStar_Range.string_of_range bv.FStar_Syntax_Syntax.ppname.FStar_Ident.idRange)
in (let _174_113 = (FStar_Syntax_Print.bv_to_string bv)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _174_114 _174_113)))
in (FStar_All.failwith _174_115))
end
| Some (y) -> begin
y
end)))
...
(let _174_120 = (lookup_bv g x)
in ((_174_120), (None)))
end
| FStar_Util.Inr (x) -> begin
(let _174_121 = (lookup_fv g x)
in ((_174_121), (x.FStar_Syntax_Syntax.fv_qual)))
end))
...
let mlx = (let _174_144 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in FStar_Extraction_ML_Syntax.MLE_Var (_174_144))
in (
...
let tcenv = (let _174_145 = (FStar_Syntax_Syntax.binders_of_list ((x)::[]))
in (FStar_TypeChecker_Env.push_binders g.tcenv _174_145))
in (
...
(let _174_149 = (mltyFvars t1)
in (let _174_148 = (mltyFvars t2)
in (FStar_List.append _174_149 _174_148)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _174_156 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _174_156 (Prims.fst tys))))
...
| FStar_Util.Inl (x) -> begin
(let _174_192 = (extend_bv g x t_x add_unit is_rec false)
in (let _174_191 = (bv_as_ml_termvar x)
in ((_174_192), (_174_191))))
end
...
| (p, y) -> begin
(let _174_193 = (extend_fv' g f ((p), (y)) t_x add_unit is_rec)
in ((_174_193), ((((FStar_Extraction_ML_Syntax.avoid_keyword y)), ((Prims.parse_int "0"))))))
end))
end))
...
in (let _174_204 = (let _174_203 = (let _174_202 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
in FStar_Util.Inr (_174_202))
in (extend_lb env _174_203 FStar_Syntax_Syntax.tun failwith_ty false false))
in (FStar_All.pipe_right _174_204 Prims.fst))))))
