...
let __proj__Mkenv__item__tcenv : env -> FStar_TypeChecker_Env.env =
  fun projectee ->
    match projectee
    with
    | { tcenv= __fname__tcenv
      ; gamma= __fname__gamma
      ; tydefs= __fname__tydefs
      ; type_names= __fname__type_names
      ; currentModule= __fname__currentModule; _ }
    -> __fname__tcenv
...
let debug : env -> (Prims.unit -> Prims.unit) -> Prims.unit =
  fun g f ->
    ...
    let uu____257 =
      FStar_Options.debug_at_level c (FStar_Options.Other "Extraction")
    in
...
let erasableTypeNoDelta : FStar_Extraction_ML_Syntax.mlty -> Prims.bool =
  fun t ->
    if t = FStar_Extraction_ML_Syntax.ml_unit_ty then true
    else
      match t with
      | FStar_Extraction_ML_Syntax.MLTY_Named
          (uu____269, (["FStar"; "Ghost"], "erased")) ->
          true
      | FStar_Extraction_ML_Syntax.MLTY_Named
...
let removeTick : Prims.string -> Prims.string =
  fun x ->
    if FStar_Util.starts_with x "'" then
...
let rec lookup_ty_local
    : binding Prims.list -> FStar_Syntax_Syntax.bv
      -> FStar_Extraction_ML_Syntax.mlty =
  fun gamma b ->
    match gamma with
    | (Bv (b', FStar_Util.Inl (mli, mlt))) :: tl1 ->
        if FStar_Syntax_Syntax.bv_eq b b' then mlt else lookup_ty_local tl1 b
    | (Bv (b', FStar_Util.Inr uu____369)) :: tl1 ->
        if FStar_Syntax_Syntax.bv_eq b b' then
          failwith
...
let tyscheme_of_td:
  ...
  fun uu____442 ->
    match uu____442
    with uu____457, uu____458, uu____459, vars, uu____461, body_opt ->
      match body_opt with
...
let lookup_ty_const
    : env -> FStar_Extraction_ML_Syntax.mlpath
      -> FStar_Extraction_ML_Syntax.mltyscheme FStar_Pervasives_Native.option =
  fun env uu____486 ->
    match uu____486 with module_name, ty_name ->
      FStar_Util.find_map env.tydefs (fun uu____506 ->
          match uu____506 with m, tds ->
            if module_name = m then
              FStar_Util.find_map tds (fun td ->
                  ...
                  ->
                    if n1 = ty_name then tyscheme_of_td td
...
let maybe_mangle_type_projector
    : env -> FStar_Syntax_Syntax.fv
      -> FStar_Extraction_ML_Syntax.mlpath FStar_Pervasives_Native.option =
  fun env fv ->
    let mname = module_name_of_fv fv in
    let ty_name =
      fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v.FStar_Ident.ident
        .FStar_Ident.idText
    in
    FStar_Util.find_map env.tydefs (fun uu____597 ->
        match uu____597 with m, tds ->
          FStar_Util.find_map tds (fun uu____631 ->
              match uu____631
              with
                ...
                if m = mname then
                  if n1 = ty_name then
                    ...
                    | FStar_Pervasives_Native.Some mangled ->
                        let modul = m in
                        FStar_Pervasives_Native.Some (modul, mangled)
                  else FStar_Pervasives_Native.None
                else FStar_Pervasives_Native.None ) )
...
let lookup_tyvar
  ...
  fun g bt -> lookup_ty_local g.gamma bt


let lookup_fv_by_lid : env -> FStar_Ident.lident -> ty_or_exp_b =
  fun g lid ->
    let x =
      FStar_Util.find_map g.gamma (fun uu___55_732 ->
...
let lookup_fv : env -> FStar_Syntax_Syntax.fv -> ty_or_exp_b =
  fun g fv ->
    let x =
      FStar_Util.find_map g.gamma (fun uu___56_752 ->
...
let lookup_bv : env -> FStar_Syntax_Syntax.bv -> ty_or_exp_b =
  fun g bv ->
    let x =
      FStar_Util.find_map g.gamma (fun uu___57_774 ->
...
let lookup
    : env -> (FStar_Syntax_Syntax.bv, FStar_Syntax_Syntax.fv) FStar_Util.either
      -> ( ty_or_exp_b
...
let extend_ty
  ...
  fun g a mapped_to ->
    ...
    in
    let gamma = Bv (a, FStar_Util.Inl (ml_a, mapped_to1)) :: g.gamma in
      ...
      FStar_List.fold_right
        (fun c cs1 ->
          let uu____919 =
            ...
            if uu____922 then [c] else [95; 95]
          in
          FStar_List.append uu____919 cs1 )
        cs []
    in
    let cs'1 =
      match cs' with
...
let find_uniq : binding Prims.list -> Prims.string -> Prims.string =
  fun gamma mlident ->
    let rec find_uniq mlident1 i =
      let suffix =
        if i = Prims.parse_int "0" then "" else FStar_Util.string_of_int i
      in
      ...
      let has_collision =
        FStar_List.existsb
          (fun uu___58_973 ->
            ...
            | Fv
                ( uu____1036
                , FStar_Util.Inr (mlident', uu____1038, uu____1039, uu____1040)
                ) ->
                target_mlident = mlident'
            | Bv
                ( uu____1069
                , FStar_Util.Inr (mlident', uu____1071, uu____1072, uu____1073)
                ) ->
                target_mlident = mlident' )
          gamma
      in
      if has_collision then find_uniq mlident1 (i + Prims.parse_int "1")
      else target_mlident
    in
...
let extend_bv
    : env -> FStar_Syntax_Syntax.bv -> FStar_Extraction_ML_Syntax.mltyscheme
      -> Prims.bool -> Prims.bool -> Prims.bool
      -> ( env
         , FStar_Extraction_ML_Syntax.mlident )
         FStar_Pervasives_Native.tuple2 =
  fun g x t_x add_unit is_rec mk_unit ->
    let ml_ty =
      match t_x with
      | [], t -> t
      | uu____1134 -> FStar_Extraction_ML_Syntax.MLTY_Top
    in
    let mlident =
      let uu____1136 = FStar_Extraction_ML_Syntax.bv_as_mlident x in
      find_uniq g.gamma uu____1136
    in
    let mlx = FStar_Extraction_ML_Syntax.MLE_Var mlident in
    let mlx1 =
      if mk_unit then FStar_Extraction_ML_Syntax.ml_unit
      else if add_unit then
        FStar_All.pipe_left
    ...
    in
    let t_x1 =
      if add_unit then FStar_Extraction_ML_Syntax.pop_unit t_x else t_x
    in
    let gamma =
      Bv (x, FStar_Util.Inr (mlident, mlx1, t_x1, is_rec)) :: g.gamma
    in
    let tcenv =
      let uu____1177 = FStar_Syntax_Syntax.binders_of_list [x] in
      FStar_TypeChecker_Env.push_binders g.tcenv uu____1177
    in
...
let extend_fv'
    : env -> FStar_Syntax_Syntax.fv -> FStar_Extraction_ML_Syntax.mlpath
      -> FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool -> Prims.bool
      -> ( env
         , FStar_Extraction_ML_Syntax.mlident )
         FStar_Pervasives_Native.tuple2 =
  fun g x y t_x add_unit is_rec ->
    let uu____1256 = tySchemeIsClosed t_x in
    if uu____1256 then
      let ml_ty =
        match t_x with
        | [], t -> t
        | uu____1265 -> FStar_Extraction_ML_Syntax.MLTY_Top
      in
      let uu____1266 =
        let uu____1277 = y in
        match uu____1277 with ns, i ->
          let mlsymbol = FStar_Extraction_ML_Syntax.avoid_keyword i in
          ((ns, mlsymbol), mlsymbol)
      in
      match uu____1266 with mlpath, mlsymbol ->
          ...
          if add_unit then
            FStar_All.pipe_left
              ...
              (FStar_Extraction_ML_Syntax.MLE_App
                 ...
                 , [FStar_Extraction_ML_Syntax.ml_unit] ))
          else FStar_Extraction_ML_Syntax.with_ty ml_ty mly
        in
        let t_x1 =
          if add_unit then FStar_Extraction_ML_Syntax.pop_unit t_x else t_x
        in
        let gamma =
          Fv (x, FStar_Util.Inr (mlsymbol, mly1, t_x1, is_rec)) :: g.gamma
        in
...
let extend_fv
    : env -> FStar_Syntax_Syntax.fv -> FStar_Extraction_ML_Syntax.mltyscheme
      -> Prims.bool -> Prims.bool
      -> ( env
         , FStar_Extraction_ML_Syntax.mlident )
         FStar_Pervasives_Native.tuple2 =
  fun g x t_x add_unit is_rec ->
    let mlp =
...
let extend_lb
    : env -> FStar_Syntax_Syntax.lbname -> FStar_Syntax_Syntax.typ
      -> FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool -> Prims.bool
      -> ( env
         , FStar_Extraction_ML_Syntax.mlident )
         FStar_Pervasives_Native.tuple2 =
  fun g l t t_x add_unit is_rec ->
    match l with
    ...
    | FStar_Util.Inr f ->
        ...
        in
        match uu____1418 with p, y -> extend_fv' g f (p, y) t_x add_unit is_rec


let extend_tydef
    : env -> FStar_Syntax_Syntax.fv -> FStar_Extraction_ML_Syntax.mltydecl
      -> env =
  fun g fv td ->
  ...
  fun e ->
    ...
    let failwith_ty =
      ...
      , FStar_Extraction_ML_Syntax.MLTY_Fun
          ( FStar_Extraction_ML_Syntax.MLTY_Named ([], (["Prims"], "string"))
    ...
    let uu____1505 =
      let uu____1510 =
        ...
        in
        FStar_Util.Inr uu____1511
      in
      extend_lb env uu____1510 FStar_Syntax_Syntax.tun failwith_ty false false
    in
...
let action_name
    : FStar_Syntax_Syntax.eff_decl -> FStar_Syntax_Syntax.action
      -> ( FStar_Extraction_ML_Syntax.mlpath
         , FStar_Ident.lident )
         FStar_Pervasives_Native.tuple2 =
  fun ed a ->
