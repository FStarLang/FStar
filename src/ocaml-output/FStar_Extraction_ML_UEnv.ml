  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___113_1001 = g in
        {
          tcenv;
          gamma;
          tydefs = (uu___113_1001.tydefs);
          type_names = (uu___113_1001.type_names);
          currentModule = (uu___113_1001.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____1069 =
               let uu____1072 = valid c in
               if uu____1072 then [c] else [95; 95] in
             FStar_List.append uu____1069 cs1) cs [] in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___112_1163  ->
               match uu___112_1163 with
               | Bv (uu____1164,FStar_Util.Inl (mlident',uu____1166)) ->
                   target_mlident = (FStar_Pervasives_Native.fst mlident')
               | Fv (uu____1195,FStar_Util.Inl (mlident',uu____1197)) ->
                   target_mlident = (FStar_Pervasives_Native.fst mlident')
               | Fv
                   (uu____1226,FStar_Util.Inr
                    (mlident',uu____1228,uu____1229,uu____1230))
                   -> target_mlident = mlident'
               | Bv
                   (uu____1259,FStar_Util.Inr
                    (mlident',uu____1261,uu____1262,uu____1263))
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____1330 -> FStar_Extraction_ML_Syntax.MLTY_Top in
              let uu____1331 = FStar_Extraction_ML_Syntax.bv_as_mlident x in
              ...
              | (mlident,nocluewhat) ->
                  ...
                  let gamma =
                    ...
                    :: (g.gamma) in
                  let tcenv =
                    let uu____1384 = FStar_Syntax_Syntax.binders_of_list [x] in
                    FStar_TypeChecker_Env.push_binders g.tcenv uu____1384 in
                  ((let uu___114_1390 = g in
                    {
                      tcenv;
                      gamma;
                      tydefs = (uu___114_1390.tydefs);
                      type_names = (uu___114_1390.type_names);
                      currentModule = (uu___114_1390.currentModule)
                    }), mlident1)
let rec mltyFvars:
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____1403 = mltyFvars t1 in
        let uu____1406 = mltyFvars t2 in
        FStar_List.append uu____1403 uu____1406
...
let tySchemeIsClosed: FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool =
  fun tys  ->
    let uu____1442 = mltyFvars (FStar_Pervasives_Native.snd tys) in
    subsetMlidents uu____1442 (FStar_Pervasives_Native.fst tys)
let extend_fv':
  env ->
    FStar_Syntax_Syntax.fv ->
      FStar_Extraction_ML_Syntax.mlpath ->
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____1477 = tySchemeIsClosed t_x in
              if uu____1477
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____1486 -> FStar_Extraction_ML_Syntax.MLTY_Top in
                let uu____1487 =
                  let uu____1498 = y in
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma) in
                    ((let uu___115_1585 = g in
                      {
                        tcenv = (uu___115_1585.tcenv);
                        gamma;
                        tydefs = (uu___115_1585.tydefs);
                        type_names = (uu___115_1585.type_names);
                        currentModule = (uu___115_1585.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___116_1682 = g in
        {
          tcenv = (uu___116_1682.tcenv);
          gamma = (uu___116_1682.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          type_names = (fv :: (g.type_names));
          currentModule = (uu___116_1682.currentModule)
        }
let extend_type_name: env -> FStar_Syntax_Syntax.fv -> env =
  fun g  ->
    fun fv  ->
      let uu___117_1699 = g in
      {
        tcenv = (uu___117_1699.tcenv);
        gamma = (uu___117_1699.gamma);
        tydefs = (uu___117_1699.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___117_1699.currentModule)
      }
...
let mkContext: FStar_TypeChecker_Env.env -> env =
  fun e  ->
    ...
    let uu____1769 =
      ...
      extend_lb env uu____1774 FStar_Syntax_Syntax.tun failwith_ty false
        false in
...
let monad_op_name:
  ...
  fun ed  ->
    fun nm  ->
      ...
      let uu____1793 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid in
      (uu____1793, lid)
let action_name:
  FStar_Syntax_Syntax.eff_decl ->
  ...
  fun ed  ->
    fun a  ->
      ...
      let uu____1811 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid in
      (uu____1811, lid)