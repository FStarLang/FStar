...
let rec lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____222))::tl ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____244::tl -> lookup_ty_local tl b
      | [] ->
          failwith
                   ...
                   then
                     FStar_Util.find_map tds
                       (fun td  ->
                          ...
                          | (uu____315,n,uu____317,uu____318,uu____319) ->
           ...
           | (m,tds) ->
               FStar_Util.find_map tds
                 (fun uu____375  ->
                    ...
                    | (uu____380,n,mangle_opt,uu____383,uu____384) ->
                        if m = mname
                        then
                          (if n = ty_name
                           then
...
let lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x ->
          let uu____504 = lookup_bv g x  in (uu____504, None)
      | FStar_Util.Inr x ->
          let uu____507 = lookup_fv g x  in
          (uu____507, (x.FStar_Syntax_Syntax.fv_qual))
  ...
  fun gamma  ->
    fun mlident  ->
      let rec find_uniq mlident i =
        ...
        let target_mlident = Prims.strcat mlident suffix  in
        let has_collision =
          FStar_List.existsb
            (fun uu___106_586  ->
           ...
           in
        if has_collision
        then find_uniq mlident (i + (Prims.parse_int "1"))
              ...
              | (mlident,nocluewhat) ->
                  ...
                  let mlx =
                    ...
                    then FStar_Extraction_ML_Syntax.ml_unit
                     ...
                     in
                  let gamma =
                    (Bv (x, (FStar_Util.Inr (mlsymbol, mlx, t_x, is_rec))))
                    :: (g.gamma)  in
                  let tcenv =
                    let uu____715 = FStar_Syntax_Syntax.binders_of_list [x]
                       in
                    FStar_TypeChecker_Env.push_binders g.tcenv uu____715  in
                  ((let uu___108_718 = g  in
                    {
                      tcenv;
                      gamma;
                      tydefs = (uu___108_718.tydefs);
                      currentModule = (uu___108_718.currentModule)
                    }), mlident)
            ...
            fun is_rec  ->
              ...
              then
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      (Fv (x, (FStar_Util.Inr (mlsymbol, mly, t_x, is_rec))))
                      :: (g.gamma)  in
                    ((let uu___109_839 = g  in
                      {
                        tcenv = (uu___109_839.tcenv);
                        gamma;
                        tydefs = (uu___109_839.tydefs);
                        currentModule = (uu___109_839.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___110_916 = g  in
        {
          tcenv = (uu___110_916.tcenv);
          gamma = (uu___110_916.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          currentModule = (uu___110_916.currentModule)
        }
