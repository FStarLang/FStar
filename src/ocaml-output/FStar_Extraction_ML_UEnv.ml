    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____916 =
               let uu____919 = valid c  in
               if uu____919 then [c] else [95; 95]  in
             FStar_List.append uu____916 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___364_960  ->
               match uu___364_960 with
               | Bv (uu____961,FStar_Util.Inl (mlident',uu____963)) ->
                   target_mlident = mlident'
               | Fv (uu____992,FStar_Util.Inl (mlident',uu____994)) ->
                   target_mlident = mlident'
               | Fv
                   (uu____1023,FStar_Util.Inr
                    (mlident',uu____1025,uu____1026,uu____1027))
                   -> target_mlident = mlident'
               | Bv
        ...
        fun add_unit  ->
          fun is_rec  ->
            fun mk_unit  ->
              ...
              let mlident =
                let uu____1123 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
              ...
              ((let uu___366_1166 = g  in
                {
                  tcenv;
                  gamma;
                  tydefs = (uu___366_1166.tydefs);
                  type_names = (uu___366_1166.type_names);
                  currentModule = (uu___366_1166.currentModule)
                }), mlident)
              ...
              then
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma)  in
                    ((let uu___367_1347 = g  in
                      {
                        tcenv = (uu___367_1347.tcenv);
                        gamma;
                        tydefs = (uu___367_1347.tydefs);
                        type_names = (uu___367_1347.type_names);
                        currentModule = (uu___367_1347.currentModule)
                      }), mlsymbol)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___368_1430 = g  in
        {
          tcenv = (uu___368_1430.tcenv);
          gamma = (uu___368_1430.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          type_names = (fv :: (g.type_names));
          currentModule = (uu___368_1430.currentModule)
        }
  
let extend_type_name : env -> FStar_Syntax_Syntax.fv -> env =
  fun g  ->
    fun fv  ->
      let uu___369_1445 = g  in
      {
        tcenv = (uu___369_1445.tcenv);
        gamma = (uu___369_1445.gamma);
        tydefs = (uu___369_1445.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___369_1445.currentModule)
      }
