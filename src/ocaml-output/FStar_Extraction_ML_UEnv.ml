...
| (x, n) -> begin
if (FStar_Util.starts_with x "\'") then begin
(let _162_59 = (FStar_Util.substring_from x 1)
in (_162_59, n))
end else begin
(x, n)
end
...
let bv_as_ml_tyvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _162_66 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (prependTick _162_66)))


let bv_as_ml_termvar : FStar_Syntax_Syntax.bv  ->  (Prims.string * Prims.int) = (fun x -> (let _162_69 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in (removeTick _162_69)))
...
in (match (x) with
| None -> begin
(let _162_98 = (let _162_97 = (FStar_Range.string_of_range fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.p)
in (let _162_96 = (FStar_Syntax_Print.lid_to_string fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (FStar_Util.format2 "(%s) free Variable %s not found\n" _162_97 _162_96)))
in (FStar_All.failwith _162_98))
end
| Some (y) -> begin
y
end)))
...
in (match (x) with
| None -> begin
(let _162_106 = (let _162_105 = (FStar_Range.string_of_range bv.FStar_Syntax_Syntax.ppname.FStar_Ident.idRange)
in (let _162_104 = (FStar_Syntax_Print.bv_to_string bv)
in (FStar_Util.format2 "(%s) bound Variable %s not found\n" _162_105 _162_104)))
in (FStar_All.failwith _162_106))
end
| Some (y) -> begin
y
end)))
...
(let _162_111 = (lookup_bv g x)
in (_162_111, None))
end
| FStar_Util.Inr (x) -> begin
(let _162_112 = (lookup_fv g x)
in (_162_112, x.FStar_Syntax_Syntax.fv_qual))
end))
...
let mlx = (let _162_135 = (FStar_Extraction_ML_Syntax.bv_as_mlident x)
in FStar_Extraction_ML_Syntax.MLE_Var (_162_135))
in (
...
let tcenv = (let _162_136 = (FStar_Syntax_Syntax.binders_of_list ((x)::[]))
in (FStar_TypeChecker_Env.push_binders g.tcenv _162_136))
in (
...
(let _162_140 = (mltyFvars t1)
in (let _162_139 = (mltyFvars t2)
in (FStar_List.append _162_140 _162_139)))
end
...
let tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme  ->  Prims.bool = (fun tys -> (let _162_147 = (mltyFvars (Prims.snd tys))
in (subsetMlidents _162_147 (Prims.fst tys))))
...
| FStar_Util.Inl (x) -> begin
(let _162_183 = (extend_bv g x t_x add_unit is_rec false)
in (let _162_182 = (bv_as_ml_termvar x)
in (_162_183, _162_182)))
end
...
| (p, y) -> begin
(let _162_184 = (extend_fv' g f (p, y) t_x add_unit is_rec)
in (_162_184, (y, 0)))
end))
end))
...
in (let _162_193 = (let _162_192 = (let _162_191 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
