...
let (uu___is_Bv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Bv _0 -> true | uu____61346 -> false
...
let (uu___is_Fv : binding -> Prims.bool) =
  fun projectee  ->
    match projectee with | Fv _0 -> true | uu____61382 -> false
    ...
    then true
      ...
      (match t with
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61717,("FStar"::"Ghost"::[],"erased")) -> true
       | FStar_Extraction_ML_Syntax.MLTY_Named
           (uu____61733,("FStar"::"Tactics"::"Effect"::[],"tactic")) ->
           let uu____61750 = FStar_Options.codegen ()  in
           uu____61750 <> (FStar_Pervasives_Native.Some FStar_Options.Plugin)
       | uu____61755 -> false)
...
let (bv_as_ml_tyvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61812 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    prependTick uu____61812
  
let (bv_as_ml_termvar : FStar_Syntax_Syntax.bv -> Prims.string) =
  fun x  ->
    let uu____61821 = FStar_Extraction_ML_Syntax.bv_as_mlident x  in
    removeTick uu____61821
...
let rec (lookup_ty_local :
  ...
  fun gamma  ->
    fun b  ->
      ...
      | (Bv (b',FStar_Util.Inr uu____61847))::tl1 ->
          if FStar_Syntax_Syntax.bv_eq b b'
          then
            failwith
      ...
      | uu____61855::tl1 -> lookup_ty_local tl1 b
      | [] ->
          failwith
...
let tyscheme_of_td :
  'Auu____61871 'Auu____61872 'Auu____61873 'Auu____61874 .
    ('Auu____61871 * 'Auu____61872 * 'Auu____61873 *
      FStar_Extraction_ML_Syntax.mlidents * 'Auu____61874 *
  ...
  =
  fun uu____61895  ->
    match uu____61895 with
    | (uu____61910,uu____61911,uu____61912,vars,uu____61914,body_opt) ->
        (match body_opt with
         | FStar_Pervasives_Native.Some
             (FStar_Extraction_ML_Syntax.MLTD_Abbrev t) ->
             FStar_Pervasives_Native.Some (vars, t)
         | uu____61925 -> FStar_Pervasives_Native.None)
    ...
    fun lid  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___526_62105  ->
             match uu___526_62105 with
             | Fv (fv',x) when FStar_Syntax_Syntax.fv_eq_lid fv' lid ->
                 FStar_Pervasives_Native.Some x
             | uu____62110 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62111 =
      ...
      | FStar_Pervasives_Native.None  ->
          let uu____62153 =
            ...
            FStar_Util.format2 "(%s) free Variable %s not found\n"
              uu____62155 uu____62157
             in
          failwith uu____62153
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.env_bindings
          (fun uu___528_62178  ->
             match uu___528_62178 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____62183 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____62184 =
            let uu____62186 =
              FStar_Range.string_of_range
               ...
               in
            let uu____62188 = FStar_Syntax_Print.bv_to_string bv  in
            FStar_Util.format2 "(%s) bound Variable %s not found\n"
              uu____62186 uu____62188
             in
          failwith uu____62184
      | FStar_Pervasives_Native.Some y -> y
...
let (lookup :
  ...
  fun g  ->
    fun x  ->
      match x with
      | FStar_Util.Inl x1 ->
          let uu____62224 = lookup_bv g x1  in
          (uu____62224, FStar_Pervasives_Native.None)
      | FStar_Util.Inr x1 ->
          let uu____62228 =
            let uu____62229 = lookup_fv g x1  in FStar_Util.Inr uu____62229
             in
          (uu____62228, (x1.FStar_Syntax_Syntax.fv_qual))
...
let (lookup_term :
  ...
  fun g  ->
    fun t  ->
      ...
      | uu____62257 -> failwith "Impossible: lookup_term for a non-name"
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___726_62293 = g  in
        {
          env_tcenv = tcenv;
          env_bindings = gamma;
          tydefs = (uu___726_62293.tydefs);
          type_names = (uu___726_62293.type_names);
          currentModule = (uu___726_62293.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____62338 =
               let uu____62342 = valid c  in
               if uu____62342 then [c] else [95; 95]  in
             FStar_List.append uu____62338 cs1) cs []
       in
    let cs'1 =
      match cs' with
    ...
    fun mlident  ->
      let rec find_uniq mlident1 i =
        ...
        let has_collision =
          FStar_List.existsb
            (fun uu___529_62432  ->
               match uu___529_62432 with
               | Bv (uu____62434,FStar_Util.Inl ty_b) ->
                   target_mlident = ty_b.ty_b_name
               | Fv (uu____62437,exp_b) -> target_mlident = exp_b.exp_b_name
               | Bv (uu____62440,FStar_Util.Inr exp_b) ->
          ...
          fun is_rec  ->
            fun mk_unit  ->
              let ml_ty =
                match t_x with
                | ([],t) -> t
                | uu____62505 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
              let mlident =
                let uu____62508 = FStar_Extraction_ML_Syntax.bv_as_mlident x
                   in
                find_uniq g.env_bindings uu____62508  in
              ...
              let gamma = (Bv (x, (FStar_Util.Inr exp_binding))) ::
                (g.env_bindings)  in
              let tcenv =
                let uu____62529 = FStar_Syntax_Syntax.binders_of_list [x]  in
                FStar_TypeChecker_Env.push_binders g.env_tcenv uu____62529
                 in
              ((let uu___787_62532 = g  in
                {
                  env_tcenv = tcenv;
                  env_bindings = gamma;
                  tydefs = (uu___787_62532.tydefs);
                  type_names = (uu___787_62532.type_names);
                  currentModule = (uu___787_62532.currentModule)
                }), mlident, exp_binding)
...
let rec (mltyFvars :
  ...
  fun t  ->
    ...
    | FStar_Extraction_ML_Syntax.MLTY_Fun (t1,f,t2) ->
        let uu____62552 = mltyFvars t1  in
        let uu____62556 = mltyFvars t2  in
        FStar_List.append uu____62552 uu____62556
...
let (tySchemeIsClosed : FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool)
  =
  fun tys  ->
    let uu____62617 = mltyFvars (FStar_Pervasives_Native.snd tys)  in
    subsetMlidents uu____62617 (FStar_Pervasives_Native.fst tys)
    ...
    fun x  ->
      fun y  ->
        fun t_x  ->
          fun add_unit  ->
            fun is_rec  ->
              let uu____62670 = tySchemeIsClosed t_x  in
              if uu____62670
              then
                let ml_ty =
                  match t_x with
                  | ([],t) -> t
                  | uu____62683 -> FStar_Extraction_ML_Syntax.MLTY_Top  in
                let uu____62684 =
                  let uu____62690 = y  in
                  match uu____62690 with
                  | (ns,i) ->
                      let mlsymbol =
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    ((let uu___838_62741 = g  in
                      {
                        env_tcenv = (uu___838_62741.env_tcenv);
                        env_bindings = gamma;
                        tydefs = (uu___838_62741.tydefs);
                        type_names = (uu___838_62741.type_names);
                        currentModule = (uu___838_62741.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu____62899 = td  in
        match uu____62899 with
        | (_assumed,name,mangled,vars,metadata,body_opt) ->
            let tydef =
              let uu____62925 =
                let uu____62926 = tyscheme_of_td td  in
                FStar_Option.get uu____62926  in
              {
                tydef_fv = fv;
                tydef_mlmodule_name = m;
                tydef_name = name;
                tydef_mangled_name = mangled;
                tydef_def = uu____62925
              }  in
            ((let uu___871_62935 = g  in
              {
                env_tcenv = (uu___871_62935.env_tcenv);
                env_bindings = (uu___871_62935.env_bindings);
                tydefs = (tydef :: (g.tydefs));
                type_names = (fv :: (g.type_names));
                currentModule = (uu___871_62935.currentModule)
              }), tydef)
  
let (extend_type_name : uenv -> FStar_Syntax_Syntax.fv -> uenv) =
  fun g  ->
    fun fv  ->
      let uu___875_62947 = g  in
      {
        env_tcenv = (uu___875_62947.env_tcenv);
        env_bindings = (uu___875_62947.env_bindings);
        tydefs = (uu___875_62947.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___875_62947.currentModule)
      }
      ...
      let uu____63022 =
        let uu____63023 =
          FStar_Syntax_Syntax.lid_as_fv FStar_Parser_Const.failwith_lid
           ...
           in
        FStar_Util.Inr uu____63023  in
      extend_lb env uu____63022 FStar_Syntax_Syntax.tun failwith_ty false
        false
       in
    match uu____63014 with | (g,uu____63027,uu____63028) -> g
...
let (monad_op_name :
  ...
  fun ed  ->
    fun nm  ->
      let lid =
        let uu____63049 = FStar_Ident.id_of_text nm  in
        FStar_Syntax_Util.mk_field_projector_name_from_ident
          ed.FStar_Syntax_Syntax.mname uu____63049
         in
      let uu____63050 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63050, lid)
  ...
  fun ed  ->
    fun a  ->
      ...
      let lid =
        let uu____63072 =
          let uu____63075 =
            let uu____63078 = FStar_Ident.id_of_text nm  in [uu____63078]  in
          FStar_List.append module_name uu____63075  in
        FStar_Ident.lid_of_ids uu____63072  in
      let uu____63079 = FStar_Extraction_ML_Syntax.mlpath_of_lident lid  in
      (uu____63079, lid)
