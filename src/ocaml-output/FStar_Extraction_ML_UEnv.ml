          ...
          fun is_rec  ->
            fun mk_unit  ->
              ...
              | (mlident,nocluewhat) ->
                  ...
                  ((let uu___109_720 = g in
                    {
                      tcenv;
                      gamma;
                      tydefs = (uu___109_720.tydefs);
                      currentModule = (uu___109_720.currentModule)
                    }), mlident1)
let rec mltyFvars:
...
let tySchemeIsClosed: FStar_Extraction_ML_Syntax.mltyscheme -> Prims.bool =
  fun tys  ->
    let uu____757 = mltyFvars (snd tys) in subsetMlidents uu____757 (fst tys)
let extend_fv':
  env ->
    FStar_Syntax_Syntax.fv ->
      FStar_Extraction_ML_Syntax.mlpath ->
              ...
              then
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma) in
                    ((let uu___110_841 = g in
                      {
                        tcenv = (uu___110_841.tcenv);
                        gamma;
                        tydefs = (uu___110_841.tydefs);
                        currentModule = (uu___110_841.currentModule)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___111_918 = g in
        {
          tcenv = (uu___111_918.tcenv);
          gamma = (uu___111_918.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          currentModule = (uu___111_918.currentModule)
        }
let emptyMlPath:
...
let mkContext: FStar_TypeChecker_Env.env -> env =
  fun e  ->
    ...
    FStar_All.pipe_right uu____955 FStar_Pervasives.fst
let monad_op_name:
