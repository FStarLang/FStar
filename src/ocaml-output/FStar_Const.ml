...
type signedness = Unsigned | Signed [@@deriving show]

let uu___is_Unsigned : signedness -> Prims.bool =
  fun projectee -> match projectee with Unsigned -> true | uu____4 -> false
...
type width = Int8 | Int16 | Int32 | Int64 [@@deriving show]
  ...
  fun c1 c2 ->
    ...
    | Const_int (s1, o1), Const_int (s2, o2) ->
        (let uu____311 = FStar_Util.ensure_decimal s1 in
         ...
         uu____311 = uu____312)
        && o1 = o2
    | Const_bytearray (a, uu____320), Const_bytearray (b, uu____322) -> a = b
...
let rec pow2 : FStar_BigInt.bigint -> FStar_BigInt.bigint =
  fun x ->
    ...
    if uu____347 then FStar_BigInt.one
    else
      let uu____349 =
        let uu____350 = FStar_BigInt.pred_big_int x in
        pow2 uu____350
      in
...
let bounds
  ...
  fun signedness width ->
    ...
    let uu____362 =
      ...
      | Unsigned ->
          let uu____371 =
            let uu____372 = pow2 n1 in
            FStar_BigInt.pred_big_int uu____372
          in
          (FStar_BigInt.zero, uu____371)
      | Signed ->
          let upper =
            let uu____374 = FStar_BigInt.pred_big_int n1 in
            pow2 uu____374
          in
...
let within_bounds : Prims.string -> signedness -> width -> Prims.bool =
  fun repr signedness width ->
    let uu____392 = bounds signedness width in
    match uu____392 with lower, upper ->
      let value =
        let uu____400 = FStar_Util.ensure_decimal repr in
        FStar_BigInt.big_int_of_string uu____400
      in
