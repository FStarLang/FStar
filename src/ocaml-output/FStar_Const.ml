...
let eq_const: sconst -> sconst -> Prims.bool =
  fun c1  ->
    fun c2  ->
      ...
      | (Const_bytearray (a,uu____240),Const_bytearray (b,uu____242)) ->
          a = b
      | (Const_string (a,uu____249),Const_string (b,uu____251)) -> a = b
...
let rec pow2: Prims.int -> Prims.int =
  fun x  ->
    ...
    | uu____265 ->
        let uu____266 = pow2 (x - (Prims.parse_int "1")) in
        (Prims.parse_int "2") * uu____266
  ...
  fun signedness  ->
    fun width  ->
      ...
      let uu____276 =
        ...
        | Unsigned  ->
            let uu____281 =
              let uu____282 = pow2 n1 in uu____282 - (Prims.parse_int "1") in
            ((Prims.parse_int "0"), uu____281)
        | Signed  ->
            let upper = pow2 (n1 - (Prims.parse_int "1")) in
            ((- upper), (upper - (Prims.parse_int "1"))) in
      match uu____276 with | (lower,upper) -> (lower, upper)