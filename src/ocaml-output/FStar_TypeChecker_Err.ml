          ...
          (FStar_List.map
             (fun uu____27  ->
                ...
                | (msg,r) ->
                    ...
                    then
                      let _0_546 = FStar_TypeChecker_Env.get_range env in
                      (msg, _0_546)
                    else
                      (let r' =
                       ...
                       let uu____39 =
                         let _0_548 = FStar_Range.file_of_range r' in
                         let _0_547 =
                       ...
                       then
                         let _0_553 =
                           let _0_551 =
                             let _0_550 =
                               let _0_549 = FStar_Range.string_of_use_range r in
                               Prims.strcat _0_549 ")" in
                             Prims.strcat "(Also see: " _0_550 in
                           Prims.strcat msg _0_551 in
...
let unexpected_signature_for_monad:
  ...
  fun env  ->
    fun m  ->
      fun k  ->
        let _0_554 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
          "Unexpected signature for monad \"%s\". Expected a signature of the form (a:Type => WP a => Effect); got %s"
          m.FStar_Ident.str _0_554
let expected_a_term_of_type_t_got_a_function:
  ...
  fun env  ->
    fun msg  ->
      fun t  ->
        fun e  ->
          let _0_556 = FStar_TypeChecker_Normalize.term_to_string env t in
...
let expected_expression_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let _0_559 = FStar_TypeChecker_Normalize.term_to_string env t1 in
          let _0_558 = FStar_Syntax_Print.term_to_string e in
          let _0_557 = FStar_TypeChecker_Normalize.term_to_string env t2 in
          FStar_Util.format3
...
let expected_function_with_parameter_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun t2  ->
        let _0_561 = FStar_TypeChecker_Normalize.term_to_string env t1 in
        let _0_560 = FStar_TypeChecker_Normalize.term_to_string env t2 in
        FStar_Util.format3
...
let expected_pattern_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let _0_564 = FStar_TypeChecker_Normalize.term_to_string env t1 in
          let _0_563 = FStar_Syntax_Print.term_to_string e in
          let _0_562 = FStar_TypeChecker_Normalize.term_to_string env t2 in
          FStar_Util.format3
            "Expected pattern of type \"%s\"; got pattern \"%s\" of type \"%s\""
            _0_564 _0_563 _0_562
        ...
        fun t2  ->
          ...
          | (s1,s2,extra) ->
              (match eopt with
               ...
               | Some e ->
                   let _0_565 = FStar_Syntax_Print.term_to_string e in
                   FStar_Util.format4
  ...
  fun env  ->
    fun k1  ->
      fun k2  ->
        ...
        let _0_566 = FStar_TypeChecker_Normalize.term_to_string env k2 in
        FStar_Util.format2 "Kinds \"%s\" and \"%s\" are incompatible" _0_567
          _0_566
let constructor_builds_the_wrong_type:
  ...
  fun env  ->
    fun d  ->
      fun t  ->
        fun t'  ->
          let _0_570 = FStar_Syntax_Print.term_to_string d in
          ...
          FStar_Util.format3
            ...
            _0_570 _0_569 _0_568
let constructor_fails_the_positivity_check env d l =
  let _0_572 = FStar_Syntax_Print.term_to_string d in
  let _0_571 = FStar_Syntax_Print.lid_to_string l in
  FStar_Util.format2
    "Constructor \"%s\" fails the strict positivity check; the constructed type \"%s\" occurs to the left of a pure function type"
    _0_572 _0_571
let inline_type_annotation_and_val_decl: FStar_Ident.lid -> Prims.string =
  fun l  ->
    let _0_573 = FStar_Syntax_Print.lid_to_string l in
    FStar_Util.format1
  ...
  fun env  ->
    fun t  ->
      fun x  ->
        ...
        let _0_574 = FStar_Syntax_Print.bv_to_string x in
        FStar_Util.format2
...
let expected_typ_of_kind:
  ...
  fun env  ->
    fun k1  ->
      fun t  ->
        fun k2  ->
          let _0_578 = FStar_TypeChecker_Normalize.term_to_string env k1 in
          let _0_577 = FStar_TypeChecker_Normalize.term_to_string env t in
          let _0_576 = FStar_TypeChecker_Normalize.term_to_string env k2 in
          FStar_Util.format3
            "Expected type of kind \"%s\"; got \"%s\" of kind \"%s\"" _0_578
            _0_577 _0_576
let expected_tcon_kind:
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        let _0_580 = FStar_TypeChecker_Normalize.term_to_string env t in
        let _0_579 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
...
let expected_dcon_kind:
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        let _0_582 = FStar_TypeChecker_Normalize.term_to_string env t in
        let _0_581 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
...
let expected_function_typ:
  ...
  fun env  ->
    fun t  ->
      ...
      FStar_Util.format1
        "Expected a function; got an expression of type \"%s\"" _0_583
let expected_poly_typ:
  ...
  fun env  ->
    fun f  ->
      fun t  ->
        fun targ  ->
          let _0_586 = FStar_Syntax_Print.term_to_string f in
          let _0_585 = FStar_TypeChecker_Normalize.term_to_string env t in
          let _0_584 = FStar_TypeChecker_Normalize.term_to_string env targ in
          FStar_Util.format3
  ...
  fun v1  ->
    fun v2  ->
      let vars v =
        ...
        FStar_All.pipe_right _0_587 (FStar_String.concat ", ") in
      let _0_589 = vars v1 in
      let _0_588 = vars v2 in
      FStar_Util.format2
...
let name_and_result c =
  ...
  | FStar_Syntax_Syntax.Comp ct ->
      ...
      (_0_590, (ct.FStar_Syntax_Syntax.result_typ))
let computed_computation_type_does_not_match_annotation env e c c' =
  ...
  | (f1,r1) ->
      ...
      (match uu____363 with
       | (f2,r2) ->
           let _0_592 = FStar_TypeChecker_Normalize.term_to_string env r1 in
           let _0_591 = FStar_TypeChecker_Normalize.term_to_string env r2 in
           FStar_Util.format4
...
let unexpected_non_trivial_precondition_on_term:
  ...
  fun env  ->
    fun f  ->
      let _0_593 = FStar_TypeChecker_Normalize.term_to_string env f in
      FStar_Util.format1
        "Term has an unexpected non-trivial pre-condition: %s" _0_593
let expected_pure_expression e c =
  let _0_596 = FStar_Syntax_Print.term_to_string e in
  let _0_595 =
    let _0_594 = name_and_result c in FStar_All.pipe_left Prims.fst _0_594 in
  FStar_Util.format2
...
let expected_ghost_expression e c =
  ...
  let _0_598 =
    let _0_597 = name_and_result c in FStar_All.pipe_left Prims.fst _0_597 in
  FStar_Util.format2
