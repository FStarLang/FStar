...
let unexpected_signature_for_monad:
  ...
  fun env  ->
    fun m  ->
      fun k  ->
        let _0_540 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
          "Unexpected signature for monad \"%s\". Expected a signature of the form (a:Type => WP a => Effect); got %s"
          m.FStar_Ident.str _0_540
let expected_a_term_of_type_t_got_a_function:
  ...
  fun env  ->
    fun msg  ->
      fun t  ->
        fun e  ->
          let _0_542 = FStar_TypeChecker_Normalize.term_to_string env t in
  ...
  fun env  ->
    fun t1  ->
      fun t2  ->
        ...
        let _0_546 = FStar_TypeChecker_Normalize.term_to_string env t2 in
        FStar_Util.format3
...
let expected_pattern_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let _0_550 = FStar_TypeChecker_Normalize.term_to_string env t1 in
...
let incompatible_kinds:
  ...
  fun env  ->
    fun k1  ->
      fun k2  ->
        let _0_553 = FStar_TypeChecker_Normalize.term_to_string env k1 in
        let _0_552 = FStar_TypeChecker_Normalize.term_to_string env k2 in
        FStar_Util.format2 "Kinds \"%s\" and \"%s\" are incompatible" _0_553
          _0_552
let constructor_builds_the_wrong_type:
  ...
  fun env  ->
    fun d  ->
      fun t  ->
        fun t'  ->
          let _0_556 = FStar_Syntax_Print.term_to_string d in
          let _0_555 = FStar_TypeChecker_Normalize.term_to_string env t in
          let _0_554 = FStar_TypeChecker_Normalize.term_to_string env t' in
          FStar_Util.format3
            "Constructor \"%s\" builds a value of type \"%s\"; expected \"%s\""
            _0_556 _0_555 _0_554
let constructor_fails_the_positivity_check env d l =
  let _0_558 = FStar_Syntax_Print.term_to_string d in
  let _0_557 = FStar_Syntax_Print.lid_to_string l in
  FStar_Util.format2
    "Constructor \"%s\" fails the strict positivity check; the constructed type \"%s\" occurs to the left of a pure function type"
    _0_558 _0_557
let inline_type_annotation_and_val_decl: FStar_Ident.lid -> Prims.string =
  fun l  ->
    let _0_559 = FStar_Syntax_Print.lid_to_string l in
    FStar_Util.format1
  ...
  fun env  ->
    fun t  ->
      fun x  ->
        ...
        let _0_560 = FStar_Syntax_Print.bv_to_string x in
        FStar_Util.format2
...
let expected_typ_of_kind:
  ...
  fun env  ->
    fun k1  ->
      fun t  ->
        fun k2  ->
          let _0_564 = FStar_TypeChecker_Normalize.term_to_string env k1 in
          let _0_563 = FStar_TypeChecker_Normalize.term_to_string env t in
          let _0_562 = FStar_TypeChecker_Normalize.term_to_string env k2 in
          FStar_Util.format3
            "Expected type of kind \"%s\"; got \"%s\" of kind \"%s\"" _0_564
            _0_563 _0_562
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        ...
        let _0_565 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        ...
        let _0_567 = FStar_TypeChecker_Normalize.term_to_string env k in
        FStar_Util.format2
...
let expected_function_typ:
  ...
  fun env  ->
    fun t  ->
      ...
      FStar_Util.format1
        "Expected a function; got an expression of type \"%s\"" _0_569
  ...
  fun v1  ->
    fun v2  ->
      let vars v =
        ...
        FStar_All.pipe_right _0_573 (FStar_String.concat ", ") in
      let _0_575 = vars v1 in
      let _0_574 = vars v2 in
      FStar_Util.format2
        "Every alternative of an 'or' pattern must bind the same variables; here one branch binds (\"%s\") and another (\"%s\")"
        _0_575 _0_574
let name_and_result c =
  match c.FStar_Syntax_Syntax.n with
  | FStar_Syntax_Syntax.Total (t,uu____296) -> ("Tot", t)
  | FStar_Syntax_Syntax.GTotal (t,uu____306) -> ("GTot", t)
  | FStar_Syntax_Syntax.Comp ct ->
      let _0_576 =
        FStar_Syntax_Print.lid_to_string ct.FStar_Syntax_Syntax.effect_name in
      (_0_576, (ct.FStar_Syntax_Syntax.result_typ))
let computed_computation_type_does_not_match_annotation env e c c' =
  let uu____352 = name_and_result c in
  match uu____352 with
  | (f1,r1) ->
      let uu____363 = name_and_result c' in
      (match uu____363 with
       | (f2,r2) ->
           let _0_578 = FStar_TypeChecker_Normalize.term_to_string env r1 in
           let _0_577 = FStar_TypeChecker_Normalize.term_to_string env r2 in
           FStar_Util.format4
...
let unexpected_non_trivial_precondition_on_term:
  ...
  fun env  ->
    fun f  ->
      let _0_579 = FStar_TypeChecker_Normalize.term_to_string env f in
      FStar_Util.format1
        "Term has an unexpected non-trivial pre-condition: %s" _0_579
let expected_pure_expression e c =
  let _0_582 = FStar_Syntax_Print.term_to_string e in
  let _0_581 =
    let _0_580 = name_and_result c in FStar_All.pipe_left Prims.fst _0_580 in
  FStar_Util.format2
...
let expected_ghost_expression e c =
  ...
  let _0_584 =
    let _0_583 = name_and_result c in FStar_All.pipe_left Prims.fst _0_583 in
  FStar_Util.format2
...
let expected_effect_1_got_effect_2:
  ...
  fun c1  ->
    fun c2  ->
      let _0_587 = FStar_Syntax_Print.lid_to_string c1 in
      let _0_586 = FStar_Syntax_Print.lid_to_string c2 in
      FStar_Util.format2
        "Expected a computation with effect %s; but it has effect %s" _0_587
        _0_586
let failed_to_prove_specification_of:
  FStar_Syntax_Syntax.lbname -> Prims.string Prims.list -> Prims.string =
  fun l  ->
    fun lbls  ->
      let _0_589 = FStar_Syntax_Print.lbname_to_string l in
      let _0_588 = FStar_All.pipe_right lbls (FStar_String.concat ", ") in
      FStar_Util.format2
...
let failed_to_prove_specification: Prims.string Prims.list -> Prims.string =
  fun lbls  ->
    ...
    | uu____444 ->
        let _0_590 = FStar_All.pipe_right lbls (FStar_String.concat "\n\t") in
...
let cardinality_constraint_violated l a =
  let _0_592 = FStar_Syntax_Print.lid_to_string l in
  let _0_591 = FStar_Syntax_Print.bv_to_string a.FStar_Syntax_Syntax.v in
  FStar_Util.format2
