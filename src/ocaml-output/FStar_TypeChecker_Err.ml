             ...
             (fun uu____27  ->
                ...
                | (msg,r) ->
                    ...
                    then
                      ...
                      (let r' =
                         let uu___188_39 = r in
                         {
                           FStar_Range.def_range = (r.FStar_Range.use_range);
                           FStar_Range.use_range =
                             (uu___188_39.FStar_Range.use_range)
                         } in
                       let uu____40 =
...
let expected_a_term_of_type_t_got_a_function:
  ...
  fun env  ->
    fun msg  ->
      fun t  ->
        fun e  ->
          let uu____112 = FStar_TypeChecker_Normalize.term_to_string env t in
...
let expected_expression_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let uu____126 = FStar_TypeChecker_Normalize.term_to_string env t1 in
          let uu____127 = FStar_Syntax_Print.term_to_string e in
          let uu____128 = FStar_TypeChecker_Normalize.term_to_string env t2 in
          FStar_Util.format3
...
let expected_pattern_of_type:
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let uu____154 = FStar_TypeChecker_Normalize.term_to_string env t1 in
          let uu____155 = FStar_Syntax_Print.term_to_string e in
          let uu____156 = FStar_TypeChecker_Normalize.term_to_string env t2 in
          FStar_Util.format3
    ...
    fun d  ->
      fun t  ->
        fun t'  ->
          ...
          let uu____204 = FStar_TypeChecker_Normalize.term_to_string env t in
          let uu____205 = FStar_TypeChecker_Normalize.term_to_string env t' in
          FStar_Util.format3
...
let expected_typ_of_kind:
  ...
  fun env  ->
    fun k1  ->
      fun t  ->
        fun k2  ->
          let uu____252 = FStar_TypeChecker_Normalize.term_to_string env k1 in
          let uu____253 = FStar_TypeChecker_Normalize.term_to_string env t in
          let uu____254 = FStar_TypeChecker_Normalize.term_to_string env k2 in
          FStar_Util.format3
    ...
    fun f  ->
      fun t  ->
        fun targ  ->
          ...
          let uu____297 = FStar_TypeChecker_Normalize.term_to_string env t in
          let uu____298 = FStar_TypeChecker_Normalize.term_to_string env targ in
          FStar_Util.format3
...
let name_and_result c =
  ...
  | FStar_Syntax_Syntax.Total (t,uu____342) -> ("Tot", t)
  | FStar_Syntax_Syntax.GTotal (t,uu____352) -> ("GTot", t)
  | FStar_Syntax_Syntax.Comp ct ->
      let uu____362 =
        FStar_Syntax_Print.lid_to_string ct.FStar_Syntax_Syntax.effect_name in
      (uu____362, (ct.FStar_Syntax_Syntax.result_typ))
let computed_computation_type_does_not_match_annotation env e c c' =
  let uu____399 = name_and_result c in
  match uu____399 with
  | (f1,r1) ->
      let uu____410 = name_and_result c' in
      (match uu____410 with
       | (f2,r2) ->
           let uu____421 = FStar_TypeChecker_Normalize.term_to_string env r1 in
           let uu____422 = FStar_TypeChecker_Normalize.term_to_string env r2 in
           FStar_Util.format4
             "Computed type \"%s\" and effect \"%s\" is not compatible with the annotated type \"%s\" effect \"%s\""
             uu____421 f1 uu____422 f2)
let unexpected_non_trivial_precondition_on_term:
  FStar_TypeChecker_Env.env -> FStar_Syntax_Syntax.term -> Prims.string =
  fun env  ->
    fun f  ->
      let uu____429 = FStar_TypeChecker_Normalize.term_to_string env f in
      FStar_Util.format1
        "Term has an unexpected non-trivial pre-condition: %s" uu____429
let expected_pure_expression e c =
  let uu____446 = FStar_Syntax_Print.term_to_string e in
  let uu____447 =
    let uu____448 = name_and_result c in
    FStar_All.pipe_left Prims.fst uu____448 in
  FStar_Util.format2
    "Expected a pure expression; got an expression \"%s\" with effect \"%s\""
    uu____446 uu____447
let expected_ghost_expression e c =
  let uu____475 = FStar_Syntax_Print.term_to_string e in
  let uu____476 =
    let uu____477 = name_and_result c in
    FStar_All.pipe_left Prims.fst uu____477 in
  FStar_Util.format2
...
let expected_effect_1_got_effect_2:
  ...
  fun c1  ->
    fun c2  ->
      let uu____494 = FStar_Syntax_Print.lid_to_string c1 in
      let uu____495 = FStar_Syntax_Print.lid_to_string c2 in
      FStar_Util.format2
...
let failed_to_prove_specification_of:
  ...
  fun l  ->
    fun lbls  ->
      let uu____504 = FStar_Syntax_Print.lbname_to_string l in
      let uu____505 = FStar_All.pipe_right lbls (FStar_String.concat ", ") in
      FStar_Util.format2
...
let cardinality_constraint_violated l a =
  let uu____532 = FStar_Syntax_Print.lid_to_string l in
  let uu____533 = FStar_Syntax_Print.bv_to_string a.FStar_Syntax_Syntax.v in
  FStar_Util.format2
