          ...
          (FStar_List.map
             (fun uu____27  ->
                ...
                | (msg,r) ->
                    ...
                    then
                      let _0_542 = FStar_TypeChecker_Env.get_range env  in
                      (msg, _0_542)
                    else
                      (let r' =
                         let uu___186_38 = r  in
                         {
                           FStar_Range.def_range = (r.FStar_Range.use_range);
                           FStar_Range.use_range =
                             (uu___186_38.FStar_Range.use_range)
                         }  in
                       let uu____39 =
                         let _0_544 = FStar_Range.file_of_range r'  in
                         let _0_543 =
                       ...
                       then
                         let _0_549 =
                           let _0_547 =
                             let _0_546 =
                               let _0_545 = FStar_Range.string_of_use_range r
                                  in
                               Prims.strcat _0_545 ")"  in
                             Prims.strcat "(Also see: " _0_546  in
                           Prims.strcat msg _0_547  in
...
let unexpected_signature_for_monad :
  ...
  fun env  ->
    fun m  ->
      fun k  ->
        let _0_550 = FStar_TypeChecker_Normalize.term_to_string env k  in
        FStar_Util.format2
          "Unexpected signature for monad \"%s\". Expected a signature of the form (a:Type => WP a => Effect); got %s"
          m.FStar_Ident.str _0_550
...
let expected_a_term_of_type_t_got_a_function :
  ...
  fun env  ->
    fun msg  ->
      fun t  ->
        fun e  ->
          let _0_552 = FStar_TypeChecker_Normalize.term_to_string env t  in
...
let expected_expression_of_type :
  ...
  fun env  ->
    fun t1  ->
      fun e  ->
        fun t2  ->
          let _0_555 = FStar_TypeChecker_Normalize.term_to_string env t1  in
  ...
  fun env  ->
    fun t1  ->
      fun t2  ->
        ...
        let _0_556 = FStar_TypeChecker_Normalize.term_to_string env t2  in
        FStar_Util.format3
        ...
        fun t2  ->
          ...
          | (s1,s2,extra) ->
              (match eopt with
               ...
               | Some e ->
                   let _0_561 = FStar_Syntax_Print.term_to_string e  in
                   FStar_Util.format4
  ...
  fun env  ->
    fun k1  ->
      fun k2  ->
        ...
        FStar_Util.format2 "Kinds \"%s\" and \"%s\" are incompatible" _0_563
          _0_562
...
let constructor_builds_the_wrong_type :
  ...
  fun env  ->
    fun d  ->
      fun t  ->
        fun t'  ->
          let _0_566 = FStar_Syntax_Print.term_to_string d  in
          ...
          FStar_Util.format3
            ...
            _0_566 _0_565 _0_564
  
let constructor_fails_the_positivity_check env d l =
  let _0_568 = FStar_Syntax_Print.term_to_string d  in
  let _0_567 = FStar_Syntax_Print.lid_to_string l  in
  FStar_Util.format2
...
let inline_type_annotation_and_val_decl : FStar_Ident.lid -> Prims.string =
  fun l  ->
    let _0_569 = FStar_Syntax_Print.lid_to_string l  in
    FStar_Util.format1
  ...
  fun env  ->
    fun t  ->
      fun x  ->
        ...
        let _0_570 = FStar_Syntax_Print.bv_to_string x  in
        FStar_Util.format2
...
let expected_typ_of_kind :
  ...
  fun env  ->
    fun k1  ->
      fun t  ->
        fun k2  ->
          let _0_574 = FStar_TypeChecker_Normalize.term_to_string env k1  in
          let _0_573 = FStar_TypeChecker_Normalize.term_to_string env t  in
          let _0_572 = FStar_TypeChecker_Normalize.term_to_string env k2  in
          FStar_Util.format3
            "Expected type of kind \"%s\"; got \"%s\" of kind \"%s\"" _0_574
            _0_573 _0_572
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        ...
        let _0_575 = FStar_TypeChecker_Normalize.term_to_string env k  in
        FStar_Util.format2
  ...
  fun env  ->
    fun t  ->
      fun k  ->
        ...
        let _0_577 = FStar_TypeChecker_Normalize.term_to_string env k  in
        FStar_Util.format2
...
let expected_function_typ :
  ...
  fun env  ->
    fun t  ->
      let _0_579 = FStar_TypeChecker_Normalize.term_to_string env t  in
      FStar_Util.format1
        "Expected a function; got an expression of type \"%s\"" _0_579
...
let expected_poly_typ :
  ...
  fun env  ->
    fun f  ->
      fun t  ->
        fun targ  ->
          let _0_582 = FStar_Syntax_Print.term_to_string f  in
          let _0_581 = FStar_TypeChecker_Normalize.term_to_string env t  in
          let _0_580 = FStar_TypeChecker_Normalize.term_to_string env targ
             in
          FStar_Util.format3
      ...
      let vars v =
        let _0_583 =
          FStar_All.pipe_right v
           ...
           in
        FStar_All.pipe_right _0_583 (FStar_String.concat ", ")  in
      let _0_585 = vars v1  in
      let _0_584 = vars v2  in
      FStar_Util.format2
  ...
  | FStar_Syntax_Syntax.Comp ct ->
      let _0_586 =
        FStar_Syntax_Print.lid_to_string ct.FStar_Syntax_Syntax.effect_name
         in
      (_0_586, (ct.FStar_Syntax_Syntax.result_typ))
...
let computed_computation_type_does_not_match_annotation env e c c' =
  ...
  | (f1,r1) ->
      ...
      (match uu____363 with
       | (f2,r2) ->
           let _0_588 = FStar_TypeChecker_Normalize.term_to_string env r1  in
           let _0_587 = FStar_TypeChecker_Normalize.term_to_string env r2  in
           FStar_Util.format4
...
let unexpected_non_trivial_precondition_on_term :
  ...
  fun env  ->
    fun f  ->
      let _0_589 = FStar_TypeChecker_Normalize.term_to_string env f  in
      FStar_Util.format1
        "Term has an unexpected non-trivial pre-condition: %s" _0_589
  
let expected_pure_expression e c =
  let _0_592 = FStar_Syntax_Print.term_to_string e  in
  let _0_591 =
    let _0_590 = name_and_result c  in FStar_All.pipe_left Prims.fst _0_590
     in
  FStar_Util.format2
...
let expected_ghost_expression e c =
  let _0_595 = FStar_Syntax_Print.term_to_string e  in
  let _0_594 =
    let _0_593 = name_and_result c  in FStar_All.pipe_left Prims.fst _0_593
     in
  FStar_Util.format2
...
let expected_effect_1_got_effect_2 :
  ...
  fun c1  ->
    fun c2  ->
      let _0_597 = FStar_Syntax_Print.lid_to_string c1  in
      let _0_596 = FStar_Syntax_Print.lid_to_string c2  in
      FStar_Util.format2
        "Expected a computation with effect %s; but it has effect %s" _0_597
        _0_596
...
let failed_to_prove_specification_of :
  ...
  fun l  ->
    fun lbls  ->
      let _0_599 = FStar_Syntax_Print.lbname_to_string l  in
      let _0_598 = FStar_All.pipe_right lbls (FStar_String.concat ", ")  in
      FStar_Util.format2
...
let failed_to_prove_specification : Prims.string Prims.list -> Prims.string =
  fun lbls  ->
    ...
    | uu____444 ->
        let _0_600 = FStar_All.pipe_right lbls (FStar_String.concat "\n\t")
           in
...
let cardinality_constraint_violated l a =
  let _0_602 = FStar_Syntax_Print.lid_to_string l  in
  let _0_601 = FStar_Syntax_Print.bv_to_string a.FStar_Syntax_Syntax.v  in
  FStar_Util.format2
