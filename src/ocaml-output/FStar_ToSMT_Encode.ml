...
in (FStar_Util.find_map _118_165 (fun _52_109 -> (match (_52_109) with
| (names, _52_108) -> begin
...
in (let _52_118 = (FStar_Util.smap_add top_scope y true)
in y)))))
...
in (FStar_Util.find_map _118_187 (fun _52_136 -> (match (_52_136) with
...
in (let _52_143 = (FStar_Util.smap_add top_scope s f)
in f))))
end))
...
in (let _52_172 = (FStar_Util.smap_fold hd2 (fun key value v -> (FStar_Util.smap_add next2 key value)) ())
...
let print_env = (fun e -> (let _118_301 = (FStar_All.pipe_right e.bindings (FStar_List.map (fun _52_2 -> (match (_52_2) with
...
let lookup_free_var = (fun env a -> (let _52_335 = (lookup_lid env a.FStar_Absyn_Syntax.v)
...
let lookup_free_var_name = (fun env a -> (let _52_359 = (lookup_lid env a.FStar_Absyn_Syntax.v)
in (match (_52_359) with
| (x, _52_356, _52_358) -> begin
x
end)))

let lookup_free_var_sym = (fun env a -> (let _52_365 = (lookup_lid env a.FStar_Absyn_Syntax.v)
...
let tok_of_name = (fun env nm -> (FStar_Util.find_map env.bindings (fun _52_7 -> (match (_52_7) with
...
| FStar_Absyn_Syntax.Const_string (bytes, _52_627) -> begin
...
in (match (t.FStar_Absyn_Syntax.n) with
| FStar_Absyn_Syntax.Typ_fun (_52_638) -> begin
t
end
| FStar_Absyn_Syntax.Typ_refine (_52_641) -> begin
...
| FStar_Absyn_Syntax.Kind_abbrev (_52_649, k0) -> begin
(let _52_653 = (match ((FStar_Tc_Env.debug env.tcenv (FStar_Options.Other ("Encoding")))) with
| true -> begin
...
| FStar_Absyn_Syntax.Kind_uvar (uv, _52_657) -> begin
...
in (FStar_All.pipe_right _118_637 (FStar_List.filter (fun _52_703 -> (match (_52_703) with
| (x, _52_702) -> begin
(x <> (Prims.fst tsym))
end)))))
...
in (let _52_721 = (FStar_Util.smap_add env.cache tkey.FStar_ToSMT_Term.hash (ksym, cvar_sorts, k_decls))
in (k, k_decls))))))))))
end)))))
end)))
end))))
...
and encode_binders = (fun fuel_opt bs env -> (let _52_734 = (match ((FStar_Tc_Env.debug env.tcenv FStar_Options.Low)) with
| true -> begin
...
in (let _52_784 = (FStar_All.pipe_right bs (FStar_List.fold_left (fun _52_741 b -> (match (_52_741) with
| (vars, guards, env, decls, names) -> begin
(let _52_778 = (match ((Prims.fst b)) with
| FStar_Util.Inl ({FStar_Absyn_Syntax.v = a; FStar_Absyn_Syntax.sort = k; FStar_Absyn_Syntax.p = _52_744}) -> begin
(let a = (unmangle a)
in (let _52_753 = (gen_typ_var env a)
in (match (_52_753) with
| (aasym, aa, env') -> begin
(let _52_754 = (match ((FStar_Tc_Env.debug env.tcenv (FStar_Options.Other ("Encoding")))) with
| true -> begin
...
| FStar_Util.Inr ({FStar_Absyn_Syntax.v = x; FStar_Absyn_Syntax.sort = t; FStar_Absyn_Syntax.p = _52_760}) -> begin
(let x = (unmangle x)
...
in (let _52_827 = (FStar_Tc_Util.pure_or_ghost_pre_and_post env.tcenv res)
...
in (FStar_All.pipe_right _118_689 (FStar_List.filter (fun _52_844 -> (match (_52_844) with
| (x, _52_843) -> begin
(x <> (Prims.fst fsym))
end)))))
...
in (let _52_866 = (FStar_Util.smap_add env.cache tkey.FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls))))))))))))))
end))))
end))
end))
...
| FStar_Absyn_Syntax.Typ_refine (_52_877) -> begin
(let _52_896 = (match ((FStar_Tc_Normalize.normalize_refinement env.tcenv t0)) with
| {FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Typ_refine (x, f); FStar_Absyn_Syntax.tk = _52_886; FStar_Absyn_Syntax.pos = _52_884; FStar_Absyn_Syntax.fvs = _52_882; FStar_Absyn_Syntax.uvs = _52_880} -> begin
(x, f)
end
| _52_893 -> begin
(FStar_All.failwith "impossible")
...
| (x, f) -> begin
(let _52_899 = (encode_typ_term x.FStar_Absyn_Syntax.sort env)
in (match (_52_899) with
| (base_t, decls) -> begin
(let _52_903 = (gen_term_var env x.FStar_Absyn_Syntax.v)
in (match (_52_903) with
| (x, xtm, env') -> begin
(let _52_906 = (encode_formula f env')
in (match (_52_906) with
| (refinement, decls') -> begin
(let _52_909 = (fresh_fvar "f" FStar_ToSMT_Term.Fuel_sort)
...
in (FStar_All.pipe_right _118_721 (FStar_List.filter (fun _52_914 -> (match (_52_914) with
| (y, _52_913) -> begin
((y <> x) && (y <> fsym))
end)))))
...
in (let _52_936 = (FStar_Util.smap_add env.cache tkey.FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))
end))))))
end))
end))
...
in (let _52_957 = (FStar_Absyn_Util.kind_formals kk)
...
in (let t = (let _118_741 = (let _118_740 = (FStar_List.map (fun _52_10 -> (match (_52_10) with
...
in (let kinding = (let _52_1027 = (let _118_759 = (FStar_Tc_Recheck.recompute_kind t0)
...
in (let _52_1030 = (FStar_Util.smap_add env.cache tkey.FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls))))))))))
end)
end))))
end))
...
| FStar_Absyn_Syntax.Typ_ascribed (t, _52_1034) -> begin
(encode_typ_term t env)
end
| FStar_Absyn_Syntax.Typ_meta (_52_1038) -> begin
...
| FStar_Absyn_Syntax.Exp_delayed (_52_1049) -> begin
...
| FStar_Absyn_Syntax.Exp_fvar (v, _52_1056) -> begin
...
| FStar_Absyn_Syntax.Exp_ascribed (e, t, _52_1064) -> begin
(let _52_1067 = (FStar_ST.op_Colon_Equals e.FStar_Absyn_Syntax.tk (Some (t)))
in (encode_exp e env))
end
| FStar_Absyn_Syntax.Exp_meta (FStar_Absyn_Syntax.Meta_desugared (e, _52_1071)) -> begin
(encode_exp e env)
end
| FStar_Absyn_Syntax.Exp_uvar (uv, _52_1077) -> begin
...
| None -> begin
(let _52_1090 = (FStar_Tc_Errors.warn e.FStar_Absyn_Syntax.pos "Losing precision when encoding a function literal")
in (fallback ()))
end
| Some (tfun) -> begin
...
| true -> begin
(let _52_1102 = (FStar_Util.first_N nformals bs)
...
in (let _52_1144 = (FStar_Util.smap_add env.cache tkey.FStar_ToSMT_Term.hash (fsym, cvar_sorts, f_decls))
in (f, f_decls)))))
end)))))))
end)
end))))
...
| FStar_Absyn_Syntax.Exp_app ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Exp_fvar (l, _52_1158); FStar_Absyn_Syntax.tk = _52_1155; FStar_Absyn_Syntax.pos = _52_1153; FStar_Absyn_Syntax.fvs = _52_1151; FStar_Absyn_Syntax.uvs = _52_1149}, (FStar_Util.Inl (_52_1173), _52_1176)::(FStar_Util.Inr (v1), _52_1170)::(FStar_Util.Inr (v2), _52_1165)::[]) when (FStar_Absyn_Syntax.lid_equals l.FStar_Absyn_Syntax.v FStar_Absyn_Const.lexcons_lid) -> begin
...
| FStar_Absyn_Syntax.Exp_app ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Exp_abs (_52_1196); FStar_Absyn_Syntax.tk = _52_1194; FStar_Absyn_Syntax.pos = _52_1192; FStar_Absyn_Syntax.fvs = _52_1190; FStar_Absyn_Syntax.uvs = _52_1188}, _52_1200) -> begin
...
(let _52_1223 = (FStar_Util.first_N (FStar_List.length args_e) formals)
...
(let tm = (let _118_812 = (let _118_811 = (let _118_810 = (FStar_List.map (fun _52_11 -> (match (_52_11) with
| (FStar_Util.Inl (t)) | (FStar_Util.Inr (t)) -> begin
t
end)) args)
...
in (let _52_1242 = (match ((FStar_All.pipe_left (FStar_Tc_Env.debug env.tcenv) (FStar_Options.Other ("186")))) with
| true -> begin
...
in (let _52_1245 = (match ((FStar_All.pipe_left (FStar_Tc_Env.debug env.tcenv) (FStar_Options.Other ("Encoding")))) with
| true -> begin
...
| FStar_Absyn_Syntax.Exp_fvar (fv, _52_1254) when ((FStar_List.length formals) = (FStar_List.length args)) -> begin
(encode_full_app fv)
end
| _52_1258 -> begin
...
| FStar_Absyn_Syntax.Exp_let ((false, {FStar_Absyn_Syntax.lbname = FStar_Util.Inr (_52_1267); FStar_Absyn_Syntax.lbtyp = _52_1265; FStar_Absyn_Syntax.lbeff = _52_1263; FStar_Absyn_Syntax.lbdef = _52_1261}::[]), _52_1273) -> begin
(FStar_All.failwith "Impossible: already handled by encoding of Sig_let")
end
| FStar_Absyn_Syntax.Exp_let ((false, {FStar_Absyn_Syntax.lbname = FStar_Util.Inl (x); FStar_Absyn_Syntax.lbtyp = t1; FStar_Absyn_Syntax.lbeff = _52_1279; FStar_Absyn_Syntax.lbdef = e1}::[]), e2) -> begin
...
| FStar_Absyn_Syntax.Exp_let (_52_1297) -> begin
(let _52_1299 = (FStar_Tc_Errors.warn e.FStar_Absyn_Syntax.pos "Non-top-level recursive functions are not yet fully encoded to the SMT solver; you may not be able to prove some facts")
...
| (scr, decls) -> begin
(let _52_1349 = (FStar_List.fold_right (fun _52_1313 _52_1316 -> (match ((_52_1313, _52_1316)) with
| ((p, w, br), (else_case, decls)) -> begin
(let patterns = (encode_pat env p)
in (FStar_List.fold_right (fun _52_1320 _52_1323 -> (match ((_52_1320, _52_1323)) with
| ((env0, pattern), (else_case, decls)) -> begin
(let guard = (pattern.guard scr)
in (let projections = (pattern.projections scr)
in (let env = (FStar_All.pipe_right projections (FStar_List.fold_left (fun env _52_1329 -> (match (_52_1329) with
| (x, t) -> begin
(match (x) with
| FStar_Util.Inl (a) -> begin
...
| FStar_Absyn_Syntax.Exp_meta (_52_1351) -> begin
...
and encode_one_pat = (fun env pat -> (let _52_1361 = (match ((FStar_Tc_Env.debug env.tcenv FStar_Options.Low)) with
| true -> begin
...
in (let _52_1365 = (FStar_Tc_Util.decorated_pattern_as_either pat)
in (match (_52_1365) with
| (vars, pat_exp_or_typ) -> begin
(let _52_1386 = (FStar_All.pipe_right vars (FStar_List.fold_left (fun _52_1368 v -> (match (_52_1368) with
| (env, vars) -> begin
(match (v) with
| FStar_Util.Inl (a) -> begin
(let _52_1376 = (gen_typ_var env a.FStar_Absyn_Syntax.v)
...
| FStar_Util.Inr (x) -> begin
(let _52_1383 = (gen_term_var env x.FStar_Absyn_Syntax.v)
...
| FStar_Absyn_Syntax.Pat_disj (_52_1391) -> begin
(FStar_All.failwith "Impossible")
end
...
| FStar_Absyn_Syntax.Pat_cons (f, _52_1415, args) -> begin
(let is_f = (mk_data_tester env f.FStar_Absyn_Syntax.v scrutinee)
in (let sub_term_guards = (FStar_All.pipe_right args (FStar_List.mapi (fun i _52_1424 -> (match (_52_1424) with
| (arg, _52_1423) -> begin
...
| FStar_Absyn_Syntax.Pat_disj (_52_1431) -> begin
(FStar_All.failwith "Impossible")
end
...
| FStar_Absyn_Syntax.Pat_constant (_52_1448) -> begin
[]
end
| FStar_Absyn_Syntax.Pat_cons (f, _52_1452, args) -> begin
(let _118_865 = (FStar_All.pipe_right args (FStar_List.mapi (fun i _52_1460 -> (match (_52_1460) with
| (arg, _52_1459) -> begin
(let proj = (primitive_projector_by_pos env.tcenv f.FStar_Absyn_Syntax.v i)
...
and encode_args = (fun l env -> (let _52_1493 = (FStar_All.pipe_right l (FStar_List.fold_left (fun _52_1473 x -> (match (_52_1473) with
| (tms, decls) -> begin
(match (x) with
| (FStar_Util.Inl (t), _52_1478) -> begin
...
| (FStar_Util.Inr (e), _52_1486) -> begin
...
| FStar_Absyn_Syntax.Exp_app (_52_1509, (FStar_Util.Inl (_52_1516), _52_1519)::(FStar_Util.Inr (e), _52_1513)::[]) -> begin
(FStar_Absyn_Syntax.varg e)
end
| FStar_Absyn_Syntax.Exp_app (_52_1525, (FStar_Util.Inl (t), _52_1529)::[]) -> begin
...
| FStar_Absyn_Syntax.Exp_app (_52_1539, _52_1551::(FStar_Util.Inr (hd), _52_1548)::(FStar_Util.Inr (tl), _52_1543)::[]) -> begin
...
in (let _52_1595 = (match ((let _118_885 = (FStar_Absyn_Util.compress_typ t)
in _118_885.FStar_Absyn_Syntax.n)) with
| FStar_Absyn_Syntax.Typ_fun (binders, {FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Comp (ct); FStar_Absyn_Syntax.tk = _52_1565; FStar_Absyn_Syntax.pos = _52_1563; FStar_Absyn_Syntax.fvs = _52_1561; FStar_Absyn_Syntax.uvs = _52_1559}) -> begin
(match (ct.FStar_Absyn_Syntax.effect_args) with
| (FStar_Util.Inl (pre), _52_1584)::(FStar_Util.Inl (post), _52_1579)::(FStar_Util.Inr (pats), _52_1574)::[] -> begin
...
(let _52_1618 = (let _118_888 = (FStar_All.pipe_right patterns (FStar_List.map (fun _52_12 -> (match (_52_12) with
| (FStar_Util.Inl (t), _52_1607) -> begin
(encode_formula t env)
end
| (FStar_Util.Inr (e), _52_1612) -> begin
...
in (let _52_1644 = (let _118_899 = (FStar_Absyn_Util.unmeta_typ pre)
in (encode_formula _118_899 env))
in (match (_52_1644) with
| (pre, decls'') -> begin
(let _52_1647 = (let _118_900 = (FStar_Absyn_Util.unmeta_typ post)
...
and encode_formula_with_labels = (fun phi env -> (let enc = (fun f -> (fun l -> (let _52_1668 = (FStar_Util.fold_map (fun decls x -> (match ((Prims.fst x)) with
| FStar_Util.Inl (t) -> begin
...
in (let enc_prop_c = (fun f -> (fun l -> (let _52_1688 = (FStar_List.fold_right (fun t _52_1676 -> (match (_52_1676) with
...
| (FStar_Util.Inl (lhs), _52_1723)::(FStar_Util.Inl (rhs), _52_1718)::[] -> begin
(let _52_1729 = (encode_formula_with_labels rhs env)
in (match (_52_1729) with
| (l1, labs1, decls1) -> begin
(match (l1.FStar_ToSMT_Term.tm) with
| FStar_ToSMT_Term.App (FStar_ToSMT_Term.True, _52_1732) -> begin
(l1, labs1, decls1)
end
| _52_1736 -> begin
...
| (FStar_Util.Inl (guard), _52_1758)::(FStar_Util.Inl (_then), _52_1753)::(FStar_Util.Inl (_else), _52_1748)::[] -> begin
...
| FStar_Absyn_Syntax.Typ_app ({FStar_Absyn_Syntax.n = FStar_Absyn_Syntax.Typ_const (ih); FStar_Absyn_Syntax.tk = _52_1804; FStar_Absyn_Syntax.pos = _52_1802; FStar_Absyn_Syntax.fvs = _52_1800; FStar_Absyn_Syntax.uvs = _52_1798}, _52_1819::(FStar_Util.Inr (l), _52_1816)::(FStar_Util.Inl (phi), _52_1811)::[]) when (FStar_Absyn_Syntax.lid_equals ih.FStar_Absyn_Syntax.v FStar_Absyn_Const.using_IH) -> begin
...
(let _52_1861 = (let _118_1054 = (FStar_All.pipe_right ps (FStar_List.map (fun _52_17 -> (match (_52_17) with
| (FStar_Util.Inl (t), _52_1850) -> begin
(encode_typ_term t env)
end
| (FStar_Util.Inr (e), _52_1855) -> begin
...
in (let _52_1866 = (match ((FStar_Tc_Env.debug env.tcenv FStar_Options.Low)) with
| true -> begin
...
(match ((FStar_All.pipe_right connectives (FStar_List.tryFind (fun _52_1878 -> (match (_52_1878) with
| (l, _52_1877) -> begin
...
(let _52_1891 = (match ((FStar_Tc_Env.debug env.tcenv FStar_Options.Low)) with
| true -> begin
...
let prims = (let _52_1918 = (fresh_fvar "a" FStar_ToSMT_Term.Type_sort)
in (match (_52_1918) with
| (asym, a) -> begin
(let _52_1921 = (fresh_fvar "x" FStar_ToSMT_Term.Term_sort)
in (match (_52_1921) with
| (xsym, x) -> begin
(let _52_1924 = (fresh_fvar "y" FStar_ToSMT_Term.Term_sort)
...
in (let mk = (fun l v -> (let _118_1322 = (FStar_All.pipe_right prims (FStar_List.filter (fun _52_1944 -> (match (_52_1944) with
| (l', _52_1943) -> begin
(FStar_Absyn_Syntax.lid_equals l l')
end))))
in (FStar_All.pipe_right _118_1322 (FStar_List.collect (fun _52_1948 -> (match (_52_1948) with
| (_52_1946, b) -> begin
(b v)
end))))))
in (let is = (fun l -> (FStar_All.pipe_right prims (FStar_Util.for_some (fun _52_1954 -> (match (_52_1954) with
| (l', _52_1953) -> begin
(FStar_Absyn_Syntax.lid_equals l l')
end)))))
in {mk = mk; is = is}))))))))
...
in (let mk_unit = (fun _52_1960 tt -> (let typing_pred = (FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_bool = (fun _52_1965 tt -> (let typing_pred = (FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int = (fun _52_1972 tt -> (let typing_pred = (FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int_alias = (fun _52_1984 tt -> (let _118_1437 = (let _118_1436 = (let _118_1435 = (let _118_1434 = (let _118_1433 = (FStar_ToSMT_Term.mkApp (FStar_Absyn_Const.int_lid.FStar_Absyn_Syntax.str, []))
in (tt, _118_1433))
in (FStar_ToSMT_Term.mkEq _118_1434))
in (_118_1435, Some ("mapping to int; for now")))
in FStar_ToSMT_Term.Assume (_118_1436))
in (_118_1437)::[]))
in (let mk_str = (fun _52_1988 tt -> (let typing_pred = (FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_ref = (fun reft_name _52_1996 -> (let r = ("r", FStar_ToSMT_Term.Ref_sort)
...
in (let mk_false_interp = (fun _52_2006 false_tm -> (let valid = (FStar_ToSMT_Term.mkApp ("Valid", (false_tm)::[]))
...
in (_118_1493)::[])))
in (let mk_and_interp = (fun conj _52_2012 -> (let aa = ("a", FStar_ToSMT_Term.Type_sort)
in (let bb = ("b", FStar_ToSMT_Term.Type_sort)
in (let a = (FStar_ToSMT_Term.mkFreeV aa)
in (let b = (FStar_ToSMT_Term.mkFreeV bb)
...
in (_118_1507)::[])))))))))
in (let mk_or_interp = (fun disj _52_2023 -> (let aa = ("a", FStar_ToSMT_Term.Type_sort)
in (let bb = ("b", FStar_ToSMT_Term.Type_sort)
in (let a = (FStar_ToSMT_Term.mkFreeV aa)
in (let b = (FStar_ToSMT_Term.mkFreeV bb)
...
in (fun t s tt -> (match ((FStar_Util.find_opt (fun _52_2116 -> (match (_52_2116) with
| (l, _52_2115) -> begin
...
let rec encode_sigelt = (fun env se -> (let _52_2125 = (match ((FStar_Tc_Env.debug env.tcenv FStar_Options.Low)) with
| true -> begin
...
in (match (se) with
| FStar_Absyn_Syntax.Sig_typ_abbrev (_52_2142, _52_2144, _52_2146, _52_2148, FStar_Absyn_Syntax.Effect::[], _52_2152) -> begin
([], env)
end
| FStar_Absyn_Syntax.Sig_typ_abbrev (lid, _52_2157, _52_2159, _52_2161, _52_2163, _52_2165) when (should_skip lid) -> begin
([], env)
end
| FStar_Absyn_Syntax.Sig_typ_abbrev (lid, _52_2170, _52_2172, _52_2174, _52_2176, _52_2178) when (FStar_Absyn_Syntax.lid_equals lid FStar_Absyn_Const.b2t_lid) -> begin
...
| FStar_Absyn_Syntax.Sig_typ_abbrev (lid, tps, _52_2192, t, tags, _52_2196) -> begin
(let _52_2202 = (new_typ_constant_and_tok_from_lid env lid)
in (match (_52_2202) with
| (tname, ttok, env) -> begin
(let _52_2211 = (match (t.FStar_Absyn_Syntax.n) with
...
in (let t = (whnf env t)
in (let _52_2236 = (match ((FStar_All.pipe_right tags (FStar_Util.for_some (fun _52_18 -> (match (_52_18) with
...
in (let kindingAx = (let _52_2240 = (let _118_1849 = (FStar_Tc_Recheck.recompute_kind t)
...
| FStar_Absyn_Syntax.Sig_val_decl (lid, t, quals, _52_2247) -> begin
(let tt = (whnf env t)
...
| FStar_Absyn_Syntax.Sig_assume (l, f, _52_2257, _52_2259) -> begin
...
| FStar_Absyn_Syntax.Sig_tycon (t, tps, k, _52_2270, datas, quals, _52_2274) when (FStar_Absyn_Syntax.lid_equals t FStar_Absyn_Const.precedes_lid) -> begin
...
| FStar_Absyn_Syntax.Sig_tycon (t, _52_2283, _52_2285, _52_2287, _52_2289, _52_2291, _52_2293) when ((FStar_Absyn_Syntax.lid_equals t FStar_Absyn_Const.char_lid) || (FStar_Absyn_Syntax.lid_equals t FStar_Absyn_Const.uint8_lid)) -> begin
...
| FStar_Absyn_Syntax.Sig_tycon (t, tps, k, _52_2303, datas, quals, _52_2307) -> begin
(let is_logical = (FStar_All.pipe_right quals (FStar_Util.for_some (fun _52_19 -> (match (_52_19) with
...
| false -> begin
(let _52_2331 = (fresh_fvar "x" FStar_ToSMT_Term.Term_sort)
in (match (_52_2331) with
| (xxsym, xx) -> begin
(let _52_2374 = (FStar_All.pipe_right datas (FStar_List.fold_left (fun _52_2334 l -> (match (_52_2334) with
| (out, decls) -> begin
(let data_t = (FStar_Tc_Env.lookup_datacon env.tcenv l)
in (let _52_2344 = (match ((FStar_Absyn_Util.function_formals data_t)) with
...
| FStar_Absyn_Syntax.Typ_app (_52_2346, indices) -> begin
indices
end
| _52_2351 -> begin
[]
...
| (indices, decls') -> begin
(let _52_2363 = (match (((FStar_List.length indices) <> (FStar_List.length vars))) with
| true -> begin
(FStar_All.failwith "Impossible")
end
| false -> begin
...
| (data_ax, decls) -> begin
(let _52_2377 = (fresh_fvar "f" FStar_ToSMT_Term.Fuel_sort)
...
in (let _52_2389 = (match ((let _118_1911 = (FStar_Absyn_Util.compress_kind k)
...
(let projection_axioms = (fun tapp vars -> (match ((FStar_All.pipe_right quals (FStar_Util.find_opt (fun _52_20 -> (match (_52_20) with
| FStar_Absyn_Syntax.Projector (_52_2402) -> begin
true
end
| _52_2405 -> begin
false
...
in (let _52_2435 = (match ((FStar_Util.first_N projectee_pos vars)) with
...
in (let pretype_axioms = (fun tapp vars -> (let _52_2444 = (fresh_fvar "x" FStar_ToSMT_Term.Term_sort)
in (match (_52_2444) with
| (xxsym, xx) -> begin
(let _52_2447 = (fresh_fvar "f" FStar_ToSMT_Term.Fuel_sort)
...
in (let _52_2477 = (let tname_decl = (let _118_1949 = (let _118_1948 = (FStar_All.pipe_right vars (FStar_List.map (fun _52_2458 -> (match (_52_2458) with
| (n, s) -> begin
...
in (let pats = (match (((not (is_logical)) && (FStar_All.pipe_right quals (FStar_Util.for_some (fun _52_22 -> (match (_52_22) with
...
| FStar_Absyn_Syntax.Sig_datacon (d, _52_2487, _52_2489, _52_2491, _52_2493, _52_2495) when (FStar_Absyn_Syntax.lid_equals d FStar_Absyn_Const.lexcons_lid) -> begin
([], env)
end
| FStar_Absyn_Syntax.Sig_datacon (d, t, _52_2501, quals, _52_2504, drange) -> begin
...
in (let _52_2520 = (match ((FStar_Absyn_Util.function_formals t)) with
| Some (f, c) -> begin
...
| (formals, t_res) -> begin
(let _52_2523 = (fresh_fvar "f" FStar_ToSMT_Term.Fuel_sort)
...
(let _52_2556 = (let xvars = (FStar_List.map FStar_ToSMT_Term.mkFreeV vars')
in (let dapp = (FStar_ToSMT_Term.mkApp (ddconstrsym, xvars))
...
| () -> begin
(let _52_2566 = (FStar_Absyn_Util.head_and_args t_res)
...
(let _52_2596 = (FStar_List.fold_left (fun _52_2576 arg -> (match (_52_2576) with
| (env, arg_vars, eqns) -> begin
(match (arg) with
| FStar_Util.Inl (targ) -> begin
(let _52_2584 = (let _118_1990 = (FStar_Absyn_Util.new_bvd None)
...
(let _52_2591 = (let _118_1993 = (FStar_Absyn_Util.new_bvd None)
...
| _52_2615 -> begin
(let _52_2616 = (let _118_2025 = (let _118_2024 = (FStar_Absyn_Print.sli d)
...
| FStar_Absyn_Syntax.Sig_bundle (ses, _52_2624, _52_2626, _52_2628) -> begin
(let _52_2633 = (encode_signature env ses)
in (match (_52_2633) with
| (g, env) -> begin
(let _52_2645 = (FStar_All.pipe_right g (FStar_List.partition (fun _52_24 -> (match (_52_24) with
| FStar_ToSMT_Term.Assume (_52_2636, Some ("inversion axiom")) -> begin
false
end
| _52_2642 -> begin
true
...
| (g', inversions) -> begin
(let _52_2654 = (FStar_All.pipe_right g' (FStar_List.partition (fun _52_25 -> (match (_52_25) with
| FStar_ToSMT_Term.DeclFun (_52_2648) -> begin
true
end
| _52_2651 -> begin
false
...
| FStar_Absyn_Syntax.Sig_let (_52_2656, _52_2658, _52_2660, quals) when (FStar_All.pipe_right quals (FStar_Util.for_some (fun _52_26 -> (match (_52_26) with
...
| FStar_Absyn_Syntax.Sig_let ((is_rec, bindings), _52_2677, _52_2679, quals) -> begin
(let eta_expand = (fun binders formals body t -> (let nbinders = (FStar_List.length binders)
in (let _52_2691 = (FStar_Util.first_N nbinders formals)
...
| true -> begin
(let _52_2743 = (FStar_Util.first_N nformals binders)
...
in (FStar_All.try_with (fun _52_2768 -> (match (()) with
| () -> begin
(match ((FStar_All.pipe_right quals (FStar_Util.for_some (fun _52_27 -> (match (_52_27) with
| FStar_Absyn_Syntax.Opaque -> begin
true
end
| _52_2779 -> begin
...
| false -> begin
(let _52_2799 = (FStar_All.pipe_right bindings (FStar_List.fold_left (fun _52_2786 lb -> (match (_52_2786) with
| (toks, typs, decls, env) -> begin
(let _52_2788 = (match ((FStar_Absyn_Util.is_smt_lemma lb.FStar_Absyn_Syntax.lbtyp)) with
| true -> begin
(Prims.raise Let_rec_unencodeable)
end
| false -> begin
...
in (let _52_2794 = (let _118_2093 = (FStar_Util.right lb.FStar_Absyn_Syntax.lbname)
...
in (match (((FStar_All.pipe_right quals (FStar_Util.for_some (fun _52_28 -> (match (_52_28) with
...
| ({FStar_Absyn_Syntax.lbname = _52_2814; FStar_Absyn_Syntax.lbtyp = _52_2812; FStar_Absyn_Syntax.lbeff = _52_2810; FStar_Absyn_Syntax.lbdef = e}::[], t_norm::[], (flid, (f, ftok))::[]) -> begin
...
in (let env0 = env
in (let _52_2864 = (FStar_All.pipe_right toks (FStar_List.fold_left (fun _52_2853 _52_2858 -> (match ((_52_2853, _52_2858)) with
...
| ((flid, f, ftok, g, gtok), {FStar_Absyn_Syntax.lbname = _52_2881; FStar_Absyn_Syntax.lbtyp = _52_2879; FStar_Absyn_Syntax.lbeff = _52_2877; FStar_Absyn_Syntax.lbdef = e}) -> begin
...
in (let _52_2943 = (let _118_2171 = (FStar_List.zip3 gtoks typs bindings)
in (FStar_List.fold_left (fun _52_2931 _52_2935 -> (match ((_52_2931, _52_2935)) with
...
| (decls, eqns, env0) -> begin
(let _52_2952 = (let _118_2173 = (FStar_All.pipe_right decls FStar_List.flatten)
in (FStar_All.pipe_right _118_2173 (FStar_List.partition (fun _52_29 -> (match (_52_29) with
| FStar_ToSMT_Term.DeclFun (_52_2946) -> begin
true
end
| _52_2949 -> begin
false
...
| FStar_Absyn_Syntax.Typ_fun (binders, _52_3008) -> begin
(FStar_All.pipe_right binders (FStar_List.map (fun _52_30 -> (match (_52_30) with
| (FStar_Util.Inl (_52_3013), _52_3016) -> begin
...
in (let _52_3038 = (match ((FStar_Absyn_Util.function_formals t_norm)) with
...
in (let mk_disc_proj_axioms = (fun guard encoded_res_t vapp vars -> (FStar_All.pipe_right quals (FStar_List.collect (fun _52_31 -> (match (_52_31) with
| FStar_Absyn_Syntax.Discriminator (d) -> begin
(let _52_3061 = (FStar_Util.prefix vars)
...
(let _52_3074 = (FStar_Util.prefix vars)
...
in (let _52_3125 = (let vname_decl = (let _118_2222 = (let _118_2221 = (FStar_All.pipe_right formals (FStar_List.map (fun _52_32 -> (match (_52_32) with
| (FStar_Util.Inl (_52_3099), _52_3102) -> begin
FStar_ToSMT_Term.Type_sort
end
| _52_3105 -> begin
FStar_ToSMT_Term.Term_sort
...
| (decls2, env) -> begin
(let _52_3133 = (let res_t = (FStar_Absyn_Util.compress_typ res_t)
...
and encode_signature = (fun env ses -> (FStar_All.pipe_right ses (FStar_List.fold_left (fun _52_3140 se -> (match (_52_3140) with
| (g, env) -> begin
...
in (let _52_3161 = (match ((FStar_All.pipe_left (FStar_Tc_Env.debug env.tcenv) (FStar_Options.Other ("Encoding")))) with
| true -> begin
...
let encode_labels = (fun labs -> (let prefix = (FStar_All.pipe_right labs (FStar_List.map (fun _52_3199 -> (match (_52_3199) with
| (l, _52_3196, _52_3198) -> begin
FStar_ToSMT_Term.DeclFun (((Prims.fst l), [], FStar_ToSMT_Term.Bool_sort, None))
end))))
in (let suffix = (FStar_All.pipe_right labs (FStar_List.collect (fun _52_3206 -> (match (_52_3206) with
...
in (let _52_3252 = (FStar_ToSMT_Z3.init ())
...
in (let _52_3292 = (match ((FStar_Tc_Env.debug tcenv FStar_Options.Low)) with
| true -> begin
...
in {bindings = _52_3295.bindings; depth = _52_3295.depth; tcenv = _52_3295.tcenv; warn = false; cache = _52_3295.cache; nolabels = _52_3295.nolabels; use_zfuel_name = _52_3295.use_zfuel_name; encode_non_total_function_typ = _52_3295.encode_non_total_function_typ}) modul.FStar_Absyn_Syntax.exports)
...
in (let _52_3307 = (match ((FStar_Tc_Env.debug tcenv FStar_Options.Low)) with
| true -> begin
...
let solve = (fun tcenv q -> (let _52_3312 = (let _118_2326 = (let _118_2325 = (let _118_2324 = (FStar_Tc_Env.get_range tcenv)
...
in (let _52_3328 = (let _118_2335 = (FStar_List.filter (fun _52_33 -> (match (_52_33) with
| FStar_Tc_Env.Binding_sig (_52_3322) -> begin
false
end
| _52_3325 -> begin
true
...
| (env_decls, env) -> begin
(let _52_3329 = (match ((FStar_Tc_Env.debug tcenv FStar_Options.Low)) with
| true -> begin
...
(match (qry) with
| FStar_ToSMT_Term.Assume ({FStar_ToSMT_Term.tm = FStar_ToSMT_Term.App (FStar_ToSMT_Term.False, _52_3352); FStar_ToSMT_Term.hash = _52_3349; FStar_ToSMT_Term.freevars = _52_3347}, _52_3357) -> begin
(let _52_3360 = (pop ())
in ())
end
| _52_3363 when tcenv.FStar_Tc_Env.admit -> begin
(let _52_3364 = (pop ())
in ())
end
| FStar_ToSMT_Term.Assume (q, _52_3368) -> begin
(let fresh = ((FStar_String.length q.FStar_ToSMT_Term.hash) >= 2048)
in (let _52_3372 = (FStar_ToSMT_Z3.giveZ3 prefix)
...
in (let _52_3389 = (match ((FStar_ST.read FStar_Options.print_fuels)) with
| true -> begin
...
in (FStar_ToSMT_Z3.ask fresh labels _118_2405 (fun _52_3407 -> (match (_52_3407) with
| (ok, errs') -> begin
(match (errs) with
| [] -> begin
(cb mi p tl (ok, errs'))
...
| true -> begin
(let _52_3423 = (let _118_2420 = (FStar_ST.read FStar_Options.split_cases)
...
in (let _52_3424 = (match ((FStar_ST.read FStar_Options.admit_smt_queries)) with
| true -> begin
()
end
| false -> begin
...
| FStar_ToSMT_Term.App (FStar_ToSMT_Term.True, _52_3441) -> begin
true
end
| _52_3445 -> begin
false
...
let dummy = {FStar_Tc_Env.init = (fun _52_3446 -> ()); FStar_Tc_Env.push = (fun _52_3448 -> ()); FStar_Tc_Env.pop = (fun _52_3450 -> ()); FStar_Tc_Env.mark = (fun _52_3452 -> ()); FStar_Tc_Env.reset_mark = (fun _52_3454 -> ()); FStar_Tc_Env.commit_mark = (fun _52_3456 -> ()); FStar_Tc_Env.encode_modul = (fun _52_3458 _52_3460 -> ()); FStar_Tc_Env.encode_sig = (fun _52_3462 _52_3464 -> ()); FStar_Tc_Env.solve = (fun _52_3466 _52_3468 -> ()); FStar_Tc_Env.is_trivial = (fun _52_3470 _52_3472 -> false); FStar_Tc_Env.finish = (fun _52_3474 -> ()); FStar_Tc_Env.refresh = (fun _52_3475 -> ())}
