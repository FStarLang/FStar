...
let (iface_decls :
  ...
  fun env1 ->
    fun l ->
      let uu___ =
        FStar_Compiler_Effect.pipe_right env1.remaining_iface_decls
...
let (try_lookup_lid :
  ...
  fun env1 ->
    fun l ->
      ...
      FStar_Compiler_Effect.pipe_right uu___ drop_attributes
  ...
  fun fail_on_dup ->
    fun env1 ->
      fun s ->
        ...
        let env3 =
          let uu___ = env2 in
          let uu___1 = FStar_Compiler_Effect.op_Bang globals in
          {
            curmodule = (uu___.curmodule);
            curmonad = (uu___.curmonad);
            modules = (uu___.modules);
        ...
        match uu___ with
        | (env4, lss) ->
            (FStar_Compiler_Effect.pipe_right lss
               (FStar_Compiler_List.iter
                  (fun uu___2 ->
                     match uu___2 with
                     | (lids, se) ->
             ...
             (let env5 =
                let uu___2 = env4 in
                let uu___3 = FStar_Compiler_Effect.op_Bang globals in
                {
                  curmodule = (uu___2.curmodule);
                  curmonad = (uu___2.curmonad);
                  modules = (uu___2.modules);
    ...
    fun ns ->
      let uu___ =
        ...
        | FStar_Pervasives_Native.None ->
            ...
            let uu___2 =
              FStar_Compiler_Effect.pipe_right modules
                (FStar_Compiler_Util.for_some
                   (fun uu___3 ->
                      match uu___3 with
                      | (m, uu___4) ->
  ...
  fun env1 ->
    fun m ->
      let admitted_sig_lids =
        FStar_Compiler_Effect.pipe_right env1.sigaccum
      ...
      let uu___ = m in
      let uu___1 =
        FStar_Compiler_Effect.pipe_right m.FStar_Syntax_Syntax.declarations
          (FStar_Compiler_List.map
             (fun s ->
                match s.FStar_Syntax_Syntax.sigel with
...
let (finish : env -> FStar_Syntax_Syntax.modul -> env) =
  fun env1 ->
    fun modul ->
      FStar_Compiler_Effect.pipe_right modul.FStar_Syntax_Syntax.declarations
        (FStar_Compiler_List.iter
           (fun se ->
              let quals = se.FStar_Syntax_Syntax.sigquals in
              match se.FStar_Syntax_Syntax.sigel with
  ...
  fun env1 ->
    FStar_Compiler_Util.atomically
      (fun uu___ ->
         ...
         (let uu___3 = env1 in
          let uu___4 = FStar_Compiler_Util.smap_copy env1.exported_ids in
          let uu___5 = FStar_Compiler_Util.smap_copy env1.trans_exported_ids in
          let uu___6 = FStar_Compiler_Util.smap_copy env1.includes in
          let uu___7 = FStar_Compiler_Util.smap_copy env1.sigmap in
          {
