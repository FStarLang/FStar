...
in (match (t.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Tm_arrow (_75_537) -> begin
t
end
| FStar_Syntax_Syntax.Tm_refine (_75_540) -> begin
...
let _75_555 = if (FStar_TypeChecker_Env.debug env.tcenv FStar_Options.Low) then begin
...
let _75_583 = (FStar_All.pipe_right bs (FStar_List.fold_left (fun _75_562 b -> (match (_75_562) with
...
let _75_568 = (gen_term_var env x)
in (match (_75_568) with
| (xxsym, xx, env') -> begin
(
# 385 "FStar.SMTEncoding.Encode.fst"
let _75_571 = (let _156_534 = (norm env x.FStar_Syntax_Syntax.sort)
...
let _75_597 = (encode_term t env)
in (match (_75_597) with
| (t, decls) -> begin
(match (fuel_opt) with
| None -> begin
...
let _75_604 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env.tcenv) (FStar_Options.Other ("SMTEncoding"))) then begin
...
| FStar_Syntax_Syntax.Tm_ascribed (t, k, _75_615) -> begin
(encode_term t env)
end
| FStar_Syntax_Syntax.Tm_meta (t, _75_620) -> begin
(encode_term t env)
end
...
| FStar_Syntax_Syntax.Tm_fvar (v, _75_628) -> begin
(let _156_558 = (lookup_free_var env v)
in (_156_558, []))
end
| FStar_Syntax_Syntax.Tm_type (_75_632) -> begin
(FStar_SMTEncoding_Term.mk_Term_type, [])
end
| FStar_Syntax_Syntax.Tm_uinst (t, _75_636) -> begin
(encode_term t env)
end
...
let _75_647 = (FStar_Syntax_Subst.open_comp binders c)
in (match (_75_647) with
| (binders, res) -> begin
...
let _75_654 = (encode_binders None binders env)
in (match (_75_654) with
| (vars, guards, env', decls, _75_653) -> begin
(
...
let _75_660 = (FStar_TypeChecker_Util.pure_or_ghost_pre_and_post env.tcenv res)
in (match (_75_660) with
| (pre_opt, res_t) -> begin
(
# 454 "FStar.SMTEncoding.Encode.fst"
let _75_663 = (encode_term_pred None res_t env' app)
in (match (_75_663) with
| (res_pred, decls') -> begin
(
# 455 "FStar.SMTEncoding.Encode.fst"
let _75_672 = (match (pre_opt) with
| None -> begin
...
let _75_669 = (encode_formula pre env')
in (match (_75_669) with
| (guard, decls0) -> begin
...
in (FStar_All.pipe_right _156_566 (FStar_List.filter (fun _75_677 -> (match (_75_677) with
| (x, _75_676) -> begin
(x <> (Prims.fst fsym))
end)))))
in (
...
let _75_699 = (FStar_Util.smap_add env.cache tkey.FStar_SMTEncoding_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls))))))))))))))
end))))
end))
end))
...
| FStar_Syntax_Syntax.Tm_refine (_75_710) -> begin
(
# 511 "FStar.SMTEncoding.Encode.fst"
let _75_730 = (match ((FStar_TypeChecker_Normalize.normalize_refinement ((FStar_TypeChecker_Normalize.WHNF)::(FStar_TypeChecker_Normalize.EraseUniverses)::[]) env.tcenv t0)) with
| {FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_refine (x, f); FStar_Syntax_Syntax.tk = _75_717; FStar_Syntax_Syntax.pos = _75_715; FStar_Syntax_Syntax.vars = _75_713} -> begin
(
# 513 "FStar.SMTEncoding.Encode.fst"
let _75_725 = (FStar_Syntax_Subst.open_term (((x, None))::[]) f)
in (match (_75_725) with
| (b, f) -> begin
...
in (FStar_All.pipe_right _156_600 (FStar_List.filter (fun _75_748 -> (match (_75_748) with
| (y, _75_747) -> begin
((y <> x) && (y <> fsym))
end)))))
in (
...
let _75_770 = (FStar_Util.smap_add env.cache tkey.FStar_SMTEncoding_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))
end))))))
end))
end))
...
let _75_779 = (encode_term_pred None k env ttm)
in (match (_75_779) with
| (t_has_k, decls) -> begin
(
...
| FStar_Syntax_Syntax.Tm_app (_75_782) -> begin
(
# 562 "FStar.SMTEncoding.Encode.fst"
let _75_786 = (FStar_Syntax_Util.head_and_args t0)
in (match (_75_786) with
| (head, args_e) -> begin
(match ((let _156_617 = (let _156_616 = (FStar_Syntax_Subst.compress head)
in _156_616.FStar_Syntax_Syntax.n)
in (_156_617, args_e))) with
| (FStar_Syntax_Syntax.Tm_abs (_75_788), _75_791) -> begin
...
let _75_837 = (encode_term v1 env)
in (match (_75_837) with
| (v1, decls1) -> begin
(
# 570 "FStar.SMTEncoding.Encode.fst"
let _75_840 = (encode_term v2 env)
...
let _75_845 = (encode_args args_e env)
in (match (_75_845) with
| (args, decls) -> begin
(
# 576 "FStar.SMTEncoding.Encode.fst"
let encode_partial_app = (fun ht_opt -> (
# 577 "FStar.SMTEncoding.Encode.fst"
let _75_850 = (encode_term head env)
in (match (_75_850) with
| (head, decls') -> begin
(
...
let _75_859 = (FStar_Util.first_N (FStar_List.length args_e) formals)
in (match (_75_859) with
| (formals, rest) -> begin
(
# 583 "FStar.SMTEncoding.Encode.fst"
let subst = (FStar_List.map2 (fun _75_863 _75_867 -> (match ((_75_863, _75_867)) with
| ((bv, _75_862), (a, _75_866)) -> begin
...
let _75_872 = (encode_term_pred None ty env app_tm)
in (match (_75_872) with
...
let _75_879 = (lookup_free_var_sym env fv)
in (match (_75_879) with
...
| FStar_Syntax_Syntax.Tm_ascribed (_75_917, t, _75_920) -> begin
Some (t)
end
| _75_924 -> begin
None
...
let _75_932 = (curried_arrow_formals_comp head_type)
...
let _75_963 = (FStar_Syntax_Subst.open_term' bs body)
in (match (_75_963) with
| (bs, body, opening) -> begin
(
# 625 "FStar.SMTEncoding.Encode.fst"
let fallback = (fun _75_965 -> (match (()) with
| () -> begin
(
...
let _75_969 = (let _156_636 = (let _156_635 = (FStar_Syntax_Print.term_to_string t0)
...
let _75_981 = (encode_binders None bs env)
in (match (_75_981) with
| (vars, guards, envbody, decls, _75_980) -> begin
(
# 643 "FStar.SMTEncoding.Encode.fst"
let _75_984 = (encode_term body envbody)
...
let _75_1007 = (encode_term_pred None tfun env f)
in (match (_75_1007) with
| (f_has_t, decls'') -> begin
(
...
let _75_1011 = (FStar_Util.smap_add env.cache tkey.FStar_SMTEncoding_Term.hash (fsym, cvar_sorts, f_decls))
in (f, f_decls)))))
end))))))))
end)
end))))
...
| FStar_Syntax_Syntax.Tm_let ((_75_1014, {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (_75_1026); FStar_Syntax_Syntax.lbunivs = _75_1024; FStar_Syntax_Syntax.lbtyp = _75_1022; FStar_Syntax_Syntax.lbeff = _75_1020; FStar_Syntax_Syntax.lbdef = _75_1018}::_75_1016), _75_1032) -> begin
(FStar_All.failwith "Impossible: already handled by encoding of Sig_let")
end
| FStar_Syntax_Syntax.Tm_let ((false, {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (x); FStar_Syntax_Syntax.lbunivs = _75_1041; FStar_Syntax_Syntax.lbtyp = t1; FStar_Syntax_Syntax.lbeff = _75_1038; FStar_Syntax_Syntax.lbdef = e1}::[]), e2) -> begin
(encode_let x t1 e1 e2 env encode_term)
end
| FStar_Syntax_Syntax.Tm_let (_75_1051) -> begin
(
# 680 "FStar.SMTEncoding.Encode.fst"
let _75_1053 = (FStar_TypeChecker_Errors.warn t0.FStar_Syntax_Syntax.pos "Non-top-level recursive functions are not yet fully encoded to the SMT solver; you may not be able to prove some facts")
in (
...
let _75_1069 = (encode_term e1 env)
in (match (_75_1069) with
| (ee1, decls1) -> begin
(
# 690 "FStar.SMTEncoding.Encode.fst"
let _75_1072 = (FStar_Syntax_Subst.open_term (((x, None))::[]) e2)
in (match (_75_1072) with
| (xs, e2) -> begin
(
# 691 "FStar.SMTEncoding.Encode.fst"
let _75_1076 = (FStar_List.hd xs)
in (match (_75_1076) with
| (x, _75_1075) -> begin
(
...
let _75_1080 = (encode_term e2 env')
in (match (_75_1080) with
| (ee2, decls2) -> begin
...
let _75_1088 = (encode_term e env)
in (match (_75_1088) with
| (scr, decls) -> begin
(
# 698 "FStar.SMTEncoding.Encode.fst"
let _75_1125 = (FStar_List.fold_right (fun b _75_1092 -> (match (_75_1092) with
| (else_case, decls) -> begin
(
# 699 "FStar.SMTEncoding.Encode.fst"
let _75_1096 = (FStar_Syntax_Subst.open_branch b)
in (match (_75_1096) with
| (p, w, br) -> begin
(
# 700 "FStar.SMTEncoding.Encode.fst"
let patterns = (encode_pat env p)
in (FStar_List.fold_right (fun _75_1100 _75_1103 -> (match ((_75_1100, _75_1103)) with
...
let env = (FStar_All.pipe_right projections (FStar_List.fold_left (fun env _75_1109 -> (match (_75_1109) with
| (x, t) -> begin
(push_term_var env x t)
end)) env))
in (
# 705 "FStar.SMTEncoding.Encode.fst"
let _75_1119 = (match (w) with
| None -> begin
(guard, [])
end
| Some (w) -> begin
...
let _75_1116 = (encode_term w env)
in (match (_75_1116) with
| (w, decls2) -> begin
...
let _75_1134 = if (FStar_TypeChecker_Env.debug env.tcenv FStar_Options.Low) then begin
...
let _75_1138 = (FStar_TypeChecker_Util.decorated_pattern_as_term pat)
in (match (_75_1138) with
| (vars, pat_term) -> begin
(
# 727 "FStar.SMTEncoding.Encode.fst"
let _75_1150 = (FStar_All.pipe_right vars (FStar_List.fold_left (fun _75_1141 v -> (match (_75_1141) with
| (env, vars) -> begin
(
# 728 "FStar.SMTEncoding.Encode.fst"
let _75_1147 = (gen_term_var env v)
in (match (_75_1147) with
| (xx, _75_1145, env) -> begin
...
| FStar_Syntax_Syntax.Pat_disj (_75_1155) -> begin
(FStar_All.failwith "Impossible")
end
...
| FStar_Syntax_Syntax.Pat_cons ((f, _75_1170), args) -> begin
(
...
let sub_term_guards = (FStar_All.pipe_right args (FStar_List.mapi (fun i _75_1180 -> (match (_75_1180) with
| (arg, _75_1179) -> begin
(
...
| FStar_Syntax_Syntax.Pat_disj (_75_1187) -> begin
(FStar_All.failwith "Impossible")
end
| (FStar_Syntax_Syntax.Pat_dot_term (x, _)) | (FStar_Syntax_Syntax.Pat_var (x)) | (FStar_Syntax_Syntax.Pat_wild (x)) -> begin
((x, scrutinee))::[]
end
| FStar_Syntax_Syntax.Pat_constant (_75_1197) -> begin
[]
end
| FStar_Syntax_Syntax.Pat_cons ((f, _75_1201), args) -> begin
(let _156_716 = (FStar_All.pipe_right args (FStar_List.mapi (fun i _75_1210 -> (match (_75_1210) with
| (arg, _75_1209) -> begin
(
...
let pat_term = (fun _75_1213 -> (match (()) with
| () -> begin
...
let _75_1229 = (FStar_All.pipe_right l (FStar_List.fold_left (fun _75_1219 _75_1223 -> (match ((_75_1219, _75_1223)) with
| ((tms, decls), (t, _75_1222)) -> begin
(
# 774 "FStar.SMTEncoding.Encode.fst"
let _75_1226 = (encode_term t env)
in (match (_75_1226) with
| (t, decls') -> begin
...
let _75_1235 = (encode_formula_with_labels phi env)
in (match (_75_1235) with
| (t, vars, decls) -> begin
(match (vars) with
| [] -> begin
(t, decls)
...
let _75_1247 = (let _156_731 = (FStar_Syntax_Util.unmeta e)
in (FStar_Syntax_Util.head_and_args _156_731))
in (match (_75_1247) with
| (head, args) -> begin
(match ((let _156_733 = (let _156_732 = (FStar_Syntax_Util.un_uinst head)
in _156_732.FStar_Syntax_Syntax.n)
in (_156_733, args))) with
| (FStar_Syntax_Syntax.Tm_fvar (fv, _75_1250), _75_1254) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.nil_lid) -> begin
[]
end
| (FStar_Syntax_Syntax.Tm_fvar (fv, _75_1258), _75_1270::(hd, _75_1267)::(tl, _75_1263)::[]) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.cons_lid) -> begin
...
let _75_1275 = (FStar_TypeChecker_Errors.warn e.FStar_Syntax_Syntax.pos "SMT pattern is not a list literal; ignoring the pattern")
in [])
end)
end)))
in (
...
let _75_1281 = (let _156_737 = (FStar_Syntax_Util.unmeta p)
in (FStar_All.pipe_right _156_737 FStar_Syntax_Util.head_and_args))
in (match (_75_1281) with
| (head, args) -> begin
(match ((let _156_739 = (let _156_738 = (FStar_Syntax_Util.un_uinst head)
in _156_738.FStar_Syntax_Syntax.n)
in (_156_739, args))) with
| (FStar_Syntax_Syntax.Tm_fvar (fv, _75_1284), (_75_1292, _75_1294)::(e, _75_1289)::[]) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.smtpat_lid) -> begin
(e, None)
end
| (FStar_Syntax_Syntax.Tm_fvar (fv, _75_1300), (e, _75_1305)::[]) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.smtpatT_lid) -> begin
(e, None)
end
| _75_1310 -> begin
...
let _75_1318 = (let _156_744 = (FStar_Syntax_Util.unmeta t)
in (FStar_All.pipe_right _156_744 FStar_Syntax_Util.head_and_args))
in (match (_75_1318) with
| (head, args) -> begin
(match ((let _156_746 = (let _156_745 = (FStar_Syntax_Util.un_uinst head)
in _156_745.FStar_Syntax_Syntax.n)
in (_156_746, args))) with
| (FStar_Syntax_Syntax.Tm_fvar (fv, _75_1321), (e, _75_1326)::[]) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.smtpatOr_lid) -> begin
Some (e)
end
| _75_1331 -> begin
None
...
let _75_1374 = (match ((let _156_752 = (FStar_Syntax_Subst.compress t)
in _156_752.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_arrow (binders, c) -> begin
(
# 820 "FStar.SMTEncoding.Encode.fst"
let _75_1347 = (FStar_Syntax_Subst.open_comp binders c)
in (match (_75_1347) with
| (binders, c) -> begin
(
# 821 "FStar.SMTEncoding.Encode.fst"
let ct = (FStar_Syntax_Util.comp_to_comp_typ c)
in (match (ct.FStar_Syntax_Syntax.effect_args) with
| (pre, _75_1359)::(post, _75_1355)::(pats, _75_1351)::[] -> begin
(
...
in (match (_75_1374) with
| (binders, pre, post, patterns) -> begin
(
# 832 "FStar.SMTEncoding.Encode.fst"
let _75_1381 = (encode_binders None binders env)
in (match (_75_1381) with
| (vars, guards, env, decls, _75_1380) -> begin
(
# 835 "FStar.SMTEncoding.Encode.fst"
let _75_1394 = (let _156_757 = (FStar_All.pipe_right patterns (FStar_List.map (fun branch -> (
# 836 "FStar.SMTEncoding.Encode.fst"
let _75_1391 = (let _156_756 = (FStar_All.pipe_right branch (FStar_List.map (fun _75_1386 -> (match (_75_1386) with
| (t, _75_1385) -> begin
(encode_term t (
# 836 "FStar.SMTEncoding.Encode.fst"
let _75_1387 = env
in {bindings = _75_1387.bindings; depth = _75_1387.depth; tcenv = _75_1387.tcenv; warn = _75_1387.warn; cache = _75_1387.cache; nolabels = _75_1387.nolabels; use_zfuel_name = true; encode_non_total_function_typ = _75_1387.encode_non_total_function_typ}))
end))))
in (FStar_All.pipe_right _156_756 FStar_List.unzip))
in (match (_75_1391) with
| (pats, decls) -> begin
(pats, decls)
end)))))
...
# 856 "FStar.SMTEncoding.Encode.fst"
let _75_1418 = env
in {bindings = _75_1418.bindings; depth = _75_1418.depth; tcenv = _75_1418.tcenv; warn = _75_1418.warn; cache = _75_1418.cache; nolabels = true; use_zfuel_name = _75_1418.use_zfuel_name; encode_non_total_function_typ = _75_1418.encode_non_total_function_typ})
in (
# 857 "FStar.SMTEncoding.Encode.fst"
let _75_1423 = (let _156_770 = (FStar_Syntax_Util.unmeta pre)
in (encode_formula _156_770 env))
in (match (_75_1423) with
| (pre, decls'') -> begin
(
# 858 "FStar.SMTEncoding.Encode.fst"
let _75_1426 = (let _156_771 = (FStar_Syntax_Util.unmeta post)
...
let _75_1440 = (FStar_Util.fold_map (fun decls x -> (
# 864 "FStar.SMTEncoding.Encode.fst"
let _75_1437 = (encode_term (Prims.fst x) env)
in (match (_75_1437) with
| (t, decls') -> begin
...
let _75_1474 = (FStar_List.fold_right (fun _75_1462 _75_1466 -> (match ((_75_1462, _75_1466)) with
| ((t, _75_1461), (phis, labs, decls)) -> begin
(
# 876 "FStar.SMTEncoding.Encode.fst"
let _75_1470 = (encode_formula_with_labels t env)
in (match (_75_1470) with
| (phi, labs', decls') -> begin
((phi)::phis, (FStar_List.append labs' labs), (FStar_List.append decls' decls))
end))
end)) l ([], [], []))
in (match (_75_1474) with
...
| (lhs, _75_1492)::(rhs, _75_1488)::[] -> begin
(
# 887 "FStar.SMTEncoding.Encode.fst"
let _75_1498 = (encode_formula_with_labels rhs env)
in (match (_75_1498) with
| (l1, labs1, decls1) -> begin
(match (l1.FStar_SMTEncoding_Term.tm) with
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.True, _75_1501) -> begin
(l1, labs1, decls1)
end
| _75_1505 -> begin
(
# 891 "FStar.SMTEncoding.Encode.fst"
let _75_1509 = (encode_formula_with_labels lhs env)
in (match (_75_1509) with
...
| (guard, _75_1524)::(_then, _75_1520)::(_else, _75_1516)::[] -> begin
(
# 898 "FStar.SMTEncoding.Encode.fst"
let _75_1530 = (encode_formula_with_labels guard env)
in (match (_75_1530) with
| (g, labs1, decls1) -> begin
(
# 899 "FStar.SMTEncoding.Encode.fst"
let _75_1534 = (encode_formula_with_labels _then env)
in (match (_75_1534) with
| (t, labs2, decls2) -> begin
(
# 900 "FStar.SMTEncoding.Encode.fst"
let _75_1538 = (encode_formula_with_labels _else env)
in (match (_75_1538) with
...
let _75_1560 = (encode_formula_with_labels phi' env)
in (match (_75_1560) with
| (phi, labs, decls) -> begin
...
let _75_1567 = (encode_match e pats FStar_SMTEncoding_Term.mkFalse env encode_formula)
in (match (_75_1567) with
| (t, decls) -> begin
(t, [], decls)
end))
end
| FStar_Syntax_Syntax.Tm_let ((false, {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (x); FStar_Syntax_Syntax.lbunivs = _75_1574; FStar_Syntax_Syntax.lbtyp = t1; FStar_Syntax_Syntax.lbeff = _75_1571; FStar_Syntax_Syntax.lbdef = e1}::[]), e2) -> begin
(
# 929 "FStar.SMTEncoding.Encode.fst"
let _75_1585 = (encode_let x t1 e1 e2 env encode_formula)
in (match (_75_1585) with
| (t, decls) -> begin
(t, [], decls)
end))
end
| FStar_Syntax_Syntax.Tm_app (head, _75_1596::(x, _75_1593)::(t, _75_1589)::[]) -> begin
(
# 933 "FStar.SMTEncoding.Encode.fst"
let head = (FStar_Syntax_Util.un_uinst head)
in (match (head.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Tm_fvar (fv, _75_1603) when (FStar_Ident.lid_equals fv.FStar_Syntax_Syntax.v FStar_Syntax_Const.has_type_lid) -> begin
(
# 936 "FStar.SMTEncoding.Encode.fst"
let _75_1608 = (encode_term x env)
in (match (_75_1608) with
| (x, decls) -> begin
(
# 937 "FStar.SMTEncoding.Encode.fst"
let _75_1611 = (encode_term t env)
in (match (_75_1611) with
| (t, decls') -> begin
...
let _75_1616 = (encode_term phi env)
...
let _75_1621 = (encode_term phi env)
in (match (_75_1621) with
| (tt, decls) -> begin
...
let _75_1633 = (encode_binders None bs env)
in (match (_75_1633) with
| (vars, guards, env, decls, _75_1632) -> begin
(
# 950 "FStar.SMTEncoding.Encode.fst"
let _75_1646 = (let _156_921 = (FStar_All.pipe_right ps (FStar_List.map (fun p -> (
# 951 "FStar.SMTEncoding.Encode.fst"
let _75_1643 = (let _156_920 = (FStar_All.pipe_right p (FStar_List.map (fun _75_1638 -> (match (_75_1638) with
| (t, _75_1637) -> begin
(encode_term t (
# 951 "FStar.SMTEncoding.Encode.fst"
let _75_1639 = env
in {bindings = _75_1639.bindings; depth = _75_1639.depth; tcenv = _75_1639.tcenv; warn = _75_1639.warn; cache = _75_1639.cache; nolabels = _75_1639.nolabels; use_zfuel_name = true; encode_non_total_function_typ = _75_1639.encode_non_total_function_typ}))
end))))
in (FStar_All.pipe_right _156_920 FStar_List.unzip))
in (match (_75_1643) with
| (p, decls) -> begin
(p, (FStar_List.flatten decls))
end)))))
in (FStar_All.pipe_right _156_921 FStar_List.unzip))
in (match (_75_1646) with
| (pats, decls') -> begin
(
# 953 "FStar.SMTEncoding.Encode.fst"
let _75_1650 = (encode_formula_with_labels body env)
in (match (_75_1650) with
...
let _75_1651 = if (FStar_TypeChecker_Env.debug env.tcenv FStar_Options.Low) then begin
...
(match ((FStar_All.pipe_right connectives (FStar_List.tryFind (fun _75_1663 -> (match (_75_1663) with
| (l, _75_1662) -> begin
...
let _75_1676 = if (FStar_TypeChecker_Env.debug env.tcenv FStar_Options.Low) then begin
...
let _75_1697 = (encode_q_body env vars pats body)
in (match (_75_1697) with
| (vars, pats, guard, body, labs, decls) -> begin
...
let _75_1703 = (fresh_fvar "a" FStar_SMTEncoding_Term.Term_sort)
in (match (_75_1703) with
| (asym, a) -> begin
(
# 992 "FStar.SMTEncoding.Encode.fst"
let _75_1706 = (fresh_fvar "x" FStar_SMTEncoding_Term.Term_sort)
in (match (_75_1706) with
| (xsym, x) -> begin
(
# 993 "FStar.SMTEncoding.Encode.fst"
let _75_1709 = (fresh_fvar "y" FStar_SMTEncoding_Term.Term_sort)
in (match (_75_1709) with
| (ysym, y) -> begin
(
...
let mk = (fun l v -> (let _156_1189 = (FStar_All.pipe_right prims (FStar_List.filter (fun _75_1729 -> (match (_75_1729) with
| (l', _75_1728) -> begin
(FStar_Ident.lid_equals l l')
end))))
in (FStar_All.pipe_right _156_1189 (FStar_List.collect (fun _75_1733 -> (match (_75_1733) with
| (_75_1731, b) -> begin
(b v)
end))))))
in (
# 1022 "FStar.SMTEncoding.Encode.fst"
let is = (fun l -> (FStar_All.pipe_right prims (FStar_Util.for_some (fun _75_1739 -> (match (_75_1739) with
| (l', _75_1738) -> begin
(FStar_Ident.lid_equals l l')
end)))))
in {mk = mk; is = is}))))))))
...
let mk_int_alias = (fun _75_1769 tt -> (let _156_1306 = (let _156_1305 = (let _156_1304 = (let _156_1303 = (let _156_1302 = (FStar_SMTEncoding_Term.mkApp (FStar_Syntax_Const.int_lid.FStar_Ident.str, []))
in (tt, _156_1302))
...
let mk_ref = (fun reft_name _75_1781 -> (
...
let mk_false_interp = (fun _75_1791 false_tm -> (
...
let mk_and_interp = (fun conj _75_1797 -> (
...
let mk_or_interp = (fun disj _75_1808 -> (
...
in (fun t s tt -> (match ((FStar_Util.find_opt (fun _75_1879 -> (match (_75_1879) with
| (l, _75_1878) -> begin
...
let _75_1923 = (let _156_1657 = (FStar_Util.right lb.FStar_Syntax_Syntax.lbname)
...
let _75_1975 = (encode_top_level_val env lid t quals)
in (match (_75_1975) with
| (decls, env) -> begin
(
...
| FStar_Syntax_Syntax.Sig_assume (l, f, _75_1981, _75_1983) -> begin
(
# 1234 "FStar.SMTEncoding.Encode.fst"
let _75_1988 = (encode_formula f env)
in (match (_75_1988) with
| (f, decls) -> begin
(
...
| FStar_Syntax_Syntax.Sig_let (lbs, r, _75_1993, _75_1995) when (FStar_All.pipe_right (Prims.snd lbs) (FStar_Util.for_some (fun lb -> (let _156_1667 = (FStar_Util.right lb.FStar_Syntax_Syntax.lbname)
in (should_skip _156_1667))))) -> begin
([], env)
end
| FStar_Syntax_Syntax.Sig_let ((_75_2000, {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (b2t); FStar_Syntax_Syntax.lbunivs = _75_2008; FStar_Syntax_Syntax.lbtyp = _75_2006; FStar_Syntax_Syntax.lbeff = _75_2004; FStar_Syntax_Syntax.lbdef = _75_2002}::[]), _75_2015, _75_2017, _75_2019) when (FStar_Ident.lid_equals b2t FStar_Syntax_Const.b2t_lid) -> begin
(
# 1242 "FStar.SMTEncoding.Encode.fst"
let _75_2025 = (new_term_constant_and_tok_from_lid env b2t)
in (match (_75_2025) with
| (tname, ttok, env) -> begin
(
...
| FStar_Syntax_Syntax.Sig_let (_75_2031, _75_2033, _75_2035, quals) when (FStar_All.pipe_right quals (FStar_Util.for_some (fun _75_13 -> (match (_75_13) with
...
| FStar_Syntax_Syntax.Sig_let ((false, lb::[]), _75_2051, _75_2053, quals) when (FStar_All.pipe_right quals (FStar_Util.for_some (fun _75_14 -> (match (_75_14) with
| FStar_Syntax_Syntax.Projector (_75_2059) -> begin
true
end
| _75_2062 -> begin
false
...
let extra_formals = (let _156_1696 = (FStar_All.pipe_right extra_formals (FStar_List.map (fun _75_2099 -> (match (_75_2099) with
| (x, i) -> begin
(let _156_1695 = (
# 1275 "FStar.SMTEncoding.Encode.fst"
let _75_2100 = x
in (let _156_1694 = (FStar_Syntax_Subst.subst subst x.FStar_Syntax_Syntax.sort)
in {FStar_Syntax_Syntax.ppname = _75_2100.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _75_2100.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = _156_1694}))
in (_156_1695, i))
end))))
...
let _75_2184 = (let _156_1724 = (FStar_Util.right lb.FStar_Syntax_Syntax.lbname)
...
| ({FStar_Syntax_Syntax.lbname = _75_2206; FStar_Syntax_Syntax.lbunivs = _75_2204; FStar_Syntax_Syntax.lbtyp = _75_2202; FStar_Syntax_Syntax.lbeff = _75_2200; FStar_Syntax_Syntax.lbdef = e}::[], t_norm::[], (flid, (f, ftok))::[]) -> begin
(
# 1333 "FStar.SMTEncoding.Encode.fst"
let e = (FStar_Syntax_Subst.compress e)
in (
# 1334 "FStar.SMTEncoding.Encode.fst"
let _75_2225 = (destruct_bound_function flid t_norm e)
in (match (_75_2225) with
| (binders, body, _75_2222, _75_2224) -> begin
(
# 1335 "FStar.SMTEncoding.Encode.fst"
let _75_2232 = (encode_binders None binders env)
in (match (_75_2232) with
| (vars, guards, env', binder_decls, _75_2231) -> begin
(
...
let _75_2241 = if (FStar_All.pipe_right quals (FStar_List.contains FStar_Syntax_Syntax.Logic)) then begin
...
let _75_2261 = (FStar_All.pipe_right toks (FStar_List.fold_left (fun _75_2250 _75_2255 -> (match ((_75_2250, _75_2255)) with
...
let encode_one_binding = (fun env0 _75_2270 t_norm _75_2281 -> (match ((_75_2270, _75_2281)) with
| ((flid, f, ftok, g, gtok), {FStar_Syntax_Syntax.lbname = _75_2280; FStar_Syntax_Syntax.lbunivs = _75_2278; FStar_Syntax_Syntax.lbtyp = _75_2276; FStar_Syntax_Syntax.lbeff = _75_2274; FStar_Syntax_Syntax.lbdef = e}) -> begin
(
# 1357 "FStar.SMTEncoding.Encode.fst"
let _75_2286 = (destruct_bound_function flid t_norm e)
in (match (_75_2286) with
| (binders, body, formals, tres) -> begin
(
# 1358 "FStar.SMTEncoding.Encode.fst"
let _75_2293 = (encode_binders None binders env)
in (match (_75_2293) with
| (vars, guards, env', binder_decls, _75_2292) -> begin
(
...
let _75_2303 = (encode_term body env')
in (match (_75_2303) with
...
let _75_2313 = (encode_binders None formals env0)
in (match (_75_2313) with
| (vars, v_guards, env, binder_decls, _75_2312) -> begin
(
...
let _75_2320 = (encode_term_pred None tres env gapp)
in (match (_75_2320) with
| (g_typing, d3) -> begin
...
in (match (_75_2323) with
| (aux_decls, typing_corr) -> begin
((FStar_List.append binder_decls aux_decls), (FStar_List.append typing_corr ((tok_corr)::[])))
end)))))
end))
in (match (_75_2326) with
...
let _75_2342 = (let _156_1807 = (FStar_List.zip3 gtoks typs bindings)
in (FStar_List.fold_left (fun _75_2330 _75_2334 -> (match ((_75_2330, _75_2334)) with
| ((decls, eqns, env0), (gtok, ty, bs)) -> begin
(
# 1387 "FStar.SMTEncoding.Encode.fst"
let _75_2338 = (encode_one_binding env0 gtok ty bs)
...
let _75_2351 = (let _156_1809 = (FStar_All.pipe_right decls FStar_List.flatten)
in (FStar_All.pipe_right _156_1809 (FStar_List.partition (fun _75_16 -> (match (_75_16) with
| FStar_SMTEncoding_Term.DeclFun (_75_2345) -> begin
true
end
| _75_2348 -> begin
false
end)))))
in (match (_75_2351) with
...
| FStar_Syntax_Syntax.Sig_bundle (ses, _75_2355, _75_2357, _75_2359) -> begin
(
# 1401 "FStar.SMTEncoding.Encode.fst"
let _75_2364 = (encode_signature env ses)
in (match (_75_2364) with
| (g, env) -> begin
(
# 1402 "FStar.SMTEncoding.Encode.fst"
let _75_2376 = (FStar_All.pipe_right g (FStar_List.partition (fun _75_17 -> (match (_75_17) with
| FStar_SMTEncoding_Term.Assume (_75_2367, Some ("inversion axiom")) -> begin
false
end
| _75_2373 -> begin
true
end))))
in (match (_75_2376) with
| (g', inversions) -> begin
(
# 1405 "FStar.SMTEncoding.Encode.fst"
let _75_2385 = (FStar_All.pipe_right g' (FStar_List.partition (fun _75_18 -> (match (_75_18) with
| FStar_SMTEncoding_Term.DeclFun (_75_2379) -> begin
true
end
| _75_2382 -> begin
false
end))))
in (match (_75_2385) with
| (decls, rest) -> begin
...
| FStar_Syntax_Syntax.Sig_inductive_typ (t, _75_2388, tps, k, _75_2392, datas, quals, _75_2396) -> begin
(
...
| (name, args, _75_2410, _75_2412) -> begin
...
let _75_2420 = (fresh_fvar "x" FStar_SMTEncoding_Term.Term_sort)
in (match (_75_2420) with
| (xxsym, xx) -> begin
(
# 1423 "FStar.SMTEncoding.Encode.fst"
let _75_2456 = (FStar_All.pipe_right datas (FStar_List.fold_left (fun _75_2423 l -> (match (_75_2423) with
| (out, decls) -> begin
(
# 1424 "FStar.SMTEncoding.Encode.fst"
let _75_2428 = (FStar_TypeChecker_Env.lookup_datacon env.tcenv l)
in (match (_75_2428) with
| (_75_2426, data_t) -> begin
(
# 1425 "FStar.SMTEncoding.Encode.fst"
let _75_2431 = (FStar_Syntax_Util.arrow_formals data_t)
in (match (_75_2431) with
| (args, res) -> begin
(
...
| FStar_Syntax_Syntax.Tm_app (_75_2433, indices) -> begin
indices
end
| _75_2438 -> begin
[]
...
let env = (FStar_All.pipe_right args (FStar_List.fold_left (fun env _75_2444 -> (match (_75_2444) with
| (x, _75_2443) -> begin
...
let _75_2448 = (encode_args indices env)
in (match (_75_2448) with
| (indices, decls') -> begin
(
# 1433 "FStar.SMTEncoding.Encode.fst"
let _75_2449 = if ((FStar_List.length indices) <> (FStar_List.length vars)) then begin
(FStar_All.failwith "Impossible")
end else begin
()
end
...
in (match (_75_2456) with
| (data_ax, decls) -> begin
(
# 1437 "FStar.SMTEncoding.Encode.fst"
let _75_2459 = (fresh_fvar "f" FStar_SMTEncoding_Term.Fuel_sort)
in (match (_75_2459) with
| (ffsym, ff) -> begin
(
...
let _75_2469 = (match ((let _156_1853 = (FStar_Syntax_Subst.compress k)
...
let _75_2472 = (FStar_Syntax_Subst.open_term formals res)
in (match (_75_2472) with
| (formals, res) -> begin
(
# 1449 "FStar.SMTEncoding.Encode.fst"
let _75_2479 = (encode_binders None formals env)
in (match (_75_2479) with
| (vars, guards, env', binder_decls, _75_2478) -> begin
(
# 1451 "FStar.SMTEncoding.Encode.fst"
let pretype_axioms = (fun tapp vars -> (
# 1452 "FStar.SMTEncoding.Encode.fst"
let _75_2485 = (fresh_fvar "x" FStar_SMTEncoding_Term.Term_sort)
in (match (_75_2485) with
| (xxsym, xx) -> begin
(
# 1453 "FStar.SMTEncoding.Encode.fst"
let _75_2488 = (fresh_fvar "f" FStar_SMTEncoding_Term.Fuel_sort)
in (match (_75_2488) with
| (ffsym, ff) -> begin
(
...
let _75_2493 = (new_term_constant_and_tok_from_lid env t)
in (match (_75_2493) with
| (tname, ttok, env) -> begin
(
...
let tname_decl = (let _156_1872 = (let _156_1871 = (FStar_All.pipe_right vars (FStar_List.map (fun _75_2499 -> (match (_75_2499) with
| (n, s) -> begin
...
in (match (_75_2514) with
| (decls, env) -> begin
(
# 1476 "FStar.SMTEncoding.Encode.fst"
let kindingAx = (
# 1477 "FStar.SMTEncoding.Encode.fst"
let _75_2517 = (encode_term_pred None res env' tapp)
in (match (_75_2517) with
| (k, decls) -> begin
(
...
| FStar_Syntax_Syntax.Sig_datacon (d, _75_2524, _75_2526, _75_2528, _75_2530, _75_2532, _75_2534, _75_2536) when (FStar_Ident.lid_equals d FStar_Syntax_Const.lexcons_lid) -> begin
([], env)
end
| FStar_Syntax_Syntax.Sig_datacon (d, _75_2541, t, _75_2544, n_tps, quals, _75_2548, drange) -> begin
(
# 1496 "FStar.SMTEncoding.Encode.fst"
let _75_2555 = (new_term_constant_and_tok_from_lid env d)
...
let _75_2559 = (FStar_Syntax_Util.arrow_formals t)
in (match (_75_2559) with
| (formals, t_res) -> begin
(
# 1499 "FStar.SMTEncoding.Encode.fst"
let _75_2562 = (fresh_fvar "f" FStar_SMTEncoding_Term.Fuel_sort)
in (match (_75_2562) with
...
let _75_2569 = (encode_binders (Some (fuel_tm)) formals env)
...
let _75_2579 = (encode_term_pred None t env ddtok_tm)
in (match (_75_2579) with
| (tok_typing, decls3) -> begin
(
# 1511 "FStar.SMTEncoding.Encode.fst"
let _75_2586 = (encode_binders (Some (fuel_tm)) formals env)
in (match (_75_2586) with
| (vars', guards', env'', decls_formals, _75_2585) -> begin
(
...
let encode_elim = (fun _75_2598 -> (match (()) with
| () -> begin
(
# 1522 "FStar.SMTEncoding.Encode.fst"
let _75_2601 = (FStar_Syntax_Util.head_and_args t_res)
...
let _75_2625 = (encode_args args env')
in (match (_75_2625) with
| (encoded_args, arg_decls) -> begin
(
# 1528 "FStar.SMTEncoding.Encode.fst"
let _75_2640 = (FStar_List.fold_left (fun _75_2629 arg -> (match (_75_2629) with
| (env, arg_vars, eqns) -> begin
(
# 1529 "FStar.SMTEncoding.Encode.fst"
let _75_2635 = (let _156_1906 = (FStar_Syntax_Syntax.new_bv None FStar_Syntax_Syntax.tun)
in (gen_term_var env _156_1906))
in (match (_75_2635) with
| (_75_2632, xv, env) -> begin
...
let _75_2659 = (let _156_1939 = (let _156_1938 = (FStar_Syntax_Print.lid_to_string d)
...
let _75_2663 = (encode_elim ())
in (match (_75_2663) with
| (decls2, elim) -> begin
(
...
let _75_2672 = (encode_free_var env x t t_norm [])
in (match (_75_2672) with
| (decls, env) -> begin
(
# 1579 "FStar.SMTEncoding.Encode.fst"
let _75_2677 = (lookup_lid env x)
in (match (_75_2677) with
| (n, x', _75_2676) -> begin
((n, x'), decls, env)
end))
end))
end
| Some (n, x, _75_2681) -> begin
((n, x), [], env)
end))
...
let _75_2689 = (encode_function_type_as_formula None None t env)
...
let _75_2698 = (new_term_constant_and_tok_from_lid env lid)
in (match (_75_2698) with
...
| FStar_Syntax_Syntax.Tm_arrow (binders, _75_2701) -> begin
(FStar_All.pipe_right binders (FStar_List.map (fun _75_2704 -> FStar_SMTEncoding_Term.Term_sort)))
end
| _75_2707 -> begin
[]
end)
...
let _75_2717 = (curried_arrow_formals_comp t_norm)
...
in (match (_75_2722) with
| (formals, (pre_opt, res_t)) -> begin
(
# 1609 "FStar.SMTEncoding.Encode.fst"
let _75_2726 = (new_term_constant_and_tok_from_lid env lid)
...
let _75_2745 = (FStar_Util.prefix vars)
in (match (_75_2745) with
| (_75_2740, (xxsym, _75_2743)) -> begin
(
...
let _75_2757 = (FStar_Util.prefix vars)
in (match (_75_2757) with
| (_75_2752, (xxsym, _75_2755)) -> begin
(
...
let _75_2769 = (encode_binders None formals env)
in (match (_75_2769) with
| (vars, guards, env', decls1, _75_2768) -> begin
(
# 1629 "FStar.SMTEncoding.Encode.fst"
let _75_2778 = (match (pre_opt) with
| None -> begin
...
let _75_2775 = (encode_formula p env')
...
let vname_decl = (let _156_2011 = (let _156_2010 = (FStar_All.pipe_right formals (FStar_List.map (fun _75_2781 -> FStar_SMTEncoding_Term.Term_sort)))
...
# 1638 "FStar.SMTEncoding.Encode.fst"
let _75_2784 = env
in {bindings = _75_2784.bindings; depth = _75_2784.depth; tcenv = _75_2784.tcenv; warn = _75_2784.warn; cache = _75_2784.cache; nolabels = _75_2784.nolabels; use_zfuel_name = _75_2784.use_zfuel_name; encode_non_total_function_typ = encode_non_total_function_typ})
...
let _75_2799 = (match (formals) with
| [] -> begin
...
let _75_2806 = (encode_term res_t env')
...
and encode_signature : env_t  ->  FStar_Syntax_Syntax.sigelt Prims.list  ->  (FStar_SMTEncoding_Term.decl Prims.list * env_t) = (fun env ses -> (FStar_All.pipe_right ses (FStar_List.fold_left (fun _75_2817 se -> (match (_75_2817) with
| (g, env) -> begin
(
# 1662 "FStar.SMTEncoding.Encode.fst"
let _75_2821 = (encode_sigelt env se)
in (match (_75_2821) with
| (g', env) -> begin
...
let encode_binding = (fun b _75_2828 -> (match (_75_2828) with
| (decls, env) -> begin
(match (b) with
| FStar_TypeChecker_Env.Binding_univ (_75_2830) -> begin
([], env)
end
| FStar_TypeChecker_Env.Binding_var (x) -> begin
(
# 1695 "FStar.SMTEncoding.Encode.fst"
let _75_2837 = (new_term_constant env x)
in (match (_75_2837) with
| (xxsym, xx, env') -> begin
(
...
let _75_2839 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env.tcenv) (FStar_Options.Other ("Encoding"))) then begin
...
let _75_2843 = (encode_term_pred None t1 env xx)
in (match (_75_2843) with
| (t, decls') -> begin
(
...
| FStar_TypeChecker_Env.Binding_lid (x, (_75_2848, t)) -> begin
(
# 1710 "FStar.SMTEncoding.Encode.fst"
let t_norm = (whnf env t)
in (
# 1712 "FStar.SMTEncoding.Encode.fst"
let _75_2856 = (encode_free_var env x t t_norm [])
in (match (_75_2856) with
| (g, env') -> begin
...
let prefix = (FStar_All.pipe_right labs (FStar_List.map (fun _75_2877 -> (match (_75_2877) with
| (l, _75_2874, _75_2876) -> begin
FStar_SMTEncoding_Term.DeclFun (((Prims.fst l), [], FStar_SMTEncoding_Term.Bool_sort, None))
end))))
in (
# 1724 "FStar.SMTEncoding.Encode.fst"
let suffix = (FStar_All.pipe_right labs (FStar_List.collect (fun _75_2884 -> (match (_75_2884) with
| (l, _75_2881, _75_2883) -> begin
...
let _75_2893 = e
in {bindings = _75_2893.bindings; depth = _75_2893.depth; tcenv = tcenv; warn = _75_2893.warn; cache = _75_2893.cache; nolabels = _75_2893.nolabels; use_zfuel_name = _75_2893.use_zfuel_name; encode_non_total_function_typ = _75_2893.encode_non_total_function_typ})
end))
...
let push_env : Prims.unit  ->  Prims.unit = (fun _75_2901 -> (match (()) with
| () -> begin
...
# 1742 "FStar.SMTEncoding.Encode.fst"
let _75_2907 = hd
in {bindings = _75_2907.bindings; depth = _75_2907.depth; tcenv = _75_2907.tcenv; warn = _75_2907.warn; cache = refs; nolabels = _75_2907.nolabels; use_zfuel_name = _75_2907.use_zfuel_name; encode_non_total_function_typ = _75_2907.encode_non_total_function_typ})
in (FStar_ST.op_Colon_Equals last_env ((top)::(hd)::tl))))
end)
end))

# 1744 "FStar.SMTEncoding.Encode.fst"
let pop_env : Prims.unit  ->  Prims.unit = (fun _75_2910 -> (match (()) with
| () -> begin
(match ((FStar_ST.read last_env)) with
| [] -> begin
...
let mark_env : Prims.unit  ->  Prims.unit = (fun _75_2916 -> (match (()) with
| () -> begin
(push_env ())
end))

# 1748 "FStar.SMTEncoding.Encode.fst"
let reset_mark_env : Prims.unit  ->  Prims.unit = (fun _75_2917 -> (match (()) with
| () -> begin
(pop_env ())
end))

# 1749 "FStar.SMTEncoding.Encode.fst"
let commit_mark_env : Prims.unit  ->  Prims.unit = (fun _75_2918 -> (match (()) with
| () -> begin
(match ((FStar_ST.read last_env)) with
| hd::_75_2921::tl -> begin
...
let _75_2928 = (init_env tcenv)
in (
# 1757 "FStar.SMTEncoding.Encode.fst"
let _75_2930 = (FStar_SMTEncoding_Z3.init ())
...
let _75_2964 = (encode_sigelt env se)
in (match (_75_2964) with
| (decls, env) -> begin
(
# 1786 "FStar.SMTEncoding.Encode.fst"
let _75_2965 = (set_env env)
...
let _75_2970 = if (FStar_TypeChecker_Env.debug tcenv FStar_Options.Low) then begin
...
let _75_2977 = (encode_signature (
# 1794 "FStar.SMTEncoding.Encode.fst"
let _75_2973 = env
in {bindings = _75_2973.bindings; depth = _75_2973.depth; tcenv = _75_2973.tcenv; warn = false; cache = _75_2973.cache; nolabels = _75_2973.nolabels; use_zfuel_name = _75_2973.use_zfuel_name; encode_non_total_function_typ = _75_2973.encode_non_total_function_typ}) modul.FStar_Syntax_Syntax.exports)
in (match (_75_2977) with
| (decls, env) -> begin
(
...
let _75_2983 = (set_env (
# 1800 "FStar.SMTEncoding.Encode.fst"
let _75_2981 = env
in {bindings = _75_2981.bindings; depth = _75_2981.depth; tcenv = _75_2981.tcenv; warn = true; cache = _75_2981.cache; nolabels = _75_2981.nolabels; use_zfuel_name = _75_2981.use_zfuel_name; encode_non_total_function_typ = _75_2981.encode_non_total_function_typ}))
in (
# 1801 "FStar.SMTEncoding.Encode.fst"
let _75_2985 = if (FStar_TypeChecker_Env.debug tcenv FStar_Options.Low) then begin
...
let _75_2990 = (let _156_2111 = (let _156_2110 = (let _156_2109 = (FStar_TypeChecker_Env.get_range tcenv)
...
let pop = (fun _75_2993 -> (match (()) with
| () -> begin
...
let _75_3006 = (aux rest)
in (match (_75_3006) with
| (out, rest) -> begin
(
...
let _75_3008 = x
in {FStar_Syntax_Syntax.ppname = _75_3008.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _75_3008.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = t}))
in (_156_2121)::out)
in (_156_2122, rest)))
end))
end
...
let _75_3014 = (aux bindings)
in (match (_75_3014) with
| (closing, bindings) -> begin
(let _156_2123 = (FStar_Syntax_Util.close_forall (FStar_List.rev closing) q)
in (_156_2123, bindings))
end)))
in (match (_75_3017) with
| (q, bindings) -> begin
(
# 1820 "FStar.SMTEncoding.Encode.fst"
let _75_3026 = (let _156_2125 = (FStar_List.filter (fun _75_21 -> (match (_75_21) with
| FStar_TypeChecker_Env.Binding_sig (_75_3020) -> begin
false
end
| _75_3023 -> begin
true
...
let _75_3027 = if ((FStar_TypeChecker_Env.debug tcenv FStar_Options.Low) || (FStar_All.pipe_left (FStar_TypeChecker_Env.debug tcenv) (FStar_Options.Other ("SMTEncoding")))) then begin
...
let _75_3031 = (encode_formula q env)
in (match (_75_3031) with
| (phi, qdecls) -> begin
(
# 1827 "FStar.SMTEncoding.Encode.fst"
let _75_3036 = (FStar_SMTEncoding_ErrorReporting.label_goals [] phi [])
in (match (_75_3036) with
| (phi, labels, _75_3035) -> begin
(
# 1828 "FStar.SMTEncoding.Encode.fst"
let _75_3039 = (encode_labels labels)
in (match (_75_3039) with
...
in (match (_75_3047) with
| (prefix, labels, qry, suffix) -> begin
(match (qry) with
| FStar_SMTEncoding_Term.Assume ({FStar_SMTEncoding_Term.tm = FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.False, _75_3054); FStar_SMTEncoding_Term.hash = _75_3051; FStar_SMTEncoding_Term.freevars = _75_3049}, _75_3059) -> begin
(
# 1837 "FStar.SMTEncoding.Encode.fst"
let _75_3062 = (pop ())
in ())
end
| _75_3065 when tcenv.FStar_TypeChecker_Env.admit -> begin
(
# 1838 "FStar.SMTEncoding.Encode.fst"
let _75_3066 = (pop ())
in ())
end
| FStar_SMTEncoding_Term.Assume (q, _75_3070) -> begin
(
...
let _75_3074 = (FStar_SMTEncoding_Z3.giveZ3 prefix)
in (
# 1843 "FStar.SMTEncoding.Encode.fst"
let with_fuel = (fun p _75_3080 -> (match (_75_3080) with
| (n, i) -> begin
...
let _75_3091 = if (FStar_ST.read FStar_Options.print_fuels) then begin
...
in (FStar_SMTEncoding_Z3.ask fresh labels _156_2195 (fun _75_3109 -> (match (_75_3109) with
| (ok, errs') -> begin
(match (errs) with
| [] -> begin
(cb mi p tl (ok, errs'))
...
and cb = (fun _75_3115 p alt _75_3120 -> (match ((_75_3115, _75_3120)) with
...
let _75_3125 = (let _156_2210 = (FStar_ST.read FStar_Options.split_cases)
in (FStar_SMTEncoding_SplitQueryCases.can_handle_query _156_2210 q))
in (match (_75_3125) with
| (b, cb) -> begin
if b then begin
...
let _75_3126 = if (FStar_ST.read FStar_Options.admit_smt_queries) then begin
()
end else begin
(process_query qry)
end
...
let _75_3133 = (push "query")
in (
# 1909 "FStar.SMTEncoding.Encode.fst"
let _75_3140 = (encode_formula_with_labels q env)
in (match (_75_3140) with
| (f, _75_3137, _75_3139) -> begin
(
# 1910 "FStar.SMTEncoding.Encode.fst"
let _75_3141 = (pop "query")
in (match (f.FStar_SMTEncoding_Term.tm) with
| FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.True, _75_3145) -> begin
true
end
| _75_3149 -> begin
false
...
let dummy : FStar_TypeChecker_Env.solver_t = {FStar_TypeChecker_Env.init = (fun _75_3150 -> ()); FStar_TypeChecker_Env.push = (fun _75_3152 -> ()); FStar_TypeChecker_Env.pop = (fun _75_3154 -> ()); FStar_TypeChecker_Env.mark = (fun _75_3156 -> ()); FStar_TypeChecker_Env.reset_mark = (fun _75_3158 -> ()); FStar_TypeChecker_Env.commit_mark = (fun _75_3160 -> ()); FStar_TypeChecker_Env.encode_modul = (fun _75_3162 _75_3164 -> ()); FStar_TypeChecker_Env.encode_sig = (fun _75_3166 _75_3168 -> ()); FStar_TypeChecker_Env.solve = (fun _75_3170 _75_3172 -> ()); FStar_TypeChecker_Env.is_trivial = (fun _75_3174 _75_3176 -> false); FStar_TypeChecker_Env.finish = (fun _75_3178 -> ()); FStar_TypeChecker_Env.refresh = (fun _75_3179 -> ())}
