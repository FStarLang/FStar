...
let uu___is_Get_value : decl -> Prims.bool =
  fun projectee  ->
    match projectee with | Get_value _0 -> true | uu____1209 -> false
  
let __proj__Get_value__item___0 : decl -> fv =
  fun projectee  -> match projectee with | Get_value _0 -> _0 
...
let mkInteger : Prims.string -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____1877 =
        let uu____1878 = FStar_Util.ensure_decimal i  in Integer uu____1878
         in
      mk uu____1877 r
      ...
      | (t1,t2) ->
          (match ((t1.tm), (t2.tm)) with
           ...
           | (uu____2171,App (Imp ,t1'::t2'::[])) ->
               let uu____2176 =
                 let uu____2183 =
                   let uu____2186 = mkAnd (t1, t1') r  in [uu____2186; t2']
                    in
                 (Imp, uu____2183)  in
               mkApp' uu____2176 r
           ...
           | uu____2662 ->
               (match ((t2.tm), (t3.tm)) with
                ...
                | (App (TrueOp ,uu____2673),uu____2674) ->
                    let uu____2679 =
                      let uu____2684 = mkNot t1 t1.rng  in (uu____2684, t3)
                       in
                    mkImp uu____2679 r
                   ...
                   FStar_List.fold_left
                     (fun uu____3081  ->
                        fun e  ->
                          ...
                          | (ix1,l) ->
                              let uu____3101 =
                                let uu____3104 = aux ix1 e  in uu____3104 ::
                                  l
                                 in
              ...
              FStar_List.fold_left
                (fun uu____3295  ->
                   fun e  ->
                     ...
                     | (ix,es1) ->
                         let uu____3315 =
                           let uu____3318 = aux shift e  in uu____3318 :: es1
                            in
...
let subst : term -> fv -> term -> term =
  fun t  ->
    fun fv  ->
      fun s  -> let uu____3351 = abstr [fv] t  in inst [s] uu____3351
               ...
               (fun i  ->
                  fun s  ->
                    let uu____3935 =
                      ...
                      (uu____3940, s)  in
                    mkFreeV uu____3935 norng))
           in
        let bvar_names = FStar_List.map fv_of_term bvars  in
        let capp = mkApp (name, bvars) norng  in
        let cid_app =
          let uu____3949 =
            let uu____3956 = constr_id_of_sort sort  in (uu____3956, [capp])
             in
...
let injective_constructor :
  ...
  fun uu____4001  ->
    ...
    | (name,fields,sort) ->
        ...
        let bvar i s =
          let uu____4042 = let uu____4047 = bvar_name i  in (uu____4047, s)
             in
               ...
               (fun i  ->
                  fun uu____4068  ->
                    ...
                    | (uu____4075,s,uu____4077) ->
                        let uu____4078 = bvar i s  in uu____4078 norng))
           in
        let bvar_names = FStar_List.map fv_of_term bvars  in
        let capp = mkApp (name, bvars) norng  in
        let uu____4087 =
          ...
          FStar_All.pipe_right fields
            (FStar_List.map
               (fun uu____4208  ->
                  ...
                  | (uu____4215,sort1,uu____4217) -> sort1))
           in
        let cdecl =
          DeclFun
        ...
        let disc =
          ...
          | (proj_terms,ex_vars) ->
              ...
              let disc_inv_body1 =
                match ex_vars1 with
                | [] -> disc_inv_body
                | uu____4463 -> mkExists ([], ex_vars1, disc_inv_body) norng
                 in
        ...
        fun depth  ->
          ...
          then enclosing_name
            ...
            (let uu____4892 = FStar_Util.string_of_int n1  in
             FStar_Util.format2 "%s.%s" enclosing_name uu____4892)
         in
                   ...
                   fun e  ->
                     ...
                     | (names0,binders,n0) ->
                         let nm =
                           ...
                           Prims.strcat "__lb" uu____5227  in
...
let rec declToSmt : decl -> Prims.string =
  fun decl  ->
    let escape s = FStar_Util.replace_char s '\'' '_'  in
    match decl with
    | DefPrelude  -> prelude ()
    | Caption c ->
        let uu____5322 = FStar_Options.log_queries ()  in
        if uu____5322
        then
          let uu____5323 =
            FStar_All.pipe_right (FStar_Util.splitlines c)
              (fun uu___92_5327  ->
                 match uu___92_5327 with | [] -> "" | h::t -> h)
             in
          FStar_Util.format1 "\n; %s" uu____5323
        else ""
    | DeclFun (f,argsorts,retsort,c) ->
        let l = FStar_List.map strSort argsorts  in
        let uu____5346 = caption_to_string c  in
        let uu____5347 = strSort retsort  in
        FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____5346 f
          (FStar_String.concat " " l) uu____5347
    | DefineFun (f,arg_sorts,retsort,body,c) ->
        let uu____5357 = name_macro_binders arg_sorts  in
        (match uu____5357 with
         | (names1,binders) ->
             let body1 =
               let uu____5389 =
                 FStar_List.map (fun x  -> mkFreeV x norng) names1  in
               inst uu____5389 body  in
             let uu____5402 = caption_to_string c  in
             let uu____5403 = strSort retsort  in
             let uu____5404 = termToSmt (escape f) body1  in
             FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____5402 f
               (FStar_String.concat " " binders) uu____5403 uu____5404)
    | Assume a ->
        let fact_ids_to_string ids =
          FStar_All.pipe_right ids
            (FStar_List.map
               (fun uu___93_5422  ->
                  match uu___93_5422 with
                  | Name n1 ->
           ...
           in
        let fids =
          let uu____5427 = FStar_Options.log_queries ()  in
          if uu____5427
          then
            let uu____5428 =
              let uu____5429 = fact_ids_to_string a.assumption_fact_ids  in
              FStar_String.concat "; " uu____5429  in
            FStar_Util.format1 ";;; Fact-ids: %s\n" uu____5428
          else ""  in
        let n1 = escape a.assumption_name  in
        let uu____5434 = caption_to_string a.assumption_caption  in
        let uu____5435 = termToSmt n1 a.assumption_term  in
        FStar_Util.format4 "%s%s(assert (! %s\n:named %s))" uu____5434 fids
          uu____5435 n1
    | Get_value (x,uu____5437) -> FStar_Util.format1 "(get-value %s)" x
    | Echo s -> FStar_Util.format1 "(echo \"%s\")" s
    | RetainAssumptions uu____5439 -> ""
...
and prelude : Prims.unit -> Prims.string =
  fun uu____5444  ->
    let basic =
      ...
      let uu____5764 =
        let uu____5767 =
          FStar_All.pipe_right constrs
           ...
           in
        FStar_All.pipe_right uu____5767 (FStar_List.map declToSmt)  in
      FStar_All.pipe_right uu____5764 (FStar_String.concat "\n")  in
    let lex_ordering =
  ...
  fun sz  ->
    let uu____5783 =
      let uu____5802 =
        ...
        FStar_Pervasives_Native.fst uu____5803  in
      let uu____5808 =
        let uu____5817 =
          let uu____5824 =
            let uu____5825 = boxBitVecFun sz  in
            FStar_Pervasives_Native.snd uu____5825  in
          (uu____5824, (BitVec_sort sz), true)  in
        [uu____5817]  in
      (uu____5802, uu____5808, Term_sort, ((Prims.parse_int "12") + sz),
        true)
       in
    FStar_All.pipe_right uu____5783 constructor_to_decl
...
let mk_Term_uvar : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____5894 =
        let uu____5901 = let uu____5904 = mkInteger' i norng  in [uu____5904]
           in
        ("Tm_uvar", uu____5901)  in
      mkApp uu____5894 r
    ...
    fun u  ->
      fun v1  ->
        fun t  ->
          ...
          | uu____5929 -> mkApp (u, [t]) t.rng
  
let maybe_elim_box : Prims.string -> Prims.string -> term -> term =
  fun u  ->
    fun v1  ->
      fun t  ->
        let uu____5944 = FStar_Options.smtencoding_elim_box ()  in
        elim_box uu____5944 u v1 t
  ...
  fun sz  ->
    fun t  ->
      let uu____5977 =
        ...
        FStar_Pervasives_Native.fst uu____5978  in
      let uu____5983 =
        let uu____5984 = boxBitVecFun sz  in
        FStar_Pervasives_Native.snd uu____5984  in
      elim_box true uu____5977 uu____5983 t
  
let unboxBitVec : Prims.int -> term -> term =
  fun sz  ->
    fun t  ->
      let uu____5997 =
        let uu____5998 = boxBitVecFun sz  in
        FStar_Pervasives_Native.snd uu____5998  in
      let uu____6003 =
        let uu____6004 = boxBitVecFun sz  in
        FStar_Pervasives_Native.fst uu____6004  in
      elim_box true uu____5997 uu____6003 t
...
let boxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | uu____6018 -> FStar_Exn.raise FStar_Util.Impos
...
let unboxTerm : sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | uu____6028 -> FStar_Exn.raise FStar_Util.Impos
...
let rec print_smt_term : term -> Prims.string =
  fun t  ->
    ...
    | BoundV n1 ->
        let uu____6044 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "(BoundV %s)" uu____6044
    | FreeV fv ->
        FStar_Util.format1 "(FreeV %s)" (FStar_Pervasives_Native.fst fv)
    | App (op,l) ->
        let uu____6056 = op_to_string op  in
        let uu____6057 = print_smt_term_list l  in
        FStar_Util.format2 "(%s %s)" uu____6056 uu____6057
    | Labeled (t1,r1,r2) ->
        let uu____6061 = print_smt_term t1  in
        FStar_Util.format2 "(Labeled '%s' %s)" r1 uu____6061
    | LblPos (t1,s) ->
        let uu____6064 = print_smt_term t1  in
        FStar_Util.format2 "(LblPos %s %s)" s uu____6064
    | Quant (qop,l,uu____6067,uu____6068,t1) ->
        let uu____6086 = print_smt_term_list_list l  in
        let uu____6087 = print_smt_term t1  in
        FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____6086
          uu____6087
    | Let (es,body) ->
        let uu____6094 = print_smt_term_list es  in
        let uu____6095 = print_smt_term body  in
        FStar_Util.format2 "(let %s %s)" uu____6094 uu____6095

and print_smt_term_list : term Prims.list -> Prims.string =
  fun l  ->
    let uu____6099 = FStar_List.map print_smt_term l  in
    FStar_All.pipe_right uu____6099 (FStar_String.concat " ")
    ...
    FStar_List.fold_left
      (fun s  ->
         fun l1  ->
           let uu____6118 =
             let uu____6119 =
               let uu____6120 = print_smt_term_list l1  in
               Prims.strcat uu____6120 " ] "  in
             Prims.strcat "; [ " uu____6119  in
           Prims.strcat s uu____6118) "" l
...
let getBoxedInteger : term -> Prims.int FStar_Pervasives_Native.option =
  fun t  ->
    ...
    | App (Var s,t2::[]) when s = (FStar_Pervasives_Native.fst boxIntFun) ->
        (match t2.tm with
         | Integer n1 ->
             let uu____6136 = FStar_Util.int_of_string n1  in
  ...
  fun t  ->
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_Equality",uu____6149::t1::t2::[]);
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_disEquality",uu____6166::t1::t2::[]);
                       freevars = uu____6169; rng = uu____6170;_}::[])
        -> let uu____6183 = mkEq (t1, t2) norng  in mkNot uu____6183 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_LessThanOrEqual",t1::t2::[]);
                       freevars = uu____6186; rng = uu____6187;_}::[])
        ->
        let uu____6200 =
          let uu____6205 = unboxInt t1  in
          let uu____6206 = unboxInt t2  in (uu____6205, uu____6206)  in
        mkLTE uu____6200 t.rng
    | App
        (Var
        ...
        ->
        let uu____6223 =
          let uu____6228 = unboxInt t1  in
          let uu____6229 = unboxInt t2  in (uu____6228, uu____6229)  in
        mkLT uu____6223 t.rng
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_GreaterThanOrEqual",t1::t2::[]);
                       freevars = uu____6232; rng = uu____6233;_}::[])
        ->
        let uu____6246 =
          let uu____6251 = unboxInt t1  in
          let uu____6252 = unboxInt t2  in (uu____6251, uu____6252)  in
        mkGTE uu____6246 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_GreaterThan",t1::t2::[]);
                       freevars = uu____6255; rng = uu____6256;_}::[])
        ->
        let uu____6269 =
          let uu____6274 = unboxInt t1  in
          let uu____6275 = unboxInt t2  in (uu____6274, uu____6275)  in
        mkGT uu____6269 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_AmpAmp",t1::t2::[]);
                       freevars = uu____6278; rng = uu____6279;_}::[])
        ->
        let uu____6292 =
          let uu____6297 = unboxBool t1  in
          let uu____6298 = unboxBool t2  in (uu____6297, uu____6298)  in
        mkAnd uu____6292 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_BarBar",t1::t2::[]);
                       freevars = uu____6301; rng = uu____6302;_}::[])
        ->
        let uu____6315 =
          let uu____6320 = unboxBool t1  in
          let uu____6321 = unboxBool t2  in (uu____6320, uu____6321)  in
        mkOr uu____6315 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_Negation",t1::[]);
                       freevars = uu____6323; rng = uu____6324;_}::[])
        -> let uu____6337 = unboxBool t1  in mkNot uu____6337 t1.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "FStar.BV.bvult",t0::t1::t2::[]);
                       freevars = uu____6341; rng = uu____6342;_}::[])
        when
        let uu____6355 = getBoxedInteger t0  in FStar_Util.is_some uu____6355
        ->
        let sz =
          let uu____6359 = getBoxedInteger t0  in
          match uu____6359 with
          | FStar_Pervasives_Native.Some sz -> sz
          | uu____6363 -> failwith "impossible"  in
        let uu____6366 =
          let uu____6371 = unboxBitVec sz t1  in
          let uu____6372 = unboxBitVec sz t2  in (uu____6371, uu____6372)  in
        mkBvUlt uu____6366 t.rng
    | App
        (Var
         "Prims.equals",uu____6373::{
                                      tm = App
                                        (Var "FStar.BV.bvult",t0::t1::t2::[]);
                                      freevars = uu____6377;
                                      rng = uu____6378;_}::uu____6379::[])
        when
        let uu____6392 = getBoxedInteger t0  in FStar_Util.is_some uu____6392
        ->
        let sz =
          let uu____6396 = getBoxedInteger t0  in
          match uu____6396 with
          | FStar_Pervasives_Native.Some sz -> sz
          | uu____6400 -> failwith "impossible"  in
        let uu____6403 =
          let uu____6408 = unboxBitVec sz t1  in
          let uu____6409 = unboxBitVec sz t2  in (uu____6408, uu____6409)  in
        mkBvUlt uu____6403 t.rng
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___94_6413 = unboxBool t1  in
        {
          tm = (uu___94_6413.tm);
          freevars = (uu___94_6413.freevars);
          rng = (t.rng)
        }
    | uu____6414 -> mkApp ("Valid", [t]) t.rng
...
let mk_HasTypeFuel : term -> term -> term -> term =
  fun f  ->
    fun v1  ->
      fun t  ->
        let uu____6455 = FStar_Options.unthrottle_inductives ()  in
        if uu____6455
...
let mk_String_const : Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____6546 =
        let uu____6553 = let uu____6556 = mkInteger' i norng  in [uu____6556]
           in
        ("FString_const", uu____6553)  in
      mkApp uu____6546 r
...
let mk_Precedes : term -> term -> FStar_Range.range -> term =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____6571 = mkApp ("Precedes", [x1; x2]) r  in
        FStar_All.pipe_right uu____6571 mk_Valid
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____6642 = mkAnd (p11, p21) r  in
...
let mk_haseq : term -> term =
  fun t  ->
    let uu____6725 = mkApp ("Prims.hasEq", [t]) t.rng  in mk_Valid uu____6725
