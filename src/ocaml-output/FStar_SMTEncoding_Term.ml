...
let qop_to_string: qop -> Prims.string =
  fun uu___108_1668  ->
    match uu___108_1668 with | Forall  -> "forall" | Exists  -> "exists"
let op_to_string: op -> Prims.string =
  fun uu___109_1672  ->
    match uu___109_1672 with
    | TrueOp  -> "true"
    | FalseOp  -> "false"
    | Not  -> "not"
    | And  -> "and"
  ...
  fun enclosing_name  ->
    fun t  ->
      let next_qid =
        ...
        fun depth  ->
          ...
          then enclosing_name
            ...
            (let uu____4997 = FStar_Util.string_of_int n1 in
             FStar_Util.format2 "%s.%s" enclosing_name uu____4997) in
                ...
                FStar_All.pipe_right ps
                  (FStar_List.map
                     (fun tm  ->
                        ...
                        | App
                            (Var
                             "Prims.guard_free",{ tm = BoundV uu____5039;
                                                  freevars = uu____5040;
                                                  rng = uu____5041;_}::[])
                            -> tm
      ...
      let rec aux' depth n1 names1 t1 =
        ...
        | Integer i -> i
        | BoundV i ->
            let uu____5095 = FStar_List.nth names1 i in
            FStar_All.pipe_right uu____5095 FStar_Pervasives_Native.fst
        | FreeV x -> FStar_Pervasives_Native.fst x
        | App (op,[]) -> op_to_string op
        | App (op,tms) ->
            let uu____5110 = op_to_string op in
            let uu____5111 =
              let uu____5112 = FStar_List.map (aux1 n1 names1) tms in
              FStar_All.pipe_right uu____5112 (FStar_String.concat "\n") in
            FStar_Util.format2 "(%s %s)" uu____5110 uu____5111
        | Labeled (t2,uu____5118,uu____5119) -> aux1 n1 names1 t2
        | LblPos (t2,s) ->
            let uu____5122 = aux1 n1 names1 t2 in
            FStar_Util.format2 "(! %s :lblpos %s)" uu____5122 s
        | Quant (qop,pats,wopt,sorts,body) ->
            let qid = next_qid () in
            let uu____5145 =
                              ...
                              (fun pats2  ->
                                 let uu____5215 =
                                   let uu____5216 =
                                     FStar_List.map
                                       (fun p  ->
                                          let uu____5222 = aux1 n2 names2 p in
                                          FStar_Util.format1 "%s" uu____5222)
                                       pats2 in
                                   FStar_String.concat " " uu____5216 in
                                 FStar_Util.format1 "\n:pattern (%s)"
                                   uu____5215)) in
                       FStar_All.pipe_right uu____5199
                         (FStar_String.concat "\n") in
                 let uu____5225 =
                   let uu____5228 =
                     let uu____5231 =
                       let uu____5234 = aux1 n2 names2 body in
                       let uu____5235 =
                         let uu____5238 = weightToSmt wopt in
                         [uu____5238; pats_str; qid] in
                       uu____5234 :: uu____5235 in
                     binders1 :: uu____5231 in
                   (qop_to_string qop) :: uu____5228 in
              ...
              FStar_List.fold_left
                (fun uu____5282  ->
                   fun e  ->
                     ...
                     | (names0,binders,n0) ->
                         let nm =
                           let uu____5332 = FStar_Util.string_of_int n0 in
                           Prims.strcat "@lb" uu____5332 in
                         let names01 = (nm, Term_sort) :: names0 in
                         let b =
                           let uu____5345 = aux1 n1 names1 e in
                           FStar_Util.format2 "(%s %s)" nm uu____5345 in
                         (names01, (b :: binders),
                           (n0 + (Prims.parse_int "1")))) (names1, [], n1) es in
            (match uu____5245 with
             | (names2,binders,n2) ->
                 let uu____5377 = aux1 n2 names2 body in
                 FStar_Util.format2 "(let (%s)\n%s)"
                   (FStar_String.concat " " binders) uu____5377)
      and aux depth n1 names1 t1 =
        let s = aux' depth n1 names1 t1 in
        if t1.rng <> norng
        then
          let uu____5385 = FStar_Range.string_of_range t1.rng in
          let uu____5386 = FStar_Range.string_of_use_range t1.rng in
          FStar_Util.format3 "\n;; def=%s; use=%s\n%s\n" uu____5385
            uu____5386 s
        else s in
          ...
          then
            let uu____5431 =
              FStar_All.pipe_right (FStar_Util.splitlines c)
                (fun uu___112_5435  ->
                   match uu___112_5435 with | [] -> "" | h::t -> h) in
            FStar_Util.format1 "\n; %s" uu____5431
          else ""
      | DeclFun (f,argsorts,retsort,c) ->
          let l = FStar_List.map strSort argsorts in
          let uu____5454 = caption_to_string c in
          let uu____5455 = strSort retsort in
          FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____5454 f
            (FStar_String.concat " " l) uu____5455
      | DefineFun (f,arg_sorts,retsort,body,c) ->
          let uu____5465 = name_macro_binders arg_sorts in
          (match uu____5465 with
           | (names1,binders) ->
               let body1 =
                 let uu____5497 =
                   FStar_List.map (fun x  -> mkFreeV x norng) names1 in
                 inst uu____5497 body in
               let uu____5510 = caption_to_string c in
               let uu____5511 = strSort retsort in
               let uu____5512 = termToSmt (escape f) body1 in
               FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____5510
                 f (FStar_String.concat " " binders) uu____5511 uu____5512)
      | Assume a ->
          let fact_ids_to_string ids =
            FStar_All.pipe_right ids
              (FStar_List.map
                 (fun uu___113_5530  ->
                    match uu___113_5530 with
                    | Name n1 ->
          ...
          let fids =
            let uu____5535 = FStar_Options.log_queries () in
            if uu____5535
            then
              let uu____5536 =
                let uu____5537 = fact_ids_to_string a.assumption_fact_ids in
                FStar_String.concat "; " uu____5537 in
              FStar_Util.format1 ";;; Fact-ids: %s\n" uu____5536
            else "" in
          let n1 = escape a.assumption_name in
          let uu____5542 = caption_to_string a.assumption_caption in
          let uu____5543 = termToSmt n1 a.assumption_term in
          FStar_Util.format4 "%s%s(assert (! %s\n:named %s))" uu____5542 fids
            uu____5543 n1
      | Eval t ->
          let uu____5545 = termToSmt "eval" t in
          FStar_Util.format1 "(eval %s)" uu____5545
      | Echo s -> FStar_Util.format1 "(echo \"%s\")" s
      | RetainAssumptions uu____5547 -> ""
  ...
  fun z3options  ->
    ...
    let bcons =
      let uu____5872 =
        ...
        FStar_All.pipe_right uu____5875
          (FStar_List.map (declToSmt z3options)) in
      FStar_All.pipe_right uu____5872 (FStar_String.concat "\n") in
    let lex_ordering =
  ...
  fun sz  ->
    let uu____5891 =
      ...
      let uu____5916 =
        let uu____5925 =
          let uu____5932 =
            let uu____5933 = boxBitVecFun sz in
            FStar_Pervasives_Native.snd uu____5933 in
          (uu____5932, (BitVec_sort sz), true) in
        [uu____5925] in
      (uu____5910, uu____5916, Term_sort, ((Prims.parse_int "12") + sz),
        true) in
    FStar_All.pipe_right uu____5891 constructor_to_decl
...
let mk_Term_uvar: Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____6002 =
        let uu____6009 = let uu____6012 = mkInteger' i norng in [uu____6012] in
        ("Tm_uvar", uu____6009) in
      mkApp uu____6002 r
    ...
    fun u  ->
      fun v1  ->
        fun t  ->
          ...
          | uu____6037 -> mkApp (u, [t]) t.rng
let maybe_elim_box: Prims.string -> Prims.string -> term -> term =
  fun u  ->
    fun v1  ->
      fun t  ->
        let uu____6052 = FStar_Options.smtencoding_elim_box () in
        elim_box uu____6052 u v1 t
...
let boxBitVec: Prims.int -> term -> term =
  fun sz  ->
    fun t  ->
      let uu____6085 =
        let uu____6086 = boxBitVecFun sz in
        FStar_Pervasives_Native.fst uu____6086 in
      let uu____6091 =
        let uu____6092 = boxBitVecFun sz in
        FStar_Pervasives_Native.snd uu____6092 in
      elim_box true uu____6085 uu____6091 t
let unboxBitVec: Prims.int -> term -> term =
  fun sz  ->
    fun t  ->
      let uu____6105 =
        let uu____6106 = boxBitVecFun sz in
        FStar_Pervasives_Native.snd uu____6106 in
      let uu____6111 =
        let uu____6112 = boxBitVecFun sz in
        FStar_Pervasives_Native.fst uu____6112 in
      elim_box true uu____6105 uu____6111 t
let boxTerm: sort -> term -> term =
  fun sort  ->
    fun t  ->
      ...
      | uu____6126 -> FStar_Exn.raise FStar_Util.Impos
...
let rec print_smt_term: term -> Prims.string =
  fun t  ->
    ...
    | BoundV n1 ->
        let uu____6152 = FStar_Util.string_of_int n1 in
        FStar_Util.format1 "(BoundV %s)" uu____6152
    | FreeV fv ->
        FStar_Util.format1 "(FreeV %s)" (FStar_Pervasives_Native.fst fv)
    | App (op,l) ->
        let uu____6164 = op_to_string op in
        let uu____6165 = print_smt_term_list l in
        FStar_Util.format2 "(%s %s)" uu____6164 uu____6165
    | Labeled (t1,r1,r2) ->
        let uu____6169 = print_smt_term t1 in
        FStar_Util.format2 "(Labeled '%s' %s)" r1 uu____6169
    | LblPos (t1,s) ->
        let uu____6172 = print_smt_term t1 in
        FStar_Util.format2 "(LblPos %s %s)" s uu____6172
    | Quant (qop,l,uu____6175,uu____6176,t1) ->
        let uu____6194 = print_smt_term_list_list l in
        let uu____6195 = print_smt_term t1 in
        FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____6194
          uu____6195
    | Let (es,body) ->
        let uu____6202 = print_smt_term_list es in
        let uu____6203 = print_smt_term body in
        FStar_Util.format2 "(let %s %s)" uu____6202 uu____6203
and print_smt_term_list: term Prims.list -> Prims.string =
  fun l  ->
    let uu____6207 = FStar_List.map print_smt_term l in
    FStar_All.pipe_right uu____6207 (FStar_String.concat " ")
    ...
    FStar_List.fold_left
      (fun s  ->
         fun l1  ->
           let uu____6226 =
             let uu____6227 =
               let uu____6228 = print_smt_term_list l1 in
               Prims.strcat uu____6228 " ] " in
             Prims.strcat "; [ " uu____6227 in
           Prims.strcat s uu____6226) "" l
  ...
  fun t  ->
    ...
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_Equality",uu____6257::t1::t2::[]);
                       freevars = uu____6260; rng = uu____6261;_}::[])
        -> mkEq (t1, t2) t.rng
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_disEquality",uu____6274::t1::t2::[]);
                       freevars = uu____6277; rng = uu____6278;_}::[])
        -> let uu____6291 = mkEq (t1, t2) norng in mkNot uu____6291 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_LessThanOrEqual",t1::t2::[]);
                       freevars = uu____6294; rng = uu____6295;_}::[])
        ->
        let uu____6308 =
          let uu____6313 = unboxInt t1 in
          let uu____6314 = unboxInt t2 in (uu____6313, uu____6314) in
        mkLTE uu____6308 t.rng
    | App
        (Var
        ...
        ->
        let uu____6331 =
          let uu____6336 = unboxInt t1 in
          let uu____6337 = unboxInt t2 in (uu____6336, uu____6337) in
        mkLT uu____6331 t.rng
    | App
        (Var
         "Prims.b2t",{
                       tm = App
                         (Var "Prims.op_GreaterThanOrEqual",t1::t2::[]);
                       freevars = uu____6340; rng = uu____6341;_}::[])
        ->
        let uu____6354 =
          let uu____6359 = unboxInt t1 in
          let uu____6360 = unboxInt t2 in (uu____6359, uu____6360) in
        mkGTE uu____6354 t.rng
    | App
        (Var
        ...
        ->
        let uu____6377 =
          let uu____6382 = unboxInt t1 in
          let uu____6383 = unboxInt t2 in (uu____6382, uu____6383) in
        mkGT uu____6377 t.rng
    | App
        (Var
        ...
        ->
        let uu____6400 =
          let uu____6405 = unboxBool t1 in
          let uu____6406 = unboxBool t2 in (uu____6405, uu____6406) in
        mkAnd uu____6400 t.rng
    | App
        (Var
        ...
        ->
        let uu____6423 =
          let uu____6428 = unboxBool t1 in
          let uu____6429 = unboxBool t2 in (uu____6428, uu____6429) in
        mkOr uu____6423 t.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "Prims.op_Negation",t1::[]);
                       freevars = uu____6431; rng = uu____6432;_}::[])
        -> let uu____6445 = unboxBool t1 in mkNot uu____6445 t1.rng
    | App
        (Var
         "Prims.b2t",{ tm = App (Var "FStar.BV.bvult",t0::t1::t2::[]);
                       freevars = uu____6449; rng = uu____6450;_}::[])
        when
        let uu____6463 = getBoxedInteger t0 in FStar_Util.is_some uu____6463
        ->
        let sz =
          let uu____6467 = getBoxedInteger t0 in
          match uu____6467 with
          | FStar_Pervasives_Native.Some sz -> sz
          | uu____6471 -> failwith "impossible" in
        let uu____6474 =
          let uu____6479 = unboxBitVec sz t1 in
          let uu____6480 = unboxBitVec sz t2 in (uu____6479, uu____6480) in
        mkBvUlt uu____6474 t.rng
    | App
        (Var
         "Prims.equals",uu____6481::{
                                      tm = App
                                        (Var "FStar.BV.bvult",t0::t1::t2::[]);
                                      freevars = uu____6485;
                                      rng = uu____6486;_}::uu____6487::[])
        when
        let uu____6500 = getBoxedInteger t0 in FStar_Util.is_some uu____6500
        ->
        let sz =
          let uu____6504 = getBoxedInteger t0 in
          match uu____6504 with
          | FStar_Pervasives_Native.Some sz -> sz
          | uu____6508 -> failwith "impossible" in
        let uu____6511 =
          let uu____6516 = unboxBitVec sz t1 in
          let uu____6517 = unboxBitVec sz t2 in (uu____6516, uu____6517) in
        mkBvUlt uu____6511 t.rng
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___114_6521 = unboxBool t1 in
        {
          tm = (uu___114_6521.tm);
          freevars = (uu___114_6521.freevars);
          rng = (t.rng)
        }
    | uu____6522 -> mkApp ("Valid", [t]) t.rng
...
let mk_HasTypeFuel: term -> term -> term -> term =
  fun f  ->
    fun v1  ->
      fun t  ->
        let uu____6563 = FStar_Options.unthrottle_inductives () in
        if uu____6563
...
let mk_String_const: Prims.int -> FStar_Range.range -> term =
  fun i  ->
    fun r  ->
      let uu____6654 =
        let uu____6661 = let uu____6664 = mkInteger' i norng in [uu____6664] in
        ("FString_const", uu____6661) in
      mkApp uu____6654 r
let mk_Precedes: term -> term -> FStar_Range.range -> term =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____6679 = mkApp ("Precedes", [x1; x2]) r in
        FStar_All.pipe_right uu____6679 mk_Valid
...
let rec n_fuel: Prims.int -> term =
  fun n1  ->
    ...
    then mkApp ("ZFuel", []) norng
      ...
      (let uu____6703 =
         let uu____6710 =
           let uu____6713 = n_fuel (n1 - (Prims.parse_int "1")) in
           [uu____6713] in
         ("SFuel", uu____6710) in
       mkApp uu____6703 norng)
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____6750 = mkAnd (p11, p21) r in
            FStar_Pervasives_Native.Some uu____6750
...
let mk_and_l: term Prims.list -> FStar_Range.range -> term =
  fun l  ->
    fun r  ->
      let uu____6807 = mkTrue r in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkAnd (p1, p2) r) l
        uu____6807
let mk_or_l: term Prims.list -> FStar_Range.range -> term =
  fun l  ->
    fun r  ->
      let uu____6824 = mkFalse r in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkOr (p1, p2) r) l
        uu____6824
let mk_haseq: term -> term =
  fun t  ->
    let uu____6833 = mkApp ("Prims.hasEq", [t]) t.rng in mk_Valid uu____6833