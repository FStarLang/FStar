...
| Array (s1, s2) -> begin
(let _148_52 = (strSort s1)
in (let _148_51 = (strSort s2)
in (FStar_Util.format2 "(Array %s %s)" _148_52 _148_51)))
end
| Arrow (s1, s2) -> begin
(let _148_54 = (strSort s1)
in (let _148_53 = (strSort s2)
in (FStar_Util.format2 "(%s -> %s)" _148_54 _148_53)))
end
| Sort (s) -> begin
s
end))
...
# 191 "FStar.SMTEncoding.Term.fst"
...
# 197 "FStar.SMTEncoding.Term.fst"
let freevar_sort : term  ->  sort = (fun _69_1 -> (match (_69_1) with
| {tm = FreeV (x); hash = _69_93; freevars = _69_91} -> begin
(fv_sort x)
end
| _69_98 -> begin
...
# 200 "FStar.SMTEncoding.Term.fst"
let fv_of_term : term  ->  fv = (fun _69_2 -> (match (_69_2) with
| {tm = FreeV (fv); hash = _69_103; freevars = _69_101} -> begin
fv
end
| _69_108 -> begin
(FStar_All.failwith "impossible")
...
# 203 "FStar.SMTEncoding.Term.fst"
...
# 210 "FStar.SMTEncoding.Term.fst"
...
let fvs = (let _148_289 = (freevars t)
in (FStar_Util.remove_dups fv_eq _148_289))
in (
# 217 "FStar.SMTEncoding.Term.fst"
let _69_145 = (FStar_ST.op_Colon_Equals t.freevars (Some (fvs)))
in fvs))
end))

# 218 "FStar.SMTEncoding.Term.fst"
let qop_to_string : qop  ->  Prims.string = (fun _69_3 -> (match (_69_3) with
| Forall -> begin
"forall"
end
| Exists -> begin
...
# 225 "FStar.SMTEncoding.Term.fst"
let op_to_string : op  ->  Prims.string = (fun _69_4 -> (match (_69_4) with
| True -> begin
"true"
end
| False -> begin
...
# 247 "FStar.SMTEncoding.Term.fst"
let weightToSmt : Prims.int Prims.option  ->  Prims.string = (fun _69_5 -> (match (_69_5) with
| None -> begin
""
end
| Some (i) -> begin
(let _148_296 = (FStar_Util.string_of_int i)
in (FStar_Util.format1 ":weight %s\n" _148_296))
end))

# 251 "FStar.SMTEncoding.Term.fst"
let hash_of_term' : term'  ->  Prims.string = (fun t -> (match (t) with
| Integer (i) -> begin
i
end
| BoundV (i) -> begin
(let _148_299 = (FStar_Util.string_of_int i)
in (Prims.strcat "@" _148_299))
end
| FreeV (x) -> begin
(let _148_300 = (strSort (Prims.snd x))
in (Prims.strcat (Prims.strcat (Prims.fst x) ":") _148_300))
end
| App (op, tms) -> begin
(let _148_304 = (let _148_303 = (let _148_302 = (FStar_List.map (fun t -> t.hash) tms)
in (FStar_All.pipe_right _148_302 (FStar_String.concat " ")))
in (Prims.strcat (Prims.strcat "(" (op_to_string op)) _148_303))
in (Prims.strcat _148_304 ")"))
end
| Labeled (t, r1, r2) -> begin
(let _148_305 = (FStar_Range.string_of_range r2)
in (Prims.strcat (Prims.strcat t.hash r1) _148_305))
end
| Quant (qop, pats, wopt, sorts, body) -> begin
(let _148_313 = (let _148_306 = (FStar_List.map strSort sorts)
in (FStar_All.pipe_right _148_306 (FStar_String.concat " ")))
in (let _148_312 = (weightToSmt wopt)
in (let _148_311 = (let _148_310 = (FStar_All.pipe_right pats (FStar_List.map (fun pats -> (let _148_309 = (FStar_List.map (fun p -> p.hash) pats)
in (FStar_All.pipe_right _148_309 (FStar_String.concat " "))))))
in (FStar_All.pipe_right _148_310 (FStar_String.concat "; ")))
in (FStar_Util.format5 "(%s (%s)(! %s %s %s))" (qop_to_string qop) _148_313 body.hash _148_312 _148_311))))
end))

# 265 "FStar.SMTEncoding.Term.fst"
let __all_terms : term FStar_Util.smap FStar_ST.ref = (let _148_314 = (FStar_Util.smap_create 10000)
in (FStar_ST.alloc _148_314))

# 268 "FStar.SMTEncoding.Term.fst"
let all_terms : Prims.unit  ->  term FStar_Util.smap = (fun _69_202 -> (match (()) with
| () -> begin
...
in (match ((let _148_319 = (all_terms ())
in (FStar_Util.smap_try_find _148_319 key))) with
| Some (tm) -> begin
tm
end
| None -> begin
...
let tm = (let _148_320 = (FStar_Util.mk_ref None)
in {tm = t; hash = key; freevars = _148_320})
in (
# 276 "FStar.SMTEncoding.Term.fst"
let _69_209 = (let _148_321 = (all_terms ())
in (FStar_Util.smap_add _148_321 key tm))
in tm))
end)))
...
let mkInteger' : Prims.int  ->  term = (fun i -> (let _148_328 = (FStar_Util.string_of_int i)
in (mkInteger _148_328)))
...
let mkApp : (Prims.string * term Prims.list)  ->  term = (fun _69_219 -> (match (_69_219) with
| (s, args) -> begin
(mk (App ((Var (s), args))))
end))
...
| App (True, _69_223) -> begin
mkFalse
end
| App (False, _69_228) -> begin
mkTrue
end
| _69_232 -> begin
(mkApp' (Not, (t)::[]))
...
# 291 "FStar.SMTEncoding.Term.fst"
let mkAnd : (term * term)  ->  term = (fun _69_235 -> (match (_69_235) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
| (App (True, _69_238), _69_242) -> begin
t2
end
| (_69_245, App (True, _69_248)) -> begin
t1
end
...
| (_69_278, App (And, ts2)) -> begin
(mkApp' (And, (t1)::ts2))
end
| (App (And, ts1), _69_289) -> begin
...
# 300 "FStar.SMTEncoding.Term.fst"
let mkOr : (term * term)  ->  term = (fun _69_295 -> (match (_69_295) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
| ((App (True, _), _)) | ((_, App (True, _))) -> begin
mkTrue
end
| (App (False, _69_314), _69_318) -> begin
t2
end
| (_69_321, App (False, _69_324)) -> begin
t1
end
| (App (Or, ts1), App (Or, ts2)) -> begin
(mkApp' (Or, (FStar_List.append ts1 ts2)))
end
| (_69_338, App (Or, ts2)) -> begin
(mkApp' (Or, (t1)::ts2))
end
| (App (Or, ts1), _69_349) -> begin
...
let mkImp : (term * term)  ->  term = (fun _69_355 -> (match (_69_355) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
| (_69_357, App (True, _69_360)) -> begin
mkTrue
end
| (App (True, _69_366), _69_370) -> begin
t2
end
| (_69_373, App (Imp, t1'::t2'::[])) -> begin
(let _148_347 = (let _148_346 = (let _148_345 = (mkAnd (t1, t1'))
in (_148_345)::(t2')::[])
in (Imp, _148_346))
in (mkApp' _148_347))
end
...
let mk_bin_op : op  ->  (term * term)  ->  term = (fun op _69_386 -> (match (_69_386) with
| (t1, t2) -> begin
(mkApp' (op, (t1)::(t2)::[]))
end))
...
let mkITE : (term * term * term)  ->  term = (fun _69_391 -> (match (_69_391) with
| (t1, t2, t3) -> begin
(match ((t2.tm, t3.tm)) with
| (App (True, _69_394), App (True, _69_399)) -> begin
mkTrue
end
| (App (True, _69_405), _69_409) -> begin
(let _148_368 = (let _148_367 = (mkNot t1)
in (_148_367, t3))
in (mkImp _148_368))
end
| (_69_412, App (True, _69_415)) -> begin
(mkImp (t1, t2))
end
| (_69_420, _69_422) -> begin
(mkApp' (ITE, (t1)::(t2)::(t3)::[]))
end)
end))

# 334 "FStar.SMTEncoding.Term.fst"
...
let mkQuant : (qop * pat Prims.list Prims.list * Prims.int Prims.option * sort Prims.list * term)  ->  term = (fun _69_436 -> (match (_69_436) with
| (qop, pats, wopt, vars, body) -> begin
if ((FStar_List.length vars) = 0) then begin
body
end else begin
(match (body.tm) with
| App (True, _69_439) -> begin
body
end
| _69_443 -> begin
...
# 343 "FStar.SMTEncoding.Term.fst"
...
in (
# 353 "FStar.SMTEncoding.Term.fst"
...
| App (op, tms) -> begin
(let _148_386 = (let _148_385 = (FStar_List.map (aux ix) tms)
in (op, _148_385))
in (mkApp' _148_386))
end
| Labeled (t, r1, r2) -> begin
(let _148_389 = (let _148_388 = (let _148_387 = (aux ix t)
in (_148_387, r1, r2))
in Labeled (_148_388))
in (mk _148_389))
end
...
(
# 368 "FStar.SMTEncoding.Term.fst"
let n = (FStar_List.length vars)
in (let _148_392 = (let _148_391 = (FStar_All.pipe_right pats (FStar_List.map (FStar_List.map (aux (ix + n)))))
in (let _148_390 = (aux (ix + n) body)
in (qop, _148_391, wopt, vars, _148_390)))
in (mkQuant _148_392)))
end)
end))
in (aux 0 t)))))

# 371 "FStar.SMTEncoding.Term.fst"
...
| App (op, tms) -> begin
(let _148_402 = (let _148_401 = (FStar_List.map (aux shift) tms)
in (op, _148_401))
in (mkApp' _148_402))
end
| Labeled (t, r1, r2) -> begin
(let _148_405 = (let _148_404 = (let _148_403 = (aux shift t)
in (_148_403, r1, r2))
in Labeled (_148_404))
in (mk _148_405))
end
...
let shift = (shift + m)
in (let _148_408 = (let _148_407 = (FStar_All.pipe_right pats (FStar_List.map (FStar_List.map (aux shift))))
in (let _148_406 = (aux shift body)
in (qop, _148_407, wopt, vars, _148_406)))
in (mkQuant _148_408))))
end))
in (aux 0 t))))

# 388 "FStar.SMTEncoding.Term.fst"
let mkQuant' : (qop * term Prims.list Prims.list * Prims.int Prims.option * fv Prims.list * term)  ->  term = (fun _69_524 -> (match (_69_524) with
| (qop, pats, wopt, vars, body) -> begin
(let _148_414 = (let _148_413 = (FStar_All.pipe_right pats (FStar_List.map (FStar_List.map (abstr vars))))
in (let _148_412 = (FStar_List.map fv_sort vars)
in (let _148_411 = (abstr vars body)
in (qop, _148_413, wopt, _148_412, _148_411))))
in (mkQuant _148_414))
end))

# 390 "FStar.SMTEncoding.Term.fst"
let mkForall'' : (pat Prims.list Prims.list * Prims.int Prims.option * sort Prims.list * term)  ->  term = (fun _69_529 -> (match (_69_529) with
| (pats, wopt, sorts, body) -> begin
(mkQuant (Forall, pats, wopt, sorts, body))
end))

# 391 "FStar.SMTEncoding.Term.fst"
let mkForall' : (pat Prims.list Prims.list * Prims.int Prims.option * fvs * term)  ->  term = (fun _69_534 -> (match (_69_534) with
| (pats, wopt, vars, body) -> begin
(mkQuant' (Forall, pats, wopt, vars, body))
end))

# 392 "FStar.SMTEncoding.Term.fst"
let mkForall : (pat Prims.list Prims.list * fvs * term)  ->  term = (fun _69_538 -> (match (_69_538) with
| (pats, vars, body) -> begin
(mkQuant' (Forall, pats, None, vars, body))
end))

# 395 "FStar.SMTEncoding.Term.fst"
let mkExists : (pat Prims.list Prims.list * fvs * term)  ->  term = (fun _69_542 -> (match (_69_542) with
| (pats, vars, body) -> begin
(mkQuant' (Exists, pats, None, vars, body))
end))

# 396 "FStar.SMTEncoding.Term.fst"
let mkDefineFun : (Prims.string * (Prims.string * sort) Prims.list * sort * term * caption)  ->  decl = (fun _69_548 -> (match (_69_548) with
| (nm, vars, s, tm, c) -> begin
(let _148_427 = (let _148_426 = (FStar_List.map fv_sort vars)
in (let _148_425 = (abstr vars tm)
in (nm, _148_426, s, _148_425, c)))
in DefineFun (_148_427))
end))

# 399 "FStar.SMTEncoding.Term.fst"
let constr_id_of_sort : sort  ->  Prims.string = (fun sort -> (let _148_430 = (strSort sort)
in (FStar_Util.format1 "%s_constr_id" _148_430)))

# 400 "FStar.SMTEncoding.Term.fst"
let fresh_token : (Prims.string * sort)  ->  Prims.int  ->  decl = (fun _69_552 id -> (match (_69_552) with
| (tok_name, sort) -> begin
(let _148_443 = (let _148_442 = (let _148_441 = (let _148_440 = (mkInteger' id)
in (let _148_439 = (let _148_438 = (let _148_437 = (constr_id_of_sort sort)
in (let _148_436 = (let _148_435 = (mkApp (tok_name, []))
in (_148_435)::[])
in (_148_437, _148_436)))
in (mkApp _148_438))
in (_148_440, _148_439)))
in (mkEq _148_441))
in (_148_442, Some ("fresh token")))
in Assume (_148_443))
end))

# 402 "FStar.SMTEncoding.Term.fst"
let fresh_constructor : (Prims.string * sort Prims.list * sort * Prims.int)  ->  decl = (fun _69_558 -> (match (_69_558) with
...
let bvars = (FStar_All.pipe_right arg_sorts (FStar_List.mapi (fun i s -> (let _148_450 = (let _148_449 = (let _148_448 = (FStar_Util.string_of_int i)
in (Prims.strcat "x_" _148_448))
in (_148_449, s))
in (mkFreeV _148_450)))))
in (
...
let cid_app = (let _148_452 = (let _148_451 = (constr_id_of_sort sort)
in (_148_451, (capp)::[]))
in (mkApp _148_452))
in (let _148_458 = (let _148_457 = (let _148_456 = (let _148_455 = (let _148_454 = (let _148_453 = (mkInteger id)
in (_148_453, cid_app))
in (mkEq _148_454))
in (((capp)::[])::[], bvar_names, _148_455))
in (mkForall _148_456))
in (_148_457, Some ("Constructor distinct")))
in Assume (_148_458)))))))
end))

# 410 "FStar.SMTEncoding.Term.fst"
let injective_constructor : (Prims.string * (Prims.string * sort) Prims.list * sort)  ->  decls_t = (fun _69_569 -> (match (_69_569) with
...
let bvar_name = (fun i -> (let _148_463 = (FStar_Util.string_of_int i)
in (Prims.strcat "x_" _148_463)))
in (
# 415 "FStar.SMTEncoding.Term.fst"
let bvar_index = (fun i -> (n_bvars - (i + 1)))
in (
# 416 "FStar.SMTEncoding.Term.fst"
let bvar = (fun i s -> (let _148_471 = (let _148_470 = (bvar_name i)
in (_148_470, s))
in (mkFreeV _148_471)))
in (
# 417 "FStar.SMTEncoding.Term.fst"
let bvars = (FStar_All.pipe_right projectors (FStar_List.mapi (fun i _69_582 -> (match (_69_582) with
| (_69_580, s) -> begin
(bvar i s)
end))))
in (
...
let capp = (mkApp (name, bvars))
in (let _148_484 = (FStar_All.pipe_right projectors (FStar_List.mapi (fun i _69_589 -> (match (_69_589) with
| (name, s) -> begin
(
...
in (let _148_483 = (let _148_482 = (let _148_481 = (let _148_480 = (let _148_479 = (let _148_478 = (let _148_477 = (let _148_476 = (bvar i s)
in (cproj_app, _148_476))
in (mkEq _148_477))
in (((capp)::[])::[], bvar_names, _148_478))
in (mkForall _148_479))
in (_148_480, Some ("Projection inverse")))
in Assume (_148_481))
in (_148_482)::[])
in (proj_name)::_148_483)))
end))))
in (FStar_All.pipe_right _148_484 FStar_List.flatten)))))))))
end))

# 426 "FStar.SMTEncoding.Term.fst"
let constructor_to_decl : constructor_t  ->  decls_t = (fun _69_597 -> (match (_69_597) with
...
let cdecl = (let _148_488 = (let _148_487 = (FStar_All.pipe_right projectors (FStar_List.map Prims.snd))
in (name, _148_487, sort, Some ("Constructor")))
in DeclFun (_148_488))
in (
# 431 "FStar.SMTEncoding.Term.fst"
let cid = (let _148_490 = (let _148_489 = (FStar_All.pipe_right projectors (FStar_List.map Prims.snd))
in (name, _148_489, sort, id))
in (fresh_constructor _148_490))
in (
...
let disc_eq = (let _148_496 = (let _148_495 = (let _148_492 = (let _148_491 = (constr_id_of_sort sort)
in (_148_491, (xx)::[]))
in (mkApp _148_492))
in (let _148_494 = (let _148_493 = (FStar_Util.string_of_int id)
in (mkInteger _148_493))
in (_148_495, _148_494)))
in (mkEq _148_496))
in (
# 437 "FStar.SMTEncoding.Term.fst"
let proj_terms = (FStar_All.pipe_right projectors (FStar_List.map (fun _69_607 -> (match (_69_607) with
| (proj, s) -> begin
(mkApp (proj, (xx)::[]))
end))))
in (
# 438 "FStar.SMTEncoding.Term.fst"
let disc_inv_body = (let _148_499 = (let _148_498 = (mkApp (name, proj_terms))
in (xx, _148_498))
in (mkEq _148_499))
in (
...
in (let _148_506 = (let _148_502 = (let _148_501 = (let _148_500 = (FStar_Util.format1 "<start constructor %s>" name)
in Caption (_148_500))
in (_148_501)::(cdecl)::(cid)::projs)
in (FStar_List.append _148_502 ((disc)::[])))
in (let _148_505 = (let _148_504 = (let _148_503 = (FStar_Util.format1 "</end constructor %s>" name)
in Caption (_148_503))
in (_148_504)::[])
in (FStar_List.append _148_506 _148_505))))))))
end))
...
let _69_631 = (FStar_All.pipe_right sorts (FStar_List.fold_left (fun _69_619 s -> (match (_69_619) with
...
let nm = (let _148_515 = (FStar_Util.string_of_int n)
in (Prims.strcat prefix _148_515))
in (
# 459 "FStar.SMTEncoding.Term.fst"
let names = ((nm, s))::names
in (
# 460 "FStar.SMTEncoding.Term.fst"
let b = (let _148_516 = (strSort s)
in (FStar_Util.format2 "(%s %s)" nm _148_516))
in (names, (b)::binders, (n + 1))))))
end)) (outer_names, [], start)))
in (match (_69_631) with
| (names, binders, n) -> begin
...
let _69_636 = (name_binders_inner [] 0 sorts)
in (match (_69_636) with
| (names, binders, n) -> begin
...
| BoundV (i) -> begin
(let _148_527 = (FStar_List.nth names i)
in (FStar_All.pipe_right _148_527 Prims.fst))
end
| FreeV (x) -> begin
(Prims.fst x)
end
...
| App (op, tms) -> begin
(let _148_529 = (let _148_528 = (FStar_List.map (aux n names) tms)
in (FStar_All.pipe_right _148_528 (FStar_String.concat "\n")))
in (FStar_Util.format2 "(%s %s)" (op_to_string op) _148_529))
end
| Labeled (t, _69_658, _69_660) -> begin
(aux n names t)
end
| Quant (qop, pats, wopt, sorts, body) -> begin
(
# 479 "FStar.SMTEncoding.Term.fst"
let _69_673 = (name_binders_inner names n sorts)
...
| _69_679 -> begin
(let _148_535 = (FStar_All.pipe_right pats (FStar_List.map (fun pats -> (let _148_534 = (let _148_533 = (FStar_List.map (fun p -> (let _148_532 = (aux n names p)
in (FStar_Util.format1 "%s" _148_532))) pats)
in (FStar_String.concat " " _148_533))
in (FStar_Util.format1 "\n:pattern (%s)" _148_534)))))
in (FStar_All.pipe_right _148_535 (FStar_String.concat "\n")))
end)
in (match ((pats, wopt)) with
| (([]::[], None)) | (([], None)) -> begin
(let _148_536 = (aux n names body)
in (FStar_Util.format3 "(%s (%s)\n %s);;no pats\n" (qop_to_string qop) binders _148_536))
end
| _69_691 -> begin
(let _148_538 = (aux n names body)
in (let _148_537 = (weightToSmt wopt)
in (FStar_Util.format5 "(%s (%s)\n (! %s\n %s %s))" (qop_to_string qop) binders _148_538 _148_537 pats_str)))
end)))
end))
end))
in (aux 0 [] t)))

# 490 "FStar.SMTEncoding.Term.fst"
let caption_to_string : Prims.string Prims.option  ->  Prims.string = (fun _69_6 -> (match (_69_6) with
| None -> begin
""
end
| Some (c) -> begin
...
let _69_705 = (match ((FStar_Util.splitlines c)) with
| [] -> begin
(FStar_All.failwith "Impossible")
end
| hd::[] -> begin
...
# 500 "FStar.SMTEncoding.Term.fst"
let rec declToSmt : Prims.string  ->  decl  ->  Prims.string = (fun z3options decl -> (match (decl) with
| DefPrelude -> begin
(mkPrelude z3options)
end
| Caption (c) -> begin
(let _148_547 = (FStar_All.pipe_right (FStar_Util.splitlines c) (fun _69_7 -> (match (_69_7) with
| [] -> begin
""
end
| h::t -> begin
...
(
# 507 "FStar.SMTEncoding.Term.fst"
let l = (FStar_List.map strSort argsorts)
in (let _148_549 = (caption_to_string c)
in (let _148_548 = (strSort retsort)
in (FStar_Util.format4 "%s(declare-fun %s (%s) %s)" _148_549 f (FStar_String.concat " " l) _148_548))))
end
| DefineFun (f, arg_sorts, retsort, body, c) -> begin
(
# 510 "FStar.SMTEncoding.Term.fst"
let _69_732 = (name_binders arg_sorts)
in (match (_69_732) with
| (names, binders) -> begin
(
# 511 "FStar.SMTEncoding.Term.fst"
let body = (let _148_550 = (FStar_List.map mkFreeV names)
in (inst _148_550 body))
in (let _148_553 = (caption_to_string c)
in (let _148_552 = (strSort retsort)
in (let _148_551 = (termToSmt body)
in (FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" _148_553 f (FStar_String.concat " " binders) _148_552 _148_551)))))
end))
end
| Assume (t, c) -> begin
(let _148_555 = (caption_to_string c)
in (let _148_554 = (termToSmt t)
in (FStar_Util.format2 "%s(assert %s)" _148_555 _148_554)))
end
| Eval (t) -> begin
(let _148_556 = (termToSmt t)
in (FStar_Util.format1 "(eval %s)" _148_556))
end
| Echo (s) -> begin
...
# 524 "FStar.SMTEncoding.Term.fst"
...
in (
# 580 "FStar.SMTEncoding.Term.fst"
let bcons = (let _148_559 = (let _148_558 = (FStar_All.pipe_right constrs (FStar_List.collect constructor_to_decl))
in (FStar_All.pipe_right _148_558 (FStar_List.map (declToSmt z3options))))
in (FStar_All.pipe_right _148_559 (FStar_String.concat "\n")))
in (
...
let mk_Term_uvar : Prims.int  ->  term = (fun i -> (let _148_568 = (let _148_567 = (let _148_566 = (mkInteger' i)
in (_148_566)::[])
in ("Tm_uvar", _148_567))
in (mkApp _148_568)))
...
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_Equality"), _69_790::t1::t2::[]); hash = _69_784; freevars = _69_782}::[]) -> begin
(mkEq (t1, t2))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_disEquality"), _69_809::t1::t2::[]); hash = _69_803; freevars = _69_801}::[]) -> begin
(let _148_597 = (mkEq (t1, t2))
in (mkNot _148_597))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_LessThanOrEqual"), t1::t2::[]); hash = _69_822; freevars = _69_820}::[]) -> begin
(let _148_600 = (let _148_599 = (unboxInt t1)
in (let _148_598 = (unboxInt t2)
in (_148_599, _148_598)))
in (mkLTE _148_600))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_LessThan"), t1::t2::[]); hash = _69_839; freevars = _69_837}::[]) -> begin
(let _148_603 = (let _148_602 = (unboxInt t1)
in (let _148_601 = (unboxInt t2)
in (_148_602, _148_601)))
in (mkLT _148_603))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_GreaterThanOrEqual"), t1::t2::[]); hash = _69_856; freevars = _69_854}::[]) -> begin
(let _148_606 = (let _148_605 = (unboxInt t1)
in (let _148_604 = (unboxInt t2)
in (_148_605, _148_604)))
in (mkGTE _148_606))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_GreaterThan"), t1::t2::[]); hash = _69_873; freevars = _69_871}::[]) -> begin
(let _148_609 = (let _148_608 = (unboxInt t1)
in (let _148_607 = (unboxInt t2)
in (_148_608, _148_607)))
in (mkGT _148_609))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_AmpAmp"), t1::t2::[]); hash = _69_890; freevars = _69_888}::[]) -> begin
(let _148_612 = (let _148_611 = (unboxBool t1)
in (let _148_610 = (unboxBool t2)
in (_148_611, _148_610)))
in (mkAnd _148_612))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_BarBar"), t1::t2::[]); hash = _69_907; freevars = _69_905}::[]) -> begin
(let _148_615 = (let _148_614 = (unboxBool t1)
in (let _148_613 = (unboxBool t2)
in (_148_614, _148_613)))
in (mkOr _148_615))
end
| App (Var ("Prims.b2t"), {tm = App (Var ("Prims.op_Negation"), t::[]); hash = _69_924; freevars = _69_922}::[]) -> begin
(let _148_616 = (unboxBool t)
in (mkNot _148_616))
end
...
let mk_String_const : Prims.int  ->  term = (fun i -> (let _148_659 = (let _148_658 = (let _148_657 = (mkInteger' i)
in (_148_657)::[])
in ("String_const", _148_658))
in (mkApp _148_659)))

# 644 "FStar.SMTEncoding.Term.fst"
let mk_Precedes : term  ->  term  ->  term = (fun x1 x2 -> (let _148_664 = (mkApp ("Precedes", (x1)::(x2)::[]))
in (FStar_All.pipe_right _148_664 mk_Valid)))
...
(let _148_673 = (let _148_672 = (let _148_671 = (n_fuel (n - 1))
in (_148_671)::[])
in ("SFuel", _148_672))
in (mkApp _148_673))
end)
...
# 664 "FStar.SMTEncoding.Term.fst"
...
# 668 "FStar.SMTEncoding.Term.fst"
let rec print_smt_term : term  ->  Prims.string = (fun t -> (match (t.tm) with
| Integer (n) -> begin
(FStar_Util.format1 "(Integer %s)" n)
end
| BoundV (n) -> begin
(let _148_695 = (FStar_Util.string_of_int n)
in (FStar_Util.format1 "(BoundV %s)" _148_695))
end
| FreeV (fv) -> begin
(FStar_Util.format1 "(FreeV %s)" (Prims.fst fv))
end
| App (op, l) -> begin
(let _148_696 = (print_smt_term_list l)
in (FStar_Util.format2 "(%s %s)" (op_to_string op) _148_696))
end
| Labeled (t, r1, r2) -> begin
(let _148_697 = (print_smt_term t)
in (FStar_Util.format2 "(Labeled \'%s\' %s)" r1 _148_697))
end
| Quant (qop, l, _69_1024, _69_1026, t) -> begin
(let _148_699 = (print_smt_term_list_list l)
in (let _148_698 = (print_smt_term t)
in (FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) _148_699 _148_698)))
end))
and print_smt_term_list : term Prims.list  ->  Prims.string = (fun l -> (let _148_701 = (FStar_List.map print_smt_term l)
in (FStar_All.pipe_right _148_701 (FStar_String.concat " "))))
and print_smt_term_list_list : term Prims.list Prims.list  ->  Prims.string = (fun l -> (FStar_List.fold_left (fun s l -> (let _148_706 = (let _148_705 = (print_smt_term_list l)
in (Prims.strcat (Prims.strcat s "; [ ") _148_705))
in (Prims.strcat _148_706 " ] "))) "" l))
