...
let freevar_sort : term  ->  sort = (fun uu___82_741 -> (match (uu___82_741) with
...
let fv_of_term : term  ->  fv = (fun uu___83_754 -> (match (uu___83_754) with
...
let qop_to_string : qop  ->  Prims.string = (fun uu___84_847 -> (match (uu___84_847) with
| Forall -> begin
"forall"
end
| Exists -> begin
...
let op_to_string : op  ->  Prims.string = (fun uu___85_850 -> (match (uu___85_850) with
| TrueOp -> begin
"true"
end
| FalseOp -> begin
...
let weightToSmt : Prims.int Prims.option  ->  Prims.string = (fun uu___86_855 -> (match (uu___86_855) with
| None -> begin
""
end
| Some (i) -> begin
...
let uu____921 = (FStar_All.pipe_right pats (FStar_List.map (fun pats -> (

let uu____929 = (FStar_List.map hash_of_term pats)
...
let uu____1423 = (FStar_ST.read t.freevars)
...
let uu____1471 = (aux ix t)
in ((uu____1471), (r1), (r2)))
in Labeled (uu____1467))
in (mk uu____1466 t.rng))
end
| LblPos (t, r) -> begin
(
...
let uu____1478 = (aux ix t)
in ((uu____1478), (r)))
in LblPos (uu____1475))
in (mk uu____1474 t.rng))
end
...
let uu____1504 = (FStar_All.pipe_right pats (FStar_List.map (FStar_List.map (aux (ix + n)))))
in (

let uu____1515 = (aux (ix + n) body)
in ((qop), (uu____1504), (wopt), (vars), (uu____1515))))
in (mkQuant uu____1494 t.rng)))
end
| Let (es, body) -> begin
(
...
let uu____1547 = (aux ix e)
in (uu____1547)::l)
in (((ix + (Prims.parse_int "1"))), (uu____1545)))
end)) ((ix), ([])) es)
...
let uu____1558 = (aux ix body)
in (((FStar_List.rev es_rev)), (uu____1558)))
in (mkLet uu____1554 t.rng))
end))
end)
end)))
...
let uu____1593 = (FStar_List.map (aux shift) tms)
in ((op), (uu____1593)))
in (mkApp' uu____1589 t.rng))
end
| Labeled (t, r1, r2) -> begin
(
...
let uu____1604 = (aux shift t)
in ((uu____1604), (r1), (r2)))
in Labeled (uu____1600))
in (mk uu____1599 t.rng))
end
| LblPos (t, r) -> begin
(
...
let uu____1611 = (aux shift t)
in ((uu____1611), (r)))
in LblPos (uu____1608))
in (mk uu____1607 t.rng))
end
| Quant (qop, pats, wopt, vars, body) -> begin
(
...
let uu____1640 = (FStar_All.pipe_right pats (FStar_List.map (FStar_List.map (aux shift))))
in (

let uu____1649 = (aux shift body)
in ((qop), (uu____1640), (wopt), (vars), (uu____1649))))
in (mkQuant uu____1630 t.rng))))
end
| Let (es, body) -> begin
(
...
let uu____1690 = (aux shift body)
in (((FStar_List.rev es_rev)), (uu____1690)))
in (mkLet uu____1686 t.rng))
end))
end))
in (aux (Prims.parse_int "0") t)))))
...
let uu____2083 = (mkInteger id norng)
...
| (uu____2238, sort, uu____2240) -> begin
sort
end))))
in (
...
let ex_vars = (FStar_List.flatten ex_vars)
in (
...
| uu____2377 -> begin
(mkExists (([]), (ex_vars), (disc_inv_body)) norng)
end)
in (
...
let prefix = (match (prefix_opt) with
| None -> begin
prefix
end
| Some (p) -> begin
(Prims.strcat p prefix)
end)
in (

let nm = (

let uu____2494 = (FStar_Util.string_of_int n)
in (Prims.strcat prefix uu____2494))
in (
...
let uu____2645 = (FStar_List.map (aux n names) tms)
in (FStar_All.pipe_right uu____2645 (FStar_String.concat "\n")))
in (FStar_Util.format2 "(%s %s)" (op_to_string op) uu____2644))
end
| Labeled (t, uu____2649, uu____2650) -> begin
(aux n names t)
end
| LblPos (t, s) -> begin
(

let uu____2653 = (aux n names t)
in (FStar_Util.format2 "(! %s :lblpos %s)" uu____2653 s))
end
| Quant (qop, pats, wopt, sorts, body) -> begin
(

let uu____2667 = (name_binders_inner None names n sorts)
...
let uu____2698 = (FStar_All.pipe_right pats (FStar_List.map (fun pats -> (

let uu____2706 = (

let uu____2707 = (FStar_List.map (fun p -> (

let uu____2710 = (aux n names p)
in (FStar_Util.format1 "%s" uu____2710))) pats)
...
let uu____2724 = (aux n names body)
in (FStar_Util.format3 "(%s (%s)\n %s);;no pats\n" (qop_to_string qop) binders uu____2724))
end
| uu____2725 -> begin
(

let uu____2731 = (aux n names body)
in (

let uu____2732 = (weightToSmt wopt)
in (FStar_Util.format5 "(%s (%s)\n (! %s\n %s %s))" (qop_to_string qop) binders uu____2731 uu____2732 pats_str)))
end))))
end))
end
| Let (es, body) -> begin
...
let uu____2806 = (aux n names body)
...
let uu____2812 = (FStar_Range.string_of_range t.rng)
in (

let uu____2813 = (FStar_Range.string_of_use_range t.rng)
...
let caption_to_string : Prims.string Prims.option  ->  Prims.string = (fun uu___87_2818 -> (match (uu___87_2818) with
| None -> begin
""
end
| Some (c) -> begin
...
let uu____2847 = (FStar_All.pipe_right (FStar_Util.splitlines c) (fun uu___88_2849 -> (match (uu___88_2849) with
| [] -> begin
""
end
| (h)::t -> begin
...
in (FStar_Util.format3 "%s(assert (!\n%s\n:named %s))" uu____2903 uu____2904 (escape n))))
end
...
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_Negation"), (t)::[]); freevars = uu____3318; rng = uu____3319})::[]) -> begin
(

let uu____3326 = (unboxBool t)
in (mkNot uu____3326 t.rng))
end
| App (Var ("Prims.b2t"), (t1)::[]) -> begin
(

let uu___89_3329 = (unboxBool t1)
in {tm = uu___89_3329.tm; freevars = uu___89_3329.freevars; rng = t.rng})
end
| uu____3332 -> begin
(mkApp (("Valid"), ((t)::[])) t.rng)
end))
...
let uu____3458 = (n_fuel (n - (Prims.parse_int "1")))
in (uu____3458)::[])
in (("SFuel"), (uu____3456)))
...
let uu____3545 = (FStar_Util.string_of_int n)
...
let uu____3557 = (print_smt_term t)
in (FStar_Util.format2 "(Labeled \'%s\' %s)" r1 uu____3557))
end
| LblPos (t, s) -> begin
(

let uu____3560 = (print_smt_term t)
in (FStar_Util.format2 "(LblPos %s %s)" s uu____3560))
end
| Quant (qop, l, uu____3563, uu____3564, t) -> begin
(

let uu____3574 = (print_smt_term_list_list l)
in (

let uu____3575 = (print_smt_term t)
...
let uu____3596 = (print_smt_term_list l)
