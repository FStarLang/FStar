...
let (mk_Precedes : term -> term -> term -> term -> FStar_Range.range -> term)
  ...
  fun x1 ->
    fun x2 ->
      fun x3 ->
        fun x4 -> fun r -> mkApp ("Prims.precedes", [x1; x2; x3; x4]) r
let (mk_lex_t : FStar_Range.range -> term) =
  fun r -> mkApp ("Prims.lex_t", []) r
let (mk_LexCons : term -> term -> term -> FStar_Range.range -> term) =
  fun x1 -> fun x2 -> fun x3 -> fun r -> mkApp ("LexCons", [x1; x2; x3]) r
let (mk_LexTop : FStar_Range.range -> term) = fun r -> mkApp ("LexTop", []) r
