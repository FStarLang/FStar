...
let freevar_sort : term  ->  sort = (fun uu___107_1378 -> (match (uu___107_1378) with
...
let fv_of_term : term  ->  fv = (fun uu___108_1398 -> (match (uu___108_1398) with
...
let qop_to_string : qop  ->  Prims.string = (fun uu___109_1668 -> (match (uu___109_1668) with
| Forall -> begin
"forall"
end
| Exists -> begin
...
let op_to_string : op  ->  Prims.string = (fun uu___110_1672 -> (match (uu___110_1672) with
| TrueOp -> begin
"true"
end
| FalseOp -> begin
...
let weightToSmt : Prims.int FStar_Pervasives_Native.option  ->  Prims.string = (fun uu___111_1683 -> (match (uu___111_1683) with
| FStar_Pervasives_Native.None -> begin
""
end
...
let uu____4992 = (FStar_Util.string_of_int n1)
in (FStar_Util.format2 "%s.%s" enclosing_name uu____4992))
end);
))))
in (
...
| App (Var ("Prims.guard_free"), ({tm = BoundV (uu____5034); freevars = uu____5035; rng = uu____5036})::[]) -> begin
tm
end
...
let uu____5090 = (FStar_List.nth names1 i)
in (FStar_All.pipe_right uu____5090 FStar_Pervasives_Native.fst))
end
| FreeV (x) -> begin
...
let uu____5105 = (op_to_string op)
in (

let uu____5106 = (

let uu____5107 = (FStar_List.map (aux1 n1 names1) tms)
in (FStar_All.pipe_right uu____5107 (FStar_String.concat "\n")))
in (FStar_Util.format2 "(%s %s)" uu____5105 uu____5106)))
end
| Labeled (t2, uu____5113, uu____5114) -> begin
(aux1 n1 names1 t2)
end
| LblPos (t2, s) -> begin
(

let uu____5117 = (aux1 n1 names1 t2)
in (FStar_Util.format2 "(! %s :lblpos %s)" uu____5117 s))
end
| Quant (qop, pats, wopt, sorts, body) -> begin
(
...
let uu____5140 = (name_binders_inner FStar_Pervasives_Native.None names1 n1 sorts)
...
let uu____5194 = (FStar_All.pipe_right pats1 (FStar_List.map (fun pats2 -> (

let uu____5210 = (

let uu____5211 = (FStar_List.map (fun p -> (

let uu____5217 = (aux1 n2 names2 p)
in (FStar_Util.format1 "%s" uu____5217))) pats2)
in (FStar_String.concat " " uu____5211))
in (FStar_Util.format1 "\n:pattern (%s)" uu____5210)))))
in (FStar_All.pipe_right uu____5194 (FStar_String.concat "\n")))
end)
in (
...
let uu____5229 = (aux1 n2 names2 body)
in (

let uu____5230 = (

let uu____5233 = (weightToSmt wopt)
in (uu____5233)::(pats_str)::(qid)::[])
in (uu____5229)::uu____5230))
in (binders1)::uu____5226)
in ((qop_to_string qop))::uu____5223)
...
let uu____5240 = (FStar_List.fold_left (fun uu____5277 e -> (match (uu____5277) with
| (names0, binders, n0) -> begin
(

let nm = (

let uu____5327 = (FStar_Util.string_of_int n0)
in (Prims.strcat "@lb" uu____5327))
in (
...
let uu____5340 = (aux1 n1 names1 e)
in (FStar_Util.format2 "(%s %s)" nm uu____5340))
...
let uu____5372 = (aux1 n2 names2 body)
in (FStar_Util.format2 "(let (%s)\n%s)" (FStar_String.concat " " binders) uu____5372))
end))
end)))
...
let uu____5380 = (FStar_Range.string_of_range t1.rng)
in (

let uu____5381 = (FStar_Range.string_of_use_range t1.rng)
in (FStar_Util.format3 "\n;; def=%s; use=%s\n%s\n" uu____5380 uu____5381 s)))
end
| uu____5382 -> begin
s
end)))
...
let caption_to_string : Prims.string FStar_Pervasives_Native.option  ->  Prims.string = (fun uu___112_5388 -> (match (uu___112_5388) with
| FStar_Pervasives_Native.None -> begin
""
end
...
let uu____5392 = (match ((FStar_Util.splitlines c)) with
| [] -> begin
(failwith "Impossible")
end
| (hd1)::[] -> begin
...
let uu____5436 = (FStar_Options.log_queries ())
in (match (uu____5436) with
| true -> begin
(

let uu____5437 = (FStar_All.pipe_right (FStar_Util.splitlines c) (fun uu___113_5441 -> (match (uu___113_5441) with
| [] -> begin
""
end
| (h)::t -> begin
...
let uu____5460 = (caption_to_string c)
in (

let uu____5461 = (strSort retsort)
in (FStar_Util.format4 "%s(declare-fun %s (%s) %s)" uu____5460 f (FStar_String.concat " " l) uu____5461))))
end
| DefineFun (f, arg_sorts, retsort, body, c) -> begin
(

let uu____5471 = (name_macro_binders arg_sorts)
...
let uu____5503 = (FStar_List.map (fun x -> (mkFreeV x norng)) names1)
in (inst uu____5503 body))
in (

let uu____5516 = (caption_to_string c)
in (

let uu____5517 = (strSort retsort)
in (

let uu____5518 = (termToSmt print_ranges (escape f) body1)
in (FStar_Util.format5 "%s(define-fun %s (%s) %s\n %s)" uu____5516 f (FStar_String.concat " " binders) uu____5517 uu____5518)))))
end))
end
| Assume (a) -> begin
(

let fact_ids_to_string = (fun ids -> (FStar_All.pipe_right ids (FStar_List.map (fun uu___114_5536 -> (match (uu___114_5536) with
| Name (n1) -> begin
...
let uu____5541 = (FStar_Options.log_queries ())
...
let uu____5543 = (fact_ids_to_string a.assumption_fact_ids)
in (FStar_String.concat "; " uu____5543))
...
let uu____5548 = (caption_to_string a.assumption_caption)
in (

let uu____5549 = (termToSmt print_ranges n1 a.assumption_term)
in (FStar_Util.format4 "%s%s(assert (! %s\n:named %s))" uu____5548 fids uu____5549 n1))))))
end
| Eval (t) -> begin
(

let uu____5551 = (termToSmt print_ranges "eval" t)
in (FStar_Util.format1 "(eval %s)" uu____5551))
end
| Echo (s) -> begin
(FStar_Util.format1 "(echo \"%s\")" s)
end
| RetainAssumptions (uu____5553) -> begin
""
end
| CheckSat -> begin
"(echo \"<result>\")\n(check-sat)\n(echo \"</result>\")"
...
let uu____5885 = (FStar_All.pipe_right constrs (FStar_List.collect constructor_to_decl))
in (FStar_All.pipe_right uu____5885 (FStar_List.map (declToSmt z3options))))
in (FStar_All.pipe_right uu____5882 (FStar_String.concat "\n")))
in (
...
let uu____5921 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.fst uu____5921))
in (
...
let uu____5943 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.snd uu____5943))
in ((uu____5942), (BitVec_sort (sz)), (true)))
in (uu____5935)::[])
in ((uu____5920), (uu____5926), (Term_sort), (((Prims.parse_int "12") + sz)), (true))))
in (FStar_All.pipe_right uu____5901 constructor_to_decl)))
...
let uu____6022 = (mkInteger' i norng)
in (uu____6022)::[])
in (("Tm_uvar"), (uu____6019)))
in (mkApp uu____6012 r)))
...
let uu____6062 = (FStar_Options.smtencoding_elim_box ())
in (elim_box uu____6062 u v1 t)))
...
let uu____6096 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.fst uu____6096))
in (

let uu____6101 = (

let uu____6102 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.snd uu____6102))
in (elim_box true uu____6095 uu____6101 t))))
...
let uu____6116 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.snd uu____6116))
in (

let uu____6121 = (

let uu____6122 = (boxBitVecFun sz)
in (FStar_Pervasives_Native.fst uu____6122))
in (elim_box true uu____6115 uu____6121 t))))
...
let uu____6162 = (FStar_Util.string_of_int n1)
in (FStar_Util.format1 "(BoundV %s)" uu____6162))
end
| FreeV (fv) -> begin
...
let uu____6174 = (op_to_string op)
in (

let uu____6175 = (print_smt_term_list l)
in (FStar_Util.format2 "(%s %s)" uu____6174 uu____6175)))
end
| Labeled (t1, r1, r2) -> begin
(

let uu____6179 = (print_smt_term t1)
in (FStar_Util.format2 "(Labeled \'%s\' %s)" r1 uu____6179))
end
| LblPos (t1, s) -> begin
(

let uu____6182 = (print_smt_term t1)
in (FStar_Util.format2 "(LblPos %s %s)" s uu____6182))
end
| Quant (qop, l, uu____6185, uu____6186, t1) -> begin
(

let uu____6204 = (print_smt_term_list_list l)
in (

let uu____6205 = (print_smt_term t1)
in (FStar_Util.format3 "(%s %s %s)" (qop_to_string qop) uu____6204 uu____6205)))
end
| Let (es, body) -> begin
(

let uu____6212 = (print_smt_term_list es)
in (

let uu____6213 = (print_smt_term body)
in (FStar_Util.format2 "(let %s %s)" uu____6212 uu____6213)))
end))
and print_smt_term_list : term Prims.list  ->  Prims.string = (fun l -> (

let uu____6217 = (FStar_List.map print_smt_term l)
in (FStar_All.pipe_right uu____6217 (FStar_String.concat " "))))
...
let uu____6238 = (print_smt_term_list l1)
in (Prims.strcat uu____6238 " ] "))
in (Prims.strcat "; [ " uu____6237))
in (Prims.strcat s uu____6236))) "" l))
...
let uu____6254 = (FStar_Util.int_of_string n1)
in FStar_Pervasives_Native.Some (uu____6254))
end
| uu____6255 -> begin
FStar_Pervasives_Native.None
end)
...
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_Equality"), (uu____6267)::(t1)::(t2)::[]); freevars = uu____6270; rng = uu____6271})::[]) -> begin
(mkEq ((t1), (t2)) t.rng)
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_disEquality"), (uu____6284)::(t1)::(t2)::[]); freevars = uu____6287; rng = uu____6288})::[]) -> begin
(

let uu____6301 = (mkEq ((t1), (t2)) norng)
in (mkNot uu____6301 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_LessThanOrEqual"), (t1)::(t2)::[]); freevars = uu____6304; rng = uu____6305})::[]) -> begin
(

let uu____6318 = (

let uu____6323 = (unboxInt t1)
in (

let uu____6324 = (unboxInt t2)
in ((uu____6323), (uu____6324))))
in (mkLTE uu____6318 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_LessThan"), (t1)::(t2)::[]); freevars = uu____6327; rng = uu____6328})::[]) -> begin
(

let uu____6341 = (

let uu____6346 = (unboxInt t1)
in (

let uu____6347 = (unboxInt t2)
in ((uu____6346), (uu____6347))))
in (mkLT uu____6341 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_GreaterThanOrEqual"), (t1)::(t2)::[]); freevars = uu____6350; rng = uu____6351})::[]) -> begin
(

let uu____6364 = (

let uu____6369 = (unboxInt t1)
in (

let uu____6370 = (unboxInt t2)
in ((uu____6369), (uu____6370))))
in (mkGTE uu____6364 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_GreaterThan"), (t1)::(t2)::[]); freevars = uu____6373; rng = uu____6374})::[]) -> begin
(

let uu____6387 = (

let uu____6392 = (unboxInt t1)
in (

let uu____6393 = (unboxInt t2)
in ((uu____6392), (uu____6393))))
in (mkGT uu____6387 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_AmpAmp"), (t1)::(t2)::[]); freevars = uu____6396; rng = uu____6397})::[]) -> begin
(

let uu____6410 = (

let uu____6415 = (unboxBool t1)
in (

let uu____6416 = (unboxBool t2)
in ((uu____6415), (uu____6416))))
in (mkAnd uu____6410 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_BarBar"), (t1)::(t2)::[]); freevars = uu____6419; rng = uu____6420})::[]) -> begin
(

let uu____6433 = (

let uu____6438 = (unboxBool t1)
in (

let uu____6439 = (unboxBool t2)
in ((uu____6438), (uu____6439))))
in (mkOr uu____6433 t.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("Prims.op_Negation"), (t1)::[]); freevars = uu____6441; rng = uu____6442})::[]) -> begin
(

let uu____6455 = (unboxBool t1)
in (mkNot uu____6455 t1.rng))
end
| App (Var ("Prims.b2t"), ({tm = App (Var ("FStar.BV.bvult"), (t0)::(t1)::(t2)::[]); freevars = uu____6459; rng = uu____6460})::[]) when (

let uu____6473 = (getBoxedInteger t0)
in (FStar_Util.is_some uu____6473)) -> begin
(

let sz = (

let uu____6477 = (getBoxedInteger t0)
...
let uu____6489 = (unboxBitVec sz t1)
in (

let uu____6490 = (unboxBitVec sz t2)
in ((uu____6489), (uu____6490))))
in (mkBvUlt uu____6484 t.rng)))
end
| App (Var ("Prims.equals"), (uu____6491)::({tm = App (Var ("FStar.BV.bvult"), (t0)::(t1)::(t2)::[]); freevars = uu____6495; rng = uu____6496})::(uu____6497)::[]) when (

let uu____6510 = (getBoxedInteger t0)
in (FStar_Util.is_some uu____6510)) -> begin
(

let sz = (

let uu____6514 = (getBoxedInteger t0)
...
let uu____6526 = (unboxBitVec sz t1)
in (

let uu____6527 = (unboxBitVec sz t2)
in ((uu____6526), (uu____6527))))
in (mkBvUlt uu____6521 t.rng)))
end
| App (Var ("Prims.b2t"), (t1)::[]) -> begin
(

let uu___115_6531 = (unboxBool t1)
in {tm = uu___115_6531.tm; freevars = uu___115_6531.freevars; rng = t.rng})
end
| uu____6532 -> begin
(mkApp (("Valid"), ((t)::[])) t.rng)
end))
...
let uu____6573 = (FStar_Options.unthrottle_inductives ())
...
let uu____6674 = (mkInteger' i norng)
in (uu____6674)::[])
in (("FString_const"), (uu____6671)))
in (mkApp uu____6664 r)))


let mk_Precedes : term  ->  term  ->  FStar_Range.range  ->  term = (fun x1 x2 r -> (

let uu____6689 = (mkApp (("Precedes"), ((x1)::(x2)::[])) r)
in (FStar_All.pipe_right uu____6689 mk_Valid)))
...
let uu____6723 = (n_fuel (n1 - (Prims.parse_int "1")))
in (uu____6723)::[])
in (("SFuel"), (uu____6720)))
in (mkApp uu____6713 norng))
end))
...
let uu____6760 = (mkAnd ((p11), (p21)) r)
in FStar_Pervasives_Native.Some (uu____6760))
end
...
let uu____6817 = (mkTrue r)
in (FStar_List.fold_right (fun p1 p2 -> (mkAnd ((p1), (p2)) r)) l uu____6817)))


let mk_or_l : term Prims.list  ->  FStar_Range.range  ->  term = (fun l r -> (

let uu____6834 = (mkFalse r)
in (FStar_List.fold_right (fun p1 p2 -> (mkOr ((p1), (p2)) r)) l uu____6834)))


let mk_haseq : term  ->  term = (fun t -> (

let uu____6843 = (mkApp (("Prims.hasEq"), ((t)::[])) t.rng)
in (mk_Valid uu____6843)))
