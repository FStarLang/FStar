...
let (mk_Valid : term -> term) =
  fun t  ->
    ...
    | App (Var "Prims.b2t",t1::[]) ->
        let uu___56_7124 = unboxBool t1  in
        {
          tm = (uu___56_7124.tm);
          freevars = (uu___56_7124.freevars);
          rng = (t.rng)
        }
...
let (mk_id_wrapper : term -> term) =
  fun t  -> mkApp ("__id_wrapper", [t]) t.rng 
let (mk_String_const : Prims.int -> FStar_Range.range -> term) =
  fun i  ->
    fun r  ->
      let uu____7301 =
        let uu____7308 = let uu____7311 = mkInteger' i norng  in [uu____7311]
           in
        ("FString_const", uu____7308)  in
      mkApp uu____7301 r
...
let (mk_Precedes : term -> term -> FStar_Range.range -> term) =
  fun x1  ->
    fun x2  ->
      fun r  ->
        let uu____7329 = mkApp ("Precedes", [x1; x2]) r  in
        FStar_All.pipe_right uu____7329 mk_Valid
...
let rec (n_fuel : Prims.int -> term) =
  fun n1  ->
    ...
    then mkApp ("ZFuel", []) norng
      ...
      (let uu____7357 =
         let uu____7364 =
           let uu____7367 = n_fuel (n1 - (Prims.parse_int "1"))  in
           [uu____7367]  in
         ("SFuel", uu____7364)  in
       mkApp uu____7357 norng)
    ...
    fun p2  ->
      fun r  ->
        ...
        | (FStar_Pervasives_Native.Some p11,FStar_Pervasives_Native.Some p21)
            ->
            let uu____7407 = mkAnd (p11, p21) r  in
...
let (mk_or_l : term Prims.list -> FStar_Range.range -> term) =
  fun l  ->
    fun r  ->
      let uu____7487 = mkFalse r  in
      FStar_List.fold_right (fun p1  -> fun p2  -> mkOr (p1, p2) r) l
        uu____7487
  
let (mk_haseq : term -> term) =
  fun t  ->
    let uu____7497 = mkApp ("Prims.hasEq", [t]) t.rng  in mk_Valid uu____7497
