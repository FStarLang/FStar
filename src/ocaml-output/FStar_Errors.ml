...
let lookup_error :
  ...
  fun settings  ->
    fun e  ->
      let uu___ =
        FStar_Util.try_find
          (fun uu___1  -> match uu___1 with | (v,uu___2,i) -> e = v) settings
         in
      match uu___ with
      ...
      | (l,h) ->
          let uu___1 =
            FStar_List.partition
              (fun uu___2  ->
                 match uu___2 with
                 | (uu___3,uu___4,i) -> (l <= i) && (i <= h)) settings
             in
      ...
      | (CWarning ,CAlwaysError ) ->
          let uu___ =
            let uu___1 =
              ...
              FStar_Util.format1 "cannot turn error %s into warning" uu___2
               in
      ...
      | (uu___,CFatal ) ->
          let uu___1 =
            let uu___2 =
              ...
              FStar_Util.format1
                "cannot change the error level of fatal error %s" uu___3
               in
      ...
      | (flag1,s) ->
          ...
          let uu___1 =
            ...
            | uu___2 ->
             ...
             in
          (match uu___1 with | (l1,h) -> (flag1, (l1, h)))
       in
    let error_range_settings = FStar_List.map compute_range l  in
    let uu___ = FStar_List.collect set_flag_for_range error_range_settings
       in
          ...
          let uu___1 =
            ...
            then ""
              ...
              (let uu___4 = FStar_Range.string_of_range r  in
               FStar_Util.format1 " (see also %s)" uu___4)
             in
          ("", uu___1)
          ...
          let uu___2 =
              ...
              (let uu___4 = FStar_Range.use_range r  in
               let uu___5 = FStar_Range.def_range r  in uu___4 = uu___5) ||
                (let uu___4 = FStar_Range.def_range r  in
                 let uu___5 = FStar_Range.def_range FStar_Range.dummyRange
                    in
                 uu___4 = uu___5)
               in
            if uu___3
            then ""
              ...
              (let uu___5 = FStar_Range.string_of_range r  in
               FStar_Util.format1 " (see also %s)" uu___5)
             in
          (uu___1, uu___2)
       in
    match uu___ with
    ...
    let add_one e =
      (let uu___1 =
         (FStar_Options.defensive_abort ()) &&
           (e.issue_number = (FStar_Pervasives_Native.Some defensive_errno))
          in
       if uu___1 then failwith "Aborting due to --defensive abort" else ());
      (match e.issue_level with
       | EInfo  -> print_issue e
       | uu___1 ->
           let uu___2 = let uu___3 = FStar_ST.op_Bang issues  in e :: uu___3
              in
           FStar_ST.op_Colon_Equals issues uu___2)
       in
      ...
      FStar_List.fold_left
        (fun n  ->
           fun i  ->
             ...
             | uu___2 -> n) Prims.int_zero uu___1
       in
    let report uu___ =
      let unique_issues =
        let uu___1 = FStar_ST.op_Bang issues  in
    ...
    FStar_List.fold_left
      (fun s1  ->
         fun p  ->
           ...
           FStar_String.op_Hat p uu___1) s uu___
     in
...
let (stop_if_err : unit -> unit) =
  fun uu___  ->
    let uu___1 = let uu___2 = get_err_count ()  in uu___2 > Prims.int_zero
       in
