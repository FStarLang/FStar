open Prims
type step =
  | Beta
  | Iota
  | Zeta
  | Exclude of step
  | WHNF
  | Primops
  | Eager_unfolding
  | Inlining
  | NoDeltaSteps
  | UnfoldUntil of FStar_Syntax_Syntax.delta_depth
  | UnfoldTac
  | PureSubtermsWithinComputations
  | Simplify
  | EraseUniverses
  | AllowUnboundUniverses
  | Reify
  | CompressUvars
  | NoFullNorm
let uu___is_Beta: step -> Prims.bool =
  fun projectee  -> match projectee with | Beta  -> true | uu____12 -> false
let uu___is_Iota: step -> Prims.bool =
  fun projectee  -> match projectee with | Iota  -> true | uu____16 -> false
let uu___is_Zeta: step -> Prims.bool =
  fun projectee  -> match projectee with | Zeta  -> true | uu____20 -> false
let uu___is_Exclude: step -> Prims.bool =
  fun projectee  ->
    match projectee with | Exclude _0 -> true | uu____25 -> false
let __proj__Exclude__item___0: step -> step =
  fun projectee  -> match projectee with | Exclude _0 -> _0
let uu___is_WHNF: step -> Prims.bool =
  fun projectee  -> match projectee with | WHNF  -> true | uu____36 -> false
let uu___is_Primops: step -> Prims.bool =
  fun projectee  ->
    match projectee with | Primops  -> true | uu____40 -> false
let uu___is_Eager_unfolding: step -> Prims.bool =
  fun projectee  ->
    match projectee with | Eager_unfolding  -> true | uu____44 -> false
let uu___is_Inlining: step -> Prims.bool =
  fun projectee  ->
    match projectee with | Inlining  -> true | uu____48 -> false
let uu___is_NoDeltaSteps: step -> Prims.bool =
  fun projectee  ->
    match projectee with | NoDeltaSteps  -> true | uu____52 -> false
let uu___is_UnfoldUntil: step -> Prims.bool =
  fun projectee  ->
    match projectee with | UnfoldUntil _0 -> true | uu____57 -> false
let __proj__UnfoldUntil__item___0: step -> FStar_Syntax_Syntax.delta_depth =
  fun projectee  -> match projectee with | UnfoldUntil _0 -> _0
let uu___is_UnfoldTac: step -> Prims.bool =
  fun projectee  ->
    match projectee with | UnfoldTac  -> true | uu____68 -> false
let uu___is_PureSubtermsWithinComputations: step -> Prims.bool =
  fun projectee  ->
    match projectee with
    | PureSubtermsWithinComputations  -> true
    | uu____72 -> false
let uu___is_Simplify: step -> Prims.bool =
  fun projectee  ->
    match projectee with | Simplify  -> true | uu____76 -> false
let uu___is_EraseUniverses: step -> Prims.bool =
  fun projectee  ->
    match projectee with | EraseUniverses  -> true | uu____80 -> false
let uu___is_AllowUnboundUniverses: step -> Prims.bool =
  fun projectee  ->
    match projectee with | AllowUnboundUniverses  -> true | uu____84 -> false
let uu___is_Reify: step -> Prims.bool =
  fun projectee  -> match projectee with | Reify  -> true | uu____88 -> false
let uu___is_CompressUvars: step -> Prims.bool =
  fun projectee  ->
    match projectee with | CompressUvars  -> true | uu____92 -> false
let uu___is_NoFullNorm: step -> Prims.bool =
  fun projectee  ->
    match projectee with | NoFullNorm  -> true | uu____96 -> false
type steps = step Prims.list
type primitive_step =
  {
  name: FStar_Ident.lid;
  arity: Prims.int;
  strong_reduction_ok: Prims.bool;
  interpretation:
    FStar_Range.range ->
      FStar_Syntax_Syntax.args -> FStar_Syntax_Syntax.term option;}
type closure =
  | Clos of (closure Prims.list* FStar_Syntax_Syntax.term* (closure
  Prims.list* FStar_Syntax_Syntax.term) FStar_Syntax_Syntax.memo*
  Prims.bool)
  | Univ of FStar_Syntax_Syntax.universe
  | Dummy
let uu___is_Clos: closure -> Prims.bool =
  fun projectee  ->
    match projectee with | Clos _0 -> true | uu____179 -> false
let __proj__Clos__item___0:
  closure ->
    (closure Prims.list* FStar_Syntax_Syntax.term* (closure Prims.list*
      FStar_Syntax_Syntax.term) FStar_Syntax_Syntax.memo* Prims.bool)
  = fun projectee  -> match projectee with | Clos _0 -> _0
let uu___is_Univ: closure -> Prims.bool =
  fun projectee  ->
    match projectee with | Univ _0 -> true | uu____218 -> false
let __proj__Univ__item___0: closure -> FStar_Syntax_Syntax.universe =
  fun projectee  -> match projectee with | Univ _0 -> _0
let uu___is_Dummy: closure -> Prims.bool =
  fun projectee  ->
    match projectee with | Dummy  -> true | uu____229 -> false
type env = closure Prims.list
let closure_to_string: closure -> Prims.string =
  fun uu___135_233  ->
    match uu___135_233 with
    | Clos (uu____234,t,uu____236,uu____237) ->
        FStar_Syntax_Print.term_to_string t
    | Univ uu____248 -> "Univ"
    | Dummy  -> "dummy"
type cfg =
  {
  steps: steps;
  tcenv: FStar_TypeChecker_Env.env;
  delta_level: FStar_TypeChecker_Env.delta_level Prims.list;
  primitive_steps: primitive_step Prims.list;}
type branches =
  (FStar_Syntax_Syntax.pat* FStar_Syntax_Syntax.term option*
    FStar_Syntax_Syntax.term) Prims.list
type subst_t = FStar_Syntax_Syntax.subst_elt Prims.list
type stack_elt =
  | Arg of (closure* FStar_Syntax_Syntax.aqual* FStar_Range.range)
  | UnivArgs of (FStar_Syntax_Syntax.universe Prims.list* FStar_Range.range)
  | MemoLazy of (env* FStar_Syntax_Syntax.term) FStar_Syntax_Syntax.memo
  | Match of (env* branches* FStar_Range.range)
  | Abs of (env* FStar_Syntax_Syntax.binders* env*
  (FStar_Syntax_Syntax.lcomp,FStar_Syntax_Syntax.residual_comp)
  FStar_Util.either option* FStar_Range.range)
  | App of (FStar_Syntax_Syntax.term* FStar_Syntax_Syntax.aqual*
  FStar_Range.range)
  | Meta of (FStar_Syntax_Syntax.metadata* FStar_Range.range)
  | Let of (env* FStar_Syntax_Syntax.binders* FStar_Syntax_Syntax.letbinding*
  FStar_Range.range)
  | Steps of (steps* primitive_step Prims.list*
  FStar_TypeChecker_Env.delta_level Prims.list)
  | Debug of FStar_Syntax_Syntax.term
let uu___is_Arg: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Arg _0 -> true | uu____375 -> false
let __proj__Arg__item___0:
  stack_elt -> (closure* FStar_Syntax_Syntax.aqual* FStar_Range.range) =
  fun projectee  -> match projectee with | Arg _0 -> _0
let uu___is_UnivArgs: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | UnivArgs _0 -> true | uu____399 -> false
let __proj__UnivArgs__item___0:
  stack_elt -> (FStar_Syntax_Syntax.universe Prims.list* FStar_Range.range) =
  fun projectee  -> match projectee with | UnivArgs _0 -> _0
let uu___is_MemoLazy: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | MemoLazy _0 -> true | uu____423 -> false
let __proj__MemoLazy__item___0:
  stack_elt -> (env* FStar_Syntax_Syntax.term) FStar_Syntax_Syntax.memo =
  fun projectee  -> match projectee with | MemoLazy _0 -> _0
let uu___is_Match: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Match _0 -> true | uu____450 -> false
let __proj__Match__item___0: stack_elt -> (env* branches* FStar_Range.range)
  = fun projectee  -> match projectee with | Match _0 -> _0
let uu___is_Abs: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Abs _0 -> true | uu____479 -> false
let __proj__Abs__item___0:
  stack_elt ->
    (env* FStar_Syntax_Syntax.binders* env*
      (FStar_Syntax_Syntax.lcomp,FStar_Syntax_Syntax.residual_comp)
      FStar_Util.either option* FStar_Range.range)
  = fun projectee  -> match projectee with | Abs _0 -> _0
let uu___is_App: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | App _0 -> true | uu____518 -> false
let __proj__App__item___0:
  stack_elt ->
    (FStar_Syntax_Syntax.term* FStar_Syntax_Syntax.aqual* FStar_Range.range)
  = fun projectee  -> match projectee with | App _0 -> _0
let uu___is_Meta: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Meta _0 -> true | uu____541 -> false
let __proj__Meta__item___0:
  stack_elt -> (FStar_Syntax_Syntax.metadata* FStar_Range.range) =
  fun projectee  -> match projectee with | Meta _0 -> _0
let uu___is_Let: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Let _0 -> true | uu____563 -> false
let __proj__Let__item___0:
  stack_elt ->
    (env* FStar_Syntax_Syntax.binders* FStar_Syntax_Syntax.letbinding*
      FStar_Range.range)
  = fun projectee  -> match projectee with | Let _0 -> _0
let uu___is_Steps: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Steps _0 -> true | uu____592 -> false
let __proj__Steps__item___0:
  stack_elt ->
    (steps* primitive_step Prims.list* FStar_TypeChecker_Env.delta_level
      Prims.list)
  = fun projectee  -> match projectee with | Steps _0 -> _0
let uu___is_Debug: stack_elt -> Prims.bool =
  fun projectee  ->
    match projectee with | Debug _0 -> true | uu____619 -> false
let __proj__Debug__item___0: stack_elt -> FStar_Syntax_Syntax.term =
  fun projectee  -> match projectee with | Debug _0 -> _0
type stack = stack_elt Prims.list
let is_fstar_tactics_embed: FStar_Syntax_Syntax.term -> Prims.bool =
  fun t  ->
    let uu____631 =
      let uu____632 = FStar_Syntax_Util.un_uinst t in
      uu____632.FStar_Syntax_Syntax.n in
    match uu____631 with
    | FStar_Syntax_Syntax.Tm_fvar fv ->
        FStar_Syntax_Syntax.fv_eq_lid fv
          FStar_Syntax_Const.fstar_refl_embed_lid
    | uu____636 -> false
let mk t r = FStar_Syntax_Syntax.mk t None r
let set_memo r t =
  let uu____676 = FStar_ST.read r in
  match uu____676 with
  | Some uu____681 -> failwith "Unexpected set_memo: thunk already evaluated"
  | None  -> FStar_ST.write r (Some t)
let env_to_string: closure Prims.list -> Prims.string =
  fun env  ->
    let uu____690 = FStar_List.map closure_to_string env in
    FStar_All.pipe_right uu____690 (FStar_String.concat "; ")
let stack_elt_to_string: stack_elt -> Prims.string =
  fun uu___136_695  ->
    match uu___136_695 with
    | Arg (c,uu____697,uu____698) ->
        let uu____699 = closure_to_string c in
        FStar_Util.format1 "Closure %s" uu____699
    | MemoLazy uu____700 -> "MemoLazy"
    | Abs (uu____704,bs,uu____706,uu____707,uu____708) ->
        let uu____715 =
          FStar_All.pipe_left FStar_Util.string_of_int (FStar_List.length bs) in
        FStar_Util.format1 "Abs %s" uu____715
    | UnivArgs uu____720 -> "UnivArgs"
    | Match uu____724 -> "Match"
    | App (t,uu____729,uu____730) ->
        let uu____731 = FStar_Syntax_Print.term_to_string t in
        FStar_Util.format1 "App %s" uu____731
    | Meta (m,uu____733) -> "Meta"
    | Let uu____734 -> "Let"
    | Steps (uu____739,uu____740,uu____741) -> "Steps"
    | Debug t ->
        let uu____747 = FStar_Syntax_Print.term_to_string t in
        FStar_Util.format1 "Debug %s" uu____747
let stack_to_string: stack_elt Prims.list -> Prims.string =
  fun s  ->
    let uu____753 = FStar_List.map stack_elt_to_string s in
    FStar_All.pipe_right uu____753 (FStar_String.concat "; ")
let log: cfg -> (Prims.unit -> Prims.unit) -> Prims.unit =
  fun cfg  ->
    fun f  ->
      let uu____767 =
        FStar_TypeChecker_Env.debug cfg.tcenv (FStar_Options.Other "Norm") in
      if uu____767 then f () else ()
let is_empty uu___137_776 =
  match uu___137_776 with | [] -> true | uu____778 -> false
let lookup_bvar env x =
  try FStar_List.nth env x.FStar_Syntax_Syntax.index
  with
  | uu____796 ->
      let uu____797 =
        let uu____798 = FStar_Syntax_Print.db_to_string x in
        FStar_Util.format1 "Failed to find %s\n" uu____798 in
      failwith uu____797
let downgrade_ghost_effect_name:
  FStar_Ident.lident -> FStar_Ident.lident option =
  fun l  ->
    if FStar_Ident.lid_equals l FStar_Syntax_Const.effect_Ghost_lid
    then Some FStar_Syntax_Const.effect_Pure_lid
    else
      if FStar_Ident.lid_equals l FStar_Syntax_Const.effect_GTot_lid
      then Some FStar_Syntax_Const.effect_Tot_lid
      else
        if FStar_Ident.lid_equals l FStar_Syntax_Const.effect_GHOST_lid
        then Some FStar_Syntax_Const.effect_PURE_lid
        else None
let norm_universe:
  cfg ->
    closure Prims.list ->
      FStar_Syntax_Syntax.universe -> FStar_Syntax_Syntax.universe
  =
  fun cfg  ->
    fun env  ->
      fun u  ->
        let norm_univs us =
          let us1 = FStar_Util.sort_with FStar_Syntax_Util.compare_univs us in
          let uu____829 =
            FStar_List.fold_left
              (fun uu____838  ->
                 fun u1  ->
                   match uu____838 with
                   | (cur_kernel,cur_max,out) ->
                       let uu____853 = FStar_Syntax_Util.univ_kernel u1 in
                       (match uu____853 with
                        | (k_u,n1) ->
                            let uu____862 =
                              FStar_Syntax_Util.eq_univs cur_kernel k_u in
                            if uu____862
                            then (cur_kernel, u1, out)
                            else (k_u, u1, (cur_max :: out))))
              (FStar_Syntax_Syntax.U_zero, FStar_Syntax_Syntax.U_zero, [])
              us1 in
          match uu____829 with
          | (uu____872,u1,out) -> FStar_List.rev (u1 :: out) in
        let rec aux u1 =
          let u2 = FStar_Syntax_Subst.compress_univ u1 in
          match u2 with
          | FStar_Syntax_Syntax.U_bvar x ->
              (try
                 let uu____888 = FStar_List.nth env x in
                 match uu____888 with
                 | Univ u3 -> aux u3
                 | Dummy  -> [u2]
                 | uu____891 ->
                     failwith "Impossible: universe variable bound to a term"
               with
               | uu____895 ->
                   let uu____896 =
                     FStar_All.pipe_right cfg.steps
                       (FStar_List.contains AllowUnboundUniverses) in
                   if uu____896
                   then [FStar_Syntax_Syntax.U_unknown]
                   else failwith "Universe variable not found")
          | FStar_Syntax_Syntax.U_zero  -> [u2]
          | FStar_Syntax_Syntax.U_unif uu____900 -> [u2]
          | FStar_Syntax_Syntax.U_name uu____903 -> [u2]
          | FStar_Syntax_Syntax.U_unknown  -> [u2]
          | FStar_Syntax_Syntax.U_max [] -> [FStar_Syntax_Syntax.U_zero]
          | FStar_Syntax_Syntax.U_max us ->
              let us1 =
                let uu____908 = FStar_List.collect aux us in
                FStar_All.pipe_right uu____908 norm_univs in
              (match us1 with
               | u_k::hd1::rest ->
                   let rest1 = hd1 :: rest in
                   let uu____919 = FStar_Syntax_Util.univ_kernel u_k in
                   (match uu____919 with
                    | (FStar_Syntax_Syntax.U_zero ,n1) ->
                        let uu____924 =
                          FStar_All.pipe_right rest1
                            (FStar_List.for_all
                               (fun u3  ->
                                  let uu____927 =
                                    FStar_Syntax_Util.univ_kernel u3 in
                                  match uu____927 with
                                  | (uu____930,m) -> n1 <= m)) in
                        if uu____924 then rest1 else us1
                    | uu____934 -> us1)
               | uu____937 -> us1)
          | FStar_Syntax_Syntax.U_succ u3 ->
              let uu____940 = aux u3 in
              FStar_List.map (fun _0_30  -> FStar_Syntax_Syntax.U_succ _0_30)
                uu____940 in
        let uu____942 =
          FStar_All.pipe_right cfg.steps (FStar_List.contains EraseUniverses) in
        if uu____942
        then FStar_Syntax_Syntax.U_unknown
        else
          (let uu____944 = aux u in
           match uu____944 with
           | [] -> FStar_Syntax_Syntax.U_zero
           | (FStar_Syntax_Syntax.U_zero )::[] -> FStar_Syntax_Syntax.U_zero
           | (FStar_Syntax_Syntax.U_zero )::u1::[] -> u1
           | (FStar_Syntax_Syntax.U_zero )::us ->
               FStar_Syntax_Syntax.U_max us
           | u1::[] -> u1
           | us -> FStar_Syntax_Syntax.U_max us)
let rec closure_as_term:
  cfg ->
    closure Prims.list ->
      FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun cfg  ->
    fun env  ->
      fun t  ->
        log cfg
          (fun uu____1041  ->
             let uu____1042 = FStar_Syntax_Print.tag_of_term t in
             let uu____1043 = FStar_Syntax_Print.term_to_string t in
             FStar_Util.print2 ">>> %s Closure_as_term %s\n" uu____1042
               uu____1043);
        (match env with
         | [] when
             FStar_All.pipe_left Prims.op_Negation
               (FStar_List.contains CompressUvars cfg.steps)
             -> t
         | uu____1044 ->
             let t1 = FStar_Syntax_Subst.compress t in
             (match t1.FStar_Syntax_Syntax.n with
              | FStar_Syntax_Syntax.Tm_delayed uu____1047 ->
                  failwith "Impossible"
              | FStar_Syntax_Syntax.Tm_unknown  -> t1
              | FStar_Syntax_Syntax.Tm_constant uu____1068 -> t1
              | FStar_Syntax_Syntax.Tm_name uu____1069 -> t1
              | FStar_Syntax_Syntax.Tm_fvar uu____1070 -> t1
              | FStar_Syntax_Syntax.Tm_uvar uu____1071 -> t1
              | FStar_Syntax_Syntax.Tm_type u ->
                  let uu____1081 =
                    let uu____1082 = norm_universe cfg env u in
                    FStar_Syntax_Syntax.Tm_type uu____1082 in
                  mk uu____1081 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_uinst (t',us) ->
                  let uu____1089 = FStar_List.map (norm_universe cfg env) us in
                  FStar_Syntax_Syntax.mk_Tm_uinst t' uu____1089
              | FStar_Syntax_Syntax.Tm_bvar x ->
                  let uu____1091 = lookup_bvar env x in
                  (match uu____1091 with
                   | Univ uu____1092 ->
                       failwith
                         "Impossible: term variable is bound to a universe"
                   | Dummy  -> t1
                   | Clos (env1,t0,r,uu____1096) ->
                       closure_as_term cfg env1 t0)
              | FStar_Syntax_Syntax.Tm_app (head1,args) ->
                  let head2 = closure_as_term_delayed cfg env head1 in
                  let args1 = closures_as_args_delayed cfg env args in
                  mk (FStar_Syntax_Syntax.Tm_app (head2, args1))
                    t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_abs (bs,body,lopt) ->
                  let uu____1164 = closures_as_binders_delayed cfg env bs in
                  (match uu____1164 with
                   | (bs1,env1) ->
                       let body1 = closure_as_term_delayed cfg env1 body in
                       let uu____1184 =
                         let uu____1185 =
                           let uu____1200 = close_lcomp_opt cfg env1 lopt in
                           (bs1, body1, uu____1200) in
                         FStar_Syntax_Syntax.Tm_abs uu____1185 in
                       mk uu____1184 t1.FStar_Syntax_Syntax.pos)
              | FStar_Syntax_Syntax.Tm_arrow (bs,c) ->
                  let uu____1230 = closures_as_binders_delayed cfg env bs in
                  (match uu____1230 with
                   | (bs1,env1) ->
                       let c1 = close_comp cfg env1 c in
                       mk (FStar_Syntax_Syntax.Tm_arrow (bs1, c1))
                         t1.FStar_Syntax_Syntax.pos)
              | FStar_Syntax_Syntax.Tm_refine (x,phi) ->
                  let uu____1261 =
                    let uu____1268 =
                      let uu____1272 = FStar_Syntax_Syntax.mk_binder x in
                      [uu____1272] in
                    closures_as_binders_delayed cfg env uu____1268 in
                  (match uu____1261 with
                   | (x1,env1) ->
                       let phi1 = closure_as_term_delayed cfg env1 phi in
                       let uu____1286 =
                         let uu____1287 =
                           let uu____1292 =
                             let uu____1293 = FStar_List.hd x1 in
                             FStar_All.pipe_right uu____1293
                               FStar_Pervasives.fst in
                           (uu____1292, phi1) in
                         FStar_Syntax_Syntax.Tm_refine uu____1287 in
                       mk uu____1286 t1.FStar_Syntax_Syntax.pos)
              | FStar_Syntax_Syntax.Tm_ascribed (t11,(annot,tacopt),lopt) ->
                  let annot1 =
                    match annot with
                    | FStar_Util.Inl t2 ->
                        let uu____1361 = closure_as_term_delayed cfg env t2 in
                        FStar_Util.Inl uu____1361
                    | FStar_Util.Inr c ->
                        let uu____1375 = close_comp cfg env c in
                        FStar_Util.Inr uu____1375 in
                  let tacopt1 =
                    FStar_Util.map_opt tacopt
                      (closure_as_term_delayed cfg env) in
                  let uu____1390 =
                    let uu____1391 =
                      let uu____1409 = closure_as_term_delayed cfg env t11 in
                      (uu____1409, (annot1, tacopt1), lopt) in
                    FStar_Syntax_Syntax.Tm_ascribed uu____1391 in
                  mk uu____1390 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_meta
                  (t',FStar_Syntax_Syntax.Meta_pattern args) ->
                  let uu____1447 =
                    let uu____1448 =
                      let uu____1453 = closure_as_term_delayed cfg env t' in
                      let uu____1456 =
                        let uu____1457 =
                          FStar_All.pipe_right args
                            (FStar_List.map
                               (closures_as_args_delayed cfg env)) in
                        FStar_Syntax_Syntax.Meta_pattern uu____1457 in
                      (uu____1453, uu____1456) in
                    FStar_Syntax_Syntax.Tm_meta uu____1448 in
                  mk uu____1447 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_meta
                  (t',FStar_Syntax_Syntax.Meta_monadic (m,tbody)) ->
                  let uu____1499 =
                    let uu____1500 =
                      let uu____1505 = closure_as_term_delayed cfg env t' in
                      let uu____1508 =
                        let uu____1509 =
                          let uu____1514 =
                            closure_as_term_delayed cfg env tbody in
                          (m, uu____1514) in
                        FStar_Syntax_Syntax.Meta_monadic uu____1509 in
                      (uu____1505, uu____1508) in
                    FStar_Syntax_Syntax.Tm_meta uu____1500 in
                  mk uu____1499 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_meta
                  (t',FStar_Syntax_Syntax.Meta_monadic_lift (m1,m2,tbody)) ->
                  let uu____1533 =
                    let uu____1534 =
                      let uu____1539 = closure_as_term_delayed cfg env t' in
                      let uu____1542 =
                        let uu____1543 =
                          let uu____1549 =
                            closure_as_term_delayed cfg env tbody in
                          (m1, m2, uu____1549) in
                        FStar_Syntax_Syntax.Meta_monadic_lift uu____1543 in
                      (uu____1539, uu____1542) in
                    FStar_Syntax_Syntax.Tm_meta uu____1534 in
                  mk uu____1533 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_meta (t',m) ->
                  let uu____1562 =
                    let uu____1563 =
                      let uu____1568 = closure_as_term_delayed cfg env t' in
                      (uu____1568, m) in
                    FStar_Syntax_Syntax.Tm_meta uu____1563 in
                  mk uu____1562 t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_let ((false ,lb::[]),body) ->
                  let env0 = env in
                  let env1 =
                    FStar_List.fold_left
                      (fun env1  -> fun uu____1589  -> Dummy :: env1) env
                      lb.FStar_Syntax_Syntax.lbunivs in
                  let typ =
                    closure_as_term_delayed cfg env1
                      lb.FStar_Syntax_Syntax.lbtyp in
                  let def =
                    closure_as_term cfg env1 lb.FStar_Syntax_Syntax.lbdef in
                  let body1 =
                    match lb.FStar_Syntax_Syntax.lbname with
                    | FStar_Util.Inr uu____1600 -> body
                    | FStar_Util.Inl uu____1601 ->
                        closure_as_term cfg (Dummy :: env0) body in
                  let lb1 =
                    let uu___150_1603 = lb in
                    {
                      FStar_Syntax_Syntax.lbname =
                        (uu___150_1603.FStar_Syntax_Syntax.lbname);
                      FStar_Syntax_Syntax.lbunivs =
                        (uu___150_1603.FStar_Syntax_Syntax.lbunivs);
                      FStar_Syntax_Syntax.lbtyp = typ;
                      FStar_Syntax_Syntax.lbeff =
                        (uu___150_1603.FStar_Syntax_Syntax.lbeff);
                      FStar_Syntax_Syntax.lbdef = def
                    } in
                  mk (FStar_Syntax_Syntax.Tm_let ((false, [lb1]), body1))
                    t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_let ((uu____1610,lbs),body) ->
                  let norm_one_lb env1 lb =
                    let env_univs =
                      FStar_List.fold_right
                        (fun uu____1634  -> fun env2  -> Dummy :: env2)
                        lb.FStar_Syntax_Syntax.lbunivs env1 in
                    let env2 =
                      let uu____1639 = FStar_Syntax_Syntax.is_top_level lbs in
                      if uu____1639
                      then env_univs
                      else
                        FStar_List.fold_right
                          (fun uu____1643  -> fun env2  -> Dummy :: env2) lbs
                          env_univs in
                    let uu___151_1646 = lb in
                    let uu____1647 =
                      closure_as_term cfg env_univs
                        lb.FStar_Syntax_Syntax.lbtyp in
                    let uu____1650 =
                      closure_as_term cfg env2 lb.FStar_Syntax_Syntax.lbdef in
                    {
                      FStar_Syntax_Syntax.lbname =
                        (uu___151_1646.FStar_Syntax_Syntax.lbname);
                      FStar_Syntax_Syntax.lbunivs =
                        (uu___151_1646.FStar_Syntax_Syntax.lbunivs);
                      FStar_Syntax_Syntax.lbtyp = uu____1647;
                      FStar_Syntax_Syntax.lbeff =
                        (uu___151_1646.FStar_Syntax_Syntax.lbeff);
                      FStar_Syntax_Syntax.lbdef = uu____1650
                    } in
                  let lbs1 =
                    FStar_All.pipe_right lbs
                      (FStar_List.map (norm_one_lb env)) in
                  let body1 =
                    let body_env =
                      FStar_List.fold_right
                        (fun uu____1661  -> fun env1  -> Dummy :: env1) lbs1
                        env in
                    closure_as_term cfg body_env body in
                  mk (FStar_Syntax_Syntax.Tm_let ((true, lbs1), body1))
                    t1.FStar_Syntax_Syntax.pos
              | FStar_Syntax_Syntax.Tm_match (head1,branches) ->
                  let head2 = closure_as_term cfg env head1 in
                  let norm_one_branch env1 uu____1716 =
                    match uu____1716 with
                    | (pat,w_opt,tm) ->
                        let rec norm_pat env2 p =
                          match p.FStar_Syntax_Syntax.v with
                          | FStar_Syntax_Syntax.Pat_constant uu____1762 ->
                              (p, env2)
                          | FStar_Syntax_Syntax.Pat_cons (fv,pats) ->
                              let uu____1778 =
                                FStar_All.pipe_right pats
                                  (FStar_List.fold_left
                                     (fun uu____1812  ->
                                        fun uu____1813  ->
                                          match (uu____1812, uu____1813) with
                                          | ((pats1,env3),(p1,b)) ->
                                              let uu____1878 =
                                                norm_pat env3 p1 in
                                              (match uu____1878 with
                                               | (p2,env4) ->
                                                   (((p2, b) :: pats1), env4)))
                                     ([], env2)) in
                              (match uu____1778 with
                               | (pats1,env3) ->
                                   ((let uu___152_1944 = p in
                                     {
                                       FStar_Syntax_Syntax.v =
                                         (FStar_Syntax_Syntax.Pat_cons
                                            (fv, (FStar_List.rev pats1)));
                                       FStar_Syntax_Syntax.ty =
                                         (uu___152_1944.FStar_Syntax_Syntax.ty);
                                       FStar_Syntax_Syntax.p =
                                         (uu___152_1944.FStar_Syntax_Syntax.p)
                                     }), env3))
                          | FStar_Syntax_Syntax.Pat_var x ->
                              let x1 =
                                let uu___153_1958 = x in
                                let uu____1959 =
                                  closure_as_term cfg env2
                                    x.FStar_Syntax_Syntax.sort in
                                {
                                  FStar_Syntax_Syntax.ppname =
                                    (uu___153_1958.FStar_Syntax_Syntax.ppname);
                                  FStar_Syntax_Syntax.index =
                                    (uu___153_1958.FStar_Syntax_Syntax.index);
                                  FStar_Syntax_Syntax.sort = uu____1959
                                } in
                              ((let uu___154_1965 = p in
                                {
                                  FStar_Syntax_Syntax.v =
                                    (FStar_Syntax_Syntax.Pat_var x1);
                                  FStar_Syntax_Syntax.ty =
                                    (uu___154_1965.FStar_Syntax_Syntax.ty);
                                  FStar_Syntax_Syntax.p =
                                    (uu___154_1965.FStar_Syntax_Syntax.p)
                                }), (Dummy :: env2))
                          | FStar_Syntax_Syntax.Pat_wild x ->
                              let x1 =
                                let uu___155_1970 = x in
                                let uu____1971 =
                                  closure_as_term cfg env2
                                    x.FStar_Syntax_Syntax.sort in
                                {
                                  FStar_Syntax_Syntax.ppname =
                                    (uu___155_1970.FStar_Syntax_Syntax.ppname);
                                  FStar_Syntax_Syntax.index =
                                    (uu___155_1970.FStar_Syntax_Syntax.index);
                                  FStar_Syntax_Syntax.sort = uu____1971
                                } in
                              ((let uu___156_1977 = p in
                                {
                                  FStar_Syntax_Syntax.v =
                                    (FStar_Syntax_Syntax.Pat_wild x1);
                                  FStar_Syntax_Syntax.ty =
                                    (uu___156_1977.FStar_Syntax_Syntax.ty);
                                  FStar_Syntax_Syntax.p =
                                    (uu___156_1977.FStar_Syntax_Syntax.p)
                                }), (Dummy :: env2))
                          | FStar_Syntax_Syntax.Pat_dot_term (x,t2) ->
                              let x1 =
                                let uu___157_1987 = x in
                                let uu____1988 =
                                  closure_as_term cfg env2
                                    x.FStar_Syntax_Syntax.sort in
                                {
                                  FStar_Syntax_Syntax.ppname =
                                    (uu___157_1987.FStar_Syntax_Syntax.ppname);
                                  FStar_Syntax_Syntax.index =
                                    (uu___157_1987.FStar_Syntax_Syntax.index);
                                  FStar_Syntax_Syntax.sort = uu____1988
                                } in
                              let t3 = closure_as_term cfg env2 t2 in
                              ((let uu___158_1995 = p in
                                {
                                  FStar_Syntax_Syntax.v =
                                    (FStar_Syntax_Syntax.Pat_dot_term
                                       (x1, t3));
                                  FStar_Syntax_Syntax.ty =
                                    (uu___158_1995.FStar_Syntax_Syntax.ty);
                                  FStar_Syntax_Syntax.p =
                                    (uu___158_1995.FStar_Syntax_Syntax.p)
                                }), env2) in
                        let uu____1998 = norm_pat env1 pat in
                        (match uu____1998 with
                         | (pat1,env2) ->
                             let w_opt1 =
                               match w_opt with
                               | None  -> None
                               | Some w ->
                                   let uu____2022 =
                                     closure_as_term cfg env2 w in
                                   Some uu____2022 in
                             let tm1 = closure_as_term cfg env2 tm in
                             (pat1, w_opt1, tm1)) in
                  let uu____2027 =
                    let uu____2028 =
                      let uu____2044 =
                        FStar_All.pipe_right branches
                          (FStar_List.map (norm_one_branch env)) in
                      (head2, uu____2044) in
                    FStar_Syntax_Syntax.Tm_match uu____2028 in
                  mk uu____2027 t1.FStar_Syntax_Syntax.pos))
and closure_as_term_delayed:
  cfg ->
    closure Prims.list ->
      (FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
        FStar_Syntax_Syntax.syntax -> FStar_Syntax_Syntax.term
  =
  fun cfg  ->
    fun env  ->
      fun t  ->
        match env with
        | [] when
            FStar_All.pipe_left Prims.op_Negation
              (FStar_List.contains CompressUvars cfg.steps)
            -> t
        | uu____2097 -> closure_as_term cfg env t
and closures_as_args_delayed:
  cfg ->
    closure Prims.list ->
      ((FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
        FStar_Syntax_Syntax.syntax* FStar_Syntax_Syntax.aqual) Prims.list ->
        ((FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
          FStar_Syntax_Syntax.syntax* FStar_Syntax_Syntax.aqual) Prims.list
  =
  fun cfg  ->
    fun env  ->
      fun args  ->
        match env with
        | [] when
            FStar_All.pipe_left Prims.op_Negation
              (FStar_List.contains CompressUvars cfg.steps)
            -> args
        | uu____2113 ->
            FStar_List.map
              (fun uu____2123  ->
                 match uu____2123 with
                 | (x,imp) ->
                     let uu____2138 = closure_as_term_delayed cfg env x in
                     (uu____2138, imp)) args
and closures_as_binders_delayed:
  cfg ->
    closure Prims.list ->
      (FStar_Syntax_Syntax.bv* FStar_Syntax_Syntax.aqual) Prims.list ->
        ((FStar_Syntax_Syntax.bv* FStar_Syntax_Syntax.aqual) Prims.list*
          closure Prims.list)
  =
  fun cfg  ->
    fun env  ->
      fun bs  ->
        let uu____2150 =
          FStar_All.pipe_right bs
            (FStar_List.fold_left
               (fun uu____2174  ->
                  fun uu____2175  ->
                    match (uu____2174, uu____2175) with
                    | ((env1,out),(b,imp)) ->
                        let b1 =
                          let uu___159_2219 = b in
                          let uu____2220 =
                            closure_as_term_delayed cfg env1
                              b.FStar_Syntax_Syntax.sort in
                          {
                            FStar_Syntax_Syntax.ppname =
                              (uu___159_2219.FStar_Syntax_Syntax.ppname);
                            FStar_Syntax_Syntax.index =
                              (uu___159_2219.FStar_Syntax_Syntax.index);
                            FStar_Syntax_Syntax.sort = uu____2220
                          } in
                        let env2 = Dummy :: env1 in
                        (env2, ((b1, imp) :: out))) (env, [])) in
        match uu____2150 with | (env1,bs1) -> ((FStar_List.rev bs1), env1)
and close_comp:
  cfg ->
    closure Prims.list ->
      (FStar_Syntax_Syntax.comp',Prims.unit) FStar_Syntax_Syntax.syntax ->
        (FStar_Syntax_Syntax.comp',Prims.unit) FStar_Syntax_Syntax.syntax
  =
  fun cfg  ->
    fun env  ->
      fun c  ->
        match env with
        | [] when
            FStar_All.pipe_left Prims.op_Negation
              (FStar_List.contains CompressUvars cfg.steps)
            -> c
        | uu____2267 ->
            (match c.FStar_Syntax_Syntax.n with
             | FStar_Syntax_Syntax.Total (t,uopt) ->
                 let uu____2279 = closure_as_term_delayed cfg env t in
                 let uu____2280 =
                   FStar_Option.map (norm_universe cfg env) uopt in
                 FStar_Syntax_Syntax.mk_Total' uu____2279 uu____2280
             | FStar_Syntax_Syntax.GTotal (t,uopt) ->
                 let uu____2290 = closure_as_term_delayed cfg env t in
                 let uu____2291 =
                   FStar_Option.map (norm_universe cfg env) uopt in
                 FStar_Syntax_Syntax.mk_GTotal' uu____2290 uu____2291
             | FStar_Syntax_Syntax.Comp c1 ->
                 let rt =
                   closure_as_term_delayed cfg env
                     c1.FStar_Syntax_Syntax.result_typ in
                 let args =
                   closures_as_args_delayed cfg env
                     c1.FStar_Syntax_Syntax.effect_args in
                 let flags =
                   FStar_All.pipe_right c1.FStar_Syntax_Syntax.flags
                     (FStar_List.map
                        (fun uu___138_2307  ->
                           match uu___138_2307 with
                           | FStar_Syntax_Syntax.DECREASES t ->
                               let uu____2311 =
                                 closure_as_term_delayed cfg env t in
                               FStar_Syntax_Syntax.DECREASES uu____2311
                           | f -> f)) in
                 let uu____2315 =
                   let uu___160_2316 = c1 in
                   let uu____2317 =
                     FStar_List.map (norm_universe cfg env)
                       c1.FStar_Syntax_Syntax.comp_univs in
                   {
                     FStar_Syntax_Syntax.comp_univs = uu____2317;
                     FStar_Syntax_Syntax.effect_name =
                       (uu___160_2316.FStar_Syntax_Syntax.effect_name);
                     FStar_Syntax_Syntax.result_typ = rt;
                     FStar_Syntax_Syntax.effect_args = args;
                     FStar_Syntax_Syntax.flags = flags
                   } in
                 FStar_Syntax_Syntax.mk_Comp uu____2315)
and filter_out_lcomp_cflags:
  FStar_Syntax_Syntax.lcomp -> FStar_Syntax_Syntax.cflags Prims.list =
  fun lc  ->
    FStar_All.pipe_right lc.FStar_Syntax_Syntax.cflags
      (FStar_List.filter
         (fun uu___139_2321  ->
            match uu___139_2321 with
            | FStar_Syntax_Syntax.DECREASES uu____2322 -> false
            | uu____2325 -> true))
and close_lcomp_opt:
  cfg ->
    closure Prims.list ->
      (FStar_Syntax_Syntax.lcomp,(FStar_Ident.lident*
                                   FStar_Syntax_Syntax.cflags Prims.list))
        FStar_Util.either option ->
        (FStar_Syntax_Syntax.lcomp,(FStar_Ident.lident*
                                     FStar_Syntax_Syntax.cflags Prims.list))
          FStar_Util.either option
  =
  fun cfg  ->
    fun env  ->
      fun lopt  ->
        match lopt with
        | Some (FStar_Util.Inl lc) ->
            let flags = filter_out_lcomp_cflags lc in
            let uu____2353 = FStar_Syntax_Util.is_total_lcomp lc in
            if uu____2353
            then
              Some
                (FStar_Util.Inr (FStar_Syntax_Const.effect_Tot_lid, flags))
            else
              (let uu____2370 = FStar_Syntax_Util.is_tot_or_gtot_lcomp lc in
               if uu____2370
               then
                 Some
                   (FStar_Util.Inr
                      (FStar_Syntax_Const.effect_GTot_lid, flags))
               else
                 Some
                   (FStar_Util.Inr ((lc.FStar_Syntax_Syntax.eff_name), flags)))
        | uu____2396 -> lopt
let built_in_primitive_steps: primitive_step Prims.list =
  let const_as_tm c p = mk (FStar_Syntax_Syntax.Tm_constant c) p in
  let int_as_const p i =
    let uu____2420 =
      let uu____2421 =
        let uu____2427 = FStar_Util.string_of_int i in (uu____2427, None) in
      FStar_Const.Const_int uu____2421 in
    const_as_tm uu____2420 p in
  let bool_as_const p b = const_as_tm (FStar_Const.Const_bool b) p in
  let string_as_const p b =
    const_as_tm
      (FStar_Const.Const_string ((FStar_Util.bytes_of_string b), p)) p in
  let arg_as_int uu____2454 =
    match uu____2454 with
    | (a,uu____2459) ->
        let uu____2461 =
          let uu____2462 = FStar_Syntax_Subst.compress a in
          uu____2462.FStar_Syntax_Syntax.n in
        (match uu____2461 with
         | FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_int (i,None ))
             ->
             let uu____2472 = FStar_Util.int_of_string i in Some uu____2472
         | uu____2473 -> None) in
  let arg_as_bounded_int uu____2482 =
    match uu____2482 with
    | (a,uu____2489) ->
        let uu____2493 =
          let uu____2494 = FStar_Syntax_Subst.compress a in
          uu____2494.FStar_Syntax_Syntax.n in
        (match uu____2493 with
         | FStar_Syntax_Syntax.Tm_app
             ({ FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar fv1;
                FStar_Syntax_Syntax.tk = uu____2501;
                FStar_Syntax_Syntax.pos = uu____2502;
                FStar_Syntax_Syntax.vars = uu____2503;_},({
                                                            FStar_Syntax_Syntax.n
                                                              =
                                                              FStar_Syntax_Syntax.Tm_constant
                                                              (FStar_Const.Const_int
                                                              (i,None ));
                                                            FStar_Syntax_Syntax.tk
                                                              = uu____2505;
                                                            FStar_Syntax_Syntax.pos
                                                              = uu____2506;
                                                            FStar_Syntax_Syntax.vars
                                                              = uu____2507;_},uu____2508)::[])
             when
             FStar_Util.ends_with
               (FStar_Ident.text_of_lid
                  (fv1.FStar_Syntax_Syntax.fv_name).FStar_Syntax_Syntax.v)
               "int_to_t"
             ->
             let uu____2539 =
               let uu____2542 = FStar_Util.int_of_string i in
               (fv1, uu____2542) in
             Some uu____2539
         | uu____2545 -> None) in
  let arg_as_bool uu____2554 =
    match uu____2554 with
    | (a,uu____2559) ->
        let uu____2561 =
          let uu____2562 = FStar_Syntax_Subst.compress a in
          uu____2562.FStar_Syntax_Syntax.n in
        (match uu____2561 with
         | FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_bool b) ->
             Some b
         | uu____2567 -> None) in
  let arg_as_char uu____2574 =
    match uu____2574 with
    | (a,uu____2579) ->
        let uu____2581 =
          let uu____2582 = FStar_Syntax_Subst.compress a in
          uu____2582.FStar_Syntax_Syntax.n in
        (match uu____2581 with
         | FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_char c) ->
             Some c
         | uu____2587 -> None) in
  let arg_as_string uu____2594 =
    match uu____2594 with
    | (a,uu____2599) ->
        let uu____2601 =
          let uu____2602 = FStar_Syntax_Subst.compress a in
          uu____2602.FStar_Syntax_Syntax.n in
        (match uu____2601 with
         | FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_string
             (bytes,uu____2607)) -> Some (FStar_Util.string_of_bytes bytes)
         | uu____2610 -> None) in
  let arg_as_list f uu____2631 =
    match uu____2631 with
    | (a,uu____2640) ->
        let rec sequence l =
          match l with
          | [] -> Some []
          | (None )::uu____2659 -> None
          | (Some x)::xs ->
              let uu____2669 = sequence xs in
              (match uu____2669 with
               | None  -> None
               | Some xs' -> Some (x :: xs')) in
        let uu____2680 = FStar_Syntax_Util.list_elements a in
        (match uu____2680 with
         | None  -> None
         | Some elts ->
             let uu____2690 =
               FStar_List.map
                 (fun x  ->
                    let uu____2695 = FStar_Syntax_Syntax.as_arg x in
                    f uu____2695) elts in
             sequence uu____2690) in
  let lift_unary f aopts =
    match aopts with
    | (Some a)::[] -> let uu____2725 = f a in Some uu____2725
    | uu____2726 -> None in
  let lift_binary f aopts =
    match aopts with
    | (Some a0)::(Some a1)::[] -> let uu____2765 = f a0 a1 in Some uu____2765
    | uu____2766 -> None in
  let unary_op as_a f r args =
    let uu____2810 = FStar_List.map as_a args in lift_unary (f r) uu____2810 in
  let binary_op as_a f r args =
    let uu____2860 = FStar_List.map as_a args in lift_binary (f r) uu____2860 in
  let as_primitive_step uu____2877 =
    match uu____2877 with
    | (l,arity,f) ->
        { name = l; arity; strong_reduction_ok = true; interpretation = f } in
  let unary_int_op f =
    unary_op arg_as_int
      (fun r  -> fun x  -> let uu____2915 = f x in int_as_const r uu____2915) in
  let binary_int_op f =
    binary_op arg_as_int
      (fun r  ->
         fun x  ->
           fun y  -> let uu____2938 = f x y in int_as_const r uu____2938) in
  let unary_bool_op f =
    unary_op arg_as_bool
      (fun r  -> fun x  -> let uu____2955 = f x in bool_as_const r uu____2955) in
  let binary_bool_op f =
    binary_op arg_as_bool
      (fun r  ->
         fun x  ->
           fun y  -> let uu____2978 = f x y in bool_as_const r uu____2978) in
  let binary_string_op f =
    binary_op arg_as_string
      (fun r  ->
         fun x  ->
           fun y  -> let uu____3001 = f x y in string_as_const r uu____3001) in
  let list_of_string' rng s =
    let name l =
      let uu____3015 =
        let uu____3016 =
          FStar_Syntax_Syntax.lid_as_fv l FStar_Syntax_Syntax.Delta_constant
            None in
        FStar_Syntax_Syntax.Tm_fvar uu____3016 in
      mk uu____3015 rng in
    let char_t = name FStar_Syntax_Const.char_lid in
    let charterm c =
      mk (FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_char c)) rng in
    let uu____3026 =
      let uu____3028 = FStar_String.list_of_string s in
      FStar_List.map charterm uu____3028 in
    FStar_All.pipe_left (FStar_Syntax_Util.mk_list char_t rng) uu____3026 in
  let string_of_list' rng l =
    let s = FStar_String.string_of_list l in FStar_Syntax_Const.exp_string s in
  let string_compare' rng s1 s2 =
    let r = FStar_String.compare s1 s2 in int_as_const rng r in
  let string_concat' rng args =
    match args with
    | a1::a2::[] ->
        let uu____3082 = arg_as_string a1 in
        (match uu____3082 with
         | Some s1 ->
             let uu____3086 = arg_as_list arg_as_string a2 in
             (match uu____3086 with
              | Some s2 ->
                  let r = FStar_String.concat s1 s2 in
                  let uu____3094 = string_as_const rng r in Some uu____3094
              | uu____3095 -> None)
         | uu____3098 -> None)
    | uu____3100 -> None in
  let string_of_int1 rng i =
    let uu____3111 = FStar_Util.string_of_int i in
    string_as_const rng uu____3111 in
  let string_of_bool1 rng b =
    string_as_const rng (if b then "true" else "false") in
  let string_of_int2 rng i =
    let uu____3127 = FStar_Util.string_of_int i in
    string_as_const rng uu____3127 in
  let string_of_bool2 rng b =
    string_as_const rng (if b then "true" else "false") in
  let basic_ops =
    let uu____3146 =
      let uu____3156 =
        let uu____3166 =
          let uu____3176 =
            let uu____3186 =
              let uu____3196 =
                let uu____3206 =
                  let uu____3216 =
                    let uu____3226 =
                      let uu____3236 =
                        let uu____3246 =
                          let uu____3256 =
                            let uu____3266 =
                              let uu____3276 =
                                let uu____3286 =
                                  let uu____3296 =
                                    let uu____3306 =
                                      let uu____3316 =
                                        let uu____3325 =
                                          FStar_Syntax_Const.p2l
                                            ["FStar";
                                            "String";
                                            "list_of_string"] in
                                        (uu____3325, (Prims.parse_int "1"),
                                          (unary_op arg_as_string
                                             list_of_string')) in
                                      let uu____3331 =
                                        let uu____3341 =
                                          let uu____3350 =
                                            FStar_Syntax_Const.p2l
                                              ["FStar";
                                              "String";
                                              "string_of_list"] in
                                          (uu____3350, (Prims.parse_int "1"),
                                            (unary_op
                                               (arg_as_list arg_as_char)
                                               string_of_list')) in
                                        let uu____3357 =
                                          let uu____3367 =
                                            let uu____3379 =
                                              FStar_Syntax_Const.p2l
                                                ["FStar"; "String"; "concat"] in
                                            (uu____3379,
                                              (Prims.parse_int "2"),
                                              string_concat') in
                                          [uu____3367] in
                                        uu____3341 :: uu____3357 in
                                      uu____3316 :: uu____3331 in
                                    (FStar_Syntax_Const.string_compare,
                                      (Prims.parse_int "2"),
                                      (binary_op arg_as_string
                                         string_compare'))
                                      :: uu____3306 in
                                  (FStar_Syntax_Const.string_of_bool_lid,
                                    (Prims.parse_int "1"),
                                    (unary_op arg_as_bool string_of_bool2))
                                    :: uu____3296 in
                                (FStar_Syntax_Const.string_of_int_lid,
                                  (Prims.parse_int "1"),
                                  (unary_op arg_as_int string_of_int2)) ::
                                  uu____3286 in
                              (FStar_Syntax_Const.strcat_lid,
                                (Prims.parse_int "2"),
                                (binary_string_op
                                   (fun x  -> fun y  -> Prims.strcat x y)))
                                :: uu____3276 in
                            (FStar_Syntax_Const.op_Or, (Prims.parse_int "2"),
                              (binary_bool_op (fun x  -> fun y  -> x || y)))
                              :: uu____3266 in
                          (FStar_Syntax_Const.op_And, (Prims.parse_int "2"),
                            (binary_bool_op (fun x  -> fun y  -> x && y))) ::
                            uu____3256 in
                        (FStar_Syntax_Const.op_Negation,
                          (Prims.parse_int "1"),
                          (unary_bool_op (fun x  -> Prims.op_Negation x))) ::
                          uu____3246 in
                      (FStar_Syntax_Const.op_Modulus, (Prims.parse_int "2"),
                        (binary_int_op (fun x  -> fun y  -> x mod y))) ::
                        uu____3236 in
                    (FStar_Syntax_Const.op_GTE, (Prims.parse_int "2"),
                      (binary_op arg_as_int
                         (fun r  ->
                            fun x  -> fun y  -> bool_as_const r (x >= y))))
                      :: uu____3226 in
                  (FStar_Syntax_Const.op_GT, (Prims.parse_int "2"),
                    (binary_op arg_as_int
                       (fun r  -> fun x  -> fun y  -> bool_as_const r (x > y))))
                    :: uu____3216 in
                (FStar_Syntax_Const.op_LTE, (Prims.parse_int "2"),
                  (binary_op arg_as_int
                     (fun r  -> fun x  -> fun y  -> bool_as_const r (x <= y))))
                  :: uu____3206 in
              (FStar_Syntax_Const.op_LT, (Prims.parse_int "2"),
                (binary_op arg_as_int
                   (fun r  -> fun x  -> fun y  -> bool_as_const r (x < y))))
                :: uu____3196 in
            (FStar_Syntax_Const.op_Division, (Prims.parse_int "2"),
              (binary_int_op (fun x  -> fun y  -> x / y))) :: uu____3186 in
          (FStar_Syntax_Const.op_Multiply, (Prims.parse_int "2"),
            (binary_int_op (fun x  -> fun y  -> x * y))) :: uu____3176 in
        (FStar_Syntax_Const.op_Subtraction, (Prims.parse_int "2"),
          (binary_int_op (fun x  -> fun y  -> x - y))) :: uu____3166 in
      (FStar_Syntax_Const.op_Addition, (Prims.parse_int "2"),
        (binary_int_op (fun x  -> fun y  -> x + y))) :: uu____3156 in
    (FStar_Syntax_Const.op_Minus, (Prims.parse_int "1"),
      (unary_int_op (fun x  -> - x))) :: uu____3146 in
  let bounded_arith_ops =
    let bounded_int_types =
      ["Int8";
      "UInt8";
      "Int16";
      "UInt16";
      "Int32";
      "UInt32";
      "Int64";
      "UInt64";
      "UInt128"] in
    let int_as_bounded r int_to_t1 n1 =
      let c = int_as_const r n1 in
      let int_to_t2 = FStar_Syntax_Syntax.fv_to_tm int_to_t1 in
      let uu____3703 =
        let uu____3704 =
          let uu____3705 = FStar_Syntax_Syntax.as_arg c in [uu____3705] in
        FStar_Syntax_Syntax.mk_Tm_app int_to_t2 uu____3704 in
      uu____3703 None r in
    FStar_All.pipe_right bounded_int_types
      (FStar_List.collect
         (fun m  ->
            let uu____3729 =
              let uu____3738 = FStar_Syntax_Const.p2l ["FStar"; m; "add"] in
              (uu____3738, (Prims.parse_int "2"),
                (binary_op arg_as_bounded_int
                   (fun r  ->
                      fun uu____3747  ->
                        fun uu____3748  ->
                          match (uu____3747, uu____3748) with
                          | ((int_to_t1,x),(uu____3759,y)) ->
                              int_as_bounded r int_to_t1 (x + y)))) in
            let uu____3765 =
              let uu____3775 =
                let uu____3784 = FStar_Syntax_Const.p2l ["FStar"; m; "sub"] in
                (uu____3784, (Prims.parse_int "2"),
                  (binary_op arg_as_bounded_int
                     (fun r  ->
                        fun uu____3793  ->
                          fun uu____3794  ->
                            match (uu____3793, uu____3794) with
                            | ((int_to_t1,x),(uu____3805,y)) ->
                                int_as_bounded r int_to_t1 (x - y)))) in
              let uu____3811 =
                let uu____3821 =
                  let uu____3830 = FStar_Syntax_Const.p2l ["FStar"; m; "mul"] in
                  (uu____3830, (Prims.parse_int "2"),
                    (binary_op arg_as_bounded_int
                       (fun r  ->
                          fun uu____3839  ->
                            fun uu____3840  ->
                              match (uu____3839, uu____3840) with
                              | ((int_to_t1,x),(uu____3851,y)) ->
                                  int_as_bounded r int_to_t1 (x * y)))) in
                [uu____3821] in
              uu____3775 :: uu____3811 in
            uu____3729 :: uu____3765)) in
  FStar_List.map as_primitive_step
    (FStar_List.append basic_ops bounded_arith_ops)
let equality_ops: primitive_step Prims.list =
  let interp_bool neg rng args =
    let tru =
      mk (FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_bool true)) rng in
    let fal =
      mk (FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_bool false)) rng in
    match args with
    | (_typ,uu____3926)::(a1,uu____3928)::(a2,uu____3930)::[] ->
        let uu____3959 = FStar_Syntax_Util.eq_tm a1 a2 in
        (match uu____3959 with
         | FStar_Syntax_Util.Equal  -> Some (if neg then fal else tru)
         | FStar_Syntax_Util.NotEqual  -> Some (if neg then tru else fal)
         | uu____3971 -> None)
    | uu____3972 -> failwith "Unexpected number of arguments" in
  let interp_prop r args =
    match args with
    | (_typ,uu____3985)::(a1,uu____3987)::(a2,uu____3989)::[] ->
        let uu____4018 = FStar_Syntax_Util.eq_tm a1 a2 in
        (match uu____4018 with
         | FStar_Syntax_Util.Equal  ->
             Some
               (let uu___161_4022 = FStar_Syntax_Util.t_true in
                {
                  FStar_Syntax_Syntax.n =
                    (uu___161_4022.FStar_Syntax_Syntax.n);
                  FStar_Syntax_Syntax.tk =
                    (uu___161_4022.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos = r;
                  FStar_Syntax_Syntax.vars =
                    (uu___161_4022.FStar_Syntax_Syntax.vars)
                })
         | FStar_Syntax_Util.NotEqual  ->
             Some
               (let uu___162_4029 = FStar_Syntax_Util.t_false in
                {
                  FStar_Syntax_Syntax.n =
                    (uu___162_4029.FStar_Syntax_Syntax.n);
                  FStar_Syntax_Syntax.tk =
                    (uu___162_4029.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos = r;
                  FStar_Syntax_Syntax.vars =
                    (uu___162_4029.FStar_Syntax_Syntax.vars)
                })
         | uu____4034 -> None)
    | (_typ,uu____4036)::uu____4037::(a1,uu____4039)::(a2,uu____4041)::[] ->
        let uu____4078 = FStar_Syntax_Util.eq_tm a1 a2 in
        (match uu____4078 with
         | FStar_Syntax_Util.Equal  ->
             Some
               (let uu___161_4082 = FStar_Syntax_Util.t_true in
                {
                  FStar_Syntax_Syntax.n =
                    (uu___161_4082.FStar_Syntax_Syntax.n);
                  FStar_Syntax_Syntax.tk =
                    (uu___161_4082.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos = r;
                  FStar_Syntax_Syntax.vars =
                    (uu___161_4082.FStar_Syntax_Syntax.vars)
                })
         | FStar_Syntax_Util.NotEqual  ->
             Some
               (let uu___162_4089 = FStar_Syntax_Util.t_false in
                {
                  FStar_Syntax_Syntax.n =
                    (uu___162_4089.FStar_Syntax_Syntax.n);
                  FStar_Syntax_Syntax.tk =
                    (uu___162_4089.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos = r;
                  FStar_Syntax_Syntax.vars =
                    (uu___162_4089.FStar_Syntax_Syntax.vars)
                })
         | uu____4094 -> None)
    | uu____4095 -> failwith "Unexpected number of arguments" in
  let decidable_equality =
    [{
       name = FStar_Syntax_Const.op_Eq;
       arity = (Prims.parse_int "3");
       strong_reduction_ok = true;
       interpretation = (interp_bool false)
     };
    {
      name = FStar_Syntax_Const.op_notEq;
      arity = (Prims.parse_int "3");
      strong_reduction_ok = true;
      interpretation = (interp_bool true)
    }] in
  let propositional_equality =
    {
      name = FStar_Syntax_Const.eq2_lid;
      arity = (Prims.parse_int "3");
      strong_reduction_ok = true;
      interpretation = interp_prop
    } in
  let hetero_propositional_equality =
    {
      name = FStar_Syntax_Const.eq3_lid;
      arity = (Prims.parse_int "4");
      strong_reduction_ok = true;
      interpretation = interp_prop
    } in
  FStar_List.append decidable_equality
    [propositional_equality; hetero_propositional_equality]
let reduce_primops:
  cfg -> FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term =
  fun cfg  ->
    fun tm  ->
      let uu____4107 =
        FStar_All.pipe_left Prims.op_Negation
          (FStar_List.contains Primops cfg.steps) in
      if uu____4107
      then tm
      else
        (let uu____4109 = FStar_Syntax_Util.head_and_args tm in
         match uu____4109 with
         | (head1,args) ->
             let uu____4135 =
               let uu____4136 = FStar_Syntax_Util.un_uinst head1 in
               uu____4136.FStar_Syntax_Syntax.n in
             (match uu____4135 with
              | FStar_Syntax_Syntax.Tm_fvar fv ->
                  let uu____4140 =
                    FStar_List.tryFind
                      (fun ps  -> FStar_Syntax_Syntax.fv_eq_lid fv ps.name)
                      cfg.primitive_steps in
                  (match uu____4140 with
                   | None  -> tm
                   | Some prim_step ->
                       if (FStar_List.length args) < prim_step.arity
                       then tm
                       else
                         (let uu____4152 =
                            prim_step.interpretation
                              head1.FStar_Syntax_Syntax.pos args in
                          match uu____4152 with
                          | None  -> tm
                          | Some reduced -> reduced))
              | uu____4155 -> tm))
let reduce_equality:
  cfg -> FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term =
  fun cfg  ->
    fun tm  ->
      reduce_primops
        (let uu___163_4162 = cfg in
         {
           steps = [Primops];
           tcenv = (uu___163_4162.tcenv);
           delta_level = (uu___163_4162.delta_level);
           primitive_steps = equality_ops
         }) tm
let maybe_simplify:
  cfg ->
    (FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
      FStar_Syntax_Syntax.syntax -> FStar_Syntax_Syntax.term
  =
  fun cfg  ->
    fun tm  ->
      let steps = cfg.steps in
      let w t =
        let uu___164_4184 = t in
        {
          FStar_Syntax_Syntax.n = (uu___164_4184.FStar_Syntax_Syntax.n);
          FStar_Syntax_Syntax.tk = (uu___164_4184.FStar_Syntax_Syntax.tk);
          FStar_Syntax_Syntax.pos = (tm.FStar_Syntax_Syntax.pos);
          FStar_Syntax_Syntax.vars = (uu___164_4184.FStar_Syntax_Syntax.vars)
        } in
      let simp_t t =
        match t.FStar_Syntax_Syntax.n with
        | FStar_Syntax_Syntax.Tm_fvar fv when
            FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.true_lid ->
            Some true
        | FStar_Syntax_Syntax.Tm_fvar fv when
            FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.false_lid ->
            Some false
        | uu____4203 -> None in
      let simplify arg = ((simp_t (fst arg)), arg) in
      let tm1 = reduce_primops cfg tm in
      let uu____4231 =
        FStar_All.pipe_left Prims.op_Negation
          (FStar_List.contains Simplify steps) in
      if uu____4231
      then tm1
      else
        (match tm1.FStar_Syntax_Syntax.n with
         | FStar_Syntax_Syntax.Tm_app
             ({
                FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_uinst
                  ({ FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar fv;
                     FStar_Syntax_Syntax.tk = uu____4234;
                     FStar_Syntax_Syntax.pos = uu____4235;
                     FStar_Syntax_Syntax.vars = uu____4236;_},uu____4237);
                FStar_Syntax_Syntax.tk = uu____4238;
                FStar_Syntax_Syntax.pos = uu____4239;
                FStar_Syntax_Syntax.vars = uu____4240;_},args)
             ->
             let uu____4260 =
               FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.and_lid in
             if uu____4260
             then
               let uu____4261 =
                 FStar_All.pipe_right args (FStar_List.map simplify) in
               (match uu____4261 with
                | (Some (true ),uu____4294)::(uu____4295,(arg,uu____4297))::[]
                    -> arg
                | (uu____4338,(arg,uu____4340))::(Some (true ),uu____4341)::[]
                    -> arg
                | (Some (false ),uu____4382)::uu____4383::[] ->
                    w FStar_Syntax_Util.t_false
                | uu____4421::(Some (false ),uu____4422)::[] ->
                    w FStar_Syntax_Util.t_false
                | uu____4460 -> tm1)
             else
               (let uu____4470 =
                  FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.or_lid in
                if uu____4470
                then
                  let uu____4471 =
                    FStar_All.pipe_right args (FStar_List.map simplify) in
                  match uu____4471 with
                  | (Some (true ),uu____4504)::uu____4505::[] ->
                      w FStar_Syntax_Util.t_true
                  | uu____4543::(Some (true ),uu____4544)::[] ->
                      w FStar_Syntax_Util.t_true
                  | (Some (false ),uu____4582)::(uu____4583,(arg,uu____4585))::[]
                      -> arg
                  | (uu____4626,(arg,uu____4628))::(Some (false ),uu____4629)::[]
                      -> arg
                  | uu____4670 -> tm1
                else
                  (let uu____4680 =
                     FStar_Syntax_Syntax.fv_eq_lid fv
                       FStar_Syntax_Const.imp_lid in
                   if uu____4680
                   then
                     let uu____4681 =
                       FStar_All.pipe_right args (FStar_List.map simplify) in
                     match uu____4681 with
                     | uu____4714::(Some (true ),uu____4715)::[] ->
                         w FStar_Syntax_Util.t_true
                     | (Some (false ),uu____4753)::uu____4754::[] ->
                         w FStar_Syntax_Util.t_true
                     | (Some (true ),uu____4792)::(uu____4793,(arg,uu____4795))::[]
                         -> arg
                     | (uu____4836,(p,uu____4838))::(uu____4839,(q,uu____4841))::[]
                         ->
                         let uu____4883 = FStar_Syntax_Util.term_eq p q in
                         (if uu____4883
                          then w FStar_Syntax_Util.t_true
                          else tm1)
                     | uu____4885 -> tm1
                   else
                     (let uu____4895 =
                        FStar_Syntax_Syntax.fv_eq_lid fv
                          FStar_Syntax_Const.not_lid in
                      if uu____4895
                      then
                        let uu____4896 =
                          FStar_All.pipe_right args (FStar_List.map simplify) in
                        match uu____4896 with
                        | (Some (true ),uu____4929)::[] ->
                            w FStar_Syntax_Util.t_false
                        | (Some (false ),uu____4953)::[] ->
                            w FStar_Syntax_Util.t_true
                        | uu____4977 -> tm1
                      else
                        (let uu____4987 =
                           FStar_Syntax_Syntax.fv_eq_lid fv
                             FStar_Syntax_Const.forall_lid in
                         if uu____4987
                         then
                           match args with
                           | (t,uu____4989)::[] ->
                               let uu____5002 =
                                 let uu____5003 =
                                   FStar_Syntax_Subst.compress t in
                                 uu____5003.FStar_Syntax_Syntax.n in
                               (match uu____5002 with
                                | FStar_Syntax_Syntax.Tm_abs
                                    (uu____5006::[],body,uu____5008) ->
                                    (match simp_t body with
                                     | Some (true ) ->
                                         w FStar_Syntax_Util.t_true
                                     | uu____5034 -> tm1)
                                | uu____5036 -> tm1)
                           | (uu____5037,Some (FStar_Syntax_Syntax.Implicit
                              uu____5038))::(t,uu____5040)::[] ->
                               let uu____5067 =
                                 let uu____5068 =
                                   FStar_Syntax_Subst.compress t in
                                 uu____5068.FStar_Syntax_Syntax.n in
                               (match uu____5067 with
                                | FStar_Syntax_Syntax.Tm_abs
                                    (uu____5071::[],body,uu____5073) ->
                                    (match simp_t body with
                                     | Some (true ) ->
                                         w FStar_Syntax_Util.t_true
                                     | uu____5099 -> tm1)
                                | uu____5101 -> tm1)
                           | uu____5102 -> tm1
                         else
                           (let uu____5109 =
                              FStar_Syntax_Syntax.fv_eq_lid fv
                                FStar_Syntax_Const.exists_lid in
                            if uu____5109
                            then
                              match args with
                              | (t,uu____5111)::[] ->
                                  let uu____5124 =
                                    let uu____5125 =
                                      FStar_Syntax_Subst.compress t in
                                    uu____5125.FStar_Syntax_Syntax.n in
                                  (match uu____5124 with
                                   | FStar_Syntax_Syntax.Tm_abs
                                       (uu____5128::[],body,uu____5130) ->
                                       (match simp_t body with
                                        | Some (false ) ->
                                            w FStar_Syntax_Util.t_false
                                        | uu____5156 -> tm1)
                                   | uu____5158 -> tm1)
                              | (uu____5159,Some
                                 (FStar_Syntax_Syntax.Implicit uu____5160))::
                                  (t,uu____5162)::[] ->
                                  let uu____5189 =
                                    let uu____5190 =
                                      FStar_Syntax_Subst.compress t in
                                    uu____5190.FStar_Syntax_Syntax.n in
                                  (match uu____5189 with
                                   | FStar_Syntax_Syntax.Tm_abs
                                       (uu____5193::[],body,uu____5195) ->
                                       (match simp_t body with
                                        | Some (false ) ->
                                            w FStar_Syntax_Util.t_false
                                        | uu____5221 -> tm1)
                                   | uu____5223 -> tm1)
                              | uu____5224 -> tm1
                            else reduce_equality cfg tm1)))))
         | FStar_Syntax_Syntax.Tm_app
             ({ FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar fv;
                FStar_Syntax_Syntax.tk = uu____5232;
                FStar_Syntax_Syntax.pos = uu____5233;
                FStar_Syntax_Syntax.vars = uu____5234;_},args)
             ->
             let uu____5250 =
               FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.and_lid in
             if uu____5250
             then
               let uu____5251 =
                 FStar_All.pipe_right args (FStar_List.map simplify) in
               (match uu____5251 with
                | (Some (true ),uu____5284)::(uu____5285,(arg,uu____5287))::[]
                    -> arg
                | (uu____5328,(arg,uu____5330))::(Some (true ),uu____5331)::[]
                    -> arg
                | (Some (false ),uu____5372)::uu____5373::[] ->
                    w FStar_Syntax_Util.t_false
                | uu____5411::(Some (false ),uu____5412)::[] ->
                    w FStar_Syntax_Util.t_false
                | uu____5450 -> tm1)
             else
               (let uu____5460 =
                  FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.or_lid in
                if uu____5460
                then
                  let uu____5461 =
                    FStar_All.pipe_right args (FStar_List.map simplify) in
                  match uu____5461 with
                  | (Some (true ),uu____5494)::uu____5495::[] ->
                      w FStar_Syntax_Util.t_true
                  | uu____5533::(Some (true ),uu____5534)::[] ->
                      w FStar_Syntax_Util.t_true
                  | (Some (false ),uu____5572)::(uu____5573,(arg,uu____5575))::[]
                      -> arg
                  | (uu____5616,(arg,uu____5618))::(Some (false ),uu____5619)::[]
                      -> arg
                  | uu____5660 -> tm1
                else
                  (let uu____5670 =
                     FStar_Syntax_Syntax.fv_eq_lid fv
                       FStar_Syntax_Const.imp_lid in
                   if uu____5670
                   then
                     let uu____5671 =
                       FStar_All.pipe_right args (FStar_List.map simplify) in
                     match uu____5671 with
                     | uu____5704::(Some (true ),uu____5705)::[] ->
                         w FStar_Syntax_Util.t_true
                     | (Some (false ),uu____5743)::uu____5744::[] ->
                         w FStar_Syntax_Util.t_true
                     | (Some (true ),uu____5782)::(uu____5783,(arg,uu____5785))::[]
                         -> arg
                     | (uu____5826,(p,uu____5828))::(uu____5829,(q,uu____5831))::[]
                         ->
                         let uu____5873 = FStar_Syntax_Util.term_eq p q in
                         (if uu____5873
                          then w FStar_Syntax_Util.t_true
                          else tm1)
                     | uu____5875 -> tm1
                   else
                     (let uu____5885 =
                        FStar_Syntax_Syntax.fv_eq_lid fv
                          FStar_Syntax_Const.not_lid in
                      if uu____5885
                      then
                        let uu____5886 =
                          FStar_All.pipe_right args (FStar_List.map simplify) in
                        match uu____5886 with
                        | (Some (true ),uu____5919)::[] ->
                            w FStar_Syntax_Util.t_false
                        | (Some (false ),uu____5943)::[] ->
                            w FStar_Syntax_Util.t_true
                        | uu____5967 -> tm1
                      else
                        (let uu____5977 =
                           FStar_Syntax_Syntax.fv_eq_lid fv
                             FStar_Syntax_Const.forall_lid in
                         if uu____5977
                         then
                           match args with
                           | (t,uu____5979)::[] ->
                               let uu____5992 =
                                 let uu____5993 =
                                   FStar_Syntax_Subst.compress t in
                                 uu____5993.FStar_Syntax_Syntax.n in
                               (match uu____5992 with
                                | FStar_Syntax_Syntax.Tm_abs
                                    (uu____5996::[],body,uu____5998) ->
                                    (match simp_t body with
                                     | Some (true ) ->
                                         w FStar_Syntax_Util.t_true
                                     | uu____6024 -> tm1)
                                | uu____6026 -> tm1)
                           | (uu____6027,Some (FStar_Syntax_Syntax.Implicit
                              uu____6028))::(t,uu____6030)::[] ->
                               let uu____6057 =
                                 let uu____6058 =
                                   FStar_Syntax_Subst.compress t in
                                 uu____6058.FStar_Syntax_Syntax.n in
                               (match uu____6057 with
                                | FStar_Syntax_Syntax.Tm_abs
                                    (uu____6061::[],body,uu____6063) ->
                                    (match simp_t body with
                                     | Some (true ) ->
                                         w FStar_Syntax_Util.t_true
                                     | uu____6089 -> tm1)
                                | uu____6091 -> tm1)
                           | uu____6092 -> tm1
                         else
                           (let uu____6099 =
                              FStar_Syntax_Syntax.fv_eq_lid fv
                                FStar_Syntax_Const.exists_lid in
                            if uu____6099
                            then
                              match args with
                              | (t,uu____6101)::[] ->
                                  let uu____6114 =
                                    let uu____6115 =
                                      FStar_Syntax_Subst.compress t in
                                    uu____6115.FStar_Syntax_Syntax.n in
                                  (match uu____6114 with
                                   | FStar_Syntax_Syntax.Tm_abs
                                       (uu____6118::[],body,uu____6120) ->
                                       (match simp_t body with
                                        | Some (false ) ->
                                            w FStar_Syntax_Util.t_false
                                        | uu____6146 -> tm1)
                                   | uu____6148 -> tm1)
                              | (uu____6149,Some
                                 (FStar_Syntax_Syntax.Implicit uu____6150))::
                                  (t,uu____6152)::[] ->
                                  let uu____6179 =
                                    let uu____6180 =
                                      FStar_Syntax_Subst.compress t in
                                    uu____6180.FStar_Syntax_Syntax.n in
                                  (match uu____6179 with
                                   | FStar_Syntax_Syntax.Tm_abs
                                       (uu____6183::[],body,uu____6185) ->
                                       (match simp_t body with
                                        | Some (false ) ->
                                            w FStar_Syntax_Util.t_false
                                        | uu____6211 -> tm1)
                                   | uu____6213 -> tm1)
                              | uu____6214 -> tm1
                            else reduce_equality cfg tm1)))))
         | uu____6221 -> tm1)
let is_norm_request hd1 args =
  let uu____6236 =
    let uu____6240 =
      let uu____6241 = FStar_Syntax_Util.un_uinst hd1 in
      uu____6241.FStar_Syntax_Syntax.n in
    (uu____6240, args) in
  match uu____6236 with
  | (FStar_Syntax_Syntax.Tm_fvar fv,uu____6246::uu____6247::[]) ->
      FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.normalize_term
  | (FStar_Syntax_Syntax.Tm_fvar fv,uu____6250::[]) ->
      FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.normalize
  | uu____6252 -> false
let get_norm_request args =
  match args with
  | uu____6271::(tm,uu____6273)::[] -> tm
  | (tm,uu____6283)::[] -> tm
  | uu____6288 -> failwith "Impossible"
let is_reify_head: stack_elt Prims.list -> Prims.bool =
  fun uu___140_6295  ->
    match uu___140_6295 with
    | (App
        ({
           FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_constant
             (FStar_Const.Const_reify );
           FStar_Syntax_Syntax.tk = uu____6297;
           FStar_Syntax_Syntax.pos = uu____6298;
           FStar_Syntax_Syntax.vars = uu____6299;_},uu____6300,uu____6301))::uu____6302
        -> true
    | uu____6308 -> false
let rec norm:
  cfg -> env -> stack -> FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun cfg  ->
    fun env  ->
      fun stack1  ->
        fun t  ->
          let t1 = FStar_Syntax_Subst.compress t in
          let firstn k l =
            if (FStar_List.length l) < k
            then (l, [])
            else FStar_Util.first_N k l in
          log cfg
            (fun uu____6423  ->
               let uu____6424 = FStar_Syntax_Print.tag_of_term t1 in
               let uu____6425 = FStar_Syntax_Print.term_to_string t1 in
               let uu____6426 =
                 let uu____6427 =
                   let uu____6429 = firstn (Prims.parse_int "4") stack1 in
                   FStar_All.pipe_left FStar_Pervasives.fst uu____6429 in
                 stack_to_string uu____6427 in
               FStar_Util.print3
                 ">>> %s\nNorm %s with top of the stack %s \n" uu____6424
                 uu____6425 uu____6426);
          (match t1.FStar_Syntax_Syntax.n with
           | FStar_Syntax_Syntax.Tm_delayed uu____6441 ->
               failwith "Impossible: got a delayed substitution"
           | FStar_Syntax_Syntax.Tm_unknown  -> rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_uvar uu____6462 ->
               rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_constant uu____6471 ->
               rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_name uu____6472 ->
               rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_fvar
               { FStar_Syntax_Syntax.fv_name = uu____6473;
                 FStar_Syntax_Syntax.fv_delta =
                   FStar_Syntax_Syntax.Delta_constant ;
                 FStar_Syntax_Syntax.fv_qual = uu____6474;_}
               -> rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_fvar
               { FStar_Syntax_Syntax.fv_name = uu____6479;
                 FStar_Syntax_Syntax.fv_delta = uu____6480;
                 FStar_Syntax_Syntax.fv_qual = Some
                   (FStar_Syntax_Syntax.Data_ctor );_}
               -> rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_fvar
               { FStar_Syntax_Syntax.fv_name = uu____6484;
                 FStar_Syntax_Syntax.fv_delta = uu____6485;
                 FStar_Syntax_Syntax.fv_qual = Some
                   (FStar_Syntax_Syntax.Record_ctor uu____6486);_}
               -> rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_app (hd1,args) when
               is_fstar_tactics_embed hd1 ->
               let args1 = closures_as_args_delayed cfg env args in
               let hd2 = closure_as_term cfg env hd1 in
               let t2 =
                 let uu___165_6519 = t1 in
                 {
                   FStar_Syntax_Syntax.n =
                     (FStar_Syntax_Syntax.Tm_app (hd2, args1));
                   FStar_Syntax_Syntax.tk =
                     (uu___165_6519.FStar_Syntax_Syntax.tk);
                   FStar_Syntax_Syntax.pos =
                     (uu___165_6519.FStar_Syntax_Syntax.pos);
                   FStar_Syntax_Syntax.vars =
                     (uu___165_6519.FStar_Syntax_Syntax.vars)
                 } in
               rebuild cfg env stack1 t2
           | FStar_Syntax_Syntax.Tm_app (hd1,args) when
               ((let uu____6545 =
                   FStar_All.pipe_right cfg.steps
                     (FStar_List.contains NoFullNorm) in
                 Prims.op_Negation uu____6545) && (is_norm_request hd1 args))
                 &&
                 (Prims.op_Negation
                    (FStar_Ident.lid_equals
                       (cfg.tcenv).FStar_TypeChecker_Env.curmodule
                       FStar_Syntax_Const.prims_lid))
               ->
               let tm = get_norm_request args in
               let s =
                 [Reify;
                 UnfoldUntil FStar_Syntax_Syntax.Delta_constant;
                 Primops] in
               let cfg' =
                 let uu___166_6558 = cfg in
                 {
                   steps = s;
                   tcenv = (uu___166_6558.tcenv);
                   delta_level =
                     [FStar_TypeChecker_Env.Unfold
                        FStar_Syntax_Syntax.Delta_constant];
                   primitive_steps = (uu___166_6558.primitive_steps)
                 } in
               let stack' = (Debug t1) ::
                 (Steps
                    ((cfg.steps), (cfg.primitive_steps), (cfg.delta_level)))
                 :: stack1 in
               norm cfg' env stack' tm
           | FStar_Syntax_Syntax.Tm_app
               ({
                  FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_constant
                    (FStar_Const.Const_reify );
                  FStar_Syntax_Syntax.tk = uu____6563;
                  FStar_Syntax_Syntax.pos = uu____6564;
                  FStar_Syntax_Syntax.vars = uu____6565;_},a1::a2::rest)
               ->
               let uu____6599 = FStar_Syntax_Util.head_and_args t1 in
               (match uu____6599 with
                | (hd1,uu____6610) ->
                    let t' =
                      FStar_Syntax_Syntax.mk
                        (FStar_Syntax_Syntax.Tm_app (hd1, [a1])) None
                        t1.FStar_Syntax_Syntax.pos in
                    let t2 =
                      FStar_Syntax_Syntax.mk
                        (FStar_Syntax_Syntax.Tm_app (t', (a2 :: rest))) None
                        t1.FStar_Syntax_Syntax.pos in
                    norm cfg env stack1 t2)
           | FStar_Syntax_Syntax.Tm_app
               ({
                  FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_constant
                    (FStar_Const.Const_reflect uu____6665);
                  FStar_Syntax_Syntax.tk = uu____6666;
                  FStar_Syntax_Syntax.pos = uu____6667;
                  FStar_Syntax_Syntax.vars = uu____6668;_},a::[])
               when
               (FStar_All.pipe_right cfg.steps (FStar_List.contains Reify))
                 && (is_reify_head stack1)
               ->
               let uu____6691 = FStar_List.tl stack1 in
               norm cfg env uu____6691 (fst a)
           | FStar_Syntax_Syntax.Tm_app
               ({
                  FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_constant
                    (FStar_Const.Const_reify );
                  FStar_Syntax_Syntax.tk = uu____6694;
                  FStar_Syntax_Syntax.pos = uu____6695;
                  FStar_Syntax_Syntax.vars = uu____6696;_},a::[])
               when
               FStar_All.pipe_right cfg.steps (FStar_List.contains Reify) ->
               let uu____6719 = FStar_Syntax_Util.head_and_args t1 in
               (match uu____6719 with
                | (reify_head,uu____6730) ->
                    let a1 =
                      let uu____6746 =
                        FStar_All.pipe_left FStar_Syntax_Util.unascribe
                          (fst a) in
                      FStar_Syntax_Subst.compress uu____6746 in
                    (match a1.FStar_Syntax_Syntax.n with
                     | FStar_Syntax_Syntax.Tm_app
                         ({
                            FStar_Syntax_Syntax.n =
                              FStar_Syntax_Syntax.Tm_constant
                              (FStar_Const.Const_reflect uu____6749);
                            FStar_Syntax_Syntax.tk = uu____6750;
                            FStar_Syntax_Syntax.pos = uu____6751;
                            FStar_Syntax_Syntax.vars = uu____6752;_},a2::[])
                         -> norm cfg env stack1 (fst a2)
                     | uu____6777 ->
                         let stack2 =
                           (App
                              (reify_head, None,
                                (t1.FStar_Syntax_Syntax.pos)))
                           :: stack1 in
                         norm cfg env stack2 a1))
           | FStar_Syntax_Syntax.Tm_type u ->
               let u1 = norm_universe cfg env u in
               let uu____6785 =
                 mk (FStar_Syntax_Syntax.Tm_type u1)
                   t1.FStar_Syntax_Syntax.pos in
               rebuild cfg env stack1 uu____6785
           | FStar_Syntax_Syntax.Tm_uinst (t',us) ->
               let uu____6792 =
                 FStar_All.pipe_right cfg.steps
                   (FStar_List.contains EraseUniverses) in
               if uu____6792
               then norm cfg env stack1 t'
               else
                 (let us1 =
                    let uu____6795 =
                      let uu____6799 =
                        FStar_List.map (norm_universe cfg env) us in
                      (uu____6799, (t1.FStar_Syntax_Syntax.pos)) in
                    UnivArgs uu____6795 in
                  let stack2 = us1 :: stack1 in norm cfg env stack2 t')
           | FStar_Syntax_Syntax.Tm_fvar f ->
               let t0 = t1 in
               let should_delta =
                 FStar_All.pipe_right cfg.delta_level
                   (FStar_Util.for_some
                      (fun uu___141_6808  ->
                         match uu___141_6808 with
                         | FStar_TypeChecker_Env.UnfoldTac  -> false
                         | FStar_TypeChecker_Env.NoDelta  -> false
                         | FStar_TypeChecker_Env.Inlining  -> true
                         | FStar_TypeChecker_Env.Eager_unfolding_only  ->
                             true
                         | FStar_TypeChecker_Env.Unfold l ->
                             FStar_TypeChecker_Common.delta_depth_greater_than
                               f.FStar_Syntax_Syntax.fv_delta l)) in
               let should_delta1 =
                 let uu____6811 =
                   (FStar_List.mem FStar_TypeChecker_Env.UnfoldTac
                      cfg.delta_level)
                     &&
                     (((((((((FStar_Syntax_Syntax.fv_eq_lid f
                                FStar_Syntax_Const.and_lid)
                               ||
                               (FStar_Syntax_Syntax.fv_eq_lid f
                                  FStar_Syntax_Const.or_lid))
                              ||
                              (FStar_Syntax_Syntax.fv_eq_lid f
                                 FStar_Syntax_Const.imp_lid))
                             ||
                             (FStar_Syntax_Syntax.fv_eq_lid f
                                FStar_Syntax_Const.forall_lid))
                            ||
                            (FStar_Syntax_Syntax.fv_eq_lid f
                               FStar_Syntax_Const.squash_lid))
                           ||
                           (FStar_Syntax_Syntax.fv_eq_lid f
                              FStar_Syntax_Const.exists_lid))
                          ||
                          (FStar_Syntax_Syntax.fv_eq_lid f
                             FStar_Syntax_Const.eq2_lid))
                         ||
                         (FStar_Syntax_Syntax.fv_eq_lid f
                            FStar_Syntax_Const.true_lid))
                        ||
                        (FStar_Syntax_Syntax.fv_eq_lid f
                           FStar_Syntax_Const.false_lid)) in
                 if uu____6811 then false else should_delta in
               if Prims.op_Negation should_delta1
               then rebuild cfg env stack1 t1
               else
                 (let r_env =
                    let uu____6815 = FStar_Syntax_Syntax.range_of_fv f in
                    FStar_TypeChecker_Env.set_range cfg.tcenv uu____6815 in
                  let uu____6816 =
                    FStar_TypeChecker_Env.lookup_definition cfg.delta_level
                      r_env
                      (f.FStar_Syntax_Syntax.fv_name).FStar_Syntax_Syntax.v in
                  match uu____6816 with
                  | None  ->
                      (log cfg
                         (fun uu____6827  ->
                            FStar_Util.print "Tm_fvar case 2\n" []);
                       rebuild cfg env stack1 t1)
                  | Some (us,t2) ->
                      (log cfg
                         (fun uu____6833  ->
                            let uu____6834 =
                              FStar_Syntax_Print.term_to_string t0 in
                            let uu____6835 =
                              FStar_Syntax_Print.term_to_string t2 in
                            FStar_Util.print2 ">>> Unfolded %s to %s\n"
                              uu____6834 uu____6835);
                       (let t3 =
                          let uu____6837 =
                            FStar_All.pipe_right cfg.steps
                              (FStar_List.contains
                                 (UnfoldUntil
                                    FStar_Syntax_Syntax.Delta_constant)) in
                          if uu____6837
                          then t2
                          else
                            FStar_Syntax_Subst.set_use_range
                              (FStar_Ident.range_of_lid
                                 (f.FStar_Syntax_Syntax.fv_name).FStar_Syntax_Syntax.v)
                              t2 in
                        let n1 = FStar_List.length us in
                        if n1 > (Prims.parse_int "0")
                        then
                          match stack1 with
                          | (UnivArgs (us',uu____6849))::stack2 ->
                              let env1 =
                                FStar_All.pipe_right us'
                                  (FStar_List.fold_left
                                     (fun env1  -> fun u  -> (Univ u) :: env1)
                                     env) in
                              norm cfg env1 stack2 t3
                          | uu____6862 when
                              FStar_All.pipe_right cfg.steps
                                (FStar_List.contains EraseUniverses)
                              -> norm cfg env stack1 t3
                          | uu____6863 ->
                              let uu____6864 =
                                let uu____6865 =
                                  FStar_Syntax_Print.lid_to_string
                                    (f.FStar_Syntax_Syntax.fv_name).FStar_Syntax_Syntax.v in
                                FStar_Util.format1
                                  "Impossible: missing universe instantiation on %s"
                                  uu____6865 in
                              failwith uu____6864
                        else norm cfg env stack1 t3)))
           | FStar_Syntax_Syntax.Tm_bvar x ->
               let uu____6872 = lookup_bvar env x in
               (match uu____6872 with
                | Univ uu____6873 ->
                    failwith
                      "Impossible: term variable is bound to a universe"
                | Dummy  -> failwith "Term variable not found"
                | Clos (env1,t0,r,fix) ->
                    if
                      (Prims.op_Negation fix) ||
                        (Prims.op_Negation
                           (FStar_List.contains (Exclude Zeta) cfg.steps))
                    then
                      let uu____6888 = FStar_ST.read r in
                      (match uu____6888 with
                       | Some (env2,t') ->
                           (log cfg
                              (fun uu____6907  ->
                                 let uu____6908 =
                                   FStar_Syntax_Print.term_to_string t1 in
                                 let uu____6909 =
                                   FStar_Syntax_Print.term_to_string t' in
                                 FStar_Util.print2
                                   "Lazy hit: %s cached to %s\n" uu____6908
                                   uu____6909);
                            (let uu____6910 =
                               let uu____6911 =
                                 FStar_Syntax_Subst.compress t' in
                               uu____6911.FStar_Syntax_Syntax.n in
                             match uu____6910 with
                             | FStar_Syntax_Syntax.Tm_abs uu____6914 ->
                                 norm cfg env2 stack1 t'
                             | uu____6929 -> rebuild cfg env2 stack1 t'))
                       | None  -> norm cfg env1 ((MemoLazy r) :: stack1) t0)
                    else norm cfg env1 stack1 t0)
           | FStar_Syntax_Syntax.Tm_abs (bs,body,lopt) ->
               (match stack1 with
                | (UnivArgs uu____6962)::uu____6963 ->
                    failwith
                      "Ill-typed term: universes cannot be applied to term abstraction"
                | (Match uu____6968)::uu____6969 ->
                    failwith
                      "Ill-typed term: cannot pattern match an abstraction"
                | (Arg (c,uu____6975,uu____6976))::stack_rest ->
                    (match c with
                     | Univ uu____6979 -> norm cfg (c :: env) stack_rest t1
                     | uu____6980 ->
                         (match bs with
                          | [] -> failwith "Impossible"
                          | uu____6983::[] ->
                              (match lopt with
                               | None  when FStar_Options.__unit_tests () ->
                                   (log cfg
                                      (fun uu____6996  ->
                                         let uu____6997 = closure_to_string c in
                                         FStar_Util.print1 "\tShifted %s\n"
                                           uu____6997);
                                    norm cfg (c :: env) stack_rest body)
                               | Some (FStar_Util.Inr (l,cflags)) when
                                   ((FStar_Ident.lid_equals l
                                       FStar_Syntax_Const.effect_Tot_lid)
                                      ||
                                      (FStar_Ident.lid_equals l
                                         FStar_Syntax_Const.effect_GTot_lid))
                                     ||
                                     (FStar_All.pipe_right cflags
                                        (FStar_Util.for_some
                                           (fun uu___142_7011  ->
                                              match uu___142_7011 with
                                              | FStar_Syntax_Syntax.TOTAL  ->
                                                  true
                                              | uu____7012 -> false)))
                                   ->
                                   (log cfg
                                      (fun uu____7014  ->
                                         let uu____7015 = closure_to_string c in
                                         FStar_Util.print1 "\tShifted %s\n"
                                           uu____7015);
                                    norm cfg (c :: env) stack_rest body)
                               | Some (FStar_Util.Inl lc) when
                                   FStar_Syntax_Util.is_tot_or_gtot_lcomp lc
                                   ->
                                   (log cfg
                                      (fun uu____7026  ->
                                         let uu____7027 = closure_to_string c in
                                         FStar_Util.print1 "\tShifted %s\n"
                                           uu____7027);
                                    norm cfg (c :: env) stack_rest body)
                               | uu____7028 when
                                   FStar_All.pipe_right cfg.steps
                                     (FStar_List.contains Reify)
                                   -> norm cfg (c :: env) stack_rest body
                               | uu____7035 ->
                                   let cfg1 =
                                     let uu___167_7043 = cfg in
                                     {
                                       steps = (WHNF :: (Exclude Iota) ::
                                         (Exclude Zeta) :: (cfg.steps));
                                       tcenv = (uu___167_7043.tcenv);
                                       delta_level =
                                         (uu___167_7043.delta_level);
                                       primitive_steps =
                                         (uu___167_7043.primitive_steps)
                                     } in
                                   let uu____7044 =
                                     closure_as_term cfg1 env t1 in
                                   rebuild cfg1 env stack1 uu____7044)
                          | uu____7045::tl1 ->
                              (log cfg
                                 (fun uu____7055  ->
                                    let uu____7056 = closure_to_string c in
                                    FStar_Util.print1 "\tShifted %s\n"
                                      uu____7056);
                               (let body1 =
                                  mk
                                    (FStar_Syntax_Syntax.Tm_abs
                                       (tl1, body, lopt))
                                    t1.FStar_Syntax_Syntax.pos in
                                norm cfg (c :: env) stack_rest body1))))
                | (Steps (s,ps,dl))::stack2 ->
                    norm
                      (let uu___168_7080 = cfg in
                       {
                         steps = s;
                         tcenv = (uu___168_7080.tcenv);
                         delta_level = dl;
                         primitive_steps = ps
                       }) env stack2 t1
                | (MemoLazy r)::stack2 ->
                    (set_memo r (env, t1);
                     log cfg
                       (fun uu____7093  ->
                          let uu____7094 =
                            FStar_Syntax_Print.term_to_string t1 in
                          FStar_Util.print1 "\tSet memo %s\n" uu____7094);
                     norm cfg env stack2 t1)
                | (Debug uu____7095)::uu____7096 ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7098 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7098
                    else
                      (let uu____7100 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7100 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7129 =
                                   let uu____7135 =
                                     let uu____7136 =
                                       let uu____7137 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7137 in
                                     FStar_All.pipe_right uu____7136
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7135
                                     (fun _0_31  -> FStar_Util.Inl _0_31) in
                                 FStar_All.pipe_right uu____7129
                                   (fun _0_32  -> Some _0_32)
                             | uu____7162 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7176  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7181  ->
                                 let uu____7182 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7182);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7192 = cfg in
                               let uu____7193 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7192.steps);
                                 tcenv = (uu___169_7192.tcenv);
                                 delta_level = (uu___169_7192.delta_level);
                                 primitive_steps = uu____7193
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1)))
                | (Meta uu____7203)::uu____7204 ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7208 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7208
                    else
                      (let uu____7210 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7210 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7239 =
                                   let uu____7245 =
                                     let uu____7246 =
                                       let uu____7247 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7247 in
                                     FStar_All.pipe_right uu____7246
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7245
                                     (fun _0_33  -> FStar_Util.Inl _0_33) in
                                 FStar_All.pipe_right uu____7239
                                   (fun _0_34  -> Some _0_34)
                             | uu____7272 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7286  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7291  ->
                                 let uu____7292 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7292);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7302 = cfg in
                               let uu____7303 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7302.steps);
                                 tcenv = (uu___169_7302.tcenv);
                                 delta_level = (uu___169_7302.delta_level);
                                 primitive_steps = uu____7303
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1)))
                | (Let uu____7313)::uu____7314 ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7320 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7320
                    else
                      (let uu____7322 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7322 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7351 =
                                   let uu____7357 =
                                     let uu____7358 =
                                       let uu____7359 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7359 in
                                     FStar_All.pipe_right uu____7358
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7357
                                     (fun _0_35  -> FStar_Util.Inl _0_35) in
                                 FStar_All.pipe_right uu____7351
                                   (fun _0_36  -> Some _0_36)
                             | uu____7384 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7398  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7403  ->
                                 let uu____7404 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7404);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7414 = cfg in
                               let uu____7415 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7414.steps);
                                 tcenv = (uu___169_7414.tcenv);
                                 delta_level = (uu___169_7414.delta_level);
                                 primitive_steps = uu____7415
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1)))
                | (App uu____7425)::uu____7426 ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7431 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7431
                    else
                      (let uu____7433 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7433 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7462 =
                                   let uu____7468 =
                                     let uu____7469 =
                                       let uu____7470 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7470 in
                                     FStar_All.pipe_right uu____7469
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7468
                                     (fun _0_37  -> FStar_Util.Inl _0_37) in
                                 FStar_All.pipe_right uu____7462
                                   (fun _0_38  -> Some _0_38)
                             | uu____7495 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7509  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7514  ->
                                 let uu____7515 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7515);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7525 = cfg in
                               let uu____7526 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7525.steps);
                                 tcenv = (uu___169_7525.tcenv);
                                 delta_level = (uu___169_7525.delta_level);
                                 primitive_steps = uu____7526
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1)))
                | (Abs uu____7536)::uu____7537 ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7547 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7547
                    else
                      (let uu____7549 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7549 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7578 =
                                   let uu____7584 =
                                     let uu____7585 =
                                       let uu____7586 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7586 in
                                     FStar_All.pipe_right uu____7585
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7584
                                     (fun _0_39  -> FStar_Util.Inl _0_39) in
                                 FStar_All.pipe_right uu____7578
                                   (fun _0_40  -> Some _0_40)
                             | uu____7611 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7625  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7630  ->
                                 let uu____7631 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7631);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7641 = cfg in
                               let uu____7642 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7641.steps);
                                 tcenv = (uu___169_7641.tcenv);
                                 delta_level = (uu___169_7641.delta_level);
                                 primitive_steps = uu____7642
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1)))
                | [] ->
                    if FStar_List.contains WHNF cfg.steps
                    then
                      let uu____7652 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7652
                    else
                      (let uu____7654 = FStar_Syntax_Subst.open_term' bs body in
                       match uu____7654 with
                       | (bs1,body1,opening) ->
                           let lopt1 =
                             match lopt with
                             | Some (FStar_Util.Inl l) ->
                                 let uu____7683 =
                                   let uu____7689 =
                                     let uu____7690 =
                                       let uu____7691 =
                                         l.FStar_Syntax_Syntax.comp () in
                                       FStar_Syntax_Subst.subst_comp opening
                                         uu____7691 in
                                     FStar_All.pipe_right uu____7690
                                       FStar_Syntax_Util.lcomp_of_comp in
                                   FStar_All.pipe_right uu____7689
                                     (fun _0_41  -> FStar_Util.Inl _0_41) in
                                 FStar_All.pipe_right uu____7683
                                   (fun _0_42  -> Some _0_42)
                             | uu____7716 -> lopt in
                           let env' =
                             FStar_All.pipe_right bs1
                               (FStar_List.fold_left
                                  (fun env1  ->
                                     fun uu____7730  -> Dummy :: env1) env) in
                           (log cfg
                              (fun uu____7735  ->
                                 let uu____7736 =
                                   FStar_All.pipe_left
                                     FStar_Util.string_of_int
                                     (FStar_List.length bs1) in
                                 FStar_Util.print1 "\tShifted %s dummies\n"
                                   uu____7736);
                            (let stack2 =
                               (Steps
                                  ((cfg.steps), (cfg.primitive_steps),
                                    (cfg.delta_level)))
                               :: stack1 in
                             let cfg1 =
                               let uu___169_7746 = cfg in
                               let uu____7747 =
                                 FStar_List.filter
                                   (fun ps  -> ps.strong_reduction_ok)
                                   cfg.primitive_steps in
                               {
                                 steps = (uu___169_7746.steps);
                                 tcenv = (uu___169_7746.tcenv);
                                 delta_level = (uu___169_7746.delta_level);
                                 primitive_steps = uu____7747
                               } in
                             norm cfg1 env'
                               ((Abs
                                   (env, bs1, env', lopt1,
                                     (t1.FStar_Syntax_Syntax.pos))) ::
                               stack2) body1))))
           | FStar_Syntax_Syntax.Tm_app (head1,args) ->
               let stack2 =
                 FStar_All.pipe_right stack1
                   (FStar_List.fold_right
                      (fun uu____7779  ->
                         fun stack2  ->
                           match uu____7779 with
                           | (a,aq) ->
                               let uu____7787 =
                                 let uu____7788 =
                                   let uu____7792 =
                                     let uu____7793 =
                                       let uu____7803 =
                                         FStar_Util.mk_ref None in
                                       (env, a, uu____7803, false) in
                                     Clos uu____7793 in
                                   (uu____7792, aq,
                                     (t1.FStar_Syntax_Syntax.pos)) in
                                 Arg uu____7788 in
                               uu____7787 :: stack2) args) in
               (log cfg
                  (fun uu____7825  ->
                     let uu____7826 =
                       FStar_All.pipe_left FStar_Util.string_of_int
                         (FStar_List.length args) in
                     FStar_Util.print1 "\tPushed %s arguments\n" uu____7826);
                norm cfg env stack2 head1)
           | FStar_Syntax_Syntax.Tm_refine (x,f) ->
               if FStar_List.contains WHNF cfg.steps
               then
                 (match (env, stack1) with
                  | ([],[]) ->
                      let t_x = norm cfg env [] x.FStar_Syntax_Syntax.sort in
                      let t2 =
                        mk
                          (FStar_Syntax_Syntax.Tm_refine
                             ((let uu___170_7847 = x in
                               {
                                 FStar_Syntax_Syntax.ppname =
                                   (uu___170_7847.FStar_Syntax_Syntax.ppname);
                                 FStar_Syntax_Syntax.index =
                                   (uu___170_7847.FStar_Syntax_Syntax.index);
                                 FStar_Syntax_Syntax.sort = t_x
                               }), f)) t1.FStar_Syntax_Syntax.pos in
                      rebuild cfg env stack1 t2
                  | uu____7848 ->
                      let uu____7851 = closure_as_term cfg env t1 in
                      rebuild cfg env stack1 uu____7851)
               else
                 (let t_x = norm cfg env [] x.FStar_Syntax_Syntax.sort in
                  let uu____7854 = FStar_Syntax_Subst.open_term [(x, None)] f in
                  match uu____7854 with
                  | (closing,f1) ->
                      let f2 = norm cfg (Dummy :: env) [] f1 in
                      let t2 =
                        let uu____7870 =
                          let uu____7871 =
                            let uu____7876 =
                              FStar_Syntax_Subst.close closing f2 in
                            ((let uu___171_7877 = x in
                              {
                                FStar_Syntax_Syntax.ppname =
                                  (uu___171_7877.FStar_Syntax_Syntax.ppname);
                                FStar_Syntax_Syntax.index =
                                  (uu___171_7877.FStar_Syntax_Syntax.index);
                                FStar_Syntax_Syntax.sort = t_x
                              }), uu____7876) in
                          FStar_Syntax_Syntax.Tm_refine uu____7871 in
                        mk uu____7870 t1.FStar_Syntax_Syntax.pos in
                      rebuild cfg env stack1 t2)
           | FStar_Syntax_Syntax.Tm_arrow (bs,c) ->
               if FStar_List.contains WHNF cfg.steps
               then
                 let uu____7890 = closure_as_term cfg env t1 in
                 rebuild cfg env stack1 uu____7890
               else
                 (let uu____7892 = FStar_Syntax_Subst.open_comp bs c in
                  match uu____7892 with
                  | (bs1,c1) ->
                      let c2 =
                        let uu____7898 =
                          FStar_All.pipe_right bs1
                            (FStar_List.fold_left
                               (fun env1  -> fun uu____7904  -> Dummy :: env1)
                               env) in
                        norm_comp cfg uu____7898 c1 in
                      let t2 =
                        let uu____7911 = norm_binders cfg env bs1 in
                        FStar_Syntax_Util.arrow uu____7911 c2 in
                      rebuild cfg env stack1 t2)
           | FStar_Syntax_Syntax.Tm_ascribed (t11,(tc,tacopt),l) ->
               (match stack1 with
                | (Match uu____7954)::uu____7955 -> norm cfg env stack1 t11
                | (Arg uu____7960)::uu____7961 -> norm cfg env stack1 t11
                | (App
                    ({
                       FStar_Syntax_Syntax.n =
                         FStar_Syntax_Syntax.Tm_constant
                         (FStar_Const.Const_reify );
                       FStar_Syntax_Syntax.tk = uu____7966;
                       FStar_Syntax_Syntax.pos = uu____7967;
                       FStar_Syntax_Syntax.vars = uu____7968;_},uu____7969,uu____7970))::uu____7971
                    -> norm cfg env stack1 t11
                | (MemoLazy uu____7977)::uu____7978 ->
                    norm cfg env stack1 t11
                | uu____7983 ->
                    let t12 = norm cfg env [] t11 in
                    (log cfg
                       (fun uu____7986  ->
                          FStar_Util.print_string
                            "+++ Normalizing ascription \n");
                     (let tc1 =
                        match tc with
                        | FStar_Util.Inl t2 ->
                            let uu____7999 = norm cfg env [] t2 in
                            FStar_Util.Inl uu____7999
                        | FStar_Util.Inr c ->
                            let uu____8007 = norm_comp cfg env c in
                            FStar_Util.Inr uu____8007 in
                      let tacopt1 =
                        FStar_Util.map_opt tacopt (norm cfg env []) in
                      let uu____8012 =
                        mk
                          (FStar_Syntax_Syntax.Tm_ascribed
                             (t12, (tc1, tacopt1), l))
                          t1.FStar_Syntax_Syntax.pos in
                      rebuild cfg env stack1 uu____8012)))
           | FStar_Syntax_Syntax.Tm_match (head1,branches) ->
               let stack2 =
                 (Match (env, branches, (t1.FStar_Syntax_Syntax.pos))) ::
                 stack1 in
               norm cfg env stack2 head1
           | FStar_Syntax_Syntax.Tm_let
               ((uu____8063,{
                              FStar_Syntax_Syntax.lbname = FStar_Util.Inr
                                uu____8064;
                              FStar_Syntax_Syntax.lbunivs = uu____8065;
                              FStar_Syntax_Syntax.lbtyp = uu____8066;
                              FStar_Syntax_Syntax.lbeff = uu____8067;
                              FStar_Syntax_Syntax.lbdef = uu____8068;_}::uu____8069),uu____8070)
               -> rebuild cfg env stack1 t1
           | FStar_Syntax_Syntax.Tm_let ((false ,lb::[]),body) ->
               let n1 =
                 FStar_TypeChecker_Env.norm_eff_name cfg.tcenv
                   lb.FStar_Syntax_Syntax.lbeff in
               let uu____8096 =
                 (let uu____8097 =
                    FStar_All.pipe_right cfg.steps
                      (FStar_List.contains NoDeltaSteps) in
                  Prims.op_Negation uu____8097) &&
                   ((FStar_Syntax_Util.is_pure_effect n1) ||
                      ((FStar_Syntax_Util.is_ghost_effect n1) &&
                         (let uu____8098 =
                            FStar_All.pipe_right cfg.steps
                              (FStar_List.contains
                                 PureSubtermsWithinComputations) in
                          Prims.op_Negation uu____8098))) in
               if uu____8096
               then
                 let env1 =
                   let uu____8101 =
                     let uu____8102 =
                       let uu____8112 = FStar_Util.mk_ref None in
                       (env, (lb.FStar_Syntax_Syntax.lbdef), uu____8112,
                         false) in
                     Clos uu____8102 in
                   uu____8101 :: env in
                 norm cfg env1 stack1 body
               else
                 (let uu____8136 =
                    let uu____8139 =
                      let uu____8140 =
                        let uu____8141 =
                          FStar_All.pipe_right lb.FStar_Syntax_Syntax.lbname
                            FStar_Util.left in
                        FStar_All.pipe_right uu____8141
                          FStar_Syntax_Syntax.mk_binder in
                      [uu____8140] in
                    FStar_Syntax_Subst.open_term uu____8139 body in
                  match uu____8136 with
                  | (bs,body1) ->
                      let lb1 =
                        let uu___172_8147 = lb in
                        let uu____8148 =
                          let uu____8151 =
                            let uu____8152 = FStar_List.hd bs in
                            FStar_All.pipe_right uu____8152
                              FStar_Pervasives.fst in
                          FStar_All.pipe_right uu____8151
                            (fun _0_43  -> FStar_Util.Inl _0_43) in
                        let uu____8161 =
                          norm cfg env [] lb.FStar_Syntax_Syntax.lbtyp in
                        let uu____8164 =
                          norm cfg env [] lb.FStar_Syntax_Syntax.lbdef in
                        {
                          FStar_Syntax_Syntax.lbname = uu____8148;
                          FStar_Syntax_Syntax.lbunivs =
                            (uu___172_8147.FStar_Syntax_Syntax.lbunivs);
                          FStar_Syntax_Syntax.lbtyp = uu____8161;
                          FStar_Syntax_Syntax.lbeff =
                            (uu___172_8147.FStar_Syntax_Syntax.lbeff);
                          FStar_Syntax_Syntax.lbdef = uu____8164
                        } in
                      let env' =
                        FStar_All.pipe_right bs
                          (FStar_List.fold_left
                             (fun env1  -> fun uu____8174  -> Dummy :: env1)
                             env) in
                      norm cfg env'
                        ((Let (env, bs, lb1, (t1.FStar_Syntax_Syntax.pos)))
                        :: stack1) body1)
           | FStar_Syntax_Syntax.Tm_let (lbs,body) when
               FStar_List.contains (Exclude Zeta) cfg.steps ->
               let uu____8190 = closure_as_term cfg env t1 in
               rebuild cfg env stack1 uu____8190
           | FStar_Syntax_Syntax.Tm_let (lbs,body) ->
               let uu____8203 =
                 FStar_List.fold_right
                   (fun lb  ->
                      fun uu____8225  ->
                        match uu____8225 with
                        | (rec_env,memos,i) ->
                            let f_i =
                              let uu____8264 =
                                let uu___173_8265 =
                                  FStar_Util.left
                                    lb.FStar_Syntax_Syntax.lbname in
                                {
                                  FStar_Syntax_Syntax.ppname =
                                    (uu___173_8265.FStar_Syntax_Syntax.ppname);
                                  FStar_Syntax_Syntax.index = i;
                                  FStar_Syntax_Syntax.sort =
                                    (uu___173_8265.FStar_Syntax_Syntax.sort)
                                } in
                              FStar_Syntax_Syntax.bv_to_tm uu____8264 in
                            let fix_f_i =
                              mk (FStar_Syntax_Syntax.Tm_let (lbs, f_i))
                                t1.FStar_Syntax_Syntax.pos in
                            let memo = FStar_Util.mk_ref None in
                            let rec_env1 = (Clos (env, fix_f_i, memo, true))
                              :: rec_env in
                            (rec_env1, (memo :: memos),
                              (i + (Prims.parse_int "1")))) (snd lbs)
                   (env, [], (Prims.parse_int "0")) in
               (match uu____8203 with
                | (rec_env,memos,uu____8325) ->
                    let uu____8340 =
                      FStar_List.map2
                        (fun lb  ->
                           fun memo  ->
                             FStar_ST.write memo
                               (Some
                                  (rec_env, (lb.FStar_Syntax_Syntax.lbdef))))
                        (snd lbs) memos in
                    let body_env =
                      FStar_List.fold_right
                        (fun lb  ->
                           fun env1  ->
                             let uu____8382 =
                               let uu____8383 =
                                 let uu____8393 = FStar_Util.mk_ref None in
                                 (rec_env, (lb.FStar_Syntax_Syntax.lbdef),
                                   uu____8393, false) in
                               Clos uu____8383 in
                             uu____8382 :: env1) (snd lbs) env in
                    norm cfg body_env stack1 body)
           | FStar_Syntax_Syntax.Tm_meta (head1,m) ->
               (match m with
                | FStar_Syntax_Syntax.Meta_monadic (m1,t2) ->
                    let should_reify =
                      match stack1 with
                      | (App
                          ({
                             FStar_Syntax_Syntax.n =
                               FStar_Syntax_Syntax.Tm_constant
                               (FStar_Const.Const_reify );
                             FStar_Syntax_Syntax.tk = uu____8431;
                             FStar_Syntax_Syntax.pos = uu____8432;
                             FStar_Syntax_Syntax.vars = uu____8433;_},uu____8434,uu____8435))::uu____8436
                          ->
                          FStar_All.pipe_right cfg.steps
                            (FStar_List.contains Reify)
                      | uu____8442 -> false in
                    if Prims.op_Negation should_reify
                    then
                      let t3 = norm cfg env [] t2 in
                      let stack2 =
                        (Steps
                           ((cfg.steps), (cfg.primitive_steps),
                             (cfg.delta_level)))
                        :: stack1 in
                      let cfg1 =
                        let uu____8449 =
                          FStar_All.pipe_right cfg.steps
                            (FStar_List.contains
                               PureSubtermsWithinComputations) in
                        if uu____8449
                        then
                          let uu___174_8450 = cfg in
                          {
                            steps =
                              [PureSubtermsWithinComputations;
                              Primops;
                              AllowUnboundUniverses;
                              EraseUniverses;
                              Exclude Zeta;
                              NoDeltaSteps];
                            tcenv = (uu___174_8450.tcenv);
                            delta_level =
                              [FStar_TypeChecker_Env.Inlining;
                              FStar_TypeChecker_Env.Eager_unfolding_only];
                            primitive_steps = (uu___174_8450.primitive_steps)
                          }
                        else
                          (let uu___175_8452 = cfg in
                           {
                             steps =
                               (FStar_List.append
                                  [NoDeltaSteps; Exclude Zeta] cfg.steps);
                             tcenv = (uu___175_8452.tcenv);
                             delta_level = [FStar_TypeChecker_Env.NoDelta];
                             primitive_steps =
                               (uu___175_8452.primitive_steps)
                           }) in
                      norm cfg1 env
                        ((Meta
                            ((FStar_Syntax_Syntax.Meta_monadic (m1, t3)),
                              (t3.FStar_Syntax_Syntax.pos))) :: stack2) head1
                    else
                      (let uu____8454 =
                         let uu____8455 = FStar_Syntax_Subst.compress head1 in
                         uu____8455.FStar_Syntax_Syntax.n in
                       match uu____8454 with
                       | FStar_Syntax_Syntax.Tm_let ((false ,lb::[]),body) ->
                           let ed =
                             FStar_TypeChecker_Env.get_effect_decl cfg.tcenv
                               m1 in
                           let uu____8469 = ed.FStar_Syntax_Syntax.bind_repr in
                           (match uu____8469 with
                            | (uu____8470,bind_repr) ->
                                (match lb.FStar_Syntax_Syntax.lbname with
                                 | FStar_Util.Inr uu____8474 ->
                                     failwith
                                       "Cannot reify a top-level let binding"
                                 | FStar_Util.Inl x ->
                                     let is_return e =
                                       let uu____8481 =
                                         let uu____8482 =
                                           FStar_Syntax_Subst.compress e in
                                         uu____8482.FStar_Syntax_Syntax.n in
                                       match uu____8481 with
                                       | FStar_Syntax_Syntax.Tm_meta
                                           (e1,FStar_Syntax_Syntax.Meta_monadic
                                            (uu____8487,uu____8488))
                                           ->
                                           let uu____8497 =
                                             let uu____8498 =
                                               FStar_Syntax_Subst.compress e1 in
                                             uu____8498.FStar_Syntax_Syntax.n in
                                           (match uu____8497 with
                                            | FStar_Syntax_Syntax.Tm_meta
                                                (e2,FStar_Syntax_Syntax.Meta_monadic_lift
                                                 (uu____8503,msrc,uu____8505))
                                                when
                                                FStar_Syntax_Util.is_pure_effect
                                                  msrc
                                                ->
                                                let uu____8514 =
                                                  FStar_Syntax_Subst.compress
                                                    e2 in
                                                Some uu____8514
                                            | uu____8515 -> None)
                                       | uu____8516 -> None in
                                     let uu____8517 =
                                       is_return lb.FStar_Syntax_Syntax.lbdef in
                                     (match uu____8517 with
                                      | Some e ->
                                          let lb1 =
                                            let uu___176_8521 = lb in
                                            {
                                              FStar_Syntax_Syntax.lbname =
                                                (uu___176_8521.FStar_Syntax_Syntax.lbname);
                                              FStar_Syntax_Syntax.lbunivs =
                                                (uu___176_8521.FStar_Syntax_Syntax.lbunivs);
                                              FStar_Syntax_Syntax.lbtyp =
                                                (uu___176_8521.FStar_Syntax_Syntax.lbtyp);
                                              FStar_Syntax_Syntax.lbeff =
                                                FStar_Syntax_Const.effect_PURE_lid;
                                              FStar_Syntax_Syntax.lbdef = e
                                            } in
                                          let uu____8522 =
                                            FStar_List.tl stack1 in
                                          let uu____8523 =
                                            let uu____8524 =
                                              let uu____8527 =
                                                let uu____8528 =
                                                  let uu____8536 =
                                                    FStar_Syntax_Util.mk_reify
                                                      body in
                                                  ((false, [lb1]),
                                                    uu____8536) in
                                                FStar_Syntax_Syntax.Tm_let
                                                  uu____8528 in
                                              FStar_Syntax_Syntax.mk
                                                uu____8527 in
                                            uu____8524 None
                                              head1.FStar_Syntax_Syntax.pos in
                                          norm cfg env uu____8522 uu____8523
                                      | None  ->
                                          let uu____8553 =
                                            let uu____8554 = is_return body in
                                            match uu____8554 with
                                            | Some
                                                {
                                                  FStar_Syntax_Syntax.n =
                                                    FStar_Syntax_Syntax.Tm_bvar
                                                    y;
                                                  FStar_Syntax_Syntax.tk =
                                                    uu____8557;
                                                  FStar_Syntax_Syntax.pos =
                                                    uu____8558;
                                                  FStar_Syntax_Syntax.vars =
                                                    uu____8559;_}
                                                ->
                                                FStar_Syntax_Syntax.bv_eq x y
                                            | uu____8564 -> false in
                                          if uu____8553
                                          then
                                            norm cfg env stack1
                                              lb.FStar_Syntax_Syntax.lbdef
                                          else
                                            (let head2 =
                                               FStar_All.pipe_left
                                                 FStar_Syntax_Util.mk_reify
                                                 lb.FStar_Syntax_Syntax.lbdef in
                                             let body1 =
                                               FStar_All.pipe_left
                                                 FStar_Syntax_Util.mk_reify
                                                 body in
                                             let body2 =
                                               let uu____8584 =
                                                 let uu____8587 =
                                                   let uu____8588 =
                                                     let uu____8603 =
                                                       let uu____8605 =
                                                         FStar_Syntax_Syntax.mk_binder
                                                           x in
                                                       [uu____8605] in
                                                     (uu____8603, body1,
                                                       (Some
                                                          (FStar_Util.Inr
                                                             (m1, [])))) in
                                                   FStar_Syntax_Syntax.Tm_abs
                                                     uu____8588 in
                                                 FStar_Syntax_Syntax.mk
                                                   uu____8587 in
                                               uu____8584 None
                                                 body1.FStar_Syntax_Syntax.pos in
                                             let close1 =
                                               closure_as_term cfg env in
                                             let bind_inst =
                                               let uu____8638 =
                                                 let uu____8639 =
                                                   FStar_Syntax_Subst.compress
                                                     bind_repr in
                                                 uu____8639.FStar_Syntax_Syntax.n in
                                               match uu____8638 with
                                               | FStar_Syntax_Syntax.Tm_uinst
                                                   (bind1,uu____8645::uu____8646::[])
                                                   ->
                                                   let uu____8652 =
                                                     let uu____8655 =
                                                       let uu____8656 =
                                                         let uu____8661 =
                                                           let uu____8663 =
                                                             let uu____8664 =
                                                               close1
                                                                 lb.FStar_Syntax_Syntax.lbtyp in
                                                             (cfg.tcenv).FStar_TypeChecker_Env.universe_of
                                                               cfg.tcenv
                                                               uu____8664 in
                                                           let uu____8665 =
                                                             let uu____8667 =
                                                               let uu____8668
                                                                 = close1 t2 in
                                                               (cfg.tcenv).FStar_TypeChecker_Env.universe_of
                                                                 cfg.tcenv
                                                                 uu____8668 in
                                                             [uu____8667] in
                                                           uu____8663 ::
                                                             uu____8665 in
                                                         (bind1, uu____8661) in
                                                       FStar_Syntax_Syntax.Tm_uinst
                                                         uu____8656 in
                                                     FStar_Syntax_Syntax.mk
                                                       uu____8655 in
                                                   uu____8652 None
                                                     t2.FStar_Syntax_Syntax.pos
                                               | uu____8680 ->
                                                   failwith
                                                     "NIY : Reification of indexed effects" in
                                             let reified =
                                               let uu____8686 =
                                                 let uu____8689 =
                                                   let uu____8690 =
                                                     let uu____8700 =
                                                       let uu____8702 =
                                                         FStar_Syntax_Syntax.as_arg
                                                           lb.FStar_Syntax_Syntax.lbtyp in
                                                       let uu____8703 =
                                                         let uu____8705 =
                                                           FStar_Syntax_Syntax.as_arg
                                                             t2 in
                                                         let uu____8706 =
                                                           let uu____8708 =
                                                             FStar_Syntax_Syntax.as_arg
                                                               FStar_Syntax_Syntax.tun in
                                                           let uu____8709 =
                                                             let uu____8711 =
                                                               FStar_Syntax_Syntax.as_arg
                                                                 head2 in
                                                             let uu____8712 =
                                                               let uu____8714
                                                                 =
                                                                 FStar_Syntax_Syntax.as_arg
                                                                   FStar_Syntax_Syntax.tun in
                                                               let uu____8715
                                                                 =
                                                                 let uu____8717
                                                                   =
                                                                   FStar_Syntax_Syntax.as_arg
                                                                    body2 in
                                                                 [uu____8717] in
                                                               uu____8714 ::
                                                                 uu____8715 in
                                                             uu____8711 ::
                                                               uu____8712 in
                                                           uu____8708 ::
                                                             uu____8709 in
                                                         uu____8705 ::
                                                           uu____8706 in
                                                       uu____8702 ::
                                                         uu____8703 in
                                                     (bind_inst, uu____8700) in
                                                   FStar_Syntax_Syntax.Tm_app
                                                     uu____8690 in
                                                 FStar_Syntax_Syntax.mk
                                                   uu____8689 in
                                               uu____8686 None
                                                 t2.FStar_Syntax_Syntax.pos in
                                             let uu____8729 =
                                               FStar_List.tl stack1 in
                                             norm cfg env uu____8729 reified))))
                       | FStar_Syntax_Syntax.Tm_app (head_app,args) ->
                           let ed =
                             FStar_TypeChecker_Env.get_effect_decl cfg.tcenv
                               m1 in
                           let uu____8747 = ed.FStar_Syntax_Syntax.bind_repr in
                           (match uu____8747 with
                            | (uu____8748,bind_repr) ->
                                let maybe_unfold_action head2 =
                                  let maybe_extract_fv t3 =
                                    let t4 =
                                      let uu____8771 =
                                        let uu____8772 =
                                          FStar_Syntax_Subst.compress t3 in
                                        uu____8772.FStar_Syntax_Syntax.n in
                                      match uu____8771 with
                                      | FStar_Syntax_Syntax.Tm_uinst
                                          (t4,uu____8778) -> t4
                                      | uu____8783 -> head2 in
                                    let uu____8784 =
                                      let uu____8785 =
                                        FStar_Syntax_Subst.compress t4 in
                                      uu____8785.FStar_Syntax_Syntax.n in
                                    match uu____8784 with
                                    | FStar_Syntax_Syntax.Tm_fvar x -> Some x
                                    | uu____8790 -> None in
                                  let uu____8791 = maybe_extract_fv head2 in
                                  match uu____8791 with
                                  | Some x when
                                      let uu____8797 =
                                        FStar_Syntax_Syntax.lid_of_fv x in
                                      FStar_TypeChecker_Env.is_action
                                        cfg.tcenv uu____8797
                                      ->
                                      let head3 = norm cfg env [] head2 in
                                      let action_unfolded =
                                        let uu____8801 =
                                          maybe_extract_fv head3 in
                                        match uu____8801 with
                                        | Some uu____8804 -> Some true
                                        | uu____8805 -> Some false in
                                      (head3, action_unfolded)
                                  | uu____8808 -> (head2, None) in
                                ((let is_arg_impure uu____8819 =
                                    match uu____8819 with
                                    | (e,q) ->
                                        let uu____8824 =
                                          let uu____8825 =
                                            FStar_Syntax_Subst.compress e in
                                          uu____8825.FStar_Syntax_Syntax.n in
                                        (match uu____8824 with
                                         | FStar_Syntax_Syntax.Tm_meta
                                             (e0,FStar_Syntax_Syntax.Meta_monadic_lift
                                              (m11,m2,t'))
                                             ->
                                             Prims.op_Negation
                                               (FStar_Syntax_Util.is_pure_effect
                                                  m11)
                                         | uu____8840 -> false) in
                                  let uu____8841 =
                                    let uu____8842 =
                                      let uu____8846 =
                                        FStar_Syntax_Syntax.as_arg head_app in
                                      uu____8846 :: args in
                                    FStar_Util.for_some is_arg_impure
                                      uu____8842 in
                                  if uu____8841
                                  then
                                    let uu____8849 =
                                      let uu____8850 =
                                        FStar_Syntax_Print.term_to_string
                                          head1 in
                                      FStar_Util.format1
                                        "Incompability between typechecker and normalizer; this monadic application contains impure terms %s\n"
                                        uu____8850 in
                                    failwith uu____8849
                                  else ());
                                 (let uu____8852 =
                                    maybe_unfold_action head_app in
                                  match uu____8852 with
                                  | (head_app1,found_action) ->
                                      let mk1 tm =
                                        FStar_Syntax_Syntax.mk tm None
                                          t2.FStar_Syntax_Syntax.pos in
                                      let body =
                                        mk1
                                          (FStar_Syntax_Syntax.Tm_app
                                             (head_app1, args)) in
                                      let body1 =
                                        match found_action with
                                        | None  ->
                                            FStar_Syntax_Util.mk_reify body
                                        | Some (false ) ->
                                            mk1
                                              (FStar_Syntax_Syntax.Tm_meta
                                                 (body,
                                                   (FStar_Syntax_Syntax.Meta_monadic
                                                      (m1, t2))))
                                        | Some (true ) -> body in
                                      let uu____8887 = FStar_List.tl stack1 in
                                      norm cfg env uu____8887 body1)))
                       | FStar_Syntax_Syntax.Tm_meta
                           (e,FStar_Syntax_Syntax.Meta_monadic_lift
                            (msrc,mtgt,t'))
                           ->
                           let lifted =
                             let uu____8901 = closure_as_term cfg env t' in
                             reify_lift cfg.tcenv e msrc mtgt uu____8901 in
                           let uu____8902 = FStar_List.tl stack1 in
                           norm cfg env uu____8902 lifted
                       | FStar_Syntax_Syntax.Tm_match (e,branches) ->
                           let branches1 =
                             FStar_All.pipe_right branches
                               (FStar_List.map
                                  (fun uu____8985  ->
                                     match uu____8985 with
                                     | (pat,wopt,tm) ->
                                         let uu____9023 =
                                           FStar_Syntax_Util.mk_reify tm in
                                         (pat, wopt, uu____9023))) in
                           let tm =
                             mk (FStar_Syntax_Syntax.Tm_match (e, branches1))
                               t2.FStar_Syntax_Syntax.pos in
                           let uu____9049 = FStar_List.tl stack1 in
                           norm cfg env uu____9049 tm
                       | uu____9050 -> norm cfg env stack1 head1)
                | FStar_Syntax_Syntax.Meta_monadic_lift (m1,m',t2) ->
                    let should_reify =
                      match stack1 with
                      | (App
                          ({
                             FStar_Syntax_Syntax.n =
                               FStar_Syntax_Syntax.Tm_constant
                               (FStar_Const.Const_reify );
                             FStar_Syntax_Syntax.tk = uu____9059;
                             FStar_Syntax_Syntax.pos = uu____9060;
                             FStar_Syntax_Syntax.vars = uu____9061;_},uu____9062,uu____9063))::uu____9064
                          ->
                          FStar_All.pipe_right cfg.steps
                            (FStar_List.contains Reify)
                      | uu____9070 -> false in
                    if should_reify
                    then
                      let uu____9071 = FStar_List.tl stack1 in
                      let uu____9072 =
                        let uu____9073 = closure_as_term cfg env t2 in
                        reify_lift cfg.tcenv head1 m1 m' uu____9073 in
                      norm cfg env uu____9071 uu____9072
                    else
                      (let t3 = norm cfg env [] t2 in
                       let uu____9076 =
                         ((FStar_Syntax_Util.is_pure_effect m1) ||
                            (FStar_Syntax_Util.is_ghost_effect m1))
                           &&
                           (FStar_All.pipe_right cfg.steps
                              (FStar_List.contains
                                 PureSubtermsWithinComputations)) in
                       if uu____9076
                       then
                         let stack2 =
                           (Steps
                              ((cfg.steps), (cfg.primitive_steps),
                                (cfg.delta_level)))
                           :: stack1 in
                         let cfg1 =
                           let uu___177_9082 = cfg in
                           {
                             steps =
                               [PureSubtermsWithinComputations;
                               Primops;
                               AllowUnboundUniverses;
                               EraseUniverses;
                               Exclude Zeta];
                             tcenv = (uu___177_9082.tcenv);
                             delta_level =
                               [FStar_TypeChecker_Env.Inlining;
                               FStar_TypeChecker_Env.Eager_unfolding_only];
                             primitive_steps =
                               (uu___177_9082.primitive_steps)
                           } in
                         norm cfg1 env
                           ((Meta
                               ((FStar_Syntax_Syntax.Meta_monadic_lift
                                   (m1, m', t3)),
                                 (head1.FStar_Syntax_Syntax.pos))) :: stack2)
                           head1
                       else
                         norm cfg env
                           ((Meta
                               ((FStar_Syntax_Syntax.Meta_monadic_lift
                                   (m1, m', t3)),
                                 (head1.FStar_Syntax_Syntax.pos))) :: stack1)
                           head1)
                | uu____9084 ->
                    (match stack1 with
                     | uu____9085::uu____9086 ->
                         (match m with
                          | FStar_Syntax_Syntax.Meta_labeled (l,r,uu____9090)
                              -> norm cfg env ((Meta (m, r)) :: stack1) head1
                          | FStar_Syntax_Syntax.Meta_pattern args ->
                              let args1 = norm_pattern_args cfg env args in
                              norm cfg env
                                ((Meta
                                    ((FStar_Syntax_Syntax.Meta_pattern args1),
                                      (t1.FStar_Syntax_Syntax.pos))) ::
                                stack1) head1
                          | uu____9105 -> norm cfg env stack1 head1)
                     | [] ->
                         let head2 = norm cfg env [] head1 in
                         let m1 =
                           match m with
                           | FStar_Syntax_Syntax.Meta_pattern args ->
                               let uu____9115 =
                                 norm_pattern_args cfg env args in
                               FStar_Syntax_Syntax.Meta_pattern uu____9115
                           | uu____9122 -> m in
                         let t2 =
                           mk (FStar_Syntax_Syntax.Tm_meta (head2, m1))
                             t1.FStar_Syntax_Syntax.pos in
                         rebuild cfg env stack1 t2)))
and reify_lift:
  FStar_TypeChecker_Env.env ->
    (FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
      FStar_Syntax_Syntax.syntax ->
      FStar_Syntax_Syntax.monad_name ->
        FStar_Syntax_Syntax.monad_name ->
          FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun env  ->
    fun e  ->
      fun msrc  ->
        fun mtgt  ->
          fun t  ->
            if FStar_Syntax_Util.is_pure_effect msrc
            then
              let ed = FStar_TypeChecker_Env.get_effect_decl env mtgt in
              let uu____9134 = ed.FStar_Syntax_Syntax.return_repr in
              match uu____9134 with
              | (uu____9135,return_repr) ->
                  let return_inst =
                    let uu____9142 =
                      let uu____9143 =
                        FStar_Syntax_Subst.compress return_repr in
                      uu____9143.FStar_Syntax_Syntax.n in
                    match uu____9142 with
                    | FStar_Syntax_Syntax.Tm_uinst (return_tm,uu____9149::[])
                        ->
                        let uu____9155 =
                          let uu____9158 =
                            let uu____9159 =
                              let uu____9164 =
                                let uu____9166 =
                                  env.FStar_TypeChecker_Env.universe_of env t in
                                [uu____9166] in
                              (return_tm, uu____9164) in
                            FStar_Syntax_Syntax.Tm_uinst uu____9159 in
                          FStar_Syntax_Syntax.mk uu____9158 in
                        uu____9155 None e.FStar_Syntax_Syntax.pos
                    | uu____9178 ->
                        failwith "NIY : Reification of indexed effects" in
                  let uu____9181 =
                    let uu____9184 =
                      let uu____9185 =
                        let uu____9195 =
                          let uu____9197 = FStar_Syntax_Syntax.as_arg t in
                          let uu____9198 =
                            let uu____9200 = FStar_Syntax_Syntax.as_arg e in
                            [uu____9200] in
                          uu____9197 :: uu____9198 in
                        (return_inst, uu____9195) in
                      FStar_Syntax_Syntax.Tm_app uu____9185 in
                    FStar_Syntax_Syntax.mk uu____9184 in
                  uu____9181 None e.FStar_Syntax_Syntax.pos
            else
              (let uu____9213 = FStar_TypeChecker_Env.monad_leq env msrc mtgt in
               match uu____9213 with
               | None  ->
                   let uu____9215 =
                     FStar_Util.format2
                       "Impossible : trying to reify a lift between unrelated effects (%s and %s)"
                       (FStar_Ident.text_of_lid msrc)
                       (FStar_Ident.text_of_lid mtgt) in
                   failwith uu____9215
               | Some
                   { FStar_TypeChecker_Env.msource = uu____9216;
                     FStar_TypeChecker_Env.mtarget = uu____9217;
                     FStar_TypeChecker_Env.mlift =
                       { FStar_TypeChecker_Env.mlift_wp = uu____9218;
                         FStar_TypeChecker_Env.mlift_term = None ;_};_}
                   ->
                   failwith
                     "Impossible : trying to reify a non-reifiable lift (from %s to %s)"
               | Some
                   { FStar_TypeChecker_Env.msource = uu____9229;
                     FStar_TypeChecker_Env.mtarget = uu____9230;
                     FStar_TypeChecker_Env.mlift =
                       { FStar_TypeChecker_Env.mlift_wp = uu____9231;
                         FStar_TypeChecker_Env.mlift_term = Some lift;_};_}
                   ->
                   let uu____9249 = FStar_Syntax_Util.mk_reify e in
                   lift t FStar_Syntax_Syntax.tun uu____9249)
and norm_pattern_args:
  cfg ->
    env ->
      ((FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
        FStar_Syntax_Syntax.syntax* FStar_Syntax_Syntax.aqual) Prims.list
        Prims.list ->
        (FStar_Syntax_Syntax.term* FStar_Syntax_Syntax.aqual) Prims.list
          Prims.list
  =
  fun cfg  ->
    fun env  ->
      fun args  ->
        FStar_All.pipe_right args
          (FStar_List.map
             (FStar_List.map
                (fun uu____9279  ->
                   match uu____9279 with
                   | (a,imp) ->
                       let uu____9286 = norm cfg env [] a in
                       (uu____9286, imp))))
and norm_comp:
  cfg -> env -> FStar_Syntax_Syntax.comp -> FStar_Syntax_Syntax.comp =
  fun cfg  ->
    fun env  ->
      fun comp  ->
        let comp1 = ghost_to_pure_aux cfg env comp in
        match comp1.FStar_Syntax_Syntax.n with
        | FStar_Syntax_Syntax.Total (t,uopt) ->
            let uu___178_9301 = comp1 in
            let uu____9304 =
              let uu____9305 =
                let uu____9311 = norm cfg env [] t in
                let uu____9312 =
                  FStar_Option.map (norm_universe cfg env) uopt in
                (uu____9311, uu____9312) in
              FStar_Syntax_Syntax.Total uu____9305 in
            {
              FStar_Syntax_Syntax.n = uu____9304;
              FStar_Syntax_Syntax.tk = (uu___178_9301.FStar_Syntax_Syntax.tk);
              FStar_Syntax_Syntax.pos =
                (uu___178_9301.FStar_Syntax_Syntax.pos);
              FStar_Syntax_Syntax.vars =
                (uu___178_9301.FStar_Syntax_Syntax.vars)
            }
        | FStar_Syntax_Syntax.GTotal (t,uopt) ->
            let uu___179_9327 = comp1 in
            let uu____9330 =
              let uu____9331 =
                let uu____9337 = norm cfg env [] t in
                let uu____9338 =
                  FStar_Option.map (norm_universe cfg env) uopt in
                (uu____9337, uu____9338) in
              FStar_Syntax_Syntax.GTotal uu____9331 in
            {
              FStar_Syntax_Syntax.n = uu____9330;
              FStar_Syntax_Syntax.tk = (uu___179_9327.FStar_Syntax_Syntax.tk);
              FStar_Syntax_Syntax.pos =
                (uu___179_9327.FStar_Syntax_Syntax.pos);
              FStar_Syntax_Syntax.vars =
                (uu___179_9327.FStar_Syntax_Syntax.vars)
            }
        | FStar_Syntax_Syntax.Comp ct ->
            let norm_args args =
              FStar_All.pipe_right args
                (FStar_List.map
                   (fun uu____9369  ->
                      match uu____9369 with
                      | (a,i) ->
                          let uu____9376 = norm cfg env [] a in
                          (uu____9376, i))) in
            let flags =
              FStar_All.pipe_right ct.FStar_Syntax_Syntax.flags
                (FStar_List.map
                   (fun uu___143_9381  ->
                      match uu___143_9381 with
                      | FStar_Syntax_Syntax.DECREASES t ->
                          let uu____9385 = norm cfg env [] t in
                          FStar_Syntax_Syntax.DECREASES uu____9385
                      | f -> f)) in
            let uu___180_9389 = comp1 in
            let uu____9392 =
              let uu____9393 =
                let uu___181_9394 = ct in
                let uu____9395 =
                  FStar_List.map (norm_universe cfg env)
                    ct.FStar_Syntax_Syntax.comp_univs in
                let uu____9396 =
                  norm cfg env [] ct.FStar_Syntax_Syntax.result_typ in
                let uu____9399 = norm_args ct.FStar_Syntax_Syntax.effect_args in
                {
                  FStar_Syntax_Syntax.comp_univs = uu____9395;
                  FStar_Syntax_Syntax.effect_name =
                    (uu___181_9394.FStar_Syntax_Syntax.effect_name);
                  FStar_Syntax_Syntax.result_typ = uu____9396;
                  FStar_Syntax_Syntax.effect_args = uu____9399;
                  FStar_Syntax_Syntax.flags = flags
                } in
              FStar_Syntax_Syntax.Comp uu____9393 in
            {
              FStar_Syntax_Syntax.n = uu____9392;
              FStar_Syntax_Syntax.tk = (uu___180_9389.FStar_Syntax_Syntax.tk);
              FStar_Syntax_Syntax.pos =
                (uu___180_9389.FStar_Syntax_Syntax.pos);
              FStar_Syntax_Syntax.vars =
                (uu___180_9389.FStar_Syntax_Syntax.vars)
            }
and ghost_to_pure_aux:
  cfg ->
    env ->
      FStar_Syntax_Syntax.comp ->
        (FStar_Syntax_Syntax.comp',Prims.unit) FStar_Syntax_Syntax.syntax
  =
  fun cfg  ->
    fun env  ->
      fun c  ->
        let norm1 t =
          norm
            (let uu___182_9416 = cfg in
             {
               steps =
                 [Eager_unfolding;
                 UnfoldUntil FStar_Syntax_Syntax.Delta_constant;
                 AllowUnboundUniverses];
               tcenv = (uu___182_9416.tcenv);
               delta_level = (uu___182_9416.delta_level);
               primitive_steps = (uu___182_9416.primitive_steps)
             }) env [] t in
        let non_info t =
          let uu____9421 = norm1 t in
          FStar_Syntax_Util.non_informative uu____9421 in
        match c.FStar_Syntax_Syntax.n with
        | FStar_Syntax_Syntax.Total uu____9424 -> c
        | FStar_Syntax_Syntax.GTotal (t,uopt) when non_info t ->
            let uu___183_9438 = c in
            {
              FStar_Syntax_Syntax.n = (FStar_Syntax_Syntax.Total (t, uopt));
              FStar_Syntax_Syntax.tk = (uu___183_9438.FStar_Syntax_Syntax.tk);
              FStar_Syntax_Syntax.pos =
                (uu___183_9438.FStar_Syntax_Syntax.pos);
              FStar_Syntax_Syntax.vars =
                (uu___183_9438.FStar_Syntax_Syntax.vars)
            }
        | FStar_Syntax_Syntax.Comp ct ->
            let l =
              FStar_TypeChecker_Env.norm_eff_name cfg.tcenv
                ct.FStar_Syntax_Syntax.effect_name in
            let uu____9448 =
              (FStar_Syntax_Util.is_ghost_effect l) &&
                (non_info ct.FStar_Syntax_Syntax.result_typ) in
            if uu____9448
            then
              let ct1 =
                match downgrade_ghost_effect_name
                        ct.FStar_Syntax_Syntax.effect_name
                with
                | Some pure_eff ->
                    let uu___184_9453 = ct in
                    {
                      FStar_Syntax_Syntax.comp_univs =
                        (uu___184_9453.FStar_Syntax_Syntax.comp_univs);
                      FStar_Syntax_Syntax.effect_name = pure_eff;
                      FStar_Syntax_Syntax.result_typ =
                        (uu___184_9453.FStar_Syntax_Syntax.result_typ);
                      FStar_Syntax_Syntax.effect_args =
                        (uu___184_9453.FStar_Syntax_Syntax.effect_args);
                      FStar_Syntax_Syntax.flags =
                        (uu___184_9453.FStar_Syntax_Syntax.flags)
                    }
                | None  ->
                    let ct1 =
                      FStar_TypeChecker_Env.unfold_effect_abbrev cfg.tcenv c in
                    let uu___185_9455 = ct1 in
                    {
                      FStar_Syntax_Syntax.comp_univs =
                        (uu___185_9455.FStar_Syntax_Syntax.comp_univs);
                      FStar_Syntax_Syntax.effect_name =
                        FStar_Syntax_Const.effect_PURE_lid;
                      FStar_Syntax_Syntax.result_typ =
                        (uu___185_9455.FStar_Syntax_Syntax.result_typ);
                      FStar_Syntax_Syntax.effect_args =
                        (uu___185_9455.FStar_Syntax_Syntax.effect_args);
                      FStar_Syntax_Syntax.flags =
                        (uu___185_9455.FStar_Syntax_Syntax.flags)
                    } in
              let uu___186_9456 = c in
              {
                FStar_Syntax_Syntax.n = (FStar_Syntax_Syntax.Comp ct1);
                FStar_Syntax_Syntax.tk =
                  (uu___186_9456.FStar_Syntax_Syntax.tk);
                FStar_Syntax_Syntax.pos =
                  (uu___186_9456.FStar_Syntax_Syntax.pos);
                FStar_Syntax_Syntax.vars =
                  (uu___186_9456.FStar_Syntax_Syntax.vars)
              }
            else c
        | uu____9462 -> c
and norm_binder:
  cfg ->
    env ->
      FStar_Syntax_Syntax.binder ->
        (FStar_Syntax_Syntax.bv* FStar_Syntax_Syntax.aqual)
  =
  fun cfg  ->
    fun env  ->
      fun uu____9465  ->
        match uu____9465 with
        | (x,imp) ->
            let uu____9468 =
              let uu___187_9469 = x in
              let uu____9470 = norm cfg env [] x.FStar_Syntax_Syntax.sort in
              {
                FStar_Syntax_Syntax.ppname =
                  (uu___187_9469.FStar_Syntax_Syntax.ppname);
                FStar_Syntax_Syntax.index =
                  (uu___187_9469.FStar_Syntax_Syntax.index);
                FStar_Syntax_Syntax.sort = uu____9470
              } in
            (uu____9468, imp)
and norm_binders:
  cfg -> env -> FStar_Syntax_Syntax.binders -> FStar_Syntax_Syntax.binders =
  fun cfg  ->
    fun env  ->
      fun bs  ->
        let uu____9476 =
          FStar_List.fold_left
            (fun uu____9483  ->
               fun b  ->
                 match uu____9483 with
                 | (nbs',env1) ->
                     let b1 = norm_binder cfg env1 b in
                     ((b1 :: nbs'), (Dummy :: env1))) ([], env) bs in
        match uu____9476 with | (nbs,uu____9500) -> FStar_List.rev nbs
and norm_lcomp_opt:
  cfg ->
    env ->
      (FStar_Syntax_Syntax.lcomp,FStar_Syntax_Syntax.residual_comp)
        FStar_Util.either option ->
        (FStar_Syntax_Syntax.lcomp,FStar_Syntax_Syntax.residual_comp)
          FStar_Util.either option
  =
  fun cfg  ->
    fun env  ->
      fun lopt  ->
        match lopt with
        | Some (FStar_Util.Inl lc) ->
            let flags = filter_out_lcomp_cflags lc in
            let uu____9517 = FStar_Syntax_Util.is_tot_or_gtot_lcomp lc in
            if uu____9517
            then
              let t = norm cfg env [] lc.FStar_Syntax_Syntax.res_typ in
              let uu____9522 = FStar_Syntax_Util.is_total_lcomp lc in
              (if uu____9522
               then
                 let uu____9526 =
                   let uu____9529 =
                     let uu____9530 =
                       let uu____9533 = FStar_Syntax_Syntax.mk_Total t in
                       FStar_Syntax_Util.comp_set_flags uu____9533 flags in
                     FStar_Syntax_Util.lcomp_of_comp uu____9530 in
                   FStar_Util.Inl uu____9529 in
                 Some uu____9526
               else
                 (let uu____9537 =
                    let uu____9540 =
                      let uu____9541 =
                        let uu____9544 = FStar_Syntax_Syntax.mk_GTotal t in
                        FStar_Syntax_Util.comp_set_flags uu____9544 flags in
                      FStar_Syntax_Util.lcomp_of_comp uu____9541 in
                    FStar_Util.Inl uu____9540 in
                  Some uu____9537))
            else
              Some
                (FStar_Util.Inr ((lc.FStar_Syntax_Syntax.eff_name), flags))
        | uu____9557 -> lopt
and rebuild:
  cfg -> env -> stack -> FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun cfg  ->
    fun env  ->
      fun stack1  ->
        fun t  ->
          match stack1 with
          | [] -> t
          | (Debug tm)::stack2 ->
              ((let uu____9569 =
                  FStar_All.pipe_left (FStar_TypeChecker_Env.debug cfg.tcenv)
                    (FStar_Options.Other "print_normalized_terms") in
                if uu____9569
                then
                  let uu____9570 = FStar_Syntax_Print.term_to_string tm in
                  let uu____9571 = FStar_Syntax_Print.term_to_string t in
                  FStar_Util.print2 "Normalized %s to %s\n" uu____9570
                    uu____9571
                else ());
               rebuild cfg env stack2 t)
          | (Steps (s,ps,dl))::stack2 ->
              rebuild
                (let uu___188_9582 = cfg in
                 {
                   steps = s;
                   tcenv = (uu___188_9582.tcenv);
                   delta_level = dl;
                   primitive_steps = ps
                 }) env stack2 t
          | (Meta (m,r))::stack2 ->
              let t1 = mk (FStar_Syntax_Syntax.Tm_meta (t, m)) r in
              rebuild cfg env stack2 t1
          | (MemoLazy r)::stack2 ->
              (set_memo r (env, t);
               log cfg
                 (fun uu____9602  ->
                    let uu____9603 = FStar_Syntax_Print.term_to_string t in
                    FStar_Util.print1 "\tSet memo %s\n" uu____9603);
               rebuild cfg env stack2 t)
          | (Let (env',bs,lb,r))::stack2 ->
              let body = FStar_Syntax_Subst.close bs t in
              let t1 =
                FStar_Syntax_Syntax.mk
                  (FStar_Syntax_Syntax.Tm_let ((false, [lb]), body)) None r in
              rebuild cfg env' stack2 t1
          | (Abs (env',bs,env'',lopt,r))::stack2 ->
              let bs1 = norm_binders cfg env' bs in
              let lopt1 = norm_lcomp_opt cfg env'' lopt in
              let uu____9640 =
                let uu___189_9641 = FStar_Syntax_Util.abs bs1 t lopt1 in
                {
                  FStar_Syntax_Syntax.n =
                    (uu___189_9641.FStar_Syntax_Syntax.n);
                  FStar_Syntax_Syntax.tk =
                    (uu___189_9641.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos = r;
                  FStar_Syntax_Syntax.vars =
                    (uu___189_9641.FStar_Syntax_Syntax.vars)
                } in
              rebuild cfg env stack2 uu____9640
          | (Arg (Univ uu____9646,uu____9647,uu____9648))::uu____9649 ->
              failwith "Impossible"
          | (Arg (Dummy ,uu____9651,uu____9652))::uu____9653 ->
              failwith "Impossible"
          | (UnivArgs (us,r))::stack2 ->
              let t1 = FStar_Syntax_Syntax.mk_Tm_uinst t us in
              rebuild cfg env stack2 t1
          | (Arg (Clos (env1,tm,m,uu____9665),aq,r))::stack2 ->
              (log cfg
                 (fun uu____9681  ->
                    let uu____9682 = FStar_Syntax_Print.term_to_string tm in
                    FStar_Util.print1 "Rebuilding with arg %s\n" uu____9682);
               if FStar_List.contains (Exclude Iota) cfg.steps
               then
                 (if FStar_List.contains WHNF cfg.steps
                  then
                    let arg = closure_as_term cfg env1 tm in
                    let t1 =
                      FStar_Syntax_Syntax.extend_app t (arg, aq) None r in
                    rebuild cfg env1 stack2 t1
                  else
                    (let stack3 = (App (t, aq, r)) :: stack2 in
                     norm cfg env1 stack3 tm))
               else
                 (let uu____9693 = FStar_ST.read m in
                  match uu____9693 with
                  | None  ->
                      if FStar_List.contains WHNF cfg.steps
                      then
                        let arg = closure_as_term cfg env1 tm in
                        let t1 =
                          FStar_Syntax_Syntax.extend_app t (arg, aq) None r in
                        rebuild cfg env1 stack2 t1
                      else
                        (let stack3 = (MemoLazy m) :: (App (t, aq, r)) ::
                           stack2 in
                         norm cfg env1 stack3 tm)
                  | Some (uu____9719,a) ->
                      let t1 =
                        FStar_Syntax_Syntax.extend_app t (a, aq) None r in
                      rebuild cfg env1 stack2 t1))
          | (App (head1,aq,r))::stack2 ->
              let t1 = FStar_Syntax_Syntax.extend_app head1 (t, aq) None r in
              let uu____9741 = maybe_simplify cfg t1 in
              rebuild cfg env stack2 uu____9741
          | (Match (env1,branches,r))::stack2 ->
              (log cfg
                 (fun uu____9748  ->
                    let uu____9749 = FStar_Syntax_Print.term_to_string t in
                    FStar_Util.print1
                      "Rebuilding with match, scrutinee is %s ...\n"
                      uu____9749);
               (let scrutinee = t in
                let norm_and_rebuild_match uu____9754 =
                  log cfg
                    (fun uu____9756  ->
                       let uu____9757 =
                         FStar_Syntax_Print.term_to_string scrutinee in
                       let uu____9758 =
                         let uu____9759 =
                           FStar_All.pipe_right branches
                             (FStar_List.map
                                (fun uu____9766  ->
                                   match uu____9766 with
                                   | (p,uu____9772,uu____9773) ->
                                       FStar_Syntax_Print.pat_to_string p)) in
                         FStar_All.pipe_right uu____9759
                           (FStar_String.concat "\n\t") in
                       FStar_Util.print2
                         "match is irreducible: scrutinee=%s\nbranches=%s\n"
                         uu____9757 uu____9758);
                  (let whnf = FStar_List.contains WHNF cfg.steps in
                   let cfg_exclude_iota_zeta =
                     let new_delta =
                       FStar_All.pipe_right cfg.delta_level
                         (FStar_List.filter
                            (fun uu___144_9783  ->
                               match uu___144_9783 with
                               | FStar_TypeChecker_Env.Inlining  -> true
                               | FStar_TypeChecker_Env.Eager_unfolding_only 
                                   -> true
                               | uu____9784 -> false)) in
                     let steps' =
                       let uu____9787 =
                         FStar_All.pipe_right cfg.steps
                           (FStar_List.contains
                              PureSubtermsWithinComputations) in
                       if uu____9787
                       then [Exclude Zeta]
                       else [Exclude Iota; Exclude Zeta] in
                     let uu___190_9790 = cfg in
                     {
                       steps = (FStar_List.append steps' cfg.steps);
                       tcenv = (uu___190_9790.tcenv);
                       delta_level = new_delta;
                       primitive_steps = (uu___190_9790.primitive_steps)
                     } in
                   let norm_or_whnf env2 t1 =
                     if whnf
                     then closure_as_term cfg_exclude_iota_zeta env2 t1
                     else norm cfg_exclude_iota_zeta env2 [] t1 in
                   let rec norm_pat env2 p =
                     match p.FStar_Syntax_Syntax.v with
                     | FStar_Syntax_Syntax.Pat_constant uu____9824 ->
                         (p, env2)
                     | FStar_Syntax_Syntax.Pat_cons (fv,pats) ->
                         let uu____9840 =
                           FStar_All.pipe_right pats
                             (FStar_List.fold_left
                                (fun uu____9874  ->
                                   fun uu____9875  ->
                                     match (uu____9874, uu____9875) with
                                     | ((pats1,env3),(p1,b)) ->
                                         let uu____9940 = norm_pat env3 p1 in
                                         (match uu____9940 with
                                          | (p2,env4) ->
                                              (((p2, b) :: pats1), env4)))
                                ([], env2)) in
                         (match uu____9840 with
                          | (pats1,env3) ->
                              ((let uu___191_10006 = p in
                                {
                                  FStar_Syntax_Syntax.v =
                                    (FStar_Syntax_Syntax.Pat_cons
                                       (fv, (FStar_List.rev pats1)));
                                  FStar_Syntax_Syntax.ty =
                                    (uu___191_10006.FStar_Syntax_Syntax.ty);
                                  FStar_Syntax_Syntax.p =
                                    (uu___191_10006.FStar_Syntax_Syntax.p)
                                }), env3))
                     | FStar_Syntax_Syntax.Pat_var x ->
                         let x1 =
                           let uu___192_10020 = x in
                           let uu____10021 =
                             norm_or_whnf env2 x.FStar_Syntax_Syntax.sort in
                           {
                             FStar_Syntax_Syntax.ppname =
                               (uu___192_10020.FStar_Syntax_Syntax.ppname);
                             FStar_Syntax_Syntax.index =
                               (uu___192_10020.FStar_Syntax_Syntax.index);
                             FStar_Syntax_Syntax.sort = uu____10021
                           } in
                         ((let uu___193_10027 = p in
                           {
                             FStar_Syntax_Syntax.v =
                               (FStar_Syntax_Syntax.Pat_var x1);
                             FStar_Syntax_Syntax.ty =
                               (uu___193_10027.FStar_Syntax_Syntax.ty);
                             FStar_Syntax_Syntax.p =
                               (uu___193_10027.FStar_Syntax_Syntax.p)
                           }), (Dummy :: env2))
                     | FStar_Syntax_Syntax.Pat_wild x ->
                         let x1 =
                           let uu___194_10032 = x in
                           let uu____10033 =
                             norm_or_whnf env2 x.FStar_Syntax_Syntax.sort in
                           {
                             FStar_Syntax_Syntax.ppname =
                               (uu___194_10032.FStar_Syntax_Syntax.ppname);
                             FStar_Syntax_Syntax.index =
                               (uu___194_10032.FStar_Syntax_Syntax.index);
                             FStar_Syntax_Syntax.sort = uu____10033
                           } in
                         ((let uu___195_10039 = p in
                           {
                             FStar_Syntax_Syntax.v =
                               (FStar_Syntax_Syntax.Pat_wild x1);
                             FStar_Syntax_Syntax.ty =
                               (uu___195_10039.FStar_Syntax_Syntax.ty);
                             FStar_Syntax_Syntax.p =
                               (uu___195_10039.FStar_Syntax_Syntax.p)
                           }), (Dummy :: env2))
                     | FStar_Syntax_Syntax.Pat_dot_term (x,t1) ->
                         let x1 =
                           let uu___196_10049 = x in
                           let uu____10050 =
                             norm_or_whnf env2 x.FStar_Syntax_Syntax.sort in
                           {
                             FStar_Syntax_Syntax.ppname =
                               (uu___196_10049.FStar_Syntax_Syntax.ppname);
                             FStar_Syntax_Syntax.index =
                               (uu___196_10049.FStar_Syntax_Syntax.index);
                             FStar_Syntax_Syntax.sort = uu____10050
                           } in
                         let t2 = norm_or_whnf env2 t1 in
                         ((let uu___197_10057 = p in
                           {
                             FStar_Syntax_Syntax.v =
                               (FStar_Syntax_Syntax.Pat_dot_term (x1, t2));
                             FStar_Syntax_Syntax.ty =
                               (uu___197_10057.FStar_Syntax_Syntax.ty);
                             FStar_Syntax_Syntax.p =
                               (uu___197_10057.FStar_Syntax_Syntax.p)
                           }), env2) in
                   let branches1 =
                     match env1 with
                     | [] when whnf -> branches
                     | uu____10061 ->
                         FStar_All.pipe_right branches
                           (FStar_List.map
                              (fun branch1  ->
                                 let uu____10064 =
                                   FStar_Syntax_Subst.open_branch branch1 in
                                 match uu____10064 with
                                 | (p,wopt,e) ->
                                     let uu____10082 = norm_pat env1 p in
                                     (match uu____10082 with
                                      | (p1,env2) ->
                                          let wopt1 =
                                            match wopt with
                                            | None  -> None
                                            | Some w ->
                                                let uu____10106 =
                                                  norm_or_whnf env2 w in
                                                Some uu____10106 in
                                          let e1 = norm_or_whnf env2 e in
                                          FStar_Syntax_Util.branch
                                            (p1, wopt1, e1)))) in
                   let uu____10111 =
                     mk (FStar_Syntax_Syntax.Tm_match (scrutinee, branches1))
                       r in
                   rebuild cfg env1 stack2 uu____10111) in
                let rec is_cons head1 =
                  match head1.FStar_Syntax_Syntax.n with
                  | FStar_Syntax_Syntax.Tm_uinst (h,uu____10121) -> is_cons h
                  | FStar_Syntax_Syntax.Tm_constant uu____10126 -> true
                  | FStar_Syntax_Syntax.Tm_fvar
                      { FStar_Syntax_Syntax.fv_name = uu____10127;
                        FStar_Syntax_Syntax.fv_delta = uu____10128;
                        FStar_Syntax_Syntax.fv_qual = Some
                          (FStar_Syntax_Syntax.Data_ctor );_}
                      -> true
                  | FStar_Syntax_Syntax.Tm_fvar
                      { FStar_Syntax_Syntax.fv_name = uu____10132;
                        FStar_Syntax_Syntax.fv_delta = uu____10133;
                        FStar_Syntax_Syntax.fv_qual = Some
                          (FStar_Syntax_Syntax.Record_ctor uu____10134);_}
                      -> true
                  | uu____10141 -> false in
                let guard_when_clause wopt b rest =
                  match wopt with
                  | None  -> b
                  | Some w ->
                      let then_branch = b in
                      let else_branch =
                        mk (FStar_Syntax_Syntax.Tm_match (scrutinee, rest)) r in
                      FStar_Syntax_Util.if_then_else w then_branch
                        else_branch in
                let rec matches_pat scrutinee1 p =
                  let scrutinee2 = FStar_Syntax_Util.unmeta scrutinee1 in
                  let uu____10240 =
                    FStar_Syntax_Util.head_and_args scrutinee2 in
                  match uu____10240 with
                  | (head1,args) ->
                      (match p.FStar_Syntax_Syntax.v with
                       | FStar_Syntax_Syntax.Pat_var uu____10272 ->
                           FStar_Util.Inl [scrutinee2]
                       | FStar_Syntax_Syntax.Pat_wild uu____10274 ->
                           FStar_Util.Inl [scrutinee2]
                       | FStar_Syntax_Syntax.Pat_dot_term uu____10276 ->
                           FStar_Util.Inl []
                       | FStar_Syntax_Syntax.Pat_constant s ->
                           (match scrutinee2.FStar_Syntax_Syntax.n with
                            | FStar_Syntax_Syntax.Tm_constant s' when 
                                s = s' -> FStar_Util.Inl []
                            | uu____10288 ->
                                let uu____10289 =
                                  let uu____10290 = is_cons head1 in
                                  Prims.op_Negation uu____10290 in
                                FStar_Util.Inr uu____10289)
                       | FStar_Syntax_Syntax.Pat_cons (fv,arg_pats) ->
                           let uu____10304 =
                             let uu____10305 =
                               FStar_Syntax_Util.un_uinst head1 in
                             uu____10305.FStar_Syntax_Syntax.n in
                           (match uu____10304 with
                            | FStar_Syntax_Syntax.Tm_fvar fv' when
                                FStar_Syntax_Syntax.fv_eq fv fv' ->
                                matches_args [] args arg_pats
                            | uu____10312 ->
                                let uu____10313 =
                                  let uu____10314 = is_cons head1 in
                                  Prims.op_Negation uu____10314 in
                                FStar_Util.Inr uu____10313))
                and matches_args out a p =
                  match (a, p) with
                  | ([],[]) -> FStar_Util.Inl out
                  | ((t1,uu____10348)::rest_a,(p1,uu____10351)::rest_p) ->
                      let uu____10379 = matches_pat t1 p1 in
                      (match uu____10379 with
                       | FStar_Util.Inl s ->
                           matches_args (FStar_List.append out s) rest_a
                             rest_p
                       | m -> m)
                  | uu____10393 -> FStar_Util.Inr false in
                let rec matches scrutinee1 p =
                  match p with
                  | [] -> norm_and_rebuild_match ()
                  | (p1,wopt,b)::rest ->
                      let uu____10464 = matches_pat scrutinee1 p1 in
                      (match uu____10464 with
                       | FStar_Util.Inr (false ) -> matches scrutinee1 rest
                       | FStar_Util.Inr (true ) -> norm_and_rebuild_match ()
                       | FStar_Util.Inl s ->
                           (log cfg
                              (fun uu____10474  ->
                                 let uu____10475 =
                                   FStar_Syntax_Print.pat_to_string p1 in
                                 let uu____10476 =
                                   let uu____10477 =
                                     FStar_List.map
                                       FStar_Syntax_Print.term_to_string s in
                                   FStar_All.pipe_right uu____10477
                                     (FStar_String.concat "; ") in
                                 FStar_Util.print2
                                   "Matches pattern %s with subst = %s\n"
                                   uu____10475 uu____10476);
                            (let env2 =
                               FStar_List.fold_left
                                 (fun env2  ->
                                    fun t1  ->
                                      let uu____10486 =
                                        let uu____10487 =
                                          let uu____10497 =
                                            FStar_Util.mk_ref (Some ([], t1)) in
                                          ([], t1, uu____10497, false) in
                                        Clos uu____10487 in
                                      uu____10486 :: env2) env1 s in
                             let uu____10520 = guard_when_clause wopt b rest in
                             norm cfg env2 stack2 uu____10520))) in
                let uu____10521 =
                  FStar_All.pipe_right cfg.steps
                    (FStar_List.contains (Exclude Iota)) in
                if uu____10521
                then norm_and_rebuild_match ()
                else matches scrutinee branches))
let config: step Prims.list -> FStar_TypeChecker_Env.env -> cfg =
  fun s  ->
    fun e  ->
      let d =
        FStar_All.pipe_right s
          (FStar_List.collect
             (fun uu___145_10535  ->
                match uu___145_10535 with
                | UnfoldUntil k -> [FStar_TypeChecker_Env.Unfold k]
                | Eager_unfolding  ->
                    [FStar_TypeChecker_Env.Eager_unfolding_only]
                | Inlining  -> [FStar_TypeChecker_Env.Inlining]
                | UnfoldTac  -> [FStar_TypeChecker_Env.UnfoldTac]
                | uu____10538 -> [])) in
      let d1 =
        match d with
        | [] -> [FStar_TypeChecker_Env.NoDelta]
        | uu____10542 -> d in
      {
        steps = s;
        tcenv = e;
        delta_level = d1;
        primitive_steps =
          (FStar_List.append built_in_primitive_steps equality_ops)
      }
let normalize_with_primitive_steps:
  primitive_step Prims.list ->
    step Prims.list ->
      FStar_TypeChecker_Env.env ->
        FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun ps  ->
    fun s  ->
      fun e  ->
        fun t  ->
          let c = config s e in
          let c1 =
            let uu___198_10562 = config s e in
            {
              steps = (uu___198_10562.steps);
              tcenv = (uu___198_10562.tcenv);
              delta_level = (uu___198_10562.delta_level);
              primitive_steps = (FStar_List.append c.primitive_steps ps)
            } in
          norm c1 [] [] t
let normalize:
  steps ->
    FStar_TypeChecker_Env.env ->
      FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  = fun s  -> fun e  -> fun t  -> normalize_with_primitive_steps [] s e t
let normalize_comp:
  steps ->
    FStar_TypeChecker_Env.env ->
      FStar_Syntax_Syntax.comp -> FStar_Syntax_Syntax.comp
  =
  fun s  ->
    fun e  ->
      fun t  -> let uu____10581 = config s e in norm_comp uu____10581 [] t
let normalize_universe:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.universe -> FStar_Syntax_Syntax.universe
  =
  fun env  ->
    fun u  ->
      let uu____10588 = config [] env in norm_universe uu____10588 [] u
let ghost_to_pure:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.comp ->
      (FStar_Syntax_Syntax.comp',Prims.unit) FStar_Syntax_Syntax.syntax
  =
  fun env  ->
    fun c  ->
      let uu____10595 = config [] env in ghost_to_pure_aux uu____10595 [] c
let ghost_to_pure_lcomp:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.lcomp -> FStar_Syntax_Syntax.lcomp
  =
  fun env  ->
    fun lc  ->
      let cfg =
        config
          [Eager_unfolding;
          UnfoldUntil FStar_Syntax_Syntax.Delta_constant;
          EraseUniverses;
          AllowUnboundUniverses] env in
      let non_info t =
        let uu____10607 = norm cfg [] [] t in
        FStar_Syntax_Util.non_informative uu____10607 in
      let uu____10608 =
        (FStar_Syntax_Util.is_ghost_effect lc.FStar_Syntax_Syntax.eff_name)
          && (non_info lc.FStar_Syntax_Syntax.res_typ) in
      if uu____10608
      then
        match downgrade_ghost_effect_name lc.FStar_Syntax_Syntax.eff_name
        with
        | Some pure_eff ->
            let uu___199_10610 = lc in
            {
              FStar_Syntax_Syntax.eff_name = pure_eff;
              FStar_Syntax_Syntax.res_typ =
                (uu___199_10610.FStar_Syntax_Syntax.res_typ);
              FStar_Syntax_Syntax.cflags =
                (uu___199_10610.FStar_Syntax_Syntax.cflags);
              FStar_Syntax_Syntax.comp =
                ((fun uu____10611  ->
                    let uu____10612 = lc.FStar_Syntax_Syntax.comp () in
                    ghost_to_pure env uu____10612))
            }
        | None  -> lc
      else lc
let term_to_string:
  FStar_TypeChecker_Env.env -> FStar_Syntax_Syntax.term -> Prims.string =
  fun env  ->
    fun t  ->
      let t1 =
        try normalize [AllowUnboundUniverses] env t
        with
        | e ->
            ((let uu____10625 = FStar_Util.message_of_exn e in
              FStar_Util.print1_warning "Normalization failed with error %s"
                uu____10625);
             t) in
      FStar_Syntax_Print.term_to_string t1
let comp_to_string:
  FStar_TypeChecker_Env.env -> FStar_Syntax_Syntax.comp -> Prims.string =
  fun env  ->
    fun c  ->
      let c1 =
        try
          let uu____10634 = config [AllowUnboundUniverses] env in
          norm_comp uu____10634 [] c
        with
        | e ->
            ((let uu____10638 = FStar_Util.message_of_exn e in
              FStar_Util.print1_warning "Normalization failed with error %s"
                uu____10638);
             c) in
      FStar_Syntax_Print.comp_to_string c1
let normalize_refinement:
  steps ->
    FStar_TypeChecker_Env.env ->
      FStar_Syntax_Syntax.typ ->
        (FStar_Syntax_Syntax.term',FStar_Syntax_Syntax.term')
          FStar_Syntax_Syntax.syntax
  =
  fun steps  ->
    fun env  ->
      fun t0  ->
        let t = normalize (FStar_List.append steps [Beta]) env t0 in
        let rec aux t1 =
          let t2 = FStar_Syntax_Subst.compress t1 in
          match t2.FStar_Syntax_Syntax.n with
          | FStar_Syntax_Syntax.Tm_refine (x,phi) ->
              let t01 = aux x.FStar_Syntax_Syntax.sort in
              (match t01.FStar_Syntax_Syntax.n with
               | FStar_Syntax_Syntax.Tm_refine (y,phi1) ->
                   let uu____10675 =
                     let uu____10676 =
                       let uu____10681 = FStar_Syntax_Util.mk_conj phi1 phi in
                       (y, uu____10681) in
                     FStar_Syntax_Syntax.Tm_refine uu____10676 in
                   mk uu____10675 t01.FStar_Syntax_Syntax.pos
               | uu____10686 -> t2)
          | uu____10687 -> t2 in
        aux t
let unfold_whnf:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun env  ->
    fun t  ->
      normalize [WHNF; UnfoldUntil FStar_Syntax_Syntax.Delta_constant; Beta]
        env t
let reduce_uvar_solutions:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun env  ->
    fun t  ->
      normalize
        [Beta;
        NoDeltaSteps;
        CompressUvars;
        Exclude Zeta;
        Exclude Iota;
        NoFullNorm] env t
let eta_expand_with_type:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.term ->
      FStar_Syntax_Syntax.typ -> FStar_Syntax_Syntax.term
  =
  fun env  ->
    fun e  ->
      fun t_e  ->
        let uu____10709 = FStar_Syntax_Util.arrow_formals_comp t_e in
        match uu____10709 with
        | (formals,c) ->
            (match formals with
             | [] -> e
             | uu____10725 ->
                 let uu____10729 = FStar_Syntax_Util.abs_formals e in
                 (match uu____10729 with
                  | (actuals,uu____10740,uu____10741) ->
                      if
                        (FStar_List.length actuals) =
                          (FStar_List.length formals)
                      then e
                      else
                        (let uu____10763 =
                           FStar_All.pipe_right formals
                             FStar_Syntax_Util.args_of_binders in
                         match uu____10763 with
                         | (binders,args) ->
                             let uu____10773 =
                               FStar_Syntax_Syntax.mk_Tm_app e args None
                                 e.FStar_Syntax_Syntax.pos in
                             let uu____10776 =
                               let uu____10783 =
                                 FStar_All.pipe_right
                                   (FStar_Syntax_Util.lcomp_of_comp c)
                                   (fun _0_44  -> FStar_Util.Inl _0_44) in
                               FStar_All.pipe_right uu____10783
                                 (fun _0_45  -> Some _0_45) in
                             FStar_Syntax_Util.abs binders uu____10773
                               uu____10776)))
let eta_expand:
  FStar_TypeChecker_Env.env ->
    FStar_Syntax_Syntax.term -> FStar_Syntax_Syntax.term
  =
  fun env  ->
    fun t  ->
      let uu____10819 =
        let uu____10823 = FStar_ST.read t.FStar_Syntax_Syntax.tk in
        (uu____10823, (t.FStar_Syntax_Syntax.n)) in
      match uu____10819 with
      | (Some sort,uu____10830) ->
          let uu____10832 = mk sort t.FStar_Syntax_Syntax.pos in
          eta_expand_with_type env t uu____10832
      | (uu____10833,FStar_Syntax_Syntax.Tm_name x) ->
          eta_expand_with_type env t x.FStar_Syntax_Syntax.sort
      | uu____10837 ->
          let uu____10841 = FStar_Syntax_Util.head_and_args t in
          (match uu____10841 with
           | (head1,args) ->
               let uu____10867 =
                 let uu____10868 = FStar_Syntax_Subst.compress head1 in
                 uu____10868.FStar_Syntax_Syntax.n in
               (match uu____10867 with
                | FStar_Syntax_Syntax.Tm_uvar (uu____10871,thead) ->
                    let uu____10885 = FStar_Syntax_Util.arrow_formals thead in
                    (match uu____10885 with
                     | (formals,tres) ->
                         if
                           (FStar_List.length formals) =
                             (FStar_List.length args)
                         then t
                         else
                           (let uu____10916 =
                              env.FStar_TypeChecker_Env.type_of
                                (let uu___204_10920 = env in
                                 {
                                   FStar_TypeChecker_Env.solver =
                                     (uu___204_10920.FStar_TypeChecker_Env.solver);
                                   FStar_TypeChecker_Env.range =
                                     (uu___204_10920.FStar_TypeChecker_Env.range);
                                   FStar_TypeChecker_Env.curmodule =
                                     (uu___204_10920.FStar_TypeChecker_Env.curmodule);
                                   FStar_TypeChecker_Env.gamma =
                                     (uu___204_10920.FStar_TypeChecker_Env.gamma);
                                   FStar_TypeChecker_Env.gamma_cache =
                                     (uu___204_10920.FStar_TypeChecker_Env.gamma_cache);
                                   FStar_TypeChecker_Env.modules =
                                     (uu___204_10920.FStar_TypeChecker_Env.modules);
                                   FStar_TypeChecker_Env.expected_typ = None;
                                   FStar_TypeChecker_Env.sigtab =
                                     (uu___204_10920.FStar_TypeChecker_Env.sigtab);
                                   FStar_TypeChecker_Env.is_pattern =
                                     (uu___204_10920.FStar_TypeChecker_Env.is_pattern);
                                   FStar_TypeChecker_Env.instantiate_imp =
                                     (uu___204_10920.FStar_TypeChecker_Env.instantiate_imp);
                                   FStar_TypeChecker_Env.effects =
                                     (uu___204_10920.FStar_TypeChecker_Env.effects);
                                   FStar_TypeChecker_Env.generalize =
                                     (uu___204_10920.FStar_TypeChecker_Env.generalize);
                                   FStar_TypeChecker_Env.letrecs =
                                     (uu___204_10920.FStar_TypeChecker_Env.letrecs);
                                   FStar_TypeChecker_Env.top_level =
                                     (uu___204_10920.FStar_TypeChecker_Env.top_level);
                                   FStar_TypeChecker_Env.check_uvars =
                                     (uu___204_10920.FStar_TypeChecker_Env.check_uvars);
                                   FStar_TypeChecker_Env.use_eq =
                                     (uu___204_10920.FStar_TypeChecker_Env.use_eq);
                                   FStar_TypeChecker_Env.is_iface =
                                     (uu___204_10920.FStar_TypeChecker_Env.is_iface);
                                   FStar_TypeChecker_Env.admit =
                                     (uu___204_10920.FStar_TypeChecker_Env.admit);
                                   FStar_TypeChecker_Env.lax = true;
                                   FStar_TypeChecker_Env.lax_universes =
                                     (uu___204_10920.FStar_TypeChecker_Env.lax_universes);
                                   FStar_TypeChecker_Env.type_of =
                                     (uu___204_10920.FStar_TypeChecker_Env.type_of);
                                   FStar_TypeChecker_Env.universe_of =
                                     (uu___204_10920.FStar_TypeChecker_Env.universe_of);
                                   FStar_TypeChecker_Env.use_bv_sorts = true;
                                   FStar_TypeChecker_Env.qname_and_index =
                                     (uu___204_10920.FStar_TypeChecker_Env.qname_and_index);
                                   FStar_TypeChecker_Env.proof_ns =
                                     (uu___204_10920.FStar_TypeChecker_Env.proof_ns)
                                 }) t in
                            match uu____10916 with
                            | (uu____10921,ty,uu____10923) ->
                                eta_expand_with_type env t ty))
                | uu____10924 ->
                    let uu____10925 =
                      env.FStar_TypeChecker_Env.type_of
                        (let uu___205_10929 = env in
                         {
                           FStar_TypeChecker_Env.solver =
                             (uu___205_10929.FStar_TypeChecker_Env.solver);
                           FStar_TypeChecker_Env.range =
                             (uu___205_10929.FStar_TypeChecker_Env.range);
                           FStar_TypeChecker_Env.curmodule =
                             (uu___205_10929.FStar_TypeChecker_Env.curmodule);
                           FStar_TypeChecker_Env.gamma =
                             (uu___205_10929.FStar_TypeChecker_Env.gamma);
                           FStar_TypeChecker_Env.gamma_cache =
                             (uu___205_10929.FStar_TypeChecker_Env.gamma_cache);
                           FStar_TypeChecker_Env.modules =
                             (uu___205_10929.FStar_TypeChecker_Env.modules);
                           FStar_TypeChecker_Env.expected_typ = None;
                           FStar_TypeChecker_Env.sigtab =
                             (uu___205_10929.FStar_TypeChecker_Env.sigtab);
                           FStar_TypeChecker_Env.is_pattern =
                             (uu___205_10929.FStar_TypeChecker_Env.is_pattern);
                           FStar_TypeChecker_Env.instantiate_imp =
                             (uu___205_10929.FStar_TypeChecker_Env.instantiate_imp);
                           FStar_TypeChecker_Env.effects =
                             (uu___205_10929.FStar_TypeChecker_Env.effects);
                           FStar_TypeChecker_Env.generalize =
                             (uu___205_10929.FStar_TypeChecker_Env.generalize);
                           FStar_TypeChecker_Env.letrecs =
                             (uu___205_10929.FStar_TypeChecker_Env.letrecs);
                           FStar_TypeChecker_Env.top_level =
                             (uu___205_10929.FStar_TypeChecker_Env.top_level);
                           FStar_TypeChecker_Env.check_uvars =
                             (uu___205_10929.FStar_TypeChecker_Env.check_uvars);
                           FStar_TypeChecker_Env.use_eq =
                             (uu___205_10929.FStar_TypeChecker_Env.use_eq);
                           FStar_TypeChecker_Env.is_iface =
                             (uu___205_10929.FStar_TypeChecker_Env.is_iface);
                           FStar_TypeChecker_Env.admit =
                             (uu___205_10929.FStar_TypeChecker_Env.admit);
                           FStar_TypeChecker_Env.lax = true;
                           FStar_TypeChecker_Env.lax_universes =
                             (uu___205_10929.FStar_TypeChecker_Env.lax_universes);
                           FStar_TypeChecker_Env.type_of =
                             (uu___205_10929.FStar_TypeChecker_Env.type_of);
                           FStar_TypeChecker_Env.universe_of =
                             (uu___205_10929.FStar_TypeChecker_Env.universe_of);
                           FStar_TypeChecker_Env.use_bv_sorts = true;
                           FStar_TypeChecker_Env.qname_and_index =
                             (uu___205_10929.FStar_TypeChecker_Env.qname_and_index);
                           FStar_TypeChecker_Env.proof_ns =
                             (uu___205_10929.FStar_TypeChecker_Env.proof_ns)
                         }) t in
                    (match uu____10925 with
                     | (uu____10930,ty,uu____10932) ->
                         eta_expand_with_type env t ty)))