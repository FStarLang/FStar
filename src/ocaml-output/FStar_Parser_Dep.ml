...
let uu____1232 = ((FStar_Options.verify_all ()) || (FStar_Options.extract_all ()))
in (not (uu____1232)))
in (
...
let uu____1244 = (FStar_Util.smap_try_find graph key)
in (uu____1244 = None))
...
let uu____1264 = (FStar_Util.smap_try_find m key)
in (FStar_Util.must uu____1264))
...
| (Some (impl1), Some (uu____1294)) when interface_only -> begin
[]
end
| (Some (impl1), uu____1298) -> begin
(collect_one1 is_user_provided_filename m impl1)
end
| (None, uu____1302) -> begin
[]
end)
in (
...
let uu____1320 = (FStar_List.existsML (fun f1 -> ((

let uu____1322 = (lowercase_module_name f1)
in (uu____1322 = m1)) && (is_implementation f1))) filenames)
in (not (uu____1320))))
...
let uu____1351 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must uu____1351))
...
let uu____1382 = (FStar_List.map (fun dep1 -> (

let uu____1387 = (discover ((key)::cycle) dep1)
in (dep1)::uu____1387)) direct_deps)
in (FStar_List.flatten uu____1382))
in (FStar_List.unique uu____1380))
in ((FStar_Util.smap_add graph key ((all_deps), (Black)));
(

let uu____1395 = (

let uu____1397 = (FStar_ST.read topologically_sorted)
in (key)::uu____1397)
in (FStar_ST.write topologically_sorted uu____1395));
all_deps;
));
)
end)
...
let uu____1419 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____1419))
in (match (uu____1414) with
| (Some (intf), Some (impl)) when ((not (partial_discovery)) && (

let uu____1438 = (FStar_List.existsML (fun f -> (

let uu____1440 = (lowercase_module_name f)
in (uu____1440 = k))) filenames)
in (not (uu____1438)))) -> begin
(intf)::(impl)::[]
end
| (Some (intf), Some (impl)) when (FStar_List.existsML (fun f -> ((is_implementation f) && (

let uu____1446 = (lowercase_module_name f)
in (uu____1446 = k)))) filenames) -> begin
(intf)::(impl)::[]
end
| (Some (intf), uu____1448) -> begin
(intf)::[]
end
...
let uu____1462 = (must_find f)
in (FStar_List.rev uu____1462)))
in (
...
let uu____1471 = (FStar_Util.smap_keys graph)
in (FStar_List.sortWith (fun x y -> (FStar_String.compare x y)) uu____1471))
in (FStar_List.collect (fun k -> (
...
let uu____1499 = (discover1 k1)
in (FStar_List.rev uu____1499))
in (

let deps_as_filenames = (

let uu____1503 = (FStar_List.collect must_find deps)
in (FStar_List.append uu____1503 suffix))
in ((f), (deps_as_filenames)))))))) as_list)))) uu____1469))
in (

let topologically_sorted1 = (

let uu____1508 = (FStar_ST.read topologically_sorted)
in (FStar_List.collect must_find_r uu____1508))
in ((FStar_List.iter (fun uu____1517 -> (match (uu____1517) with
| (m1, r) -> begin
(

let uu____1525 = ((

let uu____1526 = (FStar_ST.read r)
in (not (uu____1526))) && (

let uu____1529 = (FStar_Options.interactive ())
in (not (uu____1529))))
...
let uu____1533 = ((k > (Prims.parse_int "4")) && (

let uu____1537 = (FStar_String.substring m1 (k - (Prims.parse_int "4")) (Prims.parse_int "4"))
in (uu____1537 = ".fst")))
in (match (uu____1533) with
| true -> begin
(

let uu____1541 = (FStar_String.substring m1 (Prims.parse_int "0") (k - (Prims.parse_int "4")))
in (FStar_Util.format1 " Did you mean %s ?" uu____1541))
end
| uu____1545 -> begin
""
end)))
...
let uu____1547 = (FStar_Util.format3 "You passed --verify_module %s but I found no file that contains [module %s] in the dependency graph.%s\n" m1 m1 maybe_fst)
in FStar_Errors.Err (uu____1547))
in (Prims.raise uu____1546)))
end
| uu____1548 -> begin
()
end))
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun uu____1572 -> (match (uu____1572) with
| (f, deps1) -> begin
(
...
let print = (fun uu____1602 -> (match (uu____1602) with
| (make_deps, uu____1615, graph) -> begin
(

let uu____1633 = (FStar_Options.dep ())
