...
(FStar_List.iter (fun uu____3176 -> (match (uu____3176) with
| (t, uu____3182) -> begin
(collect_term t)
end)) termimps);
)
...
| FStar_Parser_AST.App (t1, t2, uu____3192) -> begin
((collect_term t1);
(collect_term t2);
)
end
| FStar_Parser_AST.Let (uu____3194, patterms, t) -> begin
((FStar_List.iter (fun uu____3244 -> (match (uu____3244) with
| (attrs_opt, (pat, t1)) -> begin
((

let uu____3273 = (FStar_Util.map_opt attrs_opt (FStar_List.iter collect_term))
in ());
(collect_pattern pat);
(collect_term t1);
)
...
| FStar_Parser_AST.Bind (uu____3282, t1, t2) -> begin
((collect_term t1);
(collect_term t2);
)
end
...
((FStar_Util.iter_opt t collect_term);
(FStar_List.iter (fun uu____3378 -> (match (uu____3378) with
| (uu____3383, t1) -> begin
(collect_term t1)
end)) idterms);
)
end
| FStar_Parser_AST.Project (t, uu____3386) -> begin
(collect_term t)
end
...
| FStar_Parser_AST.NamedTyp (uu____3442, t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Paren (t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Requires (t, uu____3446) -> begin
(collect_term t)
end
| FStar_Parser_AST.Ensures (t, uu____3452) -> begin
(collect_term t)
end
| FStar_Parser_AST.Labeled (t, uu____3458, uu____3459) -> begin
(collect_term t)
end
| FStar_Parser_AST.VQuote (t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Quote (uu____3461) -> begin
()
end
| FStar_Parser_AST.Antiquote (uu____3466) -> begin
()
end
...
and collect_pattern' = (fun uu___64_3478 -> (match (uu___64_3478) with
| FStar_Parser_AST.PatWild -> begin
()
end
| FStar_Parser_AST.PatOp (uu____3479) -> begin
()
end
| FStar_Parser_AST.PatConst (uu____3480) -> begin
()
end
...
| FStar_Parser_AST.PatVar (uu____3488) -> begin
()
end
| FStar_Parser_AST.PatName (uu____3495) -> begin
()
end
| FStar_Parser_AST.PatTvar (uu____3496) -> begin
()
end
...
| FStar_Parser_AST.PatTuple (ps, uu____3510) -> begin
(collect_patterns ps)
end
| FStar_Parser_AST.PatRecord (lidpats) -> begin
(FStar_List.iter (fun uu____3529 -> (match (uu____3529) with
| (uu____3534, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun uu____3579 -> (match (uu____3579) with
| (pat, t1, t2) -> begin
((collect_pattern pat);
...
let uu____3597 = (FStar_Parser_Driver.parse_file filename)
in (match (uu____3597) with
| (ast, uu____3617) -> begin
(

let mname = (lowercase_module_name filename)
in ((

let uu____3632 = (((is_interface filename) && (has_implementation original_map mname)) && (

let uu____3634 = (FStar_Options.dep ())
in (Prims.op_Equality uu____3634 (FStar_Pervasives_Native.Some ("full")))))
...
let uu____3674 = (FStar_ST.op_Bang deps)
in (

let uu____3726 = (FStar_ST.op_Bang mo_roots)
in ((uu____3674), (uu____3726))));
))
end))));
))))))))))))))
...
let uu____3814 = (FStar_Options.find_file fn)
...
let uu____3822 = (FStar_Util.format1 "File %s could not be found\n" fn)
in ((FStar_Errors.Fatal_ModuleOrFileNotFound), (uu____3822)))
in (FStar_Errors.raise_err uu____3817))
end
...
let uu____3833 = (deps_try_find dep_graph file_name)
in (Prims.op_Equality uu____3833 FStar_Pervasives_Native.None))
in (match (uu____3832) with
| true -> begin
(

let uu____3850 = (collect_one file_system_map file_name)
...
let uu____3873 = ((is_implementation file_name) && (has_interface file_system_map module_name))
...
let uu____3883 = (FStar_List.unique deps1)
in ((uu____3883), (White)))
in (deps_add_dep dep_graph file_name uu____3878));
(

let uu____3884 = (FStar_List.map (file_of_dep file_system_map all_cmd_line_files1) (FStar_List.append deps1 mo_roots))
in (FStar_List.iter discover_one uu____3884));
))
end))
end
| uu____3887 -> begin
...
let uu____3928 = (deps_try_find dep_graph filename)
in (FStar_Util.must uu____3928))
...
let uu____3947 = (FStar_Util.format1 "Recursive dependency on module %s\n" filename)
in ((FStar_Errors.Warning_RecursiveDependency), (uu____3947)))
...
let uu____3953 = (dependences_of file_system_map dep_graph all_command_line_files filename)
in (FStar_List.iter (fun k -> (aux ((k)::cycle) k)) uu____3953));
(deps_add_dep dep_graph filename ((direct_deps), (Black)));
(

let uu____3959 = (

let uu____3962 = (FStar_ST.op_Bang topologically_sorted)
in (filename)::uu____3962)
in (FStar_ST.op_Colon_Equals topologically_sorted uu____3959));
)
end)
end)))
...
let uu____4120 = (topological_dependences_of all_cmd_line_files1)
in ((uu____4120), (Mk (((dep_graph), (file_system_map), (all_cmd_line_files1))))));
));
))))))


let deps_of : deps  ->  Prims.string  ->  Prims.string Prims.list = (fun uu____4137 f -> (match (uu____4137) with
| Mk (deps, file_system_map, all_cmd_line_files) -> begin
(dependences_of file_system_map deps all_cmd_line_files f)
end))


let hash_dependences : deps  ->  Prims.string  ->  (Prims.string * Prims.string) Prims.list FStar_Pervasives_Native.option = (fun uu____4166 fn -> (match (uu____4166) with
| Mk (deps, file_system_map, all_cmd_line_files) -> begin
(

let fn1 = (

let uu____4184 = (FStar_Options.find_file fn)
...
let uu____4199 = (FStar_Options.debug_any ())
...
let uu____4210 = ((is_implementation fn1) && (has_interface file_system_map module_name))
...
let uu____4224 = (interface_of file_system_map module_name)
in (FStar_Option.get uu____4224))
in (digest_of_file1 uu____4223))
in (("interface"), (uu____4222)))
in (uu____4217)::[])
end
| uu____4235 -> begin
[]
...
let uu____4243 = (dependences_of file_system_map deps all_cmd_line_files fn1)
in (FStar_All.pipe_right uu____4243 (FStar_List.filter (fun fn2 -> (

let uu____4253 = ((is_interface fn2) && (

let uu____4255 = (lowercase_module_name fn2)
in (Prims.op_Equality uu____4255 module_name)))
in (not (uu____4253)))))))
in (

let binary_deps1 = (FStar_List.sortWith (fun fn11 fn2 -> (

let uu____4265 = (lowercase_module_name fn11)
in (

let uu____4266 = (lowercase_module_name fn2)
in (FStar_String.compare uu____4265 uu____4266)))) binary_deps)
in (

let rec hash_deps = (fun out uu___65_4293 -> (match (uu___65_4293) with
| [] -> begin
...
let uu____4349 = (lowercase_module_name fn2)
in (

let uu____4350 = (digest_of_file1 cache_fn)
in ((uu____4349), (uu____4350))))
in (uu____4344)::out)
in (hash_deps uu____4337 deps1))
end
| uu____4355 -> begin
((

let uu____4357 = (FStar_Options.debug_any ())
...
let uu____4386 = (FStar_All.pipe_right dig (FStar_List.map (fun uu____4405 -> (match (uu____4405) with
| (m, d) -> begin
(

let uu____4412 = (FStar_Util.base64_encode d)
in (FStar_Util.format2 "%s:%s" m uu____4412))
end))))
in (FStar_All.pipe_right uu____4386 (FStar_String.concat "\n"))))


let print_make : deps  ->  unit = (fun uu____4419 -> (match (uu____4419) with
| Mk (deps, file_system_map, all_cmd_line_files) -> begin
(
...
let uu____4446 = (deps_try_find deps f)
in (FStar_All.pipe_right uu____4446 FStar_Option.get))
in (match (uu____4439) with
| (f_deps, uu____4470) -> begin
(
...
let print_full : deps  ->  unit = (fun uu____4488 -> (match (uu____4488) with
| Mk (deps, file_system_map, all_cmd_line_files) -> begin
(
...
let uu____4529 = (FStar_Util.smap_try_find remaining_output_files lc_module_name)
in (FStar_Option.isSome uu____4529)) || (

let uu____4533 = (FStar_Util.smap_try_find visited_other_modules lc_module_name)
in (FStar_Option.isNone uu____4533))))
in (
...
let uu____4563 = (FStar_ST.op_Bang order)
in (ml_file)::uu____4563)
in (FStar_ST.op_Colon_Equals order uu____4560))
end))
in (

let rec aux = (fun uu___66_4671 -> (match (uu___66_4671) with
| [] -> begin
()
end
...
let uu____4689 = (deps_try_find deps file_name)
in (match (uu____4689) with
| FStar_Pervasives_Native.None -> begin
(

let uu____4700 = (FStar_Util.format2 "Impossible: module %s: %s not found" lc_module_name file_name)
in (failwith uu____4700))
end
| FStar_Pervasives_Native.Some (immediate_deps, uu____4702) -> begin
(
...
let uu____4713 = (should_visit lc_module_name)
...
let uu____4718 = (implementation_of file_system_map lc_module_name)
in (visit_file uu____4718));
(

let uu____4722 = (interface_of file_system_map lc_module_name)
in (visit_file uu____4722));
...
let uu____4730 = (FStar_ST.op_Bang order)
in (FStar_List.rev uu____4730));
))))))))))
in (
...
let uu____4801 = (FStar_Util.basename fst_file)
in (check_and_strip_suffix uu____4801))
in (FStar_Option.get uu____4798))
in (FStar_Util.replace_chars uu____4797 46 (*.*) "_"))
...
let uu____4816 = (output_file ".ml" f)
in (norm_path uu____4816)))
in (

let output_krml_file = (fun f -> (

let uu____4823 = (output_file ".krml" f)
in (norm_path uu____4823)))
in (

let output_cmx_file = (fun f -> (

let uu____4830 = (output_file ".cmx" f)
in (norm_path uu____4830)))
in (

let cache_file = (fun f -> (

let uu____4837 = (cache_file_name f)
in (norm_path uu____4837)))
in (
...
let uu____4887 = (deps_try_find deps f)
in (FStar_All.pipe_right uu____4887 FStar_Option.get))
in (match (uu____4880) with
| (f_deps, uu____4911) -> begin
(
...
let uu____4933 = (FStar_Options.prepend_cache_dir norm_f)
in (norm_path uu____4933))
in (FStar_Util.print3 "%s.source: %s \\\n\t%s\n\ttouch $@\n\n" uu____4932 norm_f files3))
end
| uu____4934 -> begin
()
end));
...
let uu____4936 = (cache_file f)
in (FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____4936 norm_f files3));
(
...
let uu____4952 = (output_file ".krml" f)
in (norm_path uu____4952))
in (FStar_Util.smap_try_find transitive_krml uu____4951))
in (match (uu____4940) with
| FStar_Pervasives_Native.Some (uu____4963, already_there, uu____4965) -> begin
already_there
end
| FStar_Pervasives_Native.None -> begin
[]
...
let uu____4988 = (output_file ".krml" f)
in (norm_path uu____4988))
in (
...
let uu____4999 = (output_file ".exe" f)
in (norm_path uu____4999))
in (
...
let uu____5009 = (deps_of (Mk (((deps), (file_system_map), (all_cmd_line_files)))) f)
in (FStar_List.map (fun x -> (

let uu____5017 = (output_file ".krml" x)
in (norm_path uu____5017))) uu____5009))
in (FStar_List.append already_there uu____5006))
in (FStar_List.unique uu____5003))
in ((uu____4998), (uu____5000), (false))))
in (FStar_Util.smap_add transitive_krml uu____4987 uu____4989)));
(

let uu____5028 = (is_implementation f)
in (match (uu____5028) with
| true -> begin
((

let uu____5030 = (output_ml_file f)
in (

let uu____5031 = (cache_file f)
in (FStar_Util.print2 "%s: %s\n\n" uu____5030 uu____5031)));
(
...
let uu____5055 = (lowercase_module_name df)
in (

let uu____5056 = (lowercase_module_name f)
in (Prims.op_disEquality uu____5055 uu____5056))) && (

let uu____5058 = (lowercase_module_name df)
in (FStar_Options.should_extract uu____5058))))))
in (FStar_All.pipe_right extracted_fst_files (FStar_List.map output_cmx_file))))
in ((

let uu____5064 = (

let uu____5065 = (lowercase_module_name f)
in (FStar_Options.should_extract uu____5065))
in (match (uu____5064) with
| true -> begin
(

let uu____5066 = (output_cmx_file f)
in (

let uu____5067 = (output_ml_file f)
in (FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____5066 uu____5067 (FStar_String.concat "\\\n\t" cmx_files))))
end
| uu____5068 -> begin
()
end));
...
let uu____5069 = (output_krml_file f)
in (

let uu____5070 = (cache_file f)
...
let uu____5076 = (lowercase_module_name f)
in (has_implementation file_system_map uu____5076))
in (not (uu____5075))) && (is_interface f))
in (match (uu____5072) with
| true -> begin
(

let uu____5077 = (output_krml_file f)
in (

let uu____5078 = (cache_file f)
in (FStar_Util.print2 "%s: %s\n\n" uu____5077 uu____5078)))
end
| uu____5079 -> begin
()
end))
...
let uu____5083 = (FStar_All.pipe_right keys (FStar_List.filter is_implementation))
in (FStar_All.pipe_right uu____5083 (FStar_Util.sort_with FStar_String.compare)))
in (
...
let uu____5109 = (FStar_Options.should_extract mname)
in (match (uu____5109) with
| true -> begin
(

let uu____5110 = (output_ml_file fst_file)
in (FStar_Util.smap_add ml_file_map mname uu____5110))
end
| uu____5111 -> begin
()
end))))));
...
let uu____5126 = (output_krml_file fst_file)
in (FStar_Util.smap_add krml_file_map mname uu____5126))))));
...
let uu____5148 = (FStar_Util.smap_try_find transitive_krml f)
in (FStar_Util.must uu____5148))
...
let uu____5214 = (FStar_List.map (fun dep1 -> (

let uu____5226 = (make_transitive dep1)
in (match (uu____5226) with
| (uu____5235, deps2) -> begin
(dep1)::deps2
end))) deps1)
in (FStar_List.flatten uu____5214))
in (FStar_List.unique uu____5211))
...
let uu____5255 = (FStar_Util.smap_keys transitive_krml)
in (FStar_List.iter (fun f -> (

let uu____5274 = (make_transitive f)
...
let uu____5288 = (FStar_List.unique ((f)::deps1))
in (FStar_String.concat " " uu____5288))
in (

let wasm = (

let uu____5292 = (FStar_Util.substring exe (Prims.parse_int "0") ((FStar_String.length exe) - (Prims.parse_int "4")))
in (Prims.strcat uu____5292 ".wasm"))
in ((FStar_Util.print2 "%s: %s\n\n" exe deps2);
...
let uu____5296 = (FStar_All.pipe_right all_fst_files (FStar_List.map norm_path))
in (FStar_All.pipe_right uu____5296 (FStar_String.concat " \\\n\t")))
in (FStar_Util.print1 "ALL_FST_FILES=\\\n\t%s\n\n" uu____5295));
(

let uu____5306 = (

let uu____5307 = (FStar_All.pipe_right all_ml_files (FStar_List.map norm_path))
in (FStar_All.pipe_right uu____5307 (FStar_String.concat " \\\n\t")))
in (FStar_Util.print1 "ALL_ML_FILES=\\\n\t%s\n\n" uu____5306));
(

let uu____5316 = (

let uu____5317 = (FStar_All.pipe_right all_krml_files (FStar_List.map norm_path))
in (FStar_All.pipe_right uu____5317 (FStar_String.concat " \\\n\t")))
...
let uu____5331 = (FStar_Options.dep ())
...
| Mk (deps1, uu____5336, uu____5337) -> begin
(print_graph deps1)
end))
end
| FStar_Pervasives_Native.Some (uu____5342) -> begin
