...
let uu____46 = ((l > lext) && (let _0_904 = (FStar_String.substring f (l - lext) lext)
in (_0_904 = ext)))
...
let is_interface : Prims.string  ->  Prims.bool = (fun f -> (let _0_905 = (FStar_String.get f ((FStar_String.length f) - (Prims.parse_int "1")))
in (_0_905 = 'i')))
...
let list_of_option = (fun uu___146_96 -> (match (uu___146_96) with
| Some (x) -> begin
(x)::[]
end
| None -> begin
...
(FStar_List.iter (fun f -> (let _0_906 = (lowercase_module_name f)
in (add_entry _0_906 f))) filenames);
map;
))))))))
...
in ((let _0_907 = (FStar_List.unique (FStar_Util.smap_keys original_map))
...
let names = (let _0_908 = (FStar_List.map (fun x -> x.FStar_Ident.idText) l.FStar_Ident.ns)
in (FStar_List.append _0_908 suffix))
...
let uu____299 = (let _0_909 = (FStar_String.lowercase (FStar_Util.must (check_and_strip_suffix (FStar_Util.basename filename))))
in (_0_909 <> k'))
in (match (uu____299) with
| true -> begin
(let _0_911 = (let _0_910 = (string_of_lid lid true)
in (_0_910)::(filename)::[])
...
let uu____339 = (not ((let _0_912 = (FStar_ST.read deps)
in (FStar_List.existsb (fun d' -> (d' = d)) _0_912))))
in (match (uu____339) with
| true -> begin
(let _0_914 = (let _0_913 = (FStar_ST.read deps)
in (d)::_0_913)
in (FStar_ST.write deps _0_914))
end
| uu____350 -> begin
()
end)))
...
| uu____394 -> begin
(let _0_916 = (let _0_915 = (string_of_lid lid true)
in (_0_915)::[])
...
| true -> begin
(let _0_918 = (let _0_917 = (string_of_lid lid false)
in (_0_917)::[])
...
let uu____472 = (let _0_919 = (FStar_Util.basename filename)
in (_0_919 = "prims.fst"))
...
let uu____475 = (let _0_920 = (FStar_String.lowercase (FStar_Util.basename filename))
in (FStar_Util.starts_with _0_920 "fstar."))
...
let rec collect_fragment = (fun uu___147_550 -> (match (uu___147_550) with
...
and collect_file = (fun uu___148_563 -> (match (uu___148_563) with
| (modul)::[] -> begin
...
and collect_module = (fun uu___149_569 -> (match (uu___149_569) with
...
let uu____591 = (let _0_921 = (FStar_String.lowercase (string_of_lid lid true))
in ((FStar_String.lowercase m) = _0_921))
...
and collect_decl = (fun uu___150_599 -> (match (uu___150_599) with
...
let uu____673 = (let _0_922 = (FStar_ST.read num_of_toplevelmods)
in (_0_922 > (Prims.parse_int "1")))
in (match (uu____673) with
| true -> begin
(Prims.raise (FStar_Errors.Err ((let _0_923 = (string_of_lid lid true)
in (FStar_Util.format1 "Automatic dependency analysis demands one module per file (module %s not supported)" _0_923)))))
end
| uu____676 -> begin
()
end));
...
and collect_tycon = (fun uu___151_677 -> (match (uu___151_677) with
...
and collect_effect_decl = (fun uu___152_775 -> (match (uu___152_775) with
...
and collect_binder = (fun uu___153_798 -> (match (uu___153_798) with
...
and collect_constant = (fun uu___154_813 -> (match (uu___154_813) with
...
and collect_term' = (fun uu___155_825 -> (match (uu___155_825) with
...
let uu____846 = (((FStar_List.length termimps) = (Prims.parse_int "1")) && (FStar_Options.universes ()))
...
(FStar_List.iter (fun uu____854 -> (match (uu____854) with
| (t, uu____858) -> begin
(collect_term t)
end)) termimps);
)
...
| FStar_Parser_AST.App (t1, t2, uu____866) -> begin
((collect_term t1);
(collect_term t2);
)
end
| FStar_Parser_AST.Let (uu____868, patterms, t) -> begin
((FStar_List.iter (fun uu____880 -> (match (uu____880) with
| (pat, t) -> begin
((collect_pattern pat);
(collect_term t);
)
...
((FStar_Util.iter_opt t collect_term);
(FStar_List.iter (fun uu____936 -> (match (uu____936) with
| (uu____939, t) -> begin
(collect_term t)
end)) idterms);
)
end
| FStar_Parser_AST.Project (t, uu____942) -> begin
(collect_term t)
end
...
| FStar_Parser_AST.NamedTyp (uu____971, t) -> begin
(collect_term t)
end
...
and collect_pattern' = (fun uu___156_987 -> (match (uu___156_987) with
...
(FStar_List.iter (fun uu____1010 -> (match (uu____1010) with
| (uu____1013, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun uu____1028 -> (match (uu____1028) with
| (pat, t1, t2) -> begin
((collect_pattern pat);
...
let uu____1040 = (FStar_Parser_Driver.parse_file filename)
in (match (uu____1040) with
| (ast, uu____1048) -> begin
((collect_file ast);
(FStar_ST.read deps);
)
end))));
...
(let _0_928 = (let _0_927 = (let _0_926 = (let _0_925 = (let _0_924 = (FStar_List.unique (FStar_Util.smap_keys graph))
...
in (FStar_List.map (fun dep -> (FStar_Util.format2 "  %s -> %s" (r k) (r dep))) deps)))) _0_924))
in (FStar_String.concat "\n" _0_925))
in (Prims.strcat _0_926 "\n}\n"))
in (Prims.strcat "digraph {\n" _0_927))
in (FStar_Util.write_file "dep.graph" _0_928));
))
...
let verify_flags = (let _0_930 = (FStar_Options.verify_module ())
in (FStar_List.map (fun f -> (let _0_929 = (FStar_Util.mk_ref false)
in ((f), (_0_929)))) _0_930))
in (
...
let uu____1167 = (let _0_931 = (FStar_Util.smap_try_find graph key)
in (_0_931 = None))
in (match (uu____1167) with
| true -> begin
(

let uu____1178 = (FStar_Util.must (FStar_Util.smap_try_find m key))
...
| (Some (impl), Some (uu____1207)) when interface_only -> begin
[]
end
| (Some (impl), uu____1211) -> begin
(collect_one is_user_provided_filename m impl)
end
| (None, uu____1215) -> begin
[]
end)
in (
...
let uu____1258 = (FStar_Util.must (FStar_Util.smap_try_find graph key))
...
let all_deps = (FStar_List.unique (FStar_List.flatten (FStar_List.map (fun dep -> (let _0_932 = (discover ((key)::cycle) dep)
in (dep)::_0_932)) direct_deps)))
in ((FStar_Util.smap_add graph key ((all_deps), (Black)));
(let _0_934 = (let _0_933 = (FStar_ST.read topologically_sorted)
in (key)::_0_933)
in (FStar_ST.write topologically_sorted _0_934));
all_deps;
));
)
end)
...
let uu____1309 = (FStar_Util.must (FStar_Util.smap_try_find m k))
in (match (uu____1309) with
| (Some (intf), Some (impl)) when ((not (partial_discovery)) && (not ((FStar_List.existsML (fun f -> (let _0_935 = (lowercase_module_name f)
in (_0_935 = k))) filenames)))) -> begin
(intf)::(impl)::[]
end
| (Some (intf), Some (impl)) when (FStar_List.existsML (fun f -> ((is_implementation f) && (let _0_936 = (lowercase_module_name f)
in (_0_936 = k)))) filenames) -> begin
(intf)::(impl)::[]
end
| (Some (intf), uu____1334) -> begin
(intf)::[]
end
...
let by_target = (let _0_938 = (FStar_Util.smap_keys graph)
in (FStar_List.collect (fun k -> (
...
let deps_as_filenames = (let _0_937 = (FStar_List.collect must_find deps)
in (FStar_List.append _0_937 suffix))
in ((f), (deps_as_filenames)))))))) as_list)))) _0_938))
in (

let topologically_sorted = (let _0_939 = (FStar_ST.read topologically_sorted)
in (FStar_List.collect must_find_r _0_939))
in ((FStar_List.iter (fun uu____1389 -> (match (uu____1389) with
| (m, r) -> begin
(

let uu____1397 = ((not ((FStar_ST.read r))) && (not ((FStar_Options.interactive ()))))
...
let uu____1403 = ((k > (Prims.parse_int "4")) && (let _0_940 = (FStar_String.substring m (k - (Prims.parse_int "4")) (Prims.parse_int "4"))
in (_0_940 = ".fst")))
in (match (uu____1403) with
| true -> begin
(let _0_941 = (FStar_String.substring m (Prims.parse_int "0") (k - (Prims.parse_int "4")))
in (FStar_Util.format1 " Did you mean %s ?" _0_941))
end
| uu____1413 -> begin
""
end)))
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun uu____1438 -> (match (uu____1438) with
| (f, deps) -> begin
(
...
let print = (fun uu____1468 -> (match (uu____1468) with
| (make_deps, uu____1481, graph) -> begin
(

let uu____1499 = (FStar_Options.dep ())
