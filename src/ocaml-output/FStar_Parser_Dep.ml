...
(let _116_4 = (FStar_String.substring f 0 (l - lext))
in Some (_116_4))
end else begin
None
end))) suffixes)
...
let print_map = (fun m -> (let _116_13 = (let _116_12 = (FStar_Util.smap_keys m)
in (FStar_List.unique _116_12))
in (FStar_List.iter (fun k -> (let _116_11 = (let _116_10 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must _116_10))
in (FStar_Util.print2 "%s: %s\n" k _116_11))) _116_13)))

let build_map = (fun _49_26 -> (match (()) with
| () -> begin
(let include_directories = (let _116_16 = (FStar_Util.getcwd ())
in (FStar_Options.get_include_path _116_16))
...
(let _116_20 = (let _116_19 = (FStar_Util.format1 "I\'m case insensitive, and I found the same file twice (%s)" f)
in FStar_Absyn_Syntax.Err (_116_19))
in (Prims.raise _116_20))
end else begin
()
end
in (let _49_40 = if ((is_interface existing_file) = (is_interface f)) then begin
(let _116_22 = (let _116_21 = (FStar_Util.format1 "Found both a .fs and a .fst (or both a .fsi and a .fsti) (%s)" f)
in FStar_Absyn_Syntax.Err (_116_21))
in (Prims.raise _116_22))
end else begin
()
end
in if (not ((is_interface existing_file))) then begin
(let _116_23 = (FStar_Util.join_paths d f)
in (FStar_Util.smap_add map key _116_23))
end else begin
()
end))
end
| None -> begin
(let _116_24 = (FStar_Util.join_paths d f)
in (FStar_Util.smap_add map key _116_24))
end))
end
| None -> begin
()
...
in (let _49_56 = (let _116_34 = (let _116_33 = (FStar_Util.smap_keys original_map)
in (FStar_List.unique _116_33))
...
in (let filename = (let _116_32 = (FStar_Util.smap_try_find original_map k)
in (FStar_Util.must _116_32))
...
in (let names = (let _116_40 = (FStar_List.map (fun x -> x.FStar_Absyn_Syntax.idText) l.FStar_Absyn_Syntax.ns)
in (FStar_List.append _116_40 suffix))
in (FStar_String.concat "." names))))

let lowercase_join_longident = (fun l last -> (let _116_45 = (string_of_lid l last)
in (FStar_String.lowercase _116_45)))

let check_module_declaration_against_filename = (fun lid filename -> (let k' = (lowercase_join_longident lid true)
in if ((let _116_52 = (let _116_51 = (let _116_50 = (FStar_Util.basename filename)
in (check_and_strip_suffix _116_50))
in (FStar_Util.must _116_51))
in (FStar_String.lowercase _116_52)) <> k') then begin
(let _116_54 = (let _116_53 = (string_of_lid lid true)
in (_116_53)::(filename)::[])
...
in (let add_dep = (fun d -> if (not ((let _116_63 = (FStar_ST.read deps)
in (FStar_List.existsb (fun d' -> (d' = d)) _116_63)))) then begin
(let _116_65 = (let _116_64 = (FStar_ST.read deps)
in (d)::_116_64)
in (FStar_ST.op_Colon_Equals deps _116_65))
end else begin
()
end)
...
(let _116_69 = (let _116_68 = (string_of_lid lid true)
in (_116_68)::[])
...
in (let me = (let _116_80 = (let _116_79 = (let _116_78 = (FStar_Util.basename filename)
in (check_and_strip_suffix _116_78))
in (FStar_Util.must _116_79))
in (FStar_String.lowercase _116_80))
...
in (FStar_List.filter (fun lid -> ((let _116_86 = (lowercase_join_longident lid true)
in (index_or_length _116_86 ordered)) < my_index)) desired_opens)))))))
...
| _49_135 -> begin
(let _116_106 = (let _116_105 = (FStar_Util.format1 "File %s does not respect the one module per file convention" filename)
in FStar_Absyn_Syntax.Err (_116_105))
in (Prims.raise _116_106))
end))
...
(let _116_121 = (let _116_120 = (string_of_lid lid false)
in (_116_120)::[])
...
| () -> begin
(let _116_145 = (let _116_144 = (FStar_Util.smap_keys graph)
in (FStar_List.unique _116_144))
in (FStar_List.iter (fun k -> (let _116_143 = (let _116_142 = (let _116_141 = (let _116_140 = (FStar_Util.smap_try_find graph k)
in (FStar_Util.must _116_140))
in (Prims.fst _116_141))
in (FStar_String.concat " " _116_142))
in (FStar_Util.print2 "%s: %s\n" k _116_143))) _116_145))
end))
in (let rec discover = (fun f -> (let short = (FStar_Util.basename f)
in (let _49_519 = (let _116_148 = (FStar_Util.smap_try_find graph short)
in (FStar_Util.must _116_148))
...
in (let all_deps = (let _116_152 = (let _116_151 = (FStar_List.map (fun dep -> (let _116_150 = (discover dep)
in (dep)::_116_150)) direct_deps)
in (FStar_List.flatten _116_151))
in (FStar_List.unique _116_152))
in (let _49_535 = (FStar_Util.smap_add graph short (all_deps, Black))
in all_deps)))
end)
end))))
in (FStar_List.map (fun f -> (let _116_155 = (let _116_154 = (discover f)
in (FStar_List.rev _116_154))
in (f, _116_155))) filenames))))))))
