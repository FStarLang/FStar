...
let list_of_option = (fun uu___127_100 -> (match (uu___127_100) with
| Some (x) -> begin
(x)::[]
end
| None -> begin
...
let rec collect_fragment = (fun uu___128_621 -> (match (uu___128_621) with
...
and collect_file = (fun uu___129_634 -> (match (uu___129_634) with
| (modul)::[] -> begin
...
and collect_module = (fun uu___130_640 -> (match (uu___130_640) with
...
and collect_decl = (fun uu___131_674 -> (match (uu___131_674) with
...
and collect_tycon = (fun uu___132_751 -> (match (uu___132_751) with
...
and collect_effect_decl = (fun uu___133_849 -> (match (uu___133_849) with
| FStar_Parser_AST.DefineEffect (uu____850, binders, t, decls) -> begin
((collect_binders binders);
(collect_term t);
(collect_decls decls);
)
end
| FStar_Parser_AST.RedefineEffect (uu____860, binders, t) -> begin
((collect_binders binders);
(collect_term t);
)
end))
and collect_binders = (fun binders -> (FStar_List.iter collect_binder binders))
and collect_binder = (fun uu___134_868 -> (match (uu___134_868) with
...
and collect_constant = (fun uu___135_883 -> (match (uu___135_883) with
| FStar_Const.Const_int (uu____884, Some (signedness, width)) -> begin
(
...
let uu____894 = (FStar_Util.format2 "fstar.%sint%s" u w)
in (add_dep uu____894))))
end
| uu____895 -> begin
()
end))
and collect_term' = (fun uu___136_896 -> (match (uu___136_896) with
...
let uu____904 = (FStar_Ident.lid_of_path (FStar_Ident.path_of_text "FStar.List.Tot.Base.append") FStar_Range.dummyRange)
in FStar_Parser_AST.Name (uu____904))
in (collect_term' uu____903))
end
| uu____905 -> begin
()
end);
...
| FStar_Parser_AST.App (t1, t2, uu____938) -> begin
((collect_term t1);
(collect_term t2);
)
end
| FStar_Parser_AST.Let (uu____940, patterms, t) -> begin
((FStar_List.iter (fun uu____952 -> (match (uu____952) with
| (pat, t) -> begin
((collect_pattern pat);
(collect_term t);
)
...
((FStar_Util.iter_opt t collect_term);
(FStar_List.iter (fun uu____1008 -> (match (uu____1008) with
| (uu____1011, t) -> begin
(collect_term t)
end)) idterms);
)
end
| FStar_Parser_AST.Project (t, uu____1014) -> begin
(collect_term t)
end
...
| FStar_Parser_AST.NamedTyp (uu____1043, t) -> begin
(collect_term t)
end
...
and collect_pattern' = (fun uu___137_1059 -> (match (uu___137_1059) with
...
(FStar_List.iter (fun uu____1082 -> (match (uu____1082) with
| (uu____1085, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun uu____1100 -> (match (uu____1100) with
| (pat, t1, t2) -> begin
((collect_pattern pat);
...
let uu____1112 = (FStar_Parser_Driver.parse_file filename)
in (match (uu____1112) with
| (ast, uu____1120) -> begin
((collect_file ast);
(FStar_ST.read deps);
)
end))));
...
let uu____1156 = (FStar_Util.smap_keys graph)
in (FStar_List.unique uu____1156))
...
let uu____1168 = (FStar_Util.smap_try_find graph k)
in (FStar_Util.must uu____1168))
in (Prims.fst uu____1164))
in (

let r = (fun s -> (FStar_Util.replace_char s '.' '_'))
in (FStar_List.map (fun dep -> (FStar_Util.format2 "  %s -> %s" (r k) (r dep))) deps)))) uu____1154))
in (FStar_String.concat "\n" uu____1152))
in (Prims.strcat uu____1151 "\n}\n"))
in (Prims.strcat "digraph {\n" uu____1150))
in (FStar_Util.write_file "dep.graph" uu____1149));
))
...
let uu____1230 = (FStar_Options.verify_module ())
in (FStar_List.map (fun f -> (

let uu____1236 = (FStar_Util.mk_ref false)
in ((f), (uu____1236)))) uu____1230))
in (
...
let uu____1252 = ((FStar_Options.verify_all ()) || (FStar_Options.extract_all ()))
in (not (uu____1252)))
in (
...
let uu____1264 = (FStar_Util.smap_try_find graph key)
in (uu____1264 = None))
...
let uu____1284 = (FStar_Util.smap_try_find m key)
in (FStar_Util.must uu____1284))
...
| (Some (impl), Some (uu____1314)) when interface_only -> begin
[]
end
| (Some (impl), uu____1318) -> begin
(collect_one is_user_provided_filename m impl)
end
| (None, uu____1322) -> begin
[]
end)
in (
...
let uu____1369 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must uu____1369))
...
let uu____1400 = (FStar_List.map (fun dep -> (

let uu____1405 = (discover ((key)::cycle) dep)
in (dep)::uu____1405)) direct_deps)
in (FStar_List.flatten uu____1400))
in (FStar_List.unique uu____1398))
...
in (key)::uu____1415)
in (FStar_ST.write topologically_sorted uu____1413));
all_deps;
));
)
end)
...
let uu____1437 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____1437))
in (match (uu____1432) with
| (Some (intf), Some (impl)) when ((not (partial_discovery)) && (

let uu____1456 = (FStar_List.existsML (fun f -> (

let uu____1458 = (lowercase_module_name f)
in (uu____1458 = k))) filenames)
in (not (uu____1456)))) -> begin
(intf)::(impl)::[]
end
| (Some (intf), Some (impl)) when (FStar_List.existsML (fun f -> ((is_implementation f) && (

let uu____1464 = (lowercase_module_name f)
in (uu____1464 = k)))) filenames) -> begin
(intf)::(impl)::[]
end
| (Some (intf), uu____1466) -> begin
(intf)::[]
end
...
let uu____1480 = (must_find f)
in (FStar_List.rev uu____1480)))
in (
...
let uu____1489 = (FStar_Util.smap_keys graph)
in (FStar_List.sortWith (fun x y -> (FStar_String.compare x y)) uu____1489))
in (FStar_List.collect (fun k -> (
...
in ((f), (deps_as_filenames)))))))) as_list)))) uu____1487))
in (
...
let deps = (FStar_List.map (fun s -> (FStar_Util.replace_chars s ' ' "\\ ")) deps)
