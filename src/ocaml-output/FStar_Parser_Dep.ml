...
let _69_147 = (let _161_121 = (let _161_120 = (string_of_lid lid true)
in (_161_120)::[])
in (FStar_Util.fprint FStar_Util.stderr "registering module reference %s\n" _161_121))
in (let _161_122 = (lowercase_join_longident lid true)
in (try_key _161_122)))
end else begin
()
end)))
...
if (let _161_124 = (let _161_123 = (FStar_Util.basename filename)
in (FStar_String.lowercase _161_123))
in (FStar_Util.starts_with _161_124 "fstar.")) then begin
...
let _69_150 = (FStar_List.iter (record_open false) auto_open)
in (
...
let _69_178 = (FStar_Util.fprint FStar_Util.stderr "Warning: file %s does not respect the one module per file convention\n" ((filename)::[]))
...
let _69_189 = (check_module_declaration_against_filename lid filename)
in (

let _69_197 = (match (verify_mode) with
| VerifyAll -> begin
(let _161_145 = (string_of_lid lid true)
in (FStar_Options.add_verify_module _161_145))
end
| VerifyFigureItOut -> begin
if is_user_provided_filename then begin
(let _161_146 = (string_of_lid lid true)
in (FStar_Options.add_verify_module _161_146))
end else begin
()
end
end
| VerifyUserList -> begin
(FStar_List.iter (fun _69_196 -> (match (_69_196) with
| (m, r) -> begin
if ((FStar_String.lowercase m) = (let _161_148 = (string_of_lid lid true)
in (FStar_String.lowercase _161_148))) then begin
...
let _69_208 = (let _161_152 = (lowercase_join_longident lid true)
in (add_dep _161_152))
in (record_module_alias ident lid))
end
| FStar_Parser_AST.ToplevelLet (_69_211, _69_213, patterms) -> begin
(FStar_List.iter (fun _69_219 -> (match (_69_219) with
| (pat, t) -> begin
(

let _69_220 = (collect_pattern pat)
in (collect_term t))
end)) patterms)
end
| FStar_Parser_AST.KindAbbrev (_69_223, binders, t) -> begin
(

let _69_228 = (collect_term t)
...
| FStar_Parser_AST.SubEffect ({FStar_Parser_AST.msource = _69_258; FStar_Parser_AST.mdest = _69_256; FStar_Parser_AST.lift_op = FStar_Parser_AST.ReifiableLift (t0, t1)}) -> begin
(

let _69_261 = (collect_term t0)
in (collect_term t1))
end
| FStar_Parser_AST.Tycon (_69_264, ts) -> begin
(FStar_List.iter collect_tycon ts)
end
| FStar_Parser_AST.Exception (_69_269, t) -> begin
(FStar_Util.iter_opt t collect_term)
end
| (FStar_Parser_AST.NewEffectForFree (_, ed)) | (FStar_Parser_AST.NewEffect (_, ed)) -> begin
(collect_effect_decl ed)
end
| FStar_Parser_AST.Pragma (_69_283) -> begin
()
end
| FStar_Parser_AST.TopLevelModule (lid) -> begin
(let _161_156 = (let _161_155 = (let _161_154 = (string_of_lid lid true)
in (FStar_Util.format1 "Automatic dependency analysis demands one module per file (module %s not supported)" _161_154))
in FStar_Absyn_Syntax.Err (_161_155))
in (Prims.raise _161_156))
end))
and collect_tycon = (fun _69_6 -> (match (_69_6) with
| FStar_Parser_AST.TyconAbstract (_69_289, binders, k) -> begin
(

let _69_294 = (collect_binders binders)
in (FStar_Util.iter_opt k collect_term))
end
| FStar_Parser_AST.TyconAbbrev (_69_297, binders, k, t) -> begin
(

let _69_303 = (collect_binders binders)
in (

let _69_305 = (FStar_Util.iter_opt k collect_term)
in (collect_term t)))
end
| FStar_Parser_AST.TyconRecord (_69_308, binders, k, identterms) -> begin
(

let _69_314 = (collect_binders binders)
in (

let _69_316 = (FStar_Util.iter_opt k collect_term)
in (FStar_List.iter (fun _69_321 -> (match (_69_321) with
| (_69_319, t) -> begin
(collect_term t)
end)) identterms)))
end
| FStar_Parser_AST.TyconVariant (_69_323, binders, k, identterms) -> begin
(

let _69_329 = (collect_binders binders)
in (

let _69_331 = (FStar_Util.iter_opt k collect_term)
in (FStar_List.iter (fun _69_338 -> (match (_69_338) with
| (_69_334, t, _69_337) -> begin
(FStar_Util.iter_opt t collect_term)
end)) identterms)))
end))
and collect_effect_decl = (fun _69_7 -> (match (_69_7) with
| FStar_Parser_AST.DefineEffect (_69_341, binders, t, decls, actions) -> begin
(

let _69_348 = (collect_binders binders)
in (

let _69_350 = (collect_term t)
in (

let _69_352 = (collect_decls decls)
in (collect_decls actions))))
end
| FStar_Parser_AST.RedefineEffect (_69_355, binders, t) -> begin
(

let _69_360 = (collect_binders binders)
in (collect_term t))
end))
...
| FStar_Const.Const_int (_69_400, Some (signedness, width)) -> begin
(
...
in (let _161_165 = (FStar_Util.format2 "fstar.%sint%s" u w)
in (add_dep _161_165))))
end
| _69_416 -> begin
()
...
let _69_425 = if (s = "@") then begin
(let _161_168 = (let _161_167 = (FStar_Ident.lid_of_path (FStar_Ident.path_of_text "FStar.List.Tot.append") FStar_Range.dummyRange)
in FStar_Parser_AST.Name (_161_167))
in (collect_term' _161_168))
end else begin
()
end
in (FStar_List.iter collect_term ts))
end
| FStar_Parser_AST.Tvar (_69_428) -> begin
()
end
...
let _69_437 = if (((FStar_List.length termimps) = 1) && (FStar_Options.universes ())) then begin
(record_lid true lid)
end else begin
()
end
in (FStar_List.iter (fun _69_442 -> (match (_69_442) with
| (t, _69_441) -> begin
(collect_term t)
end)) termimps))
end
...
let _69_447 = (collect_patterns pats)
in (collect_term t))
end
| FStar_Parser_AST.App (t1, t2, _69_452) -> begin
(

let _69_455 = (collect_term t1)
in (collect_term t2))
end
| FStar_Parser_AST.Let (_69_458, patterms, t) -> begin
(

let _69_468 = (FStar_List.iter (fun _69_465 -> (match (_69_465) with
| (pat, t) -> begin
(

let _69_466 = (collect_pattern pat)
in (collect_term t))
end)) patterms)
in (collect_term t))
end
...
let _69_474 = (record_open true lid)
in (collect_term t))
end
| FStar_Parser_AST.Seq (t1, t2) -> begin
(

let _69_480 = (collect_term t1)
in (collect_term t2))
end
...
let _69_489 = (collect_term t2)
in (collect_term t3)))
end
| (FStar_Parser_AST.Match (t, bs)) | (FStar_Parser_AST.TryWith (t, bs)) -> begin
(

let _69_497 = (collect_term t)
in (collect_branches bs))
end
| FStar_Parser_AST.Ascribed (t1, t2) -> begin
(

let _69_503 = (collect_term t1)
in (collect_term t2))
end
| FStar_Parser_AST.Record (t, idterms) -> begin
(

let _69_509 = (FStar_Util.iter_opt t collect_term)
in (FStar_List.iter (fun _69_514 -> (match (_69_514) with
| (_69_512, t) -> begin
(collect_term t)
end)) idterms))
end
| FStar_Parser_AST.Project (t, _69_517) -> begin
(collect_term t)
end
| (FStar_Parser_AST.Product (binders, t)) | (FStar_Parser_AST.Sum (binders, t)) -> begin
(

let _69_526 = (collect_binders binders)
in (collect_term t))
end
...
let _69_537 = (FStar_List.iter (FStar_List.iter collect_term) ts)
in (collect_term t)))
end
| FStar_Parser_AST.Refine (binder, t) -> begin
(

let _69_543 = (collect_binder binder)
in (collect_term t))
end
| FStar_Parser_AST.NamedTyp (_69_546, t) -> begin
(collect_term t)
end
...
(FStar_List.iter (fun _69_608 -> (match (_69_608) with
| (_69_606, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
let _69_613 = (collect_pattern p)
in (collect_term t))
end))
and collect_branches = (fun bs -> (FStar_List.iter collect_branch bs))
and collect_branch = (fun _69_619 -> (match (_69_619) with
| (pat, t1, t2) -> begin
(

let _69_620 = (collect_pattern pat)
in (

let _69_622 = (FStar_Util.iter_opt t1 collect_term)
in (collect_term t2)))
end))
in (
...
let _69_628 = (FStar_Util.print_endline "A DOT-format graph has been dumped in the current directory as dep.graph")
in (

let _69_630 = (FStar_Util.print_endline "With GraphViz installed, try: fdp -Tpng -odep.png dep.graph")
in (

let _69_632 = (FStar_Util.print_endline "Hint: cat dep.graph | grep -v _ | grep -v prims")
in (let _161_193 = (let _161_192 = (let _161_191 = (let _161_190 = (let _161_189 = (let _161_188 = (FStar_Util.smap_keys graph)
in (FStar_List.unique _161_188))
in (FStar_List.collect (fun k -> (

let deps = (let _161_184 = (let _161_183 = (FStar_Util.smap_try_find graph k)
in (FStar_Util.must _161_183))
in (Prims.fst _161_184))
in (

let r = (fun s -> (FStar_Util.replace_char s '.' '_'))
in (FStar_List.map (fun dep -> (FStar_Util.format2 "  %s -> %s" (r k) (r dep))) deps)))) _161_189))
in (FStar_String.concat "\n" _161_190))
in (Prims.strcat _161_191 "\n}\n"))
in (Prims.strcat "digraph {\n" _161_192))
in (FStar_Util.write_file "dep.graph" _161_193))))))
...
let verify_flags = (let _161_200 = (FStar_Options.verify_module ())
in (FStar_List.map (fun f -> (let _161_199 = (FStar_ST.alloc false)
in ((f), (_161_199)))) _161_200))
in (
...
let _69_651 = (let _161_208 = (FStar_Util.smap_try_find m key)
in (FStar_Util.must _161_208))
in (match (_69_651) with
| (intf, impl) -> begin
(
...
let _69_661 = (FStar_Util.smap_add graph key ((deps), (White)))
...
let _69_663 = (let _161_209 = (FStar_List.map lowercase_module_name filenames)
in (FStar_List.iter (discover_one true) _161_209))
in (
...
let _69_672 = (let _161_214 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must _161_214))
in (match (_69_672) with
| (direct_deps, color) -> begin
(match (color) with
| Gray -> begin
(

let _69_674 = (FStar_Util.print1 "Warning: recursive dependency on module %s\n" key)
in (

let _69_676 = (FStar_Util.print1 "The cycle is: %s \n" (FStar_String.concat " -> " cycle))
in (

let _69_678 = (print_graph immediate_graph)
in (

let _69_680 = (FStar_Util.print_string "\n")
in (FStar_All.exit 1)))))
end
| Black -> begin
direct_deps
...
let _69_684 = (FStar_Util.smap_add graph key ((direct_deps), (Gray)))
in (

let all_deps = (let _161_218 = (let _161_217 = (FStar_List.map (fun dep -> (let _161_216 = (discover ((key)::cycle) dep)
in (dep)::_161_216)) direct_deps)
in (FStar_List.flatten _161_217))
in (FStar_List.unique _161_218))
in (

let _69_688 = (FStar_Util.smap_add graph key ((all_deps), (Black)))
in (

let _69_690 = (let _161_220 = (let _161_219 = (FStar_ST.read topologically_sorted)
in (key)::_161_219)
in (FStar_ST.op_Colon_Equals topologically_sorted _161_220))
in all_deps))))
end)
end)))
in (
...
let must_find_r = (fun f -> (let _161_225 = (must_find f)
in (FStar_List.rev _161_225)))
in (

let by_target = (let _161_230 = (FStar_Util.smap_keys graph)
in (FStar_List.collect (fun k -> (
...
let deps = (let _161_228 = (discover k)
in (FStar_List.rev _161_228))
in (

let deps_as_filenames = (let _161_229 = (FStar_List.collect must_find deps)
in (FStar_List.append _161_229 suffix))
in ((f), (deps_as_filenames)))))))) as_list)))) _161_230))
in (

let topologically_sorted = (let _161_231 = (FStar_ST.read topologically_sorted)
in (FStar_List.collect must_find_r _161_231))
in (

let _69_710 = (FStar_List.iter (fun _69_709 -> (match (_69_709) with
| (m, r) -> begin
if ((not ((FStar_ST.read r))) && (not ((FStar_Options.interactive ())))) then begin
(let _161_234 = (let _161_233 = (FStar_Util.format2 "You passed --verify_module %s but I found no file that contains [module %s] in the dependency graph\n" m m)
in FStar_Absyn_Syntax.Err (_161_233))
in (Prims.raise _161_234))
end else begin
()
end
end)) verify_flags)
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun _69_715 -> (match (_69_715) with
| (f, deps) -> begin
(
...
let print = (fun _69_722 -> (match (_69_722) with
| (make_deps, _69_720, graph) -> begin
