           ...
           (FStar_List.iter
              (fun f  ->
                 ...
                 | (f_deps,uu____4542) ->
                     ...
                     ((let uu____4558 = is_interface f  in
                       if uu____4558
                       then
                         let uu____4559 =
                           let uu____4560 =
                             FStar_Options.prepend_cache_dir norm_f  in
                           norm_path uu____4560  in
                         FStar_Util.print3
                           "%s.source: %s \\\n\t%s\n\ttouch $@\n\n"
                           uu____4559 norm_f files3
                       else ());
                      (let uu____4563 = cache_file f  in
                       FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____4563
                         norm_f files3);
                      (let uu____4564 = is_implementation f  in
                       if uu____4564
                       then
                         ((let uu____4566 = output_ml_file f  in
                           let uu____4567 = cache_file f  in
                           FStar_Util.print2 "%s: %s\n\n" uu____4566
                             uu____4567);
                          (let cmx_files =
                             let fst_files =
                           ...
                           (let uu____4598 =
                              ...
                              FStar_Options.should_extract uu____4599  in
                            if uu____4598
                            then
                              let uu____4600 = output_cmx_file f  in
                              let uu____4601 = output_ml_file f  in
                              FStar_Util.print3 "%s: %s \\\n\t%s\n\n"
                                uu____4600 uu____4601
                                (FStar_String.concat "\\\n\t" cmx_files)
                            else ());
                           (let uu____4603 = output_krml_file f  in
                            let uu____4604 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____4603
                              uu____4604)))
                       else
                         (let uu____4606 =
                            (let uu____4609 =
                               let uu____4610 = lowercase_module_name f  in
                               has_implementation file_system_map uu____4610
                                in
                             Prims.op_Negation uu____4609) &&
                              (is_interface f)
                             in
                          if uu____4606
                          then
                            let uu____4611 = output_krml_file f  in
                            let uu____4612 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____4611
                              uu____4612
                          else ())))));
         (let all_fst_files =
            ...
            FStar_All.pipe_right all_fst_files
              (FStar_List.iter
                 (fun fst_file  ->
                    ...
                    let uu____4643 = FStar_Options.should_extract mname  in
                    if uu____4643
                    then
                      let uu____4644 = output_ml_file fst_file  in
                      FStar_Util.smap_add ml_file_map mname uu____4644
                    else ()));
          ...
          let all_krml_files =
            ...
            FStar_All.pipe_right keys
              (FStar_List.iter
                 (fun fst_file  ->
                    let mname = lowercase_module_name fst_file  in
                    let uu____4660 = output_krml_file fst_file  in
                    FStar_Util.smap_add krml_file_map mname uu____4660));
          ...
          (let uu____4673 =
             let uu____4674 =
               FStar_All.pipe_right all_ml_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____4674 (FStar_String.concat " \\\n\t")
              in
          ...
          (let uu____4683 =
             let uu____4684 =
               FStar_All.pipe_right all_krml_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____4684 (FStar_String.concat " \\\n\t")
              in
