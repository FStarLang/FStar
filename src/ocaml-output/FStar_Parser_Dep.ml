    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        ...
        (FStar_All.pipe_right keys
           (FStar_List.iter
              (fun f  ->
                 let uu____4889 =
                   let uu____4894 = deps_try_find deps f  in
                   FStar_All.pipe_right uu____4894 FStar_Option.get  in
                 ...
                 | (f_deps,uu____4916) ->
                     ...
                     ((let uu____4932 = is_interface f  in
                       if uu____4932
                       then
                         let uu____4933 =
                           let uu____4934 =
                             FStar_Options.prepend_cache_dir norm_f  in
                           norm_path uu____4934  in
                         FStar_Util.print3
                           "%s.source: %s \\\n\t%s\n\ttouch $@\n\n"
                           uu____4933 norm_f files3
                       else ());
                      (let uu____4937 = cache_file f  in
                       FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____4937
                         norm_f files3);
                      (let uu____4939 =
                         let uu____4940 = output_file ".krml" f  in
                         norm_path uu____4940  in
                       let uu____4941 =
                         let uu____4950 =
                           let uu____4951 = output_file ".exe" f  in
                           norm_path uu____4951  in
                         let uu____4952 =
                           let uu____4955 =
                             deps_of
                               (Mk
                           ...
                           FStar_List.map
                             (fun x  ->
                                let uu____4963 = output_file ".krml" x  in
                                norm_path uu____4963) uu____4955
                            in
                         (uu____4950, uu____4952, false)  in
                       FStar_Util.smap_add transitive_krml uu____4939
                         uu____4941);
                      (let uu____4974 = is_implementation f  in
                       if uu____4974
                       then
                         ((let uu____4976 = output_ml_file f  in
                           let uu____4977 = cache_file f  in
                           FStar_Util.print2 "%s: %s\n\n" uu____4976
                             uu____4977);
                          (let cmx_files =
                             let fst_files =
                             ...
                             FStar_All.pipe_right extracted_fst_files
                              ...
                              in
                           (let uu____5008 =
                              let uu____5009 = lowercase_module_name f  in
                              FStar_Options.should_extract uu____5009  in
                            if uu____5008
                            then
                              let uu____5010 = output_cmx_file f  in
                              let uu____5011 = output_ml_file f  in
                              FStar_Util.print3 "%s: %s \\\n\t%s\n\n"
                                uu____5010 uu____5011
                                (FStar_String.concat "\\\n\t" cmx_files)
                            else ());
                           (let uu____5013 = output_krml_file f  in
                            let uu____5014 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____5013
                              uu____5014)))
                       else
                         (let uu____5016 =
                            (let uu____5019 =
                               let uu____5020 = lowercase_module_name f  in
                               has_implementation file_system_map uu____5020
                                in
                          ...
                          if uu____5016
                          then
                            let uu____5021 = output_krml_file f  in
         ...
         (let all_fst_files =
            let uu____5027 =
              FStar_All.pipe_right keys (FStar_List.filter is_implementation)
               in
            FStar_All.pipe_right uu____5027
            ...
            FStar_All.pipe_right all_fst_files
              (FStar_List.iter
                 (fun fst_file  ->
                    ...
                    let uu____5053 = FStar_Options.should_extract mname  in
                    if uu____5053
                    then
                      let uu____5054 = output_ml_file fst_file  in
                      FStar_Util.smap_add ml_file_map mname uu____5054
                    else ()));
          ...
          let all_krml_files =
            ...
            FStar_All.pipe_right keys
              (FStar_List.iter
                 (fun fst_file  ->
                    let mname = lowercase_module_name fst_file  in
                    let uu____5070 = output_krml_file fst_file  in
                    FStar_Util.smap_add krml_file_map mname uu____5070));
            sort_output_files krml_file_map  in
          let rec make_transitive f =
            let uu____5083 =
              let uu____5092 = FStar_Util.smap_try_find transitive_krml f  in
              FStar_Util.must uu____5092  in
                   ...
                   (let deps2 =
                      let uu____5155 =
                        let uu____5158 =
                          FStar_List.map
                            (fun dep1  ->
                               let uu____5170 = make_transitive dep1  in
                               match uu____5170 with
                               | (uu____5179,deps2) -> dep1 :: deps2) deps1
                           in
                        FStar_List.flatten uu____5158  in
                      FStar_List.unique uu____5155  in
                    FStar_Util.smap_add transitive_krml f (exe, deps2, true);
                    (exe, deps2)))
             in
          (let uu____5199 = FStar_Util.smap_keys transitive_krml  in
           FStar_List.iter
             (fun f  ->
                let uu____5218 = make_transitive f  in
                match uu____5218 with
                | (exe,deps1) ->
                    let deps2 =
                      let uu____5232 = FStar_List.unique (f :: deps1)  in
                      FStar_String.concat " " uu____5232  in
                    let wasm =
                      let uu____5236 =
                        FStar_Util.substring exe (Prims.parse_int "0")
                          ((FStar_String.length exe) - (Prims.parse_int "4"))
                         in
                      Prims.strcat uu____5236 ".wasm"  in
                    (FStar_Util.print2 "%s: %s\n\n" exe deps2;
                     FStar_Util.print2 "%s: %s\n\n" wasm deps2)) uu____5199);
          (let uu____5239 =
             let uu____5240 =
