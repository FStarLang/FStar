    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        ...
        FStar_All.pipe_right keys
          (FStar_List.iter
             (fun f  ->
                let uu____4439 =
                  let uu____4446 = deps_try_find deps f  in
                  FStar_All.pipe_right uu____4446 FStar_Option.get  in
    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        let sort_output_files orig_output_file_map =
          ...
          let should_visit lc_module_name =
            (let uu____4529 =
               FStar_Util.smap_try_find remaining_output_files lc_module_name
                in
             FStar_Option.isSome uu____4529) ||
              (let uu____4533 =
                 FStar_Util.smap_try_find visited_other_modules
                   lc_module_name
                  in
               FStar_Option.isNone uu____4533)
             in
          ...
          let emit_output_file_opt ml_file_opt =
            ...
            | FStar_Pervasives_Native.Some ml_file ->
                let uu____4560 =
                  let uu____4563 = FStar_ST.op_Bang order  in ml_file ::
                    uu____4563
                   in
                FStar_ST.op_Colon_Equals order uu____4560
             in
          let rec aux uu___66_4671 =
            match uu___66_4671 with
            | [] -> ()
            | lc_module_name::modules_to_extract ->
                let visit_file file_opt =
                  ...
                  | FStar_Pervasives_Native.Some file_name ->
                      let uu____4689 = deps_try_find deps file_name  in
          ...
          (let uu____4730 = FStar_ST.op_Bang order  in
           FStar_List.rev uu____4730)
           in
        let keys = deps_keys deps  in
        let output_file ext fst_file =
          let ml_base_name =
            let uu____4797 =
              let uu____4798 =
                let uu____4801 = FStar_Util.basename fst_file  in
                check_and_strip_suffix uu____4801  in
              FStar_Option.get uu____4798  in
            FStar_Util.replace_chars uu____4797 46 "_"  in
          FStar_Options.prepend_output_dir (Prims.strcat ml_base_name ext)
           in
        let norm_path s = FStar_Util.replace_chars s 92 "/"  in
        let output_ml_file f =
          let uu____4816 = output_file ".ml" f  in norm_path uu____4816  in
        let output_krml_file f =
          let uu____4823 = output_file ".krml" f  in norm_path uu____4823  in
        let output_cmx_file f =
          let uu____4830 = output_file ".cmx" f  in norm_path uu____4830  in
        let cache_file f =
          let uu____4837 = cache_file_name f  in norm_path uu____4837  in
        ...
        (FStar_All.pipe_right keys
           (FStar_List.iter
              (fun f  ->
                 let uu____4880 =
                   let uu____4887 = deps_try_find deps f  in
                   FStar_All.pipe_right uu____4887 FStar_Option.get  in
                 ...
                 | (f_deps,uu____4911) ->
                     ...
                     ((let uu____4931 = is_interface f  in
                       if uu____4931
                       then
                         let uu____4932 =
                           let uu____4933 =
                             FStar_Options.prepend_cache_dir norm_f  in
                           norm_path uu____4933  in
                         FStar_Util.print3
                           "%s.source: %s \\\n\t%s\n\ttouch $@\n\n"
                           uu____4932 norm_f files3
                       else ());
                      (let uu____4936 = cache_file f  in
                       FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____4936
                         norm_f files3);
                      (let already_there =
                         let uu____4940 =
                           let uu____4951 =
                             let uu____4952 = output_file ".krml" f  in
                             norm_path uu____4952  in
                           FStar_Util.smap_try_find transitive_krml
                             uu____4951
                            in
                         match uu____4940 with
                         | FStar_Pervasives_Native.Some
                             (uu____4963,already_there,uu____4965) ->
                             already_there
                         | FStar_Pervasives_Native.None  -> []  in
                       (let uu____4987 =
                          let uu____4988 = output_file ".krml" f  in
                          norm_path uu____4988  in
                        let uu____4989 =
                          let uu____4998 =
                            let uu____4999 = output_file ".exe" f  in
                            norm_path uu____4999  in
                          let uu____5000 =
                            let uu____5003 =
                              let uu____5006 =
                                let uu____5009 =
                                ...
                                FStar_List.map
                                  (fun x  ->
                                     let uu____5017 = output_file ".krml" x
                                        in
                                     norm_path uu____5017) uu____5009
                                 in
                              FStar_List.append already_there uu____5006  in
                            FStar_List.unique uu____5003  in
                          (uu____4998, uu____5000, false)  in
                        FStar_Util.smap_add transitive_krml uu____4987
                          uu____4989);
                       (let uu____5028 = is_implementation f  in
                        if uu____5028
                        then
                          ((let uu____5030 = output_ml_file f  in
                            let uu____5031 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____5030
                              uu____5031);
                           (let cmx_files =
                              let fst_files =
