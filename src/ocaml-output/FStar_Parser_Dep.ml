        ...
        fun original_map  ->
          fun filename  ->
            ...
            let auto_open =
              ...
              then []
                ...
                (let l =
                 ...
                 let uu____556 =
                   let uu____557 = FStar_Util.basename filename in
                   let uu____558 = FStar_Options.pervasives_basename () in
            ...
            (let num_of_toplevelmods =
             ...
             let rec collect_fragment uu___85_633 =
               match uu___85_633 with
               | FStar_Util.Inl file -> collect_file file
               | FStar_Util.Inr decls -> collect_decls decls
             and collect_file uu___86_646 =
               match uu___86_646 with
             ...
             and collect_module uu___87_652 =
               match uu___87_652 with
                   ...
                   (check_module_declaration_against_filename lid filename;
                    (match verify_mode with
                     ...
                     | VerifyFigureItOut  ->
                         ...
                         then
                           let uu____660 = string_of_lid lid true in
                           FStar_Options.add_verify_module uu____660
                         else ()
                     | VerifyUserList  ->
                         FStar_List.iter
                           (fun uu____665  ->
                              ...
                              | (m,r) ->
                                  let uu____673 =
                                    let uu____674 =
                                      let uu____675 = string_of_lid lid true in
                                      FStar_String.lowercase uu____675 in
                                    (FStar_String.lowercase m) = uu____674 in
                                  if uu____673
                                  then FStar_ST.write r true
                                  else ()) verify_flags);
                    collect_decls decls)
               | FStar_Parser_AST.Interface (lid,decls,uu____681) ->
                   (check_module_declaration_against_filename lid filename;
                    (match verify_mode with
                     | VerifyAll  ->
                         let uu____686 = string_of_lid lid true in
                         FStar_Options.add_verify_module uu____686
                     | VerifyFigureItOut  ->
                         if is_user_provided_filename
                         then
                           let uu____687 = string_of_lid lid true in
                           FStar_Options.add_verify_module uu____687
                         else ()
                     | VerifyUserList  ->
                         FStar_List.iter
                           (fun uu____692  ->
                              ...
                              | (m,r) ->
                                  let uu____700 =
                                    let uu____701 =
                                      ...
                                      FStar_String.lowercase uu____702 in
                                    (FStar_String.lowercase m) = uu____701 in
                                  if uu____700
                                  then FStar_ST.write r true
                                  else ()) verify_flags);
                    collect_decls decls)
             and collect_decls decls =
               FStar_List.iter
                 ...
                 decls
             and collect_decl uu___88_710 =
               match uu___88_710 with
               | FStar_Parser_AST.Include lid -> record_open false lid
               | FStar_Parser_AST.Open lid -> record_open false lid
               | FStar_Parser_AST.ModuleAbbrev (ident,lid) ->
                   ((let uu____716 = lowercase_join_longident lid true in
                     add_dep uu____716);
                    record_module_alias ident lid)
               | FStar_Parser_AST.TopLevelLet (uu____717,patterms) ->
                   FStar_List.iter
                     (fun uu____727  ->
               ...
               | FStar_Parser_AST.Assume (uu____734,t) -> collect_term t
               | FStar_Parser_AST.SubEffect
                   { FStar_Parser_AST.msource = uu____736;
                     FStar_Parser_AST.mdest = uu____737;
                     FStar_Parser_AST.lift_op =
                       FStar_Parser_AST.NonReifiableLift t;_}
                   -> collect_term t
               | FStar_Parser_AST.SubEffect
                   { FStar_Parser_AST.msource = uu____739;
                     FStar_Parser_AST.mdest = uu____740;
                     FStar_Parser_AST.lift_op = FStar_Parser_AST.LiftForFree
                       t;_}
                   -> collect_term t
               | FStar_Parser_AST.Val (uu____742,t) -> collect_term t
               | FStar_Parser_AST.SubEffect
                   { FStar_Parser_AST.msource = uu____744;
                     FStar_Parser_AST.mdest = uu____745;
                     FStar_Parser_AST.lift_op =
                       FStar_Parser_AST.ReifiableLift (t0,t1);_}
                   -> (collect_term t0; collect_term t1)
               | FStar_Parser_AST.Tycon (uu____749,ts) ->
                   let ts1 =
                     FStar_List.map
                       (fun uu____764  ->
                          match uu____764 with | (x,doc1) -> x) ts in
                   FStar_List.iter collect_tycon ts1
               | FStar_Parser_AST.Exception (uu____772,t) ->
                   FStar_Util.iter_opt t collect_term
               | FStar_Parser_AST.NewEffect ed -> collect_effect_decl ed
               | FStar_Parser_AST.Fsdoc uu____777 -> ()
               | FStar_Parser_AST.Pragma uu____778 -> ()
               | FStar_Parser_AST.TopLevelModule lid ->
                   (FStar_Util.incr num_of_toplevelmods;
                    (let uu____784 =
                       let uu____785 = FStar_ST.read num_of_toplevelmods in
                       uu____785 > (Prims.parse_int "1") in
                     if uu____784
                     then
                       let uu____788 =
                         let uu____789 =
                           let uu____790 = string_of_lid lid true in
                           FStar_Util.format1
                             "Automatic dependency analysis demands one module per file (module %s not supported)"
                             uu____790 in
                         FStar_Errors.Err uu____789 in
                       raise uu____788
                     else ()))
             and collect_tycon uu___89_792 =
               match uu___89_792 with
               | FStar_Parser_AST.TyconAbstract (uu____793,binders,k) ->
                   (collect_binders binders;
                    FStar_Util.iter_opt k collect_term)
               | FStar_Parser_AST.TyconAbbrev (uu____801,binders,k,t) ->
                   (collect_binders binders;
                    FStar_Util.iter_opt k collect_term;
                    collect_term t)
                   ...
                   (collect_binders binders;
                    ...
                    FStar_List.iter
                      (fun uu____835  ->
                         ...
                         | (uu____840,t,uu____842) -> collect_term t)
                      identterms)
               | FStar_Parser_AST.TyconVariant
                   (uu____845,binders,k,identterms) ->
                   (collect_binders binders;
                    FStar_Util.iter_opt k collect_term;
                    FStar_List.iter
                      (fun uu____875  ->
                         match uu____875 with
                         | (uu____882,t,uu____884,uu____885) ->
                             FStar_Util.iter_opt t collect_term) identterms)
             and collect_effect_decl uu___90_890 =
               match uu___90_890 with
               | FStar_Parser_AST.DefineEffect (uu____891,binders,t,decls) ->
                   (collect_binders binders;
                    collect_term t;
                    collect_decls decls)
               | FStar_Parser_AST.RedefineEffect (uu____901,binders,t) ->
                   (collect_binders binders; collect_term t)
             and collect_binders binders =
               FStar_List.iter collect_binder binders
             and collect_binder uu___91_909 =
               match uu___91_909 with
               | {
               ...
               | {
                   FStar_Parser_AST.b = FStar_Parser_AST.TAnnotated
                     (uu____915,t);
                   FStar_Parser_AST.brange = uu____917;
                   FStar_Parser_AST.blevel = uu____918;
                   FStar_Parser_AST.aqual = uu____919;_} -> collect_term t
               | { FStar_Parser_AST.b = FStar_Parser_AST.NoName t;
                   FStar_Parser_AST.brange = uu____921;
                   FStar_Parser_AST.blevel = uu____922;
                   FStar_Parser_AST.aqual = uu____923;_} -> collect_term t
               | uu____924 -> ()
             and collect_term t = collect_term' t.FStar_Parser_AST.tm
             and collect_constant uu___92_926 =
               match uu___92_926 with
               | FStar_Const.Const_int (uu____927,Some (signedness,width)) ->
                   let u =
                   ...
                   let uu____937 = FStar_Util.format2 "fstar.%sint%s" u w in
                   add_dep uu____937
               | uu____938 -> ()
             and collect_term' uu___93_939 =
                   ...
                   (if (FStar_Ident.text_of_id s) = "@"
                    then
                      (let uu____946 =
                         ...
                         FStar_Parser_AST.Name uu____947 in
                       collect_term' uu____946)
                    else ();
                    FStar_List.iter collect_term ts)
               | FStar_Parser_AST.Tvar uu____949 -> ()
               | FStar_Parser_AST.Uvar uu____950 -> ()
               | FStar_Parser_AST.Var lid -> record_lid lid
               | FStar_Parser_AST.Projector (lid,uu____953) -> record_lid lid
                   ...
                   (if (FStar_List.length termimps) = (Prims.parse_int "1")
                    ...
                    FStar_List.iter
                      (fun uu____974  ->
                         ...
                         | (t,uu____978) -> collect_term t) termimps)
               | FStar_Parser_AST.Abs (pats,t) ->
                   (collect_patterns pats; collect_term t)
               | FStar_Parser_AST.App (t1,t2,uu____986) ->
                   (collect_term t1; collect_term t2)
               | FStar_Parser_AST.Let (uu____988,patterms,t) ->
                   (FStar_List.iter
                      (fun uu____1000  ->
               ...
               | FStar_Parser_AST.Bind (uu____1009,t1,t2) ->
                   (collect_term t1; collect_term t2)
                   ...
                   (FStar_Util.iter_opt t collect_term;
                    FStar_List.iter
                      (fun uu____1070  ->
                         ...
                         | (uu____1073,t1) -> collect_term t1) idterms)
               | FStar_Parser_AST.Project (t,uu____1076) -> collect_term t
               ...
               | FStar_Parser_AST.NamedTyp (uu____1114,t) -> collect_term t
               | FStar_Parser_AST.Paren t -> collect_term t
               | FStar_Parser_AST.Assign (uu____1117,t) -> collect_term t
               | FStar_Parser_AST.Requires (t,uu____1120) -> collect_term t
               | FStar_Parser_AST.Ensures (t,uu____1124) -> collect_term t
               | FStar_Parser_AST.Labeled (t,uu____1128,uu____1129) ->
                   collect_term t
               | FStar_Parser_AST.Attributes cattributes ->
                   FStar_List.iter collect_term cattributes
             and collect_patterns ps = FStar_List.iter collect_pattern ps
             and collect_pattern p = collect_pattern' p.FStar_Parser_AST.pat
             and collect_pattern' uu___94_1135 =
               match uu___94_1135 with
               | FStar_Parser_AST.PatWild  -> ()
               | FStar_Parser_AST.PatOp uu____1136 -> ()
               | FStar_Parser_AST.PatConst uu____1137 -> ()
               | FStar_Parser_AST.PatApp (p,ps) ->
                   (collect_pattern p; collect_patterns ps)
               | FStar_Parser_AST.PatVar uu____1143 -> ()
               | FStar_Parser_AST.PatName uu____1147 -> ()
               | FStar_Parser_AST.PatTvar uu____1148 -> ()
               | FStar_Parser_AST.PatList ps -> collect_patterns ps
               | FStar_Parser_AST.PatOr ps -> collect_patterns ps
               | FStar_Parser_AST.PatTuple (ps,uu____1157) ->
                   collect_patterns ps
               | FStar_Parser_AST.PatRecord lidpats ->
                   FStar_List.iter
                     (fun uu____1166  ->
                        match uu____1166 with
                        | (uu____1169,p) -> collect_pattern p) lidpats
               | FStar_Parser_AST.PatAscribed (p,t) ->
                   (collect_pattern p; collect_term t)
             and collect_branches bs = FStar_List.iter collect_branch bs
             and collect_branch uu____1184 =
               match uu____1184 with
               | (pat,t1,t2) ->
                   (collect_pattern pat;
                    FStar_Util.iter_opt t1 collect_term;
                    collect_term t2) in
             let uu____1196 = FStar_Parser_Driver.parse_file filename in
             match uu____1196 with
             | (ast,uu____1204) -> (collect_file ast; FStar_ST.read deps))
  ...
  (let uu____1233 =
     let uu____1234 =
       let uu____1235 =
         let uu____1236 =
           let uu____1238 =
             let uu____1240 = FStar_Util.smap_keys graph in
             FStar_List.unique uu____1240 in
           FStar_List.collect
             (fun k  ->
                let deps =
                  let uu____1248 =
                    let uu____1252 = FStar_Util.smap_try_find graph k in
             ...
             uu____1238 in
         FStar_String.concat "\n" uu____1236 in
       Prims.strcat uu____1235 "\n}\n" in
     Prims.strcat "digraph {\n" uu____1234 in
   FStar_Util.write_file "dep.graph" uu____1233)
let collect:
  verify_mode ->
    Prims.string Prims.list ->
  ...
  fun verify_mode  ->
    fun filenames  ->
      ...
      let verify_flags =
        let uu____1314 = FStar_Options.verify_module () in
        FStar_List.map
          (fun f  ->
             let uu____1320 = FStar_Util.mk_ref false in (f, uu____1320))
          uu____1314 in
      ...
      let file_names_of_key k =
        let uu____1333 =
          let uu____1338 = FStar_Util.smap_try_find m k in
        ...
        | (intf,impl) ->
            (match (intf, impl) with
             ...
             | (None ,Some i) -> i
             | (Some i,None ) -> i
             | (Some i,uu____1369) when partial_discovery -> i
      ...
      let rec discover_one is_user_provided_filename interface_only key =
        let uu____1395 =
          let uu____1396 = FStar_Util.smap_try_find graph key in
          uu____1396 = None in
        if uu____1395
        then
          let uu____1411 =
            let uu____1416 = FStar_Util.smap_try_find m key in
            FStar_Util.must uu____1416 in
          ...
          | (intf,impl) ->
              ...
              let impl_deps =
                ...
                | (Some impl1,Some uu____1446) when interface_only -> []
                | (Some impl1,uu____1450) ->
        ...
        let interface_only =
          (is_interface f) &&
            (let uu____1472 =
               FStar_List.existsML
                 (fun f1  ->
                    (let uu____1474 = lowercase_module_name f1 in
                     uu____1474 = m1) && (is_implementation f1)) filenames in
      ...
      (let immediate_graph = FStar_Util.smap_copy graph in
       ...
       let rec discover cycle key =
         let uu____1499 =
           let uu____1503 = FStar_Util.smap_try_find graph key in
           FStar_Util.must uu____1503 in
                   ...
                   (let all_deps =
                      let uu____1536 =
                        let uu____1538 =
                          FStar_List.map
                            (fun dep1  ->
                               let uu____1543 = discover (key :: cycle) dep1 in
                               dep1 :: uu____1543) direct_deps in
                        FStar_List.flatten uu____1538 in
                      FStar_List.unique uu____1536 in
                    FStar_Util.smap_add graph key (all_deps, Black);
                    (let uu____1551 =
                       let uu____1553 = FStar_ST.read topologically_sorted in
                       key :: uu____1553 in
                     FStar_ST.write topologically_sorted uu____1551);
                    all_deps))) in
       let discover1 = discover [] in
       let must_find k =
         let uu____1570 =
           let uu____1575 = FStar_Util.smap_try_find m k in
           FStar_Util.must uu____1575 in
         ...
         | (Some intf,Some impl) when
             (Prims.op_Negation partial_discovery) &&
               (let uu____1594 =
                  FStar_List.existsML
                    (fun f  ->
                       let uu____1596 = lowercase_module_name f in
                       uu____1596 = k) filenames in
                Prims.op_Negation uu____1594)
             -> [intf; impl]
         | (Some intf,Some impl) when
             FStar_List.existsML
               (fun f  ->
                  (is_implementation f) &&
                    (let uu____1602 = lowercase_module_name f in
                     uu____1602 = k)) filenames
             -> [intf; impl]
         | (Some intf,uu____1604) -> [intf]
         | (None ,Some impl) -> [impl]
         | (None ,None ) -> [] in
       let must_find_r f =
         let uu____1618 = must_find f in FStar_List.rev uu____1618 in
       let by_target =
         let uu____1625 =
           let uu____1627 = FStar_Util.smap_keys graph in
           ...
           (fun k  ->
              ...
              FStar_List.map
                (fun f  ->
                   ...
                   let suffix =
                     let uu____1653 =
                       let uu____1658 = FStar_Util.smap_try_find m k1 in
                       FStar_Util.must uu____1658 in
                     match uu____1653 with
                     | (Some intf,uu____1674) when should_append_fsti ->
                         [intf]
                     | uu____1678 -> [] in
                   let deps =
                     let uu____1685 = discover1 k1 in
                     FStar_List.rev uu____1685 in
                   let deps_as_filenames =
                     let uu____1689 = FStar_List.collect must_find deps in
                     FStar_List.append uu____1689 suffix in
                   (f, deps_as_filenames)) as_list) uu____1625 in
       let topologically_sorted1 =
         let uu____1694 = FStar_ST.read topologically_sorted in
         FStar_List.collect must_find_r uu____1694 in
       FStar_List.iter
         (fun uu____1703  ->
            match uu____1703 with
            | (m1,r) ->
                let uu____1711 =
                  (let uu____1712 = FStar_ST.read r in
                   Prims.op_Negation uu____1712) &&
                    (let uu____1715 = FStar_Options.interactive () in
                ...
                then
                  let maybe_fst =
                    ...
                    then
                      ...
                      FStar_Util.format1 " Did you mean %s ?" uu____1734
                    else "" in
                  let uu____1742 =
                    let uu____1743 =
                      FStar_Util.format3
                        ...
                        m1 m1 maybe_fst in
                    FStar_Errors.Err uu____1743 in
                  raise uu____1742
                else ()) verify_flags;
...
let print_make:
  ...
  fun deps  ->
    FStar_List.iter
      ...
      deps
let print uu____1798 =
  match uu____1798 with
  | (make_deps,uu____1811,graph) ->
      let uu____1829 = FStar_Options.dep () in
