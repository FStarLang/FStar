...
let rec collect_fragment = (fun uu___114_594 -> (match (uu___114_594) with
...
and collect_file = (fun uu___115_607 -> (match (uu___115_607) with
| (modul)::[] -> begin
...
and collect_module = (fun uu___116_613 -> (match (uu___116_613) with
...
and collect_decl = (fun uu___117_647 -> (match (uu___117_647) with
...
and collect_tycon = (fun uu___118_724 -> (match (uu___118_724) with
...
and collect_effect_decl = (fun uu___119_822 -> (match (uu___119_822) with
...
and collect_binder = (fun uu___120_841 -> (match (uu___120_841) with
...
and collect_constant = (fun uu___121_856 -> (match (uu___121_856) with
...
and collect_term' = (fun uu___122_869 -> (match (uu___122_869) with
...
((FStar_Util.iter_opt t collect_term);
(FStar_List.iter (fun uu____988 -> (match (uu____988) with
| (uu____991, t1) -> begin
(collect_term t1)
end)) idterms);
)
end
| FStar_Parser_AST.Project (t, uu____994) -> begin
(collect_term t)
end
...
| FStar_Parser_AST.NamedTyp (uu____1023, t) -> begin
(collect_term t)
end
...
and collect_pattern' = (fun uu___123_1039 -> (match (uu___123_1039) with
...
(FStar_List.iter (fun uu____1062 -> (match (uu____1062) with
| (uu____1065, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun uu____1080 -> (match (uu____1080) with
| (pat, t1, t2) -> begin
((collect_pattern pat);
...
let uu____1092 = (FStar_Parser_Driver.parse_file filename)
in (match (uu____1092) with
| (ast, uu____1100) -> begin
((collect_file ast);
(FStar_ST.read deps);
)
end))));
...
let uu____1136 = (FStar_Util.smap_keys graph)
in (FStar_List.unique uu____1136))
...
let uu____1148 = (FStar_Util.smap_try_find graph k)
in (FStar_Util.must uu____1148))
in (Prims.fst uu____1144))
in (

let r = (fun s -> (FStar_Util.replace_char s '.' '_'))
in (FStar_List.map (fun dep1 -> (FStar_Util.format2 "  %s -> %s" (r k) (r dep1))) deps)))) uu____1134))
in (FStar_String.concat "\n" uu____1132))
in (Prims.strcat uu____1131 "\n}\n"))
in (Prims.strcat "digraph {\n" uu____1130))
in (FStar_Util.write_file "dep.graph" uu____1129));
))
...
let uu____1210 = (FStar_Options.verify_module ())
in (FStar_List.map (fun f -> (

let uu____1216 = (FStar_Util.mk_ref false)
in ((f), (uu____1216)))) uu____1210))
in (

let partial_discovery = (

let uu____1223 = ((FStar_Options.verify_all ()) || (FStar_Options.extract_all ()))
in (not (uu____1223)))
in (
...
let uu____1234 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____1234))
...
| (Some (i), uu____1264) when partial_discovery -> begin
i
end
| (Some (i), Some (j)) -> begin
...
let uu____1291 = (FStar_Util.smap_try_find graph key)
in (uu____1291 = None))
...
let uu____1311 = (FStar_Util.smap_try_find m key)
in (FStar_Util.must uu____1311))
...
| (Some (impl1), Some (uu____1341)) when interface_only -> begin
[]
end
| (Some (impl1), uu____1345) -> begin
(collect_one1 is_user_provided_filename m impl1)
end
| (None, uu____1349) -> begin
[]
end)
in (
...
let uu____1367 = (FStar_List.existsML (fun f1 -> ((

let uu____1369 = (lowercase_module_name f1)
in (uu____1369 = m1)) && (is_implementation f1))) filenames)
in (not (uu____1367))))
...
let uu____1398 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must uu____1398))
...
let uu____1433 = (FStar_List.map (fun dep1 -> (

let uu____1438 = (discover ((key)::cycle) dep1)
in (dep1)::uu____1438)) direct_deps)
in (FStar_List.flatten uu____1433))
in (FStar_List.unique uu____1431))
in ((FStar_Util.smap_add graph key ((all_deps), (Black)));
(

let uu____1446 = (

let uu____1448 = (FStar_ST.read topologically_sorted)
in (key)::uu____1448)
in (FStar_ST.write topologically_sorted uu____1446));
all_deps;
));
)
end)
...
let uu____1470 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____1470))
in (match (uu____1465) with
| (Some (intf), Some (impl)) when ((not (partial_discovery)) && (

let uu____1489 = (FStar_List.existsML (fun f -> (

let uu____1491 = (lowercase_module_name f)
in (uu____1491 = k))) filenames)
in (not (uu____1489)))) -> begin
(intf)::(impl)::[]
end
| (Some (intf), Some (impl)) when (FStar_List.existsML (fun f -> ((is_implementation f) && (

let uu____1497 = (lowercase_module_name f)
in (uu____1497 = k)))) filenames) -> begin
(intf)::(impl)::[]
end
| (Some (intf), uu____1499) -> begin
(intf)::[]
end
...
let uu____1513 = (must_find f)
in (FStar_List.rev uu____1513)))
in (
...
let uu____1522 = (FStar_Util.smap_keys graph)
in (FStar_List.sortWith (fun x y -> (FStar_String.compare x y)) uu____1522))
in (FStar_List.collect (fun k -> (
...
let uu____1550 = (discover1 k1)
in (FStar_List.rev uu____1550))
in (

let deps_as_filenames = (

let uu____1554 = (FStar_List.collect must_find deps)
in (FStar_List.append uu____1554 suffix))
in ((f), (deps_as_filenames)))))))) as_list)))) uu____1520))
in (

let topologically_sorted1 = (

let uu____1559 = (FStar_ST.read topologically_sorted)
in (FStar_List.collect must_find_r uu____1559))
in ((FStar_List.iter (fun uu____1568 -> (match (uu____1568) with
| (m1, r) -> begin
(

let uu____1576 = ((

let uu____1577 = (FStar_ST.read r)
in (not (uu____1577))) && (

let uu____1580 = (FStar_Options.interactive ())
in (not (uu____1580))))
...
let uu____1584 = ((k > (Prims.parse_int "4")) && (

let uu____1588 = (FStar_String.substring m1 (k - (Prims.parse_int "4")) (Prims.parse_int "4"))
in (uu____1588 = ".fst")))
in (match (uu____1584) with
| true -> begin
(

let uu____1592 = (FStar_String.substring m1 (Prims.parse_int "0") (k - (Prims.parse_int "4")))
in (FStar_Util.format1 " Did you mean %s ?" uu____1592))
end
| uu____1596 -> begin
""
end)))
...
let uu____1598 = (FStar_Util.format3 "You passed --verify_module %s but I found no file that contains [module %s] in the dependency graph.%s\n" m1 m1 maybe_fst)
in FStar_Errors.Err (uu____1598))
in (Prims.raise uu____1597)))
end
| uu____1599 -> begin
()
end))
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun uu____1623 -> (match (uu____1623) with
| (f, deps1) -> begin
(
...
let print = (fun uu____1653 -> (match (uu____1653) with
| (make_deps, uu____1666, graph) -> begin
(

let uu____1684 = (FStar_Options.dep ())
