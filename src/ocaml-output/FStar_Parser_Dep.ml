...
and collect_term' = (fun uu___89_2901 -> (match (uu___89_2901) with
...
let uu____2911 = (FStar_Ident.lid_of_path (FStar_Ident.path_of_text "FStar.List.Tot.Base.append") FStar_Range.dummyRange)
in FStar_Parser_AST.Name (uu____2911))
in (collect_term' uu____2910))
end
| uu____2912 -> begin
()
end);
...
| FStar_Parser_AST.Tvar (uu____2913) -> begin
()
end
| FStar_Parser_AST.Uvar (uu____2914) -> begin
()
end
| FStar_Parser_AST.Var (lid) -> begin
(record_lid lid)
end
| FStar_Parser_AST.Projector (lid, uu____2917) -> begin
(record_lid lid)
end
...
(FStar_List.iter (fun uu____2947 -> (match (uu____2947) with
| (t, uu____2953) -> begin
(collect_term t)
end)) termimps);
)
...
| FStar_Parser_AST.App (t1, t2, uu____2963) -> begin
((collect_term t1);
(collect_term t2);
)
end
| FStar_Parser_AST.Let (uu____2965, patterms, t) -> begin
((FStar_List.iter (fun uu____2989 -> (match (uu____2989) with
| (pat, t1) -> begin
((collect_pattern pat);
(collect_term t1);
)
...
| FStar_Parser_AST.Bind (uu____3000, t1, t2) -> begin
((collect_term t1);
(collect_term t2);
)
end
...
((FStar_Util.iter_opt t collect_term);
(FStar_List.iter (fun uu____3096 -> (match (uu____3096) with
| (uu____3101, t1) -> begin
(collect_term t1)
end)) idterms);
)
end
| FStar_Parser_AST.Project (t, uu____3104) -> begin
(collect_term t)
end
...
| FStar_Parser_AST.NamedTyp (uu____3160, t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Paren (t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Assign (uu____3163, t) -> begin
(collect_term t)
end
| FStar_Parser_AST.Requires (t, uu____3166) -> begin
(collect_term t)
end
| FStar_Parser_AST.Ensures (t, uu____3172) -> begin
(collect_term t)
end
| FStar_Parser_AST.Labeled (t, uu____3178, uu____3179) -> begin
(collect_term t)
end
...
and collect_pattern' = (fun uu___90_3187 -> (match (uu___90_3187) with
| FStar_Parser_AST.PatWild -> begin
()
end
| FStar_Parser_AST.PatOp (uu____3188) -> begin
()
end
| FStar_Parser_AST.PatConst (uu____3189) -> begin
()
end
...
| FStar_Parser_AST.PatVar (uu____3197) -> begin
()
end
| FStar_Parser_AST.PatName (uu____3204) -> begin
()
end
| FStar_Parser_AST.PatTvar (uu____3205) -> begin
()
end
...
| FStar_Parser_AST.PatTuple (ps, uu____3219) -> begin
(collect_patterns ps)
end
| FStar_Parser_AST.PatRecord (lidpats) -> begin
(FStar_List.iter (fun uu____3238 -> (match (uu____3238) with
| (uu____3243, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun uu____3267 -> (match (uu____3267) with
| (pat, t1, t2) -> begin
((collect_pattern pat);
...
let uu____3285 = (FStar_Parser_Driver.parse_file filename)
in (match (uu____3285) with
| (ast, uu____3299) -> begin
((collect_module ast);
...
let print_graph : 'Auu____3381 . (Prims.string Prims.list * 'Auu____3381) FStar_Util.smap  ->  Prims.unit = (fun graph -> ((FStar_Util.print_endline "A DOT-format graph has been dumped in the current directory as dep.graph");
...
let uu____3414 = (FStar_Util.smap_keys graph)
in (FStar_List.unique uu____3414))
...
let uu____3437 = (FStar_Util.smap_try_find graph k)
in (FStar_Util.must uu____3437))
in (FStar_Pervasives_Native.fst uu____3430))
in (

let r = (fun s -> (FStar_Util.replace_char s 46 (*.*) 95 (*_*)))
in (FStar_List.map (fun dep1 -> (FStar_Util.format2 "  %s -> %s" (r k) (r dep1))) deps)))) uu____3411))
in (FStar_String.concat "\n" uu____3408))
in (Prims.strcat uu____3407 "\n}\n"))
in (Prims.strcat "digraph {\n" uu____3406))
in (FStar_Util.write_file "dep.graph" uu____3405));
))
...
let uu____3526 = (FStar_Options.verify_module ())
in (FStar_List.map (fun f -> (

let uu____3538 = (FStar_Util.mk_ref false)
in ((f), (uu____3538)))) uu____3526))
in (

let partial_discovery = (

let uu____3558 = ((FStar_Options.verify_all ()) || (FStar_Options.extract_all ()))
in (not (uu____3558)))
in (
...
let uu____3573 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____3573))
...
| (FStar_Pervasives_Native.Some (i), uu____3629) when partial_discovery -> begin
i
end
...
let uu____3662 = (FStar_Util.smap_try_find graph key)
in (Prims.op_Equality uu____3662 FStar_Pervasives_Native.None))
...
let uu____3700 = (FStar_Util.smap_try_find m key)
in (FStar_Util.must uu____3700))
...
| (FStar_Pervasives_Native.Some (impl1), FStar_Pervasives_Native.Some (uu____3753)) when interface_only -> begin
[]
end
| (FStar_Pervasives_Native.Some (impl1), uu____3759) -> begin
(collect_one1 is_user_provided_filename m impl1)
end
| (FStar_Pervasives_Native.None, uu____3766) -> begin
[]
end)
in (
...
let uu____3793 = (FStar_List.existsML (fun f1 -> ((

let uu____3798 = (lowercase_module_name f1)
in (Prims.op_Equality uu____3798 m1)) && (is_implementation f1))) filenames)
in (not (uu____3793))))
...
let uu____3842 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must uu____3842))
...
let uu____3901 = (FStar_List.map (fun dep1 -> (

let uu____3911 = (discover ((key)::cycle) dep1)
in (dep1)::uu____3911)) direct_deps)
in (FStar_List.flatten uu____3901))
in (FStar_List.unique uu____3898))
in ((FStar_Util.smap_add graph key ((all_deps), (Black)));
(

let uu____3924 = (

let uu____3927 = (FStar_ST.op_Bang topologically_sorted)
in (key)::uu____3927)
in (FStar_ST.op_Colon_Equals topologically_sorted uu____3924));
all_deps;
));
)
end)
...
let uu____4078 = (FStar_Util.smap_try_find m k)
in (FStar_Util.must uu____4078))
in (match (uu____4069) with
| (FStar_Pervasives_Native.Some (intf), FStar_Pervasives_Native.Some (impl)) when ((not (partial_discovery)) && (

let uu____4114 = (FStar_List.existsML (fun f -> (

let uu____4118 = (lowercase_module_name f)
in (Prims.op_Equality uu____4118 k))) filenames)
in (not (uu____4114)))) -> begin
(intf)::(impl)::[]
end
| (FStar_Pervasives_Native.Some (intf), FStar_Pervasives_Native.Some (impl)) when (FStar_List.existsML (fun f -> ((is_implementation f) && (

let uu____4128 = (lowercase_module_name f)
in (Prims.op_Equality uu____4128 k)))) filenames) -> begin
(intf)::(impl)::[]
end
| (FStar_Pervasives_Native.Some (intf), uu____4130) -> begin
(intf)::[]
end
...
let uu____4152 = (must_find f)
in (FStar_List.rev uu____4152)))
in (
...
let uu____4167 = (FStar_Util.smap_keys graph)
in (FStar_List.sortWith (fun x y -> (FStar_String.compare x y)) uu____4167))
in (FStar_List.collect (fun k -> (
...
let uu____4221 = (FStar_Util.smap_try_find m k1)
in (FStar_Util.must uu____4221))
in (match (uu____4212) with
| (FStar_Pervasives_Native.Some (intf), uu____4251) when should_append_fsti -> begin
(intf)::[]
end
| uu____4258 -> begin
[]
...
let uu____4270 = (discover1 k1)
in (FStar_List.rev uu____4270))
in (

let deps_as_filenames = (

let uu____4276 = (FStar_List.collect must_find deps)
in (FStar_List.append uu____4276 suffix))
in ((f), (deps_as_filenames)))))))) as_list)))) uu____4164))
in (

let topologically_sorted1 = (

let uu____4284 = (FStar_ST.op_Bang topologically_sorted)
in (FStar_List.collect must_find_r uu____4284))
in ((FStar_List.iter (fun uu____4456 -> (match (uu____4456) with
| (m1, r) -> begin
(

let uu____4745 = ((

let uu____4748 = (FStar_ST.op_Bang r)
in (not (uu____4748))) && (

let uu____4892 = (FStar_Options.interactive ())
in (not (uu____4892))))
...
let uu____4895 = ((k > (Prims.parse_int "4")) && (

let uu____4903 = (FStar_String.substring m1 (k - (Prims.parse_int "4")) (Prims.parse_int "4"))
in (Prims.op_Equality uu____4903 ".fst")))
in (match (uu____4895) with
| true -> begin
(

let uu____4910 = (FStar_String.substring m1 (Prims.parse_int "0") (k - (Prims.parse_int "4")))
in (FStar_Util.format1 " Did you mean %s ?" uu____4910))
end
| uu____4917 -> begin
""
end)))
...
let uu____4919 = (FStar_Util.format3 "You passed --verify_module %s but I found no file that contains [module %s] in the dependency graph.%s\n" m1 m1 maybe_fst)
in FStar_Errors.Err (uu____4919))
in (FStar_Exn.raise uu____4918)))
end
| uu____4920 -> begin
()
end))
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun uu____4969 -> (match (uu____4969) with
| (f, deps1) -> begin
(
...
let print : 'a 'b . ((Prims.string * Prims.string Prims.list) Prims.list * 'a * (Prims.string Prims.list * 'b) FStar_Util.smap)  ->  Prims.unit = (fun uu____5020 -> (match (uu____5020) with
| (make_deps, uu____5044, graph) -> begin
(

let uu____5078 = (FStar_Options.dep ())
...
| FStar_Pervasives_Native.Some (uu____5081) -> begin
