...
let (collect_one_cache :
  ...
  =
  let uu____3545 = FStar_Util.smap_create (Prims.parse_int "0")  in
  FStar_Util.mk_ref uu____3545 
let (set_collect_one_cache :
  (dependence Prims.list,dependence Prims.list)
    FStar_Pervasives_Native.tuple2 FStar_Util.smap -> Prims.unit)
  ...
  fun all_cmd_line_files  ->
    let all_cmd_line_files1 =
      FStar_All.pipe_right all_cmd_line_files
        (FStar_List.map
           (fun fn  ->
              let uu____3666 = FStar_Options.find_file fn  in
                  ...
                  let uu____3669 =
                    let uu____3674 =
                      FStar_Util.format1 "File %s could not be found\n" fn
                       in
                    (FStar_Errors.Fatal_ModuleOrFileNotFound, uu____3674)  in
                  FStar_Errors.raise_err uu____3669
    ...
    let rec discover_one file_name =
      let uu____3682 =
        let uu____3683 = deps_try_find dep_graph file_name  in
        uu____3683 = FStar_Pervasives_Native.None  in
      if uu____3682
      then
        let uu____3700 =
          let uu____3709 =
            let uu____3720 = FStar_ST.op_Bang collect_one_cache  in
            FStar_Util.smap_try_find uu____3720 file_name  in
        ...
        | (deps,mo_roots) ->
            ...
            ((let uu____3830 =
                let uu____3835 = FStar_List.unique deps1  in
                (uu____3835, White)  in
              deps_add_dep dep_graph file_name uu____3830);
             (let uu____3840 =
                FStar_List.map
                  (file_of_dep file_system_map all_cmd_line_files1)
                  (FStar_List.append deps1 mo_roots)
                 in
              FStar_List.iter discover_one uu____3840))
      else ()  in
    ...
    (let topological_dependences_of all_command_line_files =
       ...
       let rec aux cycle filename =
         let uu____3873 =
           let uu____3878 = deps_try_find dep_graph filename  in
           FStar_Util.must uu____3878  in
                  ...
                  ((let uu____3892 =
                      let uu____3897 =
                        FStar_Util.format1
                         ...
                         in
                      (FStar_Errors.Warning_RecursiveDependency, uu____3897)
                       in
                  ...
                  (deps_add_dep dep_graph filename (direct_deps, Gray);
                   (let uu____3903 =
                      dependences_of file_system_map dep_graph
                       ...
                       in
                    FStar_List.iter (fun k  -> aux (k :: cycle) k) uu____3903);
                   deps_add_dep dep_graph filename (direct_deps, Black);
                   (let uu____3909 =
                      let uu____3912 = FStar_ST.op_Bang topologically_sorted
                         in
                      filename :: uu____3912  in
                    FStar_ST.op_Colon_Equals topologically_sorted uu____3909)))
          in
     ...
     (let uu____4058 = topological_dependences_of all_cmd_line_files1  in
      (uu____4058, (Mk (dep_graph, file_system_map, all_cmd_line_files1)))))
...
let (hash_dependences :
  ...
  fun uu____4096  ->
    fun fn  ->
      ...
      | Mk (deps,file_system_map,all_cmd_line_files) ->
          let fn1 =
            let uu____4114 = FStar_Options.find_file fn  in
            match uu____4114 with
            | FStar_Pervasives_Native.Some fn1 -> fn1
            | uu____4118 -> fn  in
          let cache_file = cache_file_name fn1  in
          let digest_of_file1 fn2 =
            (let uu____4127 = FStar_Options.debug_any ()  in
             if uu____4127
             then
          ...
          let interface_hash =
            ...
            then
              let uu____4145 =
                let uu____4150 =
                  let uu____4151 =
                    let uu____4152 = interface_of file_system_map module_name
                       in
                    FStar_Option.get uu____4152  in
                  digest_of_file1 uu____4151  in
                ("interface", uu____4150)  in
              [uu____4145]
            else []  in
          let binary_deps =
            let uu____4171 =
              dependences_of file_system_map deps all_cmd_line_files fn1  in
            FStar_All.pipe_right uu____4171
              (FStar_List.filter
                 (fun fn2  ->
                    let uu____4181 =
                      (is_interface fn2) &&
                        (let uu____4183 = lowercase_module_name fn2  in
                         uu____4183 = module_name)
                       in
                    Prims.op_Negation uu____4181))
             in
          let binary_deps1 =
            FStar_List.sortWith
              (fun fn11  ->
                 fun fn2  ->
                   let uu____4193 = lowercase_module_name fn11  in
                   let uu____4194 = lowercase_module_name fn2  in
                   FStar_String.compare uu____4193 uu____4194) binary_deps
             in
          let rec hash_deps out uu___65_4217 =
            match uu___65_4217 with
            | [] ->
                FStar_Pervasives_Native.Some
                ...
                then
                  let uu____4261 =
                    let uu____4268 =
                      ...
                      let uu____4274 = digest_of_file1 cache_fn  in
                      (uu____4273, uu____4274)  in
                    uu____4268 :: out  in
                  hash_deps uu____4261 deps1
                else
      ...
      FStar_All.pipe_right dig
        (FStar_List.map
           (fun uu____4327  ->
              ...
              | (m,d) ->
                  let uu____4334 = FStar_Util.base64_encode d  in
                  FStar_Util.format2 "%s:%s" m uu____4334))
       in
    FStar_All.pipe_right uu____4308 (FStar_String.concat "\n")
    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        ...
        FStar_All.pipe_right keys
          (FStar_List.iter
             (fun f  ->
                let uu____4359 =
                  let uu____4364 = deps_try_find deps f  in
                  FStar_All.pipe_right uu____4364 FStar_Option.get  in
    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        let sort_output_files orig_output_file_map =
          ...
          let should_visit lc_module_name =
            (let uu____4435 =
               FStar_Util.smap_try_find remaining_output_files lc_module_name
                in
             FStar_Option.isSome uu____4435) ||
              (let uu____4439 =
                 FStar_Util.smap_try_find visited_other_modules
                   lc_module_name
                  in
               FStar_Option.isNone uu____4439)
             in
          ...
          let emit_output_file_opt ml_file_opt =
            ...
            | FStar_Pervasives_Native.Some ml_file ->
                let uu____4462 =
                  let uu____4465 = FStar_ST.op_Bang order  in ml_file ::
                    uu____4465
                   in
                FStar_ST.op_Colon_Equals order uu____4462
             in
          let rec aux uu___66_4563 =
            match uu___66_4563 with
            | [] -> ()
            | lc_module_name::modules_to_extract ->
                let visit_file file_opt =
                  ...
                  | FStar_Pervasives_Native.Some file_name ->
                      let uu____4579 = deps_try_find deps file_name  in
          ...
          (let uu____4620 = FStar_ST.op_Bang order  in
           FStar_List.rev uu____4620)
           in
        let keys = deps_keys deps  in
        let output_file ext fst_file =
          let ml_base_name =
            let uu____4679 =
              let uu____4680 =
                let uu____4683 = FStar_Util.basename fst_file  in
                check_and_strip_suffix uu____4683  in
              FStar_Option.get uu____4680  in
            FStar_Util.replace_chars uu____4679 46 "_"  in
          FStar_Options.prepend_output_dir (Prims.strcat ml_base_name ext)
           in
        let norm_path s = FStar_Util.replace_chars s 92 "/"  in
        let output_ml_file f =
          let uu____4694 = output_file ".ml" f  in norm_path uu____4694  in
        let output_krml_file f =
          let uu____4699 = output_file ".krml" f  in norm_path uu____4699  in
        let output_cmx_file f =
          let uu____4704 = output_file ".cmx" f  in norm_path uu____4704  in
        let cache_file f =
          let uu____4709 = cache_file_name f  in norm_path uu____4709  in
        ...
        (FStar_All.pipe_right keys
           (FStar_List.iter
              (fun f  ->
                 let uu____4752 =
                   let uu____4757 = deps_try_find deps f  in
                   FStar_All.pipe_right uu____4757 FStar_Option.get  in
                 ...
                 | (f_deps,uu____4779) ->
                     ...
                     ((let uu____4795 = is_interface f  in
                       if uu____4795
                       then
                         let uu____4796 =
                           let uu____4797 =
                             FStar_Options.prepend_cache_dir norm_f  in
                           norm_path uu____4797  in
                         FStar_Util.print3
                           "%s.source: %s \\\n\t%s\n\ttouch $@\n\n"
                           uu____4796 norm_f files3
                       else ());
                      (let uu____4800 = cache_file f  in
                       FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____4800
                         norm_f files3);
                      (let already_there =
                         let uu____4804 =
                           let uu____4815 =
                             let uu____4816 = output_file ".krml" f  in
                             norm_path uu____4816  in
                           FStar_Util.smap_try_find transitive_krml
                             uu____4815
                            in
                         match uu____4804 with
                         | FStar_Pervasives_Native.Some
                             (uu____4827,already_there,uu____4829) ->
                             already_there
                         | FStar_Pervasives_Native.None  -> []  in
                       (let uu____4851 =
                          let uu____4852 = output_file ".krml" f  in
                          norm_path uu____4852  in
                        let uu____4853 =
                          let uu____4862 =
                            let uu____4863 = output_file ".exe" f  in
                          ...
                          let uu____4864 =
                            let uu____4867 =
                              let uu____4870 =
                                ...
                                FStar_List.map
                                  (fun x  ->
                                     let uu____4881 = output_file ".krml" x
                                        in
                                     norm_path uu____4881) uu____4873
                                 in
                              FStar_List.append already_there uu____4870  in
                            FStar_List.unique uu____4867  in
                          (uu____4862, uu____4864, false)  in
                        FStar_Util.smap_add transitive_krml uu____4851
                          uu____4853);
                       (let uu____4892 = is_implementation f  in
                        if uu____4892
                        then
                          ((let uu____4894 = output_ml_file f  in
                            let uu____4895 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____4894
                              uu____4895);
                           (let cmx_files =
                              let fst_files =
                              ...
                              FStar_All.pipe_right extracted_fst_files
                               ...
                               in
                            (let uu____4926 =
                               let uu____4927 = lowercase_module_name f  in
                               FStar_Options.should_extract uu____4927  in
                             if uu____4926
                             then
                               let uu____4928 = output_cmx_file f  in
                               let uu____4929 = output_ml_file f  in
                               FStar_Util.print3 "%s: %s \\\n\t%s\n\n"
                                 uu____4928 uu____4929
                                 (FStar_String.concat "\\\n\t" cmx_files)
                             else ());
                            (let uu____4931 = output_krml_file f  in
                             let uu____4932 = cache_file f  in
                             FStar_Util.print2 "%s: %s\n\n" uu____4931
                               uu____4932)))
                        else
                          (let uu____4934 =
                             (let uu____4937 =
                                let uu____4938 = lowercase_module_name f  in
                                has_implementation file_system_map uu____4938
                                 in
                              Prims.op_Negation uu____4937) &&
                               (is_interface f)
                              in
                           if uu____4934
                           then
                             let uu____4939 = output_krml_file f  in
                             let uu____4940 = cache_file f  in
                             FStar_Util.print2 "%s: %s\n\n" uu____4939
                               uu____4940
                           else ()))))));
         (let all_fst_files =
            ...
            FStar_All.pipe_right all_fst_files
              (FStar_List.iter
                 (fun fst_file  ->
                    ...
                    let uu____4971 = FStar_Options.should_extract mname  in
                    if uu____4971
                    then
                      let uu____4972 = output_ml_file fst_file  in
                      FStar_Util.smap_add ml_file_map mname uu____4972
                    else ()));
          ...
          let all_krml_files =
            ...
            FStar_All.pipe_right keys
              (FStar_List.iter
                 (fun fst_file  ->
                    let mname = lowercase_module_name fst_file  in
                    let uu____4988 = output_krml_file fst_file  in
                    FStar_Util.smap_add krml_file_map mname uu____4988));
            sort_output_files krml_file_map  in
          let rec make_transitive f =
            let uu____4999 =
              let uu____5008 = FStar_Util.smap_try_find transitive_krml f  in
                   ...
                   (let deps2 =
                      let uu____5071 =
                        let uu____5074 =
                          FStar_List.map
                            (fun dep1  ->
                               let uu____5086 = make_transitive dep1  in
                               match uu____5086 with
                               | (uu____5095,deps2) -> dep1 :: deps2) deps1
                           in
                        FStar_List.flatten uu____5074  in
                      FStar_List.unique uu____5071  in
                    FStar_Util.smap_add transitive_krml f (exe, deps2, true);
                    (exe, deps2)))
             in
          (let uu____5115 = FStar_Util.smap_keys transitive_krml  in
           FStar_List.iter
             (fun f  ->
                let uu____5134 = make_transitive f  in
                match uu____5134 with
                | (exe,deps1) ->
                    let deps2 =
                      let uu____5148 = FStar_List.unique (f :: deps1)  in
                      FStar_String.concat " " uu____5148  in
                    let wasm =
                      let uu____5152 =
                        FStar_Util.substring exe (Prims.parse_int "0")
                          ((FStar_String.length exe) - (Prims.parse_int "4"))
                         in
                      Prims.strcat uu____5152 ".wasm"  in
                    (FStar_Util.print2 "%s: %s\n\n" exe deps2;
                     FStar_Util.print2 "%s: %s\n\n" wasm deps2)) uu____5115);
          (let uu____5155 =
             let uu____5156 =
               FStar_All.pipe_right all_fst_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____5156 (FStar_String.concat " \\\n\t")
              in
          ...
          (let uu____5166 =
             let uu____5167 =
               FStar_All.pipe_right all_ml_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____5167 (FStar_String.concat " \\\n\t")
              in
