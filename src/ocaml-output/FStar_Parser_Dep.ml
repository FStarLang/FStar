...
in (Prims.op_disEquality uu____565 k'))
...
in (FStar_List.existsML (fun d' -> (Prims.op_Equality d' d)) uu____681))
in (not (uu____680)))
...
in (FStar_Util.format1 "Unbound module reference %s" uu____1037))
in (FStar_Errors.warn (FStar_Ident.range_of_lid lid) uu____1036))
end
| uu____1038 -> begin
()
end))
...
in (Prims.op_Equality (FStar_String.lowercase m) uu____1389))
...
in (Prims.op_Equality (FStar_String.lowercase m) uu____1762))
...
in (Prims.op_Equality uu____2999 FStar_Pervasives_Native.None))
...
in (Prims.op_Equality uu____3135 m1)) && (is_implementation f1))) filenames)
in (not (uu____3130))))
...
in (Prims.op_Equality uu____3391 k))) filenames)
...
in (Prims.op_Equality uu____3401 k)))) filenames) -> begin
(intf)::(impl)::[]
end
...
in (Prims.op_Equality uu____3964 ".fst")))
