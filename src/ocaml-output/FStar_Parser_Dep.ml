...
let list_of_pair = (fun _70_35 -> (match (_70_35) with
| (intf, impl) -> begin
...
let _70_63 = (FStar_List.iter (fun d -> if (FStar_Util.file_exists d) then begin
(
...
let _70_66 = (FStar_List.iter (fun f -> (let _171_34 = (lowercase_module_name f)
...
let _70_78 = (let _171_44 = (let _171_43 = (FStar_Util.smap_keys original_map)
...
let _70_76 = (FStar_Util.smap_add working_map suffix filename)
...
let _70_125 = (let _171_104 = (lowercase_join_longident lid false)
in (try_key _171_104))
in ())))
in (
...
let _70_128 = (FStar_List.iter (record_open false) auto_open)
in (
...
let _70_157 = (FStar_Util.fprint FStar_Util.stderr "Warning: file %s does not respect the one module per file convention\n" ((filename)::[]))
...
let _70_168 = (check_module_declaration_against_filename lid filename)
in (

let _70_176 = (match (verify_mode) with
| VerifyAll -> begin
...
(FStar_List.iter (fun _70_175 -> (match (_70_175) with
| (m, r) -> begin
...
let _70_180 = (collect_decl x.FStar_Parser_AST.d)
...
let _70_190 = (let _171_134 = (lowercase_join_longident lid true)
in (add_dep _171_134))
in (record_module_alias ident lid))
end
| FStar_Parser_AST.TopLevelLet (_70_193, patterms) -> begin
(FStar_List.iter (fun _70_199 -> (match (_70_199) with
| (pat, t) -> begin
(

let _70_200 = (collect_pattern pat)
in (collect_term t))
end)) patterms)
end
| FStar_Parser_AST.KindAbbrev (_70_203, binders, t) -> begin
(

let _70_208 = (collect_term t)
...
| FStar_Parser_AST.SubEffect ({FStar_Parser_AST.msource = _70_241; FStar_Parser_AST.mdest = _70_239; FStar_Parser_AST.lift_op = FStar_Parser_AST.ReifiableLift (t0, t1)}) -> begin
(

let _70_244 = (collect_term t0)
in (collect_term t1))
end
| FStar_Parser_AST.Tycon (_70_247, ts) -> begin
(

let ts = (FStar_List.map (fun _70_253 -> (match (_70_253) with
| (x, doc) -> begin
x
end)) ts)
in (FStar_List.iter collect_tycon ts))
end
| FStar_Parser_AST.Exception (_70_256, t) -> begin
...
let _70_271 = (FStar_Util.incr num_of_toplevelmods)
...
| FStar_Parser_AST.TyconAbstract (_70_275, binders, k) -> begin
(

let _70_280 = (collect_binders binders)
in (FStar_Util.iter_opt k collect_term))
end
| FStar_Parser_AST.TyconAbbrev (_70_283, binders, k, t) -> begin
(
...
| FStar_Parser_AST.TyconRecord (_70_294, binders, k, identterms) -> begin
(

let _70_300 = (collect_binders binders)
in (

let _70_302 = (FStar_Util.iter_opt k collect_term)
in (FStar_List.iter (fun _70_309 -> (match (_70_309) with
| (_70_305, t, _70_308) -> begin
(collect_term t)
end)) identterms)))
end
| FStar_Parser_AST.TyconVariant (_70_311, binders, k, identterms) -> begin
(
...
| (_70_322, t, _70_325, _70_327) -> begin
(FStar_Util.iter_opt t collect_term)
end)) identterms)))
end))
and collect_effect_decl = (fun _70_7 -> (match (_70_7) with
| FStar_Parser_AST.DefineEffect (_70_331, binders, t, decls, actions) -> begin
(

let _70_338 = (collect_binders binders)
in (

let _70_340 = (collect_term t)
in (

let _70_342 = (collect_decls decls)
in (collect_decls actions))))
end
| FStar_Parser_AST.RedefineEffect (_70_345, binders, t) -> begin
(

let _70_350 = (collect_binders binders)
in (collect_term t))
end))
...
| FStar_Const.Const_int (_70_390, Some (signedness, width)) -> begin
(
...
let _70_435 = if (((FStar_List.length termimps) = (Prims.parse_int "1")) && (FStar_Options.universes ())) then begin
(record_lid lid)
end else begin
()
end
in (FStar_List.iter (fun _70_440 -> (match (_70_440) with
| (t, _70_439) -> begin
(collect_term t)
end)) termimps))
end
...
let _70_445 = (collect_patterns pats)
in (collect_term t))
end
| FStar_Parser_AST.App (t1, t2, _70_450) -> begin
(

let _70_453 = (collect_term t1)
in (collect_term t2))
end
| FStar_Parser_AST.Let (_70_456, patterms, t) -> begin
(

let _70_466 = (FStar_List.iter (fun _70_463 -> (match (_70_463) with
| (pat, t) -> begin
(

let _70_464 = (collect_pattern pat)
in (collect_term t))
end)) patterms)
in (collect_term t))
end
...
let _70_472 = (record_open true lid)
in (collect_term t))
end
| FStar_Parser_AST.Seq (t1, t2) -> begin
(

let _70_478 = (collect_term t1)
in (collect_term t2))
end
| FStar_Parser_AST.If (t1, t2, t3) -> begin
(

let _70_485 = (collect_term t1)
in (

let _70_487 = (collect_term t2)
in (collect_term t3)))
end
| (FStar_Parser_AST.Match (t, bs)) | (FStar_Parser_AST.TryWith (t, bs)) -> begin
(

let _70_495 = (collect_term t)
in (collect_branches bs))
end
| FStar_Parser_AST.Ascribed (t1, t2) -> begin
(

let _70_501 = (collect_term t1)
in (collect_term t2))
end
| FStar_Parser_AST.Record (t, idterms) -> begin
(

let _70_507 = (FStar_Util.iter_opt t collect_term)
in (FStar_List.iter (fun _70_512 -> (match (_70_512) with
| (_70_510, t) -> begin
(collect_term t)
end)) idterms))
end
| FStar_Parser_AST.Project (t, _70_515) -> begin
(collect_term t)
end
| (FStar_Parser_AST.Product (binders, t)) | (FStar_Parser_AST.Sum (binders, t)) -> begin
(

let _70_524 = (collect_binders binders)
in (collect_term t))
end
| (FStar_Parser_AST.QForall (binders, ts, t)) | (FStar_Parser_AST.QExists (binders, ts, t)) -> begin
(

let _70_533 = (collect_binders binders)
in (

let _70_535 = (FStar_List.iter (FStar_List.iter collect_term) ts)
in (collect_term t)))
end
| FStar_Parser_AST.Refine (binder, t) -> begin
(

let _70_541 = (collect_binder binder)
in (collect_term t))
end
| FStar_Parser_AST.NamedTyp (_70_544, t) -> begin
(collect_term t)
end
...
(FStar_List.iter (fun _70_608 -> (match (_70_608) with
| (_70_606, p) -> begin
(collect_pattern p)
end)) lidpats)
end
...
and collect_branch = (fun _70_619 -> (match (_70_619) with
| (pat, t1, t2) -> begin
(

let _70_620 = (collect_pattern pat)
in (

let _70_622 = (FStar_Util.iter_opt t1 collect_term)
in (collect_term t2)))
end))
in (

let _70_627 = (FStar_Parser_Driver.parse_file filename)
in (match (_70_627) with
| (ast, _70_626) -> begin
(

let _70_628 = (collect_file ast)
...
let _70_631 = (FStar_Util.print_endline "A DOT-format graph has been dumped in the current directory as dep.graph")
in (

let _70_633 = (FStar_Util.print_endline "With GraphViz installed, try: fdp -Tpng -odep.png dep.graph")
in (

let _70_635 = (FStar_Util.print_endline "Hint: cat dep.graph | grep -v _ | grep -v prims")
...
let _70_656 = (let _171_190 = (FStar_Util.smap_try_find m key)
...
| (Some (impl), Some (_70_664)) when interface_only -> begin
[]
end
| (Some (impl), _70_670) -> begin
...
let _70_678 = (FStar_Util.smap_add graph key ((deps), (White)))
...
let _70_683 = (FStar_List.iter discover_command_line_argument filenames)
in (
...
let _70_692 = (let _171_197 = (FStar_Util.smap_try_find graph key)
in (FStar_Util.must _171_197))
in (match (_70_692) with
| (direct_deps, color) -> begin
(match (color) with
| Gray -> begin
(

let _70_694 = (FStar_Util.print1 "Warning: recursive dependency on module %s\n" key)
in (

let _70_696 = (FStar_Util.print1 "The cycle is: %s \n" (FStar_String.concat " -> " cycle))
in (

let _70_698 = (print_graph immediate_graph)
in (

let _70_700 = (FStar_Util.print_string "\n")
...
let _70_704 = (FStar_Util.smap_add graph key ((direct_deps), (Gray)))
in (
...
let _70_708 = (FStar_Util.smap_add graph key ((all_deps), (Black)))
in (

let _70_710 = (let _171_203 = (let _171_202 = (FStar_ST.read topologically_sorted)
in (key)::_171_202)
...
let _70_757 = (FStar_List.iter (fun _70_754 -> (match (_70_754) with
| (m, r) -> begin
...
let print_make : (Prims.string * Prims.string Prims.list) Prims.list  ->  Prims.unit = (fun deps -> (FStar_List.iter (fun _70_762 -> (match (_70_762) with
| (f, deps) -> begin
(
...
let print = (fun _70_769 -> (match (_70_769) with
| (make_deps, _70_767, graph) -> begin
