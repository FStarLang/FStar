          ...
          let rec hash_deps out uu___115_4655 =
            ...
            | fn2::deps1 ->
                let digest =
                  ...
                  then
                    let uu____4696 = digest_of_file1 fn3  in
                    FStar_Pervasives_Native.Some uu____4696
                  else
                    (let uu____4698 =
                       let uu____4701 = FStar_Util.basename fn3  in
                       FStar_Options.find_file uu____4701  in
                     match uu____4698 with
                     | FStar_Pervasives_Native.None  ->
                         FStar_Pervasives_Native.None
                     | FStar_Pervasives_Native.Some fn4 ->
                         let uu____4705 = digest_of_file1 fn4  in
                         FStar_Pervasives_Native.Some uu____4705)
                   in
                (match digest with
                 | FStar_Pervasives_Native.None  ->
                     ((let uu____4715 = FStar_Options.debug_any ()  in
                       if uu____4715
                       then
                         let uu____4716 = cache_file_name fn2  in
                         FStar_Util.print2 "%s: missed digest of file %s\n"
                           cache_file uu____4716
                       else ());
                      FStar_Pervasives_Native.None)
                 | FStar_Pervasives_Native.Some dig ->
                     let uu____4725 =
                       let uu____4732 =
                         let uu____4737 = lowercase_module_name fn2  in
                         (uu____4737, dig)  in
                       uu____4732 :: out  in
                     hash_deps uu____4725 deps1)
             in
      ...
      FStar_All.pipe_right dig
        (FStar_List.map
           (fun uu____4782  ->
              ...
              | (m,d) ->
                  let uu____4789 = FStar_Util.base64_encode d  in
                  FStar_Util.format2 "%s:%s" m uu____4789))
       in
    FStar_All.pipe_right uu____4763 (FStar_String.concat "\n")
    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        ...
        FStar_All.pipe_right keys
          (FStar_List.iter
             (fun f  ->
                let uu____4816 =
                  let uu____4821 = deps_try_find deps f  in
                  FStar_All.pipe_right uu____4821 FStar_Option.get  in
    ...
    | Mk (deps,file_system_map,all_cmd_line_files) ->
        let sort_output_files orig_output_file_map =
          ...
          let should_visit lc_module_name =
            (let uu____4898 =
               FStar_Util.smap_try_find remaining_output_files lc_module_name
                in
             FStar_Option.isSome uu____4898) ||
              (let uu____4902 =
                 FStar_Util.smap_try_find visited_other_modules
                   lc_module_name
                  in
               FStar_Option.isNone uu____4902)
             in
          ...
          let emit_output_file_opt ml_file_opt =
            ...
            | FStar_Pervasives_Native.Some ml_file ->
                let uu____4929 =
                  let uu____4932 = FStar_ST.op_Bang order  in ml_file ::
                    uu____4932
                   in
                FStar_ST.op_Colon_Equals order uu____4929
             in
          let rec aux uu___116_5040 =
            match uu___116_5040 with
            | [] -> ()
            | lc_module_name::modules_to_extract ->
                let visit_file file_opt =
                  ...
                  | FStar_Pervasives_Native.Some file_name ->
                      let uu____5058 = deps_try_find deps file_name  in
                      (match uu____5058 with
                       ...
                       | FStar_Pervasives_Native.Some
                           ...
                           aux immediate_deps1)
                   in
                ((let uu____5082 = should_visit lc_module_name  in
                  if uu____5082
                  then
                    let ml_file_opt = mark_visiting lc_module_name  in
                    ((let uu____5087 =
                        implementation_of file_system_map lc_module_name  in
                      visit_file uu____5087);
                     (let uu____5091 =
          ...
          (let uu____5099 = FStar_ST.op_Bang order  in
           FStar_List.rev uu____5099)
           in
        let keys = deps_keys deps  in
        let output_file ext fst_file =
          let ml_base_name =
            let uu____5166 =
              let uu____5167 =
                let uu____5170 = FStar_Util.basename fst_file  in
                check_and_strip_suffix uu____5170  in
              FStar_Option.get uu____5167  in
            FStar_Util.replace_chars uu____5166 46 "_"  in
          FStar_Options.prepend_output_dir (Prims.strcat ml_base_name ext)
           in
        let norm_path s = FStar_Util.replace_chars s 92 "/"  in
        let output_ml_file f =
          let uu____5185 = output_file ".ml" f  in norm_path uu____5185  in
        let output_krml_file f =
          let uu____5192 = output_file ".krml" f  in norm_path uu____5192  in
        let output_cmx_file f =
          let uu____5199 = output_file ".cmx" f  in norm_path uu____5199  in
        let cache_file f =
          let uu____5206 = cache_file_name f  in norm_path uu____5206  in
        ...
        (FStar_All.pipe_right keys
           (FStar_List.iter
              (fun f  ->
                 let uu____5250 =
                   let uu____5257 = deps_try_find deps f  in
                   FStar_All.pipe_right uu____5257 FStar_Option.get  in
                 match uu____5250 with
                 | (f_deps,uu____5281) ->
                     let iface_deps =
                       let uu____5291 = is_interface f  in
                       if uu____5291
                       then FStar_Pervasives_Native.None
                       else
                         (let uu____5299 =
                            let uu____5302 = lowercase_module_name f  in
                            interface_of file_system_map uu____5302  in
                          ...
                          | FStar_Pervasives_Native.Some iface ->
                              let uu____5310 =
                                let uu____5313 =
                                  let uu____5318 = deps_try_find deps iface
                                     in
                                  FStar_Option.get uu____5318  in
                                FStar_Pervasives_Native.fst uu____5313  in
                              FStar_Pervasives_Native.Some uu____5310)
                        in
                     ...
                     ((let uu____5346 = is_interface f  in
                       if uu____5346
                       then
                         let uu____5347 =
                           let uu____5348 =
                             FStar_Options.prepend_cache_dir norm_f  in
                           norm_path uu____5348  in
                         FStar_Util.print3
                           "%s.source: %s \\\n\t%s\n\ttouch $@\n\n"
                           uu____5347 norm_f files3
                       else ());
                      (let uu____5351 = cache_file f  in
                       FStar_Util.print3 "%s: %s \\\n\t%s\n\n" uu____5351
                         norm_f files3);
                      (let already_there =
                         let uu____5355 =
                           let uu____5366 =
                             let uu____5367 = output_file ".krml" f  in
                         ...
                         | FStar_Pervasives_Native.Some
                             (uu____5378,already_there,uu____5380) ->
                             already_there
                         | FStar_Pervasives_Native.None  -> []  in
                       (let uu____5402 =
                          let uu____5403 = output_file ".krml" f  in
                          norm_path uu____5403  in
                        let uu____5404 =
                          let uu____5413 =
                            let uu____5414 = output_file ".exe" f  in
                            norm_path uu____5414  in
                          let uu____5415 =
                            let uu____5418 =
                              let uu____5421 =
                                let uu____5424 =
                                ...
                                FStar_List.map
                                  (fun x  ->
                                     let uu____5432 = output_file ".krml" x
                                        in
                                     norm_path uu____5432) uu____5424
                                 in
                              FStar_List.append already_there uu____5421  in
                            FStar_List.unique uu____5418  in
                          (uu____5413, uu____5415, false)  in
                        FStar_Util.smap_add transitive_krml uu____5402
                          uu____5404);
                       (let uu____5443 = is_implementation f  in
                        if uu____5443
                        then
                          ((let uu____5445 = output_ml_file f  in
                            let uu____5446 = cache_file f  in
                            FStar_Util.print2 "%s: %s\n\n" uu____5445
                              uu____5446);
                           (let cmx_files =
                              let fst_files =
                              ...
                              FStar_All.pipe_right extracted_fst_files
                               ...
                               in
                            (let uu____5505 =
                               let uu____5506 = lowercase_module_name f  in
                               FStar_Options.should_extract uu____5506  in
                             if uu____5505
                             then
                               let uu____5507 = output_cmx_file f  in
                               let uu____5508 = output_ml_file f  in
                               FStar_Util.print3 "%s: %s \\\n\t%s\n\n"
                                 uu____5507 uu____5508
                                 (FStar_String.concat "\\\n\t" cmx_files)
                             else ());
                            (let uu____5510 = output_krml_file f  in
                             let uu____5511 = cache_file f  in
                             FStar_Util.print2 "%s: %s\n\n" uu____5510
                               uu____5511)))
                        else
                          (let uu____5513 =
                             (let uu____5516 =
                                let uu____5517 = lowercase_module_name f  in
                                has_implementation file_system_map uu____5517
                                 in
                              Prims.op_Negation uu____5516) &&
                               (is_interface f)
                              in
                           if uu____5513
                           then
                             let uu____5518 = output_krml_file f  in
                             let uu____5519 = cache_file f  in
                             FStar_Util.print2 "%s: %s\n\n" uu____5518
                               uu____5519
                           else ()))))));
         (let all_fst_files =
            ...
            FStar_All.pipe_right all_fst_files
              (FStar_List.iter
                 (fun fst_file  ->
                    ...
                    let uu____5550 = FStar_Options.should_extract mname  in
                    if uu____5550
                    then
                      let uu____5551 = output_ml_file fst_file  in
                      FStar_Util.smap_add ml_file_map mname uu____5551
                    else ()));
          ...
          let all_krml_files =
            ...
            FStar_All.pipe_right keys
              (FStar_List.iter
                 (fun fst_file  ->
                    let mname = lowercase_module_name fst_file  in
                    let uu____5567 = output_krml_file fst_file  in
                    FStar_Util.smap_add krml_file_map mname uu____5567));
            sort_output_files krml_file_map  in
          let rec make_transitive f =
            let uu____5580 =
              let uu____5589 = FStar_Util.smap_try_find transitive_krml f  in
              FStar_Util.must uu____5589  in
                   ...
                   (let deps2 =
                      let uu____5652 =
                        let uu____5655 =
                          FStar_List.map
                            (fun dep1  ->
                               let uu____5667 = make_transitive dep1  in
                               match uu____5667 with
                               | (uu____5676,deps2) -> dep1 :: deps2) deps1
                           in
                        FStar_List.flatten uu____5655  in
                      FStar_List.unique uu____5652  in
                    FStar_Util.smap_add transitive_krml f (exe, deps2, true);
                    (exe, deps2)))
             in
          (let uu____5696 = FStar_Util.smap_keys transitive_krml  in
           FStar_List.iter
             (fun f  ->
                let uu____5715 = make_transitive f  in
                match uu____5715 with
                | (exe,deps1) ->
                    let deps2 =
                      let uu____5729 = FStar_List.unique (f :: deps1)  in
                      FStar_String.concat " " uu____5729  in
                    let wasm =
                      let uu____5733 =
                        FStar_Util.substring exe (Prims.parse_int "0")
                          ((FStar_String.length exe) - (Prims.parse_int "4"))
                         in
                      Prims.strcat uu____5733 ".wasm"  in
                    (FStar_Util.print2 "%s: %s\n\n" exe deps2;
                     FStar_Util.print2 "%s: %s\n\n" wasm deps2)) uu____5696);
          (let uu____5736 =
             let uu____5737 =
          ...
          (let uu____5747 =
             let uu____5748 =
               FStar_All.pipe_right all_ml_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____5748 (FStar_String.concat " \\\n\t")
              in
          ...
          (let uu____5757 =
             let uu____5758 =
               FStar_All.pipe_right all_krml_files (FStar_List.map norm_path)
                in
             FStar_All.pipe_right uu____5758 (FStar_String.concat " \\\n\t")
              in
           FStar_Util.print1 "ALL_KRML_FILES=\\\n\t%s\n" uu____5757)))
  
let (print : deps -> unit) =
  fun deps  ->
    let uu____5772 = FStar_Options.dep ()  in
    match uu____5772 with
    | FStar_Pervasives_Native.Some "make" -> print_make deps
    | FStar_Pervasives_Native.Some "full" -> print_full deps
    | FStar_Pervasives_Native.Some "graph" ->
        let uu____5775 = deps  in
        (match uu____5775 with
         | Mk (deps1,uu____5777,uu____5778) -> print_graph deps1)
    | FStar_Pervasives_Native.Some uu____5783 ->
