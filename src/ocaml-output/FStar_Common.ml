      ...
      fun depth  ->
        let rec aux n =
          ...
          then failwith "Too many pops"
            ...
            if n = Prims.int_one
            then pop ()
            else ((let uu___3 = pop ()  in ()); aux (n - Prims.int_one))
           in
        let curdepth =
          let uu___ = FStar_ST.op_Bang stackref  in FStar_List.length uu___
           in
        let n =
          match depth with
...
let string_of_list :
  ...
  fun f  ->
    fun l  ->
      let uu___ =
        let uu___1 =
          let uu___2 = FStar_List.map f l  in FStar_String.concat ", " uu___2
           in
