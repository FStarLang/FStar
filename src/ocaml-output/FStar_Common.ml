...
let string_of_option :
  'Auu____351 .
    ('Auu____351 -> Prims.string) ->
      'Auu____351 FStar_Pervasives_Native.option -> Prims.string
  =
  fun f  ->
    fun uu___61_366  ->
      match uu___61_366 with
      | FStar_Pervasives_Native.None  -> "None"
      | FStar_Pervasives_Native.Some x ->
          let uu____372 = f x  in Prims.strcat "Some " uu____372
...
let force_thunk : 'a . 'a thunk -> 'a =
  fun t  ->
    let uu____507 = FStar_ST.op_Bang t  in
  ...
  fun n1  ->
    fun f  ->
      let rec aux i =
        if i < n1
        then
          let uu____711 = f i  in
          let uu____712 = aux (i + (Prims.parse_int "1"))  in uu____711 ::
            uu____712
        else []  in
