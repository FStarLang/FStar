          ...
          let out =
            let uu____30045 =
              FStar_Util.run_process label "cygpath" ["-m"; s]
                FStar_Pervasives_Native.None
               in
            FStar_All.pipe_right uu____30045 FStar_Util.trim_string  in
    ...
    fun stackref  ->
      fun arg  ->
        FStar_Util.atomically
          (fun uu____30115  ->
             let len =
               let uu____30117 = FStar_ST.op_Bang stackref  in
               FStar_List.length uu____30117  in
        ...
        let rec aux n1 =
          ...
          then failwith "Too many pops"
            ...
            then pop ()
              ...
              ((let uu____30210 = pop ()  in ());
               aux (n1 - (Prims.parse_int "1")))
           in
        let curdepth =
          let uu____30213 = FStar_ST.op_Bang stackref  in
          FStar_List.length uu____30213  in
        let n1 =
          match depth with
          | FStar_Pervasives_Native.Some d -> curdepth - d
          | FStar_Pervasives_Native.None  -> (Prims.parse_int "1")  in
        FStar_Util.atomically (fun uu____30248  -> aux n1)
  
let raise_failed_assertion : 'Auu____30254 . Prims.string -> 'Auu____30254 =
  fun msg  ->
    let uu____30262 = FStar_Util.format1 "Assertion failed: %s" msg  in
    failwith uu____30262
...
let string_of_list :
  ...
  fun f  ->
    fun l  ->
      let uu____30313 =
        let uu____30315 =
          let uu____30317 = FStar_List.map f l  in
          FStar_String.concat ", " uu____30317  in
        Prims.op_Hat uu____30315 "]"  in
      Prims.op_Hat "[" uu____30313
...
let string_of_option :
  'Auu____30352 .
    ('Auu____30352 -> Prims.string) ->
      'Auu____30352 FStar_Pervasives_Native.option -> Prims.string
  =
  fun f  ->
    fun uu___289_30369  ->
      match uu___289_30369 with
      | FStar_Pervasives_Native.None  -> "None"
      | FStar_Pervasives_Native.Some x ->
          let uu____30377 = f x  in Prims.op_Hat "Some " uu____30377
...
let force_thunk : 'a . 'a thunk -> 'a =
  fun t  ->
    let uu____30436 = FStar_ST.op_Bang t  in
  ...
  fun n1  ->
    fun f  ->
      let rec aux i =
        if i < n1
        then
          let uu____30568 = f i  in
          let uu____30569 = aux (i + (Prims.parse_int "1"))  in uu____30568
            :: uu____30569
        else []  in
