...
let id_eq_lid : FStar_Ident.ident -> FStar_Ident.lident -> Prims.bool =
  fun i l -> i.FStar_Ident.idText = l.FStar_Ident.ident.FStar_Ident.idText
  ...
  fun x d ->
    ...
    | FStar_Parser_AST.Tycon (uu____22, tys) ->
        FStar_All.pipe_right tys
          (FStar_Util.for_some (fun uu____57 ->
               match uu____57 with t, uu____65 ->
...
let definition_lids : FStar_Parser_AST.decl -> FStar_Ident.lid Prims.list =
  fun d ->
    ...
    | FStar_Parser_AST.Tycon (uu____92, tys) ->
        FStar_All.pipe_right tys
          (FStar_List.collect (fun uu___57_133 ->
               match uu___57_133 with
               | ( FStar_Parser_AST.TyconAbbrev
                     (id1, uu____143, uu____144, uu____145)
                 , uu____146 ) ->
                   let uu____159 = FStar_Ident.lid_of_ids [id1] in
                   [uu____159]
               | ( FStar_Parser_AST.TyconRecord
                     (id1, uu____161, uu____162, uu____163)
                 , uu____164 ) ->
                   let uu____197 = FStar_Ident.lid_of_ids [id1] in
                   [uu____197]
               | ( FStar_Parser_AST.TyconVariant
                     (id1, uu____199, uu____200, uu____201)
                 , uu____202 ) ->
...
let is_definition_of : FStar_Ident.ident -> FStar_Parser_AST.decl -> Prims.bool =
  fun x d ->
...
let rec prefix_with_iface_decls
    : FStar_Parser_AST.decl Prims.list -> FStar_Parser_AST.decl
  ...
  fun iface1 impl ->
    let qualify_kremlin_private impl1 =
      ...
      ; FStar_Parser_AST.doc= uu___61_292.FStar_Parser_AST.doc
      ; FStar_Parser_AST.quals= uu___61_292.FStar_Parser_AST.quals
      ; FStar_Parser_AST.attrs= krem_private :: impl1.FStar_Parser_AST.attrs }
    in
    match iface1 with
    | [] -> ([], [qualify_kremlin_private impl])
    | iface_hd :: iface_tl ->
      match iface_hd.FStar_Parser_AST.d with
      | FStar_Parser_AST.Tycon (uu____317, tys)
        when FStar_All.pipe_right tys
               (FStar_Util.for_some (fun uu___58_352 ->
                    match uu___58_352 with
      ...
      | FStar_Parser_AST.Val (x, t) -> (
          ...
          if Prims.op_Negation defines_x then
            let uu____404 =
              FStar_All.pipe_right def_ids
                (FStar_Util.for_some (fun y ->
                     FStar_All.pipe_right iface_tl
                       (FStar_Util.for_some (is_val y.FStar_Ident.ident)) ))
            in
            if uu____404 then
              let uu____419 =
                let uu____424 =
                  let uu____425 =
                    ...
                    in
                    FStar_All.pipe_right uu____426 (FStar_String.concat ", ")
                  in
                  FStar_Util.format2
              ...
              in
              FStar_Errors.raise_error uu____419 impl.FStar_Parser_AST.drange
            else (iface1, [qualify_kremlin_private impl])
          else
            let mutually_defined_with_x =
              FStar_All.pipe_right def_ids
                (FStar_List.filter (fun y -> Prims.op_Negation (id_eq_lid x y)))
            in
            let rec aux mutuals iface2 =
              ...
              | y :: ys, iface_hd1 :: iface_tl1 ->
                  if is_val y.FStar_Ident.ident iface_hd1 then
                    let uu____536 = aux ys iface_tl1 in
                    match uu____536 with val_ys, iface3 ->
                    ...
                    let uu____568 =
                      let uu____569 =
                        FStar_List.tryFind
                          ...
                          iface_tl1
                      in
                      FStar_All.pipe_left FStar_Option.isSome uu____569
                    in
                    if uu____568 then
                      let uu____582 =
                        let uu____587 =
                          ...
                          FStar_Util.format2
                            ...
                            uu____588 uu____589
                        in
                        (FStar_Errors.Fatal_WrongDefinitionOrder, uu____587)
                      in
  ...
  fun iface1 ->
    let rec aux iface2 =
      ...
      | hd1 :: tl1 ->
        ...
        | FStar_Parser_AST.Tycon (uu____683, tys)
          when FStar_All.pipe_right tys
                 (FStar_Util.for_some (fun uu___59_718 ->
                      match uu___59_718 with
        ...
        | FStar_Parser_AST.Val (x, t) ->
            ...
            if uu____750 then
              ...
              in
              FStar_Errors.raise_error uu____751 hd1.FStar_Parser_AST.drange
            else
              let uu____758 =
          ...
          FStar_List.partition
            (fun d ->
              FStar_All.pipe_right xs
                (FStar_Util.for_some (fun x -> is_val x.FStar_Ident.ident d))
              )
            iface1
        in
        match uu____814 with val_xs, rest_iface ->
...
let prefix_one_decl
  ...
  fun iface1 impl ->
    ...
    | uu____909 ->
        ...
        if uu____910 then ml_mode_prefix_with_iface_decls iface1 impl
...
let initialize_interface
  ...
  fun mname l env ->
    let decls =
      ...
      if uu____942 then ml_mode_check_initial_interface l
      else check_initial_interface l
    in
        ...
        let uu____960 =
          let uu____965 =
            ...
            FStar_Util.format1 "Interface %s has already been processed"
              uu____966
          in
          (FStar_Errors.Fatal_InterfaceAlreadyProcessed, uu____965)
        in
        FStar_Errors.raise_error uu____960 (FStar_Ident.range_of_lid mname)
    | FStar_Pervasives_Native.None ->
        let uu____973 = FStar_ToSyntax_Env.set_iface_decls env mname decls in
        ((), uu____973)
...
let prefix_with_interface_decls
  ...
  fun impl env ->
    let uu____990 =
      ...
      FStar_ToSyntax_Env.iface_decls env uu____995
    in
    match uu____990 with
    | FStar_Pervasives_Native.None -> ([impl], env)
    | FStar_Pervasives_Native.Some iface1 ->
        let uu____1011 = prefix_one_decl iface1 impl in
        match uu____1011 with iface2, impl1 ->
          let env1 =
            let uu____1037 = FStar_ToSyntax_Env.current_module env in
            FStar_ToSyntax_Env.set_iface_decls env uu____1037 iface2
          in
          (impl1, env1)


let interleave_module
    : FStar_Parser_AST.modul -> Prims.bool
      -> FStar_Parser_AST.modul FStar_ToSyntax_Env.withenv =
  fun a expect_complete_modul env ->
    match a with
        ...
        | FStar_Pervasives_Native.Some iface1 ->
            let uu____1090 =
              FStar_List.fold_left
                (fun uu____1114 impl ->
                  match uu____1114 with iface2, impls1 ->
                    let uu____1142 = prefix_one_decl iface2 impl in
                    match uu____1142 with iface3, impls' ->
                      (iface3, FStar_List.append impls1 impls') )
                (iface1, []) impls
            in
            match uu____1090 with iface2, impls1 ->
              let uu____1191 =
                let uu____1200 =
                  FStar_Util.prefix_until
                    (fun uu___60_1219 ->
                      match uu___60_1219 with
                      | { FStar_Parser_AST.d= FStar_Parser_AST.Val uu____1220
                ...
                | FStar_Pervasives_Native.Some (lets, one_val, rest) ->
                    (lets, one_val :: rest)
              in
              match uu____1191 with iface_lets, remaining_iface_vals ->
                let impls2 = FStar_List.append impls1 iface_lets in
                let env1 =
                  FStar_ToSyntax_Env.set_iface_decls env l remaining_iface_vals
                in
                let a1 = FStar_Parser_AST.Module (l, impls2) in
                match remaining_iface_vals with
                | uu____1304 :: uu____1305 when expect_complete_modul ->
                    let err =
                      let uu____1309 =
                    ...
                    let uu____1314 =
                      let uu____1319 =
                        let uu____1320 = FStar_Ident.string_of_lid l in
                        FStar_Util.format2
