...
(FStar_All.pipe_right tys (FStar_List.collect (fun uu___192_79 -> (match (uu___192_79) with
...
| FStar_Parser_AST.Tycon (uu____138, tys) when (FStar_All.pipe_right tys (FStar_Util.for_some (fun uu___193_155 -> (match (uu___193_155) with
...
(Prims.raise (FStar_Errors.Error ((("Interface contains an abstract \'type\' declaration; use \'val\' instead"), (impl.FStar_Parser_AST.drange)))))
end
...
| FStar_Parser_AST.Tycon (uu____351, tys) when (FStar_All.pipe_right tys (FStar_Util.for_some (fun uu___194_368 -> (match (uu___194_368) with
...
(Prims.raise (FStar_Errors.Error ((("Interface contains an abstract \'type\' declaration; use \'val\' instead"), (hd1.FStar_Parser_AST.drange)))))
end
| FStar_Parser_AST.Val (x, t) -> begin
(
...
| true -> begin
(Prims.raise (FStar_Errors.Error ((("Interfaces cannot use `assume val x : t`; just write `val x : t` instead"), (hd1.FStar_Parser_AST.drange)))))
end
| uu____395 -> begin
()
end))
