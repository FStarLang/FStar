...
| None -> begin
(let _21_138 = (let _87_43 = (set_fstar_home ())
...
end))), "Don\'t print an error message; show an exception trace instead"))::((FStar_Getopt.noshort, "codegen", FStar_Getopt.OneArg (((fun s -> (let _87_71 = (parse_codegen s)
in (FStar_ST.op_Colon_Equals codegen _87_71))), "OCaml|FSharp|Wysteria")), "Generate code for execution"))::((FStar_Getopt.noshort, "codegen-lib", FStar_Getopt.OneArg (((fun s -> (let _87_76 = (let _87_75 = (FStar_ST.read codegen_libs)
in ((FStar_Util.split s "."))::_87_75)
in (FStar_ST.op_Colon_Equals codegen_libs _87_76))), "namespace")), "External runtime library library"))::((FStar_Getopt.noshort, "lax", FStar_Getopt.ZeroArgs ((fun _21_173 -> (match (()) with
| () -> begin
...
end))), "Print real names---you may want to use this in conjunction with logQueries"))::((FStar_Getopt.noshort, "debug", FStar_Getopt.OneArg (((fun x -> (let _87_90 = (let _87_89 = (FStar_ST.read debug)
in (x)::_87_89)
in (FStar_ST.op_Colon_Equals debug _87_90))), "module name")), "Print LOTS of debugging information while checking module [arg]"))::((FStar_Getopt.noshort, "debug_level", FStar_Getopt.OneArg (((fun x -> (let _87_95 = (let _87_94 = (FStar_ST.read debug_level)
in ((dlevel x))::_87_94)
in (FStar_ST.op_Colon_Equals debug_level _87_95))), "Low|Medium|High|Extreme")), "Control the verbosity of debugging info"))::((FStar_Getopt.noshort, "log_types", FStar_Getopt.ZeroArgs ((fun _21_182 -> (match (()) with
| () -> begin
...
end))), "Print inferred predicate transformers for all computation types"))::((FStar_Getopt.noshort, "dump_module", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dump_module (Some (x)))), "module name")), ""))::((FStar_Getopt.noshort, "z3timeout", FStar_Getopt.OneArg (((fun s -> (let _87_104 = (FStar_Util.int_of_string s)
in (FStar_ST.op_Colon_Equals z3timeout _87_104))), "t")), "Set the Z3 per-query (soft) timeout to t seconds (default 5)"))::((FStar_Getopt.noshort, "admit_smt_queries", FStar_Getopt.OneArg (((fun s -> (let _87_108 = (match ((s = "true")) with
| true -> begin
true
end
| false -> begin
...
in (FStar_ST.op_Colon_Equals admit_smt_queries _87_108))), "true|false")), "Admit SMT queries (UNSAFE! But, useful during development); default: \'false\'"))::((FStar_Getopt.noshort, "logQueries", FStar_Getopt.ZeroArgs ((fun _21_187 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals logQueries true)
end))), "Log the Z3 queries in queries.smt2"))::((FStar_Getopt.noshort, "admit_fsi", FStar_Getopt.OneArg (((fun x -> (let _87_114 = (let _87_113 = (FStar_ST.read admit_fsi)
in (x)::_87_113)
in (FStar_ST.op_Colon_Equals admit_fsi _87_114))), "module name")), "Treat .fsi as a .fst"))::((FStar_Getopt.noshort, "odir", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals outputDir (Some (x)))), "dir")), "Place output in directory dir"))::((FStar_Getopt.noshort, "smt", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals z3_exe x)), "path")), "Path to the SMT solver (usually Z3, but could be any SMT2-compatible solver)"))::((FStar_Getopt.noshort, "print_before_norm", FStar_Getopt.ZeroArgs ((fun _21_191 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals norm_then_print false)
end))), "Do not normalize types before printing (for debugging)"))::((FStar_Getopt.noshort, "show_signatures", FStar_Getopt.OneArg (((fun x -> (let _87_126 = (let _87_125 = (FStar_ST.read show_signatures)
in (x)::_87_125)
in (FStar_ST.op_Colon_Equals show_signatures _87_126))), "module name")), "Show the checked signatures for all top-level symbols in the module"))::((FStar_Getopt.noshort, "full_context_dependency", FStar_Getopt.ZeroArgs ((fun _21_193 -> (match (()) with
| () -> begin
...
end))), "Serialize compiled modules"))::((FStar_Getopt.noshort, "initial_fuel", FStar_Getopt.OneArg (((fun x -> (let _87_136 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals initial_fuel _87_136))), "non-negative integer")), "Number of unrolling of recursive functions to try initially (default 2)"))::((FStar_Getopt.noshort, "max_fuel", FStar_Getopt.OneArg (((fun x -> (let _87_140 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals max_fuel _87_140))), "non-negative integer")), "Number of unrolling of recursive functions to try at most (default 8)"))::((FStar_Getopt.noshort, "min_fuel", FStar_Getopt.OneArg (((fun x -> (let _87_144 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals min_fuel _87_144))), "non-negative integer")), "Minimum number of unrolling of recursive functions to try (default 1)"))::((FStar_Getopt.noshort, "initial_ifuel", FStar_Getopt.OneArg (((fun x -> (let _87_148 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals initial_ifuel _87_148))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at first (default 1)"))::((FStar_Getopt.noshort, "max_ifuel", FStar_Getopt.OneArg (((fun x -> (let _87_152 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals max_ifuel _87_152))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at most (default 1)"))::((FStar_Getopt.noshort, "warn_top_level_effects", FStar_Getopt.ZeroArgs ((fun _21_204 -> (match (()) with
| () -> begin
...
end))), "Do not allow the use of t<t1,...,tn> syntax for type applications"))::((FStar_Getopt.noshort, "n_cores", FStar_Getopt.OneArg (((fun x -> (let _87_163 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals n_cores _87_163))), "positive integer")), "Maximum number of cores to use for the solver (default 1)"))::((FStar_Getopt.noshort, "verify_module", FStar_Getopt.OneArg (((fun x -> (let _87_168 = (let _87_167 = (FStar_ST.read verify_module)
in (x)::_87_167)
in (FStar_ST.op_Colon_Equals verify_module _87_168))), "string")), "Name of the module to verify"))::((FStar_Getopt.noshort, "use_build_config", FStar_Getopt.ZeroArgs ((fun _21_213 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals use_build_config true)
end))), "Expect just a single file on the command line and no options; will read the \'build-config\' prelude from the file"))::((FStar_Getopt.noshort, "split_cases", FStar_Getopt.OneArg (((fun n -> (let _87_173 = (FStar_Util.int_of_string n)
in (FStar_ST.op_Colon_Equals split_cases _87_173))), "t")), "Partition VC of a match into groups of n cases"))::((FStar_Getopt.noshort, "in", FStar_Getopt.ZeroArgs ((fun _21_215 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals interactive true)
end))), "Interactive mode; reads input from stdin"))::((FStar_Getopt.noshort, "include", FStar_Getopt.OneArg (((fun s -> (let _87_179 = (let _87_178 = (FStar_ST.read _include_path)
in (FStar_List.append _87_178 ((s)::[])))
in (FStar_ST.op_Colon_Equals _include_path _87_179))), "path")), "A directory in which to search for files included on the command line"))::((FStar_Getopt.noshort, "fsi", FStar_Getopt.ZeroArgs ((fun _21_217 -> (match (()) with
| () -> begin
(set_interactive_fsi ())
...
end))), "Print the fuel amounts used for each successful query"))::((FStar_Getopt.noshort, "cardinality", FStar_Getopt.OneArg (((fun x -> (let _87_185 = (validate_cardinality x)
in (FStar_ST.op_Colon_Equals cardinality _87_185))), "off|warn|check")), "Check cardinality constraints on inductive data types(default \'off\')"))::((FStar_Getopt.noshort, "__temp_no_proj", FStar_Getopt.ZeroArgs ((fun _21_220 -> (match (()) with
| () -> begin
...
| _21_234 -> begin
(let _21_235 = (FStar_Util.print_string "Wrong argument to codegen flag\n")
in (let _21_237 = (let _87_190 = (specs ())
in (display_usage _87_190))
in (FStar_All.exit 1)))
end))
...
| _21_244 -> begin
(let _21_245 = (FStar_Util.print_string "Wrong argument to cardinality flag\n")
in (let _21_247 = (let _87_192 = (specs ())
in (display_usage _87_192))
in (FStar_All.exit 1)))
end))
and set_interactive_fsi = (fun _21_249 -> (match ((FStar_ST.read interactive)) with
| true -> begin
(FStar_ST.op_Colon_Equals interactive_fsi true)
end
| false -> begin
(let _21_251 = (FStar_Util.print_string "Set interactive flag first before setting interactive fsi flag\n")
in (let _21_253 = (let _87_194 = (specs ())
in (display_usage _87_194))
in (FStar_All.exit 1)))
end))
...
let should_print_message = (fun m -> (((should_verify m) && (not ((let _87_199 = (FStar_ST.read admit_fsi)
in (FStar_List.contains m _87_199))))) && (m <> "Prims")))

let set_options = (let no_smt_specs = (let _87_202 = (specs ())
in (FStar_All.pipe_right _87_202 (FStar_List.filter (fun _21_266 -> (match (_21_266) with
| (_21_260, name, _21_263, _21_265) -> begin
(name <> "smt")
end)))))
in (fun s -> (FStar_Getopt.parse_string no_smt_specs (fun _21_269 -> ()) s)))

let reset_options_string = (FStar_ST.alloc None)

let reset_options = (fun _21_271 -> (match (()) with
| () -> begin
(let _21_272 = (init_options ())
in (let res = (let _87_208 = (specs ())
in (FStar_Getopt.parse_cmdline _87_208 (fun x -> ())))
