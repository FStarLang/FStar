...
let set_fstar_home : Prims.unit  ->  Prims.string = (fun _24_144 -> (match (()) with
| () -> begin
(
...
let _24_148 = (FStar_ST.op_Colon_Equals _fstar_home x)
in (
# 178 "FStar.Options.fst"
let _24_150 = (FStar_ST.op_Colon_Equals fstar_home_opt (Some (x)))
in x))))
end
| Some (x) -> begin
(
# 180 "FStar.Options.fst"
let _24_154 = (FStar_ST.op_Colon_Equals _fstar_home x)
in x)
end)
in fh)
end))
...
let get_fstar_home : Prims.unit  ->  Prims.string = (fun _24_157 -> (match (()) with
| () -> begin
(match ((FStar_ST.read fstar_home_opt)) with
| None -> begin
(
# 183 "FStar.Options.fst"
let _24_159 = (let _113_43 = (set_fstar_home ())
...
let get_include_path : Prims.unit  ->  Prims.string Prims.list = (fun _24_163 -> (match (()) with
| () -> begin
...
let prims : Prims.unit  ->  Prims.string = (fun _24_178 -> (match (()) with
| () -> begin
...
let display_version : Prims.unit  ->  Prims.unit = (fun _24_190 -> (match (()) with
| () -> begin
...
let _24_192 = (FStar_Util.print_string "fstar [option] file...\n")
in (FStar_List.iter (fun _24_199 -> (match (_24_199) with
| (_24_195, flag, p, doc) -> begin
(match (p) with
...
| FStar_Getopt.OneArg (_24_203, argname) -> begin
if (doc = "") then begin
...
let rec specs : Prims.unit  ->  FStar_Getopt.opt Prims.list = (fun _24_207 -> (match (()) with
| () -> begin
(
...
in (FStar_ST.op_Colon_Equals debug_level _113_111))), "Low|Medium|High|Extreme")), "Control the verbosity of debugging info"))::((FStar_Getopt.noshort, "dep", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dep (Some (x)))), "make|nubuild")), "Output the transitive closure of the dependency graph in a format suitable for the given tool"))::((FStar_Getopt.noshort, "detail_errors", FStar_Getopt.ZeroArgs ((fun _24_215 -> (match (()) with
| () -> begin
(
# 264 "FStar.Options.fst"
let _24_216 = (FStar_ST.op_Colon_Equals detail_errors true)
in (FStar_ST.op_Colon_Equals n_cores 1))
end))), "Emit a detailed error report by asking the SMT solver many queries; will take longer; implies n_cores=1; requires --universes"))::((FStar_Getopt.noshort, "dump_module", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dump_module (Some (x)))), "module name")), ""))::((FStar_Getopt.noshort, "eager_inference", FStar_Getopt.ZeroArgs ((fun _24_219 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals eager_inference true)
end))), "Solve all type-inference constraints eagerly; more efficient but at the cost of generality"))::((FStar_Getopt.noshort, "explicit_deps", FStar_Getopt.ZeroArgs ((fun _24_220 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals explicit_deps true)
end))), "tell FStar to not find dependencies automatically because the user provides them on the command-line"))::((FStar_Getopt.noshort, "fs_typ_app", FStar_Getopt.ZeroArgs ((fun _24_221 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals fs_typ_app true)
end))), "Allow the use of t<t1,...,tn> syntax for type applications; brittle since it clashes with the integer less-than operator"))::((FStar_Getopt.noshort, "fsi", FStar_Getopt.ZeroArgs ((fun _24_222 -> (match (()) with
| () -> begin
(set_interactive_fsi ())
end))), "fsi flag; A flag to indicate if type checking a fsi in the interactive mode"))::((FStar_Getopt.noshort, "fstar_home", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals fstar_home_opt (Some (x)))), "dir")), "Set the FSTAR_HOME variable to dir"))::((FStar_Getopt.noshort, "hide_genident_nums", FStar_Getopt.ZeroArgs ((fun _24_224 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals hide_genident_nums true)
end))), "Don\'t print generated identifier numbers"))::((FStar_Getopt.noshort, "hide_uvar_nums", FStar_Getopt.ZeroArgs ((fun _24_225 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals hide_uvar_nums true)
end))), "Don\'t print unification variable numbers"))::((FStar_Getopt.noshort, "in", FStar_Getopt.ZeroArgs ((fun _24_226 -> (match (()) with
| () -> begin
...
let _24_228 = (FStar_ST.op_Colon_Equals interactive true)
...
in (FStar_ST.op_Colon_Equals initial_ifuel _113_144))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at first (default 1)"))::((FStar_Getopt.noshort, "inline_arith", FStar_Getopt.ZeroArgs ((fun _24_233 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals inline_arith true)
end))), "Inline definitions of arithmetic functions in the SMT encoding"))::((FStar_Getopt.noshort, "lax", FStar_Getopt.ZeroArgs ((fun _24_234 -> (match (()) with
| () -> begin
(
# 279 "FStar.Options.fst"
let _24_235 = (FStar_ST.op_Colon_Equals pretype true)
in (FStar_ST.op_Colon_Equals verify false))
end))), "Run the lax-type checker only (admit all verification conditions)"))::((FStar_Getopt.noshort, "log_types", FStar_Getopt.ZeroArgs ((fun _24_237 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals log_types true)
end))), "Print types computed for data/val/let-bindings"))::((FStar_Getopt.noshort, "log_queries", FStar_Getopt.ZeroArgs ((fun _24_238 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals min_fuel _113_160))), "non-negative integer")), "Minimum number of unrolling of recursive functions to try (default 1)"))::((FStar_Getopt.noshort, "MLish", FStar_Getopt.ZeroArgs ((fun _24_242 -> (match (()) with
| () -> begin
...
let _24_244 = (let _113_165 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals n_cores _113_165))
in (FStar_ST.op_Colon_Equals detail_errors false))), "positive integer")), "Maximum number of cores to use for the solver (default 1); implied detail_errors = false"))::((FStar_Getopt.noshort, "no_default_includes", FStar_Getopt.ZeroArgs ((fun _24_246 -> (match (()) with
| () -> begin
...
in (x)::_113_170)
in (FStar_ST.op_Colon_Equals no_extract _113_171))), "module name")), "Do not extract code from this module"))::((FStar_Getopt.noshort, "no_location_info", FStar_Getopt.ZeroArgs ((fun _24_248 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals no_location_info true)
end))), "Suppress location information in the generated OCaml output (only relevant with --codegen OCaml)"))::((FStar_Getopt.noshort, "odir", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals outputDir (Some (x)))), "dir")), "Place output in directory dir"))::((FStar_Getopt.noshort, "prims", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals prims_ref (Some (x)))), "file")), ""))::((FStar_Getopt.noshort, "print_before_norm", FStar_Getopt.ZeroArgs ((fun _24_251 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals norm_then_print false)
end))), "Do not normalize types before printing (for debugging)"))::((FStar_Getopt.noshort, "print_bound_var_types", FStar_Getopt.ZeroArgs ((fun _24_252 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_bound_var_types true)
end))), "Print the types of bound variables"))::((FStar_Getopt.noshort, "print_effect_args", FStar_Getopt.ZeroArgs ((fun _24_253 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_effect_args true)
end))), "Print inferred predicate transformers for all computation types"))::((FStar_Getopt.noshort, "print_fuels", FStar_Getopt.ZeroArgs ((fun _24_254 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_fuels true)
end))), "Print the fuel amounts used for each successful query"))::((FStar_Getopt.noshort, "print_implicits", FStar_Getopt.ZeroArgs ((fun _24_255 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_implicits true)
end))), "Print implicit arguments"))::((FStar_Getopt.noshort, "print_universes", FStar_Getopt.ZeroArgs ((fun _24_256 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_universes true)
end))), "Print universes"))::((FStar_Getopt.noshort, "prn", FStar_Getopt.ZeroArgs ((fun _24_257 -> (match (()) with
| () -> begin
...
in (x)::_113_189)
in (FStar_ST.op_Colon_Equals show_signatures _113_190))), "module name")), "Show the checked signatures for all top-level symbols in the module"))::((FStar_Getopt.noshort, "silent", FStar_Getopt.ZeroArgs ((fun _24_259 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals split_cases _113_198))), "t")), "Partition VC of a match into groups of n cases"))::((FStar_Getopt.noshort, "timing", FStar_Getopt.ZeroArgs ((fun _24_262 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals timing true)
end))), "Print the time it takes to verify each top-level definition"))::((FStar_Getopt.noshort, "trace_error", FStar_Getopt.ZeroArgs ((fun _24_263 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals trace_error true)
end))), "Don\'t print an error message; show an exception trace instead"))::((FStar_Getopt.noshort, "universes", FStar_Getopt.ZeroArgs ((fun _24_264 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals universes true)
end))), "Use the (experimental) support for universes"))::((FStar_Getopt.noshort, "unthrottle_inductives", FStar_Getopt.ZeroArgs ((fun _24_265 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals unthrottle_inductives true)
end))), "Let the SMT solver unfold inductive types to arbitrary depths (may affect verifier performance)"))::((FStar_Getopt.noshort, "use_eq_at_higher_order", FStar_Getopt.ZeroArgs ((fun _24_266 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals use_eq_at_higher_order true)
end))), "Use equality constraints when comparing higher-order types; temporary"))::((FStar_Getopt.noshort, "use_native_int", FStar_Getopt.ZeroArgs ((fun _24_267 -> (match (()) with
| () -> begin
...
in (x)::_113_213)
in (FStar_ST.op_Colon_Equals __temp_no_proj _113_214))), "string")), "Don\'t generate projectors for this module"))::(('v', "version", FStar_Getopt.ZeroArgs ((fun _24_270 -> (
# 311 "FStar.Options.fst"
let _24_272 = (display_version ())
in (FStar_All.exit 0)))), "Display version number"))::((FStar_Getopt.noshort, "warn_top_level_effects", FStar_Getopt.ZeroArgs ((fun _24_274 -> (match (()) with
| () -> begin
...
let _24_278 = (display_usage specs)
...
let _24_285 = (FStar_Util.print_string "Wrong argument to codegen flag\n")
in (
# 322 "FStar.Options.fst"
let _24_287 = (let _113_223 = (specs ())
...
let _24_295 = (FStar_Util.print_string "Wrong argument to cardinality flag\n")
in (
# 329 "FStar.Options.fst"
let _24_297 = (let _113_225 = (specs ())
in (display_usage _113_225))
in (FStar_All.exit 1)))
end))
and set_interactive_fsi : Prims.unit  ->  Prims.unit = (fun _24_299 -> if (FStar_ST.read interactive) then begin
(FStar_ST.op_Colon_Equals interactive_fsi true)
end else begin
(
# 333 "FStar.Options.fst"
let _24_301 = (FStar_Util.print_string "Set interactive flag first before setting interactive fsi flag\n")
in (
# 334 "FStar.Options.fst"
let _24_303 = (let _113_227 = (specs ())
...
let settable_specs = (FStar_All.pipe_right all_specs (FStar_List.filter (fun _24_357 -> (match (_24_357) with
| (_24_351, x, _24_354, _24_356) -> begin
(settable x)
end))))
in (
# 396 "FStar.Options.fst"
let resettable_specs = (FStar_All.pipe_right all_specs (FStar_List.filter (fun _24_366 -> (match (_24_366) with
| (_24_360, x, _24_363, _24_365) -> begin
(resettable x)
end))))
in (fun o s -> (
...
in (FStar_Getopt.parse_string specs (fun _24_374 -> ()) s))))))))

# 402 "FStar.Options.fst"
let restore_cmd_line_options : Prims.unit  ->  FStar_Getopt.parse_cmdline_res = (fun _24_376 -> (match (()) with
| () -> begin
(
...
let _24_382 = (FStar_ST.op_Colon_Equals verify_module old_verify_module)
in r))))
end))
