...
let get_load: Prims.unit -> Prims.string option =
  fun uu____652  -> lookup_opt "load" (as_option as_string)
let get_log_queries: Prims.unit -> Prims.bool =
  fun uu____656  -> lookup_opt "log_queries" as_bool
let get_log_types: Prims.unit -> Prims.bool =
  fun uu____659  -> lookup_opt "log_types" as_bool
let get_max_fuel: Prims.unit -> Prims.int =
  fun uu____662  -> lookup_opt "max_fuel" as_int
let get_max_ifuel: Prims.unit -> Prims.int =
  fun uu____665  -> lookup_opt "max_ifuel" as_int
let get_min_fuel: Prims.unit -> Prims.int =
  fun uu____668  -> lookup_opt "min_fuel" as_int
let get_MLish: Prims.unit -> Prims.bool =
  fun uu____671  -> lookup_opt "MLish" as_bool
let get_n_cores: Prims.unit -> Prims.int =
  fun uu____674  -> lookup_opt "n_cores" as_int
let get_no_default_includes: Prims.unit -> Prims.bool =
  fun uu____677  -> lookup_opt "no_default_includes" as_bool
let get_no_extract: Prims.unit -> Prims.string Prims.list =
  fun uu____681  -> lookup_opt "no_extract" (as_list as_string)
let get_no_location_info: Prims.unit -> Prims.bool =
  fun uu____685  -> lookup_opt "no_location_info" as_bool
let get_odir: Prims.unit -> Prims.string option =
  fun uu____689  -> lookup_opt "odir" (as_option as_string)
let get_ugly: Prims.unit -> Prims.bool =
  fun uu____693  -> lookup_opt "ugly" as_bool
let get_prims: Prims.unit -> Prims.string option =
  fun uu____697  -> lookup_opt "prims" (as_option as_string)
...
let get_no_positivity: Prims.unit -> Prims.bool =
  fun uu____821  -> lookup_opt "__no_positivity" as_bool
let dlevel: Prims.string -> debug_level_t =
  fun uu___54_824  ->
    match uu___54_824 with
    | "Low" -> Low
    | "Medium" -> Medium
    | "High" -> High
    | "Extreme" -> Extreme
...
let debug_level_geq: debug_level_t -> Prims.bool =
  fun l2  ->
    let uu____836 = get_debug_level () in
    FStar_All.pipe_right uu____836
...
let display_version: Prims.unit -> Prims.unit =
  fun uu____863  ->
    let uu____864 =
      let uu____865 = FStar_ST.read _version in
      let uu____868 = FStar_ST.read _platform in
      let uu____871 = FStar_ST.read _compiler in
      let uu____874 = FStar_ST.read _date in
      let uu____877 = FStar_ST.read _commit in
      FStar_Util.format5
        "F* %s\nplatform=%s\ncompiler=%s\ndate=%s\ncommit=%s\n" uu____865
        uu____868 uu____871 uu____874 uu____877 in
    FStar_Util.print_string uu____864
let display_usage_aux specs =
  FStar_Util.print_string "fstar.exe [options] file[s]\n";
  FStar_List.iter
    (fun uu____907  ->
       match uu____907 with
       | (uu____913,flag,p,doc) ->
           (match p with
            ...
            | FStar_Getopt.OneArg (uu____927,argname) ->
                if doc = ""
                then
                  let uu____933 =
                  ...
                  FStar_Util.print_string uu____933
                else
                  (let uu____937 =
                     let uu____938 = FStar_Util.colorize_bold flag in
                     let uu____939 = FStar_Util.colorize_bold argname in
                     FStar_Util.format3 "  --%s %s  %s\n" uu____938 uu____939
                       doc in
                   FStar_Util.print_string uu____937))) specs
let mk_spec:
    ...
    | (ns,name,arg,desc) ->
        let arg1 =
          ...
          | FStar_Getopt.ZeroArgs f ->
              let g uu____974 =
                let uu____975 = let uu____978 = f () in (name, uu____978) in
                set_option' uu____975 in
              FStar_Getopt.ZeroArgs g
          | FStar_Getopt.OneArg (f,d) ->
              let g x =
                let uu____989 = let uu____992 = f x in (name, uu____992) in
                set_option' uu____989 in
...
let cons_extract_module: Prims.string -> option_val =
  fun s  ->
    let uu____999 =
      let uu____1001 =
        let uu____1003 = get_extract_module () in (FStar_String.lowercase s)
          :: uu____1003 in
...
let cons_extract_namespace: Prims.string -> option_val =
  fun s  ->
    let uu____1010 =
      let uu____1012 =
        let uu____1014 = get_extract_namespace () in
        (FStar_String.lowercase s) :: uu____1014 in
      FStar_All.pipe_right uu____1012
        (FStar_List.map (fun _0_27  -> String _0_27)) in
    List uu____1010
let add_extract_module: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1021 = cons_extract_module s in
    set_option "extract_module" uu____1021
let add_extract_namespace: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1025 = cons_extract_namespace s in
    set_option "extract_namespace" uu____1025
let cons_verify_module: Prims.string -> option_val =
  fun s  ->
    let uu____1029 =
      let uu____1031 =
        let uu____1033 = get_verify_module () in (FStar_String.lowercase s)
          :: uu____1033 in
      FStar_All.pipe_right uu____1031
        (FStar_List.map (fun _0_28  -> String _0_28)) in
    List uu____1029
let cons_using_facts_from: Prims.string -> option_val =
  fun s  ->
    set_option "z3refresh" (Bool true);
    (let uu____1041 = get_using_facts_from () in
...
let add_verify_module: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1053 = cons_verify_module s in
    set_option "verify_module" uu____1053
let rec specs:
  Prims.unit ->
  ...
  fun uu____1065  ->
    let specs1 =
      ...
      (FStar_Getopt.noshort, "codegen",
        (FStar_Getopt.OneArg
           (((fun s  -> let uu____1094 = parse_codegen s in String uu____1094)),
      ...
      (FStar_Getopt.noshort, "codegen-lib",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1104 =
                  let uu____1106 =
                    let uu____1108 = get_codegen_lib () in s :: uu____1108 in
                  FStar_All.pipe_right uu____1106
                    (FStar_List.map (fun _0_30  -> String _0_30)) in
                List uu____1104)), "[namespace]")),
      ...
      (FStar_Getopt.noshort, "debug_level",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1138 =
                  let uu____1140 =
                    let uu____1142 = get_debug_level () in x :: uu____1142 in
                  FStar_All.pipe_right uu____1140
                    (FStar_List.map (fun _0_32  -> String _0_32)) in
                List uu____1138)), "[Low|Medium|High|Extreme|...]")),
      ...
      (FStar_Getopt.noshort, "dump_module",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1179 =
                  let uu____1181 =
                    let uu____1183 = get_dump_module () in x :: uu____1183 in
                  FStar_All.pipe_right uu____1181
                    (FStar_List.map (fun _0_33  -> String _0_33)) in
                FStar_All.pipe_right uu____1179 (fun _0_34  -> List _0_34))),
             "[module name]")), "");
      ...
      (FStar_Getopt.noshort, "include",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1277 =
                  let uu____1279 =
                    let uu____1281 = get_include () in
                    FStar_List.map (fun _0_36  -> String _0_36) uu____1281 in
                  FStar_List.append uu____1279 [Path s] in
                List uu____1277)), "[path]")),
        "A directory in which to search for files included on the command line");
      (FStar_Getopt.noshort, "indent",
        (FStar_Getopt.ZeroArgs ((fun uu____1289  -> Bool true))),
        "Parses and outputs the files on the command line");
      (FStar_Getopt.noshort, "initial_fuel",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1299 = FStar_Util.int_of_string x in Int uu____1299)),
             "[non-negative integer]")),
        "Number of unrolling of recursive functions to try initially (default 2)");
      (FStar_Getopt.noshort, "initial_ifuel",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1309 = FStar_Util.int_of_string x in Int uu____1309)),
             "[non-negative integer]")),
      ...
      (FStar_Getopt.noshort, "max_fuel",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1356 = FStar_Util.int_of_string x in Int uu____1356)),
             "[non-negative integer]")),
        "Number of unrolling of recursive functions to try at most (default 8)");
      (FStar_Getopt.noshort, "max_ifuel",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1366 = FStar_Util.int_of_string x in Int uu____1366)),
             "[non-negative integer]")),
      ...
      (FStar_Getopt.noshort, "n_cores",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1393 = FStar_Util.int_of_string x in Int uu____1393)),
             "[positive integer]")),
      ...
      (FStar_Getopt.noshort, "no_extract",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1410 =
                  let uu____1412 =
                    let uu____1414 = get_no_extract () in x :: uu____1414 in
                  FStar_All.pipe_right uu____1412
                    (FStar_List.map (fun _0_37  -> String _0_37)) in
                List uu____1410)), "[module name]")),
      ...
      (FStar_Getopt.noshort, "odir",
        (FStar_Getopt.OneArg
           (((fun p  -> let uu____1434 = validate_dir p in Path uu____1434)),
      ...
      (FStar_Getopt.noshort, "split_cases",
        (FStar_Getopt.OneArg
           (((fun n1  ->
                let uu____1586 = FStar_Util.int_of_string n1 in
                Int uu____1586)), "[positive integer]")),
      ...
      (FStar_Getopt.noshort, "__temp_no_proj",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1668 =
                  let uu____1670 =
                    let uu____1672 = get___temp_no_proj () in x :: uu____1672 in
                  FStar_All.pipe_right uu____1670
                    (FStar_List.map (fun _0_44  -> String _0_44)) in
                List uu____1668)), "[module name]")),
      ...
      (FStar_Getopt.noshort, "z3cliopt",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1700 =
                  let uu____1702 =
                    let uu____1704 = get_z3cliopt () in
                    FStar_List.append uu____1704 [s] in
                  FStar_All.pipe_right uu____1702
                    (FStar_List.map (fun _0_45  -> String _0_45)) in
                List uu____1700)), "[option]")), "Z3 command line options");
      (FStar_Getopt.noshort, "z3refresh",
        (FStar_Getopt.ZeroArgs ((fun uu____1714  -> Bool true))),
        "Restart Z3 after each query; useful for ensuring proof robustness");
      (FStar_Getopt.noshort, "z3rlimit",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1724 = FStar_Util.int_of_string s in Int uu____1724)),
             "[positive integer]")),
        "Set the Z3 per-query resource limit (default 5 units, taking roughtly 5s)");
      (FStar_Getopt.noshort, "z3rlimit_factor",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1734 = FStar_Util.int_of_string s in Int uu____1734)),
             "[positive integer]")),
        "Set the Z3 per-query resource limit multiplier. This is useful when, say, regenerating hints and you want to be more lax. (default 1)");
      (FStar_Getopt.noshort, "z3seed",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1744 = FStar_Util.int_of_string s in Int uu____1744)),
...
let load: Prims.unit -> Prims.string option = fun uu____2244  -> get_load ()
...
let output_dir: Prims.unit -> Prims.string option =
  fun uu____2287  -> get_odir ()
let ugly: Prims.unit -> Prims.bool = fun uu____2290  -> get_ugly ()
let print_bound_var_types: Prims.unit -> Prims.bool =
  fun uu____2293  -> get_print_bound_var_types ()
let print_effect_args: Prims.unit -> Prims.bool =
  fun uu____2296  -> get_print_effect_args ()
let print_fuels: Prims.unit -> Prims.bool =
  fun uu____2299  -> get_print_fuels ()
let print_implicits: Prims.unit -> Prims.bool =
  fun uu____2302  -> get_print_implicits ()
let print_real_names: Prims.unit -> Prims.bool =
  fun uu____2305  -> (get_prn ()) || (get_print_full_names ())
let print_universes: Prims.unit -> Prims.bool =
  fun uu____2308  -> get_print_universes ()
let print_z3_statistics: Prims.unit -> Prims.bool =
  fun uu____2311  -> get_print_z3_statistics ()
let record_hints: Prims.unit -> Prims.bool =
  fun uu____2314  -> get_record_hints ()
let reuse_hint_for: Prims.unit -> Prims.string option =
  fun uu____2318  -> get_reuse_hint_for ()
let silent: Prims.unit -> Prims.bool = fun uu____2321  -> get_silent ()
let smtencoding_elim_box: Prims.unit -> Prims.bool =
  fun uu____2324  -> get_smtencoding_elim_box ()
...
let smtencoding_l_arith_native: Prims.unit -> Prims.bool =
  fun uu____2339  ->
    let uu____2340 = get_smtencoding_l_arith_repr () in uu____2340 = "native"
let smtencoding_l_arith_default: Prims.unit -> Prims.bool =
  fun uu____2343  ->
    let uu____2344 = get_smtencoding_l_arith_repr () in
    uu____2344 = "boxwrap"
let split_cases: Prims.unit -> Prims.int =
  fun uu____2347  -> get_split_cases ()
let timing: Prims.unit -> Prims.bool = fun uu____2350  -> get_timing ()
let trace_error: Prims.unit -> Prims.bool =
  fun uu____2353  -> get_trace_error ()
let unthrottle_inductives: Prims.unit -> Prims.bool =
  fun uu____2356  -> get_unthrottle_inductives ()
let use_eq_at_higher_order: Prims.unit -> Prims.bool =
  fun uu____2359  -> get_use_eq_at_higher_order ()
let use_hints: Prims.unit -> Prims.bool = fun uu____2362  -> get_use_hints ()
let use_tactics: Prims.unit -> Prims.bool =
  fun uu____2365  -> get_use_tactics ()
let using_facts_from: Prims.unit -> Prims.string Prims.list option =
  fun uu____2370  -> get_using_facts_from ()
let verify_all: Prims.unit -> Prims.bool =
  fun uu____2373  -> get_verify_all ()
let verify_module: Prims.unit -> Prims.string Prims.list =
  fun uu____2377  -> get_verify_module ()
let warn_default_effects: Prims.unit -> Prims.bool =
  fun uu____2380  -> get_warn_default_effects ()
let z3_exe: Prims.unit -> Prims.string =
  fun uu____2383  ->
    let uu____2384 = get_smt () in
    match uu____2384 with | None  -> FStar_Platform.exe "z3" | Some s -> s
let z3_cliopt: Prims.unit -> Prims.string Prims.list =
  fun uu____2390  -> get_z3cliopt ()
let z3_refresh: Prims.unit -> Prims.bool =
  fun uu____2393  -> get_z3refresh ()
let z3_rlimit: Prims.unit -> Prims.int = fun uu____2396  -> get_z3rlimit ()
let z3_rlimit_factor: Prims.unit -> Prims.int =
  fun uu____2399  -> get_z3rlimit_factor ()
let z3_seed: Prims.unit -> Prims.int = fun uu____2402  -> get_z3seed ()
let no_positivity: Prims.unit -> Prims.bool =
  fun uu____2405  -> get_no_positivity ()
let should_extract: Prims.string -> Prims.bool =
  fun m  ->
    (let uu____2409 = no_extract m in Prims.op_Negation uu____2409) &&
      ((extract_all ()) ||
         (let uu____2410 = get_extract_module () in
          match uu____2410 with
          | [] ->
              let uu____2412 = get_extract_namespace () in
