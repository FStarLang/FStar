...
let get_use_tactics: Prims.unit -> Prims.bool =
  fun uu____759  ->
    let uu____760 = lookup_opt "no_tactics" as_bool in
    Prims.op_Negation uu____760
let get_verify_all: Prims.unit -> Prims.bool =
  fun uu____763  -> lookup_opt "verify_all" as_bool
let get_verify_module: Prims.unit -> Prims.string Prims.list =
  fun uu____767  -> lookup_opt "verify_module" (as_list as_string)
let get___temp_no_proj: Prims.unit -> Prims.string Prims.list =
  fun uu____772  -> lookup_opt "__temp_no_proj" (as_list as_string)
let get_version: Prims.unit -> Prims.bool =
  fun uu____776  -> lookup_opt "version" as_bool
let get_warn_default_effects: Prims.unit -> Prims.bool =
  fun uu____779  -> lookup_opt "warn_default_effects" as_bool
let get_z3cliopt: Prims.unit -> Prims.string Prims.list =
  fun uu____783  -> lookup_opt "z3cliopt" (as_list as_string)
let get_z3refresh: Prims.unit -> Prims.bool =
  fun uu____787  -> lookup_opt "z3refresh" as_bool
let get_z3rlimit: Prims.unit -> Prims.int =
  fun uu____790  -> lookup_opt "z3rlimit" as_int
let get_z3rlimit_factor: Prims.unit -> Prims.int =
  fun uu____793  -> lookup_opt "z3rlimit_factor" as_int
let get_z3seed: Prims.unit -> Prims.int =
  fun uu____796  -> lookup_opt "z3seed" as_int
let get_z3timeout: Prims.unit -> Prims.int =
  fun uu____799  -> lookup_opt "z3timeout" as_int
let get_no_positivity: Prims.unit -> Prims.bool =
  fun uu____802  -> lookup_opt "__no_positivity" as_bool
let dlevel: Prims.string -> debug_level_t =
  fun uu___56_805  ->
    match uu___56_805 with
    | "Low" -> Low
    | "Medium" -> Medium
    | "High" -> High
    | "Extreme" -> Extreme
...
let debug_level_geq: debug_level_t -> Prims.bool =
  fun l2  ->
    let uu____817 = get_debug_level () in
    FStar_All.pipe_right uu____817
...
let display_version: Prims.unit -> Prims.unit =
  fun uu____844  ->
    let uu____845 =
      let uu____846 = FStar_ST.read _version in
      let uu____849 = FStar_ST.read _platform in
      let uu____852 = FStar_ST.read _compiler in
      let uu____855 = FStar_ST.read _date in
      let uu____858 = FStar_ST.read _commit in
      FStar_Util.format5
        "F* %s\nplatform=%s\ncompiler=%s\ndate=%s\ncommit=%s\n" uu____846
        uu____849 uu____852 uu____855 uu____858 in
    FStar_Util.print_string uu____845
let display_usage_aux specs =
  FStar_Util.print_string "fstar.exe [options] file[s]\n";
  FStar_List.iter
    (fun uu____888  ->
       match uu____888 with
       | (uu____894,flag,p,doc) ->
           (match p with
            | FStar_Getopt.ZeroArgs ig ->
                if doc = ""
                then
                  let uu____903 =
                    let uu____904 = FStar_Util.colorize_bold flag in
                    FStar_Util.format1 "  --%s\n" uu____904 in
                  FStar_Util.print_string uu____903
                else
                  (let uu____906 =
                     let uu____907 = FStar_Util.colorize_bold flag in
                     FStar_Util.format2 "  --%s  %s\n" uu____907 doc in
                   FStar_Util.print_string uu____906)
            | FStar_Getopt.OneArg (uu____908,argname) ->
                if doc = ""
                then
                  let uu____914 =
                    let uu____915 = FStar_Util.colorize_bold flag in
                    let uu____916 = FStar_Util.colorize_bold argname in
                    FStar_Util.format2 "  --%s %s\n" uu____915 uu____916 in
                  FStar_Util.print_string uu____914
                else
                  (let uu____918 =
                     ...
                     FStar_Util.format3 "  --%s %s  %s\n" uu____919 uu____920
                       doc in
                   FStar_Util.print_string uu____918))) specs
let mk_spec:
    ...
    | (ns,name,arg,desc) ->
        let arg1 =
          ...
          | FStar_Getopt.ZeroArgs f ->
              let g uu____955 =
                let uu____956 = let uu____959 = f () in (name, uu____959) in
                set_option' uu____956 in
              FStar_Getopt.ZeroArgs g
          | FStar_Getopt.OneArg (f,d) ->
              let g x =
                let uu____970 = let uu____973 = f x in (name, uu____973) in
                set_option' uu____970 in
...
let cons_extract_module: Prims.string -> option_val =
  fun s  ->
    let uu____980 =
      let uu____982 =
        let uu____984 = get_extract_module () in (FStar_String.lowercase s)
          :: uu____984 in
...
let cons_extract_namespace: Prims.string -> option_val =
  fun s  ->
    let uu____991 =
      let uu____993 =
        let uu____995 = get_extract_namespace () in
        (FStar_String.lowercase s) :: uu____995 in
      FStar_All.pipe_right uu____993
        (FStar_List.map (fun _0_26  -> String _0_26)) in
    List uu____991
let add_extract_module: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1002 = cons_extract_module s in
    set_option "extract_module" uu____1002
let add_extract_namespace: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1006 = cons_extract_namespace s in
    set_option "extract_namespace" uu____1006
let cons_verify_module: Prims.string -> option_val =
  fun s  ->
    let uu____1010 =
      let uu____1012 =
        let uu____1014 = get_verify_module () in (FStar_String.lowercase s)
          :: uu____1014 in
...
let add_verify_module: Prims.string -> Prims.unit =
  fun s  ->
    let uu____1021 = cons_verify_module s in
    set_option "verify_module" uu____1021
let rec specs:
  Prims.unit ->
  ...
  fun uu____1031  ->
    let specs1 =
      ...
      (FStar_Getopt.noshort, "cardinality",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1060 = validate_cardinality x in String uu____1060)),
             "[off|warn|check]")),
      ...
      (FStar_Getopt.noshort, "dump_module",
        (FStar_Getopt.OneArg
           (((fun x  ->
                ...
                FStar_All.pipe_right uu____1155 (fun _0_32  -> List _0_32))),
             "[module name]")), "");
           ...
           (((fun s  ->
                let uu____1253 =
                  let uu____1255 =
                    ...
                    FStar_List.map (fun _0_34  -> String _0_34) uu____1257 in
      ...
      (FStar_Getopt.noshort, "split_cases",
        (FStar_Getopt.OneArg
           (((fun n1  ->
                let uu____1536 = FStar_Util.int_of_string n1 in
                Int uu____1536)), "[positive integer]")),
      ...
      (FStar_Getopt.noshort, "__temp_no_proj",
        (FStar_Getopt.OneArg
           (((fun x  ->
                let uu____1603 =
                  let uu____1605 =
                    let uu____1607 = get___temp_no_proj () in x :: uu____1607 in
                  FStar_All.pipe_right uu____1605
                    (FStar_List.map (fun _0_40  -> String _0_40)) in
                List uu____1603)), "[module name]")),
      ...
      (FStar_Getopt.noshort, "z3cliopt",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1635 =
                  let uu____1637 =
                    let uu____1639 = get_z3cliopt () in
                    FStar_List.append uu____1639 [s] in
                  FStar_All.pipe_right uu____1637
                    (FStar_List.map (fun _0_41  -> String _0_41)) in
                List uu____1635)), "[option]")), "Z3 command line options");
      ...
      (FStar_Getopt.noshort, "z3rlimit",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1659 = FStar_Util.int_of_string s in Int uu____1659)),
             "[positive integer]")),
        "Set the Z3 per-query resource limit (default 5 units, taking roughtly 5s)");
      (FStar_Getopt.noshort, "z3rlimit_factor",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1669 = FStar_Util.int_of_string s in Int uu____1669)),
             "[positive integer]")),
        "Set the Z3 per-query resource limit multiplier. This is useful when, say, regenerating hints and you want to be more lax. (default 1)");
      (FStar_Getopt.noshort, "z3seed",
        (FStar_Getopt.OneArg
           (((fun s  ->
                let uu____1679 = FStar_Util.int_of_string s in Int uu____1679)),
      ...
      (FStar_Getopt.noshort, "z3timeout",
        (FStar_Getopt.OneArg
           (((fun s  ->
                ...
                (let uu____1690 = FStar_Util.int_of_string s in
                 Int uu____1690))), "[positive integer]")),
        "Set the Z3 per-query (soft) timeout to [t] seconds (default 5)");
      (FStar_Getopt.noshort, "__no_positivity",
        (FStar_Getopt.ZeroArgs ((fun uu____1697  -> Bool true))),
        "Don't check positivity of inductive types")] in
    let uu____1703 = FStar_List.map mk_spec specs1 in
    ('h', "help",
      (FStar_Getopt.ZeroArgs
         (fun x  ->
            display_usage_aux specs1; FStar_All.exit (Prims.parse_int "0"))),
      "Display this information") :: uu____1703
  ...
  fun s  ->
    ...
    | uu____1724 ->
        (FStar_Util.print_string "Wrong argument to codegen flag\n";
         (let uu____1727 = specs () in display_usage_aux uu____1727);
  ...
  fun x  ->
    ...
    | uu____1735 ->
        (FStar_Util.print_string "Wrong argument to cardinality flag\n";
         (let uu____1738 = specs () in display_usage_aux uu____1738);
         FStar_All.exit (Prims.parse_int "1"))
...
let docs: Prims.unit -> (Prims.string* Prims.string) Prims.list =
  fun uu____1752  ->
    let uu____1753 = specs () in
    FStar_List.map
      (fun uu____1767  ->
         match uu____1767 with
         | (uu____1775,name,uu____1777,doc) -> (name, doc)) uu____1753
let settable: Prims.string -> Prims.bool =
  fun uu___57_1783  ->
    match uu___57_1783 with
    | "admit_smt_queries"
      |"cardinality"
       |"debug"
        |"debug_level"
  ...
  FStar_All.pipe_right all_specs
    (FStar_List.filter
       (fun uu____1807  ->
          ...
          | (uu____1813,x,uu____1815,uu____1816) -> settable x))
  ...
  FStar_All.pipe_right all_specs
    (FStar_List.filter
       (fun uu____1837  ->
          ...
          | (uu____1843,x,uu____1845,uu____1846) -> resettable x))
let display_usage: Prims.unit -> Prims.unit =
  fun uu____1851  ->
    let uu____1852 = specs () in display_usage_aux uu____1852
let fstar_home: Prims.unit -> Prims.string =
  fun uu____1861  ->
    let uu____1862 = get_fstar_home () in
...
let set_options: options -> Prims.string -> FStar_Getopt.parse_cmdline_res =
  fun o  ->
    fun s  ->
      ...
      FStar_Getopt.parse_string specs1 (fun uu____1887  -> ()) s
let file_list_: Prims.string Prims.list FStar_ST.ref = FStar_Util.mk_ref []
let parse_cmd_line:
  Prims.unit -> (FStar_Getopt.parse_cmdline_res* Prims.string Prims.list) =
  fun uu____1898  ->
    let res =
      let uu____1900 = specs () in
      FStar_Getopt.parse_cmdline uu____1900
        (fun i  ->
           let uu____1903 =
             let uu____1905 = FStar_ST.read file_list_ in
             FStar_List.append uu____1905 [i] in
           FStar_ST.write file_list_ uu____1903) in
    let uu____1913 =
      let uu____1915 = FStar_ST.read file_list_ in
      FStar_List.map FStar_Common.try_convert_file_name_to_mixed uu____1915 in
    (res, uu____1913)
let file_list: Prims.unit -> Prims.string Prims.list =
  fun uu____1924  -> FStar_ST.read file_list_
let restore_cmd_line_options: Prims.bool -> FStar_Getopt.parse_cmdline_res =
  fun should_clear  ->
    let old_verify_module = get_verify_module () in
    if should_clear then clear () else init ();
    (let r =
       let uu____1936 = specs () in
       FStar_Getopt.parse_cmdline uu____1936 (fun x  -> ()) in
     (let uu____1940 =
        let uu____1943 =
          let uu____1944 =
            FStar_List.map (fun _0_42  -> String _0_42) old_verify_module in
          List uu____1944 in
        ("verify_module", uu____1943) in
      set_option' uu____1940);
     r)
let should_verify: Prims.string -> Prims.bool =
  fun m  ->
    let uu____1949 = get_lax () in
    if uu____1949
    then false
    else
      (let uu____1951 = get_verify_all () in
       if uu____1951
       then true
       else
         (let uu____1953 = get_verify_module () in
          match uu____1953 with
          | [] ->
              let uu____1955 = file_list () in
              FStar_List.existsML
                (fun f  ->
                   ...
                   let f2 =
                     let uu____1960 =
                       let uu____1961 =
                         let uu____1962 =
                           let uu____1963 = FStar_Util.get_file_extension f1 in
                           FStar_String.length uu____1963 in
                         (FStar_String.length f1) - uu____1962 in
                       uu____1961 - (Prims.parse_int "1") in
                     FStar_String.substring f1 (Prims.parse_int "0")
                       uu____1960 in
                   (FStar_String.lowercase f2) = m) uu____1955
          | l -> FStar_List.contains (FStar_String.lowercase m) l))
let dont_gen_projectors: Prims.string -> Prims.bool =
  fun m  ->
    let uu____1973 = get___temp_no_proj () in
    FStar_List.contains m uu____1973
let should_print_message: Prims.string -> Prims.bool =
  fun m  ->
    let uu____1978 = should_verify m in
    if uu____1978 then m <> "Prims" else false
let include_path: Prims.unit -> Prims.string Prims.list =
  fun uu____1983  ->
    let uu____1984 = get_no_default_includes () in
    if uu____1984
    then get_include ()
    else
      (let h = fstar_home () in
       ...
       let uu____1990 =
         ...
         FStar_All.pipe_right uu____1992
           (FStar_List.filter FStar_Util.file_exists) in
       let uu____1999 =
         let uu____2001 = get_include () in
         FStar_List.append uu____2001 ["."] in
       FStar_List.append uu____1990 uu____1999)
let find_file: Prims.string -> Prims.string Prims.option =
  fun filename  ->
    let uu____2007 = FStar_Util.is_path_absolute filename in
    if uu____2007
    then (if FStar_Util.file_exists filename then Some filename else None)
    else
      (let uu____2012 =
         let uu____2014 = include_path () in FStar_List.rev uu____2014 in
       FStar_Util.find_map uu____2012
         (fun p  ->
            let path = FStar_Util.join_paths p filename in
            if FStar_Util.file_exists path then Some path else None))
let prims: Prims.unit -> Prims.string =
  fun uu____2022  ->
    let uu____2023 = get_prims () in
    match uu____2023 with
    | None  ->
        let filename = "prims.fst" in
        let uu____2026 = find_file filename in
             ...
             let uu____2029 =
               let uu____2030 =
                 FStar_Util.format1
                   ...
                   filename in
               FStar_Util.Failure uu____2030 in
             Prims.raise uu____2029)
    | Some x -> x
let prims_basename: Prims.unit -> Prims.string =
  fun uu____2034  ->
    let uu____2035 = prims () in FStar_Util.basename uu____2035
let prepend_output_dir: Prims.string -> Prims.string =
  fun fname  ->
    let uu____2039 = get_odir () in
    match uu____2039 with
    | None  -> fname
    | Some x -> Prims.strcat x (Prims.strcat "/" fname)
let __temp_no_proj: Prims.string -> Prims.bool =
  fun s  ->
    let uu____2045 = get___temp_no_proj () in
    FStar_All.pipe_right uu____2045 (FStar_List.contains s)
let admit_smt_queries: Prims.unit -> Prims.bool =
  fun uu____2050  -> get_admit_smt_queries ()
let check_cardinality: Prims.unit -> Prims.bool =
  fun uu____2053  ->
    let uu____2054 = get_cardinality () in uu____2054 = "check"
let check_hints: Prims.unit -> Prims.bool =
  fun uu____2057  -> get_check_hints ()
let codegen: Prims.unit -> Prims.string Prims.option =
  fun uu____2061  -> get_codegen ()
let codegen_libs: Prims.unit -> Prims.string Prims.list Prims.list =
  fun uu____2066  ->
    let uu____2067 = get_codegen_lib () in
    FStar_All.pipe_right uu____2067
      (FStar_List.map (fun x  -> FStar_Util.split x "."))
let debug_any: Prims.unit -> Prims.bool =
  fun uu____2076  -> let uu____2077 = get_debug () in uu____2077 <> []
let debug_at_level: Prims.string -> debug_level_t -> Prims.bool =
  fun modul  ->
    fun level  ->
      ((modul = "") ||
         (let uu____2086 = get_debug () in
          FStar_All.pipe_right uu____2086 (FStar_List.contains modul)))
        && (debug_level_geq level)
let dep: Prims.unit -> Prims.string Prims.option =
  fun uu____2092  -> get_dep ()
let detail_errors: Prims.unit -> Prims.bool =
  fun uu____2095  -> get_detail_errors ()
let doc: Prims.unit -> Prims.bool = fun uu____2098  -> get_doc ()
let dump_module: Prims.string -> Prims.bool =
  fun s  ->
    let uu____2102 = get_dump_module () in
    FStar_All.pipe_right uu____2102 (FStar_List.contains s)
let eager_inference: Prims.unit -> Prims.bool =
  fun uu____2107  -> get_eager_inference ()
let explicit_deps: Prims.unit -> Prims.bool =
  fun uu____2110  -> get_explicit_deps ()
let extract_all: Prims.unit -> Prims.bool =
  fun uu____2113  -> get_extract_all ()
let fs_typ_app: Prims.string -> Prims.bool =
  fun filename  ->
    let uu____2117 = FStar_ST.read light_off_files in
    FStar_List.contains filename uu____2117
let full_context_dependency: Prims.unit -> Prims.bool =
  fun uu____2124  -> true
let hide_genident_nums: Prims.unit -> Prims.bool =
  fun uu____2127  -> get_hide_genident_nums ()
let hide_uvar_nums: Prims.unit -> Prims.bool =
  fun uu____2130  -> get_hide_uvar_nums ()
let hint_info: Prims.unit -> Prims.bool = fun uu____2133  -> get_hint_info ()
let ide: Prims.unit -> Prims.bool = fun uu____2136  -> get_ide ()
let indent: Prims.unit -> Prims.bool = fun uu____2139  -> get_indent ()
let initial_fuel: Prims.unit -> Prims.int =
  fun uu____2142  ->
    let uu____2143 = get_initial_fuel () in
    let uu____2144 = get_max_fuel () in Prims.min uu____2143 uu____2144
let initial_ifuel: Prims.unit -> Prims.int =
  fun uu____2147  ->
    let uu____2148 = get_initial_ifuel () in
    let uu____2149 = get_max_ifuel () in Prims.min uu____2148 uu____2149
let inline_arith: Prims.unit -> Prims.bool =
  fun uu____2152  -> get_inline_arith ()
let interactive: Prims.unit -> Prims.bool =
  fun uu____2155  -> (get_in ()) || (get_ide ())
let lax: Prims.unit -> Prims.bool = fun uu____2158  -> get_lax ()
let legacy_interactive: Prims.unit -> Prims.bool =
  fun uu____2161  -> get_in ()
let log_queries: Prims.unit -> Prims.bool =
  fun uu____2164  -> get_log_queries ()
let log_types: Prims.unit -> Prims.bool = fun uu____2167  -> get_log_types ()
let max_fuel: Prims.unit -> Prims.int = fun uu____2170  -> get_max_fuel ()
let max_ifuel: Prims.unit -> Prims.int = fun uu____2173  -> get_max_ifuel ()
let min_fuel: Prims.unit -> Prims.int = fun uu____2176  -> get_min_fuel ()
let ml_ish: Prims.unit -> Prims.bool = fun uu____2179  -> get_MLish ()
let set_ml_ish: Prims.unit -> Prims.unit =
  fun uu____2182  -> set_option "MLish" (Bool true)
let n_cores: Prims.unit -> Prims.int = fun uu____2185  -> get_n_cores ()
let no_default_includes: Prims.unit -> Prims.bool =
  fun uu____2188  -> get_no_default_includes ()
let no_extract: Prims.string -> Prims.bool =
  fun s  ->
    let uu____2192 = get_no_extract () in
    FStar_All.pipe_right uu____2192 (FStar_List.contains s)
let no_location_info: Prims.unit -> Prims.bool =
  fun uu____2197  -> get_no_location_info ()
let norm_then_print: Prims.unit -> Prims.bool =
  fun uu____2200  ->
    let uu____2201 = get_print_before_norm () in uu____2201 = false
let output_dir: Prims.unit -> Prims.string Prims.option =
  fun uu____2205  -> get_odir ()
let print_bound_var_types: Prims.unit -> Prims.bool =
  fun uu____2208  -> get_print_bound_var_types ()
let print_effect_args: Prims.unit -> Prims.bool =
  fun uu____2211  -> get_print_effect_args ()
let print_fuels: Prims.unit -> Prims.bool =
  fun uu____2214  -> get_print_fuels ()
let print_implicits: Prims.unit -> Prims.bool =
  fun uu____2217  -> get_print_implicits ()
let print_real_names: Prims.unit -> Prims.bool =
  fun uu____2220  -> (get_prn ()) || (get_print_full_names ())
let print_universes: Prims.unit -> Prims.bool =
  fun uu____2223  -> get_print_universes ()
let print_z3_statistics: Prims.unit -> Prims.bool =
  fun uu____2226  -> get_print_z3_statistics ()
let record_hints: Prims.unit -> Prims.bool =
  fun uu____2229  -> get_record_hints ()
let reuse_hint_for: Prims.unit -> Prims.string Prims.option =
  fun uu____2233  -> get_reuse_hint_for ()
let silent: Prims.unit -> Prims.bool = fun uu____2236  -> get_silent ()
let split_cases: Prims.unit -> Prims.int =
  fun uu____2239  -> get_split_cases ()
let timing: Prims.unit -> Prims.bool = fun uu____2242  -> get_timing ()
let trace_error: Prims.unit -> Prims.bool =
  fun uu____2245  -> get_trace_error ()
let unthrottle_inductives: Prims.unit -> Prims.bool =
  fun uu____2248  -> get_unthrottle_inductives ()
let use_eq_at_higher_order: Prims.unit -> Prims.bool =
  fun uu____2251  -> get_use_eq_at_higher_order ()
let use_hints: Prims.unit -> Prims.bool = fun uu____2254  -> get_use_hints ()
let use_tactics: Prims.unit -> Prims.bool =
  fun uu____2257  -> get_use_tactics ()
let verify_all: Prims.unit -> Prims.bool =
  fun uu____2260  -> get_verify_all ()
let verify_module: Prims.unit -> Prims.string Prims.list =
  fun uu____2264  -> get_verify_module ()
let warn_cardinality: Prims.unit -> Prims.bool =
  fun uu____2267  ->
    let uu____2268 = get_cardinality () in uu____2268 = "warn"
let warn_default_effects: Prims.unit -> Prims.bool =
  fun uu____2271  -> get_warn_default_effects ()
let warn_top_level_effects: Prims.unit -> Prims.bool =
  fun uu____2274  -> get_warn_top_level_effects ()
let z3_exe: Prims.unit -> Prims.string =
  fun uu____2277  ->
    let uu____2278 = get_smt () in
    match uu____2278 with | None  -> FStar_Platform.exe "z3" | Some s -> s
let z3_cliopt: Prims.unit -> Prims.string Prims.list =
  fun uu____2284  -> get_z3cliopt ()
let z3_refresh: Prims.unit -> Prims.bool =
  fun uu____2287  -> get_z3refresh ()
let z3_rlimit: Prims.unit -> Prims.int = fun uu____2290  -> get_z3rlimit ()
let z3_rlimit_factor: Prims.unit -> Prims.int =
  fun uu____2293  -> get_z3rlimit_factor ()
let z3_seed: Prims.unit -> Prims.int = fun uu____2296  -> get_z3seed ()
let z3_timeout: Prims.unit -> Prims.int = fun uu____2299  -> get_z3timeout ()
let no_positivity: Prims.unit -> Prims.bool =
  fun uu____2302  -> get_no_positivity ()
let should_extract: Prims.string -> Prims.bool =
  fun m  ->
    (let uu____2306 = no_extract m in Prims.op_Negation uu____2306) &&
      ((extract_all ()) ||
         (let uu____2307 = get_extract_module () in
          match uu____2307 with
          | [] ->
              let uu____2309 = get_extract_namespace () in
