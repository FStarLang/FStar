...
let as_bool : option_val  ->  Prims.bool = (fun uu___81_467 -> (match (uu___81_467) with
| Bool (b) -> begin
b
end
| uu____471 -> begin
...
let as_int : option_val  ->  Prims.int = (fun uu___82_480 -> (match (uu___82_480) with
| Int (b) -> begin
b
end
| uu____484 -> begin
...
let as_string : option_val  ->  Prims.string = (fun uu___83_493 -> (match (uu___83_493) with
| String (b) -> begin
b
end
| Path (b) -> begin
...
let as_list' : option_val  ->  option_val Prims.list = (fun uu___84_509 -> (match (uu___84_509) with
| List (ts) -> begin
ts
end
| uu____515 -> begin
...
let as_option : 'Auu____558 . (option_val  ->  'Auu____558)  ->  option_val  ->  'Auu____558 FStar_Pervasives_Native.option = (fun as_t uu___85_573 -> (match (uu___85_573) with
| Unset -> begin
...
let as_comma_string_list : option_val  ->  Prims.string Prims.list = (fun uu___86_586 -> (match (uu___86_586) with
| List (ls) -> begin
(
...
let get_profile : unit  ->  Prims.bool = (fun uu____2740 -> (lookup_opt "profile" as_bool))


let get_protect_top_level_axioms : unit  ->  Prims.bool = (fun uu____2749 -> (lookup_opt "protect_top_level_axioms" as_bool))


let get_initial_fuel : unit  ->  Prims.int = (fun uu____2758 -> (lookup_opt "initial_fuel" as_int))


let get_initial_ifuel : unit  ->  Prims.int = (fun uu____2767 -> (lookup_opt "initial_ifuel" as_int))


let get_keep_query_captions : unit  ->  Prims.bool = (fun uu____2776 -> (lookup_opt "keep_query_captions" as_bool))


let get_lax : unit  ->  Prims.bool = (fun uu____2785 -> (lookup_opt "lax" as_bool))


let get_load : unit  ->  Prims.string Prims.list = (fun uu____2796 -> (lookup_opt "load" (as_list as_string)))


let get_log_queries : unit  ->  Prims.bool = (fun uu____2808 -> (lookup_opt "log_queries" as_bool))


let get_log_types : unit  ->  Prims.bool = (fun uu____2817 -> (lookup_opt "log_types" as_bool))


let get_max_fuel : unit  ->  Prims.int = (fun uu____2826 -> (lookup_opt "max_fuel" as_int))


let get_max_ifuel : unit  ->  Prims.int = (fun uu____2835 -> (lookup_opt "max_ifuel" as_int))


let get_min_fuel : unit  ->  Prims.int = (fun uu____2844 -> (lookup_opt "min_fuel" as_int))


let get_MLish : unit  ->  Prims.bool = (fun uu____2853 -> (lookup_opt "MLish" as_bool))


let get_n_cores : unit  ->  Prims.int = (fun uu____2862 -> (lookup_opt "n_cores" as_int))


let get_no_default_includes : unit  ->  Prims.bool = (fun uu____2871 -> (lookup_opt "no_default_includes" as_bool))


let get_no_extract : unit  ->  Prims.string Prims.list = (fun uu____2882 -> (lookup_opt "no_extract" (as_list as_string)))


let get_no_location_info : unit  ->  Prims.bool = (fun uu____2894 -> (lookup_opt "no_location_info" as_bool))


let get_no_plugins : unit  ->  Prims.bool = (fun uu____2903 -> (lookup_opt "no_plugins" as_bool))


let get_no_smt : unit  ->  Prims.bool = (fun uu____2912 -> (lookup_opt "no_smt" as_bool))


let get_normalize_pure_terms_for_extraction : unit  ->  Prims.bool = (fun uu____2921 -> (lookup_opt "normalize_pure_terms_for_extraction" as_bool))


let get_odir : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____2932 -> (lookup_opt "odir" (as_option as_string)))


let get_ugly : unit  ->  Prims.bool = (fun uu____2944 -> (lookup_opt "ugly" as_bool))


let get_prims : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____2955 -> (lookup_opt "prims" (as_option as_string)))


let get_print_bound_var_types : unit  ->  Prims.bool = (fun uu____2967 -> (lookup_opt "print_bound_var_types" as_bool))


let get_print_effect_args : unit  ->  Prims.bool = (fun uu____2976 -> (lookup_opt "print_effect_args" as_bool))


let get_print_full_names : unit  ->  Prims.bool = (fun uu____2985 -> (lookup_opt "print_full_names" as_bool))


let get_print_implicits : unit  ->  Prims.bool = (fun uu____2994 -> (lookup_opt "print_implicits" as_bool))


let get_print_universes : unit  ->  Prims.bool = (fun uu____3003 -> (lookup_opt "print_universes" as_bool))


let get_print_z3_statistics : unit  ->  Prims.bool = (fun uu____3012 -> (lookup_opt "print_z3_statistics" as_bool))


let get_prn : unit  ->  Prims.bool = (fun uu____3021 -> (lookup_opt "prn" as_bool))


let get_query_stats : unit  ->  Prims.bool = (fun uu____3030 -> (lookup_opt "query_stats" as_bool))


let get_record_hints : unit  ->  Prims.bool = (fun uu____3039 -> (lookup_opt "record_hints" as_bool))


let get_reuse_hint_for : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____3050 -> (lookup_opt "reuse_hint_for" (as_option as_string)))


let get_silent : unit  ->  Prims.bool = (fun uu____3062 -> (lookup_opt "silent" as_bool))


let get_smt : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____3073 -> (lookup_opt "smt" (as_option as_string)))


let get_smtencoding_elim_box : unit  ->  Prims.bool = (fun uu____3085 -> (lookup_opt "smtencoding.elim_box" as_bool))


let get_smtencoding_nl_arith_repr : unit  ->  Prims.string = (fun uu____3094 -> (lookup_opt "smtencoding.nl_arith_repr" as_string))


let get_smtencoding_l_arith_repr : unit  ->  Prims.string = (fun uu____3103 -> (lookup_opt "smtencoding.l_arith_repr" as_string))


let get_tactic_raw_binders : unit  ->  Prims.bool = (fun uu____3112 -> (lookup_opt "tactic_raw_binders" as_bool))


let get_tactics_failhard : unit  ->  Prims.bool = (fun uu____3121 -> (lookup_opt "tactics_failhard" as_bool))


let get_tactics_info : unit  ->  Prims.bool = (fun uu____3130 -> (lookup_opt "tactics_info" as_bool))


let get_tactic_trace : unit  ->  Prims.bool = (fun uu____3139 -> (lookup_opt "tactic_trace" as_bool))


let get_tactic_trace_d : unit  ->  Prims.int = (fun uu____3148 -> (lookup_opt "tactic_trace_d" as_int))


let get_tactics_nbe : unit  ->  Prims.bool = (fun uu____3157 -> (lookup_opt "__tactics_nbe" as_bool))


let get_tcnorm : unit  ->  Prims.bool = (fun uu____3166 -> (lookup_opt "tcnorm" as_bool))


let get_timing : unit  ->  Prims.bool = (fun uu____3175 -> (lookup_opt "timing" as_bool))


let get_trace_error : unit  ->  Prims.bool = (fun uu____3184 -> (lookup_opt "trace_error" as_bool))


let get_unthrottle_inductives : unit  ->  Prims.bool = (fun uu____3193 -> (lookup_opt "unthrottle_inductives" as_bool))


let get_unsafe_tactic_exec : unit  ->  Prims.bool = (fun uu____3202 -> (lookup_opt "unsafe_tactic_exec" as_bool))


let get_use_eq_at_higher_order : unit  ->  Prims.bool = (fun uu____3211 -> (lookup_opt "use_eq_at_higher_order" as_bool))


let get_use_hints : unit  ->  Prims.bool = (fun uu____3220 -> (lookup_opt "use_hints" as_bool))


let get_use_hint_hashes : unit  ->  Prims.bool = (fun uu____3229 -> (lookup_opt "use_hint_hashes" as_bool))


let get_use_native_tactics : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____3240 -> (lookup_opt "use_native_tactics" (as_option as_string)))


let get_use_tactics : unit  ->  Prims.bool = (fun uu____3252 -> (

let uu____3253 = (lookup_opt "no_tactics" as_bool)
in (not (uu____3253))))


let get_using_facts_from : unit  ->  Prims.string Prims.list FStar_Pervasives_Native.option = (fun uu____3267 -> (lookup_opt "using_facts_from" (as_option (as_list as_string))))


let get_vcgen_optimize_bind_as_seq : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____3286 -> (lookup_opt "vcgen.optimize_bind_as_seq" (as_option as_string)))


let get_verify_module : unit  ->  Prims.string Prims.list = (fun uu____3300 -> (lookup_opt "verify_module" (as_list as_string)))


let get___temp_no_proj : unit  ->  Prims.string Prims.list = (fun uu____3314 -> (lookup_opt "__temp_no_proj" (as_list as_string)))


let get_version : unit  ->  Prims.bool = (fun uu____3326 -> (lookup_opt "version" as_bool))


let get_warn_default_effects : unit  ->  Prims.bool = (fun uu____3335 -> (lookup_opt "warn_default_effects" as_bool))


let get_z3cliopt : unit  ->  Prims.string Prims.list = (fun uu____3346 -> (lookup_opt "z3cliopt" (as_list as_string)))


let get_z3refresh : unit  ->  Prims.bool = (fun uu____3358 -> (lookup_opt "z3refresh" as_bool))


let get_z3rlimit : unit  ->  Prims.int = (fun uu____3367 -> (lookup_opt "z3rlimit" as_int))


let get_z3rlimit_factor : unit  ->  Prims.int = (fun uu____3376 -> (lookup_opt "z3rlimit_factor" as_int))


let get_z3seed : unit  ->  Prims.int = (fun uu____3385 -> (lookup_opt "z3seed" as_int))


let get_use_two_phase_tc : unit  ->  Prims.bool = (fun uu____3394 -> (lookup_opt "use_two_phase_tc" as_bool))


let get_no_positivity : unit  ->  Prims.bool = (fun uu____3403 -> (lookup_opt "__no_positivity" as_bool))


let get_ml_no_eta_expand_coertions : unit  ->  Prims.bool = (fun uu____3412 -> (lookup_opt "__ml_no_eta_expand_coertions" as_bool))


let get_warn_error : unit  ->  Prims.string = (fun uu____3421 -> (lookup_opt "warn_error" as_string))


let get_use_extracted_interfaces : unit  ->  Prims.bool = (fun uu____3430 -> (lookup_opt "use_extracted_interfaces" as_bool))


let get_use_nbe : unit  ->  Prims.bool = (fun uu____3439 -> (lookup_opt "use_nbe" as_bool))


let dlevel : Prims.string  ->  debug_level_t = (fun uu___87_3448 -> (match (uu___87_3448) with
| "Low" -> begin
Low
end
| "Medium" -> begin
...
let rec parse_opt_val : Prims.string  ->  opt_type  ->  Prims.string  ->  option_val = (fun opt_name typ str_val -> (FStar_All.try_with (fun uu___92_4580 -> (match (()) with
| () -> begin
(match (typ) with
| Const (c) -> begin
c
...
end)) (fun uu___91_4655 -> (match (uu___91_4655) with
...
let uu____5637 = (mk_bool true)
in Const (uu____5637))
in ((FStar_Getopt.noshort), ("profile"), (uu____5636), ("Prints timing information for various operations in the compiler")))
in (
...
in Const (uu____5727))
in ((FStar_Getopt.noshort), ("lax"), (uu____5726), ("Run the lax-type checker only (admit all verification conditions)")))
in (
...
let uu____5775 = (mk_bool true)
in Const (uu____5775))
in ((FStar_Getopt.noshort), ("log_types"), (uu____5774), ("Print types computed for data/val/let-bindings")))
in (
...
let uu____5809 = (mk_bool true)
in Const (uu____5809))
in ((FStar_Getopt.noshort), ("log_queries"), (uu____5808), ("Log the Z3 queries in several queries-*.smt2 files, as we go")))
in (
...
in Const (uu____5885))
in ((FStar_Getopt.noshort), ("MLish"), (uu____5884), ("Trigger various specializations for compiling the F* compiler itself (not meant for user code)")))
in (
...
in Const (uu____5933))
in ((FStar_Getopt.noshort), ("no_default_includes"), (uu____5932), ("Ignore the default module search paths")))
in (
...
let uu____5981 = (mk_bool true)
in Const (uu____5981))
in ((FStar_Getopt.noshort), ("no_location_info"), (uu____5980), ("Suppress location information in the generated OCaml output (only relevant with --codegen OCaml)")))
in (
...
let uu____6015 = (mk_bool true)
in Const (uu____6015))
in ((FStar_Getopt.noshort), ("no_smt"), (uu____6014), ("Do not send any queries to the SMT solver, and fail on them instead")))
in (
...
let uu____6049 = (mk_bool true)
in Const (uu____6049))
in ((FStar_Getopt.noshort), ("normalize_pure_terms_for_extraction"), (uu____6048), ("Extract top-level pure terms after normalizing them. This can lead to very large code, but can result in more partial evaluation and compile-time specialization.")))
in (
...
let uu____6111 = (mk_bool true)
in Const (uu____6111))
in ((FStar_Getopt.noshort), ("print_bound_var_types"), (uu____6110), ("Print the types of bound variables")))
in (
...
let uu____6145 = (mk_bool true)
in Const (uu____6145))
in ((FStar_Getopt.noshort), ("print_effect_args"), (uu____6144), ("Print inferred predicate transformers for all computation types")))
in (
...
let uu____6179 = (mk_bool true)
in Const (uu____6179))
in ((FStar_Getopt.noshort), ("print_full_names"), (uu____6178), ("Print full names of variables")))
in (
...
let uu____6213 = (mk_bool true)
in Const (uu____6213))
in ((FStar_Getopt.noshort), ("print_implicits"), (uu____6212), ("Print implicit arguments")))
in (
...
let uu____6247 = (mk_bool true)
in Const (uu____6247))
in ((FStar_Getopt.noshort), ("print_universes"), (uu____6246), ("Print universes")))
in (
...
let uu____6281 = (mk_bool true)
in Const (uu____6281))
in ((FStar_Getopt.noshort), ("print_z3_statistics"), (uu____6280), ("Print Z3 statistics for each SMT query (details such as relevant modules, facts, etc. for each proof)")))
in (
...
let uu____6315 = (mk_bool true)
in Const (uu____6315))
in ((FStar_Getopt.noshort), ("prn"), (uu____6314), ("Print full names (deprecated; use --print_full_names instead)")))
in (
...
let uu____6349 = (mk_bool true)
in Const (uu____6349))
in ((FStar_Getopt.noshort), ("query_stats"), (uu____6348), ("Print SMT query statistics")))
in (
...
in Const (uu____6383))
in ((FStar_Getopt.noshort), ("record_hints"), (uu____6382), ("Record a database of hints for efficient proof replay")))
in (
...
let uu____6431 = (mk_bool true)
in Const (uu____6431))
in ((FStar_Getopt.noshort), ("silent"), (uu____6430), ("Disable all non-critical output")))
in (
...
let uu____6521 = (mk_bool true)
in Const (uu____6521))
in ((FStar_Getopt.noshort), ("tactic_raw_binders"), (uu____6520), ("Do not use the lexical scope of tactics to improve binder names")))
in (
...
let uu____6555 = (mk_bool true)
in Const (uu____6555))
in ((FStar_Getopt.noshort), ("tactics_failhard"), (uu____6554), ("Do not recover from metaprogramming errors, and abort if one occurs")))
in (
...
let uu____6589 = (mk_bool true)
in Const (uu____6589))
in ((FStar_Getopt.noshort), ("tactics_info"), (uu____6588), ("Print some rough information on tactics, such as the time they take to run")))
in (
...
in Const (uu____6623))
in ((FStar_Getopt.noshort), ("tactic_trace"), (uu____6622), ("Print a depth-indexed trace of tactic execution (Warning: very verbose)")))
in (
...
in Const (uu____6671))
in ((FStar_Getopt.noshort), ("__tactics_nbe"), (uu____6670), ("Use NBE to evaluate metaprograms (experimental)")))
in (
...
let uu____6719 = (mk_bool true)
in Const (uu____6719))
in ((FStar_Getopt.noshort), ("timing"), (uu____6718), ("Print the time it takes to verify each top-level definition")))
in (
...
let uu____6753 = (mk_bool true)
in Const (uu____6753))
in ((FStar_Getopt.noshort), ("trace_error"), (uu____6752), ("Don\'t print an error message; show an exception trace instead")))
in (
...
let uu____6787 = (mk_bool true)
in Const (uu____6787))
in ((FStar_Getopt.noshort), ("ugly"), (uu____6786), ("Emit output formatted for debugging")))
in (
...
let uu____6821 = (mk_bool true)
in Const (uu____6821))
in ((FStar_Getopt.noshort), ("unthrottle_inductives"), (uu____6820), ("Let the SMT solver unfold inductive types to arbitrary depths (may affect verifier performance)")))
in (
...
let uu____6855 = (mk_bool true)
in Const (uu____6855))
in ((FStar_Getopt.noshort), ("unsafe_tactic_exec"), (uu____6854), ("Allow tactics to run external processes. WARNING: checking an untrusted F* file while using this option can have disastrous effects.")))
in (
...
let uu____6889 = (mk_bool true)
in Const (uu____6889))
in ((FStar_Getopt.noshort), ("use_eq_at_higher_order"), (uu____6888), ("Use equality constraints when comparing higher-order types (Temporary)")))
in (
...
let uu____6923 = (mk_bool true)
in Const (uu____6923))
in ((FStar_Getopt.noshort), ("use_hints"), (uu____6922), ("Use a previously recorded hints database for proof replay")))
in (
...
in Const (uu____6957))
in ((FStar_Getopt.noshort), ("use_hint_hashes"), (uu____6956), ("Admit queries if their hash matches the hash recorded in the hints database")))
in (
...
let uu____7005 = (mk_bool true)
in Const (uu____7005))
in ((FStar_Getopt.noshort), ("no_plugins"), (uu____7004), ("Do not run plugins natively and interpret them as usual instead")))
in (
...
let uu____7039 = (mk_bool true)
in Const (uu____7039))
in ((FStar_Getopt.noshort), ("no_tactics"), (uu____7038), ("Do not run the tactic engine before discharging a VC")))
in (
...
let uu____7115 = (mk_bool true)
in Const (uu____7115))
in ((FStar_Getopt.noshort), ("__temp_fast_implicits"), (uu____7114), ("Don\'t use this option yet")))
in (
...
let uu____7158 = (mk_bool true)
in Const (uu____7158))
in (((fun uu____7165 -> ((display_version ());
(FStar_All.exit (Prims.parse_int "0"));
))), (uu____7157)))
in WithSideEffect (uu____7149))
in ((118 (*v*)), ("version"), (uu____7148), ("Display version number")))
in (
...
in Const (uu____7201))
in ((FStar_Getopt.noshort), ("warn_default_effects"), (uu____7200), ("Warn when (a -> b) is desugared to (a -> Tot b)")))
in (
...
let uu____7249 = (mk_bool true)
in Const (uu____7249))
in ((FStar_Getopt.noshort), ("z3refresh"), (uu____7248), ("Restart Z3 after each query; useful for ensuring proof robustness")))
in (
...
let uu____7339 = (mk_bool true)
in Const (uu____7339))
in ((FStar_Getopt.noshort), ("__no_positivity"), (uu____7338), ("Don\'t check positivity of inductive types")))
in (
...
let uu____7373 = (mk_bool true)
in Const (uu____7373))
in ((FStar_Getopt.noshort), ("__ml_no_eta_expand_coertions"), (uu____7372), ("Do not eta-expand coertions in generated OCaml")))
in (
...
let uu____7458 = (mk_bool true)
in Const (uu____7458))
in (((fun uu____7464 -> (FStar_ST.op_Colon_Equals debug_embedding true))), (uu____7457)))
in WithSideEffect (uu____7449))
in ((FStar_Getopt.noshort), ("__debug_embedding"), (uu____7448), ("Debug messages for embeddings/unembeddings of natively compiled terms")))
in (
...
let uu____7528 = (mk_bool true)
in Const (uu____7528))
in (((fun uu____7534 -> (FStar_ST.op_Colon_Equals eager_embedding true))), (uu____7527)))
in WithSideEffect (uu____7519))
in ((FStar_Getopt.noshort), ("eager_embedding"), (uu____7518), ("Eagerly embed and unembed terms to primitive operations and plugins: not recommended except for benchmarking")))
in (
...
let uu____7598 = (mk_bool true)
in Const (uu____7598))
in (((fun uu____7605 -> ((

let uu____7607 = (specs ())
in (display_usage_aux uu____7607));
(FStar_All.exit (Prims.parse_int "0"));
))), (uu____7597)))
in WithSideEffect (uu____7589))
in ((104 (*h*)), ("help"), (uu____7588), ("Display this information")))
in (uu____7576)::[])
in (uu____7506)::uu____7562))
in (uu____7436)::uu____7492))
in (((FStar_Getopt.noshort), ("use_nbe"), (BoolStr), ("Use normalization by evaluation as the default normalization srategy (default \'false\')")))::uu____7422)
in (((FStar_Getopt.noshort), ("use_extracted_interfaces"), (BoolStr), ("Extract interfaces from the dependencies and use them for verification (default \'false\')")))::uu____7408)
in (((FStar_Getopt.noshort), ("warn_error"), (SimpleStr ("")), ("The [-warn_error] option follows the OCaml syntax, namely:\n\t\t- [r] is a range of warnings (either a number [n], or a range [n..n])\n\t\t- [-r] silences range [r]\n\t\t- [+r] enables range [r]\n\t\t- [@r] makes range [r] fatal.")))::uu____7394)
in (uu____7360)::uu____7380))
in (uu____7326)::uu____7346))
in (((FStar_Getopt.noshort), ("use_two_phase_tc"), (BoolStr), ("Use the two phase typechecker (default \'true\')")))::uu____7312)
in (((FStar_Getopt.noshort), ("z3seed"), (IntStr ("positive_integer")), ("Set the Z3 random seed (default 0)")))::uu____7298)
in (((FStar_Getopt.noshort), ("z3rlimit_factor"), (IntStr ("positive_integer")), ("Set the Z3 per-query resource limit multiplier. This is useful when, say, regenerating hints and you want to be more lax. (default 1)")))::uu____7284)
in (((FStar_Getopt.noshort), ("z3rlimit"), (IntStr ("positive_integer")), ("Set the Z3 per-query resource limit (default 5 units, taking roughtly 5s)")))::uu____7270)
in (uu____7236)::uu____7256))
in (((FStar_Getopt.noshort), ("z3cliopt"), (ReverseAccumulated (SimpleStr ("option"))), ("Z3 command line options")))::uu____7222)
in (uu____7188)::uu____7208))
in (uu____7136)::uu____7174))
in (uu____7102)::uu____7122))
in (((FStar_Getopt.noshort), ("__temp_no_proj"), (Accumulated (SimpleStr ("module_name"))), ("Don\'t generate projectors for this module")))::uu____7088)
in (((FStar_Getopt.noshort), ("vcgen.optimize_bind_as_seq"), (EnumStr (("off")::("without_type")::("with_type")::[])), ("\n\t\tOptimize the generation of verification conditions, \n\t\t\tspecifically the construction of monadic `bind`,\n\t\t\tgenerating `seq` instead of `bind` when the first computation as a trivial post-condition.\n\t\t\tBy default, this optimization does not apply.\n\t\t\tWhen the `without_type` option is chosen, this imposes a cost on the SMT solver\n\t\t\tto reconstruct type information.\n\t\t\tWhen `with_type` is chosen, type information is provided to the SMT solver,\n\t\t\tbut at the cost of VC bloat, which may often be redundant.")))::uu____7074)
in (((FStar_Getopt.noshort), ("using_facts_from"), (Accumulated (SimpleStr ("One or more space-separated occurrences of \'[+|-]( * | namespace | fact id)\'"))), ("\n\t\tPrunes the context to include only the facts from the given namespace or fact id. \n\t\t\tFacts can be include or excluded using the [+|-] qualifier. \n\t\t\tFor example --using_facts_from \'* -FStar.Reflection +FStar.List -FStar.List.Tot\' will \n\t\t\t\tremove all facts from FStar.List.Tot.*, \n\t\t\t\tretain all remaining facts from FStar.List.*, \n\t\t\t\tremove all facts from FStar.Reflection.*, \n\t\t\t\tand retain all the rest.\n\t\tNote, the \'+\' is optional: --using_facts_from \'FStar.List\' is equivalent to --using_facts_from \'+FStar.List\'. \n\t\tMultiple uses of this option accumulate, e.g., --using_facts_from A --using_facts_from B is interpreted as --using_facts_from A^B.")))::uu____7060)
in (uu____7026)::uu____7046))
in (uu____6992)::uu____7012))
in (((FStar_Getopt.noshort), ("use_native_tactics"), (PathStr ("path")), ("Use compiled tactics from <path>")))::uu____6978)
in (uu____6944)::uu____6964))
in (uu____6910)::uu____6930))
in (uu____6876)::uu____6896))
in (uu____6842)::uu____6862))
in (uu____6808)::uu____6828))
in (uu____6774)::uu____6794))
in (uu____6740)::uu____6760))
in (uu____6706)::uu____6726))
in (((FStar_Getopt.noshort), ("tcnorm"), (BoolStr), ("Attempt to normalize definitions marked as tcnorm (default \'true\')")))::uu____6692)
in (uu____6658)::uu____6678))
in (((FStar_Getopt.noshort), ("tactic_trace_d"), (IntStr ("positive_integer")), ("Trace tactics up to a certain binding depth")))::uu____6644)
in (uu____6610)::uu____6630))
in (uu____6576)::uu____6596))
in (uu____6542)::uu____6562))
in (uu____6508)::uu____6528))
in (((FStar_Getopt.noshort), ("smtencoding.l_arith_repr"), (EnumStr (("native")::("boxwrap")::[])), ("Toggle the representation of linear arithmetic functions in the SMT encoding:\n\t\ti.e., if \'boxwrap\', use \'Prims.op_Addition, Prims.op_Subtraction, Prims.op_Minus\'; \n\t\tif \'native\', use \'+, -, -\'; \n\t\t(default \'boxwrap\')")))::uu____6494)
in (((FStar_Getopt.noshort), ("smtencoding.nl_arith_repr"), (EnumStr (("native")::("wrapped")::("boxwrap")::[])), ("Control the representation of non-linear arithmetic functions in the SMT encoding:\n\t\ti.e., if \'boxwrap\' use \'Prims.op_Multiply, Prims.op_Division, Prims.op_Modulus\'; \n\t\tif \'native\' use \'*, div, mod\';\n\t\tif \'wrapped\' use \'_mul, _div, _mod : Int*Int -> Int\'; \n\t\t(default \'boxwrap\')")))::uu____6480)
in (((FStar_Getopt.noshort), ("smtencoding.elim_box"), (BoolStr), ("Toggle a peephole optimization that eliminates redundant uses of boxing/unboxing in the SMT encoding (default \'false\')")))::uu____6466)
in (((FStar_Getopt.noshort), ("smt"), (PathStr ("path")), ("Path to the Z3 SMT solver (we could eventually support other solvers)")))::uu____6452)
in (uu____6418)::uu____6438))
in (((FStar_Getopt.noshort), ("reuse_hint_for"), (SimpleStr ("toplevel_name")), ("Optimistically, attempt using the recorded hint for <toplevel_name> (a top-level name in the current module) when trying to verify some other term \'g\'")))::uu____6404)
in (uu____6370)::uu____6390))
in (uu____6336)::uu____6356))
in (uu____6302)::uu____6322))
in (uu____6268)::uu____6288))
in (uu____6234)::uu____6254))
in (uu____6200)::uu____6220))
in (uu____6166)::uu____6186))
in (uu____6132)::uu____6152))
in (uu____6098)::uu____6118))
in (((FStar_Getopt.noshort), ("prims"), (PathStr ("file")), ("")))::uu____6084)
in (((FStar_Getopt.noshort), ("odir"), (PostProcessed (((pp_validate_dir), (PathStr ("dir"))))), ("Place output in directory <dir>")))::uu____6070)
in (uu____6036)::uu____6056))
in (uu____6002)::uu____6022))
in (uu____5968)::uu____5988))
in (((FStar_Getopt.noshort), ("no_extract"), (Accumulated (PathStr ("module name"))), ("Deprecated: use --extract instead; Do not extract code from this module")))::uu____5954)
in (uu____5920)::uu____5940))
in (((FStar_Getopt.noshort), ("n_cores"), (IntStr ("positive_integer")), ("Maximum number of cores to use for the solver (implies detail_errors = false) (default 1)")))::uu____5906)
in (uu____5872)::uu____5892))
in (((FStar_Getopt.noshort), ("min_fuel"), (IntStr ("non-negative integer")), ("Minimum number of unrolling of recursive functions to try (default 1)")))::uu____5858)
in (((FStar_Getopt.noshort), ("max_ifuel"), (IntStr ("non-negative integer")), ("Number of unrolling of inductive datatypes to try at most (default 2)")))::uu____5844)
in (((FStar_Getopt.noshort), ("max_fuel"), (IntStr ("non-negative integer")), ("Number of unrolling of recursive functions to try at most (default 8)")))::uu____5830)
in (uu____5796)::uu____5816))
in (uu____5762)::uu____5782))
in (((FStar_Getopt.noshort), ("load"), (ReverseAccumulated (PathStr ("module"))), ("Load compiled module")))::uu____5748)
in (uu____5714)::uu____5734))
in (((FStar_Getopt.noshort), ("keep_query_captions"), (BoolStr), ("Retain comments in the logged SMT queries (requires --log_queries; default true)")))::uu____5700)
in (((FStar_Getopt.noshort), ("initial_ifuel"), (IntStr ("non-negative integer")), ("Number of unrolling of inductive datatypes to try at first (default 1)")))::uu____5686)
in (((FStar_Getopt.noshort), ("initial_fuel"), (IntStr ("non-negative integer")), ("Number of unrolling of recursive functions to try initially (default 2)")))::uu____5672)
in (((FStar_Getopt.noshort), ("protect_top_level_axioms"), (BoolStr), ("Guard nullary top-level symbols in the SMT encoding from provide ambient ground facts (default \'false\')")))::uu____5658)
in (uu____5624)::uu____5644))
...
in (((FStar_Getopt.noshort), ("abort_on"), (PostProcessed ((((fun uu___88_9155 -> (match (uu___88_9155) with
| Int (x) -> begin
...
and specs : unit  ->  FStar_Getopt.opt Prims.list = (fun uu____9184 -> (

let uu____9187 = (specs_with_types ())
in (FStar_List.map (fun uu____9218 -> (match (uu____9218) with
| (short, long, typ, doc) -> begin
(

let uu____9240 = (

let uu____9254 = (arg_spec_of_opt_type long typ)
in ((short), (long), (uu____9254), (doc)))
in (mk_spec uu____9240))
end)) uu____9187)))


let settable : Prims.string  ->  Prims.bool = (fun uu___89_9269 -> (match (uu___89_9269) with
| "abort_on" -> begin
true
end
...
let settable_specs : (FStar_BaseTypes.char * Prims.string * unit FStar_Getopt.opt_variant * Prims.string) Prims.list = (FStar_All.pipe_right all_specs (FStar_List.filter (fun uu____9491 -> (match (uu____9491) with
| (uu____9506, x, uu____9508, uu____9509) -> begin
(settable x)
end))))


let resettable_specs : (FStar_BaseTypes.char * Prims.string * unit FStar_Getopt.opt_variant * Prims.string) Prims.list = (FStar_All.pipe_right all_specs (FStar_List.filter (fun uu____9584 -> (match (uu____9584) with
| (uu____9599, x, uu____9601, uu____9602) -> begin
(resettable x)
end))))


let display_usage : unit  ->  unit = (fun uu____9618 -> (

let uu____9619 = (specs ())
in (display_usage_aux uu____9619)))
...
| File_argument (uu____9651) -> begin
true
end
| uu____9654 -> begin
false
...
| File_argument (uu____9665) -> begin
uu____9665
end))
...
in (FStar_All.try_with (fun uu___94_9686 -> (match (()) with
| () -> begin
...
end)) (fun uu___93_9700 -> (match (uu___93_9700) with
| File_argument (s1) -> begin
(

let uu____9703 = (FStar_Util.format1 "File %s is not a valid option" s1)
in FStar_Getopt.Error (uu____9703))
end)))))
...
let parse_cmd_line : unit  ->  (FStar_Getopt.parse_cmdline_res * Prims.string Prims.list) = (fun uu____9739 -> (

let res = (FStar_Getopt.parse_cmdline all_specs (fun i -> (

let uu____9745 = (

let uu____9749 = (FStar_ST.op_Bang file_list_)
in (FStar_List.append uu____9749 ((i)::[])))
in (FStar_ST.op_Colon_Equals file_list_ uu____9745))))
in (

let uu____9806 = (

let uu____9810 = (FStar_ST.op_Bang file_list_)
in (FStar_List.map FStar_Common.try_convert_file_name_to_mixed uu____9810))
in ((res), (uu____9806)))))


let file_list : unit  ->  Prims.string Prims.list = (fun uu____9852 -> (FStar_ST.op_Bang file_list_))
...
let uu____9895 = (specs ())
in (FStar_Getopt.parse_cmdline uu____9895 (fun x -> ())))
in ((
...
let uu____9909 = (FStar_List.map mk_string old_verify_module)
in List (uu____9909))
in (("verify_module"), (uu____9908)))
in (set_option' uu____9902));
r;
));
)))
...
let uu____9934 = (FStar_Util.get_file_extension f1)
in (FStar_String.length uu____9934))
in ((FStar_String.length f1) - uu____9932))
in (uu____9930 - (Prims.parse_int "1")))
in (FStar_String.substring f1 (Prims.parse_int "0") uu____9928))
in (FStar_String.lowercase f2))))


let should_verify : Prims.string  ->  Prims.bool = (fun m -> (

let uu____9947 = (get_lax ())
...
let uu____9968 = (module_name_of_file_name fn)
in (should_verify uu____9968)))
...
let uu____9996 = (get___temp_no_proj ())
in (FStar_All.pipe_right uu____9996 (FStar_List.existsb (module_name_eq m)))))


let should_print_message : Prims.string  ->  Prims.bool = (fun m -> (

let uu____10014 = (should_verify m)
...
let include_path : unit  ->  Prims.string Prims.list = (fun uu____10031 -> (

let cache_dir = (

let uu____10036 = (get_cache_dir ())
...
let uu____10050 = (get_no_default_includes ())
in (match (uu____10050) with
| true -> begin
(

let uu____10056 = (get_include ())
in (FStar_List.append cache_dir uu____10056))
end
| uu____10061 -> begin
(
...
let uu____10083 = (FStar_All.pipe_right defs (FStar_List.map (fun x -> (Prims.strcat fstar_home x))))
in (FStar_All.pipe_right uu____10083 (FStar_List.filter FStar_Util.file_exists)))))
end
...
let uu____10118 = (get_include ())
in (FStar_List.append uu____10118 ((".")::[])))
in (FStar_List.append lib_paths uu____10114))
in (FStar_List.append cache_dir uu____10110)))
end))))
...
let uu____10145 = (FStar_Util.smap_try_find file_map filename)
...
let result = (FStar_All.try_with (fun uu___96_10167 -> (match (()) with
| () -> begin
(

let uu____10171 = (FStar_Util.is_path_absolute filename)
...
let uu____10191 = (include_path ())
in (FStar_List.rev uu____10191))
in (FStar_Util.find_map uu____10187 (fun p -> (
...
end)) (fun uu___95_10219 -> FStar_Pervasives_Native.None))
...
let prims : unit  ->  Prims.string = (fun uu____10239 -> (

let uu____10240 = (get_prims ())
...
let uu____10249 = (find_file filename)
...
let uu____10258 = (FStar_Util.format1 "unable to find required file \"%s\" in the module search path.\n" filename)
in (failwith uu____10258))
end)))
end
...
let prims_basename : unit  ->  Prims.string = (fun uu____10271 -> (

let uu____10272 = (prims ())
in (FStar_Util.basename uu____10272)))


let pervasives : unit  ->  Prims.string = (fun uu____10280 -> (

let filename = "FStar.Pervasives.fst"
in (

let uu____10284 = (find_file filename)
...
let uu____10293 = (FStar_Util.format1 "unable to find required file \"%s\" in the module search path.\n" filename)
in (failwith uu____10293))
end))))


let pervasives_basename : unit  ->  Prims.string = (fun uu____10303 -> (

let uu____10304 = (pervasives ())
in (FStar_Util.basename uu____10304)))


let pervasives_native_basename : unit  ->  Prims.string = (fun uu____10312 -> (

let filename = "FStar.Pervasives.Native.fst"
in (

let uu____10316 = (find_file filename)
...
let uu____10325 = (FStar_Util.format1 "unable to find required file \"%s\" in the module search path.\n" filename)
in (failwith uu____10325))
end))))


let prepend_output_dir : Prims.string  ->  Prims.string = (fun fname -> (

let uu____10338 = (get_odir ())
...
let uu____10365 = (FStar_Util.basename fpath)
in (FStar_Util.join_paths x uu____10365))
end)))
...
let uu____10487 = (FStar_Util.smap_try_find cache s)
...
let uu____10622 = (FStar_Util.substring_from s (Prims.parse_int "1"))
in (path_of_text uu____10622))
in ((path), (false)))
end
| uu____10630 -> begin
(
...
let uu____10645 = (FStar_All.pipe_right ns (FStar_List.collect (fun s -> (
...
let uu____10713 = (FStar_All.pipe_right (FStar_Util.splitlines s2) (FStar_List.concatMap (fun s3 -> (FStar_Util.split s3 " "))))
in (FStar_All.pipe_right uu____10713 (FStar_List.map parse_one_setting)))) s1)
end)))))
in (FStar_All.pipe_right uu____10645 FStar_List.rev))))))


let __temp_no_proj : Prims.string  ->  Prims.bool = (fun s -> (

let uu____10788 = (get___temp_no_proj ())
in (FStar_All.pipe_right uu____10788 (FStar_List.contains s))))


let __temp_fast_implicits : unit  ->  Prims.bool = (fun uu____10803 -> (lookup_opt "__temp_fast_implicits" as_bool))


let admit_smt_queries : unit  ->  Prims.bool = (fun uu____10812 -> (get_admit_smt_queries ()))


let admit_except : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____10821 -> (get_admit_except ()))


let cache_checked_modules : unit  ->  Prims.bool = (fun uu____10828 -> (get_cache_checked_modules ()))
...
let codegen : unit  ->  codegen_t FStar_Pervasives_Native.option = (fun uu____10894 -> (

let uu____10895 = (get_codegen ())
in (FStar_Util.map_opt uu____10895 (fun uu___90_10901 -> (match (uu___90_10901) with
| "OCaml" -> begin
OCaml
end
| "FSharp" -> begin
...
let codegen_libs : unit  ->  Prims.string Prims.list Prims.list = (fun uu____10920 -> (

let uu____10921 = (get_codegen_lib ())
in (FStar_All.pipe_right uu____10921 (FStar_List.map (fun x -> (FStar_Util.split x "."))))))


let debug_any : unit  ->  Prims.bool = (fun uu____10947 -> (

let uu____10948 = (get_debug ())
in (Prims.op_disEquality uu____10948 [])))


let debug_module : Prims.string  ->  Prims.bool = (fun modul -> (

let uu____10965 = (get_debug ())
in (FStar_All.pipe_right uu____10965 (FStar_List.existsb (module_name_eq modul)))))


let debug_at_level : Prims.string  ->  debug_level_t  ->  Prims.bool = (fun modul level -> ((

let uu____10990 = (get_debug ())
in (FStar_All.pipe_right uu____10990 (FStar_List.existsb (module_name_eq modul)))) && (debug_level_geq level)))


let defensive : unit  ->  Prims.bool = (fun uu____11005 -> (

let uu____11006 = (get_defensive ())
in (Prims.op_disEquality uu____11006 "no")))


let defensive_fail : unit  ->  Prims.bool = (fun uu____11016 -> (

let uu____11017 = (get_defensive ())
in (Prims.op_Equality uu____11017 "fail")))


let dep : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____11029 -> (get_dep ()))


let detail_errors : unit  ->  Prims.bool = (fun uu____11036 -> (get_detail_errors ()))


let detail_hint_replay : unit  ->  Prims.bool = (fun uu____11043 -> (get_detail_hint_replay ()))
...
let uu____11060 = (get_dump_module ())
in (FStar_All.pipe_right uu____11060 (FStar_List.existsb (module_name_eq s)))))


let eager_subtyping : unit  ->  Prims.bool = (fun uu____11075 -> (get_eager_subtyping ()))


let expose_interfaces : unit  ->  Prims.bool = (fun uu____11082 -> (get_expose_interfaces ()))


let fs_typ_app : Prims.string  ->  Prims.bool = (fun filename -> (

let uu____11092 = (FStar_ST.op_Bang light_off_files)
in (FStar_List.contains filename uu____11092)))


let full_context_dependency : unit  ->  Prims.bool = (fun uu____11128 -> true)


let hide_uvar_nums : unit  ->  Prims.bool = (fun uu____11136 -> (get_hide_uvar_nums ()))


let hint_info : unit  ->  Prims.bool = (fun uu____11143 -> ((get_hint_info ()) || (get_query_stats ())))


let hint_file : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____11152 -> (get_hint_file ()))
...
let print_in_place : unit  ->  Prims.bool = (fun uu____11173 -> (get_print_in_place ()))


let profile : 'a . (unit  ->  'a)  ->  ('a  ->  Prims.string)  ->  'a = (fun f msg -> (

let uu____11204 = (get_profile ())
in (match (uu____11204) with
| true -> begin
(

let uu____11207 = (FStar_Util.record_time f)
in (match (uu____11207) with
| (a, time) -> begin
((

let uu____11218 = (FStar_Util.string_of_int time)
in (

let uu____11220 = (msg a)
in (FStar_Util.print2 "Elapsed time %s ms: %s\n" uu____11218 uu____11220)));
a;
)
end))
end
...
let protect_top_level_axioms : unit  ->  Prims.bool = (fun uu____11231 -> (get_protect_top_level_axioms ()))


let initial_fuel : unit  ->  Prims.int = (fun uu____11238 -> (

let uu____11239 = (get_initial_fuel ())
in (

let uu____11241 = (get_max_fuel ())
in (Prims.min uu____11239 uu____11241))))


let initial_ifuel : unit  ->  Prims.int = (fun uu____11249 -> (

let uu____11250 = (get_initial_ifuel ())
in (

let uu____11252 = (get_max_ifuel ())
in (Prims.min uu____11250 uu____11252))))


let interactive : unit  ->  Prims.bool = (fun uu____11260 -> ((get_in ()) || (get_ide ())))


let lax : unit  ->  Prims.bool = (fun uu____11267 -> (get_lax ()))


let load : unit  ->  Prims.string Prims.list = (fun uu____11276 -> (get_load ()))


let legacy_interactive : unit  ->  Prims.bool = (fun uu____11283 -> (get_in ()))


let log_queries : unit  ->  Prims.bool = (fun uu____11290 -> (get_log_queries ()))


let keep_query_captions : unit  ->  Prims.bool = (fun uu____11297 -> ((log_queries ()) && (get_keep_query_captions ())))
...
let set_ml_ish : unit  ->  unit = (fun uu____11338 -> (set_option "MLish" (Bool (true))))


let n_cores : unit  ->  Prims.int = (fun uu____11347 -> (get_n_cores ()))


let no_default_includes : unit  ->  Prims.bool = (fun uu____11354 -> (get_no_default_includes ()))


let no_extract : Prims.string  ->  Prims.bool = (fun s -> (

let uu____11364 = (get_no_extract ())
in (FStar_All.pipe_right uu____11364 (FStar_List.existsb (module_name_eq s)))))


let normalize_pure_terms_for_extraction : unit  ->  Prims.bool = (fun uu____11379 -> (get_normalize_pure_terms_for_extraction ()))


let no_location_info : unit  ->  Prims.bool = (fun uu____11386 -> (get_no_location_info ()))
...
let output_dir : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____11409 -> (get_odir ()))


let ugly : unit  ->  Prims.bool = (fun uu____11416 -> (get_ugly ()))


let print_bound_var_types : unit  ->  Prims.bool = (fun uu____11423 -> (get_print_bound_var_types ()))


let print_effect_args : unit  ->  Prims.bool = (fun uu____11430 -> (get_print_effect_args ()))


let print_implicits : unit  ->  Prims.bool = (fun uu____11437 -> (get_print_implicits ()))


let print_real_names : unit  ->  Prims.bool = (fun uu____11444 -> ((get_prn ()) || (get_print_full_names ())))


let print_universes : unit  ->  Prims.bool = (fun uu____11451 -> (get_print_universes ()))


let print_z3_statistics : unit  ->  Prims.bool = (fun uu____11458 -> (get_print_z3_statistics ()))


let query_stats : unit  ->  Prims.bool = (fun uu____11465 -> (get_query_stats ()))


let record_hints : unit  ->  Prims.bool = (fun uu____11472 -> (get_record_hints ()))


let reuse_hint_for : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____11481 -> (get_reuse_hint_for ()))


let silent : unit  ->  Prims.bool = (fun uu____11488 -> (get_silent ()))


let smtencoding_elim_box : unit  ->  Prims.bool = (fun uu____11495 -> (get_smtencoding_elim_box ()))


let smtencoding_nl_arith_native : unit  ->  Prims.bool = (fun uu____11502 -> (

let uu____11503 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____11503 "native")))


let smtencoding_nl_arith_wrapped : unit  ->  Prims.bool = (fun uu____11513 -> (

let uu____11514 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____11514 "wrapped")))


let smtencoding_nl_arith_default : unit  ->  Prims.bool = (fun uu____11524 -> (

let uu____11525 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____11525 "boxwrap")))


let smtencoding_l_arith_native : unit  ->  Prims.bool = (fun uu____11535 -> (

let uu____11536 = (get_smtencoding_l_arith_repr ())
in (Prims.op_Equality uu____11536 "native")))


let smtencoding_l_arith_default : unit  ->  Prims.bool = (fun uu____11546 -> (

let uu____11547 = (get_smtencoding_l_arith_repr ())
in (Prims.op_Equality uu____11547 "boxwrap")))


let tactic_raw_binders : unit  ->  Prims.bool = (fun uu____11557 -> (get_tactic_raw_binders ()))


let tactics_failhard : unit  ->  Prims.bool = (fun uu____11564 -> (get_tactics_failhard ()))


let tactics_info : unit  ->  Prims.bool = (fun uu____11571 -> (get_tactics_info ()))


let tactic_trace : unit  ->  Prims.bool = (fun uu____11578 -> (get_tactic_trace ()))


let tactic_trace_d : unit  ->  Prims.int = (fun uu____11585 -> (get_tactic_trace_d ()))


let tactics_nbe : unit  ->  Prims.bool = (fun uu____11592 -> (get_tactics_nbe ()))
...
let trace_error : unit  ->  Prims.bool = (fun uu____11613 -> (get_trace_error ()))


let unthrottle_inductives : unit  ->  Prims.bool = (fun uu____11620 -> (get_unthrottle_inductives ()))


let unsafe_tactic_exec : unit  ->  Prims.bool = (fun uu____11627 -> (get_unsafe_tactic_exec ()))


let use_eq_at_higher_order : unit  ->  Prims.bool = (fun uu____11634 -> (get_use_eq_at_higher_order ()))


let use_hints : unit  ->  Prims.bool = (fun uu____11641 -> (get_use_hints ()))


let use_hint_hashes : unit  ->  Prims.bool = (fun uu____11648 -> (get_use_hint_hashes ()))


let use_native_tactics : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____11657 -> (get_use_native_tactics ()))


let use_tactics : unit  ->  Prims.bool = (fun uu____11664 -> (get_use_tactics ()))


let using_facts_from : unit  ->  (Prims.string Prims.list * Prims.bool) Prims.list = (fun uu____11680 -> (

let uu____11681 = (get_using_facts_from ())
...
let vcgen_optimize_bind_as_seq : unit  ->  Prims.bool = (fun uu____11735 -> (

let uu____11736 = (get_vcgen_optimize_bind_as_seq ())
in (FStar_Option.isSome uu____11736)))


let vcgen_decorate_with_type : unit  ->  Prims.bool = (fun uu____11747 -> (

let uu____11748 = (get_vcgen_optimize_bind_as_seq ())
...
let warn_default_effects : unit  ->  Prims.bool = (fun uu____11767 -> (get_warn_default_effects ()))


let z3_exe : unit  ->  Prims.string = (fun uu____11774 -> (

let uu____11775 = (get_smt ())
...
let z3_cliopt : unit  ->  Prims.string Prims.list = (fun uu____11793 -> (get_z3cliopt ()))
...
let z3_rlimit_factor : unit  ->  Prims.int = (fun uu____11814 -> (get_z3rlimit_factor ()))


let z3_seed : unit  ->  Prims.int = (fun uu____11821 -> (get_z3seed ()))


let use_two_phase_tc : unit  ->  Prims.bool = (fun uu____11828 -> ((get_use_two_phase_tc ()) && (

let uu____11830 = (lax ())
in (not (uu____11830)))))


let no_positivity : unit  ->  Prims.bool = (fun uu____11838 -> (get_no_positivity ()))


let ml_no_eta_expand_coertions : unit  ->  Prims.bool = (fun uu____11845 -> (get_ml_no_eta_expand_coertions ()))


let warn_error : unit  ->  Prims.string = (fun uu____11852 -> (get_warn_error ()))


let use_extracted_interfaces : unit  ->  Prims.bool = (fun uu____11859 -> (get_use_extracted_interfaces ()))
...
let r = (FStar_All.try_with (fun uu___98_11900 -> (match (()) with
| () -> begin
(

let uu____11905 = (f ())
in FStar_Util.Inr (uu____11905))
end)) (fun uu___97_11907 -> FStar_Util.Inl (uu___97_11907)))
in ((pop ());
(match (r) with
...
let uu____12033 = (FStar_All.pipe_right setting (FStar_Util.try_find (fun uu____12075 -> (match (uu____12075) with
| (path, uu____12086) -> begin
...
| FStar_Pervasives_Native.Some (uu____12105, flag) -> begin
flag
end)))))))
...
let uu____12165 = (get_no_extract ())
in (

let uu____12169 = (get_extract_namespace ())
in (

let uu____12173 = (get_extract_module ())
in ((uu____12165), (uu____12169), (uu____12173)))))
...
let uu____12227 = (get_extract_namespace ())
in (match (uu____12227) with
| [] -> begin
false
end
| ns -> begin
...
let uu____12255 = (get_extract_module ())
...
let uu____12277 = (no_extract m1)
in (not (uu____12277))) && (

let uu____12280 = (

let uu____12291 = (get_extract_namespace ())
in (

let uu____12295 = (get_extract_module ())
in ((uu____12291), (uu____12295))))
...
let uu____12335 = (get_already_cached ())
