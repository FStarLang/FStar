...
let z3_exe = (let _49_19 = (FStar_Platform.exe "z3")
in (FStar_Util.mk_ref _49_19))
...
let debug_level_geq = (fun l2 -> (let _49_29 = (FStar_ST.read debug_level)
in (FStar_All.pipe_right _49_29 (FStar_Util.for_some (fun l1 -> (one_debug_level_geq l1 l2))))))
...
in (let _19_39 = (let _49_38 = (FStar_Platform.exe "z3")
in (FStar_ST.op_Colon_Equals z3_exe _49_38))
...
| None -> begin
(let _19_154 = (let _49_43 = (set_fstar_home ())
in (FStar_All.pipe_left Prims.ignore _49_43))
...
let get_include_path = (fun dirname -> (let _49_48 = (let h = (get_fstar_home ())
in (let _49_47 = (FStar_ST.read _include_path)
in (FStar_List.append _49_47 ((".")::((Prims.strcat h "/lib"))::((Prims.strcat h "/lib/fstar"))::[]))))
...
| None -> begin
(let filen = "prims.fst"
in (match ((let _49_51 = (get_include_path ".")
in (FStar_Util.find_file filen _49_51))) with
| Some (result) -> begin
result
end
| None -> begin
...
if (doc = "") then begin
(let _49_60 = (FStar_Util.format1 "  --%s\n" flag)
in (FStar_Util.print_string _49_60))
end else begin
(let _49_61 = (FStar_Util.format2 "  --%s  %s\n" flag doc)
in (FStar_Util.print_string _49_61))
end
end
| FStar_Getopt.OneArg (_19_186, argname) -> begin
if (doc = "") then begin
(let _49_63 = (FStar_Util.format2 "  --%s %s\n" flag argname)
in (FStar_Util.print_string _49_63))
end else begin
(let _49_64 = (FStar_Util.format3 "  --%s %s  %s\n" flag argname doc)
in (FStar_Util.print_string _49_64))
end
end)
end)) specs)))
...
| () -> begin
(let specs = ((FStar_Getopt.noshort, "admit_fsi", FStar_Getopt.OneArg (((fun x -> (let _49_74 = (let _49_73 = (FStar_ST.read admit_fsi)
in (x)::_49_73)
in (FStar_ST.op_Colon_Equals admit_fsi _49_74))), "module name")), "Treat .fsi as a .fst"))::((FStar_Getopt.noshort, "auto_deps", FStar_Getopt.ZeroArgs ((fun _19_192 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals auto_deps true)
end))), "automatically treat files discovered by --find_deps as dependencies."))::((FStar_Getopt.noshort, "admit_smt_queries", FStar_Getopt.OneArg (((fun s -> (let _49_79 = if (s = "true") then begin
true
end else begin
...
in (FStar_ST.op_Colon_Equals admit_smt_queries _49_79))), "true|false")), "Admit SMT queries (UNSAFE! But, useful during development); default: \'false\'"))::((FStar_Getopt.noshort, "auto_deps", FStar_Getopt.ZeroArgs ((fun _19_194 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals auto_deps true)
end))), "automatically treat files discovered by --find_deps as dependencies."))::((FStar_Getopt.noshort, "codegen", FStar_Getopt.OneArg (((fun s -> (let _49_84 = (parse_codegen s)
in (FStar_ST.op_Colon_Equals codegen _49_84))), "OCaml|FSharp")), "Generate code for execution"))::((FStar_Getopt.noshort, "codegen-lib", FStar_Getopt.OneArg (((fun s -> (let _49_89 = (let _49_88 = (FStar_ST.read codegen_libs)
in ((FStar_Util.split s "."))::_49_88)
in (FStar_ST.op_Colon_Equals codegen_libs _49_89))), "namespace")), "External runtime library library"))::((FStar_Getopt.noshort, "debug", FStar_Getopt.OneArg (((fun x -> (let _49_94 = (let _49_93 = (FStar_ST.read debug)
in (x)::_49_93)
in (FStar_ST.op_Colon_Equals debug _49_94))), "module name")), "Print LOTS of debugging information while checking module [arg]"))::((FStar_Getopt.noshort, "debug_level", FStar_Getopt.OneArg (((fun x -> (let _49_99 = (let _49_98 = (FStar_ST.read debug_level)
in ((dlevel x))::_49_98)
in (FStar_ST.op_Colon_Equals debug_level _49_99))), "Low|Medium|High|Extreme")), "Control the verbosity of debugging info"))::((FStar_Getopt.noshort, "dep", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dep (Some (x)))), "make|nubuild")), "Output the transitive closure of the dependency graph in a format suitable for the given tool"))::((FStar_Getopt.noshort, "dump_module", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dump_module (Some (x)))), "module name")), ""))::((FStar_Getopt.noshort, "eager_inference", FStar_Getopt.ZeroArgs ((fun _19_201 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals interactive_context (Some (s))))), "name")), "Specify the context of an interactive session; needed for --auto_deps to work with interactive mode."))::((FStar_Getopt.noshort, "include", FStar_Getopt.OneArg (((fun s -> (let _49_123 = (let _49_122 = (FStar_ST.read _include_path)
in (FStar_List.append _49_122 ((s)::[])))
in (FStar_ST.op_Colon_Equals _include_path _49_123))), "path")), "A directory in which to search for files included on the command line"))::((FStar_Getopt.noshort, "initial_fuel", FStar_Getopt.OneArg (((fun x -> (let _49_127 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals initial_fuel _49_127))), "non-negative integer")), "Number of unrolling of recursive functions to try initially (default 2)"))::((FStar_Getopt.noshort, "initial_ifuel", FStar_Getopt.OneArg (((fun x -> (let _49_131 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals initial_ifuel _49_131))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at first (default 1)"))::((FStar_Getopt.noshort, "lax", FStar_Getopt.ZeroArgs ((fun _19_217 -> (match (()) with
| () -> begin
...
end))), "Log the Z3 queries in queries.smt2"))::((FStar_Getopt.noshort, "max_fuel", FStar_Getopt.OneArg (((fun x -> (let _49_138 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals max_fuel _49_138))), "non-negative integer")), "Number of unrolling of recursive functions to try at most (default 8)"))::((FStar_Getopt.noshort, "max_ifuel", FStar_Getopt.OneArg (((fun x -> (let _49_142 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals max_ifuel _49_142))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at most (default 2)"))::((FStar_Getopt.noshort, "min_fuel", FStar_Getopt.OneArg (((fun x -> (let _49_146 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals min_fuel _49_146))), "non-negative integer")), "Minimum number of unrolling of recursive functions to try (default 1)"))::((FStar_Getopt.noshort, "MLish", FStar_Getopt.ZeroArgs ((fun _19_225 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals full_context_dependency false)
end))), "Introduce unification variables that are only dependent on the type variables in the context"))::((FStar_Getopt.noshort, "n_cores", FStar_Getopt.OneArg (((fun x -> (let _49_151 = (FStar_Util.int_of_string x)
in (FStar_ST.op_Colon_Equals n_cores _49_151))), "positive integer")), "Maximum number of cores to use for the solver (default 1)"))::((FStar_Getopt.noshort, "no_fs_typ_app", FStar_Getopt.ZeroArgs ((fun _19_227 -> (match (()) with
| () -> begin
...
end))), "Print real names---you may want to use this in conjunction with log_queries"))::((FStar_Getopt.noshort, "show_signatures", FStar_Getopt.OneArg (((fun x -> (let _49_170 = (let _49_169 = (FStar_ST.read show_signatures)
in (x)::_49_169)
in (FStar_ST.op_Colon_Equals show_signatures _49_170))), "module name")), "Show the checked signatures for all top-level symbols in the module"))::((FStar_Getopt.noshort, "silent", FStar_Getopt.ZeroArgs ((fun _19_238 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals silent true)
end))), ""))::((FStar_Getopt.noshort, "smt", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals z3_exe x)), "path")), "Path to the SMT solver (usually Z3, but could be any SMT2-compatible solver)"))::((FStar_Getopt.noshort, "split_cases", FStar_Getopt.OneArg (((fun n -> (let _49_178 = (FStar_Util.int_of_string n)
in (FStar_ST.op_Colon_Equals split_cases _49_178))), "t")), "Partition VC of a match into groups of n cases"))::((FStar_Getopt.noshort, "timing", FStar_Getopt.ZeroArgs ((fun _19_241 -> (match (()) with
| () -> begin
...
end))), "Extract the \'int\' type to platform-specific native int; you will need to link the generated code with the appropriate version of the prims library"))::((FStar_Getopt.noshort, "verify_module", FStar_Getopt.OneArg (((fun x -> (let _49_188 = (let _49_187 = (FStar_ST.read verify_module)
in (x)::_49_187)
in (FStar_ST.op_Colon_Equals verify_module _49_188))), "string")), "Name of the module to verify"))::((FStar_Getopt.noshort, "__temp_no_proj", FStar_Getopt.OneArg (((fun x -> (let _49_193 = (let _49_192 = (FStar_ST.read __temp_no_proj)
in (x)::_49_192)
in (FStar_ST.op_Colon_Equals __temp_no_proj _49_193))), "string")), "Don\'t generate projectors for this module"))::(('v', "version", FStar_Getopt.ZeroArgs ((fun _19_248 -> (let _19_250 = (display_version ())
...
end))), "Top-level effects are ignored, by default; turn this flag on to be warned when this happens"))::((FStar_Getopt.noshort, "z3timeout", FStar_Getopt.OneArg (((fun s -> (let _49_199 = (FStar_Util.int_of_string s)
in (FStar_ST.op_Colon_Equals z3timeout _49_199))), "t")), "Set the Z3 per-query (soft) timeout to t seconds (default 5)"))::[]
...
in (let _19_265 = (let _49_202 = (specs ())
in (display_usage _49_202))
in (FStar_All.exit 1)))
end))
...
in (let _19_275 = (let _49_204 = (specs ())
in (display_usage _49_204))
in (FStar_All.exit 1)))
end))
...
in (let _19_281 = (let _49_206 = (specs ())
in (display_usage _49_206))
in (FStar_All.exit 1)))
end)
...
let dont_gen_projectors = (fun m -> (let _49_211 = (FStar_ST.read __temp_no_proj)
in (FStar_List.contains m _49_211)))

let should_print_message = (fun m -> (((should_verify m) && (not ((let _49_214 = (FStar_ST.read admit_fsi)
in (FStar_List.contains m _49_214))))) && (m <> "Prims")))

let set_options = (let no_smt_specs = (let _49_217 = (specs ())
in (FStar_All.pipe_right _49_217 (FStar_List.filter (fun _19_295 -> (match (_19_295) with
...
in (let res = (let _49_223 = (specs ())
in (FStar_Getopt.parse_cmdline _49_223 (fun x -> ())))
