...
| None -> begin
(let filen = "prims.fst"
in (match ((let _87_51 = (get_include_path ".")
in (FStar_Util.find_file filen _87_51))) with
| Some (result) -> begin
result
end
| None -> begin
...
in (x)::_87_73)
in (FStar_ST.op_Colon_Equals admit_fsi _87_74))), "module name")), "Treat .fsi as a .fst"))::((FStar_Getopt.noshort, "auto_deps", FStar_Getopt.ZeroArgs ((fun _21_188 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals initial_ifuel _87_135))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at first (default 1)"))::((FStar_Getopt.noshort, "lax", FStar_Getopt.ZeroArgs ((fun _21_214 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals min_fuel _87_150))), "non-negative integer")), "Minimum number of unrolling of recursive functions to try (default 1)"))::((FStar_Getopt.noshort, "MLish", FStar_Getopt.ZeroArgs ((fun _21_222 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals n_cores _87_155))), "positive integer")), "Maximum number of cores to use for the solver (default 1)"))::((FStar_Getopt.noshort, "no_fs_typ_app", FStar_Getopt.ZeroArgs ((fun _21_224 -> (match (()) with
| () -> begin
...
in (x)::_87_173)
in (FStar_ST.op_Colon_Equals show_signatures _87_174))), "module name")), "Show the checked signatures for all top-level symbols in the module"))::((FStar_Getopt.noshort, "silent", FStar_Getopt.ZeroArgs ((fun _21_235 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals split_cases _87_182))), "t")), "Partition VC of a match into groups of n cases"))::((FStar_Getopt.noshort, "timing", FStar_Getopt.ZeroArgs ((fun _21_238 -> (match (()) with
| () -> begin
...
in (x)::_87_197)
in (FStar_ST.op_Colon_Equals __temp_no_proj _87_198))), "string")), "Don\'t generate projectors for this module"))::(('v', "version", FStar_Getopt.ZeroArgs ((fun _21_246 -> (let _21_248 = (display_version ())
...
| _21_260 -> begin
(let _21_261 = (FStar_Util.print_string "Wrong argument to codegen flag\n")
in (let _21_263 = (let _87_207 = (specs ())
...
| _21_270 -> begin
(let _21_271 = (FStar_Util.print_string "Wrong argument to cardinality flag\n")
in (let _21_273 = (let _87_209 = (specs ())
in (display_usage _87_209))
in (FStar_All.exit 1)))
end))
and set_interactive_fsi = (fun _21_275 -> if (FStar_ST.read interactive) then begin
(FStar_ST.op_Colon_Equals interactive_fsi true)
end else begin
(let _21_277 = (FStar_Util.print_string "Set interactive flag first before setting interactive fsi flag\n")
in (let _21_279 = (let _87_211 = (specs ())
...
in (FStar_All.pipe_right _87_222 (FStar_List.filter (fun _21_293 -> (match (_21_293) with
| (_21_287, name, _21_290, _21_292) -> begin
(name <> "smt")
end)))))
in (fun s -> (FStar_Getopt.parse_string no_smt_specs (fun _21_296 -> ()) s)))

let reset_options_string = (FStar_ST.alloc None)

let reset_options = (fun _21_298 -> (match (()) with
| () -> begin
(let _21_299 = (init_options ())
