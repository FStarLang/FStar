  ...
  fun uu___  ->
    ...
    | List ls ->
        let uu___1 =
          FStar_List.map
            (fun l  ->
               let uu___2 = as_string l  in FStar_Util.split uu___2 ",") ls
           in
...
let (internal_peek : unit -> optionstate) =
  fun uu___  ->
    let uu___1 =
      let uu___2 = FStar_ST.op_Bang fstar_options  in FStar_List.hd uu___2
       in
...
let (push : unit -> unit) =
  fun uu___  ->
    let uu___1 =
      let uu___2 =
        let uu___3 =
          let uu___4 = FStar_ST.op_Bang fstar_options  in
...
let (internal_pop : unit -> Prims.bool) =
  fun uu___  ->
    let curstack =
      let uu___1 = FStar_ST.op_Bang fstar_options  in FStar_List.hd uu___1
       in
    match curstack with
...
let (internal_push : unit -> unit) =
  fun uu___  ->
    ...
    let uu___1 =
      let uu___2 =
        let uu___3 = FStar_ST.op_Bang fstar_options  in FStar_List.tl uu___3
         in
      stack' :: uu___2  in
    FStar_ST.op_Colon_Equals fstar_options uu___1
...
let (add_light_off_file : Prims.string -> unit) =
  fun filename  ->
    let uu___ =
      let uu___1 = FStar_ST.op_Bang light_off_files  in filename :: uu___1
       in
...
let accumulate_string :
  ...
  fun name  ->
    fun post_processor  ->
      fun value  ->
        let uu___ =
          let uu___1 = let uu___2 = post_processor value  in String uu___2
             in
             ...
             | () ->
                 (match typ with
                  ...
                  | PostProcessed (pp,elem_spec) ->
                      let uu___1 = parse_opt_val opt_name elem_spec str_val
                         in
                      pp uu___1
...
let rec (desc_of_opt_type :
  ...
  fun typ  ->
    let desc_of_enum cases =
      let uu___ =
        let uu___1 = FStar_String.op_Hat (FStar_String.concat "|" cases) "]"
           in
...
let (pp_lowercase : option_val -> option_val) =
  fun s  ->
    let uu___ = let uu___1 = as_string s  in FStar_String.lowercase uu___1
       in
    String uu___
...
let (interp_quake_arg : Prims.string -> (Prims.int * Prims.int * Prims.bool))
  ...
  fun s  ->
    ...
    | f1::f2::[] ->
        ...
        then
          let uu___ = ios f1  in
          let uu___1 = ios f1  in (uu___, uu___1, true)
        else
    ...
    | f1::f2::k::[] ->
        ...
        then
          let uu___ = ios f1  in
          let uu___1 = ios f2  in (uu___, uu___1, true)
    ...
    (FStar_Getopt.noshort, "fuel",
      (PostProcessed
         (((fun uu___  ->
              ...
              | String s ->
                  let p f =
                    let uu___1 = FStar_Util.int_of_string f  in Int uu___1
                     in
                  let uu___1 =
                    match FStar_Util.split s "," with
                    | f::[] -> (f, f)
...
let (display_usage : unit -> unit) =
  fun uu___  -> display_usage_aux all_specs 
    ...
    let res =
      FStar_Getopt.parse_cmdline all_specs
        (fun i  ->
           ...
           FStar_ST.op_Colon_Equals file_list_ uu___1)
       in
  ...
  fun should_clear  ->
    ...
    (let r =
     ...
     (let uu___2 =
        let uu___3 =
          let uu___4 =
            FStar_List.map (fun uu___5  -> String uu___5) old_verify_module
             in
          List uu___4  in
...
let (set_ml_ish : unit -> unit) =
  fun uu___  -> set_option "MLish" (Bool true) 
      ...
      let uu___ =
        FStar_All.pipe_right setting
          (FStar_Util.try_find
             (fun uu___1  ->
                match uu___1 with
                | (path,uu___2) -> matches_path m_components path))
         in
      match uu___ with
  ...
  fun m  ->
    ...
    | FStar_Pervasives_Native.Some extract_setting ->
        ((let uu___2 =
            ...
            let uu___5 = get_extract_module ()  in (uu___3, uu___4, uu___5)
             in
          match uu___2 with
          | ([],[],[]) -> ()
          | uu___3 ->
...
let (set_options : Prims.string -> FStar_Getopt.parse_cmdline_res) =
  fun s  ->
    ...
    with
    | File_argument s1 ->
        let uu___1 = FStar_Util.format1 "File %s is not a valid option" s1
           in
