...
let rec (specs_with_types :
  ...
  fun uu____5055  ->
    ...
    (FStar_Getopt.noshort, "__debug_embedding",
      (WithSideEffect
         (((fun uu____6980  -> FStar_ST.op_Colon_Equals debug_embedding true)),
           (Const (Bool true)))),
      "Debug messages for embeddings/unembeddings of natively compiled terms");
    (FStar_Getopt.noshort, "eager_embedding",
      (WithSideEffect
         (((fun uu____7024  -> FStar_ST.op_Colon_Equals eager_embedding true)),
           (Const (Bool true)))),
      "Eagerly embed and unembed terms to primitive operations and plugins: not recommended except for benchmarking");
    (104, "help",
      (WithSideEffect
         (((fun uu____7070  ->
              (let uu____7072 = specs ()  in display_usage_aux uu____7072);
              FStar_All.exit Prims.int_zero)), (Const (Bool true)))),
      "Display this information")]

and (specs : unit -> FStar_Getopt.opt Prims.list) =
  fun uu____7103  ->
    let uu____7106 = specs_with_types ()  in
    FStar_List.map
      (fun uu____7137  ->
         match uu____7137 with
         | (short,long,typ,doc) ->
             let uu____7159 =
               let uu____7173 = arg_spec_of_opt_type long typ  in
               (short, long, uu____7173, doc)  in
             mk_spec uu____7159) uu____7106

let (settable : Prims.string -> Prims.bool) =
  fun uu___8_7188  ->
    match uu___8_7188 with
    | "abort_on" -> true
    | "admit_except" -> true
  ...
  FStar_All.pipe_right all_specs
    (FStar_List.filter
       (fun uu____7405  ->
          ...
          | (uu____7420,x,uu____7422,uu____7423) -> settable x))
  
let (display_usage : unit -> unit) =
  fun uu____7439  ->
    let uu____7440 = specs ()  in display_usage_aux uu____7440
...
let (uu___is_File_argument : Prims.exn -> Prims.bool) =
  fun projectee  ->
    match projectee with
    | File_argument uu____7472 -> true
    | uu____7475 -> false
  
let (__proj__File_argument__item__uu___ : Prims.exn -> Prims.string) =
  fun projectee  ->
    match projectee with | File_argument uu____7485 -> uu____7485
  ...
  fun uu____7538  ->
    let res =
      FStar_Getopt.parse_cmdline all_specs
        (fun i  ->
           let uu____7544 =
             let uu____7548 = FStar_ST.op_Bang file_list_  in
             FStar_List.append uu____7548 [i]  in
           FStar_ST.op_Colon_Equals file_list_ uu____7544)
       in
    let uu____7605 =
      let uu____7609 = FStar_ST.op_Bang file_list_  in
      FStar_List.map FStar_Common.try_convert_file_name_to_mixed uu____7609
       in
    (res, uu____7605)
  
let (file_list : unit -> Prims.string Prims.list) =
  fun uu____7651  -> FStar_ST.op_Bang file_list_ 
let (restore_cmd_line_options : Prims.bool -> FStar_Getopt.parse_cmdline_res)
  ...
  fun should_clear  ->
    ...
    (let r =
       let uu____7694 = specs ()  in
       FStar_Getopt.parse_cmdline uu____7694 (fun x  -> ())  in
     (let uu____7701 =
        let uu____7707 =
          let uu____7708 =
            FStar_List.map (fun _7712  -> String _7712) old_verify_module  in
          List uu____7708  in
        ("verify_module", uu____7707)  in
      set_option' uu____7701);
     r)
  ...
  fun f  ->
    ...
    let f2 =
      let uu____7728 =
        let uu____7730 =
          let uu____7732 =
            let uu____7734 = FStar_Util.get_file_extension f1  in
            FStar_String.length uu____7734  in
          (FStar_String.length f1) - uu____7732  in
        uu____7730 - Prims.int_one  in
      FStar_String.substring f1 Prims.int_zero uu____7728  in
    FStar_String.lowercase f2
  
let (should_verify : Prims.string -> Prims.bool) =
  fun m  ->
    let uu____7747 = get_lax ()  in
    if uu____7747
    then false
    else
...
let (should_verify_file : Prims.string -> Prims.bool) =
  fun fn  ->
    let uu____7768 = module_name_of_file_name fn  in should_verify uu____7768
...
let (dont_gen_projectors : Prims.string -> Prims.bool) =
  fun m  ->
    let uu____7796 = get___temp_no_proj ()  in
    FStar_All.pipe_right uu____7796 (FStar_List.existsb (module_name_eq m))
  
let (should_print_message : Prims.string -> Prims.bool) =
  fun m  ->
    let uu____7814 = should_verify m  in
    if uu____7814 then m <> "Prims" else false
  
let (include_path : unit -> Prims.string Prims.list) =
  fun uu____7831  ->
    let cache_dir =
      let uu____7836 = get_cache_dir ()  in
      match uu____7836 with
      | FStar_Pervasives_Native.None  -> []
      | FStar_Pervasives_Native.Some c -> [c]  in
    let uu____7850 = get_no_default_includes ()  in
    if uu____7850
    then
      let uu____7856 = get_include ()  in
      FStar_List.append cache_dir uu____7856
    else
      (let lib_paths =
         let uu____7867 = FStar_Util.expand_environment_variable "FSTAR_LIB"
            in
       ...
       let uu____7910 =
         let uu____7914 =
           let uu____7918 = get_include ()  in
           FStar_List.append uu____7918 ["."]  in
         FStar_List.append lib_paths uu____7914  in
       FStar_List.append cache_dir uu____7910)
  
let (find_file : Prims.string -> Prims.string FStar_Pervasives_Native.option)
  =
  let file_map = FStar_Util.smap_create (Prims.of_int (100))  in
  fun filename  ->
    let uu____7949 = FStar_Util.smap_try_find file_map filename  in
        ...
        let result =
          try
            (fun uu___519_7980  ->
               match () with
               | () ->
                   let uu____7984 = FStar_Util.is_path_absolute filename  in
                   if uu____7984
                   then
                     ...
                     (let uu____8000 =
                        let uu____8004 = include_path ()  in
                        FStar_List.rev uu____8004  in
                      FStar_Util.find_map uu____8000
                        (fun p  ->
                           let path =
                             if p = "."
                             then filename
          ...
          with | uu___518_8032 -> FStar_Pervasives_Native.None  in
...
let (prims_basename : unit -> Prims.string) =
  fun uu____8083  ->
    let uu____8084 = prims ()  in FStar_Util.basename uu____8084
  
let (pervasives : unit -> Prims.string) =
  fun uu____8092  ->
    let filename = "FStar.Pervasives.fst"  in
    let uu____8096 = find_file filename  in
...
let (pervasives_basename : unit -> Prims.string) =
  fun uu____8115  ->
    let uu____8116 = pervasives ()  in FStar_Util.basename uu____8116
  
let (pervasives_native_basename : unit -> Prims.string) =
  fun uu____8124  ->
    let filename = "FStar.Pervasives.Native.fst"  in
    let uu____8128 = find_file filename  in
...
let (prepend_cache_dir : Prims.string -> Prims.string) =
  fun fpath  ->
    let uu____8168 = get_cache_dir ()  in
    match uu____8168 with
    | FStar_Pervasives_Native.None  -> fpath
    | FStar_Pervasives_Native.Some x ->
        let uu____8177 = FStar_Util.basename fpath  in
        FStar_Util.join_paths x uu____8177
...
let (parse_settings :
  ...
  fun ns  ->
    ...
    let with_cache f s =
      let uu____8299 = FStar_Util.smap_try_find cache s  in
    ...
    let parse_one_setting s =
      ...
      then ([], true)
        ...
        then ([], false)
          ...
          then
            (let path =
               let uu____8453 = FStar_Util.substring_from s Prims.int_one  in
               path_of_text uu____8453  in
             (path, false))
          else
            (let s1 =
               if FStar_Util.starts_with s "+"
    ...
    let uu____8476 =
      FStar_All.pipe_right ns
       ...
       in
    FStar_All.pipe_right uu____8476 FStar_List.rev
  
let (__temp_no_proj : Prims.string -> Prims.bool) =
  fun s  ->
    let uu____8641 = get___temp_no_proj ()  in
    FStar_All.pipe_right uu____8641 (FStar_List.contains s)
  
let (__temp_fast_implicits : unit -> Prims.bool) =
  fun uu____8656  -> lookup_opt "__temp_fast_implicits" as_bool 
let (admit_smt_queries : unit -> Prims.bool) =
  fun uu____8665  -> get_admit_smt_queries () 
let (admit_except : unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____8674  -> get_admit_except () 
let (cache_checked_modules : unit -> Prims.bool) =
  fun uu____8681  -> get_cache_checked_modules () 
let (cache_off : unit -> Prims.bool) = fun uu____8688  -> get_cache_off () 
let (cmi : unit -> Prims.bool) = fun uu____8695  -> get_cmi () 
type codegen_t =
  | OCaml 
  | FSharp 
  | Kremlin 
...
let (uu___is_OCaml : codegen_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | OCaml  -> true | uu____8705 -> false
  
let (uu___is_FSharp : codegen_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | FSharp  -> true | uu____8716 -> false
  
let (uu___is_Kremlin : codegen_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Kremlin  -> true | uu____8727 -> false
  
let (uu___is_Plugin : codegen_t -> Prims.bool) =
  fun projectee  ->
    match projectee with | Plugin  -> true | uu____8738 -> false
  
let (codegen : unit -> codegen_t FStar_Pervasives_Native.option) =
  fun uu____8747  ->
    let uu____8748 = get_codegen ()  in
    FStar_Util.map_opt uu____8748
      (fun uu___9_8754  ->
         match uu___9_8754 with
         | "OCaml" -> OCaml
         | "FSharp" -> FSharp
         | "Kremlin" -> Kremlin
         | "Plugin" -> Plugin
...
let (codegen_libs : unit -> Prims.string Prims.list Prims.list) =
  fun uu____8773  ->
    let uu____8774 = get_codegen_lib ()  in
    FStar_All.pipe_right uu____8774
      (FStar_List.map (fun x  -> FStar_Util.split x "."))
  
let (debug_any : unit -> Prims.bool) =
  fun uu____8800  -> let uu____8801 = get_debug ()  in uu____8801 <> [] 
let (debug_module : Prims.string -> Prims.bool) =
  fun modul  ->
    let uu____8818 = get_debug ()  in
...
let (debug_at_level : Prims.string -> debug_level_t -> Prims.bool) =
  fun modul  ->
    fun level  ->
      (let uu____8843 = get_debug ()  in
       FStar_All.pipe_right uu____8843
         (FStar_List.existsb (module_name_eq modul)))
        && (debug_level_geq level)
  
let (defensive : unit -> Prims.bool) =
  fun uu____8858  -> let uu____8859 = get_defensive ()  in uu____8859 <> "no" 
let (defensive_fail : unit -> Prims.bool) =
  fun uu____8869  ->
    let uu____8870 = get_defensive ()  in uu____8870 = "fail"
  
let (dep : unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____8882  -> get_dep () 
let (dep_ninja : unit -> Prims.string) = fun uu____8889  -> get_dep_ninja () 
let (detail_errors : unit -> Prims.bool) =
  fun uu____8896  -> get_detail_errors () 
let (detail_hint_replay : unit -> Prims.bool) =
  fun uu____8903  -> get_detail_hint_replay () 
let (doc : unit -> Prims.bool) = fun uu____8910  -> get_doc () 
let (dump_module : Prims.string -> Prims.bool) =
  fun s  ->
    let uu____8920 = get_dump_module ()  in
    FStar_All.pipe_right uu____8920 (FStar_List.existsb (module_name_eq s))
  
let (eager_subtyping : unit -> Prims.bool) =
  fun uu____8935  -> get_eager_subtyping () 
let (expose_interfaces : unit -> Prims.bool) =
  fun uu____8942  -> get_expose_interfaces () 
let (fs_typ_app : Prims.string -> Prims.bool) =
  fun filename  ->
    let uu____8952 = FStar_ST.op_Bang light_off_files  in
    FStar_List.contains filename uu____8952
  
let (full_context_dependency : unit -> Prims.bool) = fun uu____8988  -> true 
let (hide_uvar_nums : unit -> Prims.bool) =
  fun uu____8996  -> get_hide_uvar_nums () 
let (hint_info : unit -> Prims.bool) =
  fun uu____9003  -> (get_hint_info ()) || (get_query_stats ()) 
let (hint_file : unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____9012  -> get_hint_file () 
let (ide : unit -> Prims.bool) = fun uu____9019  -> get_ide () 
let (print : unit -> Prims.bool) = fun uu____9026  -> get_print () 
let (print_in_place : unit -> Prims.bool) =
  fun uu____9033  -> get_print_in_place () 
let profile : 'a . (unit -> 'a) -> ('a -> Prims.string) -> 'a =
  fun f  ->
    fun msg  ->
      let uu____9064 = get_profile ()  in
      if uu____9064
      then
        let uu____9067 = FStar_Util.record_time f  in
        match uu____9067 with
        | (a,time) ->
            ((let uu____9078 = FStar_Util.string_of_int time  in
              let uu____9080 = msg a  in
...
let (initial_fuel : unit -> Prims.int) =
  fun uu____9091  ->
    let uu____9092 = get_initial_fuel ()  in
    let uu____9094 = get_max_fuel ()  in Prims.min uu____9092 uu____9094
  
let (initial_ifuel : unit -> Prims.int) =
  fun uu____9102  ->
    let uu____9103 = get_initial_ifuel ()  in
    let uu____9105 = get_max_ifuel ()  in Prims.min uu____9103 uu____9105
  
let (interactive : unit -> Prims.bool) =
  fun uu____9113  -> (get_in ()) || (get_ide ()) 
let (lax : unit -> Prims.bool) = fun uu____9120  -> get_lax () 
let (load : unit -> Prims.string Prims.list) = fun uu____9129  -> get_load () 
let (legacy_interactive : unit -> Prims.bool) = fun uu____9136  -> get_in () 
let (lsp_server : unit -> Prims.bool) = fun uu____9143  -> get_lsp () 
let (log_queries : unit -> Prims.bool) =
  fun uu____9150  -> get_log_queries () 
let (keep_query_captions : unit -> Prims.bool) =
  fun uu____9157  -> (log_queries ()) && (get_keep_query_captions ()) 
let (log_types : unit -> Prims.bool) = fun uu____9164  -> get_log_types () 
let (max_fuel : unit -> Prims.int) = fun uu____9171  -> get_max_fuel () 
let (max_ifuel : unit -> Prims.int) = fun uu____9178  -> get_max_ifuel () 
let (min_fuel : unit -> Prims.int) = fun uu____9185  -> get_min_fuel () 
let (ml_ish : unit -> Prims.bool) = fun uu____9192  -> get_MLish () 
let (set_ml_ish : unit -> unit) =
  fun uu____9198  -> set_option "MLish" (Bool true) 
let (no_default_includes : unit -> Prims.bool) =
  fun uu____9207  -> get_no_default_includes () 
let (no_extract : Prims.string -> Prims.bool) =
  fun s  ->
    let uu____9217 = get_no_extract ()  in
    FStar_All.pipe_right uu____9217 (FStar_List.existsb (module_name_eq s))
  
let (normalize_pure_terms_for_extraction : unit -> Prims.bool) =
  fun uu____9232  -> get_normalize_pure_terms_for_extraction () 
let (no_location_info : unit -> Prims.bool) =
  fun uu____9239  -> get_no_location_info () 
let (no_plugins : unit -> Prims.bool) = fun uu____9246  -> get_no_plugins () 
let (no_smt : unit -> Prims.bool) = fun uu____9253  -> get_no_smt () 
let (output_dir : unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____9262  -> get_odir () 
let (ugly : unit -> Prims.bool) = fun uu____9269  -> get_ugly () 
let (print_bound_var_types : unit -> Prims.bool) =
  fun uu____9276  -> get_print_bound_var_types () 
let (print_effect_args : unit -> Prims.bool) =
  fun uu____9283  -> get_print_effect_args () 
let (print_implicits : unit -> Prims.bool) =
  fun uu____9290  -> get_print_implicits () 
let (print_real_names : unit -> Prims.bool) =
  fun uu____9297  -> (get_prn ()) || (get_print_full_names ()) 
let (print_universes : unit -> Prims.bool) =
  fun uu____9304  -> get_print_universes () 
let (print_z3_statistics : unit -> Prims.bool) =
  fun uu____9311  -> get_print_z3_statistics () 
let (query_stats : unit -> Prims.bool) =
  fun uu____9318  -> get_query_stats () 
let (record_hints : unit -> Prims.bool) =
  fun uu____9325  -> get_record_hints () 
let (reuse_hint_for : unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____9334  -> get_reuse_hint_for () 
let (silent : unit -> Prims.bool) = fun uu____9341  -> get_silent () 
let (smtencoding_elim_box : unit -> Prims.bool) =
  fun uu____9348  -> get_smtencoding_elim_box () 
let (smtencoding_nl_arith_native : unit -> Prims.bool) =
  fun uu____9355  ->
    let uu____9356 = get_smtencoding_nl_arith_repr ()  in
    uu____9356 = "native"
  
let (smtencoding_nl_arith_wrapped : unit -> Prims.bool) =
  fun uu____9366  ->
    let uu____9367 = get_smtencoding_nl_arith_repr ()  in
    uu____9367 = "wrapped"
  
let (smtencoding_nl_arith_default : unit -> Prims.bool) =
  fun uu____9377  ->
    let uu____9378 = get_smtencoding_nl_arith_repr ()  in
    uu____9378 = "boxwrap"
  
let (smtencoding_l_arith_native : unit -> Prims.bool) =
  fun uu____9388  ->
    let uu____9389 = get_smtencoding_l_arith_repr ()  in
    uu____9389 = "native"
  
let (smtencoding_l_arith_default : unit -> Prims.bool) =
  fun uu____9399  ->
    let uu____9400 = get_smtencoding_l_arith_repr ()  in
    uu____9400 = "boxwrap"
  
let (smtencoding_valid_intro : unit -> Prims.bool) =
  fun uu____9410  -> get_smtencoding_valid_intro () 
let (smtencoding_valid_elim : unit -> Prims.bool) =
  fun uu____9417  -> get_smtencoding_valid_elim () 
let (tactic_raw_binders : unit -> Prims.bool) =
  fun uu____9424  -> get_tactic_raw_binders () 
let (tactics_failhard : unit -> Prims.bool) =
  fun uu____9431  -> get_tactics_failhard () 
let (tactics_info : unit -> Prims.bool) =
  fun uu____9438  -> get_tactics_info () 
let (tactic_trace : unit -> Prims.bool) =
  fun uu____9445  -> get_tactic_trace () 
let (tactic_trace_d : unit -> Prims.int) =
  fun uu____9452  -> get_tactic_trace_d () 
let (tactics_nbe : unit -> Prims.bool) =
  fun uu____9459  -> get_tactics_nbe () 
let (tcnorm : unit -> Prims.bool) = fun uu____9466  -> get_tcnorm () 
let (timing : unit -> Prims.bool) = fun uu____9473  -> get_timing () 
let (trace_error : unit -> Prims.bool) =
  fun uu____9480  -> get_trace_error () 
let (unthrottle_inductives : unit -> Prims.bool) =
  fun uu____9487  -> get_unthrottle_inductives () 
let (unsafe_tactic_exec : unit -> Prims.bool) =
  fun uu____9494  -> get_unsafe_tactic_exec () 
let (use_eq_at_higher_order : unit -> Prims.bool) =
  fun uu____9501  -> get_use_eq_at_higher_order () 
let (use_hints : unit -> Prims.bool) = fun uu____9508  -> get_use_hints () 
let (use_hint_hashes : unit -> Prims.bool) =
  fun uu____9515  -> get_use_hint_hashes () 
let (use_native_tactics :
  unit -> Prims.string FStar_Pervasives_Native.option) =
  fun uu____9524  -> get_use_native_tactics () 
let (use_tactics : unit -> Prims.bool) =
  fun uu____9531  -> get_use_tactics () 
let (using_facts_from :
  unit -> (Prims.string Prims.list * Prims.bool) Prims.list) =
  fun uu____9547  ->
    let uu____9548 = get_using_facts_from ()  in
...
let (vcgen_optimize_bind_as_seq : unit -> Prims.bool) =
  fun uu____9602  ->
    ...
    FStar_Option.isSome uu____9603
  
let (vcgen_decorate_with_type : unit -> Prims.bool) =
  fun uu____9614  ->
    let uu____9615 = get_vcgen_optimize_bind_as_seq ()  in
    match uu____9615 with
    | FStar_Pervasives_Native.Some "with_type" -> true
    | uu____9623 -> false
  
let (warn_default_effects : unit -> Prims.bool) =
  fun uu____9634  -> get_warn_default_effects () 
let (z3_exe : unit -> Prims.string) =
  fun uu____9641  ->
    let uu____9642 = get_smt ()  in
    match uu____9642 with
    | FStar_Pervasives_Native.None  -> FStar_Platform.exe "z3"
    | FStar_Pervasives_Native.Some s -> s
  
let (z3_cliopt : unit -> Prims.string Prims.list) =
  fun uu____9660  -> get_z3cliopt () 
let (z3_refresh : unit -> Prims.bool) = fun uu____9667  -> get_z3refresh () 
let (z3_rlimit : unit -> Prims.int) = fun uu____9674  -> get_z3rlimit () 
let (z3_rlimit_factor : unit -> Prims.int) =
  fun uu____9681  -> get_z3rlimit_factor () 
let (z3_seed : unit -> Prims.int) = fun uu____9688  -> get_z3seed () 
let (use_two_phase_tc : unit -> Prims.bool) =
  fun uu____9695  ->
    (get_use_two_phase_tc ()) &&
      (let uu____9697 = lax ()  in Prims.op_Negation uu____9697)
  
let (no_positivity : unit -> Prims.bool) =
  fun uu____9705  -> get_no_positivity () 
let (ml_no_eta_expand_coertions : unit -> Prims.bool) =
  fun uu____9712  -> get_ml_no_eta_expand_coertions () 
let (warn_error : unit -> Prims.string) =
  fun uu____9719  ->
    let uu____9720 = get_warn_error ()  in FStar_String.concat "" uu____9720
  
let (use_extracted_interfaces : unit -> Prims.bool) =
  fun uu____9731  -> get_use_extracted_interfaces () 
let (use_nbe : unit -> Prims.bool) = fun uu____9738  -> get_use_nbe () 
let (trivial_pre_for_unannotated_effectful_fns : unit -> Prims.bool) =
  fun uu____9745  -> get_trivial_pre_for_unannotated_effectful_fns () 
let with_saved_options : 'a . (unit -> 'a) -> 'a =
  fun f  ->
    let uu____9762 =
      let uu____9764 = trace_error ()  in Prims.op_Negation uu____9764  in
    if uu____9762
    then
      (push ();
       (let r =
          try
            (fun uu___722_9779  ->
               match () with
               | () -> let uu____9784 = f ()  in FStar_Util.Inr uu____9784)
              ()
          with | uu___721_9786 -> FStar_Util.Inl uu___721_9786  in
        pop ();
        (match r with
        ...
        FStar_All.pipe_right setting
          (FStar_Util.try_find
             (fun uu____9954  ->
                ...
                | (path,uu____9965) -> matches_path m_components path))
         in
      match uu____9912 with
      | FStar_Pervasives_Native.None  -> false
      | FStar_Pervasives_Native.Some (uu____9984,flag) -> flag
  
let (should_extract : Prims.string -> Prims.bool) =
  fun m  ->
    let m1 = FStar_String.lowercase m  in
    let uu____10013 = get_extract ()  in
    match uu____10013 with
    | FStar_Pervasives_Native.Some extract_setting ->
        ((let uu____10028 =
            let uu____10044 = get_no_extract ()  in
            let uu____10048 = get_extract_namespace ()  in
            let uu____10052 = get_extract_module ()  in
            (uu____10044, uu____10048, uu____10052)  in
    ...
    | FStar_Pervasives_Native.None  ->
        let should_extract_namespace m2 =
          let uu____10106 = get_extract_namespace ()  in
        ...
        let should_extract_module m2 =
          let uu____10134 = get_extract_module ()  in
          ...
          | l ->
           ...
           in
        (let uu____10156 = no_extract m1  in Prims.op_Negation uu____10156)
          &&
          (let uu____10159 =
             let uu____10170 = get_extract_namespace ()  in
             let uu____10174 = get_extract_module ()  in
             (uu____10170, uu____10174)  in
...
let (should_be_already_cached : Prims.string -> Prims.bool) =
  fun m  ->
    let uu____10214 = get_already_cached ()  in
...
let (error_flags : unit -> error_flag Prims.list) =
  ...
  fun uu____10247  ->
    ...
    let uu____10250 = FStar_Util.smap_try_find cache we  in
