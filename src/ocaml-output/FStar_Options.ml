...
let uu____5972 = (get___temp_no_proj ())
in (FStar_All.pipe_right uu____5972 (FStar_List.contains s))))


let __temp_fast_implicits : unit  ->  Prims.bool = (fun uu____5981 -> (lookup_opt "__temp_fast_implicits" as_bool))


let admit_smt_queries : unit  ->  Prims.bool = (fun uu____5986 -> (get_admit_smt_queries ()))


let admit_except : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____5993 -> (get_admit_except ()))


let cache_checked_modules : unit  ->  Prims.bool = (fun uu____5998 -> (get_cache_checked_modules ()))
...
let codegen : unit  ->  codegen_t FStar_Pervasives_Native.option = (fun uu____6029 -> (

let uu____6030 = (get_codegen ())
in (FStar_Util.map_opt uu____6030 (fun uu___45_6034 -> (match (uu___45_6034) with
| "OCaml" -> begin
OCaml
end
| "FSharp" -> begin
...
let codegen_libs : unit  ->  Prims.string Prims.list Prims.list = (fun uu____6044 -> (

let uu____6045 = (get_codegen_lib ())
in (FStar_All.pipe_right uu____6045 (FStar_List.map (fun x -> (FStar_Util.split x "."))))))


let debug_any : unit  ->  Prims.bool = (fun uu____6062 -> (

let uu____6063 = (get_debug ())
in (Prims.op_disEquality uu____6063 [])))


let debug_module : Prims.string  ->  Prims.bool = (fun modul -> (

let uu____6073 = (get_debug ())
in (FStar_All.pipe_right uu____6073 (FStar_List.contains modul))))


let debug_at_level : Prims.string  ->  debug_level_t  ->  Prims.bool = (fun modul level -> ((

let uu____6090 = (get_debug ())
in (FStar_All.pipe_right uu____6090 (FStar_List.contains modul))) && (debug_level_geq level)))


let defensive : unit  ->  Prims.bool = (fun uu____6099 -> (

let uu____6100 = (get_defensive ())
in (Prims.op_disEquality uu____6100 "no")))


let defensive_fail : unit  ->  Prims.bool = (fun uu____6105 -> (

let uu____6106 = (get_defensive ())
in (Prims.op_Equality uu____6106 "fail")))


let dep : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____6113 -> (get_dep ()))


let detail_errors : unit  ->  Prims.bool = (fun uu____6118 -> (get_detail_errors ()))


let detail_hint_replay : unit  ->  Prims.bool = (fun uu____6123 -> (get_detail_hint_replay ()))


let doc : unit  ->  Prims.bool = (fun uu____6128 -> (get_doc ()))


let dump_module : Prims.string  ->  Prims.bool = (fun s -> (

let uu____6134 = (get_dump_module ())
in (FStar_All.pipe_right uu____6134 (FStar_List.contains s))))


let eager_inference : unit  ->  Prims.bool = (fun uu____6143 -> (get_eager_inference ()))


let expose_interfaces : unit  ->  Prims.bool = (fun uu____6148 -> (get_expose_interfaces ()))


let fs_typ_app : Prims.string  ->  Prims.bool = (fun filename -> (

let uu____6154 = (FStar_ST.op_Bang light_off_files)
in (FStar_List.contains filename uu____6154)))


let full_context_dependency : unit  ->  Prims.bool = (fun uu____6188 -> true)


let hide_uvar_nums : unit  ->  Prims.bool = (fun uu____6193 -> (get_hide_uvar_nums ()))


let hint_info : unit  ->  Prims.bool = (fun uu____6198 -> ((get_hint_info ()) || (get_query_stats ())))


let hint_file : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____6205 -> (get_hint_file ()))


let ide : unit  ->  Prims.bool = (fun uu____6210 -> (get_ide ()))


let indent : unit  ->  Prims.bool = (fun uu____6215 -> (get_indent ()))


let initial_fuel : unit  ->  Prims.int = (fun uu____6220 -> (

let uu____6221 = (get_initial_fuel ())
in (

let uu____6222 = (get_max_fuel ())
in (Prims.min uu____6221 uu____6222))))


let initial_ifuel : unit  ->  Prims.int = (fun uu____6227 -> (

let uu____6228 = (get_initial_ifuel ())
in (

let uu____6229 = (get_max_ifuel ())
in (Prims.min uu____6228 uu____6229))))


let interactive : unit  ->  Prims.bool = (fun uu____6234 -> ((get_in ()) || (get_ide ())))


let lax : unit  ->  Prims.bool = (fun uu____6239 -> (get_lax ()))


let load : unit  ->  Prims.string Prims.list = (fun uu____6246 -> (get_load ()))


let legacy_interactive : unit  ->  Prims.bool = (fun uu____6251 -> (get_in ()))


let log_queries : unit  ->  Prims.bool = (fun uu____6256 -> (get_log_queries ()))


let log_types : unit  ->  Prims.bool = (fun uu____6261 -> (get_log_types ()))


let max_fuel : unit  ->  Prims.int = (fun uu____6266 -> (get_max_fuel ()))


let max_ifuel : unit  ->  Prims.int = (fun uu____6271 -> (get_max_ifuel ()))


let min_fuel : unit  ->  Prims.int = (fun uu____6276 -> (get_min_fuel ()))


let ml_ish : unit  ->  Prims.bool = (fun uu____6281 -> (get_MLish ()))


let set_ml_ish : unit  ->  unit = (fun uu____6286 -> (set_option "MLish" (Bool (true))))


let n_cores : unit  ->  Prims.int = (fun uu____6291 -> (get_n_cores ()))


let no_default_includes : unit  ->  Prims.bool = (fun uu____6296 -> (get_no_default_includes ()))
...
let uu____6303 = (get_no_extract ())
in (FStar_All.pipe_right uu____6303 (FStar_Util.for_some (fun f -> (Prims.op_Equality (FStar_String.lowercase f) s1)))))))


let normalize_pure_terms_for_extraction : unit  ->  Prims.bool = (fun uu____6314 -> (get_normalize_pure_terms_for_extraction ()))


let no_location_info : unit  ->  Prims.bool = (fun uu____6319 -> (get_no_location_info ()))


let no_smt : unit  ->  Prims.bool = (fun uu____6324 -> (get_no_smt ()))


let output_dir : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____6331 -> (get_odir ()))


let ugly : unit  ->  Prims.bool = (fun uu____6336 -> (get_ugly ()))


let print_bound_var_types : unit  ->  Prims.bool = (fun uu____6341 -> (get_print_bound_var_types ()))


let print_effect_args : unit  ->  Prims.bool = (fun uu____6346 -> (get_print_effect_args ()))


let print_implicits : unit  ->  Prims.bool = (fun uu____6351 -> (get_print_implicits ()))


let print_real_names : unit  ->  Prims.bool = (fun uu____6356 -> ((get_prn ()) || (get_print_full_names ())))


let print_universes : unit  ->  Prims.bool = (fun uu____6361 -> (get_print_universes ()))


let print_z3_statistics : unit  ->  Prims.bool = (fun uu____6366 -> ((get_print_z3_statistics ()) || (get_query_stats ())))


let query_stats : unit  ->  Prims.bool = (fun uu____6371 -> (get_query_stats ()))


let record_hints : unit  ->  Prims.bool = (fun uu____6376 -> (get_record_hints ()))


let reuse_hint_for : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____6383 -> (get_reuse_hint_for ()))


let silent : unit  ->  Prims.bool = (fun uu____6388 -> (get_silent ()))


let smtencoding_elim_box : unit  ->  Prims.bool = (fun uu____6393 -> (get_smtencoding_elim_box ()))


let smtencoding_nl_arith_native : unit  ->  Prims.bool = (fun uu____6398 -> (

let uu____6399 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____6399 "native")))


let smtencoding_nl_arith_wrapped : unit  ->  Prims.bool = (fun uu____6404 -> (

let uu____6405 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____6405 "wrapped")))


let smtencoding_nl_arith_default : unit  ->  Prims.bool = (fun uu____6410 -> (

let uu____6411 = (get_smtencoding_nl_arith_repr ())
in (Prims.op_Equality uu____6411 "boxwrap")))


let smtencoding_l_arith_native : unit  ->  Prims.bool = (fun uu____6416 -> (

let uu____6417 = (get_smtencoding_l_arith_repr ())
in (Prims.op_Equality uu____6417 "native")))


let smtencoding_l_arith_default : unit  ->  Prims.bool = (fun uu____6422 -> (

let uu____6423 = (get_smtencoding_l_arith_repr ())
in (Prims.op_Equality uu____6423 "boxwrap")))


let tactic_raw_binders : unit  ->  Prims.bool = (fun uu____6428 -> (get_tactic_raw_binders ()))


let tactic_trace : unit  ->  Prims.bool = (fun uu____6433 -> (get_tactic_trace ()))


let tactic_trace_d : unit  ->  Prims.int = (fun uu____6438 -> (get_tactic_trace_d ()))


let timing : unit  ->  Prims.bool = (fun uu____6443 -> (get_timing ()))


let trace_error : unit  ->  Prims.bool = (fun uu____6448 -> (get_trace_error ()))


let unthrottle_inductives : unit  ->  Prims.bool = (fun uu____6453 -> (get_unthrottle_inductives ()))


let unsafe_tactic_exec : unit  ->  Prims.bool = (fun uu____6458 -> (get_unsafe_tactic_exec ()))


let use_eq_at_higher_order : unit  ->  Prims.bool = (fun uu____6463 -> (get_use_eq_at_higher_order ()))


let use_hints : unit  ->  Prims.bool = (fun uu____6468 -> (get_use_hints ()))


let use_hint_hashes : unit  ->  Prims.bool = (fun uu____6473 -> (get_use_hint_hashes ()))


let use_native_tactics : unit  ->  Prims.string FStar_Pervasives_Native.option = (fun uu____6480 -> (get_use_native_tactics ()))


let use_tactics : unit  ->  Prims.bool = (fun uu____6485 -> (get_use_tactics ()))


let using_facts_from : unit  ->  (FStar_Ident.path * Prims.bool) Prims.list = (fun uu____6496 -> (

let uu____6497 = (get_using_facts_from ())
...
let vcgen_optimize_bind_as_seq : unit  ->  Prims.bool = (fun uu____6527 -> (

let uu____6528 = (get_vcgen_optimize_bind_as_seq ())
in (FStar_Option.isSome uu____6528)))


let vcgen_decorate_with_type : unit  ->  Prims.bool = (fun uu____6535 -> (

let uu____6536 = (get_vcgen_optimize_bind_as_seq ())
...
let warn_default_effects : unit  ->  Prims.bool = (fun uu____6546 -> (get_warn_default_effects ()))


let z3_exe : unit  ->  Prims.string = (fun uu____6551 -> (

let uu____6552 = (get_smt ())
...
let z3_cliopt : unit  ->  Prims.string Prims.list = (fun uu____6562 -> (get_z3cliopt ()))


let z3_refresh : unit  ->  Prims.bool = (fun uu____6567 -> (get_z3refresh ()))


let z3_rlimit : unit  ->  Prims.int = (fun uu____6572 -> (get_z3rlimit ()))


let z3_rlimit_factor : unit  ->  Prims.int = (fun uu____6577 -> (get_z3rlimit_factor ()))


let z3_seed : unit  ->  Prims.int = (fun uu____6582 -> (get_z3seed ()))


let use_two_phase_tc : unit  ->  Prims.bool = (fun uu____6587 -> (get_use_two_phase_tc ()))


let no_positivity : unit  ->  Prims.bool = (fun uu____6592 -> (get_no_positivity ()))


let ml_no_eta_expand_coertions : unit  ->  Prims.bool = (fun uu____6597 -> (get_ml_no_eta_expand_coertions ()))


let warn_error : unit  ->  Prims.string = (fun uu____6602 -> (get_warn_error ()))


let use_extracted_interfaces : unit  ->  Prims.bool = (fun uu____6607 -> (get_use_extracted_interfaces ()))
...
let uu____6638 = (get_no_extract ())
in (

let uu____6641 = (get_extract_namespace ())
in (

let uu____6644 = (get_extract_module ())
in ((uu____6638), (uu____6641), (uu____6644)))))
...
let uu____6733 = (FStar_All.pipe_right setting (FStar_Util.try_find (fun uu____6767 -> (match (uu____6767) with
| (path, uu____6775) -> begin
...
| FStar_Pervasives_Native.Some (uu____6786, flag) -> begin
flag
end)))));
)
end
...
let uu____6806 = (get_extract_namespace ())
...
let uu____6822 = (get_extract_module ())
...
let uu____6834 = (no_extract m1)
in (not (uu____6834))) && (

let uu____6836 = (

let uu____6845 = (get_extract_namespace ())
in (

let uu____6848 = (get_extract_module ())
in ((uu____6845), (uu____6848))))
