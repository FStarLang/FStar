...
in (let _21_135 = (FStar_ST.op_Colon_Equals timing false)
in (FStar_ST.op_Colon_Equals inline_arith false))))))))))))))))))))))))))))))))))))))))))))))))))))
end))

let set_fstar_home = (fun _21_137 -> (match (()) with
| () -> begin
(let fh = (match ((FStar_ST.read fstar_home_opt)) with
| None -> begin
(let x = (FStar_Util.get_exec_dir ())
in (let x = (Prims.strcat x "/..")
in (let _21_141 = (FStar_ST.op_Colon_Equals _fstar_home x)
in (let _21_143 = (FStar_ST.op_Colon_Equals fstar_home_opt (Some (x)))
in x))))
end
| Some (x) -> begin
(let _21_147 = (FStar_ST.op_Colon_Equals _fstar_home x)
in x)
end)
in fh)
end))

let get_fstar_home = (fun _21_150 -> (match (()) with
| () -> begin
(match ((FStar_ST.read fstar_home_opt)) with
| None -> begin
(let _21_152 = (let _87_43 = (set_fstar_home ())
...
let find_file = (fun filename search_path -> (FStar_All.try_with (fun _21_162 -> (match (()) with
| () -> begin
(let _87_55 = if (FStar_Util.is_path_absolute filename) then begin
...
in (FStar_Util.map_option FStar_Util.normalize_file_path _87_55))
end)) (fun _21_161 -> (match (_21_161) with
| _21_165 -> begin
None
end))))

let prims = (fun _21_170 -> (match (()) with
| () -> begin
(match ((FStar_ST.read prims_ref)) with
| None -> begin
(let filen = "prims.fst"
in (match ((let _87_59 = (get_include_path ".")
in (find_file filen _87_59))) with
| Some (result) -> begin
result
end
| None -> begin
(let _87_61 = (let _87_60 = (FStar_Util.format1 "unable to find required file \"%s\" in the module search path.\n" filen)
...
if (doc = "") then begin
(let _87_70 = (FStar_Util.format1 "  --%s\n" flag)
in (FStar_Util.print_string _87_70))
end else begin
(let _87_71 = (FStar_Util.format2 "  --%s  %s\n" flag doc)
in (FStar_Util.print_string _87_71))
end
end
| FStar_Getopt.OneArg (_21_195, argname) -> begin
if (doc = "") then begin
(let _87_73 = (FStar_Util.format2 "  --%s %s\n" flag argname)
in (FStar_Util.print_string _87_73))
end else begin
(let _87_74 = (FStar_Util.format3 "  --%s %s  %s\n" flag argname doc)
in (FStar_Util.print_string _87_74))
end
end)
end)) specs)))

let rec specs = (fun _21_199 -> (match (()) with
| () -> begin
(let specs = ((FStar_Getopt.noshort, "admit_fsi", FStar_Getopt.OneArg (((fun x -> (let _87_84 = (let _87_83 = (FStar_ST.read admit_fsi)
in (x)::_87_83)
in (FStar_ST.op_Colon_Equals admit_fsi _87_84))), "module name")), "Treat .fsi as a .fst"))::((FStar_Getopt.noshort, "auto_deps", FStar_Getopt.ZeroArgs ((fun _21_201 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals auto_deps true)
end))), "automatically treat files discovered by --find_deps as dependencies."))::((FStar_Getopt.noshort, "admit_smt_queries", FStar_Getopt.OneArg (((fun s -> (let _87_89 = if (s = "true") then begin
true
end else begin
...
in ((dlevel x))::_87_111)
in (FStar_ST.op_Colon_Equals debug_level _87_112))), "Low|Medium|High|Extreme")), "Control the verbosity of debugging info"))::((FStar_Getopt.noshort, "dep", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dep (Some (x)))), "make|nubuild")), "Output the transitive closure of the dependency graph in a format suitable for the given tool"))::((FStar_Getopt.noshort, "dump_module", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals dump_module (Some (x)))), "module name")), ""))::((FStar_Getopt.noshort, "eager_inference", FStar_Getopt.ZeroArgs ((fun _21_210 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals eager_inference true)
end))), "Solve all type-inference constraints eagerly; more efficient but at the cost of generality"))::((FStar_Getopt.noshort, "find_deps", FStar_Getopt.ZeroArgs ((fun _21_211 -> (match (()) with
| () -> begin
(let _21_212 = (FStar_ST.op_Colon_Equals find_deps true)
in (FStar_ST.op_Colon_Equals auto_deps true))
end))), "find transitive dependencies given build-config other-files specifications."))::((FStar_Getopt.noshort, "fs_typ_app", FStar_Getopt.ZeroArgs ((fun _21_214 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals fs_typ_app true)
end))), "Allow the use of t<t1,...,tn> syntax for type applications; brittle since it clashes with the integer less-than operator"))::((FStar_Getopt.noshort, "fsi", FStar_Getopt.ZeroArgs ((fun _21_215 -> (match (()) with
| () -> begin
(set_interactive_fsi ())
end))), "fsi flag; A flag to indicate if type checking a fsi in the interactive mode"))::((FStar_Getopt.noshort, "fstar_home", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals fstar_home_opt (Some (x)))), "dir")), "Set the FSTAR_HOME variable to dir"))::((FStar_Getopt.noshort, "full_context_dependency", FStar_Getopt.ZeroArgs ((fun _21_217 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals full_context_dependency true)
end))), "Introduce unification variables that are dependent on the entire context (possibly expensive, but better for type inference (on, by default)"))::((FStar_Getopt.noshort, "hide_genident_nums", FStar_Getopt.ZeroArgs ((fun _21_218 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals hide_genident_nums true)
end))), "Don\'t print generated identifier numbers"))::((FStar_Getopt.noshort, "hide_uvar_nums", FStar_Getopt.ZeroArgs ((fun _21_219 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals hide_uvar_nums true)
end))), "Don\'t print unification variable numbers"))::((FStar_Getopt.noshort, "in", FStar_Getopt.ZeroArgs ((fun _21_220 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals interactive true)
end))), "Interactive mode; reads input from stdin"))::((FStar_Getopt.noshort, "in_context", FStar_Getopt.OneArg (((fun s -> (let _21_222 = (FStar_ST.op_Colon_Equals interactive true)
...
in (FStar_ST.op_Colon_Equals initial_ifuel _87_145))), "non-negative integer")), "Number of unrolling of inductive datatypes to try at first (default 1)"))::((FStar_Getopt.noshort, "inline_arith", FStar_Getopt.ZeroArgs ((fun _21_227 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals inline_arith true)
end))), "Inline definitions of arithmetic functions in the SMT encoding"))::((FStar_Getopt.noshort, "lax", FStar_Getopt.ZeroArgs ((fun _21_228 -> (match (()) with
| () -> begin
(let _21_229 = (FStar_ST.op_Colon_Equals pretype true)
in (FStar_ST.op_Colon_Equals verify false))
end))), "Run the lax-type checker only (admit all verification conditions)"))::((FStar_Getopt.noshort, "log_types", FStar_Getopt.ZeroArgs ((fun _21_231 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals log_types true)
end))), "Print types computed for data/val/let-bindings"))::((FStar_Getopt.noshort, "logQueries", FStar_Getopt.ZeroArgs ((fun _21_232 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals min_fuel _87_161))), "non-negative integer")), "Minimum number of unrolling of recursive functions to try (default 1)"))::((FStar_Getopt.noshort, "MLish", FStar_Getopt.ZeroArgs ((fun _21_236 -> (match (()) with
| () -> begin
...
in (FStar_ST.op_Colon_Equals n_cores _87_166))), "positive integer")), "Maximum number of cores to use for the solver (default 1)"))::((FStar_Getopt.noshort, "no_fs_typ_app", FStar_Getopt.ZeroArgs ((fun _21_238 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals fs_typ_app false)
end))), "Do not allow the use of t<t1,...,tn> syntax for type applications"))::((FStar_Getopt.noshort, "no_slack", FStar_Getopt.ZeroArgs ((fun _21_239 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals no_slack true)
end))), "Use the partially flow-insensitive variant of --rel2 (experimental)"))::((FStar_Getopt.noshort, "odir", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals outputDir (Some (x)))), "dir")), "Place output in directory dir"))::((FStar_Getopt.noshort, "prims", FStar_Getopt.OneArg (((fun x -> (FStar_ST.op_Colon_Equals prims_ref (Some (x)))), "file")), ""))::((FStar_Getopt.noshort, "print_before_norm", FStar_Getopt.ZeroArgs ((fun _21_242 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals norm_then_print false)
end))), "Do not normalize types before printing (for debugging)"))::((FStar_Getopt.noshort, "print_effect_args", FStar_Getopt.ZeroArgs ((fun _21_243 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_effect_args true)
end))), "Print inferred predicate transformers for all computation types"))::((FStar_Getopt.noshort, "print_fuels", FStar_Getopt.ZeroArgs ((fun _21_244 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_fuels true)
end))), "Print the fuel amounts used for each successful query"))::((FStar_Getopt.noshort, "print_implicits", FStar_Getopt.ZeroArgs ((fun _21_245 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_implicits true)
end))), "Print implicit arguments"))::((FStar_Getopt.noshort, "prn", FStar_Getopt.ZeroArgs ((fun _21_246 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals print_real_names true)
end))), "Print real names---you may want to use this in conjunction with logQueries"))::((FStar_Getopt.noshort, "serialize_mods", FStar_Getopt.ZeroArgs ((fun _21_247 -> (match (()) with
| () -> begin
...
end))), "Print the time it takes to verify each top-level definition"))::((FStar_Getopt.noshort, "trace_error", FStar_Getopt.ZeroArgs ((fun _21_253 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals trace_error true)
end))), "Don\'t print an error message; show an exception trace instead"))::((FStar_Getopt.noshort, "unthrottle_inductives", FStar_Getopt.ZeroArgs ((fun _21_254 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals unthrottle_inductives true)
end))), "Let the SMT solver unfold inductive types to arbitrary depths (may affect verifier performance)"))::((FStar_Getopt.noshort, "use_build_config", FStar_Getopt.ZeroArgs ((fun _21_255 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals use_build_config true)
end))), "Expect just a single file on the command line and no options; will read the \'build-config\' prelude from the file"))::((FStar_Getopt.noshort, "use_eq_at_higher_order", FStar_Getopt.ZeroArgs ((fun _21_256 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals use_eq_at_higher_order true)
end))), "Use equality constraints when comparing higher-order types; temporary"))::((FStar_Getopt.noshort, "use_native_int", FStar_Getopt.ZeroArgs ((fun _21_257 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals use_native_int true)
end))), "Extract the \'int\' type to platform-specific native int; you will need to link the generated code with the appropriate version of the prims library"))::((FStar_Getopt.noshort, "verify_module", FStar_Getopt.OneArg (((fun x -> (let _87_204 = (let _87_203 = (FStar_ST.read verify_module)
in (x)::_87_203)
in (FStar_ST.op_Colon_Equals verify_module _87_204))), "string")), "Name of the module to verify"))::((FStar_Getopt.noshort, "__temp_no_proj", FStar_Getopt.OneArg (((fun x -> (let _87_209 = (let _87_208 = (FStar_ST.read __temp_no_proj)
in (x)::_87_208)
in (FStar_ST.op_Colon_Equals __temp_no_proj _87_209))), "string")), "Don\'t generate projectors for this module"))::(('v', "version", FStar_Getopt.ZeroArgs ((fun _21_260 -> (let _21_262 = (display_version ())
in (FStar_All.exit 0)))), "Display version number"))::((FStar_Getopt.noshort, "warn_top_level_effects", FStar_Getopt.ZeroArgs ((fun _21_264 -> (match (()) with
| () -> begin
(FStar_ST.op_Colon_Equals warn_top_level_effects true)
end))), "Top-level effects are ignored, by default; turn this flag on to be warned when this happens"))::((FStar_Getopt.noshort, "z3timeout", FStar_Getopt.OneArg (((fun s -> (let _87_215 = (FStar_Util.int_of_string s)
in (FStar_ST.op_Colon_Equals z3timeout _87_215))), "t")), "Set the Z3 per-query (soft) timeout to t seconds (default 5)"))::[]
in (('h', "help", FStar_Getopt.ZeroArgs ((fun x -> (let _21_268 = (display_usage specs)
...
| _21_274 -> begin
(let _21_275 = (FStar_Util.print_string "Wrong argument to codegen flag\n")
in (let _21_277 = (let _87_218 = (specs ())
in (display_usage _87_218))
in (FStar_All.exit 1)))
end))
...
| _21_284 -> begin
(let _21_285 = (FStar_Util.print_string "Wrong argument to cardinality flag\n")
in (let _21_287 = (let _87_220 = (specs ())
in (display_usage _87_220))
in (FStar_All.exit 1)))
end))
and set_interactive_fsi = (fun _21_289 -> if (FStar_ST.read interactive) then begin
(FStar_ST.op_Colon_Equals interactive_fsi true)
end else begin
(let _21_291 = (FStar_Util.print_string "Set interactive flag first before setting interactive fsi flag\n")
in (let _21_293 = (let _87_222 = (specs ())
in (display_usage _87_222))
in (FStar_All.exit 1)))
end)
...
let dont_gen_projectors = (fun m -> (let _87_227 = (FStar_ST.read __temp_no_proj)
in (FStar_List.contains m _87_227)))

let should_print_message = (fun m -> (((should_verify m) && (not ((let _87_230 = (FStar_ST.read admit_fsi)
in (FStar_List.contains m _87_230))))) && (m <> "Prims")))

let set_options = (let no_smt_specs = (let _87_233 = (specs ())
in (FStar_All.pipe_right _87_233 (FStar_List.filter (fun _21_307 -> (match (_21_307) with
| (_21_301, name, _21_304, _21_306) -> begin
(name <> "smt")
end)))))
in (fun s -> (FStar_Getopt.parse_string no_smt_specs (fun _21_310 -> ()) s)))

let reset_options_string = (FStar_ST.alloc None)

let reset_options = (fun _21_312 -> (match (()) with
| () -> begin
(let _21_313 = (init_options ())
in (let res = (let _87_239 = (specs ())
in (FStar_Getopt.parse_cmdline _87_239 (fun x -> ())))
