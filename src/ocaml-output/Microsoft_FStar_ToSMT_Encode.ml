...
let withenv = (fun c _426062 -> (match (_426062) with
| (a, b) -> begin
(a, b, c)
end))

let vargs = (fun args -> (Support.List.filter (fun _426024 -> (match (_426024) with
...
let primitive_projector_by_pos = (fun env lid i -> (let fail = (fun _426084 -> (match (_426084) with
| () -> begin
...
in (let new_scope = (fun _426123 -> (match (_426123) with
| () -> begin
...
in (let y = (match ((Support.Microsoft.FStar.Util.find_map (! (scopes)) (fun _426131 -> (match (_426131) with
| (names, _) -> begin
...
| Some (_) -> begin
(let _426136 = (Support.Microsoft.FStar.Util.incr ctr)
...
in (let _426140 = (Support.Microsoft.FStar.Util.smap_add top_scope y true)
in y)))))
...
in (let next_id = (fun _426148 -> (match (_426148) with
| () -> begin
(let _426149 = (Support.Microsoft.FStar.Util.incr ctr)
in (! (ctr)))
end))
in (let fresh = (fun pfx -> (Support.Microsoft.FStar.Util.format2 "%s_%s" pfx (Support.Microsoft.FStar.Util.string_of_int (next_id ()))))
in (let string_const = (fun s -> (match ((Support.Microsoft.FStar.Util.find_map (! (scopes)) (fun _426158 -> (match (_426158) with
| (_, strings) -> begin
...
in (let _426165 = (Support.Microsoft.FStar.Util.smap_add top_scope s f)
in f))))
end))
in (let push = (fun _426168 -> (match (_426168) with
| () -> begin
(scopes := ((new_scope ()))::(! (scopes)))
end))
in (let pop = (fun _426170 -> (match (_426170) with
| () -> begin
(scopes := (Support.List.tl (! (scopes))))
end))
in (let mark = (fun _426172 -> (match (_426172) with
| () -> begin
(push ())
end))
in (let reset_mark = (fun _426174 -> (match (_426174) with
| () -> begin
(pop ())
end))
in (let commit_mark = (fun _426176 -> (match (_426176) with
| () -> begin
(match ((! (scopes))) with
| (hd1, hd2)::(next1, next2)::tl -> begin
(let _426189 = (Support.Microsoft.FStar.Util.smap_fold hd1 (fun key value v -> (Support.Microsoft.FStar.Util.smap_add next1 key value)) ())
in (let _426194 = (Support.Microsoft.FStar.Util.smap_fold hd2 (fun key value v -> (Support.Microsoft.FStar.Util.smap_add next2 key value)) ())
...
let print_env = (fun e -> ((Support.String.concat ", ") ((Support.List.map (fun _426025 -> (match (_426025) with
...
in (ysym, y, (let _426250 = env
in {bindings = (Binding_var ((x, y)))::env.bindings; depth = (env.depth + 1); tcenv = _426250.tcenv; warn = _426250.warn; cache = _426250.cache; nolabels = _426250.nolabels; use_zfuel_name = _426250.use_zfuel_name})))))
...
in (ysym, y, (let _426256 = env
in {bindings = (Binding_var ((x, y)))::env.bindings; depth = _426256.depth; tcenv = _426256.tcenv; warn = _426256.warn; cache = _426256.cache; nolabels = _426256.nolabels; use_zfuel_name = _426256.use_zfuel_name})))))

let push_term_var = (fun env x t -> (let _426261 = env
in {bindings = (Binding_var ((x, t)))::env.bindings; depth = _426261.depth; tcenv = _426261.tcenv; warn = _426261.warn; cache = _426261.cache; nolabels = _426261.nolabels; use_zfuel_name = _426261.use_zfuel_name}))

let lookup_term_var = (fun env a -> (match ((lookup_binding env (fun _426026 -> (match (_426026) with
...
in (ysym, y, (let _426281 = env
in {bindings = (Binding_tvar ((x, y)))::env.bindings; depth = (env.depth + 1); tcenv = _426281.tcenv; warn = _426281.warn; cache = _426281.cache; nolabels = _426281.nolabels; use_zfuel_name = _426281.use_zfuel_name})))))
...
in (let y = (Microsoft_FStar_ToSMT_Term.mkApp (ysym, []))
in (ysym, y, (let _426287 = env
in {bindings = (Binding_tvar ((x, y)))::env.bindings; depth = _426287.depth; tcenv = _426287.tcenv; warn = _426287.warn; cache = _426287.cache; nolabels = _426287.nolabels; use_zfuel_name = _426287.use_zfuel_name})))))

let push_typ_var = (fun env x t -> (let _426292 = env
in {bindings = (Binding_tvar ((x, t)))::env.bindings; depth = _426292.depth; tcenv = _426292.tcenv; warn = _426292.warn; cache = _426292.cache; nolabels = _426292.nolabels; use_zfuel_name = _426292.use_zfuel_name}))

let lookup_typ_var = (fun env a -> (match ((lookup_binding env (fun _426027 -> (match (_426027) with
...
in (fname, ftok, (let _426312 = env
in {bindings = (Binding_fvar ((x, fname, ((fun __dataconst_1 -> Some (__dataconst_1)) (Microsoft_FStar_ToSMT_Term.mkApp (ftok, []))), None)))::env.bindings; depth = _426312.depth; tcenv = _426312.tcenv; warn = _426312.warn; cache = _426312.cache; nolabels = _426312.nolabels; use_zfuel_name = _426312.use_zfuel_name})))))

let try_lookup_lid = (fun env a -> (lookup_binding env (fun _426028 -> (match (_426028) with
...
let push_free_var = (fun env x fname ftok -> (let _426334 = env
in {bindings = (Binding_fvar ((x, fname, ftok, None)))::env.bindings; depth = _426334.depth; tcenv = _426334.tcenv; warn = _426334.warn; cache = _426334.cache; nolabels = _426334.nolabels; use_zfuel_name = _426334.use_zfuel_name}))

let push_zfuel_name = (fun env x f -> (let _426343 = (lookup_lid env x)
in (match (_426343) with
| (t1, t2, _) -> begin
(let t3 = (Microsoft_FStar_ToSMT_Term.mkApp (f, ((Microsoft_FStar_ToSMT_Term.mkApp ("ZFuel", [])))::[]))
in (let _426345 = env
in {bindings = (Binding_fvar ((x, t1, t2, Some (t3))))::env.bindings; depth = _426345.depth; tcenv = _426345.tcenv; warn = _426345.warn; cache = _426345.cache; nolabels = _426345.nolabels; use_zfuel_name = _426345.use_zfuel_name}))
end)))

let lookup_free_var = (fun env a -> (let _426352 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_426352) with
...
let lookup_free_var_name = (fun env a -> (let _426376 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_426376) with
| (x, _, _) -> begin
x
end)))

let lookup_free_var_sym = (fun env a -> (let _426382 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_426382) with
| (name, sym, zf_opt) -> begin
(match (zf_opt) with
...
in (let ftok = (Support.String.strcat fname "@tok")
in (fname, ftok, (let _426409 = env
in {bindings = (Binding_ftvar ((x, fname, ((fun __dataconst_1 -> Some (__dataconst_1)) (Microsoft_FStar_ToSMT_Term.mkApp (ftok, []))))))::env.bindings; depth = _426409.depth; tcenv = _426409.tcenv; warn = _426409.warn; cache = _426409.cache; nolabels = _426409.nolabels; use_zfuel_name = _426409.use_zfuel_name})))))

let lookup_tlid = (fun env a -> (match ((lookup_binding env (fun _426029 -> (match (_426029) with
...
let push_free_tvar = (fun env x fname ftok -> (let _426428 = env
in {bindings = (Binding_ftvar ((x, fname, ftok)))::env.bindings; depth = _426428.depth; tcenv = _426428.tcenv; warn = _426428.warn; cache = _426428.cache; nolabels = _426428.nolabels; use_zfuel_name = _426428.use_zfuel_name}))
...
let tok_of_name = (fun env nm -> (Support.Microsoft.FStar.Util.find_map env.bindings (fun _426030 -> (match (_426030) with
...
let mkForall_fuel' = (fun n _426458 -> (match (_426458) with
| (pats, vars, body) -> begin
(let fallback = (fun _426460 -> (match (_426460) with
| () -> begin
...
(let _426463 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_426463) with
| (fsym, fterm) -> begin
...
let is_app = (fun _426031 -> (match (_426031) with
...
let encode_const = (fun _426032 -> (match (_426032) with
...
(let _426655 = (encode_binders None bs env)
in (match (_426655) with
| (vars, guards, env', decls, _) -> begin
(let app = (mk_ApplyT t vars)
in (let _426659 = (encode_knd' prekind k env' app)
in (match (_426659) with
| (k, decls') -> begin
...
and encode_binders = (fun fuel_opt bs env -> (let _426670 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding binders %s\n" (Microsoft_FStar_Absyn_Print.binders_to_string ", " bs))
end
in (let _426718 = ((Support.List.fold_left (fun _426677 b -> (match (_426677) with
| (vars, guards, env, decls, names) -> begin
(let _426712 = (match ((Support.Prims.fst b)) with
...
(let a = (unmangle a)
in (let _426689 = (gen_typ_var env a)
in (match (_426689) with
| (aasym, aa, env') -> begin
(let _426692 = (encode_knd' false k env aa)
in (match (_426692) with
...
(let x = (unmangle x)
in (let _426703 = (gen_term_var env x)
in (match (_426703) with
| (xxsym, xx, env') -> begin
(let _426706 = (encode_typ_pred' fuel_opt (norm_t env t) env xx)
...
in (let _426726 = (encode_typ_term t env)
in (match (_426726) with
| (t, decls) -> begin
...
(let _426744 = (encode_binders None binders env)
in (match (_426744) with
| (vars, guards, env', decls, _) -> begin
...
in (let _426750 = (encode_typ_pred' None (Microsoft_FStar_Absyn_Util.comp_result res) env' app)
in (match (_426750) with
...
in (let cvars = ((Support.List.filter (fun _426755 -> (match (_426755) with
| (x, _) -> begin
...
in (let _426776 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))))
end))))
end)))))
end))
...
(let _426806 = (match ((Microsoft_FStar_Tc_Normalize.normalize_refinement env.tcenv t0)) with
...
in (match (_426806) with
| (x, f) -> begin
(let _426809 = (encode_typ_term x.Microsoft_FStar_Absyn_Syntax.sort env)
in (match (_426809) with
| (base_t, decls) -> begin
(let _426813 = (gen_term_var env x.Microsoft_FStar_Absyn_Syntax.v)
in (match (_426813) with
| (x, xtm, env') -> begin
(let _426816 = (encode_formula f env')
in (match (_426816) with
| (refinement, decls') -> begin
(let encoding = (Microsoft_FStar_ToSMT_Term.mkAnd ((Microsoft_FStar_ToSMT_Term.mk_HasType xtm base_t), refinement))
in (let cvars = ((Support.List.filter (fun _426821 -> (match (_426821) with
| (y, _) -> begin
(y <> x)
...
in (let _426842 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))
end)))))
end))
end))
...
in (let _426851 = (encode_knd k env ttm)
in (match (_426851) with
| (t_has_k, decls) -> begin
...
(let is_full_app = (fun _426858 -> (match (_426858) with
| () -> begin
(let kk = (Microsoft_FStar_Tc_Recheck.recompute_kind head)
in (let _426863 = (Microsoft_FStar_Absyn_Util.kind_formals kk)
in (match (_426863) with
| (formals, _) -> begin
...
in (let _426870 = (encode_args args env)
in (match (_426870) with
| (args, decls) -> begin
...
| Microsoft_FStar_Absyn_Syntax.Typ_const (fv) -> begin
(let _426876 = (encode_args args env)
in (match (_426876) with
| (args, decls) -> begin
if (is_full_app ()) then begin
(let head = (lookup_free_tvar_name env fv)
in (let t = (Microsoft_FStar_ToSMT_Term.mkApp (head, (Support.List.map (fun _426033 -> (match (_426033) with
...
| Microsoft_FStar_Absyn_Syntax.Typ_uvar ((uv, k)) -> begin
(let ttm = (Microsoft_FStar_ToSMT_Term.mk_Typ_uvar (Support.Microsoft.FStar.Unionfind.uvar_id uv))
in (let _426892 = (encode_knd k env ttm)
in (match (_426892) with
| (t_has_k, decls) -> begin
(let d = Microsoft_FStar_ToSMT_Term.Assume ((t_has_k, None))
in (ttm, (d)::decls))
end)))
...
(let _426907 = (encode_binders None bs env)
in (match (_426907) with
| (vars, guards, env, decls, _) -> begin
(let _426910 = (encode_typ_term body env)
in (match (_426910) with
| (body, decls') -> begin
...
in (let kinding = (let _426933 = (encode_knd (Microsoft_FStar_Tc_Recheck.recompute_kind t0) env t)
in (match (_426933) with
| (ktm, decls'') -> begin
...
in (let _426936 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls))))))))))
end)
end))))
end))
...
(let _426971 = (e.Microsoft_FStar_Absyn_Syntax.tk := Some (t))
in (encode_exp e env))
end
...
(let fallback = (fun _426990 -> (match (_426990) with
| () -> begin
...
(let _427004 = (Support.Microsoft.FStar.Util.first_N nformals bs)
...
(let _427017 = (encode_binders None bs env)
in (match (_427017) with
| (vars, guards, envbody, decls, _) -> begin
(let _427020 = (encode_exp body envbody)
in (match (_427020) with
| (body, decls') -> begin
(let key_body = (Microsoft_FStar_ToSMT_Term.mkForall ([], vars, (Microsoft_FStar_ToSMT_Term.mkImp ((Microsoft_FStar_ToSMT_Term.mk_and_l guards), body))))
in (let cvars = (Microsoft_FStar_ToSMT_Term.free_variables key_body)
in (let tkey = (Microsoft_FStar_ToSMT_Term.mkForall ([], cvars, key_body))
...
in (let app = (mk_ApplyE f vars)
in (let _427042 = (encode_typ_pred' None tfun env f)
in (match (_427042) with
| (f_has_t, decls'') -> begin
...
in (let _427046 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (fsym, cvar_sorts, f_decls))
in (f, f_decls)))))
end)))))))
end)
end))))
...
(let _427085 = (encode_exp v1 env)
in (match (_427085) with
| (v1, decls1) -> begin
(let _427088 = (encode_exp v2 env)
...
(let _427111 = (encode_args args_e env)
in (match (_427111) with
| (args, decls) -> begin
(let encode_partial_app = (fun ht_opt -> (let _427116 = (encode_exp head env)
in (match (_427116) with
| (head, decls') -> begin
...
(let _427125 = (Support.Microsoft.FStar.Util.first_N (Support.List.length args_e) formals)
...
in (let _427130 = (encode_typ_pred' None ty env app_tm)
...
in (let encode_full_app = (fun fv -> (let _427137 = (lookup_free_var_sym env fv)
in (match (_427137) with
| (fname, fuel_args) -> begin
(let tm = (Microsoft_FStar_ToSMT_Term.mkApp' (fname, (Support.List.append fuel_args (Support.List.map (fun _426034 -> (match (_426034) with
| (Support.Microsoft.FStar.Util.Inl (t)) | (Support.Microsoft.FStar.Util.Inr (t)) -> begin
t
end)) args))))
in (tm, decls))
...
in (let _427144 = if ((Microsoft_FStar_Tc_Env.debug env.tcenv) (Microsoft_FStar_Options.Other ("186"))) then begin
...
in (let _427147 = if ((Microsoft_FStar_Tc_Env.debug env.tcenv) (Microsoft_FStar_Options.Other ("Encoding"))) then begin
...
(let _427189 = (encode_exp e1 env)
in (match (_427189) with
| (ee1, decls1) -> begin
(let env' = (push_term_var env x ee1)
in (let _427193 = (encode_exp e2 env')
in (match (_427193) with
| (ee2, decls2) -> begin
...
(let _427197 = (Microsoft_FStar_Tc_Errors.warn e.Microsoft_FStar_Absyn_Syntax.pos "Non-top-level recursive functions are not yet fully encoded to the SMT solver; you may not be able to prove some facts")
...
(let _427207 = (encode_exp e env)
in (match (_427207) with
| (scr, decls) -> begin
(let _427247 = (Support.List.fold_right (fun _427211 _427214 -> (match ((_427211, _427214)) with
| ((p, w, br), (else_case, decls)) -> begin
(let patterns = (encode_pat env p)
in (Support.List.fold_right (fun _427218 _427221 -> (match ((_427218, _427221)) with
| ((env0, pattern), (else_case, decls)) -> begin
(let guard = (pattern.guard scr)
in (let projections = (pattern.projections scr)
in (let env = ((Support.List.fold_left (fun env _427227 -> (match (_427227) with
| (x, t) -> begin
(match (x) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
...
in (let _427241 = (match (w) with
| None -> begin
(guard, [])
end
| Some (w) -> begin
(let _427238 = (encode_exp w env)
in (match (_427238) with
| (w, decls2) -> begin
...
in (match (_427241) with
| (guard, decls2) -> begin
(let _427244 = (encode_exp br env)
in (match (_427244) with
| (br, decls3) -> begin
...
and encode_one_pat = (fun env pat -> (let _427259 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding pattern %s\n" (Microsoft_FStar_Absyn_Print.pat_to_string pat))
end
in (let _427263 = (Microsoft_FStar_Tc_Util.decorated_pattern_as_either pat)
in (match (_427263) with
| (vars, pat_exp_or_typ) -> begin
(let _427284 = ((Support.List.fold_left (fun _427266 v -> (match (_427266) with
| (env, vars) -> begin
(match (v) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
(let _427274 = (gen_typ_var env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_427274) with
| (aa, _, env) -> begin
(env, ((v, (aa, Microsoft_FStar_ToSMT_Term.Type_sort)))::vars)
end))
end
| Support.Microsoft.FStar.Util.Inr (x) -> begin
(let _427281 = (gen_term_var env x.Microsoft_FStar_Absyn_Syntax.v)
in (match (_427281) with
| (xx, _, env) -> begin
(env, ((v, (xx, Microsoft_FStar_ToSMT_Term.Term_sort)))::vars)
end))
end)
end)) (env, [])) vars)
in (match (_427284) with
| (env, vars) -> begin
...
in (let pat_term = (fun _427354 -> (match (_427354) with
| () -> begin
...
and encode_args = (fun l env -> (let _427384 = ((Support.List.fold_left (fun _427364 x -> (match (_427364) with
| (tms, decls) -> begin
(match (x) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(let _427373 = (encode_typ_term t env)
in (match (_427373) with
| (t, decls') -> begin
((Support.Microsoft.FStar.Util.Inl (t))::tms, (Support.List.append decls decls'))
end))
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(let _427381 = (encode_exp e env)
...
in (match (_427384) with
| (l, decls) -> begin
((Support.List.rev l), decls)
end)))
and encode_formula = (fun phi env -> (let _427390 = (encode_formula_with_labels phi env)
in (match (_427390) with
| (t, vars, decls) -> begin
(match (vars) with
| [] -> begin
(t, decls)
...
in (let _427486 = (match ((Microsoft_FStar_Absyn_Util.compress_typ t).Microsoft_FStar_Absyn_Syntax.n) with
...
(let _427493 = (encode_binders None binders env)
in (match (_427493) with
| (vars, guards, env, decls, _) -> begin
(let _427509 = ((Support.List.unzip) ((Support.List.map (fun _426035 -> (match (_426035) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(encode_formula t env)
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(encode_exp e (let _427505 = env
in {bindings = _427505.bindings; depth = _427505.depth; tcenv = _427505.tcenv; warn = _427505.warn; cache = _427505.cache; nolabels = _427505.nolabels; use_zfuel_name = true}))
end))) patterns))
in (match (_427509) with
| (pats, decls') -> begin
...
in (let env = (let _427530 = env
in {bindings = _427530.bindings; depth = _427530.depth; tcenv = _427530.tcenv; warn = _427530.warn; cache = _427530.cache; nolabels = true; use_zfuel_name = _427530.use_zfuel_name})
in (let _427535 = (encode_formula (Microsoft_FStar_Absyn_Util.unmeta_typ pre) env)
in (match (_427535) with
| (pre, decls'') -> begin
(let _427538 = (encode_formula (Microsoft_FStar_Absyn_Util.unmeta_typ post) env)
in (match (_427538) with
| (post, decls''') -> begin
...
and encode_formula_with_labels = (fun phi env -> (let enc = (fun f l -> (let _427559 = (Support.Microsoft.FStar.Util.fold_map (fun decls x -> (match ((Support.Prims.fst x)) with
| Support.Microsoft.FStar.Util.Inl (t) -> begin
(let _427551 = (encode_typ_term t env)
in (match (_427551) with
| (t, decls') -> begin
((Support.List.append decls decls'), t)
end))
end
| Support.Microsoft.FStar.Util.Inr (e) -> begin
(let _427556 = (encode_exp e env)
in (match (_427556) with
| (e, decls') -> begin
...
in (let enc_prop_c = (fun f l -> (let _427579 = (Support.List.fold_right (fun t _427567 -> (match (_427567) with
| (phis, labs, decls) -> begin
(match ((Support.Prims.fst t)) with
| Support.Microsoft.FStar.Util.Inl (t) -> begin
(let _427573 = (encode_formula_with_labels t env)
in (match (_427573) with
...
in (match (_427579) with
| (phis, labs, decls) -> begin
((f phis), labs, decls)
end)))
in (let const_op = (fun f _427582 -> (f, [], []))
in (let un_op = (fun f l -> (f (Support.List.hd l)))
in (let bin_op = (fun f _426036 -> (match (_426036) with
| t1::t2::[] -> begin
(f (t1, t2))
end
| _ -> begin
...
in (let tri_op = (fun f _426037 -> (match (_426037) with
| t1::t2::t3::[] -> begin
(f (t1, t2, t3))
end
| _ -> begin
...
in (let eq_op = (fun _426038 -> (match (_426038) with
| _::_::e1::e2::[] -> begin
...
in (let mk_imp = (fun _426039 -> (match (_426039) with
| (Support.Microsoft.FStar.Util.Inl (lhs), _)::(Support.Microsoft.FStar.Util.Inl (rhs), _)::[] -> begin
(let _427629 = (encode_formula_with_labels rhs env)
in (match (_427629) with
...
| _ -> begin
(let _427640 = (encode_formula_with_labels lhs env)
in (match (_427640) with
...
in (let mk_ite = (fun _426040 -> (match (_426040) with
...
(let _427664 = (encode_formula_with_labels guard env)
in (match (_427664) with
| (g, labs1, decls1) -> begin
(let _427668 = (encode_formula_with_labels _then env)
in (match (_427668) with
| (t, labs2, decls2) -> begin
(let _427672 = (encode_formula_with_labels _else env)
...
(let _427693 = (encode_formula_with_labels phi' env)
in (match (_427693) with
| (phi, labs, decls) -> begin
...
(let _427725 = (encode_exp l env)
in (match (_427725) with
| (e, decls) -> begin
(let _427728 = (encode_function_type_as_formula (Some (e)) phi env)
in (match (_427728) with
| (f, decls') -> begin
...
| _ -> begin
(let _427733 = (encode_typ_term phi env)
in (match (_427733) with
| (tt, decls) -> begin
((Microsoft_FStar_ToSMT_Term.mk_Valid tt), [], decls)
end))
end))
in (let encode_q_body = (fun env bs ps body -> (let _427745 = (encode_binders None bs env)
in (match (_427745) with
| (vars, guards, env, decls, _) -> begin
(let _427761 = ((Support.List.unzip) ((Support.List.map (fun _426041 -> (match (_426041) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(encode_formula t env)
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(encode_exp e (let _427757 = env
in {bindings = _427757.bindings; depth = _427757.depth; tcenv = _427757.tcenv; warn = _427757.warn; cache = _427757.cache; nolabels = _427757.nolabels; use_zfuel_name = true}))
end))) ps))
in (match (_427761) with
| (pats, decls') -> begin
(let _427765 = (encode_formula_with_labels body env)
in (match (_427765) with
...
in (let _427766 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
...
(match (((Support.List.tryFind (fun _427778 -> (match (_427778) with
| (l, _) -> begin
...
(let _427791 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 ">>>> Got QALL [%s]\n" ((Microsoft_FStar_Absyn_Print.binders_to_string "; ") vars))
end
in (let _427799 = (encode_q_body env vars pats body)
in (match (_427799) with
...
(let _427812 = (encode_q_body env vars pats body)
in (match (_427812) with
| (vars, pats, guard, body, labs, decls) -> begin
...
let prims = (let _427818 = (fresh_fvar "a" Microsoft_FStar_ToSMT_Term.Type_sort)
in (match (_427818) with
| (asym, a) -> begin
(let _427821 = (fresh_fvar "x" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_427821) with
| (xsym, x) -> begin
(let _427824 = (fresh_fvar "y" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_427824) with
| (ysym, y) -> begin
...
in (let mk = (fun l v -> ((Support.List.collect (fun _427848 -> (match (_427848) with
| (_, b) -> begin
(b v)
end))) ((Support.List.filter (fun _427844 -> (match (_427844) with
| (l', _) -> begin
(Microsoft_FStar_Absyn_Syntax.lid_equals l l')
end))) prims)))
in (let is = (fun l -> ((Support.Microsoft.FStar.Util.for_some (fun _427854 -> (match (_427854) with
| (l', _) -> begin
(Microsoft_FStar_Absyn_Syntax.lid_equals l l')
end))) prims))
in {mk = mk; is = is}))))))))
...
in (let mk_unit = (fun _427860 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_bool = (fun _427865 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int = (fun _427872 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int_alias = (fun _427884 tt -> (Microsoft_FStar_ToSMT_Term.Assume (((Microsoft_FStar_ToSMT_Term.mkEq (tt, (Microsoft_FStar_ToSMT_Term.mkApp (Microsoft_FStar_Absyn_Const.int_lid.Microsoft_FStar_Absyn_Syntax.str, [])))), Some ("mapping to int; for now"))))::[])
in (let mk_str = (fun _427888 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_ref = (fun reft_name _427896 -> (let r = ("r", Microsoft_FStar_ToSMT_Term.Ref_sort)
...
in (fun t s tt -> (match ((Support.Microsoft.FStar.Util.find_opt (fun _427913 -> (match (_427913) with
| (l, _) -> begin
...
let rec encode_sigelt = (fun env se -> (let _427922 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
...
in (let _427930 = (encode_sigelt' env se)
in (match (_427930) with
| (g, e) -> begin
(match (g) with
| [] -> begin
...
(let _427981 = (new_typ_constant_and_tok_from_lid env lid)
in (match (_427981) with
...
(let _427999 = (new_typ_constant_and_tok_from_lid env lid)
in (match (_427999) with
| (tname, ttok, env) -> begin
(let _428008 = (match (t.Microsoft_FStar_Absyn_Syntax.n) with
...
| (tps, t) -> begin
(let _428015 = (encode_binders None tps env)
...
in (let t = (whnf env t)
in (let _428033 = if ((Support.Microsoft.FStar.Util.for_some (fun _426042 -> (match (_426042) with
...
in (let kindingAx = (let _428037 = (encode_knd (Microsoft_FStar_Tc_Recheck.recompute_kind t) env' app)
in (match (_428037) with
| (k, decls) -> begin
...
(let tt = (whnf env t)
in (let _428050 = (encode_free_var env lid t tt quals)
in (match (_428050) with
| (decls, env) -> begin
...
(let _428061 = (encode_formula f env)
in (match (_428061) with
| (f, decls) -> begin
...
(let _428077 = (new_typ_constant_and_tok_from_lid env t)
in (match (_428077) with
| (tname, ttok, env) -> begin
([], env)
end))
end
...
(let is_logical = ((Support.Microsoft.FStar.Util.for_some (fun _426043 -> (match (_426043) with
...
(let _428128 = (fresh_fvar "x" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_428128) with
| (xxsym, xx) -> begin
(let _428171 = ((Support.List.fold_left (fun _428131 l -> (match (_428131) with
| (out, decls) -> begin
(let data_t = (Microsoft_FStar_Tc_Env.lookup_datacon env.tcenv l)
in (let _428141 = (match ((Microsoft_FStar_Absyn_Util.function_formals data_t)) with
...
in (let _428159 = (encode_args indices env)
in (match (_428159) with
| (indices, decls') -> begin
(let _428160 = if ((Support.List.length indices) <> (Support.List.length vars)) then begin
(failwith "Impossible")
end
...
in (match (_428171) with
| (data_ax, decls) -> begin
(let _428174 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_428174) with
| (ffsym, ff) -> begin
...
in (let _428186 = (match ((Microsoft_FStar_Absyn_Util.compress_kind k).Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_428186) with
| (is_kind_arrow, formals, res) -> begin
(let _428193 = (encode_binders None formals env)
in (match (_428193) with
| (vars, guards, env', binder_decls, _) -> begin
(let projection_axioms = (fun tapp vars -> (match (((Support.Microsoft.FStar.Util.find_opt (fun _426044 -> (match (_426044) with
...
(let rec projectee = (fun i _426045 -> (match (_426045) with
| [] -> begin
i
end
| f::tl -> begin
...
in (let _428232 = (match ((Support.Microsoft.FStar.Util.first_N projectee_pos vars)) with
| (_, xx::suffix) -> begin
(xx, suffix)
end
| _ -> begin
...
in (let _428239 = (new_typ_constant_and_tok_from_lid env t)
in (match (_428239) with
| (tname, ttok, env) -> begin
...
in (let _428264 = (let tname_decl = (constructor_or_logic_type_decl (tname, ((Support.List.map (fun _428245 -> (match (_428245) with
| (n, s) -> begin
((Support.String.strcat tname n), s)
end))) vars), Microsoft_FStar_ToSMT_Term.Type_sort, (varops.next_id ())))
in (let _428261 = (match (vars) with
| [] -> begin
...
in (let pats = if ((not (is_logical)) && ((Support.Microsoft.FStar.Util.for_some (fun _426046 -> (match (_426046) with
...
in (match (_428264) with
| (decls, env) -> begin
(let kindingAx = (let _428267 = (encode_knd res env' tapp)
in (match (_428267) with
| (k, decls) -> begin
...
(let _428298 = (new_term_constant_and_tok_from_lid env d)
in (match (_428298) with
| (ddconstrsym, ddtok, env) -> begin
(let ddtok_tm = (Microsoft_FStar_ToSMT_Term.mkApp (ddtok, []))
in (let _428307 = (match ((Microsoft_FStar_Absyn_Util.function_formals t)) with
| Some ((f, c)) -> begin
...
in (match (_428307) with
| (formals, t_res) -> begin
(let _428310 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_428310) with
| (fuel_var, fuel_tm) -> begin
(let s_fuel_tm = (Microsoft_FStar_ToSMT_Term.mkApp ("SFuel", (fuel_tm)::[]))
in (let _428317 = (encode_binders (Some (fuel_tm)) formals env)
in (match (_428317) with
| (vars, guards, env', binder_decls, names) -> begin
(let projectors = ((Support.List.map (fun _426047 -> (match (_426047) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
...
in (let _428331 = (encode_typ_pred' None t env ddtok_tm)
in (match (_428331) with
| (tok_typing, decls3) -> begin
(let _428338 = (encode_binders (Some (s_fuel_tm)) formals env)
in (match (_428338) with
| (vars', guards', env'', decls_formals, _) -> begin
(let _428343 = (let xvars = (Support.List.map Microsoft_FStar_ToSMT_Term.mkFreeV vars')
in (let dapp = (Microsoft_FStar_ToSMT_Term.mkApp (ddconstrsym, xvars))
in (encode_typ_pred' (Some (fuel_tm)) t_res env'' dapp)))
in (match (_428343) with
...
in (let encode_elim = (fun _428350 -> (match (_428350) with
| () -> begin
(let _428353 = (Microsoft_FStar_Absyn_Util.head_and_args t_res)
in (match (_428353) with
| (head, args) -> begin
...
in (let _428359 = (encode_args args env')
in (match (_428359) with
| (encoded_args, arg_decls) -> begin
(let _428383 = (Support.List.fold_left (fun _428363 arg -> (match (_428363) with
| (env, arg_vars, eqns) -> begin
(match (arg) with
| Support.Microsoft.FStar.Util.Inl (targ) -> begin
(let _428371 = (gen_typ_var env (Microsoft_FStar_Absyn_Util.new_bvd None))
in (match (_428371) with
| (_, tv, env) -> begin
(env, (tv)::arg_vars, ((Microsoft_FStar_ToSMT_Term.mkEq (targ, tv)))::eqns)
end))
end
| Support.Microsoft.FStar.Util.Inr (varg) -> begin
(let _428378 = (gen_term_var env (Microsoft_FStar_Absyn_Util.new_bvd None))
in (match (_428378) with
| (_, xv, env) -> begin
...
| _ -> begin
(let _428403 = (Microsoft_FStar_Tc_Errors.warn drange (Support.Microsoft.FStar.Util.format2 "Constructor %s builds an unexpected type %s\n" (Microsoft_FStar_Absyn_Print.sli d) (Microsoft_FStar_Absyn_Print.typ_to_string head)))
in ([], []))
end)
end))
end))
in (let _428407 = (encode_elim ())
...
(let _428420 = (encode_signature env ses)
in (match (_428420) with
| (g, env) -> begin
(let _428432 = ((Support.List.partition (fun _426048 -> (match (_426048) with
...
in (let _428451 = (Support.Microsoft.FStar.Util.first_N nbinders formals)
in (match (_428451) with
...
(let _428501 = (Support.Microsoft.FStar.Util.first_N nformals binders)
in (match (_428501) with
| (bs0, rest) -> begin
...
(let _428510 = (eta_expand binders formals body tres)
in (match (_428510) with
| (binders, body) -> begin
...
in (let _428522 = (eta_expand [] formals e tres)
in (match (_428522) with
| (binders, body) -> begin
(binders, body, formals, tres)
end)))
end
...
in (Support.Prims.try_with (fun _428526 -> (match (_428526) with
| () -> begin
if ((Support.Microsoft.FStar.Util.for_some (fun _426049 -> (match (_426049) with
| Microsoft_FStar_Absyn_Syntax.Opaque -> begin
true
end
| _ -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _428548 -> (match (_428548) with
| (_, t, _) -> begin
(Microsoft_FStar_Absyn_Util.is_smt_lemma t)
end))) bindings) then begin
(((Support.List.collect (fun _428553 -> (match (_428553) with
| (flid, t, _) -> begin
...
(let _428575 = ((Support.List.fold_left (fun _428558 _428563 -> (match ((_428558, _428563)) with
| ((toks, typs, decls, env), (flid, t, _)) -> begin
(let _428564 = if (Microsoft_FStar_Absyn_Util.is_smt_lemma t) then begin
(raise (Let_rec_unencodeable))
end
in (let t_norm = (Microsoft_FStar_Absyn_Util.compress_typ (whnf env t))
in (let _428570 = (declare_top_level_let env (Support.Microsoft.FStar.Util.right flid) t t_norm)
in (match (_428570) with
| (tok, decl, env) -> begin
((((Support.Microsoft.FStar.Util.right flid), tok))::toks, (t_norm)::typs, (decl)::decls, env)
end))))
end)) ([], [], [], env)) bindings)
in (match (_428575) with
...
in if (((Support.Microsoft.FStar.Util.for_some (fun _426050 -> (match (_426050) with
...
(let _428604 = (destruct_bound_function flid t_norm e)
in (match (_428604) with
| (binders, body, formals, tres) -> begin
(let _428611 = (encode_binders None binders env)
in (match (_428611) with
| (vars, guards, env', binder_decls, _) -> begin
...
in (let _428618 = (encode_exp body env')
in (match (_428618) with
| (body, decls2) -> begin
...
in (let env0 = env
in (let _428638 = ((Support.List.fold_left (fun _428627 _428632 -> (match ((_428627, _428632)) with
...
in (match (_428638) with
| (gtoks, env) -> begin
(let gtoks = (Support.List.rev gtoks)
in (let encode_one_binding = (fun env0 _428647 t_norm _428654 -> (match ((_428647, _428654)) with
| ((flid, f, ftok, g, gtok), (_, _, e)) -> begin
(let _428659 = (destruct_bound_function flid t_norm e)
in (match (_428659) with
| (binders, body, formals, tres) -> begin
(let _428666 = (encode_binders None binders env)
in (match (_428666) with
| (vars, guards, env', binder_decls, _) -> begin
...
in (let _428676 = (encode_exp body env')
in (match (_428676) with
...
in (let _428699 = (let _428686 = (encode_binders None formals env0)
...
in (let _428696 = (let _428693 = (encode_typ_pred' None tres env gapp)
...
in (let _428715 = (Support.List.fold_left (fun _428703 _428707 -> (match ((_428703, _428707)) with
| ((decls, eqns, env0), (gtok, ty, bs)) -> begin
(let _428711 = (encode_one_binding env0 gtok ty bs)
in (match (_428711) with
| (decls', eqns', env0) -> begin
((decls')::decls, (Support.List.append eqns' eqns), env0)
end))
end)) ((decls)::[], [], env0) (Support.List.zip3 gtoks typs bindings))
in (match (_428715) with
| (decls, eqns, env0) -> begin
(let _428724 = ((Support.List.partition (fun _426051 -> (match (_426051) with
...
end)) (fun _428525 -> (match (_428525) with
| Let_rec_unencodeable -> begin
(let msg = ((Support.String.concat " and ") ((Support.List.map (fun _428534 -> (match (_428534) with
| (lb, _, _) -> begin
...
| None -> begin
(let _428751 = (encode_free_var env x t t_norm [])
in (match (_428751) with
| (decls, env) -> begin
(let _428756 = (lookup_lid env x)
in (match (_428756) with
| (n, x', _) -> begin
((n, x'), decls, env)
end))
end))
...
and encode_smt_lemma = (fun env lid t -> (let _428768 = (encode_function_type_as_formula None t env)
...
(let _428777 = (new_term_constant_and_tok_from_lid env lid)
in (match (_428777) with
| (vname, vtok, env) -> begin
(let arg_sorts = (match (t_norm.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((binders, _)) -> begin
((Support.List.map (fun _426052 -> (match (_426052) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
...
(let _428807 = (match ((Microsoft_FStar_Absyn_Util.function_formals t_norm)) with
...
| (formals, res) -> begin
(let _428811 = (new_term_constant_and_tok_from_lid env lid)
in (match (_428811) with
| (vname, vtok, env) -> begin
(let vtok_tm = (Microsoft_FStar_ToSMT_Term.mkApp (vtok, []))
in (let mk_disc_proj_axioms = (fun vapp vars -> ((Support.List.collect (fun _426053 -> (match (_426053) with
| Microsoft_FStar_Absyn_Syntax.Discriminator (d) -> begin
(let _428825 = (Support.Microsoft.FStar.Util.prefix vars)
in (match (_428825) with
| (_, (xxsym, _)) -> begin
...
(let _428838 = (Support.Microsoft.FStar.Util.prefix vars)
in (match (_428838) with
| (_, (xxsym, _)) -> begin
(let xx = (Microsoft_FStar_ToSMT_Term.mkFreeV (xxsym, Microsoft_FStar_ToSMT_Term.Term_sort))
...
in (let _428848 = (encode_binders None formals env)
in (match (_428848) with
...
in (let _428881 = (let vname_decl = Microsoft_FStar_ToSMT_Term.DeclFun ((vname, ((Support.List.map (fun _426054 -> (match (_426054) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
Microsoft_FStar_ToSMT_Term.Type_sort
end
| _ -> begin
...
in (let _428878 = (match (formals) with
| [] -> begin
(let _428865 = if (not ((head_normal env tt))) then begin
...
in (match (_428878) with
| (tok_decl, env) -> begin
((vname_decl)::tok_decl, env)
end)))
in (match (_428881) with
| (decls2, env) -> begin
(let _428884 = (encode_typ_pred' None res env' vapp)
in (match (_428884) with
| (ty_pred, decls3) -> begin
(let _428890 = if (not ((head_normal env tt))) then begin
(let _428887 = (encode_typ_pred' None tt env vtok_tm)
in (match (_428887) with
...
and encode_signature = (fun env ses -> ((Support.List.fold_left (fun _428897 se -> (match (_428897) with
| (g, env) -> begin
(let _428901 = (encode_sigelt env se)
in (match (_428901) with
| (g', env) -> begin
((Support.List.append g g'), env)
end))
end)) ([], env)) ses))

let encode_env_bindings = (fun env bindings -> (let encode_binding = (fun b _428908 -> (match (_428908) with
| (decls, env) -> begin
(match (b) with
| Microsoft_FStar_Tc_Env.Binding_var ((x, t0)) -> begin
(let _428916 = (new_term_constant env x)
in (match (_428916) with
| (xxsym, xx, env') -> begin
(let t1 = (norm_t env t0)
in (let _428920 = (encode_typ_pred' None t1 env xx)
...
(let _428930 = (new_typ_constant env a)
in (match (_428930) with
| (aasym, aa, env') -> begin
(let _428933 = (encode_knd k env aa)
in (match (_428933) with
| (k, decls') -> begin
...
in (let _428942 = (encode_free_var env x t t_norm [])
in (match (_428942) with
| (g, env') -> begin
((Support.List.append decls g), env')
end)))
end
| Microsoft_FStar_Tc_Env.Binding_sig (se) -> begin
(let _428947 = (encode_sigelt env se)
in (match (_428947) with
| (g, env') -> begin
((Support.List.append decls g), env')
end))
end)
...
let encode_labels = (fun labs -> (let prefix = ((Support.List.map (fun _428954 -> (match (_428954) with
| (l, _, _) -> begin
Microsoft_FStar_ToSMT_Term.DeclFun (((Support.Prims.fst l), [], Microsoft_FStar_ToSMT_Term.Bool_sort, None))
end))) labs)
in (let suffix = ((Support.List.collect (fun _428961 -> (match (_428961) with
| (l, _, _) -> begin
...
| e::_ -> begin
(let _428970 = e
in {bindings = _428970.bindings; depth = _428970.depth; tcenv = tcenv; warn = _428970.warn; cache = _428970.cache; nolabels = _428970.nolabels; use_zfuel_name = _428970.use_zfuel_name})
end))
...
let push_env = (fun _428978 -> (match (_428978) with
| () -> begin
(match ((! (last_env))) with
| [] -> begin
(failwith "Empty env stack")
...
| hd::tl -> begin
(let _428983 = (Microsoft_FStar_ToSMT_Term.push ())
in (let refs = (Support.Microsoft.FStar.Util.smap_copy hd.cache)
in (let top = (let _428986 = hd
in {bindings = _428986.bindings; depth = _428986.depth; tcenv = _428986.tcenv; warn = _428986.warn; cache = refs; nolabels = _428986.nolabels; use_zfuel_name = _428986.use_zfuel_name})
in (last_env := (top)::(hd)::tl))))
end)
end))

let pop_env = (fun _428989 -> (match (_428989) with
| () -> begin
(match ((! (last_env))) with
| [] -> begin
...
| _::tl -> begin
(let _428995 = (Microsoft_FStar_ToSMT_Term.pop ())
in (last_env := tl))
end)
end))

let mark_env = (fun _428997 -> (match (_428997) with
| () -> begin
(push_env ())
end))

let reset_mark_env = (fun _428998 -> (match (_428998) with
| () -> begin
(pop_env ())
end))

let commit_mark_env = (fun _428999 -> (match (_428999) with
| () -> begin
(let _429000 = (Microsoft_FStar_ToSMT_Term.commit_mark ())
...
let init = (fun tcenv -> (let _429011 = (init_env tcenv)
in (let _429013 = (Microsoft_FStar_ToSMT_Z3.init ())
in (Microsoft_FStar_ToSMT_Z3.giveZ3 ((Microsoft_FStar_ToSMT_Term.DefPrelude)::[])))))

let push = (fun msg -> (let _429016 = (push_env ())
in (let _429018 = (varops.push ())
in (Microsoft_FStar_ToSMT_Z3.push msg))))

let pop = (fun msg -> (let _429021 = ((Support.Prims.ignore) (pop_env ()))
in (let _429023 = (varops.pop ())
in (Microsoft_FStar_ToSMT_Z3.pop msg))))

let mark = (fun msg -> (let _429026 = (mark_env ())
in (let _429028 = (varops.mark ())
in (Microsoft_FStar_ToSMT_Z3.mark msg))))

let reset_mark = (fun msg -> (let _429031 = (reset_mark_env ())
in (let _429033 = (varops.reset_mark ())
in (Microsoft_FStar_ToSMT_Z3.reset_mark msg))))

let commit_mark = (fun msg -> (let _429036 = (commit_mark_env ())
in (let _429038 = (varops.commit_mark ())
...
in (let _429047 = (encode_sigelt env se)
in (match (_429047) with
| (decls, env) -> begin
(let _429048 = (set_env env)
...
in (let _429053 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
in (let env = (get_env tcenv)
in (let _429060 = (encode_signature (let _429056 = env
in {bindings = _429056.bindings; depth = _429056.depth; tcenv = _429056.tcenv; warn = false; cache = _429056.cache; nolabels = _429056.nolabels; use_zfuel_name = _429056.use_zfuel_name}) modul.Microsoft_FStar_Absyn_Syntax.exports)
in (match (_429060) with
| (decls, env) -> begin
...
in (let _429066 = (set_env (let _429064 = env
in {bindings = _429064.bindings; depth = _429064.depth; tcenv = _429064.tcenv; warn = true; cache = _429064.cache; nolabels = _429064.nolabels; use_zfuel_name = _429064.use_zfuel_name}))
in (let _429068 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
let solve = (fun tcenv q -> (let _429073 = (push (Support.Microsoft.FStar.Util.format1 "Starting query at %s" (Support.Microsoft.FStar.Range.string_of_range (Microsoft_FStar_Tc_Env.get_range tcenv))))
in (let pop = (fun _429076 -> (match (_429076) with
| () -> begin
...
in (let _429106 = (let env = (get_env tcenv)
in (let bindings = (Microsoft_FStar_Tc_Env.fold_env tcenv (fun bs b -> (b)::bs) [])
in (let _429089 = (encode_env_bindings env (Support.List.filter (fun _426055 -> (match (_426055) with
...
in (match (_429089) with
| (env_decls, env) -> begin
(let _429090 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding query formula: %s\n" (Microsoft_FStar_Absyn_Print.formula_to_string q))
end
in (let _429095 = (encode_formula_with_labels q env)
in (match (_429095) with
| (phi, labels, qdecls) -> begin
(let _429098 = (encode_labels labels)
in (match (_429098) with
...
in (let _429133 = (Microsoft_FStar_ToSMT_Z3.giveZ3 prefix)
in (let with_fuel = (fun _429138 -> (match (_429138) with
| (n, i) -> begin
...
in (let check = (fun _429140 -> (match (_429140) with
| () -> begin
...
in (let report = (fun _429146 -> (match (_429146) with
| (ok, errs) -> begin
if ok then begin
()
end else begin
...
in (let rec try_alt_configs = (fun errs _426056 -> (match (_426056) with
| [] -> begin
(report (false, errs))
end
| mi::[] -> begin
...
| mi::tl -> begin
(Microsoft_FStar_ToSMT_Z3.ask fresh labels (with_fuel mi) (fun _429166 -> (match (_429166) with
| (ok, errs') -> begin
(match (errs) with
| [] -> begin
(cb tl (ok, errs'))
...
and cb = (fun alt _429173 -> (match (_429173) with
| (ok, errs) -> begin
if ok then begin
()
end else begin
...
in (let _429174 = if (! (Microsoft_FStar_Options.admit_smt_queries)) then begin
()
end else begin
(check ())
end
...
in (let _429179 = (push "query")
in (let _429186 = (encode_formula_with_labels q env)
in (match (_429186) with
| (f, _, _) -> begin
(let _429187 = (pop "query")
...
let dummy = {Microsoft_FStar_Tc_Env.init = (fun _429196 -> ()); Microsoft_FStar_Tc_Env.push = (fun _429198 -> ()); Microsoft_FStar_Tc_Env.pop = (fun _429200 -> ()); Microsoft_FStar_Tc_Env.mark = (fun _429202 -> ()); Microsoft_FStar_Tc_Env.reset_mark = (fun _429204 -> ()); Microsoft_FStar_Tc_Env.commit_mark = (fun _429206 -> ()); Microsoft_FStar_Tc_Env.encode_modul = (fun _429208 _429210 -> ()); Microsoft_FStar_Tc_Env.encode_sig = (fun _429212 _429214 -> ()); Microsoft_FStar_Tc_Env.solve = (fun _429216 _429218 -> ()); Microsoft_FStar_Tc_Env.is_trivial = (fun _429220 _429222 -> false); Microsoft_FStar_Tc_Env.finish = (fun _429224 -> (match (_429224) with
| () -> begin
()
end)); Microsoft_FStar_Tc_Env.refresh = (fun _429225 -> (match (_429225) with
| () -> begin
()
end))}
