...
(let _45_722 = (encode_binders None binders env)
in (match (_45_722) with
| (vars, guards, env', decls, _) -> begin
...
in (let _45_728 = (Microsoft_FStar_Tc_Util.pure_or_ghost_pre_and_post env.tcenv res)
in (match (_45_728) with
| (pre_opt, res_t) -> begin
(let _45_731 = (encode_typ_pred' None res_t env' app)
in (match (_45_731) with
| (res_pred, decls') -> begin
(let _45_740 = (match (pre_opt) with
| None -> begin
((Microsoft_FStar_ToSMT_Term.mk_and_l guards), decls)
end
| Some (pre) -> begin
(let _45_737 = (encode_formula pre env')
in (match (_45_737) with
| (guard, decls0) -> begin
((Microsoft_FStar_ToSMT_Term.mk_and_l ((guard)::guards)), (Support.List.append decls decls0))
end))
end)
in (match (_45_740) with
| (guards, guard_decls) -> begin
(let t_interp = (Microsoft_FStar_ToSMT_Term.mkForall ((app)::[], vars, (Microsoft_FStar_ToSMT_Term.mkImp (guards, res_pred))))
in (let cvars = ((Support.List.filter (fun _45_745 -> (match (_45_745) with
| (x, _) -> begin
...
in (let _45_766 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))))
end))))
end))
end))
...
(let _45_996 = (Support.Microsoft.FStar.Util.first_N nformals bs)
...
(let _45_1009 = (encode_binders None bs env)
in (match (_45_1009) with
| (vars, guards, envbody, decls, _) -> begin
(let _45_1012 = (encode_exp body envbody)
in (match (_45_1012) with
| (body, decls') -> begin
(let key_body = (Microsoft_FStar_ToSMT_Term.mkForall ([], vars, (Microsoft_FStar_ToSMT_Term.mkImp ((Microsoft_FStar_ToSMT_Term.mk_and_l guards), body))))
in (let cvars = (Microsoft_FStar_ToSMT_Term.free_variables key_body)
in (let tkey = (Microsoft_FStar_ToSMT_Term.mkForall ([], cvars, key_body))
...
in (let app = (mk_ApplyE f vars)
in (let _45_1034 = (encode_typ_pred' None tfun env f)
in (match (_45_1034) with
| (f_has_t, decls'') -> begin
...
in (let _45_1038 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (fsym, cvar_sorts, f_decls))
in (f, f_decls)))))
end)))))))
end)
end))))
...
(match (((Support.List.tryFind (fun _45_1770 -> (match (_45_1770) with
| (l, _) -> begin
...
in (fun t s tt -> (match ((Support.Microsoft.FStar.Util.find_opt (fun _45_1905 -> (match (_45_1905) with
| (l, _) -> begin
...
(let _45_2120 = (fresh_fvar "x" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_45_2120) with
| (xxsym, xx) -> begin
(let _45_2163 = ((Support.List.fold_left (fun _45_2123 l -> (match (_45_2123) with
| (out, decls) -> begin
(let data_t = (Microsoft_FStar_Tc_Env.lookup_datacon env.tcenv l)
in (let _45_2133 = (match ((Microsoft_FStar_Absyn_Util.function_formals data_t)) with
...
in (let _45_2151 = (encode_args indices env)
in (match (_45_2151) with
| (indices, decls') -> begin
(let _45_2152 = if ((Support.List.length indices) <> (Support.List.length vars)) then begin
(failwith "Impossible")
end
...
in (match (_45_2163) with
| (data_ax, decls) -> begin
(let _45_2166 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
...
in (let _45_2224 = (match ((Support.Microsoft.FStar.Util.first_N projectee_pos vars)) with
| (_, xx::suffix) -> begin
(xx, suffix)
end
| _ -> begin
...
(let _45_2503 = (Support.Microsoft.FStar.Util.first_N nformals binders)
...
(let _45_2512 = (eta_expand binders formals body tres)
in (match (_45_2512) with
| (binders, body) -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _45_2550 -> (match (_45_2550) with
| (_, t, _) -> begin
(Microsoft_FStar_Absyn_Util.is_smt_lemma t)
end))) bindings) then begin
(((Support.List.collect (fun _45_2555 -> (match (_45_2555) with
| (flid, t, _) -> begin
...
(let _45_2577 = ((Support.List.fold_left (fun _45_2560 _45_2565 -> (match ((_45_2560, _45_2565)) with
| ((toks, typs, decls, env), (flid, t, _)) -> begin
(let _45_2566 = if (Microsoft_FStar_Absyn_Util.is_smt_lemma t) then begin
(raise (Let_rec_unencodeable))
end
in (let t_norm = (Microsoft_FStar_Absyn_Util.compress_typ (whnf env t))
in (let _45_2572 = (declare_top_level_let env (Support.Microsoft.FStar.Util.right flid) t t_norm)
in (match (_45_2572) with
| (tok, decl, env) -> begin
...
(let _45_2606 = (destruct_bound_function flid t_norm e)
in (match (_45_2606) with
| (binders, body, formals, tres) -> begin
(let _45_2613 = (encode_binders None binders env)
in (match (_45_2613) with
| (vars, guards, env', binder_decls, _) -> begin
...
in (let env0 = env
in (let _45_2640 = ((Support.List.fold_left (fun _45_2629 _45_2634 -> (match ((_45_2629, _45_2634)) with
...
in (let encode_one_binding = (fun env0 _45_2649 t_norm _45_2656 -> (match ((_45_2649, _45_2656)) with
| ((flid, f, ftok, g, gtok), (_, _, e)) -> begin
(let _45_2661 = (destruct_bound_function flid t_norm e)
in (match (_45_2661) with
| (binders, body, formals, tres) -> begin
(let _45_2668 = (encode_binders None binders env)
...
in (let _45_2678 = (encode_exp body env')
...
in (let _45_2701 = (let _45_2688 = (encode_binders None formals env0)
in (match (_45_2688) with
...
in (let _45_2698 = (let _45_2695 = (encode_typ_pred' None tres env gapp)
in (match (_45_2695) with
| (g_typing, d3) -> begin
...
in (let _45_2717 = (Support.List.fold_left (fun _45_2705 _45_2709 -> (match ((_45_2705, _45_2709)) with
| ((decls, eqns, env0), (gtok, ty, bs)) -> begin
(let _45_2713 = (encode_one_binding env0 gtok ty bs)
in (match (_45_2713) with
| (decls', eqns', env0) -> begin
((decls')::decls, (Support.List.append eqns' eqns), env0)
end))
end)) ((decls)::[], [], env0) (Support.List.zip3 gtoks typs bindings))
in (match (_45_2717) with
| (decls, eqns, env0) -> begin
(let _45_2726 = ((Support.List.partition (fun _45_28 -> (match (_45_28) with
...
(let _45_2779 = (new_term_constant_and_tok_from_lid env lid)
in (match (_45_2779) with
...
in (let _45_2812 = (match ((Microsoft_FStar_Absyn_Util.function_formals t_norm)) with
...
(let _45_2833 = (Support.Microsoft.FStar.Util.prefix vars)
...
(let _45_2846 = (Support.Microsoft.FStar.Util.prefix vars)
in (match (_45_2846) with
| (_, (xxsym, _)) -> begin
(let xx = (Microsoft_FStar_ToSMT_Term.mkFreeV (xxsym, Microsoft_FStar_ToSMT_Term.Term_sort))
...
in (let _45_2856 = (encode_binders None formals env)
in (match (_45_2856) with
| (vars, guards, env', decls1, _) -> begin
(let _45_2865 = (match (pre_opt) with
| None -> begin
((Microsoft_FStar_ToSMT_Term.mk_and_l guards), decls1)
end
| Some (p) -> begin
(let _45_2862 = (encode_formula p env')
in (match (_45_2862) with
| (g, ds) -> begin
((Microsoft_FStar_ToSMT_Term.mk_and_l ((g)::guards)), (Support.List.append decls1 ds))
end))
end)
in (match (_45_2865) with
| (guard, decls1) -> begin
...
in (let _45_2896 = (let vname_decl = Microsoft_FStar_ToSMT_Term.DeclFun ((vname, ((Support.List.map (fun _45_31 -> (match (_45_31) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
Microsoft_FStar_ToSMT_Term.Type_sort
end
| _ -> begin
...
in (let _45_2883 = (let env = (let _45_2878 = env
in {bindings = _45_2878.bindings; depth = _45_2878.depth; tcenv = _45_2878.tcenv; warn = _45_2878.warn; cache = _45_2878.cache; nolabels = _45_2878.nolabels; use_zfuel_name = _45_2878.use_zfuel_name; encode_non_total_function_typ = encode_non_total_function_typ})
...
in (let _45_2893 = (match (formals) with
| [] -> begin
...
in (match (_45_2896) with
| (decls2, env) -> begin
(let _45_2899 = (encode_typ_pred' None res_t env' vapp)
