...
let withenv = (fun c _429453 -> (match (_429453) with
| (a, b) -> begin
(a, b, c)
end))

let vargs = (fun args -> (Support.List.filter (fun _429415 -> (match (_429415) with
...
let primitive_projector_by_pos = (fun env lid i -> (let fail = (fun _429475 -> (match (_429475) with
| () -> begin
...
in (let new_scope = (fun _429514 -> (match (_429514) with
| () -> begin
...
in (let y = (match ((Support.Microsoft.FStar.Util.find_map (! (scopes)) (fun _429522 -> (match (_429522) with
| (names, _) -> begin
...
| Some (_) -> begin
(let _429527 = (Support.Microsoft.FStar.Util.incr ctr)
...
in (let _429531 = (Support.Microsoft.FStar.Util.smap_add top_scope y true)
in y)))))
...
in (let next_id = (fun _429539 -> (match (_429539) with
| () -> begin
(let _429540 = (Support.Microsoft.FStar.Util.incr ctr)
in (! (ctr)))
end))
in (let fresh = (fun pfx -> (Support.Microsoft.FStar.Util.format2 "%s_%s" pfx (Support.Microsoft.FStar.Util.string_of_int (next_id ()))))
in (let string_const = (fun s -> (match ((Support.Microsoft.FStar.Util.find_map (! (scopes)) (fun _429549 -> (match (_429549) with
| (_, strings) -> begin
...
in (let _429556 = (Support.Microsoft.FStar.Util.smap_add top_scope s f)
in f))))
end))
in (let push = (fun _429559 -> (match (_429559) with
| () -> begin
(scopes := ((new_scope ()))::(! (scopes)))
end))
in (let pop = (fun _429561 -> (match (_429561) with
| () -> begin
(scopes := (Support.List.tl (! (scopes))))
end))
in (let mark = (fun _429563 -> (match (_429563) with
| () -> begin
(push ())
end))
in (let reset_mark = (fun _429565 -> (match (_429565) with
| () -> begin
(pop ())
end))
in (let commit_mark = (fun _429567 -> (match (_429567) with
| () -> begin
(match ((! (scopes))) with
| (hd1, hd2)::(next1, next2)::tl -> begin
(let _429580 = (Support.Microsoft.FStar.Util.smap_fold hd1 (fun key value v -> (Support.Microsoft.FStar.Util.smap_add next1 key value)) ())
in (let _429585 = (Support.Microsoft.FStar.Util.smap_fold hd2 (fun key value v -> (Support.Microsoft.FStar.Util.smap_add next2 key value)) ())
...
let print_env = (fun e -> ((Support.String.concat ", ") ((Support.List.map (fun _429416 -> (match (_429416) with
...
in (ysym, y, (let _429641 = env
in {bindings = (Binding_var ((x, y)))::env.bindings; depth = (env.depth + 1); tcenv = _429641.tcenv; warn = _429641.warn; cache = _429641.cache; nolabels = _429641.nolabels; use_zfuel_name = _429641.use_zfuel_name})))))
...
in (ysym, y, (let _429647 = env
in {bindings = (Binding_var ((x, y)))::env.bindings; depth = _429647.depth; tcenv = _429647.tcenv; warn = _429647.warn; cache = _429647.cache; nolabels = _429647.nolabels; use_zfuel_name = _429647.use_zfuel_name})))))

let push_term_var = (fun env x t -> (let _429652 = env
in {bindings = (Binding_var ((x, t)))::env.bindings; depth = _429652.depth; tcenv = _429652.tcenv; warn = _429652.warn; cache = _429652.cache; nolabels = _429652.nolabels; use_zfuel_name = _429652.use_zfuel_name}))

let lookup_term_var = (fun env a -> (match ((lookup_binding env (fun _429417 -> (match (_429417) with
...
in (ysym, y, (let _429672 = env
in {bindings = (Binding_tvar ((x, y)))::env.bindings; depth = (env.depth + 1); tcenv = _429672.tcenv; warn = _429672.warn; cache = _429672.cache; nolabels = _429672.nolabels; use_zfuel_name = _429672.use_zfuel_name})))))
...
in (let y = (Microsoft_FStar_ToSMT_Term.mkApp (ysym, []))
in (ysym, y, (let _429678 = env
in {bindings = (Binding_tvar ((x, y)))::env.bindings; depth = _429678.depth; tcenv = _429678.tcenv; warn = _429678.warn; cache = _429678.cache; nolabels = _429678.nolabels; use_zfuel_name = _429678.use_zfuel_name})))))

let push_typ_var = (fun env x t -> (let _429683 = env
in {bindings = (Binding_tvar ((x, t)))::env.bindings; depth = _429683.depth; tcenv = _429683.tcenv; warn = _429683.warn; cache = _429683.cache; nolabels = _429683.nolabels; use_zfuel_name = _429683.use_zfuel_name}))

let lookup_typ_var = (fun env a -> (match ((lookup_binding env (fun _429418 -> (match (_429418) with
...
in (fname, ftok, (let _429703 = env
in {bindings = (Binding_fvar ((x, fname, ((fun __dataconst_1 -> Some (__dataconst_1)) (Microsoft_FStar_ToSMT_Term.mkApp (ftok, []))), None)))::env.bindings; depth = _429703.depth; tcenv = _429703.tcenv; warn = _429703.warn; cache = _429703.cache; nolabels = _429703.nolabels; use_zfuel_name = _429703.use_zfuel_name})))))

let try_lookup_lid = (fun env a -> (lookup_binding env (fun _429419 -> (match (_429419) with
...
let push_free_var = (fun env x fname ftok -> (let _429725 = env
in {bindings = (Binding_fvar ((x, fname, ftok, None)))::env.bindings; depth = _429725.depth; tcenv = _429725.tcenv; warn = _429725.warn; cache = _429725.cache; nolabels = _429725.nolabels; use_zfuel_name = _429725.use_zfuel_name}))

let push_zfuel_name = (fun env x f -> (let _429734 = (lookup_lid env x)
in (match (_429734) with
| (t1, t2, _) -> begin
(let t3 = (Microsoft_FStar_ToSMT_Term.mkApp (f, ((Microsoft_FStar_ToSMT_Term.mkApp ("ZFuel", [])))::[]))
in (let _429736 = env
in {bindings = (Binding_fvar ((x, t1, t2, Some (t3))))::env.bindings; depth = _429736.depth; tcenv = _429736.tcenv; warn = _429736.warn; cache = _429736.cache; nolabels = _429736.nolabels; use_zfuel_name = _429736.use_zfuel_name}))
end)))

let lookup_free_var = (fun env a -> (let _429743 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
...
let lookup_free_var_name = (fun env a -> (let _429767 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_429767) with
| (x, _, _) -> begin
x
end)))

let lookup_free_var_sym = (fun env a -> (let _429773 = (lookup_lid env a.Microsoft_FStar_Absyn_Syntax.v)
in (match (_429773) with
| (name, sym, zf_opt) -> begin
(match (zf_opt) with
...
in (let ftok = (Support.String.strcat fname "@tok")
in (fname, ftok, (let _429800 = env
in {bindings = (Binding_ftvar ((x, fname, ((fun __dataconst_1 -> Some (__dataconst_1)) (Microsoft_FStar_ToSMT_Term.mkApp (ftok, []))))))::env.bindings; depth = _429800.depth; tcenv = _429800.tcenv; warn = _429800.warn; cache = _429800.cache; nolabels = _429800.nolabels; use_zfuel_name = _429800.use_zfuel_name})))))

let lookup_tlid = (fun env a -> (match ((lookup_binding env (fun _429420 -> (match (_429420) with
...
let push_free_tvar = (fun env x fname ftok -> (let _429819 = env
in {bindings = (Binding_ftvar ((x, fname, ftok)))::env.bindings; depth = _429819.depth; tcenv = _429819.tcenv; warn = _429819.warn; cache = _429819.cache; nolabels = _429819.nolabels; use_zfuel_name = _429819.use_zfuel_name}))
...
let tok_of_name = (fun env nm -> (Support.Microsoft.FStar.Util.find_map env.bindings (fun _429421 -> (match (_429421) with
...
let mkForall_fuel' = (fun n _429849 -> (match (_429849) with
| (pats, vars, body) -> begin
(let fallback = (fun _429851 -> (match (_429851) with
| () -> begin
...
(let _429854 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_429854) with
| (fsym, fterm) -> begin
...
let is_app = (fun _429422 -> (match (_429422) with
...
let encode_const = (fun _429423 -> (match (_429423) with
...
(let _430046 = (encode_binders None bs env)
in (match (_430046) with
| (vars, guards, env', decls, _) -> begin
(let app = (mk_ApplyT t vars)
in (let _430050 = (encode_knd' prekind k env' app)
in (match (_430050) with
| (k, decls') -> begin
...
and encode_binders = (fun fuel_opt bs env -> (let _430061 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding binders %s\n" (Microsoft_FStar_Absyn_Print.binders_to_string ", " bs))
end
in (let _430109 = ((Support.List.fold_left (fun _430068 b -> (match (_430068) with
| (vars, guards, env, decls, names) -> begin
(let _430103 = (match ((Support.Prims.fst b)) with
...
(let a = (unmangle a)
in (let _430080 = (gen_typ_var env a)
in (match (_430080) with
| (aasym, aa, env') -> begin
(let _430083 = (encode_knd' false k env aa)
in (match (_430083) with
...
(let x = (unmangle x)
in (let _430094 = (gen_term_var env x)
in (match (_430094) with
| (xxsym, xx, env') -> begin
(let _430097 = (encode_typ_pred' fuel_opt (norm_t env t) env xx)
in (match (_430097) with
| (guard_x_t, decls') -> begin
((xxsym, Microsoft_FStar_ToSMT_Term.Term_sort), guard_x_t, env', decls', Support.Microsoft.FStar.Util.Inr (x))
end))
end)))
end)
in (match (_430103) with
| (v, g, env, decls', n) -> begin
((v)::vars, (g)::guards, env, (Support.List.append decls decls'), (n)::names)
end))
end)) ([], [], env, [], [])) bs)
in (match (_430109) with
| (vars, guards, env, decls, names) -> begin
...
in (let _430117 = (encode_typ_term t env)
in (match (_430117) with
| (t, decls) -> begin
...
(let _430135 = (encode_binders None binders env)
in (match (_430135) with
| (vars, guards, env', decls, _) -> begin
...
in (let _430141 = (encode_typ_pred' None (Microsoft_FStar_Absyn_Util.comp_result res) env' app)
in (match (_430141) with
...
in (let cvars = ((Support.List.filter (fun _430146 -> (match (_430146) with
| (x, _) -> begin
...
in (let _430167 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))))
end))))
end)))))
end))
...
(let _430197 = (match ((Microsoft_FStar_Tc_Normalize.normalize_refinement env.tcenv t0)) with
...
in (match (_430197) with
| (x, f) -> begin
(let _430200 = (encode_typ_term x.Microsoft_FStar_Absyn_Syntax.sort env)
in (match (_430200) with
| (base_t, decls) -> begin
(let _430204 = (gen_term_var env x.Microsoft_FStar_Absyn_Syntax.v)
in (match (_430204) with
| (x, xtm, env') -> begin
(let _430207 = (encode_formula f env')
...
in (let cvars = ((Support.List.filter (fun _430212 -> (match (_430212) with
| (y, _) -> begin
(y <> x)
...
in (let _430233 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls)))))))))))
end)))))
end))
end))
...
in (let _430242 = (encode_knd k env ttm)
in (match (_430242) with
| (t_has_k, decls) -> begin
...
(let is_full_app = (fun _430249 -> (match (_430249) with
| () -> begin
(let kk = (Microsoft_FStar_Tc_Recheck.recompute_kind head)
in (let _430254 = (Microsoft_FStar_Absyn_Util.kind_formals kk)
in (match (_430254) with
| (formals, _) -> begin
...
in (let _430261 = (encode_args args env)
...
| Microsoft_FStar_Absyn_Syntax.Typ_const (fv) -> begin
(let _430267 = (encode_args args env)
in (match (_430267) with
| (args, decls) -> begin
if (is_full_app ()) then begin
(let head = (lookup_free_tvar_name env fv)
in (let t = (Microsoft_FStar_ToSMT_Term.mkApp (head, (Support.List.map (fun _429424 -> (match (_429424) with
...
| Microsoft_FStar_Absyn_Syntax.Typ_uvar ((uv, k)) -> begin
(let ttm = (Microsoft_FStar_ToSMT_Term.mk_Typ_uvar (Support.Microsoft.FStar.Unionfind.uvar_id uv))
in (let _430283 = (encode_knd k env ttm)
in (match (_430283) with
| (t_has_k, decls) -> begin
(let d = Microsoft_FStar_ToSMT_Term.Assume ((t_has_k, None))
in (ttm, (d)::decls))
end)))
...
(let _430298 = (encode_binders None bs env)
in (match (_430298) with
| (vars, guards, env, decls, _) -> begin
(let _430301 = (encode_typ_term body env)
in (match (_430301) with
| (body, decls') -> begin
...
in (let kinding = (let _430324 = (encode_knd (Microsoft_FStar_Tc_Recheck.recompute_kind t0) env t)
in (match (_430324) with
| (ktm, decls'') -> begin
...
in (let _430327 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (tsym, cvar_sorts, t_decls))
in (t, t_decls))))))))))
end)
end))))
end))
...
(let _430362 = (e.Microsoft_FStar_Absyn_Syntax.tk := Some (t))
in (encode_exp e env))
end
...
(let fallback = (fun _430381 -> (match (_430381) with
| () -> begin
...
| None -> begin
(let _430385 = (Microsoft_FStar_Tc_Errors.warn e.Microsoft_FStar_Absyn_Syntax.pos "Losing precision when encoding a function literal")
in (fallback ()))
end
| Some (tfun) -> begin
...
(let _430397 = (Support.Microsoft.FStar.Util.first_N nformals bs)
...
(let _430410 = (encode_binders None bs env)
in (match (_430410) with
| (vars, guards, envbody, decls, _) -> begin
(let _430413 = (encode_exp body envbody)
in (match (_430413) with
| (body, decls') -> begin
(let key_body = (Microsoft_FStar_ToSMT_Term.mkForall ([], vars, (Microsoft_FStar_ToSMT_Term.mkImp ((Microsoft_FStar_ToSMT_Term.mk_and_l guards), body))))
in (let cvars = (Microsoft_FStar_ToSMT_Term.free_variables key_body)
in (let tkey = (Microsoft_FStar_ToSMT_Term.mkForall ([], cvars, key_body))
...
in (let app = (mk_ApplyE f vars)
in (let _430435 = (encode_typ_pred' None tfun env f)
...
in (let _430439 = (Support.Microsoft.FStar.Util.smap_add env.cache tkey.Microsoft_FStar_ToSMT_Term.hash (fsym, cvar_sorts, f_decls))
in (f, f_decls)))))
end)))))))
end)
end))))
...
(let _430478 = (encode_exp v1 env)
in (match (_430478) with
| (v1, decls1) -> begin
(let _430481 = (encode_exp v2 env)
...
(let _430504 = (encode_args args_e env)
in (match (_430504) with
| (args, decls) -> begin
(let encode_partial_app = (fun ht_opt -> (let _430509 = (encode_exp head env)
in (match (_430509) with
| (head, decls') -> begin
...
(let _430518 = (Support.Microsoft.FStar.Util.first_N (Support.List.length args_e) formals)
in (match (_430518) with
| (formals, rest) -> begin
...
in (let _430523 = (encode_typ_pred' None ty env app_tm)
in (match (_430523) with
...
in (let encode_full_app = (fun fv -> (let _430530 = (lookup_free_var_sym env fv)
in (match (_430530) with
| (fname, fuel_args) -> begin
(let tm = (Microsoft_FStar_ToSMT_Term.mkApp' (fname, (Support.List.append fuel_args (Support.List.map (fun _429425 -> (match (_429425) with
| (Support.Microsoft.FStar.Util.Inl (t)) | (Support.Microsoft.FStar.Util.Inr (t)) -> begin
t
end)) args))))
in (tm, decls))
...
in (let _430537 = if ((Microsoft_FStar_Tc_Env.debug env.tcenv) (Microsoft_FStar_Options.Other ("186"))) then begin
...
in (let _430540 = if ((Microsoft_FStar_Tc_Env.debug env.tcenv) (Microsoft_FStar_Options.Other ("Encoding"))) then begin
...
(let _430582 = (encode_exp e1 env)
in (match (_430582) with
| (ee1, decls1) -> begin
(let env' = (push_term_var env x ee1)
in (let _430586 = (encode_exp e2 env')
in (match (_430586) with
| (ee2, decls2) -> begin
...
(let _430590 = (Microsoft_FStar_Tc_Errors.warn e.Microsoft_FStar_Absyn_Syntax.pos "Non-top-level recursive functions are not yet fully encoded to the SMT solver; you may not be able to prove some facts")
...
(let _430600 = (encode_exp e env)
in (match (_430600) with
| (scr, decls) -> begin
(let _430640 = (Support.List.fold_right (fun _430604 _430607 -> (match ((_430604, _430607)) with
| ((p, w, br), (else_case, decls)) -> begin
(let patterns = (encode_pat env p)
in (Support.List.fold_right (fun _430611 _430614 -> (match ((_430611, _430614)) with
| ((env0, pattern), (else_case, decls)) -> begin
(let guard = (pattern.guard scr)
in (let projections = (pattern.projections scr)
in (let env = ((Support.List.fold_left (fun env _430620 -> (match (_430620) with
| (x, t) -> begin
(match (x) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
...
in (let _430634 = (match (w) with
| None -> begin
(guard, [])
end
| Some (w) -> begin
(let _430631 = (encode_exp w env)
in (match (_430631) with
| (w, decls2) -> begin
...
in (match (_430634) with
| (guard, decls2) -> begin
(let _430637 = (encode_exp br env)
in (match (_430637) with
| (br, decls3) -> begin
...
and encode_one_pat = (fun env pat -> (let _430652 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding pattern %s\n" (Microsoft_FStar_Absyn_Print.pat_to_string pat))
end
in (let _430656 = (Microsoft_FStar_Tc_Util.decorated_pattern_as_either pat)
in (match (_430656) with
| (vars, pat_exp_or_typ) -> begin
(let _430677 = ((Support.List.fold_left (fun _430659 v -> (match (_430659) with
| (env, vars) -> begin
(match (v) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
(let _430667 = (gen_typ_var env a.Microsoft_FStar_Absyn_Syntax.v)
...
| Support.Microsoft.FStar.Util.Inr (x) -> begin
(let _430674 = (gen_term_var env x.Microsoft_FStar_Absyn_Syntax.v)
in (match (_430674) with
| (xx, _, env) -> begin
...
in (let pat_term = (fun _430747 -> (match (_430747) with
| () -> begin
...
and encode_args = (fun l env -> (let _430777 = ((Support.List.fold_left (fun _430757 x -> (match (_430757) with
| (tms, decls) -> begin
(match (x) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(let _430766 = (encode_typ_term t env)
in (match (_430766) with
| (t, decls') -> begin
((Support.Microsoft.FStar.Util.Inl (t))::tms, (Support.List.append decls decls'))
end))
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(let _430774 = (encode_exp e env)
in (match (_430774) with
| (t, decls') -> begin
...
and encode_formula = (fun phi env -> (let _430783 = (encode_formula_with_labels phi env)
in (match (_430783) with
| (t, vars, decls) -> begin
(match (vars) with
| [] -> begin
(t, decls)
...
in (let _430879 = (match ((Microsoft_FStar_Absyn_Util.compress_typ t).Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_430879) with
| (binders, pre, post, patterns) -> begin
(let _430886 = (encode_binders None binders env)
in (match (_430886) with
| (vars, guards, env, decls, _) -> begin
(let _430902 = ((Support.List.unzip) ((Support.List.map (fun _429426 -> (match (_429426) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(encode_formula t env)
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(encode_exp e (let _430898 = env
in {bindings = _430898.bindings; depth = _430898.depth; tcenv = _430898.tcenv; warn = _430898.warn; cache = _430898.cache; nolabels = _430898.nolabels; use_zfuel_name = true}))
end))) patterns))
in (match (_430902) with
| (pats, decls') -> begin
...
in (let env = (let _430923 = env
in {bindings = _430923.bindings; depth = _430923.depth; tcenv = _430923.tcenv; warn = _430923.warn; cache = _430923.cache; nolabels = true; use_zfuel_name = _430923.use_zfuel_name})
in (let _430928 = (encode_formula (Microsoft_FStar_Absyn_Util.unmeta_typ pre) env)
in (match (_430928) with
| (pre, decls'') -> begin
(let _430931 = (encode_formula (Microsoft_FStar_Absyn_Util.unmeta_typ post) env)
in (match (_430931) with
| (post, decls''') -> begin
...
and encode_formula_with_labels = (fun phi env -> (let enc = (fun f l -> (let _430952 = (Support.Microsoft.FStar.Util.fold_map (fun decls x -> (match ((Support.Prims.fst x)) with
| Support.Microsoft.FStar.Util.Inl (t) -> begin
(let _430944 = (encode_typ_term t env)
in (match (_430944) with
| (t, decls') -> begin
((Support.List.append decls decls'), t)
end))
end
| Support.Microsoft.FStar.Util.Inr (e) -> begin
(let _430949 = (encode_exp e env)
in (match (_430949) with
| (e, decls') -> begin
((Support.List.append decls decls'), e)
end))
end)) [] l)
in (match (_430952) with
| (decls, args) -> begin
((f args), [], decls)
end)))
in (let enc_prop_c = (fun f l -> (let _430972 = (Support.List.fold_right (fun t _430960 -> (match (_430960) with
| (phis, labs, decls) -> begin
(match ((Support.Prims.fst t)) with
| Support.Microsoft.FStar.Util.Inl (t) -> begin
(let _430966 = (encode_formula_with_labels t env)
...
in (match (_430972) with
| (phis, labs, decls) -> begin
((f phis), labs, decls)
end)))
in (let const_op = (fun f _430975 -> (f, [], []))
in (let un_op = (fun f l -> (f (Support.List.hd l)))
in (let bin_op = (fun f _429427 -> (match (_429427) with
| t1::t2::[] -> begin
(f (t1, t2))
end
| _ -> begin
...
in (let tri_op = (fun f _429428 -> (match (_429428) with
| t1::t2::t3::[] -> begin
(f (t1, t2, t3))
end
| _ -> begin
...
in (let eq_op = (fun _429429 -> (match (_429429) with
| _::_::e1::e2::[] -> begin
...
in (let mk_imp = (fun _429430 -> (match (_429430) with
| (Support.Microsoft.FStar.Util.Inl (lhs), _)::(Support.Microsoft.FStar.Util.Inl (rhs), _)::[] -> begin
(let _431022 = (encode_formula_with_labels rhs env)
...
| _ -> begin
(let _431033 = (encode_formula_with_labels lhs env)
in (match (_431033) with
...
in (let mk_ite = (fun _429431 -> (match (_429431) with
...
(let _431057 = (encode_formula_with_labels guard env)
in (match (_431057) with
| (g, labs1, decls1) -> begin
(let _431061 = (encode_formula_with_labels _then env)
in (match (_431061) with
| (t, labs2, decls2) -> begin
(let _431065 = (encode_formula_with_labels _else env)
in (match (_431065) with
...
(let _431086 = (encode_formula_with_labels phi' env)
in (match (_431086) with
| (phi, labs, decls) -> begin
...
(let _431118 = (encode_exp l env)
in (match (_431118) with
| (e, decls) -> begin
(let _431121 = (encode_function_type_as_formula (Some (e)) phi env)
in (match (_431121) with
| (f, decls') -> begin
...
| _ -> begin
(let _431126 = (encode_typ_term phi env)
in (match (_431126) with
| (tt, decls) -> begin
((Microsoft_FStar_ToSMT_Term.mk_Valid tt), [], decls)
end))
end))
in (let encode_q_body = (fun env bs ps body -> (let _431138 = (encode_binders None bs env)
in (match (_431138) with
| (vars, guards, env, decls, _) -> begin
(let _431154 = ((Support.List.unzip) ((Support.List.map (fun _429432 -> (match (_429432) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
(encode_formula t env)
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(encode_exp e (let _431150 = env
in {bindings = _431150.bindings; depth = _431150.depth; tcenv = _431150.tcenv; warn = _431150.warn; cache = _431150.cache; nolabels = _431150.nolabels; use_zfuel_name = true}))
end))) ps))
in (match (_431154) with
| (pats, decls') -> begin
(let _431158 = (encode_formula_with_labels body env)
in (match (_431158) with
...
in (let _431159 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
...
(match (((Support.List.tryFind (fun _431171 -> (match (_431171) with
| (l, _) -> begin
...
(let _431184 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 ">>>> Got QALL [%s]\n" ((Microsoft_FStar_Absyn_Print.binders_to_string "; ") vars))
end
in (let _431192 = (encode_q_body env vars pats body)
in (match (_431192) with
...
(let _431205 = (encode_q_body env vars pats body)
in (match (_431205) with
| (vars, pats, guard, body, labs, decls) -> begin
...
let prims = (let _431211 = (fresh_fvar "a" Microsoft_FStar_ToSMT_Term.Type_sort)
in (match (_431211) with
| (asym, a) -> begin
(let _431214 = (fresh_fvar "x" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_431214) with
| (xsym, x) -> begin
(let _431217 = (fresh_fvar "y" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_431217) with
| (ysym, y) -> begin
...
in (let mk = (fun l v -> ((Support.List.collect (fun _431241 -> (match (_431241) with
| (_, b) -> begin
(b v)
end))) ((Support.List.filter (fun _431237 -> (match (_431237) with
| (l', _) -> begin
(Microsoft_FStar_Absyn_Syntax.lid_equals l l')
end))) prims)))
in (let is = (fun l -> ((Support.Microsoft.FStar.Util.for_some (fun _431247 -> (match (_431247) with
| (l', _) -> begin
(Microsoft_FStar_Absyn_Syntax.lid_equals l l')
end))) prims))
in {mk = mk; is = is}))))))))
...
in (let mk_unit = (fun _431253 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_bool = (fun _431258 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int = (fun _431265 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_int_alias = (fun _431277 tt -> (Microsoft_FStar_ToSMT_Term.Assume (((Microsoft_FStar_ToSMT_Term.mkEq (tt, (Microsoft_FStar_ToSMT_Term.mkApp (Microsoft_FStar_Absyn_Const.int_lid.Microsoft_FStar_Absyn_Syntax.str, [])))), Some ("mapping to int; for now"))))::[])
in (let mk_str = (fun _431281 tt -> (let typing_pred = (Microsoft_FStar_ToSMT_Term.mk_HasType x tt)
...
in (let mk_ref = (fun reft_name _431289 -> (let r = ("r", Microsoft_FStar_ToSMT_Term.Ref_sort)
...
in (fun t s tt -> (match ((Support.Microsoft.FStar.Util.find_opt (fun _431306 -> (match (_431306) with
| (l, _) -> begin
...
let rec encode_sigelt = (fun env se -> (let _431315 = if (Microsoft_FStar_Tc_Env.debug env.tcenv Microsoft_FStar_Options.Low) then begin
...
in (let _431323 = (encode_sigelt' env se)
in (match (_431323) with
| (g, e) -> begin
(match (g) with
| [] -> begin
...
(let _431374 = (new_typ_constant_and_tok_from_lid env lid)
in (match (_431374) with
...
(let _431392 = (new_typ_constant_and_tok_from_lid env lid)
in (match (_431392) with
| (tname, ttok, env) -> begin
(let _431401 = (match (t.Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_431401) with
| (tps, t) -> begin
(let _431408 = (encode_binders None tps env)
in (match (_431408) with
...
in (let t = (whnf env t)
in (let _431426 = if ((Support.Microsoft.FStar.Util.for_some (fun _429433 -> (match (_429433) with
...
in (let kindingAx = (let _431430 = (encode_knd (Microsoft_FStar_Tc_Recheck.recompute_kind t) env' app)
in (match (_431430) with
| (k, decls) -> begin
...
(let tt = (whnf env t)
in (let _431443 = (encode_free_var env lid t tt quals)
...
(let _431454 = (encode_formula f env)
in (match (_431454) with
| (f, decls) -> begin
...
(let _431470 = (new_typ_constant_and_tok_from_lid env t)
...
(let is_logical = ((Support.Microsoft.FStar.Util.for_some (fun _429434 -> (match (_429434) with
...
(let _431521 = (fresh_fvar "x" Microsoft_FStar_ToSMT_Term.Term_sort)
in (match (_431521) with
| (xxsym, xx) -> begin
(let _431564 = ((Support.List.fold_left (fun _431524 l -> (match (_431524) with
| (out, decls) -> begin
(let data_t = (Microsoft_FStar_Tc_Env.lookup_datacon env.tcenv l)
in (let _431534 = (match ((Microsoft_FStar_Absyn_Util.function_formals data_t)) with
...
in (let _431552 = (encode_args indices env)
in (match (_431552) with
| (indices, decls') -> begin
(let _431553 = if ((Support.List.length indices) <> (Support.List.length vars)) then begin
(failwith "Impossible")
end
...
in (match (_431564) with
| (data_ax, decls) -> begin
(let _431567 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_431567) with
| (ffsym, ff) -> begin
...
in (let _431579 = (match ((Microsoft_FStar_Absyn_Util.compress_kind k).Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_431579) with
| (is_kind_arrow, formals, res) -> begin
(let _431586 = (encode_binders None formals env)
in (match (_431586) with
| (vars, guards, env', binder_decls, _) -> begin
(let projection_axioms = (fun tapp vars -> (match (((Support.Microsoft.FStar.Util.find_opt (fun _429435 -> (match (_429435) with
...
(let rec projectee = (fun i _429436 -> (match (_429436) with
| [] -> begin
i
end
| f::tl -> begin
...
in (let _431625 = (match ((Support.Microsoft.FStar.Util.first_N projectee_pos vars)) with
| (_, xx::suffix) -> begin
(xx, suffix)
end
| _ -> begin
...
in (let _431632 = (new_typ_constant_and_tok_from_lid env t)
in (match (_431632) with
| (tname, ttok, env) -> begin
...
in (let _431657 = (let tname_decl = (constructor_or_logic_type_decl (tname, ((Support.List.map (fun _431638 -> (match (_431638) with
| (n, s) -> begin
((Support.String.strcat tname n), s)
end))) vars), Microsoft_FStar_ToSMT_Term.Type_sort, (varops.next_id ())))
in (let _431654 = (match (vars) with
| [] -> begin
...
in (let pats = if ((not (is_logical)) && ((Support.Microsoft.FStar.Util.for_some (fun _429437 -> (match (_429437) with
...
in (match (_431654) with
| (tok_decls, env) -> begin
((Support.List.append tname_decl tok_decls), env)
end)))
in (match (_431657) with
| (decls, env) -> begin
(let kindingAx = (let _431660 = (encode_knd res env' tapp)
in (match (_431660) with
| (k, decls) -> begin
...
(let _431691 = (new_term_constant_and_tok_from_lid env d)
in (match (_431691) with
| (ddconstrsym, ddtok, env) -> begin
(let ddtok_tm = (Microsoft_FStar_ToSMT_Term.mkApp (ddtok, []))
in (let _431700 = (match ((Microsoft_FStar_Absyn_Util.function_formals t)) with
| Some ((f, c)) -> begin
...
in (match (_431700) with
| (formals, t_res) -> begin
(let _431703 = (fresh_fvar "f" Microsoft_FStar_ToSMT_Term.Fuel_sort)
in (match (_431703) with
| (fuel_var, fuel_tm) -> begin
(let s_fuel_tm = (Microsoft_FStar_ToSMT_Term.mkApp ("SFuel", (fuel_tm)::[]))
in (let _431710 = (encode_binders (Some (fuel_tm)) formals env)
in (match (_431710) with
| (vars, guards, env', binder_decls, names) -> begin
(let projectors = ((Support.List.map (fun _429438 -> (match (_429438) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
...
in (let _431724 = (encode_typ_pred' None t env ddtok_tm)
in (match (_431724) with
| (tok_typing, decls3) -> begin
(let _431731 = (encode_binders (Some (s_fuel_tm)) formals env)
in (match (_431731) with
| (vars', guards', env'', decls_formals, _) -> begin
(let _431736 = (let xvars = (Support.List.map Microsoft_FStar_ToSMT_Term.mkFreeV vars')
in (let dapp = (Microsoft_FStar_ToSMT_Term.mkApp (ddconstrsym, xvars))
in (encode_typ_pred' (Some (fuel_tm)) t_res env'' dapp)))
in (match (_431736) with
...
in (let encode_elim = (fun _431743 -> (match (_431743) with
| () -> begin
(let _431746 = (Microsoft_FStar_Absyn_Util.head_and_args t_res)
in (match (_431746) with
| (head, args) -> begin
...
in (let _431752 = (encode_args args env')
in (match (_431752) with
| (encoded_args, arg_decls) -> begin
(let _431776 = (Support.List.fold_left (fun _431756 arg -> (match (_431756) with
| (env, arg_vars, eqns) -> begin
(match (arg) with
| Support.Microsoft.FStar.Util.Inl (targ) -> begin
(let _431764 = (gen_typ_var env (Microsoft_FStar_Absyn_Util.new_bvd None))
in (match (_431764) with
| (_, tv, env) -> begin
(env, (tv)::arg_vars, ((Microsoft_FStar_ToSMT_Term.mkEq (targ, tv)))::eqns)
end))
end
| Support.Microsoft.FStar.Util.Inr (varg) -> begin
(let _431771 = (gen_term_var env (Microsoft_FStar_Absyn_Util.new_bvd None))
in (match (_431771) with
| (_, xv, env) -> begin
(env, (xv)::arg_vars, ((Microsoft_FStar_ToSMT_Term.mkEq (varg, xv)))::eqns)
end))
end)
end)) (env', [], []) encoded_args)
in (match (_431776) with
...
| _ -> begin
(let _431796 = (Microsoft_FStar_Tc_Errors.warn drange (Support.Microsoft.FStar.Util.format2 "Constructor %s builds an unexpected type %s\n" (Microsoft_FStar_Absyn_Print.sli d) (Microsoft_FStar_Absyn_Print.typ_to_string head)))
in ([], []))
end)
end))
end))
in (let _431800 = (encode_elim ())
...
(let _431813 = (encode_signature env ses)
in (match (_431813) with
| (g, env) -> begin
(let _431825 = ((Support.List.partition (fun _429439 -> (match (_429439) with
...
in (let _431844 = (Support.Microsoft.FStar.Util.first_N nbinders formals)
in (match (_431844) with
...
(let _431894 = (Support.Microsoft.FStar.Util.first_N nformals binders)
...
(let _431903 = (eta_expand binders formals body tres)
in (match (_431903) with
| (binders, body) -> begin
...
in (let _431915 = (eta_expand [] formals e tres)
in (match (_431915) with
| (binders, body) -> begin
(binders, body, formals, tres)
end)))
end
...
in (Support.Prims.try_with (fun _431919 -> (match (_431919) with
| () -> begin
if ((Support.Microsoft.FStar.Util.for_some (fun _429440 -> (match (_429440) with
| Microsoft_FStar_Absyn_Syntax.Opaque -> begin
true
end
| _ -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _431941 -> (match (_431941) with
| (_, t, _) -> begin
(Microsoft_FStar_Absyn_Util.is_smt_lemma t)
end))) bindings) then begin
(((Support.List.collect (fun _431946 -> (match (_431946) with
| (flid, t, _) -> begin
...
(let _431968 = ((Support.List.fold_left (fun _431951 _431956 -> (match ((_431951, _431956)) with
| ((toks, typs, decls, env), (flid, t, _)) -> begin
(let _431957 = if (Microsoft_FStar_Absyn_Util.is_smt_lemma t) then begin
(raise (Let_rec_unencodeable))
end
in (let t_norm = (Microsoft_FStar_Absyn_Util.compress_typ (whnf env t))
in (let _431963 = (declare_top_level_let env (Support.Microsoft.FStar.Util.right flid) t t_norm)
in (match (_431963) with
| (tok, decl, env) -> begin
((((Support.Microsoft.FStar.Util.right flid), tok))::toks, (t_norm)::typs, (decl)::decls, env)
end))))
end)) ([], [], [], env)) bindings)
in (match (_431968) with
...
in if (((Support.Microsoft.FStar.Util.for_some (fun _429441 -> (match (_429441) with
...
(let _431997 = (destruct_bound_function flid t_norm e)
in (match (_431997) with
| (binders, body, formals, tres) -> begin
(let _432004 = (encode_binders None binders env)
in (match (_432004) with
| (vars, guards, env', binder_decls, _) -> begin
...
in (let _432011 = (encode_exp body env')
in (match (_432011) with
| (body, decls2) -> begin
...
in (let env0 = env
in (let _432031 = ((Support.List.fold_left (fun _432020 _432025 -> (match ((_432020, _432025)) with
...
in (let encode_one_binding = (fun env0 _432040 t_norm _432047 -> (match ((_432040, _432047)) with
| ((flid, f, ftok, g, gtok), (_, _, e)) -> begin
(let _432052 = (destruct_bound_function flid t_norm e)
in (match (_432052) with
| (binders, body, formals, tres) -> begin
(let _432059 = (encode_binders None binders env)
in (match (_432059) with
| (vars, guards, env', binder_decls, _) -> begin
...
in (let _432069 = (encode_exp body env')
in (match (_432069) with
...
in (let _432092 = (let _432079 = (encode_binders None formals env0)
in (match (_432079) with
...
in (let _432089 = (let _432086 = (encode_typ_pred' None tres env gapp)
in (match (_432086) with
| (g_typing, d3) -> begin
...
in (let _432108 = (Support.List.fold_left (fun _432096 _432100 -> (match ((_432096, _432100)) with
| ((decls, eqns, env0), (gtok, ty, bs)) -> begin
(let _432104 = (encode_one_binding env0 gtok ty bs)
in (match (_432104) with
| (decls', eqns', env0) -> begin
((decls')::decls, (Support.List.append eqns' eqns), env0)
end))
end)) ((decls)::[], [], env0) (Support.List.zip3 gtoks typs bindings))
in (match (_432108) with
| (decls, eqns, env0) -> begin
(let _432117 = ((Support.List.partition (fun _429442 -> (match (_429442) with
...
end)) (fun _431918 -> (match (_431918) with
| Let_rec_unencodeable -> begin
(let msg = ((Support.String.concat " and ") ((Support.List.map (fun _431927 -> (match (_431927) with
| (lb, _, _) -> begin
...
| None -> begin
(let _432144 = (encode_free_var env x t t_norm [])
in (match (_432144) with
| (decls, env) -> begin
(let _432149 = (lookup_lid env x)
in (match (_432149) with
| (n, x', _) -> begin
((n, x'), decls, env)
end))
end))
...
and encode_smt_lemma = (fun env lid t -> (let _432161 = (encode_function_type_as_formula None t env)
...
(let _432170 = (new_term_constant_and_tok_from_lid env lid)
in (match (_432170) with
| (vname, vtok, env) -> begin
(let arg_sorts = (match (t_norm.Microsoft_FStar_Absyn_Syntax.n) with
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((binders, _)) -> begin
((Support.List.map (fun _429443 -> (match (_429443) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
...
(let _432200 = (match ((Microsoft_FStar_Absyn_Util.function_formals t_norm)) with
...
in (match (_432200) with
| (formals, res) -> begin
(let _432204 = (new_term_constant_and_tok_from_lid env lid)
in (match (_432204) with
| (vname, vtok, env) -> begin
(let vtok_tm = (Microsoft_FStar_ToSMT_Term.mkApp (vtok, []))
in (let mk_disc_proj_axioms = (fun vapp vars -> ((Support.List.collect (fun _429444 -> (match (_429444) with
| Microsoft_FStar_Absyn_Syntax.Discriminator (d) -> begin
(let _432218 = (Support.Microsoft.FStar.Util.prefix vars)
...
(let _432231 = (Support.Microsoft.FStar.Util.prefix vars)
...
in (let _432241 = (encode_binders None formals env)
in (match (_432241) with
...
in (let _432274 = (let vname_decl = Microsoft_FStar_ToSMT_Term.DeclFun ((vname, ((Support.List.map (fun _429445 -> (match (_429445) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
Microsoft_FStar_ToSMT_Term.Type_sort
end
| _ -> begin
...
in (let _432271 = (match (formals) with
| [] -> begin
(let _432258 = if (not ((head_normal env tt))) then begin
...
in (match (_432271) with
| (tok_decl, env) -> begin
((vname_decl)::tok_decl, env)
end)))
in (match (_432274) with
| (decls2, env) -> begin
(let _432277 = (encode_typ_pred' None res env' vapp)
in (match (_432277) with
| (ty_pred, decls3) -> begin
(let _432283 = if (not ((head_normal env tt))) then begin
(let _432280 = (encode_typ_pred' None tt env vtok_tm)
in (match (_432280) with
...
and encode_signature = (fun env ses -> ((Support.List.fold_left (fun _432290 se -> (match (_432290) with
| (g, env) -> begin
(let _432294 = (encode_sigelt env se)
in (match (_432294) with
| (g', env) -> begin
((Support.List.append g g'), env)
end))
end)) ([], env)) ses))

let encode_env_bindings = (fun env bindings -> (let encode_binding = (fun b _432301 -> (match (_432301) with
| (decls, env) -> begin
(match (b) with
| Microsoft_FStar_Tc_Env.Binding_var ((x, t0)) -> begin
(let _432309 = (new_term_constant env x)
in (match (_432309) with
| (xxsym, xx, env') -> begin
(let t1 = (norm_t env t0)
in (let _432313 = (encode_typ_pred' None t1 env xx)
in (match (_432313) with
| (t, decls') -> begin
...
(let _432323 = (new_typ_constant env a)
in (match (_432323) with
| (aasym, aa, env') -> begin
(let _432326 = (encode_knd k env aa)
in (match (_432326) with
| (k, decls') -> begin
...
in (let _432335 = (encode_free_var env x t t_norm [])
in (match (_432335) with
| (g, env') -> begin
((Support.List.append decls g), env')
end)))
end
| Microsoft_FStar_Tc_Env.Binding_sig (se) -> begin
(let _432340 = (encode_sigelt env se)
...
let encode_labels = (fun labs -> (let prefix = ((Support.List.map (fun _432347 -> (match (_432347) with
| (l, _, _) -> begin
Microsoft_FStar_ToSMT_Term.DeclFun (((Support.Prims.fst l), [], Microsoft_FStar_ToSMT_Term.Bool_sort, None))
end))) labs)
in (let suffix = ((Support.List.collect (fun _432354 -> (match (_432354) with
| (l, _, _) -> begin
...
| e::_ -> begin
(let _432363 = e
in {bindings = _432363.bindings; depth = _432363.depth; tcenv = tcenv; warn = _432363.warn; cache = _432363.cache; nolabels = _432363.nolabels; use_zfuel_name = _432363.use_zfuel_name})
end))
...
let push_env = (fun _432371 -> (match (_432371) with
| () -> begin
(match ((! (last_env))) with
| [] -> begin
(failwith "Empty env stack")
...
| hd::tl -> begin
(let _432376 = (Microsoft_FStar_ToSMT_Term.push ())
in (let refs = (Support.Microsoft.FStar.Util.smap_copy hd.cache)
in (let top = (let _432379 = hd
in {bindings = _432379.bindings; depth = _432379.depth; tcenv = _432379.tcenv; warn = _432379.warn; cache = refs; nolabels = _432379.nolabels; use_zfuel_name = _432379.use_zfuel_name})
in (last_env := (top)::(hd)::tl))))
end)
end))

let pop_env = (fun _432382 -> (match (_432382) with
| () -> begin
(match ((! (last_env))) with
| [] -> begin
...
| _::tl -> begin
(let _432388 = (Microsoft_FStar_ToSMT_Term.pop ())
in (last_env := tl))
end)
end))

let mark_env = (fun _432390 -> (match (_432390) with
| () -> begin
(push_env ())
end))

let reset_mark_env = (fun _432391 -> (match (_432391) with
| () -> begin
(pop_env ())
end))

let commit_mark_env = (fun _432392 -> (match (_432392) with
| () -> begin
(let _432393 = (Microsoft_FStar_ToSMT_Term.commit_mark ())
...
let init = (fun tcenv -> (let _432404 = (init_env tcenv)
in (let _432406 = (Microsoft_FStar_ToSMT_Z3.init ())
in (Microsoft_FStar_ToSMT_Z3.giveZ3 ((Microsoft_FStar_ToSMT_Term.DefPrelude)::[])))))

let push = (fun msg -> (let _432409 = (push_env ())
in (let _432411 = (varops.push ())
in (Microsoft_FStar_ToSMT_Z3.push msg))))

let pop = (fun msg -> (let _432414 = ((Support.Prims.ignore) (pop_env ()))
in (let _432416 = (varops.pop ())
in (Microsoft_FStar_ToSMT_Z3.pop msg))))

let mark = (fun msg -> (let _432419 = (mark_env ())
in (let _432421 = (varops.mark ())
in (Microsoft_FStar_ToSMT_Z3.mark msg))))

let reset_mark = (fun msg -> (let _432424 = (reset_mark_env ())
in (let _432426 = (varops.reset_mark ())
in (Microsoft_FStar_ToSMT_Z3.reset_mark msg))))

let commit_mark = (fun msg -> (let _432429 = (commit_mark_env ())
in (let _432431 = (varops.commit_mark ())
...
in (let _432440 = (encode_sigelt env se)
in (match (_432440) with
| (decls, env) -> begin
(let _432441 = (set_env env)
...
in (let _432446 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
in (let env = (get_env tcenv)
in (let _432453 = (encode_signature (let _432449 = env
in {bindings = _432449.bindings; depth = _432449.depth; tcenv = _432449.tcenv; warn = false; cache = _432449.cache; nolabels = _432449.nolabels; use_zfuel_name = _432449.use_zfuel_name}) modul.Microsoft_FStar_Absyn_Syntax.exports)
in (match (_432453) with
| (decls, env) -> begin
...
in (let _432459 = (set_env (let _432457 = env
in {bindings = _432457.bindings; depth = _432457.depth; tcenv = _432457.tcenv; warn = true; cache = _432457.cache; nolabels = _432457.nolabels; use_zfuel_name = _432457.use_zfuel_name}))
in (let _432461 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
...
let solve = (fun tcenv q -> (let _432466 = (push (Support.Microsoft.FStar.Util.format1 "Starting query at %s" (Support.Microsoft.FStar.Range.string_of_range (Microsoft_FStar_Tc_Env.get_range tcenv))))
in (let pop = (fun _432469 -> (match (_432469) with
| () -> begin
...
in (let _432499 = (let env = (get_env tcenv)
in (let bindings = (Microsoft_FStar_Tc_Env.fold_env tcenv (fun bs b -> (b)::bs) [])
in (let _432482 = (encode_env_bindings env (Support.List.filter (fun _429446 -> (match (_429446) with
...
in (match (_432482) with
| (env_decls, env) -> begin
(let _432483 = if (Microsoft_FStar_Tc_Env.debug tcenv Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Encoding query formula: %s\n" (Microsoft_FStar_Absyn_Print.formula_to_string q))
end
in (let _432488 = (encode_formula_with_labels q env)
in (match (_432488) with
| (phi, labels, qdecls) -> begin
(let _432491 = (encode_labels labels)
in (match (_432491) with
...
in (let _432526 = (Microsoft_FStar_ToSMT_Z3.giveZ3 prefix)
in (let with_fuel = (fun _432531 -> (match (_432531) with
| (n, i) -> begin
...
in (let check = (fun _432533 -> (match (_432533) with
| () -> begin
...
in (let report = (fun _432539 -> (match (_432539) with
| (ok, errs) -> begin
if ok then begin
()
end else begin
...
in (let rec try_alt_configs = (fun errs _429447 -> (match (_429447) with
| [] -> begin
(report (false, errs))
end
| mi::[] -> begin
...
| mi::tl -> begin
(Microsoft_FStar_ToSMT_Z3.ask fresh labels (with_fuel mi) (fun _432559 -> (match (_432559) with
| (ok, errs') -> begin
(match (errs) with
| [] -> begin
(cb tl (ok, errs'))
...
and cb = (fun alt _432566 -> (match (_432566) with
| (ok, errs) -> begin
if ok then begin
()
end else begin
...
in (let _432567 = if (! (Microsoft_FStar_Options.admit_smt_queries)) then begin
()
end else begin
(check ())
end
...
in (let _432572 = (push "query")
in (let _432579 = (encode_formula_with_labels q env)
in (match (_432579) with
| (f, _, _) -> begin
(let _432580 = (pop "query")
...
let dummy = {Microsoft_FStar_Tc_Env.init = (fun _432589 -> ()); Microsoft_FStar_Tc_Env.push = (fun _432591 -> ()); Microsoft_FStar_Tc_Env.pop = (fun _432593 -> ()); Microsoft_FStar_Tc_Env.mark = (fun _432595 -> ()); Microsoft_FStar_Tc_Env.reset_mark = (fun _432597 -> ()); Microsoft_FStar_Tc_Env.commit_mark = (fun _432599 -> ()); Microsoft_FStar_Tc_Env.encode_modul = (fun _432601 _432603 -> ()); Microsoft_FStar_Tc_Env.encode_sig = (fun _432605 _432607 -> ()); Microsoft_FStar_Tc_Env.solve = (fun _432609 _432611 -> ()); Microsoft_FStar_Tc_Env.is_trivial = (fun _432613 _432615 -> false); Microsoft_FStar_Tc_Env.finish = (fun _432617 -> (match (_432617) with
| () -> begin
()
end)); Microsoft_FStar_Tc_Env.refresh = (fun _432618 -> (match (_432618) with
| () -> begin
()
end))}
