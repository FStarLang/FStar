...
(match ((ST.read m)) with
| None -> begin
typ
end
| Some (t) -> begin
(let t' = (compress_typ_aux pos t)
in (let _24_23 = (ST.op_Colon_Equals m (Some (t')))
in t'))
end)
end
...
(let _90_8 = (Microsoft_FStar_Absyn_Syntax.mk_Typ_app (t', args) None typ.Microsoft_FStar_Absyn_Syntax.pos)
in (All.pipe_left (compress_typ_aux pos) _90_8))
end
| _24_55 -> begin
typ
end)
...
in (let _24_82 = (ST.op_Colon_Equals m (Some (e')))
in e'))
end)
end
...
in (let _24_127 = (ST.op_Colon_Equals m (Some (k')))
in k'))
end)
end
| _24_130 -> begin
...
let left = (fun ext benv btv -> (match ((ext benv (Microsoft_FStar_Util.Inl (btv)))) with
| (benv, Microsoft_FStar_Util.Inl (bvd)) -> begin
(benv, bvd)
end
| _24_139 -> begin
...
let right = (fun ext benv bvv -> (match ((ext benv (Microsoft_FStar_Util.Inr (bvv)))) with
| (benv, Microsoft_FStar_Util.Inr (bvd)) -> begin
(benv, bvd)
end
| _24_148 -> begin
(All.failwith "impossible")
...
type boundvar =
(Microsoft_FStar_Absyn_Syntax.btvdef, Microsoft_FStar_Absyn_Syntax.bvvdef) Microsoft_FStar_Util.either
...
| Some (a) -> begin
(Microsoft_FStar_Util.Inl (a))::binders
end))
...
| Some (a) -> begin
(Microsoft_FStar_Util.Inr (a))::binders
end))
...
| Some (a) -> begin
(Microsoft_FStar_Util.Inl ((bvd_to_bvar_s a k)))::[]
end))
...
| Some (a) -> begin
(Microsoft_FStar_Util.Inr ((bvd_to_bvar_s a t)))::[]
end))

type knd_components =
(Microsoft_FStar_Absyn_Syntax.binders * Microsoft_FStar_Absyn_Syntax.knd Prims.list * Microsoft_FStar_Absyn_Syntax.typ Prims.list * Microsoft_FStar_Absyn_Syntax.arg Prims.list)

type typ_components =
(Microsoft_FStar_Absyn_Syntax.binders * Microsoft_FStar_Absyn_Syntax.knd Prims.list * Microsoft_FStar_Absyn_Syntax.typ Prims.list * Microsoft_FStar_Absyn_Syntax.comp Prims.list * Microsoft_FStar_Absyn_Syntax.arg Prims.list)

type exp_components =
(Microsoft_FStar_Absyn_Syntax.binders * Microsoft_FStar_Absyn_Syntax.knd Prims.list * Microsoft_FStar_Absyn_Syntax.typ Prims.list * Microsoft_FStar_Absyn_Syntax.exp Prims.list * Microsoft_FStar_Absyn_Syntax.arg Prims.list)
...
and map_args = (fun map_typ map_exp env binders arguments -> (let _24_270 = (Microsoft_FStar_List.fold_left (fun _24_254 _24_257 -> (match ((_24_254, _24_257)) with
...
| (t, env) -> begin
(((Microsoft_FStar_Util.Inl (t), imp))::out, env)
end))
end
...
| (e, env) -> begin
(((Microsoft_FStar_Util.Inr (e), imp))::out, env)
end))
end)
end)) ([], env) arguments)
in (match (_24_270) with
| (args', env) -> begin
((Microsoft_FStar_List.rev args'), env)
end)))
and map_binders = (fun map_kind map_typ env binders bs -> (let _24_301 = (All.pipe_right bs (Microsoft_FStar_List.fold_left (fun _24_280 b -> (match (_24_280) with
| (bs, binders, env) -> begin
(match (b) with
...
in (((Microsoft_FStar_Util.Inl ((bvd_to_bvar_s a.Microsoft_FStar_Absyn_Syntax.v k)), imp))::bs, binders, env))
end))
end
...
in (((Microsoft_FStar_Util.Inr ((bvd_to_bvar_s x.Microsoft_FStar_Absyn_Syntax.v t)), imp))::bs, binders, env))
end))
end)
end)) ([], binders, env)))
in (match (_24_301) with
| (bs, binders, env) -> begin
((Microsoft_FStar_List.rev bs), binders, env)
end)))
...
| (t, env) -> begin
(let _90_292 = (Microsoft_FStar_Absyn_Syntax.mk_Total t)
in (_90_292, env))
end))
end
...
| (args, env) -> begin
(let _24_343 = (All.pipe_right ct.Microsoft_FStar_Absyn_Syntax.flags (Microsoft_FStar_Util.fold_map (fun env flag -> (match (flag) with
...
| (env, flags) -> begin
(let _90_295 = (Microsoft_FStar_Absyn_Syntax.mk_Comp (let _24_344 = ct
...
in _90_299.Microsoft_FStar_Absyn_Syntax.n)) with
...
(let _24_391 = (map_binders map_kind map_typ env binders (((Microsoft_FStar_Util.Inr (x), None))::[]))
...
| (t, env) -> begin
(let _24_491 = (Microsoft_FStar_List.fold_left (fun _24_471 arg -> (match (_24_471) with
| (pats, env) -> begin
(match (arg) with
| (Microsoft_FStar_Util.Inl (t), _24_476) -> begin
(let _24_480 = (map_typ env binders t)
in (match (_24_480) with
| (t, env) -> begin
(((Microsoft_FStar_Util.Inl (t), None))::pats, env)
end))
end
| (Microsoft_FStar_Util.Inr (e), _24_484) -> begin
(let _24_488 = (map_exp env binders e)
in (match (_24_488) with
| (e, env) -> begin
(((Microsoft_FStar_Util.Inr (e), None))::pats, env)
end))
end)
end)) ([], env) ps)
in (match (_24_491) with
| (pats, env) -> begin
(([], [], (t)::[], [], (Microsoft_FStar_List.rev pats)), env)
end))
end))
end)
...
and reduce_exp = (fun map_kind' map_typ' map_exp' combine_kind combine_typ combine_exp env binders e -> (let rec map_exps = (fun env binders el -> (let _24_532 = (Microsoft_FStar_List.fold_left (fun _24_525 e -> (match (_24_525) with
| (out, env) -> begin
...
| (el, env) -> begin
((Microsoft_FStar_List.rev el), env)
end)))
and map_exps_with_binders = (fun env el -> (let _24_546 = (Microsoft_FStar_List.fold_left (fun _24_537 _24_540 -> (match ((_24_537, _24_540)) with
| ((out, env), (b, e)) -> begin
...
(Microsoft_FStar_List.fold_left (fun b _24_647 -> (match (_24_647) with
| (p, _24_646) -> begin
(pat_binders b p)
end)) b pats)
end
...
in (let branches = (All.pipe_right pl (Microsoft_FStar_List.collect (fun _24_658 -> (match (_24_658) with
| (p, w, e) -> begin
...
(let tl = (Microsoft_FStar_List.map (fun lb -> lb.Microsoft_FStar_Absyn_Syntax.lbtyp) bvdt_tl)
in (let el = (Microsoft_FStar_List.map (fun lb -> lb.Microsoft_FStar_Absyn_Syntax.lbdef) bvdt_tl)
in (let _24_716 = (All.pipe_right tl (Microsoft_FStar_List.fold_left (fun _24_709 t -> (match (_24_709) with
| (tl, env) -> begin
...
| (tl, env) -> begin
(let tl = (Microsoft_FStar_List.rev tl)
in (let binders = (Microsoft_FStar_List.fold_left (fun binders lb -> (match (lb.Microsoft_FStar_Absyn_Syntax.lbname) with
| Microsoft_FStar_Util.Inl (x) -> begin
(push_vbinder binders (Some (x)))
end
| _24_723 -> begin
...
in (let _24_727 = (map_exps env binders (Microsoft_FStar_List.append el ((e)::[])))
...
(fun p -> (Microsoft_FStar_Util.return_all k))
end
...
(Microsoft_FStar_Absyn_Syntax.mk_Kind_abbrev (((Prims.fst kabr), args), k))
end
...
in (let _90_382 = (k' k.Microsoft_FStar_Absyn_Syntax.pos)
in (_90_382, env))))
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_lam (bs, t)))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_app (_24_839), (_24_842, _24_844, t::[], _24_848, args)) -> begin
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_app (t, args)))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_refine (_24_853), ((Microsoft_FStar_Util.Inr (x), _24_858)::[], _24_862, t::[], _24_866, _24_868)) -> begin
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_refine (x, t)))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_fun (_24_872), (bs, _24_876, _24_878, c::[], _24_882)) -> begin
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_fun (bs, c)))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_uvar (x, _24_887), (_24_891, k::[], _24_895, _24_897, _24_899)) -> begin
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_uvar' (x, k)))
end
| (Microsoft_FStar_Absyn_Syntax.Typ_ascribed (_24_903), (_24_906, k::[], t::[], _24_912, _24_914)) -> begin
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_ascribed' (t, k)))
end
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta' (Microsoft_FStar_Absyn_Syntax.Meta_named ((t', l)))))
end
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta' (Microsoft_FStar_Absyn_Syntax.Meta_pattern ((t, args)))))
end
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta' (Microsoft_FStar_Absyn_Syntax.Meta_labeled ((t, l, r, p)))))
end
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta' (Microsoft_FStar_Absyn_Syntax.Meta_refresh_label ((t, b, r)))))
end
...
(let _90_423 = (let _90_422 = (let _90_421 = (let _90_420 = (Microsoft_FStar_Util.mk_ref false)
in (t1, t2, _90_420))
in Microsoft_FStar_Absyn_Syntax.Meta_slack_formula (_90_421))
in (Microsoft_FStar_Absyn_Syntax.mk_Typ_meta' _90_422))
in (All.pipe_left w _90_423))
end
| _24_1012 -> begin
(All.failwith "impossible")
end)
...
(All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Exp_meta' (Microsoft_FStar_Absyn_Syntax.Meta_desugared ((e, tag)))))
end
...
in (e1, _90_457))
in (Microsoft_FStar_Absyn_Syntax.mk_Exp_match _90_458))
in (All.pipe_left w _90_459)))
end
| (Microsoft_FStar_Absyn_Syntax.Exp_let ((is_rec, lbs), _24_1174), (_24_1178, _24_1180, tl, el, _24_1184)) -> begin
(match ((Microsoft_FStar_Util.first_N (Microsoft_FStar_List.length lbs) el)) with
| (el, e'::[]) -> begin
(let lbs' = (Microsoft_FStar_List.map3 (fun lb t e -> (let _24_1194 = lb
...
in (All.pipe_left w (Microsoft_FStar_Absyn_Syntax.mk_Exp_let ((is_rec, lbs'), e'))))
end
| _24_1198 -> begin
(All.failwith "impossible")
end)
...
in _90_540.Microsoft_FStar_Absyn_Syntax.n)) with
| (Microsoft_FStar_Absyn_Syntax.Typ_unknown) | (Microsoft_FStar_Absyn_Syntax.Typ_btvar (_)) | (Microsoft_FStar_Absyn_Syntax.Typ_const (_)) -> begin
(t, env)
end
| _24_1245 -> begin
...
in (All.pipe_left Prims.snd _90_583)))
