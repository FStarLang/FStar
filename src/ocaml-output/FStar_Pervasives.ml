...
type ('a,'uu____45) trivial_pure_post = unit
type ('a,'x) ambient = unit

type 'heap st_pre_h = unit
type ('heap,'a,'pre) st_post_h' = unit
type ('heap,'a) st_post_h = unit
type ('heap,'a) st_wp_h = unit
type ('heap,'a,'x,'p,'uu___0_114) st_return = 'p
type ('heap,'r1,'a,'b,'wp1,'wp2,'p,'h0) st_bind_wp = 'wp1
type ('heap,'a,'p,'wp_then,'wp_else,'post,'h0) st_if_then_else = unit
...
type ('a,'p,'wp_then,'wp_else,'post) ex_if_then_else = unit
...
let rec false_elim : 'a . unit -> 'a = fun u  -> false_elim () 
...
let normalize_term : 'a . 'a -> 'a = fun x  -> x 
type 'a normalize = 'a
type norm_step =
  | Simpl 
  | Weak 
...
let singleton : 'a . 'a -> 'a = fun x  -> x 
let with_type : 't . 't -> 't = fun e  -> e 