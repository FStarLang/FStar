...
| () -> begin
(let _11_26 = (show_signatures := [])
in (let _11_28 = (norm_then_print := true)
in (let _11_30 = (z3_exe := (Support.Microsoft.FStar.Platform.exe "z3"))
in (let _11_32 = (silent := false)
in (let _11_34 = (debug := [])
in (let _11_36 = (debug_level := [])
in (let _11_38 = (log_types := false)
in (let _11_40 = (print_effect_args := false)
in (let _11_42 = (print_real_names := false)
in (let _11_44 = (dump_module := None)
in (let _11_46 = (logQueries := false)
in (let _11_48 = (z3exe := true)
in (let _11_50 = (outputDir := Some ("."))
in (let _11_52 = (fstar_home_opt := None)
in (let _11_54 = (_fstar_home := "")
in (let _11_56 = (prims_ref := None)
in (let _11_58 = (z3timeout := 5)
in (let _11_60 = (admit_smt_queries := false)
in (let _11_62 = (pretype := true)
in (let _11_64 = (codegen := None)
in (let _11_66 = (codegen_libs := [])
in (let _11_68 = (admit_fsi := [])
in (let _11_70 = (trace_error := false)
in (let _11_72 = (verify := true)
in (let _11_74 = (full_context_dependency := true)
in (let _11_76 = (print_implicits := false)
in (let _11_78 = (hide_uvar_nums := false)
in (let _11_80 = (hide_genident_nums := false)
in (let _11_82 = (serialize_mods := false)
in (let _11_84 = (initial_fuel := 2)
in (let _11_86 = (initial_ifuel := 1)
in (let _11_88 = (max_fuel := 8)
in (let _11_90 = (min_fuel := 1)
in (let _11_92 = (max_ifuel := 2)
in (let _11_94 = (warn_top_level_effects := false)
in (let _11_96 = (no_slack := false)
in (let _11_98 = (eager_inference := false)
in (let _11_100 = (unthrottle_inductives := false)
in (let _11_102 = (use_eq_at_higher_order := false)
in (let _11_104 = (fs_typ_app := false)
in (let _11_106 = (n_cores := 1)
in (let _11_108 = (split_cases := 0)
in (verify_module := [])))))))))))))))))))))))))))))))))))))))))))
end))
...
in (let _11_114 = (_fstar_home := x)
in (let _11_116 = (fstar_home_opt := Some (x))
in x))))
end
| Some (x) -> begin
(let _11_120 = (_fstar_home := x)
in x)
end)
in fh)
end))
...
| () -> begin
(trace_error := true)
end))), "Don\'t print an error message; show an exception trace instead"))::((Support.Microsoft.FStar.Getopt.noshort, "codegen", Support.Microsoft.FStar.Getopt.OneArg (((fun s -> (let _11_156 = (codegen := (parse_codegen s))
in (verify := false))), "OCaml|F#|JavaScript")), "Generate code for execution"))::((Support.Microsoft.FStar.Getopt.noshort, "codegen-lib", Support.Microsoft.FStar.Getopt.OneArg (((fun s -> (codegen_libs := ((Support.Microsoft.FStar.Util.split s "."))::(! (codegen_libs)))), "namespace")), "External runtime library library"))::((Support.Microsoft.FStar.Getopt.noshort, "lax", Support.Microsoft.FStar.Getopt.ZeroArgs ((fun _11_159 -> (match (_11_159) with
| () -> begin
(let _11_160 = (pretype := true)
in (verify := false))
end))), "Run the lax-type checker only (admit all verification conditions)"))::((Support.Microsoft.FStar.Getopt.noshort, "fstar_home", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (fstar_home_opt := Some (x))), "dir")), "Set the FSTAR_HOME variable to dir"))::((Support.Microsoft.FStar.Getopt.noshort, "silent", Support.Microsoft.FStar.Getopt.ZeroArgs ((fun _11_163 -> (match (_11_163) with
| () -> begin
(silent := true)
end))), ""))::((Support.Microsoft.FStar.Getopt.noshort, "prims", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (prims_ref := Some (x))), "file")), ""))::((Support.Microsoft.FStar.Getopt.noshort, "prn", Support.Microsoft.FStar.Getopt.ZeroArgs ((fun _11_165 -> (match (_11_165) with
| () -> begin
...
| () -> begin
(logQueries := true)
end))), "Log the Z3 queries in queries.smt2"))::((Support.Microsoft.FStar.Getopt.noshort, "admit_fsi", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (admit_fsi := (x)::(! (admit_fsi)))), "module name")), "Treat .fsi as a .fst"))::((Support.Microsoft.FStar.Getopt.noshort, "odir", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (outputDir := Some (x))), "dir")), "Place output in directory dir"))::((Support.Microsoft.FStar.Getopt.noshort, "smt", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (z3_exe := x)), "path")), "Path to the SMT solver (usually Z3, but could be any SMT2-compatible solver)"))::((Support.Microsoft.FStar.Getopt.noshort, "print_before_norm", Support.Microsoft.FStar.Getopt.ZeroArgs ((fun _11_177 -> (match (_11_177) with
| () -> begin
...
| () -> begin
(fs_typ_app := false)
end))), "Do not allow the use of t<t1,...,tn> syntax for type applications"))::((Support.Microsoft.FStar.Getopt.noshort, "n_cores", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (n_cores := (Support.Microsoft.FStar.Util.int_of_string x))), "positive integer")), "Maximum number of cores to use for the solver (default 1)"))::((Support.Microsoft.FStar.Getopt.noshort, "verify_module", Support.Microsoft.FStar.Getopt.OneArg (((fun x -> (verify_module := (x)::(! (verify_module)))), "string")), "Name of the module to verify"))::((Support.Microsoft.FStar.Getopt.noshort, "use_build_config", Support.Microsoft.FStar.Getopt.ZeroArgs ((fun _11_199 -> (match (_11_199) with
| () -> begin
