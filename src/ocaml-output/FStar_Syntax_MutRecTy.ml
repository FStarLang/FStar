...
let type_abbrevs = (FStar_All.pipe_right type_abbrev_sigelts (FStar_List.map (fun uu___202_81 -> (match (uu___202_81) with
...
in (FStar_All.pipe_right uu____135 (FStar_List.filter (fun uu___203_142 -> (match (uu___203_142) with
...
and unfold_abbrev = (fun uu___205_313 -> (match (uu___205_313) with
| FStar_Syntax_Syntax.Sig_let ((false, (lb)::[]), rng1, uu____316, quals1, attr) -> begin
(

let quals2 = (FStar_All.pipe_right quals1 (FStar_List.filter (fun uu___204_333 -> (match (uu___204_333) with
...
let uu___207_359 = lb
in {FStar_Syntax_Syntax.lbname = uu___207_359.FStar_Syntax_Syntax.lbname; FStar_Syntax_Syntax.lbunivs = uu___207_359.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = ty'; FStar_Syntax_Syntax.lbeff = uu___207_359.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = tm'})
in (

let sigelt' = FStar_Syntax_Syntax.Sig_let (((((false), ((lb')::[]))), (rng1), ((lid)::[]), (quals2), (attr)))
in ((
...
let unfold_in_sig = (fun uu___206_499 -> (match (uu___206_499) with
| FStar_Syntax_Syntax.Sig_inductive_typ (lid, univs1, bnd, ty, mut, dc, quals1, rng1) -> begin
(
...
in (FStar_Syntax_Syntax.Sig_inductive_typ (((lid), (univs1), (bnd'), (ty'), (mut'), (dc), (quals1), (rng1))))::[])))
end
| FStar_Syntax_Syntax.Sig_datacon (lid, univs1, ty, res, npars, quals1, mut, rng1) -> begin
(
...
let mut' = (filter_out_type_abbrevs mut)
in (FStar_Syntax_Syntax.Sig_datacon (((lid), (univs1), (ty'), (res), (npars), (quals1), (mut'), (rng1))))::[]))
end
