...
let int1:
      'a 'b. FStar_Ident.lid -> ('a -> 'b)
      -> 'a FStar_Syntax_Embeddings.unembedder
      -> 'b FStar_Syntax_Embeddings.embedder -> FStar_Range.range
      -> FStar_Syntax_Syntax.args
      -> FStar_Syntax_Syntax.term FStar_Pervasives_Native.option =
  fun m f ua em r args ->
    match args with
    | [(a, uu____61)] ->
        let uu____78 = ua a in
        FStar_Util.bind_opt uu____78 (fun a1 ->
            let uu____86 =
              let uu____87 = f a1 in
              em r uu____87
            in
...
let int2:
      'a 'b 'c. FStar_Ident.lid -> ('a -> 'b -> 'c)
      -> 'a FStar_Syntax_Embeddings.unembedder
      -> 'b FStar_Syntax_Embeddings.unembedder
      -> 'c FStar_Syntax_Embeddings.embedder -> FStar_Range.range
      -> FStar_Syntax_Syntax.args
      -> FStar_Syntax_Syntax.term FStar_Pervasives_Native.option =
  fun m f ua ub em r args ->
    match args with
    | [(a, uu____172); (b, uu____174)] ->
        let uu____201 = ua a in
        FStar_Util.bind_opt uu____201 (fun a1 ->
            let uu____209 = ub b in
            FStar_Util.bind_opt uu____209 (fun b1 ->
                let uu____217 =
                  let uu____218 = f a1 b1 in
                  em r uu____218
                in
...
let reflection_primops : FStar_TypeChecker_Normalize.primitive_step Prims.list =
  ...
  let mk11 a b nm f u1 em =
    let l = mklid nm in
    mk1 l (Prims.parse_int "1") (int1 l f u1 em)
  in
  let mk2 a b c nm f u1 u2 em =
    let l = mklid nm in
    mk1 l (Prims.parse_int "2") (int2 l f u1 u2 em)
  in
  let uu____388 =
    mk11 () () "__inspect"
    ...
    let uu____393 =
      ...
      let uu____397 =
        ...
        let uu____410 =
          let uu____413 =
            mk11 () () "__inspect_comp"
              (fun a419 -> (Obj.magic FStar_Reflection_Basic.inspect_comp) a419)
          ...
          let uu____414 =
            let uu____417 =
              mk11 () () "__pack_comp"
                (fun a420 -> (Obj.magic FStar_Reflection_Basic.pack_comp) a420)
                ...
                let uu____426 =
                  let uu____429 =
                    mk11 () () "__type_of_binder"
                      (fun a424 ->
                        (Obj.magic FStar_Reflection_Basic.type_of_binder) a424
                        )
                      (Obj.magic FStar_Reflection_Basic.unembed_binder)
                      (Obj.magic FStar_Reflection_Basic.embed_term)
                  in
                  let uu____430 =
                    let uu____433 =
                      mk2 () () () "__is_free"
                        (fun a425 a426 ->
                          (Obj.magic FStar_Reflection_Basic.is_free) a425 a426
                          )
                        (Obj.magic FStar_Reflection_Basic.unembed_binder)
                        (Obj.magic FStar_Reflection_Basic.unembed_term)
