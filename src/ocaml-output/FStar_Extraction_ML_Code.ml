...
let ptsym_of_symbol = (fun s -> (match (((let _127_35 = (FStar_String.get s 0)
in (FStar_Char.lowercase _127_35)) <> (FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "l__" s)
end
| false -> begin
...
in (_127_40)::[])
in (FStar_List.append p _127_41))
in (FStar_String.concat "." _127_42))
end))
end))
...
| (p, s) -> begin
(let s = (match (((let _127_47 = (FStar_String.get s 0)
in (FStar_Char.uppercase _127_47)) <> (FStar_String.get s 0))) with
| true -> begin
(Prims.strcat "U__" s)
end
| false -> begin
...
| true -> begin
(FStar_List.tryFind (fun _61_55 -> (match (_61_55) with
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _61_63 -> (match (_61_63) with
| (y, _61_62) -> begin
(x = y)
end)) prim_uni_ops)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _61_71 -> (match (_61_71) with
| (y, _61_70) -> begin
(x = y)
end)) prim_types)
end
...
(match ((is_prims_ns ns)) with
| true -> begin
(FStar_List.tryFind (fun _61_79 -> (match (_61_79) with
| (y, _61_78) -> begin
(x = y)
...
in (let _127_100 = (FStar_All.pipe_left escape_tyvar (FStar_Extraction_ML_Syntax.idsym x))
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (t1, _61_191, t2) -> begin
...
in (let docs = (FStar_List.map (fun d -> (let _127_154 = (let _127_153 = (let _127_152 = (FSharp_Format.text ";")
...
| FStar_Extraction_ML_Syntax.MLE_Var (x, _61_225) -> begin
...
in (let _127_164 = (FStar_List.map for1 fields)
...
in (let ids = (FStar_List.map (fun _61_333 -> (match (_61_333) with
| ((x, _61_330), xt) -> begin
(bvar_annot x xt)
end)) ids)
in (let body = (doc_of_expr currentModule (min_op_prec, NonAssoc) body)
...
in (let _127_291 = (let _127_290 = (let _127_289 = (FStar_List.map (doc_of_branch currentModule) pats)
...
in (let _127_326 = (FStar_List.map for1 fields)
...
| (_61_425, FStar_Extraction_ML_Syntax.MLP_Tuple (_61_427)::[]) -> begin
(let _127_343 = (let _127_342 = (FSharp_Format.text name)
in (let _127_341 = (let _127_340 = (let _127_339 = (FStar_List.hd pats)
...
in (let _127_344 = (FStar_List.map (doc_of_pattern currentModule) pats)
...
| {FStar_Extraction_ML_Syntax.mllb_name = name; FStar_Extraction_ML_Syntax.mllb_tysc = tys; FStar_Extraction_ML_Syntax.mllb_add_unit = _61_459; FStar_Extraction_ML_Syntax.mllb_def = e} -> begin
(let e = (doc_of_expr currentModule (min_op_prec, NonAssoc) e)
in (let ids = []
in (let ids = (FStar_List.map (fun _61_468 -> (match (_61_468) with
| (x, _61_467) -> begin
(FSharp_Format.text x)
end)) ids)
...
in (let _127_390 = (let _127_389 = (FSharp_Format.text (FStar_Extraction_ML_Syntax.idsym name))
...
in (let lets = (FStar_List.mapi (fun i doc -> (let _127_398 = (let _127_397 = (match ((i = 0)) with
| true -> begin
letdoc
end
| false -> begin
...
in (let _127_415 = (FStar_List.map forfield fields)
...
in (let ctors = (FStar_List.map (fun d -> (let _127_427 = (let _127_426 = (FSharp_Format.text "|")
in (_127_426)::(d)::[])
...
| FStar_Extraction_ML_Syntax.MLS_Val (x, (_61_557, ty)) -> begin
(let ty = (doc_of_mltype currentModule (min_op_prec, NonAssoc) ty)
...
in (let doc = (FStar_Option.map (fun _61_609 -> (match (_61_609) with
| (s, _61_608) -> begin
(doc_of_sig x s)
end)) sigmod)
...
| (x, sigmod, FStar_Extraction_ML_Syntax.MLLib (sub)) -> begin
(let _61_623 = (FStar_Util.fprint1 "Gen Code: %s\n" x)
in (let head = (let _127_550 = (match ((FStar_Extraction_ML_Util.codegen_fsharp ())) with
| true -> begin
...
in (let doc = (FStar_Option.map (fun _61_630 -> (match (_61_630) with
| (_61_628, m) -> begin
(doc_of_mod x m)
end)) sigmod)
...
in (FStar_List.append prefix _127_564))
in (FStar_All.pipe_left FSharp_Format.reduce _127_565)))))))))
end))
in (let docs = (FStar_List.map (fun _61_642 -> (match (_61_642) with
| (x, s, m) -> begin
...
let string_of_mlexpr = (fun env e -> (let doc = (let _127_574 = (FStar_Extraction_ML_Util.flatten_mlpath env.FStar_Extraction_ML_Env.currentModule)
in (doc_of_expr _127_574 (min_op_prec, NonAssoc) e))
in (FSharp_Format.pretty 0 doc)))

let string_of_mlty = (fun env e -> (let doc = (let _127_579 = (FStar_Extraction_ML_Util.flatten_mlpath env.FStar_Extraction_ML_Env.currentModule)
