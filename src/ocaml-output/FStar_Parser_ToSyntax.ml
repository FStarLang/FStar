...
| FStar_Parser_AST.TopLevelModule (_65_2663) -> begin
...
let tcs = (FStar_List.map (fun _65_2679 -> (match (_65_2679) with
| (x, _65_2678) -> begin
x
end)) tcs)
...
| FStar_Syntax_Syntax.Tm_let (lbs, _65_2688) -> begin
(
...
| (_65_2696)::_65_2694 -> begin
(FStar_List.map trans_qual quals)
end
| _65_2699 -> begin
(FStar_All.pipe_right (Prims.snd lbs) (FStar_List.collect (fun _65_24 -> (match (_65_24) with
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (_65_2710); FStar_Syntax_Syntax.lbunivs = _65_2708; FStar_Syntax_Syntax.lbtyp = _65_2706; FStar_Syntax_Syntax.lbeff = _65_2704; FStar_Syntax_Syntax.lbdef = _65_2702} -> begin
[]
end
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (fv); FStar_Syntax_Syntax.lbunivs = _65_2720; FStar_Syntax_Syntax.lbtyp = _65_2718; FStar_Syntax_Syntax.lbeff = _65_2716; FStar_Syntax_Syntax.lbdef = _65_2714} -> begin
(FStar_Parser_Env.lookup_letbinding_quals env fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
end))))
end)
in (

let quals = if (FStar_All.pipe_right lets (FStar_Util.for_some (fun _65_2728 -> (match (_65_2728) with
| (_65_2726, t) -> begin
...
let _65_2734 = fv
in {FStar_Syntax_Syntax.fv_name = _65_2734.FStar_Syntax_Syntax.fv_name; FStar_Syntax_Syntax.fv_delta = FStar_Syntax_Syntax.Delta_abstract (fv.FStar_Syntax_Syntax.fv_delta); FStar_Syntax_Syntax.fv_qual = _65_2734.FStar_Syntax_Syntax.fv_qual})); FStar_Syntax_Syntax.lbunivs = _65_2732.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = _65_2732.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = _65_2732.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = _65_2732.FStar_Syntax_Syntax.lbdef})))))
...
let _65_2768 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) FStar_Syntax_Const.exn_lid)
in (match (_65_2768) with
| (t, _65_2767) -> begin
(
...
let _65_2797 = (desugar_binders env binders)
...
let _65_2813 = (desugar_binders env eff_binders)
in (match (_65_2813) with
| (env, binders) -> begin
(

let _65_2824 = (

let _65_2816 = (head_and_args defn)
...
let sub = (fun _65_2830 -> (match (_65_2830) with
| (_65_2828, x) -> begin
(

let _65_2833 = (FStar_Syntax_Subst.open_term ed.FStar_Syntax_Syntax.binders x)
in (match (_65_2833) with
| (edb, x) -> begin
(

let _65_2834 = if ((FStar_List.length args) <> (FStar_List.length edb)) then begin
...
| FStar_Parser_AST.NewEffectForFree (_65_2841, FStar_Parser_AST.RedefineEffect (_65_2843)) -> begin
(FStar_All.failwith "impossible")
end
...
let _65_2884 = (match (l.FStar_Parser_AST.lift_op) with
...
let desugar_decls : FStar_Parser_Env.env  ->  FStar_Parser_AST.decl Prims.list  ->  (FStar_Parser_Env.env * FStar_Syntax_Syntax.sigelts) = (fun env decls -> (FStar_List.fold_left (fun _65_2890 d -> (match (_65_2890) with
| (env, sigelts) -> begin
(

let _65_2894 = (desugar_decl env d)
in (match (_65_2894) with
| (env, se) -> begin
...
let _65_2920 = (desugar_decls env decls)
...
| FStar_Parser_AST.Interface (mname, _65_2931, _65_2933) -> begin
...
let _65_2941 = (desugar_modul_common curmod env m)
...
let _65_2947 = (desugar_modul_common None env m)
...
let _65_2949 = if (FStar_Options.dump_module modul.FStar_Syntax_Syntax.name.FStar_Ident.str) then begin
...
let _65_2962 = (FStar_List.fold_left (fun _65_2955 m -> (match (_65_2955) with
| (env, mods) -> begin
(

let _65_2959 = (desugar_modul env m)
in (match (_65_2959) with
| (env, m) -> begin
((env), ((m)::mods))
end))
end)) ((env), ([])) f)
...
let _65_2967 = (FStar_Parser_Env.prepare_module_or_interface false false en m.FStar_Syntax_Syntax.name)
...
let _65_2968 = en
in {FStar_Parser_Env.curmodule = Some (m.FStar_Syntax_Syntax.name); FStar_Parser_Env.modules = _65_2968.FStar_Parser_Env.modules; FStar_Parser_Env.open_namespaces = _65_2968.FStar_Parser_Env.open_namespaces; FStar_Parser_Env.modul_abbrevs = _65_2968.FStar_Parser_Env.modul_abbrevs; FStar_Parser_Env.sigaccum = _65_2968.FStar_Parser_Env.sigaccum; FStar_Parser_Env.localbindings = _65_2968.FStar_Parser_Env.localbindings; FStar_Parser_Env.recbindings = _65_2968.FStar_Parser_Env.recbindings; FStar_Parser_Env.sigmap = _65_2968.FStar_Parser_Env.sigmap; FStar_Parser_Env.default_result_effect = _65_2968.FStar_Parser_Env.default_result_effect; FStar_Parser_Env.iface = _65_2968.FStar_Parser_Env.iface; FStar_Parser_Env.admitted_iface = _65_2968.FStar_Parser_Env.admitted_iface; FStar_Parser_Env.expect_typ = _65_2968.FStar_Parser_Env.expect_typ}) m.FStar_Syntax_Syntax.exports)
in (
