...
| FStar_Syntax_Syntax.Pat_cons (_65_541, pats) -> begin
(FStar_All.pipe_right pats (FStar_List.fold_left (fun out _65_549 -> (match (_65_549) with
| (p, _65_548) -> begin
...
let _65_572 = (match (((is_mut), (p.FStar_Parser_AST.pat))) with
...
let _65_586 = (push_bv_maybe_mut e x)
...
let _65_598 = (push_bv_maybe_mut e a)
in (match (_65_598) with
| (e, a) -> begin
(((a)::l), (e), (a))
end))
end))
...
| (loc, env, var, p, _65_621) -> begin
(

let _65_639 = (FStar_List.fold_left (fun _65_626 p -> (match (_65_626) with
| (loc, env, ps) -> begin
(

let _65_635 = (aux loc env p)
in (match (_65_635) with
| (loc, env, _65_631, p, _65_634) -> begin
((loc), (env), ((p)::ps))
end))
end)) ((loc), (env), ([])) ps)
in (match (_65_639) with
| (loc, env, ps) -> begin
(
...
let _65_659 = x
in {FStar_Syntax_Syntax.ppname = _65_659.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_659.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = t})), (aq))))
end)
...
let _65_678 = (resolvex loc env x)
in (match (_65_678) with
| (loc, env, xbv) -> begin
...
| FStar_Parser_AST.PatApp ({FStar_Parser_AST.pat = FStar_Parser_AST.PatName (l); FStar_Parser_AST.prange = _65_684}, args) -> begin
(

let _65_706 = (FStar_List.fold_right (fun arg _65_695 -> (match (_65_695) with
| (loc, env, args) -> begin
(

let _65_702 = (aux loc env arg)
in (match (_65_702) with
| (loc, env, _65_699, arg, imp) -> begin
((loc), (env), ((((arg), (imp)))::args))
end))
end)) args ((loc), (env), ([])))
in (match (_65_706) with
| (loc, env, args) -> begin
(
...
| FStar_Parser_AST.PatApp (_65_710) -> begin
(Prims.raise (FStar_Syntax_Syntax.Error ((("Unexpected pattern"), (p.FStar_Parser_AST.prange)))))
end
| FStar_Parser_AST.PatList (pats) -> begin
(

let _65_730 = (FStar_List.fold_right (fun pat _65_718 -> (match (_65_718) with
| (loc, env, pats) -> begin
(

let _65_726 = (aux loc env pat)
in (match (_65_726) with
| (loc, env, _65_722, pat, _65_725) -> begin
((loc), (env), ((pat)::pats))
end))
end)) pats ((loc), (env), ([])))
in (match (_65_730) with
| (loc, env, pats) -> begin
(
...
let _65_756 = (FStar_List.fold_left (fun _65_743 p -> (match (_65_743) with
| (loc, env, pats) -> begin
(

let _65_752 = (aux loc env p)
in (match (_65_752) with
| (loc, env, _65_748, pat, _65_751) -> begin
...
let _65_762 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) l)
in (match (_65_762) with
| (constr, _65_761) -> begin
(
...
let _65_776 = (FStar_List.hd fields)
in (match (_65_776) with
| (f, _65_775) -> begin
(

let _65_780 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_record_by_field_name env) f)
in (match (_65_780) with
| (record, _65_779) -> begin
(

let fields = (FStar_All.pipe_right fields (FStar_List.map (fun _65_783 -> (match (_65_783) with
| (f, p) -> begin
...
let args = (FStar_All.pipe_right record.FStar_Parser_Env.fields (FStar_List.map (fun _65_788 -> (match (_65_788) with
| (f, _65_787) -> begin
(match ((FStar_All.pipe_right fields (FStar_List.tryFind (fun _65_792 -> (match (_65_792) with
| (g, _65_791) -> begin
...
let _65_807 = (aux loc env app)
in (match (_65_807) with
| (env, e, b, p, _65_806) -> begin
(
...
in Some (_160_340))
in {FStar_Syntax_Syntax.fv_name = _65_812.FStar_Syntax_Syntax.fv_name; FStar_Syntax_Syntax.fv_delta = _65_812.FStar_Syntax_Syntax.fv_delta; FStar_Syntax_Syntax.fv_qual = _160_341}))
in ((_160_342), (args)))
...
let _65_824 = (aux [] env p)
in (match (_65_824) with
| (_65_818, env, b, p, _65_823) -> begin
(

let _65_825 = (let _160_345 = (check_linear_pattern_variables p)
...
| FStar_Parser_AST.PatVar (x, _65_837) -> begin
(mklet x)
end
| FStar_Parser_AST.PatAscribed ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (x, _65_844); FStar_Parser_AST.prange = _65_841}, t) -> begin
...
let _65_856 = (desugar_data_pat env p is_mut)
...
and desugar_match_pat_maybe_top : Prims.bool  ->  FStar_Parser_Env.env  ->  FStar_Parser_AST.pattern  ->  (env_t * FStar_Syntax_Syntax.pat) = (fun _65_868 env pat -> (

let _65_876 = (desugar_data_pat env pat false)
in (match (_65_876) with
| (env, _65_874, pat) -> begin
((env), (pat))
end)))
...
let _65_881 = env
in {FStar_Parser_Env.curmodule = _65_881.FStar_Parser_Env.curmodule; FStar_Parser_Env.curmonad = _65_881.FStar_Parser_Env.curmonad; FStar_Parser_Env.modules = _65_881.FStar_Parser_Env.modules; FStar_Parser_Env.scope_mods = _65_881.FStar_Parser_Env.scope_mods; FStar_Parser_Env.sigaccum = _65_881.FStar_Parser_Env.sigaccum; FStar_Parser_Env.sigmap = _65_881.FStar_Parser_Env.sigmap; FStar_Parser_Env.default_result_effect = _65_881.FStar_Parser_Env.default_result_effect; FStar_Parser_Env.iface = _65_881.FStar_Parser_Env.iface; FStar_Parser_Env.admitted_iface = _65_881.FStar_Parser_Env.admitted_iface; FStar_Parser_Env.expect_typ = false})
...
let _65_886 = env
in {FStar_Parser_Env.curmodule = _65_886.FStar_Parser_Env.curmodule; FStar_Parser_Env.curmonad = _65_886.FStar_Parser_Env.curmonad; FStar_Parser_Env.modules = _65_886.FStar_Parser_Env.modules; FStar_Parser_Env.scope_mods = _65_886.FStar_Parser_Env.scope_mods; FStar_Parser_Env.sigaccum = _65_886.FStar_Parser_Env.sigaccum; FStar_Parser_Env.sigmap = _65_886.FStar_Parser_Env.sigmap; FStar_Parser_Env.default_result_effect = _65_886.FStar_Parser_Env.default_result_effect; FStar_Parser_Env.iface = _65_886.FStar_Parser_Env.iface; FStar_Parser_Env.admitted_iface = _65_886.FStar_Parser_Env.admitted_iface; FStar_Parser_Env.expect_typ = true})
in (desugar_term_maybe_top false env e)))
and desugar_machine_integer : FStar_Parser_Env.env  ->  Prims.string  ->  (FStar_Const.signedness * FStar_Const.width)  ->  FStar_Range.range  ->  (FStar_Syntax_Syntax.term', FStar_Syntax_Syntax.term') FStar_Syntax_Syntax.syntax = (fun env repr _65_893 range -> (match (_65_893) with
...
let _65_916 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) l)
...
| FStar_Parser_AST.Var ({FStar_Ident.ns = _65_928; FStar_Ident.ident = _65_926; FStar_Ident.nsstr = _65_924; FStar_Ident.str = "cps"}) -> begin
...
let _65_940 = e
in {FStar_Syntax_Syntax.n = _65_940.FStar_Syntax_Syntax.n; FStar_Syntax_Syntax.tk = _65_940.FStar_Syntax_Syntax.tk; FStar_Syntax_Syntax.pos = top.FStar_Parser_AST.range; FStar_Syntax_Syntax.vars = _65_940.FStar_Syntax_Syntax.vars}))
in (match ((let _160_411 = (unparen top)
in _160_411.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.Wild -> begin
(setpos FStar_Syntax_Syntax.tun)
end
| FStar_Parser_AST.Labeled (_65_944) -> begin
...
| FStar_Parser_AST.Op ("*", (_65_972)::(_65_970)::[]) when (let _160_412 = (op_as_term env (Prims.parse_int "2") top.FStar_Parser_AST.range "*")
...
let _65_991 = (let _160_420 = (FStar_Syntax_Util.mk_tuple_lid (FStar_List.length targs) top.FStar_Parser_AST.range)
in (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) _160_420))
in (match (_65_991) with
| (tup, _65_990) -> begin
...
| FStar_Parser_AST.Name ({FStar_Ident.ns = _65_1009; FStar_Ident.ident = _65_1007; FStar_Ident.nsstr = _65_1005; FStar_Ident.str = "Type0"}) -> begin
(mk (FStar_Syntax_Syntax.Tm_type (FStar_Syntax_Syntax.U_zero)))
end
| FStar_Parser_AST.Name ({FStar_Ident.ns = _65_1018; FStar_Ident.ident = _65_1016; FStar_Ident.nsstr = _65_1014; FStar_Ident.str = "Type"}) -> begin
(mk (FStar_Syntax_Syntax.Tm_type (FStar_Syntax_Syntax.U_unknown)))
end
| FStar_Parser_AST.Name ({FStar_Ident.ns = _65_1027; FStar_Ident.ident = _65_1025; FStar_Ident.nsstr = _65_1023; FStar_Ident.str = "Effect"}) -> begin
(mk (FStar_Syntax_Syntax.Tm_constant (FStar_Const.Const_effect)))
end
| FStar_Parser_AST.Name ({FStar_Ident.ns = _65_1036; FStar_Ident.ident = _65_1034; FStar_Ident.nsstr = _65_1032; FStar_Ident.str = "True"}) -> begin
...
| FStar_Parser_AST.Name ({FStar_Ident.ns = _65_1045; FStar_Ident.ident = _65_1043; FStar_Ident.nsstr = _65_1041; FStar_Ident.str = "False"}) -> begin
...
| FStar_Parser_AST.Projector (eff_name, {FStar_Ident.idText = txt; FStar_Ident.idRange = _65_1050}) when ((is_special_effect_combinator txt) && (FStar_Parser_Env.is_effect_name env eff_name)) -> begin
...
let _65_1065 = (FStar_Parser_Env.fail_or2 (FStar_Parser_Env.try_lookup_id env) ident)
in (match (_65_1065) with
| (t1, mut) -> begin
(

let _65_1066 = if (not (mut)) then begin
...
let _65_1090 = (let _160_435 = (mk (FStar_Syntax_Syntax.Tm_fvar (head)))
in ((_160_435), (true)))
...
let args = (FStar_List.map (fun _65_1096 -> (match (_65_1096) with
| (t, imp) -> begin
(
...
let _65_1125 = (FStar_List.fold_left (fun _65_1108 b -> (match (_65_1108) with
| (env, tparams, typs) -> begin
(

let _65_1112 = (desugar_binder env b)
in (match (_65_1112) with
| (xopt, t) -> begin
(

let _65_1118 = (match (xopt) with
| None -> begin
...
let _65_1119 = x
in {FStar_Syntax_Syntax.ppname = _65_1119.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_1119.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = t})), (None)))::[]))), (_160_443)))
end))
end))
end)) ((env), ([]), ([])) (FStar_List.append binders (((FStar_Parser_AST.mk_binder (FStar_Parser_AST.NoName (t)) t.FStar_Parser_AST.range FStar_Parser_AST.Type None))::[])))
in (match (_65_1125) with
| (env, _65_1123, targs) -> begin
(

let _65_1129 = (let _160_444 = (FStar_Syntax_Util.mk_dtuple_lid (FStar_List.length targs) top.FStar_Parser_AST.range)
in (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) _160_444))
in (match (_65_1129) with
| (tup, _65_1128) -> begin
...
let _65_1136 = (uncurry binders t)
...
let _65_1150 = (as_binder env hd.FStar_Parser_AST.aqual bb)
in (match (_65_1150) with
| (b, env) -> begin
(aux env ((b)::bs) tl)
end))))
end))
...
let _65_1165 = (as_binder env None b)
in (match (_65_1165) with
| ((x, _65_1162), env) -> begin
(
...
let _65_1186 = (FStar_List.fold_left (fun _65_1174 pat -> (match (_65_1174) with
| (env, ftvs) -> begin
(match (pat.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatAscribed (_65_1177, t) -> begin
...
let _65_1210 = (desugar_binding_pat env p)
in (match (_65_1210) with
| (env, b, pat) -> begin
(

let _65_1261 = (match (b) with
| LetBinder (_65_1212) -> begin
(FStar_All.failwith "Impossible")
end
| LocalBinder (x, aq) -> begin
(
...
| (FStar_Syntax_Syntax.Tm_name (_65_1234), _65_1237) -> begin
(
...
| (FStar_Syntax_Syntax.Tm_app (_65_1243, args), FStar_Syntax_Syntax.Pat_cons (_65_1248, pats)) -> begin
(
...
| FStar_Parser_AST.App ({FStar_Parser_AST.tm = FStar_Parser_AST.Var (a); FStar_Parser_AST.range = rng; FStar_Parser_AST.level = _65_1263}, phi, _65_1270) when ((FStar_Ident.lid_equals a FStar_Syntax_Const.assert_lid) || (FStar_Ident.lid_equals a FStar_Syntax_Const.assume_lid)) -> begin
(
...
| FStar_Parser_AST.App (_65_1276) -> begin
(
...
let ds_let_rec_or_app = (fun _65_1310 -> (match (()) with
| () -> begin
(
...
let funs = (FStar_All.pipe_right bindings (FStar_List.map (fun _65_1314 -> (match (_65_1314) with
| (p, def) -> begin
...
| _65_1320 -> begin
(match (p.FStar_Parser_AST.pat) with
| FStar_Parser_AST.PatAscribed ({FStar_Parser_AST.pat = FStar_Parser_AST.PatVar (id, _65_1325); FStar_Parser_AST.prange = _65_1322}, t) -> begin
...
| FStar_Parser_AST.PatVar (id, _65_1334) -> begin
if top_level then begin
...
let _65_1367 = (FStar_List.fold_left (fun _65_1343 _65_1352 -> (match (((_65_1343), (_65_1352))) with
| ((env, fnames, rec_bindings), ((f, _65_1346, _65_1348), _65_1351)) -> begin
(

let _65_1363 = (match (f) with
| FStar_Util.Inl (x) -> begin
(

let _65_1357 = (FStar_Parser_Env.push_bv env x)
in (match (_65_1357) with
| (env, xx) -> begin
...
let desugar_one_def = (fun env lbname _65_1378 -> (match (_65_1378) with
| ((_65_1373, args, result_t), def) -> begin
(
...
let _65_1387 = if (is_comp_type env t) then begin
...
let _65_1413 = (desugar_binding_pat_maybe_top top_level env pat is_mutable)
in (match (_65_1413) with
...
let desugar_branch = (fun _65_1463 -> (match (_65_1463) with
| (pat, wopt, b) -> begin
(

let _65_1466 = (desugar_match_pat env pat)
...
| FStar_Parser_AST.Record (_65_1480, []) -> begin
(Prims.raise (FStar_Syntax_Syntax.Error ((("Unexpected empty record"), (top.FStar_Parser_AST.range)))))
end
| FStar_Parser_AST.Record (eopt, fields) -> begin
(

let _65_1491 = (FStar_List.hd fields)
in (match (_65_1491) with
| (f, _65_1490) -> begin
(

let _65_1495 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_record_by_field_name env) f)
in (match (_65_1495) with
| (record, _65_1494) -> begin
(

let fields' = (FStar_All.pipe_right fields (FStar_List.map (fun _65_1498 -> (match (_65_1498) with
| (f, e) -> begin
...
let found = (FStar_All.pipe_right fields' (FStar_Util.find_opt (fun _65_1506 -> (match (_65_1506) with
| (g, _65_1505) -> begin
(FStar_Ident.lid_equals f g)
end))))
in (match (found) with
| Some (_65_1509, e) -> begin
((f), (e))
end
| None -> begin
(match (xopt) with
...
| None -> begin
(let _160_589 = (let _160_588 = (FStar_All.pipe_right record.FStar_Parser_Env.fields (FStar_List.map (fun _65_1521 -> (match (_65_1521) with
| (f, _65_1520) -> begin
...
let record = (let _160_594 = (let _160_593 = (FStar_All.pipe_right record.FStar_Parser_Env.fields (FStar_List.map (fun _65_1529 -> (match (_65_1529) with
| (f, _65_1528) -> begin
...
| FStar_Syntax_Syntax.Tm_meta ({FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_app ({FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar (fv); FStar_Syntax_Syntax.tk = _65_1545; FStar_Syntax_Syntax.pos = _65_1543; FStar_Syntax_Syntax.vars = _65_1541}, args); FStar_Syntax_Syntax.tk = _65_1539; FStar_Syntax_Syntax.pos = _65_1537; FStar_Syntax_Syntax.vars = _65_1535}, FStar_Syntax_Syntax.Meta_desugared (FStar_Syntax_Syntax.Data_app)) -> begin
(
...
let _65_1566 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_projector_by_field_name env) f)
...
| _65_1577 when (top.FStar_Parser_AST.level = FStar_Parser_AST.Formula) -> begin
(desugar_formula env top)
end
| _65_1579 -> begin
(FStar_Parser_AST.error "Unexpected term" top top.FStar_Parser_AST.range)
end
| FStar_Parser_AST.Let (_65_1581, _65_1583, _65_1585) -> begin
(FStar_All.failwith "Not implemented yet")
end
| FStar_Parser_AST.QForall (_65_1589, _65_1591, _65_1593) -> begin
(FStar_All.failwith "Not implemented yet")
end
| FStar_Parser_AST.QExists (_65_1597, _65_1599, _65_1601) -> begin
(FStar_All.failwith "Not implemented yet")
end))))
and desugar_args : FStar_Parser_Env.env  ->  (FStar_Parser_AST.term * FStar_Parser_AST.imp) Prims.list  ->  (FStar_Syntax_Syntax.term * FStar_Syntax_Syntax.arg_qualifier Prims.option) Prims.list = (fun env args -> (FStar_All.pipe_right args (FStar_List.map (fun _65_1608 -> (match (_65_1608) with
| (a, imp) -> begin
...
let is_requires = (fun _65_1620 -> (match (_65_1620) with
| (t, _65_1619) -> begin
(match ((let _160_618 = (unparen t)
in _160_618.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.Requires (_65_1622) -> begin
true
end
| _65_1625 -> begin
false
...
let is_ensures = (fun _65_1630 -> (match (_65_1630) with
| (t, _65_1629) -> begin
(match ((let _160_621 = (unparen t)
in _160_621.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.Ensures (_65_1632) -> begin
true
end
| _65_1635 -> begin
false
...
let is_app = (fun head _65_1641 -> (match (_65_1641) with
| (t, _65_1640) -> begin
(match ((let _160_626 = (unparen t)
in _160_626.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.App ({FStar_Parser_AST.tm = FStar_Parser_AST.Var (d); FStar_Parser_AST.range = _65_1645; FStar_Parser_AST.level = _65_1643}, _65_1650, _65_1652) -> begin
...
let _65_1662 = (head_and_args t)
in (match (_65_1662) with
| (head, args) -> begin
...
| _65_1693 when default_ok -> begin
...
let _65_1700 = (pre_process_comp_typ t)
in (match (_65_1700) with
| ((eff, cattributes), args) -> begin
(

let _65_1701 = if ((FStar_List.length args) = (Prims.parse_int "0")) then begin
...
let _65_1705 = (let _160_638 = (FStar_List.hd args)
...
let _65_1730 = (FStar_All.pipe_right rest (FStar_List.partition (fun _65_1711 -> (match (_65_1711) with
| (t, _65_1710) -> begin
(match (t.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Tm_app ({FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar (fv); FStar_Syntax_Syntax.tk = _65_1717; FStar_Syntax_Syntax.pos = _65_1715; FStar_Syntax_Syntax.vars = _65_1713}, (_65_1722)::[]) -> begin
...
in (match (_65_1730) with
| (dec, rest) -> begin
(

let decreases_clause = (FStar_All.pipe_right dec (FStar_List.map (fun _65_1734 -> (match (_65_1734) with
| (t, _65_1733) -> begin
(match (t.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Tm_app (_65_1736, ((arg, _65_1739))::[]) -> begin
...
let _65_1783 = t
in {FStar_Syntax_Syntax.n = _65_1783.FStar_Syntax_Syntax.n; FStar_Syntax_Syntax.tk = _65_1783.FStar_Syntax_Syntax.tk; FStar_Syntax_Syntax.pos = f.FStar_Parser_AST.range; FStar_Syntax_Syntax.vars = _65_1783.FStar_Syntax_Syntax.vars}))
in (
...
let _65_1790 = b
in {FStar_Parser_AST.b = _65_1790.FStar_Parser_AST.b; FStar_Parser_AST.brange = _65_1790.FStar_Parser_AST.brange; FStar_Parser_AST.blevel = FStar_Parser_AST.Formula; FStar_Parser_AST.aqual = _65_1790.FStar_Parser_AST.aqual}))
in (
...
let _65_1804 = (FStar_Parser_Env.push_bv env a)
in (match (_65_1804) with
| (env, a) -> begin
(
...
let _65_1805 = a
in {FStar_Syntax_Syntax.ppname = _65_1805.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_1805.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = k})
in (
...
let _65_1912 = (FStar_List.fold_left (fun _65_1893 b -> (match (_65_1893) with
| (env, out) -> begin
(
...
let _65_1895 = b
in {FStar_Parser_AST.b = _65_1895.FStar_Parser_AST.b; FStar_Parser_AST.brange = _65_1895.FStar_Parser_AST.brange; FStar_Parser_AST.blevel = FStar_Parser_AST.Formula; FStar_Parser_AST.aqual = _65_1895.FStar_Parser_AST.aqual}))
in (match (tk) with
| (Some (a), k) -> begin
(

let _65_1904 = (FStar_Parser_Env.push_bv env a)
in (match (_65_1904) with
| (env, a) -> begin
(
...
let _65_1905 = a
in {FStar_Syntax_Syntax.ppname = _65_1905.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_1905.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = k})
...
let _65_1945 = (FStar_Syntax_Util.args_of_binders binders)
in (match (_65_1945) with
| (binders, args) -> begin
(

let imp_binders = (FStar_All.pipe_right binders (FStar_List.map (fun _65_1949 -> (match (_65_1949) with
| (x, _65_1948) -> begin
...
let tps = (FStar_List.map2 (fun _65_1973 _65_1977 -> (match (((_65_1973), (_65_1977))) with
| ((_65_1971, imp), (x, _65_1976)) -> begin
((x), (imp))
...
let _65_1981 = (FStar_Syntax_Util.head_and_args t)
in (match (_65_1981) with
| (head, args0) -> begin
(
...
| (((_65_1995, Some (FStar_Syntax_Syntax.Implicit (_65_1997))))::tps', ((_65_2004, Some (FStar_Syntax_Syntax.Implicit (_65_2006))))::args') -> begin
(arguments tps' args')
end
| (((_65_2014, Some (FStar_Syntax_Syntax.Implicit (_65_2016))))::tps', ((_65_2024, _65_2026))::_65_2022) -> begin
(arguments tps' args)
end
| (((_65_2033, _65_2035))::_65_2031, ((a, Some (FStar_Syntax_Syntax.Implicit (_65_2042))))::_65_2039) -> begin
(Prims.raise (FStar_Syntax_Syntax.Error ((("Unexpected implicit annotation on argument"), (a.FStar_Syntax_Syntax.pos)))))
end
| (((_65_2050, _65_2052))::tps', ((_65_2057, _65_2059))::args') -> begin
(arguments tps' args')
end))
in (arguments inductive_tps args0))
in (

let indices = (FStar_All.pipe_right args (FStar_List.map (fun _65_2064 -> (let _160_779 = (FStar_Syntax_Syntax.new_bv (Some (p)) FStar_Syntax_Syntax.tun)
...
in (let _160_783 = (FStar_All.pipe_right (FStar_List.append tps indices) (FStar_List.map (fun _65_2069 -> (match (_65_2069) with
| (x, imp) -> begin
...
let _65_2073 = (projectee arg_typ)
...
in {FStar_Syntax_Syntax.ppname = _65_2073.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_2073.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = _160_792}))
...
let imp_binders = (let _160_795 = (FStar_All.pipe_right indices (FStar_List.map (fun _65_2083 -> (match (_65_2083) with
| (x, _65_2082) -> begin
((x), (Some (FStar_Syntax_Syntax.imp_tag)))
end))))
...
let subst = (FStar_All.pipe_right fields (FStar_List.mapi (fun i _65_2091 -> (match (_65_2091) with
| (a, _65_2090) -> begin
(

let _65_2095 = (FStar_Syntax_Util.mk_field_projector_name lid a i)
in (match (_65_2095) with
| (field_name, _65_2094) -> begin
(
...
in (let _160_838 = (FStar_All.pipe_right fields (FStar_List.mapi (fun i _65_2104 -> (match (_65_2104) with
| (x, _65_2103) -> begin
(

let _65_2108 = (FStar_Syntax_Util.mk_field_projector_name lid x i)
in (match (_65_2108) with
| (field_name, _65_2107) -> begin
(
...
let arg_pats = (FStar_All.pipe_right all_params (FStar_List.mapi (fun j _65_2130 -> (match (_65_2130) with
| (x, imp) -> begin
(
...
let mk_data_projectors : FStar_Syntax_Syntax.qualifier Prims.list  ->  FStar_Parser_Env.env  ->  (FStar_Syntax_Syntax.binders * FStar_Syntax_Syntax.sigelt)  ->  FStar_Syntax_Syntax.sigelt Prims.list = (fun iquals env _65_2144 -> (match (_65_2144) with
| (inductive_tps, se) -> begin
(match (se) with
| FStar_Syntax_Syntax.Sig_datacon (lid, _65_2147, t, l, n, quals, _65_2153, _65_2155) when (not ((FStar_Ident.lid_equals lid FStar_Syntax_Const.lexcons_lid))) -> begin
(

let refine_domain = if (FStar_All.pipe_right quals (FStar_Util.for_some (fun _65_14 -> (match (_65_14) with
| FStar_Syntax_Syntax.RecordConstructor (_65_2160) -> begin
true
end
| _65_2163 -> begin
false
...
let _65_2171 = (FStar_Syntax_Util.arrow_formals t)
in (match (_65_2171) with
| (formals, cod) -> begin
(match (formals) with
| [] -> begin
[]
...
let _65_2187 = (FStar_Util.first_N n formals)
in (match (_65_2187) with
| (tps, rest) -> begin
...
let mfields = (FStar_List.map (fun _65_2279 -> (match (_65_2279) with
| (x, t, _65_2278) -> begin
...
in (let _160_903 = (FStar_All.pipe_right fields (FStar_List.map (fun _65_2288 -> (match (_65_2288) with
| (x, _65_2285, _65_2287) -> begin
...
let _65_2304 = (typars_of_binders _env binders)
...
let _65_2332 = (FStar_List.fold_left (fun _65_2323 _65_2326 -> (match (((_65_2323), (_65_2326))) with
| ((env, tps), (x, imp)) -> begin
(

let _65_2329 = (FStar_Parser_Env.push_bv env x.FStar_Syntax_Syntax.ppname)
in (match (_65_2329) with
| (env, y) -> begin
((env), ((((y), (imp)))::tps))
end))
end)) ((env), ([])) bs)
in (match (_65_2332) with
| (env, bs) -> begin
...
let _65_2351 = (desugar_abstract_tc quals env [] tc)
in (match (_65_2351) with
| (_65_2345, _65_2347, se, _65_2350) -> begin
(

let se = (match (se) with
| FStar_Syntax_Syntax.Sig_inductive_typ (l, _65_2354, typars, k, [], [], quals, rng) -> begin
(
...
let _65_2363 = (let _160_923 = (FStar_Range.string_of_range rng)
...
let _65_2383 = (typars_of_binders env binders)
in (match (_65_2383) with
| (env', typars) -> begin
(
...
let _65_2421 = (match ((let _160_929 = (unparen t)
in _160_929.FStar_Parser_AST.tm)) with
| FStar_Parser_AST.Construct (head, args) -> begin
(

let _65_2416 = (match ((FStar_List.rev args)) with
| ((last_arg, _65_2405))::args_rev -> begin
...
| (FStar_Parser_AST.TyconRecord (_65_2434))::[] -> begin
(

let trec = (FStar_List.hd tcs)
in (

let _65_2440 = (tycon_record_as_variant trec)
in (match (_65_2440) with
| (t, fs) -> begin
(desugar_tycon env rng ((FStar_Syntax_Syntax.RecordType (fs))::quals) ((t)::[]))
end)))
end
| (_65_2444)::_65_2442 -> begin
(
...
let _65_2455 = et
in (match (_65_2455) with
| (env, tcs) -> begin
(match (tc) with
| FStar_Parser_AST.TyconRecord (_65_2457) -> begin
(

let trec = tc
in (

let _65_2462 = (tycon_record_as_variant trec)
...
let _65_2474 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (_65_2474) with
| (env, _65_2471, se, tconstr) -> begin
...
let _65_2486 = (desugar_abstract_tc quals env mutuals (FStar_Parser_AST.TyconAbstract (((id), (binders), (kopt)))))
in (match (_65_2486) with
| (env, _65_2483, se, tconstr) -> begin
...
let _65_2491 = (FStar_List.fold_left (collect_tcs quals) ((env), ([])) tcs)
...
| FStar_Util.Inr (FStar_Syntax_Syntax.Sig_inductive_typ (id, uvs, tpars, k, _65_2499, _65_2501, _65_2503, _65_2505), t, quals) -> begin
(

let _65_2515 = (push_tparams env tpars)
in (match (_65_2515) with
| (env_tps, _65_2514) -> begin
(
...
| FStar_Util.Inl (FStar_Syntax_Syntax.Sig_inductive_typ (tname, univs, tpars, k, mutuals, _65_2523, tags, _65_2526), constrs, tconstr, quals) -> begin
(

let tycon = ((tname), (tpars), (k))
in (

let _65_2537 = (push_tparams env tpars)
in (match (_65_2537) with
| (env_tps, tps) -> begin
(

let data_tpars = (FStar_List.map (fun _65_2541 -> (match (_65_2541) with
| (x, _65_2540) -> begin
((x), (Some (FStar_Syntax_Syntax.Implicit (true))))
end)) tps)
in (

let _65_2567 = (let _160_957 = (FStar_All.pipe_right constrs (FStar_List.map (fun _65_2548 -> (match (_65_2548) with
| (id, topt, _65_2546, of_notation) -> begin
(
...
| FStar_Syntax_Syntax.Sig_inductive_typ (tname, _65_2578, tps, k, _65_2582, constrs, quals, _65_2586) when ((FStar_List.length constrs) > (Prims.parse_int "1")) -> begin
(
...
let _65_2615 = (FStar_List.fold_left (fun _65_2600 b -> (match (_65_2600) with
| (env, binders) -> begin
(match ((desugar_binder env b)) with
| (Some (a), k) -> begin
(

let _65_2608 = (FStar_Parser_Env.push_bv env a)
in (match (_65_2608) with
| (env, a) -> begin
(let _160_968 = (let _160_967 = (FStar_Syntax_Syntax.mk_binder (

let _65_2609 = a
in {FStar_Syntax_Syntax.ppname = _65_2609.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _65_2609.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = k}))
in (_160_967)::binders)
in ((env), (_160_968)))
end))
end
...
let _65_2629 = (desugar_binders monad_env eff_binders)
in (match (_65_2629) with
| (env, binders) -> begin
(
...
let _65_2640 = (FStar_All.pipe_right eff_decls (FStar_List.fold_left (fun _65_2633 decl -> (match (_65_2633) with
| (env, out) -> begin
(

let _65_2637 = (desugar_decl env decl)
in (match (_65_2637) with
| (env, ses) -> begin
...
| FStar_Parser_AST.Tycon (_65_2644, ((FStar_Parser_AST.TyconAbbrev (name, _65_2647, _65_2649, {FStar_Parser_AST.tm = FStar_Parser_AST.Construct (_65_2655, ((def, _65_2662))::((cps_type, _65_2658))::[]); FStar_Parser_AST.range = _65_2653; FStar_Parser_AST.level = _65_2651}), _65_2671))::[]) when (not (for_free)) -> begin
...
| FStar_Parser_AST.Tycon (_65_2677, ((FStar_Parser_AST.TyconAbbrev (name, _65_2680, _65_2682, defn), _65_2687))::[]) when for_free -> begin
...
let _65_2724 = (desugar_binders env eff_binders)
...
let _65_2727 = (head_and_args defn)
in (match (_65_2727) with
| (head, args) -> begin
(
...
let _65_2747 = (match ((FStar_List.rev args)) with
| ((last_arg, _65_2736))::args_rev -> begin
...
let sub = (fun _65_2757 -> (match (_65_2757) with
| (_65_2755, x) -> begin
(

let _65_2760 = (FStar_Syntax_Subst.open_term ed.FStar_Syntax_Syntax.binders x)
in (match (_65_2760) with
| (edb, x) -> begin
(

let _65_2761 = if ((FStar_List.length args) <> (FStar_List.length edb)) then begin
...
| FStar_Parser_AST.Fsdoc (_65_2783) -> begin
((env), ([]))
end
...
let tcs = (FStar_List.map (fun _65_2801 -> (match (_65_2801) with
| (x, _65_2800) -> begin
x
end)) tcs)
...
| FStar_Syntax_Syntax.Tm_let (lbs, _65_2810) -> begin
(
...
| (_65_2818)::_65_2816 -> begin
(FStar_List.map (trans_qual None) quals)
end
| _65_2821 -> begin
(FStar_All.pipe_right (Prims.snd lbs) (FStar_List.collect (fun _65_25 -> (match (_65_25) with
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (_65_2832); FStar_Syntax_Syntax.lbunivs = _65_2830; FStar_Syntax_Syntax.lbtyp = _65_2828; FStar_Syntax_Syntax.lbeff = _65_2826; FStar_Syntax_Syntax.lbdef = _65_2824} -> begin
[]
end
| {FStar_Syntax_Syntax.lbname = FStar_Util.Inr (fv); FStar_Syntax_Syntax.lbunivs = _65_2842; FStar_Syntax_Syntax.lbtyp = _65_2840; FStar_Syntax_Syntax.lbeff = _65_2838; FStar_Syntax_Syntax.lbdef = _65_2836} -> begin
(FStar_Parser_Env.lookup_letbinding_quals env fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
end))))
end)
in (

let quals = if (FStar_All.pipe_right lets (FStar_Util.for_some (fun _65_2850 -> (match (_65_2850) with
| (_65_2848, t) -> begin
...
let _65_2856 = fv
in {FStar_Syntax_Syntax.fv_name = _65_2856.FStar_Syntax_Syntax.fv_name; FStar_Syntax_Syntax.fv_delta = FStar_Syntax_Syntax.Delta_abstract (fv.FStar_Syntax_Syntax.fv_delta); FStar_Syntax_Syntax.fv_qual = _65_2856.FStar_Syntax_Syntax.fv_qual})); FStar_Syntax_Syntax.lbunivs = _65_2854.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = _65_2854.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = _65_2854.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = _65_2854.FStar_Syntax_Syntax.lbdef})))))
...
let _65_2890 = (FStar_Parser_Env.fail_or env (FStar_Parser_Env.try_lookup_lid env) FStar_Syntax_Const.exn_lid)
in (match (_65_2890) with
| (t, _65_2889) -> begin
(
...
let _65_2919 = (desugar_binders env binders)
...
let _65_2983 = (match (l.FStar_Parser_AST.lift_op) with
...
let desugar_decls : FStar_Parser_Env.env  ->  FStar_Parser_AST.decl Prims.list  ->  (FStar_Parser_Env.env * FStar_Syntax_Syntax.sigelts) = (fun env decls -> (FStar_List.fold_left (fun _65_2989 d -> (match (_65_2989) with
| (env, sigelts) -> begin
(

let _65_2993 = (desugar_decl env d)
...
in (match (_65_3016) with
| ((env, pop_when_done), mname, decls, intf) -> begin
(

let _65_3019 = (desugar_decls env decls)
...
| FStar_Parser_AST.Interface (mname, _65_3030, _65_3032) -> begin
...
let _65_3040 = (desugar_modul_common curmod env m)
...
let _65_3046 = (desugar_modul_common None env m)
in (match (_65_3046) with
...
let _65_3048 = if (FStar_Options.dump_module modul.FStar_Syntax_Syntax.name.FStar_Ident.str) then begin
...
let _65_3061 = (FStar_List.fold_left (fun _65_3054 m -> (match (_65_3054) with
| (env, mods) -> begin
(

let _65_3058 = (desugar_modul env m)
in (match (_65_3058) with
| (env, m) -> begin
((env), ((m)::mods))
end))
end)) ((env), ([])) f)
in (match (_65_3061) with
| (env, mods) -> begin
...
let _65_3066 = (FStar_Parser_Env.prepare_module_or_interface false false en m.FStar_Syntax_Syntax.name)
in (match (_65_3066) with
...
let _65_3067 = en
in {FStar_Parser_Env.curmodule = Some (m.FStar_Syntax_Syntax.name); FStar_Parser_Env.curmonad = _65_3067.FStar_Parser_Env.curmonad; FStar_Parser_Env.modules = _65_3067.FStar_Parser_Env.modules; FStar_Parser_Env.scope_mods = _65_3067.FStar_Parser_Env.scope_mods; FStar_Parser_Env.sigaccum = _65_3067.FStar_Parser_Env.sigaccum; FStar_Parser_Env.sigmap = _65_3067.FStar_Parser_Env.sigmap; FStar_Parser_Env.default_result_effect = _65_3067.FStar_Parser_Env.default_result_effect; FStar_Parser_Env.iface = _65_3067.FStar_Parser_Env.iface; FStar_Parser_Env.admitted_iface = _65_3067.FStar_Parser_Env.admitted_iface; FStar_Parser_Env.expect_typ = _65_3067.FStar_Parser_Env.expect_typ}) m.FStar_Syntax_Syntax.exports)
in (
