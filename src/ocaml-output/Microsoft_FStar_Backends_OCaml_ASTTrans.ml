...
let rec in_ns = (fun _497465 -> (match (_497465) with
| ([], _) -> begin
true
end
...
in (let outsupport = (fun _497512 -> (match (_497512) with
| (ns1, ns2) -> begin
...
in (fun x -> (let _497569 = (Support.Microsoft.FStar.Util.incr c)
in (x, (! (c))))))
...
let lenv_of_mlenv = (fun _497577 -> lempty)

let lpush = (fun _497580 real pp -> (match (_497580) with
| LEnv (lenv) -> begin
(let mlid = (fresh pp.Microsoft_FStar_Absyn_Syntax.idText)
in (let _497584 = (Support.Microsoft.FStar.Util.smap_add lenv real.Microsoft_FStar_Absyn_Syntax.idText mlid)
in (LEnv (lenv), mlid)))
end))

let lresolve = (fun _497587 x -> (match (_497587) with
| LEnv (lenv) -> begin
...
let tvsym = (fun _497595 -> (match (_497595) with
| (x, n) -> begin
...
in (let _497628 = (let for1 = (fun used tv -> (match (tv) with
| Some ((real, pp)) -> begin
(let _497617 = (freshen used (Some (pp)))
in (match (_497617) with
| (used, pp) -> begin
(used, ((fresh pp), Some (real.Microsoft_FStar_Absyn_Syntax.idText)))
end))
end
| None -> begin
(let _497621 = (freshen used None)
in (match (_497621) with
| (used, pp) -> begin
(used, ((fresh pp), None))
end))
end))
...
in (match (_497628) with
| (_, tvs) -> begin
(let tparams = (Support.List.map (fun _497632 -> (match (_497632) with
| (x, _) -> begin
(tvsym x)
end)) tvs)
in (let tvs = (Support.List.choose (fun _497636 -> (match (_497636) with
| (x, y) -> begin
(match (y) with
| None -> begin
None
...
let tvar_of_btvar = (fun _497642 x -> (match (_497642) with
| TEnv (tenv) -> begin
...
(let args = (Support.List.collect (fun _497466 -> (match (_497466) with
...
let mlkind_of_kind = (fun tps k -> (let mltparam_of_tparam = (fun _497467 -> (match (_497467) with
...
let rec mlty_of_ty_core = (fun mlenv tenv _497830 -> (match (_497830) with
| (rg, ty) -> begin
...
and maybe_named = (fun mlenv tenv _497944 -> (match (_497944) with
| (rg, ty) -> begin
(let rg = ty.Microsoft_FStar_Absyn_Syntax.pos
in (let rec aux = (fun acc _497950 -> (match (_497950) with
| (rg, ty) -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _497468 -> (match (_497468) with
...
(let tys = ((Support.List.map (fun _497469 -> (match (_497469) with
...
and maybe_tuple = (fun mlenv tenv _497996 -> (match (_497996) with
| (rg, ty) -> begin
(let rg = ty.Microsoft_FStar_Absyn_Syntax.pos
...
| Microsoft_FStar_Absyn_Syntax.Typ_app ((head, args)) -> begin
if ((Support.Microsoft.FStar.Util.for_some (fun _497470 -> (match (_497470) with
| (Support.Microsoft.FStar.Util.Inr (_), _) -> begin
true
end
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
...
(let tys = ((Support.List.map (fun _497471 -> (match (_497471) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
t
end
| _ -> begin
...
(let _498105 = ((Support.List.partition (fun _497472 -> (match (_497472) with
| (Support.Microsoft.FStar.Util.Inl ({Microsoft_FStar_Absyn_Syntax.v = x; Microsoft_FStar_Absyn_Syntax.sort = {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Kind_type; Microsoft_FStar_Absyn_Syntax.tk = _; Microsoft_FStar_Absyn_Syntax.pos = _; Microsoft_FStar_Absyn_Syntax.fvs = _; Microsoft_FStar_Absyn_Syntax.uvs = _}; Microsoft_FStar_Absyn_Syntax.p = _}), _) -> begin
true
end
| _ -> begin
...
in (match (_498105) with
| (ts, vs) -> begin
(let ts = ((Support.List.collect (fun _497473 -> (match (_497473) with
...
let mlscheme_of_ty = (fun mlenv rg ty -> (let _498136 = (strip_polymorphism [] rg ty)
in (match (_498136) with
| (tparams, rg, ty) -> begin
(let some = (fun x -> Some (x))
in (let _498141 = (tenv_of_tvmap (Support.List.map some tparams))
in (match (_498141) with
| (tenv, tparams) -> begin
...
(let _498164 = (Support.Microsoft.FStar.Util.fold_map (fun le pat -> (mlpat_of_pat mlenv pat.Microsoft_FStar_Absyn_Syntax.p le pat)) le ps)
in (match (_498164) with
| (le, ps) -> begin
(le, Microsoft_FStar_Backends_OCaml_Syntax.MLP_Tuple (ps))
end))
end else begin
(let _498167 = (Support.Microsoft.FStar.Util.fold_map (mlpat_of_pat mlenv rg) le ps)
in (match (_498167) with
| (le, ps) -> begin
...
(let _498180 = (lpush le x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname)
in (match (_498180) with
| (le, mlid) -> begin
...
(let _498187 = (Support.Microsoft.FStar.Util.fold_map (mlpat_of_pat mlenv rg) le ps)
in (match (_498187) with
| (le, ps) -> begin
...
in (let rec bvs = (fun _497474 -> (match (_497474) with
| 0 -> begin
[]
end
| n -> begin
(let _498218 = (Support.Microsoft.FStar.Util.incr ctr)
...
(mlexpr_of_expr mlenv rg lenv (let _498257 = e
in {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Exp_app ((a1, (a2)::[])); Microsoft_FStar_Absyn_Syntax.tk = _498257.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = _498257.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _498257.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _498257.Microsoft_FStar_Absyn_Syntax.uvs}))
end
...
(mlexpr_of_expr mlenv rg lenv (let _498275 = e
in {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Exp_app ((a2, (a1)::[])); Microsoft_FStar_Absyn_Syntax.tk = _498275.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = _498275.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _498275.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _498275.Microsoft_FStar_Absyn_Syntax.uvs}))
end
...
| Some (k) -> begin
(let args = (Support.List.collect (fun _497475 -> (match (_497475) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
...
| _ -> begin
(let args = (Support.List.collect (fun _497476 -> (match (_497476) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
...
in (let _498353 = (match ((Support.List.rev c.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ns)) with
| [] -> begin
("", [])
end
| h::t -> begin
...
in (let rid = (let _498382 = x.Microsoft_FStar_Absyn_Syntax.v
in {Microsoft_FStar_Absyn_Syntax.ns = _498382.Microsoft_FStar_Absyn_Syntax.ns; Microsoft_FStar_Absyn_Syntax.ident = (let _498384 = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ident
in {Microsoft_FStar_Absyn_Syntax.idText = sub; Microsoft_FStar_Absyn_Syntax.idRange = _498384.Microsoft_FStar_Absyn_Syntax.idRange}); Microsoft_FStar_Absyn_Syntax.nsstr = _498382.Microsoft_FStar_Absyn_Syntax.nsstr; Microsoft_FStar_Absyn_Syntax.str = sub})
...
in (let _498397 = (Support.List.rev x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ns)
in (match (_498397) with
| cn::cr -> begin
(let crstr = (Support.List.map (fun x -> x.Microsoft_FStar_Absyn_Syntax.idText) cr)
in (let rid = {Microsoft_FStar_Absyn_Syntax.ns = cr; Microsoft_FStar_Absyn_Syntax.ident = (let _498400 = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ident
in {Microsoft_FStar_Absyn_Syntax.idText = cn.Microsoft_FStar_Absyn_Syntax.idText; Microsoft_FStar_Absyn_Syntax.idRange = _498400.Microsoft_FStar_Absyn_Syntax.idRange}); Microsoft_FStar_Absyn_Syntax.nsstr = (Support.String.concat "." crstr); Microsoft_FStar_Absyn_Syntax.str = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.nsstr}
...
(let _498438 = (lpush lenv x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname)
in (match (_498438) with
| (lenv, mlid) -> begin
...
(let _498519 = (mllets_of_lets mlenv rg lenv (rec_, lb))
in (match (_498519) with
| (lenv, bindings) -> begin
...
(let _498526 = (Support.Prims._assert ())
in (let _498548 = (match (e.Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_498548) with
| (c, args) -> begin
(let args = ((Support.List.collect (fun _497477 -> (match (_497477) with
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(e)::[]
end
| _ -> begin
...
and mllets_of_lets = (fun mlenv rg lenv _498610 -> (match (_498610) with
| (rec_, lbs) -> begin
(let downct = (fun _498616 -> (match (_498616) with
| (x, _, e) -> begin
(match (x) with
...
in (let _498630 = (Support.Microsoft.FStar.Util.fold_map (fun lenv _498627 -> (match (_498627) with
| (x, _) -> begin
(lpush lenv x.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.ppname)
end)) lenv lbs)
in (match (_498630) with
| (lenvb, mlids) -> begin
...
in (Support.List.map (fun _498634 -> (match (_498634) with
| (x, e) -> begin
...
and mlbranch_of_branch = (fun mlenv rg lenv _498643 -> (match (_498643) with
| (pat, when_, body) -> begin
(let _498646 = (mlpat_of_pat mlenv rg lenv pat)
in (match (_498646) with
| (lenv, pat) -> begin
...
let mldtype_of_indt = (fun mlenv indt -> (let rec getRecordFieldsFromType = (fun _497478 -> (match (_497478) with
| [] -> begin
None
end
...
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, c)) -> begin
(Support.List.append ((Support.List.collect (fun _497479 -> (match (_497479) with
...
in (let _498806 = (let fold1 = (fun sigelt _498737 -> (match (_498737) with
| (types, ctors) -> begin
(match (sigelt) with
...
| (Some (f), c::[]) -> begin
(let _498756 = (Support.Microsoft.FStar.Util.smap_add record_constructors c.Microsoft_FStar_Absyn_Syntax.str f)
...
in (let anames = (Support.List.map (fun _497480 -> (match (_497480) with
| None -> begin
(let _498777 = (Support.Microsoft.FStar.Util.incr actr)
in (Support.String.strcat "_" (Support.Microsoft.FStar.Util.string_of_int (! (actr)))))
end
| Some (x) -> begin
(let _498781 = (Support.Microsoft.FStar.Util.incr actr)
...
in (let _498784 = (Support.Microsoft.FStar.Util.smap_add algebraic_constructors x.Microsoft_FStar_Absyn_Syntax.str ((Support.List.length anames), anames))
...
in (let _498803 = (Support.List.fold_right fold1 indt ([], []))
in (match (_498803) with
| (ts, cs) -> begin
(ts, (Support.Microsoft.FStar.Util.smap_of_list cs))
end)))
in (match (_498806) with
| (ts, cs) -> begin
(let cons_args = (fun cname tparams rg x -> (let _498815 = ((Support.Microsoft.FStar.Util.must) (Support.Microsoft.FStar.Util.smap_try_find cs cname.Microsoft_FStar_Absyn_Syntax.ident.Microsoft_FStar_Absyn_Syntax.idText))
in (match (_498815) with
| (c, _) -> begin
(let _498819 = (strip_polymorphism [] rg c)
in (match (_498819) with
| (cparams, rgty, c) -> begin
(let _498820 = if ((Support.List.length cparams) <> (Support.List.length tparams)) then begin
(unexpected rg "invalid-number-of-ctor-params")
end
in (let cparams = (Support.List.map (fun _498825 -> (match (_498825) with
| (x, _) -> begin
...
in (let _498832 = (mltycons_of_mlty c)
in (match (_498832) with
| (args, name) -> begin
(match (name) with
...
in (let _498844 = if ((Support.List.length tyargs) <> (Support.List.length cparams)) then begin
(unexpected rg "dtype-invalid-ctor-result")
end
in (let _498846 = if (not ((Support.List.forall2 check tparams tyargs))) then begin
...
in (let _498877 = if ((Support.List.length f) <> (Support.List.length args)) then begin
...
let mlmod1_of_mod1 = (fun mode mlenv modx -> (let export_val = (fun qal -> (let export_val1 = (fun _497481 -> (match (_497481) with
...
(let _498916 = (mlscheme_of_ty mlenv rg ty)
in (match (_498916) with
| (tparams, ty) -> begin
...
(let downct = (fun _498938 -> (match (_498938) with
| (x, _, e) -> begin
(match (x) with
...
in (let lbs = (Support.List.map downct lbs)
in (let lbs = (Support.List.map (fun _498947 -> (match (_498947) with
| (x, e) -> begin
...
in (let _498982 = (tenv_of_tvmap ar)
in (match (_498982) with
| (tenv, tparams) -> begin
...
in (let _499000 = (tenv_of_tvmap ar)
...
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, c)) -> begin
(let tys = ((Support.List.collect (fun _497482 -> (match (_497482) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (x), _) -> begin
...
in (let aout = (Support.List.map (fun _499099 -> (match (_499099) with
| (x, y, z) -> begin
(x, y, Some (z))
end)) aout)
in (match (mode) with
...
let mlmod_of_mod = (fun mlenv modx -> (let asright = (fun _497483 -> (match (_497483) with
| Support.Microsoft.FStar.Util.Inr (x) -> begin
x
end
| Support.Microsoft.FStar.Util.Inl (_) -> begin
...
let mlsig_of_sig = (fun mlenv modx -> (let asleft = (fun _497484 -> (match (_497484) with
| Support.Microsoft.FStar.Util.Inl (x) -> begin
x
end
| Support.Microsoft.FStar.Util.Inr (_) -> begin
...
in (let _499149 = (Support.Microsoft.FStar.Util.fprint1 "OCaml skip: %s\n" fmod_.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.ident.Microsoft_FStar_Absyn_Syntax.idText)
...
let rec mllib_add = (fun _499152 _499156 -> (match ((_499152, _499156)) with
...
in (let rec aux = (fun _497485 -> (match (_497485) with
| [] -> begin
...
(let _499192 = (mlmod_of_iface x)
in None)
end else begin
Some ((mlmod_of_fstar x))
end) fmods)
in (let for1 = (fun mllib the -> (let _499201 = the
in (match (_499201) with
