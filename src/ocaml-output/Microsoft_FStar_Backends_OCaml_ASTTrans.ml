...
let rec in_ns = (fun _496987 -> (match (_496987) with
| ([], _) -> begin
true
end
...
in (let outsupport = (fun _497034 -> (match (_497034) with
| (ns1, ns2) -> begin
...
in (fun x -> (let _497091 = (Support.Microsoft.FStar.Util.incr c)
in (x, (! (c))))))
...
let lenv_of_mlenv = (fun _497099 -> lempty)

let lpush = (fun _497102 real pp -> (match (_497102) with
| LEnv (lenv) -> begin
(let mlid = (fresh pp.Microsoft_FStar_Absyn_Syntax.idText)
in (let _497106 = (Support.Microsoft.FStar.Util.smap_add lenv real.Microsoft_FStar_Absyn_Syntax.idText mlid)
in (LEnv (lenv), mlid)))
end))

let lresolve = (fun _497109 x -> (match (_497109) with
| LEnv (lenv) -> begin
...
let tvsym = (fun _497117 -> (match (_497117) with
| (x, n) -> begin
...
in (let _497150 = (let for1 = (fun used tv -> (match (tv) with
| Some ((real, pp)) -> begin
(let _497139 = (freshen used (Some (pp)))
in (match (_497139) with
| (used, pp) -> begin
(used, ((fresh pp), Some (real.Microsoft_FStar_Absyn_Syntax.idText)))
end))
end
| None -> begin
(let _497143 = (freshen used None)
in (match (_497143) with
| (used, pp) -> begin
(used, ((fresh pp), None))
end))
end))
...
in (match (_497150) with
| (_, tvs) -> begin
(let tparams = (Support.List.map (fun _497154 -> (match (_497154) with
| (x, _) -> begin
(tvsym x)
end)) tvs)
in (let tvs = (Support.List.choose (fun _497158 -> (match (_497158) with
| (x, y) -> begin
(match (y) with
| None -> begin
None
...
let tvar_of_btvar = (fun _497164 x -> (match (_497164) with
| TEnv (tenv) -> begin
...
(let args = (Support.List.collect (fun _496988 -> (match (_496988) with
...
let mlkind_of_kind = (fun tps k -> (let mltparam_of_tparam = (fun _496989 -> (match (_496989) with
...
let rec mlty_of_ty_core = (fun mlenv tenv _497352 -> (match (_497352) with
| (rg, ty) -> begin
...
and maybe_named = (fun mlenv tenv _497466 -> (match (_497466) with
| (rg, ty) -> begin
(let rg = ty.Microsoft_FStar_Absyn_Syntax.pos
in (let rec aux = (fun acc _497472 -> (match (_497472) with
| (rg, ty) -> begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _496990 -> (match (_496990) with
...
(let tys = ((Support.List.map (fun _496991 -> (match (_496991) with
...
and maybe_tuple = (fun mlenv tenv _497518 -> (match (_497518) with
| (rg, ty) -> begin
(let rg = ty.Microsoft_FStar_Absyn_Syntax.pos
...
| Microsoft_FStar_Absyn_Syntax.Typ_app ((head, args)) -> begin
if ((Support.Microsoft.FStar.Util.for_some (fun _496992 -> (match (_496992) with
| (Support.Microsoft.FStar.Util.Inr (_), _) -> begin
true
end
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
...
(let tys = ((Support.List.map (fun _496993 -> (match (_496993) with
| (Support.Microsoft.FStar.Util.Inl (t), _) -> begin
t
end
| _ -> begin
...
(let _497627 = ((Support.List.partition (fun _496994 -> (match (_496994) with
| (Support.Microsoft.FStar.Util.Inl ({Microsoft_FStar_Absyn_Syntax.v = x; Microsoft_FStar_Absyn_Syntax.sort = {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Kind_type; Microsoft_FStar_Absyn_Syntax.tk = _; Microsoft_FStar_Absyn_Syntax.pos = _; Microsoft_FStar_Absyn_Syntax.fvs = _; Microsoft_FStar_Absyn_Syntax.uvs = _}; Microsoft_FStar_Absyn_Syntax.p = _}), _) -> begin
true
end
| _ -> begin
...
in (match (_497627) with
| (ts, vs) -> begin
(let ts = ((Support.List.collect (fun _496995 -> (match (_496995) with
...
let mlscheme_of_ty = (fun mlenv rg ty -> (let _497658 = (strip_polymorphism [] rg ty)
in (match (_497658) with
| (tparams, rg, ty) -> begin
(let some = (fun x -> Some (x))
in (let _497663 = (tenv_of_tvmap (Support.List.map some tparams))
in (match (_497663) with
| (tenv, tparams) -> begin
...
(let _497686 = (Support.Microsoft.FStar.Util.fold_map (fun le pat -> (mlpat_of_pat mlenv pat.Microsoft_FStar_Absyn_Syntax.p le pat)) le ps)
in (match (_497686) with
| (le, ps) -> begin
(le, Microsoft_FStar_Backends_OCaml_Syntax.MLP_Tuple (ps))
end))
end else begin
(let _497689 = (Support.Microsoft.FStar.Util.fold_map (mlpat_of_pat mlenv rg) le ps)
in (match (_497689) with
| (le, ps) -> begin
...
(let _497702 = (lpush le x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname)
in (match (_497702) with
| (le, mlid) -> begin
...
(let _497709 = (Support.Microsoft.FStar.Util.fold_map (mlpat_of_pat mlenv rg) le ps)
in (match (_497709) with
| (le, ps) -> begin
...
in (let rec bvs = (fun _496996 -> (match (_496996) with
| 0 -> begin
[]
end
| n -> begin
(let _497740 = (Support.Microsoft.FStar.Util.incr ctr)
...
(mlexpr_of_expr mlenv rg lenv (let _497779 = e
in {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Exp_app ((a1, (a2)::[])); Microsoft_FStar_Absyn_Syntax.tk = _497779.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = _497779.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _497779.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _497779.Microsoft_FStar_Absyn_Syntax.uvs}))
end
...
(mlexpr_of_expr mlenv rg lenv (let _497797 = e
in {Microsoft_FStar_Absyn_Syntax.n = Microsoft_FStar_Absyn_Syntax.Exp_app ((a2, (a1)::[])); Microsoft_FStar_Absyn_Syntax.tk = _497797.Microsoft_FStar_Absyn_Syntax.tk; Microsoft_FStar_Absyn_Syntax.pos = _497797.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _497797.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _497797.Microsoft_FStar_Absyn_Syntax.uvs}))
end
...
| Some (k) -> begin
(let args = (Support.List.collect (fun _496997 -> (match (_496997) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
...
| _ -> begin
(let args = (Support.List.collect (fun _496998 -> (match (_496998) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
...
in (let _497875 = (match ((Support.List.rev c.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ns)) with
| [] -> begin
("", [])
end
| h::t -> begin
...
in (let rid = (let _497904 = x.Microsoft_FStar_Absyn_Syntax.v
in {Microsoft_FStar_Absyn_Syntax.ns = _497904.Microsoft_FStar_Absyn_Syntax.ns; Microsoft_FStar_Absyn_Syntax.ident = (let _497906 = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ident
in {Microsoft_FStar_Absyn_Syntax.idText = sub; Microsoft_FStar_Absyn_Syntax.idRange = _497906.Microsoft_FStar_Absyn_Syntax.idRange}); Microsoft_FStar_Absyn_Syntax.nsstr = _497904.Microsoft_FStar_Absyn_Syntax.nsstr; Microsoft_FStar_Absyn_Syntax.str = sub})
...
in (let _497919 = (Support.List.rev x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ns)
in (match (_497919) with
| cn::cr -> begin
(let crstr = (Support.List.map (fun x -> x.Microsoft_FStar_Absyn_Syntax.idText) cr)
in (let rid = {Microsoft_FStar_Absyn_Syntax.ns = cr; Microsoft_FStar_Absyn_Syntax.ident = (let _497922 = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ident
in {Microsoft_FStar_Absyn_Syntax.idText = cn.Microsoft_FStar_Absyn_Syntax.idText; Microsoft_FStar_Absyn_Syntax.idRange = _497922.Microsoft_FStar_Absyn_Syntax.idRange}); Microsoft_FStar_Absyn_Syntax.nsstr = (Support.String.concat "." crstr); Microsoft_FStar_Absyn_Syntax.str = x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.nsstr}
...
(let _497960 = (lpush lenv x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.ppname)
in (match (_497960) with
| (lenv, mlid) -> begin
...
(let _498041 = (mllets_of_lets mlenv rg lenv (rec_, lb))
in (match (_498041) with
| (lenv, bindings) -> begin
...
(let _498048 = (Support.Prims._assert ())
in (let _498070 = (match (e.Microsoft_FStar_Absyn_Syntax.n) with
...
in (match (_498070) with
| (c, args) -> begin
(let args = ((Support.List.collect (fun _496999 -> (match (_496999) with
| (Support.Microsoft.FStar.Util.Inr (e), _) -> begin
(e)::[]
end
| _ -> begin
...
and mllets_of_lets = (fun mlenv rg lenv _498132 -> (match (_498132) with
| (rec_, lbs) -> begin
(let downct = (fun _498138 -> (match (_498138) with
| (x, _, e) -> begin
(match (x) with
...
in (let _498152 = (Support.Microsoft.FStar.Util.fold_map (fun lenv _498149 -> (match (_498149) with
| (x, _) -> begin
(lpush lenv x.Microsoft_FStar_Absyn_Syntax.realname x.Microsoft_FStar_Absyn_Syntax.ppname)
end)) lenv lbs)
in (match (_498152) with
| (lenvb, mlids) -> begin
...
in (Support.List.map (fun _498156 -> (match (_498156) with
| (x, e) -> begin
...
and mlbranch_of_branch = (fun mlenv rg lenv _498165 -> (match (_498165) with
| (pat, when_, body) -> begin
(let _498168 = (mlpat_of_pat mlenv rg lenv pat)
in (match (_498168) with
| (lenv, pat) -> begin
...
let mldtype_of_indt = (fun mlenv indt -> (let rec getRecordFieldsFromType = (fun _497000 -> (match (_497000) with
| [] -> begin
None
end
...
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, c)) -> begin
(Support.List.append ((Support.List.collect (fun _497001 -> (match (_497001) with
...
in (let _498328 = (let fold1 = (fun sigelt _498259 -> (match (_498259) with
| (types, ctors) -> begin
(match (sigelt) with
...
| (Some (f), c::[]) -> begin
(let _498278 = (Support.Microsoft.FStar.Util.smap_add record_constructors c.Microsoft_FStar_Absyn_Syntax.str f)
...
in (let anames = (Support.List.map (fun _497002 -> (match (_497002) with
| None -> begin
(let _498299 = (Support.Microsoft.FStar.Util.incr actr)
in (Support.String.strcat "_" (Support.Microsoft.FStar.Util.string_of_int (! (actr)))))
end
| Some (x) -> begin
(let _498303 = (Support.Microsoft.FStar.Util.incr actr)
...
in (let _498306 = (Support.Microsoft.FStar.Util.smap_add algebraic_constructors x.Microsoft_FStar_Absyn_Syntax.str ((Support.List.length anames), anames))
...
in (let _498325 = (Support.List.fold_right fold1 indt ([], []))
in (match (_498325) with
| (ts, cs) -> begin
(ts, (Support.Microsoft.FStar.Util.smap_of_list cs))
end)))
in (match (_498328) with
| (ts, cs) -> begin
(let cons_args = (fun cname tparams rg x -> (let _498337 = ((Support.Microsoft.FStar.Util.must) (Support.Microsoft.FStar.Util.smap_try_find cs cname.Microsoft_FStar_Absyn_Syntax.ident.Microsoft_FStar_Absyn_Syntax.idText))
in (match (_498337) with
| (c, _) -> begin
(let _498341 = (strip_polymorphism [] rg c)
in (match (_498341) with
| (cparams, rgty, c) -> begin
(let _498342 = if ((Support.List.length cparams) <> (Support.List.length tparams)) then begin
(unexpected rg "invalid-number-of-ctor-params")
end
in (let cparams = (Support.List.map (fun _498347 -> (match (_498347) with
| (x, _) -> begin
...
in (let _498354 = (mltycons_of_mlty c)
in (match (_498354) with
| (args, name) -> begin
(match (name) with
...
in (let _498366 = if ((Support.List.length tyargs) <> (Support.List.length cparams)) then begin
(unexpected rg "dtype-invalid-ctor-result")
end
in (let _498368 = if (not ((Support.List.forall2 check tparams tyargs))) then begin
...
in (let _498399 = if ((Support.List.length f) <> (Support.List.length args)) then begin
...
let mlmod1_of_mod1 = (fun mode mlenv modx -> (let export_val = (fun qal -> (let export_val1 = (fun _497003 -> (match (_497003) with
...
(let _498438 = (mlscheme_of_ty mlenv rg ty)
...
(let downct = (fun _498460 -> (match (_498460) with
| (x, _, e) -> begin
(match (x) with
...
in (let lbs = (Support.List.map downct lbs)
in (let lbs = (Support.List.map (fun _498469 -> (match (_498469) with
| (x, e) -> begin
...
in (let _498504 = (tenv_of_tvmap ar)
in (match (_498504) with
| (tenv, tparams) -> begin
...
in (let _498522 = (tenv_of_tvmap ar)
in (match (_498522) with
| (_tenv, tparams) -> begin
...
| Microsoft_FStar_Absyn_Syntax.Typ_fun ((bs, c)) -> begin
(let tys = ((Support.List.collect (fun _497004 -> (match (_497004) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
[]
end
| (Support.Microsoft.FStar.Util.Inr (x), _) -> begin
...
in (let aout = (Support.List.map (fun _498621 -> (match (_498621) with
| (x, y, z) -> begin
(x, y, Some (z))
end)) aout)
in (match (mode) with
...
let mlmod_of_mod = (fun mlenv modx -> (let asright = (fun _497005 -> (match (_497005) with
| Support.Microsoft.FStar.Util.Inr (x) -> begin
x
end
| Support.Microsoft.FStar.Util.Inl (_) -> begin
...
let mlsig_of_sig = (fun mlenv modx -> (let asleft = (fun _497006 -> (match (_497006) with
| Support.Microsoft.FStar.Util.Inl (x) -> begin
x
end
| Support.Microsoft.FStar.Util.Inr (_) -> begin
...
in (let _498671 = (Support.Microsoft.FStar.Util.fprint1 "OCaml skip: %s\n" fmod_.Microsoft_FStar_Absyn_Syntax.name.Microsoft_FStar_Absyn_Syntax.ident.Microsoft_FStar_Absyn_Syntax.idText)
...
let rec mllib_add = (fun _498674 _498678 -> (match ((_498674, _498678)) with
...
in (let rec aux = (fun _497007 -> (match (_497007) with
| [] -> begin
...
(let _498714 = (mlmod_of_iface x)
in None)
end else begin
Some ((mlmod_of_fstar x))
end) fmods)
in (let for1 = (fun mllib the -> (let _498723 = the
in (match (_498723) with
