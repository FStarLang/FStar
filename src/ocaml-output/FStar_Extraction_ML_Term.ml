...
let uu____2339 = (FStar_All.pipe_right (FStar_List.append tyMLPats restMLPats) (FStar_List.collect (fun uu___141_2364 -> (match (uu___141_2364) with
| Some (x) -> begin
(x)::[]
end
| uu____2386 -> begin
...
let uu____2669 = (FStar_All.pipe_right ps1 (FStar_List.partition (fun uu___142_2697 -> (match (uu___142_2697) with
...
let is_total = (fun uu___144_3658 -> (match (uu___144_3658) with
| FStar_Util.Inl (l) -> begin
(FStar_Syntax_Util.is_total_lcomp l)
end
| FStar_Util.Inr (l, flags) -> begin
((FStar_Ident.lid_equals l FStar_Syntax_Const.effect_Tot_lid) || (FStar_All.pipe_right flags (FStar_List.existsb (fun uu___143_3676 -> (match (uu___143_3676) with
...
| (((arg, uu____3963))::rest, FStar_Extraction_ML_Syntax.MLTY_Fun (formal_t, f', t2)) when ((is_type g arg) && (type_leq g formal_t FStar_Extraction_ML_Syntax.ml_unit_ty)) -> begin
(

let uu____3981 = (

let uu____3984 = (FStar_Extraction_ML_Util.join arg.FStar_Syntax_Syntax.pos f f')
in ((uu____3984), (t2)))
in (extract_app is_data ((mlhead), ((((FStar_Extraction_ML_Syntax.ml_unit), (FStar_Extraction_ML_Syntax.E_PURE)))::mlargs_f)) uu____3981 rest))
end
| (((e0, uu____3991))::rest, FStar_Extraction_ML_Syntax.MLTY_Fun (tExpected, f', t2)) -> begin
(

let r = e0.FStar_Syntax_Syntax.pos
in (

let uu____4010 = (term_as_mlexpr g e0)
...
let uu____4024 = (FStar_Extraction_ML_Util.join_l r ((f)::(f')::(f0)::[]))
in ((uu____4024), (t2)))
in (extract_app is_data ((mlhead), ((((e02), (f0)))::mlargs_f)) uu____4021 rest)))
end)))
end
| uu____4030 -> begin
(

let uu____4037 = (FStar_Extraction_ML_Util.udelta_unfold g t1)
...
let extract_app_maybe_projector = (fun is_data mlhead uu____4073 args1 -> (match (uu____4073) with
| (f, t1) -> begin
(match (is_data) with
| Some (FStar_Syntax_Syntax.Record_projector (uu____4092)) -> begin
(

let rec remove_implicits = (fun args2 f1 t2 -> (match (((args2), (t2))) with
| (((a0, Some (FStar_Syntax_Syntax.Implicit (uu____4142))))::args3, FStar_Extraction_ML_Syntax.MLTY_Fun (uu____4144, f', t3)) -> begin
(

let uu____4169 = (FStar_Extraction_ML_Util.join a0.FStar_Syntax_Syntax.pos f1 f')
in (remove_implicits args3 uu____4169 t3))
end
| uu____4170 -> begin
((args2), (f1), (t2))
end))
...
let uu____4185 = (remove_implicits args1 f t1)
...
let uu____4243 = (FStar_Extraction_ML_UEnv.lookup_term g head2)
in (match (uu____4243) with
| (FStar_Util.Inr (uu____4253, x1, x2, x3), q) -> begin
((((x1), (x2), (x3))), (q))
end
| uu____4278 -> begin
(failwith "FIXME Ty")
end))
in (match (uu____4236) with
| ((head_ml, (vars, t1), inst_ok), qual) -> begin
(

let has_typ_apps = (match (args) with
| ((a, uu____4307))::uu____4308 -> begin
(is_type g a)
end
| uu____4322 -> begin
false
...
let uu____4328 = (match (vars) with
| (uu____4345)::uu____4346 when ((not (has_typ_apps)) && inst_ok) -> begin
((head_ml), (t1), (args))
end
| uu____4353 -> begin
(
...
let uu____4373 = (FStar_Util.first_N n1 args)
in (match (uu____4373) with
| (prefix1, rest) -> begin
(

let prefixAsMLTypes = (FStar_List.map (fun uu____4426 -> (match (uu____4426) with
| (x, uu____4430) -> begin
(term_as_mlty g x)
end)) prefix1)
in (
...
let uu___148_4435 = head_ml
in {FStar_Extraction_ML_Syntax.expr = uu___148_4435.FStar_Extraction_ML_Syntax.expr; FStar_Extraction_ML_Syntax.mlty = t2; FStar_Extraction_ML_Syntax.loc = uu___148_4435.FStar_Extraction_ML_Syntax.loc})
end
| FStar_Extraction_ML_Syntax.MLE_App (head3, ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Unit); FStar_Extraction_ML_Syntax.mlty = uu____4437; FStar_Extraction_ML_Syntax.loc = uu____4438})::[]) -> begin
(FStar_All.pipe_right (FStar_Extraction_ML_Syntax.MLE_App ((((

let uu___149_4441 = head3
in {FStar_Extraction_ML_Syntax.expr = uu___149_4441.FStar_Extraction_ML_Syntax.expr; FStar_Extraction_ML_Syntax.mlty = FStar_Extraction_ML_Syntax.MLTY_Fun (((FStar_Extraction_ML_Syntax.ml_unit_ty), (FStar_Extraction_ML_Syntax.E_PURE), (t2))); FStar_Extraction_ML_Syntax.loc = uu___149_4441.FStar_Extraction_ML_Syntax.loc})), ((FStar_Extraction_ML_Syntax.ml_unit)::[])))) (FStar_Extraction_ML_Syntax.with_ty t2))
end
| uu____4442 -> begin
...
let uu____4480 = (maybe_eta_data_and_project_record g qual head_t head_ml1)
in ((uu____4480), (FStar_Extraction_ML_Syntax.E_PURE), (head_t)))
end
| uu____4481 -> begin
...
let uu____4488 = (term_as_mlexpr g head2)
...
| FStar_Syntax_Syntax.Tm_ascribed (e0, (tc, uu____4500), f) -> begin
(
...
let uu____4554 = (check_term_as_mlexpr g e0 f1 t1)
...
let uu____4575 = (match (is_rec) with
| true -> begin
...
let uu____4583 = (FStar_Syntax_Syntax.is_top_level lbs)
...
let uu____4593 = (FStar_Util.left lb.FStar_Syntax_Syntax.lbname)
in (FStar_Syntax_Syntax.freshen_bv uu____4593))
in (

let lb1 = (

let uu___150_4595 = lb
in {FStar_Syntax_Syntax.lbname = FStar_Util.Inl (x); FStar_Syntax_Syntax.lbunivs = uu___150_4595.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = uu___150_4595.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = uu___150_4595.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = uu___150_4595.FStar_Syntax_Syntax.lbdef})
in (
...
let uu____4612 = (FStar_Ident.lid_of_path (FStar_List.append (Prims.fst g.FStar_Extraction_ML_UEnv.currentModule) (((Prims.snd g.FStar_Extraction_ML_UEnv.currentModule))::[])) FStar_Range.dummyRange)
in (FStar_TypeChecker_Env.set_current_module g.FStar_Extraction_ML_UEnv.tcenv uu____4612))
in ((FStar_Extraction_ML_UEnv.debug g (fun uu____4616 -> (FStar_Options.set_option "debug_level" (FStar_Options.List ((FStar_Options.String ("Norm"))::(FStar_Options.String ("Extraction"))::[])))));
(

let lbdef = (

let uu____4620 = (FStar_Options.ml_ish ())
...
let uu___151_4624 = lb
in {FStar_Syntax_Syntax.lbname = uu___151_4624.FStar_Syntax_Syntax.lbname; FStar_Syntax_Syntax.lbunivs = uu___151_4624.FStar_Syntax_Syntax.lbunivs; FStar_Syntax_Syntax.lbtyp = uu___151_4624.FStar_Syntax_Syntax.lbtyp; FStar_Syntax_Syntax.lbeff = uu___151_4624.FStar_Syntax_Syntax.lbeff; FStar_Syntax_Syntax.lbdef = lbdef}));
)))))
end
| uu____4625 -> begin
lbs1
...
let maybe_generalize = (fun uu____4638 -> (match (uu____4638) with
| {FStar_Syntax_Syntax.lbname = lbname_; FStar_Syntax_Syntax.lbunivs = uu____4649; FStar_Syntax_Syntax.lbtyp = t1; FStar_Syntax_Syntax.lbeff = lbeff; FStar_Syntax_Syntax.lbdef = e} -> begin
(
...
let uu____4692 = (FStar_List.hd bs)
in (FStar_All.pipe_right uu____4692 (is_type_binder g))) -> begin
(

let uu____4699 = (FStar_Syntax_Subst.open_comp bs c)
...
let uu____4718 = (FStar_Util.prefix_until (fun x -> (

let uu____4736 = (is_type_binder g x)
in (not (uu____4736)))) bs1)
...
let uu____4784 = (FStar_Syntax_Util.arrow ((b)::rest) c1)
in ((bs2), (uu____4784)))
end))
...
let uu____4814 = (normalize_abs e)
in (FStar_All.pipe_right uu____4814 FStar_Syntax_Util.unmeta))
in (match (e1.FStar_Syntax_Syntax.n) with
| FStar_Syntax_Syntax.Tm_abs (bs2, body, uu____4826) -> begin
(

let uu____4849 = (FStar_Syntax_Subst.open_term bs2 body)
...
let uu____4879 = (FStar_Util.first_N n_tbinders bs3)
...
let s = (FStar_List.map2 (fun uu____4922 uu____4923 -> (match (((uu____4922), (uu____4923))) with
| ((x, uu____4933), (y, uu____4935)) -> begin
(

let uu____4940 = (

let uu____4945 = (FStar_Syntax_Syntax.bv_to_name y)
in ((x), (uu____4945)))
in FStar_Syntax_Syntax.NT (uu____4940))
end)) tbinders targs)
in (FStar_Syntax_Subst.subst s tbody))
in (

let env = (FStar_List.fold_left (fun env uu____4950 -> (match (uu____4950) with
| (a, uu____4954) -> begin
...
let uu____4962 = (FStar_All.pipe_right targs (FStar_List.map (fun uu____4976 -> (match (uu____4976) with
| (x, uu____4982) -> begin
(FStar_Extraction_ML_UEnv.bv_as_ml_tyvar x)
end))))
in ((uu____4962), (expected_t)))
in (
...
let uu____4989 = (is_fstar_value body1)
in (not (uu____4989)))
end
| uu____4990 -> begin
false
...
let env = (FStar_List.fold_left (fun env uu____5055 -> (match (uu____5055) with
| (a, uu____5059) -> begin
(FStar_Extraction_ML_UEnv.extend_ty env a None)
end)) g tbinders)
in (
...
let uu____5067 = (FStar_All.pipe_right tbinders (FStar_List.map (fun uu____5078 -> (match (uu____5078) with
| (x, uu____5084) -> begin
(FStar_Extraction_ML_UEnv.bv_as_ml_tyvar x)
end))))
in ((uu____5067), (expected_t)))
in (

let args = (FStar_All.pipe_right tbinders (FStar_List.map (fun uu____5093 -> (match (uu____5093) with
| (bv, uu____5097) -> begin
(

let uu____5098 = (FStar_Syntax_Syntax.bv_to_name bv)
in (FStar_All.pipe_right uu____5098 FStar_Syntax_Syntax.as_arg))
end))))
in (
...
let check_lb = (fun env uu____5203 -> (match (uu____5203) with
| (nm, (lbname, f, (t1, (targs, polytype)), add_unit, e)) -> begin
(

let env1 = (FStar_List.fold_left (fun env1 uu____5274 -> (match (uu____5274) with
| (a, uu____5278) -> begin
...
let uu____5281 = (check_term_as_mlexpr env1 e f expected_t)
...
let uu____5322 = (FStar_List.fold_right (fun lb uu____5361 -> (match (uu____5361) with
| (env, lbs4) -> begin
(

let uu____5425 = lb
in (match (uu____5425) with
| (lbname, uu____5450, (t1, (uu____5452, polytype)), add_unit, uu____5455) -> begin
(

let uu____5462 = (FStar_Extraction_ML_UEnv.extend_lb env lbname t1 polytype add_unit true)
...
let uu____5605 = (term_as_mlexpr env_body e'1)
...
let uu____5618 = (FStar_List.map Prims.fst lbs5)
in (f')::uu____5618)
in (FStar_Extraction_ML_Util.join_l e'_rng uu____5616))
in (
...
let uu____5627 = (FStar_List.map Prims.snd lbs5)
in ((is_rec1), ([]), (uu____5627)))
in (mk_MLE_Let top_level uu____5626 e'2))
in (

let uu____5633 = (FStar_Extraction_ML_Util.mlloc_of_range t.FStar_Syntax_Syntax.pos)
in (FStar_Extraction_ML_Syntax.with_ty_loc t' uu____5625 uu____5633)))
in ((uu____5624), (f), (t')))))
end)))))
end))))))
end)))
...
let uu____5662 = (term_as_mlexpr g scrutinee)
in (match (uu____5662) with
| (e, f_e, t_e) -> begin
(

let uu____5672 = (check_pats_for_ite pats)
...
let uu____5707 = (term_as_mlexpr g then_e1)
in (match (uu____5707) with
| (then_mle, f_then, t_then) -> begin
(

let uu____5717 = (term_as_mlexpr g else_e1)
...
let uu____5734 = (type_leq g t_then t_else)
...
let uu____5746 = (type_leq g t_else t_then)
...
let uu____5782 = (maybe_lift then_mle t_then)
in (

let uu____5783 = (

let uu____5785 = (maybe_lift else_mle t_else)
in Some (uu____5785))
in ((e), (uu____5782), (uu____5783))))
in FStar_Extraction_ML_Syntax.MLE_If (uu____5777))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty t_branch) uu____5776))
in (

let uu____5787 = (FStar_Extraction_ML_Util.join then_e1.FStar_Syntax_Syntax.pos f_then f_else)
in ((uu____5775), (uu____5787), (t_branch))))
end))
end))
end))
end
...
let uu____5797 = (FStar_All.pipe_right pats (FStar_Util.fold_map (fun compat br -> (

let uu____5847 = (FStar_Syntax_Subst.open_branch br)
in (match (uu____5847) with
| (pat, when_opt, branch1) -> begin
(

let uu____5877 = (extract_pat g pat t_e)
in (match (uu____5877) with
| (env, p, pat_t_compat) -> begin
(

let uu____5908 = (match (when_opt) with
| None -> begin
...
let uu____5923 = (term_as_mlexpr env w)
...
let uu____5951 = (term_as_mlexpr env branch1)
in (match (uu____5951) with
| (mlbranch, f_branch, t_branch) -> begin
(

let uu____5970 = (FStar_All.pipe_right p (FStar_List.map (fun uu____6007 -> (match (uu____6007) with
| (p1, wopt) -> begin
(
...
in (((compat && pat_t_compat)), (uu____5970)))
end))
end))
end))
end))) true))
...
| uu____6091 -> begin
((FStar_Extraction_ML_UEnv.debug g (fun uu____6093 -> (

let uu____6094 = (FStar_Extraction_ML_Code.string_of_mlexpr g.FStar_Extraction_ML_UEnv.currentModule e)
in (

let uu____6095 = (FStar_Extraction_ML_Code.string_of_mlty g.FStar_Extraction_ML_UEnv.currentModule t_e)
...
let uu____6122 = (FStar_Syntax_Syntax.lid_as_fv FStar_Syntax_Const.failwith_lid FStar_Syntax_Syntax.Delta_constant None)
in (FStar_Extraction_ML_UEnv.lookup_fv g uu____6122))
in (FStar_All.pipe_left FStar_Util.right uu____6113))
in (match (uu____6108) with
| (uu____6144, fw, uu____6146, uu____6147) -> begin
(
...
let uu____6156 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_string_ty) (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_String ("unreachable"))))
in (uu____6156)::[])
in ((fw), (uu____6154)))
in FStar_Extraction_ML_Syntax.MLE_App (uu____6150))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_unit_ty) uu____6149))
in ((uu____6148), (FStar_Extraction_ML_Syntax.E_PURE), (FStar_Extraction_ML_Syntax.ml_unit_ty)))
end))
end
| ((uu____6158, uu____6159, (uu____6160, f_first, t_first)))::rest -> begin
(

let uu____6192 = (FStar_List.fold_left (fun uu____6208 uu____6209 -> (match (((uu____6208), (uu____6209))) with
| ((topt, f), (uu____6239, uu____6240, (uu____6241, f_branch, t_branch))) -> begin
(
...
let uu____6272 = (type_leq g t1 t_branch)
...
let uu____6275 = (type_leq g t_branch t1)
...
let mlbranches2 = (FStar_All.pipe_right mlbranches1 (FStar_List.map (fun uu____6321 -> (match (uu____6321) with
| (p, (wopt, uu____6337), (b1, uu____6339, t1)) -> begin
(
...
let uu____6354 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty t_match) (FStar_Extraction_ML_Syntax.MLE_Match (((e1), (mlbranches2)))))
in ((uu____6354), (f_match), (t_match)))))
end))
end)))
end))
end))
...
let uu____6372 = (FStar_ST.read c)
in ((x), (uu____6372)));
)))
...
let uu____6387 = (FStar_TypeChecker_Env.lookup_lid env.FStar_Extraction_ML_UEnv.tcenv discName)
in (FStar_All.pipe_left Prims.fst uu____6387))
in (match (uu____6384) with
| (uu____6400, fstar_disc_type) -> begin
(
...
let uu____6409 = (FStar_Syntax_Subst.compress fstar_disc_type)
in uu____6409.FStar_Syntax_Syntax.n)
in (match (uu____6408) with
| FStar_Syntax_Syntax.Tm_arrow (binders, uu____6418) -> begin
(

let uu____6429 = (FStar_All.pipe_right binders (FStar_List.filter (fun uu___145_6444 -> (match (uu___145_6444) with
| (uu____6448, Some (FStar_Syntax_Syntax.Implicit (uu____6449))) -> begin
true
end
| uu____6451 -> begin
false
end))))
in (FStar_All.pipe_right uu____6429 (FStar_List.map (fun uu____6471 -> (

let uu____6475 = (fresh "_")
in ((uu____6475), (FStar_Extraction_ML_Syntax.MLTY_Top)))))))
end
| uu____6480 -> begin
...
let uu____6512 = (FStar_Extraction_ML_Syntax.idsym mlid)
in (([]), (uu____6512)))
in FStar_Extraction_ML_Syntax.MLE_Name (uu____6511))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty targ) uu____6510))
in (
...
let uu____6530 = (FStar_Extraction_ML_Syntax.mlpath_of_lident constrName)
in ((uu____6530), ((FStar_Extraction_ML_Syntax.MLP_Wild)::[])))
in FStar_Extraction_ML_Syntax.MLP_CTor (uu____6526))
in (

let uu____6532 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Bool (true))))
in ((uu____6525), (None), (uu____6532))))
in (
...
let uu____6545 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Bool (false))))
in ((FStar_Extraction_ML_Syntax.MLP_Wild), (None), (uu____6545)))
in (uu____6540)::[])
in (uu____6520)::uu____6534))
in ((uu____6509), (uu____6514))))
in FStar_Extraction_ML_Syntax.MLE_Match (uu____6501))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) uu____6500))
in (((FStar_List.append wildcards ((((mlid), (targ)))::[]))), (uu____6499)))
in FStar_Extraction_ML_Syntax.MLE_Fun (uu____6493))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty disc_ty) uu____6492))
in (
...
let uu____6587 = (FStar_Extraction_ML_UEnv.convIdent discName.FStar_Ident.ident)
in {FStar_Extraction_ML_Syntax.mllb_name = uu____6587; FStar_Extraction_ML_Syntax.mllb_tysc = None; FStar_Extraction_ML_Syntax.mllb_add_unit = false; FStar_Extraction_ML_Syntax.mllb_def = discrBody; FStar_Extraction_ML_Syntax.print_typ = false})
in (uu____6586)::[])
in ((FStar_Extraction_ML_Syntax.NonRec), ([]), (uu____6584)))
in FStar_Extraction_ML_Syntax.MLM_Let (uu____6583)))))))
end)))
