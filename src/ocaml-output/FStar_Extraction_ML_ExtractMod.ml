...
(let _64_75 = (match ((FStar_All.pipe_right quals (FStar_Util.for_some (fun _64_1 -> (match (_64_1) with
...
in (let env = (let _130_34 = (let _130_32 = (FStar_Util.right lbname)
in (FStar_All.pipe_left FStar_Absyn_Util.fv _130_32))
in (let _130_33 = (FStar_Util.must ml_lb.FStar_Extraction_ML_Syntax.mllb_tysc)
in (FStar_Extraction_ML_Env.extend_fv' env _130_34 mname _130_33 ml_lb.FStar_Extraction_ML_Syntax.mllb_add_unit)))
...
| false -> begin
(let _130_37 = (let _130_36 = (let _130_35 = (FStar_Util.must ml_lb.FStar_Extraction_ML_Syntax.mllb_tysc)
in (FStar_Extraction_ML_Env.extend_lb env lbname t _130_35 ml_lb.FStar_Extraction_ML_Syntax.mllb_add_unit))
in (FStar_All.pipe_left Prims.fst _130_36))
in (_130_37, ml_lb))
end)
...
| Some (bs, c) -> begin
(let _130_39 = (let _130_38 = (fail_exp lid (FStar_Absyn_Util.comp_result c))
in (bs, _130_38))
in (FStar_Absyn_Syntax.mk_Exp_abs _130_39 None FStar_Absyn_Syntax.dummyRange))
end
| _64_92 -> begin
(fail_exp lid t)
end)
...
(let _130_43 = (let _130_42 = (FStar_Extraction_ML_ExtractExp.ind_discriminator_body g lid l)
in (_130_42)::[])
in (g, _130_43))
end
| _64_113 -> begin
...
let extract_iface = (fun g m -> (let _130_49 = (FStar_Util.fold_map extract_sig g m.FStar_Absyn_Syntax.declarations)
in (FStar_All.pipe_right _130_49 Prims.fst)))
...
in (let _64_163 = (FStar_Util.print_string (Prims.strcat (Prims.strcat "extracting: " m.FStar_Absyn_Syntax.name.FStar_Absyn_Syntax.str) "\n"))
in (let g = (let _64_165 = g
in {FStar_Extraction_ML_Env.tcenv = _64_165.FStar_Extraction_ML_Env.tcenv; FStar_Extraction_ML_Env.gamma = _64_165.FStar_Extraction_ML_Env.gamma; FStar_Extraction_ML_Env.tydefs = _64_165.FStar_Extraction_ML_Env.tydefs; FStar_Extraction_ML_Env.currentModule = name})
in (match (((m.FStar_Absyn_Syntax.name.FStar_Absyn_Syntax.str = "Prims") || m.FStar_Absyn_Syntax.is_interface)) with
| true -> begin
(let g = (extract_iface g m)
in (g, []))
end
| false -> begin
(let _64_171 = (FStar_Util.fold_map extract_sig g m.FStar_Absyn_Syntax.declarations)
in (match (_64_171) with
| (g, sigs) -> begin
(let mlm = (FStar_List.flatten sigs)
in (let _130_58 = (let _130_57 = (let _130_56 = (let _130_55 = (let _130_54 = (FStar_Extraction_ML_Util.flatten_mlpath name)
in (_130_54, Some (([], mlm)), FStar_Extraction_ML_Syntax.MLLib ([])))
in (_130_55)::[])
in FStar_Extraction_ML_Syntax.MLLib (_130_56))
in (_130_57)::[])
in (g, _130_58)))
end))
end))))))
