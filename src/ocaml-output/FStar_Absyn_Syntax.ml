...
let mk_ident : (Prims.string * FStar_Range.range)  ->  FStar_Ident.ident = (fun _26_306 -> (match (_26_306) with
| (text, range) -> begin
...
let lid_of_ids : ident Prims.list  ->  FStar_Ident.lident = (fun ids -> (let _26_317 = (FStar_Util.prefix ids)
in (match (_26_317) with
| (ns, id) -> begin
(let nsstr = (let _129_1257 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right _129_1257 text_of_path))
...
let lid_with_range : FStar_Ident.lid  ->  FStar_Range.range  ->  FStar_Ident.lident = (fun lid r -> (let id = (let _26_357 = lid.FStar_Ident.ident
...
let no_fvs : freevars = (let _129_1322 = (new_ftv_set ())
in (let _129_1321 = (new_ftv_set ())
in {ftvs = _129_1322; fxvs = _129_1321}))

let no_uvs : uvars = (let _129_1325 = (new_uv_set ())
in (let _129_1324 = (new_uvt_set ())
in (let _129_1323 = (new_uvt_set ())
in {uvars_k = _129_1325; uvars_t = _129_1324; uvars_e = _129_1323})))
...
let freevars_of_list : (btvar, bvvar) FStar_Util.either Prims.list  ->  freevars = (fun l -> (FStar_All.pipe_right l (FStar_List.fold_left (fun out _26_1 -> (match (_26_1) with
| FStar_Util.Inl (btv) -> begin
(let _26_398 = out
in (let _129_1330 = (FStar_Util.set_add btv out.ftvs)
in {ftvs = _129_1330; fxvs = _26_398.fxvs}))
end
| FStar_Util.Inr (bxv) -> begin
(let _26_402 = out
in (let _129_1331 = (FStar_Util.set_add bxv out.fxvs)
in {ftvs = _26_402.ftvs; fxvs = _129_1331}))
end)) no_fvs)))

let list_of_freevars : freevars  ->  (btvar, bvvar) FStar_Util.either Prims.list = (fun fvs -> (let _129_1339 = (let _129_1335 = (FStar_Util.set_elements fvs.ftvs)
in (FStar_All.pipe_right _129_1335 (FStar_List.map (fun x -> FStar_Util.Inl (x)))))
in (let _129_1338 = (let _129_1337 = (FStar_Util.set_elements fvs.fxvs)
in (FStar_All.pipe_right _129_1337 (FStar_List.map (fun x -> FStar_Util.Inr (x)))))
in (FStar_List.append _129_1339 _129_1338))))

let get_unit_ref : Prims.unit  ->  Prims.unit Prims.option FStar_ST.ref = (fun _26_407 -> (match (()) with
| () -> begin
...
let mk_Kind_type : (knd', Prims.unit) syntax = (let _129_1344 = (get_unit_ref ())
in (let _129_1343 = (mk_fvs ())
in (let _129_1342 = (mk_uvs ())
in {n = Kind_type; tk = _129_1344; pos = dummyRange; fvs = _129_1343; uvs = _129_1342})))

let mk_Kind_effect : (knd', Prims.unit) syntax = (let _129_1347 = (get_unit_ref ())
in (let _129_1346 = (mk_fvs ())
in (let _129_1345 = (mk_uvs ())
in {n = Kind_effect; tk = _129_1347; pos = dummyRange; fvs = _129_1346; uvs = _129_1345})))

let mk_Kind_abbrev : (kabbrev * knd)  ->  FStar_Range.range  ->  knd = (fun _26_413 p -> (match (_26_413) with
| (kabr, k) -> begin
(let _129_1354 = (get_unit_ref ())
in (let _129_1353 = (mk_fvs ())
in (let _129_1352 = (mk_uvs ())
in {n = Kind_abbrev ((kabr, k)); tk = _129_1354; pos = p; fvs = _129_1353; uvs = _129_1352})))
end))

let mk_Kind_arrow : (binders * knd)  ->  FStar_Range.range  ->  knd = (fun _26_417 p -> (match (_26_417) with
| (bs, k) -> begin
(let _129_1361 = (get_unit_ref ())
in (let _129_1360 = (mk_fvs ())
in (let _129_1359 = (mk_uvs ())
in {n = Kind_arrow ((bs, k)); tk = _129_1361; pos = p; fvs = _129_1360; uvs = _129_1359})))
end))

let mk_Kind_arrow' : (((((typ', (knd', Prims.unit) syntax) syntax bvdef, (knd', Prims.unit) syntax) withinfo_t, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list * knd)  ->  FStar_Range.range  ->  knd = (fun _26_421 p -> (match (_26_421) with
| (bs, k) -> begin
(match (bs) with
| [] -> begin
k
...
let mk_Kind_uvar : uvar_k_app  ->  FStar_Range.range  ->  (knd', Prims.unit) syntax = (fun uv p -> (let _129_1372 = (get_unit_ref ())
in (let _129_1371 = (mk_fvs ())
in (let _129_1370 = (mk_uvs ())
in {n = Kind_uvar (uv); tk = _129_1372; pos = p; fvs = _129_1371; uvs = _129_1370}))))

let mk_Kind_lam : (binders * knd)  ->  FStar_Range.range  ->  knd = (fun _26_436 p -> (match (_26_436) with
| (vs, k) -> begin
(let _129_1379 = (get_unit_ref ())
in (let _129_1378 = (mk_fvs ())
in (let _129_1377 = (mk_uvs ())
in {n = Kind_lam ((vs, k)); tk = _129_1379; pos = p; fvs = _129_1378; uvs = _129_1377})))
end))

let mk_Kind_delayed : (knd * subst_t * knd Prims.option FStar_ST.ref)  ->  FStar_Range.range  ->  knd = (fun _26_441 p -> (match (_26_441) with
| (k, s, m) -> begin
(let _129_1386 = (get_unit_ref ())
in (let _129_1385 = (mk_fvs ())
in (let _129_1384 = (mk_uvs ())
in {n = Kind_delayed ((k, s, m)); tk = _129_1386; pos = p; fvs = _129_1385; uvs = _129_1384})))
end))

let mk_Kind_unknown : (knd', Prims.unit) syntax = (let _129_1389 = (get_unit_ref ())
in (let _129_1388 = (mk_fvs ())
in (let _129_1387 = (mk_uvs ())
in {n = Kind_unknown; tk = _129_1389; pos = dummyRange; fvs = _129_1388; uvs = _129_1387})))

let get_knd_nref : Prims.unit  ->  (knd', Prims.unit) syntax Prims.option FStar_ST.ref = (fun _26_443 -> (match (()) with
| () -> begin
...
let mk_Typ_btvar : btvar  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun x k p -> (let _129_1402 = (get_knd_ref k)
in (let _129_1401 = (mk_fvs ())
in (let _129_1400 = (mk_uvs ())
in {n = Typ_btvar (x); tk = _129_1402; pos = p; fvs = _129_1401; uvs = _129_1400}))))

let mk_Typ_const : ftvar  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun x k p -> (let _129_1409 = (get_knd_ref k)
in {n = Typ_const (x); tk = _129_1409; pos = p; fvs = memo_no_fvs; uvs = memo_no_uvs}))
...
let mk_Typ_fun : (binders * comp)  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_465 k p -> (match (_26_465) with
| (bs, c) -> begin
(let _129_1422 = (check_fun bs c p)
in (let _129_1421 = (FStar_Util.mk_ref k)
in (let _129_1420 = (mk_fvs ())
in (let _129_1419 = (mk_uvs ())
in {n = _129_1422; tk = _129_1421; pos = p; fvs = _129_1420; uvs = _129_1419}))))
end))

let mk_Typ_refine : (bvvar * typ)  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_470 k p -> (match (_26_470) with
| (x, phi) -> begin
(let _129_1431 = (FStar_Util.mk_ref k)
in (let _129_1430 = (mk_fvs ())
in (let _129_1429 = (mk_uvs ())
in {n = Typ_refine ((x, phi)); tk = _129_1431; pos = p; fvs = _129_1430; uvs = _129_1429})))
end))

let mk_Typ_app : (typ * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option) Prims.list)  ->  knd Prims.option  ->  FStar_Range.range  ->  typ = (fun _26_475 k p -> (match (_26_475) with
| (t1, args) -> begin
(match (args) with
| [] -> begin
t1
...
| _26_480 -> begin
(let _129_1440 = (FStar_Util.mk_ref k)
in (let _129_1439 = (mk_fvs ())
in (let _129_1438 = (mk_uvs ())
in {n = Typ_app ((t1, args)); tk = _129_1440; pos = p; fvs = _129_1439; uvs = _129_1438})))
end)
end))

let mk_Typ_app' : (typ * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option) Prims.list)  ->  knd Prims.option  ->  FStar_Range.range  ->  typ = (fun _26_483 k p -> (match (_26_483) with
| (t1, args) -> begin
(match (args) with
| [] -> begin
t1
...
let extend_typ_app : ((typ', (knd', Prims.unit) syntax) syntax * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option))  ->  knd Prims.option  ->  FStar_Range.range  ->  typ = (fun _26_491 k p -> (match (_26_491) with
| (t, arg) -> begin
(match (t.n) with
...
let mk_Typ_lam : (((((typ', (knd', Prims.unit) syntax) syntax bvdef, (knd', Prims.unit) syntax) withinfo_t, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list * typ)  ->  knd Prims.option  ->  FStar_Range.range  ->  typ = (fun _26_502 k p -> (match (_26_502) with
| (b, t) -> begin
(match (b) with
| [] -> begin
t
...
| _26_507 -> begin
(let _129_1461 = (FStar_Util.mk_ref k)
in (let _129_1460 = (mk_fvs ())
in (let _129_1459 = (mk_uvs ())
in {n = Typ_lam ((b, t)); tk = _129_1461; pos = p; fvs = _129_1460; uvs = _129_1459})))
end)
end))

let mk_Typ_lam' : (((((typ', (knd', Prims.unit) syntax) syntax bvdef, (knd', Prims.unit) syntax) withinfo_t, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list * typ)  ->  knd Prims.option  ->  FStar_Range.range  ->  typ = (fun _26_510 k p -> (match (_26_510) with
| (bs, t) -> begin
(mk_Typ_lam (bs, t) k p)
end))

let mk_Typ_ascribed' : (typ * knd)  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_515 k' p -> (match (_26_515) with
| (t, k) -> begin
(let _129_1476 = (FStar_Util.mk_ref k')
in (let _129_1475 = (mk_fvs ())
in (let _129_1474 = (mk_uvs ())
in {n = Typ_ascribed ((t, k)); tk = _129_1476; pos = p; fvs = _129_1475; uvs = _129_1474})))
end))

let mk_Typ_ascribed : (typ * knd)  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_520 p -> (match (_26_520) with
| (t, k) -> begin
(mk_Typ_ascribed' (t, k) (Some (k)) p)
end))

let mk_Typ_meta' : meta_t  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun m k p -> (let _129_1489 = (FStar_Util.mk_ref k)
in (let _129_1488 = (mk_fvs ())
in (let _129_1487 = (mk_uvs ())
in {n = Typ_meta (m); tk = _129_1489; pos = p; fvs = _129_1488; uvs = _129_1487}))))
...
(let _129_1492 = (FStar_ST.read t.tk)
in (mk_Typ_meta' m _129_1492 t.pos))
end))

let mk_Typ_uvar' : (uvar_t * knd)  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_557 k' p -> (match (_26_557) with
| (u, k) -> begin
(let _129_1501 = (get_knd_ref k')
in (let _129_1500 = (mk_fvs ())
in (let _129_1499 = (mk_uvs ())
in {n = Typ_uvar ((u, k)); tk = _129_1501; pos = p; fvs = _129_1500; uvs = _129_1499})))
end))

let mk_Typ_uvar : (uvar_t * knd)  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_562 p -> (match (_26_562) with
| (u, k) -> begin
(mk_Typ_uvar' (u, k) (Some (k)) p)
end))

let mk_Typ_delayed : ((typ', (knd', Prims.unit) syntax) syntax * subst_t * typ Prims.option FStar_ST.ref)  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun _26_567 k p -> (match (_26_567) with
| (t, s, m) -> begin
(let _129_1521 = (match (t.n) with
...
in (let _129_1520 = (FStar_Util.mk_ref k)
in (let _129_1519 = (mk_fvs ())
in (let _129_1518 = (mk_uvs ())
in {n = _129_1521; tk = _129_1520; pos = p; fvs = _129_1519; uvs = _129_1518}))))
end))

let mk_Typ_delayed' : ((typ * subst_t), Prims.unit  ->  typ) FStar_Util.either  ->  knd Prims.option  ->  FStar_Range.range  ->  (typ', (knd', Prims.unit) syntax) syntax = (fun st k p -> (let _129_1543 = (let _129_1539 = (let _129_1538 = (FStar_Util.mk_ref None)
in (st, _129_1538))
in Typ_delayed (_129_1539))
in (let _129_1542 = (FStar_Util.mk_ref k)
in (let _129_1541 = (mk_fvs ())
in (let _129_1540 = (mk_uvs ())
in {n = _129_1543; tk = _129_1542; pos = p; fvs = _129_1541; uvs = _129_1540})))))

let mk_Typ_unknown : (typ', (knd', Prims.unit) syntax) syntax = (let _129_1546 = (get_knd_nref ())
in (let _129_1545 = (mk_fvs ())
in (let _129_1544 = (mk_uvs ())
in {n = Typ_unknown; tk = _129_1546; pos = dummyRange; fvs = _129_1545; uvs = _129_1544})))

let get_typ_nref : Prims.unit  ->  (typ', (knd', Prims.unit) syntax) syntax Prims.option FStar_ST.ref = (fun _26_578 -> (match (()) with
| () -> begin
...
let mk_Total : typ  ->  (comp', Prims.unit) syntax = (fun t -> (let _129_1555 = (FStar_Util.mk_ref None)
in (let _129_1554 = (mk_fvs ())
in (let _129_1553 = (mk_uvs ())
in {n = Total (t); tk = _129_1555; pos = t.pos; fvs = _129_1554; uvs = _129_1553}))))

let mk_Comp : comp_typ  ->  (comp', Prims.unit) syntax = (fun ct -> (let _129_1560 = (FStar_Util.mk_ref None)
in (let _129_1559 = (mk_fvs ())
in (let _129_1558 = (mk_uvs ())
in {n = Comp (ct); tk = _129_1560; pos = ct.result_typ.pos; fvs = _129_1559; uvs = _129_1558}))))

let mk_Exp_bvar : bvvar  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun x t p -> (let _129_1569 = (get_typ_ref t)
in (let _129_1568 = (mk_fvs ())
in (let _129_1567 = (mk_uvs ())
in {n = Exp_bvar (x); tk = _129_1569; pos = p; fvs = _129_1568; uvs = _129_1567}))))

let mk_Exp_fvar : (fvvar * fv_qual Prims.option)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_593 t p -> (match (_26_593) with
| (x, b) -> begin
(let _129_1578 = (get_typ_ref t)
in (let _129_1577 = (mk_fvs ())
in (let _129_1576 = (mk_uvs ())
in {n = Exp_fvar ((x, b)); tk = _129_1578; pos = p; fvs = _129_1577; uvs = _129_1576})))
end))

let mk_Exp_constant : sconst  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun s t p -> (let _129_1587 = (get_typ_ref t)
in (let _129_1586 = (mk_fvs ())
in (let _129_1585 = (mk_uvs ())
in {n = Exp_constant (s); tk = _129_1587; pos = p; fvs = _129_1586; uvs = _129_1585}))))

let mk_Exp_abs : (((((typ', (knd', Prims.unit) syntax) syntax bvdef, (knd', Prims.unit) syntax) withinfo_t, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list * exp)  ->  typ Prims.option  ->  FStar_Range.range  ->  exp = (fun _26_601 t' p -> (match (_26_601) with
| (b, e) -> begin
(match (b) with
| [] -> begin
e
...
| _26_606 -> begin
(let _129_1596 = (get_typ_ref t')
in (let _129_1595 = (mk_fvs ())
in (let _129_1594 = (mk_uvs ())
in {n = Exp_abs ((b, e)); tk = _129_1596; pos = p; fvs = _129_1595; uvs = _129_1594})))
end)
end))

let mk_Exp_abs' : (((((typ', (knd', Prims.unit) syntax) syntax bvdef, (knd', Prims.unit) syntax) withinfo_t, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list * (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_609 t' p -> (match (_26_609) with
| (b, e) -> begin
(let _129_1606 = (match ((b, e.n)) with
...
in (let _129_1605 = (get_typ_ref t')
in (let _129_1604 = (mk_fvs ())
in (let _129_1603 = (mk_uvs ())
in {n = _129_1606; tk = _129_1605; pos = p; fvs = _129_1604; uvs = _129_1603}))))
end))

let mk_Exp_app : (exp * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option) Prims.list)  ->  typ Prims.option  ->  FStar_Range.range  ->  exp = (fun _26_629 t p -> (match (_26_629) with
| (e1, args) -> begin
(match (args) with
| [] -> begin
e1
...
| _26_634 -> begin
(let _129_1615 = (get_typ_ref t)
in (let _129_1614 = (mk_fvs ())
in (let _129_1613 = (mk_uvs ())
in {n = Exp_app ((e1, args)); tk = _129_1615; pos = p; fvs = _129_1614; uvs = _129_1613})))
end)
end))

let mk_Exp_app_flat : ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option) Prims.list)  ->  typ Prims.option  ->  FStar_Range.range  ->  exp = (fun _26_637 t p -> (match (_26_637) with
| (e1, args) -> begin
(match (e1.n) with
...
let mk_Exp_app' : (exp * (((typ', (knd', Prims.unit) syntax) syntax, (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax) FStar_Util.either * arg_qualifier Prims.option) Prims.list)  ->  typ Prims.option  ->  FStar_Range.range  ->  exp = (fun _26_648 t p -> (match (_26_648) with
| (e1, args) -> begin
(match (args) with
| [] -> begin
e1
...
in if (not ((let _129_1636 = (FStar_List.tl vars)
in (let _129_1635 = (let _129_1634 = (let _129_1633 = (FStar_List.hd vars)
in (FStar_Util.set_eq order_bvd _129_1633))
in (FStar_Util.for_all _129_1634))
in (FStar_All.pipe_right _129_1636 _129_1635))))) then begin
(let vars = (let _129_1640 = (FStar_All.pipe_right vars (FStar_List.map (fun v -> (let _129_1639 = (FStar_List.map (fun _26_2 -> (match (_26_2) with
...
in (FStar_Util.concat_l ", " _129_1639)))))
in (FStar_Util.concat_l ";\n" _129_1640))
in (let _129_1643 = (let _129_1642 = (let _129_1641 = (FStar_Util.format1 "Each branch of this pattern binds different variables: %s" vars)
in (_129_1641, p.p))
in Error (_129_1642))
in (Prims.raise _129_1643)))
end else begin
(FStar_List.hd vars)
end)
end
...
let mk_Exp_match : (exp * (pat * exp Prims.option * exp) Prims.list)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_712 t p -> (match (_26_712) with
| (e, pats) -> begin
(let _129_1652 = (get_typ_ref t)
in (let _129_1651 = (mk_fvs ())
in (let _129_1650 = (mk_uvs ())
in {n = Exp_match ((e, pats)); tk = _129_1652; pos = p; fvs = _129_1651; uvs = _129_1650})))
end))

let mk_Exp_ascribed : (exp * typ * lident Prims.option)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_718 t' p -> (match (_26_718) with
| (e, t, l) -> begin
(let _129_1661 = (get_typ_ref t')
in (let _129_1660 = (mk_fvs ())
in (let _129_1659 = (mk_uvs ())
in {n = Exp_ascribed ((e, t, l)); tk = _129_1661; pos = p; fvs = _129_1660; uvs = _129_1659})))
end))

let mk_Exp_let : (letbindings * exp)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_723 t p -> (match (_26_723) with
| (lbs, e) -> begin
(let _129_1670 = (get_typ_ref t)
in (let _129_1669 = (mk_fvs ())
in (let _129_1668 = (mk_uvs ())
in {n = Exp_let ((lbs, e)); tk = _129_1670; pos = p; fvs = _129_1669; uvs = _129_1668})))
end))

let mk_Exp_uvar' : (uvar_e * typ)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_728 t' p -> (match (_26_728) with
| (u, t) -> begin
(let _129_1679 = (get_typ_ref t')
in (let _129_1678 = (mk_fvs ())
in (let _129_1677 = (mk_uvs ())
in {n = Exp_uvar ((u, t)); tk = _129_1679; pos = p; fvs = _129_1678; uvs = _129_1677})))
end))

let mk_Exp_uvar : (uvar_e * typ)  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_733 p -> (match (_26_733) with
| (u, t) -> begin
(mk_Exp_uvar' (u, t) (Some (t)) p)
end))

let mk_Exp_delayed : (exp * subst_t * exp Prims.option FStar_ST.ref)  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun _26_738 t p -> (match (_26_738) with
| (e, s, m) -> begin
(let _129_1692 = (get_typ_ref t)
in (let _129_1691 = (mk_fvs ())
in (let _129_1690 = (mk_uvs ())
in {n = Exp_delayed ((e, s, m)); tk = _129_1692; pos = p; fvs = _129_1691; uvs = _129_1690})))
end))

let mk_Exp_meta' : meta_e  ->  typ Prims.option  ->  FStar_Range.range  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun m t p -> (let _129_1701 = (get_typ_ref t)
in (let _129_1700 = (mk_fvs ())
in (let _129_1699 = (mk_uvs ())
in {n = Exp_meta (m); tk = _129_1701; pos = p; fvs = _129_1700; uvs = _129_1699}))))

let mk_Exp_meta : meta_e  ->  (exp', (typ', (knd', Prims.unit) syntax) syntax) syntax = (fun m -> (match (m) with
| Meta_desugared (e, _26_747) -> begin
(let _129_1704 = (FStar_ST.read e.tk)
in (mk_Exp_meta' m _129_1704 e.pos))
end))

let mk_lb : (lbname * lident * typ * exp)  ->  letbinding = (fun _26_754 -> (match (_26_754) with
| (x, eff, t, e) -> begin
...
let freevars_of_binders : binders  ->  freevars = (fun bs -> (FStar_All.pipe_right bs (FStar_List.fold_left (fun out _26_3 -> (match (_26_3) with
| (FStar_Util.Inl (btv), _26_802) -> begin
(let _26_804 = out
in (let _129_1741 = (FStar_Util.set_add btv out.ftvs)
in {ftvs = _129_1741; fxvs = _26_804.fxvs}))
end
| (FStar_Util.Inr (bxv), _26_809) -> begin
(let _26_811 = out
in (let _129_1742 = (FStar_Util.set_add bxv out.fxvs)
in {ftvs = _26_811.ftvs; fxvs = _129_1742}))
end)) no_fvs)))
...
let binders_of_freevars : freevars  ->  ((btvar, ((exp', (typ', (knd', Prims.unit) syntax) syntax) syntax bvdef, (typ', (knd', Prims.unit) syntax) syntax) withinfo_t) FStar_Util.either * arg_qualifier Prims.option) Prims.list = (fun fvs -> (let _129_1751 = (let _129_1748 = (FStar_Util.set_elements fvs.ftvs)
in (FStar_All.pipe_right _129_1748 (FStar_List.map t_binder)))
in (let _129_1750 = (let _129_1749 = (FStar_Util.set_elements fvs.fxvs)
in (FStar_All.pipe_right _129_1749 (FStar_List.map v_binder)))
in (FStar_List.append _129_1751 _129_1750))))

let is_implicit : arg_qualifier Prims.option  ->  Prims.bool = (fun _26_4 -> (match (_26_4) with
| Some (Implicit) -> begin
true
end
| _26_820 -> begin
...
let as_implicit : Prims.bool  ->  arg_qualifier Prims.option = (fun _26_5 -> (match (_26_5) with
| true -> begin
Some (Implicit)
end
| _26_824 -> begin
