...
(FStar_All.pipe_right args (FStar_List.for_all (fun _79_59 -> (match (_79_59) with
| (te, _79_58) -> begin
(match (te) with
| FStar_Util.Inl (_79_61) -> begin
true
end
...
| (FStar_Util.Inl (te), _79_75) -> begin
true
end
| _79_78 -> begin
false
...
| FStar_Extraction_ML_Syntax.MLE_Record (_79_115, fields) -> begin
(FStar_Util.for_all (fun _79_122 -> (match (_79_122) with
| (_79_120, e) -> begin
(is_ml_value e)
end)) fields)
end
...
(let _79_187 = (match ((FStar_Extraction_ML_Env.lookup_fv g f)) with
| ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Name (n); FStar_Extraction_ML_Syntax.ty = _79_175}, ttys, _79_181) -> begin
(n, ttys)
end
| _79_184 -> begin
(FStar_All.failwith "Expected a constructor")
end)
in (match (_79_187) with
| (d, tys) -> begin
(let nTyVars = (FStar_List.length (Prims.fst tys))
in (let _79_191 = (FStar_Util.first_N nTyVars pats)
in (match (_79_191) with
| (tysVarPats, restPats) -> begin
(let _79_198 = (FStar_Util.fold_map (fun g _79_195 -> (match (_79_195) with
| (p, imp) -> begin
(extract_one_pat disj true g p)
end)) g tysVarPats)
in (match (_79_198) with
| (g, tyMLPats) -> begin
(let _79_205 = (FStar_Util.fold_map (fun g _79_202 -> (match (_79_202) with
| (p, imp) -> begin
(extract_one_pat disj false g p)
end)) g restPats)
in (match (_79_205) with
| (g, restMLPats) -> begin
(let _79_213 = (let _181_106 = (FStar_All.pipe_right (FStar_List.append tyMLPats restMLPats) (FStar_List.collect (fun _79_2 -> (match (_79_2) with
| Some (x) -> begin
(x)::[]
end
| _79_210 -> begin
...
| FStar_Absyn_Syntax.Pat_dot_term (_79_225) -> begin
(g, Some ((FStar_Extraction_ML_Syntax.MLP_Wild, [])))
end
...
(let _79_263 = (extract_one_pat true g p)
in (match (_79_263) with
| (g, p) -> begin
...
in (p)::_181_122)
in (let _79_279 = (FStar_All.pipe_right ps (FStar_List.partition (fun _79_3 -> (match (_79_3) with
| (_79_268, _79_272::_79_270) -> begin
true
end
| _79_276 -> begin
false
end))))
in (match (_79_279) with
| (ps_when, rest) -> begin
(let ps = (FStar_All.pipe_right ps_when (FStar_List.map (fun _79_282 -> (match (_79_282) with
| (x, whens) -> begin
...
| _79_288 -> begin
(let _79_293 = (extract_one_pat false g p)
...
| FStar_Extraction_ML_Syntax.MLTY_Named (_79_340, _79_342) -> begin
...
| (FStar_Extraction_ML_Syntax.MLE_CTor (_79_351, args), Some (FStar_Absyn_Syntax.Record_ctor (_79_356, fields))) -> begin
...
in (let resugar_and_maybe_eta = (fun qual e -> (let _79_371 = (eta_args [] residualType)
in (match (_79_371) with
| (eargs, tres) -> begin
(match (eargs) with
| [] -> begin
...
| _79_374 -> begin
(let _79_377 = (FStar_List.unzip eargs)
in (match (_79_377) with
| (binders, eargs) -> begin
...
| (FStar_Extraction_ML_Syntax.MLE_App ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Name (mlp); FStar_Extraction_ML_Syntax.ty = _79_386}, mlarg::[]), _79_395) when (mlp = FStar_Extraction_ML_Syntax.mlp_lalloc) -> begin
(let _79_398 = (FStar_Extraction_ML_Env.debug g (fun _79_397 -> (match (()) with
| () -> begin
...
| (FStar_Extraction_ML_Syntax.MLE_App ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Name (mlp); FStar_Extraction_ML_Syntax.ty = _79_405}, mle::args), Some (FStar_Absyn_Syntax.Record_projector (f))) -> begin
...
(let _79_464 = (FStar_List.hd l)
in (match (_79_464) with
| (p1, w1, e1) -> begin
(let _79_468 = (let _181_180 = (FStar_List.tl l)
in (FStar_List.hd _181_180))
in (match (_79_468) with
| (p2, w2, e2) -> begin
...
| (e, _79_495, _79_497) -> begin
e
end)))
and check_exp' : FStar_Extraction_ML_Env.env  ->  FStar_Absyn_Syntax.exp  ->  FStar_Extraction_ML_Syntax.e_tag  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlexpr = (fun g e f t -> (let _79_506 = (synth_exp g e)
in (match (_79_506) with
| (e0, f0, t0) -> begin
...
and synth_exp : FStar_Extraction_ML_Env.env  ->  FStar_Absyn_Syntax.exp  ->  (FStar_Extraction_ML_Syntax.mlexpr * FStar_Extraction_ML_Syntax.e_tag * FStar_Extraction_ML_Syntax.mlty) = (fun g e -> (let _79_512 = (synth_exp' g e)
in (match (_79_512) with
| (e, f, t) -> begin
(erase g e f t)
end)))
and synth_exp' : FStar_Extraction_ML_Env.env  ->  FStar_Absyn_Syntax.exp  ->  (FStar_Extraction_ML_Syntax.mlexpr * FStar_Extraction_ML_Syntax.e_tag * FStar_Extraction_ML_Syntax.mlty) = (fun g e -> (let _79_516 = (FStar_Extraction_ML_Env.debug g (fun u -> (let _181_209 = (let _181_208 = (FStar_Absyn_Print.tag_of_exp e)
...
| (FStar_Absyn_Syntax.Exp_bvar (_)) | (FStar_Absyn_Syntax.Exp_fvar (_)) -> begin
(let _79_545 = (FStar_Extraction_ML_Env.lookup_var g e)
in (match (_79_545) with
| ((x, mltys, _79_542), qual) -> begin
(match (mltys) with
| ([], t) -> begin
...
| FStar_Absyn_Syntax.Exp_app (head, args) -> begin
(let rec synth_app = (fun is_data _79_559 _79_562 restArgs -> (match ((_79_559, _79_562)) with
| ((mlhead, mlargs_f), (f, t)) -> begin
(match ((restArgs, t)) with
| ([], _79_566) -> begin
(let _79_577 = if ((FStar_Absyn_Util.is_primop head) || (FStar_Extraction_ML_Util.codegen_fsharp ())) then begin
(let _181_224 = (FStar_All.pipe_right (FStar_List.rev mlargs_f) (FStar_List.map Prims.fst))
in ([], _181_224))
end else begin
(FStar_List.fold_left (fun _79_570 _79_573 -> (match ((_79_570, _79_573)) with
...
in (let l_app = (FStar_List.fold_right (fun _79_581 out -> (match (_79_581) with
| (x, arg) -> begin
...
| ((FStar_Util.Inl (_79_586), _79_589)::rest, FStar_Extraction_ML_Syntax.MLTY_Fun (tunit, f', t)) -> begin
...
| ((FStar_Util.Inr (e0), _79_602)::rest, FStar_Extraction_ML_Syntax.MLTY_Fun (tExpected, f', t)) -> begin
(let _79_614 = (synth_exp g e0)
in (match (_79_614) with
...
| (FStar_Absyn_Syntax.Exp_bvar (_)) | (FStar_Absyn_Syntax.Exp_fvar (_)) -> begin
(let _79_635 = (FStar_Extraction_ML_Env.lookup_var g head)
in (match (_79_635) with
| ((head_ml, (vars, t), inst_ok), qual) -> begin
(let has_typ_apps = (match (args) with
| (FStar_Util.Inl (_79_639), _79_642)::_79_637 -> begin
true
end
| _79_646 -> begin
false
end)
in (let _79_686 = (match (vars) with
| _79_651::_79_649 when ((not (has_typ_apps)) && inst_ok) -> begin
(head_ml, t, args)
end
| _79_654 -> begin
(let n = (FStar_List.length vars)
in if (n <= (FStar_List.length args)) then begin
(let _79_658 = (FStar_Util.first_N n args)
in (match (_79_658) with
| (prefix, rest) -> begin
...
(let _79_667 = head_ml
in {FStar_Extraction_ML_Syntax.expr = _79_667.FStar_Extraction_ML_Syntax.expr; FStar_Extraction_ML_Syntax.ty = t})
end
| FStar_Extraction_ML_Syntax.MLE_App (head, {FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Unit); FStar_Extraction_ML_Syntax.ty = _79_671}::[]) -> begin
(FStar_All.pipe_right (FStar_Extraction_ML_Syntax.MLE_App (((let _79_678 = head
in {FStar_Extraction_ML_Syntax.expr = _79_678.FStar_Extraction_ML_Syntax.expr; FStar_Extraction_ML_Syntax.ty = FStar_Extraction_ML_Syntax.MLTY_Fun ((FStar_Extraction_ML_Syntax.ml_unit_ty, FStar_Extraction_ML_Syntax.E_PURE, t))}), (FStar_Extraction_ML_Syntax.ml_unit)::[]))) (FStar_Extraction_ML_Syntax.with_ty t))
end
| _79_681 -> begin
(FStar_All.failwith "Impossible")
end)
...
| FStar_Absyn_Syntax.Exp_abs (bs, body) -> begin
(let _79_718 = (FStar_List.fold_left (fun _79_702 _79_706 -> (match ((_79_702, _79_706)) with
| ((ml_bs, env), (b, _79_705)) -> begin
(match (b) with
...
in (let _79_723 = (synth_exp env body)
in (match (_79_723) with
| (ml_body, f, t) -> begin
(let _79_733 = (FStar_List.fold_right (fun _79_727 _79_730 -> (match ((_79_727, _79_730)) with
| ((_79_725, targ), (f, t)) -> begin
...
(let maybe_generalize = (fun _79_745 -> (match (_79_745) with
...
(let _79_769 = (match ((FStar_Util.prefix_until (fun _79_4 -> (match (_79_4) with
| (FStar_Util.Inr (_79_754), _79_757) -> begin
true
end
| _79_760 -> begin
false
...
(let _79_778 = (FStar_Util.first_N n bs)
in (match (_79_778) with
...
| (FStar_Util.Inl (a), _79_787) -> begin
a
end
| _79_790 -> begin
(FStar_All.failwith "Impossible")
...
in (let check_lb = (fun env _79_824 -> (match (_79_824) with
...
in (let _79_853 = (FStar_List.fold_right (fun lb _79_834 -> (match (_79_834) with
| (env, lbs) -> begin
(let _79_847 = lb
in (match (_79_847) with
| (lbname, _79_837, (t, (_79_840, polytype)), add_unit, _79_846) -> begin
(let _79_850 = (FStar_Extraction_ML_Env.extend_lb env lbname t polytype add_unit true)
...
in (let _79_859 = (synth_exp env_body e')
in (match (_79_859) with
| (e', f', t') -> begin
...
(let _79_868 = (synth_exp g scrutinee)
in (match (_79_868) with
| (e, f_e, t_e) -> begin
(let _79_872 = (check_pats_for_ite pats)
...
(let _79_884 = (synth_exp g then_e)
in (match (_79_884) with
| (then_mle, f_then, t_then) -> begin
(let _79_888 = (synth_exp g else_e)
in (match (_79_888) with
| (else_mle, f_else, t_else) -> begin
(let _79_891 = if (FStar_Extraction_ML_Util.type_leq g t_then t_else) then begin
(t_else, no_lift)
end else begin
...
(let mlbranches = (FStar_All.pipe_right pats (FStar_List.collect (fun _79_897 -> (match (_79_897) with
| (pat, when_opt, branch) -> begin
(let _79_900 = (extract_pat g pat)
in (match (_79_900) with
| (env, p) -> begin
(let _79_911 = (match (when_opt) with
| None -> begin
(None, FStar_Extraction_ML_Syntax.E_PURE)
end
| Some (w) -> begin
(let _79_907 = (synth_exp env w)
in (match (_79_907) with
| (w, f_w, t_w) -> begin
(let w = (maybe_coerce env w t_w FStar_Extraction_ML_Syntax.ml_bool_ty)
in (Some (w), f_w))
end))
end)
in (match (_79_911) with
| (when_opt, f_when) -> begin
(let _79_915 = (synth_exp env branch)
in (match (_79_915) with
| (mlbranch, f_branch, t_branch) -> begin
(FStar_All.pipe_right p (FStar_List.map (fun _79_918 -> (match (_79_918) with
| (p, wopt) -> begin
...
| [] -> begin
(let _79_927 = (FStar_Extraction_ML_Env.lookup_fv g (FStar_Absyn_Util.fv FStar_Absyn_Const.failwith_lid))
in (match (_79_927) with
| (fw, _79_924, _79_926) -> begin
...
| (_79_930, _79_932, (_79_934, f_first, t_first))::rest -> begin
(let _79_960 = (FStar_List.fold_left (fun _79_942 _79_952 -> (match ((_79_942, _79_952)) with
| ((topt, f), (_79_944, _79_946, (_79_948, f_branch, t_branch))) -> begin
...
| (topt, f_match) -> begin
(let mlbranches = (FStar_All.pipe_right mlbranches (FStar_List.map (fun _79_971 -> (match (_79_971) with
| (p, (wopt, _79_964), (b, _79_968, t)) -> begin
...
| FStar_Absyn_Syntax.Exp_meta (FStar_Absyn_Syntax.Meta_desugared (e, _79_984)) -> begin
(synth_exp g e)
end
...
| (_79_1003, ts, _79_1006) -> begin
(let _79_1019 = (match ((Prims.snd ts)) with
| FStar_Extraction_ML_Syntax.MLTY_Fun (_79_1009, _79_1011, t) -> begin
((FStar_Extraction_ML_Syntax.MLP_Wild)::[], ((Prims.fst ts), t))
end
| _79_1016 -> begin
([], ts)
end)
in (match (_79_1019) with
| (arg_pat, ts) -> begin
(let rid = constrName
...
in (let discrBody = (let _181_332 = (let _181_331 = (let _181_330 = (let _181_329 = (let _181_328 = (let _181_327 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty targ) (FStar_Extraction_ML_Syntax.MLE_Name (([], (FStar_Extraction_ML_Syntax.idsym mlid)))))
in (let _181_326 = (let _181_325 = (let _181_321 = (let _181_319 = (let _181_318 = (FStar_Extraction_ML_Syntax.mlpath_of_lident rid)
in (_181_318, arg_pat))
in FStar_Extraction_ML_Syntax.MLP_CTor (_181_319))
in (let _181_320 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Bool (true))))
in (_181_321, None, _181_320)))
in (let _181_324 = (let _181_323 = (let _181_322 = (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) (FStar_Extraction_ML_Syntax.MLE_Const (FStar_Extraction_ML_Syntax.MLC_Bool (false))))
in (FStar_Extraction_ML_Syntax.MLP_Wild, None, _181_322))
in (_181_323)::[])
in (_181_325)::_181_324))
in (_181_327, _181_326)))
in FStar_Extraction_ML_Syntax.MLE_Match (_181_328))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.ml_bool_ty) _181_329))
in (((mlid, targ))::[], _181_330))
in FStar_Extraction_ML_Syntax.MLE_Fun (_181_331))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty disc_ty) _181_332))
