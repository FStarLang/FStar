...
open Prims
# 41 "rel.fs"
let new_uvar : FStar_Range.range  ->  FStar_Syntax_Syntax.binder Prims.list  ->  FStar_Syntax_Syntax.term  ->  ((FStar_Syntax_Syntax.term', FStar_Syntax_Syntax.term') FStar_Syntax_Syntax.syntax * (FStar_Syntax_Syntax.term', FStar_Syntax_Syntax.term') FStar_Syntax_Syntax.syntax) = (fun r binders k -> (
# 42 "rel.fs"
let binders = (FStar_All.pipe_right binders (FStar_List.filter (fun x -> (FStar_All.pipe_right (FStar_Syntax_Syntax.is_null_binder x) Prims.op_Negation))))
in (
# 43 "rel.fs"
...
in (
# 50 "rel.fs"
let k' = (let _192_8 = (FStar_Syntax_Syntax.mk_Total k)
...
# 143 "rel.fs"
let _90_100 = (empty_worklist env)
...
# 144 "rel.fs"
let _90_104 = (empty_worklist env)
...
# 149 "rel.fs"
let _90_118 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
# 348 "rel.fs"
let _90_357 = (base_and_refinement env wl t)
...
# 394 "rel.fs"
let solve_prob' : Prims.bool  ->  FStar_TypeChecker_Common.prob  ->  (FStar_Syntax_Syntax.term', FStar_Syntax_Syntax.term') FStar_Syntax_Syntax.syntax Prims.option  ->  uvi Prims.list  ->  worklist  ->  worklist = (fun resolve_ok prob logical_guard uvis wl -> (
# 395 "rel.fs"
...
in (
# 402 "rel.fs"
let bs = (FStar_All.pipe_right bs (FStar_List.filter (fun x -> (FStar_All.pipe_right (FStar_Syntax_Syntax.is_null_binder x) Prims.op_Negation))))
in (
# 403 "rel.fs"
let phi = (u_abs bs phi)
in (
# 404 "rel.fs"
let _90_410 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug wl.tcenv) (FStar_Options.Other ("Rel"))) then begin
...
# 415 "rel.fs"
let _90_423 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug wl.tcenv) (FStar_Options.Other ("RelCheck"))) then begin
...
# 420 "rel.fs"
let solve_prob : FStar_TypeChecker_Common.prob  ->  (FStar_Syntax_Syntax.term', FStar_Syntax_Syntax.term') FStar_Syntax_Syntax.syntax Prims.option  ->  uvi Prims.list  ->  worklist  ->  worklist = (fun prob logical_guard uvis wl -> (
# 421 "rel.fs"
...
in (
# 425 "rel.fs"
let _90_449 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug wl.tcenv) (FStar_Options.Other ("RelCheck"))) then begin
...
in (
# 454 "rel.fs"
let _90_473 = (occurs_check env wl uk t)
...
in (
# 461 "rel.fs"
let _90_491 = (FStar_All.pipe_right v2 (FStar_List.fold_left (fun _90_483 _90_486 -> (match ((_90_483, _90_486)) with
...
in (
# 613 "rel.fs"
let fail = (fun _90_681 -> (match (()) with
| () -> begin
(MisMatch, None)
end))
in (
...
in (
# 644 "rel.fs"
let tcs = (FStar_All.pipe_right args (FStar_List.map (fun _90_724 -> (match (_90_724) with
| (t, _90_723) -> begin
(None, INVARIANT, T (t))
end))))
...
in (
# 651 "rel.fs"
let _90_734 = (FStar_Syntax_Subst.open_comp bs c)
...
in (
# 660 "rel.fs"
let rec decompose = (fun out _90_19 -> (match (_90_19) with
| [] -> begin
...
let k = (
# 696 "rel.fs"
let _90_806 = (FStar_Syntax_Util.type_u ())
...
in (
# 698 "rel.fs"
let _90_810 = (new_uvar r scope k)
...
in (
# 727 "rel.fs"
let _90_889 = (let _192_624 = (FStar_List.map (sub_prob scope args) components)
...
# 800 "rel.fs"
let prob = (let _192_671 = (compress_prob wl pr)
...
| (rank, tp) -> begin
(let _192_677 = (FStar_All.pipe_right (
# 832 "rel.fs"
let _90_1053 = tp
...
(let _192_679 = (FStar_All.pipe_right (
# 834 "rel.fs"
let _90_1057 = cp
...
# 840 "rel.fs"
let rec aux = (fun _90_1064 probs -> (match (_90_1064) with
...
# 869 "rel.fs"
let _90_1104 = (FStar_Syntax_Util.univ_kernel u)
...
# 948 "rel.fs"
let _90_1234 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("RelCheck"))) then begin
...
# 1033 "rel.fs"
let _90_1371 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("RelCheck"))) then begin
...
in (
# 1035 "rel.fs"
let _90_1375 = (FStar_Syntax_Util.head_and_args tp.FStar_TypeChecker_Common.lhs)
...
# 1040 "rel.fs"
let _90_1390 = (FStar_Syntax_Util.head_and_args t1)
...
in (
# 1064 "rel.fs"
let subst = (let _192_860 = (let _192_859 = (let _192_858 = (FStar_Syntax_Syntax.bv_to_name x)
in (0, _192_858))
...
in (
# 1133 "rel.fs"
let _90_1514 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("RelCheck"))) then begin
(
# 1134 "rel.fs"
let wl' = (
# 1134 "rel.fs"
...
in (
# 1140 "rel.fs"
let _90_1527 = (FStar_List.fold_left (fun wl p -> (solve_prob' true p None [] wl)) wl upper_bounds)
in Some (wl))))
end
| _90_1530 -> begin
None
...
in (
# 1154 "rel.fs"
let _90_1549 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1162 "rel.fs"
let prob = (let _192_906 = (let _192_905 = (FStar_All.pipe_left invert_rel (p_rel orig))
...
in (
# 1164 "rel.fs"
let subst = (let _192_910 = (let _192_908 = (let _192_907 = (FStar_Syntax_Syntax.bv_to_name hd1)
in (0, _192_907))
...
in (
# 1169 "rel.fs"
let _90_1578 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1200 "rel.fs"
let _90_1621 = (imitation_sub_probs orig env xs ps qs)
...
in (
# 1202 "rel.fs"
let _90_1623 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1218 "rel.fs"
let _90_1644 = (FStar_List.nth ps i)
...
let rec gs = (fun k -> (
# 1222 "rel.fs"
let _90_1653 = (FStar_Syntax_Util.arrow_formals k)
...
in (
# 1227 "rel.fs"
let _90_1667 = (new_uvar r xs k_a)
...
in (
# 1239 "rel.fs"
let proj = (let _192_961 = (FStar_Syntax_Syntax.mk_Tm_app xi g_xs None r)
in (u_abs xs _192_961))
in (
# 1240 "rel.fs"
let sub = (let _192_967 = (let _192_966 = (FStar_Syntax_Syntax.mk_Tm_app proj ps None r)
...
in (
# 1241 "rel.fs"
let _90_1687 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1242 "rel.fs"
let wl = (let _192_971 = (let _192_970 = (FStar_All.pipe_left Prims.fst (p_guard sub))
in Some (_192_970))
...
let subterms = (fun ps -> (
# 1250 "rel.fs"
let xs = (let _192_1000 = (FStar_Syntax_Util.arrow_formals k)
...
# 1272 "rel.fs"
let _90_1731 = (FStar_Syntax_Util.head_and_args t2)
...
# 1285 "rel.fs"
let fvs_hd = (let _192_1032 = (let _192_1031 = (FStar_Syntax_Util.head_and_args t2)
...
in (
# 1296 "rel.fs"
let _90_1757 = (occurs_check env wl (uv, k) t2)
...
in (
# 1329 "rel.fs"
let _90_1769 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
let kk = (
# 1374 "rel.fs"
let _90_1805 = (FStar_Syntax_Util.type_u ())
...
in (
# 1376 "rel.fs"
let _90_1809 = (new_uvar t.FStar_Syntax_Syntax.pos pattern_vars kk)
...
in (
# 1407 "rel.fs"
let solve_both_pats = (fun wl _90_1845 _90_1849 r -> (match ((_90_1845, _90_1849)) with
...
in (
# 1418 "rel.fs"
let _90_1854 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
let _90_1867 = (
# 1422 "rel.fs"
let _90_1859 = (FStar_Syntax_Util.type_u ())
...
in (
# 1426 "rel.fs"
let _90_1871 = (occurs_check env wl (u1, k1) sub1)
...
in (
# 1434 "rel.fs"
let _90_1877 = (occurs_check env wl (u2, k2) sub2)
...
in (
# 1441 "rel.fs"
let solve_one_pat = (fun _90_1885 _90_1890 -> (match ((_90_1885, _90_1890)) with
...
in (
# 1449 "rel.fs"
let guard = (let _192_1111 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
...
in (
# 1455 "rel.fs"
let _90_1910 = (occurs_check env wl (u1, k1) t2)
...
in (
# 1465 "rel.fs"
let _90_1923 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("QuasiPattern"))) then begin
...
in (
# 1488 "rel.fs"
let _90_1962 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("QuasiPattern"))) then begin
...
in (
# 1505 "rel.fs"
let match_num_binders = (fun _90_1976 _90_1979 -> (match ((_90_1976, _90_1979)) with
| ((bs1, mk_cod1), (bs2, mk_cod2)) -> begin
(
# 1507 "rel.fs"
let curry = (fun n bs mk_cod -> (
# 1508 "rel.fs"
let _90_1986 = (FStar_Util.first_N n bs)
...
in (
# 1534 "rel.fs"
let _90_2029 = (match_num_binders (bs1, (mk_c c1)) (bs2, (mk_c c2)))
...
in (
# 1548 "rel.fs"
let _90_2060 = (match_num_binders (bs1, (mk_t tbody1)) (bs2, (mk_t tbody2)))
...
in (
# 1561 "rel.fs"
let subst = (let _192_1196 = (let _192_1195 = (let _192_1194 = (FStar_Syntax_Syntax.bv_to_name x1)
in (0, _192_1194))
...
in (
# 1565 "rel.fs"
let mk_imp = (fun imp phi1 phi2 -> (let _192_1213 = (imp phi1 phi2)
in (FStar_All.pipe_right _192_1213 (guard_on_element problem x1))))
in (
# 1566 "rel.fs"
let fallback = (fun _90_2088 -> (match (()) with
| () -> begin
(
# 1567 "rel.fs"
...
in (
# 1571 "rel.fs"
let guard = (let _192_1216 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
...
(let _192_1228 = (FStar_All.pipe_left (fun _192_1226 -> FStar_TypeChecker_Common.TProb (_192_1226)) (
# 1611 "rel.fs"
let _90_2248 = problem
...
(match (ref_opt) with
| None -> begin
(let _192_1231 = (FStar_All.pipe_left (fun _192_1229 -> FStar_TypeChecker_Common.TProb (_192_1229)) (
# 1615 "rel.fs"
let _90_2254 = problem
...
in (
# 1620 "rel.fs"
let base_prob = (let _192_1233 = (mk_problem problem.FStar_TypeChecker_Common.scope orig t1 new_rel y.FStar_Syntax_Syntax.sort problem.FStar_TypeChecker_Common.element "flex-rigid: base type")
in (FStar_All.pipe_left (fun _192_1232 -> FStar_TypeChecker_Common.TProb (_192_1232)) _192_1233))
in (
# 1622 "rel.fs"
let guard = (let _192_1234 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
...
in (
# 1664 "rel.fs"
let head2 = (let _192_1243 = (FStar_Syntax_Util.head_and_args t2)
...
in (
# 1681 "rel.fs"
let _90_2433 = (FStar_Syntax_Util.head_and_args t1)
...
in (
# 1713 "rel.fs"
let formula = (let _192_1265 = (FStar_List.map (fun p -> (Prims.fst (p_guard p))) subprobs)
...
# 1747 "rel.fs"
let _90_2555 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("EQ"))) then begin
...
in (
# 1749 "rel.fs"
let sub_probs = (FStar_List.map2 (fun _90_2560 _90_2564 -> (match ((_90_2560, _90_2564)) with
...
in (
# 1752 "rel.fs"
let guard = (let _192_1285 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
...
in (
# 1792 "rel.fs"
let _90_2636 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1810 "rel.fs"
let _90_2682 = (match ((c1.FStar_Syntax_Syntax.effect_args, c2.FStar_Syntax_Syntax.effect_args)) with
...
in (
# 1829 "rel.fs"
let base_prob = (let _192_1329 = (sub_prob c1.FStar_Syntax_Syntax.result_typ problem.FStar_TypeChecker_Common.relation c2.FStar_Syntax_Syntax.result_typ "result type")
in (FStar_All.pipe_left (fun _192_1328 -> FStar_TypeChecker_Common.TProb (_192_1328)) _192_1329))
in (
# 1830 "rel.fs"
let wl = (let _192_1333 = (let _192_1332 = (let _192_1331 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
...
in (
# 1850 "rel.fs"
let carry = (let _192_1343 = (FStar_List.map (fun _90_2715 -> (match (_90_2715) with
| (_90_2713, x) -> begin
...
# 1923 "rel.fs"
let x = (let _192_1427 = (FStar_All.pipe_left (fun _192_1426 -> Some (_192_1426)) (FStar_TypeChecker_Env.get_range env))
...
in (
# 1925 "rel.fs"
let p = (let _192_1430 = (let _192_1429 = (FStar_Syntax_Syntax.bv_to_name x)
...
in (
# 1938 "rel.fs"
let _90_2844 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("ExplainRel"))) then begin
...
in (
# 1947 "rel.fs"
let _90_2854 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Simplification"))) then begin
...
in (
# 1948 "rel.fs"
let f = (match ((let _192_1449 = (FStar_Syntax_Util.unmeta f)
...
# 1958 "rel.fs"
let try_teq : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.term  ->  FStar_TypeChecker_Env.guard_t Prims.option = (fun env t1 t2 -> (
# 1959 "rel.fs"
let _90_2875 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1961 "rel.fs"
let prob = (let _192_1471 = (new_t_problem env t1 FStar_TypeChecker_Common.EQ t2 None (FStar_TypeChecker_Env.get_range env))
in (FStar_All.pipe_left (fun _192_1470 -> FStar_TypeChecker_Common.TProb (_192_1470)) _192_1471))
in (
# 1962 "rel.fs"
let g = (let _192_1473 = (solve_and_commit env (singleton env prob) (fun _90_2878 -> None))
...
# 1976 "rel.fs"
let try_subtype : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.term  ->  FStar_TypeChecker_Env.guard_t Prims.option = (fun env t1 t2 -> (
# 1977 "rel.fs"
let _90_2892 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1979 "rel.fs"
let _90_2896 = (new_t_prob env t1 FStar_TypeChecker_Common.SUB t2)
...
in (
# 1981 "rel.fs"
let _90_2900 = if ((FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) && (FStar_Util.is_some g)) then begin
...
# 1994 "rel.fs"
let _90_2908 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
...
in (
# 1997 "rel.fs"
let prob = (let _192_1515 = (new_problem env c1 rel c2 None (FStar_TypeChecker_Env.get_range env) "sub_comp")
...
# 2003 "rel.fs"
let _90_2921 = (FStar_Unionfind.rollback tx)
in (
# 2004 "rel.fs"
...
in (
# 2034 "rel.fs"
let ad_hoc_fallback = (fun _90_2954 -> (match (()) with
| () -> begin
(match (ineqs) with
| [] -> begin
()
...
let wl = (
# 2039 "rel.fs"
let _90_2958 = (empty_worklist env)
...
in (
# 2040 "rel.fs"
let _90_2998 = (FStar_All.pipe_right ineqs (FStar_List.iter (fun _90_2963 -> (match (_90_2963) with
| (u1, u2) -> begin
(
# 2041 "rel.fs"
let u1 = (FStar_TypeChecker_Normalize.normalize_universe env u1)
...
# 2059 "rel.fs"
let _90_2994 = (FStar_Syntax_Util.univ_kernel u')
...
let wl = (
# 2070 "rel.fs"
let _90_3002 = (empty_worklist env)
...
in (
# 2085 "rel.fs"
let _90_3022 = (solve_universe_inequalities' tx env ineqs)
...
# 2089 "rel.fs"
let fail = (fun _90_3029 -> (match (_90_3029) with
| (d, s) -> begin
(
# 2090 "rel.fs"
...
in (
# 2093 "rel.fs"
let _90_3032 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("RelCheck"))) then begin
...
in (
# 2098 "rel.fs"
let _90_3041 = (solve_universe_inequalities env g.FStar_TypeChecker_Env.univ_ineqs)
in (
# 2099 "rel.fs"
let _90_3043 = g
...
in (
# 2103 "rel.fs"
let _90_3057 = if (not ((FStar_Options.should_verify env.FStar_TypeChecker_Env.curmodule.FStar_Ident.str))) then begin
()
end else begin
...
in (
# 2122 "rel.fs"
let rec until_fixpoint = (fun _90_3070 implicits -> (match (_90_3070) with
| (out, changed) -> begin
...
in (
# 2131 "rel.fs"
let _90_3084 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("RelCheck"))) then begin
...
in (
# 2134 "rel.fs"
let _90_3091 = (env.FStar_TypeChecker_Env.type_of (
# 2134 "rel.fs"
let _90_3086 = env
...
# 2140 "rel.fs"
let g = (let _192_1590 = (solve_deferred_constraints env g)
