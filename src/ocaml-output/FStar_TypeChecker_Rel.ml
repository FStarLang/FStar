...
in (let _149_209 = (new_uvar FStar_Range.dummyRange scope FStar_Syntax_Util.ktype0)
...
in (let _149_219 = (next_pid ())
in (let _149_218 = (new_uvar FStar_Range.dummyRange scope FStar_Syntax_Util.ktype0)
in {FStar_TypeChecker_Common.pid = _149_219; FStar_TypeChecker_Common.lhs = lhs; FStar_TypeChecker_Common.relation = rel; FStar_TypeChecker_Common.rhs = rhs; FStar_TypeChecker_Common.element = elt; FStar_TypeChecker_Common.logical_guard = _149_218; FStar_TypeChecker_Common.scope = scope; FStar_TypeChecker_Common.reason = (reason)::[]; FStar_TypeChecker_Common.loc = loc; FStar_TypeChecker_Common.rank = None}))))


let problem_using_guard = (fun orig lhs rel rhs elt reason -> (let _149_226 = (next_pid ())
in {FStar_TypeChecker_Common.pid = _149_226; FStar_TypeChecker_Common.lhs = lhs; FStar_TypeChecker_Common.relation = rel; FStar_TypeChecker_Common.rhs = rhs; FStar_TypeChecker_Common.element = elt; FStar_TypeChecker_Common.logical_guard = (p_guard orig); FStar_TypeChecker_Common.scope = (p_scope orig); FStar_TypeChecker_Common.reason = (reason)::(p_reason orig); FStar_TypeChecker_Common.loc = (p_loc orig); FStar_TypeChecker_Common.rank = None}))
...
(let _149_238 = (FStar_All.pipe_left FStar_Range.string_of_range (p_loc d))
in (let _149_237 = (prob_to_string env d)
in (let _149_236 = (FStar_All.pipe_right (p_reason d) (FStar_String.concat "\n\t>"))
in (FStar_Util.format4 "(%s) Failed to solve the sub-problem\n%s\nWhich arose because:\n\t%s\nFailed because:%s\n" _149_238 _149_237 _149_236 s))))
end else begin
(
...
(let _149_240 = (FStar_TypeChecker_Normalize.term_to_string env tp.FStar_TypeChecker_Common.lhs)
in (let _149_239 = (FStar_TypeChecker_Normalize.term_to_string env tp.FStar_TypeChecker_Common.rhs)
in ((_149_240), (_149_239))))
end
| FStar_TypeChecker_Common.CProb (cp) -> begin
(let _149_242 = (FStar_TypeChecker_Normalize.comp_to_string env cp.FStar_TypeChecker_Common.lhs)
in (let _149_241 = (FStar_TypeChecker_Normalize.comp_to_string env cp.FStar_TypeChecker_Common.rhs)
in ((_149_242), (_149_241))))
end)
...
let whnf : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.term = (fun env t -> (let _149_261 = (let _149_260 = (FStar_Syntax_Util.unmeta t)
in (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Beta)::(FStar_TypeChecker_Normalize.WHNF)::[]) env _149_260))
in (FStar_Syntax_Subst.compress _149_261)))


let sn : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.term = (fun env t -> (let _149_266 = (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Beta)::[]) env t)
in (FStar_Syntax_Subst.compress _149_266)))


let norm_arg = (fun env t -> (let _149_269 = (sn env (Prims.fst t))
in ((_149_269), ((Prims.snd t)))))
...
let _55_294 = x
in (let _149_275 = (sn env x.FStar_Syntax_Syntax.sort)
in {FStar_Syntax_Syntax.ppname = _55_294.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _55_294.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = _149_275}))
in ((_149_276), (imp)))
end)))))
...
(let _149_283 = (aux u)
in FStar_Syntax_Syntax.U_succ (_149_283))
end
| FStar_Syntax_Syntax.U_max (us) -> begin
(let _149_284 = (FStar_List.map aux us)
in FStar_Syntax_Syntax.U_max (_149_284))
end
| _55_306 -> begin
u
end)))
in (let _149_285 = (aux u)
in (FStar_TypeChecker_Normalize.normalize_universe wl.tcenv _149_285))))
...
| tt -> begin
(let _149_299 = (let _149_298 = (FStar_Syntax_Print.term_to_string tt)
in (let _149_297 = (FStar_Syntax_Print.tag_of_term tt)
...
in (match ((let _149_300 = (FStar_Syntax_Subst.compress t1')
...
(let _149_303 = (let _149_302 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_301 = (FStar_Syntax_Print.tag_of_term t1)
in (FStar_Util.format2 "impossible (outer): Got %s ... %s\n" _149_302 _149_301)))
in (FStar_All.failwith _149_303))
end))
in (let _149_304 = (whnf env t1)
in (aux false _149_304))))


let unrefine : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.typ  ->  FStar_Syntax_Syntax.typ = (fun env t -> (let _149_309 = (base_and_refinement env (empty_worklist env) t)
in (FStar_All.pipe_right _149_309 Prims.fst)))


let trivial_refinement : FStar_Syntax_Syntax.term  ->  (FStar_Syntax_Syntax.bv * FStar_Syntax_Syntax.term) = (fun t -> (let _149_312 = (FStar_Syntax_Syntax.null_bv t)
in ((_149_312), (FStar_Syntax_Util.t_true))))
...
| FStar_TypeChecker_Common.TProb (p) -> begin
(let _149_325 = (FStar_Util.string_of_int p.FStar_TypeChecker_Common.pid)
in (let _149_324 = (let _149_321 = (whnf wl.tcenv p.FStar_TypeChecker_Common.lhs)
in (FStar_Syntax_Print.term_to_string _149_321))
in (let _149_323 = (let _149_322 = (whnf wl.tcenv p.FStar_TypeChecker_Common.rhs)
in (FStar_Syntax_Print.term_to_string _149_322))
in (FStar_Util.format4 "%s: %s  (%s)  %s" _149_325 _149_324 (rel_to_string p.FStar_TypeChecker_Common.relation) _149_323))))
end
| FStar_TypeChecker_Common.CProb (p) -> begin
(let _149_328 = (FStar_Util.string_of_int p.FStar_TypeChecker_Common.pid)
in (let _149_327 = (FStar_TypeChecker_Normalize.comp_to_string wl.tcenv p.FStar_TypeChecker_Common.lhs)
in (let _149_326 = (FStar_TypeChecker_Normalize.comp_to_string wl.tcenv p.FStar_TypeChecker_Common.rhs)
in (FStar_Util.format4 "%s: %s  (%s)  %s" _149_328 _149_327 (rel_to_string p.FStar_TypeChecker_Common.relation) _149_326))))
end))


let wl_to_string : worklist  ->  Prims.string = (fun wl -> (let _149_334 = (let _149_333 = (let _149_332 = (FStar_All.pipe_right wl.wl_deferred (FStar_List.map (fun _55_422 -> (match (_55_422) with
| (_55_418, _55_420, x) -> begin
x
end))))
in (FStar_List.append wl.attempting _149_332))
in (FStar_List.map (wl_prob_to_string wl) _149_333))
in (FStar_All.pipe_right _149_334 (FStar_String.concat "\n\t"))))
...
let _55_441 = (match ((let _149_341 = (FStar_Syntax_Subst.compress k)
in _149_341.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_arrow (bs, c) -> begin
if ((FStar_List.length bs) = (FStar_List.length ys)) then begin
(let _149_342 = (FStar_Syntax_Subst.open_comp bs c)
in ((((ys), (t))), (_149_342)))
end else begin
(
...
| (ys', t) -> begin
(let _149_343 = (FStar_Syntax_Util.arrow_formals_comp k)
in (((((FStar_List.append ys ys')), (t))), (_149_343)))
end))
end
end
| _55_434 -> begin
(let _149_345 = (let _149_344 = (FStar_Syntax_Syntax.mk_Total k)
in (([]), (_149_344)))
in ((((ys), (t))), (_149_345)))
end)
...
let c = (let _149_346 = (FStar_Syntax_Util.rename_binders xs ys)
in (FStar_Syntax_Subst.subst_comp _149_346 c))
in (let _149_350 = (let _149_349 = (FStar_All.pipe_right (FStar_Syntax_Util.lcomp_of_comp c) (fun _149_347 -> FStar_Util.Inl (_149_347)))
in (FStar_All.pipe_right _149_349 (fun _149_348 -> Some (_149_348))))
in (FStar_Syntax_Util.abs ys t _149_350)))
end
end)))
...
let _55_466 = (match ((let _149_361 = (FStar_Syntax_Subst.compress uv)
in _149_361.FStar_Syntax_Syntax.n)) with
...
(let _149_364 = (FStar_Util.string_of_int (p_pid prob))
in (let _149_363 = (FStar_Syntax_Print.term_to_string uv)
in (let _149_362 = (FStar_Syntax_Print.term_to_string phi)
in (FStar_Util.print3 "Solving %s (%s) with formula %s\n" _149_364 _149_363 _149_362))))
end else begin
()
end
...
(let _149_373 = (FStar_Util.string_of_int pid)
in (let _149_372 = (let _149_371 = (FStar_List.map (uvi_to_string wl.tcenv) sol)
in (FStar_All.pipe_right _149_371 (FStar_String.concat ", ")))
...
(let _149_386 = (FStar_Syntax_Util.mk_conj t f)
in Some (_149_386))
end))
in (

let _55_501 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug wl.tcenv) (FStar_Options.Other ("RelCheck"))) then begin
(let _149_389 = (FStar_All.pipe_left FStar_Util.string_of_int (p_pid prob))
in (let _149_388 = (let _149_387 = (FStar_List.map (uvi_to_string wl.tcenv) uvis)
in (FStar_All.pipe_right _149_387 (FStar_String.concat ", ")))
in (FStar_Util.print2 "Solving %s: with %s\n" _149_389 _149_388)))
end else begin
()
end
...
let rec occurs = (fun wl uk t -> (let _149_399 = (let _149_397 = (FStar_Syntax_Free.uvars t)
in (FStar_All.pipe_right _149_397 FStar_Util.set_elements))
in (FStar_All.pipe_right _149_399 (FStar_Util.for_some (fun _55_510 -> (match (_55_510) with
| (uv, _55_509) -> begin
...
(let _149_406 = (let _149_405 = (FStar_Syntax_Print.uvar_to_string (Prims.fst uk))
in (let _149_404 = (FStar_Syntax_Print.term_to_string t)
in (FStar_Util.format2 "occurs-check failed (%s occurs in %s)" _149_405 _149_404)))
in Some (_149_406))
end
in ((occurs_ok), (msg)))))
...
| (occurs_ok, msg) -> begin
(let _149_412 = (FStar_Util.set_is_subset_of fvs_t fvs)
in ((occurs_ok), (_149_412), (((msg), (fvs), (fvs_t)))))
end))))
...
if (let _149_421 = (FStar_Util.set_mem x v1_set)
in (FStar_All.pipe_left Prims.op_Negation _149_421)) then begin
((isect), (isect_set))
end else begin
(
...
(let _149_422 = (FStar_Util.set_add x isect_set)
in (((((x), (imp)))::isect), (_149_422)))
end else begin
((isect), (isect_set))
end)
end
...
(let _149_437 = (FStar_All.pipe_left FStar_Syntax_Print.term_to_string (Prims.fst hd))
...
let is_flex : FStar_Syntax_Syntax.term  ->  Prims.bool = (fun t -> (match ((let _149_440 = (FStar_Syntax_Subst.compress t)
in _149_440.FStar_Syntax_Syntax.n)) with
...
(let _149_476 = (head_matches env f g)
...
(let _149_477 = (head_matches env x.FStar_Syntax_Syntax.sort y.FStar_Syntax_Syntax.sort)
in (FStar_All.pipe_right _149_477 head_match))
end
| (FStar_Syntax_Syntax.Tm_refine (x, _55_763), _55_767) -> begin
(let _149_478 = (head_matches env x.FStar_Syntax_Syntax.sort t2)
in (FStar_All.pipe_right _149_478 head_match))
end
| (_55_770, FStar_Syntax_Syntax.Tm_refine (x, _55_773)) -> begin
(let _149_479 = (head_matches env t1 x.FStar_Syntax_Syntax.sort)
...
| (head, _55_828) -> begin
(match ((let _149_492 = (FStar_Syntax_Util.un_uinst head)
in _149_492.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_fvar (fv) -> begin
if (let _149_493 = (FStar_TypeChecker_Env.lookup_definition FStar_TypeChecker_Env.OnlyInline env fv.FStar_Syntax_Syntax.fv_name.FStar_Syntax_Syntax.v)
in (FStar_All.pipe_right _149_493 FStar_Option.isSome)) then begin
(let _149_495 = (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Beta)::(FStar_TypeChecker_Normalize.Inline)::[]) env t)
in (FStar_All.pipe_right _149_495 (fun _149_494 -> Some (_149_494))))
end else begin
None
end
end
...
(match ((let _149_515 = (maybe_inline t1)
in (let _149_514 = (maybe_inline t2)
in ((_149_515), (_149_514))))) with
| (None, None) -> begin
(fail r)
end
| (Some (t1), None) -> begin
...
in (let _149_516 = (normalize_refinement ((FStar_TypeChecker_Normalize.UnfoldUntil (d1))::(FStar_TypeChecker_Normalize.WHNF)::[]) env wl t2)
in ((t1), (_149_516))))
end
...
| (t, _55_913) -> begin
(let _149_587 = (new_uvar r binders t)
in (Prims.fst _149_587))
end)))


let kind_type : FStar_Syntax_Syntax.binders  ->  FStar_Range.range  ->  FStar_Syntax_Syntax.typ = (fun binders r -> (let _149_592 = (FStar_Syntax_Util.type_u ())
in (FStar_All.pipe_right _149_592 Prims.fst)))
...
in (let _149_654 = (decompose [] bs)
in ((rebuild), (matches), (_149_654)))))
end)))
end
| _55_998 -> begin
(
...
in (let _149_686 = (let _149_685 = (mk_problem scope orig gi_ps (vary_rel rel variance) ti None "type subterm")
in (FStar_All.pipe_left (fun _149_684 -> FStar_TypeChecker_Common.TProb (_149_684)) _149_685))
in ((T (((gi_xs), (mk_kind)))), (_149_686))))
end)))
end
...
(let _149_699 = (let _149_698 = (FStar_Syntax_Syntax.mk_Total' gi_xs uopt)
in (FStar_All.pipe_left (fun _149_697 -> C (_149_697)) _149_698))
in ((_149_699), ((prob)::[])))
end
| _55_1087 -> begin
(FStar_All.failwith "impossible")
...
(let _149_706 = (let _149_705 = (FStar_Syntax_Syntax.mk_GTotal' gi_xs uopt)
in (FStar_All.pipe_left (fun _149_704 -> C (_149_704)) _149_705))
in ((_149_706), ((prob)::[])))
end
| _55_1111 -> begin
(FStar_All.failwith "impossible")
...
let _55_1132 = (let _149_712 = (FStar_List.map (sub_prob scope args) components)
in (FStar_All.pipe_right _149_712 FStar_List.unzip))
in (match (_55_1132) with
| (tcs, sub_probs) -> begin
(

let gi_xs = (let _149_717 = (let _149_716 = (let _149_713 = (FStar_List.hd tcs)
in (FStar_All.pipe_right _149_713 un_T))
in (let _149_715 = (let _149_714 = (FStar_List.tl tcs)
in (FStar_All.pipe_right _149_714 (FStar_List.map arg_of_tc)))
in {FStar_Syntax_Syntax.comp_univs = c.FStar_Syntax_Syntax.comp_univs; FStar_Syntax_Syntax.effect_name = c.FStar_Syntax_Syntax.effect_name; FStar_Syntax_Syntax.result_typ = _149_716; FStar_Syntax_Syntax.effect_args = _149_715; FStar_Syntax_Syntax.flags = c.FStar_Syntax_Syntax.flags}))
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_Comp _149_717))
...
(let _149_719 = (let _149_718 = (FStar_Syntax_Util.arg_of_non_null_binder b)
in (_149_718)::args)
in ((Some (b)), ((b)::scope), (_149_719)))
end
| _55_1150 -> begin
((None), (scope), (args))
end)
...
| None -> begin
(let _149_722 = (let _149_721 = (FStar_All.pipe_right probs (FStar_List.map (fun prob -> (FStar_All.pipe_right (p_guard prob) Prims.fst))))
in (f)::_149_721)
in (FStar_Syntax_Util.mk_conj_l _149_722))
end
| Some (b) -> begin
(let _149_726 = (let _149_725 = (FStar_Syntax_Util.mk_forall (Prims.fst b) f)
in (let _149_724 = (FStar_All.pipe_right probs (FStar_List.map (fun prob -> (FStar_All.pipe_right (p_guard prob) Prims.fst))))
in (_149_725)::_149_724))
in (FStar_Syntax_Util.mk_conj_l _149_726))
end)
...
let _55_1218 = p
in (let _149_748 = (whnf wl.tcenv p.FStar_TypeChecker_Common.lhs)
in (let _149_747 = (whnf wl.tcenv p.FStar_TypeChecker_Common.rhs)
in {FStar_TypeChecker_Common.pid = _55_1218.FStar_TypeChecker_Common.pid; FStar_TypeChecker_Common.lhs = _149_748; FStar_TypeChecker_Common.relation = _55_1218.FStar_TypeChecker_Common.relation; FStar_TypeChecker_Common.rhs = _149_747; FStar_TypeChecker_Common.element = _55_1218.FStar_TypeChecker_Common.element; FStar_TypeChecker_Common.logical_guard = _55_1218.FStar_TypeChecker_Common.logical_guard; FStar_TypeChecker_Common.scope = _55_1218.FStar_TypeChecker_Common.scope; FStar_TypeChecker_Common.reason = _55_1218.FStar_TypeChecker_Common.reason; FStar_TypeChecker_Common.loc = _55_1218.FStar_TypeChecker_Common.loc; FStar_TypeChecker_Common.rank = _55_1218.FStar_TypeChecker_Common.rank}))))


let compress_prob : worklist  ->  FStar_TypeChecker_Common.prob  ->  FStar_TypeChecker_Common.prob = (fun wl p -> (match (p) with
| FStar_TypeChecker_Common.TProb (p) -> begin
(let _149_754 = (compress_tprob wl p)
in (FStar_All.pipe_right _149_754 (fun _149_753 -> FStar_TypeChecker_Common.TProb (_149_753))))
end
...
let prob = (let _149_759 = (compress_prob wl pr)
in (FStar_All.pipe_right _149_759 maybe_invert_p))
in (match (prob) with
| FStar_TypeChecker_Common.TProb (tp) -> begin
(
...
(let _149_847 = (let _149_846 = (FStar_Syntax_Print.univ_to_string u1)
in (let _149_845 = (FStar_Syntax_Print.univ_to_string u2)
in (FStar_Util.format2 "Unable to unify universes: %s and %s" _149_846 _149_845)))
in UFailed (_149_847))
end
end
...
| _55_1397 -> begin
(let _149_854 = (let _149_853 = (FStar_Syntax_Print.univ_to_string u1)
in (let _149_852 = (FStar_Syntax_Print.univ_to_string u2)
in (FStar_Util.format3 "Unable to unify universes: %s and %s (%s)" _149_853 _149_852 msg)))
in UFailed (_149_854))
end))
in (match (((u1), (u2))) with
...
(let _149_857 = (let _149_856 = (FStar_Syntax_Print.univ_to_string u1)
in (let _149_855 = (FStar_Syntax_Print.univ_to_string u2)
in (FStar_Util.format2 "Impossible: found an de Bruijn universe variable or unknown universe: %s, %s" _149_856 _149_855)))
in (FStar_All.failwith _149_857))
end
...
(let _149_860 = (let _149_859 = (FStar_Syntax_Print.univ_to_string (FStar_Syntax_Syntax.U_unif (v1)))
in (let _149_858 = (FStar_Syntax_Print.univ_to_string u)
in (FStar_Util.format2 "Failed occurs check: %s occurs in %s" _149_859 _149_858)))
in (try_umax_components u1 u2 _149_860))
end else begin
(let _149_861 = (extend_solution orig ((UNIV (((v1), (u))))::[]) wl)
in USolved (_149_861))
end)
end
...
(let _149_915 = (wl_to_string probs)
...
(match (attempt) with
| [] -> begin
(let _149_918 = (FStar_List.map (fun _55_1541 -> (match (_55_1541) with
| (_55_1538, x, y) -> begin
((x), (y))
...
let _55_1544 = probs
in (let _149_920 = (FStar_All.pipe_right attempt (FStar_List.map (fun _55_1551 -> (match (_55_1551) with
| (_55_1547, _55_1549, y) -> begin
y
end))))
in {attempting = _149_920; wl_deferred = rest; ctr = _55_1544.ctr; defer_ok = _55_1544.defer_ok; smt_ok = _55_1544.smt_ok; tcenv = _55_1544.tcenv}))
in (solve env _149_921))
end)
end))
end)
end)))
...
| USolved (wl) -> begin
(let _149_927 = (solve_prob orig None [] wl)
in (solve env _149_927))
end
| UFailed (msg) -> begin
(giveup env msg orig)
end
...
(let _149_943 = (prob_to_string env orig)
...
(let _149_947 = (FStar_Util.string_of_int tp.FStar_TypeChecker_Common.pid)
...
| Some (t1, t2) -> begin
(let _149_953 = (FStar_Syntax_Subst.compress t1)
in (let _149_952 = (FStar_Syntax_Subst.compress t2)
in ((_149_953), (_149_952))))
end
| None -> begin
(let _149_955 = (FStar_Syntax_Subst.compress t1)
in (let _149_954 = (FStar_Syntax_Subst.compress t2)
...
let eq_prob = (fun t1 t2 -> (let _149_961 = (new_problem env t1 FStar_TypeChecker_Common.EQ t2 None t1.FStar_Syntax_Syntax.pos "meeting refinements")
in (FStar_All.pipe_left (fun _149_960 -> FStar_TypeChecker_Common.TProb (_149_960)) _149_961)))
in (match (((t1.FStar_Syntax_Syntax.n), (t2.FStar_Syntax_Syntax.n))) with
| (FStar_Syntax_Syntax.Tm_refine (x, phi1), FStar_Syntax_Syntax.Tm_refine (y, phi2)) -> begin
(let _149_968 = (let _149_967 = (let _149_964 = (let _149_963 = (let _149_962 = (FStar_Syntax_Util.mk_disj phi1 phi2)
in ((x), (_149_962)))
in FStar_Syntax_Syntax.Tm_refine (_149_963))
in (FStar_Syntax_Syntax.mk _149_964 None t1.FStar_Syntax_Syntax.pos))
in (let _149_966 = (let _149_965 = (eq_prob x.FStar_Syntax_Syntax.sort y.FStar_Syntax_Syntax.sort)
in (_149_965)::[])
in ((_149_967), (_149_966))))
in Some (_149_968))
end
| (_55_1682, FStar_Syntax_Syntax.Tm_refine (x, _55_1685)) -> begin
(let _149_971 = (let _149_970 = (let _149_969 = (eq_prob x.FStar_Syntax_Syntax.sort t1)
in (_149_969)::[])
in ((t1), (_149_970)))
in Some (_149_971))
end
| (FStar_Syntax_Syntax.Tm_refine (x, _55_1691), _55_1695) -> begin
(let _149_974 = (let _149_973 = (let _149_972 = (eq_prob x.FStar_Syntax_Syntax.sort t2)
in (_149_972)::[])
in ((t2), (_149_973)))
in Some (_149_974))
end
...
(match ((let _149_975 = (FStar_Syntax_Util.un_uinst head1)
in _149_975.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_fvar ({FStar_Syntax_Syntax.fv_name = _55_1708; FStar_Syntax_Syntax.fv_delta = FStar_Syntax_Syntax.Delta_unfoldable (i); FStar_Syntax_Syntax.fv_qual = _55_1704}) -> begin
(
...
| (u', _55_1729) -> begin
(match ((let _149_977 = (whnf env u')
in _149_977.FStar_Syntax_Syntax.n)) with
...
(match ((let _149_982 = (whnf env hd.FStar_TypeChecker_Common.lhs)
in (disjoin bound _149_982))) with
...
in (match ((let _149_984 = (let _149_983 = (whnf env tp.FStar_TypeChecker_Common.lhs)
in ((_149_983), ([])))
in (make_lower_bound _149_984 lower_bounds))) with
| None -> begin
(
...
in (let _149_985 = (wl_to_string wl')
...
(let _149_991 = (FStar_Util.string_of_int tp.FStar_TypeChecker_Common.pid)
...
(let _149_1003 = (let _149_1002 = (let _149_1001 = (new_problem env t1 FStar_TypeChecker_Common.EQ t2 None t1.FStar_Syntax_Syntax.pos "joining refinements")
in (FStar_All.pipe_left (fun _149_1000 -> FStar_TypeChecker_Common.TProb (_149_1000)) _149_1001))
in (_149_1002)::[])
in Some (_149_1003))
end
end else begin
...
(let _149_1007 = (let _149_1006 = (let _149_1005 = (new_problem env t1 FStar_TypeChecker_Common.EQ t2 None t1.FStar_Syntax_Syntax.pos "joining refinements")
in (FStar_All.pipe_left (fun _149_1004 -> FStar_TypeChecker_Common.TProb (_149_1004)) _149_1005))
in (_149_1006)::[])
in Some (_149_1007))
end
end else begin
...
in (let _149_1014 = (let _149_1013 = (let _149_1012 = (FStar_Syntax_Util.mk_conj phi1 phi2)
in (FStar_Syntax_Util.refine x _149_1012))
in ((_149_1013), (m)))
in Some (_149_1014))))))
end))
end
...
| (u', _55_1894) -> begin
(match ((let _149_1016 = (whnf env u')
in _149_1016.FStar_Syntax_Syntax.n)) with
...
| (FStar_TypeChecker_Common.TProb (hd))::tl -> begin
(match ((let _149_1021 = (whnf env hd.FStar_TypeChecker_Common.rhs)
in (conjoin bound _149_1021))) with
| Some (bound, sub) -> begin
...
in (match ((let _149_1023 = (let _149_1022 = (whnf env tp.FStar_TypeChecker_Common.rhs)
in ((_149_1022), ([])))
in (make_upper_bound _149_1023 upper_bounds))) with
| None -> begin
(
...
in (let _149_1024 = (wl_to_string wl')
...
(let _149_1052 = (prob_to_string env rhs_prob)
...
let _55_1987 = hd1
in (let _149_1053 = (FStar_Syntax_Subst.subst subst hd1.FStar_Syntax_Syntax.sort)
in {FStar_Syntax_Syntax.ppname = _55_1987.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _55_1987.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = _149_1053}))
in (

let hd2 = (

let _55_1990 = hd2
in (let _149_1054 = (FStar_Syntax_Subst.subst subst hd2.FStar_Syntax_Syntax.sort)
in {FStar_Syntax_Syntax.ppname = _55_1990.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _55_1990.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = _149_1054}))
in (

let prob = (let _149_1057 = (let _149_1056 = (FStar_All.pipe_left invert_rel (p_rel orig))
in (mk_problem scope orig hd1.FStar_Syntax_Syntax.sort _149_1056 hd2.FStar_Syntax_Syntax.sort None "Formal parameter"))
in (FStar_All.pipe_left (fun _149_1055 -> FStar_TypeChecker_Common.TProb (_149_1055)) _149_1057))
in (

let hd1 = (FStar_Syntax_Syntax.freshen_bv hd1)
in (

let subst = (let _149_1058 = (FStar_Syntax_Subst.shift_subst (Prims.parse_int "1") subst)
in (FStar_Syntax_Syntax.DB ((((Prims.parse_int "0")), (hd1))))::_149_1058)
in (
...
let phi = (let _149_1060 = (FStar_All.pipe_right (p_guard prob) Prims.fst)
in (let _149_1059 = (FStar_Syntax_Util.close_forall ((((hd1), (imp)))::[]) phi)
in (FStar_Syntax_Util.mk_conj _149_1060 _149_1059)))
in (

let _55_2002 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1062 = (FStar_Syntax_Print.term_to_string phi)
in (let _149_1061 = (FStar_Syntax_Print.bv_to_string hd1)
...
and solve_t : FStar_TypeChecker_Env.env  ->  tprob  ->  worklist  ->  solution = (fun env problem wl -> (let _149_1066 = (compress_tprob wl problem)
in (solve_t' env _149_1066 wl)))
...
in (let _149_1095 = (let _149_1094 = (FStar_Syntax_Syntax.bv_to_name x)
in (FStar_Syntax_Util.mk_has_type t1 _149_1094 t2))
in (FStar_Syntax_Util.mk_forall x _149_1095)))
end))
...
in (let _149_1096 = (solve_prob orig (Some (guard)) [] wl)
in (solve env _149_1096)))
end else begin
...
(let _149_1100 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1099 = (FStar_Syntax_Print.tag_of_term t1)
in (let _149_1098 = (FStar_Syntax_Print.term_to_string t2)
in (let _149_1097 = (FStar_Syntax_Print.tag_of_term t2)
in (FStar_Util.print4 "Head matches: %s (%s) and %s (%s)\n" _149_1100 _149_1099 _149_1098 _149_1097)))))
end else begin
()
end
in (
...
(let _149_1105 = (let _149_1104 = (FStar_Syntax_Print.term_to_string head1)
in (let _149_1103 = (args_to_string args1)
in (let _149_1102 = (FStar_Syntax_Print.term_to_string head2)
in (let _149_1101 = (args_to_string args2)
in (FStar_Util.format4 "unequal number of arguments: %s[%s] and %s[%s]" _149_1104 _149_1103 _149_1102 _149_1101)))))
in (giveup env _149_1105 orig))
end else begin
if ((nargs = (Prims.parse_int "0")) || (eq_args args1 args2)) then begin
(match ((solve_maybe_uinsts env orig head1 head2 wl)) with
| USolved (wl) -> begin
(let _149_1106 = (solve_prob orig None [] wl)
in (solve env _149_1106))
end
| UFailed (msg) -> begin
(giveup env msg orig)
end
...
(let _149_1110 = (mk_problem (p_scope orig) orig a FStar_TypeChecker_Common.EQ a' None "index")
in (FStar_All.pipe_left (fun _149_1109 -> FStar_TypeChecker_Common.TProb (_149_1109)) _149_1110))
end)) args1 args2)
in (

let formula = (let _149_1112 = (FStar_List.map (fun p -> (Prims.fst (p_guard p))) subprobs)
in (FStar_Syntax_Util.mk_conj_l _149_1112))
in (
...
let im = (let _149_1127 = (h gs_xs)
in (let _149_1126 = (let _149_1125 = (FStar_All.pipe_right (FStar_Syntax_Util.lcomp_of_comp c) (fun _149_1123 -> FStar_Util.Inl (_149_1123)))
in (FStar_All.pipe_right _149_1125 (fun _149_1124 -> Some (_149_1124))))
in (FStar_Syntax_Util.abs xs _149_1127 _149_1126)))
in (

let _55_2147 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1134 = (FStar_Syntax_Print.binders_to_string ", " xs)
in (let _149_1133 = (FStar_Syntax_Print.comp_to_string c)
in (let _149_1132 = (FStar_Syntax_Print.term_to_string im)
in (let _149_1131 = (FStar_Syntax_Print.tag_of_term im)
in (let _149_1130 = (let _149_1128 = (FStar_List.map (prob_to_string env) sub_probs)
in (FStar_All.pipe_right _149_1128 (FStar_String.concat ", ")))
in (let _149_1129 = (FStar_TypeChecker_Normalize.term_to_string env formula)
in (FStar_Util.print6 "Imitating  binders are {%s}, comp=%s\n\t%s (%s)\nsub_probs = %s\nformula=%s\n" _149_1134 _149_1133 _149_1132 _149_1131 _149_1130 _149_1129)))))))
end else begin
()
end
in (
...
| (gi_xs', gi_ps') -> begin
(let _149_1164 = (let _149_1161 = (FStar_Syntax_Syntax.as_arg gi_xs)
in (_149_1161)::gi_xs')
in (let _149_1163 = (let _149_1162 = (FStar_Syntax_Syntax.as_arg gi_ps)
in (_149_1162)::gi_ps')
in ((_149_1164), (_149_1163))))
end)))))
end)))
end))
in (aux [] bs))
end)))
in if (let _149_1165 = (matches pi)
in (FStar_All.pipe_left Prims.op_Negation _149_1165)) then begin
None
end else begin
(
...
let proj = (let _149_1170 = (FStar_Syntax_Syntax.mk_Tm_app xi g_xs None r)
in (let _149_1169 = (let _149_1168 = (FStar_All.pipe_right (FStar_Syntax_Util.lcomp_of_comp c) (fun _149_1166 -> FStar_Util.Inl (_149_1166)))
in (FStar_All.pipe_right _149_1168 (fun _149_1167 -> Some (_149_1167))))
in (FStar_Syntax_Util.abs xs _149_1170 _149_1169)))
in (

let sub = (let _149_1176 = (let _149_1175 = (FStar_Syntax_Syntax.mk_Tm_app proj ps None r)
in (let _149_1174 = (let _149_1173 = (FStar_List.map (fun _55_2219 -> (match (_55_2219) with
| (_55_2215, _55_2217, y) -> begin
y
end)) qs)
in (FStar_All.pipe_left h _149_1173))
in (mk_problem (p_scope orig) orig _149_1175 (p_rel orig) _149_1174 None "projection")))
in (FStar_All.pipe_left (fun _149_1171 -> FStar_TypeChecker_Common.TProb (_149_1171)) _149_1176))
in (

let _55_2221 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1178 = (FStar_Syntax_Print.term_to_string proj)
in (let _149_1177 = (prob_to_string env sub)
...
let wl = (let _149_1180 = (let _149_1179 = (FStar_All.pipe_left Prims.fst (p_guard sub))
in Some (_149_1179))
in (solve_prob orig _149_1180 ((TERM (((u), (proj))))::[]) wl))
in (let _149_1182 = (solve env (attempt ((sub)::[]) wl))
in (FStar_All.pipe_left (fun _149_1181 -> Some (_149_1181)) _149_1182)))))))
end))
end)
end))
end)))
...
(let _149_1206 = (let _149_1205 = (decompose env t2)
in ((((((uv), (k_uv))), (xs), (c))), (ps), (_149_1205)))
in Some (_149_1206))
end else begin
(
...
let rec elim = (fun k args -> (match ((let _149_1212 = (let _149_1211 = (FStar_Syntax_Subst.compress k)
in _149_1211.FStar_Syntax_Syntax.n)
in ((_149_1212), (args)))) with
| (_55_2247, []) -> begin
(let _149_1214 = (let _149_1213 = (FStar_Syntax_Syntax.mk_Total k)
in (([]), (_149_1213)))
in Some (_149_1214))
end
...
| (uv, uv_args) -> begin
(match ((let _149_1215 = (FStar_Syntax_Subst.compress uv)
in _149_1215.FStar_Syntax_Syntax.n)) with
...
let xs = (FStar_All.pipe_right args (FStar_List.map (fun _55_2273 -> (let _149_1221 = (let _149_1220 = (let _149_1219 = (let _149_1218 = (let _149_1217 = (FStar_Syntax_Util.type_u ())
in (FStar_All.pipe_right _149_1217 Prims.fst))
in (new_uvar k.FStar_Syntax_Syntax.pos scope _149_1218))
in (Prims.fst _149_1219))
in (FStar_Syntax_Syntax.new_bv (Some (k.FStar_Syntax_Syntax.pos)) _149_1220))
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_binder _149_1221)))))
in (

let c = (let _149_1225 = (let _149_1224 = (let _149_1223 = (let _149_1222 = (FStar_Syntax_Util.type_u ())
in (FStar_All.pipe_right _149_1222 Prims.fst))
in (new_uvar k.FStar_Syntax_Syntax.pos scope _149_1223))
in (Prims.fst _149_1224))
in (FStar_Syntax_Syntax.mk_Total _149_1225))
in (

let k' = (FStar_Syntax_Util.arrow xs c)
in (

let uv_sol = (let _149_1231 = (let _149_1230 = (let _149_1229 = (let _149_1228 = (let _149_1227 = (let _149_1226 = (FStar_Syntax_Util.type_u ())
in (FStar_All.pipe_right _149_1226 Prims.fst))
in (FStar_Syntax_Syntax.mk_Total _149_1227))
in (FStar_All.pipe_left FStar_Syntax_Util.lcomp_of_comp _149_1228))
in FStar_Util.Inl (_149_1229))
in Some (_149_1230))
in (FStar_Syntax_Util.abs scope k' _149_1231))
in (
...
(let _149_1233 = (FStar_Syntax_Subst.open_comp xs c)
in (FStar_All.pipe_right _149_1233 (fun _149_1232 -> Some (_149_1232))))
end else begin
...
| (xs, c) -> begin
(let _149_1235 = (elim (FStar_Syntax_Util.comp_result c) rest)
in (FStar_Util.bind_opt _149_1235 (fun _55_2300 -> (match (_55_2300) with
| (xs', c) -> begin
...
in (let _149_1238 = (let _149_1236 = (FStar_Syntax_Syntax.mk_Total t)
in (FStar_Syntax_Subst.open_comp xs _149_1236))
in (FStar_All.pipe_right _149_1238 (fun _149_1237 -> Some (_149_1237)))))
end))
end
end))
end
| _55_2306 -> begin
(let _149_1242 = (let _149_1241 = (FStar_Syntax_Print.uvar_to_string uv)
in (let _149_1240 = (FStar_Syntax_Print.term_to_string k)
in (let _149_1239 = (FStar_Syntax_Print.term_to_string k_uv)
in (FStar_Util.format3 "Impossible: ill-typed application %s : %s\n\t%s" _149_1241 _149_1240 _149_1239))))
in (FStar_All.failwith _149_1242))
end))
in (let _149_1280 = (elim k_uv ps)
in (FStar_Util.bind_opt _149_1280 (fun _55_2309 -> (match (_55_2309) with
| (xs, c) -> begin
(let _149_1279 = (let _149_1278 = (decompose env t2)
in ((((((uv), (k_uv))), (xs), (c))), (ps), (_149_1278)))
in Some (_149_1279))
end))))))
end
end)))
...
(let _149_1291 = (names_to_string fvs_hd)
in (FStar_Util.print1 "Free variables are %s" _149_1291))
end else begin
()
end
in false)
...
let fvs_hd = (let _149_1295 = (let _149_1294 = (FStar_Syntax_Util.head_and_args t2)
in (FStar_All.pipe_right _149_1294 Prims.fst))
in (FStar_All.pipe_right _149_1295 FStar_Syntax_Free.names))
...
(let _149_1297 = (let _149_1296 = (FStar_Option.get msg)
in (Prims.strcat "occurs-check failed: " _149_1296))
in (giveup_or_defer orig _149_1297))
end else begin
...
(let _149_1298 = (subterms args_lhs)
in (imitate' orig env wl _149_1298))
end else begin
(

let _55_2364 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1301 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1300 = (names_to_string fvs1)
in (let _149_1299 = (names_to_string fvs2)
in (FStar_Util.print3 "Pattern %s with fvars=%s succeeded fvar check: %s\n" _149_1301 _149_1300 _149_1299))))
end else begin
()
end
in (
...
| _55_2368 -> begin
(let _149_1302 = (sn_binders env vars)
in (u_abs k_uv _149_1302 t2))
end)
in (
...
(let _149_1305 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1304 = (names_to_string fvs1)
in (let _149_1303 = (names_to_string fvs2)
in (FStar_Util.print3 "Pattern %s with fvars=%s failed fvar check: %s ... imitating\n" _149_1305 _149_1304 _149_1303))))
end else begin
()
end
in (let _149_1306 = (subterms args_lhs)
in (imitate_or_project (FStar_List.length args_lhs) _149_1306 (~- ((Prims.parse_int "1"))))))
end else begin
...
if (let _149_1307 = (FStar_Syntax_Free.names t1)
in (check_head _149_1307 t2)) then begin
(
...
(let _149_1308 = (FStar_Syntax_Print.term_to_string t1)
in (FStar_Util.print2 "Not a pattern (%s) ... %s\n" _149_1308 (if (im_ok < (Prims.parse_int "0")) then begin
"imitating"
end else begin
"projecting"
end)))
...
in (let _149_1309 = (subterms args_lhs)
in (imitate_or_project (FStar_List.length args_lhs) _149_1309 im_ok))))
end else begin
...
| (x, imp) -> begin
(let _149_1331 = (FStar_Syntax_Syntax.bv_to_name x)
in ((_149_1331), (imp)))
end))))
in (
...
| (t, _55_2411) -> begin
(let _149_1332 = (new_uvar t.FStar_Syntax_Syntax.pos pattern_vars t)
in (Prims.fst _149_1332))
end))
in (
...
let sol = (let _149_1334 = (let _149_1333 = (u_abs k all_formals t')
in ((((u), (k))), (_149_1333)))
in TERM (_149_1334))
in (
...
(let _149_1336 = (FStar_Util.set_add (Prims.fst formal) pattern_var_set)
in (aux ((y)::pat_args) ((formal)::pattern_vars) _149_1336 formals tl))
end)
end)
end)
end
...
in (let _149_1337 = (FStar_Syntax_Syntax.new_bv_set ())
in (aux [] [] _149_1337 all_formals args))))
end))
end))
in (
...
(let _149_1346 = (solve_prob orig None [] wl)
in (solve env _149_1346))
end else begin
(
...
(let _149_1351 = (FStar_Syntax_Print.binders_to_string ", " xs)
in (let _149_1350 = (FStar_Syntax_Print.binders_to_string ", " ys)
in (let _149_1349 = (FStar_Syntax_Print.binders_to_string ", " zs)
in (let _149_1348 = (FStar_Syntax_Print.term_to_string k1)
in (let _149_1347 = (FStar_Syntax_Print.term_to_string k2)
in (FStar_Util.print5 "Flex-flex patterns: intersected %s and %s; got %s\n\tk1=%s\n\tk2=%s\n" _149_1351 _149_1350 _149_1349 _149_1348 _149_1347))))))
end else begin
()
end
in (
...
(let _149_1361 = (let _149_1360 = (FStar_Syntax_Print.term_to_string k)
in (let _149_1359 = (FStar_Syntax_Print.binders_to_string ", " xs)
in (let _149_1358 = (FStar_Syntax_Print.args_to_string args)
in (FStar_Util.format3 "k=%s\nxs=%s\nargs=%s\nIll-formed substitutution" _149_1360 _149_1359 _149_1358))))
in (FStar_All.failwith _149_1361))
end)))
in (

let _55_2505 = (

let _55_2473 = (let _149_1362 = (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Beta)::[]) env k1)
in (FStar_Syntax_Util.arrow_formals _149_1362))
in (match (_55_2473) with
| (bs, k1') -> begin
(

let _55_2476 = (let _149_1363 = (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Beta)::[]) env k2)
in (FStar_Syntax_Util.arrow_formals _149_1363))
in (match (_55_2476) with
| (cs, k2') -> begin
(

let k1'_xs = (let _149_1364 = (subst_of_list k1 bs args1)
in (FStar_Syntax_Subst.subst _149_1364 k1'))
in (

let k2'_ys = (let _149_1365 = (subst_of_list k2 cs args2)
in (FStar_Syntax_Subst.subst _149_1365 k2'))
in (

let sub_prob = (let _149_1367 = (mk_problem (p_scope orig) orig k1'_xs FStar_TypeChecker_Common.EQ k2'_ys None "flex-flex kinding")
in (FStar_All.pipe_left (fun _149_1366 -> FStar_TypeChecker_Common.TProb (_149_1366)) _149_1367))
in (match ((let _149_1371 = (let _149_1368 = (FStar_Syntax_Subst.compress k1')
in _149_1368.FStar_Syntax_Syntax.n)
in (let _149_1370 = (let _149_1369 = (FStar_Syntax_Subst.compress k2')
in _149_1369.FStar_Syntax_Syntax.n)
in ((_149_1371), (_149_1370))))) with
...
let kprob = (let _149_1373 = (mk_problem (p_scope orig) orig k1'_xs FStar_TypeChecker_Common.EQ k_zs None "flex-flex kinding")
in (FStar_All.pipe_left (fun _149_1372 -> FStar_TypeChecker_Common.TProb (_149_1372)) _149_1373))
...
let _55_2509 = (let _149_1379 = (let _149_1374 = (new_uvar r zs k_u')
in (FStar_All.pipe_left Prims.fst _149_1374))
in (let _149_1378 = (let _149_1375 = (FStar_Syntax_Syntax.mk_Total k_u')
in (FStar_Syntax_Util.arrow xs _149_1375))
in (let _149_1377 = (let _149_1376 = (FStar_Syntax_Syntax.mk_Total k_u')
in (FStar_Syntax_Util.arrow ys _149_1376))
in ((_149_1379), (_149_1378), (_149_1377)))))
...
(let _149_1385 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1384 = (FStar_Syntax_Print.term_to_string t2)
...
(let _149_1390 = (let _149_1388 = (FStar_Syntax_Syntax.bv_to_name a)
in (mk_problem (p_scope orig) orig _149_1388 FStar_TypeChecker_Common.EQ t2 None "flex-flex index"))
in (FStar_All.pipe_right _149_1390 (fun _149_1389 -> FStar_TypeChecker_Common.TProb (_149_1389))))
end)) xs args2)
in (

let guard = (let _149_1392 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
in (FStar_Syntax_Util.mk_conj_l _149_1392))
in (
...
let sol = (let _149_1394 = (let _149_1393 = (u_abs k1 xs t2)
in ((((u1), (k1))), (_149_1393)))
in TERM (_149_1394))
in (
...
(let _149_1395 = (uvi_to_string env sol)
...
(let _149_1396 = (uvi_to_string env sol)
...
(let _149_1397 = (solve_prob orig None [] wl)
in (solve env _149_1397))
end else begin
(
...
(let _149_1398 = (solve_prob orig None [] wl)
in (solve env _149_1398))
end else begin
(

let _55_2613 = if (FStar_TypeChecker_Env.debug env (FStar_Options.Other ("RelCheck"))) then begin
(let _149_1399 = (FStar_Util.string_of_int problem.FStar_TypeChecker_Common.pid)
...
(let _149_1433 = (let _149_1430 = (mk_cod1 [])
in ((bs1), (_149_1430)))
in (let _149_1432 = (let _149_1431 = (mk_cod2 [])
in ((bs2), (_149_1431)))
in ((_149_1433), (_149_1432))))
end else begin
if (l1 > l2) then begin
(let _149_1436 = (curry l2 bs1 mk_cod1)
in (let _149_1435 = (let _149_1434 = (mk_cod2 [])
in ((bs2), (_149_1434)))
in ((_149_1436), (_149_1435))))
end else begin
(let _149_1439 = (let _149_1437 = (mk_cod1 [])
in ((bs1), (_149_1437)))
in (let _149_1438 = (curry l1 bs2 mk_cod2)
in ((_149_1439), (_149_1438))))
end
end)))
end))
in (match (((t1.FStar_Syntax_Syntax.n), (t2.FStar_Syntax_Syntax.n))) with
...
| bs -> begin
(let _149_1444 = (FStar_Syntax_Syntax.mk (FStar_Syntax_Syntax.Tm_arrow (((bs), (c)))) None c.FStar_Syntax_Syntax.pos)
in (FStar_Syntax_Syntax.mk_Total _149_1444))
end))
in (
...
in (let _149_1451 = (mk_problem scope orig c1 rel c2 None "function co-domain")
in (FStar_All.pipe_left (fun _149_1450 -> FStar_TypeChecker_Common.CProb (_149_1450)) _149_1451)))))))
end)))
end
...
(solve_binders env bs1 bs2 orig wl (fun scope env subst -> (let _149_1466 = (let _149_1465 = (FStar_Syntax_Subst.subst subst tbody1)
in (let _149_1464 = (FStar_Syntax_Subst.subst subst tbody2)
in (mk_problem scope orig _149_1465 problem.FStar_TypeChecker_Common.relation _149_1464 None "lambda co-domain")))
in (FStar_All.pipe_left (fun _149_1463 -> FStar_TypeChecker_Common.TProb (_149_1463)) _149_1466))))
end)))
end
...
in (match ((let _149_1472 = (maybe_eta t1)
in (let _149_1471 = (maybe_eta t2)
in ((_149_1472), (_149_1471))))) with
...
(let _149_1473 = (destruct_flex_pattern env not_abs)
in (solve_t_flex_rigid true orig _149_1473 t_abs wl))
end else begin
...
let base_prob = (let _149_1475 = (mk_problem (p_scope orig) orig x1.FStar_Syntax_Syntax.sort problem.FStar_TypeChecker_Common.relation x2.FStar_Syntax_Syntax.sort problem.FStar_TypeChecker_Common.element "refinement base type")
in (FStar_All.pipe_left (fun _149_1474 -> FStar_TypeChecker_Common.TProb (_149_1474)) _149_1475))
in (
...
let mk_imp = (fun imp phi1 phi2 -> (let _149_1492 = (imp phi1 phi2)
in (FStar_All.pipe_right _149_1492 (guard_on_element problem x1))))
in (
...
let guard = (let _149_1495 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Syntax_Util.mk_conj _149_1495 impl))
in (
...
let ref_prob = (let _149_1499 = (let _149_1498 = (let _149_1497 = (FStar_Syntax_Syntax.mk_binder x1)
in (_149_1497)::(p_scope orig))
in (mk_problem _149_1498 orig phi1 FStar_TypeChecker_Common.EQ phi2 None "refinement formula"))
in (FStar_All.pipe_left (fun _149_1496 -> FStar_TypeChecker_Common.TProb (_149_1496)) _149_1499))
in (match ((solve env (
...
let guard = (let _149_1502 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (let _149_1501 = (let _149_1500 = (FStar_All.pipe_right (p_guard ref_prob) Prims.fst)
in (FStar_All.pipe_right _149_1500 (guard_on_element problem x1)))
in (FStar_Syntax_Util.mk_conj _149_1502 _149_1501)))
in (
...
(let _149_1504 = (destruct_flex_t t1)
in (let _149_1503 = (destruct_flex_t t2)
in (flex_flex orig _149_1504 _149_1503)))
end
...
(let _149_1505 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid false orig _149_1505 t2 wl))
end
...
in if (let _149_1506 = (is_top_level_prob orig)
in (FStar_All.pipe_left Prims.op_Negation _149_1506)) then begin
(let _149_1509 = (FStar_All.pipe_left (fun _149_1507 -> FStar_TypeChecker_Common.TProb (_149_1507)) (
...
in (let _149_1508 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid false _149_1509 _149_1508 t2 wl)))
end else begin
(
...
(match (ref_opt) with
| None -> begin
(let _149_1512 = (FStar_All.pipe_left (fun _149_1510 -> FStar_TypeChecker_Common.TProb (_149_1510)) (
...
in (let _149_1511 = (destruct_flex_pattern env t1)
in (solve_t_flex_rigid false _149_1512 _149_1511 t_base wl)))
end
| Some (y, phi) -> begin
(
...
let base_prob = (let _149_1514 = (mk_problem problem.FStar_TypeChecker_Common.scope orig t1 new_rel y.FStar_Syntax_Syntax.sort problem.FStar_TypeChecker_Common.element "flex-rigid: base type")
in (FStar_All.pipe_left (fun _149_1513 -> FStar_TypeChecker_Common.TProb (_149_1513)) _149_1514))
in (

let guard = (let _149_1515 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Syntax_Util.mk_conj _149_1515 impl))
in (
...
let t2 = (let _149_1516 = (base_and_refinement env wl t2)
in (FStar_All.pipe_left force_refinement _149_1516))
in (solve_t env (
...
let t1 = (let _149_1517 = (base_and_refinement env wl t1)
in (FStar_All.pipe_left force_refinement _149_1517))
in (solve_t env (
...
let head1 = (let _149_1518 = (FStar_Syntax_Util.head_and_args t1)
in (FStar_All.pipe_right _149_1518 Prims.fst))
in (

let head2 = (let _149_1519 = (FStar_Syntax_Util.head_and_args t2)
in (FStar_All.pipe_right _149_1519 Prims.fst))
...
(let _149_1521 = (FStar_Syntax_Util.mk_eq FStar_Syntax_Syntax.tun FStar_Syntax_Syntax.tun t1 t2)
in (FStar_All.pipe_left (fun _149_1520 -> Some (_149_1520)) _149_1521))
end
in (let _149_1522 = (solve_prob orig guard [] wl)
in (solve env _149_1522)))
end else begin
...
(let _149_1525 = (let _149_1524 = (FStar_Syntax_Print.tag_of_term t1)
in (let _149_1523 = (FStar_Syntax_Print.tag_of_term t2)
in (FStar_Util.format2 "Impossible: %s and %s" _149_1524 _149_1523)))
in (FStar_All.failwith _149_1525))
end
| _55_3184 -> begin
...
(let _149_1540 = (sub_prob a1 FStar_TypeChecker_Common.EQ a2 "effect arg")
in (FStar_All.pipe_left (fun _149_1539 -> FStar_TypeChecker_Common.TProb (_149_1539)) _149_1540))
end)) c1_comp.FStar_Syntax_Syntax.effect_args c2_comp.FStar_Syntax_Syntax.effect_args)
in (

let guard = (let _149_1542 = (FStar_List.map (fun p -> (FStar_All.pipe_right (p_guard p) Prims.fst)) sub_probs)
in (FStar_Syntax_Util.mk_conj_l _149_1542))
in (
...
| _55_3226 -> begin
(let _149_1550 = (let _149_1549 = (FStar_Range.string_of_range (FStar_Ident.range_of_lid c1.FStar_Syntax_Syntax.effect_name))
in (FStar_Util.format1 "Unexpected number of indices on a normalized effect (%s)" _149_1549))
in (FStar_All.failwith _149_1550))
end)
in (

let c1 = (let _149_1553 = (let _149_1552 = (let _149_1551 = (edge.FStar_TypeChecker_Env.mlift c1.FStar_Syntax_Syntax.result_typ wp)
in (FStar_Syntax_Syntax.as_arg _149_1551))
in (_149_1552)::[])
in {FStar_Syntax_Syntax.comp_univs = c1.FStar_Syntax_Syntax.comp_univs; FStar_Syntax_Syntax.effect_name = c2.FStar_Syntax_Syntax.effect_name; FStar_Syntax_Syntax.result_typ = c1.FStar_Syntax_Syntax.result_typ; FStar_Syntax_Syntax.effect_args = _149_1553; FStar_Syntax_Syntax.flags = c1.FStar_Syntax_Syntax.flags})
in (solve_eq c1 c2)))
end else begin
(
...
| _55_3252 -> begin
(let _149_1557 = (let _149_1556 = (FStar_Syntax_Print.lid_to_string c1.FStar_Syntax_Syntax.effect_name)
in (let _149_1555 = (FStar_Syntax_Print.lid_to_string c2.FStar_Syntax_Syntax.effect_name)
in (FStar_Util.format2 "Got effects %s and %s, expected normalized effects" _149_1556 _149_1555)))
in (FStar_All.failwith _149_1557))
end)
in (match (_55_3255) with
| (wpc1, wpc2) -> begin
if (FStar_Util.physical_equality wpc1 wpc2) then begin
(let _149_1558 = (problem_using_guard orig c1.FStar_Syntax_Syntax.result_typ problem.FStar_TypeChecker_Common.relation c2.FStar_Syntax_Syntax.result_typ None "result type")
in (solve_t env _149_1558 wl))
end else begin
(
...
in (let _149_1568 = (let _149_1567 = (let _149_1566 = (let _149_1560 = (let _149_1559 = (env.FStar_TypeChecker_Env.universe_of env c1.FStar_Syntax_Syntax.result_typ)
in (_149_1559)::[])
in (FStar_TypeChecker_Env.inst_effect_fun_with _149_1560 env c2_decl c2_decl.FStar_Syntax_Syntax.trivial))
in (let _149_1565 = (let _149_1564 = (FStar_Syntax_Syntax.as_arg c1.FStar_Syntax_Syntax.result_typ)
in (let _149_1563 = (let _149_1562 = (let _149_1561 = (edge.FStar_TypeChecker_Env.mlift c1.FStar_Syntax_Syntax.result_typ wpc1)
in (FStar_All.pipe_left FStar_Syntax_Syntax.as_arg _149_1561))
in (_149_1562)::[])
in (_149_1564)::_149_1563))
in ((_149_1566), (_149_1565))))
in FStar_Syntax_Syntax.Tm_app (_149_1567))
in (FStar_Syntax_Syntax.mk _149_1568 (Some (FStar_Syntax_Util.ktype0.FStar_Syntax_Syntax.n)) r)))
end else begin
(let _149_1580 = (let _149_1579 = (let _149_1578 = (let _149_1570 = (let _149_1569 = (env.FStar_TypeChecker_Env.universe_of env c2.FStar_Syntax_Syntax.result_typ)
in (_149_1569)::[])
in (FStar_TypeChecker_Env.inst_effect_fun_with _149_1570 env c2_decl c2_decl.FStar_Syntax_Syntax.stronger))
in (let _149_1577 = (let _149_1576 = (FStar_Syntax_Syntax.as_arg c2.FStar_Syntax_Syntax.result_typ)
in (let _149_1575 = (let _149_1574 = (FStar_Syntax_Syntax.as_arg wpc2)
in (let _149_1573 = (let _149_1572 = (let _149_1571 = (edge.FStar_TypeChecker_Env.mlift c1.FStar_Syntax_Syntax.result_typ wpc1)
in (FStar_All.pipe_left FStar_Syntax_Syntax.as_arg _149_1571))
in (_149_1572)::[])
in (_149_1574)::_149_1573))
in (_149_1576)::_149_1575))
in ((_149_1578), (_149_1577))))
in FStar_Syntax_Syntax.Tm_app (_149_1579))
in (FStar_Syntax_Syntax.mk _149_1580 (Some (FStar_Syntax_Util.ktype0.FStar_Syntax_Syntax.n)) r))
end
in (

let base_prob = (let _149_1582 = (sub_prob c1.FStar_Syntax_Syntax.result_typ problem.FStar_TypeChecker_Common.relation c2.FStar_Syntax_Syntax.result_typ "result type")
in (FStar_All.pipe_left (fun _149_1581 -> FStar_TypeChecker_Common.TProb (_149_1581)) _149_1582))
in (

let wl = (let _149_1586 = (let _149_1585 = (let _149_1584 = (FStar_All.pipe_right (p_guard base_prob) Prims.fst)
in (FStar_Syntax_Util.mk_conj _149_1584 g))
in (FStar_All.pipe_left (fun _149_1583 -> Some (_149_1583)) _149_1585))
in (solve_prob orig _149_1586 [] wl))
in (solve env (attempt ((base_prob)::[]) wl))))))
end
end)))
end))
in if (FStar_Util.physical_equality c1 c2) then begin
(let _149_1587 = (solve_prob orig None [] wl)
in (solve env _149_1587))
end else begin
(

let _55_3262 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1589 = (FStar_Syntax_Print.comp_to_string c1)
in (let _149_1588 = (FStar_Syntax_Print.comp_to_string c2)
in (FStar_Util.print3 "solve_c %s %s %s\n" _149_1589 (rel_to_string problem.FStar_TypeChecker_Common.relation) _149_1588)))
end else begin
()
end
in (

let _55_3266 = (let _149_1591 = (FStar_TypeChecker_Normalize.ghost_to_pure env c1)
in (let _149_1590 = (FStar_TypeChecker_Normalize.ghost_to_pure env c2)
in ((_149_1591), (_149_1590))))
...
(let _149_1592 = (problem_using_guard orig t1 problem.FStar_TypeChecker_Common.relation t2 None "result type")
in (solve_t env _149_1592 wl))
end
...
(let _149_1593 = (problem_using_guard orig t1 problem.FStar_TypeChecker_Common.relation t2 None "result type")
in (solve_t env _149_1593 wl))
end
...
in (let _149_1598 = (let _149_1597 = (FStar_TypeChecker_Normalize.comp_to_comp_typ env c2)
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_Comp _149_1597))
in {FStar_TypeChecker_Common.pid = _55_3344.FStar_TypeChecker_Common.pid; FStar_TypeChecker_Common.lhs = _55_3344.FStar_TypeChecker_Common.lhs; FStar_TypeChecker_Common.relation = _55_3344.FStar_TypeChecker_Common.relation; FStar_TypeChecker_Common.rhs = _149_1598; FStar_TypeChecker_Common.element = _55_3344.FStar_TypeChecker_Common.element; FStar_TypeChecker_Common.logical_guard = _55_3344.FStar_TypeChecker_Common.logical_guard; FStar_TypeChecker_Common.scope = _55_3344.FStar_TypeChecker_Common.scope; FStar_TypeChecker_Common.reason = _55_3344.FStar_TypeChecker_Common.reason; FStar_TypeChecker_Common.loc = _55_3344.FStar_TypeChecker_Common.loc; FStar_TypeChecker_Common.rank = _55_3344.FStar_TypeChecker_Common.rank}))
in (solve_c env _149_1599 wl))
end
...
(let _149_1600 = (problem_using_guard orig (FStar_Syntax_Util.comp_result c1) problem.FStar_TypeChecker_Common.relation (FStar_Syntax_Util.comp_result c2) None "result type")
in (solve_t env _149_1600 wl))
end else begin
(
...
| None -> begin
if (((FStar_Syntax_Util.is_ghost_effect c1.FStar_Syntax_Syntax.effect_name) && (FStar_Syntax_Util.is_pure_effect c2.FStar_Syntax_Syntax.effect_name)) && (let _149_1601 = (FStar_TypeChecker_Normalize.normalize ((FStar_TypeChecker_Normalize.Inline)::(FStar_TypeChecker_Normalize.UnfoldUntil (FStar_Syntax_Syntax.Delta_constant))::[]) env c2.FStar_Syntax_Syntax.result_typ)
in (FStar_Syntax_Util.non_informative _149_1601))) then begin
(
...
(let _149_1606 = (let _149_1605 = (FStar_Syntax_Print.lid_to_string c1.FStar_Syntax_Syntax.effect_name)
in (let _149_1604 = (FStar_Syntax_Print.lid_to_string c2.FStar_Syntax_Syntax.effect_name)
in (FStar_Util.format2 "incompatible monad ordering: %s </: %s" _149_1605 _149_1604)))
in (giveup env _149_1606 orig))
end
end
| Some (edge) -> begin
...
let print_pending_implicits : FStar_TypeChecker_Env.guard_t  ->  Prims.string = (fun g -> (let _149_1610 = (FStar_All.pipe_right g.FStar_TypeChecker_Env.implicits (FStar_List.map (fun _55_3377 -> (match (_55_3377) with
| (_55_3367, _55_3369, u, _55_3372, _55_3374, _55_3376) -> begin
(FStar_Syntax_Print.uvar_to_string u)
end))))
in (FStar_All.pipe_right _149_1610 (FStar_String.concat ", "))))
...
let carry = (let _149_1616 = (FStar_List.map (fun _55_3392 -> (match (_55_3392) with
| (_55_3390, x) -> begin
(prob_to_string env x)
end)) g.FStar_TypeChecker_Env.deferred)
in (FStar_All.pipe_right _149_1616 (FStar_String.concat ",\n")))
in (
...
let _55_3426 = g
in (let _149_1636 = (let _149_1635 = (let _149_1634 = (let _149_1628 = (FStar_Syntax_Syntax.mk_binder x)
in (_149_1628)::[])
in (let _149_1633 = (let _149_1632 = (let _149_1631 = (let _149_1629 = (FStar_Syntax_Syntax.mk_Total FStar_Syntax_Util.ktype0)
in (FStar_All.pipe_right _149_1629 FStar_Syntax_Util.lcomp_of_comp))
in (FStar_All.pipe_right _149_1631 (fun _149_1630 -> FStar_Util.Inl (_149_1630))))
in Some (_149_1632))
in (FStar_Syntax_Util.abs _149_1634 f _149_1633)))
in (FStar_All.pipe_left (fun _149_1627 -> FStar_TypeChecker_Common.NonTrivial (_149_1627)) _149_1635))
in {FStar_TypeChecker_Env.guard_f = _149_1636; FStar_TypeChecker_Env.deferred = _55_3426.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _55_3426.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _55_3426.FStar_TypeChecker_Env.implicits}))
in Some (_149_1637)))
end))
...
let _55_3433 = g
in (let _149_1648 = (let _149_1647 = (let _149_1646 = (let _149_1645 = (let _149_1644 = (let _149_1643 = (FStar_Syntax_Syntax.as_arg e)
in (_149_1643)::[])
in ((f), (_149_1644)))
in FStar_Syntax_Syntax.Tm_app (_149_1645))
in (FStar_Syntax_Syntax.mk _149_1646 (Some (FStar_Syntax_Util.ktype0.FStar_Syntax_Syntax.n)) f.FStar_Syntax_Syntax.pos))
in (FStar_All.pipe_left (fun _149_1642 -> FStar_TypeChecker_Common.NonTrivial (_149_1642)) _149_1647))
in {FStar_TypeChecker_Env.guard_f = _149_1648; FStar_TypeChecker_Env.deferred = _55_3433.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _55_3433.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _55_3433.FStar_TypeChecker_Env.implicits}))
end))
...
(let _149_1655 = (FStar_Syntax_Util.mk_conj f1 f2)
in FStar_TypeChecker_Common.NonTrivial (_149_1655))
end))
...
let _55_3483 = g
in (let _149_1693 = (let _149_1692 = (FStar_Syntax_Util.close_forall binders f)
in (FStar_All.pipe_right _149_1692 (fun _149_1691 -> FStar_TypeChecker_Common.NonTrivial (_149_1691))))
in {FStar_TypeChecker_Env.guard_f = _149_1693; FStar_TypeChecker_Env.deferred = _55_3483.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _55_3483.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _55_3483.FStar_TypeChecker_Env.implicits}))
end))
...
(let _149_1701 = (FStar_TypeChecker_Normalize.term_to_string env lhs)
in (let _149_1700 = (FStar_TypeChecker_Normalize.term_to_string env rhs)
in (FStar_Util.format3 "Top-level:\n%s\n\t%s\n%s" _149_1701 (rel_to_string rel) _149_1700)))
end else begin
"TOP"
end
in (
...
let x = (let _149_1712 = (let _149_1711 = (FStar_TypeChecker_Env.get_range env)
in (FStar_All.pipe_left (fun _149_1710 -> Some (_149_1710)) _149_1711))
in (FStar_Syntax_Syntax.new_bv _149_1712 t1))
in (

let env = (FStar_TypeChecker_Env.push_bv env x)
in (

let p = (let _149_1716 = (let _149_1714 = (FStar_Syntax_Syntax.bv_to_name x)
in (FStar_All.pipe_left (fun _149_1713 -> Some (_149_1713)) _149_1714))
in (let _149_1715 = (FStar_TypeChecker_Env.get_range env)
in (new_t_problem env t1 rel t2 _149_1716 _149_1715)))
...
(let _149_1728 = (explain env d s)
in (FStar_All.pipe_left FStar_Util.print_string _149_1728))
end else begin
()
end
in (err ((d), (s)))))
...
(let _149_1733 = (FStar_Syntax_Print.term_to_string f)
...
(let _149_1734 = (FStar_Syntax_Print.term_to_string f)
...
let f = (match ((let _149_1735 = (FStar_Syntax_Util.unmeta f)
in _149_1735.FStar_Syntax_Syntax.n)) with
...
| Some (d) -> begin
(let _149_1747 = (let _149_1746 = (let _149_1745 = (let _149_1744 = (FStar_All.pipe_right (p_guard prob) Prims.fst)
in (FStar_All.pipe_right _149_1744 (fun _149_1743 -> FStar_TypeChecker_Common.NonTrivial (_149_1743))))
in {FStar_TypeChecker_Env.guard_f = _149_1745; FStar_TypeChecker_Env.deferred = d; FStar_TypeChecker_Env.univ_ineqs = []; FStar_TypeChecker_Env.implicits = []})
in (simplify_guard env _149_1746))
in (FStar_All.pipe_left (fun _149_1742 -> Some (_149_1742)) _149_1747))
end))
...
(let _149_1755 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1754 = (FStar_Syntax_Print.term_to_string t2)
...
let prob = (let _149_1758 = (let _149_1757 = (FStar_TypeChecker_Env.get_range env)
in (new_t_problem env t1 FStar_TypeChecker_Common.EQ t2 None _149_1757))
in (FStar_All.pipe_left (fun _149_1756 -> FStar_TypeChecker_Common.TProb (_149_1756)) _149_1758))
in (

let g = (let _149_1760 = (solve_and_commit env (singleton env prob) (fun _55_3549 -> None))
in (FStar_All.pipe_left (with_guard env prob) _149_1760))
in g))))
...
| None -> begin
(let _149_1770 = (let _149_1769 = (let _149_1768 = (FStar_TypeChecker_Errors.basic_type_error env None t2 t1)
in (let _149_1767 = (FStar_TypeChecker_Env.get_range env)
in ((_149_1768), (_149_1767))))
in FStar_Syntax_Syntax.Error (_149_1769))
in (Prims.raise _149_1770))
end
| Some (g) -> begin
(

let _55_3558 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _149_1773 = (FStar_Syntax_Print.term_to_string t1)
in (let _149_1772 = (FStar_Syntax_Print.term_to_string t2)
in (let _149_1771 = (guard_to_string env g)
in (FStar_Util.print3 "teq of %s and %s succeeded with guard %s\n" _149_1773 _149_1772 _149_1771))))
end else begin
()
end
in g)
...
(let _149_1783 = (FStar_TypeChecker_Normalize.term_to_string env t1)
in (let _149_1782 = (FStar_TypeChecker_Normalize.term_to_string env t2)
...
let g = (let _149_1785 = (solve_and_commit env (singleton' env prob smt_ok) (fun _55_3570 -> None))
in (FStar_All.pipe_left (with_guard env prob) _149_1785))
in (
...
(let _149_1789 = (FStar_TypeChecker_Normalize.term_to_string env t1)
in (let _149_1788 = (FStar_TypeChecker_Normalize.term_to_string env t2)
in (let _149_1787 = (let _149_1786 = (FStar_Util.must g)
in (guard_to_string env _149_1786))
in (FStar_Util.print3 "try_subtype succeeded: %s <: %s\n\tguard is %s\n" _149_1789 _149_1788 _149_1787))))
end else begin
()
end
...
let subtype_fail : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.typ  ->  FStar_Syntax_Syntax.typ  ->  Prims.unit = (fun env e t1 t2 -> (let _149_1805 = (FStar_TypeChecker_Env.get_range env)
in (let _149_1804 = (FStar_TypeChecker_Errors.basic_type_error env (Some (e)) t2 t1)
in (FStar_TypeChecker_Errors.report _149_1805 _149_1804))))
...
(let _149_1813 = (FStar_Syntax_Print.comp_to_string c1)
in (let _149_1812 = (FStar_Syntax_Print.comp_to_string c2)
...
let prob = (let _149_1816 = (let _149_1815 = (FStar_TypeChecker_Env.get_range env)
in (new_problem env c1 rel c2 None _149_1815 "sub_comp"))
in (FStar_All.pipe_left (fun _149_1814 -> FStar_TypeChecker_Common.CProb (_149_1814)) _149_1816))
in (let _149_1818 = (solve_and_commit env (singleton env prob) (fun _55_3589 -> None))
in (FStar_All.pipe_left (with_guard env prob) _149_1818))))))
...
in (let _149_1836 = (let _149_1835 = (let _149_1834 = (let _149_1832 = (FStar_Syntax_Print.univ_to_string u1)
in (let _149_1831 = (FStar_Syntax_Print.univ_to_string u2)
in (FStar_Util.format3 "Universe %s and %s are incompatible%s" _149_1832 _149_1831 msg)))
in (let _149_1833 = (FStar_TypeChecker_Env.get_range env)
in ((_149_1834), (_149_1833))))
in FStar_Syntax_Syntax.Error (_149_1835))
in (Prims.raise _149_1836)))))
in (
...
(let _149_1848 = (insert uv u1 out)
in (group_by _149_1848 rest))
end)
end
| _55_3629 -> begin
None
...
(let _149_1869 = (wl_to_string wl)
in (let _149_1868 = (FStar_Util.string_of_int (FStar_List.length g.FStar_TypeChecker_Env.implicits))
...
(let _149_1886 = (FStar_TypeChecker_Env.get_range env)
in (let _149_1885 = (let _149_1884 = (FStar_Syntax_Print.term_to_string vc)
in (FStar_Util.format1 "Before normalization VC=\n%s\n" _149_1884))
in (FStar_TypeChecker_Errors.diag _149_1886 _149_1885)))
end else begin
()
end
in (
...
(let _149_1889 = (FStar_TypeChecker_Env.get_range env)
in (let _149_1888 = (let _149_1887 = (FStar_Syntax_Print.term_to_string vc)
in (FStar_Util.format1 "Checking VC=\n%s\n" _149_1887))
in (FStar_TypeChecker_Errors.diag _149_1889 _149_1888)))
end else begin
()
end
...
(let _149_1905 = (FStar_Syntax_Print.uvar_to_string u)
in (let _149_1904 = (FStar_Syntax_Print.term_to_string tm)
in (let _149_1903 = (FStar_Syntax_Print.term_to_string k)
in (FStar_Util.print3 "Checking uvar %s resolved to %s at type %s\n" _149_1905 _149_1904 _149_1903))))
end else begin
()
end
in (
...
let _55_3781 = g
in (let _149_1909 = (until_fixpoint (([]), (false)) g.FStar_TypeChecker_Env.implicits)
in {FStar_TypeChecker_Env.guard_f = _55_3781.FStar_TypeChecker_Env.guard_f; FStar_TypeChecker_Env.deferred = _55_3781.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _55_3781.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _149_1909})))))


let force_trivial_guard : FStar_TypeChecker_Env.env  ->  FStar_TypeChecker_Env.guard_t  ->  Prims.unit = (fun env g -> (

let g = (let _149_1914 = (solve_deferred_constraints env g)
in (FStar_All.pipe_right _149_1914 resolve_implicits))
in (match (g.FStar_TypeChecker_Env.implicits) with
| [] -> begin
(let _149_1915 = (discharge_guard env g)
in (FStar_All.pipe_left Prims.ignore _149_1915))
end
| ((reason, _55_3791, _55_3793, e, t, r))::_55_3788 -> begin
(let _149_1918 = (let _149_1917 = (FStar_Syntax_Print.term_to_string t)
in (let _149_1916 = (FStar_Syntax_Print.term_to_string e)
in (FStar_Util.format3 "Failed to resolve implicit argument of type \'%s\' introduced in %s because %s" _149_1917 _149_1916 reason)))
in (FStar_TypeChecker_Errors.report r _149_1918))
end)))
