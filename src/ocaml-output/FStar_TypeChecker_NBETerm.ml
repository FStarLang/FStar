...
let (mkConstruct :
  ...
  =
  fun i ->
    fun us -> fun ts -> FStar_All.op_Less_Bar mk_t (Construct (i, us, ts))
let (mkFV :
  FStar_Syntax_Syntax.fv ->
    FStar_Syntax_Syntax.universe Prims.list -> args -> t)
  =
...
let (mkAccuMatch :
  ...
  fun s ->
    fun ret ->
      fun bs -> FStar_All.op_Less_Bar mk_t (Accu ((Match (s, ret, bs)), []))
...
let (arg_to_string : arg -> Prims.string) =
  fun a ->
    let uu___ = FStar_All.op_Bar_Greater a FStar_Pervasives_Native.fst in
    FStar_All.op_Bar_Greater uu___ t_to_string
let (args_to_string : args -> Prims.string) =
  fun args1 ->
    let uu___ = FStar_All.op_Bar_Greater args1 (FStar_List.map arg_to_string) in
  ...
  fun em ->
    fun un ->
      mk_emb
        (fun cbs ->
           fun t1 ->
             let uu___ = em cbs t1 in FStar_All.op_Less_Bar mk_t uu___)
...
let e_option :
  ...
  fun ea ->
    let etyp =
      let uu___ =
        let uu___1 =
          FStar_All.op_Bar_Greater FStar_Parser_Const.option_lid
  ...
  fun ea ->
    fun eb ->
      let etyp =
        let uu___ =
          let uu___1 =
            FStar_All.op_Bar_Greater FStar_Parser_Const.lid_tuple2
              FStar_Ident.string_of_lid in
  ...
  fun ea ->
    fun eb ->
      let etyp =
        let uu___ =
          let uu___1 =
            FStar_All.op_Bar_Greater FStar_Parser_Const.either_lid
              FStar_Ident.string_of_lid in
          (uu___1, [ea.emb_typ; eb.emb_typ]) in
        FStar_Syntax_Syntax.ET_app uu___ in
      let em cb s =
...
let e_list : 'a . 'a embedding -> 'a Prims.list embedding =
  fun ea ->
    let etyp =
      let uu___ =
        let uu___1 =
          FStar_All.op_Bar_Greater FStar_Parser_Const.list_lid
            FStar_Ident.string_of_lid in
        (uu___1, [ea.emb_typ]) in
      FStar_Syntax_Syntax.ET_app uu___ in
    let em cb l =
        ...
        lazy_embed etyp f
          (fun uu___ ->
             let uu___1 =
               ...
               Lam uu___2 in
             FStar_All.op_Less_Bar mk_t uu___1) in
      let un cb lam =
        let k lam1 =
          FStar_Pervasives_Native.Some
            (fun x ->
...
let (arg_as_int : arg -> FStar_BigInt.t FStar_Pervasives_Native.option) =
  fun a ->
    FStar_All.op_Bar_Greater (FStar_Pervasives_Native.fst a)
...
let arg_as_list :
  ...
  fun e ->
    fun a1 ->
      ...
      FStar_All.op_Bar_Greater (FStar_Pervasives_Native.fst a1) uu___
...
let (int_as_bounded : FStar_Syntax_Syntax.fv -> FStar_BigInt.t -> t) =
  fun int_to_t ->
    fun n ->
      ...
      let int_to_t1 args1 =
        FStar_All.op_Less_Bar mk_t (FV (int_to_t, [], args1)) in
