...
let _56_637 = (let _150_258 = (FStar_TypeChecker_Env.norm_eff_name env l1)
in (let _150_257 = (FStar_TypeChecker_Env.norm_eff_name env l2)
in (FStar_TypeChecker_Env.join env _150_258 _150_257)))
in (match (_56_637) with
| (m, _56_634, _56_636) -> begin
m
end)))
...
let _56_649 = (FStar_TypeChecker_Env.join env c1.FStar_Syntax_Syntax.effect_name c2.FStar_Syntax_Syntax.effect_name)
in (match (_56_649) with
| (m, lift1, lift2) -> begin
(
...
let _56_655 = (FStar_TypeChecker_Env.wp_signature env md.FStar_Syntax_Syntax.mname)
...
in {FStar_Syntax_Syntax.eff_name = _56_669.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = _150_300; FStar_Syntax_Syntax.cflags = _56_669.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = (fun _56_671 -> (match (()) with
| () -> begin
...
| FStar_Syntax_Syntax.Tm_arrow (_56_674) -> begin
true
end
| _56_677 -> begin
false
...
let _56_684 = (FStar_TypeChecker_Env.wp_signature env FStar_Syntax_Const.effect_PURE_lid)
...
let _56_689 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Return"))) then begin
...
let bind : FStar_Range.range  ->  FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term Prims.option  ->  FStar_Syntax_Syntax.lcomp  ->  lcomp_with_binder  ->  FStar_Syntax_Syntax.lcomp = (fun r1 env e1opt lc1 _56_697 -> (match (_56_697) with
| (b, lc2) -> begin
(
...
let _56_707 = if (FStar_TypeChecker_Env.debug env FStar_Options.Extreme) then begin
(
...
let bind_it = (fun _56_710 -> (match (()) with
| () -> begin
(
...
let _56_716 = if (FStar_TypeChecker_Env.debug env FStar_Options.Extreme) then begin
...
let try_simplify = (fun _56_719 -> (match (()) with
| () -> begin
(

let aux = (fun _56_721 -> (match (()) with
| () -> begin
...
let _56_759 = (lift_and_destruct env c1 c2)
in (match (_56_759) with
...
let _56_778 = (FStar_TypeChecker_Env.wp_signature env md_pure.FStar_Syntax_Syntax.mname)
in (match (_56_778) with
| (a, kwp) -> begin
(
...
in {FStar_TypeChecker_Env.guard_f = _150_411; FStar_TypeChecker_Env.deferred = _56_797.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _56_797.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _56_797.FStar_TypeChecker_Env.implicits}))
end))
...
let weaken = (fun _56_813 -> (match (()) with
| () -> begin
(
...
let _56_822 = (destruct_comp c)
in (match (_56_822) with
...
let _56_825 = lc
in {FStar_Syntax_Syntax.eff_name = _56_825.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = _56_825.FStar_Syntax_Syntax.res_typ; FStar_Syntax_Syntax.cflags = _56_825.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = weaken})))
...
let _56_832 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) FStar_Options.Extreme) then begin
...
let strengthen = (fun _56_841 -> (match (()) with
| () -> begin
(
...
let _56_851 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) FStar_Options.Extreme) then begin
...
let _56_857 = (destruct_comp c)
in (match (_56_857) with
| (u_res_t, res_t, wp) -> begin
(
...
let _56_860 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) FStar_Options.Extreme) then begin
...
in {FStar_Syntax_Syntax.eff_name = _150_470; FStar_Syntax_Syntax.res_typ = _56_863.FStar_Syntax_Syntax.res_typ; FStar_Syntax_Syntax.cflags = _150_469; FStar_Syntax_Syntax.comp = strengthen})))
in ((_150_471), ((

let _56_865 = g0
in {FStar_TypeChecker_Env.guard_f = FStar_TypeChecker_Common.Trivial; FStar_TypeChecker_Env.deferred = _56_865.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _56_865.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _56_865.FStar_TypeChecker_Env.implicits})))))))
end)
...
let _56_875 = (let _150_479 = (FStar_Syntax_Syntax.bv_to_name x)
in (let _150_478 = (FStar_Syntax_Syntax.bv_to_name y)
in ((_150_479), (_150_478))))
in (match (_56_875) with
| (xexp, yexp) -> begin
(
...
let comp = (fun _56_887 -> (match (()) with
| () -> begin
(

let _56_904 = (let _150_515 = (lcomp_then.FStar_Syntax_Syntax.comp ())
in (let _150_514 = (lcomp_else.FStar_Syntax_Syntax.comp ())
in (lift_and_destruct env _150_515 _150_514)))
in (match (_56_904) with
| ((md, _56_890, _56_892), (u_res_t, res_t, wp_then), (_56_899, _56_901, wp_else)) -> begin
(
...
let eff = (FStar_List.fold_left (fun eff _56_923 -> (match (_56_923) with
| (_56_921, lc) -> begin
(join_effects env eff lc.FStar_Syntax_Syntax.eff_name)
end)) FStar_Syntax_Const.effect_PURE_lid lcases)
in (

let bind_cases = (fun _56_926 -> (match (()) with
| () -> begin
(
...
let comp = (FStar_List.fold_right (fun _56_942 celse -> (match (_56_942) with
| (g, cthen) -> begin
(

let _56_962 = (let _150_592 = (cthen.FStar_Syntax_Syntax.comp ())
in (lift_and_destruct env _150_592 celse))
in (match (_56_962) with
| ((md, _56_946, _56_948), (_56_951, _56_953, wp_then), (_56_957, _56_959, wp_else)) -> begin
...
let _56_971 = (destruct_comp comp)
in (match (_56_971) with
| (_56_967, _56_969, wp) -> begin
(
...
let close = (fun _56_977 -> (match (()) with
| () -> begin
(
...
let _56_994 = (destruct_comp c)
in (match (_56_994) with
| (u_res_t, res_t, wp) -> begin
(
...
let _56_997 = lc
in {FStar_Syntax_Syntax.eff_name = _56_997.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = _56_997.FStar_Syntax_Syntax.res_typ; FStar_Syntax_Syntax.cflags = _56_997.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = close})))


let maybe_assume_result_eq_pure_term : FStar_TypeChecker_Env.env  ->  FStar_Syntax_Syntax.term  ->  FStar_Syntax_Syntax.lcomp  ->  FStar_Syntax_Syntax.lcomp = (fun env e lc -> (

let refine = (fun _56_1003 -> (match (()) with
| () -> begin
(
...
let _56_1015 = lc
in {FStar_Syntax_Syntax.eff_name = _56_1015.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = _56_1015.FStar_Syntax_Syntax.res_typ; FStar_Syntax_Syntax.cflags = flags; FStar_Syntax_Syntax.comp = refine}))))
...
| FStar_Syntax_Syntax.Tm_type (_56_1029) -> begin
...
let _56_1033 = (FStar_TypeChecker_Env.lookup_lid env FStar_Syntax_Const.b2t_lid)
in (
...
let _56_1053 = lc
in {FStar_Syntax_Syntax.eff_name = _56_1053.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = t; FStar_Syntax_Syntax.cflags = _56_1053.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = _56_1053.FStar_Syntax_Syntax.comp})), (FStar_TypeChecker_Rel.trivial_guard)))
end
...
let _56_1060 = lc
in {FStar_Syntax_Syntax.eff_name = _56_1060.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = t; FStar_Syntax_Syntax.cflags = _56_1060.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = _56_1060.FStar_Syntax_Syntax.comp})
in ((e), (lc), (g)))
end
| FStar_TypeChecker_Common.NonTrivial (f) -> begin
(
...
let _56_1065 = g
in {FStar_TypeChecker_Env.guard_f = FStar_TypeChecker_Common.Trivial; FStar_TypeChecker_Env.deferred = _56_1065.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _56_1065.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _56_1065.FStar_TypeChecker_Env.implicits})
in (

let strengthen = (fun _56_1069 -> (match (()) with
| () -> begin
(
...
| FStar_Syntax_Syntax.Tm_abs (_56_1072, {FStar_Syntax_Syntax.n = FStar_Syntax_Syntax.Tm_fvar (fv); FStar_Syntax_Syntax.tk = _56_1078; FStar_Syntax_Syntax.pos = _56_1076; FStar_Syntax_Syntax.vars = _56_1074}, _56_1083) when (FStar_Syntax_Syntax.fv_eq_lid fv FStar_Syntax_Const.true_lid) -> begin
(

let lc = (

let _56_1086 = lc
in {FStar_Syntax_Syntax.eff_name = _56_1086.FStar_Syntax_Syntax.eff_name; FStar_Syntax_Syntax.res_typ = t; FStar_Syntax_Syntax.cflags = _56_1086.FStar_Syntax_Syntax.cflags; FStar_Syntax_Syntax.comp = _56_1086.FStar_Syntax_Syntax.comp})
in (lc.FStar_Syntax_Syntax.comp ()))
end
| _56_1090 -> begin
(
...
let _56_1092 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) FStar_Options.Extreme) then begin
...
let _56_1097 = (FStar_TypeChecker_Env.wp_signature env FStar_Syntax_Const.effect_PURE_lid)
in (match (_56_1097) with
| (a, kwp) -> begin
(
...
let _56_1107 = (destruct_comp ct)
in (match (_56_1107) with
| (u_t, _56_1104, _56_1106) -> begin
(
...
let _56_1113 = (let _150_704 = (FStar_All.pipe_left (fun _150_701 -> Some (_150_701)) (FStar_TypeChecker_Errors.subtyping_failed env lc.FStar_Syntax_Syntax.res_typ t))
...
let _56_1114 = x
in {FStar_Syntax_Syntax.ppname = _56_1114.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _56_1114.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = lc.FStar_Syntax_Syntax.res_typ})
in (
...
let _56_1119 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) FStar_Options.Extreme) then begin
...
let _56_1130 = g
in {FStar_TypeChecker_Env.guard_f = FStar_TypeChecker_Common.Trivial; FStar_TypeChecker_Env.deferred = _56_1130.FStar_TypeChecker_Env.deferred; FStar_TypeChecker_Env.univ_ineqs = _56_1130.FStar_TypeChecker_Env.univ_ineqs; FStar_TypeChecker_Env.implicits = _56_1130.FStar_TypeChecker_Env.implicits})
in ((e), (lc), (g)))))))
end)
end)))
...
| ((req, _56_1158))::((ens, _56_1153))::_56_1150 -> begin
...
| ((wp, _56_1168))::_56_1165 -> begin
(

let _56_1174 = (FStar_TypeChecker_Env.lookup_lid env FStar_Syntax_Const.as_requires)
in (match (_56_1174) with
| (us_r, _56_1173) -> begin
(

let _56_1178 = (FStar_TypeChecker_Env.lookup_lid env FStar_Syntax_Const.as_ensures)
in (match (_56_1178) with
| (us_e, _56_1177) -> begin
(
...
let _56_1196 = (FStar_Syntax_Subst.open_comp bs c)
in (match (_56_1196) with
| (bs, c) -> begin
(
...
let _56_1212 = (new_implicit_var "Instantiation of implicit argument" e.FStar_Syntax_Syntax.pos env t)
...
let _56_1218 = (aux subst rest)
in (match (_56_1218) with
...
| (_56_1230, []) when (not ((FStar_Syntax_Util.is_total_comp c))) -> begin
((e), (torig), (FStar_TypeChecker_Rel.trivial_guard))
end
| _56_1234 -> begin
(
...
let _56_1253 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Gen"))) then begin
...
let _56_1255 = (FStar_Unionfind.change u (Some (FStar_Syntax_Syntax.U_name (u_name))))
in u_name))))))
in u_names))))
end)
...
(let _150_776 = (string_of_univs univs)
...
let _56_1265 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Gen"))) then begin
(let _150_777 = (FStar_Syntax_Print.term_to_string t)
...
let gen : FStar_TypeChecker_Env.env  ->  (FStar_Syntax_Syntax.term * FStar_Syntax_Syntax.comp) Prims.list  ->  (FStar_Syntax_Syntax.univ_name Prims.list * FStar_Syntax_Syntax.term * FStar_Syntax_Syntax.comp) Prims.list Prims.option = (fun env ecs -> if (let _150_783 = (FStar_Util.for_all (fun _56_1273 -> (match (_56_1273) with
| (_56_1271, c) -> begin
(FStar_Syntax_Util.is_pure_or_ghost_comp c)
end)) ecs)
in (FStar_All.pipe_left Prims.op_Negation _150_783)) then begin
None
end else begin
(
...
(let _150_786 = (FStar_Syntax_Print.comp_to_string c)
...
let _56_1279 = if (FStar_TypeChecker_Env.debug env FStar_Options.Medium) then begin
(let _150_787 = (FStar_Syntax_Print.comp_to_string c)
...
let gen_uvars = (fun uvs -> (let _150_790 = (FStar_Util.set_difference uvs env_uvars)
in (FStar_All.pipe_right _150_790 FStar_Util.set_elements)))
in (

let _56_1295 = (let _150_792 = (FStar_All.pipe_right ecs (FStar_List.map (fun _56_1286 -> (match (_56_1286) with
| (e, c) -> begin
(
...
in (FStar_All.pipe_right _150_792 FStar_List.unzip))
in (match (_56_1295) with
| (univs, uvars) -> begin
(
...
let _56_1299 = if (FStar_TypeChecker_Env.debug env FStar_Options.Medium) then begin
...
let ecs = (FStar_All.pipe_right uvars (FStar_List.map (fun _56_1304 -> (match (_56_1304) with
| (uvs, e, c) -> begin
(

let tvars = (FStar_All.pipe_right uvs (FStar_List.map (fun _56_1307 -> (match (_56_1307) with
| (u, k) -> begin
...
| FStar_Syntax_Syntax.Fixed (_56_1341) -> begin
...
let _56_1348 = (FStar_Syntax_Util.arrow_formals k)
in (match (_56_1348) with
| (bs, kres) -> begin
(

let a = (let _150_798 = (let _150_797 = (FStar_TypeChecker_Env.get_range env)
in (FStar_All.pipe_left (fun _150_796 -> Some (_150_796)) _150_797))
in (FStar_Syntax_Syntax.new_bv _150_798 kres))
in (

let t = (let _150_803 = (FStar_Syntax_Syntax.bv_to_name a)
in (let _150_802 = (let _150_801 = (let _150_800 = (let _150_799 = (FStar_Syntax_Syntax.mk_Total kres)
in (FStar_Syntax_Util.lcomp_of_comp _150_799))
in FStar_Util.Inl (_150_800))
in Some (_150_801))
in (FStar_Syntax_Util.abs bs _150_803 _150_802)))
in (

let _56_1351 = (FStar_Syntax_Util.set_uvar u t)
...
let _56_1383 = (match (((tvars), (gen_univs))) with
| ([], []) -> begin
((e), (c))
end
| ([], _56_1359) -> begin
...
let t = (match ((let _150_804 = (FStar_Syntax_Subst.compress (FStar_Syntax_Util.comp_result c))
in _150_804.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_arrow (bs, cod) -> begin
(

let _56_1376 = (FStar_Syntax_Subst.open_comp bs cod)
in (match (_56_1376) with
| (bs, cod) -> begin
...
in (let _150_805 = (FStar_Syntax_Syntax.mk_Total t)
in ((e'), (_150_805)))))))
end))
end)
...
let _56_1393 = if (FStar_TypeChecker_Env.debug env FStar_Options.Low) then begin
(let _150_812 = (let _150_811 = (FStar_List.map (fun _56_1392 -> (match (_56_1392) with
| (lb, _56_1389, _56_1391) -> begin
(FStar_Syntax_Print.lbname_to_string lb)
end)) lecs)
in (FStar_All.pipe_right _150_811 (FStar_String.concat ", ")))
in (FStar_Util.print1 "Generalizing: %s\n" _150_812))
end else begin
()
end
in (match ((let _150_814 = (FStar_All.pipe_right lecs (FStar_List.map (fun _56_1399 -> (match (_56_1399) with
| (_56_1396, e, c) -> begin
((e), (c))
end))))
in (gen env _150_814))) with
| None -> begin
(FStar_All.pipe_right lecs (FStar_List.map (fun _56_1404 -> (match (_56_1404) with
| (l, t, c) -> begin
((l), ([]), (t), (c))
end))))
end
| Some (ecs) -> begin
(FStar_List.map2 (fun _56_1412 _56_1416 -> (match (((_56_1412), (_56_1416))) with
| ((l, _56_1409, _56_1411), (us, e, c)) -> begin
(

let _56_1417 = if (FStar_TypeChecker_Env.debug env FStar_Options.Medium) then begin
(let _150_821 = (FStar_Range.string_of_range e.FStar_Syntax_Syntax.pos)
in (let _150_820 = (FStar_Syntax_Print.lbname_to_string l)
in (let _150_819 = (FStar_Syntax_Print.term_to_string (FStar_Syntax_Util.comp_result c))
in (let _150_818 = (FStar_Syntax_Print.term_to_string e)
in (FStar_Util.print4 "(%s) Generalized %s at type %s\n%s\n" _150_821 _150_820 _150_819 _150_818)))))
end else begin
()
end
in ((l), (us), (e), (c)))
...
| Some (f) -> begin
(let _150_837 = (FStar_TypeChecker_Rel.apply_guard f e)
in (FStar_All.pipe_left (fun _150_836 -> Some (_150_836)) _150_837))
end)
end)
in (

let is_var = (fun e -> (match ((let _150_840 = (FStar_Syntax_Subst.compress e)
in _150_840.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_name (_56_1434) -> begin
true
end
| _56_1437 -> begin
false
...
let _56_1444 = x
in {FStar_Syntax_Syntax.ppname = _56_1444.FStar_Syntax_Syntax.ppname; FStar_Syntax_Syntax.index = _56_1444.FStar_Syntax_Syntax.index; FStar_Syntax_Syntax.sort = t2}))) (Some (t2.FStar_Syntax_Syntax.n)) e.FStar_Syntax_Syntax.pos)
end
| _56_1447 -> begin
(

let _56_1448 = e
in (let _150_845 = (FStar_Util.mk_ref (Some (t2.FStar_Syntax_Syntax.n)))
in {FStar_Syntax_Syntax.n = _56_1448.FStar_Syntax_Syntax.n; FStar_Syntax_Syntax.tk = _150_845; FStar_Syntax_Syntax.pos = _56_1448.FStar_Syntax_Syntax.pos; FStar_Syntax_Syntax.vars = _56_1448.FStar_Syntax_Syntax.vars}))
end)))
in (
...
let _56_1450 = env
in (let _150_846 = (env.FStar_TypeChecker_Env.use_eq || (env.FStar_TypeChecker_Env.is_pattern && (is_var e)))
in {FStar_TypeChecker_Env.solver = _56_1450.FStar_TypeChecker_Env.solver; FStar_TypeChecker_Env.range = _56_1450.FStar_TypeChecker_Env.range; FStar_TypeChecker_Env.curmodule = _56_1450.FStar_TypeChecker_Env.curmodule; FStar_TypeChecker_Env.gamma = _56_1450.FStar_TypeChecker_Env.gamma; FStar_TypeChecker_Env.gamma_cache = _56_1450.FStar_TypeChecker_Env.gamma_cache; FStar_TypeChecker_Env.modules = _56_1450.FStar_TypeChecker_Env.modules; FStar_TypeChecker_Env.expected_typ = _56_1450.FStar_TypeChecker_Env.expected_typ; FStar_TypeChecker_Env.sigtab = _56_1450.FStar_TypeChecker_Env.sigtab; FStar_TypeChecker_Env.is_pattern = _56_1450.FStar_TypeChecker_Env.is_pattern; FStar_TypeChecker_Env.instantiate_imp = _56_1450.FStar_TypeChecker_Env.instantiate_imp; FStar_TypeChecker_Env.effects = _56_1450.FStar_TypeChecker_Env.effects; FStar_TypeChecker_Env.generalize = _56_1450.FStar_TypeChecker_Env.generalize; FStar_TypeChecker_Env.letrecs = _56_1450.FStar_TypeChecker_Env.letrecs; FStar_TypeChecker_Env.top_level = _56_1450.FStar_TypeChecker_Env.top_level; FStar_TypeChecker_Env.check_uvars = _56_1450.FStar_TypeChecker_Env.check_uvars; FStar_TypeChecker_Env.use_eq = _150_846; FStar_TypeChecker_Env.is_iface = _56_1450.FStar_TypeChecker_Env.is_iface; FStar_TypeChecker_Env.admit = _56_1450.FStar_TypeChecker_Env.admit; FStar_TypeChecker_Env.lax = _56_1450.FStar_TypeChecker_Env.lax; FStar_TypeChecker_Env.lax_universes = _56_1450.FStar_TypeChecker_Env.lax_universes; FStar_TypeChecker_Env.type_of = _56_1450.FStar_TypeChecker_Env.type_of; FStar_TypeChecker_Env.universe_of = _56_1450.FStar_TypeChecker_Env.universe_of; FStar_TypeChecker_Env.use_bv_sorts = _56_1450.FStar_TypeChecker_Env.use_bv_sorts; FStar_TypeChecker_Env.qname_and_index = _56_1450.FStar_TypeChecker_Env.qname_and_index}))
in (match ((check env t1 t2)) with
| None -> begin
(let _150_850 = (let _150_849 = (let _150_848 = (FStar_TypeChecker_Errors.expected_expression_of_type env t2 e t1)
in (let _150_847 = (FStar_TypeChecker_Env.get_range env)
in ((_150_848), (_150_847))))
...
let _56_1456 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Rel"))) then begin
(let _150_851 = (FStar_TypeChecker_Rel.guard_to_string env g)
in (FStar_All.pipe_left (FStar_Util.print1 "Applied guard is %s\n") _150_851))
end else begin
()
end
in (let _150_852 = (decorate e t2)
in ((_150_852), (g))))
end)))))))
...
let _56_1463 = (FStar_TypeChecker_Rel.force_trivial_guard env g)
in (FStar_Syntax_Util.is_pure_lcomp lc)))
in (

let g = (FStar_TypeChecker_Rel.solve_deferred_constraints env g)
in if (FStar_Syntax_Util.is_total_lcomp lc) then begin
(let _150_862 = (discharge g)
in (let _150_861 = (lc.FStar_Syntax_Syntax.comp ())
in ((_150_862), (_150_861))))
end else begin
(
...
let c = (let _150_865 = (let _150_864 = (let _150_863 = (FStar_TypeChecker_Normalize.unfold_effect_abbrev env c)
in (FStar_All.pipe_right _150_863 FStar_Syntax_Syntax.mk_Comp))
in (FStar_All.pipe_right _150_864 (FStar_TypeChecker_Normalize.normalize_comp steps env)))
in (FStar_All.pipe_right _150_865 (FStar_TypeChecker_Normalize.comp_to_comp_typ env)))
in (

let md = (FStar_TypeChecker_Env.get_effect_decl env c.FStar_Syntax_Syntax.effect_name)
in (

let _56_1473 = (destruct_comp c)
in (match (_56_1473) with
| (u_t, t, wp) -> begin
(

let vc = (let _150_871 = (FStar_TypeChecker_Env.inst_effect_fun_with ((u_t)::[]) env md md.FStar_Syntax_Syntax.trivial)
in (let _150_870 = (let _150_868 = (FStar_Syntax_Syntax.as_arg t)
in (let _150_867 = (let _150_866 = (FStar_Syntax_Syntax.as_arg wp)
in (_150_866)::[])
in (_150_868)::_150_867))
in (let _150_869 = (FStar_TypeChecker_Env.get_range env)
in (FStar_Syntax_Syntax.mk_Tm_app _150_871 _150_870 (Some (FStar_Syntax_Util.ktype0.FStar_Syntax_Syntax.n)) _150_869))))
in (

let _56_1475 = if (FStar_All.pipe_left (FStar_TypeChecker_Env.debug env) (FStar_Options.Other ("Simplification"))) then begin
(let _150_872 = (FStar_Syntax_Print.term_to_string vc)
...
let g = (let _150_873 = (FStar_All.pipe_left FStar_TypeChecker_Rel.guard_of_guard_formula (FStar_TypeChecker_Common.NonTrivial (vc)))
in (FStar_TypeChecker_Rel.conj_guard g _150_873))
in (let _150_875 = (discharge g)
in (let _150_874 = (FStar_Syntax_Syntax.mk_Comp c)
in ((_150_875), (_150_874)))))))
end))))))
end)))
...
let op_and_e = (fun e -> (let _150_896 = (FStar_Syntax_Util.b2t e)
in (FStar_All.pipe_right _150_896 (fun _150_895 -> FStar_TypeChecker_Common.NonTrivial (_150_895)))))
in (

let op_or_e = (fun e -> (let _150_901 = (let _150_899 = (FStar_Syntax_Util.b2t e)
in (FStar_Syntax_Util.mk_neg _150_899))
in (FStar_All.pipe_right _150_901 (fun _150_900 -> FStar_TypeChecker_Common.NonTrivial (_150_900)))))
in (

let op_and_t = (fun t -> (FStar_All.pipe_right t (fun _150_904 -> FStar_TypeChecker_Common.NonTrivial (_150_904))))
in (

let op_or_t = (fun t -> (let _150_908 = (FStar_All.pipe_right t FStar_Syntax_Util.mk_neg)
in (FStar_All.pipe_right _150_908 (fun _150_907 -> FStar_TypeChecker_Common.NonTrivial (_150_907)))))
in (

let op_imp_t = (fun t -> (FStar_All.pipe_right t (fun _150_911 -> FStar_TypeChecker_Common.NonTrivial (_150_911))))
in (
...
| ((guard, _56_1505))::[] -> begin
FStar_TypeChecker_Common.NonTrivial (guard)
end
| (_then)::((guard, _56_1510))::[] -> begin
(let _150_915 = (FStar_Syntax_Util.mk_neg guard)
in (FStar_All.pipe_right _150_915 (fun _150_914 -> FStar_TypeChecker_Common.NonTrivial (_150_914))))
end
| _56_1515 -> begin
...
in (match ((FStar_Util.find_map table (fun _56_1523 -> (match (_56_1523) with
| (x, mk) -> begin
if (FStar_Ident.lid_equals x lid) then begin
(let _150_948 = (mk seen_args)
in Some (_150_948))
end else begin
None
end
...
let short_circuit_head : FStar_Syntax_Syntax.term  ->  Prims.bool = (fun l -> (match ((let _150_951 = (FStar_Syntax_Util.un_uinst l)
in _150_951.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_fvar (fv) -> begin
...
in (match (bs) with
| ((_56_1550, Some (FStar_Syntax_Syntax.Implicit (_56_1552))))::_56_1548 -> begin
bs
end
| _56_1558 -> begin
...
| Some (t) -> begin
(match ((let _150_958 = (FStar_Syntax_Subst.compress t)
in _150_958.FStar_Syntax_Syntax.n)) with
| FStar_Syntax_Syntax.Tm_arrow (bs', _56_1564) -> begin
(match ((FStar_Util.prefix_until (fun _56_7 -> (match (_56_7) with
| (_56_1569, Some (FStar_Syntax_Syntax.Implicit (_56_1571))) -> begin
false
end
| _56_1576 -> begin
true
...
| Some ([], _56_1580, _56_1582) -> begin
bs
end
| Some (imps, _56_1587, _56_1589) -> begin
if (FStar_All.pipe_right imps (FStar_Util.for_all (fun _56_1595 -> (match (_56_1595) with
| (x, _56_1594) -> begin
...
let imps = (FStar_All.pipe_right imps (FStar_List.map (fun _56_1599 -> (match (_56_1599) with
| (x, i) -> begin
(let _150_962 = (FStar_Syntax_Syntax.set_range_of_bv x r)
in ((_150_962), (i)))
end))))
...
(let _150_971 = (FStar_ST.read e.FStar_Syntax_Syntax.tk)
in (FStar_Syntax_Syntax.mk (FStar_Syntax_Syntax.Tm_meta (((e), (FStar_Syntax_Syntax.Meta_monadic_lift (((m1), (m2))))))) _150_971 e.FStar_Syntax_Syntax.pos))
end)))
...
(let _150_980 = (FStar_ST.read e.FStar_Syntax_Syntax.tk)
in (FStar_Syntax_Syntax.mk (FStar_Syntax_Syntax.Tm_meta (((e), (FStar_Syntax_Syntax.Meta_monadic (((m), (t))))))) _150_980 e.FStar_Syntax_Syntax.pos))
end))


let effect_repr_aux = (fun only_reifiable env c u_c -> (match ((let _150_985 = (FStar_TypeChecker_Env.norm_eff_name env (FStar_Syntax_Util.comp_effect_name c))
in (FStar_TypeChecker_Env.effect_decl_opt env _150_985))) with
| None -> begin
None
end
| Some (ed) -> begin
...
let _56_1627 = (let _150_986 = (FStar_List.hd c.FStar_Syntax_Syntax.effect_args)
in ((c.FStar_Syntax_Syntax.result_typ), (_150_986)))
in (match (_56_1627) with
| (res_typ, wp) -> begin
(

let repr = (FStar_TypeChecker_Env.inst_effect_fun_with ((u_c)::[]) env ed (([]), (ed.FStar_Syntax_Syntax.repr)))
in (let _150_992 = (let _150_991 = (let _150_989 = (let _150_988 = (let _150_987 = (FStar_Syntax_Syntax.as_arg res_typ)
in (_150_987)::(wp)::[])
in ((repr), (_150_988)))
in FStar_Syntax_Syntax.Tm_app (_150_989))
in (let _150_990 = (FStar_TypeChecker_Env.get_range env)
in (FStar_Syntax_Syntax.mk _150_991 None _150_990)))
in Some (_150_992)))
end)))
end)
end
...
let no_reify = (fun l -> (let _150_1010 = (let _150_1009 = (let _150_1008 = (FStar_Util.format1 "Effect %s cannot be reified" l.FStar_Ident.str)
in (let _150_1007 = (FStar_TypeChecker_Env.get_range env)
in ((_150_1008), (_150_1007))))
in FStar_Syntax_Syntax.Error (_150_1009))
in (Prims.raise _150_1010)))
in (match ((let _150_1011 = (c.FStar_Syntax_Syntax.comp ())
in (effect_repr_aux true env _150_1011 u_c))) with
| None -> begin
...
let _56_1646 = if (FStar_TypeChecker_Env.debug env (FStar_Options.Other ("ED"))) then begin
(

let _56_1644 = (d (FStar_Ident.text_of_lid lident))
in (let _150_1020 = (FStar_Syntax_Print.term_to_string def)
in (FStar_Util.print2 "Registering top-level definition: %s\n%s\n" (FStar_Ident.text_of_lid lident) _150_1020)))
end else begin
()
end
in (

let fv = (let _150_1021 = (FStar_Syntax_Util.incr_delta_qualifier def)
in (FStar_Syntax_Syntax.lid_as_fv lident _150_1021 None))
in (
...
in (let _150_1022 = (FStar_Syntax_Syntax.mk (FStar_Syntax_Syntax.Tm_fvar (fv)) None FStar_Range.dummyRange)
in ((sig_ctx), (_150_1022)))))))))
