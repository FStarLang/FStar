...
let reserved_keywords = (fun uu____20 -> (

let uu____21 = (FStar_Options.codegen_fsharp ())
...
let uu____25 = (reserved_keywords ())
in (FStar_List.existsb (fun k' -> (Prims.op_Equality k' k)) uu____25))))


let string_of_mlpath : mlpath  ->  mlsymbol = (fun uu____33 -> (match (uu____33) with
| (p, s) -> begin
...
let uu____2609 = (FStar_Options.codegen_fsharp ())
...
let uu____2626 = (((FStar_Util.starts_with x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText FStar_Ident.reserved_prefix) || (FStar_Syntax_Syntax.is_null_bv x)) || (is_reserved x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText))
...
in (Prims.strcat "_" uu____2628))
in (Prims.strcat x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText uu____2627))
end
| uu____2629 -> begin
