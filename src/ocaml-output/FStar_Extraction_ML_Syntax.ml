...
let uu____171 = (FStar_ST.op_Bang ctr)
in (FStar_Util.string_of_int uu____171))
in (Prims.strcat "_" uu____170))
in (Prims.strcat uu____107 uu____169)))
in (Prims.strcat "_" uu____106));
)); reset = (fun uu____234 -> ((FStar_ST.op_Colon_Equals ctr (Prims.parse_int "0"));
...
let uu____334 = (gensym ())
in (

let uu____335 = (gensyms (n1 - (Prims.parse_int "1")))
in (uu____334)::uu____335))
end))
...
let uu____345 = (FStar_List.map (fun x1 -> x1.FStar_Ident.idText) x.FStar_Ident.ns)
in ((uu____345), (x.FStar_Ident.ident.FStar_Ident.idText)))
end))
...
let uu____2607 = (((FStar_Util.starts_with x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText FStar_Ident.reserved_prefix) || (FStar_Syntax_Syntax.is_null_bv x)) || (is_reserved x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText))
...
let uu____2609 = (FStar_Util.string_of_int x.FStar_Syntax_Syntax.index)
in (Prims.strcat "_" uu____2609))
in (Prims.strcat x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText uu____2608))
end
| uu____2610 -> begin
