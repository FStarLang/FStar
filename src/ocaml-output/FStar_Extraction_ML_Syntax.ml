...
type mlpath =
  (mlsymbol Prims.list, mlsymbol) FStar_Pervasives_Native.tuple2
  [@@deriving show]
...
let is_reserved : Prims.string -> Prims.bool =
  fun k ->
    let reserved_keywords uu____19 =
      ...
      if uu____20 then fsharpkeywords else ocamlkeywords
    in
...
let __proj__Mkgensym_t__item__gensym : gensym_t -> Prims.unit -> mlident =
  fun projectee ->
    match projectee
    with {gensym= __fname__gensym; reset= __fname__reset; _} -> __fname__gensym


let __proj__Mkgensym_t__item__reset : gensym_t -> Prims.unit -> Prims.unit =
  fun projectee ->
    match projectee
    with {gensym= __fname__gensym; reset= __fname__reset; _} -> __fname__reset
      ...
      (fun uu____92 ->
        ...
        let uu____127 =
          let uu____128 =
            ...
            FStar_Util.string_of_int uu____129
          in
          let uu____171 =
            let uu____172 =
              let uu____173 = FStar_ST.op_Bang ctr in
              FStar_Util.string_of_int uu____173
            in
            Prims.strcat "_" uu____172
          in
          Prims.strcat uu____128 uu____171
        in
...
let rec gensyms : Prims.int -> mlident Prims.list =
  fun x ->
    ...
    | n1 ->
        ...
        let uu____308 = gensyms (n1 - Prims.parse_int "1") in
        uu____307 :: uu____308


let mlpath_of_lident : FStar_Ident.lident -> mlpath =
  fun x ->
    if FStar_Ident.lid_equals x FStar_Parser_Const.failwith_lid then
...
type e_tag = E_PURE | E_GHOST | E_IMPURE [@@deriving show]

let uu___is_E_PURE : e_tag -> Prims.bool =
  fun projectee -> match projectee with E_PURE -> true | uu____331 -> false


let uu___is_E_GHOST : e_tag -> Prims.bool =
  fun projectee -> match projectee with E_GHOST -> true | uu____335 -> false


let uu___is_E_IMPURE : e_tag -> Prims.bool =
  fun projectee -> match projectee with E_IMPURE -> true | uu____339 -> false
...
let uu___is_MLTY_Top : mlty -> Prims.bool =
  fun projectee -> match projectee with MLTY_Top -> true | uu____485 -> false


type mltyscheme =
  (mlidents, mlty) FStar_Pervasives_Native.tuple2
  [@@deriving show]
...
let uu___is_MLC_Unit : mlconstant -> Prims.bool =
  fun projectee -> match projectee with MLC_Unit -> true | uu____529 -> false
...
let uu___is_MLP_Wild : mlpattern -> Prims.bool =
  fun projectee -> match projectee with MLP_Wild -> true | uu____699 -> false
...
let __proj__MLP_CTor__item___0
    : mlpattern
      -> (mlpath, mlpattern Prims.list) FStar_Pervasives_Native.tuple2 =
...
let uu___is_Mutable : meta -> Prims.bool =
  fun projectee -> match projectee with Mutable -> true | uu____871 -> false


let uu___is_Assumed : meta -> Prims.bool =
  fun projectee -> match projectee with Assumed -> true | uu____875 -> false


let uu___is_Private : meta -> Prims.bool =
  fun projectee -> match projectee with Private -> true | uu____879 -> false
...
let uu___is_CInline : meta -> Prims.bool =
  fun projectee -> match projectee with CInline -> true | uu____887 -> false
...
let uu___is_GCType : meta -> Prims.bool =
  fun projectee -> match projectee with GCType -> true | uu____895 -> false
...
type mlletflavor = Rec | NonRec [@@deriving show]
...
let uu___is_NonRec : mlletflavor -> Prims.bool =
  fun projectee -> match projectee with NonRec -> true | uu____937 -> false
...
type mlexpr' =
  ...
  | MLE_Fun of
      ( (mlident, mlty) FStar_Pervasives_Native.tuple2 Prims.list
      , mlexpr )
      FStar_Pervasives_Native.tuple2
  | MLE_Match of
      ( mlexpr
      , ( mlpattern
        , mlexpr FStar_Pervasives_Native.option
        , mlexpr )
...
and mlexpr = {expr: mlexpr'; mlty: mlty; loc: mlloc} [@@deriving show]
...
let __proj__Mkmlexpr__item__expr : mlexpr -> mlexpr' =
  fun projectee ->
    match projectee
    with {expr= __fname__expr; mlty= __fname__mlty; loc= __fname__loc; _} ->
      __fname__expr
...
let __proj__Mkmllb__item__mllb_name : mllb -> mlident =
  fun projectee ->
    match projectee
    with
    | { mllb_name= __fname__mllb_name
      ; mllb_tysc= __fname__mllb_tysc
      ; mllb_add_unit= __fname__mllb_add_unit
      ; mllb_def= __fname__mllb_def
      ; mllb_meta= __fname__mllb_meta
      ; print_typ= __fname__print_typ; _ }
    -> __fname__mllb_name
...
let apply_obj_repr : mlexpr -> mlty -> mlexpr =
  fun x t ->
    let obj_ns =
      ...
      if uu____2451 then "FSharp.Compatibility.OCaml.Obj" else "Obj"
    in
    let obj_repr =
      with_ty (MLTY_Fun (t, E_PURE, MLTY_Top)) (MLE_Name ([obj_ns], "repr"))
    in
  ...
  fun x ->
    ...
    if uu____2466 then
      let uu____2467 =
        let uu____2468 =
          let uu____2469 =
            FStar_Util.string_of_int x.FStar_Syntax_Syntax.index
          in
          Prims.strcat "_" uu____2469
        in
        Prims.strcat x.FStar_Syntax_Syntax.ppname.FStar_Ident.idText uu____2468
      in
...
let push_unit : mltyscheme -> mltyscheme =
  fun ts ->
    ...
    match uu____2474 with vs, ty -> (vs, MLTY_Fun (ml_unit_ty, E_PURE, ty))


let pop_unit : mltyscheme -> mltyscheme =
  fun ts ->
    let uu____2480 = ts in
    match uu____2480 with vs, ty ->
      match ty with
      | MLTY_Fun (l, E_PURE, t) ->
          if l = ml_unit_ty then (vs, t)
