...
let gs : gensym_t =
  ...
  let n_resets = FStar_Util.mk_ref (Prims.lift_native_int (0))  in
  {
    gensym =
      (fun uu____121  ->
         FStar_Util.incr ctr;
    ...
    reset =
      (fun uu____254  ->
         FStar_ST.op_Colon_Equals ctr (Prims.lift_native_int (0));
...
let rec gensyms : Prims.int -> mlident Prims.list =
  fun x  ->
    match x with
    | _0_4 when _0_4 = (Prims.lift_native_int (0)) -> []
    | n1 ->
        let uu____354 = gensym ()  in
        let uu____355 = gensyms (n1 - (Prims.lift_native_int (1)))  in
        uu____354 :: uu____355
