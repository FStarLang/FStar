...
in (let _123983 = (Microsoft_FStar_Absyn_Util.kind_formals k1)
in (match (_123983) with
| (bs, k) -> begin
...
| None -> begin
(let k = (recompute t)
in (let _124010 = (t.Microsoft_FStar_Absyn_Syntax.tk := Some (k))
in k))
end)))
...
| None -> begin
(let t = (recompute e)
in (let _124090 = (e.Microsoft_FStar_Absyn_Syntax.tk := Some (t))
in t))
end)))
