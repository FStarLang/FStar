...
let z3_result_as_replay_result = (fun uu___97_23 -> (match (uu___97_23) with
...
| Some (hints) -> begin
(FStar_Util.find_map hints (fun uu___98_208 -> (match (uu___98_208) with
...
let uu___100_225 = h
in {FStar_Util.hint_name = uu___100_225.FStar_Util.hint_name; FStar_Util.hint_index = uu___100_225.FStar_Util.hint_index; FStar_Util.fuel = uu___100_225.FStar_Util.fuel; FStar_Util.ifuel = uu___100_225.FStar_Util.ifuel; FStar_Util.unsat_core = uu___100_225.FStar_Util.unsat_core; FStar_Util.query_elapsed_time = (Prims.parse_int "0")}))
end)
in (
...
let set_minimum_workable_fuel = (fun f uu___99_355 -> (match (uu___99_355) with
| ([], uu____362) -> begin
()
end
| errs -> begin
...
| ((mi)::tl1, uu____1191) -> begin
(

let uu____1206 = (with_fuel [] p1 mi)
in (FStar_SMTEncoding_Z3.ask None all_labels uu____1206 (Some (scope)) (fun uu____1209 -> (match (uu____1209) with
| (result, elapsed_time) -> begin
(

let uu____1226 = (

let uu____1229 = (use_errors errs result)
in ((uu____1229), (elapsed_time)))
in (cb false mi p1 tl1 scope uu____1226))
end))))
end);
))
and cb = (fun used_hint uu____1231 p1 alt scope uu____1235 -> (match (((uu____1231), (uu____1235))) with
...
let uu____1262 = (FStar_TypeChecker_Env.get_range env)
in (record_hint_stat hint_opt result elapsed_time uu____1262));
)
end
| uu____1263 -> begin
()
...
let uu____1265 = ((FStar_Options.z3_refresh ()) || (FStar_Options.print_z3_statistics ()))
...
let uu____1274 = (FStar_TypeChecker_Env.get_range env)
in (FStar_Range.string_of_range uu____1274))
in (

let uu____1275 = (

let uu____1277 = (FStar_SMTEncoding_Z3.at_log_file ())
in (
...
let uu____1282 = (FStar_Util.string_of_int query_index)
in (
...
let uu____1289 = (FStar_Util.string_of_int elapsed_time)
in (

let uu____1290 = (

let uu____1292 = (FStar_Util.string_of_int prev_fuel)
in (

let uu____1293 = (

let uu____1295 = (FStar_Util.string_of_int prev_ifuel)
in (uu____1295)::[])
in (uu____1292)::uu____1293))
...
in (tag)::uu____1285)
in (uu____1282)::uu____1283))
in (query_name)::uu____1280)
in (uu____1277)::uu____1278))
in (uu____1273)::uu____1275))
in (FStar_Util.print "(%s%s)\tQuery-stats (%s, %s)\t%s%s in %s milliseconds with fuel %s and ifuel %s\n" uu____1271)))
...
let uu____1301 = ((FStar_Options.print_fuels ()) || (FStar_Options.hint_info ()))
...
let uu____1305 = ((FStar_Options.print_fuels ()) || (FStar_Options.hint_info ()))
...
let uu____1308 = ((FStar_Option.isSome unsat_core) || (FStar_Options.z3_refresh ()))
...
let uu____1311 = (with_fuel [] p initial_config)
in (

let uu____1313 = (

let uu____1322 = (FStar_ST.read FStar_SMTEncoding_Z3.fresh_scope)
in (cb (FStar_Option.isSome unsat_core) initial_config p alt_configs uu____1322))
in (FStar_SMTEncoding_Z3.ask unsat_core all_labels uu____1311 None uu____1313)));
)))))
end))))))
in (
...
let uu____1332 = (FStar_Options.admit_smt_queries ())
...
let uu____1353 = (FStar_TypeChecker_Env.get_range tcenv)
in (FStar_All.pipe_left FStar_Range.string_of_range uu____1353))
in (FStar_Util.format1 "Starting query at %s" uu____1352))
in (FStar_SMTEncoding_Encode.push uu____1351));
(

let tcenv1 = (FStar_TypeChecker_Env.incr_query_index tcenv)
in (

let uu____1355 = (FStar_SMTEncoding_Encode.encode_query use_env_msg tcenv1 q)
in (match (uu____1355) with
| (prefix1, labels, qry, suffix) -> begin
(

let pop1 = (fun uu____1376 -> (

let uu____1377 = (

let uu____1378 = (

let uu____1379 = (FStar_TypeChecker_Env.get_range tcenv1)
in (FStar_All.pipe_left FStar_Range.string_of_range uu____1379))
in (FStar_Util.format1 "Ending query at %s" uu____1378))
in (FStar_SMTEncoding_Encode.pop uu____1377)))
in (match (qry) with
| FStar_SMTEncoding_Term.Assume ({FStar_SMTEncoding_Term.tm = FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.FalseOp, uu____1380); FStar_SMTEncoding_Term.freevars = uu____1381; FStar_SMTEncoding_Term.rng = uu____1382}, uu____1383, uu____1384) -> begin
(pop1 ())
end
| uu____1392 when tcenv1.FStar_TypeChecker_Env.admit -> begin
(pop1 ())
end
| FStar_SMTEncoding_Term.Assume (q1, uu____1395, uu____1396) -> begin
...
let dummy : FStar_TypeChecker_Env.solver_t = {FStar_TypeChecker_Env.init = (fun uu____1405 -> ()); FStar_TypeChecker_Env.push = (fun uu____1406 -> ()); FStar_TypeChecker_Env.pop = (fun uu____1407 -> ()); FStar_TypeChecker_Env.mark = (fun uu____1408 -> ()); FStar_TypeChecker_Env.reset_mark = (fun uu____1409 -> ()); FStar_TypeChecker_Env.commit_mark = (fun uu____1410 -> ()); FStar_TypeChecker_Env.encode_modul = (fun uu____1411 uu____1412 -> ()); FStar_TypeChecker_Env.encode_sig = (fun uu____1413 uu____1414 -> ()); FStar_TypeChecker_Env.preprocess = (fun e g -> (((e), (g)))::[]); FStar_TypeChecker_Env.solve = (fun uu____1421 uu____1422 uu____1423 -> ()); FStar_TypeChecker_Env.is_trivial = (fun uu____1427 uu____1428 -> false); FStar_TypeChecker_Env.finish = (fun uu____1429 -> ()); FStar_TypeChecker_Env.refresh = (fun uu____1430 -> ())}
