                ...
                (let uu___1 =
                   let uu___2 =
                     ...
                     | (uu___3, FStar_Pervasives_Native.Some (q, n)) ->
                         let uu___4 = FStar_Ident.string_of_lid q in
                         (uu___4, n) in
                   match uu___2 with
                   | (qname, index) ->
                       let rlimit =
                           ...
                           query_hash =
                             (match next_hint with
                              | FStar_Pervasives_Native.None ->
                                  FStar_Pervasives_Native.None
                              | FStar_Pervasives_Native.Some
                                  { FStar_Compiler_Util.hint_name = uu___6;
                                    FStar_Compiler_Util.hint_index = uu___7;
                                    FStar_Compiler_Util.fuel = uu___8;
                                    FStar_Compiler_Util.ifuel = uu___9;
                                    FStar_Compiler_Util.unsat_core = uu___10;
                 ...
                 | (default_settings, next_hint) ->
                       ...
                       (let uu___3 = with_fuel_and_diagnostics config [] in
                        ...
                        FStar_SMTEncoding_Z3.ask config.query_range
                          (filter_assertions config.query_env
                             config.query_hint) config.query_hash
                          config.query_all_labels uu___3 uu___4
                          (used_hint config)) in
                     ...
                     let quake_and_check_all_configs configs =
                       ...
                       let hi1 =
                         if hi < Prims.int_one then Prims.int_one else hi in
                       let lo1 =
                         if lo < Prims.int_one
                         then Prims.int_one
                           ...
                           (if quaking
                            then
                              (let fuel_msg =
                                 let uu___4 =
                                   let uu___5 =
                                     FStar_Compiler_Effect.op_Bang best_fuel in
                                   let uu___6 =
                                     FStar_Compiler_Effect.op_Bang best_ifuel in
                                   (uu___5, uu___6) in
                                 match uu___4 with
                                 | (FStar_Pervasives_Native.Some f,
                                    FStar_Pervasives_Native.Some i) ->
                                     let uu___5 =
                                       FStar_Compiler_Util.string_of_int f in
                                     let uu___6 =
                              ...
                              (let all_errs =
                               ...
                               then
                                 ...
                                 let errs1 =
                                   let uu___5 =
                                     FStar_Compiler_Effect.op_Bar_Greater
                                       errs FStar_Compiler_List.flatten in
                                   FStar_Compiler_Effect.op_Bar_Greater
                                     uu___5 collect in
                                 let errs2 =
                                 ...
                                 let quake_err =
                                   ...
                                   then
                                     ...
                                     let uu___5 =
                                       FStar_TypeChecker_Err.detailed_error
                                         env rng
          ...
          (let pop uu___2 =
           ...
           try
             (fun uu___2 ->
                ...
                | () ->
                    let uu___3 =
                      FStar_SMTEncoding_Encode.encode_query use_env_msg tcenv
                        q in
                    (match uu___3 with
                         ...
                         (match qry with
                          ...
                          | FStar_SMTEncoding_Term.Assume uu___4 ->
                              ((let uu___6 =
                               ...
                               (let res =
                                  ask_and_return_errors is_retry tcenv1
           ...
           | SplitQueryAndRetry ->
               (pop ();
                ...
                then
                  failwith
                    "Impossible: retried queries should always produce an error reportand cannot be split and retried further"
                else ();
                (let res =
                   let uu___5 = FStar_TypeChecker_Env.split_smt_query tcenv q in
                   match uu___5 with
                   | FStar_Pervasives_Native.None ->
                         ...
                         FStar_Compiler_Effect.op_Bar_Greater goals
                           (FStar_Compiler_List.map
                              (fun uu___7 ->
                                 match uu___7 with
                                 | (env, goal) ->
                                     do_solve true use_env_msg env goal)) in
                       FStar_Compiler_Effect.op_Bar_Greater uu___6
                         FStar_Compiler_List.flatten in
                 if (FStar_Compiler_List.length res) = Prims.int_zero
                 then
                   let uu___5 =
                     FStar_TypeChecker_Err.detailed_error tcenv
                       tcenv.FStar_TypeChecker_Env.range
                       (FStar_Errors.Warning_SplitAndRetryQueries,
                         "The verification condition succeeded after splitting it to localize potential errors, although the original non-split verification condition failed. If you want to rely on splitting queries for verifying your program please use the --split_queries option rather than relying on it implicitly.") in
                   [uu___5]
                 else res))
        ...
        then
          ...
          (let uu___2 =
           ...
           FStar_Profiling.profile
             (fun uu___3 -> do_solve false use_env_msg tcenv q) uu___2
