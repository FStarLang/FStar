...
let _85_72 = (FStar_SMTEncoding_Z3.giveZ3 prefix)
in (
...
let with_fuel = (fun label_assumptions p _85_90 -> (match (_85_90) with
...
let _85_104 = (match (hint_opt) with
| None -> begin
...
let _85_101 = if (FStar_Option.isSome hint.FStar_Util.unsat_core) then begin
(hint.FStar_Util.unsat_core, (3 * 1000))
end else begin
(None, (60 * 1000))
end
in (match (_85_101) with
| (core, timeout) -> begin
(core, (hint.FStar_Util.fuel, hint.FStar_Util.ifuel, timeout))
end))
end)
in (match (_85_104) with
...
let _85_116 = (match ((FStar_ST.read minimum_workable_fuel)) with
| Some (f, errs) -> begin
(f, errs)
end
| None -> begin
...
let _85_121 = (let _176_131 = (with_fuel label_assumptions p min_fuel)
...
let _85_128 = (record_hint None)
in (

let _85_130 = if (FStar_Options.print_fuels ()) then begin
...
let _85_137 = (let _176_140 = (FStar_All.pipe_right errs (FStar_List.map (fun _85_136 -> (match (_85_136) with
| (_85_133, x, y) -> begin
(x, y)
end))))
...
| (_85_153, FStar_Util.Inr (_85_155)) -> begin
FStar_Util.Inr (errs)
end))
in (
...
let _85_164 = (set_minimum_workable_fuel prev_f errs)
in (match (cfgs) with
| [] -> begin
(report p errs)
end
...
let _85_172 = (set_minimum_workable_fuel prev_f errs)
in (report p errs))
end)
end
| (mi)::tl -> begin
(let _176_160 = (with_fuel [] p mi)
in (FStar_SMTEncoding_Z3.ask use_fresh_z3_context None all_labels _176_160 (fun _85_179 -> (match (_85_179) with
| (result, elapsed_time) -> begin
(cb false mi p tl ((use_errors errs result), elapsed_time))
end))))
end)))
and cb = (fun used_hint _85_184 p alt _85_189 -> (match ((_85_184, _85_189)) with
| ((prev_fuel, prev_ifuel, timeout), (result, elapsed_time)) -> begin
(

let _85_192 = if used_hint then begin
(

let _85_190 = (FStar_SMTEncoding_Z3.refresh ())
...
let _85_197 = (record_hint (Some (hint)))
...
let _85_201 = if (FStar_Options.print_fuels ()) then begin
...
let _85_203 = if (FStar_Option.isSome unsat_core) then begin
...
let _85_209 = (let _176_185 = (FStar_Options.split_cases ())
in (FStar_SMTEncoding_SplitQueryCases.can_handle_query _176_185 q))
in (match (_85_209) with
| (b, cb) -> begin
if b then begin
...
let _85_213 = (let _176_204 = (let _176_203 = (let _176_202 = (FStar_TypeChecker_Env.get_range tcenv)
...
let _85_219 = (FStar_SMTEncoding_Encode.encode_query use_env_msg tcenv q)
in (match (_85_219) with
| (prefix, labels, qry, suffix) -> begin
(

let pop = (fun _85_221 -> (match (()) with
| () -> begin
...
in (match (qry) with
| FStar_SMTEncoding_Term.Assume ({FStar_SMTEncoding_Term.tm = FStar_SMTEncoding_Term.App (FStar_SMTEncoding_Term.False, _85_228); FStar_SMTEncoding_Term.hash = _85_225; FStar_SMTEncoding_Term.freevars = _85_223}, _85_233, _85_235) -> begin
(

let _85_238 = (pop ())
in ())
end
| _85_241 when tcenv.FStar_TypeChecker_Env.admit -> begin
(

let _85_242 = (pop ())
in ())
end
| FStar_SMTEncoding_Term.Assume (q, _85_246, _85_248) -> begin
(

let _85_251 = (ask_and_report_errors tcenv false labels prefix qry suffix)
in (pop ()))
end
| _85_254 -> begin
...
let dummy : FStar_TypeChecker_Env.solver_t = {FStar_TypeChecker_Env.init = (fun _85_255 -> ()); FStar_TypeChecker_Env.push = (fun _85_257 -> ()); FStar_TypeChecker_Env.pop = (fun _85_259 -> ()); FStar_TypeChecker_Env.mark = (fun _85_261 -> ()); FStar_TypeChecker_Env.reset_mark = (fun _85_263 -> ()); FStar_TypeChecker_Env.commit_mark = (fun _85_265 -> ()); FStar_TypeChecker_Env.encode_modul = (fun _85_267 _85_269 -> ()); FStar_TypeChecker_Env.encode_sig = (fun _85_271 _85_273 -> ()); FStar_TypeChecker_Env.solve = (fun _85_275 _85_277 _85_279 -> ()); FStar_TypeChecker_Env.is_trivial = (fun _85_281 _85_283 -> false); FStar_TypeChecker_Env.finish = (fun _85_285 -> ()); FStar_TypeChecker_Env.refresh = (fun _85_286 -> ())}
