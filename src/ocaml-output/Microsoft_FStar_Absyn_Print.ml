...
if ((Support.ST.read Microsoft_FStar_Options.hide_genident_nums) && (Support.Microsoft.FStar.Util.starts_with bvd.Microsoft_FStar_Absyn_Syntax.ppname.Microsoft_FStar_Absyn_Syntax.idText "\x5f")) then begin
...
(Support.Microsoft.FStar.Util.format2 "\x28U%s : %s\x29" (if (Support.ST.read Microsoft_FStar_Options.hide_uvar_nums) then begin
"?"
end else begin
...
(match (((Support.List.tryFind (fun _96334 -> (match (_96334) with
| (l, _) -> begin
...
and uvar_k_to_string = (fun uv -> (Support.String.strcat "\'k\x5f" (if (Support.ST.read Microsoft_FStar_Options.hide_uvar_nums) then begin
"?"
end else begin
(Support.Microsoft.FStar.Util.string_of_int (Support.Microsoft.FStar.Unionfind.uvar_id uv))
end)))
...
let subst_to_string = (fun subst -> ((Support.Microsoft.FStar.Util.format1 "\x7b%s\x7d") ((Support.String.concat ", ") (Support.List.map (fun _95822 -> (match (_95822) with
