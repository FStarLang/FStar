...
in (FStar_List.iter (fun _96_48 -> (match (_96_48) with
| (n, d) -> begin
...
in (FStar_List.iter (fun _96_54 -> (match (_96_54) with
| (n, d) -> begin
(

let res = (FStar_Extraction_JavaScript_PrintAst.pretty_print d)
in (let _193_29 = (FStar_Options.prepend_output_dir (Prims.strcat n ext))
in (FStar_Util.write_file _193_29 (FStar_Format.pretty (Prims.parse_int "120") res))))
end)) newDocs))
end
| Some ("Kremlin") -> begin
(

let programs = (let _193_30 = (FStar_List.map FStar_Extraction_Kremlin.translate mllibs)
in (FStar_List.flatten _193_30))
in (

let bin = ((FStar_Extraction_Kremlin.current_version), (programs))
in (let _193_31 = (FStar_Options.prepend_output_dir "out.krml")
in (FStar_Util.save_value_to_file _193_31 bin))))
end
| _96_61 -> begin
(FStar_All.failwith "Unrecognized option")
end))))
...
let _96_68 = (FStar_Options.display_usage ())
...
(let _193_33 = (FStar_Parser_Dep.collect FStar_Parser_Dep.VerifyAll filenames)
in (FStar_Parser_Dep.print _193_33))
end else begin
if (FStar_Options.interactive ()) then begin
(

let _96_82 = if (FStar_Options.explicit_deps ()) then begin
(

let _96_73 = if ((FStar_List.length filenames) = (Prims.parse_int "0")) then begin
...
let _96_75 = if ((FStar_List.length filenames) > (Prims.parse_int "0")) then begin
...
let _96_79 = (FStar_Interactive.detect_dependencies_with_first_interactive_chunk ())
in (match (_96_79) with
| (fn, deps) -> begin
((Some (fn)), (deps))
end)))
end
...
let _96_86 = (FStar_Universal.batch_mode_tc FStar_Parser_Dep.VerifyUserList filenames)
...
let _96_90 = (FStar_Stratified.batch_mode_tc FStar_Parser_Dep.VerifyUserList filenames)
...
let _96_93 = if ((FStar_Options.verify_module ()) <> []) then begin
(

let _96_91 = (FStar_Util.print_error "--verify_module is incompatible with --verify_all")
in (FStar_All.exit (Prims.parse_int "1")))
end else begin
()
end
...
let _96_99 = (FStar_Universal.batch_mode_tc verify_mode filenames)
in (match (_96_99) with
| (fmods, dsenv, env) -> begin
(

let module_names_and_times = (FStar_All.pipe_right fmods (FStar_List.map (fun _96_102 -> (match (_96_102) with
| (x, t) -> begin
(((FStar_Universal.module_or_interface_name x)), (t))
end))))
in (

let _96_104 = (report_errors module_names_and_times)
in (

let _96_106 = (let _193_37 = (let _193_36 = (let _193_35 = (FStar_All.pipe_right fmods (FStar_List.map Prims.fst))
in ((_193_35), (env)))
in FStar_Util.Inr (_193_36))
in (codegen _193_37))
...
let _96_111 = (FStar_Stratified.batch_mode_tc verify_mode filenames)
in (match (_96_111) with
| (fmods, dsenv, env) -> begin
(

let module_names_and_times = (FStar_All.pipe_right fmods (FStar_List.map (fun _96_114 -> (match (_96_114) with
| (x, t) -> begin
(((FStar_Stratified.module_or_interface_name x)), (t))
end))))
in (

let _96_116 = (report_errors module_names_and_times)
in (

let _96_118 = (let _193_41 = (let _193_40 = (let _193_39 = (FStar_All.pipe_right fmods (FStar_List.map Prims.fst))
in ((_193_39), (env)))
in FStar_Util.Inl (_193_40))
in (codegen _193_41))
in (finished_message module_names_and_times (Prims.parse_int "0")))))
end))
end)
...
let main = (fun _96_120 -> (match (()) with
| () -> begin
try
(match (()) with
| () -> begin
...
let _96_125 = if (FStar_Absyn_Util.handleable e) then begin
...
let _96_127 = if (FStar_TypeChecker_Errors.handleable e) then begin
...
let _96_133 = (let _193_48 = (FStar_TypeChecker_Errors.report_all ())
in (FStar_All.pipe_right _193_48 Prims.ignore))
in (

let _96_135 = (report_errors [])
in (FStar_All.exit (Prims.parse_int "1"))))))
end
end))
