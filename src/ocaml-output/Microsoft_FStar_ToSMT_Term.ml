...
let freevar_sort = (fun _417060 -> (match (_417060) with
...
let fv_of_term = (fun _417061 -> (match (_417061) with
...
in (let _417170 = (t.freevars := Some (fvs))
in fvs))
end))

let qop_to_string = (fun _417062 -> (match (_417062) with
| Forall -> begin
"forall"
end
| Exists -> begin
...
let op_to_string = (fun _417063 -> (match (_417063) with
| True -> begin
"true"
end
| False -> begin
...
let weightToSmt = (fun _417064 -> (match (_417064) with
| None -> begin
""
end
| Some (i) -> begin
...
let all_terms = (fun _417222 -> (match (_417222) with
| () -> begin
(Support.List.hd (! (all_terms_l)))
end))

let push = (fun _417223 -> (match (_417223) with
| () -> begin
()
end))

let pop = (fun _417224 -> (match (_417224) with
| () -> begin
()
end))

let commit_mark = (fun _417225 -> (match (_417225) with
| () -> begin
()
end))
...
in (let _417232 = (Support.Microsoft.FStar.Util.smap_add (all_terms ()) key tm)
in tm))
end)))
...
let mkApp = (fun _417240 -> (match (_417240) with
| (s, args) -> begin
(mk (App ((Var (s), args))))
end))
...
let mkAnd = (fun _417256 -> (match (_417256) with
| (t1, t2) -> begin
...
let mkOr = (fun _417316 -> (match (_417316) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
...
let mkImp = (fun _417376 -> (match (_417376) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
| (_, App ((True, _))) -> begin
mkTrue
...
let mk_bin_op = (fun op _417407 -> (match (_417407) with
| (t1, t2) -> begin
(mkApp' (op, (t1)::(t2)::[]))
end))
...
let mkITE = (fun _417412 -> (match (_417412) with
| (t1, t2, t3) -> begin
...
let mkQuant = (fun _417457 -> (match (_417457) with
...
let mkDefineFun = (fun _417566 -> (match (_417566) with
...
let fresh_token = (fun _417570 id -> (match (_417570) with
| (tok_name, sort) -> begin
Assume (((mkEq ((mkInteger id), (mkApp ((constr_id_of_sort sort), ((mkApp (tok_name, [])))::[])))), Some ("fresh token")))
end))

let constructor_to_decl = (fun _417576 -> (match (_417576) with
...
in (let bvars = ((Support.List.mapi (fun i _417590 -> (match (_417590) with
| (_, s) -> begin
(bvar i s)
end))) projectors)
...
in (let proj_terms = ((Support.List.map (fun _417602 -> (match (_417602) with
| (proj, s) -> begin
(mkApp (proj, (xx)::[]))
end))) projectors)
...
in (let projs = ((Support.List.flatten) ((Support.List.mapi (fun i _417610 -> (match (_417610) with
| (name, s) -> begin
...
let name_binders_inner = (fun outer_names start sorts -> (let _417632 = ((Support.List.fold_left (fun _417619 s -> (match (_417619) with
...
in (match (_417632) with
| (names, binders, n) -> begin
(names, (Support.List.rev binders), n)
end)))

let name_binders = (fun sorts -> (let _417637 = (name_binders_inner [] 0 sorts)
in (match (_417637) with
| (names, binders, n) -> begin
...
| Quant ((qop, pats, wopt, sorts, body)) -> begin
(let _417667 = (name_binders_inner names n sorts)
in (match (_417667) with
| (names, binders, n) -> begin
...
let caption_to_string = (fun _417065 -> (match (_417065) with
| None -> begin
""
end
| Some (c) -> begin
(let _417692 = (Support.Microsoft.FStar.Util.splitlines c)
in (match (_417692) with
| hd::tl -> begin
...
(let _417723 = (name_binders arg_sorts)
in (match (_417723) with
| (names, binders) -> begin
