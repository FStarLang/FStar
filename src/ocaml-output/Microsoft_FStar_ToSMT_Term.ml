...
let freevar_sort = (fun _417292 -> (match (_417292) with
...
let fv_of_term = (fun _417293 -> (match (_417293) with
...
in (let _417402 = (t.freevars := Some (fvs))
in fvs))
end))

let qop_to_string = (fun _417294 -> (match (_417294) with
| Forall -> begin
"forall"
end
| Exists -> begin
...
let op_to_string = (fun _417295 -> (match (_417295) with
| True -> begin
"true"
end
| False -> begin
...
let weightToSmt = (fun _417296 -> (match (_417296) with
| None -> begin
""
end
| Some (i) -> begin
...
let all_terms = (fun _417454 -> (match (_417454) with
| () -> begin
(Support.List.hd (! (all_terms_l)))
end))

let push = (fun _417455 -> (match (_417455) with
| () -> begin
()
end))

let pop = (fun _417456 -> (match (_417456) with
| () -> begin
()
end))

let commit_mark = (fun _417457 -> (match (_417457) with
| () -> begin
()
end))
...
in (let _417464 = (Support.Microsoft.FStar.Util.smap_add (all_terms ()) key tm)
in tm))
end)))
...
let mkApp = (fun _417472 -> (match (_417472) with
| (s, args) -> begin
(mk (App ((Var (s), args))))
end))
...
let mkAnd = (fun _417488 -> (match (_417488) with
| (t1, t2) -> begin
...
let mkOr = (fun _417548 -> (match (_417548) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
...
let mkImp = (fun _417608 -> (match (_417608) with
| (t1, t2) -> begin
(match ((t1.tm, t2.tm)) with
| (_, App ((True, _))) -> begin
mkTrue
...
let mk_bin_op = (fun op _417639 -> (match (_417639) with
| (t1, t2) -> begin
(mkApp' (op, (t1)::(t2)::[]))
end))
...
let mkITE = (fun _417644 -> (match (_417644) with
| (t1, t2, t3) -> begin
...
let mkQuant = (fun _417689 -> (match (_417689) with
...
let mkDefineFun = (fun _417803 -> (match (_417803) with
...
let fresh_token = (fun _417807 id -> (match (_417807) with
| (tok_name, sort) -> begin
Assume (((mkEq ((mkInteger id), (mkApp ((constr_id_of_sort sort), ((mkApp (tok_name, [])))::[])))), Some ("fresh token")))
end))

let constructor_to_decl = (fun _417813 -> (match (_417813) with
...
in (let bvars = ((Support.List.mapi (fun i _417827 -> (match (_417827) with
| (_, s) -> begin
(bvar i s)
end))) projectors)
...
in (let proj_terms = ((Support.List.map (fun _417839 -> (match (_417839) with
| (proj, s) -> begin
(mkApp (proj, (xx)::[]))
end))) projectors)
...
in (let projs = ((Support.List.flatten) ((Support.List.mapi (fun i _417847 -> (match (_417847) with
| (name, s) -> begin
...
let name_binders_inner = (fun outer_names start sorts -> (let _417869 = ((Support.List.fold_left (fun _417856 s -> (match (_417856) with
...
in (match (_417869) with
| (names, binders, n) -> begin
(names, (Support.List.rev binders), n)
end)))

let name_binders = (fun sorts -> (let _417874 = (name_binders_inner [] 0 sorts)
in (match (_417874) with
| (names, binders, n) -> begin
...
| Quant ((qop, pats, wopt, sorts, body)) -> begin
(let _417904 = (name_binders_inner names n sorts)
in (match (_417904) with
| (names, binders, n) -> begin
...
let caption_to_string = (fun _417297 -> (match (_417297) with
| None -> begin
""
end
| Some (c) -> begin
(let _417929 = (Support.Microsoft.FStar.Util.splitlines c)
in (match (_417929) with
| hd::tl -> begin
...
(let _417960 = (name_binders arg_sorts)
in (match (_417960) with
| (names, binders) -> begin
...
| Integer (n) -> begin
(Support.Microsoft.FStar.Util.format1 "Integer %s" (Support.Microsoft.FStar.Util.string_of_int n))
end
| BoundV (n) -> begin
...
| App ((op, l)) -> begin
(Support.Microsoft.FStar.Util.format2 "App %s [ %s ]" (op_to_string op) (print_smt_term_list l))
end
...
and print_smt_term_list_list = (fun l -> (Support.List.fold_left (fun s l -> (Support.String.strcat (Support.String.strcat (Support.String.strcat s "; [ ") (print_smt_term_list l)) " ] ")) "" l))
