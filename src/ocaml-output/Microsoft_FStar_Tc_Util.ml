...
| Some (g) -> begin
(let _233983 = if ((Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Rel"))) then begin
...
| _ -> begin
(let _233990 = e
in {Microsoft_FStar_Absyn_Syntax.n = _233990.Microsoft_FStar_Absyn_Syntax.n; Microsoft_FStar_Absyn_Syntax.tk = (Support.Microsoft.FStar.Util.mk_ref (Some (t2))); Microsoft_FStar_Absyn_Syntax.pos = _233990.Microsoft_FStar_Absyn_Syntax.pos; Microsoft_FStar_Absyn_Syntax.fvs = _233990.Microsoft_FStar_Absyn_Syntax.fvs; Microsoft_FStar_Absyn_Syntax.uvs = _233990.Microsoft_FStar_Absyn_Syntax.uvs})
end)
in (e, g))))
end)
end)))
...
let as_uvar_e = (fun _233935 -> (match (_233935) with
...
let new_implicit_tvar = (fun env k -> (let _234037 = (Microsoft_FStar_Tc_Rel.new_tvar (Microsoft_FStar_Tc_Env.get_range env) (env_binders env) k)
in (match (_234037) with
| (t, u) -> begin
(t, ((as_uvar_t u), u.Microsoft_FStar_Absyn_Syntax.pos))
end)))

let new_implicit_evar = (fun env t -> (let _234042 = (Microsoft_FStar_Tc_Rel.new_evar (Microsoft_FStar_Tc_Env.get_range env) (env_binders env) t)
in (match (_234042) with
| (e, u) -> begin
...
let tks_of_args = (fun args -> ((Support.List.map (fun _233936 -> (match (_233936) with
...
let is_implicit = (fun _233937 -> (match (_233937) with
...
in (let _234101 = (mk_implicits vars subst brest)
in (match (_234101) with
| (imp_args, bs) -> begin
((imp_arg)::imp_args, bs)
end))))
end else begin
...
(let _234106 = (mk_implicits (Microsoft_FStar_Tc_Env.binders env) [] bs)
in (match (_234106) with
| (imps, bs) -> begin
(imps, bs, k')
end))
end))
...
in (let _234120 = ((force_trivial env) (Microsoft_FStar_Tc_Rel.keq env None k kar))
in ([], bs, kres)))))))
end
| _ -> begin
...
in (let vars_of_bindings = (fun bs -> ((Support.List.map (fun _233938 -> (match (_233938) with
...
in (let _234181 = (Microsoft_FStar_Tc_Rel.new_evar p.Microsoft_FStar_Absyn_Syntax.p [] t)
in (match (_234181) with
| (e, u) -> begin
(let p = (let _234182 = p
in {Microsoft_FStar_Absyn_Syntax.v = Microsoft_FStar_Absyn_Syntax.Pat_dot_term ((x, e)); Microsoft_FStar_Absyn_Syntax.sort = _234182.Microsoft_FStar_Absyn_Syntax.sort; Microsoft_FStar_Absyn_Syntax.p = _234182.Microsoft_FStar_Absyn_Syntax.p})
in ([], [], [], env, (Microsoft_FStar_Absyn_Syntax.varg e), p))
end)))
end
| Microsoft_FStar_Absyn_Syntax.Pat_dot_typ ((a, _)) -> begin
(let k = (new_kvar env)
in (let _234193 = (Microsoft_FStar_Tc_Rel.new_tvar p.Microsoft_FStar_Absyn_Syntax.p (Microsoft_FStar_Tc_Env.binders env) k)
in (match (_234193) with
| (t, u) -> begin
(let p = (let _234194 = p
in {Microsoft_FStar_Absyn_Syntax.v = Microsoft_FStar_Absyn_Syntax.Pat_dot_typ ((a, t)); Microsoft_FStar_Absyn_Syntax.sort = _234194.Microsoft_FStar_Absyn_Syntax.sort; Microsoft_FStar_Absyn_Syntax.p = _234194.Microsoft_FStar_Absyn_Syntax.p})
...
(let _234247 = ((Support.List.fold_left (fun _234232 p -> (match (_234232) with
| (b, a, w, env, args, pats) -> begin
(let _234240 = (pat_as_arg_with_env allow_wc_dependence env p)
in (match (_234240) with
| (b', a', w', env, arg, pat) -> begin
((b')::b, (a')::a, (w')::w, env, (arg)::args, (pat)::pats)
end))
end)) ([], [], [], env, [], [])) pats)
in (match (_234247) with
| (b, a, w, env, args, pats) -> begin
...
in (((Support.List.flatten) (Support.List.rev b)), ((Support.List.flatten) (Support.List.rev a)), ((Support.List.flatten) (Support.List.rev w)), env, (Microsoft_FStar_Absyn_Syntax.varg e), (let _234249 = p
in {Microsoft_FStar_Absyn_Syntax.v = Microsoft_FStar_Absyn_Syntax.Pat_cons ((fv, (Support.List.rev pats))); Microsoft_FStar_Absyn_Syntax.sort = _234249.Microsoft_FStar_Absyn_Syntax.sort; Microsoft_FStar_Absyn_Syntax.p = _234249.Microsoft_FStar_Absyn_Syntax.p})))
end))
end
...
in (let _234395 = p
in {Microsoft_FStar_Absyn_Syntax.v = Microsoft_FStar_Absyn_Syntax.Pat_cons ((fv, pats)); Microsoft_FStar_Absyn_Syntax.sort = _234395.Microsoft_FStar_Absyn_Syntax.sort; Microsoft_FStar_Absyn_Syntax.p = _234395.Microsoft_FStar_Absyn_Syntax.p}))))
end
| _ -> begin
p
end))
in (let one_pat = (fun allow_wc_dependence env p -> (let p = (elaborate_pat env p)
in (let _234410 = (pat_as_arg_with_env allow_wc_dependence env p)
in (match (_234410) with
...
(let _234440 = (one_pat false env q)
in (match (_234440) with
| (b, a, _, arg, q) -> begin
(let _234455 = (Support.List.fold_right (fun p _234445 -> (match (_234445) with
| (w, args, pats) -> begin
(let _234451 = (one_pat false env p)
in (match (_234451) with
| (b', a', w', arg, p) -> begin
...
in (match (_234455) with
| (w, args, pats) -> begin
((Support.List.append b w), (arg)::args, (let _234456 = p
in {Microsoft_FStar_Absyn_Syntax.v = Microsoft_FStar_Absyn_Syntax.Pat_disj ((q)::pats); Microsoft_FStar_Absyn_Syntax.sort = _234456.Microsoft_FStar_Absyn_Syntax.sort; Microsoft_FStar_Absyn_Syntax.p = _234456.Microsoft_FStar_Absyn_Syntax.p}))
end))
end))
end
| _ -> begin
(let _234467 = (one_pat true env p)
in (match (_234467) with
| (b, _, _, arg, p) -> begin
(b, (arg)::[], p)
end))
end))
in (let _234471 = (top_level_pat_as_args env p)
in (match (_234471) with
| (b, args, p) -> begin
(let exps = ((Support.List.map (fun _233939 -> (match (_233939) with
...
(let _234511 = if (not ((Microsoft_FStar_Absyn_Util.bvar_eq x y))) then begin
...
in (let _234513 = if ((Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("Pat"))) then begin
...
in (let x = (let _234516 = x
in {Microsoft_FStar_Absyn_Syntax.v = _234516.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = s; Microsoft_FStar_Absyn_Syntax.p = _234516.Microsoft_FStar_Absyn_Syntax.p})
...
(let _234524 = if (not ((Microsoft_FStar_Absyn_Util.bvar_eq x y))) then begin
(failwith (Support.Microsoft.FStar.Util.format2 "Expected pattern variable %s; got %s" (Microsoft_FStar_Absyn_Print.strBvd x.Microsoft_FStar_Absyn_Syntax.v) (Microsoft_FStar_Absyn_Print.strBvd y.Microsoft_FStar_Absyn_Syntax.v)))
end
in (let x = (let _234526 = x
in {Microsoft_FStar_Absyn_Syntax.v = _234526.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = (force_tk e); Microsoft_FStar_Absyn_Syntax.p = _234526.Microsoft_FStar_Absyn_Syntax.p})
...
(let x = (let _234537 = x
in {Microsoft_FStar_Absyn_Syntax.v = _234537.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = (force_tk e); Microsoft_FStar_Absyn_Syntax.p = _234537.Microsoft_FStar_Absyn_Syntax.p})
...
(let _234550 = if (not ((Microsoft_FStar_Absyn_Util.fvar_eq fv fv'))) then begin
...
(let _234574 = if (not ((Microsoft_FStar_Absyn_Util.fvar_eq fv fv'))) then begin
(failwith (Support.Microsoft.FStar.Util.format2 "Expected pattern constructor %s; got %s" fv.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.str fv'.Microsoft_FStar_Absyn_Syntax.v.Microsoft_FStar_Absyn_Syntax.str))
end
in (let fv = fv'
...
(let _234646 = if (not ((Microsoft_FStar_Absyn_Util.bvar_eq a b))) then begin
...
(let _234653 = if (not ((Microsoft_FStar_Absyn_Util.bvar_eq a b))) then begin
(failwith (Support.Microsoft.FStar.Util.format2 "Expected pattern variable %s; got %s" (Microsoft_FStar_Absyn_Print.strBvd a.Microsoft_FStar_Absyn_Syntax.v) (Microsoft_FStar_Absyn_Print.strBvd b.Microsoft_FStar_Absyn_Syntax.v)))
end
...
(let k0 = (force_tk t0)
in (let a = (let _234664 = a
in {Microsoft_FStar_Absyn_Syntax.v = _234664.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = k0; Microsoft_FStar_Absyn_Syntax.p = _234664.Microsoft_FStar_Absyn_Syntax.p})
...
in (let pat_as_arg = (fun p -> (let _234696 = (decorated_pattern_as_either p)
in (match (_234696) with
| (vars, te) -> begin
...
| Microsoft_FStar_Absyn_Syntax.Pat_cons ((fv, pats)) -> begin
(let _234714 = ((Support.List.unzip) ((Support.List.map pat_as_arg) pats))
in (match (_234714) with
| (vars, args) -> begin
...
| (Microsoft_FStar_Absyn_Syntax.Pat_twild (_)) | (Microsoft_FStar_Absyn_Syntax.Pat_tvar (_)) | (Microsoft_FStar_Absyn_Syntax.Pat_dot_typ (_)) -> begin
(let _234751 = (decorated_pattern_as_typ p)
in (match (_234751) with
| (vars, t) -> begin
(vars, Support.Microsoft.FStar.Util.Inl (t))
end))
end
| _ -> begin
(let _234756 = (decorated_pattern_as_exp p)
in (match (_234756) with
| (vars, e) -> begin
...
(let _234802 = ((Support.List.fold_left (fun _234779 _234783 -> (match ((_234779, _234783)) with
| ((out, subst), (b, _)) -> begin
(match (b) with
...
in ((let _234815 = a
in {Microsoft_FStar_Absyn_Syntax.v = _234815.Microsoft_FStar_Absyn_Syntax.v; Microsoft_FStar_Absyn_Syntax.sort = k; Microsoft_FStar_Absyn_Syntax.p = _234815.Microsoft_FStar_Absyn_Syntax.p}), false))
end
| _ -> begin
(a, true)
end))
...
(let _234879 = ((Support.List.fold_left (fun _234860 b -> (match (_234860) with
| (scope, bs, check) -> begin
(match ((Support.Prims.fst b)) with
| Support.Microsoft.FStar.Util.Inl (a) -> begin
(let _234866 = (mk_t_binder scope a)
in (match (_234866) with
| (tb, c) -> begin
...
| Support.Microsoft.FStar.Util.Inr (x) -> begin
(let _234874 = (mk_v_binder scope x)
in (match (_234874) with
| (vb, c) -> begin
...
in (match (_234879) with
| (scope, bs, check) -> begin
(let _234883 = (aux scope body)
in (match (_234883) with
...
in (let _234886 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.High) then begin
...
let destruct_comp = (fun c -> (let _234909 = (match (c.Microsoft_FStar_Absyn_Syntax.effect_args) with
...
in (match (_234909) with
| (wp, wlp) -> begin
(c.Microsoft_FStar_Absyn_Syntax.result_typ, wp, wlp)
end)))

let lift_comp = (fun c m lift -> (let _234917 = (destruct_comp c)
in (match (_234917) with
| (_, wp, wlp) -> begin
...
| None -> begin
(let m = (find l)
in (let _234933 = (Support.Microsoft.FStar.Util.smap_add cache l.Microsoft_FStar_Absyn_Syntax.str m)
in m))
end))))

let join_effects = (fun env l1 l2 -> (let _234943 = (Microsoft_FStar_Tc_Env.join env (norm_eff_name env l1) (norm_eff_name env l2))
in (match (_234943) with
| (m, _, _) -> begin
m
end)))
...
in (let _234955 = (Microsoft_FStar_Tc_Env.join env c1.Microsoft_FStar_Absyn_Syntax.effect_name c2.Microsoft_FStar_Absyn_Syntax.effect_name)
in (match (_234955) with
| (m, lift1, lift2) -> begin
(let m1 = (lift_comp c1 m lift1)
in (let m2 = (lift_comp c2 m lift2)
in (let md = (Microsoft_FStar_Tc_Env.get_effect_decl env m)
in (let _234961 = (Microsoft_FStar_Tc_Env.wp_signature env md.Microsoft_FStar_Absyn_Syntax.mname)
in (match (_234961) with
| (a, kwp) -> begin
...
in {Microsoft_FStar_Absyn_Syntax.eff_name = c.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.res_typ = c.Microsoft_FStar_Absyn_Syntax.result_typ; Microsoft_FStar_Absyn_Syntax.cflags = c.Microsoft_FStar_Absyn_Syntax.flags; Microsoft_FStar_Absyn_Syntax.comp = (fun _234972 -> (match (_234972) with
| () -> begin
c0
end))}))

let subst_lcomp = (fun subst lc -> (let _234975 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _234975.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = (Microsoft_FStar_Absyn_Util.subst_typ subst lc.Microsoft_FStar_Absyn_Syntax.res_typ); Microsoft_FStar_Absyn_Syntax.cflags = _234975.Microsoft_FStar_Absyn_Syntax.cflags; Microsoft_FStar_Absyn_Syntax.comp = (fun _234977 -> (match (_234977) with
| () -> begin
...
| Some (m) -> begin
(let _234992 = (Microsoft_FStar_Tc_Env.wp_signature env Microsoft_FStar_Absyn_Const.pure_effect_lid)
in (match (_234992) with
| (a, kwp) -> begin
...
in (let _234997 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.High) then begin
...
let bind = (fun env e1opt lc1 _235004 -> (match (_235004) with
| (b, lc2) -> begin
(let _235015 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Extreme) then begin
...
in (let bind_it = (fun _235018 -> (match (_235018) with
| () -> begin
(let c1 = (lc1.Microsoft_FStar_Absyn_Syntax.comp ())
in (let c2 = (lc2.Microsoft_FStar_Absyn_Syntax.comp ())
in (let try_simplify = (fun _235022 -> (match (_235022) with
| () -> begin
(let aux = (fun _235024 -> (match (_235024) with
| () -> begin
...
| Some (c) -> begin
(let _235064 = if ((Microsoft_FStar_Tc_Env.debug env) (Microsoft_FStar_Options.Other ("bind"))) then begin
...
| None -> begin
(let _235079 = (lift_and_destruct env c1 c2)
in (match (_235079) with
...
in (let _235110 = (Microsoft_FStar_Tc_Env.wp_signature env md_pure.Microsoft_FStar_Absyn_Syntax.mname)
in (match (_235110) with
| (a, kwp) -> begin
(let k = (Microsoft_FStar_Absyn_Util.subst_kind ((Support.Microsoft.FStar.Util.Inl ((a.Microsoft_FStar_Absyn_Syntax.v, t)))::[]) kwp)
...
let refresh_comp_label = (fun env b lc -> (let refresh = (fun _235119 -> (match (_235119) with
| () -> begin
...
(let _235126 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Low) then begin
...
in (let _235129 = if ((not ((Microsoft_FStar_Absyn_Syntax.lid_equals ct.Microsoft_FStar_Absyn_Syntax.effect_name c'.Microsoft_FStar_Absyn_Syntax.effect_name))) && (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Low)) then begin
...
in (let _235134 = (destruct_comp c')
in (match (_235134) with
| (t, wp, wlp) -> begin
...
in (Microsoft_FStar_Absyn_Syntax.mk_Comp (let _235137 = c'
in {Microsoft_FStar_Absyn_Syntax.effect_name = _235137.Microsoft_FStar_Absyn_Syntax.effect_name; Microsoft_FStar_Absyn_Syntax.result_typ = _235137.Microsoft_FStar_Absyn_Syntax.result_typ; Microsoft_FStar_Absyn_Syntax.effect_args = ((Microsoft_FStar_Absyn_Syntax.targ wp))::((Microsoft_FStar_Absyn_Syntax.targ wlp))::[]; Microsoft_FStar_Absyn_Syntax.flags = c'.Microsoft_FStar_Absyn_Syntax.flags}))))
end)))))
end)
end)
end))
in (let _235139 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _235139.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = _235139.Microsoft_FStar_Absyn_Syntax.res_typ; Microsoft_FStar_Absyn_Syntax.cflags = _235139.Microsoft_FStar_Absyn_Syntax.cflags; Microsoft_FStar_Absyn_Syntax.comp = refresh})))
...
let weaken_precondition = (fun env lc f -> (let weaken = (fun _235171 -> (match (_235171) with
| () -> begin
(let c = (lc.Microsoft_FStar_Absyn_Syntax.comp ())
in (match (f) with
| Microsoft_FStar_Tc_Rel.Trivial -> begin
...
in (let _235180 = (destruct_comp c)
in (match (_235180) with
| (res_t, wp, wlp) -> begin
...
in (let _235184 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _235184.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = _235184.Microsoft_FStar_Absyn_Syntax.res_typ; Microsoft_FStar_Absyn_Syntax.cflags = _235184.Microsoft_FStar_Absyn_Syntax.cflags; Microsoft_FStar_Absyn_Syntax.comp = weaken})))

let strengthen_precondition = (fun reason env e lc g0 -> if (Microsoft_FStar_Tc_Rel.is_trivial g0) then begin
(lc, g0)
end else begin
(let flags = ((Support.List.collect (fun _233940 -> (match (_233940) with
...
in (let strengthen = (fun _235198 -> (match (_235198) with
| () -> begin
(let c = (lc.Microsoft_FStar_Absyn_Syntax.comp ())
...
in (let c = (Microsoft_FStar_Tc_Normalize.weak_norm_comp env c)
in (let _235213 = (destruct_comp c)
in (match (_235213) with
| (res_t, wp, wlp) -> begin
(let md = (Microsoft_FStar_Tc_Env.get_effect_decl env c.Microsoft_FStar_Absyn_Syntax.effect_name)
...
in ((let _235218 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = (norm_eff_name env lc.Microsoft_FStar_Absyn_Syntax.eff_name); Microsoft_FStar_Absyn_Syntax.res_typ = _235218.Microsoft_FStar_Absyn_Syntax.res_typ; Microsoft_FStar_Absyn_Syntax.cflags = if ((Microsoft_FStar_Absyn_Util.is_pure_lcomp lc) && (not ((Microsoft_FStar_Absyn_Util.is_function_typ lc.Microsoft_FStar_Absyn_Syntax.res_typ)))) then begin
flags
end else begin
[]
end; Microsoft_FStar_Absyn_Syntax.comp = strengthen}), (let _235220 = g0
in {Microsoft_FStar_Tc_Rel.guard_f = Microsoft_FStar_Tc_Rel.Trivial; Microsoft_FStar_Tc_Rel.deferred = _235220.Microsoft_FStar_Tc_Rel.deferred; Microsoft_FStar_Tc_Rel.implicits = _235220.Microsoft_FStar_Tc_Rel.implicits}))))
end)

let ite = (fun env guard lcomp_then lcomp_else -> (let comp = (fun _235227 -> (match (_235227) with
| () -> begin
(let _235243 = (lift_and_destruct env (lcomp_then.Microsoft_FStar_Absyn_Syntax.comp ()) (lcomp_else.Microsoft_FStar_Absyn_Syntax.comp ()))
in (match (_235243) with
...
| hd::tl -> begin
(Support.List.fold_left (fun eff _235265 -> (match (_235265) with
| (_, lc) -> begin
...
in (let bind_cases = (fun _235268 -> (match (_235268) with
| () -> begin
(let ifthenelse = (fun md res_t g wp_t wp_e -> (Microsoft_FStar_Absyn_Syntax.mk_Typ_app (md.Microsoft_FStar_Absyn_Syntax.if_then_else, ((Microsoft_FStar_Absyn_Syntax.targ res_t))::((Microsoft_FStar_Absyn_Syntax.targ g))::((Microsoft_FStar_Absyn_Syntax.targ wp_t))::((Microsoft_FStar_Absyn_Syntax.targ wp_e))::[]) None (Support.Microsoft.FStar.Range.union_ranges wp_t.Microsoft_FStar_Absyn_Syntax.pos wp_e.Microsoft_FStar_Absyn_Syntax.pos)))
...
in (mk_comp md res_t wp wlp [])))))))
in (let comp = (Support.List.fold_right (fun _235284 celse -> (match (_235284) with
| (g, cthen) -> begin
(let _235302 = (lift_and_destruct env (cthen.Microsoft_FStar_Absyn_Syntax.comp ()) celse)
in (match (_235302) with
...
in (let _235310 = (destruct_comp comp)
in (match (_235310) with
| (_, wp, wlp) -> begin
...
let close_comp = (fun env bindings lc -> (let close = (fun _235317 -> (match (_235317) with
| () -> begin
(let c = (lc.Microsoft_FStar_Absyn_Syntax.comp ())
in if (Microsoft_FStar_Absyn_Util.is_ml_comp c) then begin
c
...
in (let c = (Microsoft_FStar_Tc_Normalize.weak_norm_comp env c)
in (let _235348 = (destruct_comp c)
in (match (_235348) with
| (t, wp, wlp) -> begin
(let md = (Microsoft_FStar_Tc_Env.get_effect_decl env c.Microsoft_FStar_Absyn_Syntax.effect_name)
...
in (let _235352 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _235352.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = _235352.Microsoft_FStar_Absyn_Syntax.res_typ; Microsoft_FStar_Absyn_Syntax.cflags = _235352.Microsoft_FStar_Absyn_Syntax.cflags; Microsoft_FStar_Absyn_Syntax.comp = close})))

let maybe_assume_result_eq_pure_term = (fun env e lc -> (let refine = (fun _235358 -> (match (_235358) with
| () -> begin
(let c = (lc.Microsoft_FStar_Absyn_Syntax.comp ())
...
in (let _235368 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _235368.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = _235368.Microsoft_FStar_Absyn_Syntax.res_typ; Microsoft_FStar_Absyn_Syntax.cflags = flags; Microsoft_FStar_Absyn_Syntax.comp = refine}))))
...
in (let _235415 = (new_implicit_evar env t)
in (match (_235415) with
| (v, u) -> begin
(let subst = (Support.Microsoft.FStar.Util.Inr ((x.Microsoft_FStar_Absyn_Syntax.v, v)))::subst
in (let _235421 = (aux subst rest)
in (match (_235421) with
| (args, bs, subst, us) -> begin
...
in (let _235427 = (aux [] bs)
in (match (_235427) with
...
(let rec aux = (fun subst _233942 -> (match (_233942) with
| (Support.Microsoft.FStar.Util.Inl (a), _)::rest -> begin
(let k = (Microsoft_FStar_Absyn_Util.subst_kind subst a.Microsoft_FStar_Absyn_Syntax.sort)
in (let _235453 = (new_implicit_tvar env k)
in (match (_235453) with
| (t, u) -> begin
(let subst = (Support.Microsoft.FStar.Util.Inl ((a.Microsoft_FStar_Absyn_Syntax.v, t)))::subst
in (let _235459 = (aux subst rest)
in (match (_235459) with
| (args, bs, subst, us) -> begin
(((Support.Microsoft.FStar.Util.Inl (t), Some (Microsoft_FStar_Absyn_Syntax.Implicit)))::args, bs, subst, (Support.Microsoft.FStar.Util.Inl (u))::us)
end)))
end)))
...
| (Support.Microsoft.FStar.Util.Inr (x), Some (Microsoft_FStar_Absyn_Syntax.Implicit))::rest -> begin
(let t = (Microsoft_FStar_Absyn_Util.subst_typ subst x.Microsoft_FStar_Absyn_Syntax.sort)
in (let _235470 = (new_implicit_evar env t)
in (match (_235470) with
| (v, u) -> begin
(let subst = (Support.Microsoft.FStar.Util.Inr ((x.Microsoft_FStar_Absyn_Syntax.v, v)))::subst
in (let _235476 = (aux subst rest)
in (match (_235476) with
| (args, bs, subst, us) -> begin
(((Support.Microsoft.FStar.Util.Inr (v), Some (Microsoft_FStar_Absyn_Syntax.Implicit)))::args, bs, subst, (Support.Microsoft.FStar.Util.Inr (u))::us)
end)))
end)))
...
in (let _235482 = (aux [] bs)
in (match (_235482) with
| (args, bs, subst, implicits) -> begin
...
| Microsoft_FStar_Tc_Rel.Trivial -> begin
(let lc = (let _235512 = lc
in {Microsoft_FStar_Absyn_Syntax.eff_name = _235512.Microsoft_FStar_Absyn_Syntax.eff_name; Microsoft_FStar_Absyn_Syntax.res_typ = t; Microsoft_FStar_Absyn_Syntax.cflags = _235512.Microsoft_FStar_Absyn_Syntax.cflags; Microsoft_FStar_Absyn_Syntax.comp = _235512.Microsoft_FStar_Absyn_Syntax.comp})
in (e, lc, g))
end
| Microsoft_FStar_Tc_Rel.NonTrivial (f) -> begin
(let g = (let _235517 = g
in {Microsoft_FStar_Tc_Rel.guard_f = Microsoft_FStar_Tc_Rel.Trivial; Microsoft_FStar_Tc_Rel.deferred = _235517.Microsoft_FStar_Tc_Rel.deferred; Microsoft_FStar_Tc_Rel.implicits = _235517.Microsoft_FStar_Tc_Rel.implicits})
in (let strengthen = (fun _235521 -> (match (_235521) with
| () -> begin
(let c = (lc.Microsoft_FStar_Absyn_Syntax.comp ())
in (let _235523 = if ((Microsoft_FStar_Tc_Env.debug env) Microsoft_FStar_Options.Extreme) then begin
...
in (let _235528 = (Microsoft_FStar_Tc_Env.wp_signature env Microsoft_FStar_Absyn_Const.pure_effect_lid)
in (match (_235528) with
| (a, kwp) -> begin
(let k = (Microsoft_FStar_Absyn_Util.subst_kind ((Support.Microsoft.FStar.Util.Inl ((a.Microsoft_FStar_Absyn_Syntax.v, t)))::[]) kwp)
...
in (let _235541 = if ((Microsoft_FStar_Tc_Env.debug env) Microsoft_FStar_Options.Extreme) then begin
...
in (let flags = ((Support.List.collect (fun _233943 -> (match (_233943) with
| (Microsoft_FStar_Absyn_Syntax.RETURN) | (Microsoft_FStar_Absyn_Syntax.PARTIAL_RETURN) -> begin
(Microsoft_FStar_Absyn_Syntax.PARTIAL_RETURN)::[]
end
| _ -> begin
...
in (let _235561 = (Microsoft_FStar_Options.hide_uvar_nums := false)
in (let _235563 = (Microsoft_FStar_Options.print_implicits := true)
in (let _235565 = (Microsoft_FStar_Tc_Errors.report r (Support.Microsoft.FStar.Util.format2 "Unconstrained unification variables %s in type signature %s; please add an annotation" union (Microsoft_FStar_Absyn_Print.typ_to_string t)))
in (let _235567 = (Microsoft_FStar_Options.hide_uvar_nums := hide_uvar_nums_saved)
in (Microsoft_FStar_Options.print_implicits := print_implicits_saved)))))))))))
end))

let gen = (fun verify env ecs -> if (not ((Support.Microsoft.FStar.Util.for_all (fun _235575 -> (match (_235575) with
| (_, c) -> begin
(Microsoft_FStar_Absyn_Util.is_pure_comp c)
end)) ecs))) then begin
None
end else begin
(let norm = (fun c -> (let _235578 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Medium) then begin
...
in (let _235582 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Medium) then begin
...
if ((Support.Microsoft.FStar.Util.for_some (fun _233944 -> (match (_233944) with
| (Support.Microsoft.FStar.Util.Inl (_), _) -> begin
true
end
| _ -> begin
...
in (let uvars = ((Support.List.map (fun _235607 -> (match (_235607) with
| (e, c) -> begin
...
in (let _235623 = if (((Microsoft_FStar_Options.should_verify env.Microsoft_FStar_Tc_Env.curmodule.Microsoft_FStar_Absyn_Syntax.str) && verify) && (not ((Microsoft_FStar_Absyn_Util.is_total_comp c)))) then begin
(let _235619 = (destruct_comp ct)
in (match (_235619) with
| (_, wp, _) -> begin
...
in (let ecs = ((Support.List.map (fun _235629 -> (match (_235629) with
| (uvs, e, c) -> begin
(let tvars = ((Support.List.map (fun _235632 -> (match (_235632) with
| (u, k) -> begin
...
in (let _235676 = (Microsoft_FStar_Absyn_Util.unchecked_unify u t)
...
let generalize = (fun verify env lecs -> (let _235703 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Low) then begin
(Support.Microsoft.FStar.Util.fprint1 "Generalizing: %s" ((Support.String.concat ", ") (Support.List.map (fun _235702 -> (match (_235702) with
| (lb, _, _) -> begin
(Microsoft_FStar_Absyn_Print.lbname_to_string lb)
end)) lecs)))
end
in (match ((gen verify env ((Support.List.map (fun _235709 -> (match (_235709) with
| (_, e, c) -> begin
(e, c)
end))) lecs))) with
| None -> begin
...
| Some (ecs) -> begin
(Support.List.map2 (fun _235718 _235721 -> (match ((_235718, _235721)) with
| ((l, _, _), (e, c)) -> begin
(let _235722 = if (Microsoft_FStar_Tc_Env.debug env Microsoft_FStar_Options.Medium) then begin
...
let check_top_level = (fun env g lc -> (let discharge = (fun g -> (let _235733 = (Microsoft_FStar_Tc_Rel.try_discharge_guard env g)
in (let _235751 = (match (((Support.List.tryFind (fun _233945 -> (match (_233945) with
...
in (let _235762 = (destruct_comp c)
in (match (_235762) with
| (t, wp, _) -> begin
...
let short_circuit_exp = (fun head seen_args -> (let short_bin_op_e = (fun f _233946 -> (match (_233946) with
| [] -> begin
None
end
...
in (match ((Support.Microsoft.FStar.Util.find_map table (fun _235792 -> (match (_235792) with
| (x, mk) -> begin
...
let short_circuit_typ = (fun head seen_args -> (let short_bin_op_t = (fun f _233947 -> (match (_233947) with
| [] -> begin
...
in (let short_op_ite = (fun _233948 -> (match (_233948) with
| [] -> begin
Microsoft_FStar_Tc_Rel.Trivial
end
...
(let lid = fv.Microsoft_FStar_Absyn_Syntax.v
in (match ((Support.Microsoft.FStar.Util.find_map table (fun _235860 -> (match (_235860) with
| (x, mk) -> begin
if (Microsoft_FStar_Absyn_Syntax.lid_equals x lid) then begin
Some ((mk seen_args))
end else begin
