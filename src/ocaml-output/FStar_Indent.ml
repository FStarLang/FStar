...
let generate : FStar_Parser_ParseIt.filename Prims.list  ->  unit = (fun filenames -> (

let parse_and_indent = (fun filename -> (

let uu____16 = (FStar_Parser_Driver.parse_file filename)
...
let uu____59 = (FStar_Parser_ToDocument.modul_with_comments_to_document modul comments1)
...
let uu____101 = (FStar_Parser_ToDocument.comments_to_document leftover_comments)
in (uu____101)::[])
in (FStar_Pprint.hardline)::uu____98)
in (FStar_Pprint.hardline)::uu____95)
in (FStar_Pprint.concat uu____92))
