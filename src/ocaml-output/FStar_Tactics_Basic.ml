  ...
  fun tm  ->
    with_cur_goal
      (fun goal  ->
                                      ...
                                      (let sub_goals =
                                       ...
                                       let sub_goals1 =
                                         ...
                                         then sub_goals
                                           ...
                                           ((let uu___98_1588 = goal in
                                             {
                                               context =
                                                 (uu___98_1588.context);
                                               witness = None;
                                               goal_ty = pre
                                             }))
                                           :: sub_goals in
         ...
         with | uu____1595 -> fail "apply_lemma: ill-typed term")
         ...
         try
           ...
           | (uu____1610,t,guard) ->
               ...
               then (solve goal tm; dismiss)
                 ...
                 (let msg =
                    let uu____1618 = FStar_Syntax_Print.term_to_string tm in
                    let uu____1619 = FStar_Syntax_Print.term_to_string t in
                    let uu____1620 =
                      FStar_Syntax_Print.term_to_string goal.goal_ty in
                    FStar_Util.format3
                      "%s : %s does not exactly solve the goal %s" uu____1618
                      uu____1619 uu____1620 in
                  fail msg)
         with
         | e ->
             let uu____1624 =
               let uu____1625 = FStar_Syntax_Print.term_to_string tm in
               FStar_Util.format1 "Term is not typeable: %s" uu____1625 in
             fail uu____1624)
      ...
      (fun goal  ->
         ...
         bind uu____1632
           (fun uu____1640  ->
                ...
                let uu____1643 =
                  let uu____1644 =
                    FStar_TypeChecker_Env.lookup_bv goal.context
                      (Prims.fst h) in
                  FStar_All.pipe_left Prims.fst uu____1644 in
                FStar_Syntax_Util.destruct_typ_as_formula uu____1643 in
              match uu____1641 with
              | Some (FStar_Syntax_Util.BaseConn
                  (l,uu____1651::(x,uu____1653)::(e,uu____1655)::[])) when
                  FStar_Ident.lid_equals l FStar_Syntax_Const.eq2_lid ->
                  let uu____1689 =
                    let uu____1690 = FStar_Syntax_Subst.compress x in
                    uu____1690.FStar_Syntax_Syntax.n in
                  (match uu____1689 with
                   | FStar_Syntax_Syntax.Tm_name x1 ->
                       let goal1 =
                         let uu___101_1696 = goal in
                         let uu____1697 =
                           FStar_Syntax_Subst.subst
                             [FStar_Syntax_Syntax.NT (x1, e)] goal.goal_ty in
                         {
                           context = (uu___101_1696.context);
                           witness = (uu___101_1696.witness);
                           goal_ty = uu____1697
                         } in
                       replace_cur goal1
                   | uu____1700 ->
              ...
              | uu____1701 -> fail "Not an equality hypothesis"))
  ...
  with_cur_goal
    (fun goal  ->
       ...
       | Some (x,env') ->
           ...
           let uu____1718 = FStar_Util.set_mem x fns in
           if uu____1718
           then fail "Cannot clear; variable appears in goal"
           else
             (let new_goal =
                let uu___102_1722 = goal in
                {
                  context = env';
                  witness = (uu___102_1722.witness);
                  goal_ty = (uu___102_1722.goal_ty)
                } in
              bind dismiss (fun uu____1723  -> add_goals [new_goal])))
let clear_hd: name -> Prims.unit tac =
  fun x  ->
    with_cur_goal
      (fun goal  ->
         let uu____1730 = FStar_TypeChecker_Env.pop_bv goal.context in
...
let revert: Prims.unit tac =
  with_cur_goal
    (fun goal  ->
       let uu____1745 = FStar_TypeChecker_Env.pop_bv goal.context in
       match uu____1745 with
       | None  -> fail "Cannot revert; empty context"
       | Some (x,env') ->
           let fvs = FStar_Syntax_Free.names goal.goal_ty in
           let new_goal =
             let uu____1759 = FStar_Util.set_mem x fvs in
             if uu____1759
             then
               let uu___103_1760 = goal in
               let uu____1761 =
                 let uu____1762 =
                   FStar_TypeChecker_TcTerm.universe_of env'
                     x.FStar_Syntax_Syntax.sort in
                 FStar_Syntax_Util.mk_forall uu____1762 x goal.goal_ty in
               {
                 context = env';
                 witness = (uu___103_1760.witness);
                 goal_ty = uu____1761
               }
             else
               (let uu___104_1764 = goal in
                let uu____1765 =
                  FStar_Syntax_Util.mk_imp x.FStar_Syntax_Syntax.sort
                    goal.goal_ty in
                {
                  context = env';
                  witness = (uu___104_1764.witness);
                  goal_ty = uu____1765
                }) in
           bind dismiss (fun uu____1766  -> add_goals [new_goal]))
let revert_hd: name -> Prims.unit tac =
  fun x  ->
    with_cur_goal
      (fun goal  ->
         let uu____1773 = FStar_TypeChecker_Env.pop_bv goal.context in
         ...
         | Some (y,env') ->
             ...
             then
               let uu____1785 =
                 let uu____1786 = FStar_Syntax_Print.bv_to_string x in
                 let uu____1787 = FStar_Syntax_Print.bv_to_string y in
                 FStar_Util.format2
                   "Cannot revert_hd %s; head variable mismatch ... egot %s"
                   uu____1786 uu____1787 in
               fail uu____1785
             else revert)
...
let destruct_equality_imp:
  ...
  =
  fun t  ->
    let uu____1832 = FStar_Syntax_Util.destruct_typ_as_formula t in
    match uu____1832 with
    | Some (FStar_Syntax_Util.BaseConn
        (l,(lhs,uu____1844)::(rhs,uu____1846)::[])) when
        FStar_Ident.lid_equals l FStar_Syntax_Const.imp_lid ->
        let uu____1872 = FStar_Syntax_Util.destruct_typ_as_formula lhs in
        (match uu____1872 with
         | Some (FStar_Syntax_Util.BaseConn (eq1,_::(x,_)::(e,_)::[]))|Some
           (FStar_Syntax_Util.BaseConn (eq1,(x,_)::(e,_)::[])) when
             ...
             ->
             let uu____1944 =
               let uu____1952 = as_name x in (uu____1952, e, rhs) in
             Some uu____1944
         | uu____1964 -> None)
    | uu____1973 -> None
let at_most_one t =
    ...
    (fun a  ->
       bind get
         (fun p  ->
            match p.goals with
            | []|_::[] -> ret a
            | uu____1996 -> fail "expected at most one goal remaining"))
  ...
  bind get
    (fun p  ->
       ...
       | g1::g2::rest ->
           ...
           then
             let uu____2007 =
               let uu___105_2008 = p in
               let uu____2009 =
                 let uu____2011 = conj_goals g1 g2 in uu____2011 :: rest in
               {
                 main_context = (uu___105_2008.main_context);
                 main_goal = (uu___105_2008.main_goal);
                 all_implicits = (uu___105_2008.all_implicits);
                 goals = uu____2009;
                 smt_goals = (uu___105_2008.smt_goals);
                 transaction = (uu___105_2008.transaction)
               } in
             set uu____2007
           else
             (let g1_binders =
                let uu____2014 = FStar_TypeChecker_Env.all_binders g1.context in
                FStar_All.pipe_right uu____2014
                  (FStar_Syntax_Print.binders_to_string ", ") in
              let g2_binders =
                let uu____2016 = FStar_TypeChecker_Env.all_binders g2.context in
                FStar_All.pipe_right uu____2016
                  (FStar_Syntax_Print.binders_to_string ", ") in
              let uu____2017 =
                let uu____2018 = goal_to_string g1 in
                let uu____2019 = goal_to_string g2 in
                let uu____2020 =
                  let uu____2021 =
                    FStar_TypeChecker_Env.eq_gamma g1.context g2.context in
                  FStar_All.pipe_right uu____2021 FStar_Util.string_of_bool in
                FStar_Util.format3
                  "Cannot merge sub-goals: incompatible contexts:\ng1=%s\ng2=%s\neq_gamma=%s\n"
                  uu____2018 uu____2019 uu____2020 in
              fail uu____2017)
       | uu____2022 ->
           let goals =
             let uu____2025 =
             ...
             FStar_All.pipe_right uu____2025 (FStar_String.concat "\n\t") in
           let uu____2031 =
             FStar_Util.format1
  ...
  fun s  ->
    with_cur_goal
      (fun g  ->
         ...
         let g' =
           let uu___106_2041 = g in
           {
             context = ctx';
             witness = (uu___106_2041.witness);
             goal_ty = (uu___106_2041.goal_ty)
           } in
         bind dismiss (fun uu____2042  -> add_goals [g']))
      ...
      (fun g  ->
         ...
         let ctx' =
           FStar_TypeChecker_Env.add_proof_ns ctx
             (FStar_Ident.path_of_text s) in
         let g' =
           let uu___107_2052 = g in
           {
             context = ctx';
             witness = (uu___107_2052.witness);
             goal_ty = (uu___107_2052.goal_ty)
           } in
         bind dismiss (fun uu____2053  -> add_goals [g']))
      ...
      fun t  ->
        ...
        let tn1 =
          ...
          | FStar_Syntax_Syntax.Tm_app (hd1,args) ->
              ...
              let uu____2097 =
                let uu____2107 = ff hd1 in
                let uu____2108 =
                  FStar_List.map
                    (fun uu____2116  ->
                       match uu____2116 with
                       | (a,q) -> let uu____2123 = ff a in (uu____2123, q))
                    args in
                (uu____2107, uu____2108) in
              FStar_Syntax_Syntax.Tm_app uu____2097
          | FStar_Syntax_Syntax.Tm_abs (bs,t1,k) ->
              let uu____2152 = FStar_Syntax_Subst.open_term bs t1 in
              (match uu____2152 with
               | (bs1,t') ->
                   let t'' =
                     let uu____2158 =
                       FStar_TypeChecker_Env.push_binders env bs1 in
                     bottom_fold_env f uu____2158 t' in
                   let uu____2159 =
                     let uu____2174 = FStar_Syntax_Subst.close bs1 t'' in
                     (bs1, uu____2174, k) in
                   FStar_Syntax_Syntax.Tm_abs uu____2159)
          | FStar_Syntax_Syntax.Tm_arrow (bs,k) -> tn
          | uu____2193 -> tn in
        f env
          (let uu___108_2194 = t in
           {
             FStar_Syntax_Syntax.n = tn1;
             FStar_Syntax_Syntax.tk = (uu___108_2194.FStar_Syntax_Syntax.tk);
             FStar_Syntax_Syntax.pos =
               (uu___108_2194.FStar_Syntax_Syntax.pos);
             FStar_Syntax_Syntax.vars =
               (uu___108_2194.FStar_Syntax_Syntax.vars)
           })
let rec mapM f l =
  match l with
  | [] -> ret []
  | x::xs ->
      let uu____2228 = f x in
      bind uu____2228
        (fun y  ->
           let uu____2232 = mapM f xs in
           bind uu____2232 (fun ys  -> ret (y :: ys)))
let rec tac_bottom_fold_env:
  ...
  fun f  ->
    fun env  ->
      fun t  ->
        let tn =
          let uu____2264 = FStar_Syntax_Subst.compress t in
          uu____2264.FStar_Syntax_Syntax.n in
        let tn1 =
          match tn with
          | FStar_Syntax_Syntax.Tm_app (hd1,args) ->
              let ff = tac_bottom_fold_env f env in
              let uu____2290 = ff hd1 in
              bind uu____2290
                (fun hd2  ->
                   let fa uu____2301 =
                     match uu____2301 with
                     | (a,q) ->
                         let uu____2309 = ff a in
                         bind uu____2309 (fun a1  -> ret (a1, q)) in
                   let uu____2316 = mapM fa args in
                   bind uu____2316
                     (fun args1  ->
              ...
              (match uu____2360 with
               | (bs1,t') ->
                   let uu____2366 =
                     let uu____2368 =
                       FStar_TypeChecker_Env.push_binders env bs1 in
                     tac_bottom_fold_env f uu____2368 t' in
                   bind uu____2366
                     (fun t''  ->
                        let uu____2370 =
                          let uu____2371 =
                            let uu____2386 = FStar_Syntax_Subst.close bs1 t'' in
                            (bs1, uu____2386, k) in
                          FStar_Syntax_Syntax.Tm_abs uu____2371 in
                        ret uu____2370))
          | FStar_Syntax_Syntax.Tm_arrow (bs,k) -> ret tn
          | uu____2405 -> ret tn in
        bind tn1
          (fun tn2  ->
             f env
               (let uu___109_2407 = t in
                {
                  FStar_Syntax_Syntax.n = tn2;
                  FStar_Syntax_Syntax.tk =
                    (uu___109_2407.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos =
                    (uu___109_2407.FStar_Syntax_Syntax.pos);
                  FStar_Syntax_Syntax.vars =
                    (uu___109_2407.FStar_Syntax_Syntax.vars)
                }))
let pointwise_rec:
  proofstate ->
    Prims.unit tac ->
  ...
  fun ps  ->
    fun tau  ->
      fun env  ->
        fun t  ->
          let env1 =
            let uu___110_2429 = env in
            {
              FStar_TypeChecker_Env.solver =
                (uu___110_2429.FStar_TypeChecker_Env.solver);
              FStar_TypeChecker_Env.range =
                (uu___110_2429.FStar_TypeChecker_Env.range);
              FStar_TypeChecker_Env.curmodule =
                (uu___110_2429.FStar_TypeChecker_Env.curmodule);
              FStar_TypeChecker_Env.gamma =
                (uu___110_2429.FStar_TypeChecker_Env.gamma);
              FStar_TypeChecker_Env.gamma_cache =
                (uu___110_2429.FStar_TypeChecker_Env.gamma_cache);
              FStar_TypeChecker_Env.modules =
                (uu___110_2429.FStar_TypeChecker_Env.modules);
              FStar_TypeChecker_Env.expected_typ =
                (uu___110_2429.FStar_TypeChecker_Env.expected_typ);
              FStar_TypeChecker_Env.sigtab =
                (uu___110_2429.FStar_TypeChecker_Env.sigtab);
              FStar_TypeChecker_Env.is_pattern =
                (uu___110_2429.FStar_TypeChecker_Env.is_pattern);
              FStar_TypeChecker_Env.instantiate_imp = false;
              FStar_TypeChecker_Env.effects =
                (uu___110_2429.FStar_TypeChecker_Env.effects);
              FStar_TypeChecker_Env.generalize =
                (uu___110_2429.FStar_TypeChecker_Env.generalize);
              FStar_TypeChecker_Env.letrecs =
                (uu___110_2429.FStar_TypeChecker_Env.letrecs);
              FStar_TypeChecker_Env.top_level =
                (uu___110_2429.FStar_TypeChecker_Env.top_level);
              FStar_TypeChecker_Env.check_uvars =
                (uu___110_2429.FStar_TypeChecker_Env.check_uvars);
              FStar_TypeChecker_Env.use_eq =
                (uu___110_2429.FStar_TypeChecker_Env.use_eq);
              FStar_TypeChecker_Env.is_iface =
                (uu___110_2429.FStar_TypeChecker_Env.is_iface);
              FStar_TypeChecker_Env.admit =
                (uu___110_2429.FStar_TypeChecker_Env.admit);
              FStar_TypeChecker_Env.lax =
                (uu___110_2429.FStar_TypeChecker_Env.lax);
              FStar_TypeChecker_Env.lax_universes =
                (uu___110_2429.FStar_TypeChecker_Env.lax_universes);
              FStar_TypeChecker_Env.type_of =
                (uu___110_2429.FStar_TypeChecker_Env.type_of);
              FStar_TypeChecker_Env.universe_of =
                (uu___110_2429.FStar_TypeChecker_Env.universe_of);
              FStar_TypeChecker_Env.use_bv_sorts =
                (uu___110_2429.FStar_TypeChecker_Env.use_bv_sorts);
              FStar_TypeChecker_Env.qname_and_index =
                (uu___110_2429.FStar_TypeChecker_Env.qname_and_index);
              FStar_TypeChecker_Env.proof_ns =
                (uu___110_2429.FStar_TypeChecker_Env.proof_ns)
            } in
          let uu____2430 = FStar_TypeChecker_TcTerm.tc_term env1 t in
          match uu____2430 with
          | (t1,lcomp,g) ->
              let uu____2438 =
                (let uu____2439 = FStar_Syntax_Util.is_total_lcomp lcomp in
                 Prims.op_Negation uu____2439) ||
                  (let uu____2440 = FStar_TypeChecker_Rel.is_trivial g in
                   Prims.op_Negation uu____2440) in
              if uu____2438
              then ret t1
              else
                (let typ = lcomp.FStar_Syntax_Syntax.res_typ in
                 ...
                 | (ut,uvs,guard) ->
                     ((let uu____2464 = FStar_ST.read tacdbg in
                       if uu____2464
                       then
                         let uu____2467 =
                      ...
                      (let g' =
                         let uu____2471 =
                           ...
                           FStar_Syntax_Util.mk_eq2 uu____2472 typ t1 ut in
                         {
                           context = env1;
                           witness = None;
                           goal_ty = uu____2471
                         } in
                       let uu____2473 = add_goals [g'] in
                       bind uu____2473
                         (fun uu____2475  ->
                            let uu____2476 =
                              bind tau
    ...
    bind get
      (fun ps  ->
         ...
         | (g,gs) ->
             ...
             ((let uu____2512 = FStar_ST.read tacdbg in
               if uu____2512
               then
                 let uu____2515 = FStar_Syntax_Print.term_to_string gt1 in
              ...
              bind dismiss_all
                (fun uu____2517  ->
                   ...
                   bind uu____2518
                     (fun gt'  ->
                        (let uu____2522 = FStar_ST.read tacdbg in
                         if uu____2522
                         then
                           let uu____2525 =
                        ...
                        (let uu____2527 = push_goals gs in
                         bind uu____2527
                           (fun uu____2529  ->
                              add_goals
                                [(let uu___111_2530 = g in
                                  {
                                    context = (uu___111_2530.context);
                                    witness = (uu___111_2530.witness);
                                    goal_ty = gt'
                                  })]))))))
let refl: Prims.unit tac =
  with_cur_goal
    (fun g  ->
       let uu____2533 = FStar_Syntax_Util.head_and_args' g.goal_ty in
       match uu____2533 with
       | (hd1,args) ->
           let uu____2554 =
             let uu____2562 =
               let uu____2563 = FStar_Syntax_Util.un_uinst hd1 in
               uu____2563.FStar_Syntax_Syntax.n in
             (uu____2562, args) in
           (match uu____2554 with
            | (FStar_Syntax_Syntax.Tm_fvar
               fv,uu____2573::(l,uu____2575)::(r,uu____2577)::[]) when
            ...
            | (hd2,uu____2615) ->
                let uu____2626 =
                  let uu____2627 =
                    FStar_Syntax_Print.term_to_string
                      (let uu___112_2628 = g.goal_ty in
                       {
                         FStar_Syntax_Syntax.n = hd2;
                         FStar_Syntax_Syntax.tk =
                           (uu___112_2628.FStar_Syntax_Syntax.tk);
                         FStar_Syntax_Syntax.pos =
                           (uu___112_2628.FStar_Syntax_Syntax.pos);
                         FStar_Syntax_Syntax.vars =
                           (uu___112_2628.FStar_Syntax_Syntax.vars)
                       }) in
                  FStar_Util.format1 "refl: not an equality (%s)" uu____2627 in
                fail uu____2626))
let later: Prims.unit tac =
  bind get
    (fun ps  ->
       ...
       | g::gs ->
           set
             (let uu___113_2639 = ps in
              {
                main_context = (uu___113_2639.main_context);
                main_goal = (uu___113_2639.main_goal);
                all_implicits = (uu___113_2639.all_implicits);
                goals = (FStar_List.append gs [g]);
                smt_goals = (uu___113_2639.smt_goals);
                transaction = (uu___113_2639.transaction)
              }))
type order =
  | Lt
  | Eq
...
let uu___is_Lt: order -> Prims.bool =
  fun projectee  -> match projectee with | Lt  -> true | uu____2643 -> false
let uu___is_Eq: order -> Prims.bool =
  fun projectee  -> match projectee with | Eq  -> true | uu____2647 -> false
let uu___is_Gt: order -> Prims.bool =
  fun projectee  -> match projectee with | Gt  -> true | uu____2651 -> false
let order_binder:
  ...
  fun env  ->
    fun g  ->
      let g1 =
        ...
        { context = env; witness = None; goal_ty = uu____2668 } in
      let uu____2669 = FStar_Unionfind.new_transaction () in
      {
        main_context = env;
        main_goal = g1;
        all_implicits = [];
