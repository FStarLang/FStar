                                             ...
                                             FStar_All.pipe_right implicits1
                                               (FStar_List.map
                                                  (fun uu____3560  ->
                                                     ...
                                                     | (_msg,_env,_uvar,term,typ,uu____3578)
                                                         ->
                                                         let uu____3579 =
                                                           bnorm goal.context
                                                             term in
  ...
  fun f  ->
    fun l  ->
      ...
      | x::xs ->
          ...
          bind uu____4176
            (fun y  ->
               let uu____4184 = mapM f xs in
               bind uu____4184 (fun ys  -> ret (y :: ys)))
let rec tac_bottom_fold_env:
  ...
  fun f  ->
    fun env  ->
      fun t  ->
        let tn =
          let uu____4230 = FStar_Syntax_Subst.compress t in
          uu____4230.FStar_Syntax_Syntax.n in
        let tn1 =
          match tn with
          | FStar_Syntax_Syntax.Tm_app (hd1,args) ->
              let ff = tac_bottom_fold_env f env in
              let uu____4265 = ff hd1 in
              bind uu____4265
                (fun hd2  ->
                   let fa uu____4285 =
                     match uu____4285 with
                     | (a,q) ->
                         let uu____4298 = ff a in
                         bind uu____4298 (fun a1  -> ret (a1, q)) in
                   let uu____4311 = mapM fa args in
                   bind uu____4311
                     (fun args1  ->
                        ret (FStar_Syntax_Syntax.Tm_app (hd2, args1))))
          | FStar_Syntax_Syntax.Tm_abs (bs,t1,k) ->
              let uu____4371 = FStar_Syntax_Subst.open_term bs t1 in
              (match uu____4371 with
               | (bs1,t') ->
                   let uu____4380 =
                     let uu____4383 =
                       FStar_TypeChecker_Env.push_binders env bs1 in
                     tac_bottom_fold_env f uu____4383 t' in
                   bind uu____4380
                     (fun t''  ->
                        let uu____4387 =
                          let uu____4388 =
                            ...
                            let uu____4406 = FStar_Syntax_Subst.close bs1 t'' in
                            (uu____4405, uu____4406, k) in
                          FStar_Syntax_Syntax.Tm_abs uu____4388 in
                        ret uu____4387))
          | FStar_Syntax_Syntax.Tm_arrow (bs,k) -> ret tn
          | uu____4427 -> ret tn in
        bind tn1
          (fun tn2  ->
             f env
               (let uu___110_4431 = t in
                {
                  FStar_Syntax_Syntax.n = tn2;
                  FStar_Syntax_Syntax.pos =
                    (uu___110_4431.FStar_Syntax_Syntax.pos);
                  FStar_Syntax_Syntax.vars =
                    (uu___110_4431.FStar_Syntax_Syntax.vars)
                }))
let pointwise_rec:
  proofstate ->
    Prims.unit tac ->
  ...
  fun ps  ->
    fun tau  ->
      fun opts  ->
        fun env  ->
          fun t  ->
            let uu____4460 = FStar_TypeChecker_TcTerm.tc_term env t in
            match uu____4460 with
            | (t1,lcomp,g) ->
                let uu____4472 =
                  (let uu____4475 = FStar_Syntax_Util.is_total_lcomp lcomp in
                   Prims.op_Negation uu____4475) ||
                    (let uu____4477 = FStar_TypeChecker_Rel.is_trivial g in
                     Prims.op_Negation uu____4477) in
                if uu____4472
                then ret t1
                else
                  (let typ = lcomp.FStar_Syntax_Syntax.res_typ in
                   let uu____4484 = new_uvar env typ in
                   bind uu____4484
                     (fun ut  ->
                        log ps
                          (fun uu____4495  ->
                        ...
                        (let uu____4498 =
                           let uu____4501 =
                             ...
                             FStar_Syntax_Util.mk_eq2 uu____4502 typ t1 ut in
                           add_irrelevant_goal env uu____4501 opts in
                         bind uu____4498
                           (fun uu____4505  ->
                              let uu____4506 =
                                bind tau
    ...
    bind get
      (fun ps  ->
         ...
         | (g,gs) ->
             ...
             (log ps
                (fun uu____4569  ->
                   let uu____4570 = FStar_Syntax_Print.term_to_string gt1 in
                        ...
                        log ps
                          (fun uu____4584  ->
                             ...
                             FStar_Util.print1
                               ...
                               uu____4585);
                        (let uu____4586 = push_goals gs in
                         bind uu____4586
                           (fun uu____4590  ->
                              add_goals
                                [(let uu___111_4592 = g in
                                  {
                                    context = (uu___111_4592.context);
                                    witness = (uu___111_4592.witness);
                                    goal_ty = gt';
                                    opts = (uu___111_4592.opts)
                                  })]))))))
let trefl: Prims.unit tac =
  bind cur_goal
    (fun g  ->
       let uu____4612 = FStar_Syntax_Util.un_squash g.goal_ty in
       match uu____4612 with
       | FStar_Pervasives_Native.Some t ->
           let uu____4624 = FStar_Syntax_Util.head_and_args' t in
           (match uu____4624 with
            | (hd1,args) ->
                let uu____4657 =
                  let uu____4670 =
                    let uu____4671 = FStar_Syntax_Util.un_uinst hd1 in
                    uu____4671.FStar_Syntax_Syntax.n in
                  (uu____4670, args) in
                (match uu____4657 with
                 | (FStar_Syntax_Syntax.Tm_fvar
                    fv,uu____4685::(l,uu____4687)::(r,uu____4689)::[]) when
                     ...
                     if uu____4736
                     then
                       let uu____4740 = FStar_Syntax_Print.term_to_string l in
                       let uu____4741 = FStar_Syntax_Print.term_to_string r in
                       fail2 "trefl: not a trivial equality (%s vs %s)"
                         uu____4740 uu____4741
                     else (solve g FStar_Syntax_Util.exp_unit; dismiss)
                 | (hd2,uu____4745) ->
                     let uu____4762 = FStar_Syntax_Print.term_to_string t in
                     fail1 "trefl: not an equality (%s)" uu____4762))
       | FStar_Pervasives_Native.None  -> fail "not an irrelevant goal")
let dup: Prims.unit tac =
  bind cur_goal
    (fun g  ->
       let uu____4770 = new_uvar g.context g.goal_ty in
       bind uu____4770
         (fun u  ->
            let g' =
              let uu___112_4777 = g in
              {
                context = (uu___112_4777.context);
                witness = u;
                goal_ty = (uu___112_4777.goal_ty);
                opts = (uu___112_4777.opts)
              } in
            bind dismiss
              (fun uu____4780  ->
                 let uu____4781 =
                   let uu____4784 =
                     let uu____4785 =
                       FStar_TypeChecker_TcTerm.universe_of g.context
                         g.goal_ty in
                     FStar_Syntax_Util.mk_eq2 uu____4785 g.goal_ty u
                       g.witness in
                   add_irrelevant_goal g.context uu____4784 g.opts in
                 bind uu____4781
                   (fun uu____4788  ->
                      let uu____4789 = add_goals [g'] in
                      bind uu____4789 (fun uu____4793  -> ret ())))))
let flip: Prims.unit tac =
  bind get
    (fun ps  ->
       ...
       | g1::g2::gs ->
           set
             (let uu___113_4810 = ps in
              {
                main_context = (uu___113_4810.main_context);
                main_goal = (uu___113_4810.main_goal);
                all_implicits = (uu___113_4810.all_implicits);
                goals = (g2 :: g1 :: gs);
                smt_goals = (uu___113_4810.smt_goals)
              })
       | uu____4811 -> fail "flip: less than 2 goals")
let later: Prims.unit tac =
  bind get
    (fun ps  ->
       ...
       | g::gs ->
           set
             (let uu___114_4826 = ps in
              {
                main_context = (uu___114_4826.main_context);
                main_goal = (uu___114_4826.main_goal);
                all_implicits = (uu___114_4826.all_implicits);
                goals = (FStar_List.append gs [g]);
                smt_goals = (uu___114_4826.smt_goals)
              }))
let qed: Prims.unit tac =
  bind get
    (fun ps  ->
       match ps.goals with | [] -> ret () | uu____4833 -> fail "Not done!")
let cases:
  ...
  fun t  ->
    bind cur_goal
      (fun g  ->
         ...
         | (t1,typ,guard) ->
             let uu____4891 = FStar_Syntax_Util.head_and_args typ in
             (match uu____4891 with
              | (hd1,args) ->
                  let uu____4934 =
                    let uu____4947 =
                      let uu____4948 = FStar_Syntax_Util.un_uinst hd1 in
                      uu____4948.FStar_Syntax_Syntax.n in
                    (uu____4947, args) in
                  (match uu____4934 with
                   | (FStar_Syntax_Syntax.Tm_fvar
                      fv,(p,uu____4967)::(q,uu____4969)::[]) when
                       FStar_Syntax_Syntax.fv_eq_lid fv
                       ...
                       let g1 =
                         let uu___115_5007 = g in
                         let uu____5008 =
                           FStar_TypeChecker_Env.push_bv g.context v_p in
                         {
                           context = uu____5008;
                           witness = (uu___115_5007.witness);
                           goal_ty = (uu___115_5007.goal_ty);
                           opts = (uu___115_5007.opts)
                         } in
                       let g2 =
                         let uu___116_5010 = g in
                         let uu____5011 =
                           FStar_TypeChecker_Env.push_bv g.context v_q in
                         {
                           context = uu____5011;
                           witness = (uu___116_5010.witness);
                           goal_ty = (uu___116_5010.goal_ty);
                           opts = (uu___116_5010.opts)
                         } in
                       bind dismiss
                         (fun uu____5018  ->
                            let uu____5019 = add_goals [g1; g2] in
                            bind uu____5019
                              (fun uu____5028  ->
                                 let uu____5029 =
                                   let uu____5034 =
                                 ...
                                 ret uu____5029))
                   | uu____5040 ->
                       let uu____5053 = FStar_Syntax_Print.term_to_string typ in
                       fail1 "Not a disjunction: %s" uu____5053)))
let set_options: Prims.string -> Prims.unit tac =
  fun s  ->
    bind cur_goal
      (fun g  ->
         FStar_Options.push ();
         (let uu____5076 = FStar_Util.smap_copy g.opts in
          FStar_Options.set uu____5076);
         (let res = FStar_Options.set_options FStar_Options.Set s in
          ...
          (match res with
           | FStar_Getopt.Success  ->
               let g' =
                 let uu___117_5083 = g in
                 {
                   context = (uu___117_5083.context);
                   witness = (uu___117_5083.witness);
                   goal_ty = (uu___117_5083.goal_ty);
                   opts = opts'
                 } in
               replace_cur g'
  ...
  fun env  ->
    fun ty  ->
      let uu____5153 =
        ...
        | FStar_Pervasives_Native.None  ->
            let uu____5159 =
              let uu____5160 = FStar_Syntax_Util.type_u () in
              FStar_All.pipe_left FStar_Pervasives_Native.fst uu____5160 in
            new_uvar env uu____5159 in
      bind uu____5153
        (fun typ  ->
           let uu____5172 = new_uvar env typ in
           bind uu____5172 (fun t  -> ret t))
let unify:
...
let goal_of_goal_ty:
  ...
  fun env  ->
    fun typ  ->
      ...
      | (u,uu____5231,g_u) ->
          let g =
            let uu____5246 = FStar_Options.peek () in
            { context = env; witness = u; goal_ty = typ; opts = uu____5246 } in
          (g, g_u)
let proofstate_of_goal_ty:
  ...
  fun env  ->
    fun typ  ->
      let uu____5263 = goal_of_goal_ty env typ in
