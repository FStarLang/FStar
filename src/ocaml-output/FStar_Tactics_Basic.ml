      ...
      (fun goal  ->
         ...
         bind uu____2057
           (fun uu____2065  ->
              ...
              | uu____2129 -> fail "Not an equality hypothesis"))
let clear: Prims.unit tac =
  bind cur_goal
    (fun goal  ->
       let uu____2133 = FStar_TypeChecker_Env.pop_bv goal.context in
       ...
       | Some (x,env') ->
           ...
           let uu____2148 = FStar_Util.set_mem x fns_ty in
           if uu____2148
             ...
             (let uu____2151 =
              ...
              | (u,uu____2160,g) ->
                  ...
                  then fail "clear: unification failed"
                    ...
                    (let new_goal =
                       let uu___95_2173 = goal in
                       let uu____2174 = bnorm env' u in
                       {
                         context = env';
                         witness = uu____2174;
                         goal_ty = (uu___95_2173.goal_ty)
                       } in
                     bind dismiss (fun uu____2175  -> add_goals [new_goal]))))
let clear_hd: name -> Prims.unit tac =
  fun x  ->
    bind cur_goal
      (fun goal  ->
         let uu____2182 = FStar_TypeChecker_Env.pop_bv goal.context in
...
let revert: Prims.unit tac =
  bind cur_goal
    (fun goal  ->
       let uu____2197 = FStar_TypeChecker_Env.pop_bv goal.context in
       match uu____2197 with
       | None  -> fail "Cannot revert; empty context"
       | Some (x,env') ->
           let typ' =
             let uu____2211 = FStar_Syntax_Syntax.mk_Total goal.goal_ty in
             FStar_Syntax_Util.arrow [(x, None)] uu____2211 in
           let w' = FStar_Syntax_Util.abs [(x, None)] goal.witness None in
           replace_cur
             (let uu___96_2234 = goal in
              { context = env'; witness = w'; goal_ty = typ' }))
let revert_hd: name -> Prims.unit tac =
  fun x  ->
    bind cur_goal
      (fun goal  ->
         let uu____2241 = FStar_TypeChecker_Env.pop_bv goal.context in
         ...
         | Some (y,env') ->
             ...
             then
               let uu____2253 =
                 let uu____2254 = FStar_Syntax_Print.bv_to_string x in
                 let uu____2255 = FStar_Syntax_Print.bv_to_string y in
                 FStar_Util.format2
...
let rec revert_all_hd: name Prims.list -> Prims.unit tac =
  fun xs  ->
    ...
    | [] -> ret ()
    | x::xs1 ->
        let uu____2268 = revert_all_hd xs1 in
        bind uu____2268 (fun uu____2270  -> revert_hd x)
  ...
  fun s  ->
    bind cur_goal
      (fun g  ->
         ...
         let g' =
           let uu___97_2280 = g in
           {
             context = ctx';
             witness = (uu___97_2280.witness);
             goal_ty = (uu___97_2280.goal_ty)
           } in
         bind dismiss (fun uu____2281  -> add_goals [g']))
      ...
      (fun g  ->
         ...
         let ctx' =
           FStar_TypeChecker_Env.add_proof_ns ctx
             (FStar_Ident.path_of_text s) in
         let g' =
           let uu___98_2291 = g in
           {
             context = ctx';
             witness = (uu___98_2291.witness);
             goal_ty = (uu___98_2291.goal_ty)
           } in
         bind dismiss (fun uu____2292  -> add_goals [g']))
let rec bottom_fold_env:
  ...
  fun f  ->
    fun env  ->
      fun t  ->
        let tn =
          let uu____2313 = FStar_Syntax_Subst.compress t in
                ...
                let uu____2347 =
                  FStar_List.map
                    (fun uu____2355  ->
                       ...
                       | (a,q) -> let uu____2362 = ff a in (uu____2362, q))
                    args in
                (uu____2346, uu____2347) in
              FStar_Syntax_Syntax.Tm_app uu____2336
          | FStar_Syntax_Syntax.Tm_abs (bs,t1,k) ->
              let uu____2391 = FStar_Syntax_Subst.open_term bs t1 in
              (match uu____2391 with
               | (bs1,t') ->
                   let t'' =
                     let uu____2397 =
                       FStar_TypeChecker_Env.push_binders env bs1 in
                     bottom_fold_env f uu____2397 t' in
                   let uu____2398 =
                     let uu____2413 = FStar_Syntax_Subst.close bs1 t'' in
                     (bs1, uu____2413, k) in
                   FStar_Syntax_Syntax.Tm_abs uu____2398)
          | FStar_Syntax_Syntax.Tm_arrow (bs,k) -> tn
          | uu____2432 -> tn in
        f env
          (let uu___99_2433 = t in
           {
             FStar_Syntax_Syntax.n = tn1;
             FStar_Syntax_Syntax.tk = (uu___99_2433.FStar_Syntax_Syntax.tk);
             FStar_Syntax_Syntax.pos = (uu___99_2433.FStar_Syntax_Syntax.pos);
             FStar_Syntax_Syntax.vars =
               (uu___99_2433.FStar_Syntax_Syntax.vars)
           })
let rec mapM f l =
  match l with
  | [] -> ret []
  | x::xs ->
      let uu____2467 = f x in
      bind uu____2467
        (fun y  ->
           let uu____2471 = mapM f xs in
           bind uu____2471 (fun ys  -> ret (y :: ys)))
      ...
      fun t  ->
        ...
        let tn1 =
          ...
          | FStar_Syntax_Syntax.Tm_app (hd1,args) ->
              ...
              let uu____2529 = ff hd1 in
              bind uu____2529
                (fun hd2  ->
                   let fa uu____2540 =
                     match uu____2540 with
                     | (a,q) ->
                         let uu____2548 = ff a in
                         bind uu____2548 (fun a1  -> ret (a1, q)) in
                   let uu____2555 = mapM fa args in
                   bind uu____2555
                     (fun args1  ->
                        ret (FStar_Syntax_Syntax.Tm_app (hd2, args1))))
          | FStar_Syntax_Syntax.Tm_abs (bs,t1,k) ->
              let uu____2599 = FStar_Syntax_Subst.open_term bs t1 in
              (match uu____2599 with
               | (bs1,t') ->
                   let uu____2605 =
                     let uu____2607 =
                       FStar_TypeChecker_Env.push_binders env bs1 in
                     tac_bottom_fold_env f uu____2607 t' in
                   bind uu____2605
                     (fun t''  ->
                        let uu____2609 =
                          let uu____2610 =
                            let uu____2625 = FStar_Syntax_Subst.close bs1 t'' in
                            (bs1, uu____2625, k) in
                          FStar_Syntax_Syntax.Tm_abs uu____2610 in
                        ret uu____2609))
          | FStar_Syntax_Syntax.Tm_arrow (bs,k) -> ret tn
          | uu____2644 -> ret tn in
        bind tn1
          (fun tn2  ->
             f env
               (let uu___100_2646 = t in
                {
                  FStar_Syntax_Syntax.n = tn2;
                  FStar_Syntax_Syntax.tk =
                    (uu___100_2646.FStar_Syntax_Syntax.tk);
                  FStar_Syntax_Syntax.pos =
                    (uu___100_2646.FStar_Syntax_Syntax.pos);
                  FStar_Syntax_Syntax.vars =
                    (uu___100_2646.FStar_Syntax_Syntax.vars)
                }))
let pointwise_rec:
  proofstate ->
    Prims.unit tac ->
  ...
  fun ps  ->
    fun tau  ->
      fun env  ->
        fun t  ->
          let uu____2667 = FStar_TypeChecker_TcTerm.tc_term env t in
          match uu____2667 with
          | (t1,lcomp,g) ->
              let uu____2675 =
                (let uu____2676 = FStar_Syntax_Util.is_total_lcomp lcomp in
                 Prims.op_Negation uu____2676) ||
                  (let uu____2677 = FStar_TypeChecker_Rel.is_trivial g in
                     ...
                     (log ps
                      ...
                      (let g' =
                         let uu____2705 =
                           ...
                           FStar_Syntax_Util.mk_eq2 uu____2706 typ t1 ut in
                         mk_irrelevant env uu____2705 in
                       let uu____2707 = add_goals [g'] in
                       bind uu____2707
                         (fun uu____2709  ->
                            let uu____2710 =
                              bind tau
                                (fun uu____2712  ->
                                   let guard1 =
                                     let uu____2714 =
                                       FStar_TypeChecker_Rel.solve_deferred_constraints
                                         env guard in
                                     FStar_All.pipe_right uu____2714
                                       FStar_TypeChecker_Rel.resolve_implicits in
    ...
    bind get
      (fun ps  ->
         ...
         | (g,gs) ->
             ...
             (log ps
                (fun uu____2746  ->
                   let uu____2747 = FStar_Syntax_Print.term_to_string gt1 in
                        ...
                        log ps
                          (fun uu____2753  ->
                             ...
                             FStar_Util.print1
                               ...
                               uu____2754);
                        (let uu____2755 = push_goals gs in
                         bind uu____2755
                           (fun uu____2757  ->
                              add_goals
                                [(let uu___101_2758 = g in
                                  {
                                    context = (uu___101_2758.context);
                                    witness = (uu___101_2758.witness);
                                    goal_ty = gt'
                                  })]))))))
let trefl: Prims.unit tac =
  bind cur_goal
    (fun g  ->
       let uu____2761 = FStar_Syntax_Util.un_squash g.goal_ty in
       match uu____2761 with
       | Some t ->
           let uu____2771 = FStar_Syntax_Util.head_and_args' t in
           (match uu____2771 with
            | (hd1,args) ->
                let uu____2792 =
                  let uu____2800 =
                    let uu____2801 = FStar_Syntax_Util.un_uinst hd1 in
                    uu____2801.FStar_Syntax_Syntax.n in
                  (uu____2800, args) in
                (match uu____2792 with
                 | (FStar_Syntax_Syntax.Tm_fvar
                    fv,uu____2811::(l,uu____2813)::(r,uu____2815)::[]) when
                     ...
                     let uu____2849 =
                       ...
                       Prims.op_Negation uu____2850 in
                     if uu____2849
                     then
                       let uu____2852 =
                         let uu____2853 = FStar_Syntax_Print.term_to_string l in
                         let uu____2854 = FStar_Syntax_Print.term_to_string r in
                         FStar_Util.format2
                 ...
                 | (hd2,uu____2861) ->
                     let uu____2872 =
                       let uu____2873 = FStar_Syntax_Print.term_to_string t in
...
let flip: Prims.unit tac =
  bind get
    (fun ps  ->
       ...
       | g1::g2::gs ->
           set
             (let uu___102_2883 = ps in
              {
                main_context = (uu___102_2883.main_context);
                main_goal = (uu___102_2883.main_goal);
                all_implicits = (uu___102_2883.all_implicits);
                goals = (g2 :: g1 :: gs);
                smt_goals = (uu___102_2883.smt_goals)
              })
       | uu____2884 -> fail "flip: less than 2 goals")
let later: Prims.unit tac =
  bind get
    (fun ps  ->
       ...
       | g::gs ->
           set
             (let uu___103_2892 = ps in
              {
                main_context = (uu___103_2892.main_context);
                main_goal = (uu___103_2892.main_goal);
                all_implicits = (uu___103_2892.all_implicits);
                goals = (FStar_List.append gs [g]);
                smt_goals = (uu___103_2892.smt_goals)
              }))
let qed: Prims.unit tac =
  bind get
    (fun ps  ->
       match ps.goals with | [] -> ret () | uu____2896 -> fail "Not done!")
let cases:
  ...
  fun t  ->
    bind cur_goal
      (fun g  ->
         ...
         | (t1,typ,guard) ->
             let uu____2920 = FStar_Syntax_Util.head_and_args typ in
             (match uu____2920 with
              | (hd1,args) ->
                  ...
                  (match uu____2949 with
                   | (FStar_Syntax_Syntax.Tm_fvar
                      fv,(p,uu____2971)::(q,uu____2973)::[]) when
                       FStar_Syntax_Syntax.fv_eq_lid fv
                       ...
                       let g1 =
                         let uu___104_3002 = g in
                         let uu____3003 =
                           FStar_TypeChecker_Env.push_bv g.context v_p in
                         {
                           context = uu____3003;
                           witness = (uu___104_3002.witness);
                           goal_ty = (uu___104_3002.goal_ty)
                         } in
                       let g2 =
                         let uu___105_3005 = g in
                         let uu____3006 =
                           FStar_TypeChecker_Env.push_bv g.context v_q in
                         {
                           context = uu____3006;
                           witness = (uu___105_3005.witness);
                           goal_ty = (uu___105_3005.goal_ty)
                         } in
                       bind dismiss
                         (fun uu____3009  ->
                            let uu____3010 = add_goals [g1; g2] in
                            bind uu____3010
                              (fun uu____3014  ->
                                 let uu____3015 =
                                   let uu____3018 =
...
let uu___is_Lt: order -> Prims.bool =
  fun projectee  -> match projectee with | Lt  -> true | uu____3037 -> false
let uu___is_Eq: order -> Prims.bool =
  fun projectee  -> match projectee with | Eq  -> true | uu____3041 -> false
let uu___is_Gt: order -> Prims.bool =
  fun projectee  -> match projectee with | Gt  -> true | uu____3045 -> false
let order_binder:
...
let proofstate_of_goal_ty: env -> FStar_Syntax_Syntax.term -> proofstate =
  fun env  ->
    fun g  ->
      let g1 = let uu____3062 = bnorm env g in mk_irrelevant env uu____3062 in
      {
        main_context = env;
        main_goal = g1;
        all_implicits = [];
