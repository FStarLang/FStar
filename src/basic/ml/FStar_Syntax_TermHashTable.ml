...
module HashKey =
  struct
    ...
    let equal (x:t) (y:t) =
    P.profile (fun _ -> 
                 let res, time = record_time (fun _ -> FStar_Syntax_Hash.equal_term x y) in
    ...
    let hash (x:t) = 
        P.profile (fun _ -> FStar_Syntax_Hash.ext_hash_term x)
                  None
...
let insert (key: S.term) (v:'a) (ht:'a hashtable) =
  HT.add ht key v
...
(*
  BU.print2 "THT Statistics { num_bindings = %s; max_bucket_length = %s }\n"
...
(*    (string_of_int stats.num_buckets) *)
