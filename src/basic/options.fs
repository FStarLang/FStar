...
let interactive_session_name = Util.mk_ref None
...
let rec specs () : list<Getopt.opt> =
  let specs =
    [( noshort, "admit_fsi", OneArg ((fun x -> admit_fsi := x::!admit_fsi), "module name"), "Treat .fsi as a .fst");
     ...
     ( noshort, "session_name", OneArg ((fun s -> interactive_session_name := Some s), "name"), "Specify name of session interactive mode session.");
