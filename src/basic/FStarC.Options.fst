...
let parse_cmd_line () =
  ...
  let () =
    ...
    (* Absolute command-line paths resolve directly while their parents become implicit roots. *)
    let files = List.map Filepath.normalize_file_path !file_list_ in
    file_list_ := files;
    Find.set_file_list files;
    ()
  in
