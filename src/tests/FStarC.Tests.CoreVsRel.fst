...
let run_all () : ML bool =
    ...
    (* ----------------------------------------------------------
       Group 18: Head-injective API tests
       Tests the check_term_equality_head_injective API directly.
       This API treats the head as injective and checks args with
       unfolding + guards. Used by the Pulse prover for slprops.
       ---------------------------------------------------------- *)

    (* Test 1800: head-injective on identical terms — trivial success *)
    let _ =
      let env, c_bv, a_bv, _b_bv = env_with_cab_ints () in
      let c_name = S.bv_to_name c_bv in
      let a_name = S.bv_to_name a_bv in
      let t0 = S.mk_Tm_app cvr_ha_like_head [(c_name, None); (a_name, None)] FStarC.Range.dummyRange in
      match Core.check_term_equality_head_injective env t0 t0 with
      | Inl None ->
        Format.print_string "Test 1800 (head-injective reflexive): PASS (trivial)\n"
      | Inl (Some _) ->
        Format.print_string "Test 1800 (head-injective reflexive): PASS (guarded, expected trivial)\n"
      | Inr _ ->
        Format.print_string "Test 1800 (head-injective reflexive): FAIL (error)\n";
        success := false
    in

    (* Test 1801: head-injective on different args — should produce guard *)
    let _ =
      let env, c_bv, a_bv, b_bv = env_with_cab_ints () in
      let c_name = S.bv_to_name c_bv in
      let a_name = S.bv_to_name a_bv in
      ...
      let t0 = S.mk_Tm_app cvr_ha_like_head [(c_name, None); (a_name, None)] FStarC.Range.dummyRange in
      let t1 = S.mk_Tm_app cvr_ha_like_head [(c_name, None); (b_name, None)] FStarC.Range.dummyRange in
      match Core.check_term_equality_head_injective env t0 t1 with
      | Inl (Some _) ->
        Format.print_string "Test 1801 (head-injective diff args): PASS (guarded)\n"
      | Inl None ->
        Format.print_string "Test 1801 (head-injective diff args): FAIL (trivial, expected guard)\n";
        success := false
      | Inr _ ->
        Format.print_string "Test 1801 (head-injective diff args): FAIL (error)\n";
        success := false
    in

    (* Test 1802: head-injective with mismatched heads — should fail *)
    let _ =
      let env, _c_bv, a_bv, _b_bv = env_with_cab_ints () in
      let a_name = S.bv_to_name a_bv in
      let int_t = tc "int" in
      let t0 = S.mk_Tm_app cvr_ha_like_head [(a_name, None)] FStarC.Range.dummyRange in
      let t1 = S.mk_Tm_app int_t [(a_name, None)] FStarC.Range.dummyRange in
      match Core.check_term_equality_head_injective env t0 t1 with
      | Inr _ ->
        Format.print_string "Test 1802 (head-injective head mismatch): PASS (error)\n"
      | Inl _ ->
        Format.print_string "Test 1802 (head-injective head mismatch): FAIL (should have failed)\n";
        success := false
    in
