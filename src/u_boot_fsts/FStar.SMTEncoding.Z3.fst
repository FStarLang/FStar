...
let print_decl decls v =
   decls |> List.iter (function d -> match d with
      | DeclFun (s, sorts, sort, cap) -> if BU.starts_with s "label_" then print_string (v ^ " - " ^ s ^ "\n")
      | _ -> ())
...
let bgtheory fresh =
    if fresh
    // we need to load the whole scope each time in the fresh mode
    then (bg_scope := [];
          List.rev !fresh_scope |> List.flatten)
    // reset bg scope for next queries when working on single core (!fresh) mode
