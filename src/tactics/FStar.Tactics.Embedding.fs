...
let unembed_direction (t : term) : direction =
    match (SS.compress t).n with
    | Tm_fvar fv when S.fv_eq_lid fv fstar_tactics_topdown_lid -> TopDown
    | Tm_fvar fv when S.fv_eq_lid fv fstar_tactics_bottomup_lid -> BottomUp
    | _ -> failwith (BU.format1 "Expected FStar.Tactics.Types.TopDown or FStar.Tactics.Types.BottomUp, got %s" (Print.term_to_string t))
