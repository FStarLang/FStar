...
let unembed_proofstate (t:term) : option<proofstate> =
    ...
    with | _ ->
        Err.maybe_fatal_error t.pos (Err.NotEmbedded "proofstate", (BU.format1 "Not an embedded proofstate: %s" (Print.term_to_string t)));
        None
...
let unembed_result (t:term) (unembed_a:unembedder<'a>)
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.NotEmbedded "tactic_result", (BU.format1 "Not an embedded tactic result: %s" (Print.term_to_string t)));
        None
...
let unembed_direction (t : term) : option<direction> =
    ...
    | _ ->
        Err.maybe_fatal_error t.pos (Err.NotEmbedded "direction", (BU.format1 "Not an embedded direction: %s" (Print.term_to_string t)));
        None
