...
=
  ...
  *)
  ...
  let mrelation_from_interp (interp:term) : term =
    ...
    let c  = S.gen_bv "c"  (Some interp.pos) repr_a in
    let wp = S.gen_bv "wp" (Some interp.pos) wp_a in
    ...
    in
    let abs = U.abs (binders @ [S.mk_implicit_binder a; S.mk_binder c; S.mk_binder wp])
                    body
                    ret_tot_type
    in
    abs
  ...
  let mrelation =
    ...
    | _, Some t -> Some (snd t)
    | None, None -> None
    | Some i, None -> Some (mrelation_from_interp (snd i))
  in
