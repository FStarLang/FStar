...
and star_type' env t =
  ...
  | Tm_app (head, args) ->
      ...
      let debug t s =
        ...
        in
        Errors.maybe_fatal_error t.pos (Errors.DependencyFound, (BU.format2 "Dependency found in term %s : %s" (Print.term_to_string t) (string_of_set Print.bv_to_string s)))
      in
      let rec is_non_dependent_arrow ty n =
        ...
        | _ ->
            Errors.maybe_fatal_error ty.pos (Errors.NotDependentArrow, (BU.format1 "Not a dependent arrow : %s" (Print.term_to_string ty))) ;
            false
      in
             ...
             then 
               // We need to check that the result of the application is a datatype
                ...
                begin match res.n with
                  | Tm_app _ -> true
                  | _ ->
                    Errors.maybe_fatal_error head.pos (Errors.NondependentUserDefinedDataType, (BU.format1 "Got a term which might be a non-dependent user-defined data-type %s\n" (Print.term_to_string head))) ;
                    false
                end
             else false
        | Tm_bvar _
...
and infer (env: env) (e: term): nm * term * term =
  ...
  | Tm_app({n=Tm_constant Const_set_range_of}, _) ->
    raise_error (Errors.IllAppliedConstant, BU.format1 "DMFF: Ill-applied constant %s" (Print.term_to_string e)) e.pos
...
and trans_F_ (env: env_) (c: typ) (wp: term): term =
  ...
  | Tm_app (head, args) ->
      ...
      mk (Tm_app (head, List.map2 (fun (arg, q) (wp_arg, q') ->
        ...
        if q <> q' then Errors.maybe_fatal_error head.pos (Errors.IncoherentImplicitQualifier, (BU.format2 "Incoherent implicit qualifiers %b %b\n" (print_implicit q) (print_implicit q'))) ;
