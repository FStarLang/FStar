...
let build_lattice env se = match se.sigel with
  ...
  | Sig_sub_effect(sub) ->
    let src_ed = get_effect_decl env sub.source in
    let dst_ed = get_effect_decl env sub.target in
    if src_ed.is_layered || dst_ed.is_layered
    then
      let edge = { msource = sub.source; mtarget = sub.target; mlift = ({
        mlift_t = sub.lift_wp;
      ...
      let compose_edges e1 e2 : edge =
        let composed_lift =
          ...
          let mlift_term =
            ...
            | _ -> None
          in
          { mlift_t=None; mlift_wp=mlift_wp ; mlift_term=mlift_term}
        in
        { msource=e1.msource;
          mtarget=e2.mtarget;
          mlift=composed_lift }
      ...
      let edge =
        { msource=sub.source;
          mtarget=sub.target;
          mlift={ mlift_t=None; mlift_wp=sub_mlift_wp; mlift_term=sub_mlift_term } }
      in
