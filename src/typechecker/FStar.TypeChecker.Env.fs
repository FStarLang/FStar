  ...
  let mapper = function
    ...
    | Inr (Sig_inductive_typ (lid, uvs, tps, k, _, _, _, _), Some us) ->
      begin match tps with
        ...
        | _ ->  Some <| (us, inst_tscheme_with (uvs, U.flat_arrow tps (mk_Total k)) us)
      end
...
let null_wp_for_eff env eff_name (res_u:universe) (res_t:term) = //TODO: This seems to only work if we have a non-indexed effect (merge remnant)
...
let push_sigelt env s = push_in_gamma env (Binding_sig(lids_of_sigelt s, s))

let push_sigelt_inst env s us = push_in_gamma env (Binding_sig_inst(lids_of_sigelt s,s,us))
