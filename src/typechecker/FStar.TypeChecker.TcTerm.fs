...
let value_check_expected_typ env (e:term) (tlc:either<term,lcomp>) (guard:guard_t)
  ...
  let e, lc, g =
   ...
   | Some t' ->
     let e, lc = TcUtil.maybe_coerce_bool_to_prop env e lc t' in //add a b2p coercion is e:bool and t'=prop
     let t = lc.res_typ in
     let e, g = TcUtil.check_and_ascribe env e t t' in
     if debug env Options.High
     then BU.print5 "check_and_ascribe for e %s: type is %s<:%s \tguard is %s, %s\n"
                (Print.term_to_string e)
                (Print.term_to_string t) (Print.term_to_string t')
                (Rel.guard_to_string env g) (Rel.guard_to_string env guard);
     let msg = if Rel.is_trivial_guard_formula g then None else Some <| Err.subtyping_failed env t t' in
...
let comp_check_expected_typ env e lc : term * lcomp * guard_t =
  match Env.expected_typ env with
   ...
   | Some t ->
     let e, lc = TcUtil.maybe_coerce_bool_to_prop env e lc t in //Add a b2p coercion if e:bool and t=prop
