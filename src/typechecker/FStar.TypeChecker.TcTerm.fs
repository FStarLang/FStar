...
and tc_maybe_toplevel_term env (e:term) : term                  (* type-checked and elaborated version of e            *)
  ...
  | Tm_meta(e, Meta_desugared Sequence) ->
    //Expect e to be a Tm_let, otherwise ToSyntax has gone wrong
    //Typecheck e as usual, but then make sure that the type
    //computed for the let-bound variable is a subtype of unit
    //since the user wrote e1; e2
    begin
    let e' = U.unmeta e in
    match (SS.compress e').n with
    | Tm_let((false,[_]),_) ->
      ...
      let e' = U.unmeta e in
      match (SS.compress e').n with
      | Tm_let((false,[{lbtyp=t; lbdef=e1}]),_) ->
        let gopt = Rel.get_subtyping_prop env t S.t_unit in
        let g =
          match gopt with
          | None ->
            raise_error (Err.expected_expression_of_type env S.t_unit e1 t)
                        e1.pos
          | Some g' ->
            Env.conj_guard g g'
        in
      ...
      | _ -> failwith
              (BU.format1 "Impossible: Typechecking a Tm_let produced a %s"
                          (Print.tag_of_term e'))
      end

    | _ ->
      failwith (BU.format1 "Impossible: Unexpected Sequence label on a %s"
                          (Print.tag_of_term e'))
    end
