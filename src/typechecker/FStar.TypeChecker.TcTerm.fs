...
let rec tc_term env e =
    if Env.debug env Options.Medium then
        BU.print6 "(%s) Starting tc_term (phase1=%s) of %s (%s) with expected type: (%s, use_eq=%s) {\n"
          ...
          (match Env.expected_typ env with
           | None -> "None"
           | Some (_, use_eq) -> string_of_bool use_eq);
...
and tc_maybe_toplevel_term env (e:term) : term                  (* type-checked and elaborated version of e            *)
  ...
  | Tm_ascribed (e, (asc, Some tac, use_eq), labopt) ->
    ...
    let t' =
      match (SS.compress t').n with
      | Tm_ascribed (e, (asc, None, _use_eq0), labopt) ->
        //assert (use_eq = _use_eq0)
  ...
  | Tm_app(head, args) ->
    ...
    then BU.print3 "(%s) Checking app %s, expected type is %s\n"
                    ...
                    (Env.expected_typ env0 |> (function None -> "none" | Some (t, use_eq) -> Print.term_to_string t ^ ", use_eq = " ^ string_of_bool use_eq));
...
and tc_synth head env args rng =
    ...
    let typ =
        ...
        | None -> begin match Env.expected_typ env with
                  | Some (t, use_eq) ->
                    if use_eq then failwith "NYI: use_eq with the synth hook type";
                    t
...
and tc_value env (e:term) : term
  ...
  | Tm_unknown ->
    ...
    let t, _, g0 =
        ...
        | Some (t, use_eq) ->
          if use_eq then failwith "NYI: use_eq for an unknown type ascription";
        ...
        let t, g_env =
          ...
          | _ ->
            ...
            let g2_env =
              ...
              | None ->
                if use_eq
                then raise_error (Err.basic_type_error env None expected_t t) (Env.get_range env);
      ...
      let envbody, body, should_check_expected_effect =
        if c_opt |> is_some &&
           (match (SS.compress body).n with  //body is an M.reflect
            ...
            | _ -> false)
        then
          let use_eq = match topt with | None -> failwith "Impossible!" | Some (_, use_eq) -> use_eq in
