...
(* TODO : should we use the functions inside Env *)
...
let set_lcomp_result env lc t =
    {lc with lcomp_res_typ=t;
             lcomp_as_comp=fun () -> Env.set_result_typ env (lc.lcomp_as_comp()) t}
...
let value_check_expected_typ env (e:term) (tlc:either<term,lcomp>) (guard:guard_t)
  ...
  let should_return t =
    ...
    | Tm_arrow(_, c) ->
      ...
      then let t = U.unrefine <| Env.result_typ env c in
           match (SS.compress t).n with
  ...
  let lc =
    match tlc with
    | Inl t -> Env.lcomp_of_comp env (if not (should_return t)
  ...
  let e, lc, g = match Env.expected_typ env with
   ...
   | Some t' ->
     ...
     memo_tk e t', set_lcomp_result env lc t', g in
...
let check_expected_effect env (copt:option<comp>) (e, c) : term * comp * guard_t =
  ...
  match expected_c_opt with
    ...
    | Some expected_c -> //expected effects should already be normalized
       ...
       //let expected_c' = TcUtil.refresh_comp_label env true (Env.lcomp_of_comp env <| expected_c) in
       ...
       let e = TcUtil.maybe_lift env e (U.comp_effect_name c) (U.comp_effect_name expected_c) (Env.result_typ env c) in
       e, expected_c, g
...
and tc_maybe_toplevel_term env (e:term) : term                  (* type-checked and elaborated version of e            *)
  ...
  | Tm_meta(e, Meta_desugared Sequence) ->
    begin match (SS.compress e).n with
        | Tm_let((_,[{lbname=x; lbdef=e1}]), e2) -> //NS: Why not handle this specially in the deugaring phase, adding a unit annotation on x?
          ...
          let e = mk (Tm_let((false, [mk_lb (x, [], c1.lcomp_name, Common.t_unit, e1)]), e2)) (Some c.lcomp_res_typ.n) e.pos in
          let e = mk (Tm_meta(e, Meta_desugared Sequence)) (Some c.lcomp_res_typ.n) top.pos in
  ...
  | Tm_ascribed (e, Inl t, _) ->
    ...
    let e, c, f2 = comp_check_expected_typ env (mk (Tm_ascribed(e, Inl t, Some c.lcomp_name)) (Some t.n) top.pos) c in
  ...
  | Tm_app({n=Tm_constant Const_reify}, [(e, aqual)]) ->
    ...
    let repr = TcUtil.reify_comp env c in
    let e = mk (Tm_app(reify_op, [(e, aqual)])) (Some repr.n) top.pos in
    let c = S.mk_Total repr |> Env.lcomp_of_comp env in
    ...
    | Some ed ->
      if not (ed.qualifiers |> S.contains_reflectable) then
        ...
        let e, g =
              ...
              match Rel.try_teq env_no_ex c.lcomp_res_typ expected_repr_typ with
              | None -> Err.add_errors env [BU.format2 "Expected an instance of %s; got %s" (Print.term_to_string ed.repr) (Print.term_to_string c.lcomp_res_typ), e.pos];
        ...
        let c = S.mk_Comp ({
              ...
              effect_args=[as_arg res_typ; as_arg wp];
              flags=[]
            }) |> Env.lcomp_of_comp env in
  ...
  | Tm_app(head, args) ->
    ...
    let c = if Env.should_verify env
            ...
            //&& not (Util.is_unit c.lcomp_res_typ)
    ...
    let gimp =
        match (SS.compress head).n with
            | Tm_uvar(u, _) ->
              let imp = ("head of application is a uvar", env0, u, e, c.lcomp_res_typ, head.pos) in
  ...
  | Tm_match(e1, eqns) ->
    ...
    let guard_x = S.new_bv (Some e1.pos) c1.lcomp_res_typ in
    ...
    let e =
        let mk_match scrutinee =
            let scrutinee = TcUtil.maybe_lift env scrutinee c1.lcomp_name cres.lcomp_name c1.lcomp_res_typ in
            let branches = t_eqns |> List.map (fun ((pat, wopt, br), _, lc, _) ->
                 (pat, wopt, TcUtil.maybe_lift env br lc.lcomp_name cres.lcomp_name lc.lcomp_res_typ)) in
            let e = mk (Tm_match(scrutinee, branches)) (Some cres.lcomp_res_typ.n) top.pos in
             //The ascription with the result type is useful for re-checking a term, translating it to Lean etc.
            mk (Tm_ascribed(e, Inl cres.lcomp_res_typ, Some cres.lcomp_name)) None e.pos in
        ...
        if TcUtil.is_pure_or_ghost_effect env c1.lcomp_name
        then mk_match e1
             ...
             let lb = {
                lbname=Inl guard_x;
                lbunivs=[];
                lbtyp=c1.lcomp_res_typ;
                lbeff=Env.norm_eff_name env c1.lcomp_name;
                lbdef=e1;
             } in
             let e = mk (Tm_let((false, [lb]), SS.close [S.mk_binder guard_x] e_match)) (Some cres.lcomp_res_typ.n) top.pos in
             TcUtil.maybe_monadic env e cres.lcomp_name cres.lcomp_res_typ in
    if debug env Options.Extreme
    then BU.print2 "(%s) comp type = %s\n"
                      (Range.string_of_range top.pos) (Print.comp_to_string <| cres.lcomp_as_comp());
...
//    let g = Rel.trivial_guard in
  ...
  | Tm_unknown -> //only occurs where type annotations are missing in source programs
    ...
    e, S.mk_Total t |> Env.lcomp_of_comp env, (Rel.conj_guard g0 g1)
...
and tc_comp env c : comp                                      (* checked version of c                       *)
  ...
  match c.n with
    ...
    | Comp c ->
      let head = S.fvar c.comp_typ_name Delta_constant None in
      let head = match c.comp_univs with
         | [] -> head
         | us -> S.mk (Tm_uinst(head, us)) None c0.pos
      in
      let tc = mk_Tm_app head c.effect_args None c0.pos in
      ...
      let c = mk_Comp ({c with
          ...
          effect_args=args}) in
      let u_c =
        match TcUtil.effect_repr env c with
        | None ->
            (* KM : Do we need to recompute the universe here or can we recover it *)
            (* from the normal form of the comp typ c ? *)
            env.universe_of env (fst (Env.comp_as_normal_comp_typ env c).nct_result)
...
and tc_universe env u : universe =
  let rec aux u =
    ...
    | U_name x  -> u
  in
  if env.lax_universes then U_zero
  else match u with
                  ...
                  let check_actuals_against_formals env bs bs_expected =
                      let rec handle_more (env, bs, more, guard, subst) c_expected = match more with
                        ...
                        | Some (Inl more_bs) ->  //more actual args
                          ...
                          if U.is_named_tot c
                          then let t = unfold_whnf env (Env.result_typ env c) in
                               match t.n with
                                | Tm_arrow(bs_expected, c_expected) ->
...
//                        printfn "Checking let rec annot: %s\n" (Print.term_to_string t);
                 ...
                 let envbody = Env.set_expected_typ envbody (Env.result_typ env c) in
    ...
    then BU.print2 "Introduced %s implicits in body of abstraction\nAfter solving constraints, cbody is %s\n"
        ...
        (Print.comp_to_string <| cbody.lcomp_as_comp());

    let body, cbody, guard = check_expected_effect ({envbody with use_eq=use_eq}) c_opt (body, cbody.lcomp_as_comp()) in
    ...
    let e = U.abs bs body (Some (Inl (Env.lcomp_of_comp env (dflt cbody c_opt)))) in
    ...
    let c, g = TcUtil.strengthen_precondition None env e (Env.lcomp_of_comp env c) guard in
    e, c, g
...
and check_application_args env head chead ghead args expected_topt : term * lcomp * guard_t=
    ...
    let r = Env.get_range env in
    let thead = chead.lcomp_res_typ in
    ...
    *)
    let monadic_application (head, chead, ghead, cres)
        ...
        * guard_t //and whatever guard remains
        =
        let rt = check_no_escape (Some head) env fvs cres.lcomp_res_typ in
        let cres = {cres with lcomp_res_typ=rt} in
        let cres, guard =
            match bs with
            | [] -> (* full app *)
                let cres = SS.subst_lcomp subst cres in
                (* If we have f e1 e2
                ...
                *)
                ...
                let refine_with_equality =
                    ...
                    //not (Util.is_unit cres.lcomp_res_typ)
                    //&&
                ...
                let cres = //NS: Choosing when to add an equality refinement is VERY important for performance.
                    ...
                    then TcUtil.maybe_assume_result_eq_pure_term env
                            (mk_Tm_app head (List.rev arg_rets) (Some cres.lcomp_res_typ.n) r)
                            cres
            ...
            | _ ->  (* partial app *)
                ...
                Env.lcomp_of_comp env <| mk_Total  (SS.subst subst <| U.arrow bs (cres.lcomp_as_comp())), g in
        ...
        let args, comp, monadic = List.fold_left (fun (args, out_c, monadic) ((e, q), x, c) ->
                    ...
                    | Inl (eff_name, arg_typ) ->
                      (TcUtil.maybe_lift env e eff_name out_c.lcomp_name arg_typ, q)::args, out_c, monadic

                    | Inr c ->
                        let monadic = monadic || not (U.is_pure_or_ghost_lcomp c) in
                        let out_c =
                            TcUtil.bind env None  //proving (Some e) here instead of None causes significant Z3 overhead
                                        c (x, out_c) in
                        let e = TcUtil.maybe_monadic env e c.lcomp_name c.lcomp_res_typ in
                        let e = TcUtil.maybe_lift env e c.lcomp_name out_c.lcomp_name c.lcomp_res_typ in
                        (e, q)::args, out_c, monadic) ([], cres, false) arg_comps_rev in
        let comp = TcUtil.bind env None chead (None, comp) in
        let app =  mk_Tm_app head args (Some comp.lcomp_res_typ.n) r in
        let app =
          (* TODO : should we use a test with env for pure or ghost here *)
          if monadic || not (U.is_pure_or_ghost_lcomp comp)
          then TcUtil.maybe_monadic env app comp.lcomp_name comp.lcomp_res_typ
          else app
        in
...
//                if debug env Options.High then BU.print2 "Guard on this arg is %s;\naccumulated guard is %s\n" (guard_to_string env g_e) (guard_to_string env g);
            ...
            then let subst = maybe_extend_subst subst (List.hd bs) e in
                    tc_args head_info (subst, (arg, None, Inl (c.lcomp_name, c.lcomp_res_typ))::outargs, arg::arg_rets, g, fvs) rest rest'
            else if TcUtil.is_pure_or_ghost_effect env c.lcomp_name //its conditionally pure; can substitute, but must check its WP
            then let subst = maybe_extend_subst subst (List.hd bs) e in
            ...
            then let newx = S.new_bv (Some e.pos) c.lcomp_res_typ in
            ...
            let rec aux norm tres =
                ...
                match tres.n with
                    | Tm_arrow(bs, cres') ->
                        ...
                        let head_info = (head, chead, ghead, Env.lcomp_of_comp env cres') in
                        if debug env Options.Low
       ...
       match (U.unrefine tf).n with
        ...
        | Tm_app({n=Tm_uvar _}, _) ->
            ...
            *)
            ...
            let bs = null_binders_of_tks (comps |> List.map (fun (_, c) -> c.lcomp_res_typ, None)) in
            let ml_or_tot t r =
                if Options.ml_ish ()
                then U.ml_comp t r
                else S.mk_Total t
            ...
            let comp =
                  List.fold_right (fun (r1, c) out ->
                       TcUtil.bind env None c (None, out))
                  ((head.pos, chead)::comps)
                  (Env.lcomp_of_comp env <| cres) in
            mk_Tm_app head args (Some comp.lcomp_res_typ.n) r, comp, Rel.conj_guard ghead g_args

        | Tm_arrow(bs, c) ->
            let bs, c = SS.open_comp bs c in
            let head_info = head, chead, ghead, Env.lcomp_of_comp env c in
            tc_args head_info ([], [], [], Rel.trivial_guard, []) bs args
...
and check_short_circuit_args env head chead g_head args expected_topt : term * lcomp * guard_t =
    let r = Env.get_range env in
    let tf = SS.compress chead.lcomp_res_typ in
    match tf.n with
        | Tm_arrow(bs, c) when (U.is_total_comp c && List.length bs=List.length args) ->
          let res_t = Env.result_typ env c in
          let args, guard, ghost = List.fold_left2 (fun (seen, guard, ghost) (e, aq) (b, aq') ->
                ...
                let ghost = ghost
                          || (not (U.is_total_lcomp c)
                              && not (TcUtil.is_pure_effect env c.lcomp_name)) in
                seen@[as_arg e], Rel.conj_guard guard g, ghost) ([], g_head, false) args bs in
          let e = mk_Tm_app head args (Some res_t.n) r  in
          let c = if ghost then S.mk_GTotal res_t |> Env.lcomp_of_comp env else Env.lcomp_of_comp env c in
  ...
  let tc_pat (allow_implicits:bool) (pat_t:typ) p0 :
    ...
    let exps, norm_exps = exps |> List.map (fun e ->
        ...
        let e, lc, g =  tc_term env1 e in //only keep the unification/subtyping constraints; discard the logical guard for patterns
                                          //Q: where is it being discarded? A: we only use lc.lcomp_res_typ below, and forget abouts its WP
        if Env.debug env Options.High
        then BU.print2 "Pre-checked pattern expression %s at type %s\n" (N.term_to_string env e) (N.term_to_string env lc.lcomp_res_typ);

        let g' = Rel.teq env lc.lcomp_res_typ expected_pat_t in
...
(*open*) let e1, univ_vars, c1, g1, annotated = check_let_bound_def true env lb in
         ...
         let g1, e1, univ_vars, c1 =
            ...
            then g1, e1, univ_vars, c1
                 ...
                 assert (univ_vars = []) ;
                 let _, univs, e1, c1 = List.hd (TcUtil.generalize env [lb.lbname, e1, c1.lcomp_as_comp()]) in
                 g1, e1, univs, Env.lcomp_of_comp env c1 in
         ...
         let e2, c1 =
            ...
            then
              ...
              let c = c1.lcomp_as_comp () |> N.normalize_comp [N.Beta] env in
              let e2 =
                if Util.is_pure_comp c
                then e2
                else mk (Tm_meta(e2, Meta_desugared Masked_effect)) None e2.pos
              in
              e2, c
         in
         ...
         let cres = Env.lcomp_of_comp env <| Util.ml_comp Common.t_unit e.pos in
         e2.tk := Some (Common.t_unit.n);

(*close*)let lb = U.close_univs_and_mk_letbinding None lb.lbname univ_vars (Env.result_typ env c1) (U.comp_effect_name c1) e1 in
...
and check_inner_let env e =
   ...
   match e.n with
     | Tm_let((false, [lb]), e2) ->
       ...
       let x = {BU.left lb.lbname with sort=c1.lcomp_res_typ} in
       ...
       let e1 = TcUtil.maybe_lift env e1 c1.lcomp_name cres.lcomp_name c1.lcomp_res_typ in
       let e2 = TcUtil.maybe_lift env e2 c2.lcomp_name cres.lcomp_name c2.lcomp_res_typ in
       let lb = Util.mk_letbinding (Inl x) [] c1.lcomp_res_typ c1.lcomp_name e1 in
       let e = mk (Tm_let((false, [lb]), SS.close xb e2)) (Some cres.lcomp_res_typ.n) e.pos in
       let e = TcUtil.maybe_monadic env e cres.lcomp_name cres.lcomp_res_typ in
       let x_eq_e1 = NonTrivial <| U.mk_eq2 (env.universe_of env c1.lcomp_res_typ) c1.lcomp_res_typ (S.bv_to_name x) e1 in
       ...
       then (let tt = Env.expected_typ env |> Option.get in
             ...
             then BU.print2 "Got expected type from env %s\ncres.lcomp_res_typ=%s\n"
                        ...
                        (Print.term_to_string cres.lcomp_res_typ);
             e, cres, guard)
       else (* no expected type; check that x doesn't escape it's scope *)
            (let t = check_no_escape None env [x] cres.lcomp_res_typ in
             if Env.debug env <| Options.Other "Exports"
             then BU.print2 "Checked %s has no escaping types; normalized to %s\n"
                        (Print.term_to_string cres.lcomp_res_typ)
                        (Print.term_to_string t);
             e, ({cres with lcomp_res_typ=t}), guard)
           ...
           let lbs =
            ...
            (* TODO : Should we gather the fre univnames ? e.g. (TcUtil.gather_free_univnames env e1)@lb.lbunivs *)
                   ...
                   ecs |> List.map (fun (x, uvs, e, c) ->
                      U.close_univs_and_mk_letbinding all_lb_names x uvs (Env.result_typ env c) (U.comp_effect_name c) e) in

          let cres = Env.lcomp_of_comp env <| S.mk_Total Common.t_unit in
...
(*open*)  let lbs, e2 = SS.open_let_rec lbs e2 in
          ...
          let tres = norm env cres.lcomp_res_typ in
          let cres = {cres with lcomp_res_typ=tres} in
...
and check_let_bound_def top_level env lb
    ...
    then BU.print3 "checked top-level def %s, result type is %s, guard is %s\n"
            ...
            (Print.term_to_string c1.lcomp_res_typ)
...
and tc_tot_or_gtot_term env e : term
  ...
  then e, c, g
       ...
       let target_comp, allow_ghost =
            ...
            then S.mk_Total (Env.result_typ env c), false
            else S.mk_GTotal (Env.result_typ env c), true in
       match Rel.sub_comp env c target_comp with
        | Some g' -> e, Env.lcomp_of_comp env target_comp, Rel.conj_guard g g'
        | _ ->
            if allow_ghost
...
(* universe_of_aux env e:
      ...
      This is generally called from within TcUtil
...
let rec universe_of_aux env e =
   ...
   | Tm_ascribed(_, Inr c, _) -> Env.result_typ env c
   ...
   | Tm_arrow(bs, c) ->
     ...
     let u_res =
        let res = Env.result_typ env c in
   ...
   | Tm_app(hd, args) ->
     let rec type_of_head retry hd args =
        ...
        | _ ->
          ...
          let _, ({lcomp_res_typ=t}), g = tc_term env hd in
     ...
     let res = Env.result_typ env res in
