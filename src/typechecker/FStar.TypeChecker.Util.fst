 ...
 *)
let lift_comps_sep_guards env guard_indexed_effect_uvars c1 c2 (b:option bv) (for_bind:bool)
: lident * comp * comp * guard_t * guard_t =
  ...
  | Some (m, lift1, lift2) ->
    let c1, g1 = lift_comp env guard_indexed_effect_uvars c1 lift1 in
    let c2, g2 =
      if not for_bind then lift_comp env guard_indexed_effect_uvars c2 lift2
      else
        let x_a =
          match b with
        ...
        let c2, g2 = lift_comp env_x guard_indexed_effect_uvars c2 lift2 in
...
let lift_comps env guard_indexed_effect_uvars c1 c2 (b:option bv) (for_bind:bool)
...
and mk_indexed_bind env
  ...
  then mk_ad_hoc_indexed_bind env guard_indexed_effect_uvars m n p bind_t ct1 b ct2 flags r1 has_range_args
    ...
    let bind_t_bs, bind_t_comp = U.arrow_formals_comp bind_t in

    let bind_t_bs, binder_kinds, subst =
      let a_b::b_b::bind_t_bs = bind_t_bs in
      bind_t_bs,
      List.splitAt 2 binder_kinds |> snd,
      [NT (a_b.binder_bv, t1); NT (b_b.binder_bv, t2)] in
    ...
    let bind_t_bs, subst, guard, guard_uvars_tms =
    ...
    let bind_t_bs =
      if has_range_args
      then List.splitAt 2 bind_t_bs |> snd
      else bind_t_bs in

    let bind_t_bs = List.splitAt 2 bind_t_bs |> snd in

    let subst, guard, guard_uvar_tms =
      let ss, g, g_uv_tms =
        List.fold_left (fun (ss, g, g_uv_tms) b ->
          let [uv_t], [guard_uv_t], g_uv = Env.uvars_for_binders env [b] (subst@ss) guard_indexed_effect_uvars
            (fun _ -> "") r1 in
      ...
      subst@ss,
      Env.conj_guard guard g,
      guard_uvars_tms@g_uv_tms in

    let bind_ct = bind_t_comp |> SS.subst_comp subst |> U.comp_to_comp_typ in
...
and mk_bind env guard_indexed_effect_uvars
  ...
  | Some (p, f_bind) -> f_bind env guard_indexed_effect_uvars ct1 b ct2 flags r1
  | None ->
    (*
     ...
     *)
    let m, c1, c2, g_lift = lift_comps env guard_indexed_effect_uvars c1 c2 b true in
...
let weaken_comp env (c:comp) (formula:term) : comp * guard_t =
  ...
  then c, Env.trivial_guard
       ...
       let guard_indexed_effect_uvars = false in
       mk_bind env guard_indexed_effect_uvars pure_c None c (weaken_flags ct.flags) r
         ...
         lax_mk_tot_or_comp_l joined_eff u_t lc2.res_typ [], Env.trivial_guard  //AR: TODO: FIXME: fix for layered effects
          ...
          | Inr reason ->
            ...
            let mk_bind c1 b c2 g =  (* AR: end code for inlining pure and ghost terms *)
              let c, g_bind = mk_bind env guard_indexed_effect_uvars c1 b c2 bind_flags r1 in
            ...
            let mk_seq c1 b c2 =
                ...
                let c2, g2 = lift_comp env guard_indexed_effect_uvars c2 lift2 in
                ...
                List.fold_right2 (fun (g, eff_label, _, cthen) neg_cond (_, celse, g_comp) ->
                  ...
                  let md, ct_then, ct_else, g_lift_then, g_lift_else =
                    let m, cthen, celse, g_lift_then, g_lift_else =
                      lift_comps_sep_guards env guard_indexed_effect_uvars cthen celse None false in
                    let md = Env.get_effect_decl env m in
                    md,
...
let get_mlift_for_subeff env (sub:S.sub_eff) : Env.mlift =
  ...
  then
    ...
    let mk_mlift_wp ts env guard_indexed_effect_uvars c =
      let ct = U.comp_to_comp_typ c in
...
and ty_strictly_positive_in_type env (ty_lid:lident) (btype:term) (unfolded:unfolded_memo_t) : bool =
  ...
  not (ty_occurs_in ty_lid btype) ||  //ty does not occur in btype
  (debug_positivity env (fun () -> "ty does occur in this type, pressing ahead");
   ...
   | Tm_app (t, args) ->  //the binder type is an application
     ...
     if fv_us_opt |> is_none then begin
       ...
       //
       let t_ty, _ = env.typeof_well_typed_tot_or_gtot_term
         env
         t
    ...
    let b, _ =
      List.fold_left (fun (r, env) b ->
        if not r then r, env  //if we have already found some binder that does not satisfy the condition, short circuit
          ...
          //
          // AR: iterate over all the inductive type parameters,
          //     and see if they are marked strictly positive,
          //       and if so, they are indeed strictly positive in the
          //       data constructor binder type
          //
          (List.iter (fun {binder_bv=ty_bv; binder_attrs=ty_b_attrs} ->
             if U.has_attribute ty_b_attrs C.binder_strictly_positive_attr &&
                not (name_strictly_positive_in_type env ty_bv b.binder_bv.sort)
             then raise_error (Error_InductiveTypeNotSatisfyPositivityCondition,
                    BU.format1 "Binder %s is marked strictly positive, \
                               ...
                               (Print.bv_to_string ty_bv)) (S.range_of_bv ty_bv)) ty_bs);

          //
          // AR: And now check that the inductive type itself is 
          //     strictly positive in this data constructor binder type
          //
          ty_strictly_positive_in_type env ty_lid b.binder_bv.sort unfolded,
          push_binders env [b]  //push the binder in the environment,
                                //we do some normalization, so better to keep env good
        end
      ) (true, env) dbs in
    b
