...
(* NB: an [lcomp] records only an effect name, a result type and some flags --
   never a specification.  So these two must test for the *spec-free* spellings
   [Tot] and [GTot] specifically, and must NOT be widened to the whole pure or
