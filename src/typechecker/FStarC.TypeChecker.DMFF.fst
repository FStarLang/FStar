...
let d s = Format.print1 "\x1b[01;36m%s\x1b[00m\n" s
...
let mk_toplevel_definition (env: env_t) lident (def: term): ML (sigelt & term) =
  // Debug
  if !dbg then begin
    d (string_of_lid lident);
    Format.print2 "Registering top-level definition: %s\n%s\n" (show lident) (show def)
  end;
...
let gen_wps_for_free
  ...
  ML (Syntax.sigelts & Syntax.eff_decl)
=
   ...
   * the context. *)
  let rec collect_binders (t : term) : ML _ =
    ...
    | Tm_arrow {bs; comp} ->
        ...
        let rest = match comp.n with
          ...
          | _ -> raise_error comp Error_UnexpectedDM4FType
                               (Format.fmt1 "wp_a contains non-Tot arrow: %s" (show comp))
        in
    ...
    | _ ->
        raise_error t Error_UnexpectedDM4FType
                     (Format.fmt1 "wp_a doesn't end in Type0, but rather in %s" (show t))
  in
  let mk_lid name : ML lident = U.dm4f_lid ed name in

  let gamma = collect_binders wp_a |> U.name_binders in
  if !dbg then
    d (Format.fmt1 "Gamma is %s\n" (show gamma));
  ...
  *)
  ...
  let mk_forall (x: S.bv) (body: S.term): ML S.term =
...
let nm_of_comp c = match c.n with
  ...
  | _ ->
      raise_error c Error_UnexpectedDM4FType
                   (Format.fmt1 "[nm_of_comp]: unexpected computation type %s" (show c))

let string_of_nm = function
  | N t -> Format.fmt1 "N[%s]" (show t)
  | M t -> Format.fmt1 "M[%s]" (show t)
...
let rec mk_star_to_type (mk: term' -> ML term) env a : ML term =
...
and star_type' env t : ML term =
  ...
  | Tm_app {hd=head; args} ->
      ...
      let debug (t : term) (s : FlatSet.t bv) =
        Errors.log_issue t Errors.Warning_DependencyFound (Format.fmt2 "Dependency found in term %s : %s" (show t) (show s))
      in
      let rec is_non_dependent_arrow ty n : ML bool =
        match (SS.compress ty).n with
        | Tm_arrow {bs=binders; comp=c} -> begin
        ...
        | _ ->
            Errors.log_issue ty Errors.Warning_NotDependentArrow (Format.fmt1 "Not a dependent arrow : %s" (show ty));
            false
      in
      let rec is_valid_application head : ML bool =
        ...
        | Tm_fvar fv ->
             ...
             then
               // We need to check that the result of the application is a datatype
                let res = N.normalize [Env.EraseUniverses; Env.Inlining ; Env.DontUnfoldAttr [PC.tac_opaque_attr]; Env.UnfoldUntil S.delta_constant] env.tcenv t in
                begin match (SS.compress res).n with
                  | Tm_app _ -> true
                  | _ ->
                    Errors.log_issue head Errors.Warning_NondependentUserDefinedDataType (Format.fmt1 "Got a term which might be a non-dependent user-defined data-type %s\n" (show head));
                    false
                end
             else false
        | Tm_bvar _
      ...
      if is_valid_application head then
        ...
        raise_error0 Errors.Fatal_WrongTerm
          (Format.fmt1 "For now, only [either], [option] and [eq2] are supported in the definition language (got: %s)"
            (show t))
  ...
  | Tm_abs {bs=binders; body=repr; rc_opt=something} ->
  ...
  | Tm_refine {b=x; phi=t} when false ->
      ...
      let subst = [NM(x, 0)] in
      let t = SS.subst subst t in
      mk (Tm_refine {b={ x with sort = sort }; phi=t})

  | Tm_meta {tm=t; meta=m} ->
      mk (Tm_meta {tm=star_type' env t; meta=m})

  | Tm_ascribed {tm=e; asc=(Inl t, None, use_eq); eff_opt=something} ->
      mk (Tm_ascribed {tm=star_type' env e; asc=(Inl (star_type' env t), None, use_eq); eff_opt=something})

  | Tm_ascribed {tm=e; asc=(Inr c, None, use_eq); eff_opt=something} ->
      ...
      (*raise_error0 (Errors.Fatal_TermOutsideOfDefLanguage, (Format.fmt1 "Tm_ascribed is outside of the definition language: %s"
              (show t)))*)

 | Tm_ascribed {asc=(_, Some _, _)} ->
      raise_error0 Errors.Fatal_TermOutsideOfDefLanguage
        (Format.fmt1 "Ascriptions with tactics are outside of the definition language: %s" (show t))
  | Tm_refine _
  | Tm_uinst _
  | Tm_quoted _
  | Tm_constant _
  ...
  | Tm_uvar _
  | Tm_unknown ->
    let open FStarC.Class.Tagged in
    raise_error0 Errors.Fatal_TermOutsideOfDefLanguage
      (Format.fmt2 "%s is outside of the definition language: %s" (tag_of t) (show t))
...
let rec is_C (t: typ): ML bool =
  ...
  | Tm_arrow {bs=binders; comp} ->
      ...
      | M t ->
          if (is_C t) then
            raise_error t Error_UnexpectedDM4FType
                           (Format.fmt1 "Not a C-type (C -> C): %s" (show t));
          true
      | N t ->
...
let mk_return env (t: typ) (e: term) =
  ...
  let body = mk (Tm_app {hd=S.bv_to_name p; args=[ e, S.as_aqual_implicit false ]}) in
...
let rec check (env: env) (e: term) (context_nm: nm): ML (nm & term & term) =
  // Format.print1 "[debug]: check %s\n" (show e);
  ...
  let return_if (rec_nm, s_e, u_e) =
    ...
    | M t1,  N t2 ->
        raise_error0 Errors.Fatal_EffectfulAndPureComputationMismatch
          (Format.fmt3 "[check %s]: got an effectful computation [%s] in lieu of a pure computation [%s]" (show e) (show t1) (show t2))

  in

  let ensure_m (env: env_) (e2: term): ML (term & term & term) =
  ...
  | Tm_let {lbs=(false, [ binding ]); body=e2} ->
  ...
  | Tm_match {scrutinee=e0; brs=branches} ->
  ...
  | Tm_let _ ->
      failwith (Format.fmt1 "[check]: Tm_let %s" (show e))
  | Tm_type _ ->
      failwith "impossible (DM stratification)"
  | Tm_arrow _ ->
      failwith "impossible (DM stratification)"
  | Tm_refine _ ->
      failwith (Format.fmt1 "[check]: Tm_refine %s" (show e))
  | Tm_uvar _ ->
      failwith (Format.fmt1 "[check]: Tm_uvar %s" (show e))
  | Tm_delayed _ ->
      failwith "impossible (compressed)"
  | Tm_unknown ->
      failwith (Format.fmt1 "[check]: Tm_unknown %s" (show e))


and infer (env: env) (e: term): ML (nm & term & term) =
  // Format.print1 "[debug]: infer %s\n" (show e);
  let mk x = mk x e.pos in
  ...
  | Tm_abs {bs=binders;body;rc_opt} ->
      ...
      (*
      Format.print2_warning "Term %s ::: what %s \n"
                                (show body)
      ...
      *)
      ...
      let s_rc_opt = match rc_opt with
        ...
        | Some rc -> begin
            ...
            | Some rt ->
              let rt = N.normalize [ Env.Beta; Env.Eager_unfolding; Env.DontUnfoldAttr [PC.tac_opaque_attr]; Env.UnfoldUntil S.delta_constant; Env.EraseUniverses ] (get_env env) rt in
              if rc.residual_flags |> BU.for_some (function CPS -> true | _ -> false)
              then
      ...
      let s_term = mk (Tm_abs {bs=s_binders; body=s_body; rc_opt=subst_rc_opt (Subst.closing_of_binders s_binders) s_rc_opt}) in

      let u_body = close u_binders u_body in
      let u_binders = close_binders u_binders in
      let u_term = mk (Tm_abs {bs=u_binders; body=u_body; rc_opt=subst_rc_opt (Subst.closing_of_binders u_binders) u_rc_opt}) in
  ...
  | Tm_app {hd={n=Tm_constant Const_range_of}; args=a::hd::rest} ->
    ...
    let head = mk (Tm_app {hd=unary_op; args=[a]}) in
    let t = mk (Tm_app {hd=head; args=rest}) in
    infer env t
  ...
  | Tm_app {hd={n=Tm_constant Const_set_range_of}; args=a1::a2::hd::rest} ->
    let rest = hd::rest in //no 'as' clauses in F* yet, so we need to do this ugliness
    let unary_op, _ = U.head_and_args e in
    let head = mk (Tm_app {hd=unary_op; args=[a1; a2]}) in
    let t = mk (Tm_app {hd=head; args=rest}) in
    infer env t

  | Tm_app {hd={n=Tm_constant Const_range_of}; args=[(a, None)]} ->
    let t, s, u = infer env a in
    let head,_ = U.head_and_args e in
    N (tabbrev PC.range_lid),
        mk (Tm_app {hd=head; args=[S.as_arg s]}),
        mk (Tm_app {hd=head; args=[S.as_arg u]})

  | Tm_app {hd={n=Tm_constant Const_set_range_of}; args=(a1, _)::a2::[]} ->
    let t, s, u = infer env a1 in
    let head,_ = U.head_and_args e in
    t,
        mk (Tm_app {hd=head; args=[S.as_arg s; a2]}),
        mk (Tm_app {hd=head; args=[S.as_arg u; a2]})

  | Tm_app {hd={n=Tm_constant Const_range_of}}
  | Tm_app {hd={n=Tm_constant Const_set_range_of}} ->
    raise_error e Errors.Fatal_IllAppliedConstant (Format.fmt1 "DMFF: Ill-applied constant %s" (show e))
  ...
  | Tm_app {hd=head; args} ->
      ...
      let rec flatten t : ML _ = match (SS.compress t).n with
        | Tm_arrow {bs=binders; comp={ n = Total t }} when is_arrow t ->
        ...
        | Tm_ascribed {tm=e} ->
            flatten e
        | _ ->
            raise_error0 Errors.Fatal_NotFunctionType (Format.fmt1 "%s: not a function type" (show t_head))
      in
      let binders, comp = flatten t_head in
      // Format.print1 "[debug] type of [head] is %s\n" (show t_head);
      ...
      if List.length binders < List.length args then
        raise_error0 Errors.Fatal_BinderAndArgsLengthMismatch (Format.fmt3 "The head of this application, after being applied to %s \
      ...
      // Format.print2 "[debug] length binders=%s, length args=%s\n"
      //  (show n) (show n');

      let binders, comp = SS.open_comp binders comp in
      let rec final_type subst binders_comp args : ML _ =
        let binders = fst binders_comp in
        let comp = snd binders_comp in
        match binders, args with
        | [], [] ->
            nm_of_comp (SS.subst_comp subst comp)
        | binders, [] ->
            begin match (SS.compress (SS.subst subst (mk (Tm_arrow {bs=binders; comp})))).n with
            | Tm_arrow {bs=binders; comp} -> N (mk (Tm_arrow {bs=binders; comp=close_comp binders comp}))
            | _ -> failwith "wat?"
            end
        | [], _ :: _ ->
      ...
      // Format.print1 "[debug]: final type of application is %s\n" (string_of_nm final_type);
      ...
      final_type, mk (Tm_app {hd=s_head; args=s_args}), mk (Tm_app {hd=u_head; args=u_args})
  ...
  | Tm_let _ ->
      failwith (Format.fmt1 "[infer]: Tm_let %s" (show e))
  | Tm_type _ ->
      failwith "impossible (DM stratification)"
  | Tm_arrow _ ->
      failwith "impossible (DM stratification)"
  | Tm_refine _ ->
      failwith (Format.fmt1 "[infer]: Tm_refine %s" (show e))
  | Tm_uvar _ ->
      failwith (Format.fmt1 "[infer]: Tm_uvar %s" (show e))
  | Tm_delayed _ ->
      failwith "impossible (compressed)"
  | Tm_unknown ->
      failwith (Format.fmt1 "[infer]: Tm_unknown %s" (show e))

and mk_match env e0 branches f : ML _ =
  ...
  if has_m then begin
    ...
    let s_branches = List.map (fun (pat, guard, s_body) ->
      let s_body = mk (Tm_app {hd=s_body; args=[ S.bv_to_name p, S.as_aqual_implicit false ]}) in
        (pat, guard, s_body)
      ) s_branches in
    ...
    let s_e =
      U.abs [ S.mk_binder p ]
            (mk (Tm_match {scrutinee=s_e0; ret_opt=None; brs= s_branches; rc_opt=None}))
            (Some (U.residual_tot U.ktype0))
    in
    let t1_star =  U.arrow [S.mk_binder <| S.new_bv None p_type] (S.mk_Total U.ktype0) in
    M t1,
    mk (Tm_ascribed {tm=s_e; asc=(Inl t1_star, None, false); eff_opt=None}) ,
    mk (Tm_match {scrutinee=u_e0; ret_opt=None; brs=u_branches; rc_opt=None})
  end else begin
    let s_branches = List.map close_branch s_branches in
    let u_branches = List.map close_branch u_branches in
    let t1_star = t1 in
    N t1,
    mk (Tm_ascribed {tm=mk (Tm_match {scrutinee=s_e0; ret_opt=None; brs=s_branches; rc_opt=None}); asc=(Inl t1_star, None, false); eff_opt=None}),
    mk (Tm_match {scrutinee=u_e0; ret_opt=None; brs=u_branches; rc_opt=None})
  end

and mk_let (env: env_) (binding: letbinding) (e2: term)
    (proceed: env_ -> term -> ML (nm & term & term))
    (ensure_m: env_ -> term -> ML (term & term & term)) : ML _ =
  ...
  | N t1, s_e1, u_e1 ->
      // Format.print1 "[debug] %s is NOT a monadic let-binding\n" (show binding.lbname);
      ...
      nm_rec,
      mk (Tm_let {lbs=(false, [ { s_binding with lbdef = s_e1 } ]); body=SS.close x_binders s_e2}),
      mk (Tm_let {lbs=(false, [ { u_binding with lbdef = u_e1 } ]); body=SS.close x_binders u_e2})

  | M t1, s_e1, u_e1 ->
      // Format.print1 "[debug] %s IS a monadic let-binding\n" (show binding.lbname);
      ...
      let p = S.gen_bv "p''" None p_type in
      // e2* p
      let s_e2 = mk (Tm_app {hd=s_e2; args=[ S.bv_to_name p, S.as_aqual_implicit false ]}) in
      // fun x -> s_e2* p; this takes care of closing [x].
      let s_e2 = U.abs x_binders s_e2 (Some (U.residual_tot U.ktype0)) in
      // e1* (fun x -> e2* p)
      let body = mk (Tm_app {hd=s_e1; args=[ s_e2, S.as_aqual_implicit false ]}) in
      M t2,
      U.abs [ S.mk_binder p ] body (Some (U.residual_tot U.ktype0)),
      mk (Tm_let {lbs=(false, [ { u_binding with lbdef = u_e1 } ]); body=SS.close x_binders u_e2})
  end
...
and type_of_comp t : ML _ = U.comp_result t

// This function expects its argument [c] to be normalized and to satisfy [is_C c]
and trans_F_ (env: env_) (c: typ) (wp: term): ML term =
  if not (is_C c) then
    raise_error c Error_UnexpectedDM4FType (Format.fmt1 "Not a DM4F C-type: %s" (show c));
  ...
  | Tm_app {hd=head; args} ->
      ...
      mk (Tm_app {hd=head; args=List.map2 (fun (arg, q) (wp_arg, q') ->
        ...
        then Errors.log_issue
                    ...
                    (Format.fmt2 "Incoherent implicit qualifiers %s %s\n"
                                (print_implicit q)
                                (print_implicit q'));
  ...
  | Tm_arrow {bs=binders; comp} ->
      ...
      let app = mk (Tm_app {hd=wp;args=List.map (fun bv -> S.bv_to_name bv, S.as_aqual_implicit false) bvs}) in
...
and trans_G (env: env_) (h: typ) (is_monadic: bool) (wp: typ): ML comp =
  if is_monadic then
    mk_Comp ({
      comp_univs = [U_unknown];
...
let n = N.normalize [ Env.DontUnfoldAttr [PC.tac_opaque_attr]; Env.Beta; Env.UnfoldUntil delta_constant; Env.DoNotUnfoldPureLets; Env.Eager_unfolding; Env.EraseUniverses ]
...
let trans_F (env: env_) (c: typ) (wp: term): ML term =
  trans_F_ env (n env.tcenv c) (n env.tcenv wp)

// A helper to check that the terms elaborated by DMFF are well-typed
let recheck_debug (s:string) (env:FStarC.TypeChecker.Env.env) (t:S.term) : ML S.term =
  if !dbg then
    Format.print2 "Term has been %s-transformed to:\n%s\n----------\n" s (show t);
  let t', _, _ = TcTerm.tc_term env t in
  if !dbg then
    Format.print1 "Re-checked; got:\n%s\n----------\n" (show t');
  t'
...
let cps_and_elaborate (env:FStarC.TypeChecker.Env.env) (ed:S.eff_decl)
  ...
  let raise_error #a code msg : ML a = Errors.raise_error signature.pos code msg in
  ...
  let wp_a = N.normalize [ Env.Beta ] env (mk (Tm_app {hd=wp_type; args=[ (S.bv_to_name a, S.as_aqual_implicit false) ]})) in
  ...
  let effect_signature =
    ...
    mk (Tm_arrow {bs=binders; comp=effect_marker})
  in
  ...
  let elaborate_and_star dmff_env other_binders item =
    ...
    if not (TcComm.is_total_lcomp item_comp) then
      raise_error0 Errors.Fatal_ComputationNotTotal (Format.fmt2 "Computation for [%s] is not total : %s !" (show item) (TcComm.lcomp_to_string item_comp));
  ...
  let lift_from_pure_wp =
      ...
      | Tm_abs {bs=b1 :: b2 :: bs; body; rc_opt=what} ->
        ...
        let wp_b1 =
          let raw_wp_b1 = mk (Tm_app {hd=wp_type; args=[ (S.bv_to_name b1.binder_bv, S.as_aqual_implicit false) ]}) in
        ...
        let fail () =
          let error_msg =
            Format.fmt2 "The body of return_wp (%s) should be of type Type0 but is of type %s"
              (show body)
              (match what' with
               | None -> "None"
               | Some rc -> FStarC.Ident.string_of_lid rc.residual_effect)
  ...
  let register maybe_admit name item =
    ...
    | Some (_us,_t) -> begin
      if Debug.any () then
          Format.print1 "DM4F: Applying override %s\n" (string_of_lid l');
  ...
  let dmff_env, actions = List.fold_left (fun (dmff_env, actions) action ->
    ...
    in
    if !dbg
    then Format.print4 "original action_params %s, end action_params %s, type %s, term %s\n"
        (show params_un)
        (show action_params)
  ...
  let repr =
    ...
    U.abs binders (trans_F dmff_env (mk (Tm_app {hd=repr; args=[ S.bv_to_name a, S.as_aqual_implicit false ]})) (S.bv_to_name wp)) None
  in
  ...
  let pre, post =
    ...
    | Tm_abs {bs=type_param :: effect_param; body=arrow} ->
        ...
        | Tm_arrow {bs=wp_binders; comp=c} ->
            ...
            let post = match post_args with
                ...
                | [] ->
                  let err_msg =
                    Format.fmt1 "Impossible to generate DM effect: no post candidate %s (Type variable does not appear)"
                      (show arrow)
                  in
                  raise_error0 Errors.Fatal_ImpossibleToGenerateDMEffect err_msg
                | _ ->
                  let err_msg =
                      Format.fmt1 "Impossible to generate DM effect: multiple post candidates %s" (show arrow)
                  in
                  raise_error0 Errors.Fatal_ImpossibleToGenerateDMEffect err_msg
            in
        ...
        | _ ->
            raise_error Errors.Fatal_ImpossiblePrePostArrow (Format.fmt1 "Impossible: pre/post arrow %s" (show arrow))
        end
    | _ ->
        raise_error Errors.Fatal_ImpossiblePrePostAbs (Format.fmt1 "Impossible: pre/post abs %s" (show wp_type))
  in
