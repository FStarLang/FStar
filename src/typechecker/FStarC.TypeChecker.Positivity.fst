...
let debug_positivity (env:env_t) (msg:unit -> ML string) : ML unit =
  if !dbg_Positivity
  then Format.print_string ("Positivity::" ^ msg () ^ "\n")
...
let string_of_lids lids : ML _ =
    L.map string_of_lid lids |> String.concat ", "

(* Normalize a term before checking for non-strictly positive occurrences *)
let normalize env t : ML _ =
  ...
  = let rec aux t args : ML _ =
        ...
        | _ ->
          raise_error 
             ...
             (Format.fmt3 "Unexpected application of type parameters %s to a data constructor %s : %s"
...
let rec term_as_fv_or_name (t:term) 
  : ML (option (either (fv & universes) bv))
...
let open_sig_inductive_typ env se : ML _ =
    match se.sigel with
...
let rec min_l (#a:Type) (def:int) (l:list a) (f:a -> ML int) : ML int =
    match l with
    | [] -> def
...
let max_uniformly_recursive_parameters (env:env_t)
                                       ...
                                       (ty:term)
  : ML int
  = let max_matching_prefix (longer:list 'a) (shorter:list 'b) (f:'a -> 'b -> ML bool)
      : ML (option int)
      = let rec aux n ls ms : ML (option int) =
          match ls, ms with
          | _, [] -> Some n
          | l::ls, m::ms -> 
    ...
    let params_to_string () =
        (L.map show params |> String.concat ", ")
    in
    debug_positivity env (fun _ ->
      Format.fmt2 "max_uniformly_recursive_parameters? params=%s in %s"
                 (params_to_string())
                 (show ty));
    let rec aux ty : ML int =
        debug_positivity env (fun _ ->
          Format.fmt1 "max_uniformly_recursive_parameters.aux? %s"
                 (show ty));
        if L.for_all (fun mutual -> not (ty_occurs_in mutual ty)) mutuals
        then n_params
        else (
        ...
        | Tm_app _ ->
          ...
          | Tm_fvar fv ->
            ...
            then (
              debug_positivity env (fun _ -> 
                Format.fmt2 "Searching for max matching prefix of params=%s in args=%s"
                           (params_to_string())
        ...
        | Tm_match {scrutinee; brs=branches} ->
          min (aux scrutinee)
              (min_l branches
                     (fun (p, _, t) ->
                       let bs = L.map mk_binder (pat_bvs p) in
    ...
    let res = aux ty in
    debug_positivity env (fun _ ->
      Format.fmt3 "result: max_uniformly_recursive_parameters(params=%s in %s) = %s"
                 (params_to_string())
                 (show ty)
                 (show res));
    res
...
let mark_uniform_type_parameters (env:env_t)
  ...
  = let mark_tycon_parameters tc datas =
        ...
        then (
          let _, non_uniform_params = L.splitAt max_uniform_prefix ty_param_binders in
          List.iter 
            (fun param ->
                 ...
                 then ( //if marked strictly positive, it must be uniform
                   raise_error 
                     ...
                     (Format.fmt1 "Binder %s is marked strictly positive, \
    ...
    | Sig_bundle {ses; lids} ->
      ...
      let tcs = L.map (fun tc -> mark_tycon_parameters tc datas) tcs in
...
(* t is an application of a type constructor T ps is 
   ...
   indexes and arities
 *)
let check_no_index_occurrences_in_arities env mutuals (t:term) : ML _ =
  debug_positivity env (fun _ ->
    Format.fmt2 "check_no_index_occurrences of (mutuals %s) in arities of %s"
   ...
   *)
   ...
   L.iter (fun mutual -> 
             ...
             then raise_error index Errors.Error_InductiveTypeNotSatisfyPositivityCondition
                            (Format.fmt3 "Type %s is not strictly positive since it instantiates \
          ...
          let rec aux subst formals indices : ML unit =
            ...
            | f::formals, i::indices ->
              ...
              then (
                ...
                debug_positivity env (fun _ ->
                  Format.fmt2 "Skipping %s : %s (non-arity)"
                    (show (fst i))
                    (show f_t))
              );
...
////////////////////////////////////////////////////////////////////////////////
let mutuals_unused_in_type (mutuals:list lident) t : ML _ =
  let mutuals_occur_in t = BU.for_some (fun lid -> ty_occurs_in lid t) mutuals in
  let rec ok t : ML bool =
  ...
  = //normalize the type to unfold any type abbreviations
    ...
    debug_positivity env (fun _ ->
      Format.fmt2
        "Checking strict positivity of {%s} in type, after normalization %s "
          (string_of_lids mutuals)
          (show in_type));
    if L.for_all (fun mutual -> not (ty_occurs_in mutual in_type)) mutuals
    then true   //ty does not occur in in_type, so obviously strictly positive
      ...
      | Tm_app {hd=t; args} ->  //the binder type is an application
        ...
        | None ->
          debug_positivity env (fun _ -> 
            Format.fmt2 "Failed to check positivity of %s in a term with head %s"
                       (string_of_lids mutuals)
                       (show t));
        ...
        | Some (Inr x) -> //head is an name
          ...
          debug_positivity env (fun _ ->
            Format.fmt3 "Tm_app, head bv, in_type=%s, head_bv=%s, head_ty=%s"
                       (show in_type)
                       (show x)
                       (show head_ty));
        ...
        | Some (Inl (fv, us)) ->
          ...
          then (
            ...
            debug_positivity env (fun _ ->
              ...
              L.for_all (fun (t, _) -> mutuals_unused_in_type mutuals t) args
          )
          else (
            ...
            debug_positivity env (fun _ -> 
              Format.fmt1 "Checking strict positivity in the Tm_app node, \
    ...
    let rec aux (bs:binders) args
      ...
      = match bs, args with
        ...
        | [], _ ->
          ...
          L.for_all (fun (arg, _) -> mutuals_unused_in_type mutuals arg) args
          
        | b::bs, (arg, _)::args ->
          debug_positivity env (fun _ -> 
            Format.fmt3 "Checking positivity of %s in argument %s and binder %s"
                       (string_of_lids mutuals)
                       (show arg)
                       (show b));
          ...
          then ( 
            debug_positivity env (fun _ -> 
              Format.fmt3 "Failed checking positivity of %s in argument %s and binder %s"
                              (string_of_lids mutuals)
                              (show arg)
                              (show b));
            false
...
and ty_strictly_positive_in_arguments_to_fvar
  ...
  : ML bool
  = debug_positivity env (fun _ ->
        Format.fmt4 "Checking positivity of %s in application of fv %s to %s (t=%s)"
                   (string_of_lids mutuals)
                   (string_of_lid fv)
                   (Print.args_to_string args)
                   (show t));
    ...
    then (
      ...
      let fv_ty = 
        ...
        | _ ->
          raise_error fv Errors.Error_InductiveTypeNotSatisfyPositivityCondition
            (Format.fmt1 "Type of %s not found when checking positivity"
                       (string_of_lid fv))
      in
      ...
      //if fv has already been unfolded with same arguments, return true
        ...
        let num_uniform_params = 
          ...
          | Some n -> n
        in
        let params, _rest = L.splitAt num_uniform_params args in            
        ...
        then (
          ...
          debug_positivity env (fun _ -> 
            Format.fmt3 "Checking positivity in datacon, number of type parameters is %s, \
...
and ty_strictly_positive_in_datacon_of_applied_inductive (env:env_t)
  ...
  = debug_positivity env (fun _ ->
    ...
    let dt =
      ...
      | None -> 
        raise_error 
          (range_of_lid dlid)
          Errors.Error_InductiveTypeNotSatisfyPositivityCondition
          (Format.fmt1 "Data constructor %s not found when checking positivity"
                      (string_of_lid dlid))
    in
    ...
    let args, rest = L.splitAt num_ibs args in
    ...
    check_no_index_occurrences_in_arities env mutuals t;
    let rec strictly_positive_in_all_fields env fields : ML bool =
        match fields with
        | [] -> true
        | f::fields ->
          debug_positivity env (fun _ ->
            Format.fmt2 "Checking field %s : %s for indexes and positivity"
              (show f.binder_bv)
...
*)
let name_strictly_positive_in_type env (bv:bv) t : ML _ =
...
let ty_strictly_positive_in_datacon_decl (env:env_t)
  ...
  = let dt =
      ...
      | None -> raise_error dlid
                            Errors.Error_InductiveTypeNotSatisfyPositivityCondition
                            (Format.fmt1 "Error looking up data constructor %s when checking positivity"
                                       (string_of_lid dlid))
    in
    ...
    let check_annotated_binders_are_strictly_positive_in_field f =
        ...
        | Some b ->
          raise_error b Error_InductiveTypeNotSatisfyPositivityCondition
                      (Format.fmt2 "Binder %s is marked %s, \
    ...
    in
    let rec check_all_fields env fields : ML bool =
        match fields with
        | [] -> true
        | field::fields ->
