...
let tc_eff_decl env0 (ed:Syntax.eff_decl) =
  ...
  let wp_with_fresh_result_type env mname signature =
       let fail t = raise_error (Err.unexpected_signature_for_monad env mname t) (range_of_lid mname) in
    ...
    (if Options.no_positivity () || Options.lax ()  then ()  //skipping positivity check if lax mode
       ...
       let b = List.iter (fun ty ->
         ...
         if not b then
           ...
           in
           Errors.maybe_fatal_error r (Errors.InductiveTypeNotSatisfyPositivityCondition, ("Inductive type " ^ lid.str ^ " does not satisfy the positivity condition"))
         else ()
       ) tcs in
       ());
