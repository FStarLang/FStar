 ...
 match s.n with
  | Tm_arrow(bs, c) ->
    ...
    if n < 2 then fail ()
      ...
      let args, wp = BU.first_N (n-1) bs in
      args, begin match wp with | [wp, _] -> wp.sort | _ -> failwith "Imposible" end
  | _ -> fail()


let tc_subeff_decl env sub r =
  let env0 = env in

  (* 0. We open the sub_effect, typecheck the binders, *)
  (*    retrieve the type of the wp of both source and target and *)
  (*    make sure that the source and the target are well typed *)
  (*    in an environment extended with a variable [wp] of the relevant type *)

  let uvs, bs, sub = SS.open_sub_eff sub in
  (* There should not be any universes variables yet *)
  assert( uvs = [] );
  let effect_binders, env, _ = tc_tparams env0 bs in

  if Env.debug env <| Options.Other "Lift"
  then BU.print1 "After opening a sub effect %s\n" (Print.sigelt_to_string (Sig_sub_effect (sub, r))) ;

  (* Note : at this point effect args contains everything but the wp *)
  let effect_wp_typ effect_lid (effect_args : args) =
    let eff_signature = Env.lookup_effect_lid env effect_lid in
    let effect_bs, wp_typ = monad_signature env effect_lid eff_signature in
    let effect_args_sub = List.map2 (fun (x, _) (arg, _) -> NT(x, arg)) effect_bs effect_args in
    let wp_typ = SS.subst effect_args_sub wp_typ in
    tc_check_trivial_guard env wp_typ (fst <| U.type_u ())
  in

  let source = sub.sub_eff_source.comp_typ_name in
  let target = sub.sub_eff_target.comp_typ_name in

  let src_wp = effect_wp_typ source sub.sub_eff_source.effect_args in
  let tgt_wp = effect_wp_typ target sub.sub_eff_target.effect_args in

  let tc_partial_comp_typ env ct wp_typ =
    let bwp = S.new_bv None wp_typ in
    let env = Env.push_binders env [S.mk_binder bwp] in
    let wp = S.bv_to_name bwp in
    let complete_effect =
      S.mk (Comp ({ ct with effect_args = ct.effect_args @ [as_arg wp] })) None Range.dummyRange
    in
    let c, _, g = TcTerm.tc_comp env complete_effect in
    Rel.force_trivial_guard env g ;
    match c.n with
    | Comp ct -> ct.comp_univs
    | _ -> failwith "Impossible : typechecking a Comp should result in a Comp"
  in

  (* We only retrieve the universe variables which will be generalized together at the last step *)
  let src_univs = tc_partial_comp_typ env sub.sub_eff_source src_wp in
  let tgt_univs = tc_partial_comp_typ env sub.sub_eff_target tgt_wp in


  (* 1. We find out which kind of lifting we must typecheck and typecheck the wp part *)

  let lift, lift_wp =
    match sub.sub_eff_lift, sub.sub_eff_lift_wp with
    | None, None ->
      failwith "Impossible : a subEffect without any lift should not be generated by desugaring"
    | lift, Some lift_wp ->
      (* Covers both the "classic" format and the reifiable case. *)
      let expected_k  = U.arrow [S.null_binder src_wp] (S.mk_Total tgt_wp) in
  ...
  (* 2. If we have a reifiable lift we typecheck it *)
  ...
  let repr_type eff_name effect_args wp =
    ...
    | Some ed ->
      let univ_inst = List.map (fun _ -> U_unknown) ed.univs in
      let repr = Env.inst_effect_fun_with univ_inst env ed ([], ed.repr) in
      if not (ed.qualifiers |> List.contains Reifiable) then
        no_reify eff_name
      else
        mk (Tm_app(repr, effect_args@[as_arg wp])) None (Env.get_range env)
  in

  let tc_lift lift =
    let wp_a = S.new_bv None src_wp in
    let wp_a_typ = S.bv_to_name wp_a in
    let repr_f = repr_type source sub.sub_eff_source.effect_args wp_a_typ in
    let repr_result =
      let lift_wp = N.normalize [N.EraseUniverses; N.AllowUnboundUniverses] env lift_wp in
      let lift_wp_a = mk (Tm_app(lift_wp, [as_arg wp_a_typ])) None (Env.get_range env) in
      repr_type target sub.sub_eff_target.effect_args lift_wp_a
    in
    let expected_k =
      let bs = [S.mk_binder wp_a; S.null_binder repr_f] in
      U.arrow bs (S.mk_Total repr_result)
    in
    let expected_k, _, _ = tc_tot_or_gtot_term env expected_k in
    tc_check_trivial_guard env lift expected_k
  in
  ...
  (* 3. We generalize the universes and rebuild the resulting sub_effect *)
  ...
  let univs =
    ...
    in
    TcUtil.generalize_universes env t |> fst
  in

  let sub = {
      sub_eff_univs = univs ;
      sub_eff_binders = effect_binders ;
      sub_eff_source = { sub.sub_eff_source with comp_univs = src_univs } ;
      sub_eff_target = { sub.sub_eff_target with comp_univs = tgt_univs } ;
      sub_eff_lift_wp = Some lift_wp ;
      sub_eff_lift = lift
    }
  in
  SS.close_sub_eff sub
...
and tc_decl env se: list<sigelt> * Env.env * list<sigelt> =
  ...
  | Sig_sub_effect(sub, r) ->
    let sub = tc_subeff_decl env sub r in
