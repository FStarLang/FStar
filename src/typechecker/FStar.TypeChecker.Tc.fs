...
and tc_inductive env ses quals lids =
    ...
    *)
    ...
    let env, tcs, g = List.fold_right (fun tc (env, all_tcs, g)  ->
            let env, tc, tc_u, guard = TcInductive.tc_tycon env tc in
    ...
    let datas, g = List.fold_right (fun se (datas, g) ->
            let data, g' = TcInductive.tc_data env tcs se in
    ...
    let tcs, datas = TcInductive.generalize_and_inst_within env0 g tcs datas in
    ...
    then [sig_bndle], data_ops_ses
        ...
        let ses = if is_unopteq then unoptimized_haseq_scheme () else TcInductive.optimized_haseq_scheme sig_bndle tcs datas env0 tc_assume in
