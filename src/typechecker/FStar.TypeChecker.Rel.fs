...
let mk_eq2 env t1 t2 =
    ...
    *)
    ...
    let tt, _ = Env.new_uvar t1.pos (Env.all_binders env) t_type in
    U.mk_eq2 u tt t1 t2
...
and solve_t' (env:Env.env) (problem:tprob) (wl:worklist) : solution =
    ...
    let project orig (env:Env.env) (wl:worklist) (i:int) (p:im_or_proj_t) : option<solution> =
        ...
        let rec gs k =
            let bs, _ = U.arrow_formals_comp k in
                        ...
                        | Tm_uvar (uvar, _) ->
                          (match pat_vars env [] uv_args with
                           ...
                           | Some scope ->
                             ...
                             let uv_sol = mk_abs env scope k' (S.mk_Total (U.type_u () |> fst)) in
   ...
   let flex_flex orig (lhs:flex_t) (rhs:flex_t) : solution =
        ...
        let solve_both_pats wl (u1, k1, xs, args1) (u2, k2, ys, args2) r =
            ...
            then solve env (solve_prob orig None [] wl)
                ...
                let k_u', sub_probs =
                    let bs, c_k1' = U.arrow_formals_comp (N.normalize [N.Beta] env k1) in
                    let cs, c_k2' = U.arrow_formals_comp (N.normalize [N.Beta] env k2) in
