...
let check_guard msg env g =
    ...
    then ()
    else raise_err ((Errors.FreeVariables "in guard"), BU.format2 "Guard has free variables (%s): %s"
                                msg
...
let check_term msg env t =
    ...
    if BU.set_is_empty s
    then ()
    else raise_err ((Errors.FreeVariables "in term"), BU.format3 "Term <%s> has free variables (%s): %s"
    ...
    let solve_t_flex_rigid (patterns_only:bool) (orig:prob)
        ...
        let subterms ps : option<im_or_proj_t> =
            ...
            then Some (((uv,k_uv),xs,c), ps, decompose env t2)
                     ...
                     | _ -> raise_error ((Errors.IllTyped "Application"), (BU.format3 "Impossible: ill-typed application %s : %s\n\t%s"
