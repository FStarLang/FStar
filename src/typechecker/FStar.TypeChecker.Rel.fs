...
and solve_t_flex_rigid_eq env (orig:prob) wl
    ...
    let imitate_app (orig:prob) (env:Env.env) (wl:worklist)
        ...
        let lhs', lhs'_last_arg, wl =
              let t_last_arg = Env.typeof_well_typed_tot_or_gtot_term_maybe_fastpath ({env with lax=true; use_bv_sorts=true; expected_typ=None}) (fst last_arg_rhs) in
...
and solve_t_flex_flex env orig wl (lhs:flex_t) (rhs:flex_t) : solution =
    ...
    | SUBINV ->
      if wl.defer_ok
      then giveup_or_defer env orig wl Deferred_flex (Thunk.mkv "flex-flex subtyping")
...
let try_solve_deferred_constraints defer_ok smt_ok deferred_to_tac_ok env (g:guard_t) : guard_t =
  Profiling.profile (fun () ->
   let fail (d,s) =
