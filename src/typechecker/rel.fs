...
let find_univ_uvar u s = Util.find_map s (function
...
let rec pat_vars env seen args : option<binders> = match args with
    ...
    | hd::rest ->
        begin match pat_var_opt env seen hd with
            | None -> if Env.debug env <| Options.Other "Rel" then Util.print1 "Not a pattern: %s\n" (Print.term_to_string <| fst hd); None //not a pattern
...
//              end
            ...
            | Some (rhs_bound, sub_probs) ->
              ...
              let _ = if Env.debug env <| Options.Other "RelCheck"
                      then let wl' = {wl with attempting=TProb eq_prob::sub_probs} in
                           Util.print1 "After joining refinements: %s\n" (wl_to_string wl') in
              match solve_t env eq_prob ({wl with attempting=sub_probs}) with
                | Success _ ->
                  let wl = {wl with attempting=rest} in
                  let wl = solve_prob' false (TProb tp) None [] wl in
...
and solve_t' (env:Env.env) (problem:tprob) (wl:worklist) : solution =
    ...
    let imitate orig (env:Env.env) (wl:worklist) (p:im_or_proj_t) : solution =
        ...
        let im = mk_abs env xs (h gs_xs) c in
        if Env.debug env <| Options.Other "Rel"
    ...
    let project orig (env:Env.env) (wl:worklist) (i:int) (p:im_or_proj_t) : option<solution> =
        ...
        let rec gs k =
            let bs, _ = Util.arrow_formals_comp k in
        ...
        then None
             ...
             let proj = mk_abs wl.tcenv xs (S.mk_Tm_app xi g_xs None r) c in
    ...
    let solve_t_flex_rigid patterns_only orig (lhs:(flex_t * option<binders>)) (t2:typ) (wl:worklist) =
            ...
            then Some (((uv,k_uv),xs,c), ps, decompose env t2) 
                 ...
                 let rec elim k args : option<(binders * comp)> = 
                     ...
                     | Tm_app _, _ -> //k=?u x1..xn
                       let uv, uv_args = Util.head_and_args k in 
                        ...
                        | Tm_uvar (uvar, _) -> 
                          (match pat_vars env [] uv_args with 
                           ...
                           | Some scope -> 
                             ...
                             let uv_sol = mk_abs env scope k' (S.mk_Total (Util.type_u () |> fst)) in
        ...
        let rec imitate_or_project n (stopt:option<im_or_proj_t>) i =
            ...
            match hd.n with
                ...
                | _ ->
                    ...
                    then true
                          ...
                          then Util.print1 "Free variables are %s" (names_to_string fvs_hd); false) in
            ...
            then (if not patterns_only 
                  ...
                  then imitate' orig env wl (subterms args_lhs)
                    ...
                    let sol = match vars with
                        | [] -> t2
                        | _ -> u_abs wl k_uv (sn_binders env vars) t2 in
   ...
   *)
   let flex_flex orig (lhs:flex_t) (rhs:flex_t) : solution =
        ...
        let force_quasi_pattern xs_opt (t, u, k, args) =
            ...
            *)
           let all_formals, _ = Util.arrow_formals_comp k in 
            ...
            let rec aux pat_args              (* pattern arguments, so far *)
              ...
              match formals, args with
                | [], [] ->
                    ...
                    let sol = TERM((u,k), u_abs wl k all_formals t') in
        ...
        let solve_both_pats wl (u1, k1, xs, args1) (u2, k2, ys, args2) r =
            ...
            then solve env (solve_prob orig None [] wl)
                ...
                let k_u', sub_probs =
                    let bs, c_k1' = Util.arrow_formals_comp (N.normalize [N.Beta] env k1) in
                    let cs, c_k2' = Util.arrow_formals_comp (N.normalize [N.Beta] env k2) in
                    let k1' = Env.result_typ wl.tcenv c_k1' in
                    let k2' = Env.result_typ wl.tcenv c_k2' in
                ...
                let sub1 = u_abs wl knew1 xs u_zs in
                ...
                then giveup_or_defer orig "flex-flex: failed occcurs check"
                     ...
                     then let wl = solve_prob orig None [sol1] wl in
                          solve env wl
                     else let sub2 = u_abs wl knew2 ys u_zs in
...
and solve_c (env:Env.env) (problem:problem<comp,unit>) (wl:worklist) : solution =
    ...
    then solve env (solve_prob orig None [] wl)
                 ...
                 then Util.print3 "solve_c %s %s %s\n" 
                      ...
                      then solve_eq c1_comp c2_comp
                         ...
                         begin match Env.monad_leq env c1.effect_name c2.effect_name with
                           | None -> 
                             ...
                             && Env.non_informative env (N.normalize [N.Eager_unfolding; N.UnfoldUntil Delta_constant] env (Env.result_typ env (S.mk_Comp c2)))
...
let abstract_guard env x g = match g with
    | None
    ...
    | Some g ->
      let f = match g.guard_f with
        | NonTrivial f -> f
        | _ -> failwith "impossible" in
      Some ({g with guard_f=NonTrivial <| mk_abs env [mk_binder x] f (mk_Total U.ktype0)})
...
let teq env t1 t2 : guard_t =
 match try_teq env t1 t2 with
    ...
    | Some g ->
      if debug env <| Options.Other "Rel" 
      then Util.print3 "teq of %s and %s succeeded with guard %s\n" 
                        (Print.term_to_string t1) 
...
let rec solve_deferred_constraints env (g:guard_t) =
   ...
   if Env.debug env <| Options.Other "RelCheck"
   then Util.print2 "Trying to solve carried problems: begin\n\t%s\nend\n and %s implicits\n"  (wl_to_string wl) (string_of_int (List.length g.implicits));
...
let resolve_implicits g = 
  let unresolved u = match Unionfind.find u with
    | Uvar -> true
    | _ -> false in
  let rec until_fixpoint (out, changed) implicits = 
