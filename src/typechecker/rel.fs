...
let new_problem env lhs rel rhs elt loc reason = {
    ...
    rhs=rhs;
    element=elt;
    logical_guard=new_uvar (Env.get_range env) (Env.bound_vars env |> List.map mk_binder) U.ktype0; //logical guards are always squashed?
    scope=[];
    reason=[reason];
    loc=loc;
    rank=None;
        ...
        let check_head fvs1 t2 =
            ...
            match hd.n with
                ...
                | _ ->
                    ...
                    then true
                    else (if Tc.Env.debug env <| Options.Other "Rel" then Util.fprint1 "Free variables are %s" (Print.freevars_to_string fvs_hd); false) in
