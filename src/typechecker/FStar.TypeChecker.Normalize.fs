...
let eta_expand (env:Env.env) (t:term) : term =
  ...
  | _ ->
      ...
      | Tm_uvar (u,s) ->
        ...
        then t
        else let _, ty, _ = env.type_of_tot_or_gtot_term ({env with lax=true; use_bv_sorts=true; expected_typ=None}) t true in
             eta_expand_with_type env t ty
      | _ ->
        let _, ty, _ = env.type_of_tot_or_gtot_term ({env with lax=true; use_bv_sorts=true; expected_typ=None}) t true in
        eta_expand_with_type env t ty
      end
