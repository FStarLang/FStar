...
and close_lcomp_opt cfg env lopt = match lopt with
    | Some (Inl lc) -> //NS: Too expensive to close potentially huge VCs that are hardly read
      ...
      else Some (Inr (lc.lcomp_name, flags)) //retaining the effect name and flags is sufficient; TODO: see note in FStar.Syntax.Syntax ... maybe retain indices
    | _ -> lopt
...
let ghost_to_pure_lcomp env (lc:lcomp) =
    ...
    if U.is_ghost_effect lc.lcomp_name
...
let eta_expand (env:Env.env) (t:typ) : typ =
  ...
  | _ ->
      ...
      begin match (SS.compress head).n with
      | Tm_uvar(_, thead) ->
        let formals, _ = U.arrow_formals_comp thead in
