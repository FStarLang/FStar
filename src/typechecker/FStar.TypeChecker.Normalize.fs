...
let built_in_primitive_steps : list<primitive_step> =
    ...
    let bounded_arith_ops
        ...
        let div_mod_signed =
          let abs (a:Z.t) =
              if Z.le_big_int a Z.zero
              then Z.minus_big_int a
              else a
          in
          let div (a:Z.t) (b:Z.t) =
            if (Z.ge_big_int a Z.zero &&
                Z.lt_big_int b Z.zero)
            ||  (Z.lt_big_int a Z.zero &&
                Z.ge_big_int b Z.zero)
            then Z.minus_big_int (Z.div_big_int (abs a) (abs b))
            else Z.div_big_int (abs a) (abs b)
          in
          let rem (a:Z.t) (b:Z.t) =
