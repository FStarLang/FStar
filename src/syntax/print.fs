...
let term_to_string (t:term) = Printf.sprintf "%A" t
...
let bv_to_string bv =
    ...
    then bv.ppname.idText ^ (string_of_int bv.index)
        ...
        if !Options.hide_genident_nums && starts_with (bv.ppname.idText) "_" then
            try
                let _ = int_of_string (substring_from (bv.ppname.idText) 1) in "_?"
            with _ -> bv.ppname.idText
