...
let rec is_t_true t =
     ...
     (* [squash True] and [auto_squash u True] are also trivially true;
        the typechecker inserts these when checking a [prop] at [Type0]. *)
...
let is_ndet_effect l =
     lid_equals l PC.effect_NDET_lid
     || lid_equals l PC.effect_Ndet_lid
     || lid_equals l PC.effect_Nd_lid
...
let comp_smt_pats (c:comp) : ML (option term) =
  match comp_flags c |> U.try_find (function SMTPAT _ -> true | _ -> false) with
  | Some (SMTPAT p) -> Some p
  | _ -> None
...
let destruct typ lid =
  ...
  let head, args = head_and_args_full (un_uinst typ) in
...
let remove_inacc (t:term) : ML term =
    ...
    match bs with
    | [] -> t
    | _ -> mk_Tm_arrow (List.map no_acc bs) c t.pos
...
let mk_conj_opt phi1 phi2 = match phi1 with
  | None -> Some phi2
  | Some phi1 -> Some (mk_Tm_app tand [as_arg phi1; as_arg phi2] (Range.union_ranges phi1.pos phi2.pos))
let mk_binop op_t phi1 phi2 = mk_Tm_app op_t [as_arg phi1; as_arg phi2] (Range.union_ranges phi1.pos phi2.pos)
let mk_neg phi = mk_Tm_app t_not [as_arg phi] phi.pos
...
let b2t e = mk_Tm_app b2t_v [as_arg e] e.pos//implicitly coerce a boolean to a type
...
let refinement_hypothesis (t:typ) (v:term) : ML term =
  match (compress t).n with
  | Tm_refine {b; phi} ->
    let bs, phi = Subst.open_term [mk_binder b] phi in
    Subst.subst [NT ((List.hd bs).binder_bv, v)] phi
  | _ ->
    let hd, args = head_and_args_full t in
    (match (un_uinst hd).n, args with
     (* [squash p = _:unit{p}] *)
     | Tm_fvar fv, [(p, _)] when fv_eq_lid fv PC.squash_lid -> p
     | _ -> t_true)

(* Apply a postcondition to a result term, beta-reducing on the spot: a
   postcondition is almost always a literal abstraction, and leaving the redex
   in place makes every verification condition much harder to read (and to
   discharge). *)
let apply_post (p:term) (e:term) : ML term =
  if is_trivial_post p then t_true
  else
    match (compress p).n with
    | Tm_abs {b; body} ->
...
let mk_eq2 (u:universe) (t:typ) (e1:term) (e2:term) : ML term =
    ...
    mk_Tm_app eq_inst [iarg t; as_arg e1; as_arg e2] (Range.union_ranges e1.pos e2.pos)

let mk_eq3_no_univ =
  let teq3 = fvar_const PC.eq3_lid in
  fun t1 t2 e1 e2 ->
    mk_Tm_app teq3 [iarg t1; iarg t2; as_arg e1; as_arg e2]
...
let mk_decidable_eq t e1 e2 =
  mk_Tm_app decidable_eq [iarg t; as_arg e1; as_arg e2] (Range.union_ranges e1.pos e2.pos)

let b_or = fvar_const PC.op_Or
let mk_or e1 e2 =
  mk_Tm_app b_or [as_arg e1; as_arg e2] (Range.union_ranges e1.pos e2.pos)

let b_and = fvar_const PC.op_And
let mk_and e1 e2 =
  mk_Tm_app b_and [as_arg e1; as_arg e2] (Range.union_ranges e1.pos e2.pos)
