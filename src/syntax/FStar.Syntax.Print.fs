...
>>>>>>> origin/master:src/syntax/FStar.Syntax.Print.fs
        | _ ->
  ...
  match c.n with
    ...
    | Comp c ->
        let basic =
          ...
          then Util.format1 "Tot %s" (term_to_string (fst (List.hd c.effect_args)))
...
let abs_ascription_to_string ascription =
  ...
  begin match ascription with
      ...
      | Some (Inl lc) ->
          ...
          U.string_builder_append strb (Ident.text_of_lid lc.lcomp_name)
      | Some (Inr lid) ->
          U.string_builder_append strb "Some Inr " ;
