...
let rec maybe_equal_term (t1:term) (t2:term)
  ...
  = let t1 = canon_app t1 in
    ...
    let rec maybe_equal_args (args1 args2:args) : bool= 
        match args1, args2 with 
        | [], [] -> true
        | (a1, q1)::args1, (a2, q2)::args2 ->
          q1 = q2 &&
          maybe_equal_term a1 a2 &&
          maybe_equal_args args1 args2
        | _ -> false

    in        
    let maybe_equal_binders (bs1 bs2:binders) =
      if List.length bs1 = List.length bs2
      then List.forall2 (fun b1 b2 -> maybe_equal_term b1.binder_bv.sort b2.binder_bv.sort) bs1 bs2
      else false
    in
    let rec maybe_equal_univs u1 u2 = 
      match u1, u2 with
      | U_zero, U_zero -> true
      | U_bvar x, U_bvar y -> x = y
      | U_name x, U_name y -> Ident.ident_equals x y
      | U_succ u1, U_succ u2 -> maybe_equal_univs u1 u2
      | U_max us1, U_max us2 -> 
        List.length us1 = List.length us2 &&
        List.forall2 maybe_equal_univs us1 us2
      | U_unknown, U_unknown -> true
      | U_unif _, _
      | _, U_unif _ -> true
      | _ -> false
    in
    let maybe_equal_comp c1 c2 = 
      match c1.n, c2.n with
      | Total (t1, u1opt), Total (t2, u2opt)
      | GTotal (t1, u1opt), GTotal (t2, u2opt) ->
        maybe_equal_term t1 t2
        //Rel ignores the universe annotations when comparing these comps for equality.
      ...
      | Comp ct1, Comp ct2 ->
        ...
        maybe_equal_univs (U_max ct1.comp_univs) (U_max ct2.comp_univs) &&
        maybe_equal_term ct1.result_typ ct2.result_typ  &&      
        maybe_equal_args ct1.effect_args ct2.effect_args
      | _ -> false
    in
    let equal_data (f1:fv) (args1:Syntax.args) (f2:fv) (args2:Syntax.args) =
        ...
        if fv_eq f1 f2
        then maybe_equal_args args1 args2
        else false
    in
    let qual_is_inj = function
      ...
      | _ -> false
    in
    let heads_and_args_in_case_both_data_or_injective :option (fv * args * fv * args) =
      ...
      | Tm_fvar f, Tm_fvar g
        when fv_eq f g &&
             (qual_is_inj f.fv_qual ||
              List.mem (string_of_lid (lid_of_fv f)) injectives) ->
        Some (f, args1, g, args2)
      | _ -> None
    in
    let t1 = unmeta t1 in
    let t2 = unmeta t2 in
    ...
    | _ when heads_and_args_in_case_both_data_or_injective |> is_some ->  //matches only when both are data constructors
      heads_and_args_in_case_both_data_or_injective |> must |> (fun (f, args1, g, args2) ->
    ...
    | Tm_uinst (f, us1), Tm_uinst(g, us2) -> 
      maybe_equal_term f g &&
      maybe_equal_univs (U_max us1) (U_max us2)
    ...
    | Tm_app (h1, args1), Tm_app (h2, args2) ->
      maybe_equal_term h1 h2 &&
      maybe_equal_args args1 args2

    | Tm_type u, Tm_type v ->
      maybe_equal_univs u v
    ...
    | Tm_refine (t1, phi1), Tm_refine (t2, phi2) ->
      maybe_equal_term t1.sort t2.sort &&
      maybe_equal_term phi1 phi2

    | Tm_abs (bs1, body1, _rc1), Tm_abs (bs2, body2, _rc2)
      when List.length bs1 = List.length bs2 ->
      maybe_equal_binders bs1 bs2 &&
      maybe_equal_term body1 body2

    | Tm_arrow(bs1, c1), Tm_arrow(bs2, c2)
          when List.length bs1 = List.length bs2 ->
      maybe_equal_binders bs1 bs2 &&
      maybe_equal_comp c1 c2
