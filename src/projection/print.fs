...
let current_mod = ref "" 
let comp_arg_counter = ref 0

let space n = Util.string_make n " "
let get_ind () = space (!indent)
...
let arg_is_abs a = if not (is_inr (fst a)) then false else
  match (Util.compress_exp (right (fst a))).n with
  | Exp_fvar _  -> false
  | _ -> true
...
let rec sli (l:lident) : string =
   if l.nsstr = !current_mod || l.nsstr="Prims" then
    l.ident.idText
   else 
    l.str
...
(* This function prints the type it gets as argument verbatim.
   For already type-checked types use the typ_ norm_to_string
...
and typ_to_string x =
  ...
  | Typ_meta meta -> (meta|> meta_to_string)
...
and binder_to_string' is_arrow b = match b with
    ...
    | Inr x, imp -> if is_null_binder b || (!Options.print_real_names |> not && is_null_pp x.v)
                    ...
                    else imp_to_string ((strBvd x.v) ^ ":" ^ (typ_to_string x.sort) ) imp (* TODO *)
  ...
  match c.n with
    ...
    | Comp c ->
      let basic =
          ...
          else Util.format4 "%s (%s)\n%s%s" (sli c.effect_name) (typ_to_string c.result_typ) (get_ind ()) (c.effect_args |> List.map effect_arg_to_string |> String.concat ("\n" ^ get_ind ())) in 
      let dec = c.flags |> List.collect (function DECREASES e -> [Util.format1 " (decreases %s)" (exp_to_string e)] | _ -> []) |> String.concat " " in
      indent := String.length (ind) ;
      Util.format2 "%s%s" basic dec

and effect_arg_to_string e = match e with
    | Inr e, _ -> exp_to_string e
    | Inl wp, _ -> let r = Util.format2 "(%s %s)" (match !comp_arg_counter with
                                                  | 0 -> "requires"
                                                  | 1 -> "ensures "
                                                  | _ -> "unexpected")
                                                  (formula_to_string wp) in
                   comp_arg_counter := !comp_arg_counter + 1;
                   r
...
and exp_to_string x = 
  ...
  | Exp_abs(binders, e) -> indent := !indent + 2;
                           begin match (filter_imp binders) with 
                           | [] -> exp_to_string e 
                           | _ -> Util.format2 ("(fun %s ->\n" ^ get_ind () ^ "%s)") (binders_to_string " " binders) (e|> exp_to_string) end
  | Exp_app(e, args) ->
      ...
      | None ->
          ...
          if is_infix_prim_op e && List.length args' = 2 then
            Util.format3 ("(%s %s %s)") (List.nth args' 0 |> arg_to_string) 
                         (e|> infix_prim_op_to_string) (List.nth args' 1 |> arg_to_string)
          else if is_unary_prim_op e && List.length args' = 1 then
            Util.format2 "(%s %s)" (e|> unary_prim_op_to_string) 
                                   (List.nth args' 0 |> arg_to_string)
          else 
          begin
            indent := !indent + 2;
            Util.format3 ("(%s%s%s)") (e|> exp_to_string) (if arg_is_abs (List.hd args) then "\n" ^ (get_ind ()) else " ")  (args_to_string args)
          end)
  | Exp_match(e, pats) -> indent := !indent + 2; Util.format2 "(match %s with %s)"
    (e |> exp_to_string)
    ("\n" ^ (get_ind ()) ^ Util.concat_l ("\n" ^ (get_ind ())) (pats |> List.map (fun (p,wopt,e) -> Util.format3 "| %s %s -> %s"
      (p |> pat_to_string)
      (match wopt with | None -> "" | Some w -> Util.format1 "when %s" (w |> exp_to_string))
      (e |> exp_to_string))))
  | Exp_ascribed(e, t, _) -> (* Util.format2 "(%s:%s)" (e|> exp_to_string) (t |> typ_to_string) *) (*TODO*)
                             exp_to_string e
  | Exp_let(lbs, e) -> Util.format2 ("%s in\n" ^ ind ^ "%s")
    (let ind' = !indent in indent := !indent + 2; let r = lbs_to_string lbs in indent := ind'; r) 
    (e|> exp_to_string)
  in 
  indent := String.length ind;
  res
...
and lbs_to_string lbs =
    ...
    (Util.concat_l ("\nand ") (snd lbs |> List.map (fun lb -> Util.format2 "%s = %s" (lbname_to_string lb.lbname) (lb.lbdef |> exp_to_string)))))

and lbs_to_string_tl lbs =
    indent := !indent + 2;
    (Util.format2 "let %s%s"
    (if fst lbs then "rec " else "")
    (Util.concat_l ("\nand ") (snd lbs |> List.map (fun lb -> Util.format2 "%s =\n  %s" (lbname_to_string lb.lbname) (lb.lbdef |> exp_to_string)))))
...
and meta_to_string x = match x with
  ...
  | Meta_labeled(t, l, _, _) -> typ_to_string t
...
and pat_to_string x = match x.v with
  | Pat_cons(l, _, pats) -> Util.format2 "(%s %s)" (sli l.v) (List.map (fun (x, b) -> let p = pat_to_string x in if b then (* TODO "#"^p *) "" else p) pats |> String.concat " ")
...
let is_discriminator_projector qs = 
  list_exists (function  
               | Discriminator _ -> true
               | Projector _ -> skip_let := true; true
               | _ -> false) qs
...
let quals_to_string quals = 
  let res = quals |> List.map qual_to_string |> String.concat " " in
  if res = "" then res else res ^ " "
let rec sigelt_to_string x = 
  indent := 0;
  match x with
  | Sig_pragma(ResetOptions, _) -> "#reset-options"
  | Sig_pragma(SetOptions s, _) -> Util.format1 "#set-options \"%s\"" s
  | Sig_tycon(lid, tps, k, _, _, quals, _) -> Util.format4 "%stype %s %s : %s =" (quals_to_string quals) (sli lid) (binders_to_string " " tps) (kind_to_string k)
  | Sig_typ_abbrev(lid, tps, k, t, _, _) ->  Util.format4 "type %s %s : %s = %s" (sli lid) (binders_to_string " " tps) (kind_to_string k) (typ_to_string t)
  | Sig_datacon(lid, t, _, _, _, _) -> Util.format2 "| %s : %s" (sli lid) (typ_to_string t)
  | Sig_val_decl(lid, t, quals, _) ->
    if not (is_discriminator_projector quals) then 
      Util.format3 "%sval %s : %s" (quals_to_string quals) (sli lid) (typ_to_string t)
    else
      ""
                                    
  | Sig_assume(lid, f, _, _) -> Util.format2 "val %s : %s" lid.str (typ_to_string f)
  | Sig_let(lbs, _, _, b) -> if !skip_let then (skip_let := false; "") else  Util.format1 "%s\n" (lbs_to_string_tl lbs)
  | Sig_main(e, _) -> Util.format1 "let _ = %s\n" (exp_to_string e)
