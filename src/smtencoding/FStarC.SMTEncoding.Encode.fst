...
let primitive_type_axioms : env -> lident -> string -> term -> ML (list decl) =
   ...
   in
   let mk_with_type_axiom : env -> string -> term -> ML (list decl) = fun env with_type _tt ->
   ...
   in
   let mk_has_eq_refine : env -> string -> term -> ML (list decl) = fun env has_eq _has_eq_tt ->
      (* (assert (forall ((u Universe) (base Term) (refine (=> Term Bool)))
        ...
        [Util.mkAssume(mkForall' (Env.get_range env) 
                       ...
                       "has_eq_refine")]
   in
   let mk_forall_interp  : env -> string -> term -> ML (list decl) = fun env forall_lid _ ->
    let uu = mk_fv ("u", univ_sort) in
    let u = mkFreeV uu in
    let tt = mk_fv ("t", Term_sort) in
    let t = mkFreeV tt in
    ...
    [Util.mkAssume(mkForall' (Env.get_range env)
                      ...
                      "l_forall_interp")]
   in
   let mk_exists_interp  : env -> string -> term -> ML (list decl) = fun env exists_lid _ ->
    let uu = mk_fv ("u", univ_sort) in
    let u = mkFreeV uu in
    let tt = mk_fv ("t", Term_sort) in
    let t = mkFreeV tt in
