...
type prims_t = {
    mk:lident -> string -> ML (term & int & list decl);
    is:lident -> ML bool;
}
...
let prims =
    ...
    let prims : list (lident & (Range.t -> string -> ML (term & int & list decl))) = [
        //equality
    ...
    let mk : lident -> string -> ML (term & int & list decl) =
        fun l v ->
            let found = List.find (fun (l', _) -> lid_equals l l') prims in
            let (_, b) = Option.must found in
            b (Ident.range_of_lid l) v in
    let is : lident -> ML bool =
        fun l -> BU.for_some (fun (l', _) -> lid_equals l l') prims in
    {mk=mk;
     is=is}
...
let primitive_type_axioms : env -> lident -> string -> term -> ML (list decl) =
    ...
    let mk_unit : env -> string -> term -> ML (list decl) = fun env nm tt ->
    ...
    let mk_bool : env -> string -> term -> ML (list decl) = fun env nm tt ->
        let typing_pred = mk_HasType x tt in
    ...
    let mk_real : env -> string -> term -> ML (list decl) = fun env nm tt ->
        let typing_pred = mk_HasType x tt in
        ...
        [Util.mkAssume(mkForall
         ...
         Util.mkAssume(mkForall_fuel
                         ...
                         ([[typing_pred]],
                          ...
                          "real_inversion")]
    in
    let mk_str : env -> string -> term -> ML (list decl) = fun env nm tt ->
        let typing_pred = mk_HasType x tt in
    ...
    let mk_or_interp : env -> string -> term -> ML (list decl) = fun env disj _ ->
        let aa = mk_fv ("a", Term_sort) in
        let bb = mk_fv ("b", Term_sort) in
        let a = mkFreeV aa in
        let b = mkFreeV bb in
    ...
    let mk_eq2_interp : env -> string -> term -> ML (list decl) = fun env eq2 tt ->
        ...
        [Util.mkAssume(mkForall (Env.get_range env)
                       ...
                       "eq2-interp")]
    in
    let mk_eq3_interp : env -> string -> term -> ML (list decl) = fun env eq3 tt ->
        let uu = mk_fv ("u", univ_sort) in
        let aa = mk_fv ("a", Term_sort) in
        let bb = mk_fv ("b", Term_sort) in
        let xx = mk_fv ("x", Term_sort) in
        ...
        [Util.mkAssume(mkForall (Env.get_range env) ([[eq3_x_y]], [uu;aa;bb;xx;yy], mkIff(mkEq(x, y), valid)),
                       ...
                       "eq3-interp")]
    in
    let mk_imp_interp : env -> string -> term -> ML (list decl) = fun env imp tt ->
        let aa = mk_fv ("a", Term_sort) in
        let bb = mk_fv ("b", Term_sort) in
        let a = mkFreeV aa in
        let b = mkFreeV bb in
    ...
    let mk_iff_interp : env -> string -> term -> ML (list decl) = fun env iff tt ->
        let aa = mk_fv ("a", Term_sort) in
        let bb = mk_fv ("b", Term_sort) in
        let a = mkFreeV aa in
        let b = mkFreeV bb in
    ...
    let mk_not_interp : env -> string -> term -> ML (list decl) = fun env l_not tt ->
        let aa = mk_fv ("a", Term_sort) in
        let a = mkFreeV aa in
   ...
   in
   let mk_with_type_axiom : env -> string -> term -> ML (list decl) = fun env with_type tt ->
...
let encode_free_var uninterpreted env fv us tt t_norm quals : ML (decls_t & env_t) =
    ...
    let is_pure_or_ghost = U.is_pure_or_ghost_function t_norm in
    let is_reifiable = is_smt_reifiable_function env.tcenv t_norm in
    let is_lemma = U.is_lemma t_norm in
    if not <| (is_pure_or_ghost || is_reifiable)
    || is_lemma
    || uninterpreted
         ...
         then let _ = if List.length us <> 0 then failwith "Impossible: unexpected universe-polymorphic primitive function" in
              ...
              let should_thunk () =
                ...
                let is_squash t =
                    ...
                    | _ -> false
                in
                let nsstr_check = nsstr lid <> "Prims" in
                let no_univs = List.length us = 0 in
                let not_logic = not (quals |> List.contains Logic) in
                let not_squash = not (is_squash t_norm) in
                let not_type = not (is_type t_norm) in
                // Thunk if ...
                nsstr_check  //not in prims
                && no_univs //has no universe binders
                && not_logic //not logic qualified terms
                && not_squash //not ambient squashed properties
                && not_type //not : Type terms, since ambient typing hypotheses for these are cheap
              in
              let thunked, vars =
                 match vars with
                 | [] when should_thunk () ->
...
let declare_top_level_let env x us t t_norm : ML (fvar_binding & decls_t & env_t) =
...
//       * bool         //if set, we should generate a curried application of f
    ...
    try
      ...
      then encode_top_level_vals env bindings quals
            ...
            | [{lbunivs=uvs;lbdef=e;lbname=lbn}], [t_norm], [(fvb, _)] ->
                ...
                let eqns,decls2 =
                  ...
                  let basic_eqn, decls =
                    ...
                    if should_encode_logical
                    then (
                       let ext_enabled = Options.Ext.enabled "retain_old_prop_typing" in
                      if is_sub_singleton && not ext_enabled
                      then (
                        Util.mkAssume(mkForall (S.range_of_lbname lbn)
          ...
          let prefix_decls, elts, rest =
            ...
            decls |> List.flatten |> (fun decls ->
              ...
              let prefix_decls, elts, rest = List.fold_left (fun (prefix_decls, elts, rest) elt ->
                let has_key = elt.key |> Some? in
                let has_decl_fun = List.existsb isDeclFun elt.decls in
                if has_key && has_decl_fun
        ...
        let has_masked = quals |> BU.for_some (function HasMaskedEffect -> true | _ -> false) in
        let has_impure = typs  |> BU.for_some (fun t -> let is_pg = U.is_pure_or_ghost_function t in
                                                  let is_reif = is_smt_reifiable_function env.tcenv t in
                                                  not <| (is_pg || is_reif)) in
        if has_masked || has_impure
        ...
        List.fold_left
          (fun (out, decls) l ->
            ...
            let inversion_case, decls' =
              let compat_injectivity = Options.Ext.enabled "compat:injectivity" in
...
= let Sig_datacon {lid=d; us; t; num_ty_params=n_tps; mutuals; injective_type_params } = se.sigel in
  ...
  let injective_type_params =
    let compat = Options.Ext.enabled "compat:injectivity" in
    injective_type_params || compat
  in
  let univ_fields =
          univs
          |> List.mapi
          ...
          let codomain_prec_l, cod_decls =
            List.fold_left2
              (fun (codomain_prec_l, cod_decls) formal var ->
                  let rec binder_and_codomain_type t : ML (option (S.binders & S.comp)) =
                      ...
                      | Tm_arrow _ ->
                        ...
                        | _ ->
                          ...
                          then None //not useful for lemmas
                            ...
                            let t = norm t in
                            let is_t = is_type t in
                            let is_ss = U.is_sub_singleton t in
                            if is_t || is_ss
...
and encode_sigelt' (env:env_t) (se:sigelt) : ML (decls_t & env_t) =
    if !dbg_SMTEncoding
    ...
    match se.sigel with
     ...
     (* Irreducible and opaque lets. Replace the definitions by a dummy val decl (if none
        exists) and re-run. *)
     | Sig_let {lbs}
        when (let is_irred = se.sigquals |> List.contains S.Irreducible in
              let is_opaque = se.sigattrs |> BU.for_some is_opaque_to_smt in
              is_irred || is_opaque) ->
    ...
    | Sig_let {lids} when (let from_prims = lids |> BU.for_some (fun (l:lident) -> string_of_id (List.hd (ns_of_lid l)) = "Prims") in
                             let has_unfold = se.sigquals |> BU.for_some (function Unfold_for_unification_and_vcgen -> true | _ -> false) in
                             from_prims && has_unfold) ->
      ...
      let decls, elts, rest =
        List.fold_left
          (fun (decls, elts, rest) elt ->
             let has_key = Some? elt.key in
            let has_decl_fun = List.existsb (function | Term.DeclFun _ -> true | _ -> false) elt.decls in
            if has_key //NS: Not sure what this case is for
            && has_decl_fun
...
let encode_env_bindings (env:env_t) (bindings:list S.binding) : ML (decls_t & env_t) =
...
let place_decl_elt_in_fact_dbs (env:env_t) (fact_db_ids:list fact_db_id) (elt:decls_elt) : ML decls_elt =
 ...
 *)
let recover_caching_and_update_env (env:env_t) (decls:decls_t) : ML decls_t =
...
let encode_query use_env_msg (tcenv:Env.env) (q:S.term)
  ...
  Errors.with_ctx "While encoding a query" (fun () ->
    ...
    let env = get_env (Env.current_module tcenv) tcenv in
    let q, bindings =
        let rec aux bindings : ML _ = match bindings with
    ...
    let dbg_med = Debug.medium () in
    let dbg_smt = !dbg_SMTEncoding in
    if dbg_med || dbg_smt
    ...
    let caption =
      ...
      let log_q = Options.log_queries () in
      let log_fq = Options.log_failing_queries () in
      if log_q || log_fq
    ...
    let dbg_t = !dbg_Time in
    if dbg_med || dbg_smt || dbg_t
