         ...
         then let _ = if List.length us <> 0 then failwith "Impossible: unexpected universe-polymorphic primitive function" in
              ...
              let freshness =
                ...
                then [Term.fresh_constructor (S.range_of_fv fv) (vname, univ_sorts @ (vars |> List.map fv_sort), Term_sort, varops.next_id());
                      pretype_axiom (S.range_of_fv fv) env vapp (univ_fvs@vars)]
                else [] in
...
let declare_top_level_let env x us t t_norm =
...
let encode_top_level_val uninterpreted env lid us t quals =
...
//            (Print.term_to_string tt);
    let decls, env = encode_free_var uninterpreted env lid us t tt quals in
    if U.is_smt_lemma t
      ...
      then encode_top_level_vals env bindings quals
        ...
        let encode_rec_lbdefs (bindings:list<letbinding>)
          ...
          let encode_one_binding env0 (fvb, g, gtok) t_norm ({lbunivs=uvs;lbname=lbn; lbdef=e}) =
            ...
            let mk_g_app args =  maybe_curry_app rng (BU.Inl (Var g)) (fvb_with_univs_arity.smt_arity + 1) args in
            ...
            let aux_decls, g_typing =
              ...
              let tok_corr =
                ...
                let tot_fun_axioms =
                  ...
                  | [] -> tot_fun_axioms
                  | _ -> mkForall (U.range_of_lbname lbn) ([], univ_vars, tot_fun_axioms)
                in
                Util.mkAssume(mkAnd(mkForall (U.range_of_lbname lbn) ([[tok_app]], fuel::(univ_vars@vars), mkEq(tok_app, gapp)),
                                    tot_fun_axioms),
                          ...
                          List.mapi
                            (fun i u ->
                              let univ_projector =
                                ...
                                mkApp(mk_term_projector_name l bv, [xx])
                              in
                              mkEq(u, univ_projector))
                            univ_terms
                        in
                        univ_eqs
                    in
                    mkOr(out, mkAnd(mk_data_tester env l xx, (eqs @ univ_eqs) |> mk_and_l)), decls@decls') (mkFalse, []) in
                let ffsym, ff = fresh_fvar env.current_module_name "f" Fuel_sort in
