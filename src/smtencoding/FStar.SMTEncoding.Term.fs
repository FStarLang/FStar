...
let mkForallFlat (vars, body) =
    ...
    | Quant(Forall, pats, wopt, sorts, body') ->
      let closing = vars @ List.map (fun s -> "ignore", s, true) sorts in
...
let fresh_token (tok, univ_fvs, sort) id =
    ...
    let t = mkEq(mkInteger' id norng,
                 mkApp(constr_id_of_sort sort, [tok]) norng) norng
    in
    let t = mkForall norng ([[tok]], univ_fvs, t) in
