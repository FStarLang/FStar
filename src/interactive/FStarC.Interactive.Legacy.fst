...
// A custom version of the function that's in FStarC.Universal.fs just for the
...
let check_frag (env:TcEnv.env) curmod frag =
    ...
    with
        | FStarC.Errors.Error(e, msg, r, ctx) when not ((Options.trace_error())) ->
          FStarC.TypeChecker.Err.add_errors env [(e, msg, r, ctx)];
          None
...
let deps_of_our_file filename =
  ...
  let deps, dep_graph = FStarC.Dependencies.find_deps_if_needed [ filename ] FStarC.CheckedFiles.load_parsing_data_from_cache in
...
let rec go (line_col:(int&int))
  ...
  | Info(symbol, fqn_only, pos_opt) ->
    let info_at_pos_opt = match pos_opt with
      | None -> None
      | Some (file, row, col) -> FStarC.TypeChecker.Err.info_at_pos env file row col in
  ...
  | Completions search_term ->
    ...
    let rec measure_anchored_match
      ...
      = fun search_term candidate ->
          ...
          | _, [] -> None
          | hs :: ts, hc :: tc ->
            let hc_text = FStarC.Ident.string_of_id hc in
    ...
    let all_lidents_in_env = FStarC.TypeChecker.Env.lidents env in
    let matches =
        ...
        let matched_ids =
            ...
            | _ ->
              ...
              match FStarC.Syntax.DsEnv.resolve_module_name env.dsenv l true with
              | None ->
                case_b_find_matches_in_env ()
              | Some m ->
...
let interactive_mode (filename:string): unit =
  ...
  let env =
    ...
    | Some intf ->
        ...
        FStarC.Universal.load_interface_decls env intf
    | None ->
        env
  in
